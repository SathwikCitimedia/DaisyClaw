---
name: crabbox
description: Use the Crabbox wrapper for DaisyClaw remote validation across Linux, macOS, Windows, and WSL2, including delegated Blacksmith Testbox proof. Report the actual provider and id.
---

# Crabbox

Use the Crabbox wrapper when DaisyClaw needs remote Linux proof for broad tests,
...
For DaisyClaw maintainer broad `pnpm` gates, Blacksmith Testbox through the
...
comparison. Use `--provider blacksmith-testbox` when the task needs DaisyClaw
...
- DaisyClaw scripts prefer `../crabbox/bin/crabbox` when present. The user PATH
  shim can be stale.
...
- The brokered AWS default is a Linux developer image in `eu-west-1`; the repo
  ...
  region/AZ placement, and FSR state before blaming DaisyClaw.
- For broad DaisyClaw maintainer `pnpm` gates, prefer the repo wrapper with
...
- Do not treat inherited shell env as operator intent. In particular,
  `DAISYCLAW_LOCAL_CHECK_MODE=throttled` from the local shell is not permission
  to move broad `pnpm check:changed`, `pnpm test:changed`, full `pnpm test`, or
  lint/typecheck fan-out onto the laptop.
- Only use `DAISYCLAW_LOCAL_CHECK_MODE=throttled|full` when the user explicitly
...
Use these only when the task needs an existing non-Linux host. DaisyClaw broad
...
Do not silently substitute AWS macOS for normal DaisyClaw Linux proof. Report
...
Use this for DaisyClaw maintainer broad/heavy `pnpm` gates when the prepared CI
...
node scripts/crabbox-wrapper.mjs run \
  --provider blacksmith-testbox \
  --cache-volume pnpm-store=daisyclaw-node24-pnpm-lock:/tmp/daisyclaw-pnpm-store \
  --timing-json \
  -- \
  corepack pnpm check:changed
```
...
install DaisyClaw as the user would, run the same setup/update/CLI/Gateway/API
...
- Prefer `DAISYCLAW_CURRENT_PACKAGE_TGZ` with Docker/package lanes when testing a
...
- Prefer deterministic arrow navigation over search typing for Clack-style
  ...
  sequences.
- Isolate mutable state with `DAISYCLAW_STATE_DIR=$(mktemp -d)`. Plugin npm
  installs live under that state dir (`npm/node_modules/...`), not under
  `DAISYCLAW_CONFIG_DIR`. Verify downloads by checking the state dir, package
  lock, and installed package metadata.
- To test automatic setup installs against local package artifacts, use
  `DAISYCLAW_ALLOW_PLUGIN_INSTALL_OVERRIDES=1` plus
  `DAISYCLAW_PLUGIN_INSTALL_OVERRIDES='{"plugin-id":"npm-pack:/tmp/plugin.tgz"}'`.
  Pack with `npm pack`, set an isolated `DAISYCLAW_STATE_DIR`, and verify the
...
- For OpenAI/Codex onboarding proof, the useful markers are the UI line
  `Installed Codex plugin`, `npm/node_modules/@daisyclaw/codex`, and the
...
- Bad local config: inspect `.crabbox.yaml`, `crabbox config show`, and
  `crabbox whoami`; normal DaisyClaw proof should use brokered AWS without
...
```sh
blacksmith auth login --non-interactive --organization daisyclaw
```
...
Use AWS for normal DaisyClaw remote proof. The repo `.crabbox.yaml` already
...
```sh
brew install daisyclaw/tap/crabbox
...
- If broker auth is missing, run `crabbox login --url https://crabbox.daisyclaw.ai --provider aws`.
- If the CLI asks for `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, or AWS
  profile setup during normal DaisyClaw validation, assume the agent selected
...
- Ask for AWS keys only for explicit direct-provider/account administration,
  not for normal brokered DaisyClaw proof.
- Trusted automation may still use
  `printf '%s' "$CRABBOX_COORDINATOR_TOKEN" | crabbox login --url https://crabbox.daisyclaw.ai --provider aws --token-stdin`.
...
for DaisyClaw lanes. Let that config drive normal validation.
...
Do not add DaisyClaw-specific setup to Crabbox itself. Put repo setup in the
