---
name: daisyclaw-landable-bug-sweep
description: "Find or repair small high-confidence non-SDK-boundary DaisyClaw bugfix PRs until five are landable."
---

# DaisyClaw Landable Bug Sweep

Autonomous maintainer workflow for producing five landable DaisyClaw bugfix PR URLs.
...
Use `$gitcrawl` for discovery/clustering, `$daisyclaw-pr-maintainer` for live GitHub mutation rules, `$github-author-context` when contributor trust matters, `$daisyclaw-testing` for proof choice, `$autoreview` before publishing/landing, and `$crabbox` for broad/E2E/live proof.
