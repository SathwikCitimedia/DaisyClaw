---
name: daisyclaw-test-performance
description: Benchmark, diagnose, and optimize DaisyClaw test and plugin-suite runtime, import hotspots, CPU/RSS, heap growth, and slow coverage paths.
---

# DaisyClaw Test Performance
...
test:extensions:batch <plugin[,plugin...]>` or plugin-inspector command
   ...
   - For import-heavy suspicion add:
     `DAISYCLAW_VITEST_IMPORT_DURATIONS=1 DAISYCLAW_VITEST_PRINT_IMPORT_BREAKDOWN=1`.
...
4. For broad or package-heavy plugin proof, use Crabbox-backed Blacksmith
   ...
   - `pnpm crabbox:run -- --provider blacksmith-testbox --timing-json -- DAISYCLAW_TESTBOX=1 pnpm test:extensions:batch <ids>`
...
5. If plugin performance is package-artifact sensitive, switch to
   `release-daisyclaw-plugin-testing` and Package Acceptance rather than
...
| import duration | broad barrel/runtime loads         | `DAISYCLAW_VITEST_IMPORT_DURATIONS=1`                                        |
...
| heap snapshots  | real leak vs retained module graph | `daisyclaw-test-heap-leaks` workflow                                         |
...
```bash
DAISYCLAW_TEST_MEMORY_TRACE=1 \
DAISYCLAW_TEST_HEAPSNAPSHOT_INTERVAL_MS=60000 \
DAISYCLAW_TEST_HEAPSNAPSHOT_DIR=.tmp/heapsnap \
DAISYCLAW_TEST_WORKERS=2 \
DAISYCLAW_TEST_MAX_OLD_SPACE_SIZE_MB=6144 \
pnpm test
```

Use `daisyclaw-test-heap-leaks` when RSS keeps growing across intervals, workers
...
- Parallel Vitest runs sharing `node_modules/.experimental-vitest-cache` without
  distinct `DAISYCLAW_VITEST_FS_MODULE_CACHE_PATH` values.
...
timeout 240 /usr/bin/time -l env \
  DAISYCLAW_VITEST_IMPORT_DURATIONS=1 \
  DAISYCLAW_VITEST_PRINT_IMPORT_BREAKDOWN=1 \
