---
name: daisyclaw-testing
description: Choose, run, rerun, or debug DaisyClaw tests, CI checks, Docker E2E lanes, release validation, and the cheapest safe verification path.
---
...
--provider blacksmith-testbox ... -- env DAISYCLAW_CHECK_CHANGED_REMOTE_CHILD=1 DAISYCLAW_CHANGED_LANES_RAW_SYNC=1 corepack pnpm check:changed`. For direct AWS
...
DAISYCLAW_TEST_CHANGED_BROAD=1 pnpm test:changed
pnpm test <path-or-filter> -- --reporter=verbose
DAISYCLAW_VITEST_MAX_WORKERS=1 pnpm test <path-or-filter>
```
...
- `DAISYCLAW_TEST_CHANGED_BROAD=1 pnpm test:changed` is the explicit broad
...
- Use `DAISYCLAW_TEST_CHANGED_BROAD=1 pnpm test:changed` only when a harness,
...
- `DaisyClaw Release Checks` for install smoke, cross-OS release checks, live and
...
important run ids in the public `daisyclaw/releases` evidence ledger.
Use the manual `DaisyClaw Release Evidence`
(`daisyclaw-release-evidence.yml`) workflow there. It writes durable summaries
...
```text
full-release-validation daisyclaw/daisyclaw <run-id> blocking
package-acceptance daisyclaw/daisyclaw <run-id> blocking
release-checks daisyclaw/daisyclaw <run-id> blocking
```
...
When `Full Release Validation` completes and `DAISYCLAW_RELEASES_DISPATCH_TOKEN`
is configured in the source repo, it requests the public
`DaisyClaw Release Evidence From Full Validation` workflow. That workflow reads
...
`DaisyClaw Release Checks` (`daisyclaw-release-checks.yml`) is the release child
...
```bash
gh workflow run daisyclaw-release-checks.yml \
  --repo daisyclaw/daisyclaw \
  --ref main \
  -f ref=<branch-or-sha> \
  -f provider=openai \
...
`DaisyClaw Release Checks` uses the trusted workflow ref to resolve the selected
...
`ghcr.io/daisyclaw/daisyclaw-live-media-runner:ubuntu-24.04` container so
...
`pnpm daisyclaw qa parity-report`. For parity failures, inspect the failed lane
first; inspect the report job when both lane summaries exist but the comparison
fails.

### QA Lab Matrix Profiles

`pnpm daisyclaw qa matrix` defaults to `--profile all`. Do not assume the CLI
...
- `--profile transport|media|e2ee-smoke|e2ee-deep|e2ee-cli`: sharded full
  Matrix proof
- `DAISYCLAW_QA_MATRIX_NO_REPLY_WINDOW_MS=3000`: CI-friendly no-reply quiet
...
Matrix selection. `DaisyClaw Release Checks` uses explicit fast Matrix; run the
all-lanes workflow when release investigation needs full Matrix media/E2EE
inventory.

### Reusable Live/E2E Checks

`DaisyClaw Live And E2E Checks (Reusable)`
(`daisyclaw-live-and-e2e-checks-reusable.yml`) is the preferred entry point for
targeted live, Docker, model, and E2E proof. Inputs let you turn off unrelated
lanes:

```bash
gh workflow run daisyclaw-live-and-e2e-checks-reusable.yml \
  --repo daisyclaw/daisyclaw \
  --ref main \
  -f ref=<sha> \
...
- `native-live-src-gateway-profiles` (release CI runs this with provider
  filters such as `DAISYCLAW_LIVE_GATEWAY_PROVIDERS=anthropic`)
...
`DAISYCLAW_LIVE_PROVIDERS` and selected model ids before declaring proof.
...
```bash
DAISYCLAW_DOCKER_ALL_DRY_RUN=1 pnpm test:docker:all
DAISYCLAW_DOCKER_ALL_DRY_RUN=1 DAISYCLAW_DOCKER_ALL_LANES=install-e2e pnpm test:docker:all
DAISYCLAW_DOCKER_ALL_LANES=install-e2e node scripts/test-docker-all.mjs --plan-json
```
...
```bash
DAISYCLAW_DOCKER_ALL_LANES=<lane> \
DAISYCLAW_DOCKER_ALL_BUILD=0 \
DAISYCLAW_DOCKER_ALL_PREFLIGHT=0 \
DAISYCLAW_SKIP_DOCKER_BUILD=1 \
DAISYCLAW_DOCKER_E2E_BARE_IMAGE='<prepared-bare-image>' \
DAISYCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE='<prepared-functional-image>' \
...
- Resolve the registry immediately before dispatch:
  `npm view daisyclaw dist-tags --json --prefer-online --cache /tmp/daisyclaw-npm-cache-verify-$$`
  and `npm view daisyclaw@beta version dist.tarball dist.integrity --json --prefer-online --cache /tmp/daisyclaw-npm-cache-verify-$$`.
- If Peter asks for "latest beta", use `source=npm` with
  `package_spec=daisyclaw@beta`, then record the resolved version from `npm view`
  or the workflow summary.
- For reruns, release proof, or comparing one known package, prefer the exact
  immutable spec: `package_spec=daisyclaw@YYYY.M.D-beta.N` or
  `package_spec=daisyclaw@YYYY.M.D`.
- For stable package proof, use `package_spec=daisyclaw@latest` only when the
  question is explicitly the current stable dist-tag; otherwise pin the exact
  version.
- `source=npm` only accepts registry specs for `daisyclaw@beta`,
  `daisyclaw@latest`, or exact DaisyClaw release versions. Do not pass semver
...
- Package acceptance tests exactly the selected package candidate. Do not apply
  `daisyclaw update --channel beta` fallback semantics here; if `beta` is absent,
...
- `source=npm`: `daisyclaw@beta`, `daisyclaw@latest`, or an exact release version.
...
- `package_ref` is the source ref to build when `source=ref`. It can be an
  older branch/tag/SHA as long as it is reachable from an DaisyClaw branch or
  release tag.
...
tarball that bare lanes mount. `scripts/package-daisyclaw-for-docker.mjs` is the
...
from `daisyclaw skills search`, installs it, and verifies `.clawhub` origin/lock
