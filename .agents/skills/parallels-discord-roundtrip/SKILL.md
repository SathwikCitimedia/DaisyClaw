...
```bash
export DAISYCLAW_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.daisyclaw/daisyclaw.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env DAISYCLAW_PARALLELS_DISCORD_TOKEN \
...
- Use the `daisyclaw` wrapper for guest `message send/read`; `node daisyclaw.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/daisyclaw-parallels-smoke.*`
...
- host inbound nonce appears in `daisyclaw message read` output
