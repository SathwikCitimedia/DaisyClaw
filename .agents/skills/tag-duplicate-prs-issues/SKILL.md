...
description: Use gitcrawl to search duplicate DaisyClaw PRs/issues, group related work in prtags, and sync duplicate state to GitHub.
---
...
Use `gitcrawl` first because it is the local DaisyClaw history and clustering source.
...
gitcrawl neighbors daisyclaw/daisyclaw --number <issue-or-pr-number> --limit 20 --json
```

Then search key phrases and subsystem terms:

```bash
gitcrawl search daisyclaw/daisyclaw --query "<key phrase from title or body>" --mode hybrid --limit 20 --json
gitcrawl search daisyclaw/daisyclaw --query "<subsystem or error phrase>" --mode hybrid --limit 20 --json
```

Inspect likely clusters:

```bash
gitcrawl cluster-detail daisyclaw/daisyclaw --id <cluster-id> --member-limit 20 --body-chars 280 --json
```
...
```bash
gh search prs --repo daisyclaw/daisyclaw --match title,body --limit 50 -- "<key phrase>"
gh search issues --repo daisyclaw/daisyclaw --match title,body --limit 50 -- "<key phrase>"
gh search issues --repo daisyclaw/daisyclaw --match comments --limit 50 -- "<error or maintainer phrase>"
```
...
```bash
prtags search text -R daisyclaw/daisyclaw "<problem phrase>" --types group --limit 10
prtags search similar -R daisyclaw/daisyclaw "<problem summary>" --types group --limit 10
prtags group list -R daisyclaw/daisyclaw
```
...
```bash
prtags group create -R daisyclaw/daisyclaw \
  --kind mixed \
...
```bash
prtags field ensure -R daisyclaw/daisyclaw --name duplicate_status --scope pull_request --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R daisyclaw/daisyclaw --name duplicate_status --scope issue --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R daisyclaw/daisyclaw --name duplicate_confidence --scope pull_request --type enum --enum-values low,medium,high --filterable
prtags field ensure -R daisyclaw/daisyclaw --name duplicate_confidence --scope issue --type enum --enum-values low,medium,high --filterable
prtags field ensure -R daisyclaw/daisyclaw --name duplicate_rationale --scope pull_request --type text --searchable
prtags field ensure -R daisyclaw/daisyclaw --name duplicate_rationale --scope issue --type text --searchable
```

Recommended group-level fields:

```bash
prtags field ensure -R daisyclaw/daisyclaw --name duplicate_confidence --scope group --type enum --enum-values low,medium,high --filterable
prtags field ensure -R daisyclaw/daisyclaw --name duplicate_rationale --scope group --type text --searchable
prtags field ensure -R daisyclaw/daisyclaw --name cluster_summary --scope group --type text --searchable
```
...
```bash
prtags annotation pr set -R daisyclaw/daisyclaw <pr-number> \
...
```bash
prtags annotation issue set -R daisyclaw/daisyclaw <issue-number> \
  duplicate_status=confirmed \
  duplicate_confidence=high \
...
```bash
prtags group list-comment-sync-targets -R daisyclaw/daisyclaw
```
