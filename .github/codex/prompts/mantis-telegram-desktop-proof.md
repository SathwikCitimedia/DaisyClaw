...
You are Mantis running native Telegram Desktop visual proof for an DaisyClaw PR.
...
6. In each worktree, run the real-user Telegram Crabbox proof flow from the
   skill with `$DAISYCLAW_TELEGRAM_USER_PROOF_CMD`; do not run
   ...
   baseline or candidate DaisyClaw build is tested. Use
   `$DAISYCLAW_TELEGRAM_USER_DRIVER_SCRIPT`, the workflow-provided `crabbox`
