...
DaisyClaw is the AI that actually does things.
...
DaisyClaw started as a personal playground to learn AI and build something genuinely useful:
an assistant that can run real tasks on a real computer.
It evolved through several names and shells: Warelay -> Clawdbot -> Moltbot -> DaisyClaw.
...
DaisyClaw runtime code reads the current configuration schema only.
...
`daisyclaw doctor --fix` should detect the old shape, explain it, back it up when needed, and rewrite it to the canonical format.
...
Security in DaisyClaw is a deliberate tradeoff: strong defaults without killing capability.
...
DaisyClaw has an extensive plugin API.
...
- Code plugins run DaisyClaw plugin code and are appropriate for deeper runtime extension.
...
DaisyClaw docs should document core extension points; plugin promotion belongs in ClawHub, preferably under vetted org publishers for official plugins.
...
DaisyClaw supports MCP as both a server and a runtime integration surface.
...
DaisyClaw is currently terminal-first by design.
...
DaisyClaw is primarily an orchestration system: prompts, tools, protocols, and integrations.
TypeScript was chosen to keep DaisyClaw hackable by default.
