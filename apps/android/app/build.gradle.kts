...
val androidStoreFile = providers.gradleProperty("DAISYCLAW_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("DAISYCLAW_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("DAISYCLAW_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("DAISYCLAW_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
  error(
    "Missing Android release signing properties. Set DAISYCLAW_ANDROID_STORE_FILE, " +
      "DAISYCLAW_ANDROID_STORE_PASSWORD, DAISYCLAW_ANDROID_KEY_ALIAS, and " +
      "DAISYCLAW_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
  )
}
...
android {
  ...
  sourceSets {
    getByName("main") {
      assets.directories.add("../../shared/DaisyClawKit/Sources/DaisyClawKit/Resources")
    }
  }

  defaultConfig {
    applicationId = "ai.daisyclaw.app"
    minSdk = 31
    targetSdk = 36
  ...
  onVariants { variant ->
    variant.outputs
      ...
      .forEach { output ->
        ...
        val outputFileName =
          if (flavorName == null) {
            "daisyclaw-$versionName-$buildType.apk"
          } else {
            "daisyclaw-$versionName-$flavorName-$buildType.apk"
          }
