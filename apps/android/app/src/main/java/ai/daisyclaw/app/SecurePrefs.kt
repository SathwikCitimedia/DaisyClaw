...
class SecurePrefs(
  ...
  companion object {
    val defaultWakeWords: List<String> = listOf("daisyclaw", "claude")
    private const val displayNameKey = "node.displayName"
    private const val locationModeKey = "location.enabledMode"
    private const val voiceWakeModeKey = "voiceWake.mode"
    private const val plainPrefsName = "daisyclaw.node"
    private const val securePrefsName = "daisyclaw.node.secure"
  ...
  internal fun getNotificationForwardingPolicy(appPackageName: String): NotificationForwardingPolicy {
    ...
    // Always block DaisyClaw's own notifications in blocklist mode to prevent forwarding loops.
