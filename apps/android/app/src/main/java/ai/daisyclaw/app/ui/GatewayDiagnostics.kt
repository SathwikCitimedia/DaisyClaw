...
internal fun daisyClawAndroidVersionLabel(): String {
...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this DaisyClaw Android gateway connection failure.
    ...
    - tell me whether `daisyclaw devices list` should show a pending pairing request
    - if more signal is needed, ask for `daisyclaw qr --json`, `daisyclaw devices list`, and `daisyclaw nodes status`
    - give the next exact command or tap

    Debug info:
    - screen: $screen
    - app version: ${daisyClawAndroidVersionLabel()}
    - device: $device
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("DaisyClaw gateway diagnostics", report))
