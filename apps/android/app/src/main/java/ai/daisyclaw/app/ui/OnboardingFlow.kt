...
import ai.daisyclaw.app.GatewayConnectionProblem
import ai.daisyclaw.app.LocationMode
import ai.daisyclaw.app.MainViewModel
import ai.daisyclaw.app.R
import ai.daisyclaw.app.SensitiveFeatureConfig
import ai.daisyclaw.app.node.DeviceNotificationListenerService
import ai.daisyclaw.app.ui.design.ClawDesignTheme
import ai.daisyclaw.app.ui.design.ClawErrorState
import ai.daisyclaw.app.ui.design.ClawListItem
...
fun OnboardingFlow(
  ...
  ClawDesignTheme(dark = onboardingDark) {
    ...
    DaisyClawSystemBarAppearance(lightAppearance = !onboardingDark && step != OnboardingStep.Welcome)
...
private fun WelcomeLogo() {
  Surface(
    ...
    Box(modifier = Modifier.fillMaxSize().padding(12.dp), contentAlignment = Alignment.Center) {
      Image(painter = painterResource(id = R.drawable.daisyclaw_logo), contentDescription = "DaisyClaw logo", modifier = Modifier.fillMaxSize())
    }
  }
}
      ...
      LazyColumn(
        ...
        item {
          Column(modifier = Modifier.padding(top = 10.dp, bottom = 18.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
            ...
            Text(
              text = "These permissions keep DaisyClaw secure\nand useful.",
              style = ClawTheme.type.body,
              color = ClawTheme.colors.textMuted,
            )
          }
...
private fun PermissionTopBar(onBack: () -> Unit) {
  ...
  if (showHelp) {
    AlertDialog(
      ...
      text = {
        Text(
          "Choose what this phone can share with DaisyClaw. You can change these later in Settings.",
          style = ClawTheme.type.body,
          color = ClawTheme.colors.textMuted,
        )
      },
...
internal enum class GatewayRecoveryUiState(
  ...
  Pairing(
    title = "Pairing Gateway",
    message = "Approval is in progress.\nDaisyClaw will reconnect automatically.",
  ),
  Finishing(
    title = "Connecting Gateway",
    message = "DaisyClaw is checking gateway and node access.",
  ),
  Failed(
  ...
  remoteAddress
    ...
    ?: if (ready) {
      ...
      recoveryGatewayApprovalCommand(gatewayConnectionProblem)
        ...
        ?: "Gateway approval is pending. Run daisyclaw devices list on the gateway host, approve this phone, then retry."
      ...
      "Gateway approval is in progress. DaisyClaw will retry automatically."
    } else {
      "Gateway unreachable"
    }
...
private fun copyApprovalCommand(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("DaisyClaw pairing approval command", command))
...
private fun copyGatewayDiagnostic(
  ...
  val diagnostic =
    listOfNotNull(
      "DaisyClaw Android gateway diagnostic",
      "Status: $statusText",
  ...
  val clipboard = context.getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager
  clipboard.setPrimaryClip(ClipData.newPlainText("DaisyClaw gateway diagnostic", diagnostic))
