...
class InvokeDispatcherTest {
    ...
    runTest {
      val result =
        newDispatcher(
          ...
          smsFeatureEnabled = true,
          smsTelephonyAvailable = true,
        ).handleInvoke(DaisyClawSmsCommand.Search.rawValue, "not-json")
    ...
    runTest {
      val result =
        newDispatcher(
          ...
          smsFeatureEnabled = true,
          smsTelephonyAvailable = true,
        ).handleInvoke(DaisyClawSmsCommand.Send.rawValue, """{"to":"+15551234567","message":"hi"}""")
  ...
  fun handleInvoke_blocksCameraCommandsWhenCameraDisabled() =
    runTest {
      val result = newDispatcher(cameraEnabled = false).handleInvoke(DaisyClawCameraCommand.List.rawValue, null)
  ...
  fun handleInvoke_blocksLocationCommandWhenLocationDisabled() =
    runTest {
      val result = newDispatcher(locationEnabled = false).handleInvoke(DaisyClawLocationCommand.Get.rawValue, null)
    ...
    runTest {
      val result =
        newDispatcher(installedAppsSharingEnabled = false)
          .handleInvoke(DaisyClawDeviceCommand.Apps.rawValue, """{"limit":1}""")
    ...
    runTest {
      val result =
        newDispatcher(motionActivityAvailable = false)
          .handleInvoke(DaisyClawMotionCommand.Activity.rawValue, null)
    ...
    runTest {
      val result =
        newDispatcher(motionPedometerAvailable = false)
          .handleInvoke(DaisyClawMotionCommand.Pedometer.rawValue, null)
  ...
  fun handleInvoke_blocksCallLogWhenUnavailable() =
    runTest {
      val result =
        newDispatcher(callLogAvailable = false).handleInvoke(DaisyClawCallLogCommand.Search.rawValue, null)
  ...
  fun handleInvoke_blocksPhotosWhenUnavailable() =
    runTest {
      val result = newDispatcher(photosAvailable = false).handleInvoke(DaisyClawPhotosCommand.Latest.rawValue, null)
  ...
  fun handleInvoke_routesTalkPttCommands() =
    runTest {
      ...
      val start = dispatcher.handleInvoke(DaisyClawTalkCommand.PttStart.rawValue, null)
      val stop = dispatcher.handleInvoke(DaisyClawTalkCommand.PttStop.rawValue, null)
      val cancel = dispatcher.handleInvoke(DaisyClawTalkCommand.PttCancel.rawValue, null)
      val once = dispatcher.handleInvoke(DaisyClawTalkCommand.PttOnce.rawValue, null)
