...
PACKAGE_NAME="ai.daisyclaw.app"
RECEIVER="$PACKAGE_NAME/.VoiceE2eReceiver"
RUN_ACTION="ai.daisyclaw.app.debug.RUN_VOICE_E2E"
OPEN_ACTION="ai.daisyclaw.app.debug.OPEN_VOICE_E2E"
PORT=18789
HOST="127.0.0.1"
MODE="both"
...
ARTIFACT_DIR="/tmp/daisyclaw-android-voice-e2e-$(date +%Y%m%d-%H%M%S)"
...
if [[ "$START_GATEWAY" -eq 1 ]]; then
  if command -v bws_get_secret >/dev/null 2>&1; then
    DAISYCLAW_OPENAI_API_KEY="$(bws_get_secret DAISYCLAW_OPENAI_API_KEY)"
  else
    DAISYCLAW_OPENAI_API_KEY="$(zsh -ic 'bws_get_secret DAISYCLAW_OPENAI_API_KEY')"
  fi
  (
    cd "$ROOT_DIR"
    OPENAI_API_KEY="$DAISYCLAW_OPENAI_API_KEY" \
      pnpm daisyclaw gateway run \
        --port "$PORT" \
        --auth none \
        --bind loopback \
        --force \
  ...
  unset DAISYCLAW_OPENAI_API_KEY
fi
...
adb logcat -d -v time |
  rg -i 'DaisyClaw|TalkMode|MicCapture|AudioRecord|SpeechRecognizer|realtime|talk.session|appendAudio|transcript|Talk failed|Transcription failed|Speech network|VoiceE2E' |
