# DaisyClaw iOS (Super Alpha)

This iOS app is super-alpha and internal-use only. The first public App Store release targets iPhone and connects to an DaisyClaw Gateway as a `role: node`.
...
- Beta release uses canonical `ai.daisyclaw.client*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/BetaRelease.xcconfig`.
- Beta release also switches the app to `DaisyClawPushTransport=relay`, `DaisyClawPushDistribution=official`, and `DaisyClawPushAPNsEnvironment=production`.
...
- Beta builds default to `https://ios-push-relay.daisyclaw.ai`.
- Optional custom relay override: `DAISYCLAW_PUSH_RELAY_BASE_URL=https://relay.example.com`
...
3. Optional: set a custom official/TestFlight relay URL for the build. If unset, the beta flow uses `https://ios-push-relay.daisyclaw.ai`.

```bash
export DAISYCLAW_PUSH_RELAY_BASE_URL=https://relay.example.com
```
...
7. Expected outputs after a successful run:
   - `apps/ios/build/beta/DaisyClaw-<version>.ipa`
   - `apps/ios/build/beta/DaisyClaw-<version>.app.dSYM.zip`
...
- `apps/ios/Sources/DaisyClaw.entitlements` sets `aps-environment` to `development`.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `DaisyClawPushTransport=direct` and `DaisyClawPushDistribution=local`.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- The gateway host also needs direct APNs auth configured separately with `DAISYCLAW_APNS_TEAM_ID`, `DAISYCLAW_APNS_KEY_ID`, and either `DAISYCLAW_APNS_PRIVATE_KEY_P8` or `DAISYCLAW_APNS_PRIVATE_KEY_PATH`.
- Recommended gateway-host storage for the APNs `.p8` file is `~/.daisyclaw/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `DAISYCLAW_APNS_PRIVATE_KEY_PATH` at that file.
...
- Debug builds default to `DaisyClawPushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `daisyclaw.json`. `DAISYCLAW_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine DaisyClaw official builds and to ensure a
...
The iOS app is not a Codex Computer Use backend. Computer Use and `cua-driver mcp` are macOS desktop-control paths; iOS exposes device capabilities as DaisyClaw node commands through the gateway. Agents can drive the iPhone canvas, camera, screen, location, voice, and other node capabilities with `node.invoke`, subject to iOS foreground/background limits.
