...
struct AppleReviewDemoChatTransport: DaisyClawChatTransport {
...
private actor AppleReviewDemoChatStore {
    ...
    private var messages: [DaisyClawChatMessage]

    init() {
        self.messages = AppleReviewDemoChatStore.seedMessages()
    }

    func createSession(key: String) throws -> DaisyClawChatCreateSessionResponse {
        try Self.decode(
            CreateSessionPayload(ok: true, key: key, sessionId: "apple-review-demo-\(key)"),
            as: DaisyClawChatCreateSessionResponse.self)
    }

    func history(sessionKey: String) throws -> DaisyClawChatHistoryPayload {
        ...
        return try Self.decode(
            HistoryPayload(
                ...
                thinkingLevel: "auto"),
            as: DaisyClawChatHistoryPayload.self)
    }

    func sendMessage(sessionKey _: String, message: String, runId: String) throws -> DaisyClawChatSendResponse {
        ...
        return try Self.decode(
            ...
            as: DaisyClawChatSendResponse.self)
    }

    func sessions() throws -> DaisyClawChatSessionsListResponse {
        let entry = DaisyClawChatSessionEntry(
            key: self.sessionKey,
            kind: "chat",
    ...
    private static func seedMessages() -> [DaisyClawChatMessage] {
        let now = Date().timeIntervalSince1970 * 1000
        return [
            self.message(
                role: "assistant",
    ...
    private static func message(role: String, text: String, timestamp: Double) -> DaisyClawChatMessage {
    ...
    private struct HistoryPayload: Encodable {
        ...
        var messages: [DaisyClawChatMessage]?
