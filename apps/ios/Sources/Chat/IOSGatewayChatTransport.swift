...
struct IOSGatewayChatTransport: DaisyClawChatTransport {
    static let logger = Logger(subsystem: "ai.daisyclaw", category: "ios.chat.transport")
    ...
    private struct ChatSendParams: Codable {
        ...
        var message: String
        var thinking: String
        var attachments: [DaisyClawChatAttachmentPayload]?
        var timeoutMs: Int
    ...
    static func makeChatSendParamsJSON(
        ...
        attachments: [DaisyClawChatAttachmentPayload]) throws -> String
    {
        ...
        parentSessionKey: String?) async throws -> DaisyClawChatCreateSessionResponse
    {
        ...
        return try JSONDecoder().decode(DaisyClawChatCreateSessionResponse.self, from: res)
    }
    ...
    func listSessions(limit: Int?) async throws -> DaisyClawChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(DaisyClawChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> DaisyClawChatHistoryPayload {
        ...
        return try JSONDecoder().decode(DaisyClawChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [DaisyClawChatAttachmentPayload]) async throws -> DaisyClawChatSendResponse
    {
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(DaisyClawChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(DaisyClawGatewayHealthOK.self, from: res))?.ok ?? true
    }
    ...
    static func mapEventFrame(_ evt: EventFrame) -> DaisyClawChatTransportEvent? {
        switch evt.event {
        case "tick":
            return .tick
        case "seqGap":
        ...
        case "health":
            ...
            let ok = (try? GatewayPayloadDecoding.decode(
                payload,
                as: DaisyClawGatewayHealthOK.self))?.ok ?? true
            return .health(ok: ok)
        case "chat":
            guard let payload = evt.payload else { return nil }
        ...
        case "session.message":
            ...
            guard let message = try? GatewayPayloadDecoding.decode(
                payload,
                as: DaisyClawSessionMessageEventPayload.self)
            else {
                return nil
            }
        ...
        case "agent":
            ...
            guard let agentPayload = try? GatewayPayloadDecoding.decode(
                payload,
                as: DaisyClawAgentEventPayload.self)
            else {
                return nil
            }
