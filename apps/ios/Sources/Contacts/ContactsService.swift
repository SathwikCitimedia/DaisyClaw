...
final class ContactsService: ContactsServicing {
    ...
    func search(params: DaisyClawContactsSearchParams) async throws -> DaisyClawContactsSearchPayload {
        ...
        return DaisyClawContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: DaisyClawContactsAddParams) async throws -> DaisyClawContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return DaisyClawContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return DaisyClawContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> DaisyClawContactPayload {
        DaisyClawContactPayload(
