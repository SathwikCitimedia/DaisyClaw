...
@MainActor
enum DaisyClawAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class DaisyClawAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.daisyclaw.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.daisyclaw.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.daisyclaw.ios.bgrefresh"
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? DaisyClawAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    static let typeKey = "daisyclaw.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "daisyclaw.watch.promptId"
    static let sessionKeyKey = "daisyclaw.watch.sessionKey"
    static let actionPrimaryIDKey = "daisyclaw.watch.action.primary.id"
    static let actionPrimaryLabelKey = "daisyclaw.watch.action.primary.label"
    static let actionSecondaryIDKey = "daisyclaw.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "daisyclaw.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "daisyclaw.watch.action.primary"
    static let actionSecondaryIdentifier = "daisyclaw.watch.action.secondary"
    static let actionIdentifierPrefix = "daisyclaw.watch.action."
    static let actionIDKeyPrefix = "daisyclaw.watch.action.id."
    static let actionLabelKeyPrefix = "daisyclaw.watch.action.label."
    static let categoryPrefix = "daisyclaw.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: DaisyClawWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> DaisyClawWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return DaisyClawWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "DaisyClaw" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [DaisyClawWatchAction]) -> [UNNotificationAction] {
...
struct DaisyClawApp: App {
    ...
    init() {
        ...
        DaisyClawAppModelRegistry.appModel = appModel
...
extension DaisyClawApp {
    ...
    private static func installUncaughtExceptionLogger() {
        NSLog("DaisyClaw: installing uncaught exception handler")
