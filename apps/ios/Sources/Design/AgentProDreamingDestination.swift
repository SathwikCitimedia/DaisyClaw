...
struct AgentProDreamingDestination: View {
            ...
            ProCard(padding: 0) {
                if let diary = self.overview?.dreamDiary {
                    if diary.found, let content = self.normalizedMultiline(diary.content) {
                        ...
                        VStack(alignment: .leading, spacing: 12) {
                            HStack {
                                ProIconBadge(systemName: "book.pages", color: DaisyClawBrand.accent)
    ...
    private func dreamingEntryRow(_ entry: DreamingEntryLite) -> some View {
        HStack(alignment: .top, spacing: 12) {
            ProIconBadge(systemName: "text.page", color: DaisyClawBrand.accent)
    ...
    private func dreamingPhaseRow(_ phase: DreamingPhaseRow) -> some View {
        HStack(alignment: .top, spacing: 12) {
            ProIconBadge(
                ...
                color: phase.status.enabled == false ? .secondary : DaisyClawBrand.accent)
            VStack(alignment: .leading, spacing: 4) {
                Text(phase.title)
                    .font(.subheadline.weight(.semibold))
            ...
            Text(self.dreamingPhaseState(phase.status))
                .font(.caption2.weight(.semibold))
                .foregroundStyle(phase.status.managedCronPresent == true ? DaisyClawBrand.accent : .secondary)
                .lineLimit(1)
        }
        .padding(.vertical, 10)
        .padding(.horizontal, 14)
    ...
    private static func dreamDiaryInnerContent(_ content: String) -> String {
        let start = "<!-- daisyclaw:dreaming:diary:start -->"
        let end = "<!-- daisyclaw:dreaming:diary:end -->"
