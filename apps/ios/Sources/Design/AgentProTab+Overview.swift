...
extension AgentProTab {
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 10) {
                self.metricTile(
                    ...
                    color: self.gatewayConnected ? DaisyClawBrand.accent : .secondary,
                    route: .skills)
                self.metricTile(
    ...
    func agentRow(_ agent: AgentSummary) -> some View {
        ...
        return HStack(alignment: .top, spacing: 12) {
            ...
            .buttonStyle(.plain)
            .foregroundStyle(isActive ? DaisyClawBrand.accent : .primary)
    ...
    func cronJobRow(_ job: CronJob) -> some View {
        HStack(spacing: 12) {
            ProIconBadge(
                ...
                color: job.enabled ? DaisyClawBrand.accent : .secondary)
            VStack(alignment: .leading, spacing: 3) {
                Text(job.name)
                    .font(.subheadline.weight(.semibold))
                    .lineLimit(1)
            ...
            Text(self.cronJobState(job))
                .font(.caption2.weight(.semibold))
                .foregroundStyle(job.enabled ? DaisyClawBrand.accent : .secondary)
                .lineLimit(1)
        }
        .padding(.vertical, 10)
    ...
    var instancesColor: Color {
        ...
        return (self.overview?.presence.isEmpty == false) ? DaisyClawBrand.accent : .secondary
    }
    ...
    var cronColor: Color {
        guard self.gatewayConnected else { return .secondary }
        return self.overview?.cronStatus?.enabled == true ? DaisyClawBrand.accent : .secondary
    }
    ...
    var dreamingColor: Color {
        guard self.gatewayConnected else { return .secondary }
        return self.overview?.dreaming?.enabled == true ? DaisyClawBrand.accent : .secondary
    }
