...
extension AgentProTab {
    ...
    var clawHubSearchCard: some View {
        ProCard(radius: AgentLayout.cardRadius) {
            VStack(alignment: .leading, spacing: 12) {
                HStack(spacing: 10) {
                    ProIconBadge(systemName: "square.and.arrow.down", color: DaisyClawBrand.accent)
    ...
    func clawHubResultRow(_ result: ClawHubSearchResultLite) -> some View {
        ...
        return HStack(alignment: .top, spacing: 10) {
            ProIconBadge(systemName: "sparkles", color: DaisyClawBrand.accent)
            VStack(alignment: .leading, spacing: 3) {
        ...
        ProCard(radius: AgentLayout.cardRadius) {
            VStack(alignment: .leading, spacing: 12) {
                ...
                if let message = self.skillConfigMessages[skill.effectiveSkillKey] {
                    Text(message.text)
                        .font(.caption2)
                        .foregroundStyle(message.kind == .success ? DaisyClawBrand.accent : DaisyClawBrand.warn)
                }
            }
        }
        .padding(.horizontal, DaisyClawProMetric.pagePadding)
    ...
    func skillStatus(_ skill: SkillStatusEntryLite) -> (text: String, color: Color) {
        ...
        if skill.hasMissingRequirements {
            return ("setup", DaisyClawBrand.warn)
        }
        return ("enabled", DaisyClawBrand.accent)
    }
}
