...
struct CommandPanel<Content: View>: View {
    ...
    var body: some View {
        ProCard(
            ...
            radius: DaisyClawProMetric.cardRadius)
        {
            self.content
        }
    }
...
struct CommandSessionRow: View {
    ...
    var body: some View {
        ...
        .padding(.vertical, 8)
        .background {
            RoundedRectangle(cornerRadius: DaisyClawProMetric.controlRadius, style: .continuous)
                .fill(self.rowFill)
                .overlay {
                    RoundedRectangle(cornerRadius: DaisyClawProMetric.controlRadius, style: .continuous)
    ...
    var body: some View {
        HStack(spacing: 10) {
            Image(systemName: self.icon)
                ...
                .foregroundStyle(DaisyClawBrand.ok)
                .frame(width: 30, height: 30)
                .background {
                    RoundedRectangle(cornerRadius: 8, style: .continuous)
                        .fill(DaisyClawBrand.ok.opacity(0.10))
                }
