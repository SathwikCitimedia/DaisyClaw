...
struct DaisyClawProBackground: View {
            ...
            if let actionTitle {
                if let action {
                    Button(actionTitle, action: action)
                        .font(.caption.weight(.medium))
                        .foregroundStyle(DaisyClawBrand.accent)
                } else {
                    Text(actionTitle)
                        .font(.caption.weight(.medium))
                        .foregroundStyle(.secondary)
...
struct ProCard<Content: View>: View {
    ...
    var radius: CGFloat = DaisyClawProMetric.cardRadius
    ...
    var body: some View {
        ...
        shape
            ...
            .overlay {
                if self.isProminent {
                    shape.strokeBorder(
                        DaisyClawBrand.accent.opacity(self.colorScheme == .dark ? 0.12 : 0.07),
                        lineWidth: 1)
                        .padding(1)
                }
            }
...
extension View {
    func proPanelSurface(
        tint: Color? = nil,
        radius: CGFloat = DaisyClawProMetric.cardRadius,
...
struct DaisyClawSidebarHeaderAction {
    let systemName: String
...
struct DaisyClawSidebarRevealButton: View {
    let headerAction: DaisyClawSidebarHeaderAction

    init(action: DaisyClawSidebarHeaderAction) {
        self.headerAction = action
    }

    init(action: @escaping () -> Void) {
        self.headerAction = DaisyClawSidebarHeaderAction(
...
struct DaisyClawSidebarHeaderLeadingSlot: View {
    let action: DaisyClawSidebarHeaderAction

    var body: some View {
        DaisyClawSidebarRevealButton(action: self.action)
            .frame(width: 44, height: 44, alignment: .center)
    }
}

struct DaisyClawAdaptiveHeaderRow<Leading: View, Accessory: View>: View {
    let title: String
    let subtitle: String
...
struct DaisyClawProMark: View {
    ...
    var body: some View {
        Image("DaisyClawIcon")
            ...
            .shadow(color: DaisyClawBrand.accent.opacity(0.18), radius: self.shadowRadius, y: self.shadowRadius / 3)
            .accessibilityLabel("DaisyClaw")
    }
}

struct ProProgressBar: View {
    let progress: Double
    var color: Color = DaisyClawBrand.accentHot
...
struct DaisyClawGatewayCompactPill: View {
        ...
        Button(action: self.action) {
            HStack(spacing: 12) {
                Image(systemName: self.systemImage)
                    ...
                    .background(DaisyClawBrand.accentHot, in: RoundedRectangle(cornerRadius: 13, style: .continuous))
                ...
                Image(systemName: "arrow.right")
                    ...
                    .foregroundStyle(DaisyClawBrand.accentHot)
            }
            .padding(12)
            .proGlassSurface(
                fill: self.colorScheme == .dark ? Color.white.opacity(0.045) : Color.white.opacity(0.68),
                stroke: DaisyClawBrand.accent.opacity(self.colorScheme == .dark ? 0.22 : 0.14),
                radius: 18,
                isProminent: true,
                interactive: true)
        }
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 10) {
            ProIconBadge(
                ...
                color: self.done ? DaisyClawBrand.ok : DaisyClawBrand.warn)
            VStack(alignment: .leading, spacing: 3) {
                Text(self.title)
