            ...
            Text(IPadWorkboardDefaults.label(for: status))
                ...
                .overlay {
                    Capsule()
                        .strokeBorder(
                            self.selectedStatus == status
                                ? DaisyClawBrand.accent.opacity(0.42)
                                : Color.primary.opacity(0.08),
                            lineWidth: 1)
                }
        }
        .buttonStyle(.plain)
        .foregroundStyle(self.selectedStatus == status ? DaisyClawBrand.accent : .primary)
...
private struct IPadWorkboardKanbanCard: View {
    ...
    private var color: Color {
        switch self.card.status {
        case "running": DaisyClawBrand.ok
        case "review": DaisyClawBrand.accent
        case "blocked": DaisyClawBrand.warn
        case "done": .secondary
        default: DaisyClawBrand.accentHot
        }
    }
