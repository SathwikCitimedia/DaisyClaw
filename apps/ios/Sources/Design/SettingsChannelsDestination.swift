...
struct SettingsChannelsDestination: View {
    ...
    private var summaryColor: Color {
        ...
        if self.errorText != nil { return DaisyClawBrand.warn }
        return self.channelEntries.contains(where: { $0.running || $0.connected }) ? DaisyClawBrand.ok : DaisyClawBrand
            .accent
    }
...
private struct SettingsChannelEntry: Identifiable {
    ...
    var color: Color {
        if self.connected || self.running { return DaisyClawBrand.ok }
        if self.lastError != nil { return DaisyClawBrand.warn }
        return self.configured ? DaisyClawBrand.accent : .secondary
    }
