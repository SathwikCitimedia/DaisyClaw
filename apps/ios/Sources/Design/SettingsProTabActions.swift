...
extension SettingsProTab {
        ...
        ProCard(padding: 0, radius: SettingsLayout.cardRadius) {
            VStack(spacing: 0) {
                ...
                self.diagnosticCheckRow(
                    ...
                    color: self.gatewayController.gateways.isEmpty ? .secondary : DaisyClawBrand.accent)
                Divider().padding(.leading, 60)
                self.diagnosticCheckRow(
                    icon: "waveform",
                    title: "Talk Config",
                ...
                self.diagnosticCheckRow(
                    ...
                    color: self.appModel.screenRecordActive ? DaisyClawBrand.ok : .secondary)
                Divider().padding(.leading, 60)
                self.diagnosticCheckRow(
                    icon: "mic",
                    title: "Voice Wake",
                    ...
                    color: self.voiceWakeEnabled ? DaisyClawBrand.ok : .secondary)
            }
        }
        .padding(.horizontal, DaisyClawProMetric.pagePadding)
    }
    ...
    func handleLocationModeChange(_ newValue: String) {
        ...
        guard let mode = DaisyClawLocationMode(rawValue: newValue) else { return }
    ...
    func friendlyGatewayMessage(from raw: String) -> String? {
        ...
        if lower.contains("pairing required") {
            return "Pairing required. Run /pair approve in your DaisyClaw chat, then connect again."
        }
    ...
    var gatewayStatusColor: Color {
        if self.appModel.isAppleReviewDemoModeEnabled { return DaisyClawBrand.accent }
        return self.gatewayConnected ? DaisyClawBrand.ok : .secondary
    }
    ...
    var gatewayTalkConfigColor: Color {
        ...
        return self.appModel.talkMode.gatewayTalkConfigLoaded ? DaisyClawBrand.ok : .secondary
    }
    ...
    var gatewayServer: String {
        self.appModel.gatewayServerName ?? "DaisyClaw Gateway"
    }

    var permissionsDetail: String {
        var enabled = 0
        if self.cameraEnabled { enabled += 1 }
        if self.locationModeRaw != DaisyClawLocationMode.off.rawValue { enabled += 1 }
    ...
    var diagnosticsRunColor: Color {
        ...
        return diagnosticsIssueCount == 0 ? DaisyClawBrand.ok : DaisyClawBrand.warn
    }

    var privacyDetail: String {
        let location = DaisyClawLocationMode(rawValue: self.locationModeRaw) ?? .off
        return location == .off ? "Location off" : "Location \(self.locationLabel)"
    }

    var locationLabel: String {
        switch DaisyClawLocationMode(rawValue: self.locationModeRaw) ?? .off {
        case .off: "Off"
