...
extension SettingsProTab {
    ...
    var settingsListSection: some View {
        VStack(spacing: 10) {
            self.settingsListRow(
                ...
                route: .approvals,
                color: self.pendingApproval == nil ? .secondary : DaisyClawBrand.warn,
            ...
            self.settingsListRow(
                icon: "info.circle",
                title: "About",
                detail: DeviceInfoHelper.daisyClawVersionString(),
                route: .about)
        }
        .padding(.horizontal, DaisyClawProMetric.pagePadding)
    }
    ...
    func destination(for route: SettingsRoute) -> some View {
        ZStack {
            ...
            ScrollView {
                ...
                .padding(.top, 18)
                .padding(.bottom, DaisyClawProMetric.bottomScrollInset)
            }
        }
    ...
    var approvalsDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                ...
                color: self.pendingApproval == nil ? DaisyClawBrand.ok : DaisyClawBrand.warn)
                ...
                if let pendingApproval {
                    ...
                    if let errorText = self.appModel.pendingExecApprovalPromptErrorText {
                        Text(errorText)
                            ...
                            .foregroundStyle(DaisyClawBrand.danger)
                    }
                    ...
                    HStack(spacing: 12) {
                        ProIconBadge(systemName: "checkmark.shield.fill", color: DaisyClawBrand.ok)
                        VStack(alignment: .leading, spacing: 3) {
    ...
    var permissionsDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.toggleCard(
                icon: "camera",
                title: "Camera",
                detail: "Allow the gateway to request photos or video while DaisyClaw is foregrounded.",
            ...
            self.toggleCard(
                icon: "lock.display",
                title: "Keep Awake",
                detail: "Keep the screen awake while DaisyClaw is open.",
    ...
    var voiceDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                ...
                color: self.talkEnabled || self.voiceWakeEnabled ? DaisyClawBrand.accent : .secondary)
    ...
    var diagnosticsDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                ...
                color: self.gatewayDiagnosticConnected ? DaisyClawBrand.ok : DaisyClawBrand.warn)
            ...
            self.detailListCard {
                ...
                Divider()
                self.detailRow("App", value: DeviceInfoHelper.daisyClawVersionString())
                Divider()
    ...
    var privacyDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                icon: "hand.raised",
                title: "Privacy",
                detail: "Control what device context DaisyClaw can expose to the gateway.",
    ...
    var notificationsDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                icon: "bell",
                title: "Notifications",
                detail: "Approvals and event alerts from DaisyClaw.",
                value: self.notificationStatusText,
                color: self.notificationStatusText == "Allowed" ? DaisyClawBrand.ok : .secondary)
            ...
            ProCard(radius: SettingsLayout.cardRadius) {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    Text("DaisyClaw uses notifications for approval prompts and mirrored event alerts.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
    ...
    var aboutDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                ...
                value: DeviceInfoHelper.daisyClawVersionString(),
                color: DaisyClawBrand.accent)

            self.detailListCard {
                self.detailRow("Version", value: DeviceInfoHelper.daisyClawVersionString())
                Divider()
                self.detailRow("Device", value: DeviceInfoHelper.deviceFamily())
                Divider()
                self.detailRow("Platform", value: DeviceInfoHelper.platformStringForDisplay())
        ...
        isOn: Binding<Bool>) -> some View
    {
        ProCard(radius: SettingsLayout.cardRadius) {
            Toggle(isOn: isOn) {
                HStack(spacing: 12) {
                    ProIconBadge(systemName: icon, color: isOn.wrappedValue ? DaisyClawBrand.accent : .secondary)
                    VStack(alignment: .leading, spacing: 3) {
                        Text(title)
                            .font(.subheadline.weight(.semibold))
                        Text(detail)
        ...
        ProCard(radius: SettingsLayout.cardRadius) {
            VStack(alignment: .leading, spacing: 12) {
                HStack(spacing: 12) {
                    ProIconBadge(
                        systemName: "location",
                        color: self.locationModeRaw == DaisyClawLocationMode.off.rawValue ? .secondary : DaisyClawBrand
                            .accent)
                    VStack(alignment: .leading, spacing: 3) {
                        Text("Location")
                            .font(.subheadline.weight(.semibold))
                ...
                Picker("Location", selection: self.$locationModeRaw) {
                    Text("Off").tag(DaisyClawLocationMode.off.rawValue)
                    Text("While Using").tag(DaisyClawLocationMode.whileUsing.rawValue)
                    Text("Always").tag(DaisyClawLocationMode.always.rawValue)
                }
                .pickerStyle(.segmented)
                ...
                if let locationStatusText {
                    Text(locationStatusText)
                        .font(.caption2)
                        .foregroundStyle(DaisyClawBrand.warn)
                }
            }
        }
        .padding(.horizontal, DaisyClawProMetric.pagePadding)
