...
struct TalkProTab: View {
    ...
    private var header: some View {
        HStack(alignment: .center, spacing: 11) {
            ...
            DaisyClawProMark(size: 31, shadowRadius: 9)
...
struct TalkProState: Equatable {
    ...
    var title: String {
        ...
        if self.normalizedStatus.contains("thinking") { return "Asking DaisyClaw" }
    ...
    var color: Color {
        ...
        case .requestFailed, .loadFailed:
            return DaisyClawBrand.danger
        case .missingScope, .requestingUpgrade, .upgradeRequested, .apiKeyMissing:
            return DaisyClawBrand.warn
        default:
            if !self.isConfigLoaded { return DaisyClawBrand.warn }
            return self.isEnabled ? DaisyClawBrand.ok : DaisyClawBrand.accentHot
        }
    }
    ...
    var primaryButtonFill: AnyShapeStyle {
        switch self.primaryAction {
        case .stop:
            AnyShapeStyle(DaisyClawBrand.danger)
        case .waiting:
            AnyShapeStyle(DaisyClawBrand.warn.opacity(0.72))
        default:
            AnyShapeStyle(LinearGradient(
                colors: [self.color.opacity(0.95), DaisyClawBrand.accent],
