...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> DaisyClawDeviceStatusPayload {
        ...
        return DaisyClawDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> DaisyClawDeviceInfoPayload {
        ...
        return DaisyClawDeviceInfoPayload(
    ...
    private func batteryStatus() -> DaisyClawBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: DaisyClawBatteryState = switch device.batteryState {
        ...
        return DaisyClawBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> DaisyClawThermalStatusPayload {
        let state: DaisyClawThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return DaisyClawThermalStatusPayload(state: state)
    }

    private func storageStatus() -> DaisyClawStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return DaisyClawStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
