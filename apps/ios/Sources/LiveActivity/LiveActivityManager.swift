...
final class LiveActivityManager {
    ...
    private let logger = Logger(subsystem: "ai.daisyclaw.ios", category: "LiveActivity")
    private let connectingStaleSeconds: TimeInterval = 120
    private let hydrationStaleSeconds: TimeInterval = 300
    private var currentActivity: Activity<DaisyClawActivityAttributes>?
    ...
    func showConnecting(statusText: String = "Connecting...", agentName: String, sessionKey: String) {
        ...
        let attributes = DaisyClawActivityAttributes(agentName: agentName, sessionKey: sessionKey)
    ...
    private func hydrateCurrentAndPruneDuplicates() {
        let active = Activity<DaisyClawActivityAttributes>.activities
    ...
    private func updateCurrent(state: DaisyClawActivityAttributes.ContentState, staleDate: Date? = nil) {
    ...
    private func end(activity: Activity<DaisyClawActivityAttributes>) {
        Task {
            await activity.end(
                ActivityContent(state: self.disconnectedState(), staleDate: nil),
                dismissalPolicy: .immediate)
    ...
    private func connectingState(statusText: String = "Connecting...") -> DaisyClawActivityAttributes.ContentState {
        DaisyClawActivityAttributes.ContentState(
    ...
    private func attentionState(statusText: String) -> DaisyClawActivityAttributes.ContentState {
        DaisyClawActivityAttributes.ContentState(
            statusText: statusText,
            isIdle: false,
            isDisconnected: false,
    ...
    private func idleState() -> DaisyClawActivityAttributes.ContentState {
        DaisyClawActivityAttributes.ContentState(
            statusText: "Idle",
            isIdle: true,
            isDisconnected: false,
    ...
    private func disconnectedState() -> DaisyClawActivityAttributes.ContentState {
        DaisyClawActivityAttributes.ContentState(
