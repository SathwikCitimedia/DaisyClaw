...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: DaisyClawWatchNotifyParams) -> DaisyClawWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [DaisyClawWatchAction]?,
        kind: String?,
        promptId: String?) -> [DaisyClawWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> DaisyClawWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return DaisyClawWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                DaisyClawWatchAction(id: "approve", label: "Approve"),
                DaisyClawWatchAction(id: "decline", label: "Decline", style: "destructive"),
                DaisyClawWatchAction(id: "open_phone", label: "Open iPhone"),
                DaisyClawWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            DaisyClawWatchAction(id: "done", label: "Done"),
            DaisyClawWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            DaisyClawWatchAction(id: "open_phone", label: "Open iPhone"),
            DaisyClawWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: DaisyClawWatchRisk?,
        priority: DaisyClawNotificationPriority?) -> DaisyClawWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: DaisyClawNotificationPriority?,
        risk: DaisyClawWatchRisk?) -> DaisyClawNotificationPriority?
    {
