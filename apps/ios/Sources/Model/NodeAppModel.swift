...
final class NodeAppModel {
    ...
    private let deepLinkLogger = Logger(subsystem: "ai.daisyclaw.ios", category: "DeepLink")
    private let pushWakeLogger = Logger(subsystem: "ai.daisyclaw.ios", category: "PushWake")
    private let pendingActionLogger = Logger(subsystem: "ai.daisyclaw.ios", category: "PendingAction")
    private let locationWakeLogger = Logger(subsystem: "ai.daisyclaw.ios", category: "LocationWake")
    private let watchReplyLogger = Logger(subsystem: "ai.daisyclaw.ios", category: "WatchReply")
    private let watchExecApprovalLogger = Logger(subsystem: "ai.daisyclaw.ios", category: "WatchExecApproval")
    private let execApprovalNotificationLogger = Logger(
        subsystem: "ai.daisyclaw.ios",
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = DaisyClawCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = DaisyClawCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = DaisyClawCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = DaisyClawCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = DaisyClawCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    func requestLocationPermissions(mode: DaisyClawLocationMode) async -> Bool {
            ...
            check: { [weak self] in
                ...
                do {
                    let data = try await self.operatorGateway.request(
                        ...
                        paramsJSON: nil,
                        timeoutSeconds: 6)
                    guard let decoded = try? JSONDecoder().decode(DaisyClawGatewayHealthOK.self, from: data) else {
                        return false
                    }
        ...
        do {
            ...
            case .unknownCommand:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: DaisyClawNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            case .handlerUnavailable:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: DaisyClawNodeError(code: .unavailable, message: "node handler unavailable"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: DaisyClawNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(DaisyClawLocationGetParams.self, from: req.paramsJSON)) ??
            DaisyClawLocationGetParams()
        ...
        let payload = DaisyClawLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case DaisyClawCanvasCommand.present.rawValue:
            // iOS ignores placement hints; canvas always fills the screen.
            let params = (try? Self.decodeParams(DaisyClawCanvasPresentParams.self, from: req.paramsJSON)) ??
                DaisyClawCanvasPresentParams()
        ...
        case DaisyClawCanvasCommand.hide.rawValue:
            self.screen.hideCanvas()
            return BridgeInvokeResponse(id: req.id, ok: true)
        case DaisyClawCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(DaisyClawCanvasNavigateParams.self, from: req.paramsJSON)
            let trimmedURL = params.url.trimmingCharacters(in: .whitespacesAndNewlines)
            self.screen.present(urlString: trimmedURL)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case DaisyClawCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(DaisyClawCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case DaisyClawCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(DaisyClawCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let command = req.command
        switch command {
        case DaisyClawCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              const host = globalThis.daisyclawA2UI;
              if (!host) return JSON.stringify({ ok: false, error: "missing daisyclawA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)

        case DaisyClawCanvasA2UICommand.push.rawValue, DaisyClawCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [DaisyClawKit.AnyCodable]
            if command == DaisyClawCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(DaisyClawCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try DaisyClawCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(DaisyClawCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try DaisyClawCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.daisyclawA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case DaisyClawCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case DaisyClawCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(DaisyClawCameraSnapParams.self, from: req.paramsJSON)) ??
                DaisyClawCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case DaisyClawCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(DaisyClawCameraClipParams.self, from: req.paramsJSON)) ??
                DaisyClawCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(DaisyClawScreenRecordParams.self, from: req.paramsJSON)) ??
            DaisyClawScreenRecordParams()
    ...
    private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(DaisyClawSystemNotifyParams.self, from: req.paramsJSON)
        ...
        if title.isEmpty, body.isEmpty {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: DaisyClawNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty notification"))
        }
        ...
        guard finalStatus == .authorized || finalStatus == .provisional || finalStatus == .ephemeral else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: DaisyClawNodeError(code: .unavailable, message: "NOT_AUTHORIZED: notifications"))
        }
        ...
        if case let .failure(error) = addResult {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: DaisyClawNodeError(code: .unavailable, message: "NOTIFICATION_FAILED: \(error.message)"))
        }
        return BridgeInvokeResponse(id: req.id, ok: true)
    }

    private func handleChatPushInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(DaisyClawChatPushParams.self, from: req.paramsJSON)
        let text = params.text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !text.isEmpty else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: DaisyClawNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty chat.push text"))
        }
        ...
        let payload = DaisyClawChatPushPayload(messageId: messageId)
        let json = try Self.encodePayload(payload)
        return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
    }
    ...
    private func handleDeviceInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case DaisyClawDeviceCommand.status.rawValue:
            let payload = try await self.deviceStatusService.status()
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case DaisyClawDeviceCommand.info.rawValue:
    ...
    private func handlePhotosInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(DaisyClawPhotosLatestParams.self, from: req.paramsJSON)) ??
            DaisyClawPhotosLatestParams()
    ...
    private func handleContactsInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case DaisyClawContactsCommand.search.rawValue:
            let params = (try? Self.decodeParams(DaisyClawContactsSearchParams.self, from: req.paramsJSON)) ??
                DaisyClawContactsSearchParams()
            let payload = try await self.contactsService.search(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case DaisyClawContactsCommand.add.rawValue:
            let params = try Self.decodeParams(DaisyClawContactsAddParams.self, from: req.paramsJSON)
    ...
    private func handleCalendarInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case DaisyClawCalendarCommand.events.rawValue:
            let params = (try? Self.decodeParams(DaisyClawCalendarEventsParams.self, from: req.paramsJSON)) ??
                DaisyClawCalendarEventsParams()
            let payload = try await self.calendarService.events(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case DaisyClawCalendarCommand.add.rawValue:
            let params = try Self.decodeParams(DaisyClawCalendarAddParams.self, from: req.paramsJSON)
    ...
    private func handleRemindersInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case DaisyClawRemindersCommand.list.rawValue:
            let params = (try? Self.decodeParams(DaisyClawRemindersListParams.self, from: req.paramsJSON)) ??
                DaisyClawRemindersListParams()
            let payload = try await self.remindersService.list(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case DaisyClawRemindersCommand.add.rawValue:
            let params = try Self.decodeParams(DaisyClawRemindersAddParams.self, from: req.paramsJSON)
    ...
    private func handleMotionInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case DaisyClawMotionCommand.activity.rawValue:
            let params = (try? Self.decodeParams(DaisyClawMotionActivityParams.self, from: req.paramsJSON)) ??
                DaisyClawMotionActivityParams()
            let payload = try await self.motionService.activities(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case DaisyClawMotionCommand.pedometer.rawValue:
            let params = (try? Self.decodeParams(DaisyClawPedometerParams.self, from: req.paramsJSON)) ??
                DaisyClawPedometerParams()
    ...
    private func handleTalkInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case DaisyClawTalkCommand.pttStart.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case DaisyClawTalkCommand.pttStop.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case DaisyClawTalkCommand.pttCancel.rawValue:
            let payload = await self.talkMode.cancelPushToTalk()
            self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
            self.pttVoiceWakeSuspended = false
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case DaisyClawTalkCommand.pttOnce.rawValue:
            self.pttVoiceWakeSuspended = self.voiceWake.suspendForExternalAudioCapture()
            defer {
                self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
                self.pttVoiceWakeSuspended = false
...
extension NodeAppModel {
    ...
    private func buildCapabilityRouter() -> NodeCapabilityRouter {
        ...
        register([DaisyClawLocationCommand.get.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleLocationInvoke(req)
        }

        register([
            DaisyClawCanvasCommand.present.rawValue,
            DaisyClawCanvasCommand.hide.rawValue,
            DaisyClawCanvasCommand.navigate.rawValue,
            DaisyClawCanvasCommand.evalJS.rawValue,
            DaisyClawCanvasCommand.snapshot.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            DaisyClawCanvasA2UICommand.reset.rawValue,
            DaisyClawCanvasA2UICommand.push.rawValue,
            DaisyClawCanvasA2UICommand.pushJSONL.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([DaisyClawScreenCommand.record.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleScreenRecordInvoke(req)
        }

        register([DaisyClawSystemCommand.notify.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleSystemNotify(req)
        }

        register([DaisyClawChatCommand.push.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            DaisyClawWatchCommand.status.rawValue,
            DaisyClawWatchCommand.notify.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleWatchInvoke(req)
        }

        register([DaisyClawPhotosCommand.latest.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            DaisyClawRemindersCommand.list.rawValue,
            DaisyClawRemindersCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
    ...
    private func handleWatchInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case DaisyClawWatchCommand.status.rawValue:
            let status = await self.watchMessagingService.status()
            let payload = DaisyClawWatchStatusPayload(
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case DaisyClawWatchCommand.notify.rawValue:
            let params = try Self.decodeParams(DaisyClawWatchNotifyParams.self, from: req.paramsJSON)
            ...
            do {
                ...
                let payload = DaisyClawWatchNotifyPayload(
    ...
    private func locationMode() -> DaisyClawLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return DaisyClawLocationMode(rawValue: raw) ?? .off
    }
...
extension NodeAppModel {
    ...
    private func legacyClientIdFallback(currentClientId: String, error: Error) -> String? {
        ...
        guard normalizedClientId == "daisyclaw-ios" else { return nil }
...
extension NodeAppModel {
    ...
    func runSharePipelineSelfTest() async {
        ...
        let payload = SharedContentPayload(
            title: "DaisyClaw Share Self-Test",
            url: URL(string: "https://daisyclaw.ai/share-self-test"),
    ...
    private static func makeWatchExecApprovalItem(from prompt: ExecApprovalPrompt) -> DaisyClawWatchExecApprovalItem {
        let decisions = prompt.allowedDecisions.compactMap { decision in
            ...
            return DaisyClawWatchExecApprovalDecision(rawValue: normalizedDecision)
        }
        let preview = Self.trimmedOrNil(prompt.commandPreview) ?? Self.trimmedOrNil(prompt.commandText)
        return DaisyClawWatchExecApprovalItem(
            id: prompt.id,
    ...
    private func publishWatchExecApprovalPrompt(_ prompt: ExecApprovalPrompt, reason: String) async {
        let message = DaisyClawWatchExecApprovalPromptMessage(
        ...
        source: String) async
    {
        ...
        let message = DaisyClawWatchExecApprovalResolvedMessage(
    ...
    private func publishWatchExecApprovalExpired(
        approvalId: String,
        reason: DaisyClawWatchExecApprovalCloseReason) async
    {
        let normalizedApprovalID = approvalId.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !normalizedApprovalID.isEmpty else { return }
        self.removeWatchExecApprovalPrompt(normalizedApprovalID)
        let message = DaisyClawWatchExecApprovalExpiredMessage(
            approvalId: normalizedApprovalID,
            reason: reason,
    ...
    private func syncWatchExecApprovalSnapshot(reason: String) async {
        ...
        let message = DaisyClawWatchExecApprovalSnapshotMessage(
            approvals: approvals,
            sentAtMs: Int(Date().timeIntervalSince1970 * 1000),
    ...
    func handleSilentPushWake(_ userInfo: [AnyHashable: Any]) async -> Bool {
        ...
        let pushKind = Self.daisyclawPushKind(userInfo)
        let receivedMessage =
    ...
    private static func daisyclawPushKind(_ userInfo: [AnyHashable: Any]) -> String {
        if let payload = userInfo["daisyclaw"] as? [String: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
        }
        if let payload = userInfo["daisyclaw"] as? [AnyHashable: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
        ...
        sourceReason: String? = nil) async -> ExecApprovalResolutionOutcome
    {
        ...
        guard connected else {
            ...
            return .failed(message: "DaisyClaw couldn't connect to the gateway operator session.")
        }
        ...
        do {
            ...
            await self.publishWatchExecApprovalResolved(
                approvalId: normalizedApprovalID,
                decision: DaisyClawWatchExecApprovalDecision(rawValue: normalizedDecision),
                source: "iphone")
            return .resolved
        } catch {
            if Self.isApprovalNotificationStaleError(error) {
            ...
            return .failed(
                message: "DaisyClaw couldn't resolve this approval right now. Try again.")
        }
    }
