...
final class MotionService: MotionServicing {
    func activities(params: DaisyClawMotionActivityParams) async throws -> DaisyClawMotionActivityPayload {
        ...
        let mapped: [DaisyClawMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return DaisyClawMotionActivityPayload(activities: mapped)
    }
        ...
        return try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    ...
                    let formatter = ISO8601DateFormatter()
                    let payload = DaisyClawPedometerPayload(
