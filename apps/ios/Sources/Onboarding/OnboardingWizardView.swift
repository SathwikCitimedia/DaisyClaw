...
struct OnboardingWizardView: View {
    ...
    @State private var statusLine: String = "In your DaisyClaw chat, run /pair qr, then scan the code here."
            ...
            if self.issue.needsPairing {
                Section {
                    ...
                    let requestLine: String = {
                        ...
                        return "Request ID: check `daisyclaw devices list`."
                    }()
                    let commandLine = self.currentProblem?.actionCommand ?? "daisyclaw devices approve <requestId>"
                    Text(
                        "Approve this device on the gateway.\n"
                            + "1) `\(commandLine)`\n"
                            + "2) `/pair approve` in your DaisyClaw chat\n"
                            + "\(requestLine)\n"
                            + "DaisyClaw will also retry automatically when you return to this app.")
                }
            }
...
extension OnboardingWizardView {
    ...
    private func advanceFromIntro() {
        ...
        self.statusLine = "In your DaisyClaw chat, run /pair qr, then scan the code here."
        self.step = .welcome
    }
    ...
    private func initializeState() {
        if self.manualHost.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            if let last = GatewaySettingsStore.loadLastGatewayConnection() {
                ...
                case .discovered:
                    self.manualHost = "daisyclaw.local"
        ...
        if self.selectedMode == .developerLocal, self.manualHost == "daisyclaw.local" {
        ...
        if !hasSavedGateway, !hasToken, !hasPassword {
            self.statusLine = "No saved pairing found. In your DaisyClaw chat, run /pair qr, then scan the code here."
        }
    }
    ...
    private func applyModeDefaults(_ mode: OnboardingConnectionMode) {
        ...
        let hostIsDefaultLike = host.isEmpty || host == "daisyclaw.local" || host == "localhost"

        switch mode {
        case .homeNetwork:
            if hostIsDefaultLike { self.manualHost = "daisyclaw.local" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .remoteDomain:
            if host == "daisyclaw.local" || host == "localhost" { self.manualHost = "" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .developerLocal:
