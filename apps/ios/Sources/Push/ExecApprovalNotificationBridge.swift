...
enum ExecApprovalNotificationBridge {
    ...
    static let categoryIdentifier = "daisyclaw.exec-approval"
    static let reviewActionIdentifier = "daisyclaw.exec-approval.review"
    ...
    static func approvalID(from userInfo: [AnyHashable: Any]) -> String? {
        let raw = self.daisyClawPayload(userInfo: userInfo)?["approvalId"] as? String
    ...
    static func payloadKind(userInfo: [AnyHashable: Any]) -> String {
        let raw = self.daisyClawPayload(userInfo: userInfo)?["kind"] as? String
        let trimmed = raw?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        return trimmed.isEmpty ? "unknown" : trimmed
    }

    private static func daisyClawPayload(userInfo: [AnyHashable: Any]) -> [String: Any]? {
