...
final class RemindersService: RemindersServicing {
    func list(params: DaisyClawRemindersListParams) async throws -> DaisyClawRemindersListPayload {
        ...
        let payload: [DaisyClawReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return DaisyClawReminderPayload(
        ...
        return DaisyClawRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: DaisyClawRemindersAddParams) async throws -> DaisyClawRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = DaisyClawReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return DaisyClawRemindersAddPayload(reminder: payload)
    }
