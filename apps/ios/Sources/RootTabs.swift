...
struct RootTabs: View {
    ...
    private static var initialTab: AppTab {
        ...
        guard let flagIndex = arguments.firstIndex(of: "--daisyclaw-initial-tab") else {
            return .control
        }
    ...
    private static var requestedInitialSidebarDestination: SidebarDestination? {
        let arguments = ProcessInfo.processInfo.arguments
        guard let flagIndex = arguments.firstIndex(of: "--daisyclaw-initial-destination") else {
            return nil
        }
        let valueIndex = arguments.index(after: flagIndex)
    ...
    private static var initialChatSessionKey: String? {
        let arguments = ProcessInfo.processInfo.arguments
        guard let flagIndex = arguments.firstIndex(of: "--daisyclaw-chat-session") else {
            return nil
        }
        let valueIndex = arguments.index(after: flagIndex)
        guard arguments.indices.contains(valueIndex) else { return nil }
    ...
    private func sidebarNavigationSplitContent(sidebarWidth: CGFloat) -> some View {
        ...
        .background(DaisyClawProBackground())
    }
    ...
    private var sidebarIdentityHeader: some View {
        HStack(spacing: 10) {
            DaisyClawProMark(size: 30, shadowRadius: 3)
        ...
        .accessibilityLabel("DaisyClaw \(self.sidebarGatewayStatusTitle)")
    }
        ...
        VStack(spacing: 0) {
            ...
            HStack(spacing: 10) {
                Text("VERSION")
                    ...
                    .foregroundStyle(.secondary)
                Spacer(minLength: 8)
                Text("v\(DeviceInfoHelper.daisyClawVersionString())")
        ...
        title: String? = nil) -> some View
    {
        ...
        .buttonStyle(.plain)
        .foregroundStyle(destination == self.selectedSidebarDestination ? DaisyClawBrand.accent : .primary)
        .listRowBackground(
            destination == self.selectedSidebarDestination
                ? DaisyClawBrand.accent.opacity(0.12)
                : Color.clear)
    ...
    private var sidebarHeaderLeadingAction: DaisyClawSidebarHeaderAction? {
        ...
        if self.isSidebarVisible {
            return DaisyClawSidebarHeaderAction(
    ...
    private func makeHomeCanvasPayload() -> RootTabsHomeCanvasPayload {
        ...
        case .connected:
            return RootTabsHomeCanvasPayload(
                ...
                footer: "DaisyClaw only runs phone-side capabilities while the app is connected and permitted.")
        case .connecting:
            return RootTabsHomeCanvasPayload(
        ...
        case .error, .disconnected:
            return RootTabsHomeCanvasPayload(
                ...
                eyebrow: self.gatewayStatus == .error ? "Gateway needs attention" : "DaisyClaw iOS",
                ...
                agents: Array(agents.prefix(4)),
                footer:
                "Use Settings to scan a pairing QR code or paste a setup code from your DaisyClaw gateway.")
        }
    }
