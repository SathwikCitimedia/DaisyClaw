...
final class RealtimeTalkRelaySession {
    private static let agentControlToolName = "daisyclaw_agent_control"
    ...
    private let logger = Logger(subsystem: "ai.daisyclaw", category: "RealtimeTalkRelay")
    ...
    private func handleToolCall(_ payload: [String: AnyCodable]) async {
        ...
        do {
            ...
            let result: [String: Any] = completion.failed
                ? ["error": "DaisyClaw tool call failed"]
                : ["text": completion.text ?? "DaisyClaw finished with no text."]
                ...
                for await event in stream {
                    ...
                    if chatEvent.state == "final" {
                        return ChatCompletionResult(
                            text: DaisyClawChatEventText.assistantText(from: chatEvent),
                            failed: false)
                    }
