...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__DAISYCLAW_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<DaisyClawTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.daisyclaw", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = DaisyClawTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> DaisyClawTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = activePTTCaptureId {
            return DaisyClawTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> DaisyClawTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> DaisyClawTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return DaisyClawTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> DaisyClawTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = DaisyClawTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: DaisyClawTalkPTTStopPayload) {
            ...
            group.addTask { [runId] in
                ...
                for await evt in stream {
                    ...
                    if evt.event == "chat" {
                        ...
                        if let text = DaisyClawChatEventText.assistantText(from: chatEvent) {
