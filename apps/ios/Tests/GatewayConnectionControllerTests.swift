...
@Suite(.serialized) struct GatewayConnectionControllerTests {
    ...
    @Test @MainActor func currentCapsReflectToggles() {
        withUserDefaults([
            ...
            "camera.enabled": true,
            "location.enabledMode": DaisyClawLocationMode.always.rawValue,
            ...
            #expect(caps.contains(DaisyClawCapability.canvas.rawValue))
            #expect(caps.contains(DaisyClawCapability.screen.rawValue))
            #expect(caps.contains(DaisyClawCapability.camera.rawValue))
            #expect(caps.contains(DaisyClawCapability.location.rawValue))
            #expect(caps.contains(DaisyClawCapability.voiceWake.rawValue))
            #expect(caps.contains(DaisyClawCapability.talk.rawValue))
        }
    }
    ...
    @Test @MainActor func currentCommandsIncludeLocationWhenEnabled() {
        withUserDefaults([
            "node.instanceId": "ios-test",
            "location.enabledMode": DaisyClawLocationMode.whileUsing.rawValue,
        ]) {
            let appModel = NodeAppModel()
            let controller = GatewayConnectionController(appModel: appModel, startDiscovery: false)
            let commands = Set(controller._test_currentCommands())

            #expect(commands.contains(DaisyClawLocationCommand.get.rawValue))
        }
    }
    ...
    @Test @MainActor func currentCommandsExcludeDangerousSystemExecCommands() {
        withUserDefaults([
            ...
            #expect(commands.contains(DaisyClawSystemCommand.notify.rawValue))
            #expect(!commands.contains(DaisyClawSystemCommand.run.rawValue))
            #expect(!commands.contains(DaisyClawSystemCommand.which.rawValue))
            #expect(!commands.contains(DaisyClawSystemCommand.execApprovalsGet.rawValue))
            #expect(!commands.contains(DaisyClawSystemCommand.execApprovalsSet.rawValue))
        }
    }
    ...
    @Test @MainActor func loadLastConnectionReadsSavedValues() {
        let prior = KeychainStore.loadString(service: "ai.daisyclaw.gateway", account: "lastConnection")
        defer {
            if let prior {
                _ = KeychainStore.saveString(prior, service: "ai.daisyclaw.gateway", account: "lastConnection")
            } else {
                _ = KeychainStore.delete(service: "ai.daisyclaw.gateway", account: "lastConnection")
            }
        }
        _ = KeychainStore.delete(service: "ai.daisyclaw.gateway", account: "lastConnection")
