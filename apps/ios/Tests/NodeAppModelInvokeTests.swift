...
private final class MockWatchMessagingService: @preconcurrency WatchMessagingServicing, @unchecked Sendable {
    ...
    var sendError: Error?
    var lastSent: (id: String, params: DaisyClawWatchNotifyParams)?
    var lastSentExecApprovalPrompt: DaisyClawWatchExecApprovalPromptMessage?
    var lastSentExecApprovalResolved: DaisyClawWatchExecApprovalResolvedMessage?
    var lastSentExecApprovalExpired: DaisyClawWatchExecApprovalExpiredMessage?
    var lastSentExecApprovalSnapshot: DaisyClawWatchExecApprovalSnapshotMessage?
    ...
    func sendNotification(id: String, params: DaisyClawWatchNotifyParams) async throws -> WatchNotificationSendResult {
...
@Suite(.serialized) struct NodeAppModelInvokeTests {
    @Test @MainActor func decodeParamsFailsWithoutJSON() {
        #expect(throws: Error.self) {
            _ = try NodeAppModel._test_decodeParams(DaisyClawCanvasNavigateParams.self, from: nil)
        }
    }
    ...
    @Test @MainActor func handleInvokeRejectsBackgroundCommands() async {
        ...
        let req = BridgeInvokeRequest(id: "bg", command: DaisyClawCanvasCommand.present.rawValue)
    ...
    @Test @MainActor func handleInvokeRejectsCameraWhenDisabled() async {
        let appModel = NodeAppModel()
        let req = BridgeInvokeRequest(id: "cam", command: DaisyClawCameraCommand.snap.rawValue)
    ...
    @Test @MainActor func handleInvokeRejectsInvalidScreenFormat() async {
        let appModel = NodeAppModel()
        let params = DaisyClawScreenRecordParams(format: "gif")
        let data = try? JSONEncoder().encode(params)
        let json = data.flatMap { String(data: $0, encoding: .utf8) }

        let req = BridgeInvokeRequest(
            id: "screen",
            command: DaisyClawScreenCommand.record.rawValue,
            paramsJSON: json)
    ...
    @Test @MainActor func handleInvokeCanvasCommandsUpdateScreen() async throws {
        ...
        let present = BridgeInvokeRequest(id: "present", command: DaisyClawCanvasCommand.present.rawValue)
        ...
        let navigateParams = DaisyClawCanvasNavigateParams(url: "http://example.com/")
        let navData = try JSONEncoder().encode(navigateParams)
        let navJSON = String(decoding: navData, as: UTF8.self)
        let navigate = BridgeInvokeRequest(
            id: "nav",
            command: DaisyClawCanvasCommand.navigate.rawValue,
            paramsJSON: navJSON)
        let navRes = await appModel._test_handleInvoke(navigate)
        #expect(navRes.ok == true)
        #expect(appModel.screen.urlString == "http://example.com/")

        let evalParams = DaisyClawCanvasEvalParams(javaScript: "1+1")
        ...
        let eval = BridgeInvokeRequest(
            id: "eval",
            command: DaisyClawCanvasCommand.evalJS.rawValue,
            paramsJSON: evalJSON)
    ...
    @Test @MainActor func handleInvokeA2UICommandsFailWhenLocalHostUnavailable() async throws {
        ...
        let reset = BridgeInvokeRequest(id: "reset", command: DaisyClawCanvasA2UICommand.reset.rawValue)
        ...
        let pushParams = DaisyClawCanvasA2UIPushJSONLParams(jsonl: jsonl)
        ...
        let push = BridgeInvokeRequest(
            id: "push",
            command: DaisyClawCanvasA2UICommand.pushJSONL.rawValue,
            paramsJSON: pushJSON)
    ...
    @Test @MainActor func handleInvokeWatchStatusReturnsServiceSnapshot() async throws {
        ...
        let appModel = NodeAppModel(watchMessagingService: watchService)
        let req = BridgeInvokeRequest(id: "watch-status", command: DaisyClawWatchCommand.status.rawValue)

        let res = await appModel._test_handleInvoke(req)
        #expect(res.ok == true)

        let payloadData = try #require(res.payloadJSON?.data(using: .utf8))
        let payload = try JSONDecoder().decode(DaisyClawWatchStatusPayload.self, from: payloadData)
    ...
    @Test @MainActor func handleInvokeWatchNotifyRoutesToWatchService() async throws {
        ...
        let appModel = NodeAppModel(watchMessagingService: watchService)
        let params = DaisyClawWatchNotifyParams(
            title: "DaisyClaw",
        ...
        let req = BridgeInvokeRequest(
            id: "watch-notify",
            command: DaisyClawWatchCommand.notify.rawValue,
            paramsJSON: paramsJSON)

        let res = await appModel._test_handleInvoke(req)
        #expect(res.ok == true)
        #expect(watchService.lastSent?.params.title == "DaisyClaw")
        ...
        let payloadData = try #require(res.payloadJSON?.data(using: .utf8))
        let payload = try JSONDecoder().decode(DaisyClawWatchNotifyPayload.self, from: payloadData)
    ...
    @Test @MainActor func handleInvokeWatchNotifyRejectsEmptyMessage() async throws {
        let watchService = MockWatchMessagingService()
        let appModel = NodeAppModel(watchMessagingService: watchService)
        let params = DaisyClawWatchNotifyParams(title: "   ", body: "\n")
        let paramsData = try JSONEncoder().encode(params)
        let paramsJSON = String(decoding: paramsData, as: UTF8.self)
        let req = BridgeInvokeRequest(
    ...
    @Test @MainActor func handleInvokeWatchNotifyDerivesPriorityFromRiskAndCapsActions() async throws {
        ...
        let params = DaisyClawWatchNotifyParams(
            ...
            risk: .high,
            actions: [
                DaisyClawWatchAction(id: "a1", label: "A1"),
                DaisyClawWatchAction(id: "a2", label: "A2"),
                DaisyClawWatchAction(id: "a3", label: "A3"),
                DaisyClawWatchAction(id: "a4", label: "A4"),
                DaisyClawWatchAction(id: "a5", label: "A5"),
            ])
        let paramsData = try JSONEncoder().encode(params)
        let paramsJSON = String(decoding: paramsData, as: UTF8.self)
        let req = BridgeInvokeRequest(
    ...
    @Test @MainActor func handleInvokeWatchNotifyReturnsUnavailableOnDeliveryFailure() async throws {
        ...
        let appModel = NodeAppModel(watchMessagingService: watchService)
        let params = DaisyClawWatchNotifyParams(title: "DaisyClaw", body: "Delivery check")
        let paramsData = try JSONEncoder().encode(params)
        let paramsJSON = String(decoding: paramsData, as: UTF8.self)
        let req = BridgeInvokeRequest(
    ...
    @Test @MainActor func handleDeepLinkSetsErrorWhenNotConnected() async throws {
        let appModel = NodeAppModel()
        let url = try #require(URL(string: "daisyclaw://agent?message=hello"))
    ...
    @Test @MainActor func handleDeepLinkRejectsOversizedMessage() async throws {
        ...
        let url = try #require(URL(string: "daisyclaw://agent?message=\(msg)"))
        await appModel.handleDeepLink(url: url)
    ...
    @Test @MainActor func operatorAdminScopeCacheRefreshesFromStoredToken() throws {
        ...
        let previousStateDir = ProcessInfo.processInfo.environment["DAISYCLAW_STATE_DIR"]
        setenv("DAISYCLAW_STATE_DIR", tempDir.path, 1)
        defer {
            if let previousStateDir {
                setenv("DAISYCLAW_STATE_DIR", previousStateDir, 1)
            } else {
                unsetenv("DAISYCLAW_STATE_DIR")
            }
