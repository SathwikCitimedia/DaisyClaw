...
ASC_KEYCHAIN_SERVICE=daisyclaw-asc-key
...
```bash
ASC_APP_IDENTIFIER=ai.daisyclaw.client
# or
...
Tip: run `scripts/ios-team-id.sh` from repo root to print a Team ID for `.env`. The helper prefers the canonical DaisyClaw team (`Y5PE65HELJ`) when present locally; otherwise it prefers the first non-personal team from your Xcode account (then personal team if needed). Fastlane uses this helper automatically if `IOS_DEVELOPMENT_TEAM` is missing.

For local/manual iOS builds that stay on direct APNs, configure the gateway host separately with `DAISYCLAW_APNS_TEAM_ID`, `DAISYCLAW_APNS_KEY_ID`, and either `DAISYCLAW_APNS_PRIVATE_KEY_P8` or `DAISYCLAW_APNS_PRIVATE_KEY_PATH`. Those gateway runtime env vars are separate from Fastlane's `.env`.
...
- confirm `apps/ios/build/beta/DaisyClaw-<version>.ipa` exists
...
- The beta flow regenerates `apps/ios/DaisyClaw.xcodeproj` from `apps/ios/project.yml` before archiving
