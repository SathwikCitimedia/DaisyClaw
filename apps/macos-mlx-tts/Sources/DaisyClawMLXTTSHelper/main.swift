...
enum DaisyClawMLXTTSHelper {
    static func main() async {
        do {
            ...
            FileHandle.standardError.write(Data("daisyclaw-mlx-tts: \(error)\n".utf8))
            exit(1)
        }
    }
    ...
    private enum Usage: Error, CustomStringConvertible {
        ...
        var description: String {
            switch self {
            case .requested:
                "usage: daisyclaw-mlx-tts --text <text> --output <wav> [--model <hf-repo>] [--language <id>] [--voice <name>]"
