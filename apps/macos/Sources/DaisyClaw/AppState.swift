...
final class AppState {
    private static let logger = Logger(subsystem: "ai.daisyclaw", category: "app-state")
    ...
    init(preview: Bool = false) {
        ...
        let configRoot = DaisyClawConfigFile.loadDict()
    ...
    private static func sshTunnelGatewayUrl(existingUrl: String?, expectedRemoteHost: String?) -> String {
        ...
        let preservePort: Bool = if LoopbackHost.isLoopbackHost(host) {
            ...
            DaisyClawConfigFile.canonicalHostForComparison(host) ==
                DaisyClawConfigFile.canonicalHostForComparison(expectedRemoteHost)
        } else {
            false
        }
    ...
    private func startConfigWatcher() {
        let configUrl = DaisyClawConfigFile.url()
    ...
    func syncGatewayConfigNow() {
        ...
        let synced = Self.syncedGatewayRoot(
            currentRoot: DaisyClawConfigFile.loadDict(),
            draft: .init(
        ...
        guard DaisyClawConfigFile.saveDict(synced.root) else {
...
extension AppState {
    static var preview: AppState {
        ...
        state.remoteProjectRoot = "~/Projects/daisyclaw"
        state.remoteCliPath = ""
        return state
    }
}
