...
enum CommandResolver {
    private static let projectRootDefaultsKey = "daisyclaw.gatewayProjectRootPath"
    private static let helperName = "daisyclaw"
    ...
    static func gatewayEntrypoint(in root: URL) -> String? {
        ...
        let daisyclawEntry = root.appendingPathComponent("daisyclaw.mjs").path
        if FileManager().isReadableFile(atPath: daisyclawEntry) { return daisyclawEntry }
        let binEntry = root.appendingPathComponent("bin/daisyclaw.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/daisyclaw")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let daisyclawPaths = self.daisyclawManagedPaths(home: home)
        if !daisyclawPaths.isEmpty {
            extras.insert(contentsOf: daisyclawPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + daisyclawPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func daisyclawManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".daisyclaw"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func daisyclawExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectDaisyClawExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("daisyclaw.mjs").path,
            root.appendingPathComponent("bin/daisyclaw.js").path,
        ]
    ...
    static func hasAnyDaisyClawInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.daisyclawExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func daisyclawNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let daisyclawPath = self.projectDaisyClawExecutable(projectRoot: root) {
            return [daisyclawPath, subcommand] + extraArgs
        }
        if let daisyclawPath = self.daisyclawExecutable(searchPaths: searchPaths) {
            return [daisyclawPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "daisyclaw", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            daisyclaw entrypoint missing (looked for dist/index.js or daisyclaw.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real daisyclaw CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/daisyclaw"
        ...
        if command -v daisyclaw >/dev/null 2>&1; then
          CLI="$(command -v daisyclaw)"
          daisyclaw \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/daisyclaw.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/daisyclaw.mjs"
            node "$PRJ/daisyclaw.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/daisyclaw.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/daisyclaw.js"
            node "$PRJ/bin/daisyclaw.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent daisyclaw"
          pnpm --silent daisyclaw \(quotedArgs);
        else
          echo "daisyclaw CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? DaisyClawConfigFile.loadDict()
