...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: DaisyClawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.daisyclaw", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: DaisyClawKit.AnyCodable]()) {
                $0[$1.key] = DaisyClawKit.AnyCodable($1.value.base)
            }
        ...
        if let urlError = error as? URLError {
            ...
            case .notConnectedToInternet:
                if Self.isLikelyLocalNetworkPermissionBlock() {
                    return """
                    macOS is blocking DaisyClaw Local Network access.
                    Allow DaisyClaw in System Settings → Privacy & Security → Local Network, then relaunch the app.
                    """
                }
    ...
    private static func bridgeToProtocolArgs(
        _ value: DaisyClawProtocol.AnyCodable?) -> [String: DaisyClawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: DaisyClawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: DaisyClawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: DaisyClawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("daisyclaw.control.heartbeat")
    static let controlAgentEvent = Notification.Name("daisyclaw.control.agent")
}
