...
private let deepLinkLogger = Logger(subsystem: "ai.daisyclaw", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "DaisyClaw is paused", message: "Unpause DaisyClaw to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run DaisyClaw agent?", message: body) else { return }
        }
