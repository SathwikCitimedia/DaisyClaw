...
final class DevicePairingApprovalPrompter {
    ...
    private let logger = Logger(subsystem: "ai.daisyclaw", category: "device-pairing")
    ...
    static func alertSummary(for req: PendingRequest) -> String {
        ...
        return "Approve \(subject) to control DaisyClaw. Only approve if this is yours; you can remove it later in Settings."
    }
    ...
    static func deviceName(for req: PendingRequest) -> String {
        ...
        return self.isMac(req.platform) ? "DaisyClaw Mac app" : "New device"
    }
