...
enum GatewayDiscoverySelectionSupport {
        ...
        state: AppState)
    {
        ...
        if preferredTransport == .direct {
            DaisyClawConfigFile.setRemoteGatewayTransport(AppState.RemoteTransport.direct.rawValue)
            if !state.remoteUrl.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                DaisyClawConfigFile.setRemoteGatewayUrlString(state.remoteUrl)
            } else {
                DaisyClawConfigFile.clearRemoteGatewayUrl()
            }
        } else {
            DaisyClawConfigFile.setRemoteGatewayTransport(AppState.RemoteTransport.ssh.rawValue)
            DaisyClawConfigFile.setRemoteGatewayUrlString(state.remoteUrl)
        }
    }
