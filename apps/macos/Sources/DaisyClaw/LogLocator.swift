...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["DAISYCLAW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/daisyclaw")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("daisyclaw-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("daisyclaw-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/daisyclaw/ (rolling or stdout), or nil if none exist.
