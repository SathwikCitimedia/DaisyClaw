...
enum DaisyClawLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = DaisyClawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = DaisyClawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        DaisyClawLogging.bootstrapIfNeeded()
        let label = DaisyClawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct DaisyClawOSLogHandler: AppLogLevelBackedHandler {
...
struct DaisyClawFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = DaisyClawLogging.parseLabel(self.label)
