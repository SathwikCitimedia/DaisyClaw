...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.daisyclaw", category: "mac-node")
    ...
    private func run() async {
        ...
        while !Task.isCancelled {
            ...
            let browserControlEnabled = DaisyClawConfigFile.browserControlEnabled()
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: DaisyClawNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    nonisolated static func resolvedCaps(
        ...
        cameraEnabled: Bool,
        locationMode: DaisyClawLocationMode,
        connectionMode: AppState.ConnectionMode) -> [String]
    {
        ...
        if browserControlEnabled, connectionMode == .local {
            caps.append(DaisyClawCapability.browser.rawValue)
        }
        if cameraEnabled {
            caps.append(DaisyClawCapability.camera.rawValue)
        }
        if locationMode != .off {
            caps.append(DaisyClawCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCaps() -> [String] {
        ...
        return Self.resolvedCaps(
            browserControlEnabled: DaisyClawConfigFile.browserControlEnabled(),
            cameraEnabled: UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false,
            locationMode: DaisyClawLocationMode(rawValue: rawLocationMode) ?? .off,
            connectionMode: AppStateStore.shared.connectionMode)
    }
    ...
    nonisolated static func resolvedCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            DaisyClawSystemCommand.which.rawValue,
            DaisyClawSystemCommand.run.rawValue,
            DaisyClawSystemCommand.execApprovalsGet.rawValue,
            DaisyClawSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(DaisyClawCapability.browser.rawValue) {
            commands.append(DaisyClawBrowserCommand.proxy.rawValue)
        }
        if capsSet.contains(DaisyClawCapability.camera.rawValue) {
        ...
        if capsSet.contains(DaisyClawCapability.location.rawValue) {
    ...
    private func buildSessionBox(url: URL, connectionMode: AppState.ConnectionMode) -> WebSocketSessionBox? {
        ...
        guard let params = Self.tlsParams(
            ...
            root: DaisyClawConfigFile.loadDict(),
