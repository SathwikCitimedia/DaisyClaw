        ...
        return HStack(alignment: .center, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text(complete ? "All access granted" : "\(granted) of \(total) permissions granted")
                    .font(.headline)
                Text("DaisyClaw only asks for macOS capabilities when a feature needs them.")
                    .font(.footnote)
...
private struct LocationAccessSettings: View {
    ...
    @AppStorage(locationModeKey) private var locationModeRaw: String = DaisyClawLocationMode.off.rawValue
    @AppStorage(locationPreciseKey) private var locationPreciseEnabled: Bool = true
    @State private var lastLocationModeRaw: String = DaisyClawLocationMode.off.rawValue
    ...
    private var locationMode: DaisyClawLocationMode {
        DaisyClawLocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: DaisyClawLocationMode) async -> Bool {
