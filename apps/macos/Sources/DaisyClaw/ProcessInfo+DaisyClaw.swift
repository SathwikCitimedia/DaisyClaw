...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.daisyclaw.mac`) even if the shipped
    ...
    {
        if environment["DAISYCLAW_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "daisyclaw.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "daisyclaw.nixMode") { return true }
