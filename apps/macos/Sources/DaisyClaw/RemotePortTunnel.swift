...
final class RemotePortTunnel: @unchecked Sendable {
    private static let logger = Logger(subsystem: "ai.daisyclaw", category: "remote.tunnel")
    ...
    private static func resolveRemotePortOverride(defaultRemotePort: Int, for sshHost: String) -> Int? {
        ...
        guard let sshKey = DaisyClawConfigFile.canonicalHostForComparison(sshHost),
              let urlKey = DaisyClawConfigFile.canonicalHostForComparison(host)
        else {
            return nil
        }
    ...
    private static func findPort(preferred: UInt16?, allowRandom: Bool) async throws -> UInt16 {
        ...
        return try await withCheckedThrowingContinuation { cont in
            let queue = DispatchQueue(label: "ai.daisyclaw.remote.tunnel.port", qos: .utility)
            do {
