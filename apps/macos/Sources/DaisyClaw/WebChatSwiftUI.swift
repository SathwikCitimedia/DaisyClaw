...
private let webChatSwiftLogger = Logger(subsystem: "ai.daisyclaw", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "daisyclaw.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: DaisyClawChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> DaisyClawChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(DaisyClawChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            DaisyClawChatSessionsDefaults(
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> DaisyClawChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                DaisyClawGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    DaisyClawChatEventPayload.self,
            ...
            case "session.message":
                ...
                guard let message = try? JSONDecoder().decode(
                    DaisyClawSessionMessageEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    DaisyClawAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: DaisyClawProtocol.ModelChoice) -> DaisyClawChatModelChoice {
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<DaisyClawChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any DaisyClawChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = DaisyClawChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: DaisyClawChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<DaisyClawChatView>) -> NSViewController
    {
