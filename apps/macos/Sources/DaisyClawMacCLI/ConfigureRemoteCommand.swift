...
private let appDefaultsSuites = ["ai.daisyclaw.mac", "ai.daisyclaw.mac.debug"]
...
func runConfigureRemote(_ args: [String]) {
    do {
        ...
        if opts.help {
            ...
            Usage:
              daisyclaw-mac configure-remote --ssh-target <user@host[:port]> [--local-port <port>]
                                          [--remote-port <port>] [--token <token>] [--password <password>]
                                          [--identity <path>] [--project-root <path>] [--cli-path <path>] [--json]
              daisyclaw-mac configure-remote --direct-url <ws://host:port|wss://host> [--token <token>]
            ...
            Options:
              ...
              --project-root <p>  Remote DaisyClaw checkout for CLI commands.
              --cli-path <path>   Remote daisyclaw executable or entrypoint.
              --json              Emit JSON.
              -h, --help          Show help.
            """)
            return
...
private func configureSSHRemote(_ opts: ConfigureRemoteOptions) throws -> ConfigureRemoteOutput {
    ...
    let configURL = daisyClawConfigURL()
...
private func daisyClawConfigURL() -> URL {
    if let raw = ProcessInfo.processInfo.environment["DAISYCLAW_CONFIG_PATH"],
       !raw.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    {
        return URL(fileURLWithPath: NSString(string: raw).expandingTildeInPath)
    }
    return FileManager().homeDirectoryForCurrentUser.appendingPathComponent(".daisyclaw/daisyclaw.json")
}
...
private func writeAppDefaults(opts: ConfigureRemoteOptions, target: String) {
    for suite in appDefaultsSuites {
        ...
        defaults.set("remote", forKey: "daisyclaw.connectionMode")
        setDefaultString(defaults, key: "daisyclaw.remoteTarget", value: target)
        defaults.set(true, forKey: "daisyclaw.onboardingSeen")
        defaults.set(appOnboardingVersion, forKey: "daisyclaw.onboardingVersion")
        setDefaultStringIfProvided(defaults, key: "daisyclaw.remoteIdentity", value: opts.identity)
        setDefaultStringIfProvided(defaults, key: "daisyclaw.remoteProjectRoot", value: opts.projectRoot)
        setDefaultStringIfProvided(defaults, key: "daisyclaw.remoteCliPath", value: opts.cliPath)
...
private func printConfigureRemoteOutput(_ output: ConfigureRemoteOutput, json: Bool) {
    ...
    print("DaisyClaw macOS Remote Config")
