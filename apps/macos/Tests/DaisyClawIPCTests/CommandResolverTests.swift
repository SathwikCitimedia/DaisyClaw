...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func `prefers open claw binary`() throws {
        ...
        let cmd = CommandResolver.daisyclawCommand(
            subcommand: "gateway",
            defaults: defaults,
            configRoot: [:],
            searchPaths: searchPaths,
            projectRoot: tmp)
        #expect(cmd.prefix(2).elementsEqual([daisyclawPath.path, "gateway"]))
    }
    ...
    @Test func `falls back to node and script`() throws {
        ...
        let scriptPath = tmp.appendingPathComponent("bin/daisyclaw.js")
    ...
    @Test func `prefers open claw binary over pnpm`() throws {
        ...
        let daisyclawPath = binDir.appendingPathComponent("daisyclaw")
        ...
        #expect(cmd.prefix(2).elementsEqual([daisyclawPath.path, "rpc"]))
    }
    ...
    @Test func `falls back to pnpm`() throws {
        ...
        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "daisyclaw", "rpc"]))
    }
    ...
    @Test func `pnpm keeps extra args after subcommand`() throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "daisyclaw", "health", "--json"]))
    ...
    @Test func `builds SSH command for remote mode`() {
        ...
        defaults.set("daisyclaw@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/daisyclaw", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "daisyclaw@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/daisyclaw'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("daisyclaw"))
    ...
    @Test func `config root local overrides remote defaults`() throws {
        ...
        #expect(cmd.first == daisyclawPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
