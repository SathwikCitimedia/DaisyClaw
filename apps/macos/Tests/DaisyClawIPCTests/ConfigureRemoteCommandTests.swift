import Foundation
import Testing
@testable import DaisyClawMacCLI
...
struct ConfigureRemoteCommandTests {
    @Test @MainActor func `configure remote writes ssh config and app defaults`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("daisyclaw-configure-remote-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }

        let defaultSuites = ["ai.daisyclaw.mac", "ai.daisyclaw.mac.debug"]
        let keys = [
            "daisyclaw.connectionMode",
            "daisyclaw.remoteTarget",
            "daisyclaw.onboardingSeen",
            "daisyclaw.onboardingVersion",
            "daisyclaw.remoteCliPath",
        ]
        ...
        try await TestIsolation.withIsolatedState(env: ["DAISYCLAW_CONFIG_PATH": configURL.path]) {
            ...
            for (_, defaults) in defaultsBySuite {
                #expect(defaults.string(forKey: "daisyclaw.connectionMode") == "remote")
                #expect(defaults.string(forKey: "daisyclaw.remoteTarget") == "alice@gateway.example")
                #expect(defaults.bool(forKey: "daisyclaw.onboardingSeen") == true)
                #expect(defaults.string(forKey: "daisyclaw.remoteCliPath") == "/opt/homebrew/bin/daisyclaw")
            }
        }
    }
    ...
    @Test @MainActor func `configure remote preserves existing optional credentials when flags omitted`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("daisyclaw-configure-remote-preserve-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote can write direct private url`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("daisyclaw-configure-direct-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote rejects plaintext public prefix bypass`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("daisyclaw-configure-direct-reject-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }

        _ = await TestIsolation.withIsolatedState(env: ["DAISYCLAW_CONFIG_PATH": configURL.path]) {
            #expect(throws: Error.self) {
