...
@Suite(.serialized)
struct DaisyClawConfigFileTests {
    private func makeConfigOverridePath() -> String {
        FileManager().temporaryDirectory
            .appendingPathComponent("daisyclaw-config-\(UUID().uuidString)")
            .appendingPathComponent("daisyclaw.json")
            .path
    }
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["DAISYCLAW_CONFIG_PATH": override]) {
            #expect(DaisyClawConfigFile.url().path == override)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["DAISYCLAW_CONFIG_PATH": override]) {
            ...
            #expect(DaisyClawConfigFile.remoteGatewayPort() == 19999)
            #expect(DaisyClawConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(DaisyClawConfigFile.remoteGatewayPort(matchingHost: "GATEWAY.ts.net.") == 19999)
            #expect(DaisyClawConfigFile.remoteGatewayPort(matchingHost: "gateway") == nil)
            #expect(DaisyClawConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
            #expect(DaisyClawConfigFile.remoteGatewayPort(matchingHost: "gateway.attacker.tld") == nil)
        }
    }
    ...
    func `set remote gateway url string replaces scheme`() async {
        ...
        await TestIsolation.withEnvValues(["DAISYCLAW_CONFIG_PATH": override]) {
            ...
            DaisyClawConfigFile.setRemoteGatewayUrlString("ws://127.0.0.1:18789")
            let root = DaisyClawConfigFile.loadDict()
    ...
    func `set remote gateway url preserves scheme`() async {
        ...
        await TestIsolation.withEnvValues(["DAISYCLAW_CONFIG_PATH": override]) {
            ...
            DaisyClawConfigFile.setRemoteGatewayUrl(host: "new-host", port: 2222)
            let root = DaisyClawConfigFile.loadDict()
            let url = ((root["gateway"] as? [String: Any])?["remote"] as? [String: Any])?["url"] as? String
    ...
    func `clear remote gateway url removes only url field`() async {
        ...
        await TestIsolation.withEnvValues(["DAISYCLAW_CONFIG_PATH": override]) {
            ...
            DaisyClawConfigFile.clearRemoteGatewayUrl()
            let root = DaisyClawConfigFile.loadDict()
    ...
    func `state dir override sets config path`() async {
        ...
        await TestIsolation.withEnvValues([
            "DAISYCLAW_CONFIG_PATH": nil,
            "DAISYCLAW_STATE_DIR": dir,
        ]) {
            #expect(DaisyClawConfigFile.stateDirURL().path == dir)
            #expect(DaisyClawConfigFile.url().path == "\(dir)/daisyclaw.json")
        }
    }
    ...
    func `save dict appends config audit log`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("daisyclaw-state-\(UUID().uuidString)", isDirectory: true)
        let configPath = stateDir.appendingPathComponent("daisyclaw.json")
        let auditPath = stateDir.appendingPathComponent("logs/config-audit.jsonl")

        defer { try? FileManager().removeItem(at: stateDir) }

        try await TestIsolation.withEnvValues([
            "DAISYCLAW_STATE_DIR": stateDir.path,
            "DAISYCLAW_CONFIG_PATH": configPath.path,
        ]) {
            DaisyClawConfigFile.saveDict([
                "gateway": ["mode": "local"],
            ])
            ...
            #expect(auditRoot?["source"] as? String == "macos-daisyclaw-config-file")
    ...
    func `save dict preserves gateway auth unless explicitly allowed`() async throws {
        ...
        await TestIsolation.withEnvValues([
            ...
            let allowedRoot = DaisyClawConfigFile.loadDict()
    ...
    func `load dict audits suspicious out-of-band clobbers`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            try DaisyClawConfigFile.withTestingFileLock {
                DaisyClawConfigFile.saveDict([
                    "update": ["channel": "beta"],
                ...
                _ = DaisyClawConfigFile.loadDict()
                ...
                let loaded = DaisyClawConfigFile.loadDict()
    ...
    func `save dict records preserved gateway auth in audit`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            let saved = DaisyClawConfigFile.saveDict([
                "gateway": [
                    "mode": "local",
                ],
                "browser": [
