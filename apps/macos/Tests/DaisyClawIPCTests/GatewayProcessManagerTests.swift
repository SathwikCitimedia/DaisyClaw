...
struct GatewayProcessManagerTests {
    ...
    @Test func `attaches to existing gateway without spawning launchd`() async throws {
        let port = 19097
        try await TestIsolation.withEnvValues(["DAISYCLAW_GATEWAY_PORT": "\(port)"]) {
            let healthData = Data(
                """
                {
                  "ok": true,
            ...
            let descriptor = PortGuardian.Descriptor(
                pid: 4242,
                command: "daisyclaw-gateway",
                executablePath: "/tmp/daisyclaw-gateway")
            ...
            do {
                ...
                #expect(details.contains("pid 4242 daisyclaw-gateway @ /tmp/daisyclaw-gateway"))
                await cleanup()
            } catch {
                await cleanup()
                throw error
