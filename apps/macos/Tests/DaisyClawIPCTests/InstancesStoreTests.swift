...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: DaisyClawProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: DaisyClawProtocol.AnyCodable] = [
            "presence": .init([DaisyClawProtocol.AnyCodable(entry)]),
        ]
        let payload = DaisyClawProtocol.AnyCodable(payloadMap)
