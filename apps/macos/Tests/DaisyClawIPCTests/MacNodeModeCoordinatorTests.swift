...
struct MacNodeModeCoordinatorTests {
    @Test func `remote mode does not advertise browser proxy`() {
        ...
        #expect(!caps.contains(DaisyClawCapability.browser.rawValue))
        #expect(!commands.contains(DaisyClawBrowserCommand.proxy.rawValue))
        #expect(commands.contains(DaisyClawCanvasCommand.present.rawValue))
