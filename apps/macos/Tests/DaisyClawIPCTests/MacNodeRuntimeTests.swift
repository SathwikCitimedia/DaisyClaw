...
struct MacNodeRuntimeTests {
    ...
    final class ScreenSnapshotProbeServices: MacNodeRuntimeMainActorServices, @unchecked Sendable {
        typealias SnapshotResult = (
            data: Data,
            format: DaisyClawScreenSnapshotFormat,
            width: Int,
            height: Int)
        ...
        func snapshotScreen(
            ...
            maxWidth: Int?,
            quality: Double?,
            format: DaisyClawScreenSnapshotFormat?) async throws -> SnapshotResult
        {
            ...
            outPath: String?) async throws -> (path: String, hasAudio: Bool)
        {
            let url = FileManager().temporaryDirectory
                .appendingPathComponent("daisyclaw-test-screen-record-\(UUID().uuidString).mp4")
    ...
    @Test func `A2UI host capability refresh uses injected node session refresher`() async {
        ...
        #expect(current == "http://127.0.0.1:18789/current/__daisyclaw__/a2ui/?platform=macos")
        #expect(await probe.calls == 0)

        let refreshed = await runtime.resolveA2UIHostUrlWithCapabilityRefresh(forceRefresh: true)
        #expect(refreshed == "http://127.0.0.1:18789/refreshed/__daisyclaw__/a2ui/?platform=macos")
        #expect(await probe.calls == 1)
    }

    @Test func `handle invoke rejects empty system run`() async throws {
        let runtime = MacNodeRuntime()
        let params = DaisyClawSystemRunParams(command: [])
        let json = try String(data: JSONEncoder().encode(params), encoding: .utf8)
        let response = await runtime.handleInvoke(
            BridgeInvokeRequest(id: "req-2", command: DaisyClawSystemCommand.run.rawValue, paramsJSON: json))
        #expect(response.ok == false)
    }

    @Test func `system run denied event preserves gateway run id`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("daisyclaw-state-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager().removeItem(at: stateDir) }

        try await TestIsolation.withEnvValues(["DAISYCLAW_STATE_DIR": stateDir.path]) {
            let probe = ExecEventProbe()
            let runtime = MacNodeRuntime()
            await runtime.setEventSender { event, json in
                await probe.append(event: event, json: json)
            }
            let params = DaisyClawSystemRunParams(
            ...
            let response = await runtime.handleInvoke(
                BridgeInvokeRequest(
                    id: "req-run-id",
                    command: DaisyClawSystemCommand.run.rawValue,
                    paramsJSON: json))
    ...
    @Test func `handle invoke rejects blocked system run env override before execution`() async throws {
        ...
        let json = try String(data: JSONEncoder().encode(params), encoding: .utf8)
        let response = await runtime.handleInvoke(
            BridgeInvokeRequest(id: "req-2c", command: DaisyClawSystemCommand.run.rawValue, paramsJSON: json))
        #expect(response.ok == false)
    ...
    @Test func `handle invoke rejects invalid system run env override key before execution`() async throws {
        ...
        let json = try String(data: JSONEncoder().encode(params), encoding: .utf8)
        let response = await runtime.handleInvoke(
            BridgeInvokeRequest(id: "req-2d", command: DaisyClawSystemCommand.run.rawValue, paramsJSON: json))
        #expect(response.ok == false)
        #expect(response.error?.message.contains("SYSTEM_RUN_DENIED: environment override rejected") == true)
    ...
    @Test func `handle invoke rejects empty system which`() async throws {
        let runtime = MacNodeRuntime()
        let params = DaisyClawSystemWhichParams(bins: [])
        let json = try String(data: JSONEncoder().encode(params), encoding: .utf8)
        let response = await runtime.handleInvoke(
            BridgeInvokeRequest(id: "req-2b", command: DaisyClawSystemCommand.which.rawValue, paramsJSON: json))
        #expect(response.ok == false)
    }

    @Test func `handle invoke rejects empty notification`() async throws {
        let runtime = MacNodeRuntime()
        let params = DaisyClawSystemNotifyParams(title: "", body: "")
        let json = try String(data: JSONEncoder().encode(params), encoding: .utf8)
        let response = await runtime.handleInvoke(
            BridgeInvokeRequest(id: "req-3", command: DaisyClawSystemCommand.notify.rawValue, paramsJSON: json))
        #expect(response.ok == false)
    }
    ...
    @Test func `handle invoke camera list requires enabled camera`() async {
        await TestIsolation.withUserDefaultsValues([cameraEnabledKey: false]) {
            let runtime = MacNodeRuntime()
            let response = await runtime.handleInvoke(
                BridgeInvokeRequest(id: "req-4", command: DaisyClawCameraCommand.list.rawValue))
            #expect(response.ok == false)
    ...
    @Test func `handle invoke screen record uses injected services`() async throws {
        ...
        final class FakeMainActorServices: MacNodeRuntimeMainActorServices, @unchecked Sendable {
            func snapshotScreen(
                ...
                maxWidth: Int?,
                quality: Double?,
                format: DaisyClawScreenSnapshotFormat?) async throws
                -> (data: Data, format: DaisyClawScreenSnapshotFormat, width: Int, height: Int)
            {
                _ = screenIndex
                _ = maxWidth
                _ = quality
    ...
    @Test func `handle invoke browser proxy uses injected request`() async {
        ...
        let response = await runtime.handleInvoke(
            BridgeInvokeRequest(
                id: "req-browser",
                command: DaisyClawBrowserCommand.proxy.rawValue,
    ...
    @Test func `handle invoke browser proxy rejects disabled browser control`() async throws {
        ...
        try await TestIsolation.withEnvValues(["DAISYCLAW_CONFIG_PATH": override]) {
