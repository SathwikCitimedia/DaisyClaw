...
import Foundation
import Testing
@testable import DaisyClawDiscovery
...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() {
        let originalWideAreaDomain = getenv("DAISYCLAW_WIDE_AREA_DOMAIN").map { String(cString: $0) }
        setenv("DAISYCLAW_WIDE_AREA_DOMAIN", "daisyclaw.internal", 1)
        defer {
            if let originalWideAreaDomain {
                setenv("DAISYCLAW_WIDE_AREA_DOMAIN", originalWideAreaDomain, 1)
            } else {
                unsetenv("DAISYCLAW_WIDE_AREA_DOMAIN")
            }
        }
        let statusJson = """
        {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._daisyclaw-gw._tcp.daisyclaw.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.daisyclaw.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (DaisyClaw)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/daisyclaw/src/entry.ts\""
                }
                return ""
            })
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (DaisyClaw)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/daisyclaw/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._daisyclaw-gw._tcp.daisyclaw.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
