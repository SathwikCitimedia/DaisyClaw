...
public struct DaisyClawChatTalkControl {
...
@MainActor
struct DaisyClawChatComposer: View {
    @Bindable var viewModel: DaisyClawChatViewModel
    let style: DaisyClawChatView.Style
    let showsSessionSwitcher: Bool
    let userAccent: Color?
    let assistantName: String?
    let assistantAvatarText: String?
    let assistantAvatarTint: Color?
    let composerChrome: DaisyClawChatView.ComposerChrome
    let isComposerEnabled: Bool
    let messagePlaceholder: String?
    let talkControl: DaisyClawChatTalkControl?
            ...
            if self.composerChrome == .full {
                ...
                if self.style == .standard {
                    let shape = UnevenRoundedRectangle(
                        ...
                        style: .continuous)
                    shape
                        .fill(DaisyClawChatTheme.composerBackground)
                        .overlay(shape.strokeBorder(DaisyClawChatTheme.composerBorder, lineWidth: 1))
    ...
    private var modelPicker: some View {
        ...
        {
            Text(self.viewModel.defaultModelLabel).tag(DaisyClawChatViewModel.defaultModelSelectionID)
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \DaisyClawPendingAttachment.id)
                { (att: DaisyClawPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            DaisyClawPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var fullEditor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(DaisyClawChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 14, style: .continuous)
                .fill(DaisyClawChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 14, style: .continuous)
                        .strokeBorder(DaisyClawChatTheme.composerBorder)))
    ...
    private func talkButton(_ talkControl: DaisyClawChatTalkControl) -> some View {
        Button {
    ...
    private func compactTalkButton(_ talkControl: DaisyClawChatTalkControl) -> some View {
        Button {
            talkControl.toggle(self.viewModel.sessionKey)
        } label: {
            Image(systemName: talkControl.isEnabled ? "stop.fill" : "waveform")
    ...
    private func talkButtonFill(_ talkControl: DaisyClawChatTalkControl) -> AnyShapeStyle {
        if talkControl.isEnabled {
            return AnyShapeStyle(DaisyClawChatTheme.userBubble)
        }
        if !talkControl.isGatewayConnected {
            return AnyShapeStyle(Color.secondary.opacity(0.12))
        }
        return DaisyClawChatTheme.subtleCard
    }

    private func talkButtonStroke(_ talkControl: DaisyClawChatTalkControl) -> Color {
        if talkControl.isEnabled {
            return Color.white.opacity(0.18)
        }
        return DaisyClawChatTheme.composerBorder
    }

    private func talkAccessibilityValue(_ talkControl: DaisyClawChatTalkControl) -> String {
    ...
    private func talkHelpText(_ talkControl: DaisyClawChatTalkControl) -> String {
    ...
    private var connectionPill: some View {
        ...
        .background {
            if self.composerChrome == .full {
                Capsule()
                    .fill(DaisyClawChatTheme.subtleCard)
            }
        }
    }
    ...
    private var sendButtonFill: Color {
        self.userAccent ?? DaisyClawChatTheme.userBubble
    }
