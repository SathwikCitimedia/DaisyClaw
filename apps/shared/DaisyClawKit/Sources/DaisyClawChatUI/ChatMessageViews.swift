...
struct ChatMessageBubble: View {
    let message: DaisyClawChatMessage
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? DaisyClawChatTheme.userText : DaisyClawChatTheme.assistantText
    ...
    private var inlineAttachments: [DaisyClawChatMessageContent] {
    ...
    private var toolCalls: [DaisyClawChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [DaisyClawChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? DaisyClawChatTheme.userBubble
        }
        if self.style == .onboarding {
            return DaisyClawChatTheme.onboardingAssistantBubble
        }
        return DaisyClawChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return DaisyClawChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: DaisyClawChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? DaisyClawChatTheme.userText : DaisyClawChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: DaisyClawChatMessageContent
    let isUser: Bool
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [DaisyClawChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: DaisyClawChatTheme.assistantText)
            }
        }
    }
}
