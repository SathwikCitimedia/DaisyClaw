...
@MainActor
public struct DaisyClawChatView: View {
    public enum Style {
        case standard
        case onboarding
    }
    ...
    @State private var viewModel: DaisyClawChatViewModel
    ...
    private let talkControl: DaisyClawChatTalkControl?
    ...
    public init(
        viewModel: DaisyClawChatViewModel,
        ...
        talkControl: DaisyClawChatTalkControl? = nil)
    {
    ...
    private var visibleMessages: [DaisyClawChatMessage] {
        let base: [DaisyClawChatMessage]
    ...
    private func mergeToolResults(in messages: [DaisyClawChatMessage]) -> [DaisyClawChatMessage] {
        ...
        for message in messages {
            ...
            let merged = DaisyClawChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: DaisyClawChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: DaisyClawChatMessage) -> Bool {
    ...
    private func primaryText(in message: DaisyClawChatMessage) -> String {
        let parts = message.content.compactMap { content -> String? in
            let kind = (content.type ?? "text").lowercased()
            guard kind == "text" || kind.isEmpty else { return nil }
            return content.text
        }
        return DaisyClawChatMessage.displayText(
    ...
    private func hasInlineAttachments(in message: DaisyClawChatMessage) -> Bool {
    ...
    private func toolCalls(in message: DaisyClawChatMessage) -> [DaisyClawChatMessageContent] {
    ...
    private func inlineToolResults(in message: DaisyClawChatMessage) -> [DaisyClawChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: DaisyClawChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: DaisyClawChatMessage) -> String {
...
private struct ChatAssistantIntroCard: View {
    ...
    var body: some View {
        Text(self.text)
            .font(.system(size: 15))
            .lineSpacing(4)
            .foregroundStyle(DaisyClawChatTheme.assistantText)
            .multilineTextAlignment(.leading)
            .padding(.vertical, 12)
            .padding(.horizontal, 14)
            .background(
                RoundedRectangle(cornerRadius: 16, style: .continuous)
                    .fill(DaisyClawChatTheme.assistantBubble)
                    .overlay(
                        RoundedRectangle(cornerRadius: 16, style: .continuous)
