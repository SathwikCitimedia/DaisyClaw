...
private let chatUILogger = Logger(subsystem: "ai.daisyclaw", category: "DaisyClawChatUI")

@MainActor
@Observable
// swiftlint:disable:next type_body_length
public final class DaisyClawChatViewModel {
    public static let defaultModelSelectionID = "__default__"
    static let maxAttachmentBytes = 5_000_000

    public private(set) var messages: [DaisyClawChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var thinkingLevelOptions: [DaisyClawChatThinkingLevelOption]
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [DaisyClawChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [DaisyClawPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [DaisyClawChatPendingToolCall] = []
    public private(set) var sessions: [DaisyClawChatSessionEntry] = []
    private let transport: any DaisyClawChatTransport
    private var sessionDefaults: DaisyClawChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: DaisyClawChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any DaisyClawChatTransport,
    ...
    public var sessionChoices: [DaisyClawChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [DaisyClawChatSessionEntry] = []
    ...
    private static let baseThinkingLevelOptions: [DaisyClawChatThinkingLevelOption] = [
        DaisyClawChatThinkingLevelOption(id: "off", label: "off"),
        DaisyClawChatThinkingLevelOption(id: "minimal", label: "minimal"),
        DaisyClawChatThinkingLevelOption(id: "low", label: "low"),
        DaisyClawChatThinkingLevelOption(id: "medium", label: "medium"),
        DaisyClawChatThinkingLevelOption(id: "high", label: "high"),
    ]
    ...
    public func removeAttachment(_ id: DaisyClawPendingAttachment.ID) {
    ...
    private func applyHistoryPayload(
        _ payload: DaisyClawChatHistoryPayload,
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [DaisyClawChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: DaisyClawChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: DaisyClawChatMessage) -> DaisyClawChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> DaisyClawChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return DaisyClawChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: DaisyClawChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: DaisyClawChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: DaisyClawChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private func adoptPendingLocalUserEcho(incoming: DaisyClawChatMessage) -> Bool {
        ...
        updated[matchIndex] = DaisyClawChatMessage(
            id: existing.id,
            role: incoming.role,
    ...
    private static func reconcileMessageIDs(
        previous: [DaisyClawChatMessage],
        incoming: [DaisyClawChatMessage]) -> [DaisyClawChatMessage]
    {
    ...
    private static func reconcileRunRefreshMessages(
        previous: [DaisyClawChatMessage],
        incoming: [DaisyClawChatMessage],
        pendingLocalUserEchoIDs: Set<UUID>) -> [DaisyClawChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [DaisyClawChatMessage]) -> [DaisyClawChatMessage] {
        var result: [DaisyClawChatMessage] = []
    ...
    private static func dedupeKey(for message: DaisyClawChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [DaisyClawChatMessageContent] = [
            DaisyClawChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> DaisyClawChatAttachmentPayload in
            DaisyClawChatAttachmentPayload(
                type: att.type,
    ...
    private static func isUnsupportedCreateSessionError(_ error: Error) -> Bool {
        ...
        return nsError.domain == "DaisyClawChatTransport"
    ...
    private func resolvedThinkingLevelOptions(
        for currentSession: DaisyClawChatSessionEntry?) -> [DaisyClawChatThinkingLevelOption]
    {
    ...
    private static func sessionModelMatchesDefaults(
        _ session: DaisyClawChatSessionEntry,
        defaults: DaisyClawChatSessionsDefaults?) -> Bool
    {
    ...
    private static func normalizedThinkingLevelOptions(
        _ levels: [DaisyClawChatThinkingLevelOption]?) -> [DaisyClawChatThinkingLevelOption]?
    {
        ...
        return Self.dedupedThinkingOptions(
            levels.compactMap { level in
                ...
                return DaisyClawChatThinkingLevelOption(id: id, label: label.isEmpty ? id : label)
            })
    }

    private static func thinkingOptions(from labels: [String]?) -> [DaisyClawChatThinkingLevelOption]? {
        guard let labels else { return nil }
        return Self.dedupedThinkingOptions(
            labels.compactMap { label in
                guard let id = Self.normalizedThinkingLevel(label) else { return nil }
                let trimmed = label.trimmingCharacters(in: .whitespacesAndNewlines)
                return DaisyClawChatThinkingLevelOption(id: id, label: trimmed.isEmpty ? id : trimmed)
            })
    }

    private static func withCurrentThinkingOption(
        _ options: [DaisyClawChatThinkingLevelOption],
        current: String) -> [DaisyClawChatThinkingLevelOption]
    {
        guard !options.contains(where: { $0.id == current }) else { return options }
        return options + [DaisyClawChatThinkingLevelOption(id: current, label: current)]
    }

    private static func dedupedThinkingOptions(
        _ options: [DaisyClawChatThinkingLevelOption]) -> [DaisyClawChatThinkingLevelOption]
    {
        var result: [DaisyClawChatThinkingLevelOption] = []
        var seen = Set<String>()
    ...
    private func placeholderSession(key: String) -> DaisyClawChatSessionEntry {
        DaisyClawChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func updateCurrentSessionThinkingLevel(_ thinkingLevel: String?, sessionKey: String) {
        ...
        self.sessions[index] = DaisyClawChatSessionEntry(
            key: current.key,
            kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: DaisyClawChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleSessionMessageEvent(_ payload: DaisyClawSessionMessageEventPayload) {
    ...
    private func handleChatEvent(_ chat: DaisyClawChatEventPayload) {
    ...
    private func appendFinalChatMessageIfPresent(_ chat: DaisyClawChatEventPayload) {
        guard chat.state == "final" else { return }
        guard let text = DaisyClawChatEventText.assistantText(from: chat) else { return }

        let decoded = chat.message.flatMap {
            try? ChatPayloadDecoding.decode($0, as: DaisyClawChatMessage.self)
        }
    ...
    private static func isAssistantMessage(_ message: DaisyClawChatMessage) -> Bool {
        message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "assistant"
    }

    private static func messageWithTimestampIfNeeded(_ message: DaisyClawChatMessage) -> DaisyClawChatMessage {
    ...
    private func handleAgentEvent(_ evt: DaisyClawAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = DaisyClawChatPendingToolCall(
    ...
    private func handleAgentLifecycleEvent(_ evt: DaisyClawAgentEventPayload, isPendingRun: Bool) {
    ...
    private static func agentLifecycleErrorMessage(_ evt: DaisyClawAgentEventPayload, aborted: Bool) -> String {
        if aborted {
            return "Run aborted"
        }
    ...
    private static func hasUnansweredLatestUser(in messages: [DaisyClawChatMessage]) -> Bool {
        self.latestUserTurn(in: messages) != nil && !self.hasAssistantMessageAfterLatestUser(in: messages)
    }

    private static func latestUserTurn(in messages: [DaisyClawChatMessage]) -> LatestUserTurn? {
    ...
    private static func hasAnsweredUser(
        ...
        in messages: [DaisyClawChatMessage])
        -> Bool
    {
        guard let refreshKey = user.refreshKey else { return false }
        var occurrence = 0
        var latestMatchingUserIndex: [DaisyClawChatMessage].Index?
    ...
    private static func hasAssistantMessageAfterLatestUser(in messages: [DaisyClawChatMessage]) -> Bool {
        guard let lastUserIndex = messages.lastIndex(where: { $0.role.lowercased() == "user" }) else {
            return false
        }
