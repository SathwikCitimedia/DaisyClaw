...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> DaisyClawChatHistoryPayload
{
    DaisyClawChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> DaisyClawChatSessionEntry {
...
private func thinkingOption(_ id: String, label: String? = nil) -> DaisyClawChatThinkingLevelOption {
    DaisyClawChatThinkingLevelOption(id: id, label: label ?? id)
}
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> DaisyClawChatSessionEntry
{
    DaisyClawChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> DaisyClawChatModelChoice {
    DaisyClawChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [DaisyClawChatHistoryPayload],
    sessionsResponses: [DaisyClawChatSessionsListResponse] = [],
    modelResponses: [[DaisyClawChatModelChoice]] = [],
    ...
    -> (TestChatTransport, DaisyClawChatViewModel)
{
...
private func sendUserMessage(_ vm: DaisyClawChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, DaisyClawChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [DaisyClawChatHistoryPayload]
    private let sessionsResponses: [DaisyClawChatSessionsListResponse]
    private let modelResponses: [[DaisyClawChatModelChoice]]
    ...
    private let stream: AsyncStream<DaisyClawChatTransportEvent>
    private let continuation: AsyncStream<DaisyClawChatTransportEvent>.Continuation
        ...
        healthResponses: [Bool] = [true])
    {
        ...
        var cont: AsyncStream<DaisyClawChatTransportEvent>.Continuation!
        ...
        parentSessionKey: String?) async throws -> DaisyClawChatCreateSessionResponse
    {
        ...
        return DaisyClawChatCreateSessionResponse(ok: true, key: key, sessionId: "created-\(key)")
    }
    ...
    func requestHistory(sessionKey: String) async throws -> DaisyClawChatHistoryPayload {
        ...
        return self.historyResponses.last ?? DaisyClawChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [DaisyClawChatAttachmentPayload]) async throws -> DaisyClawChatSendResponse
    {
        await self.state.sentSessionKeysAppend(sessionKey)
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return DaisyClawChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> DaisyClawChatSessionsListResponse {
        let idx = await self.state.nextSessionsCallIndex()
        if idx < self.sessionsResponses.count {
            return self.sessionsResponses[idx]
        }
        return self.sessionsResponses.last ?? DaisyClawChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: DaisyClawChatTransportEvent) {
...
struct ChatViewModelTests {
    @Test func `displays error message fallback only for assistant error turns`() throws {
        func decodeMessage(role: String, stopReason: String, contentText: String? = nil) throws -> DaisyClawChatMessage {
        ...
        #expect(
            DaisyClawChatMessage.displayText(
                contentText: "",
                role: assistantError.role,
                stopReason: assistantError.stopReason,
                errorMessage: assistantError.errorMessage) == "stale provider failure")
    ...
    @Test @MainActor func `bounded repeated same text reply invalidates older stale refresh`() async throws {
        ...
        transport.emit(DaisyClawChatTransportEvent.seqGap)
    ...
    @Test func `appends external session user message for active session`() async throws {
        ...
        transport.emit(
            .sessionMessage(
                DaisyClawSessionMessageEventPayload(
                    sessionKey: "agent:aiden:main",
                    message: DaisyClawChatMessage(
                        role: "user",
                        content: [
    ...
    @Test func `session choices prefer main and recent`() async throws {
        ...
        let sessions = DaisyClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func `session choices use resolved main session key instead of literal main`() async throws {
        ...
        let sessions = DaisyClawChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: DaisyClawChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func `bootstraps model selection from session and defaults`() async throws {
        ...
        let sessions = DaisyClawChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: DaisyClawChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func `selecting default model patches nil and updates selection`() async throws {
        ...
        await MainActor.run { vm.selectModel(DaisyClawChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == DaisyClawChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func `selecting provider qualified model disambiguates duplicate model I ds`() async throws {
        ...
        let sessions = DaisyClawChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: DaisyClawChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func `late model completion does not replay current session selection into previous session`() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = DaisyClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = DaisyClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func `explicit thinking level wins over history and persists changes`() async throws {
        let history = DaisyClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func `decodes gateway thinking metadata from session list`() throws {
        ...
        let decoded = try JSONDecoder().decode(
            DaisyClawChatSessionsListResponse.self,
            from: Data(json.utf8))
    ...
    @Test func `strips inbound metadata from history messages`() async throws {
        ...
        let vm = await MainActor.run { DaisyClawChatViewModel(sessionKey: "main", transport: transport) }
