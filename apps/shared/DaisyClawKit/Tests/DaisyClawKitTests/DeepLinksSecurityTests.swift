...
@Suite struct DeepLinksSecurityTests {
    @Test func dashboardDeepLinkParses() {
        let url = URL(string: "daisyclaw://dashboard")!
    ...
    @Test func gatewayDeepLinkAllowsLoopbackWs() {
        let url = URL(
            string: "daisyclaw://gateway?host=127.0.0.1&port=18789&tls=0&token=abc")!
        #expect(
    ...
    @Test func setupCodeAllowsPrivateLanHostPayload() {
        let payload = #"{"host":"daisyclaw.local","port":18789,"tls":false,"bootstrapToken":"tok"}"#
        #expect(
            GatewayConnectDeepLink.fromSetupCode(setupCode(from: payload)) == .init(
                host: "daisyclaw.local",
                port: 18789,
