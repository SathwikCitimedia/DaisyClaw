...
const ensureSupportedNodeVersion = () => {
  ...
  process.stderr.write(
    `daisyclaw: Node.js v${MIN_NODE_VERSION}+ is required (current: v${process.versions.node}).\n` +
      "If you use nvm, run:\n" +
...
const runRespawnedChild = (command, args, env) => {
  ...
  child.once("error", (error) => {
    detach();
    process.stderr.write(
      `[daisyclaw] Failed to respawn launcher: ${
...
const respawnWithoutCompileCacheIfNeeded = () => {
  ...
  if (process.env.DAISYCLAW_SOURCE_COMPILE_CACHE_RESPAWNED === "1") {
    return false;
  }
...
const respawnWithPackagedCompileCacheIfNeeded = () => {
  ...
  if (process.env.DAISYCLAW_PACKAGED_COMPILE_CACHE_RESPAWNED === "1") {
    return false;
  }
  ...
  const env = {
    ...
    DAISYCLAW_PACKAGED_COMPILE_CACHE_RESPAWNED: "1",
  };
  return runRespawnedChild(
    process.execPath,
    [...process.execArgv, fileURLToPath(import.meta.url), ...process.argv.slice(2)],
...
const buildMissingEntryErrorMessage = async () => {
  const lines = ["daisyclaw: missing dist/entry.(m)js (build output)."];
  ...
  lines.push(
    "For pinned GitHub installs, use `npm install -g github:daisyclaw/daisyclaw#<ref>` instead of a raw `/archive/<ref>.tar.gz` URL.",
  );
  lines.push("For releases, use `npm install -g daisyclaw@latest`.");
  return lines.join("\n");
};
...
const isHelpFastPathDisabled = () =>
  process.env.DAISYCLAW_DISABLE_CLI_STARTUP_HELP_FAST_PATH === "1";
...
const resolveLauncherHomeDir = () => {
  const explicit = normalizeLauncherHomeValue(process.env.DAISYCLAW_HOME);
  const rawHome =
...
const resolveLauncherConfigPaths = () => {
  const explicit = process.env.DAISYCLAW_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveLauncherUserPath(explicit)];
  }
  const stateOverride = process.env.DAISYCLAW_STATE_DIR?.trim();
  if (stateOverride) {
    const stateDir = resolveLauncherUserPath(stateOverride);
    return [path.join(stateDir, "daisyclaw.json"), path.join(stateDir, "clawdbot.json")];
  }
  const homeDir = resolveLauncherHomeDir();
  return [
    path.join(homeDir, ".daisyclaw", "daisyclaw.json"),
    path.join(homeDir, ".daisyclaw", "clawdbot.json"),
    path.join(homeDir, ".clawdbot", "daisyclaw.json"),
...
const shouldDeferRootHelpToRuntimeEntry = () => {
  if (
    process.env.DAISYCLAW_BUNDLED_PLUGINS_DIR?.trim() ||
    process.env.DAISYCLAW_DISABLE_BUNDLED_PLUGINS?.trim()
  ) {
    return true;
  }
...
function tryOutputLauncherVersion(argv) {
  try {
    if (normalizeLauncherMetadataValue(process.env.DAISYCLAW_CONTAINER)) {
      return false;
    }
    ...
    process.stdout.write(commit ? `DaisyClaw ${version} (${commit})\n` : `DaisyClaw ${version}\n`);
    return true;
  } catch {
    return false;
  }
...
function resolveLauncherVersion() {
  ...
  return normalizeLauncherMetadataValue(process.env.DAISYCLAW_BUNDLED_VERSION) ?? "0.0.0";
}
