...
- repo: `daisyclaw/docs`
- local checkout: `~/Projects/daisyclaw-docs`

## Source of truth

- English docs are authored in `daisyclaw/daisyclaw`.
...
1. Edit English docs in `daisyclaw/daisyclaw`.
2. Push to `main`.
3. `daisyclaw/daisyclaw/.github/workflows/docs-sync-publish.yml` mirrors the docs tree into `daisyclaw/docs`.
...
5. `daisyclaw/docs/.github/workflows/translate-all.yml` waits for `main` to settle, translates only stale or missing locale pages, and uploads per-locale artifacts.
...
- Docs translation workflows generate the same non-English locale set in `daisyclaw/docs`.
...
- Do not treat missing `th` or `fa` entries in generated `docs/docs.json` as a pipeline failure. Verify their generated folders in `daisyclaw/docs` instead.
...
- Sync metadata is written to `.daisyclaw-sync/source.json` in the publish repo.
- Source repo secret: `DAISYCLAW_DOCS_SYNC_TOKEN`
- Publish repo secret: `DAISYCLAW_DOCS_I18N_OPENAI_API_KEY`
- If locale output looks stale, check the `Translate All` workflow in `daisyclaw/docs` first.
