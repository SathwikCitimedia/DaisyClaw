...
Explicit copy flows, such as `daisyclaw agents add`, use this portability policy:
...
has such a marker, `daisyclaw doctor --fix` moves it to `auth.profiles` and
...
- Probe targets can come from auth profiles, environment credentials, or
  `models.json`.
- If a provider has credentials but DaisyClaw cannot resolve a probeable model
