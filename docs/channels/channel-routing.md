...
DaisyClaw routes replies **back to the channel where a message came from**. The
...
DaisyClaw infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, DaisyClaw still records inbound session metadata, but it
...
when a guarded path must not create a new DaisyClaw session. In that mode,
DaisyClaw may update metadata and `lastRoute` for an existing session, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when DaisyClaw would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.daisyclaw/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.daisyclaw`):

- `~/.daisyclaw/agents/<agentId>/sessions/sessions.json`
