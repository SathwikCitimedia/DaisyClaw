...
<Note>
Requires DaisyClaw 2026.5.29 or above. Run `daisyclaw --version` to check. Upgrade with `daisyclaw update`.
</Note>

<Steps>
  <Step title="Run the channel setup wizard">
  ```bash
  daisyclaw channels login --channel feishu
  ```
...
```bash
daisyclaw pairing list feishu
daisyclaw pairing approve feishu <CODE>
```
...
4. Check logs: `daisyclaw logs --follow`
...
5. Ensure the gateway is running: `daisyclaw gateway status`
6. Check logs: `daisyclaw logs --follow`

### QR setup does not react in the Feishu mobile app

1. Rerun setup: `daisyclaw channels login --channel feishu`
...
3. Restart the gateway: `daisyclaw gateway restart`
  ...
  channels: {
    feishu: {
      ...
      dynamicAgentCreation: {
        enabled: true,
        workspaceTemplate: "~/.daisyclaw/workspace-{agentId}",
        agentDirTemplate: "~/.daisyclaw/agents/{agentId}/agent",
      },
    },
  },
  session: {
...
| `channels.feishu.dynamicAgentCreation.workspaceTemplate` | Path template for dynamic agent workspaces | `~/.daisyclaw/workspace-{agentId}`    |
| `channels.feishu.dynamicAgentCreation.agentDirTemplate`  | Agent directory name template              | `~/.daisyclaw/agents/{agentId}/agent` |
...
workspace: /Users/you/.daisyclaw/workspace-feishu-ou_xxxxxx
...
of raw `file_key` JSON. When `tools.media.audio` is configured, DaisyClaw
...
conversion fails, DaisyClaw falls back to a file attachment and logs the reason.
...
topic session key. If a native topic starter event omits `thread_id`, DaisyClaw
hydrates it from Feishu before routing the turn. Normal group replies that
DaisyClaw turns into threads keep using the reply root message ID (`om_*`) so the
