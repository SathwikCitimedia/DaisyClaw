...
read_when:
  - You want to connect DaisyClaw to LINE
  - You need LINE webhook + credential setup
  - You want LINE-specific message options
title: LINE
---

LINE connects to DaisyClaw via the LINE Messaging API. The plugin runs as a webhook
...
```bash
daisyclaw plugins install @daisyclaw/line
```

Local checkout (when running from a git repo):

```bash
daisyclaw plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so DaisyClaw applies strict pre-auth body limits and timeout before verification.
- DaisyClaw processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
daisyclaw pairing list line
daisyclaw pairing approve line <CODE>
```
...
- Inbound media is saved under `~/.daisyclaw/media/inbound/` before it is passed
...
Outbound media URLs must be public HTTPS URLs. DaisyClaw validates the target hostname before handing the URL to LINE and rejects loopback, link-local, and private-network targets.
