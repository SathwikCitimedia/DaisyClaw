---
summary: "Mattermost bot setup and DaisyClaw config"
read_when:
...
<Tabs>
  <Tab title="npm registry">
    ```bash
    daisyclaw plugins install @daisyclaw/mattermost
    ```
  </Tab>
  <Tab title="Local checkout">
    ```bash
    daisyclaw plugins install ./path/to/local/mattermost-plugin
    ```
  </Tab>
</Tabs>
...
<Steps>
  <Step title="Ensure plugin is available">
    Current packaged DaisyClaw releases already bundle it. Older/custom installs can add it manually with the commands above.
  </Step>
  ...
  <Step title="Configure DaisyClaw and start the gateway">
    Minimal config:
...
Native slash commands are opt-in. When enabled, DaisyClaw registers `oc_*` slash commands via the Mattermost API and receives callback POSTs on the gateway HTTP server.
...
<AccordionGroup>
  <Accordion title="Behavior notes">
    ...
    - If `callbackUrl` is omitted, DaisyClaw derives one from gateway host/port + `callbackPath`.
    ...
    - Command callbacks are validated with the per-command tokens returned by Mattermost when DaisyClaw registers `oc_*` commands.
    - DaisyClaw refreshes current Mattermost command registration before accepting each callback so stale tokens from deleted or regenerated slash commands stop being accepted without a gateway restart.
  ...
  <Accordion title="Reachability requirement">
    ...
    - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as DaisyClaw.
    - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to DaisyClaw.
    - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from DaisyClaw, not `404`.
...
- Approve via:
  - `daisyclaw pairing list mattermost`
  - `daisyclaw pairing approve mattermost <CODE>`
...
Use these target formats with `daisyclaw message send` or cron/webhooks:
...
DaisyClaw resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), DaisyClaw sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When DaisyClaw sends to a Mattermost DM target and needs to resolve the direct channel first, it retries transient direct-channel creation failures by default.
...
<AccordionGroup>
  ...
  <Accordion title="Streaming behavior notes">
    - If the stream cannot be finalized in place (for example the post was deleted mid-stream), DaisyClaw falls back to sending a fresh final post so the reply is never lost.
...
Normal agent replies can also include semantic `presentation` payloads. DaisyClaw renders value buttons as Mattermost interactive buttons, keeps URL buttons visible in the message text, and downgrades select menus to readable text.
...
<AccordionGroup>
  ...
  <Accordion title="Config and reachability">
    ...
    - If `interactions.callbackBaseUrl` is omitted, DaisyClaw derives the callback URL from `gateway.customBindHost` + `gateway.port`, then falls back to `http://localhost:<port>`.
    - Reachability rule: the button callback URL must be reachable from the Mattermost server. `localhost` only works when Mattermost and DaisyClaw run on the same host/network namespace.
...
<Steps>
  <Step title="Derive the secret from the bot token">
    `HMAC-SHA256(key="daisyclaw-mattermost-interactions", data=botToken)`
  </Step>
...
secret = hmac.new(
    b"daisyclaw-mattermost-interactions",
...
The Mattermost plugin includes a directory adapter that resolves channel and user names via the Mattermost API. This enables `#channel-name` and `@username` targets in `daisyclaw message send` and cron/webhook deliveries.
...
<AccordionGroup>
  ...
  <Accordion title="Native slash commands fail">
    - `Unauthorized: invalid command token.`: DaisyClaw did not accept the callback token. Typical causes:
    ...
    - If `callbackUrl` is omitted and logs warn that the callback resolved to `http://127.0.0.1:18789/...`, that URL is probably only reachable when Mattermost runs on the same host/network namespace as DaisyClaw. Set an explicit externally reachable `commands.callbackUrl` instead.
