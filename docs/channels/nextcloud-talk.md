...
Nextcloud Talk ships as a bundled plugin in current DaisyClaw releases, so
...
```bash
daisyclaw plugins install @daisyclaw/nextcloud-talk
```
...
```bash
daisyclaw plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "DaisyClaw" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction
   ```
...
4. Configure DaisyClaw:
   ...
   ```bash
   daisyclaw channels add --channel nextcloud-talk \
...
- Approve via:
  - `daisyclaw pairing list nextcloud-talk`
  - `daisyclaw pairing approve nextcloud-talk <CODE>`
