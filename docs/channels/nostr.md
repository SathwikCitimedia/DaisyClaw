...
read_when:
  - You want DaisyClaw to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables DaisyClaw to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Bundled plugin

Current DaisyClaw releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`daisyclaw onboard`) and `daisyclaw channels add` still surface
  Nostr from the shared channel catalog.
- If your build excludes bundled Nostr, install the npm package directly.

```bash
daisyclaw plugins install @daisyclaw/nostr
```
...
```bash
daisyclaw plugins install --link <path-to-local-nostr-plugin>
```
...
daisyclaw channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
...
```json5
{
  channels: {
    nostr: {
      ...
      profile: {
        name: "daisyclaw",
        displayName: "DaisyClaw",
        about: "Personal assistant DM bot",
        picture: "https://example.com/avatar.png",
        banner: "https://example.com/banner.png",
        website: "https://example.com",
        nip05: "daisyclaw@example.com",
        lud16: "daisyclaw@example.com",
      },
    },
  },
}
