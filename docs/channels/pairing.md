...
read_when:
  ...
  - Reviewing DaisyClaw security posture
title: "Pairing"
---

"Pairing" is DaisyClaw's explicit access approval step.
...
Supported channels: `discord`, `feishu`, `googlechat`, `imessage`, `irc`, `line`, `matrix`, `mattermost`, `msteams`, `nextcloud-talk`, `nostr`, `daisyclaw-weixin`, `signal`, `slack`, `synology-chat`, `telegram`, `twitch`, `whatsapp`, `zalo`, `zalouser`.
...
Stored under `~/.daisyclaw/credentials/`:
...
3. On your phone, open the DaisyClaw iOS app → Settings → Gateway.
...
<Note>
An already paired device does not get broader access silently. If it reconnects asking for more scopes or a broader role, DaisyClaw keeps the existing approval as-is and creates a fresh pending upgrade request. Use `daisyclaw devices list` to compare the currently approved access with the newly requested access before you approve.
</Note>
...
Stored under `~/.daisyclaw/devices/`:
...
- The legacy `node.pair.*` API (CLI: `daisyclaw nodes pending|approve|reject|remove|rename`) is a
