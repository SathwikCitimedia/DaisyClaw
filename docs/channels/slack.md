...
| Request signing              | Not used (auth is the App-Level Token)                                                                                                               | Slack signs every request; DaisyClaw verifies with `signingSecret`                                              |
| Recovery on connection drop  | Slack SDK auto-reconnect is enabled; DaisyClaw also restarts failed Socket Mode sessions with bounded backoff. Pong-timeout transport tuning applies. | No persistent connection to drop; retries are per-request from Slack                                           |
...
```bash
daisyclaw plugins install @daisyclaw/slack
```
...
```json Recommended
{
  "display_information": {
    "name": "DaisyClaw",
    "description": "Slack connector for DaisyClaw"
  },
  "features": {
    "bot_user": { "display_name": "DaisyClaw", "always_online": true },
    "app_home": {
    ...
    "assistant_view": {
      "assistant_description": "DaisyClaw connects Slack assistant threads to DaisyClaw agents.",
      "suggested_prompts": [
    ...
    "slash_commands": [
      {
        "command": "/daisyclaw",
        "description": "Send a message to DaisyClaw",
        "should_escape": false
      }
    ]
  },
...
JSON5
daisyclaw config patch --file ./slack.socket.patch.json5 --dry-run
daisyclaw config patch --file ./slack.socket.patch.json5
```
...
JSON5
daisyclaw config patch --file ./slack.http.patch.json5 --dry-run
daisyclaw config patch --file ./slack.http.patch.json5
```
...
DaisyClaw sets the Slack SDK client pong timeout to 15 seconds by default for Socket Mode. Override the transport settings only when you need workspace- or host-specific tuning:
...
- Only `clientPingTimeout` has an DaisyClaw default (`15000`). `serverPingTimeout` and `pingPongLoggingEnabled` are passed to the Slack SDK only when configured.
...
The default manifest enables the Slack App Home **Home** tab and subscribes to `app_home_opened`. When a workspace member opens the Home tab, DaisyClaw publishes a safe default Home view with `views.publish`; no conversation payload or private configuration is included. The **Messages** tab remains enabled for Slack DMs. The manifest also enables Slack assistant threads with `features.assistant_view`, `assistant:write`, `assistant_thread_started`, and `assistant_thread_context_changed`; assistant threads route to their own DaisyClaw thread sessions and keep Slack-provided thread context available to the agent.
...
<Tabs>
  <Tab title="DM policy">
    ...
    Legacy `channels.slack.dm.policy` and `channels.slack.dm.allowFrom` still read for compatibility. `daisyclaw doctor --fix` migrates them to `dmPolicy` and `allowFrom` when it can do so without changing access.

    Pairing in DMs uses `daisyclaw pairing approve slack <code>`.
...
When a `message` tool call runs inside a Slack thread and targets the same channel, DaisyClaw normally inherits the current Slack thread according to `replyToMode`. Set `topLevel: true` on `action: "send"` or `action: "upload-file"` to force a new parent-channel message instead. `threadId: null` is accepted as the same top-level opt-out.
...
`ackReaction` sends an acknowledgement emoji while DaisyClaw is processing an inbound message. `ackReactionScope` decides _when_ that emoji is actually sent.
...
- Top-level Slack DMs stay off-thread by default, so they do not show Slack's thread-style native stream/status preview; DaisyClaw posts and edits a draft preview in the DM instead.
...
- If streaming fails mid-reply, DaisyClaw falls back to normal delivery for remaining payloads.
...
- Run `daisyclaw doctor --fix` to rewrite persisted Slack streaming config to the canonical keys.

## Typing reaction fallback

`typingReaction` adds a temporary reaction to the inbound Slack message while DaisyClaw is processing a reply, then removes it when the run finishes. This is most useful outside of thread replies, which use a default "is typing..." status indicator.
...
<AccordionGroup>
  <Accordion title="Inbound attachments">
    ...
    Downloads use bounded idle and total timeouts. If Slack file retrieval stalls or fails, DaisyClaw keeps processing the message and falls back to the file placeholder.
...
- The interactive callback values are DaisyClaw-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, DaisyClaw falls back to the original text reply instead of sending an invalid blocks payload.

### Plugin-owned modal submissions

Slack plugins that register an interactive handler can also receive modal
`view_submission` and `view_closed` lifecycle events before DaisyClaw compacts
the payload for the agent-visible system event. Use one of these routing
patterns when opening a Slack modal:

- Set `callback_id` to `daisyclaw:<namespace>:<payload>`.
...
```
  ...
  <Accordion title="Socket mode not connecting">
    ...
    If `daisyclaw channels status --probe --json` shows `botTokenStatus` or
...
1. DaisyClaw downloads the file from Slack's private URL using the bot token.
...
| Forwarded/shared attachments           | Text and Slack-hosted image/file media are best-effort                       | Re-share directly in the DaisyClaw thread                                   |
...
- [PDF tool](/tools/pdf)
- Epic: [#51349](https://github.com/daisyclaw/daisyclaw/issues/51349) — Slack attachment vision enablement
- Regression tests: [#51353](https://github.com/daisyclaw/daisyclaw/issues/51353)
- Live verification: [#51354](https://github.com/daisyclaw/daisyclaw/issues/51354)
