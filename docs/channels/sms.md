...
read_when:
  - You want to connect DaisyClaw to SMS through Twilio
  - You need SMS webhook or allowlist setup
title: "SMS"
---

DaisyClaw can receive and send SMS through a Twilio phone number or Messaging Service. The Gateway registers an inbound webhook route, validates Twilio request signatures by default, and sends replies back through Twilio's Messages API.
...
- A public HTTPS URL that reaches your DaisyClaw Gateway.
...
```bash
daisyclaw config patch --file ./sms.patch.json5 --dry-run
daisyclaw config patch --file ./sms.patch.json5
```
...
```bash
daisyclaw pairing list sms
daisyclaw pairing approve sms <CODE>
```
...
`authToken` can be a SecretRef. Use this when the Gateway should resolve the Twilio Auth Token from the DaisyClaw secrets runtime instead of storing plaintext config:
...
```bash
daisyclaw message send --channel sms --target sms:+15551234567 --message "hello"
```
...
```bash
daisyclaw message send --target twilio-sms:+15551234567 --message "hello"
```
...
SMS output is plain text. DaisyClaw strips markdown, flattens fenced code blocks, preserves readable links, and chunks long replies before sending them through Twilio.
...
```bash
daisyclaw channels capabilities --channel sms
daisyclaw channels status --channel sms --probe --json
```

3. Send an SMS to the Twilio number from your phone.
4. Run `daisyclaw pairing list sms`.
5. Approve the pairing code with `daisyclaw pairing approve sms <CODE>`.
6. Send another SMS and confirm the agent replies.

For outbound-only testing, use:

```bash
daisyclaw message send --channel sms --target sms:+15557654321 --message "DaisyClaw SMS test"
```
...
```bash
imsg send --to "+15551234567" --service sms --text "DaisyClaw SMS E2E $(date -u +%Y%m%dT%H%M%SZ)" --json
daisyclaw pairing list sms
daisyclaw pairing approve sms <CODE>
...
By default, DaisyClaw validates `X-Twilio-Signature` using `publicWebhookUrl` and `authToken`. Keep `publicWebhookUrl` byte-for-byte aligned with the URL configured in Twilio, including scheme, host, path, and query string.
...
### Twilio returns 403 or DaisyClaw rejects the webhook
