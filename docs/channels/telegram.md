...
```
    ...
    Telegram does **not** use `daisyclaw channels login telegram`; configure token in config/env, then start gateway.
...
```
  ...
  <Step title="Add the bot to a group">
    ...
    For first-time setup, get the group chat ID from `daisyclaw logs --follow`, a forwarded-ID bot, or Bot API `getUpdates`. After the group is allowed, `/whoami@<bot_username>` can confirm the user and group IDs.
...
After a successful startup, DaisyClaw caches the bot identity in the state directory for up to 24 hours so restarts can avoid an extra Telegram `getMe` call; changing or removing the token clears that cache.
</Note>
...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `daisyclaw doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
    If you previously relied on pairing-store allowlist files, `daisyclaw doctor --fix` can recover entries into `channels.telegram.allowFrom` in allowlist flows (for example when `dmPolicy: "allowlist"` has no explicit IDs yet).
...
```
    ...
    - or read `chat.id` from `daisyclaw logs --follow`
...
- DM messages can carry `message_thread_id`; DaisyClaw preserves it for replies. DM topic sessions split only when Telegram `getMe` reports `has_topics_enabled: true` for the bot; otherwise DMs stay on the flat session.
...
- Long polling is guarded inside each gateway process so only one active poller can use a bot token at a time. If you still see `getUpdates` 409 conflicts, another DaisyClaw gateway, script, or external poller is likely using the same token.
...
<Note>
  `channels.telegram.dm.threadReplies` and `channels.telegram.direct.<chatId>.threadReplies` were removed. Run `daisyclaw doctor --fix` after upgrading if your config still has those keys. DM topic routing now follows the bot capability from Telegram `getMe.has_topics_enabled`, which is controlled by BotFather threaded mode: topics-enabled bots use thread-scoped DM sessions when Telegram sends `message_thread_id`; other DMs stay on the flat session.
</Note>
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    DaisyClaw can stream partial replies in real time:
    ...
    - legacy `channels.telegram.streamMode` and boolean `streaming` values are detected; run `daisyclaw doctor --fix` to migrate them to `channels.telegram.streaming.mode`

    Tool-progress preview updates are the short status lines shown while tools run, for example command execution, file reads, planning updates, patch summaries, or Codex preamble/commentary text in Codex app-server mode. Telegram keeps these enabled by default to match released DaisyClaw behavior from `v2026.4.22` and later.
    ...
    <Note>
      Telegram selected quote replies are the exception. When `replyToMode` is `"first"`, `"all"`, or `"batched"` and the inbound message includes selected quote text, DaisyClaw sends the final answer through Telegram's native quote-reply path instead of editing the answer preview, so `streaming.preview.toolProgress` cannot show the short status lines for that turn. Current-message replies without selected quote text still keep preview streaming. Set `replyToMode: "off"` when tool-progress visibility matters more than native quote replies, or set `streaming.preview.toolProgress: false` to acknowledge the trade-off.
    </Note>

    For text-only replies:

    - short DM/group/topic previews: DaisyClaw keeps the same preview message and performs the final edit in place
    ...
    - if the final edit fails before the completed text is confirmed, DaisyClaw uses normal final delivery and cleans up the stale preview

    For complex replies (for example media payloads), DaisyClaw falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, DaisyClaw skips the preview stream to avoid double-streaming.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, DaisyClaw retries as plain text.
...
```
    ...
    - `deleteWebhook`, `deleteMyCommands`, or `setMyCommands` failing with `404: Not Found` while direct Bot API curl commands work can mean `channels.telegram.apiRoot` was set to the full `/bot<TOKEN>` endpoint. `apiRoot` must be only the Bot API root, and `daisyclaw doctor --fix` removes an accidental trailing `/bot<TOKEN>`.
    - `getMe returned 401` means Telegram rejected the configured bot token. Update `botToken`, `tokenFile`, or `TELEGRAM_BOT_TOKEN` with the current BotFather token; DaisyClaw stops before polling so this is not reported as a webhook cleanup failure.
...
```
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Thread-bound ACP spawn from chat**: `/acp spawn <agent> --thread here|auto` binds the current topic to a new ACP session; follow-ups route there directly. DaisyClaw pins the spawn confirmation in-topic. Requires `channels.telegram.threadBindings.spawnSessions` to remain enabled (default: `true`).
    ...
    The former `dm.threadReplies` and `direct.*.threadReplies` overrides are intentionally retired; use BotFather threaded mode as the single source of truth and run `daisyclaw doctor --fix` to remove stale config keys.
...
```
    ...
    Sticker descriptions are cached in DaisyClaw SQLite plugin state to reduce repeated vision calls.
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, DaisyClaw enqueues system events like:
  ...
  <Accordion title="Ack reactions">
    `ackReaction` sends an acknowledgement emoji while DaisyClaw is processing an inbound message. `ackReactionScope` decides *when* that emoji is actually sent.
...
```
  ...
  <Accordion title="Long polling vs webhook">
    ...
    In long-polling mode DaisyClaw persists its restart watermark only after an update dispatches successfully. If a handler fails, that update remains retryable in the same process and is not written as completed for restart dedupe.
    ...
    DaisyClaw then processes the update asynchronously through the same per-chat/per-topic bot lanes used by long polling, so slow agent turns do not hold Telegram's delivery ACK.
...
```bash
daisyclaw message send --channel telegram --target 123456789 --message "hi"
daisyclaw message send --channel telegram --target @name --message "hi"
daisyclaw message send --channel telegram --target -1001234567890:topic:42 --message "hi topic"
```

    Telegram polls use `daisyclaw message poll` and support forum topics:

```bash
daisyclaw message poll --channel telegram --target 123456789 \
  --poll-question "Ship it?" --poll-option "Yes" --poll-option "No"
daisyclaw message poll --channel telegram --target -1001234567890:topic:42 \
...
```
  ...
  <Accordion title="Exec approvals in Telegram">
    ...
    Channel delivery shows the command text in the chat; only enable `channel` or `both` in trusted groups/topics. When the prompt lands in a forum topic, DaisyClaw preserves the topic for the approval prompt and the follow-up. Exec approvals expire after 30 minutes by default.
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `daisyclaw channels status` warns when config expects unmentioned group messages.
    - `daisyclaw channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `daisyclaw logs --follow` for skip reasons
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, DaisyClaw now retries these as recoverable network errors.
    - During polling startup, DaisyClaw reuses the successful startup `getMe` probe for grammY so the runner does not need a second `getMe` before the first `getUpdates`.
    - If `deleteWebhook` fails with a transient network error during polling startup, DaisyClaw continues into long polling instead of making another pre-poll control-plane call. A still-active webhook surfaces as a `getUpdates` conflict; DaisyClaw then rebuilds the Telegram transport and retries webhook cleanup.
    ...
    - If logs include `Polling stall detected`, DaisyClaw restarts polling and rebuilds the Telegram transport after 120 seconds without completed long-poll liveness by default.
    - `daisyclaw channels status --probe` and `daisyclaw doctor` warn when a running polling account has not completed `getUpdates` after startup grace, when a running webhook account has not completed `setWebhook` after startup grace, or when the last successful polling transport activity is stale.
    ...
    - If the DaisyClaw managed proxy is configured through `DAISYCLAW_PROXY_URL` for a service environment and no standard proxy env is present, Telegram uses that URL for Bot API transport too.
...
```
    ...
    - Node 22+ defaults to `autoSelectFamily=true` (except WSL2). Telegram DNS result order honors `DAISYCLAW_TELEGRAM_DNS_RESULT_ORDER`, then `channels.telegram.network.dnsResultOrder`, then the process default such as `NODE_OPTIONS=--dns-result-order=ipv4first`; if none applies, Node 22+ falls back to `ipv4first`.
...
```
    ...
    - Environment overrides (temporary):
      - `DAISYCLAW_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY=1`
      - `DAISYCLAW_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY=1`
      - `DAISYCLAW_TELEGRAM_DNS_RESULT_ORDER=ipv4first`
...
<Note>
Multi-account precedence: when two or more account IDs are configured, set `channels.telegram.defaultAccount` (or include `channels.telegram.accounts.default`) to make default routing explicit. Otherwise DaisyClaw falls back to the first normalized account ID and `daisyclaw doctor` warns. Named accounts inherit `channels.telegram.allowFrom` / `groupAllowFrom`, but not `accounts.default.*` values.
</Note>
