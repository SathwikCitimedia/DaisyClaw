...
Tlon is a decentralized messenger built on Urbit. DaisyClaw connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current DaisyClaw releases, so normal packaged
...
```bash
daisyclaw plugins install @daisyclaw/tlon
```
...
```bash
daisyclaw plugins install ./path/to/local/tlon-plugin
```
...
By default, DaisyClaw blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `daisyclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, DaisyClaw replies in-thread.
