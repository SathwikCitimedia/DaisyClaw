...
- Onboarding (`daisyclaw onboard`) and `daisyclaw channels add --channel whatsapp`
  prompt to install the WhatsApp plugin the first time you select it.
- `daisyclaw channels login --channel whatsapp` also offers the install flow when
  the plugin is not present yet.
- Dev channel + git checkout: defaults to the local plugin path.
- Stable/Beta: installs the official `@daisyclaw/whatsapp` plugin from ClawHub
  first, with npm as the fallback.
- The WhatsApp runtime is distributed outside the core DaisyClaw npm package so
  WhatsApp-specific runtime dependencies stay with the external plugin.

Manual install stays available:

```bash
daisyclaw plugins install clawhub:@daisyclaw/whatsapp
```

Use the bare npm package (`@daisyclaw/whatsapp`) only when you need the registry
...
```bash
daisyclaw channels login --channel whatsapp
```
...
```bash
daisyclaw channels login --channel whatsapp --account work
```

    To attach an existing/custom WhatsApp Web auth directory before login:

```bash
daisyclaw channels add --channel whatsapp --account work --auth-dir /path/to/wa-auth
daisyclaw channels login --channel whatsapp --account work
```
...
<Note>
DaisyClaw recommends running WhatsApp on a separate number when possible. (The channel metadata and setup flow are optimized for that setup, but personal-number setups are also supported.)
</Note>
...
<AccordionGroup>
  <Accordion title="Dedicated number (recommended)">
    ...
    - separate WhatsApp identity for DaisyClaw
  ...
  <Accordion title="WhatsApp Web-only channel scope">
    The messaging platform channel is WhatsApp Web-based (`Baileys`) in current DaisyClaw channel architecture.
...
- When `messages.removeAckAfterReply` is enabled, DaisyClaw clears the WhatsApp ack reaction after a visible reply is delivered.
...
<Tabs>
  <Tab title="DM policy">
    ...
    - DaisyClaw never auto-pairs outbound `fromMe` DMs (messages you send to yourself from the linked device)
...
- if `messages.responsePrefix` is unset, self-chat replies default to `[{identity.name}]` or `[daisyclaw]`
...
<AccordionGroup>
  <Accordion title="Inbound envelope + reply context">
    ...
    When the quoted reply target is downloadable media, DaisyClaw saves it through
...
Set `messages.statusReactions.enabled: true` to let WhatsApp replace the ack reaction during a turn instead of leaving a static receipt emoji. When enabled, DaisyClaw uses the same inbound message reaction slot for lifecycle states such as queued, thinking, tool activity, compaction, done, and error.
...
<AccordionGroup>
  ...
  <Accordion title="Credential paths and legacy compatibility">
    - current auth path: `~/.daisyclaw/credentials/whatsapp/<accountId>/creds.json`
    - backup file: `creds.json.bak`
    - legacy default auth in `~/.daisyclaw/credentials/` is still recognized/migrated for default-account flows

  </Accordion>

  <Accordion title="Logout behavior">
    `daisyclaw channels logout --channel whatsapp [--account <id>]` clears WhatsApp auth state for that account.

    When a Gateway is reachable, logout first stops the live WhatsApp listener for the selected account so the linked session does not keep receiving messages until the next restart. `daisyclaw channels remove --channel whatsapp` also stops the live listener before disabling or deleting account config.
...
<AccordionGroup>
  ...
  <Accordion title="Linked but disconnected / reconnect loop">
    ...
    ```bash
    cp -a ~/.daisyclaw/credentials/whatsapp/<accountId> \
      ~/.daisyclaw/credentials/whatsapp/<accountId>.bak
    daisyclaw channels logout --channel whatsapp --account <accountId>
    daisyclaw channels login --channel whatsapp --account <accountId>
    ```

    If `~/.daisyclaw/logs/whatsapp-health.log` says `Gateway inactive` but
    `daisyclaw gateway status` and `daisyclaw channels status --probe` show the
    gateway and WhatsApp are healthy, run `daisyclaw doctor`. On Linux, doctor
    warns about legacy crontab entries that still invoke
    `~/.daisyclaw/bin/ensure-whatsapp.sh`; remove those stale entries with
  ...
  <Accordion title="QR login times out behind a proxy">
    Symptom: `daisyclaw channels login --channel whatsapp` fails before showing a usable QR code with `status=408 Request Time-out` or a TLS socket disconnect.
  ...
  <Accordion title="Reply appears in transcript but not in WhatsApp">
    Transcript rows record what the agent generated. WhatsApp delivery is checked separately: DaisyClaw only treats an auto-reply as sent after Baileys returns an outbound message id for at least one visible text or media send.
  ...
  <Accordion title="Group messages unexpectedly ignored">
    ...
    - duplicate keys in `daisyclaw.json` (JSON5): later entries override earlier ones, so keep a single `groupPolicy` per scope

    If `channels.whatsapp.groups` is present, WhatsApp can still observe messages from other groups, but DaisyClaw drops them before session routing. Add the group JID to `channels.whatsapp.groups` or add `groups["*"]` to admit all groups while keeping sender authorization under `groupPolicy` and `groupAllowFrom`.
