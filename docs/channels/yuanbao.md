...
Tencent Yuanbao is Tencent's AI assistant platform. The DaisyClaw channel plugin
connects Yuanbao bots to DaisyClaw over WebSocket so they can interact with users
...
> **Requires DaisyClaw 2026.4.10 or above.** Run `daisyclaw --version` to check. Upgrade with `daisyclaw update`.

<Steps>
  <Step title="Add the Yuanbao channel with your credentials">
  ```bash
  daisyclaw channels add --channel yuanbao --token "appKey:appSecret"
  ```
...
```bash
daisyclaw channels login --channel yuanbao
```
...
```bash
daisyclaw pairing list yuanbao
daisyclaw pairing approve yuanbao <CODE>
```
