...
read_when:
  - Setting up Zalo Personal for DaisyClaw
  - Debugging Zalo Personal login or message flow
title: "Zalo personal"
---

Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside DaisyClaw.
...
Zalo Personal ships as a bundled plugin in current DaisyClaw releases, so normal
packaged builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo Personal,
install the npm package directly:

- Install via CLI: `daisyclaw plugins install @daisyclaw/zalouser`
- Pinned version: `daisyclaw plugins install @daisyclaw/zalouser@2026.5.2`
- Or from a source checkout: `daisyclaw plugins install ./path/to/local/zalouser-plugin`
...
- `daisyclaw pairing list zalouser`
- `daisyclaw pairing approve zalouser <code>`
...
- On startup, DaisyClaw resolves group/user names in allowlists to IDs and logs the mapping only when `channels.zalouser.dangerouslyAllowNameMatching: true` is enabled.
...
- When a group message is skipped because mention is required, DaisyClaw stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in DaisyClaw state. Example:
...
Profile names select the saved Zalo login credentials in DaisyClaw state. Resolution order is:
...
- DaisyClaw sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, DaisyClaw sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `daisyclaw channels logout --channel zalouser && daisyclaw channels login --channel zalouser`
...
- The channel now runs fully in DaisyClaw without external CLI binaries.
