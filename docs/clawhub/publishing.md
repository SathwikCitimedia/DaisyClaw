...
An owner is a ClawHub publisher handle, such as `@alice` or `@daisyclaw`.
...
`@daisyclaw/dronzer`, it can only be published as `@daisyclaw`. If you publish as
...
```text
Package scope "@daisyclaw" must match selected owner "@vintageayu".
Publish as "@daisyclaw" or rename this package to "@vintageayu/dronzer".
```
...
```sh
clawhub package transfer @opik/opik-daisyclaw --to opik
```
...
This protects org namespaces. A package named `@daisyclaw/dronzer` claims the
`@daisyclaw` namespace, so only publishers with access to the `@daisyclaw` owner
can publish it.
