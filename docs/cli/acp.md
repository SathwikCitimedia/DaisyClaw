...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an DaisyClaw Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`daisyclaw acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to DaisyClaw channel
conversations instead of hosting an ACP harness session, use
[`daisyclaw mcp serve`](/cli/mcp) instead.
...
- DaisyClaw acts as an ACP server
- an IDE or ACP client connects to DaisyClaw
- DaisyClaw forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where DaisyClaw runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to DaisyClaw: use `daisyclaw acp`
- DaisyClaw should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Session lineage metadata                                              | Implemented | Session listings and session info snapshots include DaisyClaw parent and child lineage in `_meta` so ACP clients can render subagent graphs without private Gateway side channels.                                                                |
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the DaisyClaw gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
daisyclaw acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
daisyclaw acp --url wss://gateway-host:18789 --token-file ~/.daisyclaw/gateway.token

# Attach to an existing session key
daisyclaw acp --session agent:main:main

# Attach by label (must already exist)
daisyclaw acp --session-label "support inbox"

# Reset the session key before the first prompt
daisyclaw acp --session agent:main:main --reset-session
```
...
```bash
daisyclaw acp client

# Point the spawned bridge at a remote Gateway
daisyclaw acp client --server-args --url wss://gateway-host:18789 --token-file ~/.daisyclaw/gateway.token

# Override the server command (default: daisyclaw)
daisyclaw acp client --server "node" --server-args daisyclaw.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run DaisyClaw through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass "yolo" switch for that harness session.

## Protocol smoke testing

For protocol-level debugging, start a Gateway with isolated state and drive
`daisyclaw acp` over stdio with an ACP JSON-RPC client. Cover `initialize`,
...
Avoid using `daisyclaw gateway call sessions.list` as the only ACP proof. That
...
it to drive an DaisyClaw Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `daisyclaw acp` over stdio.

Example config (persisted):

```bash
daisyclaw config set gateway.remote.url wss://gateway-host:18789
daisyclaw config set gateway.remote.token <token>
```
...
```bash
daisyclaw acp --session agent:main:main
daisyclaw acp --session agent:design:main
daisyclaw acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see DaisyClaw plugin tools or selected
...
[DaisyClaw tools MCP bridge](/tools/acp-agents-setup#daisyclaw-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your
DaisyClaw bot over ACP, use `acpx` with its built-in `daisyclaw` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx daisyclaw` at `daisyclaw acp`.
3. Target the DaisyClaw session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default DaisyClaw ACP session
acpx daisyclaw exec "Summarize the active DaisyClaw session state."

# Persistent named session for follow-up turns
acpx daisyclaw sessions ensure --name codex-bridge
acpx daisyclaw -s codex-bridge --cwd /path/to/repo \
  "Ask my DaisyClaw work agent for recent context relevant to this repo."
```

If you want `acpx daisyclaw` to target a specific Gateway and session key every
time, override the `daisyclaw` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "daisyclaw": {
      "command": "env DAISYCLAW_HIDE_BANNER=1 DAISYCLAW_SUPPRESS_NOTES=1 daisyclaw acp --url ws://127.0.0.1:18789 --token-file ~/.daisyclaw/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local DaisyClaw checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env DAISYCLAW_HIDE_BANNER=1 DAISYCLAW_SUPPRESS_NOTES=1 node daisyclaw.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an DaisyClaw agent without scraping a terminal.
...
In Zed, open the Agent panel and select "DaisyClaw ACP" to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`DAISYCLAW_GATEWAY_TOKEN`, `DAISYCLAW_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`DAISYCLAW_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `DAISYCLAW_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `daisyclaw acp client` sets `DAISYCLAW_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `daisyclaw`).
