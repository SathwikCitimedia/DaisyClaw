---
summary: "CLI reference for `daisyclaw agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
daisyclaw agent --to +15555550123 --message "status update" --deliver
daisyclaw agent --agent ops --message "Summarize logs"
daisyclaw agent --agent ops --model openai/gpt-5.4 --message "Summarize logs"
daisyclaw agent --session-key agent:ops:incident-42 --message "Summarize status"
daisyclaw agent --agent ops --session-key incident-42 --message "Summarize status"
daisyclaw agent --session-id 1234 --message "Summarize inbox" --thinking medium
daisyclaw agent --to +15555550123 --message "Trace logs" --verbose on --json
daisyclaw agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"
daisyclaw agent --agent ops --message "Run locally" --local
```
