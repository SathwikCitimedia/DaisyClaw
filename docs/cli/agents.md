---
summary: "CLI reference for `daisyclaw agents` (list/add/delete/bindings/bind/unbind/set identity)"
read_when:
...
```bash
daisyclaw agents list
daisyclaw agents list --bindings
daisyclaw agents add work --workspace ~/.daisyclaw/workspace-work
daisyclaw agents add work --workspace ~/.daisyclaw/workspace-work --bind telegram:*
daisyclaw agents add ops --workspace ~/.daisyclaw/workspace-ops --bind telegram:ops --non-interactive
daisyclaw agents bindings
daisyclaw agents bind --agent work --bind telegram:ops
daisyclaw agents unbind --agent work --bind telegram:ops
daisyclaw agents set-identity --workspace ~/.daisyclaw/workspace --from-identity
daisyclaw agents set-identity --agent main --avatar avatars/daisyclaw.png
daisyclaw agents delete work
```
...
If you also want different visible skills per agent, configure `agents.defaults.skills` and `agents.list[].skills` in `daisyclaw.json`. See [Skills config](/tools/skills-config) and [Configuration reference](/gateway/config-agents#agents-defaults-skills).

List bindings:

```bash
daisyclaw agents bindings
daisyclaw agents bindings --agent work
daisyclaw agents bindings --json
```

Add bindings:

```bash
daisyclaw agents bind --agent work --bind telegram:ops --bind discord:guild-a
```

You can also add bindings when creating an agent:

```bash
daisyclaw agents add work --workspace ~/.daisyclaw/workspace-work --bind telegram:* --bind discord:*
```

If you omit `accountId` (`--bind <channel>`), DaisyClaw resolves it from plugin setup hooks, forced account binding, or the channel's configured account count.

If you omit `--agent` for `bind` or `unbind`, DaisyClaw targets the current default agent.
...
- If the same agent already has a matching channel binding without `accountId`, and you later bind with an explicit or resolved `accountId`, DaisyClaw upgrades that existing binding in place instead of adding a duplicate.

Examples:

```bash
# match all accounts on the channel
daisyclaw agents bind --agent work --bind telegram:*
...
daisyclaw agents bind --agent work --bind telegram:alerts
```
...
```bash
daisyclaw agents unbind --agent work --bind telegram:ops
daisyclaw agents unbind --agent work --all
```
...
Running `daisyclaw agents` with no subcommand is equivalent to `daisyclaw agents list`.
...
- Example path: `~/.daisyclaw/workspace/IDENTITY.md`
...
```bash
daisyclaw agents set-identity --agent main --name "DaisyClaw" --emoji "🦞" --avatar avatars/daisyclaw.png
```
