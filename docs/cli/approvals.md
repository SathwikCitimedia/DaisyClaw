---
summary: "CLI reference for `daisyclaw approvals` and `daisyclaw exec-policy`"
read_when:
...
Alias: `daisyclaw exec-approvals`
...
## `daisyclaw exec-policy`

`daisyclaw exec-policy` is the local convenience command for keeping the requested
...
```bash
daisyclaw exec-policy show
daisyclaw exec-policy show --json

daisyclaw exec-policy preset yolo
daisyclaw exec-policy preset cautious --json

daisyclaw exec-policy set --host gateway --security full --ask off --ask-fallback full
```
...
- `daisyclaw exec-policy show` marks `host=node` scopes as node-managed at runtime instead of deriving an effective policy from the local approvals file

If you need to edit remote host approvals directly, keep using `daisyclaw approvals set --gateway`
or `daisyclaw approvals set --node <id|name|ip>`.

## Common commands

```bash
daisyclaw approvals get
daisyclaw approvals get --node <id|name|ip>
daisyclaw approvals get --gateway
```

`daisyclaw approvals get` now shows the effective exec policy for local, gateway, and node targets:
...
```bash
daisyclaw approvals set --file ./exec-approvals.json
daisyclaw approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off", askFallback: "full" } }
EOF
daisyclaw approvals set --node <id|name|ip> --file ./exec-approvals.json
daisyclaw approvals set --gateway --file ./exec-approvals.json
```
...
This changes the **host approvals file** only. To keep the requested DaisyClaw policy aligned, also set:
...
```bash
daisyclaw approvals allowlist add "~/Projects/**/bin/rg"
daisyclaw approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"
daisyclaw approvals allowlist add --agent "*" "/usr/bin/uname"

daisyclaw approvals allowlist remove "~/Projects/**/bin/rg"
```
...
- `--node` uses the same resolver as `daisyclaw nodes` (id, name, ip, or id prefix).
- `--agent` defaults to `"*"`, which applies to all agents.
- The node host must advertise `system.execApprovals.get/set` (macOS app or headless node host).
- Approvals files are stored per host in the DaisyClaw state dir
  (`$DAISYCLAW_STATE_DIR/exec-approvals.json`, or
  `~/.daisyclaw/exec-approvals.json` when the variable is unset).
