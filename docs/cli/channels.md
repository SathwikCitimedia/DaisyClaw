---
summary: "CLI reference for `daisyclaw channels` (accounts, status, login/logout, logs)"
read_when:
...
Do not use `daisyclaw sessions`, Gateway `sessions.list`, or the agent
...
```bash
daisyclaw channels add --channel telegram --token <bot-token>
daisyclaw channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY"
daisyclaw channels remove --channel telegram --delete
```

<Tip>
`daisyclaw channels add --help` shows per-channel flags (token, private key, app token, signal-cli paths, etc).
</Tip>
...
If a channel plugin needs to be installed during a flag-driven add command, DaisyClaw uses the channel's default install source without opening the interactive plugin install prompt.

When you run `daisyclaw channels add` without flags, the interactive wizard can prompt:
...
- Run `daisyclaw status --deep` for a broad probe.
- Use `daisyclaw doctor` for guided fixes.
- `daisyclaw channels list` no longer prints model provider usage/quota snapshots. For those, use `daisyclaw status` (overview) or `daisyclaw models list` (per-provider).
- `daisyclaw channels status` falls back to config-only summaries when the gateway is unreachable. If a supported channel credential is configured via SecretRef but unavailable in the current command path, it reports that account as configured with degraded notes instead of showing it as not configured.
...
```bash
daisyclaw channels resolve --channel slack "#general" "@jane"
daisyclaw channels resolve --channel discord "My Server/#support" "@someone"
daisyclaw channels resolve --channel matrix "Project Room"
```
