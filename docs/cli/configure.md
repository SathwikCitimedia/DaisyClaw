---
summary: "CLI reference for `daisyclaw configure` (interactive configuration prompts)"
read_when:
...
Use `daisyclaw onboard` for the full guided first-run journey, `daisyclaw setup` for the baseline config/workspace only, and `daisyclaw channels add` when you only need channel account setup.
...
<Tip>
`daisyclaw config` without a subcommand opens the same wizard. Use `daisyclaw config get|set|unset` for non-interactive edits.
</Tip>

For web search, `daisyclaw configure --section web` lets you choose a provider
...
daisyclaw configure --section model --section channels
daisyclaw configure --section gateway --section daemon
```
