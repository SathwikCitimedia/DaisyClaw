...
read_when:
  - You run daisyclaw with no command after setup and want to understand Crestodian
  - You need a configless-safe way to inspect or repair DaisyClaw
  - You are designing or enabling message-channel rescue mode
title: "Crestodian"
---

# `daisyclaw crestodian`

Crestodian is DaisyClaw's local setup, repair, and configuration helper. It is
designed to stay reachable when the normal agent path is broken.

Running `daisyclaw` with no command starts classic onboarding first when the
active config file is missing or has no authored settings (empty or
metadata-only). After a config file has authored settings, running `daisyclaw`
with no command starts Crestodian in an interactive terminal. Running
`daisyclaw crestodian` starts the same helper explicitly.

## What Crestodian shows

On startup, interactive Crestodian opens the same TUI shell used by
`daisyclaw tui`, with a Crestodian chat backend. The chat log starts with a short
greeting:
...
Crestodian uses the same DaisyClaw reference discovery as regular agents. In a Git checkout,
...
[https://github.com/daisyclaw/daisyclaw](https://github.com/daisyclaw/daisyclaw), with explicit
...
daisyclaw
daisyclaw crestodian
daisyclaw crestodian --json
daisyclaw crestodian --message "models"
daisyclaw crestodian --message "validate config"
daisyclaw crestodian --message "setup workspace ~/Projects/work model openai/gpt-5.5" --yes
daisyclaw crestodian --message "set default model openai/gpt-5.5" --yes
daisyclaw onboard --modern
```
...
config set-ref gateway.auth.token env DAISYCLAW_GATEWAY_TOKEN
gateway status
restart gateway
agents
...
plugin install clawhub:daisyclaw-codex-app-server
plugin uninstall daisyclaw-codex-app-server
talk to work agent
...
- `daisyclaw.json` is missing
- `daisyclaw.json` is invalid
- the Gateway is down
- plugin command registration is unavailable
- no agent has been configured yet

`daisyclaw --help` and `daisyclaw --version` still use the normal fast paths.
Noninteractive bare `daisyclaw` exits with a short message instead of printing
...
```text
~/.daisyclaw/audit/crestodian.jsonl
```

Discovery is not audited. Only applied operations and writes are logged.

`daisyclaw onboard --modern` starts Crestodian as the modern onboarding preview.
Plain `daisyclaw onboard` still runs classic onboarding.
...
planner turn through DaisyClaw's normal runtime paths. It first uses the
configured DaisyClaw model. If no configured model is usable yet, it can fall
...
`daisyclaw tui`, `daisyclaw chat`, and `daisyclaw terminal` still open the normal
...
DaisyClaw: Crestodian rescue mode. Gateway reachable: no. Config valid: no.
You: /crestodian restart gateway
DaisyClaw: Plan: restart the Gateway. Reply /crestodian yes to apply.
You: /crestodian yes
DaisyClaw: Applied. Audit entry written.
```
...
- Remote rescue cannot open the local TUI or switch into an interactive agent
  session. Use local `daisyclaw` for agent handoff.
