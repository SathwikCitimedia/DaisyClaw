---
summary: "CLI reference for `daisyclaw cron` (schedule and run background jobs)"
read_when:
...
<Tip>
Run `daisyclaw cron --help` for the full command surface. See [Cron jobs](/automation/cron-jobs) for the conceptual guide.
</Tip>

## Create jobs quickly

`daisyclaw cron create` is an alias for `daisyclaw cron add`. For new jobs, put the schedule first and the prompt second:
...
Use `--command` for deterministic shell-style jobs that should run inside DaisyClaw cron without starting an isolated agent/model run:
...
`daisyclaw cron list` and `daisyclaw cron show <job-id>` preview the resolved delivery route. For `channel: "last"`, the preview shows whether the route resolved from the main or current session, or will fail closed.
...
If an isolated run times out before the first model request, `daisyclaw cron show`
and `daisyclaw cron runs` include a phase-specific error such as
...
Skipped runs are tracked separately from execution errors. They do not affect retry backoff, but `daisyclaw cron edit <job-id> --failure-alert-include-skipped` can opt failure alerts into repeated skipped-run notifications.
...
Note: cron jobs, pending runtime state, and run history live in the shared SQLite state database. Legacy `jobs.json`, `jobs-state.json`, and `runs/*.jsonl` files are imported once and renamed with a `.migrated` suffix. After import, edit schedules with `daisyclaw cron add|edit|remove` instead of editing JSON files.

### Manual runs

`daisyclaw cron run <job-id>` force-runs by default and returns as soon as the manual run is queued. Successful responses include `{ ok: true, enqueued: true, runId }`. Use the returned `runId` to inspect the later result:

```bash
daisyclaw cron run <job-id>
daisyclaw cron runs --id <job-id> --run-id <run-id>
```

Add `--wait` when a script should block until that exact queued run records a terminal status:

```bash
daisyclaw cron run <job-id> --wait --wait-timeout 10m --poll-interval 2s
```
...
- When a job has `--model` but no fallback list is configured, DaisyClaw passes an explicit empty fallback override so the agent primary is not appended as a hidden retry target.
- Local-provider preflight checks walk configured fallbacks before marking a cron run `skipped`.

`daisyclaw doctor` reports jobs that already have `payload.model` set, including provider namespace counts and mismatches against `agents.defaults.model`. Use that check when auth, provider, or billing behavior looks different between live chat and scheduled jobs.
...
```bash
daisyclaw cron edit <job-id> --announce --channel telegram --to "123456789"
```

Disable delivery for an isolated job:

```bash
daisyclaw cron edit <job-id> --no-deliver
```

Enable lightweight bootstrap context for an isolated job:

```bash
daisyclaw cron edit <job-id> --light-context
```

Announce to a specific channel:

```bash
daisyclaw cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
```

Announce to a Telegram forum topic:

```bash
daisyclaw cron edit <job-id> --announce --channel telegram --to "-1001234567890" --thread-id 42
```
...
```bash
daisyclaw cron list
daisyclaw cron list --agent ops
daisyclaw cron get <job-id>
daisyclaw cron show <job-id>
daisyclaw cron run <job-id>
daisyclaw cron run <job-id> --due
daisyclaw cron run <job-id> --wait --wait-timeout 10m
daisyclaw cron run <job-id> --wait --wait-timeout 10m --poll-interval 2s
daisyclaw cron runs --id <job-id> --limit 50
daisyclaw cron runs --id <job-id> --run-id <run-id>
```

`daisyclaw cron list` shows all matching jobs by default. Pass `--agent <id>` to show only jobs whose effective normalized agent id matches; jobs without a stored agent id count as the configured default agent.

`daisyclaw cron get <job-id>` returns the stored job JSON directly. Use `cron show <job-id>` when you want the human-readable view with delivery-route preview.
...
```bash
daisyclaw cron edit <job-id> --agent ops
daisyclaw cron edit <job-id> --clear-agent
daisyclaw cron edit <job-id> --session current
daisyclaw cron edit <job-id> --session "session:daily-brief"
```

`daisyclaw cron add` warns when `--agent` is omitted on agent-turn jobs and falls back to the default agent (`main`). Pass `--agent <id>` at create time to pin a specific agent.

Delivery tweaks:

```bash
daisyclaw cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
daisyclaw cron edit <job-id> --webhook "https://example.invalid/daisyclaw/cron"
daisyclaw cron edit <job-id> --best-effort-deliver
daisyclaw cron edit <job-id> --no-best-effort-deliver
daisyclaw cron edit <job-id> --no-deliver
```
