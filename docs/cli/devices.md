---
summary: "CLI reference for `daisyclaw devices` (device pairing + token rotation/revocation)"
read_when:
...
### `daisyclaw devices remove <deviceId>`
...
```
daisyclaw devices remove <deviceId>
daisyclaw devices remove <deviceId> --json
```

### `daisyclaw devices clear --yes [--pending]`

Clear paired devices in bulk.

```
daisyclaw devices clear --yes
daisyclaw devices clear --yes --pending
daisyclaw devices clear --yes --pending --json
```

### `daisyclaw devices approve [requestId] [--latest]`

Approve a pending device pairing request by exact `requestId`. If `requestId`
is omitted or `--latest` is passed, DaisyClaw only prints the selected pending
request and exits; rerun approval with the exact request ID after verifying
the details.

<Note>
If a device retries pairing with changed auth details (role, scopes, or public key), DaisyClaw supersedes the previous pending entry and issues a new `requestId`. Run `daisyclaw devices list` right before approval to use the current ID.
</Note>

If the device is already paired and asks for broader scopes or a broader role,
DaisyClaw keeps the existing approval in place and creates a new pending upgrade
request. Review the `Requested` vs `Approved` columns in `daisyclaw devices list`
or use `daisyclaw devices approve --latest` to preview the exact upgrade before
approving it.
...
```
daisyclaw devices approve
daisyclaw devices approve <requestId>
daisyclaw devices approve --latest
```

## Paperclip / `daisyclaw_gateway` first-run approval

When a new Paperclip agent connects through the `daisyclaw_gateway` adapter for the first time, the Gateway may require a one-time device pairing approval before runs can succeed. If Paperclip reports `daisyclaw_gateway_pairing_required`, approve the pending device and retry.
...
The preview prints the exact `daisyclaw devices approve <requestId>` command. Verify the request details, then rerun that command with the request ID to approve it.

For remote gateways or explicit credentials, pass the same options while previewing and approving:

```bash
daisyclaw devices approve --latest --url <gateway-ws-url> --token <gateway-token>
```
...
If approval keeps failing, run `daisyclaw devices list` first to confirm a pending request exists.
...
### `daisyclaw devices rotate --device <id> --role <role> [--scope <scope...>]`
...
```
daisyclaw devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
```
...
### `daisyclaw devices revoke --device <id> --role <role>`
...
```
daisyclaw devices revoke --device <deviceId> --role node
```
...
```bash
daisyclaw devices rotate --device <deviceId> --role operator
```
