---
summary: "CLI reference for `daisyclaw directory` (self, peers, groups)"
read_when:
...
- `directory` is meant to help you find IDs you can paste into other commands (especially `daisyclaw message send --target ...`).
...
```bash
daisyclaw directory peers list --channel slack --query "U0"
daisyclaw message send --channel slack --target user:U012ABCDEF --message "hello"
```
...
```bash
daisyclaw directory peers list --channel zalouser
daisyclaw directory peers list --channel zalouser --query "name"
daisyclaw directory peers list --channel zalouser --limit 50
```

## Groups

```bash
daisyclaw directory groups list --channel zalouser
