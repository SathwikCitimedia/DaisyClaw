---
summary: "CLI reference for `daisyclaw dns` (wide-area discovery helpers)"
read_when:
  - You want wide-area discovery (DNS-SD) via Tailscale + CoreDNS
  - You're setting up split DNS for a custom discovery domain (example: daisyclaw.internal)
title: "DNS"
---
...
```bash
daisyclaw dns setup
daisyclaw dns setup --domain daisyclaw.internal
...
- `--domain <domain>`: wide-area discovery domain (for example `daisyclaw.internal`)
...
- zone file path
- current tailnet IPs
- recommended `daisyclaw.json` discovery config
...
- If `--domain` is omitted, DaisyClaw uses `discovery.wideArea.domain` from config.
