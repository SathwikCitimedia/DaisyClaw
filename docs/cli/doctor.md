---
summary: "CLI reference for `daisyclaw doctor` (health checks + guided repairs)"
read_when:
...
`daisyclaw doctor` is the DaisyClaw health surface. Use it when the gateway,
...
| ------- | ------------------------ | ------------------------------------------------------------------------------- |
| Inspect | `daisyclaw doctor`        | Human-oriented checks and guided prompts.                                       |
| Repair  | `daisyclaw doctor --fix`  | Applies supported repairs, using prompts unless non-interactive repair is safe. |
| Lint    | `daisyclaw doctor --lint` | Read-only structured findings for CI, preflight, and review gates.              |
...
daisyclaw doctor --lint --allow-exec
daisyclaw doctor --deep
daisyclaw doctor --fix
daisyclaw doctor --fix --non-interactive
daisyclaw doctor --generate-gateway-token
daisyclaw doctor --post-upgrade
daisyclaw doctor --post-upgrade --json
```

For channel-specific permissions, use the channel probes instead of `doctor`:

```bash
daisyclaw channels capabilities --channel discord --target channel:<channel-id>
...
`daisyclaw doctor --lint` is the read-only automation posture for doctor checks.
...
doctor --lint: ran 6 check(s), 1 finding(s)
  [warning] core/doctor/gateway-config gateway.mode - gateway.mode is unset; gateway start will be blocked.
    fix: Run `daisyclaw configure` and set Gateway mode (local/remote), or `daisyclaw config set gateway.mode local`.
```
  ...
  "findings": [
    {
      ...
      "path": "gateway.mode",
      "fixHint": "Run `daisyclaw configure` and set Gateway mode (local/remote), or `daisyclaw config set gateway.mode local`."
    }
  ]
}
```
...
example, `daisyclaw doctor --lint --severity-min error` can print no findings and
...
health path. The `daisyclaw/plugin-sdk/health` subpath exposes the same
...
```bash
daisyclaw doctor --lint --only core/doctor/gateway-config --json
daisyclaw doctor --lint --skip core/doctor/skills-readiness
```
...
`daisyclaw doctor --post-upgrade` runs plugin compatibility probes intended to be
...
- In Nix mode (`DAISYCLAW_NIX_MODE=1`), read-only doctor checks still work, but `doctor --fix`, `doctor --repair`, `doctor --yes`, and `doctor --generate-gateway-token` are disabled because `daisyclaw.json` is immutable. Edit the Nix source for this install instead; for nix-daisyclaw, use the agent-first [Quick Start](https://github.com/daisyclaw/nix-daisyclaw#quick-start).
...
- By default, doctor does not execute `exec` SecretRefs while checking secrets. Use `daisyclaw doctor --allow-exec` or `daisyclaw doctor --lint --allow-exec` only when you intentionally want doctor to run those configured secret resolvers.
- `--fix` (alias for `--repair`) writes a backup to `~/.daisyclaw/daisyclaw.json.bak` and drops unknown config keys, listing each removal.
...
- `doctor --fix --non-interactive` reports missing or stale gateway service definitions but does not install or rewrite them outside update repair mode. Run `daisyclaw gateway install` for a missing service, or `daisyclaw gateway install --force` when you intentionally want to replace the launcher.
...
- Doctor also scans `~/.daisyclaw/cron/jobs.json` (or `cron.store`) for legacy cron job shapes and rewrites them before importing canonical rows into SQLite.
...
- On Linux, doctor warns when the user's crontab still runs legacy `~/.daisyclaw/bin/ensure-whatsapp.sh`; that script is no longer maintained and can log false WhatsApp gateway outages when cron lacks the systemd user-bus environment.
- When WhatsApp is enabled, doctor checks for a degraded Gateway event loop with local `daisyclaw-tui` clients still running. `doctor --fix` stops only verified local TUI clients so WhatsApp replies are not queued behind stale TUI refresh loops.
...
- Doctor cleans legacy plugin dependency staging state created by older DaisyClaw versions and relinks the host `daisyclaw` package for managed npm plugins that declare it as a peer dependency. It also repairs missing downloadable plugins that are referenced by config, such as `plugins.entries`, configured channels, configured provider/search settings, or configured agent runtimes. During package updates, doctor skips package-manager plugin repair until the package swap is complete; rerun `daisyclaw doctor --fix` afterward if a configured plugin still needs recovery. If the download fails, doctor reports the install error and preserves the configured plugin entry for the next repair attempt.
...
- Set `DAISYCLAW_SERVICE_REPAIR_POLICY=external` when another supervisor owns the gateway lifecycle. Doctor still reports gateway/service health and applies non-service repairs, but skips service install/start/restart/bootstrap and legacy service cleanup.
- On Linux, doctor ignores inactive extra gateway-like systemd units and does not rewrite command/entrypoint metadata for a running systemd gateway service during repair. Stop the service first or use `daisyclaw gateway install --force` when you intentionally want to replace the active launcher.
...
- Doctor includes a memory-search readiness check and can recommend `daisyclaw configure --section model` when embedding credentials are missing.
...
- Doctor reports an info note when Codex-mode agents are configured and personal Codex CLI assets exist in the operator's Codex home. Local Codex app-server launches use isolated per-agent homes, so install the Codex plugin first if needed, then use `daisyclaw migrate plan codex` to inventory assets that should be promoted deliberately.
...
- If sandbox mode is enabled but Docker is unavailable, doctor reports a high-signal warning with remediation (`install Docker` or `daisyclaw config set agents.defaults.sandbox.mode off`).
- If legacy sandbox registry files or shard directories are present (`~/.daisyclaw/sandbox/containers.json`, `~/.daisyclaw/sandbox/browsers.json`, `~/.daisyclaw/sandbox/containers/`, or `~/.daisyclaw/sandbox/browsers/`), doctor reports them; `daisyclaw doctor --fix` migrates valid entries into SQLite and quarantines invalid legacy files.
...
If you previously ran `launchctl setenv DAISYCLAW_GATEWAY_TOKEN ...` (or `...PASSWORD`), that value overrides your config file and can cause persistent "unauthorized" errors.

```bash
launchctl getenv DAISYCLAW_GATEWAY_TOKEN
launchctl getenv DAISYCLAW_GATEWAY_PASSWORD

launchctl unsetenv DAISYCLAW_GATEWAY_TOKEN
launchctl unsetenv DAISYCLAW_GATEWAY_PASSWORD
```
