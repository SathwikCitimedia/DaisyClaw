---
summary: "DaisyClaw Gateway CLI (`daisyclaw gateway`) — run, query, and discover gateways"
read_when:
...
The Gateway is DaisyClaw's WebSocket server (channels, nodes, sessions, hooks). Subcommands in this page live under `daisyclaw gateway …`.
...
<CardGroup cols={3}>
  ...
  <Card title="Discovery overview" href="/gateway/discovery">
    How DaisyClaw advertises and finds gateways.
  </Card>
...
<AccordionGroup>
  <Accordion title="Startup behavior">
    - By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.daisyclaw/daisyclaw.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
    - `daisyclaw onboard --mode local` and `daisyclaw setup` are expected to write `gateway.mode=local`. If the file exists but `gateway.mode` is missing, treat that as a broken or clobbered config and repair it instead of assuming local mode implicitly.
...
<ParamField path="--token <token>" type="string">
  Token override (also sets `DAISYCLAW_GATEWAY_TOKEN` for the process).
</ParamField>
...
<ParamField path="--bind custom + gateway.customBindHost" type="string">
  Expects an IPv4 address today. For IPv6-only BYOH, place an IPv4 sidecar or proxy in front of the Gateway and point DaisyClaw at that IPv4 endpoint.
</ParamField>
...
daisyclaw gateway restart --safe
daisyclaw gateway restart --safe --skip-deferral
daisyclaw gateway restart --force
```

`daisyclaw gateway restart --safe` asks the running Gateway to preflight active DaisyClaw work before restarting. If queued operations, reply delivery, embedded runs, or task runs are active, the Gateway reports the blockers, coalesces duplicate safe restart requests, and restarts once the active work drains. Plain `restart` keeps the existing service-manager behavior for compatibility. Use `--force` only when you explicitly want the immediate override path.

`daisyclaw gateway restart --safe --skip-deferral` runs the same DaisyClaw-aware coordinated restart as `--safe`, but bypasses the active-work deferral gate so the Gateway emits the restart immediately even when blockers are reported. Use it as the operator escape hatch when a deferral has been pinned by a stuck task run and `--safe` alone would wait indefinitely. `--skip-deferral` requires `--safe`.
...
- Set `DAISYCLAW_GATEWAY_STARTUP_TRACE=1` to log phase timings during Gateway startup, including per-phase `eventLoopMax` delay and plugin lookup-table timings for installed-index, manifest registry, startup planning, and owner-map work.
- Set `DAISYCLAW_GATEWAY_RESTART_TRACE=1` to log restart-scoped `restart trace:` lines for restart signal handling, active-work drain, shutdown phases, next start, ready timing, and memory metrics.
- Set `DAISYCLAW_DIAGNOSTICS=timeline` with `DAISYCLAW_DIAGNOSTICS_TIMELINE_PATH=<path>` to write a best-effort JSONL startup diagnostics timeline for external QA harnesses. You can also enable the flag with `diagnostics.flags: ["timeline"]` in config; the path is still env-provided. Add `DAISYCLAW_DIAGNOSTICS_EVENT_LOOP=1` to include event-loop samples.
...
```bash
daisyclaw gateway health --url ws://127.0.0.1:18789
```
...
```bash
daisyclaw gateway usage-cost
daisyclaw gateway usage-cost --days 7
daisyclaw gateway usage-cost --json
```
...
```bash
daisyclaw gateway stability
daisyclaw gateway stability --type payload.large
daisyclaw gateway stability --bundle latest
daisyclaw gateway stability --bundle latest --export
daisyclaw gateway stability --json
```
...
<AccordionGroup>
  <Accordion title="Privacy and bundle behavior">
    ...
    - On fatal Gateway exits, shutdown timeouts, and restart startup failures, DaisyClaw writes the same diagnostic snapshot to `~/.daisyclaw/logs/stability/daisyclaw-stability-*.json` when the recorder has events. Inspect the newest bundle with `daisyclaw gateway stability --bundle latest`; `--limit`, `--type`, and `--since-seq` also apply to bundle output.
...
daisyclaw gateway diagnostics export --output daisyclaw-diagnostics.zip
daisyclaw gateway diagnostics export --json
```
...
daisyclaw gateway status --require-rpc
```
...
<AccordionGroup>
  ...
  <Accordion title="Common warning codes">
    ...
    - `multiple_gateways`: distinct gateway identities were reachable, or DaisyClaw could not prove reachable targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway does not trigger this warning.
...
```bash
daisyclaw gateway call status
daisyclaw gateway call logs.tail --params '{"sinceMs": 60000}'
```
...
responsible for eventually exec'ing `daisyclaw` or Node with those args.

```bash
cat > ~/.local/bin/daisyclaw-doppler <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
exec doppler run --project my-project --config production -- daisyclaw "$@"
EOF
chmod +x ~/.local/bin/daisyclaw-doppler

daisyclaw gateway install --wrapper ~/.local/bin/daisyclaw-doppler --force
daisyclaw gateway restart
```
...
`DAISYCLAW_WRAPPER` in the service environment for later forced reinstalls, updates, and doctor
repairs.

```bash
DAISYCLAW_WRAPPER="$HOME/.local/bin/daisyclaw-doppler" daisyclaw gateway install --force
daisyclaw doctor
```

To remove a persisted wrapper, clear `DAISYCLAW_WRAPPER` while reinstalling:

```bash
DAISYCLAW_WRAPPER= daisyclaw gateway install --force
daisyclaw gateway restart
```
...
<AccordionGroup>
  ...
  <Accordion title="Lifecycle behavior">
    ...
    - `gateway restart --safe` asks the running Gateway to preflight active DaisyClaw work and defer the restart until reply delivery, embedded runs, and task runs drain. `--safe` cannot be combined with `--force` or `--wait`.
    ...
    - `gateway restart --safe --skip-deferral` runs the DaisyClaw-aware safe restart but bypasses the deferral gate so the Gateway emits the restart immediately even when blockers are reported. Operator escape hatch for stuck-task-run deferrals; requires `--safe`.
  ...
  <Accordion title="Auth and SecretRefs at install time">
    ...
    - For password auth on `gateway run`, prefer `DAISYCLAW_GATEWAY_PASSWORD`, `--password-file`, or a SecretRef-backed `gateway.auth.password` over inline `--password`.
    - In inferred auth mode, shell-only `DAISYCLAW_GATEWAY_PASSWORD` does not relax install token requirements; use durable config (`gateway.auth.password` or config `env`) when installing a managed service.
...
`gateway discover` scans for Gateway beacons (`_daisyclaw-gw._tcp`).

- Multicast DNS-SD: `local.`
- Unicast DNS-SD (Wide-Area Bonjour): choose a domain (example: `daisyclaw.internal.`) and set up split DNS + a DNS server; see [Bonjour](/gateway/bonjour).
...
```bash
daisyclaw gateway discover --timeout 4000
daisyclaw gateway discover --json | jq '.beacons[].wsUrl'
```
