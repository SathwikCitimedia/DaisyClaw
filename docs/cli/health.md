---
summary: "CLI reference for `daisyclaw health` (gateway health snapshot via RPC)"
read_when:
...
daisyclaw health --timeout 2500
daisyclaw health --verbose
daisyclaw health --debug
```

Notes:

- Default `daisyclaw health` asks the running gateway for its health snapshot. When the
