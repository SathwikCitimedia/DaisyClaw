---
summary: "DaisyClaw CLI index: command list, global flags, and links to per-command pages"
read_when:
  - Finding the right `daisyclaw` subcommand
  - Looking up global flags or output styling rules
title: "CLI reference"
---

`daisyclaw` is the main CLI entry point. Each core command has either a
...
- `daisyclaw setup` creates the baseline config and workspace without walking the full guided onboarding flow.
- `daisyclaw onboard` is the full guided first-run path for gateway, model auth, workspace, channels, skills, and health.
- `daisyclaw configure` changes targeted parts of an existing setup, such as model auth, gateway, channels, plugins, or skills.
- `daisyclaw channels add` configures channel accounts after the baseline exists; run it without flags for guided channel setup or with channel-specific flags for scripts.
...
| Flag                    | Purpose                                                               |
| ----------------------- | --------------------------------------------------------------------- |
| `--dev`                 | Isolate state under `~/.daisyclaw-dev` and shift default ports         |
| `--profile <name>`      | Isolate state under `~/.daisyclaw-<name>`                              |
| `--container <name>`    | Target a named container for execution                                |
| `--no-color`            | Disable ANSI colors (`NO_COLOR=1` is also respected)                  |
| `--update`              | Shorthand for [`daisyclaw update`](/cli/update) (source installs only) |
...
```
daisyclaw [--dev] [--profile <name>] <command>
  crestodian
  setup
  onboard
  configure
...
[`daisyclaw workboard`](/cli/workboard) or `daisyclaw voicecall`.
...
`daisyclaw status --usage` and the Control UI surface provider usage/quota when
