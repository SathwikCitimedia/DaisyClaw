---
summary: "Expose DaisyClaw channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to DaisyClaw-backed channels
  - Running `daisyclaw mcp serve`
  - Managing DaisyClaw-saved MCP server definitions
title: "MCP"
sidebarTitle: "MCP"
---

`daisyclaw mcp` has two jobs:

- run DaisyClaw as an MCP server with `daisyclaw mcp serve`
- manage DaisyClaw-owned outbound MCP server definitions with `list`, `show`, `status`, `doctor`, `probe`, `add`, `set`, `configure`, `tools`, `login`, `logout`, `reload`, and `unset`

In other words:

- `serve` is DaisyClaw acting as an MCP server
- the other subcommands are DaisyClaw acting as an MCP client-side registry for MCP servers its runtimes may consume later

Use [`daisyclaw acp`](/cli/acp) when DaisyClaw should host a coding harness session itself and route that runtime through ACP.

## Choose the right MCP path

DaisyClaw has several MCP surfaces. Pick the one that matches who owns the agent runtime and who owns the tools.

| Goal                                                                | Use                                                                  | Why                                                                                                             |
| ------------------------------------------------------------------- | -------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| Let an external MCP client read/send DaisyClaw channel conversations | `daisyclaw mcp serve`                                                 | DaisyClaw is the MCP server and exposes Gateway-backed conversations over stdio.                                 |
| Save third-party MCP servers for DaisyClaw-managed agent runs        | `daisyclaw mcp add`, `set`, `configure`, `tools`, `login`             | DaisyClaw is the MCP client-side registry and later projects those servers into eligible runtimes.               |
| Check a saved server without running an agent turn                  | `daisyclaw mcp status`, `doctor`, `probe`                             | `status` and `doctor` inspect config; `probe` opens a live MCP connection and lists capabilities.               |
...
| Run ACP-hosted harness sessions                                     | [`daisyclaw acp`](/cli/acp) and [ACP Agents](/tools/acp-agents-setup) | ACP bridge mode does not accept per-session MCP server injection; configure gateway/plugin bridges instead.     |

<Tip>
If you are not sure which path you need, start with `daisyclaw mcp status --verbose`. It shows what DaisyClaw has saved without starting any MCP servers.
</Tip>

## DaisyClaw as an MCP server

This is the `daisyclaw mcp serve` path.

### When to use `serve`

Use `daisyclaw mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to DaisyClaw-backed channel conversations
- you already have a local or remote DaisyClaw Gateway with routed sessions
- you want one MCP server that works across DaisyClaw's channel backends instead of running separate per-channel bridges

Use [`daisyclaw acp`](/cli/acp) instead when DaisyClaw should host the coding runtime itself and keep the agent session inside DaisyClaw.

### How it works

`daisyclaw mcp serve` starts a stdio MCP server. The MCP client owns that process. While the client keeps the stdio session open, the bridge connects to a local or remote DaisyClaw Gateway over WebSocket and exposes routed channel conversations over MCP.

<Steps>
  <Step title="Client spawns the bridge">
    The MCP client spawns `daisyclaw mcp serve`.
  </Step>
  <Step title="Bridge connects to Gateway">
    The bridge connects to the DaisyClaw Gateway over WebSocket.
  </Step>
...
<AccordionGroup>
  <Accordion title="Important behavior">
    ...
    - one-shot agent entry points such as `daisyclaw agent` and `daisyclaw infer model run` retire any bundled MCP runtimes they open when the reply completes, so repeated scripted runs do not accumulate stdio MCP child processes
    - stdio MCP servers launched by DaisyClaw (bundled or user-configured) are torn down as a process tree on shutdown, so child subprocesses started by the server do not survive after the parent stdio client exits
...
The bridge uses existing Gateway session route metadata to expose channel-backed conversations. A conversation appears when DaisyClaw already has session state with a known route such as:
...
<Tabs>
  ...
  <Tab title="Remote Gateway (token)">
    ```bash
    daisyclaw mcp serve --url wss://gateway-host:18789 --token-file ~/.daisyclaw/gateway.token
    ```
  </Tab>
  <Tab title="Remote Gateway (password)">
    ```bash
    daisyclaw mcp serve --url wss://gateway-host:18789 --password-file ~/.daisyclaw/gateway.password
    ```
  </Tab>
  <Tab title="Verbose / Claude off">
    ```bash
    daisyclaw mcp serve --verbose
    daisyclaw mcp serve --claude-channel-mode off
    ```
  </Tab>
</Tabs>
...
The bridge can also expose Claude-specific channel notifications. This is the DaisyClaw equivalent of a Claude Code channel adapter: standard MCP tools remain available, but live inbound messages can also arrive as Claude-specific MCP notifications.
...
- sender allowlists, pairing, and channel-level trust still belong to the underlying DaisyClaw channel configuration
...
DaisyClaw ships a deterministic Docker smoke for this bridge:
...
- starts a second container that spawns `daisyclaw mcp serve`
...
## DaisyClaw as an MCP client registry

This is the `daisyclaw mcp list`, `show`, `status`, `doctor`, `probe`, `add`, `set`,
`configure`, `tools`, `login`, `logout`, `reload`, and `unset` path.

These commands do not expose DaisyClaw over MCP. They manage DaisyClaw-owned MCP server definitions under `mcp.servers` in DaisyClaw config.

Those saved definitions are for runtimes that DaisyClaw launches or configures later, such as embedded DaisyClaw and other runtime adapters. DaisyClaw stores the definitions centrally so those runtimes do not need to keep their own duplicate MCP server lists.

<AccordionGroup>
  <Accordion title="Important behavior">
    - these commands only read or write DaisyClaw config
    ...
    - embedded DaisyClaw exposes configured MCP tools in normal `coding` and `messaging` tool profiles; `minimal` still hides them, and `tools.deny: ["bundle-mcp"]` disables them explicitly
    - per-server `toolFilter.include` and `toolFilter.exclude` filter discovered MCP tools before they become DaisyClaw tools
...
Runtime adapters may normalize this shared registry into the shape their downstream client expects. For example, embedded DaisyClaw consumes DaisyClaw `transport` values directly, while Claude Code and Gemini receive CLI-native `type` values such as `http`, `sse`, or `stdio`.

Codex app-server also honors an optional `codex` block on each server. This is
DaisyClaw projection metadata for Codex app-server threads only; it does not
change ACP sessions, generic Codex harness config, or other runtime adapters.
Use non-empty `codex.agents` to project a server only into specific DaisyClaw
...
DaisyClaw strips the `codex` metadata before handing the native `mcp_servers`
config to Codex.

### Saved MCP server definitions

DaisyClaw also stores a lightweight MCP server registry in config for surfaces that want DaisyClaw-managed MCP definitions.

Commands:

- `daisyclaw mcp list`
- `daisyclaw mcp show [name]`
- `daisyclaw mcp status [--verbose]`
- `daisyclaw mcp doctor [name] [--probe]`
- `daisyclaw mcp probe [name]`
- `daisyclaw mcp add <name> [flags]`
- `daisyclaw mcp set <name> <json>`
- `daisyclaw mcp configure <name> [flags]`
- `daisyclaw mcp tools <name> [--include csv] [--exclude csv] [--clear]`
- `daisyclaw mcp login <name> [--code code]`
- `daisyclaw mcp logout <name>`
- `daisyclaw mcp reload`
- `daisyclaw mcp unset <name>`
...
- Use `transport: "streamable-http"` for Streamable HTTP MCP servers. `daisyclaw mcp set` also normalizes CLI-native `type: "http"` to the same canonical config shape for compatibility.
...
```bash
daisyclaw mcp list
daisyclaw mcp show context7 --json
daisyclaw mcp status --verbose
daisyclaw mcp doctor --probe
daisyclaw mcp probe context7 --json
daisyclaw mcp add memory --command npx --arg -y --arg @modelcontextprotocol/server-memory
daisyclaw mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
daisyclaw mcp tools context7 --include 'resolve-library-id,get-library-docs'
daisyclaw mcp set docs '{"url":"https://mcp.example.com","transport":"streamable-http"}'
daisyclaw mcp configure docs --timeout 20 --connect-timeout 5 --include 'search,read_*'
daisyclaw mcp configure docs --auth oauth --oauth-scope 'docs.read'
daisyclaw mcp login docs
daisyclaw mcp logout docs
daisyclaw mcp unset context7
```

### Common server recipes

These examples save server definitions only. Run `daisyclaw mcp doctor --probe` afterward to prove that the server starts and exposes tools.

<Tabs>
  <Tab title="Filesystem">
    ```bash
    daisyclaw mcp add files \
      --command npx \
      --arg -y \
      --arg @modelcontextprotocol/server-filesystem \
      --arg "$HOME/Documents" \
      --include 'read_file,list_directory,search_files'
    daisyclaw mcp doctor files --probe
    ```
  ...
  <Tab title="Memory">
    ```bash
    daisyclaw mcp add memory \
      --command npx \
      --arg -y \
      --arg @modelcontextprotocol/server-memory
    daisyclaw mcp probe memory --json
    ```
  ...
  <Tab title="Local script">
    ```bash
    daisyclaw mcp add local-tools \
      --command node \
      --arg ./dist/mcp-server.js \
      --cwd /srv/daisyclaw-tools \
  ...
  <Tab title="Remote HTTP">
    ...
    daisyclaw mcp doctor docs --probe
    ```
  ...
  <Tab title="Desktop/CUA">
    ```bash
    daisyclaw mcp set cua-driver '{"command":"cua-driver","args":["mcp"]}'
    daisyclaw mcp tools cua-driver --include 'list_apps,observe,click,type'
    daisyclaw mcp doctor cua-driver --probe
    ```
...
<AccordionGroup>
  <Accordion title="status --json">
    ```json
    {
      "path": "/home/user/.daisyclaw/daisyclaw.json",
      "servers": [
        {
          "name": "docs",
          "configured": true,
      ...
      "servers": [
        {
          ...
          "issues": [
            {
              "level": "error",
              "message": "OAuth credentials are not authorized; run daisyclaw mcp login docs"
            }
          ]
        }
      ]
...
| `auth: "oauth"`                | Use MCP OAuth token storage and `daisyclaw mcp login`             |
...
Sensitive values in `url` (userinfo) and `headers` are redacted in logs and status output. `daisyclaw mcp doctor` warns when sensitive-looking `headers` or `env` entries contain literal values, so operators can move those values out of committed config.
...
<Steps>
  <Step title="Save the server">
    ...
    ```bash
    daisyclaw mcp set docs '{"url":"https://mcp.example.com/mcp","transport":"streamable-http","auth":"oauth","oauth":{"scope":"docs.read"}}'
    ```
  ...
  <Step title="Start login">
    ...
    DaisyClaw prints the authorization URL and stores temporary OAuth verifier state under the DaisyClaw state directory.

  </Step>
  <Step title="Finish with the code">
    After approving in the browser, pass the returned code back to DaisyClaw.

    ```bash
    daisyclaw mcp login docs --code abc123
    ```
...
If the provider rotates tokens or the authorization state gets stuck, run `daisyclaw mcp logout <name>`, then repeat `login`. `logout` can clear credentials for a saved HTTP server even after `auth: "oauth"` has been removed from config, as long as the server name and URL still identify the credential store entry.
...
| ------------------------------ | -------------------------------------------------------------------------------------- |
| `url`                          | HTTP or HTTPS URL of the remote server (required)                                      |
| `transport`                    | Set to `"streamable-http"` to select this transport; when omitted, DaisyClaw uses `sse` |
| `headers`                      | Optional key-value map of HTTP headers (for example auth tokens)                       |
| `connectionTimeoutMs`          | Per-server connection timeout in ms (optional)                                         |
| `connectTimeout`               | Per-server connection timeout in seconds (optional)                                    |
| `timeout` / `requestTimeoutMs` | Per-server MCP request timeout in seconds or ms                                        |
...
DaisyClaw config uses `transport: "streamable-http"` as the canonical spelling. CLI-native MCP `type: "http"` values are accepted when saved through `daisyclaw mcp set` and repaired by `daisyclaw doctor --fix` in existing config, but `transport` is what embedded DaisyClaw consumes directly.
...
Use the page for operator edits and quick inventory. Use `daisyclaw mcp doctor --probe` or `daisyclaw mcp probe` when you need live server proof.
...
7. Run `daisyclaw mcp doctor --probe` when you need live proof that the edited server starts and lists tools.
...
- active runtimes may need `daisyclaw mcp reload`, Gateway config publish, or process restart depending on which process owns the MCP clients
