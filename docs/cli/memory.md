---
summary: "CLI reference for `daisyclaw memory` (status/index/search/promote/promote-explain/rem-harness)"
read_when:
...
```bash
daisyclaw memory status
daisyclaw memory status --deep
daisyclaw memory status --fix
daisyclaw memory index --force
daisyclaw memory search "meeting notes"
daisyclaw memory search --query "deployment" --max-results 20
daisyclaw memory promote --limit 10 --min-score 0.75
daisyclaw memory promote --apply
daisyclaw memory promote --json --min-recall-count 0 --min-unique-queries 0
daisyclaw memory promote-explain "router vlan"
daisyclaw memory promote-explain "router vlan" --json
daisyclaw memory rem-harness
daisyclaw memory rem-harness --json
daisyclaw memory status --json
daisyclaw memory status --deep --index
daisyclaw memory status --deep --index --verbose
daisyclaw memory status --agent main
daisyclaw memory index --agent main --verbose
```
...
```bash
daisyclaw memory promote [--apply] [--limit <n>] [--include-promoted]
```
...
- When dreaming is enabled, `memory-core` auto-manages one cron job that runs a full sweep (`light -> REM -> deep`) in the background (no manual `daisyclaw cron add` required).
- `--agent <id>`: scope to a single agent (default: the default agent).
...
```bash
daisyclaw memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
daisyclaw memory rem-harness [--agent <id>] [--include-promoted] [--json]
```
