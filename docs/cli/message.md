---
summary: "CLI reference for `daisyclaw message` (send + channel actions)"
read_when:
...
```
daisyclaw message <subcommand> [flags]
```
...
- `daisyclaw message` resolves the selected channel to its owning plugin when `--channel` or a channel-prefixed target is present; otherwise it loads configured channel plugins for default-channel inference.
...
- On cache miss, DaisyClaw will attempt a live directory lookup when the provider supports it.
...
- `daisyclaw message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, DaisyClaw does not force a `default` account SecretRef scope
...
```
daisyclaw message send --channel discord \
...
```bash
daisyclaw message send --channel googlechat --target spaces/AAA... \
  --message "Choose:" \
...
```
daisyclaw message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
daisyclaw message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
daisyclaw message poll --channel msteams \
...
```
daisyclaw message react --channel slack \
...
```
daisyclaw message react --channel signal \
...
```
daisyclaw message send --channel telegram --target @mychat --message "Choose:" \
...
```
daisyclaw message send --channel telegram --target 123456789 --message "Open app:" \
...
```bash
daisyclaw message send --channel telegram --target @mychat \
