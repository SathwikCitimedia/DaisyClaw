---
summary: "CLI reference for `daisyclaw migrate` (import state from another agent system)"
read_when:
  - You want to migrate from Hermes or another agent system into DaisyClaw
...
```bash
daisyclaw migrate list
daisyclaw migrate claude --dry-run
daisyclaw migrate codex --dry-run
daisyclaw migrate codex --skill gog-vault77-google-workspace
daisyclaw migrate codex --plugin google-calendar --dry-run
daisyclaw migrate codex --plugin google-calendar --verify-plugin-apps --dry-run
daisyclaw migrate hermes --dry-run
daisyclaw migrate hermes
daisyclaw migrate apply codex --yes --skill gog-vault77-google-workspace
daisyclaw migrate apply codex --yes --plugin google-calendar
daisyclaw migrate apply codex --yes
daisyclaw migrate apply claude --yes
daisyclaw migrate apply hermes --yes
daisyclaw migrate apply hermes --include-secrets --yes
daisyclaw onboard --flow import
daisyclaw onboard --import-from claude --import-source ~/.claude
daisyclaw onboard --import-from hermes --import-source ~/.hermes
```

<ParamField path="<provider>" type="string">
  Name of a registered migration provider, for example `hermes`. Run `daisyclaw migrate list` to see installed providers.
</ParamField>
...
<ParamField path="--no-backup" type="boolean">
  Skip the pre-apply backup. Requires `--force` when local DaisyClaw state exists.
</ParamField>
...
`daisyclaw migrate` is preview-first.
...
<AccordionGroup>
  <Accordion title="Preview before apply">
    ...
    `daisyclaw migrate apply <provider>` previews the plan and prompts before changing state unless `--yes` is set. In non-interactive mode, apply requires `--yes`.

  </Accordion>
  <Accordion title="Backups">
    Apply creates and verifies an DaisyClaw backup before applying the migration. If no local DaisyClaw state exists yet, the backup step is skipped and the migration can continue. To skip a backup when state exists, pass both `--no-backup` and `--force`.
  </Accordion>
...
- Project `CLAUDE.md` and `.claude/CLAUDE.md` into the DaisyClaw agent workspace.
...
- Claude command Markdown files converted into DaisyClaw skills with manual invocation only.

### Archive and manual-review state

Claude hooks, permissions, environment defaults, local memory, path-scoped rules, subagents, caches, plans, and project history are preserved in the migration report or reported as manual-review items. DaisyClaw does not execute hooks, copy broad allowlists, or import OAuth/Desktop credential state automatically.
...
Use this provider when moving to the DaisyClaw Codex harness and you want to
...
Running `daisyclaw migrate codex` in an interactive terminal previews the full
...
start checked unless the target DaisyClaw Codex plugin config already has that
...
```bash
daisyclaw migrate codex --dry-run --skill gog-vault77-google-workspace
daisyclaw migrate apply codex --yes --skill gog-vault77-google-workspace
```
...
```bash
daisyclaw migrate codex --dry-run --plugin google-calendar
daisyclaw migrate apply codex --yes --plugin google-calendar
```
...
- Personal AgentSkills under `$HOME/.agents/skills`, copied into the current
  DaisyClaw agent workspace when you want per-agent ownership.
- Source-installed `openai-curated` Codex plugins discovered through Codex
  ...
  native Codex harness; they are not exposed to DaisyClaw provider runs,
...
- `SOUL.md` and `AGENTS.md` into the DaisyClaw agent workspace.
- `memories/MEMORY.md` and `memories/USER.md` appended to workspace memory files.
- Memory config defaults for DaisyClaw file memory, plus archive or manual-review items for external memory providers such as Honcho.
...
Hermes state that DaisyClaw cannot safely interpret is copied into the migration report for manual review, but it is not loaded into live DaisyClaw config or credentials. This preserves opaque or unsafe state without pretending DaisyClaw can execute or trust it automatically:
...
Migration sources are plugins. A plugin declares its provider ids in `daisyclaw.plugin.json`:
...
Provider plugins can use `daisyclaw/plugin-sdk/migration` for item construction and summary counts, plus `daisyclaw/plugin-sdk/migration-runtime` for conflict-aware file copies, archive-only report copies, cached config-runtime wrappers, and migration reports.

## Onboarding integration

Onboarding can offer migration when a provider detects a known source. Both `daisyclaw onboard --flow import` and `daisyclaw setup --wizard --import-from hermes` use the same plugin migration provider and still show a preview before applying.

<Note>
Onboarding imports require a fresh DaisyClaw setup. Reset config, credentials, sessions, and the workspace first if you already have local state. Backup-plus-overwrite or merge imports are feature-gated for existing setups.
</Note>
...
- [Migrating](/install/migrating): move DaisyClaw to a new machine.
