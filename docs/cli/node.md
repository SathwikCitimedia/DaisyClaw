---
summary: "CLI reference for `daisyclaw node` (headless node host)"
read_when:
...
`daisyclaw node run` and `daisyclaw node install` resolve gateway auth from config/env (no `--token`/`--password` flags on node commands):

- `DAISYCLAW_GATEWAY_TOKEN` / `DAISYCLAW_GATEWAY_PASSWORD` are checked first.
...
trusted private-DNS names, set `DAISYCLAW_ALLOW_INSECURE_PRIVATE_WS=1`; without
it, node startup fails closed and asks you to use `wss://`, an SSH tunnel, or
Tailscale. This is a process-environment opt-in, not an `daisyclaw.json` config
key.
`daisyclaw node install` persists it into the supervised node service when it is
...
```bash
daisyclaw node install --host <gateway-host> --port 18789
```
...
```bash
daisyclaw node status
daisyclaw node start
daisyclaw node stop
daisyclaw node restart
daisyclaw node uninstall
```

Use `daisyclaw node run` for a foreground node host (no service).
...
- `daisyclaw approvals --node <id|name|ip>` (edit from the Gateway)

For approved async node exec, DaisyClaw prepares a canonical `systemRunPlan`
