---
summary: "CLI reference for `daisyclaw onboard` (interactive onboarding)"
read_when:
...
Full guided onboarding for local or remote Gateway setup. Use this when you want DaisyClaw to walk through model auth, workspace, gateway, channels, skills, and health in one flow.
...
<CardGroup cols={2}>
  ...
  <Card title="Onboarding overview" href="/start/onboarding-overview" icon="map">
    How DaisyClaw onboarding fits together.
  </Card>
...
daisyclaw onboard --flow quickstart
daisyclaw onboard --flow manual
daisyclaw onboard --flow import
daisyclaw onboard --import-from hermes --import-source ~/.hermes
daisyclaw onboard --skip-bootstrap
daisyclaw onboard --mode remote --remote-url wss://gateway-host:18789
```

`--flow import` uses plugin-owned migration providers such as Hermes. It only runs against a fresh DaisyClaw setup; if existing config, credentials, sessions, or workspace memory/identity files are present, reset or choose a fresh setup before importing.

`--modern` starts the Crestodian conversational onboarding preview. Without
`--modern`, `daisyclaw onboard` keeps the classic onboarding flow.

On a fresh install where the active config file is missing or has no authored
settings (empty or metadata-only), bare `daisyclaw` also starts the classic
onboarding flow. Once a config file has authored settings, bare `daisyclaw`
...
`DAISYCLAW_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
```bash
DAISYCLAW_LOCALE=zh-CN daisyclaw onboard
```
...
DaisyClaw marks common vision model IDs as image-capable automatically. Pass `--custom-image-input` for unknown custom vision IDs, or `--custom-text-input` to force text-only metadata.
...
```bash
export DAISYCLAW_GATEWAY_TOKEN="your-token"
daisyclaw onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `daisyclaw gateway run`.
...
<AccordionGroup>
  ...
  <Accordion title="Other behaviors">
    ...
    - Fastest first chat: `daisyclaw dashboard` (Control UI, no channel setup).
...
daisyclaw configure
daisyclaw agents add <name>
```

Use `daisyclaw setup` instead when you only need the baseline config/workspace. Use `daisyclaw configure` later for targeted changes and `daisyclaw channels add` for channel-only setup.
