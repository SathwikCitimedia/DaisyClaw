---
summary: "CLI reference for `daisyclaw path` (inspect and edit workspace files via the `oc://` addressing scheme)"
read_when:
...
```bash
daisyclaw plugins enable oc-path
```

## Why use it

DaisyClaw state is spread across human-edited markdown, commented JSONC config,
...
`daisyclaw path` gives those callers a stable address instead of a one-off grep,
...
You probably do not need `daisyclaw path` for ordinary whole-file edits, rich
...
```bash
daisyclaw path resolve 'oc://config.jsonc/plugins/github/enabled'
```

Preview a write without touching disk:

```bash
daisyclaw path set 'oc://config.jsonc/plugins/github/enabled' 'true' --dry-run
```

Find matching records in an append-only JSONL log:

```bash
daisyclaw path find 'oc://session.jsonl/[event=tool_call]/name'
```
...
```bash
daisyclaw path resolve 'oc://AGENTS.md/runtime-safety/daisyclaw-gateway'
```

Validate a path in CI or a preflight script before the script reads or writes:

```bash
daisyclaw path validate 'oc://AGENTS.md/tools/$last/risk'
```
...
`daisyclaw path` does four things:
...
# Read a leaf
daisyclaw path resolve 'oc://gateway.jsonc/version'

# Wildcard search
daisyclaw path find 'oc://session.jsonl/*/event' --file ./logs/session.jsonl

# Dry-run a write
daisyclaw path set 'oc://gateway.jsonc/version' '2.0' --dry-run

# Dry-run a write as a unified diff
daisyclaw path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff

# Apply the write
daisyclaw path set 'oc://gateway.jsonc/version' '2.0'

# Byte-fidelity round-trip (diagnostic)
daisyclaw path emit ./AGENTS.md
```
...
daisyclaw path resolve 'oc://config.jsonc/agents.defaults.models/"anthropic/claude-opus-4-7"/alias'

# Deep JSON/JSONC paths can use slash segments; they normalize to dotted subsegments
daisyclaw path set 'oc://daisyclaw.json/agents/list/0/tools/exec/security' 'allowlist' --dry-run

# Replace a JSONC leaf with a parsed object
daisyclaw path set 'oc://daisyclaw.json/gateway/auth/token' '{"source":"file","provider":"secrets","id":"/test"}' --value-json --dry-run

# Predicate search over JSONC children
daisyclaw path find 'oc://config.jsonc/plugins/[enabled=true]/id'

# Insert into a JSONC array
daisyclaw path set 'oc://config.jsonc/items/+1' '{"id":"new","enabled":true}' --dry-run

# Insert a JSONC object key
daisyclaw path set 'oc://config.jsonc/plugins/+github' '{"enabled":true}' --dry-run

# Append a JSONL event
daisyclaw path set 'oc://session.jsonl/+' '{"event":"checkpoint","ok":true}' --file ./logs/session.jsonl

# Resolve the last JSONL value line
daisyclaw path resolve 'oc://session.jsonl/$last/event' --file ./logs/session.jsonl

# Resolve a YAML workflow step
daisyclaw path resolve 'oc://workflow.yaml/steps/0/id'

# Update a YAML scalar
daisyclaw path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --dry-run

# Address markdown frontmatter
daisyclaw path resolve 'oc://AGENTS.md/[frontmatter]/name'

# Insert markdown frontmatter
daisyclaw path set 'oc://AGENTS.md/[frontmatter]/+description' 'Agent instructions' --dry-run

# Find markdown item fields
daisyclaw path find 'oc://SKILL.md/Tools/*/send_email'

# Validate a session-scoped path
daisyclaw path validate 'oc://AGENTS.md/Tools/$last/risk?session=cron-daily'
```
...
```bash
$ daisyclaw path resolve 'oc://x.md/[frontmatter]/tier' --file frontmatter.md --human
leaf @ L4: "core" (string)

$ daisyclaw path resolve 'oc://x.md/tools/gh/gh' --file frontmatter.md --human
leaf @ L9: "GitHub CLI" (string)

$ daisyclaw path find 'oc://x.md/tools/*' --file frontmatter.md --human
...
```bash
$ daisyclaw path resolve 'oc://config.jsonc/plugins/github/enabled' --file config.jsonc --human
leaf @ L4: "true" (boolean)

$ daisyclaw path set 'oc://config.jsonc/plugins/slack/enabled' 'true' --file config.jsonc --dry-run
...
```bash
$ daisyclaw path find 'oc://session.jsonl/[event=action]/userId' --file session.jsonl --human
1 match for oc://session.jsonl/[event=action]/userId:
  oc://session.jsonl/L2/userId  →  leaf @ L2: "u1" (string)

$ daisyclaw path resolve 'oc://session.jsonl/L2/ts' --file session.jsonl --human
leaf @ L2: "2" (number)
```
...
steps:
  ...
  - id: classify
    command: daisyclaw.invoke
```

```bash
$ daisyclaw path resolve 'oc://workflow.yaml/steps/0/id' --file workflow.yaml --human
leaf @ L3: "fetch" (string)

$ daisyclaw path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --file workflow.yaml --dry-run
...
```bash
daisyclaw path resolve 'oc://AGENTS.md/tools/gh/risk' --human
daisyclaw path resolve 'oc://gateway.jsonc/server/port' --json
```
...
```bash
daisyclaw path find 'oc://AGENTS.md/tools/**/risk'
daisyclaw path find 'oc://session.jsonl/[event=action]/userId'
daisyclaw path find 'oc://config.jsonc/plugins/{github,slack}/enabled'
```
...
daisyclaw path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff
daisyclaw path set 'oc://gateway.jsonc/version' '2.0'
daisyclaw path set 'oc://AGENTS.md/Tools/+gh/risk' 'low'
```
...
```bash
$ daisyclaw path validate 'oc://AGENTS.md/tools/gh' --human
...
```bash
daisyclaw path emit ./AGENTS.md
daisyclaw path emit ./gateway.jsonc --json
```
...
`daisyclaw path` is TTY-aware: human-readable output on a terminal, JSON when
...
- `set` writes bytes through the substrate's emit path, which applies the
  ...
  `__DAISYCLAW_REDACTED__` (verbatim or as a substring) is refused at write
  time.
