---
summary: "CLI reference for `daisyclaw policy` conformance checks"
read_when:
  - You want to check DaisyClaw settings against an authored policy.jsonc
...
`daisyclaw policy` is provided by the bundled Policy plugin. Policy is an
enterprise conformance layer over existing DaisyClaw settings. It does not add a
second configuration system. `policy.jsonc` defines authored requirements,
DaisyClaw observes the active workspace as evidence, and policy health checks
...
data-handling posture, DaisyClaw config secret provider/auth profile posture, and governed tool
...
posture, require DaisyClaw config SecretRefs to use managed providers, require
...
repeatable way to prove that DaisyClaw still conforms to that statement. Use
...
```bash
daisyclaw plugins enable policy
```
...
exposure, agent workspace posture, configured sandbox runtime posture, DaisyClaw
...
when a concrete rule is present. DaisyClaw reads current `channels.*` settings
...
posture, DaisyClaw config agent sandbox workspace access and tool deny posture,
...
`edit`, and `apply_patch`; DaisyClaw config `group:fs` covers file mutation tools
...
present in `policy.jsonc`. The observed state is existing DaisyClaw config or
...
present in `agents.list[]`, DaisyClaw evaluates the scoped rule against inherited
...
Container posture policy is evaluated only against evidence DaisyClaw can
...
```bash
daisyclaw policy check
daisyclaw policy check --json
daisyclaw policy check --severity-min error
```

`policy check` runs only the policy check set and emits evidence, findings, and
attestation hashes. The same findings also appear in `daisyclaw doctor --lint`
when the Policy plugin is enabled.

Compare an operator policy file to an authored baseline policy file:

```bash
daisyclaw policy compare --baseline official.policy.jsonc
daisyclaw policy compare --baseline official.policy.jsonc --policy policy.jsonc --json
```

`policy compare` compares policy file syntax to policy file syntax. It does not
inspect DaisyClaw runtime state, evidence, credentials, or secrets. The command
...
```json
{
  ...
  "evidence": {
    "channels": [
      {
        "id": "telegram",
        "provider": "telegram",
        "source": "oc://daisyclaw.config/channels/telegram",
        "enabled": false
      }
    ],
    "mcpServers": [
      {
        "id": "docs",
        "transport": "stdio",
        "source": "oc://daisyclaw.config/mcp/servers/docs",
        "command": "npx"
      }
    ],
    "modelProviders": [
      {
        "id": "openai",
        "source": "oc://daisyclaw.config/models/providers/openai"
      }
    ],
    "modelRefs": [
      {
        ...
        "provider": "openai",
        "model": "gpt-5.5",
        "source": "oc://daisyclaw.config/agents/defaults/model"
      }
    ],
    "network": [
      {
        "id": "browser-private-network",
        "source": "oc://daisyclaw.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
        "value": false
      }
    ],
    "gatewayExposure": [
    ...
    "agentWorkspace": [
      {
        ...
        "value": "ro",
        "sandboxMode": "all",
        "sandboxModeSource": "oc://daisyclaw.config/agents/defaults/sandbox/mode",
        "sandboxEnabled": true,
        "explicit": true
      },
      {
    ...
    "secrets": [
      {
        "id": "vault",
        "kind": "provider",
        "source": "oc://daisyclaw.config/secrets/providers/vault",
        "providerSource": "env"
      },
      {
        "id": "oc://daisyclaw.config/models/providers/openai/apiKey",
        "kind": "input",
    ...
    "authProfiles": [
      {
        "id": "github",
        "source": "oc://daisyclaw.config/auth/profiles/github",
        "validMetadata": true,
        "provider": "github",
        "mode": "token"
      }
...
records the observed DaisyClaw state used by the policy checks. The
...
2. Run `daisyclaw policy check --json`.
...
5. Re-run `daisyclaw doctor --lint` in CI or release gates.
...
`daisyclaw policy watch` runs the same check repeatedly and reports when the
current evidence no longer matches `expectedAttestationHash`:

```bash
daisyclaw policy watch --json
```
...
```json
{
  ...
  "target": "oc://daisyclaw.config/mcp/servers/remote",
...
```json
{
  ...
  "target": "oc://daisyclaw.config/agents/defaults/model/fallbacks/#0",
...
```json
{
  ...
  "target": "oc://daisyclaw.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
...
```json
{
  ...
  "target": "oc://daisyclaw.config/gateway/bind",
...
```json
{
  ...
  "target": "oc://daisyclaw.config/agents/defaults/sandbox/workspaceAccess",
...
In this version, repair can disable channels that are enabled in DaisyClaw config
