---
summary: "CLI reference for `daisyclaw qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
daisyclaw qr
daisyclaw qr --setup-code-only
daisyclaw qr --json
daisyclaw qr --remote
daisyclaw qr --url wss://gateway.example/ws
```
...
- With `--remote`, DaisyClaw requires either `gateway.remote.url` or
