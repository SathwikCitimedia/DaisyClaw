---
summary: "CLI reference for `daisyclaw secrets` (reload, audit, configure, apply)"
read_when:
...
Use `daisyclaw secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
daisyclaw secrets reload
daisyclaw secrets reload --json
daisyclaw secrets reload --url ws://127.0.0.1:18789 --token <token>
```
...
Scan DaisyClaw state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `daisyclaw.json` refs)
...
```bash
daisyclaw secrets audit
daisyclaw secrets audit --check
daisyclaw secrets audit --json
...
```bash
daisyclaw secrets configure
daisyclaw secrets configure --plan-out /tmp/daisyclaw-secrets-plan.json
daisyclaw secrets configure --apply --yes
daisyclaw secrets configure --providers-only
daisyclaw secrets configure --skip-provider-setup
daisyclaw secrets configure --agent ops
daisyclaw secrets configure --json
```
...
- `configure` targets secret-bearing fields in `daisyclaw.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, DaisyClaw fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
daisyclaw secrets apply --from /tmp/daisyclaw-secrets-plan.json --json
```
...
- `daisyclaw.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.daisyclaw/.env` known secret keys whose values were migrated
