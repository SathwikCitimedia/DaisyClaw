---
summary: "CLI reference for `daisyclaw sessions` (list stored sessions + usage)"
read_when:
...
until a message is processed. Use `daisyclaw channels status --probe`,
`daisyclaw status --deep`, or `daisyclaw health --verbose` when you need live
channel connectivity.

`daisyclaw sessions` and Gateway `sessions.list` responses are bounded by
...
```bash
daisyclaw sessions
daisyclaw sessions --agent work
daisyclaw sessions --all-agents
daisyclaw sessions --active 120
daisyclaw sessions --limit 25
daisyclaw sessions --verbose
daisyclaw sessions --json
```
...
```bash
daisyclaw sessions tail
daisyclaw sessions tail --follow
daisyclaw sessions tail --session-key "agent:main:telegram:direct:123" --tail 25
daisyclaw sessions --agent work tail --follow
daisyclaw sessions --all-agents tail --follow
```

`daisyclaw sessions tail` renders recent trajectory JSONL events as compact progress lines. Without `--session-key`, it tails running sessions first, then the latest stored session. `--tail <count>` controls how many existing events print before follow mode; the default is `80`, and `0` starts at the current end. `--follow` keeps watching the selected trajectory files, including relocated files referenced by `<session>.trajectory-path.json`.
...
```bash
daisyclaw sessions export-trajectory --session-key "agent:main:telegram:direct:123" --workspace .
daisyclaw sessions export-trajectory --session-key "agent:main:telegram:direct:123" --output bug-123 --json
```
...
inside `.daisyclaw/trajectory-exports/` under the selected workspace.

`daisyclaw sessions --all-agents` reads configured agent stores. Gateway and ACP
...
`daisyclaw sessions --all-agents --json`:

```json
{
  "path": null,
  "stores": [
    { "agentId": "main", "path": "/home/user/.daisyclaw/agents/main/sessions/sessions.json" },
    { "agentId": "work", "path": "/home/user/.daisyclaw/agents/work/sessions/sessions.json" }
  ],
  "allAgents": true,
  "count": 2,
  "totalCount": 2,
...
daisyclaw sessions cleanup --agent work --dry-run
daisyclaw sessions cleanup --all-agents --dry-run
daisyclaw sessions cleanup --enforce
daisyclaw sessions cleanup --enforce --active-key "agent:main:telegram:direct:123"
daisyclaw sessions cleanup --dry-run --fix-dm-scope
daisyclaw sessions cleanup --json
```

`daisyclaw sessions cleanup` uses `session.maintenance` settings from config:

- Scope note: `daisyclaw sessions cleanup` maintains session stores, transcripts, and trajectory sidecars. It does not prune cron run history, which is managed by `cron.runLog.keepLines` in [Cron configuration](/automation/cron-jobs#configuration) and explained in [Cron maintenance](/automation/cron-jobs#maintenance).
...
`daisyclaw sessions cleanup --all-agents --dry-run --json`:
...
```json
{
  ...
  "stores": [
    {
      "agentId": "main",
      "storePath": "/home/user/.daisyclaw/agents/main/sessions/sessions.json",
      "beforeCount": 120,
      "afterCount": 80,
      "missing": 0,
      "dmScopeRetired": 0,
    ...
    {
      "agentId": "work",
      "storePath": "/home/user/.daisyclaw/agents/work/sessions/sessions.json",
      "beforeCount": 18,
      "afterCount": 18,
      "missing": 0,
      "dmScopeRetired": 0,
