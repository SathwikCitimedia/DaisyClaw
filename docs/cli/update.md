---
summary: "CLI reference for `daisyclaw update` (safe-ish source update + gateway auto-restart)"
read_when:
  - You want to update a source checkout safely
  - You are debugging `daisyclaw update` output or options
...
Safely update DaisyClaw and switch between stable/beta/dev channels.
...
daisyclaw update repair
daisyclaw update wizard
daisyclaw update --channel beta
daisyclaw update --channel dev
daisyclaw update --tag beta
daisyclaw update --tag main
daisyclaw update --dry-run
daisyclaw update --no-restart
daisyclaw update --yes
daisyclaw update --json
daisyclaw --update
```
...
- `--tag <dist-tag|version|spec>`: override the package target for this update only. For package installs, `main` maps to `github:daisyclaw/daisyclaw#main`; GitHub/git source specs are packed into a temporary tarball before the staged global npm install.
...
`daisyclaw update` does not have a `--verbose` flag. Use `--dry-run` to preview
the planned channel/tag/install/restart actions, `--json` for machine-readable
results, and `daisyclaw update status --json` when you only need channel and
...
<Note>
In Nix mode (`DAISYCLAW_NIX_MODE=1`), mutating `daisyclaw update` runs are disabled. Update the Nix source or flake input for this install instead; for nix-daisyclaw, use the agent-first [Quick Start](https://github.com/daisyclaw/nix-daisyclaw#quick-start). `daisyclaw update status` and `daisyclaw update --dry-run` remain read-only.
</Note>
...
daisyclaw update status --timeout 10
```
...
`daisyclaw update` installed the new core package but post-core plugin sync,
managed npm plugin metadata, registry refresh, or doctor repair still needs to
converge.

```bash
daisyclaw update repair
daisyclaw update repair --channel beta
daisyclaw update repair --json
```
...
`daisyclaw update repair` runs `daisyclaw doctor --fix`, reloads the repaired
...
When you switch channels explicitly (`--channel ...`), DaisyClaw also keeps the
install method aligned:

- `dev` → ensures a git checkout (default: `~/daisyclaw`, or `$DAISYCLAW_HOME/daisyclaw` when
  `DAISYCLAW_HOME` is set; override with `DAISYCLAW_GIT_DIR`),
...
exits, and the helper runs the normal `daisyclaw update --yes --json` CLI path
...
For package-manager installs, `daisyclaw update` resolves the target package
version before invoking the package manager. npm global installs use a staged
install: DaisyClaw installs the new package into a temporary npm prefix, verifies
...
installed DaisyClaw build while leaving full plugin-command completion rebuilds to
explicit `daisyclaw completion --write-state` runs.
...
but launchd is not supervising it, DaisyClaw re-bootstraps the LaunchAgent
...
`Gateway: restart failed: ...` with a manual `daisyclaw gateway restart` hint.
...
- `ok: true`, `result.status: "skipped"`,
  ...
  `daisyclaw update --yes --json` outside the live service process.
- `ok: false`, `result.reason: "managed-service-handoff-unavailable"`, and
  `handoff.status: "unavailable"` mean DaisyClaw could not find a supervising
  service boundary and durable service identity for a safe handoff. For
  example, systemd handoff requires the DaisyClaw unit identity
  (`DAISYCLAW_SYSTEMD_UNIT`), not only ambient systemd process markers. The
...
result. `daisyclaw status` and `daisyclaw status --all` show an `Update restart`
...
<Steps>
  ...
  <Step title="Preflight build (dev only)">
    Runs the TypeScript build in a temp worktree. If the tip fails, walks back up to 10 commits to find the newest buildable commit. Set `DAISYCLAW_UPDATE_PREFLIGHT_LINT=1` to also run lint during this preflight; lint runs in constrained serial mode because user update hosts are often smaller than CI runners.
  </Step>
  <Step title="Rebase">
  ...
  <Step title="Run doctor">
    `daisyclaw doctor` runs as the final safe-update check.
  </Step>
...
beta release, DaisyClaw falls back to the recorded default/latest spec and reports
that as a warning. For npm plugins, DaisyClaw also falls back when the beta
...
<Warning>
If an exact pinned npm plugin update resolves to an artifact whose integrity differs from the stored install record, `daisyclaw update` aborts that plugin artifact update instead of installing it. Reinstall or update the plugin explicitly only after verifying that you trust the new artifact.
</Warning>

<Note>
Post-update plugin sync failures that are scoped to a managed plugin and that the sync path can route around (e.g. an unreachable npm registry for a non-essential plugin) are reported as warnings after the core update succeeds. The JSON result keeps the top-level update `status: "ok"` and reports `postUpdate.plugins.status: "warning"` with `daisyclaw update repair` and `daisyclaw plugins inspect <id> --runtime --json` guidance. Unexpected updater or sync exceptions still fail the update result. Fix the plugin install or update error, then rerun `daisyclaw update repair`.

After the per-plugin sync step, `daisyclaw update` runs a mandatory **post-core convergence** pass before the gateway is restarted: it repairs missing configured plugin payloads, validates each _active_ tracked install record on disk, and statically verifies its `package.json` is parseable (and any explicitly-declared `main` exists). Failures from this pass — and an invalid DaisyClaw config snapshot — return `postUpdate.plugins.status: "error"` and flip the top-level update `status` to `"error"`, so `daisyclaw update` exits non-zero and the gateway is _not_ restarted with an unverified plugin set. The error includes structured `postUpdate.plugins.warnings[].guidance` lines pointing at `daisyclaw update repair` and `daisyclaw plugins inspect <id> --runtime --json` for follow-up. Disabled plugin entries and records that are not trusted-source-linked official sync targets are skipped here, mirroring the `skipDisabledPlugins` policy used by the missing-payload check, so a stale disabled plugin record cannot block an otherwise valid update.
...
`daisyclaw --update` rewrites to `daisyclaw update` (useful for shells and launcher scripts).

## Related

- `daisyclaw doctor` (offers to run update first on git checkouts)
