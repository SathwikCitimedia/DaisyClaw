---
summary: "CLI reference for `daisyclaw webhooks` (Gmail Pub/Sub setup and runner)"
read_when:
  - You want to wire Gmail Pub/Sub events into DaisyClaw
...
```bash
daisyclaw webhooks gmail setup --account <email> [...]
daisyclaw webhooks gmail run   [--account <email>] [...]
```

| Subcommand    | Description                                                                                  |
| ------------- | -------------------------------------------------------------------------------------------- |
| `gmail setup` | Configure Gmail watch, Pub/Sub topic/subscription, and the DaisyClaw webhook delivery target. |
| `gmail run`   | Run `gog watch serve` plus the watch auto-renew loop.                                        |

## `webhooks gmail setup`

Configure Gmail watch, Pub/Sub, and DaisyClaw webhook delivery.

```bash
daisyclaw webhooks gmail setup --account you@example.com
daisyclaw webhooks gmail setup --account you@example.com --project my-gcp-project --json
daisyclaw webhooks gmail setup --account you@example.com --hook-url https://gateway.example.com/hooks/gmail
```
...
### DaisyClaw delivery options

| Flag                   | Default | Description                                |
| ---------------------- | ------- | ------------------------------------------ |
| `--hook-url <url>`     | (none)  | DaisyClaw webhook URL.                      |
| `--hook-token <token>` | (none)  | DaisyClaw webhook token.                    |
...
```bash
daisyclaw webhooks gmail run --account you@example.com
```

`run` accepts the same `gog watch serve`, DaisyClaw delivery, Pub/Sub, and Tailscale flags as `setup`, except:
...
| DaisyClaw delivery | `--hook-url`, `--hook-token`, `--push-token`                                     |
