---
summary: "CLI reference for `daisyclaw wiki` (memory-wiki vault status, search, compile, lint, apply, bridge, and Obsidian helpers)"
read_when:
  - You want to use the memory-wiki CLI
  - You are documenting or changing `daisyclaw wiki`
title: "Wiki"
---
...
Use `daisyclaw wiki` when you want a compiled knowledge vault with:
...
```bash
daisyclaw wiki status
daisyclaw wiki doctor
daisyclaw wiki init
daisyclaw wiki ingest ./notes/alpha.md
daisyclaw wiki compile
daisyclaw wiki lint
daisyclaw wiki search "alpha"
daisyclaw wiki search "who should I ask about Teams?" --mode route-question
daisyclaw wiki get entity.alpha --from 1 --lines 80

daisyclaw wiki apply synthesis "Alpha Summary" \
  --body "Short synthesis body" \
  --source-id source.alpha

daisyclaw wiki apply metadata entity.alpha \
  --source-id source.alpha \
  --status review \
  --question "Still active?"

daisyclaw wiki bridge import
daisyclaw wiki unsafe-local import

daisyclaw wiki obsidian status
daisyclaw wiki obsidian search "alpha"
daisyclaw wiki obsidian open syntheses/alpha-summary.md
daisyclaw wiki obsidian command workspace:quick-switcher
daisyclaw wiki obsidian daily
```
...
- `.daisyclaw-wiki/cache/agent-digest.json`
- `.daisyclaw-wiki/cache/claims.jsonl`
...
For one broad shared recall pass, prefer `daisyclaw memory search` when the
...
```bash
daisyclaw wiki search "bgroux" --mode find-person
daisyclaw wiki search "who knows Teams rollout?" --mode route-question
daisyclaw wiki search "maintainer-whois" --mode source-evidence
daisyclaw wiki search "strong route Teams" --mode raw-claim --json
```
...
```bash
daisyclaw wiki get entity.alpha
daisyclaw wiki get syntheses/alpha-summary.md --from 1 --lines 80
```
...
`daisyclaw wiki` behavior is shaped by:
