---
summary: "CLI reference for `daisyclaw workboard` cards, dispatch, and worker runs"
read_when:
...
`daisyclaw workboard` is the terminal surface for the bundled
...
```bash
daisyclaw plugins enable workboard
...
```bash
daisyclaw workboard list [--board <id>] [--status <status>] [--json]
daisyclaw workboard create <title...> [--notes <text>] [--status <status>] [--priority <priority>] [--agent <id>] [--board <id>] [--labels <items>] [--json]
daisyclaw workboard show <id> [--json]
daisyclaw workboard dispatch [--url <url>] [--token <token>] [--timeout <ms>] [--json]
```
...
```bash
daisyclaw workboard list
daisyclaw workboard list --board default --status ready
daisyclaw workboard list --json
```
...
```bash
daisyclaw workboard create "Fix stale worker heartbeat" --priority high --labels bug,workboard
daisyclaw workboard create "Write Workboard docs" --status ready --agent docs-agent --board docs --notes "Cover CLI, slash command, dispatch, and SQLite state."
```
...
```bash
daisyclaw workboard show 7f4a2c10
daisyclaw workboard show 7f4a2c10 --json
```
...
```bash
daisyclaw workboard dispatch
daisyclaw workboard dispatch --json
daisyclaw workboard dispatch --url http://127.0.0.1:18789 --token "$DAISYCLAW_GATEWAY_TOKEN"
```
...
Local `list`, `create`, and `show` commands operate on the local DaisyClaw state
directory used by the current profile. Use `--dev` or `--profile <name>` on the
top-level `daisyclaw` command when you need a different state root.
...
```bash
daisyclaw plugins inspect workboard --runtime --json
```
...
Then retry `daisyclaw workboard dispatch`. Data-only fallback is useful for local
...
```bash
daisyclaw workboard list --status ready
```
