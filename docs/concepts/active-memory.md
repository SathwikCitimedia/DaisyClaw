...
Paste this into `daisyclaw.json` for a safe-default setup — plugin on, scoped to
...
With those enabled, DaisyClaw can show:
...
case-insensitive. If `allowedChatIds` is non-empty and DaisyClaw cannot resolve a
...
`config.promptOverride` replaces the default Active Memory prompt. DaisyClaw
...
5. Verify memory search itself works with `daisyclaw memory status --deep`.
...
<AccordionGroup>
  <Accordion title="Embedding provider switched or stopped working">
    If `memorySearch.provider` is unset, DaisyClaw uses OpenAI embeddings. Set
  ...
  <Accordion title="Recall feels slow, empty, or inconsistent">
    ...
    - Run `daisyclaw memory status --deep` to inspect the memory-search backend
      and index health.
