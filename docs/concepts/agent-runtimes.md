---
summary: "How DaisyClaw separates model providers, models, channels, and agent runtimes"
title: "Agent runtimes"
read_when:
  - You are choosing between DaisyClaw, Codex, ACP, or another native agent runtime
...
the finished turn to DaisyClaw.
...
| ------------- | -------------------------------------------- | ------------------------------------------------------------------- |
| Provider      | `openai`, `anthropic`, `github-copilot`      | How DaisyClaw authenticates, discovers models, and names model refs. |
| Model         | `gpt-5.5`, `claude-opus-4-6`                 | The model selected for the agent turn.                              |
| Agent runtime | `daisyclaw`, `codex`, `copilot`, `claude-cli` | The low level loop or backend that executes the prepared turn.      |
| Channel       | Telegram, Discord, Slack, WhatsApp           | Where messages enter and leave DaisyClaw.                            |
...
`daisyclaw doctor --fix` removes old whole-agent runtime pins and rewrites
...
- **Embedded harnesses** run inside DaisyClaw's prepared agent loop. Today this
  is the built-in `daisyclaw` runtime plus registered plugin harnesses such as
  `codex` and `copilot`.
...
| Surface                                          | DaisyClaw name/config                 | What it does                                                                                                   |
| ------------------------------------------------ | ------------------------------------ | -------------------------------------------------------------------------------------------------------------- |
...
the native app-server features available; `daisyclaw doctor --fix` owns legacy
...
That means DaisyClaw selects an OpenAI model ref, then asks the Codex app-server
runtime to run the embedded agent turn. It does not mean "use API billing," and
it does not mean the channel, model provider catalog, or DaisyClaw session store
becomes Codex.
...
3. If the user explicitly chooses **DaisyClaw for an OpenAI model**, keep the model ref
   as `openai/<model>` and set provider/model runtime policy to
   `agentRuntime.id: "daisyclaw"`. A selected `openai` OAuth profile is routed
   internally through DaisyClaw's Codex-auth transport.
4. If legacy config still contains **legacy Codex model refs**, repair it to
   `openai/<model>` with `daisyclaw doctor --fix`; doctor keeps the Codex auth
   ...
   app-server route; DaisyClaw no longer keeps a bundled Codex CLI backend.
...
| Surface                     | DaisyClaw embedded                             | Codex app-server                                                            |
| --------------------------- | --------------------------------------------- | --------------------------------------------------------------------------- |
| Model loop owner            | DaisyClaw through the DaisyClaw embedded runner | Codex app-server                                                            |
| Canonical thread state      | DaisyClaw transcript                           | Codex thread, plus DaisyClaw transcript mirror                               |
| DaisyClaw dynamic tools      | Native DaisyClaw tool loop                     | Bridged through the Codex adapter                                           |
| Native shell and file tools | DaisyClaw path                                 | Codex-native tools, bridged through native hooks where supported            |
| Context engine              | Native DaisyClaw context assembly              | DaisyClaw projects assembled context into the Codex turn                     |
| Compaction                  | DaisyClaw or selected context engine           | Codex-native compaction, with DaisyClaw notifications and mirror maintenance |
| Channel delivery            | DaisyClaw                                      | DaisyClaw                                                                    |

This ownership split is the main design rule:

- If DaisyClaw owns the surface, DaisyClaw can provide normal plugin hook behavior.
- If the native runtime owns the surface, DaisyClaw needs runtime events or native hooks.
- If the native runtime owns canonical thread state, DaisyClaw should mirror and project context, not rewrite unsupported internals.

## Runtime selection

DaisyClaw chooses an embedded runtime after provider and model resolution:
...
3. In `auto` mode, registered plugin runtimes can claim supported provider/model
   pairs.
4. If no runtime claims a turn in `auto` mode, DaisyClaw uses `daisyclaw` as the
...
`DAISYCLAW_AGENT_RUNTIME`, session `agentHarnessId`/`agentRuntimeOverride` state,
`agents.defaults.agentRuntime`, and `agents.list[].agentRuntime`. Run
`daisyclaw doctor --fix` to remove stale whole-agent runtime config and convert
legacy runtime model refs where DaisyClaw can preserve the intent.
...
selection/runtime error; it is never silently routed back to DaisyClaw.
...
harness. Explicit DaisyClaw runtime config remains an opt-in compatibility route for
`openai/*` agent turns; when paired with a selected `openai` OAuth profile,
DaisyClaw routes that path internally through the Codex-auth transport while
keeping the public model ref as `openai/*`. Stale OpenAI runtime session pins are
ignored by runtime selection and can be cleaned with `daisyclaw doctor --fix`.

If `daisyclaw doctor` warns that the `codex` plugin is enabled while
legacy Codex model refs remain in config, treat that as legacy route state. Run
`daisyclaw doctor --fix` to rewrite it to `openai/*` with the Codex runtime.

## GitHub Copilot agent runtime

The external `@daisyclaw/copilot` plugin registers an opt-in `copilot` runtime
...
`daisyclaw doctor` auto-loads. For configuration, auth, transcript mirroring,
...
When a runtime is not DaisyClaw, it should document what DaisyClaw surfaces it supports.
...
| Who owns canonical thread history?     | Determines whether DaisyClaw can edit history or only mirror it.                                   |
| Do DaisyClaw dynamic tools work?        | Messaging, sessions, cron, and DaisyClaw-owned tools rely on this.                                 |
| Do dynamic tool hooks work?            | Plugins expect `before_tool_call`, `after_tool_call`, and middleware around DaisyClaw-owned tools. |
...
| What is intentionally unsupported?     | Users should not assume DaisyClaw equivalence where the native runtime owns more state.            |
