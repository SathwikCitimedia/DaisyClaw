...
This is separate from `~/.daisyclaw/`, which stores config, credentials, and sessions.
...
When sandboxing is enabled and `workspaceAccess` is not `"rw"`, tools operate inside a sandbox workspace under `~/.daisyclaw/sandboxes`, not your host workspace.
</Warning>

## Default location

- Default: `~/.daisyclaw/workspace`
- If `DAISYCLAW_PROFILE` is set and not `"default"`, the default becomes `~/.daisyclaw/workspace-<profile>`.
- Override in `~/.daisyclaw/daisyclaw.json`:
...
`daisyclaw onboard`, `daisyclaw configure`, or `daisyclaw setup` will create the workspace and seed the bootstrap files if they are missing.
...
Older installs may have created `~/daisyclaw`. Keeping multiple workspace directories around can cause confusing auth or state drift, because only one workspace is active at a time.

<Note>
**Recommendation:** keep a single active workspace. If you no longer use the extra folders, archive or move them to Trash (for example `trash ~/daisyclaw`). If you intentionally keep multiple workspaces, make sure `agents.defaults.workspace` points to the active one.

`daisyclaw doctor` warns when it detects extra workspace directories.
</Note>

## Workspace file map

These are the standard files DaisyClaw expects inside the workspace:
...
<Note>
If any bootstrap file is missing, DaisyClaw injects a "missing file" marker into the session and continues. Large bootstrap files are truncated when injected; adjust limits with `agents.defaults.bootstrapMaxChars` (default: 20000) and `agents.defaults.bootstrapTotalMaxChars` (default: 60000). `daisyclaw setup` can recreate missing defaults without overwriting existing files.
</Note>

## What is NOT in the workspace

These live under `~/.daisyclaw/` and should NOT be committed to the workspace repo:

- `~/.daisyclaw/daisyclaw.json` (config)
- `~/.daisyclaw/agents/<agentId>/agent/auth-profiles.json` (model auth profiles: OAuth + API keys)
- `~/.daisyclaw/agents/<agentId>/agent/codex-home/` (per-agent Codex runtime account, config, skills, plugins, and native thread state)
- `~/.daisyclaw/credentials/` (channel/provider state plus legacy OAuth import data)
- `~/.daisyclaw/agents/<agentId>/sessions/` (session transcripts + metadata)
- `~/.daisyclaw/skills/` (managed skills)
  ...
  <Step title="Add a private remote">
    <Tabs>
      ...
      <Tab title="GitHub CLI (gh)">
        ```bash
        gh auth login
        gh repo create daisyclaw-workspace --private --source . --remote origin --push
        ```
      </Tab>
...
- Anything under `~/.daisyclaw/`.
- Raw dumps of chats or sensitive attachments.

If you must store sensitive references, use placeholders and keep the real secret elsewhere (password manager, environment variables, or `~/.daisyclaw/`).
</Warning>
...
<Steps>
  <Step title="Clone the repo">
    Clone the repo to the desired path (default `~/.daisyclaw/workspace`).
  </Step>
  <Step title="Update config">
    Set `agents.defaults.workspace` to that path in `~/.daisyclaw/daisyclaw.json`.
  </Step>
  <Step title="Seed missing files">
    Run `daisyclaw setup --workspace <path>` to seed any missing files.
  </Step>
  <Step title="Copy sessions (optional)">
    If you need sessions, copy `~/.daisyclaw/agents/<agentId>/sessions/` from the old machine separately.
  </Step>
</Steps>
