---
summary: "Move one DaisyClaw session's reply route between linked chat channels"
...
Channel docking is call forwarding for one DaisyClaw session.
...
Alice can message DaisyClaw on Telegram and Discord:
...
DaisyClaw keeps the current session context and changes the reply route:
...
4. Keep the same DaisyClaw session, but receive future replies in Discord.
