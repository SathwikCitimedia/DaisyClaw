---
summary: "Delegate architecture: running DaisyClaw as a named agent on behalf of an organization"
title: Delegate architecture
read_when: "You want an agent with its own identity that acts on behalf of humans in an organization."
status: active
---

Goal: run DaisyClaw as a **named delegate** - an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an DaisyClaw agent that:
...
DaisyClaw's default mode is a **personal assistant** - one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of DaisyClaw's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.daisyclaw/workspace-delegate",
  tools: {
...
- Cron run history: DaisyClaw shared SQLite state database
- Session transcripts: `~/.daisyclaw/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through DaisyClaw's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
daisyclaw agents add delegate
```

This creates:

- Workspace: `~/.daisyclaw/workspace-delegate`
- State: `~/.daisyclaw/agents/delegate/agent`
- Sessions: `~/.daisyclaw/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.daisyclaw/workspace" },
      {
        id: "delegate",
        workspace: "~/.daisyclaw/workspace-delegate",
        tools: {
...
~/.daisyclaw/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.daisyclaw/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.daisyclaw/workspace-org",
        agentDir: "~/.daisyclaw/agents/org-assistant/agent",
...
recall view. DaisyClaw redacts credential/token-like text, truncates long
