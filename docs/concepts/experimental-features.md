---
summary: "What experimental flags mean in DaisyClaw and which ones are currently documented"
...
Experimental features in DaisyClaw are **opt-in preview surfaces**. They are
...
- If you are rolling DaisyClaw out broadly, test experimental flags in a smaller
...
| ------------------------ | ------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| Local model runtime      | `agents.defaults.experimental.localModelLean`, `agents.list[].experimental.localModelLean` | A smaller or stricter local backend chokes on DaisyClaw's full default tool surface                                                | [Local Models](/gateway/local-models)                                                         |
...
| Codex harness            | `plugins.entries.codex.config.appServer.experimental.sandboxExecServer`                    | You want native Codex app-server 0.132.0 or newer to target an DaisyClaw sandbox-backed exec-server instead of disabling Code Mode | [Codex harness reference](/plugins/codex-harness-reference#sandboxed-native-execution)        |
...
`agents.defaults.experimental.localModelLean: true` is a pressure-release valve for weaker local-model setups. When it is on, DaisyClaw drops three default tools — `browser`, `cron`, and `message` — from the agent's tool surface for every turn. It also defaults that run to structured Tool Search controls when `tools.toolSearch` is not explicitly configured, so larger plugin, MCP, or client tool catalogs stay behind `tool_search`, `tool_describe`, and `tool_call` instead of being dumped into the prompt. Runs that require direct `message` delivery keep that tool direct instead of enabling the lean-mode Tool Search default. Use `agents.list[].experimental.localModelLean` to enable or disable the same behavior for one configured agent.

### Why these three tools

These three tools have the largest descriptions and the most parameter shapes in the default DaisyClaw runtime. On a small-context or stricter OpenAI-compatible backend that is the difference between:
...
Removing them does not silently rewire DaisyClaw — it just makes the direct tool list shorter. The model still has `read`, `write`, `edit`, `exec`, `apply_patch`, web search/fetch (when configured), memory, and session/agent tools available. Extra catalogs remain callable through Tool Search unless you explicitly set `tools.toolSearch: false`.
...
1. `daisyclaw infer model run --gateway --model <ref> --prompt "Reply with exactly: pong"` succeeds.
...
If you already tune Tool Search globally, DaisyClaw leaves that operator config alone. Set `tools.toolSearch: false` to opt out of the lean-mode Tool Search default.
...
If a feature is experimental, DaisyClaw should say so plainly in docs and in the
