---
summary: "Mantis is the visual end-to-end verification system for reproducing DaisyClaw bugs on live transports, capturing before and after evidence, and attaching artifacts to PRs."
title: "Mantis"
read_when:
  - Building or running live visual QA for DaisyClaw bugs
...
Mantis is the DaisyClaw end-to-end verification system for bugs that need a real
...
Mantis lives in the DaisyClaw QA stack.

- DaisyClaw owns the scenario runtime, transport adapters, evidence schema, and
  local CLI under `pnpm daisyclaw qa mantis`.
...
- DaisyClaw agents drive Mantis through Codex when a scenario needs agentic setup,
  debugging, or stuck-state reporting.

This boundary keeps transport knowledge in DaisyClaw, machine scheduling in
...
```bash
pnpm daisyclaw qa mantis discord-smoke \
...
thread, calls DaisyClaw's `message.thread-reply` action with a repo-local
...
```bash
pnpm daisyclaw qa mantis desktop-browser-smoke \
...
`DAISYCLAW_MANTIS_CRABBOX_PROVIDER` when running against another Crabbox fleet.

Useful desktop smoke flags:

- `--lease-id <cbx_...>` or `DAISYCLAW_MANTIS_CRABBOX_LEASE_ID` reuses a warmed desktop.
...
- `--browser-profile-archive-env <name>` restores a base64 `.tgz` Chrome user-data-dir archive from the named environment variable before launching the browser. Use this for logged-in witnesses such as Discord Web. The default env var is `DAISYCLAW_MANTIS_BROWSER_PROFILE_TGZ_B64`.
...
- `--keep-lease` or `DAISYCLAW_MANTIS_KEEP_VM=1` keeps a newly created passing lease open for VNC inspection. Failed runs keep the lease by default when one was created so an operator can reconnect.
...
REST. When `DAISYCLAW_QA_DISCORD_CAPTURE_UI_METADATA=1` is set, the scenario also
writes a Discord Web URL artifact. When `DAISYCLAW_QA_DISCORD_KEEP_THREADS=1` is
...
the VM, runs `pnpm daisyclaw qa slack` inside that VM, opens Slack Web in the VNC
...
shape where the SUT DaisyClaw gateway and the browser both live inside the same
Linux desktop VM.

With `--gateway-setup`, the command prepares a persistent disposable DaisyClaw
home at `$HOME/.daisyclaw-mantis/slack-daisyclaw`, patches Slack Socket Mode
configuration for the selected channel, starts `daisyclaw gateway run` on port
...
- `DAISYCLAW_LIVE_OPENAI_KEY` for the remote model lane. If only
  `OPENAI_API_KEY` is set locally, Mantis maps it to `DAISYCLAW_LIVE_OPENAI_KEY`
  before invoking Crabbox so Crabbox's `DAISYCLAW_*` env forwarding can carry it
  into the VM.
...
channel id, Socket Mode app token, and bot token as the `DAISYCLAW_MANTIS_SLACK_*`
...
- `--gateway-setup` starts a persistent DaisyClaw Slack gateway in the VM instead of only running the bot-to-bot QA lane.
...
- `DAISYCLAW_MANTIS_SLACK_BROWSER_PROFILE_DIR` controls the persistent Chrome profile inside the VM. The default is `$HOME/.config/daisyclaw-mantis/slack-chrome-profile`, so a manual Slack Web login survives reruns on the same lease.
...
Crabbox CLI from
`daisyclaw/crabbox` main so it can use the current desktop/browser lease flags
...
runs `pnpm daisyclaw qa mantis slack-desktop-smoke --gateway-setup` against that
...
worktree, runs `pnpm daisyclaw qa telegram --credential-source convex
...
`@daisyclaw-mantis telegram desktop proof`, from the Actions UI with freeform
...
DaisyClaw with the leased Telegram SUT bot token, starts `daisyclaw gateway run`
...
token never logs Telegram Desktop in; it only configures DaisyClaw. The desktop
...
- `--no-gateway-setup` installs and opens Telegram Desktop without configuring DaisyClaw.
...
comment or creates one. The workflows publish to `daisyclaw-crabbox-artifacts`
with public URLs under `https://artifacts.daisyclaw.ai`. They provide bucket,
...
```text
@daisyclaw-mantis discord status reactions
```
...
```text
@daisyclaw-mantis discord status reactions baseline=origin/main candidate=HEAD
```
...
6. Start a child DaisyClaw Gateway with an isolated state directory.
...
- It exercises a real DaisyClaw Gateway, Discord bot auth, message dispatch,
...
- `pnpm daisyclaw qa discord` already runs a live Discord lane with driver and
  SUT bots.
...
- **Headless automation**: default for CI. Chrome runs with CDP enabled, and
  Playwright or DaisyClaw browser control captures screenshots.
...
- Node 22 and pnpm
- DaisyClaw checkout and dependency cache
- Playwright Chromium browser cache when Playwright is used
- enough CPU and memory for one DaisyClaw Gateway, one browser, and one model run
...
- `DAISYCLAW_QA_DISCORD_MANTIS_BOT_TOKEN`
...
- `DAISYCLAW_QA_DISCORD_NOTIFY_CHANNEL_ID`
- `DAISYCLAW_QA_REDACT_PUBLIC_METADATA=1` for public GitHub artifact uploads
- `DAISYCLAW_QA_CONVEX_SITE_URL`
- `DAISYCLAW_QA_CONVEX_SECRET_CI`
- `DAISYCLAW_QA_MANTIS_CRABBOX_COORDINATOR`
- `DAISYCLAW_QA_MANTIS_CRABBOX_COORDINATOR_TOKEN`
...
`DAISYCLAW_QA_REDACT_PUBLIC_METADATA=1` for this reason.
