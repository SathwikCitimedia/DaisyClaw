...
```bash
daisyclaw plugins install @daisyclaw/llama-cpp-provider
```
...
| Local             | `local`             | `@daisyclaw/llama-cpp-provider`      |
| Mistral           | `mistral`           |                                     |
...
DaisyClaw indexes `MEMORY.md` and `memory/*.md` into chunks (~400 tokens with
80-token overlap) and stores them in a per-agent SQLite database.

- **Index location:** `~/.daisyclaw/memory/<agentId>.sqlite`
...
- **Reindex on demand:** `daisyclaw memory index --force`
...
**Memory search disabled?** Check `daisyclaw memory status`. If no provider is
...
**Stale results?** Run `daisyclaw memory index --force` to rebuild. The watcher
may miss changes in rare edge cases.

**sqlite-vec not loading?** DaisyClaw falls back to in-process cosine similarity
automatically. `daisyclaw memory status --deep` reports the local vector store
