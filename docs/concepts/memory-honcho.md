...
[Honcho](https://honcho.dev) adds AI-native memory to DaisyClaw. It persists
...
```bash
daisyclaw plugins install @honcho-ai/daisyclaw-honcho
daisyclaw honcho setup
...
Settings live under `plugins.entries["daisyclaw-honcho"].config`:
    ...
    entries: {
      "daisyclaw-honcho": {
        config: {
          ...
          workspaceId: "daisyclaw", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `daisyclaw honcho setup` detects and
...
```bash
daisyclaw honcho setup                        # Configure API key and migrate files
daisyclaw honcho status                       # Check connection status
daisyclaw honcho ask <question>               # Query Honcho about the user
daisyclaw honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/daisyclaw-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho DaisyClaw integration guide](https://docs.honcho.dev/v3/guides/integrations/daisyclaw)
- [Memory](/concepts/memory) -- DaisyClaw memory overview
