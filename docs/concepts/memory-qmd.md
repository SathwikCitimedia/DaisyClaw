...
alongside DaisyClaw. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, DaisyClaw falls back to the
...
DaisyClaw creates a self-contained QMD home under
`~/.daisyclaw/agents/<agentId>/qmd/` and manages the sidecar lifecycle
...
- DaisyClaw creates collections from your workspace memory files and any
...
- If QMD fails entirely, DaisyClaw falls back to the builtin SQLite engine.
  ...
  `daisyclaw memory status` and one-shot CLI probes still recheck QMD directly.
...
DaisyClaw keeps the QMD search path compatible with both current and older QMD
installs.

On startup, DaisyClaw checks the installed QMD help text once per manager. If the
binary advertises support for multiple collection filters, DaisyClaw searches all
...
Older QMD builds only accept one collection filter. When DaisyClaw detects one
...
process, so you can tune QMD globally without adding new DaisyClaw config:
...
collection under `~/.daisyclaw/agents/<id>/qmd/sessions/`.
...
When scope denies a search, DaisyClaw logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If DaisyClaw
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

If `qmd --version` works in your shell but DaisyClaw still reports
...
with `daisyclaw memory status --deep`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs DaisyClaw uses.

**Many QMD subprocesses during search?** Update QMD if possible. DaisyClaw uses
...
`memory.qmd.searchMode = "search"`. DaisyClaw treats that mode as lexical-only,
...
the next startup reconciliation. DaisyClaw recreates stale managed collections
...
DaisyClaw's builtin symlink rules. Keep temporary monorepo checkouts under
