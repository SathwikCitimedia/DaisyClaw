...
`@daisyclaw/llama-cpp-provider` and set `provider: "local"`. Source checkouts
...
DaisyClaw runs two retrieval paths in parallel and merges the results:
...
**No results?** Run `daisyclaw memory status` to check the index. If empty, run
