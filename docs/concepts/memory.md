---
summary: "How DaisyClaw remembers things across sessions"
...
DaisyClaw remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.daisyclaw/workspace`).
...
If `MEMORY.md` grows past the bootstrap file budget, DaisyClaw keeps the file on
...
`daisyclaw doctor` to see raw vs injected sizes and truncation status.
...
Memory can preserve approval context, but it does not enforce policy. Use DaisyClaw approval settings, sandboxing, and scheduled tasks for hard operational controls.
...
for that case. DaisyClaw infers them in a hidden background pass, scopes them to
...
<Info>
DaisyClaw uses OpenAI embeddings by default. Set
...
Before [compaction](/concepts/compaction) summarizes your conversation, DaisyClaw
...
```bash
daisyclaw memory rem-backfill --rollback
daisyclaw memory rem-backfill --rollback-short-term
```

## CLI

```bash
daisyclaw memory status          # Check index status and provider
daisyclaw memory search "query"  # Search from the command line
daisyclaw memory index --force   # Rebuild the index
```
