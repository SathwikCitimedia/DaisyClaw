...
That shape fixes local bugs, but it leaves DaisyClaw with too many public
...
That gives DaisyClaw at-least-once recovery. Exactly-once behavior exists only
...
- Structured DaisyClaw-origin metadata for operational/system output so visible
...
DaisyClaw should borrow the vocabulary, not copy the surface.

What DaisyClaw needs beyond that model:
...
`thread.post()` style promises are not enough for DaisyClaw. They hide the
...
Origin describes who produced a message and how DaisyClaw should treat echoes of
that message. It is separate from relation: a message can be a reply to a user
and still be DaisyClaw-originated operational output.
...
Core owns the meaning of DaisyClaw-originated output. Channels own how that
...
DaisyClaw operational output must not be accepted as bot-authored input in shared
...
- **Transport ack:** tells the platform webhook or socket that DaisyClaw accepted
...
- **Inbound record ack:** confirms DaisyClaw persisted enough inbound metadata to
...
Before bot authorization, receive must apply the shared DaisyClaw echo policy
when the channel can decode message origin metadata:

```typescript
function shouldDropDaisyClawEcho(params: {
  origin?: MessageOrigin;
  isBotAuthor: boolean;
  isRoomish: boolean;
}): boolean {
  return (
    params.isBotAuthor &&
    params.isRoomish &&
    params.origin?.source === "daisyclaw" &&
...
same visible gateway-failure text but without DaisyClaw origin metadata still
...
middleware chain, but DaisyClaw persists only the safe completed update id after
...
source if we need platform-level redelivery beyond DaisyClaw's restart
...
process dies there, DaisyClaw cannot know whether the platform message exists
...
```typescript
import { defineChannelMessageAdapter } from "daisyclaw/plugin-sdk/channel-outbound";
```
...
Before preflight authorization, core must run the shared DaisyClaw echo predicate
whenever `origin.decode` returns DaisyClaw-origin metadata. The receive adapter
...
- iMessage monitor delivery records sent messages in an echo cache after a
  ...
  DaisyClaw can re-ingest its own final replies as inbound user messages.
...
- Direct-DM helpers can have a reply callback that is the only correct transport
  ...
  that callback.
- DaisyClaw gateway failure output must stay visible to humans, but tagged
...
| Slack           | Send adapter handles normal chat posts. Live adapter chooses native stream when thread shape supports it, otherwise draft preview. Receipts preserve thread timestamps. Origin adapter maps DaisyClaw gateway failures to Slack `chat.postMessage.metadata` and drops tagged bot-room echoes before `allowBots` authorization.                                  |
...
| Matrix          | Live adapter owns draft event edits, finalization, redaction, encrypted media constraints, and reply-target mismatch fallback. Receive adapter owns encrypted event hydration and dedupe. Origin adapter should encode DaisyClaw gateway-failure origin into Matrix event content and drop configured-bot room echoes before `allowBots` handling.              |
...
| Google Chat     | Simple receive plus send adapter with thread relation mapped to spaces and thread ids. Audit `allowBots=true` room behavior for tagged DaisyClaw gateway-failure echoes.                                                                                                                                                                                        |
...
- Persist message origin through durable send intents so recovery, replay, and
  chunked sends preserve DaisyClaw operational provenance.
...
- Add `daisyclaw/plugin-sdk/channel-outbound`.
...
- Include `MessageOrigin`, origin encode/decode hooks, and the shared
  `shouldDropDaisyClawEcho` predicate in the channel-outbound SDK surface.
...
This is where the model stops being "agent replies" and becomes "DaisyClaw sends
messages".
...
- Gateway-failure origin factory and `shouldDropDaisyClawEcho` predicate.
...
- Slack tagged DaisyClaw gateway failure messages stay visible outbound, tagged
...
- Matrix tagged DaisyClaw gateway-failure room echoes from configured bot
...
- Whether Telegram should eventually replace the grammY runner source with a
  ...
  only DaisyClaw's persisted restart watermark.
...
- DaisyClaw-originated gateway failure output is visible to humans but tagged
