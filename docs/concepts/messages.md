...
DaisyClaw handles inbound messages through a pipeline of session resolution, queueing, streaming, tool execution, and reasoning visibility. This page maps the path from inbound message to reply.
...
Channels can redeliver the same message after reconnects. DaisyClaw keeps a
...
DaisyClaw keeps that boundary explicit:
...
DaisyClaw separates the **prompt body** from the **command body**:
...
DaisyClaw can expose or hide model reasoning:
...
When a turn also has pending tool media, such as generated TTS audio, DaisyClaw
strips the silent text but still delivers the media attachment.
DaisyClaw resolves that behavior by conversation type:

- Direct conversations never receive `NO_REPLY` prompt guidance. If a direct
  run accidentally returns a bare silent token, DaisyClaw suppresses it instead
...
DaisyClaw also uses silent replies for generic internal runner failures in
