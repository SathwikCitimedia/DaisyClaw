...
An **agent** here is the full per-persona scope: workspace files, auth profiles, model registry, and session store. `agentDir` is the on-disk state directory that holds this per-agent config at `~/.daisyclaw/agents/<agentId>/`. A **binding** maps a channel account (e.g. a Slack workspace or a WhatsApp number) to one of those agents.
...
- **Session store** (chat history + routing state) under `~/.daisyclaw/agents/<agentId>/sessions`.
...
a local profile, but DaisyClaw does not clone OAuth refresh tokens into the
...
Skills are loaded from each agent workspace plus shared roots such as `~/.daisyclaw/skills`, then filtered by the effective agent skill allowlist when configured. Use `agents.defaults.skills` for a shared baseline and `agents.list[].skills` for per-agent replacement. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills) and [Skills: agent skill allowlists](/tools/skills#agent-skill-allowlists).
...
- State dir: `~/.daisyclaw` (or `DAISYCLAW_STATE_DIR`)
- Workspace: `~/.daisyclaw/workspace` (or `~/.daisyclaw/workspace-<agentId>`)
- Agent dir: `~/.daisyclaw/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.daisyclaw/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, DaisyClaw runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/.daisyclaw/workspace` (or `~/.daisyclaw/workspace-<profile>` when `DAISYCLAW_PROFILE` is set).
- State defaults to `~/.daisyclaw/agents/main/agent`.
...
```bash
daisyclaw agents list --bindings
```
...
<Steps>
  <Step title="Create each agent workspace">
    ...
    ```bash
    daisyclaw agents add coding
    daisyclaw agents add social
    ```

    Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.daisyclaw/agents/<agentId>`.
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.daisyclaw/workspace-alex" },
      { id: "mia", workspace: "~/.daisyclaw/workspace-mia" },
    ],
  },
  bindings: [
    {
...
<AccordionGroup>
  ...
  <Accordion title="Account-scope detail">
    ...
    - If you later add the same binding for the same agent with an explicit account id, DaisyClaw upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
If you want a channel-wide default account when `accountId` is omitted, set `channels.<channel>.defaultAccount` (optional). When unset, DaisyClaw falls back to `default` if present, otherwise the first configured account id (sorted).
...
<AccordionGroup>
  <Accordion title="Discord bots per agent">
    ...
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.daisyclaw/workspace-main" },
          { id: "coding", workspace: "~/.daisyclaw/workspace-coding" },
        ],
      },
      bindings: [
  ...
  <Accordion title="Telegram bots per agent">
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.daisyclaw/workspace-main" },
          { id: "alerts", workspace: "~/.daisyclaw/workspace-alerts" },
        ],
      },
      bindings: [
  ...
  <Accordion title="WhatsApp numbers per agent">
    ...
    ```bash
    daisyclaw channels login --channel whatsapp --account personal
    daisyclaw channels login --channel whatsapp --account biz
    ```

    `~/.daisyclaw/daisyclaw.json` (JSON5):
    ...
    ```js
    {
        ...
        list: [
          {
            ...
            default: true,
            name: "Home",
            workspace: "~/.daisyclaw/workspace-home",
            agentDir: "~/.daisyclaw/agents/home/agent",
          },
          {
            id: "work",
            name: "Work",
            workspace: "~/.daisyclaw/workspace-work",
            agentDir: "~/.daisyclaw/agents/work/agent",
          },
        ],
      },
      ...
      channels: {
        whatsapp: {
          accounts: {
            personal: {
              // Optional override. Default: ~/.daisyclaw/credentials/whatsapp/personal
              // authDir: "~/.daisyclaw/credentials/whatsapp/personal",
            },
            biz: {
              // Optional override. Default: ~/.daisyclaw/credentials/whatsapp/biz
        ...
        list: [
          {
            id: "chat",
            name: "Everyday",
            workspace: "~/.daisyclaw/workspace-chat",
            model: "anthropic/claude-sonnet-4-6",
          },
          {
            id: "opus",
            name: "Deep Work",
            workspace: "~/.daisyclaw/workspace-opus",
