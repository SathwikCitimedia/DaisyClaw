---
summary: "OAuth in DaisyClaw: token exchange, storage, and multi-account patterns"
read_when:
  - You want to understand DaisyClaw OAuth end-to-end
...
DaisyClaw supports "subscription auth" via OAuth for providers that offer it
(notably **OpenAI Codex (ChatGPT OAuth)**). For Anthropic, the practical split
is now:

- **Anthropic API key**: normal Anthropic API billing
- **Anthropic Claude CLI / subscription auth inside DaisyClaw**: Anthropic staff
  told us this usage is allowed again

OpenAI Codex OAuth is explicitly supported for use in external tools like
DaisyClaw.

DaisyClaw stores both OpenAI API-key auth and ChatGPT/Codex OAuth under the
...
`daisyclaw doctor --fix`; use `openai:*` profile ids and `auth.order.openai` for
new config.
...
DaisyClaw also supports **provider plugins** that ship their own OAuth or API-key
flows. Run them via:

```bash
daisyclaw models auth login --provider <id>
```
...
- you log in via DaisyClaw _and_ via Claude Code / Codex CLI → one of them randomly gets "logged out" later

To reduce that, DaisyClaw treats `auth-profiles.json` as a **token sink**:

- the runtime reads credentials from **one place**
- we can keep multiple profiles and route them deterministically
- external CLI reuse is provider-specific: Codex CLI can bootstrap an empty
  `openai:default` profile, but once DaisyClaw has a local OAuth profile,
  the local refresh token is canonical. If that local refresh token is rejected,
  DaisyClaw can use a usable same-account Codex CLI token as a runtime-only
...
- Auth profiles (OAuth + API keys + optional value-level refs): `~/.daisyclaw/agents/<agentId>/agent/auth-profiles.json`
- Legacy compatibility file: `~/.daisyclaw/agents/<agentId>/agent/auth.json`
  (static `api_key` entries are scrubbed when discovered)

Legacy import-only file (still supported, but not the main store):

- `~/.daisyclaw/credentials/oauth.json` (imported into `auth-profiles.json` on first use)

All of the above also respect `$DAISYCLAW_STATE_DIR` (state dir override). Full reference: [/gateway/configuration](/gateway/configuration-reference#auth-storage)

For static secret refs and runtime snapshot activation behavior, see [Secrets Management](/gateway/secrets).

When a secondary agent has no local auth profile, DaisyClaw uses read-through
...
Claude subscription limits, and Anthropic staff told us DaisyClaw-style Claude
CLI usage is allowed again. DaisyClaw therefore treats Claude CLI reuse and
...
If you want other subscription-style options in DaisyClaw, see [OpenAI
Codex](/providers/openai), [Qwen Cloud Coding
Plan](/providers/qwen), [MiniMax Coding Plan](/providers/minimax),
and [Z.AI / GLM Coding Plan](/providers/zai).
</Warning>

DaisyClaw also exposes Anthropic setup-token as a supported token-auth path, but it now prefers Claude CLI reuse and `claude -p` when available.

## Anthropic Claude CLI migration

DaisyClaw supports Anthropic Claude CLI reuse again. If you already have a local
Claude login on the host, onboarding/configure can reuse it directly.

## OAuth exchange (how login works)

DaisyClaw's interactive login flows are implemented in `daisyclaw/plugin-sdk/llm` and wired into the wizards/commands.

### Anthropic setup-token

Flow shape:

1. start Anthropic setup-token or paste-token from DaisyClaw
2. DaisyClaw stores the resulting Anthropic credential in an auth profile
3. model selection stays on `anthropic/...`
4. existing Anthropic auth profiles remain available for rollback/order control

### OpenAI Codex (ChatGPT OAuth)

OpenAI Codex OAuth is explicitly supported for use outside the Codex CLI, including DaisyClaw workflows.
...
`daisyclaw models auth list --provider openai` after repair before copying
...
Wizard path is `daisyclaw onboard` → auth choice `openai`.
...
- exception: some external CLI credentials stay externally managed; DaisyClaw
  ...
  `openai:default` profile, then DaisyClaw-owned refreshes keep the local
  profile canonical. If the local Codex refresh fails and Codex CLI has a
  usable token for the same account, DaisyClaw may use that token for the current
...
- `daisyclaw channels list --json` (shows `auth[]`)
