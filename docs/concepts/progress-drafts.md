...
read_when:
  ...
  - Explaining how DaisyClaw updates one channel message while work is in progress
...
When progress drafts are enabled, DaisyClaw creates one visible work-in-progress
...
That is usually enough. DaisyClaw will pick an automatic one-word label, wait
...
DaisyClaw sends the final answer normally and cleans up or stops updating the
...
The default label is `auto`, which chooses from DaisyClaw's built-in
...
DaisyClaw renders only the `progress.text` in the channel progress UI. The
...
DaisyClaw uses the same formatter for progress drafts and `/verbose`:
...
DaisyClaw truncates long progress lines by default so repeated draft edits do not
...
With `toolProgress: false`, DaisyClaw still suppresses the older standalone
...
When the final answer is ready, DaisyClaw tries to keep the chat clean:

- If the draft can safely become the final answer, DaisyClaw edits it in place.
- If the channel uses native progress streaming, DaisyClaw finalizes that stream
...
- If the final answer has media, an approval prompt, an explicit reply target,
  too many chunks, or a failed edit/send, DaisyClaw sends the final answer through
...
Check `streaming.progress.toolProgress`. If it is `false`, DaisyClaw keeps the
