...
The private QA stack is meant to exercise DaisyClaw in a more realistic,
...
Every QA flow runs under `pnpm daisyclaw qa <subcommand>`. Many have `pnpm qa:*`
script aliases; both forms are supported.

| Command                                             | Purpose                                                                                                                                                                                                                                                                 |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `qa run`                                            | Bundled QA self-check; writes a Markdown report.                                                                                                                                                                                                                        |
| `qa suite`                                          | Run repo-backed scenarios against the QA gateway lane. Aliases: `pnpm daisyclaw qa suite --runner multipass` for a disposable Linux VM.                                                                                                                                  |
| `qa coverage`                                       | Print the markdown scenario-coverage inventory (`--json` for machine output).                                                                                                                                                                                           |
| `qa parity-report`                                  | Compare two `qa-suite-summary.json` files and write the agentic parity report, or use `--runtime-axis --token-efficiency` to write Codex-vs-DaisyClaw runtime parity and token-efficiency reports from one runtime-pair summary.                                         |
...
```bash
pnpm daisyclaw qa docker-build-image
pnpm qa:lab:build
pnpm qa:lab:up:fast
pnpm qa:lab:watch
```
...
`daisyclaw.run`, `daisyclaw.harness.run`, a latest GenAI semantic-convention
model-call span, `daisyclaw.context.assembled`, and `daisyclaw.message.delivery`
...
`daisyclaw.content.*` attributes must stay out of the trace. The raw OTLP
...
```bash
pnpm daisyclaw qa matrix --profile fast --fail-fast
```
...
CI uses the same command surface in `.github/workflows/qa-live-transports-convex.yml`. Scheduled and default manual runs execute the fast Matrix profile with live frontier credentials, `--fast`, and `DAISYCLAW_QA_MATRIX_NO_REPLY_WINDOW_MS=3000`. Manual `matrix_profile=all` fans out into the five profile shards so the exhaustive catalog can run in parallel while keeping one artifact directory per shard.

For transport-real Telegram, Discord, Slack, and WhatsApp smoke lanes:

```bash
pnpm daisyclaw qa telegram
pnpm daisyclaw qa discord
pnpm daisyclaw qa slack
pnpm daisyclaw qa whatsapp
```
...
```bash
pnpm daisyclaw qa mantis slack-desktop-smoke \
  --gateway-setup \
...
With `--gateway-setup`, Mantis leaves a persistent DaisyClaw Slack gateway
...
```bash
pnpm daisyclaw qa mantis visual-task \
...
`visual-driver`, captures `visual-task.png`, runs `daisyclaw infer image describe`
...
`daisyclaw/plugin-sdk/qa-live-transport-scenarios`.
...
```bash
pnpm daisyclaw qa suite --runner multipass --scenario channel-chat-baseline
```

This boots a fresh Multipass guest, installs dependencies, builds DaisyClaw
...
- `DAISYCLAW_QA_TELEGRAM_GROUP_ID` - numeric chat id (string).
- `DAISYCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN`
- `DAISYCLAW_QA_TELEGRAM_SUT_BOT_TOKEN`

Optional:

- `DAISYCLAW_QA_TELEGRAM_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts (default redacts).
...
The implicit default set always covers canary, mention gating, native command replies, command addressing, and bot-to-bot group replies. `mock-openai` defaults also include deterministic reply-chain and final-message streaming checks. `telegram-current-session-status-tool` remains opt-in because it is only stable when threaded directly after canary, not after arbitrary native command replies. Use `pnpm daisyclaw qa telegram --list-scenarios --provider-mode mock-openai` to print the current default/optional split with regression refs.

Output artifacts:

- `telegram-qa-report.md`
- `telegram-qa-summary.json` - includes per-reply RTT (driver send → observed SUT reply) starting with the canary.
- `telegram-qa-observed-messages.json` - bodies redacted unless `DAISYCLAW_QA_TELEGRAM_CAPTURE_CONTENT=1`.
...
`DAISYCLAW_NPM_TELEGRAM_WARM_SAMPLES` and
`DAISYCLAW_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS`, so `result.json` remains comparable
...
Targets one real private Discord guild channel with two bots: a driver bot controlled by the harness and a SUT bot started by the child DaisyClaw gateway through the bundled Discord plugin. Verifies channel mention handling, that the SUT bot has registered the native `/help` command with Discord, and opt-in Mantis evidence scenarios.

Required env when `--credential-source env`:

- `DAISYCLAW_QA_DISCORD_GUILD_ID`
- `DAISYCLAW_QA_DISCORD_CHANNEL_ID`
- `DAISYCLAW_QA_DISCORD_DRIVER_BOT_TOKEN`
- `DAISYCLAW_QA_DISCORD_SUT_BOT_TOKEN`
- `DAISYCLAW_QA_DISCORD_SUT_APPLICATION_ID` - must match the SUT bot user id returned by Discord (the lane fails fast otherwise).

Optional:

- `DAISYCLAW_QA_DISCORD_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts.
- `DAISYCLAW_QA_DISCORD_VOICE_CHANNEL_ID` selects the voice/stage channel for `discord-voice-autojoin`; without it, the scenario picks the first visible voice/stage channel for the SUT bot.
...
- `discord-qa-observed-messages.json` - bodies redacted unless `DAISYCLAW_QA_DISCORD_CAPTURE_CONTENT=1`.
...
Targets one real private Slack channel with two distinct bots: a driver bot controlled by the harness and a SUT bot started by the child DaisyClaw gateway through the bundled Slack plugin.

Required env when `--credential-source env`:

- `DAISYCLAW_QA_SLACK_CHANNEL_ID`
- `DAISYCLAW_QA_SLACK_DRIVER_BOT_TOKEN`
- `DAISYCLAW_QA_SLACK_SUT_BOT_TOKEN`
- `DAISYCLAW_QA_SLACK_SUT_APP_TOKEN`

Optional:

- `DAISYCLAW_QA_SLACK_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts.
- `DAISYCLAW_QA_SLACK_APPROVAL_CHECKPOINT_DIR` enables visual approval
...
- `DAISYCLAW_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS` overrides the checkpoint
...
- `slack-qa-observed-messages.json` - bodies redacted unless `DAISYCLAW_QA_SLACK_CAPTURE_CONTENT=1`.
- `approval-checkpoints/` - only when Mantis sets
  `DAISYCLAW_QA_SLACK_APPROVAL_CHECKPOINT_DIR`; contains checkpoint JSON,
...
{
  "display_information": {
    "name": "DaisyClaw QA Driver",
    "description": "Test driver bot for DaisyClaw QA Slack live lane"
  },
  "features": {
    "bot_user": {
      "display_name": "DaisyClaw QA Driver",
      "always_online": true
    }
  },
  "oauth_config": {
...
{
  "display_information": {
    "name": "DaisyClaw QA SUT",
    "description": "DaisyClaw QA SUT connector for DaisyClaw"
  },
  "features": {
    "bot_user": {
      "display_name": "DaisyClaw QA SUT",
      "always_online": true
    },
    "app_home": {
...
In the QA workspace, create a channel (e.g. `#daisyclaw-qa`) and invite both bots from inside the channel:

```
/invite @DaisyClaw QA Driver
/invite @DaisyClaw QA SUT
```
...
Two options. Use env vars for single-machine debugging (set the four `DAISYCLAW_QA_SLACK_*` variables and pass `--credential-source env`), or seed the shared Convex pool so CI and other maintainers can lease them.
...
With `DAISYCLAW_QA_CONVEX_SITE_URL` and `DAISYCLAW_QA_CONVEX_SECRET_MAINTAINER` exported in your shell, register and verify:

```bash
pnpm daisyclaw qa credentials add \
  --kind slack \
  --payload-file slack-creds.json \
  --note "QA Slack pool seed"

pnpm daisyclaw qa credentials list --kind slack --status all --json
```
...
the harness and a SUT account started by the child DaisyClaw gateway through the
bundled WhatsApp plugin.

Required env when `--credential-source env`:

- `DAISYCLAW_QA_WHATSAPP_DRIVER_PHONE_E164`
- `DAISYCLAW_QA_WHATSAPP_SUT_PHONE_E164`
- `DAISYCLAW_QA_WHATSAPP_DRIVER_AUTH_ARCHIVE_BASE64`
- `DAISYCLAW_QA_WHATSAPP_SUT_AUTH_ARCHIVE_BASE64`

Optional:

- `DAISYCLAW_QA_WHATSAPP_GROUP_JID` enables group scenarios such as
  `whatsapp-mention-gating` and `whatsapp-group-allowlist-block`.
- `DAISYCLAW_QA_WHATSAPP_CAPTURE_CONTENT=1` keeps message bodies in
...
`@daisyclaw/whatsapp/api.js` package surface instead of reaching into private
...
- `whatsapp-qa-observed-messages.json` - bodies redacted unless `DAISYCLAW_QA_WHATSAPP_CAPTURE_CONTENT=1`.

### Convex credential pool

Telegram, Discord, Slack, and WhatsApp lanes can lease credentials from a shared Convex pool instead of reading the env vars above. Pass `--credential-source convex` (or set `DAISYCLAW_QA_CREDENTIAL_SOURCE=convex`); QA Lab acquires an exclusive lease, heartbeats it for the duration of the run, and releases it on shutdown. Pool kinds are `"telegram"`, `"discord"`, `"slack"`, and `"whatsapp"`.
...
- `mock-openai` is the scenario-aware DaisyClaw mock. It remains the default
...
- how `daisyclaw qa <runner>` is mounted beneath the shared `qa` root
...
4. Mount the runner as `daisyclaw qa <runner>` instead of registering a competing root command. Runner plugins should declare `qaRunners` in `daisyclaw.plugin.json` and export a matching `qaRunnerCliRegistrations` array from `runtime-api.ts`. Keep `runtime-api.ts` light; lazy CLI and runner execution should stay behind separate entrypoints.
...
For the inventory of available scenarios - useful when sizing follow-up work or wiring a new transport - run `pnpm daisyclaw qa coverage` (add `--json` for machine-readable output).
When choosing focused proof for a touched behavior or file path, run `pnpm daisyclaw qa coverage --match <query>`.
...
```bash
pnpm daisyclaw qa character-eval \
