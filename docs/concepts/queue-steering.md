...
When a normal prompt arrives while a session run is already streaming, DaisyClaw
...
behavior. DaisyClaw and the native Codex app-server harness implement the delivery
details differently.

## Runtime boundary

Steering does not interrupt a tool call that is already running. DaisyClaw checks for
queued steering messages at model boundaries:

1. The assistant asks for tool calls.
2. DaisyClaw executes the current assistant message's tool-call batch.
3. DaisyClaw emits the turn end event.
4. DaisyClaw drains queued steering messages.
5. DaisyClaw appends those messages as user messages before the next LLM call.
...
The native Codex app-server harness exposes `turn/steer` instead of DaisyClaw runtime's
internal steering queue. DaisyClaw batches queued prompts for the configured
...
runtime cannot accept steering in `steer` mode, DaisyClaw waits for the active
...
- With default behavior, the active runtime receives all four messages in
  arrival order before its next model decision. DaisyClaw drains them at the next model
  boundary; Codex receives them as one batched `turn/steer`.
- With `/queue collect`, DaisyClaw does not steer. It waits until the active run
  ends, then creates a followup turn with compatible queued messages after the
  debounce window.
- With `/queue interrupt`, DaisyClaw aborts the active run and starts the newest
...
before sending batched `turn/steer`. For DaisyClaw, active steering itself does not use
the debounce timer because DaisyClaw naturally batches messages until the next model
boundary.
