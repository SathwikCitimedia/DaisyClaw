...
DaisyClaw gives agents tools to work across sessions, inspect status, and
...
After the target responds, DaisyClaw can run a **reply-back loop** where the
...
the caller's current session; visible client labels such as `daisyclaw-tui` are
not session keys.
...
- `deliveryContext` is the persisted delivery route stored on the session,
  which DaisyClaw can reuse for later delivery even when the active surface
  differs.
...
`subagents` is the visibility helper for already spawned DaisyClaw
...
the completion origin only identifies a channel DaisyClaw can still reuse the
