---
summary: "How DaisyClaw manages conversation sessions"
read_when:
...
DaisyClaw organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `daisyclaw security audit`.
...
DaisyClaw automatically bounds session storage over time. By default, it runs
...
`daisyclaw sessions cleanup --dry-run --fix-dm-scope`. Applying the same flag
retires those old direct-DM rows and keeps their transcripts as deleted
archives.

Preview with `daisyclaw sessions cleanup --dry-run`.

## Inspecting sessions

- `daisyclaw status` -- session store path and recent activity.
- `daisyclaw sessions --json` -- all sessions (filter with `--active <minutes>`).
