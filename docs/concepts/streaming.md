...
DaisyClaw has two separate streaming layers:
...
streaming sends media early, DaisyClaw remembers that delivery for the turn. If
...
distinct text around media that was already streamed, DaisyClaw still sends the
...
When block streaming is enabled, DaisyClaw can **merge consecutive block chunks**
...
- Discord: `streamMode` + boolean `streaming` remain runtime aliases for the `streaming` enum; run `daisyclaw doctor --fix` to rewrite persisted config.
- Slack: `streamMode` remains a runtime alias for `streaming.mode`; boolean `streaming` remains a runtime alias for `streaming.mode` plus `streaming.nativeTransport`; legacy `nativeStreaming` remains a runtime alias for `streaming.nativeTransport`. Run `daisyclaw doctor --fix` to rewrite persisted config.
