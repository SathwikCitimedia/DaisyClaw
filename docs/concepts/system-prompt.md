---
summary: "What the DaisyClaw system prompt contains and how it is assembled"
read_when:
...
DaisyClaw builds a custom system prompt for every agent run. The prompt is **DaisyClaw-owned** and does not use a runtime default prompt.

The prompt is assembled by DaisyClaw and injected into each agent run.
...
the full DaisyClaw-owned prompt. The provider runtime can:
...
- **DaisyClaw Control**: tells the model to prefer the `gateway` tool for
  config/restart work and to avoid inventing CLI commands.
- **DaisyClaw Self-Update**: how to inspect config safely with
...
- **Documentation**: local path to DaisyClaw docs/source and when to read them.
...
DaisyClaw keeps large stable content, including **Project Context**, above the
...
DaisyClaw can render smaller system prompts for sub-agents. The runtime sets a
`promptMode` for each run (not a user-facing config):

- `full` (default): includes all sections above.
- `minimal`: used for sub-agents; omits **Memory Recall**, **DaisyClaw
...
For channel auto-reply runs, DaisyClaw omits the generic **Silent Replies**
...
DaisyClaw keeps committed prompt snapshots for the Codex runtime happy path under
...
DaisyClaw developer instructions, turn-scoped collaboration-mode instructions
when DaisyClaw provides them, user turn input, and references to the dynamic tool
specs.
...
collaboration-mode instructions inside the Codex runtime after DaisyClaw sends
...
On the native Codex harness, DaisyClaw avoids repeating stable workspace files
...
Codex developer instructions. The compact DaisyClaw skills list is also forwarded
...
DaisyClaw prompt according to their existing gates. `HEARTBEAT.md` is omitted on
...
occurs, DaisyClaw can inject a concise system-prompt warning notice; control this with
...
When eligible skills exist, DaisyClaw injects a compact **available skills list**
...
points to the local DaisyClaw docs directory (`docs/` in a Git checkout or the bundled npm
package docs). If local docs are unavailable, it falls back to
[https://docs.daisyclaw.ai](https://docs.daisyclaw.ai).

The same section also includes the DaisyClaw source location. Git checkouts expose the local
...
consult docs first for DaisyClaw behavior, commands, configuration, or architecture, and to
run `daisyclaw status` itself when possible (asking the user only when it lacks access).
