...
- `/usage cost` in chats: local cost summary aggregated from DaisyClaw session logs.
- CLI: `daisyclaw status --usage` prints a full per-provider breakdown.
- CLI: `daisyclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. DaisyClaw treats
  ...
  quota, so DaisyClaw inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, DaisyClaw prefers the
...
- **DeepSeek**: API key via env/config/auth store (`DEEPSEEK_API_KEY`).
  DaisyClaw calls DeepSeek's balance endpoint and shows the provider-reported
...
can supply plugin-specific usage auth logic; otherwise DaisyClaw falls back to
