---
summary: "Developer workflow for DaisyClaw agent runtime: build, test, and live validation"
title: "DaisyClaw agent runtime workflow"
read_when:
  - Working on DaisyClaw agent runtime code or tests
  - Running agent-runtime lint, typecheck, and live test flows
---

A sane workflow for working on the DaisyClaw agent runtime in DaisyClaw.
...
```bash
DAISYCLAW_LIVE_TEST=1 pnpm test src/agents/embedded-agent-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm daisyclaw agent --message "Hello" --thinking low`
...
State lives under the DaisyClaw state directory. Default is `~/.daisyclaw`. If `DAISYCLAW_STATE_DIR` is set, use that directory instead.

To reset everything:

- `daisyclaw.json` for config
...
- [DaisyClaw agent runtime architecture](/agent-runtime-architecture)
