...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in DaisyClaw
...
Running DaisyClaw via Node with `tsx` fails at startup with:

```
[daisyclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `daisyclaw status` and `gateway:watch` worked.
