...
```bash
DAISYCLAW_DIAGNOSTICS=telegram.http,telegram.payload
```

Disable all flags:

```bash
DAISYCLAW_DIAGNOSTICS=0
```

`DAISYCLAW_DIAGNOSTICS=0` is a process-level disable override: it disables
...
```bash
DAISYCLAW_DIAGNOSTICS=profiler daisyclaw gateway run
```

Enable only reply-dispatch profiler spans:

```bash
DAISYCLAW_DIAGNOSTICS=reply.profiler daisyclaw gateway run
```

Enable only Codex app-server startup/tool/thread profiler spans:

```bash
DAISYCLAW_DIAGNOSTICS=codex.profiler daisyclaw gateway run
```
...
```bash
DAISYCLAW_DIAGNOSTICS=0 daisyclaw gateway run
```
...
DAISYCLAW_DIAGNOSTICS_TIMELINE_PATH=/tmp/daisyclaw-timeline.jsonl \
...
`DAISYCLAW_DIAGNOSTICS_TIMELINE_PATH`. When `timeline` is enabled only from
config, the earliest config-loading spans are not emitted because DaisyClaw has
not read config yet; subsequent startup spans use the config flag.

`DAISYCLAW_DIAGNOSTICS=1`, `DAISYCLAW_DIAGNOSTICS=all`, and
`DAISYCLAW_DIAGNOSTICS=*` also enable the timeline because they enable every
diagnostics flag. Prefer `timeline` when you only want the JSONL timing
artifact.

Timeline records use the `daisyclaw.diagnostics.v1` envelope. Events can include
...
```bash
ls -t /tmp/daisyclaw/daisyclaw-*.log | head -n 1
```

Filter for Telegram HTTP diagnostics:

```bash
rg "telegram http error" /tmp/daisyclaw/daisyclaw-*.log
```

Filter for Brave Search HTTP diagnostics:

```bash
rg "brave http" /tmp/daisyclaw/daisyclaw-*.log
```

Or tail while reproducing:

```bash
tail -f /tmp/daisyclaw/daisyclaw-$(date +%F).log | rg "telegram http error"
```

For remote gateways, you can also use `daisyclaw logs --follow` (see [/cli/logs](/cli/logs)).
