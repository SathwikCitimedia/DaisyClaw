...
DaisyClaw supports OAuth and API keys for model providers. For always-on gateway
...
setup, but DaisyClaw also supports reusing a local Claude CLI login.

1. Create an API key in your provider console.
2. Put it on the **gateway host** (the machine running `daisyclaw gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.daisyclaw/.env` so the daemon can read it:

```bash
cat >> ~/.daisyclaw/.env <<'EOF'
<PROVIDER>_API_KEY=...
EOF
```
...
API keys for daemon use: `daisyclaw onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.daisyclaw/.env`, systemd/launchd).

## Anthropic: Claude CLI and token compatibility

Anthropic setup-token auth is still available in DaisyClaw as a supported token
path. Anthropic staff has since told us that DaisyClaw-style Claude CLI usage is
allowed again, so DaisyClaw treats Claude CLI reuse and `claude -p` usage as
...
2. Tell DaisyClaw to switch Anthropic model selection to the local `claude-cli`
   backend and store the matching DaisyClaw auth profile.
...
```bash
daisyclaw models auth paste-token --provider openrouter
```
...
- If auth exists but DaisyClaw cannot resolve a probeable model candidate for
...
- Anthropic staff told us this DaisyClaw integration path is allowed again.
- DaisyClaw therefore treats Claude CLI reuse and `claude -p` usage as sanctioned
...
- Priority order:
  - `DAISYCLAW_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- DaisyClaw retries with the next key only for rate-limit errors (for example
...
When provider auth is removed through the Gateway control plane, DaisyClaw deletes
...
Use `daisyclaw models auth login --provider <id> --profile-id <profileId>` for
...
```bash
daisyclaw models auth login --provider anthropic --force
```
...
When you debug order issues, `daisyclaw models status --probe` shows omitted
...
Run `daisyclaw models status` to confirm which profile is expiring. If an
