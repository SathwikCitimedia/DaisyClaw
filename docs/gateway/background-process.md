...
DaisyClaw runs shell commands through the `exec` tool and keeps long-running tasks in memory. The `process` tool manages those background sessions.
...
- Spawned exec commands receive `DAISYCLAW_SHELL=exec` for context-aware shell/profile rules.
...
- `DAISYCLAW_BASH_YIELD_MS`: default yield (ms)
- `DAISYCLAW_BASH_MAX_OUTPUT_CHARS`: in-memory output cap (chars)
- `DAISYCLAW_BASH_PENDING_MAX_OUTPUT_CHARS`: pending stdout/stderr cap per stream (chars)
- `DAISYCLAW_BASH_JOB_TTL_MS`: TTL for finished sessions (ms, bounded to 1m–3h)
- `DAISYCLAW_PROCESS_INPUT_WAIT_IDLE_MS`: idle-output threshold before writable background sessions are marked as likely waiting for input (default 15000 ms)
