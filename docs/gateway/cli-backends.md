...
DaisyClaw can run **local AI CLIs** as a **text-only fallback** when API providers are down,
rate-limited, or temporarily misbehaving. This is intentionally conservative:

- **DaisyClaw tools are not injected directly**, but backends with `bundleMcp: true`
...
```bash
daisyclaw agent --message "hi" --model claude-cli/claude-sonnet-4-6
```
...
gateway host, DaisyClaw now auto-loads the owning bundled plugin when your config
...
- If the primary provider fails (auth, rate limits, timeouts), DaisyClaw will
    ...
    defaults: {
      cliBackends: {
        "my-cli": {
          ...
          // from bounded raw DaisyClaw transcript history before compaction.
...
2. **Builds a system prompt** using the same DaisyClaw prompt + workspace context.
3. **Executes the CLI** with a session id (if supported) so history stays consistent.
   The bundled `claude-cli` backend keeps a Claude stdio process alive per
   DaisyClaw session and sends follow-up turns over stream-json stdin.
4. **Parses output** (JSON or plain text) and returns the final text.
5. **Persists session ids** per backend, so follow-ups reuse the same CLI session.

<Note>
The bundled Anthropic `claude-cli` backend is supported again. Anthropic staff
told us DaisyClaw-style Claude CLI usage is allowed again, so DaisyClaw treats
...
resolver for DaisyClaw skills. When the current skills snapshot includes at least
one selected skill with a materialized path, DaisyClaw passes a temporary Claude
Code plugin with `--plugin-dir` and omits the duplicate DaisyClaw skills catalog
from the appended system prompt. If the snapshot has no materialized plugin
skill, DaisyClaw keeps the prompt catalog as a fallback. Skill env/API key
overrides are still applied by DaisyClaw to the child process environment for the
run.

Claude CLI also has its own noninteractive permission mode. DaisyClaw maps that
to the existing exec policy instead of adding Claude-specific policy config.
For DaisyClaw-managed Claude live sessions, the effective DaisyClaw exec policy is
...
Claude launches normalize that flag to match the effective DaisyClaw exec policy.

The bundled Anthropic `claude-cli` backend also maps DaisyClaw `/think` levels
...
Before DaisyClaw can use the bundled `claude-cli` backend, Claude Code itself
must already be logged in on the same host:

```bash
claude auth login
claude auth status --text
daisyclaw models auth login --provider anthropic --method cli --set-default
```
...
- `claude-cli` defaults to `liveSession: "claude-stdio"`, `output: "jsonl"`,
  ...
  exits, DaisyClaw resumes from the stored Claude session id. Stored session
...
- Claude live sessions keep bounded JSONL output guards. Defaults allow up to
  ...
  and `maxTurnLines`; DaisyClaw clamps those settings to 64 MiB and 100,000
  lines.
- Stored CLI sessions are provider-owned continuity. The implicit daily session
  ...
  do.
- Fresh CLI sessions normally reseed only from DaisyClaw's compaction summary
  plus post-compaction tail. To recover short sessions that are invalidated
  before compaction, a backend can opt in with
  `reseedFromRawTranscriptWhenUncompacted: true`. DaisyClaw still keeps raw
...
- DaisyClaw drops stored CLI session reuse when the selected auth identity changes,
  ...
  stable OAuth account id, DaisyClaw lets that CLI enforce resume permissions.

## Fallback prelude from claude-cli sessions

When a `claude-cli` attempt fails over to a non-CLI candidate in
[`agents.defaults.model.fallbacks`](/concepts/model-failover), DaisyClaw seeds
...
provider would start cold because DaisyClaw's own session transcript is empty
for `claude-cli` runs.
...
DaisyClaw will write base64 images to temp files. If `imageArg` is set, those
paths are passed as CLI args. If `imageArg` is missing, DaisyClaw appends the
...
- For Gemini CLI JSON output, DaisyClaw reads reply text from `response` and
...
agent runs use the Codex app-server harness through `openai/*`; DaisyClaw no
...
- `stats.cached` is normalized into DaisyClaw `cacheRead`.
- If `stats.input` is missing, DaisyClaw derives input tokens from
...
rewrites streamed assistant deltas and parsed final text before DaisyClaw handles
...
Some CLI backends run an agent that compacts its **own** transcript, so DaisyClaw must
...
`ownsNativeCompaction: true`, and DaisyClaw returns a no-op from the compaction path.
...
`contextTokens: 1_000_000` purely to keep DaisyClaw's safeguard from firing on a
...
CLI backends do **not** receive DaisyClaw tool calls directly, but a backend can
...
When bundle MCP is enabled, DaisyClaw:

- spawns a loopback HTTP MCP server that exposes gateway tools to the CLI process
- authenticates the bridge with a per-session token (`DAISYCLAW_MCP_TOKEN`)
...
If no MCP servers are enabled, DaisyClaw still injects a strict config when a
...
When a fresh CLI session is seeded from a prior DaisyClaw transcript (for
...
- **No direct DaisyClaw tool calls.** DaisyClaw does not inject tool calls into
