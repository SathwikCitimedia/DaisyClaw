...
Default: `DAISYCLAW_WORKSPACE_DIR` when set, otherwise `~/.daisyclaw/workspace`.
...
`DAISYCLAW_WORKSPACE_DIR`. Use the environment variable to point default agents
at a mounted workspace when you do not want to write that path into config.

### `agents.defaults.repoRoot`

Optional repository root shown in the system prompt's Runtime line. If unset, DaisyClaw auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/daisyclaw" } },
}
```
...
DaisyClaw has multiple high-volume prompt/context budgets, and they are
...
- `toolResultMaxChars`: advanced live tool-result ceiling used for persisted
  ...
  model context window. `daisyclaw doctor --deep` prints the effective cap, and
...
DaisyClaw adapts the resize ladder to the selected image model. For example, Claude Opus 4.8, OpenAI GPT-5.5, Qwen VL, and hosted Llama 4 vision models can use larger images than older/default high-detail vision paths, while multi-image turns are compressed more aggressively in `auto` mode to control token and latency cost.
...
- `imageModel`: accepts either a string (`"provider/model"`) or an object (`{ primary, fallbacks }`).
  ...
  - Prefer explicit `provider/model` refs. Bare IDs are accepted for compatibility; if a bare ID uniquely matches a configured image-capable entry in `models.providers.*.models`, DaisyClaw qualifies it to that provider. Ambiguous configured matches require an explicit provider prefix.
...
- `model.primary`: format `provider/model` (e.g. `openai/gpt-5.5` for OpenAI API-key or Codex OAuth access). If you omit the provider, DaisyClaw tries an alias first, then a unique configured-provider match for that exact model id, and only then falls back to the configured default provider (deprecated compatibility behavior, so prefer explicit `provider/model`). If that provider no longer exposes the configured default model, DaisyClaw falls back to the first configured provider/model instead of surfacing a stale removed-provider default.
- `models`: the configured model catalog and allowlist for `/model`. Each entry can include `alias` (shortcut) and `params` (provider-specific, for example `temperature`, `maxTokens`, `cacheRetention`, `context1m`, `responsesServerCompaction`, `responsesCompactThreshold`, OpenRouter `provider` routing, `chat_template_kwargs`, `extra_body`/`extraBody`).
  ...
  - Safe edits: use `daisyclaw config set agents.defaults.models '<json>' --strict-json --merge` to add entries. `config set` refuses replacements that would remove existing allowlist entries unless you pass `--replace`.
...
- `compat.thinkingFormat`: OpenAI-compatible thinking payload style. Use `"together"` for Together-style `reasoning.enabled`, `"qwen"` for Qwen-style top-level `enable_thinking`, or `"qwen-chat-template"` for `chat_template_kwargs.enable_thinking` on Qwen-family backends that support request-level chat-template kwargs, such as vLLM. DaisyClaw maps disabled thinking to `false` and enabled thinking to `true`, and configured vLLM Qwen models expose binary `/think` choices for these formats.
- `compat.supportedReasoningEfforts`: per-model OpenAI-compatible reasoning effort list. Include `"xhigh"` for custom endpoints that truly accept it; DaisyClaw then exposes `/think xhigh` in command menus, Gateway session rows, session patch validation, agent CLI validation, and `llm-task` validation for that configured provider/model. Use `compat.reasoningEffortMap` when the backend wants a provider-specific value for a canonical level.
- `params.preserveThinking`: Z.AI-only opt-in for preserved thinking. When enabled and thinking is on, DaisyClaw sends `thinking.clear_thinking: false` and replays prior `reasoning_content`; see [Z.AI thinking and preserved thinking](/providers/zai#thinking-and-preserved-thinking).
- `localService`: optional provider-level process manager for local/self-hosted model servers. When the selected model belongs to that provider, DaisyClaw probes `healthUrl` (or `baseUrl + "/models"`), starts `command` with `args` if the endpoint is down, waits up to `readyTimeoutMs`, then sends the model request. `command` must be an absolute path. `idleStopMs: 0` keeps the process alive until DaisyClaw exits; a positive value stops the DaisyClaw-spawned process after that many idle milliseconds. See [Local model services](/gateway/local-model-services).
...
- `id`: `"auto"`, `"daisyclaw"`, a registered plugin harness id, or a supported CLI backend alias. The bundled Codex plugin registers `codex`; the bundled Anthropic plugin provides the `claude-cli` CLI backend.
- `id: "auto"` lets registered plugin harnesses claim supported turns and uses DaisyClaw when no harness matches. An explicit plugin runtime such as `id: "codex"` requires that harness and fails closed if it is unavailable or fails.
- `id: "pi"` is accepted only as a deprecated alias for `daisyclaw` to preserve shipped configs from v2026.5.22 and earlier. New config should use `daisyclaw`.
...
- Whole-agent runtime keys are legacy. `agents.defaults.agentRuntime`, `agents.list[].agentRuntime`, session runtime pins, and `DAISYCLAW_AGENT_RUNTIME` are ignored by runtime selection. Run `daisyclaw doctor --fix` to remove stale values.
...
Anthropic Claude Opus 4.8 keeps thinking off by default in DaisyClaw; when adaptive thinking is explicitly enabled, Anthropic's provider-owned effort default is `high`. Claude 4.6 models default to `adaptive` when no explicit thinking level is set.
...
- `reseedFromRawTranscriptWhenUncompacted: true` lets a backend recover safe
  invalidated sessions from a bounded raw DaisyClaw transcript tail before the
  first compaction summary exists. Auth profile or credential-epoch changes
  still never raw-reseed.

### `agents.defaults.promptOverlays`

Provider-independent prompt overlays applied by model family on DaisyClaw-assembled prompt surfaces. GPT-5-family model ids receive the shared behavior contract across DaisyClaw/provider routes; `personality` controls only the friendly interaction-style layer. Native Codex app-server routes keep Codex-owned base/model instructions instead of this DaisyClaw GPT-5 overlay, and DaisyClaw disables Codex's built-in personality for native threads.
...
- `timeoutSeconds`: maximum seconds allowed for a single compaction operation before DaisyClaw aborts it. Default: `180`.
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.daisyclaw/sandboxes",
        docker: {
          image: "daisyclaw-sandbox:bookworm-slim",
          containerPrefix: "daisyclaw-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "daisyclaw-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "daisyclaw-sandbox-browser:bookworm-slim",
          network: "daisyclaw-sandbox-browser",
          cdpPort: 9222,
...
- `identityData` / `certificateData` / `knownHostsData`: inline contents or SecretRefs that DaisyClaw materializes into temp files at runtime
...
- `none`: per-scope sandbox workspace under `~/.daisyclaw/sandboxes`
...
In `remote` mode, host-local edits made outside DaisyClaw are not synced into the sandbox automatically after the seed step.
...
Codex app-server turns in an active DaisyClaw sandbox use this same egress setting for their native code-mode network access.

**Inbound attachments** are staged into `media/inbound/*` in the active workspace.

**`docker.binds`** mounts additional host directories; global and per-agent binds are merged.

**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `daisyclaw.json`.
noVNC observer access uses VNC auth by default and DaisyClaw emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `daisyclaw-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `DAISYCLAW_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `DAISYCLAW_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `DAISYCLAW_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.daisyclaw/workspace",
        agentDir: "~/.daisyclaw/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@daisyclaw"] },
        sandbox: { mode: "off" },
        runtime: {
          type: "acp",
          acp: {
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.daisyclaw/workspace-home" },
      { id: "work", workspace: "~/.daisyclaw/workspace-work" },
    ],
  },
  bindings: [
...
For `type: "acp"` entries, DaisyClaw resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.daisyclaw/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.daisyclaw/workspace-family",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.daisyclaw/workspace-public",
...
```json5
{
  session: {
    ...
    store: "~/.daisyclaw/agents/{agentId}/sessions/sessions.json",
    maintenance: {
...
- **`maintenance`**: session-store cleanup + retention controls.
  ...
  - `maxEntries`: maximum number of entries in `sessions.json` (default `500`). Runtime writes batch cleanup with a small high-water buffer for production-sized caps; `daisyclaw sessions cleanup --enforce` applies the cap immediately.
  - `rotateBytes`: deprecated and ignored; `daisyclaw doctor --fix` removes it from older configs.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.daisyclaw/settings/tts.json",
      providers: {
        elevenlabs: {
...
- When `providers.openai.baseUrl` points to a non-OpenAI endpoint, DaisyClaw treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
- Legacy flat Talk keys (`talk.voiceId`, `talk.voiceAliases`, `talk.modelId`, `talk.outputFormat`, `talk.apiKey`) are compatibility-only. Run `daisyclaw doctor --fix` to rewrite persisted config into `talk.providers.<provider>`.
...
- macOS MLX playback runs through the bundled `daisyclaw-mlx-tts` helper when present, or an executable on `PATH`; `DAISYCLAW_MLX_TTS_BIN` overrides the helper path for development.
- `consultThinkingLevel` controls the thinking level for the full DaisyClaw agent run behind Control UI Talk realtime `daisyclaw_agent_consult` calls. Leave unset to preserve normal session/model behavior.
...
- `realtime.instructions` appends provider-facing system instructions to DaisyClaw's built-in realtime prompt, so voice style can be configured without losing default `daisyclaw_agent_consult` guidance.
- `realtime.consultRouting` controls Gateway relay fallback when the realtime provider produces a final user transcript without `daisyclaw_agent_consult`: `provider-direct` preserves direct provider replies, while `force-agent-consult` routes the finalized request through DaisyClaw.
