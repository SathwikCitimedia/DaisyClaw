---
summary: "Schema-accurate configuration examples for common DaisyClaw setups"
read_when:
  - Learning how to configure DaisyClaw
...
Save to `~/.daisyclaw/daisyclaw.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.daisyclaw/agents/default/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
  ...
  cron: {
    enabled: true,
    store: "~/.daisyclaw/cron/cron.json",
  ...
  gateway: {
    ...
    port: 18789,
    bind: "loopback",
    controlUi: { enabled: true, basePath: "/daisyclaw" },
    auth: {
      mode: "token",
      token: "gateway-token",
      allowTailscale: true,
...
```json5
{
  agents: {
    ...
    list: [
      ...
      { id: "docs", workspace: "~/.daisyclaw/workspace-docs", skills: ["docs-search"] },
    ],
  },
}
```
