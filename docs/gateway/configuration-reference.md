---
summary: "Gateway config reference for core DaisyClaw keys, defaults, and links to dedicated subsystem references"
...
Core config reference for `~/.daisyclaw/daisyclaw.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Covers the main DaisyClaw config surfaces and links out when a subsystem has its own deeper reference. Channel- and plugin-owned command catalogs and deep memory/QMD knobs live on their own pages rather than on this one.

Code truth:

- `daisyclaw config schema` prints the live JSON Schema used for validation and Control UI, with bundled/plugin/channel metadata merged in when available
...
Config format is **JSON5** (comments + trailing commas allowed). All fields are optional - DaisyClaw uses safe defaults when omitted.
...
- `talk.*` (Talk mode)
  - `talk.consultThinkingLevel`: thinking level override for the full DaisyClaw agent run behind Control UI Talk realtime consults
  ...
  - `talk.realtime.consultRouting`: Gateway relay fallback for finalized realtime Talk transcripts that skip `daisyclaw_agent_consult`
...
- `models.providers.*.localService`: optional on-demand process manager for
  local model servers. DaisyClaw probes the configured health endpoint, starts
...
DaisyClaw-managed MCP server definitions live under `mcp.servers` and are
consumed by embedded DaisyClaw and other runtime adapters. The `daisyclaw mcp list`,
...
- `mcp.servers`: named stdio or remote MCP server definitions for runtimes that
  ...
  `type: "http"` is a CLI-native alias that `daisyclaw mcp set` and
  `daisyclaw doctor --fix` normalize into the canonical `transport` field.
- `mcp.servers.<name>.enabled`: set `false` to keep a saved server definition
  while excluding it from embedded DaisyClaw MCP discovery and tool projection.
...
- `mcp.servers.<name>.auth`: set `"oauth"` for HTTP MCP servers that require
  OAuth. Run `daisyclaw mcp login <name>` to store tokens under DaisyClaw state.
...
- `mcp.servers.<name>.codex`: optional Codex app-server projection controls.
  This block is DaisyClaw metadata for Codex app-server threads only; it does not
  affect ACP sessions, generic Codex harness config, or other runtime adapters.
  Non-empty `codex.agents` limits the server to the listed DaisyClaw agent ids.
  ...
  `default_tools_approval_mode` for that server. DaisyClaw strips the `codex`
...
See [MCP](/cli/mcp#daisyclaw-as-an-mcp-client-registry) and
...
- `install.nodeManager`: node installer preference for `metadata.daisyclaw.install`
...
- Loaded from package or bundle directories under `~/.daisyclaw/extensions` and `<workspace>/.daisyclaw/extensions`, plus files or directories listed in `plugins.load.paths`.
...
- Discovery accepts native DaisyClaw plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native DaisyClaw plugin schema when available).
- Channel plugin account/runtime settings live under `channels.<id>` and should be described by the owning plugin's manifest `channelConfigs` metadata, not by a central DaisyClaw option registry.
...
It does not enable Codex plugins for DaisyClaw provider runs, ACP
...
- Enabled Claude bundle plugins can also contribute embedded DaisyClaw defaults from `settings.json`; DaisyClaw applies those as sanitized agent settings, not as raw DaisyClaw config patches.
...
`commitments` controls inferred follow-up memory: DaisyClaw can detect check-ins from conversation turns and deliver them through heartbeat runs.
...
```json5
{
  browser: {
    ...
    profiles: {
      daisyclaw: { cdpPort: 18800, color: "#FF4500" },
      work: {
        cdpPort: 18801,
        color: "#0066CC",
...
- `profiles.*.cdpUrl` accepts `http://`, `https://`, `ws://`, and `wss://`.
  Use HTTP(S) when you want DaisyClaw to discover `/json/version`; use WS(S)
...
- If an externally managed CDP service is reachable through loopback, set that
  profile's `attachOnly: true`; otherwise DaisyClaw treats the loopback port as a
...
- `existing-session` profiles can set `cdpUrl` when Chrome is already running
  ...
  mode DaisyClaw passes the endpoint to Chrome MCP instead of using auto-connect;
...
- Local managed `daisyclaw` profiles auto-assign `cdpPort` and `cdpUrl`; set
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or DAISYCLAW_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `DAISYCLAW_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `tailscale.serviceName`: optional Tailscale Service name for Serve mode, such
  as `svc:daisyclaw`. When set, DaisyClaw passes it to `tailscale serve
...
- `tailscale.preserveFunnel`: when `true` and `tailscale.mode = "serve"`, DaisyClaw
...
- `DAISYCLAW_APNS_RELAY_BASE_URL` / `DAISYCLAW_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `DAISYCLAW_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
- `gateway.handshakeTimeoutMs`: pre-auth Gateway WebSocket handshake timeout in milliseconds. Default: `15000`. `DAISYCLAW_HANDSHAKE_TIMEOUT_MS` takes precedence when set. Increase this on loaded or low-powered hosts where local clients can connect while startup warmup is still settling.
...
```bash
DAISYCLAW_CONFIG_PATH=~/.daisyclaw/a.json \
DAISYCLAW_STATE_DIR=~/.daisyclaw-a \
daisyclaw gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.daisyclaw-dev` + port `19001`), `--profile <name>` (uses `~/.daisyclaw-<name>`).
...
```json5
{
  gateway: {
    tls: {
      enabled: false,
      autoGenerate: false,
      certPath: "/etc/daisyclaw/tls/server.crt",
      keyPath: "/etc/daisyclaw/tls/server.key",
      caPath: "/etc/daisyclaw/tls/ca-bundle.crt",
    },
  },
}
```
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.daisyclaw/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-daisyclaw-token: <token>`.
Query-string hook tokens are rejected.

Validation and safety notes:

- `hooks.enabled=true` requires a non-empty `hooks.token`.
- `hooks.token` should be distinct from active Gateway shared-secret auth (`gateway.auth.token` / `DAISYCLAW_GATEWAY_TOKEN` or `gateway.auth.password` / `DAISYCLAW_GATEWAY_PASSWORD`); startup logs a non-fatal security warning when it detects reuse.
- `daisyclaw security audit` flags hook/Gateway auth reuse as a critical finding, including Gateway password auth supplied only at audit time (`--auth password --password <password>`). Run `daisyclaw doctor --fix` to rotate a persisted reused `hooks.token`, then update external hook senders to use the new hook token.
...
- `transform` can point to a JS/TS module returning a hook action.
  ...
  - Keep `hooks.transformsDir` under `~/.daisyclaw/hooks/transforms`; workspace skill directories are rejected. If `daisyclaw doctor` reports this path as invalid, move the transform module into the hooks transforms directory or remove `hooks.transformsDir`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `DAISYCLAW_SKIP_GMAIL_WATCHER=1` to disable.
  ...
  plugins: {
    entries: {
      canvas: {
        config: {
          host: {
            root: "~/.daisyclaw/workspace/canvas",
            liveReload: true,
            // enabled: false, // or DAISYCLAW_SKIP_CANVAS_HOST=1
          },
        },
      },
    },
...
- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__daisyclaw__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__daisyclaw__/a2ui/`
...
- Also serves A2UI at `/__daisyclaw__/a2ui/`.
...
- Hostname defaults to the system hostname when it is a valid DNS label, falling back to `daisyclaw`. Override with `DAISYCLAW_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.daisyclaw/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `daisyclaw dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.daisyclaw/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${DAISYCLAW_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `daisyclaw.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.daisyclaw/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/daisyclaw-vault-resolver",
...
- Legacy flat `auth-profiles.json` maps such as `{ "provider": { "apiKey": "..." } }` are not a runtime format; `daisyclaw doctor --fix` rewrites them to canonical `provider:default` API-key profiles with a `.legacy-flat.*.bak` backup.
...
- Legacy OAuth imports from `~/.daisyclaw/credentials/oauth.json`.
...
- Default log file: `/tmp/daisyclaw/daisyclaw-YYYY-MM-DD.log`.
- Set `logging.file` for a stable path.
- `consoleLevel` bumps to `debug` when `--verbose`.
- `maxFileBytes`: maximum active log file size in bytes before rotation (positive integer; default: `104857600` = 100 MB). DaisyClaw keeps up to five numbered archives beside the active file.
...
```json5
{
  diagnostics: {
    ...
    cacheTrace: {
      enabled: false,
      filePath: "~/.daisyclaw/logs/cache-trace.jsonl",
...
- `stuckSessionAbortMs`: no-progress age threshold in ms before eligible stalled active work may be abort-drained for recovery. When unset, DaisyClaw uses the safer extended embedded-run window of at least 5 minutes and 3x `stuckSessionWarnMs`.
...
- `OTEL_SEMCONV_STABILITY_OPT_IN=gen_ai_latest_experimental`: environment toggle for latest experimental GenAI inference span shape, including `{gen_ai.operation.name} {gen_ai.request.model}` span names, `CLIENT` span kind, and `gen_ai.provider.name` instead of legacy `gen_ai.system`. By default spans keep `daisyclaw.model.call` and `gen_ai.system` for compatibility; GenAI metrics use bounded semantic attributes.
- `DAISYCLAW_OTEL_PRELOADED=1`: environment toggle for hosts that already registered a global OpenTelemetry SDK. DaisyClaw then skips plugin-owned SDK startup/shutdown while keeping diagnostic listeners active.
...
- `cacheTrace.filePath`: output path for cache trace JSONL (default: `$DAISYCLAW_STATE_DIR/logs/cache-trace.jsonl`).
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one DaisyClaw.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `DAISYCLAW_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `daisyclaw doctor --fix` can strip unknown keys).
...
- `webhook`: deprecated legacy fallback webhook URL (http/https) used by `daisyclaw doctor --fix` to migrate stored jobs that still have `notify: true`; runtime delivery uses per-job `delivery.mode="webhook"` plus `delivery.to`, or `delivery.completionDestination` when preserving announce delivery.
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `daisyclaw.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary. Paths must not contain null bytes and must be strictly shorter than 4096 characters before and after resolution.
- DaisyClaw-owned writes that change only one top-level section backed by a single-file include write through to that included file. For example, `plugins install` updates `plugins: { $include: "./plugins.json5" }` in `plugins.json5` and leaves `daisyclaw.json` intact.
- Root includes, include arrays, and includes with sibling overrides are read-only for DaisyClaw-owned writes; those writes fail closed instead of flattening the config.
