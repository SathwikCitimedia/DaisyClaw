...
read_when:
  - Setting up DaisyClaw for the first time
  - Looking for common configuration patterns
  - Navigating to specific config sections
title: "Configuration"
---

DaisyClaw reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.daisyclaw/daisyclaw.json`.
The active config path must be a regular file. Symlinked `daisyclaw.json`
layouts are unsupported for DaisyClaw-owned writes; an atomic write may replace
the path instead of preserving the symlink. If you keep config outside the
default state directory, point `DAISYCLAW_CONFIG_PATH` directly at the real file.

If the file is missing, DaisyClaw uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `daisyclaw onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    daisyclaw onboard       # full onboarding flow
    daisyclaw configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.daisyclaw/daisyclaw.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
DaisyClaw only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

`daisyclaw config schema` prints the canonical JSON Schema used by Control UI
...
- Only diagnostic commands work (`daisyclaw doctor`, `daisyclaw logs`, `daisyclaw health`, `daisyclaw status`)
- Run `daisyclaw doctor` to see exact issues
- Run `daisyclaw doctor --fix` (or `--yes`) to apply repairs

The Gateway keeps a trusted last-known-good copy after each successful startup,
but startup and hot reload do not restore it automatically. If `daisyclaw.json`
...
Run `daisyclaw doctor --fix` (or `--yes`) to repair prefixed/clobbered config or
...
<AccordionGroup>
  ...
  <Accordion title="Choose and configure models">
    ...
    - Use `daisyclaw config set agents.defaults.models '<json>' --strict-json --merge` to add allowlist entries without removing existing models. Plain replacements that would remove entries are rejected unless you pass `--replace`.
        ...
        list: [
          {
            id: "main",
            groupChat: {
              mentionPatterns: ["@daisyclaw", "daisyclaw"],
            },
          },
        ],
      },
  ...
  <Accordion title="Tune gateway WebSocket handshake timeout">
    ...
    - `DAISYCLAW_HANDSHAKE_TIMEOUT_MS` still takes precedence for one-off service or shell overrides.
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push uses the hosted DaisyClaw relay by default: `https://ios-push-relay.daisyclaw.ai`.
    ...
    ```bash
    daisyclaw config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `DAISYCLAW_APNS_RELAY_BASE_URL` and `DAISYCLAW_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - Custom gateway relay URLs must match the relay base URL baked into the official/TestFlight iOS build.
    - `DAISYCLAW_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
  ...
  <Accordion title="Set up webhooks (hooks)">
    ...
    - Use a dedicated `hooks.token`; do not reuse active Gateway auth secrets (`gateway.auth.token` / `DAISYCLAW_GATEWAY_TOKEN` or `gateway.auth.password` / `DAISYCLAW_GATEWAY_PASSWORD`).
    - Hook auth is header-only (`Authorization: Bearer ...` or `x-daisyclaw-token`); query-string tokens are rejected.
  ...
  <Accordion title="Split config into multiple files ($include)">
    ...
    - **DaisyClaw-owned writes**: when a write changes only one top-level section
      backed by a single-file include such as `plugins: { $include: "./plugins.json5" }`,
      DaisyClaw updates that included file and leaves `daisyclaw.json` intact
    - **Unsupported write-through**: root includes, include arrays, and includes
      with sibling overrides fail closed for DaisyClaw-owned writes instead of
      flattening the config
    - **Confinement**: `$include` paths must resolve under the directory holding
      `daisyclaw.json`. To share a tree across machines or users, set
      `DAISYCLAW_INCLUDE_ROOTS` to a path-list (`:` on POSIX, `;` on Windows) of
...
The Gateway watches `~/.daisyclaw/daisyclaw.json` and applies changes automatically - no manual restart needed for most settings.

Direct file edits are treated as untrusted until they validate. The watcher waits
for editor temp-write/rename churn to settle, reads the final file, and rejects
invalid external edits without rewriting `daisyclaw.json`. DaisyClaw-owned config
...
config`, inspect the config, run `daisyclaw config validate`, then run `daisyclaw
...
When you edit a source file that is referenced through `$include`, DaisyClaw plans
...
```bash
daisyclaw gateway call config.get --params '{}'  # capture payload.hash
daisyclaw gateway call config.patch --params '{
...
DaisyClaw reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.daisyclaw/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, DaisyClaw runs your login shell and imports only the missing keys:
...
Env var equivalent: `DAISYCLAW_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${DAISYCLAW_GATEWAY_TOKEN}" } },
