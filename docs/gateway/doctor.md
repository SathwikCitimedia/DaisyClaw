...
`daisyclaw doctor` is the repair + migration tool for DaisyClaw. It fixes stale config/state, checks health, and provides actionable repair steps.
...
<Tabs>
  ...
  <Tab title="--lint">
    ```bash
    daisyclaw doctor --lint
    daisyclaw doctor --lint --json
    ```
  ...
  <Tab title="--non-interactive">
    ```bash
    daisyclaw doctor --non-interactive
    ```
...
```bash
cat ~/.daisyclaw/daisyclaw.json
```

## Read-only lint mode

`daisyclaw doctor --lint` is the automation-friendly sibling of
`daisyclaw doctor --fix`. Both use doctor health checks, but their posture is
different:

| Mode                     | Prompts   | Writes config/state     | Output                 | Use it for                      |
| ------------------------ | --------- | ----------------------- | ---------------------- | ------------------------------- |
| `daisyclaw doctor`        | yes       | no                      | friendly health report | a human checking status         |
| `daisyclaw doctor --fix`  | sometimes | yes, with repair policy | friendly repair log    | applying approved repairs       |
| `daisyclaw doctor --lint` | no        | no                      | structured findings    | CI, preflight, and review gates |
...
```bash
daisyclaw doctor --lint
daisyclaw doctor --lint --severity-min warning
daisyclaw doctor --lint --json
daisyclaw doctor --lint --only core/doctor/gateway-config --json
```
...
<AccordionGroup>
  ...
  <Accordion title="State and integrity">
    ...
    - Extra workspace dir detection (`~/daisyclaw`).
  ...
  <Accordion title="Gateway, services, and supervisors">
    ...
    - Channel-specific permission checks live under `daisyclaw channels capabilities`; for example, Discord voice channel permissions are audited with `daisyclaw channels capabilities --channel discord --target channel:<channel-id>`.
...
The Control UI Dreams scene includes **Backfill**, **Reset**, and **Clear Grounded** actions for the grounded dreaming workflow. These actions use gateway doctor-style RPC methods, but they are **not** part of `daisyclaw doctor` CLI repair/migration.
...
```bash
daisyclaw memory rem-backfill --path ./memory --stage-short-term
```
...
<AccordionGroup>
  ...
  <Accordion title="2. Legacy config key migrations">
    When the config contains deprecated keys, other commands refuse to run and ask you to run `daisyclaw doctor`.

    Doctor will:

    - Explain which legacy keys were found.
    - Show the migration it applied.
    - Rewrite `~/.daisyclaw/daisyclaw.json` with the updated schema.

    Gateway startup refuses legacy config formats and asks you to run `daisyclaw doctor --fix`; it does not rewrite `daisyclaw.json` on startup. Cron job store migrations are also handled by `daisyclaw doctor --fix`.
  ...
  <Accordion title="2b. OpenCode provider overrides">
    If you've added `models.providers.opencode`, `opencode-zen`, or `opencode-go` manually, it overrides the built-in OpenCode catalog from `daisyclaw/plugin-sdk/llm`. That can force models onto the wrong API or zero out costs. Doctor warns so you can remove the override and restore per-model API routing + costs.
  </Accordion>
  ...
  <Accordion title="2f. Codex route repair">
    Doctor checks for legacy `openai-codex/*` model refs. Native Codex harness routing uses canonical `openai/*` model refs; OpenAI agent turns go through the Codex app-server harness instead of the DaisyClaw OpenAI provider path.
  ...
  <Accordion title="3. Legacy state migrations (disk layout)">
    ...
    - Sessions store + transcripts:
      - from `~/.daisyclaw/sessions/` to `~/.daisyclaw/agents/<agentId>/sessions/`
    - Agent dir:
      - from `~/.daisyclaw/agent/` to `~/.daisyclaw/agents/<agentId>/agent/`
    - WhatsApp auth state (Baileys):
      - from legacy `~/.daisyclaw/credentials/*.json` (except `oauth.json`)
      - to `~/.daisyclaw/credentials/whatsapp/<accountId>/...` (default account id: `default`)

    These migrations are best-effort and idempotent; doctor will emit warnings when it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates the legacy sessions + agent dir on startup so history/auth/models land in the per-agent path without a manual doctor run. WhatsApp auth is intentionally only migrated via `daisyclaw doctor`. Talk provider/provider-map normalization now compares by structural equality, so key-order-only diffs no longer trigger repeat no-op `doctor --fix` changes.
  ...
  <Accordion title="3b. Legacy cron store migrations">
    Doctor also checks the cron job store (`~/.daisyclaw/cron/jobs.json` by default, or `cron.store` when overridden) for old job shapes that the scheduler still accepts for compatibility.
    ...
    On Linux, doctor also warns when the user's crontab still invokes legacy `~/.daisyclaw/bin/ensure-whatsapp.sh`. That host-local script is not maintained by current DaisyClaw and can write false `Gateway inactive` messages to `~/.daisyclaw/logs/whatsapp-health.log` when cron cannot reach the systemd user bus. Remove the stale crontab entry with `crontab -e`; use `daisyclaw channels status --probe`, `daisyclaw doctor`, and `daisyclaw gateway status` for current health checks.

  </Accordion>
  <Accordion title="3c. Session lock cleanup">
    Doctor scans every agent session directory for stale write-lock files — files left behind when a session exited abnormally. For each lock file found it reports: the path, PID, whether the PID is still alive, lock age, and whether it is considered stale (dead PID, malformed owner metadata, older than 30 minutes, or a live PID that can be proven to belong to a non-DaisyClaw process). In `--fix` / `--repair` mode it removes locks with dead, orphaned, recycled, malformed-old, or non-DaisyClaw owners automatically. Old locks that are still owned by a live DaisyClaw process are reported but left in place so doctor does not cut off an active transcript writer.
  </Accordion>
  <Accordion title="3d. Session transcript branch repair">
    Doctor scans agent session JSONL files for the duplicated branch shape created by the 2026.4.24 prompt transcript rewrite bug: an abandoned user turn with DaisyClaw internal runtime context plus an active sibling containing the same visible user prompt. In `--fix` / `--repair` mode, doctor backs up each affected file next to the original and rewrites the transcript to the active branch so gateway history and memory readers no longer see duplicate turns.
  </Accordion>
  <Accordion title="4. State integrity checks (session persistence, routing, and safety)">
    ...
    - **Multiple state dirs**: warns when multiple `~/.daisyclaw` folders exist across home directories or when `DAISYCLAW_STATE_DIR` points elsewhere (history can split between installs).
    - **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run it on the remote host (the state lives there).
    - **Config file permissions**: warns if `~/.daisyclaw/daisyclaw.json` is group/world readable and offers to tighten to `600`.
  ...
  <Accordion title="5. Model auth health (OAuth expiry)">
    ...
    When an OAuth refresh fails permanently (for example `refresh_token_reused`, `invalid_grant`, or a provider telling you to sign in again), doctor reports that re-auth is required and prints the exact `daisyclaw models auth login --provider ...` command to run.
    ...
    Legacy Codex OAuth profiles whose tokens live in macOS Keychain (older onboarding before the file-based sidecar layout) are repaired only by doctor. Run `daisyclaw doctor --fix` once from an interactive terminal to migrate Keychain-backed legacy tokens inline into `auth-profiles.json`; after that, embedded turns (Telegram, cron, sub-agent dispatch) resolve them as canonical OpenAI OAuth profiles.
  ...
  <Accordion title="7b. Plugin install cleanup">
    Doctor removes legacy DaisyClaw-generated plugin dependency staging state in `daisyclaw doctor --fix` / `daisyclaw doctor --repair` mode. This covers stale generated dependency roots, old install-stage directories, package-local debris from earlier bundled-plugin dependency repair code, and orphaned or recovered managed npm copies of bundled `@daisyclaw/*` plugins that can shadow the current bundled manifest. Doctor also relinks the host `daisyclaw` package into managed npm plugins that declare `peerDependencies.daisyclaw`, so package-local runtime imports such as `daisyclaw/plugin-sdk/*` keep resolving after updates or npm repairs.

    Doctor can also reinstall missing downloadable plugins when config references them but the local plugin registry cannot find them. Examples include material `plugins.entries`, configured channel/provider/search settings, and configured agent runtimes. During package updates, doctor avoids running package-manager plugin repair while the core package is being swapped; run `daisyclaw doctor --fix` again after the update if a configured plugin still needs recovery. Gateway startup and config reload do not run package managers; plugin installs remain explicit doctor/install/update work.

  </Accordion>
  <Accordion title="8. Gateway service migrations and cleanup hints">
    Doctor detects legacy gateway services (launchd/systemd/schtasks) and offers to remove them and install the DaisyClaw service using the current gateway port. It can also scan for extra gateway-like services and print cleanup hints. Profile-named DaisyClaw gateway services are considered first-class and are not flagged as "extra."

    On Linux, if the user-level gateway service is missing but a system-level DaisyClaw gateway service exists, doctor does not install a second user-level service automatically. Inspect with `daisyclaw gateway status --deep` or `daisyclaw doctor --deep`, then remove the duplicate or set `DAISYCLAW_SERVICE_REPAIR_POLICY=external` when a system supervisor owns the gateway lifecycle.

  </Accordion>
  <Accordion title="8b. Startup Matrix migration">
    When a Matrix channel account has a pending or actionable legacy state migration, doctor (in `--fix` / `--repair` mode) creates a pre-migration snapshot and then runs the best-effort migration steps: legacy Matrix state migration and legacy encrypted-state preparation. Both steps are non-fatal; errors are logged and startup continues. In read-only mode (`daisyclaw doctor` without `--fix`) this check is skipped entirely.
  </Accordion>
  <Accordion title="8c. Device pairing and auth drift">
    ...
    - inspect pending requests with `daisyclaw devices list`
    - approve the exact request with `daisyclaw devices approve <requestId>`
    - rotate a fresh token with `daisyclaw devices rotate --device <deviceId> --role <role>`
    - remove and re-approve a stale record with `daisyclaw devices remove <deviceId>`
  ...
  <Accordion title="11. Workspace status (skills, plugins, and legacy dirs)">
    ...
    - **Legacy workspace dirs**: warns when `~/daisyclaw` or other legacy workspace directories exist alongside the current workspace.
  ...
  <Accordion title="11c. Shell completion">
    ...
    Run `daisyclaw completion --write-state` to regenerate the cache manually.
  ...
  <Accordion title="12. Gateway auth checks (local token)">
    ...
    - `daisyclaw doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
  ...
  <Accordion title="12b. Read-only SecretRef-aware repairs">
    ...
    - `daisyclaw doctor --fix` now uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
  ...
  <Accordion title="13b. Memory search readiness">
    ...
    Use `daisyclaw memory status --deep` to verify embedding readiness at runtime.
  ...
  <Accordion title="15. Supervisor config audit + repair">
    ...
    - `daisyclaw doctor` prompts before rewriting supervisor config.
    - `daisyclaw doctor --yes` accepts the default repair prompts.
    - `daisyclaw doctor --fix` applies recommended fixes without prompts (`--repair` is an alias).
    - `daisyclaw doctor --fix --force` overwrites custom supervisor configs.
    - `DAISYCLAW_SERVICE_REPAIR_POLICY=external` keeps doctor read-only for gateway service lifecycle. It still reports service health and runs non-service repairs, but skips service install/start/restart/bootstrap, supervisor config rewrites, and legacy service cleanup because an external supervisor owns that lifecycle.
    ...
    - Doctor service repairs refuse to rewrite, stop, or restart a gateway service from an older DaisyClaw binary when the config was last written by a newer version. See [Gateway troubleshooting](/gateway/troubleshooting#split-brain-installs-and-newer-config-guard).
    - You can always force a full rewrite via `daisyclaw gateway install --force`.
