...
DaisyClaw has two log "surfaces":
...
- Default rolling log file is under `/tmp/daisyclaw/` (one file per day): `daisyclaw-YYYY-MM-DD.log`
...
- The log file path and level can be configured via `~/.daisyclaw/daisyclaw.json`:
  - `logging.file`
  - `logging.level`
...
DaisyClaw can mask sensitive tokens before log or transcript output leaves the
...
`daisyclaw gateway` supports a per-gateway style switch:
