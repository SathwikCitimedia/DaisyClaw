---
summary: "Run multiple DaisyClaw Gateways on one host (isolation, ports, and profiles)"
read_when:
...
daisyclaw --profile rescue onboard
daisyclaw --profile rescue gateway install --port 19789
```

If your main bot is already running, that is usually all you need.

During `daisyclaw --profile rescue onboard`:
...
`daisyclaw --profile rescue onboard` uses the normal onboarding flow, but it
...
- config file
- state directory
- workspace (by default `~/.daisyclaw/workspace-rescue`)
...
# extra gateway
daisyclaw --profile ops setup
daisyclaw --profile ops gateway --port 19789
```

If you want both Gateways to use named profiles, that also works:

```bash
daisyclaw --profile main setup
daisyclaw --profile main gateway --port 18789
...
daisyclaw --profile ops gateway install --port 19789
```
...
- `DAISYCLAW_CONFIG_PATH` — per-instance config file
- `DAISYCLAW_STATE_DIR` — per-instance sessions, creds, caches
...
Base port = `gateway.port` (or `DAISYCLAW_GATEWAY_PORT` / `--port`).
...
```bash
DAISYCLAW_CONFIG_PATH=~/.daisyclaw/main.json \
DAISYCLAW_STATE_DIR=~/.daisyclaw \
daisyclaw gateway --port 18789

DAISYCLAW_CONFIG_PATH=~/.daisyclaw/rescue.json \
DAISYCLAW_STATE_DIR=~/.daisyclaw-rescue \
daisyclaw gateway --port 19789
```
...
daisyclaw --profile rescue gateway status --deep
daisyclaw --profile rescue gateway probe
daisyclaw status
daisyclaw --profile rescue status
daisyclaw --profile rescue browser status
```

Interpretation:

- `gateway status --deep` helps catch stale launchd/systemd/schtasks services from older installs.
- `gateway probe` warning text such as `multiple reachable gateway identities detected` is expected only when you intentionally run more than one isolated gateway, or when DaisyClaw cannot prove reachable probe targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway is one gateway with multiple transports, even when transport ports differ.
