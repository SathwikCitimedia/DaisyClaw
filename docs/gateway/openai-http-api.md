...
DaisyClaw's Gateway can serve a small OpenAI-compatible Chat Completions endpoint.
...
Under the hood, requests are executed as a normal Gateway agent run (same codepath as `daisyclaw agent`), so routing/permissions/config match your Gateway.
...
- When `gateway.auth.mode="token"`, use `gateway.auth.token` (or `DAISYCLAW_GATEWAY_TOKEN`).
- When `gateway.auth.mode="password"`, use `gateway.auth.password` (or `DAISYCLAW_GATEWAY_PASSWORD`).
...
- Internal same-host callers that bypass the proxy can use
  `gateway.auth.password` / `DAISYCLAW_GATEWAY_PASSWORD` as a local direct
...
- There is no separate non-owner/per-user tool boundary on this endpoint; once a caller passes Gateway auth here, DaisyClaw treats that caller as a trusted operator for this gateway.
- For shared-secret auth modes (`token` and `password`), the endpoint restores the normal full operator defaults even if the caller sends a narrower `x-daisyclaw-scopes` header.
- Trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"`) honor `x-daisyclaw-scopes` when present and otherwise fall back to the normal operator default scope set.
...
- `gateway.auth.mode="token"` or `"password"` + `Authorization: Bearer ...`
  ...
  - ignores narrower `x-daisyclaw-scopes`
...
- trusted identity-bearing HTTP modes (for example trusted proxy auth, or `gateway.auth.mode="none"` on private ingress)
  ...
  - honor `x-daisyclaw-scopes` when the header is present
...
DaisyClaw treats the OpenAI `model` field as an **agent target**, not a raw provider model id.

- `model: "daisyclaw"` routes to the configured default agent.
- `model: "daisyclaw/default"` also routes to the configured default agent.
- `model: "daisyclaw/<agentId>"` routes to a specific agent.

Optional request headers:

- `x-daisyclaw-model: <provider/model-or-bare-id>` overrides the backend model for the selected agent.
- `x-daisyclaw-agent-id: <agentId>` remains supported as a compatibility override.
- `x-daisyclaw-session-key: <sessionKey>` fully controls session routing.
- `x-daisyclaw-message-channel: <channel>` sets the synthetic ingress channel context for channel-aware prompts and policies.

Compatibility aliases still accepted:

- `model: "daisyclaw:<agentId>"`
- `model: "agent:<agentId>"`
...
For custom apps, the safest default is to reuse the same `user` value per conversation thread. Avoid account-level identifiers unless you explicitly want multiple conversations or devices to share one DaisyClaw session. Use `x-daisyclaw-session-key` when you need explicit routing control across multiple clients or threads.
...
<AccordionGroup>
  <Accordion title="What does `/v1/models` return?">
    An DaisyClaw agent-target list.

    The returned ids are `daisyclaw`, `daisyclaw/default`, and `daisyclaw/<agentId>` entries.
  ...
  <Accordion title="Why is `daisyclaw/default` included?">
    `daisyclaw/default` is the stable alias for the configured default agent.
  ...
  <Accordion title="How do I override the backend model?">
    Use `x-daisyclaw-model`.

    Examples:
    `x-daisyclaw-model: openai/gpt-5.4`
    `x-daisyclaw-model: gpt-5.5`
  ...
  <Accordion title="How do embeddings fit this contract?">
    ...
    Use `model: "daisyclaw/default"` or `model: "daisyclaw/<agentId>"`.
    When you need a specific embedding model, send it in `x-daisyclaw-model`.
...
- Model: `daisyclaw/default`

Expected behavior:

- `GET /v1/models` should list `daisyclaw/default`
- Open WebUI should use `daisyclaw/default` as the chat model id
- If you want a specific backend provider/model for that agent, set the agent's normal default model or send `x-daisyclaw-model`
...
If that returns `daisyclaw/default`, most Open WebUI setups can connect with the same base URL and token.
...
curl -N http://127.0.0.1:18789/v1/chat/completions \
  -H 'Authorization: Bearer YOUR_TOKEN' \
  -H 'Content-Type: application/json' \
  -H 'x-daisyclaw-model: openai/gpt-5.4' \
  -d '{
    "model": "daisyclaw/research",
    "stream": true,
    "messages": [{"role":"user","content":"hi"}]
  }'
```
...
```bash
curl -sS http://127.0.0.1:18789/v1/models/daisyclaw%2Fdefault \
  -H 'Authorization: Bearer YOUR_TOKEN'
```
...
curl -sS http://127.0.0.1:18789/v1/embeddings \
  -H 'Authorization: Bearer YOUR_TOKEN' \
  -H 'Content-Type: application/json' \
  -H 'x-daisyclaw-model: openai/text-embedding-3-small' \
  -d '{
    "model": "daisyclaw/default",
    "input": ["alpha", "beta"]
  }'
...
- `/v1/models` returns DaisyClaw agent targets, not raw provider catalogs.
- `daisyclaw/default` is always present so one stable id works across environments.
- Backend provider/model overrides belong in `x-daisyclaw-model`, not the OpenAI `model` field.
