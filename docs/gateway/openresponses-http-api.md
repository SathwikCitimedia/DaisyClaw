...
DaisyClaw's Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`daisyclaw agent`), so routing/permissions/config match your Gateway.
...
- use the matching Gateway HTTP auth path:
  ...
  - trusted-proxy local direct fallback: same-host callers with no `Forwarded`, `X-Forwarded-*`, or `X-Real-IP` headers can use `gateway.auth.password` / `DAISYCLAW_GATEWAY_PASSWORD`
  - private-ingress open auth (`gateway.auth.mode="none"`): no auth header
- treat the endpoint as full operator access for the gateway instance
- for shared-secret auth modes (`token` and `password`), ignore narrower bearer-declared `x-daisyclaw-scopes` values and restore the normal full operator defaults
- for trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"`), honor `x-daisyclaw-scopes` when present and otherwise fall back to the normal operator default scope set
- select agents with `model: "daisyclaw"`, `model: "daisyclaw/default"`, `model: "daisyclaw/<agentId>"`, or `x-daisyclaw-agent-id`
- use `x-daisyclaw-model` when you want to override the selected agent's backend model
- use `x-daisyclaw-session-key` for explicit session routing
- use `x-daisyclaw-message-channel` when you want a non-default synthetic ingress channel context
...
- `previous_response_id`: DaisyClaw reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
...
For `tool_choice: "required"` and function-pinned `tool_choice`, the endpoint narrows the exposed client function-tool set, instructs the runtime to call a client tool before responding, and rejects the turn if it does not include a matching structured client-tool call. This contract applies to the caller-supplied HTTP `tools` list, not every internal DaisyClaw agent tool. Non-streaming requests return `502` with an `api_error`; streaming requests emit a `response.failed` event. This matches the `/v1/chat/completions` contract.
...
DaisyClaw normalizes common OpenAI-style aliases before those counters reach
