---
summary: "Use OpenShell as a managed sandbox backend for DaisyClaw agents"
title: OpenShell
read_when:
...
OpenShell is a managed sandbox backend for DaisyClaw. Instead of running Docker
containers locally, DaisyClaw delegates sandbox lifecycle to the `openshell` CLI,
...
- OpenShell plugin installed (`daisyclaw plugins install @daisyclaw/openshell-sandbox`)
- The `openshell` CLI installed and on `PATH` (or set a custom path via
  `plugins.entries.openshell.config.command`)
- An OpenShell account with sandbox access
- DaisyClaw Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, DaisyClaw creates an OpenShell
...
- You edit files locally outside DaisyClaw and want those changes visible in the
...
- When the sandbox is first created, DaisyClaw seeds the remote workspace from
...
- DaisyClaw does **not** sync remote changes back into the local workspace.
...
<Warning>
If you edit files on the host outside DaisyClaw after the initial seed, the remote sandbox does **not** see those changes. Use `daisyclaw sandbox recreate` to re-seed.
</Warning>
...
| `from`                    | `string`                 | `"daisyclaw"`  | Sandbox source for first-time create                  |
...
1. DaisyClaw calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. DaisyClaw calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `daisyclaw sandbox` commands
