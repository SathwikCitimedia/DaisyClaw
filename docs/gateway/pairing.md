...
daisyclaw nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
changes (for example, display name or client platform hints), DaisyClaw treats
...
Pairing state is stored under the Gateway state directory (default `~/.daisyclaw`):

- `~/.daisyclaw/nodes/paired.json`
- `~/.daisyclaw/nodes/pending.json`

If you override `DAISYCLAW_STATE_DIR`, the `nodes/` folder moves with it.
