...
DaisyClaw has three related (but different) controls:
...
Use the inspector to see what DaisyClaw is _actually_ doing:
...
- DaisyClaw validates bind sources twice: first on the normalized source path, then again after resolving through the deepest existing ancestor. Symlink-parent escapes do not bypass blocked-path or allowed-root checks.
...
- Gateway logs include `agents/tool-policy` audit entries when a tool policy step removes tools or a sandbox tool policy blocks a call. Use `daisyclaw logs` to see the rule label, config key, and affected tool names.
...
- `group:daisyclaw`: all built-in DaisyClaw tools (excludes provider plugins)
...
- Check `daisyclaw logs` for the `agents/tool-policy` entry. It records the sandbox mode and whether the allow or deny rule blocked the tool.
