...
DaisyClaw supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
agent can inspect, including `daisyclaw.json`, `auth-profiles.json`, `.env`, or
...
`daisyclaw secrets audit --check` reports no plaintext secret residue.
</Warning>
...
- legacy plaintext residue has been scrubbed from `daisyclaw.json`,
  `auth-profiles.json`, `.env`, and generated `models.json` files
- `daisyclaw secrets audit --check` is clean after the migration
...
<AccordionGroup>
  <Accordion title="Examples of inactive surfaces">
    ...
    - `gateway.auth.token` SecretRef is inactive for startup auth resolution when `DAISYCLAW_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, DaisyClaw runs preflight validation before saving:
...
<AccordionGroup>
  ...
  <Accordion title="Exec provider">
    ...
    - Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). DaisyClaw validates the resolved target path.
    ...
    - Plugin-managed exec providers can use `pluginIntegration` instead of
      copied `command`/`args`. DaisyClaw resolves the current command details
...
<AccordionGroup>
      ...
      secrets: {
        providers: {
          onepassword_openai: {
            ...
            args: ["read", "op://Personal/DaisyClaw QA API Key/password"],
            passEnv: ["HOME"],
            jsonOnly: false,
          },
        },
  ...
  <Accordion title="Bitwarden Secrets Manager (`bws`)">
    ...
    `scripts/secrets/daisyclaw-bws-resolver.mjs`; install or copy it to an absolute
    ...
    ```json5
    {
      secrets: {
        providers: {
          bws: {
            source: "exec",
            command: "/usr/local/bin/daisyclaw-bws-resolver.mjs",
        ...
        providers: {
          openai: {
            ...
            apiKey: {
              source: "exec",
              provider: "bws",
              id: "daisyclaw/providers/openai/apiKey",
            },
          },
        },
      },
    ...
    SecretRef id contract, such as `daisyclaw/providers/openai/apiKey`; env-var
    ...
    ```bash
    daisyclaw secrets audit --allow-exec
    ```
      ...
      secrets: {
        providers: {
          vault_openai: {
            ...
            allowSymlinkCommand: true, // required for Homebrew symlinked binaries
            trustedDirs: ["/opt/homebrew"],
            args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/daisyclaw"],
  ...
  <Accordion title="password-store (`pass`)">
    ...
    `/usr/local/bin/daisyclaw-pass-resolver`. The `#!/usr/bin/env node` shebang
    ...
    ```json5
    {
      secrets: {
        providers: {
          pass_store: {
            source: "exec",
            command: "/usr/local/bin/daisyclaw-pass-resolver",
...
- DaisyClaw resolves these refs during sandbox activation, not lazily during each SSH call.
...
- The redaction sentinel `__DAISYCLAW_REDACTED__` is reserved for internal config redaction/restore and is rejected as literal submitted config data.

Warning and audit signals:

- `SECRETS_REF_OVERRIDES_PLAINTEXT` (runtime warning)
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `daisyclaw.json` refs)
...
When reload-time activation fails after a healthy state, DaisyClaw enters degraded secrets state.
...
<Tabs>
  <Tab title="Strict command paths">
    For example `daisyclaw memory` remote-memory paths and `daisyclaw qr --remote` when it needs remote shared-secret refs. They read from the active snapshot and fail fast when a required SecretRef is unavailable.
  </Tab>
  <Tab title="Read-only command paths">
    For example `daisyclaw status`, `daisyclaw status --all`, `daisyclaw channels status`, `daisyclaw channels resolve`, `daisyclaw security audit`, and read-only doctor/config repair flows. They also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `daisyclaw secrets reload`.
...
<Steps>
  ...
  <Step title="Configure and apply SecretRefs">
    ```bash
    daisyclaw secrets configure --apply
    ```
  </Step>
  <Step title="Re-audit">
    ```bash
...
with `daisyclaw secrets apply --from <plan-path>` before the re-audit.

<AccordionGroup>
  <Accordion title="secrets audit">
    Findings include:

    - plaintext values at rest (`daisyclaw.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
    - plaintext sensitive provider header residues in generated `models.json` entries
    - unresolved refs
    - precedence shadowing (`auth-profiles.json` taking priority over `daisyclaw.json` refs)
    - legacy residues (`auth.json`, OAuth reminders)

    Exec note:

    - By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
    - Use `daisyclaw secrets audit --allow-exec` to execute exec providers during audit.
  ...
  <Accordion title="secrets configure">
    ...
    - lets you select supported secret-bearing fields in `daisyclaw.json` plus `auth-profiles.json` for one agent scope
    ...
    - `daisyclaw secrets configure --providers-only`
    - `daisyclaw secrets configure --skip-provider-setup`
    - `daisyclaw secrets configure --agent <id>`
  ...
  <Accordion title="secrets apply">
    ...
    ```bash
    daisyclaw secrets apply --from /tmp/daisyclaw-secrets-plan.json
    daisyclaw secrets apply --from /tmp/daisyclaw-secrets-plan.json --allow-exec
    daisyclaw secrets apply --from /tmp/daisyclaw-secrets-plan.json --dry-run
    daisyclaw secrets apply --from /tmp/daisyclaw-secrets-plan.json --dry-run --allow-exec
    ```
...
<Warning>
DaisyClaw intentionally does not write rollback backups containing historical plaintext secret values.
</Warning>
