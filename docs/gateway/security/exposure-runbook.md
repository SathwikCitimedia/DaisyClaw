---
summary: "Pre-flight and rollback checklist before exposing an DaisyClaw Gateway beyond loopback"
...
- Backup location for `~/.daisyclaw/daisyclaw.json` and credentials.
...
```bash
daisyclaw gateway probe --url ws://127.0.0.1:18789 --token "$DAISYCLAW_GATEWAY_TOKEN"
```
...
Run `daisyclaw security audit --deep` after proxy changes. Trusted-proxy findings
...
1. Re-run `daisyclaw security audit --deep`.
...
5. Re-run `daisyclaw security audit --deep`.
