...
DaisyClaw can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. DaisyClaw requires a shared password.
- `off`: Default (no Tailscale automation).

Status and audit output use **Tailscale exposure** for this DaisyClaw Serve/Funnel
mode. `off` means DaisyClaw is not managing Serve or Funnel; it does not mean the
...
- `token` (default when `DAISYCLAW_GATEWAY_TOKEN` is set)
- `password` (shared secret via `DAISYCLAW_GATEWAY_PASSWORD` or config)
...
(`tailscale-user-login`) without supplying a token/password. DaisyClaw verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
DaisyClaw only treats a request as Serve when it arrives from loopback with
...
```json5
{
  gateway: {
    bind: "loopback",
    tailscale: { mode: "serve", serviceName: "svc:daisyclaw" },
  },
}
```

With the example above, startup reports the Service URL as
`https://daisyclaw.<tailnet-name>.ts.net/` instead of the device hostname.
...
Prefer `DAISYCLAW_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
daisyclaw gateway --tailscale serve
daisyclaw gateway --tailscale funnel --auth password
```
...
- `gateway.tailscale.serviceName` applies only to Serve mode and is passed to
  ...
  `svc:<dns-label>` Service name format, for example `svc:daisyclaw`.
...
- Set `gateway.tailscale.resetOnExit` if you want DaisyClaw to undo `tailscale serve`
...
- Set `gateway.tailscale.preserveFunnel: true` to keep an externally configured
  ...
  gateway runs in `mode: "serve"`, DaisyClaw checks `tailscale funnel status`
  before re-applying Serve and skips it when a Funnel route already covers the
  gateway port. The DaisyClaw-managed Funnel password-only policy is unchanged.
