...
- `daisyclaw gateway status` shows `Runtime: running`, `Connectivity probe: ok`, and a `Capability: ...` line.
- `daisyclaw doctor` reports no blocking config/service issues.
- `daisyclaw channels status --probe` shows live per-account transport status and, where supported, probe/audit results such as `works` or `audit ok`.
...
daisyclaw update status --json
daisyclaw gateway status --deep
daisyclaw doctor --fix
daisyclaw gateway restart
```

Look for:

- `Update restart` in `daisyclaw status` / `daisyclaw status --all`. Pending or
  failed handoffs include the next command to run.
- `plugin load failed: dependency tree corrupted; run daisyclaw doctor --fix`
  under Channels. That means the channel config still exists, but plugin
  registration failed before the channel could load.
- provider 401s after re-auth. `daisyclaw doctor --fix` checks for stale
...
Use this when a gateway service unexpectedly stops after an update, or logs show that one `daisyclaw` binary is older than the version that last wrote `daisyclaw.json`.

DaisyClaw stamps config writes with `meta.lastTouchedVersion`. Read-only commands can still inspect a config written by a newer DaisyClaw, but process and service mutations refuse to continue from an older binary. Blocked actions include gateway service start, stop, restart, uninstall, forced service reinstall, service-mode gateway startup, and `gateway --force` port cleanup.

```bash
which daisyclaw
daisyclaw --version
daisyclaw gateway status --deep
daisyclaw config get meta.lastTouchedVersion
```

<Steps>
  <Step title="Fix PATH">
    Fix `PATH` so `daisyclaw` resolves to the newer install, then rerun the action.
  </Step>
  ...
  <Step title="Remove stale wrappers">
    Remove stale system package or old wrapper entries that still point at an old `daisyclaw` binary.
  </Step>
</Steps>

<Warning>
For intentional downgrade or emergency recovery only, set `DAISYCLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1` for the single command. Leave it unset for normal operation.
</Warning>

## Protocol mismatch after rollback

Use this when logs keep printing `protocol mismatch` after you downgrade or roll back DaisyClaw. This means an older Gateway is running, but a newer local client process is still trying to reconnect with a protocol range that the older Gateway cannot speak.

```bash
daisyclaw --version
which -a daisyclaw
daisyclaw gateway status --deep
daisyclaw doctor --deep
daisyclaw logs --follow
```
...
- `Established clients:` in `daisyclaw gateway status --deep` or `Gateway clients` in `daisyclaw doctor --deep`. This lists active TCP clients connected to the Gateway port, including PIDs and command lines when the OS allows it.
- A client process whose command line points at the newer DaisyClaw install or wrapper you rolled back from.

Fix:

1. Stop or restart the stale DaisyClaw client process shown by `gateway status --deep`.
2. Restart apps or wrappers that embed DaisyClaw, such as local dashboards, editors, app-server helpers, or long-running `daisyclaw logs --follow` shells.
3. Re-run `daisyclaw gateway status --deep` or `daisyclaw doctor --deep` and confirm the stale client PID is gone.
...
DaisyClaw treats every skill root as a containment boundary. A symlink under
`~/.agents/skills`, `<workspace>/.agents/skills`, `<workspace>/skills`, or
`~/.daisyclaw/skills` is skipped when its real target resolves outside that root
...
daisyclaw config get skills.load
```
...
daisyclaw config get agents.defaults.models
```
...
Do not assume this is always an DaisyClaw configuration issue. The response can
...
block. Prefer a narrowly scoped allow or skip rule for the API path DaisyClaw
...
- DaisyClaw model runs fail only on normal agent turns
...
daisyclaw infer model run --model <provider/model> --prompt "hi" --json
daisyclaw logs --follow
```

Look for:

- direct tiny calls succeed, but DaisyClaw runs fail only on larger prompts
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `incomplete turn detected ... stopReason=stop payloads=0` → the backend completed the Chat Completions request but returned no user-visible assistant text for that turn. DaisyClaw retries replay-safe empty OpenAI-compatible turns once; persistent failures usually mean the backend is emitting empty/non-text content or suppressing final-answer text.
    - direct tiny requests succeed, but DaisyClaw agent runs fail with backend/model crashes (for example Gemma on some `inferrs` builds) → DaisyClaw transport is likely already correct; the backend is failing on the larger agent-runtime prompt shape.
  ...
  <Accordion title="Fix options">
    ...
    3. Set `compat.supportsTools: false` for models/backends that cannot handle DaisyClaw's tool schema surface reliably.
    ...
    5. If tiny direct requests keep passing while DaisyClaw agent turns still crash inside the backend, treat it as an upstream server/model limitation and file a repro there with the accepted payload shape.
  </Accordion>
</AccordionGroup>
...
daisyclaw status
daisyclaw channels status --probe
daisyclaw pairing list --channel <channel> [--account <id>]
daisyclaw config get channels
daisyclaw logs --follow
```
...
If `curl` returns DaisyClaw HTML, the Gateway is working and the remaining issue
...
does not leave the service running, run `daisyclaw gateway start` and recheck
`daisyclaw gateway status`.
...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.                                                                                                                                                 | Paste/set token in the client and retry. For dashboard paths: `daisyclaw config get gateway.auth.token` then paste into Control UI settings.                                                                                                                                              |
...
| `PAIRING_REQUIRED`           | Device identity needs approval. Check `error.details.reason` for `not-paired`, `scope-upgrade`, `role-upgrade`, or `metadata-upgrade`, and use `requestId` / `remediationHint` when present. | Approve pending request: `daisyclaw devices list` then `daisyclaw devices approve <requestId>`. Scope/role upgrades use the same flow after you review the requested access.                                                                                                               |
...
If `daisyclaw devices rotate` / `revoke` / `remove` is denied unexpectedly:

- paired-device token sessions can manage only **their own** device unless the caller also has `operator.admin`
- `daisyclaw devices rotate --scope ...` can only request operator scopes that the caller session already holds
...
daisyclaw logs --follow
daisyclaw doctor
daisyclaw gateway status --deep   # also scan system-level services
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    - `Gateway start blocked: set gateway.mode=local` or `existing config is missing gateway.mode` → local gateway mode is not enabled, or the config file was clobbered and lost `gateway.mode`. Fix: set `gateway.mode="local"` in your config, or re-run `daisyclaw onboard --mode local` / `daisyclaw setup` to restamp the expected local-mode config. If you are running DaisyClaw via Podman, the default config path is `~/.daisyclaw/daisyclaw.json`.
    ...
    - `System-level DaisyClaw gateway service detected` from doctor → a systemd system unit exists while the user-level service is missing. Remove or disable the duplicate before allowing doctor to install a user service, or set `DAISYCLAW_SERVICE_REPAIR_POLICY=external` if the system unit is the intended supervisor.
    - `Gateway service port does not match current gateway config` → the installed supervisor still pins the old `--port`. Run `daisyclaw doctor --fix` or `daisyclaw gateway install --force`, then restart the gateway service.
...
Use this when channels (Telegram, WhatsApp, etc.) on a macOS host go quiet for minutes to hours at a time, and the gateway appears to come back the moment you open the Control UI, SSH in, or otherwise interact with the host. There is usually no obvious symptom in `daisyclaw status` because by the time you look the gateway is alive again.

```bash
ls ~/.daisyclaw/logs/stability/ | tail -5
daisyclaw gateway stability --bundle latest
pmset -g log | grep -iE "sleep|wake|maintenance" | tail -50
launchctl print gui/$UID/ai.daisyclaw.gateway | grep -E "state|last exit|runs"
```

Look for:

- One or more `*-uncaught_exception.json` bundles in `~/.daisyclaw/logs/stability/` with `error.code` set to a transient network code such as `ENETDOWN`, `ENETUNREACH`, `EHOSTUNREACH`, or `ECONNREFUSED`.
...
- `runs` count incrementing across the day with no corresponding `received SIG*; shutting down` line in `~/Library/Logs/daisyclaw/gateway.log`: clean shutdowns log a signal; transient crashes do not.
...
3. **Add a liveness watchdog** so a future crash burst that gets parked by launchd is caught quickly:
   ...
   state=$(launchctl print gui/$UID/ai.daisyclaw.gateway 2>/dev/null | awk -F'= ' '/state =/ {print $2; exit}')
   if [ "$state" != "running" ]; then
     launchctl kickstart -k gui/$UID/ai.daisyclaw.gateway
   fi
   ```
...
daisyclaw logs --follow
daisyclaw gateway stability --bundle latest
daisyclaw gateway diagnostics export
```
...
- `critical memory pressure bundle written` appears shortly before restart → DaisyClaw captured a pre-OOM stability bundle. Inspect it with `daisyclaw gateway stability --bundle latest`.
- `memory pressure: level=critical ... memoryPressureSnapshot=disabled` appears in gateway logs → DaisyClaw detected critical memory pressure, but the pre-OOM stability snapshot is off.
...
- A timestamped `daisyclaw.json.rejected.*` file beside the active config
- A timestamped `daisyclaw.json.clobbered.*` file if `doctor --fix` repaired a broken direct edit
- DaisyClaw keeps the latest 32 `.clobbered.*` files for each config path and rotates older ones

<AccordionGroup>
  <Accordion title="What happened">
    - The config did not validate during startup, hot reload, or an DaisyClaw-owned write.
    - Gateway startup fails closed instead of rewriting `daisyclaw.json`.
    - Hot reload skips invalid external edits and keeps the current runtime config active.
    - DaisyClaw-owned writes reject invalid/destructive payloads before commit and save `.rejected.*`.
    - `daisyclaw doctor --fix` owns repair. It can remove non-JSON prefixes or restore the last-known-good copy while preserving the rejected payload as `.clobbered.*`.
    - When many repairs happen for one config path, DaisyClaw rotates older `.clobbered.*` files so the newest repaired payload is still available.

  </Accordion>
  <Accordion title="Inspect and repair">
    ```bash
    CONFIG="$(daisyclaw config file)"
  ...
  <Accordion title="Common signatures">
    ...
    - `.rejected.*` exists → an DaisyClaw-owned config write failed schema or clobber checks before commit.
    ...
    - `missing-meta-vs-last-good`, `gateway-mode-missing-vs-last-good`, or `size-drop-vs-last-good:*` → an DaisyClaw-owned write was rejected because it lost fields or size compared with the last-known-good backup.
    - `Config last-known-good promotion skipped` → the candidate contained redacted secret placeholders such as `***`.

  </Accordion>
  <Accordion title="Fix options">
    1. Run `daisyclaw doctor --fix` to let doctor repair prefixed/clobbered config or restore last-known-good.
    2. Copy only the intended keys from `.clobbered.*` or `.rejected.*`, then apply them with `daisyclaw config set` or `config.patch`.
    3. Run `daisyclaw config validate` before restarting.
...
Use this when `daisyclaw gateway probe` reaches something, but still prints a warning block.

```bash
daisyclaw gateway probe
daisyclaw gateway probe --json
daisyclaw gateway probe --ssh user@gateway-host
```
...
- `multiple reachable gateway identities detected` → distinct gateways answered, or DaisyClaw could not prove reachable targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway is treated as one gateway with multiple transports, even when transport ports differ.
...
```bash
daisyclaw cron status
daisyclaw cron list
daisyclaw cron runs --id <jobId> --limit 20
daisyclaw system heartbeat last
daisyclaw logs --follow
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `heartbeat skipped` with `reason=empty-heartbeat-file` → `HEARTBEAT.md` exists but only contains blank, comment, header, fence, or empty-checklist scaffolding, so DaisyClaw skips the model call.
...
daisyclaw nodes describe --node <idOrNameOrIp>
daisyclaw approvals get --node <idOrNameOrIp>
daisyclaw logs --follow
daisyclaw status
```
...
daisyclaw browser start --browser-profile daisyclaw
...
<AccordionGroup>
  ...
  <Accordion title="Element / screenshot / upload signatures">
    ...
    - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `daisyclaw browser stop --browser-profile <name>` to close the active control session and release Playwright/CDP emulation state without restarting the whole gateway.
...
- [Browser (DaisyClaw-managed)](/tools/browser)
...
<AccordionGroup>
  <Accordion title="1. Auth and URL override behavior changed">
    ```bash
    daisyclaw gateway status
    daisyclaw config get gateway.mode
    daisyclaw config get gateway.remote.url
    daisyclaw config get gateway.auth.mode
    ```
  ...
  <Accordion title="2. Bind and auth guardrails are stricter">
    ```bash
    daisyclaw config get gateway.bind
    daisyclaw config get gateway.auth.mode
    daisyclaw config get gateway.auth.token
    daisyclaw gateway status
    daisyclaw logs --follow
    ```
