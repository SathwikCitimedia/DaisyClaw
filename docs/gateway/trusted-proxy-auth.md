...
read_when:
  - Running DaisyClaw behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of DaisyClaw
...
- You run DaisyClaw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth).
...
<Steps>
  ...
  <Step title="Gateway verifies trusted source">
    DaisyClaw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`).
  </Step>
  <Step title="Gateway extracts identity">
    DaisyClaw extracts the user identity from the configured header.
  </Step>
...
- Device-less Control UI WebSocket sessions connect but receive no operator scopes by default. DaisyClaw clears the requested scope list to `[]` so a session that is not bound to an approved paired device/token cannot self-declare permissions.
...
- If your proxy sends `x-daisyclaw-scopes` on the Control UI WebSocket upgrade request, DaisyClaw caps the session scopes to the intersection of the requested scopes and the declared scopes. This header does not grant scopes; it only narrows what the session can hold.
...
- Internal Gateway clients that do not travel through the reverse proxy should use `gateway.auth.password` / `DAISYCLAW_GATEWAY_PASSWORD`, not trusted-proxy identity headers.
...
<Tabs>
  <Tab title="Proxy TLS termination (recommended)">
    ...
    - DaisyClaw can stay on loopback HTTP behind the proxy.
  ...
  <Tab title="Gateway TLS termination">
    If DaisyClaw itself serves HTTPS directly (no TLS-terminating proxy), set:
...
<AccordionGroup>
  <Accordion title="Pomerium">
    ...
    ```yaml
    routes:
      - from: https://daisyclaw.example.com
        to: http://daisyclaw-gateway:18789
        policy:
          - allow:
              or:
                - email:
  ...
  <Accordion title="Caddy with OAuth">
    ...
    ```
    daisyclaw.example.com {
        authenticate with oauth2_provider
        authorize with policy1

        reverse_proxy daisyclaw:18789 {
  ...
  <Accordion title="nginx + oauth2-proxy">
    ...
    location / {
        ...
        proxy_pass http://daisyclaw:18789;
...
DaisyClaw rejects ambiguous configurations where both a `gateway.auth.token` (or `DAISYCLAW_GATEWAY_TOKEN`) and `trusted-proxy` mode are active at the same time. Mixed token configs can cause loopback requests to silently authenticate on the wrong auth path.
...
Loopback trusted-proxy identity headers still fail closed: same-host callers are not silently authenticated as proxy users. Internal DaisyClaw callers that bypass the proxy may authenticate with `gateway.auth.password` / `DAISYCLAW_GATEWAY_PASSWORD` instead. Token fallback remains intentionally unsupported in trusted-proxy mode.

## Operator scopes header

Trusted-proxy auth is an **identity-bearing** HTTP mode, so callers may optionally declare operator scopes with `x-daisyclaw-scopes` on HTTP API requests.

Note: WebSocket scopes are determined by the Gateway protocol handshake and device identity binding. On Control UI WebSocket upgrade requests, `x-daisyclaw-scopes` is only a cap on the negotiated session scopes, not a grant. For WebSocket scope behavior with trusted-proxy, see [Control UI pairing behavior](#control-ui-pairing-behavior).

Examples:

- `x-daisyclaw-scopes: operator.read`
- `x-daisyclaw-scopes: operator.read,operator.write`
- `x-daisyclaw-scopes: operator.admin,operator.write`

Behavior:

- When the header is present, DaisyClaw honors the declared scope set.
...
- Gateway-auth **plugin HTTP routes** are narrower by default: when `x-daisyclaw-scopes` is absent, their runtime scope falls back to `operator.write`.
...
- For Control UI WebSocket sessions, `x-daisyclaw-scopes` is a scope cap when present on the upgrade request. An empty value yields no scopes.

Practical rule: send `x-daisyclaw-scopes` explicitly when you want a trusted-proxy request to be narrower than the defaults, or when a gateway-auth plugin route needs something stronger than write scope.
...
`daisyclaw security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
<AccordionGroup>
  ...
  <Accordion title="trusted_proxy_loopback_source">
    DaisyClaw rejected a loopback-source trusted-proxy request.
  ...
  <Accordion title="Connection succeeds but methods report missing scope">
    ...
    - Device-less Control UI session: trusted-proxy auth can admit the WebSocket connection without device identity, but DaisyClaw clears scopes on device-less sessions by design.
    ...
    - Overly narrow `x-daisyclaw-scopes`: if your proxy injects this header on the Control UI WebSocket upgrade request, the session scopes are capped to that set. An empty header value yields no scopes.
...
<Steps>
  ...
  <Step title="Update DaisyClaw config">
    Update DaisyClaw config with trusted-proxy auth.
  </Step>
  ...
  <Step title="Audit">
    Run `daisyclaw security audit` and review findings.
  </Step>
</Steps>
