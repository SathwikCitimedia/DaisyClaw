...
This is handy when you need to toggle obscure settings without editing `daisyclaw.json`.

Examples:

```
/debug show
/debug set messages.responsePrefix="[daisyclaw]"
...
Use `DAISYCLAW_PLUGIN_LIFECYCLE_TRACE=1` when plugin lifecycle commands feel slow
...
```bash
DAISYCLAW_PLUGIN_LIFECYCLE_TRACE=1 daisyclaw plugins install tokenjuice --force
```
...
runtime with `node dist/entry.js ...` after `pnpm build`; `pnpm daisyclaw ...`
...
`DAISYCLAW_RUN_NODE_CPU_PROF_DIR`:

```bash
DAISYCLAW_RUN_NODE_CPU_PROF_DIR=.artifacts/cli-cpu pnpm daisyclaw status
```
...
```bash
DAISYCLAW_TRACE_SYNC_IO=1 pnpm daisyclaw gateway --force
```

`pnpm gateway:watch` leaves this flag disabled by default for the watched
Gateway child. Set `DAISYCLAW_TRACE_SYNC_IO=1` when you explicitly want Node
...
`daisyclaw-gateway-watch-main` (or a profile/port-specific variant such as
`daisyclaw-gateway-watch-dev-19001`) and auto-attaches from interactive terminals.
...
```bash
tmux attach -t daisyclaw-gateway-watch-main
```
...
# or
DAISYCLAW_GATEWAY_WATCH_TMUX=0 pnpm gateway:watch
```

Disable auto-attach while keeping tmux management:

```bash
DAISYCLAW_GATEWAY_WATCH_ATTACH=0 pnpm gateway:watch
```
...
`DAISYCLAW_TRACE_SYNC_IO=1` with `--benchmark` when you explicitly want both CPU
...
`DAISYCLAW_PROFILE`, `DAISYCLAW_CONFIG_PATH`, `DAISYCLAW_STATE_DIR`,
`DAISYCLAW_GATEWAY_PORT`, and `DAISYCLAW_SKIP_CHANNELS` into the pane. Put
...
`daisyclaw doctor --fix --non-interactive` once and restarts the Gateway child.
Use `DAISYCLAW_GATEWAY_WATCH_AUTO_DOCTOR=0` when you want the original startup
...
extension `package.json` and `daisyclaw.plugin.json` metadata, `tsconfig.json`,
...
- **Global `--dev` (profile):** isolates state under `~/.daisyclaw-dev` and
...
```bash
pnpm gateway:dev
DAISYCLAW_PROFILE=dev daisyclaw tui
```

If you don't have a global install yet, run the CLI via `pnpm daisyclaw ...`.

What this does:

1. **Profile isolation** (global `--dev`)
   - `DAISYCLAW_PROFILE=dev`
   - `DAISYCLAW_STATE_DIR=~/.daisyclaw-dev`
   - `DAISYCLAW_CONFIG_PATH=~/.daisyclaw-dev/daisyclaw.json`
   - `DAISYCLAW_GATEWAY_PORT=19001` (browser/canvas shift accordingly)
...
2. **Dev bootstrap** (`gateway --dev`)
   ...
   - Skips channel providers in dev mode (`DAISYCLAW_SKIP_CHANNELS=1`).
...
```bash
DAISYCLAW_PROFILE=dev daisyclaw gateway --dev --reset
```
...
## Raw stream logging (DaisyClaw)

DaisyClaw can log the **raw assistant stream** before any filtering/formatting.
...
```bash
pnpm gateway:watch --raw-stream --raw-stream-path ~/.daisyclaw/logs/raw-stream.jsonl
```

Equivalent env vars:

```bash
DAISYCLAW_RAW_STREAM=1
DAISYCLAW_RAW_STREAM_PATH=~/.daisyclaw/logs/raw-stream.jsonl
```

Default file:

`~/.daisyclaw/logs/raw-stream.jsonl`
...
```bash
DAISYCLAW_RAW_STREAM_PATH=~/.daisyclaw/logs/raw-openai-completions.jsonl
```

Default file:

`~/.daisyclaw/logs/raw-openai-completions.jsonl`
...
- If you need to use the built DaisyClaw CLI for other tasks (i.e. `dashboard --no-open` if your debug session spawns a new auth token), you can execute it in another terminal as `node ./daisyclaw.mjs` or create a shell alias like `alias daisyclaw-build="node $(pwd)/daisyclaw.mjs"`
