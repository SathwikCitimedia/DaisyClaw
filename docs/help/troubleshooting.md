---
summary: "Symptom first troubleshooting hub for DaisyClaw"
read_when:
  - DaisyClaw is not working and you need the fastest path to a fix
...
- `daisyclaw status` → shows configured channels and no obvious auth errors.
- `daisyclaw status --all` → full report is present and shareable.
- `daisyclaw gateway probe` → expected gateway target is reachable (`Reachable: yes`). `Capability: ...` tells you what auth level the probe could prove, and `Read probe: limited - missing scope: operator.read` is degraded diagnostics, not a connect failure.
- `daisyclaw gateway status` → `Runtime: running`, `Connectivity probe: ok`, and a plausible `Capability: ...` line. Use `--require-rpc` if you need read-scope RPC proof too.
- `daisyclaw doctor` → no blocking config/service errors.
- `daisyclaw channels status --probe` → reachable gateway returns live per-account
...
- `daisyclaw logs --follow` → steady activity, no repeating fatal errors.
...
and run `daisyclaw status --all` again. See [Tools](/tools) for the profile
...
## Local OpenAI-compatible backend works directly but fails in DaisyClaw

If your local or self-hosted `/v1` backend answers small direct
`/v1/chat/completions` probes but fails on `daisyclaw infer model run` or normal
agent turns:

1. If the error mentions `messages[].content` expecting a string, set
   `models.providers.<provider>.models[].compat.requiresStringContent: true`.
2. If the backend still fails only on DaisyClaw agent turns, set
   `models.providers.<provider>.models[].compat.supportsTools: false` and retry.
3. If tiny direct calls still work but larger DaisyClaw prompts crash the
...
## Plugin install fails with missing daisyclaw extensions

If install fails with `package.json missing daisyclaw.extensions`, the plugin package
is using an old shape that DaisyClaw no longer accepts.

Fix in the plugin package:

1. Add `daisyclaw.extensions` to `package.json`.
2. Point entries at built runtime files (usually `./dist/index.js`).
3. Republish the plugin and run `daisyclaw plugins install <package>` again.

Example:

```json
{
  "name": "@daisyclaw/my-plugin",
  "version": "1.2.3",
  "daisyclaw": {
...
Install policy runs on plugin installs and updates. DaisyClaw-owned plugin
versions normally move with the DaisyClaw release, so an DaisyClaw update can
also need matching `@daisyclaw/*` plugin updates during post-update sync.

Avoid these broad policy shapes unless you also maintain the matching upgrade
rule:

- Freezing DaisyClaw-owned plugins to one exact old version, such as allowing
  only `@daisyclaw/*@2026.5.3`.
...
- Approving plugin versions without considering the policy request's
  `daisyclawVersion` and the plugin candidate metadata.

Safer policy rules allow trusted DaisyClaw-owned plugin updates when the
candidate is compatible with the current DaisyClaw host, instead of pinning a
single release forever. If you block npm by default, make a narrow exception
for the trusted `@daisyclaw/*` plugin packages or plugin ids you use. If you
...
If the policy is intentionally strict, relax it for the trusted DaisyClaw upgrade
window, rerun `daisyclaw plugins update --all`, then restore the stricter rule.
...
If `daisyclaw doctor`, setup, or startup warnings show:
...
them. Do not remove the plugin config. Fix the file ownership or run DaisyClaw as
...
```mermaid
flowchart TD
  A[DaisyClaw is not working] --> B{What breaks first}
  B --> C[No replies]
...
<AccordionGroup>
  ...
  <Accordion title="Dashboard or Control UI will not connect">
    ...
    - `Dashboard: http://...` is shown in `daisyclaw gateway status`
    - `Connectivity probe: ok`
    - `Capability: read-only`, `write-capable`, or `admin-capable`
  ...
  <Accordion title="Exec suddenly asks for approval">
    ```bash
    daisyclaw config get tools.exec.host
    daisyclaw config get tools.exec.security
    daisyclaw config get tools.exec.ask
    ...
    ```bash
    daisyclaw config set tools.exec.host gateway
    daisyclaw config set tools.exec.security full
    daisyclaw config set tools.exec.ask off
    daisyclaw gateway restart
    ```
  ...
  <Accordion title="Browser tool fails">
    ...
    - `daisyclaw` starts, or `user` can see local Chrome tabs.
    ...
    - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `daisyclaw browser stop --browser-profile <name>` to close the active control session and release emulation state without restarting the gateway.
