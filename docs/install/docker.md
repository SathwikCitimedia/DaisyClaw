---
summary: "Optional Docker-based setup and onboarding for DaisyClaw"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run DaisyClaw on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export DAISYCLAW_IMAGE="ghcr.io/daisyclaw/daisyclaw:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/daisyclaw/daisyclaw/pkgs/container/daisyclaw).
  ...
  <Step title="Complete onboarding">
    ...
    `daisyclaw-gateway` directly. `daisyclaw-cli` is for commands you run after
  ...
  <Step title="Open the Control UI">
    ...
    ```bash
    docker compose run --rm daisyclaw-cli dashboard --no-open
    ```
  ...
  <Step title="Configure channels (optional)">
    ...
    ```bash
    # WhatsApp (QR)
    docker compose run --rm daisyclaw-cli channels login

    # Telegram
    docker compose run --rm daisyclaw-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm daisyclaw-cli channels add --channel discord --token "<token>"
    ```
...
```bash
docker build -t daisyclaw:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node daisyclaw-gateway \
  dist/index.js onboard --mode local --no-install-daemon
docker compose run --rm --no-deps --entrypoint node daisyclaw-gateway \
  dist/index.js config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:18789","http://127.0.0.1:18789"]}]'
docker compose up -d daisyclaw-gateway
```

<Note>
Run `docker compose` from the repo root. If you enabled `DAISYCLAW_EXTRA_MOUNTS`
or `DAISYCLAW_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
...
<Note>
Because `daisyclaw-cli` shares `daisyclaw-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d daisyclaw-gateway`, run onboarding
and setup-time config writes through `daisyclaw-gateway` with
...
| Variable                                   | Purpose                                                               |
| ------------------------------------------ | --------------------------------------------------------------------- |
| `DAISYCLAW_IMAGE`                           | Use a remote image instead of building locally                        |
| `DAISYCLAW_IMAGE_APT_PACKAGES`              | Install extra apt packages during build (space-separated)             |
| `DAISYCLAW_IMAGE_PIP_PACKAGES`              | Install extra Python packages during build (space-separated)          |
| `DAISYCLAW_EXTENSIONS`                      | Pre-install plugin dependencies at build time (space-separated names) |
| `DAISYCLAW_EXTRA_MOUNTS`                    | Extra host bind mounts (comma-separated `source:target[:opts]`)       |
| `DAISYCLAW_HOME_VOLUME`                     | Persist `/home/node` in a named Docker volume                         |
| `DAISYCLAW_SANDBOX`                         | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)                |
| `DAISYCLAW_SKIP_ONBOARDING`                 | Skip the interactive onboarding step (`1`, `true`, `yes`, `on`)       |
| `DAISYCLAW_DOCKER_SOCKET`                   | Override Docker socket path                                           |
| `DAISYCLAW_DISABLE_BONJOUR`                 | Disable Bonjour/mDNS advertising (defaults to `1` for Docker)         |
| `DAISYCLAW_DISABLE_BUNDLED_SOURCE_OVERLAYS` | Disable bundled plugin source bind-mount overlays                     |
...
| `DAISYCLAW_OTEL_PRELOADED`                  | Skip starting a second OpenTelemetry SDK when one is preloaded        |

The official Docker image does not ship Homebrew. During onboarding, DaisyClaw
...
`DAISYCLAW_IMAGE_APT_PACKAGES` during image build. The legacy
`DAISYCLAW_DOCKER_APT_PACKAGES` name is still accepted.
For Python dependencies, use `DAISYCLAW_IMAGE_PIP_PACKAGES`. This runs
...
`DAISYCLAW_EXTRA_MOUNTS=/path/to/fork/extensions/synology-chat:/app/extensions/synology-chat:ro`.
...
```bash
export DAISYCLAW_EXTENSIONS="diagnostics-otel"
export OTEL_EXPORTER_OTLP_ENDPOINT="http://otel-collector:4318"
export OTEL_SERVICE_NAME="daisyclaw-gateway"
./scripts/docker/setup.sh
```

Install the official `@daisyclaw/diagnostics-otel` plugin from ClawHub in
packaged Docker installs before enabling export. Custom source-built images can
still include the local plugin source with
`DAISYCLAW_EXTENSIONS=diagnostics-otel`. To enable export, allow and enable the
...
`clawhub:@daisyclaw/diagnostics-prometheus`, enable the
...
```bash
docker compose exec daisyclaw-gateway node dist/index.js health --token "$DAISYCLAW_GATEWAY_TOKEN"
```

### LAN vs loopback

`scripts/docker/setup.sh` defaults `DAISYCLAW_GATEWAY_BIND=lan` so host access to
...
When DaisyClaw runs in Docker, `127.0.0.1` inside the container is the container
...
`DAISYCLAW_DISABLE_BONJOUR=1` so the Gateway does not crash-loop or repeatedly
...
Set `DAISYCLAW_DISABLE_BONJOUR=0` only when running with host networking, macvlan,
...
Docker Compose bind-mounts `DAISYCLAW_CONFIG_DIR` to `/home/node/.daisyclaw`,
`DAISYCLAW_WORKSPACE_DIR` to `/home/node/.daisyclaw/workspace`, and
`DAISYCLAW_AUTH_PROFILE_SECRET_DIR` to `/home/node/.config/daisyclaw`, so those
...
That mounted config directory is where DaisyClaw keeps:

- `daisyclaw.json` for behavior config
- `agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `.env` for env-backed runtime secrets such as `DAISYCLAW_GATEWAY_TOKEN`
...
but separate from `DAISYCLAW_CONFIG_DIR`.

Installed downloadable plugins store their package state under the mounted
DaisyClaw home, so plugin install records and package roots survive container
...
<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ...
    ```bash
    export DAISYCLAW_SANDBOX=1
    export DAISYCLAW_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
    ...
    `workspace-write` while the DaisyClaw sandbox is active; do not mount the
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm daisyclaw-cli gateway probe
    docker compose run -T --rm daisyclaw-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `daisyclaw-cli` uses `network_mode: "service:daisyclaw-gateway"` so CLI
    ...
    `no-new-privileges` on both `daisyclaw-gateway` and `daisyclaw-cli`.
  </Accordion>

  <Accordion title="Docker Desktop DNS failures in daisyclaw-cli">
    Some Docker Desktop setups fail DNS lookups from the shared-network
    `daisyclaw-cli` sidecar after `NET_RAW` is dropped, which shows up as
    `EAI_AGAIN` during npm-backed commands such as `daisyclaw plugins install`.
    ...
    docker compose -f docker-compose.yml -f docker-compose.cli-no-dropped-caps.local.yml run --rm daisyclaw-cli plugins install <package>
    ```

    If you already created a long-running `daisyclaw-cli` container, recreate it
  ...
  <Accordion title="Permissions and EACCES">
    ...
    `/home/node/.daisyclaw`, make sure your host bind mounts are owned by uid 1000:

    ```bash
    sudo chown -R 1000:1000 /path/to/daisyclaw-config /path/to/daisyclaw-workspace
    ```
    ...
    `/path/to/daisyclaw-config/npm` to `root:root` if you intentionally run
    DaisyClaw as root long term.
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export DAISYCLAW_HOME_VOLUME="daisyclaw_home"`
    2. **Bake system deps**: `export DAISYCLAW_IMAGE_APT_PACKAGES="git curl jq"`
    3. **Bake Python deps**: `export DAISYCLAW_IMAGE_PIP_PACKAGES="requests==2.32.5 humanize==4.14.0"`
    4. **Bake Playwright Chromium**: `export DAISYCLAW_INSTALL_BROWSER=1`
    5. **Or install Playwright browsers into a persisted volume**:
       ```bash
       docker compose run --rm daisyclaw-cli \
         node /app/node_modules/playwright-core/cli.js install chromium
       ```
    6. **Persist browser downloads**: use `DAISYCLAW_HOME_VOLUME` or
       `DAISYCLAW_EXTRA_MOUNTS`. DaisyClaw auto-detects the Docker image's
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/daisyclaw/daisyclaw/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    DaisyClaw runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Unauthorized or pairing required in Control UI">
    ...
    ```bash
    docker compose run --rm daisyclaw-cli dashboard --no-open
    docker compose run --rm daisyclaw-cli devices list
    docker compose run --rm daisyclaw-cli devices approve <requestId>
    ```
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm daisyclaw-cli config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"}]'
    docker compose run --rm daisyclaw-cli devices list --url ws://127.0.0.1:18789
    ```
...
- [Updating](/install/updating) — keeping DaisyClaw up to date
