---
summary: "Run DaisyClaw Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want DaisyClaw running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

Run a persistent DaisyClaw Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "DaisyClaw 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the DaisyClaw Gateway in Docker
- Persist `~/.daisyclaw` + `~/.daisyclaw/workspace` on the host (survives restarts/rebuilds)
- Access the Control UI from your laptop via an SSH tunnel

That mounted `~/.daisyclaw` state includes `daisyclaw.json`, per-agent
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-daisyclaw-project --name="DaisyClaw Gateway"
    gcloud config set project my-daisyclaw-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create daisyclaw-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh daisyclaw-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the DaisyClaw repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    DAISYCLAW_IMAGE=daisyclaw:latest
    DAISYCLAW_GATEWAY_TOKEN=
    DAISYCLAW_GATEWAY_BIND=lan
    DAISYCLAW_GATEWAY_PORT=18789

    DAISYCLAW_CONFIG_DIR=/home/$USER/.daisyclaw
    DAISYCLAW_WORKSPACE_DIR=/home/$USER/.daisyclaw/workspace

    GOG_KEYRING_PASSWORD=
    XDG_CONFIG_HOME=/home/node/.daisyclaw
    ```

    Set `DAISYCLAW_GATEWAY_TOKEN` when you want to manage the stable gateway
    token through `.env`; otherwise configure `gateway.auth.token` before
    relying on clients across restarts. If neither source exists, DaisyClaw uses
    ...
    **Do not commit this file.**
    ...
    This `.env` file is for container/runtime env such as `DAISYCLAW_GATEWAY_TOKEN`.
    ...
    services:
      daisyclaw-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - DAISYCLAW_GATEWAY_BIND=${DAISYCLAW_GATEWAY_BIND}
          - DAISYCLAW_GATEWAY_PORT=${DAISYCLAW_GATEWAY_PORT}
        ...
        ports:
          ...
          - "127.0.0.1:${DAISYCLAW_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`DAISYCLAW_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm daisyclaw-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh daisyclaw-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop daisyclaw-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type daisyclaw-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start daisyclaw-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create daisyclaw-deploy \
     --display-name="DaisyClaw Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-daisyclaw-project \
     --member="serviceAccount:daisyclaw-deploy@my-daisyclaw-project.iam.gserviceaccount.com" \
