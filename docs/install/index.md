---
summary: "Install DaisyClaw - installer script, npm/pnpm/bun, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs DaisyClaw, and launches onboarding.
...
<Tabs>
  <Tab title="macOS / Linux / WSL2">
    ```bash
    curl -fsSL https://daisyclaw.ai/install.sh | bash -s -- --no-onboard
    ```
  </Tab>
  <Tab title="Windows (PowerShell)">
    ```powershell
...
Use this when you want DaisyClaw and Node kept under a local prefix such as
`~/.daisyclaw`, without depending on a system-wide Node install:

```bash
curl -fsSL https://daisyclaw.ai/install-cli.sh | bash
```
...
`daisyclaw update --channel dev` and `daisyclaw update --channel stable`. See
...
<Tabs>
  <Tab title="npm">
    ...
    for the DaisyClaw package install. If you install manually with npm, your own
...
Or skip the link and use `pnpm daisyclaw ...` from inside the repo. See [Setup](/start/setup) for full development workflows.
...
```bash
daisyclaw --version      # confirm the CLI is available
daisyclaw doctor         # check for config issues
daisyclaw gateway status # verify the Gateway is running
```

If you want managed startup after install:

- macOS: LaunchAgent via `daisyclaw onboard --install-daemon` or `daisyclaw gateway install`
- Linux/WSL2: systemd user service via the same commands
- Native Windows: Scheduled Task first, with a per-user Startup-folder login item fallback if task creation is denied

## Hosting and deployment

Deploy DaisyClaw on a cloud server or VPS:
...
<CardGroup cols={3}>
  ...
  <Card title="Uninstall" href="/install/uninstall" icon="trash-2">
    Remove DaisyClaw completely.
  </Card>
</CardGroup>

## Troubleshooting: `daisyclaw` not found

If the install succeeded but `daisyclaw` is not found in your terminal:
