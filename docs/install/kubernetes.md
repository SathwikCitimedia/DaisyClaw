---
summary: "Deploy DaisyClaw Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run DaisyClaw on a Kubernetes cluster
  - You want to test DaisyClaw in a Kubernetes environment
title: "Kubernetes"
---

A minimal starting point for running DaisyClaw on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

DaisyClaw is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/daisyclaw 18789:18789 -n daisyclaw
...
```bash
kubectl get secret daisyclaw-secrets -n daisyclaw -o jsonpath='{.data.DAISYCLAW_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: daisyclaw (configurable via DAISYCLAW_NAMESPACE)
├── Deployment/daisyclaw        # Single pod, init container + gateway
├── Service/daisyclaw           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/daisyclaw-config  # daisyclaw.json + AGENTS.md
└── Secret/daisyclaw-secrets    # Gateway token + API keys
```
...
Edit `daisyclaw.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret daisyclaw-secrets -n daisyclaw \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/daisyclaw -n daisyclaw
```

### Custom namespace

```bash
DAISYCLAW_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/daisyclaw/daisyclaw:latest # or pin to a specific version from https://github.com/daisyclaw/daisyclaw/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # daisyclaw.json + AGENTS.md
