---
summary: "Run DaisyClaw in a sandboxed macOS VM (local or hosted) when you need isolation or iMessage"
read_when:
  - You want DaisyClaw isolated from your main macOS environment
...
Run DaisyClaw in a sandboxed macOS VM on your existing Apple Silicon Mac using [Lume](https://cua.ai/docs/lume).
...
1. Install Lume
2. `lume create daisyclaw --os macos --ipsw latest`
3. Complete Setup Assistant, enable Remote Login (SSH)
4. `lume run daisyclaw --no-display`
5. SSH in, install DaisyClaw, configure channels
6. Done
...
```bash
lume create daisyclaw --os macos --ipsw latest
```
...
```bash
lume stop daisyclaw
lume run daisyclaw --no-display
```

The VM runs in the background. DaisyClaw's daemon keeps the gateway running.

To check status:

```bash
ssh youruser@192.168.64.X "daisyclaw status"
```
...
This is the killer feature of running on macOS. Use [iMessage](/channels/imessage) with `imsg` to add Messages to DaisyClaw.

Inside the VM:

1. Sign in to Messages.
2. Install `imsg`.
3. Grant Full Disk Access and Automation permission for the process running DaisyClaw/`imsg`.
4. Verify RPC support with `imsg rpc --help`.

Add to your DaisyClaw config:
...
```bash
lume stop daisyclaw
lume clone daisyclaw daisyclaw-golden
```

Reset anytime:

```bash
lume stop daisyclaw && lume delete daisyclaw
lume clone daisyclaw-golden daisyclaw
lume run daisyclaw --no-display
```
...
| VM IP not showing        | Wait for VM to fully boot, run `lume get daisyclaw` again                           |
| Lume command not found   | Add `~/.local/bin` to your PATH                                                    |
| WhatsApp QR not scanning | Ensure you're logged into the VM (not host) when running `daisyclaw channels login` |
