---
summary: "Host DaisyClaw on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up DaisyClaw on Oracle Cloud
  - Looking for free VPS hosting for DaisyClaw
  - Want 24/7 DaisyClaw on a small server
title: "Oracle Cloud"
---

Run a persistent DaisyClaw Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Configure user and hostname">
    ```bash
    sudo hostnamectl set-hostname daisyclaw
    sudo passwd ubuntu
  ...
  <Step title="Install Tailscale">
    ...
    sudo tailscale up --ssh --hostname=daisyclaw
    ```

    From now on, connect via Tailscale: `ssh ubuntu@daisyclaw`.
  ...
  <Step title="Configure the gateway">
    ...
    ```bash
    daisyclaw config set gateway.bind loopback
    daisyclaw config set gateway.auth.mode token
    daisyclaw doctor --generate-gateway-token
    daisyclaw config set gateway.tailscale.mode serve
    daisyclaw config set gateway.trustedProxies '["127.0.0.1"]'
  ...
  <Step title="Verify">
    ...
    ```
    https://daisyclaw.<tailnet-name>.ts.net/
    ```
...
- `chmod 700 ~/.daisyclaw` to restrict credential file permissions.
- `daisyclaw security audit` for an DaisyClaw-specific posture check.
...
The Always Free tier is ARM (`aarch64`). Most DaisyClaw features work fine; a small number of native binaries need ARM builds:
...
- `~/.daisyclaw/` — `daisyclaw.json`, per-agent `auth-profiles.json`, channel/provider state, and session data.
- `~/.daisyclaw/workspace/` — the agent workspace (SOUL.md, memory, artifacts).
...
```bash
ssh -L 18789:127.0.0.1:18789 ubuntu@daisyclaw
```
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=daisyclaw --reset` to re-authenticate.

**Gateway will not start** -- Run `daisyclaw doctor --non-interactive` and check logs with `journalctl --user -u daisyclaw-gateway.service -n 50`.
