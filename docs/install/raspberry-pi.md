---
summary: "Host DaisyClaw on a Raspberry Pi for always-on self-hosting"
read_when:
  - Setting up DaisyClaw on a Raspberry Pi
  - Running DaisyClaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

Run a persistent, always-on DaisyClaw Gateway on a Raspberry Pi. Since the Pi is just the gateway (models run in the cloud via API), even a modest Pi handles the workload well — typical hardware cost is **$35–80 one-time**, no monthly fees.
...
<Steps>
  ...
  <Step title="Verify">
    ```bash
    daisyclaw status
    systemctl --user status daisyclaw-gateway.service
    journalctl --user -u daisyclaw-gateway.service -f
    ```
  </Step>
  ...
  <Step title="Access the Control UI">
    ...
    ```bash
    ssh user@gateway-host 'daisyclaw dashboard --no-open'
    ```
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/daisyclaw-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/daisyclaw-compile-cache
mkdir -p /var/tmp/daisyclaw-compile-cache
export DAISYCLAW_NO_RESPAWN=1
EOF
source ~/.bashrc
```

`DAISYCLAW_NO_RESPAWN=1` keeps routine Gateway restarts in-process, which avoids extra process handoffs and keeps PID tracking simple on small hosts.
...
**systemd drop-in for stable restarts** -- If this Pi is mostly running DaisyClaw, add a service drop-in:

```bash
systemctl --user edit daisyclaw-gateway.service
```

```ini
[Service]
Environment=DAISYCLAW_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/daisyclaw-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```

Then `systemctl --user daemon-reload && systemctl --user restart daisyclaw-gateway.service`. On a headless Pi, also enable lingering once so the user service survives logout: `sudo loginctl enable-linger "$(whoami)"`.
...
DaisyClaw state lives under:

- `~/.daisyclaw/` — `daisyclaw.json`, per-agent `auth-profiles.json`, channel/provider state, sessions.
- `~/.daisyclaw/workspace/` — agent workspace (SOUL.md, memory, artifacts).
...
**Service will not start** -- Check logs with `journalctl --user -u daisyclaw-gateway.service --no-pager -n 100` and run `daisyclaw doctor --non-interactive`. If this is a headless Pi, also verify lingering is enabled: `sudo loginctl enable-linger "$(whoami)"`.
...
- [Updating](/install/updating) -- keep DaisyClaw up to date
