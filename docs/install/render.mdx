---
summary: "Deploy DaisyClaw on Render with Infrastructure-as-Code"
read_when:
  - Deploying DaisyClaw to Render
...
Deploy DaisyClaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/daisyclaw/daisyclaw)
...
repository configures everything needed to run DaisyClaw:
...
services:
  - type: web
    ...
    envVars:
      - key: DAISYCLAW_GATEWAY_PORT
        value: "8080"
      - key: DAISYCLAW_STATE_DIR
        value: /data/.daisyclaw
      - key: DAISYCLAW_WORKSPACE_DIR
        value: /data/workspace
      - key: DAISYCLAW_GATEWAY_TOKEN
...
your fork's `render.yaml` (but note: no persistent disk means DaisyClaw state
...
`DAISYCLAW_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you use the original DaisyClaw repository, Render will not auto-deploy your DaisyClaw. To update it, run a manual Blueprint sync from the dashboard.
...
For DaisyClaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive with DaisyClaw state plus any configured
...
- Missing `DAISYCLAW_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `DAISYCLAW_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `daisyclaw backup create` in the Render shell.
