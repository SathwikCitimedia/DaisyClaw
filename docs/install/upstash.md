---
summary: "Host DaisyClaw on Upstash Box with keep-alive and SSH tunnel access"
read_when:
  - Deploying DaisyClaw to Upstash Box
  - You want a managed Linux environment for DaisyClaw with SSH-tunneled dashboard access
title: "Upstash Box"
---

Run a persistent DaisyClaw Gateway on Upstash Box, a managed Linux environment
...
Upstash maintains its current DaisyClaw Box walkthrough at
[DaisyClaw Setup](https://upstash.com/docs/box/guides/daisyclaw-setup).

## Connect with an SSH tunnel

Forward the DaisyClaw dashboard port to your local machine. Use your Box API key
...
```bash
daisyclaw config set gateway.bind lan
nohup daisyclaw gateway > gateway.log 2>&1 &
```
...
- [Updating DaisyClaw](/install/updating)
