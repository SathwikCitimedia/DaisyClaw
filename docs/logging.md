...
read_when:
  - You need a beginner-friendly overview of DaisyClaw logging
  - You want to configure log levels, formats, or redaction
  - You are troubleshooting and need to find logs quickly
title: "Logging"
---

DaisyClaw has two main log surfaces:
...
DaisyClaw keeps up to five numbered archives beside the active file, such as
`daisyclaw-YYYY-MM-DD.1.log`, and keeps writing to a fresh active log instead of
suppressing diagnostics.

You can override this in `~/.daisyclaw/daisyclaw.json`:

```json
{
  "logging": {
    "file": "/path/to/daisyclaw.log"
  }
}
```
...
or times out before `logs.tail` answers, `daisyclaw logs` falls back to the
configured Gateway file log automatically. Explicit `--url` targets do not use
this fallback. `daisyclaw logs --follow` is stricter: on Linux it uses the active
...
```bash
daisyclaw channels logs --channel whatsapp
```
...
DaisyClaw preserves the original structured log arguments alongside these fields
...
`daisyclaw gateway` also has WebSocket protocol logging for RPC traffic:
...
```bash
daisyclaw gateway
daisyclaw gateway --verbose --ws-log compact
daisyclaw gateway --verbose --ws-log full
```

## Configuring logging

All logging configuration lives under `logging` in `~/.daisyclaw/daisyclaw.json`.

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/daisyclaw/daisyclaw-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
You can override both via the **`DAISYCLAW_LOG_LEVEL`** environment variable (e.g. `DAISYCLAW_LOG_LEVEL=debug`). The env var takes precedence over the config file, so you can raise verbosity for a single run without editing `daisyclaw.json`. You can also pass the global CLI option **`--log-level <level>`** (for example, `daisyclaw --log-level debug gateway run`), which overrides the environment variable for that command.
...
```bash
DAISYCLAW_DEBUG_MODEL_TRANSPORT=1 daisyclaw gateway
DAISYCLAW_DEBUG_MODEL_PAYLOAD=tools DAISYCLAW_DEBUG_SSE=events daisyclaw gateway
```

Available flags:

- `DAISYCLAW_DEBUG_MODEL_TRANSPORT=1`: emit request start, fetch response, SDK
  headers, first streaming event, stream completion, and transport errors at
  `info` level.
- `DAISYCLAW_DEBUG_MODEL_PAYLOAD=summary`: include a bounded request payload
  summary in model request logs.
- `DAISYCLAW_DEBUG_MODEL_PAYLOAD=tools`: include all model-facing tool names in
  the payload summary.
- `DAISYCLAW_DEBUG_MODEL_PAYLOAD=full-redacted`: include a redacted, capped JSON
  payload snapshot. Use only while debugging; secrets are redacted but prompts
  and message text may still be present.
- `DAISYCLAW_DEBUG_SSE=events`: emit first-event and stream-completion timing.
- `DAISYCLAW_DEBUG_SSE=peek`: also emit the first five redacted SSE event
  payloads, capped per event.
- `DAISYCLAW_DEBUG_CODE_MODE=1`: emit code-mode model-surface diagnostics,
  including when native provider tools are hidden because code mode owns the
  tool surface.

These flags log through normal DaisyClaw logging, so `daisyclaw logs --follow`
...
DaisyClaw writes the trace fields as top-level JSON keys (`traceId`, `spanId`,
...
DaisyClaw can redact sensitive tokens before they hit console output, file logs,
...
policy. DaisyClaw still redacts safety-boundary payloads that can be shown to UI
...
- **Diagnostics flags** — targeted debug-log flags that route extra logs to
  ...
  or via the `DAISYCLAW_DIAGNOSTICS=...` env override. Full guide:
...
- **Gateway not reachable?** Run `daisyclaw doctor` first.
