...
DaisyClaw supports **camera capture** for agent workflows:
...
```bash
daisyclaw nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
daisyclaw nodes camera snap --node <id> --facing front
daisyclaw nodes camera clip --node <id> --duration 3000
daisyclaw nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`daisyclaw.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return "Camera disabled by user".

### CLI helper (node invoke)

Use the main `daisyclaw` CLI to invoke camera commands on the macOS node.

Examples:

```bash
daisyclaw nodes camera list --node <id>            # list camera ids
daisyclaw nodes camera snap --node <id>            # prints saved path
daisyclaw nodes camera snap --node <id> --max-width 1280
daisyclaw nodes camera snap --node <id> --delay-ms 2000
daisyclaw nodes camera snap --node <id> --device-id <id>
daisyclaw nodes camera clip --node <id> --duration 10s          # prints saved path
daisyclaw nodes camera clip --node <id> --duration-ms 3000      # prints saved path (legacy flag)
daisyclaw nodes camera clip --node <id> --device-id <id>
daisyclaw nodes camera clip --node <id> --no-audio
```

Notes:

- `daisyclaw nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
daisyclaw nodes screen record --node <id> --duration 10s --fps 15   # prints saved path
```
