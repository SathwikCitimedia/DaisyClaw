...
`daisyclaw nodes …` works against this Mac). In remote gateway mode, browser
automation is handled by the CLI node host (`daisyclaw node run` or the
...
daisyclaw devices reject <requestId>
...
`daisyclaw devices list` before approving.
...
- `node.pair.*` (CLI: `daisyclaw nodes pending/approve/reject/remove/rename`) is a separate gateway-owned
  node pairing store; it does **not** gate the WS `connect` handshake.
- `daisyclaw nodes remove --node <id|name|ip>` deletes stale entries from that
...
- **Approvals**: enforced on the node host via `~/.daisyclaw/exec-approvals.json`.

Approval note:

- Approval-backed node runs bind exact request context.
- For direct shell/runtime file executions, DaisyClaw also best-effort binds one concrete local
  file operand and denies the run if that file changes before execution.
- If DaisyClaw cannot identify exactly one concrete local file for an interpreter/runtime command,
...
```bash
daisyclaw node run --host <gateway-host> --port 18789 --display-name "Build Node"
```
...
export DAISYCLAW_GATEWAY_TOKEN="<gateway-token>"
daisyclaw node run --host 127.0.0.1 --port 18790 --display-name "Build Node"
```

Notes:

- `daisyclaw node run` supports token or password auth.
- Env vars are preferred: `DAISYCLAW_GATEWAY_TOKEN` / `DAISYCLAW_GATEWAY_PASSWORD`.
...
- Node-host auth resolution only honors `DAISYCLAW_GATEWAY_*` env vars.

### Start a node host (service)

```bash
daisyclaw node install --host <gateway-host> --port 18789 --display-name "Build Node"
daisyclaw node start
...
If the node retries with changed auth details, re-run `daisyclaw devices list`
and approve the current `requestId`.

Naming options:

- `--display-name` on `daisyclaw node run` / `daisyclaw node install` (persists in `~/.daisyclaw/node.json` on the node).
- `daisyclaw nodes rename --node <id|name|ip> --name "Build Node"` (gateway override).
...
```bash
daisyclaw approvals allowlist add --node <id|name|ip> "/usr/bin/uname"
daisyclaw approvals allowlist add --node <id|name|ip> "/usr/bin/sw_vers"
```

Approvals live on the node host at `~/.daisyclaw/exec-approvals.json`.
...
```bash
daisyclaw config set tools.exec.host node
daisyclaw config set tools.exec.security allowlist
daisyclaw config set tools.exec.node "<id-or-name>"
```
...
```bash
daisyclaw nodes invoke --node <idOrNameOrIp> --command canvas.eval --params '{"javaScript":"location.href"}'
```
...
```bash
daisyclaw nodes canvas snapshot --node <idOrNameOrIp> --format png
daisyclaw nodes canvas snapshot --node <idOrNameOrIp> --format jpg --max-width 1200 --quality 0.9
```

### Canvas controls

```bash
daisyclaw nodes canvas present --node <idOrNameOrIp> --target https://example.com
daisyclaw nodes canvas hide --node <idOrNameOrIp>
daisyclaw nodes canvas navigate https://example.com --node <idOrNameOrIp>
daisyclaw nodes canvas eval --node <idOrNameOrIp> --js "document.title"
```
...
```bash
daisyclaw nodes canvas a2ui push --node <idOrNameOrIp> --text "Hello"
daisyclaw nodes canvas a2ui push --node <idOrNameOrIp> --jsonl ./payload.jsonl
daisyclaw nodes canvas a2ui reset --node <idOrNameOrIp>
```
...
```bash
daisyclaw nodes camera list --node <idOrNameOrIp>
daisyclaw nodes camera snap --node <idOrNameOrIp>            # default: both facings (2 MEDIA lines)
daisyclaw nodes camera snap --node <idOrNameOrIp> --facing front
```

Video clips (`mp4`):

```bash
daisyclaw nodes camera clip --node <idOrNameOrIp> --duration 10s
daisyclaw nodes camera clip --node <idOrNameOrIp> --duration 3000 --no-audio
```
...
```bash
daisyclaw nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10
daisyclaw nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10 --no-audio
```
...
```bash
daisyclaw nodes location get --node <idOrNameOrIp>
daisyclaw nodes location get --node <idOrNameOrIp> --accuracy precise --max-age 15000 --location-timeout 10000
```
...
```bash
daisyclaw nodes invoke --node <idOrNameOrIp> --command sms.send --params '{"to":"+15555550123","message":"Hello from DaisyClaw"}'
```
...
```bash
daisyclaw nodes invoke --node <idOrNameOrIp> --command device.status --params '{}'
daisyclaw nodes invoke --node <idOrNameOrIp> --command device.apps --params '{"limit":10}'
daisyclaw nodes invoke --node <idOrNameOrIp> --command notifications.list --params '{}'
daisyclaw nodes invoke --node <idOrNameOrIp> --command photos.latest --params '{"limit":1}'
```
...
```bash
daisyclaw nodes notify --node <idOrNameOrIp> --title "Ping" --body "Gateway ready"
daisyclaw nodes invoke --node <idOrNameOrIp> --command system.which --params '{"name":"git"}'
```
...
- On headless node host, `system.run` is gated by exec approvals (`~/.daisyclaw/exec-approvals.json`).
...
```bash
daisyclaw config set tools.exec.node "node-id-or-name"
```
...
```bash
daisyclaw config unset tools.exec.node
daisyclaw config unset 'agents.list[0].tools.exec.node'
```
...
DaisyClaw can run a **headless node host** (no UI) that connects to the Gateway
...
```bash
daisyclaw node run --host <gateway-host> --port 18789
```

Notes:

- Pairing is still required (the Gateway will show a device pairing prompt).
- The node host stores its node id, token, display name, and gateway connection info in `~/.daisyclaw/node.json`.
- Exec approvals are enforced locally via `~/.daisyclaw/exec-approvals.json`
  (see [Exec approvals](/tools/exec-approvals)).
- On macOS, the headless node host executes `system.run` locally by default. Set
  `DAISYCLAW_NODE_EXEC_HOST=app` to route `system.run` through the companion app exec host; add
  `DAISYCLAW_NODE_EXEC_FALLBACK=0` to require the app host and fail closed if it is unavailable.
- Add `--tls` / `--tls-fingerprint` when the Gateway WS uses TLS.

## Mac node mode

- The macOS menubar app connects to the Gateway WS server as a node (so `daisyclaw nodes …` works against this Mac).
