...
## Config (`~/.daisyclaw/daisyclaw.json`)
...
- `consultThinkingLevel`: optional thinking level override for the full DaisyClaw agent run behind realtime `daisyclaw_agent_consult` calls.
- `consultFastMode`: optional fast-mode override for realtime `daisyclaw_agent_consult` calls.
...
- `realtime.consultRouting`: `provider-direct` preserves the provider's direct reply when it skips `daisyclaw_agent_consult`; `force-agent-consult` makes Gateway relay route finalized user transcripts through DaisyClaw instead.
- `realtime.instructions`: appends provider-facing system instructions to DaisyClaw's built-in realtime prompt. Use it for voice style and tone; DaisyClaw keeps the default `daisyclaw_agent_consult` guidance.
...
- Browser realtime Talk uses `talk.client.toolCall` for `daisyclaw_agent_consult` instead of exposing `chat.send` to provider-owned browser sessions.
...
- macOS local MLX playback uses the bundled `daisyclaw-mlx-tts` helper when present, or an executable on `PATH`. Set `DAISYCLAW_MLX_TTS_BIN` to point at a custom helper binary during development.
