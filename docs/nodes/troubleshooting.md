...
```bash
daisyclaw nodes describe --node <idOrNameOrIp>
daisyclaw nodes canvas snapshot --node <idOrNameOrIp>
daisyclaw logs --follow
```
...
daisyclaw nodes status
daisyclaw approvals get --node <idOrNameOrIp>
daisyclaw approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`daisyclaw approvals get --node ...`), not in the gateway pairing record.
