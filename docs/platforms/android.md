...
<Note>
The official Android app is available on [Google Play](https://play.google.com/store/apps/details?id=ai.daisyclaw.app&hl=en_IN). It is a companion node and requires a running DaisyClaw Gateway. The source code is also available in the [DaisyClaw repository](https://github.com/daisyclaw/daisyclaw) under `apps/android`; see [apps/android/README.md](https://github.com/daisyclaw/daisyclaw/blob/main/apps/android/README.md) for build instructions.
</Note>
...
- Install: [Google Play](https://play.google.com/store/apps/details?id=ai.daisyclaw.app&hl=en_IN) for the app, [Getting Started](/start/getting-started) for the Gateway, then [Pairing](/channels/pairing).
...
- You can run the CLI (`daisyclaw`) on the gateway machine (or via SSH).
...
```bash
daisyclaw gateway discover --json
```
...
1. Set up a DNS-SD zone (example `daisyclaw.internal.`) on the gateway host and publish `_daisyclaw-gw._tcp` records.
...
1. Create `~/.daisyclaw/workspace/canvas/index.html` on the gateway host.

2. Navigate the node to it (LAN):

```bash
daisyclaw nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18789/__daisyclaw__/canvas/"}'
```

Tailnet (optional): if both devices are on Tailscale, use a MagicDNS name or tailnet IP instead of `.local`, e.g. `http://<gateway-magicdns>:18789/__daisyclaw__/canvas/`.

This server injects a live-reload client into HTML and reloads on file changes.
The Gateway also serves `/__daisyclaw__/a2ui/`, but the Android app treats remote A2UI pages as render-only. Action-capable A2UI commands use the bundled app-owned A2UI page before applying messages.
...
Android supports launching DaisyClaw from the system assistant trigger (Google
Assistant). When configured, holding the home button or saying "Hey Google, ask
DaisyClaw..." opens the app and hands the prompt into the chat composer.
...
and whether the user has set DaisyClaw as the default assistant app.
</Note>
