...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `daisyclaw.internal.`), **or**
...
Run `daisyclaw devices list` again before approval.
...
daisyclaw gateway call node.list --params "{}"
```
...
By default, official/TestFlight builds and gateways use the hosted relay at `https://ios-push-relay.daisyclaw.ai`.
...
- `DAISYCLAW_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
- `DAISYCLAW_PUSH_RELAY_BASE_URL` still works as a temporary env override for official/TestFlight iOS builds.
...
- Only genuine DaisyClaw iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight DaisyClaw builds.
...
```bash
export DAISYCLAW_APNS_TEAM_ID="TEAMID"
export DAISYCLAW_APNS_KEY_ID="KEYID"
export DAISYCLAW_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
```bash
mkdir -p ~/.daisyclaw/credentials/apns
chmod 700 ~/.daisyclaw/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.daisyclaw/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.daisyclaw/credentials/apns/AuthKey_KEYID.p8
export DAISYCLAW_APNS_PRIVATE_KEY_PATH="$HOME/.daisyclaw/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_daisyclaw-gw._tcp` on `local.` and, when configured, the same
...
`daisyclaw.internal.`) and Tailscale split DNS.
...
```bash
daisyclaw nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__daisyclaw__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__daisyclaw__/canvas/` and `/__daisyclaw__/a2ui/`.
...
tools; the iOS app exposes iPhone capabilities through DaisyClaw node commands
such as `canvas.*`, `camera.*`, `screen.*`, `location.*`, and `talk.*`.

Agents can still operate the iOS app through DaisyClaw by invoking node
...
```bash
daisyclaw nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__daisyclaw; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
daisyclaw nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `daisyclaw devices list` and approve manually.
