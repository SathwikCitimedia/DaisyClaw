...
2. `npm i -g daisyclaw@latest`
3. `daisyclaw onboard --install-daemon`
...
DaisyClaw installs a systemd **user** service by default. Use a **system**
service for shared or always-on servers. `daisyclaw gateway install` and
`daisyclaw onboard --install-daemon` already render the current canonical unit
...
Create `~/.config/systemd/user/daisyclaw-gateway[-<profile>].service`:

```
[Unit]
Description=DaisyClaw Gateway (profile: <profile>, v<version>)
...
sessions and channel connections. DaisyClaw therefore biases transient child
processes to be killed before the Gateway when possible.

For eligible Linux child spawns, DaisyClaw starts the child through a short
...
- DaisyClaw-launched browser/Chrome processes.

The wrapper is Linux-only and is skipped when `/bin/sh` is unavailable. It is
also skipped if the child env sets `DAISYCLAW_CHILD_OOM_SCORE_ADJ=0`, `false`,
`no`, or `off`.
