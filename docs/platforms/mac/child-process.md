...
service via the external `daisyclaw` CLI (no embedded runtime). This gives you
...
- The app installs a per-user LaunchAgent labeled `ai.daisyclaw.gateway`
  (or `ai.daisyclaw.<profile>` when using `--profile`/`DAISYCLAW_PROFILE`; legacy `com.daisyclaw.*` is supported).
