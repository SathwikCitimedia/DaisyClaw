...
- Same path: run the app from a fixed location (for DaisyClaw, `dist/DaisyClaw.app`).
...
Prefer granting Accessibility to DaisyClaw.app, Peekaboo.app, or another signed
...
```bash
sudo tccutil reset Accessibility ai.daisyclaw.mac
sudo tccutil reset ScreenCapture ai.daisyclaw.mac
...
Workaround: move files into the DaisyClaw workspace (`~/.daisyclaw/workspace`) if you want to avoid per-folder grants.
