---
summary: "DaisyClaw macOS companion app (menu bar + gateway broker)"
read_when:
...
The macOS app is the **menu-bar companion** for DaisyClaw. It owns permissions,
...
- Installs the global CLI (`daisyclaw`) on request via npm, pnpm, or bun (the app prefers npm, then pnpm, then bun; Node remains the recommended Gateway runtime).

## Local vs remote mode

- **Local** (default): the app attaches to a running local Gateway if present;
  otherwise it enables the launchd service via `daisyclaw gateway install`.
...
The app manages a per-user LaunchAgent labeled `ai.daisyclaw.gateway`
(or `ai.daisyclaw.<profile>` when using `--profile`/`DAISYCLAW_PROFILE`; legacy `com.daisyclaw.*` still unloads).
...
Replace the label with `ai.daisyclaw.<profile>` when running a named profile.
...
The app registers the `daisyclaw://` URL scheme for local actions.
...
```bash
open 'daisyclaw://agent?message=Hello%20from%20deep%20link'
```
...
1. Install and launch **DaisyClaw.app**.
...
Avoid putting your DaisyClaw state dir in iCloud or other cloud-synced folders.
...
If `daisyclaw doctor` detects state under:
...
```bash
cd apps/macos
swift run daisyclaw-mac connect --json
swift run daisyclaw-mac discover --timeout 3000 --json
```
...
<Tip>
Compare against `daisyclaw gateway discover --json` to see whether the macOS app's discovery pipeline (`local.` plus the configured wide-area domain, with wide-area and Tailscale Serve fallbacks) differs from the Node CLI's `dns-sd` based discovery.
</Tip>
