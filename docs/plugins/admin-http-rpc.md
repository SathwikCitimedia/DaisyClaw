...
The plugin is included with DaisyClaw, but it is off by default. When disabled, the route is not registered. When enabled, it adds:
...
Use the WebSocket RPC path for DaisyClaw clients and interactive tools that can keep a Gateway WebSocket connection open.
...
<Tabs>
  <Tab title="CLI">
    ```bash
    daisyclaw plugins enable admin-http-rpc
...
```bash
daisyclaw plugins disable admin-http-rpc
daisyclaw gateway restart
```
...
- For `token` and `password` auth, narrower `x-daisyclaw-scopes` headers are ignored and the normal full operator defaults are restored.
- Trusted identity-bearing HTTP modes honor `x-daisyclaw-scopes` when present.
...
The normal Gateway WebSocket RPC path remains the preferred control-plane API for DaisyClaw clients. Use admin HTTP RPC only for host tooling that needs a request/response HTTP surface.
