...
At startup, DaisyClaw does roughly this:
...
The manifest is the control-plane source of truth. DaisyClaw uses it to:
...
channels, slots, and auto-enable rules. If that derived set is empty, DaisyClaw
...
DaisyClaw does not cache plugin discovery results or direct manifest registry
...
DaisyClaw still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, DaisyClaw calls hooks in this rough order.
The "When to use" column is the quick decision guide.
Compatibility-only provider fields that DaisyClaw no longer calls, such as
...
| --  | _(built-in model lookup)_         | DaisyClaw tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                         |
...
| 24  | `refreshOAuth`                    | OAuth refresh override for custom refresh endpoints or refresh-failure policy                                  | Provider does not fit the shared DaisyClaw refreshers                                                                                          |
...
that still runs on DaisyClaw's normal inference loop.

`resolveUsageAuth` decides whether DaisyClaw should call `fetchUsageSnapshot` or
...
<AccordionGroup>
  <Accordion title="Pass-through catalog providers">
    ...
    model ids ahead of DaisyClaw's static catalog.
  </Accordion>
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as DaisyClaw adds those
...
- `extractStructuredWithModel(...)` is the plugin-facing seam for bounded
  ...
  product plugins own their routes and schemas while DaisyClaw owns the
...
- DaisyClaw only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "DaisyClaw plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-daisyclaw-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-daisyclaw-scopes` only when the header is explicitly present
  - if `x-daisyclaw-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-daisyclaw-scopes` header contract.

## Plugin SDK import paths

Use narrow SDK subpaths instead of the monolithic `daisyclaw/plugin-sdk` root
barrel when authoring new plugins. Core subpaths:

| Subpath                             | Purpose                                            |
| ----------------------------------- | -------------------------------------------------- |
| `daisyclaw/plugin-sdk/plugin-entry`  | Plugin registration primitives                     |
| `daisyclaw/plugin-sdk/channel-core`  | Channel entry/build helpers                        |
| `daisyclaw/plugin-sdk/core`          | Generic shared helpers and umbrella contract       |
| `daisyclaw/plugin-sdk/config-schema` | Root `daisyclaw.json` Zod schema (`DaisyClawSchema`) |
...
<Info>
`daisyclaw/plugin-sdk/channel-runtime`, `daisyclaw/plugin-sdk/channel-lifecycle`,
small channel helper facades, `daisyclaw/plugin-sdk/outbound-runtime`,
`daisyclaw/plugin-sdk/outbound-send-deps`, `daisyclaw/plugin-sdk/config-runtime`,
and `daisyclaw/plugin-sdk/infra-runtime` are deprecated compatibility shims for
...
External plugins should only import `daisyclaw/plugin-sdk/*` subpaths. Never
...
`daisyclaw/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape DaisyClaw writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to DaisyClaw's
...
- if both `catalog` and `discovery` are registered, DaisyClaw uses `catalog`
...
- Read-only command paths such as `daisyclaw status`, `daisyclaw status --all`,
  `daisyclaw channels status`, `daisyclaw channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `daisyclaw.extensions`:
...
Security guardrail: every `daisyclaw.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `daisyclaw plugins install` installs plugin dependencies with a
...
Optional: `daisyclaw.setupEntry` can point at a lightweight setup-only module.
When DaisyClaw needs setup surfaces for a disabled channel plugin, or
...
Optional: `daisyclaw.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let DaisyClaw load the
...
Channel plugins can advertise setup/discovery metadata via `daisyclaw.channel` and
install hints via `daisyclaw.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@daisyclaw/nextcloud-talk",
  "daisyclaw": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@daisyclaw/nextcloud-talk",
...
Useful `daisyclaw.channel` fields beyond the minimal example:
...
DaisyClaw can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.daisyclaw/mpm/plugins.json`
- `~/.daisyclaw/mpm/catalog.json`
- `~/.daisyclaw/plugins/catalog.json`

Or point `DAISYCLAW_PLUGIN_CATALOG_PATHS` (or `DAISYCLAW_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "daisyclaw": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.

Generated channel catalog entries and provider install catalog entries expose
normalized install-source facts next to the raw `daisyclaw.install` block. The
...
2. add typed plugin registration/runtime surfaces
   Extend `DaisyClawPluginApi` and/or `api.runtime` with the smallest useful
...
This is how DaisyClaw stays opinionated without becoming hardcoded to one
