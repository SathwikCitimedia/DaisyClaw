---
summary: "Create your first DaisyClaw plugin in minutes"
title: "Building plugins"
sidebarTitle: "Getting Started"
doc-schema-version: 1
read_when:
  - You want to create a new DaisyClaw plugin
  - You need a quick-start for plugin development
  - You are choosing between channel, provider, CLI backend, tool, or hook docs
---

Plugins extend DaisyClaw without changing core. A plugin can add a messaging
channel, model provider, local CLI backend, agent tool, hook, media provider,
or another plugin-owned capability.

You do not need to add an external plugin to the DaisyClaw repository. Publish
the package to [ClawHub](/clawhub) and users install it with:

```bash
daisyclaw plugins install clawhub:<package-name>
```
...
- For in-repo bundled plugin work, clone the repository and run `pnpm install`.
  Source-checkout plugin development is pnpm-only because DaisyClaw loads bundled
...
<CardGroup cols={2}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect DaisyClaw to a messaging platform.
  </Card>
  ...
  <Card title="CLI backend plugin" icon="terminal" href="/plugins/cli-backend-plugins">
    Run a local AI CLI through DaisyClaw model fallback.
  </Card>
...
```json daisyclaw.plugin.json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds a custom tool to DaisyClaw",
  "contracts": {
    "tools": ["my_tool"]
  },
  "activation": {
...
```
    ...
    must appear in `contracts.tools` so DaisyClaw can discover ownership without
  ...
  <Step title="Test the runtime">
    ...
    ```bash
    daisyclaw plugins inspect my-plugin --runtime --json
    ```
    ...
    `daisyclaw demo-plugin ping`.

    For a bundled plugin in this repository, DaisyClaw discovers source-checkout
  ...
  <Step title="Install">
    ...
    ```bash
    daisyclaw plugins install clawhub:your-org/your-plugin
    ```
...
operator, installed plugin code, or a modified DaisyClaw runtime. Sensitive local
...
aligned with `api.registerTool(..., { optional: true })` so DaisyClaw can avoid
...
```typescript
import { definePluginEntry } from "daisyclaw/plugin-sdk";
```
...
`daisyclaw/plugin-sdk/gateway-method-runtime` bridge is reserved for plugin HTTP
...
<Check>**package.json** has correct `daisyclaw` metadata</Check>
<Check>**daisyclaw.plugin.json** manifest is present and valid</Check>
...
1. Watch for GitHub release tags on [daisyclaw/daisyclaw](https://github.com/daisyclaw/daisyclaw/releases) and subscribe via `Watch` > `Releases`. Beta tags look like `v2026.3.N-beta.1`. You can also turn on notifications for the official DaisyClaw X account [@daisyclaw](https://x.com/daisyclaw) for release announcements.
