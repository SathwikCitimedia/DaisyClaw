---
summary: "Install and use Codex, Claude, and Cursor bundles as DaisyClaw plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how DaisyClaw maps bundle content into native features
  - You are debugging bundle detection or missing capabilities
title: "Plugin bundles"
---

DaisyClaw can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
DaisyClaw maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native DaisyClaw plugins. Native plugins run
...
of requiring authors to rewrite them as native DaisyClaw plugins, DaisyClaw
...
<Steps>
  <Step title="Install from a directory, archive, or marketplace">
    ```bash
    # Local directory
    daisyclaw plugins install ./my-bundle

    # Archive
    daisyclaw plugins install ./my-bundle.tgz
...
## What DaisyClaw maps from bundles

Not every bundle feature runs in DaisyClaw today. Here is what works and what
...
| ------------- | ------------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal DaisyClaw skills                                                 | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                        | Claude, Cursor |
| Hook packs    | DaisyClaw-style `HOOK.md` + `handler.ts` layouts                                                   | Codex          |
| MCP tools     | Bundle MCP config merged into embedded DaisyClaw settings; supported stdio and HTTP servers loaded | All formats    |
| LSP servers   | Claude `.lsp.json` and manifest-declared `lspServers` merged into embedded DaisyClaw LSP defaults  | Claude         |
| Settings      | Claude `settings.json` imported as embedded DaisyClaw defaults                                     | Claude         |

#### Skill content

- bundle skill roots load as normal DaisyClaw skill roots
- Claude `commands` roots are treated as additional skill roots
- Cursor `.cursor/commands` roots are treated as additional skill roots

This means Claude markdown command files work through the normal DaisyClaw skill
loader. Cursor command markdown works through the same path.

#### Hook packs

- bundle hook roots work **only** when they use the normal DaisyClaw hook-pack
  layout. Today this is primarily the Codex-compatible case:
  - `HOOK.md`
  - `handler.ts` or `handler.js`

#### MCP for embedded DaisyClaw

- enabled bundles can contribute MCP server config
- DaisyClaw merges bundle MCP config into the effective embedded DaisyClaw settings as
  `mcpServers`
- DaisyClaw exposes supported bundle MCP tools during embedded DaisyClaw agent turns by
...
- `transport` may be set to `"streamable-http"` or `"sse"`; when omitted, DaisyClaw uses `sse`
- `type: "http"` is a CLI-native downstream shape; use `transport: "streamable-http"` in DaisyClaw config. `daisyclaw mcp set` and `daisyclaw doctor --fix` normalize the common alias.
...
DaisyClaw registers bundle MCP tools with provider-safe names in the form
...
#### Embedded DaisyClaw settings

- Claude `settings.json` is imported as default embedded DaisyClaw settings when the
  bundle is enabled
- DaisyClaw sanitizes shell override keys before applying them
...
#### Embedded DaisyClaw LSP

- enabled Claude bundles can contribute LSP server config
- DaisyClaw loads `.lsp.json` plus any manifest-declared `lspServers` paths
- bundle LSP config is merged into the effective embedded DaisyClaw LSP defaults
- only supported stdio-backed LSP servers are runnable today; unsupported
  transports still show up in `daisyclaw plugins inspect <id>`

### Detected but not executed

These are recognized and shown in diagnostics, but DaisyClaw does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit DaisyClaw best when they use skill roots and DaisyClaw-style
  ...
  <Accordion title="Claude bundles">
    ...
    - `settings.json` is imported into embedded DaisyClaw settings (shell override keys are sanitized)
    - `.mcp.json` exposes supported stdio tools to embedded DaisyClaw
    - `.lsp.json` plus manifest-declared `lspServers` paths load into embedded DaisyClaw LSP defaults
...
DaisyClaw checks for native plugin format first:

1. `daisyclaw.plugin.json` or valid `package.json` with `daisyclaw.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, DaisyClaw uses the native path. This prevents
...
- Third-party compatible bundles do not get startup `npm install` repair. They
  should be installed through `daisyclaw plugins install` and ship everything
  they need in the installed plugin directory.
- DaisyClaw-owned bundled plugins are either shipped lightweight in core or
  downloadable through the plugin installer. Gateway startup never runs a
  package manager for them.
- `daisyclaw doctor --fix` removes legacy staged dependency directories and can
...
- DaisyClaw does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `daisyclaw plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded DaisyClaw settings from `settings.json` are supported. DaisyClaw does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    DaisyClaw hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
