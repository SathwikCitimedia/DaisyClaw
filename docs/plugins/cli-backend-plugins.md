...
read_when:
  ...
  - You need to map a third-party CLI into DaisyClaw's text fallback runner
---

CLI backend plugins let DaisyClaw call a local AI CLI as a text inference
...
| -------------------- | ---------------------- | --------------------------------------------------------- |
| Package entry        | `package.json`         | Points DaisyClaw at the plugin runtime module              |
| Manifest ownership   | `daisyclaw.plugin.json` | Declares the backend id before runtime loads              |
...
<Steps>
  <Step title="Create package metadata">
    ...
    entry is `./src/index.ts`, add `daisyclaw.runtimeExtensions` that points at
  ...
  <Step title="Declare backend ownership">
    ```json daisyclaw.plugin.json
    {
      "id": "acme-cli",
      "name": "Acme CLI",
      "description": "Run Acme's local AI CLI through DaisyClaw",
    ...
    `cliBackends` is the runtime ownership list. It lets DaisyClaw auto-load the
  ...
  <Step title="Register the backend">
    ...
    } from "daisyclaw/plugin-sdk/cli-backend";
...
`CliBackendConfig` describes how DaisyClaw should launch and parse the CLI:
...
| `modelAliases`                            | Map DaisyClaw model ids to CLI-native ids                    |
| `sessionArg` / `sessionArgs`              | How to pass a session id                                    |
| `sessionMode`                             | `always`, `existing`, or `none`                             |
| `sessionIdFields`                         | JSON fields DaisyClaw reads from CLI output                  |
...
| `defaultAuthProfileId`             | Prefer a specific DaisyClaw auth profile                |
| `authEpochMode`                    | Decide how auth changes invalidate stored CLI sessions |
| `nativeToolMode`                   | Declare whether the CLI has always-on native tools     |
| `bundleMcp` / `bundleMcpMode`      | Opt into DaisyClaw's loopback MCP tool bridge           |
| `ownsNativeCompaction`             | Backend owns its own compaction - DaisyClaw defers      |

Keep these hooks provider-owned. Do not add CLI-specific branches to core when a
backend hook can express the behavior.

### `ownsNativeCompaction`: opting out of DaisyClaw compaction

If your backend runs an agent that compacts its **own** transcript, set
`ownsNativeCompaction: true` so DaisyClaw's safeguard summarizer never runs against its
...
stay over budget / go stale (DaisyClaw no longer rescues it):
...
CLI backends do not receive DaisyClaw tools by default. If the CLI can consume an
...
"always-on"` so DaisyClaw can fail closed when a caller requires no native tools.
...
```bash
daisyclaw plugins inspect acme-cli --runtime --json
daisyclaw agent --message "reply exactly: backend ok" --model acme-cli/acme-large
```
...
<Check>`package.json` has `daisyclaw.extensions` and built runtime entries for published packages</Check>
<Check>`daisyclaw.plugin.json` declares `cliBackends` and intentional `activation.onStartup`</Check>
