---
summary: "Set up Codex Computer Use for Codex-mode DaisyClaw agents"
title: "Codex Computer Use"
read_when:
  - You want Codex-mode DaisyClaw agents to use Codex Computer Use
...
Computer Use is a Codex-native MCP plugin for local desktop control. DaisyClaw
...
Use this page when DaisyClaw is already using the native Codex harness. For the
runtime setup itself, see [Codex harness](/plugins/codex-harness).

## DaisyClaw.app and Peekaboo

DaisyClaw.app's Peekaboo integration is separate from Codex Computer Use. The
...
Use [Peekaboo bridge](/platforms/mac/peekaboo) when you want DaisyClaw.app to be
a permission-aware host for Peekaboo CLI automation. Use this page when a
Codex-mode DaisyClaw agent should have Codex's native `computer-use` MCP plugin
...
Instead, the iOS app connects as an DaisyClaw node and exposes mobile
...
DaisyClaw-managed runtimes to call TryCua's driver directly, use the upstream
`cua-driver mcp` server through DaisyClaw's MCP registry instead of the
Codex-specific marketplace flow.

After installing `cua-driver`, either ask it for the DaisyClaw command:

```bash
cua-driver mcp-config --client daisyclaw
```
...
available as a normal DaisyClaw MCP server. Use the Codex Computer Use setup on
...
that its app prompts for, such as Accessibility and Screen Recording. DaisyClaw
...
Computer Use in and lets DaisyClaw install or re-enable it before the turn:
...
With this config, DaisyClaw checks Codex app-server before each Codex-mode turn.
If Computer Use is missing but Codex app-server has already discovered an
installable marketplace, DaisyClaw asks Codex app-server to install or re-enable
the plugin and reload MCP servers. On macOS, when no matching marketplace is
registered and the standard Codex app bundle exists, DaisyClaw also tries to
...
plugin command surface is available. These are DaisyClaw chat/runtime commands,
not `daisyclaw codex ...` CLI subcommands:
...
DaisyClaw uses the same app-server API that Codex itself exposes. The
...
During install, DaisyClaw polls `plugin/list` for up to
`marketplaceDiscoveryTimeoutMs` milliseconds. The default is 60 seconds.

If multiple known marketplaces contain Computer Use, DaisyClaw prefers
...
`computer-use` is registered, DaisyClaw tries to add the standard bundled
...
## What DaisyClaw checks

DaisyClaw reports a stable setup reason internally and formats the user-facing
status for chat:
...
permissions before it can inspect or control apps. If DaisyClaw says Computer Use
...
DaisyClaw intentionally fails closed when `computerUse.enabled` is true. A
...
fresh DaisyClaw session.

**A Computer Use tool says `Native hook relay unavailable`.** The Codex-native
tool hook could not reach an active DaisyClaw relay through the local bridge or
Gateway fallback. Start a fresh DaisyClaw session with `/new` or `/reset`. If it
works once and then fails again on a later tool call, `/new` is only clearing the
current attempt; restart the Codex app-server or DaisyClaw Gateway so old threads
