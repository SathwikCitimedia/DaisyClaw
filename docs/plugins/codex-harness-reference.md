...
By default, DaisyClaw starts the managed Codex binary shipped with the bundled
plugin:
...
| `clearEnv`                                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after DaisyClaw builds its inherited environment.                                                                                                                                                                                                                              |
...
| `sandbox`                                     | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start and resume. Active DaisyClaw sandboxes narrow `danger-full-access` turns to Codex `workspace-write`; the turn network flag follows DaisyClaw sandbox egress.                                                                                                                                                        |
...
unattended DaisyClaw turns and heartbeats make progress without native approval
prompts that nobody is around to answer.

If Codex's local system requirements file disallows implicit YOLO approval,
reviewer, or sandbox values, DaisyClaw treats the implicit default as guardian
...
When an DaisyClaw sandbox is active, the local Codex app-server process still
runs on the Gateway host. DaisyClaw therefore disables Codex native Code Mode,
user MCP servers, and app-backed plugin execution for that turn instead of
treating Codex host-side sandboxing as equivalent to the DaisyClaw sandbox
backend. Shell access is exposed through DaisyClaw sandbox-backed dynamic tools
...
`workspace-write` without active DaisyClaw sandboxing. If you see
`bwrap: setting up uid map: Permission denied` or
`bwrap: loopback: Failed RTM_NEWADDR: Operation not permitted`, run
`daisyclaw doctor` and fix the reported host namespace policy for the DaisyClaw
...
The stable default is fail-closed: active DaisyClaw sandboxing disables native
...
try Codex's remote environment support with DaisyClaw's sandbox backend. This
...
When the flag is on and the current DaisyClaw session is sandboxed, DaisyClaw
...
DaisyClaw-owned environment. If the app-server cannot register the environment,
...
loopback exec-server unless it is running on the same host, so DaisyClaw rejects
that combination.
...
1. An explicit DaisyClaw Codex auth profile for the agent.
...
Stdio app-server launches inherit DaisyClaw's process environment by default.
DaisyClaw owns the Codex app-server account bridge and sets `CODEX_HOME` to a
per-agent directory under that agent's DaisyClaw state. That keeps Codex config,
accounts, plugin cache/data, and thread state scoped to the DaisyClaw agent
instead of leaking in from the operator's personal `~/.codex` home.

DaisyClaw does not rewrite `HOME` for normal local app-server launches. Codex-run
subprocesses such as `daisyclaw`, `gh`, `git`, cloud CLIs, and shell commands see
...
DaisyClaw plugins and DaisyClaw skill snapshots still flow through DaisyClaw's own
...
part of an DaisyClaw agent, inventory them explicitly:

```bash
daisyclaw migrate codex --dry-run
daisyclaw migrate apply codex --yes
```
...
Most remaining DaisyClaw integration tools, such as messaging, media, cron,
browser, nodes, gateway, `heartbeat_respond`, and `web_search`, are available
through Codex tool search under the `daisyclaw` namespace. This keeps the initial
...
surface, while explicit DaisyClaw or ACP delegation is still available through
the `daisyclaw` dynamic tool namespace.
...
Dynamic tool budgets are capped at 600000 ms. On timeout, DaisyClaw aborts the
...
post-tool idle budget: after DaisyClaw returns an `item/tool/call` response, after
...
`turn/completed`, DaisyClaw best-effort interrupts the native turn and releases
...
availability is owned by Codex app-server, so the list can change when DaisyClaw
...
If discovery fails or times out, DaisyClaw uses a bundled fallback catalog for:
...
Codex handles `AGENTS.md` itself through native project-doc discovery. DaisyClaw
...
For DaisyClaw workspace parity, the Codex harness resolves the other bootstrap
files. `SOUL.md`, `IDENTITY.md`, `TOOLS.md`, and `USER.md` are forwarded as
DaisyClaw Codex developer instructions because they define the active agent,
available workspace guidance, and user profile. The compact DaisyClaw skills
...
`BOOTSTRAP.md` when present is forwarded as DaisyClaw turn input reference
context.
