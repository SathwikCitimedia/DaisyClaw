...
read_when:
  ...
  - You are changing plugin behavior across DaisyClaw and Codex harness turns
---
...
Codex mode is not DaisyClaw with a different model call underneath. Codex owns more of
the native model loop, and DaisyClaw adapts its plugin, tool, session, and
diagnostic surfaces around that boundary.

DaisyClaw still owns channel routing, session files, visible message delivery,
DaisyClaw dynamic tools, approvals, media delivery, and a transcript mirror.
...
explicit DaisyClaw compatibility route keeps the normal DaisyClaw system prompt even
...
according to the active Codex thread config. DaisyClaw starts and resumes native
Codex threads with Codex's built-in personality disabled so workspace
personality files and DaisyClaw agent identity stay authoritative. Lightweight
DaisyClaw runs still preserve their existing project-doc suppression. DaisyClaw
developer instructions cover DaisyClaw runtime concerns such as source-channel
delivery, DaisyClaw dynamic tools, ACP delegation, adapter context, and the
active agent workspace profile files. DaisyClaw skill catalogs and tool-routed
...
When an DaisyClaw session is attached to an existing Codex thread, the next turn
...
normally, and DaisyClaw posts the final text to the source conversation. Set
...
Codex heartbeat turns also get `heartbeat_respond` in the searchable DaisyClaw
...
| Layer                                 | Owner                    | Purpose                                                             |
| ------------------------------------- | ------------------------ | ------------------------------------------------------------------- |
| DaisyClaw plugin hooks                 | DaisyClaw                 | Product/plugin compatibility across DaisyClaw and Codex harnesses.   |
| Codex app-server extension middleware | DaisyClaw bundled plugins | Per-turn adapter behavior around DaisyClaw dynamic tools.            |
| Codex native hooks                    | Codex                    | Low-level Codex lifecycle and native tool policy from Codex config. |

DaisyClaw does not use project or global Codex `hooks.json` files to route
DaisyClaw plugin behavior. For the supported native tool and permission bridge,
DaisyClaw injects per-thread Codex config for `PreToolUse`, `PostToolUse`,
...
Codex's app-server reviewer and DaisyClaw's approval bridge handle real
...
Codex-level controls. They are not exposed as DaisyClaw plugin hooks in the v1
contract.

For DaisyClaw dynamic tools, DaisyClaw executes the tool after Codex asks for the
call, so DaisyClaw fires the plugin and middleware behavior it owns in the
harness adapter. For Codex-native tools, Codex owns the canonical tool record.
DaisyClaw can mirror selected events, but it cannot rewrite the native Codex
...
to the matching app-server approval. If an DaisyClaw `before_tool_call` hook
returns `requireApproval` while the native payload sets report approval mode
(`daisyclaw_approval_mode` is `"report"`), the native hook relay records the
plugin approval requirement and returns no native decision. When Codex sends the
app-server approval request for the same tool use, DaisyClaw opens the plugin
approval prompt and maps the decision back to Codex. Codex `PermissionRequest`
events are a separate approval path and can still route through DaisyClaw
...
notifications and DaisyClaw adapter state, not native Codex hook commands.
DaisyClaw's `before_compaction`, `after_compaction`, `llm_input`, and
...
They do not invoke DaisyClaw plugin hooks.
...
| DaisyClaw channel routing and delivery         | Supported                                                                        | Telegram, Discord, Slack, WhatsApp, iMessage, and other channels stay outside the model runtime.                                                                                                                                                                                                                                                                                                                                                                                    |
| DaisyClaw dynamic tools                        | Supported                                                                        | Codex asks DaisyClaw to execute these tools, so DaisyClaw stays in the execution path.                                                                                                                                                                                                                                                                                                                                                                                                |
| Prompt and context plugins                    | Supported                                                                        | DaisyClaw projects DaisyClaw-specific prompt/context into the Codex turn while leaving Codex-owned base, model, and configured project-doc prompts in the native Codex lane. DaisyClaw disables Codex's built-in personality for native threads so agent workspace personality files remain authoritative. Native Codex developer instructions accept only command guidance explicitly scoped to `codex_app_server`; legacy global command hints remain for non-Codex prompt surfaces. |
...
| Dynamic tool hooks                            | Supported                                                                        | `before_tool_call`, `after_tool_call`, and tool-result middleware run around DaisyClaw-owned dynamic tools.                                                                                                                                                                                                                                                                                                                                                                          |
...
| Native permission policy                      | Supported through Codex app-server approvals and compatibility native hook relay | Codex app-server approval requests route through DaisyClaw after Codex review. The `PermissionRequest` native hook relay is opt-in for native approval modes because Codex emits it before guardian review.                                                                                                                                                                                                                                                                          |
| App-server trajectory capture                 | Supported                                                                        | DaisyClaw records the request it sent to app-server and the app-server notifications it receives.                                                                                                                                                                                                                                                                                                                                                                                    |

Not supported in Codex runtime v1:

| Surface                                             | V1 boundary                                                                                                                                     | Future path                                                                               |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Native tool argument mutation                       | Codex native pre-tool hooks can block, but DaisyClaw does not rewrite Codex-native tool arguments.                                               | Requires Codex hook/schema support for replacement tool input.                            |
| Editable Codex-native transcript history            | Codex owns canonical native thread history. DaisyClaw owns a mirror and can project future context, but should not mutate unsupported internals. | Add explicit Codex app-server APIs if native thread surgery is needed.                    |
| `tool_result_persist` for Codex-native tool records | That hook transforms DaisyClaw-owned transcript writes, not Codex-native tool records.                                                           | Could mirror transformed records, but canonical rewrite needs Codex support.              |
| Rich native compaction metadata                     | DaisyClaw can request native compaction, but does not receive a stable kept/dropped list, token delta, completion summary, or summary payload.   | Needs richer Codex compaction events.                                                     |
| Compaction intervention                             | DaisyClaw does not let plugins or context engines veto, rewrite, or replace native Codex compaction.                                             | Add Codex pre/post compaction hooks if plugins need to veto or rewrite native compaction. |
| Byte-for-byte model API request capture             | DaisyClaw can capture app-server requests and notifications, but Codex core builds the final OpenAI API request internally.                      | Needs a Codex model-request tracing event or debug API.                                   |

## Native permissions and MCP elicitations

For `PermissionRequest`, DaisyClaw only returns explicit allow or deny decisions
...
DaisyClaw remembers that exact provider/session/tool input/cwd fingerprint for a
...
Codex MCP tool approval elicitations are routed through DaisyClaw's plugin
...
default `messages.queue.mode: "steer"`, DaisyClaw batches steer-mode chat
...
case, DaisyClaw waits for the active run to finish before starting the prompt.
...
harness, DaisyClaw also calls Codex app-server `feedback/upload` for relevant
...
error. The completed diagnostics reply lists the channels, DaisyClaw session ids,
Codex thread ids, and local `codex resume <thread-id>` commands for the threads
that were sent.

If you deny or ignore the approval, DaisyClaw does not print those Codex ids and
...
to Codex app-server. DaisyClaw does not run preflight compaction for Codex turns,
does not replace Codex compaction with context-engine compaction, and does not
fall back to DaisyClaw or public OpenAI summarization when native Codex
compaction cannot be started. DaisyClaw keeps a transcript mirror for channel
...
DaisyClaw returns after starting that native operation. It does not wait for
completion, impose a separate DaisyClaw timeout, restart the shared Codex
app-server, or record the operation as an DaisyClaw-completed compaction.

When a context engine requests Codex thread-bootstrap projection, DaisyClaw
...
reasoning or plan records when the app-server emits them. Today, DaisyClaw only
...
DaisyClaw is writing an DaisyClaw-owned session transcript tool result.

## Media and delivery

DaisyClaw continues to own media delivery and media provider selection. Image,
...
through the normal DaisyClaw delivery path. Media generation does not require the legacy runtime.
When Codex emits a native image-generation item with a `savedPath`, DaisyClaw
