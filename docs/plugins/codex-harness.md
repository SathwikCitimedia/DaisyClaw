---
summary: "Run DaisyClaw embedded agent turns through the bundled Codex app-server harness"
title: "Codex harness"
read_when:
  - You want to use the bundled Codex app-server harness
  - You need Codex harness config examples
  - You want Codex-only deployments to fail instead of falling back to DaisyClaw
---

The bundled `codex` plugin lets DaisyClaw run embedded OpenAI agent turns
through Codex app-server instead of the built-in DaisyClaw harness.
...
app-server execution. DaisyClaw still owns chat channels, session files, model
selection, DaisyClaw dynamic tools, approvals, media delivery, and the visible
transcript mirror.
...
When no DaisyClaw sandbox is active, DaisyClaw starts Codex app-server threads
...
DaisyClaw dynamic tools continue through the app-server `item/tool/call` bridge.
Active DaisyClaw sandboxing and restricted tool policies disable native code mode
entirely unless you opt into the experimental sandbox exec-server path.

This Codex-native feature is separate from
[DaisyClaw code mode](/reference/code-mode), which is an opt-in QuickJS-WASI
runtime for generic DaisyClaw runs with a different `exec` input shape.
...
- DaisyClaw with the bundled `codex` plugin available.
...
- Codex auth available through `daisyclaw models auth login --provider openai`,
...
Most users who want Codex in DaisyClaw want this path: sign in with a
...
```bash
daisyclaw models auth login --provider openai
```
...
harness options in DaisyClaw config, and use the CLI only for Codex auth:

| Need                                   | Set                                                                              | Where                              |
| -------------------------------------- | -------------------------------------------------------------------------------- | ---------------------------------- |
| Enable the harness                     | `plugins.entries.codex.enabled: true`                                            | DaisyClaw config                    |
| Keep an allowlisted plugin install     | Include `codex` in `plugins.allow`                                               | DaisyClaw config                    |
| Route OpenAI agent turns through Codex | `agents.defaults.model` or `agents.list[].model` as `openai/gpt-*`               | DaisyClaw agent config              |
| Sign in with ChatGPT/Codex OAuth       | `daisyclaw models auth login --provider openai`                                   | CLI auth profile                   |
| Add API-key backup for Codex runs      | `openai:*` API-key profile listed after subscription auth in `auth.order.openai` | CLI auth profile + DaisyClaw config |
| Fail closed when Codex is unavailable  | Provider or model `agentRuntime.id: "codex"`                                     | DaisyClaw model/provider config     |
| Use direct OpenAI API traffic          | Provider or model `agentRuntime.id: "daisyclaw"` with normal OpenAI auth          | DaisyClaw model/provider config     |
...
Codex compacts through its native app-server thread state, so DaisyClaw ignores
those local summarizer overrides at runtime and `daisyclaw doctor --fix` removes
...
`agents.defaults.compaction.provider`. `daisyclaw doctor --fix` migrates the old
...
the bound thread. DaisyClaw does not wait for completion, impose an DaisyClaw
...
turns. The API key is only an auth fallback, not a request to switch to DaisyClaw or
...
- Do not use legacy Codex GPT refs in config. Run `daisyclaw doctor --fix` to
...
- `agentRuntime.id: "daisyclaw"` opts a provider or model into the DaisyClaw
...
| Direct OpenAI API-key traffic through DaisyClaw       | Provider or model `agentRuntime.id: "daisyclaw"` and normal OpenAI auth | `/status` shows DaisyClaw runtime        | Use only when DaisyClaw is intentional |
| Legacy config                                        | legacy Codex GPT refs                                                  | `daisyclaw doctor --fix` rewrites it     | Do not write new config this way      |
...
With Codex forced, DaisyClaw fails early if the Codex plugin is disabled, the
app-server is too old, or the app-server cannot start.

## App-server policy

By default, the plugin starts DaisyClaw's managed Codex binary locally with stdio
...
implicit YOLO posture, DaisyClaw selects allowed guardian permissions instead.
When an DaisyClaw sandbox is active for the session, DaisyClaw disables Codex
...
through DaisyClaw sandbox-backed dynamic tools such as `sandbox_exec` and
`sandbox_process` when the normal exec/process tools are available.

Use normalized DaisyClaw exec mode when you want Codex native auto-review before
...
For Codex app-server sessions, DaisyClaw maps `tools.exec.mode: "auto"` to Codex
...
In `tools.exec.mode: "auto"`, DaisyClaw does not preserve legacy unsafe Codex
...
works, but `tools.exec.mode: "auto"` is the normalized DaisyClaw surface.
...
supports DaisyClaw text commands.
...
- `/codex resume <thread-id>` attaches the current DaisyClaw session to an
...
1. Ordered OpenAI auth profiles for the agent, preferably under
   `auth.order.openai`. Run `daisyclaw doctor --fix` to migrate older
...
When DaisyClaw sees a ChatGPT subscription-style Codex auth profile, it removes
...
If a subscription profile hits a Codex usage limit, DaisyClaw records the reset
...
For local stdio app-server launches, DaisyClaw sets `CODEX_HOME` to a per-agent
...
default. DaisyClaw preserves the normal process `HOME`; Codex-run subprocesses
...
DaisyClaw removes `CODEX_HOME` and `HOME` from this list during local launch
normalization: `CODEX_HOME` stays per-agent, and `HOME` stays inherited so
subprocesses can use normal user-home state.

Codex dynamic tools default to `searchable` loading. DaisyClaw does not expose
...
DaisyClaw integration tools such as messaging, media, cron, browser, nodes,
gateway, `heartbeat_respond`, and `web_search` are available through Codex tool
search under the `daisyclaw` namespace, keeping the initial model context
smaller.
...
DaisyClaw or ACP delegation is still available through the `daisyclaw` dynamic
...
| -------------------------- | -------------- | ---------------------------------------------------------------------------------------- |
| `codexDynamicToolsLoading` | `"searchable"` | Use `"direct"` to put DaisyClaw dynamic tools directly in the initial Codex tool context. |
| `codexDynamicToolsExclude` | `[]`           | Additional DaisyClaw dynamic tool names to omit from Codex app-server turns.              |
...
| `clearEnv`                                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after DaisyClaw builds its inherited environment. DaisyClaw keeps per-agent `CODEX_HOME` and inherited `HOME` for local launches.                                                                                                                                               |
| `codeModeOnly`                                | `false`                                                | Opt into Codex's code-mode-only tool surface. DaisyClaw dynamic tools remain registered with Codex so nested `tools.*` calls return through the app-server `item/tool/call` bridge.                                                                                                                                                                               |
| `requestTimeoutMs`                            | `60000`                                                | Timeout for app-server control-plane calls.                                                                                                                                                                                                                                                                                                                      |
| `turnCompletionIdleTimeoutMs`                 | `60000`                                                | Quiet window after Codex accepts a turn or after a turn-scoped app-server request while DaisyClaw waits for `turn/completed`.                                                                                                                                                                                                                                     |
| `postToolRawAssistantCompletionIdleTimeoutMs` | `300000`                                               | Completion-idle and progress guard used after a tool handoff, native tool completion, post-tool raw assistant progress, raw reasoning completion, or reasoning progress while DaisyClaw waits for `turn/completed`. Use this for trusted or heavy workloads where post-tool synthesis can legitimately stay quiet longer than the final assistant release budget. |
...
| `sandbox`                                     | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start/resume. Guardian defaults prefer `"workspace-write"` when allowed, otherwise `"read-only"`. When an DaisyClaw sandbox is active, `danger-full-access` turns use Codex `workspace-write` with network access derived from the DaisyClaw sandbox egress setting.                                                      |
| `approvalsReviewer`                           | `"user"` or an allowed guardian reviewer               | Use `"auto_review"` to let Codex review native approval prompts when allowed, otherwise `guardian_subagent` or `user`. `guardian_subagent` remains a legacy alias.                                                                                                                                                                                               |
| `serviceTier`                                 | unset                                                  | Optional Codex app-server service tier. `"priority"` enables fast-mode routing, `"flex"` requests flex processing, `null` clears the override, and legacy `"fast"` is accepted as `"priority"`.                                                                                                                                                                  |
| `experimental.sandboxExecServer`              | `false`                                                | Preview opt-in that registers an DaisyClaw sandbox-backed Codex environment with Codex app-server 0.132.0 or newer so native Codex execution can run inside the active DaisyClaw sandbox.                                                                                                                                                                          |

DaisyClaw-owned dynamic tool calls are bounded independently from
`appServer.requestTimeoutMs`: Codex `item/tool/call` requests use a 90 second
DaisyClaw watchdog by default. A positive per-call `timeoutMs` argument extends
...
budgets are capped at 600000 ms. On timeout, DaisyClaw aborts the tool signal
...
After Codex accepts a turn, and after DaisyClaw responds to a turn-scoped
...
quiet for `appServer.turnCompletionIdleTimeoutMs`, DaisyClaw best-effort
interrupts the Codex turn, records a diagnostic timeout, and releases the
DaisyClaw session lane so follow-up chat messages are not queued behind a stale
...
longer post-tool idle budget: after DaisyClaw returns an `item/tool/call`
...
without `turn/completed`, DaisyClaw best-effort interrupts the native turn and
...
timeouts still retire the stuck app-server client and release the DaisyClaw
...
- `DAISYCLAW_CODEX_APP_SERVER_BIN`
- `DAISYCLAW_CODEX_APP_SERVER_ARGS`
- `DAISYCLAW_CODEX_APP_SERVER_MODE=yolo|guardian`
- `DAISYCLAW_CODEX_APP_SERVER_APPROVAL_POLICY`
- `DAISYCLAW_CODEX_APP_SERVER_SANDBOX`

`DAISYCLAW_CODEX_APP_SERVER_BIN` bypasses the managed binary when
`appServer.command` is unset.

`DAISYCLAW_CODEX_APP_SERVER_GUARDIAN=1` was removed. Use
`plugins.entries.codex.config.appServer.mode: "guardian"` instead, or
`DAISYCLAW_CODEX_APP_SERVER_MODE=guardian` for one-off local testing. Config is
...
capabilities in the same Codex thread as the DaisyClaw harness turn. DaisyClaw
does not translate Codex plugins into synthetic `codex_plugin_*` DaisyClaw
dynamic tools.
...
Thread app config is computed when DaisyClaw establishes a Codex harness session
...
The short version: DaisyClaw does not vendor the desktop-control app or execute
...
- DaisyClaw dynamic tools are supported. Codex asks DaisyClaw to execute those
  tools, so DaisyClaw remains in the execution path.
- Codex-native shell, patch, MCP, and native app tools are owned by Codex.
  DaisyClaw can observe or block selected native events through the supported
  relay, but it does not rewrite native tool arguments.
- Codex owns native compaction. DaisyClaw keeps a transcript mirror for channel
  history, search, `/new`, `/reset`, and future model or harness switching, but
  it does not replace Codex compaction with an DaisyClaw or context-engine
  summarizer.
- Media generation, media understanding, TTS, approvals, and messaging-tool
  output continue through the matching DaisyClaw provider/model settings.
- `tool_result_persist` applies to DaisyClaw-owned transcript tool results, not
...
**DaisyClaw uses the built-in harness instead of Codex:** make sure the model ref is
...
Ask affected collaborators to run this read-only command on their DaisyClaw host:

```bash
(
  pattern='openai/gpt-5\.[45]|openai[-]codex|agentRuntime(\.id)?|harnessRuntime|Runtime: OpenAI Codex|legacy OpenAI Codex prefix|resolveSelectedOpenAIRuntimeProvider|candidateProvider[": ]+openai|status[": ]+401|Incorrect API key|No API key|api-key path|API-key path|OAuth'

  if ls /tmp/daisyclaw/daisyclaw-*.log >/dev/null 2>&1; then
    grep -E -i -n "$pattern" /tmp/daisyclaw/daisyclaw-*.log 2>/dev/null || true
  else
    journalctl --user -u daisyclaw-gateway --since today --no-pager 2>/dev/null \
...
**Legacy Codex model refs config remains:** run `daisyclaw doctor --fix`.
...
`0.125.0-alpha.2` or `0.125.0+custom` are rejected because DaisyClaw tests the
...
the Codex thread is still trying to use a native hook relay id that DaisyClaw no
...
app-server or DaisyClaw Gateway so old threads are dropped and native hook
