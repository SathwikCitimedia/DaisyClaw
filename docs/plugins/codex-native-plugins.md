---
summary: "Configure migrated native Codex plugins for Codex-mode DaisyClaw agents"
title: "Native Codex plugins"
read_when:
  - You want Codex-mode DaisyClaw agents to use native Codex plugins
  - You are migrating source-installed openai-curated Codex plugins
  - You are troubleshooting codexPlugins, app inventory, destructive actions, or plugin app diagnostics
---

Native Codex plugin support lets a Codex-mode DaisyClaw agent use Codex
app-server's own app and plugin capabilities inside the same Codex thread that
handles the DaisyClaw turn.

DaisyClaw does not translate Codex plugins into synthetic `codex_plugin_*`
DaisyClaw dynamic tools. Plugin calls stay in the native Codex transcript, and
...
- The selected DaisyClaw agent runtime must be the native Codex harness.
...
`codexPlugins` has no effect on DaisyClaw runs, normal OpenAI provider runs, ACP
...
```bash
daisyclaw migrate codex --dry-run --verify-plugin-apps
```
...
`enable` and `disable` write only to DaisyClaw config at
`~/.daisyclaw/daisyclaw.json`; they do not edit `~/.codex/config.toml` or install
...
- Enabled: DaisyClaw config is willing to make the plugin available to Codex
  harness turns.
...
Migration is the durable install/eligibility step. During planning, DaisyClaw
...
DaisyClaw reads Codex app inventory through app-server `app/list`, caches it for
one hour, and refreshes stale or missing entries asynchronously. The cache is
in memory only; restarting the CLI or gateway drops it, and DaisyClaw rebuilds it
...
A plugin app is exposed only when DaisyClaw can map it back to the migrated
...
DaisyClaw injects a restrictive `config.apps` patch for the Codex thread:
`_default` is disabled and only apps owned by enabled migrated plugins are
enabled.

DaisyClaw sets app-level `destructive_enabled` from the effective global or
...
are emitted with `open_world_enabled: true`; DaisyClaw does not expose a separate
...
- Per-plugin `allow_destructive_actions` overrides the global policy for that
  plugin.
- When policy is `false`, DaisyClaw returns a deterministic decline.
- When policy is `true`, DaisyClaw auto-accepts only safe schemas it can map to
...
empty or stale cache. DaisyClaw schedules an async refresh and excludes plugin
...
Codex thread bindings keep the app config they started with until DaisyClaw
