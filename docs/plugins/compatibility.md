...
read_when:
  - You maintain an DaisyClaw plugin
  - You see a plugin compatibility warning
  - You are planning a plugin SDK or manifest migration
---

DaisyClaw keeps older plugin contracts wired through named compatibility
...
The plugin inspector should live outside the core DaisyClaw repo as a separate
...
```sh
daisyclaw-plugin-inspector ./my-plugin
```
...
Use `--json` for stable machine-readable output in CI annotations. DaisyClaw
core should expose contracts and fixtures the inspector can consume, but should
not publish the inspector binary from the main `daisyclaw` package.

### Maintainer acceptance lane

Use Crabbox-backed Blacksmith Testbox for the installable-package acceptance
lane when validating the external inspector against DaisyClaw plugin packages.
Run it from a clean DaisyClaw checkout after the package is built:

```sh
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "pnpm install && pnpm build && npm exec --yes @daisyclaw/plugin-inspector@0.1.0 -- ./extensions/telegram --json"
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "npm exec --yes @daisyclaw/plugin-inspector@0.1.0 -- ./extensions/discord --json"
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "npm exec --yes @daisyclaw/plugin-inspector@0.1.0 -- <clawhub-plugin-dir> --json"
```
...
DaisyClaw should not remove a documented plugin contract in the same release
...
- legacy broad SDK imports such as `daisyclaw/plugin-sdk/compat`
...
- legacy `activate(api)` plugin entrypoints while plugins migrate to
  `register(api)`
- legacy SDK aliases such as `daisyclaw/extension-api`,
  `daisyclaw/plugin-sdk/channel-runtime`, `daisyclaw/plugin-sdk/command-auth`
  status builders, `daisyclaw/plugin-sdk/test-utils` (replaced by focused
  `daisyclaw/plugin-sdk/*` test subpaths), and the `ClawdbotConfig` /
  `DaisyClawSchemaType` type aliases
...
- legacy channel route key and comparable-target helper aliases while plugins
  move to `daisyclaw/plugin-sdk/channel-route`
...
- persisted plugin registry disable and install-migration env flags while
  repair flows migrate operators to `daisyclaw plugins registry --refresh` and
