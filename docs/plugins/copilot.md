---
summary: "Run DaisyClaw embedded agent turns through the external GitHub Copilot SDK harness"
...
read_when:
  ...
  - You are wiring an agent to subscription Copilot (github / daisyclaw / copilot) and want it to run through the Copilot CLI
---

The external `@daisyclaw/copilot` plugin lets DaisyClaw run embedded subscription
...
DaisyClaw still owns chat channels, session files, model selection, DaisyClaw
...
[Side questions (`/btw`)](#side-questions-btw)), and `daisyclaw doctor`.
...
- DaisyClaw with the `@daisyclaw/copilot` plugin installed.
- If your config uses `plugins.allow`, include `copilot` (the manifest
  id declared by the plugin). A restrictive
  allowlist that uses the npm-style `@daisyclaw/copilot` package name
...
- A writable `copilotHome` directory. The harness defaults to
  `~/.daisyclaw/agents/<agentId>/copilot` for full per-agent isolation. The
...
`daisyclaw doctor` runs the plugin
...
The Copilot runtime is an external plugin so the core `daisyclaw` package does
...
```bash
daisyclaw plugins install @daisyclaw/copilot
```
...
Without the opt-in, daisyclaw uses its built-in GitHub Copilot provider
and never installs the runtime plugin.

The runtime resolves the SDK in this order:

1. `import("@github/copilot-sdk")` from the installed `@daisyclaw/copilot`
   package.
2. The well-known fallback dir `~/.daisyclaw/npm-runtime/copilot/` (the
...
4. **Env-var fallback** for direct CLI / dogfood runs where no auth
   ...
   setup:
   1. `DAISYCLAW_GITHUB_TOKEN` -- harness-specific override; set this
      to pin a token for the DaisyClaw harness without disturbing
...
(isolating SDK state from DaisyClaw's `models.json` / `auth-profiles.json` in
the same directory), or `~/.daisyclaw/agents/<agentId>/copilot` otherwise.
...
- `model` — string or `{ provider, id, api? }`. When omitted, DaisyClaw uses
...
- `reasoningEffort` — `"low" | "medium" | "high" | "xhigh"`. Maps from
  DaisyClaw's `ThinkLevel` / `ReasoningLevel` resolution in
...
- `infiniteSessionConfig` — optional override for the SDK
  ...
  leave as-is.
- `hooksConfig` — optional bridge config exposing DaisyClaw
...
- `permissionPolicy` — optional override for the SDK's
  ...
  invokes any of those kinds because every bridged DaisyClaw tool is
  registered with `overridesBuiltInTool: true` and
  `skipPermission: true` so 100% of tool calls flow through DaisyClaw's
...
Nothing in the rest of DaisyClaw needs to know about these fields. Other
...
The DaisyClaw side transcript mirror (see below) continues to receive the
...
DaisyClaw audit transcript via
...
deliberately leaves `harness.runSideQuestion` undefined, so DaisyClaw's `/btw`
...
asserted in
[`extensions/copilot/harness.test.ts`](https://github.com/daisyclaw/daisyclaw/blob/main/extensions/copilot/harness.test.ts)
...
that hosts (including `daisyclaw doctor`) can call to verify the environment:
...
- **Interactive `ask_user` is not yet wired.** The SDK's
  ...
  `UserInputRequest`s through the DaisyClaw channel/TUI prompt path; the
...
Permission enforcement for bridged DaisyClaw tools happens **inside the
...
`createDaisyClawCodingTools` to every coding tool: loop detection,
...
`createDaisyClawCodingTools` — identity (`senderIsOwner`,
