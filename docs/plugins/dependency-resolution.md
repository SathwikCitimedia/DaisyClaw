---
summary: "How DaisyClaw installs plugin packages and resolves plugin dependencies"
read_when:
  - You are debugging plugin package installs
  - You are changing plugin startup, doctor, or package-manager install behavior
  - You are maintaining packaged DaisyClaw installs or bundled plugin manifests
title: "Plugin dependency resolution"
sidebarTitle: "Dependencies"
---

DaisyClaw keeps plugin dependency work at install/update time. Runtime loading
does not run package managers, repair dependency trees, or mutate the DaisyClaw
package directory.
...
- SDK/core imports are peer or supplied DaisyClaw imports
- local development plugins bring their own already-installed dependencies
- npm and git plugins are installed into DaisyClaw-owned package roots

DaisyClaw owns only the plugin lifecycle:
...
DaisyClaw uses stable per-source roots:

- npm packages install into per-plugin projects under
  `~/.daisyclaw/npm/projects/<encoded-package>`
- git packages clone under `~/.daisyclaw/git`
...
```bash
cd ~/.daisyclaw/npm/projects/<encoded-package>
npm install --omit=dev --omit=peer --legacy-peer-deps --ignore-scripts --no-audit --no-fund
```

`daisyclaw plugins install npm-pack:<path.tgz>` uses that same per-plugin npm
project root for a local npm-pack tarball. DaisyClaw reads the tarball's npm
...
`node_modules` beside the plugin package. DaisyClaw scans the managed project
...
publishable lockfile during install, and DaisyClaw's managed npm project root
supports it through the normal npm install path. DaisyClaw-owned publishable
...
DaisyClaw does not require it for community packages, but npm will respect it
when present.

DaisyClaw-owned npm plugin packages can also publish with explicit
...
with `daisyclaw.release.bundleRuntimeDependencies: false`; those packages still
...
`daisyclaw` package does not bundle its full dependency tree.

Plugins that import `daisyclaw/plugin-sdk/*` declare `daisyclaw` as a peer
dependency. DaisyClaw does not let npm install a separate registry copy of the
...
resolution/materialization and DaisyClaw reasserts plugin-local
`node_modules/daisyclaw` links for installed packages that declare the host peer
...
Local plugins are treated as developer-controlled directories. DaisyClaw does not
...
```bash
daisyclaw plugins update <id>
daisyclaw plugins install <source>
daisyclaw doctor --fix
```

`doctor --fix` can clean legacy DaisyClaw-generated dependency state and recover
...
Lightweight and core-critical bundled plugins are shipped as part of DaisyClaw.
...
In source checkouts, DaisyClaw treats the repository as a pnpm monorepo. After
...
| -------------------------------- | ------------------------------------- | -------------------------------------------------------------------- |
| `npm install -g daisyclaw`        | Built runtime tree inside the package | DaisyClaw package and explicit plugin install/update/doctor flows     |
...
| `daisyclaw plugins install ...`   | Managed npm project/git/ClawHub root  | The plugin install/update flow                                       |

## Legacy cleanup

Older DaisyClaw versions generated bundled-plugin dependency roots at startup or
...
`.daisyclaw-runtime-deps*` manifests, generated plugin `node_modules`, install
...
Older npm installs also used a shared `~/.daisyclaw/npm/node_modules` root.
