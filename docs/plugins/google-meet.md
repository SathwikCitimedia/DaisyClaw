...
read_when:
  - You want an DaisyClaw agent to join a Google Meet call
  - You want an DaisyClaw agent to create a new Google Meet call
  - You are configuring Chrome, Chrome node, or Twilio as a Google Meet transport
title: "Google Meet plugin"
---

Google Meet participant support for DaisyClaw — the plugin is explicit by design:
...
- `agent` is the default talk-back mode: realtime transcription listens, the
  configured DaisyClaw agent answers, and regular DaisyClaw TTS speaks into Meet.
...
provider plus regular DaisyClaw TTS. OpenAI is the default transcription
...
```bash
daisyclaw googlemeet setup --transport chrome-node --mode transcribe
```
...
before asking an agent to join. Use `daisyclaw googlemeet setup --json` for
...
```bash
daisyclaw googlemeet setup --transport twilio
```
...
```bash
daisyclaw googlemeet join https://meet.google.com/abc-defg-hij
```
...
```bash
daisyclaw googlemeet create --transport chrome-node --mode agent
```
...
```bash
daisyclaw googlemeet create --access-type OPEN --transport chrome-node --mode agent
```
...
- Browser fallback: used when OAuth credentials are absent. DaisyClaw uses the
  ...
  the DaisyClaw Chrome profile on the node to already be signed in to Google.
...
DaisyClaw's microphone/camera permission grant and avoid the Meet **Use
...
Use `daisyclaw googlemeet test-listen <meet-url> --transport chrome-node` when
...
Local Chrome joins through the signed-in DaisyClaw browser profile. Realtime mode
requires `BlackHole 2ch` for the microphone/speaker path used by DaisyClaw. For
...
You do **not** need a full DaisyClaw Gateway or model API key inside a macOS VM
...
- Gateway host: DaisyClaw Gateway, agent workspace, model/API keys, realtime
  provider, and the Google Meet plugin config.
- Parallels macOS VM: DaisyClaw CLI/node host, Google Chrome, SoX, BlackHole 2ch,
...
Install or update DaisyClaw in the VM, then enable the bundled plugin there:

```bash
daisyclaw plugins enable google-meet
```

Start the node host in the VM:

```bash
daisyclaw node run --host <gateway-host> --port 18789 --display-name parallels-macos
```
...
```bash
DAISYCLAW_ALLOW_INSECURE_PRIVATE_WS=1 \
  daisyclaw node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```
...
```bash
DAISYCLAW_ALLOW_INSECURE_PRIVATE_WS=1 \
  daisyclaw node install --host <gateway-lan-ip> --port 18789 --display-name parallels-macos --force
daisyclaw node restart
```

`DAISYCLAW_ALLOW_INSECURE_PRIVATE_WS=1` is process environment, not an
`daisyclaw.json` setting. `daisyclaw node install` stores it in the LaunchAgent
...
```bash
daisyclaw googlemeet test-speech https://meet.google.com/abc-defg-hij
```

During realtime join, DaisyClaw browser automation fills the guest name, clicks
...
If `chromeNode.node` is omitted, DaisyClaw auto-selects only when exactly one
...
- `No connected Google Meet-capable node`: start `daisyclaw node run` in the VM,
  approve pairing, and make sure `daisyclaw plugins enable google-meet` and
  `daisyclaw plugins enable browser` were run in the VM. Also confirm the
...
- Chrome opens but cannot join: sign in to the browser profile inside the VM, or
  keep `chrome.guestName` set for guest join. Guest auto-join uses DaisyClaw
...
- Duplicate Meet tabs: leave `chrome.reuseExistingTab: true` enabled. DaisyClaw
...
- No audio: in Meet, route microphone/speaker through the virtual audio device
  path used by DaisyClaw; use separate virtual devices or Loopback-style routing
...
DaisyClaw does not bundle or redistribute either package. The docs ask users to
...
installer or appliance that bundles BlackHole with DaisyClaw, review BlackHole's
...
Chrome transport opens the Meet URL through DaisyClaw browser control and joins
as the signed-in DaisyClaw browser profile. On macOS, the plugin checks for
...
```bash
daisyclaw googlemeet join https://meet.google.com/abc-defg-hij --transport chrome
daisyclaw googlemeet join https://meet.google.com/abc-defg-hij --transport chrome-node
```

Route Chrome microphone and speaker audio through the local DaisyClaw audio
...
meeting; DaisyClaw does not discover those from the Meet page.
      ...
      "voice-call": {
        ...
        config: {
          ...
          realtime: {
            enabled: true,
            provider: "google",
            instructions: "Join this Google Meet as an DaisyClaw agent. Be brief.",
...
```bash
daisyclaw config validate
daisyclaw plugins list | grep -E 'google-meet|voice-call'
daisyclaw googlemeet setup
```
...
`DAISYCLAW_GOOGLE_MEET_*` environment variables.
...
4. Add the scopes DaisyClaw requests:
...
`meetings.space.readonly` lets DaisyClaw resolve Meet URLs/codes to spaces.
`meetings.space.settings` lets DaisyClaw pass `SpaceConfig` settings such as
...
```bash
daisyclaw googlemeet auth login --json
```
...
```bash
DAISYCLAW_GOOGLE_MEET_CLIENT_ID="your-client-id" \
DAISYCLAW_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
daisyclaw googlemeet auth login --json
```
...
DAISYCLAW_GOOGLE_MEET_CLIENT_ID="your-client-id" \
DAISYCLAW_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
daisyclaw googlemeet auth login --json --manual
```
...
support existed, rerun `daisyclaw googlemeet auth login --json` so the refresh
...
```bash
daisyclaw googlemeet doctor --oauth --json
```
...
```bash
daisyclaw googlemeet doctor --oauth --create-space --json
daisyclaw googlemeet create --no-join --json
```
...
```bash
daisyclaw googlemeet doctor --oauth --meeting https://meet.google.com/abc-defg-hij --json
daisyclaw googlemeet resolve-space --meeting https://meet.google.com/abc-defg-hij
```
...
space. A refresh-token error means rerun `daisyclaw googlemeet auth login
...
- `DAISYCLAW_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID`
- `DAISYCLAW_GOOGLE_MEET_CLIENT_SECRET` or `GOOGLE_MEET_CLIENT_SECRET`
- `DAISYCLAW_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN`
- `DAISYCLAW_GOOGLE_MEET_ACCESS_TOKEN` or `GOOGLE_MEET_ACCESS_TOKEN`
- `DAISYCLAW_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` or
  `GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT`
- `DAISYCLAW_GOOGLE_MEET_DEFAULT_MEETING` or `GOOGLE_MEET_DEFAULT_MEETING`
- `DAISYCLAW_GOOGLE_MEET_PREVIEW_ACK` or `GOOGLE_MEET_PREVIEW_ACK`
...
```bash
daisyclaw googlemeet preflight --meeting https://meet.google.com/abc-defg-hij
```

List meeting artifacts and attendance after Meet has created conference records:

```bash
daisyclaw googlemeet artifacts --meeting https://meet.google.com/abc-defg-hij
daisyclaw googlemeet attendance --meeting https://meet.google.com/abc-defg-hij
daisyclaw googlemeet export --meeting https://meet.google.com/abc-defg-hij --output ./meet-export
```
...
```bash
daisyclaw googlemeet latest --today
daisyclaw googlemeet calendar-events --today --json
daisyclaw googlemeet artifacts --event "Weekly sync"
daisyclaw googlemeet attendance --today --format csv --output attendance.csv
```
...
```bash
daisyclaw googlemeet latest --meeting https://meet.google.com/abc-defg-hij
daisyclaw googlemeet artifacts --conference-record conferenceRecords/abc123 --json
daisyclaw googlemeet attendance --conference-record conferenceRecords/abc123 --json
```
...
```bash
daisyclaw googlemeet end-active-conference https://meet.google.com/abc-defg-hij
```
...
DaisyClaw accepts a Meet URL, meeting code, or `spaces/{id}` input and resolves it
to the API space resource before ending the active conference.
It is separate from `googlemeet leave`: `leave` stops DaisyClaw's local/session
...
```bash
daisyclaw googlemeet artifacts --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-artifacts.md
daisyclaw googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-attendance.md
daisyclaw googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format csv --output meet-attendance.csv
daisyclaw googlemeet export --conference-record conferenceRecords/abc123 \
...
DAISYCLAW_GOOGLE_MEET_LIVE_MEETING=https://meet.google.com/abc-defg-hij \
...
```bash
daisyclaw googlemeet setup --transport chrome-node --mode transcribe
daisyclaw googlemeet test-listen https://meet.google.com/abc-defg-hij --transport chrome-node --timeout-ms 30000
```

Live smoke environment:

- `DAISYCLAW_LIVE_TEST=1` enables guarded live tests.
- `DAISYCLAW_GOOGLE_MEET_LIVE_MEETING` points at a retained Meet URL, code, or
  `spaces/{id}`.
- `DAISYCLAW_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID` provides the OAuth
  client id.
- `DAISYCLAW_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN` provides
  the refresh token.
- Optional: `DAISYCLAW_GOOGLE_MEET_CLIENT_SECRET`,
  `DAISYCLAW_GOOGLE_MEET_ACCESS_TOKEN`, and
  `DAISYCLAW_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` use the same fallback names
  without the `DAISYCLAW_` prefix.
...
{
  "source": "browser",
  "error": "google-login-required: Sign in to Google in the DaisyClaw browser profile, then retry meeting creation.",
  "manualActionRequired": true,
  "manualActionReason": "google-login-required",
  "manualActionMessage": "Sign in to Google in the DaisyClaw browser profile, then retry meeting creation.",
  "browser": {
    "nodeId": "ba0f4e4bc...",
    "targetId": "tab-1",
...
profile is signed out, DaisyClaw reports `manualActionRequired: true` or a
...
realtime transcription provider key, and a configured DaisyClaw TTS provider.
...
- `chrome.guestName: "DaisyClaw Agent"`: name used on the signed-out Meet guest
  screen
- `chrome.autoJoin: true`: best-effort guest-name fill and Join Now click
  through DaisyClaw browser automation on `chrome-node`
...
- `mode: "agent"`: default talk-back mode. Participant speech is transcribed by
  ...
  DaisyClaw agent in a per-meeting sub-agent session, and spoken back through the
  normal DaisyClaw TTS runtime.
- `mode: "bidi"`: fallback direct bidirectional realtime model mode. The
  ...
  `daisyclaw_agent_consult` for deeper/tool-backed answers.
...
- `realtime.instructions`: brief spoken replies, with
  `daisyclaw_agent_consult` for deeper answers
- `realtime.introMessage`: short spoken readiness check when the realtime bridge
  connects; set it to `""` to join silently
- `realtime.agentId`: optional DaisyClaw agent id for
  `daisyclaw_agent_consult`; defaults to `main`
...
VM. In both cases the model providers and `daisyclaw_agent_consult` run on the
Gateway host, so model credentials stay there. With the default `mode: "agent"`,
the realtime transcription provider handles listening, the configured DaisyClaw
agent produces the answer, and regular DaisyClaw TTS speaks it into Meet. Use
...
- `speechReady` / `speechBlockedReason` / `speechBlockedMessage`: whether
  managed Chrome speech is allowed now. `speechReady: false` means DaisyClaw did
...
transcripts are routed through the configured DaisyClaw agent, and the answer is
spoken through the normal DaisyClaw TTS runtime. Set `mode: "bidi"` when you want
...
| ------- | ----------------------------- | -------------------------------------- | ----------------------------------------------------- |
| `agent` | The configured DaisyClaw agent | Normal DaisyClaw TTS runtime            | You want "my agent is in the meeting" behavior        |
...
information, or normal DaisyClaw tools, it can call `daisyclaw_agent_consult`.

The consult tool runs the regular DaisyClaw agent behind the scenes with recent
meeting transcript context and returns a concise spoken answer. In `agent` mode,
DaisyClaw sends that answer directly to the TTS runtime; in `bidi` mode, the
...
Meet lane should consult a dedicated DaisyClaw agent workspace, model defaults,
...
```bash
daisyclaw googlemeet speak meet_... "Say exactly: I'm here and listening."
```
...
```bash
daisyclaw googlemeet setup
daisyclaw nodes status --connected
daisyclaw nodes invoke \
  --node parallels-macos \
...
- `daisyclaw logs --follow` shows DTMF TwiML served before realtime TwiML, then a
...
```bash
daisyclaw plugins list | grep google-meet
daisyclaw googlemeet setup
```
...
```bash
daisyclaw plugins enable google-meet
daisyclaw plugins enable browser
DAISYCLAW_ALLOW_INSECURE_PRIVATE_WS=1 \
  daisyclaw node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```
...
hear you in the meeting?" That is Meet's audio-choice interstitial; DaisyClaw
clicks **Use microphone** through browser automation when available and keeps
waiting for the real meeting state. For create-only browser fallback, DaisyClaw
...
- For API creation: `oauth.clientId` and `oauth.refreshToken` are configured,
  or matching `DAISYCLAW_GOOGLE_MEET_*` environment variables are present.
- For API creation: the refresh token was minted after create support was
  ...
  `daisyclaw googlemeet auth login --json` and update plugin config.
- For browser fallback: `defaultTransport: "chrome-node"` and
  `chromeNode.node` point at a connected node with `browser.proxy` and
  `googlemeet.chrome`.
- For browser fallback: the DaisyClaw Chrome profile on that node is signed in
...
- For browser fallback: if Meet shows "Do you want people to hear you in the
  meeting?", leave the tab open. DaisyClaw should click **Use microphone** or, for
...
```bash
daisyclaw googlemeet setup
daisyclaw googlemeet doctor
```

Use `mode: "agent"` for the normal STT -> DaisyClaw agent -> TTS talk-back path,
...
run `daisyclaw googlemeet status --json <session-id>` after participants speak
...
utterance but DaisyClaw did not see new output bytes reach the Chrome audio
bridge.
...
- Meet microphone and speaker are routed through the virtual audio path used by
  DaisyClaw. `doctor` should show `meet output routed: yes` for local Chrome
  realtime joins.
...
```bash
daisyclaw googlemeet recover-tab
daisyclaw googlemeet recover-tab https://meet.google.com/abc-defg-hij
```
...
```bash
daisyclaw googlemeet setup --transport twilio
daisyclaw voicecall setup
daisyclaw voicecall smoke
```

`voicecall smoke` is readiness-only by default. To dry-run a specific number:

```bash
daisyclaw voicecall smoke --to "+15555550123"
```
...
```bash
daisyclaw voicecall smoke --to "+15555550123" --yes
```
...
- Run `daisyclaw googlemeet doctor <session-id>` to confirm the delegated Twilio
  call ID, whether DTMF was queued, and whether the intro greeting was requested.
- Run `daisyclaw voicecall status --call-id <id>` and confirm the call is still
  active.
- Run `daisyclaw voicecall tail` and check that Twilio webhooks are arriving at
  the Gateway.
- Run `daisyclaw logs --follow` and look for the Twilio Meet sequence: Google
...
- Re-run `daisyclaw googlemeet setup --transport twilio`; a green setup check is
...
- If the participant joins but you do not hear the greeting, check
  `daisyclaw logs --follow` for the post-DTMF `voicecall.speak` request and
...
- `chrome.audioInputCommand` plus `chrome.audioOutputCommand`: DaisyClaw owns the
