...
Plugin hooks are in-process extension points for DaisyClaw plugins. Use them
...
current plugin options; DaisyClaw injects it per handler without mutating the
...
- `subagent_spawned` includes `resolvedModel` and `resolvedProvider` when DaisyClaw has resolved the child session's native model before launch.
...
use `daisyclaw sessions` or the Gateway session/status surfaces. When debugging
...
- DaisyClaw strips `toolResult.details` before provider replay and compaction
...
When a run is blocked, DaisyClaw stores only the replacement text in
...
`before_agent_start` and `agent_end` include `event.runId` when DaisyClaw can
...
identifier when DaisyClaw can derive one from the session key or delivery
metadata.
...
forever. A timeout is logged and DaisyClaw continues; it does not cancel
...
`durationMs`/`outcome`, and `upstreamRequestIdHash` when DaisyClaw can derive a
...
Codex native `Stop` hooks are relayed into this hook as DaisyClaw
...
reach the next model turn exactly once. DaisyClaw drains queued injections before
...
compatibility, but DaisyClaw no longer runs built-in install-time dangerous-code
...
context when syncing external wake schedulers, and keep DaisyClaw as the
