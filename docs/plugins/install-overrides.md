...
[`daisyclaw plugins install`](/cli/plugins).
...
```bash
export DAISYCLAW_ALLOW_PLUGIN_INSTALL_OVERRIDES=1
export DAISYCLAW_PLUGIN_INSTALL_OVERRIDES='{
  "codex": "npm-pack:/tmp/daisyclaw-codex-2026.5.8.tgz",
  "daisyclaw-web-search": "npm:@daisyclaw/web-search@2026.5.8"
}'
```
...
DaisyClaw uses the override source instead of the catalog, bundled, or default
...
entry normally represents an DaisyClaw-owned package, an override is treated as
operator-supplied test input.

Workspace `.env` files cannot enable install overrides. Set these variables in
the trusted shell, CI job, or remote test command that launches DaisyClaw.

## Package E2E

Use an isolated state directory so package installs and install records do not
touch your normal DaisyClaw state:

```bash
npm pack extensions/codex --pack-destination /tmp

DAISYCLAW_STATE_DIR="$(mktemp -d)" \
DAISYCLAW_ALLOW_PLUGIN_INSTALL_OVERRIDES=1 \
DAISYCLAW_PLUGIN_INSTALL_OVERRIDES='{"codex":"npm-pack:/tmp/daisyclaw-codex-2026.5.8.tgz"}' \
pnpm daisyclaw onboard --mode local
```

Verify the installed package under the state directory:

```bash
find "$DAISYCLAW_STATE_DIR/npm/projects" -path '*/node_modules/@daisyclaw/codex/package.json' -print
grep -R '"@daisyclaw/codex"' "$DAISYCLAW_STATE_DIR/npm/projects"/*/package-lock.json
```
