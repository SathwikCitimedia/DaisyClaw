...
read_when:
  ...
  - You need the DaisyClaw plugin that owns the node-llama-cpp runtime
...
The main `daisyclaw` npm package does not include `node-llama-cpp`. Keeping the
native dependency in this plugin prevents normal DaisyClaw npm updates from
deleting a manually installed runtime inside the DaisyClaw package directory.
