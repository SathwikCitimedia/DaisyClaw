...
read_when:
  - You are building an DaisyClaw plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin manifest"
---

This page is for the **native DaisyClaw plugin manifest** only.
...
DaisyClaw auto-detects those bundle layouts too, but they are not validated
against the `daisyclaw.plugin.json` schema described here.

For compatible bundles, DaisyClaw currently reads bundle metadata plus declared
...
DaisyClaw runtime expectations.

Every native DaisyClaw plugin **must** ship a `daisyclaw.plugin.json` file in the
**plugin root**. DaisyClaw uses this manifest to validate configuration
...
`daisyclaw.plugin.json` is the metadata DaisyClaw reads **before it loads your
...
- QA runner metadata the shared `daisyclaw qa` host can inspect
...
| `providerAuthEnvVars`                | No       | `Record<string, string[]>`       | Deprecated compatibility env metadata for provider auth/status lookup. Prefer `setup.providers[].envVars` for new plugins; DaisyClaw still reads this during the deprecation window.                                                             |
| `providerAuthAliases`                | No       | `Record<string, string>`         | Provider ids that should reuse another provider id for auth lookup, for example a coding provider that shares the base provider API key and auth profiles.                                                                                      |
| `channelEnvVars`                     | No       | `Record<string, string[]>`       | Cheap channel env metadata that DaisyClaw can inspect without loading plugin code. Use this for env-driven channel setup or auth surfaces that generic startup/config helpers should see.                                                        |
...
| `qaRunners`                          | No       | `object[]`                       | Cheap QA runner descriptors used by the shared `daisyclaw qa` host before plugin runtime loads.                                                                                                                                                  |
...
DaisyClaw reads these fields before provider runtime loads so core tools can
...
ownership. `toolMetadata` declares cheap availability evidence so DaisyClaw can
...
If a tool has no `toolMetadata`, DaisyClaw preserves the existing behavior and
...
DaisyClaw reads this before provider runtime loads.
...
| `choiceLabel`         | No       | `string`                                                              | User-facing label. If omitted, DaisyClaw falls back to `choiceId`.                                        |
...
mistakenly put in `plugins.allow` or try to run as a root CLI command. DaisyClaw
...
the shared `daisyclaw qa` root. Keep this metadata cheap and static; the plugin
...
| Field         | Required | Type     | What it means                                                      |
| ------------- | -------- | -------- | ------------------------------------------------------------------ |
| `commandName` | Yes      | `string` | Subcommand mounted beneath `daisyclaw qa`, for example `matrix`.    |
...
DaisyClaw also includes `setup.providers[].envVars` in generic provider auth and
...
DaisyClaw can also derive simple setup choices from `setup.providers[].authMethods`
...
setup surface. DaisyClaw treats explicit `false` as a descriptor-only contract
and will not execute `setup-api` or `daisyclaw.setupEntry` for setup lookup. If
a descriptor-only plugin still ships one of those setup runtime entries,
DaisyClaw reports an additive diagnostic and continues ignoring it. Omitted
...
Use `contracts` only for static capability ownership metadata that DaisyClaw can
read without importing the plugin runtime.

```json
{
  "contracts": {
    "agentToolResultMiddleware": ["daisyclaw", "codex"],
...
| `migrationProviders`             | `string[]` | Import provider ids this plugin owns for `daisyclaw migrate`.                                                                         |
...
DaisyClaw reads manifest metadata to decide which plugin owns that configured
...
`channelConfigs` entries. Without them, DaisyClaw can still load the plugin, but
...
the same defaults through `package.json#daisyclaw.channel.commands` alongside
...
When `channels.chat` is configured, DaisyClaw considers both the channel id and
the preferred plugin id. If the lower-priority plugin was only selected because
it is bundled or enabled by default, DaisyClaw disables it in the effective
runtime config so one plugin owns the channel and its tools. Explicit user
selection still wins: if the user explicitly enables both plugins, DaisyClaw
...
Use `modelSupport` when DaisyClaw should infer your provider plugin from
...
DaisyClaw applies this precedence:
...
Use `modelCatalog` when DaisyClaw should know provider model metadata before
...
provider-filtered list uses an alias, DaisyClaw can read the owning manifest and
...
| `compat`        | `object`                                                       | Optional compatibility flags matching DaisyClaw model config compatibility.  |
...
refreshable rows are not authoritative by themselves. Use `runtime` when DaisyClaw
...
exec provider preset. DaisyClaw reads this metadata before plugin runtime loads,
...
The map key is the integration id. If `providerAlias` is omitted, DaisyClaw uses
...
When an operator selects the preset, DaisyClaw writes a provider reference like:
...
At startup/reload, DaisyClaw resolves that provider by loading current plugin
...
DaisyClaw materializes it at startup/reload to the current Node executable and
...
DaisyClaw derives `trustedDirs` for manifest presets from the plugin root and,
...
| Field                      | Type               | What it means                                                                                                        |
| -------------------------- | ------------------ | -------------------------------------------------------------------------------------------------------------------- |
| `provider`                 | `string`           | External catalog provider id when it differs from the DaisyClaw provider id, for example `z-ai` for a `zai` provider. |
...
### DaisyClaw Provider Index

The DaisyClaw Provider Index is DaisyClaw-owned preview metadata for providers
...
4. DaisyClaw Provider Index preview rows.
...
Legacy top-level capability keys are deprecated. Use `daisyclaw doctor --fix` to
...
| File                   | Use it for                                                                                                                       |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `daisyclaw.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs                         |
| `package.json`         | npm metadata, dependency installation, and the `daisyclaw` block used for entrypoints, install gating, setup, or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if DaisyClaw must know it before loading plugin code, put it in `daisyclaw.plugin.json`
...
`daisyclaw` block instead of `daisyclaw.plugin.json`.
`daisyclaw.bundle` and `daisyclaw.bundle.json` are not DaisyClaw plugin contracts;
native plugins must use `daisyclaw.plugin.json` plus the supported
`package.json#daisyclaw` fields below.

Important examples:

| Field                                                                                      | What it means                                                                                                                                                                        |
| ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `daisyclaw.extensions`                                                                      | Declares native plugin entrypoints. Must stay inside the plugin package directory.                                                                                                   |
| `daisyclaw.runtimeExtensions`                                                               | Declares built JavaScript runtime entrypoints for installed packages. Must stay inside the plugin package directory.                                                                 |
| `daisyclaw.setupEntry`                                                                      | Lightweight setup-only entrypoint used during onboarding, deferred channel startup, and read-only channel status/SecretRef discovery. Must stay inside the plugin package directory. |
| `daisyclaw.runtimeSetupEntry`                                                               | Declares the built JavaScript setup entrypoint for installed packages. Requires `setupEntry`, must exist, and must stay inside the plugin package directory.                         |
| `daisyclaw.channel`                                                                         | Cheap channel catalog metadata like labels, docs paths, aliases, and selection copy.                                                                                                 |
| `daisyclaw.channel.commands`                                                                | Static native command and native skill auto-default metadata used by config, audit, and command-list surfaces before channel runtime loads.                                          |
| `daisyclaw.channel.configuredState`                                                         | Lightweight configured-state checker metadata that can answer "does env-only setup already exist?" without loading the full channel runtime.                                         |
| `daisyclaw.channel.persistedAuthState`                                                      | Lightweight persisted-auth checker metadata that can answer "is anything already signed in?" without loading the full channel runtime.                                               |
| `daisyclaw.install.clawhubSpec` / `daisyclaw.install.npmSpec` / `daisyclaw.install.localPath` | Install/update hints for bundled and externally published plugins.                                                                                                                   |
| `daisyclaw.install.defaultChoice`                                                           | Preferred install path when multiple install sources are available.                                                                                                                  |
| `daisyclaw.install.minHostVersion`                                                          | Minimum supported DaisyClaw host version, using a semver floor like `>=2026.3.22` or `>=2026.5.1-beta.1`.                                                                             |
| `daisyclaw.compat.pluginApi`                                                                | Minimum DaisyClaw plugin API range required by this package, using a semver floor like `>=2026.5.27`.                                                                                 |
| `daisyclaw.install.expectedIntegrity`                                                       | Expected npm dist integrity string such as `sha512-...`; install and update flows verify the fetched artifact against it.                                                            |
| `daisyclaw.install.allowInvalidConfigRecovery`                                              | Allows a narrow bundled-plugin reinstall recovery path when config is invalid.                                                                                                       |
| `daisyclaw.startup.deferConfiguredChannelFullLoadUntilAfterListen`                          | Lets setup-runtime channel surfaces load before listen, then defers the full configured channel plugin until post-listen activation.                                                 |

Manifest metadata decides which provider/channel/setup choices appear in
onboarding before runtime loads. `package.json#daisyclaw.install` tells
onboarding how to fetch or enable that plugin when the user picks one of those
choices. Do not move install hints into `daisyclaw.plugin.json`.

`daisyclaw.install.minHostVersion` is enforced during install and manifest
...
`daisyclaw.compat.pluginApi` is enforced during package install for non-bundled
plugin sources. Use it for the DaisyClaw plugin SDK/runtime API floor that the
...
flows. Official DaisyClaw release sync bumps existing official plugin API floors
to the DaisyClaw release version by default, but plugin-only releases can keep a
lower floor when the package intentionally supports older hosts. Do not use the
package version alone as the compatibility contract. `peerDependencies.daisyclaw`
remains npm package metadata; DaisyClaw uses the `daisyclaw.compat.pluginApi`
...
`"npmSpec": "@wecom/wecom-daisyclaw-plugin@1.2.3"`. Official external catalog
...
Channel plugins should provide `daisyclaw.setupEntry` when status, channel list,
...
entrypoint fields. For example, `daisyclaw.runtimeExtensions` cannot make an
escaping `daisyclaw.extensions` path loadable.

`daisyclaw.install.allowInvalidConfigRecovery` is intentionally narrow. It does
...
to `daisyclaw doctor --fix`.

`daisyclaw.channel.persistedAuthState` is package metadata for a tiny checker
module:
...
`daisyclaw.channel.configuredState` follows the same shape for cheap env-only
configured checks:
...
DaisyClaw discovers plugins from several roots. For the raw filesystem scan
...
2. **Bundled** — plugins shipped with DaisyClaw
3. **Global install** — plugins installed into the global DaisyClaw plugin root
...
- When extending or forking a bundled plugin with new config keys, update that plugin's `daisyclaw.plugin.json` `configSchema` at the same time. Bundled plugin schemas are strict, so adding `plugins.entries.<id>.config.myNewKey` in user config without adding `myNewKey` to `configSchema.properties` will be rejected before the plugin runtime loads.
...
- The manifest is **required for native DaisyClaw plugins**, including local filesystem loads. Runtime still loads the plugin module separately; the manifest is only for discovery + validation.
...
- Declare exclusive plugin kind in this manifest. Runtime-entry `DaisyClawPluginDefinition.kind` is deprecated and remains only as a compatibility fallback for older plugins.
