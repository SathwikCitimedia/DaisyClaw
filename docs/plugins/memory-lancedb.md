...
```bash
daisyclaw plugins install @daisyclaw/memory-lancedb
```

The plugin is published to npm and is not bundled into the DaisyClaw runtime image.
...
Set `dimensions` for non-standard embedding models. DaisyClaw knows the
...
`memory_forget` query path, and `daisyclaw ltm search`. Auto-recall prefers the
...
```bash
daisyclaw ltm list
daisyclaw ltm search "project preferences"
daisyclaw ltm stats
```
...
```bash
daisyclaw ltm query --cols id,text,createdAt --limit 20
daisyclaw ltm query --filter "category = 'preference'" --order-by createdAt:desc
```
...
By default, LanceDB data lives under `~/.daisyclaw/memory/lancedb`. Override the
path with `dbPath`:
  ...
  plugins: {
    entries: {
      "memory-lancedb": {
        enabled: true,
        config: {
          dbPath: "~/.daisyclaw/memory/lancedb",
          embedding: {
  ...
  plugins: {
    entries: {
      "memory-lancedb": {
        enabled: true,
        config: {
          dbPath: "s3://memory-bucket/daisyclaw",
          storageOptions: {
...
DaisyClaw treats that package as part of the plugin package. Gateway startup
...
`@lancedb/lancedb` error during plugin load, upgrade DaisyClaw and restart the
Gateway.
...
```bash
daisyclaw ltm stats
daisyclaw ltm search "recent preference"
```
