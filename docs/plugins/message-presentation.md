...
Message presentation is DaisyClaw's shared contract for rich outbound chat UI.
...
```bash
daisyclaw message send --channel slack \
  --target channel:C123 \
...
```bash
daisyclaw message send --channel telegram \
...
Use helpers from `daisyclaw/plugin-sdk/interactive-runtime` when bridging older
code:
