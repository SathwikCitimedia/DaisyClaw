---
summary: "Bundled `oc-path` plugin: ships the `daisyclaw path` CLI for the `oc://` workspace-file addressing scheme"
read_when:
...
The bundled `oc-path` plugin adds the [`daisyclaw path`](/cli/path) CLI for the
`oc://` workspace-file addressing scheme. It ships in the DaisyClaw repo under
...
- **Local automation**: shell scripts can resolve or update one workspace value
  with `daisyclaw path … --json` instead of carrying separate markdown, JSONC,
...
daisyclaw path resolve 'oc://config.jsonc/plugins/github/enabled' --json

# Which tool-call names appear in this session log?
daisyclaw path find 'oc://session.jsonl/[event=tool_call]/name' --json
...
The plugin runs **in-process inside the `daisyclaw` CLI** on the host where you
...
The plugin metadata lives in `extensions/oc-path/daisyclaw.plugin.json`:
...
`daisyclaw path …`, so installs that never use the verb pay no cost.
...
state. Bare `daisyclaw path` invocations work immediately on the same host —
the CLI loads the plugin on demand.

Disable with:

```bash
daisyclaw plugins disable oc-path
```
...
| Surface                        | Provided by                                             |
| ------------------------------ | ------------------------------------------------------- |
| `daisyclaw path` CLI            | `extensions/oc-path/cli-registration.ts`                |
...
`__DAISYCLAW_REDACTED__` (verbatim or as a substring) is refused at write time
...
- [`daisyclaw path` CLI reference](/cli/path)
