---
summary: "Adds Microsoft Foundry model provider support to DaisyClaw."
read_when:
...
Adds Microsoft Foundry model provider support to DaisyClaw.

## Distribution

- Package: `@daisyclaw/microsoft-foundry`
- Install route: included in DaisyClaw
...
- For Entra ID auth, install the Azure CLI and run `az login` before
  onboarding. DaisyClaw refreshes Microsoft Foundry runtime tokens through
...
DaisyClaw uses the Foundry `/openai/v1` endpoint for supported OpenAI-compatible
chat APIs:
...
Claude model ID, set `params.canonicalModelId` on the model entry so DaisyClaw
...
deployment through onboarding or include model metadata so DaisyClaw can verify
