...
An **agent harness** is the low level executor for one prepared DaisyClaw agent
...
runtime and the normal DaisyClaw provider transport is the wrong abstraction.
...
- a model runtime that needs its own resume id in addition to the DaisyClaw
  session transcript
...
Before a harness is selected, DaisyClaw has already resolved:

- provider and model
- runtime auth state
- thinking level and context budget
- the DaisyClaw transcript/session file
...
The prepared attempt also includes `params.runtimePlan`, an DaisyClaw-owned
policy bundle for runtime decisions that must stay shared across DaisyClaw and native
harnesses:
...
Harnesses may use the plan for decisions that need to match DaisyClaw behavior, but
should still treat it as host-owned attempt state. Do not mutate it or use it to
switch providers/models inside a turn.

## Register a harness

**Import:** `daisyclaw/plugin-sdk/agent-harness`

```typescript
import type { AgentHarness } from "daisyclaw/plugin-sdk/agent-harness";
...
DaisyClaw chooses a harness after provider/model resolution:
...
3. `auto` asks registered harnesses if they support the resolved
   provider/model.
4. If no registered harness matches, DaisyClaw uses its embedded runtime.
...
provider/model. Once a plugin harness has claimed a run, DaisyClaw does not
...
`agents.list[].agentRuntime`, and `DAISYCLAW_AGENT_RUNTIME`. `/status` shows the
...
DaisyClaw. The harness then claims that provider in `supports(...)`.
...
- app-server request: DaisyClaw sends the bare model id to Codex and lets the
...
DaisyClaw requires Codex app-server `0.125.0` or newer. The Codex plugin checks
the app-server initialize handshake and blocks older or unversioned servers so
DaisyClaw only runs against the protocol surface it has been tested with. The
`0.125.0` floor includes the native MCP hook payload support that landed in
Codex `0.124.0`, while pinning DaisyClaw to the newer tested stable line.
...
seam is for async tool-result transforms that must run before DaisyClaw or Codex
...
`daisyclaw/plugin-sdk/agent-harness-runtime` when a completed turn produced no
visible assistant text. The helper returns `empty`, `reasoning-only`, or
`planning-only` so DaisyClaw's fallback policy can decide whether to retry on a
...
The bundled `codex` harness is the native Codex mode for embedded DaisyClaw
...
`daisyclaw doctor --fix`, and legacy `codex/*` model refs remain compatibility
aliases for the native harness.

When this mode runs, Codex owns the native thread id, resume behavior,
compaction, and app-server execution. DaisyClaw still owns the chat channel,
...
By default, DaisyClaw uses `auto` provider/model runtime policy: registered
...
fail hard. This does not block an explicit provider/model `agentRuntime.id: "daisyclaw"`.
...
Keep that binding explicitly associated with the DaisyClaw session, and keep
mirroring user-visible assistant/tool output into the DaisyClaw transcript.

The DaisyClaw transcript remains the compatibility layer for:

- channel-visible session history
- transcript search and indexing
- switching back to the built-in DaisyClaw harness on a later turn
- generic `/new`, `/reset`, and session deletion behavior

If your harness stores a sidecar binding, implement `reset(...)` so DaisyClaw can
clear it when the owning DaisyClaw session is reset.

## Tool and media results

Core constructs the DaisyClaw tool list and passes it into the prepared attempt.
...
on the same delivery path as DaisyClaw-backed runs.
