---
summary: "Step-by-step guide to building a messaging channel plugin for DaisyClaw"
title: "Building channel plugins"
sidebarTitle: "Channel Plugins"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect DaisyClaw to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

This guide walks through building a channel plugin that connects DaisyClaw to a
...
<Info>
  If you have not built any DaisyClaw plugin before, read
...
Channel plugins do not need their own send/edit/react tools. DaisyClaw keeps one
...
`defineChannelMessageAdapter` from `daisyclaw/plugin-sdk/channel-outbound`. The
...
receive/send lifecycle helpers on `daisyclaw/plugin-sdk/channel-outbound`.

Channels migrating inbound authorization can use the experimental
`daisyclaw/plugin-sdk/channel-ingress-runtime` subpath from runtime receive
...
`daisyclaw/plugin-sdk/outbound-media` with plugin state stores. Keep platform
...
Use `daisyclaw/plugin-sdk/channel-route` when plugin code needs to normalize
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `daisyclaw/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
- If custom approval auth intentionally allows only same-chat fallback, return `markImplicitSameChatApprovalAuthorization({ authorized: true })` from `daisyclaw/plugin-sdk/approval-auth-runtime`; otherwise core treats the result as explicit approver authorization.
...
- `daisyclaw/plugin-sdk/approval-auth-runtime`
- `daisyclaw/plugin-sdk/approval-client-runtime`
- `daisyclaw/plugin-sdk/approval-delivery-runtime`
- `daisyclaw/plugin-sdk/approval-gateway-runtime`
- `daisyclaw/plugin-sdk/approval-handler-adapter-runtime`
- `daisyclaw/plugin-sdk/approval-handler-runtime`
- `daisyclaw/plugin-sdk/approval-native-runtime`
- `daisyclaw/plugin-sdk/approval-reply-runtime`
- `daisyclaw/plugin-sdk/channel-runtime-context`

Likewise, prefer `daisyclaw/plugin-sdk/setup-runtime`,
`daisyclaw/plugin-sdk/setup-runtime`,
`daisyclaw/plugin-sdk/reply-runtime`,
`daisyclaw/plugin-sdk/reply-dispatch-runtime`,
`daisyclaw/plugin-sdk/reply-reference`, and
`daisyclaw/plugin-sdk/reply-chunking` when you do not need the broader umbrella
surface.

For setup specifically:

- `daisyclaw/plugin-sdk/setup-runtime` covers the runtime-safe setup helpers:
...
- `daisyclaw/plugin-sdk/setup-runtime` includes the env-aware adapter seam for
  `createEnvPatchedAccountSetupAdapter`
- `daisyclaw/plugin-sdk/channel-setup` covers the optional-install setup
...
SecretRef scans before the plugin runtime starts, add `daisyclaw.setupEntry` in
...
- use the broader `daisyclaw/plugin-sdk/setup` seam only when you also need the
...
- `daisyclaw/plugin-sdk/account-core`,
  `daisyclaw/plugin-sdk/account-id`,
  `daisyclaw/plugin-sdk/account-resolution`, and
  `daisyclaw/plugin-sdk/account-helpers` for multi-account config and
  default-account fallback
- `daisyclaw/plugin-sdk/inbound-envelope` and
  `daisyclaw/plugin-sdk/channel-inbound` for inbound route/envelope and
  record-and-dispatch wiring
- `daisyclaw/plugin-sdk/channel-targets` for target parsing helpers
- `daisyclaw/plugin-sdk/outbound-media` for media loading and
  `daisyclaw/plugin-sdk/channel-outbound` for outbound identity/send delegates
  and payload planning
- `buildThreadAwareOutboundSessionRoute(...)` from
  `daisyclaw/plugin-sdk/channel-core` when an outbound route should preserve an
...
- `daisyclaw/plugin-sdk/thread-bindings-runtime` for thread-binding lifecycle
  and adapter registration
- `daisyclaw/plugin-sdk/agent-media-payload` only when a legacy agent/media
  payload field layout is still required
- `daisyclaw/plugin-sdk/telegram-command-config` for Telegram custom-command
...
Use `daisyclaw/plugin-sdk/channel-mention-gating` for mention-policy decisions.
Use `daisyclaw/plugin-sdk/channel-inbound` only when you need the broader inbound
helper barrel.
...
`daisyclaw/plugin-sdk/channel-mention-gating` to avoid loading unrelated inbound
runtime helpers.
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    see [Plugin Setup and Config](/plugins/sdk-setup#daisyclaw-channel):

    <CodeGroup>
    ```json package.json
    {
      "name": "@myorg/daisyclaw-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "daisyclaw": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect DaisyClaw to Acme Chat."
        }
      }
    }
    ```

    ```json daisyclaw.plugin.json
    {
      "id": "acme-chat",
      "kind": "channel",
      "channels": ["acme-chat"],
  ...
  <Step title="Build the channel plugin object">
    ...
    import type { DaisyClawConfig } from "daisyclaw/plugin-sdk/channel-core";
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "daisyclaw/plugin-sdk/channel-core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so DaisyClaw
  ...
  <Step title="Add a setup entry">
    ...
    import { defineSetupPluginEntry } from "daisyclaw/plugin-sdk/channel-core";
    import { acmeChatPlugin } from "./src/channel.js";

    export default defineSetupPluginEntry(acmeChatPlugin);
    ```

    DaisyClaw loads this instead of the full entry when the channel is disabled
    ...
    `daisyclaw/plugin-sdk/channel-entry-contract` when they also need an
  ...
  <Step title="Handle inbound messages">
    ...
    DaisyClaw. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to DaisyClaw.
...
├── package.json              # daisyclaw.channel metadata
├── daisyclaw.plugin.json      # Manifest with config schema
