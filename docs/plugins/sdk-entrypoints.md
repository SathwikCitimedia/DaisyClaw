...
when DaisyClaw loads an installed package and let npm packages avoid runtime
...
an installed package only declares a TypeScript source entry, DaisyClaw will use a
...
**Import:** `daisyclaw/plugin-sdk/tool-plugin`
...
schemas, wraps plain return values in the DaisyClaw tool-result format, and
exposes static metadata that `daisyclaw plugins build` writes into the plugin
manifest.
...
- `configSchema` is optional. When omitted, DaisyClaw uses a strict empty object
...
- Tool names are static. `daisyclaw plugins build` derives `contracts.tools`
...
- Runtime loading stays strict. Installed plugins still need
  `daisyclaw.plugin.json` and `package.json` `daisyclaw.extensions`; DaisyClaw does
  not execute plugin code to infer missing manifest data.

## `definePluginEntry`

**Import:** `daisyclaw/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | -                   |
| `configSchema` | `DaisyClawPluginConfigSchema \| () => DaisyClawPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: DaisyClawPluginApi) => void`                               | Yes      | -                   |

- `id` must match your `daisyclaw.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.
- DaisyClaw resolves and memoizes that schema on first access, so expensive schema
  builders only run once.

## `defineChannelPluginEntry`

**Import:** `daisyclaw/plugin-sdk/channel-core`
...
| `registerCliMetadata` | `(api: DaisyClawPluginApi) => void`                               | No       | -                   |
| `registerFull`        | `(api: DaisyClawPluginApi) => void`                               | No       | -                   |
...
- Discovery registration is non-activating, not import-free. DaisyClaw may
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory and DaisyClaw
...
- For plugin-owned root CLI commands, prefer `api.registerCli(..., { descriptors: [...] })`
  ...
  `api.registerNodeCliFeature(...)` so the command lands under `daisyclaw nodes`.
  For other nested plugin commands, add `parentPath` and register commands on
  the `program` object passed to the registrar; DaisyClaw resolves it to the
...
DaisyClaw loads this instead of the full entry when a channel is disabled,
...
- `daisyclaw/plugin-sdk/setup-runtime` for runtime-safe setup helpers such as
...
- `daisyclaw/plugin-sdk/channel-setup` for optional-install setup surfaces
- `daisyclaw/plugin-sdk/setup-tools` for setup/install CLI/archive/docs helpers
...
`daisyclaw/plugin-sdk/channel-entry-contract` instead. That contract lets the
setup entry keep setup-safe plugin/secrets exports while still exposing a
runtime setter:

```typescript
import { defineBundledChannelSetupEntry } from "daisyclaw/plugin-sdk/channel-entry-contract";
...
the plugin entry and the channel plugin object so DaisyClaw can register channel
...
- use `descriptors` when the registrar owns one or more root commands and you
  want DaisyClaw to lazy-load the real CLI module on first invocation
...
- keep descriptor command names to letters, numbers, hyphen, and underscore,
  starting with a letter or number; DaisyClaw rejects descriptor names outside
...
DaisyClaw classifies loaded plugins by their registration behavior:
...
Use `daisyclaw plugins inspect <id>` to see a plugin's shape.
