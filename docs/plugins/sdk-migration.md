...
read_when:
  - You see the DAISYCLAW_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the DAISYCLAW_EXTENSION_API_DEPRECATED warning
  - You used api.registerEmbeddedExtensionFactory before DaisyClaw 2026.4.25
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external DaisyClaw plugin
---

DaisyClaw has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`daisyclaw/plugin-sdk/compat`** - a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`daisyclaw/plugin-sdk/infra-runtime`** - a broad runtime helper barrel that
...
- **`daisyclaw/plugin-sdk/config-runtime`** - a broad config compatibility barrel
  that still carries deprecated direct load/write helpers during the migration
  window.
- **`daisyclaw/extension-api`** - a bridge that gave plugins direct access to
...
DaisyClaw does not remove or reinterpret documented plugin behavior in the same
...
The modern plugin SDK fixes this: each import path (`daisyclaw/plugin-sdk/\<subpath\>`)
...
`daisyclaw/plugin-sdk/realtime-voice`. The new controller owns the common Talk
...
`talk.providers` should be repaired with `daisyclaw doctor --fix`; runtime Talk
...
<Steps>
  <Step title="Migrate runtime config load/write helpers">
    ...
    `daisyclaw/plugin-sdk/config-runtime` compatibility barrel. Use the narrow
    SDK subpath that matches the job:

    | Need | Import |
    | --- | --- |
    | Config types such as `DaisyClawConfig` | `daisyclaw/plugin-sdk/config-contracts` |
    | Already-loaded config assertions and plugin-entry config lookup | `daisyclaw/plugin-sdk/plugin-config-runtime` |
    | Current runtime snapshot reads | `daisyclaw/plugin-sdk/runtime-config-snapshot` |
    | Config writes | `daisyclaw/plugin-sdk/config-mutation` |
    | Session store helpers | `daisyclaw/plugin-sdk/session-store-runtime` |
    | Markdown table config | `daisyclaw/plugin-sdk/markdown-table-runtime` |
    | Group policy runtime helpers | `daisyclaw/plugin-sdk/runtime-group-policy` |
    | Secret input resolution | `daisyclaw/plugin-sdk/secret-input-runtime` |
    | Model/session overrides | `daisyclaw/plugin-sdk/model-session-runtime` |
  ...
  <Step title="Migrate embedded tool-result extensions to middleware">
    ...
    ```typescript
    // DaisyClaw and Codex runtime dynamic tools
    api.registerAgentToolResultMiddleware(async (event) => {
      return compactToolResult(event);
    }, {
      runtimes: ["daisyclaw", "codex"],
    });
    ```
  ...
  <Step title="Migrate approval-native handlers to capability facts">
    ...
    - Register channel-owned runtime objects such as clients, tokens, or Bolt
      apps through `daisyclaw/plugin-sdk/channel-runtime-context`
  ...
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `daisyclaw/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "daisyclaw/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "daisyclaw/plugin-sdk/compat";

    // After (modern focused imports)
    import { createChannelReplyPipeline } from "daisyclaw/plugin-sdk/channel-reply-pipeline";
    import { createPluginRuntimeStore } from "daisyclaw/plugin-sdk/runtime-store";
    import { resolveControlCommandGate } from "daisyclaw/plugin-sdk/command-auth";
    ```
    ...
    import { runEmbeddedAgent } from "daisyclaw/extension-api";
  ...
  <Step title="Replace broad infra-runtime imports">
    `daisyclaw/plugin-sdk/infra-runtime` still exists for external
    compatibility, but new code should import the focused helper surface it
    actually needs:

    | Need | Import |
    | --- | --- |
    | System event queue helpers | `daisyclaw/plugin-sdk/system-event-runtime` |
    | Heartbeat wake, event, and visibility helpers | `daisyclaw/plugin-sdk/heartbeat-runtime` |
    | Pending delivery queue drain | `daisyclaw/plugin-sdk/delivery-queue-runtime` |
    | Channel activity telemetry | `daisyclaw/plugin-sdk/channel-activity-runtime` |
    | In-memory dedupe caches | `daisyclaw/plugin-sdk/dedupe-runtime` |
    | Safe local-file/media path helpers | `daisyclaw/plugin-sdk/file-access-runtime` |
    | Dispatcher-aware fetch | `daisyclaw/plugin-sdk/runtime-fetch` |
    | Proxy and guarded fetch helpers | `daisyclaw/plugin-sdk/fetch-runtime` |
    | SSRF dispatcher policy types | `daisyclaw/plugin-sdk/ssrf-dispatcher` |
    | Approval request/resolution types | `daisyclaw/plugin-sdk/approval-runtime` |
    | Approval reply payload and command helpers | `daisyclaw/plugin-sdk/approval-reply-runtime` |
    | Error formatting helpers | `daisyclaw/plugin-sdk/error-runtime` |
    | Transport readiness waits | `daisyclaw/plugin-sdk/transport-ready-runtime` |
    | Secure token helpers | `daisyclaw/plugin-sdk/secure-random-runtime` |
    | Bounded async task concurrency | `daisyclaw/plugin-sdk/concurrency-runtime` |
    | Numeric coercion | `daisyclaw/plugin-sdk/number-runtime` |
    | Process-local async lock | `daisyclaw/plugin-sdk/async-lock-runtime` |
    | File locks | `daisyclaw/plugin-sdk/file-lock` |
  ...
  <Step title="Migrate channel route helpers">
    New channel route code should use `daisyclaw/plugin-sdk/channel-route`.
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `DaisyClawSchema` |
  ...
  | `plugin-sdk/bundled-channel-config-schema` | Bundled config schemas | DaisyClaw-maintained bundled plugins only; new plugins must define plugin-local schemas |
...
`@daisyclaw/discord@2026.3.13` package. Owner-specific helpers live inside the
...
<AccordionGroup>
  <Accordion title="command-auth help builders → command-status">
    **Old (`daisyclaw/plugin-sdk/command-auth`)**: `buildCommandsMessage`,
    `buildCommandsMessagePaginated`, `buildHelpMessage`.

    **New (`daisyclaw/plugin-sdk/command-status`)**: same signatures, same
    exports - just imported from the narrower subpath. `command-auth`
    re-exports them as compat stubs.

    ```typescript
    // Before
    import { buildHelpMessage } from "daisyclaw/plugin-sdk/command-auth";

    // After
    import { buildHelpMessage } from "daisyclaw/plugin-sdk/command-status";
    ```
  ...
  <Accordion title="Mention gating helpers → resolveInboundMentionDecision">
    **Old**: `resolveInboundMentionRequirement({ facts, policy })` and
    ...
    `daisyclaw/plugin-sdk/channel-inbound` or
    `daisyclaw/plugin-sdk/channel-mention-gating`.
  ...
  <Accordion title="Channel runtime shim and channel actions helpers">
    `daisyclaw/plugin-sdk/channel-runtime` is a compatibility shim for older
    channel plugins. Do not import it from new code; use
    `daisyclaw/plugin-sdk/channel-runtime-context` for registering runtime
    objects.

    `channelActions*` helpers in `daisyclaw/plugin-sdk/channel-actions` are
  ...
  <Accordion title="Web search provider tool() helper → createTool() on the plugin">
    **Old**: `tool()` factory from `daisyclaw/plugin-sdk/provider-web-search`.

    **New**: implement `createTool(...)` directly on the provider plugin.
    DaisyClaw no longer needs the SDK helper to register the tool wrapper.
  ...
  <Accordion title="Thinking policy hooks → resolveThinkingProfile">
    ...
    **New**: a single `resolveThinkingProfile(ctx)` that returns a
    ...
    ranked level list. DaisyClaw downgrades stale stored values by profile
    rank automatically.
  ...
  <Accordion title="DaisyClawSchemaType alias → DaisyClawConfig">
    `DaisyClawSchemaType` re-exported from `daisyclaw/plugin-sdk` is now a
    one-line alias for `DaisyClawConfig`. Prefer the canonical name.

    ```typescript
    // Before
    import type { DaisyClawSchemaType } from "daisyclaw/plugin-sdk";
    // After
    import type { DaisyClawConfig } from "daisyclaw/plugin-sdk/config-schema";
    ```
...
```bash
DAISYCLAW_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 daisyclaw gateway run
DAISYCLAW_SUPPRESS_EXTENSION_API_WARNING=1 daisyclaw gateway run
```
