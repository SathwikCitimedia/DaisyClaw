...
read_when:
  ...
  - You want a reference for all registration methods on DaisyClawPluginApi
...
<Note>
  This page is for plugin authors using `daisyclaw/plugin-sdk/*` inside
  DaisyClaw. For external apps, scripts, dashboards, CI jobs, and IDE extensions
...
prefer `daisyclaw/plugin-sdk/channel-core`; keep `daisyclaw/plugin-sdk/core` for
...
`daisyclaw.plugin.json#channelConfigs`. The `plugin-sdk/channel-config-schema`
subpath is for shared schema primitives and the generic builder. DaisyClaw's
...
<Warning>
  ...
  `daisyclaw/plugin-sdk/slack`, `.../discord`, `.../signal`, `.../whatsapp`).
...
`daisyclaw/plugin-sdk/discord` and `daisyclaw/plugin-sdk/telegram-account` are
...
The `register(api)` callback receives an `DaisyClawPluginApi` object with these
methods:
...
agentPromptGuidance: [
  "Global command hint.",
  { text: "Only show this in the main DaisyClaw prompt.", surfaces: ["daisyclaw_main"] },
];
```

Structured `surfaces` may include `daisyclaw_main`, `codex_app_server`,
`cli_backend`, `acp_backend`, or `subagent`. `pi_main` remains a deprecated alias
for `daisyclaw_main`. Omit `surfaces` for intentional all-surface guidance. Do
...
| `api.registerNodeCliFeature(registrar, opts?)` | Node feature CLI under `daisyclaw nodes` |
...
runtime, for example `["daisyclaw", "codex"]`. Installed plugins without that
contract, or without explicit enablement, cannot register this middleware; keep
normal DaisyClaw plugin hooks for work that does not need pre-model tool-result
timing. The old
...
Gateway on a local discovery transport such as mDNS/Bonjour. DaisyClaw calls the
...
`daisyclaw nodes canvas` explicit plugin-owned node features.
...
- User config still wins. DaisyClaw merges `agents.defaults.cliBackends.<id>` over the
...
- Use `resolveExecutionArgs` for request-scoped argv rewrites that belong to
  the CLI dialect, such as mapping DaisyClaw thinking levels to a native effort
  flag.
...
- `registerMemoryCapability` may also expose `publicArtifacts.listArtifacts(...)`
  ...
  `daisyclaw/plugin-sdk/memory-host-core` instead of reaching into a specific
...
- `cron_changed`: observe gateway-owned cron lifecycle changes. Use `event.job?.state?.nextRunAtMs` and `ctx.getCron?.()` when syncing external wake schedulers, and keep DaisyClaw as the source of truth for due checks and execution.
...
| `api.config`             | `DaisyClawConfig`          | Current config snapshot (active in-memory runtime snapshot when available)                  |
...
<Warning>
  Never import your own plugin through `daisyclaw/plugin-sdk/<your-plugin>`
...
active runtime config snapshot when DaisyClaw is already running. If no runtime
snapshot exists yet, they fall back to the resolved config file on disk.
Packaged bundled plugin facades should be loaded through DaisyClaw's plugin
...
- **`@daisyclaw/openai-provider`**: `api.ts` exports provider builders,
  default-model helpers, and realtime provider builders.
- **`@daisyclaw/openrouter-provider`**: `api.ts` exports the provider builder
  plus onboarding/config helpers.

<Warning>
  Extension production code should also avoid `daisyclaw/plugin-sdk/<other-plugin>`
  imports. If a helper is truly shared, promote it to a neutral SDK subpath
  such as `daisyclaw/plugin-sdk/speech`, `.../provider-model-shared`, or another
