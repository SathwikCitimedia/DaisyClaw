---
summary: "Step-by-step guide to building a model provider plugin for DaisyClaw"
title: "Building provider plugins"
sidebarTitle: "Provider plugins"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to DaisyClaw
  - You need to understand provider auth, catalogs, and runtime hooks
---

This guide walks through building a provider plugin that adds a model provider
(LLM) to DaisyClaw. By the end you will have a provider with a model catalog,
...
<Tip>
  Provider plugins add models to DaisyClaw's normal inference loop. If the model
...
<Steps>
  <Step title="Package and manifest">
    ...
    The manifest declares `setup.providers[].envVars` so DaisyClaw can detect
    ...
    is optional and lets DaisyClaw auto-load your provider plugin from shorthand
    model ids like `acme-large` before runtime hooks exist. If you publish the
    provider on ClawHub, those `daisyclaw.compat` and `daisyclaw.build` fields
  ...
  <Step title="Register the provider">
    ...
    import { createProviderApiKeyAuthMethod } from "daisyclaw/plugin-sdk/provider-auth";
    ...
    response mapping in the plugin; DaisyClaw owns the shared row shape, source
    labels, and help rendering.

    That is a working provider. Users can now
    `daisyclaw onboard --acme-ai-api-key <key>` and select
    ...
    `daisyclaw/plugin-sdk/provider-catalog-live-runtime` for the shared fetch
    ...
    putting provider policy in DaisyClaw core.
    ...
    } from "daisyclaw/plugin-sdk/provider-catalog-live-runtime";
    ...
    metadata and the plugin needs to project rows into DaisyClaw model
    definitions itself:
    ...
    If the upstream provider uses different control tokens than DaisyClaw, add a
    ...
    DaisyClaw parses its own control markers or channel delivery.
    ...
    ```typescript
    import { defineSingleProviderPluginEntry } from "daisyclaw/plugin-sdk/provider-entry";
    ...
    `buildProvider` is the live catalog path used when DaisyClaw can resolve real
    ...
    DaisyClaw's `models list --all` display currently executes static catalogs
    ...
    `daisyclaw/plugin-sdk/provider-onboard`. The narrowest helpers are
    ...
    `daisyclaw/plugin-sdk/provider-catalog-shared` instead of hardcoding
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    ```typescript
    import { buildProviderReplayFamilyHooks } from "daisyclaw/plugin-sdk/provider-model-shared";
    import { buildProviderStreamFamilyHooks } from "daisyclaw/plugin-sdk/provider-stream";
    import { buildProviderToolCompatFamilyHooks } from "daisyclaw/plugin-sdk/provider-tools";
    ...
    <Accordion title="SDK seams powering the family builders">
      ...
      - `daisyclaw/plugin-sdk/provider-model-shared` - `ProviderReplayFamily`, `buildProviderReplayFamilyHooks(...)`, and the raw replay builders (`buildOpenAICompatibleReplayPolicy`, `buildAnthropicReplayPolicyForModel`, `buildGoogleGeminiReplayPolicy`, `buildHybridAnthropicOrOpenAIReplayPolicy`). Also exports Gemini replay helpers (`sanitizeGoogleGeminiReplayHistory`, `resolveTaggedReasoningOutputMode`) and endpoint/model helpers (`resolveProviderEndpoint`, `normalizeProviderId`, `normalizeGooglePreviewModelId`).
      - `daisyclaw/plugin-sdk/provider-stream` - `ProviderStreamFamily`, `buildProviderStreamFamilyHooks(...)`, `composeProviderStreamWrappers(...)`, plus the shared OpenAI/Codex wrappers (`createOpenAIAttributionHeadersWrapper`, `createOpenAIFastModeWrapper`, `createOpenAIServiceTierWrapper`, `createOpenAIResponsesContextManagementWrapper`, `createCodexNativeWebSearchWrapper`), DeepSeek V4 OpenAI-compatible wrapper (`createDeepSeekV4OpenAICompatibleThinkingWrapper`), Anthropic Messages thinking prefill cleanup (`createAnthropicThinkingPrefillPayloadWrapper`), plain-text tool-call compat (`createPlainTextToolCallCompatWrapper`), and shared proxy/provider wrappers (`createOpenRouterWrapper`, `createToolStreamWrapper`, `createMinimaxFastModeWrapper`).
      - `daisyclaw/plugin-sdk/provider-tools` - `ProviderToolCompatFamily`, `buildProviderToolCompatFamilyHooks("deepseek" | "gemini" | "openai")`, and underlying provider schema helpers.
      ...
      reasoning output so DaisyClaw consumes native thought parts without adding
      ...
      Some stream helpers stay provider-local on purpose. `@daisyclaw/anthropic-provider` keeps `wrapAnthropicProviderStream`, `resolveAnthropicBetas`, `resolveAnthropicFastMode`, `resolveAnthropicServiceTier`, and the lower-level Anthropic wrapper builders in its own public `api.ts` / `contract-api.ts` seam because they encode Claude OAuth beta handling and `context1m` gating. The xAI plugin similarly keeps native xAI Responses shaping in its own `wrapStreamFn` (`/fast` aliases, default `tool_stream`, unsupported strict-tool cleanup, xAI-specific reasoning-payload removal).

      The same package-root pattern also backs `@daisyclaw/openai-provider` (provider builders, default-model helpers, realtime provider builders) and `@daisyclaw/openrouter-provider` (provider builder plus onboarding/config helpers).
    </Accordion>
    ...
    <Tabs>
      ...
      <Tab title="Usage and billing">
        ...
        auth but has no usable usage token, and DaisyClaw must skip generic
        API-key/OAuth fallback. Return `null` or `undefined` when the provider did
        not handle the request and DaisyClaw should continue with generic fallback.
      </Tab>
    </Tabs>

    <Accordion title="All available provider hooks">
      DaisyClaw calls hooks in this order. Most providers only use 2-3:
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    web fetch, and web search alongside text inference. DaisyClaw classifies this as a
    **hybrid-capability** plugin - the recommended pattern for company plugins
    ...
    <Tabs>
      ...
      <Tab title="Realtime transcription">
        ...
        `daisyclaw/plugin-sdk/provider-http`. The helper normalizes upload
      ...
      <Tab title="Media understanding">
        ...
        DaisyClaw still keeps the normal auth gate for providers that do not
...
├── package.json              # daisyclaw.providers metadata
├── daisyclaw.plugin.json      # Manifest with provider auth metadata
