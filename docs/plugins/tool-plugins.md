---
summary: "Build simple typed agent tools with defineToolPlugin and daisyclaw plugins init/build/validate"
title: "Tool plugins"
sidebarTitle: "Tool Plugins"
read_when:
  - You want to build a simple DaisyClaw plugin that only adds agent tools
...
Tool plugins add agent-callable tools to DaisyClaw without adding a channel,
model provider, hook, service, or setup backend. Use `defineToolPlugin` when the
plugin owns a fixed list of tools and you want DaisyClaw to generate the manifest
metadata that keeps those tools discoverable without loading runtime code.

The recommended flow is:

1. Scaffold a package with `daisyclaw plugins init`.
2. Write tools with `defineToolPlugin`.
3. Build JavaScript.
4. Generate `daisyclaw.plugin.json` and `package.json` metadata with
   `daisyclaw plugins build`.
...
- `daisyclaw >=2026.5.17`, the first DaisyClaw version that exports
  `daisyclaw/plugin-sdk/tool-plugin`.
- A package root that can ship `dist/`, `daisyclaw.plugin.json`, and
  `package.json`.
...
- `daisyclaw.plugin.json`: generated manifest metadata for the initial tool.
...
import { defineToolPlugin } from "daisyclaw/plugin-sdk/tool-plugin";
...
`daisyclaw plugins build` writes the matching `toolMetadata.<tool>.optional`
manifest entry, so DaisyClaw can discover the tool without loading plugin
runtime code.
...
`defineToolPlugin` wraps plain return values into the DaisyClaw tool-result
format:
...
- Return a JSON-compatible value when you want the model to see formatted JSON
  and DaisyClaw to keep the original value in `details`.
...
`configSchema` is optional. If you omit it, DaisyClaw uses a strict empty object
...
DaisyClaw reads plugin config from the plugin entry in the Gateway config. Do not
...
DaisyClaw discovers installed plugins from cold metadata. It must be able to read
the plugin manifest before importing plugin runtime code. `defineToolPlugin`
therefore exposes static metadata, and `daisyclaw plugins build` writes that
...
`contracts.tools` is the important discovery contract. It tells DaisyClaw which
...
For the simple tool-plugin workflow, `daisyclaw plugins build` aligns
`package.json` to the selected single runtime entry:

```json
{
  "type": "module",
  "files": ["dist", "daisyclaw.plugin.json", "README.md"],
  "dependencies": {
    "typebox": "^1.1.38"
  },
  "peerDependencies": {
...
- `daisyclaw.plugin.json` exists and passes the normal manifest loader.
...
- `package.json` points `daisyclaw.extensions` at the selected runtime entry.

## Install and inspect locally

From a separate DaisyClaw checkout or installed CLI, install the package path:

```bash
daisyclaw plugins install ./stock-quotes
daisyclaw plugins inspect stock-quotes --runtime
```
...
```bash
npm pack
daisyclaw plugins install npm-pack:./daisyclaw-plugin-stock-quotes-0.1.0.tgz
daisyclaw plugins inspect stock-quotes --runtime --json
```
...
```bash
daisyclaw plugins install clawhub:your-org/stock-quotes
```

Bare npm package specs remain supported during the launch cutover, but ClawHub
is the preferred discovery and distribution surface for DaisyClaw plugins.
...
`daisyclaw plugins build --entry ./dist/index.js` or
`daisyclaw plugins validate --entry ./dist/index.js`.
...
### `daisyclaw.plugin.json generated metadata is stale`
...
Commit both `daisyclaw.plugin.json` and `package.json` changes.

### `package.json daisyclaw.extensions must include ./dist/index.js`

The package metadata points at a different runtime entry. Run
`daisyclaw plugins build --entry ./dist/index.js` so the generator aligns the
...
1. `daisyclaw plugins inspect <plugin-id> --runtime`
2. `daisyclaw plugins validate --root <plugin-root> --entry ./dist/index.js`
3. `daisyclaw.plugin.json` has `contracts.tools` with the expected tool names.
4. `package.json` has `daisyclaw.extensions: ["./dist/index.js"]`.
