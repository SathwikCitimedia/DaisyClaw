...
read_when:
  - You want to place an outbound voice call from DaisyClaw
...
Voice calls for DaisyClaw via a plugin. Supports outbound notifications,
...
<Steps>
  <Step title="Install the plugin">
    <Tabs>
      <Tab title="From npm">
        ```bash
        daisyclaw plugins install @daisyclaw/voice-call
        ```
      </Tab>
...
<AccordionGroup>
  ...
  <Accordion title="Legacy config migrations">
    ...
    `streaming.*` OpenAI keys are rewritten by `daisyclaw doctor --fix`.
    Runtime fallback still accepts the old voice-call keys for now, but
    the rewrite path is `daisyclaw doctor --fix` and the compat shim is
    temporary.
...
- Voice Call exposes the shared `daisyclaw_agent_consult` realtime tool by default. The realtime model can call it when the caller asks for deeper reasoning, current information, or normal DaisyClaw tools.
- `realtime.consultPolicy` optionally adds guidance for when the realtime model should call `daisyclaw_agent_consult`.
...
configured DaisyClaw agent without paying a full agent-consult round trip on
...
`daisyclaw_agent_consult` still run the full DaisyClaw agent and should be used
          ...
          "voice-call": {
            config: {
              ...
              realtime: {
                enabled: true,
                provider: "google",
                instructions: "Speak briefly. Call daisyclaw_agent_consult before using deeper tools.",
                toolPolicy: "safe-read-only",
                consultPolicy: "substantive",
...
- Legacy `tts.<provider>` keys inside plugin config (`openai`, `elevenlabs`, `microsoft`, `edge`) are repaired by `daisyclaw doctor --fix`; committed config should use `tts.providers.<provider>`.
...
```bash
daisyclaw voicecall call --to "+15555550123" --message "Hello from DaisyClaw"
daisyclaw voicecall start --to "+15555550123"   # alias for call
daisyclaw voicecall continue --call-id <id> --message "Any questions?"
daisyclaw voicecall speak --call-id <id> --message "One moment"
daisyclaw voicecall dtmf --call-id <id> --digits "ww123456#"
daisyclaw voicecall end --call-id <id>
daisyclaw voicecall status --call-id <id>
daisyclaw voicecall tail
daisyclaw voicecall latency                      # summarize turn latency from logs
daisyclaw voicecall expose --mode funnel
```
...
```bash
daisyclaw voicecall setup
daisyclaw voicecall setup --json
```
...
Provider signatures are checked against the public URL DaisyClaw reconstructs
...
Use `daisyclaw logs --follow` for the live phase trace. A healthy Twilio Meet
...
`daisyclaw voicecall tail` still shows persisted call records; it is useful for
...
- `daisyclaw logs --follow` shows realtime TwiML served, the realtime bridge
