...
DaisyClaw Gateway; it is not a replacement for GitHub Issues, Linear, Jira, or
...
state directory and move with the rest of that Gateway's DaisyClaw state.
...
under the DaisyClaw state directory. Boards, cards, labels, lifecycle events,
...
`daisyclaw doctor --fix` to migrate the shipped legacy plugin-state namespaces
...
system processes; normal DaisyClaw subagent sessions still own execution. The
...
DaisyClaw records the orchestration intent and exposes it in worker context; the
...
daisyclaw workboard create "Fix stale card lifecycle" --priority high --labels bug,workboard
daisyclaw workboard show <card-id>
daisyclaw workboard dispatch
```

`daisyclaw workboard dispatch` calls the running Gateway so worker starts use the
...
```bash
daisyclaw plugins disable workboard
daisyclaw gateway restart
```
