...
summary: "OpenProse is a markdown-first workflow format for multi-agent AI sessions. In DaisyClaw it ships as a plugin with a /prose slash command and a skill pack."
read_when:
  - You want to run or write .prose workflow files
  - You want to enable the OpenProse plugin
  - You need to understand how OpenProse maps to DaisyClaw primitives
---

OpenProse is a portable, markdown-first workflow format for orchestrating AI
sessions. In DaisyClaw it ships as a plugin that installs an OpenProse skill
...
<Steps>
  <Step title="Enable the plugin">
    ...
    ```bash
    daisyclaw plugins enable open-prose
    ```
  ...
  <Step title="Verify">
    ```bash
    daisyclaw plugins list | grep prose
    ```
...
For a local checkout: `daisyclaw plugins install ./path/to/local/open-prose-plugin`
...
## DaisyClaw runtime mapping

OpenProse programs map to DaisyClaw primitives:

| OpenProse concept         | DaisyClaw tool    |
| ------------------------- | ---------------- |
...
Treat `.prose` files like code. Review them before running. Use DaisyClaw tool
...
<CardGroup cols={2}>
  ...
  <Card title="Subagents" href="/tools/subagents" icon="users">
    DaisyClaw's native multi-agent coordination layer.
  </Card>
