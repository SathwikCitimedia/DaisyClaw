---
summary: "Use Anthropic Claude via API keys or Claude CLI in DaisyClaw"
read_when:
  - You want to use Anthropic models in DaisyClaw
title: "Anthropic"
---

Anthropic builds the **Claude** model family. DaisyClaw supports two auth routes:
...
<Warning>
DaisyClaw's Claude CLI backend runs the installed Claude Code CLI in
    ...
    **Best for:** standard API access and usage-based billing.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        daisyclaw onboard --anthropic-api-key "$ANTHROPIC_API_KEY"
        ```
      </Step>
      <Step title="Verify the model is available">
        ```bash
        daisyclaw models list --provider anthropic
        ```
      </Step>
    </Steps>
    ...
    **Best for:** reusing an existing Claude CLI login without a separate API key.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        DaisyClaw detects and reuses the existing Claude CLI credentials.
      </Step>
      <Step title="Verify the model is available">
        ```bash
        daisyclaw models list --provider anthropic
    ...
    <Warning>
    Claude CLI reuse expects the DaisyClaw process to run on the same host as the
    ...
    there, or choose a provider with DaisyClaw-managed OAuth such as
    ...
    DaisyClaw uses Claude Code's non-interactive `claude -p` path for Claude CLI
    ...
    DaisyClaw release. Check `claude auth status`, `/status`, and
    ...
    Claude CLI. DaisyClaw also supports subscription-style options from
...
`/think off` and `/think minimal` use `low` effort. DaisyClaw also omits custom
temperature values for Fable 5 requests.

Claude Opus 4.8 keeps thinking off by default in DaisyClaw. When you explicitly enable adaptive thinking with `/think high|xhigh|max`, DaisyClaw sends Anthropic's Opus 4.8 effort values; Claude 4.6 models default to `adaptive`.
...
DaisyClaw supports Anthropic's prompt caching feature for API-key auth.
...
<AccordionGroup>
  <Accordion title="Fast mode">
    DaisyClaw's shared `/fast` toggle supports direct Anthropic traffic (API-key and OAuth to `api.anthropic.com`).
  ...
  <Accordion title="Media understanding (image and PDF)">
    The bundled Anthropic plugin registers image and PDF understanding. DaisyClaw
    ...
    When an image or PDF is attached to a conversation, DaisyClaw automatically
  ...
  <Accordion title="1M context window">
    ...
    such as Opus 4.8, Opus 4.7, Opus 4.6, and Sonnet 4.6. DaisyClaw sizes those models at
    1M automatically:
    ...
    Older configs can keep `params.context1m: true`, but DaisyClaw no longer sends
    ...
    <Warning>
    Requires long-context access on your Anthropic credential. OAuth/subscription token auth keeps its required Anthropic beta headers, but DaisyClaw strips the retired 1M beta header if it remains in older config.
    </Warning>
...
<AccordionGroup>
  ...
  <Accordion title='No API key found for provider "anthropic"'>
    Anthropic auth is **per agent** — new agents do not inherit the main agent's keys. Re-run onboarding for that agent (or configure an API key on the gateway host), then verify with `daisyclaw models status`.
  </Accordion>

  <Accordion title='No credentials found for profile "anthropic:default"'>
    Run `daisyclaw models status` to see which auth profile is active. Re-run onboarding, or configure an API key for that profile path.
  </Accordion>

  <Accordion title="No available auth profile (all in cooldown)">
    Check `daisyclaw models status --json` for `auth.unusableProfiles`. Anthropic rate-limit cooldowns can be model-scoped, so a sibling Anthropic model may still be usable. Add another Anthropic profile or wait for cooldown.
  </Accordion>
</AccordionGroup>
