---
summary: "Use DeepInfra's unified API to access the most popular open source and frontier models in DaisyClaw"
read_when:
  - You want a single API key for the top open source LLMs
  - You want to run models via DeepInfra's API in DaisyClaw
title: "DeepInfra"
---
...
```bash
daisyclaw onboard --deepinfra-api-key <key>
```
...
## Supported DaisyClaw surfaces

The bundled plugin registers all DeepInfra surfaces that match current
DaisyClaw provider contracts. Chat, image generation, and video generation
refresh their model catalogues live from `/v1/openai/models?sort_by=daisyclaw&filter=with_meta`
when `DEEPINFRA_API_KEY` is configured; the other surfaces use the curated
static defaults below.

| Surface                  | Default model                                                                                         | DaisyClaw config/tool                                     |
| ------------------------ | ----------------------------------------------------------------------------------------------------- | -------------------------------------------------------- |
...
native model types. DaisyClaw does not currently have first-class provider
contracts for those categories, so this plugin does not register them yet.

## Available models

DaisyClaw dynamically discovers available DeepInfra models at startup. Use
