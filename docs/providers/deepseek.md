...
title: "DeepSeek"
read_when:
  - You want to use DeepSeek with DaisyClaw
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    daisyclaw onboard --auth-choice deepseek-api-key
    ```
  ...
  <Step title="Verify models are available">
    ```bash
    daisyclaw models list --provider deepseek
    ```
    ...
    ```bash
    daisyclaw models list --all --provider deepseek
    ```
...
<Tip>
V4 models support DeepSeek's `thinking` control. DaisyClaw also replays
...
`reasoning_content` on follow-up requests. DaisyClaw handles this inside the
...
DeepSeek `reasoning_content`. DaisyClaw fills that missing field on replayed
assistant messages for DeepSeek V4 thinking requests so the provider can accept
the history without requiring `/new`.

When thinking is disabled in DaisyClaw (including the UI **None** selection),
DaisyClaw sends DeepSeek `thinking: { type: "disabled" }` and strips replayed
...
```bash
DAISYCLAW_LIVE_PROVIDERS=deepseek \
DAISYCLAW_LIVE_MODELS="deepseek/deepseek-v4-flash,deepseek/deepseek-v4-pro" \
