---
summary: "Run DaisyClaw through ds4, a local DeepSeek V4 Flash OpenAI-compatible server"
read_when:
  - You want to run DaisyClaw against antirez/ds4
  - You want a local DeepSeek V4 Flash backend with tool calls
  - You need the DaisyClaw config for ds4-server
title: "ds4"
---

[ds4](https://github.com/antirez/ds4) serves DeepSeek V4 Flash from a local
Metal backend with an OpenAI-compatible `/v1` API. DaisyClaw connects to ds4
through the generic `openai-completions` provider family.

ds4 is not a bundled DaisyClaw provider plugin. Configure it under
...
<Warning>
DaisyClaw agent turns include tool schemas and workspace context. A tiny context
...
<Steps>
  ...
  <Step title="Add the DaisyClaw provider config">
    ...
    daisyclaw infer model run \
      ...
      --thinking off \
      --prompt "Reply with exactly: daisyclaw-ds4-ok" \
      --json
    ```
...
aligned with `--tokens` unless you intentionally want DaisyClaw to request less
output than the server default.

## On-demand startup

DaisyClaw can start ds4 only when a `ds4/...` model is selected. Add
...
If you run that large context, update both the server flags and DaisyClaw model
metadata:
...
Then test DaisyClaw model routing:
...
<AccordionGroup>
  ...
  <Accordion title="500 prompt exceeds context">
    The configured `--ctx` is too small for the DaisyClaw turn. Raise
  ...
  <Accordion title="The first request is slow">
    ...
    `localService.readyTimeoutMs: 300000` when DaisyClaw starts the server on
    demand.
  </Accordion>
</AccordionGroup>
