---
summary: "fal image, video, and music generation setup in DaisyClaw"
title: "Fal"
read_when:
  - You want to use fal image generation in DaisyClaw
  - You need the FAL_KEY auth flow
  - You want fal defaults for image_generate, video_generate, or music_generate
---

DaisyClaw ships a bundled `fal` provider for hosted image, video, and music
generation.
...
<Steps>
  <Step title="Set the API key">
    ```bash
    daisyclaw onboard --auth-choice fal-api-key
    ```
  </Step>
...
Krea 2 models use fal's native Krea payload schema. DaisyClaw sends
...
`aspectRatio`; DaisyClaw maps `size` to the closest supported Krea aspect ratio
...
control in DaisyClaw, so `background: "transparent"` is reported as an ignored
override for fal models.
Krea 2 endpoints do not expose an `output_format` request field through fal, so
DaisyClaw rejects `outputFormat` overrides for Krea requests.
...
<Tip>
Use `daisyclaw models list --provider fal` to see the full list of available fal
