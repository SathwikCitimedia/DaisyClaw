---
summary: "Use GMI Cloud's OpenAI-compatible API with DaisyClaw"
read_when:
  - You want to run DaisyClaw with GMI Cloud models
  - You need the GMI provider id, key, or endpoint
title: "GMI Cloud"
---

GMI Cloud is a hosted inference platform for frontier and open-weight models
behind an OpenAI-compatible API. In DaisyClaw it is a bundled model provider,
...
This provider uses OpenAI-compatible chat semantics. DaisyClaw owns the provider
...
```bash
daisyclaw onboard --auth-choice gmi-api-key
```
...
all times. Use DaisyClaw's model listing command to see what the configured
provider reports in your environment:

```bash
daisyclaw models list --provider gmi
```

## Troubleshooting

- `401` or `403`: check that `GMI_API_KEY` is set for the process running
  DaisyClaw, or re-run onboarding to store the key in the provider auth profile.
- Unknown model errors: confirm the model exists in your GMI account and use the
  full `gmi/<route-id>` ref shown by `daisyclaw models list --provider gmi`.
