---
summary: "Run DaisyClaw through inferrs (OpenAI-compatible local server)"
read_when:
  - You want to run DaisyClaw against a local inferrs server
  - You are serving Gemma or another model through inferrs
  - You need the exact DaisyClaw compat flags for inferrs
title: "Inferrs"
---

[inferrs](https://github.com/ericcurtin/inferrs) can serve local models behind an OpenAI-compatible `/v1` API. DaisyClaw works with `inferrs` through the generic `openai-completions` path.

| Property           | Value                                                              |
| ------------------ | ------------------------------------------------------------------ |
| Provider id        | `inferrs` (custom; configure under `models.providers.inferrs`)     |
| Plugin             | none — `inferrs` is not a bundled DaisyClaw provider plugin         |
...
<Note>
  `inferrs` is currently best treated as a custom self-hosted OpenAI-compatible backend, not a dedicated DaisyClaw provider plugin. You configure it through `models.providers.inferrs` rather than an onboarding choice flag. If you need a true bundled plugin with auto-discovery, see [SGLang](/providers/sglang) or [vLLM](/providers/vllm).
</Note>
...
<Steps>
  ...
  <Step title="Add an DaisyClaw provider entry">
...
Inferrs can also be started by DaisyClaw only when an `inferrs/...` model is
...
<AccordionGroup>
  <Accordion title="Why requiresStringContent matters">
    ...
    <Warning>
    If DaisyClaw runs fail with an error like:
    ...
    DaisyClaw will flatten pure text content parts into plain strings before sending
    the request.
  ...
  <Accordion title="Gemma and tool-schema caveat">
    ...
    `/v1/chat/completions` requests but still fail on full DaisyClaw agent-runtime
    turns.
    ...
    That disables DaisyClaw's tool schema surface for the model and can reduce prompt
    pressure on stricter local backends.

    If tiny direct requests still work but normal DaisyClaw agent turns continue to
    crash inside `inferrs`, the remaining issue is usually upstream model/server
    behavior rather than DaisyClaw's transport layer.
...
<AccordionGroup>
  ...
  <Accordion title="Direct /v1/chat/completions calls pass but daisyclaw infer model run fails">
  ...
  <Accordion title="inferrs still crashes on larger agent turns">
    If DaisyClaw no longer gets schema errors but `inferrs` still crashes on larger
...
<CardGroup cols={2}>
  <Card title="Local models" href="/gateway/local-models" icon="server">
    Running DaisyClaw against local model servers.
  </Card>
