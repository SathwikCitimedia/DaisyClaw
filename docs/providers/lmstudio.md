---
summary: "Run DaisyClaw with LM Studio"
read_when:
  - You want to run DaisyClaw with open source models via LM Studio
...
If LM Studio authentication is disabled, you can leave the API key blank during interactive DaisyClaw setup.
...
```bash
daisyclaw models set lmstudio/qwen/qwen3.5-9b
```

LM Studio model keys follow a `author/model-name` format (e.g. `qwen/qwen3.5-9b`). DaisyClaw
...
For unauthenticated LM Studio servers, omit the key; DaisyClaw stores a local non-secret marker.
...
`usage` object, DaisyClaw recovers token counts from llama.cpp-style
...
options, DaisyClaw exposes the matching OpenAI-compatible `reasoning_effort`
...
on `/v1/chat/completions`; DaisyClaw normalizes that binary discovery shape to
...
LM Studio supports just-in-time (JIT) model loading, where models are loaded on first request. DaisyClaw preloads models through LM Studio's native load endpoint by default, which helps when JIT is disabled. To let LM Studio's JIT, idle TTL, and auto-evict behavior own model lifecycle, disable DaisyClaw's preload step:
