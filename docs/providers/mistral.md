---
summary: "Use Mistral models and Voxtral transcription with DaisyClaw"
read_when:
  - You want to use Mistral models in DaisyClaw
  - You want Voxtral realtime transcription for Voice Call
  - You need Mistral API key onboarding and model refs
title: "Mistral"
---

DaisyClaw includes a bundled Mistral plugin that registers four contracts: chat completions, media understanding (Voxtral batch transcription), realtime STT for Voice Call (Voxtral Realtime), and memory embeddings (`mistral-embed`).
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    daisyclaw onboard --auth-choice mistral-api-key
    ```

    Or pass the key directly:

    ```bash
    daisyclaw onboard --mistral-api-key "$MISTRAL_API_KEY"
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    daisyclaw models list --provider mistral
    ```
  </Step>
</Steps>
...
DaisyClaw currently ships this bundled Mistral catalog:
...
```bash
daisyclaw infer model run --local \
...
```bash
daisyclaw models list --all --provider mistral --plain
```
...
<Note>
DaisyClaw defaults Mistral realtime STT to `pcm_mulaw` at 8 kHz so Voice Call
...
<AccordionGroup>
  <Accordion title="Adjustable reasoning">
    ...
    DaisyClaw maps the session **thinking** level to Mistral's API:

    | DaisyClaw thinking level                          | Mistral `reasoning_effort` |
    | ------------------------------------------------ | -------------------------- |
    | **off** / **minimal**                            | `none`                     |
    ...
    answers, turn thinking off/minimal so DaisyClaw sends
