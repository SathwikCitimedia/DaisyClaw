    ...
    **Best for:** Kimi K2 models via the Moonshot Open Platform.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        daisyclaw onboard --auth-choice moonshot-api-key
        ```

        Or for the China endpoint:

        ```bash
        daisyclaw onboard --auth-choice moonshot-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify models are available">
        ```bash
        daisyclaw models list --provider moonshot
        ```
      </Step>
      <Step title="Run a live smoke test">
        ...
        ```bash
        DAISYCLAW_CONFIG_PATH=/tmp/daisyclaw-kimi/daisyclaw.json \
        DAISYCLAW_STATE_DIR=/tmp/daisyclaw-kimi \
  ...
  <Tab title="Kimi Coding">
    **Best for:** code-focused tasks via the Kimi Coding endpoint.
    ...
    <Steps>
      <Step title="Run onboarding">
        ```bash
        daisyclaw onboard --auth-choice kimi-code-api-key
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
      ...
      <Step title="Verify the model is available">
        ```bash
        daisyclaw models list --provider kimi
        ```
      </Step>
    </Steps>
...
DaisyClaw also ships **Kimi** as a `web_search` provider, backed by Moonshot web
search.
...
<AccordionGroup>
  <Accordion title="Native thinking mode">
    ...
    DaisyClaw also maps runtime `/think` levels for Moonshot:
    ...
    <Warning>
    When Moonshot thinking is enabled, `tool_choice` must be `auto` or `none`. DaisyClaw normalizes incompatible `tool_choice` values to `auto` for compatibility.
    </Warning>
    ...
    default strategy. DaisyClaw only forwards `thinking.keep` for
  ...
  <Accordion title="Tool call id sanitization">
    Moonshot Kimi serves tool_call ids shaped like `functions.<name>:<index>`. DaisyClaw preserves them unchanged so multi-turn tool calls keep working.
