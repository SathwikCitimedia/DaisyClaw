---
summary: "Use NovitaAI's OpenAI-compatible API with DaisyClaw"
read_when:
  - You want to run DaisyClaw with NovitaAI models
...
API. In DaisyClaw it is a bundled model provider, so the provider id is
...
This provider uses Novita's OpenAI-compatible endpoint. DaisyClaw handles
...
```bash
daisyclaw onboard --auth-choice novita-api-key
```
...
The catalog is a starting point for DaisyClaw model selection. Your account,
...
```bash
daisyclaw models list --provider novita
```

## Troubleshooting

- `401` or `403`: verify the key in Novita's key management page and re-run
  `daisyclaw onboard --auth-choice novita-api-key` if the stored profile is
  stale.
