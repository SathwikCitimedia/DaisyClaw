---
summary: "Use NVIDIA's OpenAI-compatible API in DaisyClaw"
read_when:
  - You want to use open models in DaisyClaw for free
...
[build.nvidia.com](https://build.nvidia.com/settings/api-keys). DaisyClaw
...
<Steps>
  ...
  <Step title="Export the key and run onboarding">
    ...
    daisyclaw onboard --auth-choice nvidia-api-key
    ```
  </Step>
  <Step title="Set an NVIDIA model">
    ```bash
    daisyclaw models set nvidia/nvidia/nemotron-3-ultra-550b-a55b
    ```
  </Step>
</Steps>
...
```bash
daisyclaw onboard --auth-choice nvidia-api-key --nvidia-api-key "nvapi-..."
```
...
When an NVIDIA API key is configured, DaisyClaw setup and model-selection paths
...
DaisyClaw release. When the live feed is available, the first returned model is
...
malformed, DaisyClaw falls back to the bundled catalog and bundled default below.

## Nemotron 3 Ultra

Nemotron 3 Ultra is the default NVIDIA model in DaisyClaw. NVIDIA's build page for
...
<AccordionGroup>
  ...
  <Accordion title="Catalog and pricing">
    DaisyClaw prefers NVIDIA's public featured-model catalog when NVIDIA auth is
  ...
  <Accordion title="Nemotron 3 Ultra reasoning params">
    ...
    and `reasoning_budget` for reasoning output. DaisyClaw's bundled Ultra row
