---
summary: "Use Ollama Cloud directly with DaisyClaw"
read_when:
...
Ollama Cloud is Ollama's hosted model API. It lets DaisyClaw call Ollama-hosted
...
`/api/chat` style, not the OpenAI-compatible `/v1` route. DaisyClaw registers it
...
- You want the same native Ollama chat API shape DaisyClaw uses for local
...
DaisyClaw discovers Ollama Cloud models from the live hosted catalog. Commonly
...
```bash
daisyclaw models list --provider ollama-cloud
daisyclaw models set ollama-cloud/kimi-k2.6
```
...
DAISYCLAW_LIVE_OLLAMA_MODEL=kimi-k2.6 \
...
- Unknown model errors: run `daisyclaw models list --provider ollama-cloud` and
