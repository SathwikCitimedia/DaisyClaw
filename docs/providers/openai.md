---
summary: "Use OpenAI via API keys or Codex subscription in DaisyClaw"
read_when:
  - You want to use OpenAI models in DaisyClaw
...
ChatGPT-plan coding agent through OpenAI's Codex clients. DaisyClaw uses one
provider id, `openai`, for both auth shapes.

DaisyClaw uses `openai/*` as the canonical OpenAI model route. Embedded agent
...
- **Legacy config** - legacy Codex model refs are repaired by
  `daisyclaw doctor --fix` to `openai/*` plus the Codex runtime.

OpenAI explicitly supports subscription OAuth usage in external tools and workflows like DaisyClaw.
...
| Direct API-key billing through explicit DaisyClaw     | `openai/gpt-5.5` plus provider/model runtime `daisyclaw`  | Select a normal `openai` API-key profile.                             |
| Latest ChatGPT Instant API alias                     | `openai/chat-latest`                                     | Direct API-key only. Moving alias for experiments, not the default.   |
| ChatGPT/Codex subscription auth through DaisyClaw     | `openai/gpt-5.5` plus provider/model runtime `daisyclaw`  | Select an `openai` OAuth profile for the compatibility route.         |
...
| legacy OpenAI Codex prefix              | Legacy prefix     | Older model/profile namespace. `daisyclaw doctor --fix` migrates it to `openai`.                   |
| `codex` plugin                          | Plugin            | Bundled DaisyClaw plugin that provides native Codex app-server runtime and `/codex` chat controls. |
...
`auth.order.openai` for config; `daisyclaw doctor --fix` rewrites legacy
...
DaisyClaw runtime config remains available as an opt-in compatibility route. When DaisyClaw is
explicitly selected with an `openai` OAuth profile, DaisyClaw keeps the
public model ref as `openai/*` and routes internally through the Codex-auth
transport. Run `daisyclaw doctor --fix` to repair stale
legacy Codex model refs, `codex-cli/*`, or old runtime session pins that do not come from
explicit runtime config.
</Note>

## DaisyClaw feature coverage

| OpenAI capability         | DaisyClaw surface                                                                              | Status                                                                 |
| ------------------------- | --------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
...
the `openai` API-key auth profile created by `daisyclaw onboard --auth-choice openai-api-key`,
...
DaisyClaw install, but they do not configure Realtime voice.
</Note>

## Memory embeddings

DaisyClaw can use OpenAI, or an OpenAI-compatible embedding endpoint, for
...
`queryInputType` and `documentInputType` under `memorySearch`. DaisyClaw forwards
    ...
    **Best for:** direct API access and usage-based billing.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        daisyclaw onboard --auth-choice openai-api-key
        ```

        Or pass the key directly:

        ```bash
        daisyclaw onboard --openai-api-key "$OPENAI_API_KEY"
        ```
      </Step>
      <Step title="Verify the model is available">
        ```bash
        daisyclaw models list --provider openai
        ```
      </Step>
    </Steps>
    ...
    | `openai/gpt-5.5`      | provider/model `agentRuntime.id: "daisyclaw"`              | DaisyClaw embedded runtime      | Selected `openai` profile |
    ...
    non-agent OpenAI API surfaces. Run `daisyclaw doctor --fix` to migrate older
    ...
    DaisyClaw normalizes incompatible OpenAI text-verbosity overrides for this
    model.

    <Warning>
    DaisyClaw does **not** expose `gpt-5.3-codex-spark` on the direct OpenAI API-key route. It is available only through Codex subscription catalog entries when your signed-in account exposes it.
    </Warning>
    ...
    **Best for:** using your ChatGPT/Codex subscription with native Codex app-server execution instead of a separate API key. Codex cloud requires ChatGPT sign-in.
    ...
    <Steps>
      <Step title="Run Codex OAuth">
        ```bash
        daisyclaw onboard --auth-choice openai
        ```
        ...
        ```bash
        daisyclaw models auth login --provider openai --device-code
        ```
      </Step>
      <Step title="Use the canonical OpenAI model route">
        ```bash
        daisyclaw config set agents.defaults.model.primary openai/gpt-5.5
        ```

        No runtime config is required for the default path. OpenAI agent turns
        select the native Codex app-server runtime automatically, and DaisyClaw
    ...
    | `openai/gpt-5.5` | provider/model `agentRuntime.id: "daisyclaw"` | DaisyClaw embedded runtime with internal Codex-auth transport | Selected `openai` OAuth profile |
    ...
    legacy Codex GPT refs are legacy DaisyClaw routes, not the native Codex runtime
    path; run `daisyclaw doctor --fix` when you want to migrate them to canonical
    ...
    auth order under `openai`. DaisyClaw will try the subscription first, then
    ...
    <Note>
    Onboarding no longer imports OAuth material from `~/.codex`. Sign in with browser OAuth (default) or the device-code flow above — DaisyClaw manages the resulting credentials in its own agent auth store.
    </Note>
    ...
    daisyclaw models auth list --provider openai
    daisyclaw config get agents.defaults.model --json
    daisyclaw config get models.providers.openai.agentRuntime --json
    ```

    For a specific agent, add `--agent <id>`:

    ```bash
    daisyclaw models status --agent <id>
    daisyclaw models auth list --agent <id> --provider openai
    ```
    ...
    ```bash
    daisyclaw models auth login --provider openai
    daisyclaw models status --probe --probe-provider openai
    ```
    ...
    ```bash
    daisyclaw models auth login --provider openai --profile-id openai:ritsuko
    daisyclaw models auth login --provider openai --profile-id openai:lain
    ```

    `openai/*` is the model route for OpenAI agent turns through Codex. Run
    `daisyclaw doctor --fix` to migrate older legacy OpenAI Codex prefix profile ids and
    ...
    config explicitly pins DaisyClaw.

    ### Doctor warning

    If legacy Codex model refs or stale OpenAI runtime pins remain in config or
    session state, `daisyclaw doctor --fix` rewrites them to `openai/*` with the
    Codex runtime unless DaisyClaw is explicitly configured.

    ### Context window cap

    DaisyClaw treats model metadata and the runtime context cap as separate values.
    ...
    DaisyClaw uses upstream Codex catalog metadata for `gpt-5.5` when it is
    present. If live Codex discovery omits the `gpt-5.5` row while
    the account is authenticated, DaisyClaw synthesizes that OAuth model row so
...
still account-based. DaisyClaw selects auth in this order:
...
profile is selected, DaisyClaw also keeps `CODEX_API_KEY` and `OPENAI_API_KEY`
...
Codex usage limit, DaisyClaw can rotate to the next ordered `openai:*` API-key
...
still accepted. DaisyClaw also protects the public OpenAI and
...
`daisyclaw infer image edit` when starting from an input file.
...
`openai` OAuth profile is configured, DaisyClaw resolves that stored OAuth
...
`browser.ssrfPolicy.dangerouslyAllowPrivateNetwork: true`; DaisyClaw keeps
...
```
/tool image_generate model=openai/gpt-image-2 prompt="A polished launch poster for DaisyClaw on macOS" size=3840x2160 count=1
```
...
DaisyClaw adds a shared GPT-5 prompt contribution for GPT-5-family runs on DaisyClaw-assembled prompt surfaces. It applies by model id, so DaisyClaw/provider routes such as legacy pre-repair refs (legacy Codex GPT-5.5 ref), `openrouter/openai/gpt-5.5`, `opencode/gpt-5.5`, and other compatible GPT-5 refs receive the same overlay. Older GPT-4.x models do not.

The bundled native Codex harness does not receive this DaisyClaw GPT-5 overlay through Codex app-server developer instructions. Native Codex keeps Codex-owned base, model, and project-doc behavior, while DaisyClaw disables Codex's built-in personality for native threads so agent workspace personality files stay authoritative. DaisyClaw contributes only runtime context such as channel delivery, DaisyClaw dynamic tools, ACP delegation, workspace context, and DaisyClaw skills.

The GPT-5 contribution adds a tagged behavior contract for persona persistence, execution safety, tool discipline, output shape, completion checks, and verification on matching DaisyClaw-assembled prompts. Channel-specific reply and silent-message behavior stays in the shared DaisyClaw system prompt and outbound delivery policy. The friendly interaction-style layer is separate and configurable.
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash
    daisyclaw config set agents.defaults.promptOverlays.gpt5.personality off
    ```
  </Tab>
</Tabs>
...
<AccordionGroup>
  <Accordion title="Speech synthesis (TTS)">
    ...
    `extraBody` is merged into `/audio/speech` request JSON after DaisyClaw's generated fields, so use it for OpenAI-compatible endpoints that require additional keys such as `lang`. Prototype keys are ignored.
  ...
  <Accordion title="Speech-to-text">
    ...
    DaisyClaw's media-understanding transcription surface.

    - Default model: `gpt-4o-transcribe`
    - Endpoint: OpenAI REST `/v1/audio/transcriptions`
    - Input path: multipart audio file upload
    - Supported by DaisyClaw wherever inbound audio transcription uses
  ...
  <Accordion title="Realtime voice">
    ...
    model has emitted audio in that session. DaisyClaw currently exposes the
...
generation by overriding the base URL. On the image-generation path, DaisyClaw
...
DaisyClaw recognizes these Azure host suffixes for the Azure image-generation
route:
...
For image-generation requests on a recognized Azure host, DaisyClaw:
...
DaisyClaw 2026.4.22 or later. Earlier versions treat any custom
...
routed through the bundled `openai` provider, the `model` field in DaisyClaw
...
DaisyClaw. If an Azure request fails with a validation error, check the
...
DaisyClaw's hidden attribution headers — see the **Native vs OpenAI-compatible
...
<AccordionGroup>
  <Accordion title="Transport (WebSocket vs SSE)">
    DaisyClaw uses WebSocket-first with SSE fallback (`"auto"`) for `openai/*`.

    In `"auto"` mode, DaisyClaw:
  ...
  <Accordion title="Fast mode">
    DaisyClaw exposes a shared fast-mode toggle for `openai/*`:

    - **Chat/UI:** `/fast status|on|off`
    - **Config:** `agents.defaults.models["<provider>/<model>"].params.fastMode`

    When enabled, DaisyClaw maps fast mode to OpenAI priority processing (`service_tier = "priority"`). Existing `service_tier` values are preserved, and fast mode does not rewrite `reasoning` or `text.verbosity`.
  ...
  <Accordion title="Priority processing (service_tier)">
    OpenAI's API exposes priority processing via `service_tier`. Set it per model in DaisyClaw:
    ...
    <Warning>
    `serviceTier` is only forwarded to native OpenAI endpoints (`api.openai.com`) and native Codex endpoints (`chatgpt.com/backend-api`). If you route either provider through a proxy, DaisyClaw leaves `service_tier` untouched.
    </Warning>

  </Accordion>

  <Accordion title="Server-side compaction (Responses API)">
    For direct OpenAI Responses models (`openai/*` on `api.openai.com`), the OpenAI plugin's DaisyClaw stream wrapper auto-enables server-side compaction:

    - Forces `store: true` (unless model compat sets `supportsStore: false`)
    - Injects `context_management: [{ type: "compaction", compact_threshold: ... }]`
    - Default `compact_threshold`: 70% of `contextWindow` (or `80000` when unavailable)

    This applies to the built-in DaisyClaw runtime path and to OpenAI provider hooks used by embedded runs. The native Codex app-server harness manages its own context through Codex and is configured by OpenAI's default agent route or provider/model runtime policy.
  ...
  <Accordion title="Strict-agentic GPT mode">
    For GPT-5-family runs on `openai/*`, DaisyClaw can use a stricter embedded execution contract:
  ...
  <Accordion title="Native vs OpenAI-compatible routes">
    DaisyClaw treats direct OpenAI, Codex, and Azure OpenAI endpoints differently from generic OpenAI-compatible `/v1` proxies:
