...
DaisyClaw sources most Go catalog rows from the bundled DaisyClaw model registry and
supplements current upstream rows while the registry catches up. Run
`daisyclaw models list --provider opencode-go` for the current model list.
...
<AccordionGroup>
  <Accordion title="Routing behavior">
    DaisyClaw handles per-model routing automatically when the model ref uses
