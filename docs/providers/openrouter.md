---
summary: "Use OpenRouter's unified API to access many models in DaisyClaw"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via OpenRouter in DaisyClaw
  ...
  <Tab title="OAuth">
    <Steps>
      <Step title="Run OAuth onboarding">
        ```bash
        daisyclaw onboard --auth-choice openrouter-oauth
        ```

        DaisyClaw opens OpenRouter's browser sign-in flow, exchanges the PKCE
        code for an OpenRouter API key, and stores that key in the default
        OpenRouter auth profile. On remote/headless hosts, DaisyClaw prints the
      ...
      <Step title="(Optional) Switch to a specific model">
        ...
        ```bash
        daisyclaw models set openrouter/<provider>/<model>
        ```
  ...
  <Tab title="API key">
    <Steps>
      ...
      <Step title="Run API-key onboarding">
        ```bash
        daisyclaw onboard --auth-choice openrouter-api-key
        ```
      </Step>
      <Step title="(Optional) Switch to a specific model">
        Onboarding defaults to `openrouter/auto`. Pick a concrete model later:
...
DaisyClaw submits text-to-video and image-to-video jobs to OpenRouter, polls
...
`google/lyria-3-clip-preview`. DaisyClaw sends `modalities: ["text",
...
DaisyClaw sends OpenRouter STT requests as JSON with base64 audio under
...
OAuth is a PKCE login flow that issues an OpenRouter API key, so DaisyClaw stores
...
```bash
daisyclaw models auth login --provider openrouter --method oauth
```

Use `daisyclaw models auth login --provider openrouter --method api-key` when
you want to paste a key you created manually at OpenRouter.

On real OpenRouter requests (`https://openrouter.ai/api/v1`), DaisyClaw also adds
OpenRouter's documented app-attribution headers:

| Header                    | Value                                                                                                  |
| ------------------------- | ------------------------------------------------------------------------------------------------------ |
| `HTTP-Referer`            | `https://daisyclaw.ai`                                                                                  |
| `X-OpenRouter-Title`      | `DaisyClaw`                                                                                             |
| `X-OpenRouter-Categories` | `cli-agent,cloud-agent,programming-app,creative-writing,writing-assistant,general-chat,personal-agent` |

<Warning>
If you repoint the OpenRouter provider at some other proxy or base URL, DaisyClaw
...
<AccordionGroup>
  <Accordion title="Response caching">
    ...
    DaisyClaw sends `X-OpenRouter-Cache: true` and, when configured,
  ...
  <Accordion title="Anthropic cache markers">
    ...
    OpenRouter-specific Anthropic `cache_control` markers that DaisyClaw uses for
  ...
  <Accordion title="Thinking / reasoning injection">
    On supported non-`auto` routes, DaisyClaw maps the selected thinking level to
  ...
  <Accordion title="DeepSeek V4 reasoning replay">
    ...
    required follow-up shape. DaisyClaw sends OpenRouter-supported
  ...
  <Accordion title="Gemini-backed routes">
    Gemini-backed OpenRouter refs stay on the proxy-Gemini path: DaisyClaw keeps
  ...
  <Accordion title="Provider routing metadata">
    ...
    DaisyClaw forwards that object to OpenRouter as the request `provider`
