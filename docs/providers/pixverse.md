---
summary: "PixVerse video generation setup in DaisyClaw"
title: "PixVerse"
read_when:
  - You want to use PixVerse video generation in DaisyClaw
  - You need the PixVerse API key/env setup
  - You want to make PixVerse the default video provider
---

DaisyClaw provides `pixverse` as an official external plugin for hosted PixVerse video generation. The plugin registers the `pixverse` provider against the `videoGenerationProviders` contract.

| Property           | Value                                                                |
| ------------------ | -------------------------------------------------------------------- |
| Provider id        | `pixverse`                                                           |
| Plugin package     | `@daisyclaw/pixverse-provider`                                        |
...
<Steps>
  <Step title="Install the plugin">
    ```bash
    daisyclaw plugins install clawhub:@daisyclaw/pixverse-provider
    daisyclaw gateway restart
    ```
  </Step>
  <Step title="Set the API key">
    ```bash
    daisyclaw onboard --auth-choice pixverse-api-key
    ```
  ...
  <Step title="Set PixVerse as the default video provider">
    ```bash
    daisyclaw config set agents.defaults.videoGenerationModel.primary "pixverse/v6"
    ```
  </Step>
...
The provider exposes PixVerse generation models through DaisyClaw's shared video tool.
...
<Note>
PixVerse image template generation is not exposed through `image_generate` yet. That API is template-id driven, while DaisyClaw's shared image-generation contract does not currently have a PixVerse-specific typed option bag.
</Note>
...
<AccordionGroup>
  <Accordion title="API region">
    DaisyClaw defaults to the international PixVerse API. Set `models.providers.pixverse.region`
    manually when your key belongs to a specific PixVerse platform region, or use
    `daisyclaw onboard --auth-choice pixverse-api-key` to choose one in the setup wizard:
  ...
  <Accordion title="Task polling">
    PixVerse returns a `video_id` from the generation request. DaisyClaw polls
