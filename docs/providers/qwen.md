---
summary: "Use Qwen Cloud via DaisyClaw's bundled qwen provider"
read_when:
  - You want to use Qwen with DaisyClaw
  - You previously used Qwen OAuth
title: "Qwen"
---

DaisyClaw now treats Qwen as a first-class bundled provider with canonical id
    ...
    **Best for:** subscription-based access through the Qwen Coding Plan.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        daisyclaw onboard --auth-choice qwen-api-key
        ```

        For the **China** endpoint:

        ```bash
        daisyclaw onboard --auth-choice qwen-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify the model is available">
        ```bash
        daisyclaw models list --provider qwen
        ```
      </Step>
    </Steps>
    ...
    **Best for:** pay-as-you-go access through the Standard Model Studio endpoint, including models like `qwen3.6-plus` that may not be available on the Coding Plan.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        daisyclaw onboard --auth-choice qwen-standard-api-key
        ```

        For the **China** endpoint:

        ```bash
        daisyclaw onboard --auth-choice qwen-standard-api-key-cn
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
    ...
    **Best for:** a Qwen Portal token against `https://portal.qwen.ai/v1`.
    ...
    <Steps>
      <Step title="Provide your portal token">
        ```bash
        daisyclaw onboard --auth-choice qwen-oauth
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
      ...
      <Step title="Verify the model is available">
        ```bash
        daisyclaw models list --provider qwen-oauth
        ```
      </Step>
    </Steps>
...
DaisyClaw currently ships this bundled Qwen catalog. The configured catalog is
...
For reasoning-enabled Qwen Cloud models, the bundled provider maps DaisyClaw
...
<AccordionGroup>
  ...
  <Accordion title="Qwen 3.6 Plus availability">
    ...
    DaisyClaw's bundled Qwen catalog does not advertise `qwen3.6-plus` on Coding
  ...
  <Accordion title="Video generation details">
    For video generation, DaisyClaw maps the configured Qwen region to the matching
  ...
  <Accordion title="Streaming usage compatibility">
    ...
    shared `openai-completions` transport. DaisyClaw keys that off endpoint
  ...
  <Accordion title="Environment and daemon setup">
    ...
    available to that process (for example, in `~/.daisyclaw/.env` or via
    `env.shellEnv`).
  </Accordion>
</AccordionGroup>
