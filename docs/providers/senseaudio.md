...
SenseAudio can transcribe inbound audio and voice-note attachments through DaisyClaw's shared `tools.media.audio` pipeline. DaisyClaw posts multipart audio to the OpenAI-compatible transcription endpoint and injects the returned text as `{{Transcript}}` plus an `[Audio]` block.
...
<Steps>
  ...
  <Step title="Send a voice note">
    Send an audio message through any connected channel. DaisyClaw uploads the
...
<Note>
SenseAudio is batch STT only in DaisyClaw. Voice Call realtime transcription
