---
summary: "Use Synthetic's Anthropic-compatible API in DaisyClaw"
read_when:
...
DaisyClaw registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    daisyclaw onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
DaisyClaw's Anthropic client appends `/v1` to the base URL automatically, so use
...
`daisyclaw models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that DaisyClaw appends `/v1` automatically.
