---
summary: "Use Venice AI privacy-focused models in DaisyClaw"
read_when:
  - You want privacy-focused inference in DaisyClaw
...
<Steps>
  ...
  <Step title="Configure DaisyClaw">
    ...
    <Tabs>
      <Tab title="Interactive (recommended)">
        ```bash
        daisyclaw onboard --auth-choice venice-api-key
        ```
  ...
  <Step title="Verify setup">
    ```bash
    daisyclaw agent --model venice/kimi-k2-5 --message "Hello, are you working?"
    ```
  </Step>
</Steps>
...
After setup, DaisyClaw shows all available Venice models. Pick based on your needs:
...
```bash
daisyclaw models set venice/kimi-k2-5
daisyclaw models set venice/claude-opus-4-6
```

List all available models:

```bash
daisyclaw models list --all --provider venice
```

You can also run `daisyclaw configure`, select **Model/auth**, and choose **Venice AI**.
...
`venice/deepseek-v4-flash`, DaisyClaw fills the required DeepSeek V4
...
DaisyClaw keeps that provider-specific replay fix separate from the native
...
DaisyClaw ships a manifest-backed Venice seed catalog for read-only model listing. Runtime refresh can still discover models from the Venice API, and falls back to the manifest catalog if the API is unreachable.
...
daisyclaw agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
daisyclaw agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
daisyclaw agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
daisyclaw agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
daisyclaw agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
<AccordionGroup>
  <Accordion title="API key not recognized">
    ```bash
    echo $VENICE_API_KEY
    daisyclaw models list | grep venice
    ```
  ...
  <Accordion title="Model not available">
    The Venice model catalog updates dynamically. Run `daisyclaw models list` to see currently available models. Some models may be temporarily offline.
  </Accordion>
