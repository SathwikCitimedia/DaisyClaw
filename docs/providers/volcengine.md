...
read_when:
  - You want to use Volcano Engine or Doubao models with DaisyClaw
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    daisyclaw onboard --auth-choice volcengine-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    daisyclaw models list --provider volcengine
    daisyclaw models list --provider volcengine-plan
    ```
  </Step>
</Steps>
...
Then enable it in `daisyclaw.json`:
...
For voice-note targets, DaisyClaw asks Volcengine for provider-native
...
<AccordionGroup>
  <Accordion title="Default model after onboarding">
    `daisyclaw onboard --auth-choice volcengine-api-key` currently sets
  ...
  <Accordion title="Model picker fallback behavior">
    ...
    loaded yet, DaisyClaw falls back to the unfiltered catalog instead of showing an
  ...
  <Accordion title="Environment variables for daemon processes">
    ...
    `~/.daisyclaw/.env` or via `env.shellEnv`).
  </Accordion>
</AccordionGroup>

<Warning>
When running DaisyClaw as a background service, environment variables set in your
...
<CardGroup cols={2}>
  ...
  <Card title="FAQ" href="/help/faq" icon="circle-question">
    Frequently asked questions about DaisyClaw setup.
  </Card>
</CardGroup>
