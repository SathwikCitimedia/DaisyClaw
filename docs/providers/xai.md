---
summary: "Use xAI Grok models in DaisyClaw"
read_when:
  - You want to use Grok models in DaisyClaw
  - You are configuring xAI auth or model ids
title: "xAI"
---

DaisyClaw ships a bundled `xai` provider plugin for Grok models. For most
users, the recommended path is Grok OAuth with an eligible SuperGrok or X Premium
subscription. DaisyClaw stays local-first: the Gateway, config, routing, and
...
app. xAI may still show Grok Build on the consent screen because DaisyClaw uses
xAI's shared OAuth client.

## Choose your setup path

Use the path that matches your DaisyClaw install state:

<Steps>
  <Step title="New DaisyClaw install">
    ...
    ```bash
    daisyclaw onboard --install-daemon --auth-choice xai-device-code
    ```

    OAuth does not require an xAI API key. DaisyClaw does not require the Grok
    Build app. xAI may still label the consent app as Grok Build because
    DaisyClaw uses xAI's shared OAuth client.

  </Step>
  <Step title="Existing DaisyClaw install">
    If DaisyClaw is already configured, sign in to xAI only. Do not rerun full
    onboarding or reinstall the daemon just to connect Grok:

    ```bash
    daisyclaw models auth login --provider xai --method oauth
    ```
    ...
    ```bash
    daisyclaw models auth login --provider xai --device-code
    ```

    To make Grok the default model after signing in, apply it separately:

    ```bash
    daisyclaw models set xai/grok-4.3
    ```
  ...
  <Step title="API-key path">
    ...
    ```bash
    daisyclaw models auth login --provider xai --method api-key
...
<Note>
DaisyClaw uses the xAI Responses API as the bundled xAI transport. The same
credential from `daisyclaw models auth login --provider xai --method oauth`,
`daisyclaw models auth login --provider xai --device-code`, or
`daisyclaw models auth login --provider xai --method api-key` can also power first-class
...
- To inspect saved xAI auth profiles, run:
  ...
  ```bash
  daisyclaw models auth list --provider xai
...
<Tip>
Use `xai-device-code` when signing in from SSH, Docker, or a VPS. DaisyClaw
...
DaisyClaw includes the current xAI chat models out of the box, ordered newest
...
normalize to `grok-build-0.1`; DaisyClaw no longer shows the other retired
...
The bundled plugin maps xAI's current public API surface onto DaisyClaw's shared
...
| xAI capability             | DaisyClaw surface                          | Status                                                              |
| -------------------------- | ----------------------------------------- | ------------------------------------------------------------------- |
...
| Streaming TTS              | -                                         | Not exposed; DaisyClaw's TTS contract returns complete audio buffers |
| Batch speech-to-text       | `tools.media.audio` / media understanding | Yes                                                                 |
| Streaming speech-to-text   | Voice Call `streaming.provider: "xai"`    | Yes                                                                 |
| Realtime voice             | -                                         | Not exposed yet; different session/WebSocket contract               |
| Files / batches            | Generic model API compatibility only      | Not a first-class DaisyClaw tool                                     |

<Note>
DaisyClaw uses xAI's REST image/video/TTS/STT APIs for media generation,
...
code-execution tools. Features that need different DaisyClaw contracts, such as
...
<AccordionGroup>
  <Accordion title="Web search">
    ...
    ```bash
    daisyclaw models auth login --provider xai --method oauth
    daisyclaw config set tools.web.search.provider grok
    ```
  ...
  <Accordion title="Video generation">
    ...
    DaisyClaw can encode those as data URLs for xAI.
    </Warning>
  ...
  <Accordion title="Image generation">
    ...
    DaisyClaw asks xAI for `b64_json` image responses so generated media can be
    ...
    such as `1:2`, `2:1`, `9:20`, and `20:9`. DaisyClaw forwards only the
  ...
  <Accordion title="Text-to-speech">
    ...
    <Note>
    DaisyClaw uses xAI's batch `/v1/tts` endpoint. xAI also offers streaming TTS
    over WebSocket, but the DaisyClaw speech provider contract currently expects
  ...
  <Accordion title="Speech-to-text">
    The bundled `xai` plugin registers batch speech-to-text through DaisyClaw's
    ...
    transcription request. Prompt hints are accepted by the shared DaisyClaw
  ...
  <Accordion title="x_search configuration">
    The bundled xAI plugin exposes `x_search` as an DaisyClaw tool for searching
  ...
  <Accordion title="Code execution configuration">
    The bundled xAI plugin exposes `code_execution` as an DaisyClaw tool for
  ...
  <Accordion title="Known limits">
    - xAI auth can use an API key, environment variable, plugin config fallback,
      ...
      the consent page may show Grok Build even though DaisyClaw does not require
      the Grok Build app.
    - `grok-4.20-multi-agent-experimental-beta-0304` is not supported on the
      normal xAI provider path because it requires a different upstream API
      surface than the standard DaisyClaw xAI transport.
    - xAI Realtime voice is not registered as an DaisyClaw provider yet. It
  ...
  <Accordion title="Advanced notes">
    - DaisyClaw applies xAI-specific tool-schema and tool-call compatibility fixes
    ...
    - `web_search`, `x_search`, and `code_execution` are exposed as DaisyClaw
      tools. DaisyClaw enables the specific xAI built-in it needs inside each tool
...
DAISYCLAW_LIVE_TEST=1 DAISYCLAW_LIVE_TEST_QUIET=1 pnpm test:live -- extensions/xai/xai.live.test.ts
DAISYCLAW_LIVE_TEST=1 DAISYCLAW_LIVE_TEST_QUIET=1 DAISYCLAW_LIVE_IMAGE_GENERATION_PROVIDERS=xai pnpm test:live -- test/image-generation.runtime.live.test.ts
```
...
shared image live file verifies the same xAI provider through DaisyClaw's
