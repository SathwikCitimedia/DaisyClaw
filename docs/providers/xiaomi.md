---
summary: "Use Xiaomi MiMo pay-as-you-go and Token Plan models with DaisyClaw"
read_when:
  - You want Xiaomi MiMo models in DaisyClaw
  - You need Xiaomi MiMo auth or Token Plan setup
title: "Xiaomi MiMo"
---

Xiaomi MiMo is the API platform for **MiMo** models. DaisyClaw includes a bundled Xiaomi plugin with two text-provider presets:
...
<Steps>
  ...
  <Step title="Run onboarding">
    ...
    ```bash
    daisyclaw onboard --auth-choice xiaomi-api-key
    ```

    Token Plan:

    ```bash
    daisyclaw onboard --auth-choice xiaomi-token-plan-sgp
    ```

    Or pass the keys directly:

    ```bash
    daisyclaw onboard --auth-choice xiaomi-api-key --xiaomi-api-key "$XIAOMI_API_KEY"
    daisyclaw onboard --auth-choice xiaomi-token-plan-sgp --xiaomi-token-plan-api-key "$XIAOMI_TOKEN_PLAN_API_KEY"
    ```

  </Step>
  <Step title="Verify the model is available">
    ```bash
    daisyclaw models list --provider xiaomi
    daisyclaw models list --provider xiaomi-token-plan
    ```
  </Step>
</Steps>
...
DaisyClaw sends `speakerVoice` for `mimo-v2.5-tts` and `mimo-v2-tts`.
...
`style` with the desired voice description; DaisyClaw sends it as the `user`
...
For voice-note targets such as Feishu and Telegram, DaisyClaw transcodes Xiaomi
...
<AccordionGroup>
  ...
  <Accordion title="Troubleshooting">
    ...
    - When the Gateway runs as a daemon, ensure the key is available to that process (for example in `~/.daisyclaw/.env` or via `env.shellEnv`).

    <Warning>
    Keys set only in your interactive shell are not visible to daemon-managed gateway processes. Use `~/.daisyclaw/.env` or `env.shellEnv` config for persistent availability.
    </Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" href="/gateway/configuration-reference" icon="gear">
    Full DaisyClaw configuration reference.
  </Card>
