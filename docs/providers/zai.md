---
summary: "Use Z.AI (GLM models) with DaisyClaw"
read_when:
  - You want Z.AI / GLM models in DaisyClaw
...
DaisyClaw uses the `zai` provider with a Z.AI API key.
...
GLM is a model family, not a separate provider. In DaisyClaw, GLM models use
...
<Tabs>
  <Tab title="Auto-detect endpoint">
    **Best for:** most users. DaisyClaw probes supported Z.AI endpoints with your API key and applies the correct base URL automatically.

    <Steps>
      <Step title="Run onboarding">
        ```bash
        daisyclaw onboard --auth-choice zai-api-key
        ```
      </Step>
      <Step title="Verify the model is listed">
        ```bash
        daisyclaw models list --all --provider zai
        ```
      </Step>
    </Steps>
    ...
    **Best for:** users who want to force a specific Coding Plan or general API surface.
    ...
    <Steps>
      <Step title="Pick the right onboarding choice">
        ...
        daisyclaw onboard --auth-choice zai-coding-global

        # Coding Plan CN (China region)
        daisyclaw onboard --auth-choice zai-coding-cn

        # General API
        daisyclaw onboard --auth-choice zai-global

        # General API CN (China region)
        daisyclaw onboard --auth-choice zai-cn
        ```
      </Step>
      <Step title="Verify the model is listed">
        ```bash
...
<Tip>
`zai-api-key` lets DaisyClaw detect the matching Z.AI endpoint from the key and
...
DaisyClaw ships the bundled `zai` provider catalog in the plugin manifest, so read-only
...
can change; run `daisyclaw models list --all --provider zai` to see the catalog
...
<AccordionGroup>
  ...
  <Accordion title="Thinking and preserved thinking">
    Z.AI thinking follows DaisyClaw's `/think` controls. With thinking off,
    DaisyClaw sends `thinking: { type: "disabled" }` to avoid responses that
    ...
    When enabled and thinking is on, DaisyClaw sends
  ...
  <Accordion title="Auth details">
    ...
    - The legacy env var `Z_AI_API_KEY` is still accepted; DaisyClaw copies it to `ZAI_API_KEY` at startup if `ZAI_API_KEY` is unset.
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" href="/gateway/configuration-reference" icon="gear">
    Full DaisyClaw config schema, including provider and model settings.
  </Card>
</CardGroup>
