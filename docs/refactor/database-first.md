...
read_when:
  - Moving DaisyClaw runtime data, cache, transcripts, task state, or scratch files into SQLite
...
- Global database: `~/.daisyclaw/state/daisyclaw.sqlite`
...
- Configuration stays file-backed: `daisyclaw.json` remains outside the
  database. Runtime auth profiles move to SQLite; external provider or CLI
  credential files remain owner-managed outside DaisyClaw's database.
...
- Raw stream logging uses `DAISYCLAW_RAW_STREAM=1` plus SQLite diagnostics rows.
  The old pi-mono `PI_RAW_STREAM`, `PI_RAW_STREAM_PATH`, and
  `raw-openai-completions.jsonl` file logger contract is not part of DaisyClaw
  runtime or tests.
...
- One global SQLite database owns control-plane state:
  `state/daisyclaw.sqlite`.
- One per-agent SQLite database owns data-plane state:
  `agents/<agentId>/agent/daisyclaw-agent.sqlite`.
- Config remains file-backed. `daisyclaw.json` is not part of this database
  refactor.
...
- Task registry: `clean`. Task and Task Flow runtime rows live in
  `state/daisyclaw.sqlite`; unshipped sidecar SQLite importers are deleted.
...
- [x] Keep Kysely generated types aligned after any schema change.
      Files: `src/state/daisyclaw-state-schema.sql`,
      `src/state/daisyclaw-agent-schema.sql`,
      `src/state/*generated*`.
...
- `daisyclaw doctor --fix` owns the legacy file-to-database migration step.
  Runtime startup and `daisyclaw migrate` should not carry legacy DaisyClaw
...
- The old runtime-owned JSONL transcript streaming helper was deleted. Doctor
  ...
  SQLite rows.
- Codex app-server bindings use the DaisyClaw `sessionId` as the canonical
...
former stores now write to `state/daisyclaw.sqlite` or the owning
`daisyclaw-agent.sqlite` database.
...
- Plugin migrations are data migrations over plugin-owned namespaces, not host
  ...
  `daisyclaw-state-schema.sql` unless the host itself is taking ownership of a
  new cross-plugin contract.
- `src/state/daisyclaw-agent-db.ts` opens
  `agents/<agentId>/agent/daisyclaw-agent.sqlite`, registers the database in the
...
- Per-agent external conversation identity is relational too:
  ...
  `session_conversations` links one DaisyClaw session to one or more external
  ...
  DaisyClaw session can represent multiple external DM peers without demoting
...
- Secret target metadata now talks about stores instead of pretending every
  credential target is a config file. `daisyclaw.json` remains the config store;
...
- Workspace bootstrap completion markers now live in typed shared
  ...
  `.daisyclaw/workspace-state.json`; runtime no longer reads or rewrites the
  legacy workspace marker, and helper APIs no longer pass around a fake
  `.daisyclaw/setup-state` path just to derive storage identity.
- Exec approvals now live in the typed shared SQLite `exec_approvals_config`
  singleton row. Doctor imports legacy `~/.daisyclaw/exec-approvals.json`;
  ...
  `state/daisyclaw.sqlite` table row; it keeps only the Unix prompt socket on disk
...
- GitHub Copilot compaction no longer writes `daisyclaw-compaction-*.json`
  workspace sidecars. The harness calls the SDK history compaction RPC for the
  tracked SDK session, and DaisyClaw keeps durable session/transcript state in
  SQLite instead of compatibility marker files.
- The shared Swift runtime (`DaisyClawKit`) uses the same
  `state/daisyclaw.sqlite` rows for device identity and device auth. macOS app
...
- Android device identity uses the same TypeScript-compatible key material
  stored in typed `state/daisyclaw.sqlite#table/device_identities` rows. It never
  reads or writes `daisyclaw/identity/device.json`; a leftover legacy file blocks
  startup until doctor imports it into SQLite.
- Android cached device auth tokens also use typed
  `state/daisyclaw.sqlite#table/device_auth_tokens` rows and share the same
...
- Non-doctor commands do not auto-run legacy config repair. For example,
  `daisyclaw update --channel` now fails on invalid legacy config and asks the
...
- Plugin state now uses the shared `state/daisyclaw.sqlite` database. The old
...
- Task and Task Flow runtime tables now live in the shared
  `state/daisyclaw.sqlite` database instead of `tasks/runs.sqlite` and
...
- Session target resolution now exposes per-agent database targets, not legacy
  ...
  `daisyclaw sessions` enumerate `agent_databases` plus configured agents.
...
- Cache trace, Anthropic payload, raw stream, and diagnostics timeline records
  ...
  `diagnostics.cacheTrace.filePath`, `DAISYCLAW_CACHE_TRACE_FILE`,
  `DAISYCLAW_ANTHROPIC_PAYLOAD_LOG_FILE`, and
  `DAISYCLAW_DIAGNOSTICS_TIMELINE_PATH` JSONL override paths are removed, and
...
- Memory Wiki activity logs now use SQLite plugin state instead of
  `.daisyclaw-wiki/log.jsonl`. The Memory Wiki migration provider imports old
  JSONL logs; wiki markdown and user vault content stay file-backed as
  workspace content.
- Memory Wiki no longer creates `.daisyclaw-wiki/state.json` or the unused
  `.daisyclaw-wiki/locks` directory. The migration provider removes those retired
...
- The macOS companion no longer writes app-local `logs/config-audit.jsonl` or
  `logs/config-health.json` sidecars while editing `daisyclaw.json`. The config
...
- Codex app-server conversation bindings now key SQLite plugin state by
  DaisyClaw session key or explicit `{agentId, sessionId}` scope. They must not
...
- Zalo hosted outbound media now uses shared SQLite `plugin_blob_entries`
  instead of `daisyclaw-zalo-outbound-media` JSON/bin temp sidecars.
...
- ACPX generated wrapper scripts and the isolated Codex home are temporary
  materialization under the DaisyClaw temp root, not durable DaisyClaw state. The
...
- Gateway media attachments now use the shared `media_blobs` SQLite table as
  ...
  `$DAISYCLAW_STATE_DIR/media` or config-dir `media` roots; those directories are
  doctor import sources only.
- Shell completion no longer writes `$DAISYCLAW_STATE_DIR/completions/*` cache
...
- Subagent inline attachments no longer materialize under workspace
  `.daisyclaw/attachments/*`. The spawn path prepares SQLite VFS seed entries,
...
- CLI image hydration no longer maintains stable `daisyclaw-cli-images` cache
...
- `daisyclaw secrets apply` scrubs the config file, env file, and SQLite
...
- User-facing auth docs now describe
  `state/daisyclaw.sqlite#table/auth_profile_stores/<agentDir>` instead of
...
- Memory Wiki imported-source sync ledgers now store one SQLite plugin-state row
  per vault/source key instead of rewriting `.daisyclaw-wiki/source-sync.json`;
...
- Memory Wiki ChatGPT import-run records now store one SQLite plugin-state row
  per vault/run id instead of writing `.daisyclaw-wiki/import-runs/*.json`.
...
- Memory Wiki compiled digests now store SQLite plugin blob rows instead of
  writing `.daisyclaw-wiki/cache/agent-digest.json` and
  `.daisyclaw-wiki/cache/claims.jsonl`. The migration provider imports old cache
...
- ACPX wrapper scripts and the isolated Codex home are generated in the
  DaisyClaw temp root. They are recreated as needed and are not backup or
  migration inputs.
...
- Backup stages the state directory before archiving, copies non-database files,
  ...
  completed backup runs in SQLite with the archive manifest. `daisyclaw backup
create` validates the written archive by default; `--no-verify` is the
  explicit fast path.
- `daisyclaw backup restore` validates the archive before extraction, reuses the
...
- `daisyclaw reset --scope config+creds+sessions` removes per-agent
  `daisyclaw-agent.sqlite` databases plus WAL/SHM sidecars, not only legacy
...
- Generated model catalog config is stored in typed global SQLite
  ...
  `ensureDaisyClawModelCatalog`; there is no `models.json` compatibility API in
...
- Memory-core runtime imports SQLite transcript indexing helpers from
  `daisyclaw/plugin-sdk/memory-core-host-engine-session-transcripts`, not the
...
- QMD's own `index.sqlite` is now a temp runtime materialization backed by the
  ...
  `~/.daisyclaw/agents/<agentId>/qmd` sidecar.
- The optional `memory-lancedb` plugin no longer creates
  `~/.daisyclaw/memory/lancedb` as an implicit DaisyClaw-managed store. It is an
...
- `check:database-first-legacy-stores` fails new runtime source that pairs
  ...
  `state/daisyclaw.sqlite` location instead of pretending those features own
...
`daisyclaw doctor --fix` invokes the state migration implementation after
ordinary config preflight and creates a verified backup before import. Runtime
startup and `daisyclaw migrate` must not import legacy DaisyClaw state files.
...
- Add a single doctor-owned state migration service for file-to-database import.
  Done: `daisyclaw doctor --fix` uses the legacy-state migration implementation.
...
Keep shared coordination state in `state/daisyclaw.sqlite`:
...
```text
~/.daisyclaw/state/daisyclaw.sqlite
~/.daisyclaw/agents/<agentId>/agent/daisyclaw-agent.sqlite
```
...
- Verify by opening every DB snapshot and running `PRAGMA integrity_check`.
  `daisyclaw backup create` does this archive verification by default;
...
5. Verify the archive by opening every included SQLite snapshot and running
   ...
   `daisyclaw backup create` does this by default; `--no-verify` is only for
...
- Use `{ agentId, sessionKey }` row operations.
  ...
  and the `daisyclaw sessions` listing command now read per-agent rows directly
...
8. Backup integration.
   ...
   - Add restore from verified archive snapshots. Done: `daisyclaw backup
...
- Remove tests that assert runtime creation of `sessions.json` or transcript
  ...
  DaisyClaw test-state helper no longer exposes a legacy `auth-profiles.json`
...
- ad-hoc `daisyclaw-state.sqlite` runtime sidecars
- `tasks/runs.sqlite`
...
- Memory Wiki `.daisyclaw-wiki/log.jsonl`
- Memory Wiki `.daisyclaw-wiki/state.json`
- Memory Wiki `.daisyclaw-wiki/locks/`
- Memory Wiki `.daisyclaw-wiki/source-sync.json`
- Memory Wiki `.daisyclaw-wiki/import-runs/*.json`
- Memory Wiki `.daisyclaw-wiki/cache/agent-digest.json`
- Memory Wiki `.daisyclaw-wiki/cache/claims.jsonl`
- ClawHub `.clawhub/lock.json`
- ClawHub `.clawhub/origin.json`
- Browser profile decoration `.daisyclaw-profile-decorated`
