...
DaisyClaw has three public release lanes:
...
- Every stable DaisyClaw release ships the npm package, macOS app, and signed
...
6. Run `DaisyClaw NPM Release` with `preflight_only=true`. Before a tag exists,
...
vYYYY.M.PATCH-beta.N` from the matching `release/YYYY.M.PATCH` branch. The helper runs
   ...
   `DaisyClaw Release Publish` command only after the evidence bundle is green.
   `DaisyClaw Release Publish` dispatches the selected or all-publishable plugin
   packages to npm and the same set to ClawHub in parallel, and then promotes the
   prepared DaisyClaw npm preflight artifact with the matching dist-tag as soon as
   plugin npm publish succeeds.
   After the DaisyClaw npm publish child succeeds, it creates or updates the
   ...
   `daisyclaw-<version>-dependency-evidence.zip` for post-release incident
   ...
   evidence as soon as DaisyClaw npm publish succeeds, waits for ClawHub whenever
   DaisyClaw npm is being published, then runs `pnpm release:verify-beta` and
   ...
   `daisyclaw@YYYY.M.PATCH-beta.N` or
   `daisyclaw@beta` package. If a pushed or published prerelease needs a fix,
...
10. For stable, continue only after the vetted beta or release candidate has the
    ...
    `DaisyClaw Release Publish`, reusing the successful preflight artifact via
    ...
    readiness requires the signed `DaisyClawCompanion-Setup-x64.exe`,
    `DaisyClawCompanion-Setup-arm64.exe`, and
    `DaisyClawCompanion-SHA256SUMS.txt` assets on the DaisyClaw GitHub release;
    promote them with the `Windows Node Release` workflow after the matching
    `daisyclaw/daisyclaw-windows-node` release has passed its signing workflow.
...
- Plugin version sync updates official plugin package versions and existing
  `daisyclaw.compat.pluginApi` floors to the DaisyClaw release version by
  ...
  compatible with older DaisyClaw hosts, keep the floor at the oldest supported
...
- Run the manual `Full Release Validation` workflow before release approval to
  ...
  `DaisyClaw Release Checks` for install smoke, package acceptance, cross-OS
...
- Run the manual `Package Acceptance` workflow when you want side-channel proof
  ...
  `daisyclaw@beta`, `daisyclaw@latest`, or an exact release version; `source=ref`
  ...
  Example: `gh workflow run package-acceptance.yml --ref main -f workflow_ref=main -f source=npm -f package_spec=daisyclaw@beta -f suite_profile=product -f published_upgrade_survivor_baseline=daisyclaw@2026.4.26 -f telegram_mode=mock-openai`
  Common profiles:
...
- `DaisyClaw NPM Release` preflight generates dependency release evidence before
...
- The preflight uploads dependency evidence as
  `daisyclaw-release-dependency-evidence-<tag>` and also embeds it under
  ...
  to the GitHub release as `daisyclaw-<version>-dependency-evidence.zip`.
- Run `DaisyClaw Release Publish` for the mutating publish sequence after the
  tag exists. Dispatch it from `release/YYYY.M.PATCH` (or `main` when publishing a
  main-reachable tag), pass the release tag and successful DaisyClaw npm
  ...
  workflow serializes plugin npm publish, plugin ClawHub publish, and DaisyClaw
...
- Run the manual `Windows Node Release` workflow for stable releases after the
  matching `daisyclaw/daisyclaw-windows-node` release exists. It downloads the
  ...
  uploads the installers plus manifest onto the canonical DaisyClaw GitHub
  release. Website download links should target exact DaisyClaw release asset
...
- Release checks now run in a separate manual workflow:
  `DaisyClaw Release Checks`
- `DaisyClaw Release Checks` also runs the QA Lab mock parity lane plus the fast
...
- Cross-OS install and upgrade runtime validation is part of public
  `DaisyClaw Release Checks` and `Full Release Validation`, which call the
  reusable workflow
  `.github/workflows/daisyclaw-cross-os-release-checks-reusable.yml` directly
...
- `DaisyClaw Release Checks` accepts a branch, tag, or full commit SHA as long
  as the resolved commit is reachable from an DaisyClaw branch or release tag
- `DaisyClaw NPM Release` validation-only preflight also accepts the current
...
- That workflow runs
  `DAISYCLAW_LIVE_TEST=1 DAISYCLAW_LIVE_CACHE_TEST=1 pnpm test:live:cache`
...
- Before tagging a release candidate locally, run
  ...
  UI build, and `release:daisyclaw:npm:check` in the order that catches common
  approval-blocking mistakes before the GitHub publish workflow starts.
- Run `RELEASE_TAG=vYYYY.M.PATCH node --import tsx scripts/daisyclaw-npm-release-check.ts`
  (or the matching beta/correction tag) before approval
- After npm publish, run
  `node --import tsx scripts/daisyclaw-npm-postpublish-verify.ts YYYY.M.PATCH`
  (or the matching beta/correction version) to verify the published registry
  install path in a fresh temp prefix
- After a beta publish, run `DAISYCLAW_NPM_TELEGRAM_PACKAGE_SPEC=daisyclaw@YYYY.M.PATCH-beta.N DAISYCLAW_NPM_TELEGRAM_CREDENTIAL_SOURCE=convex DAISYCLAW_NPM_TELEGRAM_CREDENTIAL_ROLE=ci pnpm test:docker:npm-telegram-live`
  ...
  `DAISYCLAW_QA_TELEGRAM_*` env credentials directly.
...
- Maintainer release automation now uses preflight-then-promote:
  ...
  - token-based npm dist-tag mutation now lives in
    `daisyclaw/releases/.github/workflows/daisyclaw-npm-dist-tags.yml` because
...
gh workflow run full-release-validation.yml \
  ...
  -f evidence_package_spec=daisyclaw@YYYY.M.PATCH-beta.N
```

The workflow resolves the target ref, dispatches manual `CI` with
`target_ref=<release-ref>`, dispatches `DaisyClaw Release Checks`, prepares a
...
`npm_telegram_package_spec` is set. `DaisyClaw Release
...
`DaisyClaw Release Checks` uses the trusted workflow ref to resolve the target
...
After a beta is already on npm, set `release_package_spec=daisyclaw@YYYY.M.PATCH-beta.N`
...
The cross-OS OpenAI install smoke uses `DAISYCLAW_CROSS_OS_OPENAI_MODEL` when the
...
gh workflow run full-release-validation.yml \
  ...
  -f release_package_spec=daisyclaw@YYYY.M.PATCH-beta.N \
  -f evidence_package_spec=daisyclaw@YYYY.M.PATCH-beta.N \
...
validation, including required DaisyClaw dynamic tool drift in the standard tier.
...
`DAISYCLAW_RELEASE_QA_*_LIVE_CI_ENABLED` repo variable must be enabled; otherwise
...
The Docker box lives in `DaisyClaw Release Checks` through
`daisyclaw-live-and-e2e-checks-reusable.yml`, plus the release-mode
...
The QA Lab box is also part of `DaisyClaw Release Checks`. It is the agentic
...
`scripts/resolve-daisyclaw-package-candidate.mjs`. The resolver normalizes a
...
- `source=npm`: `daisyclaw@beta`, `daisyclaw@latest`, or an exact DaisyClaw release
  version
...
`DaisyClaw Release Checks` runs Package Acceptance with `source=artifact`, the
...
gh workflow run package-acceptance.yml \
  ...
  -f published_upgrade_survivor_baseline=daisyclaw@2026.4.26
```
...
`DaisyClaw Release Publish` is the normal mutating publish entrypoint. It
...
6. Dispatch `DaisyClaw NPM Release` with the release tag, npm dist-tag, and
...
only for focused repair or republish work. `DaisyClaw Release Publish` rejects
`plugin_publish_scope=selected` when `publish_daisyclaw_npm=true` so the core
package cannot ship without every publishable official plugin, including
`@daisyclaw/diffs-language-pack`. For a selected plugin repair, set
`publish_daisyclaw_npm=false` with `plugin_publish_scope=selected` and
`plugins=@daisyclaw/name`, or dispatch the child workflow directly.

## NPM workflow inputs

`DaisyClaw NPM Release` accepts these operator-controlled inputs:
...
`DaisyClaw Release Publish` accepts these operator-controlled inputs:

- `tag`: required release tag; must already exist
- `preflight_run_id`: successful `DaisyClaw NPM Release` preflight run id;
  required when `publish_daisyclaw_npm=true`
- `npm_dist_tag`: npm target tag for the DaisyClaw package
- `plugin_publish_scope`: defaults to `all-publishable`; use `selected` only
  for focused plugin-only repair work with `publish_daisyclaw_npm=false`
- `plugins`: comma-separated `@daisyclaw/*` package names when
  `plugin_publish_scope=selected`
- `publish_daisyclaw_npm`: defaults to `true`; set `false` only when using the
...
`DaisyClaw Release Checks` accepts these operator-controlled inputs:

- `ref`: branch, tag, or full commit SHA to validate. Secret-bearing checks
  require the resolved commit to be reachable from an DaisyClaw branch or
  release tag.
...
- For `DaisyClaw NPM Release`, full commit SHA input is allowed only when
  `preflight_only=true`
- `DaisyClaw Release Checks` and `Full Release Validation` are always
  validation-only
...
1. Run `DaisyClaw NPM Release` with `preflight_only=true`
...
6. Run `DaisyClaw Release Publish` with the same `tag`, the same `npm_dist_tag`,
   and the saved `preflight_run_id`; it publishes externalized plugins to npm
   and ClawHub before promoting the DaisyClaw npm package
7. If the release landed on `beta`, use the
   `daisyclaw/releases/.github/workflows/daisyclaw-npm-dist-tags.yml`
...
- [`.github/workflows/full-release-validation.yml`](https://github.com/daisyclaw/daisyclaw/blob/main/.github/workflows/full-release-validation.yml)
- [`.github/workflows/package-acceptance.yml`](https://github.com/daisyclaw/daisyclaw/blob/main/.github/workflows/package-acceptance.yml)
- [`.github/workflows/daisyclaw-npm-release.yml`](https://github.com/daisyclaw/daisyclaw/blob/main/.github/workflows/daisyclaw-npm-release.yml)
- [`.github/workflows/daisyclaw-release-checks.yml`](https://github.com/daisyclaw/daisyclaw/blob/main/.github/workflows/daisyclaw-release-checks.yml)
- [`.github/workflows/daisyclaw-cross-os-release-checks-reusable.yml`](https://github.com/daisyclaw/daisyclaw/blob/main/.github/workflows/daisyclaw-cross-os-release-checks-reusable.yml)
- [`scripts/resolve-daisyclaw-package-candidate.mjs`](https://github.com/daisyclaw/daisyclaw/blob/main/scripts/resolve-daisyclaw-package-candidate.mjs)
- [`scripts/daisyclaw-npm-release-check.ts`](https://github.com/daisyclaw/daisyclaw/blob/main/scripts/daisyclaw-npm-release-check.ts)
- [`scripts/package-mac-dist.sh`](https://github.com/daisyclaw/daisyclaw/blob/main/scripts/package-mac-dist.sh)
- [`scripts/make_appcast.sh`](https://github.com/daisyclaw/daisyclaw/blob/main/scripts/make_appcast.sh)

Maintainers use the private release docs in
[`daisyclaw/maintainers/release/README.md`](https://github.com/daisyclaw/maintainers/blob/main/release/README.md)
