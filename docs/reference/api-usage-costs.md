...
DaisyClaw features that can generate provider usage or paid API calls.
...
- If DaisyClaw has usage metadata and local pricing for the active model,
...
- Gemini CLI note: when the CLI returns JSON output, DaisyClaw reads usage from
  `stats`, normalizes `stats.cached` into `cacheRead`, and derives input tokens
  from `stats.input_tokens - stats.cached` when needed.

Anthropic note: Anthropic staff told us DaisyClaw-style Claude CLI usage is
...
Anthropic still does not expose a per-message dollar estimate that DaisyClaw can
show in `/usage full`.

**CLI usage windows (provider quotas)**

- `daisyclaw status --usage` and `daisyclaw channels list` show provider **usage windows**
...
- MiniMax note: its raw `usage_percent` / `usagePercent` fields mean remaining
  quota, so DaisyClaw inverts them before display. Count-based fields still win
  when present. If the provider returns `model_remains`, DaisyClaw prefers the
...
- Usage auth for those quota windows comes from provider-specific hooks when
  available; otherwise DaisyClaw falls back to matching OAuth/API-key
...
DaisyClaw can pick up credentials from:
...
DaisyClaw's local UI, such as **OpenAI Codex**, **Alibaba Cloud Model Studio
Coding Plan**, **MiniMax Coding Plan**, **Z.AI / GLM Coding Plan**, and
Anthropic's DaisyClaw Claude-login path with **Extra Usage** enabled.
...
`daisyclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
