---
summary: "DaisyClaw code mode: an opt-in exec/wait tool surface backed by QuickJS-WASI and a hidden run-scoped tool catalog"
title: "Code mode"
sidebarTitle: "Code mode"
read_when:
  - You want to enable DaisyClaw code mode for an agent run
...
Code mode is an experimental DaisyClaw agent-runtime feature. It is off by
default. When you enable it, DaisyClaw changes what the model sees for one run:
instead of exposing every enabled tool schema directly, the model sees only
`exec` and `wait`.

This page documents DaisyClaw code mode. It is not Codex Code mode. The two
...
- DaisyClaw code mode is disabled unless `tools.codeMode.enabled: true` is
  configured. It runs in the DaisyClaw generic agent runtime, where the model
...
surfaces. DaisyClaw code mode is an DaisyClaw-owned experimental tool-surface
adapter for generic DaisyClaw runs. It uses `quickjs-wasi`, a hidden DaisyClaw
tool catalog, and the normal DaisyClaw tool executor.

## What is this?

DaisyClaw code mode lets the model write a small JavaScript or TypeScript program
...
- `exec` evaluates model-generated JavaScript or TypeScript in a constrained
  QuickJS-WASI worker.
- Normal DaisyClaw tools are hidden from the model prompt and exposed inside the
  guest program through `ALL_TOOLS` and `tools`.
- Guest code can search the hidden catalog, describe a tool, and call a tool
  through the same DaisyClaw execution path used by normal agent turns.
...
surface. It does not replace DaisyClaw tools, plugin tools, MCP tools, auth,
...
- Provider neutral: it works for DaisyClaw, plugin, MCP, and client tools without
  depending on provider-native code execution.
- Existing policy stays in force: nested tool calls still go through DaisyClaw
...
- Clear failure mode: when code mode is explicitly enabled and the runtime is
  unavailable, DaisyClaw fails closed instead of falling back to broad direct tool
  exposure.
...
```bash
DAISYCLAW_DEBUG_CODE_MODE=1 \
DAISYCLAW_DEBUG_MODEL_TRANSPORT=1 \
DAISYCLAW_DEBUG_MODEL_PAYLOAD=tools \
daisyclaw gateway
```
...
`DAISYCLAW_DEBUG_MODEL_PAYLOAD=full-redacted` for a short debugging session.
...
- Stability: experimental DaisyClaw surface; Codex Code mode is a separate stable
  Codex harness surface.
- Target surface: generic DaisyClaw agent runs.
...
**Code mode** is the DaisyClaw runtime mode that hides normal model tools and
...
If code mode is enabled but QuickJS-WASI cannot load, DaisyClaw fails closed for
...
2. Build the effective DaisyClaw tool list.
...
- If `language` is `"typescript"`, DaisyClaw transpiles before evaluation.
...
final value is JSON-compatible after DaisyClaw's output adapter runs.
...
`wait` exists because nested DaisyClaw tools can be slow, interactive, approval
...
2. On suspension, DaisyClaw snapshots the QuickJS VM and records pending host
   work.
3. When pending work settles, `wait` restores the VM snapshot.
4. DaisyClaw re-registers host callbacks by stable names.
5. DaisyClaw delivers nested tool results into the restored VM.
6. DaisyClaw drains QuickJS pending jobs.
...
type ToolCatalogEntry = {
  ...
  label?: string;
  description: string;
  source: "daisyclaw" | "plugin" | "mcp" | "client";
  sourceName?: string;
};
```
...
const hits = await tools.web_search({ query: "DaisyClaw code mode" });
```
...
state directory. For each code-mode `exec` call, DaisyClaw builds the run-scoped
...
inside the QuickJS program while DaisyClaw still shows only `exec` and `wait` to
the model.
...
7. DaisyClaw auto-drains ready namespace bridge calls inside the active
...
- `globalName` collisions with built-in code-mode globals such as `tools`,
  `namespaces`, `text`, `json`, `yield_control`, or `__daisyclaw*`
...
catalog for arbitrary enabled DaisyClaw, plugin, and client tools; use `MCP` for
...
- images and files travel through ordinary DaisyClaw tools, not through the
  code-mode bridge
...
Code mode supersedes the DaisyClaw Tool Search model surface for runs where it is
active.

When `tools.codeMode.enabled` is true and code mode activates:

- DaisyClaw does not expose `tool_search_code`, `tool_search`, `tool_describe`,
...
- MCP calls use the generated `MCP` namespace and its `$api()` headers instead
  of `tools.call(...)`.
- Nested calls dispatch through the same DaisyClaw executor path that Tool Search
  uses.

The existing [Tool Search](/tools/tool-search) page describes the DaisyClaw compact
catalog bridge. Code mode is the generic DaisyClaw alternative for runs that can
use `exec` and `wait`.

## Tool names and collisions

The model-visible `exec` tool is the code-mode tool. If the normal DaisyClaw
shell `exec` tool is enabled, it is hidden from the model and cataloged like any
other tool.

Inside the guest runtime:

- `tools.call("daisyclaw:core:exec", input)` can call the shell exec tool if
  policy allows it.
...
If two tools normalize to the same safe convenience name, DaisyClaw omits the
convenience function and requires `tools.call(id, input)`.

## Nested tool execution

Every nested tool call crosses the host bridge and re-enters DaisyClaw.
...
DaisyClaw loads `quickjs-wasi` as a direct dependency in the owning package. The
...
The runtime executes outside DaisyClaw's main event loop in a worker. A guest
...
inputs beyond existing DaisyClaw trajectory policy.
...
DAISYCLAW_DEBUG_MODEL_TRANSPORT=1 \
DAISYCLAW_DEBUG_MODEL_PAYLOAD=tools \
DAISYCLAW_DEBUG_SSE=events \
daisyclaw gateway
```

For payload-shape debugging, use `DAISYCLAW_DEBUG_MODEL_PAYLOAD=full-redacted`.
...
For stream debugging, use `DAISYCLAW_DEBUG_SSE=peek` to log the first five
...
- `tools.search`, `tools.describe`, and `tools.call` work for DaisyClaw tools
...
5. Send an agent turn with DaisyClaw, plugin, MCP, and client test tools.
...
8. In `exec`, call DaisyClaw/plugin/client tools through `tools.search`,
