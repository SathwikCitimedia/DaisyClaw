...
beta publish, pass `release_package_spec=daisyclaw@YYYY.M.PATCH-beta.N` to reuse the
...
`release_package_spec` values derive `codex_plugin_spec=npm:@daisyclaw/codex@<version>`;
...
`DaisyClaw Release Checks` is the largest child workflow. It resolves the target
...
| `release-checks`    | All DaisyClaw Release Checks stages.                                                             |
...
QA release-check failures block normal release validation. Required DaisyClaw
...
- `release-package-under-test` from the Full Release Validation parent and `DaisyClaw Release Checks`
...
- `.github/workflows/daisyclaw-live-and-e2e-checks-reusable.yml`
