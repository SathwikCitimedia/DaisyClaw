...
Prompt caching means the model provider can reuse unchanged prompt prefixes (usually system/developer instructions and other stable context) across turns instead of re-processing them every time. DaisyClaw normalizes provider usage into `cacheRead` and `cacheWrite` where the upstream API exposes those counters directly.
...
- With Anthropic API-key auth profiles, DaisyClaw seeds `cacheRetention: "short"` for Anthropic model refs when unset.
- Anthropic native Messages responses expose both `cache_read_input_tokens` and `cache_creation_input_tokens`, so DaisyClaw can show both `cacheRead` and `cacheWrite`.
...
- Prompt caching is automatic on supported recent models. DaisyClaw does not need to inject block-level cache markers.
- DaisyClaw uses `prompt_cache_key` to keep cache routing stable across turns. Direct OpenAI hosts use `prompt_cache_retention: "24h"` when `cacheRetention: "long"` is selected.
...
- OpenAI responses expose cached prompt tokens via `usage.prompt_tokens_details.cached_tokens` (or `input_tokens_details.cached_tokens` on Responses API events). DaisyClaw maps that to `cacheRead`.
...
For `openrouter/anthropic/*` model refs, DaisyClaw injects Anthropic
...
handles provider-side prompt caching automatically. DaisyClaw does not inject
...
If you repoint the model at an arbitrary OpenAI-compatible proxy URL, DaisyClaw
...
- Direct Gemini transport (`api: "google-generative-ai"`) reports cache hits
  through upstream `cachedContentTokenCount`; DaisyClaw maps that to `cacheRead`.
- When `cacheRetention` is set on a direct Gemini model, DaisyClaw automatically
...
- This is separate from Anthropic/OpenAI prompt-prefix caching. For Gemini,
  DaisyClaw manages a provider-native `cachedContents` resource rather than
...
- Gemini CLI JSON output can also surface cache hits through `stats.cached`;
  DaisyClaw maps that to `cacheRead`.
- If the CLI omits a direct `stats.input` value, DaisyClaw derives input tokens
  from `stats.input_tokens - stats.cached`.
- This is usage normalization only. It does not mean DaisyClaw is creating
  Anthropic/OpenAI-style prompt-cache markers for Gemini CLI.

## System-prompt cache boundary

DaisyClaw splits the system prompt into a **stable prefix** and a **volatile
...
## DaisyClaw cache-stability guards

DaisyClaw also keeps several cache-sensitive payload shapes deterministic before
...
DaisyClaw exposes dedicated cache-trace diagnostics for embedded agent runs.
...
DaisyClaw keeps one combined live cache regression gate for repeated prefixes, tool turns, image turns, MCP-style tool transcripts, and an Anthropic no-cache control.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.daisyclaw/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$DAISYCLAW_STATE_DIR/logs/cache-trace.jsonl`
...
- `DAISYCLAW_CACHE_TRACE=1` enables cache tracing.
- `DAISYCLAW_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `DAISYCLAW_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `DAISYCLAW_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `DAISYCLAW_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
...
- If you need request tracing, log request IDs and rate-limit headers separately from cache metrics. DaisyClaw's current cache-trace output is focused on prompt/session shape and normalized token usage rather than raw provider response headers.
