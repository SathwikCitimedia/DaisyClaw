...
read_when:
  ...
  - You need the numbers behind the DaisyClaw performance and dependency blog post
  - You are changing release gates, package shrinkwrap, or plugin dependency boundaries
title: "Release performance sweep"
---

This page captures the evidence behind the May 2026 DaisyClaw performance,
...
- **Release performance sweep:** GitHub Releases from `v2026.5.28` back through
  stable `v2026.4.23`, using the `DaisyClaw Performance` workflow,
...
- **npm package size sweep:** `npm pack daisyclaw@<version> --dry-run --json`
...
<CardGroup cols={2}>
  ...
  <Card title="Latest stable install" icon="hard-drive">
    **361.7MiB fresh install**
    ...
    `v2026.5.28` cuts the nested DaisyClaw dependency tree sharply, but a
...
<CardGroup cols={2}>
  ...
  <Card title="Shrinkwrap introduced" icon="lock">
    **1,020.6MB install**
    ...
    911.8MB landed under nested `daisyclaw/node_modules`.
  ...
  <Card title="Latest stable" icon="tag">
    **361.7MiB install**
    ...
    installs a 259.7MiB nested DaisyClaw tree.
...
<CardGroup cols={2}>
  ...
  <Card title="Nested tree" icon="unplug">
    Nested `daisyclaw/node_modules` fell from **656.1MiB** to **259.7MiB** in
...
| Nested `daisyclaw/node_modules` from `2026.5.22` |   911.8MB |     259.7MiB | 71.5% lower |
| Nested `daisyclaw/node_modules` from `2026.5.27` |  656.1MiB |     259.7MiB | 60.4% lower |
...
| Point              | Installed deps | Fresh install | DaisyClaw package | Nested `daisyclaw/node_modules` | Root shrinkwrap | Canvas install behavior                   |
| ------------------ | -------------: | ------------: | ---------------: | -----------------------------: | --------------- | ----------------------------------------- |
...
<CardGroup cols={2}>
  <Card title="Before shrinkwrap" icon="unlock">
    `2026.5.20` has no root shrinkwrap and no large nested DaisyClaw dependency
    tree.
  </Card>
  <Card title="Introduced" icon="lock">
    `2026.5.22` adds root shrinkwrap and installs 911.8MB under nested
    `daisyclaw/node_modules`.
  </Card>
  <Card title="Latest stable" icon="tag">
...
that made npm materialize a large nested DaisyClaw dependency tree and all 12
