...
- In scope: strictly user-supplied credentials that DaisyClaw does not mint or rotate.
- Out of scope: runtime-minted or rotating credentials, OAuth refresh material, and session-like artifacts.

## Supported credentials

### `daisyclaw.json` targets (`secrets configure` + `secrets apply` + `secrets audit`)
...
- In `daisyclaw.json`, SecretRefs must use structured objects such as `{"source":"env","provider":"default","id":"DISCORD_BOT_TOKEN"}`. Legacy `secretref-env:<ENV_VAR>` marker strings are rejected on SecretRef credential paths; run `daisyclaw doctor --fix` to migrate valid markers.
...
- Marker persistence is source-authoritative: DaisyClaw writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
- For web search:
