...
DaisyClaw manages sessions end-to-end across these areas:
...
DaisyClaw is designed around a single **Gateway process** that owns session state.
...
DaisyClaw persists sessions in two layers:
...
- Transcripts: `~/.daisyclaw/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

DaisyClaw resolves these via `src/config/sessions.ts`.
...
DaisyClaw no longer creates automatic `sessions.json.bak.*` rotation backups during Gateway writes. The legacy `session.maintenance.rotateBytes` key is ignored and `daisyclaw doctor --fix` removes it from older configs.
...
`DAISYCLAW_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS`, `DAISYCLAW_SESSION_WRITE_LOCK_STALE_MS`, and
`DAISYCLAW_SESSION_WRITE_LOCK_MAX_HOLD_MS`.
...
In `mode: "warn"`, DaisyClaw reports potential evictions but does not mutate the store/files.

Run maintenance on demand:

```bash
daisyclaw sessions cleanup --dry-run
daisyclaw sessions cleanup --enforce
```
...
- **Parent fork policy** uses DaisyClaw's active branch when creating a thread or subagent fork. If that branch is too large, DaisyClaw starts the child with isolated context instead of failing or inheriting unusable history. The sizing policy is automatic; legacy `session.parentForkMaxTokens` config is removed by `daisyclaw doctor --fix`.
...
Transcripts are managed by `daisyclaw/plugin-sdk/agent-sessions`'s `SessionManager`.
...
DaisyClaw intentionally does **not** "fix up" transcripts; the Gateway uses `SessionManager` to read/write them.
...
DaisyClaw does not append AGENTS.md excerpts on top of the compaction summary.

Compaction is **persistent** (unlike session pruning). See [/concepts/session-pruning](/concepts/session-pruning).

## Compaction chunk boundaries and tool pairing

When DaisyClaw splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, DaisyClaw
...
- If a trailing tool-result block would otherwise push the chunk over target,
  DaisyClaw preserves that pending tool block and keeps the unsummarized tail
  intact.
- Aborted/error tool-call blocks do not hold a pending split open.

---

## When auto-compaction happens (DaisyClaw runtime)

In the embedded DaisyClaw agent, auto-compaction triggers in two cases:
...
exceeded`, and similar provider-shaped variants) → compact → retry.
   When the provider reports the attempted token count, DaisyClaw forwards that
   observed count into overflow recovery compaction. If the provider confirms
   overflow but does not expose a parseable count, DaisyClaw passes a minimally
   over-budget synthetic count to compaction engines and diagnostics.
   If overflow recovery still fails, DaisyClaw surfaces explicit guidance to the
...
These are DaisyClaw runtime semantics.

DaisyClaw can also trigger a preflight local compaction before opening the next
...
reopen cost, not raw archival: DaisyClaw still runs normal semantic compaction,
and it requires `truncateAfterCompaction` so the compacted summary can become a
new successor transcript.

For embedded DaisyClaw runs, `agents.defaults.compaction.midTurnPrecheck.enabled: true`
adds an opt-in tool-loop guard. After a tool result is appended and before the
next model call, DaisyClaw estimates the prompt pressure using the same preflight
budget logic used at turn start. If the context no longer fits, the guard does
not compact inside DaisyClaw runtime's `transformContext` hook. It raises a structured
...
before a turn opens, while mid-turn precheck runs later in the embedded DaisyClaw tool
...
DaisyClaw runtime's compaction settings live in agent settings:
...
DaisyClaw also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, DaisyClaw bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it's already higher, DaisyClaw leaves it alone.
- Manual `/compact` honors an explicit `agents.defaults.compaction.keepRecentTokens`
  and keeps DaisyClaw runtime's recent-tail cut point. Without an explicit keep budget,
...
- When `agents.defaults.compaction.truncateAfterCompaction` is enabled,
  DaisyClaw rotates the active transcript to a compacted successor JSONL after
...
- If the provider fails or returns an empty result, DaisyClaw falls back to built-in LLM summarization automatically.
...
- `daisyclaw status` (CLI)
- `daisyclaw sessions` / `sessions --json`
- Gateway logs (`pnpm gateway:watch` or `daisyclaw logs --follow`): `embedded run auto-compaction start` + `complete`
...
DaisyClaw supports "silent" turns for background tasks where the user should not see intermediate output.
...
- DaisyClaw strips/suppresses this in the delivery layer.
...
As of `2026.1.10`, DaisyClaw also suppresses **draft/typing streaming** when a
...
DaisyClaw uses the **pre-threshold flush** approach:

1. Monitor session context usage.
2. When it crosses a "soft threshold" (below DaisyClaw runtime's compaction threshold), run a silent
...
- The flush runs only for embedded DaisyClaw sessions (CLI backends skip it).
- The flush is skipped when the session workspace is read-only (`workspaceAccess: "ro"` or `"none"`).
- See [Memory](/concepts/memory) for the workspace file layout and write patterns.

DaisyClaw also exposes a `session_before_compact` hook in the extension API, but DaisyClaw's
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `daisyclaw status`.
