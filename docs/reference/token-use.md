---
summary: "How DaisyClaw builds prompt context and reports token usage + costs"
read_when:
...
DaisyClaw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

DaisyClaw assembles its own system prompt on every run. It includes:
...
`toolResultMaxChars` is an advanced ceiling. When it is unset, DaisyClaw chooses
...
For images, DaisyClaw downscales transcript/tool image payloads before provider calls.
...
- `/usage off|tokens|full` → appends a **per-response usage footer** to every reply.
  ...
  - `/usage full` shows estimated cost only when DaisyClaw has usage metadata and
    local pricing for the active model. Otherwise it shows tokens only.
- `/usage cost` → shows a local cost summary from DaisyClaw session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `daisyclaw status --usage` and `daisyclaw channels list` show
...
available; otherwise DaisyClaw falls back to matching OAuth/API-key credentials
...
DaisyClaw keeps provider usage accounting separate from the current context
...
`cacheWrite`. If pricing is missing, DaisyClaw shows tokens only. Cost display is
...
After sidecars and channels reach the Gateway ready path, DaisyClaw starts an
...
Provider prompt caching only applies within the cache TTL window. DaisyClaw can
...
DaisyClaw sizes GA-capable Claude 4.x models such as Opus 4.8, Opus 4.7, Opus 4.6, and
...
Older configs can keep `context1m: true`, but DaisyClaw no longer sends
...
DaisyClaw preserves the OAuth-required Anthropic beta headers while stripping the
