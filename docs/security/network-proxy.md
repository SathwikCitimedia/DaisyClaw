---
summary: "How to route DaisyClaw runtime HTTP and WebSocket traffic through an operator-managed filtering proxy"
title: "Network proxy"
read_when:
  - You want defense-in-depth against SSRF and DNS rebinding attacks
  - Configuring an external forward proxy for DaisyClaw runtime traffic
---

DaisyClaw can route runtime HTTP and WebSocket traffic through an operator-managed forward proxy. This is optional defense in depth for deployments that want central egress control, stronger SSRF protection, and better network auditability.

DaisyClaw does not ship, download, start, configure, or certify a proxy. You run the proxy technology that fits your environment, and DaisyClaw routes normal process-local HTTP and WebSocket clients through it.
...
- Operational control: enforce destination rules, network segmentation, rate limits, or outbound allowlists without rebuilding DaisyClaw.

Proxy routing is a process-level guardrail for normal HTTP and WebSocket egress. It gives operators a fail-closed path for routing supported JavaScript HTTP clients through their own filtering proxy, but it is not an OS-level network sandbox and does not make DaisyClaw certify the proxy's destination policy.

## How DaisyClaw routes traffic

When `proxy.enabled=true` and a proxy URL is configured, protected runtime processes such as `daisyclaw gateway run`, `daisyclaw node run`, and `daisyclaw agent --local` route normal HTTP and WebSocket egress through the configured proxy:
...
The public contract is the routing behavior, not the internal Node hooks used to implement it. DaisyClaw Gateway control-plane WebSocket clients use a narrow direct path for local loopback Gateway RPC traffic when the Gateway URL uses `localhost` or a literal loopback IP such as `127.0.0.1` or `[::1]`. That control-plane path must be able to reach loopback Gateways even when the operator proxy blocks loopback destinations. Normal runtime HTTP and WebSocket requests still use the configured proxy.

Internally, DaisyClaw installs Proxyline as the process-level routing runtime for this feature. Proxyline covers `fetch`, undici-backed clients, Node core `node:http` / `node:https` callers, common WebSocket clients, and helper-created CONNECT tunnels. Managed proxy mode replaces caller-provided Node HTTP agents so explicit agents do not accidentally bypass the operator proxy.

Some plugins own custom transports that need explicit proxy wiring even when process-level routing exists. For example, Telegram's Bot API transport uses its own HTTP/1 undici dispatcher and therefore honors process proxy env plus the managed `DAISYCLAW_PROXY_URL` fallback in that owner-specific transport path.

The proxy URL itself can use either `http://` or `https://`. These schemes describe the connection from DaisyClaw to the proxy endpoint:

- `http://proxy.example:3128`: DaisyClaw opens a plain TCP connection to the forward proxy and sends HTTP proxy requests, including `CONNECT` for HTTPS destinations.
- `https://proxy.example:8443`: DaisyClaw opens TLS to the proxy endpoint, verifies the proxy certificate, and then sends HTTP proxy requests inside that TLS session.

Destination HTTPS is separate from proxy endpoint TLS. For an HTTPS destination, DaisyClaw still asks the proxy for an HTTP `CONNECT` tunnel and then starts destination TLS through that tunnel.

While the proxy is active, DaisyClaw clears `no_proxy` and `NO_PROXY`. Those bypass lists are destination-based, so leaving `localhost` or `127.0.0.1` there would let high-risk SSRF targets skip the filtering proxy.

On shutdown, DaisyClaw restores the previous proxy environment and resets cached process routing state.

## Related proxy terms

- `proxy.enabled` / `proxy.proxyUrl`: outbound forward-proxy routing for DaisyClaw runtime egress. This page documents that feature.
...
- `daisyclaw proxy`: local debug proxy and capture inspector for development and support. See [daisyclaw proxy](/cli/proxy).
...
proxy:
  ...
  tls:
    caFile: /etc/daisyclaw/proxy-ca.pem
```

You can also provide the URL through the environment, while keeping `proxy.enabled=true` in config:

```bash
DAISYCLAW_PROXY_URL=http://127.0.0.1:3128 daisyclaw gateway run
```

`proxy.proxyUrl` takes precedence over `DAISYCLAW_PROXY_URL`.
...
- `gateway-only` (default): DaisyClaw registers the Gateway loopback authority in Proxyline's managed bypass policy so local Gateway WebSocket traffic can connect directly. Custom loopback Gateway ports work because the active Gateway URL's host and port are registered. The bundled browser plugin can also register the exact local CDP readiness and DevTools WebSocket endpoints for DaisyClaw-launched managed browsers, and the bundled Ollama memory embedding provider can use its own narrower guarded direct path for the exact configured host-local loopback embedding origin.
- `proxy`: DaisyClaw does not register Gateway or Ollama loopback bypasses, so that loopback traffic is sent through the managed proxy. If the proxy is remote, it must provide special routing for the DaisyClaw host's loopback service, such as mapping it to a proxy-reachable hostname, IP, or tunnel. Standard remote proxies resolve `127.0.0.1` and `localhost` from the proxy host, not from the DaisyClaw host.
- `block`: DaisyClaw denies Gateway loopback control-plane connections and guarded Ollama host-local embedding loopback connections before opening a socket.
...
For managed gateway services started with `daisyclaw gateway start`, prefer storing the URL in config:

```bash
daisyclaw config set proxy.enabled true
daisyclaw config set proxy.proxyUrl http://127.0.0.1:3128
daisyclaw gateway install --force
daisyclaw gateway start
```

The environment fallback is best for foreground runs. If you use it with an installed service, put `DAISYCLAW_PROXY_URL` in the service durable environment, such as `$DAISYCLAW_STATE_DIR/.env` or `~/.daisyclaw/.env`, then reinstall the service so launchd, systemd, or Scheduled Tasks starts the gateway with that value.

For `daisyclaw --container ...` commands, DaisyClaw forwards `DAISYCLAW_PROXY_URL` into the container-targeted child CLI when it is set. The URL must be reachable from inside the container; `127.0.0.1` refers to the container itself, not the host. DaisyClaw rejects loopback proxy URLs for container-targeted commands unless you explicitly override that safety check.

## Proxy Requirements

The proxy policy is the security boundary. DaisyClaw cannot verify that the proxy blocks the right targets.

Configure the proxy to:

- Bind only to loopback or a private trusted interface.
- Restrict access so only the DaisyClaw process, host, container, or service account can use it.
...
DaisyClaw application-level classifier logic lives in `src/infra/net/ssrf.ts` and `packages/net-policy/src/ip.ts`. The relevant parity hooks are `BLOCKED_HOSTNAMES`, `BLOCKED_IPV4_SPECIAL_USE_RANGES`, `BLOCKED_IPV6_SPECIAL_USE_RANGES`, `RFC2544_BENCHMARK_PREFIX`, and the embedded IPv4 sentinel handling for NAT64, 6to4, Teredo, ISATAP, and IPv4-mapped forms. Those files are useful references when maintaining an external proxy policy, but DaisyClaw does not automatically export or enforce those rules in your proxy.
...
Then enable DaisyClaw proxy routing:

```bash
daisyclaw config set proxy.enabled true
daisyclaw config set proxy.proxyUrl https://proxy.corp.example:8443
daisyclaw config set proxy.tls.caFile /etc/daisyclaw/proxy-ca.pem
...
- Gateway loopback control-plane traffic defaults to direct local bypass through `proxy.loopbackMode: "gateway-only"`. DaisyClaw implements that bypass by registering the active Gateway loopback authority in Proxyline's managed bypass policy. Operators can set `proxy.loopbackMode: "proxy"` to send Gateway loopback traffic through the managed proxy, or `proxy.loopbackMode: "block"` to deny loopback Gateway connections. See [Gateway Loopback Mode](#gateway-loopback-mode) for the remote-proxy caveat.
- Raw `net`, `tls`, and `http2` sockets, native addons, and non-DaisyClaw child processes may bypass Node-level proxy routing unless they inherit and respect proxy environment variables. Forked DaisyClaw child CLIs inherit the managed proxy URL and `proxy.loopbackMode` state.
...
- DaisyClaw does not inspect, test, or certify your proxy policy.
