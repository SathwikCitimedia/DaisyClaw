...
description: Fleet supervision plan for Codex app-server sessions controlled by DaisyClaw.
readWhen:
  - Designing Codex fleet supervision
  - Building DaisyClaw tools that read, steer, or spawn Codex sessions
...
Claw Supervisor lets one always-on DaisyClaw instance monitor and drive a fleet of Codex sessions without changing the normal Codex user experience. A user can SSH into a host, start Codex, work in the TUI, and still have the supervisor read the session, steer it, interrupt it, spawn related sessions, and accept handoffs. Codex sessions can also call back into DaisyClaw through MCP.

## Product Model

Codex remains the primary work surface. DaisyClaw supervises Codex rather than hiding Codex inside an opaque DaisyClaw subagent.

The DaisyClaw plugin is named `codex-supervisor`. `crabfleet` remains the deployment
...
- Supervisor Claw: an always-on DaisyClaw agent with tools for fleet state, transcript reads, steering, interruption, spawning, and handoff.

DaisyClaw may use its existing subagent machinery internally, but the external contract is an attachable Codex session with a Codex thread id.
...
- DaisyClaw tools for Claw-to-Codex control.
...
Every supervised Codex gets an MCP server named `daisyclaw-codex-supervisor`.
...
- Supervisor state lives in DaisyClaw Gateway storage.
...
- Transcript reads redact secrets at DaisyClaw boundary.
...
- Durable state owner: DaisyClaw Gateway DB, Cloudflare Durable Object, or VPS database.
