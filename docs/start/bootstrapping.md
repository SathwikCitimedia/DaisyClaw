...
On the first agent run, DaisyClaw bootstraps the workspace (default
`~/.daisyclaw/workspace`):
...
For embedded/local model runs, DaisyClaw keeps `BOOTSTRAP.md` out of the
...
To skip this for a pre-seeded workspace, run `daisyclaw onboard --skip-bootstrap`.
...
host (for example, `user@gateway-host:~/.daisyclaw/workspace`).
</Note>
