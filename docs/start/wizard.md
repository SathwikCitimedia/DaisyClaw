...
CLI onboarding is the **recommended** terminal setup path for DaisyClaw on
...
`DAISYCLAW_LOCALE`, then `LC_ALL`, then `LC_MESSAGES`, then `LANG`, and falls
...
Names and stable identifiers stay literal: `DaisyClaw`, `Gateway`, `Tailscale`,
commands, config keys, URLs, provider IDs, model IDs, and plugin/channel labels
are not translated.

<Info>
Fastest first chat: open the Control UI (no channel setup needed). Run
`daisyclaw dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`daisyclaw configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.daisyclaw/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `daisyclaw doctor` first.
</Note>
...
Use `daisyclaw agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.daisyclaw/workspace-<agentId>`.
...
- CLI command reference: [`daisyclaw onboard`](/cli/onboard)
