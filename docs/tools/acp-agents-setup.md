...
read_when:
  ...
  - Enabling the plugin-tools or DaisyClaw-tools MCP bridge
...
Codex has two DaisyClaw routes:
...
When DaisyClaw uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.
...
Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal DaisyClaw `agentId` path).

Model control is adapter-capability dependent. Codex ACP model refs are
normalized by DaisyClaw before startup. Other harnesses need ACP `models` plus
`session/set_model` support; if a harness exposes neither that ACP capability
nor its own startup model flag, DaisyClaw/acpx cannot force a model selection.
...
Packaged installs use the official `@daisyclaw/acpx` runtime plugin for ACP.
...
```bash
daisyclaw plugins install ./path/to/local/acpx-plugin
```
...
`ready` signal. Set `DAISYCLAW_ACPX_RUNTIME_STARTUP_PROBE=0` or
`DAISYCLAW_SKIP_ACPX_RUNTIME_PROBE=1` only for scripts or environments that
...
- `command` accepts an absolute path, relative path (resolved from the DaisyClaw workspace), or command name.
...
- `agents.<id>.args` is optional. Each array item is shell-quoted before DaisyClaw passes it through the current acpx command-string registry.

See [Plugins](/tools/plugin).

### Automatic dependency install

When you install DaisyClaw globally with `npm install -g daisyclaw`, the acpx
...
normally and reports the missing dependency through `daisyclaw acp doctor`.

### Plugin tools MCP bridge

By default, ACPX sessions do **not** expose DaisyClaw plugin-registered tools to
the ACP harness.

If you want ACP agents such as Codex or Claude Code to call installed
DaisyClaw plugin tools such as memory recall/store, enable the dedicated bridge:

```bash
daisyclaw config set plugins.entries.acpx.config.pluginToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `daisyclaw-plugin-tools` into ACPX session
  bootstrap.
- Exposes plugin tools already registered by installed and enabled DaisyClaw
  plugins.
...
### DaisyClaw tools MCP bridge

By default, ACPX sessions also do **not** expose built-in DaisyClaw tools through
MCP. Enable the separate core-tools bridge when an ACP agent needs selected
built-in tools such as `cron`:

```bash
daisyclaw config set plugins.entries.acpx.config.daisyClawToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `daisyclaw-tools` into ACPX session
  bootstrap.
- Exposes selected built-in DaisyClaw tools. The initial server exposes `cron`.
...
```bash
daisyclaw config set plugins.entries.acpx.config.timeoutSeconds 180
```

Runtime turns use DaisyClaw agent/run timeouts, including `/acp timeout`.
...
```bash
daisyclaw config set plugins.entries.acpx.config.probeAgent claude
```
...
These ACPX harness permissions are separate from DaisyClaw exec approvals and separate from CLI-backend vendor bypass flags such as Claude CLI `--permission-mode bypassPermissions`. ACPX `approve-all` is the harness-level break-glass switch for ACP sessions.

For the broader comparison between DaisyClaw `tools.exec.mode`, Codex Guardian
...
```bash
daisyclaw config set plugins.entries.acpx.config.permissionMode approve-all
daisyclaw config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

<Warning>
DaisyClaw defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
