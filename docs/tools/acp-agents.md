---
summary: "Run external coding harnesses (Claude Code, Cursor, Gemini CLI, explicit Codex ACP, DaisyClaw ACP, OpenCode) through the ACP backend"
read_when:
...
let DaisyClaw run external coding harnesses (for example Claude Code,
Cursor, Copilot, Droid, DaisyClaw ACP, OpenCode, Gemini CLI, and other
...
directly to existing DaisyClaw channel conversations, use
[`daisyclaw mcp serve`](/cli/mcp) instead of ACP.
</Note>
...
| Run Claude Code, Gemini CLI, explicit Codex ACP, or another external harness _through_ DaisyClaw | This page                             | Chat-bound sessions, `/acp spawn`, `sessions_spawn({ runtime: "acp" })`, background tasks, runtime controls                                                                                   |
| Expose an DaisyClaw Gateway session _as_ an ACP server for an editor or client                   | [`daisyclaw acp`](/cli/acp)            | Bridge mode. IDE/client talks ACP to DaisyClaw over stdio/WebSocket                                                                                                                            |
| Reuse a local AI CLI as a text-only fallback model                                              | [CLI Backends](/gateway/cli-backends) | Not ACP. No DaisyClaw tools, no ACP controls, no harness runtime                                                                                                                               |
...
daisyclaw config set plugins.entries.acpx.enabled true
```
...
DaisyClaw only teaches agents about ACP spawning when ACP is **truly
...
<AccordionGroup>
  <Accordion title="First-run gotchas">
    ...
    - Codex ACP runs with an isolated `CODEX_HOME`; DaisyClaw copies trusted project entries plus safe model/provider routing config from the host Codex config, while auth, notifications, and hooks stay on the host config.
  ...
  <Accordion title="Runtime prerequisites">
    ACP launches a real external harness process. DaisyClaw owns routing,
    ...
    Before blaming DaisyClaw, verify:
...
DaisyClaw plugin tools and built-in DaisyClaw tools are **not** exposed to
...
| `daisyclaw` | DaisyClaw Gateway bridge through `daisyclaw acp` | Lets an ACP-aware harness talk back to an DaisyClaw Gateway session.                 |
| `qwen`     | Qwen Code / Qwen CLI                           | Requires Qwen-compatible auth on the host.                                          |

Custom acpx agent aliases can be configured in acpx itself, but DaisyClaw
...
<AccordionGroup>
  <Accordion title="Lifecycle details">
    - Spawn creates or resumes an ACP runtime session, records ACP metadata in the DaisyClaw session store, and may create a background task when the run is parent-owned.
    ...
    - `close` ends the ACP session from DaisyClaw's point of view and removes the binding. A harness may still keep its own upstream history if it supports resume.
    - The acpx plugin cleans up DaisyClaw-owned wrapper and adapter process trees after `close`, and reaps stale DaisyClaw-owned ACPX orphans during Gateway startup.
  ...
  <Accordion title="Native Codex routing rules">
    ...
    DaisyClaw dynamic tools still execute through DaisyClaw, while
    Codex-native tools such as shell/apply-patch execute inside Codex.
    For Codex-native tool events, DaisyClaw injects a per-turn native
    hook relay so plugin hooks can block `before_tool_call`, observe
    `after_tool_call`, and route Codex `PermissionRequest` events
    through DaisyClaw approvals. Codex `Stop` hooks are relayed to
    DaisyClaw `before_agent_finalize`, where plugins can request one more
  ...
  <Accordion title="ACP-routing natural-language triggers">
    ...
    DaisyClaw picks `runtime: "acp"`, resolves the harness `agentId`,
    ...
    DaisyClaw config agent id from `agents_list` unless that entry is
    explicitly configured with `agents.list[].runtime.type="acp"`;
    otherwise use the default sub-agent runtime. When an DaisyClaw agent
    is configured with `runtime.type="acp"`, DaisyClaw uses
...
plugin is enabled. Use **sub-agents** when you want DaisyClaw-native
delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | DaisyClaw native sub-agent runtime  |
...
1. DaisyClaw ACP session control plane.
2. Official `@daisyclaw/acpx` runtime plugin.
...
- **ACP session** - the durable Codex/Claude/Gemini runtime state DaisyClaw routes to.
...
spawned ACP session - no child thread, same chat surface. DaisyClaw keeps
...
<AccordionGroup>
  <Accordion title="Binding rules and exclusivity">
    ...
    - `--bind here` only works on channels that advertise current-conversation binding; DaisyClaw returns a clear unsupported message otherwise. Bindings persist across gateway restarts.
    - On Discord, `spawnSessions` gates child thread creation for `--thread auto|here` - not `--bind here`.
    - If you spawn to a different ACP agent without `--cwd`, DaisyClaw inherits the **target agent's** workspace by default. Missing inherited paths (`ENOENT`/`ENOTDIR`) fall back to the backend default; other access errors (e.g. `EACCES`) surface as spawn errors.
    - Gateway management commands stay local in bound conversations - `/acp ...` commands are handled by DaisyClaw even when normal follow-up text routes to the bound ACP session; `/status` and `/unfocus` also stay local whenever command handling is enabled for that surface.

  </Accordion>
  <Accordion title="Thread-bound sessions">
    When thread bindings are enabled for a channel adapter:

    - DaisyClaw binds a thread to a target ACP session.
    ...
    adapter does not support thread bindings, DaisyClaw returns a clear
...
- DaisyClaw ensures the configured ACP session exists before use.
...
- For cross-agent ACP spawns without an explicit `cwd`, DaisyClaw inherits the target agent workspace from agent config.
...
<Tabs>
  <Tab title="From sessions_spawn">
    ...
    for ACP sessions. If `agentId` is omitted, DaisyClaw uses
...
<ParamField path="mode" type='"run" | "session"' default="run">
  ...
  `mode` is omitted, DaisyClaw may default to persistent behaviour per
...
<ParamField path="model" type="string">
  ...
  `session/set_model`; otherwise DaisyClaw/acpx fails clearly instead of
...
<AccordionGroup>
  <Accordion title="Interactive ACP sessions">
    ...
    What DaisyClaw sends to the harness:
    ...
    - Runtime-generated completion events are materialized per target. DaisyClaw agents get DaisyClaw's internal runtime-context envelope; external ACP harnesses get a plain prompt with the child result and instruction. The raw `<<<BEGIN_DAISYCLAW_INTERNAL_CONTEXT>>>` envelope should never be sent to external harnesses or persisted as ACP user transcript text.
    - ACP transcript entries use the user-visible trigger text or the plain completion prompt. Internal event metadata stays structured in DaisyClaw where possible and is not treated as user-authored chat content.
  ...
  <Accordion title="Parent-owned one-shot ACP sessions">
    ...
    - Completion reports back through the task-completion announce path. DaisyClaw converts internal completion metadata into a plain ACP prompt before sending it to an external harness, so harnesses do not see DaisyClaw-only runtime context markers.
  ...
  <Accordion title="sessions_send and A2A delivery">
    ...
    peer sessions, DaisyClaw uses an agent-to-agent (A2A) follow-up path
    ...
    DaisyClaw skips the A2A follow-up only when the requester is the
  ...
  <Accordion title="Resume an existing session">
    ...
    - `resumeSessionId` is a host-local ACP/harness resume id, not an DaisyClaw channel session key; DaisyClaw still checks ACP spawn policy and target agent policy before dispatch, while the ACP backend or harness owns authorization for loading that upstream id.
    - `resumeSessionId` restores the upstream ACP conversation history; `thread` and `mode` still apply normally to the new DaisyClaw session you are creating, so `mode: "session"` still requires `thread: true`.
...
- DaisyClaw's sandbox policy does **not** wrap ACP harness execution.
- DaisyClaw still enforces ACP feature gates, allowed agents, session ownership, channel bindings, and Gateway delivery policy.
- Use `runtime: "subagent"` for sandbox-enforced DaisyClaw-native work.
...
If no target resolves, DaisyClaw returns a clear error
...
| ---------------------------- | ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/acp model <id>`            | runtime config key `model`           | For Codex ACP, DaisyClaw normalizes `openai/<model>` to the adapter model id and maps slash reasoning suffixes such as `openai/gpt-5.4/high` to `reasoning_effort`.                                         |
| `/acp set thinking <level>`  | canonical option `thinking`          | DaisyClaw sends the backend-advertised equivalent when present, preferring `thinking`, then `effort`, `reasoning_effort`, or `thought_level`. For Codex ACP, the adapter maps values to `reasoning_effort`. |
| `/acp permissions <profile>` | canonical option `permissionProfile` | DaisyClaw sends the backend-advertised equivalent when present, such as `approval_policy`, `permission_profile`, `permissions`, or `permission_mode`.                                                       |
| `/acp timeout <seconds>`     | canonical option `timeoutSeconds`    | DaisyClaw sends the backend-advertised equivalent when present, such as `timeout` or `timeout_seconds`.                                                                                                     |
...
aliases), the plugin-tools and DaisyClaw-tools MCP bridges, and ACP
...
| Vendor auth error from the harness                                          | DaisyClaw is healthy, but the target CLI/provider is not logged in.                                                     | Log in or provide the required provider key on the Gateway host environment.                                                                                             |
...
| ACP session stalls indefinitely after completing work                       | Harness process finished but ACP session did not report completion.                                                    | Update DaisyClaw; current acpx cleanup reaps DaisyClaw-owned stale wrapper and adapter processes on close and Gateway startup.                                             |
| Harness sees `<<<BEGIN_DAISYCLAW_INTERNAL_CONTEXT>>>`                        | Internal event envelope leaked across the ACP boundary.                                                                | Update DaisyClaw and rerun the completion flow; external harnesses should receive plain completion prompts only.                                                          |
...
native tool call, restart the Codex app-server or DaisyClaw Gateway instead of
...
- [`daisyclaw acp` (bridge mode)](/cli/acp)
