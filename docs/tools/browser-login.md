...
When a site requires login, **sign in manually** in the **host** browser profile (the daisyclaw browser).
...
DaisyClaw controls a **dedicated Chrome profile** (named `daisyclaw`, orange-tinted UI). This is separate from your daily browser profile.

For agent browser tool calls:

- Default choice: the agent should use its isolated `daisyclaw` browser.
...
daisyclaw browser open https://x.com
```

If you have multiple profiles, pass `--browser-profile <name>` (the default is `daisyclaw`).
...
```bash
daisyclaw browser open https://x.com --browser-profile daisyclaw
```
