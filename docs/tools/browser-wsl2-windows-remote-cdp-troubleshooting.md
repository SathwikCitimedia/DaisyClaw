...
read_when:
  - Running DaisyClaw Gateway in WSL2 while Chrome lives on Windows
...
In the common split-host setup, DaisyClaw Gateway runs inside WSL2, Chrome runs on Windows, and browser control must cross the WSL2 and Windows boundary. The layered failure pattern from [issue #39369](https://github.com/daisyclaw/daisyclaw/issues/39369) means several independent problems can show up at once, which makes the wrong layer look broken first.
...
- DaisyClaw and Chrome are on the same machine
...
- DaisyClaw points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, DaisyClaw is not the problem yet.
...
Fix that before touching DaisyClaw config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point DaisyClaw at the address that is reachable from WSL2:
...
- use HTTP(S) when you want DaisyClaw to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools socket URL
- test the same URL with `curl` before expecting DaisyClaw to succeed
...
```bash
daisyclaw browser open https://example.com --browser-profile remote
daisyclaw browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `daisyclaw browser tabs` returns the target
...
- stale viewport / dark-mode / locale / offline overrides after a remote session
  - run `daisyclaw browser stop --browser-profile remote`
...
3. DaisyClaw config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug DaisyClaw config or Control UI auth
