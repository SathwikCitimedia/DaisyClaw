...
read_when:
  ...
  - Debugging why daisyclaw is interfering with your own Chrome
  - Implementing browser settings + lifecycle in the macOS app
title: "Browser (DaisyClaw-managed)"
---

DaisyClaw can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls.
...
- The `daisyclaw` profile does **not** touch your personal browser profile.
- The agent can **open tabs, read pages, click, and type** in a safe lane.
- The built-in `user` profile attaches to your real signed-in Chrome session via Chrome MCP.

## What you get

- A separate browser profile named **daisyclaw** (orange accent by default).
...
- A bundled `browser-automation` skill that teaches agents the snapshot,
  ...
  plugin is enabled.
- Optional multi-profile support (`daisyclaw`, `work`, `remote`, ...).
...
daisyclaw browser --browser-profile daisyclaw doctor --deep
daisyclaw browser --browser-profile daisyclaw status
...
If `daisyclaw browser` is missing entirely, or the agent says the browser tool
...
Defaults need both `plugins.entries.browser.enabled` **and** `browser.enabled=true`. Disabling only the plugin removes the `daisyclaw browser` CLI, `browser.request` gateway method, agent tool, and control service as one unit; your `browser.*` config stays intact for a replacement.
...
If `daisyclaw browser` is unknown after an upgrade, `browser.request` is missing, or the agent reports the browser tool as unavailable, the usual cause is a `plugins.allow` list that omits `browser` and no root `browser` config block exists. Add it:
...
## Profiles: `daisyclaw` vs `user`

- `daisyclaw`: managed, isolated browser (no extension required).
- `user`: built-in Chrome MCP attach profile for your **real signed-in Chrome**
  session.

For agent browser tool calls:

- Default: use the isolated `daisyclaw` browser.
...
Set `browser.defaultProfile: "daisyclaw"` if you want managed mode by default.

## Configuration

Browser settings live in `~/.daisyclaw/daisyclaw.json`.
...
understanding model is configured, DaisyClaw keeps the normal image result so the
...
- Control service binds to loopback on a port derived from `gateway.port` (default `18791` = gateway + 2). Overriding `gateway.port` or `DAISYCLAW_GATEWAY_PORT` shifts the derived ports in the same family.
- Local `daisyclaw` profiles auto-assign `cdpPort`/`cdpUrl`; set those only for
...
- Repeated managed Chrome launch/readiness failures are circuit-broken per
  profile. After several consecutive failures, DaisyClaw pauses new launch
...
- Gateway/provider `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, and `NO_PROXY` environment variables do not automatically proxy the DaisyClaw-managed browser. Managed Chrome launches direct by default so provider proxy settings do not weaken browser SSRF checks.
- DaisyClaw-managed local CDP readiness probes and DevTools WebSocket connections bypass the managed network proxy for the exact launched loopback endpoint, so `daisyclaw browser start` still works when an operator proxy blocks loopback egress.
...
- `POST /start?headless=true` and `daisyclaw browser start --headless` request a
  ...
  remote CDP profiles reject the override because DaisyClaw does not launch those
  browser processes.
- On Linux hosts without `DISPLAY` or `WAYLAND_DISPLAY`, local managed profiles
  default to headless automatically when neither the environment nor profile/global
  config explicitly chooses headed mode. `daisyclaw browser status --json`
  reports `headlessSource` as `env`, `profile`, `config`,
  `request`, `linux-display-fallback`, or `default`.
- `DAISYCLAW_BROWSER_HEADLESS=1` forces local managed launches headless for the
  current process. `DAISYCLAW_BROWSER_HEADLESS=0` forces headed mode for ordinary
...
- Default profile is `daisyclaw` (managed standalone). Use `defaultProfile: "user"` to opt into the signed-in user browser.
...
DaisyClaw uses it automatically. Set `browser.executablePath` to override
auto-detection. Top-level and per-profile `executablePath` values accept `~`
for your OS home directory:

```bash
daisyclaw config set browser.executablePath "/usr/bin/google-chrome"
daisyclaw config set browser.profiles.work.executablePath "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
```
...
Per-profile `executablePath` only affects local managed profiles that DaisyClaw
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, DaisyClaw will not launch a local browser.
- For externally managed CDP services on loopback (for example Browserless in
  ...
  without `attachOnly` is treated as a local DaisyClaw-managed browser profile.
- `headless` only affects local managed profiles that DaisyClaw launches. It does not restart or change existing-session or remote CDP browsers.
...
- local managed profiles: `daisyclaw browser stop` stops the browser process that
  DaisyClaw launched
- attach-only and remote CDP profiles: `daisyclaw browser stop` closes the active
  ...
  though no browser process was launched by DaisyClaw
...
DaisyClaw preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, DaisyClaw can
...
- If you set `nodeHost.browserProxy.allowProfiles`, DaisyClaw treats it as a least-privilege boundary: only allowlisted profiles can be targeted, and persistent profile create/delete routes are blocked on the proxy surface.
...
CDP connection URLs over HTTPS and WebSocket. DaisyClaw can use either form, but
...
- If Browserless gives you an HTTPS base URL, you can either convert it to
  `wss://` for a direct CDP connection or keep the HTTPS URL and let DaisyClaw
  discover `/json/version`.

### Browserless Docker on the same host

When Browserless is self-hosted in Docker and DaisyClaw runs on the host, treat
...
DaisyClaw process. Browserless must also advertise a matching reachable endpoint;
set Browserless `EXTERNAL` to that same public-to-DaisyClaw WebSocket base, such
...
an address DaisyClaw cannot reach, CDP HTTP can look healthy while the WebSocket
attach still fails.

Do not leave `attachOnly` unset for a loopback Browserless profile. Without
`attachOnly`, DaisyClaw treats the loopback port as a local managed browser
profile and may report that the port is in use but not owned by DaisyClaw.

## Direct WebSocket CDP providers

Some hosted browser services expose a **direct WebSocket** endpoint rather than
the standard HTTP-based CDP discovery (`/json/version`). DaisyClaw accepts three
CDP URL shapes and picks the right connection strategy automatically:

- **HTTP(S) discovery** - `http://host[:port]` or `https://host[:port]`.
  DaisyClaw calls `/json/version` to discover the WebSocket debugger URL, then
...
- **Direct WebSocket endpoints** - `ws://host[:port]/devtools/<kind>/<id>` or
  ...
  path. DaisyClaw connects directly via a WebSocket handshake and skips
...
- **Bare WebSocket roots** - `ws://host[:port]` or `wss://host[:port]` with no
  ...
  [Browserbase](https://www.browserbase.com)). DaisyClaw tries HTTP
  `/json/version` discovery first (normalising the scheme to `http`/`https`);
  if discovery returns a `webSocketDebuggerUrl` it is used, otherwise DaisyClaw
  ...
  accepts it, DaisyClaw falls back to that root as well. This lets a bare `ws://`
...
`daisyclaw browser doctor` uses the same discovery-first, WebSocket-fallback
...
- The standalone loopback browser HTTP API uses **shared-secret auth only**:
  gateway token bearer auth, `x-daisyclaw-password`, or HTTP Basic auth with the
...
- If browser control is enabled and no shared-secret auth is configured, DaisyClaw
  generates a runtime-only gateway token for that startup. Configure
  `gateway.auth.token`, `gateway.auth.password`, `DAISYCLAW_GATEWAY_TOKEN`, or
  `DAISYCLAW_GATEWAY_PASSWORD` explicitly if clients need a stable secret across
  restarts.
- DaisyClaw does **not** auto-generate that token when `gateway.auth.mode` is
...
DaisyClaw supports multiple named profiles (routing configs). Profiles can be:

- **daisyclaw-managed**: a dedicated Chromium-based browser instance with its own user data directory + CDP port
- **remote**: an explicit CDP URL (Chromium-based browser running elsewhere)
- **existing session**: your existing Chrome profile via Chrome DevTools MCP auto-connect

Defaults:

- The `daisyclaw` profile is auto-created if missing.
...
DaisyClaw can also attach to a running Chromium-based browser profile through the
...
3. Keep the browser running and approve the connection prompt when DaisyClaw attaches.
...
```bash
daisyclaw browser --browser-profile user start
daisyclaw browser --browser-profile user status
daisyclaw browser --browser-profile user tabs
daisyclaw browser --browser-profile user snapshot --format ai
```
...
- `daisyclaw doctor` migrates old extension-based browser config and checks that
...
- This path is higher-risk than the isolated `daisyclaw` profile because it can
  act inside your signed-in browser session.
- DaisyClaw does not launch the browser for this driver; it only attaches.
- DaisyClaw uses the official Chrome DevTools MCP `--autoConnect` flow here. If
...
When `cdpUrl` is set on an existing-session profile, DaisyClaw skips
...
Compared to the managed `daisyclaw` profile, existing-session drivers are more constrained:
...
- **Dialog visibility** - Managed browser action responses include `blockedByDialog` and `browserState.dialogs.pending` when an action opens a modal dialog; snapshots also include pending dialog state. Respond with `browser dialog --accept/--dismiss --dialog-id <id>` while a dialog is pending. Dialogs handled outside DaisyClaw appear under `browserState.dialogs.recent`.
...
When launching locally, DaisyClaw picks the first available:
...
control API** plus a matching `daisyclaw browser` CLI (snapshots, refs, wait
...
- **CDP startup or readiness failure** means DaisyClaw cannot confirm that the browser control plane is healthy.
...
- CDP startup or readiness failure:
  - `Chrome CDP websocket for profile "daisyclaw" is not reachable after start`
  - `Remote CDP for profile "<name>" is not reachable at <cdpUrl>`
  - `Port <port> is in use for profile "<name>" but not by daisyclaw` when a
...
- For the local loopback `daisyclaw` managed profile, CDP health checks intentionally skip browser SSRF reachability enforcement for DaisyClaw's own local control plane.
...
- `browser` accepts:
  - `profile` to choose a named browser profile (daisyclaw, chrome, or remote CDP).
