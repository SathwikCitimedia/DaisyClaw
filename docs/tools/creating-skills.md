...
summary: "Build, test, and publish custom SKILL.md workspace skills for your DaisyClaw agents."
read_when:
...
DaisyClaw loads skills from several roots in a defined [precedence order](/tools/skills#loading-order).
...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.daisyclaw/workspace/skills/hello-world
    ```
    ...
    ```bash
    mkdir -p ~/.daisyclaw/workspace/skills/personal/hello-world
  ...
  <Step title="Verify the skill loaded">
    ...
    DaisyClaw watches `SKILL.md` files under skills roots by default. If the
  ...
  <Step title="Test it">
    ...
    ```bash
    daisyclaw agent --message "give me a greeting"
    ```
...
metadata: { "daisyclaw": { "requires": { "bins": ["gemini"] }, "primaryEnv": "GEMINI_API_KEY" } }
---
```
...
<AccordionGroup>
  <Accordion title="Gating options">
    ...
    | `requires.config` | Each `daisyclaw.json` path must be truthy |
  ...
  <Accordion title="Environment and API keys">
    Wire an API key to a skill entry in `daisyclaw.json`:
...
daisyclaw skills workshop propose-update hello-world \
  --proposal ./PROPOSAL.md \
...
<Steps>
  <Step title="Ensure your SKILL.md is complete">
    Make sure `name`, `description`, and any `metadata.daisyclaw` gating fields
  ...
  <Step title="Install the ClawHub skill">
    ...
    ```bash
    daisyclaw skills install clawhub-publish
    ```
...
<Tip>
  ...
  - **Test locally** — use `daisyclaw agent --message "..."` before sharing.
