...
<Steps>
  <Step title="Install the plugin">
    ```bash
    daisyclaw plugins install diffs
    ```
  </Step>
...
DaisyClaw includes syntax highlighting for common source, config, and documentation languages:
...
Set plugin-wide defaults in `~/.daisyclaw/daisyclaw.json`:
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/daisyclaw",
        },
      },
    },
  },
...
- Artifacts are stored under the temp subfolder: `$TMPDIR/daisyclaw-diffs`.
...
<Steps>
  <Step title="Config">
    `browser.executablePath` in DaisyClaw config.
  </Step>
