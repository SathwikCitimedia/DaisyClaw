...
- allowlist entries live in the host-local approvals file under `agents.<id>.allowlist` (or via Control UI / `daisyclaw approvals allowlist ...`).
- `daisyclaw security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `daisyclaw doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, DaisyClaw still rejects the `env` builtin in safe-bin
...
session, DaisyClaw also resumes that session with an internal followup so the agent observes that
the command did not run instead of later repairing a missing result.

### Followup delivery behavior

After an approved async exec finishes, DaisyClaw sends a followup `agent` turn to the same session.
...
the originating channel, DaisyClaw keeps the local deterministic `/approve`
prompt visible. If the native runtime is active and attempts delivery but no
target receives the card, DaisyClaw sends a same-chat fallback notice with the
...
- Matrix native prompts include `com.daisyclaw.approval` custom event content on the first prompt
  event so DaisyClaw-aware Matrix clients can read structured approval state while stock clients
...
owner routing for approval prompts and final results. DaisyClaw first tries a private route on the
...
topics, DaisyClaw preserves the topic for the approval prompt and the post-approval follow-up.
