...
| ---------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| `daisyclaw approvals get` / `--gateway` / `--node <id\|name\|ip>` | Requested policy, host policy sources, and the effective result.                       |
| `daisyclaw exec-policy show`                                      | Local-machine merged view.                                                             |
| `daisyclaw exec-policy set` / `preset`                            | Synchronize the local requested policy with the local host approvals file in one step. |
...
- **Gateway host** → `daisyclaw` process on the gateway machine.
...
- For shell scripts and direct interpreter/runtime file invocations, DaisyClaw also tries to bind one concrete local file operand. If that bound file changes after approval but before execution, the run is denied instead of executing drifted content.
...
`DAISYCLAW_STATE_DIR` is set, the file follows that state directory;
otherwise it uses the default DaisyClaw state directory:

```text
$DAISYCLAW_STATE_DIR/exec-approvals.json
# otherwise
~/.daisyclaw/exec-approvals.json
```

The default approval socket follows the same root:
`$DAISYCLAW_STATE_DIR/exec-approvals.sock`, or
`~/.daisyclaw/exec-approvals.sock` when the variable is unset.
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.daisyclaw/exec-approvals.sock",
...
- `auto` - use allowlist policy, run deterministic matches directly, and send approval misses through DaisyClaw's native auto reviewer before falling back to a human approval route.
...
<ParamField path="askFallback" type='"deny" | "allowlist" | "full"'>
  ...
  field is omitted, DaisyClaw defaults to `deny`.
...
<ParamField path="strictInlineEval" type="boolean">
  When `true`, DaisyClaw treats inline code-eval forms as approval-only
...
<ParamField path="commandHighlighting" type="boolean" default="false">
  ...
  DaisyClaw may attach parser-derived command spans so Web approval
...
**both** policy layers - requested exec policy in DaisyClaw config
(`tools.exec.*`) **and** host-local approvals policy in
the execution host approvals file.

DaisyClaw defaults omitted `askFallback` to `deny`. Set host
...
- In YOLO mode, DaisyClaw does **not** add a separate heuristic command-obfuscation approval gate or script-preflight rejection layer on top of the configured host exec policy.
...
`--permission-mode bypassPermissions` when DaisyClaw's effective exec
policy is YOLO. For DaisyClaw-managed Claude live sessions, DaisyClaw's
...
If you want a more conservative setup, tighten DaisyClaw exec policy back to
...
<Steps>
  ...
  <Step title="Match the host approvals file">
    ```bash
    daisyclaw approvals set --stdin <<'EOF'
    {
      version: 1,
      defaults: {
        security: "full",
...
```bash
daisyclaw exec-policy preset yolo
```
...
approvals remotely, use `daisyclaw approvals set --gateway` or
`daisyclaw approvals set --node <id|name|ip>`.
...
```bash
daisyclaw approvals set --node <id|name|ip> --stdin <<'EOF'
{
  version: 1,
  defaults: {
    security: "full",
...
- `daisyclaw exec-policy` does not synchronize node approvals.
- `daisyclaw exec-policy set --host node` is rejected.
- Node exec approvals are fetched from the node at runtime, so node-targeted updates must use `daisyclaw approvals --node ...`.
...
specific argument shape. DaisyClaw evaluates the regular expression
...
entries instead of hand-editing the encoded value. If DaisyClaw cannot
...
CLI: `daisyclaw approvals` supports gateway or node editing - see
...
DaisyClaw posts the denial back into that session as an internal followup so the
agent can stop waiting on the async command and avoid a missing-result repair.
If there is no session or the session cannot be resumed, DaisyClaw can still
...
When an async exec approval is denied, DaisyClaw treats the host command as
...
session delivery is unavailable, DaisyClaw falls back to a concise operator or
