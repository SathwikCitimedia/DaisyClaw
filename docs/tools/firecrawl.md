...
DaisyClaw can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `daisyclaw configure --section web` enables the bundled Firecrawl plugin automatically.
...
- Legacy `tools.web.fetch.firecrawl.*` config is auto-migrated by `daisyclaw doctor --fix`.
...
when you run Firecrawl yourself. DaisyClaw accepts `http://` only for loopback,
...
DaisyClaw always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
...
The selection knob is `tools.web.fetch.provider`. If you omit it, DaisyClaw
