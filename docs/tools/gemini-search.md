...
DaisyClaw supports Gemini models with built-in
...
For a gateway install, put env keys in `~/.daisyclaw/.env`.
...
`pd`, `pw`, `pm`, and `py`. DaisyClaw converts these values, or an explicit
