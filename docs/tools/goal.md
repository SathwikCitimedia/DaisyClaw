...
read_when:
  - You want DaisyClaw to keep one objective visible across a long session
...
A **goal** is one durable objective attached to the current DaisyClaw session.
...
DaisyClaw waits for the next fresh session token snapshot and uses that as the
...
cost reporting, and context-window behavior still use the normal DaisyClaw
usage and model controls.

## Model tools

DaisyClaw exposes three core goal tools to agent harnesses:
...
The `/goal` command works in command-capable DaisyClaw sessions, including the
...
token snapshot yet. Usage refreshes as DaisyClaw records session usage and
