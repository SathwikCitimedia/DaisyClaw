...
DaisyClaw supports Grok as a `web_search` provider, using xAI web-grounded
...
DaisyClaw reuses it as a fallback for the bundled xAI model provider too.
...
DaisyClaw can use an existing xAI OAuth profile without prompting for a separate
web-search key. If OAuth is not available, it falls back to xAI API-key setup.
DaisyClaw can also show a separate follow-up step to enable `x_search` with the
...
<Steps>
  <Step title="Use xAI OAuth">
    ...
    ```bash
    daisyclaw onboard --auth-choice xai-oauth
...
**Credential alternatives:** sign in with `daisyclaw models auth login
...
route through an operator proxy or xAI-compatible Responses endpoint. DaisyClaw
