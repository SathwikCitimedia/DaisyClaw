...
DaisyClaw records a background task, returns the task id immediately, and wakes
...
generated images are still missing from the completion reply, DaisyClaw sends an
...
<Steps>
  ...
  <Step title="Pick a default model (optional)">
    ...
    `openai` OAuth profile is configured, DaisyClaw routes image requests
...
nearby geometry option instead of the exact requested one, DaisyClaw remaps to
...
DaisyClaw tries providers in this order:
...
<AccordionGroup>
  ...
  <Accordion title="Auto-detection is auth-aware">
    A provider default only enters the candidate list when DaisyClaw can
  ...
  <Accordion title="Timeouts">
    ...
    DaisyClaw's 600000 ms dynamic-tool bridge maximum.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="OpenAI gpt-image-2 (and gpt-image-1.5)">
    ...
    `openai` OAuth profile is configured, DaisyClaw reuses the same
    ...
    `https://chatgpt.com/backend-api/codex` for image requests. DaisyClaw
    ...
    DaisyClaw forwards `prompt`, `count`, `size`, `quality`, `outputFormat`,
    and reference images to OpenAI. OpenAI does **not** receive
    `aspectRatio` or `resolution` directly; when possible DaisyClaw maps
    ...
    transparency-capable OpenAI image model. DaisyClaw routes default
  ...
  <Accordion title="Microsoft Foundry MAI image models">
    ...
    onboarding/model metadata so DaisyClaw can verify that the deployment is
  ...
  <Accordion title="OpenRouter image models">
    ...
    DaisyClaw forwards `prompt`, `count`, reference images, and
  ...
  <Accordion title="fal Krea 2">
    ...
    `image_size` schema used by Flux. DaisyClaw sends:
    ...
    Krea; DaisyClaw maps `size` to the closest supported Krea aspect ratio and
  ...
  <Accordion title="xAI grok-imagine-image">
    ...
    - Resolutions: `1K`, `2K`
    - Outputs: returned as DaisyClaw-managed image attachments

    DaisyClaw intentionally does not expose xAI-native `quality`, `mask`,
...
```text
/tool image_generate action=generate model=openai/gpt-image-2 prompt="A clean editorial poster for DaisyClaw image generation" size=3840x2160 count=1
```
  </Tab>
...
`daisyclaw infer image edit`; `--openai-background` remains as an
