...
DaisyClaw supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), DaisyClaw reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.daisyclaw/.env`.

If you omit `baseUrl`, DaisyClaw defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, DaisyClaw defaults to `kimi-k2.6`.
...
DaisyClaw treats Kimi `web_search` as successful only after Moonshot returns
...
DaisyClaw returns a structured `kimi_web_search_ungrounded` error instead of
