...
read_when:
  - Looking for an overview of DaisyClaw's media capabilities
...
DaisyClaw generates images, videos, and music, understands inbound media
...
For async tools, DaisyClaw submits the request to the provider, returns a task
...
DaisyClaw wakes the agent with the generated media paths so it can tell the
...
DaisyClaw sends an idempotent direct fallback with only the missing media. Media
...
<AccordionGroup>
  ...
  <Accordion title="DeepInfra">
    ...
    registered until DaisyClaw has dedicated provider contracts for those
    categories.
  </Accordion>
  <Accordion title="xAI">
    ...
    not registered in DaisyClaw until the shared realtime-voice contract can
    represent it.
  </Accordion>
</AccordionGroup>
