...
For session-backed agent runs, DaisyClaw starts music generation as a
...
from the completion reply, DaisyClaw sends an idempotent direct fallback with
...
<Note>
Not all providers support all parameters. DaisyClaw still validates hard
limits such as input counts before submission. When a provider supports
duration but uses a shorter maximum than the requested value, DaisyClaw
...
Provider request timeouts are operator configuration only. DaisyClaw uses
...
- **Status lookup:** `daisyclaw tasks list` or `daisyclaw tasks show <taskId>`
  inspects queued, running, and terminal status.
- **Completion wake:** DaisyClaw injects an internal completion event back
...
```bash
DAISYCLAW_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts
```
