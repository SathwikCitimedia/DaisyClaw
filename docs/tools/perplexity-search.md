...
DaisyClaw supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, DaisyClaw also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `daisyclaw configure --section web`. It stores the key in
`~/.daisyclaw/daisyclaw.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.daisyclaw/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
