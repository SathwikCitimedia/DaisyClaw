---
summary: "Install, configure, and manage DaisyClaw plugins"
read_when:
...
Plugins extend DaisyClaw with channels, model providers, agent harnesses, tools,
...
- an DaisyClaw checkout or installation with the `daisyclaw` CLI available
...
<Steps>
  <Step title="Find the plugin">
    ...
    they match an official plugin id. Raw `@daisyclaw/*` package specs that match
    bundled plugins use the bundled copy from the current DaisyClaw build. Use an
  ...
  <Step title="Configure and enable it">
    ...
    ```bash
    daisyclaw plugins enable <plugin-id>
    ```
    ...
    `daisyclaw plugins install` adds the installed id to an existing
  ...
  <Step title="Let the Gateway reload">
    ...
    enabled, DaisyClaw detects the changed plugin install record and restarts the
...
| ----------- | ------------------------------------------------------------------------------ | -------------------------------------------------------------- |
| ClawHub     | You want DaisyClaw-native discovery, scans, version metadata, and install hints | `daisyclaw plugins install clawhub:<package>`                   |
| npm         | You need direct npm registry or dist-tag workflows                             | `daisyclaw plugins install npm:<package>`                       |
| git         | You need a branch, tag, or commit from a repository                            | `daisyclaw plugins install git:github.com/<owner>/<repo>@<ref>` |
...
a bundled plugin id, DaisyClaw uses that bundled source. If it matches an
official external plugin id, DaisyClaw uses the official package catalog. Other
ordinary bare package specs install through npm during the launch cutover. Raw
`@daisyclaw/*` package specs that match bundled plugins also resolve to the
bundled copy before npm fallback. Use `npm:@daisyclaw/<plugin>@<version>` when
...
deterministic source selection. See [`daisyclaw plugins`](/cli/plugins#install)
for the full command contract.

For npm installs, unpinned package specs and `@latest` choose the newest stable
package that advertises compatibility with this DaisyClaw build. If npm's
current latest release declares a newer `daisyclaw.compat.pluginApi` or
`daisyclaw.install.minHostVersion`, DaisyClaw scans older stable package versions
...
DaisyClaw's built-in plugin dependency denylist.
...
Run `daisyclaw doctor` or `daisyclaw doctor --fix` when config validation reports
stale plugin ids, allowlist/tool mismatches, or legacy bundled plugin paths.

## Understand plugin formats

DaisyClaw recognizes two plugin formats:

| Format                 | How it loads                                                                 | Use when                                                               |
| ---------------------- | ---------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| Native DaisyClaw plugin | `daisyclaw.plugin.json` plus a runtime module loaded in process               | You are installing or building DaisyClaw-specific runtime capabilities  |
| Compatible bundle      | Codex, Claude, or Cursor plugin layout mapped into DaisyClaw plugin inventory | You are reusing compatible skills, commands, hooks, or bundle metadata |

Both formats appear in `daisyclaw plugins list`, `daisyclaw plugins inspect`,
`daisyclaw plugins enable`, and `daisyclaw plugins disable`. See
...
Plugin-managed internal hooks show up in `daisyclaw hooks list` with
`plugin:<id>`. You cannot enable or disable them through `daisyclaw hooks`;
enable or disable the plugin instead.

## Verify the active Gateway

`daisyclaw plugins list` and plain `daisyclaw plugins inspect` read cold config,
...
```bash
daisyclaw gateway status --deep --require-rpc
daisyclaw plugins inspect <plugin-id> --runtime --json
daisyclaw gateway restart
```
...
sure any manual restart targets the actual `daisyclaw gateway run` child that
...
| Symptom                                                        | Check                                                                                                                                      | Fix                                                                                                     |
| -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------- |
| Plugin appears in `plugins list` but runtime hooks do not run  | Use `daisyclaw plugins inspect <id> --runtime --json` and confirm the active Gateway with `gateway status --deep --require-rpc`             | Restart the live Gateway after install, update, config, or source changes                               |
| Duplicate channel or tool ownership diagnostics appear         | Run `daisyclaw plugins list --enabled --verbose`, inspect each suspected plugin with `--runtime --json`, and compare channel/tool ownership | Disable one owner, remove stale installs, or use manifest `preferOver` for intentional replacement      |
...
| Config is invalid during install                               | Read the validation message and run `daisyclaw doctor --fix` when it points to stale plugin state                                           | Doctor can quarantine invalid plugin config by disabling the entry and removing the invalid payload     |
| Plugin path is blocked for suspicious ownership or permissions | Inspect the diagnostic before the config error                                                                                             | Fix filesystem ownership/permissions, then run `daisyclaw plugins registry --refresh`                    |
| `DAISYCLAW_NIX_MODE=1` blocks lifecycle commands                | Confirm the install is managed by Nix                                                                                                      | Change plugin selection in the Nix source instead of using plugin mutator commands                      |
| Dependency import fails at runtime                             | Check whether the plugin was installed through npm/git/ClawHub or loaded from a local path                                                 | Run `daisyclaw plugins update <id>`, reinstall the source, or install local plugin dependencies yourself |
...
other channel. Run `daisyclaw doctor --fix` to remove stale plugin and channel
...
plugin id. If both plugins are explicitly enabled, DaisyClaw keeps that request
...
DaisyClaw needs at runtime. Update or reinstall after the publisher ships
...
and config validation follows with `plugin present but blocked`, DaisyClaw found
...
DaisyClaw as the same user that owns the state directory.

For Docker installs, the official image runs as `node` (uid `1000`), so the
host bind-mounted DaisyClaw config and workspace directories should normally be
owned by uid `1000`:
...
If you intentionally run DaisyClaw as root, repair the managed plugin root to
root ownership instead:

```bash
sudo chown -R root:root /path/to/daisyclaw-config/npm
```

After fixing ownership, rerun `daisyclaw doctor --fix` or
`daisyclaw plugins registry --refresh` so the persisted plugin registry matches
the repaired files.
...
```bash
daisyclaw config set logging.level trace
...
DaisyClaw caches successful plugin tool factory results for repeated resolutions
