...
Skill Workshop is DaisyClaw's governed path for creating and updating workspace
skills.
...
```bash
daisyclaw skills workshop propose-update trip-planning --proposal ./PROPOSAL.md
```
...
- `autonomous.enabled`: allows DaisyClaw to create pending proposals from durable
...
```text
<DAISYCLAW_STATE_DIR>/skill-workshop/
  proposals.json
...
Default state directory: `~/.daisyclaw`.
...
| Proposal does not show in list                 | Check the selected `--agent` workspace and `DAISYCLAW_STATE_DIR`.                                                                                                                                            |
...
- [Skills CLI](/cli/skills) for `daisyclaw skills` commands
