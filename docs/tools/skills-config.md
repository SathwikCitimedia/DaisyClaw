...
`~/.daisyclaw/daisyclaw.json`. Agent-specific visibility lives under
...
<ParamField path="skills.install.nodeManager" type='"npm" | "pnpm" | "yarn" | "bun"' default='"npm"'>
  ...
  for WhatsApp/Telegram). Use `daisyclaw setup --node-manager` for npm, pnpm,
...
runs after DaisyClaw has staged source material and before the install or update
...
```json5
{
  security: {
    installPolicy: {
      ...
      exec: {
        source: "exec",
        command: "/usr/local/bin/daisyclaw-install-policy",
        args: ["--json"],
        timeoutMs: 10000,
        noOutputTimeoutMs: 10000,
        maxOutputBytes: 1048576,
        passEnv: ["DAISYCLAW_STATE_DIR", "PATH"],
...
<ParamField path="security.installPolicy.exec.command" type="string">
  Absolute path to the trusted policy executable. DaisyClaw runs it without a
...
<ParamField path="security.installPolicy.exec.passEnv" type="string[]">
  Environment variable names copied from the DaisyClaw process into the policy
...
`daisyclawVersion`, `targetType`, `targetName`, `sourcePath`, `sourcePathKind`,
...
DaisyClaw does not execute install policy during normal Gateway startup. Installs
and updates fail closed when policy is enabled but unavailable. `daisyclaw doctor`
performs static validation, and `daisyclaw doctor --deep` executes a synthetic
...
{
  "protocolVersion": 1,
  "daisyclawVersion": "2026.6.1",
  "targetType": "skill",
  "targetName": "weather",
  "sourcePath": "/var/folders/.../daisyclaw-skill-clawhub/root",
  ...
  "origin": {
    "type": "clawhub",
    "registry": "https://clawhub.daisyclaw.ai",
    "slug": "weather",
    "version": "1.0.0"
  },
  "request": {
...
`metadata.daisyclaw.skillKey`, use that key instead. Quote hyphenated names
...
Managed `~/.daisyclaw/skills` and personal `~/.agents/skills` directories
