...
frontmatter and a markdown body. DaisyClaw loads bundled skills plus any local
...
DaisyClaw loads from these sources, **highest precedence first**. When the same
...
| 4           | Managed / local skills | `~/.daisyclaw/skills`                    |
| 5           | Bundled skills         | shipped with the install                |
| 6 — lowest  | Extra directories      | `skills.load.extraDirs` + plugin skills |

Skill roots support grouped layouts. DaisyClaw discovers a skill whenever
...
<Note>
  Codex CLI's native `$CODEX_HOME/skills` directory is **not** an DaisyClaw
  skill root. Use `daisyclaw migrate plan codex` to inventory those skills, then
  `daisyclaw migrate codex` to copy them into your DaisyClaw workspace.
</Note>
...
| Shared managed | `~/.daisyclaw/skills`         | All agents on this machine  |
...
`daisyclaw.plugin.json` (paths relative to the plugin root). Plugin skills load
...
skill overrides them. Gate them via `metadata.daisyclaw.requires.config` on the
...
```bash
daisyclaw skills workshop list
daisyclaw skills workshop inspect <proposal-id>
daisyclaw skills workshop apply <proposal-id>
```
...
`daisyclaw skills` commands for install and update, or the `clawhub` CLI for
publish and sync.

| Action                             | Command                                                |
| ---------------------------------- | ------------------------------------------------------ |
| Install a skill into the workspace | `daisyclaw skills install <slug>`                       |
| Install from a Git repository      | `daisyclaw skills install git:owner/repo@ref`           |
| Install a local skill directory    | `daisyclaw skills install ./path/to/skill --as my-tool` |
| Install for all local agents       | `daisyclaw skills install <slug> --global`              |
| Update all workspace skills        | `daisyclaw skills update --all`                         |
| Update a shared managed skill      | `daisyclaw skills update <slug> --global`               |
| Update all shared managed skills   | `daisyclaw skills update --all --global`                |
| Verify a skill's trust envelope    | `daisyclaw skills verify <slug>`                        |
| Print the generated Skill Card     | `daisyclaw skills verify <slug> --card`                 |
| Publish / sync via ClawHub CLI     | `clawhub sync --all`                                   |

<AccordionGroup>
  <Accordion title="Install details">
    `daisyclaw skills install` installs into the active workspace `skills/`
    directory by default. Add `--global` to install into the shared
    `~/.daisyclaw/skills` directory, visible to all local agents unless agent
    ...
    `daisyclaw skills update` tracks ClawHub installs only — reinstall Git or
    local sources to refresh them.

  </Accordion>
  <Accordion title="Verification and security scanning">
    `daisyclaw skills verify <slug>` asks ClawHub for the skill's
  ...
  <Accordion title="Private archive installs">
    ...
    `daisyclaw.json`. Normal ClawHub installs never need that setting.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Path containment">
    ...
    Managed `~/.daisyclaw/skills` and personal `~/.agents/skills` may contain
...
<Note>
  DaisyClaw follows the [AgentSkills](https://agentskills.io) spec. The
...
<ParamField path="disable-model-invocation" type="boolean" default="false">
  When `true`, DaisyClaw keeps the skill's instructions out of the agent's normal
...
DaisyClaw filters skills at load time using `metadata.daisyclaw` (single-line
JSON in the frontmatter). A skill with no `metadata.daisyclaw` block is always
...
<ParamField path="requires.config" type="string[]">
  Each `daisyclaw.json` path must be truthy.
</ParamField>
...
<Note>
  ...
  `metadata.daisyclaw` is absent, so older installed skills keep their
...
<AccordionGroup>
  <Accordion title="Installer selection rules">
    ...
    - If all installers are `download`, DaisyClaw lists each entry so you can
      see all available artifacts.
    - Specs can include `os: ["darwin"|"linux"|"win32"]` to filter by platform.
    - Node installs honor `skills.install.nodeManager` in `daisyclaw.json`
  ...
  <Accordion title="Per-installer details">
    - **Homebrew:** DaisyClaw does not auto-install Homebrew or translate brew
    ...
    - **Download:** `url` (required), `archive` (`tar.gz` | `tar.bz2` | `zip`),
      ...
      `targetDir` (default: `~/.daisyclaw/tools/<skillKey>`).
  </Accordion>
...
<ParamField path="apiKey" type='string | { source, provider, id }'>
  Convenience field for skills that declare `metadata.daisyclaw.primaryEnv`.
...
<Note>
  ...
  `metadata.daisyclaw.skillKey`, use that key under `skills.entries`. Quote
  hyphenated names: JSON5 allows quoted keys.
</Note>

## Environment injection

When an agent run starts, DaisyClaw:

<Steps>
  <Step title="Reads skill metadata">
    DaisyClaw resolves the effective skill list for the agent, applying gating
...
For the bundled `claude-cli` backend, DaisyClaw also materializes the same
...
DaisyClaw snapshots eligible skills **when a session starts** and reuses that
...
allowlist changes, DaisyClaw refreshes the snapshot to keep visible skills
aligned.

<AccordionGroup>
  <Accordion title="Skills watcher">
    By default, DaisyClaw watches skill folders and bumps the snapshot when
  ...
  <Accordion title="Remote macOS nodes (Linux gateway)">
    ...
    `system.run` allowed, DaisyClaw can treat macOS-only skills as eligible when
    ...
    answering bin probes, DaisyClaw clears its cached bin matches.
...
When skills are eligible, DaisyClaw injects a compact XML block into the system
