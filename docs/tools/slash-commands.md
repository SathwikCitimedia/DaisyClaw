...
the DaisyClaw command handler, and `/status` plus `/unfocus` stay local whenever
...
<ParamField path="commands.config" type="boolean" default="false">
  Enables `/config` (reads/writes `daisyclaw.json`). Owner-only.
</ParamField>

<ParamField path="commands.mcp" type="boolean" default="false">
  Enables `/mcp` (reads/writes DaisyClaw-managed MCP config under `mcp.servers`). Owner-only.
</ParamField>
...
<AccordionGroup>
  ...
  <Accordion title="Owner-only writes and admin">
    ...
    | --- | --- | --- |
    | `/config show\|get\|set\|unset` | `commands.config: true` | Read or write `daisyclaw.json`. Owner-only |
    | `/mcp show\|get\|set\|unset` | `commands.mcp: true` | Read or write DaisyClaw-managed MCP server config. Owner-only |
    ...
    | `/restart` | `commands.restart: true` (default) | Restart DaisyClaw |
...
/config set messages.responsePrefix="[daisyclaw]"
...
`/mcp` stores config in DaisyClaw config, not embedded-agent project settings.
...
<AccordionGroup>
  ...
  <Accordion title="Slack specifics">
    `channels.slack.slashCommand` supports a single `/daisyclaw`-style command.
...
- **Execution vs runtime:** `/status` reports `Execution` for the effective sandbox path and `Runtime` for who is running the session: `DaisyClaw Default`, `OpenAI Codex`, a CLI backend, or an ACP backend.
