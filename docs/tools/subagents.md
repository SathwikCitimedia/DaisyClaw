...
<AccordionGroup>
  <Accordion title="Non-blocking, push-based completion">
    ...
    - On completion, DaisyClaw best-effort closes tracked browser tabs/processes opened by that sub-agent session before the announce cleanup flow continues.

  </Accordion>
  <Accordion title="Completion delivery">
    - DaisyClaw hands completions back to the requester session through an `agent` turn with a stable idempotency key.
    - If the requester run is still active, DaisyClaw first tries to wake/steer that run instead of starting a second visible reply path.
    - If an active requester cannot be woken, DaisyClaw falls back to a requester-agent handoff with the same completion context instead of dropping the announce.
  ...
  <Accordion title="Modes and ACP runtime">
    ...
    - DaisyClaw hides `runtime: "acp"` until ACP is enabled, the requester is not sandboxed, and a backend plugin such as `acpx` is loaded. `runtime: "acp"` expects an external ACP harness id, or an `agents.list[]` entry with `runtime.type="acp"`; use the default sub-agent runtime for normal DaisyClaw config agents from `agents_list`.
...
- **Run timeout:** DaisyClaw uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise it falls back to `0` (no timeout). `sessions_spawn` does not accept per-call timeout overrides.
...
When active children exist, DaisyClaw injects a compact runtime-generated
...
<Steps>
  ...
  <Step title="Bind">
    DaisyClaw creates or binds a thread to that session target in the active channel.
  </Step>
...
model and embedded runtime metadata so callers can distinguish DaisyClaw, Codex
...
and `agents_list` omits it. Run `daisyclaw doctor --fix` to clean stale
...
- If a nested requester subagent session is gone, DaisyClaw falls back to that session's requester when available.
...
target agent first. After that, DaisyClaw applies the sub-agent restriction
layer.
...
DaisyClaw does not treat `endedAt` absence as permanent proof that a
...
rapid re-wedge window, DaisyClaw persists a recovery tombstone on that
session and stops auto-resuming it on later restarts. Run
`daisyclaw tasks maintenance --apply` to reconcile the task record, or
`daisyclaw doctor --fix` to clear stale aborted recovery flags on
tombstoned sessions.
