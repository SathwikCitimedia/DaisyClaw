...
title: "Tokenjuice"
read_when:
  - You want shorter `exec` or `bash` tool results in DaisyClaw
...
Today this applies to DaisyClaw embedded runs and DaisyClaw dynamic tools in the Codex
app-server harness. Tokenjuice hooks DaisyClaw's tool-result middleware and
...
```bash
daisyclaw plugins install clawhub:@daisyclaw/tokenjuice
```
...
```bash
daisyclaw config set plugins.entries.tokenjuice.enabled false
```

Or:

```bash
daisyclaw plugins disable tokenjuice
```
