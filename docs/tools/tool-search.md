---
summary: "Tool Search: compact large DaisyClaw tool catalogs behind search, describe, and call"
title: "Tool Search"
read_when:
  - You want DaisyClaw agents to use a large tool catalog without adding every tool schema to the prompt
  - You want DaisyClaw tools, MCP tools, and client tools exposed through one compact runtime surface
  - You are implementing or debugging tool discovery for DaisyClaw runs
---

Tool Search is an experimental DaisyClaw agent runtime feature. It gives agents one
...
This page documents DaisyClaw Tool Search. It is not the Codex-native tool
...
When enabled for DaisyClaw runs, the model receives one `tool_search_code` tool by default.
That tool runs a short JavaScript body in an isolated Node subprocess with an
`daisyclaw.tools` bridge:

```js
const hits = await daisyclaw.tools.search("create a GitHub issue");
const tool = await daisyclaw.tools.describe(hits[0].id);
return await daisyclaw.tools.call(tool.id, {
  title: "Crash on startup",
  body: "Steps to reproduce...",
});
```

The catalog can include DaisyClaw tools, plugin tools, MCP tools, and
...
needs the exact schema, and calls that tool through DaisyClaw.

Codex harness runs do not receive these experimental DaisyClaw Tool Search
controls. DaisyClaw passes product capabilities to Codex as dynamic tools, and
Codex owns the stable native code mode, native tool search, deferred dynamic
tools, and nested tool calls.

## How a turn runs

At planning time the DaisyClaw embedded runner builds the effective catalog for the
run:

1. Resolve the active tool policy for the agent, profile, sandbox, and session.
2. List eligible DaisyClaw and plugin tools.
...
6. Expose either the DaisyClaw code bridge or the structured fallback tools to the
   model.

At execution time every real tool call returns to DaisyClaw. The isolated Node
runtime does not hold plugin implementations, MCP client objects, or secrets.
`daisyclaw.tools.call(...)` crosses the bridge back into the Gateway, where the
...
Both modes are experimental. Prefer direct tool exposure for small DaisyClaw tool
...
catalog includes eligible DaisyClaw, MCP, and client tools after normal policy
filtering.
...
`daisyclaw.tools.search(query, options?)`

Searches the effective catalog for the current run. Results are compact and safe
to put back into prompt context.

```js
const hits = await daisyclaw.tools.search("calendar event", { limit: 5 });
```

`daisyclaw.tools.describe(id)`

Loads full metadata for one search result, including the exact input schema.

```js
const calendarCreate = await daisyclaw.tools.describe("mcp:calendar:create_event");
```

`daisyclaw.tools.call(id, args)`

Calls a selected tool through DaisyClaw.

```js
await daisyclaw.tools.call(calendarCreate.id, {
  summary: "Planning",
...
network grants, and no child-process or worker grants. DaisyClaw enforces a
...
- `daisyclaw.tools.search`
- `daisyclaw.tools.describe`
- `daisyclaw.tools.call`

Normal DaisyClaw behavior still applies to final calls:
...
Enable Tool Search for DaisyClaw runs with the default code bridge:

```bash
daisyclaw config set tools.toolSearch true
```
...
Use the structured fallback tools instead for DaisyClaw runs:
...
- final tool calls executed through DaisyClaw
...
- whether the result came from DaisyClaw, MCP, or a client tool

## E2E validation

The gateway E2E runner proves both paths with the DaisyClaw runtime:
