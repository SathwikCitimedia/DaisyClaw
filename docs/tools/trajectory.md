---
summary: "Export redacted trajectory bundles for debugging an DaisyClaw agent session"
read_when:
  - Debugging why an agent answered, failed, or called tools a certain way
  - Exporting a support bundle for an DaisyClaw session
...
Trajectory capture is DaisyClaw's per-session flight recorder. It records a
...
DaisyClaw writes the bundle under the workspace:

```text
.daisyclaw/trajectory-exports/daisyclaw-trajectory-<session>-<timestamp>/
```
...
The custom path is resolved inside `.daisyclaw/trajectory-exports/`. Absolute
...
create the bundle; do not use allow-all. In group chats, DaisyClaw sends the
...
Trajectory capture is on by default for DaisyClaw agent runs.
...
```json
{
  "traceSchema": "daisyclaw-trajectory",
  "schemaVersion": 1
}
```
...
| `metadata.json`       | DaisyClaw version, OS/runtime, model, config snapshot, plugins, skills, and prompt metadata     |
...
DaisyClaw also writes a best-effort pointer file beside the session:

```text
<session>.trajectory-path.json
```

Set `DAISYCLAW_TRAJECTORY_DIR` to store runtime trajectory sidecars in a
dedicated directory:

```bash
export DAISYCLAW_TRAJECTORY_DIR=/var/lib/daisyclaw/trajectories
```

When this variable is set, DaisyClaw writes one JSONL file per session id in that
directory.
...
Set `DAISYCLAW_TRAJECTORY=0` before starting DaisyClaw:

```bash
export DAISYCLAW_TRAJECTORY=0
```
...
DaisyClaw flushes runtime trajectory sidecars during agent cleanup. The default
cleanup timeout is 10,000 ms. On slow disks or large stores, set
`DAISYCLAW_TRAJECTORY_FLUSH_TIMEOUT_MS` before starting DaisyClaw:

```bash
export DAISYCLAW_TRAJECTORY_FLUSH_TIMEOUT_MS=30000
```

This controls when DaisyClaw logs an `daisyclaw-trajectory-flush` timeout and continues.
It does not change the trajectory size caps. To tune all agent cleanup steps
that do not pass an explicit timeout, set `DAISYCLAW_AGENT_CLEANUP_TIMEOUT_MS`.

## Privacy and limits

Trajectory bundles are designed for support and debugging, not public posting.
DaisyClaw redacts sensitive values before writing export files:
...
- confirm DaisyClaw was started without `DAISYCLAW_TRAJECTORY=0`
- check whether `DAISYCLAW_TRAJECTORY_DIR` points to a writable directory
...
- do not pass `/tmp/...` or `~/...`
- keep the export inside `.daisyclaw/trajectory-exports/`
