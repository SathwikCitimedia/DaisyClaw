...
DaisyClaw agents can generate videos from text prompts, reference images, or
...
DaisyClaw treats video generation as three runtime modes:
...
<Steps>
  ...
  <Step title="Pick a default model (optional)">
    ```bash
    daisyclaw config set agents.defaults.videoGenerationModel.primary "google/veo-3.1-fast-generate-preview"
    ```
  </Step>
...
1. DaisyClaw submits the request to the provider and immediately returns a task id.
...
3. When the video is ready, DaisyClaw wakes the same session with an internal completion event.
4. The agent tells the user through the session's normal visible-reply mode:
   ...
   completion reply, DaisyClaw sends an idempotent direct fallback with only the
   missing video.
...
generation. Use `daisyclaw tasks list` or `daisyclaw tasks show <taskId>` to
...
Generated video files are saved under DaisyClaw-managed media storage when
...
larger renders. When a provider also returns a hosted output URL, DaisyClaw
...
daisyclaw tasks show <taskId>
daisyclaw tasks cancel <taskId>
```
...
<ParamField path="aspectRatio" type="string">
  Aspect-ratio hint such as `1:1`, `16:9`, `9:16`, `adaptive`, or a provider-specific value. DaisyClaw normalizes or ignores unsupported values per provider.
</ParamField>
<ParamField path="resolution" type="string">Resolution hint such as `480P`, `720P`, `768P`, `1080P`, `4K`, or a provider-specific value. DaisyClaw normalizes or ignores unsupported values per provider.</ParamField>
...
<ParamField path="timeoutMs" type="number">Optional provider operation timeout in milliseconds. When omitted, DaisyClaw uses `agents.defaults.videoGenerationModel.timeoutMs` if configured, otherwise the plugin-authored provider default when one exists.</ParamField>
...
<Note>
Not all providers support all parameters. DaisyClaw normalizes duration to
...
DaisyClaw resolves the model in this order:
...
<AccordionGroup>
  ...
  <Accordion title="BytePlus Seedance 1.5">
    Requires the [`@daisyclaw/byteplus-modelark`](https://www.npmjs.com/package/@daisyclaw/byteplus-modelark)
  ...
  <Accordion title="fal">
    Uses a queue-backed flow for long-running jobs. DaisyClaw waits up to 20
  ...
  <Accordion title="OpenRouter">
    Uses OpenRouter's asynchronous `/videos` API. DaisyClaw submits the
...
```bash
DAISYCLAW_LIVE_TEST=1 pnpm test:live -- extensions/video-generation-providers.live.test.ts
```
...
- Per-provider operation cap from
  `DAISYCLAW_LIVE_VIDEO_GENERATION_TIMEOUT_MS` (`180000` by default).
...
Set `DAISYCLAW_LIVE_VIDEO_GENERATION_FULL_MODES=1` to also run declared
...
Set the default video-generation model in your DaisyClaw config:
...
```bash
daisyclaw config set agents.defaults.videoGenerationModel.primary "qwen/wan2.6-t2v"
```
