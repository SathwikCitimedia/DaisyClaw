...
DaisyClaw also includes `x_search` for X (formerly Twitter) posts and
...
<Steps>
  ...
  <Step title="Configure">
    ```bash
    daisyclaw configure --section web
    ```
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "DaisyClaw plugin SDK" });
    ```
...
Direct OpenAI Responses models use OpenAI's hosted `web_search` tool automatically when DaisyClaw web search is enabled and no managed provider is pinned. This is provider-owned behavior in the bundled OpenAI plugin and only applies to native OpenAI API traffic, not OpenAI-compatible proxy base URLs or Azure routes. Set `tools.web.search.provider` to another provider such as `brave` to keep the managed `web_search` tool for OpenAI models, or set `tools.web.search.enabled: false` to disable both managed search and native OpenAI search.

## Native Codex web search

Codex-capable models can optionally use the provider-native Responses `web_search` tool instead of DaisyClaw's managed `web_search` function.
...
If native Codex search is enabled but the current model is not Codex-capable, DaisyClaw keeps the normal managed `web_search` behavior.

## Network safety

Managed `web_search` provider calls use DaisyClaw's guarded fetch path. For
trusted provider API hosts, DaisyClaw allows Surge, Clash, and sing-box fake-IP
...
If no `provider` is set, DaisyClaw checks providers in this order and uses the
...
When no API-backed provider is configured, DaisyClaw defaults to **Parallel
...
<Note>
  ...
  selected through auto-detect. In auto-detect mode, DaisyClaw resolves only the
...
Grok can also reuse an xAI OAuth auth profile from `daisyclaw models auth login
...
DaisyClaw keeps startup resilient and reports a warning so you can reinstall the
plugin or run `daisyclaw doctor --fix` to clean up the stale config.

`web_fetch` fallback provider selection is separate:

- choose it with `tools.web.fetch.provider`
- or omit that field and let DaisyClaw auto-detect the first ready web-fetch
...
When you choose **Kimi** during `daisyclaw onboard` or
`daisyclaw configure --section web`, DaisyClaw can also ask for:
...
Legacy `tools.web.x_search.*` config is auto-migrated by `daisyclaw doctor --fix`.
When you choose Grok during `daisyclaw onboard` or `daisyclaw configure --section web`,
DaisyClaw can also offer optional `x_search` setup with the same credential.
This is a separate follow-up step inside the Grok path, not a separate top-level
web-search provider choice. If you pick another provider, DaisyClaw does not
...
<Tabs>
  <Tab title="Config file">
    Run `daisyclaw configure --section web` or set the key directly:
  ...
  <Tab title="Environment variable">
    ...
    For a gateway install, put it in `~/.daisyclaw/.env`.
...
optional structured filters. DaisyClaw only enables the built-in xAI `x_search`
