---
summary: "Run DaisyClaw on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for DaisyClaw
title: "Linux server"
sidebarTitle: "Linux Server"
---

Run the DaisyClaw Gateway on any Linux server or cloud VPS. This page helps you
...
Before you install DaisyClaw on a public VPS, decide how you want to administer
the box itself.
...
- This is separate from Gateway access. You can still keep DaisyClaw bound to
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/daisyclaw-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- Add service env for a stable startup path:
  ...
  - `NODE_COMPILE_CACHE=/var/tmp/daisyclaw-compile-cache`
...
For the standard `daisyclaw onboard --install-daemon` path, edit the user unit:
...
`daisyclaw-gateway.service` via `sudo systemctl edit daisyclaw-gateway.service`.
