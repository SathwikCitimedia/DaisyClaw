...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/daisyclaw`)
...
If the page fails to load, start the Gateway first: `daisyclaw gateway`.
...
If the browser retries pairing with changed auth details (role/scopes/public key), the previous pending request is superseded and a new `requestId` is created. Re-run `daisyclaw devices list` before approval.

If the browser is already paired and you change it from read access to write/admin access, this is treated as an approval upgrade, not a silent reconnect. DaisyClaw keeps the old approval active, blocks the broader reconnect, and asks you to approve the new scope set explicitly.

Once approved, the device is remembered and won't require re-approval unless you revoke it with `daisyclaw devices revoke --device <id> --role <role>`. See [Devices CLI](/cli/devices) for token rotation and revocation.

Paperclip agents that connect through the `daisyclaw_gateway` adapter use the same first-run approval flow. After the initial connection attempt, run `daisyclaw devices approve --latest` to preview the pending request, then rerun the printed `daisyclaw devices approve <requestId>` command to approve it. Pass explicit `--url` and `--token` values for a remote gateway. To keep approvals stable across restarts, configure a persistent `adapterConfig.devicePrivateKeyPem` in Paperclip instead of letting it generate a new ephemeral device identity each run.
...
The Control UI fetches its runtime settings from `/control-ui-config.json`, resolved relative to the gateway's Control UI base path (for example `/__daisyclaw__/control-ui-config.json` when the UI is served under `/__daisyclaw__/`). That endpoint is gated by the same gateway auth as the rest of the HTTP surface: unauthenticated browsers cannot fetch it, and a successful fetch requires either an already valid gateway token/password, Tailscale Serve identity, or a trusted-proxy identity.
...
<AccordionGroup>
  ...
  <Accordion title="Config">
    - View/edit `~/.daisyclaw/daisyclaw.json` (`config.get`, `config.set`).
  ...
  <Accordion title="Cron jobs panel notes">
    ...
    - Deprecated fallback: run `daisyclaw doctor --fix` to migrate stored legacy jobs with `notify: true` from `cron.webhook` to explicit per-job webhook or completion delivery.
...
The dedicated MCP page is an operator view for DaisyClaw-managed MCP servers under `mcp.servers`. It does not start MCP transports by itself; use it to inspect and edit saved config, then use `daisyclaw mcp doctor --probe` when you need live server proof.
...
7. Run `daisyclaw mcp status --verbose`, `daisyclaw mcp doctor --probe`, or `daisyclaw mcp reload` from a terminal when the edited process needs static diagnostics, live proof, or cached-runtime disposal.
...
<AccordionGroup>
  ...
  <Accordion title="Stop and abort">
    ...
    - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop daisyclaw`, `please stop`) to abort out-of-band.
...
| `push/vapid-keys.json` (under the DaisyClaw state dir) | Auto-generated VAPID keypair used to sign Web Push payloads.       |
...
- `DAISYCLAW_VAPID_PUBLIC_KEY`
- `DAISYCLAW_VAPID_PRIVATE_KEY`
- `DAISYCLAW_VAPID_SUBJECT` (defaults to `https://daisyclaw.ai`)
...
<Tabs>
  ...
  <Tab title="Bind to tailnet + token">
    ```bash
    daisyclaw gateway --bind tailnet --token "$(openssl rand -hex 32)"
    ```
...
If you open the dashboard over plain HTTP (`http://<lan-ip>` or `http://<tailscale-ip>`), the browser runs in a **non-secure context** and blocks WebCrypto. By default, DaisyClaw **blocks** Control UI connections without device identity.
...
- `GET /__daisyclaw__/assistant-media?meta=1&source=<path>` requires the normal Control UI operator auth. The browser sends the gateway token as a bearer header when checking availability.
...
```bash
DAISYCLAW_CONTROL_UI_BASE_PATH=/daisyclaw/ pnpm ui:build
```
...
If the browser loads a blank dashboard and DevTools shows no useful error, an extension or early content script may have prevented the JavaScript module app from evaluating. The static page includes a plain HTML recovery panel that appears when `<daisyclaw-app>` is not registered after startup.
