...
```bash
daisyclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- `daisyclaw chat` and `daisyclaw terminal` are aliases for `daisyclaw tui --local`.
- `--local` cannot be combined with `--url`, `--token`, or `--password`.
- Local mode uses the embedded agent runtime directly. Most local tools work, but Gateway-only features are unavailable.
- After a config file has authored settings, `daisyclaw` and `daisyclaw crestodian` also use this TUI shell, with Crestodian as the local setup and repair chat backend.
...
- To show the Gateway host for non-local URL-backed connections, opt in with:
  ...
  ```bash
  daisyclaw config set tui.footer.showRemoteHost true
  ```
...
- Turn delivery on:
  ...
  - or start with `daisyclaw tui --deliver`
...
- Local shell commands receive `DAISYCLAW_SHELL=tui-local` in their environment.
...
If `daisyclaw config validate` is already failing, start with `daisyclaw configure`
or `daisyclaw doctor --fix` first. `daisyclaw chat` does not bypass the invalid-
config guard.
...
4. Apply narrow changes with `daisyclaw config set` or `daisyclaw configure`, then rerun `!daisyclaw config validate`.
5. If Doctor recommends an automatic migration or repair, review it and run `!daisyclaw doctor --fix`.

Tips:

- Prefer `daisyclaw config set` or `daisyclaw configure` over hand-editing `daisyclaw.json`.
- `daisyclaw docs "<query>"` searches the live docs index from the same machine.
- `daisyclaw config validate --json` is useful when you want structured schema and SecretRef/resolvability errors.
...
- If your terminal uses a light background and auto-detection is wrong, set `DAISYCLAW_THEME=light` before launching `daisyclaw tui`.
- To force the original dark palette instead, set `DAISYCLAW_THEME=dark`.
...
- Check the Gateway logs: `daisyclaw logs --follow`.
- Confirm the agent can run: `daisyclaw status` and `daisyclaw models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).

## Connection troubleshooting

- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `daisyclaw agents list` and your routing config.
- Empty session picker: you might be in global scope or have no sessions yet.

## Related

- [Control UI](/web/control-ui) — web-based control interface
- [Config](/cli/config) — inspect, validate, and edit `daisyclaw.json`
