...
- Extension production code should import from `daisyclaw/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `daisyclaw.plugin.json` and the package
  `daisyclaw` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `daisyclaw/plugin-sdk/*`.
