...
The ACPX extension is a thin DaisyClaw wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `daisyclaw/acpx`, not in this extension.
...
Use this flow when DaisyClaw needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `daisyclaw/acpx` repo first.
2. In DaisyClaw, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild DaisyClaw and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for DaisyClaw ACP validation.
...
If a change feels like shared ACP runtime behavior instead of DaisyClaw-specific glue, move it to `daisyclaw/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
