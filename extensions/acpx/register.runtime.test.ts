...
vi.mock("daisyclaw/plugin-sdk/acp-runtime-backend", () => ({
...
const previousSkipRuntime = process.env.DAISYCLAW_SKIP_ACPX_RUNTIME;

function restoreEnv(): void {
  if (previousSkipRuntime === undefined) {
    delete process.env.DAISYCLAW_SKIP_ACPX_RUNTIME;
  } else {
    process.env.DAISYCLAW_SKIP_ACPX_RUNTIME = previousSkipRuntime;
  }
}

function createServiceContext() {
  return {
    workspaceDir: "/tmp/daisyclaw-acpx-register-test",
    stateDir: "/tmp/daisyclaw-acpx-register-test/state",
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
