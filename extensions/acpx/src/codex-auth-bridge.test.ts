...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-acpx-codex-auth-"));
  tempDirs.push(dir);
  return dir;
}
...
afterEach(async () => {
  ...
  restoreEnv("DAISYCLAW_AGENT_DIR");
...
describe("prepareAcpxCodexAuthConfig", () => {
  it("installs an isolated Codex ACP wrapper without synthesizing auth from canonical DaisyClaw OAuth", async () => {
  ...
  it("launches the locally installed Codex ACP bin with isolated CODEX_HOME", async () => {
    ...
    const { stdout } = await execFileAsync(
      ...
      [
        ...
        "--daisyclaw-acpx-lease-id",
        "lease-1",
        "--daisyclaw-gateway-instance-id",
        "gateway-1",
      ],
      {
        cwd: root,
