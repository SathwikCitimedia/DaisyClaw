...
import { readJsonFileWithFallback } from "daisyclaw/plugin-sdk/json-store";
import {
...
import {
  DAISYCLAW_ACPX_LEASE_ID_ARG,
  DAISYCLAW_ACPX_LEASE_ID_ENV,
  DAISYCLAW_GATEWAY_INSTANCE_ID_ARG,
...
const RUN_CONFIGURED_COMMAND_SENTINEL = "--daisyclaw-run-configured";
...
function readManifestDependencyVersion(packageName: string): string {
  ...
  if (typeof version !== "string" || version.trim() === "") {
    throw new Error(`Missing ${packageName} dependency version in @daisyclaw/acpx manifest`);
  }
  return version;
}
...
async function resolveInstalledCodexAcpBinPath(): Promise<string | undefined> {
  // Keep DaisyClaw's isolated CODEX_HOME wrapper, but launch the plugin-local
...
const daisyClawWrapperArgs = new Set([
  ${quoteCommandPart(DAISYCLAW_ACPX_LEASE_ID_ARG)},
  ${quoteCommandPart(DAISYCLAW_GATEWAY_INSTANCE_ID_ARG)},
]);

function readDaisyClawWrapperArg(args, name) {
...
function resolveStderrLogPath(args) {
  ...
  const leaseId =
    process.env[${JSON.stringify(DAISYCLAW_ACPX_LEASE_ID_ENV)}] ||
    readDaisyClawWrapperArg(args, ${quoteCommandPart(DAISYCLAW_ACPX_LEASE_ID_ARG)}) ||
    "pid-" + process.pid;
...
function stripDaisyClawWrapperArgs(args) {
  const stripped = [];
  for (let index = 0; index < args.length; index += 1) {
    const value = args[index];
    if (daisyClawWrapperArgs.has(value)) {
      index += 1;
      continue;
    }
    stripped.push(value);
...
const configuredArgs = stripDaisyClawWrapperArgs(rawConfiguredArgs);
...
if (!command) {
  console.error("[daisyclaw] missing configured ${params.displayName} ACP command");
  process.exit(1);
}
...
child.on("error", (error) => {
  console.error(\`[daisyclaw] failed to launch ${params.displayName} ACP wrapper: \${error.message}\`);
  process.exit(1);
});
...
function buildClaudeAcpWrapperScript(installedBinPath?: string): string {
  return buildAdapterWrapperScript({
    displayName: "Claude",
    // This package is patched in DaisyClaw; fallback must not float to an unpatched newer release.
