...
describe("embedded acpx plugin config", () => {
  it("resolves workspace stateDir and cwd by default", () => {
    const workspaceDir = path.resolve("/tmp/daisyclaw-acpx");
  ...
  it("keeps explicit timeoutSeconds config", () => {
    const resolved = resolveAcpxPluginConfig({
      ...
      workspaceDir: "/tmp/daisyclaw-acpx",
    });
  ...
  it("injects the built-in plugin-tools MCP server only when explicitly enabled", () => {
    ...
    const server = resolved.mcpServers["daisyclaw-plugin-tools"];
  ...
  it("injects the built-in DaisyClaw tools MCP server only when explicitly enabled", () => {
    const resolved = resolveAcpxPluginConfig({
      rawConfig: {
    ...
    const server = resolved.mcpServers["daisyclaw-tools"];
    expect(server).toEqual({
      command: process.execPath,
      args: expectedMcpServerArgs({
        sourceEntry: "src/mcp/daisyclaw-tools-serve.ts",
        distEntry: "dist/mcp/daisyclaw-tools-serve.js",
      }),
    });
  });
  ...
  it("keeps the runtime json schema in sync with the manifest config schema", () => {
    ...
    const manifest = JSON.parse(
      fs.readFileSync(path.join(pluginRoot, "daisyclaw.plugin.json"), "utf8"),
