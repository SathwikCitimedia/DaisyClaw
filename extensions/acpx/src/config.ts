...
import { formatPluginConfigIssue } from "daisyclaw/plugin-sdk/extension-shared";
...
const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "daisyclaw-plugin-tools";
const ACPX_DAISYCLAW_TOOLS_MCP_SERVER_NAME = "daisyclaw-tools";
const requireFromHere = createRequire(import.meta.url);

function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "daisyclaw.plugin.json")) &&
...
function resolveAcpxPluginRootFromDaisyClawLayout(moduleUrl: string): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (let i = 0; i < 5; i += 1) {
    const candidates = [
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  return (
    ...
    // Scan common DaisyClaw layouts before falling back to the nearest path guess.
    resolveAcpxPluginRootFromDaisyClawLayout(moduleUrl) ??
    resolvedRoot
  );
}
...
function resolveDaisyClawRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
function resolvePluginToolsMcpServerConfig(moduleUrl: string = import.meta.url): McpServerConfig {
  ...
  const daisyClawRoot = resolveDaisyClawRoot(pluginRoot);
  const distEntry = path.join(daisyClawRoot, "dist", "mcp", "plugin-tools-serve.js");
  ...
  const sourceEntry = path.join(daisyClawRoot, "src", "mcp", "plugin-tools-serve.ts");
  return {
    command: process.execPath,
...
function resolveDaisyClawToolsMcpServerConfig(moduleUrl: string = import.meta.url): McpServerConfig {
  const pluginRoot = resolveAcpxPluginRoot(moduleUrl);
  const daisyClawRoot = resolveDaisyClawRoot(pluginRoot);
  const distEntry = path.join(daisyClawRoot, "dist", "mcp", "daisyclaw-tools-serve.js");
  if (fs.existsSync(distEntry)) {
    return {
      command: process.execPath,
      args: [distEntry],
  ...
  const sourceEntry = path.join(daisyClawRoot, "src", "mcp", "daisyclaw-tools-serve.ts");
  return {
    command: process.execPath,
    args: ["--import", resolveTsxImportSpecifier(), sourceEntry],
  };
...
function resolveConfiguredMcpServers(params: {
  ...
  daisyClawToolsMcpBridge: boolean;
  moduleUrl?: string;
  ...
  if (params.daisyClawToolsMcpBridge && resolved[ACPX_DAISYCLAW_TOOLS_MCP_SERVER_NAME]) {
    throw new Error(
      `mcpServers.${ACPX_DAISYCLAW_TOOLS_MCP_SERVER_NAME} is reserved when daisyClawToolsMcpBridge=true`,
    );
  }
  ...
  if (params.daisyClawToolsMcpBridge) {
    resolved[ACPX_DAISYCLAW_TOOLS_MCP_SERVER_NAME] = resolveDaisyClawToolsMcpServerConfig(
      params.moduleUrl,
    );
  }
  return resolved;
...
/** Convert DaisyClaw MCP server config into ACPX runtime MCP server entries. */
...
export function resolveAcpxPluginConfig(params: {
  ...
  const daisyClawToolsMcpBridge = normalized.daisyClawToolsMcpBridge === true;
  const mcpServers = resolveConfiguredMcpServers({
    mcpServers: normalized.mcpServers,
    pluginToolsMcpBridge,
    daisyClawToolsMcpBridge,
