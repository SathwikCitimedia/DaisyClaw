 ...
 * DaisyClaw-owned wrapper trees after validating paths, packages, and lease ids.
 */
...
const DAISYCLAW_PLUGIN_DEPS_MARKER = "/plugin-runtime-deps/";
...
export type AcpxProcessCleanupResult = {
  ...
  skippedReason?: "missing-root" | "not-daisyclaw-owned" | "unverified-root";
};
...
function resolveDaisyClawInstallRoot(pluginRoot: string): string {
  if (
...
function resolveOwnedAcpPackageRootCandidates(packageName: string): string[] {
  ...
  const daisyClawRoot = resolveDaisyClawInstallRoot(pluginRoot);
  return [
    resolvePackageRoot(packageName),
    path.join(pluginRoot, "node_modules", packageName),
    path.join(daisyClawRoot, "node_modules", packageName),
...
/** Check whether a command references an DaisyClaw-generated ACPX wrapper path. */
export function isDaisyClawLeaseAwareAcpxProcessCommand(params: {
...
function liveCommandMatchesLeaseIdentity(params: {
  ...
  return (
    commandOptionEquals(parts, DAISYCLAW_ACPX_LEASE_ID_ARG, params.expectedLeaseId) &&
    commandOptionEquals(parts, DAISYCLAW_GATEWAY_INSTANCE_ID_ARG, params.expectedGatewayInstanceId)
  );
}

/** Check whether a command is owned by DaisyClaw ACPX runtime packages or wrappers. */
export function isDaisyClawOwnedAcpxProcessCommand(params: {
  command: string | undefined;
  wrapperRoot?: string;
}): boolean {
  const command = params.command?.trim();
  ...
  if (!normalized.includes(DAISYCLAW_PLUGIN_DEPS_MARKER)) {
    return false;
  }
...
/** Terminate one validated DaisyClaw-owned ACPX wrapper process tree. */
export async function cleanupDaisyClawOwnedAcpxProcessTree(params: {
  rootPid?: number;
  rootCommand?: string;
  expectedLeaseId?: string;
  expectedGatewayInstanceId?: string;
  ...
  // that this PID still belongs to an DaisyClaw-owned wrapper, fail closed to
...
/** Reap orphaned DaisyClaw-owned ACPX wrapper trees during runtime startup. */
export async function reapStaleDaisyClawOwnedAcpxOrphans(params: {
  wrapperRoot: string;
  deps?: AcpxProcessCleanupDeps;
