/**
 * DaisyClaw ACPX runtime adapter. It wraps the upstream acpx runtime with
 * DaisyClaw session metadata, lease tracking, model scoping, and cleanup policy.
 */
...
import { parseStrictPositiveInteger } from "daisyclaw/plugin-sdk/number-runtime";
import { redactSensitiveText } from "daisyclaw/plugin-sdk/security-runtime";
...
import {
  cleanupDaisyClawOwnedAcpxProcessTree,
  isDaisyClawLeaseAwareAcpxProcessCommand,
...
type DaisyClawAcpxRuntimeOptions = AcpRuntimeOptions & {
  daisyclawWrapperRoot?: string;
  daisyclawGatewayInstanceId?: string;
  daisyclawProcessLeaseStore?: AcpxProcessLeaseStore;
};
type AcpxRuntimeTestOptions = Record<string, unknown> & {
  daisyclawProcessCleanup?: AcpxProcessCleanupDeps;
};
type DaisyClawRuntimeTurnInput = Parameters<NonNullable<AcpRuntime["startTurn"]>>[0];
type DaisyClawRuntimeEnsureInput = Parameters<AcpRuntime["ensureSession"]>[0];
...
type DaisyClawLeaseSessionMetadata = {
  daisyclawLeaseId: string;
  daisyclawGatewayInstanceId: string;
};

function withDaisyClawManagedTurnTimeout<T extends object>(input: T): T & { timeoutMs: 0 } {
  // DaisyClaw owns ACP turn deadlines. acpx treats timeout after partial agent
...
function withDaisyClawLeaseSessionMetadata<T extends object>(
  record: T,
  metadata: DaisyClawLeaseSessionMetadata,
): T & DaisyClawLeaseSessionMetadata {
  return {
    ...record,
    daisyclawLeaseId: metadata.daisyclawLeaseId,
    daisyclawGatewayInstanceId: metadata.daisyclawGatewayInstanceId,
  };
}
...
function readDaisyClawLeaseIdFromRecord(record: unknown): string | undefined {
  if (typeof record !== "object" || record === null) {
    return undefined;
  }
  const { daisyclawLeaseId } = record as { daisyclawLeaseId?: unknown };
  return typeof daisyclawLeaseId === "string" ? daisyclawLeaseId.trim() || undefined : undefined;
}

function readDaisyClawGatewayInstanceIdFromRecord(record: unknown): string | undefined {
  if (typeof record !== "object" || record === null) {
    return undefined;
  }
  const { daisyclawGatewayInstanceId } = record as { daisyclawGatewayInstanceId?: unknown };
  return typeof daisyclawGatewayInstanceId === "string"
    ? daisyclawGatewayInstanceId.trim() || undefined
    : undefined;
}
...
function createResetAwareSessionStore(
  ...
  return {
    async load(sessionId: string): Promise<AcpLoadedSessionRecord> {
      ...
      return withDaisyClawLeaseSessionMetadata(record, {
        daisyclawLeaseId: lease.leaseId,
        daisyclawGatewayInstanceId: lease.gatewayInstanceId,
      });
    },
    async save(record: AcpSessionRecord): Promise<void> {
      ...
      if (
        ...
        recordToSave = withDaisyClawLeaseSessionMetadata(
          {
            ...record,
          ...
          {
            daisyclawLeaseId: launch.leaseId,
            daisyclawGatewayInstanceId: launch.gatewayInstanceId,
          },
        );
      }
...
const DAISYCLAW_BRIDGE_EXECUTABLE = "daisyclaw";
const DAISYCLAW_BRIDGE_SUBCOMMAND = "acp";
const CODEX_ACP_AGENT_ID = "codex";
const CODEX_ACP_DAISYCLAW_PREFIX = "openai/";
const CLAUDE_ACP_DAISYCLAW_PREFIX = "anthropic/";
...
function isDaisyClawBridgeCommand(command: string | undefined): boolean {
  if (!command) {
    return false;
  }
  const parts = unwrapEnvCommand(splitCommandParts(command.trim()));
  if (basename(parts[0] ?? "") === DAISYCLAW_BRIDGE_EXECUTABLE) {
    return parts[1] === DAISYCLAW_BRIDGE_SUBCOMMAND;
  }
  if (basename(parts[0] ?? "") !== "node") {
    return false;
  }
  const scriptName = basename(parts[1] ?? "");
  return /^daisyclaw(?:\.[cm]?js)?$/i.test(scriptName) && parts[2] === DAISYCLAW_BRIDGE_SUBCOMMAND;
}
...
function normalizeCodexAcpModelOverride(
  ...
  let value = raw;
  if (value.toLowerCase().startsWith(CODEX_ACP_DAISYCLAW_PREFIX)) {
    value = value.slice(CODEX_ACP_DAISYCLAW_PREFIX.length);
  }
...
function normalizeClaudeAcpModelOverride(rawModel: string | undefined): string | undefined {
  ...
  if (!raw.toLowerCase().startsWith(CLAUDE_ACP_DAISYCLAW_PREFIX)) {
    return raw;
  }
  return raw.slice(CLAUDE_ACP_DAISYCLAW_PREFIX.length).trim() || undefined;
}

function withAcpxSessionOptions(input: DaisyClawRuntimeEnsureInput): AcpxDelegateEnsureInput {
...
function shouldUseBridgeSafeDelegateForCommand(command: string | undefined): boolean {
  return isDaisyClawBridgeCommand(command);
}
...
/** DaisyClaw-managed ACP runtime implementation backed by the upstream acpx runtime. */
export class AcpxRuntime implements AcpRuntime {
  ...
  constructor(options: DaisyClawAcpxRuntimeOptions, testOptions?: AcpxRuntimeTestOptions) {
    const { daisyclawProcessCleanup, ...delegateTestOptions } = testOptions ?? {};
    this.processCleanupDeps = daisyclawProcessCleanup;
    this.wrapperRoot = options.daisyclawWrapperRoot;
    this.gatewayInstanceId = options.daisyclawGatewayInstanceId;
    this.processLeaseStore = options.daisyclawProcessLeaseStore;
  ...
  private async readCodexTurnFailureStderr(params: { handle: AcpRuntimeHandle }): Promise<string> {
    ...
    return readCodexWrapperStderrTail({
      wrapperRoot: this.wrapperRoot,
      leaseId: readDaisyClawLeaseIdFromRecord(record),
    });
  }
  ...
  private async cleanupProcessTreeForRecord(
    ...
    const leaseId = readDaisyClawLeaseIdFromRecord(record);
    ...
    if (lease && lease.gatewayInstanceId === this.gatewayInstanceId && lease.rootPid > 0) {
      ...
      const result = await cleanupDaisyClawOwnedAcpxProcessTree({
    ...
    const expectedGatewayInstanceId = readDaisyClawGatewayInstanceIdFromRecord(record);
    await cleanupDaisyClawOwnedAcpxProcessTree({
      rootPid,
      rootCommand,
  ...
  async *runTurn(input: Parameters<AcpRuntime["runTurn"]>[0]): AsyncIterable<AcpRuntimeEvent> {
    ...
    try {
      for await (const event of delegate.runTurn(withDaisyClawManagedTurnTimeout(input))) {
        if (
          event.type !== "error" ||
          !isCodexAcpCommand(command) ||
  ...
  startTurn(input: DaisyClawRuntimeTurnInput): AcpRuntimeTurn {
    ...
    const turnPromise = Promise.all([
      ...
      try {
        return {
          command,
          turn: delegate.startTurn(withDaisyClawManagedTurnTimeout(input)),
        };
      } catch (error) {
        if (!isCodexAcpCommand(command) || !isGenericInternalAcpError(error)) {
          throw error;
