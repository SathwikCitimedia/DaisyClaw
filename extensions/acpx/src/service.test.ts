...
const { cleanupDaisyClawOwnedAcpxProcessTreeMock } = vi.hoisted(() => ({
  cleanupDaisyClawOwnedAcpxProcessTreeMock: vi.fn(
    async (): Promise<{
...
const { reapStaleDaisyClawOwnedAcpxOrphansMock } = vi.hoisted(() => ({
  reapStaleDaisyClawOwnedAcpxOrphansMock: vi.fn(
    async (): Promise<{
      inspectedPids: number[];
      terminatedPids: number[];
      skippedReason?: string;
...
vi.mock("./process-reaper.js", () => ({
  cleanupDaisyClawOwnedAcpxProcessTree: cleanupDaisyClawOwnedAcpxProcessTreeMock,
  reapStaleDaisyClawOwnedAcpxOrphans: reapStaleDaisyClawOwnedAcpxOrphansMock,
}));

import { getAcpRuntimeBackend } from "../runtime-api.js";
import type { DaisyClawPluginServiceContext } from "../runtime-api.js";
...
const previousEnv = {
  DAISYCLAW_ACPX_RUNTIME_STARTUP_PROBE: process.env.DAISYCLAW_ACPX_RUNTIME_STARTUP_PROBE,
  DAISYCLAW_SKIP_ACPX_RUNTIME: process.env.DAISYCLAW_SKIP_ACPX_RUNTIME,
  DAISYCLAW_SKIP_ACPX_RUNTIME_PROBE: process.env.DAISYCLAW_SKIP_ACPX_RUNTIME_PROBE,
};
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-acpx-service-"));
  tempDirs.push(dir);
  return dir;
}
...
afterEach(async () => {
  ...
  cleanupDaisyClawOwnedAcpxProcessTreeMock.mockClear();
  reapStaleDaisyClawOwnedAcpxOrphansMock.mockClear();
  acpxRuntimeConstructorMock.mockClear();
  createAgentRegistryMock.mockClear();
  createFileSessionStoreMock.mockClear();
  restoreEnv("DAISYCLAW_ACPX_RUNTIME_STARTUP_PROBE");
  restoreEnv("DAISYCLAW_SKIP_ACPX_RUNTIME");
  restoreEnv("DAISYCLAW_SKIP_ACPX_RUNTIME_PROBE");
  for (const dir of tempDirs.splice(0)) {
    await fs.rm(dir, { recursive: true, force: true });
  }
});

function createServiceContext(workspaceDir: string): DaisyClawPluginServiceContext {
  return {
    workspaceDir,
    stateDir: path.join(workspaceDir, ".daisyclaw-plugin-state"),
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
...
function createOpenKeyedStore(ctx: DaisyClawPluginServiceContext) {
  const env = { ...process.env, DAISYCLAW_STATE_DIR: ctx.stateDir };
...
function createAcpxRuntimeService(
  ctx: DaisyClawPluginServiceContext,
...
function openGatewayInstanceStore(ctx: DaisyClawPluginServiceContext) {
...
function openProcessLeaseStore(ctx: DaisyClawPluginServiceContext) {
...
describe("createAcpxRuntimeService", () => {
  ...
  it("skips the startup probe and does not advertise backend health when explicitly disabled", async () => {
    process.env.DAISYCLAW_ACPX_RUNTIME_STARTUP_PROBE = "0";
    delete process.env.DAISYCLAW_SKIP_ACPX_RUNTIME_PROBE;
    const workspaceDir = await makeTempDir();
  ...
  it("waits for the embedded runtime startup probe before resolving by default", async () => {
    delete process.env.DAISYCLAW_ACPX_RUNTIME_STARTUP_PROBE;
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
  ...
  it("reaps stale ACPX process leases from the generated wrapper root at startup", async () => {
    ...
    cleanupDaisyClawOwnedAcpxProcessTreeMock.mockResolvedValueOnce({
    ...
    expect(cleanupDaisyClawOwnedAcpxProcessTreeMock).toHaveBeenCalledWith({
      rootPid: 101,
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith("reaped 2 stale DaisyClaw-owned ACPX processes");
  ...
  it("runs wrapper-root orphan cleanup before dropping pending ACPX leases", async () => {
    ...
    await openProcessLeaseStore(ctx).register(lease.leaseId, lease);
    reapStaleDaisyClawOwnedAcpxOrphansMock.mockResolvedValueOnce({
    ...
    expect(cleanupDaisyClawOwnedAcpxProcessTreeMock).not.toHaveBeenCalled();
    expect(reapStaleDaisyClawOwnedAcpxOrphansMock).toHaveBeenCalledWith({
      wrapperRoot,
      deps: processCleanupDeps,
    });
    expect(ctx.logger.info).toHaveBeenCalledWith("reaped 2 stale DaisyClaw-owned ACPX processes");
  ...
  it("runs the embedded runtime probe at startup when explicitly enabled and reports health", async () => {
    process.env.DAISYCLAW_ACPX_RUNTIME_STARTUP_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("lets the skip env override the opt-in embedded runtime startup probe without advertising health", async () => {
    process.env.DAISYCLAW_ACPX_RUNTIME_STARTUP_PROBE = "1";
    process.env.DAISYCLAW_SKIP_ACPX_RUNTIME_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("can skip the embedded runtime backend via env", async () => {
    process.env.DAISYCLAW_SKIP_ACPX_RUNTIME = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith(
      "skipping embedded acpx runtime backend (DAISYCLAW_SKIP_ACPX_RUNTIME=1)",
    );
  });
});
