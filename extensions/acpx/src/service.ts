...
import { finiteSecondsToTimerSafeMilliseconds } from "daisyclaw/plugin-sdk/number-runtime";
import type {
...
const ENABLE_STARTUP_PROBE_ENV = "DAISYCLAW_ACPX_RUNTIME_STARTUP_PROBE";
const SKIP_RUNTIME_PROBE_ENV = "DAISYCLAW_SKIP_ACPX_RUNTIME_PROBE";
  ...
  async function resolveRuntime(): Promise<AcpxRuntimeLike> {
    ...
    runtimePromise ??= loadRuntimeModule().then((module) => {
      runtime = new module.AcpxRuntime({
        ...
        daisyclawGatewayInstanceId: params.gatewayInstanceId,
        daisyclawProcessLeaseStore: params.processLeaseStore,
        daisyclawWrapperRoot: params.wrapperRoot,
...
function resolveAllowedAgentsProbeAgent(ctx: DaisyClawPluginServiceContext): string | undefined {
...
async function reapOpenAcpxProcessLeases(params: {
  ...
  for (const lease of leases) {
    if (lease.rootPid <= 0) {
      ...
      if (!result) {
        result = await reapStaleDaisyClawOwnedAcpxOrphans({
...
export function createAcpxRuntimeService(
  ...
  return {
    id: "acpx-runtime",
    async start(ctx: DaisyClawPluginServiceContext): Promise<void> {
      if (process.env.DAISYCLAW_SKIP_ACPX_RUNTIME === "1") {
        ctx.logger.info("skipping embedded acpx runtime backend (DAISYCLAW_SKIP_ACPX_RUNTIME=1)");
        return;
      }
      ...
      if (startupReap.terminatedPids.length > 0) {
        ctx.logger.info(
          `reaped ${startupReap.terminatedPids.length} stale DaisyClaw-owned ACPX process${startupReap.terminatedPids.length === 1 ? "" : "es"}`,
        );
      }
    ...
    async stop(_ctx: DaisyClawPluginServiceContext): Promise<void> {
      lifecycleRevision += 1;
