...
describe("active-memory plugin", () => {
  ...
  const api: any = {
    ...
    runtime: {
      agent: {
        runEmbeddedAgent,
        session: {
          resolveStorePath: vi.fn(() => "/tmp/daisyclaw-session-store.json"),
          ...
          patchSessionEntry: vi.fn(
            async (params: {
              ...
              await hoisted.updateSessionStore(
                "/tmp/daisyclaw-session-store.json",
  ...
  beforeEach(async () => {
    ...
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-active-memory-test-"));
    configFile = {
      plugins: {
        entries: {
          "active-memory": {
    ...
    plugin.register(api as unknown as DaisyClawPluginApi);
  });
  ...
  it("keeps subagent transcripts off disk by default by using a temp session file", async () => {
    ...
    expect(sessionFile).toMatch(/daisyclaw-active-memory-.*\/session\.jsonl$/);
