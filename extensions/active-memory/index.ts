...
import { closeActiveMemorySearchManager } from "daisyclaw/plugin-sdk/memory-host-search";
import {
  asDateTimestampMs,
...
} from "daisyclaw/plugin-sdk/plugin-config-runtime";
import { definePluginEntry, type DaisyClawPluginApi } from "daisyclaw/plugin-sdk/plugin-entry";
import { parseAgentSessionKey, parseThreadSessionSuffix } from "daisyclaw/plugin-sdk/routing";
import { isPathInside } from "daisyclaw/plugin-sdk/security-runtime";
import {
...
import { tempWorkspace, resolvePreferredDaisyClawTmpDir } from "daisyclaw/plugin-sdk/temp-path";
...
function resolveDefaultToolsAllow(cfg: DaisyClawConfig | undefined): string[] {
  return cfg?.plugins?.slots?.memory === "memory-lancedb"
    ? [...LANCEDB_ACTIVE_MEMORY_TOOLS_ALLOW]
    : [...DEFAULT_ACTIVE_MEMORY_TOOLS_ALLOW];
}

function resolveToolsAllow(params: { pluginToolsAllow: unknown; cfg?: DaisyClawConfig }): string[] {
  return (
...
function resolvePersistentTranscriptBaseDir(api: DaisyClawPluginApi, agentId: string): string {
  return path.join(
...
function openActiveMemoryToggleStore(api: DaisyClawPluginApi) {
...
function isActiveMemoryGloballyEnabled(cfg: DaisyClawConfig): boolean {
...
function resolveActiveMemoryCleanupConfig(api: DaisyClawPluginApi): DaisyClawConfig | undefined {
  try {
    return (
      (api.runtime.config?.current?.() as DaisyClawConfig | undefined) ??
      (api.config as DaisyClawConfig | undefined)
    );
  } catch {
    return api.config as DaisyClawConfig | undefined;
  }
}
...
async function runRecallSubagent(params: {
  ...
  const transientWorkspace = params.config.persistTranscripts
    ...
    : await tempWorkspace({
        ...
        prefix: "daisyclaw-active-memory-",
      });
...
export default definePluginEntry({
  ...
  register(api: DaisyClawPluginApi) {
    const readCurrentConfig = (): DaisyClawConfig | undefined => {
      try {
        return (
          (api.runtime.config?.current?.() as DaisyClawConfig | undefined) ??
          (api.config as DaisyClawConfig | undefined)
    ...
    api.registerCommand({
      ...
      handler: async (ctx) => {
        ...
        if (isGlobal) {
          const currentConfig = api.runtime.config.current() as DaisyClawConfig;
          if (action === "status") {
            return {
