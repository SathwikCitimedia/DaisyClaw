...
import { registerApiProvider, streamSimple } from "daisyclaw/plugin-sdk/llm";
import { resolvePluginConfigObject } from "daisyclaw/plugin-sdk/plugin-config-runtime";
import type {
  DaisyClawPluginApi,
...
import { streamWithPayloadPatch } from "daisyclaw/plugin-sdk/provider-stream-shared";
 ...
 * IDs that may not contain the model name). When DaisyClaw's `isAnthropicBedrockModel`
...
function needsCachePointInjection(modelId: string): boolean {
  ...
  // Check if DaisyClaw identifies this as an Anthropic model via the ARN heuristic.
 ...
 * same matcher DaisyClaw uses for direct model IDs.
 */
 ...
 * the DaisyClaw config region differs from the profile's home region.
 */
...
export function registerAmazonBedrockPlugin(api: DaisyClawPluginApi): void {
  ...
  function resolveCurrentPluginConfig(
    config: DaisyClawConfig | undefined,
