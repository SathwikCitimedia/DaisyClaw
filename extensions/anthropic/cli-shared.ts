 ...
 * thinking levels, and keeps DaisyClaw-managed CLI runs isolated from shell env.
 */
import type {
  CliBackendConfig,
...
// steer DaisyClaw-managed Claude CLI runs toward a different provider,
// endpoint, token source, plugin/config tree, or telemetry bootstrap mode.
/** Environment variables removed before launching DaisyClaw-managed Claude CLI runs. */
...
function isDaisyClawRequestedYolo(context?: CliBackendNormalizeConfigContext): boolean {
...
/** Resolve Claude permission mode from DaisyClaw exec security settings. */
export function resolveClaudePermissionMode(context?: CliBackendNormalizeConfigContext): {
  mode?: string;
  overrideExisting: boolean;
} {
  return isDaisyClawRequestedYolo(context)
...
/** Map DaisyClaw thinking levels to Claude CLI effort flags for a model id. */
