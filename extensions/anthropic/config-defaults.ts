 ...
 */
import type { DaisyClawConfig } from "daisyclaw/plugin-sdk/plugin-entry";
import {
  isRecord,
...
function usesClaudeCliModelSelection(config: DaisyClawConfig): boolean {
...
function usesSelectedClaudeCliAuthProfile(config: DaisyClawConfig): boolean {
  const profiles = config.auth?.profiles ?? {};
...
function collectClaudeCliRuntimeRefsFromConfig(config: DaisyClawConfig): string[] {
  const refs = new Set<string>(
...
/** Apply Anthropic and Claude CLI defaults to an DaisyClaw config object. */
