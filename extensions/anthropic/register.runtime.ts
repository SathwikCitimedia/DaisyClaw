 ...
 */
import { formatCliCommand, parseDurationMs } from "daisyclaw/plugin-sdk/cli-runtime";
import { resolveExpiresAtMsFromDurationMs } from "daisyclaw/plugin-sdk/number-runtime";
import type {
  DaisyClawPluginApi,
  ProviderAuthContext,
...
import {
  ...
  type DaisyClawConfig as ProviderAuthConfig,
...
} from "daisyclaw/plugin-sdk/provider-model-shared";
import { fetchClaudeUsage } from "daisyclaw/plugin-sdk/provider-usage";
...
const ANTHROPIC_SETUP_TOKEN_NOTE_LINES = [
  ...
  `If you want a direct API billing path instead, use ${formatCliCommand("daisyclaw models auth login --provider anthropic --method api-key --set-default")} or ${formatCliCommand("daisyclaw models auth login --provider anthropic --method cli --set-default")}.`,
] as const;
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("daisyclaw doctor --yes")}"`,
  ].join("\n");
}
...
export function registerAnthropicPlugin(api: DaisyClawPluginApi): void {
