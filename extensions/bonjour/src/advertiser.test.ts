...
function enableAdvertiserUnitMode(hostname = "test-host") {
  ...
  process.env.DAISYCLAW_MDNS_HOSTNAME = hostname;
}
...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    const started = await startAdvertiser({
      ...
      cliPath: "/opt/homebrew/bin/daisyclaw",
      minimal: false,
    });

    expect(createService).toHaveBeenCalledTimes(1);
    const [gatewayCall] = createService.mock.calls as Array<[Record<string, unknown>]>;
    expect(gatewayCall?.[0]?.type).toBe("daisyclaw-gw");
  ...
  it("honors truthy DAISYCLAW_DISABLE_BONJOUR values", async () => {
    enableAdvertiserUnitMode();
    process.env.DAISYCLAW_DISABLE_BONJOUR = "true";
  ...
  it("auto-disables Bonjour on Fly Machines without Docker sentinel files", async () => {
    ...
    process.env.FLY_APP_NAME = "daisyclaw-clawcks-test";
  ...
  it("honors explicit Bonjour opt-in inside detected containers", async () => {
    enableAdvertiserUnitMode();
    process.env.DAISYCLAW_DISABLE_BONJOUR = "0";
    vi.spyOn(fs, "existsSync").mockImplementation((filePath) => String(filePath) === "/.dockerenv");
  ...
  it("logs ciao handler classifications at the bonjour caller", async () => {
    ...
    expect(
      handler?.(
        new Error(
          "Can't probe for a service which is announced already. Received announcing for service DaisyClaw Gateway._daisyclaw._tcp.local.",
        ),
      ),
    ).toBe(true);
  ...
  it("suppresses ciao self-probe retry console noise while advertising", async () => {
    ...
    try {
      ...
      console.log(
        "[test._daisyclaw-gw._tcp.local.] failed probing with reason: Error: Can't probe for a service which is announced already. Received announcing for service test._daisyclaw-gw._tcp.local.. Trying again in 2 seconds!",
      );
  ...
  it("defers probing recovery while a name conflict is still settling", async () => {
    ...
    await vi.advanceTimersByTimeAsync(10_000);
    listenerMap.get("name-change")?.("test-host (DaisyClaw) (2)");

    await vi.advanceTimersByTimeAsync(15_000);

    expect(createService).toHaveBeenCalledTimes(1);
    expectWarnContaining('name conflict resolved; newName="test-host (DaisyClaw) (2)"');
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("Mac (DaisyClaw)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
  ...
  it("falls back to daisyclaw when system hostname is invalid for DNS", async () => {
    // Allow advertiser to run in unit tests.
    delete process.env.VITEST;
    process.env.NODE_ENV = "development";
    delete process.env.DAISYCLAW_MDNS_HOSTNAME;
    ...
    const [gatewayCall] = createService.mock.calls as Array<[ServiceCall]>;
    expect(gatewayCall?.[0]?.hostname).toBe("daisyclaw");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("daisyclaw.local");
  ...
  it("truncates reported Kubernetes service name at the DNS label byte limit", async () => {
    ...
    expectDnsLabelByteLength(`${reportedHostname} (DaisyClaw)`, 64);
  ...
  it("truncates multi-byte hostname within DNS label byte limit", async () => {
    // 21 CJK characters = 63 bytes in UTF-8, adding " (DaisyClaw)" pushes over
  ...
  it("uses system hostname when DAISYCLAW_MDNS_HOSTNAME is unset", async () => {
    // Allow advertiser to run in unit tests.
    delete process.env.VITEST;
    process.env.NODE_ENV = "development";
    delete process.env.DAISYCLAW_MDNS_HOSTNAME;
