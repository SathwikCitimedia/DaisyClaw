...
import type { PluginLogger } from "daisyclaw/plugin-sdk/plugin-entry";
import { isTruthyEnvValue } from "daisyclaw/plugin-sdk/runtime-env";
...
/** Input data used to publish DaisyClaw gateway Bonjour records. */
...
// See https://github.com/daisyclaw/daisyclaw/issues/72481
...
function readBonjourDisableOverride(): boolean | null {
  const raw = process.env.DAISYCLAW_DISABLE_BONJOUR;
...
function truncateToDnsLabel(name: string, fallback = "DaisyClaw"): string {
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? truncateToDnsLabel(trimmed) : "DaisyClaw";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(DaisyClaw\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw =
      process.env.DAISYCLAW_MDNS_HOSTNAME?.trim() || resolveSystemMdnsHostname() || "daisyclaw";
    ...
    const recreateAdvertiser = async (reason: string, optsLocal?: { stuckState?: boolean }) => {
      ...
      recreatePromise = (async () => {
        ...
        if (tooManyConsecutive || tooManyStuckStates || tooManyInWindow) {
          ...
          logger.warn(
            `bonjour: disabling advertiser after ${detail} (${reason}); set discovery.mdns.mode="off" or DAISYCLAW_DISABLE_BONJOUR=1 to disable mDNS discovery`,
          );
