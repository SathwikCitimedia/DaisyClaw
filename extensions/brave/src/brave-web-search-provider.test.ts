...
import { validateJsonSchemaValue } from "daisyclaw/plugin-sdk/json-schema-runtime";
...
const braveManifest = JSON.parse(
  fs.readFileSync(new URL("../daisyclaw.plugin.json", import.meta.url), "utf-8"),
) as {
...
describe("brave web search provider", () => {
  ...
  it("points provider metadata at the canonical Brave docs page", () => {
    expect(createBraveWebSearchProvider().docsUrl).toBe(
      "https://docs.daisyclaw.ai/tools/brave-search",
    );
  ...
  it("points missing-key users to fetch/browser alternatives", async () => {
    ...
    const result = await tool.execute({ query: "DaisyClaw docs" });

    expect(result).toEqual({
      error: "missing_brave_api_key",
      message:
        "web_search (brave) needs a Brave Search API key. Run `daisyclaw configure --section web` to store it, or set BRAVE_API_KEY in the Gateway environment. If you do not want to configure a search API key, use web_fetch for a specific URL or the browser tool for interactive pages.",
