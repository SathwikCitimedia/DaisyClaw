...
import type { DaisyClawPluginApi } from "./runtime-api.js";
import setupPlugin from "./setup-api.js";

type BrowserAutoEnableProbe = Parameters<DaisyClawPluginApi["registerAutoEnableProbe"]>[0];
...
describe("browser plugin", () => {
  ...
  it("bundles the browser automation skill with the plugin", () => {
    const manifest = JSON.parse(
      fs.readFileSync(path.join(__dirname, "daisyclaw.plugin.json"), "utf8"),
    ) as { skills?: string[] };
  ...
  it("registers a lazy browser control service", async () => {
    ...
    await service.start({ config: {}, stateDir: "/tmp/daisyclaw", logger: { warn: vi.fn() } });
    expect(runtimeApiMocks.createBrowserPluginService).not.toHaveBeenCalled();

    await service.stop({ config: {}, stateDir: "/tmp/daisyclaw", logger: { warn: vi.fn() } });
  ...
  it("eager-loads the browser control service when explicitly requested", async () => {
    vi.stubEnv("DAISYCLAW_EAGER_BROWSER_CONTROL_SERVER", "1");
    const { api, registerService } = createApi();
    registerBrowserPlugin(api);
  ...
  for (const value of ["false", "", "disabled"]) {
    it(`keeps browser control service env value ${JSON.stringify(value)} lazy`, async () => {
      vi.stubEnv("DAISYCLAW_EAGER_BROWSER_CONTROL_SERVER", value);
      const { api, registerService } = createApi();
      registerBrowserPlugin(api);
  ...
  it("declares setup auto-enable reasons for browser config surfaces", () => {
    ...
    expect(probe({ config: { browser: { defaultProfile: "daisyclaw" } }, env: {} })).toBe(
      "browser configured",
    );
    ...
    expect(
      probe({ config: { browser: { defaultProfile: "daisyclaw", enabled: false } }, env: {} }),
    ).toBeNull();
  });
});
