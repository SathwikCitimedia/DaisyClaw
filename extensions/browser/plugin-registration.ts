...
const EAGER_BROWSER_CONTROL_SERVICE_ENV = "DAISYCLAW_EAGER_BROWSER_CONTROL_SERVER";
...
const BROWSER_CLI_DESCRIPTOR = {
  name: "browser",
  description: "Manage DaisyClaw's dedicated browser (Chrome/Chromium)",
  hasSubcommands: true,
};
...
function createLazyBrowserTool(opts?: {
  ...
  return {
    ...
    name: "browser",
    description: [
      "Control the browser via DaisyClaw's browser control server (status/start/stop/profiles/tabs/open/snapshot/screenshot/actions).",
      "Browser choice: omit profile by default for the isolated DaisyClaw-managed browser (`daisyclaw`).",
...
function createBrowserToolOptions(ctx: DaisyClawPluginToolContext): {
  sandboxBridgeUrl?: string;
  allowHostControl?: boolean;
  agentSessionKey?: string;
  agentDir?: string;
...
export const browserPluginNodeHostCommands: DaisyClawPluginNodeHostCommand[] = [
  {
...
export const browserSecurityAuditCollectors: DaisyClawPluginSecurityAuditCollector[] = [
  async (ctx) => {
...
function createLazyBrowserPluginService(): DaisyClawPluginService {
  let service: DaisyClawPluginService | null = null;
...
export function registerBrowserPlugin(api: DaisyClawPluginApi) {
  api.registerTool(((ctx: DaisyClawPluginToolContext) =>
    createLazyBrowserTool(createBrowserToolOptions(ctx))) as DaisyClawPluginToolFactory);
  api.registerCli(
    async ({ program }) => {
