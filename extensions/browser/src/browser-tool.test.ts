...
const toolCommonMocks = vi.hoisted(() => ({
  ...
  saveMediaBuffer: vi.fn(async () => ({ path: "/tmp/daisyclaw-media/resized.jpg" })),
}));
...
vi.mock("./browser-tool.runtime.js", () => {
  ...
  return {
    ...
    DEFAULT_UPLOAD_DIR: "/tmp/daisyclaw-browser-uploads",
    BrowserToolSchema: {},
    ...browserActionsMocks,
    ...browserClientMocks,
    ...browserConfigMocks,
...
function resetBrowserToolMocks() {
  ...
  toolCommonMocks.saveMediaBuffer.mockResolvedValue({ path: "/tmp/daisyclaw-media/resized.jpg" });
...
describe("browser tool upload inbound media fallback (#83544)", () => {
  ...
  it("resolves upload paths before arming the file chooser", async () => {
    const inboundPath = "/home/user/.daisyclaw/media/inbound/report.pdf";
