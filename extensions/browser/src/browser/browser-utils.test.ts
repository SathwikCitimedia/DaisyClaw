...
describe("browser server-context listKnownProfileNames", () => {
  it("includes configured and runtime-only profile names", () => {
    ...
    if (!daisyclaw) {
      throw new Error("expected daisyclaw profile");
    }
    ...
    const state: BrowserServerState = {
      ...
      profiles: new Map([
        [
          "stale-removed",
          {
            profile: { ...daisyclaw, name: "stale-removed" },
            running: null,
          },
        ],
      ]),
    ...
    expect(listKnownProfileNames(state).toSorted()).toEqual(["daisyclaw", "stale-removed", "user"]);
  });
});
