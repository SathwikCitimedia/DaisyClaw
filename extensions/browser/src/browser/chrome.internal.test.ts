...
vi.mock("daisyclaw/plugin-sdk/ssrf-runtime-internal", () => ({
...
vi.mock("../infra/tmp-daisyclaw-dir.js", () => ({
  resolvePreferredDaisyClawTmpDir: () => "/tmp/daisyclaw-browser-test",
}));
...
import {
  buildDaisyClawChromeLaunchArgs,
  getChromeWebSocketUrl,
  isChromeCdpReady,
  isChromeReachable,
  launchDaisyClawChrome,
  resolveDaisyClawUserDataDir,
  stopDaisyClawChrome,
} from "./chrome.js";
 ...
 * not exercise: launchDaisyClawChrome (with child_process.spawn mocked),
 * canRunCdpHealthCommand all branches, canOpenWebSocket failure,
 * stopDaisyClawChrome SIGKILL fallback, fs.exists() catch, default
 * profile name, buildDaisyClawChromeLaunchArgs branches, and friends.
 */
...
describe("chrome.ts internal", () => {
  ...
  describe("resolveDaisyClawUserDataDir", () => {
    it("falls back to the default profile name when none is supplied", () => {
      const dir = resolveDaisyClawUserDataDir();
      expect(dir.endsWith(path.join("daisyclaw", "user-data"))).toBe(true);
    });

    it("respects an explicit profile name", () => {
      const dir = resolveDaisyClawUserDataDir("my-profile");
      expect(dir.endsWith(path.join("my-profile", "user-data"))).toBe(true);
    });
  });

  describe("buildDaisyClawChromeLaunchArgs branches", () => {
    ...
    it("toggles headless args", () => {
      const args = buildDaisyClawChromeLaunchArgs({
    ...
    it("lets a request headless override beat env and profile headed settings", () => {
      const args = buildDaisyClawChromeLaunchArgs({
        ...
        env: { DAISYCLAW_BROWSER_HEADLESS: "0" },
      });
      expect(args).toContain("--headless=new");
      expect(args).toContain("--disable-gpu");
    });
  ...
  describe("fs.exists() catch branch", () => {
    it("treats a throwing fs.existsSync (for prefs files) as non-existent to force bootstrap", async () => {
      ...
      // exists() invocation inside launchDaisyClawChrome.
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchDaisyClawChrome(resolved, profile);
  ...
  describe("launchDaisyClawChrome", () => {
    let tmpDir = "";

    beforeEach(async () => {
      tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), "daisyclaw-launch-"));
    });
    ...
    it("rejects a remote profile before attempting to spawn", async () => {
      ...
      await expect(launchDaisyClawChrome(makeResolved(), profile)).rejects.toThrow(
    ...
    it("completes successfully when Chrome reports /json/version and CDP is reachable", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchDaisyClawChrome(makeResolved(), profile);
          expect(running.pid).toBe(4242);
    ...
    it("accepts a ready CDP diagnostic after the launch HTTP probe expires", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          const port = new URL(baseUrl).port;
          const profile = makeProfile(Number(port));
          const running = await launchDaisyClawChrome(
    ...
    it("clears stale singleton locks and retries once after profile-in-use launch failure", async () => {
      const configPath = path.join(tmpDir, "daisyclaw.json");
      await fsp.writeFile(
        configPath,
        JSON.stringify({
          logging: {
      ...
      vi.stubEnv("DAISYCLAW_CONFIG_PATH", configPath);
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const userDataDir = resolveDaisyClawUserDataDir(profile.name);
    ...
    it("throws with stderr hint + sandbox hint when CDP never becomes reachable", async () => {
      ...
      try {
        ...
        await expect(launchDaisyClawChrome(resolved, profile)).rejects.toThrow(
  ...
  describe("stopDaisyClawChrome SIGKILL fallback", () => {
    it("escalates to SIGKILL when CDP keeps reporting reachable past the deadline", async () => {
      ...
      await stopDaisyClawChrome(
        { proc, cdpPort: 12345 } as unknown as Parameters<typeof stopDaisyClawChrome>[0],
        1,
      );
  ...
  describe("launchDaisyClawChrome remaining branches", () => {
    it("skips decoration entirely when the profile is already decorated", async () => {
      ...
      const stageDir = await fsp.mkdtemp(path.join(os.tmpdir(), "daisyclaw-decorated-"));
      try {
        const profileName = path.basename(stageDir);
        const colorHex = "#FF4500";
        const colorInt = ((0xff << 24) | 0xff4500) >> 0;
        const userDataDir = path.join(resolveDaisyClawUserDataDir(profileName));
        ...
        const staged = resolveDaisyClawUserDataDir(path.basename(stageDir));
    ...
    it("falls back to the default color when profile.color is undefined", async () => {
      // Covers the `profile.color ?? DEFAULT_DAISYCLAW_BROWSER_COLOR` coalescing.
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
    ...
    it("buffers stderr chunks when Chrome emits diagnostics while CDP comes up", async () => {
      ...
      const configDir = await fsp.mkdtemp(path.join(os.tmpdir(), "daisyclaw-redact-off-"));
      const configPath = path.join(configDir, "daisyclaw.json");
      ...
      let message = "";
      try {
        await launchDaisyClawChrome(resolved, profile);
      } catch (err) {
    ...
    it("logs a warning when decorateDaisyClawProfile throws and still returns a running Chrome", async () => {
      // Covers the decoration catch branch (log.warn).
      const { decorateDaisyClawProfile } = await import("./chrome.profile-decoration.js");
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
      ...
      const decorationSpy = vi
        .spyOn({ decorateDaisyClawProfile }, "decorateDaisyClawProfile")
        .mockImplementation(() => {
      ...
      const writeSpy = vi.spyOn(fs, "writeFileSync").mockImplementation((p) => {
        const s = String(p);
        if (s.endsWith(".daisyclaw-profile-decorated") || s.endsWith("Preferences")) {
  ...
  describe("launchDaisyClawChrome managed-proxy CDP bypass", () => {
    const makeLoopbackProfile = (cdpPort: number): ResolvedBrowserProfile =>
      ({
        name: "daisyclaw-bypass",
        color: "#FF4500",
        cdpPort,
        cdpUrl: `http://127.0.0.1:${cdpPort}`,
        cdpIsLoopback: true,
    ...
    it("surfaces loopbackMode=block as BrowserProfileUnavailableError without spawning Chrome", async () => {
      ...
      await expect(launchDaisyClawChrome(makeResolved(), profile)).rejects.toBeInstanceOf(
    ...
    it("does not register a bypass for a remote attachOnly CDP URL (loopback gate)", async () => {
      stubExecutableAndPrefsExist();
      // For this test we want launchDaisyClawChrome to reject before any
      ...
      const remoteProfile = {
        name: "daisyclaw-remote",
        color: "#FF4500",
        cdpPort: 19222,
        cdpUrl: "http://browserless.example.com:19222",
        cdpIsLoopback: false,
      } as unknown as ResolvedBrowserProfile;
      await expect(launchDaisyClawChrome(makeResolved(), remoteProfile)).rejects.toThrow(
        /is remote; cannot launch local Chrome/,
      );
