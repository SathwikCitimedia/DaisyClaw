/**
 * DaisyClaw-managed Chrome profile decoration.
 *
...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".daisyclaw-profile-decorated");
}
 ...
 */
export function decorateDaisyClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string; downloadDir?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_DAISYCLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_DAISYCLAW_BROWSER_COLOR).toUpperCase();
