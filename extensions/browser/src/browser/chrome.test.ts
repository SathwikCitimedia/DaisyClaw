...
import {
  DEFAULT_DAISYCLAW_BROWSER_COLOR,
  DEFAULT_DAISYCLAW_BROWSER_PROFILE_NAME,
} from "./constants.js";
import { BrowserCdpEndpointBlockedError } from "./errors.js";
import { DEFAULT_DOWNLOAD_DIR } from "./paths.js";

type StopChromeTarget = Parameters<typeof stopDaisyClawChrome>[0];
...
describe("browser chrome profile decoration", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "daisyclaw-chrome-suite-"));
  });
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    ...
    decorateDaisyClawProfile(userDataDir, { color: DEFAULT_DAISYCLAW_BROWSER_COLOR });

    const expectedSignedArgb = ((0xff << 24) | 0xff4500) >> 0;

    const def = await readDefaultProfileFromLocalState(userDataDir);

    expect(def.name).toBe(DEFAULT_DAISYCLAW_BROWSER_PROFILE_NAME);
    expect(def.shortcut_name).toBe(DEFAULT_DAISYCLAW_BROWSER_PROFILE_NAME);
    ...
    const marker = await fsp.readFile(
      path.join(userDataDir, ".daisyclaw-profile-decorated"),
      "utf-8",
    );
  ...
  it("writes managed download prefs when a download dir is provided", async () => {
    const userDataDir = await createUserDataDir();
    decorateDaisyClawProfile(userDataDir, {
      color: DEFAULT_DAISYCLAW_BROWSER_COLOR,
  ...
  it("best-effort writes name when color is invalid", async () => {
    const userDataDir = await createUserDataDir();
    decorateDaisyClawProfile(userDataDir, { color: "lobster-orange" });
    const def = await readDefaultProfileFromLocalState(userDataDir);
  ...
  it("is idempotent when rerun on an existing profile", async () => {
    ...
    expect(profile.name).toBe(DEFAULT_DAISYCLAW_BROWSER_PROFILE_NAME);
  });
...
describe("browser chrome helpers", () => {
  ...
  it("finds Playwright-managed Linux Chromium", () => {
    const browserPath = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-ms-playwright-"));
  ...
  it("stopDaisyClawChrome no-ops when process is already killed", async () => {
    const proc = makeChromeTestProc({ killed: true });
    await stopChromeWithProc(proc, 10);
    expect(proc.kill).not.toHaveBeenCalled();
  });

  it("stopDaisyClawChrome sends SIGTERM and returns once CDP is down", async () => {
  ...
  it("stopDaisyClawChrome escalates to SIGKILL when CDP stays reachable", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
        ok: true,
  ...
  it("stopDaisyClawChrome releases the managed-proxy CDP bypass exactly once on a double stop", async () => {
    vi.stubGlobal("fetch", vi.fn().mockRejectedValue(new Error("down")));
    const proc = makeChromeTestProc();
    const release = vi.fn();
    const running = {
    ...
    await stopDaisyClawChrome(running, 10);
    await stopDaisyClawChrome(running, 10);
    expect(release).toHaveBeenCalledOnce();
  });

  it("stopDaisyClawChrome still releases the bypass when the SIGKILL fallback fires", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
        ok: true,
    ...
    await stopDaisyClawChrome(running, 1);
    expect(proc.kill).toHaveBeenNthCalledWith(1, "SIGTERM");
    expect(proc.kill).toHaveBeenNthCalledWith(2, "SIGKILL");
    expect(release).toHaveBeenCalledOnce();
  });

  it("stopDaisyClawChrome swallows a throw from the bypass release callback", async () => {
    vi.stubGlobal("fetch", vi.fn().mockRejectedValue(new Error("down")));
    const proc = makeChromeTestProc();
    const release = vi.fn(() => {
    ...
    await expect(stopDaisyClawChrome(running, 10)).resolves.toBeUndefined();
    expect(release).toHaveBeenCalledOnce();
  });
});
...
describe("browser chrome launch args", () => {
  it("does not force an about:blank tab at startup", () => {
    const args = buildDaisyClawChromeLaunchArgs({
      ...
      userDataDir: "/tmp/daisyclaw-test-user-data",
    });

    expect(args).not.toContain("about:blank");
    expect(args).toContain("--remote-debugging-port=18800");
    expect(args).toContain("--user-data-dir=/tmp/daisyclaw-test-user-data");
  });
});
