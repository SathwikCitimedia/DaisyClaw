/**
 * DaisyClaw-managed Chrome lifecycle and CDP helpers.
 *
...
import { prepareOomScoreAdjustedSpawn } from "daisyclaw/plugin-sdk/process-runtime";
...
function chromeLaunchHints(params: {
  ...
  if (CHROME_MISSING_DISPLAY_PATTERN.test(params.stderrOutput) && !headlessMode.headless) {
    hints.push(
      "No DISPLAY/X server was detected. Set DAISYCLAW_BROWSER_HEADLESS=1, remove the headed override, start Xvfb, or run the Gateway in a desktop session.",
    );
  }
  if (CHROME_SINGLETON_IN_USE_PATTERN.test(params.stderrOutput)) {
    hints.push(
      `The Chromium profile "${params.profile.name}" is locked. Stop the existing browser or remove stale Singleton* lock files under ~/.daisyclaw/browser/${params.profile.name}/user-data.`,
    );
  }
...
export type RunningChrome = {
   ...
   * through stopDaisyClawChrome without type churn.
   */
...
/** Resolve the user-data-dir path for a managed DaisyClaw Chrome profile. */
export function resolveDaisyClawUserDataDir(profileName = DEFAULT_DAISYCLAW_BROWSER_PROFILE_NAME) {
...
/** Build Chrome launch arguments for the managed DaisyClaw browser. */
export function buildDaisyClawChromeLaunchArgs(params: {
  resolved: ResolvedBrowserConfig;
  profile: ResolvedBrowserProfile;
  userDataDir: string;
...
/** Launch or attach to the managed DaisyClaw Chrome profile. */
export async function launchDaisyClawChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  launchOptions: ManagedBrowserHeadlessOptions = {},
  ...
  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_DAISYCLAW_BROWSER_COLOR).toUpperCase(),
  ...
  const spawnOnce = () => {
    ...
    if (process.platform === "linux") {
      const chromiumStateDir = path.join(resolvePreferredDaisyClawTmpDir(), ".chromium");
  ...
  if (needsDecorate) {
    try {
      ...
      log.info(`🦞 daisyclaw browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`daisyclaw browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`daisyclaw browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  const launchOnceAndWait = async (allowSingletonRecovery: boolean): Promise<RunningChrome> => {
    ...
    try {
      ...
      log.info(
        `🦞 daisyclaw browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
      );
...
export async function stopDaisyClawChrome(
