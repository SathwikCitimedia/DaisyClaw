 ...
 */
import { parseBrowserHttpUrl } from "daisyclaw/plugin-sdk/browser-config";
...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-daisyclaw-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-daisyclaw-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-daisyclaw-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function resolveBrowserFetchOperatorHint(
  ...
  if (opts?.ownership === "external-browser") {
    return (
      "The browser profile is external to DaisyClaw; make sure its browser/CDP endpoint " +
      "is running and reachable. Restarting the DaisyClaw gateway will not launch it."
    );
  }
  const isLocal = !isAbsoluteHttp(url);
  return isLocal
    ? `Restart the DaisyClaw gateway (DaisyClaw.app menubar, or \`${formatCliCommand("daisyclaw gateway")}\`).`
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  if (kind === "timeout") {
    return new Error(
      `Can't reach the DaisyClaw browser control service (timed out after ${timeoutMs}ms). ${operatorHint}`,
  ...
  return new Error(
    appendBrowserToolModelHint(
      `Can't reach the DaisyClaw browser control service. ${operatorHint} (${msg})`,
    ),
    err instanceof Error ? { cause: err } : undefined,
  );
}
