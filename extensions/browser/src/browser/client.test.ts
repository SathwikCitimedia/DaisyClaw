...
describe("browser client", () => {
  ...
  it("uses the expected endpoints + methods for common calls", async () => {
    ...
    expect(doctorResult.profile).toBe("daisyclaw");
    ...
    expect(deepDoctorResult.profile).toBe("daisyclaw");
    ...
    expect(urls.some((url) => url.endsWith("/doctor?profile=daisyclaw&deep=true"))).toBe(true);
    ...
    const deepDoctor = calls.find((c) => c.url.endsWith("/doctor?profile=daisyclaw&deep=true"));
