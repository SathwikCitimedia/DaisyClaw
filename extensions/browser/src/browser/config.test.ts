...
import {
  ...
  DAISYCLAW_BROWSER_HEADLESS_ENV,
...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("daisyclaw");
    expect(profile?.driver).toBe("daisyclaw");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const daisyclaw = resolveProfile(resolved, "daisyclaw");
    expect(daisyclaw?.driver).toBe("daisyclaw");
    expect(daisyclaw?.cdpPort).toBe(18800);
    expect(daisyclaw?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from DAISYCLAW_GATEWAY_PORT when unset", () => {
    withEnv({ DAISYCLAW_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      expect(resolved.controlPort).toBe(19003);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const daisyclaw = resolveProfile(resolved, "daisyclaw");
      expect(daisyclaw?.cdpPort).toBe(19012);
      expect(daisyclaw?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ DAISYCLAW_GATEWAY_PORT: undefined }, () => {
      const resolved = resolveBrowserConfig(undefined, { gateway: { port: 19011 } });
      expect(resolved.controlPort).toBe(19013);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const daisyclaw = resolveProfile(resolved, "daisyclaw");
      expect(daisyclaw?.cdpPort).toBe(19022);
      expect(daisyclaw?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(daisyclaw?.cdpPort).toBe(19000);
    expect(daisyclaw?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "daisyclaw");
  ...
  describe("managed browser headless mode", () => {
    const noDisplayEnv = {
      ...
      [DAISYCLAW_BROWSER_HEADLESS_ENV]: undefined,
    };
    ...
    it("lets explicit profile headless=false beat the Linux no-display fallback", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        profiles: {
          daisyclaw: { cdpPort: 18800, color: "#FF4500", headless: false },
        },
      });
      const profile = resolveProfile(resolved, "daisyclaw")!;
    ...
    it("lets DAISYCLAW_BROWSER_HEADLESS override profile/global config", () => {
      const resolved = resolveBrowserConfig({
        profiles: {
          daisyclaw: { cdpPort: 18800, color: "#FF4500", headless: false },
        },
      ...
      expect(
        resolveManagedBrowserHeadlessMode(resolved, profile, {
          platform: "linux",
          env: { ...noDisplayEnv, [DAISYCLAW_BROWSER_HEADLESS_ENV]: "1" },
        }),
    ...
    it("lets request-local headless override beat env and profile/global config", () => {
      ...
      expect(
        resolveManagedBrowserHeadlessMode(resolved, profile, {
          ...
          platform: "linux",
          env: { ...noDisplayEnv, [DAISYCLAW_BROWSER_HEADLESS_ENV]: "0" },
        }),
    ...
    it("returns an actionable error only when headed mode is explicitly selected", () => {
      ...
      const defaultProfile = resolveProfile(defaultResolved, "daisyclaw")!;
      expect(
      ...
      const profile = resolveProfile(profileResolved, "daisyclaw")!;
      expect(
        getManagedBrowserMissingDisplayError(profileResolved, profile, {
          platform: "linux",
          env: noDisplayEnv,
        }),
      ).toContain("browser.profiles.daisyclaw.headless=false");
    });
  });
  ...
  it("sets usesChromeMcp only for existing-session profiles", () => {
    ...
    const managed = resolveProfile(resolved, "daisyclaw")!;
  ...
  describe("default profile preference", () => {
    it("defaults to daisyclaw profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
      expect(resolved.defaultProfile).toBe("daisyclaw");
    });

    it("keeps daisyclaw default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("daisyclaw");
    ...
    it("keeps daisyclaw default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("daisyclaw");
    ...
    it("keeps daisyclaw default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
