...
import type { BrowserConfig, BrowserProfileConfig, DaisyClawConfig } from "../config/config.js";
...
export const DAISYCLAW_BROWSER_HEADLESS_ENV = "DAISYCLAW_BROWSER_HEADLESS";
...
function normalizeHexColor(raw: string | undefined): string {
  ...
  if (!value) {
    return DEFAULT_DAISYCLAW_BROWSER_COLOR;
  }
...
function isLocalManagedProfile(profile: ResolvedBrowserProfile): boolean {
  return profile.driver === "daisyclaw" && profile.cdpIsLoopback && !profile.attachOnly;
}
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_DAISYCLAW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_DAISYCLAW_BROWSER_PROFILE_NAME] = {
...
export function resolveBrowserConfig(
  ...
  const enabled = cfg?.enabled ?? DEFAULT_DAISYCLAW_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_DAISYCLAW_BROWSER_PROFILE_NAME]
        ? DEFAULT_DAISYCLAW_BROWSER_PROFILE_NAME
        : "user");
...
export function resolveProfile(
  ...
  let cdpUrl;
  const driver = profile.driver === "existing-session" ? "existing-session" : "daisyclaw";
...
export function resolveManagedBrowserHeadlessMode(
  ...
  const envHeadless = parseBooleanValue(env[DAISYCLAW_BROWSER_HEADLESS_ENV]);
...
export function getManagedBrowserMissingDisplayError(
  ...
  const sourceHint =
    mode.source === "request"
      ? "request override"
      : mode.source === "env"
        ? `${DAISYCLAW_BROWSER_HEADLESS_ENV}=0`
        : mode.source === "profile"
  ...
  return (
    ...
    `Set ${DAISYCLAW_BROWSER_HEADLESS_ENV}=1, remove the headed override, or launch under Xvfb.`
  );
}
