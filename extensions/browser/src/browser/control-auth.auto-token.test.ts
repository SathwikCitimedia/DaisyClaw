...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayAuth: vi.fn(
    ({
      ...
      authConfig?: NonNullable<NonNullable<DaisyClawConfig["gateway"]>["auth"]>;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: DaisyClawConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
...
function readPersistedConfig(): DaisyClawConfig {
...
async function expectUnresolvedBrowserSecretRefSkipsPersistence(cfg: DaisyClawConfig) {
