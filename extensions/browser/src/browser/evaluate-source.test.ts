...
describe("normalizeBrowserEvaluateFunctionSource", () => {
  ...
  it("wraps expressions as page functions", () => {
    expect(normalizeBrowserEvaluateFunctionSource("document.title")).toBe(
      [
        "() => {",
        "const __daisyclawEvaluateExpressionResult = (document.title);",
        'return typeof __daisyclawEvaluateExpressionResult === "function" ? __daisyclawEvaluateExpressionResult() : __daisyclawEvaluateExpressionResult;',
        "}",
      ].join("\n"),
    );
  });
  ...
  it("preserves function-valued expression invocation", () => {
    expect(normalizeBrowserEvaluateFunctionSource("extractTitle")).toBe(
      [
        "() => {",
        "const __daisyclawEvaluateExpressionResult = (extractTitle);",
        'return typeof __daisyclawEvaluateExpressionResult === "function" ? __daisyclawEvaluateExpressionResult() : __daisyclawEvaluateExpressionResult;',
        "}",
      ].join("\n"),
    );
    expect(normalizeBrowserEvaluateFunctionSource("extractText", { argumentName: "el" })).toBe(
      [
        "(el) => {",
        "const __daisyclawEvaluateExpressionResult = (extractText);",
        'return typeof __daisyclawEvaluateExpressionResult === "function" ? __daisyclawEvaluateExpressionResult(el) : __daisyclawEvaluateExpressionResult;',
        "}",
      ].join("\n"),
    );
  });
