 ...
 */
import { resolveNonNegativeIntegerOption } from "daisyclaw/plugin-sdk/number-runtime";
  ...
  try {
    if (boxes.length > 0) {
      await page.evaluate((labels) => {
        const existing = document.querySelectorAll("[data-daisyclaw-labels]");
        existing.forEach((el) => el.remove());

        const root = document.createElement("div");
        root.setAttribute("data-daisyclaw-labels", "1");
        ...
        for (const label of labels) {
          ...
          box.setAttribute("data-daisyclaw-labels", "1");
          ...
          tag.setAttribute("data-daisyclaw-labels", "1");
