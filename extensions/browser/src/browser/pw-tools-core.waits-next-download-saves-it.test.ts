...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredDaisyClawTmpDir: vi.fn(() => "/tmp/daisyclaw"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-daisyclaw-dir.js");
...
describe("pw-tools-core", () => {
  ...
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-daisyclaw-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredDaisyClawTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredDaisyClawTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredDaisyClawTmpDir.mockReturnValue("/tmp/daisyclaw");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-browser-download-test-"));
    try {
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredDaisyClawTmpDir.mockReturnValue("/tmp/daisyclaw-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "daisyclaw-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "daisyclaw-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredDaisyClawTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(outPath)).not.toBe(
      path.resolve(path.join(path.sep, "tmp", "daisyclaw-preferred", "downloads")),
    );
    expect(path.basename(outPath)).toContain(path.basename(res.path));
    expect(path.basename(outPath)).toMatch(/\.part$/);
    await expect(fs.readFile(res.path, "utf8")).resolves.toBe("download-content");
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "daisyclaw-preferred", "downloads")}${path.sep}`),
    );
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir(async (tempDir) => {
        ...
        tmpDirMocks.resolvePreferredDaisyClawTmpDir.mockReturnValue(tempDir);
