...
describe("server-context hot-reload profiles", () => {
  ...
  it("forProfile hot-reloads newly added profiles from config", () => {
    // Start with only daisyclaw profile
    ...
    // 2. Simulate adding a new profile to config (like user editing daisyclaw.json)
  ...
  it("forProfile refreshes existing profile config after getRuntimeConfig cache updates", () => {
    ...
    mockState.cfgProfiles.daisyclaw = { cdpPort: 19999, color: "#FF4500" };
    ...
    expect(state.resolved.profiles.daisyclaw?.cdpPort).toBe(19999);
  });
  ...
  it("marks existing runtime state for reconcile when profile invariants change", () => {
    const cfg = getRuntimeConfig();
    const resolved = resolveBrowserConfig(cfg.browser, cfg);
    const daisyclawProfile = requireValue(
      resolveProfile(resolved, "daisyclaw"),
      "daisyclaw profile missing",
    );
    ...
    const runtime = requireValue(state.profiles.get("daisyclaw"), "daisyclaw runtime missing");
  ...
  it("marks local managed runtime state for reconcile when profile headless changes", () => {
    ...
    expect(daisyclawProfile.headless).toBe(true);
    const state: BrowserServerState = {
      server: null,
      port: 18791,
      resolved,
    ...
    mockState.cfgProfiles.daisyclaw = {
      cdpPort: 18800,
      color: "#FF4500",
      headless: false,
    };
  ...
  it("marks local managed runtime state for reconcile when profile executablePath changes", () => {
    ...
    expect(daisyclawProfile.executablePath).toBe("/usr/bin/chrome-old");
    const state: BrowserServerState = {
      server: null,
      port: 18791,
      resolved,
  ...
  it("does not reconcile remote cdp runtime when only headless changes", () => {
    ...
    expect(remoteProfile.driver).toBe("daisyclaw");
    ...
    const runtime = requireValue(state.profiles.get("remote"), "remote runtime missing");
    expect(runtime.profile.driver).toBe("daisyclaw");
