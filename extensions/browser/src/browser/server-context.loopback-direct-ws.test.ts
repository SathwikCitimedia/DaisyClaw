...
describe("browser server-context loopback direct WebSocket profiles", () => {
  it("uses an HTTP /json/list base when opening about:blank under strict SSRF", async () => {
    ...
    state.resolved.profiles.daisyclaw = {
    ...
    const opened = await daisyclaw.openTab("about:blank");
  ...
  it("uses an HTTPS /json base for secure direct WebSocket profiles with a /cdp suffix", async () => {
    ...
    const tabs = await daisyclaw.listTabs();
    expect(tabs.map((tab) => tab.targetId)).toEqual(["T2"]);

    await daisyclaw.focusTab("T2");
    await daisyclaw.closeTab("T2");
  });
  ...
  it("blocks direct WebSocket tab operations when strict SSRF hostname allowlist rejects the cdpUrl", async () => {
    ...
    await expect(daisyclaw.listTabs()).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
    await expect(daisyclaw.focusTab("T1")).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
    await expect(daisyclaw.closeTab("T1")).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
