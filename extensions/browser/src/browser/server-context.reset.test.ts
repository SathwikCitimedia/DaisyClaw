...
function localDaisyClawProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "daisyclaw",
...
function createLocalDaisyClawResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localDaisyClawProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveDaisyClawUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-reset-"));
    const profileDir = path.join(tempRoot, "daisyclaw");
    ...
    const ops = createLocalDaisyClawResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveDaisyClawUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-reset-no-own-"));
    const profileDir = path.join(tempRoot, "daisyclaw");
    fs.mkdirSync(profileDir, { recursive: true });
