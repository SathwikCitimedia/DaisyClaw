...
async function openManagedTabWithRunningProfile(params: {
  ...
  const state = makeState("daisyclaw");
  seedRunningProfileState(state);
  const ctx = createTestBrowserRouteContext({ getState: () => state });
  const daisyclaw = ctx.forProfile("daisyclaw");
  return await daisyclaw.openTab(params.url ?? "http://127.0.0.1:3009");
}
...
describe("browser server-context tab selection state", () => {
  it("updates lastTargetId when openTab is created via CDP", async () => {
    ...
    const opened = await daisyclaw.openTab("http://127.0.0.1:8080");
    expect(opened.targetId).toBe("CREATED");
    expect(state.profiles.get("daisyclaw")?.lastTargetId).toBe("CREATED");
  ...
  it("can bootstrap a managed loopback tab under strict SSRF because CDP control stays local", async () => {
    ...
    const selected = await daisyclaw.ensureTabAvailable();
  ...
  it("opens a real tab when only browser-internal CDP targets are listed", async () => {
    ...
    expect(state.profiles.get("daisyclaw")?.lastTargetId).toBe("REAL");
    expect(createTargetViaCdp).toHaveBeenCalledWith({
      cdpUrl: "http://127.0.0.1:18800",
      url: "about:blank",
      ssrfPolicy: undefined,
  ...
  it("does not fail tab open when managed-tab cleanup list fails", async () => {
    ...
    const opened = await daisyclaw.openTab("http://127.0.0.1:3009");
    expect(opened.targetId).toBe("NEW");
  });
  ...
  it("blocks unsupported non-network URLs before any HTTP tab-open fallback", async () => {
    ...
    await expect(daisyclaw.openTab("file:///etc/passwd")).rejects.toBeInstanceOf(
  ...
  it("uses the loopback CDP control policy for /json/new fallback requests", async () => {
    ...
    const opened = await daisyclaw.openTab("https://example.com");
    expect(opened.targetId).toBe("NEW");
  ...
  it("assigns stable tab ids and prefers labels as suggested target ids", async () => {
    ...
    expect(await daisyclaw.listTabs()).toEqual([
    ...
    await expect(daisyclaw.labelTab("t1", "docs")).resolves.toEqual(
      expect.objectContaining({
        targetId: "DOCS_RAW",
        tabId: "t1",
        label: "docs",
  ...
  it("carries a stale alias to a single replacement target", async () => {
    ...
    expect((await daisyclaw.listTabs()).map((tab) => tab.tabId)).toEqual(["t1", "t2"]);
    expect(await daisyclaw.listTabs()).toEqual([
  ...
  it("carries stable aliases across confident raw target replacement", async () => {
    ...
    await expect(daisyclaw.labelTab("OLD_RAW", "checkout")).resolves.toEqual(
      expect.objectContaining({
        targetId: "OLD_RAW",
        tabId: "t1",
    ...
    const profileState = state.profiles.get("daisyclaw");
    if (!profileState) {
      throw new Error("expected profile state");
    }
    profileState.lastTargetId = "OLD_RAW";

    await expect(daisyclaw.listTabs()).resolves.toEqual([
      expect.objectContaining({
        targetId: "NEW_RAW",
        tabId: "t1",
        label: "checkout",
    ...
    expect(state.profiles.get("daisyclaw")?.lastTargetId).toBe("NEW_RAW");
  });
  ...
  it("resolves friendly tab references before backend focus and close calls", async () => {
    ...
    await daisyclaw.labelTab("DOCS_RAW", "docs");
    await expect(daisyclaw.ensureTabAvailable("t1")).resolves.toEqual(
      expect.objectContaining({ targetId: "DOCS_RAW" }),
    );
    await daisyclaw.focusTab("docs");
    await daisyclaw.closeTab("t1");
