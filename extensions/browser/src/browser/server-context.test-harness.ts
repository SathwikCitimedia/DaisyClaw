...
/** Mocks a launched DaisyClaw Chrome process with the supplied pid. */
export function mockLaunchedChrome(
  launchDaisyClawChrome: { mockResolvedValue: (value: RunningChrome) => unknown },
  pid: number,
) {
  const proc = new EventEmitter() as unknown as ChildProcessWithoutNullStreams;
  launchDaisyClawChrome.mockResolvedValue({
    pid,
    exe: { kind: "chromium", path: "/usr/bin/chromium" },
    userDataDir: "/tmp/daisyclaw-test",
    cdpPort: 18800,
    startedAt: Date.now(),
    proc,
  });
