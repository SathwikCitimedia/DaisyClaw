...
type HarnessState = {
  ...
  cfgProfiles: Record<
    string,
    {
      ...
      cdpUrl?: string;
      color: string;
      driver?: "daisyclaw" | "existing-session";
      attachOnly?: boolean;
    }
  >;
  tabUrl: string;
...
function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.DAISYCLAW_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "daisyclaw",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/daisyclaw" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    daisyclaw: { cdpPort: defaultBrowserCdpPortForState(testPort), color: "#FF4500" },
  };
}
...
vi.mock("../config/config.js", async () => {
  ...
  const mutateConfigFile = vi.fn(
    async (params: {
      ...
      const result = await params.mutate(draft, { snapshot: { path: "/tmp/daisyclaw.json" } });
      await writeConfigFile(draft);
      return {
        path: "/tmp/daisyclaw.json",
        previousHash: "test-hash",
        persistedHash: "test-hash",
        snapshot: { path: "/tmp/daisyclaw.json" },
        nextConfig: draft,
        result,
        attempts: 1,
...
vi.mock("./chrome.js", () => ({
  ...
  launchDaisyClawChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveDaisyClawUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopDaisyClawChrome: vi.fn(async () => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "daisyclaw";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.DAISYCLAW_GATEWAY_PORT;
  process.env.DAISYCLAW_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.DAISYCLAW_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.DAISYCLAW_GATEWAY_PASSWORD;
...
function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.DAISYCLAW_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.DAISYCLAW_GATEWAY_PASSWORD;
  } else {
    process.env.DAISYCLAW_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
