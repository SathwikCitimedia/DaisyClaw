...
vi.mock("../config/config.js", async () => {
  ...
  const loadConfig = () => ({
    browser: {
      ...
      profiles: {
        daisyclaw: { cdpPort: testPort + 1, color: "#FF4500" },
      },
    },
  });
  return {
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.DAISYCLAW_GATEWAY_PORT;
    process.env.DAISYCLAW_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.DAISYCLAW_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.DAISYCLAW_GATEWAY_PASSWORD;
