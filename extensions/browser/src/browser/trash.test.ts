...
const resolvePreferredDaisyClawTmpDirMock = vi.hoisted(() => vi.fn(() => "/tmp/daisyclaw"));
...
describe("browser trash", () => {
  ...
  beforeEach(() => {
    vi.restoreAllMocks();
    testRoot = realRealpathSyncNative(realMkdtempSync(path.join(os.tmpdir(), "daisyclaw-browser-")));
    ...
    resolvePreferredDaisyClawTmpDirMock.mockReturnValue(tmpDir);
  ...
  it("refuses to trash paths outside allowed roots", async () => {
    ...
    const outsidePath = path.join(outsideDir, "daisyclaw-demo");
