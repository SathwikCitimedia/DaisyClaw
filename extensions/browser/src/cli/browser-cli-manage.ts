...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "daisyclaw";
...
async function runBrowserDoctor(parent: BrowserParentOpts, profile?: string, deep?: boolean) {
  ...
  checks.push({
    name: "profile",
    ok: true,
    detail: `${status.profile ?? "daisyclaw"} (${usesChromeMcpTransport(status) ? "chrome-mcp" : (status.transport ?? "cdp")})`,
  });
  checks.push({
    name: "browser",
    ok: status.running,
    detail: status.running
      ...
      : "not running; run `daisyclaw browser start`",
  });
...
export function registerBrowserManageCommands(
  ...
  browser
    ...
    .action(async (_opts, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        defaultRuntime.log(
          [
            `profile: ${status.profile ?? "daisyclaw"}`,
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "daisyclaw" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (daisyclaw|existing-session). Default: daisyclaw")
    .action(
      async (
        opts: {
          name: string;
