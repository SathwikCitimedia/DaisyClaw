...
describe("browser doctor readiness", () => {
  ...
  it("warns when managed browser profiles have no local executable", async () => {
    ...
    expect(noteFn).toHaveBeenCalledWith(
      [
        "- DaisyClaw-managed browser profile(s) are configured: daisyclaw.",
  ...
  it("warns about legacy clawd managed browser profile residue", async () => {
    const noteFn = vi.fn();
    const configDir = "/tmp/daisyclaw-home";
    ...
    expect(note).toContain("/tmp/daisyclaw-home/browser/clawd");
    expect(note).toContain("/tmp/daisyclaw-home/browser/daisyclaw/user-data");
    expect(note).toContain("daisyclaw doctor --fix");
  });
...
describe("legacy clawd browser profile cleanup", () => {
  it("archives stale clawd residue with the safe trash mover", async () => {
    const movePathToTrash = vi.fn(async () => "/tmp/daisyclaw-home/browser/.trash/clawd");
    ...
    expect(movePathToTrash).toHaveBeenCalledWith("/tmp/daisyclaw-home/browser/clawd");
    expect(result.warnings).toStrictEqual([]);
    expect(result.changes.join("\n")).toContain(
      "Archived legacy clawd managed browser profile residue.",
    );
    expect(result.changes.join("\n")).toContain("/tmp/daisyclaw-home/browser/daisyclaw/user-data");
  });
