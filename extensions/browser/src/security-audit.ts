 ...
 */
import type { DaisyClawPluginSecurityAuditContext } from "daisyclaw/plugin-sdk/plugin-entry";
...
import { isPrivateNetworkOptInEnabled, isPrivateIpAddress } from "daisyclaw/plugin-sdk/ssrf-policy";
...
export function collectBrowserSecurityAuditFindings(ctx: DaisyClawPluginSecurityAuditContext) {
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${ctx.configPath} and re-run "${formatCliCommand("daisyclaw security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(ctx.env.DAISYCLAW_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(ctx.env.DAISYCLAW_GATEWAY_PASSWORD) ||
