...
import type { AnyAgentTool, DaisyClawPluginApi } from "daisyclaw/plugin-sdk/plugin-entry";
...
function registerCanvas() {
  const routes: Array<Parameters<DaisyClawPluginApi["registerHttpRoute"]>[0]> = [];
  const services: Array<Parameters<DaisyClawPluginApi["registerService"]>[0]> = [];
  const resolvers: Array<Parameters<DaisyClawPluginApi["registerHostedMediaResolver"]>[0]> = [];
  const tools: Array<Parameters<DaisyClawPluginApi["registerTool"]>[0]> = [];
  const cliFeatures: Array<{
    registrar: Parameters<DaisyClawPluginApi["registerNodeCliFeature"]>[0];
    opts: Parameters<DaisyClawPluginApi["registerNodeCliFeature"]>[1];
  }> = [];
...
describe("Canvas plugin entry", () => {
  ...
  it("defers Canvas host implementation until a registered route is used", async () => {
    ...
    await routes[0]?.handler({ url: "/__daisyclaw__/canvas" } as never, {} as never);
  ...
  it("defers Canvas resolver, CLI, and tool implementations until use", async () => {
    ...
    await expect(resolvers[0]?.("/__daisyclaw__/canvas/documents/id/index.html")).resolves.toBe(
      "/tmp/canvas-asset",
    );
