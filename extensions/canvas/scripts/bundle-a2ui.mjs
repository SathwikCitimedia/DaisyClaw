...
async function main() {
  ...
  if (!hasA2uiPackage || !hasAppDir) {
    ...
    if (process.env.DAISYCLAW_SPARSE_PROFILE || process.env.DAISYCLAW_A2UI_SKIP_MISSING === "1") {
      console.error(
        "A2UI package missing; skipping bundle because DAISYCLAW_A2UI_SKIP_MISSING=1 or DAISYCLAW_SPARSE_PROFILE is set.",
      );
      return;
    }
