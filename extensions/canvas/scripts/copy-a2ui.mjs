...
function getA2uiPaths(env = process.env) {
  const srcDir = env.DAISYCLAW_A2UI_SRC_DIR ?? path.join(pluginDir, "src", "host", "a2ui");
  const outDir = env.DAISYCLAW_A2UI_OUT_DIR ?? path.join(rootDir, "dist", "canvas-host", "a2ui");
  return { srcDir, outDir };
}

function shouldSkipMissingA2uiAssets(env = process.env) {
  return env.DAISYCLAW_A2UI_SKIP_MISSING === "1" || Boolean(env.DAISYCLAW_SPARSE_PROFILE);
}
...
export async function copyA2uiAssets({ srcDir, outDir }) {
  ...
  try {
    ...
    if (skipMissing) {
      console.warn(
        `${message} Skipping copy because DAISYCLAW_A2UI_SKIP_MISSING=1 or DAISYCLAW_SPARSE_PROFILE is set.`,
      );
      return;
    }
