...
import { resolvePreferredDaisyClawTmpDir, withTempWorkspace } from "daisyclaw/plugin-sdk/temp-path";
import { afterEach, beforeEach, describe, expect, it } from "vitest";
import { copyA2uiAssets } from "./copy-a2ui.mjs";

const ORIGINAL_SKIP_MISSING = process.env.DAISYCLAW_A2UI_SKIP_MISSING;
const ORIGINAL_SPARSE_PROFILE = process.env.DAISYCLAW_SPARSE_PROFILE;
...
describe("canvas a2ui copy", () => {
  beforeEach(() => {
    delete process.env.DAISYCLAW_A2UI_SKIP_MISSING;
    delete process.env.DAISYCLAW_SPARSE_PROFILE;
  });
  ...
  afterEach(() => {
    if (ORIGINAL_SKIP_MISSING === undefined) {
      ...
      process.env.DAISYCLAW_A2UI_SKIP_MISSING = ORIGINAL_SKIP_MISSING;
    }

    if (ORIGINAL_SPARSE_PROFILE === undefined) {
      delete process.env.DAISYCLAW_SPARSE_PROFILE;
    } else {
      process.env.DAISYCLAW_SPARSE_PROFILE = ORIGINAL_SPARSE_PROFILE;
    }
  });

  async function withA2uiFixture(run: (dir: string) => Promise<void>) {
    await withTempWorkspace(
      { rootDir: resolvePreferredDaisyClawTmpDir(), prefix: "daisyclaw-a2ui-" },
  ...
  it("skips missing assets when DAISYCLAW_A2UI_SKIP_MISSING=1", async () => {
    await withA2uiFixture(async (dir) => {
      process.env.DAISYCLAW_A2UI_SKIP_MISSING = "1";
      await expect(
  ...
  it("skips missing assets when DAISYCLAW_SPARSE_PROFILE is set", async () => {
    await withA2uiFixture(async (dir) => {
      process.env.DAISYCLAW_SPARSE_PROFILE = "core";
      await expect(
        copyA2uiAssets({ srcDir: dir, outDir: path.join(dir, "out") }),
      ).resolves.toBeUndefined();
    });
