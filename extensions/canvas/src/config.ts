...
export function isCanvasPluginEnabled(config?: DaisyClawConfig): boolean {
  if (!config) {
    return true;
  }
...
export function isCanvasHostEnabled(config?: DaisyClawConfig): boolean {
...
export const canvasConfigSchema: CanvasPluginConfigSchema = {
  ...
  uiHints: {
    ...
    "host.root": {
      ...
      help: "Directory to serve. Defaults to the DaisyClaw state canvas directory.",
      advanced: true,
    },
    "host.port": {
