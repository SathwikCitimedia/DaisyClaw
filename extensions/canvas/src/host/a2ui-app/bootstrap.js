...
import { themeContext } from "@daisyclaw/a2ui-theme-context";
...
class DaisyClawA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  connectedCallback() {
    ...
    globalThis.daisyclawA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["daisyclaw:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    const handler =
      globalThis.webkit?.messageHandlers?.daisyclawCanvasA2UIAction ??
      globalThis.daisyclawCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.daisyclawCanvasA2UIAction) {
...
if (!customElements.get("daisyclaw-a2ui-host")) {
  customElements.define("daisyclaw-a2ui-host", DaisyClawA2UIHost);
}
