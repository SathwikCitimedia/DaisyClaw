...
import { withStateDirEnv } from "daisyclaw/plugin-sdk/test-env";
...
describe("canvas host state dir defaults", () => {
  ...
  it("uses DAISYCLAW_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("daisyclaw-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("DaisyClaw Canvas");
      } finally {
        await handler.close();
      }
    });
