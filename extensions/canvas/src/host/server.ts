...
import { detectMime } from "daisyclaw/plugin-sdk/media-mime";
import { isTruthyEnvValue, type RuntimeEnv } from "daisyclaw/plugin-sdk/runtime-env";
import { resolveStateDir } from "daisyclaw/plugin-sdk/state-paths";
import {
  lowercasePreservingWhitespace,
  normalizeOptionalString,
} from "daisyclaw/plugin-sdk/string-coerce-runtime";
import { ensureDir, resolveUserPath } from "daisyclaw/plugin-sdk/text-utility-runtime";
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.daisyclawCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.daisyclawCanvasA2UIAction &&
        typeof window.daisyclawCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.daisyclawSendUserAction === "function";
  ...
  window.addEventListener("daisyclaw:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android DaisyClaw node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.daisyclawSendUserAction === "function"
        ? window.daisyclawSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.DAISYCLAW_SKIP_CANVAS_HOST)) {
    return true;
  }
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>DaisyClaw Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
