...
export function resolveSafeApprovalResult(method: string): Record<string, unknown> | undefined {
  ...
  if (method.endsWith("/requestApproval")) {
    return {
      decision: "decline",
      reason: "DaisyClaw Codex supervisor does not grant native approvals.",
    };
  }
...
abstract class BaseCodexJsonRpcConnection implements CodexJsonRpcConnection {
  ...
  async initialize(): Promise<void> {
    await this.request("initialize", {
      clientInfo: {
        name: "daisyclaw-codex-supervisor",
        title: "DaisyClaw Codex Supervisor",
        version: "0.1.0",
      },
      capabilities: {
        ...
        JSON.stringify(
          result === undefined
            ? {
                ...
                error: {
                  code: -32601,
                  message: `DaisyClaw Codex supervisor cannot handle app-server request: ${method}`,
                },
              }
            : { id, result },
        ),
