...
async function describeCodexImages(
  ...
  const text = await runBoundedCodexVisionTurn({
    ...
    developerInstructions:
      "You are DaisyClaw's bounded image-understanding worker. Describe only the provided image content. Do not call tools, edit files, or ask follow-up questions.",
    input: [
...
async function extractCodexStructured(
  ...
  const text = await runBoundedCodexVisionTurn({
    ...
    developerInstructions:
      "You are DaisyClaw's bounded structured-extraction worker. Return only the requested extraction. Do not call tools, edit files, ask follow-up questions, or include secrets.",
...
function denyCodexImageApprovalRequest(request: { method: string }): JsonValue | undefined {
  if (
    ...
    return {
      decision: "decline",
      reason: "DaisyClaw Codex image understanding does not grant tool or file approvals.",
    };
  }
  ...
  if (request.method.includes("requestApproval")) {
    return {
      decision: "decline",
      reason: "DaisyClaw Codex image understanding does not grant native approvals.",
    };
  }
