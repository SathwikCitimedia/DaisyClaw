...
describe("Codex app-server policy", () => {
  it("keeps implicit Codex yolo approval policy when untrusted approvals are disallowed", () => {
    ...
    const resolved = resolveCodexAppServerForDaisyClawToolPolicy({
      appServer,
  ...
  it("promotes implicit yolo approval policy when DaisyClaw tool policy requires review", () => {
    const appServer = resolveCodexAppServerRuntimeOptions({ env: {}, requirementsToml: null });
  ...
  it("preserves explicit operator app-server policy", () => {
    ...
    const explicitConfig = resolveCodexAppServerForDaisyClawToolPolicy({
      appServer,
    ...
    const explicitEnv = resolveCodexAppServerForDaisyClawToolPolicy({
      appServer,
      pluginConfig: readCodexPluginConfig({}),
      env: { DAISYCLAW_CODEX_APP_SERVER_APPROVAL_POLICY: "never" },
      shouldPromote: true,
      canUseUntrustedApprovalPolicy: true,
    });
    const explicitRequirements = resolveCodexAppServerForDaisyClawToolPolicy({
