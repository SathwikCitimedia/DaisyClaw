...
import { reviewExecRequestWithConfiguredModel } from "daisyclaw/plugin-sdk/agent-harness-exec-review-runtime";
import {
  callGatewayTool,
...
vi.mock("daisyclaw/plugin-sdk/agent-harness-runtime", async (importOriginal) => ({
  ...(await importOriginal<typeof import("daisyclaw/plugin-sdk/agent-harness-runtime")>()),
...
vi.mock("daisyclaw/plugin-sdk/agent-harness-exec-review-runtime", async (importOriginal) => ({
  ...(await importOriginal<
    typeof import("daisyclaw/plugin-sdk/agent-harness-exec-review-runtime")
  >()),
...
describe("Codex app-server approval bridge", () => {
  ...
  it("routes command approvals through plugin approvals and accepts allowed commands", async () => {
    ...
    expect(requestPayload.pluginId).toBe("daisyclaw-codex-app-server");
  ...
  it("uses the configured DaisyClaw exec auto-review model before plugin approvals", async () => {
    const params = createParams();
    ...
    findApprovalEvent(params, {
      status: "approved",
      message:
        "Codex app-server command approval granted by DaisyClaw exec auto-reviewer: read-only version check",
    });
  });
  ...
  it("falls back to plugin approval when Codex native OpenAI config uses a local base URL", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-codex-approval-"));
    try {
  ...
  it.each([
    ...
    "sudo -EH bash -lc 'daisyclaw channels login --channel whatsapp'",
  ...
  it("keeps security audit suppression edits on the plugin approval route", async () => {
    ...
    const result = await handleCodexAppServerApprovalRequest({
      ...
      requestParams: {
        ...
        command: "daisyclaw config set security.audit.suppressions '[]'",
      },
      paramsForRun: params,
      threadId: "thread-1",
      turnId: "turn-1",
  ...
  it("normalizes prefixed channel targets for DaisyClaw tool policy context", async () => {
    const params = createParams();
  ...
  it("denies command approvals before prompting when DaisyClaw tool policy blocks", async () => {
    const params = createParams();
  ...
  it("routes command approvals through the active native hook relay before prompting", async () => {
    ...
    expect(mockInvokeNativeHookRelay).toHaveBeenCalledWith({
      ...
      rawPayload: {
        ...
        daisyclaw_approval_mode: "report",
  ...
  it("fails closed when the native hook relay returns unreadable approval output", async () => {
    ...
    findApprovalEvent(params, {
      status: "denied",
      message:
        "DaisyClaw native hook relay returned an unreadable Codex app-server approval result.",
    });
  });
  ...
  it("fails closed when the native hook relay returns a non-deny decision", async () => {
    ...
    findApprovalEvent(params, {
      status: "denied",
      message: "DaisyClaw native hook relay returned a non-deny Codex app-server approval decision.",
    });
  });
  ...
  it("fails closed when the expected native hook relay cannot be invoked", async () => {
    ...
    findApprovalEvent(params, {
      status: "denied",
      message:
        "DaisyClaw native hook relay unavailable for Codex app-server approval: native hook relay not found",
    });
  });
  ...
  it("denies command approvals when DaisyClaw tool policy rewrites params", async () => {
    const params = createParams();
    mockRunBeforeToolCallHook.mockResolvedValueOnce({
      blocked: false,
      params: {
    ...
    findApprovalEvent(params, {
      status: "denied",
      message:
        "DaisyClaw tool policy rewrote Codex app-server approval params; refusing original request.",
    });
  });

  it("keeps DaisyClaw plugin allow-always approvals scoped to one Codex request", async () => {
    const params = createParams();
    mockRunBeforeToolCallHook.mockResolvedValueOnce({
      blocked: false,
      params: {
  ...
  it("denies command approvals when DaisyClaw tool policy requires approval", async () => {
    const params = createParams();
    mockRunBeforeToolCallHook.mockResolvedValueOnce({
      blocked: true,
      kind: "failure",
  ...
  it("fails closed for unsupported native approval methods without requesting plugin approval", async () => {
    ...
    expect(result).toEqual({
      decision: "decline",
      reason: "DaisyClaw codex app-server bridge does not grant native approvals yet.",
    });
    expect(mockCallGatewayTool).not.toHaveBeenCalled();
