...
} from "daisyclaw/plugin-sdk/agent-harness-exec-review-runtime";
/**
 * Bridges Codex app-server approval requests into DaisyClaw policy hooks and
 * plugin approval UX.
 */
import {
...
export async function handleCodexAppServerApprovalRequest(params: {
  ...
  try {
    const policyOutcome = await runDaisyClawToolPolicyForApprovalRequest({
      method: params.method,
      requestParams,
      paramsForRun: params.paramsForRun,
      context,
    ...
    // Native hook/model policy did not decide; fall back to the DaisyClaw
...
/** Converts an DaisyClaw approval outcome into the app-server method response. */
...
async function runInternalExecAutoReviewForApprovalRequest(params: {
  ...
  return {
    ...
    reason: `Codex app-server command approval granted by DaisyClaw exec auto-reviewer: ${formatCodexDisplayText(
      decision.rationale,
    )}`,
  };
}
...
async function runDaisyClawToolPolicyForApprovalRequest(params: {
  method: string;
  requestParams: JsonObject | undefined;
  paramsForRun: EmbeddedRunAttemptParams;
  context: ApprovalContext;
  ...
  const policyRequest = buildDaisyClawToolPolicyRequest(params.method, params.requestParams);
  if (!policyRequest) {
    return undefined;
  }
...
> {
  ...
  try {
    ...
    return {
      handled: true,
      blocked: true,
      reason: `DaisyClaw native hook relay unavailable for Codex app-server approval: ${formatCodexDisplayText(
        formatErrorMessage(error),
      )}`,
    };
  }
...
function readNativeRelayPreToolUseDecision(
  ...
  if (!response || response.exitCode !== 0) {
    return {
      ...
      reason:
        ...
        "DaisyClaw native hook relay failed for Codex app-server approval.",
    };
  }
  ...
  if (output?.permissionDecision === "deny") {
    return {
      ...
      reason:
        ...
        "DaisyClaw native hook policy denied Codex app-server approval.",
    };
  }
  ...
  return {
    blocked: true,
    reason: output
      ? "DaisyClaw native hook relay returned a non-deny Codex app-server approval decision."
...
function buildDaisyClawToolPolicyRequest(
  method: string,
  requestParams: JsonObject | undefined,
