...
import { resolveAgentWorkspaceDir } from "daisyclaw/plugin-sdk/agent-runtime";
...
export async function buildCodexWorkspaceBootstrapContext(params: {
  ...
  try {
    ...
    const developerInstructionFiles = shouldInjectCodexDaisyClawPromptContext(params.params)
      ? selectCodexWorkspaceInheritedDeveloperInstructionFiles(contextFiles)
      : [];
    const turnScopedDeveloperInstructionFiles = shouldInjectCodexDaisyClawPromptContext(
      params.params,
    )
    ...
    return {
      ...
      memoryCollaborationInstructions: shouldInjectCodexDaisyClawPromptContext(params.params)
...
/**
 * Builds DaisyClaw-provided workspace prompt context for the current Codex turn.
 */
export function buildCodexDaisyClawPromptContext(params: {
  params: EmbeddedRunAttemptParams;
  workspacePromptContext?: string;
}): string | undefined {
  if (!shouldInjectCodexDaisyClawPromptContext(params.params)) {
    return undefined;
  }
  const sections = [
    params.workspacePromptContext?.trim()
      ? ["## DaisyClaw Workspace Context", "", params.workspacePromptContext.trim()].join("\n")
      : undefined,
  ].filter(isNonEmptyString);
...
function shouldInjectCodexDaisyClawPromptContext(params: EmbeddedRunAttemptParams): boolean {
...
/** Renders loaded DaisyClaw skill prompts as Codex collaboration instructions. */
export function renderCodexSkillsCollaborationInstructions(params: {
  attempt: EmbeddedRunAttemptParams;
  skillsPrompt?: string;
}): string | undefined {
  if (!shouldInjectCodexDaisyClawPromptContext(params.attempt)) {
    return undefined;
  }
  return params.skillsPrompt?.trim()
    ? ["## DaisyClaw Skills", "", params.skillsPrompt.trim()].join("\n")
    : undefined;
}

/**
 * Prepends DaisyClaw context while preserving leading delivery metadata as
 * routing guidance instead of user request text.
 */
export function prependCodexDaisyClawPromptContext(
  prompt: string,
...
function renderCodexWorkspaceThreadDeveloperInstructions(
  ...
  return renderCodexWorkspaceDeveloperInstructions({
    files,
    header: "## DaisyClaw Workspace Instructions",
    preamble:
...
export function renderCodexWorkspaceMemoryReference(params: {
  ...
  const lines = [
    "## DaisyClaw Workspace Memory",
    "",
    `MEMORY.md exists in the active agent workspace as a memory file, not an instruction file. DaisyClaw does not paste its contents into native Codex turns; use ${toolNames.join(" or ")} when durable memory is relevant and the tools are available.`,
    "",
  ];
