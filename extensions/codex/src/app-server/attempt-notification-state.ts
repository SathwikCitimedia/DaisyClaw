...
import {
  ...
  isPendingDaisyClawDynamicToolCompletionNotification,
...
export function applyCodexTurnNotificationState(params: {
  ...
  pendingDaisyClawDynamicToolCompletionIds: Set<string>;
  ...
  const trackedDynamicToolCompletion = isPendingDaisyClawDynamicToolCompletionNotification(
    notification,
    params.pendingDaisyClawDynamicToolCompletionIds,
  );
  ...
  if (
    ...
    // accepts a turn or after DaisyClaw hands a turn-scoped request result
    // back to Codex. Bookkeeping that closes the just-served DaisyClaw
  ...
  if (trackedDynamicToolCompletion) {
    ...
    if (itemId) {
      params.pendingDaisyClawDynamicToolCompletionIds.delete(itemId);
      params.onScheduleTerminalDynamicToolReleaseCheck();
    }
  }
