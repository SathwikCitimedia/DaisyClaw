 ...
 */
import { asBoolean } from "daisyclaw/plugin-sdk/string-coerce-runtime";
import {
...
/** Detects completion for an DaisyClaw dynamic tool result still awaited by Codex. */
export function isPendingDaisyClawDynamicToolCompletionNotification(
  notification: CodexServerNotification,
  pendingDaisyClawDynamicToolCompletionIds: ReadonlySet<string>,
): boolean {
  if (notification.method !== "item/completed" || !isJsonObject(notification.params)) {
    return false;
  }
  const itemId = readNotificationItemId(notification);
  if (!itemId || !pendingDaisyClawDynamicToolCompletionIds.has(itemId)) {
    return false;
  }
  const item = isJsonObject(notification.params.item) ? notification.params.item : undefined;
