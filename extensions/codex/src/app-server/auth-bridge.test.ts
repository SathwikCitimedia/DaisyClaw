...
vi.mock("daisyclaw/plugin-sdk/agent-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("daisyclaw/plugin-sdk/agent-runtime")>();
  return {
    ...actual,
    resolveApiKeyForProfile: async (
      ...
      if (params.forceRefresh || (oauthCredential.expires ?? 0) <= Date.now()) {
        ...
        if (refreshed?.access) {
          ...
          if (params.agentDir || process.env.DAISYCLAW_STATE_DIR) {
...
describe("bridgeCodexAppServerStartOptions", () => {
  it("sets agent-owned CODEX_HOME without overriding HOME for local app-server launches", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-codex-app-server-"));
  ...
  it("applies native Codex CLI OAuth when no DaisyClaw auth profile exists", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-codex-app-server-"));
  ...
  it("answers refresh from native Codex CLI OAuth without persisting an DaisyClaw profile", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-codex-app-server-"));
    const agentDir = path.join(root, "agent");
    const codexHome = path.join(root, "codex-cli");
