...
import { embeddedAgentLog, DAISYCLAW_VERSION } from "daisyclaw/plugin-sdk/agent-harness-runtime";
...
export class CodexAppServerClient {
  ...
  /** Registers a handler for app-server requests sent back to DaisyClaw. */
  ...
  if (request.method === "item/tool/call") {
    return {
      contentItems: [
        {
          type: "inputText",
          text: "DaisyClaw did not register a handler for this app-server tool call.",
        },
      ],
      success: false,
    };
...
function timeoutServerRequestResponse(
  ...
  return {
    contentItems: [
      {
        type: "inputText",
        text: `DaisyClaw dynamic tool call timed out after ${CODEX_DYNAMIC_TOOL_SERVER_REQUEST_TIMEOUT_MS}ms before sending a response to Codex.`,
      },
    ],
    success: false,
  };
...
function assertSupportedCodexAppServerVersion(response: CodexInitializeResponse): string {
  ...
  if (!detectedVersion) {
    throw new Error(
      `Codex app-server ${MIN_CODEX_APP_SERVER_VERSION} or newer is required, but DaisyClaw could not determine the running Codex version. Update the configured Codex app-server binary, or remove custom command overrides to use the managed binary.`,
    );
  }
...
export function readCodexVersionFromUserAgent(userAgent: string | undefined): string | undefined {
  ...
  // DaisyClaw, Codex Desktop, or an env override, so only the slash-delimited
...
/** Returns true for app-server approval request methods DaisyClaw can answer. */
