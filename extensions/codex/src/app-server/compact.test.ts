...
describe("maybeCompactCodexAppServerSession", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-codex-compact-"));
  });
  ...
  it("blocks native app-server compaction when the current DaisyClaw session is sandboxed", async () => {
    const fake = createFakeCodexClient();
    setCodexAppServerClientFactoryForTest(async () => fake.client);
    const sessionFile = await writeTestBinding();
    ...
    expect(result.reason).toContain(
      "Codex-native native compaction is unavailable because DaisyClaw sandboxing is active for this session.",
    );
    expect(fake.request).not.toHaveBeenCalled();
  });
  ...
  it("blocks native app-server compaction when exec host=node is active", async () => {
    ...
    expect(result.compacted).toBe(false);
    expect(result.reason).toContain(
      "Codex-native native compaction is unavailable because DaisyClaw exec host=node is active for this session.",
    );
    expect(fake.request).not.toHaveBeenCalled();
  });
  ...
  it("does not impose an DaisyClaw timeout after Codex accepts native compaction", async () => {
    const fake = createFakeCodexClient();
  ...
  it("warns when stale DaisyClaw compaction overrides are ignored", async () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      "ignoring DaisyClaw compaction overrides for Codex app-server compaction; Codex uses native server-side compaction",
      {
        sessionId: "session-1",
        sessionKey: "agent:main:session-1",
