...
import { MAX_TIMER_TIMEOUT_MS } from "daisyclaw/plugin-sdk/number-runtime";
...
import {
  ...
  resolveDaisyClawExecModeForCodexAppServer,
  resolveDaisyClawExecModeFromConfig,
  resolveDaisyClawExecPolicyForCodexAppServer,
...
describe("Codex app-server config", () => {
  ...
  it("parses typed plugin config before falling back to environment knobs", () => {
    const runtime = resolveRuntimeForTest({
      ...
      env: {
        DAISYCLAW_CODEX_APP_SERVER_APPROVAL_POLICY: "never",
        DAISYCLAW_CODEX_APP_SERVER_SANDBOX: "read-only",
      },
  ...
  it("treats only explicit OpenAI model context as safe for Codex-backed auto-review", () => {
    ...
    for (const openAIProvider of [
      ...
      {
        baseUrl: "https://api.openai.com/v1",
        headers: { "x-daisyclaw-reviewer-proxy": "local" },
        models: [],
      },
      {
        baseUrl: "https://api.openai.com/v1",
...
`,
  ...
  it("keeps explicit yolo mode when requirements disallow full access", () => {
    ...
    expectRuntimePolicy(
      resolveRuntimeForTest({
        pluginConfig: {},
        env: { DAISYCLAW_CODEX_APP_SERVER_MODE: "yolo" },
        requirementsToml,
      }),
      {
        approvalPolicy: "never",
  ...
  it("rejects the retired dynamic tool profile key", () => {
    expect(
      readCodexPluginConfig({
        codexDynamicToolsProfile: "daisyclaw-compat",
        codexDynamicToolsLoading: "direct",
      }),
    ).toEqual({});
  });
  ...
  it("treats configured and environment commands as explicit overrides", () => {
    expectFields(
      resolveRuntimeForTest({
        ...
        env: { DAISYCLAW_CODEX_APP_SERVER_BIN: "/usr/local/bin/codex" },
      }).start,
      "configured start",
      {
  ...
  it("rejects Codex app-server command overrides that include inline arguments", () => {
    ...
    ).toThrow("DAISYCLAW_CODEX_APP_SERVER_BIN must be only the Codex app-server executable path");
  });
  ...
  it("resolves Computer Use setup from plugin config and environment fallbacks", () => {
    expect(
      resolveCodexComputerUseConfig({
        ...
        env: {
          DAISYCLAW_CODEX_COMPUTER_USE_PLUGIN_NAME: "env-fallback-plugin",
        },
      }),
    ).toEqual({
      enabled: true,
    ...
    expectFields(
      resolveCodexComputerUseConfig({
        pluginConfig: {},
        env: {
          DAISYCLAW_CODEX_COMPUTER_USE: "1",
          DAISYCLAW_CODEX_COMPUTER_USE_MARKETPLACE_SOURCE: "github:example/plugins",
          DAISYCLAW_CODEX_COMPUTER_USE_AUTO_INSTALL: "true",
          DAISYCLAW_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS: "30000",
        },
      }),
      "computer use config",
      {
    ...
    for (const value of ["0x10", "1e3"]) {
      expectFields(
        resolveCodexComputerUseConfig({
          ...
          env: {
            DAISYCLAW_CODEX_COMPUTER_USE: "1",
            DAISYCLAW_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS: value,
          },
        }),
        "computer use config",
        {
  ...
  it("allows environment mode fallback to opt in to guardian-reviewed local execution", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      modelProvider: "openai",
      env: { DAISYCLAW_CODEX_APP_SERVER_MODE: "guardian" },
    });
  ...
  it("maps normalized DaisyClaw auto exec mode to guardian-reviewed local execution", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      execMode: "auto",
      modelProvider: "openai",
  ...
  it("forces guarded app-server policy fields for auto mode", () => {
    const runtime = resolveRuntimeForTest({
      ...
      env: {
        DAISYCLAW_CODEX_APP_SERVER_APPROVAL_POLICY: "never",
        DAISYCLAW_CODEX_APP_SERVER_SANDBOX: "danger-full-access",
      },
      execMode: "auto",
      modelProvider: "openai",
    });
  ...
  it("preserves explicit read-only app-server sandbox for auto mode", () => {
    ...
    const envRuntime = resolveRuntimeForTest({
      ...
      modelProvider: "openai",
      env: {
        DAISYCLAW_CODEX_APP_SERVER_MODE: "yolo",
        DAISYCLAW_CODEX_APP_SERVER_APPROVAL_POLICY: "never",
        DAISYCLAW_CODEX_APP_SERVER_SANDBOX: "read-only",
      },
    });
  ...
  it.each(["deny", "allowlist"] as const)(
    "blocks Codex app-server local execution for normalized DaisyClaw %s exec mode",
    (execMode) => {
      expect(() =>
        resolveRuntimeForTest({
          pluginConfig: {},
  ...
  it("maps normalized DaisyClaw ask exec mode away from Codex yolo", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      execMode: "ask",
    });
  ...
  it("fails closed when normalized DaisyClaw ask mode cannot use user approvals", () => {
    expect(() =>
      resolveRuntimeForTest({
        pluginConfig: {},
        execMode: "ask",
  ...
  it.each([
    ...
    "fails closed when normalized DaisyClaw $execMode mode can only use $policies approvals",
    ({ execMode, policies }) => {
      expect(() =>
        resolveRuntimeForTest({
          pluginConfig: {},
  ...
  it("keeps normalized DaisyClaw full exec mode on default Codex yolo", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      execMode: "full",
    });
  ...
  it("fails closed when normalized DaisyClaw auto mode can only use on-failure approvals", () => {
    expect(() =>
      resolveRuntimeForTest({
        pluginConfig: {},
        execMode: "auto",
  ...
  it("fails closed when normalized DaisyClaw auto mode cannot force prompting over yolo", () => {
    expect(() =>
      resolveRuntimeForTest({
        pluginConfig: {},
        execMode: "auto",
  ...
  it("uses user approvals when normalized DaisyClaw auto mode cannot use Codex auto-review", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      execMode: "auto",
      requirementsToml:
  ...
  it("keeps normalized DaisyClaw auto mode when legacy app-server yolo was schema-defaulted", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {
        appServer: {
          command: "codex",
  ...
  it("forces guarded policy fields for normalized DaisyClaw auto mode", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {
        appServer: {
          approvalPolicy: "never",
  ...
  it("resolves agent-scoped normalized DaisyClaw exec mode for Codex app-server mapping", () => {
    const config = {
      tools: {
        exec: {
          mode: "ask",
    ...
    expect(resolveDaisyClawExecModeFromConfig({ config, agentId: "codex-agent" })).toBe("auto");
    expect(resolveDaisyClawExecModeFromConfig({ config, agentId: "other-agent" })).toBe("ask");
  });

  it("keeps legacy exec security overrides ahead of normalized DaisyClaw exec mode", () => {
    expect(
      resolveDaisyClawExecModeFromConfig({
        config: {
          tools: {
  ...
  it.each(["always"] as const)(
    ...
    (ask) => {
      ...
      const execPolicy = resolveDaisyClawExecPolicyForCodexAppServer({ config });

      expect(resolveDaisyClawExecModeForCodexAppServer({ config })).toBe("ask");
      expectRuntimePolicy(
        resolveRuntimeForTest({
          pluginConfig: {
            appServer: {
  ...
  it("keeps legacy full exec security with ask=on-miss on default Codex yolo", () => {
    ...
    expect(resolveDaisyClawExecModeForCodexAppServer({ config })).toBe("full");
  ...
  it("fails closed when legacy full exec with ask cannot use full Codex sandbox", () => {
    ...
    expect(() =>
      resolveRuntimeForTest({
        execPolicy: resolveDaisyClawExecPolicyForCodexAppServer({ config }),
        requirementsToml: 'allowed_sandbox_modes = ["read-only", "workspace-write"]\n',
      }),
    ).toThrow("legacy full exec security with ask requires Codex app-server danger-full-access");
  });

  it("clamps legacy full exec with ask when an DaisyClaw sandbox is active", () => {
    const config = {
      tools: {
        exec: {
          security: "full",
  ...
  it("applies host exec approval security floors before starting Codex app-server", () => {
    const execPolicy = resolveDaisyClawExecPolicyForCodexAppServer({
      config: {
        tools: {
          exec: {
            mode: "full",
  ...
  it("treats ask-only legacy overrides as normalized mode overrides", () => {
    ...
    expect(resolveDaisyClawExecModeFromConfig({ config, agentId: "codex-agent" })).toBe("allowlist");
    const execMode = resolveDaisyClawExecModeForCodexAppServer({
      config,
      agentId: "main",
      execOverrides: {
        ask: "always",
  ...
  it("ignores removed DAISYCLAW_CODEX_APP_SERVER_GUARDIAN fallback", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      env: { DAISYCLAW_CODEX_APP_SERVER_GUARDIAN: "1" },
    });
  ...
  it("keeps runtime config keys aligned with manifest schema and UI hints", async () => {
    const manifest = JSON.parse(
      await fs.readFile(new URL("../../daisyclaw.plugin.json", import.meta.url), "utf8"),
    ) as {
      configSchema: {
        properties: {
