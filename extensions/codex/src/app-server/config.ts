...
} from "daisyclaw/plugin-sdk/exec-approvals-runtime";
import { resolvePositiveTimerTimeoutMs } from "daisyclaw/plugin-sdk/number-runtime";
import { normalizeAgentId } from "daisyclaw/plugin-sdk/routing";
import { normalizeTrimmedStringList } from "daisyclaw/plugin-sdk/string-coerce-runtime";
import { detectWindowsSpawnCommandInlineArgs } from "daisyclaw/plugin-sdk/windows-spawn";
import { z } from "zod";
import type { CodexSandboxPolicy, CodexServiceTier } from "./protocol.js";

const START_OPTIONS_KEY_SECRET_SYMBOL = Symbol.for("daisyclaw.codexAppServerStartOptionsKeySecret");
...
type DaisyClawExecMode = "deny" | "allowlist" | "ask" | "auto" | "full";
type DaisyClawExecSecurity = "deny" | "allowlist" | "full";
type DaisyClawExecAsk = "off" | "on-miss" | "always";
type DaisyClawExecApprovalFloorsForCodexAppServer = {
  security?: DaisyClawExecSecurity;
  ask?: DaisyClawExecAsk;
};
export type DaisyClawExecPolicyForCodexAppServer = {
  mode?: DaisyClawExecMode;
  security: DaisyClawExecSecurity;
  ask: DaisyClawExecAsk;
  touched: boolean;
};
type DaisyClawExecPolicy = DaisyClawExecPolicyForCodexAppServer;
...
export function resolveCodexAppServerRuntimeOptions(
  params: {
    ...
    hostName?: string;
    daisyClawSandboxActive?: boolean;
  } = {},
  ...
  const envCommand = readNonEmptyString(env.DAISYCLAW_CODEX_APP_SERVER_BIN);
  ...
  const args = resolveArgs(config.args, env.DAISYCLAW_CODEX_APP_SERVER_ARGS);
  const headers = normalizeHeaders(config.headers);
  const clearEnv = normalizeStringList(config.clearEnv);
  const authToken = readNonEmptyString(config.authToken);
  const url = readNonEmptyString(config.url);
  const execMode = resolveEffectiveDaisyClawExecModeForCodexAppServer({
    execMode: params.execMode,
    execPolicy: params.execPolicy,
  });
  assertCodexAppServerAllowedForDaisyClawExecMode(execMode);
  const explicitPolicyMode =
    resolvePolicyMode(config.mode) ?? resolvePolicyMode(env.DAISYCLAW_CODEX_APP_SERVER_MODE);
  const configuredSandbox =
    resolveSandbox(config.sandbox) ?? resolveSandbox(env.DAISYCLAW_CODEX_APP_SERVER_SANDBOX);
  const explicitApprovalsReviewer = resolveApprovalsReviewer(config.approvalsReviewer);
  const normalizedPolicyMode = resolveCodexPolicyModeForDaisyClawExecMode(execMode);
  ...
  const execModeRequiringPromptingApprovals: Extract<DaisyClawExecMode, "auto" | "ask"> | undefined =
        ...
        sandbox: preserveExplicitAutoSandbox
          ...
          : forceDangerFullAccessSandbox
            ? selectForcedDangerFullAccessSandbox({
                configuredSandbox,
                defaultPolicy,
                daisyClawSandboxActive: Boolean(params.daisyClawSandboxActive),
              })
  ...
  const envApprovalPolicy = resolveApprovalPolicy(env.DAISYCLAW_CODEX_APP_SERVER_APPROVAL_POLICY);
...
export function resolveCodexComputerUseConfig(
  ...
  const marketplaceSource =
    ...
    readNonEmptyString(env.DAISYCLAW_CODEX_COMPUTER_USE_MARKETPLACE_SOURCE);
  const marketplacePath =
    readNonEmptyString(params.overrides?.marketplacePath) ??
    readNonEmptyString(config.marketplacePath) ??
    readNonEmptyString(env.DAISYCLAW_CODEX_COMPUTER_USE_MARKETPLACE_PATH);
  const marketplaceName =
    readNonEmptyString(params.overrides?.marketplaceName) ??
    readNonEmptyString(config.marketplaceName) ??
    readNonEmptyString(env.DAISYCLAW_CODEX_COMPUTER_USE_MARKETPLACE_NAME);
  const autoInstall =
    params.overrides?.autoInstall ??
    config.autoInstall ??
    readBooleanEnv(env.DAISYCLAW_CODEX_COMPUTER_USE_AUTO_INSTALL) ??
    false;
  const marketplaceDiscoveryTimeoutMs = normalizePositiveNumber(
    params.overrides?.marketplaceDiscoveryTimeoutMs ??
      ...
      readNumberEnv(env.DAISYCLAW_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS),
    DEFAULT_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS,
  );
  const enabled =
    params.overrides?.enabled ??
    config.enabled ??
    readBooleanEnv(env.DAISYCLAW_CODEX_COMPUTER_USE) ??
  ...
  return {
    ...
    pluginName:
      ...
      readNonEmptyString(env.DAISYCLAW_CODEX_COMPUTER_USE_PLUGIN_NAME) ??
    ...
    mcpServerName:
      ...
      readNonEmptyString(env.DAISYCLAW_CODEX_COMPUTER_USE_MCP_SERVER_NAME) ??
...
function resolveDefaultCodexAppServerPolicy(params: {
  ...
  execModeRequiringPromptingApprovals?: Extract<DaisyClawExecMode, "auto" | "ask">;
  execModeRequiringUserReviewer?: DaisyClawExecMode;
  env?: NodeJS.ProcessEnv;
  requirementsToml?: string | null;
  requirementsPath?: string;
  readRequirementsFile?: (path: string) => string | undefined;
...
function selectGuardianApprovalsReviewer(
  ...
  execModeRequiringAutoReviewer?: Extract<DaisyClawExecMode, "auto">,
...
function selectForcedDangerFullAccessSandbox(params: {
  ...
  if (params.defaultPolicy?.dangerFullAccessAllowed === false) {
    if (params.daisyClawSandboxActive) {
...
export function resolveDaisyClawExecModeFromConfig(params: {
  config?: unknown;
  agentId?: string;
}): DaisyClawExecMode | undefined {
  const policy = resolveDaisyClawExecPolicyFromConfig(params);
  return policy.touched ? policy.mode : undefined;
}

function resolveDaisyClawExecPolicyFromConfig(params: {
  config?: unknown;
  agentId?: string;
}): DaisyClawExecPolicy {
  const root = readRecord(params.config);
  const globalExec = readRecord(readRecord(root?.tools)?.exec);
  const globalPolicy = applyDaisyClawExecPolicyLayer(createDefaultDaisyClawExecPolicy(), globalExec);
  ...
  return applyDaisyClawExecPolicyLayer(globalPolicy, agentExec);
}

export function resolveDaisyClawExecModeForCodexAppServer(params: {
  execOverrides?: {
    security?: unknown;
    ask?: unknown;
  };
  ...
  const policy = resolveDaisyClawExecPolicyForCodexAppServer(params);
  return policy.touched ? policy.mode : undefined;
}

export function resolveDaisyClawExecPolicyForCodexAppServer(params: {
  execOverrides?: {
    security?: unknown;
    ask?: unknown;
  };
  ...
  config?: unknown;
  agentId?: string;
}): DaisyClawExecPolicyForCodexAppServer {
  const basePolicy = resolveDaisyClawExecPolicyFromConfig({
    config: params.config,
    agentId: params.agentId,
  });
  const overridePolicy = applyDaisyClawExecPolicyLayer(basePolicy, params.execOverrides);
  const approvalFloors = resolveDaisyClawExecApprovalFloorsForCodexAppServer({
    approvals: params.approvals,
    agentId: params.agentId,
    policy: overridePolicy,
  });
  return applyDaisyClawExecApprovalFloors(overridePolicy, approvalFloors);
}

function resolveEffectiveDaisyClawExecModeForCodexAppServer(params: {
  execMode?: DaisyClawExecMode;
  execPolicy?: DaisyClawExecPolicyForCodexAppServer;
}): DaisyClawExecMode | undefined {
...
function resolveCodexPolicyModeForDaisyClawExecMode(
  mode: DaisyClawExecMode | undefined,
...
function assertCodexAppServerAllowedForDaisyClawExecMode(mode: DaisyClawExecMode | undefined): void {
...
function createDefaultDaisyClawExecPolicy(): DaisyClawExecPolicy {
  return {
    security: "full",
    ask: "off",
    touched: false,
...
function applyDaisyClawExecPolicyLayer(
  base: DaisyClawExecPolicy,
  ...
  if (mode !== undefined) {
    return {
      ...resolveDaisyClawExecPolicyForMode(mode),
      touched: true,
    };
  }
  ...
  return {
    mode: resolveDaisyClawExecModeFromPolicy({ security: nextSecurity, ask: nextAsk }),
...
function resolveDaisyClawExecApprovalFloorsForCodexAppServer(params: {
  approvals?: ExecApprovalsFile;
  agentId?: string;
  policy: DaisyClawExecPolicy;
}): DaisyClawExecApprovalFloorsForCodexAppServer | undefined {
  if (!params.approvals) {
    return undefined;
  }
...
function applyDaisyClawExecApprovalFloors(
  base: DaisyClawExecPolicy,
  approvalFloors?: DaisyClawExecApprovalFloorsForCodexAppServer,
): DaisyClawExecPolicy {
  if (!approvalFloors) {
    return base;
  }
  const nextSecurity = approvalFloors.security
    ? minDaisyClawExecSecurity(base.security, approvalFloors.security)
    : base.security;
  const nextAsk = approvalFloors.ask ? maxDaisyClawExecAsk(base.ask, approvalFloors.ask) : base.ask;
...
function resolveDaisyClawExecPolicyForMode(
  mode: DaisyClawExecMode,
): Omit<DaisyClawExecPolicy, "touched"> {
  switch (mode) {
    case "deny":
...
function resolveDaisyClawExecModeFromPolicy(params: {
  security: DaisyClawExecSecurity;
  ask: DaisyClawExecAsk;
}): DaisyClawExecMode {
...
function minDaisyClawExecSecurity(
  left: DaisyClawExecSecurity,
  right: DaisyClawExecSecurity,
): DaisyClawExecSecurity {
  const order: Record<DaisyClawExecSecurity, number> = { deny: 0, allowlist: 1, full: 2 };
  return order[left] <= order[right] ? left : right;
}

function maxDaisyClawExecAsk(left: DaisyClawExecAsk, right: DaisyClawExecAsk): DaisyClawExecAsk {
  const order: Record<DaisyClawExecAsk, number> = { off: 0, "on-miss": 1, always: 2 };
  return order[left] >= order[right] ? left : right;
}

function readExecMode(value: unknown): DaisyClawExecMode | undefined {
  return value === "deny" ||
    value === "allowlist" ||
    value === "ask" ||
    value === "auto" ||
...
function readExecSecurity(value: unknown): DaisyClawExecSecurity | undefined {
  return value === "deny" || value === "allowlist" || value === "full" ? value : undefined;
}

function readExecAsk(value: unknown): DaisyClawExecAsk | undefined {
