...
import { isSilentReplyPayloadText } from "daisyclaw/plugin-sdk/reply-chunking";
...
async function createParams(): Promise<EmbeddedRunAttemptParams> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-codex-delivery-contract-"));
