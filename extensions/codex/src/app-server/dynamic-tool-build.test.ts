...
describe("Codex app-server dynamic tool build", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-codex-tools-"));
  });
  ...
  it("quarantines unreadable tool entries before Codex-specific filtering", async () => {
    ...
    setDaisyClawCodingToolsFactoryForTests(() => sourceTools);
  ...
  it("quarantines non-object plugin schemas before Codex-specific filtering", async () => {
    ...
    setDaisyClawCodingToolsFactoryForTests(() => [brokenTool, messageTool]);
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const params = createParams(sessionFile, workspaceDir);
    params.disableTools = false;
  ...
  it("limits Codex memory flush runs to managed read and write tools", async () => {
    ...
    setDaisyClawCodingToolsFactoryForTests((options) => {
  ...
  it("exposes DaisyClaw sandbox shell tools under distinct names for non-Docker sandbox backends", async () => {
    setDaisyClawCodingToolsFactoryForTests(() => [
      createRuntimeDynamicTool("read"),
      createRuntimeDynamicTool("write"),
  ...
  it("exposes Docker sandbox shell tools when DaisyClaw sandboxing disables native Code Mode", async () => {
    setDaisyClawCodingToolsFactoryForTests(() => [
      createRuntimeDynamicTool("exec"),
      createRuntimeDynamicTool("process"),
      createRuntimeDynamicTool("message"),
  ...
  it("keeps DaisyClaw shell tools for node-targeted Codex app-server runs", async () => {
    setDaisyClawCodingToolsFactoryForTests(() => [
      createRuntimeDynamicTool("exec"),
      createRuntimeDynamicTool("process"),
      createRuntimeDynamicTool("message"),
  ...
  it("exposes Docker sandbox shell tools when native Code Mode cannot honor sandbox paths", async () => {
    ...
    const tools = await buildDynamicToolsForTest(params, workspaceDir, {
      sandbox: {
        enabled: true,
        backendId: "docker",
        docker: { binds: ["/tmp/daisyclaw-data:/data:rw"] },
      } as never,
      nativeToolSurfaceEnabled: false,
    });
  ...
  it("enables gateway subagent binding for forced private QA Codex runs", async () => {
    vi.stubEnv("DAISYCLAW_BUILD_PRIVATE_QA", "1");
    vi.stubEnv("DAISYCLAW_QA_FORCE_RUNTIME", "codex");
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const params = createParams(sessionFile, workspaceDir);
    params.disableTools = false;
  ...
  it("disables Codex native tool surfaces whenever an DaisyClaw sandbox is active", () => {
    const workspaceDir = path.join(tempDir, "workspace");
    const params = createParams(path.join(tempDir, "session.jsonl"), workspaceDir);
    params.disableTools = false;
    ...
    expect(
      shouldEnableCodexAppServerNativeToolSurface(params, {
        enabled: true,
        backendId: "docker",
        docker: { binds: ["/tmp/daisyclaw-data:/tmp/daisyclaw-data:rw"] },
      } as never),
    ).toBe(false);
    ...
    expect(
      shouldEnableCodexAppServerNativeToolSurface(params, {
        ...
        docker: {
          binds: [
            "/tmp/daisyclaw-data:/tmp/daisyclaw-data:rw",
            "/tmp/daisyclaw-data/secrets:/tmp/daisyclaw-data/secrets:ro",
          ],
        },
      } as never),
    ).toBe(false);
  ...
  it("passes the live run session key to Codex dynamic tools when sandbox policy uses another key", () => {
    ...
    expect(
      resolveDaisyClawCodingToolsSessionKeys(params, "agent:main:telegram:default:direct:1234"),
    ).toEqual({
      sessionKey: "agent:main:telegram:default:direct:1234",
      runSessionKey: "agent:main:main",
    });

    expect(resolveDaisyClawCodingToolsSessionKeys(params, "agent:main:main")).toEqual({
