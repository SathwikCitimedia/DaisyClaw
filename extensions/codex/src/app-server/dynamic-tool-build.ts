 ...
 * DaisyClaw-owned tools, Codex native-tool fallback rules, sandbox shell shims,
...
import { resolveAgentDir } from "daisyclaw/plugin-sdk/agent-runtime";
import { isToolAllowed } from "daisyclaw/plugin-sdk/sandbox";
...
type DaisyClawCodingToolsOptions = NonNullable<
  Parameters<(typeof import("daisyclaw/plugin-sdk/agent-harness"))["createDaisyClawCodingTools"]>[0]
>;

/** Factory seam for constructing DaisyClaw runtime tools without eagerly loading agent-harness. */
export type DaisyClawCodingToolsFactory =
  (typeof import("daisyclaw/plugin-sdk/agent-harness"))["createDaisyClawCodingTools"];
type DaisyClawDynamicTool = ReturnType<DaisyClawCodingToolsFactory>[number];
type DaisyClawSandboxContext = Awaited<ReturnType<typeof resolveSandboxContext>>;
...
export type DynamicToolBuildParams = {
  ...
  sandbox: DaisyClawSandboxContext;
...
let daisyClawCodingToolsFactoryForTests: DaisyClawCodingToolsFactory | undefined;

/** Overrides the runtime tool factory for tests that need deterministic tool catalogs. */
export function setDaisyClawCodingToolsFactoryForTests(factory: DaisyClawCodingToolsFactory): void {
  daisyClawCodingToolsFactoryForTests = factory;
}

/** Clears the test-only runtime tool factory override. */
export function resetDaisyClawCodingToolsFactoryForTests(): void {
  daisyClawCodingToolsFactoryForTests = undefined;
}

/** Splits sandbox and run session keys so tool calls can bind to both scopes when needed. */
export function resolveDaisyClawCodingToolsSessionKeys(
  params: EmbeddedRunAttemptParams,
  sandboxSessionKey: string,
): Pick<DaisyClawCodingToolsOptions, "sessionKey" | "runSessionKey"> {
  return {
...
export async function buildDynamicTools(input: DynamicToolBuildParams) {
  ...
  const createDaisyClawCodingTools =
    daisyClawCodingToolsFactoryForTests ??
    (await import("daisyclaw/plugin-sdk/agent-harness")).createDaisyClawCodingTools;
  toolBuildStages.mark("load-agent-harness-tools");
  const sessionKeys = resolveDaisyClawCodingToolsSessionKeys(params, input.sandboxSessionKey);
  const allTools = createDaisyClawCodingTools({
    ...
    modelCompat:
      params.model.compat && typeof params.model.compat === "object"
        ? (params.model.compat as DaisyClawCodingToolsOptions["modelCompat"])
        : undefined,
  ...
  toolBuildStages.mark("create-daisyclaw-coding-tools");
...
export function shouldEnableCodexAppServerNativeToolSurface(
  ...
  // capability, so narrow DaisyClaw allowlists must fail closed rather than
...
/** Returns true when DaisyClaw policy requires the Node-owned exec/process tools instead. */
...
function canCodexAppServerNativeToolSurfaceHonorSandbox(
  sandbox: DaisyClawSandboxContext | undefined,
  ...
  // active DaisyClaw sandboxing must disable the native surface and route shell
...
/** Requires a Codex sandbox environment only when native tools must run inside DaisyClaw sandboxing. */
...
/** Converts DaisyClaw sandbox networking into Codex's external-sandbox policy shape. */
export function resolveCodexExternalSandboxPolicyForDaisyClawSandbox(
  sandbox: DaisyClawSandboxContext | undefined,
): CodexSandboxPolicy {
  return {
    type: "externalSandbox",
    networkAccess: codexNetworkAccessForDaisyClawSandbox(sandbox) ? "enabled" : "restricted",
  };
}

function codexNetworkAccessForDaisyClawSandbox(
  sandbox: DaisyClawSandboxContext | undefined,
): boolean {
...
export function addSandboxShellDynamicToolsIfAvailable(
  filteredTools: DaisyClawDynamicTool[],
  allTools: DaisyClawDynamicTool[],
  input: DynamicToolBuildParams,
): DaisyClawDynamicTool[] {
  if (
  ...
  const sandboxExecTool: DaisyClawDynamicTool = {
    ...execTool,
    name: "sandbox_exec",
    description:
      "Run a shell command through DaisyClaw's configured sandbox backend for this session. Use when DaisyClaw sandboxing is active or when a command must execute in the sandbox backend, such as an SSH-backed sandbox or Docker container-path bind layout. Use Codex's native shell only when no DaisyClaw sandbox is active and native Code Mode is available.",
  ...
  const sandboxProcessTool: DaisyClawDynamicTool = {
    ...processTool,
    name: "sandbox_process",
    description:
      "Manage sandbox_exec sessions that were started through DaisyClaw's configured sandbox backend for this session: list, poll, log, write, send-keys, submit, paste, kill, clear, or remove. Use only for sandbox_exec follow-up; use Codex's native shell session handling only when no DaisyClaw sandbox is active and native Code Mode is available.",
  };
