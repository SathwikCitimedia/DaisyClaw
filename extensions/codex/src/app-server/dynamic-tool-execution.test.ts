  ...
  it("returns a failed dynamic tool response when an app-server tool call exceeds the deadline", async () => {
    ...
    await expect(response).resolves.toEqual({
      ...
      contentItems: [
        {
          type: "inputText",
          text: "DaisyClaw dynamic tool call timed out after 1ms while running tool message.",
        },
      ],
    });
  ...
  it("logs process poll timeout context separately from session idle", async () => {
    ...
    await expect(response).resolves.toEqual({
      ...
      contentItems: [
        {
          type: "inputText",
          text: "DaisyClaw dynamic tool call timed out after 1ms while waiting for process action=poll sessionId=process-session. This is a tool RPC timeout, not a session idle timeout.",
        },
      ],
    });
  ...
  it("allows turn release after successful terminal dynamic tool responses", () => {
    expect(
      shouldReleaseTurnAfterTerminalDynamicTool({
        ...
        pendingDaisyClawDynamicToolCompletionIdsCount: 0,
      }),
    ).toBe(true);
    expect(
      shouldReleaseTurnAfterTerminalDynamicTool({
    ...
    expect(
      shouldReleaseTurnAfterTerminalDynamicTool({
        ...
        activeAppServerTurnRequests: 0,
        activeTurnItemIdsCount: 0,
        pendingDaisyClawDynamicToolCompletionIdsCount: 1,
      }),
    ).toBe(false);
  });
