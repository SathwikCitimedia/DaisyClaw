...
import { parseStrictNonNegativeInteger } from "daisyclaw/plugin-sdk/number-runtime";
...
function formatDynamicToolTimeoutDetails(params: {
  ...
  if (tool !== "process" || !isJsonObject(params.call.arguments)) {
    return {
      responseMessage: `DaisyClaw dynamic tool call timed out after ${params.timeoutMs}ms while running tool ${tool}.`,
  ...
  return {
    responseMessage: `DaisyClaw dynamic tool call timed out after ${params.timeoutMs}ms${responseTarget}. This is a tool RPC timeout, not a session idle timeout.`,
...
export async function handleDynamicToolCallWithTimeout(params: {
  ...
  if (params.signal.aborted) {
    return failedDynamicToolResponse("DaisyClaw dynamic tool call aborted before execution.");
  }
  ...
  const abortFromRun = () => {
    const message = "DaisyClaw dynamic tool call aborted.";
  ...
  try {
    ...
    if (!timedOut && !controller.signal.aborted) {
      controller.abort(new Error("DaisyClaw dynamic tool call finished."));
    }
  }
}
...
/** Strips DaisyClaw-only metadata before sending a dynamic tool response to Codex. */
...
type TerminalDynamicToolReleaseState = {
  ...
  pendingDaisyClawDynamicToolCompletionIdsCount: number;
};
...
export function shouldReleaseTurnAfterTerminalDynamicTool(
  ...
  return (
    ...
    state.pendingDaisyClawDynamicToolCompletionIdsCount === 0
  );
}
