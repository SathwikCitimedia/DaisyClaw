...
/** Tool names owned by Codex app-server and normally excluded from DaisyClaw dynamic tools. */
...
type CodexDynamicToolProfileEnv = {
  DAISYCLAW_BUILD_PRIVATE_QA?: string;
  DAISYCLAW_QA_FORCE_RUNTIME?: string;
};

/** Normalizes DaisyClaw/Codex tool names before filtering and allowlist checks. */
...
export function isForcedPrivateQaCodexRuntime(
  ...
  return (
    ...
    env.DAISYCLAW_QA_FORCE_RUNTIME?.trim().toLowerCase() === "codex"
  );
}
...
/** Filters DaisyClaw tools that Codex owns natively or config explicitly excludes. */
