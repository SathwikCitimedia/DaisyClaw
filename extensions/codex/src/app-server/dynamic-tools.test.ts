...
import type { AnyAgentTool } from "daisyclaw/plugin-sdk/agent-harness";
import {
...
describe("createCodexDynamicToolBridge", () => {
  it("keeps turn-yield direct while deferring DaisyClaw session spawn", () => {
  ...
  it("can register a durable tool schema while denying execution for the current turn", async () => {
    ...
    expect(result).toEqual({
      ...
      contentItems: [
        {
          type: "inputText",
          text: `DaisyClaw tool is not available for this turn: ${HEARTBEAT_RESPONSE_TOOL_NAME}`,
        },
      ],
    });
  ...
  it("quarantines dynamic tools with unsupported input schemas", async () => {
    ...
    expect(result).toEqual({
      success: false,
      contentItems: [{ type: "inputText", text: "Unknown DaisyClaw tool: fuzzplugin_move_angles" }],
    });
  ...
  it("truncates configured text tool results before returning them to Codex", async () => {
    ...
    expect(text).toContain("DaisyClaw truncated dynamic tool result");
  ...
  it("honors normalized per-agent dynamic tool result caps", async () => {
    ...
    expect(firstItem.text).toContain("DaisyClaw truncated dynamic tool result");
  });
  ...
  it("keeps truncation notices within tiny configured caps", async () => {
    ...
    expect(firstItem.text).toBe("...(DaisyClaw truncated dynamic tool".slice(0, 32));
  });
  ...
  it("keeps config out of Codex tool-result contexts", async () => {
    const config = { session: { store: "/tmp/daisyclaw-session-store.json" } };
    const registry = createEmptyPluginRegistry();
