/**
 * Bridges DaisyClaw runtime tools into Codex app-server dynamic tool specs and
 * tool-call responses.
 */
...
import { emitTrustedDiagnosticEvent } from "daisyclaw/plugin-sdk/diagnostic-runtime";
import type { ImageContent, TextContent } from "daisyclaw/plugin-sdk/llm";
...
/** Namespace attached to DaisyClaw-owned dynamic tools exposed to Codex. */
export const CODEX_DAISYCLAW_DYNAMIC_TOOL_NAMESPACE = "daisyclaw";

// Keep DaisyClaw session spawning searchable in Codex mode so Codex's native
...
/**
 * Creates dynamic tool specs and a call handler that executes DaisyClaw tools,
  ...
  return {
    ...
    handleToolCall: async (call, options) => {
      ...
      if (!toolEntry) {
        if (registeredToolNames.has(call.tool)) {
          return {
            contentItems: [
              {
                type: "inputText",
                text: `DaisyClaw tool is not available for this turn: ${call.tool}`,
              },
            ],
            success: false,
          };
        ...
        return {
          contentItems: [{ type: "inputText", text: `Unknown DaisyClaw tool: ${call.tool}` }],
          success: false,
        };
      }
...
function createCodexDynamicToolSpec(params: {
  ...
  return {
    ...base,
    namespace: CODEX_DAISYCLAW_DYNAMIC_TOOL_NAMESPACE,
    deferLoading: true,
  };
}
...
function convertToolContents(
  ...
  const noticeText = `...(DaisyClaw truncated dynamic tool result: original ${totalTextChars} chars, showing ${maxChars}; rerun with narrower args.)`;
  const notice = `\n${noticeText}`;
