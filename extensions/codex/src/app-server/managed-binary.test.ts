...
describe("managed Codex app-server binary", () => {
  ...
  it("resolves the plugin-local bundled Codex binary", async () => {
    const pluginRoot = path.join("/tmp", "daisyclaw", "extensions", "codex");
  ...
  it("resolves Windows Codex command shims", () => {
    const pluginRoot = path.win32.join("C:\\", "DaisyClaw", "dist", "extensions", "codex");
  ...
  it("uses the package root when the resolver is bundled into a dist chunk", () => {
    expect(testing.resolveDefaultCodexPluginRoot("/repo/daisyclaw/dist")).toBe("/repo/daisyclaw");
    expect(testing.resolveDefaultCodexPluginRoot("/repo/daisyclaw/dist-runtime")).toBe(
      "/repo/daisyclaw",
    );
    expect(
      testing.resolveDefaultCodexPluginRoot("/repo/daisyclaw/extensions/codex/src/app-server"),
    ).toBe("/repo/daisyclaw/extensions/codex");
  });

  it("finds Codex in the package install root used by packaged plugins", async () => {
    const installRoot = path.join("/tmp", "daisyclaw-plugin-package", "codex");
  ...
  it("finds Codex bins hoisted into an isolated npm project root", async () => {
    const projectRoot = path.join("/tmp", "state", "npm", "projects", "daisyclaw-codex-hash");
    const pluginRoot = path.join(projectRoot, "node_modules", "@daisyclaw", "codex");
  ...
  it("finds Windows Codex shims hoisted into an isolated npm project root", async () => {
    const projectRoot = path.win32.join(
      ...
      "npm",
      "projects",
      "daisyclaw-codex-hash",
    );
    const pluginRoot = path.win32.join(projectRoot, "node_modules", "@daisyclaw", "codex");
  ...
  it("falls back to the resolved Codex package bin when no command shim exists", async () => {
    const installRoot = await mkdtemp(path.join(os.tmpdir(), "daisyclaw-codex-package-"));
    const pluginRoot = path.join(installRoot, "dist", "extensions", "codex");
  ...
  it("fails clearly when the managed Codex binary is missing", async () => {
    await expect(
      resolveManagedCodexAppServerStartOptions(startOptions("managed"), {
        platform: "darwin",
        pluginRoot: path.join("/tmp", "daisyclaw", "extensions", "codex"),
