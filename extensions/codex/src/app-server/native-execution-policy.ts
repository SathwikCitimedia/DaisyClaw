 ...
 * or whether DaisyClaw must keep exec/process on a configured node host.
 */
import type { DaisyClawConfig } from "daisyclaw/plugin-sdk/config-contracts";
import { resolveSandboxRuntimeStatus } from "daisyclaw/plugin-sdk/sandbox";
import { getSessionEntry, type SessionEntry } from "daisyclaw/plugin-sdk/session-store-runtime";
...
type AgentEntry = NonNullable<NonNullable<DaisyClawConfig["agents"]>["list"]>[number];
...
export function resolveCodexNativeExecutionPolicy(params: {
  ...
  return {
    ...
    node,
    blockReason:
      "DaisyClaw exec host=node is active for this session. Codex app-server native execution cannot route shell, filesystem, MCP, or app-backed work through the selected DaisyClaw node.",
  };
}
...
export function formatCodexNativeNodeExecBlock(params: {
  ...
  return [
    `Codex-native ${params.surface} is unavailable because DaisyClaw exec host=node is active for this session.`,
    params.reason ??
      "Codex app-server native execution cannot route execution through the selected DaisyClaw node.",
    "Use a normal Codex harness turn so DaisyClaw exec/process tools run on the node, or switch exec host to gateway for native Codex app-server execution.",
  ].join(" ");
}
...
function listAgentEntries(config: DaisyClawConfig): AgentEntry[] {
