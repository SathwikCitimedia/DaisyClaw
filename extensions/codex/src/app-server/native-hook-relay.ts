/**
 * Bridges Codex native hook callbacks into DaisyClaw's native hook relay so
 * app-server tool events can still run DaisyClaw policy and diagnostics.
 */
...
/** Codex hook events that can be registered through DaisyClaw's native relay. */
...
/** Registers an DaisyClaw native hook relay for a Codex app-server turn. */
...
export function resolveCodexNativeHookRelayEvents(params: {
  ...
  // pre-guardian DaisyClaw plugin approval prompt.
...
export function buildCodexNativeHookRelayId(params: {
  ...
  hash.update("daisyclaw:codex:native-hook-relay:v1");
  hash.update("\0");
