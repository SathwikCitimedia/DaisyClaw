...
import { classifyEmbeddedAgentRunResultForModelFallback } from "daisyclaw/plugin-sdk/agent-harness-runtime";
import {
  createContractRunResult,
  OUTCOME_FALLBACK_RUNTIME_CONTRACT,
} from "daisyclaw/plugin-sdk/agent-runtime-test-contracts";
...
type MirrorTaggedMessage = { __daisyclaw?: { mirrorIdentity?: string } };

async function createParams(): Promise<EmbeddedRunAttemptParams> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-codex-outcome-contract-"));
...
function readMirrorIdentity(message: unknown): string | undefined {
  const meta = (message as MirrorTaggedMessage | undefined)?.["__daisyclaw"];
...
describe("Outcome/fallback runtime contract - Codex app-server adapter", () => {
  it("preserves an empty terminal turn for DaisyClaw-owned fallback classification", async () => {
  ...
  it("preserves reasoning-only terminal turns for DaisyClaw-owned fallback classification", async () => {
    const projector = await createProjector();
    await projector.handleNotification(
  ...
  it("preserves planning-only terminal turns for DaisyClaw-owned fallback classification", async () => {
    const projector = await createProjector();
    await projector.handleNotification(
