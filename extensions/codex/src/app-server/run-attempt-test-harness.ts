...
import { resetDiagnosticEventsForTest } from "daisyclaw/plugin-sdk/diagnostic-runtime";
import { clearInternalHooks, resetGlobalHookRunner } from "daisyclaw/plugin-sdk/hook-runtime";
import { clearPluginCommands } from "daisyclaw/plugin-sdk/plugin-runtime";
...
export function setupRunAttemptTestHooks(): void {
  beforeEach(async () => {
    ...
    vi.stubEnv("DAISYCLAW_TRAJECTORY", "0");
    vi.stubEnv("CODEX_API_KEY", "");
    vi.stubEnv("OPENAI_API_KEY", "");
    tempDir = await fs.mkdtemp(path.join(resolvePreferredDaisyClawTmpDir(), "daisyclaw-codex-run-"));
  });
  ...
  afterEach(async () => {
    ...
    testing.resetDaisyClawCodingToolsFactoryForTests();
