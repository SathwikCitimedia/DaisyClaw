...
import type { EmbeddedRunAttemptParams } from "daisyclaw/plugin-sdk/agent-harness";
import {
  embeddedAgentLog,
  type HarnessContextEngine as ContextEngine,
} from "daisyclaw/plugin-sdk/agent-harness-runtime";
import { SessionManager } from "daisyclaw/plugin-sdk/agent-sessions";
import { registerSandboxBackend } from "daisyclaw/plugin-sdk/sandbox";
...
function threadStartResult(threadId = "thread-1") {
  return {
    thread: {
      ...
      status: { type: "idle" },
      path: null,
      cwd: tempDir || "/tmp/daisyclaw-codex-test",
      cliVersion: "0.125.0",
      source: "unknown",
      agentNickname: null,
      agentRole: null,
...
describe("runCodexAppServerAttempt context-engine lifecycle", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-codex-context-engine-"));
  });
  ...
  it("bootstraps and assembles non-legacy context before the Codex turn starts", async () => {
    ...
    expectRequestInputTextContains(harness, "DaisyClaw assembled context for this turn:");
  ...
  it("projects thread-bootstrap context only once for a matching context-engine epoch", async () => {
    ...
    expectRequestInputTextContains(firstHarness, "DaisyClaw assembled context for this turn:");
  ...
  it("fails first-turn Codex context overflow instead of falling back to DaisyClaw compaction", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const compact = vi.fn<ContextEngine["compact"]>(async () => ({
      ok: true,
  ...
  it("keeps current inbound context at the front of the Codex context-engine prompt", async () => {
    ...
    params.currentInboundContext = {
      text: [
        ...
        "#6474 Sun 2026-05-10 22:22 GMT+5:30 [reply target] DaisyClaw: anchor REPLYCTX this is the old message",
        "#6498 Sun 2026-05-10 22:22 GMT+5:30 DaisyClaw: filler REPLYCTX 23",
      ].join("\n"),
    };
    ...
    expect(inputText).toContain("DaisyClaw assembled context for this turn:");
    expect(inputText).toContain("Current user request:\nhello");
    expect(inputText).toContain("[reply target] DaisyClaw: anchor REPLYCTX");
