...
import { onAgentEvent, type AgentEventPayload } from "daisyclaw/plugin-sdk/agent-harness-runtime";
import {
...
import { initializeGlobalHookRunner } from "daisyclaw/plugin-sdk/hook-runtime";
...
describe("runCodexAppServerAttempt dynamic tools", () => {
  it("passes the live run session key to Codex dynamic tools when sandbox policy uses another key", () => {
    ...
    expect(
      testing.resolveDaisyClawCodingToolsSessionKeys(
        params,
    ...
    expect(testing.resolveDaisyClawCodingToolsSessionKeys(params, "agent:main:main")).toEqual({
  ...
  it("emits normalized tool progress around app-server dynamic tool requests", async () => {
    ...
    expect(toolResult.contentItems?.[0]?.text).toMatch(/^Unknown DaisyClaw tool: lookup$/u);
    ...
    expect(resultEvent?.data?.result?.content?.[0]?.text).toBe("Unknown DaisyClaw tool: lookup");
      ...
      emitDynamicToolTerminalDiagnostic({
        ...
        response: {
          ...
          contentItems: [
            {
              type: "inputText",
              text: "DaisyClaw dynamic tool call timed out after 1ms while running tool echo.",
            },
          ],
        },
      });
