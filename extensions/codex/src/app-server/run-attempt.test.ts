...
import { SessionManager } from "daisyclaw/plugin-sdk/agent-sessions";
import {
  onInternalDiagnosticEvent,
  waitForDiagnosticEventsDrained,
  type DiagnosticEventPayload,
} from "daisyclaw/plugin-sdk/diagnostic-runtime";
import { initializeGlobalHookRunner, registerInternalHook } from "daisyclaw/plugin-sdk/hook-runtime";
import { registerPluginCommand } from "daisyclaw/plugin-sdk/plugin-runtime";
import { createMockPluginRegistry } from "daisyclaw/plugin-sdk/plugin-test-runtime";
import { describe, expect, it, vi } from "vitest";
import WebSocket from "ws";
import { CODEX_GPT5_BEHAVIOR_CONTRACT } from "../../prompt-overlay.js";
import { defaultCodexAppInventoryCache } from "./app-inventory-cache.js";
import {
  buildCodexDaisyClawPromptContext,
  buildCodexSystemPromptReport,
  buildCodexWorkspaceBootstrapContext,
  getCodexWorkspaceMemoryToolNames,
  prependCodexDaisyClawPromptContext,
} from "./attempt-context.js";
import { resolveCodexAppServerEnvApiKeyCacheKey } from "./auth-bridge.js";
import { CodexAppServerRpcError } from "./client.js";
import { readCodexPluginConfig, resolveCodexAppServerRuntimeOptions } from "./config.js";
import {
  CODEX_DAISYCLAW_DYNAMIC_TOOL_NAMESPACE,
...
async function buildCodexTurnContextForTest(
  ...
  const threadDeveloperInstructions = [
    ...
    .join("\n\n");
  const daisyClawPromptContext = buildCodexDaisyClawPromptContext({
    params,
    workspacePromptContext: workspaceBootstrapContext.promptContext,
  });
  const codexTurnPromptText = prependCodexDaisyClawPromptContext(
    params.prompt,
    daisyClawPromptContext,
  );
...
describe("runCodexAppServerAttempt", () => {
  ...
  it("starts active DaisyClaw sandbox threads with Codex native execution disabled", async () => {
    testing.setDaisyClawCodingToolsFactoryForTests(() => [
  ...
  it("routes native Codex execution through an DaisyClaw sandbox exec-server when opted in", async () => {
    const appServer = {
    ...
    try {
      ...
      expect(environmentAddParams?.environmentId).toMatch(/^daisyclaw-sandbox-/);
  ...
  it("starts Codex threads without duplicate DaisyClaw workspace tools by default", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
  ...
  it("includes Codex app-server scoped plugin command guidance in developer instructions", () => {
    registerPluginCommand("demo-plugin", {
      ...
      agentPromptGuidance: [
        ...
        {
          text: "DaisyClaw main command guidance.",
    ...
    expect(instructions).not.toContain("DaisyClaw main command guidance.");
  });

  it("passes DaisyClaw skills as turn collaboration developer instructions", async () => {
    const llmInput = vi.fn();
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "llm_input", handler: llmInput }]),
    );
    vi.stubEnv("DAISYCLAW_TRAJECTORY", "1");
    vi.stubEnv("DAISYCLAW_TRAJECTORY_DIR", path.join(tempDir, "trajectory"));
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    ...
    const collaborationInstructions =
      turnStartParams.collaborationMode?.settings?.developer_instructions ?? "";
    expect(collaborationInstructions).toContain("## DaisyClaw Skills");
    expect(collaborationInstructions).toContain("<available_skills>");
    const inputText = turnStartParams.input?.[0]?.text ?? "";
    expect(inputText).not.toContain("## DaisyClaw Skills");
    ...
    expect(compiledContext?.data?.systemPrompt).toContain("## DaisyClaw Skills");
  ...
  it("emits TUI-compatible tool events for Codex dynamic tool calls", async () => {
    ...
    await expect(
      ...
      success: false,
      contentItems: [{ type: "inputText", text: "Unknown DaisyClaw tool: python" }],
    });
    await harness.completeTurn({ threadId: "thread-1", turnId: "turn-1" });
    await run;
    ...
    expect(onRunAgentEvent).toHaveBeenCalledWith({
      ...
      data: {
        ...
        isError: true,
        result: {
          content: [{ type: "text", text: "Unknown DaisyClaw tool: python" }],
        },
      },
    });
  ...
  it("keeps leading delivery hints out of the Codex current user request", async () => {
    ...
    const inputText = turnStartParams.input?.[0]?.text ?? "";
    expect(inputText).toContain("DaisyClaw delivery metadata:");
  ...
  it("keeps searchable DaisyClaw dynamic tools when code-mode-only is enabled", () => {
    const tools = [
      createRuntimeDynamicTool("message"),
    ...
    expect(webSearch?.namespace).toBe(CODEX_DAISYCLAW_DYNAMIC_TOOL_NAMESPACE);
    expect(webSearch?.deferLoading).toBe(true);
    expect(heartbeat?.namespace).toBe(CODEX_DAISYCLAW_DYNAMIC_TOOL_NAMESPACE);
    expect(heartbeat?.deferLoading).toBe(true);
    expect(sessionsSpawn?.namespace).toBe(CODEX_DAISYCLAW_DYNAMIC_TOOL_NAMESPACE);
  ...
  it("registers heartbeat response durably without advertising it on normal turns", async () => {
    testing.setDaisyClawCodingToolsFactoryForTests((options) => [
      createRuntimeDynamicTool("message"),
    ...
    expect(normalInstructions).toContain(
      "Deferred searchable DaisyClaw dynamic tools available: message.",
    );
    expect(normalInstructions).not.toContain(
      "Deferred searchable DaisyClaw dynamic tools available: heartbeat_respond",
    );
  ...
  it("fails closed for Codex app defaults when restricted native tools have no plugin config", async () => {
    testing.setDaisyClawCodingToolsFactoryForTests(() => [createRuntimeDynamicTool("message")]);
    const params = createParams(
      path.join(tempDir, "session.jsonl"),
      path.join(tempDir, "workspace"),
    );
  ...
  it("applies before_prompt_build to Codex developer instructions and turn input", async () => {
    ...
    const wrappedPluginSystemContext = (text: string) =>
      `---\n\nDaisyClaw plugin-injected system context. This block is not workspace file content.\n\n${text}\n\n---`;
  ...
  it("projects bounded continuity when starting Codex without a native thread binding", async () => {
    ...
    const harness = createStartedThreadHarness();
    const params = createParams(sessionFile, workspaceDir);
    params.prompt = "make the default webpage daisyclaw";
    ...
    expect(inputText).toContain("DaisyClaw assembled context for this turn:");
    expect(inputText).toContain("we are fixing the Opik default project");
    expect(inputText).toContain("Opik default project context");
    expect(inputText).toContain("Current user request:");
    expect(inputText).toContain("make the default webpage daisyclaw");
  });
  ...
  it("does not replay mirrored history already covered by an existing Codex binding", async () => {
    ...
    expect(inputText).not.toContain("DaisyClaw assembled context for this turn:");
  ...
  it("projects only newer visible history when a resumed Codex binding is stale", async () => {
    ...
    const copilotMirrorMessage = {
      ...
      __daisyclaw: { mirrorIdentity: "copilot:assistant-1" },
    } as ReturnType<typeof assistantMessage> & { __daisyclaw: { mirrorIdentity: string } };
  ...
  it("does not project Codex mirrored transcript echoes as stale binding continuity", async () => {
    ...
    const codexMirrorAssistantMessage = {
      ...
      __daisyclaw: { mirrorIdentity: "codex-app-server:assistant-1" },
    } as ReturnType<typeof assistantMessage> & { __daisyclaw: { mirrorIdentity: string } };
  ...
  it("does not project mirrored messages on consecutive resumes", async () => {
    ...
    const firstInputText =
      (firstTurnStart?.params as { input?: Array<{ text?: string }> } | undefined)?.input?.[0]
        ?.text ?? "";
    expect(firstInputText).toContain("DaisyClaw assembled context for this turn:");
    ...
    const secondInputText =
      (secondTurnStart?.params as { input?: Array<{ text?: string }> } | undefined)?.input?.[0]
        ?.text ?? "";
    expect(secondInputText).not.toContain("DaisyClaw assembled context for this turn:");
  ...
  it("passes stable workspace files as Codex developer instructions and routes MEMORY.md through tools", async () => {
    ...
    expect(threadDeveloperInstructions).toContain("DaisyClaw Workspace Instructions");
    ...
    expect(collaborationInstructions).toContain("DaisyClaw Agent Soul");
    ...
    expect(collaborationInstructions).toContain("DaisyClaw Workspace Memory");
    ...
    expect(inputText).not.toContain("DaisyClaw runtime context for this turn:");
    ...
    expect(inputText).not.toContain("DaisyClaw Workspace Memory");
  ...
  it("sends workspace bootstrap instructions through Codex app-server payloads", async () => {
    ...
    expect(threadStartParams.developerInstructions).toContain("DaisyClaw Workspace Instructions");
  ...
  it("routes MEMORY.md through memory_get when search is unavailable", async () => {
    ...
    await fs.mkdir(workspaceDir, { recursive: true });
    await fs.writeFile(path.join(workspaceDir, "MEMORY.md"), memorySummary);
    testing.setDaisyClawCodingToolsFactoryForTests(() => [createRuntimeDynamicTool("memory_get")]);
    const params = createParams(sessionFile, workspaceDir);
    params.disableTools = false;
    params.runtimePlan = createCodexRuntimePlanFixture();
    setAgentWorkspaceForTest(params, workspaceDir);
  ...
  it("omits heartbeat Codex workspace pointers for empty HEARTBEAT.md files", async () => {
    ...
    expect(collaborationInstructions).toContain("This is an DaisyClaw heartbeat turn");
    expect(collaborationInstructions).not.toContain("HEARTBEAT.md exists");
  });

  it("keeps lightweight cron Codex turns out of DaisyClaw bootstrap context", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const exactCommand =
      "cd /Users/phaedrus/Projects/daisyclaw && /Users/phaedrus/clawd/scripts/clawsweeper-related-scan.py";
    await fs.mkdir(workspaceDir, { recursive: true });
  ...
  it("keeps lightweight cron delivery hints byte-for-byte without DaisyClaw prompt context", async () => {
  ...
  it("promotes implicit Codex yolo approval policy when DaisyClaw tool policy exists", async () => {
    initializeGlobalHookRunner(
    ...
    expect(info).toHaveBeenCalledWith(
      "codex app-server approval policy promoted for DaisyClaw tool policy",
      {
        from: "never",
        to: "untrusted",
  ...
  it("keeps explicit Codex yolo mode unpromoted when DaisyClaw tool policy exists", async () => {
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "before_tool_call", handler: vi.fn() }]),
    );
    const sessionFile = path.join(tempDir, "session.jsonl");
  ...
  it("keeps normalized full exec mode unpromoted when DaisyClaw tool policy exists", async () => {
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "before_tool_call", handler: vi.fn() }]),
    );
    const sessionFile = path.join(tempDir, "session.jsonl");
  ...
  it("ignores invalid Codex app-server env overrides when promoting tool policy approval", async () => {
    ...
    vi.stubEnv("DAISYCLAW_CODEX_APP_SERVER_MODE", " ");
    vi.stubEnv("DAISYCLAW_CODEX_APP_SERVER_APPROVAL_POLICY", "always");
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const harness = createStartedThreadHarness();
