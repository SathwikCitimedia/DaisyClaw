...
import { loadExecApprovals } from "daisyclaw/plugin-sdk/exec-approvals-runtime";
import { pathExists } from "daisyclaw/plugin-sdk/security-runtime";
import {
  resolveCodexAppServerForModelProvider,
  resolveCodexAppServerForDaisyClawToolPolicy,
...
import {
  ...
  resolveDaisyClawCodingToolsSessionKeys,
  resetDaisyClawCodingToolsFactoryForTests,
  setDaisyClawCodingToolsFactoryForTests,
...
export async function runCodexAppServerAttempt(
  ...
  let configuredAppServer = resolveCodexAppServerRuntimeOptions({
    ...
    config: params.config,
    agentDir,
    daisyClawSandboxActive: sandbox?.enabled === true,
  });
  ...
  let policyAppServer = resolveCodexAppServerForDaisyClawToolPolicy({
  ...
  if (configuredAppServer.approvalPolicy === "never" && appServer.approvalPolicy === "untrusted") {
    embeddedAgentLog.info("codex app-server approval policy promoted for DaisyClaw tool policy", {
      from: "never",
      to: "untrusted",
  ...
  policyAppServer = resolveCodexAppServerForDaisyClawToolPolicy({
    appServer: configuredAppServer,
    pluginConfig,
    env: process.env,
    shouldPromote:
  ...
  // DaisyClaw transcript is persistence/search state. Context-engine output is
  ...
  const decorateCodexTurnPromptText = (prompt: string) =>
    prependCodexDaisyClawPromptContext(prompt, daisyClawPromptContext, {
  ...
  const pendingDaisyClawDynamicToolCompletionIds = new Set<string>();
  ...
  const releaseTurnAfterTerminalDynamicTool = (paramsValue: {
    ...
    if (
      !shouldReleaseTurnAfterTerminalDynamicTool({
        ...
        pendingDaisyClawDynamicToolCompletionIdsCount: pendingDaisyClawDynamicToolCompletionIds.size,
      })
    ) {
      return;
    }
  ...
  const handleNotification = async (notification: CodexServerNotification) => {
    ...
    const notificationState = applyCodexTurnNotificationState({
      ...
      activeTurnItemIds,
      activeAppServerTurnRequests,
      pendingDaisyClawDynamicToolCompletionIds,
  ...
  const requestCleanup = client.addRequestHandler(async (request) => {
    ...
    try {
      ...
      pendingDaisyClawDynamicToolCompletionIds.add(call.callId);
      ...
      try {
        ...
        pendingDaisyClawDynamicToolCompletionIds.delete(call.callId);
  ...
  if (activeNativeTurnIds.length > 0) {
    ...
    // Starting an DaisyClaw turn before that native turn completes can wedge the
  ...
  try {
    ...
    if (isCodexActiveCompactTurnError(turnStartError)) {
      ...
      // the next DaisyClaw turn collides with that compact turn, wait for the
    ...
    if (
      ...
      // Do not try to pre-compact or summarize through DaisyClaw here. Codex owns
      // automatic compaction; DaisyClaw may only discard a stale projection thread
  ...
  try {
    ...
    const canUseCompletedAssistantTextAfterClientClose =
      ...
      pendingDaisyClawDynamicToolCompletionIds.size === 0;
...
function handleApprovalRequest(params: {
  ...
  execPolicy?: Pick<DaisyClawExecPolicyForCodexAppServer, "mode">;
