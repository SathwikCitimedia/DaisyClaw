...
describe("runCodexAppServerAttempt turn watches", () => {
  it("releases the session when Codex never completes after a dynamic tool response", async () => {
    ...
    expect(toolResult.contentItems?.[0]?.text).toMatch(
      /^(Unknown DaisyClaw tool: message|Action send requires a target\.)$/u,
    );
  ...
  it("preserves raw image-generation media when Codex never sends turn completion", async () => {
    ...
    params.timeoutMs = 200;
    vi.stubEnv("DAISYCLAW_STATE_DIR", path.join(tempDir, "state"));
  ...
  it("keeps waiting after an DaisyClaw dynamic tool response before final synthesis", async () => {
    let notify: (notification: CodexServerNotification) => Promise<void> = async () => undefined;
    let handleRequest:
      | ((request: { id: string; method: string; params?: unknown }) => Promise<unknown>)
      | undefined;
  ...
  it("clears the thread binding after a completion-idle timeout so the next turn starts fresh", async () => {
    // Regression for daisyclaw#89974. The "user interrupted the previous turn on
    // purpose" wording is Codex's generic <turn_aborted> rollout marker, written
    // whenever a turn is interrupted (including DaisyClaw's own watchdog abort).
    // DaisyClaw cannot change that text (turn/interrupt carries no reason); it can
    ...
    // Turn 2: with no binding, DaisyClaw starts a brand-new thread instead of
  ...
  it("releases completion when a projector callback throws during turn/completed", async () => {
    // Regression for daisyclaw/daisyclaw#67996: a throw inside the projector's
