...
describe("DaisyClaw Codex sandbox exec-server", () => {
  ...
  it("registers a sandbox-backed Codex environment and routes process execution through it", async () => {
    ...
    expect(environment).toEqual({
      environmentId: expect.stringMatching(/^daisyclaw-sandbox-/),
      cwd: "/workspace",
    });
    const execServerUrl =
  ...
  it("does not let Codex env policy inherit host secret variables", async () => {
    ...
    vi.stubEnv("DAISYCLAW_TEST_SECRET_TOKEN", "host-secret");
    vi.stubEnv("DAISYCLAW_TEST_DATABASE_PASSWORD", "host-password");
    vi.stubEnv("DAISYCLAW_TEST_PRIVATE_KEY", "host-private-key");
    const buildExecSpec = vi.fn(async () => ({
      argv: [process.execPath, "-e", ""],
      env: {},
      stdinMode: "pipe-closed" as const,
