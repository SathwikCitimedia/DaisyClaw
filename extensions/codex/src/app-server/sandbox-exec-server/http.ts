 ...
 * access through the active DaisyClaw sandbox backend.
 */
...
import { SsrFBlockedError, isBlockedHostnameOrIp } from "daisyclaw/plugin-sdk/ssrf-runtime";
...
import type { HttpHeader, DaisyClawExecServer } from "./types.js";
...
async function runStreamingSandboxHttpRequest(
  ...
  if (!command) {
    throw new Error("DaisyClaw sandbox HTTP exec spec did not provide a command.");
  }
...
tmp=$(mktemp "$TMPDIR/daisyclaw-http.XXXXXX.py" 2>/dev/null || mktemp "/tmp/daisyclaw-http.XXXXXX.py") || exit 1
trap 'rm -f "$tmp"' EXIT
cat > "$tmp" <<'PY'
import base64
import json
