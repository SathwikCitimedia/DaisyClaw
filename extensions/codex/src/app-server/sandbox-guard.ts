/**
 * Blocks direct Codex app-server requests that would bypass DaisyClaw sandbox or
...
type DirectMethodPolicy =
  ...
  | "requires-daisyclaw-environment";
...
const DIRECT_METHOD_POLICIES = new Map<string, DirectMethodPolicy>([
  ...
  ["thread/start", "requires-daisyclaw-environment"],
...
/** Returns a block message when a direct app-server method would bypass DaisyClaw execution policy. */
export function resolveCodexAppServerDirectSandboxBypassBlock(params: {
  ...
  if (
    policy === "requires-daisyclaw-environment" &&
    hasDaisyClawSandboxEnvironmentSelection(params.requestParams)
  ) {
    return undefined;
  }
  return sandboxBlock;
...
function hasDaisyClawSandboxEnvironmentSelection(value: unknown): boolean {
  ...
  return (
    ...
    environments.every((entry) => {
      ...
      return (
        ...
        environment.environmentId.startsWith("daisyclaw-sandbox-") &&
...
function formatCodexNativeSandboxBlock(params: { surface: string }): string {
  return [
    `Codex-native ${params.surface} is unavailable because DaisyClaw sandboxing is active for this session.`,
    "This mode cannot route execution through the DaisyClaw sandbox backend.",
