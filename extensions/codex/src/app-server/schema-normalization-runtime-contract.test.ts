...
describe("Codex app-server dynamic tool schema boundary contract", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-codex-schema-contract-"));
  });
  ...
  it("passes prepared executable dynamic tool schemas through thread start unchanged", async () => {
    ...
    expect(startPayload?.serviceName).toBe("DaisyClaw");
    ...
    expect(startPayload?.developerInstructions).toContain("DaisyClaw");
  });
