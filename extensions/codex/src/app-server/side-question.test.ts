...
import { nativeHookRelayTesting } from "daisyclaw/plugin-sdk/agent-harness-runtime";
import {
...
const createDaisyClawCodingToolsMock = vi.fn();
...
vi.mock("daisyclaw/plugin-sdk/agent-harness", () => ({
  createDaisyClawCodingTools: (...args: unknown[]) => createDaisyClawCodingToolsMock(...args),
}));
...
describe("runCodexAppServerSideQuestion", () => {
  beforeEach(() => {
    ...
    createDaisyClawCodingToolsMock.mockReset();
    ...
    createDaisyClawCodingToolsMock.mockReturnValue([
      {
        name: "wiki_status",
  ...
  it("forks an ephemeral side thread and returns the completed assistant text", async () => {
    ...
    const [toolOptions] = mockCall(createDaisyClawCodingToolsMock);
  ...
  it("rejects /btw before forking when the current DaisyClaw session is sandboxed", async () => {
    await expect(
      ...
      "Codex-native /btw side-question mode is unavailable because DaisyClaw sandboxing is active for this session.",
    );
  ...
  it("rejects /btw before forking when exec host=node is active", async () => {
    await expect(
      ...
      "Codex-native /btw side-question mode is unavailable because DaisyClaw exec host=node is active for this session.",
    );
  ...
  it("bridges side-thread dynamic tool requests to DaisyClaw tools", async () => {
    const client = createFakeClient();
  ...
  it("normalizes hook channel ids for side-thread dynamic tool requests", async () => {
    ...
    expect(createDaisyClawCodingToolsMock).toHaveBeenCalledWith(
  ...
  it("cleans up notification handlers when side tool setup fails", async () => {
    const client = createFakeClient();
    createDaisyClawCodingToolsMock.mockImplementation(() => {
