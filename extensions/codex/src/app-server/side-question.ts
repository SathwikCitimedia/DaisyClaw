...
async function createCodexSideToolBridge(input: {
  ...
  if (supportsModelTools(runtimeModel)) {
    const createDaisyClawCodingTools = (await import("daisyclaw/plugin-sdk/agent-harness"))
...
async function handleSideDynamicToolCallWithTimeout(params: {
  ...
  if (params.signal.aborted) {
    return failedSideDynamicToolResponse("DaisyClaw dynamic tool call aborted before execution.");
  }
  ...
  const timeoutPromise = new Promise<CodexDynamicToolCallResponse>((resolve) => {
    ...
    timeout = setTimeout(() => {
      controller.abort(new Error(`DaisyClaw dynamic tool call timed out after ${timeoutMs}ms.`));
      resolve(
        failedSideDynamicToolResponse(`DaisyClaw dynamic tool call timed out after ${timeoutMs}ms.`),
      );
    }, timeoutMs);
    timeout.unref?.();
  });
