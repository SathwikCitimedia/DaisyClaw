...
import type { EmbeddedRunAttemptParams } from "daisyclaw/plugin-sdk/agent-harness-runtime";
...
describe("Codex app-server native code mode config", () => {
  it("keeps Codex-native subagents primary while limiting DaisyClaw spawn to DaisyClaw delegation", () => {
    const instructions = buildDeveloperInstructions(createAttemptParams({ provider: "openai" }));

    expect(instructions).toContain("Use Codex native `spawn_agent` for Codex subagents");
    expect(instructions).toContain(
      "Use DaisyClaw `sessions_spawn` only for DaisyClaw or ACP delegation.",
    );
  });
  ...
  it("summarizes deferred dynamic tool names in developer instructions", () => {
    ...
    expect(instructions).toContain(
      "Deferred searchable DaisyClaw dynamic tools available: image_generate, music_generate.",
    );
  ...
  it("keeps developer instructions compact when no dynamic tools are deferred", () => {
    ...
    expect(instructions).not.toContain("Deferred searchable DaisyClaw dynamic tools available");
  });
  ...
  it("keeps DaisyClaw skill catalogs out of developer instructions", () => {
...
describe("Codex app-server turn params", () => {
  it("builds resume and turn params from the currently selected DaisyClaw model", () => {
  ...
  it("uses turn-scoped collaboration instructions for heartbeat Codex turns", () => {
    ...
    expect(heartbeatCollaborationMode.settings.developer_instructions).toContain(
      "This is an DaisyClaw heartbeat turn. Apply these instructions only to this heartbeat wake",
    );
    expect(heartbeatCollaborationMode.settings.developer_instructions).toContain(
  ...
  it("uses turn-scoped collaboration instructions for cron Codex turns", () => {
    ...
    expect(cronCollaborationMode.settings.developer_instructions).toContain(
      "This is an DaisyClaw cron automation turn",
    );
    expect(cronCollaborationMode.settings.developer_instructions).toContain(
...
describe("Codex app-server thread lifecycle timing", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-codex-thread-lifecycle-"));
  });
