...
import { buildCodexUserMcpServersThreadConfigPatch } from "daisyclaw/plugin-sdk/codex-mcp-projection";
import { listRegisteredPluginAgentPromptGuidance } from "daisyclaw/plugin-sdk/plugin-runtime";
...
function buildDefaultCollaborationInstructions(): string {
  ...
  // is null. DaisyClaw adds per-turn workspace instructions here, so preserve that
...
function buildCronCollaborationInstructions(): string {
  return [
    "This is an DaisyClaw cron automation turn. Apply these instructions only to this scheduled job; ordinary chat turns should stay in Codex Default mode.",
...
function buildHeartbeatCollaborationInstructions(): string {
  return [
    "This is an DaisyClaw heartbeat turn. Apply these instructions only to this heartbeat wake; ordinary chat turns should stay in Codex Default mode.",
    "When you are ready to end the heartbeat, prefer the structured `heartbeat_respond` tool so DaisyClaw can record the wake outcome and notification decision. If `heartbeat_respond` is not already available and `tool_search` is available, search for `heartbeat_respond`, load it, then call it. Use `notify=false` when nothing should visibly interrupt the user.",
...
export function buildDeveloperInstructions(
  ...
  const sections = [
    "You are a personal agent running inside DaisyClaw. DaisyClaw has dynamic tools for DaisyClaw-owned messaging, cron, sessions, media, gateway, and nodes.",
    buildDeferredDynamicToolManifest(options.dynamicTools),
    buildSkillWorkshopInstruction(options.dynamicTools),
    "Use Codex native `spawn_agent` for Codex subagents. Use DaisyClaw `sessions_spawn` only for DaisyClaw or ACP delegation.",
...
function buildDeferredDynamicToolManifest(
  ...
  return `Deferred searchable DaisyClaw dynamic tools available: ${deferredToolNames.join(", ")}. Use \`tool_search\` to load exact callable specs before use.`;
}
...
function buildVisibleReplyInstruction(
  ...
  if (messageToolAvailable) {
    return "For the current source conversation, reply normally in your final assistant message; DaisyClaw will deliver it through the active source conversation. Use `message` only for explicit out-of-band sends, media/file sends, or sends to a different target.";
  }
  return "For the current source conversation, reply normally in your final assistant message; DaisyClaw will deliver it through the active source conversation.";
}
...
export function resolveCodexAppServerModelProvider(params: {
  ...
  if (!normalized || normalizedLower === "codex") {
    // `codex` is DaisyClaw's virtual provider; let Codex app-server keep its
  ...
  if (isCodexAppServerNativeAuthProfile(params) && normalizedLower === "openai") {
    // When DaisyClaw is forwarding ChatGPT/Codex OAuth, `openai` is Codex's
