...
import { resolveUserPath } from "daisyclaw/plugin-sdk/agent-harness-runtime";
import type {
...
function parseTrajectoryEnabled(env: NodeJS.ProcessEnv): boolean {
  const value = env.DAISYCLAW_TRAJECTORY?.trim().toLowerCase();
...
function resolveTrajectoryFilePath(params: {
  ...
  const dirOverride = params.env.DAISYCLAW_TRAJECTORY_DIR?.trim();
  if (dirOverride) {
