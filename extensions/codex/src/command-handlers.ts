...
import { resolveAgentDir, resolveSessionAgentIds } from "daisyclaw/plugin-sdk/agent-runtime";
import { parseStrictPositiveInteger } from "daisyclaw/plugin-sdk/number-runtime";
import type { PluginCommandContext, PluginCommandResult } from "daisyclaw/plugin-sdk/plugin-entry";
...
const CODEX_DIAGNOSTICS_SOURCE = "daisyclaw-diagnostics";
    ...
    if (!deps.codexPluginsManagementIo) {
      return {
        text:
          ...
          "Edit ~/.daisyclaw/daisyclaw.json or use `daisyclaw config patch` until the runtime exposes the IO.",
      };
    }
...
async function bindConversation(
  ...
  if (!ctx.sessionFile) {
    return {
      text: "Cannot bind Codex because this command did not include an DaisyClaw session file.",
    };
  }
...
async function resumeThread(
  ...
  if (!ctx.sessionFile) {
    return "Cannot attach a Codex thread because this command did not include an DaisyClaw session file.";
  }
  ...
  return `Attached this DaisyClaw session to Codex thread ${formatCodexDisplayText(
    effectiveThreadId,
  )}.`;
}
...
async function stopConversationTurn(
  ...
  if (!target) {
    return "Cannot stop Codex because this command did not include an DaisyClaw session file.";
  }
  return (
...
async function steerConversationTurn(
  ...
  const target = await resolveControlTarget(ctx);
  if (!target) {
    return "Cannot steer Codex because this command did not include an DaisyClaw session file.";
  }
  return (
...
async function setConversationModel(
  ...
  const target = await resolveControlTarget(ctx);
  if (!target) {
    return "Cannot set Codex model because this command did not include an DaisyClaw session file.";
  }
  const [model = ""] = args;
...
async function setConversationFastMode(
  ...
  const target = await resolveControlTarget(ctx);
  if (!target) {
    return "Cannot set Codex fast mode because this command did not include an DaisyClaw session file.";
  }
  const value = args[0];
...
async function setConversationPermissions(
  ...
  const target = await resolveControlTarget(ctx);
  if (!target) {
    return "Cannot set Codex permissions because this command did not include an DaisyClaw session file.";
  }
  const value = args[0];
...
async function requestCodexDiagnosticsFeedbackApproval(
  ...
  if (!(await hasAnyCodexDiagnosticsSessionFile(ctx))) {
    return {
      text: "Cannot send Codex diagnostics because this command did not include an DaisyClaw session file.",
    };
  }
  ...
  if (targets.length === 0) {
    return {
      text: [
        "No Codex thread is attached to this DaisyClaw session yet.",
...
async function previewCodexDiagnosticsFeedbackApproval(
  ...
  if (!(await hasAnyCodexDiagnosticsSessionFile(ctx))) {
    return "Cannot send Codex diagnostics because this command did not include an DaisyClaw session file.";
  }
  const targets = await resolveCodexDiagnosticsTargets(deps, ctx);
  if (targets.length === 0) {
    return [
  ...
  return [
    ...
    "The completed diagnostics reply will list the DaisyClaw session ids and Codex thread ids that were sent.",
    ...(displayReason ? [`Note: ${displayReason}`] : []),
    "Included: Codex logs and spawned Codex subthreads when available.",
  ].join("\n");
}
...
function formatCodexDiagnosticsTargetBlock(
  ...
  if (target.sessionKey) {
    lines.push(`DaisyClaw session key: ${formatCodexCopyableValueForDisplay(target.sessionKey)}`);
  }
  if (target.sessionId) {
    lines.push(`DaisyClaw session id: ${formatCodexCopyableValueForDisplay(target.sessionId)}`);
  }
...
function formatCodexDiagnosticsTargetLine(target: CodexDiagnosticsTarget): string {
  ...
  if (sessionLabel) {
    parts.push(`DaisyClaw session ${formatCodexValueForDisplay(sessionLabel)}`);
  }
...
async function startThreadAction(
  ...
  const target = await resolveControlTarget(ctx);
  if (!target) {
    return `Cannot start Codex ${label} because this command did not include an DaisyClaw session file.`;
  }
  const binding = await deps.readCodexAppServerBinding(target.sessionFile, {
    agentDir: target.agentDir,
    config: ctx.config,
  ...
  if (!binding?.threadId) {
    return `No Codex thread is attached to this DaisyClaw session yet.`;
  }
