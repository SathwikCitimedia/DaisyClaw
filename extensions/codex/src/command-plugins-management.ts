...
/**
 * Lightweight read/write surface over the Daisyclaw config file. Plugged in by
...
// Plugin lifecycle changes (enable/disable) write to daisyclaw.json
...
export async function handleCodexPluginsSubcommand(
  ...
  if (normalized === "enable" || normalized === "disable") {
    ...
    return {
      text: `${formatCodexDisplayText(target)}: ${wantEnabled ? "enabled" : "disabled"} in daisyclaw.json. ${POLICY_REFRESH_HINT}`,
    };
  }
...
export function buildPluginsHelp(): string {
  return [
    "Codex sub-plugin management (writes only to ~/.daisyclaw/daisyclaw.json, never to ~/.codex/config.toml):",
...
export function formatPluginList(
  ...
  return [
    "Codex sub-plugins in Daisyclaw config (~/.daisyclaw/daisyclaw.json):",
    "",
    ...rows.map(
      (r) =>
