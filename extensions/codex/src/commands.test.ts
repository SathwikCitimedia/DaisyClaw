...
function expectedDiagnosticsTargetBlock(params: {
  ...
  return [
    ...
    ...(params.sessionKey ? [`DaisyClaw session key: \`${params.sessionKey}\``] : []),
    ...(params.sessionId ? [`DaisyClaw session id: \`${params.sessionId}\``] : []),
...
describe("codex command", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-codex-command-"));
    vi.stubEnv("DAISYCLAW_STATE_DIR", tempDir);
  });
  ...
  it("lists Codex sub-plugins through the /codex plugins command surface", async () => {
    ...
    expectResultTextContains(result, "daisyclaw.json");
  });
  ...
  it("enables and disables Codex sub-plugins through the /codex plugins command surface", async () => {
    ...
    expectResultTextContains(disabled, "google-calendar: disabled in daisyclaw.json");
    ...
    expectResultTextContains(enabled, "google-calendar: enabled in daisyclaw.json");
  ...
  it("attaches the current session to an existing Codex thread", async () => {
    ...
    await expect(
      ...
      text: "Attached this DaisyClaw session to Codex thread thread-123.",
    });
  ...
  it.each([
    ...
    expect(result.text).toContain(
      "Codex-native /codex " +
        args.split(/\s+/u)[0] +
        " is unavailable because DaisyClaw sandboxing is active for this session.",
    );
    expect(codexControlRequest).not.toHaveBeenCalled();
  ...
  it.each([
    ...
    expect(result.text).toContain(
      "Codex-native /codex " +
        args.split(/\s+/u)[0] +
        " is unavailable because DaisyClaw exec host=node is active for this session.",
    );
    expect(codexControlRequest).not.toHaveBeenCalled();
    expect(startCodexConversationThread).not.toHaveBeenCalled();
    expect(steerCodexConversationTurn).not.toHaveBeenCalled();
  ...
  it("blocks config-level exec host=node without a session key", async () => {
    ...
    expect(result.text).toContain(
      "Codex-native /codex bind is unavailable because DaisyClaw exec host=node is active for this session.",
    );
    expect(startCodexConversationThread).not.toHaveBeenCalled();
  });
  ...
  it("explains compaction when no Codex thread is attached", async () => {
    ...
    await expect(
      ...
      text: "No Codex thread is attached to this DaisyClaw session yet.",
    });
  });
  ...
  it("uploads all Codex diagnostics sessions and reports their channel/thread breakdown", async () => {
    ...
    expect(request.text).toContain("DaisyClaw session key: `agent:main:whatsapp:one`");
    expect(request.text).toContain("DaisyClaw session id: `session-one`");
    expect(request.text).toContain("Codex thread id: `thread-111`");
    expect(request.text).toContain("DaisyClaw session key: `agent:main:discord:two`");
    expect(request.text).toContain("DaisyClaw session id: `session-two`");
