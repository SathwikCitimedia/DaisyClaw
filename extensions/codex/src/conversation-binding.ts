...
import { resolveSessionAgentIds } from "daisyclaw/plugin-sdk/agent-runtime";
...
const NATIVE_CONVERSATION_INTERACTIVE_APPROVALS_UNAVAILABLE =
  "DaisyClaw native Codex conversation binding cannot route interactive approvals yet; use the Codex harness or explicit /acp spawn codex for that workflow.";
...
async function resolveConversationAppServerRuntime(params: {
  ...
  const runtime = resolveCodexAppServerRuntimeOptions({
    ...
    daisyClawSandboxActive: Boolean(sandboxForPolicy?.enabled),
  });
  return { execPolicy, runtime };
}

const CODEX_CONVERSATION_GLOBAL_STATE = Symbol.for("daisyclaw.codex.conversationBinding");
    ...
    const response: CodexThreadStartResponse = await resolved.client.request(
      "thread/start",
      {
        ...
        developerInstructions:
          "This Codex thread is bound to an DaisyClaw conversation. Answer normally; DaisyClaw will deliver your final response back to the conversation.",
      ...
      if (request.method === "item/tool/call") {
        return {
          contentItems: [
            {
              type: "inputText",
              text: "DaisyClaw native Codex conversation binding does not expose dynamic DaisyClaw tools yet.",
            },
          ],
          success: false,
        };
...
function resolveConversationExecPolicy(params: {
  ...
  const agentId =
    ...
    (params.config
      ...
      : undefined);
  return resolveDaisyClawExecPolicyForCodexAppServer({
    config: params.config,
    agentId,
