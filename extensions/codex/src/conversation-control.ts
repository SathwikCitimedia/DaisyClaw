...
const CODEX_CONVERSATION_CONTROL_STATE = Symbol.for("daisyclaw.codex.conversationControl");
...
async function requireThreadBinding(sessionFile: string, lookup: CodexAppServerBindingLookup = {}) {
  ...
  if (!binding?.threadId) {
    throw new Error("No Codex thread is attached to this DaisyClaw session yet.");
  }
  return binding;
}
