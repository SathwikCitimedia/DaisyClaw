...
import { loadAuthProfileStoreWithoutExternalProfiles } from "daisyclaw/plugin-sdk/agent-runtime";
import {
  createMigrationItem,
  markMigrationItemConflict,
  markMigrationItemError,
  markMigrationItemSkipped,
} from "daisyclaw/plugin-sdk/migration";
import type { MigrationItem, MigrationProviderContext } from "daisyclaw/plugin-sdk/plugin-entry";
import {
...
type AgentDefaultModelConfigs = NonNullable<
  NonNullable<NonNullable<DaisyClawConfig["agents"]>["defaults"]>["models"]
>;
...
async function buildCodexOAuthCredential(source: CodexSource): Promise<CodexAuthCredential | null> {
  ...
  } satisfies Partial<DaisyClawConfig>;
...
function replaceConfigDraft(draft: DaisyClawConfig, next: DaisyClawConfig): void {
  for (const key of Object.keys(draft) as Array<keyof DaisyClawConfig>) {
    delete draft[key];
  }
...
function existingAuthProfileConfigIsCompatible(
  existing: NonNullable<NonNullable<DaisyClawConfig["auth"]>["profiles"]>[string],
...
function hasCurrentAuthProfileConfigConflict(
  ...
  try {
    config = (ctx.runtime?.config?.current?.() as DaisyClawConfig | undefined) ?? config;
  } catch {
...
function applyDefaultModelIfMissing(cfg: DaisyClawConfig): DaisyClawConfig {
...
async function applyCodexAuthProfileConfig(
  ctx: MigrationProviderContext,
  profile: CodexAuthProfileConfig,
  applyConfig: (config: DaisyClawConfig) => DaisyClawConfig,
