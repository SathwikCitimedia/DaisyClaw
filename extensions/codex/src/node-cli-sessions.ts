...
import { isRecord } from "daisyclaw/plugin-sdk/string-coerce-runtime";
...
export function createCodexCliSessionNodeHostCommands(): DaisyClawPluginNodeHostCommand[] {
  return [
    {
...
export function createCodexCliSessionNodeInvokePolicies(): DaisyClawPluginNodeInvokePolicy[] {
  return [
    {
...
async function runCodexExecResume(params: {
  ...
  const outputPath = path.join(
    await fs.mkdtemp(path.join(resolvePreferredDaisyClawTmpDir(), "daisyclaw-codex-cli-")),
    "last-message.txt",
  );
  try {
    const args = [
