...
export function buildComfyConfig(config: Record<string, unknown>): DaisyClawConfig {
  return {
    plugins: {
      entries: {
        comfy: { config },
...
export function buildLegacyComfyConfig(config: Record<string, unknown>): DaisyClawConfig {
  return {
    models: {
      providers: {
        comfy: config,
