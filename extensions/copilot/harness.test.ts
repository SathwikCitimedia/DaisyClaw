...
describe("createCopilotAgentHarness", () => {
  ...
  it("supports rejects providers outside the whitelist", () => {
    ...
    for (const legacyId of ["github", "daisyclaw", "copilot"]) {
      expect(
        harness.supports({
          provider: legacyId,
          modelId: "gpt-4.1",
  ...
  describe("reset", () => {
    ...
    it("does not invoke deleteSession for a session belonging to a different daisyclawSessionId", async () => {
      const pool = makePoolMock();
      const deleteSession = vi.fn().mockResolvedValue(undefined);
      const client = { deleteSession } as any;
      mocks.runCopilotAttempt.mockImplementation(async (params, deps) => {
  ...
  describe("session reuse across turns (dogfood finding #4)", () => {
    ...
    // `runAttempt` calls within the same DaisyClaw session should pass
