...
import type { PluginStateSyncKeyedStore } from "daisyclaw/plugin-sdk/plugin-state-runtime";
...
export function createCopilotAgentHarness(
  ...
  // Maps DaisyClaw session id (from AgentHarnessAttemptParams.sessionId) to
  ...
  return {
    ...
    async runAttempt(params: AgentHarnessAttemptParams): Promise<AgentHarnessAttemptResult> {
      const attemptPromise = (async () => {
        ...
        const daisyclawSessionId =
          typeof params.sessionId === "string" ? params.sessionId : undefined;

        // Dogfood finding #4: reuse the SDK session across turns within
        // the same DaisyClaw session so that the GitHub Copilot agent runtime's prompt
        ...
        const tracked = daisyclawSessionId ? trackedSessions.get(daisyclawSessionId) : undefined;
        const stored = daisyclawSessionId
          ? resetBlockedStoredSessions.has(daisyclawSessionId)
            ? undefined
            : lookupStoredBinding(options?.sessionStore, daisyclawSessionId)
          : undefined;
        ...
        return runCopilotAttempt(effectiveParams, {
          pool,
          onSessionEstablished: daisyclawSessionId
            ? ({
                sdkSessionId,
                pooledClient,
                sessionConfig,
                ...
                trackedSessions.set(daisyclawSessionId, {
                  sdkSessionId,
                ...
                const persisted = registerStoredBinding(options?.sessionStore, daisyclawSessionId, {
                  schemaVersion: 2,
                  sdkSessionId,
                  compatKey: currentCompatKey,
                  compactKey: currentCompactKey,
                ...
                if (persisted) {
                  resetBlockedStoredSessions.delete(daisyclawSessionId);
                }
              }
            : undefined,
        });
    ...
    async reset(params: AgentHarnessResetParams): Promise<void> {
      const daisyclawSessionId = typeof params.sessionId === "string" ? params.sessionId : undefined;
      if (!daisyclawSessionId) {
        return;
      }
      const tracked = trackedSessions.get(daisyclawSessionId);
      if (deleteStoredBinding(options?.sessionStore, daisyclawSessionId)) {
        resetBlockedStoredSessions.delete(daisyclawSessionId);
      } else {
        resetBlockedStoredSessions.add(daisyclawSessionId);
      }
      if (!tracked) {
        // Session was created by a different harness, or already reset.
        return;
      }
      trackedSessions.delete(daisyclawSessionId);
      try {
    ...
    async compact(
      ...
      // The SDK owns Copilot history compaction. DaisyClaw only resumes
      // the tracked SDK session and calls the session-scoped RPC; durable
      // DaisyClaw session/transcript state stays in SQLite, with no marker
      ...
      if (!compatibleTracked) {
        ...
        // needs the live SessionConfig with DaisyClaw hooks/tools, so preserve the
      ...
      try {
        ...
        if (isStaleSdkSessionError(err)) {
          trackedSessions.delete(daisyclawSessionId);
          deleteStoredBinding(options?.sessionStore, daisyclawSessionId);
          return {
            ok: false,
            compacted: false,
