...
import type { AgentHarnessAttemptParams } from "daisyclaw/plugin-sdk/agent-harness-runtime";
...
vi.mock("daisyclaw/plugin-sdk/agent-harness", async (importOriginal) => {
  const actual = await importOriginal<typeof import("daisyclaw/plugin-sdk/agent-harness")>();

  return {
    ...actual,
    createDaisyClawCodingTools: vi.fn(() => [
      {
...
const LIVE = isLiveTestEnabled(["DAISYCLAW_COPILOT_AGENT_LIVE_TEST"]);
const TOKEN =
  process.env.DAISYCLAW_COPILOT_AGENT_LIVE_TOKEN ||
...
describeLive("copilot agent runtime live smoke", () => {
  it("runs one turn on gpt-4.1 with one custom tool", async () => {
    ...
    const copilotHome = await mkdtemp(join(tmpdir(), "daisyclaw-copilot-live-"));
