...
// daisyclaw/plugin-sdk/agent-harness-runtime declares auth, messages,
// onAssistantDelta, and initialReplayState.sdkSessionId fields. Tracked by
// project daisyclaw-copilot-harness; reviewer-attempt-bridge note.
...
type AttemptParamsLike = AgentHarnessAttemptParams & {
  ...
  // current-turn user message for the DaisyClaw audit transcript so
...
export interface CopilotAttemptDeps {
   ...
   * `daisyclaw/plugin-sdk/agent-harness-runtime#resolveSandboxContext`, which is
   ...
   * harness uses this to track the daisyclawSessionId -> sdkSessionId
...
export async function runCopilotAttempt(
  ...
  try {
    ...
    try {
      const toolBridge = await createToolBridge({
        ...
        // tool-bridge.ts buildDaisyClawCodingToolsOptions().
        attemptParams: input,
        sessionRef,
        onYieldDetected: () => {
          yieldDetected = true;
    ...
    // Load DaisyClaw workspace bootstrap files (SOUL.md, IDENTITY.md,
    ...
    // DaisyClaw bootstrap block (SDK still loads AGENTS.md natively).
    ...
    if (abortRequested || params.abortSignal?.aborted) {
      ...
      if (!bridge.recordSendResult(result) && !aborted) {
        ...
        // capability inventory. Do not call session.abort() here: DaisyClaw may
  ...
  //
  // Without this synthesis the DaisyClaw audit transcript never sees
  ...
  // (user/assistant/toolResult) into the DaisyClaw audit transcript at
  // params.sessionFile, alongside the SDK's own session storage. The
  // DaisyClaw shell (attempt-execution.ts) writes only the user prompt
  ...
  if (sessionFileForMirror && messagesSnapshot.length > 0) {
    ...
    await dualWriteCopilotTranscriptBestEffort({
      ...
      // authoritative; only the DaisyClaw audit transcript would be
...
function createSessionConfig(
  ...
  return {
    ...
    // tool-bridge.ts) so 100% of tool calls go through DaisyClaw's
    ...
    // requires routing the request to the DaisyClaw channel/TUI prompt
    ...
    ...(resolvedAuth.authMode === "gitHubToken" && resolvedAuth.gitHubToken
      ...
      : {}),
    // DaisyClaw workspace bootstrap plus per-turn runtime guidance
    // injected via the SDK's `systemMessage` field in append mode:
    // SDK foundation + DaisyClaw context. Append keeps every SDK
    ...
    // L1036). Omitted when there is no DaisyClaw-owned context so the
...
function hasMirrorIdentity(message: AgentMessage): boolean {
  const record = message as unknown as { __daisyclaw?: unknown };
