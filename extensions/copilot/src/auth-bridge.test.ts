...
describe("resolveCopilotAuth - copilotHome resolution", () => {
  ...
  it("synthesises per-agent default from homeDir when no path is given", () => {
    ...
    expect(result.copilotHome).toBe(
      resolve(join(FAKE_HOME, ".daisyclaw", "agents", "agent-1", "copilot")),
    );
  });

  it("respects DAISYCLAW_HOME env var as the home root", () => {
    const result = resolveCopilotAuth({
      agentId: "agent-1",
      env: { DAISYCLAW_HOME: "/custom/daisyclaw" } as NodeJS.ProcessEnv,
      homeDir: fakeHomeDir,
    });
    expect(result.copilotHome).toBe(
      resolve(join("/custom/daisyclaw", ".daisyclaw", "agents", "agent-1", "copilot")),
    );
  });
  ...
  it("uses the default agent id when agentId is invalid/missing", () => {
    ...
    expect(result.copilotHome).toBe(
      resolve(join(FAKE_HOME, ".daisyclaw", "agents", COPILOT_DEFAULT_AGENT_ID, "copilot")),
    );
  });
...
describe("resolveCopilotAuth - contract-resolved auth (resolvedApiKey + authProfileId)", () => {
  ...
  it("contract resolvedApiKey takes precedence over env fallback", () => {
    const result = resolveCopilotAuth({
      ...
      authProfileId: "p",
      env: {
        DAISYCLAW_GITHUB_TOKEN: "env-should-be-ignored",
...
describe("resolveCopilotAuth - env var fallbacks", () => {
  ...
  it("DAISYCLAW_GITHUB_TOKEN takes precedence over GITHUB_TOKEN", () => {
    const result = resolveCopilotAuth({
      agentId: "agent-1",
      env: {
        DAISYCLAW_GITHUB_TOKEN: "daisyclaw-tok",
        GITHUB_TOKEN: "github-tok",
      } as NodeJS.ProcessEnv,
      homeDir: fakeHomeDir,
    });
    expect(result.gitHubToken).toBe("daisyclaw-tok");
    expect(result.authProfileId).toBe("env:DAISYCLAW_GITHUB_TOKEN");
    expect(result.authProfileVersion).toBe(tokenFingerprint("daisyclaw-tok"));
  });
  ...
  it("DAISYCLAW_GITHUB_TOKEN takes precedence over COPILOT_GITHUB_TOKEN, GH_TOKEN and GITHUB_TOKEN", () => {
    const result = resolveCopilotAuth({
      agentId: "agent-1",
      env: {
        DAISYCLAW_GITHUB_TOKEN: "daisyclaw-tok",
  ...
  it("explicit auth.useLoggedInUser=true wins over env tokens", () => {
    const result = resolveCopilotAuth({
      agentId: "agent-1",
      auth: { useLoggedInUser: true },
      env: { DAISYCLAW_GITHUB_TOKEN: "env-tok" } as NodeJS.ProcessEnv,
      homeDir: fakeHomeDir,
    });
    expect(result.authMode).toBe("useLoggedInUser");
  });
...
describe("resolveCopilotAuth - defaults wiring", () => {
  ...
  it("uses os.homedir() when homeDir is not injected", () => {
    ...
    expect(result.copilotHome.endsWith(join(".daisyclaw", "agents", "agent-1", "copilot"))).toBe(
      true,
    );
  });
  ...
  it("falls back to process.cwd() if homeDir throws", () => {
    ...
    expect(result.copilotHome.includes(join(".daisyclaw", "agents", "agent-1", "copilot"))).toBe(
      true,
    );
  });
});
