 ...
 *     use cases.
 *   - Falls back to DAISYCLAW_GITHUB_TOKEN, COPILOT_GITHUB_TOKEN,
 ...
 *     (`<daisyClawHome>/.daisyclaw/agents/<agentId>/copilot`, or
 *     `<agentDir>/copilot` when an agent directory is supplied) that
 *     respects `DAISYCLAW_HOME` for the home directory root.
 ...
 *   4. DAISYCLAW_GITHUB_TOKEN, then COPILOT_GITHUB_TOKEN, then
...
function resolveCopilotHome(args: {
  ...
  // "copilot" subdir so it cannot collide with DaisyClaw's own files
  ...
  const daisyClawHome = readString(args.env.DAISYCLAW_HOME);
  const rootHome = daisyClawHome ? resolve(daisyClawHome) : safeHomeDir(args.homeDir);
  // Per-agent isolation per proposal section 3.6:
  //   <daisyClawHome>/.daisyclaw/agents/<agentId>/copilot
  return resolve(join(rootHome, ".daisyclaw", "agents", args.agentId, "copilot"));
}
...
function readEnvTokenFallback(
  ...
  // DAISYCLAW_GITHUB_TOKEN is the harness-specific override and stays at
  ...
  const candidates: Array<{ name: string; value: string | undefined }> = [
    { name: "DAISYCLAW_GITHUB_TOKEN", value: readString(env.DAISYCLAW_GITHUB_TOKEN) },
