...
import type { AgentMessage } from "daisyclaw/plugin-sdk/agent-harness-runtime";
import {
...
async function createTempSessionFile() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-copilot-mirror-"));
  tempDirs.push(dir);
...
describe("mirrorCopilotTranscript", () => {
  it("mirrors user, assistant, and tool result messages into the DaisyClaw transcript", async () => {
  ...
  it("creates the transcript directory on first mirror", async () => {
    const root = await makeRoot("daisyclaw-copilot-mirror-missing-dir-");
  ...
  it("uses attached identity instead of content fingerprint when provided", async () => {
    ...
    await mirrorCopilotTranscript({
      sessionFile,
      messages: [tagged],
      idempotencyScope: "copilot:daisyclaw-session-1",
    });

    const raw = await fs.readFile(sessionFile, "utf8");
    expect(raw).toContain(
      '"idempotencyKey":"copilot:daisyclaw-session-1:sdk-session-1:assistant:0"',
    );
