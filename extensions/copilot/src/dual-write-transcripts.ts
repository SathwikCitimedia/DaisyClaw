 ...
 * DaisyClaw audit transcript that sits next to (but is distinct from) the
 * SDK's own session storage.
 *
 * The DaisyClaw shell (src/agents/command/attempt-execution.ts) already
 ...
 * assistant + toolResult) into the DaisyClaw transcript via the same
...
export function attachCopilotMirrorIdentity<T extends AgentMessage>(
  ...
  const existing = record["__daisyclaw"];
  const baseMeta =
  ...
  return {
    ...record,
    __daisyclaw: { ...baseMeta, [MIRROR_IDENTITY_META_KEY]: identity },
  } as unknown as T;
}
 ...
 * the DaisyClaw audit trail just misses the intermediate messages for
 * this turn.
 */
