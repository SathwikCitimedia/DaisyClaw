...
describe("sdk-loader", () => {
  ...
  it("default fallback importer resolves and imports the installed SDK entry", async () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        "module.exports = { daisyclawDefaultImporterSentinel: true };",
      );
      ...
      })) as unknown as { daisyclawDefaultImporterSentinel?: boolean };

      expect(sdk.daisyclawDefaultImporterSentinel).toBe(true);
      expect(primaryImport).toHaveBeenCalledTimes(1);
    } finally {
      rmSync(tmp, { recursive: true, force: true });
    }
  ...
  it("throws an actionable error with plugin install instructions when both probes fail", async () => {
    ...
    await expect(
      ...
      message: expect.stringContaining("daisyclaw plugins install @daisyclaw/copilot"),
    });
  ...
  it("error message includes the fallback path and underlying primary error", async () => {
    ...
    expect(message).toContain("daisyclaw plugins install @daisyclaw/copilot");
  });
  ...
  it("default fallback dir points at ~/.daisyclaw/npm-runtime/copilot", () => {
    expect(COPILOT_SDK_FALLBACK_DIR).toMatch(/\.daisyclaw[\\/]+npm-runtime[\\/]+copilot$/);
  });

  it("resolves the fallback dir from DAISYCLAW_STATE_DIR for relocated profiles", () => {
    expect(
      resolveCopilotSdkFallbackDir({
        ...process.env,
        DAISYCLAW_STATE_DIR: "/tmp/daisyclaw-state",
      }),
    ).toBe(path.join("/tmp/daisyclaw-state", "npm-runtime", "copilot"));
  });
