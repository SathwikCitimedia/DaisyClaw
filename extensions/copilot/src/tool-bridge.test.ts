...
import type { AnyAgentTool, SandboxContext } from "daisyclaw/plugin-sdk/agent-harness-runtime";
import { afterEach, describe, expect, it, vi } from "vitest";
import {
  createCopilotToolBridge,
  convertDaisyClawToolToSdkTool,
  supportsModelTools,
} from "./tool-bridge.js";
...
describe("supportsModelTools", () => {
  it("returns true for github-copilot and false otherwise", () => {
    ...
    expect(supportsModelTools("daisyclaw")).toBe(false);
...
describe("createCopilotToolBridge", () => {
  it("returns empty arrays for unsupported providers without calling the seam", async () => {
    const createDaisyClawCodingTools = vi.fn(async () => [makeTool()]);
    ...
    expect(createDaisyClawCodingTools).toHaveBeenCalledTimes(0);
  });

  it("forwards supported fields to injected createDaisyClawCodingTools", async () => {
    ...
    expect(createDaisyClawCodingTools).toHaveBeenCalledTimes(1);
    ...
    expect(createDaisyClawCodingTools).toHaveBeenCalledWith(
  ...
  it("returns sdkTools and sourceTools with matching lengths", async () => {
    ...
    const result = await createCopilotToolBridge({
      agentId: "agent-1",
      createDaisyClawCodingTools: async () => sourceTools,
      modelId: "gpt-4o",
      modelProvider: "github-copilot",
      sessionId: "session-1",
    });
  ...
  it("throws when createDaisyClawCodingTools returns a non-array", async () => {
    await expect(
      createCopilotToolBridge({
        agentId: "agent-1",
        createDaisyClawCodingTools: async () => ({ tools: [] }) as never,
        modelId: "gpt-4o",
        modelProvider: "github-copilot",
        sessionId: "session-1",
      }),
    ).rejects.toThrow("createDaisyClawCodingTools must return an array");
  });

  it("throws when createDaisyClawCodingTools rejects and includes the cause", async () => {
    await expect(
      createCopilotToolBridge({
        agentId: "agent-1",
        createDaisyClawCodingTools: async () => {
  ...
  // F6: PI-parity tool context. The bridged DaisyClaw tools register
  ...
  describe("PI-parity attempt context (F6)", () => {
    function captureCall() {
      ...
      return {
        createDaisyClawCodingTools,
        getOpts: () =>
          (createDaisyClawCodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as Record<
            string,
            unknown
          >,
      };
    ...
    it("forwards identity, owner/policy, and channel/routing fields from attemptParams", async () => {
      const { createDaisyClawCodingTools, getOpts } = captureCall();
    ...
    it("prefers the unscoped toolAuthProfileStore when building DaisyClaw tools", async () => {
      const { createDaisyClawCodingTools, getOpts } = captureCall();
  ...
  describe("sandbox forwarding (PR #86155 [P1])", () => {
    ...
    it("defaults sandbox to undefined and derives spawnWorkspaceDir from workspaceDir when no sandbox is passed (back-compat)", async () => {
      ...
      const opts = (createDaisyClawCodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as {
        sandbox?: unknown;
    ...
    it("forwards an explicit sandbox and spawnWorkspaceDir verbatim to createDaisyClawCodingTools", async () => {
  ...
  // DaisyClaw attempt would suppress. These tests pin the contract.
  describe("tool-surface gating (PR #86155 [P1] round-6)", () => {
    it("short-circuits when attemptParams.disableTools is true and never calls createDaisyClawCodingTools", async () => {
      const createDaisyClawCodingTools = vi.fn(async () => [makeTool()]);
      const result = await createCopilotToolBridge({
        agentId: "agent-1",
    ...
    it("filters constructed tools to exactly the allowlist when toolsAllow is narrow", async () => {
      const createDaisyClawCodingTools = vi.fn(async () => [
        makeTool({ name: "read" }),
        makeTool({ name: "edit" }),
    ...
    it("leaves the tool list unchanged when toolsAllow is undefined", async () => {
      ...
      const createDaisyClawCodingTools = vi.fn(async () => tools);
      const result = await createCopilotToolBridge({
        agentId: "agent-1",
  ...
  describe("tool-name aliases (PR #86155 [P1] round-7)", () => {
    ...
    it("keeps core tools available for glob allowlists", async () => {
      ...
      const options = (createDaisyClawCodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as {
        toolConstructionPlan?: { includeDaisyClawTools?: boolean };
      };
      expect(options?.toolConstructionPlan?.includeDaisyClawTools).toBe(true);
    });
...
describe("convertDaisyClawToolToSdkTool", () => {
  it("throws on empty and non-string names", () => {
    expect(() => convertDaisyClawToolToSdkTool(makeTool({ name: "" as never }), {})).toThrow(
      "tool name must be a non-empty string",
    );
    expect(() => convertDaisyClawToolToSdkTool(makeTool({ name: 42 as never }), {})).toThrow(
      "tool name must be a non-empty string",
    );
  });

  it("throws on non-function execute", () => {
    expect(() => convertDaisyClawToolToSdkTool(makeTool({ execute: "nope" as never }), {})).toThrow(
  ...
  it("preserves name, description, and parameters exactly", () => {
    ...
    const result = convertDaisyClawToolToSdkTool(sourceTool, {});
  ...
  it("sets skipPermission: true so DaisyClaw's wrapped-tool internal enforcement handles permission decisions (PI-parity model)", () => {
    // Per the harness docs: every bridged DaisyClaw tool comes from
    // `createDaisyClawCodingTools`, which already wraps each tool with
    ...
    // for kind: "custom-tool" would either short-circuit DaisyClaw's
    // richer enforcement (allow-all) or block every call (reject-all).
    // Setting `skipPermission: true` lets the wrapped execute() run
    // DaisyClaw's hook with the right context — mirrors codex
    // (`extensions/codex/src/app-server/dynamic-tools.ts`).
    const result = convertDaisyClawToolToSdkTool(makeTool(), {}) as SdkTool & {
  ...
  it("marks every bridged tool as overridesBuiltInTool so DaisyClaw owns names that collide with Copilot CLI built-ins (edit/read/write/bash/...)", () => {
    // Real-world dogfood found that daisyclaw's createDaisyClawCodingTools
    ...
    for (const name of ["edit", "read", "write", "bash", "live_echo"]) {
      const result = convertDaisyClawToolToSdkTool(makeTool({ name }), {}) as SdkTool & {
  ...
  it("returns a failure result when the signal is already aborted", async () => {
    ...
    controller.abort();
    const sourceTool = makeTool();
    const sdkTool = convertDaisyClawToolToSdkTool(sourceTool, { abortSignal: controller.signal });
  ...
  it("calls beforeExecute with the invocation context before execute", async () => {
    ...
    const sourceTool = makeTool();
    const sdkTool = convertDaisyClawToolToSdkTool(sourceTool, { beforeExecute });
  ...
  it("returns a failure result when beforeExecute throws", async () => {
    ...
    const sourceTool = makeTool();
    const sdkTool = convertDaisyClawToolToSdkTool(sourceTool, {
  ...
  it("converts single text content to an exact textResultForLlm", async () => {
    const sdkTool = convertDaisyClawToolToSdkTool(
