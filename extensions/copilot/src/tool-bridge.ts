...
type CreateDaisyClawCodingTools =
  (typeof import("daisyclaw/plugin-sdk/agent-harness"))["createDaisyClawCodingTools"];
type DaisyClawCodingToolsOptions = NonNullable<Parameters<CreateDaisyClawCodingTools>[0]>;
 ...
 * authoritative `createDaisyClawCodingTools({...})` call shape).
 *
 * Declared as `Partial<EmbeddedRunAttemptParams>` (imported from the
 * `daisyclaw/plugin-sdk/agent-harness-runtime` boundary, *not* from
...
export interface CopilotToolBridgeInput {
   ...
   * model, and run-trace fields to `createDaisyClawCodingTools` so the
  ...
  createDaisyClawCodingTools?: (opts: unknown) => AnyAgentTool[] | Promise<AnyAgentTool[]>;
  beforeExecute?: (ctx: {
    toolName: string;
    toolCallId: string;
    args: unknown;
...
export async function createCopilotToolBridge(
  ...
  const createDaisyClawCodingTools =
    input.createDaisyClawCodingTools ??
    (await import("daisyclaw/plugin-sdk/agent-harness")).createDaisyClawCodingTools;

  const toolOptions = buildDaisyClawCodingToolsOptions(input, effectiveToolPlan);

  let sourceTools: unknown;
  try {
    sourceTools = await createDaisyClawCodingTools(toolOptions);
  } catch (error: unknown) {
    throw createError(
      `[copilot-tool-bridge] createDaisyClawCodingTools failed: ${toError(error).message}`,
      error,
    );
  }
  ...
  if (!Array.isArray(sourceTools)) {
    throw new Error(
      "[copilot-tool-bridge] createDaisyClawCodingTools must return an array of tools",
    );
  }
  ...
  return {
    sdkTools: filteredTools.map((sourceTool) =>
      convertDaisyClawToolToSdkTool(sourceTool, {
...
/**
 * Builds the full `createDaisyClawCodingTools` options bag mirroring the
 * PI in-tree call at `src/agents/pi-embedded-runner/run/attempt.ts:1029-1117`.
 *
 * Why PI parity matters: bridged DaisyClaw tools register with the SDK
 * as `overridesBuiltInTool: true, skipPermission: true` (see
 * `convertDaisyClawToolToSdkTool` below). That means the wrapped-tool
 * enforcement layer
...
function buildDaisyClawCodingToolsOptions(
  ...
  const modelCompat =
    ...
    typeof model.compat === "object"
      ? (model.compat as DaisyClawCodingToolsOptions["modelCompat"])
      : undefined;
...
export function convertDaisyClawToolToSdkTool(
  sourceTool: AnyAgentTool,
  ctx: {
    abortSignal?: AbortSignal;
  ...
  return {
    ...
    // DaisyClaw owns its bridged tools by design (the harness docs:
    // "DaisyClaw still owns ... DaisyClaw dynamic tools (bridged)"). The bundled
    // Copilot CLI ships built-in tools whose names (edit, read, write, bash,
    // ...) collide with DaisyClaw's coding-tool set. Mark every bridged tool as
    ...
    // same name." DaisyClaw's tool layer is the source of truth for these
    // names within a copilot attempt.
    overridesBuiltInTool: true,
    parameters: sourceTool.parameters as Record<string, unknown> | undefined,
    // Bridged DaisyClaw tools enforce their own permission/policy decisions
    ...
    // `kind: "custom-tool"` would either short-circuit DaisyClaw's richer
    // enforcement (if we allow-all) or block every call (if we
    // reject-all) — neither matches PI parity. The in-tree codex harness
    // takes the same approach: bridged DaisyClaw tools are wrapped with
