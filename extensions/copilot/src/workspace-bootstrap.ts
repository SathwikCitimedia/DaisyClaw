...
// Filtering them out of the DaisyClaw bootstrap injection avoids
...
/**
 * Loads DaisyClaw workspace bootstrap files (IDENTITY.md, SOUL.md,
...
export async function resolveCopilotWorkspaceBootstrapContext(params: {
  ...
  try {
    ...
    // SOUL.md / IDENTITY.md / .daisyclaw conventions live) onto the
...
export function renderCopilotWorkspaceBootstrapInstructions(
  ...
  const lines: string[] = [
    "DaisyClaw loaded these user-editable workspace files. Treat them as project/user context. The Copilot SDK loads AGENTS.md natively from its instruction directories, so AGENTS.md is not repeated here.",
    "",
    "# Project Context",
    "",
