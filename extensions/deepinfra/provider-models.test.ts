...
vi.mock("daisyclaw/plugin-sdk/provider-auth", async () => {
  const actual = await vi.importActual<typeof import("daisyclaw/plugin-sdk/provider-auth")>(
    "daisyclaw/plugin-sdk/provider-auth",
  );
  return {
    ...actual,
...
describe("DEEPINFRA_MODELS_URL", () => {
  it("points at /v1/openai/models with the daisyclaw sort + filter=with_meta gate", () => {
    expect(DEEPINFRA_MODELS_URL).toBe(
      "https://api.deepinfra.com/v1/openai/models?sort_by=daisyclaw&filter=with_meta",
    );
  });
});
...
describe("hasDeepInfraApiKey", () => {
  ...
  it("falls back to the auth-profile store when no env var is set", () => {
    ...
    expect(hasDeepInfraApiKey({ env: {}, agentDir: "/tmp/daisyclaw-agent" })).toBe(true);
  ...
  it("returns false when env is empty and the auth-profile store has no deepinfra profile", () => {
    ...
    expect(hasDeepInfraApiKey({ env: {}, agentDir: "/tmp/daisyclaw-agent" })).toBe(false);
...
describe("discoverDeepInfraModels (chat-only shim)", () => {
  ...
  it("fetches the daisyclaw-projection endpoint and parses chat-surface entries when an API key is configured", async () => {
