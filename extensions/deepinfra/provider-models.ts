...
import { isProviderApiKeyConfigured } from "daisyclaw/plugin-sdk/provider-auth";
import {
...
import { asPositiveSafeInteger } from "daisyclaw/plugin-sdk/string-coerce-runtime";
...
export const DEEPINFRA_MODELS_URL = `${DEEPINFRA_BASE_URL}/models?sort_by=daisyclaw&filter=with_meta`;
...
// Static fallback. Chat rows live in daisyclaw.plugin.json (manifest-validated);
...
// `/v1/openai/models?sort_by=daisyclaw&filter=with_meta` projection is the
...
// `isProviderApiKeyConfigured` helper (covers SecretRef / `DAISYCLAW_LIVE_*`
// indirection too).
