...
import type { DaisyClawPluginApi } from "./api.js";
...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredDaisyClawTmpDir: vi.fn(() => path.join(os.tmpdir(), "daisyclaw-device-pair-tests")),
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredDaisyClawTmpDir: pluginApiMocks.resolvePreferredDaisyClawTmpDir,
...
function createApi(params?: {
  ...
  runtime?: DaisyClawPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as DaisyClawPluginApi["runtime"],
...
function registerPairCommand(params?: {
  ...
  let command: DaisyClawPluginCommandDefinition | undefined;
...
function createChannelRuntime(
  ...
  } as unknown as DaisyClawPluginApi["runtime"];
}
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredDaisyClawTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredDaisyClawTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the DaisyClaw iOS app:");
    ...
    expect(text).not.toContain("![DaisyClaw pairing QR]");
  });
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the DaisyClaw iOS app:");
...
describe("device-pair /pair default setup code", () => {
  ...
  it("allows mdns cleartext setup urls", async () => {
    const command = registerPairCommand({
      pluginConfig: {
        publicUrl: "ws://daisyclaw.local:18789",
      },
    });
    const result = await command.handler(
      createCommandContext({
    ...
    expect(pluginApiMocks.issueDeviceBootstrapToken).toHaveBeenCalledTimes(1);
    expect(requireText(result)).toContain("Gateway: ws://daisyclaw.local:18789");
  });
