...
vi.mock("daisyclaw/plugin-sdk/runtime-env", () => ({
...
import type { DaisyClawPluginServiceContext } from "../api.js";
import { emitDiagnosticEvent } from "../api.js";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/daisyclaw-diagnostics-otel-test";
...
const ORIGINAL_DAISYCLAW_OTEL_PRELOADED = process.env.DAISYCLAW_OTEL_PRELOADED;
...
type OtelContextFlags = {
  ...
  captureContent?: NonNullable<
    NonNullable<DaisyClawPluginServiceContext["config"]["diagnostics"]>["otel"]
  >["captureContent"];
};
...
function createTraceOnlyContext(endpoint: string): DaisyClawPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  beforeEach(() => {
    resetDiagnosticEventsForTest();
    delete process.env.DAISYCLAW_OTEL_PRELOADED;
  ...
  afterEach(() => {
    resetDiagnosticEventsForTest();
    if (ORIGINAL_DAISYCLAW_OTEL_PRELOADED === undefined) {
      delete process.env.DAISYCLAW_OTEL_PRELOADED;
    } else {
      process.env.DAISYCLAW_OTEL_PRELOADED = ORIGINAL_DAISYCLAW_OTEL_PRELOADED;
    }
  ...
  test("drops camelCase and snake_case diagnostic id log attributes before export", async () => {
    ...
    expect(emitCall.attributes?.["daisyclaw.provider"]).toBe("openai");
    for (const key of [
      "daisyclaw.callId",
      "daisyclaw.call_id",
      "daisyclaw.chatId",
      "daisyclaw.chat_id",
      "daisyclaw.messageId",
      "daisyclaw.message_id",
      "daisyclaw.parentSpanId",
      "daisyclaw.parent_span_id",
      "daisyclaw.runId",
      "daisyclaw.run_id",
      "daisyclaw.sessionId",
      "daisyclaw.session_id",
      "daisyclaw.sessionKey",
      "daisyclaw.session_key",
      "daisyclaw.spanId",
      "daisyclaw.span_id",
      "daisyclaw.toolCallId",
      "daisyclaw.tool_call_id",
      "daisyclaw.traceId",
      "daisyclaw.trace_id",
    ]) {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("daisyclaw.webhook.received")?.add).toHaveBeenCalledWith(1, {
      "daisyclaw.channel": "telegram",
      "daisyclaw.webhook": "telegram-post",
    });
    expect(
      telemetryState.histograms.get("daisyclaw.webhook.duration_ms")?.record,
    ).toHaveBeenCalledWith(120, {
      "daisyclaw.channel": "telegram",
      "daisyclaw.webhook": "telegram-post",
    });
    expect(telemetryState.counters.get("daisyclaw.message.queued")?.add).toHaveBeenCalledWith(1, {
      "daisyclaw.channel": "telegram",
      "daisyclaw.source": "telegram",
    });
    expect(telemetryState.histograms.get("daisyclaw.queue.depth")?.record).toHaveBeenCalledTimes(2);
    expect(telemetryState.histograms.get("daisyclaw.queue.depth")?.record).toHaveBeenCalledWith(2, {
      "daisyclaw.channel": "telegram",
      "daisyclaw.source": "telegram",
    });
    expect(telemetryState.histograms.get("daisyclaw.queue.depth")?.record).toHaveBeenCalledWith(3, {
      "daisyclaw.lane": "main",
    });
    expect(telemetryState.counters.get("daisyclaw.message.processed")?.add).toHaveBeenCalledWith(1, {
      "daisyclaw.channel": "telegram",
      "daisyclaw.outcome": "completed",
    });
    expect(telemetryState.counters.get("daisyclaw.message.received")?.add).toHaveBeenCalledWith(1, {
      "daisyclaw.channel": "telegram",
      "daisyclaw.source": "webhook",
    });
    expect(telemetryState.counters.get("daisyclaw.message.received")?.add).toHaveBeenCalledWith(1, {
      "daisyclaw.channel": "unknown",
      "daisyclaw.source": "unknown",
    });
    expect(
      telemetryState.counters.get("daisyclaw.message.dispatch.started")?.add,
    ...
    expect(
      telemetryState.counters.get("daisyclaw.message.dispatch.completed")?.add,
    ...
    expect(
      ...
      expect.objectContaining({
        "daisyclaw.channel": "unknown",
        "daisyclaw.reason": "none",
        "daisyclaw.source": "unknown",
      }),
    );
    expect(
      telemetryState.histograms.get("daisyclaw.message.dispatch.duration_ms")?.record,
    ).toHaveBeenCalledWith(
      25,
      expect.objectContaining({
        "daisyclaw.channel": "telegram",
    ...
    expect(
      telemetryState.histograms.get("daisyclaw.message.duration_ms")?.record,
    ).toHaveBeenCalledWith(55, {
      "daisyclaw.channel": "telegram",
      "daisyclaw.outcome": "completed",
    });
    expect(telemetryState.histograms.get("daisyclaw.queue.wait_ms")?.record).toHaveBeenCalledWith(
      10,
      {
        "daisyclaw.lane": "main",
      },
    );
    expect(telemetryState.counters.get("daisyclaw.session.stuck")?.add).toHaveBeenCalledTimes(1);
    expect(telemetryState.counters.get("daisyclaw.session.stuck")?.add).toHaveBeenCalledWith(1, {
      "daisyclaw.state": "processing",
    });
    expect(
      telemetryState.histograms.get("daisyclaw.session.stuck_age_ms")?.record,
    ).toHaveBeenCalledWith(125_000, {
      "daisyclaw.state": "processing",
    });
    expect(telemetryState.counters.get("daisyclaw.run.attempt")?.add).toHaveBeenCalledWith(1, {
      "daisyclaw.attempt": 2,
    });
    ...
    expect(telemetryState.counters.get("daisyclaw.session.turn.created")?.add).toHaveBeenCalledWith(
      1,
      {
        "daisyclaw.agent": "agent.default",
        "daisyclaw.channel": "telegram",
        "daisyclaw.trigger": "user",
      },
    );

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("daisyclaw.webhook.processed");
    expect(spanNames).toContain("daisyclaw.message.processed");
    expect(spanNames).toContain("daisyclaw.session.stuck");
    const webhookSpanOptions = startedSpanOptions("daisyclaw.webhook.processed");
    expect(webhookSpanOptions?.attributes).not.toHaveProperty("daisyclaw.chatId");
    expect(webhookSpanOptions?.startTime).toBeTypeOf("number");
    const messageSpanOptions = startedSpanOptions("daisyclaw.message.processed");
    expect(messageSpanOptions?.attributes?.["daisyclaw.channel"]).toBe("telegram");
    expect(messageSpanOptions?.attributes?.["daisyclaw.outcome"]).toBe("completed");
    expect(messageSpanOptions?.attributes?.["daisyclaw.reason"]).toBe("unknown");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("daisyclaw.chatId");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("daisyclaw.messageId");
  ...
  test("uses a preloaded OpenTelemetry SDK without dropping diagnostic listeners", async () => {
    process.env.DAISYCLAW_OTEL_PRELOADED = "1";
    const service = createDiagnosticsOtelService();
    const ctx = createOtelContext(OTEL_TEST_ENDPOINT, { traces: true, metrics: true, logs: true });
    await service.start(ctx);
    ...
    const runDurationRecordCall = lastHistogramRecord("daisyclaw.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    const runDurationAttributes = runDurationRecordCall?.[1];
    expect(runDurationAttributes?.["daisyclaw.provider"]).toBe("openai");
    expect(runDurationAttributes?.["daisyclaw.model"]).toBe("gpt-5.4");
    const runSpanOptions = startedSpanOptions("daisyclaw.run");
    expect(runSpanOptions?.attributes?.["daisyclaw.outcome"]).toBe("completed");
    expect(logEmit).toHaveBeenCalled();
  ...
  test("emits and records bounded telemetry exporter health events", async () => {
    ...
    expect(
      telemetryState.counters.get("daisyclaw.telemetry.exporter.events")?.add,
    ).toHaveBeenCalledWith(1, {
      "daisyclaw.exporter": "diagnostics-otel",
      "daisyclaw.signal": "logs",
      "daisyclaw.status": "started",
      "daisyclaw.reason": "configured",
    });
  ...
  test("records liveness warning diagnostics", async () => {
    ...
    expect(telemetryState.counters.get("daisyclaw.liveness.warning")?.add).toHaveBeenCalledWith(1, {
      "daisyclaw.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("daisyclaw.liveness.event_loop_delay_p99_ms")?.record,
    ).toHaveBeenCalledWith(250, {
      "daisyclaw.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("daisyclaw.liveness.cpu_core_ratio")?.record,
    ).toHaveBeenCalledWith(1.4, {
      "daisyclaw.liveness.reason": "event_loop_delay:cpu",
    });
    const livenessSpanOptions = startedSpanOptions("daisyclaw.liveness.warning");
    expect(livenessSpanOptions?.attributes?.["daisyclaw.liveness.reason"]).toBe(
      "event_loop_delay:cpu",
    );
    expect(livenessSpanOptions?.attributes?.["daisyclaw.liveness.active"]).toBe(2);
    expect(livenessSpanOptions?.attributes?.["daisyclaw.liveness.queued"]).toBe(4);
    const span = telemetryState.spans.find((item) => item.name === "daisyclaw.liveness.warning");
  ...
  test("records oversized payload metrics without raw identifiers", async () => {
    ...
    expect(telemetryState.counters.get("daisyclaw.payload.large")?.add).toHaveBeenCalledWith(1, {
      "daisyclaw.payload.action": "rejected",
      "daisyclaw.payload.surface": "gateway.frame",
      "daisyclaw.channel": "web",
      "daisyclaw.plugin": "none",
      "daisyclaw.reason": "body-too-large",
    });
    expect(
      telemetryState.histograms.get("daisyclaw.payload.large_bytes")?.record,
  ...
  test("reports log exporter emit failures without exporting raw error text", async () => {
    ...
    expect(
      ...
      "daisyclaw.exporter": "diagnostics-otel",
      "daisyclaw.signal": "logs",
      "daisyclaw.status": "failure",
      "daisyclaw.reason": "emit_failed",
      "daisyclaw.errorCategory": "TypeError",
    });
  ...
  test("ignores untrusted telemetry exporter events for OTEL metrics", async () => {
    ...
    await service.start(ctx);
    telemetryState.counters.get("daisyclaw.telemetry.exporter.events")?.add.mockClear();
    emitDiagnosticEvent({
  ...
  test("records hook-blocked run metrics with safe blocker originator", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("daisyclaw.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["daisyclaw.outcome"]).toBe("blocked");
    expect(runDurationRecordCall?.[1]?.["daisyclaw.blocked_by"]).toBe("policy-plugin");
  ...
  test("honors disabled traces when an OpenTelemetry SDK is preloaded", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("daisyclaw.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["daisyclaw.provider"]).toBe("openai");
    expect(telemetryState.tracer.startSpan).not.toHaveBeenCalled();
  ...
  test("treats omitted diagnostics enabled flag as enabled", async () => {
    ...
    const modelCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "daisyclaw.model.call",
    );
    const attrs = (modelCall?.[1] as { attributes?: Record<string, unknown> } | undefined)
      ?.attributes;
    expect(attrs?.["daisyclaw.content.input_messages"]).toBe("user prompt");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["daisyclaw.token"];
  ...
  test("does not attach untrusted diagnostic trace context to exported logs", async () => {
    ...
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "daisyclaw.traceId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "daisyclaw.spanId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "daisyclaw.traceFlags")).toBe(false);
  ...
  test("bounds plugin-emitted log attributes and omits source paths", async () => {
    ...
    emitDiagnosticEvent({
      ...
      attributes,
      code: {
        filepath: "/Users/alice/daisyclaw/src/private.ts",
        line: 42,
        functionName: "handler",
        location: "/Users/alice/daisyclaw/src/private.ts:42",
      },
    ...
    expect(String(emitCall.attributes["daisyclaw.good"])).toMatch(/^y+/);
    expect(emitCall.attributes["code.lineno"]).toBe(42);
    expect(emitCall.attributes["code.function"]).toBe("handler");
    expect(String(emitCall.attributes["daisyclaw.good"]).length).toBeLessThanOrEqual(4200);
    expect(Object.hasOwn(emitCall.attributes, `daisyclaw.${PROTO_KEY}`)).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "daisyclaw.constructor")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "daisyclaw.prototype")).toBe(false);
    expect(
      Object.hasOwn(
        emitCall.attributes,
        "daisyclaw.sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
      ),
    ).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "daisyclaw.bad key")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "code.filepath")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "daisyclaw.code.location")).toBe(false);
    await service.stop?.(ctx);
  });
  ...
  test("does not parent diagnostic event spans from plugin-emittable trace context", async () => {
    ...
    const modelUsageCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "daisyclaw.model.usage",
    );
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
  ...
  test("exports GenAI client token usage histogram for input and output only", async () => {
    ...
    const tokens = telemetryState.counters.get("daisyclaw.tokens");
    expect(tokens?.add).toHaveBeenCalledWith(12, {
      "daisyclaw.channel": "webchat",
      "daisyclaw.agent": "ops",
      "daisyclaw.provider": "openai",
      "daisyclaw.model": "gpt-5.4",
      "daisyclaw.token": "input",
    });
  ...
  test("bounds agent identifiers on model usage metric attributes", async () => {
    ...
    expect(telemetryState.counters.get("daisyclaw.tokens")?.add).toHaveBeenCalledWith(2, {
      "daisyclaw.channel": "unknown",
      "daisyclaw.agent": "unknown",
      "daisyclaw.provider": "openai",
      "daisyclaw.model": "gpt-5.4",
      "daisyclaw.token": "input",
    });
    expect(
      JSON.stringify(telemetryState.counters.get("daisyclaw.tokens")?.add.mock.calls),
  ...
  test("drops session-shaped queue lane metric attributes", async () => {
    ...
    expect(telemetryState.counters.get("daisyclaw.queue.lane.enqueue")?.add).toHaveBeenCalledWith(
      1,
      {
        "daisyclaw.lane": "session",
      },
    );
    expect(
      JSON.stringify(telemetryState.counters.get("daisyclaw.queue.lane.enqueue")?.add.mock.calls),
    ).not.toContain("Agent:qa:otel-trace-smoke");
    await service.stop?.(ctx);
  });
  ...
  test("keeps only the bounded prefix from scoped queue lane metric attributes", async () => {
    ...
    expect(telemetryState.counters.get("daisyclaw.queue.lane.enqueue")?.add).toHaveBeenCalledWith(
      1,
      {
        "daisyclaw.lane": "dreaming-narrative",
      },
    );
    expect(
      JSON.stringify(telemetryState.counters.get("daisyclaw.queue.lane.enqueue")?.add.mock.calls),
  ...
  test("exports GenAI usage attributes on model usage spans without diagnostic identifiers", async () => {
    ...
    const modelUsageOptions = startedSpanOptions("daisyclaw.model.usage");
    ...
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "daisyclaw.sessionKey")).toBe(false);
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "daisyclaw.sessionId")).toBe(false);
  ...
  test("exports skill usage counter and span without raw identifiers", async () => {
    ...
    const expectedAttrs = {
      "daisyclaw.agent": "main",
      "daisyclaw.skill.activation": "read",
      "daisyclaw.skill.name": "tiny-llm-brainstorm",
      "daisyclaw.skill.source": "workspace",
      "daisyclaw.toolName": "read",
    };
    expect(telemetryState.counters.get("daisyclaw.skill.used")?.add).toHaveBeenCalledWith(
      1,
      expectedAttrs,
    );
    const skillSpanCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "daisyclaw.skill.used",
    );
  ...
  test("exports run, model call, and tool execution lifecycle spans", async () => {
    ...
    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("daisyclaw.run");
    expect(spanNames).toContain("daisyclaw.model.call");
    expect(spanNames).toContain("daisyclaw.harness.run");
    expect(spanNames).toContain("daisyclaw.tool.execution");

    const runOptions = startedSpanOptions("daisyclaw.run");
    expect(runOptions?.attributes?.["daisyclaw.outcome"]).toBe("completed");
    expect(runOptions?.attributes?.["daisyclaw.provider"]).toBe("openai");
    expect(runOptions?.attributes?.["daisyclaw.model"]).toBe("gpt-5.4");
    expect(runOptions?.attributes?.["daisyclaw.channel"]).toBe("webchat");
    ...
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "daisyclaw.runId")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "daisyclaw.sessionKey")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "daisyclaw.traceId")).toBe(false);
    expect(runOptions?.startTime).toBeTypeOf("number");

    const modelCall = startedSpanCall("daisyclaw.model.call");
    ...
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "daisyclaw.callId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "daisyclaw.runId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "daisyclaw.sessionKey")).toBe(false);
    ...
    const harnessCall = startedSpanCall("daisyclaw.harness.run");
    const harnessOptions = harnessCall?.[1];
    expect(harnessOptions?.attributes?.["daisyclaw.harness.id"]).toBe("codex");
    expect(harnessOptions?.attributes?.["daisyclaw.harness.plugin"]).toBe("codex-plugin");
    expect(harnessOptions?.attributes?.["daisyclaw.outcome"]).toBe("completed");
    expect(harnessOptions?.attributes?.["daisyclaw.provider"]).toBe("codex");
    expect(harnessOptions?.attributes?.["daisyclaw.model"]).toBe("gpt-5.4");
    expect(harnessOptions?.attributes?.["daisyclaw.channel"]).toBe("qa");
    expect(harnessOptions?.attributes?.["daisyclaw.harness.result_classification"]).toBe(
      "reasoning-only",
    );
    expect(harnessOptions?.attributes?.["daisyclaw.harness.yield_detected"]).toBe(true);
    expect(harnessOptions?.attributes?.["daisyclaw.harness.items.started"]).toBe(3);
    expect(harnessOptions?.attributes?.["daisyclaw.harness.items.completed"]).toBe(2);
    expect(harnessOptions?.attributes?.["daisyclaw.harness.items.active"]).toBe(1);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "daisyclaw.runId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "daisyclaw.sessionId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "daisyclaw.sessionKey")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "daisyclaw.traceId")).toBe(false);
    expect(harnessOptions?.startTime).toBeTypeOf("number");
    expect(harnessCall?.[2]).toBeUndefined();

    const toolCall = startedSpanCall("daisyclaw.tool.execution");
    const toolOptions = toolCall?.[1];
    expect(toolOptions?.attributes?.["daisyclaw.toolName"]).toBe("read");
    expect(toolOptions?.attributes?.["daisyclaw.tool.source"]).toBe("core");
    expect(toolOptions?.attributes?.["daisyclaw.errorCategory"]).toBe("TypeError");
    expect(toolOptions?.attributes?.["daisyclaw.errorCode"]).toBe("429");
    expect(toolOptions?.attributes?.["daisyclaw.tool.params.kind"]).toBe("object");
    expect(toolOptions?.attributes?.["gen_ai.tool.name"]).toBe("read");
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "daisyclaw.toolCallId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "daisyclaw.runId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "daisyclaw.sessionKey")).toBe(false);
    expect(toolOptions?.startTime).toBeTypeOf("number");
    expect(toolCall?.[2]).toBeUndefined();

    const modelCallDuration = lastHistogramRecord("daisyclaw.model_call.duration_ms");
    expect(modelCallDuration?.[0]).toBe(80);
    expect(modelCallDuration?.[1]?.["daisyclaw.provider"]).toBe("openai");
    expect(modelCallDuration?.[1]?.["daisyclaw.model"]).toBe("gpt-5.4");
    const requestBytes = lastHistogramRecord("daisyclaw.model_call.request_bytes");
    expect(requestBytes?.[0]).toBe(1234);
    expect(requestBytes?.[1]?.["daisyclaw.provider"]).toBe("openai");
    expect(requestBytes?.[1]?.["daisyclaw.model"]).toBe("gpt-5.4");
    const responseBytes = lastHistogramRecord("daisyclaw.model_call.response_bytes");
    expect(responseBytes?.[0]).toBe(567);
    expect(responseBytes?.[1]?.["daisyclaw.provider"]).toBe("openai");
    expect(responseBytes?.[1]?.["daisyclaw.model"]).toBe("gpt-5.4");
    const timeToFirstByte = lastHistogramRecord("daisyclaw.model_call.time_to_first_byte_ms");
    expect(timeToFirstByte?.[0]).toBe(45);
    expect(timeToFirstByte?.[1]?.["daisyclaw.provider"]).toBe("openai");
    expect(timeToFirstByte?.[1]?.["daisyclaw.model"]).toBe("gpt-5.4");
    const modelSpanAttributes = firstSpanAttributes("daisyclaw.model.call");
    expect(modelSpanAttributes["daisyclaw.model_call.request_bytes"]).toBe(1234);
    expect(modelSpanAttributes["daisyclaw.model_call.response_bytes"]).toBe(567);
    expect(modelSpanAttributes["daisyclaw.model_call.time_to_first_byte_ms"]).toBe(45);
    const runDuration = lastHistogramRecord("daisyclaw.run.duration_ms");
    expect(runDuration?.[0]).toBe(100);
    expect(Object.hasOwn(runDuration?.[1] ?? {}, "daisyclaw.runId")).toBe(false);
    const harnessDuration = lastHistogramRecord("daisyclaw.harness.duration_ms");
    expect(harnessDuration?.[0]).toBe(90);
    expect(harnessDuration?.[1]?.["daisyclaw.harness.id"]).toBe("codex");
    expect(harnessDuration?.[1]?.["daisyclaw.harness.plugin"]).toBe("codex-plugin");
    expect(harnessDuration?.[1]?.["daisyclaw.outcome"]).toBe("completed");
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "daisyclaw.runId")).toBe(false);
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "daisyclaw.sessionKey")).toBe(false);
    const toolDuration = lastHistogramRecord("daisyclaw.tool.execution.duration_ms");
    expect(toolDuration?.[0]).toBe(20);
    expect(toolDuration?.[1]?.["daisyclaw.tool.source"]).toBe("core");
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "daisyclaw.errorCode")).toBe(false);
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "daisyclaw.runId")).toBe(false);

    const toolSpan = spanByName("daisyclaw.tool.execution");
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
    expect(firstSpanEndTime("daisyclaw.tool.execution")).toBeTypeOf("number");
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    await service.stop?.(ctx);
  });
  ...
  test("exports model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("daisyclaw.model.failover");
    expect(failoverOptions?.attributes?.["daisyclaw.provider"]).toBe("anthropic");
    expect(failoverOptions?.attributes?.["daisyclaw.model"]).toBe("claude-opus-4-6");
    expect(failoverOptions?.attributes?.["daisyclaw.failover.to_provider"]).toBe("openai");
    expect(failoverOptions?.attributes?.["daisyclaw.failover.to_model"]).toBe("gpt-5.4");
    expect(failoverOptions?.attributes?.["daisyclaw.failover.reason"]).toBe("overloaded");
    expect(failoverOptions?.attributes?.["daisyclaw.failover.suspended"]).toBe(true);
    expect(failoverOptions?.attributes?.["daisyclaw.failover.cascade_depth"]).toBe(1);
    expect(failoverOptions?.attributes?.["daisyclaw.lane"]).toBe("main");
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "daisyclaw.sessionId")).toBe(false);
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "daisyclaw.sessionKey")).toBe(false);
    expect(failoverOptions?.startTime).toBeTypeOf("number");
    expect(firstSpanEndTime("daisyclaw.model.failover")).toBeTypeOf("number");
    expect(firstCounterAddCall("daisyclaw.model.failover")).toStrictEqual([
      1,
      {
        "daisyclaw.failover.reason": "overloaded",
        "daisyclaw.failover.suspended": "true",
        "daisyclaw.lane": "main",
        "daisyclaw.model": "claude-opus-4-6",
        "daisyclaw.provider": "anthropic",
        "daisyclaw.failover.to_model": "gpt-5.4",
        "daisyclaw.failover.to_provider": "openai",
      },
    ]);
    await service.stop?.(ctx);
  });
  ...
  test("records blocked tool metrics even when traces are disabled", async () => {
    ...
    expect(firstCounterAddCall("daisyclaw.tool.execution.blocked")).toStrictEqual([
      1,
      {
        "daisyclaw.toolName": "browser",
        "daisyclaw.tool.source": "mcp",
        "gen_ai.tool.name": "browser",
        "daisyclaw.tool.owner": "browser-tools",
        "daisyclaw.tool.params.kind": "object",
        "daisyclaw.deniedReason": "tools.deny",
      },
    ]);
    expect(telemetryState.tracer.startSpan).not.toHaveBeenCalledWith(
      "daisyclaw.tool.execution",
      expect.anything(),
      expect.anything(),
    );
  ...
  test("drops session-shaped queue lanes from model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("daisyclaw.model.failover");
    expect(failoverOptions?.attributes?.["daisyclaw.lane"]).toBe("session");
  ...
  test("maps model call APIs to GenAI operation names and error type", async () => {
    ...
    const modelCallAttrs = telemetryState.tracer.startSpan.mock.calls
      .filter((call) => call[0] === "daisyclaw.model.call")
  ...
  test("uses latest GenAI inference span shape only when semconv opt-in is set", async () => {
    ...
    expect(startedSpanOptions("daisyclaw.model.call")).toBeUndefined();
  ...
  test("records upstream request id hashes as model call span events only", async () => {
    ...
    const modelCallOptions = startedSpanOptions("daisyclaw.model.call");
    expect(modelCallOptions?.attributes?.["daisyclaw.failureKind"]).toBe("terminated");
    expect(
      Object.hasOwn(modelCallOptions?.attributes ?? {}, "daisyclaw.upstreamRequestIdHash"),
    ).toBe(false);
    expect(modelCallOptions?.startTime).toBeTypeOf("number");
    const span = telemetryState.spans.find((candidate) => candidate.name === "daisyclaw.model.call");
    expect(span?.addEvent).toHaveBeenCalledWith("daisyclaw.provider.request", {
      "daisyclaw.upstreamRequestIdHash": "sha256:123456abcdef",
    });
    const modelCallDuration = lastHistogramRecord("daisyclaw.model_call.duration_ms");
    expect(modelCallDuration?.[0]).toBe(40);
    expect(modelCallDuration?.[1]?.["daisyclaw.failureKind"]).toBe("terminated");
    expect(Object.hasOwn(modelCallDuration?.[1] ?? {}, "daisyclaw.upstreamRequestIdHash")).toBe(
      false,
    );
    await service.stop?.(ctx);
  });
  ...
  test("exports trusted context assembly spans without prompt content", async () => {
    ...
    const contextCall = startedSpanCall("daisyclaw.context.assembled");
    const contextOptions = contextCall?.[1];
    const runSpan = telemetryState.spans.find((span) => span.name === "daisyclaw.run");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    expect(contextOptions?.attributes?.["daisyclaw.provider"]).toBe("openai");
    expect(contextOptions?.attributes?.["daisyclaw.model"]).toBe("gpt-5.4");
    expect(contextOptions?.attributes?.["daisyclaw.channel"]).toBe("webchat");
    expect(contextOptions?.attributes?.["daisyclaw.trigger"]).toBe("message");
    expect(contextOptions?.attributes?.["daisyclaw.context.message_count"]).toBe(12);
    expect(contextOptions?.attributes?.["daisyclaw.context.history_text_chars"]).toBe(1234);
    expect(contextOptions?.attributes?.["daisyclaw.context.history_image_blocks"]).toBe(2);
    expect(contextOptions?.attributes?.["daisyclaw.context.max_message_text_chars"]).toBe(456);
    expect(contextOptions?.attributes?.["daisyclaw.context.system_prompt_chars"]).toBe(789);
    expect(contextOptions?.attributes?.["daisyclaw.context.prompt_chars"]).toBe(42);
    expect(contextOptions?.attributes?.["daisyclaw.context.prompt_images"]).toBe(1);
    expect(contextOptions?.attributes?.["daisyclaw.context.token_budget"]).toBe(128_000);
    expect(contextOptions?.attributes?.["daisyclaw.context.reserve_tokens"]).toBe(4096);
  ...
  test("exports tool loop diagnostics without loop messages or session identifiers", async () => {
    ...
    expect(telemetryState.counters.get("daisyclaw.tool.loop")?.add).toHaveBeenCalledWith(1, {
      "daisyclaw.toolName": "process",
      "daisyclaw.loop.level": "critical",
      "daisyclaw.loop.action": "block",
      "daisyclaw.loop.detector": "known_poll_no_progress",
      "daisyclaw.loop.count": 20,
      "daisyclaw.loop.paired_tool": "read",
    });
    const loopSpanCall = startedSpanCall("daisyclaw.tool.loop");
    const loopOptions = loopSpanCall?.[1];
    expect(loopOptions?.attributes?.["daisyclaw.toolName"]).toBe("process");
    expect(loopOptions?.attributes?.["daisyclaw.loop.level"]).toBe("critical");
    expect(loopOptions?.attributes?.["daisyclaw.loop.action"]).toBe("block");
    expect(loopOptions?.attributes?.["daisyclaw.loop.detector"]).toBe("known_poll_no_progress");
    expect(loopOptions?.attributes?.["daisyclaw.loop.count"]).toBe(20);
    expect(loopOptions?.attributes?.["daisyclaw.loop.paired_tool"]).toBe("read");
    const loopSpan = telemetryState.spans.find((span) => span.name === "daisyclaw.tool.loop");
  ...
  test("exports diagnostic memory samples and pressure without session identifiers", async () => {
    ...
    expect(telemetryState.histograms.get("daisyclaw.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      100,
      {},
    );
    expect(telemetryState.histograms.get("daisyclaw.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      200,
      {
        "daisyclaw.memory.level": "critical",
        "daisyclaw.memory.reason": "rss_growth",
      },
    );
    expect(telemetryState.counters.get("daisyclaw.memory.pressure")?.add).toHaveBeenCalledWith(1, {
      "daisyclaw.memory.level": "critical",
      "daisyclaw.memory.reason": "rss_growth",
    });
    const pressureCall = startedSpanCall("daisyclaw.memory.pressure");
    const pressureOptions = pressureCall?.[1];
    expect(pressureOptions?.attributes?.["daisyclaw.memory.level"]).toBe("critical");
    expect(pressureOptions?.attributes?.["daisyclaw.memory.reason"]).toBe("rss_growth");
    expect(pressureOptions?.attributes?.["daisyclaw.memory.rss_bytes"]).toBe(200);
    expect(pressureOptions?.attributes?.["daisyclaw.memory.heap_used_bytes"]).toBe(50);
    expect(pressureOptions?.attributes?.["daisyclaw.memory.heap_total_bytes"]).toBe(90);
    expect(pressureOptions?.attributes?.["daisyclaw.memory.external_bytes"]).toBe(20);
    expect(pressureOptions?.attributes?.["daisyclaw.memory.array_buffers_bytes"]).toBe(6);
    expect(pressureOptions?.attributes?.["daisyclaw.memory.threshold_bytes"]).toBe(512);
    expect(pressureOptions?.attributes?.["daisyclaw.memory.rss_growth_bytes"]).toBe(256);
    expect(pressureOptions?.attributes?.["daisyclaw.memory.window_ms"]).toBe(60_000);
    const pressureSpan = telemetryState.spans.find(
      (span) => span.name === "daisyclaw.memory.pressure",
    );
  ...
  test("records async diagnostic queue drop summaries", async () => {
    ...
    const counter = telemetryState.counters.get("daisyclaw.diagnostic.async_queue.dropped");
    expect(counter?.add).toHaveBeenCalledWith(4, {
      "daisyclaw.diagnostic.async_queue.drop_class": "total",
    });
    expect(counter?.add).toHaveBeenCalledWith(1, {
      "daisyclaw.diagnostic.async_queue.drop_class": "trusted",
    });
    expect(counter?.add).toHaveBeenCalledWith(2, {
      "daisyclaw.diagnostic.async_queue.drop_class": "untrusted",
    });
    expect(counter?.add).toHaveBeenCalledWith(1, {
      "daisyclaw.diagnostic.async_queue.drop_class": "priority",
    });
  ...
  test("parents trusted diagnostic lifecycle spans from active started spans", async () => {
    ...
    const runSpan = telemetryState.spans.find((span) => span.name === "daisyclaw.run");
    const modelSpan = telemetryState.spans.find((span) => span.name === "daisyclaw.model.call");
    const toolSpan = telemetryState.spans.find((span) => span.name === "daisyclaw.tool.execution");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    ...
    expect(parentBySpanName["daisyclaw.run"]).toBeUndefined();
    expect(parentBySpanName["daisyclaw.model.call"]).toBe(runSpanId);
    expect(parentBySpanName["daisyclaw.tool.execution"]).toBe(modelSpanId);
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
  ...
  test("correlates one channel message waterfall across message, harness, usage, and model spans", async () => {
    ...
    const messageSpan = spanByName("daisyclaw.message.processed");
    const harnessSpan = spanByName("daisyclaw.harness.run");
    const runSpan = spanByName("daisyclaw.run");
    const usageSpan = spanByName("daisyclaw.model.usage");
    const modelCallSpan = spanByName("daisyclaw.model.call");
    ...
    expect(parentBySpanName["daisyclaw.message.processed"]?.spanId).toBe(SPAN_ID);
    expect(parentBySpanName["daisyclaw.harness.run"]?.spanId).toBe(messageSpanContext.spanId);
    expect(parentBySpanName["daisyclaw.run"]?.spanId).toBe(harnessSpanContext.spanId);
    expect(parentBySpanName["daisyclaw.model.usage"]?.spanId).toBe(harnessSpanContext.spanId);
    expect(parentBySpanName["daisyclaw.model.call"]?.spanId).toBe(runSpanContext.spanId);
    await service.stop?.(ctx);
  });
  ...
  test("does not force a remote parent for root message lifecycle helpers", async () => {
    ...
    expect(spanByName("daisyclaw.message.processed").spanContext().traceId).toBe(TRACE_ID);
    expect(startedSpanParentContexts("daisyclaw.message.processed")[0]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("parents outbound delivery spans under the active message lifecycle span", async () => {
    ...
    const messageSpanContext = spanByName("daisyclaw.message.processed").spanContext();
    const deliveryParentContexts = startedSpanParentContexts("daisyclaw.message.delivery");
  ...
  test("correlates skipped duplicate message lifecycle helpers to the active inbound trace", async () => {
    ...
    const messageSpan = spanByName("daisyclaw.message.processed");
    const messageSpanContext = messageSpan.spanContext();
    const parentContext = startedSpanParentContexts("daisyclaw.message.processed")[0];

    expect(messageSpanContext.traceId).toBe(TRACE_ID);
    expect(parentContext?.traceId).toBe(TRACE_ID);
    expect(parentContext?.spanId).toBe(SPAN_ID);
    expect(firstSpanAttributes("daisyclaw.message.processed")["daisyclaw.reason"]).toBe("duplicate");
    expect(messageSpan.end).toHaveBeenCalledTimes(1);
    await service.stop?.(ctx);
  });
  ...
  test("does not retain fallback message processed spans as active parents", async () => {
    ...
    expect(spanByName("daisyclaw.message.processed").end).toHaveBeenCalledTimes(1);
    ...
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    expect(startedSpanCall("daisyclaw.harness.run")?.[2]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("retains trusted run context long enough for exact post-completion usage parenting", async () => {
    ...
    expect(firstSpanEndTime("daisyclaw.run")).toBeTypeOf("number");
    await service.stop?.(ctx);
  });
  ...
  test("does not parent sibling active runs through shared upstream aliases", async () => {
    ...
    const runContexts = startedSpanParentContextsByName("daisyclaw.run");
  ...
  test("parents retained upstream alias events only when the owner matches", async () => {
    ...
    const runSpanContext = spanByName("daisyclaw.run").spanContext();
    const modelParentContext = startedSpanParentContexts("daisyclaw.model.call")[0];
  ...
  test("parents multi-batch late model spans from the retained run context", async () => {
    ...
    const modelParentContexts = startedSpanParentContexts("daisyclaw.model.call");
  ...
  test("removes retained run contexts after queued diagnostics drain", async () => {
    ...
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    expect(startedSpanCall("daisyclaw.model.usage")?.[2]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not force remote parents for completed-only trusted lifecycle spans", async () => {
    ...
    expect(parentBySpanName["daisyclaw.run"]).toBeUndefined();
    expect(parentBySpanName["daisyclaw.model.call"]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not parent untrusted diagnostic lifecycle spans from injected trace ids", async () => {
    ...
    expect(parentBySpanName["daisyclaw.run"]).toBeUndefined();
    expect(parentBySpanName["daisyclaw.model.call"]).toBeUndefined();
    expect(parentBySpanName["daisyclaw.tool.execution"]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not create live started spans for untrusted lifecycle diagnostics", async () => {
    ...
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter((call) => call[0] === "daisyclaw.run"),
    ).toHaveLength(1);
    expect(
    ...
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "daisyclaw.tool.execution",
      ),
    ).toHaveLength(1);
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "daisyclaw.harness.run",
      ),
    ).toHaveLength(1);
    await service.stop?.(ctx);
  });
  ...
  test("exports exec process spans without command text", async () => {
    ...
    const execDuration = lastHistogramRecord("daisyclaw.exec.duration_ms");
    expect(execDuration?.[0]).toBe(30);
    expect(execDuration?.[1]?.["daisyclaw.exec.target"]).toBe("host");
    expect(execDuration?.[1]?.["daisyclaw.exec.mode"]).toBe("child");
    expect(execDuration?.[1]?.["daisyclaw.outcome"]).toBe("failed");
    expect(execDuration?.[1]?.["daisyclaw.failureKind"]).toBe("runtime-error");

    const execCall = startedSpanCall("daisyclaw.exec");
    const execOptions = execCall?.[1];
    expect(execOptions?.attributes?.["daisyclaw.exec.target"]).toBe("host");
    expect(execOptions?.attributes?.["daisyclaw.exec.mode"]).toBe("child");
    expect(execOptions?.attributes?.["daisyclaw.outcome"]).toBe("failed");
    expect(execOptions?.attributes?.["daisyclaw.exec.command_length"]).toBe(42);
    expect(execOptions?.attributes?.["daisyclaw.exec.exit_code"]).toBe(1);
    expect(execOptions?.attributes?.["daisyclaw.exec.timed_out"]).toBe(false);
    expect(execOptions?.attributes?.["daisyclaw.failureKind"]).toBe("runtime-error");
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "daisyclaw.exec.command")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "daisyclaw.exec.workdir")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "daisyclaw.sessionKey")).toBe(false);
    expect(execOptions?.startTime).toBeTypeOf("number");

    const execSpan = spanByName("daisyclaw.exec");
    expect(execSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "runtime-error",
    });
    expect(firstSpanEndTime("daisyclaw.exec")).toBeTypeOf("number");
    await service.stop?.(ctx);
  });
  ...
  test("exports message delivery spans and metrics with low-cardinality attributes", async () => {
    ...
    expect(
      telemetryState.counters.get("daisyclaw.message.delivery.started")?.add,
    ).toHaveBeenCalledWith(1, {
      "daisyclaw.channel": "matrix",
      "daisyclaw.delivery.kind": "text",
    });
    const deliveryDurationRecords = telemetryState.histograms.get(
      "daisyclaw.message.delivery.duration_ms",
    )?.record.mock.calls as Array<[unknown, Record<string, unknown>]>;
    expect(deliveryDurationRecords[0]?.[0]).toBe(25);
    expect(deliveryDurationRecords[0]?.[1]["daisyclaw.channel"]).toBe("matrix");
    expect(deliveryDurationRecords[0]?.[1]["daisyclaw.delivery.kind"]).toBe("text");
    expect(deliveryDurationRecords[0]?.[1]["daisyclaw.outcome"]).toBe("completed");
    expect(deliveryDurationRecords[1]?.[0]).toBe(40);
    expect(deliveryDurationRecords[1]?.[1]["daisyclaw.channel"]).toBe("discord");
    expect(deliveryDurationRecords[1]?.[1]["daisyclaw.delivery.kind"]).toBe("media");
    expect(deliveryDurationRecords[1]?.[1]["daisyclaw.outcome"]).toBe("error");
    expect(deliveryDurationRecords[1]?.[1]["daisyclaw.errorCategory"]).toBe("TypeError");

    const deliverySpanCalls = telemetryState.tracer.startSpan.mock.calls.filter(
      (call) => call[0] === "daisyclaw.message.delivery",
    );
    ...
    const firstDeliveryOptions = deliverySpanCalls[0]?.[1] as
      ...
      | undefined;
    expect(firstDeliveryOptions?.attributes?.["daisyclaw.channel"]).toBe("matrix");
    expect(firstDeliveryOptions?.attributes?.["daisyclaw.delivery.kind"]).toBe("text");
    expect(firstDeliveryOptions?.attributes?.["daisyclaw.outcome"]).toBe("completed");
    expect(firstDeliveryOptions?.attributes?.["daisyclaw.delivery.result_count"]).toBe(1);
    expect(firstDeliveryOptions?.startTime).toBeTypeOf("number");
    const secondDeliveryOptions = deliverySpanCalls[1]?.[1] as
      | { attributes?: Record<string, unknown>; startTime?: unknown }
      | undefined;
    expect(secondDeliveryOptions?.attributes?.["daisyclaw.channel"]).toBe("discord");
    expect(secondDeliveryOptions?.attributes?.["daisyclaw.delivery.kind"]).toBe("media");
    expect(secondDeliveryOptions?.attributes?.["daisyclaw.outcome"]).toBe("error");
    expect(secondDeliveryOptions?.attributes?.["daisyclaw.errorCategory"]).toBe("TypeError");
    ...
    for (const call of deliverySpanCalls) {
      ...
      expect(Object.hasOwn(options.attributes ?? {}, "daisyclaw.chatId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "daisyclaw.sessionKey")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "daisyclaw.messageId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "daisyclaw.conversationId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "daisyclaw.content")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "daisyclaw.to")).toBe(false);
      expect(options.startTime).toBeTypeOf("number");
    }
    const errorSpan = telemetryState.spans.find(
      (span) => span.name === "daisyclaw.message.delivery" && span.setStatus.mock.calls.length > 0,
    );
  ...
  test("bounds unsafe message delivery attributes before export", async () => {
    ...
    const deliveryDuration = lastHistogramRecord("daisyclaw.message.delivery.duration_ms");
    expect(deliveryDuration?.[0]).toBe(20);
    expect(deliveryDuration?.[1]?.["daisyclaw.channel"]).toBe("unknown");
    expect(deliveryDuration?.[1]?.["daisyclaw.delivery.kind"]).toBe("other");
    expect(deliveryDuration?.[1]?.["daisyclaw.outcome"]).toBe("completed");
    const deliverySpanCall = startedSpanCall("daisyclaw.message.delivery");
    const deliveryOptions = deliverySpanCall?.[1];
    expect(deliveryOptions?.attributes?.["daisyclaw.channel"]).toBe("unknown");
    expect(deliveryOptions?.attributes?.["daisyclaw.delivery.kind"]).toBe("other");
    expect(deliveryOptions?.attributes?.["daisyclaw.outcome"]).toBe("completed");
    expect(deliveryOptions?.attributes?.["daisyclaw.delivery.result_count"]).toBe(1);
  ...
  test("exports session recovery and talk metrics with bounded attributes", async () => {
    ...
    const recoveryRequestedCall = firstCounterAddCall("daisyclaw.session.recovery.requested");
    expect(recoveryRequestedCall[0]).toBe(1);
    expect(recoveryRequestedCall[1]?.["daisyclaw.state"]).toBe("processing");
    expect(recoveryRequestedCall[1]?.["daisyclaw.action"]).toBe("abort");
    expect(recoveryRequestedCall[1]?.["daisyclaw.active_work_kind"]).toBe("tool_call");
    const recoveryCompletedCall = firstCounterAddCall("daisyclaw.session.recovery.completed");
    expect(recoveryCompletedCall[0]).toBe(1);
    expect(recoveryCompletedCall[1]?.["daisyclaw.state"]).toBe("processing");
    expect(recoveryCompletedCall[1]?.["daisyclaw.status"]).toBe("released");
    expect(recoveryCompletedCall[1]?.["daisyclaw.action"]).toBe("abort-active-run");
    const recoveryAgeRecord = lastHistogramRecord("daisyclaw.session.recovery.age_ms");
    expect(recoveryAgeRecord?.[0]).toBe(13_000);
    expect(recoveryAgeRecord?.[1]?.["daisyclaw.status"]).toBe("released");
    expect(telemetryState.counters.get("daisyclaw.talk.event")?.add).toHaveBeenCalledWith(1, {
      "daisyclaw.talk.brain": "agent-consult",
      "daisyclaw.talk.event_type": "input.audio.delta",
      "daisyclaw.talk.mode": "realtime",
      "daisyclaw.talk.provider": "openai",
      "daisyclaw.talk.transport": "gateway-relay",
    });
    expect(telemetryState.histograms.get("daisyclaw.talk.audio.bytes")?.record).toHaveBeenCalledWith(
      320,
      {
        "daisyclaw.talk.brain": "agent-consult",
        "daisyclaw.talk.event_type": "input.audio.delta",
    ...
    expect(
      telemetryState.histograms.get("daisyclaw.talk.event.duration_ms")?.record,
    ).toHaveBeenCalledWith(45, {
      "daisyclaw.talk.brain": "agent-consult",
      "daisyclaw.talk.event_type": "latency.metrics",
      "daisyclaw.talk.mode": "realtime",
      "daisyclaw.talk.provider": "openai",
      "daisyclaw.talk.transport": "gateway-relay",
    });
    ...
    const talkCounterCalls = JSON.stringify(
      telemetryState.counters.get("daisyclaw.talk.event")?.add.mock.calls,
    );
  ...
  test("does not export model or tool content unless capture is explicitly enabled", async () => {
    ...
    const modelOptions = startedSpanOptions("daisyclaw.model.call");
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "daisyclaw.content.input_messages")).toBe(
      false,
    );
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "daisyclaw.content.output_messages")).toBe(
      false,
    );
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "daisyclaw.content.system_prompt")).toBe(
      false,
    );
    expect(modelOptions?.startTime).toBeTypeOf("number");
    const toolOptions = startedSpanOptions("daisyclaw.tool.execution");
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "daisyclaw.content.tool_input")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "daisyclaw.content.tool_output")).toBe(
      false,
    );
    expect(toolOptions?.startTime).toBeTypeOf("number");
    await service.stop?.(ctx);
  ...
  test("exports bounded redacted content when capture fields are opted in", async () => {
    ...
    expect(modelAttrs?.["daisyclaw.content.output_messages"]).toBe("model reply");
    expect(modelAttrs?.["daisyclaw.content.system_prompt"]).toBe("system prompt");
    expect(String(modelAttrs?.["daisyclaw.content.input_messages"])).not.toContain(
      "sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
    );
    expect(toolAttrs?.["daisyclaw.content.tool_input"]).toBe("tool input");
    expect(String(toolAttrs?.["daisyclaw.content.tool_output"]).length).toBeLessThanOrEqual(
      MAX_TEST_OTEL_CONTENT_ATTRIBUTE_CHARS + OTEL_TRUNCATED_SUFFIX_MAX_CHARS,
    );
    expect(String(toolAttrs?.["daisyclaw.content.tool_output"])).not.toContain("a".repeat(11));
    await service.stop?.(ctx);
  });
  ...
  test("omits absent model content fields when capture fields are opted in", async () => {
    ...
    expect(attrs["daisyclaw.content.input_messages"]).toBe("user prompt");
    expect(Object.hasOwn(attrs, "daisyclaw.content.output_messages")).toBe(false);
    expect(Object.hasOwn(attrs, "daisyclaw.content.system_prompt")).toBe(false);
    expect(Object.hasOwn(attrs, "daisyclaw.content.tool_definitions")).toBe(false);
  ...
  test("exports tool definitions without requiring input message capture", async () => {
    ...
    expect(Object.hasOwn(attrs ?? {}, "daisyclaw.content.input_messages")).toBe(false);
    expect(JSON.parse(stringAttribute(attrs, "gen_ai.tool.definitions"))).toEqual([
      {
        type: "function",
        name: "lookup",
    ...
    expect(JSON.parse(String(attrs?.["daisyclaw.content.tool_definitions"]))).toEqual([
      {
        name: "lookup",
        description: "Lookup data",
        parameters: { type: "object" },
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionStateCall = firstCounterAddCall("daisyclaw.session.state");
    const attrs = sessionStateCall[1];
    expect(sessionStateCall[0]).toBe(1);
    expect(String(attrs?.["daisyclaw.reason"])).toContain("…");
    expect(typeof attrs?.["daisyclaw.reason"]).toBe("string");
    expect(String(attrs?.["daisyclaw.reason"])).not.toContain(
