...
import { waitForDiagnosticEventsDrained } from "daisyclaw/plugin-sdk/diagnostic-runtime";
import { registerUnhandledRejectionHandler } from "daisyclaw/plugin-sdk/runtime-env";
import type {
...
const DEFAULT_SERVICE_NAME = "daisyclaw";
...
const PRELOADED_OTEL_SDK_ENV = "DAISYCLAW_OTEL_PRELOADED";
...
function assignModelCallSizeTimingAttrs(
  ...
  assignPositiveNumberAttr(attrs, "daisyclaw.model_call.request_bytes", evt.requestPayloadBytes);
  assignPositiveNumberAttr(attrs, "daisyclaw.model_call.response_bytes", evt.responseStreamBytes);
  assignPositiveNumberAttr(
    attrs,
    "daisyclaw.model_call.time_to_first_byte_ms",
...
function addUpstreamRequestIdSpanEvent(
  ...
  span.addEvent?.("daisyclaw.provider.request", {
    "daisyclaw.upstreamRequestIdHash": boundedHash,
  });
}
...
function assignOtelModelContentAttributes(
  ...
  if (policy.inputMessages) {
    assignOtelContentAttribute(
      attributes,
      "daisyclaw.content.input_messages",
  ...
  if (policy.toolDefinitions) {
    assignOtelContentAttribute(
      attributes,
      "daisyclaw.content.tool_definitions",
  ...
  if (policy.outputMessages) {
    assignOtelContentAttribute(
      attributes,
      "daisyclaw.content.output_messages",
      content?.outputMessages,
    );
  }
  if (policy.systemPrompt) {
    assignOtelContentAttribute(attributes, "daisyclaw.content.system_prompt", content?.systemPrompt);
  }
}
...
function assignOtelToolContentAttributes(
  ...
  if (policy.toolInputs) {
    assignOtelContentAttribute(attributes, "daisyclaw.content.tool_input", content?.toolInput);
  }
  if (policy.toolOutputs) {
    assignOtelContentAttribute(attributes, "daisyclaw.content.tool_output", content?.toolOutput);
  }
}
...
function assignOtelLogEventAttributes(
  ...
  for (const rawKey in eventAttributes) {
    ...
    assignOtelLogAttribute(attributes, `daisyclaw.${key}`, eventAttributes[rawKey]);
  }
}
...
function addTraceAttributes(
  ...
  attributes["daisyclaw.traceId"] = normalized.traceId;
  if (normalized.spanId) {
    attributes["daisyclaw.spanId"] = normalized.spanId;
  }
  if (normalized.parentSpanId) {
    attributes["daisyclaw.parentSpanId"] = normalized.parentSpanId;
  }
  if (normalized.traceFlags) {
    attributes["daisyclaw.traceFlags"] = normalized.traceFlags;
  }
}

export function createDiagnosticsOtelService(): DaisyClawPluginService {
  ...
  return {
    ...
    async start(ctx) {
      ...
      const meter = metrics.getMeter("daisyclaw");
      const tracer = trace.getTracer("daisyclaw");
      ...
      const tokensCounter = meter.createCounter("daisyclaw.tokens", {
        unit: "1",
      ...
      const costCounter = meter.createCounter("daisyclaw.cost.usd", {
        unit: "1",
        description: "Estimated model cost (USD)",
      });
      const durationHistogram = meter.createHistogram("daisyclaw.run.duration_ms", {
        unit: "ms",
        description: "Agent run duration",
      });
      const harnessDurationHistogram = meter.createHistogram("daisyclaw.harness.duration_ms", {
        unit: "ms",
        description: "Agent harness lifecycle duration",
      });
      const contextHistogram = meter.createHistogram("daisyclaw.context.tokens", {
        unit: "1",
        description: "Context window size and usage",
      });
      const webhookReceivedCounter = meter.createCounter("daisyclaw.webhook.received", {
        unit: "1",
        description: "Webhook requests received",
      });
      const webhookErrorCounter = meter.createCounter("daisyclaw.webhook.error", {
        unit: "1",
        description: "Webhook processing errors",
      });
      const webhookDurationHistogram = meter.createHistogram("daisyclaw.webhook.duration_ms", {
        unit: "ms",
        description: "Webhook processing duration",
      });
      const messageQueuedCounter = meter.createCounter("daisyclaw.message.queued", {
        unit: "1",
        description: "Messages queued for processing",
      });
      const messageReceivedCounter = meter.createCounter("daisyclaw.message.received", {
        unit: "1",
        description: "Inbound messages received",
      });
      const messageDispatchStartedCounter = meter.createCounter(
        "daisyclaw.message.dispatch.started",
        {
          unit: "1",
      ...
      const messageDispatchCompletedCounter = meter.createCounter(
        "daisyclaw.message.dispatch.completed",
        {
          unit: "1",
      ...
      const messageDispatchDurationHistogram = meter.createHistogram(
        "daisyclaw.message.dispatch.duration_ms",
        {
          unit: "ms",
          description: "Inbound message dispatch duration",
        },
      );
      const messageProcessedCounter = meter.createCounter("daisyclaw.message.processed", {
        unit: "1",
        description: "Messages processed by outcome",
      });
      const messageDurationHistogram = meter.createHistogram("daisyclaw.message.duration_ms", {
        unit: "ms",
        description: "Message processing duration",
      });
      const messageDeliveryStartedCounter = meter.createCounter(
        "daisyclaw.message.delivery.started",
        {
          unit: "1",
      ...
      const queueDepthHistogram = meter.createHistogram("daisyclaw.queue.depth", {
        unit: "1",
        description: "Queue depth on enqueue/dequeue",
      });
      const queueWaitHistogram = meter.createHistogram("daisyclaw.queue.wait_ms", {
        unit: "ms",
        description: "Queue wait time before execution",
      });
      const laneEnqueueCounter = meter.createCounter("daisyclaw.queue.lane.enqueue", {
        unit: "1",
        description: "Command queue lane enqueue events",
      });
      const laneDequeueCounter = meter.createCounter("daisyclaw.queue.lane.dequeue", {
        unit: "1",
        description: "Command queue lane dequeue events",
      });
      const sessionStateCounter = meter.createCounter("daisyclaw.session.state", {
        unit: "1",
        description: "Session state transitions",
      });
      const sessionTurnCreatedCounter = meter.createCounter("daisyclaw.session.turn.created", {
        unit: "1",
        description: "Agent session turns created",
      });
      const sessionStuckCounter = meter.createCounter("daisyclaw.session.stuck", {
        unit: "1",
        description: "Sessions stuck in processing",
      });
      const sessionStuckAgeHistogram = meter.createHistogram("daisyclaw.session.stuck_age_ms", {
        unit: "ms",
        description: "Age of stuck sessions",
      });
      const sessionRecoveryRequestedCounter = meter.createCounter(
        "daisyclaw.session.recovery.requested",
        {
          unit: "1",
      ...
      const sessionRecoveryCompletedCounter = meter.createCounter(
        "daisyclaw.session.recovery.completed",
        {
          unit: "1",
      ...
      const sessionRecoveryAgeHistogram = meter.createHistogram(
        "daisyclaw.session.recovery.age_ms",
        {
          unit: "ms",
          description: "Age of sessions selected for recovery",
        },
      );
      const talkEventCounter = meter.createCounter("daisyclaw.talk.event", {
        unit: "1",
        description: "Talk events emitted by type",
      });
      const talkEventDurationHistogram = meter.createHistogram("daisyclaw.talk.event.duration_ms", {
        unit: "ms",
        description: "Talk event duration when reported",
      });
      const talkAudioBytesHistogram = meter.createHistogram("daisyclaw.talk.audio.bytes", {
        unit: "By",
        description: "Talk audio frame byte lengths",
      });
      const runAttemptCounter = meter.createCounter("daisyclaw.run.attempt", {
        unit: "1",
        description: "Run attempts",
      });
      const toolLoopCounter = meter.createCounter("daisyclaw.tool.loop", {
        unit: "1",
        description: "Detected repetitive tool-call loop events",
      });
      const skillUsedCounter = meter.createCounter("daisyclaw.skill.used", {
        unit: "1",
        description: "Skills used by agent runs",
      });
      const modelCallDurationHistogram = meter.createHistogram("daisyclaw.model_call.duration_ms", {
        unit: "ms",
        description: "Model call duration",
      });
      const modelCallRequestBytesHistogram = meter.createHistogram(
        "daisyclaw.model_call.request_bytes",
        {
          unit: "By",
      ...
      const modelCallResponseBytesHistogram = meter.createHistogram(
        "daisyclaw.model_call.response_bytes",
        {
          unit: "By",
      ...
      const modelFailoverCounter = meter.createCounter("daisyclaw.model.failover", {
        unit: "1",
      ...
      const toolExecutionDurationHistogram = meter.createHistogram(
        "daisyclaw.tool.execution.duration_ms",
        {
          unit: "ms",
          description: "Tool execution duration",
        },
      );
      const toolExecutionBlockedCounter = meter.createCounter("daisyclaw.tool.execution.blocked", {
        unit: "1",
        description: "Tool executions blocked by policy or sandbox diagnostics",
      });
      const execProcessDurationHistogram = meter.createHistogram("daisyclaw.exec.duration_ms", {
        unit: "ms",
        description: "Exec process duration",
      });
      const memoryRssHistogram = meter.createHistogram("daisyclaw.memory.rss_bytes", {
        unit: "By",
        description: "Resident set size reported by diagnostic memory samples",
      });
      const memoryHeapUsedHistogram = meter.createHistogram("daisyclaw.memory.heap_used_bytes", {
        unit: "By",
        description: "Heap used bytes reported by diagnostic memory samples",
      });
      const memoryHeapTotalHistogram = meter.createHistogram("daisyclaw.memory.heap_total_bytes", {
        unit: "By",
        description: "Heap total bytes reported by diagnostic memory samples",
      });
      const memoryExternalHistogram = meter.createHistogram("daisyclaw.memory.external_bytes", {
        unit: "By",
      ...
      const livenessEventLoopDelayMaxHistogram = meter.createHistogram(
        "daisyclaw.liveness.event_loop_delay_max_ms",
        {
          unit: "ms",
      ...
      const livenessEventLoopUtilizationHistogram = meter.createHistogram(
        "daisyclaw.liveness.event_loop_utilization",
        {
          unit: "1",
      ...
      const livenessCpuCoreRatioHistogram = meter.createHistogram(
        "daisyclaw.liveness.cpu_core_ratio",
        {
          unit: "1",
          description: "CPU core ratio reported by diagnostic liveness warnings",
        },
      );
      const telemetryExporterCounter = meter.createCounter("daisyclaw.telemetry.exporter.events", {
        unit: "1",
      ...
      if (logsEnabled) {
        ...
        const otelLogger = logProvider.getLogger("daisyclaw");
        recordLogRecord = (evt, metadata) => {
          try {
            ...
            assignOtelLogAttribute(attributes, "daisyclaw.log.level", logLevelName);
            if (evt.loggerName) {
              assignOtelLogAttribute(attributes, "daisyclaw.logger", evt.loggerName);
            }
            if (evt.loggerParents?.length) {
              assignOtelLogAttribute(
                attributes,
                "daisyclaw.logger.parents",
      ...
      const addRunAttrs = (
        ...
        if (evt.provider) {
          spanAttrs["daisyclaw.provider"] = evt.provider;
        }
        if (evt.model) {
          spanAttrs["daisyclaw.model"] = evt.model;
        }
        if (evt.channel) {
          spanAttrs["daisyclaw.channel"] = evt.channel;
        }
        if (evt.trigger) {
          spanAttrs["daisyclaw.trigger"] = evt.trigger;
        }
      };
      ...
      const paramsSummaryAttrs = (
        ...
        return {
          "daisyclaw.tool.params.kind": summary.kind,
          ...("length" in summary ? { "daisyclaw.tool.params.length": summary.length } : {}),
        };
      };
      ...
      const recordModelUsage = (
        ...
        const attrs = {
          "daisyclaw.channel": evt.channel ?? "unknown",
          "daisyclaw.agent": lowCardinalityAttr(evt.agentId),
          "daisyclaw.provider": evt.provider ?? "unknown",
          "daisyclaw.model": evt.model ?? "unknown",
        };
        ...
        if (usage.input) {
          tokensCounter.add(usage.input, { ...attrs, "daisyclaw.token": "input" });
        ...
        if (usage.output) {
          tokensCounter.add(usage.output, { ...attrs, "daisyclaw.token": "output" });
        ...
        if (usage.cacheRead) {
          tokensCounter.add(usage.cacheRead, { ...attrs, "daisyclaw.token": "cache_read" });
        }
        if (usage.cacheWrite) {
          tokensCounter.add(usage.cacheWrite, { ...attrs, "daisyclaw.token": "cache_write" });
        }
        if (usage.promptTokens) {
          tokensCounter.add(usage.promptTokens, { ...attrs, "daisyclaw.token": "prompt" });
        }
        if (usage.total) {
          tokensCounter.add(usage.total, { ...attrs, "daisyclaw.token": "total" });
        }
        ...
        if (evt.context?.limit) {
          contextHistogram.record(evt.context.limit, {
            ...attrs,
            "daisyclaw.context": "limit",
          });
        }
        if (evt.context?.used) {
          contextHistogram.record(evt.context.used, {
            ...attrs,
            "daisyclaw.context": "used",
          });
        }
        ...
        const spanAttrs: Record<string, string | number> = {
          ...attrs,
          "daisyclaw.tokens.input": usage.input ?? 0,
          "daisyclaw.tokens.output": usage.output ?? 0,
          "daisyclaw.tokens.cache_read": usage.cacheRead ?? 0,
          "daisyclaw.tokens.cache_write": usage.cacheWrite ?? 0,
          "daisyclaw.tokens.total": usage.total ?? 0,
        };
        ...
        const span = spanWithDuration("daisyclaw.model.usage", spanAttrs, evt.durationMs, {
      ...
      const recordWebhookReceived = (
        ...
        const attrs = {
          "daisyclaw.channel": evt.channel ?? "unknown",
          "daisyclaw.webhook": evt.updateType ?? "unknown",
        };
      ...
      const recordWebhookProcessed = (
        ...
        const attrs = {
          "daisyclaw.channel": lowCardinalityAttr(evt.channel),
          "daisyclaw.webhook": lowCardinalityAttr(evt.updateType),
        };
        ...
        const span = spanWithDuration("daisyclaw.webhook.processed", spanAttrs, evt.durationMs);
        span.end();
      };
      ...
      const recordWebhookError = (
        ...
        const spanAttrs: Record<string, string | number> = {
          ...attrs,
          "daisyclaw.error": redactedError,
        };
        const span = tracer.startSpan("daisyclaw.webhook.error", {
      ...
      const recordMessageQueued = (
        ...
        const attrs = {
          "daisyclaw.channel": lowCardinalityAttr(evt.channel),
          "daisyclaw.source": lowCardinalityAttr(evt.source),
        };
      ...
      const recordMessageDispatchStarted = (
        ...
        trackInternalOrTrustedSpan(
          evt,
          metadata,
          spanWithDuration("daisyclaw.message.processed", attrs, undefined, {
      ...
      const recordMessageDispatchCompleted = (
        ...
        const attrs = {
          "daisyclaw.channel": lowCardinalityAttr(evt.channel),
          "daisyclaw.outcome": evt.outcome,
          "daisyclaw.reason": lowCardinalityAttr(evt.reason, "none"),
          "daisyclaw.source": lowCardinalityAttr(evt.source),
        };
      ...
      const recordMessageProcessed = (
        ...
        const attrs = {
          "daisyclaw.channel": lowCardinalityAttr(evt.channel),
          "daisyclaw.outcome": evt.outcome ?? "unknown",
        };
        ...
        const spanAttrs: Record<string, string | number> = { ...attrs };
        if (evt.reason) {
          spanAttrs["daisyclaw.reason"] = lowCardinalityAttr(evt.reason, "unknown");
        }
        const trackedSpan = getTrackedInternalOrTrustedSpan(evt, metadata);
        const span =
          trackedSpan ??
          spanWithDuration("daisyclaw.message.processed", spanAttrs, evt.durationMs, {
            parentContext: internalOrTrustedExplicitParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
      ...
      const messageDeliveryAttrs = (
        ...
        "daisyclaw.channel": lowCardinalityAttr(evt.channel),
        "daisyclaw.delivery.kind": lowCardinalityAttr(evt.deliveryKind, "other"),
      });
      ...
      const recordMessageDeliveryCompleted = (
        ...
        const span = spanWithDuration(
          "daisyclaw.message.delivery",
          {
            ...attrs,
            "daisyclaw.delivery.result_count": evt.resultCount,
          },
          evt.durationMs,
      ...
      const recordMessageDeliveryError = (
        ...
        const attrs = {
          ...messageDeliveryAttrs(evt),
          "daisyclaw.outcome": "error",
          "daisyclaw.errorCategory": lowCardinalityAttr(evt.errorCategory, "other"),
        };
        messageDeliveryDurationHistogram.record(evt.durationMs, attrs);
        if (!tracesEnabled) {
          return;
        }
        const span = spanWithDuration("daisyclaw.message.delivery", attrs, evt.durationMs, {
      ...
      const recordRunStarted = (
        ...
        const span = trackTrustedSpan(
          evt,
          metadata,
          spanWithDuration("daisyclaw.run", spanAttrs, undefined, {
            parentContext: activeTrustedParentContext(evt, metadata),
            startTimeMs: evt.ts,
          }),
        );
      ...
      const recordLaneEnqueue = (
        ...
        const attrs = { "daisyclaw.lane": lowCardinalityQueueLaneAttr(evt.lane) };
      ...
      const recordSessionState = (
        ...
        const attrs: Record<string, string> = { "daisyclaw.state": evt.state };
        if (evt.reason) {
          attrs["daisyclaw.reason"] = redactSensitiveText(evt.reason);
        }
      ...
      const recordSessionTurnCreated = (
        ...
        sessionTurnCreatedCounter.add(1, {
          "daisyclaw.agent": lowCardinalityAttr(evt.agentId, "unknown"),
          "daisyclaw.channel": lowCardinalityAttr(evt.channel, "unknown"),
          "daisyclaw.trigger": evt.trigger,
        });
      };
      ...
      const recordSessionStuck = (
        ...
        const spanAttrs: Record<string, string | number> = { ...attrs };
        spanAttrs["daisyclaw.queueDepth"] = evt.queueDepth ?? 0;
        spanAttrs["daisyclaw.ageMs"] = evt.ageMs;
        const span = tracer.startSpan("daisyclaw.session.stuck", { attributes: spanAttrs });
      ...
      const sessionRecoveryAttrs = (evt: SessionRecoveryDiagnosticEvent) => {
        ...
        if (evt.activeWorkKind) {
          attrs["daisyclaw.active_work_kind"] = evt.activeWorkKind;
        }
        return attrs;
      };
      ...
      const recordSessionRecoveryRequested = (
        ...
        attrs["daisyclaw.action"] = evt.allowActiveAbort ? "abort" : "recover";
      ...
      const recordSessionRecoveryCompleted = (
        ...
        const attrs = sessionRecoveryAttrs(evt);
        attrs["daisyclaw.status"] = evt.status;
        attrs["daisyclaw.action"] = lowCardinalityAttr(evt.action, "unknown");
        if (evt.outcomeReason) {
          attrs["daisyclaw.reason"] = redactSensitiveText(evt.outcomeReason);
        }
      ...
      const talkEventAttrs = (evt: TalkDiagnosticEvent): Record<string, string> => ({
        "daisyclaw.talk.brain": lowCardinalityAttr(evt.brain),
        "daisyclaw.talk.event_type": lowCardinalityAttr(evt.talkEventType),
        "daisyclaw.talk.mode": lowCardinalityAttr(evt.mode),
        "daisyclaw.talk.provider": lowCardinalityAttr(evt.provider),
        "daisyclaw.talk.transport": lowCardinalityAttr(evt.transport),
      });
      ...
      const recordRunAttempt = (evt: Extract<DiagnosticEventPayload, { type: "run.attempt" }>) => {
        runAttemptCounter.add(1, { "daisyclaw.attempt": evt.attempt });
      };

      const toolLoopAttrs = (
        evt: Extract<DiagnosticEventPayload, { type: "tool.loop" }>,
      ): Record<string, string | number> => ({
        "daisyclaw.toolName": lowCardinalityAttr(evt.toolName, "tool"),
        "daisyclaw.loop.level": evt.level,
        "daisyclaw.loop.action": evt.action,
        "daisyclaw.loop.detector": evt.detector,
        "daisyclaw.loop.count": evt.count,
        ...(evt.pairedToolName
          ? { "daisyclaw.loop.paired_tool": lowCardinalityAttr(evt.pairedToolName, "tool") }
          : {}),
      });
      ...
      const recordToolLoop = (evt: Extract<DiagnosticEventPayload, { type: "tool.loop" }>) => {
        ...
        const span = spanWithDuration("daisyclaw.tool.loop", attrs, 0, { endTimeMs: evt.ts });
      ...
      const recordMemoryPressure = (
        ...
        const attrs = {
          "daisyclaw.memory.level": evt.level,
          "daisyclaw.memory.reason": evt.reason,
        };
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          ...attrs,
          "daisyclaw.memory.rss_bytes": evt.memory.rssBytes,
          "daisyclaw.memory.heap_used_bytes": evt.memory.heapUsedBytes,
          "daisyclaw.memory.heap_total_bytes": evt.memory.heapTotalBytes,
          "daisyclaw.memory.external_bytes": evt.memory.externalBytes,
          "daisyclaw.memory.array_buffers_bytes": evt.memory.arrayBuffersBytes,
          ...(evt.thresholdBytes !== undefined
            ? { "daisyclaw.memory.threshold_bytes": evt.thresholdBytes }
            : {}),
          ...(evt.rssGrowthBytes !== undefined
            ? { "daisyclaw.memory.rss_growth_bytes": evt.rssGrowthBytes }
            : {}),
          ...(evt.windowMs !== undefined ? { "daisyclaw.memory.window_ms": evt.windowMs } : {}),
        };
        const span = spanWithDuration("daisyclaw.memory.pressure", spanAttrs, 0, {
          endTimeMs: evt.ts,
        });
      ...
      const recordRunCompleted = (
        ...
        if (evt.channel) {
          attrs["daisyclaw.channel"] = evt.channel;
        }
        if (evt.blockedBy) {
          attrs["daisyclaw.blocked_by"] = lowCardinalityAttr(evt.blockedBy, "unknown");
        }
        durationHistogram.record(evt.durationMs, attrs);
        if (!tracesEnabled) {
          return;
        ...
        addRunAttrs(spanAttrs, evt);
        if (evt.blockedBy) {
          spanAttrs["daisyclaw.blocked_by"] = lowCardinalityAttr(evt.blockedBy, "unknown");
        }
        if (evt.errorCategory) {
          spanAttrs["daisyclaw.errorCategory"] = lowCardinalityAttr(evt.errorCategory, "other");
        }
        ...
        const span =
          trackedSpan ??
          spanWithDuration("daisyclaw.run", spanAttrs, evt.durationMs, {
            parentContext: activeTrustedParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
        setSpanAttrs(span, spanAttrs);
      ...
      const harnessRunMetricAttrs = (evt: HarnessRunDiagnosticEvent) => ({
        "daisyclaw.harness.id": lowCardinalityAttr(evt.harnessId, "unknown"),
        "daisyclaw.harness.plugin": lowCardinalityAttr(evt.pluginId),
        ...(evt.type === "harness.run.started"
          ? {}
          : {
              "daisyclaw.outcome": evt.type === "harness.run.error" ? "error" : evt.outcome,
            }),
        "daisyclaw.provider": lowCardinalityAttr(evt.provider, "unknown"),
        "daisyclaw.model": lowCardinalityAttr(evt.model, "unknown"),
        ...(evt.channel ? { "daisyclaw.channel": lowCardinalityAttr(evt.channel) } : {}),
      });
      ...
      const recordHarnessRunStarted = (
        ...
        trackTrustedSpan(
          evt,
          metadata,
          spanWithDuration("daisyclaw.harness.run", harnessRunMetricAttrs(evt), undefined, {
            parentContext: activeTrustedParentContext(evt, metadata),
            startTimeMs: evt.ts,
          }),
        );
      ...
      const recordHarnessRunCompleted = (
        ...
        if (evt.resultClassification) {
          spanAttrs["daisyclaw.harness.result_classification"] = lowCardinalityAttr(
            evt.resultClassification,
          );
        }
        if (typeof evt.yieldDetected === "boolean") {
          spanAttrs["daisyclaw.harness.yield_detected"] = evt.yieldDetected;
        }
        if (evt.itemLifecycle) {
          spanAttrs["daisyclaw.harness.items.started"] = evt.itemLifecycle.startedCount;
          spanAttrs["daisyclaw.harness.items.completed"] = evt.itemLifecycle.completedCount;
          spanAttrs["daisyclaw.harness.items.active"] = evt.itemLifecycle.activeCount;
        }
        const trustedTrace = trustedTraceContext(evt, metadata);
        const trackedSpan = trustedTrace?.spanId
          ? activeTrustedSpans.get(trustedTrace.spanId)
        ...
        const span =
          trackedSpan ??
          spanWithDuration("daisyclaw.harness.run", spanAttrs, evt.durationMs, {
            parentContext: activeTrustedParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordHarnessRunError = (
        ...
        const attrs = {
          ...harnessRunMetricAttrs(evt),
          "daisyclaw.harness.phase": evt.phase,
          "daisyclaw.errorCategory": errorType,
        };
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          ...
          ...(evt.cleanupFailed ? { "daisyclaw.harness.cleanup_failed": true } : {}),
        };
        const span =
      ...
      const recordContextAssembled = (
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          "daisyclaw.context.message_count": evt.messageCount,
          "daisyclaw.context.history_text_chars": evt.historyTextChars,
          "daisyclaw.context.history_image_blocks": evt.historyImageBlocks,
          "daisyclaw.context.max_message_text_chars": evt.maxMessageTextChars,
          "daisyclaw.context.system_prompt_chars": evt.systemPromptChars,
          "daisyclaw.context.prompt_chars": evt.promptChars,
          "daisyclaw.context.prompt_images": evt.promptImages,
        };
        addRunAttrs(spanAttrs, evt);
        if (evt.contextTokenBudget !== undefined) {
          spanAttrs["daisyclaw.context.token_budget"] = evt.contextTokenBudget;
        }
        if (evt.reserveTokens !== undefined) {
          spanAttrs["daisyclaw.context.reserve_tokens"] = evt.reserveTokens;
        }
        const span = spanWithDuration("daisyclaw.context.assembled", spanAttrs, 0, {
          parentContext: activeTrustedParentContext(evt, metadata),
          endTimeMs: evt.ts,
        });
        span.end(evt.ts);
      ...
      const recordModelFailover = (
        ...
        const metricAttrs: Record<string, string> = {
          "daisyclaw.failover.reason": lowCardinalityAttr(evt.reason, "unknown"),
          "daisyclaw.failover.suspended":
            evt.suspended === undefined ? "unknown" : String(evt.suspended),
          "daisyclaw.lane": lowCardinalityQueueLaneAttr(evt.lane, "unknown"),
          "daisyclaw.model": lowCardinalityAttr(evt.fromModel),
          "daisyclaw.provider": lowCardinalityAttr(evt.fromProvider),
          "daisyclaw.failover.to_model": lowCardinalityAttr(evt.toModel),
          "daisyclaw.failover.to_provider": lowCardinalityAttr(evt.toProvider),
        };
        ...
        if (evt.fromProvider) {
          spanAttrs["daisyclaw.provider"] = evt.fromProvider;
        }
        if (evt.fromModel) {
          spanAttrs["daisyclaw.model"] = evt.fromModel;
        }
        if (evt.toProvider) {
          spanAttrs["daisyclaw.failover.to_provider"] = evt.toProvider;
        }
        if (evt.toModel) {
          spanAttrs["daisyclaw.failover.to_model"] = evt.toModel;
        }
        if (evt.lane) {
          spanAttrs["daisyclaw.lane"] = lowCardinalityQueueLaneAttr(evt.lane, "unknown");
        }
        if (evt.suspended !== undefined) {
          spanAttrs["daisyclaw.failover.suspended"] = evt.suspended;
        }
        if (evt.cascadeDepth !== undefined) {
          spanAttrs["daisyclaw.failover.cascade_depth"] = evt.cascadeDepth;
        }
        const span = spanWithDuration("daisyclaw.model.failover", spanAttrs, 0, {
          parentContext: activeTrustedParentContext(evt, metadata),
          endTimeMs: evt.ts,
        });
        span.end(evt.ts);
      ...
      const modelCallMetricAttrs = (evt: ModelCallLifecycleDiagnosticEvent) => ({
        "daisyclaw.provider": evt.provider,
        "daisyclaw.model": evt.model,
        "daisyclaw.api": lowCardinalityAttr(evt.api),
        "daisyclaw.transport": lowCardinalityAttr(evt.transport),
      });
      ...
      const recordModelCallStarted = (
        ...
        if (evt.api) {
          spanAttrs["daisyclaw.api"] = evt.api;
        }
        if (evt.transport) {
          spanAttrs["daisyclaw.transport"] = evt.transport;
        }
        trackTrustedSpan(
          evt,
          metadata,
      ...
      const recordModelCallError = (
        ...
        const metricAttrs = {
          ...
          "daisyclaw.errorCategory": errorType,
          ...(evt.failureKind
            ? { "daisyclaw.failureKind": lowCardinalityAttr(evt.failureKind, "other") }
            : {}),
        };
        modelCallDurationHistogram.record(evt.durationMs, metricAttrs);
        recordModelCallSizeTimingMetrics(evt, metricAttrs);
        ...
        if (evt.failureKind) {
          spanAttrs["daisyclaw.failureKind"] = lowCardinalityAttr(evt.failureKind, "other");
        }
        assignGenAiModelCallAttrs(spanAttrs, evt);
        if (evt.api) {
          spanAttrs["daisyclaw.api"] = evt.api;
      ...
      const toolExecutionBaseAttrs = (
        ...
        "daisyclaw.toolName": evt.toolName,
        "daisyclaw.tool.source": lowCardinalityAttr(evt.toolSource, "core"),
        "gen_ai.tool.name": evt.toolName,
        ...(evt.toolOwner ? { "daisyclaw.tool.owner": lowCardinalityAttr(evt.toolOwner) } : {}),
      ...
      const skillUsedAttrs = (
        ...
        "daisyclaw.skill.name": lowCardinalityAttr(evt.skillName, "skill"),
        "daisyclaw.skill.source": lowCardinalityAttr(evt.skillSource),
        "daisyclaw.skill.activation": lowCardinalityAttr(evt.activation),
        ...(evt.agentId ? { "daisyclaw.agent": lowCardinalityAttr(evt.agentId) } : {}),
        ...(evt.toolName ? { "daisyclaw.toolName": lowCardinalityAttr(evt.toolName, "tool") } : {}),
      });
      ...
      const recordSkillUsed = (
        ...
        addRunAttrs(spanAttrs, evt);
        const span = spanWithDuration("daisyclaw.skill.used", spanAttrs, 0, {
          parentContext: activeTrustedParentContext(evt, metadata),
          endTimeMs: evt.ts,
        });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordToolExecutionStarted = (
        ...
        trackTrustedSpan(
          evt,
          metadata,
          spanWithDuration("daisyclaw.tool.execution", toolExecutionBaseAttrs(evt), undefined, {
            parentContext: activeTrustedParentContext(evt, metadata),
            startTimeMs: evt.ts,
          }),
        );
      ...
      const recordToolExecutionCompleted = (
        ...
        const span =
          takeTrackedTrustedSpan(evt, metadata) ??
          spanWithDuration("daisyclaw.tool.execution", spanAttrs, evt.durationMs, {
            parentContext: activeTrustedParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordToolExecutionError = (
        ...
        addRunAttrs(spanAttrs, evt);
        if (evt.errorCode) {
          spanAttrs["daisyclaw.errorCode"] = lowCardinalityAttr(evt.errorCode, "other");
        }
        assignOtelToolContentAttributes(spanAttrs, toolContent, contentCapturePolicy);
        const span =
          takeTrackedTrustedSpan(evt, metadata) ??
      ...
      const recordToolExecutionBlocked = (
        ...
        toolExecutionBlockedCounter.add(1, {
          ...toolExecutionBaseAttrs(evt),
          "daisyclaw.deniedReason": lowCardinalityAttr(evt.deniedReason, "other"),
        });
        if (!tracesEnabled) {
          return;
        }
        const spanAttrs: Record<string, string | number | boolean> = {
          ...toolExecutionBaseAttrs(evt),
          "daisyclaw.outcome": "blocked",
          "daisyclaw.deniedReason": lowCardinalityAttr(evt.deniedReason, "other"),
        };
        addRunAttrs(spanAttrs, evt);
        const span = spanWithDuration("daisyclaw.tool.execution", spanAttrs, 0, {
          parentContext: activeTrustedParentContext(evt, metadata),
          endTimeMs: evt.ts,
        });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordPayloadLarge = (
        ...
        const attrs = {
          "daisyclaw.payload.action": evt.action,
          "daisyclaw.payload.surface": lowCardinalityAttr(evt.surface, "unknown"),
          "daisyclaw.channel": lowCardinalityAttr(evt.channel, "none"),
          "daisyclaw.plugin": lowCardinalityAttr(evt.pluginId, "none"),
          "daisyclaw.reason": lowCardinalityAttr(evt.reason, "none"),
        };
      ...
      const recordExecProcessCompleted = (
        ...
        const attrs: Record<string, string | number> = {
          "daisyclaw.exec.target": evt.target,
          "daisyclaw.exec.mode": evt.mode,
          "daisyclaw.outcome": evt.outcome,
        };
        if (evt.failureKind) {
          attrs["daisyclaw.failureKind"] = evt.failureKind;
        }
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          ...attrs,
          "daisyclaw.exec.command_length": evt.commandLength,
        };
        if (typeof evt.exitCode === "number") {
          spanAttrs["daisyclaw.exec.exit_code"] = evt.exitCode;
        }
        if (evt.exitSignal) {
          spanAttrs["daisyclaw.exec.exit_signal"] = lowCardinalityAttr(evt.exitSignal, "other");
        }
        if (evt.timedOut !== undefined) {
          spanAttrs["daisyclaw.exec.timed_out"] = evt.timedOut;
        }

        const span = spanWithDuration("daisyclaw.exec", spanAttrs, evt.durationMs, {
          endTimeMs: evt.ts,
        });
      ...
      const recordHeartbeat = (
        ...
        queueDepthHistogram.record(evt.queued, { "daisyclaw.channel": "heartbeat" });
      };
      ...
      const recordLivenessWarning = (
        ...
        const attrs = {
          "daisyclaw.liveness.reason": lowCardinalityAttr(reason, "unknown"),
        };
        livenessWarningCounter.add(1, attrs);
        queueDepthHistogram.record(evt.queued, { "daisyclaw.channel": "liveness" });
        ...
        const spanAttrs: Record<string, string | number> = {
          ...attrs,
          "daisyclaw.liveness.active": evt.active,
          "daisyclaw.liveness.waiting": evt.waiting,
          "daisyclaw.liveness.queued": evt.queued,
          "daisyclaw.liveness.interval_ms": evt.intervalMs,
          ...(evt.eventLoopDelayP99Ms !== undefined
            ? { "daisyclaw.liveness.event_loop_delay_p99_ms": evt.eventLoopDelayP99Ms }
            : {}),
          ...(evt.eventLoopDelayMaxMs !== undefined
            ? { "daisyclaw.liveness.event_loop_delay_max_ms": evt.eventLoopDelayMaxMs }
            : {}),
          ...(evt.eventLoopUtilization !== undefined
            ? { "daisyclaw.liveness.event_loop_utilization": evt.eventLoopUtilization }
            : {}),
          ...(evt.cpuUserMs !== undefined
            ? { "daisyclaw.liveness.cpu_user_ms": evt.cpuUserMs }
            : {}),
          ...(evt.cpuSystemMs !== undefined
            ? { "daisyclaw.liveness.cpu_system_ms": evt.cpuSystemMs }
            : {}),
          ...(evt.cpuTotalMs !== undefined
            ? { "daisyclaw.liveness.cpu_total_ms": evt.cpuTotalMs }
            : {}),
          ...(evt.cpuCoreRatio !== undefined
            ? { "daisyclaw.liveness.cpu_core_ratio": evt.cpuCoreRatio }
            : {}),
        };
        const span = spanWithDuration("daisyclaw.liveness.warning", spanAttrs, 0, {
          endTimeMs: evt.ts,
        });
        span.setStatus({
          code: SpanStatusCode.ERROR,
      ...
      const recordDiagnosticPhaseCompleted = (
        ...
        const spanAttrs: Record<string, string | number> = {
          "daisyclaw.phase": lowCardinalityAttr(evt.name, "unknown"),
          ...(evt.cpuUserMs !== undefined ? { "daisyclaw.phase.cpu_user_ms": evt.cpuUserMs } : {}),
          ...(evt.cpuSystemMs !== undefined
            ? { "daisyclaw.phase.cpu_system_ms": evt.cpuSystemMs }
            : {}),
          ...(evt.cpuTotalMs !== undefined
            ? { "daisyclaw.phase.cpu_total_ms": evt.cpuTotalMs }
            : {}),
          ...(evt.cpuCoreRatio !== undefined
            ? { "daisyclaw.phase.cpu_core_ratio": evt.cpuCoreRatio }
            : {}),
        };
        for (const [key, value] of Object.entries(evt.details ?? {})) {
          spanAttrs[`daisyclaw.phase.detail.${key}`] =
            typeof value === "boolean" ? String(value) : value;
        }
        const span = spanWithDuration("daisyclaw.diagnostic.phase", spanAttrs, evt.durationMs, {
          endTimeMs: evt.ts,
        });
        span.end(evt.ts);
      };
      ...
      const recordTelemetryExporter = (
        ...
        telemetryExporterCounter.add(1, {
          "daisyclaw.exporter": lowCardinalityAttr(evt.exporter, "unknown"),
          "daisyclaw.signal": evt.signal,
          "daisyclaw.status": evt.status,
          ...(evt.reason ? { "daisyclaw.reason": evt.reason } : {}),
          ...(evt.errorCategory
            ? { "daisyclaw.errorCategory": lowCardinalityAttr(evt.errorCategory, "other") }
            : {}),
        });
  ...
  } satisfies DaisyClawPluginService;
}
