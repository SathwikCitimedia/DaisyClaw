...
const disableAutoStartKey = Symbol.for("daisyclaw.diffs.disableAutoStart");
...
function renderCard(): void {
  document.body.insertAdjacentHTML(
    ...
    `<section class="oc-diff-card">
      <div data-daisyclaw-diff-host></div>
      <script type="application/json" data-daisyclaw-diff-payload>${viewerPayload}</script>
    </section>`,
  );
}
...
describe("hydrateViewer", () => {
  beforeEach(() => {
    ...
    delete document.documentElement.dataset.daisyclawDiffsError;
    delete document.documentElement.dataset.daisyclawDiffsReady;
    vi.clearAllMocks();
  });
  ...
  it("continues hydrating later cards when one card throws", async () => {
    ...
    expect(document.documentElement.dataset.daisyclawDiffsError).toBeUndefined();
    warn.mockRestore();
  });
