...
import { createUnionActionGate } from "daisyclaw/plugin-sdk/channel-actions";
import type {
  ChannelMessageActionAdapter,
  ChannelMessageActionName,
  ChannelMessageToolDiscovery,
} from "daisyclaw/plugin-sdk/channel-contract";
import type { DiscordActionConfig, DaisyClawConfig } from "daisyclaw/plugin-sdk/config-contracts";
...
function listDiscoverableDiscordAccounts(cfg: DaisyClawConfig) {
  return listDiscordAccountIds(cfg)
    .map((accountId) => inspectDiscordAccount({ cfg, accountId }))
    .filter((account) => account.enabled && account.configured);
}

function resolveDiscordActionDiscovery(cfg: DaisyClawConfig) {
