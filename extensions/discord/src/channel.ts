...
import { sleepWithAbort } from "daisyclaw/plugin-sdk/runtime-env";
import {
...
import { resolveTargetsWithOptionalToken } from "daisyclaw/plugin-sdk/target-resolver-runtime";
import {
...
function resolveDiscordStartupDelayMs(cfg: DaisyClawConfig, accountId: string): number {
    ...
    base: {
      ...
      agentPrompt: {
        messageToolHints: () => [
          ...
          "- Forms: add `components.modal` (title, fields). DaisyClaw adds a trigger button and routes submissions as new messages.",
        ],
      },
      messaging: {
      ...
      status: createComputedAccountStatusAdapter<ResolvedDiscordAccount, DiscordProbe>({
        ...
        buildCapabilitiesDiagnostics: async ({ account, target }) => {
          ...
          const statusCfg: DaisyClawConfig = {
            channels: {
              discord: {
                accounts: {
                  [account.accountId]: {
