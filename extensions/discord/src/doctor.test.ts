...
describe("discord doctor", () => {
  ...
  it("removes unsupported Discord realtime wake names", () => {
    ...
    expect(result.changes).toEqual([
      ...
      "Removed empty channels.discord.accounts.empty.voice.realtime.wakeNames; unset wake names use the default agent/DaisyClaw fallback.",
  ...
  it("repairs safe numeric ids into strings and warns for unsafe lists", () => {
    ...
    const result = maybeRepairDiscordNumericIds(cfg, "daisyclaw doctor --fix");
  ...
  it("formats repair guidance for unsafe numeric ids", () => {
    ...
    expect(warnings[1]).toContain("daisyclaw doctor --fix");
  });
