...
import { danger } from "daisyclaw/plugin-sdk/runtime-env";
import { typedCases } from "daisyclaw/plugin-sdk/test-fixtures";
...
describe("DiscordMessageListener", () => {
  ...
  it("logs handler failures", async () => {
    const logger = {
      ...
      typeof import("daisyclaw/plugin-sdk/logging-core").createSubsystemLogger
    >;
    const handler = vi.fn(async () => {
...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of DaisyClaw")).toBe("friends-of-daisyclaw");
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    const guildEntries = makeEntries({
      "123": { slug: "friends-of-daisyclaw" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of DaisyClaw"),
      guildEntries,
    });
    expect(resolved?.id).toBe("123");
    expect(resolved?.slug).toBe("friends-of-daisyclaw");
  });
  ...
  it("resolves guild entry by slug key", () => {
    const guildEntries = makeEntries({
      "friends-of-daisyclaw": { slug: "friends-of-daisyclaw" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of DaisyClaw"),
      guildEntries,
...
describe("discord group DM gating", () => {
  ...
  it("matches group DM allowlist", () => {
    expect(
      resolveGroupDmAllow({
        channels: ["daisyclaw-dm"],
        channelId: "1",
        channelName: "DaisyClaw DM",
        channelSlug: "daisyclaw-dm",
      }),
    ).toBe(true);
    expect(
      resolveGroupDmAllow({
...
const channelRuntimeModule = await import("daisyclaw/plugin-sdk/system-event-runtime");
vi.spyOn(channelRuntimeModule, "enqueueSystemEvent").mockImplementation(enqueueSystemEventSpy);

const routingModule = await import("daisyclaw/plugin-sdk/routing");
...
function makeReactionListenerParams(overrides?: {
  ...
  return {
    cfg: {} as import("daisyclaw/plugin-sdk/config-contracts").DaisyClawConfig,
    accountId: "acc-1",
    runtime: {} as import("daisyclaw/plugin-sdk/runtime-env").RuntimeEnv,
