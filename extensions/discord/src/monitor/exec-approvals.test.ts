...
vi.mock("daisyclaw/plugin-sdk/approval-gateway-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("daisyclaw/plugin-sdk/approval-gateway-runtime")>();
  return {
    ...actual,
...
function buildConfig(
  execApprovals?: NonNullable<NonNullable<DaisyClawConfig["channels"]>["discord"]>["execApprovals"],
): DaisyClawConfig {
  return {
    channels: {
      discord: {
