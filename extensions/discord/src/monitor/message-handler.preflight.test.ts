...
vi.mock("daisyclaw/plugin-sdk/media-runtime", async () => {
  const actual = await vi.importActual<typeof import("daisyclaw/plugin-sdk/media-runtime")>(
    "daisyclaw/plugin-sdk/media-runtime",
  );
  return {
    ...actual,
...
beforeEach(() => {
  ...
  saveRemoteMediaMock.mockImplementation(
    async (options: { fallbackContentType?: string; filePathHint?: string }) => ({
      id: "test-media",
      path: `/tmp/daisyclaw-discord-test/${options.filePathHint ?? "media"}`,
      size: 5,
...
function createThreadBinding(
  overrides?: Partial<import("daisyclaw/plugin-sdk/conversation-runtime").SessionBindingRecord>,
) {
  return {
    ...
    ...overrides,
  } satisfies import("daisyclaw/plugin-sdk/conversation-runtime").SessionBindingRecord;
}

function createPreflightArgs(params: {
  cfg: import("daisyclaw/plugin-sdk/config-contracts").DaisyClawConfig;
...
async function runThreadBoundPreflight(params: {
  ...
  threadBinding: import("daisyclaw/plugin-sdk/conversation-runtime").SessionBindingRecord;
  discordConfig: DiscordConfig;
...
describe("preflightDiscordMessage", () => {
  ...
  it("preflights direct-message voice notes without mention gating", async () => {
    transcribeFirstAudioMock.mockResolvedValue("hello daisyclaw from dm audio");
    ...
    expect(preflight.preflightAudioTranscript).toBe("hello daisyclaw from dm audio");
  });
  ...
  it("passes bot-loop protection facts for accepted bot-authored Discord messages (#58789)", async () => {
    ...
    const message = createDiscordMessage({
      id: "m-loop-1",
      channelId,
      content: "chatter <@daisyclaw-bot>",
      mentionedUsers: [{ id: "daisyclaw-bot" }],
  ...
  it("bypasses mention gating in bound threads for allowed bot senders", async () => {
    ...
    const result = await preflightDiscordMessage(
      createPreflightArgs({
        cfg: {
          ...DEFAULT_PREFLIGHT_CFG,
        } as import("daisyclaw/plugin-sdk/config-contracts").DaisyClawConfig,
        discordConfig: {
          allowBots: true,
        } as DiscordConfig,
        data: createGuildEvent({
  ...
  it("hydrates mention metadata from REST when bot mention syntax is present but mentions are missing", async () => {
    ...
    client.rest = {
      get: vi.fn(async () => ({
        ...
        mentions: [{ id: botId, username: "DaisyClaw", bot: true }],
        mention_roles: [],
        mention_everyone: false,
      })),
  ...
  it("still allows bot control commands with an explicit mention when allowBots=mentions", async () => {
    ...
    const message = createDiscordMessage({
      ...
      channelId,
      content: "<@daisyclaw-bot> /new incident room",
      mentionedUsers: [{ id: "daisyclaw-bot" }],
      author: {
        id: "relay-bot-1",
        bot: true,
  ...
  it("records local image media for skipped mention-gated guild history", async () => {
    ...
    saveRemoteMediaMock.mockResolvedValueOnce({
      id: "test-media",
      path: "C:\\daisyclaw\\media\\history.png",
      size: 5,
    ...
    expect(entries?.[0]?.media?.[0]?.path).toBe("C:\\daisyclaw\\media\\history.png");
  ...
  it("records sticker image media for skipped mention-gated guild history", async () => {
    ...
    saveRemoteMediaMock.mockResolvedValueOnce({
      id: "test-sticker",
      path: "/tmp/daisyclaw-discord-test/sticker.png",
      size: 5,
      contentType: "image/png",
    });
  ...
  it("uses attachment content_type for guild audio preflight mention detection", async () => {
    transcribeFirstAudioMock.mockResolvedValue("hey daisyclaw");
    ...
    const preflight = expectPreflightResult(result);
    expect(preflight.wasMentioned).toBe(true);
    expect(preflight.preflightAudioTranscript).toBe("hey daisyclaw");
  });
