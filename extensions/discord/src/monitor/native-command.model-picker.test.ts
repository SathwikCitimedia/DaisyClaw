...
import * as commandRegistryModule from "daisyclaw/plugin-sdk/command-auth-native";
import type {
  ChatCommandDefinition,
  CommandArgsParsing,
} from "daisyclaw/plugin-sdk/command-auth-native";
import type { ModelsProviderData } from "daisyclaw/plugin-sdk/command-auth-native";
import type { DaisyClawConfig } from "daisyclaw/plugin-sdk/config-contracts";
import * as globalsModule from "daisyclaw/plugin-sdk/runtime-env";
import {
  loadSessionStore,
  resolveStorePath,
  saveSessionStore,
} from "daisyclaw/plugin-sdk/session-store-runtime";
import * as commandTextModule from "daisyclaw/plugin-sdk/text-utility-runtime";
...
describe("Discord model picker interactions", () => {
  beforeEach(async () => {
    tempDir = await mkdtemp(path.join(os.tmpdir(), "daisyclaw-discord-model-picker-"));
    vi.useRealTimers();
    vi.restoreAllMocks();
  });
    ...
    pickerData.runtimeChoicesByProvider = new Map([
      [
        ...
        [
          ...
          { id: "daisyclaw", label: "DaisyClaw Default", description: "Use DaisyClaw." },
        ],
      ],
    ]);
    const modelCommand = createModelCommandDefinition();
