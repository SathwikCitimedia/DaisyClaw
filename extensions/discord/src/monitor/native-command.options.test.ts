...
let listNativeCommandSpecs: typeof import("daisyclaw/plugin-sdk/command-auth-native").listNativeCommandSpecs;
...
function createNativeCommand(
  ...
  opts?: {
    cfg?: DaisyClawConfig;
    discordConfig?: NonNullable<DaisyClawConfig["channels"]>["discord"];
  },
  ...
  const baseCfg: DaisyClawConfig = opts?.cfg ?? {};
  const discordConfig: NonNullable<DaisyClawConfig["channels"]>["discord"] =
...
function createAllowedGuildAutocompleteConfig(
  commands: NonNullable<DaisyClawConfig["commands"]>,
): DaisyClawConfig {
  return {
    commands,
    channels: {
...
describe("createDiscordNativeCommand option wiring", () => {
  beforeAll(async () => {
    ({ listNativeCommandSpecs } = await import("daisyclaw/plugin-sdk/command-auth-native"));
  ...
  it("returns no autocomplete choices for group DMs outside dm.groupChannels", async () => {
    ...
    } satisfies NonNullable<DaisyClawConfig["channels"]>["discord"];
    const command = createNativeCommand("think", {
      cfg: {
        commands: {
          allowFrom: {
  ...
  it("truncates Discord command and option descriptions to Discord's limit", () => {
    ...
    const discordConfig = {} as NonNullable<DaisyClawConfig["channels"]>["discord"];
    const command = createDiscordNativeCommand({
      command: {
        name: "longdesc",
