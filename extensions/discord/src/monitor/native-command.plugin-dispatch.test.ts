...
import type { NativeCommandSpec } from "daisyclaw/plugin-sdk/command-auth-native";
import { resolveDirectStatusReplyForSession } from "daisyclaw/plugin-sdk/command-status-runtime";
...
import { dispatchReplyWithDispatcher } from "daisyclaw/plugin-sdk/reply-dispatch-runtime";
import { getSessionEntry } from "daisyclaw/plugin-sdk/session-store-runtime";
...
function createConfig(): DaisyClawConfig {
  return {
    channels: {
      discord: {
...
async function createNativeCommand(cfg: DaisyClawConfig, commandSpec: NativeCommandSpec) {
...
async function createPluginCommand(params: { cfg: DaisyClawConfig; name: string }) {
  return createDiscordNativeCommand({
    command: {
      name: params.name,
      description: "Pair",
...
async function createStatusCommand(cfg: DaisyClawConfig) {
...
describe("Discord native plugin command dispatch", () => {
  ...
  beforeEach(() => {
    ...
    discordNativeCommandTesting.setMatchPluginCommand(
      runtimeModuleMocks.matchPluginCommand as typeof import("daisyclaw/plugin-sdk/plugin-runtime").matchPluginCommand,
    );
    discordNativeCommandTesting.setExecutePluginCommand(
      runtimeModuleMocks.executePluginCommand as typeof import("daisyclaw/plugin-sdk/plugin-runtime").executePluginCommand,
    );
    ...
    discordNativeCommandTesting.setGetSessionEntry(
      runtimeModuleMocks.getSessionEntry as typeof import("daisyclaw/plugin-sdk/session-store-runtime").getSessionEntry,
    );
  });
  ...
  it("passes the configured binding agent to plugin-owned Discord command sessions", async () => {
    const cfg = createConfig();
    const interaction = createInteraction();
    const pluginSessionKey = "plugin-binding:daisyclaw-codex-app-server:dm";
