...
import { createPluginRuntimeMock } from "daisyclaw/plugin-sdk/channel-test-helpers";
...
let runtimeEnvModule: typeof import("daisyclaw/plugin-sdk/runtime-env");
...
function createConfigWithDiscordAccount(overrides: Record<string, unknown> = {}): DaisyClawConfig {
  return {
    channels: {
      discord: {
        accounts: {
...
describe("monitorDiscordProvider", () => {
  ...
  beforeAll(async () => {
    vi.doMock("daisyclaw/plugin-sdk/plugin-runtime", async () => {
    ...
    runtimeEnvModule = await import("daisyclaw/plugin-sdk/runtime-env");
  ...
  it("passes DaisyClaw event queue defaults to the Discord client", async () => {
    await monitorDiscordProvider({
      config: baseConfig(),
      runtime: baseRuntime(),
    });
