...
vi.mock("daisyclaw/plugin-sdk/web-media", async () => {
...
afterAll(() => {
  vi.doUnmock("daisyclaw/plugin-sdk/web-media");
});
...
describe("uploadStickerDiscord", () => {
  ...
  it("uploads sticker assets", async () => {
    ...
    expect(stickerBody.name).toBe("daisyclaw_wave");
    expect(stickerBody.description).toBe("DaisyClaw waving");
