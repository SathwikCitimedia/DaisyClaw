...
import type { RuntimeEnv } from "daisyclaw/plugin-sdk/runtime-env";
...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: DaisyClawConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): DaisyClawConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("daisyclaw/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("daisyclaw/plugin-sdk/acp-runtime")>(
    "daisyclaw/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("daisyclaw/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("daisyclaw/plugin-sdk/reply-runtime")>(
...
vi.mock("daisyclaw/plugin-sdk/native-command-config-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("daisyclaw/plugin-sdk/native-command-config-runtime")
  >("daisyclaw/plugin-sdk/native-command-config-runtime");
  return {
    ...actual,
...
vi.mock("daisyclaw/plugin-sdk/runtime-config-snapshot", async () => {
  const actual = await vi.importActual<
    typeof import("daisyclaw/plugin-sdk/runtime-config-snapshot")
  >("daisyclaw/plugin-sdk/runtime-config-snapshot");
  return {
    ...actual,
    getRuntimeConfig: () => ({}),
  };
...
vi.mock("daisyclaw/plugin-sdk/error-runtime", async () => {
  const actual = await vi.importActual<typeof import("daisyclaw/plugin-sdk/error-runtime")>(
    "daisyclaw/plugin-sdk/error-runtime",
  );
  return {
    ...actual,
