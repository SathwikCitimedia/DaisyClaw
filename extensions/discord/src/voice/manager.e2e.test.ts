...
import type { RealtimeVoiceAgentControlResult } from "daisyclaw/plugin-sdk/realtime-voice";
  ...
  return {
    ...
    controlRealtimeVoiceAgentRunMock: vi.fn<() => Promise<RealtimeVoiceAgentControlResult>>(
      async () => ({
        ...
        message: "There is no active DaisyClaw run to steer.",
        speak: true,
        show: true,
        suppress: false,
      }),
...
vi.mock("daisyclaw/plugin-sdk/routing", async () => {
  const actual = await vi.importActual<typeof import("daisyclaw/plugin-sdk/routing")>(
    "daisyclaw/plugin-sdk/routing",
  );
  return {
    ...actual,
...
vi.mock("daisyclaw/plugin-sdk/agent-runtime", async () => {
  const actual = await vi.importActual<typeof import("daisyclaw/plugin-sdk/agent-runtime")>(
    "daisyclaw/plugin-sdk/agent-runtime",
  );
  return {
    ...actual,
    agentCommandFromIngress: agentCommandMock,
    getTtsProvider: vi.fn(() => "openai"),
    resolveAgentDir: vi.fn(() => "/tmp/daisyclaw-agent"),
    resolveTtsConfig: vi.fn(() => ({
      modelOverrides: {},
      providerConfigs: {},
    })),
    resolveTtsPrefsPath: vi.fn(() => "/tmp/daisyclaw-tts.json"),
  };
});

vi.mock("daisyclaw/plugin-sdk/realtime-bootstrap-context", async () => {
  const actual = await vi.importActual<
    typeof import("daisyclaw/plugin-sdk/realtime-bootstrap-context")
  >("daisyclaw/plugin-sdk/realtime-bootstrap-context");
  return {
    ...actual,
...
vi.mock("daisyclaw/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("daisyclaw/plugin-sdk/runtime-env")>(
    "daisyclaw/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
...
vi.mock("daisyclaw/plugin-sdk/realtime-voice", async () => {
  const actual = await vi.importActual<typeof import("daisyclaw/plugin-sdk/realtime-voice")>(
    "daisyclaw/plugin-sdk/realtime-voice",
  );
  return {
    ...actual,
...
describe("DiscordVoiceManager", () => {
  ...
  it("uses agent-proxy realtime voice by default", async () => {
    ...
    expect(bridgeParams?.instructions).toContain("same DaisyClaw agent");
    expect(bridgeParams?.instructions).toContain("short natural backchannel");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("daisyclaw_agent_consult");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("daisyclaw_agent_control");
    const player = getLastAudioPlayer();
    bridgeParams?.audioSink?.sendAudio(Buffer.alloc(24_000));
    ...
    bridgeParams?.onToolCall?.(
      {
        ...
        args: {
          question: [
            "Speak this exact DaisyClaw answer to the Discord voice channel, without adding, removing, or rephrasing words.",
  ...
  it("routes active-run realtime transcripts to voice control before forced consults", async () => {
    controlRealtimeVoiceAgentRunMock.mockResolvedValueOnce({
      ...
      active: true,
      aborted: true,
      message: "Cancelled the active DaisyClaw run.",
      speak: true,
      show: true,
      suppress: false,
    });
    ...
    await vi.waitFor(() => expectUserMessageIncludes("Cancelled the active DaisyClaw run."));
    expect(textToSpeechMock).not.toHaveBeenCalledWith(
      expect.objectContaining({ text: "Cancelled the active DaisyClaw run." }),
    );

    const stopCallsAfterControl = player.stop.mock.calls.length;
    bridgeParams?.onTranscript?.("assistant", "Cancelled the active DaisyClaw run.", true);
  ...
  it("accepts DaisyClaw as a default wake name before realtime agent-proxy consults", async () => {
    agentCommandMock.mockResolvedValueOnce({ payloads: [{ text: "daisyclaw wake answer" }] });
    const manager = createManager(
      {
        groupPolicy: "open",
        voice: {
    ...
    ownerTurn?.sendInputAudio(Buffer.alloc(8));
    await emitFinalRealtimeUserTranscript(bridgeParams, "DaisyClaw, how is it going");
    ...
    expect(lastAgentCommandArgs().message).toContain("how is it going");
    expect(lastAgentCommandArgs().message).not.toContain("DaisyClaw");
    expectUserMessageIncludes("daisyclaw wake answer");
  });
  ...
  it("ignores default agent wake names longer than two words", async () => {
    ...
    await emitFinalRealtimeUserTranscript(bridgeParams, "DaisyClaw, fallback still wakes");
  ...
  it("accepts leading fuzzy wake names before realtime agent-proxy consults", async () => {
    ...
    const daisyClawTurn = entry.realtime?.beginSpeakerTurn(
      { extraSystemPrompt: undefined, senderIsOwner: true, speakerLabel: "Owner" },
      "u-owner",
    );
    daisyClawTurn?.sendInputAudio(Buffer.alloc(8));
  ...
  it("does not accept configured realtime wake names longer than two words", async () => {
    ...
    fallbackTurn?.sendInputAudio(Buffer.alloc(8));
    await emitFinalRealtimeUserTranscript(bridgeParams, "DaisyClaw, ship it");
  ...
  it("keeps forced agent-proxy fallback diagnostics out of agent prompts", async () => {
    ...
    expect(lastAgentCommandArgs().message).not.toContain("daisyclaw_agent_consult");
  ...
  it("reuses forced agent-proxy answers for late matching consult tool calls", async () => {
    ...
    expect(realtimeSessionMock.submitToolResult).toHaveBeenCalledWith(
      "call-late",
      {
        ...
        message: "DaisyClaw already delivered this answer to Discord voice.",
      },
  ...
  it("starts Discord realtime voice in bidi mode with the consult tool", async () => {
    ...
    expect(bridgeParams?.autoRespondToAudio).toBe(true);
    expect(bridgeParams?.interruptResponseOnInputAudio).toBe(false);
    expect(bridgeParams?.instructions).toContain("Call daisyclaw_agent_consult");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("daisyclaw_agent_consult");
  ...
  it("adds default bootstrap profile context to realtime voice instructions", async () => {
    ...
    resolveRealtimeBootstrapContextInstructionsMock.mockResolvedValue(
      "DaisyClaw realtime voice profile context:\n\n### IDENTITY.md\nName: Wilfred",
    );
    const manager = createManager({
      groupPolicy: "open",
      voice: {
    ...
    const bridgeParams = lastRealtimeBridgeParams() as
      ...
      | undefined;
    expect(bridgeParams?.instructions).toContain("DaisyClaw realtime voice profile context");
