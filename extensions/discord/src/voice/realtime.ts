...
import type { DiscordAccountConfig, DaisyClawConfig } from "daisyclaw/plugin-sdk/config-contracts";
import {
  asDateTimestampMs,
...
import { createSubsystemLogger } from "daisyclaw/plugin-sdk/runtime-env";
import { formatErrorMessage } from "daisyclaw/plugin-sdk/ssrf-runtime";
import { asBoolean, uniqueStrings } from "daisyclaw/plugin-sdk/string-coerce-runtime";
...
const DISCORD_REALTIME_FORCED_CONSULT_REASON =
  "provider_final_transcript_without_daisyclaw_agent_consult";
...
export function buildDiscordSpeakExactUserMessage(text: string): string {
  return [
    "Internal DaisyClaw voice playback result.",
    "Do not call daisyclaw_agent_consult or any other tool for this message.",
...
function extractDiscordExactSpeechConsultText(args: unknown): string | undefined {
  ...
  if (
    !message.includes("Speak this exact DaisyClaw answer") &&
...
function resolveDiscordRealtimeWakeNames(params: {
  ...
  const configuredAgentNames = [agent?.name, agent?.identity?.name]
    .map((name) => normalizeSupportedRealtimeVoiceActivationName(name))
    .filter((name): name is string => Boolean(name));
  const productWakeNames = [normalizeSupportedRealtimeVoiceActivationName("DaisyClaw")].filter(
...
function buildDiscordRealtimeInstructions(params: {
  ...
  const base =
    params.instructions ??
    [
      "You are DaisyClaw's Discord voice interface.",
  ...
  if (isDiscordAgentProxyVoiceMode(params.mode)) {
    return [
      ...
      "Mode: DaisyClaw agent proxy.",
      "You are the realtime voice surface for the same DaisyClaw agent the user can message directly.",
      "Do not mention a backend, supervisor, helper, or separate system. Present the result as your own work.",
      "Delegate substantive requests, actions, tool work, current facts, memory, workspace context, and user-specific context with daisyclaw_agent_consult.",
      "Do not block, refuse, or downscope at the voice layer. Delegate to DaisyClaw and treat its result as authoritative.",
      "Answer directly only for greetings, acknowledgements, brief latency tests, or filler while waiting.",
      'While waiting for DaisyClaw data or tool results, use at most one short natural backchannel such as "yeah", "mm-hmm", "got it", or "one sec"; vary it and do not treat it as the final answer.',
      "When DaisyClaw sends an internal exact answer to speak, do not call tools. Say only that answer.",
      buildRealtimeVoiceAgentConsultPolicyInstructions({
