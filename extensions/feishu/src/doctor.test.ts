...
function feishuConfig(): DaisyClawConfig {
  return {
    channels: {
      feishu: {
        appId: "cli_xxx",
...
function stateDir(): string {
  const dir = process.env.DAISYCLAW_STATE_DIR;
  if (!dir) {
...
describe("Feishu doctor state repair", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-feishu-doctor-"));
  ...
  it("warns before repair when Feishu local state is corrupt", async () => {
    ...
    expect(result.warningNotes.join("\n")).toContain("daisyclaw doctor --fix");
  });
