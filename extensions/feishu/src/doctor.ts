...
function collectConfiguredAgentIds(cfg: DaisyClawConfig): string[] {
...
function resolveConfiguredDefaultAgentId(cfg: DaisyClawConfig): string {
...
function formatPreviewWarning(inspection: FeishuDoctorInspection): string {
  ...
  return [
    ...
    '- Run "daisyclaw doctor --fix" to rebuild Feishu local state.',
  ].join("\n");
}
...
function hasConfiguredFeishuChannel(cfg: DaisyClawConfig): boolean {
