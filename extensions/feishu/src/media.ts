...
import type { MessageReceipt } from "daisyclaw/plugin-sdk/channel-outbound";
import { mediaKindFromMime } from "daisyclaw/plugin-sdk/media-mime";
import { MEDIA_FFMPEG_MAX_AUDIO_DURATION_SECS, runFfmpeg } from "daisyclaw/plugin-sdk/media-runtime";
import { saveMediaBuffer, saveMediaStream, type SavedMedia } from "daisyclaw/plugin-sdk/media-store";
import { readByteStreamWithLimit } from "daisyclaw/plugin-sdk/response-limit-runtime";
import { readRegularFile, writeExternalFileWithinRoot } from "daisyclaw/plugin-sdk/security-runtime";
...
export async function downloadImageFeishu(params: {
  ...
  const buffer = await readFeishuResponseBuffer({
    response,
    tmpDirPrefix: "daisyclaw-feishu-img-",
...
async function downloadMessageResourceWithType(params: {
  ...
  const buffer = await readFeishuResponseBuffer({
    response,
    tmpDirPrefix: "daisyclaw-feishu-resource-",
...
async function transcodeToFeishuVoiceOpus(params: {
  ...
  return await withTempWorkspace(
    { rootDir: resolvePreferredDaisyClawTmpDir(), prefix: "feishu-voice-" },
    async (workspace) => {
      const ext = normalizeLowercaseStringOrEmpty(path.extname(params.fileName));
