...
export function createToolFactoryHarness(cfg: DaisyClawPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<DaisyClawPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
