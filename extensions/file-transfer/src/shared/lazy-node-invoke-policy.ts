...
import type { DaisyClawPluginNodeInvokePolicy } from "daisyclaw/plugin-sdk/plugin-entry";
import { FILE_TRANSFER_NODE_INVOKE_COMMANDS } from "./node-invoke-policy-commands.js";

type LoadFileTransferNodeInvokePolicy = () => Promise<DaisyClawPluginNodeInvokePolicy>;
...
export function createLazyFileTransferNodeInvokePolicy(
  ...
  let policyPromise: Promise<DaisyClawPluginNodeInvokePolicy> | undefined;

  return {
    commands: [...FILE_TRANSFER_NODE_INVOKE_COMMANDS],
    async handle(ctx) {
      let policy: DaisyClawPluginNodeInvokePolicy;
      try {
