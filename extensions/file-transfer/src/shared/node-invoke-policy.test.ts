...
import type { DaisyClawPluginNodeInvokePolicyContext } from "daisyclaw/plugin-sdk/plugin-entry";
...
function createCtx(overrides: {
  ...
  approvals?: DaisyClawPluginNodeInvokePolicyContext["approvals"];
}) {
  const invokeNode = vi.fn<DaisyClawPluginNodeInvokePolicyContext["invokeNode"]>(
    async ({
      params,
    }: Parameters<DaisyClawPluginNodeInvokePolicyContext["invokeNode"]>[0] = {}) => ({
      ok: true,
      payload: {
        ok: true,
        path:
...
function requireInvokeParams(
  invokeNode: ReturnType<typeof vi.fn<DaisyClawPluginNodeInvokePolicyContext["invokeNode"]>>,
  callIndex: number,
) {
