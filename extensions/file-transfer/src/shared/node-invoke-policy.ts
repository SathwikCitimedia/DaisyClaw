...
import { readPositiveIntegerParam } from "daisyclaw/plugin-sdk/param-readers";
import type {
  DaisyClawPluginNodeInvokePolicy,
  DaisyClawPluginNodeInvokePolicyContext,
  DaisyClawPluginNodeInvokePolicyResult,
...
}): Promise<DaisyClawPluginNodeInvokePolicyResult | null> {
  const nodeDisplayName = input.ctx.node?.displayName;
  const missingCode =
...
type PreflightResult =
  ...
  | {
      ok: false;
      result: DaisyClawPluginNodeInvokePolicyResult;
    };
...
async function handleFileTransferInvoke(
  ctx: DaisyClawPluginNodeInvokePolicyContext,
): Promise<DaisyClawPluginNodeInvokePolicyResult> {
...
export function createFileTransferNodeInvokePolicy(): DaisyClawPluginNodeInvokePolicy {
  return {
