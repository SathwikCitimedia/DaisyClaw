...
import { canResolveEnvSecretRefInReadOnlyPath } from "daisyclaw/plugin-sdk/extension-shared";
import { resolvePositiveTimeoutSeconds } from "daisyclaw/plugin-sdk/provider-web-fetch";
import { resolveSecretInputString, normalizeSecretInput } from "daisyclaw/plugin-sdk/secret-input";
...
type WebSearchConfig = NonNullable<DaisyClawConfig["tools"]>["web"] extends infer Web
  ? Web extends { search?: infer Search }
    ? Search
    : undefined
  : undefined;

type WebFetchConfig = NonNullable<DaisyClawConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: DaisyClawConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: DaisyClawConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: DaisyClawConfig): FirecrawlSearchConfig {
...
function resolveFirecrawlFetchConfig(cfg?: DaisyClawConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: DaisyClawConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: DaisyClawConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: DaisyClawConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: DaisyClawConfig, override?: number): number {
