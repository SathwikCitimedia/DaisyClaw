...
import { MAX_DATE_TIMESTAMP_MS, MAX_TIMER_TIMEOUT_MS } from "daisyclaw/plugin-sdk/number-runtime";
import type {
  DaisyClawConfig,
  DaisyClawPluginApi,
  ProviderAuthResult,
...
type RegisteredMemoryEmbeddingProvider = Parameters<
  DaisyClawPluginApi["registerMemoryEmbeddingProvider"]
>[0];
type RegisteredProvider = Parameters<DaisyClawPluginApi["registerProvider"]>[0];
type GithubCopilotTestProvider = RegisteredProvider & {
  auth: Array<{
    run: (ctx: unknown) => Promise<ProviderAuthResult | null>;
    runNonInteractive: (ctx: unknown) => Promise<DaisyClawConfig | null>;
  }>;
  catalog: {
...
async function createAgentDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-github-copilot-test-"));
  tempDirs.push(dir);
  return dir;
}
...
function registerProviderAndCatalogWithPluginConfig(pluginConfig: Record<string, unknown>) {
  const registerProviderMock = vi.fn<DaisyClawPluginApi["registerProvider"]>();
  const registerModelCatalogProviderMock =
    vi.fn<DaisyClawPluginApi["registerModelCatalogProvider"]>();
...
describe("github-copilot plugin", () => {
  it("registers embedding provider", () => {
    const registerMemoryEmbeddingProviderMock =
      vi.fn<DaisyClawPluginApi["registerMemoryEmbeddingProvider"]>();
