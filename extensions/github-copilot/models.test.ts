...
import { createProviderUsageFetch, makeResponse } from "daisyclaw/plugin-sdk/test-env";
...
vi.mock("daisyclaw/plugin-sdk/provider-model-shared", async (importOriginal) => ({
  ...(await importOriginal<typeof import("daisyclaw/plugin-sdk/provider-model-shared")>()),
...
vi.mock("daisyclaw/plugin-sdk/json-store", () => ({
  loadJsonFile: jsonStoreMocks.loadJsonFile,
  saveJsonFile: jsonStoreMocks.saveJsonFile,
}));

vi.mock("daisyclaw/plugin-sdk/state-paths", () => ({
  resolveStateDir: () => "/tmp/daisyclaw-state",
}));

import type { ProviderResolveDynamicModelContext } from "daisyclaw/plugin-sdk/core";
...
describe("github-copilot token", () => {
  const cachePath = "/tmp/daisyclaw-state/credentials/github-copilot.token.json";
