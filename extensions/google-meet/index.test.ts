...
} from "daisyclaw/plugin-sdk/json-schema-runtime";
import type { RealtimeTranscriptionProviderPlugin } from "daisyclaw/plugin-sdk/realtime-transcription";
import type { RealtimeVoiceProviderPlugin } from "daisyclaw/plugin-sdk/realtime-voice";
...
vi.mock("daisyclaw/plugin-sdk/ssrf-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("daisyclaw/plugin-sdk/ssrf-runtime")>();
  return {
    ...actual,
...
describe("google-meet plugin", () => {
  ...
  afterAll(() => {
    vi.doUnmock("daisyclaw/plugin-sdk/ssrf-runtime");
  ...
  it("defaults to chrome agent mode with safe read-only tools", () => {
    ...
    expect(config.realtime.instructions).toContain("daisyclaw_agent_consult");
  ...
  it("declares advanced config metadata in the plugin entry and manifest", () => {
    const manifest = JSON.parse(
      readFileSync(new URL("./daisyclaw.plugin.json", import.meta.url), "utf8"),
    ) as {
  ...
  it("uses env fallbacks for OAuth, preview, and default meeting values", () => {
    const config = resolveGoogleMeetConfigWithEnv(
      {},
      {
        DAISYCLAW_GOOGLE_MEET_CLIENT_ID: "client-id",
        GOOGLE_MEET_CLIENT_SECRET: "client-secret",
        DAISYCLAW_GOOGLE_MEET_REFRESH_TOKEN: "refresh-token",
        GOOGLE_MEET_ACCESS_TOKEN: "access-token",
        DAISYCLAW_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT: "123456",
        GOOGLE_MEET_DEFAULT_MEETING: "https://meet.google.com/abc-defg-hij",
        DAISYCLAW_GOOGLE_MEET_PREVIEW_ACK: "true",
      },
    );
  ...
  it.each(["0x10", "1e3"])("ignores non-decimal env numeric fallbacks: %s", (expiresAt) => {
    const config = resolveGoogleMeetConfigWithEnv(
      {},
      {
        DAISYCLAW_GOOGLE_MEET_ACCESS_TOKEN: "access-token",
        DAISYCLAW_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT: expiresAt,
      },
    );
  ...
  it("uses a provider-safe flat tool parameter schema", () => {
    ...
    expect(properties.mode).toEqual({
      ...
      description:
        "Join mode. agent uses realtime transcription, the configured DaisyClaw agent, and regular TTS. bidi uses the realtime voice model directly. transcribe joins observe-only.",
    });
  });
  ...
  it("reports setup status through the tool", async () => {
    ...
    try {
      const { tools } = setup({
        chrome: {
          audioInputCommand: ["daisyclaw-audio-bridge", "capture"],
          audioOutputCommand: ["daisyclaw-audio-bridge", "play"],
        },
      });
      const tool = tools[0] as {
  ...
  it("writes export bundles through the tool", async () => {
    stubMeetArtifactsApi();
    const tempDir = mkdtempSync(path.join(tmpdir(), "daisyclaw-google-meet-tool-export-"));
    const { tools } = setup();
    const tool = tools[0] as {
      execute: (
        id: string,
  ...
  it("dry-runs export bundles through the tool", async () => {
    stubMeetArtifactsApi();
    const parentDir = mkdtempSync(path.join(tmpdir(), "daisyclaw-google-meet-tool-dry-run-"));
          ...
          if (request.path === "/act") {
            return {
              result: JSON.stringify({
                ...
                manualActionMessage: "Admit the DaisyClaw browser participant in Google Meet.",
                title: "Meet",
                url: "https://meet.google.com/abc-defg-hij",
              }),
            };
  ...
  it("retries caption enable until the captions button is available", async () => {
    ...
    const captionsStateKey = "__daisyclawMeetCaptions";
    ...
    const { tools } = setup(
      ...
      },
      {
        browserActResult: {
          ...
          manualActionMessage:
            "Sign in to Google in the DaisyClaw browser profile, then retry the Meet join.",
  ...
  it("explains when chrome-node has no capable paired node", async () => {
    ...
    expect(result.details.error).toContain("daisyclaw node run");
  });
  ...
  it("pipes Chrome command-pair audio through the realtime provider", async () => {
    ...
    expect(progressPayload.tool).toBe("daisyclaw_agent_consult");
    ...
    expect(callbacks.tools?.map((tool) => tool.name)).toContain("daisyclaw_agent_consult");
    await vi.waitFor(() => {
