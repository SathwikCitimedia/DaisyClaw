...
export const legacyConfigRules: LegacyConfigRule[] = [
  {
    ...
    message:
      'plugins.entries.google-meet.config.realtime.provider="google" is legacy for Gemini Live bidi mode; use realtime.voiceProvider="google" and realtime.transcriptionProvider="openai". Run "daisyclaw doctor --fix".',
    match: hasLegacyGoogleRealtimeProvider,
  },
];

export function migrateGoogleMeetLegacyRealtimeProvider(config: DaisyClawConfig): {
...
export function normalizeCompatibilityConfig({ cfg }: { cfg: DaisyClawConfig }): {
  config: DaisyClawConfig;
  changes: string[];
} {
