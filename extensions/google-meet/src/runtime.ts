...
import type { PluginRuntime, RuntimeLogger } from "daisyclaw/plugin-sdk/plugin-runtime";
  ...
  if (health?.manualActionRequired) {
    return {
      ...
      message:
        ...
        "Resolve the Google Meet browser prompt before asking DaisyClaw to speak.",
    };
  }
  if (health?.inCall === true) {
    if (health.micMuted === true) {
      return {
        ...
        message: "Turn on the DaisyClaw Google Meet microphone before asking DaisyClaw to speak.",
      };
    }
    if (session.chrome.audioBridge) {
      return { ready: true };
