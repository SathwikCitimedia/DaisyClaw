...
import { isBlockedHostnameOrIp } from "daisyclaw/plugin-sdk/ssrf-runtime";
import { asRecord, normalizeOptionalString } from "daisyclaw/plugin-sdk/string-coerce-runtime";
...
export function getGoogleMeetSetupStatus(
  ...
  checks.push({
    ...
    message: config.chrome.browserProfile
      ? "Local Chrome uses the DaisyClaw browser profile; chrome.browserProfile is passed to chrome-node hosts"
      : "Local Chrome uses the DaisyClaw browser profile; configure browser.defaultProfile to choose another profile",
  });
