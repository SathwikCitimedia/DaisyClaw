...
import { transcodeAudioBufferToOpus } from "daisyclaw/plugin-sdk/media-runtime";
import {
...
import { asObject, trimToUndefined } from "daisyclaw/plugin-sdk/speech-core";
...
function resolveGoogleTtsModelProviderApiKey(cfg?: DaisyClawConfig): string | undefined {
...
function isDaisyClawGoogleAudioProfilePrompt(text: string): boolean {
  return (
...
export function buildGoogleSpeechProvider(): SpeechProviderPlugin {
  return {
    ...
    prepareSynthesis: (ctx) => {
      ...
      if (!shouldWrap || isDaisyClawGoogleAudioProfilePrompt(ctx.text)) {
        return undefined;
      }
      return {
