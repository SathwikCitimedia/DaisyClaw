...
vi.mock("daisyclaw/plugin-sdk/provider-transport-runtime", async (importOriginal) => ({
...
const MODEL_PROVIDER_REQUEST_TRANSPORT_SYMBOL = Symbol.for(
  "daisyclaw.modelProviderRequestTransport",
);
...
describe("google transport stream", () => {
  ...
  afterAll(() => {
    vi.doUnmock("daisyclaw/plugin-sdk/provider-transport-runtime");
  ...
  it("rejects non-integer Gemini 3 first-response retry env values", () => {
    const envName = "DAISYCLAW_GOOGLE_GEMINI_FIRST_RESPONSE_RETRY_MS";
  ...
  it("retries Gemini 3 requests with lean thinking when the first attempt has no first response", async () => {
    vi.stubEnv("DAISYCLAW_GOOGLE_GEMINI_FIRST_RESPONSE_RETRY_MS", "10");
    guardedFetchMock
  ...
  it("detects supported Vertex ADC sources synchronously", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "daisyclaw-google-vertex-adc-detect-"));
  ...
  it("resolves non-file Vertex ADC through google-auth-library without OAuth refresh fetch", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "daisyclaw-google-vertex-authlib-"));
  ...
  it("does not cache google-auth ADC tokens when fallback expiry would exceed Date range", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "daisyclaw-google-vertex-authlib-expiry-"));
    vi.useFakeTimers();
  ...
  it("uses google-auth-library bearer auth for Google Vertex credential marker requests", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "daisyclaw-google-vertex-authlib-stream-"));
    vi.stubEnv("GOOGLE_APPLICATION_CREDENTIALS", "");
    vi.stubEnv("HOME", path.join(tempDir, "home"));
    vi.stubEnv("APPDATA", "");
  ...
  it("refreshes authorized_user ADC before Google Vertex requests", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "daisyclaw-google-vertex-adc-"));
  ...
  it("refreshes authorized_user ADC from a compressed token response", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "daisyclaw-google-vertex-adc-gzip-"));
    const credentialsPath = path.join(tempDir, "application_default_credentials.json");
    await writeFile(
      credentialsPath,
      JSON.stringify({
  ...
  it("does not reuse authorized_user ADC tokens with unsafe expiry lifetimes", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "daisyclaw-google-vertex-unsafe-adc-"));
    const credentialsPath = path.join(tempDir, "application_default_credentials.json");
    await writeFile(
      credentialsPath,
      JSON.stringify({
  ...
  it("refreshes authorized_user ADC from the Windows APPDATA fallback for Google Vertex requests", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "daisyclaw-google-vertex-appdata-adc-"));
  ...
  it("treats the Google transport alias as the same route for signature replay", () => {
    const model = {
      ...
      api: "daisyclaw-google-generative-ai-transport",
    } as Model<"daisyclaw-google-generative-ai-transport">;
