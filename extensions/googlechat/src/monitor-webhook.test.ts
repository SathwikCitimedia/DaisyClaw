...
import type { FixedWindowRateLimiter } from "daisyclaw/plugin-sdk/webhook-ingress";
...
vi.mock("daisyclaw/plugin-sdk/webhook-request-guards", () => ({
  readJsonWebhookBodyOrReject,
}));

vi.mock("daisyclaw/plugin-sdk/webhook-targets", () => ({
...
describe("googlechat monitor webhook", () => {
  ...
  afterAll(() => {
    vi.doUnmock("daisyclaw/plugin-sdk/webhook-request-guards");
    vi.doUnmock("daisyclaw/plugin-sdk/webhook-targets");
    vi.doUnmock("./auth.js");
    vi.resetModules();
  });
    ...
    expect(processEvent).toHaveBeenCalledWith(
      {
        ...
        action: {
          parameters: [
            { key: "daisyclaw_action", value: "approval" },
