...
import { readBooleanParam } from "daisyclaw/plugin-sdk/boolean-param";
import {
  createActionGate,
  jsonResult,
...
import { normalizeOptionalLowercaseString } from "daisyclaw/plugin-sdk/string-coerce-runtime";
import { extractToolSend } from "daisyclaw/plugin-sdk/tool-send";
  ...
  handleAction: async ({ action, params, cfg, accountId, toolContext }) => {
    ...
    const assertPrivateApiEnabled = async () => {
      ...
      if (!privateApiStatus?.available) {
        ...
        const reason = privateApiStatus?.statusMessage
          ...
          : "";
        log.warn(
          `iMessage ${action} blocked: private API bridge unavailable (accountId=${account.accountId}, cliPath=${cliPathForProbe}). Run \`imsg launch\` to re-inject the dylib, then \`daisyclaw channels status\` to refresh.${reason}`,
        );
        throw new Error(
          `iMessage ${action} requires the imsg private API bridge. Run imsg launch, then daisyclaw channels status to refresh capability detection.${reason}`,
        );
      }
    };
    const opts = {
    ...
    if (action === "reply") {
      ...
      if (attachment) {
        ...
        // (daisyclaw/imsg#114). Older imsg builds reject the option, so
        ...
        if (privateApiStatus?.cliCapabilities?.sendRichSupportsAttachment !== true) {
          throw new Error(
            "iMessage reply with an attachment needs an imsg build that exposes `send-rich --file` " +
              "(daisyclaw/imsg#114). Upgrade imsg, or use action 'upload-file' (with filePath/filename) " +
