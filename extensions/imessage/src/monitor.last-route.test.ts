...
import type { waitForTransportReady } from "daisyclaw/plugin-sdk/transport-ready-runtime";
...
vi.mock("daisyclaw/plugin-sdk/transport-ready-runtime", () => ({
  waitForTransportReady: waitForTransportReadyMock,
}));

vi.mock("daisyclaw/plugin-sdk/conversation-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("daisyclaw/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
...
vi.mock("daisyclaw/plugin-sdk/channel-inbound", async (importOriginal) => {
  const actual = await importOriginal<typeof import("daisyclaw/plugin-sdk/channel-inbound")>();
  return {
    ...actual,
...
vi.mock("daisyclaw/plugin-sdk/reply-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("daisyclaw/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
...
describe("iMessage monitor last-route updates", () => {
      ...
      config: {
        channels: {
          imessage: {
            ...
            dbPath: path.join(os.tmpdir(), `daisyclaw-missing-chat-${Date.now()}.db`),
            dmPolicy: "allowlist",
            allowFrom: ["+15550001111"],
          },
        },
  ...
  it("passes the startup rowid watermark as since_rowid when chat.db is readable", async () => {
    ...
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-imsg-startup-rowid-"));
  ...
  it("preserves enabled legacy catchup as the startup replay path", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-imsg-catchup-window-"));
    tempDirs.push(stateDir);
    const dbPath = path.join(stateDir, "chat.db");
    const { DatabaseSync } = await import("node:sqlite");
    const database = new DatabaseSync(dbPath);
  ...
  it("recovers downtime messages: replays from the cursor and delivers replay rows older than the live fence", async () => {
    advanceIMessageRecoveryCursor("default", 4990);
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-imsg-recovery-"));
    tempDirs.push(stateDir);
    const dbPath = path.join(stateDir, "chat.db");
    const { DatabaseSync } = await import("node:sqlite");
    const database = new DatabaseSync(dbPath);
  ...
  it("does not treat startup-boundary rows as recovery replay without a prior cursor", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-imsg-first-run-boundary-"));
    tempDirs.push(stateDir);
    const dbPath = path.join(stateDir, "chat.db");
    const { DatabaseSync } = await import("node:sqlite");
    const database = new DatabaseSync(dbPath);
  ...
  it("records a suppressed live row so a later replay of the same row is deduped, not delivered", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-imsg-suppress-record-"));
    tempDirs.push(stateDir);
    const dbPath = path.join(stateDir, "chat.db");
    const { DatabaseSync } = await import("node:sqlite");
    const database = new DatabaseSync(dbPath);
  ...
  it("does not advance the recovery cursor past a failed replay row", async () => {
    ...
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-imsg-recovery-failed-"));
    tempDirs.push(stateDir);
    const dbPath = path.join(stateDir, "chat.db");
    const { DatabaseSync } = await import("node:sqlite");
    const database = new DatabaseSync(dbPath);
  ...
  it("advances the recovery cursor after lower pending replay rows complete", async () => {
    advanceIMessageRecoveryCursor("default", 4990);
    debouncerControl.holdEntries = true;
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-imsg-recovery-ordered-"));
    tempDirs.push(stateDir);
    const dbPath = path.join(stateDir, "chat.db");
    const { DatabaseSync } = await import("node:sqlite");
    const database = new DatabaseSync(dbPath);
  ...
  it("repairs anchorless group watch payloads before routing or cursor updates", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-imsg-anchor-repair-"));
    tempDirs.push(stateDir);
        ...
        onNotification?.({
          ...
          params: {
            message: {
              ...
              sender: "+15550001111",
              is_from_me: false,
              text: "@daisyclaw check this https://example.com",
              is_group: false,
              chat_guid: "",
              chat_identifier: "",
              chat_name: "",
  ...
  it("legacy-merges coalesce buckets when imsg emits no balloon metadata (older builds)", async () => {
    ...
    // upstream (daisyclaw/imsg#141, tracked by #91243).
    debouncerControl.holdEntries = true;
