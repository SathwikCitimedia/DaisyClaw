...
import type { BaseProbeResult } from "daisyclaw/plugin-sdk/channel-contract";
import {
  asDateTimestampMs,
  resolveExpiresAtMsFromDurationMs,
} from "daisyclaw/plugin-sdk/number-runtime";
import { runCommandWithTimeout } from "daisyclaw/plugin-sdk/process-runtime";
...
import { detectBinary } from "daisyclaw/plugin-sdk/setup";
import {
...
export function resolveIMessageNonMacHostError(
  ...
  return "iMessage via the default imsg CLI must run on macOS. Run DaisyClaw on the signed-in Messages Mac, or set channels.imessage.cliPath to an SSH wrapper that runs imsg on that Mac.";
}
...
// subcommand (added by daisyclaw/imsg#114, which lets a single bridge call
...
export async function probeIMessagePrivateApi(
  ...
  try {
    ...
    // daisyclaw/imsg#114. We do this even when the bridge is unavailable
