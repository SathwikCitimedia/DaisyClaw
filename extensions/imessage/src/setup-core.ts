...
export const imessageDmPolicy = {
  ...
  resolveConfigKeys: (_cfg: DaisyClawConfig, accountId?: string) => {
  ...
  getCurrent: (cfg: DaisyClawConfig, accountId?: string) => {
...
function resolveIMessageCliPath(params: { cfg: DaisyClawConfig; accountId: string }) {
...
export const imessageCompletionNote = {
  ...
  lines: [
    "Run DaisyClaw on the Mac signed into Messages, or set cliPath to an SSH wrapper that runs imsg on that Mac.",
    "Linux/Windows hosts cannot run the default local imsg path directly.",
    "Run `imsg launch`, then `daisyclaw channels status --probe` to verify private API actions.",
    "Ensure DaisyClaw has Full Disk Access to Messages DB.",
...
export const imessageSetupStatusBase = {
  ...
  configuredScore: 1,
  unconfiguredScore: 0,
  resolveConfigured: ({ cfg, accountId }: { cfg: DaisyClawConfig; accountId?: string }) =>
