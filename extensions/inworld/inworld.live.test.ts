...
describeLive("inworld plugin live", () => {
  ...
  it("synthesizes MP3, native voice-note Ogg/Opus, and telephony PCM", async () => {
    ...
    const audioFile = await provider.synthesize({
      text: "DaisyClaw Inworld text to speech integration test OK.",
    ...
    const voiceNote = await provider.synthesize({
      text: "DaisyClaw Inworld voice note integration test OK.",
      cfg: { plugins: { enabled: true } } as never,
      providerConfig,
      target: "voice-note",
      timeoutMs: 90_000,
    ...
    const telephony = await provider.synthesizeTelephony?.({
      text: "DaisyClaw Inworld telephony check OK.",
      cfg: { plugins: { enabled: true } } as never,
      providerConfig,
      timeoutMs: 90_000,
    });
