...
import { resolveInboundRouteEnvelopeBuilderWithRuntime } from "daisyclaw/plugin-sdk/inbound-envelope";
import {
...
export async function handleIrcInbound(params: {
  ...
  const allowTextCommands = core.channel.commands.shouldHandleTextCommands({
    cfg: config as DaisyClawConfig,
    surface: CHANNEL_ID,
  });
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as DaisyClawConfig);
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as DaisyClawConfig);
