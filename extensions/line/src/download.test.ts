...
vi.mock("daisyclaw/plugin-sdk/media-store", () => ({
...
describe("downloadLineMedia", () => {
  ...
  afterAll(() => {
    ...
    vi.doUnmock("daisyclaw/plugin-sdk/media-store");
    vi.resetModules();
  });
  ...
  beforeEach(() => {
    ...
    saveMediaStreamMock.mockImplementation(
      async (stream: AsyncIterable<Buffer>, contentType?: string, subdir?: string) => {
        ...
        return {
          path: `/home/user/.daisyclaw/media/${subdir ?? "unknown"}/saved-media`,
  ...
  it("persists inbound media with the shared media store", async () => {
    ...
    expect(result).toEqual({
      path: "/home/user/.daisyclaw/media/inbound/saved-media",
