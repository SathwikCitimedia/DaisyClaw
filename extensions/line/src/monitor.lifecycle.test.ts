...
import { createMockIncomingRequest } from "daisyclaw/plugin-sdk/test-env";
import { WEBHOOK_IN_FLIGHT_DEFAULTS } from "daisyclaw/plugin-sdk/webhook-request-guards";
...
vi.mock("daisyclaw/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<typeof import("daisyclaw/plugin-sdk/webhook-ingress")>(
    "daisyclaw/plugin-sdk/webhook-ingress",
  );
  return {
    ...actual,
...
describe("monitorLineProvider lifecycle", () => {
  ...
  afterAll(() => {
    vi.doUnmock("./bot.js");
    vi.doUnmock("daisyclaw/plugin-sdk/reply-runtime");
    vi.doUnmock("daisyclaw/plugin-sdk/runtime-env");
    vi.doUnmock("daisyclaw/plugin-sdk/webhook-ingress");
