...
`daisyclaw.invoke --each`) without adding new DaisyClaw code per workflow.
...
This extension depends on DaisyClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** DaisyClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.daisyclaw/extensions` as a standalone plugin directory.
