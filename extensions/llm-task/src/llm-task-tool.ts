...
import { buildModelAliasIndex, resolveModelRefFromString } from "daisyclaw/plugin-sdk/agent-runtime";
import {
...
import { resolvePreferredDaisyClawTmpDir, withTempWorkspace } from "../api.js";
...
type ThinkingPolicy = ReturnType<DaisyClawPluginApi["runtime"]["agent"]["resolveThinkingPolicy"]>;

export const llmTaskToolDefinition = {
  name: "llm-task",
  label: "LLM Task",
  description:
    "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via daisyclaw.invoke.",
...
function supportsThinkingPolicyLevel(
  ...
  level: ReturnType<DaisyClawPluginApi["runtime"]["agent"]["normalizeThinkingLevel"]>,
): boolean {
  return Boolean(level) && policy.levels.some((entry) => entry.id === level);
}

export function createLlmTaskTool(api: DaisyClawPluginApi) {
  return {
    ...
    async execute(_id: string, params: LlmTaskParams) {
      ...
      let thinkLevel: ReturnType<DaisyClawPluginApi["runtime"]["agent"]["normalizeThinkingLevel"]> =
        undefined;
      if (thinkingRaw) {
      ...
      return await withTempWorkspace(
        { rootDir: resolvePreferredDaisyClawTmpDir(), prefix: "daisyclaw-llm-task-" },
        async ({ dir: tmpDir }) => {
