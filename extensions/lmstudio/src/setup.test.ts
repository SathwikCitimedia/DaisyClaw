...
import { CUSTOM_LOCAL_AUTH_MARKER } from "daisyclaw/plugin-sdk/provider-auth";
import type { DaisyClawConfig } from "daisyclaw/plugin-sdk/provider-auth";
import type { ModelDefinitionConfig } from "daisyclaw/plugin-sdk/provider-model-shared";
import { resolveAgentModelPrimaryValue } from "daisyclaw/plugin-sdk/provider-onboard";
import {
  SELF_HOSTED_DEFAULT_CONTEXT_WINDOW,
  type ProviderAuthMethodNonInteractiveContext,
  type ProviderCatalogContext,
} from "daisyclaw/plugin-sdk/provider-setup";
import type { WizardPrompter } from "daisyclaw/plugin-sdk/setup";
...
vi.mock("daisyclaw/plugin-sdk/provider-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("daisyclaw/plugin-sdk/provider-auth")>();
  return {
    ...actual,
...
vi.mock("daisyclaw/plugin-sdk/provider-setup", async (importOriginal) => {
  const actual = await importOriginal<typeof import("daisyclaw/plugin-sdk/provider-setup")>();
  return {
    ...actual,
...
afterAll(() => {
  ...
  vi.doUnmock("daisyclaw/plugin-sdk/provider-auth");
  vi.doUnmock("daisyclaw/plugin-sdk/provider-setup");
  vi.resetModules();
});
...
function buildConfig(): DaisyClawConfig {
  return {
    models: {
      providers: {
        lmstudio: {
...
describe("lmstudio setup", () => {
  ...
  it("non-interactive setup fails when requested model is missing", async () => {
    ...
    const dockerSetup = ["1", "true", "yes", "on"].includes(
      process.env.DAISYCLAW_DOCKER_SETUP?.trim().toLowerCase() ?? "",
    );
  ...
  it("interactive Docker setup defaults to the host LM Studio endpoint", async () => {
    vi.stubEnv("DAISYCLAW_DOCKER_SETUP", "1");
    const { prompter, text } = createQueuedWizardPrompterHarness([
