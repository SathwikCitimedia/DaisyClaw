...
import { withAgentModelAliases } from "daisyclaw/plugin-sdk/provider-onboard";
import {
...
import { WizardCancelledError, type WizardPrompter } from "daisyclaw/plugin-sdk/setup";
...
function resolveLmstudioSetupDefaultBaseUrl(env: NodeJS.ProcessEnv = process.env): string {
  return isTruthyEnvValue(env.DAISYCLAW_DOCKER_SETUP)
    ? LMSTUDIO_DOCKER_HOST_BASE_URL
    : LMSTUDIO_DEFAULT_BASE_URL;
}
...
function stripLmstudioStoredAuthConfig(cfg: DaisyClawConfig): DaisyClawConfig {
...
function mergeDiscoveredLmstudioAllowlistEntries(params: {
  existing?: NonNullable<NonNullable<DaisyClawConfig["agents"]>["defaults"]>["models"];
  discoveredModels: ModelDefinitionConfig[];
}) {
