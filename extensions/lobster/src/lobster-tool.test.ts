...
import type { DaisyClawPluginApi, DaisyClawPluginToolContext } from "../runtime-api.js";
import { createLobsterTool } from "./lobster-tool.js";
import { createFakeTaskFlow } from "./taskflow-test-helpers.js";

function fakeApi(overrides: Partial<DaisyClawPluginApi> = {}): DaisyClawPluginApi {
...
function fakeCtx(overrides: Partial<DaisyClawPluginToolContext> = {}): DaisyClawPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", () => {
    const api = fakeApi();
    const factoryTool = (ctx: DaisyClawPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
