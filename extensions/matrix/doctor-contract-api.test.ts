...
import type { PluginDoctorStateMigrationContext } from "daisyclaw/plugin-sdk/runtime-doctor";
...
describe("matrix doctor contract state migrations", () => {
  ...
  it("migrates legacy sync cache JSON to SQLite plugin state", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-matrix-doctor-"));
  ...
  it("migrates Matrix IndexedDB snapshot JSON to SQLite plugin state", async () => {
    ...
    const snapshot = [
      {
        name: "daisyclaw-matrix::matrix-sdk-crypto",
        version: 1,
        stores: [
          {
            name: "sessions",
