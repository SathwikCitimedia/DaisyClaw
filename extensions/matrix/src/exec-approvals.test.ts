...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-matrix-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<DaisyClawConfig["channels"]>["matrix"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<DaisyClawConfig["channels"]>["matrix"]>>,
): DaisyClawConfig {
  return {
    channels: {
      matrix: {
