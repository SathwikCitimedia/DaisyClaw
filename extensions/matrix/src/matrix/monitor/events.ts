...
function formatMatrixPostHealthySyncDecryptionHint(accountId: string): string {
  return (
    ...
    `Check 'daisyclaw matrix verify status --verbose --account ${accountId}' and 'daisyclaw matrix devices list --account ${accountId}'.`
  );
}
...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another DaisyClaw runtime is using the same account. " +
    `Check 'daisyclaw matrix verify status --verbose --account ${accountId}' and 'daisyclaw matrix devices list --account ${accountId}'.`
  );
}
