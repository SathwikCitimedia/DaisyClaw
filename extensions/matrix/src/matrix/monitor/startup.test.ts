...
function createDeps(
  ...
  return {
    ...
    summarizeMatrixDeviceHealth: vi.fn(() => ({
      ...
      staleDaisyClawDevices: [] as MatrixManagedDeviceInfo[],
      currentDaisyClawDevices: [] as MatrixManagedDeviceInfo[],
    })),
  ...
  it("reports stale devices, pending verification, and restored legacy backups", async () => {
    ...
    vi.mocked(deps.summarizeMatrixDeviceHealth).mockReturnValue({
      ...
      staleDaisyClawDevices: [
        { deviceId: "DEV123", displayName: "DaisyClaw Device", current: false },
      ],
    ...
    expect(params.logger.warn).toHaveBeenCalledWith(
      "matrix: stale DaisyClaw devices detected for @bot:example.org: DEV123. Run 'daisyclaw matrix devices prune-stale --account ops' to keep encrypted-room trust healthy.",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
      "matrix: device not verified — run 'daisyclaw matrix verify device <key>' to enable E2EE",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
