...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleDaisyClawDevices.length > 0) {
      params.logger.warn(
        `matrix: stale DaisyClaw devices detected for ${params.auth.userId}: ${deviceHealth.staleDaisyClawDevices.map((device) => device.deviceId).join(", ")}. Run 'daisyclaw matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {
