...
import { getSessionBindingService, testing } from "daisyclaw/plugin-sdk/session-binding-runtime";
...
describe("matrix thread bindings", () => {
  ...
  async function readPersistedBindings(bindingsPath: string) {
    const store = createPluginStateKeyedStoreForTests<{
      ...
      maxEntries: 10_000,
      env: { ...process.env, DAISYCLAW_STATE_DIR: path.dirname(bindingsPath) },
    });
    return {
      version: 1,
