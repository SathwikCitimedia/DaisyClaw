...
import { writeJsonFileAtomically } from "daisyclaw/plugin-sdk/json-store";
import { resolveRequiredHomeDir, resolveStateDir } from "daisyclaw/plugin-sdk/state-paths";

const MATRIX_MIGRATION_SNAPSHOT_DIRNAME = "daisyclaw-migrations";
...
export async function maybeCreateMatrixMigrationSnapshot(params: {
  ...
  outputDir?: string;
  createBackupArchive?: typeof import("daisyclaw/plugin-sdk/runtime").createBackupArchive;
  log?: { info?: (message: string) => void; warn?: (message: string) => void };
}): Promise<MatrixMigrationSnapshotResult> {
  const env = params.env ?? process.env;
  const createBackupArchive =
    params.createBackupArchive ?? (await import("daisyclaw/plugin-sdk/runtime")).createBackupArchive;
