...
import type { DmPolicy } from "daisyclaw/plugin-sdk/config-contracts";
import {
...
import { isPrivateNetworkOptInEnabled } from "daisyclaw/plugin-sdk/ssrf-policy";
import {
...
async function configureMatrixInviteAutoJoin(params: {
  ...
  await params.prompter.note(
    [
      ...
      "DaisyClaw agents will not join invited rooms or fresh DM-style invites unless you set autoJoin.",
  ...
  const deviceName =
    normalizeStringifiedOptionalString(
      await params.prompter.text({
        ...
        initialValue: existing.deviceName ?? "DaisyClaw Gateway",
      }),
    ) ?? "";
