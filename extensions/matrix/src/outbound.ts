...
import { createReplyToFanout } from "daisyclaw/plugin-sdk/channel-outbound";
import {
  renderMessagePresentationFallbackText,
  type MessagePresentation,
} from "daisyclaw/plugin-sdk/interactive-runtime";
import { resolvePayloadMediaUrls } from "daisyclaw/plugin-sdk/reply-payload";
...
const MATRIX_DAISYCLAW_PRESENTATION_KEY = "com.daisyclaw.presentation" as const;
const MATRIX_DAISYCLAW_PRESENTATION_TYPE = "message.presentation" as const;
...
function buildMatrixPresentationContent(presentation: MessagePresentation) {
  return {
    ...presentation,
    version: 1,
    type: MATRIX_DAISYCLAW_PRESENTATION_TYPE,
  };
}
...
function resolveMatrixPresentationContent(
  ...
  const presentation = toRecord(extraContent?.[MATRIX_DAISYCLAW_PRESENTATION_KEY]);
  if (
    !presentation ||
    presentation.version !== 1 ||
    presentation.type !== MATRIX_DAISYCLAW_PRESENTATION_TYPE
  ) {
    return undefined;
  }
  return presentation;
  ...
  return {
    ...
    channelData: {
      ...
      matrix: {
        ...matrixData,
        extraContent: {
          [MATRIX_DAISYCLAW_PRESENTATION_KEY]: buildMatrixPresentationContent(params.presentation),
        },
      },
    },
  };
...
function resolveMatrixExtraContent(payload: ReplyPayload): MatrixExtraContentFields | undefined {
  ...
  return presentation ? { [MATRIX_DAISYCLAW_PRESENTATION_KEY]: presentation } : undefined;
}
