...
function hasTrustedDaisyClawRootIndicator(packageRoot, packageJson) {
  ...
  const hasDaisyClawBin =
    (typeof packageJson?.bin === "string" &&
      normalizeLowercaseStringOrEmpty(packageJson.bin).includes("daisyclaw")) ||
    (typeof packageJson?.bin === "object" &&
      packageJson.bin !== null &&
      typeof packageJson.bin.daisyclaw === "string");
  const hasDaisyClawEntrypoint = fs.existsSync(path.join(packageRoot, "daisyclaw.mjs"));
  return hasCliEntryExport || hasDaisyClawBin || hasDaisyClawEntrypoint;
}

function findDaisyClawPackageRoot(startDir) {
  let cursor = path.resolve(startDir);
  for (let i = 0; i < 12; i += 1) {
    const pkg = readPackageJson(cursor);
    if (pkg?.name === "daisyclaw" && hasTrustedDaisyClawRootIndicator(cursor, pkg)) {
...
function resolveBundledPluginRuntimeModulePath(moduleUrl, params) {
  ...
  const location = findDaisyClawPackageRoot(moduleDir);
  if (location) {
