...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__daisyclaw_channel_id";
...
type InteractionCallbackConfig = Pick<DaisyClawConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "daisyclaw-mattermost-interactions").update(botToken).digest("hex");
}
