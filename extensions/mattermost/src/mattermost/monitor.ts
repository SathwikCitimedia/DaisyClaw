...
import { isLoopbackHost } from "daisyclaw/plugin-sdk/gateway-runtime";
...
export async function monitorMattermostProvider(opts: MonitorMattermostOpts = {}): Promise<void> {
  ...
  const cfg = (opts.config ?? core.config.current()) as DaisyClawConfig;
      ...
      authorizeButtonClick: async ({ payload, post }) => {
        ...
        return {
          ...
          response: {
            ...
            ephemeral_text: `DaisyClaw ignored this action for ${decision.roomLabel}.`,
          },
        };
      },
