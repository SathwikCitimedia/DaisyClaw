...
import type { MemoryEmbeddingProbeResult } from "daisyclaw/plugin-sdk/memory-core-host-engine-storage";
import {
  resolveMemoryDreamingConfig,
  resolveMemoryRemDreamingConfig,
} from "daisyclaw/plugin-sdk/memory-core-host-status";
import { buildAgentSessionKey } from "daisyclaw/plugin-sdk/routing";
...
function formatMemoryIndexIdentityWarning(
  ...
  return {
    reason,
    fix: `Run: daisyclaw memory status --index --agent ${agentId}`,
  };
}
...
function resolveMemoryPluginConfig(cfg: DaisyClawConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredDaisyClawTmpDir(), "daisyclaw-rem-harness-"),
  );
...
function formatDreamingSummary(cfg: DaisyClawConfig): string {
...
function resolveAgent(cfg: DaisyClawConfig, agent?: string) {
...
function resolveAgentIds(cfg: DaisyClawConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: daisyclaw memory status --fix --agent ${agentId}`)}`);
      }
    }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredDaisyClawTmpDir(), "daisyclaw-rem-backfill-"),
      );
      try {
