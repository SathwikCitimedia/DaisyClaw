...
import {
  ...
  formatHelpExamples,
  theme,
} from "daisyclaw/plugin-sdk/memory-core-host-runtime-cli";
import {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["daisyclaw memory status", "Show index and provider status."],
          [
            "daisyclaw memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["daisyclaw memory status --deep", "Probe embedding provider readiness."],
          ["daisyclaw memory index --force", "Force a full reindex."],
          ['daisyclaw memory search "meeting notes"', "Quick search using positional query."],
          [
            'daisyclaw memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `daisyclaw memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
          ...
          [
            "daisyclaw memory rem-backfill --path ./memory",
          ...
          ["daisyclaw memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.daisyclaw.ai/cli/memory")}\n`,
    );
