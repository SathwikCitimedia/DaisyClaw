...
function resolveStoredDreaming(config: DaisyClawConfig): Record<string, unknown> {
  const entry = asRecord(config.plugins?.entries?.["memory-core"]);
  const pluginConfig = asRecord(entry?.config);
  return asRecord(pluginConfig?.dreaming) ?? {};
}

function createHarness(initialConfig: DaisyClawConfig = {}) {
  const registered: { command?: DaisyClawPluginCommandDefinition } = {};
  let runtimeConfig: DaisyClawConfig = initialConfig;

  const runtime = {
    config: {
      current: vi.fn(() => runtimeConfig),
      loadConfig: vi.fn(() => runtimeConfig),
      mutateConfigFile: vi.fn(async ({ mutate }: { mutate: (draft: DaisyClawConfig) => void }) => {
      ...
      writeConfigFile: vi.fn(async (nextConfig: DaisyClawConfig) => {
        runtimeConfig = nextConfig;
      }),
    },
  ...
  const api = {
    runtime,
    registerCommand: vi.fn((definition: DaisyClawPluginCommandDefinition) => {
