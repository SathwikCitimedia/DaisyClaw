...
import { resolveMemoryDreamingConfig } from "daisyclaw/plugin-sdk/memory-core-host-status";
import type { DaisyClawPluginApi, PluginCommandContext } from "daisyclaw/plugin-sdk/plugin-entry";
import { normalizeLowercaseStringOrEmpty } from "daisyclaw/plugin-sdk/string-coerce-runtime";
import { asRecord } from "./dreaming-shared.js";
import { resolveShortTermPromotionDreamingConfig } from "./dreaming.js";

function resolveMemoryCorePluginConfig(cfg: DaisyClawConfig): Record<string, unknown> {
  const entry = asRecord(cfg.plugins?.entries?.["memory-core"]);
  return asRecord(entry?.config) ?? {};
}

function updateDreamingEnabledInConfig(cfg: DaisyClawConfig, enabled: boolean): DaisyClawConfig {
...
function formatStatus(cfg: DaisyClawConfig): string {
...
export async function handleDreamingCommand(api: DaisyClawPluginApi, ctx: PluginCommandContext) {
...
export function registerDreamingCommand(api: DaisyClawPluginApi): void {
  api.registerCommand({
    name: "dreaming",
