...
} from "daisyclaw/plugin-sdk/error-runtime";
import { resolveGlobalMap } from "daisyclaw/plugin-sdk/global-singleton";
import * as memoryCoreHostRuntimeCoreModule from "daisyclaw/plugin-sdk/memory-core-host-runtime-core";
import * as runtimeConfigSnapshotModule from "daisyclaw/plugin-sdk/runtime-config-snapshot";
import * as sessionStoreRuntimeModule from "daisyclaw/plugin-sdk/session-store-runtime";
...
const DREAMS_FILE_LOCKS_KEY = Symbol.for("daisyclaw.memoryCore.dreamingNarrative.fileLocks");
const NARRATIVE_SESSION_LOCKS_KEY = Symbol.for(
  "daisyclaw.memoryCore.dreamingNarrative.sessionLocks",
);
...
describe("backfill diary entries", () => {
  ...
  it("builds a marked backfill diary entry", () => {
    ...
    expect(entry).toContain("daisyclaw:dreaming:backfill-entry");
    expect(entry).toContain("What Happened");
  });

  it("writes and replaces backfill diary entries", async () => {
    const workspaceDir = await createTempWorkspace("daisyclaw-dreaming-backfill-");
    ...
    expect(content.match(/daisyclaw:dreaming:backfill-entry/g)?.length).toBe(1);
  });
...
describe("appendNarrativeEntry", () => {
  it("creates DREAMS.md with diary header on fresh workspace", async () => {
    const workspaceDir = await createTempWorkspace("daisyclaw-dreaming-narrative-");
    ...
    expect(content).toContain("<!-- daisyclaw:dreaming:diary:start -->");
    expect(content).toContain("<!-- daisyclaw:dreaming:diary:end -->");
  });
  ...
  it("appends a second entry within the diary markers", async () => {
    ...
    const start = content.indexOf("<!-- daisyclaw:dreaming:diary:start -->");
    const end = content.indexOf("<!-- daisyclaw:dreaming:diary:end -->");
  ...
  it("prepends diary before existing managed blocks", async () => {
    ...
    await fs.writeFile(
      dreamsPath,
      "## Light Sleep\n<!-- daisyclaw:dreaming:light:start -->\n- Candidate: test\n<!-- daisyclaw:dreaming:light:end -->\n",
      "utf-8",
    );
    await appendNarrativeEntry({
      workspaceDir,
  ...
  it("dedupes only exact diary duplicates while keeping distinct timestamps", async () => {
    const workspaceDir = await createTempWorkspace("daisyclaw-dreaming-dedupe-");
    const dreamsPath = path.join(workspaceDir, "DREAMS.md");
    await fs.writeFile(
      dreamsPath,
      [
        "# Dream Diary",
        "",
        "<!-- daisyclaw:dreaming:diary:start -->",
        "---",
        "",
...
describe("generateAndAppendDreamNarrative", () => {
  ...
  it("scrubs stale dreaming entries and orphan transcripts after cleanup", async () => {
    const workspaceDir = await createTempWorkspace("daisyclaw-dreaming-narrative-");
    const stateDir = await createTempWorkspace("daisyclaw-dreaming-state-");
  ...
  it("isolates narrative sessions across workspaces even at the same timestamp", async () => {
    const firstWorkspaceDir = await createTempWorkspace("daisyclaw-dreaming-narrative-");
    const secondWorkspaceDir = await createTempWorkspace("daisyclaw-dreaming-narrative-");
...
describe("runDetachedDreamNarrative", () => {
  ...
  it("caps the number of in-flight detached narratives at 3", async () => {
    ...
    const workspaceDirs = await Promise.all(
      Array.from({ length: 5 }, () => createTempWorkspace("daisyclaw-dreaming-detach-")),
    );
    const logger = createMockLogger();
  ...
  it("serializes detached narratives that reuse a workspace and phase session", async () => {
    ...
    const workspaceDir = await createTempWorkspace("daisyclaw-dreaming-detach-");
    const logger = createMockLogger();
