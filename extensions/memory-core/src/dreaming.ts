...
import { peekSystemEventEntries } from "daisyclaw/plugin-sdk/system-event-runtime";
...
export async function runShortTermDreamingPromotionIfTriggered(params: {
  ...
  config: ShortTermPromotionDreamingConfig;
  logger: Logger;
  subagent?: DaisyClawPluginApi["runtime"]["subagent"];
...
export function registerShortTermPromotionDreaming(api: DaisyClawPluginApi): void {
  ...
  const resolveCurrentConfig = (): DaisyClawConfig =>
    (api.runtime.config?.current?.() ?? api.config) as DaisyClawConfig;
