...
} from "daisyclaw/plugin-sdk/embedding-providers";
import {
...
function createMissingLlamaCppProviderError(): Error {
  return new Error(
    [
      ...
      "Install it with: daisyclaw plugins install @daisyclaw/llama-cpp-provider",
      "Then restart DaisyClaw and retry: daisyclaw memory status --deep",
    ].join("\n"),
  );
}
