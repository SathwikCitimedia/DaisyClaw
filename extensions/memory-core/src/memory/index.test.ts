...
describe("memory index", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-mem-fixtures-"));
  ...
  beforeEach(async () => {
    ...
    vi.stubEnv("DAISYCLAW_TEST_MEMORY_UNSAFE_REINDEX", "1");
    clearRegistry();
    registerBuiltInMemoryEmbeddingProviders({ registerMemoryEmbeddingProvider: registerAdapter });
    embedBatchCalls = 0;
  ...
  async function getFtsSessionManager(params: {
    ...
    vi.stubEnv("DAISYCLAW_STATE_DIR", path.join(workspaceDir, params.stateDirName));
    const cfg = createCfg({
  ...
  it("rebuilds missing metadata with existing chunks on gateway sync", async () => {
    ...
    try {
      ...
      vi.stubEnv("DAISYCLAW_TEST_MEMORY_UNSAFE_REINDEX", "0");
  ...
  it("clears dirty after sessions-only identity reindex", async () => {
    try {
      vi.stubEnv("DAISYCLAW_STATE_DIR", path.join(workspaceDir, ".state-sessions-only-reindex"));
      const sessionsDir = resolveSessionTranscriptsDirForAgent("main");
      await fs.mkdir(sessionsDir, { recursive: true });
      await fs.writeFile(
  ...
  it("marks sessions-only indexes dirty when metadata is missing but chunks exist", async () => {
    try {
      vi.stubEnv("DAISYCLAW_STATE_DIR", path.join(workspaceDir, ".state-sessions-missing-meta"));
      const sessionsDir = resolveSessionTranscriptsDirForAgent("main");
      await fs.mkdir(sessionsDir, { recursive: true });
      await fs.writeFile(
  ...
  it("keeps provider cutover vector search paused during targeted session sync", async () => {
    try {
      vi.stubEnv("DAISYCLAW_STATE_DIR", path.join(workspaceDir, ".state-targeted-cutover"));
      const sessionsDir = resolveSessionTranscriptsDirForAgent("main");
      await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it("preserves memory dirty events raised during session identity reindex", async () => {
    try {
      vi.stubEnv("DAISYCLAW_STATE_DIR", path.join(workspaceDir, ".state-dirty-during-session"));
      const sessionsDir = resolveSessionTranscriptsDirForAgent("main");
      await fs.mkdir(sessionsDir, { recursive: true });
      await fs.writeFile(
