...
import { classifyMemoryMultimodalPath } from "daisyclaw/plugin-sdk/memory-core-host-engine-embeddings";
import {
...
import {
  ...
  sessionPathForFile,
} from "daisyclaw/plugin-sdk/memory-core-host-engine-qmd";
import {
  buildFileEntry,
...
const TEST_MEMORY_WATCH_FACTORY_KEY = Symbol.for("daisyclaw.test.memoryWatchFactory");
const TEST_MEMORY_NATIVE_WATCH_FACTORY_KEY = Symbol.for("daisyclaw.test.memoryNativeWatchFactory");
...
export abstract class MemoryManagerSyncOps {
  protected abstract readonly cfg: DaisyClawConfig;
  ...
  private warnIfMemoryWatchPressure(count: number, unit: MemoryWatchPressureUnit): void {
    warnIfMemoryWatchPressureHigh(
      ...
      count,
      unit,
      "Large memory folders or extraPaths can make DaisyClaw run out of file watchers or open files.",
    ...
    try {
      if (needsFullReindex) {
        if (
          ...
          process.env.DAISYCLAW_TEST_MEMORY_UNSAFE_REINDEX === "1"
        ) {
