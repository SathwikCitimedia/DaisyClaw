...
import type {
  MemorySearchConfig,
  DaisyClawConfig,
} from "daisyclaw/plugin-sdk/memory-core-host-engine-foundation";
...
const {
  ...
  const chokidarKey = Symbol.for("daisyclaw.test.memoryWatchFactory");
  const nativeKey = Symbol.for("daisyclaw.test.memoryNativeWatchFactory");
...
const CHOKIDAR_FACTORY_KEY = Symbol.for("daisyclaw.test.memoryWatchFactory");
const NATIVE_FACTORY_KEY = Symbol.for("daisyclaw.test.memoryNativeWatchFactory");

vi.mock("daisyclaw/plugin-sdk/memory-core-host-engine-foundation", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("daisyclaw/plugin-sdk/memory-core-host-engine-foundation")>();
  return {
    ...actual,
...
} from "daisyclaw/plugin-sdk/memory-core-host-engine-embeddings";
import {
...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): DaisyClawConfig {
    const defaults: NonNullable<NonNullable<DaisyClawConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: DaisyClawConfig) {
  ...
  it("routes Linux directories through directory-only native watchers", async () => {
    ...
    // internal/fs/recursive_watch. DaisyClaw watches directories only so
