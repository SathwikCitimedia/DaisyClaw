...
import { resolveMemoryBackendConfig } from "daisyclaw/plugin-sdk/memory-core-host-engine-storage";
...
describe("QmdMemoryManager slugified path resolution", () => {
  ...
  async function createManager(params?: { cfg?: DaisyClawConfig }) {
  ...
  beforeEach(async () => {
    ...
    tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-qmd-slugified-"));
