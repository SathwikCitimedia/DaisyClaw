...
import { withMockedWindowsPlatform } from "daisyclaw/plugin-sdk/test-node-mocks";
...
const MEMORY_EMBEDDING_PROVIDERS_KEY = Symbol.for("daisyclaw.memoryEmbeddingProviders");
const MCPORTER_STATE_KEY = Symbol.for("daisyclaw.mcporterState");
const QMD_EMBED_QUEUE_KEY = Symbol.for("daisyclaw.qmdEmbedQueueTail");
...
vi.mock("daisyclaw/plugin-sdk/memory-core-host-engine-foundation", async () => {
  const actual = await vi.importActual<
    typeof import("daisyclaw/plugin-sdk/memory-core-host-engine-foundation")
  >("daisyclaw/plugin-sdk/memory-core-host-engine-foundation");
  return {
    ...actual,
...
vi.mock("daisyclaw/plugin-sdk/file-lock", async () => {
  const actual = await vi.importActual<typeof import("daisyclaw/plugin-sdk/file-lock")>(
    "daisyclaw/plugin-sdk/file-lock",
  );
  return {
    ...actual,
...
import type { DaisyClawConfig } from "daisyclaw/plugin-sdk/memory-core-host-engine-foundation";
import {
  requireNodeSqlite,
  resolveMemoryBackendConfig,
} from "daisyclaw/plugin-sdk/memory-core-host-engine-storage";
...
describe("QmdMemoryManager", () => {
  ...
  it("rebinds a stale in-container collection root to the host workspace (sandbox-mode transition)", async () => {
    ...
    // /home/node/.daisyclaw/... stored that in-container path as the collection root. Resolved
    ...
    const containerRoot = "/home/node/.daisyclaw/teams/x/workspace";
  ...
  it("parseShownCollection extracts path and pattern from qmd collection show output", async () => {
    ...
    const sampleOutput = [
      ...
      "  Path:     /home/node/.daisyclaw/teams/example-team/workspace-example/memory",
      "  Pattern:  **/*.md",
      "  Include:  yes (default)",
    ].join("\n");

    const result = parser(sampleOutput);
    expect(result.path).toBe("/home/node/.daisyclaw/teams/example-team/workspace-example/memory");
