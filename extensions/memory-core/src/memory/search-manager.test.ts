...
import type { checkQmdBinaryAvailability as checkQmdBinaryAvailabilityFn } from "daisyclaw/plugin-sdk/memory-core-host-engine-qmd";
...
vi.mock("daisyclaw/plugin-sdk/memory-core-host-engine-qmd", () => ({
...
function createBuiltinCfg(agentId: string): DaisyClawConfig {
  return {
    agents: {
      defaults: {
        workspace: "/tmp/workspace",
...
async function expectPendingQmdReplacement(params: {
  agentId: string;
  firstCfg: DaisyClawConfig;
  secondCfg: DaisyClawConfig;
...
describe("getMemorySearchManager caching", () => {
  it("repairs an invalid shared singleton cache shape before using qmd cache maps", async () => {
    await closeAllMemorySearchManagers();
    vi.resetModules();
    const cacheKey = Symbol.for("daisyclaw.memorySearchManagerCache");
  ...
  it("creates a missing agent workspace before probing qmd availability", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-qmd-workspace-"));
