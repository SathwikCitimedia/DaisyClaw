...
import { appendMemoryHostEvent } from "daisyclaw/plugin-sdk/memory-host-events";
import {
...
const PROMOTION_MARKER_PREFIX = "daisyclaw-memory-promotion:";
...
function isContaminatedDreamingSnippet(raw: string): boolean {
  ...
  if (
    /<!--\s*daisyclaw-memory-promotion:/i.test(snippet) ||
...
const DREAMING_FENCE_START_RE = /<!--\s*daisyclaw:dreaming:[a-z][a-z0-9-]*:start\s*-->/i;
const DREAMING_FENCE_END_RE = /<!--\s*daisyclaw:dreaming:[a-z][a-z0-9-]*:end\s*-->/i;
...
async function rehydratePromotionCandidate(
  ...
  for (const sourcePath of sourcePaths) {
    ...
    // content. If rehydration lands inside an daisyclaw:dreaming fence (for example
...
function extractPromotionMarkers(memoryText: string): Set<string> {
  ...
  const matches = memoryText.matchAll(/<!--\s*daisyclaw-memory-promotion:([^\n]*?)\s*-->/gi);
