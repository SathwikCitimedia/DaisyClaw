/**
 * DaisyClaw Memory (LanceDB) Plugin
 *
...
import type { AgentToolResult } from "daisyclaw/plugin-sdk/agent-core";
import {
  optionalFiniteNumberSchema,
  optionalPositiveIntegerSchema,
} from "daisyclaw/plugin-sdk/channel-actions";
import { BUNDLED_CHAT_CHANNEL_ENVELOPE_PREFIXES } from "daisyclaw/plugin-sdk/chat-channel-ids";
import type { DaisyClawConfig } from "daisyclaw/plugin-sdk/config-contracts";
import type { MemoryEmbeddingProvider } from "daisyclaw/plugin-sdk/memory-core-host-engine-embeddings";
import {
  parseStrictPositiveInteger,
  resolveTimerTimeoutMs,
} from "daisyclaw/plugin-sdk/number-runtime";
import { readFiniteNumberParam, readPositiveIntegerParam } from "daisyclaw/plugin-sdk/param-readers";
import { resolveLivePluginConfigObject } from "daisyclaw/plugin-sdk/plugin-config-runtime";
import { ensureGlobalUndiciEnvProxyDispatcher } from "daisyclaw/plugin-sdk/runtime-env";
import {
...
import { definePluginEntry, type DaisyClawPluginApi } from "./api.js";
import {
...
let memoryEmbeddingProviderModulePromise:
  | Promise<typeof import("daisyclaw/plugin-sdk/memory-core-host-engine-embeddings")>
  | undefined;
function loadMemoryEmbeddingProviderModule(): Promise<
  typeof import("daisyclaw/plugin-sdk/memory-core-host-engine-embeddings")
> {
  memoryEmbeddingProviderModulePromise ??=
    import("daisyclaw/plugin-sdk/memory-core-host-engine-embeddings");
  return memoryEmbeddingProviderModulePromise;
}

let memoryHostCoreModulePromise:
  | Promise<typeof import("daisyclaw/plugin-sdk/memory-host-core")>
  | undefined;
function loadMemoryHostCoreModule(): Promise<
  typeof import("daisyclaw/plugin-sdk/memory-host-core")
> {
  memoryHostCoreModulePromise ??= import("daisyclaw/plugin-sdk/memory-host-core");
...
class ProviderAdapterEmbeddings implements Embeddings {
  ...
  constructor(
    private api: DaisyClawPluginApi,
  ...
  private async createProvider(): Promise<MemoryEmbeddingProvider> {
    const cfg = (this.api.runtime.config?.current?.() ?? this.api.config) as DaisyClawConfig;
...
function createEmbeddings(api: DaisyClawPluginApi, cfg: MemoryConfig): Embeddings {
...
/**
 * Matches JSON blobs that look like DaisyClaw transport envelope metadata.
...
/**
 * Returns true if `text` looks like it contains DaisyClaw-injected envelope or
...
/**
 * Strips DaisyClaw-injected envelope metadata from a user message so that only
...
export default definePluginEntry({
  ...
  register(api: DaisyClawPluginApi) {
