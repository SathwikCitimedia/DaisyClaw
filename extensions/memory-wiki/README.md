# @daisyclaw/memory-wiki

Persistent wiki compiler and Obsidian-friendly knowledge vault for **DaisyClaw**.
...
Key beliefs can live in structured `claims` frontmatter with per-claim evidence, confidence, and status. Compile also emits machine-readable digests under `.daisyclaw-wiki/cache/` so agent/runtime consumers do not have to scrape markdown pages.
...
- The compiled agent digests in `.daisyclaw-wiki/cache/agent-digest.json` and `.daisyclaw-wiki/cache/claims.jsonl` are the stable machine-facing view of the wiki.
