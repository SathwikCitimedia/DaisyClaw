...
import { runTasksWithConcurrency } from "daisyclaw/plugin-sdk/concurrency-runtime";
import { retryTransientMemoryRead } from "daisyclaw/plugin-sdk/memory-core-host-engine-storage";
import {
  replaceManagedMarkdownBlock,
  withTrailingNewline,
} from "daisyclaw/plugin-sdk/memory-host-markdown";
import { root as fsRoot } from "daisyclaw/plugin-sdk/security-runtime";
import {
...
const AGENT_DIGEST_PATH = ".daisyclaw-wiki/cache/agent-digest.json";
const CLAIMS_DIGEST_PATH = ".daisyclaw-wiki/cache/claims.jsonl";
...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- daisyclaw:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- daisyclaw:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
...
export async function compileMemoryWikiVault(
  ...
  if (
    await writeManagedMarkdownFile({
      ...
      title: "Wiki Index",
      startMarker: "<!-- daisyclaw:wiki:index:start -->",
      endMarker: "<!-- daisyclaw:wiki:index:end -->",
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        ...
        relativePath,
        title: group.heading,
        startMarker: `<!-- daisyclaw:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- daisyclaw:wiki:${group.dir}:index:end -->`,
