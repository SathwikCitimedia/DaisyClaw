...
vi.mock("daisyclaw/plugin-sdk/memory-host-search", () => ({
  getActiveMemorySearchManager: getActiveMemorySearchManagerMock,
}));

vi.mock("daisyclaw/plugin-sdk/memory-host-core", () => ({
  resolveDefaultAgentId: resolveDefaultAgentIdMock,
  resolveSessionAgentId: resolveSessionAgentIdMock,
}));

vi.mock("daisyclaw/plugin-sdk/session-transcript-hit", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("daisyclaw/plugin-sdk/session-transcript-hit")>();
  return {
    ...actual,
...
function createAppConfig(): DaisyClawConfig {
  return {
    agents: {
...
function createSessionVisibilityAppConfig(): DaisyClawConfig {
  return {
    agents: {
  ...
  loadCombinedSessionStoreForGatewayMock.mockReturnValue({
    ...
    store: {
      "agent:main:child-session": {
        ...
        updatedAt: 1,
        sessionFile: "/tmp/daisyclaw/child-session.jsonl",
      },
      "agent:main:sibling-session": {
        sessionId: "sibling-session",
        updatedAt: 2,
        sessionFile: "/tmp/daisyclaw/sibling-session.jsonl",
      },
    },
  });
}
    ...
    await fs.writeFile(
      ...
      renderWikiMarkdown({
        ...
        body: [
          ...
          "",
          "## Related",
          "<!-- daisyclaw:wiki:related:start -->",
          "### Related Pages",
          "- [Needle Person](entities/needle-person.md)",
          "<!-- daisyclaw:wiki:related:end -->",
          "",
        ].join("\n"),
      }),
      "utf8",
    ...
    await fs.writeFile(
      ...
      renderWikiMarkdown({
        ...
        body: [
          ...
          "",
          "## Agent Card",
          "- Maintainer lane: CEO; Microsoft-facing DaisyClaw maintainer",
          "",
          "## AI Notes",
    ...
    loadCombinedSessionStoreForGatewayMock.mockReturnValue({
      ...
      store: {
        "agent:main:abc-uuid": {
          sessionId: "abc-uuid",
          updatedAt: 1,
          sessionFile: "/tmp/daisyclaw/abc-uuid.jsonl",
        },
      },
    });
    const manager = createMemoryManager({
    ...
    loadCombinedSessionStoreForGatewayMock.mockReturnValue({
      ...
      store: {
        "agent:secondary:visible-session": {
          ...
          updatedAt: 1,
          sessionFile: "/tmp/daisyclaw/visible-session.jsonl",
        },
      },
    });
    const manager = createMemoryManager({
    ...
    loadCombinedSessionStoreForGatewayMock.mockReturnValue({
      ...
      store: {
        "agent:secondary:main": {
          sessionId: "main",
          updatedAt: 1,
          sessionFile: "/tmp/daisyclaw/main.jsonl",
        },
      },
    });
    const manager = createMemoryManager({
