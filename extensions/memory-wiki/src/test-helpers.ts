...
import type { PluginStateEntry } from "daisyclaw/plugin-sdk/plugin-state-runtime";
...
export function createMemoryWikiTestHarness() {
  ...
  async function createTempDir(prefix: string): Promise<string> {
    const tempDir = await fs.mkdtemp(path.join(resolvePreferredDaisyClawTmpDir(), prefix));
