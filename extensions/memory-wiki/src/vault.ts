...
import { FsSafeError, pathExists, root as fsRoot } from "daisyclaw/plugin-sdk/security-runtime";
...
export const WIKI_VAULT_DIRECTORIES = [
  ...
  "_views",
  ".daisyclaw-wiki",
  ".daisyclaw-wiki/locks",
  ".daisyclaw-wiki/cache",
] as const;
...
- Use \`.daisyclaw-wiki/cache/agent-digest.json\` and \`claims.jsonl\` for machine reads; markdown pages are the human view.
`);
}
...
This vault is maintained by the DaisyClaw memory-wiki plugin.
...
- \`.daisyclaw-wiki/cache/agent-digest.json\` is the agent-facing compiled digest.

## Notes
<!-- daisyclaw:human:start -->
<!-- daisyclaw:human:end -->
`);
}
...
export async function initializeMemoryWikiVault(
  ...
  await writeFileIfMissing(rootDir, ".daisyclaw-wiki/log.jsonl", "", createdFiles);
