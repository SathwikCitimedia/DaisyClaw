  ...
  if (await hasLegacyHermesAuthJson(params.source.authPath)) {
    items.push(
      createMigrationManualItem({
        ...
        message:
          "Hermes auth.json contains legacy OAuth credentials. DaisyClaw no longer imports those into live auth during Hermes migration.",
        recommendation:
          "Run daisyclaw models auth login --provider openai after migration, or run daisyclaw doctor --fix for existing DaisyClaw legacy auth state.",
      }),
    );
  }
