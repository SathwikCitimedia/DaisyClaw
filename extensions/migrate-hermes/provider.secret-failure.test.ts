...
import { resolveAuthStorePathForDisplay } from "daisyclaw/plugin-sdk/agent-runtime";
...
vi.mock("daisyclaw/plugin-sdk/provider-auth", async (importOriginal) => ({
  ...(await importOriginal<typeof import("daisyclaw/plugin-sdk/provider-auth")>()),
...
async function makeTempRoot() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-hermes-secret-failure-"));
  tempRoots.add(root);
  return root;
}
