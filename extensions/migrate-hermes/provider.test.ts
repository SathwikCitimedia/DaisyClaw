...
import { createCapturedPluginRegistration } from "daisyclaw/plugin-sdk/plugin-test-runtime";
...
describe("Hermes migration provider", () => {
  ...
  it("resolves tilde source paths against the OS home when DAISYCLAW_HOME is set", () => {
    const previous = process.env.DAISYCLAW_HOME;
    process.env.DAISYCLAW_HOME = path.join(path.sep, "tmp", "daisyclaw-home");
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.DAISYCLAW_HOME = previous;
      }
    }
  });
