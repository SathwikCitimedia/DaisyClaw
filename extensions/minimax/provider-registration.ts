...
import { buildOauthProviderAuthResult } from "daisyclaw/plugin-sdk/provider-auth";
import { createProviderApiKeyAuthMethod } from "daisyclaw/plugin-sdk/provider-auth-api-key";
...
import { MINIMAX_FAST_MODE_STREAM_HOOKS } from "daisyclaw/plugin-sdk/provider-stream-family";
import { fetchMinimaxUsage } from "daisyclaw/plugin-sdk/provider-usage";
...
function getProviderBaseUrl(cfg: DaisyClawConfig, providerId: string): string | undefined {
  return normalizeOptionalString(cfg.models?.providers?.[providerId]?.baseUrl);
}

function resolveMinimaxUsageBaseUrl(cfg: DaisyClawConfig): string | undefined {
...
export function registerMinimaxProviders(api: DaisyClawPluginApi) {
