...
function resolveKimiBaseUrl(kimi?: KimiConfig, daisyClawConfig?: DaisyClawConfig): string {
  ...
  const moonshotBaseUrl = daisyClawConfig?.models?.providers?.moonshot?.baseUrl;
...
export async function executeKimiWebSearchProviderTool(
  ctx: { config?: DaisyClawConfig; searchConfig?: SearchConfigRecord },
  ...
  if (!result.grounded) {
    return {
      ...
      provider: "kimi",
      model,
      docs: "https://docs.daisyclaw.ai/tools/kimi-search",
