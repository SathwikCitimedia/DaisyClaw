...
describe("kimi web search provider", () => {
  ...
  it("accepts final responses backed by Kimi web search tool replay", async () => {
    const toolArguments = JSON.stringify({
      query: "DaisyClaw GitHub repository",
      ...
      .mockResolvedValueOnce(
        jsonResponse({
          choices: [
            {
              finish_reason: "stop",
              message: { content: "DaisyClaw is available on GitHub." },
            },
          ],
        }),
      );
    ...
    await withEnvAsync({ KIMI_API_KEY: "kimi-test-key" }, async () => {
      ...
      expect(result.provider).toBe("kimi");
      expectStringFieldContains(result, "content", "DaisyClaw is available on GitHub.");
  ...
  it("accepts final responses with search result citations", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      jsonResponse({
        search_results: [{ title: "DaisyClaw", url: "https://github.com/daisyclaw/daisyclaw" }],
        choices: [
          {
            finish_reason: "stop",
            message: { content: "DaisyClaw is on GitHub." },
          },
        ],
      }),
    );
    ...
    await withEnvAsync({ KIMI_API_KEY: "kimi-test-key" }, async () => {
      ...
      expect(result.provider).toBe("kimi");
      expectStringFieldContains(result, "content", "DaisyClaw is on GitHub.");
      expect(result.citations).toEqual(["https://github.com/daisyclaw/daisyclaw"]);
      expect(result).not.toHaveProperty("error");
    });
  });
