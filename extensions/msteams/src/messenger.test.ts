...
import { SILENT_REPLY_TOKEN } from "daisyclaw/plugin-sdk/reply-chunking";
...
describe("msteams messenger", () => {
  ...
  describe("sendMSTeamsMessages", () => {
    ...
    it("preserves parsed mentions when appending OneDrive fallback file links", async () => {
      const tmpDir = await mkdtemp(path.join(resolvePreferredDaisyClawTmpDir(), "msteams-mention-"));
    ...
    it("retries full activity preparation when media upload fails transiently", async () => {
      const tmpDir = await mkdtemp(path.join(resolvePreferredDaisyClawTmpDir(), "msteams-retry-"));
