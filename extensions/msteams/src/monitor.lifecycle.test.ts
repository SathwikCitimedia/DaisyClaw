...
function createConfig(port: number): DaisyClawConfig {
  return {
    channels: {
      msteams: {
        enabled: true,
...
function updateMSTeamsConfig(
  cfg: DaisyClawConfig,
  patch: NonNullable<NonNullable<DaisyClawConfig["channels"]>["msteams"]>,
): void {
...
function requireRegisteredMSTeamsConfig(): DaisyClawConfig {
  ...
  it("gates poll card votes before recording them", async () => {
    ...
    const response = await cardActionHandler({
      activity: {
        ...
        value: { action: { data: { daisyclawPollId: "poll-1", choices: "0" } } },
      },
    });
