...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["daisyclawPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["daisyclaw", "pollId"]) ??
    readNestedString(value, ["daisyclaw", "poll", "id"]) ??
    readNestedString(value, ["data", "daisyclawPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "daisyclaw", "pollId"]) ??
    // Action.Execute (Universal Action Model) payload shape: value.action.data
    readNestedString(value, ["action", "data", "daisyclawPollId"]) ??
    ...
    actions: [
      {
        type: "Action.Execute",
        title: "Vote",
        verb: "daisyclaw.poll.vote",
        data: {
          daisyclawPollId: pollId,
          pollId,
        },
      },
    ],
