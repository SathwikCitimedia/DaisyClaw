...
describe("createMSTeamsApp", () => {
  ...
  it("preserves both Teams SDK and DaisyClaw User-Agent fragments", async () => {
    const creds: MSTeamsCredentials = {
      type: "secret",
      appId: "test-app-id",
      appPassword: "test-secret",
    ...
    expect(headers?.["User-Agent"]).toMatch(/^teams\.ts\[apps\]\/\S+ DaisyClaw\/\S+$/);
  });
  ...
  it("passes China cloud to the SDK App without requiring a configured serviceUrl", async () => {
    ...
    // default. DaisyClaw proactive sends use stored reference serviceUrls instead.
