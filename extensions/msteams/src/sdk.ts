...
import { buildDaisyClawUserAgentFragment } from "./user-agent.js";
...
type MSTeamsAppOn = {
  ...
  // delegates back into the SDK handlers after DaisyClaw sender policy passes.
...
export async function createMSTeamsApp(
  ...
  // backend can identify DaisyClaw traffic for usage telemetry. Teams SDK
  ...
  const appOptions: Record<string, unknown> = {
    client: options?.httpClient ?? {
      headers: { "User-Agent": buildDaisyClawUserAgentFragment() },
    },
