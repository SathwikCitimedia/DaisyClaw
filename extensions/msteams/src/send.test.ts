...
// pulls from `daisyclaw/plugin-sdk/outbound-media` (post-migration). Mock the
// canonical source so the re-export carries our stub through.
vi.mock("daisyclaw/plugin-sdk/outbound-media", () => ({
  loadOutboundMediaFromUrl: mockState.loadOutboundMediaFromUrl,
}));

vi.mock("daisyclaw/plugin-sdk/markdown-table-runtime", () => ({
  resolveMarkdownTableMode: mockState.resolveMarkdownTableMode,
}));

vi.mock("daisyclaw/plugin-sdk/text-chunking", async (importOriginal) => {
  const actual = await importOriginal<typeof import("daisyclaw/plugin-sdk/text-chunking")>();
  return {
    ...actual,
