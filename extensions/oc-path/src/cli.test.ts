/**
 * Smoke tests for the `daisyclaw path` CLI handlers.
 *
...
describe("daisyclaw path CLI", () => {
  ...
  describe("set", () => {
    ...
    it("CLI-S08 sets slash-deep JSONC paths and parsed JSON values", async () => {
      const filePath = join(workspaceDir, "daisyclaw.json");
      writeFileSync(
        filePath,
      ...
      await pathSetCommand(
        "oc://daisyclaw.json/gateway/auth/token",
      ...
      await pathSetCommand(
        "oc://daisyclaw.json/agents/list/0/tools/exec/security",
        "allowlist",
        { cwd: workspaceDir, json: true },
        rt2,
      );
