 ...
 * `__DAISYCLAW_REDACTED__` literal anywhere in the output throws
...
export function emitMd(ast: MdAst, opts: EmitOptions = {}): string {
  ...
  if (mode === "roundtrip") {
    ...
    if (!acceptPreExisting && ast.raw.includes("__DAISYCLAW_REDACTED__")) {
      guardSentinel("__DAISYCLAW_REDACTED__", `${guardPath}/[raw]`);
    }
    return ast.raw;
  }
