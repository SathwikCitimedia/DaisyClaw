...
metadata: { "daisyclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == DaisyClaw `sessions_spawn`
- **File I/O** == DaisyClaw `read`/`write`
- **Remote fetch** == DaisyClaw `web_fetch` (or `exec` with curl when POST is required)
