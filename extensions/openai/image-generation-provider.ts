...
} from "daisyclaw/plugin-sdk/image-generation";
import {
  parseOpenAiCompatibleImageResponse,
  toImageDataUrl,
} from "daisyclaw/plugin-sdk/image-generation";
import { createSubsystemLogger } from "daisyclaw/plugin-sdk/logging-core";
import { resolveClosestSize } from "daisyclaw/plugin-sdk/media-generation-runtime";
import { extensionForMime } from "daisyclaw/plugin-sdk/media-mime";
import {
...
function shouldAllowPrivateImageEndpoint(req: {
  ...
  return process.env.DAISYCLAW_QA_ALLOW_LOCAL_IMAGE_PROVIDER === "1";
}
...
function hasExplicitOpenAIImageApiKeyConfig(cfg: DaisyClawConfig | undefined): boolean {
  const providerConfig = cfg?.models?.providers?.openai;
  return providerConfig?.apiKey !== undefined || providerConfig?.auth === "api-key";
}

function hasExplicitDirectOpenAIImageConfig(cfg: DaisyClawConfig | undefined): boolean {
  const providerConfig = cfg?.models?.providers?.openai;
  if (!providerConfig) {
    return false;
  }
...
function hasChatGPTImageRouteConfig(cfg: DaisyClawConfig | undefined): boolean {
  const providerConfig = cfg?.models?.providers?.openai;
  return (
...
function forceOpenAIImageApiKeyAuth(cfg: DaisyClawConfig | undefined): DaisyClawConfig | undefined {
