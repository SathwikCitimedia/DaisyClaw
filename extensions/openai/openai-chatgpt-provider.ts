...
import { CODEX_CLI_PROFILE_ID, type OAuthCredential } from "daisyclaw/plugin-sdk/provider-auth";
...
import { fetchCodexUsage } from "daisyclaw/plugin-sdk/provider-usage";
import {
 ...
 * entirely when they were written by older DaisyClaw versions. When that row wins
...
async function runOpenAICodexDeviceCode(ctx: ProviderAuthContext) {
  ...
  try {
    ...
    await ctx.prompter.note(
      "Trouble with device code login? See https://docs.daisyclaw.ai/start/faq",
      "OAuth help",
    );
    throw error;
  }
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `daisyclaw models auth login --provider openai` or `daisyclaw configure`.";
}
