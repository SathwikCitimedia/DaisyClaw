...
import type { ResolvedTtsConfig } from "daisyclaw/plugin-sdk/agent-runtime";
import { AuthStorage, ModelRegistry } from "daisyclaw/plugin-sdk/agent-sessions";
import type { DaisyClawConfig } from "daisyclaw/plugin-sdk/config-contracts";
import { encodePngRgba, fillPixel } from "daisyclaw/plugin-sdk/media-runtime";
import {
  registerProviderPlugin,
  requireRegisteredProvider,
} from "daisyclaw/plugin-sdk/plugin-test-runtime";
import { runRealtimeSttLiveTest } from "daisyclaw/plugin-sdk/provider-test-contracts";
...
const LIVE_MODEL_ID = process.env.DAISYCLAW_LIVE_OPENAI_PLUGIN_MODEL?.trim() || "gpt-5.5";
const LIVE_IMAGE_MODEL = process.env.DAISYCLAW_LIVE_OPENAI_IMAGE_MODEL?.trim() || "gpt-image-2";
const LIVE_VISION_MODEL = process.env.DAISYCLAW_LIVE_OPENAI_VISION_MODEL?.trim() || "gpt-5.4-mini";
const liveEnabled = OPENAI_API_KEY.trim().length > 0 && process.env.DAISYCLAW_LIVE_TEST === "1";
...
function createLiveConfig(): DaisyClawConfig {
...
describeLive("openai plugin live", () => {
  ...
  it("lists voices and synthesizes audio through the registered speech provider", async () => {
    ...
    const audioFile = await speechProvider.synthesize({
      text: "DaisyClaw integration test OK.",
      cfg,
  ...
  it("streams realtime STT through the registered transcription provider", async () => {
    ...
    const cfg = createLiveConfig();
    const ttsConfig = createLiveTtsConfig();
    const phrase = "Testing DaisyClaw OpenAI realtime transcription integration test OK.";
