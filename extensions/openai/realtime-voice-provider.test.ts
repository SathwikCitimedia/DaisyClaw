...
import { REALTIME_VOICE_AUDIO_FORMAT_PCM16_24KHZ } from "daisyclaw/plugin-sdk/realtime-voice";
...
describe("buildOpenAIRealtimeVoiceProvider", () => {
  ...
  it("adds DaisyClaw attribution headers to native realtime websocket requests", () => {
  ...
  it("returns browser-safe DaisyClaw attribution headers for native WebRTC offers", async () => {
    vi.stubEnv("DAISYCLAW_VERSION", "2026.3.22");
  ...
  it("resolves keychain OPENAI_API_KEY refs before creating browser sessions", async () => {
    vi.stubEnv("OPENAI_API_KEY", "keychain:daisyclaw:OPENAI_REALTIME_BROWSER_TEST");
  ...
  it("resolves and caches keychain OPENAI_API_KEY refs before creating bridges", async () => {
    vi.stubEnv("OPENAI_API_KEY", "keychain:daisyclaw:OPENAI_REALTIME_BRIDGE_TEST");
  ...
  it("does not resolve keychain refs during configured checks", () => {
    vi.stubEnv("OPENAI_API_KEY", "keychain:daisyclaw:OPENAI_REALTIME_CONFIGURED_TEST");
    const provider = buildOpenAIRealtimeVoiceProvider();
  ...
  it("fails closed when keychain refs cannot be resolved", async () => {
    vi.stubEnv("OPENAI_API_KEY", "keychain:daisyclaw:OPENAI_REALTIME_MISSING_TEST");
  ...
  it("emits tool calls from realtime conversation item done events", async () => {
    ...
    expect(onEvent).toHaveBeenCalledWith({
      direction: "server",
      type: "conversation.item.done",
      detail: "function_call name=daisyclaw_agent_consult",
    });
  });
