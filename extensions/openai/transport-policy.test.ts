...
import type { ProviderRuntimeModel } from "daisyclaw/plugin-sdk/plugin-entry";
...
describe("openai transport policy", () => {
  ...
  it("builds native turn state for direct OpenAI routes", () => {
    ...
    expect(state?.headers?.["x-daisyclaw-session-id"]).toBe("session-123");
    expect(state?.headers?.["x-daisyclaw-turn-id"]).toBe("turn-123");
    expect(state?.headers?.["x-daisyclaw-turn-attempt"]).toBe("2");
    expect(state?.metadata?.daisyclaw_session_id).toBe("session-123");
    expect(state?.metadata?.daisyclaw_turn_id).toBe("turn-123");
    expect(state?.metadata?.daisyclaw_turn_attempt).toBe("2");
    expect(state?.metadata?.daisyclaw_transport).toBe("websocket");
  });
  ...
  it("returns websocket session headers and cooldown for native routes", () => {
    ...
    expect(policy?.headers?.["x-daisyclaw-session-id"]).toBe("session-123");
