...
function resolveSessionHeaders(params: {
  ...
  return {
    ...
    "x-daisyclaw-session-id": sessionId,
  };
}
...
export function resolveOpenAITransportTurnState(
  ...
  return {
    headers: {
      ...sessionHeaders,
      "x-daisyclaw-turn-id": turnId,
      "x-daisyclaw-turn-attempt": attempt,
    },
    metadata: {
      daisyclaw_session_id: sessionHeaders["x-daisyclaw-session-id"] ?? "",
      daisyclaw_turn_id: turnId,
      daisyclaw_turn_attempt: attempt,
      daisyclaw_transport: ctx.transport,
    },
  };
}
