...
} from "daisyclaw/plugin-sdk/proxy-capture";
...
describe("openai tts", () => {
  ...
  describe("openaiTTS diagnostics", () => {
    it("adds DaisyClaw attribution headers to native OpenAI speech requests", async () => {
      ...
      expect(headers?.originator).toBe("daisyclaw");
      expect(headers?.version).toBe("2026.3.22");
      expect(headers?.["User-Agent"]).toBe("daisyclaw/2026.3.22");
    });
    ...
    it("records TTS exchanges in debug proxy capture mode", async () => {
      ...
      process.env.DAISYCLAW_DEBUG_PROXY_SESSION_ID = "tts-session";
      ...
      store.upsertSession({
        ...
        startedAt: Date.now(),
        mode: "test",
        sourceScope: "daisyclaw",
        sourceProcess: "daisyclaw",
        dbPath: process.env.DAISYCLAW_DEBUG_PROXY_DB_PATH,
        blobDir: process.env.DAISYCLAW_DEBUG_PROXY_BLOB_DIR,
      });
    ...
    it("does not double-capture TTS exchanges when the global fetch patch is installed", async () => {
      ...
      process.env.DAISYCLAW_DEBUG_PROXY_DB_PATH = path.join(tempDir, "capture.sqlite");
      process.env.DAISYCLAW_DEBUG_PROXY_BLOB_DIR = path.join(tempDir, "blobs");
      process.env.DAISYCLAW_DEBUG_PROXY_SESSION_ID = "tts-patched-session";
