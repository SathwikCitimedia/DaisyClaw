...
import { createSandboxTestContext } from "daisyclaw/plugin-sdk/test-fixtures";
import {
...
const DAISYCLAW_OPENSHELL_E2E = process.env.DAISYCLAW_E2E_OPENSHELL === "1";
const DAISYCLAW_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const DAISYCLAW_OPENSHELL_COMMAND =
  process.env.DAISYCLAW_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "daisyclaw-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && DAISYCLAW_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: DAISYCLAW_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(DAISYCLAW_OPENSHELL_COMMAND))) {
        return;
      }
      if (!(await openshellGatewayAvailable(DAISYCLAW_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-openshell-e2e-"));
      ...
      const gatewayName = `daisyclaw-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `daisyclaw-policy-allow-${scopeSuffix}`;
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: DAISYCLAW_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("daisyclaw-openshell-e2e");
