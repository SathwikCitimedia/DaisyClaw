...
const MATERIALIZED_SKILLS_REMOTE_PARTS = [".daisyclaw", "sandbox-skills"] as const;
...
class OpenShellSandboxBackendImpl {
  ...
  private async syncWorkspaceFromRemote(): Promise<void> {
    await withTempWorkspace(
      { rootDir: resolveOpenShellTmpRoot(), prefix: "daisyclaw-openshell-sync-" },
      async ({ dir: tmpDir }) => {
        const result = await runOpenShellCli({
          context: this.params.execContext,
          args: [
  ...
  private async uploadPathToRemote(localPath: string, remotePath: string): Promise<void> {
    await withTempWorkspace(
      { rootDir: resolveOpenShellTmpRoot(), prefix: "daisyclaw-openshell-upload-" },
      async ({ dir: tmpDir }) => {
...
export function buildOpenShellSandboxName(scopeKey: string): string {
  ...
  return `daisyclaw-${safe || "session"}-${hash.toString(16).slice(0, 8)}`;
}
...
async function moveMaterializedSkillsShadowAside(params: {
  ...
  const preserveRoot = await fs.mkdtemp(
    path.join(path.dirname(params.tmpDir), "daisyclaw-openshell-preserve-"),
  );
...
function resolveOpenShellTmpRoot(): string {
  return path.resolve(resolvePreferredDaisyClawTmpDir());
}
