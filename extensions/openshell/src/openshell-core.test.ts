...
import type { CreateSandboxBackendParams } from "daisyclaw/plugin-sdk/sandbox";
import {
...
describe("openshell backend manager", () => {
  ...
  it("checks runtime status with config override from DaisyClaw config", async () => {
    ...
    const result = await manager.describeRuntime({
      entry: {
        containerName: "daisyclaw-session-1234",
        backendId: "openshell",
        runtimeLabel: "daisyclaw-session-1234",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: {
        sandboxName: "daisyclaw-session-1234",
        config: expectedConfig,
      },
      args: ["sandbox", "get", "daisyclaw-session-1234"],
    });
  });
  ...
  it("removes runtimes via openshell sandbox delete", async () => {
    ...
    await manager.removeRuntime({
      entry: {
        containerName: "daisyclaw-session-5678",
        backendId: "openshell",
        runtimeLabel: "daisyclaw-session-5678",
        sessionKey: "agent:main",
        createdAtMs: 1,
        lastUsedAtMs: 1,
        image: "daisyclaw",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: {
        sandboxName: "daisyclaw-session-5678",
        config: expectedConfig,
      },
      args: ["sandbox", "delete", "daisyclaw-session-5678"],
    });
  });
  ...
  it("preserves a local sandbox skills shadow when mirror sync crosses filesystems", async () => {
    const workspaceDir = await makeTempDir("daisyclaw-openshell-workspace-");
    const shadowFile = path.join(workspaceDir, ".daisyclaw", "sandbox-skills", "user-note.txt");
    ...
    cliMocks.runOpenShellCli.mockImplementation(async ({ args }: { args: string[] }) => {
      if (args[0] === "sandbox" && args[1] === "download") {
        ...
        await fs.mkdir(path.join(tmpDir, ".daisyclaw", "sandbox-skills", "skills"), {
          recursive: true,
        });
        await fs.writeFile(
          path.join(tmpDir, ".daisyclaw", "sandbox-skills", "skills", "generated.txt"),
          "generated",
          "utf8",
        );
      }
    ...
    try {
      ...
      await expectPathMissing(
        path.join(workspaceDir, ".daisyclaw", "sandbox-skills", "skills", "generated.txt"),
      );
    } finally {
  ...
  it("drops non-directory materialized sandbox skills from mirror downloads", async () => {
    ...
    cliMocks.runOpenShellCli.mockImplementation(async ({ args }: { args: string[] }) => {
      if (args[0] === "sandbox" && args[1] === "download") {
        const tmpDir = args[4];
        await fs.writeFile(path.join(tmpDir, "from-remote.txt"), "remote", "utf8");
        await fs.mkdir(path.join(tmpDir, ".daisyclaw"), { recursive: true });
        await fs.writeFile(path.join(tmpDir, ".daisyclaw", "sandbox-skills"), "poison", "utf8");
      }
      return { code: 0, stdout: "", stderr: "" };
    });
    ...
    await expectPathMissing(path.join(workspaceDir, ".daisyclaw", "sandbox-skills"));
  });
  ...
  it("restores a local sandbox skills shadow when mirror download has a file parent", async () => {
    ...
    cliMocks.runOpenShellCli.mockImplementation(async ({ args }: { args: string[] }) => {
      if (args[0] === "sandbox" && args[1] === "download") {
        const tmpDir = args[4];
        await fs.writeFile(path.join(tmpDir, "from-remote.txt"), "remote", "utf8");
        await fs.writeFile(path.join(tmpDir, ".daisyclaw"), "poison", "utf8");
      }
      return { code: 0, stdout: "", stderr: "" };
    });
    ...
    await expect(fs.readFile(shadowFile, "utf8")).resolves.toBe("local shadow");
    expect((await fs.stat(path.join(workspaceDir, ".daisyclaw"))).isDirectory()).toBe(true);
  });
});
...
function createOpenShellBackendSandboxConfig(): CreateSandboxBackendParams["cfg"] {
  return {
    ...
    ssh: createSandboxSshConfig("/tmp/daisyclaw-sandboxes"),
...
async function makeExecutable(params: { name: string; script: string }): Promise<string> {
  const dir = await makeTempDir("daisyclaw-openshell-bin-");
...
describe("openshell fs bridges", () => {
  it("writes locally and syncs the file to the remote workspace", async () => {
    const workspaceDir = await makeTempDir("daisyclaw-openshell-fs-");
  ...
  it("rejects symlink-parent writes instead of escaping the local mount root", async () => {
    const workspaceDir = await makeTempDir("daisyclaw-openshell-fs-");
    const outsideDir = await makeTempDir("daisyclaw-openshell-outside-");
  ...
  it("reads materialized sandbox skills from the protected skills workspace", async () => {
    const workspaceDir = await makeTempDir("daisyclaw-openshell-fs-");
    const skillsWorkspaceDir = await makeTempDir("daisyclaw-openshell-skills-");
    ...
    await expect(
      bridge.readFile({
        filePath: "/sandbox/.daisyclaw/sandbox-skills/skills/demo/SKILL.md",
      }),
    ).resolves.toEqual(Buffer.from("# Demo\nmaterialized\n"));
    await expect(
      bridge.readFile({
        filePath: ".daisyclaw/sandbox-skills/skills/demo/SKILL.md",
      }),
    ).resolves.toEqual(Buffer.from("# Demo\nmaterialized\n"));
    await expect(
      bridge.writeFile({
  ...
  it("maps agent mount paths when the sandbox workspace is read-only", async () => {
    const workspaceDir = await makeTempDir("daisyclaw-openshell-fs-");
    const agentWorkspaceDir = await makeTempDir("daisyclaw-openshell-agent-");
