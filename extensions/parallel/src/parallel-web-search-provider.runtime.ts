...
import { readPluginPackageVersion } from "daisyclaw/plugin-sdk/extension-shared";
import {
...
const USER_AGENT = `daisyclaw-parallel/${PLUGIN_VERSION} (${process.platform})`;
...
export async function executeParallelWebSearchProviderTool(
  ...
  // Generic `query` arg fallback: daisyclaw's operator-facing CLI
  // (`daisyclaw capability web.search ...`) always passes the shared
  ...
  // Always pass max_results so Parallel matches the daisyclaw web_search default
