...
vi.mock("daisyclaw/plugin-sdk/provider-web-search", async (importOriginal) => {
  const actual = await importOriginal<typeof import("daisyclaw/plugin-sdk/provider-web-search")>();
...
describe("parallel web search provider", () => {
  ...
  it("resolves Parallel search base URL overrides", () => {
    ...
    expect(
      ...
      docs: "https://docs.daisyclaw.ai/tools/parallel-search",
  ...
  it("partitions Parallel cache keys by resolved endpoint", () => {
    const base = {
      objective: "Find DaisyClaw on GitHub",
      searchQueries: ["daisyclaw github"],
      count: 5,
    };
    expect(
  ...
  it("partitions Parallel cache keys by objective and by search_queries set", () => {
    ...
    expect(
      ...
      testing.buildParallelCacheKey({
        ...base,
        objective: "Find the DaisyClaw release notes",
        searchQueries: ["daisyclaw github"],
      }),
    );
    expect(
      ...
      testing.buildParallelCacheKey({
        ...base,
        objective: "Find DaisyClaw on GitHub",
        searchQueries: ["daisyclaw github", "daisyclaw repository"],
      }),
    );
  });
  ...
  it("normalizes objectives by trimming and capping at 5000 chars", () => {
    expect(testing.normalizeParallelObjective("  Find DaisyClaw  ")).toBe("Find DaisyClaw");
  ...
  it("normalizes search_queries: trim, drop blanks, dedupe, cap length, cap count", () => {
    ...
    ).toEqual(["daisyclaw github", "daisyclaw releases"]);
    expect(testing.normalizeParallelSearchQueries(undefined)).toEqual([]);
    expect(testing.normalizeParallelSearchQueries("daisyclaw github")).toEqual([]);
  ...
  it("identifies the plugin via a versioned User-Agent header", () => {
    expect(testing.USER_AGENT).toMatch(/^daisyclaw-parallel\/\d+\.\d+\.\d+/);
  });
  ...
  it("treats objective as optional and omits it from the request when absent", async () => {
    ...
    const result = await tool.execute({ search_queries: ["daisyclaw"] });
    ...
    expect(body).toMatchObject({ search_queries: ["daisyclaw"] });
  ...
  it("returns an error payload when search_queries is missing or empty", async () => {
    ...
    expect(await tool.execute({ objective: "Find DaisyClaw on GitHub" })).toMatchObject({
      error: "invalid_search_queries",
    });
    expect(
      await tool.execute({ objective: "Find DaisyClaw on GitHub", search_queries: [] }),
  ...
  it("promotes a generic `query` arg into search_queries when search_queries is absent (no synthesized objective)", async () => {
    // The operator CLI (`daisyclaw capability web.search`) always sends the
    ...
    const result = await tool.execute({ query: "DaisyClaw GitHub", count: 3 });
    expect(endpointMockState.calls).toHaveLength(1);
    const body = readMockedBody(endpointMockState.calls[0]) as Record<string, unknown>;
    expect(body).not.toHaveProperty("objective");
    expect(body).toMatchObject({
      search_queries: ["DaisyClaw GitHub"],
  ...
  it("honors top-level web search settings and sends the native Parallel payload shape", async () => {
    ...
    const result = await tool.execute({
      objective: "Find the DaisyClaw repository on GitHub",
      search_queries: ["daisyclaw github", "daisyclaw repository"],
    });
    ...
    expect(headers["User-Agent"]).toMatch(/^daisyclaw-parallel\//);
  ...
  it("always sends max_results matching the DaisyClaw web_search default when no count is provided", async () => {
    endpointMockState.responses.push(
      new Response(JSON.stringify({ search_id: "x", session_id: "y", results: [] }), {
        status: 200,
        headers: { "Content-Type": "application/json" },
    ...
    await tool.execute({
      objective: "Find DaisyClaw",
      search_queries: ["daisyclaw"],
    });
    expect(endpointMockState.calls).toHaveLength(1);
    const body = readMockedBody(endpointMockState.calls[0]) as {
      advanced_settings?: { max_results?: number };
    };
    // DaisyClaw's web_search default is 5 results; Parallel's own default is 10.
