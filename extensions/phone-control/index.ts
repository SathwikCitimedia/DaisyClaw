// Phone Control plugin entrypoint registers its DaisyClaw integration.
import {
  asDateTimestampMs,
...
function openArmStateStore(api: DaisyClawPluginApi) {
...
async function readArmState(api: DaisyClawPluginApi): Promise<ArmStateFile | null> {
  return (await openArmStateStore(api).lookup(ARM_STATE_KEY)) ?? null;
}

async function writeArmState(api: DaisyClawPluginApi, state: ArmStateFile | null): Promise<void> {
...
export default definePluginEntry({
  ...
  register(api: DaisyClawPluginApi) {
    ...
    const timerService: DaisyClawPluginService = {
