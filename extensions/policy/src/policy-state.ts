...
import { normalizeProviderId } from "daisyclaw/plugin-sdk/provider-model-shared";
import { normalizeAgentId } from "daisyclaw/plugin-sdk/routing";
import { coerceSecretRef } from "daisyclaw/plugin-sdk/secret-input";
import {
  asBoolean as readBoolean,
  isRecord,
  normalizeOptionalString as readString,
} from "daisyclaw/plugin-sdk/string-coerce-runtime";
...
const DEFAULT_POLICY_SANDBOX_BROWSER_NETWORK = "daisyclaw-sandbox-browser";
  ...
  return Object.entries(configuredChannels(cfg))
    ...
    .map(([id, value]) => {
      const entry: {
        ...
        id,
        provider: id,
        source: `oc://daisyclaw.config/channels/${id}`,
      };
  ...
  return Object.entries(configuredMcpServers(cfg))
    ...
    .map(([id, value]) => {
      const entry: {
        ...
        source: `oc://daisyclaw.config/mcp/servers/${ocPathSegment(id)}`,
      };
      if (isRecord(value)) {
...
export function scanPolicyModelProviders(
  ...
  return Object.keys(configuredModelProviders(cfg))
    ...
    .map((id) => ({
      ...
      source: `oc://daisyclaw.config/models/providers/${id}`,
    }));
}
...
export function scanPolicyModelRefs(
  ...
  if (isRecord(cfg.agents)) {
    collectModelRefsFromRecord(refs, cfg.agents, "oc://daisyclaw.config/agents");
...
export function scanPolicyNetwork(cfg: Record<string, unknown>): readonly PolicyNetworkEvidence[] {
  return [
    networkBooleanEvidence(
      ...
      "oc://daisyclaw.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
    ),
    networkBooleanEvidence(
      cfg,
      "browser-private-network-legacy",
      ["browser", "ssrfPolicy", "allowPrivateNetwork"],
      "oc://daisyclaw.config/browser/ssrfPolicy/allowPrivateNetwork",
    ),
    networkBooleanEvidence(
      cfg,
      "web-fetch-private-network",
      ["tools", "web", "fetch", "ssrfPolicy", "dangerouslyAllowPrivateNetwork"],
      "oc://daisyclaw.config/tools/web/fetch/ssrfPolicy/dangerouslyAllowPrivateNetwork",
    ),
    networkBooleanEvidence(
      cfg,
      "web-fetch-private-network-legacy",
      ["tools", "web", "fetch", "ssrfPolicy", "allowPrivateNetwork"],
      "oc://daisyclaw.config/tools/web/fetch/ssrfPolicy/allowPrivateNetwork",
    ),
    networkBooleanEvidence(
      cfg,
      "web-fetch-rfc2544-benchmark-range",
      ["tools", "web", "fetch", "ssrfPolicy", "allowRfc2544BenchmarkRange"],
      "oc://daisyclaw.config/tools/web/fetch/ssrfPolicy/allowRfc2544BenchmarkRange",
    ),
    networkBooleanEvidence(
      cfg,
      "web-fetch-ipv6-unique-local-range",
      ["tools", "web", "fetch", "ssrfPolicy", "allowIpv6UniqueLocalRange"],
      "oc://daisyclaw.config/tools/web/fetch/ssrfPolicy/allowIpv6UniqueLocalRange",
    ),
...
export function scanPolicyIngress(cfg: Record<string, unknown>): readonly PolicyIngressEvidence[] {
  ...
  const channelDefaultsSource = "oc://daisyclaw.config/channels/defaults";
  ...
  entries.push({
    id: "session-dm-scope",
    kind: "sessionDmScope",
    source: "oc://daisyclaw.config/session/dmScope",
    value: dmScope ?? "main",
  ...
  for (const [channel, value] of Object.entries(channels)) {
    ...
    const channelSource = `oc://daisyclaw.config/channels/${ocPathSegment(channel)}`;
...
export function scanPolicyGatewayExposure(
  ...
  entries.push({
    ...
    kind: "bind",
    source: "oc://daisyclaw.config/gateway/bind",
  ...
  if (bind === "custom" && hasCustomBindHost) {
    entries.push({
      ...
      kind: "bind",
      source: "oc://daisyclaw.config/gateway/customBindHost",
  ...
  entries.push({
    ...
    kind: "auth",
    source: "oc://daisyclaw.config/gateway/auth/mode",
  ...
  entries.push({
    ...
    kind: "authRateLimit",
    source: "oc://daisyclaw.config/gateway/auth/rateLimit",
  ...
  pushGatewayBooleanEvidence(
    ...
    "controlUi",
    controlUi.enabled,
    "oc://daisyclaw.config/gateway/controlUi/enabled",
  );
  pushGatewayBooleanEvidence(
    entries,
    ...
    "oc://daisyclaw.config/gateway/controlUi/allowInsecureAuth",
  );
  pushGatewayBooleanEvidence(
    entries,
    ...
    "oc://daisyclaw.config/gateway/controlUi/dangerouslyDisableDeviceAuth",
  );
  pushGatewayBooleanEvidence(
    entries,
    ...
    "oc://daisyclaw.config/gateway/controlUi/dangerouslyAllowHostHeaderOriginFallback",
  );
  ...
  if (typeof tailscale.mode === "string") {
    entries.push({
      ...
      kind: "tailscale",
      source: "oc://daisyclaw.config/gateway/tailscale/mode",
  ...
  if (tailscale.mode === "serve" && tailscale.preserveFunnel === true) {
    entries.push({
      ...
      kind: "tailscale",
      source: "oc://daisyclaw.config/gateway/tailscale/preserveFunnel",
      value: "funnel",
    });
  }
  ...
  if (gateway.mode === "remote") {
    entries.push({
      ...
      kind: "remote",
      source: "oc://daisyclaw.config/gateway/mode",
      value: "remote",
    });
    if (typeof remote.url === "string" && remote.url.trim() !== "") {
      entries.push({
        ...
        kind: "remote",
        source: "oc://daisyclaw.config/gateway/remote/url",
        value: true,
      });
    }
  }
...
export function scanPolicyAgentWorkspace(
  ...
  pushAgentWorkspaceEvidence(entries, {
    ...
    tools: defaultTools,
    inheritedTools: {},
    workspaceSourceBase: "oc://daisyclaw.config/agents/defaults",
    inheritedWorkspaceSourceBase: "oc://daisyclaw.config/agents/defaults",
    toolsSourceBase: "oc://daisyclaw.config/tools",
    inheritedToolsSourceBase: "oc://daisyclaw.config/tools",
  });
  ...
  list.forEach((agent, index) => {
    ...
    pushAgentWorkspaceEvidence(entries, {
      ...
      workspaceSourceBase: `oc://daisyclaw.config/agents/list/#${index}`,
      inheritedWorkspaceSourceBase: "oc://daisyclaw.config/agents/defaults",
      toolsSourceBase: `oc://daisyclaw.config/agents/list/#${index}/tools`,
      inheritedToolsSourceBase: "oc://daisyclaw.config/tools",
    });
  });
  return entries.toSorted((a, b) => a.source.localeCompare(b.source) || a.id.localeCompare(b.id));
...
export function scanPolicySandboxPosture(
  ...
  pushSandboxPostureEvidence(entries, {
    ...
    sandbox: defaultSandbox,
    inheritedSandbox: {},
    sourceBase: "oc://daisyclaw.config/agents/defaults/sandbox",
    inheritedSourceBase: "oc://daisyclaw.config/agents/defaults/sandbox",
  });
  ...
  list.forEach((agent, index) => {
    ...
    pushSandboxPostureEvidence(entries, {
      ...
      sourceBase: `oc://daisyclaw.config/agents/list/#${index}/sandbox`,
      inheritedSourceBase: "oc://daisyclaw.config/agents/defaults/sandbox",
    });
  });
...
export function scanPolicyToolPosture(
  ...
  pushToolPostureEvidence(entries, {
    ...
    sandbox: defaultSandbox,
    inheritedSandbox: {},
    sourceBase: "oc://daisyclaw.config/tools",
    inheritedSourceBase: "oc://daisyclaw.config/tools",
  });
  ...
  list.forEach((agent, index) => {
    ...
    pushToolPostureEvidence(entries, {
      ...
      inheritedSandbox: defaultSandbox,
      sourceBase: `oc://daisyclaw.config/agents/list/#${index}/tools`,
      inheritedSourceBase: "oc://daisyclaw.config/tools",
    });
  });
  ...
  return Object.entries(profiles)
    ...
    .map(([id, value]) => {
      const entry: {
        ...
        id,
        source: `oc://daisyclaw.config/auth/profiles/${ocPathSegment(id)}`,
...
function pushMemorySessionTranscriptIndexing(
  ...
  if (defaultSessionMemory !== undefined) {
    entries.push({
      ...
      kind: "memorySessionTranscriptIndexing",
      source: "oc://daisyclaw.config/agents/defaults/memorySearch/experimental/sessionMemory",
      scope: "global",
  ...
  agents.list.forEach((rawAgent, index) => {
    ...
    entries.push({
      ...
      kind: "memorySessionTranscriptIndexing",
      source: explicit
        ? `oc://daisyclaw.config/agents/list/#${index}/memorySearch/experimental/sessionMemory`
        : "oc://daisyclaw.config/agents/defaults/memorySearch/experimental/sessionMemory",
      scope: "agent",
...
function scanPolicySecretProviders(cfg: Record<string, unknown>): readonly PolicySecretEvidence[] {
  ...
  return Object.entries(providers).map(([id, value]) => {
    ...
    const entry: {
      ...
      id,
      kind: "provider",
      source: `oc://daisyclaw.config/secrets/providers/${ocPathSegment(id)}`,
    };
...
function configPathSource(path: readonly string[]): string {
  return `oc://daisyclaw.config/${path.map(ocPathSegment).join("/")}`;
}
  ...
  const sandboxModeSource =
    explicitSandboxMode !== undefined
      ...
      : inheritedSandboxMode !== undefined
        ...
        : "oc://daisyclaw.config/agents/defaults/sandbox/mode";
    ...
    source:
      explicitWorkspaceAccess !== undefined
        ...
        : inheritedWorkspaceAccess !== undefined
          ...
          : "oc://daisyclaw.config/agents/defaults/sandbox/workspaceAccess",
    scope: params.scope,
...
function pushGatewayHttpEndpointEvidence(
  ...
  const source = `oc://daisyclaw.config/gateway/http/endpoints/${endpoint}`;
...
function collectModelRefsFromAgentAllowlist(
  ...
  if (isRecord(defaults) && isRecord(defaults.models)) {
    collectModelRefsFromModelMap(
      refs,
      defaults.models,
      "oc://daisyclaw.config/agents/defaults/models",
    );
  }
  ...
  for (const [index, agent] of list.entries()) {
    ...
    collectModelRefsFromModelMap(
      refs,
      agent.models,
      `oc://daisyclaw.config/agents/list/#${index}/models`,
    );
  }
}
