...
import { resolveMergedAccountConfig } from "daisyclaw/plugin-sdk/account-resolution-runtime";
...
export function resolveQaChannelAccount(params: {
  ...
  const botUserId = merged.botUserId?.trim() || "daisyclaw";
  const botDisplayName = merged.botDisplayName?.trim() || "DaisyClaw QA";
  return {
    accountId,
    enabled,
