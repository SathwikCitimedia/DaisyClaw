...
type QaRuntimeParityScenarioReport = {
  ...
  driftDetails?: string;
  daisyclawStatus: "pass" | "fail" | "missing";
  codexStatus: "pass" | "fail" | "missing";
  daisyclawTokens: number;
  codexTokens: number;
...
function scenarioHasRuntimeToolCallEvidence(scenario: QaParityReportScenario): boolean {
  ...
  return (
    ...
    parity.cells.daisyclaw.toolCalls.length > 0 &&
...
function describeLiveUsageFailure(scenarioName: string, scenario: QaRuntimeParityScenarioReport) {
  const missing = [
    scenario.daisyclawTokens > 0
      ? undefined
      : `${scenario.daisyclawStatus === "pass" ? "daisyclaw" : "daisyclaw failed"}=0`,
...
export function renderQaAgenticParityMarkdownReport(comparison: QaAgenticParityComparison): string {
  ...
  const lines = [
    `# DaisyClaw Agentic Parity Report — ${comparison.candidateLabel} vs ${comparison.baselineLabel}`,
    "",
  ...
  const scenarios: QaRuntimeParityScenarioReport[] = params.summary.scenarios.map((scenario) => {
    ...
    if (!parity) {
      ...
      return {
        ...
        daisyclawStatus: "missing",
        codexStatus: "missing",
        daisyclawTokens: 0,
        codexTokens: 0,
        daisyclawToolCalls: 0,
        codexToolCalls: 0,
      } satisfies QaRuntimeParityScenarioReport;
    }
    driftCounts[parity.drift] += 1;
    const daisyclawCell = parity.cells.daisyclaw;
    const codexCell = parity.cells.codex;
    const daisyclawStatus = runtimeParityCellStatus(daisyclawCell);
    ...
    const reportScenario = {
      ...
      daisyclawStatus,
      codexStatus,
      daisyclawTokens: daisyclawCell.usage.totalTokens,
      codexTokens: codexCell.usage.totalTokens,
      daisyclawToolCalls: daisyclawCell.toolCalls.length,
...
export function renderQaRuntimeParityMarkdownReport(report: QaRuntimeParityReport): string {
  const lines = [
    `# DaisyClaw Runtime Parity Report — ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
  ...
  for (const scenario of report.scenarios) {
    ...
    lines.push(
      `- daisyclaw: ${scenario.daisyclawStatus} (${scenario.daisyclawToolCalls} tool calls, ${scenario.daisyclawTokens} tokens)`,
    );
    lines.push(
