...
function parseQaRuntimePair(value: string | undefined): [RuntimeId, RuntimeId] | undefined {
  ...
  if (runtimes.length !== 2) {
    throw new Error('--runtime-pair must use exactly two runtimes, e.g. "daisyclaw,codex".');
  }
  const [left, right] = runtimes;
  if (!left || !right) {
    throw new Error('--runtime-pair only supports "daisyclaw" and "codex".');
  }
  if (left === right) {
...
export async function runQaJsonlReplayCommand(opts: {
  ...
  const repoRoot = path.resolve(opts.repoRoot ?? process.cwd());
  const runtimePair = parseQaRuntimePair(opts.runtimePair) ?? ["daisyclaw", "codex"];
  if (runtimePair[0] !== "daisyclaw" || runtimePair[1] !== "codex") {
    throw new Error('--runtime-pair for jsonl-replay must be "daisyclaw,codex".');
  }
...
export async function runQaLabUiCommand(opts: {
  ...
  const server = await startQaLabServer({
    ...
    controlUiProxyToken: process.env.DAISYCLAW_QA_CONTROL_UI_PROXY_TOKEN,
