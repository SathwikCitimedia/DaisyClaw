...
import type { QaRunnerCliContribution } from "daisyclaw/plugin-sdk/qa-runner-runtime";
...
vi.mock("daisyclaw/plugin-sdk/qa-runner-runtime", () => ({
...
describe("qa cli registration", () => {
  ...
  it("routes mantis desktop browser smoke flags into the mantis runtime command", async () => {
    ...
    expect(runMantisDesktopBrowserSmokeCommand).toHaveBeenCalledWith({
      browserUrl: "https://daisyclaw.ai/docs",
  ...
  it("routes mantis Slack desktop smoke flags into the mantis runtime command", async () => {
    ...
    expect(runMantisSlackDesktopSmokeCommand).toHaveBeenCalledWith({
      ...
      fastMode: true,
      freshPr: "daisyclaw/daisyclaw#85141",
  ...
  it("registers standalone provider server commands from the provider registry", async () => {
    ...
    await program.parseAsync(["node", "daisyclaw", "qa", "aimock", "--port", "44080"]);
  ...
  it("normalizes signed decimal QA numeric option values through the shared parser", async () => {
    await program.parseAsync(["node", "daisyclaw", "qa", "aimock", "--port", "+044080"]);
  ...
  it.each([
    ...
    await expect(invalidProgram.parseAsync(["node", "daisyclaw", ...args])).rejects.toThrow(message);
  });
  ...
  it("shows an enable hint when a discovered runner plugin is installed but blocked", async () => {
    ...
    await expect(
      blockedProgram.parseAsync(["node", "daisyclaw", "qa", TEST_QA_RUNNER.commandName]),
  ...
  it("routes telegram CLI defaults into the lane runtime", async () => {
    await program.parseAsync(["node", "daisyclaw", "qa", "telegram"]);
  ...
  it("forwards --list-scenarios for telegram runs", async () => {
    await program.parseAsync(["node", "daisyclaw", "qa", "telegram", "--list-scenarios"]);
  ...
  it("forwards --allow-failures for telegram runs", async () => {
    await program.parseAsync(["node", "daisyclaw", "qa", "telegram", "--allow-failures"]);
  ...
  it("forwards --allow-failures for suite runs", async () => {
    await program.parseAsync(["node", "daisyclaw", "qa", "suite", "--allow-failures"]);
  ...
  it("forwards --pack for suite runs", async () => {
    await program.parseAsync(["node", "daisyclaw", "qa", "suite", "--pack", "personal-agent"]);
