...
describe("codex plugin lifecycle: pinned-old codex plugin with new DaisyClaw", () => {
  it("blocks with a precise update remediation when the plugin is older than the host", async () => {
    ...
    expect(result.remediation).toBe(
      'Codex plugin version 2026.5.19 is older than DaisyClaw 2026.5.21. Run "daisyclaw plugins update codex" or unpin codex, then rerun "daisyclaw doctor --fix".',
    );
  });
});

describe("codex plugin lifecycle: pinned-new codex plugin with old DaisyClaw", () => {
  it("blocks with a precise host-upgrade remediation when the plugin is newer than the host", async () => {
    ...
    expect(result.status).toBe("blocked");
    expect(result.remediation).toBe(
      "Codex plugin version 2026.5.22 requires a newer DaisyClaw host than 2026.5.21. Upgrade DaisyClaw or install a codex plugin version pinned to 2026.5.21.",
    );
  });
});
...
describe("codex plugin lifecycle: doctor migration safety matrix", () => {
  it.each([
    ...
    {
      name: "mixed profile with defaults DaisyClaw pin",
      profileShape: "mixed" as const,
      config: { agents: { defaults: { agentRuntime: { id: "daisyclaw" } } } },
      expectedRemovedRuntimePins: ["agentRuntime.id=daisyclaw"],
    },
    {
      name: "mixed profile with main-agent DaisyClaw pin",
      profileShape: "mixed" as const,
      config: { agents: { list: { main: { agentRuntime: { id: "daisyclaw" } } } } },
      expectedRemovedRuntimePins: ["agentRuntime.id=daisyclaw"],
    },
  ])(
    "keeps codex auth and strips stale DaisyClaw runtime pins for $name",
