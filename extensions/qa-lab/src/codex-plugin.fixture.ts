...
export const CODEX_PLUGIN_LIFECYCLE_MESSAGES = Object.freeze({
  missingPlugin:
    'Codex plugin is required for Codex runtime. Run "daisyclaw doctor --fix" to install @daisyclaw/codex, then retry.',
});
...
function formatPinnedOldRemediation(pluginVersion: string, hostVersion: string) {
  return `Codex plugin version ${pluginVersion} is older than DaisyClaw ${hostVersion}. Run "daisyclaw plugins update codex" or unpin codex, then rerun "daisyclaw doctor --fix".`;
}

function formatPinnedNewRemediation(pluginVersion: string, hostVersion: string) {
  return `Codex plugin version ${pluginVersion} requires a newer DaisyClaw host than ${hostVersion}. Upgrade DaisyClaw or install a codex plugin version pinned to ${hostVersion}.`;
}
...
export async function seedCodexPluginAt(
  ...
  await fs.writeFile(
    path.join(targetDir, "daisyclaw.plugin.json"),
