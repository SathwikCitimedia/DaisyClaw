...
const QA_LAB_UI_OVERLAY_DIR = "/opt/daisyclaw-qa-lab-ui";
...
function renderImageBlock(params: {
  ...
  return `    build:\n      context: ${context}\n      dockerfile: Dockerfile\n      args:\n        DAISYCLAW_EXTENSIONS: "qa-channel qa-lab"\n`;
}
...
${imageBlock}    pull_policy: never
    ...
    volumes:
      - ./state:/opt/daisyclaw-scaffold:ro
${params.bindUiDist ? `      - ${qaLabUiMount}:${QA_LAB_UI_OVERLAY_DIR}:ro\n` : ""}    healthcheck:
    ...
    command:
      - sh
      - -lc
      - DAISYCLAW_QA_CONTROL_UI_PROXY_TOKEN="$(node -e 'const fs=require("node:fs");const cfg=JSON.parse(fs.readFileSync("/opt/daisyclaw-scaffold/daisyclaw.json","utf8"));process.stdout.write(cfg.gateway?.auth?.token ?? "")')" exec node dist/index.js qa ui --host 0.0.0.0 --port ${QA_LAB_INTERNAL_PORT} --advertise-host 127.0.0.1 --advertise-port ${params.qaLabPort} --control-ui-url http://127.0.0.1:${params.gatewayPort}/ --control-ui-proxy-target http://daisyclaw-qa-gateway:18789/${params.bindUiDist ? ` --ui-dist-dir ${QA_LAB_UI_OVERLAY_DIR}` : ""} --auto-kickoff-target direct --send-kickoff-on-start --embedded-gateway disabled
    depends_on:
      qa-mock-openai:
        condition: service_healthy
`
    : ""
}  daisyclaw-qa-gateway:
${imageBlock}    pull_policy: never
    extra_hosts:
    ...
    environment:
      DAISYCLAW_CONFIG_PATH: /tmp/daisyclaw/daisyclaw.json
      DAISYCLAW_STATE_DIR: /tmp/daisyclaw/state
    ...
    volumes:
      - ./state:/opt/daisyclaw-scaffold:ro
      - ${repoMount}:/opt/daisyclaw-repo:ro
    healthcheck:
      test:
        - CMD
        - node
...
`
    ...
    command:
      - sh
      - -lc
      - mkdir -p /tmp/daisyclaw/workspace /tmp/daisyclaw/state && cp /opt/daisyclaw-scaffold/daisyclaw.json /tmp/daisyclaw/daisyclaw.json && cp -R /opt/daisyclaw-scaffold/seed-workspace/. /tmp/daisyclaw/workspace/ && ln -snf /opt/daisyclaw-repo /tmp/daisyclaw/workspace/repo && exec node dist/index.js gateway run --port 18789 --bind lan --allow-unconfigured
`;
}
...
- \`.env.example\`
- \`state/daisyclaw.json\`

Suggested flow:

1. Build the prebaked image once:
   - \`docker build -t daisyclaw:qa-local-prebaked --build-arg DAISYCLAW_EXTENSIONS="qa-channel qa-lab" -f Dockerfile .\`
2. Start the stack:
...
export async function writeQaDockerHarnessFiles(params: {
  ...
  const imageName = params.imageName ?? "daisyclaw:qa-local-prebaked";
  ...
  const files = [
    ...
    path.join(params.outputDir, "state", "daisyclaw.json"),
  ];
