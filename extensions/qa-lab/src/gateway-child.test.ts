...
vi.mock("daisyclaw/plugin-sdk/ssrf-runtime", () => ({
  fetchWithSsrFGuard: fetchWithSsrFGuardMock,
}));

vi.mock("daisyclaw/plugin-sdk/temp-path", () => ({
  resolvePreferredDaisyClawTmpDir: () => qaTempPathState.preferredTmpDir,
}));
...
function createParams(baseEnv?: NodeJS.ProcessEnv) {
  return {
    configPath: "/tmp/daisyclaw-qa/daisyclaw.json",
    gatewayToken: "qa-token",
    homeDir: "/tmp/daisyclaw-qa/home",
    stateDir: "/tmp/daisyclaw-qa/state",
    tempRoot: "/tmp/daisyclaw-qa",
    xdgConfigHome: "/tmp/daisyclaw-qa/xdg-config",
    xdgDataHome: "/tmp/daisyclaw-qa/xdg-data",
    xdgCacheHome: "/tmp/daisyclaw-qa/xdg-cache",
    bundledPluginsDir: "/tmp/daisyclaw-qa/bundled-plugins",
    stagedBundledPluginsRoot: "/repo/.artifacts/qa-runtime/daisyclaw-qa-suite-test",
...
describe("buildQaRuntimeEnv", () => {
  ...
  it("keeps the slow-reply QA opt-out enabled under fast mode", () => {
    ...
    expect(env.DAISYCLAW_TEST_FAST).toBe("1");
    expect(env.DAISYCLAW_EMBEDDED_ABORT_SETTLE_TIMEOUT_MS).toBe("2000");
    expect(env.DAISYCLAW_QA_PARENT_PID).toBe(String(process.pid));
    expect(env.DAISYCLAW_QA_TEMP_ROOT).toBe("/tmp/daisyclaw-qa");
    expect(env.DAISYCLAW_QA_STAGED_RUNTIME_ROOT).toBe(
      "/repo/.artifacts/qa-runtime/daisyclaw-qa-suite-test",
    );
    expect(env.DAISYCLAW_QA_ALLOW_LOCAL_IMAGE_PROVIDER).toBe("1");
    expect(env.DAISYCLAW_ALLOW_SLOW_REPLY_TESTS).toBe("1");
    expect(env.DAISYCLAW_BUNDLED_PLUGINS_DIR).toBe("/tmp/daisyclaw-qa/bundled-plugins");
    expect(env.DAISYCLAW_COMPATIBILITY_HOST_VERSION).toBe("2026.4.8");
  });

  it("maps live frontier key aliases into provider env vars", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        DAISYCLAW_LIVE_OPENAI_KEY: "openai-live",
        DAISYCLAW_LIVE_ANTHROPIC_KEY: "anthropic-live",
        DAISYCLAW_LIVE_GEMINI_KEY: "gemini-live",
      }),
  ...
  it("preserves Codex CLI auth home for live frontier runs while sandboxing DaisyClaw home", async () => {
    ...
    expect(env.HOME).toBe("/tmp/daisyclaw-qa/home");
    expect(env.DAISYCLAW_HOME).toBe("/tmp/daisyclaw-qa/home");
    expect(env.CODEX_HOME).toBe(codexHome);
  });

  it("forwards host HOME for live Claude CLI runs while keeping DaisyClaw home sandboxed", async () => {
    const hostHome = await mkdtemp(path.join(os.tmpdir(), "qa-host-home-"));
    cleanups.push(async () => {
      await rm(hostHome, { recursive: true, force: true });
    });
    ...
    expect(env.DAISYCLAW_HOME).toBe("/tmp/daisyclaw-qa/home");
    expect(env.DAISYCLAW_STATE_DIR).toBe("/tmp/daisyclaw-qa/state");
  });

  it("can forward host HOME for browser-backed QA runs while keeping DaisyClaw home sandboxed", async () => {
    const hostHome = await mkdtemp(path.join(os.tmpdir(), "qa-host-home-"));
    cleanups.push(async () => {
      await rm(hostHome, { recursive: true, force: true });
    });
  ...
  it("preserves the live Anthropic key for live Claude CLI runs without writing it into config", async () => {
    ...
    const env = buildQaRuntimeEnv({
      ...createParams({
        HOME: hostHome,
        DAISYCLAW_LIVE_ANTHROPIC_KEY: "anthropic-live",
        DAISYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV: '["SAFE_KEEP"]',
      }),
      providerMode: "live-frontier",
      forwardHostHomeForClaudeCli: true,
    ...
    expect(env.ANTHROPIC_API_KEY).toBe("anthropic-live");
    expect(env.DAISYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV).toBe('["SAFE_KEEP","ANTHROPIC_API_KEY"]');
    expect(env.DAISYCLAW_LIVE_CLI_BACKEND_AUTH_MODE).toBe("api-key");
  });
  ...
  it("removes preserved Anthropic keys for live Claude CLI subscription runs", async () => {
    ...
    const env = buildQaRuntimeEnv({
      ...createParams({
        ...
        DAISYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV: '["SAFE_KEEP","ANTHROPIC_API_KEY"]',
      }),
      providerMode: "live-frontier",
      forwardHostHomeForClaudeCli: true,
    ...
    expect(env.ANTHROPIC_API_KEY).toBe("anthropic-live");
    expect(env.DAISYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV).toBe('["SAFE_KEEP"]');
    expect(env.DAISYCLAW_LIVE_CLI_BACKEND_AUTH_MODE).toBe("subscription");
  });

  it("does not pass QA setup-token values to the gateway child env", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        DAISYCLAW_LIVE_SETUP_TOKEN_VALUE: `sk-ant-oat01-${"a".repeat(80)}`,
        DAISYCLAW_QA_LIVE_ANTHROPIC_SETUP_TOKEN: `sk-ant-oat01-${"b".repeat(80)}`,
      }),
      providerMode: "live-frontier",
    });

    expect(env.DAISYCLAW_LIVE_SETUP_TOKEN_VALUE).toBeUndefined();
    expect(env.DAISYCLAW_QA_LIVE_ANTHROPIC_SETUP_TOKEN).toBeUndefined();
  });

  it("does not pass Convex credential broker secrets to the gateway child env", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        DAISYCLAW_QA_CONVEX_SECRET_CI: "convex-ci-secret",
        DAISYCLAW_QA_CONVEX_SECRET_MAINTAINER: "convex-maintainer-secret",
      }),
      providerMode: "live-frontier",
    });

    expect(env.DAISYCLAW_QA_CONVEX_SECRET_CI).toBeUndefined();
    expect(env.DAISYCLAW_QA_CONVEX_SECRET_MAINTAINER).toBeUndefined();
  });
    ...
    (providerMode) => {
      const env = buildQaRuntimeEnv({
        ...createParams({
          ...
          DAISYCLAW_LIVE_ANTHROPIC_KEY: "anthropic-live",
          DAISYCLAW_LIVE_ANTHROPIC_KEYS: "anthropic-a,anthropic-b",
          DAISYCLAW_LIVE_CODEX_API_KEY: "codex-live",
          DAISYCLAW_LIVE_GEMINI_KEY: "gemini-live",
          DAISYCLAW_LIVE_OPENAI_KEY: "openai-live",
        }),
        providerMode,
      ...
      expect(env.DAISYCLAW_LIVE_OPENAI_KEY).toBeUndefined();
      expect(env.DAISYCLAW_LIVE_ANTHROPIC_KEY).toBeUndefined();
      expect(env.DAISYCLAW_LIVE_ANTHROPIC_KEYS).toBeUndefined();
      expect(env.DAISYCLAW_LIVE_CODEX_API_KEY).toBeUndefined();
      expect(env.DAISYCLAW_LIVE_GEMINI_KEY).toBeUndefined();
    },
  );
  ...
  it("stages a live Anthropic setup-token profile for isolated QA workers", async () => {
    ...
    const cfg = await testing.stageQaLiveAnthropicSetupToken({
      ...
      stateDir,
      env: {
        DAISYCLAW_LIVE_SETUP_TOKEN_VALUE: token,
      },
    });
  ...
  it("stages the OpenAI API-key fallback for live OpenAI QA workers", async () => {
    ...
    const cfg = await testing.stageQaLiveApiKeyProfiles({
      ...
      providerIds: ["openai"],
      env: {
        DAISYCLAW_LIVE_OPENAI_KEY: "qa-live-codex-fallback-key",
      },
    });
  ...
  it("stages direct live OpenAI API-key aliases for isolated QA workers", async () => {
    ...
    const cfg = await testing.stageQaLiveApiKeyProfiles({
      ...
      providerIds: ["openai"],
      env: {
        DAISYCLAW_LIVE_CODEX_API_KEY: "qa-live-direct-codex-key",
      },
    });
  ...
  it("fails fast when live OpenAI runs have no portable QA auth", () => {
    expect(() =>
      testing.assertQaLiveCodexAuthAvailable({
        ...
        providerIds: ["openai"],
        env: {
          CODEX_HOME: path.join(os.tmpdir(), "missing-daisyclaw-codex-home"),
        },
        readCodexCredentials: () => null,
      }),
  ...
  it("fails fast when forced Codex runtime uses OpenAI model refs without portable QA auth", () => {
    expect(() =>
      testing.assertQaLiveCodexAuthAvailable({
        ...
        env: {
          CODEX_HOME: path.join(os.tmpdir(), "missing-daisyclaw-codex-home"),
          DAISYCLAW_QA_FORCE_RUNTIME: "codex",
        },
        readCodexCredentials: () => null,
      }),
    ).toThrow("QA live-frontier cannot run Codex-backed OpenAI models");
  ...
  it("stages configured OpenAI env secret refs for default OpenAI live QA runs", async () => {
    ...
    const env = {
      DAISYCLAW_LIVE_CODEX_API_KEY: "qa-configured-env-ref-not-a-real-key",
    };
    const cfg = await testing.stageQaLiveApiKeyProfiles({
      cfg: {
        models: {
          providers: {
            openai: {
              ...
              apiKey: {
                source: "env",
                provider: "default",
                id: "DAISYCLAW_LIVE_CODEX_API_KEY",
              },
            },
          },
        },
  ...
  it("stages configured OpenAI env markers for live QA runs", async () => {
    ...
    const cfg = await testing.stageQaLiveApiKeyProfiles({
        ...
        models: {
          providers: {
            openai: {
              baseUrl: "",
              models: [],
              apiKey: "DAISYCLAW_LIVE_CODEX_API_KEY",
            },
          },
        },
      },
      ...
      providerIds: ["openai"],
      env: {
        DAISYCLAW_LIVE_CODEX_API_KEY: "qa-configured-marker-not-a-real-key",
      },
    });
  ...
  it("preserves only sanitized gateway debug artifacts", async () => {
    ...
    await writeFile(
      stdoutLogPath,
      [
        "DAISYCLAW_GATEWAY_TOKEN=qa-suite-token",
        'OPENAI_API_KEY="openai-live"',
        "DAISYCLAW_QA_CONVEX_SECRET_CI=convex-ci-secret",
        "DAISYCLAW_QA_CONVEX_SECRET_MAINTAINER=convex-maintainer-secret",
        "DAISYCLAW_LIVE_CODEX_API_KEY=codex-live-secret",
    ...
    await expect(readFile(path.join(artifactDir, "gateway.stdout.log"), "utf8")).resolves.toBe(
      [
        "DAISYCLAW_GATEWAY_TOKEN=<redacted>",
        "OPENAI_API_KEY=<redacted>",
        "DAISYCLAW_QA_CONVEX_SECRET_CI=<redacted>",
        "DAISYCLAW_QA_CONVEX_SECRET_MAINTAINER=<redacted>",
        "DAISYCLAW_LIVE_CODEX_API_KEY=<redacted>",
        "botToken=<redacted>",
  ...
  it("rejects preserved gateway artifacts outside the repo root", async () => {
    await expect(
      testing.assertQaArtifactDirWithinRepo("/tmp/daisyclaw-repo", "/tmp/outside"),
  ...
  it("cleans startup temp roots when they are not preserved", async () => {
    ...
    await writeFile(path.join(tempRoot, "daisyclaw.json"), "{}", "utf8");
...
describe("qa bundled plugin dir", () => {
  ...
  it("resolves bundled plugins by manifest id when the directory name differs", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "kimi-coding", "daisyclaw.plugin.json"),
  ...
  it("uses a source bundled plugin when the built copy is missing CLI metadata", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "memory-core", "daisyclaw.plugin.json"),
    ...
    await writeFile(
      path.join(repoRoot, "extensions", "memory-core", "daisyclaw.plugin.json"),
      JSON.stringify({ id: "memory-core", kind: "memory" }),
      "utf8",
    );
    await writeFile(
  ...
  it("creates a scoped bundled plugin tree for allowed plugins plus always-allowed runtime facades", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "qa-channel", "package.json"),
      JSON.stringify({ name: "@daisyclaw/qa-channel", type: "module" }, null, 2),
      "utf8",
    );
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "qa-channel", "index.js"),
      [
        'import { normalizeAccountId } from "daisyclaw/plugin-sdk/account-id";',
  ...
  it("preserves dist-runtime-only root chunks when dist also exists", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ name: "@daisyclaw/runtime-only", type: "module" }, null, 2),
      "utf8",
    );
    await writeFile(
  ...
  it("maps cli backend provider ids to their owning bundled plugin ids", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "openai", "daisyclaw.plugin.json"),
      JSON.stringify({
        id: "openai",
  ...
  it("copies selected live provider configs from the host config", async () => {
    ...
    const overrides = await testing.readQaLiveProviderConfigOverrides({
      providerIds: ["custom-openai"],
      env: { DAISYCLAW_QA_LIVE_PROVIDER_CONFIG_PATH: configPath },
    });
  ...
  it("raises the QA runtime host version to the highest allowed plugin floor", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ daisyclaw: { install: { minHostVersion: ">=2026.4.8" } } }),
      "utf8",
    );
    ...
    await writeFile(
      ...
      JSON.stringify({ daisyclaw: { install: { minHostVersion: ">=2026.4.7" } } }),
      "utf8",
    );
  ...
  it("includes always-allowed runtime facade plugins when raising the QA runtime host version", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ daisyclaw: { install: { minHostVersion: ">=2026.4.9" } } }),
      "utf8",
    );
