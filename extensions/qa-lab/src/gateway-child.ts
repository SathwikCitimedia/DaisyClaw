...
import { resolveTimerTimeoutMs } from "daisyclaw/plugin-sdk/number-runtime";
...
export function buildQaRuntimeEnv(params: {
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ...(provider?.appliesLiveEnvAliases
      ...
      : {}),
    DAISYCLAW_HOME: params.homeDir,
    DAISYCLAW_CONFIG_PATH: params.configPath,
    DAISYCLAW_STATE_DIR: params.stateDir,
    DAISYCLAW_OAUTH_DIR: path.join(params.stateDir, "credentials"),
    ...
    DAISYCLAW_EMBEDDED_ABORT_SETTLE_TIMEOUT_MS: "2000",
    DAISYCLAW_QA_PARENT_PID: String(process.pid),
    DAISYCLAW_QA_TEMP_ROOT: params.tempRoot,
    ...(params.stagedBundledPluginsRoot
      ? { DAISYCLAW_QA_STAGED_RUNTIME_ROOT: params.stagedBundledPluginsRoot }
      : {}),
    DAISYCLAW_QA_ALLOW_LOCAL_IMAGE_PROVIDER: "1",
    ...
    DAISYCLAW_ALLOW_SLOW_REPLY_TESTS: "1",
    XDG_CONFIG_HOME: params.xdgConfigHome,
    XDG_DATA_HOME: params.xdgDataHome,
    XDG_CACHE_HOME: params.xdgCacheHome,
    ...(params.bundledPluginsDir ? { DAISYCLAW_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir } : {}),
    ...(params.compatibilityHostVersion
      ? { DAISYCLAW_COMPATIBILITY_HOST_VERSION: params.compatibilityHostVersion }
      : {}),
  };
...
export async function startQaGatewayChild(params: {
  ...
  forwardHostHome?: boolean;
  mutateConfig?: (cfg: DaisyClawConfig) => DaisyClawConfig;
  runtimeEnvPatch?: NodeJS.ProcessEnv;
}) {
  const tempRoot = await fs.mkdtemp(
    path.join(resolvePreferredDaisyClawTmpDir(), "daisyclaw-qa-suite-"),
  );
  ...
  const keepTemp = process.env.DAISYCLAW_QA_KEEP_TEMP === "1";
  let gatewayPort = 0;
  let baseUrl = "";
  let wsUrl = "";
