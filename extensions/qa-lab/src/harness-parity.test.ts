...
const LEFT: HarnessVariant = { id: "left", label: "Left", runtime: "daisyclaw" };
const RIGHT: HarnessVariant = { id: "right", label: "Right", runtime: "daisyclaw" };
...
function classify(
  ...
  return buildHarnessParityResult({
    ...
    left: buildHarnessParityCell({
      variant: LEFT,
      cell: makeCell("daisyclaw", left),
      tokenUsageSource: "live-usage",
    }),
    right: buildHarnessParityCell({
      variant: RIGHT,
      cell: makeCell("daisyclaw", right),
      tokenUsageSource: "live-usage",
    }),
...
describe("harness parity", () => {
  ...
  it("labels mock token estimates separately from live usage", () => {
    const sourceCell = makeCell("daisyclaw", {
