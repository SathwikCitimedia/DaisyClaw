...
describe("jsonl replay", () => {
  ...
  it("reports the earliest divergent turn using runtime parity drift classes", async () => {
      ...
      if (turn.turn === 2) {
        return {
          ...
          cell: makeCell(runtime, {
            toolCalls: [makeToolCall(runtime === "daisyclaw" ? {} : { argsHash: "args-codex" })],
          }),
        };
      }
      if (turn.turn === 3) {
        return {
          scenarioStatus: "pass",
          cell: makeCell(runtime, {
            finalText: runtime === "daisyclaw" ? "daisyclaw wording" : "codex wording",
          }),
        };
      }
      return {
    ...
    expect(result.transcripts[0]?.cells.daisyclaw).toHaveLength(3);
