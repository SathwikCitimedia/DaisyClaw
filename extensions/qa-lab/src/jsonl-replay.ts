...
export type JsonlReplayResult = {
  transcripts: Array<{
    ...
    cells: { daisyclaw: RuntimeParityCell[]; codex: RuntimeParityCell[] };
...
export async function runJsonlReplay(
  ...
  for (const transcriptPath of transcriptPaths) {
    ...
    const cells: { daisyclaw: RuntimeParityCell[]; codex: RuntimeParityCell[] } = {
      daisyclaw: [],
      codex: [],
    };
    ...
    for (const turn of turns) {
      ...
      cells.daisyclaw.push(parity.cells.daisyclaw);
...
export function renderJsonlReplayMarkdownReport(report: JsonlReplayMarkdownReport): string {
  ...
  const lines = [
    `# DaisyClaw JSONL Replay Report - ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
