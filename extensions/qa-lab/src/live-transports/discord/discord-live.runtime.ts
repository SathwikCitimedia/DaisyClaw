...
import {
  ...
  requestDiscord,
} from "@daisyclaw/discord/api.js";
import { DEFAULT_EMOJIS } from "daisyclaw/plugin-sdk/channel-feedback";
...
import { writeExternalFileWithinRoot } from "daisyclaw/plugin-sdk/security-runtime";
import { uniqueStrings } from "daisyclaw/plugin-sdk/string-coerce-runtime";
...
const DISCORD_QA_CAPTURE_CONTENT_ENV = "DAISYCLAW_QA_DISCORD_CAPTURE_CONTENT";
const DISCORD_QA_CAPTURE_UI_METADATA_ENV = "DAISYCLAW_QA_DISCORD_CAPTURE_UI_METADATA";
const DISCORD_QA_KEEP_THREADS_ENV = "DAISYCLAW_QA_DISCORD_KEEP_THREADS";
...
const DISCORD_QA_ENV_KEYS = [
  ...
  "DAISYCLAW_QA_DISCORD_SUT_APPLICATION_ID",
] as const;
...
function resolveDiscordQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): DiscordQaRuntimeEnv {
  const voiceChannelId = env.DAISYCLAW_QA_DISCORD_VOICE_CHANNEL_ID?.trim();
  const runtimeEnv = {
    guildId: resolveEnvValue(env, "DAISYCLAW_QA_DISCORD_GUILD_ID"),
    channelId: resolveEnvValue(env, "DAISYCLAW_QA_DISCORD_CHANNEL_ID"),
    driverBotToken: resolveEnvValue(env, "DAISYCLAW_QA_DISCORD_DRIVER_BOT_TOKEN"),
    sutBotToken: resolveEnvValue(env, "DAISYCLAW_QA_DISCORD_SUT_BOT_TOKEN"),
    sutApplicationId: resolveEnvValue(env, "DAISYCLAW_QA_DISCORD_SUT_APPLICATION_ID"),
    ...(voiceChannelId ? { voiceChannelId } : {}),
  };
  validateDiscordQaRuntimeEnv(runtimeEnv, "DAISYCLAW_QA_DISCORD");
  return runtimeEnv;
}
...
async function resolveDiscordQaVoiceChannel(params: {
  ...
  if (!first) {
    throw new Error(
      "Discord voice auto-join scenario could not find a visible voice/stage channel for the SUT bot. Add voiceChannelId to the Convex discord credential payload or set DAISYCLAW_QA_DISCORD_VOICE_CHANNEL_ID.",
    );
  }
  return first;
}
...
<body>
  <main>
    ...
    <section class="message">
      <div class="author">DaisyClaw Discord SUT</div>
