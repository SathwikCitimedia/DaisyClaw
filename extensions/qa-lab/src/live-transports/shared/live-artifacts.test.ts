...
describe("live transport artifacts", () => {
  ...
  it("preserves cleanup phase labels while redacting details", () => {
    expect(
      ...
      "credential lease release: details redacted (DAISYCLAW_QA_REDACT_PUBLIC_METADATA=1)",
      "live gateway cleanup: details redacted (DAISYCLAW_QA_REDACT_PUBLIC_METADATA=1)",
    ]);
  });
    ...
    expect(
      redactQaLiveLaneIssues([
        [
          ...
          "Artifacts:",
          "- gatewayDebug: /tmp/daisyclaw-whatsapp-qa/gateway-debug",
        ].join("\n"),
      ]),
    ).toEqual(["details redacted (DAISYCLAW_QA_REDACT_PUBLIC_METADATA=1)"]);
  });
});
