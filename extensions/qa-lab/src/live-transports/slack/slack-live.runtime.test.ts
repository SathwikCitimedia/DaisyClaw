...
describe("Slack live QA runtime helpers", () => {
  ...
  it("resolves env credential payloads", () => {
    expect(
      testing.resolveSlackQaRuntimeEnv({
        DAISYCLAW_QA_SLACK_CHANNEL_ID: "C123456789",
        DAISYCLAW_QA_SLACK_DRIVER_BOT_TOKEN: "xoxb-driver",
        DAISYCLAW_QA_SLACK_SUT_BOT_TOKEN: "xoxb-sut",
        DAISYCLAW_QA_SLACK_SUT_APP_TOKEN: "xapp-sut",
      }),
    ).toEqual({
  ...
  it("rejects malformed Slack channel ids", () => {
    expect(() =>
      testing.resolveSlackQaRuntimeEnv({
        DAISYCLAW_QA_SLACK_CHANNEL_ID: "qa-channel",
        DAISYCLAW_QA_SLACK_DRIVER_BOT_TOKEN: "xoxb-driver",
        DAISYCLAW_QA_SLACK_SUT_BOT_TOKEN: "xoxb-sut",
        DAISYCLAW_QA_SLACK_SUT_APP_TOKEN: "xapp-sut",
      }),
    ).toThrow("DAISYCLAW_QA_SLACK channelId must be a Slack id like C123 or U123.");
  });
  ...
  it("resolves Slack approval checkpoint configuration from env", () => {
    expect(
      testing.resolveSlackApprovalCheckpointConfig({
        DAISYCLAW_QA_SLACK_APPROVAL_CHECKPOINT_DIR: "/tmp/checkpoints",
        DAISYCLAW_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS: "5000",
      }),
    ).toEqual({
  ...
  it("writes artifacts when Convex credential acquisition fails", async () => {
    const outputDir = await fs.mkdtemp(path.join(tmpdir(), "daisyclaw-slack-qa-"));
    ...
    expect(result.scenarios[0]?.details).toContain("Missing DAISYCLAW_QA_CONVEX_SITE_URL");
