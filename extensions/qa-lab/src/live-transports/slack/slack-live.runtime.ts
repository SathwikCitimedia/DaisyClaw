...
import { createSlackWebClient, createSlackWriteClient } from "@daisyclaw/slack/api.js";
...
const SLACK_QA_CAPTURE_CONTENT_ENV = "DAISYCLAW_QA_SLACK_CAPTURE_CONTENT";
const SLACK_QA_APPROVAL_CHECKPOINT_DIR_ENV = "DAISYCLAW_QA_SLACK_APPROVAL_CHECKPOINT_DIR";
const SLACK_QA_APPROVAL_CHECKPOINT_TIMEOUT_MS_ENV =
  "DAISYCLAW_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS";
...
const SLACK_QA_SCENARIOS: SlackQaScenarioDefinition[] = [
  ...
  },
  {
    ...
    timeoutMs: 8_000,
    configOverrides: {
      allowFrom: ["U_DAISYCLAW_QA_NEVER_ALLOWED"],
      users: ["U_DAISYCLAW_QA_NEVER_ALLOWED"],
    },
    buildRun: (sutUserId) => {
...
function resolveSlackQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): SlackQaRuntimeEnv {
  const runtimeEnv = {
    channelId: resolveEnvValue(env, "DAISYCLAW_QA_SLACK_CHANNEL_ID"),
    driverBotToken: resolveEnvValue(env, "DAISYCLAW_QA_SLACK_DRIVER_BOT_TOKEN"),
    sutBotToken: resolveEnvValue(env, "DAISYCLAW_QA_SLACK_SUT_BOT_TOKEN"),
    sutAppToken: resolveEnvValue(env, "DAISYCLAW_QA_SLACK_SUT_APP_TOKEN"),
  };
  return validateSlackQaRuntimeEnv(runtimeEnv, "DAISYCLAW_QA_SLACK");
}
...
function resolveSlackQaReadyTimeoutMs(env: NodeJS.ProcessEnv = process.env) {
  const raw = env.DAISYCLAW_QA_TRANSPORT_READY_TIMEOUT_MS;
  if (!raw) {
