...
vi.mock("daisyclaw/plugin-sdk/ssrf-runtime", async () => {
  const actual = await vi.importActual<typeof import("daisyclaw/plugin-sdk/ssrf-runtime")>(
    "daisyclaw/plugin-sdk/ssrf-runtime",
  );
  return {
    ...actual,
...
describe("telegram live qa runtime", () => {
  ...
  it("resolves required Telegram QA env vars", () => {
    expect(
      testing.resolveTelegramQaRuntimeEnv({
        ...
        DAISYCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN: "driver",
        DAISYCLAW_QA_TELEGRAM_SUT_BOT_TOKEN: "sut",
      }),
    ).toEqual({
      groupId: "-100123",
      driverToken: "driver",
  ...
  it("fails when a required Telegram QA env var is missing", () => {
    ...
    ).toThrow("DAISYCLAW_QA_TELEGRAM_SUT_BOT_TOKEN");
  });

  it("fails when the Telegram group id is not numeric", () => {
    expect(() =>
      testing.resolveTelegramQaRuntimeEnv({
        DAISYCLAW_QA_TELEGRAM_GROUP_ID: "qa-group",
        DAISYCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN: "driver",
        DAISYCLAW_QA_TELEGRAM_SUT_BOT_TOKEN: "sut",
      }),
    ).toThrow("DAISYCLAW_QA_TELEGRAM_GROUP_ID must be a numeric Telegram chat id.");
  });
  ...
  it("applies DAISYCLAW_QA_SUITE_PROGRESS override to Telegram live logging", () => {
    expect(
      testing.shouldLogTelegramQaLiveProgress({
        CI: "false",
        DAISYCLAW_QA_SUITE_PROGRESS: "true",
      }),
    ).toBe(true);
    expect(
      testing.shouldLogTelegramQaLiveProgress({
        CI: "true",
        DAISYCLAW_QA_SUITE_PROGRESS: "false",
      }),
    ).toBe(false);
    expect(
      testing.shouldLogTelegramQaLiveProgress({
        CI: "true",
        DAISYCLAW_QA_SUITE_PROGRESS: "definitely",
      }),
    ).toBe(true);
  });
  ...
  it("normalizes the Telegram QA canary timeout env", () => {
    ...
    expect(
      testing.resolveTelegramQaCanaryTimeoutMs({
        DAISYCLAW_QA_TELEGRAM_CANARY_TIMEOUT_MS: "90000",
      }),
    ).toBe(90_000);
    expect(
      testing.resolveTelegramQaCanaryTimeoutMs({
        DAISYCLAW_QA_TELEGRAM_CANARY_TIMEOUT_MS: "nope",
      }),
    ).toBe(30_000);
    for (const value of ["0x10", "1e3", "10.5"]) {
      expect(
        testing.resolveTelegramQaCanaryTimeoutMs({
          DAISYCLAW_QA_TELEGRAM_CANARY_TIMEOUT_MS: value,
        }),
      ).toBe(30_000);
    }
  });
  ...
  it("normalizes the Telegram QA scenario timeout env", () => {
    ...
    expect(
      testing.resolveTelegramQaScenarioTimeoutMs(45_000, {
        DAISYCLAW_QA_TELEGRAM_SCENARIO_TIMEOUT_MS: "180000",
      }),
    ).toBe(180_000);
    expect(
      testing.resolveTelegramQaScenarioTimeoutMs(45_000, {
        DAISYCLAW_QA_TELEGRAM_SCENARIO_TIMEOUT_MS: "nope",
      }),
    ).toBe(45_000);
    for (const value of ["0x10", "1e3", "10.5"]) {
      expect(
        testing.resolveTelegramQaScenarioTimeoutMs(45_000, {
          DAISYCLAW_QA_TELEGRAM_SCENARIO_TIMEOUT_MS: value,
        }),
      ).toBe(45_000);
    }
  });
  ...
  it("includes mention gating in the Telegram live scenario catalog", () => {
    ...
    ).toEqual(["DaisyClaw", "Model:", "Session:", "Activation:"]);
    expect(
      scenarios
    ...
    expect(otherBotStep?.input).toBe("/status@DaisyClawQaOtherBot");
  ...
  it("lists default status and regression refs in the Telegram scenario catalog", () => {
    ...
    expect(status.regressionRefs).toEqual(["daisyclaw/daisyclaw#74698"]);
    ...
    expect(usageFooter.regressionRefs).toEqual(["daisyclaw/daisyclaw#87392"]);
    ...
    expect(streamSingle.regressionRefs).toEqual(["daisyclaw/daisyclaw#39905"]);
