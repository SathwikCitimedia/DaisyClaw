...
import { formatErrorMessage } from "daisyclaw/plugin-sdk/error-runtime";
import {
  parseStrictPositiveInteger,
  resolveTimerTimeoutMs,
} from "daisyclaw/plugin-sdk/number-runtime";
import { fetchWithSsrFGuard } from "daisyclaw/plugin-sdk/ssrf-runtime";
import { isRecord, uniqueStrings } from "daisyclaw/plugin-sdk/string-coerce-runtime";
...
const TELEGRAM_QA_SCENARIOS: TelegramQaScenarioDefinition[] = [
  ...
  },
  {
    ...
    regressionRefs: ["daisyclaw/daisyclaw#74698"],
    timeoutMs: 45_000,
    buildRun: (sutUsername) =>
      telegramQaStepRun({
        expectReply: true,
        input: `/status@${sutUsername}`,
        expectedTextIncludes: ["DaisyClaw", "Model:", "Session:", "Activation:"],
      }),
  },
  {
    ...
    buildRun: (sutUsername) => {
      const steps = [
        ...
        },
        {
          ...
          expectReply: true,
          input: `/status@${sutUsername}`,
          expectedTextIncludes: ["DaisyClaw", "Session:"],
        },
        {
          expectReply: true,
          input: `/help@${sutUsername}`,
  ...
  },
  {
    ...
    buildRun: () =>
      telegramQaStepRun({
        expectReply: false,
        input: "/status@DaisyClawQaOtherBot",
      }),
  },
  {
  ...
  },
  {
    ...
    regressionRefs: ["daisyclaw/daisyclaw#87392"],
    timeoutMs: 90_000,
    buildRun: (sutUsername) => {
  ...
  },
  {
    ...
    regressionRefs: ["daisyclaw/daisyclaw#39905"],
    timeoutMs: 75_000,
    buildRun: (sutUsername) =>
      telegramQaStepRun({
        allowAnySutReply: true,
...
const TELEGRAM_QA_CAPTURE_CONTENT_ENV = "DAISYCLAW_QA_TELEGRAM_CAPTURE_CONTENT";
const QA_REDACT_PUBLIC_METADATA_ENV = "DAISYCLAW_QA_REDACT_PUBLIC_METADATA";
const QA_SUITE_PROGRESS_ENV = "DAISYCLAW_QA_SUITE_PROGRESS";
...
function resolveTelegramQaScenarioTimeoutMs(
  ...
  return parsePositiveTelegramQaEnvMs(env, "DAISYCLAW_QA_TELEGRAM_SCENARIO_TIMEOUT_MS", fallbackMs);
}
...
function resolveTelegramQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): TelegramQaRuntimeEnv {
  const groupId = resolveEnvValue(env, "DAISYCLAW_QA_TELEGRAM_GROUP_ID");
  if (!/^-?\d+$/u.test(groupId)) {
    throw new Error("DAISYCLAW_QA_TELEGRAM_GROUP_ID must be a numeric Telegram chat id.");
  }
  return {
    groupId,
    driverToken: resolveEnvValue(env, "DAISYCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN"),
    sutToken: resolveEnvValue(env, "DAISYCLAW_QA_TELEGRAM_SUT_BOT_TOKEN"),
  };
}
...
async function runInstalledDaisyClawTelegramOnboardingPreflight(params: {
  daisyClawCommand: string;
  providerMode: ReturnType<typeof normalizeQaProviderMode>;
  sutToken: string;
}) {
  const tempRoot = await fs.mkdtemp(
    path.join(resolvePreferredDaisyClawTmpDir(), "daisyclaw-npm-telegram-"),
  );
  const homeDir = path.join(tempRoot, "home");
  const stateDir = path.join(homeDir, ".daisyclaw");
  ...
  const env = {
    ...process.env,
    HOME: homeDir,
    DAISYCLAW_HOME: stateDir,
    DAISYCLAW_CONFIG_PATH: path.join(stateDir, "daisyclaw.json"),
    DAISYCLAW_STATE_DIR: stateDir,
    DAISYCLAW_GATEWAY_TOKEN: "npm-telegram-live-onboard",
    ...(params.providerMode === "live-frontier"
      ? {}
      : { OPENAI_API_KEY: process.env.OPENAI_API_KEY ?? "sk-daisyclaw-npm-telegram-preflight" }),
  };
  try {
    await execFileAsync(
      params.daisyClawCommand,
      [
        "onboard",
        "--non-interactive",
        "--accept-risk",
    ...
    await execFileAsync(params.daisyClawCommand, ["doctor", "--non-interactive"], { env });
  } finally {
...
export async function runTelegramQaLive(params: {
  ...
  try {
    if (params.sutDaisyClawCommand && params.preflightInstalledOnboarding === true) {
      writeTelegramQaProgress(progressEnabled, "installed package onboarding preflight start");
      await runInstalledDaisyClawTelegramOnboardingPreflight({
        daisyClawCommand: params.sutDaisyClawCommand,
        providerMode,
    ...
    const gatewayHarness = await startQaLiveLaneGateway({
      repoRoot,
      command: params.sutDaisyClawCommand
        ? {
            executablePath: params.sutDaisyClawCommand,
