...
import type { WhatsAppQaDriverSession } from "@daisyclaw/whatsapp/api.js";
...
describe("WhatsApp QA live runtime", () => {
  ...
  it("redacts published scenario details before rendering public artifacts", () => {
    ...
    const report = testing.renderWhatsAppQaMarkdown({
      cleanupIssues: [
        "temporary auth cleanup failed: details redacted (DAISYCLAW_QA_REDACT_PUBLIC_METADATA=1)",
      ],
      credentialSource: "convex",
      finishedAt: "2026-05-04T12:01:00.000Z",
      redactMetadata: true,
    ...
    expect(publishedScenarios[0]?.details).toBe(
      "details redacted (DAISYCLAW_QA_REDACT_PUBLIC_METADATA=1)",
    );
  ...
  it("unpacks auth archives into a caller-provided temp directory", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-wa-qa-test-"));
    try {
  ...
  it("redacts per-scenario progress details when public metadata redaction is enabled", () => {
    ...
    ).toBe("details redacted (DAISYCLAW_QA_REDACT_PUBLIC_METADATA=1)");
    expect(
      testing.formatWhatsAppScenarioProgressDetails({
        details:
  ...
  it("requires the long-reply delivery-shape tail marker in the second chunk", async () => {
    ...
    const context = {
      ...
      gateway: {
        call: async () => ({}),
        restart: async () => {},
        workspaceDir: "/tmp/daisyclaw-whatsapp-qa-gateway",
      },
      gatewayTarget: "+15550000001",
      gatewayWorkspaceDir: "/tmp/daisyclaw-whatsapp-qa-gateway",
  ...
  it("enables WhatsApp native exec and plugin approval delivery for approval scenarios", () => {
    const cfg = testing.buildWhatsAppQaConfig(
      {},
      {
        ...
        authDir: "/tmp/daisyclaw-whatsapp-qa-auth",
        dmPolicy: "allowlist",
        overrides: {
          approvals: {
            exec: true,
  ...
  it("uses automatic visible replies for WhatsApp group mention gating", () => {
    ...
    expect(scenarioRun.input).toContain("daisyclawqa reply with only this exact marker");
    ...
    expect(cfg.messages?.groupChat?.mentionPatterns).toContain("\\bdaisyclawqa\\b");
  });
