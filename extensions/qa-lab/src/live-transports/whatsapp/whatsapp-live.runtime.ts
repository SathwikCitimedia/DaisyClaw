...
} from "@daisyclaw/whatsapp/api.js";
import { normalizeE164 } from "daisyclaw/plugin-sdk/account-resolution";
...
import { normalizeStringEntries, uniqueStrings } from "daisyclaw/plugin-sdk/string-coerce-runtime";
...
const WHATSAPP_QA_CAPTURE_CONTENT_ENV = "DAISYCLAW_QA_WHATSAPP_CAPTURE_CONTENT";
...
const WHATSAPP_QA_SCENARIOS: WhatsAppQaScenarioDefinition[] = [
  ...
  },
  {
    ...
    buildRun: () => ({
      ...
      matchText: /DaisyClaw: access not configured|Pairing code:/iu,
      target: "dm",
    }),
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        configMode: "allowlist",
        expectReply: true,
        input: `daisyclawqa reply with only this exact marker: ${replyToken}`,
  ...
  },
  {
    ...
    buildRun: () => ({
      ...
      expectReply: true,
      input: "/help",
      matchText: /DaisyClaw|commands|status|\/new/iu,
      target: "dm",
    }),
  },
  {
    ...
    buildRun: () => ({
      ...
      expectReply: true,
      input: "/status",
      matchText: /DaisyClaw|status|session|agent/iu,
      target: "dm",
    }),
  },
  {
  ...
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        configMode: "allowlist",
        expectReply: true,
        input: `daisyclawqa reply with only this exact marker under groupPolicy open: ${token}`,
        matchText: token,
        target: "group",
      };
    },
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        configMode: "allowlist",
        expectReply: false,
        input: `daisyclawqa groupPolicy disabled must not reply with ${token}`,
        matchText: token,
        target: "group",
      };
    },
  ...
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        configMode: "allowlist",
        expectReply: false,
        input: `daisyclawqa blocked group should not reply with ${quietToken}`,
...
function inferWhatsAppCredentialSource(
  ...
  const normalized =
    value?.trim().toLowerCase() || env.DAISYCLAW_QA_CREDENTIAL_SOURCE?.trim().toLowerCase();
...
function resolveWhatsAppQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): WhatsAppQaRuntimeEnv {
  return validateWhatsAppQaRuntimeEnv(
    {
      driverPhoneE164: resolveEnvValue(env, "DAISYCLAW_QA_WHATSAPP_DRIVER_PHONE_E164"),
      sutPhoneE164: resolveEnvValue(env, "DAISYCLAW_QA_WHATSAPP_SUT_PHONE_E164"),
      driverAuthArchiveBase64: resolveEnvValue(
        env,
        "DAISYCLAW_QA_WHATSAPP_DRIVER_AUTH_ARCHIVE_BASE64",
      ),
      sutAuthArchiveBase64: resolveEnvValue(env, "DAISYCLAW_QA_WHATSAPP_SUT_AUTH_ARCHIVE_BASE64"),
      groupJid: env.DAISYCLAW_QA_WHATSAPP_GROUP_JID?.trim() || undefined,
    },
    "DAISYCLAW_QA_WHATSAPP",
  );
}
...
async function writeWhatsAppQaWorkspaceFixture(
  ...
  const fixtureDir = path.join(context.gatewayWorkspaceDir, ".daisyclaw", "qa-whatsapp-media");
...
export async function runWhatsAppQaLive(params: {
  ...
  try {
    ...
    tempAuthRoot = await fs.mkdtemp(
      path.join(resolvePreferredDaisyClawTmpDir(), "daisyclaw-whatsapp-qa-"),
    );
