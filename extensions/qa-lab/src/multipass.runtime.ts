...
import { sleep } from "daisyclaw/plugin-sdk/runtime-env";
import { appendRegularFile } from "daisyclaw/plugin-sdk/security-runtime";
...
const MULTIPASS_MOUNTED_REPO_PATH = "/workspace/daisyclaw-host";
const MULTIPASS_GUEST_REPO_PATH = "/workspace/daisyclaw";
const MULTIPASS_GUEST_CODEX_HOME_PATH = "/workspace/daisyclaw-codex-home";
...
export function createQaMultipassPlan(params: {
  ...
  const hostLiveProviderConfigPath =
    liveProviderConfig && fs.existsSync(liveProviderConfig.path)
      ...
      : undefined;
  const vmName = `daisyclaw-qa-${createVmSuffix()}`;
...
export function renderQaMultipassGuestScript(
  ...
  const qaCommand = [
    ...Object.entries(plan.forwardedEnv)
      .filter(
        ([key]) =>
          ...
          key !== "DAISYCLAW_QA_LIVE_PROVIDER_CONFIG_PATH",
      )
    ...
    ...(plan.guestLiveProviderConfigPath
      ? [
          `DAISYCLAW_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
          `DAISYCLAW_QA_LIVE_PROVIDER_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
        ]
      : []),
...
export async function runQaMultipass(params: {
  ...
  await writeFile(
    plan.hostLogPath,
    `# DaisyClaw QA Multipass host log\nvmName=${plan.vmName}\noutputDir=${plan.outputDir}\n\n`,
    "utf8",
  );
  await writeFile(
  ...
  try {
    ...
    throw new Error(
      `Multipass is not installed on this host. Install it with '${resolveMultipassInstallHint()}', then rerun 'pnpm daisyclaw qa suite --runner multipass'.`,
      { cause: error },
    );
  }

  const hostTransferDirPath = await fs.promises.mkdtemp(
    path.join(resolvePreferredDaisyClawTmpDir(), `${plan.vmName}-qa-suite-`),
  );
