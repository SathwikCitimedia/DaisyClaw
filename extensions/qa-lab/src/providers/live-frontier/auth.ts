...
} from "daisyclaw/plugin-sdk/provider-auth";
...
export const QA_LIVE_ANTHROPIC_SETUP_TOKEN_ENV = "DAISYCLAW_QA_LIVE_ANTHROPIC_SETUP_TOKEN";
export const QA_LIVE_SETUP_TOKEN_VALUE_ENV = "DAISYCLAW_LIVE_SETUP_TOKEN_VALUE";
const QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE_ENV = "DAISYCLAW_QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE";
...
const QA_LIVE_API_KEY_ALIASES: Readonly<Record<string, readonly string[]>> = Object.freeze({
  anthropic: ["DAISYCLAW_LIVE_ANTHROPIC_KEY"],
  gemini: ["DAISYCLAW_LIVE_GEMINI_KEY"],
  openai: [
    "CODEX_API_KEY",
    "DAISYCLAW_LIVE_CODEX_API_KEY",
...
function qaLiveOpenAiUsesCodexByDefault(cfg: DaisyClawConfig): boolean {
...
function resolveQaLiveProviderConfig(params: { cfg: DaisyClawConfig; providerId: string }) {
...
function hasQaLiveStagedApiKeyProfile(params: { cfg: DaisyClawConfig; providerId: string }) {
...
function qaLiveRequiresCodexAuth(params: {
  ...
  const forcedRuntime = params.env.DAISYCLAW_QA_FORCE_RUNTIME?.trim().toLowerCase();
  if (forcedRuntime === "daisyclaw") {
    return false;
  }
...
export function assertQaLiveCodexAuthAvailable(params: {
  ...
  throw new Error(
    [
      ...
      "Set OPENAI_API_KEY or DAISYCLAW_LIVE_OPENAI_KEY for an API-key fallback, or set CODEX_HOME to a logged-in Codex CLI home.",
      "Host DaisyClaw OAuth refresh profiles are not copied into QA temp stores.",
    ].join(" "),
  );
}
