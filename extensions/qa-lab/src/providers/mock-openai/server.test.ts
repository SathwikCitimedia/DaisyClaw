...
describe("qa mock openai server", () => {
    ...
    const final = await expectResponsesJson<{
      ...
      input: [
        makeUserInput(prompt),
        {
          type: "function_call_output",
          call_id: "call_mock_read_1",
          output: JSON.stringify({ text: "QA mission: understand this DaisyClaw repo." }),
        },
      ],
    });
  ...
  it("supports advanced QA memory and subagent recovery prompts", async () => {
    ...
    const threadMemorySearch = await fetch(`${server.baseUrl}/v1/responses`, {
      ...
      body: JSON.stringify({
        stream: true,
        instructions:
          "@daisyclaw Thread memory check: what is the hidden thread codename stored only in memory? Use memory tools first and reply only in this thread.",
        input: [
          {
            role: "user",
            content: [
  ...
  it("plans QA tool-search calls for instruction-declared Codex dynamic tools", async () => {
    ...
    const response = await postResponses(server, {
      stream: false,
      instructions: "Codex dynamic DaisyClaw tools available in this turn: web_search.",
      input: [
        makeUserInput(
    ...
    expect(String(toolPlanOutput.arguments)).toContain("DaisyClaw runtime parity fixed query");
  });
    ...
    const response = await fetch(`${server.baseUrl}/v1/responses`, {
      ...
      body: JSON.stringify({
        ...
        input: [
          ...
          {
            type: "function_call_output",
            output: "QA mission: Understand this DaisyClaw repo from source + docs before acting.",
          },
        ],
      }),
    });
  ...
  it("serves deterministic WhatsApp group audio transcription for large audio uploads", async () => {
    ...
    await expect(triggered.json()).resolves.toEqual({
      text: "daisyclawqa reply with only this exact marker after group audio preflight: WHATSAPP_QA_GROUP_AUDIO_TRANSCRIPT_OK",
    });
