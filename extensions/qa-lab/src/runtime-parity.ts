...
export type RuntimeId = "daisyclaw" | "codex";
...
export function isRuntimeParityResultPass(result: RuntimeParityResult) {
  return (
    ...
    isRuntimeParityCellPassable(result.cells.daisyclaw) &&
...
const HEARTBEAT_TRANSCRIPT_PROMPT = "[DaisyClaw heartbeat poll]";
...
function classifyRuntimeParityCells(params: {
  daisyclaw: RuntimeParityCell;
  codex: RuntimeParityCell;
  daisyclawScenarioStatus: "pass" | "fail";
  codexScenarioStatus: "pass" | "fail";
}): Pick<RuntimeParityResult, "drift" | "driftDetails"> {
  if (
    isHardFailureRuntimeError(params.daisyclaw.runtimeErrorClass) ||
    isHardFailureRuntimeError(params.codex.runtimeErrorClass) ||
    params.daisyclaw.transportErrorClass ||
    params.codex.transportErrorClass
  ) {
    return {
      drift: "failure-mode",
      driftDetails:
        params.daisyclaw.transportErrorClass || params.codex.transportErrorClass
  ...
  if (
    hasMissingToolResult(params.daisyclaw.toolCalls) ||
  ...
  const daisyclawTranscriptLines = params.daisyclaw.transcriptBytes.trim().length
    ? params.daisyclaw.transcriptBytes.trim().split(/\r?\n/u).length
    : 0;
  const codexTranscriptLines = params.codex.transcriptBytes.trim().length
    ? params.codex.transcriptBytes.trim().split(/\r?\n/u).length
    : 0;
  if (
    daisyclawTranscriptLines !== codexTranscriptLines ||
    (!params.daisyclaw.finalText && Boolean(params.codex.finalText)) ||
    (Boolean(params.daisyclaw.finalText) && !params.codex.finalText)
  ) {
    return {
      drift: "structural",
      driftDetails: `transcript/final-text structure differs (${daisyclawTranscriptLines} lines vs ${codexTranscriptLines})`,
    };
  }

  if (
    params.daisyclawScenarioStatus === "fail" ||
    params.codexScenarioStatus === "fail" ||
    !isRuntimeParityCellPassable(params.daisyclaw) ||
    !isRuntimeParityCellPassable(params.codex)
  ) {
    return {
      drift: "failure-mode",
      driftDetails:
        params.daisyclawScenarioStatus === params.codexScenarioStatus
          ? "at least one runtime failed"
          : `scenario status differs (${params.daisyclawScenarioStatus} vs ${params.codexScenarioStatus})`,
    };
  }

  if (
    normalizeTextForParity(params.daisyclaw.finalText) ===
...
export async function runRuntimeParityScenario(params: {
  ...
  const daisyclaw = await params.runCell("daisyclaw");
  const codex = await params.runCell("codex");
  const drift = classifyRuntimeParityCells({
    daisyclaw: daisyclaw.cell,
    codex: codex.cell,
    daisyclawScenarioStatus: daisyclaw.scenarioStatus,
