...
describe("runtime tool fixture", () => {
  ...
  it("does not fail Codex-native fixtures solely because DaisyClaw dynamic exposure is absent", async () => {
    const env = await makeEnv({
      ...
      gateway: {
        ...
        tempRoot: "",
        workspaceDir: "",
        runtimeEnv: { DAISYCLAW_QA_FORCE_RUNTIME: "codex" },
        call: vi.fn(),
      },
    });
    ...
    expect(details).toContain("DaisyClaw dynamic exposure is intentionally omitted");
  ...
  it("still fails required DaisyClaw dynamic fixtures when the tool is absent", async () => {
    const env = await makeEnv();
