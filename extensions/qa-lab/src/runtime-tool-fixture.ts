...
function formatCodexNativeWorkspaceDetails(params: {
  ...
  return [
    `codex-native-workspace ${params.toolName}: DaisyClaw dynamic exposure is intentionally omitted because Codex owns this workspace operation natively`,
    params.reason ? `reason: ${params.reason}` : undefined,
    `available DaisyClaw dynamic tools: ${[...params.tools].toSorted().join(", ")}`,
    params.happyRequest
...
export async function runRuntimeToolFixture(
  ...
  const dynamicExposureIntentionallyExcluded =
    env.gateway.runtimeEnv.DAISYCLAW_QA_FORCE_RUNTIME === "codex" &&
