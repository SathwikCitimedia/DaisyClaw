...
describe("qa scenario catalog", () => {
  ...
  it("loads the opt-in update.run package self-upgrade sentinel", () => {
    ...
    expect(config?.allowEnv).toBe("DAISYCLAW_QA_ALLOW_UPDATE_RUN_SELF");
  ...
  it("includes the Kitchen Sink live OpenAI plugin gauntlet", () => {
    ...
    expect(config?.requiredProviderMode).toBe("live-frontier");
    expect(config?.requiredProvider).toBe("openai");
    expect(config?.pluginSpec).toBe("npm:@daisyclaw/kitchen-sink@latest");
    expect(config?.pluginId).toBe("daisyclaw-kitchen-sink-fixture");
