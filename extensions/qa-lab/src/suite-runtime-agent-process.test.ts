...
describe("qa suite runtime agent process helpers", () => {
  ...
  it("merges isolated env overrides into qa cli runs", async () => {
    ...
    const pending = runQaCli(
      {
        ...
        gateway: {
          tempRoot: "/tmp/runtime",
          runtimeEnv: { PATH: "/usr/bin", DAISYCLAW_STATE_DIR: "/tmp/default-state" },
        },
        primaryModel: "openai/gpt-5.5",
        alternateModel: "openai/gpt-5.5-mini",
        providerMode: "mock-openai",
      ...
      {
        env: {
          DAISYCLAW_STATE_DIR: "/tmp/isolated-state",
          DAISYCLAW_CONFIG_PATH: "/tmp/isolated-state/daisyclaw.json",
        },
      },
    );
    ...
    expect(spawnEnv?.DAISYCLAW_STATE_DIR).toBe("/tmp/isolated-state");
    expect(spawnEnv?.DAISYCLAW_CONFIG_PATH).toBe("/tmp/isolated-state/daisyclaw.json");
  });
  ...
  it("finds managed dreaming cron jobs across legacy and current payload contracts", () => {
    ...
    const current = {
      ...
      payload: {
        kind: "agentTurn",
        message: "__daisyclaw_memory_core_short_term_promotion_dream__",
        lightContext: true,
      },
