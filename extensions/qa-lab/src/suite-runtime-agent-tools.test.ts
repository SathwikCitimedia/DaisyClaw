...
const repoRoot = "/repo/daisyclaw";
const gatewayTempRoot = "/tmp/daisyclaw-qa-runtime";
  ...
  it("keeps plugin-tools MCP stderr on startup failures", async () => {
    connectMock.mockImplementationOnce(async () => {
      const stderrListener = stderrOnMock.mock.calls[0]?.[1] as
        ...
        | undefined;
      stderrListener?.(
        Buffer.from("Error [ERR_MODULE_NOT_FOUND]: Cannot find package '@daisyclaw/example'\n"),
      );
    ...
    expect(message).toContain("MCP stderr tail:");
    expect(message).toContain("Cannot find package '@daisyclaw/example'");
    expect(closeMock).toHaveBeenCalled();
  });
});
