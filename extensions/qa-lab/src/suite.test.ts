...
describe("qa suite", () => {
  ...
  it("resolves transport-ready timeout from params and env", () => {
    ...
    expect(
      qaSuiteProgressTesting.resolveQaSuiteTransportReadyTimeoutMs(undefined, {
        DAISYCLAW_QA_TRANSPORT_READY_TIMEOUT_MS: "180000",
      }),
    ).toBe(180_000);
    expect(
      qaSuiteProgressTesting.resolveQaSuiteTransportReadyTimeoutMs(undefined, {
        DAISYCLAW_QA_TRANSPORT_READY_TIMEOUT_MS: "bad",
      }),
    ).toBe(120_000);
    for (const value of ["0x10", "1e3", "10.5"]) {
      expect(
        qaSuiteProgressTesting.resolveQaSuiteTransportReadyTimeoutMs(undefined, {
          DAISYCLAW_QA_TRANSPORT_READY_TIMEOUT_MS: value,
        }),
      ).toBe(120_000);
    }
  ...
  it("applies DAISYCLAW_QA_SUITE_PROGRESS override and falls back on invalid values", () => {
    expect(
    ...
    expect(
      qaSuiteProgressTesting.shouldLogQaSuiteProgress({
        CI: "false",
        DAISYCLAW_QA_SUITE_PROGRESS: "on",
      }),
    ).toBe(true);
    expect(
      qaSuiteProgressTesting.shouldLogQaSuiteProgress({
        CI: "true",
        DAISYCLAW_QA_SUITE_PROGRESS: "off",
      }),
    ).toBe(false);
    expect(
      qaSuiteProgressTesting.shouldLogQaSuiteProgress({
  ...
  it("arms gateway heap checkpoint env only when requested", () => {
    expect(
      qaSuiteProgressTesting.buildQaGatewayHeapCheckpointRuntimeEnvPatch({
        DAISYCLAW_QA_GATEWAY_HEAP_CHECKPOINTS: "0",
      }),
    ).toBeUndefined();
    expect(
      qaSuiteProgressTesting.buildQaGatewayHeapCheckpointRuntimeEnvPatch({
        DAISYCLAW_QA_GATEWAY_HEAP_CHECKPOINTS: "1",
  ...
  it("omits mock OpenAI rewiring for non-codex runtime overrides", () => {
    expect(
      qaSuiteProgressTesting.buildQaRuntimeEnvPatch({
        providerMode: "mock-openai",
        forcedRuntime: "daisyclaw",
        mockBaseUrl: "http://127.0.0.1:44080",
      }),
    ).toEqual({
      DAISYCLAW_BUILD_PRIVATE_QA: "1",
      DAISYCLAW_QA_FORCE_RUNTIME: "daisyclaw",
    });
  });
