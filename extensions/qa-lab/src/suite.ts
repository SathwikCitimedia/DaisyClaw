...
import { disposeRegisteredAgentHarnesses } from "daisyclaw/plugin-sdk/agent-harness";
...
} from "daisyclaw/plugin-sdk/qa-runtime";
...
function shouldLogQaSuiteProgress(env: NodeJS.ProcessEnv = process.env) {
  const override = parseQaSuiteBooleanEnv(env.DAISYCLAW_QA_SUITE_PROGRESS);
...
async function runScenario(name: string, steps: QaSuiteStep[]): Promise<QaSuiteScenarioResult> {
  ...
  for (const step of steps) {
    try {
      if (process.env.DAISYCLAW_QA_DEBUG === "1") {
...
function buildRuntimeParityScenarioResult(params: {
  ...
  const daisyclawCell = params.result.cells.daisyclaw;
  return {
    ...
    steps: [
      {
        name: daisyclawCell.runtime,
        status:
          daisyclawCell.runtimeErrorClass || daisyclawCell.transportErrorClass ? "fail" : "pass",
        details: formatRuntimeParityCellDetails(daisyclawCell),
      },
      {
...
function buildQaRuntimeEnvPatch(params: {
  ...
  if (params.forcedRuntime) {
    patch.DAISYCLAW_BUILD_PRIVATE_QA = "1";
    patch.DAISYCLAW_QA_FORCE_RUNTIME = params.forcedRuntime;
  }
  if (params.forcedRuntime !== "codex" || params.providerMode !== "mock-openai") {
  ...
  patch.DAISYCLAW_CODEX_APP_SERVER_ARGS = `app-server -c openai_base_url=${mockBaseUrl}/v1 --listen stdio://`;
...
function shouldCaptureGatewayHeapCheckpoints(env: NodeJS.ProcessEnv = process.env) {
  return parseQaSuiteBooleanEnv(env.DAISYCLAW_QA_GATEWAY_HEAP_CHECKPOINTS) === true;
}
...
async function writeQaSuiteArtifacts(params: {
  ...
  const report = renderQaMarkdownReport({
    title: "DaisyClaw QA Scenario Suite",
    startedAt: params.startedAt,
...
export async function runQaSuite(params?: QaSuiteRunParams): Promise<QaSuiteResult> {
  ...
  const enabledPluginIds = [
    ...new Set([
      ...
      ...(params?.forcedRuntime && params.forcedRuntime !== "daisyclaw"
  ...
  const gateway = await startQaGatewayChild({
    ...
    mutateConfig: gatewayConfigPatch
      ? (cfg) => applyQaMergePatch(cfg, gatewayConfigPatch) as DaisyClawConfig
      : undefined,
  ...
  try {
    ...
    const keepTemp = process.env.DAISYCLAW_QA_KEEP_TEMP === "1" || false;
    await gateway.stop({
      keepTemp,
