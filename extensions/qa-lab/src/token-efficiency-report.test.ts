  ...
  it("does not fail live reports solely because Codex uses fewer tokens", () => {
    const report = buildTokenEfficiencyReport({
      ...
      summary: makeLiveSummary([
        makeRuntimeParity(
          "codex-savings",
          makeCell("daisyclaw", { inputTokens: 120, outputTokens: 80, totalTokens: 200 }),
    ...
    const report = buildTokenEfficiencyReport({
      ...
      summary: makeLiveSummary([
        makeRuntimeParity(
          ...
          makeCell("daisyclaw", { inputTokens: 72_000, outputTokens: 381, totalTokens: 72_381 }, [
  ...
  it("keeps live zero-usage rows failing instead of passing as neutral", () => {
    const report = buildTokenEfficiencyReport({
      summary: makeLiveSummary([
        makeRuntimeParity(
          "missing-live-usage",
          makeCell("daisyclaw", { inputTokens: 0, outputTokens: 0, totalTokens: 0 }),
    ...
    expect(report.pass).toBe(false);
    expect(report.failures).toEqual([
      "missing-live-usage daisyclaw live usage totalTokens=0",
        ...
        scenarios: [
          {
            ...
            runtimeParity: makeRuntimeParity(
              "mock-regression",
              makeCell("daisyclaw", { inputTokens: 100, outputTokens: 0, totalTokens: 100 }),
  ...
  it("renders savings and regression classifications in the markdown report", () => {
    const report = buildTokenEfficiencyReport({
      ...
      summary: makeLiveSummary([
        makeRuntimeParity(
          "codex-savings",
          makeCell("daisyclaw", { inputTokens: 100, outputTokens: 100, totalTokens: 200 }),
