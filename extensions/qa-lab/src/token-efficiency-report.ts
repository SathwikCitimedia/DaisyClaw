...
export type TokenEfficiencyRow = {
  ...
  daisyclaw: TokenEfficiencyRuntimeUsage;
...
export type TokenEfficiencyReport = {
  ...
  aggregate: {
    daisyclaw: { totalTokens: number; p50PerScenario: number; p90PerScenario: number };
...
const ZERO_AGGREGATE: TokenEfficiencyReport["aggregate"] = {
  daisyclaw: { totalTokens: 0, p50PerScenario: 0, p90PerScenario: 0 },
...
function deltaPercent(daisyclawTotalTokens: number, codexTotalTokens: number): number {
  if (daisyclawTotalTokens === 0) {
    return codexTotalTokens === 0 ? 0 : 100;
  }
  return ((codexTotalTokens - daisyclawTotalTokens) / daisyclawTotalTokens) * 100;
}
...
function toolNamesForCells(daisyclaw: RuntimeParityCell, codex: RuntimeParityCell): string[] {
  return [
    ...new Set([...daisyclaw.toolCalls, ...codex.toolCalls].map((call) => call.tool)),
...
function buildRow(params: {
  ...
  const daisyclaw = runtimeUsage(params.result.cells.daisyclaw);
  const codex = runtimeUsage(params.result.cells.codex);
  const delta = deltaPercent(daisyclaw.totalTokens, codex.totalTokens);
  ...
  return {
    ...
    classification,
    flagged,
    toolsUsed: toolNamesForCells(params.result.cells.daisyclaw, params.result.cells.codex),
  };
}

function buildAggregate(rows: readonly TokenEfficiencyRow[]): TokenEfficiencyReport["aggregate"] {
  const daisyclawTotals = rows.map((row) => row.daisyclaw.totalTokens);
  const codexTotals = rows.map((row) => row.codex.totalTokens);
  const daisyclawTotalTokens = daisyclawTotals.reduce((sum, value) => sum + value, 0);
  const codexTotalTokens = codexTotals.reduce((sum, value) => sum + value, 0);
  return {
    daisyclaw: {
      totalTokens: daisyclawTotalTokens,
      p50PerScenario: percentile(daisyclawTotals, 50),
      p90PerScenario: percentile(daisyclawTotals, 90),
    },
    codex: {
    ...
    deltaPercent: deltaPercent(daisyclawTotalTokens, codexTotalTokens),
...
function liveEvidenceFailures(row: TokenEfficiencyRow): string[] {
  ...
  if (row.daisyclaw.totalTokens <= 0) {
    failures.push(`${row.scenarioId} daisyclaw live usage totalTokens=${row.daisyclaw.totalTokens}`);
  }
...
export function buildTokenEfficiencyReport(
  ...
  return {
    ...
    notes: [
      ...
      "Codex savings are reported as savings and do not fail the gate; only positive Codex-over-DaisyClaw live deltas exceed the threshold.",
...
export function renderTokenEfficiencyMarkdownReport(report: TokenEfficiencyReport): string {
  const lines = [
    `# DaisyClaw Runtime Token Efficiency - ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
  ...
  lines.push(
    ...
    "| --- | ---: | ---: | ---: |",
    `| daisyclaw | ${report.aggregate.daisyclaw.totalTokens} | ${report.aggregate.daisyclaw.p50PerScenario} | ${report.aggregate.daisyclaw.p90PerScenario} |`,
  ...
  if (report.rows.length > 0) {
    lines.push(
      "## Scenario Efficiency",
      "",
      "| Scenario | Source | DaisyClaw in/out/total/tools | Codex in/out/total/tools | Token delta | Classification | Flagged | Tools used |",
      "| --- | --- | ---: | ---: | ---: | --- | --- | --- |",
    );
    for (const row of report.rows) {
      lines.push(
        `| ${row.scenarioId} | ${row.usageSource} | ${row.daisyclaw.inputTokens}/${row.daisyclaw.outputTokens}/${row.daisyclaw.totalTokens}/${row.daisyclaw.toolCallCount} | ${row.codex.inputTokens}/${row.codex.outputTokens}/${row.codex.totalTokens}/${row.codex.toolCallCount} | ${formatPercent(row.deltaPercent)} | ${row.classification} | ${row.flagged ? "yes" : "no"} | ${row.toolsUsed.join(", ")} |`,
      );
    }
    lines.push("");
  }
