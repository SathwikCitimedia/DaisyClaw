...
export type QaToolCoverageRow = {
  ...
  scenarios: string[];
  sourcePaths: string[];
  daisyclaw: QaToolCoverageStatus;
  codex: QaToolCoverageStatus;
  drift: QaToolCoverageDrift;
  daisyclawToolCalls: number;
...
function countRuntimeToolCalls(
  ...
  const cell = runtime === "daisyclaw" ? result.cells.daisyclaw : result.cells.codex;
...
function buildRow(params: {
  ...
  return {
    ...
    daisyclaw: result ? cellStatus(result.cells.daisyclaw) : "not-run",
    codex: result ? cellStatus(result.cells.codex) : "not-run",
    drift: result?.drift ?? "not-run",
    daisyclawToolCalls: countRuntimeToolCalls(result, "daisyclaw", runtimeToolName),
...
function coverageFailureForRow(row: QaToolCoverageRow): string | undefined {
  ...
  if (row.daisyclaw !== "pass" || row.codex !== "pass") {
    return `${row.tool} status daisyclaw=${row.daisyclaw} codex=${row.codex}`;
  }
  if (row.drift === "failure-mode") {
    return `${row.tool} drift=failure-mode${row.details ? ` (${row.details})` : ""}`;
  }
  if (row.runtimeToolName && row.daisyclawToolCalls === 0) {
    return `${row.tool} missing daisyclaw tool call ${row.runtimeToolName}`;
  }
...
export function buildQaToolCoverageReport(params: {
  ...
  return {
    ...
    dynamicIntegrationTools: rows.filter((row) => row.bucket === "daisyclaw-dynamic-integration")
      .length,
    searchableDynamicTools: rows.filter(
      (row) => row.capabilityLayer === "daisyclaw-dynamic-searchable",
    ).length,
...
export function renderQaToolCoverageMarkdownReport(report: QaToolCoverageReport): string {
  const lines = [
    `# DaisyClaw Runtime Tool Coverage — ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
    ...
    `- DaisyClaw dynamic integration tools: ${report.dynamicIntegrationTools}`,
    ...
    "",
    "| Tool | Bucket | Expected layer | Capability layer | Required | Fixtures | DaisyClaw | Codex | Drift | Codex default impact | QA impact | Action | Tracking |",
    "| --- | --- | --- | --- | --- | ---: | --- | --- | --- | --- | --- | --- | --- |",
  ];

  for (const row of report.rows) {
    lines.push(
      `| ${row.tool} | ${row.bucket} | ${row.expectedLayer} | ${row.capabilityLayer} | ${row.required ? "yes" : "no"} | ${row.fixtureCount} | ${row.daisyclaw} | ${row.codex} | ${row.drift} | ${row.codexDefaultImpact ?? ""} | ${row.qaImpact ?? ""} | ${row.action ?? ""} | ${row.tracking ?? ""} |`,
    );
  }
