...
import { spawn as startDaisyClawCliProcess } from "node:child_process";
...
export function formatMatrixQaCliCommand(args: string[]) {
  return `daisyclaw ${redactMatrixQaCliArgs(args).join(" ")}`;
}

export function resolveMatrixQaDaisyClawCliEntryPath(cwd: string): string {
...
function killMatrixQaCliChild(
  child: ReturnType<typeof startDaisyClawCliProcess>,
...
export function startMatrixQaDaisyClawCli(params: {
  ...
  const distEntryPath = resolveMatrixQaDaisyClawCliEntryPath(cwd);
  const stdout: Buffer[] = [];
  const stderr: Buffer[] = [];
  let closed = false;
  ...
  const child = startDaisyClawCliProcess(process.execPath, [distEntryPath, ...params.args], {
    cwd,
...
export async function runMatrixQaDaisyClawCli(params: {
  allowNonZero?: boolean;
  args: string[];
  cwd?: string;
  env: NodeJS.ProcessEnv;
  ...
  return await startMatrixQaDaisyClawCli(params).wait();
}
...
export async function createMatrixQaDaisyClawCliRuntime(params: {
  accountId: string;
  accessToken: string;
