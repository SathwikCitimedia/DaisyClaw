...
import type {
  ...
  MessageEventContent,
} from "@daisyclaw/matrix/test-api.js";
import type {
...
type MatrixQaE2eeRuntime = typeof import("@daisyclaw/matrix/test-api.js");
...
function resolveMatrixQaPluginStateNamespaceKey(options: OpenKeyedStoreOptions): string {
  return `${options.env?.DAISYCLAW_STATE_DIR ?? ""}\0${options.namespace}`;
}
...
export async function loadMatrixQaE2eeRuntime(): Promise<MatrixQaE2eeRuntime> {
  const { loadQaRunnerBundledPluginTestApi } =
    await import("daisyclaw/plugin-sdk/qa-runner-runtime");
