...
function resolveQianfanPreset(cfg: DaisyClawConfig): {
  api: ModelApi;
  baseUrl: string;
...
const qianfanPresetAppliers = createDefaultModelsPresetAppliers({
  ...
  resolveParams: (cfg: DaisyClawConfig) => {
...
export function applyQianfanProviderConfig(cfg: DaisyClawConfig): DaisyClawConfig {
  return qianfanPresetAppliers.applyProviderConfig(cfg);
}

export function applyQianfanConfig(cfg: DaisyClawConfig): DaisyClawConfig {
