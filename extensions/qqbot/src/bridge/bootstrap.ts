...
let mediaRuntimeModulePromise: Promise<typeof import("daisyclaw/plugin-sdk/media-runtime")> | null =
  null;

const loadMediaRuntimeModule = async () => {
  mediaRuntimeModulePromise ??= import("daisyclaw/plugin-sdk/media-runtime");
...
function createBuiltinAdapter(): PlatformAdapter {
  return {
    ...
    async resolveApproval(approvalId: string, decision: string): Promise<boolean> {
      try {
        const { getRuntimeConfig } = await import("daisyclaw/plugin-sdk/runtime-config-snapshot");
        const { resolveApprovalOverGateway } =
          await import("daisyclaw/plugin-sdk/approval-gateway-runtime");
