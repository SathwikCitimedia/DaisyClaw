...
import { coerceSecretRef, normalizeSecretInputString } from "daisyclaw/plugin-sdk/secret-input";
...
export function listQQBotAccountIds(cfg: DaisyClawConfig): string[] {
  return listAccountIds(cfg as unknown as Record<string, unknown>);
}

/** Resolve the default QQBot account ID. */
export function resolveDefaultQQBotAccountId(cfg: DaisyClawConfig): string {
...
/** Apply account config updates back into the DaisyClaw config object. */
