...
import { getExecApprovalReplyMetadata } from "daisyclaw/plugin-sdk/approval-runtime";
import {
...
import { sanitizeAssistantVisibleText } from "daisyclaw/plugin-sdk/text-chunking";
...
import { toGatewayAccount, writeDaisyClawConfigThroughRuntime } from "./bridge/narrowing.js";
...
export const qqbotPlugin: ChannelPlugin<ResolvedQQBotAccount> = {
  ...
  config: {
     ...
     * that wiped daisyclaw.json mid-flight.
     */
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      // daisyclaw.json). We only restore when both fields are empty so a
      ...
      if (!account.appId || !account.clientSecret) {
        ...
        if (backup?.appId && backup?.clientSecret) {
          try {
            ...
            await writeDaisyClawConfigThroughRuntime(getQQBotRuntime(), nextCfg);
            cfg = nextCfg;
      ...
      await startGateway({
        ...
        onReady: () => {
          ...
          // upgrade that might wipe daisyclaw.json mid-flight.
    ...
    logoutAccount: async ({ accountId, cfg }) => {
      ...
      if (changed) {
        await writeDaisyClawConfigThroughRuntime(getQQBotRuntime(), nextCfg as DaisyClawConfig);
      }

      const resolved = resolveQQBotAccount((changed ? nextCfg : cfg) as DaisyClawConfig, accountId);
