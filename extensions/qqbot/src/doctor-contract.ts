...
import type { GroupToolPolicyConfig } from "daisyclaw/plugin-sdk/channel-policy";
import type { DaisyClawConfig } from "daisyclaw/plugin-sdk/config-contracts";
import { asObjectRecord } from "daisyclaw/plugin-sdk/runtime-doctor";
...
export const legacyConfigRules: ChannelDoctorLegacyConfigRule[] = [
  {
    ...
    message:
      'channels.qqbot.groups.<id>.toolPolicy is legacy and was ignored by QQBot group tool enforcement; use channels.qqbot.groups.<id>.tools instead. Run "daisyclaw doctor --fix".',
    match: hasLegacyGroupToolPolicy,
  },
  {
    path: ["channels", "qqbot", "accounts"],
    message:
      'channels.qqbot.accounts.<id>.groups.<groupId>.toolPolicy is legacy and was ignored by QQBot group tool enforcement; use channels.qqbot.accounts.<id>.groups.<groupId>.tools instead. Run "daisyclaw doctor --fix".',
...
export function normalizeCompatibilityConfig({
  ...
  return {
    config: {
      ...
      channels: {
        ...cfg.channels,
        qqbot: updated as unknown as NonNullable<DaisyClawConfig["channels"]>["qqbot"],
