...
import { isImplicitSameChatApprovalAuthorization } from "daisyclaw/plugin-sdk/approval-auth-runtime";
...
async function handleApprovalButtonInteraction(params: {
  ...
  event: InteractionEvent;
  getActiveCfg?: () => DaisyClawConfig | Record<string, unknown>;
  log?: EngineLogger;
  ...
  try {
    cfg = params.getActiveCfg() as DaisyClawConfig;
  } catch (err) {
    await acknowledgeApprovalInteraction(params.creds, params.event, params.log, {
      content: "Approval is unavailable.",
    });
