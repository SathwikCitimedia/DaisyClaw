...
let daisyclawVersion = "unknown";

/** Build the User-Agent string from the current plugin and framework versions. */
function buildUserAgent(): string {
  return `QQBotPlugin/${pluginVersion} (Node/${process.versions.node}; ${os.platform()}; DaisyClaw/${daisyclawVersion})`;
}
 ...
 */
export function initSender(options: { pluginVersion?: string; daisyclawVersion?: string }): void {
  if (options.pluginVersion) {
    pluginVersion = options.pluginVersion;
  }
  if (options.daisyclawVersion) {
    daisyclawVersion = options.daisyclawVersion;
  }
}

/** Update the DaisyClaw framework version in the User-Agent (called after runtime injection). */
export function setDaisyClawVersion(version: string): void {
  if (version) {
