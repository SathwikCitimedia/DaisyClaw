...
describe("qqbot storage laziness", () => {
  ...
  it("does not create ~/.daisyclaw/qqbot from module imports or read-only probes", async () => {
    ...
    const qqbotRoot = path.join(homeDir, ".daisyclaw", "qqbot");
  ...
  it("creates storage when qqbot persists runtime state", async () => {
    ...
    const qqbotRoot = path.join(homeDir, ".daisyclaw", "qqbot");
    const sqlitePath = path.join(stateDir, "state", "daisyclaw.sqlite");
