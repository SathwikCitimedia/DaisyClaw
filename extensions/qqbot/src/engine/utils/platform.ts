 ...
 * silk-wasm availability checks — all without importing `daisyclaw/plugin-sdk`.
 ...
 * `DAISYCLAW_HOME`. QQ Bot still checks this tree for legacy state imports and
...
/**
 * Resolve the effective DaisyClaw home directory.
 *
 * Mirrors the contract from core (`src/infra/home-dir.ts::resolveEffectiveHomeDir`)
 * so QQ Bot media roots live under the same tree the rest of DaisyClaw treats as
 * `~`. The extension cannot import the core helper directly (it is a separate
 * package with `daisyclaw` as a peer dependency), so this re-implements the
 * minimal contract:
 *
 * 1. `DAISYCLAW_HOME` when set (with `~` / `~/...` expanded against the OS home).
 ...
 */
function resolveDaisyClawHome(): string {
  const raw = process.env.DAISYCLAW_HOME?.trim();
...
/**
 * Return a legacy path under `~/.daisyclaw/qqbot` without creating it.
 *
...
export function getQQBotDataPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".daisyclaw", "qqbot", ...subPaths);
}

/** Return a path under `~/.daisyclaw/qqbot`, creating it on demand. */
...
/**
 * Return a path under `<daisyclaw-home>/.daisyclaw/media/qqbot` without creating it.
 *
 * Unlike `getQQBotDataPath`, this lives under DaisyClaw's core media allowlist
 * so downloaded images and audio can be accessed by framework media tooling.
 * The base honors `DAISYCLAW_HOME` (when set) so files written by agents into
 * the DaisyClaw-managed media tree are reachable by this plugin even when
 * `HOME` and `DAISYCLAW_HOME` differ (Docker, multi-user hosts). Fixes #83562.
 */
export function getQQBotMediaPath(...subPaths: string[]): string {
  return path.join(resolveDaisyClawHome(), ".daisyclaw", "media", "qqbot", ...subPaths);
}

/** Return a path under `<daisyclaw-home>/.daisyclaw/media/qqbot`, creating it on demand. */
...
/**
 * Return `<daisyclaw-home>/.daisyclaw/media`, DaisyClaw's shared media root.
 *
 * This mirrors the directory that core's `buildMediaLocalRoots` exposes as an
 * allowlisted location (see `daisyclaw/src/media/local-roots.ts`). Using it as a
 ...
 * {@link getQQBotMediaPath}, the base honors `DAISYCLAW_HOME`.
 */
function getDaisyClawMediaDir(): string {
  return path.join(resolveDaisyClawHome(), ".daisyclaw", "media");
}
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const daisyclawHomeDir = resolveDaisyClawHome();
  const mediaRoot = getQQBotMediaPath();
  const dataRoot = getQQBotDataPath();
  // When DAISYCLAW_HOME differs from HOME we have to consider workspace roots
  // under both trees: agents may be configured with `~`-relative paths (HOME)
  // or with the DaisyClaw-managed home tree. Deduplicate when they match.
  const workspaceRoots = Array.from(
    new Set([
      path.join(osHomeDir, ".daisyclaw", "workspace", "qqbot"),
      path.join(daisyclawHomeDir, ".daisyclaw", "workspace", "qqbot"),
    ]),
  );
...
export function resolveQQBotPayloadLocalFilePath(p: string): string | null {
  ...
  // Trust both the QQ Bot-owned subdirectory and DaisyClaw's shared `~/.daisyclaw/media`
  ...
  const allowedRoots = [getDaisyClawMediaDir(), getQQBotMediaPath()];
