...
import { runPluginCommandWithTimeout } from "daisyclaw/plugin-sdk/run-command";
import type { RuntimeEnv } from "daisyclaw/plugin-sdk/runtime-env";
import { CONFIG_DIR, extractArchive, resolveBrewExecutable } from "daisyclaw/plugin-sdk/setup-tools";
...
export async function installSignalCliFromRelease(
  ...
  const tmpDir = await fs.mkdtemp(path.join(resolvePreferredDaisyClawTmpDir(), "daisyclaw-signal-"));
