...
import type { MockFn } from "daisyclaw/plugin-sdk/plugin-test-runtime";
...
const signalToolResultSessionStorePath = vi.hoisted(
  () => `/tmp/daisyclaw-signal-tool-result-sessions-${process.pid}.json`,
);
...
vi.mock("daisyclaw/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("daisyclaw/plugin-sdk/security-runtime")>(
    "daisyclaw/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
vi.mock("daisyclaw/plugin-sdk/system-event-runtime", async () => {
  const actual = await vi.importActual<typeof import("daisyclaw/plugin-sdk/system-event-runtime")>(
    "daisyclaw/plugin-sdk/system-event-runtime",
  );
  return {
    ...actual,
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("daisyclaw/plugin-sdk/reply-runtime"),
      import("daisyclaw/plugin-sdk/system-event-runtime"),
    ]);
    resetInboundDedupe();
    config = {
