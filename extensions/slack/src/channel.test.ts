...
async function getSlackConfiguredState(cfg: DaisyClawConfig) {
...
describe("slackPlugin status", () => {
  it("uses the direct Slack probe helper when runtime is not initialized", async () => {
    const probeSpy = vi.spyOn(probeModule, "probeSlack").mockResolvedValueOnce({
      ok: true,
      status: 200,
      bot: { id: "B1", name: "daisyclaw-bot" },
      team: { id: "T1", name: "DaisyClaw" },
    });
    clearSlackRuntime();
    const cfg = {
      channels: {
  ...
  it("keeps Slack MPIM outbound routing as group", async () => {
    ...
    const route = await resolveRoute({
      cfg: { channels: { slack: { botToken: "xoxb-test" } } } as DaisyClawConfig,
      agentId: "main",
      target: "G123",
    });
...
describe("slackPlugin agentPrompt", () => {
  it("tells agents interactive replies are disabled by default", () => {
    ...
    expect(hints).toContain(
      "- Slack plain text sends: write standard Markdown; DaisyClaw converts it to Slack mrkdwn, including `**bold**`, headings, lists, and `[label](url)` links.",
    );
    expect(hints).toContain(
