...
import { expectPairingReplyText } from "daisyclaw/plugin-sdk/channel-test-helpers";
import { HISTORY_CONTEXT_MARKER } from "daisyclaw/plugin-sdk/reply-history";
...
describe("monitorSlackProvider tool results", () => {
  ...
  async function expectMentionPatternMessageAccepted(text: string): Promise<void> {
    setRequireMentionChannelConfig(["\\bdaisyclaw\\b"]);
  ...
  it("accepts channel messages when mentionPatterns match", async () => {
    await expectMentionPatternMessageAccepted("daisyclaw: hello");
  });

  it("accepts channel messages when mentionPatterns match even if another user is mentioned", async () => {
    await expectMentionPatternMessageAccepted("daisyclaw: hello <@U2>");
  });
