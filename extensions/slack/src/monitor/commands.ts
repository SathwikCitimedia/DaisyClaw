...
import type { SlackSlashCommandConfig } from "daisyclaw/plugin-sdk/config-contracts";
...
export function resolveSlackSlashCommandConfig(
  ...
  const normalizedName = normalizeSlackSlashCommandName(
    normalizeOptionalString(raw?.name) ?? "daisyclaw",
  );
  const name = normalizedName || "daisyclaw";
  return {
