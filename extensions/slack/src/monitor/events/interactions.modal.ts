...
const DAISYCLAW_MODAL_CALLBACK_PREFIX = "daisyclaw:";
...
function resolveSlackModalPluginInteractiveData(params: {
  ...
  if (!params.callbackId.startsWith(DAISYCLAW_MODAL_CALLBACK_PREFIX)) {
    return undefined;
  }
  const callbackData = params.callbackId.slice(DAISYCLAW_MODAL_CALLBACK_PREFIX.length).trim();
...
function shouldHandleSlackModalLifecycleBody(body: unknown): boolean {
  ...
  if (callbackId.startsWith(DAISYCLAW_MODAL_CALLBACK_PREFIX)) {
    return true;
  }
