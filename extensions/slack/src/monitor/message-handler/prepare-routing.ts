...
type SlackRouteBinding = NonNullable<DaisyClawConfig["bindings"]>[number];
type SlackRouteBindingPeer = NonNullable<SlackRouteBinding["match"]["peer"]>;

const slackRouteBindingConfigCache = new WeakMap<
  DaisyClawConfig,
  { bindingsRef: DaisyClawConfig["bindings"]; normalizedCfg: DaisyClawConfig }
>();
...
function normalizeSlackRouteBindingConfig(cfg: DaisyClawConfig): DaisyClawConfig {
  ...
  const normalizedCfg = changed
    ? ({ ...cfg, bindings: normalizedBindings } as DaisyClawConfig)
    : cfg;
