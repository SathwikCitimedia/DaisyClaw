...
import { resolveEnvelopeFormatOptions } from "daisyclaw/plugin-sdk/channel-inbound";
...
describe("resolveSlackThreadContextData", () => {
  const storeFixture = createSlackSessionStoreFixture("daisyclaw-slack-thread-context-");
  ...
  async function resolveAllowlistedThreadContext(params: {
    ...
    const result = await resolveSlackThreadContextData({
      ...
      envelopeOptions: resolveEnvelopeFormatOptions({} as DaisyClawConfig),
