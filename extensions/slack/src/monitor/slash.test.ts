...
describe("Slack native command argument menus", () => {
  ...
  beforeAll(async () => {
    ...
    argMenuHandler = requireHandler(harness.actions, /^daisyclaw_cmdarg/, "arg-menu action");
    argMenuOptionsHandler = requireHandler(harness.options, "daisyclaw_cmdarg", "arg-menu options");
  });
  ...
  it("registers options handlers without losing app receiver binding", async () => {
    ...
    expect(
      Array.from(testHarness.actions.keys()).some(
        (key) => key instanceof RegExp && String(key) === String(/^daisyclaw_cmdarg/),
      ),
    ).toBe(true);
    expect(testHarness.options.has("daisyclaw_cmdarg")).toBe(true);
  ...
  it("shows a button menu when required args are omitted", async () => {
    ...
    expect(actions?.elements?.[0]?.action_id).toBe("daisyclaw_cmdarg_0_0");
    expect(actions?.elements?.[1]?.action_id).toBe("daisyclaw_cmdarg_0_1");
  ...
  it("shows a static_select menu when choices exceed button row size", async () => {
    ...
    expect(element?.action_id).toBe("daisyclaw_cmdarg");
  ...
  it("shows an external_select menu when choices exceed static_select options max", async () => {
    ...
    expect(element?.action_id).toBe("daisyclaw_cmdarg");
    expect(blockId).toContain("daisyclaw_cmdarg_ext:");
    const token = (blockId ?? "").slice("daisyclaw_cmdarg_ext:".length);
