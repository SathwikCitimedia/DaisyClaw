...
import { hasConfiguredSecretInput } from "daisyclaw/plugin-sdk/secret-input";
import {
...
} from "daisyclaw/plugin-sdk/setup-runtime";
import { formatDocsLink } from "daisyclaw/plugin-sdk/setup-tools";
import {
...
function enableSlackAccount(cfg: DaisyClawConfig, accountId: string): DaisyClawConfig {
...
function hasSlackInteractiveRepliesConfig(cfg: DaisyClawConfig, accountId: string): boolean {
...
function createSlackTokenCredential(params: {
  ...
  return {
    ...
    inspect: ({ cfg, accountId }: { cfg: DaisyClawConfig; accountId: string }) => {
    ...
    applyUseEnv: ({ cfg, accountId }: { cfg: DaisyClawConfig; accountId: string }) =>
...
export function createSlackSetupWizardBase(handlers: {
  ...
  return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: DaisyClawConfig; accountId: string }) =>
        resolveSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: DaisyClawConfig; accountId: string }) =>
        Object.entries(resolveSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: DaisyClawConfig; accountId: string }) =>
    ...
    disable: (cfg: DaisyClawConfig) => setSetupChannelEnabled(cfg, channel, false),
