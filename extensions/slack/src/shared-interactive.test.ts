...
describe("buildSlackInteractiveBlocks", () => {
  it("renders shared interactive blocks in authored order", () => {
    expect(
      ...
      {
        type: "actions",
        block_id: "daisyclaw_reply_select_1",
        elements: [
          {
            type: "static_select",
            action_id: "daisyclaw:reply_select:1",
            placeholder: {
              type: "plain_text",
              text: "Pick one",
              emoji: true,
      ...
      {
        type: "actions",
        block_id: "daisyclaw_reply_buttons_1",
        elements: [
          {
            type: "button",
            action_id: "daisyclaw:reply_button:1:1",
            text: {
              type: "plain_text",
              text: "Retry",
              emoji: true,
  ...
  it("preserves original callback payloads for round-tripping", () => {
    ...
    expect(buttonBlock.elements?.[0]?.action_id).toBe("daisyclaw:reply_button:1:1");
    expect(buttonBlock.elements?.[0]?.value).toBe("pluginbind:approval-123:o");
    expect(selectBlock.elements?.[0]?.action_id).toBe("daisyclaw:reply_select:1");
  ...
  it("drops value-only Slack buttons with values beyond Block Kit limits", () => {
    ...
    expect(buttonBlock.elements?.[1]).toEqual({
      type: "button",
      action_id: "daisyclaw:reply_button:1:3",
      text: {
        type: "plain_text",
        text: "Docs",
        emoji: true,
...
describe("resolveSlackReplyBlocks", () => {
  it("offsets legacy interactive blocks after channel and presentation controls", () => {
    ...
    const legacyButtonBlock = blocks?.[2] as
      | { elements?: Array<{ action_id?: string }> }
      | undefined;
    expect(blocks?.[0]?.block_id).toBe("daisyclaw_reply_buttons_1");
    expect(blocks?.[1]?.block_id).toBe("daisyclaw_reply_buttons_2");
    expect(presentationButtonBlock?.elements?.[0]?.action_id).toBe("daisyclaw:reply_button:2:1");
    expect(blocks?.[2]?.block_id).toBe("daisyclaw_reply_buttons_3");
    expect(legacyButtonBlock?.elements?.[0]?.action_id).toBe("daisyclaw:reply_button:3:1");
  });
});
