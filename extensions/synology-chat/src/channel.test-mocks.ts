...
vi.mock("daisyclaw/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<object>("daisyclaw/plugin-sdk/setup");
  return {
    ...actual,
...
vi.mock("daisyclaw/plugin-sdk/channel-config-schema", async () => {
  const actual = await vi.importActual<object>("daisyclaw/plugin-sdk/channel-config-schema");
  return {
    ...actual,
...
vi.mock("daisyclaw/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<object>("daisyclaw/plugin-sdk/webhook-ingress");
  return {
    ...actual,
...
vi.mock("./runtime.js", () => ({
  getSynologyRuntime: vi.fn(() => ({
    ...
    channel: {
      ...
      session: {
        resolveStorePath: vi.fn(() => "/tmp/daisyclaw/synology-chat-sessions.json"),
