...
const synologyChatSetupPlugin = {
  ...
  config: {
    ...
    resolveAllowFrom: ({ cfg, accountId }: { cfg: DaisyClawConfig; accountId?: string }) =>
...
describe("synology-chat core", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.DAISYCLAW_BOT_NAME;
  });
...
describe("synology-chat account resolution", () => {
  ...
  it("returns full defaults for empty config", () => {
    ...
    expect(account.botName).toBe("DaisyClaw");
  });
  ...
  it("uses env var fallbacks", () => {
    ...
    process.env.DAISYCLAW_BOT_NAME = "TestBot";
