...
import { resolvePositiveTimeoutSeconds } from "daisyclaw/plugin-sdk/provider-web-search";
import {
...
export function resolveTavilySearchConfig(cfg?: DaisyClawConfig): TavilySearchConfig {
...
export function resolveTavilyApiKey(cfg?: DaisyClawConfig): string | undefined {
...
export function resolveTavilyBaseUrl(cfg?: DaisyClawConfig): string {
  const search = resolveTavilySearchConfig(cfg);
  const configured =
