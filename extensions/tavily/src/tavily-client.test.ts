...
vi.mock("daisyclaw/plugin-sdk/provider-web-search", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: daisyclaw", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "daisyclaw" });
  });
  ...
  it("runTavilyExtract sends X-Client-Source: daisyclaw", async () => {
