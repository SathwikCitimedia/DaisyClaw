...
function fakeApi(): DaisyClawPluginApi {
  return {
    config: {},
...
describe("tavily tools", () => {
  ...
  it("normalizes optional parameters before invoking Tavily", async () => {
    ...
    const result = await tool.execute("call-1", {
      ...
      include_answer: true,
      time_range: "week",
      include_domains: ["docs.daisyclaw.ai", "", "daisyclaw.ai"],
    ...
    expect(runTavilySearch).toHaveBeenCalledWith({
      ...
      includeAnswer: true,
      timeRange: "week",
      includeDomains: ["docs.daisyclaw.ai", "daisyclaw.ai"],
  ...
  it("late-binds dedicated tools to the resolved runtime config snapshot", async () => {
    ...
    const registeredTools: Array<Parameters<DaisyClawPluginApi["registerTool"]>[0]> = [];
    const registeredOptions: Array<Parameters<DaisyClawPluginApi["registerTool"]>[1]> = [];
    ...
    await searchTool.execute("search-call", { query: "daisyclaw" });
    ...
    expect(searchParams.query).toBe("daisyclaw");
  ...
  it("falls back to environment values and defaults", () => {
    ...
    expect(resolveTavilyBaseUrl({} as DaisyClawConfig)).not.toBe(DEFAULT_TAVILY_BASE_URL);
