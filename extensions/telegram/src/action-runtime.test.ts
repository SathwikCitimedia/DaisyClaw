...
import { resolveStorePath } from "daisyclaw/plugin-sdk/session-store-runtime";
...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): DaisyClawConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as DaisyClawConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): DaisyClawConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  function topicCacheScopeFor(cfg: DaisyClawConfig, accountId: string): string {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["DAISYCLAW_STATE_DIR", "TELEGRAM_BOT_TOKEN"]);
    resetTopicNameCacheForTest();
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as DaisyClawConfig;
    await expect(
  ...
  it("persists sendMessage action deliveries before Telegram platform send", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-telegram-action-durable-"));
    const {
      createOutboundTestPlugin,
      createTestRegistry,
      readQueuedDeliveryEntriesForTest,
      setActivePluginRegistry,
    } = await import("daisyclaw/plugin-sdk/plugin-test-runtime");
  ...
  it("stores created forum topic names in the account-scoped cache", async () => {
    ...
    const cfg = {
      ...
      session: { store: path.join(os.tmpdir(), "daisyclaw-telegram-action-sessions.json") },
    } as DaisyClawConfig;
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: DaisyClawConfig, accountId = "media") {
    await handleTelegramAction(
