...
import { parseExecApprovalCommandText } from "daisyclaw/plugin-sdk/approval-reply-runtime";
import { resolveChannelConfigWrites } from "daisyclaw/plugin-sdk/channel-config-helpers";
import {
  buildMentionRegexes,
...
} from "daisyclaw/plugin-sdk/channel-inbound-debounce";
import { resolveStoredModelOverride } from "daisyclaw/plugin-sdk/command-auth-native";
import { hasControlCommand } from "daisyclaw/plugin-sdk/command-detection";
import { isAbortRequestText } from "daisyclaw/plugin-sdk/command-primitives-runtime";
import { buildCommandsMessagePaginated } from "daisyclaw/plugin-sdk/command-status";
import type { DmPolicy, DaisyClawConfig } from "daisyclaw/plugin-sdk/config-contracts";
import type {
  TelegramGroupConfig,
  TelegramTopicConfig,
} from "daisyclaw/plugin-sdk/config-contracts";
import { mutateConfigFile } from "daisyclaw/plugin-sdk/config-mutation";
import {
...
import { applyModelOverrideToSessionEntry } from "daisyclaw/plugin-sdk/model-session-runtime";
import { formatModelsAvailableHeader } from "daisyclaw/plugin-sdk/models-provider-runtime";
...
import { resolveThreadSessionKeys } from "daisyclaw/plugin-sdk/routing";
import { danger, logVerbose, warn } from "daisyclaw/plugin-sdk/runtime-env";
import {
  loadSessionStore,
        ...
        if (modelCallback.type === "select") {
          ...
          try {
            ...
            const scopeText = isDefaultSelection
              ...
              : `Session-only model selection. Runtime unchanged. Use /model ${escapeHtml(selection.provider)}/${escapeHtml(selection.model)} --runtime &lt;runtime&gt; to switch harnesses. The agent default in daisyclaw.json is unchanged; /reset or a new session may return to that default.`;
            await editMessageWithButtons(
              `✅ Model ${actionText}\n\n${scopeText}`,
