...
describe("buildTelegramMessageContext thread binding override", () => {
  ...
  it("bypasses mention gating for bound forum topic messages", async () => {
    resolveTelegramConversationRouteMock.mockReturnValue(
      createBoundRoute({
        accountId: "default",
        sessionKey: "plugin-binding:daisyclaw-codex-app-server:session-1",
        agentId: "main",
    ...
    expect(ctx?.ctxPayload?.SessionKey).toBe("plugin-binding:daisyclaw-codex-app-server:session-1");
  });
