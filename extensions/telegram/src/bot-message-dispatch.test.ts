...
const getAgentScopedMediaLocalRoots = vi.hoisted(() =>
  vi.fn((_cfg: unknown, agentId: string) => [`/tmp/.daisyclaw/workspace-${agentId}`]),
);
...
vi.mock("daisyclaw/plugin-sdk/channel-outbound", async (importOriginal) => {
  const actual = await importOriginal<typeof import("daisyclaw/plugin-sdk/channel-outbound")>();
  return {
    ...actual,
...
describe("dispatchTelegramMessage draft streaming", () => {
  ...
  function createContext(overrides?: Partial<TelegramMessageContext>): TelegramMessageContext {
    ...
    base.turn = {
      storePath: "/tmp/daisyclaw/telegram-sessions.json",
  ...
  it("streams drafts in private threads and forwards thread id", async () => {
    ...
    expect(mediaLocalRoots?.some((root) => /[\\/]\.daisyclaw[\\/]workspace-work$/u.test(root))).toBe(
      true,
    );
  ...
  it("records streamed final replies into the prompt context cache", async () => {
    const storePath = `/tmp/daisyclaw-telegram-stream-context-${process.pid}-${Date.now()}.json`;
