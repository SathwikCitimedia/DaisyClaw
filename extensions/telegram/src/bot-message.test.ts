...
describe("telegram bot message processor", () => {
  ...
  function createMessageContext(context: Record<string, unknown> = {}) {
    return {
      ...
      primaryCtx: { me: { username: "daisyclaw_bot" } },
  ...
  it("dispatches when context is available", async () => {
    ...
    expect(telegramInboundInfo).toHaveBeenCalledWith(
      "Inbound message telegram:123 -> @daisyclaw_bot (direct, 11 chars)",
    );
  });
  ...
  it("formats Telegram inbound summaries without message content", () => {
    ...
    ).toBe("Inbound message telegram:123 -> @daisyclaw_bot (direct, 14 chars)");
    expect(
      formatTelegramInboundLogLine({
    ...
    ).toBe("Inbound message telegram:group:-100 -> @daisyclaw_bot (group, image/jpeg, 13 chars)");
  });
