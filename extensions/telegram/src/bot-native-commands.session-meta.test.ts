...
type ResolveConfiguredBindingRouteFn =
  typeof import("daisyclaw/plugin-sdk/conversation-runtime").resolveConfiguredBindingRoute;
type EnsureConfiguredBindingRouteReadyFn =
  typeof import("daisyclaw/plugin-sdk/conversation-runtime").ensureConfiguredBindingRouteReady;
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("daisyclaw/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
type LoadModelCatalogFn = typeof import("daisyclaw/plugin-sdk/agent-runtime").loadModelCatalog;
...
vi.mock("daisyclaw/plugin-sdk/conversation-runtime", async () => {
  const actual = await vi.importActual<typeof import("daisyclaw/plugin-sdk/conversation-runtime")>(
    "daisyclaw/plugin-sdk/conversation-runtime",
  );
  return {
    ...actual,
...
vi.mock("daisyclaw/plugin-sdk/session-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("daisyclaw/plugin-sdk/session-store-runtime")>(
    "daisyclaw/plugin-sdk/session-store-runtime",
  );
  return {
    ...actual,
...
vi.mock("daisyclaw/plugin-sdk/command-auth-native", async () => {
  const actual = await vi.importActual<typeof import("daisyclaw/plugin-sdk/command-auth-native")>(
    "daisyclaw/plugin-sdk/command-auth-native",
  );
...
vi.mock("daisyclaw/plugin-sdk/plugin-runtime", async () => {
  const actual = await vi.importActual<typeof import("daisyclaw/plugin-sdk/plugin-runtime")>(
    "daisyclaw/plugin-sdk/plugin-runtime",
  );
  return {
    ...actual,
...
describe("registerTelegramNativeCommands — session metadata", () => {
  ...
  beforeEach(() => {
    ...
    sessionMocks.resolveAndPersistSessionFile.mockClear().mockImplementation(async (params) => {
      const sessionFile =
        params.fallbackSessionFile ?? `/tmp/daisyclaw-sessions/${params.sessionId}.jsonl`;
      return {
        sessionFile,
        sessionEntry: {
          ...params.sessionEntry,
    ...
    sessionMocks.resolveStorePath.mockClear().mockReturnValue("/tmp/daisyclaw-sessions.json");
  ...
  it("uses the target session model when building native argument menus", async () => {
    ...
    expect(sessionMocks.loadSessionStore).toHaveBeenCalledWith("/tmp/daisyclaw-sessions.json");
    expectSendMessageCall({
      sendMessage,
      chatId: 100,
  ...
  it("passes a persisted topic session file to plugin commands", async () => {
    sessionMocks.resolveStorePath.mockReturnValue("/tmp/daisyclaw-sessions/sessions.json");
    sessionMocks.loadSessionStore.mockReturnValue({
    ...
    const { handler } = registerAndResolveCommandHandler({
      commandName: "codex",
      cfg: { commands: { allowFrom: { telegram: ["200"] } } } as DaisyClawConfig,
    ...
    expectRecordFields(
      firstMockArg(sessionMocks.resolveAndPersistSessionFile, "resolveAndPersistSessionFile"),
      {
        ...
        storePath: "/tmp/daisyclaw-sessions/sessions.json",
        sessionsDir: "/tmp/daisyclaw-sessions",
        fallbackSessionFile: path.resolve("/tmp/daisyclaw-sessions", "sess-topic-topic-42.jsonl"),
      },
    ...
    expectRecordFields(
      (pluginRuntimeMocks.executePluginCommand.mock.calls as unknown as Array<[unknown]>)[0]?.[0],
      {
        sessionKey: "agent:main:telegram:group:-1001234567890:topic:42",
        sessionId: "sess-topic",
        sessionFile: path.resolve("/tmp/daisyclaw-sessions", "sess-topic-topic-42.jsonl"),
