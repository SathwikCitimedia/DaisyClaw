...
import { listSkillCommandsForAgents as listActualSkillCommandsForAgents } from "daisyclaw/plugin-sdk/skill-commands-runtime";
...
describe("registerTelegramNativeCommands skill allowlist integration", () => {
  ...
  it("registers only allowlisted skills for the bound agent menu", async () => {
    const workspaceDir = await makeWorkspace("daisyclaw-telegram-skills-");
    await writeSkill({
    ...
    listSkillCommandsForAgents.mockImplementation(
      ({ cfg: cfgLocal, agentIds }: { cfg: DaisyClawConfig; agentIds?: string[] }) =>
