...
import { buildChannelInboundEventContext } from "daisyclaw/plugin-sdk/channel-inbound";
...
type GetRuntimeConfigFn =
  typeof import("daisyclaw/plugin-sdk/runtime-config-snapshot").getRuntimeConfig;
type LoadSessionStoreFn =
  typeof import("daisyclaw/plugin-sdk/session-store-runtime").loadSessionStore;
type ResolveStorePathFn =
  typeof import("daisyclaw/plugin-sdk/session-store-runtime").resolveStorePath;
type ReadSessionUpdatedAtFn =
  typeof import("daisyclaw/plugin-sdk/session-store-runtime").readSessionUpdatedAt;
...
const { sessionStorePath } = vi.hoisted(() => {
  ...
  return {
    sessionStorePath: `${tempRoot.replace(/[\\/]+$/u, "")}${separator}daisyclaw-telegram-${
      process.pid
...
vi.mock("daisyclaw/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
...
function resolveDefaultModelForAgentForTest(params: { cfg: DaisyClawConfig }): {
  provider: string;
  model: string;
} {
...
function createModelsProviderDataFromConfig(cfg: DaisyClawConfig): {
...
const DEFAULT_TELEGRAM_TEST_CONFIG: DaisyClawConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: DaisyClawConfig) => {
