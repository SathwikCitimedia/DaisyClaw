...
import { escapeRegExp, formatEnvelopeTimestamp } from "daisyclaw/plugin-sdk/channel-test-helpers";
import type { TelegramGroupConfig } from "daisyclaw/plugin-sdk/config-contracts";
import type { GetReplyOptions, MsgContext } from "daisyclaw/plugin-sdk/reply-runtime";
import { withEnvAsync } from "daisyclaw/plugin-sdk/test-env";
import { sanitizeTerminalText } from "daisyclaw/plugin-sdk/test-fixtures";
...
const pluginStateTestRuntime = await import("daisyclaw/plugin-sdk/plugin-state-test-runtime");
const conversationRuntime = await import("daisyclaw/plugin-sdk/conversation-runtime");
const configMutation = await import("daisyclaw/plugin-sdk/config-mutation");
const sessionStoreRuntime = await import("daisyclaw/plugin-sdk/session-store-runtime");
...
function createTelegramBotTestStateDir(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "daisyclaw-telegram-bot-"));
...
describe("createTelegramBot", () => {
  ...
  afterEach(() => {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.DAISYCLAW_STATE_DIR = previousStateDir;
    }
  ...
  beforeEach(async () => {
    previousStateDir = process.env.DAISYCLAW_STATE_DIR;
    process.env.DAISYCLAW_STATE_DIR = createTelegramBotTestStateDir();
    resetTelegramForumFlagCacheForTest();
  ...
  it("passes startup probe botInfo to grammY", () => {
    const botInfo = {
      id: 123456,
      is_bot: true,
      first_name: "DaisyClaw",
      username: "daisyclaw_bot",
  ...
  it.each(["stop", "/stop@daisyclaw_bot"] as const)(
    ...
    try {
      ...
      await runTelegramMiddlewareChain({
        ctx: {
          ...
          message: {
            chat: { id: -1007, type: "supergroup", title: "DaisyClaw Ops" },
            text: "first",
            date: 1736380804,
            message_id: 104,
            from: { id: 42, first_name: "Ada" },
  ...
  it("handles pairing DM flows for new and already-pending requests", async () => {
    ...
    for (const [index, testCase] of cases.entries()) {
      ...
      expect(pairingText, testCase.name).toContain("daisyclaw pairing approve telegram");
      expectRecordFields(
  ...
  it("ignores private self-authored message updates instead of issuing a pairing challenge", async () => {
    ...
    await handler({
      message: {
        ...
        message_id: 1884,
        date: 1736380800,
        from: { id: 7, is_bot: true, first_name: "DaisyClaw", username: "daisyclaw_bot" },
        pinned_message: {
          message_id: 1883,
          date: 1736380799,
          chat: { id: 1234, type: "private", first_name: "Harold" },
          from: { id: 7, is_bot: true, first_name: "DaisyClaw", username: "daisyclaw_bot" },
          text: "Binding: Review pull request 54118 (daisyclaw)",
        },
      },
      me: { id: 7, is_bot: true, first_name: "DaisyClaw", username: "daisyclaw_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  ...
  it("ignores group self-authored message updates instead of re-processing bot output", async () => {
    ...
    await handler({
      message: {
        chat: { id: -1001234, type: "supergroup", title: "DaisyClaw Ops" },
        message_id: 1884,
        date: 1736380800,
        from: { id: 7, is_bot: true, first_name: "DaisyClaw", username: "daisyclaw_bot" },
  ...
  it("reloads topic agent overrides between messages without recreating the bot", async () => {
    ...
    const sendTopicMessage = async (chatId: number, messageId: number, text: string) => {
      await handler({
        ...
        me: { username: "daisyclaw_bot", has_topics_enabled: true },
        getFile: async () => ({ download: async () => new Uint8Array() }),
      });
    };
  ...
  async function dispatchMessage(params: {
    ...
    await handler({
      ...
      me: params.me ?? { username: "daisyclaw_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  }
  ...
  it("honors routed group activation from session store", async () => {
    const storePath = "/tmp/daisyclaw-telegram-group-activation.json";
