...
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("daisyclaw/plugin-sdk/reply-runtime").dispatchReplyWithBufferedBlockDispatcher;
type DispatchReplyHarnessParams = Parameters<DispatchReplyWithBufferedBlockDispatcherFn>[0];
type ReadRemoteMediaBufferFn =
  typeof import("daisyclaw/plugin-sdk/media-runtime").readRemoteMediaBuffer;
...
function ensureMediaHarnessStoreRoot(): string {
  mediaHarnessStoreRoot ??= mkdtempSync(path.join(os.tmpdir(), "daisyclaw-telegram-media-e2e-"));
...
const defaultRuntimeConfig = (() =>
  ...
  }) as DaisyClawConfig) as TelegramBotDeps["getRuntimeConfig"];
