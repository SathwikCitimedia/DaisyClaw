...
import type { MsgContext } from "daisyclaw/plugin-sdk/reply-runtime";
import { loadSessionStore } from "daisyclaw/plugin-sdk/session-store-runtime";
import { mockPinnedHostnameResolution } from "daisyclaw/plugin-sdk/test-env";
...
async function loadEnvelopeTimestampHelpers() {
  return await import("daisyclaw/plugin-sdk/channel-test-helpers");
}
...
describe("createTelegramBot", () => {
  ...
  it("blocks DM model-selection callbacks for unpaired users when inline buttons are DM-scoped", async () => {
    ...
    const storePath = `/tmp/daisyclaw-telegram-callback-authz-${process.pid}-${Date.now()}.json`;
  ...
  it("blocks group model-selection callbacks for senders who are not authorized for /models", async () => {
    ...
    const storePath = `/tmp/daisyclaw-telegram-group-model-authz-${process.pid}-${Date.now()}.json`;
  ...
  it("recomputes group model-selection callback auth from runtime command config", async () => {
    ...
    const storePath = `/tmp/daisyclaw-telegram-group-model-authz-runtime-${process.pid}-${Date.now()}.json`;
  ...
  it("routes compact model callbacks against the configured provider", async () => {
    ...
    const storePath = `/tmp/daisyclaw-telegram-model-compact-${process.pid}-${Date.now()}.json`;
  ...
  it("resets overrides when selecting the configured default model", async () => {
    ...
    const storePath = `/tmp/daisyclaw-telegram-model-default-${process.pid}-${Date.now()}.json`;
    const config: DaisyClawConfig = {
      agents: {
        defaults: {
  ...
  it("formats non-default model selection confirmations with Telegram HTML parse mode", async () => {
    ...
    const storePath = `/tmp/daisyclaw-telegram-model-html-${process.pid}-${Date.now()}.json`;
    ...
    try {
      ...
      expect(editCall[2]).toBe(
        `${CHECK_MARK_EMOJI} Model changed to <b>openai/gpt-5.4</b>\n\nSession-only model selection. Runtime unchanged. Use /model openai/gpt-5.4 --runtime &lt;runtime&gt; to switch harnesses. The agent default in daisyclaw.json is unchanged; /reset or a new session may return to that default.`,
      );
  ...
  it("persists non-default model override using fresh config, not stale startup snapshot", async () => {
    ...
    const storePath = `/tmp/daisyclaw-telegram-model-fresh-cfg-${process.pid}-${Date.now()}.json`;
  ...
  it("adds live chat and reply-target windows for stale group replies", async () => {
    ...
    const handler = getOnHandler("message") as (ctx: Record<string, unknown>) => Promise<void>;
    const baseCtx = {
      me: { id: 999, username: "daisyclaw_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    };
    ...
    try {
      ...
      await handler({
        ...
        message: {
          ...
          reply_to_message: {
            ...
            date: 1736380700,
            from: { id: 999, is_bot: true, first_name: "DaisyClaw" },
      ...
      await handler({
        ...
        message: {
          chat,
          message_id: 103,
          text: "@daisyclaw_bot explain what went wrong",
          date: 1736380800,
    ...
    await handler({
      message: {
        ...
        reply_to_message: {
          message_id: 42,
          text: "original reply",
          from: { id: 999, first_name: "DaisyClaw" },
        },
      },
      me: { id: 999, username: "daisyclaw_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
