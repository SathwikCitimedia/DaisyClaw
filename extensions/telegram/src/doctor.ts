...
function formatTelegramAccountConfigPath(cfg: DaisyClawConfig, accountId: string): string {
  const telegram = asObjectRecord((cfg.channels as Record<string, unknown> | undefined)?.telegram);
  const accounts = asObjectRecord(telegram?.accounts);
...
export function maybeRepairTelegramApiRoots(cfg: DaisyClawConfig): {
...
async function repairTelegramConfig(params: { cfg: DaisyClawConfig }): Promise<{
  config: DaisyClawConfig;
  changes: string[];
}> {
...
export async function maybeRepairTelegramAllowFromUsernames(cfg: DaisyClawConfig): Promise<{
  config: DaisyClawConfig;
  changes: string[];
}> {
  ...
  const { getChannelsCommandSecretTargetIds, resolveCommandSecretRefsViaGateway } =
    await import("daisyclaw/plugin-sdk/runtime");
