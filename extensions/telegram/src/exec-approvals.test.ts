...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-telegram-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<DaisyClawConfig["channels"]>["telegram"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<DaisyClawConfig["channels"]>["telegram"]>>,
): DaisyClawConfig {
  return {
    channels: {
      telegram: {
