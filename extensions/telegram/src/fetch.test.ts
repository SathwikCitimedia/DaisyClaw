...
import { resolveFetch } from "daisyclaw/plugin-sdk/fetch-runtime";
import { MAX_DATE_TIMESTAMP_MS } from "daisyclaw/plugin-sdk/number-runtime";
...
beforeEach(() => {
  ...
  for (const key of [
    ...
    "no_proxy",
    "DAISYCLAW_PROXY_URL",
    "DAISYCLAW_PROXY_ACTIVE",
    "DAISYCLAW_PROXY_CA_FILE",
  ]) {
    vi.stubEnv(key, "");
  }
...
function writeTempCa(contents: string): string {
  const dir = mkdtempSync(path.join(os.tmpdir(), "daisyclaw-telegram-proxy-ca-"));
  tempDirs.push(dir);
...
describe("resolveTelegramFetch", () => {
  ...
  it("uses the DaisyClaw debug proxy URL when no explicit proxy fetch is provided", async () => {
    vi.stubEnv("DAISYCLAW_DEBUG_PROXY_ENABLED", "1");
    vi.stubEnv("DAISYCLAW_DEBUG_PROXY_URL", "http://127.0.0.1:7777");
    undiciFetch.mockResolvedValue({ ok: true } as Response);
  ...
  it("uses DAISYCLAW_PROXY_URL as a Telegram explicit proxy when proxy env is absent", async () => {
    vi.stubEnv("DAISYCLAW_PROXY_URL", "http://127.0.0.1:7788");
    undiciFetch.mockResolvedValue({ ok: true } as Response);
    ...
    expect(ProxyAgentCtor).toHaveBeenCalledTimes(1);
    const proxyOptions = constructorOptions(ProxyAgentCtor, "DaisyClaw proxy") as {
      allowH2?: boolean;
      uri?: string;
  ...
  it("preserves caller-provided custom fetch when DAISYCLAW_PROXY_URL is present", async () => {
    vi.stubEnv("DAISYCLAW_PROXY_URL", "http://127.0.0.1:7788");
    const proxyFetch = vi.fn(async () => ({ ok: true }) as Response) as unknown as typeof fetch;
  ...
  it("prefers standard proxy env over DAISYCLAW_PROXY_URL for Telegram", async () => {
    vi.stubEnv("DAISYCLAW_PROXY_URL", "http://127.0.0.1:7788");
    vi.stubEnv("https_proxy", "http://127.0.0.1:7890");
    undiciFetch.mockResolvedValue({ ok: true } as Response);
  ...
  it("keeps resolver-scoped transport policy for DaisyClaw proxy fetches", async () => {
