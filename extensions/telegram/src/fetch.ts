...
} from "daisyclaw/plugin-sdk/fetch-runtime";
import {
...
import { resolveRequestUrl } from "daisyclaw/plugin-sdk/request-url";
...
// fix for the leak observed in daisyclaw#68128 is the transport lifecycle that
...
function resolveDaisyClawProxyUrlForTelegram(
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const proxyUrl = env.DAISYCLAW_PROXY_URL?.trim();
...
export function resolveTelegramTransport(
  ...
  const managedProxyUrl =
    !effectiveProxyFetch && !hasEnvProxy ? resolveDaisyClawProxyUrlForTelegram() : undefined;
