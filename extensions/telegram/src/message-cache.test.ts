...
describe("telegram message cache", () => {
  ...
  it("parses legacy sidecar records for doctor migration only", async () => {
    const storePath = `/tmp/daisyclaw-telegram-message-cache-legacy-${process.pid}-${Date.now()}.json`;
  ...
  it("does not select messages before the latest session reset command", async () => {
    ...
    const staleInstruction = "okay so we just flip in daisyclaw? if yes do it up";
    const record = (params: {
      id: number;
      text: string;
      timestampMs: number;
