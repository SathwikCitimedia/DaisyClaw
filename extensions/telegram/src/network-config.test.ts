...
import type { TelegramNetworkConfig } from "daisyclaw/plugin-sdk/config-contracts";
...
let isWSL2Sync: typeof import("daisyclaw/plugin-sdk/runtime-env").isWSL2Sync;
...
async function loadModule() {
  ({ isWSL2Sync } = await import("daisyclaw/plugin-sdk/runtime-env"));
  ({
  ...
  it.each([
    {
      ...
      expected: {
        value: true,
        source: "env:DAISYCLAW_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY",
      },
    },
    {
      name: "uses env disable when set",
      env: { DAISYCLAW_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY: "1" },
      expected: {
        value: false,
        source: "env:DAISYCLAW_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY",
      },
    },
    {
      name: "prefers env enable over config",
      env: { DAISYCLAW_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY: "1" },
  ...
  it.each([
    {
      ...
      env: { DAISYCLAW_TELEGRAM_DNS_RESULT_ORDER: "verbatim" },
      nodeMajor: 22,
      expected: {
        value: "verbatim",
        source: "env:DAISYCLAW_TELEGRAM_DNS_RESULT_ORDER",
      },
    },
    {
      name: "normalizes trimmed env values",
      env: { DAISYCLAW_TELEGRAM_DNS_RESULT_ORDER: "  IPV4FIRST  " },
      nodeMajor: 20,
      expected: {
        value: "ipv4first",
        source: "env:DAISYCLAW_TELEGRAM_DNS_RESULT_ORDER",
    ...
    },
    {
      ...
      env: { DAISYCLAW_TELEGRAM_DNS_RESULT_ORDER: "bogus" },
      network: { dnsResultOrder: "ipv4first" },
      nodeMajor: 20,
      expected: { value: "ipv4first", source: "config" },
    },
