...
import {
  ...
  type DaisyClawStateKyselyDatabaseForTests,
...
vi.mock("daisyclaw/plugin-sdk/delivery-queue-runtime", () => ({
...
type TelegramPollingTestDatabase = Pick<
  DaisyClawStateKyselyDatabaseForTests,
...
function openTelegramSpoolTestKysely(spoolDir: string) {
  const database = openDaisyClawStateDatabase({
    env: { ...process.env, DAISYCLAW_STATE_DIR: spoolDir },
  });
  return {
    database,
...
async function withTempSpool<T>(fn: (spoolDir: string) => Promise<T>): Promise<T> {
  const spoolDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-telegram-spool-"));
  try {
...
describe("TelegramPollingSession", () => {
  ...
  beforeEach(() => {
    ...
    installTelegramIngressQueueRuntime(() =>
      path.join(os.tmpdir(), "daisyclaw-telegram-test-state"),
    );
  });
  ...
  it("does not call getUpdates for offset confirmation (avoiding 409 conflicts)", async () => {
    ...
    // DaisyClaw middleware still skips duplicates using the persisted update offset.
  ...
  it("initializes the main-thread bot before draining isolated ingress spool", async () => {
    const abort = new AbortController();
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-telegram-spool-"));
  ...
  it("restarts isolated ingress on a getUpdates conflict instead of crashing the account", async () => {
    ...
    try {
      ...
      expectLogIncludes(log, "Another DaisyClaw gateway, script, or Telegram poller");
      expect(
        statusPatches(setStatus).some(
          (patch) =>
            patch.connected === false &&
            String(patch.lastError).includes("Another DaisyClaw gateway"),
        ),
      ).toBe(true);
    } finally {
      abort.abort();
  ...
  it("logs an actionable duplicate-poller hint for getUpdates conflicts", async () => {
    ...
    expect(
      statusPatches(setStatus).some(
        (patch) =>
          patch.connected === false && String(patch.lastError).includes("Another DaisyClaw gateway"),
      ),
    ).toBe(true);
  });
