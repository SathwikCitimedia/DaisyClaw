...
describe("createTelegramSendChatActionHandler", () => {
  ...
  it("suspends after maxConsecutive401 failures", async () => {
    ...
    expect(logger.mock.calls.at(-1)).toEqual([
      "CRITICAL: sendChatAction suspended after 3 consecutive 401 errors. Bot token is likely invalid. Telegram may DELETE the bot if requests continue. Replace the token and restart: daisyclaw channels restart telegram",
    ]);
