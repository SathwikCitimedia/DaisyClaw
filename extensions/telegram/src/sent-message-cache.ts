...
const TELEGRAM_SENT_MESSAGES_STATE_KEY = Symbol.for("daisyclaw.telegramSentMessagesState");
const TELEGRAM_SENT_MESSAGES_STORE_FOR_TEST_KEY = Symbol.for(
  "daisyclaw.telegramSentMessagesStoreForTest",
);
...
function resolveSentMessageStorePath(cfg?: Pick<DaisyClawConfig, "session">): string {
  return `${resolveStorePath(cfg?.session?.store)}.telegram-sent-messages.json`;
}

function resolveSentMessageScopeKey(cfg?: Pick<DaisyClawConfig, "session">): string {
...
function getSentMessageBucket(cfg?: Pick<DaisyClawConfig, "session">): SentMessageBucket {
...
function getSentMessages(cfg?: Pick<DaisyClawConfig, "session">): SentMessageStore {
...
export function recordSentMessage(
  ...
  messageId: number,
  cfg?: Pick<DaisyClawConfig, "session">,
): void {
