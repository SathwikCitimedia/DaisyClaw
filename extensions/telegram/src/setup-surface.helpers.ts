...
export function shouldShowTelegramDmAccessWarning(cfg: DaisyClawConfig, accountId: string): boolean {
...
export function buildTelegramDmAccessWarningLines(accountId: string): string[] {
  ...
  return [
    ...
    "  " + formatCliCommand(`daisyclaw config set ${configBase}.dmPolicy "allowlist"`),
    "  " + formatCliCommand(`daisyclaw config set ${configBase}.allowFrom '["YOUR_USER_ID"]'`),
