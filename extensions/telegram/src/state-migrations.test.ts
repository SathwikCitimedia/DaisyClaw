...
import { resolvePersistentDedupePluginStateNamespace } from "daisyclaw/plugin-sdk/persistent-dedupe";
import {
...
describe("telegram state migrations", () => {
  it("detects legacy bot-info cache import", async () => {
    const dir = await mkdtemp(path.join(os.tmpdir(), "daisyclaw-telegram-state-migration-"));
    const env = { ...process.env, DAISYCLAW_STATE_DIR: dir };
