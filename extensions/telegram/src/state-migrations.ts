...
import type { ChannelLegacyStateMigrationPlan } from "daisyclaw/plugin-sdk/channel-contract";
import { resolveChannelAllowFromPath } from "daisyclaw/plugin-sdk/channel-pairing";
...
} from "daisyclaw/plugin-sdk/persistent-dedupe";
import { createPluginStateSyncKeyedStore } from "daisyclaw/plugin-sdk/runtime-doctor";
import { statRegularFileSync } from "daisyclaw/plugin-sdk/security-runtime";
...
function detectTelegramMessageDispatchLegacyStateMigration(params: {
  ...
  const storePath = resolveStorePath(params.cfg.session?.store, { env: params.env });
  const legacyStorePath = resolveLegacySessionStorePath(params);
  const env = params.stateDir ? { ...params.env, DAISYCLAW_STATE_DIR: params.stateDir } : params.env;
