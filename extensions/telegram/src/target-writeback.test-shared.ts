...
vi.mock("daisyclaw/plugin-sdk/config-mutation", async () => {
  const actual = await vi.importActual<typeof import("daisyclaw/plugin-sdk/config-mutation")>(
    "daisyclaw/plugin-sdk/config-mutation",
  );
  return {
    ...actual,
...
vi.mock("daisyclaw/plugin-sdk/cron-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("daisyclaw/plugin-sdk/cron-store-runtime")>(
    "daisyclaw/plugin-sdk/cron-store-runtime",
  );
  return {
    ...actual,
    loadCronStore,
...
export function installMaybePersistResolvedTelegramTargetTests(params?: {
  ...
  describe("maybePersistResolvedTelegramTarget", () => {
    ...
    it("writes back matching config and cron targets", async () => {
      ...
      expect(writeOptions.expectedConfigPath).toBe("/tmp/daisyclaw.json");
      expect(saveCronStore).toHaveBeenCalledTimes(1);
