// Tlon plugin entrypoint registers its DaisyClaw integration.
import type { ReplyPayload } from "daisyclaw/plugin-sdk/reply-runtime";
import type { RuntimeEnv } from "daisyclaw/plugin-sdk/runtime";
import { asFiniteNumber } from "daisyclaw/plugin-sdk/string-coerce-runtime";
...
export async function monitorTlonProvider(opts: MonitorTlonOpts = {}): Promise<void> {
  ...
  const cfg = core.config.current() as DaisyClawConfig;
      ...
      if (senders.size > 0 && !senders.has(senderShip)) {
        runtime.log?.(
          `[tlon] ⚠️ SECURITY: Multiple users sharing DM session. ` +
            `Configure "session.dmScope: per-channel-peer" in DaisyClaw config.`,
        );
        ...
        if (!sharedSessionWarningSent && effectiveOwnerShip) {
          ...
          const warningMsg =
            ...
            `Fix: Add to your DaisyClaw config:\n` +
            `session:\n  dmScope: "per-channel-peer"\n\n` +
            `Docs: https://docs.daisyclaw.ai/concepts/session#secure-dm-mode`;
