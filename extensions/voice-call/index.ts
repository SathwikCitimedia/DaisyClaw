// Voice Call plugin entrypoint registers its DaisyClaw integration.
import { formatErrorMessage } from "daisyclaw/plugin-sdk/error-runtime";
import { ErrorCodes, errorShape } from "daisyclaw/plugin-sdk/gateway-runtime";
import { timestampMsToIsoString } from "daisyclaw/plugin-sdk/number-runtime";
...
const voiceCallConfigSchema = {
  ...
  uiHints: {
    ...
    "realtime.toolPolicy": {
      ...
      help: "Controls the shared daisyclaw_agent_consult tool.",
      advanced: true,
    },
    "realtime.consultPolicy": {
      label: "Realtime Consult Policy",
      help: "Guides when the realtime voice model should call daisyclaw_agent_consult.",
      advanced: true,
    },
...
const VoiceCallToolSchema = Type.Union([
  Type.Object({
    ...
    sessionKey: Type.Optional(Type.String({ description: "DaisyClaw session key for the call" })),
    requesterSessionKey: Type.Optional(
      Type.String({ description: "DaisyClaw session key that initiated the call" }),
    ),
...
function isCliOnlyProcess(): boolean {
  return process.env.DAISYCLAW_CLI === "1" && !process.argv.slice(2).includes("gateway");
}

const VOICE_CALL_RUNTIME_KEY = Symbol.for("daisyclaw.voice-call.runtime");
const VOICE_CALL_RUNTIME_PROMISE_KEY = Symbol.for("daisyclaw.voice-call.runtimePromise");
const VOICE_CALL_RUNTIME_STOP_PROMISE_KEY = Symbol.for("daisyclaw.voice-call.runtimeStopPromise");
