...
async function createWorkspace(): Promise<string> {
  const workspaceDir = await mkdtemp(path.join(tmpdir(), "daisyclaw-voice-context-"));
...
describe("buildRealtimeVoiceInstructions", () => {
  it("injects bounded identity and workspace context", async () => {
    ...
    expect(instructions).toContain("DaisyClaw agent voice context:");
    expect(instructions).toContain("Consult behavior:");
    expect(instructions).toContain("Call daisyclaw_agent_consult before answering requests");
