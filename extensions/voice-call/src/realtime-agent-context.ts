...
import { buildRealtimeVoiceAgentConsultPolicyInstructions } from "daisyclaw/plugin-sdk/realtime-voice";
import { root } from "daisyclaw/plugin-sdk/security-runtime";
import { normalizeOptionalString as normalizeString } from "daisyclaw/plugin-sdk/string-coerce-runtime";
...
export async function buildRealtimeVoiceInstructions(params: {
  ...
  const capsule: string[] = [
    "DaisyClaw agent voice context:",
    `- Agent id: ${agentId}`,
    "- Use this context to match the DaisyClaw agent's personality and standing preferences on fast voice turns.",
    "- Treat this as compact context only; call daisyclaw_agent_consult when the caller needs the full agent brain, tools, memory, or workspace state.",
  ];

  if (contextConfig.includeIdentity) {
    const identity = params.agentRuntime.resolveAgentIdentity(
      params.coreConfig as DaisyClawConfig,
      agentId,
