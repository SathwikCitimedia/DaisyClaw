...
describe("WhatsApp auth dir profile resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["DAISYCLAW_STATE_DIR", "DAISYCLAW_OAUTH_DIR"]);
    tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-wa-profile-"));
  ...
  it("resolves the default web auth dir from DAISYCLAW_STATE_DIR at call time", async () => {
