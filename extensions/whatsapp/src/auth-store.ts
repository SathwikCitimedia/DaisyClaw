...
import { info, success } from "daisyclaw/plugin-sdk/runtime-env";
import { getChildLogger } from "daisyclaw/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "daisyclaw/plugin-sdk/runtime-env";
...
export async function pickWebChannel(
  ...
  if (!auth.exists) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("daisyclaw channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
