...
import { resetInboundDedupe } from "daisyclaw/plugin-sdk/reply-runtime";
...
const WEB_AUTO_REPLY_SOCKETS_KEY = Symbol.for("daisyclaw:webAutoReplySessionSockets");
...
export function installWebAutoReplyTestHomeHooks() {
  beforeAll(async () => {
    tempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-web-home-suite-"));
  });
...
export async function makeSessionStore(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-session-"));
