...
import { resolveAccountEntry } from "daisyclaw/plugin-sdk/account-core";
import { CHANNEL_APPROVAL_NATIVE_RUNTIME_CONTEXT_CAPABILITY } from "daisyclaw/plugin-sdk/approval-handler-runtime";
import { resolveInboundDebounceMs } from "daisyclaw/plugin-sdk/channel-inbound-debounce";
import { registerChannelRuntimeContext } from "daisyclaw/plugin-sdk/channel-runtime-context";
import { formatCliCommand } from "daisyclaw/plugin-sdk/cli-runtime";
import { isControlCommandMessage } from "daisyclaw/plugin-sdk/command-detection";
import { drainPendingDeliveries } from "daisyclaw/plugin-sdk/delivery-queue-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "daisyclaw/plugin-sdk/reply-history";
import { resolveAgentRoute } from "daisyclaw/plugin-sdk/routing";
import { logVerbose } from "daisyclaw/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "daisyclaw/plugin-sdk/runtime-env";
import { getChildLogger } from "daisyclaw/plugin-sdk/runtime-env";
import {
  defaultRuntime,
  formatDurationPrecise,
  warn,
  type RuntimeEnv,
} from "daisyclaw/plugin-sdk/runtime-env";
import { enqueueSystemEvent } from "daisyclaw/plugin-sdk/system-event-runtime";
...
async function clearTerminalWebAuthState(params: {
  ...
  try {
    ...
    params.runtime.error(
      `WhatsApp Web cleanup failed after terminal close (status ${params.statusLabel}). Run \`${formatCliCommand("daisyclaw channels logout --channel whatsapp")}\`, then relink with \`${formatCliCommand("daisyclaw channels login --channel whatsapp")}\`.`,
    );
  }
}
    ...
    while (true) {
        ...
        if (setupDecision) {
          ...
          if (setupDecision.action === "stop") {
            ...
            if (setupDecision.healthState === "logged-out") {
              ...
              runtime.error(
                `WhatsApp session logged out during setup. Run \`${formatCliCommand("daisyclaw channels login --channel whatsapp")}\` to relink.`,
              );
              ...
              runtime.error(
                `WhatsApp Web connection closed during setup (status ${setupDecision.normalized.statusLabel}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("daisyclaw channels login --channel whatsapp")}\`. Stopping web monitoring.`,
              );
            } else {
              runtime.error(
                `WhatsApp Web connection closed during setup (status ${setupDecision.normalized.statusLabel}) after ${setupDecision.reconnectAttempts}/${reconnectPolicy.maxAttempts} attempts. Relink with \`${formatCliCommand("daisyclaw channels login --channel whatsapp")}\` if the issue persists.`,
              );
            }
            await controller.shutdown();
            break;
      ...
      if (decision.action === "stop") {
        ...
        if (decision.healthState === "logged-out") {
          ...
          runtime.error(
            `WhatsApp session logged out. Run \`${formatCliCommand("daisyclaw channels login --channel whatsapp")}\` to relink.`,
          );
          ...
          runtime.error(
            `WhatsApp Web connection closed (status ${decision.normalized.statusLabel}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("daisyclaw channels login --channel whatsapp")}\`. Stopping web monitoring.`,
          );
        } else {
          reconnectLogger.warn(
            {
