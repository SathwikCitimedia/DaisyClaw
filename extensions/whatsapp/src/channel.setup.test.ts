...
import { createQueuedWizardPrompter } from "daisyclaw/plugin-sdk/plugin-test-runtime";
import { DEFAULT_ACCOUNT_ID } from "daisyclaw/plugin-sdk/routing";
...
vi.mock("daisyclaw/plugin-sdk/setup", async () => {
  ...
  return {
    ...
    setSetupChannelEnabled: (cfg: DaisyClawConfig, channel: string, enabled: boolean) => ({
      ...cfg,
      channels: {
        ...cfg.channels,
        [channel]: {
          ...(cfg.channels?.[channel as keyof NonNullable<DaisyClawConfig["channels"]>] as object),
          enabled,
        },
      },
    }),
...
async function runConfigureWithHarness(params: {
  ...
  const result = await finalizeWhatsAppSetup({
    cfg: params.cfg ?? ({} as DaisyClawConfig),
