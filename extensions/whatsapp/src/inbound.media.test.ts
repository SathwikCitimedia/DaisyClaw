...
vi.mock("daisyclaw/plugin-sdk/channel-pairing", async () => {
  const actual = await vi.importActual<typeof import("daisyclaw/plugin-sdk/channel-pairing")>(
...
vi.mock("daisyclaw/plugin-sdk/media-store", async () => {
  const actual = await vi.importActual<typeof import("daisyclaw/plugin-sdk/media-store")>(
    "daisyclaw/plugin-sdk/media-store",
  );
  return {
    ...actual,
...
vi.mock("./runtime.js", async () => {
  const { createChannelIngressQueueForTests: createChannelIngressQueue } = await Promise.resolve(
    vi.importActual<typeof import("daisyclaw/plugin-sdk/plugin-state-test-runtime")>(
      "daisyclaw/plugin-sdk/plugin-state-test-runtime",
    ),
  );
  const stateDir = `/tmp/daisyclaw-whatsapp-inbound-media-${Date.now()}-${Math.random()}`;
  return {
...
const HOME = path.join(os.tmpdir(), `daisyclaw-inbound-media-${crypto.randomUUID()}`);
