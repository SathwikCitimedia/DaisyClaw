...
import { logInfo } from "daisyclaw/plugin-sdk/logging-core";
...
export async function loginWeb(
  ...
  try {
    ...
    if (result.outcome === "logged-out") {
      runtime.error(
        danger(
          `WhatsApp reported the session is logged out. Cleared cached web session; please rerun ${formatCliCommand("daisyclaw channels login")} and scan the QR again.`,
        ),
      );
