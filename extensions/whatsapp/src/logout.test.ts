...
describe("web logout", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "daisyclaw-test-web-logout-"));
    previousOAuthDir = process.env.DAISYCLAW_OAUTH_DIR;
    process.env.DAISYCLAW_OAUTH_DIR = path.join(fixtureRoot, "oauth");
  ...
  it("does not delete through symlinked auth dirs inside the DaisyClaw auth root", async () => {
  ...
  it("does not delete through intermediate symlinks inside the DaisyClaw auth root", async () => {
