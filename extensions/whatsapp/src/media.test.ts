...
import { resolveStateDir } from "daisyclaw/plugin-sdk/state-paths";
import { resolvePreferredDaisyClawTmpDir } from "daisyclaw/plugin-sdk/temp-path";
import { captureEnv } from "daisyclaw/plugin-sdk/test-env";
...
import { withMockedWindowsPlatform, withRestoredMocks } from "daisyclaw/plugin-sdk/test-node-mocks";
import { optimizeImageToPng } from "daisyclaw/plugin-sdk/web-media";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredDaisyClawTmpDir(), "daisyclaw-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub DAISYCLAW_STATE_DIR.
    // Also keep it outside the DaisyClaw temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["DAISYCLAW_STATE_DIR"]);
    process.env.DAISYCLAW_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "daisyclaw-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredDaisyClawTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("accepts win32 dev=0 stat mismatch for local file loads", async () => {
    ...
    // os.tmpdir() fallback rather than the POSIX `/tmp/daisyclaw` root that
    // actually holds `tinyPngFile` on this Linux test runner (#60713).
    const realTmpRoot = resolvePreferredDaisyClawTmpDir();
  ...
  it("allows default DaisyClaw state workspace and sandbox roots", async () => {
  ...
  it("rejects default DaisyClaw state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
