...
vi.mock("daisyclaw/plugin-sdk/logging-core", async () => {
  const actual = await vi.importActual<typeof import("daisyclaw/plugin-sdk/logging-core")>(
    "daisyclaw/plugin-sdk/logging-core",
  );
  return {
    ...actual,
    getChildLogger: () => ({
...
describe("web monitor inbox", () => {
  ...
  it("detaches inbound listeners and ends the socket on close()", async () => {
    ...
    expect(closeError?.message).toBe("DaisyClaw WhatsApp listener close");
