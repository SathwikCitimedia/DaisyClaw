...
const pluginRuntimeMocks = vi.hoisted(() => {
  ...
  let stateDir = `/tmp/daisyclaw-whatsapp-ingress-${Date.now()}-${Math.random()}`;
  ...
  return {
    ...
    reset: () => {
      ...
      stateDir = `/tmp/daisyclaw-whatsapp-ingress-${Date.now()}-${Math.random()}`;
    },
  };
});
...
vi.mock("daisyclaw/plugin-sdk/channel-activity-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("daisyclaw/plugin-sdk/channel-activity-runtime")
  >("daisyclaw/plugin-sdk/channel-activity-runtime");
  return {
    ...actual,
...
function expectInboxPairingReplyText(
  ...
  expect(text).toContain("DaisyClaw: access not configured.");
...
export function installWebMonitorInboxUnitTestHooks(opts?: { authDir?: boolean }) {
  ...
  beforeEach(async () => {
    ...
    resetWebInboundDedupe();
    if (createAuthDir) {
      authDir = fsSync.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-auth-"));
    } else {
      authDir = undefined;
    }
  });
