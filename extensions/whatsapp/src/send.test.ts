...
import { MEDIA_FFMPEG_MAX_AUDIO_DURATION_SECS } from "daisyclaw/plugin-sdk/media-runtime";
...
let resetLogger: typeof import("daisyclaw/plugin-sdk/runtime-env").resetLogger;
let setLoggerOverride: typeof import("daisyclaw/plugin-sdk/runtime-env").setLoggerOverride;

const WHATSAPP_TEST_CFG: DaisyClawConfig = {
  channels: { whatsapp: {} },
};
...
describe("web outbound", () => {
  ...
  beforeAll(async () => {
    ...
    ({ resetLogger, setLoggerOverride } = await import("daisyclaw/plugin-sdk/runtime-env"));
  });
  ...
  it("redacts recipients and poll text in outbound logs", async () => {
    const logPath = path.join(os.tmpdir(), `daisyclaw-outbound-${crypto.randomUUID()}.log`);
