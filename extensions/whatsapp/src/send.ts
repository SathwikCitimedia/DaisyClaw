...
import { generateSecureUuid } from "daisyclaw/plugin-sdk/core";
import { redactIdentifier } from "daisyclaw/plugin-sdk/logging-core";
import {
  convertMarkdownTables,
  resolveMarkdownTableMode,
} from "daisyclaw/plugin-sdk/markdown-table-runtime";
import { requireRuntimeConfig } from "daisyclaw/plugin-sdk/plugin-config-runtime";
import { normalizePollInput, type PollInput } from "daisyclaw/plugin-sdk/poll-runtime";
import { createSubsystemLogger, getChildLogger } from "daisyclaw/plugin-sdk/runtime-env";
import {
...
function requireOutboundActiveWebListener(params: { cfg: DaisyClawConfig; accountId?: string }): {
  accountId: string;
  ...
  if (!listener) {
    throw new Error(
      `No active WhatsApp Web listener (account: ${resolvedAccountId}). Start the gateway, then link WhatsApp with: ${formatCliCommand(`daisyclaw channels login --channel whatsapp --account ${resolvedAccountId}`)}.`,
    );
  }
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg: DaisyClawConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
