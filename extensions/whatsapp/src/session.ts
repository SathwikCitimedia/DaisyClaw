...
import { VERSION } from "daisyclaw/plugin-sdk/cli-runtime";
import {
  createHttp1EnvHttpProxyAgent,
  createHttp1ProxyAgent,
  createNodeProxyAgent,
} from "daisyclaw/plugin-sdk/fetch-runtime";
import { danger, success } from "daisyclaw/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "daisyclaw/plugin-sdk/runtime-env";
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("daisyclaw channels login")}`,
              ),
            );
          }
        }
