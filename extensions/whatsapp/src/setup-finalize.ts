...
import type { ChannelSetupWizard } from "daisyclaw/plugin-sdk/setup";
import { formatCliCommand, formatDocsLink } from "daisyclaw/plugin-sdk/setup-tools";
import {
...
type WhatsAppConfig = NonNullable<NonNullable<DaisyClawConfig["channels"]>["whatsapp"]>;
...
function shouldWriteDefaultWhatsAppAccountConfigAtAccountScope(cfg: DaisyClawConfig): boolean {
...
function resolveDefaultWhatsAppAccountWriteKey(cfg: DaisyClawConfig): string {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts) {
...
function resolveWhatsAppConfigPathPrefix(cfg: DaisyClawConfig, accountId: string): string {
  if (
...
async function detectWhatsAppLinked(cfg: DaisyClawConfig, accountId: string): Promise<boolean> {
  ...
  if (wantsLink) {
    ...
    await params.prompter.note(
      t("wizard.whatsapp.linkLater", {
        command: formatCliCommand("daisyclaw channels login"),
      }),
      "WhatsApp",
    );
  }
