...
import { DEFAULT_ACCOUNT_ID, type DaisyClawConfig } from "daisyclaw/plugin-sdk/setup";
...
const hoisted = vi.hoisted(() => ({
  detectWhatsAppLinked: vi.fn<(cfg: DaisyClawConfig, accountId: string) => Promise<boolean>>(
    async () => false,
  ),
  ...
  resolveWhatsAppAuthDir: vi.fn<
    (params: { cfg: DaisyClawConfig; accountId: string }) => { authDir: string }
  >(() => ({
    authDir: "/tmp/daisyclaw-whatsapp-test",
  })),
}));
...
vi.mock("daisyclaw/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<typeof import("daisyclaw/plugin-sdk/setup")>(
    "daisyclaw/plugin-sdk/setup",
  );
  return {
    ...actual,
...
function expectFinalizeResult(result: Awaited<ReturnType<typeof runFinalizeWithHarness>>): {
  ...
  return result as { cfg: DaisyClawConfig };
}
...
describe("whatsapp setup wizard", () => {
  beforeEach(() => {
    ...
    hoisted.resolveWhatsAppAuthDir.mockReturnValue({ authDir: "/tmp/daisyclaw-whatsapp-test" });
  });
  ...
  it("writes named-account DM policy and allowFrom instead of the channel root", async () => {
    ...
    const named = expectFinalizeResult(
      await runFinalizeWithHarness({
        harness,
        accountId: "work",
        cfg: createWhatsAppWorkAccountConfig() as DaisyClawConfig,
      }),
    );
  ...
  it("uses configured defaultAccount for omitted-account finalize writes", async () => {
    ...
    const result = expectFinalizeResult(
      await runFinalizeWithHarness({
        harness,
        accountId: "",
        cfg: createWhatsAppWorkAccountConfig({ defaultAccount: "work" }) as DaisyClawConfig,
      }),
    );
  ...
  it("forces wildcard allowFrom for open policy without allowFrom follow-up prompts", async () => {
    ...
    const result = expectFinalizeResult(
      await runFinalizeWithHarness({
        harness,
        cfg: createWhatsAppRootAllowFromConfig() as DaisyClawConfig,
      }),
    );
