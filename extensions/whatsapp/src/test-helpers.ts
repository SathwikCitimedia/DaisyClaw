...
import { formatEnvelopeTimestamp } from "daisyclaw/plugin-sdk/channel-test-helpers";
...
const CONFIG_KEY = Symbol.for("daisyclaw:testConfigMock");
const SOURCE_CONFIG_KEY = Symbol.for("daisyclaw:testSourceConfigMock");
...
vi.mock("./auto-reply/monitor/message-line.runtime.js", () => ({
  ...
  resolveMessagePrefix: (
    ...
    return params?.hasAllowFrom === true ? "" : "[daisyclaw]";
  },
}));

vi.mock("./auth-store.runtime.js", () => ({
  resolveOAuthDir: () => "/tmp/daisyclaw-oauth",
}));

vi.mock("./session.runtime.js", () => {
  const created = createMockBaileys();
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("daisyclaw:lastSocket")] =
    created.lastSocket;
  return {
    ...created.mod,
  };
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("daisyclaw:lastSocket")];
  if (typeof getter === "function") {
