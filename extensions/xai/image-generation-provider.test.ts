...
const {
  ...
  resolveProviderHttpRequestConfigMock: vi.fn((params: Record<string, unknown>) => {
    ...
    if (params.provider === "xai") {
      const version = process.env.DAISYCLAW_VERSION?.trim() || "unknown";
      headers.set("User-Agent", `daisyclaw/${version}`);
      headers.set("originator", "daisyclaw");
...
vi.mock("daisyclaw/plugin-sdk/provider-auth-runtime", () => ({
...
vi.mock("daisyclaw/plugin-sdk/provider-http", () => ({
...
vi.mock("daisyclaw/plugin-sdk/string-coerce-runtime", () => ({
...
describe("xai image generation provider", () => {
  ...
  it("builds provider with correct models, default, and capabilities", () => {
    ...
    expect(isConfigured({ agentDir: "/tmp/daisyclaw-xai-test" })).toBe(true);
    expect(isProviderApiKeyConfiguredMock).toHaveBeenCalledWith({
      provider: "xai",
      agentDir: "/tmp/daisyclaw-xai-test",
    });
  });
  ...
  it("forwards xAI attribution User-Agent through the SDK image request", async () => {
    ...
    const request = requirePostJsonCall();
    expect(request.headers?.get("user-agent")).toBe("daisyclaw/2026.3.22");
    expect(request.headers?.get("originator")).toBe("daisyclaw");
