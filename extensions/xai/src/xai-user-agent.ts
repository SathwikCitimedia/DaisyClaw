// Shared User-Agent for xAI sidecar HTTP/WS requests; mirrors `formatDaisyClawUserAgent`.

import { DAISYCLAW_VERSION as PACKAGE_VERSION } from "daisyclaw/plugin-sdk/agent-harness-runtime";

const ORIGINATOR = "daisyclaw";
...
function resolveXaiUserAgentVersion(): string {
  ...
  const envVersion = trimToUndefined(process.env.DAISYCLAW_VERSION);
  if (envVersion) {
    return envVersion;
  }
  ...
  return (
    trimToUndefined(process.env.DAISYCLAW_SERVICE_VERSION) ??
...
// an empty record so the daisyclaw identity is not forwarded to the proxy.
