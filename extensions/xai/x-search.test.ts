                ...
                content: [
                  {
                    type: "output_text",
                    text: "Found X posts",
                    annotations: [{ type: "url_citation", url: "https://x.com/daisyclaw/status/1" }],
                  },
                ],
              },
            ],
            citations: ["https://x.com/daisyclaw/status/1"],
          },
        ),
    } as Response),
  );
...
describe("xai x_search tool", () => {
  ...
  it("uses the xAI Responses x_search tool with structured filters", async () => {
    ...
    expect((result?.details as { citations?: string[] } | undefined)?.citations).toEqual([
      "https://x.com/daisyclaw/status/1",
    ]);
  });
