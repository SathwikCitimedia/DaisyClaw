...
import type { OAuthCredential } from "daisyclaw/plugin-sdk/provider-auth";
...
describe("xAI OAuth", () => {
  ...
  it("builds the xAI authorize URL for DaisyClaw", () => {
    const url = new URL(
      buildXaiOAuthAuthorizeUrl({
    ...
    expect(url.searchParams.get("referrer")).toBe("daisyclaw");
  ...
  it("validates discovered endpoints before using them", async () => {
    ...
    expect(discoveryHeaders.get("user-agent")).toBe("daisyclaw/2026.3.22");
    vi.unstubAllEnvs();
  ...
  it("refreshes with the cached token endpoint and preserves refresh fallback", async () => {
    ...
    const fetchImpl = vi.fn<typeof fetch>(async (_url, init) => {
      ...
      expect(headers.get("user-agent")).toBe("daisyclaw/2026.3.22");
      return jsonResponse({
