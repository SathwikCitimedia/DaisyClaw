...
import { isBillingErrorMessage } from "daisyclaw/plugin-sdk/test-env";
import { describe, expect, it } from "vitest";
import plugin from "./index.js";
import { XAI_DEFAULT_STT_MODEL } from "./stt.js";

const XAI_API_KEY = process.env.XAI_API_KEY ?? "";
const LIVE_IMAGE_MODEL = process.env.DAISYCLAW_LIVE_XAI_IMAGE_MODEL?.trim() || "grok-imagine-image";
const liveEnabled = XAI_API_KEY.trim().length > 0 && process.env.DAISYCLAW_LIVE_TEST === "1";
...
describeLive("xai plugin live", () => {
  it("synthesizes TTS through the registered speech provider", async () => {
    await runXaiLiveCase("tts", async () => {
      ...
      const audioFile = await speechProvider.synthesize({
        text: "DaisyClaw xAI text to speech integration test OK.",
        cfg,
        providerConfig: {
          apiKey: XAI_API_KEY,
          baseUrl: "https://api.x.ai/v1",
      ...
      const telephony = await speechProvider.synthesizeTelephony?.({
        text: "DaisyClaw xAI telephony check OK.",
        cfg,
        providerConfig: {
          apiKey: XAI_API_KEY,
          baseUrl: "https://api.x.ai/v1",
  ...
  it("transcribes audio through the registered media provider", async () => {
    await runXaiLiveCase("stt", async () => {
      ...
      const speechProvider = requireRegisteredProvider(speechProviders, "xai");
      const cfg = createLiveConfig();
      const phrase = "DaisyClaw xAI speech to text integration test OK.";
      ...
      expectDaisyClawLiveTranscriptMarker(normalized);
  ...
  it("streams realtime STT through the registered transcription provider", async () => {
    await runXaiLiveCase("realtime-stream", async () => {
      ...
      const speechProvider = requireRegisteredProvider(speechProviders, "xai");
      const cfg = createLiveConfig();
      const phrase = "DaisyClaw xAI realtime transcription integration test OK.";
