...
import { buildOpenAICompletionsParams } from "daisyclaw/plugin-sdk/provider-transport-runtime";
...
describe("zai provider plugin", () => {
  ...
  it("uses deprecated pi agent auth.json for usage auth when modern sources are empty", async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-zai-legacy-auth-"));
    try {
