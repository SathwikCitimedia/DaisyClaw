...
import { createScopedDmSecurityResolver } from "daisyclaw/plugin-sdk/channel-config-helpers";
...
function resolveZalouserOutboundChunkMode(cfg: DaisyClawConfig, accountId?: string) {
  return getZalouserRuntime().channel.text.resolveChunkMode(cfg, "zalouser", accountId);
}

function resolveZalouserOutboundTextChunkLimit(cfg: DaisyClawConfig, accountId?: string) {
...
export const zalouserPairingTextAdapter = {
  ...
  notify: async ({ cfg, id, message }: { cfg: DaisyClawConfig; id: string; message: string }) => {
    const { sendMessageZalouser } = await loadZalouserChannelRuntime();
