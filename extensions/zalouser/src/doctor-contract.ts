...
type ZalouserChannelsConfig = NonNullable<DaisyClawConfig["channels"]>;
...
function normalizeZalouserCompatibilityConfig(cfg: DaisyClawConfig): ChannelDoctorConfigMutation {
...
export const legacyConfigRules: ChannelDoctorLegacyConfigRule[] = [
  {
    ...
    message:
      'channels.zalouser.groups.<id>.allow is legacy; use channels.zalouser.groups.<id>.enabled instead. Run "daisyclaw doctor --fix".',
    match: hasLegacyZalouserGroupAllowAliases,
  },
  {
    path: ["channels", "zalouser", "accounts"],
    message:
      'channels.zalouser.accounts.<id>.groups.<id>.allow is legacy; use channels.zalouser.accounts.<id>.groups.<id>.enabled instead. Run "daisyclaw doctor --fix".',
