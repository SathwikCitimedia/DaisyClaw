...
import type {
  ...
  RequestFrame,
  ResponseFrame,
} from "@daisyclaw/gateway-protocol";
import {
  GATEWAY_CLIENT_MODES,
  GATEWAY_CLIENT_NAMES,
  type GatewayClientMode,
  type GatewayClientName,
} from "@daisyclaw/gateway-protocol/client-info";
import {
...
} from "@daisyclaw/gateway-protocol/connect-error-details";
import { resolveGatewayStartupRetryAfterMs } from "@daisyclaw/gateway-protocol/startup-unavailable";
import { MIN_CLIENT_PROTOCOL_VERSION, PROTOCOL_VERSION } from "@daisyclaw/gateway-protocol/version";
...
// The package stays reusable by depending on host callbacks for DaisyClaw-owned
...
export class GatewayClient {
  ...
  constructor(opts: GatewayClientOptions) {
    this.deps = {
      // Defaults keep the package inert outside DaisyClaw; device signing throws
  ...
  start() {
    ...
    const allowPrivateWs =
      (this.opts.env ?? process.env).DAISYCLAW_ALLOW_INSECURE_PRIVATE_WS === "1";
    ...
    if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
      ...
      const error = new Error(
        `SECURITY ERROR: Cannot connect to "${displayHost}" over plaintext ws://. ` +
          ...
          (allowPrivateWs
            ? ""
            : "Break-glass (trusted private networks only): set DAISYCLAW_ALLOW_INSECURE_PRIVATE_WS=1. ") +
          "Run `daisyclaw doctor --fix` for guidance.",
      );
