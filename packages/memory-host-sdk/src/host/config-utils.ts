...
export { splitShellArgs } from "./daisyclaw-runtime-io.js";

// Shared DaisyClaw config helpers used by memory host, QMD, and agent context code.
...
/** Narrow DaisyClaw config shape consumed by memory host utilities. */
...
/** Resolve the underlying OS home before applying DaisyClaw-specific overrides. */
...
/** Resolve DAISYCLAW_HOME or the OS home, falling back to cwd for hermetic tests. */
function resolveRequiredHomeDir(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = os.homedir,
): string {
  const explicitHome = normalizeHomeValue(env.DAISYCLAW_HOME);
...
/** Resolve absolute user paths, including "~" against the effective DaisyClaw home. */
...
export function resolveStateDir(
  ...
  if (env.DAISYCLAW_TEST_FAST === "1" || fs.existsSync(nextDir)) {
    return nextDir;
  }
  // Existing legacy state remains authoritative until an explicit migration creates .daisyclaw.
...
/** Resolve the default agent workspace, partitioned by DAISYCLAW_PROFILE when set. */
function resolveDefaultAgentWorkspaceDir(env: NodeJS.ProcessEnv = process.env): string {
  const home = resolveRequiredHomeDir(env, os.homedir);
  const profile = env.DAISYCLAW_PROFILE?.trim();
  if (profile && normalizeLowercaseStringOrEmpty(profile) !== "default") {
    return path.join(home, ".daisyclaw", `workspace-${profile}`);
  }
  return path.join(home, ".daisyclaw", "workspace");
}

/** Return configured agent entries after dropping nullish placeholders. */
function listAgentEntries(cfg: DaisyClawConfig): AgentConfig[] {
...
function resolveDefaultAgentId(cfg: DaisyClawConfig): string {
...
function resolveAgentConfig(cfg: DaisyClawConfig, agentId: string): AgentConfig | undefined {
