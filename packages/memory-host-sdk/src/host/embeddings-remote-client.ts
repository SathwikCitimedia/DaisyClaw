...
import { requireApiKey, resolveApiKeyForProvider } from "./daisyclaw-runtime-auth.js";
...
function resolveDaisyClawAttributionHeaders(): Record<string, string> {
  const version = typeof process !== "undefined" ? process.env.DAISYCLAW_VERSION?.trim() : undefined;
  return {
    originator: "daisyclaw",
    ...(version ? { version } : {}),
    "User-Agent": version ? `daisyclaw/${version}` : "daisyclaw",
  };
}
...
export async function resolveRemoteEmbeddingBearerClient(params: {
  ...
  if (isNativeOpenAIEmbeddingRoute(params.provider, baseUrl)) {
    Object.assign(headers, resolveDaisyClawAttributionHeaders());
  }
