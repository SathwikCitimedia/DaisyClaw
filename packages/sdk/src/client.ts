// DaisyClaw SDK module implements client behavior.
...
// High-level DaisyClaw SDK client. Namespaces below translate friendly SDK calls
...
/** Connection and transport options for the DaisyClaw SDK client. */
export type DaisyClawOptions = {
  gateway?: "auto" | (string & {});
  url?: string;
  token?: string;
  password?: string;
  requestTimeoutMs?: number;
  transport?: DaisyClawTransport;
};

function resolveGatewayUrl(options: DaisyClawOptions): string | undefined {
  if (options.url) {
    return options.url;
  }
...
function assertNoUnsupportedRunOptions(params: AgentRunParams): void {
  ...
  throw new Error(
    `DaisyClaw Gateway does not support per-run SDK option${
...
function unsupportedGatewayApi(api: string): never {
  throw new Error(`${api} is not supported by the current DaisyClaw Gateway yet`);
}
...
function readChatProjection(event: DaisyClawEvent): ChatProjection | undefined {
  const raw = event.raw;
...
function isAssistantRunEvent(event: DaisyClawEvent): boolean {
  return event.type === "assistant.delta" || event.type === "assistant.message";
}

function isTerminalRunEvent(event: DaisyClawEvent): boolean {
  return (
...
export class DaisyClaw {
  ...
  private readonly transport: DaisyClawTransport;
  private readonly normalizedEvents = new EventHub<DaisyClawEvent>({
    replayLimit: MAX_NORMALIZED_REPLAY_EVENTS,
  });
  private readonly replayByRunId = new Map<string, DaisyClawEvent[]>();
  private connected = false;
  private eventPumpPromise: Promise<void> | null = null;
  private eventPumpReady: Promise<void> | null = null;

  constructor(options: DaisyClawOptions = {}) {
    this.transport =
      options.transport ??
  ...
  events(filter?: (event: DaisyClawEvent) => boolean): AsyncIterable<DaisyClawEvent> {
    return this.iterateEvents(filter);
  }

  runEvents(
    runId: string,
    filter?: (event: DaisyClawEvent) => boolean,
  ): AsyncIterable<DaisyClawEvent> {
  ...
  private async *iterateRunEvents(
    ...
    const toRunStreamEvent = (event: DaisyClawEvent): DaisyClawEvent | undefined => {
    ...
    const matches = (event: DaisyClawEvent) => event.runId === runId;
  ...
  private recordReplayEvent(event: DaisyClawEvent): void {
    if (!event.runId) {
      return;
    }
  ...
  private replaySnapshot(runId: string): DaisyClawEvent[] {
...
export class Agent {
  constructor(
    private readonly client: DaisyClaw,
    readonly id: string,
  ) {}
...
export class AgentsNamespace {
  constructor(private readonly client: DaisyClaw) {}
...
export class RunsNamespace {
  ...
  events(runId: string): AsyncIterable<DaisyClawEvent> {
...
class RpcNamespace {
  constructor(
    protected readonly client: DaisyClaw,
