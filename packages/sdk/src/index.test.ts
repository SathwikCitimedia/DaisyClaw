// DaisyClaw SDK tests cover index behavior.
import { describe, expect, it } from "vitest";
import { EventHub, DaisyClaw, normalizeGatewayEvent } from "./index.js";
import type {
  GatewayEvent,
...
class FakeTransport implements DaisyClawTransport {
...
class EventsOnlyTransport implements DaisyClawTransport {
...
describe("DaisyClaw SDK", () => {
  it("runs an agent through the Gateway agent method", async () => {
    ...
    const oc = new DaisyClaw({ transport });
  ...
  it("splits provider-qualified model refs and rejects unsupported run options", async () => {
    ...
    await expect(
      ...
      "DaisyClaw Gateway does not support per-run SDK options yet: workspace, runtime, environment, approvals",
    );
  });
  ...
  it("throws explicit unsupported errors for SDK namespaces without Gateway RPCs", async () => {
    ...
    await expect(oc.environments.create({ provider: "testbox" })).rejects.toThrow(
      "oc.environments.create is not supported by the current DaisyClaw Gateway yet",
    );
    await expect(oc.environments.delete("environment_123")).rejects.toThrow(
      "oc.environments.delete is not supported by the current DaisyClaw Gateway yet",
    );
    expect(transport.calls).toStrictEqual([]);
  });
  ...
  it("rejects normalized event streams when the event pump fails before yielding", async () => {
    ...
    let futureIterator: AsyncIterator<DaisyClawEvent> | undefined;
  ...
  it("uses cumulative text for the first replayed chat projection", async () => {
    ...
    let text = "";
    let iterator: AsyncIterator<DaisyClawEvent> | undefined;
