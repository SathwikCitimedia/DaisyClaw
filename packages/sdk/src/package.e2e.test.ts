// DaisyClaw SDK tests cover package behavior.
...
const WORKSPACE_PACKAGE_NAMES = [
  "@daisyclaw/gateway-protocol",
  "@daisyclaw/gateway-client",
  "@daisyclaw/sdk",
] as const;
...
function normalizeWorkspaceDependencies(
  ...
  for (const [name, spec] of Object.entries(dependencies)) {
    normalized[name] =
      name.startsWith("@daisyclaw/") && spec === "workspace:*" ? "0.0.0-private" : spec;
  }
  return normalized;
}
...
async function startDaisyClawRegistry(packages: PackedPackage[]): Promise<{
  registryUrl: string;
  close: () => Promise<void>;
}> {
...
describe("DaisyClaw SDK package e2e", () => {
  afterEach(async () => {
    await Promise.all(
  ...
  it("packs and imports from an external temp consumer", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-sdk-consumer-"));
    ...
    const sdkTarball =
      packedPackages.find((pkg) => pkg.manifest.name === "@daisyclaw/sdk")?.tarball ?? "";
    expect(sdkTarball).not.toBe("");
    const registry = await startDaisyClawRegistry(packedPackages);

    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ private: true, type: "module" }),
    );
    await fs.writeFile(path.join(tempDir, ".npmrc"), `@daisyclaw:registry=${registry.registryUrl}`);
    try {
      await runCommand(
        "npm",
    ...
    const importScript = `
      import { GatewayClientTransport, DaisyClaw, normalizeGatewayEvent } from "@daisyclaw/sdk";
      if (typeof GatewayClientTransport !== "function") throw new Error("missing transport export");
      if (typeof DaisyClaw !== "function") throw new Error("missing client export");
