...
import { resolveChannelTtsVoiceDelivery } from "daisyclaw/plugin-sdk/channel-targets";
import type {
  DaisyClawConfig,
  ResolvedTtsPersona,
  TtsAutoMode,
...
import { redactSensitiveText } from "daisyclaw/plugin-sdk/logging-core";
import { transcodeAudioBuffer } from "daisyclaw/plugin-sdk/media-runtime";
import { clampTimerTimeoutMs } from "daisyclaw/plugin-sdk/number-runtime";
import {
...
import { isVerbose, logVerbose } from "daisyclaw/plugin-sdk/runtime-env";
import { tempWorkspaceSync, resolvePreferredDaisyClawTmpDir } from "daisyclaw/plugin-sdk/sandbox";
import { privateFileStoreSync } from "daisyclaw/plugin-sdk/security-runtime";
import {
  normalizeLowercaseStringOrEmpty,
  normalizeOptionalLowercaseString,
  normalizeOptionalString,
} from "daisyclaw/plugin-sdk/string-coerce-runtime";
import { stripMarkdown } from "daisyclaw/plugin-sdk/text-chunking";
import { resolveConfigDir, resolveUserPath } from "daisyclaw/plugin-sdk/text-utility-runtime";
import {
...
function resolveTtsPrefsPathValue(prefsPath: string | undefined): string {
  ...
  const envPath = process.env.DAISYCLAW_TTS_PREFS?.trim();
  if (envPath) {
...
function resolveConfiguredSpeechVoiceModelRefs(cfg: DaisyClawConfig | undefined): VoiceModelRef[] {
...
function sortSpeechProvidersForAutoSelection(cfg?: DaisyClawConfig) {
...
function resolveTtsRuntimeConfig(cfg: DaisyClawConfig): DaisyClawConfig {
  return (
    selectApplicableRuntimeConfig({
      inputConfig: cfg,
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: DaisyClawConfig): TtsProvider[] {
  const effectiveCfg = cfg ? resolveTtsRuntimeConfig(cfg) : undefined;
