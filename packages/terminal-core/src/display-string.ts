...
/** Resolve the underlying OS home before applying DaisyClaw overrides. */
...
/** Resolve raw home with DAISYCLAW_HOME tilde expansion. */
function resolveRawHomeDir(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = os.homedir,
): string | undefined {
  const explicitHome = normalize(env.DAISYCLAW_HOME);
  if (explicitHome) {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  return explicitHome ? { home, prefix: "$DAISYCLAW_HOME" } : { home, prefix: "~" };
}

/** Replace the effective home path with "~" or "$DAISYCLAW_HOME" for terminal display. */
