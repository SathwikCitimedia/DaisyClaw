...
- `DAISYCLAW_QA_CONVEX_SITE_URL` must use `https://` in normal use.
- Local development may use loopback `http://` only when `DAISYCLAW_QA_ALLOW_INSECURE_HTTP=1`.
...
Admin endpoints require `DAISYCLAW_QA_CONVEX_SECRET_MAINTAINER`.
