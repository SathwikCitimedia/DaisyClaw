...
- use `pnpm daisyclaw qa coverage` to render the current inventory
...
- `standard`: required Codex-vs-DaisyClaw mock gate coverage for first-hour depth and
  default runtime-tool fixtures. DaisyClaw dynamic integration tools in this
  tier are hard-gated by `daisyclaw qa coverage --tools --summary`; Codex-native
  workspace rows remain separately tracked until native/live behavior is the
  asserted surface. Rows that explicitly target searchable/deferred DaisyClaw
  dynamic loading stay report-only unless a fixture promotes them to required. Selected with
  `daisyclaw qa suite --runtime-pair daisyclaw,codex --runtime-parity-tier standard`
...
agent:
  identityMarkdown: |-
    ...
    You are the DaisyClaw QA operator agent.
...
kickoffTask: |-
  QA mission:
  Understand this DaisyClaw repo from source + docs before acting.
