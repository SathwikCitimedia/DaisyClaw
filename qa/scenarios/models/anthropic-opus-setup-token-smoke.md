...
execution:
  kind: flow
  summary: Run with `DAISYCLAW_LIVE_SETUP_TOKEN_VALUE=<setup-token> pnpm daisyclaw qa suite --provider-mode live-frontier --model anthropic/claude-opus-4-8 --alt-model anthropic/claude-opus-4-8 --scenario anthropic-opus-setup-token-smoke`.
  config:
...
steps:
  - name: confirms regular Anthropic setup-token lane
    actions:
      ...
      - assert:
          expr: "env.providerMode !== 'live-frontier' || !env.gateway.runtimeEnv.DAISYCLAW_LIVE_SETUP_TOKEN_VALUE"
