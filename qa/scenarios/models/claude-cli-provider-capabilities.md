...
execution:
  kind: flow
  summary: Run with `pnpm daisyclaw qa suite --provider-mode live-frontier --cli-auth-mode api-key --model claude-cli/claude-sonnet-4-6 --alt-model claude-cli/claude-sonnet-4-6 --scenario claude-cli-provider-capabilities`.
  config:
    authMode: api-key
