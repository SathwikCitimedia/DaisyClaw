...
coverage:
  primary:
    - tools.apply-patch
objective: Verify apply_patch behavior is tracked across DaisyClaw and Codex while Codex owns patching natively.
successCriteria:
  - DaisyClaw may expose DaisyClaw apply_patch while Codex app-server mode may omit duplicate DaisyClaw dynamic apply_patch.
...
execution:
  ...
  config:
    ...
    knownHarnessGap:
      issue: "#80320"
      reason: Codex-native apply_patch is intentionally not an DaisyClaw dynamic tool; QA fault injection still uses synthetic failure-path inputs.
