# Medium game plan DaisyClaw harness

```yaml qa-scenario
id: medium-game-plan-daisyclaw-harness
title: Medium game plan DaisyClaw harness
surface: workspace
coverage:
  primary:
    - workspace.planning
  secondary:
    - agents.daisyclaw-harness
objective: Verify GPT-5.5 can use the DaisyClaw harness to plan and build a medium-complex self-contained browser game.
successCriteria:
  - A live-frontier run fails fast unless the selected primary model is openai/gpt-5.5.
  - The scenario forces the embedded DaisyClaw harness before the build turn.
...
codeRefs:
  ...
  - src/agents/harness/builtin-daisyclaw.ts
  - extensions/qa-lab/src/suite.ts
execution:
  kind: flow
  summary: Run with `pnpm daisyclaw qa suite --provider-mode live-frontier --model openai/gpt-5.5 --alt-model openai/gpt-5.5 --fast --thinking medium --scenario medium-game-plan-daisyclaw-harness`.
  config:
    requiredProvider: openai
    requiredModel: gpt-5.5
    harnessRuntime: daisyclaw
    artifactFile: star-garden-defenders-daisyclaw.html
    gameTitle: Star Garden Defenders
    minBytes: 5000
    buildPrompt: |-
      Enter plan mode first and write a short implementation plan before editing.

      Then build a medium-complex, self-contained browser game at ./star-garden-defenders-daisyclaw.html.
...
```yaml qa-flow
steps:
  - name: confirms GPT-5.5 DaisyClaw harness target
    actions:
      - set: selected
        value:
      ...
      - if:
          ...
          then:
            ...
            - call: runAgentPrompt
              args:
                - ref: env
                - sessionKey: agent:qa:medium-game-daisyclaw
                  message:
