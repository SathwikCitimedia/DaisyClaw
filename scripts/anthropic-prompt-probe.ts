// Anthropic Prompt Probe script supports DaisyClaw repository automation.
import { spawn } from "node:child_process";
// Live prompt probe for Anthropic setup-token and Claude CLI prompt-path debugging.
// Usage:
// DAISYCLAW_PROMPT_TRANSPORT=direct|gateway
// DAISYCLAW_PROMPT_MODE=extra
// DAISYCLAW_PROMPT_TEXT='...'
// DAISYCLAW_PROMPT_CAPTURE=1
...
const TRANSPORT = process.env.DAISYCLAW_PROMPT_TRANSPORT?.trim() === "direct" ? "direct" : "gateway";
const GATEWAY_PROMPT_MODE = "extra";
const PROMPT_TEXT = process.env.DAISYCLAW_PROMPT_TEXT?.trim() ?? "";
const PROMPT_LIST_JSON = process.env.DAISYCLAW_PROMPT_LIST_JSON?.trim() ?? "";
const USER_PROMPT = process.env.DAISYCLAW_USER_PROMPT?.trim() || "is clawd here?";
const ENABLE_CAPTURE = parseBooleanEnv({
  fallback: false,
  name: "DAISYCLAW_PROMPT_CAPTURE",
  raw: process.env.DAISYCLAW_PROMPT_CAPTURE,
});
const INCLUDE_RAW = parseBooleanEnv({
  fallback: false,
  name: "DAISYCLAW_PROMPT_INCLUDE_RAW",
  raw: process.env.DAISYCLAW_PROMPT_INCLUDE_RAW,
});
const KEEP_TMP = parseBooleanEnv({
  fallback: false,
  name: "DAISYCLAW_PROMPT_KEEP_TMP",
  raw: process.env.DAISYCLAW_PROMPT_KEEP_TMP,
});
const CLAUDE_BIN = process.env.CLAUDE_BIN?.trim() || "claude";
const NODE_BIN = process.env.DAISYCLAW_NODE_BIN?.trim() || process.execPath;
const TIMEOUT_MS = parseStrictIntegerOption({
  fallback: 45_000,
  label: "DAISYCLAW_PROMPT_TIMEOUT_MS",
  min: 1,
  raw: process.env.DAISYCLAW_PROMPT_TIMEOUT_MS,
});
const GATEWAY_TIMEOUT_MS = parseStrictIntegerOption({
  fallback: 120_000,
  label: "DAISYCLAW_PROMPT_GATEWAY_TIMEOUT_MS",
  min: 1,
  raw: process.env.DAISYCLAW_PROMPT_GATEWAY_TIMEOUT_MS,
});
const CAPTURE_PROXY_MAX_BODY_BYTES = parseStrictIntegerOption({
  fallback: 2 * 1024 * 1024,
  label: "DAISYCLAW_PROMPT_CAPTURE_MAX_BODY_BYTES",
  min: 1,
  raw: process.env.DAISYCLAW_PROMPT_CAPTURE_MAX_BODY_BYTES,
});
const GATEWAY_LOG_TAIL_BYTES = 256 * 1024;
const SETUP_TOKEN_RAW = process.env.DAISYCLAW_LIVE_SETUP_TOKEN?.trim() ?? "";
const SETUP_TOKEN_VALUE = process.env.DAISYCLAW_LIVE_SETUP_TOKEN_VALUE?.trim() ?? "";
const SETUP_TOKEN_PROFILE = process.env.DAISYCLAW_LIVE_SETUP_TOKEN_PROFILE?.trim() ?? "";
...
function resolveSetupTokenSource(): TokenSource {
  ...
  if (!match) {
    throw new Error(
      "no Anthropics setup-token profile found; set DAISYCLAW_LIVE_SETUP_TOKEN_VALUE or DAISYCLAW_LIVE_SETUP_TOKEN_PROFILE",
    );
  }
  return { profileId: match.id, token: validateSetupToken(match.token) };
}
...
async function runDirectPrompt(prompt: string): Promise<PromptResult> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-direct-prompt-probe-"));
...
async function startGatewayProcess(params: {
  ...
  const child = spawn(
    NODE_BIN,
    ["daisyclaw.mjs", "gateway", "--port", String(params.port), "--bind", "loopback", "--force"],
    {
      cwd: process.cwd(),
      env: {
        ...process.env,
        ...
        DAISYCLAW_AGENT_DIR: params.agentDir,
        DAISYCLAW_GATEWAY_TOKEN: params.gatewayToken,
        DAISYCLAW_SKIP_CHANNELS: "1",
        DAISYCLAW_SKIP_GMAIL_WATCHER: "1",
        DAISYCLAW_SKIP_CANVAS_HOST: "1",
        DAISYCLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
        DAISYCLAW_DISABLE_BONJOUR: "1",
        DAISYCLAW_SKIP_CRON: "1",
        DAISYCLAW_TEST_MINIMAL_GATEWAY: "1",
        DAISYCLAW_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir,
        ANTHROPIC_API_KEY: "",
        ANTHROPIC_API_KEY_OLD: "",
      },
      stdio: ["ignore", "pipe", "pipe"],
...
async function runGatewayPrompt(prompt: string): Promise<PromptResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-gateway-prompt-probe-"));
...
async function main() {
  if (!PROMPT_TEXT && !PROMPT_LIST_JSON) {
    throw new Error("missing DAISYCLAW_PROMPT_TEXT or DAISYCLAW_PROMPT_LIST_JSON");
  }
