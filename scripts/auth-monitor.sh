...
#
# Suggested cron: */30 * * * * /home/admin/daisyclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send DaisyClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., daisyclaw-alerts)
...
STATE_FILE="$HOME/.daisyclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via DaisyClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use daisyclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via DaisyClaw to $NOTIFY_PHONE..."
            daisyclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! DaisyClaw is down. Run: ssh l36 '~/daisyclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
