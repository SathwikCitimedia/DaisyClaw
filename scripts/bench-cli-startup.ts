// Bench Cli Startup script supports DaisyClaw repository automation.
...
const DEFAULT_ENTRY = "daisyclaw.mjs";
...
function parseGatewayPortEnv(raw: string | undefined): number {
  ...
  if (bracketHostMatch) {
    return parsePositiveInt(bracketHostMatch[1], 32123, "DAISYCLAW_GATEWAY_PORT");
  }
  ...
  return parsePositiveInt(portRaw, 32123, "DAISYCLAW_GATEWAY_PORT");
}
...
function buildConfigFixture(commandCase: CommandCase): Record<string, unknown> | null {
  ...
  const port = parseGatewayPortEnv(process.env.DAISYCLAW_GATEWAY_PORT);
  return {
    gateway: {
      auth: { mode: "none" },
      bind: "loopback",
...
async function runSample(params: {
  ...
  const runRoot = mkdtempSync(path.join(os.tmpdir(), "daisyclaw-cli-bench-home-"));
  const stateDir = path.join(runRoot, ".daisyclaw");
...
function printUsage(): void {
  console.log(`DaisyClaw CLI benchmark
...
Options:
  ...
  --entry <path>               Primary entry file (default: daisyclaw.mjs)
...
async function main(): Promise<void> {
  ...
  const tmpDir = mkdtempSync(path.join(os.tmpdir(), "daisyclaw-cli-bench-"));
