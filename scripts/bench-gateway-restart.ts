// Bench Gateway Restart script supports DaisyClaw repository automation.
...
const GATEWAY_CASES: readonly GatewayBenchCase[] = [
  {
    ...
    env: { DAISYCLAW_SKIP_CHANNELS: "1" },
    config: BASE_CONFIG,
  },
  {
    id: "skipChannelsAcpxProbe",
    name: "gateway restart, skip channels, ACPX startup probe on",
    env: { DAISYCLAW_ACPX_RUNTIME_STARTUP_PROBE: "1", DAISYCLAW_SKIP_CHANNELS: "1" },
    config: BASE_CONFIG,
  },
  {
    id: "skipChannelsNoAcpxProbe",
    name: "gateway restart, skip channels, ACPX startup probe off",
    env: { DAISYCLAW_ACPX_RUNTIME_STARTUP_PROBE: "0", DAISYCLAW_SKIP_CHANNELS: "1" },
    config: BASE_CONFIG,
  },
  {
    id: "default",
...
function printUsage(): void {
  console.log(`DaisyClaw Gateway restart benchmark
...
function writeConfig(root: string, benchCase: GatewayBenchCase): string {
  ...
  const configPath = path.join(root, "daisyclaw.json");
...
function sanitizedEnv(
  ...
  return {
    ...
    LOGNAME: process.env.LOGNAME ?? "daisyclaw-bench",
    NO_COLOR: "1",
    PATH: process.env.PATH,
    SHELL: process.env.SHELL,
    TMPDIR: process.env.TMPDIR,
    USER: process.env.USER ?? "daisyclaw-bench",
    npm_config_update_notifier: "false",
    DAISYCLAW_CONFIG: configPath,
    ...
    DAISYCLAW_TEST_DISABLE_UPDATE_CHECK: "1",
    ...benchCase.env,
  };
}
...
async function runGatewaySample(options: {
  ...
  const root = mkdtempSync(path.join(tmpdir(), "daisyclaw-gateway-restart-bench-"));
