// SQLite state benchmark seeds DaisyClaw DBs and reports hot-query proof lines.
...
} from "../src/state/daisyclaw-agent-db.js";
import {
  closeDaisyClawStateDatabaseForTest,
  openDaisyClawStateDatabase,
} from "../src/state/daisyclaw-state-db.js";
...
function printUsage(): void {
  console.log(`DaisyClaw SQLite state benchmark
...
function main(): void {
  ...
  const stateDir =
    options.stateDir ?? fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-sqlite-perf-"));
  const env = { DAISYCLAW_STATE_DIR: stateDir };
  const started = nowMs();
  try {
    const stateDatabase = openDaisyClawStateDatabase({ env });
    const agentDatabases = Array.from({ length: config.agentCount }, (_, index) =>
      openDaisyClawAgentDatabase({ agentId: `perf-agent-${index}`, env }),
    );
