#!/usr/bin/env node
// Builds DaisyClaw packages and plugin SDK artifacts with cache-aware orchestration.
...
const PLUGIN_SDK_ENTRY_DTS_CACHE_ENV = ["DAISYCLAW_BUILD_PRIVATE_QA"];
...
export const BUILD_ALL_STEPS = [
  ...
  },
  {
    ...
    // from package.json, git HEAD, and DAISYCLAW_CONTROL_UI_BUILD_ID env, so a
...
export function buildAllUsage() {
  return [
    ...
    "",
    "Builds DaisyClaw artifacts for the selected profile.",
    "",
    "Profiles:",
...
export function resolveBuildAllStep(step, params = {}) {
  ...
  if (step.kind === "pnpm") {
    const nodeFallbackArgs =
      env.DAISYCLAW_BUILD_ALL_NO_PNPM === "1" ? PNPM_STEP_NODE_FALLBACKS.get(step.label) : undefined;
    if (nodeFallbackArgs) {
      return {
...
if (isMainModule()) {
  ...
  if (args?.help) {
    ...
    for (const step of resolveBuildAllSteps(args.profile)) {
      ...
      if (process.env.DAISYCLAW_BUILD_CACHE !== "0" && cacheState.fresh) {
