...
const MAX_RSS_MARKER = "__DAISYCLAW_MAX_RSS_KB__=";
const DEFAULT_COMMAND_TIMEOUT_MS = 60_000;
const COMMAND_TIMEOUT_MS = readPositiveIntEnv(
  "DAISYCLAW_STARTUP_MEMORY_TIMEOUT_MS",
...
function parseArgs(argv) {
  const options = {
    jsonPath:
      readNonEmptyEnv("DAISYCLAW_STARTUP_MEMORY_JSON_PATH") ??
      path.join(repoRoot, ".artifacts", "startup-memory", "startup-memory.json"),
    summaryPath:
      readNonEmptyEnv("DAISYCLAW_STARTUP_MEMORY_SUMMARY_PATH") ??
...
const cases = [
  {
    id: "help",
    label: "--help",
    args: ["daisyclaw.mjs", "--help"],
    limitMb: readPositiveNumberEnv("DAISYCLAW_STARTUP_MEMORY_HELP_MB", DEFAULT_LIMITS_MB.help),
  },
  {
    id: "statusJson",
    label: "status --json",
    args: ["daisyclaw.mjs", "status", "--json"],
    limitMb: readPositiveNumberEnv(
      "DAISYCLAW_STARTUP_MEMORY_STATUS_JSON_MB",
  ...
  },
  {
    ...
    args: ["daisyclaw.mjs", "gateway", "status"],
    limitMb: readPositiveNumberEnv(
      "DAISYCLAW_STARTUP_MEMORY_GATEWAY_STATUS_MB",
...
function formatFixGuidance(testCase, details) {
  ...
  const guidance = [
    ...
    "LLM prompt:",
    `"DaisyClaw startup-memory CI failed for '${testCase.label}'. Analyze this failure, identify the first runtime/import side effect that makes startup heavier or broken, and propose the smallest safe patch. Failure output:\n${details}"`,
  ];
...
function runStartupMemoryCheck(argv = process.argv.slice(2), params = {}) {
  ...
  tmpHome = mkdtempSync(path.join(os.tmpdir(), "daisyclaw-startup-memory-"));
