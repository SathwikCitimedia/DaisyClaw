...
function usage() {
  return "Usage: node scripts/check-daisyclaw-package-tarball.mjs <daisyclaw.tgz>";
}
...
if (!fs.existsSync(tarball)) {
  fail(`DaisyClaw package tarball does not exist: ${tarball}`);
}

const phaseTimingsEnabled = process.env.DAISYCLAW_PACKAGE_TARBALL_CHECK_TIMINGS !== "0";
function runPhase(label, action) {
  ...
  try {
    ...
    if (phaseTimingsEnabled) {
      ...
      console.error(`check-daisyclaw-package-tarball: ${label} completed in ${durationMs}ms`);
    }
  }
}
...
const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-package-tarball-"));
try {
...
if (!entrySet.has("npm-shrinkwrap.json")) {
  ...
  try {
    ...
    if (shrinkwrap.name !== "daisyclaw") {
      errors.push("npm-shrinkwrap.json root name must be daisyclaw");
    }
    ...
    if (!rootPackage || rootPackage.name !== "daisyclaw") {
      errors.push("npm-shrinkwrap.json packages root must name daisyclaw");
    }
...
if (errors.length > 0) {
  fs.rmSync(extractDir, { recursive: true, force: true });
  fail(`DaisyClaw package tarball integrity failed:\n${errors.join("\n")}`);
}

for (const warning of warnings) {
  console.warn(`DaisyClaw package tarball integrity warning: ${warning}`);
}
fs.rmSync(extractDir, { recursive: true, force: true });
console.log("DaisyClaw package tarball integrity passed.");
