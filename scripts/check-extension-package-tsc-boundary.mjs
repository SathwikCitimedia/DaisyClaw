...
export function resolveCompileConcurrency(
  ...
  const raw = env.DAISYCLAW_EXTENSION_BOUNDARY_CONCURRENCY?.trim();
  if (raw) {
    return parsePositiveInt(raw, "DAISYCLAW_EXTENSION_BOUNDARY_CONCURRENCY");
  }
...
async function runCompileCheck(extensionIds) {
  ...
  const verboseFreshLogs = process.env.DAISYCLAW_EXTENSION_BOUNDARY_VERBOSE_FRESH === "1";
