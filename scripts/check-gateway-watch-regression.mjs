...
const WATCH_GATEWAY_SKIP_ENV = {
  ...
  DAISYCLAW_RUNTIME_POSTBUILD_STATIC_ASSETS: "0",
...
function formatCapturedWatchLog(text, truncated) {
  return truncated
    ? `[daisyclaw] log truncated to last ${WATCH_LOG_CAPTURE_MAX_CHARS} chars\n${text}`
    : text;
}
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const isolatedStateDir = path.join(isolatedHomeDir, ".daisyclaw");
  const isolatedConfigPath = path.join(isolatedStateDir, "daisyclaw.json");
  const shellSource = [
    'echo "$$" > "$DAISYCLAW_WATCH_PID_FILE"',
    'mkdir -p "$DAISYCLAW_STATE_DIR"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}},"plugins":{"enabled":false}}' > "$DAISYCLAW_CONFIG_PATH"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    DAISYCLAW_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    DAISYCLAW_HOME: isolatedHomeDir,
    DAISYCLAW_CONFIG_PATH: isolatedConfigPath,
    DAISYCLAW_STATE_DIR: isolatedStateDir,
...
export async function runTimedWatch(options, outputDir, deps = {}) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-gateway-watch-"));
