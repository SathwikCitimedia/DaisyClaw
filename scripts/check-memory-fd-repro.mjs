...
const SKIP_GATEWAY_ENV = {
  ...
  DAISYCLAW_SKIP_ACPX_RUNTIME: "1",
  DAISYCLAW_SKIP_ACPX_RUNTIME_PROBE: "1",
...
Options:
  ...
  --keep                         Keep the synthetic DAISYCLAW_HOME and workspace after the run.
...
export function parseArgs(argv) {
  ...
  const options = {
    fileCount: undefined,
    mode: process.env.DAISYCLAW_MEMORY_FD_REPRO_MODE || "fixed",
    ...
    keep: process.env.DAISYCLAW_MEMORY_FD_REPRO_KEEP === "1",
    allowNonDarwin: process.env.DAISYCLAW_MEMORY_FD_REPRO_ALLOW_NON_DARWIN === "1",
  };
  ...
  options.fileCount ??= readPositiveNumberEnv("DAISYCLAW_MEMORY_FD_REPRO_FILES", DEFAULT_FILE_COUNT);
  options.maxWorkspaceRegFds ??= readNumberEnv(
    "DAISYCLAW_MEMORY_FD_REPRO_MAX_WORKSPACE_REG_FDS",
    DEFAULT_MAX_WORKSPACE_REG_FDS,
  );
  options.invokeTimeoutMs ??= readPositiveNumberEnv("DAISYCLAW_MEMORY_FD_REPRO_TIMEOUT_MS", 30_000);
  options.sampleDelayMs ??= readNumberEnv("DAISYCLAW_MEMORY_FD_REPRO_SAMPLE_DELAY_MS", 1_000);
  options.settleDelayMs ??= readNumberEnv("DAISYCLAW_MEMORY_FD_REPRO_SETTLE_DELAY_MS", 5_000);
...
/**
 * Writes isolated DaisyClaw config for the synthetic memory workspace.
 */
export function writeConfig({ homeDir, workspaceDir, port, token }) {
  const configDir = path.join(homeDir, ".daisyclaw");
...
async function main() {
  ...
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-memory-fd-repro-"));
  ...
  const env = {
    ...
    ...SKIP_GATEWAY_ENV,
    HOME: homeDir,
    DAISYCLAW_STATE_DIR: path.join(homeDir, ".daisyclaw"),
