// Check No Monolithic Plugin Sdk Entry Imports script supports DaisyClaw repository automation.
import fs from "node:fs";
import path from "node:path";
import { discoverDaisyClawPlugins } from "../src/plugins/discovery.js";
...
const ROOT_IMPORT_PATTERN = /["']daisyclaw\/plugin-sdk["']/;
const LEGACY_COMPAT_IMPORT_PATTERN = /["']daisyclaw\/plugin-sdk\/compat["']/;
const LEGACY_BROAD_SUBPATH_PATTERNS = [
  {
    pattern: /["']daisyclaw\/plugin-sdk\/channel-runtime["']/,
    label: "daisyclaw/plugin-sdk/channel-runtime",
  },
  {
    pattern: /["']daisyclaw\/plugin-sdk\/config-runtime["']/,
    label: "daisyclaw/plugin-sdk/config-runtime",
  },
  {
    pattern: /["']daisyclaw\/plugin-sdk\/infra-runtime["']/,
    label: "daisyclaw/plugin-sdk/infra-runtime",
  },
] as const;
...
function main() {
  ...
  if (
    ...
    if (monolithicOffenders.length > 0) {
      console.error("Bundled plugin source files must not import monolithic daisyclaw/plugin-sdk.");
    ...
    if (legacyCompatOffenders.length > 0) {
      console.error(
        "Bundled plugin source files must not import legacy daisyclaw/plugin-sdk/compat.",
      );
    ...
    if (
      ...
      console.error(
        "Use focused daisyclaw/plugin-sdk/<domain> subpaths for bundled plugins; root, compat, and broad runtime barrels are legacy surfaces only.",
      );
    }
    process.exit(1);
  }
