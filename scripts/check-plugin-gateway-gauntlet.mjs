...
export function parseArgs(argv) {
  ...
  const options = {
    ...
    pluginIds: [],
    shardTotal: readOptionalPositiveIntEnv("DAISYCLAW_PLUGIN_GATEWAY_GAUNTLET_TOTAL") ?? 1,
    shardIndex: readOptionalNonNegativeIntEnv("DAISYCLAW_PLUGIN_GATEWAY_GAUNTLET_INDEX") ?? 0,
    limit: undefined,
    skipPrebuild: false,
    skipLifecycle: false,
    skipQa: false,
    ...
    qaTimeoutMs: 900_000,
    allowEmpty: false,
    failOnObservation: process.env.DAISYCLAW_PLUGIN_GATEWAY_GAUNTLET_FAIL_ON_OBSERVATION === "1",
    keepRunRoot: process.env.DAISYCLAW_PLUGIN_GATEWAY_GAUNTLET_KEEP_RUN_ROOT === "1",
  };
  const envIds = normalizeCsv(process.env.DAISYCLAW_PLUGIN_GATEWAY_GAUNTLET_IDS);
...
function daisyclawCommand(repoRoot, args) {
  return {
    command: process.execPath,
...
function sourceDaisyclawCommand(repoRoot, args) {
  return {
    command: process.execPath,
...
function createIsolatedEnv(repoRoot, runRoot) {
  ...
  return {
    ...
    DAISYCLAW_LOG_DIR: path.join(runRoot, "logs"),
    DAISYCLAW_QA_SUITE_PROGRESS: process.env.DAISYCLAW_QA_SUITE_PROGRESS ?? "1",
...
function buildSlashHelpProbe(params) {
  ...
  return {
    ...
    ...daisyclawCommand(params.repoRoot, [command, "--help"]),
    ...
    for (const { phase, args, alias } of commands) {
      ...
      params.rows.push(
        await runMeasuredCommand({
          ...
          ...daisyclawCommand(params.repoRoot, ["plugins", ...args]),
          label: `${plugin.id}-${phase}`,
...
async function runQaChunks(params) {
  ...
  for (let index = 0; index < chunks.length; index += 1) {
    ...
    const row = await runMeasuredCommand({
      ...
      ...sourceDaisyclawCommand(params.repoRoot, [
        "qa",
        "suite",
        "--provider-mode",
        "mock-openai",
...
async function main() {
  ...
  const runRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-plugin-gauntlet-"));
