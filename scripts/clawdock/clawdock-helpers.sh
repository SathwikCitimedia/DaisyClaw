#!/usr/bin/env bash
# ClawDock - Docker helpers for DaisyClaw
# Inspired by Simon Willison's "Running DaisyClaw in Docker"
# https://til.simonwillison.net/llms/daisyclaw-docker
#
# Installation:
...
# Common paths to check for DaisyClaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/daisyclaw"
  "${HOME}/workspace/daisyclaw"
  "${HOME}/projects/daisyclaw"
  "${HOME}/dev/daisyclaw"
  "${HOME}/code/daisyclaw"
  "${HOME}/src/daisyclaw"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found DaisyClaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/daisyclaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ DaisyClaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/daisyclaw/daisyclaw.git ~/daisyclaw"
    echo "  cd ~/daisyclaw && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^DAISYCLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d daisyclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart daisyclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f daisyclaw-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.daisyclaw"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show daisyclaw.json
  if [[ -f "${config_dir}/daisyclaw.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/daisyclaw.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/daisyclaw.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No daisyclaw.json found${_CLR_RESET}"
  fi
  echo ""
...
clawdock-workspace() {
  cd ~/.daisyclaw/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec daisyclaw-gateway \
    bash -c 'echo "alias daisyclaw=\"./daisyclaw.mjs\"" > /tmp/.bashrc_daisyclaw && bash --rcfile /tmp/.bashrc_daisyclaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm daisyclaw-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build daisyclaw-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d daisyclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build daisyclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "DAISYCLAW_GATEWAY_TOKEN=$token" daisyclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" daisyclaw-gateway \
    bash -c './daisyclaw.mjs config set gateway.remote.token "$TOKEN" && ./daisyclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec daisyclaw-gateway \
    bash -c "./daisyclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart daisyclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm daisyclaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec daisyclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'daisyclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for DaisyClaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (daisyclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to daisyclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.daisyclaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'daisyclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'daisyclaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.daisyclaw.ai${_CLR_RESET}"
  echo ""
}
