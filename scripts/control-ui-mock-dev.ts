// Control Ui Mock Dev script supports DaisyClaw repository automation.
...
function createChatPickerScenario(): ControlUiMockGatewayScenario {
  ...
  const workspaceListCases = ["main", "alpha", "daisyclaw-mock"].map((agentId) => ({
    match: { agentId },
    response: {
      agentId,
  ...
  const workspaceFileCases = ["main", "alpha", "daisyclaw-mock"].flatMap((agentId) =>
  ...
  return {
    assistantAgentId: "daisyclaw-mock",
    assistantName: "DaisyClaw mock",
    defaultAgentId: "daisyclaw-mock",
...
function createMockGatewayPlugin(scenario: ControlUiMockGatewayScenario): Plugin {
  ...
  return {
    ...
    name: "daisyclaw-control-ui-mock-gateway",
    transformIndexHtml(html) {
      return html.replace(
        "</head>",
        `    <script data-daisyclaw-control-ui-mock-gateway>\n${initScript}\n    </script>\n  </head>`,
      );
    },
  };
}
...
const server = await createServer({
  ...
  define: {
    DAISYCLAW_CONTROL_UI_BUILD_ID: JSON.stringify("mock"),
  },
  logLevel: "error",
  optimizeDeps: {
