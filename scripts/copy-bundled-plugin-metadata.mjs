...
function shouldCopyBundledPluginMetadata(id, env, buildablePluginDirs) {
  ...
  return env.DAISYCLAW_BUILD_PRIVATE_QA === "1";
}
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "daisyclaw.plugin.json");
    ...
    if (packageJson.daisyclaw && "extensions" in packageJson.daisyclaw) {
      packageJson.daisyclaw = {
        ...packageJson.daisyclaw,
        extensions: rewritePackageExtensions(packageJson.daisyclaw.extensions),
        ...(typeof packageJson.daisyclaw.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.daisyclaw.setupEntry) }
          : {}),
      };
    }
