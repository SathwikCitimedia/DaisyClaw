...
const ignoreRepoBinary = process.env.DAISYCLAW_CRABBOX_WRAPPER_IGNORE_REPO_BINARY === "1";
...
function shouldRequireBrokeredAws(commandArgs, providerName) {
  if (process.env.DAISYCLAW_CRABBOX_ALLOW_DIRECT_AWS === "1") {
    return false;
  }
...
function enforceBrokeredAws(commandArgs, providerName) {
  ...
  console.error(
    [
      "[crabbox] provider=aws requires a configured Crabbox broker for DaisyClaw proof.",
      "[crabbox] run `crabbox login --url https://crabbox.daisyclaw.ai --provider aws`, then retry.",
      "[crabbox] for intentional direct AWS provider debugging, set DAISYCLAW_CRABBOX_ALLOW_DIRECT_AWS=1.",
    ].join("\n"),
  );
  process.exit(2);
}
...
function remoteGitBootstrapForChangedGate(changedGateBase) {
  ...
  return [
    ...
    "rm -rf .git;",
    "git init -q;",
    "git remote add origin https://github.com/daisyclaw/daisyclaw.git 2>/dev/null || git remote set-url origin https://github.com/daisyclaw/daisyclaw.git;",
    `git fetch -q --depth=1 origin ${quotedBase}:refs/remotes/origin/main;`,
    "git reset --mixed --quiet refs/remotes/origin/main;",
    "git add -A;",
    "if ! git diff --cached --quiet; then git -c user.name=DaisyClaw -c user.email=ci@daisyclaw.local commit -q --no-gpg-sign -m remote-changed-gate-tree; fi;",
    "fi",
  ].join(" ");
}
...
function remoteWindowsHydratedNodeModulesBootstrap() {
  return [
    "$daisyclawModulesDir = $env:PNPM_CONFIG_MODULES_DIR",
    "if ($daisyclawModulesDir) {",
    'if (-not (Test-Path $daisyclawModulesDir)) { throw "PNPM_CONFIG_MODULES_DIR does not exist: $daisyclawModulesDir" }',
    '$daisyclawWorkspaceModules = Join-Path (Get-Location).Path "node_modules"',
    '$daisyclawSelfModules = Join-Path $daisyclawModulesDir "node_modules"',
    'if (-not (Test-Path $daisyclawSelfModules)) { cmd /c mklink /J "$daisyclawSelfModules" "$daisyclawModulesDir" | Out-Host; if ($LASTEXITCODE -ne 0) { throw "failed to link hydrated pnpm node_modules" } }',
    'if (-not (Test-Path $daisyclawWorkspaceModules)) { cmd /c mklink /J "$daisyclawWorkspaceModules" "$daisyclawModulesDir" | Out-Host; if ($LASTEXITCODE -ne 0) { throw "failed to link workspace node_modules" } }',
    "}",
  ].join("; ");
}
...
function remoteAwsMacosJsBootstrap({ packageManager = false } = {}) {
  const nodeVersion = process.env.DAISYCLAW_CRABBOX_MACOS_NODE_VERSION?.trim() || "24.15.0";
  const bootstrap = [
    "daisyclaw_crabbox_bootstrap_macos_js() {",
    'tool_root="${DAISYCLAW_CRABBOX_MACOS_TOOLCHAIN_DIR:-$HOME/.daisyclaw-crabbox-toolchain}";',
    ...
    'macos_locale="${DAISYCLAW_CRABBOX_MACOS_LOCALE:-en_US.UTF-8}";',
    ...
    'ready_marker="$node_dir/.daisyclaw-crabbox-node-ready";',
    ...
    "daisyclaw_crabbox_env() {",
    "daisyclaw_env_args=();",
    "daisyclaw_env_ignore=0;",
    "daisyclaw_env_path_seen=0;",
    'while [ "$#" -gt 0 ]; do',
    'case "$1" in',
    '-i|--ignore-environment) daisyclaw_env_ignore=1; daisyclaw_env_args+=("$1"); shift ;;',
    '-S|--split-string|-S*|--split-string=*) command env "${daisyclaw_env_args[@]}" "$@"; return ;;',
    '-[!-]*i*) daisyclaw_env_ignore=1; daisyclaw_env_args+=("$1"); shift ;;',
    '-u|--unset|-C|--chdir) daisyclaw_env_args+=("$1"); shift; if [ "$#" -gt 0 ]; then daisyclaw_env_args+=("$1"); shift; fi ;;',
    '--unset=*|--chdir=*) daisyclaw_env_args+=("$1"); shift ;;',
    'PATH=*) if [ "$daisyclaw_env_ignore" = "1" ]; then daisyclaw_env_args+=("PATH=$PATH:${1#PATH=}"); else daisyclaw_env_args+=("$1"); fi; daisyclaw_env_path_seen=1; shift ;;',
    '[A-Za-z_]*=*) daisyclaw_env_args+=("$1"); shift ;;',
    '--) daisyclaw_env_args+=("--"); shift; break ;;',
    "*) break ;;",
    "esac;",
    "done;",
    'if [ "$daisyclaw_env_ignore" = "1" ] && [ "$daisyclaw_env_path_seen" = "0" ]; then daisyclaw_env_args+=("PATH=$PATH"); fi;',
    'command env "${daisyclaw_env_args[@]}" "$@";',
    "};",
  ];
  if (packageManager) {
    bootstrap.push(
  ...
  bootstrap.push("};", "daisyclaw_crabbox_bootstrap_macos_js");
...
function scopedAwsMacosEnvCommand(commandArgs) {
  ...
  return {
    ...
    shellCommand: `daisyclaw_crabbox_env ${shellJoin(commandArgs.slice(1))}`,
  };
}
...
function prepareAwsMacosScriptStdinBootstrap(commandArgs, providerName) {
  ...
  const scriptRoot = mkdtempSync(resolve(tmpdir(), "daisyclaw-crabbox-macos-script-"));
...
function createAwsMacosScriptStdinWrapper(script) {
  ...
  return [
    ...
    'tmp_script="$(mktemp "${TMPDIR:-/tmp}/daisyclaw-crabbox-script.XXXXXX")" || exit $?',
    'cleanup_daisyclaw_crabbox_script() { rm -f "$tmp_script"; }',
    "trap cleanup_daisyclaw_crabbox_script EXIT",
...
function uniqueHereDocDelimiter(script) {
  let index = 0;
  for (;;) {
    const delimiterLocal = `DAISYCLAW_CRABBOX_SCRIPT_${index}`;
...
function defaultFullCheckoutSyncRoot() {
  const home = homedir();
  if (home) {
    return resolve(home, ".cache", "daisyclaw", "crabbox-sync");
  }
  return resolve(tmpdir(), "daisyclaw-crabbox-sync");
}

function fullCheckoutSyncRoot() {
  const configured = process.env.DAISYCLAW_CRABBOX_SYNC_TMPDIR?.trim();
...
function assertFullCheckoutSyncDisk(root) {
  const requiredBytes = parseNonNegativeIntegerEnv(
    "DAISYCLAW_CRABBOX_SYNC_MIN_FREE_BYTES",
    1024 * 1024 * 1024,
    "byte count",
  );
  if (requiredBytes === 0) {
  ...
  throw new Error(
    [
      ...
      "set DAISYCLAW_CRABBOX_SYNC_TMPDIR to a roomier filesystem or lower DAISYCLAW_CRABBOX_SYNC_MIN_FREE_BYTES if you know this checkout fits",
    ].join("; "),
  );
}
...
function prepareFullCheckoutForSync(options = {}) {
  ...
  const dir = mkdtempSync(resolve(syncRoot, "daisyclaw-crabbox-sync-"));
  let active = false;
...
function fullCheckoutKeepaliveIntervalMs() {
  return parseNonNegativeIntegerEnv(
    "DAISYCLAW_CRABBOX_SYNC_KEEPALIVE_MS",
    5000,
  ...
  if (!satisfiesMinimumCrabboxVersion(version.text, minimumBlacksmithCrabboxVersion)) {
    console.error(
      [
        ...
        "[crabbox] if using ../crabbox, rebuild it: version=$(git -C ../crabbox describe --tags --always --dirty | sed 's/^v//') && go build -C ../crabbox -trimpath -ldflags \"-s -w -X github.com/daisyclaw/crabbox/internal/cli.version=${version}\" -o bin/crabbox ./cmd/crabbox",
      ].join("\n"),
    );
    process.exit(2);
  }
...
if (
  ...
  if (isAwsMacosRemoteTarget(normalizedArgs, provider)) {
    ...
    console.error(
      `[crabbox] warning: provider=aws raw boxes may lack Node/Corepack/pnpm for ${runtimeEntrypoint}; hydrate first (${hydrate}) or pass --provider blacksmith-testbox for DaisyClaw CI-like proof; not switching providers automatically`,
    );
  }
}
...
if (
  ...
  console.error(
    "[crabbox] provider=docker enabling host Docker socket pass-through for DaisyClaw Docker tests",
  );
}
if (
  isLocalContainerProvider(provider) &&
  ...
  childEnv.CRABBOX_LOCAL_CONTAINER_WORK_ROOT = "/tmp/daisyclaw-crabbox-docker-work";
  console.error(
