...
const PUBLISHED_INSTALLER_BASE_URL = "https://daisyclaw.ai";
...
export const CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS = parsePositiveIntegerEnv(
  "DAISYCLAW_CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS",
  600,
);
...
const CROSS_OS_PROCESS_TREE_KILL_AFTER_MS = parsePositiveIntegerEnv(
  "DAISYCLAW_CROSS_OS_PROCESS_TREE_KILL_AFTER_MS",
  15_000,
);
...
export function resolveProviderConfig(provider, env = process.env) {
  ...
  const providerEnvKey = `DAISYCLAW_CROSS_OS_${provider.toUpperCase().replace(/[^A-Z0-9]+/gu, "_")}_MODEL`;
  const model = env[providerEnvKey]?.trim() || env.DAISYCLAW_CROSS_OS_MODEL?.trim() || config.model;
  return { ...config, model };
}
...
function buildReleaseProviderConfigOverride(providerMeta) {
  ...
  return {
    ...
    ...(providerMeta.extensionId === "openai" ? { agentRuntime: { id: "daisyclaw" } } : {}),
    models: [],
...
const INSTALL_STAGE_DEBRIS_DIR_PATTERN = /^\.daisyclaw-install-stage(?:-[^/]+)?$/iu;
...
export const CROSS_OS_DISCORD_FETCH_TIMEOUT_MS = parsePositiveIntegerEnv(
  "DAISYCLAW_CROSS_OS_DISCORD_FETCH_TIMEOUT_MS",
  10_000,
);
...
export const CROSS_OS_COMMAND_HEARTBEAT_SECONDS = parsePositiveIntegerEnv(
  "DAISYCLAW_CROSS_OS_COMMAND_HEARTBEAT_SECONDS",
  60,
);
...
export function resolveCrossOsAgentTurnOptional(env = process.env) {
  return parseBooleanEnv("DAISYCLAW_CROSS_OS_AGENT_TURN_OPTIONAL", false, env);
}
...
export function readRunnerOverrideEnv(env = process.env) {
  ...
  return {
    varUbuntuRunner: preferNonEmptyEnv(
      ...
      env.DAISYCLAW_RELEASE_CHECKS_UBUNTU_RUNNER,
    ),
    varWindowsRunner: preferNonEmptyEnv(
      env.VAR_WINDOWS_RUNNER,
      env.DAISYCLAW_RELEASE_CHECKS_WINDOWS_RUNNER,
    ),
    varMacosRunner: preferNonEmptyEnv(
      env.VAR_MACOS_RUNNER,
      env.DAISYCLAW_RELEASE_CHECKS_MACOS_RUNNER,
    ),
  };
}
...
async function main(argv) {
  ...
  const baselineSpec =
    ...
    (previousVersion ? `daisyclaw@${previousVersion}` : "daisyclaw@latest");
  ...
  const summary = {
    ...
    runnerOs: process.env.DAISYCLAW_RELEASE_CHECK_OS ?? "",
    runnerLabel: process.env.DAISYCLAW_RELEASE_CHECK_RUNNER ?? "",
    provider,
    mode,
    suite,
    ref: inputRef || null,
...
async function runUpgradeLane(params) {
  ...
  try {
    ...
    await runTimedLanePhase(lane, "update", async () => {
      try {
        updateResult = await runDaisyClaw({
          lane,
          env: updateEnv,
          args: updateArgs,
  ...
  try {
    ...
    await runInstalledCli({
      ...
      env: {
        ...
        DAISYCLAW_UPDATE_DEV_TARGET_REF: verificationRef,
      },
      cwd: lane.homeDir,
...
function createLaneState(name) {
  const rootDir = mkdtempSync(join(tmpdir(), `daisyclaw-${name}-`));
  const prefixDir = join(rootDir, "prefix");
  const homeDir = join(rootDir, "home");
  const stateDir = join(homeDir, ".daisyclaw");
...
function buildLaneEnv(lane, providerMeta, providerSecretValue) {
  ...
  return {
    ...
    DAISYCLAW_HOME: lane.homeDir,
    DAISYCLAW_STATE_DIR: lane.stateDir,
    DAISYCLAW_CONFIG_PATH: join(lane.stateDir, "daisyclaw.json"),
    DAISYCLAW_DISABLE_BONJOUR: "1",
...
export function buildRealUpdateEnv(env) {
  ...
  delete updateEnv.DAISYCLAW_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function isRecoverableWindowsPackagedUpgradeTimeoutError(
  ...
  return (
    ...
    /[/\\]daisyclaw\.mjs update --tag http:\/\/127\.0\.0\.1:\d+\/daisyclaw[^/\s]*\.tgz --yes --json(?: --no-restart)? --timeout \d+/u.test(
      message,
    )
  );
}
...
export function resolveExplicitBaselineVersion(baselineSpec) {
  ...
  if (!trimmed || trimmed === "daisyclaw@latest") {
    return "";
  }
  if (trimmed.startsWith("daisyclaw@")) {
    return trimmed.slice("daisyclaw@".length);
  }
  return trimmed;
}
...
async function resolveInstallerTargetVersion(params) {
  ...
  const latestResult = await runCommand(npmCommand(), ["view", "daisyclaw@latest", "version"], {
    logPath: join(params.logsDir, `${params.suiteName}-latest-version.log`),
    timeoutMs: 2 * 60 * 1000,
  });
  const latestVersion = latestResult.stdout.trim();
  if (!latestVersion) {
    throw new Error("npm view daisyclaw@latest version did not return a version.");
  }
...
if ($null -ne $npmCommand) {
  ...
  if (-not [string]::IsNullOrWhiteSpace($npmPrefix)) {
    ...
    foreach ($candidate in @(
      (Join-Path $npmPrefix 'daisyclaw.cmd'),
      (Join-Path $npmPrefix 'daisyclaw.ps1')
    )) {
...
if ([string]::IsNullOrWhiteSpace($commandPath)) {
  $cmd = Get-Command daisyclaw -ErrorAction Stop
...
Write-Output "__DAISYCLAW_PATH__=$commandPath"
...
async function verifyFreshShellCommand(params) {
  if (process.platform === "win32") {
    ...
    const cliPath = normalizeWindowsInstalledCliPath(
      parseMarkerLine(result.stdout, "__DAISYCLAW_PATH__="),
    );
    if (!cliPath) {
      throw new Error("Failed to resolve installed daisyclaw path from fresh Windows shell.");
    }
    return {
      cliPath,
  ...
  const script = [
    ...
    "command -v daisyclaw >/dev/null 2>&1",
    'printf "__DAISYCLAW_PATH__=%s\\n" "$(command -v daisyclaw)"',
    "daisyclaw --version",
  ].join("\n");
  ...
  const cliPath = parseMarkerLine(result.stdout, "__DAISYCLAW_PATH__=");
  const versionOutput = `${result.stdout}\n${result.stderr}`.trim();
  if (!cliPath) {
    throw new Error("Failed to resolve installed daisyclaw path from fresh POSIX shell.");
  }
...
async function ensureDevUpdateGitInstall(params) {
  ...
  // The dev-update lane must prove that `daisyclaw update --channel dev` landed on
...
async function configureDiscordSmoke(params) {
  ...
  if (!shouldUseManagedGatewayService()) {
    ...
    const gateway = await startManualGatewayFromInstalledCli({
      ...
      cliPath: params.cliPath,
      env: gatewayEnv,
      logPath: join(params.cwd, `.daisyclaw/logs/${params.lane.name}-discord-gateway.log`),
    });
...
export function dashboardHtmlMarkerStatus(html: string): {
  ...
  const title = html.includes("<title>DaisyClaw Control</title>");
  const app = html.includes("<daisyclaw-app></daisyclaw-app>");
...
async function maybeRunDiscordRoundtrip(params) {
  const token =
    process.env.DAISYCLAW_DISCORD_SMOKE_BOT_TOKEN?.trim() ||
    process.env.DISCORD_BOT_TOKEN?.trim() ||
    "";
  const guildId = process.env.DAISYCLAW_DISCORD_SMOKE_GUILD_ID?.trim() || "";
  const channelId = process.env.DAISYCLAW_DISCORD_SMOKE_CHANNEL_ID?.trim() || "";
...
async function runBundledPluginPostinstall(params) {
  ...
  delete installEnv.DAISYCLAW_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function buildInstalledBrowserOverrideImportProbeScript(
  runtimeModuleSpecifier = "daisyclaw/plugin-sdk/plugin-runtime",
) {
  return `
import { existsSync } from "node:fs";
import { startLazyPluginServiceModule } from ${JSON.stringify(runtimeModuleSpecifier)};

const startedPath = process.env.DAISYCLAW_BROWSER_OVERRIDE_STARTED_PATH;
const stoppedPath = process.env.DAISYCLAW_BROWSER_OVERRIDE_STOPPED_PATH;

if (!process.env.DAISYCLAW_BROWSER_CONTROL_MODULE) {
  throw new Error("Missing DAISYCLAW_BROWSER_CONTROL_MODULE.");
}
...
const handle = await startLazyPluginServiceModule({
  overrideEnvVar: "DAISYCLAW_BROWSER_CONTROL_MODULE",
...
export async function startBrowserControlService() {
  writeFileSync(process.env.DAISYCLAW_BROWSER_OVERRIDE_STARTED_PATH, "started\\n", "utf8");
}

export async function stopBrowserControlService() {
  writeFileSync(process.env.DAISYCLAW_BROWSER_OVERRIDE_STOPPED_PATH, "stopped\\n", "utf8");
}
`.trim();
}
...
async function runInstalledBrowserOverrideImportSmoke(params) {
  ...
  await runCommand(process.execPath, [probePath], {
    ...
    env: {
      ...
      DAISYCLAW_BROWSER_OVERRIDE_STARTED_PATH: startedPath,
      DAISYCLAW_BROWSER_OVERRIDE_STOPPED_PATH: stoppedPath,
    },
    logPath: params.logPath,
    timeoutMs: 60_000,
  });
...
async function runAgentTurn(params) {
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    ...
    try {
      const logOffset = readLogFileSize(params.logPath);
      const result = await runDaisyClaw({
        lane: params.lane,
        env: params.env,
        args: buildReleaseAgentTurnArgs(sessionId),
        logPath: params.logPath,
...
async function runDaisyClaw(params) {
...
function installedEntryPath(prefixDir) {
  return join(installedPackageRoot(prefixDir), "daisyclaw.mjs");
}
