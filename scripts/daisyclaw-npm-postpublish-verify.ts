...
// Daisyclaw Npm Postpublish Verify script supports DaisyClaw repository automation.
...
import { parseReleaseVersion, resolveNpmCommandInvocation } from "./daisyclaw-npm-release-check.ts";
...
export function buildPublishedInstallScenarios(version: string): PublishedInstallScenario[] {
  ...
  const exactSpec = `daisyclaw@${version}`;
  ...
  if (parsed.channel === "stable" && parsed.correctionNumber !== undefined) {
    scenarios.push({
      ...
      installSpecs: [`daisyclaw@${parsed.baseVersion}`, exactSpec],
      expectedVersion: version,
    });
  }
...
export function collectInstalledPluginSdkDeclarationErrors(packageRoot: string): string[] {
  ...
  const errors: string[] = [];
  const forbiddenPrivateWorkspaceSpecifiers = ["@daisyclaw/llm-core"];
...
export function resolveInstalledBinaryPath(prefixDir: string, platform = process.platform): string {
  return platform === "win32"
    ? pathWin32.join(prefixDir, "daisyclaw.cmd")
    : pathPosix.join(prefixDir, "bin", "daisyclaw");
}
...
function verifyScenario(version: string, scenario: PublishedInstallScenario): void {
  const workingDir = mkdtempSync(join(tmpdir(), `daisyclaw-postpublish-${scenario.name}.`));
  ...
  try {
    ...
    const packageRoot = join(globalRoot, "daisyclaw");
    ...
    if (normalizeInstalledBinaryVersion(installedBinaryVersion) !== scenario.expectedVersion) {
      errors.push(
        `installed daisyclaw binary version mismatch: expected ${scenario.expectedVersion}, found ${installedBinaryVersion || "<missing>"}.`,
      );
    }
    ...
    console.log(`daisyclaw-npm-postpublish-verify: ${scenario.name} OK (${version})`);
  } finally {
...
function main(): void {
  ...
  if (!version) {
    throw new Error(
      "Usage: node --import tsx scripts/daisyclaw-npm-postpublish-verify.ts <version>",
    );
  }
  ...
  console.log(
    `daisyclaw-npm-postpublish-verify: verified published npm install paths for ${version}.`,
  );
}
...
if (entrypoint !== null && import.meta.url === entrypoint) {
  try {
    ...
    console.error(`daisyclaw-npm-postpublish-verify: ${formatErrorMessage(error)}`);
    process.exitCode = 1;
  }
}
