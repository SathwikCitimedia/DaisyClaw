...
// Daisyclaw Npm Release Check script supports DaisyClaw repository automation.
...
const EXPECTED_REPOSITORY_URL = "https://github.com/daisyclaw/daisyclaw";
...
const skipPackValidationEnv = "DAISYCLAW_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function resolveNpmReleaseCheckCommandTimeoutMs(
  ...
  return positiveEnvInt(
    "DAISYCLAW_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS",
    env,
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "daisyclaw") {
    errors.push(`package.json name must be "daisyclaw"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.daisyclaw !== "daisyclaw.mjs") {
    errors.push(
      `package.json bin.daisyclaw must be "daisyclaw.mjs"; found "${pkg.bin?.daisyclaw ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`daisyclaw-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `daisyclaw-npm-release-check: validated ${channel} release ${pkg.version} (monthly patch version${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
