// Debug Claude Usage script supports DaisyClaw repository automation.
...
const loadAuthProfiles = (agentId: string) => {
  const stateRoot = process.env.DAISYCLAW_STATE_DIR?.trim() || path.join(os.homedir(), ".daisyclaw");
...
const resolveFetchTimeoutMs = (raw = process.env.DAISYCLAW_DEBUG_CLAUDE_USAGE_FETCH_TIMEOUT_MS) => {
  return parseStrictIntegerOption({
    fallback: DEFAULT_FETCH_TIMEOUT_MS,
    label: "DAISYCLAW_DEBUG_CLAUDE_USAGE_FETCH_TIMEOUT_MS",
    min: 1,
    raw,
  });
};
