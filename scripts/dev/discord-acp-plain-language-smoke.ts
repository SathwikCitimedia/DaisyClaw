#!/usr/bin/env bun
// Discord Acp Plain Language Smoke script supports DaisyClaw repository automation.
...
type DriverMode = "token" | "webhook" | "daisyclaw";
...
const DEFAULT_DAISYCLAW_CLI_TIMEOUT_MS = 60_000;
...
function resolveStateDir(): string {
  const override = process.env.DAISYCLAW_STATE_DIR?.trim();
  if (override) {
    if (override === "~") {
  ...
  const home = process.env.DAISYCLAW_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".daisyclaw");
}
...
function parseDriverMode(raw: string): DriverMode {
  ...
  if (normalized === "token" || normalized === "webhook" || normalized === "daisyclaw") {
    return normalized;
  }
  throw new Error(
    `Invalid --driver value ${JSON.stringify(raw)}; expected token, webhook, or daisyclaw.`,
  );
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver daisyclaw] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) DaisyClaw spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|daisyclaw> Driver transport mode (default: token)\n" +
    ...
    "  --state-dir <p>              Override DaisyClaw state dir for plugin-state polling\n" +
    "  --daisyclaw-bin <path>        DaisyClaw CLI binary for driver=daisyclaw (default: daisyclaw)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  DAISYCLAW_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  DAISYCLAW_DISCORD_SMOKE_DRIVER\n" +
    "  DAISYCLAW_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  DAISYCLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  DAISYCLAW_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  DAISYCLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  DAISYCLAW_DISCORD_SMOKE_AGENT\n" +
    "  DAISYCLAW_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  DAISYCLAW_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  DAISYCLAW_DISCORD_SMOKE_POLL_MS\n" +
    "  DAISYCLAW_STATE_DIR\n" +
    "  DAISYCLAW_DISCORD_SMOKE_DAISYCLAW_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.DAISYCLAW_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.DAISYCLAW_DISCORD_SMOKE_DRIVER || "token";
  const driverMode = parseDriverMode(driverModeRaw);
  const driverToken =
    resolveArg("--token") || process.env.DAISYCLAW_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.DAISYCLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.DAISYCLAW_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.DAISYCLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent = resolveArg("--agent") || process.env.DAISYCLAW_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.DAISYCLAW_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.DAISYCLAW_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.DAISYCLAW_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
    "--timeout-ms",
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.DAISYCLAW_DISCORD_SMOKE_POLL_MS,
    1_500,
    "--poll-ms",
  );
  const stateDir = path.resolve(resolveArg("--state-dir") || resolveStateDir());
  const daisyclawBin =
    resolveArg("--daisyclaw-bin") || process.env.DAISYCLAW_DISCORD_SMOKE_DAISYCLAW_BIN || "daisyclaw";
...
async function daisyclawCliJson<T>(params: {
  daisyclawBin: string;
  args: string[];
  timeoutMs?: number;
}): Promise<T> {
  const result = await execFileAsync(params.daisyclawBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
    timeout: params.timeoutMs ?? DEFAULT_DAISYCLAW_CLI_TIMEOUT_MS,
    killSignal: "SIGKILL",
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`daisyclaw ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithDaisyclaw(params: {
  daisyclawBin: string;
  target: string;
  limit: number;
  timeoutMs?: number;
}): Promise<DiscordMessage[]> {
  const response = await daisyclawCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    daisyclawBin: params.daisyclawBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function readThreadBindings(stateDir: string): Promise<ThreadBindingRecord[]> {
  const store = createPluginStateKeyedStore<ThreadBindingRecord>("discord", {
    ...
    env: { ...process.env, DAISYCLAW_STATE_DIR: stateDir },
  });
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "daisyclaw") {
    return await readMessagesWithDaisyclaw({
      daisyclawBin: params.args.daisyclawBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        timeoutMs: remainingTimeoutMs(deadline),
        body: {
          name: `daisyclaw-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      minBindingBoundAt = Date.now() - 3_000;
      const sent = await daisyclawCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
        ...
        daisyclawBin: args.daisyclawBin,
        args: [
          "message",
          "send",
          "--channel",
      ...
      if (!sentMessageId) {
        throw new Error("daisyclaw message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "daisyclaw"
            ? await readMessagesWithDaisyclaw({
                daisyclawBin: args.daisyclawBin,
                target: threadId,
                limit: 50,
                timeoutMs: remainingTimeoutMs(deadline),
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from DaisyClaw.`,
        diagnostics: {
          bindingCandidates: [
            {
