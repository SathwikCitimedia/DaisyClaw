// Realtime Talk Live Smoke script supports DaisyClaw repository automation.
...
const OPENAI_REALTIME_MODEL =
  process.env.DAISYCLAW_REALTIME_OPENAI_MODEL?.trim() || "gpt-realtime-2";
const OPENAI_REALTIME_VOICE = process.env.DAISYCLAW_REALTIME_OPENAI_VOICE?.trim() || "alloy";
const DEFAULT_OPENAI_HTTP_TIMEOUT_MS = 30_000;
const OPENAI_HTTP_RESPONSE_MAX_BYTES = 256 * 1024;
const GOOGLE_REALTIME_MODEL =
  process.env.DAISYCLAW_REALTIME_GOOGLE_MODEL?.trim() ||
  "gemini-2.5-flash-native-audio-preview-12-2025";
const GOOGLE_REALTIME_VOICE = process.env.DAISYCLAW_REALTIME_GOOGLE_VOICE?.trim() || "Kore";
...
function resolveOpenAIHttpTimeoutMs(
  raw = process.env.DAISYCLAW_REALTIME_OPENAI_HTTP_TIMEOUT_MS,
): number {
  return parseStrictIntegerOption({
    fallback: DEFAULT_OPENAI_HTTP_TIMEOUT_MS,
    label: "DAISYCLAW_REALTIME_OPENAI_HTTP_TIMEOUT_MS",
    min: 1,
    raw,
  });
}
...
async function smokeOpenAIBackendBridge(apiKey: string): Promise<SmokeResult> {
  ...
  const bridge = provider.createBridge({
    ...
    instructions: "DaisyClaw backend realtime live smoke. Do not speak yet.",
    onAudio: () => {},
    onClearAudio: () => {},
    onEvent: (event) => {
    ...
    config: {
      ...
      liveConnectConstraints: {
        ...
        config: {
          ...
          systemInstruction: "DaisyClaw browser Talk live smoke.",
...
async function smokeGatewayRelayBrowser(browser: Browser): Promise<SmokeResult> {
  ...
  const dir = await mkdtemp(path.join(tmpdir(), "daisyclaw-realtime-talk-"));
  try {
