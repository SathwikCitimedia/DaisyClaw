...
const DEFAULT_WORKFLOW = "daisyclaw-live-and-e2e-checks-reusable.yml";
...
function ghWorkflowCommand(lanes, ref, workflow, reuseInputs = {}) {
  const workflowRef = process.env.DAISYCLAW_DOCKER_E2E_WORKFLOW_REF || process.env.GITHUB_REF_NAME;
...
function main() {
  ...
  if (isLocalJson) {
    ...
    const outputDir =
      options.dir || path.join(os.tmpdir(), `daisyclaw-docker-e2e-rerun-${options.input}`);
