...
IMAGE_NAME="$(docker_e2e_resolve_image "daisyclaw-agents-delete-shared-workspace-e2e:local" DAISYCLAW_AGENTS_DELETE_SHARED_WORKSPACE_E2E_IMAGE)"
SKIP_BUILD="${DAISYCLAW_AGENTS_DELETE_SHARED_WORKSPACE_E2E_SKIP_BUILD:-0}"
DOCKER_COMMAND_TIMEOUT="${DAISYCLAW_AGENTS_DELETE_SHARED_WORKSPACE_DOCKER_COMMAND_TIMEOUT:-300s}"
DAISYCLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 agents-delete-shared-workspace empty)"
...
run_logged agents-delete-shared-workspace docker_e2e_docker_cmd run --rm \
  ...
  -e DAISYCLAW_SKIP_BROWSER_CONTROL_SERVER=1 \
  -e DAISYCLAW_SKIP_ACPX_RUNTIME=1 \
  -e DAISYCLAW_SKIP_ACPX_RUNTIME_PROBE=1 \
  -e DAISYCLAW_GATEWAY_TOKEN=agents-delete-shared-workspace-token \
...
mkdir -p "$DAISYCLAW_STATE_DIR" "$SHARED_WORKSPACE"
node scripts/e2e/lib/fixture.mjs agents-delete-config

run_daisyclaw agents delete ops --force --json > "$output_file"
