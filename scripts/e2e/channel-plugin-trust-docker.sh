...
#   mode against the installed DaisyClaw package.
#
# Parameters:
#   --container: run the in-container scenario. Host mode is the default.
#   DAISYCLAW_CHANNEL_PLUGIN_TRUST_E2E_IMAGE: override the Docker image name.
#   DAISYCLAW_CHANNEL_PLUGIN_TRUST_E2E_SKIP_BUILD=1: reuse/pull the image.
#
# Outputs:
...
Description:
  Proves the packaged DaisyClaw CLI enforces local channel plugin trust for
...
Environment:
  DAISYCLAW_CHANNEL_PLUGIN_TRUST_E2E_IMAGE       Override Docker image name.
  DAISYCLAW_CHANNEL_PLUGIN_TRUST_E2E_SKIP_BUILD  Reuse/pull image instead of building.
  DAISYCLAW_TEST_STATE_SCRIPT_B64                Required in --container mode.
...
Examples:
  ...
  DAISYCLAW_CHANNEL_PLUGIN_TRUST_E2E_SKIP_BUILD=1 bash scripts/e2e/channel-plugin-trust-docker.sh
EOF
}
...
run_daisyclaw() {
  ...
  if [ -f /app/daisyclaw.mjs ]; then
    node /app/daisyclaw.mjs "$@"
    return
  fi
  echo "daisyclaw CLI not found in Docker image" >&2
  exit 1
}
...
EOF
  ...
  cat >"$plugin_dir/daisyclaw.plugin.json" <<EOF
{
  "id": "$plugin_id",
...
run_case() {
  ...
  local trusted="${2:?missing trusted flag}"
  local scratch
  scratch="$(mktemp -d "/tmp/daisyclaw-channel-plugin-trust-$case_id.XXXXXX")"
  ...
  PLUGINTRUST_IMPORT_MARKER="$marker_dir/import.marker" \
    ...
    run_daisyclaw channels add --channel e2e-load-paths --token "$canary" \
...
run_container() {
  ...
  export DAISYCLAW_WORKSPACE_DIR="$HOME/.daisyclaw/workspace"

  run_daisyclaw --version
...
run_host() {
  ...
  image_name="$(
    docker_e2e_resolve_image \
      "daisyclaw-channel-plugin-trust-e2e:local" \
      DAISYCLAW_CHANNEL_PLUGIN_TRUST_E2E_IMAGE
  )"
  local skip_build="${DAISYCLAW_CHANNEL_PLUGIN_TRUST_E2E_SKIP_BUILD:-0}"
  ...
  docker_e2e_run_logged_print_with_harness \
    ...
    -e "DAISYCLAW_TEST_STATE_SCRIPT_B64=$state_script_b64" \
    "$image_name" \
    bash scripts/e2e/channel-plugin-trust-docker.sh --container
}
