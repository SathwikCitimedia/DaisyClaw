#!/usr/bin/env bash
# Installs DaisyClaw from a prepared package tarball, installs @daisyclaw/codex
...
CANDIDATE_ROOT="${DAISYCLAW_LIVE_DOCKER_REPO_ROOT:-$SCRIPT_ROOT_DIR}"
...
IMAGE_NAME="$(docker_e2e_resolve_image "daisyclaw-codex-npm-plugin-live-e2e" DAISYCLAW_CODEX_NPM_PLUGIN_E2E_IMAGE)"
DOCKER_TARGET="${DAISYCLAW_CODEX_NPM_PLUGIN_DOCKER_TARGET:-bare}"
HOST_BUILD="${DAISYCLAW_CODEX_NPM_PLUGIN_HOST_BUILD:-1}"
PACKAGE_TGZ="${DAISYCLAW_CURRENT_PACKAGE_TGZ:-}"
PROFILE_FILE="${DAISYCLAW_CODEX_NPM_PLUGIN_PROFILE_FILE:-${DAISYCLAW_TESTBOX_PROFILE_FILE:-$HOME/.daisyclaw-testbox-live.profile}}"
CODEX_PLUGIN_SPEC="${DAISYCLAW_CODEX_NPM_PLUGIN_SPEC:-}"
CODEX_PLUGIN_MOUNT=()
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${DAISYCLAW_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "DAISYCLAW_CODEX_NPM_PLUGIN_HOST_BUILD=0 requires DAISYCLAW_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
...
prepare_codex_plugin_spec() {
  ...
  if [ -z "$CODEX_PLUGIN_SPEC" ]; then
    CODEX_PLUGIN_PACK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/daisyclaw-codex-plugin-pack.XXXXXX")"
    (
      cd "$CANDIDATE_ROOT"
    ...
    ) >/tmp/daisyclaw-codex-plugin-pack.log 2>&1
    pack_output=()
    ...
    if [ "${#pack_output[@]}" -ne 1 ]; then
      ...
      docker_e2e_print_log /tmp/daisyclaw-codex-plugin-pack.log >&2
      exit 1
    fi
...
DAISYCLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 codex-npm-plugin-live empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e DAISYCLAW_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS="${DAISYCLAW_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS:-0}" \
  -e DAISYCLAW_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL="${DAISYCLAW_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL:-1}" \
  -e DAISYCLAW_CODEX_NPM_PLUGIN_MODEL="${DAISYCLAW_CODEX_NPM_PLUGIN_MODEL:-codex/gpt-5.4}" \
  -e DAISYCLAW_CODEX_NPM_PLUGIN_SPEC="$CODEX_PLUGIN_SPEC" \
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
...
CODEX_PLUGIN_SPEC="${DAISYCLAW_CODEX_NPM_PLUGIN_SPEC:?missing DAISYCLAW_CODEX_NPM_PLUGIN_SPEC}"
MODEL_REF="${DAISYCLAW_CODEX_NPM_PLUGIN_MODEL:?missing DAISYCLAW_CODEX_NPM_PLUGIN_MODEL}"
POST_UNINSTALL_MODEL_REF="codex/${MODEL_REF#*/}"
SESSION_ID="codex-npm-plugin-live"
SUCCESS_MARKER="DAISYCLAW-CODEX-NPM-PLUGIN-LIVE-OK"
PLUGIN_INSTALL_FLAGS=(--force)
if [ "${DAISYCLAW_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL:-0}" = "1" ]; then
...
dump_debug_logs() {
  ...
  daisyclaw_e2e_dump_logs \
    /tmp/daisyclaw-install.log \
    /tmp/daisyclaw-codex-plugin-install.log \
    /tmp/daisyclaw-codex-plugin-enable.log \
    /tmp/daisyclaw-codex-plugins-list.json \
    /tmp/daisyclaw-codex-plugin-inspect.json \
    /tmp/daisyclaw-codex-preflight.log \
    /tmp/daisyclaw-codex-agent.json \
    /tmp/daisyclaw-codex-agent.err \
    /tmp/daisyclaw-codex-agent-turn1.json \
    /tmp/daisyclaw-codex-agent-turn1.err \
    /tmp/daisyclaw-codex-agent-turn2.json \
    /tmp/daisyclaw-codex-agent-turn2.err \
    /tmp/daisyclaw-codex-plugin-uninstall.log \
    /tmp/daisyclaw-codex-plugins-list-after-uninstall.json \
    /tmp/daisyclaw-codex-agent-after-uninstall.json \
    /tmp/daisyclaw-codex-agent-after-uninstall.err
}
...
daisyclaw_e2e_install_package /tmp/daisyclaw-install.log
command -v daisyclaw >/dev/null
daisyclaw_e2e_enable_daisyclaw_cli_timeout

echo "Installing Codex plugin: $CODEX_PLUGIN_SPEC"
daisyclaw plugins install "$CODEX_PLUGIN_SPEC" "${PLUGIN_INSTALL_FLAGS[@]}" >/tmp/daisyclaw-codex-plugin-install.log 2>&1

node scripts/e2e/lib/codex-npm-plugin-live/assertions.mjs configure "$MODEL_REF"

echo "Enabling Codex plugin..."
daisyclaw plugins enable codex >/tmp/daisyclaw-codex-plugin-enable.log 2>&1

daisyclaw plugins list --json >/tmp/daisyclaw-codex-plugins-list.json
daisyclaw plugins inspect codex --runtime --json >/tmp/daisyclaw-codex-plugin-inspect.json
...
"$CODEX_BIN" exec \
  ...
  --color never \
  --skip-git-repo-check \
  "Reply exactly: ${SUCCESS_MARKER}-PREFLIGHT" >/tmp/daisyclaw-codex-preflight.log 2>&1 </dev/null
...
echo "Running DaisyClaw local agent turns through npm-installed Codex plugin..."
run_agent_turn \
  "turn1" \
  "${SUCCESS_MARKER}-TURN-1" \
  "Reply in one short sentence. Include token ${SUCCESS_MARKER}-TURN-1 and say hello from the DaisyClaw Codex plugin." \
  /tmp/daisyclaw-codex-agent-turn1.json \
  /tmp/daisyclaw-codex-agent-turn1.err
run_agent_turn \
  "turn2" \
...
daisyclaw plugins uninstall codex --force >/tmp/daisyclaw-codex-plugin-uninstall.log 2>&1
daisyclaw plugins list --json >/tmp/daisyclaw-codex-plugins-list-after-uninstall.json
node scripts/e2e/lib/codex-npm-plugin-live/assertions.mjs assert-uninstalled

if daisyclaw agent --local \
  --agent main \
  ...
  --thinking low \
  --timeout 120 \
  --json >/tmp/daisyclaw-codex-agent-after-uninstall.json 2>/tmp/daisyclaw-codex-agent-after-uninstall.err; then
  echo "Expected DaisyClaw agent to fail after Codex uninstall, got status 0" >&2
  exit 1
fi
if ! grep -Fq 'Requested agent harness "codex" is not registered' /tmp/daisyclaw-codex-agent-after-uninstall.err &&
  ! grep -Fq 'Unknown model: codex/' /tmp/daisyclaw-codex-agent-after-uninstall.err; then
  echo "Unexpected post-uninstall agent error:" >&2
  tail -n 120 /tmp/daisyclaw-codex-agent-after-uninstall.err >&2 || true
  exit 1
fi
