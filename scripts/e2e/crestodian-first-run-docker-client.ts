...
async function main() {
  ...
  const tempState = await createE2eStateDir("daisyclaw-crestodian-first-run-");
  ...
  assert(
    await shouldStartOnboardingForFreshInstall(["node", "daisyclaw"]),
    "fresh bare DaisyClaw invocation did not route to onboarding",
  );
  assert(
    shouldStartCrestodianForModernOnboard(["node", "daisyclaw", "onboard", "--modern"]),
    "modern onboard invocation did not route to Crestodian",
  );
  process.exitCode = undefined;
  await runCli(["node", "daisyclaw", "onboard", "--modern", "--non-interactive", "--json"]);
  assert(
