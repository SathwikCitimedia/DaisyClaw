...
import type { DaisyClawConfig } from "../../dist/config/types.daisyclaw.js";
...
function makeParams(commandBody: string, cfg: DaisyClawConfig, isGroup = false) {
  return {
    cfg,
    command: {
      surface: "whatsapp",
...
async function invoke(commandBody: string, cfg: DaisyClawConfig, isGroup = false): Promise<string> {
...
async function main() {
  const tempState = await createE2eStateDir("daisyclaw-crestodian-");
  tempState.registerExitCleanup();
  const stateDir = tempState.stateDir;
  const configPath = process.env.DAISYCLAW_CONFIG_PATH ?? path.join(stateDir, "daisyclaw.json");
  ...
  const refPlan = await invoke(
    "/crestodian config set-ref gateway.auth.token env DAISYCLAW_GATEWAY_TOKEN",
    cfg,
  );
  assert(
  ...
  const agentPlan = await invoke("/crestodian create agent work workspace /tmp/daisyclaw-work", cfg);
  assert(
  ...
  const setupPlan = await invoke(
    "/crestodian setup workspace /tmp/daisyclaw-setup model openai/gpt-5.2",
    cfg,
  );
  ...
  const updatedConfig = JSON.parse(await fs.readFile(configPath, "utf8")) as DaisyClawConfig;
  assert(
  ...
  assert(
    updatedConfig.gateway?.auth?.token &&
      ...
      updatedConfig.gateway.auth.token.id === "DAISYCLAW_GATEWAY_TOKEN",
    "SecretRef set did not update gateway.auth.token",
  );
  assert(
    updatedConfig.agents?.defaults?.workspace === "/tmp/daisyclaw-setup",
    "setup did not update default workspace",
  );
  assert(
    updatedConfig.agents?.list?.some(
      (agent) => agent.id === "work" && agent.workspace === "/tmp/daisyclaw-work",
    ),
