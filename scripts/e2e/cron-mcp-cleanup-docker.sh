...
IMAGE_NAME="$(docker_e2e_resolve_image "daisyclaw-cron-mcp-cleanup-e2e" DAISYCLAW_IMAGE)"
PORT="18789"
TOKEN="cron-mcp-e2e-$(date +%s)-$$"
CONTAINER_NAME="daisyclaw-cron-mcp-e2e-$$"
CLIENT_LOG="$(mktemp -t daisyclaw-cron-mcp-client-log.XXXXXX)"
...
DAISYCLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 cron-mcp-cleanup empty)"
...
docker_e2e_run_with_harness \
  ...
  bash -lc "set -euo pipefail
    ...
    export SUCCESS_MARKER=DAISYCLAW_CRON_MCP_CLEANUP_OK
    export MOCK_REQUEST_LOG=/tmp/daisyclaw-cron-mock-openai-requests.jsonl
    export DAISYCLAW_DOCKER_OPENAI_BASE_URL=\"http://127.0.0.1:\$MOCK_PORT/v1\"
    mock_pid=\"\$(daisyclaw_e2e_start_mock_openai \"\$MOCK_PORT\" /tmp/cron-mcp-cleanup-mock-openai.log)\"
    gateway_pid=
    cleanup_inner() {
    ...
    gateway_pid=\"\$(daisyclaw_e2e_start_gateway \"\$entry\" $PORT /tmp/cron-mcp-cleanup-gateway.log)\"
    daisyclaw_e2e_wait_gateway_ready \"\$gateway_pid\" /tmp/cron-mcp-cleanup-gateway.log 300 $PORT
