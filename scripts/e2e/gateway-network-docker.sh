...
IMAGE_NAME="$(docker_e2e_resolve_image "daisyclaw-gateway-network-e2e" DAISYCLAW_GATEWAY_NETWORK_E2E_IMAGE)"
SKIP_BUILD="${DAISYCLAW_GATEWAY_NETWORK_E2E_SKIP_BUILD:-0}"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="daisyclaw-net-e2e-$$"
GW_NAME="daisyclaw-gateway-e2e-$$"
DOCKER_COMMAND_TIMEOUT="${DAISYCLAW_GATEWAY_NETWORK_DOCKER_COMMAND_TIMEOUT:-600s}"
CLIENT_TIMEOUT="${DAISYCLAW_GATEWAY_NETWORK_CLIENT_TIMEOUT:-90s}"
...
docker_e2e_docker_cmd run -d \
  ...
  "$IMAGE_NAME" \
  bash -lc "set -euo pipefail; source scripts/lib/daisyclaw-e2e-instance.sh; entry=\"\$(daisyclaw_e2e_resolve_entrypoint)\"; node \"\$entry\" config set gateway.controlUi.enabled false >/dev/null; daisyclaw_e2e_exec_gateway \"\$entry\" $PORT lan /tmp/gateway-net-e2e.log" >/dev/null

echo "Waiting for gateway to come up..."
if ! docker_e2e_wait_container_bash "$GW_NAME" 180 0.5 "source scripts/lib/daisyclaw-e2e-instance.sh; daisyclaw_e2e_probe_tcp 127.0.0.1 $PORT"; then
