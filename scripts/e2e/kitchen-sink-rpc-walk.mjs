...
const PLUGIN_SPEC =
  process.env.DAISYCLAW_KITCHEN_SINK_NPM_SPEC || "npm:@daisyclaw/kitchen-sink@latest";
const PLUGIN_ID = process.env.DAISYCLAW_KITCHEN_SINK_PLUGIN_ID || "daisyclaw-kitchen-sink-fixture";
...
Runs the external Kitchen Sink plugin RPC walk against a built DaisyClaw entry.

Environment:
  DAISYCLAW_ENTRY                         Built DaisyClaw entrypoint. Defaults to dist/index.mjs or dist/index.js.
  DAISYCLAW_KITCHEN_SINK_NPM_SPEC         Plugin package spec. Default: npm:@daisyclaw/kitchen-sink@latest.
  DAISYCLAW_KITCHEN_SINK_PLUGIN_ID        Plugin id. Default: daisyclaw-kitchen-sink-fixture.
  DAISYCLAW_KITCHEN_SINK_RPC_READY_MS     Gateway readiness timeout.
  DAISYCLAW_KITCHEN_SINK_RPC_COMMAND_MS   DaisyClaw command timeout.
  DAISYCLAW_KITCHEN_SINK_RPC_INSTALL_MS   Plugin install timeout.
  DAISYCLAW_KITCHEN_SINK_RPC_CALL_MS      RPC call timeout.
  DAISYCLAW_KITCHEN_SINK_MAX_RSS_MIB      Gateway RSS ceiling.
  DAISYCLAW_KITCHEN_SINK_COMMAND_MAX_RSS_MIB  Install/CLI command RSS ceiling.
  DAISYCLAW_KITCHEN_SINK_KEEP_TMP=1       Preserve the isolated temp home.
`;
}
...
export function resolveKitchenSinkRpcConfig(env = process.env) {
  const commandTimeoutMs = readPositiveInt(
    env.DAISYCLAW_KITCHEN_SINK_RPC_COMMAND_MS,
    DEFAULT_COMMAND_TIMEOUT_MS,
    "DAISYCLAW_KITCHEN_SINK_RPC_COMMAND_MS",
  );
  return {
    commandMaxRssMiB: readPositiveInt(
      env.DAISYCLAW_KITCHEN_SINK_COMMAND_MAX_RSS_MIB,
      DEFAULT_MAX_COMMAND_RSS_MIB,
      "DAISYCLAW_KITCHEN_SINK_COMMAND_MAX_RSS_MIB",
    ),
    commandTimeoutMs,
    fetchBodyMaxBytes: readPositiveInt(
      env.DAISYCLAW_KITCHEN_SINK_RPC_FETCH_BODY_BYTES,
      DEFAULT_FETCH_BODY_MAX_BYTES,
      "DAISYCLAW_KITCHEN_SINK_RPC_FETCH_BODY_BYTES",
    ),
    fetchTimeoutMs: readPositiveInt(
      env.DAISYCLAW_KITCHEN_SINK_RPC_FETCH_MS,
      DEFAULT_FETCH_TIMEOUT_MS,
      "DAISYCLAW_KITCHEN_SINK_RPC_FETCH_MS",
    ),
    installTimeoutMs: readPositiveInt(
      env.DAISYCLAW_KITCHEN_SINK_RPC_INSTALL_MS,
      Math.max(commandTimeoutMs, DEFAULT_INSTALL_TIMEOUT_MS),
      "DAISYCLAW_KITCHEN_SINK_RPC_INSTALL_MS",
    ),
    maxRssMiB: readPositiveInt(
      env.DAISYCLAW_KITCHEN_SINK_MAX_RSS_MIB,
      DEFAULT_MAX_RSS_MIB,
      "DAISYCLAW_KITCHEN_SINK_MAX_RSS_MIB",
    ),
    outputCaptureChars: readPositiveInt(
      env.DAISYCLAW_KITCHEN_SINK_OUTPUT_CAPTURE_CHARS,
      DEFAULT_OUTPUT_CAPTURE_CHARS,
      "DAISYCLAW_KITCHEN_SINK_OUTPUT_CAPTURE_CHARS",
    ),
    readyTimeoutMs: readPositiveInt(
      env.DAISYCLAW_KITCHEN_SINK_RPC_READY_MS,
      DEFAULT_READY_TIMEOUT_MS,
      "DAISYCLAW_KITCHEN_SINK_RPC_READY_MS",
    ),
    rpcTimeoutMs: readPositiveInt(
      env.DAISYCLAW_KITCHEN_SINK_RPC_CALL_MS,
      DEFAULT_RPC_TIMEOUT_MS,
      "DAISYCLAW_KITCHEN_SINK_RPC_CALL_MS",
    ),
  };
}
...
export function makeEnv() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-kitchen-sink-rpc-"));
  ...
  return {
    ...
    env: {
      ...
      DAISYCLAW_KITCHEN_SINK_PERSONALITY:
        process.env.DAISYCLAW_KITCHEN_SINK_PERSONALITY || "conformance",
    },
  };
}
...
async function runDaisyClaw(runner, args, env, options = {}) {
  const config = resolveKitchenSinkRpcConfig(env);
  const command = await resolveDaisyClawCommand(runner, args, env, {
    stdio: ["ignore", "pipe", "pipe"],
  });
...
async function resolveDaisyClawCommand(runner, args, env, options = {}) {
  if (runner.pnpm) {
...
async function rpcCall(method, params, options) {
  ...
  const payload = module
    ? await module.callGateway({
        config: readJson(options.env.DAISYCLAW_CONFIG_PATH),
        configPath: options.env.DAISYCLAW_CONFIG_PATH,
...
async function loadCallGatewayModule(runner) {
  if (!usesBuiltDaisyClawEntry(runner)) {
    return null;
  }
...
async function rpcCallViaCli(method, params, options) {
  const config = resolveKitchenSinkRpcConfig(options.env);
  const { stdout } = await runDaisyClaw(
    options.runner,
    [
      "gateway",
      "call",
...
export function usesBuiltDaisyClawEntry(runner, cwd = process.cwd(), env = process.env) {
  if (runner?.pnpm || !runner?.baseArgs?.[0]) {
    return false;
  }
  const entry = runner.baseArgs[0];
  if (env.DAISYCLAW_ENTRY && entry === env.DAISYCLAW_ENTRY) {
    return true;
  }
...
function configureKitchenSink(env, port) {
  const configPath = env.DAISYCLAW_CONFIG_PATH;
    ...
    entries: {
      ...
      [PLUGIN_ID]: {
        ...
        config: {
          ...
          personality: env.DAISYCLAW_KITCHEN_SINK_PERSONALITY,
        },
        hooks: {
...
async function samplePosixProcessTree(pid, run, commandLineNeedles) {
  ...
  try {
    ...
    const gatewayTitleMatches = descendants.filter((row) =>
      row.command.toLowerCase().includes("daisyclaw-gateway"),
    );
...
export async function main() {
  const config = resolveKitchenSinkRpcConfig();
  let runner = resolveDaisyClawRunner();
  const port = readPositiveInt(
    process.env.DAISYCLAW_KITCHEN_SINK_RPC_PORT,
    DEFAULT_PORT,
    "DAISYCLAW_KITCHEN_SINK_RPC_PORT",
  );
  const { root, env } = makeEnv();
  const logPath = path.join(root, "gateway.log");
  const keepTmp = process.env.DAISYCLAW_KITCHEN_SINK_KEEP_TMP === "1";
  let failed = false;
  let child;
  ...
  try {
    ...
    const inspect = parseJsonOutput(
      (
        await runDaisyClaw(runner, ["plugins", "inspect", PLUGIN_ID, "--runtime", "--json"], env, {
          ...commandResourceOptions,
    ...
    assertKitchenSinkUiDescriptors(uiDescriptors, {
      expectDescriptor: env.DAISYCLAW_KITCHEN_SINK_PERSONALITY !== "conformance",
    });
