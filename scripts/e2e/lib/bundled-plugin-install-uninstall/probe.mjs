...
function resolveStateDir() {
  if (process.env.DAISYCLAW_STATE_DIR) {
    return process.env.DAISYCLAW_STATE_DIR;
  }
  return path.join(process.env.HOME || os.homedir(), ".daisyclaw");
}
...
function resolveDaisyClawEntry() {
  if (process.env.DAISYCLAW_ENTRY) {
    return process.env.DAISYCLAW_ENTRY;
  }
  ...
  throw new Error("Missing DAISYCLAW_ENTRY and dist/index.(m)js");
}

function readPluginsList() {
  const entry = resolveDaisyClawEntry();
  const timeoutMs = readPositiveIntEnv(
    "DAISYCLAW_BUNDLED_PLUGIN_LIST_TIMEOUT_MS",
  ...
  const result = spawnSync(process.execPath, [entry, "plugins", "list", "--json"], {
    ...
    maxBuffer: readPositiveIntEnv(
      "DAISYCLAW_BUNDLED_PLUGIN_LIST_MAX_BUFFER_BYTES",
...
async function loadManifestEntries() {
  const explicit = (process.env.DAISYCLAW_BUNDLED_PLUGIN_SWEEP_IDS || "")
    .split(/[,\s]+/u)
  ...
  return explicit.map((lookup) => {
    ...
    if (!found) {
      throw new Error(
        `DAISYCLAW_BUNDLED_PLUGIN_SWEEP_IDS entry is not an installable bundled plugin in this package: ${lookup}. Available: ${available}`,
      );
    }
    return found;
  });
...
async function selectedManifestEntries() {
  ...
  const total = readPositiveIntEnv("DAISYCLAW_BUNDLED_PLUGIN_SWEEP_TOTAL", 1);
  const index = readNonNegativeIntEnv("DAISYCLAW_BUNDLED_PLUGIN_SWEEP_INDEX", 0);
  if (index >= total) {
    throw new Error(
      `DAISYCLAW_BUNDLED_PLUGIN_SWEEP_INDEX must be in [0, ${total - 1}], got ${process.env.DAISYCLAW_BUNDLED_PLUGIN_SWEEP_INDEX}`,
    );
  }
