...
const OUTPUT_CAPTURE_CHARS = readPositiveIntEnv(
  "DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_OUTPUT_CHARS",
  1024 * 1024,
);
const LOG_SCAN_BYTES = readPositiveIntEnv(
  "DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_LOG_SCAN_BYTES",
  256 * 1024,
);
const GATEWAY_LOG_CAPTURE_BYTES = readPositiveIntEnv(
  "DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_GATEWAY_LOG_BYTES",
  16 * 1024 * 1024,
);
const WATCHDOG_MS = readPositiveIntEnv("DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_WATCHDOG_MS", 1000);
const READY_TIMEOUT_MS = readPositiveIntEnv("DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_READY_MS", 900000);
const RPC_TIMEOUT_MS = readPositiveIntEnv("DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_RPC_MS", 60000);
const RPC_READY_TIMEOUT_MS = readPositiveIntEnv(
  "DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_RPC_READY_MS",
  210000,
);
const COMMAND_TIMEOUT_MS = readPositiveIntEnv("DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_COMMAND_MS", 120000);
const HTTP_PROBE_TIMEOUT_MS = readPositiveIntEnv("DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_HTTP_MS", 5000);
const GATEWAY_TEARDOWN_GRACE_MS = readPositiveIntEnv(
  "DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_TEARDOWN_GRACE_MS",
  10000,
);
const GATEWAY_TEARDOWN_KILL_GRACE_MS = readPositiveIntEnv(
  "DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_TEARDOWN_KILL_GRACE_MS",
  1000,
);
...
function manifestPath(pluginDir, pluginRoot) {
  const candidates = [
    ...(isNonEmptyString(pluginRoot) ? [path.join(pluginRoot, "daisyclaw.plugin.json")] : []),
    path.join(process.cwd(), "dist", "extensions", pluginDir, "daisyclaw.plugin.json"),
    path.join(process.cwd(), "dist-runtime", "extensions", pluginDir, "daisyclaw.plugin.json"),
  ];
...
function configPathFromEnv(env = process.env) {
  return (
    env.DAISYCLAW_CONFIG_PATH || path.join(env.HOME || os.homedir(), ".daisyclaw", "daisyclaw.json")
  );
}
  ...
  const child = childProcess.spawn(
    ...
    ],
    {
      env: {
        ...
        DAISYCLAW_SKIP_CHANNELS: params.skipChannels ? "1" : "0",
...
export async function rpcCall(method, params, options) {
  const rpcStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-plugin-runtime-rpc-"));
  const args = [
    options.entrypoint,
    "gateway",
    "call",
  ...
  try {
    const { stdout } = await runCommand("node", args, {
      env: {
        ...
        ...options.env,
        DAISYCLAW_NO_ONBOARD: "1",
        DAISYCLAW_STATE_DIR: rpcStateDir,
      },
    });
...
async function smokePlugin(pluginId, pluginDir, requiresConfig, pluginIndex, pluginRoot) {
  ...
  const entrypoint = process.env.DAISYCLAW_ENTRY;
  if (!entrypoint) {
    throw new Error("missing DAISYCLAW_ENTRY");
  }
  const manifest = loadManifest(pluginDir, pluginRoot);
  const plan = buildPluginPlan(manifest);
  const port =
    readPositiveIntEnv("DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_PORT_BASE", 19000) + pluginIndex * 3;
  ...
  const logPath = `/tmp/daisyclaw-plugin-runtime-${pluginIndex}-${pluginId}.log`;
...
async function smokeTtsGlobalDisable(pluginId, pluginDir, provider, pluginIndex, pluginRoot) {
  ...
  const port =
    readPositiveIntEnv("DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_PORT_BASE", 19000) + pluginIndex * 3 + 1;
  ...
  const logPath = `/tmp/daisyclaw-plugin-runtime-${pluginIndex}-${pluginId}-tts-disabled.log`;
...
async function smokeOpenAiTts(pluginIndex) {
  ...
  const port =
    readPositiveIntEnv("DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_PORT_BASE", 19000) + pluginIndex * 3 + 2;
  ...
  const logPath = `/tmp/daisyclaw-plugin-runtime-${pluginIndex}-openai-tts-live.log`;
  const child = startGateway({ entrypoint, port, logPath, env, skipChannels: true });
  try {
    await waitForReady({ child, port, logPath });
    await assertBaseGatewayProbes({ entrypoint, port, env });
...
export function createIsolatedStateEnv(label) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `daisyclaw-${label}-`));
