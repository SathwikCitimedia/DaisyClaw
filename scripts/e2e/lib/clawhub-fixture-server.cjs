...
const packageName = "@daisyclaw/kitchen-sink";
...
async function buildNpmPackArtifact(fixture) {
  const packRoot = await fs.promises.mkdtemp(path.join(os.tmpdir(), "daisyclaw-clawhub-fixture-"));
  try {
...
`,
    ...
    packageDetail(artifact) {
      ...
      const packageDetail = {
        package: {
          name: packageName,
          displayName: "DaisyClaw Kitchen Sink",
          family: "code-plugin",
          runtimeId: pluginId,
          channel: "official",
          isOfficial: true,
          summary: "Kitchen sink plugin fixture for prerelease CI.",
          ownerHandle: "daisyclaw",
          createdAt: 0,
          updatedAt: 0,
          ...
          verification: {
            tier: "source-linked",
            sourceRepo: "https://github.com/daisyclaw/kitchen-sink",
            hasProvenance: false,
            scanStatus: "passed",
          },
...
async function main() {
  ...
  zip.file("package/daisyclaw.plugin.json", manifestJson, { date: new Date(0) });
  ...
  const artifactResolverDetail = {
    package: versionDetail.package ?? {
      name: packageName,
      displayName: packageDetail.package?.displayName ?? "DaisyClaw Kitchen Sink",
