...
const allowBetaCompatDiagnostics =
  process.env.DAISYCLAW_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS === "1";
const MAX_TEXT_FILE_BYTES = readPositiveIntEnv(
  "DAISYCLAW_CODEX_NPM_PLUGIN_ASSERT_MAX_TEXT_FILE_BYTES",
  1024 * 1024,
);
const MAX_ERROR_TAIL_BYTES = readPositiveIntEnv(
  "DAISYCLAW_CODEX_NPM_PLUGIN_ASSERT_MAX_ERROR_TAIL_BYTES",
  64 * 1024,
);
const MAX_TRANSCRIPT_FILES = readPositiveIntEnv(
  "DAISYCLAW_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_FILES",
  64,
);
const MAX_TRANSCRIPT_WALK_ENTRIES = readPositiveIntEnv(
  "DAISYCLAW_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_WALK_ENTRIES",
  4096,
);
const MAX_TRANSCRIPT_SCAN_BYTES = readPositiveIntEnv(
  "DAISYCLAW_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_SCAN_BYTES",
  2 * 1024 * 1024,
);
...
function assertPlugin() {
  const spec = process.argv[3] || "npm:@daisyclaw/codex";
  const list = readJson("/tmp/daisyclaw-codex-plugins-list.json");
  const inspect = readJson("/tmp/daisyclaw-codex-plugin-inspect.json");
...
function codexNpmProjectRoot() {
  return npmProjectRootForInstalledPackage(codexInstallPath(), "@daisyclaw/codex");
}
...
function assertNpmDeps() {
  ...
  if (!fs.existsSync(pluginPackageJson)) {
    throw new Error(`missing npm-installed @daisyclaw/codex package.json: ${pluginPackageJson}`);
  }
  ...
  if (pluginPackage.name !== "@daisyclaw/codex") {
  ...
  if (!openAiCodexPackageJson) {
    throw new Error("missing @openai/codex dependency under .daisyclaw/npm");
  }
...
function assertPreflight() {
  ...
  const output = readTextFileBounded("/tmp/daisyclaw-codex-preflight.log", "Codex preflight log");
...
function assertAgentTurn() {
  ...
  const stdout = readTextFileBounded("/tmp/daisyclaw-codex-agent.json", "DaisyClaw agent JSON");
  const stderr = readTextFileTail("/tmp/daisyclaw-codex-agent.err", "DaisyClaw agent stderr");
  const response = JSON.parse(stdout);
  const text = extractAgentReplyTexts(JSON.stringify(response)).join("\n");
  if (!text.includes(marker)) {
    throw new Error(
      `DaisyClaw agent reply did not contain ${marker}:\nstdout=${stdout}\nstderr=${stderr}`,
    );
  }
  ...
  if (typeof entry.sessionFile !== "string" || !fs.existsSync(entry.sessionFile)) {
    throw new Error(`missing DaisyClaw session file: ${entry.sessionFile}`);
  }
...
function assertUninstalled() {
  ...
  const list = readJson("/tmp/daisyclaw-codex-plugins-list-after-uninstall.json");
  const plugin = (list.plugins || []).find((entry) => entry.id === "codex");
  if (plugin?.status === "loaded" || plugin?.enabled === true) {
...
function assertAgentError() {
  ...
  if (!Number.isInteger(status) || status === 0) {
    throw new Error(
      `expected DaisyClaw agent to fail after Codex uninstall, got status ${process.argv[3]}`,
    );
  }
  const stdout = fs.existsSync("/tmp/daisyclaw-codex-agent-after-uninstall.json")
    ? readTextFileTail(
        "/tmp/daisyclaw-codex-agent-after-uninstall.json",
        "post-uninstall agent stdout",
      )
    : "";
  const stderr = fs.existsSync("/tmp/daisyclaw-codex-agent-after-uninstall.err")
    ? readTextFileTail(
