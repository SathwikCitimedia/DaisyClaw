...
const inspect = readJson("/tmp/daisyclaw-codex-inspect.json");
...
if (!String(codexRecord.spec || "").includes("@daisyclaw/codex")) {
  throw new Error(`expected @daisyclaw/codex install spec, got ${codexRecord.spec}`);
}
...
if (!fs.existsSync(codexPackageJson)) {
  throw new Error(`missing npm-installed @daisyclaw/codex package: ${codexPackageJson}`);
}
const codexPackage = readJson(codexPackageJson);
if (codexPackage.name !== "@daisyclaw/codex") {
  throw new Error(`unexpected codex package name: ${codexPackage.name}`);
}

const npmProjectRoot = npmProjectRootForInstalledPackage(installPath, "@daisyclaw/codex");
...
function readAuthProfileStoreText(agentDir) {
  const dbPath = path.join(agentDir, "daisyclaw-agent.sqlite");
...
assertOpenAiEnvAuthProfileStore(authRaw, {
  ...
  rawKeyNeedle: "sk-daisyclaw-codex-on-demand-e2e",
});
