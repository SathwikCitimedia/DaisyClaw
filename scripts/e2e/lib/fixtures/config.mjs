...
function writeConfig(kind) {
  const configPath = requireArg(process.env.DAISYCLAW_CONFIG_PATH, "DAISYCLAW_CONFIG_PATH");
      ...
      : kind === "browser-cdp"
        ? {
            gateway: {
              ...
              auth: {
                mode: "token",
                token: requireArg(process.env.DAISYCLAW_GATEWAY_TOKEN, "DAISYCLAW_GATEWAY_TOKEN"),
              },
              controlUi: { enabled: false },
            },
            browser: {
...
function writeOpenAiWebSearchMinimalConfig() {
  writeJson(path.join(process.env.DAISYCLAW_STATE_DIR, "daisyclaw.json"), {
    agents: {
      defaults: {
    ...
    gateway: { auth: { mode: "token", token: process.env.DAISYCLAW_GATEWAY_TOKEN } },
  });
}

function writeOpenWebUiConfig([openaiApiKey]) {
  const batchPath = requireArg(
    process.env.DAISYCLAW_CONFIG_BATCH_PATH,
    "DAISYCLAW_CONFIG_BATCH_PATH",
  );
  writeJson(batchPath, [
    ...
    { path: "models.providers.openai.models", value: [] },
    {
      ...
      value: readPositiveIntEnv("DAISYCLAW_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS", 900),
    },
    { path: "models.providers.openai.agentRuntime", value: { id: "daisyclaw" } },
    { path: "gateway.controlUi.enabled", value: false },
    { path: "gateway.mode", value: "local" },
    { path: "gateway.bind", value: "lan" },
    { path: "gateway.auth.mode", value: "token" },
    { path: "gateway.auth.token", value: process.env.DAISYCLAW_GATEWAY_TOKEN },
    { path: "gateway.http.endpoints.chatCompletions.enabled", value: true },
    { path: "agents.defaults.model.primary", value: process.env.DAISYCLAW_OPENWEBUI_MODEL },
  ]);
}
