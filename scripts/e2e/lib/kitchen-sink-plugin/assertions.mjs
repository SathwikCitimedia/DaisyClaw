...
function shouldScanLogFile(entry) {
  if (!(/\.(?:log|jsonl)$/u.test(entry) || /daisyclaw-kitchen-sink-/u.test(path.basename(entry)))) {
    return false;
  }
...
function scanLogs() {
  ...
  const roots = [scratchRoot, path.join(process.env.HOME, ".daisyclaw")];
  const deny = [
  ...
  if (scannedFiles === 0) {
    throw new Error(
      "kitchen-sink log scan found no files under the isolated scratch root or DaisyClaw home",
    );
  }
...
function assertClawHubExternalInstallContract(installPath) {
  ...
  if (!fs.existsSync(daisyclawPeerPath)) {
    throw new Error(`missing kitchen-sink daisyclaw peer symlink: ${daisyclawPeerPath}`);
  }
  if (!fs.lstatSync(daisyclawPeerPath).isSymbolicLink()) {
    throw new Error(`kitchen-sink daisyclaw peer is not a symlink: ${daisyclawPeerPath}`);
  }
  ...
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected kitchen-sink daisyclaw peer ${linkedHostRoot} to target ${hostRoot}`);
  }
