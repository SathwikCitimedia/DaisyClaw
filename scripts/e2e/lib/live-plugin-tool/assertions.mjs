...
const agentTurnTimeoutSeconds = readPositiveIntEnv(
  "DAISYCLAW_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS",
  300,
);
...
const AGENT_OUTPUT_MAX_BYTES = readPositiveIntEnv(
  "DAISYCLAW_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_MAX_BYTES",
  1024 * 1024,
);
const SESSION_FILE_LIST_LIMIT = 20;
const SESSION_SCAN_MAX_ENTRIES = readPositiveIntEnv(
  "DAISYCLAW_LIVE_PLUGIN_TOOL_SESSION_SCAN_MAX_ENTRIES",
  50_000,
);
...
function stateDir() {
  return process.env.DAISYCLAW_STATE_DIR || path.join(process.env.HOME, ".daisyclaw");
}

function configPath() {
  return process.env.DAISYCLAW_CONFIG_PATH || path.join(stateDir(), "daisyclaw.json");
}

function agentOutputPath() {
  return process.env.DAISYCLAW_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_PATH || "/tmp/daisyclaw-agent.json";
}

function agentErrorPath() {
  return process.env.DAISYCLAW_LIVE_PLUGIN_TOOL_AGENT_ERROR_PATH || "/tmp/daisyclaw-agent.err";
}
...
function pluginInstallPath() {
  ...
  const inspect = fs.existsSync("/tmp/daisyclaw-plugin-inspect.json")
    ? readJson("/tmp/daisyclaw-plugin-inspect.json")
    : {};
...
function writeFixture() {
  ...
  writeJson(path.join(dir, "daisyclaw.plugin.json"), {
    id: pluginId,
    name: "E2E Slug Tool",
...
function assertInstalled() {
  ...
  const list = readJson("/tmp/daisyclaw-plugins-list.json");
  ...
  const inspect = readJson("/tmp/daisyclaw-plugin-inspect.json");
