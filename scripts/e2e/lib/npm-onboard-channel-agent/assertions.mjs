...
function assertOnboardState() {
  ...
  if (!fs.existsSync(configPath)) {
    throw new Error("onboard did not write daisyclaw.json");
  }
  ...
  assertOpenAiEnvAuthProfileStore(authStoreText, {
    ...
    rawKeyNeedle: "sk-daisyclaw-npm-onboard-e2e",
  });
}
...
function assertMockModelConfig() {
  ...
  if (provider?.agentRuntime?.id !== "daisyclaw") {
  ...
  if (agent && agentRuntime !== "daisyclaw") {
...
function assertAgentTurn() {
  ...
  assertAgentReplyContainsMarker(marker, "/tmp/daisyclaw-agent.combined");
