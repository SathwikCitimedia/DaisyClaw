...
DAISYCLAW_ONBOARD_SCENARIO_SOURCE_ONLY="${DAISYCLAW_ONBOARD_SCENARIO_SOURCE_ONLY:-0}"
if [ "$DAISYCLAW_ONBOARD_SCENARIO_SOURCE_ONLY" != "1" ]; then
...
if [ -z "${DAISYCLAW_ENTRY:-}" ] && [ "$DAISYCLAW_ONBOARD_SCENARIO_SOURCE_ONLY" != "1" ]; then
  DAISYCLAW_ENTRY="$(daisyclaw_e2e_resolve_entrypoint)"
fi
export DAISYCLAW_ENTRY
ONBOARD_TMP_ROOT="${DAISYCLAW_ONBOARD_E2E_TMPDIR:-${TMPDIR:-/tmp}}"
ONBOARD_TMP_ROOT="${ONBOARD_TMP_ROOT%/}"
[ -n "$ONBOARD_TMP_ROOT" ] || ONBOARD_TMP_ROOT="/tmp"
mkdir -p "$ONBOARD_TMP_ROOT"
ONBOARD_TMP_DIR="$(mktemp -d "$ONBOARD_TMP_ROOT/daisyclaw-onboard.XXXXXX")"
DAISYCLAW_E2E_LOG_DIR="$ONBOARD_TMP_DIR/logs"
GATEWAY_LOG_PATH="$ONBOARD_TMP_DIR/gateway-e2e.log"
export DAISYCLAW_E2E_LOG_DIR
export GATEWAY_LOG_PATH
mkdir -p "$DAISYCLAW_E2E_LOG_DIR"
...
start_gateway() {
  GATEWAY_PID="$(daisyclaw_e2e_start_gateway "$DAISYCLAW_ENTRY" 18789 "$GATEWAY_LOG_PATH")"
}

wait_for_gateway() {
  local wait_attempts="${DAISYCLAW_ONBOARD_GATEWAY_WAIT_ATTEMPTS:-20}"
  local wait_interval_s="${DAISYCLAW_ONBOARD_GATEWAY_WAIT_INTERVAL_S:-1}"
  local saw_listening_log="false"
  for _ in $(seq 1 "$wait_attempts"); do
    if daisyclaw_e2e_probe_tcp 127.0.0.1 18789 500 >/dev/null 2>&1; then
      return 0
    fi
...
cleanup_wizard_case() {
  ...
  daisyclaw_e2e_stop_process "${wizard_pid:-}"
  stop_gateway "${gw_pid:-}"
...
run_wizard_cmd() {
  ...
  set_isolated_daisyclaw_env "$state_ref"
  ...
  local log_path="$DAISYCLAW_E2E_LOG_DIR/${case_name}.log"
  WIZARD_LOG_PATH="$log_path"
  export WIZARD_LOG_PATH
  # Run under script to keep an interactive TTY for clack prompts.
  daisyclaw_e2e_run_script_with_pty "$command" "$log_path" <"$input_fifo" >/dev/null 2>&1 &
  wizard_pid=$!
...
run_wizard() {
  ...
  run_wizard_cmd "$case_name" "$state_ref" "node \"$DAISYCLAW_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
}

assert_onboard_config() {
  local scenario="$1"
  shift
  daisyclaw_e2e_assert_file "$DAISYCLAW_CONFIG_PATH"
  node scripts/e2e/lib/onboard/assert-config.mjs "$scenario" "$DAISYCLAW_CONFIG_PATH" "$@"
}

set_isolated_daisyclaw_env() {
  local state_ref="$1"
  daisyclaw_test_state_create "$state_ref" empty
}
...
run_case_local_basic() {
  set_isolated_daisyclaw_env local-basic
  daisyclaw_e2e_run_logged local-basic node "$DAISYCLAW_ENTRY" onboard \
    --non-interactive \
    --accept-risk \
    --flow quickstart \
    --mode local \
  ...
  validate_local_basic_log "$DAISYCLAW_E2E_LAST_LOG_PATH"

  # Assert config + workspace scaffolding.
  workspace_dir="$DAISYCLAW_STATE_DIR/workspace"
  sessions_dir="$DAISYCLAW_STATE_DIR/agents/main/sessions"

  daisyclaw_e2e_assert_dir "$sessions_dir"
  for file in AGENTS.md BOOTSTRAP.md IDENTITY.md SOUL.md TOOLS.md USER.md; do
    daisyclaw_e2e_assert_file "$workspace_dir/$file"
  done
...
run_case_remote_non_interactive() {
  set_isolated_daisyclaw_env remote-non-interactive
  # Smoke test non-interactive remote config write.
  daisyclaw_e2e_run_logged remote-non-interactive node "$DAISYCLAW_ENTRY" onboard --non-interactive --accept-risk \
    --mode remote \
...
run_case_reset() {
  set_isolated_daisyclaw_env reset-config
  node scripts/e2e/lib/onboard/write-config.mjs reset "$DAISYCLAW_CONFIG_PATH"

  daisyclaw_e2e_run_logged reset-config node "$DAISYCLAW_ENTRY" onboard \
    --non-interactive \
    --accept-risk \
    --flow quickstart \
    --mode local \
...
run_case_channels() {
  ...
  run_wizard_cmd channels channels "node \"$DAISYCLAW_ENTRY\" configure --section channels" send_channels_flow
...
run_case_skills() {
  local home_dir
  set_isolated_daisyclaw_env skills
  home_dir="$HOME"
  node scripts/e2e/lib/onboard/write-config.mjs skills "$DAISYCLAW_CONFIG_PATH"

  run_wizard_cmd skills "$home_dir" "node \"$DAISYCLAW_ENTRY\" configure --section skills" send_skills_flow
...
validate_local_basic_log() {
  local log_path="$1"
  daisyclaw_e2e_assert_log_not_contains "$log_path" "systemctl --user unavailable"
}
