...
const timeoutSeconds = readPositiveIntEnv("DAISYCLAW_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS", 180);
const maxBodyBytes = readPositiveIntEnv("DAISYCLAW_OPENAI_CHAT_TOOLS_MAX_BODY_BYTES", 1048576);

if (!port || !token) {
  throw new Error("missing PORT/DAISYCLAW_GATEWAY_TOKEN");
}
if (!Number.isFinite(timeoutSeconds) || timeoutSeconds <= 0) {
  throw new Error(`invalid DAISYCLAW_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS: ${timeoutSeconds}`);
}
if (!Number.isFinite(maxBodyBytes) || maxBodyBytes <= 0) {
  throw new Error(`invalid DAISYCLAW_OPENAI_CHAT_TOOLS_MAX_BODY_BYTES: ${maxBodyBytes}`);
}
...
try {
  response = await fetch(`http://127.0.0.1:${port}/v1/chat/completions`, {
    ...
    headers: {
      ...
      "x-daisyclaw-model": backendModel,
    },
    body: JSON.stringify({
      model: "daisyclaw",
      stream: false,
