...
parallels_macos_desktop_user_exec_with_secret_file() {
  ...
  local secret_path
  secret_path="/tmp/daisyclaw-secret-${api_key_env:-env}-$RANDOM-$RANDOM"
  ...
  local wrapper
  local wrapper_path
  wrapper_path="/tmp/daisyclaw-secret-env-wrapper-$RANDOM-$RANDOM.sh"
  wrapper='#!/bin/bash
set -e
cleanup() {
  rm -f "${DAISYCLAW_WRAPPER_FILE:-}"
}
trap cleanup EXIT
if [ -n "${DAISYCLAW_SECRET_ENV_NAME:-}" ] && [ -n "${DAISYCLAW_SECRET_FILE:-}" ] && [ -f "$DAISYCLAW_SECRET_FILE" ]; then
  secret_value="$(cat "$DAISYCLAW_SECRET_FILE")"
  rm -f "$DAISYCLAW_SECRET_FILE"
  export "${DAISYCLAW_SECRET_ENV_NAME}=${secret_value}"
fi
"$@"
'
  ...
  if [[ "$user_flag" == "current-user" ]]; then
    prlctl exec "$vm_name" --current-user /usr/bin/env \
      "PATH=$path_value" \
      "DAISYCLAW_SECRET_ENV_NAME=$api_key_env" \
      "DAISYCLAW_SECRET_FILE=$secret_path" \
      "DAISYCLAW_WRAPPER_FILE=$wrapper_path" \
