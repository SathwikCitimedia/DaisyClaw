...
const pageSize = readPositiveIntEnv("DAISYCLAW_PROC_PAGE_SIZE", 4096);
const clockTicks = readPositiveIntEnv("DAISYCLAW_PROC_CLK_TCK", 100);
const pollMs = readPositiveIntEnv("DAISYCLAW_PLUGIN_LIFECYCLE_METRIC_POLL_MS", 100);
const timeoutMs = readPositiveIntEnv("DAISYCLAW_PLUGIN_LIFECYCLE_PHASE_TIMEOUT_MS", 300000);
const timeoutKillGraceMs = readPositiveIntEnv(
  "DAISYCLAW_PLUGIN_LIFECYCLE_TIMEOUT_KILL_GRACE_MS",
  2000,
);
const maxRssKbThreshold = readPositiveIntEnv(
  "DAISYCLAW_PLUGIN_LIFECYCLE_MAX_RSS_KB",
  4 * 1024 * 1024,
);
const maxWallMs = readPositiveIntEnv("DAISYCLAW_PLUGIN_LIFECYCLE_MAX_WALL_MS", timeoutMs);
const maxCpuCoreRatio = readPositiveNumberEnv("DAISYCLAW_PLUGIN_LIFECYCLE_MAX_CPU_CORE_RATIO", 16);
