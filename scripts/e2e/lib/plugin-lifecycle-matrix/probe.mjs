...
function daisyclawPath(...parts) {
  return path.join(home, ".daisyclaw", ...parts);
}
...
function config() {
  return readJson(process.env.DAISYCLAW_CONFIG_PATH ?? daisyclawPath("daisyclaw.json"));
}

function requiredConfig() {
  return readRequiredJson(process.env.DAISYCLAW_CONFIG_PATH ?? daisyclawPath("daisyclaw.json"));
}
...
function assertNpmProjectRoot(pluginId, packageName) {
  const record = recordFor(pluginId);
  assert(record?.installPath, `install path missing for ${pluginId}`);
  const relative = path.relative(daisyclawPath("npm", "projects"), record.installPath);
  assert(
  ...
  assert(
    !fs.existsSync(daisyclawPath("npm", "node_modules", ...packageSegments)),
