...
daisyclaw_e2e_install_package /tmp/daisyclaw-install.log "mounted DaisyClaw package" /tmp/npm-prefix
...
DAISYCLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT="$(node "$probe" legacy-compat "$package_version")"
...
node scripts/e2e/lib/plugin-update/registry-server.mjs >/tmp/daisyclaw-e2e-registry.log 2>&1 &
registry_pid=$!
trap 'daisyclaw_e2e_stop_process "${registry_pid:-}"' EXIT

if ! node "$probe" wait-registry; then
  echo "Local npm metadata registry failed to start"
  daisyclaw_e2e_print_log /tmp/daisyclaw-e2e-registry.log
  exit 1
fi

before_config_hash=""
if [ "$DAISYCLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT" != "1" ]; then
  before_config_hash="$(sha256sum "$DAISYCLAW_CONFIG_PATH" | awk '{print $1}')"
fi
plugin_update_timeout_seconds="${DAISYCLAW_PLUGIN_UPDATE_TIMEOUT_SECONDS:-180}"

node "$probe" snapshot > /tmp/plugin-update-before.json

set +e
daisyclaw_e2e_maybe_timeout "${plugin_update_timeout_seconds}s" node "$entry" plugins update @example/lossless-claw > /tmp/plugin-update-output.log 2>&1
...
if [ "$plugin_update_status" -ne 0 ]; then
  ...
  daisyclaw_e2e_print_log /tmp/plugin-update-output.log
...
if [ -n "$before_config_hash" ]; then
  after_config_hash="$(sha256sum "$DAISYCLAW_CONFIG_PATH" | awk '{print $1}')"
