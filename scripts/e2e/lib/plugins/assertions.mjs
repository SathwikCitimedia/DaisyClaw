...
const scratchRoot = process.env.DAISYCLAW_PLUGINS_TMP_DIR || os.tmpdir();
...
function readClawHubPreflightLimits() {
  return {
    bodyMaxBytes: readPositiveIntEnv(
      "DAISYCLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_BODY_MAX_BYTES",
      1024 * 1024,
    ),
    timeoutMs: readPositiveIntEnv("DAISYCLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS", 30_000),
  };
}
...
function getInstallRecords() {
  const configPath = daisyClawConfigPath();
  const config = readDaisyClawConfig();
  const allowLegacyCompat = process.env.DAISYCLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT === "1";
...
function daisyClawConfigPath() {
  return path.join(process.env.HOME, ".daisyclaw", "daisyclaw.json");
}

function readDaisyClawConfig() {
  const configPath = daisyClawConfigPath();
  return fs.existsSync(configPath) ? readRequiredDaisyClawConfig() : {};
}

function readRequiredDaisyClawConfig() {
  const configPath = daisyClawConfigPath();
  try {
    return readJson(configPath);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(`failed to read DaisyClaw config ${configPath}: ${message}`, { cause: error });
  }
}
...
function recordFixturePluginTrust() {
  ...
  const configPath = path.join(process.env.HOME, ".daisyclaw", "daisyclaw.json");
...
function assertGitPlugin() {
  ...
  const gitRoot = path.join(process.env.HOME, ".daisyclaw", "git");
...
function assertClawHubExternalInstallContract(installPath) {
  const daisyclawPeerPath = path.join(installPath, "node_modules", "daisyclaw");
  if (!fs.existsSync(daisyclawPeerPath)) {
    throw new Error(`missing ClawHub daisyclaw peer symlink: ${daisyclawPeerPath}`);
  }
  if (!fs.lstatSync(daisyclawPeerPath).isSymbolicLink()) {
    throw new Error(`ClawHub daisyclaw peer is not a symlink: ${daisyclawPeerPath}`);
  }
  const hostRoot = fs.realpathSync(process.cwd());
  const linkedHostRoot = fs.realpathSync(daisyclawPeerPath);
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected ClawHub daisyclaw peer ${linkedHostRoot} to target ${hostRoot}`);
  }
...
function assertNpmPlugin() {
  ...
  if (record.spec !== "@daisyclaw/demo-plugin-npm@0.0.1") {
    throw new Error(`unexpected npm spec: ${record.spec}`);
  }
  if (record.resolvedName !== "@daisyclaw/demo-plugin-npm") {
...
function assertInvalidDaisyClawExtensionsRejected() {
  const pluginId = "demo-plugin-invalid-metadata";
  for (const expected of ["daisyclaw.extensions[1]", "non-empty string"]) {
    assertTextFileIncludes(
      scratchFile("plugins-invalid-daisyclaw-extensions.log"),
      expected,
      "malformed metadata install output",
    );
  }

  const list = readJson(scratchFile("plugins-invalid-daisyclaw-extensions-list.json"));
  ...
  const managedInstallPath = path.join(process.env.HOME, ".daisyclaw", "extensions", pluginId);
...
async function assertClawHubPreflight() {
  ...
  const baseUrl = (
    process.env.DAISYCLAW_CLAWHUB_URL ||
    process.env.CLAWHUB_URL ||
    "https://clawhub.ai"
  ).replace(/\/+$/, "");
  const token =
    process.env.DAISYCLAW_CLAWHUB_TOKEN ||
...
function assertClawHubInstalled() {
  ...
  const installPath = resolveHomePath(record.installPath);
  const extensionsRoot = path.join(process.env.HOME, ".daisyclaw", "extensions");
...
function assertClawHubRemoved() {
  ...
  const configAfterUninstallPath = path.join(process.env.HOME, ".daisyclaw", "daisyclaw.json");
...
const commands = {
  ...
  "invalid-daisyclaw-extensions": assertInvalidDaisyClawExtensionsRejected,
