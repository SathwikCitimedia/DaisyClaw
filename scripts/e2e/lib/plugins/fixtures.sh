DAISYCLAW_PLUGINS_FIXTURE_PID_FILES=()
DAISYCLAW_PLUGINS_FIXTURE_EXIT_TRAP_INSTALLED=0
DAISYCLAW_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION=""

daisyclaw_plugins_cleanup_fixture_servers() {
  local pid_file
  local pid
  for pid_file in "${DAISYCLAW_PLUGINS_FIXTURE_PID_FILES[@]:-}"; do
    [[ -f "$pid_file" ]] || continue
...
daisyclaw_plugins_print_fixture_log() {
  local log_file="$1"
...
daisyclaw_plugins_register_fixture_pid_file() {
  local pid_file="$1"
  DAISYCLAW_PLUGINS_FIXTURE_PID_FILES+=("$pid_file")
  daisyclaw_plugins_install_fixture_cleanup_trap
}

daisyclaw_plugins_install_fixture_cleanup_trap() {
  if [[ "${DAISYCLAW_PLUGINS_FIXTURE_EXIT_TRAP_INSTALLED:-0}" = "1" ]]; then
    return
  fi

  local existing_trap
  existing_trap="$(trap -p EXIT || true)"
  if [[ -n "$existing_trap" && "$existing_trap" != *daisyclaw_plugins_fixture_exit_trap* ]]; then
    local existing_action="${existing_trap#trap -- }"
    existing_action="${existing_action% EXIT}"
    eval "DAISYCLAW_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION=$existing_action"
  fi

  DAISYCLAW_PLUGINS_FIXTURE_EXIT_TRAP_INSTALLED=1
  trap daisyclaw_plugins_fixture_exit_trap EXIT
}

daisyclaw_plugins_fixture_exit_trap() {
  local status="$?"
  daisyclaw_plugins_cleanup_fixture_servers
  if [[ -n "${DAISYCLAW_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION:-}" ]]; then
    eval "$DAISYCLAW_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION"
  fi
  exit "$status"
}
...
packageJson.daisyclaw.extensions = ["./index.js", " "];
