...
export OPENAI_API_KEY="sk-daisyclaw-release-media-memory"
export DAISYCLAW_QA_ALLOW_LOCAL_IMAGE_PROVIDER=1

PORT="18789"
MOCK_PORT="44200"
SUCCESS_MARKER="DAISYCLAW_E2E_OK_MEDIA_MEMORY"
MEMORY_MARKER="release-media-memory-saffron-$(date +%s)"
media_root="$(mktemp -d /tmp/daisyclaw-release-media-memory.XXXXXX)"
...
entry="$(daisyclaw_e2e_package_entrypoint "$package_root")"
{
  ...
  printf 'entry=%s\n' "$entry"
  printf 'HOME=%s\n' "$HOME"
  printf 'DAISYCLAW_HOME=%s\n' "$DAISYCLAW_HOME"
  printf 'DAISYCLAW_STATE_DIR=%s\n' "$DAISYCLAW_STATE_DIR"
  printf 'DAISYCLAW_CONFIG_PATH=%s\n' "$DAISYCLAW_CONFIG_PATH"
} >"$ENV_LOG"
...
mock_pid="$(daisyclaw_e2e_start_mock_openai "$MOCK_PORT" "$MOCK_OPENAI_LOG")"
...
cp "$DAISYCLAW_CONFIG_PATH" "$CONFIG_JSON"
daisyclaw plugins list --json >"$PLUGINS_JSON" 2>"$PLUGINS_STDERR_LOG"
...
mkdir -p "$DAISYCLAW_STATE_DIR/workspace/memory"
...
cat >"$DAISYCLAW_STATE_DIR/workspace/MEMORY.md" <<EOF
# Long-term memory

- The release media memory marker is $MEMORY_MARKER.
EOF

daisyclaw memory index --force >"$INDEX_LOG" 2>&1
daisyclaw memory search "$MEMORY_MARKER" --json >"$SEARCH_BEFORE_JSON" 2>"$SEARCH_BEFORE_STDERR_LOG"
...
daisyclaw memory search "$MEMORY_MARKER" --json >"$SEARCH_AFTER_JSON" 2>"$SEARCH_AFTER_STDERR_LOG"
