...
export OPENAI_API_KEY="sk-daisyclaw-release-upgrade-user-journey"
export CLICKCLACK_BOT_TOKEN="clickclack-release-upgrade-token"

PORT="18789"
MOCK_PORT="44210"
CLICKCLACK_PORT="44211"
SUCCESS_MARKER="DAISYCLAW_E2E_OK_RELEASE_UPGRADE"
...
BASELINE_SPEC="${DAISYCLAW_RELEASE_UPGRADE_BASELINE_SPEC:-daisyclaw@latest}"
export SUCCESS_MARKER MOCK_REQUEST_LOG CLICKCLACK_STATE

candidate_version="$(
  tar -xOf "${DAISYCLAW_CURRENT_PACKAGE_TGZ:?missing DAISYCLAW_CURRENT_PACKAGE_TGZ}" package/package.json |
...
cleanup() {
  ...
  daisyclaw_e2e_stop_process "${clickclack_pid:-}"
...
start_gateway() {
  local log_path="$1"
  gateway_pid="$(daisyclaw_e2e_start_gateway "$entry" "$PORT" "$log_path")"
  daisyclaw_e2e_wait_gateway_ready "$gateway_pid" "$log_path" 300 "$PORT"
}

echo "Installing published baseline $BASELINE_SPEC..."
if ! daisyclaw_e2e_maybe_timeout "${DAISYCLAW_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g "$BASELINE_SPEC" --no-fund --no-audit >"$BASELINE_INSTALL_LOG" 2>&1; then
  cat "$BASELINE_INSTALL_LOG" >&2 || true
  exit 1
fi
command -v daisyclaw >/dev/null
baseline_root="$(daisyclaw_e2e_package_root)"
baseline_entry="$(daisyclaw_e2e_package_entrypoint "$baseline_root")"
daisyclaw_e2e_enable_daisyclaw_cli_timeout

mock_pid="$(daisyclaw_e2e_start_mock_openai "$MOCK_PORT" "$OPENAI_LOG")"
...
for _ in $(seq 1 100); do
  if daisyclaw_e2e_probe_http_status "http://127.0.0.1:$CLICKCLACK_PORT/health" 200 >/dev/null 2>&1; then
    break
  fi
  sleep 0.1
done
daisyclaw_e2e_probe_http_status "http://127.0.0.1:$CLICKCLACK_PORT/health" 200
...
daisyclaw plugins install "$plugin_dir" >"$PLUGIN_INSTALL_LOG" 2>&1
daisyclaw release-upgrade ping >"$PLUGIN_CLI_BEFORE_LOG" 2>&1
...
daisyclaw_e2e_install_package "$CANDIDATE_INSTALL_LOG" "candidate DaisyClaw package"
...
daisyclaw release-upgrade ping >"$PLUGIN_CLI_AFTER_LOG" 2>&1
