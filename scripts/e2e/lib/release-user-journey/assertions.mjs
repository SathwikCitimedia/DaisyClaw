...
function clickClackHttpTimeoutMs() {
  return readPositiveInt(
    process.env.DAISYCLAW_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS,
    5000,
    "DAISYCLAW_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS",
  );
}

function clickClackHttpBodyMaxBytes() {
  return readPositiveInt(
    process.env.DAISYCLAW_RELEASE_USER_JOURNEY_HTTP_BODY_MAX_BYTES,
    1024 * 1024,
    "DAISYCLAW_RELEASE_USER_JOURNEY_HTTP_BODY_MAX_BYTES",
  );
}
...
function configPath() {
  return (
    ...
    path.join(process.env.HOME ?? "", ".daisyclaw", "daisyclaw.json")
  );
}
...
function assertOnboard() {
  ...
  assert(fs.existsSync(configPath()), "onboard did not write daisyclaw.json");
  const stateRaw =
    fs.readFileSync(configPath(), "utf8") +
    (fs.existsSync(authPath) ? fs.readFileSync(authPath, "utf8") : "");
  assert(
    !stateRaw.includes("sk-daisyclaw-release-user-journey"),
