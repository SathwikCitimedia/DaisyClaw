...
source "$ROOT_DIR/scripts/lib/daisyclaw-e2e-instance.sh"

DAISYCLAW_TEST_STATE_SCRIPT_B64="${DAISYCLAW_TEST_STATE_SCRIPT_B64:-}"
daisyclaw_skill_install_owns_home=0
cleanup_clawhub_skill_install_home() {
  if [ "$daisyclaw_skill_install_owns_home" = "1" ] && [ -n "${HOME:-}" ]; then
    rm -rf "$HOME"
  fi
}
trap cleanup_clawhub_skill_install_home EXIT

if [ -n "$DAISYCLAW_TEST_STATE_SCRIPT_B64" ]; then
  daisyclaw_e2e_eval_test_state_from_b64 "$DAISYCLAW_TEST_STATE_SCRIPT_B64"
else
  export HOME="$(mktemp -d "${TMPDIR:-/tmp}/daisyclaw-skill-install-home.XXXXXX")"
  daisyclaw_skill_install_owns_home=1
  export USERPROFILE="$HOME"
  export DAISYCLAW_HOME="$HOME"
  export DAISYCLAW_STATE_DIR="$HOME/.daisyclaw"
...
if [ -n "${DAISYCLAW_CURRENT_PACKAGE_TGZ:-}" ]; then
  ...
  daisyclaw_e2e_install_package /tmp/daisyclaw-skill-install-npm.log
fi

if [ -n "${DAISYCLAW_CURRENT_PACKAGE_TGZ:-}" ] && command -v daisyclaw >/dev/null 2>&1; then
  DAISYCLAW_CMD=(daisyclaw)
elif command -v pnpm >/dev/null 2>&1 && [ -f package.json ]; then
  if [ "${DAISYCLAW_SKILL_INSTALL_E2E_BUILD_SOURCE:-0}" = "1" ]; then
    pnpm build >/tmp/daisyclaw-skill-install-build.log 2>&1
  fi
  DAISYCLAW_CMD=(pnpm --silent daisyclaw)
elif command -v daisyclaw >/dev/null 2>&1; then
  DAISYCLAW_CMD=(daisyclaw)
else
  echo "daisyclaw command not found; install package first or run from repo with pnpm" >&2
  exit 1
fi

mkdir -p "$(dirname "$DAISYCLAW_CONFIG_PATH")"
node --input-type=module - "$DAISYCLAW_CONFIG_PATH" <<'NODE'
...
query="${DAISYCLAW_SKILL_INSTALL_E2E_QUERY:-homeassistant}"
requested_slug="${DAISYCLAW_SKILL_INSTALL_E2E_SLUG:-}"
preferred_slug="${DAISYCLAW_SKILL_INSTALL_E2E_PREFERRED_SLUG:-homeassistant-skill}"
search_json="/tmp/daisyclaw-skill-install-search.json"
resolve_json="/tmp/daisyclaw-skill-install-resolved.json"
install_log="/tmp/daisyclaw-skill-install.log"
info_json="/tmp/daisyclaw-skill-install-info.json"

echo "Searching live ClawHub skills for: $query"
"${DAISYCLAW_CMD[@]}" skills search "$query" --limit 8 --json >"$search_json"
...
if ! "${DAISYCLAW_CMD[@]}" skills install "$slug" --force >"$install_log" 2>&1; then
  echo "Skill install failed" >&2
  daisyclaw_e2e_dump_logs /tmp/daisyclaw-skill-install-npm.log "$search_json" "$resolve_json" "$install_log"
  exit 1
fi

workspace_dir="$HOME/.daisyclaw/workspace"
skill_dir="$workspace_dir/skills/$slug"
origin_json="$skill_dir/.clawhub/origin.json"
lock_json="$workspace_dir/.clawhub/lock.json"

daisyclaw_e2e_assert_file "$skill_dir/SKILL.md"
daisyclaw_e2e_assert_file "$origin_json"
daisyclaw_e2e_assert_file "$lock_json"

"${DAISYCLAW_CMD[@]}" skills info "$slug" --json >"$info_json"

node --input-type=module - "$DAISYCLAW_CONFIG_PATH" "$skill_dir" "$origin_json" "$lock_json" "$info_json" "$slug" <<'NODE'
import fs from "node:fs";
