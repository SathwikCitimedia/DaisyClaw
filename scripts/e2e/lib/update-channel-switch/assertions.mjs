...
function assertConfigChannel(channel) {
  const config = readJson(path.join(process.env.HOME, ".daisyclaw", "daisyclaw.json"));
  if (config.update?.channel === channel) {
    return;
  }
  if (process.env.DAISYCLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT === "1") {
    console.log(
...
switch (command) {
  case "prepare-git-fixture":
    prepareGitFixture(args[0] ?? "/tmp/daisyclaw-git");
    break;
  case "write-control-ui":
    writeControlUi(args[0] ?? "/tmp/daisyclaw-git");
    break;
  case "assert-update":
    assertUpdate(args[0]);
    break;
