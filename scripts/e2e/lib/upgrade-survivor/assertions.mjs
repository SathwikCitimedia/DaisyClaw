        ...
        resolvedSkills: [
          {
            ...
            filePath: "/tmp/daisyclaw-old-package/skills/legacy-heavy-skill-cache/SKILL.md",
          },
        ],
      },
    },
...
function getScenario() {
  const scenario = process.env.DAISYCLAW_UPGRADE_SURVIVOR_SCENARIO || "base";
  assert(SCENARIOS.has(scenario), `unknown upgrade survivor scenario: ${scenario}`);
  return scenario;
}

function getConfig() {
  return readJson(requireEnv("DAISYCLAW_CONFIG_PATH"));
}

function getCoverage() {
  const file = process.env.DAISYCLAW_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON;
...
function seedState() {
  const stateDir = requireEnv("DAISYCLAW_STATE_DIR");
  const workspace = requireEnv("DAISYCLAW_TEST_WORKSPACE_DIR");
  ...
  writeJson(path.join(workspace, ".daisyclaw", "workspace-state.json"), {
    version: 1,
  ...
  for (const plugin of ["discord", "telegram", "whatsapp"]) {
    writeJson(path.join(runtimeRoot, plugin, ".daisyclaw-runtime-deps-stamp.json"), {
      version: 0,
      plugin,
      stale: true,
    });
    write(
      path.join(
        runtimeRoot,
        plugin,
        ".daisyclaw-runtime-deps-copy-stale",
        "node_modules",
        "stale-sentinel",
        "package.json",
      ),
  ...
  if (scenario === "versioned-runtime-deps") {
    const version = process.env.DAISYCLAW_UPGRADE_SURVIVOR_BASELINE_VERSION || "2026.4.24";
    for (const plugin of ["discord", "feishu", "telegram", "whatsapp"]) {
      writeJson(
        path.join(
          runtimeRoot,
          `daisyclaw-${version}-${plugin}`,
          ".daisyclaw-runtime-deps-stamp.json",
        ),
        {
...
function assertConfigSurvived() {
  ...
  if (hasCoverage(coverage) && acceptsIntent(coverage, "logging")) {
    assert(
      config.logging?.file === "~/daisyclaw-upgrade-survivor/gateway.jsonl",
...
function assertStateSurvived() {
  ...
  const workspace = requireEnv("DAISYCLAW_TEST_WORKSPACE_DIR");
  const scenario = getScenario();
  const stage = process.env.DAISYCLAW_UPGRADE_SURVIVOR_ASSERT_STAGE || "survival";
  ...
  if (scenario === "stale-source-plugin-shadow") {
    const staleRoot = path.join(stateDir, "extensions", "opik-daisyclaw");
    assert(
  ...
  if (scenario === "versioned-runtime-deps") {
    ...
    const staleVersionedRoots = fs.existsSync(runtimeRoot)
      ? fs.readdirSync(runtimeRoot).filter((entry) => entry.startsWith(`daisyclaw-${version}-`))
      : [];
    assert(
...
function readMigratedSessionStore(stateDir, targetStorePath) {
  ...
  const dbPath = path.join(stateDir, "agents", "main", "agent", "daisyclaw-agent.sqlite");
...
function assertExternalPluginInstall(records, pluginId, packageName) {
  ...
  const extensionsRoot = path.join(requireEnv("DAISYCLAW_STATE_DIR"), "extensions");
  assert(
...
function assertConfiguredPluginInstalls() {
  ...
  assertOptionalConfiguredPluginIndex(records, index.plugins ?? [], {
    packageName: "@daisyclaw/brave-plugin",
    pluginId: "brave",
  });
