    ...
    [
      {
        id: "logging-file",
        intent: "logging",
        argv: ["config", "set", "logging.file", "~/daisyclaw-upgrade-survivor/gateway.jsonl"],
      },
    ],
  ],
  [
...
function selectedScenario() {
  return process.env.DAISYCLAW_UPGRADE_SURVIVOR_SCENARIO || "base";
}
...
export function resolveUpgradeSurvivorDaisyClawCommand(argv, params = {}) {
  const platform = params.platform ?? process.platform;
  if (platform === "win32") {
    const comSpec = params.comSpec ?? process.env.ComSpec ?? "cmd.exe";
    return {
      command: comSpec,
      args: ["/d", "/s", "/c", buildCmdExeCommandLine("daisyclaw.cmd", argv)],
      commandLabel: ["daisyclaw", ...argv].join(" "),
      shell: false,
...
export function runUpgradeSurvivorDaisyClawStep(step, params = {}) {
  const invocation = resolveUpgradeSurvivorDaisyClawCommand(step.argv);
...
function applyRecipe() {
  ...
  for (const step of [...recipe.slice(0, -1), ...scenarioSteps, recipe.at(-1)]) {
    ...
    const outcome = runUpgradeSurvivorDaisyClawStep(adaptedStep);
