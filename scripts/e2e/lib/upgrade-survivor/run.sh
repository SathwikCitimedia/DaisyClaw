...
export DAISYCLAW_SKIP_PROVIDERS=1
export DAISYCLAW_SKIP_CHANNELS=1
export DAISYCLAW_DISABLE_BONJOUR=1
export GATEWAY_AUTH_TOKEN_REF="upgrade-survivor-token"
export OPENAI_API_KEY="sk-daisyclaw-upgrade-survivor"
...
ARTIFACT_ROOT="$(dirname "${DAISYCLAW_UPGRADE_SURVIVOR_SUMMARY_JSON:-/tmp/daisyclaw-upgrade-survivor-artifacts/summary.json}")"
export DAISYCLAW_UPGRADE_SURVIVOR_RUNTIME_ROOT="${DAISYCLAW_UPGRADE_SURVIVOR_RUNTIME_ROOT:-/tmp/daisyclaw-upgrade-survivor-runtime}"
RUNTIME_ROOT="$DAISYCLAW_UPGRADE_SURVIVOR_RUNTIME_ROOT"
STATE_HOME_ROOT="${DAISYCLAW_UPGRADE_SURVIVOR_STATE_HOME_ROOT:-$RUNTIME_ROOT/state-home}"
mkdir -p "$ARTIFACT_ROOT"
mkdir -p "$RUNTIME_ROOT"
export TMPDIR="${DAISYCLAW_UPGRADE_SURVIVOR_TMPDIR:-$RUNTIME_ROOT/tmp}"
export DAISYCLAW_TEST_STATE_TMPDIR="${DAISYCLAW_UPGRADE_SURVIVOR_TEST_STATE_TMPDIR:-$RUNTIME_ROOT/state-tmp}"
mkdir -p "$TMPDIR" "$DAISYCLAW_TEST_STATE_TMPDIR"
...
SUMMARY_JSON="${DAISYCLAW_UPGRADE_SURVIVOR_SUMMARY_JSON:-$ARTIFACT_ROOT/summary.json}"
PHASE_LOG="$ARTIFACT_ROOT/phases.jsonl"
BASELINE_RAW="${DAISYCLAW_UPGRADE_SURVIVOR_BASELINE:?missing DAISYCLAW_UPGRADE_SURVIVOR_BASELINE}"
CANDIDATE_KIND="${DAISYCLAW_UPGRADE_SURVIVOR_CANDIDATE_KIND:-tarball}"
CANDIDATE_SPEC="${DAISYCLAW_UPGRADE_SURVIVOR_CANDIDATE_SPEC:-${DAISYCLAW_CURRENT_PACKAGE_TGZ:-}}"
SCENARIO="${DAISYCLAW_UPGRADE_SURVIVOR_SCENARIO:-base}"
UPDATE_RESTART_MODE="${DAISYCLAW_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE:-manual}"
ROOT_MANAGED_VPS="${DAISYCLAW_UPGRADE_SURVIVOR_ROOT_MANAGED_VPS:-0}"
...
export DAISYCLAW_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON="$CONFIG_COVERAGE_JSON"
...
validate_baseline_package_spec() {
  ...
  echo "DAISYCLAW_UPGRADE_SURVIVOR_BASELINE must be daisyclaw@latest, daisyclaw@beta, daisyclaw@alpha, an exact DaisyClaw release version, or a bare release version; got: $spec" >&2
  return 1
}
...
normalize_baseline() {
  ...
  if [ -z "$raw" ]; then
    echo "DAISYCLAW_UPGRADE_SURVIVOR_BASELINE cannot be empty" >&2
    return 1
  fi
  case "$raw" in
    daisyclaw@*)
      baseline_spec="$raw"
      baseline_version="${raw#daisyclaw@}"
      ;;
    *@*)
      echo "DAISYCLAW_UPGRADE_SURVIVOR_BASELINE must be daisyclaw@<version> or a bare version" >&2
      return 1
      ;;
    *)
      baseline_version="$raw"
      baseline_spec="daisyclaw@$raw"
      ;;
  esac
  case "$baseline_version" in
    latest | beta | alpha)
      ...
      ;;
    dev | main | "")
      echo "DAISYCLAW_UPGRADE_SURVIVOR_BASELINE must be daisyclaw@latest, daisyclaw@beta, daisyclaw@alpha, daisyclaw@<version>, or a bare version" >&2
      return 1
      ;;
    *)
...
validate_update_restart_mode() {
  case "$UPDATE_RESTART_MODE" in
    manual | auto-auth)
      ;;
    *)
      echo "DAISYCLAW_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE must be manual or auto-auth; got: $UPDATE_RESTART_MODE" >&2
      return 1
      ;;
  esac
}
...
const summary = {
  ...
  candidate: {
    kind: process.env.DAISYCLAW_UPGRADE_SURVIVOR_CANDIDATE_KIND || null,
    spec: process.env.DAISYCLAW_UPGRADE_SURVIVOR_CANDIDATE_SPEC || process.env.DAISYCLAW_CURRENT_PACKAGE_TGZ || null,
...
cleanup() {
  ...
  if [ -s "$SYSTEMCTL_SHIM_PID_FILE" ]; then
    ...
    if [[ "$shim_pid" =~ ^[0-9]+$ ]] && [ "$shim_pid" -gt 1 ]; then
      daisyclaw_e2e_terminate_gateways "$shim_pid"
    fi
  fi
}
...
package_root() {
  printf '%s/lib/node_modules/daisyclaw\n' "$npm_config_prefix"
}

legacy_runtime_deps_symlink_plugin() {
  local plugin="${DAISYCLAW_UPGRADE_SURVIVOR_LEGACY_RUNTIME_DEPS_SYMLINK:-}"
  if [ -z "$plugin" ]; then
    return 1
  fi
  case "$plugin" in
    *[!A-Za-z0-9._-]*)
      echo "DAISYCLAW_UPGRADE_SURVIVOR_LEGACY_RUNTIME_DEPS_SYMLINK must be a plugin id, got: $plugin" >&2
      return 2
      ;;
  esac
  printf '%s\n' "$plugin"
...
legacy_runtime_deps_symlink_target() {
  local plugin="$1"
  printf '%s/@daisyclaw-upgrade-survivor/%s-runtime-dep\n' "$(dirname "$(package_root)")" "$plugin"
}
...
plugin_deps_cleanup_plugins() {
  printf '%s\n' "${DAISYCLAW_UPGRADE_SURVIVOR_PLUGIN_DEPS_CLEANUP_PLUGINS:-discord telegram}"
}
...
seed_source_only_plugin_shadow() {
  ...
  local shadow_root="$DAISYCLAW_STATE_DIR/extensions/opik-daisyclaw"
  mkdir -p "$shadow_root/src"
  cat >"$shadow_root/package.json" <<'JSON'
{
  "name": "@opik/opik-daisyclaw",
...
JSON
  cat >"$shadow_root/daisyclaw.plugin.json" <<'JSON'
{
  "id": "opik-daisyclaw",
  "activation": {
    "onStartup": false
...
configure_configured_plugin_install_fixture_registry() {
  ...
  local tarball="$fixture_root/daisyclaw-brave-plugin-2026.5.2.tgz"
...
fs.writeFileSync(
  ...
  `${JSON.stringify(
    {
      name: "@daisyclaw/brave-plugin",
      version: "2026.5.2",
      daisyclaw: { extensions: ["./index.js"] },
    },
    null,
    2,
  )}\n`,
...
fs.writeFileSync(
  path.join(root, "daisyclaw.plugin.json"),
  `${JSON.stringify(
    {
      id: "brave",
...
NODE
  ...
  for _ in $(seq 1 100); do
    ...
    if ! kill -0 "$plugin_registry_pid" 2>/dev/null; then
      daisyclaw_e2e_print_log "$log_file" >&2
      return 1
    fi
    sleep 0.1
  done
...
legacy_plugin_dependency_probe_paths() {
  ...
  while IFS= read -r plugin_dir; do
    printf '%s\n' \
      ...
      "$plugin_dir/.daisyclaw-runtime-deps.json" \
      "$plugin_dir/.daisyclaw-runtime-deps-stamp.json" \
      "$plugin_dir/.daisyclaw-runtime-deps-copy-upgrade-survivor" \
      "$plugin_dir/.daisyclaw-install-stage-upgrade-survivor" \
      "$plugin_dir/.daisyclaw-pnpm-store"
  done < <(plugin_deps_cleanup_plugin_dirs "$plugin")
  printf '%s\n' \
    "$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor" \
    "$DAISYCLAW_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor" \
    "$DAISYCLAW_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor"
}
...
seed_legacy_plugin_dependency_debris() {
  ...
  for plugin in $(plugin_deps_cleanup_plugins); do
    ...
    found=1
    mkdir -p \
      "$plugin_dir/node_modules/daisyclaw-upgrade-survivor-dep" \
      "$plugin_dir/.daisyclaw-runtime-deps-copy-upgrade-survivor/node_modules/daisyclaw-upgrade-survivor-dep" \
      "$plugin_dir/.daisyclaw-install-stage-upgrade-survivor" \
      "$plugin_dir/.daisyclaw-pnpm-store" \
      "$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/daisyclaw-upgrade-survivor-dep" \
      "$DAISYCLAW_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/daisyclaw-upgrade-survivor-dep" \
      "$DAISYCLAW_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/daisyclaw-upgrade-survivor-dep"
    printf '{"name":"daisyclaw-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$plugin_dir/node_modules/daisyclaw-upgrade-survivor-dep/package.json"
    printf '{"plugin":"%s","scenario":"plugin-deps-cleanup"}\n' "$plugin" \
      >"$plugin_dir/.daisyclaw-runtime-deps.json"
    printf '{"plugin":"%s","scenario":"plugin-deps-cleanup","stale":true}\n' "$plugin" \
      >"$plugin_dir/.daisyclaw-runtime-deps-stamp.json"
    printf '{"name":"daisyclaw-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$plugin_dir/.daisyclaw-runtime-deps-copy-upgrade-survivor/node_modules/daisyclaw-upgrade-survivor-dep/package.json"
    printf '{"name":"daisyclaw-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/daisyclaw-upgrade-survivor-dep/package.json"
    printf '{"name":"daisyclaw-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$DAISYCLAW_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/daisyclaw-upgrade-survivor-dep/package.json"
    printf '{"name":"daisyclaw-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$DAISYCLAW_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/daisyclaw-upgrade-survivor-dep/package.json"
...
seed_legacy_runtime_deps_symlink() {
  ...
  printf '{"name":"daisyclaw-upgrade-survivor-legacy-runtime-deps","version":"0.0.0"}\n' \
...
reset_run_state() {
  rm_rf_retry "$npm_config_prefix" "$TMPDIR" "$DAISYCLAW_TEST_STATE_TMPDIR" "$STATE_HOME_ROOT"
  rm -f "$SYSTEMCTL_SHIM_PID_FILE" "$SYSTEMCTL_SHIM_DAEMON_LOG"
  mkdir -p "$npm_config_prefix" "$npm_config_cache" "$TMPDIR" "$DAISYCLAW_TEST_STATE_TMPDIR"
}

install_baseline() {
  normalize_baseline
  echo "Installing baseline package: $baseline_spec"
  if ! daisyclaw_e2e_maybe_timeout "${DAISYCLAW_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix "$npm_config_prefix" "$baseline_spec" --no-fund --no-audit >"$BASELINE_INSTALL_LOG" 2>&1; then
    echo "baseline npm install failed" >&2
    daisyclaw_e2e_print_log "$BASELINE_INSTALL_LOG" >&2
    return 1
  fi
  if ! command -v daisyclaw >/dev/null; then
    echo "baseline install did not expose daisyclaw on PATH" >&2
    echo "PATH=$PATH" >&2
  ...
  if ! version_output="$(daisyclaw_e2e_maybe_timeout "$COMMAND_TIMEOUT" daisyclaw --version 2>&1)"; then
    echo "baseline daisyclaw --version failed" >&2
    echo "$version_output" >&2
    return 1
  fi
  if [[ "$version_output" != *"$baseline_version"* ]]; then
    echo "baseline daisyclaw --version mismatch: expected output to include $baseline_version" >&2
    echo "$version_output" >&2
    return 1
  fi
}
...
seed_state() {
  daisyclaw_e2e_eval_test_state_from_b64 "${DAISYCLAW_TEST_STATE_FUNCTION_B64:?missing DAISYCLAW_TEST_STATE_FUNCTION_B64}"
  if [ "$ROOT_MANAGED_VPS" = "1" ]; then
    if [ "$(id -u)" -ne 0 ]; then
      echo "root-managed VPS survivor mode must run as uid 0" >&2
      return 1
    fi
    rm -rf /root/.daisyclaw /root/workspace
    daisyclaw_test_state_create /root minimal
  else
    daisyclaw_test_state_create "$STATE_HOME_ROOT" minimal
  fi
  export DAISYCLAW_UPGRADE_SURVIVOR_BASELINE_VERSION="$baseline_version"
...
validate_baseline_config() {
  if ! daisyclaw_e2e_maybe_timeout "$COMMAND_TIMEOUT" daisyclaw config validate >"$BASELINE_CONFIG_VALIDATE_LOG" 2>&1; then
    echo "generated baseline config failed baseline validation" >&2
    daisyclaw_e2e_print_log "$BASELINE_CONFIG_VALIDATE_LOG" >&2
    return 1
  fi
}
...
log_file="${DAISYCLAW_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_LOG:-/tmp/daisyclaw-systemctl-shim.log}"
pid_file="${DAISYCLAW_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_PID_FILE:-/tmp/daisyclaw-systemctl-shim.pid}"
daemon_log="${DAISYCLAW_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_DAEMON_LOG:-/tmp/daisyclaw-systemctl-shim-gateway.log}"
printf '%s\n' "$*" >>"$log_file"
...
unit_path() {
  printf '%s/.config/systemd/user/daisyclaw-gateway.service\n' "${HOME:?missing HOME}"
}
...
SHIM
  ...
  export DAISYCLAW_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_LOG="$SYSTEMCTL_SHIM_LOG"
  export DAISYCLAW_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_PID_FILE="$SYSTEMCTL_SHIM_PID_FILE"
  export DAISYCLAW_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_DAEMON_LOG="$SYSTEMCTL_SHIM_DAEMON_LOG"
  export PATH="$shim_dir:$PATH"
}

install_update_restart_service_unit() {
  if ! daisyclaw_e2e_maybe_timeout "$COMMAND_TIMEOUT" env -u DAISYCLAW_GATEWAY_TOKEN -u DAISYCLAW_GATEWAY_PASSWORD daisyclaw gateway install --force --json >"$BASELINE_SERVICE_INSTALL_JSON" 2>"$BASELINE_SERVICE_INSTALL_ERR"; then
    echo "baseline gateway service install failed" >&2
    daisyclaw_e2e_print_log "$BASELINE_SERVICE_INSTALL_ERR" >&2
    daisyclaw_e2e_print_log "$BASELINE_SERVICE_INSTALL_JSON" >&2
    return 1
  fi
}
...
const stateDir = process.env.DAISYCLAW_STATE_DIR;
if (!stateDir) {
  throw new Error("missing DAISYCLAW_STATE_DIR");
}
...
write_update_restart_service_secretref_env() {
  ...
  local dotenv_path="$DAISYCLAW_STATE_DIR/.env"
...
write_update_restart_service_auth_env() {
  ...
  printf 'GATEWAY_AUTH_TOKEN_REF=%s\n' "$GATEWAY_AUTH_TOKEN_REF" >>"$tmp_path"
  mv "$tmp_path" "$dotenv_path"
  local systemd_env_path="$DAISYCLAW_STATE_DIR/gateway.systemd.env"
...
assert_baseline_state() {
  DAISYCLAW_UPGRADE_SURVIVOR_ASSERT_STAGE=baseline \
...
resolve_candidate_version() {
  if [ -z "$CANDIDATE_SPEC" ]; then
    echo "missing DAISYCLAW_UPGRADE_SURVIVOR_CANDIDATE_SPEC" >&2
    return 1
  fi
  ...
  DAISYCLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT="$(
...
update_candidate() {
  ...
  local update_env=(
    env
    -u DAISYCLAW_GATEWAY_TOKEN
    -u DAISYCLAW_GATEWAY_PASSWORD
    -u DAISYCLAW_ALLOW_ROOT
  )
  ...
  if [ "$ROOT_MANAGED_VPS" != "1" ]; then
    update_env+=(DAISYCLAW_ALLOW_ROOT=1)
  fi
  if ! daisyclaw_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${update_env[@]}" daisyclaw "${update_args[@]}" >"$UPDATE_JSON" 2>"$UPDATE_ERR"; then
    echo "daisyclaw update failed" >&2
    daisyclaw_e2e_print_log "$UPDATE_ERR" >&2
    daisyclaw_e2e_print_log "$UPDATE_JSON" >&2
    return 1
  fi
  if [ "$UPDATE_RESTART_MODE" = "auto-auth" ]; then
...
assert_root_managed_vps_cli_usable() {
  ...
  daisyclaw_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${root_cli_env[@]}" daisyclaw config file >"$ARTIFACT_ROOT/root-vps-config-file.out" 2>"$ARTIFACT_ROOT/root-vps-config-file.err"
  daisyclaw_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${root_cli_env[@]}" daisyclaw plugins >"$ARTIFACT_ROOT/root-vps-plugins.out" 2>"$ARTIFACT_ROOT/root-vps-plugins.err"
}

run_doctor() {
  if ! daisyclaw_e2e_maybe_timeout "$COMMAND_TIMEOUT" daisyclaw doctor --fix --non-interactive >"$DOCTOR_LOG" 2>&1; then
    echo "daisyclaw doctor failed" >&2
    daisyclaw_e2e_print_log "$DOCTOR_LOG" >&2
    return 1
  fi
}

validate_post_doctor_config() {
  if ! daisyclaw_e2e_maybe_timeout "$COMMAND_TIMEOUT" daisyclaw config validate >>"$DOCTOR_LOG" 2>&1; then
...
probe_gateway_endpoint() {
  ...
  if [ -n "${DAISYCLAW_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING:-}" ]; then
    args+=(--allow-failing "$DAISYCLAW_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING")
  fi
  if [ "${DAISYCLAW_UPGRADE_SURVIVOR_READYZ_ALLOW_DEGRADED:-}" = "1" ]; then
...
start_gateway() {
  local port=18789
  local budget="${DAISYCLAW_UPGRADE_SURVIVOR_START_BUDGET_SECONDS:-90}"
  local start_epoch
  local ready_epoch
  start_epoch="$(node -e "process.stdout.write(String(Date.now()))")"
  env -u DAISYCLAW_GATEWAY_TOKEN -u DAISYCLAW_GATEWAY_PASSWORD daisyclaw gateway --port "$port" --bind loopback --allow-unconfigured >"$GATEWAY_LOG" 2>&1 &
  gateway_pid="$!"
  if [ "$UPDATE_RESTART_MODE" = "auto-auth" ]; then
  ...
  if [ "$start_seconds" -gt "$budget" ]; then
    ...
    daisyclaw_e2e_print_log "$GATEWAY_LOG" >&2
    return 1
  fi
}
...
check_gateway_status() {
  local port=18789
  local budget="${DAISYCLAW_UPGRADE_SURVIVOR_STATUS_BUDGET_SECONDS:-30}"
  local status_start
  local status_end
  status_start="$(node -e "process.stdout.write(String(Date.now()))")"
  if ! daisyclaw_e2e_maybe_timeout "$COMMAND_TIMEOUT" daisyclaw gateway status --url "ws://127.0.0.1:$port" --token "$GATEWAY_AUTH_TOKEN_REF" --require-rpc --timeout 30000 --json >"$STATUS_JSON" 2>"$STATUS_ERR"; then
    echo "gateway status failed" >&2
    daisyclaw_e2e_print_log "$STATUS_ERR" >&2
    daisyclaw_e2e_print_log "$GATEWAY_LOG" >&2
    return 1
  fi
  ...
  if [ "$status_seconds" -gt "$budget" ]; then
    ...
    daisyclaw_e2e_print_log "$STATUS_JSON" >&2
    return 1
  fi
