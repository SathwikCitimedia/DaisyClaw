...
// against the DaisyClaw npm tarball installed in the functional image.
...
export async function connectMcpClient(params: {
  ...
  const transport = new StdioClientTransport({
    ...
    env: {
      ...
      DAISYCLAW_STATE_DIR: tempState.stateDir,
    },
    stderr: "pipe",
  });
  transport.stderr?.on("data", (chunk) => {
    process.stderr.write(`[daisyclaw mcp] ${String(chunk)}`);
  });
