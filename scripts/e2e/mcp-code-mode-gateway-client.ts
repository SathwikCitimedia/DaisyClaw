// Mcp Code Mode Gateway Client script supports DaisyClaw repository automation.
...
export function readMcpCodeModeClientFetchLimits(
  ...
  return {
    bodyMaxBytes: readPositiveIntEnv(
      "DAISYCLAW_MCP_CODE_MODE_CLIENT_BODY_MAX_BYTES",
      1024 * 1024,
      env,
    ),
    timeoutMs: readPositiveIntEnv("DAISYCLAW_MCP_CODE_MODE_CLIENT_TIMEOUT_MS", 300_000, env),
  };
}
...
async function main() {
  ...
  const model = process.env.DAISYCLAW_MCP_CODE_MODE_MODEL?.trim() || "daisyclaw/main";
  assert(gatewayUrl, "missing GW_URL");
  assert(gatewayToken, "missing GW_TOKEN");
  assert(stateDir, "missing DAISYCLAW_STATE_DIR");
