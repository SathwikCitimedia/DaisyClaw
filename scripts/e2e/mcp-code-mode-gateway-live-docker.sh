...
IMAGE_NAME="$(docker_e2e_resolve_image "daisyclaw-mcp-code-mode-gateway-live-e2e" DAISYCLAW_IMAGE)"
PORT="${DAISYCLAW_MCP_CODE_MODE_LIVE_GATEWAY_PORT:-18789}"
TOKEN="mcp-code-mode-live-e2e-$(date +%s)-$$"
CONTAINER_NAME="daisyclaw-mcp-code-mode-live-e2e-$$"
CLIENT_LOG="$(mktemp -t daisyclaw-mcp-code-mode-live-log.XXXXXX)"
PROFILE_FILE="${DAISYCLAW_MCP_CODE_MODE_LIVE_PROFILE_FILE:-${DAISYCLAW_TESTBOX_PROFILE_FILE:-$HOME/.daisyclaw-testbox-live.profile}}"
...
# The profile is only a credential source. Keep this lane's DaisyClaw runtime
# isolated from host/testbox mode flags that can change packaged behavior.
unset DAISYCLAW_TESTBOX

docker_e2e_build_or_reuse "$IMAGE_NAME" mcp-code-mode-gateway-live
DAISYCLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 mcp-code-mode-gateway-live empty)"
...
docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e "DAISYCLAW_DOCKER_OPENAI_BASE_URL=${OPENAI_BASE_URL:-https://api.openai.com/v1}" \
  ...
  -e "DAISYCLAW_MCP_CODE_MODE_MODEL=${DAISYCLAW_MCP_CODE_MODE_LIVE_MODEL:-daisyclaw/main}" \
  "${PROFILE_MOUNT[@]}" \
  "$IMAGE_NAME" \
  bash -lc "set -euo pipefail
    ...
    gateway_pid=\"\$(daisyclaw_e2e_start_gateway \"\$entry\" $PORT /tmp/mcp-code-mode-live-gateway.log)\"
    daisyclaw_e2e_wait_gateway_ready \"\$gateway_pid\" /tmp/mcp-code-mode-live-gateway.log 480 $PORT
