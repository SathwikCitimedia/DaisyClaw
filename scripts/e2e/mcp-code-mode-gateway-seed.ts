// Mcp Code Mode Gateway Seed script supports DaisyClaw repository automation.
import fs from "node:fs/promises";
import { createRequire } from "node:module";
import os from "node:os";
import path from "node:path";
import { applyDockerOpenAiProviderConfig, type DaisyClawConfig } from "./docker-openai-seed.ts";
...
async function main() {
  ...
  const configPath =
    process.env.DAISYCLAW_CONFIG_PATH?.trim() || path.join(stateDir, "daisyclaw.json");
  const workspaceDir = path.join(stateDir, "workspace");
  const serverPath = path.join(stateDir, "mcp-code-mode-fixture", "fixture-server.mjs");
  const apiKey =
    process.env.OPENAI_API_KEY?.trim() ||
    process.env.DAISYCLAW_MCP_CODE_MODE_OPENAI_API_KEY?.trim() ||
