...
const port =
  process.env.MOCK_PORT != null
    ...
    : readPositiveIntEnv("DAISYCLAW_MOCK_OPENAI_PORT");
const successMarker = process.env.SUCCESS_MARKER ?? "DAISYCLAW_E2E_OK";
    ...
    data: [
      {
        ...
        mime_type: "image/png",
        revised_prompt: "daisyclaw mock image",
      },
    ],
  });
}
...
function resolveResponseText(bodyText) {
  const matches = Array.from(bodyText.matchAll(/\bDAISYCLAW_E2E_OK(?:_\d+)?\b/gu));
...
const server = http.createServer((req, res) => {
  void (async () => {
    ...
    if (req.method === "GET" && url.pathname === "/v1/models") {
      writeJson(res, 200, {
        object: "list",
        data: [{ id: "gpt-5.5", object: "model", owned_by: "daisyclaw-e2e" }],
      });
      return;
    }
