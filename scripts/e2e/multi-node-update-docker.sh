...
# DaisyClaw under node-A, registers the gateway service pointing at node-A, then
# switches PATH so node-B comes first and runs `daisyclaw update`. Verifies that:
#
...
#
# Requires: Docker, a built daisyclaw-current.tgz (or will build one).
set -euo pipefail
...
IMAGE_NAME="daisyclaw-multi-node-update-e2e"
DOCKER_RUN_TIMEOUT="${DAISYCLAW_MULTI_NODE_DOCKER_TIMEOUT:-300s}"
...
docker_e2e_build_or_reuse "$IMAGE_NAME" multi-node-update "$ROOT_DIR/scripts/e2e/Dockerfile" "$ROOT_DIR" "bare" "${DAISYCLAW_SKIP_DOCKER_BUILD:-0}"
PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz multi-node-update "${DAISYCLAW_CURRENT_PACKAGE_TGZ:-}")"
...
docker_e2e_run_with_harness \
  ...
  -e DAISYCLAW_SKIP_PROVIDERS=1 \
  -e DAISYCLAW_SKIP_CHANNELS=1 \
  -e DAISYCLAW_DISABLE_BONJOUR=1 \
...
echo ""
echo "── Step 2: Install DaisyClaw under node-A ──"

# Use node-A to install daisyclaw with npm prefix A.
...
echo "Installing DaisyClaw package under node-A prefix: $NPM_PREFIX_A"
daisyclaw_e2e_install_package "$ARTIFACTS/install-a.log" "DaisyClaw package under node-A prefix" "$NPM_PREFIX_A"
echo "Installed. Checking daisyclaw location..."

DAISYCLAW_A="$(command -v daisyclaw)"
echo "daisyclaw binary: $DAISYCLAW_A"
echo "daisyclaw version: $(daisyclaw --version 2>/dev/null || echo unknown)"

# Record the package root for node-A install.
PACKAGE_ROOT_A="$NPM_PREFIX_A/lib/node_modules/daisyclaw"
...
GATEWAY_UNIT_PATH="/root/.config/systemd/user/daisyclaw-gateway.service"
...
if ! daisyclaw gateway install --json >"$ARTIFACTS/gateway-install.json" 2>"$ARTIFACTS/gateway-install.err"; then
...
# but daisyclaw is NOT installed there.
...
echo "which daisyclaw: $(command -v daisyclaw)"
echo "process.execPath will be: $(node -e "console.log(process.execPath)")"

echo ""
echo "── Step 6: Run daisyclaw update (this is the bug) ──"
...
echo "Running daisyclaw update --yes --json..."
UPDATE_EXIT=0
daisyclaw update --yes --json \
  --tag /tmp/daisyclaw-current.tgz \
...
if [ "$UPDATE_EXIT" -ne 0 ] && ! grep -q "gateway" "$ARTIFACTS/update.err" 2>/dev/null; then
  echo "FAIL: daisyclaw update failed before reaching the package install step"
...
if [ "$BAKED_NODE_AFTER" = "$NODE_B" ] && [ "$BAKED_NODE_BEFORE" != "$NODE_B" ]; then
  ...
  echo "   but DaisyClaw package is still under node-A prefix ($PACKAGE_ROOT_A)."
...
# Check 2: Is the DaisyClaw package installed under node-B npm prefix?
if [ -f "$NPM_PREFIX_B/lib/node_modules/daisyclaw/package.json" ]; then
  echo "WARNING: DaisyClaw was ALSO installed under node-B prefix (split install)"
else
  echo "OK: DaisyClaw is NOT under node-B prefix (expected: only under node-A)"
fi
...
if [ -f "$ARTIFACTS/update.err" ]; then
  if grep -qi "Shell DaisyClaw root differs" "$ARTIFACTS/update.err" 2>/dev/null; then
