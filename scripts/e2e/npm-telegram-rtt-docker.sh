...
IMAGE_NAME="$(docker_e2e_resolve_image "daisyclaw-npm-telegram-rtt-e2e" DAISYCLAW_NPM_TELEGRAM_RTT_E2E_IMAGE)"
...
validate_daisyclaw_package_spec() {
  local spec="$1"
  if [[ "$spec" =~ ^daisyclaw@(main|alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$ ]]; then
    return 0
  fi
  echo "DAISYCLAW_NPM_TELEGRAM_PACKAGE_SPEC must be daisyclaw@main, daisyclaw@alpha, daisyclaw@beta, daisyclaw@latest, or an exact DaisyClaw release version; got: $spec" >&2
  exit 1
}
...
validate_credential_source() {
  case "$credential_source" in
    "" | env | convex) ;;
    *)
      echo "DAISYCLAW_NPM_TELEGRAM_CREDENTIAL_SOURCE must be env or convex; got: $credential_source" >&2
      exit 1
      ;;
  esac
}
...
validate_credential_role() {
  case "$credential_role" in
    "" | maintainer | ci) ;;
    *)
      echo "DAISYCLAW_NPM_TELEGRAM_CREDENTIAL_ROLE must be maintainer or ci; got: $credential_role" >&2
      exit 1
      ;;
  esac
}
...
if [ -n "$credential_source" ]; then
  export DAISYCLAW_QA_CREDENTIAL_SOURCE="$credential_source"
fi
if [ -n "$credential_role" ]; then
  export DAISYCLAW_QA_CREDENTIAL_ROLE="$credential_role"
fi
...
run_log="$(mktemp "${TMPDIR:-/tmp}/daisyclaw-npm-telegram-rtt.XXXXXX")"
...
docker_env=(
  ...
  -e DAISYCLAW_QA_TELEGRAM_GROUP_ID
  -e DAISYCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN
  -e DAISYCLAW_QA_TELEGRAM_SUT_BOT_TOKEN
  -e DAISYCLAW_QA_TELEGRAM_CANARY_TIMEOUT_MS="${DAISYCLAW_QA_TELEGRAM_CANARY_TIMEOUT_MS:-180000}"
  -e DAISYCLAW_QA_TELEGRAM_SCENARIO_TIMEOUT_MS="${DAISYCLAW_QA_TELEGRAM_SCENARIO_TIMEOUT_MS:-180000}"
  -e DAISYCLAW_NPM_TELEGRAM_SCENARIOS="${DAISYCLAW_NPM_TELEGRAM_SCENARIOS:-telegram-mentioned-message-reply}"
  -e DAISYCLAW_NPM_TELEGRAM_PROVIDER_MODE="${DAISYCLAW_NPM_TELEGRAM_PROVIDER_MODE:-mock-openai}"
  -e DAISYCLAW_NPM_TELEGRAM_WARM_SAMPLES="${DAISYCLAW_NPM_TELEGRAM_WARM_SAMPLES:-20}"
  -e DAISYCLAW_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS="${DAISYCLAW_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS:-30000}"
  -e DAISYCLAW_NPM_TELEGRAM_MAX_FAILURES="${DAISYCLAW_NPM_TELEGRAM_MAX_FAILURES:-${DAISYCLAW_NPM_TELEGRAM_WARM_SAMPLES:-20}}"
...
if [ -n "${DAISYCLAW_QA_CREDENTIAL_SOURCE:-}" ]; then
  docker_env+=(-e DAISYCLAW_QA_CREDENTIAL_SOURCE="$DAISYCLAW_QA_CREDENTIAL_SOURCE")
fi
if [ -n "${DAISYCLAW_QA_CREDENTIAL_ROLE:-}" ]; then
  docker_env+=(-e DAISYCLAW_QA_CREDENTIAL_ROLE="$DAISYCLAW_QA_CREDENTIAL_ROLE")
fi
...
for key in \
  ...
  DAISYCLAW_QA_ALLOW_INSECURE_HTTP; do
...
command -v daisyclaw
daisyclaw --version
node -p "require('/npm-global/lib/node_modules/daisyclaw/package.json').version"
EOF
...
export HOME="$(mktemp -d "/tmp/daisyclaw-npm-telegram-rtt.XXXXXX")"
export NPM_CONFIG_PREFIX="/npm-global"
export PATH="$NPM_CONFIG_PREFIX/bin:$PATH"
export OPENAI_API_KEY="sk-daisyclaw-rtt"
export GATEWAY_AUTH_TOKEN_REF="daisyclaw-rtt"
export DAISYCLAW_DISABLE_BONJOUR="1"

install_source="${DAISYCLAW_NPM_TELEGRAM_INSTALL_SOURCE:?missing DAISYCLAW_NPM_TELEGRAM_INSTALL_SOURCE}"
package_label="${DAISYCLAW_NPM_TELEGRAM_PACKAGE_LABEL:-$install_source}"
mock_port="${DAISYCLAW_NPM_TELEGRAM_MOCK_PORT:-44080}"
config_path="$HOME/.daisyclaw/daisyclaw.json"
gateway_log="/tmp/daisyclaw-npm-telegram-rtt-gateway.log"
mock_log="/tmp/daisyclaw-npm-telegram-rtt-mock.log"
...
if [ "${DAISYCLAW_QA_CREDENTIAL_SOURCE:-}" = "convex" ]; then
  credential_env_file="$(mktemp "/tmp/daisyclaw-npm-telegram-rtt-credential-env.XXXXXX")"
  credential_lease_file="$(mktemp "/tmp/daisyclaw-npm-telegram-rtt-credential-lease.XXXXXX")"
  rm -f "$credential_env_file" "$credential_lease_file"
...
command -v daisyclaw
daisyclaw --version
installed_version="$(node -p "require('/npm-global/lib/node_modules/daisyclaw/package.json').version")"
...
if [ "$mock_ready" != "1" ]; then
  ...
  daisyclaw_e2e_print_log "$mock_log" >&2
  exit 1
fi

mkdir -p "$(dirname "$config_path")" "$HOME/.daisyclaw/workspace" "$HOME/.daisyclaw/agents/main/sessions" "$HOME/workspace"
...
daisyclaw gateway run --verbose >"$gateway_log" 2>&1 &
gateway_pid="$!"
