...
const groupId = process.env.DAISYCLAW_QA_TELEGRAM_GROUP_ID;
const driverToken = process.env.DAISYCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN;
const sutToken = process.env.DAISYCLAW_QA_TELEGRAM_SUT_BOT_TOKEN;
const outputDir = process.env.DAISYCLAW_NPM_TELEGRAM_OUTPUT_DIR ?? ".artifacts/rtt/raw";
const telegramApiBaseUrl = (
  process.env.DAISYCLAW_QA_TELEGRAM_API_BASE_URL ?? "https://api.telegram.org"
).replace(/\/+$/u, "");
const timeoutMs = readPositiveIntEnv("DAISYCLAW_QA_TELEGRAM_SCENARIO_TIMEOUT_MS", 180000);
const canaryTimeoutMs = readPositiveIntEnv("DAISYCLAW_QA_TELEGRAM_CANARY_TIMEOUT_MS", timeoutMs);
const warmSampleCount = readPositiveIntEnv("DAISYCLAW_NPM_TELEGRAM_WARM_SAMPLES", 20);
const sampleTimeoutMs = readPositiveIntEnv("DAISYCLAW_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS", 30000);
const botApiTimeoutMs = readPositiveIntEnv("DAISYCLAW_NPM_TELEGRAM_BOT_API_TIMEOUT_MS", 30000);
const botApiBodyMaxBytes = readPositiveIntEnv(
  "DAISYCLAW_NPM_TELEGRAM_BOT_API_BODY_MAX_BYTES",
  1024 * 1024,
);
const maxWarmFailures = readPositiveIntEnv("DAISYCLAW_NPM_TELEGRAM_MAX_FAILURES", warmSampleCount);
const successMarker = process.env.DAISYCLAW_NPM_TELEGRAM_SUCCESS_MARKER ?? "DAISYCLAW_E2E_OK";
const supportedScenarioIds = new Set(["telegram-mentioned-message-reply"]);
const requestedScenarioIds = (
  process.env.DAISYCLAW_NPM_TELEGRAM_SCENARIOS ?? "telegram-mentioned-message-reply"
)
  .split(",")
...
if (requestedScenarioIds.length === 0) {
  throw new Error("DAISYCLAW_NPM_TELEGRAM_SCENARIOS must include at least one RTT scenario");
}
...
if (unknownScenarioIds.length > 0) {
  throw new Error(`unknown DAISYCLAW_NPM_TELEGRAM_SCENARIOS: ${unknownScenarioIds.join(", ")}`);
}
...
if (!groupId || !driverToken || !sutToken) {
  throw new Error(
    "missing Telegram env: DAISYCLAW_QA_TELEGRAM_GROUP_ID, DAISYCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN, DAISYCLAW_QA_TELEGRAM_SUT_BOT_TOKEN",
  );
}
