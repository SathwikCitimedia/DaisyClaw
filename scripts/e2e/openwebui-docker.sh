#!/usr/bin/env bash
# Runs Open WebUI against a Dockerized DaisyClaw Gateway and verifies the proxied
...
IMAGE_NAME="$(docker_e2e_resolve_image "daisyclaw-openwebui-e2e" DAISYCLAW_OPENWEBUI_E2E_IMAGE)"
OPENWEBUI_IMAGE="${OPENWEBUI_IMAGE:-ghcr.io/open-webui/open-webui:v0.8.10}"
MAX_MEMORY_MIB="${DAISYCLAW_OPENWEBUI_MAX_MEMORY_MIB:-8192}"
MAX_CPU_PERCENT="${DAISYCLAW_OPENWEBUI_MAX_CPU_PERCENT:-1600}"
...
MODEL="${DAISYCLAW_OPENWEBUI_MODEL:-openai/gpt-5.5}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${DAISYCLAW_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}"
PORT="${DAISYCLAW_OPENWEBUI_GATEWAY_PORT:-18789}"
WEBUI_PORT="${DAISYCLAW_OPENWEBUI_PORT:-8080}"
TOKEN="openwebui-e2e-$(date +%s)-$$"
ADMIN_EMAIL="${DAISYCLAW_OPENWEBUI_ADMIN_EMAIL:-openwebui-e2e@example.com}"
ADMIN_PASSWORD="${DAISYCLAW_OPENWEBUI_ADMIN_PASSWORD:-OpenWebUI-E2E-Password-$(date +%s)-$$}"
NET_NAME="daisyclaw-openwebui-e2e-$$"
GW_NAME="daisyclaw-openwebui-gateway-$$"
OW_NAME="daisyclaw-openwebui-$$"
PROVIDER_TIMEOUT_SECONDS="${DAISYCLAW_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS:-900}"
PROBE_FETCH_TIMEOUT_MS="${DAISYCLAW_OPENWEBUI_FETCH_TIMEOUT_MS:-$((PROVIDER_TIMEOUT_SECONDS * 1000 + 60000))}"
DOCKER_COMMAND_TIMEOUT="${DAISYCLAW_OPENWEBUI_DOCKER_COMMAND_TIMEOUT:-$((PROVIDER_TIMEOUT_SECONDS + 90))s}"
DOCKER_PULL_TIMEOUT="${DAISYCLAW_OPENWEBUI_DOCKER_PULL_TIMEOUT:-600s}"
...
PROFILE_FILE="${DAISYCLAW_TESTBOX_PROFILE_FILE:-$HOME/.daisyclaw-testbox-live.profile}"
...
STATS_LOG="$(mktemp "${TMPDIR:-/tmp}/daisyclaw-openwebui-stats.XXXXXX")"
PROBE_LOG="$(mktemp "${TMPDIR:-/tmp}/daisyclaw-openwebui-probe.XXXXXX")"
STATS_STOP_FILE="$(mktemp "${TMPDIR:-/tmp}/daisyclaw-openwebui-stats-stop.XXXXXX")"
STATS_PIDS=()
...
docker_e2e_docker_cmd run -d \
  ...
  bash -lc '
    ...
    batch_file="$(mktemp /tmp/daisyclaw-openwebui-config.XXXXXX.json)"
    DAISYCLAW_CONFIG_BATCH_PATH="$batch_file" node scripts/e2e/lib/fixture.mjs openwebui-config "$openai_api_key"
    node "$entry" config set --batch-file "$batch_file" >/dev/null
    rm -f "$batch_file"
    node scripts/e2e/lib/fixture.mjs openwebui-workspace

    daisyclaw_e2e_exec_gateway "$entry" '"$PORT"' lan /tmp/openwebui-gateway.log
  ' >/dev/null
start_openwebui_stats_sampler

echo "Waiting for gateway HTTP surface..."
if ! docker_e2e_wait_container_bash "$GW_NAME" 240 1 "DAISYCLAW_HTTP_PROBE_BEARER='$TOKEN' node scripts/e2e/lib/openwebui/http-probe.mjs 'http://127.0.0.1:$PORT/v1/models' 200"; then
...
docker_e2e_docker_cmd run -d \
  ...
  --network "$NET_NAME" \
  -e ENV=prod \
  -e WEBUI_NAME="DaisyClaw E2E" \
  -e WEBUI_SECRET_KEY="daisyclaw-openwebui-e2e-secret-key-v1" \
  -e OFFLINE_MODE=True \
  ...
  -e WEBUI_ADMIN_NAME="DaisyClaw E2E" \
  -e ENABLE_SIGNUP=False \
  -e DEFAULT_MODELS="daisyclaw/default" \
...
if ! docker_e2e_wait_container_bash "$GW_NAME" 90 5 "DAISYCLAW_HTTP_PROBE_BEARER='$TOKEN' DAISYCLAW_HTTP_PROBE_TIMEOUT_MS=8000 node scripts/e2e/lib/openwebui/http-probe.mjs 'http://$GW_NAME:$PORT/v1/models' 200"; then
...
echo "Running Open WebUI -> DaisyClaw smoke..."
set +e
