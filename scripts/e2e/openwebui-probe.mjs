...
const smokeMode =
  process.env.OPENWEBUI_SMOKE_MODE ?? process.env.DAISYCLAW_OPENWEBUI_SMOKE_MODE ?? "chat";
...
for (let attempt = 1; attempt <= modelAttempts; attempt += 1) {
  ...
  if (modelsResult?.ok) {
    ...
    targetModel =
      modelIds.find((id) => id === "daisyclaw/default") ?? modelIds.find((id) => id === "daisyclaw");
    if (targetModel) {
      break;
    }
    lastModelsError = `missing daisyclaw model: ${JSON.stringify(modelIds)}`;
...
if (!targetModel) {
  throw new Error(
    `daisyclaw model missing from Open WebUI model list after retry: ${JSON.stringify(modelIds)} (${lastModelsError})`,
  );
}
if (smokeMode === "models") {
