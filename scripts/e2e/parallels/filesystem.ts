// Filesystem script supports DaisyClaw repository automation.
...
const DAISYCLAW_VERSION_PATTERN = /DaisyClaw\s+([0-9][^\s]*)/gi;
...
export async function extractLastDaisyClawVersionFromLog(
  logPath: string,
  pattern = DAISYCLAW_VERSION_PATTERN,
  maxBytes = DEFAULT_TEXT_FILE_TAIL_BYTES,
): Promise<string> {
...
export async function makeTempDir(prefix: string): Promise<string> {
  const root =
    process.env.DAISYCLAW_PARALLELS_ARTIFACT_ROOT || path.join(repoRoot, ".artifacts", "parallels");
