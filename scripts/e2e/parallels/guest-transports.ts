// Guest Transports script supports DaisyClaw repository automation.
...
export async function runWindowsBackgroundPowerShell(
  ...
  const fileBase = `daisyclaw-parallels-${nonce}`;
...
${pathsScript}
function Add-DaisyClawBackgroundLog {
...
try {
  & {
${options.script}
  } *>&1 | Add-DaisyClawBackgroundLog
  Set-Content -Path $exitPath -Value '0' -Encoding UTF8
} catch {
  $_ | Add-DaisyClawBackgroundLog
...
if (Test-Path $logPath) {
  ...
  try {
    ...
    "__DAISYCLAW_LOG_LENGTH__:$length"
    if ($length -gt $offset) {
      ...
      if ($read -gt 0) {
        ...
        "__DAISYCLAW_LOG_OFFSET__:$nextOffset"
...
if (Test-Path $donePath) {
  ...
  "__DAISYCLAW_BACKGROUND_EXIT__:$backgroundExit"
  ...
  exit 0
      ...
      const offsetMatch = poll.stdout.match(/__DAISYCLAW_LOG_OFFSET__:(\d+)/);
      if (offsetMatch) {
        lastLogOffset = Number(offsetMatch[1]);
      }
      const lengthMatch = poll.stdout.match(/__DAISYCLAW_LOG_LENGTH__:(\d+)/);
      const logLength = lengthMatch ? Number(lengthMatch[1]) : lastLogOffset;
      if (poll.stdout.includes("__DAISYCLAW_BACKGROUND_DONE__")) {
        doneSeen = true;
        ...
        const exitMatch = poll.stdout.match(/__DAISYCLAW_BACKGROUND_EXIT__:(\S+)/);
...
function cleanupWindowsBackground(
  ...
  const stopProcessTree = options.stopProcessTree
    ? `function Stop-DaisyClawBackgroundProcessTree([int]$ProcessId) {
  Get-CimInstance Win32_Process -Filter "ParentProcessId=$ProcessId" -ErrorAction SilentlyContinue | ForEach-Object {
    Stop-DaisyClawBackgroundProcessTree ([int]$_.ProcessId)
  }
...
if (Test-Path $pidPath) {
  ...
  if ($backgroundPid) {
    Stop-DaisyClawBackgroundProcessTree ([int]$backgroundPid)
  }
}
`
    : "";
...
export class LinuxGuest {
  ...
  exec(args: string[], options: GuestExecOptions = {}): string {
    const result = run(
      "prlctl",
      ["exec", this.vmName, "/usr/bin/env", "HOME=/root", "DAISYCLAW_ALLOW_ROOT=1", ...args],
      {
        check: false,
        input: options.input,
        quiet: true,
  ...
  bash(script: string): string {
    const scriptPath = `/tmp/daisyclaw-parallels-${process.pid}-${Date.now()}.sh`;
    const write = run(
      "prlctl",
      [
        "exec",
...
export class WindowsGuest {
  ...
  powershell(script: string, options: GuestExecOptions = {}): string {
    const scriptName = `daisyclaw-parallels-${process.pid}-${Date.now()}.ps1`;
