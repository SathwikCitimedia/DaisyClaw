...
// Linux Smoke script supports DaisyClaw repository automation.
...
function parseDaisyClawPackageVersion(value: string): string | null {
  return value.match(/\b(\d{4}\.\d{1,2}\.\d{1,2}(?:-[A-Za-z0-9.]+)?)\b/u)?.[1] ?? null;
}

function compareDaisyClawPackageVersions(left: string, right: string): number {
  const parse = (value: string): [number, number, number] => {
    const match = parseDaisyClawPackageVersion(value)?.match(/^(\d{4})\.(\d+)\.(\d+)/u);
    if (!match) {
      return [0, 0, 0];
    }
...
class LinuxSmoke extends SmokeRunController<LinuxOptions> {
  ...
  private disableBonjour = parseBoolEnv(process.env.DAISYCLAW_PARALLELS_LINUX_DISABLE_BONJOUR);
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("daisyclaw-parallels-linux.");
    this.phases = new PhaseRunner(this.runDir);
    this.tgzDir = await makeTempDir("daisyclaw-parallels-linux-tgz.");
    try {
  ...
  protected async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", 420, () =>
      this.installMainTgz("daisyclaw-main-fresh.tgz"),
    );
  ...
  protected async runUpgradeLane(): Promise<void> {
    ...
    await this.phase("upgrade.install-main", 420, () =>
      this.installMainTgz("daisyclaw-main-upgrade.tgz"),
    );
...
printf 'preflight.npmRoot=%s\n' "$(npm root -g 2>/dev/null || true)"`);
  ...
  private installLatestRelease(): void {
    this.guestExec(["curl", "-fsSL", this.options.installUrl, "-o", "/tmp/daisyclaw-install.sh"]);
    if (this.options.installVersion) {
      this.guestExec([
        "/usr/bin/env",
        "DAISYCLAW_NO_ONBOARD=1",
        "bash",
        "/tmp/daisyclaw-install.sh",
        "--version",
    ...
    this.guestExec(["daisyclaw", "--version"]);
  }
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestExec(["daisyclaw", "--version"]);
  ...
  private injectBadPluginFixture(): void {
    this.guestBash(String.raw`set -euo pipefail
plugin_dir=/root/.daisyclaw/test-bad-plugin
mkdir -p "$plugin_dir"
cat >"$plugin_dir/package.json" <<'JSON'
{"name":"@daisyclaw/test-bad-plugin","version":"1.0.0","daisyclaw":{"extensions":["./index.cjs"],"setupEntry":"./setup-entry.cjs"}}
JSON
cat >"$plugin_dir/daisyclaw.plugin.json" <<'JSON'
...
config_path = Path("/root/.daisyclaw/daisyclaw.json")
...
PY`);
  ...
  private shouldExpectBadPluginDiagnostic(lane: "fresh" | "upgrade"): boolean {
    const version = parseDaisyClawPackageVersion(this.versionForLane(lane));
    if (!version) {
      return true;
    }
    return compareDaisyClawPackageVersions(version, BAD_PLUGIN_DIAGNOSTIC_MIN_VERSION) >= 0;
  }
  ...
  private startGatewayBackground(): void {
    const bonjourEnv = this.disableBonjour ? " DAISYCLAW_DISABLE_BONJOUR=1" : "";
    this.guestBash(
      String.raw`pkill -f "daisyclaw gateway run" >/dev/null 2>&1 || true
rm -f /tmp/daisyclaw-parallels-linux-gateway.log
setsid sh -lc ` +
        shellQuote(
          `exec env DAISYCLAW_HOME=/root DAISYCLAW_STATE_DIR=/root/.daisyclaw DAISYCLAW_CONFIG_PATH=/root/.daisyclaw/daisyclaw.json DAISYCLAW_ALLOW_ROOT=1${bonjourEnv} ${this.auth.apiKeyEnv}=${shellQuote(
  ...
  private showGatewayStatusCompat(check = true): boolean {
    const help = this.guestExec(["daisyclaw", "gateway", "status", "--help"], { check: false });
    const args = help.includes("--require-rpc")
      ? ["daisyclaw", "gateway", "status", "--deep", "--require-rpc"]
      : ["daisyclaw", "gateway", "status", "--deep"];
    const result = run(
      "prlctl",
      ["exec", this.options.vmName, "/usr/bin/env", "HOME=/root", "DAISYCLAW_ALLOW_ROOT=1", ...args],
      {
        check: false,
        quiet: true,
        timeoutMs: this.remainingPhaseTimeoutMs(),
...
if isinstance(paths, list):
    load["paths"] = [path for path in paths if path != "/root/.daisyclaw/test-bad-plugin"]
allow = plugins.get("allow")
if isinstance(allow, list):
    plugins["allow"] = [plugin_id for plugin_id in allow if plugin_id != "test-bad-plugin"]
config_path.write_text(json.dumps(config, indent=2) + "\n")
PY
rm -rf /root/.daisyclaw/test-bad-plugin`);
  }
  ...
  private verifyLocalTurn(): void {
    this.guestExec(["daisyclaw", "models", "set", this.auth.modelId]);
...
JSON
daisyclaw config set --batch-file "$provider_config_batch" --strict-json
rm -f "$provider_config_batch"`);
    ...
    this.guestExec(["daisyclaw", "config", "set", "tools.profile", "minimal"]);
...
for attempt in 1 2; do
  ...
  output_file="$(mktemp)"
  set +e
  /usr/bin/env DAISYCLAW_ALLOW_ROOT=1 ${shellQuote(`${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`)} daisyclaw agent --local --agent main --session-id "$session_id" --message ${shellQuote(
...
if [ "$agent_ok" != true ]; then
  ...
  private async extractLastVersion(phaseId: string): Promise<string> {
    return await extractLastDaisyClawVersion(
      this.runDir,
      phaseId,
      /(DaisyClaw [^\r\n]+ \([0-9a-f]{7,}\))/g,
    );
