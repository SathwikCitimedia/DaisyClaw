...
// Macos Smoke script supports DaisyClaw repository automation.
...
const guestDaisyClaw = "daisyclaw";
const guestDaisyClawEntry = '"$(npm root -g)/daisyclaw/daisyclaw.mjs"';
const guestDaisyClawEntryRunner = `node ${guestDaisyClawEntry}`;
...
const defaultOptions = (): MacosOptions => ({
  ...
  installUrl: "https://daisyclaw.ai/install.sh",
  installVersion: "",
  json: false,
  keepServer: false,
  latestVersion: "",
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://daisyclaw.ai/install.sh
...
class MacosSmoke {
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("daisyclaw-parallels-macos.");
    ...
    this.tgzDir = await makeTempDir("daisyclaw-parallels-macos-tgz.");
    try {
      ...
      if (await this.needsHostTgz()) {
        this.artifact = await packDaisyClaw({
  ...
  private async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", this.targetInstallsDirectly() ? 420 : 420, () =>
      this.installMain("daisyclaw-main-fresh.tgz"),
    );
  ...
  private async runUpgradeLane(): Promise<void> {
    ...
    if (this.options.targetPackageSpec) {
      await this.phase("upgrade.install-main", this.targetInstallsDirectly() ? 420 : 420, () =>
        this.installMain("daisyclaw-main-upgrade.tgz"),
      );
  ...
  private guestDaisyClawEntryExec(
    args: string[],
    options: { check?: boolean; env?: Record<string, string> } = {},
  ): string {
    ...
    return this.guestSh(
      `set -e
entry="$(npm root -g)/daisyclaw/daisyclaw.mjs"
exec node "$entry" ${argv}`,
  ...
  private resetState(): void {
    this.guestSh(String.raw`/usr/bin/pkill -f 'daisyclaw.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'daisyclaw-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'daisyclaw.mjs gateway' >/dev/null 2>&1 || true
...
${guestNpm} uninstall -g daisyclaw >/dev/null 2>&1 || true
rm -rf "$HOME/.daisyclaw"
...
curl -fsSL ${shellQuote(this.options.installUrl)} -o /tmp/daisyclaw-install.sh
bash /tmp/daisyclaw-install.sh --version ${shellQuote(this.installVersion)}
${guestDaisyClaw} --version`,
    );
  }
...
${guestDaisyClaw} --version`);
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestExec([guestDaisyClaw, "--version"]);
...
check_path "$root/daisyclaw"
check_path "$root/daisyclaw/extensions"
if [ -d "$root/daisyclaw/extensions" ]; then
  while IFS= read -r -d '' extension_dir; do
    check_path "$extension_dir"
  done < <(/usr/bin/find "$root/daisyclaw/extensions" -mindepth 1 -maxdepth 1 -type d -print0)
fi`);
  }
  ...
  private ensureGuestPnpm(): void {
    this.guestSh(String.raw`set -eu
bootstrap_root=/tmp/daisyclaw-smoke-pnpm-bootstrap
...
"$bootstrap_bin/pnpm" --version`);
  ...
  private runDevChannelUpdate(): void {
    ...
    this.guestSh(
      `set -eu
rm -rf ${shellQuote(`${home}/daisyclaw`)}
export PATH=${shellQuote(`/tmp/daisyclaw-smoke-pnpm-bootstrap/node_modules/.bin:${guestPath}`)}
${guestNode} - <<'JS'
const fs = require("node:fs");
const path = require("node:path");
const configPath = path.join(process.env.HOME || ${JSON.stringify(home)}, ".daisyclaw", "daisyclaw.json");
const config = JSON.parse(fs.readFileSync(configPath, "utf8"));
config.update = { ...(config.update || {}), channel: "dev" };
fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + "\\n");
JS
/usr/bin/env NODE_OPTIONS=--max-old-space-size=8192 DAISYCLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 DAISYCLAW_DISABLE_BUNDLED_PLUGINS=1 ${guestDaisyClawEntryRunner} update --channel dev --yes --json
${guestDaisyClawEntryRunner} --version
${guestDaisyClawEntryRunner} update status --json`,
    );
  }

  private verifyDevChannelUpdate(): void {
    const status = this.guestDaisyClawEntryExec(["update", "status", "--json"]);
  ...
  private startManualGatewayIfNeeded(): void {
    ...
    this.guestSh(
      `set -euo pipefail
trap '' HUP
/usr/bin/pkill -f 'daisyclaw.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'daisyclaw-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'daisyclaw.mjs gateway' >/dev/null 2>&1 || true
/usr/bin/env HOME=${shellQuote(home)} USER=${shellQuote(this.guestUser)} LOGNAME=${shellQuote(this.guestUser)} PATH=${shellQuote(guestPath)} ${shellQuote(
        `${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`,
      )} DAISYCLAW_HOME=${shellQuote(home)} DAISYCLAW_STATE_DIR=${shellQuote(`${home}/.daisyclaw`)} DAISYCLAW_CONFIG_PATH=${shellQuote(
        `${home}/.daisyclaw/daisyclaw.json`,
      )} ${guestDaisyClawEntryRunner} gateway run --bind loopback --port 18789 --force </dev/null >/tmp/daisyclaw-parallels-macos-gateway.log 2>&1 &
sleep 1`,
    );
  }
  ...
  private verifyGateway(): void {
    for (let attempt = 1; attempt <= 8; attempt++) {
      const result = this.guestDaisyClaw(
  ...
  private showGatewayStatusCompat(): void {
    const help = this.guestExec([guestDaisyClaw, "gateway", "status", "--help"], { check: false });
    const args = help.includes("--require-rpc")
      ? ["gateway", "status", "--deep", "--require-rpc"]
      : ["gateway", "status", "--deep"];
    if (!this.guestDaisyClaw(args, false)) {
      throw new Error("gateway status failed");
    }
  }

  private guestDaisyClaw(args: string[], check: boolean): boolean {
    const result = run(
      "prlctl",
      [
        "exec",
    ...
    if (check && result.status !== 0) {
      throw new Error(`daisyclaw ${args.join(" ")} failed`);
    }
...
while [ $SECONDS -lt $deadline ]; do
  if curl -fsSL --connect-timeout 2 --max-time 5 http://127.0.0.1:18789/ >/tmp/daisyclaw-dashboard-smoke.html 2>/dev/null; then
    if grep -F '<title>DaisyClaw Control</title>' /tmp/daisyclaw-dashboard-smoke.html >/dev/null &&
      grep -F '<daisyclaw-app></daisyclaw-app>' /tmp/daisyclaw-dashboard-smoke.html >/dev/null; then
      asset_paths="$(
        sed -nE 's/.*<(script|link)[^>]*(src|href)=["'"'"']([^"'"'"']+)["'"'"'].*/\3/p' /tmp/daisyclaw-dashboard-smoke.html |
          grep -E '(^|/)assets/' |
          grep -Ev '^(https?:)?//' |
          sort -u
      )"
...
exit 1`);
  ...
  private verifyTurn(): void {
    this.guestDaisyClawEntryExec(["models", "set", this.auth.modelId]);
...
JSON
${guestDaisyClawEntryRunner} config set --batch-file "$provider_config_batch" --strict-json
rm -f "$provider_config_batch"`);
    }
    this.guestDaisyClawEntryExec([
      "config",
      "set",
    ...
    this.guestDaisyClawEntryExec(["config", "set", "tools.profile", "minimal"]);
...
for attempt in 1 2; do
  ...
  output_file="$(mktemp)"
  set +e
  /usr/bin/env ${shellQuote(`${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`)} ${guestDaisyClawEntryRunner} agent --local --agent main --session-id "$session_id" --message ${shellQuote(
...
if [ "$agent_ok" != true ]; then
  ...
  private ensureDiscordGatewayReady(): void {
    ...
    this.verifyGateway();
    const status = this.guestDaisyClawEntryExec(["channels", "status", "--probe", "--json"]);
  ...
  private async extractLastVersion(phaseName: string): Promise<string> {
    return await extractLastDaisyClawVersionFromLog(path.join(this.runDir, `${phaseName}.log`));
  }
