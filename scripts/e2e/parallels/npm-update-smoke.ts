...
// Npm Update Smoke script supports DaisyClaw repository automation.
...
import {
  ...
  resolveDaisyClawRegistryVersion,
  resolveProviderAuth,
...
const updateTimeoutSeconds = readPositiveIntEnv("DAISYCLAW_PARALLELS_NPM_UPDATE_TIMEOUT_S", 1200);
const updateCleanupBackstopMs = 60_000;
const freshLaneTimeoutKillGraceMs = readPositiveIntEnv(
  "DAISYCLAW_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_KILL_GRACE_MS",
  2_000,
);
...
export function freshLaneTimeoutMs(platform: Platform): number {
  ...
  return readPositiveIntEnv("DAISYCLAW_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_S", defaultSeconds) * 1000;
}
...
Options:
  --package-spec <npm-spec>  Baseline npm package spec. Default: daisyclaw@latest
  --update-target <target>    Target passed to guest 'daisyclaw update --tag'.
...
function daisyClawVersionFamily(version: string): string {
  return /^(\d{4}\.\d{1,2}\.\d{1,2})(?:[-.]|$)/u.exec(version.trim())?.[1] ?? "";
}

function parseDaisyClawPackageSpecVersion(spec: string): string {
  const value = spec.trim();
  if (!value) {
    return "";
  }
  return resolveDaisyClawRegistryVersion(value) || "";
}
...
export class NpmUpdateSmoke {
  ...
  async run(): Promise<void> {
    ...
    this.runDir = await this.makeRunTempDir("daisyclaw-parallels-npm-update.");
    this.tgzDir = await this.makeRunTempDir("daisyclaw-parallels-npm-update-tgz.");
    try {
      await this.runSteps();
    } finally {
  ...
  protected async runSteps(): Promise<void> {
    ...
    this.packageSpec = this.options.packageSpec || `daisyclaw@${this.latestVersion}`;
    ...
    say(`Run same-guest daisyclaw update to ${this.updateTargetEffective}`);
    ...
    if (this.options.platforms.has("linux")) {
      jobs.push(
        this.spawnFresh("Linux", "linux", ["--vm", this.linuxVm], {
          DAISYCLAW_PARALLELS_LINUX_DISABLE_BONJOUR: "1",
        }),
      );
    }
  ...
  private async prepareUpdateTarget(): Promise<void> {
    ...
    this.updateExpectedNeedle = this.isExplicitPackageTarget(this.updateTargetEffective)
      ? ""
      : resolveDaisyClawRegistryVersion(this.updateTargetEffective) || this.updateTargetEffective;
  ...
  private resolveRegistryPackageMetadata(target: string): {
    ...
    const spec = target.startsWith("daisyclaw@") ? target : `daisyclaw@${target}`;
  ...
  private async guestMacos(
    ...
    const scriptPath = this.writeGuestScript(
      macosVm,
      script,
      "daisyclaw-parallels-npm-update-macos",
    );
  ...
  private async guestLinux(
    ...
    const scriptPath = this.writeGuestScript(
      this.linuxVm,
      script,
      "daisyclaw-parallels-npm-update-linux",
    );
    try {
      const status = await this.runStreamingToJobLog(
        "prlctl",
  ...
  private preflightRegistryUpdateTarget(): void {
    ...
    const baseline = resolveDaisyClawRegistryVersion(this.packageSpec);
    const target = resolveDaisyClawRegistryVersion(this.options.updateTarget);
    if (baseline && target && baseline === target) {
      die(
        `--update-target ${this.options.updateTarget} resolves to daisyclaw@${target}, same as baseline ${this.packageSpec}; publish or choose a newer --update-target before running VM update coverage`,
      );
    }
  }
  ...
  private async extractLastVersion(logPath: string): Promise<string> {
    return await extractLastDaisyClawVersionFromLog(logPath);
  }
  ...
  private configurePublishedTargets(): void {
    if (this.options.betaValidation) {
      const version = resolveDaisyClawRegistryVersion(this.options.betaValidation);
      if (!version) {
        die(`could not resolve beta validation target: ${this.options.betaValidation}`);
      }
      this.options.updateTarget = version;
      this.options.freshTargetSpec = `daisyclaw@${version}`;
      say(`Beta validation target: daisyclaw@${version}`);
    } else if (
      this.options.updateTarget &&
      this.options.updateTarget !== "local-main" &&
      !this.isExplicitPackageTarget(this.options.updateTarget)
    ) {
      const version = resolveDaisyClawRegistryVersion(this.options.updateTarget);
      if (version) {
        this.options.updateTarget = version;
      }
    }
    ...
    if (this.options.freshTargetSpec) {
      const version = resolveDaisyClawRegistryVersion(this.options.freshTargetSpec);
      this.freshTargetSpec = version ? `daisyclaw@${version}` : this.options.freshTargetSpec;
    }
  }

  private assertPublishedTargetMatchesHarnessCheckout(): void {
    if (process.env.DAISYCLAW_PARALLELS_ALLOW_HARNESS_TARGET_MISMATCH === "1") {
      return;
    }
    const candidateVersion = this.freshTargetSpec
      ? parseDaisyClawPackageSpecVersion(this.freshTargetSpec)
      : parseDaisyClawPackageSpecVersion(this.options.updateTarget);
    const targetFamily = daisyClawVersionFamily(candidateVersion);
    if (!targetFamily) {
      return;
    }
    this.harnessTargetFamily = targetFamily;
    const checkoutFamily = daisyClawVersionFamily(this.harnessCheckoutVersion);
    if (checkoutFamily === targetFamily) {
      return;
    }
    die(
      `refusing to run Parallels ${candidateVersion} target with harness checkout ${this.harnessCheckoutVersion || "unknown"}; checkout the matching release branch or set DAISYCLAW_PARALLELS_ALLOW_HARNESS_TARGET_MISMATCH=1 for an intentional cross-version harness run`,
    );
  }
