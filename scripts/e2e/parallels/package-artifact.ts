// Package Artifact script supports DaisyClaw repository automation.
...
export function resolveDaisyClawRegistryVersion(specOrAlias: string): string {
  const rawValue = specOrAlias.trim();
  const value = rawValue.startsWith("daisyclaw@") ? rawValue.slice("daisyclaw@".length) : rawValue;
  if (!value) {
    return "";
  }
  if (value === "latest" || value === "beta" || /^\d/.test(value)) {
    return npmViewVersion(`daisyclaw@${value}`);
  }
  const betaMatch = /^beta(\d+)$/u.exec(value);
  if (betaMatch) {
    const betaSuffix = `-beta.${betaMatch[1]}`;
    const versions = JSON.parse(
      run("npm", ["view", "daisyclaw", "versions", "--json"], { quiet: true }).stdout,
    ) as string[];
    const match = versions
      ...
      .at(-1);
    if (!match) {
      die(`no daisyclaw registry version found for alias ${value}`);
    }
    return match;
  }
  return "";
...
export async function packDaisyClaw(input: {
  destination: string;
  packageSpec?: string;
  requireControlUi?: boolean;
  ...
  return await withPackageLock(path.join(tmpdir(), "daisyclaw-parallels-build.lock"), async () => {
    ...
    const tgzPath = path.join(input.destination, `daisyclaw-main-${shortHead}.tgz`);
...
async function acquirePackageLock(lockDir: string, ownerToken: string): Promise<void> {
  const timeoutMs = readPositiveIntEnv("DAISYCLAW_PARALLELS_PACKAGE_LOCK_TIMEOUT_MS", 30 * 60_000);
  const staleMs = readPositiveIntEnv("DAISYCLAW_PARALLELS_PACKAGE_LOCK_STALE_MS", 2 * 60 * 60_000);
