// Plugin Isolation script supports DaisyClaw repository automation.
...
export function posixProviderOnlyPluginIsolationScript(options: PluginIsolationOptions): string {
  ...
  const homeEnv = options.homeFallback
    ? `DAISYCLAW_PARALLELS_HOME=${shellQuote(options.homeFallback)} `
    : "";
...
export function windowsProviderOnlyPluginIsolationScript(options: PluginIsolationOptions): string {
  ...
  return `$env:DAISYCLAW_PARALLELS_PLUGIN_ISOLATION = @'
${payloadJson}
'@
$isolationScriptPath = Join-Path ([System.IO.Path]::GetTempPath()) 'daisyclaw-parallels-plugin-isolation.cjs'
@'
...
Remove-Item Env:DAISYCLAW_PARALLELS_PLUGIN_ISOLATION -Force -ErrorAction SilentlyContinue`;
}
...
function providerOnlyPluginIsolationNodeScript(options: PluginIsolationOptions): string {
  ...
  return `process.env.DAISYCLAW_PARALLELS_PLUGIN_ISOLATION = ${JSON.stringify(payloadJson)};
${providerOnlyPluginIsolationNodeSource()}`;
}
...
const payload = JSON.parse(process.env.DAISYCLAW_PARALLELS_PLUGIN_ISOLATION || "{}");
const home =
  process.env.DAISYCLAW_PARALLELS_HOME ||
...
fs.rmSync(path.join(stateDir, "npm", "node_modules", "@daisyclaw", "codex"), {
  recursive: true,
  force: true,
});
