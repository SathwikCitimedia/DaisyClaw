// Powershell script supports DaisyClaw repository automation.
import {
  configPathMapKey,
...
export function windowsModelProviderTimeoutScript(modelId: string): string {
  ...
  return `$providerTimeoutBatchPath = Join-Path ([System.IO.Path]::GetTempPath()) 'daisyclaw-provider-timeout.batch.json'
@'
${batchJson}
'@ | Set-Content -Path $providerTimeoutBatchPath -Encoding UTF8
Invoke-DaisyClaw config set --batch-file $providerTimeoutBatchPath --strict-json
...
export function windowsAgentTurnConfigPatchScript(modelId: string): string {
  ...
  return `$agentTurnConfigPatchPath = $env:DAISYCLAW_CONFIG_PATH
if (-not $agentTurnConfigPatchPath) { $agentTurnConfigPatchPath = Join-Path $env:USERPROFILE '.daisyclaw\\daisyclaw.json' }
$agentTurnVersionText = Invoke-DaisyClaw --version 2>$null | Out-String
$agentTurnRuntimePolicySupported = $false
if ($agentTurnVersionText -match 'DaisyClaw\\s+(\\d{4})\\.(\\d{1,2})\\.(\\d{1,2})') {
...
$env:DAISYCLAW_PARALLELS_AGENT_CONFIG_PATCH = @'
${payloadJson}
'@
$env:DAISYCLAW_PARALLELS_AGENT_CONFIG_PATH = $agentTurnConfigPatchPath
$env:DAISYCLAW_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED = if ($agentTurnRuntimePolicySupported) { '1' } else { '0' }
$agentTurnConfigPatchScriptPath = Join-Path ([System.IO.Path]::GetTempPath()) 'daisyclaw-agent-turn-config-patch.cjs'
@'
const fs = require("node:fs");
const path = require("node:path");
const configPath = process.env.DAISYCLAW_PARALLELS_AGENT_CONFIG_PATH;
const payload = JSON.parse(process.env.DAISYCLAW_PARALLELS_AGENT_CONFIG_PATCH || "{}");
const canWriteAgentRuntime = process.env.DAISYCLAW_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED === "1";
...
fs.rmSync(path.join(stateDir, "npm", "node_modules", "@daisyclaw", "codex"), { recursive: true, force: true });
...
if (selectedModelEntry && typeof selectedModelEntry === "object" && !Array.isArray(selectedModelEntry)) {
  if (canWriteAgentRuntime) {
    selectedModelEntry.agentRuntime = { id: "daisyclaw" };
  } else {
...
Remove-Item Env:DAISYCLAW_PARALLELS_AGENT_CONFIG_PATCH -Force -ErrorAction SilentlyContinue
Remove-Item Env:DAISYCLAW_PARALLELS_AGENT_CONFIG_PATH -Force -ErrorAction SilentlyContinue
Remove-Item Env:DAISYCLAW_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED -Force -ErrorAction SilentlyContinue
if ($agentTurnConfigPatchExit -ne 0) { throw "agent turn config patch failed" }`;
}

export const windowsDaisyClawResolver = String.raw`$portableNode = if ($env:LOCALAPPDATA) { Join-Path $env:LOCALAPPDATA 'Programs\nodejs' } else { $null }
if ($portableNode -and (Test-Path (Join-Path $portableNode 'node.exe'))) {
  $env:PATH = "$portableNode;$env:PATH"
}
function Resolve-DaisyClawCommand {
  if ($script:DaisyClawResolvedCommand) { return $script:DaisyClawResolvedCommand }
  $shimCandidates = @()
  if ($env:APPDATA) {
    $shimCandidates += Join-Path $env:APPDATA 'npm\daisyclaw.cmd'
    $shimCandidates += Join-Path $env:APPDATA 'npm\daisyclaw.ps1'
  }
  foreach ($name in @('daisyclaw.cmd', 'daisyclaw.ps1', 'daisyclaw')) {
  ...
  if ($npmPrefix) {
    $shimCandidates += Join-Path $npmPrefix 'daisyclaw.cmd'
    $shimCandidates += Join-Path $npmPrefix 'daisyclaw.ps1'
  }
  foreach ($candidate in $shimCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:DaisyClawResolvedCommand = @{ Kind = 'shim'; Path = $candidate }
      return $script:DaisyClawResolvedCommand
    }
  }
  $entryCandidates = @()
  if ($env:APPDATA) {
  ...
  if ($npmPrefix) {
    $entryCandidates += Join-Path $npmPrefix 'node_modules\daisyclaw\daisyclaw.mjs'
  }
  foreach ($candidate in $entryCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:DaisyClawResolvedCommand = @{ Kind = 'node'; Path = $candidate }
      return $script:DaisyClawResolvedCommand
    }
  }
  throw 'daisyclaw command not found in PATH, APPDATA npm, or npm global prefix'
}
function Invoke-DaisyClaw {
  param([Parameter(ValueFromRemainingArguments = $true)][string[]] $DaisyClawArgs)
  $command = Resolve-DaisyClawCommand
  ...
  try {
    if ($command.Kind -eq 'node') {
      & node.exe $command.Path @DaisyClawArgs
    } else {
      & $command.Path @DaisyClawArgs
    }
  } finally {
