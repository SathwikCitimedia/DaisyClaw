// Smoke Common helper supports DaisyClaw script workflows.
import { readFile, rm } from "node:fs/promises";
import path from "node:path";
import { extractLastDaisyClawVersionFromLog } from "./filesystem.ts";
...
export async function packAndServeSmokeArtifact(
  ...
  const artifact = await packDaisyClaw({
    destination: tgzDir,
    packageSpec,
    requireControlUi,
  });
...
export async function extractLastDaisyClawVersion(
  runDir: string,
  phaseName: string,
  pattern: RegExp,
): Promise<string> {
  return await extractLastDaisyClawVersionFromLog(path.join(runDir, `${phaseName}.log`), pattern);
}
