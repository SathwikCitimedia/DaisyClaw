...
// Windows Smoke script supports DaisyClaw repository automation.
...
const defaultOptions = (): WindowsOptions => ({
  ...
  installUrl: "https://daisyclaw.ai/install.ps1",
  installVersion: "",
  json: false,
  keepServer: false,
  latestVersion: "",
  ...
  snapshotHint: "pre-daisyclaw-native-e2e-2026-03-12",
  targetPackageSpec: "",
  upgradeFromPackedMain: false,
  vmName: "Windows 11",
});

const windowsPortableGitPathScript = `$portableGit = Join-Path (Join-Path (Join-Path $env:LOCALAPPDATA 'DaisyClaw\\deps') 'portable-git') ''
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-daisyclaw-native-e2e-2026-03-12"
  --mode <fresh|upgrade|both>
  --provider <openai|anthropic|minimax>
  --model <provider/model>    Override the model used for the agent-turn smoke.
  --api-key-env <var>        Host env var name for provider API key.
  --openai-api-key-env <var> Alias for --api-key-env (backward compatible)
  --install-url <url>        Installer URL for latest release. Default: https://daisyclaw.ai/install.ps1
  ...
  --upgrade-from-packed-main
                             ...
                             then run daisyclaw update --channel dev.
...
class WindowsSmoke extends SmokeRunController<WindowsOptions> {
  ...
  private gatewayRecoveryAfterMs =
    readPositiveIntEnv("DAISYCLAW_PARALLELS_WINDOWS_GATEWAY_RECOVERY_AFTER_S", 180) * 1000;
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("daisyclaw-parallels-windows.");
    this.phases = new PhaseRunner(this.runDir);
    this.guest = new WindowsGuest(this.options.vmName, this.phases);
    this.tgzDir = await makeTempDir("daisyclaw-parallels-windows-tgz.");
    try {
  ...
  protected async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", 420, () => this.installMain("daisyclaw-main-fresh.tgz"));
  ...
  private guestPowerShell(
    ...
    return this.guest.powershell(`${windowsDaisyClawResolver}\n${script}`, options);
  }
  ...
  private logGuestPreflight(cleanDaisyClaw: boolean): void {
    const cleanScript = cleanDaisyClaw
      ? "npm.cmd uninstall -g daisyclaw --no-fund --no-audit --loglevel=error 2>$null; $global:LASTEXITCODE = 0"
      : "";
...
if ($LASTEXITCODE -ne 0) { throw "daisyclaw --version failed with exit code $LASTEXITCODE" }`,
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestPowerShell("Invoke-DaisyClaw --version");
...
Invoke-DaisyClaw onboard --non-interactive --mode local --auth-choice ${psSingleQuote(this.auth.authChoice)} --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --skip-health --accept-risk --json
if ($LASTEXITCODE -ne 0) { throw "daisyclaw onboard failed with exit code $LASTEXITCODE" }
${this.windowsPluginIsolationScript()}`,
  ...
  private async guestPowerShellBackground(
    ...
    await runWindowsBackgroundPowerShell({
      ...
      label,
      onLaunchRetry: warn,
      script: `${windowsDaisyClawResolver}\n${script}`,
      timeoutMs,
...
${windowsScopedEnvFunction}
$script:DaisyClawUpdateExit = 0
Invoke-WithScopedEnv @{ DAISYCLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = '1'; DAISYCLAW_DISABLE_BUNDLED_PLUGINS = '1' } {
  Invoke-DaisyClaw update --channel dev --yes --json
  $script:DaisyClawUpdateExit = $LASTEXITCODE
}
if ($script:DaisyClawUpdateExit -ne 0) { throw "daisyclaw update failed with exit code $script:DaisyClawUpdateExit" }
Invoke-DaisyClaw --version
Invoke-DaisyClaw update status --json`,
...
Invoke-DaisyClaw update status --json`,
  ...
  private gatewayAction(action: "restart" | "stop"): Promise<void> {
    return this.guestPowerShellBackground(
      ...
      `$ErrorActionPreference = 'Continue'
$PSNativeCommandUseErrorActionPreference = $false
Invoke-DaisyClaw gateway ${action}
if ($LASTEXITCODE -ne 0) { throw "gateway ${action} failed with exit code $LASTEXITCODE" }`,
  ...
  private verifyGatewayReachable(): void {
    ...
    while (Date.now() < deadline) {
      const probe = this.guestPowerShell(
        "Invoke-DaisyClaw gateway probe --url ws://127.0.0.1:18789 --timeout 30000 --json",
      ...
      if (!recoveryTried && Date.now() - start >= this.gatewayRecoveryAfterMs) {
        ...
        this.guestPowerShell("Invoke-DaisyClaw gateway start", {
          check: false,
          timeoutMs: 120_000,
        });
        recoveryTried = true;
  ...
  private showGatewayStatusCompat(): void {
    const help = this.guestPowerShell("Invoke-DaisyClaw gateway status --help", {
      check: false,
    });
    const suffix = help.includes("--require-rpc") ? "--deep --require-rpc" : "--deep";
    this.guestPowerShell(`Invoke-DaisyClaw gateway status ${suffix}`);
  }
...
for ($attempt = 1; $attempt -le 2; $attempt++) {
  ...
  $output = Invoke-DaisyClaw @args 2>&1
...
if (-not $agentOk) { throw 'daisyclaw agent finished without OK response' }`,
  ...
  private async extractLastVersion(phaseName: string): Promise<string> {
    return await extractLastDaisyClawVersion(this.runDir, phaseName, /DaisyClaw\s+([0-9][^\s]*)/gi);
  }
