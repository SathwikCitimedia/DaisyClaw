...
IMAGE_NAME="${DAISYCLAW_PLUGIN_BINDING_COMMAND_ESCAPE_E2E_IMAGE:-daisyclaw-plugin-binding-command-escape-e2e}"
CONTAINER_NAME="daisyclaw-plugin-binding-command-escape-e2e-$$"
DOCKER_RUN_TIMEOUT="${DAISYCLAW_PLUGIN_BINDING_COMMAND_ESCAPE_DOCKER_RUN_TIMEOUT:-900s}"
RUN_LOG="$(mktemp -t daisyclaw-plugin-binding-command-escape-log.XXXXXX)"
...
DOCKER_COMMAND_TIMEOUT="$DOCKER_RUN_TIMEOUT" docker_e2e_docker_run_cmd run --rm \
  ...
  -e DAISYCLAW_VITEST_FS_MODULE_CACHE_PATH=/tmp/daisyclaw-vitest-cache \
  "$IMAGE_NAME" \
