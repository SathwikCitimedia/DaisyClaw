...
async function verifyRuntimeContextTranscriptShape(root: string) {
  const sessionFile = path.join(root, ".daisyclaw", "agents", "main", "sessions", "runtime.jsonl");
  ...
  assert(
    runtimeContextMessage.customType === "daisyclaw.runtime-context",
  ...
  assert(
    !userText.includes("DAISYCLAW_INTERNAL_CONTEXT") && !userText.includes("secret docker context"),
...
async function main() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-session-runtime-context-"));
  process.env.HOME = root;
  process.env.DAISYCLAW_STATE_DIR = path.join(root, ".daisyclaw");
  process.env.DAISYCLAW_CONFIG_PATH = path.join(process.env.DAISYCLAW_STATE_DIR, "daisyclaw.json");
  try {
    ...
    if (process.env.DAISYCLAW_SESSION_RUNTIME_CONTEXT_KEEP_ARTIFACTS !== "1") {
