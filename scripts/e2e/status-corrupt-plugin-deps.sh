...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/daisyclaw-status-corrupt-plugin-deps.XXXXXX")"
cleanup() {
  rm -rf "$TMP_DIR"
}
trap cleanup EXIT
...
const stageDir = process.env.DAISYCLAW_PLUGIN_STAGE_DIR || "";
const renameResidue = path.join(stageDir, "node_modules", "ansi-escapes", ".daisyclaw-rename-tmp");
...
run_daisyclaw() {
  HOME="$HOME_DIR" \
  DAISYCLAW_HOME="$STATE_DIR" \
  DAISYCLAW_STATE_DIR="$STATE_DIR" \
  DAISYCLAW_CONFIG_PATH="$CONFIG_PATH" \
  DAISYCLAW_PLUGIN_STAGE_DIR="$STAGE_DIR" \
...
run_daisyclaw status --all --timeout 1 > "$BEFORE"
grep -F "e2e-corrupt-chat" "$BEFORE" >/dev/null
grep -F "plugin load failed: dependency tree corrupted; run daisyclaw doctor --fix" "$BEFORE" >/dev/null

run_daisyclaw doctor --fix --non-interactive --yes > "$DOCTOR"
if [[ -e "$STAGE_DIR" ]]; then
  echo "doctor --fix did not remove corrupt plugin stage dir: $STAGE_DIR" >&2
  exit 1
fi

run_daisyclaw status --all --timeout 1 > "$AFTER"
