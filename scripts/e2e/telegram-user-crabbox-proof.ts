...
// Telegram User Crabbox Proof script supports DaisyClaw repository automation.
...
const REMOTE_ROOT = "/tmp/daisyclaw-telegram-user-crabbox";
...
export function readTelegramUserProofLogTailBytes(env: NodeJS.ProcessEnv = process.env): number {
  return readPositiveIntEnv("DAISYCLAW_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES", 256 * 1024, env);
}
...
function usageText() {
  return [
    "Usage:",
    "  node --import tsx scripts/e2e/telegram-user-crabbox-proof.ts [probe] [--text /status] [--expect DaisyClaw]",
    ...
    "  --repo <owner/name>           GitHub repo for publish. Default: daisyclaw/daisyclaw.",
...
function parseArgs(argvInput: string[]): Options {
  ...
  const opts: Options = {
    ...
    command,
    crabboxBin: trimToValue(process.env.DAISYCLAW_TELEGRAM_USER_CRABBOX_BIN) ?? "crabbox",
    desktopChatTitle:
      trimToValue(process.env.DAISYCLAW_TELEGRAM_USER_DESKTOP_CHAT_TITLE) ?? "DaisyClaw Testing",
    dryRun: false,
    expect: ["DaisyClaw"],
    gatewayPort: 19_879,
    idleTimeout: "60m",
    keepBox: false,
    mockResponseText: "DAISYCLAW_E2E_OK",
    mockPort: 19_882,
    outputDir: path.join(DEFAULT_OUTPUT_ROOT, stamp),
    previewCropWidth: TELEGRAM_PROOF_CROP.cropWidth,
    previewFps: 24,
    previewWidth: 1920,
    provider: process.env.DAISYCLAW_TELEGRAM_USER_CRABBOX_PROVIDER?.trim() || "aws",
    publishFullArtifacts: false,
    publishRepo: "daisyclaw/daisyclaw",
    recordFps: 24,
    recordSeconds: 35,
    remoteCommand: [],
    target: "linux",
    ...
    ttl: "120m",
    userDriverScript:
      trimToValue(process.env.DAISYCLAW_TELEGRAM_USER_DRIVER_SCRIPT) ?? DEFAULT_USER_DRIVER,
  };
...
function repoRoot() {
  ...
  if (
    ...
    throw new Error("Run from the DaisyClaw repo root.");
  }
  return cwd;
}
...
function childProcessBaseEnv() {
  const keys = [
    ...
    "LC_ALL",
    "NODE_OPTIONS",
    "DAISYCLAW_BUILD_PRIVATE_QA",
    "DAISYCLAW_ENABLE_PRIVATE_QA_CLI",
    "PATH",
    "PNPM_HOME",
    "SHELL",
    "TEMP",
...
function gatewayEnv(params: { configPath: string; stateDir: string; sutToken: string }) {
  return {
    ...childProcessBaseEnv(),
    OPENAI_API_KEY: "sk-daisyclaw-e2e-mock",
    DAISYCLAW_CONFIG_PATH: params.configPath,
    DAISYCLAW_STATE_DIR: params.stateDir,
    TELEGRAM_BOT_TOKEN: params.sutToken,
  };
}

export function createDaisyClawGatewaySpawnSpec(params: {
  ...
  const spec = createPnpmRunnerSpawnSpec({
    ...
    pnpmArgs: ["daisyclaw", "gateway", "--port", String(params.gatewayPort)],
  });
  return {
    args: spec.args,
...
function writeSutConfig(params: {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-tg-crabbox-sut-"));
  const stateDir = path.join(tempRoot, "state");
  const workspace = path.join(tempRoot, "workspace");
  fs.mkdirSync(stateDir, { recursive: true });
  fs.mkdirSync(workspace, { recursive: true });
  const configPath = path.join(tempRoot, "daisyclaw.json");
  const config = {
    agents: {
      defaults: {
...
type StartLocalSutDeps = {
  createGatewaySpawnSpec?: typeof createDaisyClawGatewaySpawnSpec;
...
export async function startLocalSut(
  ...
  const createGatewaySpawnSpec = deps.createGatewaySpawnSpec ?? createDaisyClawGatewaySpawnSpec;
...
async function startLocalSutDaemon(params: {
  ...
  try {
    ...
    const gatewaySpec = createDaisyClawGatewaySpawnSpec({
      env: gatewayEnvVars,
      gatewayPort: params.gatewayPort,
      repoRoot: params.repoRoot,
    });
...
tdlib_url=${tdlibUrl}
setup_step_timeout_kill_after="\${DAISYCLAW_TELEGRAM_USER_SETUP_KILL_AFTER_SECONDS:-30}s"
apt_timeout="\${DAISYCLAW_TELEGRAM_USER_APT_TIMEOUT_SECONDS:-900}s"
download_timeout="\${DAISYCLAW_TELEGRAM_USER_DOWNLOAD_TIMEOUT_SECONDS:-600}"
download_connect_timeout="\${DAISYCLAW_TELEGRAM_USER_DOWNLOAD_CONNECT_TIMEOUT_SECONDS:-15}"
download_retries="\${DAISYCLAW_TELEGRAM_USER_DOWNLOAD_RETRIES:-3}"
download_retry_delay="\${DAISYCLAW_TELEGRAM_USER_DOWNLOAD_RETRY_DELAY_SECONDS:-5}"
tdlib_clone_timeout="\${DAISYCLAW_TELEGRAM_USER_TDLIB_CLONE_TIMEOUT_SECONDS:-600}s"
tdlib_build_timeout="\${DAISYCLAW_TELEGRAM_USER_TDLIB_BUILD_TIMEOUT_SECONDS:-1800}s"
run_setup_step() {
  local label="$1"
...
async function startSession(root: string, opts: Options, outputDir: string) {
  ...
  const hasConvexEnv =
    trimToValue(process.env.DAISYCLAW_QA_CONVEX_SITE_URL) &&
    trimToValue(process.env.DAISYCLAW_QA_CONVEX_SECRET_CI);
  ...
  try {
    ...
    return {
      ...
      commands: {
        send: `daisyclaw-telegram-user-crabbox-proof send --session ${path.relative(root, pathname)} --text '/status'`,
        view: `daisyclaw-telegram-user-crabbox-proof view --session ${path.relative(root, pathname)} --message-id <message-id>`,
        run: `daisyclaw-telegram-user-crabbox-proof run --session ${path.relative(root, pathname)} -- bash -lc 'source ${REMOTE_ROOT}/env.sh && python3 ${REMOTE_ROOT}/user-driver.py transcript --limit 20 --json'`,
        finish: `daisyclaw-telegram-user-crabbox-proof finish --session ${path.relative(root, pathname)} --preview-crop telegram-window`,
      },
    };
  } catch (error) {
...
async function main() {
  ...
  const localRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-telegram-crabbox-"));
  const summary: JsonObject = {
    artifacts: {},
