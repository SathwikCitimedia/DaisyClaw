...
IMAGE_NAME="$(docker_e2e_resolve_image "daisyclaw-update-channel-switch-e2e" DAISYCLAW_UPDATE_CHANNEL_SWITCH_E2E_IMAGE)"
SKIP_BUILD="${DAISYCLAW_UPDATE_CHANNEL_SWITCH_E2E_SKIP_BUILD:-0}"
cleanup() {
  docker_e2e_cleanup_package_tgz "${PACKAGE_TGZ:-}"
}
trap cleanup EXIT

PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz update-channel-switch "${DAISYCLAW_CURRENT_PACKAGE_TGZ:-}")"
# Bare lanes mount the package artifact instead of baking app sources into the image.
docker_e2e_package_mount_args "$PACKAGE_TGZ"
DAISYCLAW_TEST_STATE_SCRIPT_B64="$(
  node "$ROOT_DIR/scripts/lib/daisyclaw-test-state.mjs" shell \
...
(
  cd "$git_root"
  if ! daisyclaw_e2e_maybe_timeout "${DAISYCLAW_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install --omit=optional --no-fund --no-audit >/tmp/daisyclaw-git-install.log 2>&1; then
    daisyclaw_e2e_print_log /tmp/daisyclaw-git-install.log >&2
    exit 1
  fi
)
node scripts/e2e/lib/update-channel-switch/assertions.mjs write-control-ui "$git_root"

git config --global user.email "docker-e2e@daisyclaw.local"
git config --global user.name "DaisyClaw Docker E2E"
...
package_install_log="/tmp/daisyclaw-update-channel-switch-package-install.log"
if ! daisyclaw_e2e_maybe_timeout "${DAISYCLAW_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix /tmp/npm-prefix --omit=optional "$pkg_tgz_path" >"$package_install_log" 2>&1; then
  daisyclaw_e2e_print_log "$package_install_log" >&2
  exit 1
fi
package_version="$(node -p "JSON.parse(require(\"node:fs\").readFileSync(\"/tmp/npm-prefix/lib/node_modules/daisyclaw/package.json\", \"utf8\")).version")"
...
export DAISYCLAW_GIT_DIR="$git_root"
export DAISYCLAW_UPDATE_DEV_TARGET_REF="$fixture_sha"

echo "==> package -> git dev channel"
set +e
dev_json="$(daisyclaw update --channel dev --yes --json --no-restart)"
dev_status=$?
set -e
printf "%s\n" "$dev_json"
...
status_json="$(daisyclaw update status --json)"
printf "%s\n" "$status_json"
...
set +e
stable_json="$(daisyclaw update --channel stable --tag "$pkg_tgz_path" --yes --json --no-restart)"
stable_status=$?
set -e
printf "%s\n" "$stable_json"
