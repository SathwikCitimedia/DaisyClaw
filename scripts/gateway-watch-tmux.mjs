...
const TMUX_CWD_ENV_KEY = "DAISYCLAW_GATEWAY_WATCH_CWD";
const TMUX_CWD_OPTION_KEY = "@daisyclaw.gateway_watch.cwd";
const TMUX_CHILD_ENV_KEYS = [
  "NODE_OPTIONS",
  "DAISYCLAW_CONFIG_PATH",
  "DAISYCLAW_DIAGNOSTICS",
  "DAISYCLAW_DIAGNOSTICS_EVENT_LOOP",
  "DAISYCLAW_DIAGNOSTICS_TIMELINE_PATH",
  "DAISYCLAW_GATEWAY_PORT",
  "DAISYCLAW_GATEWAY_RESTART_TRACE",
  "DAISYCLAW_GATEWAY_STARTUP_TRACE",
  "DAISYCLAW_HOME",
  "DAISYCLAW_PROFILE",
...
const resolveGatewayWatchBenchmarkArgs = ({ args = [], env = process.env } = {}) => {
  ...
  if (benchmarkFlagSeen) {
    ...
    nextEnv.DAISYCLAW_TRACE_SYNC_IO ??= "0";
    if (nextEnv.DAISYCLAW_TRACE_SYNC_IO === "1") {
...
export const resolveGatewayWatchTmuxSessionName = ({ args = [], env = process.env } = {}) => {
  const profile =
    env.DAISYCLAW_PROFILE ||
    readArgValue(args, "--profile") ||
    (args.includes("--dev") ? "dev" : null);
  const port = env.DAISYCLAW_GATEWAY_PORT || readArgValue(args, "--port");
  const parts = [
    "daisyclaw",
    "gateway",
    "watch",
...
export const buildGatewayWatchTmuxCommand = ({
  ...
  const childEnv = [
    "env",
    ...colorEnv.options,
    `DAISYCLAW_GATEWAY_WATCH_TMUX_CHILD=1`,
    `DAISYCLAW_GATEWAY_WATCH_SESSION=${sessionName}`,
...
const log = (stderr, message) => {
  stderr.write(`[daisyclaw] ${message}\n`);
};
...
const shouldAttachTmux = ({ env, stdinIsTTY, stdoutIsTTY }) => {
  const raw = String(env.DAISYCLAW_GATEWAY_WATCH_ATTACH ?? "").toLowerCase();
...
export const runGatewayWatchTmuxMain = (params = {}) => {
  ...
  if (TMUX_DISABLE_VALUES.has((deps.env.DAISYCLAW_GATEWAY_WATCH_TMUX ?? "").toLowerCase())) {
  ...
  if (deps.env.DAISYCLAW_GATEWAY_WATCH_TMUX_CHILD === "1") {
    return runForegroundWatcher({
      args: deps.args,
      cwd: deps.cwd,
      env: deps.env,
