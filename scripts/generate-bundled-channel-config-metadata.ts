#!/usr/bin/env node
// Generate Bundled Channel Config Metadata script supports DaisyClaw repository automation.
...
function resolvePackageChannelMeta(source: BundledPluginSource) {
  const daisyclawMeta =
    ...
    "daisyclaw" in source.packageJson
      ? (source.packageJson.daisyclaw as Record<string, unknown> | undefined)
      : undefined;
  const channelMeta =
    daisyclawMeta &&
    typeof daisyclawMeta.channel === "object" &&
    daisyclawMeta.channel &&
    !Array.isArray(daisyclawMeta.channel)
      ? (daisyclawMeta.channel as Record<string, unknown>)
      : undefined;
  return channelMeta;
}
