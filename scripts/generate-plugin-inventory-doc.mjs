...
const MANUAL_SECTION_START = "<!-- daisyclaw-plugin-reference:manual-start -->";
const MANUAL_SECTION_END = "<!-- daisyclaw-plugin-reference:manual-end -->";
...
function resolveDescription({ manifest, packageJson }) {
  ...
  if (channels.length > 0) {
    ...
    return `Adds the ${channelLabel}${channelNoun} surface for sending and receiving DaisyClaw messages.`;
  }

  const providers = Array.isArray(manifest.providers) ? manifest.providers : [];
  if (providers.length > 0) {
    return `Adds ${displayList(providers)} model provider support to DaisyClaw.`;
  }
  ...
  return packageDescription ? `${packageDescription}.` : "Provides an DaisyClaw plugin.";
}
...
function resolveDocs({ dirName, manifest, packageJson }) {
  ...
  const channelDoc = normalizeDocPath(packageJson.daisyclaw?.channel?.docsPath);
  if (channelDoc) {
...
function resolveInstallRoute(packageJson, status) {
  ...
  if (status === "core") {
    return "included in DaisyClaw";
  }
  const install = packageJson.daisyclaw?.install;
  const release = packageJson.daisyclaw?.release;
  const clawhubSpec =
...
function resolveStatus({ dirName, packageJson, excludedDirs }) {
  ...
  const hasInstallSpec =
    typeof packageJson.daisyclaw?.install?.clawhubSpec === "string" ||
    typeof packageJson.daisyclaw?.install?.npmSpec === "string";
...
function renderReferenceIndex(records) {
  ...
  return `---
summary: "Generated index of DaisyClaw plugin reference pages"
read_when:
  - You need a reference page for a specific DaisyClaw plugin
...
\`daisyclaw.plugin.json\`. Regenerate it with:
...
function collectPluginSourceEntries() {
  ...
  for (const dirName of fs
    ...
    const manifestPath = path.join(EXTENSIONS_DIR, dirName, "daisyclaw.plugin.json");
