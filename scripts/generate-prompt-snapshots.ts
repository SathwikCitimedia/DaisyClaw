// Generate Prompt Snapshots script supports DaisyClaw repository automation.
...
export async function createFormattedPromptSnapshotFiles(): Promise<PromptSnapshotFile[]> {
  ...
  const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-prompt-snapshots-"));
  try {
