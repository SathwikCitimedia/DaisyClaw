// Gh Read script supports DaisyClaw repository automation.
...
const APP_ID_ENV = "DAISYCLAW_GH_READ_APP_ID";
const KEY_FILE_ENV = "DAISYCLAW_GH_READ_PRIVATE_KEY_FILE";
const INSTALLATION_ID_ENV = "DAISYCLAW_GH_READ_INSTALLATION_ID";
const PERMISSIONS_ENV = "DAISYCLAW_GH_READ_PERMISSIONS";
...
export function resolveGitHubFetchTimeoutMs(raw = process.env.DAISYCLAW_GH_READ_FETCH_TIMEOUT_MS) {
  return parseStrictIntegerOption({
    fallback: DEFAULT_GITHUB_FETCH_TIMEOUT_MS,
    label: "DAISYCLAW_GH_READ_FETCH_TIMEOUT_MS",
    min: 1,
    raw,
  });
}
...
async function main() {
  if (process.argv.length <= 2) {
    fail(
      "usage: scripts/gh-read <gh args...>\nset DAISYCLAW_GH_READ_APP_ID and DAISYCLAW_GH_READ_PRIVATE_KEY_FILE first",
    );
  }
