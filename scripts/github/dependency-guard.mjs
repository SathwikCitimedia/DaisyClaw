...
export const dependencyChangeMarker = "<!-- daisyclaw:dependency-guard -->";
export const dependencyGraphGuardMarker = "<!-- daisyclaw:dependency-graph-guard -->";
...
const securityTeamSlug = process.env.DAISYCLAW_SECURITY_TEAM_SLUG ?? "daisyclaw-secops";
...
export function renderAuthorizedDependencyComment(override) {
  const lines = [
    ...
    "",
    "This PR includes dependency graph changes. A repository admin or member of `@daisyclaw/daisyclaw-secops` authorized this exact head SHA with `/allow-dependencies-change`.",
    "",
...
export function renderTrustedDependencyComment({ actor, headSha }) {
  return [
    ...
    "",
    "This PR includes dependency graph changes. The dependency guard is informational because the PR author is a repository admin or a member of `@daisyclaw/daisyclaw-secops`.",
    "",
...
DaisyClaw does not accept package lockfile changes through PRs. This PR did not change dependency graph fields in package manifests, so the workflow restored the lockfile residue from the target branch automatically.
...
export function renderBlockedDependencyComment({
  ...
  return [
    ...
    "",
    "DaisyClaw does not accept dependency graph changes through PRs unless a repository admin or security explicitly authorizes the current head SHA. Dependency updates are generated internally by maintainers so external PRs cannot change the resolved graph.",
    "",
    ...
    ...removalSteps,
    "",
    "If this PR intentionally needs a dependency graph change, ask a repository admin or member of `@daisyclaw/daisyclaw-secops` to comment:",
    "",
    "```text",
...
export function githubApi(token, options = {}) {
  ...
  const baseHeaders = {
    ...
    "user-agent": "daisyclaw-dependency-guard",
...
async function main() {
  ...
  const autoscrubToken = process.env.DAISYCLAW_DEPENDENCY_GUARD_AUTOSCRUB_TOKEN;
  const autoscrubApi = autoscrubToken ? githubApi(autoscrubToken) : null;
  const explicitSecurityApprovers = securityApproverSet(process.env.DAISYCLAW_SECURITY_APPROVERS);
  const trustedCommentAuthors = dependencyGuardCommentAuthors(
    process.env.DAISYCLAW_DEPENDENCY_GUARD_COMMENT_BOTS,
  );
  ...
  const mode = process.env.DAISYCLAW_DEPENDENCY_GUARD_MODE ?? "enforce";
