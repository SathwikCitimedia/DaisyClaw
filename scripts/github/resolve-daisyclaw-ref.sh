...
REMOTE_URL="${DAISYCLAW_REF_REMOTE:-https://github.com/daisyclaw/daisyclaw.git}"
REF=""
EXPECTED_SHA=""
FALLBACK_OK=0
...
usage() {
  cat >&2 <<'EOF'
Usage: resolve-daisyclaw-ref.sh --ref <ref> [--expected-sha <sha>] [--fallback-ok] [--github-output <file>]

Fast-resolves DaisyClaw branch and tag refs with git ls-remote. Full commit SHAs
...
echo "Failed to resolve DaisyClaw ref: ${REF}" >&2
exit 1
