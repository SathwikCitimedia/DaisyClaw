...
# DaisyClaw CLI installer (non-interactive, no onboarding)
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://daisyclaw.ai/install-cli.sh | bash -s -- [--json] [--prefix <path>] [--version <ver>] [--node-version <ver>] [--onboard]
...
resolve_daisyclaw_effective_home() {
  ...
  case "$daisyclaw_home" in
    \~)
      echo "$HOME"
      ;;
    \~/*)
      echo "${HOME}/${daisyclaw_home#~/}"
      ;;
    *)
      echo "$daisyclaw_home"
      ;;
...
DAISYCLAW_EFFECTIVE_HOME="$(resolve_daisyclaw_effective_home)"
PREFIX="${DAISYCLAW_PREFIX:-${HOME}/.daisyclaw}"
DAISYCLAW_VERSION="${DAISYCLAW_VERSION:-latest}"
NODE_VERSION="${DAISYCLAW_NODE_VERSION:-22.22.0}"
NODE_VERSION_REQUESTED=0
if [[ -n "${DAISYCLAW_NODE_VERSION:-}" ]]; then
  NODE_VERSION_REQUESTED=1
fi
MIN_NODE_VERSION="22.19.0"
APK_NODE_BIN_DIR="/usr/bin"
NPM_LOGLEVEL="${DAISYCLAW_NPM_LOGLEVEL:-error}"
INSTALL_METHOD="${DAISYCLAW_INSTALL_METHOD:-npm}"
GIT_DIR="${DAISYCLAW_GIT_DIR:-${DAISYCLAW_EFFECTIVE_HOME}/daisyclaw}"
...
Usage: install-cli.sh [options]
  ...
  --prefix <path>                     Install prefix (default: ~/.daisyclaw; use \$DAISYCLAW_PREFIX to override)
  --install-method, --method npm|git  Install via npm (default) or from a git checkout
  --npm                               Shortcut for --install-method npm
  --git, --github                     Shortcut for --install-method git
  --git-dir, --dir <path>             Checkout directory (default: ~/daisyclaw, or \$DAISYCLAW_HOME/daisyclaw)
  --version <ver>                     DaisyClaw version (default: latest)
...
Environment variables:
  ...
  DAISYCLAW_HOME=...
  DAISYCLAW_PREFIX=...
  DAISYCLAW_VERSION=latest|next|<semver>
  DAISYCLAW_GIT_DIR=...
...
cleanup_legacy_submodules() {
  local repo_dir="${1:-${DAISYCLAW_GIT_DIR:-${DAISYCLAW_EFFECTIVE_HOME}/daisyclaw}}"
...
resolve_git_daisyclaw_ref() {
  ...
  case "$requested" in
    ""|latest)
      resolved_version="$("$(npm_bin)" view "daisyclaw" "dist-tags.${requested:-latest}" 2>/dev/null || true)"
...
install_daisyclaw() {
  local requested="${DAISYCLAW_VERSION:-latest}"
  if is_daisyclaw_source_package_install_spec "$requested"; then
    fail "npm installs do not support DaisyClaw GitHub source targets like '${requested}'. Use --install-method git --version main, latest, beta, an exact version, or a built .tgz package."
  fi
  ...
  emit_json "{\"event\":\"step\",\"name\":\"daisyclaw\",\"status\":\"start\",\"version\":\"${requested}\"}"
  log "Installing DaisyClaw (${requested})..."
  if [[ "$SET_NPM_PREFIX" -eq 1 ]]; then
    fix_npm_prefix_if_needed
  fi

  if [[ "${requested}" == "latest" ]]; then
    if ! env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "daisyclaw@latest"; then
      log "npm install daisyclaw@latest failed; retrying daisyclaw@next"
      emit_json "{\"event\":\"step\",\"name\":\"daisyclaw\",\"status\":\"retry\",\"version\":\"next\"}"
      env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "daisyclaw@next"
      requested="next"
    fi
  else
    env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "daisyclaw@${requested}"
  fi

  mkdir -p "${PREFIX}/bin"
  rm -f "${PREFIX}/bin/daisyclaw"
  cat > "${PREFIX}/bin/daisyclaw" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec "${PREFIX}/tools/node/bin/node" "$(node_dir)/lib/node_modules/daisyclaw/dist/entry.js" "\$@"
EOF
  chmod +x "${PREFIX}/bin/daisyclaw"
  emit_json "{\"event\":\"step\",\"name\":\"daisyclaw\",\"status\":\"ok\",\"version\":\"${requested}\"}"
}
...
install_daisyclaw_from_git() {
  ...
  emit_json "{\"event\":\"step\",\"name\":\"daisyclaw\",\"status\":\"start\",\"method\":\"git\",\"repo\":\"${repo_url//\"/\\\"}\"}"
  if [[ -d "$repo_dir/.git" ]]; then
    log "Installing Daisyclaw from git checkout: ${repo_dir}"
  else
    log "Installing Daisyclaw from GitHub (${repo_url})..."
  fi
...
EOF
  chmod +x "${PREFIX}/bin/daisyclaw"
  emit_json "{\"event\":\"step\",\"name\":\"daisyclaw\",\"status\":\"ok\",\"method\":\"git\"}"
}

resolve_daisyclaw_version() {
  local version=""
  if [[ -x "${PREFIX}/bin/daisyclaw" ]]; then
    version="$("${PREFIX}/bin/daisyclaw" --version 2>/dev/null | head -n 1 | tr -d '\r')"
  fi
  echo "$version"
}
...
refresh_gateway_service_if_loaded() {
  local claw="${PREFIX}/bin/daisyclaw"
  if [[ ! -x "$claw" ]]; then
    return 0
  fi
...
main() {
  ...
  if [[ "${DAISYCLAW_NO_ONBOARD:-0}" == "1" ]]; then
    RUN_ONBOARD=0
  fi
  ...
  if [[ "$INSTALL_METHOD" == "git" ]]; then
    install_daisyclaw_from_git "$GIT_DIR"
  ...
  if [[ -n "$installed_version" ]]; then
    ...
    log "DaisyClaw installed (${installed_version})."
  else
    emit_json "{\"event\":\"done\",\"ok\":true}"
    log "DaisyClaw installed."
  fi

  if [[ "$RUN_ONBOARD" -eq 1 ]]; then
    "${PREFIX}/bin/daisyclaw" onboard
  fi
}

if [[ "${DAISYCLAW_INSTALL_CLI_SH_NO_RUN:-0}" != "1" ]]; then
  main "$@"
fi
