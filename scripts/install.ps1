# DaisyClaw Installer for Windows
# Usage: powershell -c "irm https://daisyclaw.ai/install.ps1 | iex"
#        powershell -c "& ([scriptblock]::Create((irm https://daisyclaw.ai/install.ps1))) -Tag beta -NoOnboard -DryRun"
...
function Complete-Install {
    ...
    throw "DaisyClaw installation failed with exit code $($script:InstallExitCode)."
}

Write-Host ""
Write-Host "  DaisyClaw Installer" -ForegroundColor Cyan
Write-Host ""
...
if (-not $PSBoundParameters.ContainsKey("InstallMethod")) {
    if (-not [string]::IsNullOrWhiteSpace($env:DAISYCLAW_INSTALL_METHOD)) {
        $InstallMethod = $env:DAISYCLAW_INSTALL_METHOD
    }
}
if (-not $PSBoundParameters.ContainsKey("GitDir")) {
    if (-not [string]::IsNullOrWhiteSpace($env:DAISYCLAW_GIT_DIR)) {
        $GitDir = $env:DAISYCLAW_GIT_DIR
    }
}
if (-not $PSBoundParameters.ContainsKey("NoOnboard")) {
    if ($env:DAISYCLAW_NO_ONBOARD -eq "1") {
        $NoOnboard = $true
    }
}
if (-not $PSBoundParameters.ContainsKey("NoGitUpdate")) {
    if ($env:DAISYCLAW_GIT_UPDATE -eq "0") {
        $NoGitUpdate = $true
    }
}
if (-not $PSBoundParameters.ContainsKey("DryRun")) {
    if ($env:DAISYCLAW_DRY_RUN -eq "1") {
        $DryRun = $true
    }
}
...
if ([string]::IsNullOrWhiteSpace($GitDir)) {
    ...
    $GitDir = (Join-Path $userHome "daisyclaw")
}
...
function Get-DaisyClawDepsRoot {
    ...
    return (Join-Path $localAppData "DaisyClaw\deps")
}

function Get-PortableNodeRoot {
    return (Join-Path (Get-DaisyClawDepsRoot) "portable-node")
}
...
function Ensure-PortableNodeOnUserPath {
    ...
    if (Add-ToUserPath $nodeDir) {
        Write-Host "[!] Added $nodeDir to user PATH (restart terminal if node or daisyclaw is not found)" -ForegroundColor Yellow
    }
}
...
function Check-ExistingDaisyClaw {
    if (Get-DaisyClawCommandPath) {
        Write-Host "[*] Existing DaisyClaw installation detected" -ForegroundColor Yellow
        return $true
    }
    return $false
}
...
function Get-PortableGitRoot {
    return (Join-Path (Get-DaisyClawDepsRoot) "portable-git")
}
...
function Resolve-PortableGitDownload {
    ...
    $headers = @{
        "User-Agent" = "daisyclaw-installer"
...
function Install-PortableGit {
    ...
    $portableParent = Split-Path -Parent $portableRoot
    $tmpZip = Join-Path $env:TEMP $download.Name
    $tmpExtract = Join-Path $env:TEMP ("daisyclaw-portable-git-" + [guid]::NewGuid().ToString("N"))
...
function Ensure-Git {
    ...
    Write-Host ""
    Write-Host "Error: Git is required to install DaisyClaw." -ForegroundColor Red
...
function Get-DaisyClawCommandPath {
    $daisyclawCmd = Get-Command daisyclaw.cmd -ErrorAction SilentlyContinue
    if ($daisyclawCmd -and $daisyclawCmd.Source) {
        return $daisyclawCmd.Source
    }

    $daisyclaw = Get-Command daisyclaw -ErrorAction SilentlyContinue
    if ($daisyclaw -and $daisyclaw.Source) {
        return $daisyclaw.Source
    }

    return $null
}

function Invoke-DaisyClawCommand {
    param(
        [Parameter(ValueFromRemainingArguments = $true)]
        [string[]]$Arguments
    )

    $commandPath = Get-DaisyClawCommandPath
    if (-not $commandPath) {
        throw "daisyclaw command not found on PATH."
    }

    & $commandPath @Arguments
}

function Invoke-InteractiveDaisyClawCommand {
    param(
        [Parameter(ValueFromRemainingArguments = $true)]
        [string[]]$Arguments
    )
    ...
    if ($process.ExitCode -ne 0) {
        throw "daisyclaw $($Arguments -join ' ') failed with exit code $($process.ExitCode)."
    }
}
...
function Ensure-DaisyClawOnPath {
    if (Get-DaisyClawCommandPath) {
        return $true
    }
    ...
    foreach ($npmBin in $npmBins) {
        if (-not (Test-Path (Join-Path $npmBin "daisyclaw.cmd"))) {
            continue
        }
    ...
    Write-Host "[!] daisyclaw is not on PATH yet." -ForegroundColor Yellow
...
function Resolve-NpmDaisyClawInstallSpec {
    param(
        [string]$PackageName,
        [string]$RequestedTag
    )
...
function Test-DaisyClawSourcePackageInstallSpec {
    ...
    if ($normalizedTag.StartsWith("daisyclaw@")) {
        $normalizedTag = $normalizedTag.Substring("daisyclaw@".Length)
    }

    if ($normalizedTag -eq "main") {
        return $true
    }
    if ($normalizedTag -match '^github:daisyclaw/daisyclaw($|[#/])') {
        return $true
    }
    ...
    return (
        $normalizedTag -match '^https?://github\.com/daisyclaw/daisyclaw(\.git)?($|[?#])' -or
        $normalizedTag -match '^ssh://git@github\.com[:/]daisyclaw/daisyclaw(\.git)?($|[?#])' -or
        $normalizedTag -match '^git://github\.com/daisyclaw/daisyclaw(\.git)?($|[?#])' -or
        $normalizedTag -match '^git@github\.com:daisyclaw/daisyclaw(\.git)?($|[?#])'
    )
}
...
function Install-DaisyClaw {
    if ([string]::IsNullOrWhiteSpace($Tag)) {
        $Tag = "latest"
    }
    if (Test-DaisyClawSourcePackageInstallSpec -RequestedTag $Tag) {
        Write-Host "Error: npm installs do not support DaisyClaw GitHub source targets like '$Tag'." -ForegroundColor Red
    ...
    # Use daisyclaw package for beta, daisyclaw for stable
    $packageName = "daisyclaw"
    if ($Tag -eq "beta" -or $Tag -match "^beta\.") {
        $packageName = "daisyclaw"
    }
    $installSpec = Resolve-NpmDaisyClawInstallSpec -PackageName $packageName -RequestedTag $Tag
    Write-Host "[*] Installing DaisyClaw ($installSpec)..." -ForegroundColor Yellow
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            ...
            if ($npmOutput -match "spawn git" -or $npmOutput -match "ENOENT.*git") {
                ...
                Write-Host '  powershell -c "irm https://daisyclaw.ai/install.ps1 | iex"' -ForegroundColor Cyan
            }
    ...
    Write-Host "[OK] DaisyClaw installed" -ForegroundColor Green
    return $true
}

# Install DaisyClaw from GitHub
function Install-DaisyClawFromGit {
    param(
        [string]$RepoDir,
        [switch]$SkipUpdate
    )
    ...
    $repoUrl = "https://github.com/daisyclaw/daisyclaw.git"
    Write-Host "[*] Installing DaisyClaw from GitHub ($repoUrl)..." -ForegroundColor Yellow
    ...
    if (-not (Test-Path $entryPath)) {
        Write-Host "[!] DaisyClaw build did not produce $entryPath" -ForegroundColor Red
        return $false
    }
    ...
    $cmdPath = Join-Path $binDir "daisyclaw.cmd"
    ...
    Write-Host "[OK] DaisyClaw wrapper installed to $cmdPath" -ForegroundColor Green
...
function Run-Doctor {
    ...
    try {
        Invoke-DaisyClawCommand doctor --non-interactive
    } catch {
...
function Test-GatewayServiceLoaded {
    try {
        $statusJson = (Invoke-DaisyClawCommand daemon status --json 2>$null)
...
function Refresh-GatewayServiceIfLoaded {
    if (-not (Get-DaisyClawCommandPath)) {
        return
    }
    ...
    try {
        Invoke-DaisyClawCommand gateway install --force | Out-Null
    } catch {
    ...
    try {
        Invoke-DaisyClawCommand gateway restart | Out-Null
        Invoke-DaisyClawCommand gateway status --json | Out-Null
...
function Get-LegacyRepoDir {
    if (-not [string]::IsNullOrWhiteSpace($env:DAISYCLAW_GIT_DIR)) {
        return $env:DAISYCLAW_GIT_DIR
    }
    $userHome = [Environment]::GetFolderPath("UserProfile")
    return (Join-Path $userHome "daisyclaw")
}
...
function Main {
    ...
    $isUpgrade = Check-ExistingDaisyClaw
    ...
    if ($InstallMethod -eq "git") {
        try {
            ...
            if ($npmCommand) {
                Invoke-NpmCommand -Arguments @("uninstall", "-g", "daisyclaw") 2>$null | Out-Null
                Write-Host "[OK] Removed npm global install if present" -ForegroundColor Green
            }
        } catch { }
        $finalGitDir = $GitDir
        $gitInstallResults = @(Install-DaisyClawFromGit -RepoDir $GitDir -SkipUpdate:$NoGitUpdate)
        if (-not (Test-BooleanSuccessResult -Results $gitInstallResults)) {
            return (Fail-Install)
        }
    } else {
        $gitWrapper = Join-Path (Join-Path $env:USERPROFILE ".local\\bin") "daisyclaw.cmd"
        ...
        $npmInstallResults = @(Install-DaisyClaw)
        if (-not (Test-BooleanSuccessResult -Results $npmInstallResults)) {
            return (Fail-Install)
        }
    }

    if (-not (Ensure-DaisyClawOnPath)) {
        Write-Host "Install completed, but DaisyClaw is not on PATH yet." -ForegroundColor Yellow
        Write-Host "Open a new terminal, then run: daisyclaw doctor" -ForegroundColor Cyan
        return
    }
    ...
    try {
        $installedVersion = (Invoke-DaisyClawCommand --version 2>$null).Trim()
    } catch {
        $installedVersion = $null
    }
    if (-not $installedVersion) {
        try {
            ...
            if ($npmList -and $npmList.dependencies -and $npmList.dependencies.daisyclaw -and $npmList.dependencies.daisyclaw.version) {
                $installedVersion = $npmList.dependencies.daisyclaw.version
            }
        } catch {
            $installedVersion = $null
        }
    ...
    Write-Host ""
    if ($installedVersion) {
        Write-Host "DaisyClaw installed successfully ($installedVersion)!" -ForegroundColor Green
    } else {
        Write-Host "DaisyClaw installed successfully!" -ForegroundColor Green
    }
    Write-Host ""
    if ($isUpgrade) {
        $updateMessages = @(
    ...
    if ($InstallMethod -eq "git") {
        ...
        Write-Host "Wrapper: $env:USERPROFILE\\.local\\bin\\daisyclaw.cmd" -ForegroundColor Cyan
        Write-Host ""
    }

    if ($isUpgrade) {
        Write-Host "Upgrade complete. Run " -NoNewline
        Write-Host "daisyclaw doctor" -ForegroundColor Cyan -NoNewline
        Write-Host " to check for additional migrations."
    } else {
        if ($NoOnboard) {
            Write-Host "Skipping onboard (requested). Run " -NoNewline
            Write-Host "daisyclaw onboard" -ForegroundColor Cyan -NoNewline
            Write-Host " later."
        } else {
            Write-Host "Starting setup..." -ForegroundColor Cyan
            Write-Host ""
            Invoke-InteractiveDaisyClawCommand onboard
        }
    }
