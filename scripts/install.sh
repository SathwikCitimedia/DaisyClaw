...
# DaisyClaw Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://daisyclaw.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one DaisyClaw."
...
resolve_daisyclaw_effective_home() {
    local daisyclaw_home="${DAISYCLAW_HOME:-}"
    if [[ -z "$daisyclaw_home" ]]; then
        echo "$HOME"
        return
    fi
    if [[ "$daisyclaw_home" == "~" ]]; then
        echo "$HOME"
        return
    fi
    if [[ "$daisyclaw_home" == \~/* ]]; then
        echo "${HOME}${daisyclaw_home:1}"
        return
    fi
    echo "$daisyclaw_home"
}
...
GUM_VERSION="${DAISYCLAW_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 DaisyClaw Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://daisyclaw.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$DAISYCLAW_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.daisyclaw.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_daisyclaw_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.daisyclaw-* "$npm_root"/daisyclaw 2>/dev/null || true
}

extract_daisyclaw_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_daisyclaw_bin_conflict() {
    local bin_path="$1"
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/daisyclaw" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/daisyclaw"|"/usr/local/bin/daisyclaw")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/daisyclaw/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale daisyclaw symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing daisyclaw binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing DaisyClaw package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi

    ui_info "Installing DaisyClaw package"
    "${cmd[@]}" >"$log" 2>&1
}
...
install_daisyclaw_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "DaisyClaw npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*daisyclaw" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_daisyclaw_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_daisyclaw_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "DaisyClaw npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an daisyclaw binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${DAISYCLAW_TAGLINE_INDEX:-}" ]]; then
        if [[ "${DAISYCLAW_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((DAISYCLAW_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${DAISYCLAW_NO_ONBOARD:-0}
NO_PROMPT=${DAISYCLAW_NO_PROMPT:-0}
DRY_RUN=${DAISYCLAW_DRY_RUN:-0}
INSTALL_METHOD=${DAISYCLAW_INSTALL_METHOD:-}
DAISYCLAW_VERSION=${DAISYCLAW_VERSION:-latest}
USE_BETA=${DAISYCLAW_BETA:-0}
GIT_DIR_DEFAULT="$(resolve_daisyclaw_effective_home)/daisyclaw"
GIT_DIR=${DAISYCLAW_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${DAISYCLAW_GIT_UPDATE:-1}
NPM_LOGLEVEL="${DAISYCLAW_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${DAISYCLAW_VERBOSE:-0}"
VERIFY_INSTALL="${DAISYCLAW_VERIFY_INSTALL:-0}"
DAISYCLAW_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
DaisyClaw installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://daisyclaw.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/daisyclaw)
...
Environment variables:
  DAISYCLAW_INSTALL_METHOD=git|npm
  DAISYCLAW_VERSION=latest|next|<semver>|<spec>
  DAISYCLAW_BETA=0|1
  DAISYCLAW_GIT_DIR=...
  DAISYCLAW_GIT_UPDATE=0|1
  DAISYCLAW_NO_PROMPT=1
  DAISYCLAW_VERIFY_INSTALL=1
  DAISYCLAW_DRY_RUN=1
  DAISYCLAW_NO_ONBOARD=1
  DAISYCLAW_VERBOSE=1
  DAISYCLAW_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://daisyclaw.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://daisyclaw.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://daisyclaw.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://daisyclaw.ai/install.sh | bash -s -- --install-method git --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://daisyclaw.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected DaisyClaw checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a DaisyClaw source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_daisyclaw_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"daisyclaw"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://daisyclaw.ai/install.sh | bash"
}
...
fix_npm_permissions() {
    ...
    ui_warn "Avoid sudo npm i -g for future DaisyClaw updates; use npm i -g daisyclaw@latest so npm keeps using this user prefix instead of a different global prefix."
...
ensure_daisyclaw_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/daisyclaw" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/daisyclaw" ]]; then
        ln -sf "$npm_root/daisyclaw/dist/entry.js" "${npm_bin}/daisyclaw"
        ui_info "Created daisyclaw bin link at ${npm_bin}/daisyclaw"
    fi
    return 0
}

# Check for existing DaisyClaw installation
check_existing_daisyclaw() {
    if [[ -n "$(type -P daisyclaw 2>/dev/null || true)" ]]; then
        ui_info "Existing DaisyClaw installation detected, upgrading"
        return 0
    fi
    return 1
}
...
resolve_git_daisyclaw_ref() {
    local requested="${DAISYCLAW_VERSION:-latest}"
    local resolved_version=""

    case "$requested" in
        ""|latest)
            resolved_version="$(npm view "daisyclaw" "dist-tags.${requested:-latest}" 2>/dev/null || true)"
...
collect_daisyclaw_npm_root_candidates() {
    local root=""
    ...
    local old_ifs="$IFS"
    IFS=":"
    for extra_root in ${DAISYCLAW_INSTALL_EXTRA_NPM_ROOTS:-}; do
...
find_daisyclaw_global_installs() {
    local seen="|"
    local npm_root=""
    while IFS= read -r npm_root; do
        [[ -n "$npm_root" ]] || continue
        local package_dir="${npm_root%/}/daisyclaw"
        ...
        local version=""
        version="$(daisyclaw_package_version "$package_json")"
        printf '%s\t%s\t%s\n' "$version" "$real_package_dir" "$npm_root"
    done < <(collect_daisyclaw_npm_root_candidates)
}

warn_duplicate_daisyclaw_global_installs() {
    local installs=()
    local line=""
    while IFS= read -r line; do
        [[ -n "$line" ]] && installs+=("$line")
    done < <(find_daisyclaw_global_installs)

    if [[ "${#installs[@]}" -le 1 ]]; then
        return 0
    fi

    ui_warn "Multiple DaisyClaw global installs detected"
    echo "  Different Node/npm environments can run different DaisyClaw versions."

    local active_node active_npm active_daisyclaw
    active_node="$(command -v node 2>/dev/null || true)"
    active_npm="$(command -v npm 2>/dev/null || true)"
    active_daisyclaw="${DAISYCLAW_BIN:-}"
    if [[ -z "$active_daisyclaw" ]]; then
        active_daisyclaw="$(type -P daisyclaw 2>/dev/null || true)"
    fi
    echo -e "  Active node: ${INFO}${active_node:-none}${NC}"
    echo -e "  Active npm: ${INFO}${active_npm:-none}${NC}"
    echo -e "  Active daisyclaw: ${INFO}${active_daisyclaw:-none}${NC}"
    echo ""
    echo "  Found installs:"
    ...
    echo "    npm uninstall -g daisyclaw"
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make daisyclaw show as \"command not found\" in new terminals."
    echo "  Fix (zsh: ~/.zshrc, bash: ~/.bashrc):"
    echo "    export PATH=\"${dir}:\$PATH\""
}

daisyclaw_command_for_user() {
    local claw="${1:-}"
    if [[ -z "$claw" ]]; then
        echo "daisyclaw"
        return 0
...
bounded_probe_output() {
    local label="$1"
    shift
    local timeout_seconds="${DAISYCLAW_INSTALL_PROBE_TIMEOUT_SECONDS:-5}"
...
warn_daisyclaw_not_found() {
    ui_warn "Installed, but daisyclaw is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t daisyclaw 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named daisyclaw; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_daisyclaw_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P daisyclaw 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/daisyclaw" ]]; then
        echo "${npm_bin}/daisyclaw"
        return 0
    fi
...
install_daisyclaw_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/daisyclaw/daisyclaw.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing DaisyClaw from git checkout: ${repo_dir}"
    else
        ui_info "Installing DaisyClaw from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        ...
        run_quiet_step "Cloning DaisyClaw" git clone "$repo_url" "$repo_dir"
    fi

    local git_ref
    git_ref="$(resolve_git_daisyclaw_ref)"
    if [[ -z "$(git -C "$repo_dir" status --porcelain 2>/dev/null || true)" ]]; then
        ui_info "Using git ref: ${git_ref}"
        checkout_git_daisyclaw_ref "$repo_dir" "$git_ref"
    else
    ...
    run_quiet_step "Building DaisyClaw" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/daisyclaw" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/daisyclaw"
    ui_success "DaisyClaw wrapper installed to \$HOME/.local/bin/daisyclaw"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view daisyclaw dist-tags.beta 2>/dev/null || true)"
...
is_daisyclaw_source_package_install_spec() {
    local value="${1:-}"
    local normalized_value=""
    normalized_value="$(to_lowercase_ascii "$value")"
    normalized_value="${normalized_value#daisyclaw@}"

    [[ "$normalized_value" == "main" ]] && return 0
    [[ "$normalized_value" =~ ^github:daisyclaw/daisyclaw($|[#/]) ]] && return 0

    normalized_value="${normalized_value#git+}"
    [[ "$normalized_value" =~ ^https?://github\.com/daisyclaw/daisyclaw(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^ssh://git@github\.com[:/]daisyclaw/daisyclaw(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^git://github\.com/daisyclaw/daisyclaw(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^git@github\.com:daisyclaw/daisyclaw(\.git)?($|[?#]) ]] && return 0
    return 1
}
...
resolve_package_install_spec() {
    ...
    normalized_value="$(to_lowercase_ascii "$value")"
    if [[ "$normalized_value" == "main" ]]; then
        echo "github:daisyclaw/daisyclaw#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_daisyclaw() {
    local package_name="daisyclaw"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            DAISYCLAW_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="daisyclaw"
        else
            DAISYCLAW_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${DAISYCLAW_VERSION}" ]]; then
        DAISYCLAW_VERSION="latest"
    fi

    if is_daisyclaw_source_package_install_spec "${DAISYCLAW_VERSION}"; then
        ui_error "npm installs do not support DaisyClaw GitHub source targets like '${DAISYCLAW_VERSION}'."
    ...
    local resolved_version=""
    if can_resolve_registry_package_version "${DAISYCLAW_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${DAISYCLAW_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing DaisyClaw v${resolved_version}"
    else
        ui_info "Installing DaisyClaw (${DAISYCLAW_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${DAISYCLAW_VERSION}")"

    if ! install_daisyclaw_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_daisyclaw_paths
        install_daisyclaw_npm "${install_spec}"
    fi

    if [[ "${DAISYCLAW_VERSION}" == "latest" && "${package_name}" == "daisyclaw" ]]; then
        if ! resolve_daisyclaw_bin &> /dev/null; then
            ui_warn "npm install daisyclaw@latest failed; retrying daisyclaw@next"
            cleanup_npm_daisyclaw_paths
            install_daisyclaw_npm "daisyclaw@next"
        fi
    fi

    ensure_daisyclaw_bin_link || true

    ui_success "DaisyClaw installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${DAISYCLAW_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_daisyclaw_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (daisyclaw not on PATH yet)"
        warn_daisyclaw_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${DAISYCLAW_PROFILE:-default}"
    local effective_home
    effective_home="$(resolve_daisyclaw_effective_home)"
    if [[ "${profile}" != "default" ]]; then
        echo "${effective_home}/.daisyclaw/workspace-${profile}"
    else
        echo "${effective_home}/.daisyclaw/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local effective_home
    effective_home="$(resolve_daisyclaw_effective_home)"
    local config_path="${DAISYCLAW_CONFIG_PATH:-$effective_home/.daisyclaw/daisyclaw.json}"
    local legacy_config_path="${HOME}/.daisyclaw/daisyclaw.json"
    local legacy_clawdbot_path="${HOME}/.clawdbot/clawdbot.json"
    if [[ -f "${config_path}" || -f "$effective_home/.clawdbot/clawdbot.json" ]]; then
        return
    fi
    if [[ -z "${DAISYCLAW_CONFIG_PATH:-}" && "${effective_home}" != "${HOME}" ]]; then
    ...
    if ! is_promptable; then
        local user_claw
        user_claw="$(daisyclaw_command_for_user "${DAISYCLAW_BIN:-}")"
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but daisyclaw not on PATH; skipping onboarding"
        warn_daisyclaw_not_found
        return
    fi
    ...
    "$claw" onboard || {
        local user_claw
        user_claw="$(daisyclaw_command_for_user "$claw")"
...
if ! declare -F extract_daisyclaw_semver >/dev/null 2>&1; then
...
resolve_daisyclaw_version() {
    ...
    local claw="${DAISYCLAW_BIN:-}"
    if [[ -z "$claw" ]] && command -v daisyclaw &> /dev/null; then
        claw="$(command -v daisyclaw)"
    fi
    if [[ -n "$claw" ]]; then
        ...
        version="$(extract_daisyclaw_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/daisyclaw/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/daisyclaw/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
is_gateway_daemon_loaded() {
    ...
    local status_json=""
    status_json="$(bounded_probe_output "daisyclaw daemon status --json" "$claw" daemon status --json || true)"
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: daisyclaw not on PATH yet"
        warn_daisyclaw_not_found
        return 1
    fi

    run_quiet_step "Checking DaisyClaw version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: daisyclaw gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_daisyclaw_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found DaisyClaw checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set DAISYCLAW_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_daisyclaw; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing DaisyClaw"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g daisyclaw &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g daisyclaw 2>/dev/null || true
        ...
        install_daisyclaw_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/daisyclaw" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/daisyclaw"
    ...
    DAISYCLAW_BIN="$(resolve_daisyclaw_bin || true)"
    warn_duplicate_daisyclaw_global_installs || true
    ...
    installed_version=$(resolve_daisyclaw_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 DaisyClaw installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 DaisyClaw installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/daisyclaw"
        ui_kv "Update command" "daisyclaw update"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://daisyclaw.ai/install.sh | bash -s -- --install-method npm"
        ...
        if has_controlling_tty || [[ "$NO_ONBOARD" == "1" || "$NO_PROMPT" == "1" ]]; then
            ...
            ui_info "Running daisyclaw doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                DAISYCLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/null && doctor_ok=1
            else
                DAISYCLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                DAISYCLAW_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
        ...
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ...
            if is_promptable; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (daisyclaw not on PATH yet)"
                    warn_daisyclaw_not_found
                    return 0
                fi
                exec </dev/tty
    ...
    if command -v daisyclaw &> /dev/null; then
        local claw="${DAISYCLAW_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_daisyclaw_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (daisyclaw daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if DAISYCLAW_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: daisyclaw daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${DAISYCLAW_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_install_stage_total
    configure_verbose
    main
