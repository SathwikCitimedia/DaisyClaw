...
Optional custom relay:
  DAISYCLAW_PUSH_RELAY_BASE_URL=https://relay.example.com \
    scripts/ios-beta-prepare.sh --build-number 7 [--team-id TEAMID]
...
- regenerates apps/ios/DaisyClaw.xcodeproj via xcodegen
EOF
}
...
DEFAULT_IOS_PUSH_RELAY_BASE_URL="https://ios-push-relay.daisyclaw.ai"
PUSH_RELAY_BASE_URL="${DAISYCLAW_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-${DEFAULT_IOS_PUSH_RELAY_BASE_URL}}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid DAISYCLAW_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid DAISYCLAW_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid DAISYCLAW_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid DAISYCLAW_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(DAISYCLAW_URL_SLASH)$(DAISYCLAW_URL_SLASH)#g'
)"
...
DAISYCLAW_CODE_SIGN_STYLE = Automatic
DAISYCLAW_DEVELOPMENT_TEAM = ${TEAM_ID}
DAISYCLAW_IOS_SELECTED_TEAM = ${TEAM_ID}
DAISYCLAW_APP_BUNDLE_ID = ai.daisyclaw.client
DAISYCLAW_SHARE_BUNDLE_ID = ai.daisyclaw.client.share
DAISYCLAW_ACTIVITY_WIDGET_BUNDLE_ID = ai.daisyclaw.client.activitywidget
DAISYCLAW_WATCH_APP_BUNDLE_ID = ai.daisyclaw.client.watchkitapp
DAISYCLAW_WATCH_EXTENSION_BUNDLE_ID = ai.daisyclaw.client.watchkitapp.extension
DAISYCLAW_APP_PROFILE =
DAISYCLAW_SHARE_PROFILE =
DAISYCLAW_PUSH_TRANSPORT = relay
DAISYCLAW_PUSH_DISTRIBUTION = official
DAISYCLAW_URL_SLASH = /
DAISYCLAW_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
DAISYCLAW_PUSH_APNS_ENVIRONMENT = production
EOF
