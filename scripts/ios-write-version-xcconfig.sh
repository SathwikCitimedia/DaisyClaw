...
- DAISYCLAW_IOS_VERSION = exact canonical iOS version
- DAISYCLAW_MARKETING_VERSION = short iOS/App Store version
- DAISYCLAW_BUILD_VERSION = explicit build number or local numeric fallback
EOF
}
...
while IFS='=' read -r key value; do
  case "${key}" in
    DAISYCLAW_IOS_VERSION)
      IOS_VERSION="${value}"
      ;;
    DAISYCLAW_MARKETING_VERSION)
...
DAISYCLAW_IOS_VERSION = ${IOS_VERSION}
DAISYCLAW_MARKETING_VERSION = ${MARKETING_VERSION}
DAISYCLAW_BUILD_VERSION = ${BUILD_NUMBER}
EOF
