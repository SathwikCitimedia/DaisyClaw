// Bundled Extension Manifest script supports DaisyClaw repository automation.
...
export function collectBundledExtensionManifestErrors(extensions: BundledExtension[]): string[] {
  ...
  for (const extension of extensions) {
    const install = extension.packageJson.daisyclaw?.install;
    if (install !== undefined && !isRecord(install)) {
      errors.push(
        `bundled extension '${extension.id}' manifest invalid | daisyclaw.install must be an object`,
      );
      continue;
    }
    ...
    if (
      ...
      errors.push(
        `bundled extension '${extension.id}' manifest invalid | daisyclaw.install.npmSpec must be a non-empty string`,
      );
    }
