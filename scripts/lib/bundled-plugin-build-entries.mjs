...
const BUNDLED_PLUGIN_BUILD_IDS_ENV = "DAISYCLAW_BUNDLED_PLUGIN_BUILD_IDS";
...
function isManifestlessBundledRuntimeSupportPackage(params) {
  if (params.packageJson?.daisyclaw?.release?.publishToNpm === true) {
    return false;
  }
  const packageName = typeof params.packageJson?.name === "string" ? params.packageJson.name : "";
  if (packageName !== `@daisyclaw/${params.dirName}`) {
    return false;
  }
...
function shouldBuildBundledDistEntry(packageJson) {
  return packageJson?.daisyclaw?.build?.bundledDist !== false;
}
...
export function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.daisyclaw?.extensions)
    ? packageJson.daisyclaw.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.daisyclaw?.setupEntry === "string" &&
    packageJson.daisyclaw.setupEntry.trim().length > 0
...
export function collectBundledPluginBuildEntries(params = {}) {
  ...
  for (const candidate of collectBundledPluginCandidates(cwd, extensionsRoot)) {
    ...
    const hasManifest =
      relativeFiles?.includes("daisyclaw.plugin.json") ?? fs.existsSync(manifestPath);
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasManifest, hasPackageJson, sourceEntries } of entries) {
    if (hasManifest) {
      artifacts.add(bundledDistPluginFile(id, "daisyclaw.plugin.json"));
    }
    if (hasPackageJson) {
