...
docker_build_on_missing_enabled() {
  case "${DAISYCLAW_DOCKER_BUILD_ON_MISSING:-}" in
    1 | true | TRUE | yes | YES)
      return 0
      ;;
    0 | false | FALSE | no | NO)
  ...
  [ "${DAISYCLAW_TESTBOX:-0}" = "1" ]
}

docker_build_command() {
  local build_cmd=(docker build)
  if [ "${DAISYCLAW_DOCKER_BUILD_USE_BUILDX:-0}" = "1" ] || docker_build_args_need_buildx "$@"; then
    build_cmd=(docker buildx build --load)
    if [ -n "${DAISYCLAW_DOCKER_BUILD_CACHE_FROM:-}" ]; then
      build_cmd+=(--cache-from "${DAISYCLAW_DOCKER_BUILD_CACHE_FROM}")
    fi
    if [ -n "${DAISYCLAW_DOCKER_BUILD_CACHE_TO:-}" ]; then
      build_cmd+=(--cache-to "${DAISYCLAW_DOCKER_BUILD_CACHE_TO}")
    fi
  fi
...
docker_build_retry_count() {
  local configured="${DAISYCLAW_DOCKER_BUILD_RETRIES:-2}"
...
docker_build_timeout_required() {
  case "${DAISYCLAW_DOCKER_BUILD_REQUIRE_TIMEOUT:-0}" in
    1 | true | TRUE | yes | YES)
      return 0
      ;;
  esac
...
docker_build_heartbeat_seconds() {
  local configured="${DAISYCLAW_DOCKER_BUILD_HEARTBEAT_SECONDS:-30}"
...
docker_build_with_retries() {
  ...
  local timeout_value="${DAISYCLAW_DOCKER_BUILD_TIMEOUT:-3600s}"
  while true; do
...
docker_build_run() {
  ...
  DAISYCLAW_DOCKER_BUILD_REQUIRE_TIMEOUT="${DAISYCLAW_DOCKER_BUILD_REQUIRE_TIMEOUT:-1}" \
