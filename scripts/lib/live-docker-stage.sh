...
daisyclaw_live_stage_source_tree() {
  local dest_dir="${1:?destination directory required}"
  local stage_mode="${DAISYCLAW_LIVE_DOCKER_SOURCE_STAGE_MODE:-copy}"

  if [ "$stage_mode" = "symlink" ]; then
    echo "DAISYCLAW_LIVE_DOCKER_SOURCE_STAGE_MODE=symlink is disabled; using copy staging." >&2
  fi
...
daisyclaw_live_link_runtime_tree() {
  local dest_dir="${1:?destination directory required}"
...
daisyclaw_live_stage_node_modules() {
  local dest_dir="${1:?destination directory required}"
...
daisyclaw_live_scrub_staged_plugin_index() {
  local dest_dir="${1:?destination directory required}"
  local db_path="$dest_dir/state/daisyclaw.sqlite"
...
daisyclaw_live_stage_state_dir() {
  local dest_dir="${1:?destination directory required}"
  local source_dir="${HOME}/.daisyclaw"
  ...
  if [ -d "$source_dir" ]; then
    ...
    daisyclaw_live_scrub_staged_plugin_index "$dest_dir"
    if [ -d "$source_dir/workspace" ] && [ ! -e "$dest_dir/workspace" ]; then
      ln -s "$source_dir/workspace" "$dest_dir/workspace"
    fi
  fi

  export DAISYCLAW_STATE_DIR="$dest_dir"
  export DAISYCLAW_CONFIG_PATH="$dest_dir/daisyclaw.json"
}
