// Npm Verify Exec script supports DaisyClaw repository automation.
...
export function runNpmVerifyCommand(
  ...
  const timeoutMs =
    options.timeoutMs ??
    positiveEnvInt("DAISYCLAW_NPM_VERIFY_COMMAND_TIMEOUT_MS", DEFAULT_NPM_VERIFY_COMMAND_TIMEOUT_MS);
  const maxBuffer =
    options.maxBufferBytes ??
    positiveEnvInt(
      "DAISYCLAW_NPM_VERIFY_COMMAND_MAX_BUFFER_BYTES",
