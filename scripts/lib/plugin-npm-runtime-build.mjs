...
export function isPublishablePluginPackage(packageJson) {
  return packageJson.daisyclaw?.release?.publishToNpm === true;
}
...
function createNeverBundleDependencyMatcher(packageJson) {
  ...
  return (id) => {
    if (id === "daisyclaw" || id.startsWith("daisyclaw/")) {
      return true;
    }
...
export function resolvePluginNpmRuntimePackageFiles(plan) {
  ...
  merged.add("dist/**");
  if (packageRelativePathExists(plan.packageDir, "daisyclaw.plugin.json")) {
    merged.add("daisyclaw.plugin.json");
  }
...
function normalizeDaisyClawPeerRange(value) {
...
function resolveDaisyClawPeerRange(packageJson, rootPackageJson) {
  return (
    normalizeDaisyClawPeerRange(packageJson.daisyclaw?.compat?.pluginApi) ||
    normalizeDaisyClawPeerRange(packageJson.peerDependencies?.daisyclaw) ||
    normalizeDaisyClawPeerRange(packageJson.daisyclaw?.build?.daisyclawVersion) ||
    normalizeDaisyClawPeerRange(rootPackageJson?.version) ||
    normalizeDaisyClawPeerRange(packageJson.version)
  );
}

/** Resolve package peer dependency metadata for the DaisyClaw plugin API. */
export function resolvePluginNpmRuntimePackagePeerMetadata(plan) {
  const daisyclawPeerRange = resolveDaisyClawPeerRange(plan.packageJson, plan.rootPackageJson);
  if (!daisyclawPeerRange) {
    throw new Error(
      `cannot infer daisyclaw peerDependency range for ${plan.pluginDir}; set daisyclaw.compat.pluginApi or package version`,
    );
  }
  ...
  const existingDaisyClawMeta = getRecord(existingPeerDependenciesMeta.daisyclaw);
  return {
    peerDependencies: {
      ...existingPeerDependencies,
      daisyclaw: daisyclawPeerRange,
    },
    peerDependenciesMeta: {
      ...existingPeerDependenciesMeta,
      daisyclaw: {
        ...existingDaisyClawMeta,
        optional: true,
      },
    },
  };
...
export function resolvePluginNpmRuntimeBuildPlan(params) {
  ...
  const plan = {
    ...
    runtimeExtensions: (Array.isArray(packageJson.daisyclaw?.extensions)
      ? packageJson.daisyclaw.extensions
      : []
    )
      .map(normalizePackageEntry)
      .filter(Boolean)
      .map(toPackageRuntimeEntry),
    runtimeSetupEntry: normalizePackageEntry(packageJson.daisyclaw?.setupEntry)
      ? toPackageRuntimeEntry(packageJson.daisyclaw.setupEntry)
      : undefined,
  };
  return {
    ...plan,
