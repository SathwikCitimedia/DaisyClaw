// Release Beta Verifier script supports DaisyClaw repository automation.
...
export function parseReleaseVerifyBetaArgs(argv: string[]): ReleaseVerifyBetaArgs {
  ...
  if (!version || version.startsWith("-")) {
    throw new Error(
      "Usage: pnpm release:verify-beta -- <version> [--workflow-ref REF] [--full-release-validation-run ID] [--daisyclaw-npm-run ID] [--plugin-npm-run ID] [--plugin-clawhub-run ID] [--npm-telegram-run ID] [--skip-github-release] [--skip-clawhub]",
    );
  }
  ...
  for (let index = 0; index < values.length; index += 1) {
    ...
    switch (arg) {
      ...
      case "--full-release-validation-run":
        ...
        break;
      case "--daisyclaw-npm-run":
        parsed.workflowRuns.daisyclawNpm = next();
        break;
      case "--plugin-npm-run":
...
export async function verifyBetaRelease(
  ...
  const daisyclawNpm = verifyNpmPackage("daisyclaw", args.version, args.distTag);
  lines.push(`daisyclaw npm OK: ${args.version} (${args.distTag})`);
  ...
  if (!args.skipPostpublish) {
    ...
    lines.push("daisyclaw postpublish verifier OK");
  }
  ...
  if (args.workflowRuns.daisyclawNpm !== undefined) {
    workflowRuns.push(
      verifyWorkflowRun({
        id: args.workflowRuns.daisyclawNpm,
        label: "DaisyClaw NPM Release",
        repo: args.repo,
        expectedWorkflowName: "DaisyClaw NPM Release",
        expectedHeadBranch: args.workflowRef,
        rerunFailed: false,
      }),
    );
      ...
      `${JSON.stringify(
        {
          ...
          daisyclawNpmIntegrity: daisyclawNpm.integrity,
