// Rtt Harness script supports DaisyClaw repository automation.
...
const DAISYCLAW_PACKAGE_SPEC_RE =
  /^daisyclaw@(main|alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;
...
function resolveRttCredentialSource(
  ...
  const rawSource =
    env.DAISYCLAW_NPM_TELEGRAM_CREDENTIAL_SOURCE ?? env.DAISYCLAW_QA_CREDENTIAL_SOURCE;
  if (rawSource?.trim()) {
    return parseRttCredentialSource(rawSource);
  }
  if (
    env.CI &&
    env.DAISYCLAW_QA_CONVEX_SITE_URL?.trim() &&
    (env.DAISYCLAW_QA_CONVEX_SECRET_CI?.trim() || env.DAISYCLAW_QA_CONVEX_SECRET_MAINTAINER?.trim())
  ) {
    return "convex";
  }
  return "env";
...
function resolveRttCredentialRole(
  ...
  const rawRole = env.DAISYCLAW_NPM_TELEGRAM_CREDENTIAL_ROLE ?? env.DAISYCLAW_QA_CREDENTIAL_ROLE;
  if (rawRole?.trim()) {
...
export function validateDaisyClawPackageSpec(spec: string) {
  if (!DAISYCLAW_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `Package spec must be daisyclaw@main, daisyclaw@alpha, daisyclaw@beta, daisyclaw@latest, or an exact DaisyClaw release version; got: ${spec}`,
    );
  }
  return spec;
}
...
export function createHarnessEnv(params: {
  ...
  return {
    ...params.baseEnv,
    DAISYCLAW_NPM_TELEGRAM_PACKAGE_SPEC: params.spec,
    ...(params.packageTgz ? { DAISYCLAW_NPM_TELEGRAM_PACKAGE_TGZ: params.packageTgz } : {}),
    DAISYCLAW_NPM_TELEGRAM_PACKAGE_LABEL: `${params.spec} (${params.version})`,
    DAISYCLAW_NPM_TELEGRAM_PROVIDER_MODE: params.providerMode,
    ...(params.credentialSource
      ? { DAISYCLAW_NPM_TELEGRAM_CREDENTIAL_SOURCE: params.credentialSource }
      : {}),
    ...(params.credentialRole
      ? { DAISYCLAW_NPM_TELEGRAM_CREDENTIAL_ROLE: params.credentialRole }
      : {}),
    DAISYCLAW_NPM_TELEGRAM_SCENARIOS: params.scenarios.join(","),
    DAISYCLAW_NPM_TELEGRAM_OUTPUT_DIR: params.rawOutputDir,
    DAISYCLAW_NPM_TELEGRAM_FAST: params.baseEnv.DAISYCLAW_NPM_TELEGRAM_FAST ?? "1",
    DAISYCLAW_NPM_TELEGRAM_WARM_SAMPLES: String(params.samples),
    DAISYCLAW_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS: String(params.sampleTimeoutMs),
    DAISYCLAW_QA_TELEGRAM_CANARY_TIMEOUT_MS: String(params.timeoutMs),
    DAISYCLAW_QA_TELEGRAM_SCENARIO_TIMEOUT_MS: String(params.timeoutMs),
  };
}
...
export function assertRequiredEnv(
  ...
  if (credentialSource === "convex") {
    ...
    if (!env.DAISYCLAW_QA_CONVEX_SITE_URL?.trim()) {
      missing.push("DAISYCLAW_QA_CONVEX_SITE_URL");
    }
    if (credentialRole === "ci" && !env.DAISYCLAW_QA_CONVEX_SECRET_CI?.trim()) {
      missing.push("DAISYCLAW_QA_CONVEX_SECRET_CI");
    }
    if (credentialRole === "maintainer" && !env.DAISYCLAW_QA_CONVEX_SECRET_MAINTAINER?.trim()) {
      missing.push("DAISYCLAW_QA_CONVEX_SECRET_MAINTAINER");
    }
    if (missing.length > 0) {
...
export async function assertHarnessRoot(harnessRoot: string) {
  ...
  try {
    ...
    throw new Error(`Missing DaisyClaw Telegram npm harness: ${scriptPath}`);
  }
}
...
export async function resolveMainVersion(harnessRoot: string) {
  ...
  if (typeof packageJson.version !== "string" || packageJson.version.trim().length === 0) {
    throw new Error("DaisyClaw package.json must contain a non-empty version.");
  }
