// Scope script supports DaisyClaw repository automation.
...
export function createPluginSdkScope(_repoRoot: string): TopologyScope {
  const entrypoints = publicPluginSdkEntrypoints.map((entrypoint) => ({
    ...
    importSpecifier:
      entrypoint === "index" ? "daisyclaw/plugin-sdk" : `daisyclaw/plugin-sdk/${entrypoint}`,
  }));
  return buildScopeFromEntrypoints("plugin-sdk", "DaisyClaw plugin-sdk public surface", entrypoints);
}
