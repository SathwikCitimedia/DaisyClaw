// Verifies installed packages can bootstrap the default DaisyClaw workspace files.
...
export function runInstalledWorkspaceBootstrapSmoke(params) {
  const tempRoot = mkdtempSync(join(tmpdir(), "daisyclaw-workspace-bootstrap-smoke-"));
  ...
  try {
    try {
      execFileSync(
        process.execPath,
        [
          join(params.packageRoot, "daisyclaw.mjs"),
          "agent",
          "--message",
    ...
    const workspaceDir = join(homeDir, ".daisyclaw", "workspace");
