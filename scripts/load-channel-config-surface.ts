// Load Channel Config Surface script supports DaisyClaw repository automation.
...
const jitiFactoryOverrideKey = Symbol.for("daisyclaw.channelConfigSurfaceJitiFactoryOverride");
...
export async function loadChannelConfigSurfaceModule(
  ...
  const loadViaBun = (candidatePath: string) => {
    const script = `
      ...
      const modulePath = process.env.DAISYCLAW_CONFIG_SURFACE_MODULE;
      if (!modulePath) {
        throw new Error("missing DAISYCLAW_CONFIG_SURFACE_MODULE");
      }
    ...
    const result = spawnSync("bun", ["-e", script], {
      ...
      env: {
        ...process.env,
        DAISYCLAW_CONFIG_SURFACE_MODULE: path.resolve(candidatePath),
      },
    });
    if (result.error) {
  ...
  const loadViaJiti = (candidatePath: string) => {
    ...
    const aliasMap = {
      ...(pluginSdkAlias ? { "daisyclaw/plugin-sdk": pluginSdkAlias } : {}),
      ...resolvePluginSdkScopedAliasMap({
        modulePath: resolvedPath,
        pluginSdkResolution: "src",
      }),
