// Mcp Code Mode Gateway E2E script supports DaisyClaw repository automation.
...
export async function main() {
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-mcp-code-mode-"));
  const keep = process.env.DAISYCLAW_MCP_CODE_MODE_GATEWAY_E2E_KEEP === "1";
  const previousEnv = {
  ...
  try {
    ...
    await provider?.stop();
    resetConfigRuntimeState();
    restoreEnvValue("DAISYCLAW_STATE_DIR", previousEnv.stateDir);
    restoreEnvValue("DAISYCLAW_CONFIG_PATH", previousEnv.configPath);
    restoreEnvValue("DAISYCLAW_TEST_FAST", previousEnv.testFast);
    if (!keep) {
