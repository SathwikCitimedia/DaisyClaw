...
function resolveDaisyClawLaunchArgs(repoRoot, sourceEntryExists = existsSync) {
  const sourceEntry = path.join(repoRoot, "src", "entry.ts");
  if (sourceEntryExists(sourceEntry)) {
    return ["--import", "tsx", sourceEntry];
  }
  return [path.join(repoRoot, "daisyclaw.mjs")];
}
...
export async function startGateway({
  ...
  let child;
  const launcherArgs = resolveDaisyClawLaunchArgs(repoRoot, sourceEntryExists);
  try {
    child = spawnImpl(
      process.execPath,
      [
      ...
      ],
      {
        ...
        env: {
          ...
          DAISYCLAW_STATE_DIR: path.join(tempRoot, "state"),
...
async function main() {
  ...
  const repoRoot = path.resolve(args.repoRoot ?? process.env.DAISYCLAW_REPO_ROOT ?? process.cwd());
  ...
  try {
    ...
    const requireFromDaisyClaw = createRequire(path.join(repoRoot, "package.json"));
    const WebSocket = requireFromDaisyClaw("ws");
    ...
    const connect = await client.request(
      "connect",
      {
        ...
        client: {
          id: "gateway-client",
          displayName: "daisyclaw-rtt rpc probe",
          version: "rtt",
          platform: process.platform,
          mode: "backend",
          instanceId: `daisyclaw-rtt-rpc-${randomUUID()}`,
        },
        locale: "en-US",
        userAgent: "daisyclaw-rtt-rpc",
        role: "operator",
