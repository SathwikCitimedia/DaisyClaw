#!/usr/bin/env node
// Builds the DaisyClaw package artifact used by Docker E2E.
...
const PACKAGE_ARTIFACT_BUILD_STEPS = [
  {
    label: "Building DaisyClaw package artifacts",
    command: "node",
  ...
  for (const step of PACKAGE_ARTIFACT_BUILD_STEPS) {
    ...
    await runImpl(step.command, step.args, sourceDir, {
      ...
      timeoutMs: resolveTimeoutMs(
        "DAISYCLAW_DOCKER_PACKAGE_BUILD_TIMEOUT_MS",
...
async function newestDaisyClawTarball(outputDir, packOutput) {
  let fromOutput = "";
  ...
  const packed = entries
    ...
    .at(-1);
  if (!packed) {
    throw new Error(`missing packed DaisyClaw tarball in ${outputDir}`);
  }
  return path.join(outputDir, packed);
}

export async function packDaisyClawPackageForDocker(sourceDir, outputDir, options = {}) {
  ...
  console.error("==> Packing DaisyClaw package");
  ...
  try {
    packOutput = await runCaptureImpl(
      ...
      sourceDir,
      {
        ...
        timeoutMs: resolveTimeoutMs(
          "DAISYCLAW_DOCKER_PACKAGE_PACK_TIMEOUT_MS",
  ...
  return await newestDaisyClawTarball(outputDir, packOutput);
}
...
async function main() {
  ...
  console.error("==> Writing DaisyClaw package inventory");
  await run(
    "node",
    [
      "--import",
    ...
    {
      timeoutMs: resolveTimeoutMs(
        "DAISYCLAW_DOCKER_PACKAGE_INVENTORY_TIMEOUT_MS",
        DEFAULT_PACKAGE_INVENTORY_TIMEOUT_MS,
      ),
    },
  );

  let tarball = await packDaisyClawPackageForDocker(sourceDir, outputDir);
  ...
  console.error("==> Checking DaisyClaw package tarball");
  const checkStartedAt = Date.now();
  await run(
    "node",
    [path.join(ROOT_DIR, "scripts/check-daisyclaw-package-tarball.mjs"), tarball],
    sourceDir,
    {
      timeoutMs: resolveTimeoutMs(
        "DAISYCLAW_DOCKER_PACKAGE_TARBALL_CHECK_TIMEOUT_MS",
