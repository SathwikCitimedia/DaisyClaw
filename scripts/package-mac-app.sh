...
# Build and bundle DaisyClaw into a minimal .app we can open.
# Outputs to dist/DaisyClaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
source "$ROOT_DIR/scripts/lib/plistbuddy.sh"
APP_ROOT="$ROOT_DIR/dist/DaisyClaw.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="DaisyClaw"
MLX_TTS_HELPER_PRODUCT="daisyclaw-mlx-tts"
MLX_TTS_HELPER_ROOT="$ROOT_DIR/apps/macos-mlx-tts"
MLX_TTS_HELPER_BUILD_ROOT="$MLX_TTS_HELPER_ROOT/.build"
BUNDLE_ID="${BUNDLE_ID:-ai.daisyclaw.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/daisyclaw/daisyclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/DaisyClaw/Resources/Info.plist"
...
plist_set_string_required "$APP_ROOT/Contents/Info.plist" DaisyClawBuildTimestamp "$BUILD_TS"
plist_set_string_required "$APP_ROOT/Contents/Info.plist" DaisyClawGitCommit "$GIT_COMMIT"
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/DaisyClaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/DaisyClaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/DaisyClaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/DaisyClaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/DaisyClaw/Resources/DaisyClaw.icns" "$APP_ROOT/Contents/Resources/DaisyClaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/DaisyClaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying DaisyClawKit resources"
DAISYCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/DaisyClawKit_DaisyClawKit.bundle"
if [ -d "$DAISYCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/DaisyClawKit_DaisyClawKit.bundle"
  cp -R "$DAISYCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/DaisyClawKit_DaisyClawKit.bundle"
else
  echo "ERROR: DaisyClawKit resource bundle not found at $DAISYCLAWKIT_BUNDLE" >&2
  exit 1
fi
...
running_packaged_app_pids() {
  ...
  local app_binary="$APP_ROOT/Contents/MacOS/DaisyClaw"
  local pid
...
stop_packaged_app_if_running() {
  ...
  echo "⏹  Stopping packaged DaisyClaw bundle (${pids[*]})"
  ...
  echo "ERROR: Packaged DaisyClaw bundle did not exit: ${pids[*]}" >&2
  return 1
}
