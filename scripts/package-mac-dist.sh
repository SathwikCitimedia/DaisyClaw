...
# Output:
# - dist/DaisyClaw.app
# - dist/DaisyClaw-<version>.zip
# - dist/DaisyClaw-<version>.dmg
...
export BUNDLE_ID="${BUNDLE_ID:-ai.daisyclaw.mac}"
...
canonical_sparkle_build() {
  ...
  stderr_file="$(mktemp "${TMPDIR:-/tmp}/daisyclaw-sparkle-build.XXXXXX")" || {
...
APP="$ROOT_DIR/dist/DaisyClaw.app"
if [[ ! -d "$APP" ]]; then
...
ZIP="$ROOT_DIR/dist/DaisyClaw-$VERSION.zip"
DMG="$ROOT_DIR/dist/DaisyClaw-$VERSION.dmg"
NOTARY_ZIP="$ROOT_DIR/dist/DaisyClaw-$VERSION.notary.zip"
DSYM_ZIP="$ROOT_DIR/dist/DaisyClaw-$VERSION.dSYM.zip"
