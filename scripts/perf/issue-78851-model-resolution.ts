// Issue 78851 Model Resolution script supports DaisyClaw repository automation.
...
function printUsage(): void {
  process.stdout.write(`DaisyClaw issue #78851 model-resolution profiler
...
function buildConfig(options: Options, workspaceDir: string): DaisyClawConfig {
...
async function measurePhase(params: {
  ...
  const ensureResult = await ensureDaisyClawModelsJson(params.config, params.agentDir, {
...
async function main(): Promise<void> {
  ...
  const tempRoot = await mkdtemp(path.join(tmpdir(), "daisyclaw-issue-78851-"));
