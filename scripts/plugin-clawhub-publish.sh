...
source_repo="${SOURCE_REPO:-${GITHUB_REPOSITORY:-daisyclaw/daisyclaw}}"
...
pack_dir="$(mktemp -d "${RUNNER_TEMP:-/tmp}/daisyclaw-clawhub-pack.XXXXXX")"
cleanup() {
  rm -rf "${pack_dir}"
}
trap cleanup EXIT
...
build_package_runtime() {
  if [[ "${DAISYCLAW_PLUGIN_NPM_RUNTIME_BUILD:-1}" == "0" || "${DAISYCLAW_PLUGIN_NPM_RUNTIME_BUILD:-1}" == "false" ]]; then
...
if [[ "${mode}" == "--pack" ]]; then
  output_dir="${DAISYCLAW_CLAWHUB_PACK_OUTPUT_DIR:-}"
  if [[ -z "${output_dir}" ]]; then
    echo "DAISYCLAW_CLAWHUB_PACK_OUTPUT_DIR is required for --pack" >&2
    exit 2
  fi
  mkdir -p "${output_dir}"
...
for attempt in $(seq 1 "${DAISYCLAW_CLAWHUB_PUBLISH_ATTEMPTS:-8}"); do
  ...
  echo "ClawHub publish hit a rate limit; retrying (${attempt}/${DAISYCLAW_CLAWHUB_PUBLISH_ATTEMPTS:-8})." >&2
  sleep "${DAISYCLAW_CLAWHUB_PUBLISH_RETRY_DELAY_SECONDS:-60}"
done

echo "ClawHub publish failed after ${DAISYCLAW_CLAWHUB_PUBLISH_ATTEMPTS:-8} attempts." >&2
exit 1
