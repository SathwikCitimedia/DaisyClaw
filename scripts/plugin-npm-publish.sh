...
if [[ "${DAISYCLAW_NPM_PUBLISH_PROVENANCE:-1}" != "0" && "${DAISYCLAW_NPM_PUBLISH_PROVENANCE:-1}" != "false" ]]; then
...
if [[ "${DAISYCLAW_NPM_PUBLISH_AUTH_MODE:-}" == "trusted-publisher" ]]; then
  publish_auth_token=""
...
if [[ "${mode}" == "--pack-dry-run" ]]; then
  DAISYCLAW_PLUGIN_NPM_BUNDLE_DEPENDENCIES=1 \
