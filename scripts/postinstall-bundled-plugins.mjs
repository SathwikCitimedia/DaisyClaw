...
const DISABLE_POSTINSTALL_ENV = "DAISYCLAW_DISABLE_BUNDLED_PLUGIN_POSTINSTALL";
const DISABLE_PLUGIN_REGISTRY_MIGRATION_ENV = "DAISYCLAW_DISABLE_PLUGIN_REGISTRY_MIGRATION";
...
function resolvePostinstallDaisyClawHomeDir(env, getHomedir = homedir) {
  const osHome = resolvePostinstallOsHomeDir(env, getHomedir);
  const override = env?.DAISYCLAW_HOME?.trim();
  return override ? pathResolve(resolvePostinstallTildePath(override, osHome)) : osHome;
}

function resolvePostinstallUserPath(input, daisyClawHome) {
  return pathResolve(resolvePostinstallTildePath(input, daisyClawHome));
}
...
function isLegacyInstalledPluginDependencyDirName(name) {
  return name === "node_modules" || /^\.daisyclaw-install-stage(?:-[^/]+)?$/iu.test(name);
}
...
export function collectLegacyPluginRuntimeDepsStateRoots(params = {}) {
  ...
  const daisyClawHome = resolvePostinstallDaisyClawHomeDir(env, getHomedir);
  const stateRoots = [];
  ...
  if (stateOverride) {
    addStateRoot(resolvePostinstallUserPath(stateOverride, daisyClawHome));
  }
  const configPath = env?.DAISYCLAW_CONFIG_PATH?.trim();
  if (configPath) {
    addStateRoot(dirname(resolvePostinstallUserPath(configPath, daisyClawHome)));
  }
  addStateRoot(join(daisyClawHome, ".daisyclaw"));
  addStateRoot(join(daisyClawHome, ".clawdbot"));

  for (const entry of splitPostinstallPathList(env?.STATE_DIRECTORY)) {
    addStateRoot(resolvePostinstallUserPath(entry, daisyClawHome));
  }
  ...
  const createTempPath =
    ...
    ((unsafeTargetPath) =>
      join(
        ...
        `.${basename(unsafeTargetPath)}.daisyclaw-hotfix-${randomUUID()}`,
      ));
  const writeFile =
...
export function pruneDaisyClawCompileCache(params = {}) {
  const env = params.env ?? process.env;
  const pathExists = params.existsSync ?? existsSync;
  const readDir = params.readdirSync ?? readdirSync;
    ...
    try {
      for (const entry of readDir(baseDir, { withFileTypes: true })) {
        ...
        try {
          ...
          log.warn?.(`[postinstall] could not prune DaisyClaw compile cache: ${String(error)}`);
        }
      }
    } catch (error) {
      if (isCompileCachePrunePermissionDenied(error)) {
