...
prepare_gates() {
  ...
  if [ "$reuse_gates" = "true" ]; then
    ...
    if [ "$docs_only" = "true" ]; then
      ...
      gates_mode="full"
      if [ -n "${DAISYCLAW_VITEST_MAX_WORKERS:-}" ]; then
        echo "Running pnpm test with DAISYCLAW_VITEST_MAX_WORKERS=$DAISYCLAW_VITEST_MAX_WORKERS."
        run_quiet_logged \
          "pnpm test" \
          ".local/gates-test.log" \
          env DAISYCLAW_VITEST_MAX_WORKERS="$DAISYCLAW_VITEST_MAX_WORKERS" pnpm test
      else
