...
export function createPackageManagerWarningMessage(packageManager) {
  ...
  return [
    `[daisyclaw] warning: detected ${packageManager} for install lifecycle.`,
    "[daisyclaw] this repo works best with pnpm; npm-compatible installs are slower and much larger here.",
    "[daisyclaw] prefer: corepack pnpm install",
  ].join("\n");
}
