...
const ROOT_SHIMS_MAX_OLD_SPACE_SIZE =
  process.env.DAISYCLAW_ROOT_SHIMS_MAX_OLD_SPACE_SIZE?.trim() || "8192";
...
export function resolveBoundaryRootShimsTimeoutMs(env = process.env) {
  const raw = env.DAISYCLAW_PLUGIN_SDK_BOUNDARY_ROOT_SHIMS_TIMEOUT_MS?.trim();
  if (!raw) {
    return 300_000;
  }
  return parsePositiveInt(raw, "DAISYCLAW_PLUGIN_SDK_BOUNDARY_ROOT_SHIMS_TIMEOUT_MS");
}
    ...
    if (mode === "all") {
      if (!rootDtsFresh) {
        ...
        prerequisiteSteps.push({
          ...
          env: { DAISYCLAW_TSGO_HEAVY_CHECK_LOCK_HELD: "1" },
          timeoutMs: 300_000,
