...
// Qa Otel Smoke script supports DaisyClaw repository automation.
...
const DEFAULT_DOCKER_COLLECTOR_IMAGE =
  process.env.DAISYCLAW_QA_OTEL_COLLECTOR_IMAGE || "otel/opentelemetry-collector:0.104.0";
...
const REQUIRED_METRIC_NAMES = ["daisyclaw.harness.duration_ms"] as const;
...
const MAX_OTLP_COMPRESSED_BODY_BYTES = readPositiveIntegerEnv(
  "DAISYCLAW_QA_OTEL_MAX_COMPRESSED_BODY_BYTES",
  2 * 1024 * 1024,
);
const MAX_OTLP_DECODED_BODY_BYTES = readPositiveIntegerEnv(
  "DAISYCLAW_QA_OTEL_MAX_DECODED_BODY_BYTES",
  8 * 1024 * 1024,
);
const MAX_CAPTURED_BODY_TEXT_BYTES = readPositiveIntegerEnv(
  "DAISYCLAW_QA_OTEL_MAX_CAPTURED_BODY_TEXT_BYTES",
  512 * 1024,
);
const QA_SUITE_TIMEOUT_MS = readPositiveIntegerEnv(
  "DAISYCLAW_QA_OTEL_SUITE_TIMEOUT_MS",
  10 * 60 * 1000,
);
const QA_SUITE_KILL_GRACE_MS = readPositiveIntegerEnv("DAISYCLAW_QA_OTEL_SUITE_KILL_GRACE_MS", 5000);
...
async function startDockerOtelCollector(
  ...
  const tempDir = await makeTempDir(path.join(osTmpdir(), "daisyclaw-otel-collector-"));
  const configPath = path.join(tempDir, "collector.yaml");
  const containerName = `daisyclaw-otel-smoke-${makeUuid()}`;
...
function daisyClawEntryArgs(): string[] {
  if (existsSync(path.join(process.cwd(), "scripts", "run-node.mjs"))) {
    return ["scripts/run-node.mjs"];
  }
  return ["daisyclaw.mjs"];
}

function spawnDaisyClaw(args: string[], env: NodeJS.ProcessEnv): ChildProcess {
  return spawn(process.execPath, [...daisyClawEntryArgs(), ...args], {
...
async function waitForChild(
  ...
  throw new Error(`daisyclaw qa suite timed out after ${timeoutMs}ms`);
}
...
function buildQaEnv(port: number): NodeJS.ProcessEnv {
  ...
  env.OTEL_SERVICE_NAME = "daisyclaw-qa-lab-otel-smoke";
  env.OTEL_SEMCONV_STABILITY_OPT_IN = "gen_ai_latest_experimental";
  env.DAISYCLAW_QA_SUITE_PROGRESS = env.DAISYCLAW_QA_SUITE_PROGRESS ?? "1";
  return env;
}
...
function isLatestGenAiModelCallSpan(span: CapturedSpan): boolean {
  ...
  return (
    ...
    typeof span.attributes["daisyclaw.provider"] === "string" &&
    typeof span.attributes["daisyclaw.model"] === "string"
  );
}
...
function assertSmoke(params: {
  ...
  if (spanNames.has("daisyclaw.model.call")) {
    failures.push("legacy daisyclaw.model.call span exported with GenAI semconv opt-in");
  }
  ...
  const contentKeys = [...attributeKeys].filter((key) => key.startsWith("daisyclaw.content."));
  ...
  const modelErrorSpans = modelSpans.filter((span) => {
    ...
    return (
      ...
      Object.hasOwn(span.attributes, "daisyclaw.errorCategory") ||
...
async function main() {
  ...
  try {
    ...
    const child = spawnDaisyClaw(buildQaArgs(options), buildQaEnv(exportPort));
