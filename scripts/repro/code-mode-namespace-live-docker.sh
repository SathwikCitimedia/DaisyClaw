...
IMAGE_NAME="$(docker_e2e_resolve_image "daisyclaw-code-mode-namespace-live-e2e" DAISYCLAW_CODE_MODE_NAMESPACE_LIVE_E2E_IMAGE)"
SKIP_BUILD="${DAISYCLAW_CODE_MODE_NAMESPACE_LIVE_E2E_SKIP_BUILD:-0}"
PROFILE_FILE="${DAISYCLAW_CODE_MODE_NAMESPACE_LIVE_PROFILE_FILE:-${DAISYCLAW_TESTBOX_PROFILE_FILE:-$HOME/.daisyclaw-testbox-live.profile}}"
run_log=""
...
if ! docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e "DAISYCLAW_CODE_MODE_LIVE_MODEL=${DAISYCLAW_CODE_MODE_LIVE_MODEL:-gpt-5.4-mini}" \
  -e "DAISYCLAW_CODE_MODE_LIVE_TASKS=${DAISYCLAW_CODE_MODE_LIVE_TASKS:-3}" \
  -v "$ROOT_DIR":/src:ro \
  "${PROFILE_MOUNT[@]}" \
  "$IMAGE_NAME" \
