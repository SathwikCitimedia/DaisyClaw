...
// Code Mode Namespace Live script supports DaisyClaw repository automation.
...
async function runOne(mode: Mode, task: Task, model: string, apiKey: string): Promise<RunMetrics> {
  ...
  return {
    ...
    ...(process.env.DAISYCLAW_CODE_MODE_LIVE_DEBUG === "1" ? { toolResults } : {}),
  };
}
...
async function main() {
  ...
  const model = readArg("model") ?? process.env.DAISYCLAW_CODE_MODE_LIVE_MODEL ?? "gpt-5.4-mini";
  const modeArg = readArg("modes");
  const modes = (modeArg ? modeArg.split(",") : ["regular", "code-namespace"]) as Mode[];
  const taskLimit = Number(readArg("tasks") ?? process.env.DAISYCLAW_CODE_MODE_LIVE_TASKS ?? "3");
