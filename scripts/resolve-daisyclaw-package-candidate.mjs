...
const DEFAULT_OUTPUT_NAME = "daisyclaw-current.tgz";
...
const TRUSTED_PACKAGE_SOURCE_TOKEN_ENV = "DAISYCLAW_TRUSTED_PACKAGE_TOKEN";
...
export const DAISYCLAW_PACKAGE_SPEC_RE =
  /^daisyclaw@(alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;

function usage() {
  return `Usage: node scripts/resolve-daisyclaw-package-candidate.mjs --source <ref|npm|url|trusted-url|artifact> --output-dir <dir> [options]
...
export function validateDaisyClawPackageSpec(spec) {
  if (!DAISYCLAW_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `package_spec must be daisyclaw@alpha, daisyclaw@beta, daisyclaw@latest, or an exact DaisyClaw release version; got: ${spec}`,
    );
  }
}

export function resolveNpmPackageCandidatePackRunner(packageSpec, outputDir, params = {}) {
  validateDaisyClawPackageSpec(packageSpec);
...
async function resolveTrustedRepoRef(ref) {
  ...
  throw new Error(
    `package_ref ${ref} resolved to ${selectedSha}, which is not reachable from an DaisyClaw branch or release tag`,
  );
}
...
async function preparePackageSourceWorktree(ref) {
  ...
  const sourceDir = path.join(
    ...
    `daisyclaw-package-source-${process.pid}`,
  );
...
async function moveNewestPackedTarball(outputDir, packOutput, outputName) {
  ...
  if (!filename) {
    for (const line of packOutput.split(/\r?\n/u)) {
      ...
      if (/^daisyclaw-.*\.tgz$/u.test(trimmed)) {
        filename = trimmed;
      }
    }
  }
  if (!filename) {
    ...
    filename = entries
      .filter((entry) => /^daisyclaw-.*\.tgz$/u.test(entry))
      .toSorted((a, b) => a.localeCompare(b))
      .at(-1);
  }
  if (!filename) {
    throw new Error(`npm pack produced no DaisyClaw tarball in ${outputDir}`);
  }
...
async function resolveCandidate(options) {
  ...
  console.error(`Checking DaisyClaw package tarball: ${target}`);
  const checkStartedAt = Date.now();
  await run("node", ["scripts/check-daisyclaw-package-tarball.mjs", target], {
    timeoutMs: 5 * 60 * 1000,
  });
  console.error(
    `DaisyClaw package tarball check finished in ${Math.round((Date.now() - checkStartedAt) / 1000)}s`,
  );
  ...
  if (pkg.name !== "daisyclaw") {
    throw new Error(`package candidate must be named "daisyclaw"; got: ${pkg.name || "<missing>"}`);
  }
  if (!pkg.version) {
