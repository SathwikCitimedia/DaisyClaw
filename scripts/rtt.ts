...
// Rtt script supports DaisyClaw repository automation.
...
function usage() {
  return [
    "Usage: pnpm rtt <daisyclaw@spec> [--package-tgz PATH] [--provider mock-openai|live-frontier] [--credential-source env|convex] [--credential-role maintainer|ci] [--runs N] [--samples N] [--sample-timeout-ms N] [--timeout-ms N] [--harness-root PATH] [--output PATH]",
    "",
    "Examples:",
    "  pnpm rtt daisyclaw@main --package-tgz .artifacts/package/daisyclaw.tgz",
    "  pnpm rtt daisyclaw@beta",
    "  pnpm rtt daisyclaw@2026.4.30",
    "  pnpm rtt daisyclaw@latest --provider live-frontier",
  ].join("\n");
}
...
function parseArgs(argv: string[]) {
  ...
  return {
    spec: validateDaisyClawPackageSpec(spec),
    options: {
      packageTgz,
      credentialRole,
      credentialSource,
...
async function main() {
  ...
  if (spec === "daisyclaw@main" && !options.packageTgz) {
    throw new Error("daisyclaw@main requires --package-tgz.");
  }
  const version =
    spec === "daisyclaw@main"
