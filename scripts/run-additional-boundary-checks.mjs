...
function resolveCliShardSpec(args, env) {
  ...
  return env.DAISYCLAW_ADDITIONAL_BOUNDARY_SHARD ?? "";
}

if (import.meta.url === `file://${process.argv[1]}`) {
  const concurrencyRaw =
    process.env.DAISYCLAW_ADDITIONAL_BOUNDARY_CONCURRENCY ??
    process.env.DAISYCLAW_EXTENSION_BOUNDARY_CONCURRENCY;
  const concurrencyLabel =
    process.env.DAISYCLAW_ADDITIONAL_BOUNDARY_CONCURRENCY === undefined
      ? "DAISYCLAW_EXTENSION_BOUNDARY_CONCURRENCY"
      : "DAISYCLAW_ADDITIONAL_BOUNDARY_CONCURRENCY";
  const concurrency = resolveConcurrency(concurrencyRaw, 4, concurrencyLabel);
  const checkTimeoutMs = resolvePositiveInteger(
    process.env.DAISYCLAW_ADDITIONAL_BOUNDARY_TIMEOUT_MS,
    DEFAULT_CHECK_TIMEOUT_MS,
    "DAISYCLAW_ADDITIONAL_BOUNDARY_TIMEOUT_MS",
  );
  const outputMaxBytes = resolvePositiveInteger(
    process.env.DAISYCLAW_ADDITIONAL_BOUNDARY_OUTPUT_MAX_BYTES,
    DEFAULT_OUTPUT_MAX_BYTES,
    "DAISYCLAW_ADDITIONAL_BOUNDARY_OUTPUT_MAX_BYTES",
  );
