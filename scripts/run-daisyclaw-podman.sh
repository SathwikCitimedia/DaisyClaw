#!/usr/bin/env bash
# Rootless DaisyClaw in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-daisyclaw-podman.sh launch        # Start gateway
#   ./scripts/run-daisyclaw-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   daisyclaw --container daisyclaw dashboard --no-open
#   daisyclaw --container daisyclaw channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      DAISYCLAW_GATEWAY_TOKEN|DAISYCLAW_PODMAN_CONTAINER|DAISYCLAW_PODMAN_IMAGE|DAISYCLAW_IMAGE|DAISYCLAW_PODMAN_PULL|DAISYCLAW_PODMAN_RUN_TIMEOUT|DAISYCLAW_PODMAN_GATEWAY_HOST_PORT|DAISYCLAW_GATEWAY_PORT|DAISYCLAW_PODMAN_BRIDGE_HOST_PORT|DAISYCLAW_BRIDGE_PORT|DAISYCLAW_GATEWAY_BIND|DAISYCLAW_PODMAN_USERNS|DAISYCLAW_BIND_MOUNT_OPTIONS|DAISYCLAW_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-daisyclaw-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${DAISYCLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.daisyclaw}"
ENV_FILE="${DAISYCLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${DAISYCLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.daisyclaw}"
ENV_FILE="${DAISYCLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${DAISYCLAW_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${DAISYCLAW_PODMAN_CONTAINER:-daisyclaw}"
DAISYCLAW_IMAGE="${DAISYCLAW_PODMAN_IMAGE:-${DAISYCLAW_IMAGE:-daisyclaw:local}}"
PODMAN_PULL="${DAISYCLAW_PODMAN_PULL:-never}"
PODMAN_RUN_TIMEOUT="${DAISYCLAW_PODMAN_RUN_TIMEOUT:-600s}"
HOST_GATEWAY_PORT="${DAISYCLAW_PODMAN_GATEWAY_HOST_PORT:-${DAISYCLAW_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${DAISYCLAW_PODMAN_BRIDGE_HOST_PORT:-${DAISYCLAW_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${DAISYCLAW_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$DAISYCLAW_IMAGE"
...
resolve_config_gateway_bind() {
  local config_dir="$1"
  if ! command -v daisyclaw >/dev/null 2>&1; then
    return 0
  fi
  DAISYCLAW_CONTAINER="" DAISYCLAW_CONFIG_DIR="$config_dir" \
    daisyclaw config get gateway.bind 2>/dev/null || true
}
...
# DAISYCLAW_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${DAISYCLAW_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'DAISYCLAW_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v daisyclaw >/dev/null 2>&1; then
    echo "Warning: daisyclaw not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    DAISYCLAW_CONTAINER="" DAISYCLAW_CONFIG_DIR="$config_dir" \
      daisyclaw config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    DAISYCLAW_CONTAINER="" DAISYCLAW_CONFIG_DIR="$config_dir" \
      daisyclaw config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  DAISYCLAW_CONTAINER="" DAISYCLAW_CONFIG_DIR="$config_dir" \
    daisyclaw config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${DAISYCLAW_GATEWAY_TOKEN:-}" ]]; then
  export DAISYCLAW_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "DAISYCLAW_GATEWAY_TOKEN" "$DAISYCLAW_GATEWAY_TOKEN"
  echo "Generated DAISYCLAW_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/daisyclaw.json"
...
PODMAN_USERNS="${DAISYCLAW_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported DAISYCLAW_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (DAISYCLAW_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${DAISYCLAW_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${DAISYCLAW_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$DAISYCLAW_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.daisyclaw/.npm \
    -e DAISYCLAW_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.daisyclaw:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.daisyclaw/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$DAISYCLAW_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME daisyclaw dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME daisyclaw devices approve --latest  # if pairing required"
