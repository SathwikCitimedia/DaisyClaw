...
export function resolveVitestNodeArgs(env = process.env) {
  if (isTruthyEnvValue(env.DAISYCLAW_VITEST_ENABLE_MAGLEV)) {
    return [];
  }
...
export function resolveVitestCliEntry({
  ...
  try {
    ...
    if (isMissingVitestResolveError(error)) {
      ...
      wrappedError.code = "DAISYCLAW_MISSING_VITEST";
      throw wrappedError;
    }
    throw error;
  }
...
function shouldApplyNativeWorkerBudget(env) {
  ...
  return (
    env.DAISYCLAW_TEST_PROJECTS_SERIAL === "1" || resolveExplicitVitestWorkerBudget(env) !== null
  );
}
...
function resolveExplicitVitestWorkerBudget(env) {
  return parsePositiveInt(env.DAISYCLAW_VITEST_MAX_WORKERS ?? env.DAISYCLAW_TEST_WORKERS);
}
...
function main(argv = process.argv.slice(2), env = process.env) {
  ...
  try {
    ...
    if (error instanceof Error && error.code === "DAISYCLAW_MISSING_VITEST") {
