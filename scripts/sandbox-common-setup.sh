...
BASE_IMAGE="${BASE_IMAGE:-daisyclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-daisyclaw-sandbox-common:bookworm-slim}"
...
DAISYCLAW_DOCKER_BUILD_USE_BUILDX="${DAISYCLAW_DOCKER_BUILD_USE_BUILDX:-0}"
DAISYCLAW_DOCKER_BUILD_CACHE_FROM="${DAISYCLAW_DOCKER_BUILD_CACHE_FROM:-}"
DAISYCLAW_DOCKER_BUILD_CACHE_TO="${DAISYCLAW_DOCKER_BUILD_CACHE_TO:-}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=daisyclaw.sandbox=1)
NOTE
