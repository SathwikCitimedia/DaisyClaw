#!/bin/bash
# Setup DaisyClaw Auth Management System
...
echo "=== DaisyClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'daisyclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  DaisyClaw message: Send warning via DaisyClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/daisyclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/daisyclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/daisyclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now daisyclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status daisyclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
