// Sync Plugin Versions script supports DaisyClaw repository automation.
...
const DAISYCLAW_VERSION_RANGE_RE = /^>=\d{4}\.\d{1,2}\.\d{1,2}(?:[-.][^"\s]+)?$/u;

function syncDaisyClawDependencyRange(
  deps: Record<string, string> | undefined,
  targetVersion: string,
): boolean {
  const current = deps?.daisyclaw;
  if (!current || current === "workspace:*" || !DAISYCLAW_VERSION_RANGE_RE.test(current)) {
    return false;
  }
...
function syncPluginApiVersion(pkg: PackageJson, targetVersion: string): boolean {
  const compat = pkg.daisyclaw?.compat;
  const current = compat?.pluginApi;
  if (!current || !DAISYCLAW_VERSION_RANGE_RE.test(current)) {
    return false;
  }
  const next = `>=${targetVersion}`;
  if (current === next) {
...
function syncBuildDaisyClawVersion(pkg: PackageJson, targetVersion: string): boolean {
  const build = pkg.daisyclaw?.build;
  const current = build?.daisyclawVersion;
  if (!current) {
    return false;
  }
  ...
  build.daisyclawVersion = targetVersion;
  return true;
}
...
export function syncPluginVersions(
  ...
  for (const dir of dirs) {
    ...
    const devDependencyChanged = syncDaisyClawDependencyRange(pkg.devDependencies, targetVersion);
    const peerDependencyChanged = syncDaisyClawDependencyRange(pkg.peerDependencies, targetVersion);
    ...
    const buildDaisyClawVersionChanged = syncBuildDaisyClawVersion(pkg, targetVersion);
    const packageChanged =
      versionChanged ||
      devDependencyChanged ||
      peerDependencyChanged ||
      pluginApiChanged ||
      buildDaisyClawVersionChanged;
    if (!packageChanged) {
