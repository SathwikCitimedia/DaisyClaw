...
# DaisyClaw Auth Widget for Termux
...
# Check auth status
termux-toast "Checking DaisyClaw auth..."

STATUS=$(ssh "$SERVER" '$HOME/daisyclaw/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/daisyclaw/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|DAISYCLAW_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/daisyclaw/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|DAISYCLAW_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: DaisyClaw won't work until re-auth"
                ;;
        esac
        ;;
