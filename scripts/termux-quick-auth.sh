...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/daisyclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "DaisyClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/daisyclaw/scripts/mobile-reauth.sh'" --id daisyclaw-auth
        ;;
    *)
