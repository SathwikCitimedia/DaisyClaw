...
process.env.DAISYCLAW_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK ??= "1";

const { packageRoot } = parsePackageRootArg(
  process.argv.slice(2),
  "DAISYCLAW_BUNDLED_CHANNEL_SMOKE_ROOT",
);
...
const installedLayoutEnv = "DAISYCLAW_BUNDLED_CHANNEL_SMOKE_INSTALLED_LAYOUT";
...
function packageRootLooksInstalled(root) {
  return root.replaceAll("\\", "/").endsWith("/node_modules/daisyclaw");
}
...
function smokeInInstalledLayoutIfNeeded() {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-channel-entry-smoke-"));
  const nodeModulesRoot = path.join(tempRoot, "node_modules");
  const installedPackageRoot = path.join(nodeModulesRoot, "daisyclaw");
...
function collectBundledChannelEntryFiles() {
  ...
  for (const dirent of fs.readdirSync(distExtensionsRoot, { withFileTypes: true })) {
    ...
    const packageJson = readJson(packageJsonPath);
    if (!packageJson.daisyclaw?.channel) {
      continue;
    }
    ...
    const extensionEntries =
      Array.isArray(packageJson.daisyclaw.extensions) && packageJson.daisyclaw.extensions.length > 0
    ...
    const setupEntry = packageJson.daisyclaw.setupEntry;
