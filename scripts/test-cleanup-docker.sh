...
IMAGE_NAME="${DAISYCLAW_CLEANUP_SMOKE_IMAGE:-daisyclaw-cleanup-smoke:local}"
...
resolve_default_cleanup_platform() {
  local host_arch
  if [[ -n "${DAISYCLAW_CLEANUP_SMOKE_PLATFORM:-}" ]]; then
    printf "%s" "$DAISYCLAW_CLEANUP_SMOKE_PLATFORM"
    return
  fi
  host_arch="$(uname -m)"
