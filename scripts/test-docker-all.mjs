...
// Builds shared Docker images, prepares one DaisyClaw npm tarball, assigns lanes
...
const ROOT_DIR = path.resolve(process.env.DAISYCLAW_DOCKER_E2E_REPO_ROOT || SCRIPT_ROOT_DIR);
...
const DEFAULT_GITHUB_WORKFLOW = "daisyclaw-live-and-e2e-checks-reusable.yml";
...
export function dockerAllUsage() {
  return [
    ...
    "",
    "Lane selection and scheduler settings are configured with DAISYCLAW_DOCKER_ALL_* env vars.",
  ].join("\n");
}
...
function resourceLimitEnvName(resource) {
  return `DAISYCLAW_DOCKER_ALL_${resource.toUpperCase().replace(/[^A-Z0-9]+/g, "_")}_LIMIT`;
}
...
function parseSchedulerOptions(env, parallelism) {
  const weightLimit = parsePositiveInt(
    env.DAISYCLAW_DOCKER_ALL_WEIGHT_LIMIT,
    parallelism,
    "DAISYCLAW_DOCKER_ALL_WEIGHT_LIMIT",
  );
...
function appendExtension(env, extension) {
  const current = env.DAISYCLAW_DOCKER_BUILD_EXTENSIONS ?? env.DAISYCLAW_EXTENSIONS ?? "";
  const tokens = current.split(/\s+/).filter(Boolean);
  if (!tokens.includes(extension)) {
    tokens.push(extension);
  }
  env.DAISYCLAW_DOCKER_BUILD_EXTENSIONS = tokens.join(" ");
}
...
function githubWorkflowRef() {
  const explicit = process.env.DAISYCLAW_DOCKER_E2E_WORKFLOW_REF;
  if (explicit) {
    return explicit;
  }
...
function githubWorkflowRerunCommand(laneNames, ref) {
  ...
  const releasePath = process.env.DAISYCLAW_DOCKER_ALL_PROFILE === RELEASE_PATH_PROFILE;
  const fields = [
    "gh workflow run",
    shellQuote(process.env.DAISYCLAW_DOCKER_E2E_WORKFLOW || DEFAULT_GITHUB_WORKFLOW),
  ...
  if (process.env.GITHUB_RUN_ID) {
    ...
    fields.push(
      ...
      `package_artifact_name=${shellQuote(
        process.env.DAISYCLAW_DOCKER_E2E_PACKAGE_ARTIFACT_NAME || "docker-e2e-package",
      )}`,
    );
  }
  if (process.env.DAISYCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC) {
    fields.push(
      "-f",
      `published_upgrade_survivor_baseline=${shellQuote(process.env.DAISYCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC)}`,
    );
  }
  if (process.env.DAISYCLAW_UPGRADE_SURVIVOR_BASELINE_SPECS) {
    fields.push(
      "-f",
      `published_upgrade_survivor_baselines=${shellQuote(process.env.DAISYCLAW_UPGRADE_SURVIVOR_BASELINE_SPECS)}`,
    );
  }
  if (process.env.DAISYCLAW_UPGRADE_SURVIVOR_SCENARIOS) {
    fields.push(
      "-f",
      `published_upgrade_survivor_scenarios=${shellQuote(process.env.DAISYCLAW_UPGRADE_SURVIVOR_SCENARIOS)}`,
    );
  }
  if (process.env.DAISYCLAW_DOCKER_E2E_BARE_IMAGE) {
    fields.push(
      "-f",
      `docker_e2e_bare_image=${shellQuote(process.env.DAISYCLAW_DOCKER_E2E_BARE_IMAGE)}`,
    );
  }
  if (process.env.DAISYCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE) {
    fields.push(
      "-f",
      `docker_e2e_functional_image=${shellQuote(process.env.DAISYCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE)}`,
    );
  }
  return fields.join(" ");
}
...
function buildLaneRerunCommand(name, baseEnv) {
  ...
  const image = poolLane ? e2eImageForLane(poolLane, baseEnv) : baseEnv.DAISYCLAW_DOCKER_E2E_IMAGE;
  const env = [
    ["DAISYCLAW_DOCKER_ALL_LANES", name],
    ["DAISYCLAW_DOCKER_ALL_BUILD", build],
    ["DAISYCLAW_DOCKER_ALL_PREFLIGHT", "0"],
    ["DAISYCLAW_SKIP_DOCKER_BUILD", "1"],
    ["DAISYCLAW_DOCKER_E2E_IMAGE", image || DEFAULT_E2E_IMAGE],
    ["DAISYCLAW_DOCKER_E2E_BARE_IMAGE", baseEnv.DAISYCLAW_DOCKER_E2E_BARE_IMAGE],
    ["DAISYCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE", baseEnv.DAISYCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE],
    ["DAISYCLAW_CURRENT_PACKAGE_TGZ", baseEnv.DAISYCLAW_CURRENT_PACKAGE_TGZ],
    ["DAISYCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC", baseEnv.DAISYCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC],
    ["DAISYCLAW_UPGRADE_SURVIVOR_BASELINE_SPECS", baseEnv.DAISYCLAW_UPGRADE_SURVIVOR_BASELINE_SPECS],
    ["DAISYCLAW_UPGRADE_SURVIVOR_SCENARIOS", baseEnv.DAISYCLAW_UPGRADE_SURVIVOR_SCENARIOS],
  ];
  if (baseEnv.DAISYCLAW_DOCKER_ALL_PNPM_COMMAND) {
    env.push(["DAISYCLAW_DOCKER_ALL_PNPM_COMMAND", baseEnv.DAISYCLAW_DOCKER_ALL_PNPM_COMMAND]);
  }
  return `${env
...
function withResolvedPnpmCommand(command, env) {
  const pnpmCommand = env.DAISYCLAW_DOCKER_ALL_PNPM_COMMAND?.trim();
  if (!pnpmCommand) {
    return command;
  }
...
function liveDockerHarnessScriptCommand(script) {
  return `bash -c 'harness="\${DAISYCLAW_DOCKER_E2E_TRUSTED_HARNESS_DIR:-${LIVE_DOCKER_DEFAULT_HARNESS_DIR}}"; DAISYCLAW_LIVE_DOCKER_REPO_ROOT="\${DAISYCLAW_DOCKER_E2E_REPO_ROOT:-$PWD}" bash "$harness/scripts/${script}"'`;
}
...
async function writeRunSummary(logDir, summary) {
  ...
  const payload = {
    ...summary,
    packageArtifactName: process.env.DAISYCLAW_DOCKER_E2E_PACKAGE_ARTIFACT_NAME || undefined,
    ...
    github: {
      ...
      runUrl:
        process.env.GITHUB_SERVER_URL && process.env.GITHUB_REPOSITORY && process.env.GITHUB_RUN_ID
          ...
          : undefined,
      selectedSha: process.env.DAISYCLAW_DOCKER_E2E_SELECTED_SHA || undefined,
...
async function writeFailureIndex(logDir, summary) {
  const ref =
    ...
    process.env.DAISYCLAW_DOCKER_E2E_SELECTED_SHA ||
    summary.github?.sha ||
    summary.github?.ref ||
  ...
  const failureIndex = {
    ...
    version: 1,
    workflow: process.env.DAISYCLAW_DOCKER_E2E_WORKFLOW || DEFAULT_GITHUB_WORKFLOW,
  };
...
export function dockerPreflightContainerNames(raw) {
  return raw
    ...
    .filter((name) =>
      /^(?:daisyclaw-[a-z0-9-]+-e2e-\d+|daisyclaw-openwebui(?:-gateway)?-\d+)$/u.test(name),
    );
}
...
async function recordCleanupSmokeFailure(error, baseEnv, logDir, command, startedAtMs) {
  ...
  return {
    ...
    elapsedSeconds: phaseElapsedSeconds(startedAtMs),
    finishedAt: new Date().toISOString(),
    image: baseEnv.DAISYCLAW_DOCKER_E2E_IMAGE,
    logFile,
...
async function prepareDaisyClawPackage(baseEnv, logDir) {
  const existing = baseEnv.DAISYCLAW_CURRENT_PACKAGE_TGZ;
  if (existing) {
    const packageTgz = path.resolve(existing);
    baseEnv.DAISYCLAW_CURRENT_PACKAGE_TGZ = packageTgz;
    baseEnv.DAISYCLAW_BUNDLED_CHANNEL_HOST_BUILD = "0";
    baseEnv.DAISYCLAW_NPM_ONBOARD_HOST_BUILD = "0";
    console.log(`==> DaisyClaw package: ${packageTgz}`);
    return;
  }

  const packDir = path.join(logDir, "daisyclaw-package");
  await mkdir(packDir, { recursive: true });
  const packageTgz = path.join(packDir, "daisyclaw-current.tgz");
  await runForeground(
    "Prepare DaisyClaw package once",
    `node scripts/package-daisyclaw-for-docker.mjs --output-dir ${shellQuote(packDir)} --output-name daisyclaw-current.tgz`,
    baseEnv,
  );
  ...
  baseEnv.DAISYCLAW_BUNDLED_CHANNEL_HOST_BUILD = "0";
  baseEnv.DAISYCLAW_NPM_ONBOARD_HOST_BUILD = "0";
  console.log(`==> DaisyClaw package: ${baseEnv.DAISYCLAW_CURRENT_PACKAGE_TGZ}`);
}

function e2eImageForLane(poolLane, baseEnv) {
  if (poolLane.e2eImageKind === "bare") {
    return baseEnv.DAISYCLAW_DOCKER_E2E_BARE_IMAGE;
  }
  if (poolLane.e2eImageKind === "functional") {
    return baseEnv.DAISYCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE;
  }
  return undefined;
}
...
function laneEnv(poolLane, baseEnv, logDir, cacheKey) {
  ...
  env.DAISYCLAW_DOCKER_ALL_LANE_NAME = name;
  const image = e2eImageForLane(poolLane, baseEnv);
  if (image) {
    env.DAISYCLAW_DOCKER_E2E_IMAGE = image;
  }
  if (poolLane.e2eImageKind) {
    env.DAISYCLAW_DOCKER_E2E_IMAGE_KIND = poolLane.e2eImageKind;
  }
  const cacheName = cacheKey || name;
  if (!process.env.DAISYCLAW_DOCKER_CLI_TOOLS_DIR) {
    env.DAISYCLAW_DOCKER_CLI_TOOLS_DIR = path.join(logDir, `${cacheName}-cli-tools`);
  }
  if (!process.env.DAISYCLAW_DOCKER_CACHE_HOME_DIR) {
    env.DAISYCLAW_DOCKER_CACHE_HOME_DIR = path.join(logDir, `${cacheName}-cache`);
  }
  return env;
}
...
async function runLane(lane, baseEnv, logDir, fallbackTimeoutMs) {
  ...
  await mkdir(env.DAISYCLAW_DOCKER_CLI_TOOLS_DIR, { recursive: true });
  await mkdir(env.DAISYCLAW_DOCKER_CACHE_HOME_DIR, { recursive: true });
  await fs.promises.writeFile(
    logFile,
    [
      `==> [${name}] cli tools dir: ${env.DAISYCLAW_DOCKER_CLI_TOOLS_DIR}`,
      `==> [${name}] cache dir: ${env.DAISYCLAW_DOCKER_CACHE_HOME_DIR}`,
      ...
      `==> [${name}] e2e image: ${env.DAISYCLAW_DOCKER_E2E_IMAGE ?? ""}`,
      "",
    ].join("\n"),
  );
  ...
  return {
    ...
    attempts,
    finishedAt: new Date().toISOString(),
    image: env.DAISYCLAW_DOCKER_E2E_IMAGE,
      ...
      if (running.size === 0) {
        ...
        throw new Error(
          `No Docker lanes fit scheduler limits (${describeDockerSchedulerLimits(
            parallelism,
            options,
          )}): ${blocked}. Tune DAISYCLAW_DOCKER_ALL_PARALLELISM, DAISYCLAW_DOCKER_ALL_WEIGHT_LIMIT, or DAISYCLAW_DOCKER_ALL_<RESOURCE>_LIMIT.`,
        );
      }
...
async function main() {
  ...
  const parallelism = parsePositiveInt(
    process.env.DAISYCLAW_DOCKER_ALL_PARALLELISM,
    DEFAULT_PARALLELISM,
    "DAISYCLAW_DOCKER_ALL_PARALLELISM",
  );
  const tailParallelism = parsePositiveInt(
    process.env.DAISYCLAW_DOCKER_ALL_TAIL_PARALLELISM,
    Math.min(parallelism, DEFAULT_TAIL_PARALLELISM),
    "DAISYCLAW_DOCKER_ALL_TAIL_PARALLELISM",
  );
  const tailLines = parsePositiveInt(
    process.env.DAISYCLAW_DOCKER_ALL_FAILURE_TAIL_LINES,
    DEFAULT_FAILURE_TAIL_LINES,
    "DAISYCLAW_DOCKER_ALL_FAILURE_TAIL_LINES",
  );
  const laneTimeoutMs = parsePositiveInt(
    process.env.DAISYCLAW_DOCKER_ALL_LANE_TIMEOUT_MS,
    DEFAULT_LANE_TIMEOUT_MS,
    "DAISYCLAW_DOCKER_ALL_LANE_TIMEOUT_MS",
  );
  const laneStartStaggerMs = parseNonNegativeInt(
    process.env.DAISYCLAW_DOCKER_ALL_START_STAGGER_MS,
    DEFAULT_LANE_START_STAGGER_MS,
    "DAISYCLAW_DOCKER_ALL_START_STAGGER_MS",
  );
  const statusIntervalMs = parseNonNegativeInt(
    process.env.DAISYCLAW_DOCKER_ALL_STATUS_INTERVAL_MS,
    DEFAULT_STATUS_INTERVAL_MS,
    "DAISYCLAW_DOCKER_ALL_STATUS_INTERVAL_MS",
  );
  const preflightRunTimeoutMs = parsePositiveInt(
    process.env.DAISYCLAW_DOCKER_ALL_PREFLIGHT_RUN_TIMEOUT_MS,
    DEFAULT_PREFLIGHT_RUN_TIMEOUT_MS,
    "DAISYCLAW_DOCKER_ALL_PREFLIGHT_RUN_TIMEOUT_MS",
  );
  const failFast = parseBool(process.env.DAISYCLAW_DOCKER_ALL_FAIL_FAST, true);
  const dryRun = parseBool(process.env.DAISYCLAW_DOCKER_ALL_DRY_RUN, false);
  const preflightEnabled = parseBool(process.env.DAISYCLAW_DOCKER_ALL_PREFLIGHT, true);
  const preflightCleanup = parseBool(process.env.DAISYCLAW_DOCKER_ALL_PREFLIGHT_CLEANUP, true);
  const timingsEnabled = parseBool(process.env.DAISYCLAW_DOCKER_ALL_TIMINGS, true);
  const buildEnabled = parseBool(process.env.DAISYCLAW_DOCKER_ALL_BUILD, true);
  const planJson =
    cliOptions.planJson || parseBool(process.env.DAISYCLAW_DOCKER_ALL_PLAN_JSON, false);
  const planReleaseAll = parseBool(process.env.DAISYCLAW_DOCKER_ALL_PLAN_RELEASE_ALL, false);
  const profile = parseProfile(process.env.DAISYCLAW_DOCKER_ALL_PROFILE);
  const releaseProfile = normalizeReleaseProfile(
    process.env.DAISYCLAW_DOCKER_ALL_RELEASE_PROFILE || process.env.DAISYCLAW_RELEASE_PROFILE,
  );
  const releaseChunk = process.env.DAISYCLAW_DOCKER_ALL_CHUNK || process.env.DOCKER_E2E_CHUNK || "";
  const includeOpenWebUI = parseBool(
    process.env.DAISYCLAW_DOCKER_ALL_INCLUDE_OPENWEBUI ?? process.env.INCLUDE_OPENWEBUI,
    true,
  );
  const selectedLaneNamesRaw =
    process.env.DAISYCLAW_DOCKER_ALL_LANES || process.env.DOCKER_E2E_LANES || "";
  const selectedLaneNames = parseLaneSelection(selectedLaneNamesRaw);
  if (selectedLaneNamesRaw && selectedLaneNames.length === 0) {
    throw new Error("DAISYCLAW_DOCKER_ALL_LANES must include at least one lane name");
  }
  const liveMode = parseLiveMode(process.env.DAISYCLAW_DOCKER_ALL_LIVE_MODE);
  const liveRetries = parseNonNegativeInt(
    process.env.DAISYCLAW_DOCKER_ALL_LIVE_RETRIES,
    DEFAULT_LIVE_RETRIES,
    "DAISYCLAW_DOCKER_ALL_LIVE_RETRIES",
  );
  const timingsFile = path.resolve(
    process.env.DAISYCLAW_DOCKER_ALL_TIMINGS_FILE || DEFAULT_TIMINGS_FILE,
  );
  const runId = process.env.DAISYCLAW_DOCKER_ALL_RUN_ID || utcStampForPath();
  const logDir = path.resolve(
    process.env.DAISYCLAW_DOCKER_ALL_LOG_DIR ||
      path.join(ROOT_DIR, ".artifacts/docker-tests", runId),
  );

  const baseEnv = commandEnv({
    DAISYCLAW_DOCKER_E2E_BARE_IMAGE:
      process.env.DAISYCLAW_DOCKER_E2E_BARE_IMAGE ||
      process.env.DAISYCLAW_DOCKER_E2E_IMAGE ||
      DEFAULT_E2E_BARE_IMAGE,
    DAISYCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE:
      process.env.DAISYCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE ||
      process.env.DAISYCLAW_DOCKER_E2E_IMAGE ||
      DEFAULT_E2E_FUNCTIONAL_IMAGE,
  });
  baseEnv.DAISYCLAW_DOCKER_E2E_IMAGE =
    process.env.DAISYCLAW_DOCKER_E2E_IMAGE || baseEnv.DAISYCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE;
  ...
  const { orderedLanes, orderedTailLanes, plan, scheduledLanes } = resolveDockerE2ePlan({
    ...
    selectedLaneNames,
    timingStore,
    upgradeSurvivorBaselines: process.env.DAISYCLAW_UPGRADE_SURVIVOR_BASELINE_SPECS,
    upgradeSurvivorScenarios: process.env.DAISYCLAW_UPGRADE_SURVIVOR_SCENARIOS,
  });
  ...
  console.log(`==> Docker E2E bare image: ${baseEnv.DAISYCLAW_DOCKER_E2E_BARE_IMAGE}`);
  console.log(`==> Docker E2E functional image: ${baseEnv.DAISYCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE}`);
  if (profile === RELEASE_PATH_PROFILE) {
  ...
  console.log(`==> Live-test bundled plugins: ${baseEnv.DAISYCLAW_DOCKER_BUILD_EXTENSIONS}`);
  ...
  if (lanesNeedDaisyClawPackage(scheduledLanes)) {
    await runPhase(phases, "prepare-daisyclaw-package", {}, async () => {
      await prepareDaisyClawPackage(baseEnv, logDir);
    });
  } else {
    console.log("==> DaisyClaw package: not needed for selected lanes");
  }
  ...
  if (buildEnabled) {
    ...
    if (lanesNeedE2eImageKind(scheduledLanes, "bare")) {
      buildEntries.push({
        ...
        env: {
          DAISYCLAW_DOCKER_E2E_IMAGE: baseEnv.DAISYCLAW_DOCKER_E2E_BARE_IMAGE,
          DAISYCLAW_DOCKER_E2E_TARGET: "bare",
        },
        label: `shared bare Docker E2E image once: ${baseEnv.DAISYCLAW_DOCKER_E2E_BARE_IMAGE}`,
        phaseDetails: { image: baseEnv.DAISYCLAW_DOCKER_E2E_BARE_IMAGE, imageKind: "bare" },
        phases,
      });
    }
    if (lanesNeedE2eImageKind(scheduledLanes, "functional")) {
      buildEntries.push({
        command: "pnpm test:docker:e2e-build",
        env: {
          DAISYCLAW_DOCKER_E2E_IMAGE: baseEnv.DAISYCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE,
          DAISYCLAW_DOCKER_E2E_TARGET: "functional",
        },
        label: `shared functional Docker E2E image once: ${baseEnv.DAISYCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE}`,
        phaseDetails: {
          image: baseEnv.DAISYCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE,
  ...
  if (failFast && failures.length > 0) {
    await writeRunSummary(logDir, {
      ...
      image: baseEnv.DAISYCLAW_DOCKER_E2E_IMAGE,
      images: {
        bare: baseEnv.DAISYCLAW_DOCKER_E2E_BARE_IMAGE,
        functional: baseEnv.DAISYCLAW_DOCKER_E2E_FUNCTIONAL_IMAGE,
      },
      lanes: allResults,
      phases,
      profile,
