#!/usr/bin/env node
// Test Env Mutation Report script supports DaisyClaw repository automation.
...
const DEFAULT_ALLOWED_FILES = new Map([
  ["src/test-utils/daisyclaw-test-state.ts", "canonical DaisyClaw test state helper"],
  ...
  ["test/setup-daisyclaw-runtime.ts", "global runtime-test setup owns process env isolation"],
  [
...
export function renderTestEnvMutationReport(
  ...
  const lines = [
    "DaisyClaw test env mutation report",
