...
// Test Force script supports DaisyClaw repository automation.
...
function usage(): string {
  return [
    ...
    "",
    "Clears the configured DaisyClaw gateway port, then runs the local test suite.",
    "",
    "Options:",
...
function runTests() {
  const isolatedLock =
    process.env.DAISYCLAW_GATEWAY_LOCK ??
    path.join(os.tmpdir(), `daisyclaw-gateway.lock.test.${Date.now()}`);
  const result = spawnSync(process.execPath, ["scripts/test-projects.mjs"], {
    stdio: "inherit",
    env: {
      ...process.env,
      DAISYCLAW_GATEWAY_LOCK: isolatedLock,
    },
  });
  if (result.error) {
