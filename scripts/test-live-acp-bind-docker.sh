...
ACP_AGENT_LIST_RAW="${DAISYCLAW_LIVE_ACP_BIND_AGENTS:-${DAISYCLAW_LIVE_ACP_BIND_AGENT:-claude,codex,gemini}}"
TEMP_DIRS=()
...
daisyclaw_live_acp_bind_append_build_extension() {
...
daisyclaw_live_acp_bind_resolve_auth_provider() {
  case "${1:-}" in
    claude) printf '%s\n' "claude-cli" ;;
    codex) printf '%s\n' "codex-cli" ;;
    droid) printf '%s\n' "droid" ;;
    ...
    opencode) printf '%s\n' "opencode" ;;
    *)
      echo "Unsupported DAISYCLAW_LIVE_ACP_BIND agent: ${1:-} (expected claude, codex, droid, gemini, or opencode)" >&2
      return 1
      ;;
  esac
}

daisyclaw_live_acp_bind_resolve_agent_command() {
  case "${1:-}" in
    claude) printf '%s' "${DAISYCLAW_LIVE_ACP_BIND_AGENT_COMMAND_CLAUDE:-${DAISYCLAW_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    codex) printf '%s' "${DAISYCLAW_LIVE_ACP_BIND_AGENT_COMMAND_CODEX:-${DAISYCLAW_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    droid) printf '%s' "${DAISYCLAW_LIVE_ACP_BIND_AGENT_COMMAND_DROID:-${DAISYCLAW_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    gemini) printf '%s' "${DAISYCLAW_LIVE_ACP_BIND_AGENT_COMMAND_GEMINI:-${DAISYCLAW_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    opencode) printf '%s' "${DAISYCLAW_LIVE_ACP_BIND_AGENT_COMMAND_OPENCODE:-${DAISYCLAW_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    *) return 1 ;;
  esac
}
...
daisyclaw_live_acp_bind_load_factory_api_key_from_profile() {
...
run_setup_command() {
  local timeout_value="${DAISYCLAW_LIVE_ACP_BIND_SETUP_TIMEOUT_SECONDS:-180}s"
  local timeout_bin=""
...
agent="${DAISYCLAW_LIVE_ACP_BIND_AGENT:-claude}"
case "$agent" in
  claude)
...
if [ -n "\${DAISYCLAW_LIVE_ACP_BIND_ANTHROPIC_API_KEY:-}" ]; then
  export ANTHROPIC_API_KEY="\${DAISYCLAW_LIVE_ACP_BIND_ANTHROPIC_API_KEY}"
fi
if [ -n "\${DAISYCLAW_LIVE_ACP_BIND_ANTHROPIC_API_KEY_OLD:-}" ]; then
  export ANTHROPIC_API_KEY_OLD="\${DAISYCLAW_LIVE_ACP_BIND_ANTHROPIC_API_KEY_OLD}"
fi
...
NODE
  ...
  opencode)
    ...
    export OPENCODE_CONFIG_CONTENT="$(
      node -e 'process.stdout.write(JSON.stringify({model: process.env.DAISYCLAW_LIVE_ACP_BIND_OPENCODE_MODEL || "opencode/kimi-k2.6"}))'
    )"
    ;;
  *)
    echo "Unsupported DAISYCLAW_LIVE_ACP_BIND_AGENT: $agent" >&2
    exit 1
    ;;
esac
tmp_dir="$(mktemp -d)"
...
cd "$tmp_dir"
export DAISYCLAW_LIVE_ACP_BIND_AGENT_COMMAND="${DAISYCLAW_LIVE_ACP_BIND_AGENT_COMMAND:-}"
node scripts/test-live.mjs -- ${DAISYCLAW_LIVE_ACP_BIND_TEST_FILES:-src/gateway/gateway-acp-bind.live.test.ts}
EOF

daisyclaw_live_acp_bind_append_build_extension acpx
...
for token in "${ACP_AGENT_TOKENS[@]}"; do
  agent="$(daisyclaw_live_trim "$token")"
  [[ -n "$agent" ]] || continue
  daisyclaw_live_acp_bind_resolve_auth_provider "$agent" >/dev/null
  ACP_AGENTS+=("$agent")
done

if ((${#ACP_AGENTS[@]} == 0)); then
  echo "No ACP bind agents selected. Use DAISYCLAW_LIVE_ACP_BIND_AGENTS=claude,codex,droid,gemini,opencode." >&2
  exit 1
fi

for ACP_AGENT in "${ACP_AGENTS[@]}"; do
  AUTH_PROVIDER="$(daisyclaw_live_acp_bind_resolve_auth_provider "$ACP_AGENT")"
  AGENT_COMMAND="$(daisyclaw_live_acp_bind_resolve_agent_command "$ACP_AGENT")"
  ...
  if [[ -n "${DAISYCLAW_DOCKER_AUTH_DIRS:-}" ]]; then
    ...
    while IFS= read -r auth_dir; do
      [[ -n "$auth_dir" ]] || continue
      AUTH_DIRS+=("$auth_dir")
    done < <(daisyclaw_live_collect_auth_dirs_from_csv "$AUTH_PROVIDER")
    while IFS= read -r auth_file; do
      [[ -n "$auth_file" ]] || continue
      AUTH_FILES+=("$auth_file")
    done < <(daisyclaw_live_collect_auth_files_from_csv "$AUTH_PROVIDER")
  fi
  ...
  echo "==> Run ACP bind live test in Docker"
  echo "==> Agent: $ACP_AGENT"
  echo "==> Test files: ${DAISYCLAW_LIVE_ACP_BIND_TEST_FILES:-src/gateway/gateway-acp-bind.live.test.ts}"
  echo "==> Profile file: $PROFILE_STATUS"
  echo "==> Auth dirs: ${AUTH_DIRS_CSV:-none}"
  echo "==> Auth files: ${AUTH_FILES_CSV:-none}"
  if daisyclaw_live_uses_managed_bind_dirs; then
  ...
  DOCKER_RUN_ARGS=()
  daisyclaw_live_init_docker_run_args DOCKER_RUN_ARGS "${DAISYCLAW_LIVE_ACP_BIND_DOCKER_RUN_TIMEOUT:-2700s}"
  DOCKER_RUN_ARGS+=(--rm -t \
    -u "$DOCKER_USER" \
    --entrypoint bash \
    -e ANTHROPIC_API_KEY \
    -e ANTHROPIC_API_KEY_OLD \
    -e DAISYCLAW_LIVE_ACP_BIND_ANTHROPIC_API_KEY="${ANTHROPIC_API_KEY:-}" \
    -e DAISYCLAW_LIVE_ACP_BIND_ANTHROPIC_API_KEY_OLD="${ANTHROPIC_API_KEY_OLD:-}" \
    -e GEMINI_API_KEY \
    -e GOOGLE_API_KEY \
    -e FACTORY_API_KEY \
    -e OPENAI_API_KEY \
    ...
    -e DAISYCLAW_LIVE_ACP_BIND=1 \
    -e DAISYCLAW_LIVE_ACP_BIND_AGENT="$ACP_AGENT" \
    -e DAISYCLAW_LIVE_ACP_BIND_TEST_FILES="${DAISYCLAW_LIVE_ACP_BIND_TEST_FILES:-}" \
    -e DAISYCLAW_LIVE_ACP_BIND_CODEX_MODEL="${DAISYCLAW_LIVE_ACP_BIND_CODEX_MODEL:-}" \
    -e DAISYCLAW_LIVE_ACP_BIND_SETUP_TIMEOUT_SECONDS="${DAISYCLAW_LIVE_ACP_BIND_SETUP_TIMEOUT_SECONDS:-180}" \
    -e DAISYCLAW_LIVE_ACP_BIND_OPENCODE_MODEL="${DAISYCLAW_LIVE_ACP_BIND_OPENCODE_MODEL:-opencode/kimi-k2.6}" \
    -e DAISYCLAW_LIVE_ACP_SPAWN_DEFAULTS="${DAISYCLAW_LIVE_ACP_SPAWN_DEFAULTS:-}" \
    -e DAISYCLAW_LIVE_ACP_SPAWN_DEFAULTS_AGENT="${DAISYCLAW_LIVE_ACP_SPAWN_DEFAULTS_AGENT:-}" \
    -e DAISYCLAW_LIVE_ACP_SPAWN_DEFAULTS_CONNECT_TIMEOUT_MS="${DAISYCLAW_LIVE_ACP_SPAWN_DEFAULTS_CONNECT_TIMEOUT_MS:-}" \
    -e DAISYCLAW_LIVE_ACP_SPAWN_DEFAULTS_MODEL="${DAISYCLAW_LIVE_ACP_SPAWN_DEFAULTS_MODEL:-}" \
    -e DAISYCLAW_LIVE_ACP_SPAWN_DEFAULTS_THINKING="${DAISYCLAW_LIVE_ACP_SPAWN_DEFAULTS_THINKING:-}" \
    -e DAISYCLAW_LIVE_ACP_SPAWN_DEFAULTS_TIMEOUT_MS="${DAISYCLAW_LIVE_ACP_SPAWN_DEFAULTS_TIMEOUT_MS:-}" \
    -e DAISYCLAW_LIVE_ACP_BIND_AGENT_COMMAND="$AGENT_COMMAND")
