...
DOCKER_BUILD_EXTENSIONS="${DAISYCLAW_DOCKER_BUILD_EXTENSIONS:-${DAISYCLAW_EXTENSIONS:-}}"
LIVE_IMAGE_PULL_ATTEMPTS="${DAISYCLAW_LIVE_DOCKER_PULL_ATTEMPTS:-3}"
LIVE_IMAGE_PULL_RETRY_DELAY_SECONDS="${DAISYCLAW_LIVE_DOCKER_PULL_RETRY_DELAY_SECONDS:-5}"

if ! [[ "$LIVE_IMAGE_PULL_ATTEMPTS" =~ ^[1-9][0-9]*$ ]]; then
  echo "DAISYCLAW_LIVE_DOCKER_PULL_ATTEMPTS must be a positive integer, got: $LIVE_IMAGE_PULL_ATTEMPTS" >&2
  exit 2
fi
if ! [[ "$LIVE_IMAGE_PULL_RETRY_DELAY_SECONDS" =~ ^[0-9]+$ ]]; then
  echo "DAISYCLAW_LIVE_DOCKER_PULL_RETRY_DELAY_SECONDS must be a non-negative integer, got: $LIVE_IMAGE_PULL_RETRY_DELAY_SECONDS" >&2
  exit 2
fi
...
if [[ -n "${DOCKER_BUILD_EXTENSIONS}" ]]; then
  DOCKER_BUILD_ARGS+=(--build-arg "DAISYCLAW_EXTENSIONS=${DOCKER_BUILD_EXTENSIONS}")
fi
...
if [[ "${DAISYCLAW_SKIP_DOCKER_BUILD:-}" == "1" ]]; then
  ...
  if ! docker_build_on_missing_enabled; then
    ...
    echo "Build it first or unset DAISYCLAW_SKIP_DOCKER_BUILD." >&2
    exit 1
  fi
  echo "==> Live-test image not available; building because DAISYCLAW_DOCKER_BUILD_ON_MISSING/DAISYCLAW_TESTBOX allows fallback."
fi
