...
TRUSTED_HARNESS_DIR="${DAISYCLAW_LIVE_DOCKER_TRUSTED_HARNESS_DIR:-$SCRIPT_ROOT_DIR}"
...
DEFAULT_PROVIDER="${DAISYCLAW_DOCKER_CLI_BACKEND_PROVIDER:-claude-cli}"
CLI_MODEL="${DAISYCLAW_LIVE_CLI_BACKEND_MODEL:-}"
CLI_PROVIDER="${CLI_MODEL%%/*}"
CLI_DISABLE_MCP_CONFIG="${DAISYCLAW_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG:-}"
CLI_AUTH_MODE="${DAISYCLAW_LIVE_CLI_BACKEND_AUTH:-auto}"
CLI_SETUP_TIMEOUT_SECONDS="${DAISYCLAW_LIVE_CLI_BACKEND_SETUP_TIMEOUT_SECONDS:-180}"
TEMP_DIRS=()
...
case "$CLI_AUTH_MODE" in
  auto | api-key | subscription)
    ;;
  *)
    echo "ERROR: DAISYCLAW_LIVE_CLI_BACKEND_AUTH must be one of: auto, api-key, subscription." >&2
    exit 1
    ;;
esac

if [[ "$CLI_AUTH_MODE" == "subscription" && "$CLI_PROVIDER" != "claude-cli" ]]; then
  echo "ERROR: DAISYCLAW_LIVE_CLI_BACKEND_AUTH=subscription is only supported for claude-cli." >&2
  exit 1
fi
...
export DAISYCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE="${DAISYCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE:-0}"
export DAISYCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE="${DAISYCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE:-0}"
export DAISYCLAW_LIVE_CLI_BACKEND_MCP_PROBE="${DAISYCLAW_LIVE_CLI_BACKEND_MCP_PROBE:-0}"
...
elif daisyclaw_live_is_ci; then
...
if [[ "$CLI_PROVIDER" == "claude-cli" && "$CLI_AUTH_MODE" == "subscription" ]]; then
  ...
  if [[ -z "${DAISYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV:-}" ]]; then
    if [[ "$CLAUDE_SUBSCRIPTION_AUTH_SOURCE" == "env-token" ]]; then
      export DAISYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV='["CLAUDE_CODE_OAUTH_TOKEN"]'
    else
      export DAISYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV="[]"
    fi
  fi
  if [[ "$DAISYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV" == *ANTHROPIC_API_KEY* ]]; then
    echo "ERROR: subscription auth smoke must not preserve Anthropic API-key env vars." >&2
    exit 1
  fi
  if [[ "$CLAUDE_SUBSCRIPTION_AUTH_SOURCE" == "env-token" && "$DAISYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV" != *CLAUDE_CODE_OAUTH_TOKEN* ]]; then
  ...
  export DAISYCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE="${DAISYCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE:-0}"
  export DAISYCLAW_LIVE_CLI_BACKEND_RESUME_PROBE="${DAISYCLAW_LIVE_CLI_BACKEND_RESUME_PROBE:-1}"
  export DAISYCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE="${DAISYCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE:-0}"
  export DAISYCLAW_LIVE_CLI_BACKEND_MCP_PROBE="${DAISYCLAW_LIVE_CLI_BACKEND_MCP_PROBE:-0}"
fi
...
AUTH_DIRS=()
AUTH_FILES=()
if [[ -n "${DAISYCLAW_DOCKER_AUTH_DIRS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(daisyclaw_live_collect_auth_dirs)
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    AUTH_FILES+=("$auth_file")
  done < <(daisyclaw_live_collect_auth_files)
else
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(daisyclaw_live_collect_auth_dirs_from_csv "$CLI_PROVIDER")
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    AUTH_FILES+=("$auth_file")
  done < <(daisyclaw_live_collect_auth_files_from_csv "$CLI_PROVIDER")
fi
AUTH_DIRS_CSV=""
if ((${#AUTH_DIRS[@]} > 0)); then
  AUTH_DIRS_CSV="$(daisyclaw_live_join_csv "${AUTH_DIRS[@]}")"
fi
AUTH_FILES_CSV=""
...
if [[ -n "${DOCKER_HOME_DIR:-}" ]]; then
  daisyclaw_live_stage_auth_into_home "$DOCKER_HOME_DIR" "${AUTH_DIRS[@]}" --files "${AUTH_FILES[@]}"
...
if ((${#AUTH_DIRS[@]} > 0)); then
  for auth_dir in "${AUTH_DIRS[@]}"; do
    auth_dir="$(daisyclaw_live_validate_relative_home_path "$auth_dir")"
...
run_setup_command() {
  local timeout_value="${DAISYCLAW_LIVE_CLI_BACKEND_SETUP_TIMEOUT_SECONDS:-180}s"
  local timeout_bin=""
...
if [ "${DAISYCLAW_DOCKER_AUTH_PRESTAGED:-0}" != "1" ]; then
  IFS=',' read -r -a auth_dirs <<<"${DAISYCLAW_DOCKER_AUTH_DIRS_RESOLVED:-}"
...
provider="${DAISYCLAW_DOCKER_CLI_BACKEND_PROVIDER:-claude-cli}"
default_command="${DAISYCLAW_DOCKER_CLI_BACKEND_COMMAND_DEFAULT:-}"
docker_package="${DAISYCLAW_DOCKER_CLI_BACKEND_NPM_PACKAGE:-}"
binary_name="${DAISYCLAW_DOCKER_CLI_BACKEND_BINARY_NAME:-}"
if [ -z "$binary_name" ] && [ -n "$default_command" ]; then
  binary_name="$(basename "$default_command")"
fi
if [ -z "${DAISYCLAW_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ -n "$binary_name" ]; then
  export DAISYCLAW_LIVE_CLI_BACKEND_COMMAND="$NPM_CONFIG_PREFIX/bin/$binary_name"
fi
...
if [ -n "${DAISYCLAW_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ ! -x "${DAISYCLAW_LIVE_CLI_BACKEND_COMMAND}" ] && [ -n "$docker_package" ]; then
  run_setup_command npm install -g "$docker_package"
elif [ -n "$docker_package" ] && package_has_explicit_version "$docker_package"; then
  run_setup_command npm install -g "$docker_package"
fi
if [ -n "${DAISYCLAW_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ -x "${DAISYCLAW_LIVE_CLI_BACKEND_COMMAND}" ]; then
  echo "==> CLI backend binary: ${DAISYCLAW_LIVE_CLI_BACKEND_COMMAND}"
  "${DAISYCLAW_LIVE_CLI_BACKEND_COMMAND}" -V || "${DAISYCLAW_LIVE_CLI_BACKEND_COMMAND}" --version || true
fi
if [ "$provider" = "claude-cli" ]; then
  auth_mode="${DAISYCLAW_LIVE_CLI_BACKEND_AUTH:-auto}"
...
if [ -n "\${DAISYCLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY:-}" ]; then
  export ANTHROPIC_API_KEY="\${DAISYCLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY}"
fi
if [ -n "\${DAISYCLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD:-}" ]; then
  export ANTHROPIC_API_KEY_OLD="\${DAISYCLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD}"
fi
...
WRAP
  ...
  if [ -z "${DAISYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV:-}" ]; then
    export DAISYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV='["ANTHROPIC_API_KEY","ANTHROPIC_API_KEY_OLD"]'
  fi
  if [ "$auth_mode" = "subscription" ]; then
    claude --version
    direct_token="DAISYCLAW-CLAUDE-SUBSCRIPTION-DIRECT"
    direct_output="$(
      claude \
...
DOCKER_AUTH_ENV=(
  -e DAISYCLAW_LIVE_CLI_BACKEND_AUTH="$CLI_AUTH_MODE"
)
if [[ "$CLI_PROVIDER" == "claude-cli" && "$CLI_AUTH_MODE" == "subscription" ]]; then
  DOCKER_AUTH_ENV+=(
    -e CLAUDE_CODE_OAUTH_TOKEN="${CLAUDE_CODE_OAUTH_TOKEN:-}"
    -e DAISYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV="$DAISYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV"
  )
else
  DOCKER_AUTH_ENV+=(
    -e ANTHROPIC_API_KEY
    -e ANTHROPIC_API_KEY_OLD
    -e DAISYCLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY="${ANTHROPIC_API_KEY:-}"
    -e DAISYCLAW_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD="${ANTHROPIC_API_KEY_OLD:-}"
    -e DAISYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV="${DAISYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV:-}"
  )
fi

DOCKER_RUN_ARGS=()
daisyclaw_live_init_docker_run_args DOCKER_RUN_ARGS "${DAISYCLAW_LIVE_CLI_BACKEND_DOCKER_RUN_TIMEOUT:-2700s}"
DOCKER_RUN_ARGS+=(--rm -t \
  ...
  -e DAISYCLAW_DOCKER_AUTH_DIRS_RESOLVED="$AUTH_DIRS_CSV" \
  ...
  -e DAISYCLAW_LIVE_CLI_BACKEND_SETUP_TIMEOUT_SECONDS="$CLI_SETUP_TIMEOUT_SECONDS" \
  -e DAISYCLAW_DOCKER_CLI_BACKEND_PROVIDER="$CLI_PROVIDER" \
  -e DAISYCLAW_DOCKER_CLI_BACKEND_COMMAND_DEFAULT="$CLI_DEFAULT_COMMAND" \
  -e DAISYCLAW_DOCKER_CLI_BACKEND_NPM_PACKAGE="$CLI_DOCKER_NPM_PACKAGE" \
  -e DAISYCLAW_DOCKER_CLI_BACKEND_BINARY_NAME="$CLI_DOCKER_BINARY_NAME" \
  -e DAISYCLAW_LIVE_TEST=1 \
  -e DAISYCLAW_LIVE_CLI_BACKEND=1 \
  -e DAISYCLAW_LIVE_CLI_BACKEND_DEBUG="${DAISYCLAW_LIVE_CLI_BACKEND_DEBUG:-}" \
  -e DAISYCLAW_CLI_BACKEND_LOG_OUTPUT="${DAISYCLAW_CLI_BACKEND_LOG_OUTPUT:-}" \
  -e DAISYCLAW_TEST_CONSOLE="${DAISYCLAW_TEST_CONSOLE:-}" \
  -e DAISYCLAW_LIVE_CLI_BACKEND_MODEL="$CLI_MODEL" \
  -e DAISYCLAW_LIVE_CLI_BACKEND_COMMAND="${DAISYCLAW_LIVE_CLI_BACKEND_COMMAND:-}" \
  -e DAISYCLAW_LIVE_CLI_BACKEND_ARGS="${DAISYCLAW_LIVE_CLI_BACKEND_ARGS:-}" \
  -e DAISYCLAW_LIVE_CLI_BACKEND_RESUME_ARGS="${DAISYCLAW_LIVE_CLI_BACKEND_RESUME_ARGS:-}" \
  -e DAISYCLAW_LIVE_CLI_BACKEND_CLEAR_ENV="${DAISYCLAW_LIVE_CLI_BACKEND_CLEAR_ENV:-}" \
  -e DAISYCLAW_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG="$CLI_DISABLE_MCP_CONFIG" \
  -e DAISYCLAW_LIVE_CLI_BACKEND_RESUME_PROBE="${DAISYCLAW_LIVE_CLI_BACKEND_RESUME_PROBE:-}" \
  -e DAISYCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE="${DAISYCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE:-}" \
  -e DAISYCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE="${DAISYCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE:-}" \
  -e DAISYCLAW_LIVE_CLI_BACKEND_MCP_PROBE="${DAISYCLAW_LIVE_CLI_BACKEND_MCP_PROBE:-}" \
  -e DAISYCLAW_LIVE_CLI_BACKEND_MCP_SCHEMA_PROBE="${DAISYCLAW_LIVE_CLI_BACKEND_MCP_SCHEMA_PROBE:-}" \
  -e DAISYCLAW_LIVE_CLI_BACKEND_IMAGE_ARG="${DAISYCLAW_LIVE_CLI_BACKEND_IMAGE_ARG:-}" \
  -e DAISYCLAW_LIVE_CLI_BACKEND_IMAGE_MODE="${DAISYCLAW_LIVE_CLI_BACKEND_IMAGE_MODE:-}")
