...
elif [[ -n "${DAISYCLAW_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(daisyclaw_live_collect_auth_dirs_from_csv "${DAISYCLAW_LIVE_GATEWAY_PROVIDERS:-}")
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    AUTH_FILES+=("$auth_file")
  done < <(daisyclaw_live_collect_auth_files_from_csv "${DAISYCLAW_LIVE_GATEWAY_PROVIDERS:-}")
else
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
...
CONTAINER_NODE_OPTIONS="$(daisyclaw_live_container_node_options)"
...
DOCKER_RUN_ARGS=()
daisyclaw_live_init_docker_run_args DOCKER_RUN_ARGS "${DAISYCLAW_LIVE_GATEWAY_DOCKER_RUN_TIMEOUT:-2100s}"
DOCKER_RUN_ARGS+=(--rm -t \
  ...
  -e DAISYCLAW_LIVE_TEST_QUIET="${DAISYCLAW_LIVE_TEST_QUIET:-}" \
  -e DAISYCLAW_LIVE_WRAPPER_HEARTBEAT_MS="${DAISYCLAW_LIVE_WRAPPER_HEARTBEAT_MS:-}" \
  -e DAISYCLAW_LIVE_REQUIRE_PROFILE_KEYS="${DAISYCLAW_LIVE_REQUIRE_PROFILE_KEYS:-}" \
  -e DAISYCLAW_LIVE_GATEWAY_MODELS="${DAISYCLAW_LIVE_GATEWAY_MODELS:-modern}" \
  -e DAISYCLAW_LIVE_GATEWAY_PROVIDERS="${DAISYCLAW_LIVE_GATEWAY_PROVIDERS:-}" \
  -e DAISYCLAW_LIVE_GATEWAY_THINKING="${DAISYCLAW_LIVE_GATEWAY_THINKING:-}" \
  -e DAISYCLAW_LIVE_GATEWAY_SMOKE="${DAISYCLAW_LIVE_GATEWAY_SMOKE:-1}" \
  -e DAISYCLAW_LIVE_GATEWAY_MAX_MODELS="${DAISYCLAW_LIVE_GATEWAY_MAX_MODELS:-8}" \
  -e DAISYCLAW_LIVE_GATEWAY_HEARTBEAT_MS="${DAISYCLAW_LIVE_GATEWAY_HEARTBEAT_MS:-}" \
  -e DAISYCLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS="${DAISYCLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS:-45000}" \
  -e DAISYCLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS="${DAISYCLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-90000}" \
