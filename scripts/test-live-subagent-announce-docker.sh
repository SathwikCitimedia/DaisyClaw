...
if [[ -n "${OPENAI_API_KEY:-}" || -n "${OPENAI_BASE_URL:-}" || -n "${GEMINI_API_KEY:-}" || -n "${GOOGLE_API_KEY:-}" ]]; then
  docker_env_dir="$(mktemp -d "${RUNNER_TEMP:-/tmp}/daisyclaw-subagent-live-env.XXXXXX")"
  TEMP_DIRS+=("$docker_env_dir")
  docker_env_file="$docker_env_dir/provider.env"
  {
    if [[ -n "${OPENAI_API_KEY:-}" ]]; then
      printf 'DAISYCLAW_DOCKER_LIVE_OPENAI_API_KEY=%s\n' "${OPENAI_API_KEY}"
    fi
    if [[ -n "${OPENAI_BASE_URL:-}" ]]; then
      printf 'DAISYCLAW_DOCKER_LIVE_OPENAI_BASE_URL=%s\n' "${OPENAI_BASE_URL}"
    fi
    if [[ -n "${GEMINI_API_KEY:-}" ]]; then
      printf 'DAISYCLAW_DOCKER_LIVE_GEMINI_API_KEY=%s\n' "${GEMINI_API_KEY}"
    fi
    if [[ -n "${GOOGLE_API_KEY:-}" ]]; then
      printf 'DAISYCLAW_DOCKER_LIVE_GOOGLE_API_KEY=%s\n' "${GOOGLE_API_KEY}"
    fi
  } >"$docker_env_file"
...
if [ -n "${DAISYCLAW_DOCKER_LIVE_OPENAI_API_KEY:-}" ]; then
  export OPENAI_API_KEY="$DAISYCLAW_DOCKER_LIVE_OPENAI_API_KEY"
  unset DAISYCLAW_DOCKER_LIVE_OPENAI_API_KEY
fi
if [ -n "${DAISYCLAW_DOCKER_LIVE_OPENAI_BASE_URL:-}" ]; then
  export OPENAI_BASE_URL="$DAISYCLAW_DOCKER_LIVE_OPENAI_BASE_URL"
  unset DAISYCLAW_DOCKER_LIVE_OPENAI_BASE_URL
fi
if [ -n "${DAISYCLAW_DOCKER_LIVE_GEMINI_API_KEY:-}" ]; then
  export GEMINI_API_KEY="$DAISYCLAW_DOCKER_LIVE_GEMINI_API_KEY"
  unset DAISYCLAW_DOCKER_LIVE_GEMINI_API_KEY
fi
if [ -n "${DAISYCLAW_DOCKER_LIVE_GOOGLE_API_KEY:-}" ]; then
  export GOOGLE_API_KEY="$DAISYCLAW_DOCKER_LIVE_GOOGLE_API_KEY"
  unset DAISYCLAW_DOCKER_LIVE_GOOGLE_API_KEY
fi
...
DAISYCLAW_LIVE_SUBAGENT_E2E=1 \
DAISYCLAW_VITEST_MAX_WORKERS="${DAISYCLAW_VITEST_MAX_WORKERS:-1}" \
...
echo "==> Model: ${DAISYCLAW_LIVE_SUBAGENT_E2E_MODEL:-openai/gpt-5.5}"
echo "==> Profile file: $PROFILE_STATUS"
DOCKER_RUN_ARGS=()
daisyclaw_live_init_docker_run_args DOCKER_RUN_ARGS "${DAISYCLAW_LIVE_SUBAGENT_DOCKER_RUN_TIMEOUT:-1200s}"
DOCKER_RUN_ARGS+=(--rm -t \
  ...
  -e DAISYCLAW_LIVE_SUBAGENT_E2E=1 \
  -e DAISYCLAW_LIVE_SUBAGENT_E2E_MODEL="${DAISYCLAW_LIVE_SUBAGENT_E2E_MODEL:-}" \
  -e DAISYCLAW_VITEST_FS_MODULE_CACHE=0 \
  -e DAISYCLAW_VITEST_MAX_WORKERS="${DAISYCLAW_VITEST_MAX_WORKERS:-1}")
