...
function applyDefaultParallelVitestWorkerBudget(specs, env) {
  if (env.DAISYCLAW_VITEST_MAX_WORKERS || env.DAISYCLAW_TEST_WORKERS || isCiLikeEnv(env)) {
    return specs;
  }
  ...
  return specs.map((spec) => ({
    ...
    env: {
      ...spec.env,
      DAISYCLAW_VITEST_MAX_WORKERS: String(vitestMaxWorkers),
    },
  }));
}
...
function printNoChangedTestTargets(args, cwd, baseEnv) {
  ...
  console.error("[test] run `DAISYCLAW_TEST_CHANGED_BROAD=1 pnpm test:changed` for broad coverage.");
}
...
async function main() {
  ...
  const isExplicitParallelMultiConfigRun =
    Boolean(baseEnv.DAISYCLAW_TEST_PROJECTS_PARALLEL) &&
    runSpecs.length > 1 &&
    !runSpecs.some((spec) => spec.watchMode);
  ...
  if (isParallelShardRun) {
    ...
    if (concurrency > 1) {
      ...
      if (
        !isCiLikeEnv(baseEnv) &&
        !baseEnv.DAISYCLAW_TEST_PROJECTS_PARALLEL &&
        !baseEnv.DAISYCLAW_VITEST_MAX_WORKERS &&
        !baseEnv.DAISYCLAW_TEST_WORKERS &&
