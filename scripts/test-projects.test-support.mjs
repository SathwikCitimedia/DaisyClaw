...
const TOOLING_ISOLATED_TEST_TARGET = "test/scripts/daisyclaw-e2e-instance.test.ts";
...
const INCLUDE_FILE_ENV_KEY = "DAISYCLAW_VITEST_INCLUDE_FILE";
const FS_MODULE_CACHE_PATH_ENV_KEY = "DAISYCLAW_VITEST_FS_MODULE_CACHE_PATH";
...
const BROAD_CHANGED_FALLBACK_PATTERNS = [
  /^package\.json$/u,
  /^pnpm-lock\.yaml$/u,
  /^test\/setup(?:\.shared|\.extensions|-daisyclaw-runtime)?\.ts$/u,
...
const TOOLING_SOURCE_TEST_TARGETS = new Map([
  ...
  ["scripts/lib/daisyclaw-test-state.mjs", ["test/scripts/daisyclaw-test-state.test.ts"]],
  ...
  ["scripts/daisyclaw-npm-postpublish-verify.ts", ["test/daisyclaw-npm-postpublish-verify.test.ts"]],
  ...
  ["scripts/package-daisyclaw-for-docker.mjs", ["test/scripts/package-daisyclaw-for-docker.test.ts"]],
  ...
  [
    ...
    ["test/scripts/docker-build-helper.test.ts", "test/scripts/daisyclaw-test-state.test.ts"],
  ],
...
const TOOLING_TEST_TARGETS = new Map([
  ...
  ["test/scripts/daisyclaw-test-state.test.ts", ["test/scripts/daisyclaw-test-state.test.ts"]],
...
const SOURCE_TEST_TARGETS = new Map([
  ...
  ["src/test-utils/daisyclaw-test-state.ts", ["src/test-utils/daisyclaw-test-state.test.ts"]],
  [
...
const BROAD_CHANGED_ENV_KEY = "DAISYCLAW_TEST_CHANGED_BROAD";
const VITEST_NO_OUTPUT_TIMEOUT_ENV_KEY = "DAISYCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS";
const VITEST_NO_OUTPUT_HEARTBEAT_ENV_KEY = "DAISYCLAW_VITEST_NO_OUTPUT_HEARTBEAT_MS";
const VITEST_NO_OUTPUT_RETRY_ENV_KEY = "DAISYCLAW_VITEST_NO_OUTPUT_RETRY";
...
export function buildFullSuiteVitestRunPlans(args, cwd = process.cwd()) {
  ...
  const parallelShardCount = parsePositiveInt(
    process.env.DAISYCLAW_TEST_PROJECTS_PARALLEL,
    "DAISYCLAW_TEST_PROJECTS_PARALLEL",
  );
...
export function shouldUseLocalFullSuiteParallelByDefault(env = process.env) {
  ...
  return (
    env.DAISYCLAW_TEST_PROJECTS_SERIAL !== "1" && env.CI !== "true" && env.GITHUB_ACTIONS !== "true"
  );
}
...
function hasConservativeVitestWorkerBudget(env) {
  const workerBudget = parsePositiveInt(
    env.DAISYCLAW_VITEST_MAX_WORKERS ?? env.DAISYCLAW_TEST_WORKERS,
    env.DAISYCLAW_VITEST_MAX_WORKERS === undefined
      ? "DAISYCLAW_TEST_WORKERS"
      : "DAISYCLAW_VITEST_MAX_WORKERS",
  );
...
export function resolveParallelFullSuiteConcurrency(specCount, envInput, hostInfo) {
  ...
  const override = parsePositiveInt(
    env.DAISYCLAW_TEST_PROJECTS_PARALLEL,
    "DAISYCLAW_TEST_PROJECTS_PARALLEL",
  );
  if (override !== null) {
    return Math.min(override, specCount);
  }
  if (env.DAISYCLAW_TEST_PROJECTS_SERIAL === "1") {
    return 1;
  }
  if (isCiLikeEnv(env)) {
    return 1;
  ...
  if (
    env.DAISYCLAW_TEST_PROJECTS_LEAF_SHARDS !== "1" &&
  ...
  return plans.map((plan, index) => {
    const includeFilePath = plan.includePatterns
      ? path.join(
          ...
          `daisyclaw-vitest-include-${process.pid}-${Date.now()}-${index}.json`,
        )
      : null;
    return {
      config: plan.config,
...
export function shouldAcquireLocalHeavyCheckLock(runSpecs, env = process.env) {
  if (env.DAISYCLAW_TEST_HEAVY_CHECK_LOCK_HELD === "1") {
    return false;
  }

  if (env.DAISYCLAW_TEST_PROJECTS_FORCE_LOCK === "1") {
    return true;
  }
