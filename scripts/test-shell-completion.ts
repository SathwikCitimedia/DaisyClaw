 ...
 * `daisyclaw update`. Use it to verify the completion installation flow
...
\`daisyclaw update\`, \`daisyclaw doctor\`, and \`daisyclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# DaisyClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`daisyclaw completion --install\` later to enable.`));
    return;
  }
