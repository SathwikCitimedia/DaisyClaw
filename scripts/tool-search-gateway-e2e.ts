// Tool Search Gateway E2E script supports DaisyClaw repository automation.
...
export function readToolSearchGatewayFetchLimits(
  ...
  return {
    bodyMaxBytes: readPositiveIntEnv(
      "DAISYCLAW_TOOL_SEARCH_GATEWAY_E2E_FETCH_BODY_MAX_BYTES",
      1024 * 1024,
      env,
    ),
    timeoutMs: readPositiveIntEnv(
      "DAISYCLAW_TOOL_SEARCH_GATEWAY_E2E_FETCH_TIMEOUT_MS",
      180_000,
      env,
    ),
  };
...
export function snapshotToolSearchGatewayEnv(
  ...
  return {
    configPath: env.DAISYCLAW_CONFIG_PATH,
    stateDir: env.DAISYCLAW_STATE_DIR,
    testFast: env.DAISYCLAW_TEST_FAST,
  };
}
...
function restoreEnvValue(
  ...
  key: keyof Pick<
    NodeJS.ProcessEnv,
    "DAISYCLAW_CONFIG_PATH" | "DAISYCLAW_STATE_DIR" | "DAISYCLAW_TEST_FAST"
  >,
...
export function restoreToolSearchGatewayEnv(
  ...
  restoreEnvValue(env, "DAISYCLAW_CONFIG_PATH", snapshot.configPath);
  restoreEnvValue(env, "DAISYCLAW_STATE_DIR", snapshot.stateDir);
  restoreEnvValue(env, "DAISYCLAW_TEST_FAST", snapshot.testFast);
}
...
async function writeFakePlugin(params: {
  ...
  await fs.writeFile(
    ...
    `${JSON.stringify(
      {
        name: "@daisyclaw/tool-search-e2e-fixture",
        version: "0.0.0",
        type: "module",
        daisyclaw: {
          extensions: ["./index.js"],
...
export async function main() {
  ...
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-tool-search-"));
