...
// metadata expected by DaisyClaw.
...
export function collectPluginNpmPublishedRuntimeErrors(params) {
  ...
  const runtimeExtensionsResult = readPackageStringList(
    packageLabel,
    "daisyclaw.runtimeExtensions",
    packageJson.daisyclaw?.runtimeExtensions,
  );
  const setupEntryResult = readOptionalPackageString(
    packageLabel,
    "daisyclaw.setupEntry",
    packageJson.daisyclaw?.setupEntry,
  );
  const runtimeSetupEntryResult = readOptionalPackageString(
    packageLabel,
    "daisyclaw.runtimeSetupEntry",
    packageJson.daisyclaw?.runtimeSetupEntry,
  );
  errors.push(
  ...
  if (runtimeExtensions.length > 0 && runtimeExtensions.length !== extensions.length) {
    errors.push(
      `${packageLabel} package.json daisyclaw.runtimeExtensions length (${runtimeExtensions.length}) must match daisyclaw.extensions length (${extensions.length})`,
    );
    return errors;
  }
  ...
  if (runtimeSetupEntry && !setupEntry) {
    errors.push(
      `${packageLabel} package.json daisyclaw.runtimeSetupEntry requires daisyclaw.setupEntry`,
    );
    return errors;
  }
...
export function readPluginNpmCommandOptions(env = process.env) {
  return {
    ...
    maxBuffer: readPositiveIntEnv(
      "DAISYCLAW_PLUGIN_NPM_COMMAND_MAX_BUFFER_BYTES",
      DEFAULT_NPM_COMMAND_MAX_BUFFER_BYTES,
      env,
    ),
    stdio: ["ignore", "pipe", "pipe"],
    timeout: readPositiveIntEnv(
      "DAISYCLAW_PLUGIN_NPM_COMMAND_TIMEOUT_MS",
...
async function packPublishedPackage(spec, destinationDir) {
  const attempts = readPositiveIntEnv("DAISYCLAW_PLUGIN_NPM_VERIFY_ATTEMPTS", 90);
  const delayMs = readPositiveIntEnv("DAISYCLAW_PLUGIN_NPM_VERIFY_DELAY_MS", 10000);
  let lastError;
...
async function verifyPublishedPackageReadme(spec) {
  const attempts = readPositiveIntEnv("DAISYCLAW_PLUGIN_NPM_README_VERIFY_ATTEMPTS", 6);
  const delayMs = readPositiveIntEnv("DAISYCLAW_PLUGIN_NPM_README_VERIFY_DELAY_MS", 10000);
  let lastError;
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
...
export async function verifyPublishedPluginRuntime(spec) {
  const workingDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-plugin-npm-runtime."));
  try {
