...
const shouldRunAutoDoctor = (deps, autoDoctorAttempted) =>
  ...
  !AUTO_DOCTOR_DISABLE_VALUES.has(
    String(deps.env.DAISYCLAW_GATEWAY_WATCH_AUTO_DOCTOR ?? "").toLowerCase(),
  );
...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[daisyclaw] ${message}\n`);
};
...
const printFriendlyWatchStartupError = (err) => {
  ...
  if (packageConfigPath) {
    console.error(`[daisyclaw] Invalid package config: ${packageConfigPath}`);
  }
  console.error("[daisyclaw] This usually means a file in node_modules is empty or truncated.");
  console.error("[daisyclaw] Recommended recovery:");
  console.error("[daisyclaw]   rm -rf node_modules");
  console.error("[daisyclaw]   pnpm store prune");
  console.error("[daisyclaw]   pnpm install");
  console.error("");
  console.error("[daisyclaw] Original error:");
  console.error(err);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.DAISYCLAW_WATCH_MODE = "1";
  childEnv.DAISYCLAW_WATCH_SESSION = watchSession;
  ...
  childEnv.DAISYCLAW_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.DAISYCLAW_WATCH_COMMAND = deps.args.join(" ");
  }
  ...
  return await new Promise((resolve, reject) => {
    ...
    const runAutoDoctorAndRestart = () => {
      ...
      logWatcher(
        "Gateway exited early; running `daisyclaw doctor --fix --non-interactive` once.",
        deps,
      );
