// Write Cli Startup Metadata script supports DaisyClaw repository automation.
...
export function readBundledChannelCatalog(
  ...
  for (const dirEntry of readdirSync(extensionsDirOverride, { withFileTypes: true })) {
    ...
    try {
      ...
      const id = parsed.daisyclaw?.channel?.id;
      if (typeof id !== "string" || !id.trim()) {
        continue;
      }
      const orderRaw = parsed.daisyclaw?.channel?.order;
      const labelRaw = parsed.daisyclaw?.channel?.label;
      entries.push({
        id: id.trim(),
...
function createIsolatedRootHelpRenderContext(
  ...
  const stateDir = path.join(rootDir, ".daisyclaw-build-root-help");
  const workspaceDir = path.join(stateDir, "workspace");
  const homeDir = path.join(stateDir, "home");
  const env: NodeJS.ProcessEnv = {
    HOME: homeDir,
    LOGNAME: process.env.LOGNAME ?? process.env.USER ?? "daisyclaw-build",
    USER: process.env.USER ?? process.env.LOGNAME ?? "daisyclaw-build",
...
async function renderSourceBrowserHelpText(
  ...
  const inlineModule = [
    ...
    `registerBrowserCli(program, ["node", "daisyclaw", "browser", "--help"]);`,
...
async function renderSourceCommandHelpText(
  ...
  return await spawnText(["daisyclaw.mjs", command, "--help"], {
    cwd: rootDir,
    env: {
      ...renderContext.env,
      DAISYCLAW_DISABLE_CLI_STARTUP_HELP_FAST_PATH: "1",
