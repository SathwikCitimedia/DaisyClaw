...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.daisyclaw) ? packageJson.daisyclaw : null;
...
function getCatalogChannelId(entry) {
  return trimString(entry?.daisyclaw?.channel?.id) || trimString(entry?.name);
}
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.daisyclaw?.channel?.id) || left.name;
    const rightId = trimString(right.daisyclaw?.channel?.id) || right.name;
