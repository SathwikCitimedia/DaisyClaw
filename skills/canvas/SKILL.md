---
name: canvas
description: "Present HTML on connected DaisyClaw node canvases, navigate/eval/snapshot, and debug canvas host URLs."
metadata: { "daisyclaw": { "emoji": "🖼️" } }
---
...
Active config: `$DAISYCLAW_CONFIG_PATH` or `~/.daisyclaw/daisyclaw.json`.
...
4. Present the hosted URL: `/__daisyclaw__/canvas/<file>.html`.
5. Use `snapshot` when the user needs proof.

## URL shape

```text
http://<gateway-host>:<gateway.port>/__daisyclaw__/canvas/index.html
http://<gateway-host>:<gateway.port>/__daisyclaw__/canvas/games/snake.html
```

Path mapping:

- `/__daisyclaw__/canvas/index.html` -> `<canvas host root>/index.html`
- `/__daisyclaw__/canvas/games/snake.html` -> `<canvas host root>/games/snake.html`
