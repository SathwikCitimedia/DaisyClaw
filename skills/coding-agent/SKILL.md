...
Use for background feature builds, PR reviews, large refactors, and issue-to-PR loops. Do not use for simple edits, read-only lookup, ACP thread-bound work, or any run inside `~/.daisyclaw`, `$DAISYCLAW_STATE_DIR`, or active DaisyClaw state dirs.
...
- Worker must send completion/failure via `daisyclaw message send`.
...
- Never checkout branches or run background coding agents in `~/Projects/daisyclaw`; use an isolated checkout.
...
daisyclaw message send --channel <channel> --target '<target>' --message '<brief result>'
Add --account, --reply-to, or --thread-id only when present above.
Do not use daisyclaw system event or heartbeat.
```
...
```bash
PROMPT=$(mktemp -t daisyclaw-worker-prompt.XXXXXX)
cat >"$PROMPT" <<'EOF'
Task.
<notification block>
EOF
