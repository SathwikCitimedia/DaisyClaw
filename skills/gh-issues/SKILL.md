...
if [ -z "${GH_TOKEN:-}" ]; then
  CONFIG_PATH="${DAISYCLAW_CONFIG_PATH:-${DAISYCLAW_STATE_DIR:-$HOME/.daisyclaw}/daisyclaw.json}"
...
```text
${DAISYCLAW_STATE_DIR:-$HOME/.daisyclaw}/gh-issues-<owner>-<repo>.json
```
...
Send completion/failure with daisyclaw message send if route provided.
```
