---
name: node-connect
description: "Diagnose DaisyClaw Android, iOS, or macOS node pairing, QR/setup code, route, auth, and connection failures."
---

# Node Connect

Goal: find the one real route from node -> gateway, verify DaisyClaw is advertising that route, then fix pairing/auth.
...
- whether `daisyclaw devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `daisyclaw qr --json`. It uses the same setup-code payload Android scans.

```bash
daisyclaw config get gateway.mode
daisyclaw config get gateway.bind
daisyclaw config get gateway.tailscale.mode
daisyclaw config get gateway.remote.url
daisyclaw config get gateway.auth.mode
daisyclaw config get gateway.auth.allowTailscale
daisyclaw config get plugins.entries.device-pair.config.publicUrl
daisyclaw qr --json
daisyclaw devices list
daisyclaw nodes status
```

If this DaisyClaw instance is pointed at a remote gateway, also run:

```bash
daisyclaw qr --remote --json
```
...
`daisyclaw qr --json` success means:
...
If `daisyclaw qr --json` says `Gateway is only bound to loopback`:
...
```bash
daisyclaw devices list
daisyclaw devices approve --latest   # preview only; copy the requestId from output
...
- `daisyclaw devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run daisyclaw qr again, rescan, then approve the pending device pairing.`
