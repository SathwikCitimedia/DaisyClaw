...
1. Download the runtime for your OS (extracts into `$DAISYCLAW_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.daisyclaw/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$DAISYCLAW_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.daisyclaw/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active DaisyClaw config file (`$DAISYCLAW_CONFIG_PATH`, default `~/.daisyclaw/daisyclaw.json`):
