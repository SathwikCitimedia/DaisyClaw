...
const createTempDir = () => tempDirs.make("daisyclaw-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets DAISYCLAW_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "daisyclaw",
    });

    expect(env.DAISYCLAW_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("daisyclaw");
  });

  it("overrides pre-existing DAISYCLAW_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      DAISYCLAW_SHELL: "wrong",
    });
    expect(env.DAISYCLAW_SHELL).toBe("acp-client");
  });
  ...
  it("preserves DAISYCLAW_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        DAISYCLAW_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["DAISYCLAW_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default DaisyClaw bridge", () => {
    ...
    expect(env.DAISYCLAW_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.DAISYCLAW_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "daisyclaw",
        serverArgs: ["acp"],
        defaultServerCommand: "daisyclaw",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("DAISYCLAW_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "daisyclaw", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "daisyclaw", "dist", "entry.js");
    const shimPath = path.join(dir, "daisyclaw.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\daisyclaw\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/daisyclaw-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/daisyclaw-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/daisyclaw-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
