/** Interactive stdio ACP client used to connect a terminal session to an DaisyClaw ACP server. */
...
async function createAcpClient(opts: AcpClientOptions = {}): Promise<AcpClientHandle> {
  ...
  const defaultServerCommand = entryPath ? process.execPath : "daisyclaw";
  ...
  await client.initialize({
    ...
    clientInfo: { name: "daisyclaw-acp-client", version: "1.0.0" },
  });
...
export async function runAcpClientInteractive(opts: AcpClientOptions = {}): Promise<void> {
  ...
  console.log("DaisyClaw ACP client");
