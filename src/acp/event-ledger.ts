...
import { resolveIntegerOption } from "@daisyclaw/acp-core/numeric-options";
...
/** Resolves the legacy file-backed ACP ledger path under the DaisyClaw state directory. */
...
export async function migrateFileAcpEventLedgerToSqlite(
  params: { filePath: string; archiveSource?: boolean } & DaisyClawStateDatabaseOptions,
...
export function createSqliteAcpEventLedger(
  params: DaisyClawStateDatabaseOptions & LedgerOptions = {},
): AcpEventLedger {
  const normalized = normalizeLedgerOptions(params);
  ...
  const mutate = (fn: (db: DatabaseSync) => void) =>
    runDaisyClawStateWriteTransaction((database) => fn(database.db), dbOptions);
  const read = <T>(fn: (db: DatabaseSync) => T): T => fn(openDaisyClawStateDatabase(dbOptions).db);
