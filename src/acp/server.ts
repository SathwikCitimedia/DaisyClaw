#!/usr/bin/env node
/** ACP stdio server that bridges Agent Client Protocol clients to the DaisyClaw Gateway. */
...
function printHelp(): void {
  console.log(`Usage: daisyclaw acp [options]
...
if (isMainModule({ currentFile: fileURLToPath(import.meta.url) })) {
  ...
  if (argv.includes("--token") || argv.includes("--gateway-token")) {
    console.error(
      "Warning: --token can be exposed via process listings. Prefer --token-file or DAISYCLAW_GATEWAY_TOKEN.",
    );
  }
