...
import { readBool, readNonNegativeInteger, readString } from "@daisyclaw/acp-core/meta";
import { defaultAcpSessionStore, type AcpSessionStore } from "@daisyclaw/acp-core/session";
import { toAcpSessionLineageMeta } from "@daisyclaw/acp-core/session-lineage-meta";
...
/** ACP Agent implementation backed by the DaisyClaw Gateway and replay ledger. */
export class AcpGatewayAgent implements Agent {
  ...
  private assertSupportedSessionSetup(mcpServers: ReadonlyArray<unknown>): void {
    ...
    throw new Error(
      "ACP bridge mode does not support per-session MCP servers. Configure MCP on the DaisyClaw gateway or agent instead.",
    );
  }
