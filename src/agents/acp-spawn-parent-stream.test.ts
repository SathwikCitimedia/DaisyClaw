...
function progressModeConfig(acp?: DaisyClawConfig["acp"]): DaisyClawConfig {
  return {
    ...(acp ? { acp } : {}),
    channels: {
      forum: {
...
describe("startAcpSpawnParentStreamRelay", () => {
  ...
  it("resolves ACP spawn stream log path from session metadata", () => {
    readAcpSessionEntryMock.mockReturnValue({
      storePath: "/tmp/daisyclaw/agents/codex/sessions/sessions.json",
      entry: {
        sessionId: "sess-123",
        sessionFile: "/tmp/daisyclaw/agents/codex/sessions/sess-123.jsonl",
      },
    });
    resolveSessionFilePathMock.mockReturnValue(
      "/tmp/daisyclaw/agents/codex/sessions/sess-123.jsonl",
    );
    ...
    expect(resolved).toBe("/tmp/daisyclaw/agents/codex/sessions/sess-123.acp-stream.jsonl");
    ...
    expect(options.storePath).toBe("/tmp/daisyclaw/agents/codex/sessions/sessions.json");
  });
});
