...
function createDefaultSpawnConfig(): DaisyClawConfig {
  return {
    acp: {
      enabled: true,
      backend: "acpx",
...
function replaceSpawnConfig(next: DaisyClawConfig): void {
...
async function createCrossAgentWorkspaceFixture(options?: {
  ...
  const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-acp-spawn-"));
...
describe("spawnAcpDirect", () => {
  ...
  it("rejects DaisyClaw config agent ids when runtime=acp targets a native agent", async () => {
    replaceSpawnConfig({
      ...createDefaultSpawnConfig(),
      acp: {
        enabled: true,
    ...
    expect(result).toHaveProperty(
      "error",
      'agentId "pleres" is an DaisyClaw config agent, not an ACP harness. Use runtime="subagent" or omit runtime for DaisyClaw config agents. Use runtime="acp" only with external ACP harness ids such as codex, claude, droid, gemini, or opencode, or configure agents.list[].runtime.type="acp" with runtime.acp.agent.',
    );
    expect(hoisted.initializeSessionMock).not.toHaveBeenCalled();
  ...
  it("maps DaisyClaw ACP runtime agent aliases to their configured harness id", async () => {
    replaceSpawnConfig({
      ...createDefaultSpawnConfig(),
      agents: {
        list: [
