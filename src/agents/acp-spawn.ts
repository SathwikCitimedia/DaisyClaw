...
} from "@daisyclaw/acp-core/runtime/session-identifiers";
import type { AcpRuntimeSessionMode } from "@daisyclaw/acp-core/runtime/types";
import {
...
function resolveHeartbeatConfigForAgent(params: {
  cfg: DaisyClawConfig;
  agentId: string;
}): NonNullable<NonNullable<DaisyClawConfig["agents"]>["defaults"]>["heartbeat"] {
  ...
  if (requested) {
    ...
    if (configuredAgent && !isExplicitlyAllowedAcpAgent(params.cfg, requested)) {
      return {
        ok: false,
        error:
          `agentId "${requested}" is an DaisyClaw config agent, not an ACP harness. ` +
          'Use runtime="subagent" or omit runtime for DaisyClaw config agents. ' +
...
function isExplicitlyAllowedAcpAgent(cfg: DaisyClawConfig, agentId: string): boolean {
...
function resolveConfiguredAcpSubagentTargetIds(cfg: DaisyClawConfig): string[] {
