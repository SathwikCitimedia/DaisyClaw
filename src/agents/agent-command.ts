...
type AcpSessionIdentifiersRuntime = typeof import("@daisyclaw/acp-core/runtime/session-identifiers");
...
async function prepareAgentCommandExecution(opts: AgentCommandOpts, runtime: RuntimeEnv) {
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("daisyclaw agents list")}" to see configured agents.`,
      );
    }
  }
