...
import type { ExtensionAPI, ExtensionContext } from "daisyclaw/plugin-sdk/agent-sessions";
...
describe("compaction-safeguard recent-turn preservation", () => {
  ...
  it("passes compaction instructions to providers and preserves suffix context", async () => {
    ...
    expect(JSON.stringify(providerMessages)).not.toContain("daisyclaw.runtime-context");
...
async function expectWorkspaceSummaryEmptyForAgentsAlias(
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-compaction-summary-"));
...
describe("readWorkspaceContextForSummary", () => {
  ...
  it("reads workspace context from the configured workspace instead of process cwd", async () => {
    const processRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-compaction-cwd-"));
    const workspaceRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-compaction-workspace-"));
