...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: DaisyClawConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/daisyclaw" }],
      },
    };
        ...
        list: [
          {
            ...
            name: "Main Agent",
            workspace: "~/daisyclaw",
            agentDir: "~/.daisyclaw/agents/main",
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: DaisyClawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: DaisyClawConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: DaisyClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStringModel: DaisyClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStrictAgentWithDefaultFallbacks: DaisyClawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgDisable: DaisyClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaultsWithoutAgentModel: DaisyClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("updates the effective model primary at the winning config layer", () => {
    ...
    const inheritedCfg: DaisyClawConfig = {
      agents: {
        defaults: {
          model: {
            primary: "openai/gpt-5.4",
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: DaisyClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: DaisyClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
    ...
    const cfg: DaisyClawConfig = {
      agents: {
        list: [
          {
            id: "restricted",
            workspace: "~/daisyclaw-restricted",
            tools: {
              allow: ["read"],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/daisyclaw");
  });

  it("uses DAISYCLAW_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "daisyclaw-home");
    withEnv({ DAISYCLAW_HOME: home }, () => {
      const workspace = resolveAgentWorkspaceDir({} as DaisyClawConfig, "main");
      expect(workspace).toBe(path.join(path.resolve(home), ".daisyclaw", "workspace"));
    });
  });

  it("uses DAISYCLAW_WORKSPACE_DIR for default agent workspace", () => {
    const workspaceDir = path.join(path.sep, "srv", "daisyclaw-workspace");
    withEnv(
      {
        DAISYCLAW_WORKSPACE_DIR: workspaceDir,
        DAISYCLAW_HOME: path.join(path.sep, "srv", "daisyclaw-home"),
      },
      () => {
        const workspace = resolveAgentWorkspaceDir({} as DaisyClawConfig, "main");
  ...
  it("uses DAISYCLAW_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "daisyclaw-home");
    withEnv({ DAISYCLAW_HOME: home, DAISYCLAW_STATE_DIR: "" }, () => {
      const agentDir = resolveAgentDir({} as DaisyClawConfig, "main");
      expect(agentDir).toBe(path.join(path.resolve(home), ".daisyclaw", "agents", "main", "agent"));
    });
  });
  ...
  it("resolves default agentDir from the configured default agent", () => {
    ...
    const agentDir = withEnv({ DAISYCLAW_STATE_DIR: stateDir }, () => resolveDefaultAgentDir(cfg));
  ...
  it("non-default agent without defaults.workspace falls back to stateDir", () => {
    ...
    const workspace = withEnv({ DAISYCLAW_STATE_DIR: stateDir }, () =>
...
describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/daisyclaw-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/daisyclaw-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-agent-scope-"));
