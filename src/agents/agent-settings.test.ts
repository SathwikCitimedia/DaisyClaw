...
describe("isSilentOverflowProneModel", () => {
  ...
  // daisyclaw#75799 reporter's setup: an OpenAI-compatible in-house gateway
  ...
  // string. False positives only disable DaisyClaw runtime's secondary compaction path;
  // DaisyClaw's preemptive compaction continues to handle real overflow.
...
describe("applyAgentAutoCompactionGuard", () => {
  // Direct repro of daisyclaw#75799: shared model runtime's silent-overflow detection misfires
  // on a successful turn against z.ai-style providers, triggering DaisyClaw runtime's
  ...
  // state.messages intact; DaisyClaw's preemptive compaction continues to
  ...
  // Default-mode runs against ordinary providers must keep DaisyClaw runtime's auto-compaction
  // enabled. Disabling it across the board would silently remove DaisyClaw runtime's
