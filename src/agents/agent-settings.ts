...
export function resolveCompactionReserveTokensFloor(cfg?: DaisyClawConfig): number {
...
export function resolveEffectiveCompactionMode(cfg?: DaisyClawConfig): AgentCompactionMode {
 ...
 * fires on a successful turn and triggers DaisyClaw runtime's `_runAutoCompaction` from
 * inside `Session.prompt()`, collapsing `agent.state.messages` before the
 * provider call (daisyclaw#75799).
 *
...
/**
 * Disable DaisyClaw runtime's `_checkCompaction → _runAutoCompaction` (which would otherwise
 * fire from inside `Session.prompt()` and reassign `agent.state.messages`
 * before the provider call) when DaisyClaw or a plugin owns compaction:
 * `contextEngineInfo.ownsCompaction === true`, effective safeguard compaction,
 * or an active model that is silent-overflow-prone (daisyclaw#75799).
 * Default-mode runs against ordinary providers keep DaisyClaw runtime's auto-compaction as
 ...
 * rehydrates `compaction.enabled` from disk and silently restores DaisyClaw runtime's
