...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-agent-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: DaisyClawConfig) {
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<DaisyClawConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<DaisyClawConfig["agents"]>["list"]>[number]["tools"];
  }): DaisyClawConfig {
    return {
      tools: params.tools,
      agents: {
