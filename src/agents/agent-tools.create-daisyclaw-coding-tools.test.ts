/**
 * Broad coverage for createDaisyClawCodingTools.
...
import "./test-helpers/fast-daisyclaw-tools.js";
import { createDaisyClawCodingTools } from "./agent-tools.js";
import type { AuthProfileStore } from "./auth-profiles/types.js";
import * as daisyClawPluginTools from "./daisyclaw-plugin-tools.js";
import { createDaisyClawTools } from "./daisyclaw-tools.js";
...
function createToolsForStoredSession(storeTemplate: string, sessionKey: string) {
  return createDaisyClawCodingTools({
    sessionKey,
    config: {
      session: {
        store: storeTemplate,
...
function expectNoSubagentControlTools(tools: ReturnType<typeof createDaisyClawCodingTools>) {
...
type DaisyClawCodingTool = ReturnType<typeof createDaisyClawCodingTools>[number];
type DaisyClawToolsOptions = NonNullable<Parameters<typeof createDaisyClawTools>[0]>;

function toolNameList(tools: readonly { name: string }[]): string[] {
  return tools.map((tool) => tool.name);
}

function requireTool(tools: DaisyClawCodingTool[], name: string): DaisyClawCodingTool {
...
function requireToolExecute(tool: DaisyClawCodingTool): NonNullable<DaisyClawCodingTool["execute"]> {
  if (!tool.execute) {
...
function latestCreateDaisyClawToolsOptions(): DaisyClawToolsOptions {
  const calls = vi.mocked(createDaisyClawTools).mock.calls;
  const lastCall = calls.at(-1);
  const options = lastCall?.[0];
  if (!options) {
    throw new Error("expected createDaisyClawTools call");
  }
  return options;
}
...
describe("createDaisyClawCodingTools", () => {
  const testConfig: DaisyClawConfig = {};
  ...
  it("exposes gateway config and restart actions to owner sessions", () => {
    const tools = createDaisyClawCodingTools({ config: testConfig });
  ...
  it("passes explicit hook channel ids to wrapped tool hooks", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-hook-channel-"));
  ...
  it("keeps Tool Search controls when core DaisyClaw tools are not materialized", () => {
    const createDaisyClawToolsMock = vi.mocked(createDaisyClawTools);
    createDaisyClawToolsMock.mockClear();
    ...
    expect(createDaisyClawToolsMock).not.toHaveBeenCalled();
    expect(names.has("tool_search_code")).toBe(true);
    expect(names.has("tool_search")).toBe(true);
    expect(names.has("tool_describe")).toBe(true);
    expect(names.has("tool_call")).toBe(true);
  ...
  it("uses runtime toolsAllow when materializing plugin tools", () => {
    ...
    expect(createDaisyClawToolsMock).toHaveBeenCalledTimes(1);
    const options = latestCreateDaisyClawToolsOptions();
  ...
  it("preserves runtime allowlist groups containing message through restrictive profiles", () => {
    for (const runtimeToolAllowlist of [["group:messaging"], ["group:daisyclaw"], ["*"]]) {
      const tools = createDaisyClawCodingTools({
        config: { tools: { profile: "minimal" } },
  ...
  it("passes source reply delivery mode to DaisyClaw tool construction", () => {
    const createDaisyClawToolsMock = vi.mocked(createDaisyClawTools);
    createDaisyClawToolsMock.mockClear();
    ...
    expect(createDaisyClawToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateDaisyClawToolsOptions().sourceReplyDeliveryMode).toBe("message_tool_only");
  });
  ...
  it("passes plugin suppression into DaisyClaw tool construction plans", () => {
    const createDaisyClawToolsMock = vi.mocked(createDaisyClawTools);
    createDaisyClawToolsMock.mockClear();
    ...
    expect(createDaisyClawToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateDaisyClawToolsOptions().disablePluginTools).toBe(true);
  });

  it("keeps plugin-only construction off the DaisyClaw core factory", () => {
    const createDaisyClawToolsMock = vi.mocked(createDaisyClawTools);
    createDaisyClawToolsMock.mockClear();
  ...
  it("forwards active model metadata to plugin-only tool construction", () => {
    ...
    const resolvePluginToolsSpy = vi
      .spyOn(daisyClawPluginTools, "resolveDaisyClawPluginToolsForOptions")
      .mockReturnValue([]);
  ...
  it("uses tools.alsoAllow for optional plugin discovery without widening to all plugins", () => {
    ...
    expect(createDaisyClawToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateDaisyClawToolsOptions().pluginToolAllowlist).toStrictEqual([
      "lobster",
      DEFAULT_PLUGIN_TOOLS_ALLOWLIST_ENTRY,
    ]);
  });

  it("passes explicit denylist entries to DaisyClaw tool factory planning", () => {
    const createDaisyClawToolsMock = vi.mocked(createDaisyClawTools);
    createDaisyClawToolsMock.mockClear();
    ...
    expect(createDaisyClawToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateDaisyClawToolsOptions().pluginToolDenylist, ["pdf"]);
  });

  it("passes inherited allowlist entries to DaisyClaw plugin discovery", async () => {
    const createDaisyClawToolsMock = vi.mocked(createDaisyClawTools);
    createDaisyClawToolsMock.mockClear();
    const agentId = `inherited-allow-${Date.now()}-${Math.random().toString(16).slice(2)}`;
    const storeTemplate = path.join(
      os.tmpdir(),
      `daisyclaw-session-store-${agentId}-{agentId}.json`,
    );
    ...
    expect(createDaisyClawToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateDaisyClawToolsOptions().pluginToolAllowlist, [
      "custom_plugin_tool",
      "sessions_spawn",
    ]);
  });
  ...
  it("passes effective allow-list-restricted tool surface to spawned sessions", () => {
    ...
    expect(createDaisyClawToolsMock).toHaveBeenCalledTimes(1);
    const inheritedAllow = latestCreateDaisyClawToolsOptions().inheritedToolAllowlist;
  ...
  it("records core tool-prep stages for hot-path diagnostics", () => {
    ...
    expect(stages.indexOf("daisyclaw-tools:test-helper")).toBeLessThan(
      stages.indexOf("daisyclaw-tools"),
    );
    expect(stages.indexOf("schema-normalization")).toBeLessThan(stages.indexOf("tool-hooks"));
  });

  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createDaisyClawCodingTools({ config: testConfig });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createDaisyClawCodingTools({
      config: testConfig,
    ...
    const codexTools = createDaisyClawCodingTools({
      config: testConfig,
      modelProvider: "openai",
      modelId: "gpt-5.4",
    });
    ...
    const disabledConfig: DaisyClawConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createDaisyClawCodingTools({
    ...
    const anthropicTools = createDaisyClawCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: DaisyClawConfig = {
      tools: {
        exec: {
    ...
    const allowed = createDaisyClawCodingTools({
    ...
    const denied = createDaisyClawCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
    ...
    const oauthTools = createDaisyClawCodingTools({
      config: testConfig,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createDaisyClawCodingTools({ messageProvider: "discord" });
    const names = new Set(tools.map((tool) => tool.name));
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-depth-policy-"));
    try {
  ...
  it("applies subagent tool policy to ACP children spawned under a subagent envelope", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-acp-subagent-policy-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("applies leaf tool policy for cross-agent subagent sessions when spawnDepth is missing", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-cross-agent-subagent-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("keeps browser out of coding-profile subagents unless profile-stage alsoAllow adds it", () => {
    ...
    const codingSubagent = createDaisyClawCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: baseConfig,
    });
    ...
    const subagentAllowOnly = createDaisyClawCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
        tools: {
    ...
    const profileStageAlsoAllow = createDaisyClawCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
  ...
  it("can keep message available when a cron route needs it under the coding profile", () => {
    const codingTools = createDaisyClawCodingTools({
      config: { tools: { profile: "coding" } },
    });
    expect(toolNameList(codingTools)).not.toContain("message");

    const cronTools = createDaisyClawCodingTools({
      config: { tools: { profile: "coding" } },
      forceMessageTool: true,
    });
  ...
  it("can keep message available when a cron route needs it under a provider coding profile", () => {
    const providerProfileTools = createDaisyClawCodingTools({
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createDaisyClawCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createDaisyClawCodingTools({
      modelProvider: "xai",
      modelCompat: {
  ...
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    const defaultTools = createDaisyClawCodingTools();
    const readTool = requireTool(defaultTools, "read");
    const readExecute = requireToolExecute(readTool);

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-read-"));
    try {
      ...
      const contents = "Hello from daisyclaw read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "daisyclaw-sandbox");
    const sandbox = createAgentToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "daisyclaw-workspace"),
    ...
    const tools = createDaisyClawCodingTools({ sandbox });
  ...
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-canonical-"));
    try {
      const tools = createDaisyClawCodingTools({ workspaceDir: tmpDir });
  ...
  it("roots memory flush append-only writes in the workspace when cwd differs", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-memory-workspace-"));
    const taskCwd = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-memory-cwd-"));
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-legacy-alias-"));
    try {
      const tools = createDaisyClawCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("rejects structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-structured-write-"));
    try {
      const tools = createDaisyClawCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects structured edit payloads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-structured-edit-"));
    try {
