...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("daisyclaw-tilde-test-edit-");
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("daisyclaw-tilde-test-write-");
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("daisyclaw-tilde-test-mkdir-");
  ...
  it("ignores DAISYCLAW_HOME for write operations", async () => {
    const daisyclawHome = await createTempDir("daisyclaw-home-override-", os.tmpdir());
    const dir = await createTempDir("daisyclaw-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");
    vi.stubEnv("DAISYCLAW_HOME", daisyclawHome);

    createHostWorkspaceWriteTool(daisyclawHome, { workspaceOnly: false });
    ...
    await expectMissingPath(fs.access(path.join(daisyclawHome, path.basename(testFile))));
  });

  it("ignores DAISYCLAW_HOME for mkdir operations", async () => {
    const daisyclawHome = await createTempDir("daisyclaw-home-override-", os.tmpdir());
    const dir = await createTempDir("daisyclaw-tilde-test-mkdir-");
    ...
    expect((await fs.stat(newDir)).isDirectory()).toBe(true);
    await expectMissingPath(fs.access(path.join(daisyclawHome, path.basename(newDir))));
  });

  it("ignores DAISYCLAW_HOME for readFile operations", async () => {
    const daisyclawHome = await createTempDir("daisyclaw-home-override-", os.tmpdir());
    const dir = await createTempDir("daisyclaw-tilde-test-edit-");
    ...
    createHostWorkspaceEditTool(daisyclawHome, { workspaceOnly: false });
    const content = await readEditOps().readFile(toTildePath(testFile));
  ...
  it("ignores DAISYCLAW_HOME for access operations", async () => {
    const daisyclawHome = await createTempDir("daisyclaw-home-override-", os.tmpdir());
    const dir = await createTempDir("daisyclaw-tilde-test-edit-");
