...
function resolveAdaptiveReadMaxBytes(options?: DaisyClawReadToolOptions): number {
...
function withWorkspaceSafeTempHint(error: unknown): unknown {
  ...
  const message = error.message.includes(".daisyclaw/tmp/")
    ? error.message
    : `${error.message}. Use a relative path under \`.daisyclaw/tmp/\` inside the workspace for scratch/temp/meta files that file tools need to read or write later.`;
...
/** Create a sandbox-backed read tool with DaisyClaw result normalization. */
export function createSandboxedReadTool(params: SandboxToolParams) {
  const base = createReadTool(params.root, {
    operations: createSandboxReadOperations(params),
  }) as unknown as AnyAgentTool;
  return createDaisyClawReadTool(base, {
...
/** Wrap the base read tool with DaisyClaw paging, MIME, and image handling. */
export function createDaisyClawReadTool(
  base: AnyAgentTool,
  options?: DaisyClawReadToolOptions,
): AnyAgentTool {
  return {
    ...base,
