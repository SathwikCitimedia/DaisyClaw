...
let createDaisyClawCodingTools: typeof import("./agent-tools.js").createDaisyClawCodingTools;
...
beforeAll(async () => {
  await withEnvAsync(
    {
      DAISYCLAW_BUNDLED_PLUGINS_DIR: path.join(os.tmpdir(), "daisyclaw-test-no-bundled-extensions"),
    },
    async () => {
      ({ createDaisyClawCodingTools } = await import("./agent-tools.js"));
    },
  );
});
...
vi.mock("daisyclaw/plugin-sdk/agent-sessions", () => ({
  AuthStorage: vi.fn(),
...
async function withSafeBinsExecTool(
  ...
  try {
    await withEnvAsync(
      {
        DAISYCLAW_SHELL_ENV_TIMEOUT_MS: "1",
        PATH: "/usr/bin:/bin",
        SHELL: "/bin/sh",
      },
      async () => {
...
describe("createDaisyClawCodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "daisyclaw-safe-bins-",
        safeBins: ["echo"],
        safeBinProfiles: {
  ...
  it("rejects unprofiled custom safe-bin entries", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "daisyclaw-safe-bins-unprofiled-",
        safeBins: ["echo"],
      },
      async ({ tmpDir, execTool }) => {
        await expect(
  ...
  it("does not allow env var expansion to smuggle file args via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "daisyclaw-safe-bins-expand-",
        safeBins: ["head", "wc"],
  ...
  it("blocks sort output/compress bypass attempts in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "daisyclaw-safe-bins-sort-",
        safeBins: ["sort"],
  ...
  it("blocks shell redirection metacharacters in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "daisyclaw-safe-bins-redirect-",
        safeBins: ["head"],
  ...
  it("blocks grep recursive flags from reading cwd via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "daisyclaw-safe-bins-grep-",
        safeBins: ["grep"],
