/**
 * Builds the effective DaisyClaw agent tool surface.
 * Assembles core, shell, channel, DaisyClaw, plugin, and Tool Search tools, then
...
function resolveExecConfig(params: { cfg?: DaisyClawConfig; agentId?: string }) {
...
export type DaisyClawCodingToolConstructionPlan = {
  includeBaseCodingTools: boolean;
  includeShellTools: boolean;
  includeChannelTools: boolean;
  includeDaisyClawTools: boolean;
  includePluginTools: boolean;
};

/** Build the runtime tool list for one agent run. */
export function createDaisyClawCodingTools(options?: {
  agentId?: string;
  ...
  modelCompat?: ModelCompatConfig;
  /** If false, keep DaisyClaw web_search even when a provider-native search tool is active. */
  suppressManagedWebSearch?: boolean;
  /**
  ...
  toolConstructionPlan?: DaisyClawCodingToolConstructionPlan;
  ...
  const toolConstructionPlan = options?.toolConstructionPlan ?? {
    ...
    includeDaisyClawTools: includeCoreTools,
  ...
  const includeDaisyClawTools = includeCoreTools && toolConstructionPlan.includeDaisyClawTools;
  ...
  if (includeBaseCodingTools) {
    for (const tool of createCodingTools(codingRoot) as unknown as AnyAgentTool[]) {
      if (tool.name === "read") {
        ...
        const wrapped = createDaisyClawReadTool(freshReadTool, {
          modelContextWindowTokens: options?.modelContextWindowTokens,
          imageSanitization,
        });
        base.push(
  ...
  const pluginToolsOnly =
    includeDaisyClawTools || !includePluginTools
      ? []
  ...
  options?.recordToolPrepStage?.("daisyclaw-tools");
  ...
  // Always normalize tool JSON Schemas before handing them to DaisyClaw model runtime.
