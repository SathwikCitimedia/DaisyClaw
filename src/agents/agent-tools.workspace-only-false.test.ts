...
import { createReadTool } from "daisyclaw/plugin-sdk/agent-sessions";
...
describe("FS tools with workspaceOnly=false", () => {
  ...
  const toolsFor = (workspaceOnly: boolean | undefined): AnyAgentTool[] => {
    const read = createDaisyClawReadTool(createReadTool(workspaceDir) as unknown as AnyAgentTool);
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-test-"));
  ...
  it("restricts memory-triggered writes to append-only canonical memory files", async () => {
    ...
    const tools = [
      createDaisyClawReadTool(createReadTool(workspaceDir) as unknown as AnyAgentTool),
