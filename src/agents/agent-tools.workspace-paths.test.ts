...
describe("workspace path resolution", () => {
  it("uses cwd for coding filesystem tools while workspaceDir remains the agent workspace", async () => {
    await withTempDir("daisyclaw-agent-ws-", async (workspaceDir) => {
      await withTempDir("daisyclaw-task-cwd-", async (cwd) => {
        const tools = createDaisyClawCodingTools({ workspaceDir, cwd });
  ...
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("daisyclaw-ws-", async (workspaceDir) => {
      await withTempDir("daisyclaw-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createDaisyClawCodingTools({ workspaceDir });
          ...
          await editTool.execute("ws-edit", {
            path: editFile,
            edits: [{ oldText: "world", newText: "daisyclaw" }],
          });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("daisyclaw-ws-", async (workspaceDir) => {
      await withTempDir("daisyclaw-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("daisyclaw-ws-", async (workspaceDir) => {
      const cfg: DaisyClawConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createDaisyClawCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-daisyclaw.txt");
      await expect(
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("daisyclaw-ws-symlink-write-", async (workspaceDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("daisyclaw-ws-symlink-edit-", async (workspaceDir) => {
        const realDir = path.join(workspaceDir, "oc_system", "memory");
        const aliasDir = path.join(workspaceDir, "memory");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("daisyclaw-ws-symlink-escape-", async (rootDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("daisyclaw-ws-symlink-leaf-", async (workspaceDir) => {
  ...
  it("allows workspaceOnly reads for resolved skill roots without allowing other filesystem access", async () => {
    await withTempDir("daisyclaw-skill-read-", async (rootDir) => {
      const workspaceDir = path.join(rootDir, "workspace");
  ...
  it("rejects symlink escapes inside resolved skill roots", async () => {
    ...
    await withTempDir("daisyclaw-skill-read-symlink-", async (rootDir) => {
      const workspaceDir = path.join(rootDir, "workspace");
      const skillDir = path.join(rootDir, "global-skills", "demo");
      await fs.mkdir(workspaceDir, { recursive: true });
      await fs.mkdir(skillDir, { recursive: true });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("daisyclaw-sandbox-", async (sandboxDir) => {
      await withTempDir("daisyclaw-workspace-", async (workspaceDir) => {
        ...
        const tools = createDaisyClawCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
