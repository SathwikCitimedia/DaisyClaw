...
describe("anthropic transport stream", () => {
  ...
  it("classifies malformed Anthropic SSE data as a stable transport error", async () => {
    ...
    expect(result.stopReason).toBe("error");
    expect(result.errorMessage).toBe("DaisyClaw transport error: malformed_streaming_fragment");
  });
  ...
  it("backfills MiMo v2-flash tool-use replay when DaisyClaw thinking is off", async () => {
    await runTransportStream(
      makeAnthropicTransportModel({
        id: "mimo-v2-flash",
        name: "MiMo V2 Flash",
