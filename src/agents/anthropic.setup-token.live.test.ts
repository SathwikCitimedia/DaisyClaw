...
import { completeSimple, type Model } from "daisyclaw/plugin-sdk/llm";
...
const SETUP_TOKEN_MODEL = process.env.DAISYCLAW_LIVE_SETUP_TOKEN_MODEL?.trim() ?? "";
...
async function resolveTokenSource(): Promise<TokenSource> {
  ...
  if (explicitToken) {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-setup-token-"));
  ...
  if (SETUP_TOKEN_RAW && SETUP_TOKEN_RAW !== "1" && SETUP_TOKEN_RAW !== "auto") {
    throw new Error(
      "DAISYCLAW_LIVE_SETUP_TOKEN did not look like a setup-token. Use DAISYCLAW_LIVE_SETUP_TOKEN_VALUE for raw tokens.",
    );
  }

  if (candidates.length === 0) {
    throw new Error(
      "No Anthropics setup-token profiles found. Set DAISYCLAW_LIVE_SETUP_TOKEN_VALUE or DAISYCLAW_LIVE_SETUP_TOKEN_PROFILE.",
    );
  }
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        await ensureDaisyClawModelsJson(cfg, tokenSource.agentDir);
