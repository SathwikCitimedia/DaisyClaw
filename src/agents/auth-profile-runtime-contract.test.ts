...
function providerRuntimeConfig(provider: string, runtime: string): DaisyClawConfig {
  return {
    models: {
      providers: {
        [provider]: {
          baseUrl: "https://api.daisyclaw.test/v1",
...
async function runAuthContractAttempt(params: {
  ...
  const cfg = params.cfg ?? ({} as DaisyClawConfig);
...
describe("Auth profile runtime contract - embedded DaisyClaw and CLI adapter", () => {
  let tmpDir: string;
  let storePath: string;

  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-auth-contract-"));
  ...
  it("forwards a legacy OpenAI Codex auth profile through the embedded DaisyClaw path", async () => {
    await runAuthContractAttempt({
      tmpDir,
      storePath,
  ...
  it("forwards an OpenAI auth profile through the explicit embedded OpenAI DaisyClaw path", async () => {
    await runAuthContractAttempt({
      tmpDir,
      storePath,
      ...
      authProfileProvider: AUTH_PROFILE_RUNTIME_CONTRACT.openAiProvider,
      authProfileOverride: AUTH_PROFILE_RUNTIME_CONTRACT.openAiProfileId,
      cfg: providerRuntimeConfig(AUTH_PROFILE_RUNTIME_CONTRACT.openAiProvider, "daisyclaw"),
    });
  ...
  it("routes explicit OpenAI DaisyClaw runs with legacy Codex OAuth through OpenAI transport", async () => {
    await runAuthContractAttempt({
      tmpDir,
      storePath,
      providerOverride: AUTH_PROFILE_RUNTIME_CONTRACT.openAiProvider,
