...
} from "../state/daisyclaw-agent-db.js";
...
describe("auth profile sqlite store", () => {
  ...
  it("persists auth profiles and runtime scheduling state in the agent sqlite database", async () => {
    await withAgentDirEnv("daisyclaw-auth-sqlite-", (agentDir) => {
      saveAuthProfileStore(
        {
          ...apiKeyStore("sk-test"),
      ...
      expect(fs.existsSync(path.join(agentDir, "daisyclaw-agent.sqlite"))).toBe(true);
    });
  });

  it("does not read legacy auth-profiles.json at runtime", async () => {
    await withAgentDirEnv("daisyclaw-auth-no-json-fallback-", (agentDir) => {
      fs.writeFileSync(
  ...
  it("does not create sqlite files for missing-store reads", async () => {
    await withAgentDirEnv("daisyclaw-auth-sqlite-no-create-", (agentDir) => {
      expect(loadPersistedAuthProfileStore(agentDir)).toBeNull();
      expect(fs.existsSync(path.join(agentDir, "daisyclaw-agent.sqlite"))).toBe(false);
    });
  });

  it("reads existing sqlite auth stores without registering shared state", async () => {
    await withAgentDirEnv("daisyclaw-auth-sqlite-readonly-", (agentDir) => {
      saveAuthProfileStore(apiKeyStore("sk-test"), agentDir);
      closeDaisyClawAgentDatabasesForTest();
      closeDaisyClawStateDatabaseForTest();
      const stateDbPath = resolveDaisyClawStateSqlitePath();
  ...
  it("uses the configured agent id for custom agentDir databases", async () => {
    await withAgentDirEnv("daisyclaw-auth-sqlite-custom-agent-", (envAgentDir) => {
  ...
  it("keeps SecretRef-backed credentials from persisting duplicate plaintext", async () => {
    await withAgentDirEnv("daisyclaw-auth-sqlite-secret-ref-", (agentDir) => {
      saveAuthProfileStore(
        {
          version: 1,
          profiles: {
  ...
  it("recomputes runtime-only external auth overlays from the sqlite base store", async () => {
    await withAgentDirEnv("daisyclaw-auth-sqlite-overlay-", (agentDir) => {
      saveAuthProfileStore(apiKeyStore("sk-test"), agentDir);
