...
import type { DB as DaisyClawAgentKyselyDatabase } from "../../state/daisyclaw-agent-db.generated.js";
import {
  openDaisyClawAgentDatabase,
  runDaisyClawAgentWriteTransaction,
  type DaisyClawAgentDatabase,
...
// The auth database lives in the agent dir and shares the daisyclaw-agent schema
...
function resolveAuthProfileDatabaseOptions(agentDir?: string) {
  ...
  return {
    ...
    path: path.join(dir, "daisyclaw-agent.sqlite"),
  };
}
...
export function openAuthProfileDatabase(agentDir?: string): DaisyClawAgentDatabase {
  return openDaisyClawAgentDatabase(resolveAuthProfileDatabaseOptions(agentDir));
}
...
export function readPersistedAuthProfileStoreRaw(
  agentDir?: string,
  database?: DaisyClawAgentDatabase,
): unknown {
  if (database) {
...
export function writePersistedAuthProfileStoreRaw(
  ...
  const write = (target: DaisyClawAgentDatabase) => {
  ...
  runDaisyClawAgentWriteTransaction(write, resolveAuthProfileDatabaseOptions(agentDir));
}
...
export function deletePersistedAuthProfileStoreRaw(
  ...
  const remove = (target: DaisyClawAgentDatabase) => {
    const db = getAuthProfileKysely(target.db);
    executeSqliteQuerySync(
      target.db,
  ...
  runDaisyClawAgentWriteTransaction(remove, resolveAuthProfileDatabaseOptions(agentDir));
}
...
export function runAuthProfileWriteTransaction<T>(
  ...
  return runDaisyClawAgentWriteTransaction(operation, resolveAuthProfileDatabaseOptions(agentDir));
}
