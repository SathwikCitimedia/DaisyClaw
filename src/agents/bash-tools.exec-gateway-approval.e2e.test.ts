...
describe("gateway-hosted exec approvals", () => {
  ...
  it(
    "lets DaisyClaw-style gateway tool calls request and wait for approval over separate connections",
    async () => {
      const envSnapshot = captureEnv(TEST_ENV_KEYS);
      cleanup.push(() => envSnapshot.restore());

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-exec-approval-e2e-"));
