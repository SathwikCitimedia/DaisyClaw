...
describe("processGatewayAllowlist", () => {
  ...
  it("does not require suppression edit approval for read-only suppression inspection", async () => {
    evaluateShellAllowlistMock.mockReturnValue({
      ...
      allowlistSatisfied: true,
      segments: [
        { resolution: null, argv: ["daisyclaw", "config", "get", "security.audit.suppressions"] },
      ],
      segmentAllowlistEntries: [],
    });
    resolveExecHostApprovalContextMock.mockReturnValue({
    ...
    await runGatewayAllowlist({
      command: "daisyclaw config get security.audit.suppressions",
      security: "full",
      ask: "on-miss",
    });
  ...
  it("does not require suppression edit approval for profile-scoped read-only inspection", async () => {
    evaluateShellAllowlistMock.mockReturnValue({
      ...
      segments: [
        {
          resolution: null,
          argv: ["daisyclaw", "--profile", "rescue", "config", "get", "security.audit.suppressions"],
        },
      ],
      segmentAllowlistEntries: [],
    });
    ...
    await runGatewayAllowlist({
      command: "daisyclaw --profile rescue config get security.audit.suppressions",
      security: "full",
      ask: "on-miss",
    });
  ...
  it("requires suppression edit approval when a mutating segment follows read-only inspection", async () => {
    evaluateShellAllowlistMock.mockReturnValue({
      ...
      segments: [
        ...
        {
          resolution: null,
          argv: ["daisyclaw", "config", "set", "security.audit.suppressions", "[]"],
        },
      ],
      segmentAllowlistEntries: [],
    });
    ...
    const result = await runGatewayAllowlist({
      command:
        "daisyclaw config get security.audit.suppressions; daisyclaw config set security.audit.suppressions '[]'",
      security: "full",
      ask: "on-miss",
    });
  ...
  it("requires suppression edit approval when a heredoc patch follows read-only inspection", async () => {
    evaluateShellAllowlistMock.mockReturnValue({
      ...
      segments: [
        {
          resolution: null,
          argv: ["daisyclaw", "config", "get", "security.audit.suppressions"],
        },
      ],
      segmentAllowlistEntries: [],
    });
    analyzeShellCommandMock.mockReturnValueOnce({
      ...
      segments: [
        {
          raw: "daisyclaw config get security.audit.suppressions",
          resolution: null,
          argv: ["daisyclaw", "config", "get", "security.audit.suppressions"],
        },
        {
          raw: "daisyclaw config patch --stdin <<'EOF'",
          resolution: null,
          argv: ["daisyclaw", "config", "patch", "--stdin"],
        },
      ],
    });
    resolveExecHostApprovalContextMock.mockReturnValue({
    ...
    const result = await runGatewayAllowlist({
      command: `daisyclaw config get security.audit.suppressions; daisyclaw config patch --stdin <<'EOF'
  ...
  it("keeps webchat diagnostics approvals as direct pasteable followups", async () => {
    ...
    const outcome = {
      ...
      aggregated: JSON.stringify({
        path: "/tmp/daisyclaw-diagnostics.zip",
        bytes: 1234,
        manifest: {
          generatedAt: "2026-04-28T20:58:29.311Z",
          daisyclawVersion: "2026.4.27",
          contents: [
    ...
    const approvalFollowup = vi.fn<ExecApprovalFollowupFactory>(async () =>
      [
        ...
        "Session 1",
        "Channel: telegram",
        "DaisyClaw session id: `session-1`",
        "Codex thread id: `thread-1`",
      ].join("\n"),
    );

    const result = await runGatewayAllowlist({
      command: "daisyclaw gateway diagnostics export --json",
      trigger: "diagnostics",
    ...
    expect(followupText).toContain("Path: /tmp/daisyclaw-diagnostics.zip");
