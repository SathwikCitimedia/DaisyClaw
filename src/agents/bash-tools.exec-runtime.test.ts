...
describe("runExecProcess POSIX command wrapper", () => {
  ...
  it("wraps command with PATH export if DAISYCLAW_PREPEND_PATH is present", async () => {
    ...
    expect(commandStr).toContain(
      'export PATH="${DAISYCLAW_PREPEND_PATH}${PATH:+:$PATH}"; unset DAISYCLAW_PREPEND_PATH; echo test',
    );
  });
