...
const parseDaisyClawChannelsLoginShellCommand = testing.parseDaisyClawChannelsLoginShellCommand;
...
async function expectSymlinkSwapDuringPreflightToAvoidErrors(params: {
  ...
  await withTempDir("daisyclaw-exec-preflight-open-race-", async (parent) => {
...
describe("exec interactive DaisyClaw channel login guard", () => {
  it("recognizes direct and package-runner channel login commands before execution", () => {
    expect(
      parseDaisyClawChannelsLoginShellCommand("daisyclaw channels login --channel whatsapp"),
    ).toBe(true);
    expect(
      parseDaisyClawChannelsLoginShellCommand(
        "pnpm exec daisyclaw channels login --channel whatsapp --verbose",
      ),
    ).toBe(true);
    expect(parseDaisyClawChannelsLoginShellCommand("daisyclaw channels status --deep")).toBe(false);
  });
  ...
  it("blocks interactive channel login commands from exec", async () => {
    ...
    await expect(
      tool.execute("call-daisyclaw-channel-login", {
        command: "daisyclaw channels login --channel whatsapp --verbose",
      }),
    ).rejects.toThrow(/exec cannot run interactive DaisyClaw channel login commands/);
    await expect(
      tool.execute("call-wrapped-daisyclaw-channel-login", {
        command: "sudo -u daisyclaw bash -lc 'daisyclaw channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive DaisyClaw channel login commands/);
    await expect(
      tool.execute("call-clustered-sudo-channel-login", {
        command: "sudo -EH bash -lc 'daisyclaw channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive DaisyClaw channel login commands/);
    await expect(
      tool.execute("call-deep-env-channel-login", {
        command: "env env env env env env daisyclaw channels login --channel whatsapp",
      }),
    ).rejects.toThrow(/exec cannot run interactive DaisyClaw channel login commands/);
    await expect(
      tool.execute("call-env-s-trailing-channel-login", {
        command: "env -S 'daisyclaw channels' login --channel whatsapp",
      }),
    ).rejects.toThrow(/exec cannot run interactive DaisyClaw channel login commands/);
  });
});
...
describeNonWin("exec script preflight", () => {
  it("blocks shell env var injection tokens in python scripts before execution", async () => {
    await withTempDir("daisyclaw-exec-preflight-", async (tmp) => {
  ...
  it("skips preflight file reads for script paths outside the workdir", async () => {
    await withTempDir("daisyclaw-exec-preflight-parent-", async (parent) => {
      const outsidePath = path.join(parent, "outside.js");
      const workdir = path.join(parent, "workdir");
      await fs.mkdir(workdir, { recursive: true });
  ...
  it("opens preflight script reads with O_NONBLOCK to avoid FIFO stalls", async () => {
    await withTempDir("daisyclaw-exec-preflight-nonblock-", async (tmp) => {
...
describeWin("exec script preflight on windows path syntax", () => {
  it("preserves windows-style python relative path separators during script extraction", async () => {
    await withTempDir("daisyclaw-exec-preflight-win-", async (tmp) => {
      await fs.writeFile(path.join(tmp, "bad.py"), "payload = $DM_JSON", "utf-8");
...
describe("exec interpreter heuristics ReDoS guard", () => {
  it("does not hang on long commands with VAR=value assignments and whitespace-heavy text", async () => {
    ...
    const command = `ACCESS_TOKEN=$(__daisyclaw_missing_redos_guard__)\ncat > /tmp/out.html << 'EOF'\n${htmlBlock}\nEOF`;
