...
function writeExecApprovalsFixture(root: string, file: Record<string, unknown>): void {
  const stateDir = process.env.DAISYCLAW_STATE_DIR ?? path.join(root, "state");
...
describe("exec security floor", () => {
  ...
  beforeEach(() => {
    ...
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-exec-security-floor-"));
    process.env.HOME = tempRoot;
    process.env.USERPROFILE = tempRoot;
    process.env.DAISYCLAW_HOME = tempRoot;
    process.env.DAISYCLAW_STATE_DIR = path.join(tempRoot, "state");
