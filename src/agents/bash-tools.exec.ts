...
import { DAISYCLAW_CLI_ENV_VAR } from "../infra/daisyclaw-exec-env.js";
import {
...
function filterPluginExecEnv(rawEnv: Record<string, string>): Record<string, string> | undefined {
  ...
  for (const [rawKey, value] of Object.entries(rawEnv)) {
    ...
    if (
      upperKey === "PATH" ||
      upperKey === DAISYCLAW_CLI_ENV_VAR ||
...
export function createExecTool(
  ...
  const defaultBackgroundMs = clampWithDefault(
    defaults?.backgroundMs ?? readEnvInt("DAISYCLAW_BASH_YIELD_MS", "PI_BASH_YIELD_MS"),
    10_000,
    10,
    120_000,
  );
