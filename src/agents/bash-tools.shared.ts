...
import { parseStrictInteger } from "@daisyclaw/normalization-core/number-coercion";
...
export function buildDockerExecArgs(params: {
  ...
  for (const [key, value] of Object.entries(params.env)) {
    ...
    // PATH is handled separately via DAISYCLAW_PREPEND_PATH below.
    if (key === "PATH") {
      continue;
    }
  ...
  if (hasCustomPath) {
    ...
    args.push("-e", `DAISYCLAW_PREPEND_PATH=${params.env.PATH}`);
  }
  ...
  const pathExport = hasCustomPath
    ? 'export PATH="${DAISYCLAW_PREPEND_PATH}:$PATH"; unset DAISYCLAW_PREPEND_PATH; '
    : "";
