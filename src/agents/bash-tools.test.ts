...
vi.mock("../process/supervisor/index.js", () => {
  ...
  const applySegmentShellEffects = (segment: string, env: NodeJS.ProcessEnv) => {
    if (segment === 'export PATH="${DAISYCLAW_PREPEND_PATH}${PATH:+:$PATH}"') {
      const prepend = env.DAISYCLAW_PREPEND_PATH ?? "";
      const current = readEnvPath(env);
      writeEnvPath(env, `${prepend}${current ? `:${current}` : ""}`);
      return;
    }
    if (segment === "unset DAISYCLAW_PREPEND_PATH") {
      delete env.DAISYCLAW_PREPEND_PATH;
    }
  };
...
const defaultShell = isWin
  ? undefined
  : process.env.DAISYCLAW_TEST_SHELL || resolveShellFromPath("bash") || process.env.SHELL || "sh";
...
const SHELL_ENV_KEYS = ["DAISYCLAW_EXEC_SHELL_SNAPSHOT", "SHELL"] as const;
const PATH_SHELL_ENV_KEYS = ["DAISYCLAW_EXEC_SHELL_SNAPSHOT", "PATH", "SHELL"] as const;
...
const notifyCfg = {} as DaisyClawConfig;
