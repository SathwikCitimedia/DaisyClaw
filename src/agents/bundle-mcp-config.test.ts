/** Tests merging bundled MCP defaults with DaisyClaw user MCP configuration. */
...
describe("loadMergedBundleMcpConfig", () => {
  it("lets DaisyClaw mcp.servers override bundle defaults while preserving raw transport shape", () => {
  ...
  it("maps DaisyClaw transports to downstream CLI types when requested", () => {
    expect(
  ...
  it("keeps disabled DaisyClaw MCP servers out of embedded runtimes", () => {
    const merged = loadMergedBundleMcpConfig({
      workspaceDir: "/workspace",
      cfg: {
        mcp: {
  ...
  it("lets disabled DaisyClaw MCP servers tombstone bundle defaults with the same name", () => {
    const merged = loadMergedBundleMcpConfig({
      workspaceDir: "/workspace",
      cfg: {
        mcp: {
