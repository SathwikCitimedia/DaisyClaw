...
function createTempStateDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-agent-cache-"));
}
...
describe("SQLite agent cache store", () => {
  it("stores scoped JSON values and blobs in the agent database", () => {
    const env = { DAISYCLAW_STATE_DIR: createTempStateDir() };
  ...
  it("hides invalid persisted expiries and ignores invalid clear clocks", () => {
    ...
    const database = openDaisyClawAgentDatabase({ agentId: "main", env });
    database.db
  ...
  it("honors explicit per-agent database paths", () => {
    ...
    expect(
      listDaisyClawRegisteredAgentDatabases({ env }).find((entry) => entry.path === dbPath),
    ).toMatchObject({
      agentId: "worker",
      path: dbPath,
    });
