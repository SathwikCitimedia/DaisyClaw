...
import {
  ...
  type DaisyClawAgentDatabaseOptions,
...
export type SqliteAgentCacheStoreOptions = DaisyClawAgentDatabaseOptions & {
  scope: string;
  now?: () => number;
};
...
type CacheEntriesTable = DaisyClawAgentKyselyDatabase["cache_entries"];
type AgentCacheDatabase = Pick<DaisyClawAgentKyselyDatabase, "cache_entries">;
...
function toDatabaseOptions(options: SqliteAgentCacheStoreOptions): DaisyClawAgentDatabaseOptions {
  return {
...
export function writeSqliteAgentCacheEntry(
  ...
  runDaisyClawAgentWriteTransaction((database) => {
...
export function readSqliteAgentCacheEntry(
  ...
  const scope = normalizeScope(options);
  const key = normalizeKey(options.key);
  const database = openDaisyClawAgentDatabase(toDatabaseOptions(options));
  const db = getNodeSqliteKysely<AgentCacheDatabase>(database.db);
  const row =
    executeSqliteQueryTakeFirstSync(
      database.db,
...
export function deleteSqliteAgentCacheEntry(
  ...
  const scope = normalizeScope(options);
  const key = normalizeKey(options.key);
  return runDaisyClawAgentWriteTransaction((database) => {
    const db = getNodeSqliteKysely<AgentCacheDatabase>(database.db);
...
/** Agent runtime cache store implementation backed by DaisyClaw's SQLite DB. */
