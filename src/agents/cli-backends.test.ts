...
function createClaudeCliOverrideConfig(config: CliBackendConfig): DaisyClawConfig {
  return {
    agents: {
      defaults: {
        cliBackends: {
...
describe("resolveCliBackendConfig claude-cli defaults", () => {
  it("derives bypassPermissions from DaisyClaw's default YOLO exec policy", () => {
    ...
    expect(resolved?.config.args).toContain("mcp__daisyclaw__*");
    ...
    expect(resolved?.config.resumeArgs).toContain("mcp__daisyclaw__*");
  ...
  it("keeps Claude permission mode unset when DaisyClaw exec policy is not YOLO", () => {
    const resolved = requireCliBackendConfig("claude-cli", {
