...
describe("createCliJsonlStreamingParser", () => {
  ...
  it.each(["server_tool_use", "mcp_tool_use"])("recognizes %s blocks", (type) => {
      ...
      [
        ...
        JSON.stringify({
          ...
          event: {
            type: "content_block_delta",
            index: 0,
            delta: { type: "input_json_delta", partial_json: '{"query":"daisyclaw"}' },
          },
        }),
        JSON.stringify({
          type: "stream_event",
    ...
    expect(starts).toEqual([
      { toolCallId: "toolu_hosted", name: "web_search", args: { query: "daisyclaw" } },
    ]);
  });
  ...
  it.each([
    {
      ...
      name: "web_search",
      input: { query: "daisyclaw" },
      result: [{ type: "web_search_result", title: "DaisyClaw", url: "https://example.com" }],
      isError: false,
    },
    {
