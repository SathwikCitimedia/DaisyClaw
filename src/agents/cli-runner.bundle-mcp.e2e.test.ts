...
async function createBundleMcpFixture(params: {
  ...
  process.env.DAISYCLAW_DISABLE_PERSISTED_PLUGIN_REGISTRY = "1";
  ...
  const fakeClaudePidPath = params.liveSession
    ...
    : undefined;
  const pluginRoot = path.join(tempHome, ".daisyclaw", "extensions", "bundle-probe");
...
describe("CLI bundle MCP e2e", () => {
  it(
    ...
    async () => {
      ...
      const fixture = await createBundleMcpFixture({
        tempPrefix: "daisyclaw-cli-bundle-mcp-",
      });
  ...
  it(
    ...
    async () => {
      ...
      const fixture = await createBundleMcpFixture({
        liveSession: true,
        tempPrefix: "daisyclaw-cli-live-cleanup-",
      });
      const context = await prepareBundleMcpExecutionContext({
