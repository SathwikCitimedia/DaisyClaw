...
async function withTempExecApprovalsFile(
  ...
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-cli-exec-approvals-"));
...
async function withTempDaisyClawHome(run: (home: string) => Promise<void>): Promise<void> {
  const originalDaisyClawHome = process.env.DAISYCLAW_HOME;
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-cli-home-"));
  process.env.DAISYCLAW_HOME = home;
  try {
    await run(home);
  } finally {
    if (originalDaisyClawHome === undefined) {
      delete process.env.DAISYCLAW_HOME;
    } else {
      process.env.DAISYCLAW_HOME = originalDaisyClawHome;
    }
    await fs.rm(home, { recursive: true, force: true });
  }
}
...
describe("runCliAgent spawn path", () => {
  ...
  it("includes the DaisyClaw skills prompt in CLI system prompts", () => {
  ...
  it("passes Claude system prompts through a file instead of argv", async () => {
    ...
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      ...
      expect(systemPromptPath).toContain("daisyclaw-cli-system-prompt-");
  ...
  it("passes DaisyClaw skills to Claude as a session plugin", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-cli-skills-"));
    ...
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      ...
      expect(manifest.name).toBe("daisyclaw-skills");
  ...
  it("preserves Claude resume args when building live session argv", () => {
    ...
    expect(args).not.toContain("daisyclaw-session");
...
`,
  ...
  it("does not create exec approvals file while resolving Claude live policy", async () => {
    await withTempDaisyClawHome(async (home) => {
      const approvalsPath = path.join(home, ".daisyclaw", "exec-approvals.json");
      let stdoutListener: ((chunk: string) => void) | undefined;
      const stdin = {
...
`,
  ...
  it("answers Claude live control_request can_use_tool with allow when DaisyClaw exec is YOLO despite raw --permission-mode default", async () => {
    let stdoutListener: ((chunk: string) => void) | undefined;
    const writes: string[] = [];
    const stdin = {
      write: vi.fn((data: string, cb?: (err?: Error | null) => void) => {
...
`,
    ...
    // and DaisyClaw policy is authoritative over raw Claude permission-mode
  ...
  it("restarts Claude live sessions when selected skills change", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-live-skills-"));
  ...
  it("can preserve selected clearEnv keys for live CLI backend probes", async () => {
    try {
      process.env.DAISYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV = '["SAFE_CLEAR"]';
      process.env.SAFE_CLEAR = "from-base";
      mockSuccessfulCliRun();
      await executePreparedCliRun(
        buildPreparedCliRunContext({
      ...
      delete process.env.DAISYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV;
  ...
  it("loads workspace bootstrap files into the Claude CLI system prompt", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "daisyclaw-cli-bootstrap-context-"),
    );
