/** Tests projecting DaisyClaw user MCP servers into Codex app-server config. */
...
describe("buildCodexUserMcpServersThreadConfigPatch", () => {
  it("returns undefined when cfg has no mcp.servers (regression: #80814)", () => {
    ...
    expect(buildCodexUserMcpServersThreadConfigPatch({} as DaisyClawConfig)).toBeUndefined();
    expect(
      buildCodexUserMcpServersThreadConfigPatch({ mcp: {} } as DaisyClawConfig),
    ).toBeUndefined();
    expect(
      buildCodexUserMcpServersThreadConfigPatch({ mcp: { servers: {} } } as DaisyClawConfig),
    ).toBeUndefined();
  });
  ...
  it("filters Codex-scoped user MCP servers by DaisyClaw agent id", () => {
    // Agent-scoped MCP servers should follow the active DaisyClaw agent, while
  ...
  it("omits scoped Codex MCP servers when no DaisyClaw agent id is available", () => {
    const patch = buildCodexUserMcpServersThreadConfigPatch({
      mcp: {
        servers: {
          atlas: {
