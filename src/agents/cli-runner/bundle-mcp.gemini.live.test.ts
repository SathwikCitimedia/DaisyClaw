...
const LIVE = isLiveTestEnabled(["DAISYCLAW_LIVE_CLI_MCP_GEMINI"]);
...
async function startLocalStreamableHttpMcpServer(): Promise<{
  ...
  const mcpServer = new McpServer({ name: "daisyclaw-gemini-live-probe", version: "1.0.0" });
  mcpServer.tool("daisyclaw_live_probe", "DaisyClaw Gemini MCP live probe", async () => ({
...
describeLive("Gemini CLI MCP settings smoke", () => {
  it("connects to an DaisyClaw-configured streamable-http server", async () => {
    const geminiCommand = process.env.DAISYCLAW_LIVE_GEMINI_COMMAND ?? "gemini";
    ...
    try {
      ...
      expect(output).toContain("daisyclawLiveProbe");
