...
describe("prepareCliBundleMcpConfig gemini", () => {
  it("writes Gemini system settings for bundle MCP servers", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/daisyclaw-bundle-mcp-gemini",
    ...
    expect(prepared.env?.DAISYCLAW_MCP_TOKEN).toBe("loopback-token-123");
    ...
    expect(raw.mcp?.allowed).toEqual(["daisyclaw"]);
    expect(raw.mcpServers?.daisyclaw?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(raw.mcpServers?.daisyclaw?.headers?.Authorization).toBe("Bearer loopback-token-123");
  ...
  it("translates user mcp.servers transport fields in Gemini system settings", async () => {
    ...
    expect(typeof prepared.env?.GEMINI_CLI_SYSTEM_SETTINGS_PATH).toBe("string");
    // User DaisyClaw transport names are normalized to Gemini's expected schema.
    const raw = JSON.parse(
      await fs.readFile(prepared.env?.GEMINI_CLI_SYSTEM_SETTINGS_PATH as string, "utf-8"),
    ) as {
      mcp?: { allowed?: string[] };
