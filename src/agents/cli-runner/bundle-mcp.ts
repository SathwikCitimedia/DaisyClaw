...
function normalizeDaisyClawLoopbackUrl(value: string): string {
  const match =
    /^(http:\/\/(?:127\.0\.0\.1|localhost|\[::1\])):\d+(\/mcp)$/.exec(value.trim()) ?? undefined;
  if (!match) {
    return value;
  }
  return `${match[1]}:<daisyclaw-loopback>${match[2]}`;
}

function canonicalizeBundleMcpConfigForResume(config: BundleMcpConfig): BundleMcpConfig {
  // The DaisyClaw loopback MCP port changes across runs. Replace it before
  ...
  const canonicalServers = Object.fromEntries(
    Object.entries(config.mcpServers).map(([name, server]) => {
      if (name !== "daisyclaw" || typeof server.url !== "string") {
      ...
      return [
        ...
        sortJsonValue({
          ...server,
          url: normalizeDaisyClawLoopbackUrl(server.url),
        }),
      ];
    }),
...
async function prepareModeSpecificBundleMcpConfig(params: {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-cli-mcp-"));
