/** Tests merging user DaisyClaw MCP server config into Claude bundle-MCP overlays. */
...
describe("prepareCliBundleMcpConfig user mcp.servers", () => {
  it("merges user-configured mcp.servers from DaisyClaw config", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "daisyclaw-cli-bundle-mcp-user-servers-",
    );
  ...
  it("translates DaisyClaw transport field on user mcp.servers into Claude type", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "daisyclaw-cli-bundle-mcp-user-servers-transport-",
    );
  ...
  it("preserves explicit type and still strips transport on user mcp.servers", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "daisyclaw-cli-bundle-mcp-user-servers-transport-explicit-",
    );
  ...
  it("user mcp.servers do not override the loopback additionalConfig", async () => {
    // The DaisyClaw loopback server is generated runtime state and must win over
    // user config with the same server name.
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "daisyclaw-cli-bundle-mcp-user-servers-loopback-",
    );
      ...
      additionalConfig: {
        mcpServers: {
          daisyclaw: {
            ...
            headers: { Authorization: "Bearer ${DAISYCLAW_MCP_TOKEN}" },
          },
        },
      },
    });
    ...
    expect(raw.mcpServers?.daisyclaw?.url).toBe("http://127.0.0.1:23119/mcp");
  ...
  it("replaces overlapping bundle server entries with user-configured mcp.servers", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "daisyclaw-cli-bundle-mcp-user-servers-replace-",
    );
