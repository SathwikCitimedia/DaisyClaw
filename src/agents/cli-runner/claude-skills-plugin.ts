/**
 * Materializes selected DaisyClaw skills as a temporary Claude CLI plugin.
 */
...
const DAISYCLAW_CLAUDE_PLUGIN_NAME = "daisyclaw-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredDaisyClawTmpDir(), "daisyclaw-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, DAISYCLAW_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: DAISYCLAW_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",
    description: "Session-scoped DaisyClaw skills selected for this agent run.",
    skills: "./skills",
  };
  await fs.writeFile(
