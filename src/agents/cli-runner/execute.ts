...
const CLI_BACKEND_PRESERVE_ENV = "DAISYCLAW_LIVE_CLI_BACKEND_PRESERVE_ENV";
...
function buildCliEnvMcpLog(childEnv: Record<string, string>): string {
  return [
    `token=${childEnv.DAISYCLAW_MCP_TOKEN ? "set" : "missing"}`,
    `sessionKey=${childEnv.DAISYCLAW_MCP_SESSION_KEY ? "set" : "<empty>"}`,
    `agentId=${childEnv.DAISYCLAW_MCP_AGENT_ID || "<empty>"}`,
    `accountId=${childEnv.DAISYCLAW_MCP_ACCOUNT_ID || "<empty>"}`,
    `messageChannel=${childEnv.DAISYCLAW_MCP_MESSAGE_CHANNEL || "<empty>"}`,
  ].join(" ");
}
...
export async function executePreparedCliRun(
  ...
  const basePrompt = cliSessionIdToUse
    ? params.prompt
    : (context.daisyClawHistoryPrompt ?? params.prompt);
      ...
      try {
        cliBackendLog.info(
          buildCliExecLogLine({
            ...
            hasHistoryPrompt: Boolean(context.daisyClawHistoryPrompt),
          }),
        );
        const logOutputText =
        ...
        if (logOutputText) {
          ...
          if (env.DAISYCLAW_MCP_TOKEN || env.DAISYCLAW_MCP_SESSION_KEY || env.DAISYCLAW_MCP_AGENT_ID) {
        ...
        if (result.exitCode !== 0 || result.reason !== "exit") {
          if (result.reason === "no-output-timeout" || result.noOutputTimedOut) {
            ...
            const deferWatchdogNoticeForFreshRetry =
              ...
              Boolean(context.daisyClawHistoryPrompt) &&
