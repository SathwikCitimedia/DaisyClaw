...
import { extensionForMime } from "@daisyclaw/media-core/mime";
import {
  normalizeLowercaseStringOrEmpty,
  normalizeOptionalLowercaseString,
} from "@daisyclaw/normalization-core/string-coerce";
import { KeyedAsyncQueue } from "daisyclaw/plugin-sdk/keyed-async-queue";
...
function resolveCliImagePath(image: ImageContent): string {
  ...
  const digest = crypto
    ...
    .update(image.data)
    .digest("hex");
  return path.join(resolvePreferredDaisyClawTmpDir(), "daisyclaw-cli-images", `${digest}${ext}`);
}
...
function resolveCliImageRoot(params: { backend: CliBackendConfig; workspaceDir: string }): string {
  if (params.backend.imagePathScope === "workspace") {
    return path.join(params.workspaceDir, ".daisyclaw-cli-images");
  }
  return path.join(resolvePreferredDaisyClawTmpDir(), "daisyclaw-cli-images");
}
...
export async function writeCliSystemPromptFile(params: {
  ...
  const workspace = await tempWorkspace({
    ...
    prefix: "daisyclaw-cli-system-prompt-",
  });
