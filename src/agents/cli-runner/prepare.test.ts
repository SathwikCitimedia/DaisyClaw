...
function createSessionFile() {
  // Prepare tests use canonical DaisyClaw session paths because several cases
  // assert that external or stale transcript paths are ignored.
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-cli-prepare-"));
  sessionFileEnvSnapshot ??= captureEnv(["DAISYCLAW_STATE_DIR"]);
  setTestEnvValue("DAISYCLAW_STATE_DIR", dir);
...
describe("shouldSkipLocalCliCredentialEpoch", () => {
  beforeEach(() => {
    ...
    setCliRunnerPrepareTestDeps({
      ...
      resolveDaisyClawReferencePaths: vi.fn(async () => ({ docsPath: null, sourcePath: null })),
  ...
  it("uses compact current-turn context when a room event resumes a CLI session", async () => {
    ...
    try {
      ...
      // richer room context in DaisyClaw history for reseed and audits.
      const context = await prepareCliRunContext({
        sessionId: "session-test",
        sessionKey: "agent:main:test",
        agentId: "main",
      ...
      expect(context.daisyClawHistoryPrompt).toContain("Room context:\nAlice: lunch?");
      expect(context.daisyClawHistoryPrompt).toContain("Current event:\nBob: yes");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  it("does not allocate a non-legacy context engine before fallible CLI preparation finishes", async () => {
    ...
    setCliRunnerPrepareTestDeps({
      resolveDaisyClawReferencePaths: vi.fn(async () => {
        ...
        info: {
          ...
          hostRequirements: {
            "agent-run": {
              ...
              unsupportedMessage: "Use the native Codex or DaisyClaw embedded runtime.",
            },
          },
        },
        ingest: vi.fn(async () => ({ ingested: true })),
  ...
  it("uses cwd for CLI system prompt workspace guidance", async () => {
    const { dir, sessionFile } = createSessionFile();
    const taskDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-cli-task-"));
    try {
      const context = await prepareCliRunContext({
        sessionId: "session-test",
        sessionFile,
  ...
  it("prepares raw-tail history for safe invalidations only when the backend opts in", async () => {
    ...
    try {
      ...
      expect(context.daisyClawHistoryPrompt).toContain("prior no-compaction ask");
      expect(context.daisyClawHistoryPrompt).toContain("latest ask");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  it("prepares opted-in raw-tail history for session-expired retry without disabling native resume", async () => {
    ...
    try {
      ...
      expect(context.reusableCliSession).toEqual({ sessionId: "cli-session" });
      expect(context.daisyClawHistoryPrompt).toContain("prior resumable ask");
      expect(context.daisyClawHistoryPrompt).toContain("latest ask");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  ...
  it("passes current turn kind into bundle MCP loopback env", async () => {
    ...
    try {
      ...
      expect(context.preparedBackend.env).toMatchObject({
        DAISYCLAW_MCP_MESSAGE_CHANNEL: "telegram",
        DAISYCLAW_MCP_CURRENT_CHANNEL_ID: "telegram:-100123:topic:42",
        DAISYCLAW_MCP_CURRENT_THREAD_TS: "42",
        DAISYCLAW_MCP_CURRENT_MESSAGE_ID: "reply-message-1",
        DAISYCLAW_MCP_CURRENT_INBOUND_AUDIO: "true",
        DAISYCLAW_MCP_INBOUND_EVENT_KIND: "room_event",
        DAISYCLAW_MCP_SOURCE_REPLY_DELIVERY_MODE: "message_tool_only",
      });
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  ...
  it("omits Claude CLI prompt skills when the native skills plugin can carry them", async () => {
    ...
    try {
      ...
      setCliRunnerPrepareTestDeps({
        prepareClaudeCliSkillsPlugin: vi.fn(async () => ({
          args: ["--plugin-dir", path.join(dir, "daisyclaw-skills")],
          cleanup: vi.fn(async () => undefined),
          pluginDir: path.join(dir, "daisyclaw-skills"),
        })),
      });
      ...
      expect(context.claudeSkillsPluginArgs).toEqual([
        "--plugin-dir",
        path.join(dir, "daisyclaw-skills"),
      ]);
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  ...
  it("uses a larger automatic reseed history cap for Claude CLI", async () => {
    ...
    try {
      ...
      expect(context.daisyClawHistoryPrompt).toBeDefined();
      expect(context.daisyClawHistoryPrompt).toContain(summaryMarker);
      expect(context.daisyClawHistoryPrompt).not.toContain("DaisyClaw reseed history truncated");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  it("keeps the default reseed history cap for non-Claude CLI backends", async () => {
    ...
    try {
      ...
      expect(context.daisyClawHistoryPrompt).toBeDefined();
      expect(context.daisyClawHistoryPrompt).toContain("DaisyClaw reseed history truncated");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  it("uses the automatic Claude CLI cap through the raw-tail reseed path", async () => {
    ...
    try {
      ...
      expect(context.reusableCliSession).toEqual({ sessionId: "cli-session" });
      expect(context.daisyClawHistoryPrompt).toBeDefined();
      expect(context.daisyClawHistoryPrompt).toContain(recentMarker);
      expect(context.daisyClawHistoryPrompt).toContain("EARLIEST_USER");
      expect(context.daisyClawHistoryPrompt).not.toContain("DaisyClaw reseed history truncated");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
