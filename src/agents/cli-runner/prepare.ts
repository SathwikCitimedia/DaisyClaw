...
const prepareDeps = {
  ...
  resolveDaisyClawReferencePaths: async (
    params: Parameters<typeof import("../docs-path.js").resolveDaisyClawReferencePaths>[0],
  ) => (await import("../docs-path.js")).resolveDaisyClawReferencePaths(params),
...
export async function prepareCliRunContext(
  ...
  const preparedBackend = await prepareCliBundleMcpConfig({
    ...
    env: mcpLoopbackRuntime
      ? {
          DAISYCLAW_MCP_TOKEN: prepareDeps.resolveMcpLoopbackBearerToken(
            mcpLoopbackRuntime,
            params.senderIsOwner === true,
          ),
          DAISYCLAW_MCP_AGENT_ID: sessionAgentId ?? "",
          DAISYCLAW_MCP_ACCOUNT_ID: params.agentAccountId ?? "",
          DAISYCLAW_MCP_SESSION_KEY: params.sessionKey ?? "",
          DAISYCLAW_MCP_MESSAGE_CHANNEL: params.messageChannel ?? params.messageProvider ?? "",
          DAISYCLAW_MCP_CURRENT_CHANNEL_ID: params.currentChannelId ?? "",
          DAISYCLAW_MCP_CURRENT_THREAD_TS: params.currentThreadTs ?? "",
          DAISYCLAW_MCP_CURRENT_MESSAGE_ID:
            params.currentMessageId != null ? String(params.currentMessageId) : "",
          DAISYCLAW_MCP_CURRENT_INBOUND_AUDIO: params.currentInboundAudio === true ? "true" : "",
          DAISYCLAW_MCP_INBOUND_EVENT_KIND: params.currentInboundEventKind ?? "",
          DAISYCLAW_MCP_SOURCE_REPLY_DELIVERY_MODE: params.sourceReplyDeliveryMode ?? "",
        }
      : undefined,
    warn: (message) => cliBackendLog.warn(message),
  });
  ...
  let daisyClawHistoryMessages: unknown[] | undefined;
  const loadDaisyClawHistoryMessages = async () => {
    daisyClawHistoryMessages ??= await loadCliSessionHistoryMessages({
      sessionId: params.sessionId,
    ...
    return daisyClawHistoryMessages;
  };
  ...
  const daisyClawReferences = await prepareDeps.resolveDaisyClawReferencePaths({
    workspaceDir,
  ...
  const builtSystemPrompt = buildCliAgentSystemPrompt({
    ...
    heartbeatPrompt,
    docsPath: daisyClawReferences.docsPath ?? undefined,
    sourcePath: daisyClawReferences.sourcePath ?? undefined,
  ...
  try {
    const hookResult = await resolvePromptBuildHookResult({
      ...
      messages: await loadDaisyClawHistoryMessages(),
      hookCtx: {
        runId: params.runId,
        agentId: sessionAgentId,
        sessionKey: params.sessionKey,
  ...
  const shouldPrepareDaisyClawHistoryPrompt =
    !reusableCliSession.sessionId || allowRawTranscriptReseed;
  const daisyClawHistoryPrompt = shouldPrepareDaisyClawHistoryPrompt
    ? buildCliSessionHistoryPrompt({
  ...
  try {
    ...
    return {
      ...
      ...(daisyClawHistoryPrompt ? { daisyClawHistoryPrompt } : {}),
      heartbeatPrompt,
      authEpoch,
      authEpochVersion: CLI_AUTH_EPOCH_VERSION,
      extraSystemPromptHash,
