...
function createSessionTranscript(params: {
  ...
  // same JSONL record order used by persisted DaisyClaw sessions.
  const sessionFile =
    params.filePath ??
    path.join(
      params.rootDir,
...
async function withCliSessionState<T>(stateDir: string, run: () => Promise<T>): Promise<T> {
  return await withEnvAsync({ DAISYCLAW_STATE_DIR: stateDir }, run);
}
...
describe("loadCliSessionHistoryMessages", () => {
  it("reads the canonical session transcript instead of an arbitrary external path", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-cli-state-"));
    const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-cli-outside-"));
  ...
  it("honors custom session store roots when resolving hook history transcripts", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-cli-state-"));
    const customStoreDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-cli-store-"));
...
describe("buildCliSessionHistoryPrompt", () => {
  it("renders DaisyClaw transcript history around the next user message", () => {
  ...
  it("caps rendered reseed history before adding the next user message", () => {
    ...
    expect(prompt).toContain("[DaisyClaw reseed history truncated; older turns dropped]");
