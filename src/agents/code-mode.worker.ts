...
(() => {
  ...
  const catalog = Array.isArray(globalThis.__daisyclawCatalog) ? globalThis.__daisyclawCatalog : [];
  const apiFiles = Array.isArray(globalThis.__daisyclawApiFiles) ? globalThis.__daisyclawApiFiles : [];
  const namespaceDescriptors = Array.isArray(globalThis.__daisyclawNamespaces) ? globalThis.__daisyclawNamespaces : [];
  ...
  function request(method, args) {
    const id = String(globalThis.__daisyclawHostRequest(String(method), JSON.stringify(safe(args ?? []))));
  ...
  Object.defineProperties(globalThis, {
    ...
    __daisyclawSettleBridge: { value: settle },
    __daisyclawTakeOutput: { value: () => output.splice(0) },
  });
})();
`;

function buildUserSource(code: string): string {
  return `globalThis.__daisyclawResult = (async () => {\n${code}\n})()`;
}
...
async function createVm(params: {
  ...
  try {
    vm.setProp(vm.global, "__daisyclawHostRequest", hostRequest);
  } finally {
    hostRequest.dispose();
  }
  vm.evalCode(CONTROLLER_SOURCE, "daisyclaw-code-mode:controller.js").dispose();
...
function takeOutput(vm: QuickJS): unknown[] {
  const take = vm.global.getProp("__daisyclawTakeOutput");
  try {
...
function getResultHandle(vm: QuickJS): JSValueHandle {
  return vm.global.getProp("__daisyclawResult");
}
...
async function runResume(input: Extract<CodeModeWorkerInput, { kind: "resume" }>) {
  ...
  let output: unknown[] = [];
  try {
    const settle = vm.global.getProp("__daisyclawSettleBridge");
    try {
