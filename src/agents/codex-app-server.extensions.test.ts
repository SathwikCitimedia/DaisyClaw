...
function createTempDir(): string {
  return createTempPluginDir(tempDirs, "daisyclaw-codex-ext-");
}
...
describe("agent tool result middleware", () => {
  it("includes plugin-registered middleware and restores it from cache", async () => {
    ...
    process.env.DAISYCLAW_BUNDLED_PLUGINS_DIR = tmp;
  ...
  api.registerAgentToolResultMiddleware(async (event) => ({
    ...
    loadDaisyClawPlugins(options);
    expect(listAgentToolResultMiddlewares("codex")).toHaveLength(1);
    expect(listAgentToolResultMiddlewares("daisyclaw")).toHaveLength(0);
...
export default { id: "tool-result-middleware", register(api) {
  api.registerAgentToolResultMiddleware(middleware, { runtimes: ["daisyclaw"] });
  api.registerAgentToolResultMiddleware(middleware, { runtimes: ["codex"] });
    ...
    expect(listAgentToolResultMiddlewares("daisyclaw")).toHaveLength(1);
    expect(listAgentToolResultMiddlewares("codex")).toHaveLength(1);
  });
