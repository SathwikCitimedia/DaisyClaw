...
function isDaisyClawLoopbackMcpServer(name: string, server: BundleMcpServerConfig): boolean {
  return (
    name === "daisyclaw" &&
...
export function normalizeCodexMcpServerConfig(
  ...
  } else if (isDaisyClawLoopbackMcpServer(name, server)) {
    // DaisyClaw's loopback MCP exposes local tools; Codex should ask for approval
