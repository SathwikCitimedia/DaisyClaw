...
function firstEmbeddedAgentArg(callIndex = 0) {
  return requireMockArg(runEmbeddedAgentMock, callIndex, "embedded DaisyClaw agent argument");
}
...
describe("CLI attempt execution", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-cli-attempt-"));
  ...
  function makeClaudeCliSessionEntry(
    daisyclawSessionId: string,
    cliSessionId: string,
  ): SessionEntry {
    return {
      sessionId: daisyclawSessionId,
      updatedAt: Date.now(),
      cliSessionBindings: {
        "claude-cli": {
  ...
  it("does not pass --resume when the stored Claude CLI transcript is missing", async () => {
    ...
    process.env.HOME = homeDir;
    const sessionEntry: SessionEntry = {
      sessionId: "daisyclaw-session-123",
      updatedAt: Date.now(),
      cliSessionBindings: {
        "claude-cli": {
  ...
  it("keeps Claude CLI resume when the stored transcript has assistant content", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "daisyclaw-session-456",
      updatedAt: Date.now(),
      cliSessionBindings: {
        "claude-cli": {
          sessionId: cliSessionId,
  ...
  it("checks Claude CLI transcript content under the process cwd", async () => {
    ...
    const sessionEntry = makeClaudeCliSessionEntry("daisyclaw-session-cwd", cliSessionId);
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
    runCliAgentMock.mockResolvedValueOnce(makeCliResult("resumed cli response"));
  ...
  it("passes session-bound OpenAI Codex auth profile to codex-cli aliases", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "daisyclaw-session-codex",
      updatedAt: Date.now(),
  ...
  it("embedded assistant gap-fill skips trailing daisyclaw.cache-ttl custom entries (regression for #83427)", async () => {
    ...
    await fs.appendFile(
      ...
      `${JSON.stringify({
        type: "custom",
        customType: "daisyclaw.cache-ttl",
  ...
  it("embedded assistant gap-fill appends repeated replies after a user tail", async () => {
    ...
    expect(
      sessionFile.endsWith(
        path.join(".daisyclaw", "agents", "main", "sessions", `${sessionEntry.sessionId}.jsonl`),
      ),
    ).toBe(true);
  ...
  it("forwards separate user trigger, channel, and provider context to CLI runs", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "daisyclaw-session-channel",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("forwards runtime toolsAllow into CLI attempts so the CLI harness can fail closed", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "daisyclaw-session-cli-tools-allow",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("stamps CLI prompts with current timestamp context", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "daisyclaw-session-cli-timestamp",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
    ...
    await runAgentAttempt({
      ...
      originalProvider: "claude-cli",
      modelOverride: "opus",
      cfg: { agents: { defaults: { userTimezone: "UTC" } } } as DaisyClawConfig,
      sessionEntry,
      sessionId: sessionEntry.sessionId,
      sessionKey,
      sessionAgentId: "main",
  ...
  it("routes canonical Anthropic models through the configured Claude CLI runtime", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "daisyclaw-session-canonical-cli",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("routes provider-qualified Anthropic shorthand through the configured Claude CLI runtime", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "daisyclaw-session-shorthand-cli",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("routes canonical OpenAI models through the configured embedded Codex runtime", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "daisyclaw-session-canonical-codex-cli",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
    runEmbeddedAgentMock.mockResolvedValueOnce({
      ...
      meta: {
        ...
        executionTrace: { runner: "daisyclaw" },
      },
    });
  ...
  it("forwards user-pinned OpenAI API-key backup profiles to Codex harness runs", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "daisyclaw-session-openai-chatgpt-api-key",
      updatedAt: Date.now(),
  ...
  it("keeps one-shot model runs on the raw embedded provider path", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "daisyclaw-session-model-run-raw",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("forwards trusted elevated defaults to embedded agent runs", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "daisyclaw-session-elevated-followup",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    const bashElevated = {
  ...
  it("forwards one-shot CLI cleanup to CLI providers", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "daisyclaw-session-cleanup-cli",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
...
describe("embedded attempt harness pinning", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-embedded-attempt-"));
    runCliAgentMock.mockReset();
    runEmbeddedAgentMock.mockReset();
  });
  ...
  it("ignores stale OpenAI sessions pinned to DaisyClaw and relies on default Codex routing", async () => {
    const sessionEntry: SessionEntry = {
  ...
  it("routes explicit OpenAI native runs with legacy Codex OAuth through DaisyClaw", async () => {
    const sessionEntry: SessionEntry = {
