...
async function writeSessionFile(params: { sessionFile: string; sessionId: string }) {
  // The lifecycle compacts canonical DaisyClaw session JSONL, so tests write the
...
describe("runCliTurnCompactionLifecycle", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-cli-compaction-"));
  ...
  it("compacts over-budget CLI transcripts and clears external CLI resume state", async () => {
    ...
    // Once DaisyClaw rewrites the transcript, external CLI resume ids are stale
  ...
  it("ignores stale native harness ids when the active provider no longer matches", async () => {
    const sessionKey = "agent:main:daisyclaw-after-codex";
    const sessionId = "session-daisyclaw-after-codex";
    const sessionFile = path.join(tmpDir, "session-daisyclaw-after-codex.jsonl");
    const storePath = path.join(tmpDir, "sessions-daisyclaw-after-codex.json");
    await writeSessionFile({ sessionFile, sessionId });
  ...
  it("bounds a hung CLI context-engine compaction and leaves resume state intact", async () => {
    ...
    const pending = runCliTurnCompactionLifecycle({
      cfg: { agents: { defaults: { compaction: { timeoutSeconds: 1 } } } } as DaisyClawConfig,
      sessionId,
      sessionKey,
      sessionEntry,
      sessionStore,
