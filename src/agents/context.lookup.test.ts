...
vi.mock("./models-config.runtime.js", () => ({
  ensureDaisyClawModelsJson: contextTestState.ensureDaisyClawModelsJson,
}));
...
function mockContextDeps(params: {
  ...
  contextTestState.ensureDaisyClawModelsJson.mockClear();
  return { ensureDaisyClawModelsJson: contextTestState.ensureDaisyClawModelsJson };
}
...
describe("lookupContextTokens", () => {
  ...
  it("skips model normalization during warmup but preserves provider-owned context metadata", async () => {
    ...
    expect(
      path.normalize(discoverAgentDir).endsWith(path.join(".daisyclaw", "agents", "main", "agent")),
    ).toBe(true);
