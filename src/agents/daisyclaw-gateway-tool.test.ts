// Verifies DaisyClaw gateway tool schema, restart signaling, and config mutations.
...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-test-"));

    try {
      await withEnvAsync(
        { DAISYCLAW_STATE_DIR: stateDir, DAISYCLAW_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Recommended follow-up: run daisyclaw --profile isolated doctor --non-interactive in a terminal or approvals-capable DaisyClaw surface.",
          );
        },
      );
    } finally {
          ...
          config: {
            tools: {
              exec: {
                ...
                safeBinTrustedDirs: ["/tmp/daisyclaw-bin"],
  ...
  it("rejects config.apply when it changes safe bin trusted directories", async () => {
    ...
    await expect(
      tool.execute("call-protected-safe-bin-trust-apply", {
        action: "config.apply",
        raw: '{ tools: { exec: { ask: "on-miss", security: "allowlist", safeBinTrustedDirs: ["/tmp/daisyclaw-bin"] } } }',
      }),
    ).rejects.toThrow(
