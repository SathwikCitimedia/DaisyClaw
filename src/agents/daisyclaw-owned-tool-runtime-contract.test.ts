// Verifies DaisyClaw-owned tool hooks preserve adjusted params and telemetry.
import type { AgentTool } from "daisyclaw/plugin-sdk/agent-core";
import {
  installDaisyClawOwnedToolHooks,
  resetDaisyClawOwnedToolHooks,
  textToolResult,
} from "daisyclaw/plugin-sdk/agent-runtime-test-contracts";
import type { ExtensionContext } from "daisyclaw/plugin-sdk/agent-sessions";
...
describe("DaisyClaw-owned tool runtime contract - embedded agent adapter", () => {
  afterEach(() => {
    resetDaisyClawOwnedToolHooks();
  });
  ...
  it("preserves partially adjusted before_tool_call params through execution and after_tool_call", async () => {
    ...
    const hooks = installDaisyClawOwnedToolHooks({ adjustedParams });
  ...
  it("commits successful embedded agent messaging text, media, and target telemetry", async () => {
    const hooks = installDaisyClawOwnedToolHooks();
    ...
    const originalParams = {
      ...
      mediaUrl: "/tmp/daisyclaw-reply.png",
      provider: "telegram",
      to: "chat-1",
    };
    ...
    expect(ctx.state.messagingToolSentMediaUrls).toEqual(["/tmp/daisyclaw-reply.png"]);
    expect(
    ...
    ).toEqual([
      {
        ...
        mediaUrls: ["/tmp/daisyclaw-reply.png"],
      },
    ]);
    const [afterPayload, afterContext] = await waitForAfterToolCall(hooks);
  ...
  it("fails closed when before_tool_call blocks an embedded agent dynamic tool", async () => {
    const hooks = installDaisyClawOwnedToolHooks({ blockReason: "blocked by policy" });
