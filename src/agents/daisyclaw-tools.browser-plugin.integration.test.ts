// Verifies DaisyClaw plugin tools are resolved with browser/runtime context.
...
function firstResolvePluginToolsParams(): Record<string, unknown> {
  // Captures the plugin runtime contract passed from DaisyClaw tool resolution.
...
describe("createDaisyClawTools browser plugin integration", () => {
  afterEach(() => {
  ...
  it("keeps the browser tool returned by plugin resolution", () => {
    ...
    const tools = resolveDaisyClawPluginToolsForOptions({
      options: { config },
  ...
  it("does not pass a stale active snapshot as plugin runtime config for a resolved run config", () => {
    ...
    let capturedRuntimeConfig: DaisyClawConfig | undefined;
    hoisted.resolvePluginTools.mockImplementation((params: unknown) => {
      capturedRuntimeConfig = (params as { context?: { runtimeConfig?: DaisyClawConfig } }).context
        ?.runtimeConfig;
      return [];
    });
  ...
  it("does not let a source-less pinned config snapshot override explicit plugin tool config", () => {
    ...
    let capturedRuntimeConfig: DaisyClawConfig | undefined;
    let getRuntimeConfig: (() => DaisyClawConfig | undefined) | undefined;
    hoisted.resolvePluginTools.mockImplementation((params: unknown) => {
      const context = (
        params as {
          context?: {
            runtimeConfig?: DaisyClawConfig;
            getRuntimeConfig?: () => DaisyClawConfig | undefined;
          };
        }
      ).context;
  ...
  it("exposes a live runtime config getter to plugin tool factories", () => {
    ...
    hoisted.resolvePluginTools.mockImplementation((params: unknown) => {
      getRuntimeConfig = (
        params as { context?: { getRuntimeConfig?: () => DaisyClawConfig | undefined } }
