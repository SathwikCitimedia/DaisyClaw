...
import { resolveOptionalMediaToolFactoryPlan } from "./daisyclaw-tools.media-factory-plan.js";
...
type CreateDaisyClawToolsOptions = Parameters<
  typeof import("./daisyclaw-tools.js").createDaisyClawTools
>[0];
let createDaisyClawToolsForTestModule: typeof import("./daisyclaw-tools.js").createDaisyClawTools;
let legacyComfyToolNames: string[];

async function createDaisyClawToolsForTest(options?: CreateDaisyClawToolsOptions) {
  return createDaisyClawToolsForTestModule(options);
}
...
function createPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/plugins/${params.id}/daisyclaw.plugin.json`,
    channels: [],
    providers: [],
    cliBackends: [],
    skills: [],
...
function legacyModelProviderConfig(provider: Record<string, unknown>): DaisyClawConfig {
  return {
    models: {
      providers: {
...
describe("optional media tool factory planning", () => {
  beforeAll(async () => {
    ({ createDaisyClawTools: createDaisyClawToolsForTestModule } =
      await import("./daisyclaw-tools.js"));
    ...
    legacyComfyToolNames = (
      await createDaisyClawToolsForTest({
        config,
  ...
  it("preserves implicit allow-all from alsoAllow-only policies for built-in media factories", async () => {
    ...
    const toolNames = (
      await createDaisyClawToolsForTest({
        config,
        agentDir: "/tmp/daisyclaw-agent-main",
        authProfileStore: createAuthStore(),
  ...
  it("applies global tool policy before optional media factories run", () => {
    const config: DaisyClawConfig = { tools: { deny: ["pdf"] } };
    installSnapshot(config, [
      createPlugin({
        id: "media-owner",
        contracts: { mediaUnderstandingProviders: ["anthropic"] },
  ...
  it("defers PDF model resolution from the tool-prep hot path", async () => {
    ...
    const tools = await createDaisyClawToolsForTest({
      config,
      agentDir: "/tmp/daisyclaw-agent-main",
  ...
  it("does not register the image tool without cheap vision availability evidence", async () => {
    const config: DaisyClawConfig = {};
    const workspaceDir = "/tmp/daisyclaw-workspace";
