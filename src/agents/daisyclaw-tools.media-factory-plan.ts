...
function hasExplicitImageModelConfig(config: DaisyClawConfig | undefined): boolean {
  return hasExplicitToolModelConfig(config?.agents?.defaults?.imageModel);
}

function hasExplicitPdfModelConfig(config: DaisyClawConfig | undefined): boolean {
  return (
