...
import "./test-helpers/fast-daisyclaw-tools-sessions.js";
...
function createDaisyClawTools(options?: {
...
describe("sessions tools", () => {
  ...
  it("sessions_list forwards mailbox filters and includes messages", async () => {
    ...
    const tool = createDaisyClawTools().find((candidate) => candidate.name === "sessions_list");
    if (!tool) {
  ...
  it("derives mailbox previews only after agent visibility filtering", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-sessions-list-preview-"));
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    const tool = createDaisyClawTools().find((candidate) => candidate.name === "sessions_history");
    if (!tool) {
    ...
    expect(details.messages).toContainEqual(
      expect.objectContaining({ provider: "daisyclaw", model: "gateway-injected" }),
    );
    expect(details.messages).toContainEqual(
  ...
  it("sessions_history caps oversized payloads and strips heavy fields", async () => {
    ...
    expect(thinkingBlock?.daisyclawReasoningReplay).toBeUndefined();
  });
