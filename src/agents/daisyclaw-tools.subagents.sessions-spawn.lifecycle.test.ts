...
} from "./daisyclaw-tools.subagents.sessions-spawn.test-harness.js";
import {
...
const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.DAISYCLAW_TEST_FAST;
...
describe("daisyclaw-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(async () => {
  ...
  afterAll(() => {
    ...
    process.env.DAISYCLAW_TEST_FAST = fastModeEnv.previous;
  });
  ...
  it("sessions_spawn retires bundle MCP runtime when run-mode cleanup completes", async () => {
    ...
    await getOrCreateSessionMcpRuntime({
      ...
      workspaceDir: "/tmp/daisyclaw-subagent-mcp-retire",
