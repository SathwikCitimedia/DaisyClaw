/**
 * DaisyClaw built-in and plugin tool assembly.
 *
...
import { resolveDaisyClawPluginToolsForOptions } from "./daisyclaw-plugin-tools.js";
import {
  isToolExplicitlyAllowedByFactoryPolicy,
  mergeFactoryPolicyList,
  resolveImageToolFactoryAvailable,
  resolveOptionalMediaToolFactoryPlan,
} from "./daisyclaw-tools.media-factory-plan.js";
import { applyNodesToolWorkspaceGuard } from "./daisyclaw-tools.nodes-workspace-guard.js";
import {
  collectPresentDaisyClawTools,
  shouldIncludeUpdatePlanToolForDaisyClawTools,
} from "./daisyclaw-tools.registration.js";
...
type DaisyClawToolsDeps = {
  callGateway: typeof callGateway;
  config?: DaisyClawConfig;
};

const defaultDaisyClawToolsDeps: DaisyClawToolsDeps = {
  callGateway,
};

let daisyClawToolsDeps: DaisyClawToolsDeps = defaultDaisyClawToolsDeps;

export function createDaisyClawTools(
  options?: {
  ...
  const resolvedConfig = options?.config ?? daisyClawToolsDeps.config;
  ...
  options?.recordToolPrepStage?.("daisyclaw-tools:session-workspace");
  ...
  const imageTool = resolveImageToolFactoryAvailable({
    ...
    : null;
  options?.recordToolPrepStage?.("daisyclaw-tools:image-tool");
  const imageGenerateTool = optionalMediaTools.imageGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("daisyclaw-tools:image-generate-tool");
  const videoGenerateTool = optionalMediaTools.videoGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("daisyclaw-tools:video-generate-tool");
  const musicGenerateTool = optionalMediaTools.musicGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("daisyclaw-tools:music-generate-tool");
  const pdfTool =
    optionalMediaTools.pdf && options?.agentDir?.trim()
      ...
      : null;
  options?.recordToolPrepStage?.("daisyclaw-tools:pdf-tool");
  ...
  options?.recordToolPrepStage?.("daisyclaw-tools:web-search-tool");
  ...
  options?.recordToolPrepStage?.("daisyclaw-tools:web-fetch-tool");
  ...
  options?.recordToolPrepStage?.("daisyclaw-tools:message-tool");
  ...
  options?.recordToolPrepStage?.("daisyclaw-tools:nodes-tool");
  ...
  const effectiveCallGateway = embedded
    ...
    : daisyClawToolsDeps.callGateway;
  const includeUpdatePlanTool = shouldIncludeUpdatePlanToolForDaisyClawTools({
    config: resolvedConfig,
    agentSessionKey: options?.agentSessionKey,
    agentId: options?.requesterAgentIdOverride,
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...collectPresentDaisyClawTools([heartbeatTool]),
    createTtsTool({
      agentChannel: options?.agentChannel,
      config: resolvedConfig,
      agentId: sessionAgentId,
    ...
    ...collectPresentDaisyClawTools([imageGenerateTool, musicGenerateTool, videoGenerateTool]),
    ...(embedded
      ? []
      : [
          createGatewayTool({
    ...
    ...(embedded
      ...
      : [
          createSessionsSendTool({
            ...
            sandboxed: options?.sandboxed,
            config: resolvedConfig,
            callGateway: daisyClawToolsDeps.callGateway,
          }),
        ]),
    ...
    ...collectPresentDaisyClawTools([webSearchTool, webFetchTool, imageTool, pdfTool]),
  ];
  options?.recordToolPrepStage?.("daisyclaw-tools:core-tool-list");
  let allTools = tools;
  if (!options?.disablePluginTools) {
    ...
    allTools = [
      ...tools,
      ...resolveDaisyClawPluginToolsForOptions({
        options,
        resolvedConfig,
        existingToolNames,
      }),
    ];
    options?.recordToolPrepStage?.("daisyclaw-tools:plugin-tools");
  }
  ...
  options?.recordToolPrepStage?.("daisyclaw-tools:tool-hooks");
...
export const testing = {
  resolveOptionalMediaToolFactoryPlan,
  setDepsForTest(overrides?: Partial<DaisyClawToolsDeps>) {
    daisyClawToolsDeps = overrides
      ? {
          ...defaultDaisyClawToolsDeps,
          ...overrides,
        }
      : defaultDaisyClawToolsDeps;
  },
