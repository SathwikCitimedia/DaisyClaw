// Verifies update_plan registration gates and base DaisyClaw tool inclusion policy.
...
type UpdatePlanGatingParams = Parameters<typeof isUpdatePlanToolEnabledForDaisyClawTools>[0];
type CreateDaisyClawToolsOptions = NonNullable<Parameters<typeof createDaisyClawTools>[0]>;

function expectUpdatePlanEnabled(params: UpdatePlanGatingParams, expected: boolean): void {
  expect(isUpdatePlanToolEnabledForDaisyClawTools(params)).toBe(expected);
}

function toolNames(tools: ReturnType<typeof createDaisyClawTools>): string[] {
  return tools.map((tool) => tool.name);
}

function createFastToolNames(options: CreateDaisyClawToolsOptions): string[] {
  // Disable unrelated dynamic surfaces so registration assertions stay deterministic.
  return toolNames(
    createDaisyClawTools({
...
function expectToolNamed(
  tools: ReturnType<typeof createDaisyClawTools>,
  name: string,
): ReturnType<typeof createDaisyClawTools>[number] {
...
describe("daisyclaw-tools update_plan gating", () => {
  afterEach(() => {
    setEmbeddedMode(false);
  });

  it("keeps update_plan disabled by default", () => {
    expectUpdatePlanEnabled({ config: {} as DaisyClawConfig }, false);
  });
  ...
  it("does not expose update_plan from default tool construction", () => {
    ...
    expect(shouldIncludeUpdatePlanToolForDaisyClawTools(emptyAllowlistParams)).toBe(false);
  });

  it("wraps constructed tools with before-tool-call hooks by default", () => {
    const tools = createDaisyClawTools({
      config: {} as DaisyClawConfig,
      disablePluginTools: true,
    });
    const unwrappedTools = createDaisyClawTools({
      config: {} as DaisyClawConfig,
      disablePluginTools: true,
      wrapBeforeToolCallHook: false,
    });
  ...
  it("requires explicit transcripts enablement before registering the transcripts tool", () => {
    ...
    const enabledTools = createFastToolNames({
      config: { transcripts: { enabled: true } } as DaisyClawConfig,
    });
  ...
  it("keeps explicitly allowed message tool in embedded completions", () => {
    setEmbeddedMode(true);
    const fromRuntimeAllowlist = createDaisyClawTools({
      config: {} as DaisyClawConfig,
      disablePluginTools: true,
      pluginToolAllowlist: ["message"],
      wrapBeforeToolCallHook: false,
    });
    const fromGlobalAlsoAllow = createDaisyClawTools({
      config: { tools: { profile: "minimal", alsoAllow: ["message"] } } as DaisyClawConfig,
      disablePluginTools: true,
      wrapBeforeToolCallHook: false,
    });
    const denied = createDaisyClawTools({
      config: {} as DaisyClawConfig,
      disablePluginTools: true,
      pluginToolAllowlist: ["message"],
  ...
  it("includes update_plan when a config allowlist group includes it", () => {
    const includeUpdatePlan = shouldIncludeUpdatePlanToolForDaisyClawTools({
      config: { tools: { allow: ["group:agents"] } } as DaisyClawConfig,
      modelProvider: "anthropic",
      modelId: "claude-sonnet-4-6",
    });
