...
describe("sanitizeUserFacingText", () => {
  ...
  it.each(["disk full", "ENOSPC: no space left on device"])(
    ...
    (input) => {
      expect(sanitizeUserFacingText(input, { errorContext: true })).toBe(
        "DaisyClaw could not write local session data because the disk is full. Free some disk space and try again.",
      );
    },
  );
  ...
  it("strips internal tool trace warning lines from error-context delivery text", () => {
    const input = [
      "Visible intro.",
      "⚠️ 🛠️ `run daisyclaw definitely-not-a-real-subcommand (agent)` failed",
      "⚠️ 🛠️ gh search issues --repo daisyclaw/daisyclaw --state open --no-search-pages.jsonl /tmp/daisyclaw_open_unlabeled_current.json (agent) failed",
      "⚠️ 🛠️ gh search issues --repo daisyclaw/daisyclaw --state open (agent) failed: command timed out",
      "🛠️ run git status",
  ...
  it("strips copied inbound metadata blocks from user-facing assistant text", () => {
    const input = [
      ...
      "```json",
      '{"chat_id":"channel:123","sender":"DaisyClaw"}',
      "```",
      "",
  ...
  it("does not strip ordinary text that merely mentions internal marker strings", () => {
    const input = [
      "The literal header `DaisyClaw runtime context (internal):` appears in this note.",
  ...
  it("does not strip text that starts with the legacy header phrase but is not the canonical block", () => {
    const input =
      "DaisyClaw runtime context (internal): is the label used by the old runtime block formatter.";
