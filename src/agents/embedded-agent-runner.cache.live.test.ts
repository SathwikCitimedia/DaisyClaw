...
import type { AssistantMessage, Message, Tool } from "daisyclaw/plugin-sdk/llm";
...
describeCacheLive("embedded agent runner prompt caching (live)", () => {
  beforeAll(async () => {
    liveRunnerRootDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-live-cache-"));
    ...
    previousCacheTraceEnv = {
      enabled: process.env.DAISYCLAW_CACHE_TRACE,
      file: process.env.DAISYCLAW_CACHE_TRACE_FILE,
      messages: process.env.DAISYCLAW_CACHE_TRACE_MESSAGES,
      prompt: process.env.DAISYCLAW_CACHE_TRACE_PROMPT,
      system: process.env.DAISYCLAW_CACHE_TRACE_SYSTEM,
    };
    process.env.DAISYCLAW_CACHE_TRACE = "1";
    process.env.DAISYCLAW_CACHE_TRACE_FILE = liveCacheTraceFile;
    process.env.DAISYCLAW_CACHE_TRACE_MESSAGES = "0";
    process.env.DAISYCLAW_CACHE_TRACE_PROMPT = "0";
    process.env.DAISYCLAW_CACHE_TRACE_SYSTEM = "0";
  }, 120_000);
  ...
  afterAll(async () => {
    if (previousCacheTraceEnv) {
      ...
      restore("DAISYCLAW_CACHE_TRACE", previousCacheTraceEnv.enabled);
      restore("DAISYCLAW_CACHE_TRACE_FILE", previousCacheTraceEnv.file);
      restore("DAISYCLAW_CACHE_TRACE_MESSAGES", previousCacheTraceEnv.messages);
      restore("DAISYCLAW_CACHE_TRACE_PROMPT", previousCacheTraceEnv.prompt);
      restore("DAISYCLAW_CACHE_TRACE_SYSTEM", previousCacheTraceEnv.system);
    }
  ...
  describe("openai", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "DAISYCLAW_LIVE_OPENAI_CACHE_MODEL",
  ...
  describe("anthropic", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "DAISYCLAW_LIVE_ANTHROPIC_CACHE_MODEL",
