...
const ensureDaisyClawModelsJsonMock = vi.fn(async () => ({ wrote: false }));
const loggerWarnMock = vi.fn();
let refreshRuntimeAuthOnFirstPromptError = false;

vi.mock("daisyclaw/plugin-sdk/llm", async () => {
  const actual =
    await vi.importActual<typeof import("daisyclaw/plugin-sdk/llm")>("daisyclaw/plugin-sdk/llm");
...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async () => {
    ...
    return {
      ...mod,
      ensureDaisyClawModelsJson: (...args: Parameters<typeof ensureDaisyClawModelsJsonMock>) =>
        ensureDaisyClawModelsJsonMock(...args),
    };
  });
};

let runEmbeddedAgent: typeof import("./embedded-agent-runner/run.js").runEmbeddedAgent;
let SessionManager: typeof import("daisyclaw/plugin-sdk/agent-sessions").SessionManager;
...
beforeAll(async () => {
  ...
  ({ SessionManager } = await import("daisyclaw/plugin-sdk/agent-sessions"));
  e2eWorkspace = await createEmbeddedAgentRunnerTestWorkspace("daisyclaw-embedded-agent-");
...
beforeEach(() => {
  ...
  ensureDaisyClawModelsJsonMock.mockReset();
  ensureDaisyClawModelsJsonMock.mockResolvedValue({ wrote: false });
...
describe("runEmbeddedAgent", () => {
  it("skips models.json generation when dynamic model resolution succeeds", async () => {
    ...
    expect(ensureDaisyClawModelsJsonMock).not.toHaveBeenCalled();
  });

  it("resolves explicit OpenAI DaisyClaw runs through Codex when auth order starts with Codex OAuth", async () => {
    const sessionFile = nextSessionFile();
    ...
    await runEmbeddedAgent({
      sessionId: "codex-first-daisyclaw",
      sessionFile,
      workspaceDir,
      config: cfg,
      prompt: "hello",
      ...
      timeoutMs: 5_000,
      agentDir,
      runId: nextRunId("codex-first-daisyclaw"),
      enqueue: immediateEnqueue,
    });
