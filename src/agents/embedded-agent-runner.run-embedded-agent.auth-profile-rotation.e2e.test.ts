...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async () => {
    ...
    return {
      ...mod,
      ensureDaisyClawModelsJson: vi.fn(async () => ({ wrote: false })),
    };
  });
};
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): DaisyClawConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): DaisyClawConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
...
async function withTimedAgentWorkspace<T>(
  ...
  try {
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-workspace-"));
    const now = Date.now();
...
describe("runEmbeddedAgent auth profile rotation", () => {
  ...
  it("logs structured failover decision metadata for overloaded assistant rotation", async () => {
    ...
    setLoggerOverrideFn({
      level: "trace",
      consoleLevel: "silent",
      file: path.join(os.tmpdir(), `daisyclaw-auth-rotation-${Date.now()}.log`),
    });
