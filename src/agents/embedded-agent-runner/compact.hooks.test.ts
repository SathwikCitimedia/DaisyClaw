...
describe("compactEmbeddedAgentSessionDirect hooks", () => {
  ...
  it("preserves full sender identity when building compaction tools", async () => {
    ...
    expectRecordFields(mockCallArg(createDaisyClawCodingToolsMock), {
      senderId: "sender-1",
      senderName: "Alice",
      senderUsername: "alice_u",
      senderE164: "+15551234567",
  ...
  it("quarantines unsupported tool schemas before creating the compaction model session", async () => {
    ...
    createDaisyClawCodingToolsMock.mockReturnValueOnce([
      {
        name: "healthy_lookup",
  ...
  it("preserves direct OpenAI API-key compaction when DaisyClaw runtime is active", async () => {
    resolveAgentHarnessPolicyMock.mockReturnValue({ runtime: "daisyclaw" });
    resolveModelMock.mockImplementation((provider = "openai", modelId = "fake") => ({
      model: { provider, api: "responses", id: modelId, input: [] },
      error: null,
      authStorage: { setRuntimeApiKey: vi.fn() },
    ...
    const result = await compactEmbeddedAgentSessionDirect({
      ...
      config: {
        ...
        agents: { defaults: { embeddedHarness: { runtime: "daisyclaw" } } },
      } as never,
    });
...
describe("compactEmbeddedAgentSession hooks (ownsCompaction engine)", () => {
  ...
  it("preserves concrete DaisyClaw pins over explicit Codex policy for queued compaction", async () => {
    resolveAgentHarnessPolicyMock.mockReturnValue({
      runtime: "codex",
      runtimeSource: "model",
    } as never);
