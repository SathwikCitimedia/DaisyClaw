...
function authFingerprint(agentDir: string): object {
  return {
    authProfilesSqlite: fileFingerprint(path.join(agentDir, "daisyclaw-agent.sqlite")),
    authProfilesSqliteWal: fileFingerprint(path.join(agentDir, "daisyclaw-agent.sqlite-wal")),
  };
}
