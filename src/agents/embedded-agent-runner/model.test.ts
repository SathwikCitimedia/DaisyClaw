...
vi.mock("../model-suppression.js", () => {
  ...
  // extensions/qwen/daisyclaw.plugin.json and src/plugins/manifest-model-suppression.ts.
...
import type { DaisyClawConfig, DaisyClawConfigInput } from "../../config/config.js";
...
describe("resolveModel", () => {
  ...
  it("invalidates agent discovery stores when generated plugin catalogs change", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-model-cache-plugin-"));
  ...
  it("invalidates agent discovery stores when inherited default auth changes", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-model-cache-"));
    const agentDir = path.join(rootDir, "agent");
  ...
  it("uses the resolved default agent workspace for cached model discovery", () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-model-workspace-"));
    const agentDir = path.join(rootDir, "agent");
  ...
  it("invalidates agent discovery stores when implicit main auth changes without config", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-model-cache-state-"));
    const agentDir = path.join(rootDir, "agents", "worker", "agent");
    const mainAgentDir = path.join(rootDir, "agents", "main", "agent");
    fs.mkdirSync(agentDir, { recursive: true });
    fs.mkdirSync(mainAgentDir, { recursive: true });
    try {
      await withEnvAsync({ DAISYCLAW_STATE_DIR: rootDir }, async () => {
        mockDiscoveredModel(discoverModels, {
          provider: "openai",
          modelId: "gpt-5.5",
          templateModel: {
  ...
  it("skips DaisyClaw auth and model discovery during dynamic model resolution", async () => {
  ...
  it("defaults missing model cost before handing models to DaisyClaw", () => {
  ...
  it("does not create fallback models from provider overlays alone", () => {
    ...
    } satisfies DaisyClawConfigInput;

    const result = resolveModelForTest(
      "typoProvider",
      "typoed-model",
      "/tmp/agent",
      cfg as unknown as DaisyClawConfig,
    );
