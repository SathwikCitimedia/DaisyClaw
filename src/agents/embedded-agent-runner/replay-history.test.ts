...
function daisyclawTranscriptAssistant(model: "delivery-mirror" | "gateway-injected"): AgentMessage {
  return {
    role: "assistant",
...
describe("normalizeAssistantReplayContent", () => {
  ...
  it("filters daisyclaw delivery-mirror and gateway-injected assistant messages from replay", () => {
    ...
    const messages = [
      userMessage("hello"),
      daisyclawTranscriptAssistant("delivery-mirror"),
      bedrockAssistant([{ type: "text", text: "real reply" }]),
      daisyclawTranscriptAssistant("gateway-injected"),
    ];
    const out = normalizeAssistantReplayContent(messages);
