 ...
 * See: https://github.com/daisyclaw/daisyclaw/issues/60552
 */
...
export async function runEmbeddedAgent(
  ...
  return enqueueSession(() => {
    ...
    return enqueueGlobal(async () => {
      ...
      const pluginHarnessOwnsTransport = agentHarness.id !== "daisyclaw";
      ...
      for (const candidateProvider of modelResolutionProviders) {
        const candidateResolution = await resolveModelAsync(
          ...
          params.config,
          {
            ...
            // first generating DaisyClaw models.json. This keeps one-shot model runs from
      ...
      if (!modelResolution) {
        await ensureDaisyClawModelsJson(params.config, agentDir, {
          workspaceDir: resolvedWorkspace,
        });
        for (const candidateProvider of modelResolutionProviders) {
          const candidateResolution = await resolveModelAsync(
      ...
      const pluginHarnessNeedsDaisyClawAuthBootstrap =
        pluginHarnessOwnsTransport &&
        provider === OPENAI_PROVIDER_ID &&
        effectiveModel.api === "openai-chatgpt-responses";
      const daisyClawNativeCodexResponsesNeedsAuthBootstrap =
        !pluginHarnessOwnsTransport &&
        provider === OPENAI_PROVIDER_ID &&
        effectiveModel.api === "openai-chatgpt-responses";
      let piExternalCliAuthScope = pluginHarnessOwnsTransport
        ? { ignoreAutoPreferredProfile: false }
        : daisyClawNativeCodexResponsesNeedsAuthBootstrap
          ? {
      ...
      if (
        !pluginHarnessOwnsTransport &&
        !pluginHarnessNeedsDaisyClawAuthBootstrap &&
      ...
      const authStore =
        pluginHarnessOwnsTransport && !pluginHarnessNeedsDaisyClawAuthBootstrap
          ? createEmptyAuthProfileStore()
          : pluginHarnessNeedsDaisyClawAuthBootstrap
      ...
      // Plugin harnesses own their model transport/auth. Running DaisyClaw's generic
      ...
      if (!pluginHarnessOwnsTransport || pluginHarnessNeedsDaisyClawAuthBootstrap) {
      ...
      const runAttemptAuthProfileStore = pluginHarnessOwnsTransport
        ...
        : attemptAuthProfileStore;
      const harnessBuildsDaisyClawTools =
      ...
      const maybeMarkAuthProfileFailure = async (failure: {
        ...
        if (pluginHarnessOwnsTransport && reason === "timeout") {
          ...
          // credential evidence. Do not poison DaisyClaw auth cooldowns.
          return;
        }
      ...
      try {
        ...
        while (true) {
          ...
          const rawAttempt = await runEmbeddedAttemptWithBackend({
            ...
            // attempt too. Otherwise plugin-owned transports can skip DaisyClaw auth
            // bootstrap but drift back to DaisyClaw when the attempt is created.
            agentHarnessId: agentHarness.id,
            ...(params.sessionKey
              ? {
            ...
            // These harnesses build DaisyClaw tools internally. Keep transport auth
            // scoped while letting tool construction see plugin/provider creds.
            toolAuthProfileStore: harnessBuildsDaisyClawTools ? attemptAuthProfileStore : undefined,
            modelRegistry,
            agentId: workspaceResolution.agentId,
