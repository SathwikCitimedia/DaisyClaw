...
import type { DaisyClawCodingToolConstructionPlan } from "../../agent-tools.js";
...
// Names here must be emitted directly by createDaisyClawTools(). Catalog entries
...
const DAISYCLAW_TOOL_FACTORY_NAMES = new Set([
  "agents_list",
  "canvas",
  "cron",
  "gateway",
...
const ALL_CODING_TOOL_CONSTRUCTION_PLAN: DaisyClawCodingToolConstructionPlan = {
...
const NO_CODING_TOOL_CONSTRUCTION_PLAN: DaisyClawCodingToolConstructionPlan = {
...
function cloneCodingToolConstructionPlan(
  plan: DaisyClawCodingToolConstructionPlan,
): DaisyClawCodingToolConstructionPlan {
  return { ...plan };
}
...
function isKnownLocalCodingToolName(normalized: string): boolean {
  ...
  return (
    ...
    DAISYCLAW_TOOL_FACTORY_NAMES.has(normalized)
  );
}
...
function resolveCodingToolConstructionPlanForAllowlist(
  ...
  const includeDaisyClawTools = normalized.some((name) => DAISYCLAW_TOOL_FACTORY_NAMES.has(name));
...
export function resolveEmbeddedAttemptToolConstructionPlan(params: {
  ...
  codingToolConstructionPlan: DaisyClawCodingToolConstructionPlan;
} {
  ...
  const includeCoreTools =
    ...
    codingToolConstructionPlan.includeDaisyClawTools;
...
/** Returns whether the allowlist requires any built-in coding/DaisyClaw tools. */
