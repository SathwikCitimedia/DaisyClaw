...
describe("runEmbeddedAttempt cwd/workspace split", () => {
  ...
  it("uses workspace for bootstrap and cwd for runtime tools", async () => {
    ...
    const taskRepo = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-task-repo-"));
    ...
    const toolsCall = hoisted.createDaisyClawCodingToolsMock.mock.calls[0]?.[0] as
  ...
  it("rejects cwd overrides for sandboxed runs instead of silently ignoring them", async () => {
    ...
    hoisted.resolveSandboxContextMock.mockResolvedValueOnce({
      enabled: true,
      workspaceAccess: "ro",
      workspaceDir: "/tmp/daisyclaw-sandbox-copy",
    });
    ...
    expect(hoisted.createDaisyClawCodingToolsMock).not.toHaveBeenCalled();
  });
});
