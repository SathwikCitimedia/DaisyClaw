...
function stripUnsafeBlockedRunMetadata(messages: AgentMessage[]): AgentMessage[] {
  ...
  const nextMessages = messages.map((message) => {
    const daisyclaw = (message as unknown as Record<string, unknown>)["__daisyclaw"];
    if (!daisyclaw || typeof daisyclaw !== "object") {
      return message;
    }
    const beforeAgentRunBlocked = (daisyclaw as { beforeAgentRunBlocked?: unknown })
    ...
    const nextDaisyClaw = {
      ...(daisyclaw as Record<string, unknown>),
      beforeAgentRunBlocked: safeBlocked,
    };
    changed = true;
    return {
      ...(message as unknown as Record<string, unknown>),
      __daisyclaw: nextDaisyClaw,
