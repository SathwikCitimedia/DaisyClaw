...
import type { AuthStorage, ModelRegistry } from "daisyclaw/plugin-sdk/agent-sessions";
...
describe("runEmbeddedAttempt memory flush tool forwarding", () => {
  it("forwards memory trigger metadata into tool creation so append-only guards activate", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-attempt-memory-flush-"));
