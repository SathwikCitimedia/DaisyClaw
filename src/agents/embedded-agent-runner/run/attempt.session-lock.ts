...
type PromptReleaseStreamFn = ((...args: unknown[]) => unknown) & {
  __daisyclawSessionLockPromptReleaseInstalled?: boolean;
};
...
const TRANSCRIPT_ONLY_DAISYCLAW_ASSISTANT_MODELS = new Set(["delivery-mirror", "gateway-injected"]);
...
function isTranscriptOnlyDaisyClawAssistantLine(line: string): boolean {
  try {
    ...
    return (
      ...
      TRANSCRIPT_ONLY_DAISYCLAW_ASSISTANT_MODELS.has(message.model)
    );
  } catch {
    return false;
  }
...
async function sessionFenceAdvanceIsBenign(params: {
  ...
  return lines.length > 0 && lines.every(isTranscriptOnlyDaisyClawAssistantLine);
}
...
async function sessionFenceRewriteIsBenign(params: {
  ...
  return appendedLines.every(isTranscriptOnlyDaisyClawAssistantLine);
}
...
// Controllers in the same DaisyClaw process can legitimately take turns writing
// the same session file while another attempt is released for model I/O. Track
// only fingerprints that changed while DaisyClaw held the write lock so the
...
const EMBEDDED_ATTEMPT_SESSION_FILE_OWNER_STATE_KEY = Symbol.for(
  "daisyclaw.embeddedAttemptSessionFileOwnerState",
);
...
export function installPromptSubmissionLockRelease(params: {
  ...
  if (currentStreamFn["__daisyclawSessionLockPromptReleaseInstalled"] === true) {
    return;
  }
  ...
  wrappedStreamFn["__daisyclawSessionLockPromptReleaseInstalled"] = true;
