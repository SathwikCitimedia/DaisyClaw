...
import type { AgentMessage } from "daisyclaw/plugin-sdk/agent-core";
...
describe("runEmbeddedAttempt context engine sessionKey forwarding", () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      toolSearchControlsCase = mockParams(
        hoisted.createDaisyClawCodingToolsMock,
        0,
        "createDaisyClawCodingTools options",
      );
    } finally {
  ...
  it("enables Tool Search controls for embedded DaisyClaw runs when configured", async () => {
  ...
  it("defaults local-model lean embedded runs to Tool Search controls", async () => {
    ...
    expect(hoisted.createDaisyClawCodingToolsMock).toHaveBeenCalledTimes(1);
    ...
    const optionsConfig = requireRecord(options.config, "createDaisyClawCodingTools config");
    const toolsConfig = requireRecord(
      optionsConfig.tools,
      "createDaisyClawCodingTools tools config",
    );
  ...
  it("keeps Tool Search controls off for lean message-tool-only delivery", async () => {
    hoisted.createDaisyClawCodingToolsMock.mockReturnValueOnce([
      {
        name: "message",
        label: "Message",
  ...
  it("quarantines unsupported tool schemas before creating the model session", async () => {
    hoisted.createDaisyClawCodingToolsMock.mockReturnValue([
      {
        name: "healthy_lookup",
  ...
  it("enforces code-mode payload surface from active-agent config during an embedded attempt", async () => {
    ...
    expect(observedOptions.at(-1)?.daisyclawCodeModeToolSurface).toBe(true);
  ...
  it("sends transcriptPrompt visibly and keeps runtime context out of transcript messages", async () => {
    ...
    const result = await createContextEngineAttemptRunner({
      ...
      attemptOverrides: {
        prompt: [
          "visible ask",
          "",
          "<<<BEGIN_DAISYCLAW_INTERNAL_CONTEXT>>>",
          "secret runtime context",
          "<<<END_DAISYCLAW_INTERNAL_CONTEXT>>>",
        ].join("\n"),
    ...
    expectFields(
      findRecord(
        ...
        (message) => message.customType === "daisyclaw.runtime-context",
        "runtime context message",
      ),
      {
        role: "custom",
        customType: "daisyclaw.runtime-context",
        display: false,
      },
    );
    ...
    for (const value of [
      ...
      expect(String(value)).not.toContain("DAISYCLAW_INTERNAL_CONTEXT");
  ...
  it("rebuilds skill prompt inputs from the sandbox workspace for non-rw sandbox runs", async () => {
    const sandboxWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-sandbox-skills-"));
    ...
    await createContextEngineAttemptRunner({
      ...
      attemptOverrides: {
        skillsSnapshot: {
          prompt:
            "<available_skills><skill><location>~/.daisyclaw/skills/smaug/SKILL.md</location></skill></available_skills>",
          skills: [{ name: "smaug" }],
          resolvedSkills: [
            {
              name: "smaug",
              ...
              filePath: "/Users/alice/.daisyclaw/skills/smaug/SKILL.md",
              baseDir: "/Users/alice/.daisyclaw/skills/smaug",
              source: "daisyclaw-workspace",
              sourceInfo: {
                path: "/Users/alice/.daisyclaw/skills/smaug/SKILL.md",
                source: "daisyclaw-workspace",
                scope: "project",
                origin: "top-level",
                baseDir: "/Users/alice/.daisyclaw/skills/smaug",
  ...
  it("keeps bootstrap truncation warnings out of WebChat runtime context", async () => {
    ...
    hoisted.resolveBootstrapContextForRunMock.mockResolvedValueOnce({
      bootstrapFiles: [
        {
          name: "AGENTS.md",
          path: "/tmp/daisyclaw-warning-workspace/AGENTS.md",
          content: "A".repeat(200),
          missing: false,
        },
      ],
      contextFiles: [
        { path: "/tmp/daisyclaw-warning-workspace/AGENTS.md", content: "A".repeat(20) },
      ],
    });
  ...
  it("preserves bootstrap system context in the assembled system prompt", async () => {
    ...
    hoisted.createDaisyClawCodingToolsMock.mockImplementationOnce(() => [
    ...
    hoisted.resolveBootstrapContextForRunMock.mockResolvedValueOnce({
      bootstrapFiles: [
        {
          name: "BOOTSTRAP.md",
          path: "/tmp/daisyclaw-bootstrap-workspace/BOOTSTRAP.md",
  ...
  it("includes hook-adjusted bootstrap files preloaded before routing", async () => {
    const workspaceDir = "/tmp/daisyclaw-hook-workspace";
  ...
  it("adds current-turn context to the current model input without exposing internal runtime context", async () => {
    ...
    expect(seenPrompt).not.toContain("DAISYCLAW_INTERNAL_CONTEXT");
  ...
  it("submits runtime-only context through system prompt without visible prompt", async () => {
    ...
    expect(seenPrompt).toBe("Continue the DaisyClaw runtime event.");
    expect(result.finalPromptText).toBe("Continue the DaisyClaw runtime event.");
  ...
  it("keeps current inbound context visible on runtime-only turns", async () => {
    ...
    expect(seenPrompt).toContain("Continue the DaisyClaw runtime event.");
    expect(result.finalPromptText).toBe(seenPrompt);
    const trajectoryEvents = (
      await fs.readFile(path.join(tempPaths[0] ?? "", "session.trajectory.jsonl"), "utf8")
    )
  ...
  it("submits suppressed room event context as the model prompt", async () => {
    ...
    const result = await createContextEngineAttemptRunner({
      ...
      trajectory: true,
      attemptOverrides: {
        prompt: "[DaisyClaw room event]",
        transcriptPrompt: "",
        currentInboundEventKind: "room_event",
        currentInboundContext: {
          text: [
            "[DaisyClaw room event]",
    ...
    expect(seenPrompt).toContain("[DaisyClaw room event]");
    ...
    expect(seenPrompt?.trim().endsWith("[DaisyClaw room event]")).toBe(true);
    expect(seenPrompt).not.toBe("Continue the DaisyClaw runtime event.");
    ...
    expect(contextCompiled?.data?.prompt).toContain("[DaisyClaw room event]");
  });
