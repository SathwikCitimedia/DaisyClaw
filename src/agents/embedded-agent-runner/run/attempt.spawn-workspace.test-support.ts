...
type AttemptSpawnWorkspaceHoisted = {
  ...
  createDaisyClawCodingToolsMock: UnknownMock;
...
const hoisted = vi.hoisted((): AttemptSpawnWorkspaceHoisted => {
  ...
  return {
    ...
    createDaisyClawCodingToolsMock,
...
vi.mock("../../docs-path.js", () => ({
  resolveDaisyClawReferencePaths: async () => ({ docsPath: undefined, sourcePath: undefined }),
}));
...
vi.mock("../../agent-tools.js", () => ({
  createDaisyClawCodingTools: (options?: { workspaceDir?: string; spawnWorkspaceDir?: string }) =>
    hoisted.createDaisyClawCodingToolsMock(options),
...
vi.mock("../cache-ttl.js", () => ({
  appendCacheTtlTimestamp: (
    ...
    data: unknown,
  ) => sessionManager.appendCustomEntry?.("daisyclaw.cache-ttl", data),
  ...
  readLastCacheTtlTimestamp: (
    ...
    for (let index = calls.length - 1; index >= 0; index -= 1) {
      ...
      if (customType !== "daisyclaw.cache-ttl") {
        continue;
      }
      const entry = data as
        | {
...
export function resetEmbeddedAttemptHarness(
  ...
  hoisted.createDaisyClawCodingToolsMock.mockReset().mockImplementation((...args: unknown[]) => {
...
export async function createContextEngineAttemptRunner(params: {
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-ctx-engine-workspace-"));
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-ctx-engine-agent-"));
  ...
  const previousTrajectoryEnv = process.env.DAISYCLAW_TRAJECTORY;
  if (params.trajectory !== true) {
    process.env.DAISYCLAW_TRAJECTORY = "0";
  }
  try {
    return await (
      await loadRunEmbeddedAttempt()
    ...
    if (previousTrajectoryEnv === undefined) {
      delete process.env.DAISYCLAW_TRAJECTORY;
    } else {
      process.env.DAISYCLAW_TRAJECTORY = previousTrajectoryEnv;
    }
  }
}
