...
import {
  ...
  DAISYCLAW_EMBEDDED_CONTEXT_ENGINE_HOST,
...
import { resolveDaisyClawReferencePaths } from "../../docs-path.js";
import {
  ...
  try {
    ...
    if (activeContextEngine && activeContextEngine.info.id !== "legacy") {
      assertContextEngineHostSupport({
        ...
        operation: "agent-run",
        host: DAISYCLAW_EMBEDDED_CONTEXT_ENGINE_HOST,
      });
    }
    ...
    const toolsRaw = !shouldConstructTools
      ...
      : (() => {
          ...
          corePluginToolStages.mark("attempt:create-daisyclaw-coding-tools");
    ...
    const daisyClawReferences = await resolveDaisyClawReferencePaths({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      // DefaultResourceLoader.reload() rehydrates settings from disk and can drop DaisyClaw
      // compaction overrides applied in createPreparedEmbeddedAgentSettingsManager — same
      // rehydration also restores DaisyClaw runtime's auto-compaction (daisyclaw#75799), so re-apply
      // both guards.
      applyAgentCompactionSettingsFromConfig({
        settingsManager,
        cfg: params.config,
      ...
      // exact DaisyClaw-managed registrations so custom tools survive startup and
      ...
      if (isRawModelRun) {
        ...
        // normal DaisyClaw agent/tool prompt when `session.prompt()` starts.
      ...
      try {
        ...
        await sessionLockController.withSessionWriteLock(async () => {
          ...
          // double-compaction. See: https://github.com/daisyclaw/daisyclaw/issues/9282
          ...
          if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
            try {
              activeSessionManager.appendCustomEntry("daisyclaw:prompt-error", {
                timestamp: Date.now(),
                runId: params.runId,
                sessionId: params.sessionId,
                provider: params.provider,
      ...
      // See: https://github.com/daisyclaw/daisyclaw/issues/8643
