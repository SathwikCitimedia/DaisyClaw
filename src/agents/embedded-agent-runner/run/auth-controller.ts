...
export function createEmbeddedRunAuthController(params: {
  ...
  const applyApiKeyInfo = async (candidate?: string): Promise<void> => {
    ...
    if (!apiKeyInfo.apiKey) {
      ...
      // DaisyClaw runtime's authStorage considers the provider authenticated.  Try
      ...
      // sentinel so DaisyClaw runtime's hasConfiguredAuth() passes and the AWS SDK default
