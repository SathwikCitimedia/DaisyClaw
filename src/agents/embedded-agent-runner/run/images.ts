...
function isDaisyClawCliImageCachePath(filePath: string): boolean {
  const parts = filePath.replaceAll("\\", "/").split("/");
  return parts.some((part, index) => {
    if (part === ".daisyclaw-cli-images") {
      return true;
    }
    const parent = parts[index - 1] ?? "";
    return part === "daisyclaw-cli-images" && /^daisyclaw(?:-\d+)?$/.test(parent);
  });
}
...
export function detectImageReferences(prompt: string): DetectedImageRef[] {
  ...
  const addPathRef = (raw: string) => {
    ...
    if (isDaisyClawCliImageCachePath(resolved)) {
      return;
    }
    seen.add(dedupeKey);
