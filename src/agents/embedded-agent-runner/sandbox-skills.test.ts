...
const hostSkillPath = "/usr/lib/node_modules/daisyclaw/skills/demo/SKILL.md";
const hostSkillBaseDir = "/usr/lib/node_modules/daisyclaw/skills/demo";
const snapshot: SkillSnapshot = {
  prompt:
    "<available_skills><skill><location>/usr/lib/node_modules/daisyclaw/skills/demo/SKILL.md</location></skill></available_skills>",
  skills: [{ name: "demo" }],
  resolvedSkills: [
    {
      name: "demo",
...
describe("resolveSandboxSkillRuntimeInputs", () => {
  ...
  it("uses the materialized skills workspace and drops host-path snapshots for sandboxes", () => {
    ...
    expect(
      ...
      skillsPromptWorkspaceDir: "/workspace/.daisyclaw/sandbox-skills",
      skillsWorkspaceDir: "/state/sandbox-skills",
      workspaceOnly: true,
    });
  });
  ...
  it("rebuilds sandbox prompts from materialized skill paths", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-sandbox-skills-"));
    try {
      ...
      await fs.writeFile(
        ...
        [
          ...
          "name: demo",
          "description: Demo skill",
          'daisyclaw: {"requires":{"anyBins":["sandboxbin"]}}',
          "---",
          "# Demo",
          "",
        ].join("\n"),
      ...
      expect(prompt).toContain("/workspace/.daisyclaw/sandbox-skills/skills/demo/SKILL.md");
