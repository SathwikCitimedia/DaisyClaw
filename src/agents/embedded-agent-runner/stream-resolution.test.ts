...
import type { StreamFn } from "daisyclaw/plugin-sdk/agent-core";
...
afterEach(() => {
  testing.resetDaisyClawNativeCodexResponsesStreamFnForTest();
});
...
describe("describeEmbeddedAgentStreamStrategy", () => {
  ...
  it("describes default Codex fallback as DaisyClaw native", () => {
    expect(
      describeEmbeddedAgentStreamStrategy({
        currentStreamFn: undefined,
        model: {
    ...
    ).toBe("daisyclaw-native-codex-responses");
  });
...
describe("resolveEmbeddedAgentStreamFn", () => {
  ...
  it("routes Codex responses fallbacks through DaisyClaw native transport", async () => {
    // Codex OAuth models use the DaisyClaw native transport, with prompt-cache
    ...
    testing.setDaisyClawNativeCodexResponsesStreamFnForTest(nativeStreamFn as never);
    const streamFn = resolveEmbeddedAgentStreamFn({
      currentStreamFn: undefined,
      sessionId: "session-1",
      model: {
  ...
  it("routes DaisyClaw native OpenAI-compatible provider streams through boundary-aware transports", async () => {
  ...
  it("injects the resolved run api key into the DaisyClaw native Codex Responses fallback", async () => {
  ...
  it("falls back to authStorage when no resolved api key is available for DaisyClaw native fallback", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
    const authStorage = {
  ...
  it("forwards the run abort signal into the DaisyClaw native fallback when callers omit one", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
    const runSignal = new AbortController().signal;
    testing.setDaisyClawNativeCodexResponsesStreamFnForTest(nativeStreamFn as never);
    const streamFn = resolveEmbeddedAgentStreamFn({
  ...
  it("does not overwrite an explicit signal on the DaisyClaw native fallback path", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
    const runSignal = new AbortController().signal;
    const explicitSignal = new AbortController().signal;
    testing.setDaisyClawNativeCodexResponsesStreamFnForTest(nativeStreamFn as never);
  ...
  it("forwards the run signal on the sync DaisyClaw native fallback path without auth credentials", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
    const runSignal = new AbortController().signal;
    testing.setDaisyClawNativeCodexResponsesStreamFnForTest(nativeStreamFn as never);
    const streamFn = resolveEmbeddedAgentStreamFn({
  ...
  it("strips cache boundary markers on the DaisyClaw native fallback path", async () => {
