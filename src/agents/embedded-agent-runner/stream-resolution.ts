...
let daisyClawNativeCodexResponsesStreamFnForTest: StreamFn | undefined;
...
function isDefaultDaisyClawStreamFnForModel(
...
function resolveDaisyClawNativeCodexResponsesStreamFn(params: {
  model: EmbeddedRunAttemptParams["model"];
  ...
  if (!isDefaultDaisyClawStreamFnForModel(params.model, params.currentStreamFn)) {
    return undefined;
  }
  return daisyClawNativeCodexResponsesStreamFnForTest ?? params.currentStreamFn ?? streamSimple;
}
...
export function describeEmbeddedAgentStreamStrategy(params: {
  ...
  if (
    resolveDaisyClawNativeCodexResponsesStreamFn({
      model: params.model,
      currentStreamFn: params.currentStreamFn,
    })
  ) {
    return "daisyclaw-native-codex-responses";
  }
  if (isDefaultDaisyClawStreamFnForModel(params.model, params.currentStreamFn)) {
...
export function resolveEmbeddedAgentStreamFn(params: {
  ...
  const daisyClawNativeCodexResponsesStreamFn = resolveDaisyClawNativeCodexResponsesStreamFn({
    model: params.model,
    currentStreamFn: params.currentStreamFn,
  });
  if (daisyClawNativeCodexResponsesStreamFn) {
    return wrapEmbeddedAgentStreamFn(daisyClawNativeCodexResponsesStreamFn, {
      runSignal: params.signal,
      resolvedApiKey: params.resolvedApiKey,
      authProfileId: params.authProfileId,
      authStorage: params.authStorage,
  ...
  if (
    isDefaultDaisyClawStreamFnForModel(params.model, params.currentStreamFn) ||
    hasResolvedRuntimeApiKey(params.resolvedApiKey)
  ) {
    const boundaryAwareStreamFn = createBoundaryAwareStreamFnForModel(params.model);
    if (boundaryAwareStreamFn) {
      // Some DaisyClaw session factories return a provider-specific stream wrapper
      // once runtime auth is resolved. Keep transport-supported APIs on
      // DaisyClaw's HTTP transport so provider-specific auth/header semantics
...
export const testing = {
  setDaisyClawNativeCodexResponsesStreamFnForTest(streamFn: StreamFn | undefined): void {
    daisyClawNativeCodexResponsesStreamFnForTest = streamFn;
  },
  resetDaisyClawNativeCodexResponsesStreamFnForTest(): void {
    daisyClawNativeCodexResponsesStreamFnForTest = undefined;
  },
};
