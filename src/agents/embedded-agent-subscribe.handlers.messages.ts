...
function isTranscriptOnlyDaisyClawAssistantMessage(message: AgentMessage | undefined): boolean {
  if (!message || message.role !== "assistant") {
    return false;
  }
  const provider = normalizeOptionalString(message.provider) ?? "";
  const model = normalizeOptionalString(message.model) ?? "";
  return provider === "daisyclaw" && (model === "delivery-mirror" || model === "gateway-injected");
}
...
function isOpenAiCompletionsAssistantMessage(message: AgentMessage | undefined): boolean {
  ...
  const api = normalizeOptionalString((message as { api?: unknown }).api) ?? "";
  return api === "openai-completions" || api === "daisyclaw-openai-completions-transport";
}
...
export function handleMessageStart(
  ...
  if (msg?.role !== "assistant" || isTranscriptOnlyDaisyClawAssistantMessage(msg)) {
    return;
  }
