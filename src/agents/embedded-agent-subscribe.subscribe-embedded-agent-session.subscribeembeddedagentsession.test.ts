...
describe("subscribeEmbeddedAgentSession", () => {
  ...
  async function captureToolLifecycleLogSubsystems(messageChannel?: string): Promise<string[]> {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-tool-log-attribution-"));
    const logFile = path.join(tempDir, "daisyclaw.log");
    try {
      setLoggerOverride({
        level: "debug",
        consoleLevel: "silent",
