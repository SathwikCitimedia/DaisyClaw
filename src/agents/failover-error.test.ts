...
// https://github.com/daisyclaw/daisyclaw/issues/33785
...
describe("failover-error", () => {
  ...
  it("403 OpenRouter 'Key limit exceeded' returns billing (model fallback trigger)", () => {
    // GitHub: daisyclaw/daisyclaw#53849 — OpenRouter returns 403 with "Key limit exceeded"
  ...
  describe("isNonProviderRuntimeCoordinationError", () => {
    ...
    const makeEmbeddedTakeoverError = () => {
      const err = new Error(
        "session file changed while embedded prompt lock was released: /tmp/daisyclaw/session.jsonl",
      );
...
describe("buildFailoverRemediationHint", () => {
  it("returns a copy-pasteable login command for auth failures", () => {
    ...
    expect(buildFailoverRemediationHint(err)).toBe(
      "Re-authenticate with: daisyclaw models auth login --provider 'anthropic' --force",
    );
  });
  ...
  it("returns a hint for auth_permanent as well", () => {
    ...
    expect(buildFailoverRemediationHint(err)).toBe(
      "Re-authenticate with: daisyclaw models auth login --provider 'google-gemini-cli' --force",
    );
  });

  it("quotes provider ids that contain shell metacharacters", () => {
    expect(buildProviderReauthCommand("custom;touch /tmp/pwned")).toBe(
      "daisyclaw models auth login --provider 'custom;touch /tmp/pwned' --force",
    );
    expect(buildProviderReauthCommand("custom'provider")).toBe(
      "daisyclaw models auth login --provider 'custom'\\''provider' --force",
    );
  });
  ...
  it("wraps rendered provider commands in the standard CLI formatter", () => {
    expect(buildProviderReauthCommand("anthropic", { DAISYCLAW_PROFILE: "work" })).toBe(
      "daisyclaw --profile work models auth login --provider 'anthropic' --force",
    );
    expect(buildProviderReauthCommand("anthropic", { DAISYCLAW_CONTAINER_HINT: "dev" })).toBe(
      "daisyclaw --container dev models auth login --provider 'anthropic' --force",
    );
  });
