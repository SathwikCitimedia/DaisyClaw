 ...
 * DAISYCLAW_FALLBACK_SKIP_TTL_MS.
 */
export const DEFAULT_FALLBACK_SKIP_TTL_MS = 0;
const FALLBACK_SKIP_TTL_ENV = "DAISYCLAW_FALLBACK_SKIP_TTL_MS";
...
function getState(): SkipCacheState {
  const globalStore = globalThis as typeof globalThis & {
    daisyclawFallbackSkipCache?: SkipBySession;
    daisyclawFallbackSkipCacheState?: SkipCacheState;
  };
  if (!globalStore.daisyclawFallbackSkipCacheState) {
    ...
    const buckets = globalStore.daisyclawFallbackSkipCache ?? new Map();
    globalStore.daisyclawFallbackSkipCacheState = {
      buckets,
      lastGlobalPruneAtMs: 0,
    };
    globalStore.daisyclawFallbackSkipCache = buckets;
  }
  return globalStore.daisyclawFallbackSkipCacheState;
}
