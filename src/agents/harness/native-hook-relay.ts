...
const NATIVE_HOOK_RELAY_STATE_SYMBOL = Symbol.for("daisyclaw.nativeHookRelay.state");
        ...
        hookSpecificOutput: {
          ...
          decision:
            decision === "allow"
              ...
              : {
                  behavior: "deny",
                  message: message?.trim() || "Denied by DaisyClaw",
                },
        },
      })}\n`,
      stderr: "",
...
export function buildNativeHookRelayCommand(params: {
  ...
  const executable = params.executable ?? resolveDaisyClawCliExecutable();
  const argv =
...
function registerNativeHookRelayBridge(registration: ActiveNativeHookRelayRegistration): void {
  ...
  // directory is scoped by OS user (uid) and is shared across all DaisyClaw
...
function nativeHookRelayBridgeDir(): string {
  ...
  return path.join(tmpdir(), `daisyclaw-native-hook-relays-${uid}`);
}
...
function nativeHookRelayPermissionAllowAlwaysKey(params: {
  ...
  hash.update("daisyclaw:native-hook-relay:permission-allow-always:v2");
  hash.update("\0");
...
function readNativeHookRelayApprovalMode(rawPayload: JsonValue): "report" | undefined {
  const payload = isJsonObject(rawPayload) ? rawPayload : {};
  return payload.daisyclaw_approval_mode === "report" ? "report" : undefined;
}
...
async function requestNativeHookRelayPermissionApproval(
  ...
  const requestResult: {
    ...
    {
      pluginId: `daisyclaw-native-hook-relay-${request.provider}`,
      title: truncateText(
...
function resolveDaisyClawCliExecutable(): string {
  const envPath = process.env.DAISYCLAW_CLI_PATH?.trim();
  ...
  if (packageRoot) {
    for (const candidate of [
      path.join(packageRoot, "daisyclaw.mjs"),
  ...
  throw new Error("Cannot resolve DaisyClaw CLI executable path for native hook relay");
}
