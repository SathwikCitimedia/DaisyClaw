...
describe("agent harness registry", () => {
  ...
  it("keeps model-specific harnesses behind plugin registration in auto mode", () => {
    ...
    process.env.DAISYCLAW_AGENT_RUNTIME = "auto";
  ...
  it("falls back to DaisyClaw for other models", () => {
    process.env.DAISYCLAW_AGENT_RUNTIME = "auto";
  ...
  it("honors explicit provider DaisyClaw runtime policy", () => {
    registerAgentHarness(makeHarness("plugin-harness", { priority: 200 }), {
      ownerPluginId: "plugin-a",
    });
    ...
    expect(
      selectAgentHarness({
        provider: "codex",
        modelId: "gpt-5.4",
        config: providerRuntimeConfig("codex", "daisyclaw"),
      }).id,
    ).toBe("daisyclaw");
  });
