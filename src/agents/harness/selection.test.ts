...
import { DAISYCLAW_EMBEDDED_CONTEXT_ENGINE_HOST } from "../../context-engine/host-compat.js";
...
vi.mock("./builtin-daisyclaw.js", () => ({
  createDaisyClawAgentHarness: (): AgentHarness => ({
    id: "daisyclaw",
    label: "DaisyClaw embedded agent",
    contextEngineHostCapabilities: DAISYCLAW_EMBEDDED_CONTEXT_ENGINE_HOST.capabilities,
...
const originalRuntime = process.env.DAISYCLAW_AGENT_RUNTIME;
...
afterEach(() => {
  ...
  if (originalRuntime == null) {
    delete process.env.DAISYCLAW_AGENT_RUNTIME;
  } else {
    process.env.DAISYCLAW_AGENT_RUNTIME = originalRuntime;
  }
});

function createAttemptParams(config?: DaisyClawConfig): EmbeddedRunAttemptParams {
  return {
    prompt: "hello",
    sessionId: "session-1",
    runId: "run-1",
...
function groupSenderDenyAllConfig(): DaisyClawConfig {
...
function groupDenyAllConfig(): DaisyClawConfig {
  return {
    channels: {
      telegram: {
        groups: {
...
function providerRuntimeConfig(provider: string, runtime: string): DaisyClawConfig {
  return {
    models: {
      providers: {
        [provider]: {
...
describe("runAgentHarnessAttempt", () => {
  it("fails when a forced plugin harness is unavailable and fallback is omitted", async () => {
    process.env.DAISYCLAW_AGENT_RUNTIME = "codex";
  ...
  it("falls back to the DaisyClaw harness in auto mode when no plugin harness matches", async () => {
    const result = await runAgentHarnessAttempt(createAttemptParams());

    expect(result.sessionIdUsed).toBe("daisyclaw");
    expect(agentRunAttempt).toHaveBeenCalledTimes(1);
  });

  it("allows the selected DaisyClaw harness to satisfy context-engine pre-prompt assembly", async () => {
    const result = await runAgentHarnessAttempt({
      ...createAttemptParams(providerRuntimeConfig("codex", "daisyclaw")),
  ...
  it("surfaces an auto-selected plugin harness failure instead of replaying through DaisyClaw", async () => {
  ...
  it("surfaces a forced plugin harness failure instead of replaying through DaisyClaw", async () => {
    registerFailingCodexHarness();
  ...
  it("falls back to DaisyClaw when the implicit OpenAI Codex harness is unavailable", async () => {
    expect(resolveAgentHarnessPolicy({ provider: "openai", modelId: "gpt-5.4" })).toEqual({
      runtime: "codex",
      runtimeSource: "implicit",
    });
  ...
  it("honors explicit DaisyClaw runtime for OpenAI agent model runs", async () => {
    const result = await runAgentHarnessAttempt({
      ...createAttemptParams(providerRuntimeConfig("openai", "daisyclaw")),
      provider: "openai",
      modelId: "gpt-5.4",
    });
    expect(result.sessionIdUsed).toBe("daisyclaw");
  ...
  it("honors provider wildcard DaisyClaw runtime policy for OpenAI agent model runs", async () => {
    registerSuccessfulCodexHarness();

    const result = await runAgentHarnessAttempt({
      ...createAttemptParams(agentModelRuntimeConfig("openai/*", "daisyclaw")),
      provider: "openai",
      modelId: "gpt-5.4",
    });
    expect(result.sessionIdUsed).toBe("daisyclaw");
  ...
  it("leaves DaisyClaw harness params unchanged for channel group sender deny-all policy", async () => {
    await runAgentHarnessAttempt({
      ...createAttemptParams(groupSenderDenyAllConfig()),
      sessionKey: "agent:main:telegram:group:test-deny-room",
      messageProvider: "telegram",
  ...
  it("does not let a strict agent model plugin runtime fall back to DaisyClaw", async () => {
    await expect(
      runAgentHarnessAttempt({
...
describe("selectAgentHarness", () => {
  ...
  it("ignores session-level DaisyClaw pins when selecting a harness", () => {
    const supports = vi.fn(() => ({ supported: true as const, priority: 100 }));
    registerAgentHarness({
      id: "codex",
      label: "Codex",
  ...
  it("honors explicit DaisyClaw runtime overrides when selecting a harness", async () => {
    registerSuccessfulCodexHarness();

    const harness = selectAgentHarness({
      provider: "openai",
      modelId: "gpt-5.4",
      agentHarnessRuntimeOverride: "daisyclaw",
    });

    expect(harness.id).toBe("daisyclaw");
  ...
  it("treats legacy PI runtime overrides as the built-in DaisyClaw harness", async () => {
    registerSuccessfulCodexHarness();
  ...
  it("selects DaisyClaw when the implicit OpenAI Codex harness is unavailable", () => {
    expect(selectAgentHarness({ provider: "openai", modelId: "gpt-5.4" }).id).toBe("daisyclaw");
  });
  ...
  it("ignores existing session DaisyClaw pins when provider policy forces a plugin harness", () => {
    registerFailingCodexHarness();
  ...
  it("ignores env-forced DaisyClaw for OpenAI default runtime selection", () => {
    process.env.DAISYCLAW_AGENT_RUNTIME = "daisyclaw";
    registerFailingCodexHarness();
      ...
      maybeCompactAgentHarnessSession({
        ...
        config: {
          agents: {
            ...
            defaults: {
              models: {
                "openai/gpt-5.5": { agentRuntime: { id: "daisyclaw" } },
              },
            },
          },
        } as DaisyClawConfig,
  ...
  it("keeps compaction recoverable when auth profile lookup fails", async () => {
    ...
    await expect(
      maybeCompactAgentHarnessSession({
        ...
        agentHarnessId: "codex",
        config: agentModelRuntimeConfig("openai/gpt-5.5", "daisyclaw"),
      }),
    ).resolves.toEqual({ ok: true, compacted: false });
    expect(compact).toHaveBeenCalledTimes(1);
  ...
  it("does not compact a selected plugin harness through DaisyClaw when the plugin has no compactor", async () => {
    registerFailingCodexHarness();
  ...
  it.each([
    ...
    "returns DaisyClaw for explicit CLI runtime alias $alias on $provider instead of throwing MissingAgentHarnessError",
