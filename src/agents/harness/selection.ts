...
import { createDaisyClawAgentHarness } from "./builtin-daisyclaw.js";
...
type AgentHarnessSelectionDecision = {
  ...
  selectedReason:
    | "forced_daisyclaw"
    | "forced_plugin"
    // Implicit Codex preference found no registered Codex harness, so DaisyClaw handled the run.
    | "implicit_plugin_unavailable_daisyclaw"
    // Provider-owned CLI runtime aliases have no agent harness plugin counterpart.
    | "cli_runtime_passthrough_daisyclaw"
    // Auto mode chose a registered plugin harness that supports the provider/model.
    | "auto_plugin"
    // Auto mode found no supporting plugin harness, so DaisyClaw handled the run.
    | "auto_daisyclaw";
...
function selectAgentHarnessDecision(params: {
  ...
  const policy =
    runtimeOverride && !isDefaultAgentRuntimeId(runtimeOverride)
      ...
      : resolvedPolicy;
  // DaisyClaw's built-in harness is intentionally not part of the plugin candidate list. Explicit plugin
  // runtimes fail closed; only `auto` may route an unmatched turn to DaisyClaw.
  const pluginHarnesses = listPluginAgentHarnesses();
  const daisyClawHarness = createDaisyClawAgentHarness();
  const runtime = policy.runtime;
  if (runtime === "daisyclaw") {
    return buildSelectionDecision({
      harness: daisyClawHarness,
      policy,
      selectedReason: "forced_daisyclaw",
  ...
  if (runtime !== "auto") {
    ...
    if (forced) {
      ...
      if (isCliRuntimeAliasForProvider({ runtime, provider: params.provider })) {
        return buildSelectionDecision({
          ...
          selectedReason: "cli_runtime_passthrough_daisyclaw",
          candidates: listHarnessCandidates(pluginHarnesses),
        });
      }
      throw new Error(
    ...
    if (runtime === "codex" && policy.runtimeSource === "implicit") {
      return buildSelectionDecision({
        ...
        selectedReason: "implicit_plugin_unavailable_daisyclaw",
        candidates: listHarnessCandidates(pluginHarnesses),
      });
    }
    if (
...
export async function runAgentHarnessAttempt(
  ...
  const attemptParams =
    harness.id === "daisyclaw" ? params : applyPluginHarnessDenyAllToolPolicy(params);
  ...
  try {
    ...
    log.warn(`${harness.label} failed; not falling back to embedded DaisyClaw backend`, {
