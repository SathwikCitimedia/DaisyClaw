 ...
 */
/** Opening delimiter for protected DaisyClaw runtime context blocks. */
export const INTERNAL_RUNTIME_CONTEXT_BEGIN = "<<<BEGIN_DAISYCLAW_INTERNAL_CONTEXT>>>";
/** Closing delimiter for protected DaisyClaw runtime context blocks. */
export const INTERNAL_RUNTIME_CONTEXT_END = "<<<END_DAISYCLAW_INTERNAL_CONTEXT>>>";

const ESCAPED_INTERNAL_RUNTIME_CONTEXT_BEGIN = "[[DAISYCLAW_INTERNAL_CONTEXT_BEGIN]]";
const ESCAPED_INTERNAL_RUNTIME_CONTEXT_END = "[[DAISYCLAW_INTERNAL_CONTEXT_END]]";

/** Notice inserted into runtime-generated context blocks. */
export const DAISYCLAW_RUNTIME_CONTEXT_NOTICE =
  "This context is runtime-generated, not user-authored. Keep internal details private.";
/** Header for context attached to the immediately preceding user message. */
export const DAISYCLAW_NEXT_TURN_RUNTIME_CONTEXT_HEADER =
  "DaisyClaw runtime context for the immediately preceding user message.";
/** Header for runtime events passed as prompt context. */
export const DAISYCLAW_RUNTIME_EVENT_HEADER = "DaisyClaw runtime event.";
/** Custom message type used for structured runtime-context messages. */
export const DAISYCLAW_RUNTIME_CONTEXT_CUSTOM_TYPE = "daisyclaw.runtime-context";

const LEGACY_INTERNAL_CONTEXT_HEADER =
  ["DaisyClaw runtime context (internal):", DAISYCLAW_RUNTIME_CONTEXT_NOTICE, ""].join("\n") + "\n";
...
function isRuntimeContextPromptHeader(line: string): boolean {
  return (
    line === DAISYCLAW_NEXT_TURN_RUNTIME_CONTEXT_HEADER || line === DAISYCLAW_RUNTIME_EVENT_HEADER
  );
}
...
function stripRuntimeContextPromptPreface(text: string): string {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (
      ...
      nextLine.trim() === DAISYCLAW_RUNTIME_CONTEXT_NOTICE
    ) {
      changed = true;
      index += 1;
...
export function hasInternalRuntimeContext(text: string): boolean {
  ...
  return (
    ...
    text.includes(
      `${DAISYCLAW_NEXT_TURN_RUNTIME_CONTEXT_HEADER}\n${DAISYCLAW_RUNTIME_CONTEXT_NOTICE}`,
    ) ||
    text.includes(`${DAISYCLAW_RUNTIME_EVENT_HEADER}\n${DAISYCLAW_RUNTIME_CONTEXT_NOTICE}`)
  );
}

function isDaisyClawRuntimeContextCustomMessage(message: unknown): boolean {
  if (!message || typeof message !== "object") {
    return false;
  }
  const candidate = message as { role?: unknown; customType?: unknown };
  return (
    candidate.role === "custom" && candidate.customType === DAISYCLAW_RUNTIME_CONTEXT_CUSTOM_TYPE
  );
}

/** Remove all structured runtime-context custom messages. */
export function stripRuntimeContextCustomMessages<T>(messages: T[]): T[] {
  if (!messages.some(isDaisyClawRuntimeContextCustomMessage)) {
    return messages;
  }
  return messages.filter((message) => !isDaisyClawRuntimeContextCustomMessage(message));
}
...
export function stripHistoricalRuntimeContextCustomMessages<T>(messages: T[]): T[] {
  ...
  for (let index = lastUserIndex - 1; index >= 0; index -= 1) {
    if (!isDaisyClawRuntimeContextCustomMessage(messages[index])) {
      break;
    }
    currentRuntimeContextIndexes.add(index);
  }
  return messages.filter((message, index) => {
    if (!isDaisyClawRuntimeContextCustomMessage(message)) {
      return true;
    }
