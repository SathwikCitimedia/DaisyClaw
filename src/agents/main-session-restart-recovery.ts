...
export async function markRestartAbortedMainSessions(params: {
  cfg?: DaisyClawConfig;
  additionalCfgs?: Iterable<DaisyClawConfig | undefined>;
  stateDir?: string;
  ...
  const env =
    params.stateDir === undefined
      ? process.env
      : { ...process.env, DAISYCLAW_STATE_DIR: params.stateDir };
  const stateDir = resolveStateDir(env);
  const configs = [params.cfg, ...(params.additionalCfgs ?? [])].filter(
    (cfg): cfg is DaisyClawConfig => Boolean(cfg),
  );
...
function buildResumeMessage(pendingFinalDeliveryText?: string | null): string {
  const base =
    ...
    "DaisyClaw was waiting on tool/model work. Continue from the existing " +
...
async function resolveRestartRecoveryStorePaths(params: {
  ...
  if (params.cfg) {
    const env = { ...process.env, DAISYCLAW_STATE_DIR: stateDir };
