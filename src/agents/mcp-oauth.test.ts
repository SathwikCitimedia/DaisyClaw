...
describe("MCP OAuth provider", () => {
  it("stores token state under the DaisyClaw state directory with restricted permissions", async () => {
    await withTempHome(
      async (home) => {
        ...
        const tokenDir = `${home}/.daisyclaw/mcp-oauth`;
  ...
  it("isolates token state by configured server URL", async () => {
    await withTempHome(
      ...
      {
        prefix: "daisyclaw-mcp-oauth-url-",
        skipSessionCleanup: true,
        env: {
          DAISYCLAW_CONFIG_PATH: undefined,
          DAISYCLAW_STATE_DIR: undefined,
  ...
  it("does not persist localhost when the fallback attempt fails", async () => {
    await withTempHome(
      async (home) => {
        ...
        await expect(fs.readdir(`${home}/.daisyclaw/mcp-oauth`)).rejects.toThrow();
      },
      {
        prefix: "daisyclaw-mcp-oauth-localhost-failure-",
        skipSessionCleanup: true,
        env: {
          DAISYCLAW_CONFIG_PATH: undefined,
          DAISYCLAW_STATE_DIR: undefined,
  ...
  it("persists localhost redirect for a later code exchange login", async () => {
    await withTempHome(
      ...
      {
        prefix: "daisyclaw-mcp-oauth-localhost-persist-",
        skipSessionCleanup: true,
        env: {
          DAISYCLAW_CONFIG_PATH: undefined,
          DAISYCLAW_STATE_DIR: undefined,
  ...
  it("does not start hidden authorization flows without an authorization callback", async () => {
    ...
    await withTempHome(
      async () => {
        ...
        await expect(provider.state?.()).rejects.toThrow("Run daisyclaw mcp login Remote Docs.");
        await expect(provider.saveCodeVerifier?.("verifier")).rejects.toThrow(
          "Run daisyclaw mcp login Remote Docs.",
        );
        await expect(
          provider.redirectToAuthorization?.(new URL("https://auth.example.com/authorize")),
        ).rejects.toThrow("Run daisyclaw mcp login Remote Docs.");
      },
      {
        prefix: "daisyclaw-mcp-oauth-noninteractive-",
        skipSessionCleanup: true,
        env: {
          DAISYCLAW_CONFIG_PATH: undefined,
          DAISYCLAW_STATE_DIR: undefined,
  ...
  it("clears stored credentials for a configured server URL", async () => {
    await withTempHome(
      ...
      {
        prefix: "daisyclaw-mcp-oauth-clear-",
        skipSessionCleanup: true,
        env: {
          DAISYCLAW_CONFIG_PATH: undefined,
          DAISYCLAW_STATE_DIR: undefined,
