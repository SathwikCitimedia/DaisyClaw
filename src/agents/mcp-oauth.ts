 ...
 * private DaisyClaw state directory with one hashed file per MCP server URL.
 */
...
/** Creates the MCP SDK OAuth provider backed by DaisyClaw's private store. */
export function createMcpOAuthClientProvider(params: {
  ...
  const assertAuthorizationRedirectAllowed = () => {
    if (!allowAuthorizationRedirect) {
      throw new Error(
        `MCP server "${params.serverName}" requires OAuth authorization. Run daisyclaw mcp login ${params.serverName}.`,
      );
    }
  };
  return {
