...
describe("DaisyClawStdioClientTransport", () => {
  afterEach(() => {
    vi.useRealTimers();
    spawnMock.mockReset();
  ...
  it("starts stdio MCP servers in a disposable process group on POSIX", async () => {
    // Detached POSIX process groups let DaisyClaw clean up child tool servers
  ...
  it("kills the process tree when graceful stdio close does not exit", async () => {
    ...
    const transport = new DaisyClawStdioClientTransport({ command: "npx" });
    const started = transport.start();
    child.emit("spawn");
    await started;
