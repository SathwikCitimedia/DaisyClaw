/**
 * DaisyClaw stdio transport wrapper for MCP server subprocesses.
 */
...
export type DaisyClawStdioServerParameters = {
  command: string;
  args?: string[];
...
export class DaisyClawStdioClientTransport implements Transport {
  onclose?: () => void;
  ...
  constructor(private readonly serverParams: DaisyClawStdioServerParameters) {
  ...
  async start(): Promise<void> {
    if (this.process) {
      throw new Error(
        "DaisyClawStdioClientTransport already started; Client.connect() starts transports automatically.",
      );
    }
