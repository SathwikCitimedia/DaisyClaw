...
async function expectVertexAdcEnvApiKey(params: {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), params.tempPrefix ?? "daisyclaw-adc-"));
...
vi.mock("./model-auth-env-vars.js", () => {
  ...
  const resolveProviderEnvAuthEvidence = (params?: { config?: DaisyClawConfig }) => {
    const evidence = {
      "google-vertex": [
        {
  ...
  return {
    ...
    resolveProviderEnvAuthLookupMaps: (params?: { config?: DaisyClawConfig }) => ({
      aliasMap,
...
function buildDemoLocalProviderCfg(apiKey: string): DaisyClawConfig {
  return {
    models: {
      providers: {
        "demo-local": {
...
describe("getApiKeyForModel", () => {
  it("reads oauth auth-profiles entries from auth-profiles.json via explicit profile", async () => {
    await withDaisyClawTestState(
      ...
      async (state) => {
        ...
        const store = ensureAuthProfileStore(process.env.DAISYCLAW_AGENT_DIR, {
  ...
  it("uses the config default agent dir when resolving provider profiles", async () => {
    await withDaisyClawTestState(
      {
        layout: "state-only",
        prefix: "daisyclaw-auth-agent-dir-",
        agentEnv: "clear",
        env: {
  ...
  it("reports the config default agent dir when provider auth is missing", async () => {
    await withDaisyClawTestState(
      {
        layout: "state-only",
        prefix: "daisyclaw-auth-missing-agent-dir-",
        agentEnv: "clear",
        env: {
          XAI_API_KEY: undefined,
        },
  ...
  it("reads Claude CLI credentials when the Claude CLI provider is resolved", async () => {
    ...
    await withDaisyClawTestState(
      {
        layout: "state-only",
        prefix: "daisyclaw-auth-claude-cli-",
        agentEnv: "main",
      },
      async () => {
  ...
  it("uses trusted workspace manifest auth evidence in runtime auth checks", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-workspace-cloud-auth-"));
  ...
  it("resolveEnvApiKey('google-vertex') accepts ADC credentials from the provided env snapshot", async () => {
    await expectVertexAdcEnvApiKey({
      ...
      credentialsJson: "{}",
      tempPrefix: "daisyclaw-google-adc-",
      env: {
  ...
  it("resolveEnvApiKey('google-vertex') accepts Unicode explicit ADC credential paths", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-google-adc-unicode-"));
  ...
  it("resolveEnvApiKey('google-vertex') accepts Unicode ADC fallback home paths", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-google-adc-home-"));
  ...
  it("resolveEnvApiKey('google-vertex') rejects GOOGLE_CLOUD_PROJECT_ID-only ADC auth evidence", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-google-adc-project-id-"));
    const credentialsPath = path.join(tempDir, "adc.json");
    await fs.writeFile(credentialsPath, "{}", "utf8");
  ...
  it("resolveEnvApiKey('google-vertex') accepts Windows APPDATA ADC fallback evidence", async () => {
    const appDataDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-google-adc-appdata-"));
  ...
  it("resolveEnvApiKey('google-vertex') does not synthesize APPDATA from USERPROFILE", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-google-adc-home-"));
    const userProfileDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "daisyclaw-google-adc-userprofile-"),
    );
  ...
  it("resolveEnvApiKey('google-vertex') keeps ADC fallback when manifest env candidates are empty", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-google-adc-candidates-"));
    const credentialsPath = path.join(tempDir, "adc.json");
    await fs.writeFile(credentialsPath, "{}", "utf8");
