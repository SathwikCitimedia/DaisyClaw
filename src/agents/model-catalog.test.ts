...
let prepareDaisyClawModelsJsonSourceMock: ReturnType<typeof vi.fn>;
...
describe("loadModelCatalog", () => {
  beforeAll(async () => {
    ...
    prepareDaisyClawModelsJsonSourceMock = vi.fn().mockResolvedValue({
      agentDir: "/tmp/daisyclaw",
    ...
    vi.doMock("./models-config.js", () => ({
      ...
      prepareDaisyClawModelsJsonSource: prepareDaisyClawModelsJsonSourceMock,
    }));
    ...
    vi.doMock("./agent-scope.js", () => ({
      resolveAgentWorkspaceDir: (cfg: DaisyClawConfig, agentId: string) => {
        const entry = cfg.agents?.list?.find((entryEntry) => entryEntry.id === agentId);
        return entry?.workspace ?? cfg.agents?.defaults?.workspace ?? "/tmp/daisyclaw-workspace";
      },
      resolveDefaultAgentDir: () => "/tmp/daisyclaw",
      resolveDefaultAgentId: (cfg: DaisyClawConfig) =>
  ...
  beforeEach(() => {
    ...
    prepareDaisyClawModelsJsonSourceMock.mockReset();
    prepareDaisyClawModelsJsonSourceMock.mockResolvedValue({
      agentDir: "/tmp/daisyclaw",
      fingerprint: "source-fingerprint",
      workspaceDir: "/tmp/daisyclaw-workspace",
      wrote: false,
  ...
  it("uses the state cached catalog before runtime discovery", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as DaisyClawConfig });
    ...
    expect(prepareDaisyClawModelsJsonSourceMock).not.toHaveBeenCalled();
  ...
  it("bypasses the state cached catalog when a refresh is requested", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as DaisyClawConfig, useCache: false });
  ...
  it("misses the state cached catalog when source freshness changes", async () => {
    ...
    await expect(loadModelCatalog({ config: {} as DaisyClawConfig })).resolves.toEqual([
      { id: "cached-stale", name: "Cached Stale", provider: "openai" },
    ]);
    resetModelCatalogCacheForTest();
    mockAgentDiscoveryModels([{ id: "fresh-fast", name: "Fresh Fast", provider: "openai" }]);
  ...
  it("reloads dynamic registry entries after clearing the cache", async () => {
    ...
    const first = await loadModelCatalog({ config: {} as DaisyClawConfig });
    expect(first).toStrictEqual([
      {
        id: "existing",
        name: "Existing",
    ...
    const second = await loadModelCatalog({ config: {} as DaisyClawConfig });
  ...
  it("filters suppressed built-ins from persisted read-only catalog rows", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as DaisyClawConfig, readOnly: true });
  ...
  it("loads generated plugin catalog rows in read-only mode", async () => {
    const catalogPath = "/tmp/daisyclaw/plugins/read-only-shard/catalog.json";
    mkdirSync("/tmp/daisyclaw/plugins/read-only-shard", { recursive: true });
    ...
    try {
      ...
      rmSync("/tmp/daisyclaw/plugins/read-only-shard", { recursive: true, force: true });
    }
  });
  ...
  it("loads manifest catalog rows from the current metadata snapshot without provider runtime", () => {
    ...
    const result = loadManifestModelCatalog({ config: {} as DaisyClawConfig });
  ...
  it("passes explicit env when checking current manifest catalog snapshot compatibility", () => {
    const env = { HOME: "/tmp/daisyclaw-model-catalog-env" } as NodeJS.ProcessEnv;
