...
import { resolveClaudeFable5ModelIdentity } from "@daisyclaw/llm-core";
...
import {
  ...
  prepareDaisyClawModelsJsonSource,
...
let manifestModelCatalogCache = new WeakMap<DaisyClawConfig, ManifestModelCatalogCacheEntry>();
...
function shouldLogModelCatalogTiming(): boolean {
  return process.env.DAISYCLAW_DEBUG_INGRESS_TIMING === "1";
}
...
function hasConfiguredProviderRowsNeedingManifestLookup(cfg: DaisyClawConfig): boolean {
...
export async function loadModelCatalog(params?: {
  ...
  const loadCatalog = async () => {
    ...
    try {
      ...
      if (!readOnly) {
        const preparedSource = await prepareDaisyClawModelsJsonSource(cfg, agentDir, {
          pluginMetadataSnapshot: params?.metadataSnapshot,
          workspaceDir,
        });
