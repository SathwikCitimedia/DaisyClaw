    ...
    plugins: [
      {
        ...
        manifestPath: `/tmp/fallback-normalizer-${params.manifestHash}/daisyclaw.plugin.json`,
...
function makeFallbacksOnlyCfg(): DaisyClawConfig {
  return {
    agents: {
      defaults: {
        model: {
...
function makeProviderFallbackCfg(provider: string): DaisyClawConfig {
  return makeCfg({
    agents: {
      defaults: {
        model: {
...
async function makeAuthTempDir(): Promise<string> {
  authTempRoot ||= path.join("/tmp", "daisyclaw-auth-suite-mock");
...
// https://github.com/daisyclaw/daisyclaw/issues/23440
const INSUFFICIENT_QUOTA_PAYLOAD =
  '{"type":"error","error":{"type":"insufficient_quota","message":"Your account has insufficient quota balance to run this request."}}';

describe("runWithModelFallback", () => {
  it("uses the opt-in auth skip cache on the second turn for the same session", async () => {
    const previous = process.env.DAISYCLAW_FALLBACK_SKIP_TTL_MS;
    process.env.DAISYCLAW_FALLBACK_SKIP_TTL_MS = "60000";
    try {
      const cfg = makeCfg({
        agents: {
          defaults: {
      ...
      if (previous === undefined) {
        delete process.env.DAISYCLAW_FALLBACK_SKIP_TTL_MS;
      } else {
        process.env.DAISYCLAW_FALLBACK_SKIP_TTL_MS = previous;
      }
    }
  });
  ...
  it("does not prepare agent harness plugins for forced DaisyClaw candidates", async () => {
    const cfg = makeCfg({
      models: {
        providers: {
          openai: {
    ...
    const prepareAgentHarnessRuntime = vi.fn(() => {
      throw new Error("DaisyClaw candidates should not prepare plugin harnesses");
    });
    const run = vi.fn().mockResolvedValueOnce("ok");
  ...
  it("does not prepare agent harness plugins for forced DaisyClaw runtime candidates", async () => {
    const cfg = makeCfg({
      models: {
        providers: {
          openai: {
  ...
  it("aborts the fallback chain on session write-lock timeout instead of trying every model (#83510)", async () => {
    ...
    const lockError = new SessionWriteLockTimeoutError({
      timeoutMs: 10_000,
      owner: "pid=37121",
      lockPath: "/tmp/daisyclaw/session.jsonl.lock",
    });
  ...
  it("warns when falling back due to model_not_found", async () => {
    const warnLogs = createWarnLogCapture("daisyclaw-model-fallback-test");
    try {
      const cfg = makeCfg();
      const run = vi
        .fn()
