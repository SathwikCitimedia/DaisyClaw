...
function hasConfiguredProvider(params: { cfg?: DaisyClawConfig; provider: string }): boolean {
  const normalizedProvider = normalizeProviderId(params.provider);
...
function isCliAgentRuntime(runtime: string | undefined, cfg: DaisyClawConfig | undefined): boolean {
...
async function resolveModelFallbackCandidateHarnessAuthPrecheck(
  ...
  // Explicit non-Codex plugin harnesses own transport/auth; stale DaisyClaw
...
export function resolveImageFallbackDefaultProvider(cfg: DaisyClawConfig | undefined): string {
...
function resolveFallbackCandidateModelProviderCacheParts(cfg: DaisyClawConfig | undefined): unknown {
