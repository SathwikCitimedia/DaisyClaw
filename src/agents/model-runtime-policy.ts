...
export function resolveModelRuntimePolicy(params: {
  ...
  if (process.env.DAISYCLAW_BUILD_PRIVATE_QA === "1") {
    const forcedRuntime = process.env.DAISYCLAW_QA_FORCE_RUNTIME?.trim().toLowerCase();
    if (forcedRuntime === "daisyclaw" || forcedRuntime === "codex") {
