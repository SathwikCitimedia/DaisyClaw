...
function resolveAnthropicOpusThinking(cfg: DaisyClawConfig) {
...
function resolveAnthropicOpus47Thinking(cfg: DaisyClawConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic",
...
function resolveAnthropicOpus48Thinking(cfg: DaisyClawConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic",
...
function resolveAnthropicVertexOpus48Thinking(cfg: DaisyClawConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic-vertex",
    model: "claude-opus-4-8",
...
function resolveClaudeCliOpus48Thinking(cfg: DaisyClawConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "claude-cli",
    model: "claude-opus-4-8",
...
function resolveConfiguredRefForTest(cfg: Partial<DaisyClawConfig>) {
  return resolveConfiguredModelRef({
    cfg: cfg as DaisyClawConfig,
...
describe("model-selection", () => {
  ...
  describe("buildModelAliasIndex", () => {
    it("should build alias index from config", () => {
      const cfg: Partial<DaisyClawConfig> = {
        agents: {
          defaults: {
            models: {
  ...
  describe("resolveConfiguredModelRef", () => {
    ...
    it("should fall back to the configured default provider and warn if provider is missing for non-alias", async () => {
      const warnLogs = createWarnLogCapture("daisyclaw-model-selection-test");
      try {
        const cfg: Partial<DaisyClawConfig> = {
          agents: {
            defaults: {
