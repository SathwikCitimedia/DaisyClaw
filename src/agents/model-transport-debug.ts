 ...
 * interpret DaisyClaw debug environment variables consistently.
 */
...
/** Payload debug detail levels accepted by `DAISYCLAW_DEBUG_MODEL_PAYLOAD`. */
export type ModelPayloadDebugMode = "off" | "summary" | "tools" | "full-redacted";
/** SSE debug detail levels accepted by `DAISYCLAW_DEBUG_SSE`. */
...
/** Resolves model payload debug verbosity from `DAISYCLAW_DEBUG_MODEL_PAYLOAD`. */
export function resolveModelPayloadDebugMode(
  env: ModelTransportDebugEnv = process.env,
): ModelPayloadDebugMode {
  const normalized = normalizeEnv(env.DAISYCLAW_DEBUG_MODEL_PAYLOAD);
...
/** Resolves SSE stream debug verbosity from `DAISYCLAW_DEBUG_SSE`. */
export function resolveModelSseDebugMode(
  env: ModelTransportDebugEnv = process.env,
): ModelSseDebugMode {
  const normalized = normalizeEnv(env.DAISYCLAW_DEBUG_SSE);
  if (normalized === "peek") {
    return "peek";
  }
...
export function isModelTransportDebugEnabled(env: ModelTransportDebugEnv = process.env): boolean {
  return (
    isTruthyEnv(env.DAISYCLAW_DEBUG_MODEL_TRANSPORT) ||
    resolveModelPayloadDebugMode(env) !== "off" ||
    resolveModelSseDebugMode(env) !== "off" ||
    isTruthyEnv(env.DAISYCLAW_DEBUG_CODE_MODE)
  );
}
