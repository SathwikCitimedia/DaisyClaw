...
async function resolveProvidersForConfigEnvTest(params: {
  ...
  return await resolveProvidersForModelsJsonWithDeps(
    {
      cfg: params.cfg,
      agentDir: "/tmp/daisyclaw-models-config-env-vars-test",
      env,
    },
    {
...
function createConfigEnvVarsConfig(): DaisyClawConfig {
  return {
    models: { providers: {} },
    env: {
      vars: {
...
async function resolveProvidersAndCaptureDiscoveryEnv(cfg: DaisyClawConfig) {
...
let unauthenticatedProviderWritePlan: Awaited<ReturnType<typeof planDaisyClawModelsJsonWithDeps>>;
...
beforeAll(async () => {
  ...
  unauthenticatedProviderWritePlan = await planDaisyClawModelsJsonWithDeps(
    {
...
describe("models-config", () => {
  it("keeps the implicit provider catalog when explicit baseUrl is blank", async () => {
    let observedConfig: DaisyClawConfig | undefined;
  ...
  it("threads workspace scope into implicit provider discovery", async () => {
    ...
    expect(observedWorkspaceDir).toBe("/tmp/daisyclaw-workspace");
  });
  ...
  it("threads plugin metadata snapshots through models.json planning", async () => {
    ...
    await planDaisyClawModelsJsonWithDeps(
      {
        cfg: { models: { providers: {} } },
        agentDir: "/tmp/daisyclaw-models-config-env-vars-test",
        env: {},
  ...
  it("keeps google-vertex static catalog rows when an auth profile supplies the API key", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-google-vertex-models-"));
    try {
  ...
  it("keeps google-vertex static catalog rows when ADC auth evidence supplies the marker", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-google-vertex-adc-models-"));
