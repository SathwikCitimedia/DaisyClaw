...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousDaisyClawAgentDir: string | undefined;
  const originalFetch = globalThis.fetch;
  const shouldResetPluginLoaderState = opts?.resetPluginLoaderState !== false;

  beforeEach(() => {
    previousHome = process.env.HOME;
    previousDaisyClawAgentDir = process.env.DAISYCLAW_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousDaisyClawAgentDir === undefined) {
      delete process.env.DAISYCLAW_AGENT_DIR;
    } else {
      process.env.DAISYCLAW_AGENT_DIR = previousDaisyClawAgentDir;
    }
    clearRuntimeConfigSnapshot();
    clearConfigCache();
    if (shouldResetPluginLoaderState) {
...
export const MODELS_CONFIG_IMPLICIT_ENV_VARS = [
  "DAISYCLAW_TEST_ONLY_PROVIDER_PLUGIN_IDS",
  "VITEST",
  "NODE_ENV",
  "AI_GATEWAY_API_KEY",
...
export const CUSTOM_PROXY_MODELS_CONFIG: DaisyClawConfig = {
  models: {
    providers: {
      "custom-proxy": {
