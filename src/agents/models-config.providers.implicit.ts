...
function resolveLiveProviderCatalogTimeoutMs(env: NodeJS.ProcessEnv): number | null {
  const live =
    env.DAISYCLAW_LIVE_TEST === "1" || env.DAISYCLAW_LIVE_GATEWAY === "1" || env.LIVE === "1";
  if (!live) {
    return null;
  }
  const raw = env.DAISYCLAW_LIVE_PROVIDER_DISCOVERY_TIMEOUT_MS?.trim();
  if (!raw) {
    return 15_000;
  }
...
function resolveProviderDiscoveryFilter(params: {
  ...
  const testRaw = env.DAISYCLAW_TEST_ONLY_PROVIDER_PLUGIN_IDS?.trim();
  if (testRaw) {
  ...
  const rawValues = [
    env.DAISYCLAW_LIVE_PROVIDERS?.trim(),
    env.DAISYCLAW_LIVE_GATEWAY_PROVIDERS?.trim(),
...
function buildPluginCatalogConfig(ctx: ImplicitProviderContext): DaisyClawConfig {
...
): Promise<NonNullable<DaisyClawConfig["models"]>["providers"]> {
