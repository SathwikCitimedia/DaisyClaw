...
describe("normalizeProviders", () => {
  const createModel = (
    overrides: Partial<
      NonNullable<NonNullable<DaisyClawConfig["models"]>["providers"]>[string]["models"][number]
    > = {},
  ) => ({
  ...
  it("trims provider keys so image models remain discoverable for custom providers", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-agent-"));
    try {
      const providers: NonNullable<NonNullable<DaisyClawConfig["models"]>["providers"]> = {
        " dashscope-vision ": {
  ...
  it("normalizes retired Google Gemini model ids before emitting provider config", async () => {
    ...
    try {
      const providers: NonNullable<NonNullable<DaisyClawConfig["models"]>["providers"]> = {
        ...
        "google-gemini-cli": {
          baseUrl: "daisyclaw://google-gemini-cli",
          models: [
            createModel({
              id: "gemini-3-pro-preview",
  ...
  it("replaces resolved env var value with env var name to prevent plaintext persistence", async () => {
    ...
    const env = {
      ...
      DAISYCLAW_TEST_MINIMAL_GATEWAY: undefined,
    };
  ...
  it("ignores non-object provider entries during source-managed enforcement", () => {
    ...
    } as unknown as NonNullable<NonNullable<DaisyClawConfig["models"]>["providers"]>;

    const sourceProviders: NonNullable<NonNullable<DaisyClawConfig["models"]>["providers"]> = {
      openai: {
        baseUrl: "https://api.openai.com/v1",
        api: "openai-completions",
