...
vi.mock("../plugins/provider-runtime.js", () => ({
  applyProviderConfigDefaultsWithPlugin: (config: DaisyClawConfig) => config,
...
let ensureDaisyClawModelsJson: typeof import("./models-config.js").ensureDaisyClawModelsJson;
let resetModelsJsonReadyCacheForTest: typeof import("./models-config.js").resetModelsJsonReadyCacheForTest;
let planDaisyClawModelsJsonWithDeps: typeof import("./models-config.plan.js").planDaisyClawModelsJsonWithDeps;
let readGeneratedModelsJson: typeof import("./models-config.test-utils.js").readGeneratedModelsJson;
const fixtureSuite = createFixtureSuite("daisyclaw-models-runtime-source-");

beforeAll(async () => {
  await fixtureSuite.setup();
  ({ clearConfigCache, clearRuntimeConfigSnapshot, setRuntimeConfigSnapshot } =
    await import("../config/io.js"));
  ({ ensureDaisyClawModelsJson, resetModelsJsonReadyCacheForTest } =
    await import("./models-config.js"));
  ({ planDaisyClawModelsJsonWithDeps } = await import("./models-config.plan.js"));
...
function createOpenAiApiKeySourceConfig(): DaisyClawConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiApiKeyRuntimeConfig(): DaisyClawConfig {
...
function createCustomProviderApiKeySourceConfig(): DaisyClawConfig {
  return {
    models: {
      providers: {
        litellm: {
          ...
          apiKey: {
            source: "env",
            provider: "default",
            id: "DAISYCLAW_MODEL_LITELLM_API_KEY", // pragma: allowlist secret
          },
          api: "openai-completions" as const,
          models: [],
        },
...
function createCustomProviderApiKeyRuntimeConfig(): DaisyClawConfig {
  return {
    models: {
      providers: {
        litellm: {
...
function createOpenAiHeaderSourceConfig(): DaisyClawConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiHeaderRuntimeConfig(): DaisyClawConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiSourceConfigWithHeadersAndApiKey(): DaisyClawConfig {
...
function createOpenAiRuntimeConfigWithHeadersAndApiKey(): DaisyClawConfig {
...
function withGatewayTokenMode(config: DaisyClawConfig): DaisyClawConfig {
  return {
    ...config,
    gateway: {
      auth: {
...
async function planGeneratedProviders(params: {
  ...
  sourceConfigForSecrets: DaisyClawConfig;
}) {
  // Planner assertions avoid filesystem noise for marker-projection cases.
  const plan = await planDaisyClawModelsJsonWithDeps(
    {
      cfg: params.config,
      sourceConfigForSecrets: params.sourceConfigForSecrets,
      agentDir: "/tmp/daisyclaw-models-plan",
      env: {},
      existingRaw: "",
      existingParsed: null,
    },
...
describe("models-config runtime source snapshot", () => {
  it("uses runtime source snapshot markers when passed the active runtime config", () => {
    const sourceConfig: DaisyClawConfig = {
      models: {
        providers: {
    ...
    const runtimeConfig: DaisyClawConfig = {
      models: {
        providers: {
  ...
  it("projects cloned runtime configs onto source snapshot when preserving provider auth", async () => {
    ...
    await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
      ...
      const clonedRuntimeConfig: DaisyClawConfig = {
        ...runtimeConfig,
        agents: {
          defaults: {
      ...
      try {
        ...
        await ensureDaisyClawModelsJson(clonedRuntimeConfig, agentDir);
  ...
  it("preserves source markers for custom-provider api keys after models status secret resolution", async () => {
    ...
    await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
      ...
      try {
        setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
        await ensureDaisyClawModelsJson(runtimeConfig, agentDir);
        await expectGeneratedProviderApiKey(agentDir, "litellm", "DAISYCLAW_MODEL_LITELLM_API_KEY"); // pragma: allowlist secret
      } finally {
        clearRuntimeConfigSnapshot();
        clearConfigCache();
      }
  ...
  it("invalidates cached readiness when projected config changes under the same runtime snapshot", async () => {
    ...
    await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
      ...
      const sourceConfig = createOpenAiApiKeySourceConfig();
      const runtimeConfig = createOpenAiApiKeyRuntimeConfig();
      const firstCandidate: DaisyClawConfig = {
        ...runtimeConfig,
        models: {
          providers: {
            openai: {
              ...
              baseUrl: "https://api.openai.com/v1",
              headers: {
                "X-DaisyClaw-Test": "one",
              },
            },
          },
        },
      };
      const secondCandidate: DaisyClawConfig = {
        ...runtimeConfig,
        models: {
          providers: {
            openai: {
      ...
      try {
        setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
        await ensureDaisyClawModelsJson(firstCandidate, agentDir);
        ...
        expect(parsed.providers.openai?.headers?.["X-DaisyClaw-Test"]).toBe("one");

        // Header changes still rewrite models.json, but merge mode preserves the existing baseUrl.
        await ensureDaisyClawModelsJson(secondCandidate, agentDir);
        ...
        expect(parsed.providers.openai?.baseUrl).toBe("https://api.openai.com/v1");
        expect(parsed.providers.openai?.apiKey).toBe("OPENAI_API_KEY"); // pragma: allowlist secret
        expect(parsed.providers.openai?.headers?.["X-DaisyClaw-Test"]).toBe("two");
      } finally {
        clearRuntimeConfigSnapshot();
        clearConfigCache();
      }
