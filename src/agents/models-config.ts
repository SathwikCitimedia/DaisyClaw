...
import { planDaisyClawModelsJson } from "./models-config.plan.js";
import {
...
export type PreparedDaisyClawModelsJsonSource = ModelsJsonReadyResult & {
  fingerprint: string;
  workspaceDir?: string;
};

type EnsureDaisyClawModelsJsonOptions = {
...
function resolveModelsConfigInput(config?: DaisyClawConfig): {
  config: DaisyClawConfig;
  sourceConfigForSecrets: DaisyClawConfig;
} {
...
export async function prepareDaisyClawModelsJsonSource(
  config?: DaisyClawConfig,
  agentDirOverride?: string,
  options: EnsureDaisyClawModelsJsonOptions = {},
): Promise<PreparedDaisyClawModelsJsonSource> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  ...
  const pending: Promise<ModelsJsonReadyState> = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planDaisyClawModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
...
/** Ensures models.json and plugin catalog sidecars are current for an agent. */
export async function ensureDaisyClawModelsJson(
  config?: DaisyClawConfig,
  agentDirOverride?: string,
  options: EnsureDaisyClawModelsJsonOptions = {},
): Promise<ModelsJsonReadyResult> {
  const prepared = await prepareDaisyClawModelsJsonSource(config, agentDirOverride, options);
