...
const planDaisyClawModelsJsonMock = vi.fn();
...
function planParamsAt(callIndex: number): {
  ...
  // Planner call shape is the contract between ensureDaisyClawModelsJson and planning.
  const call = planDaisyClawModelsJsonMock.mock.calls[callIndex];
  if (!call) {
...
beforeAll(async () => {
  vi.doMock("./models-config.plan.js", () => ({
    planDaisyClawModelsJson: (...args: unknown[]) => planDaisyClawModelsJsonMock(...args),
  }));
  ...
  ({ ensureDaisyClawModelsJson } = await import("./models-config.js"));
...
describe("models-config write serialization", () => {
  it("does not reuse default workspace plugin metadata for explicit agent dirs without workspace", async () => {
    await withModelsTempHome(async (home) => {
      ...
      await ensureDaisyClawModelsJson({}, agentDir);
  ...
  it("reuses current plugin metadata for explicit agent dirs with matching workspace", async () => {
    await withModelsTempHome(async (home) => {
      ...
      await ensureDaisyClawModelsJson({}, agentDir, { workspaceDir });
  ...
  it("writes implicit models.json into the configured default agent dir", async () => {
    await withModelsTempHome(async (home) => {
      ...
      const result = await ensureDaisyClawModelsJson(cfg);

      expect(result.agentDir).toBe(path.join(home, ".daisyclaw", "agents", "ops", "agent"));
      await expect(fs.access(path.join(result.agentDir, "models.json"))).resolves.toBeUndefined();
      await expectMissingPath(
        fs.access(path.join(home, ".daisyclaw", "agents", "main", "agent", "models.json")),
      );
    });
  });
  ...
  it("writes plugin-owned model catalogs beside the agent plugin state", async () => {
    await withModelsTempHome(async (home) => {
      ...
      planDaisyClawModelsJsonMock.mockImplementation(async () => ({
        action: "write",
  ...
  it("removes stale plugin-owned model catalogs", async () => {
    await withModelsTempHome(async (home) => {
      ...
      const result = await ensureDaisyClawModelsJson({}, agentDir);
  ...
  it("keeps generated plugin catalogs on non-authoritative skip plans", async () => {
    await withModelsTempHome(async (home) => {
      ...
      planDaisyClawModelsJsonMock.mockImplementation(async () => ({ action: "skip" }));
  ...
  it("does not reuse scoped startup discovery cache for a different provider scope", async () => {
    await withModelsTempHome(async (home) => {
      ...
      expect(planDaisyClawModelsJsonMock).toHaveBeenCalledTimes(2);
  ...
  it("keeps the ready cache warm after models.json is written", async () => {
    await withModelsTempHome(async () => {
      await ensureDaisyClawModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
      await ensureDaisyClawModelsJson(CUSTOM_PROXY_MODELS_CONFIG);

      expect(planDaisyClawModelsJsonMock).toHaveBeenCalledTimes(1);
    });
  });
  ...
  it("keeps distinct config fingerprints cached without evicting each other", async () => {
    await withModelsTempHome(async () => {
      planDaisyClawModelsJsonMock.mockImplementation(async () => ({ action: "noop" }));
      ...
      await ensureDaisyClawModelsJson(first);
      await ensureDaisyClawModelsJson(second);
      await ensureDaisyClawModelsJson(first);
  ...
  it("serializes concurrent models.json writes to avoid overlap", async () => {
    await withModelsTempHome(async () => {
      ...
      const writes = Promise.all([
        ensureDaisyClawModelsJson(first),
        ensureDaisyClawModelsJson(second),
      ]);
