...
import type { Api, Model } from "daisyclaw/plugin-sdk/llm";
...
import {
  ...
  createDaisyClawTransportStreamFnForModel,
...
describe("openai transport stream", () => {
  ...
  it("tags Responses encrypted reasoning with replay provenance while streaming", async () => {
    ...
    const thinkingBlock = output.content[0] as {
      ...
      daisyclawReasoningReplay?: unknown;
    };
    ...
    expect(replayItem).not.toHaveProperty("__daisyclaw_replay");
    expect(thinkingBlock.daisyclawReasoningReplay).toEqual(expectedReplayMetadata);
  });
  ...
  it("summarizes model payload tools with full names when requested", () => {
    const previous = process.env.DAISYCLAW_DEBUG_MODEL_PAYLOAD;
    process.env.DAISYCLAW_DEBUG_MODEL_PAYLOAD = "tools";
    try {
      expect(
      ...
      if (previous === undefined) {
        delete process.env.DAISYCLAW_DEBUG_MODEL_PAYLOAD;
      } else {
        process.env.DAISYCLAW_DEBUG_MODEL_PAYLOAD = previous;
      }
    }
  });
  ...
  it("redacts full model payload debug summaries", () => {
    const previous = process.env.DAISYCLAW_DEBUG_MODEL_PAYLOAD;
    process.env.DAISYCLAW_DEBUG_MODEL_PAYLOAD = "full-redacted";
    try {
  ...
  it("enforces the code mode responses tool surface before requests leave DaisyClaw", () => {
    const payload = {
      tools: [
        { type: "function", name: "exec" },
  ...
  it("adds DaisyClaw attribution to native OpenAI transport headers and protects it from provider overrides", () => {
    vi.stubEnv("DAISYCLAW_VERSION", "2026.3.22");
    const headers = testing.buildOpenAIClientHeaders(
      {
        ...
        headers: {
          originator: "daisyclaw",
          "User-Agent": "daisyclaw",
          "X-Provider": "model",
        },
        reasoning: true,
        input: ["text"],
    ...
    expectRecordFields(headers, {
      originator: "daisyclaw",
      version: "2026.3.22",
      "User-Agent": "daisyclaw/2026.3.22",
      "X-Provider": "model",
      "X-Caller": "request",
    });
  });

  it("adds DaisyClaw attribution to native OpenAI Codex transport headers", () => {
    vi.stubEnv("DAISYCLAW_VERSION", "2026.3.22");
    const headers = testing.buildOpenAIClientHeaders(
      {
        id: "gpt-5.4-codex",
  ...
  it("prepares a custom simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe("daisyclaw-openai-responses-transport");
    expectRecordFields(prepared, {
      api: "daisyclaw-openai-responses-transport",
      provider: "openai",
      id: "gpt-5.4",
    });
  ...
  it("prepares an Anthropic simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe("daisyclaw-anthropic-messages-transport");
    expectRecordFields(prepared, {
      api: "daisyclaw-anthropic-messages-transport",
      provider: "anthropic",
      id: "claude-sonnet-4-6",
    });
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
  ...
  it("reports the Google simple-completion api alias without loading provider runtime", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe(
      "daisyclaw-google-generative-ai-transport",
    );
  });
  ...
  it("uses top-level instructions for Codex responses and preserves prompt cache identity", () => {
    const params = buildOpenAIResponsesParams(
      ...
      {
        daisyclaw_session_id: "session-123",
        daisyclaw_turn_id: "turn-123",
      },
    ) as Record<string, unknown> & {
  ...
  it("keeps Codex response shaping when simple completions use the DaisyClaw transport alias", () => {
    const params = buildOpenAIResponsesParams(
      {
        id: "gpt-5.5",
        name: "GPT-5.5",
        api: "daisyclaw-openai-responses-transport" as Api,
        provider: "openai",
        baseUrl: "https://chatgpt.com/backend-api/codex",
        reasoning: true,
        input: ["text"],
  ...
  it("sanitizes Codex responses params after payload hooks mutate them without stripping cache identity", () => {
    const payload = {
      ...
      metadata: { daisyclaw_session_id: "session-123" },
  ...
  it("omits prior Responses replay item ids when store is disabled for custom Codex-compatible responses", () => {
            ...
            content: [
              {
                ...
                daisyclawReasoningReplay: testing.buildOpenAIResponsesReasoningReplayMetadata(
                  model,
                  {
                    authProfileId: "openai:oauth",
                    sessionId: "session-123",
    ...
    expect(reasoningItem?.id).toBeUndefined();
    expect(reasoningItem).not.toHaveProperty("__daisyclaw_replay");
    const assistantMessage = params.input?.find(
      (item) => item.type === "message" && item.role === "assistant",
    );
    expectRecordFields(assistantMessage, {
  ...
  it("adds native OpenAI turn metadata on direct Responses routes", () => {
    const params = buildOpenAIResponsesParams(
      ...
      { sessionId: "session-123" } as never,
      {
        daisyclaw_session_id: "session-123",
        daisyclaw_turn_id: "turn-123",
        daisyclaw_turn_attempt: "1",
        daisyclaw_transport: "stream",
      },
