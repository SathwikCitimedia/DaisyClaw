 ...
 * provider-specific payload policy before converting SDK streams into DaisyClaw assistant events.
 */
...
const OPENAI_RESPONSES_REASONING_REPLAY_META_KEY = "__daisyclaw_replay";
const OPENAI_RESPONSES_REASONING_REPLAY_BLOCK_META_KEY = "daisyclawReasoningReplay";
...
type BaseStreamOptions = {
  ...
  daisyclawCodeModeToolSurface?: boolean;
...
function createResponsesFirstEventTimeoutError(model: Model, timeoutMs: number): Error {
  return new Error(
    `Azure OpenAI Responses stream did not deliver a first event within ${timeoutMs}ms after HTTP streaming headers. ` +
      ...
      "The provider may be stalled while parsing the tool payload; retry with a smaller tool surface or enable DAISYCLAW_DEBUG_MODEL_PAYLOAD=tools to inspect exposed tools.",
  );
}
  ...
  return (model, context, options) => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (
          (options as { daisyclawCodeModeToolSurface?: unknown } | undefined)
            ?.daisyclawCodeModeToolSurface === true
        ) {
...
function resolveCacheRetention(cacheRetention: string | undefined): "short" | "long" | "none" {
  ...
  if (typeof process !== "undefined" && process.env.DAISYCLAW_CACHE_RETENTION === "long") {
    return "long";
  }
  return "short";
}
...
function isOpenAICodexResponsesModel(model: Model): boolean {
  return (
    ...
    (model.api === "openai-chatgpt-responses" ||
      model.api === "daisyclaw-openai-responses-transport")
  );
}
...
function getCompletionsContentDeltas(content: unknown): CompletionsReasoningDelta[] {
  ...
  // Preserve provider reasoning as DaisyClaw thinking blocks so channel/UI
