...
import { normalizeTrimmedStringList } from "@daisyclaw/normalization-core/string-normalization";
import { listDaisyClawPluginManifestMetadata } from "../plugins/manifest-metadata-scan.js";
...
const DAISYCLAW_ATTRIBUTION_PRODUCT = "DaisyClaw";
const DAISYCLAW_ATTRIBUTION_ORIGINATOR = "daisyclaw";
...
function formatDaisyClawUserAgent(version: string): string {
  return `${DAISYCLAW_ATTRIBUTION_ORIGINATOR}/${version}`;
}
...
function collectManifestProviderEndpoints(): ManifestProviderEndpointCacheEntry[] {
  const entries: ManifestProviderEndpointCacheEntry[] = [];
  for (const { manifest } of listDaisyClawPluginManifestMetadata()) {
...
export function resolveProviderAttributionIdentity(
  ...
  return {
    product: DAISYCLAW_ATTRIBUTION_PRODUCT,
...
function buildOpenRouterAttributionPolicy(
  ...
  return {
    ...
    reviewNote: "Documented app attribution headers. Verified in DaisyClaw runtime wrapper.",
    ...identity,
    headers: {
...
function buildNvidiaAttributionPolicy(
  ...
  return {
    ...
    headers: {
      "X-BILLING-INVOKE-ORIGIN": DAISYCLAW_ATTRIBUTION_PRODUCT,
    },
  };
}
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      originator: DAISYCLAW_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
      "User-Agent": formatDaisyClawUserAgent(identity.version),
    },
  };
}
...
function buildXaiAttributionPolicy(
  ...
  return {
    ...
    hook: "request-headers",
    reviewNote:
      "xAI api.x.ai accepts a standard daisyclaw User-Agent. Companion originator/version headers mirror the OpenAI attribution shape for consistency; they are not validated against an xAI-specific spec and are expected to be ignored by xAI's OpenAI-compatible surface.",
    ...identity,
    headers: {
      originator: DAISYCLAW_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
