...
describe("provider transport stream contracts", () => {
  it("covers the supported transport api alias matrix", () => {
    // Supported APIs can be projected to DaisyClaw transport aliases when needed.
    const cases = [
      {
        ...
        alias: "daisyclaw-openai-responses-transport",
      },
      {
      ...
      },
      {
        ...
        alias: "daisyclaw-openai-completions-transport",
      },
      {
        ...
        alias: "daisyclaw-azure-openai-responses-transport",
      },
      {
        ...
        alias: "daisyclaw-anthropic-messages-transport",
      },
      {
        ...
        alias: "daisyclaw-google-generative-ai-transport",
  ...
  it("keeps OpenAI API-key default streams on DaisyClaw transport", () => {
    const cases = [
    ...
    for (const model of cases) {
      expect(createBoundaryAwareStreamFnForModel(model)).toBeTypeOf("function");
      expect(createDaisyClawTransportStreamFnForModel(model)).toBeTypeOf("function");
      expect(createTransportAwareStreamFnForModel(model)).toBeUndefined();
      expect(buildTransportAwareSimpleStreamFn(model)).toBeUndefined();
      expect(prepareTransportAwareSimpleModel(model)).toBe(model);
    }
  ...
  it("routes localService models through the DaisyClaw simple-completion transport", () => {
    ...
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
    const preparedModel = prepareTransportAwareSimpleModel(model);
    expect(preparedModel.api).toBe("daisyclaw-openai-completions-transport");
    expect(preparedModel.provider).toBe("inferrs");
    expect(preparedModel.id).toBe("google/gemma-4-E2B-it");
  });

  it("keeps Codex defaults on the DaisyClaw transport until DaisyClaw preserves attribution", () => {
