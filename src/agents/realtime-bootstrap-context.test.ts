...
async function makeWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-realtime-bootstrap-"));
  tempDirs.push(dir);
  return dir;
}

function makeConfig(workspaceDir: string): DaisyClawConfig {
...
describe("resolveRealtimeBootstrapContextInstructions", () => {
  it("formats the default profile bootstrap files without exposing local paths", async () => {
    ...
    expect(instructions).toContain("DaisyClaw realtime voice profile context");
    ...
    expect(instructions).not.toContain("daisyclaw_agent_consult");
