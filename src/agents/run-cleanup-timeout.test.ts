...
describe("agent cleanup timeout", () => {
  ...
  it("uses the trajectory flush timeout environment override for trajectory cleanup", async () => {
    ...
    const result = runAgentCleanupStep({
      ...
      step: "daisyclaw-trajectory-flush",
      cleanup,
      log,
      env: {
        DAISYCLAW_TRAJECTORY_FLUSH_TIMEOUT_MS: "25000",
      },
    });
    ...
    expect(cleanup).toHaveBeenCalledTimes(1);
    expect(log.warn).toHaveBeenCalledWith(
      "agent cleanup timed out: runId=run-trajectory sessionId=session-trajectory step=daisyclaw-trajectory-flush timeoutMs=25000",
    );
  });
  ...
  it("includes cleanup timeout details when the cleanup step exposes them", async () => {
    ...
    expect(log.warn).toHaveBeenCalledWith(
      "agent cleanup timed out: runId=run-trajectory sessionId=session-trajectory step=daisyclaw-trajectory-flush timeoutMs=5 details=pendingWrites=2 queuedBytes=128 activeOperation=file-append",
    );
  });
  ...
  it("does not fail cleanup when timeout details throw", async () => {
    ...
    expect(log.warn).toHaveBeenCalledWith(
      "agent cleanup timed out: runId=run-trajectory sessionId=session-trajectory step=daisyclaw-trajectory-flush timeoutMs=5 detailsError=details unavailable",
    );
  });
  ...
  it("uses the general cleanup timeout environment override for other cleanup steps", async () => {
    ...
    const result = runAgentCleanupStep({
      ...
      log,
      env: {
        DAISYCLAW_AGENT_CLEANUP_TIMEOUT_MS: "1500",
      },
    });
  ...
  it("prefers explicit cleanup timeout values over environment overrides", () => {
    expect(
      resolveAgentCleanupStepTimeoutMs({
        ...
        env: {
          DAISYCLAW_TRAJECTORY_FLUSH_TIMEOUT_MS: "25000",
          DAISYCLAW_AGENT_CLEANUP_TIMEOUT_MS: "15000",
        },
      }),
    ).toBe(2_000);
  });
  ...
  it("ignores invalid cleanup timeout environment values", () => {
    expect(
      resolveAgentCleanupStepTimeoutMs({
        step: "daisyclaw-trajectory-flush",
        env: {
          DAISYCLAW_TRAJECTORY_FLUSH_TIMEOUT_MS: "0",
          DAISYCLAW_AGENT_CLEANUP_TIMEOUT_MS: "not-a-number",
        },
      }),
    ...
    expect(
      resolveAgentCleanupStepTimeoutMs({
        step: "daisyclaw-trajectory-flush",
        env: {
          DAISYCLAW_TRAJECTORY_FLUSH_TIMEOUT_MS: "1e3",
          DAISYCLAW_AGENT_CLEANUP_TIMEOUT_MS: "0x10",
        },
      }),
    ).toBe(AGENT_CLEANUP_STEP_TIMEOUT_MS);
  });
