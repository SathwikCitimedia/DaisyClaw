...
async function withManagedMediaRoot<T>(run: (ctx: { stateDir: string }) => Promise<T>) {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-managed-media-"));
  try {
    return await withEnvAsync({ DAISYCLAW_STATE_DIR: stateDir }, async () => {
...
async function withOutsideHardlinkInDaisyClawTmp<T>(
  params: {
    daisyClawTmpDir: string;
  ...
  const hardlinkPath = path.join(params.daisyClawTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.daisyClawTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.daisyClawTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.daisyClawTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const daisyClawTmpDir = resolvePreferredDaisyClawTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred DaisyClaw tmp root",
      media: path.join(daisyClawTmpDir, "image.png"),
      expected: path.join(daisyClawTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred DaisyClaw tmp root",
      media: pathToFileURL(path.join(daisyClawTmpDir, "photo.png")).href,
      expected: path.join(daisyClawTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred DaisyClaw tmp root",
      media: path.join(daisyClawTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(daisyClawTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(daisyClawTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside daisyclaw tmp root",
      media: path.join(os.tmpdir(), "outside-daisyclaw", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked DaisyClaw tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(daisyClawTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(daisyClawTmpDir, { recursive: true });
      const symlinkPath = path.join(daisyClawTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked DaisyClaw tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInDaisyClawTmp(
      {
        daisyClawTmpDir,
  ...
  it("rejects symlinked DaisyClaw tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInDaisyClawTmp(
