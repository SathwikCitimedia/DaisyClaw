...
import { isPassThroughRemoteMediaSource } from "@daisyclaw/media-core/media-source-url";
...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const daisyClawTmpDir = path.resolve(resolvePreferredDaisyClawTmpDir());
  if (!isPathInside(daisyClawTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: daisyClawTmpDir });
  return resolved;
}
