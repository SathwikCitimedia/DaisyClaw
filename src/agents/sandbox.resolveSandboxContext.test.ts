...
beforeAll(async () => {
  sandboxFixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-sandbox-context-"));
});
...
describe("resolveSandboxContext", () => {
  it("does not sandbox the agent main session in non-main mode", async () => {
    ...
    const result = await resolveSandboxContext({
      ...
      workspaceDir: "/tmp/daisyclaw-test",
    });
  ...
  it("materializes skills into a hidden read-only workspace for writable sandboxes", async () => {
      ...
      agents: {
        defaults: {
          sandbox: {
            ...
            scope: "session",
            workspaceAccess: "rw",
            workspaceRoot: path.join(workspaceDir, ".daisyclaw", "sandboxes"),
          },
        },
      },
    };
    ...
    expect(syncOptions?.targetWorkspaceDir).toContain(
      path.join(".daisyclaw", "sandbox", "skills-workspaces"),
    );
    expect(syncOptions?.targetWorkspaceDir).toMatch(
      /[\\/]agent-main-main-[a-f0-9]{8}[\\/]\.daisyclaw[\\/]sandbox-skills$/,
    );
    expect(syncOptions?.targetWorkspaceDir).not.toBe(
      path.join(workspaceDir, ".daisyclaw", "sandbox-skills"),
    );
    expect(syncOptions?.targetWorkspaceDir?.startsWith(path.join(workspaceDir, ".daisyclaw"))).toBe(
      false,
    );
    expect(syncOptions?.config).toBe(cfg);
    expect(syncOptions?.agentId).toBe("main");
  ...
  it("uses the SSH backend remote workspace for sandbox workspace info", async () => {
    ...
    expect(result?.containerWorkdir).toMatch(
      /^\/remote\/daisyclaw\/daisyclaw-ssh-agent-main-main-[a-f0-9]{8}\/workspace$/,
    );
  ...
  it("materializes skills for shared writable sandboxes even when roots match", async () => {
    ...
    expect(syncOptions?.targetWorkspaceDir).toMatch(
      /[\\/]shared-[a-f0-9]{8}[\\/]\.daisyclaw[\\/]sandbox-skills$/,
    );
    expect(syncOptions?.targetWorkspaceDir).not.toBe(
      path.join(workspaceDir, ".daisyclaw", "sandbox-skills"),
    );
