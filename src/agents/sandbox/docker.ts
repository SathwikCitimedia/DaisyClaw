...
import { markDaisyClawExecEnv } from "../../infra/daisyclaw-exec-env.js";
...
export async function ensureDockerImage(image: string) {
  ...
  if (image === DEFAULT_SANDBOX_IMAGE) {
    throw new Error(
      `Sandbox image not found: ${image}. Build it with scripts/sandbox-setup.sh before enabling Docker sandboxing. The default image includes python3 for sandbox write/edit helpers; DaisyClaw will not substitute plain debian:bookworm-slim.`,
    );
  }
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "daisyclaw.sandbox=1");
  args.push("--label", `daisyclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `daisyclaw.createdAtMs=${createdAtMs}`);
  args.push("--label", `daisyclaw.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `daisyclaw.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markDaisyClawExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "daisyclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`daisyclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`daisyclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("daisyclaw sandbox recreate --all");
}
