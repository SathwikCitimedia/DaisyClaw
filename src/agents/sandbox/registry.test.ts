...
const {
  ...
  const baseDir = mkdtempSync(nodePath.join(tmpdir(), "daisyclaw-sandbox-registry-"));
  const previousStateDir = process.env.DAISYCLAW_STATE_DIR;
  process.env.DAISYCLAW_STATE_DIR = baseDir;

  return {
    TEST_STATE_DIR: baseDir,
    PREVIOUS_DAISYCLAW_STATE_DIR: previousStateDir,
...
afterAll(async () => {
  ...
  if (PREVIOUS_DAISYCLAW_STATE_DIR === undefined) {
    delete process.env.DAISYCLAW_STATE_DIR;
  } else {
    process.env.DAISYCLAW_STATE_DIR = PREVIOUS_DAISYCLAW_STATE_DIR;
  }
});
