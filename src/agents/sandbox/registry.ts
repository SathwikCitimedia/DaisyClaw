...
type SandboxRegistryTable = DaisyClawStateKyselyDatabase["sandbox_registry_entries"];
type SandboxRegistryDatabase = Pick<DaisyClawStateKyselyDatabase, "sandbox_registry_entries">;
...
function readRegistryRows(kind: SandboxRegistryKind): SandboxRegistryRow[] {
  const { db } = openDaisyClawStateDatabase();
