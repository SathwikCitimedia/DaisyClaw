  ...
  const runtime: RemoteShellSandboxHandle = {
    ...
    runRemoteShellScript: async (command) => {
      ...
      const result = command.script.includes("python3 /dev/fd/3 \"$@\" 3<<'PY'")
        ...
        : spawnSync("sh", ["-c", command.script, "daisyclaw-sandbox-fs", ...(command.args ?? [])], {
            input: command.stdin,
            encoding: "buffer",
            stdio: ["pipe", "pipe", "pipe"],
          });
...
describe("remote sandbox fs bridge", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("daisyclaw-remote-fs-bridge-", async (stateDir) => {
  ...
  it("saturates unsafe stat size output without returning NaN", async () => {
    ...
    await withTempDir("daisyclaw-remote-fs-bridge-stat-", async (stateDir) => {
      const workspaceDir = path.join(stateDir, "workspace");
      await fs.mkdir(workspaceDir, { recursive: true });
      const runtime: RemoteShellSandboxHandle = {
        remoteWorkspaceDir: workspaceDir,
  ...
  it("does not reject malformed non-decimal hardlink counts", async () => {
    await withTempDir("daisyclaw-remote-fs-bridge-hardlink-", async (stateDir) => {
      const workspaceDir = path.join(stateDir, "workspace");
      await fs.mkdir(workspaceDir, { recursive: true });
      const runtime: RemoteShellSandboxHandle = {
        remoteWorkspaceDir: workspaceDir,
