      ...
      defaults: {
        sandbox: {
          ...
          ssh: {
            ...
            command: "ssh",
            workspaceRoot: "/remote/daisyclaw",
...
function createSession() {
  return {
    command: "ssh",
    configPath: path.join(os.tmpdir(), "daisyclaw-test-ssh-config"),
    host: "daisyclaw-sandbox",
  };
}
...
describe("ssh sandbox backend", () => {
  ...
  it("describes runtimes via the configured ssh target", async () => {
    const result = await sshSandboxBackendManager.describeRuntime({
      entry: {
        containerName: "daisyclaw-ssh-worker-abcd1234",
        backendId: "ssh",
        runtimeLabel: "daisyclaw-ssh-worker-abcd1234",
    ...
    expect(sessionSettings.workspaceRoot).toBe("/remote/daisyclaw");
    const commandParams = requireSshRunCommandParams();
    expect(commandParams.remoteCommand).toContain("/remote/daisyclaw/daisyclaw-ssh-agent-worker");
  });
  ...
  it("creates a remote-canonical backend that seeds once and reuses ssh exec", async () => {
    ...
    const skillsWorkspaceDir = await createTempDir("daisyclaw-ssh-skills-");
    ...
    const backend = await createSshSandboxBackend({
      ...
      cfg: {
        ...
        browser: {
          enabled: false,
          image: "daisyclaw-browser",
          containerPrefix: "daisyclaw-browser-",
          network: "bridge",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
    ...
    expect(execSpec.argv.at(-1)).toContain("/remote/daisyclaw/daisyclaw-ssh-agent-worker");
    ...
    expect(skillsUploadParams.remoteDir).toContain("/workspace/.daisyclaw/sandbox-skills");
  ...
  it("clears stale remote materialized skills when the local copy is missing", async () => {
    const tmpDir = await createTempDir("daisyclaw-ssh-skills-");
    ...
    expect(commandParams.remoteCommand).toContain("daisyclaw-sandbox-clear");
    expect(commandParams.remoteCommand).toContain("/workspace/.daisyclaw/sandbox-skills");
    await backend.finalizeExec?.({
      status: "completed",
      exitCode: 0,
      timedOut: false,
