...
describe("sandbox ssh helpers", () => {
  it("materializes inline ssh auth data into a temp config", async () => {
    ...
    expect(config).toContain("Host daisyclaw-sandbox");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const realParent = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-ssh-real-"));
      ...
      const target = path.join(root, "workspace", ".daisyclaw", "sandbox-skills");
      await execFileAsync("/bin/sh", [
        "-c",
        ENSURE_REMOTE_REAL_DIRECTORY_SCRIPT,
        "daisyclaw-remote-dir",
        target,
        root,
      ]);

      await expect(
        fs.stat(path.join(realParent, "runtime", "workspace", ".daisyclaw", "sandbox-skills")),
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await fs.symlink(realParent, path.join(root, "workspace", ".daisyclaw"));
      ...
      await expect(
        execFileAsync("/bin/sh", [
          ...
          ENSURE_REMOTE_REAL_DIRECTORY_SCRIPT,
          "daisyclaw-remote-dir",
          path.join(root, "workspace", ".daisyclaw", "sandbox-skills"),
          root,
        ]),
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const localDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-ssh-upload-"));
      ...
      await expect(
        uploadDirectoryToSshTarget({
          session: {
            command: "ssh",
            configPath: "/tmp/daisyclaw-test-ssh-config",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const localDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-ssh-upload-safe-"));
      tempDirs.push(localDir);
