...
const runRemoteShellScript: RemoteShellSandboxHandle["runRemoteShellScript"] = async (command) => {
  ...
  const result = command.script.includes('python3 /dev/fd/3 "$@" 3<<')
    ...
    : spawnSync("sh", ["-c", command.script, "daisyclaw-test", ...(command.args ?? [])], {
        input: command.stdin,
        encoding: "buffer",
        stdio: ["pipe", "pipe", "pipe"],
      });
...
describe("workspace skills bridge mount policy", () => {
  it("resolves workspace skill roots as read-only", async () => {
    await withTempDir("daisyclaw-skills-bridge-", async (stateDir) => {
      ...
      expect(resolve(".daisyclaw/sandbox-skills/skills/demo/SKILL.md").writable).toBe(false);
      expect(resolve(".daisyclaw/sandbox-skills/skills/demo/SKILL.md").hostPath).toBe(
        path.join(skillsWorkspaceDir, "skills", "demo", "SKILL.md"),
      );
      expect(resolve("/workspace/skills/demo/SKILL.md").writable).toBe(false);
      expect(resolve("/workspace/.daisyclaw/sandbox-skills/skills/demo/SKILL.md").writable).toBe(
        false,
      );
    });
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("daisyclaw-skills-remote-absent-", async (stateDir) => {
        const workspaceDir = path.join(stateDir, "workspace");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("daisyclaw-skills-remote-only-", async (stateDir) => {
        const workspaceDir = path.join(stateDir, "workspace");
        const skillsWorkspaceDir = path.join(stateDir, "sandbox-state");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await withTempDir("daisyclaw-skills-remote-link-", async (stateDir) => {
        const workspaceDir = path.join(stateDir, "workspace");
        const remoteWorkspaceDir = path.join(stateDir, "remote-workspace");
        await fs.mkdir(workspaceDir, { recursive: true });
        await fs.mkdir(path.join(remoteWorkspaceDir, "skills", "demo"), { recursive: true });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("daisyclaw-skills-remote-cwd-", async (stateDir) => {
        const workspaceDir = path.join(stateDir, "workspace");
        const remoteWorkspaceDir = path.join(stateDir, "remote-workspace");
        await fs.mkdir(path.join(workspaceDir, "skills", "demo"), { recursive: true });
        await fs.mkdir(path.join(remoteWorkspaceDir, "skills", "demo"), { recursive: true });
