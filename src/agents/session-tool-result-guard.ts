 ...
 */
import { resolveIntegerOption } from "@daisyclaw/normalization-core/number-coercion";
...
import { isTranscriptOnlyDaisyClawAssistantModel } from "../shared/transcript-only-daisyclaw-assistant.js";
...
const PERSISTED_DETAIL_REDACTION_BOUNDARY = "\u0000DAISYCLAW_PERSISTED_DETAIL_BOUNDARY\u0000";
...
function redactPersistedDetailString(
  ...
  const redactedPrefix =
    boundaryIndex >= 0
      ...
      : "[DaisyClaw persisted detail redacted: boundary marker removed]";
  ...
  const persistedPrefix =
    ...
    PARTIAL_PRIVATE_KEY_BLOCK_RE.test(initialPersistedPrefix)
      ? "[DaisyClaw persisted detail redacted: partial secret span omitted]"
      : initialPersistedPrefix;
  const boundaryNotice = "[DaisyClaw persisted detail redacted: boundary overlap omitted]";
  return `${persistedPrefix}${persistedPrefix ? "\n" : ""}${boundaryNotice}\n\n[DaisyClaw persisted detail truncated: ${Math.max(
    0,
...
function redactPersistedDetailValue(
  ...
  if (depth >= 8) {
    return "[DaisyClaw persisted detail redacted: max depth exceeded]";
  }
...
function isTranscriptOnlyDaisyClawAssistantMessage(message: AgentMessage): boolean {
  ...
  return isTranscriptOnlyDaisyClawAssistantModel(provider, model);
}
...
export function installSessionToolResultGuard(
  ...
  const guardedAppend = (message: AgentMessage) => {
    ...
    const transcriptOnlyAssistant =
      nextRole === "assistant" &&
      toolCalls.length === 0 &&
      isTranscriptOnlyDaisyClawAssistantMessage(nextMessage);
    if (
