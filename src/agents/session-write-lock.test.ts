...
import { MAX_TIMER_TIMEOUT_MS } from "@daisyclaw/normalization-core/number-coercion";
...
async function withTempSessionLockFile(
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-lock-"));
  try {
...
describe("acquireSessionWriteLock", () => {
  ...
  it("reports live DaisyClaw-owned stale locks without removing them", async () => {
    await withTempSessionLockFile(async ({ sessionFile, lockPath }) => {
      const owner = spawn(process.execPath, ["-e", "setInterval(() => {}, 1000)", "daisyclaw"], {
        stdio: "ignore",
      });
      if (!owner.pid) {
  ...
  it("lets session write-lock env override config for emergency tuning", () => {
    expect(
      resolveSessionWriteLockOptions(
        ...
        {
          env: {
            DAISYCLAW_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS: "120000",
            DAISYCLAW_SESSION_WRITE_LOCK_STALE_MS: "60000",
            DAISYCLAW_SESSION_WRITE_LOCK_MAX_HOLD_MS: "50000",
          },
        },
      ),
    ).toEqual({
  ...
  it("ignores non-decimal and unsafe session write-lock env values", () => {
    expect(
      resolveSessionWriteLockOptions(
        ...
        {
          env: {
            DAISYCLAW_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS: "1e3",
            DAISYCLAW_SESSION_WRITE_LOCK_STALE_MS: "0x1000",
            DAISYCLAW_SESSION_WRITE_LOCK_MAX_HOLD_MS: "9007199254740993",
          },
        },
      ),
    ).toEqual({
  ...
  it("allows explicit infinite acquire timeout but not infinite stale policy", () => {
    expect(
      resolveSessionWriteLockOptions(undefined, {
        env: {
          DAISYCLAW_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS: "Infinity",
          DAISYCLAW_SESSION_WRITE_LOCK_STALE_MS: "Infinity",
        },
      }),
    ).toMatchObject({
  ...
  it("uses resolved stale policy when cleaning stale lock files", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-lock-policy-"));
    ...
    try {
      ...
      const configOnly = await cleanStaleLockFiles({
        ...
        nowMs,
        removeStale: false,
        readOwnerProcessArgs: () => ["node", "/opt/daisyclaw/daisyclaw.mjs", "doctor"],
      });
      ...
      const envOverride = await cleanStaleLockFiles({
        sessionsDir,
        config: { session: { writeLock: { staleMs: 30_000 } } },
        env: { DAISYCLAW_SESSION_WRITE_LOCK_STALE_MS: "60000" },
        nowMs,
        removeStale: false,
        readOwnerProcessArgs: () => ["node", "/opt/daisyclaw/daisyclaw.mjs", "doctor"],
      });
  ...
  it("does not clean live DaisyClaw locks just because holder max hold expired", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-lock-policy-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
    const nowMs = Date.now();
    ...
    try {
      ...
      const result = await cleanStaleLockFiles({
        ...
        nowMs,
        removeStale: true,
        readOwnerProcessArgs: () => ["node", "/opt/daisyclaw/daisyclaw.mjs", "agent"],
      });
  ...
  it("cleans old live .jsonl lock files owned by non-DaisyClaw processes", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
    const nowMs = Date.now();
    const lockPath = path.join(sessionsDir, "old-non-daisyclaw.jsonl.lock");
    ...
    try {
      ...
      expect(lockCleanupRecords(result.cleaned)).toEqual([
        {
          name: "old-non-daisyclaw.jsonl.lock",
          removed: true,
          stale: true,
          staleReasons: ["too-old", "non-daisyclaw-owner"],
        },
      ]);
      await expectPathMissing(lockPath);
    } finally {
  ...
  it("cleans fresh live .jsonl lock files owned by a non-DaisyClaw process", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
      ...
      expect(lockCleanupRecords(result.locks)).toEqual([
        {
          ...
          removed: true,
          stale: true,
          staleReasons: ["non-daisyclaw-owner"],
        },
      ]);
      expect(lockCleanupRecords(result.cleaned)).toEqual([
        {
  ...
  it("cleans fresh live .jsonl lock files owned by generic non-DaisyClaw entrypoints", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it("keeps fresh live .jsonl lock files with DaisyClaw or unknown owners", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });

    const nowMs = Date.now();
    const daisyclawLock = path.join(sessionsDir, "daisyclaw-live.jsonl.lock");
    ...
    try {
      ...
      const daisyclawResult = await cleanStaleLockFiles({
        sessionsDir,
        staleMs: 30_000,
        nowMs,
        removeStale: true,
      ...
      expect(daisyclawResult.cleaned).toEqual([]);
      await expect(fs.access(daisyclawLock)).resolves.toBeUndefined();

      await fs.rm(daisyclawLock, { force: true });
      await fs.writeFile(
        gatewayLock,
        JSON.stringify({
          pid: process.pid,
  ...
  it("removes held locks on termination signals", async () => {
    ...
    try {
      for (const signal of signals) {
        const root = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-lock-cleanup-"));
        try {
          const sessionFile = path.join(root, "sessions.json");
          const lockPath = `${sessionFile}.lock`;
