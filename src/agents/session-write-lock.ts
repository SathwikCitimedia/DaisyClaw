...
const CLEANUP_STATE_KEY = Symbol.for("daisyclaw.sessionWriteLockCleanupState");
const WATCHDOG_STATE_KEY = Symbol.for("daisyclaw.sessionWriteLockWatchdogState");
...
const SESSION_LOCKS = createFileLockManager("daisyclaw.session-write-lock");
...
const SESSION_WRITE_LOCK_ENV: Record<SessionWriteLockMsKey, string> = {
  acquireTimeoutMs: "DAISYCLAW_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS",
  staleMs: "DAISYCLAW_SESSION_WRITE_LOCK_STALE_MS",
  maxHoldMs: "DAISYCLAW_SESSION_WRITE_LOCK_MAX_HOLD_MS",
};
...
function shouldStartBackgroundWatchdog(): boolean {
  return process.env.VITEST !== "true" || process.env.DAISYCLAW_TEST_SESSION_LOCK_WATCHDOG === "1";
}
...
function isDaisyClawSessionOwnerArgv(args: string[]): boolean {
  const normalized = args.map(normalizeOwnerProcessArg).filter(Boolean);
  if (normalized.length === 0) {
    return false;
  }
  const exe = (normalized[0] ?? "").replace(/\.(bat|cmd|exe)$/i, "");
  if (exe === "daisyclaw" || exe.endsWith("/daisyclaw") || exe.endsWith("/daisyclaw-gateway")) {
    return true;
  }
  if (
    normalized.some(
      (arg) =>
        arg === "daisyclaw" ||
        arg.endsWith("/daisyclaw") ||
        arg === "daisyclaw.mjs" ||
        arg.endsWith("/daisyclaw.mjs"),
    )
  ) {
    return true;
  }
  ...
  const hasDaisyClawCommandToken = normalized.some((arg) => arg === "gateway" || arg === "agent");
  return normalized.some(
    (arg) => entryCandidates.some((entry) => arg.endsWith(entry)) && hasDaisyClawCommandToken,
  );
}
...
function shouldTreatAsNonDaisyClawOwner(params: {
  payload: LockFilePayload | null;
  ...
  return !isDaisyClawSessionOwnerArgv(args);
}
...
function inspectLockPayloadForSession(params: {
  ...
  if (
    shouldTreatAsNonDaisyClawOwner({
      payload: params.payload,
      inspected,
      heldByThisProcess: params.heldByThisProcess,
    ...
    return {
      ...inspected,
      stale: true,
      staleReasons: [...inspected.staleReasons, "non-daisyclaw-owner"],
    };
  }
