...
import * as bundledLlm from "daisyclaw/plugin-sdk/llm";
...
import { installDaisyClawInternalCorePackageNativeResolver } from "../../../plugins/plugin-sdk-native-resolver.js";
import {
...
const VIRTUAL_MODULES: Record<string, unknown> = {
  ...
  "daisyclaw/plugin-sdk/agent-core": bundledAgentCore,
  "@daisyclaw/plugin-sdk/agent-core": bundledAgentCore,
  "daisyclaw/plugin-sdk/llm": bundledLlm,
  "@daisyclaw/plugin-sdk/llm": bundledLlm,
  "daisyclaw/plugin-sdk/agent-sessions": bundledAgentSessions,
  "@daisyclaw/plugin-sdk/agent-sessions": bundledAgentSessions,
};
...
const EXTENSION_LOADER_ALIAS_IMPORT_PATTERN =
  /(?:@daisyclaw\/plugin-sdk|daisyclaw\/plugin-sdk|@sinclair\/typebox|typebox)(?:\/[A-Za-z0-9_-]+)?/u;
...
function getExtensionLoaderAliases(): Record<string, string> {
  ...
  aliases = {
    ...
    "daisyclaw/plugin-sdk/agent-sessions": agentSessionsEntry,
    "@daisyclaw/plugin-sdk/agent-sessions": agentSessionsEntry,
...
async function loadExtensionSourceTransformModule(
  ...
  if (!extensionSourceTransformLoader) {
    installDaisyClawInternalCorePackageNativeResolver({ moduleUrl: import.meta.url });
...
function readResourceManifest(packageJsonPath: string): ResourceManifest | null {
  try {
    ...
    if (pkg.daisyclaw && typeof pkg.daisyclaw === "object") {
      return pkg.daisyclaw as ResourceManifest;
    }
    return null;
  } catch {
    return null;
 ...
 *
 * Checks for:
 * 1. package.json with "daisyclaw.extensions" field -> returns declared paths
 * 2. index.ts or index.js -> returns the index file
 *
 * Returns resolved paths or null if no entry points found.
 */
function resolveExtensionEntries(dir: string): string[] | null {
  // Check for package.json with "daisyclaw" field first
 ...
 * 3. Subdirectory with package.json: `extensions/* /package.json` with "daisyclaw" field → load what it declares
 *
...
export async function discoverAndLoadExtensions(
  ...
  for (const p of configuredPaths) {
    ...
    if (fs.existsSync(resolved) && fs.statSync(resolved).isDirectory()) {
      // Check for package.json with DaisyClaw manifest or index.ts
