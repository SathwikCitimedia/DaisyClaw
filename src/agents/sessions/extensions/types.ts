...
export interface ExtensionUIContext {
   ...
   * extend `CustomEditor` from `daisyclaw/plugin-sdk/agent-sessions` and call
   * `super.handleInput(data)` for keys you don't handle.
   *
   * @example
   * ```ts
   * import { CustomEditor } from "daisyclaw/plugin-sdk/agent-sessions";
   *
...
export interface ExtensionContext {
  ...
  /** Gracefully shut down DaisyClaw and exit. Available in all contexts. */
  shutdown(): void;
...
export interface ProviderModelConfig {
  ...
  reasoning: boolean;
  /** Maps DaisyClaw thinking levels to provider/model-specific values; null marks a level unsupported. */
