...
describe("DefaultPackageManager", () => {
  it("keeps manifest resource entries inside the package root", async () => {
    ...
    const root = await makeTempDir("daisyclaw-package-manager-");
    ...
    await writeFile(
      ...
      JSON.stringify({ daisyclaw: { skills: entries } }),
      "utf-8",
    );
  ...
  it("keeps auto-discovered project resources inside their resource roots", async () => {
    ...
    const root = await makeTempDir("daisyclaw-package-manager-");
    const configRoot = join(root, ".daisyclaw");
    const outsideRoot = join(root, "outside");
  ...
  it("does not auto-install missing npm package resources", async () => {
    ...
    const manager = new DefaultPackageManager({
      cwd: root,
      agentDir: join(root, "agent"),
      settingsManager: SettingsManager.inMemory({ packages: ["npm:@daisyclaw/missing-test"] }),
    });
