...
type SkillDiscoveryMode = "daisyclaw" | "agents";
...
function readResourceManifestFile(packageJsonPath: string): ResourceManifest | null {
  try {
    ...
    const pkg = JSON.parse(content) as { daisyclaw?: ResourceManifest };
    return pkg.daisyclaw ?? null;
  } catch {
    return null;
  }
}
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
  if (resourceType === "skills") {
    return collectSkillEntries(dir, "daisyclaw");
  }
...
export class DefaultPackageManager implements PackageManager {
  ...
  private getTemporaryDir(prefix: string, suffix?: string): string {
    const hash = createHash("sha256")
      ...
      .digest("hex")
      .slice(0, 8);
    return join(tmpdir(), "daisyclaw-resources", prefix, hash, suffix ?? "");
  }
  ...
  private addAutoDiscoveredResources(
    ...
    addResources(
      "skills",
      collectAutoSkillEntries(projectDirs.skills, "daisyclaw"),
      projectMetadata,
    ...
    // User extensions from ~/.daisyclaw/agent/
    addResources(
      "extensions",
    ...
    // User skills from ~/.daisyclaw/agent/
    addResources(
      "skills",
      collectAutoSkillEntries(userDirs.skills, "daisyclaw"),
      userMetadata,
