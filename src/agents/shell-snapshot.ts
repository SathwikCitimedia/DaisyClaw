...
const CAPTURE_MARKER = "__DAISYCLAW_SHELL_SNAPSHOT_CAPTURE__";
const ENV_MARKER = "__DAISYCLAW_SHELL_SNAPSHOT_ENV__";
...
async function captureShellSnapshot(opts: ShellSnapshotWrapOptions): Promise<string | null> {
  ...
  const captureOutputDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-shell-snapshot-"));
...
function buildTrustedSnapshotCaptureEnv(
  ...
  // DAISYCLAW_SHELL is injected by the exec runtime, so startup files can keep
  // their documented exec-specific branches without trusting model input.
  if (runtimeEnv.DAISYCLAW_SHELL === "exec") {
    env.DAISYCLAW_SHELL = "exec";
  }
  return env;
}
