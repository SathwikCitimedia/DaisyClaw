...
describe("prepareSimpleCompletionModel", () => {
  ...
  it("applies provider runtime auth before storing simple-completion credentials", async () => {
    ...
    expect(runtimeAuthInput.workspaceDir).toBe("/tmp/daisyclaw-agent");
...
describe("completeWithPreparedSimpleCompletionModel", () => {
  it("prepares provider-owned stream APIs before running a completion", async () => {
    ...
    const preparedModel = {
      ...model,
      api: "daisyclaw-ollama-simple-test",
    };
    const cfg = {
  ...
  it("normalizes DaisyClaw-only thinking levels before using shared model runtime simple completion", async () => {
    const model = {
      provider: "openai",
      id: "gpt-5.4",
      name: "gpt-5.4",
