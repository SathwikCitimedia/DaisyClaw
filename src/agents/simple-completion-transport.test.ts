...
const createDaisyClawTransportStreamFnForModel = vi.fn();
...
describe("prepareModelForSimpleCompletion", () => {
  ...
  beforeEach(() => {
    ...
    createDaisyClawTransportStreamFnForModel.mockReset();
    ...
    createDaisyClawTransportStreamFnForModel.mockReturnValue(undefined);
  ...
  it("uses a custom api alias for Anthropic Vertex simple completions", () => {
    ...
    expect(ensureCustomApiRegistered).toHaveBeenCalledWith(
      "daisyclaw-anthropic-vertex-simple:https%3A%2F%2Fus-central1-aiplatform.googleapis.com",
      "vertex-stream",
    );
    expect(result).toEqual({
      ...model,
      api: "daisyclaw-anthropic-vertex-simple:https%3A%2F%2Fus-central1-aiplatform.googleapis.com",
    });
  });
  ...
  it("uses a transport-aware custom api alias when llm request transport overrides are present", () => {
    ...
    expect(ensureCustomApiRegistered).toHaveBeenCalledWith(
      "daisyclaw-openai-responses-transport",
      "transport-stream",
    );
    expect(result).toEqual({
      ...model,
  ...
  it("uses the Google simple-completion sanitizer alias after transport checks pass through", () => {
    ...
    prepareGoogleSimpleCompletionModel.mockImplementationOnce((m: unknown) => ({
      ...
      api: "daisyclaw-google-generative-ai-simple",
    }));
    resolveProviderStreamFn.mockReturnValueOnce(undefined);
  ...
  it.each([
    ...
    "uses DaisyClaw transport for OpenAI Codex-response simple completions with baseUrl %s",
    (baseUrl, expectedBaseUrl) => {
      ...
      resolveProviderStreamFn.mockReturnValueOnce(undefined);
      createDaisyClawTransportStreamFnForModel.mockReturnValueOnce("codex-transport-stream");
      resolveTransportAwareSimpleApi.mockReturnValueOnce("daisyclaw-openai-responses-transport");
      ...
      expect(createDaisyClawTransportStreamFnForModel).toHaveBeenCalledWith(
        {
          ...model,
          baseUrl: expectedBaseUrl,
        },
