 ...
 *
 * Registers provider-specific stream functions and rewrites models that need DaisyClaw-managed transport semantics.
 */
...
function resolveAnthropicVertexSimpleApi(baseUrl?: string): Api {
  ...
  return `daisyclaw-anthropic-vertex-simple:${suffix}`;
}
...
function prepareCodexSimpleTransportModel<TApi extends Api>(
  ...
  // completion path must use DaisyClaw's transport so resolved request auth is applied.
  const transportModel = {
    ...model,
    baseUrl: normalizeCodexResponsesBaseUrlForOpenAISdk(model.baseUrl),
  } as Model;
  const api = resolveTransportAwareSimpleApi(model.api);
  const streamFn = createDaisyClawTransportStreamFnForModel(transportModel, { cfg });
  if (!api || !streamFn) {
    return undefined;
  }
