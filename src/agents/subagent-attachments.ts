...
function resolveAttachmentLimits(config: DaisyClawConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".daisyclaw", "attachments");
  const relDir = path.posix.join(".daisyclaw", "attachments", attachmentId);
