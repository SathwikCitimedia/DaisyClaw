 ...
 *
 * @see https://github.com/daisyclaw/daisyclaw/issues/47711
 */
...
export async function recoverOrphanedSubagentSessions(params: {
  ...
  const configChangePattern = /daisyclaw\.json|daisyclaw gateway restart|config\.patch/i;
      ...
      try {
        ...
        const resumeResult = await resumeOrphanedSession({
          ...
          configChangeHint: configChangeDetected
            ? "\n\n[config changes from your previous run were already applied — do not re-modify daisyclaw.json or restart the gateway]"
            : undefined,
          originalRunId: runId,
