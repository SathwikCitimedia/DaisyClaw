...
describe("subagent registry state read cache", () => {
  const previousReadDiskFlag = process.env.DAISYCLAW_TEST_READ_SUBAGENT_RUNS_FROM_DISK;
  ...
  afterEach(() => {
    ...
    if (previousReadDiskFlag === undefined) {
      ...
      process.env.DAISYCLAW_TEST_READ_SUBAGENT_RUNS_FROM_DISK = previousReadDiskFlag;
    }
    vi.useRealTimers();
  });
