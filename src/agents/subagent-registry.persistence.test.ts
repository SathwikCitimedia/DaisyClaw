...
describe("subagent registry persistence", () => {
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-subagent-"));
  ...
  it("reuses the persisted registry cache on hot internal read snapshots", async () => {
    ...
    const previousFlag = process.env.DAISYCLAW_TEST_READ_SUBAGENT_RUNS_FROM_DISK;
    let cloneSpy: { mockRestore(): void } | undefined;
    try {
      process.env.DAISYCLAW_TEST_READ_SUBAGENT_RUNS_FROM_DISK = "1";
      ...
      if (previousFlag === undefined) {
        delete process.env.DAISYCLAW_TEST_READ_SUBAGENT_RUNS_FROM_DISK;
      } else {
        process.env.DAISYCLAW_TEST_READ_SUBAGENT_RUNS_FROM_DISK = previousFlag;
      }
    }
  });
  ...
  it("prefers active runs and can resolve them from persisted registry snapshots", async () => {
    ...
    const resolved = withEnv({ DAISYCLAW_TEST_READ_SUBAGENT_RUNS_FROM_DISK: "1" }, () =>
  ...
  it("uses isolated temp state when DAISYCLAW_STATE_DIR is unset in tests", () => {
    delete process.env.DAISYCLAW_STATE_DIR;
    const registryPath = resolveSubagentRegistryPath();
    expect(registryPath).toContain(path.join(os.tmpdir(), "daisyclaw-test-state"));
  });
});
