...
describe("subagent registry sqlite store", () => {
  ...
  beforeEach(async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-subagent-sqlite-"));
  });
  ...
  async function withTempStateEnv<T>(fn: () => Promise<T>): Promise<T> {
    ...
    return await withEnvAsync({ DAISYCLAW_STATE_DIR: tempStateDir }, fn);
  }
  ...
  it("persists subagent runs in the shared sqlite state database", async () => {
    await withTempStateEnv(async () => {
      ...
      expect(await fs.stat(path.join(tempStateDir!, "state", "daisyclaw.sqlite"))).toBeTruthy();
  ...
  it("imports the legacy json registry when sqlite has no runs", async () => {
    await withTempStateEnv(async () => {
      ...
      expect(
        openDaisyClawStateDatabase().db.prepare("SELECT COUNT(*) AS count FROM subagent_runs").get(),
      ).toEqual({ count: 1 });
    });
  });
});
