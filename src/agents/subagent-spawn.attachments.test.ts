...
describe("spawnSubagentDirect filename validation", () => {
  beforeEach(async () => {
    workspaceDirOverride = fs.mkdtempSync(
      path.join(os.tmpdir(), `daisyclaw-subagent-attachments-${process.pid}-${Date.now()}-`),
    );
  ...
  it("materializes attachments under explicit cwd when native subagent cwd is provided", async () => {
    const explicitWorkspaceDir = fs.mkdtempSync(
      path.join(os.tmpdir(), `daisyclaw-subagent-cwd-attachments-${process.pid}-${Date.now()}-`),
    );
    try {
      const { spawnSubagentDirect } = subagentSpawnModule;
      const result = await spawnSubagentDirect(
      ...
      const explicitAttachmentsRoot = path.join(explicitWorkspaceDir, ".daisyclaw", "attachments");
      const targetAttachmentsRoot = path.join(workspaceDirOverride, ".daisyclaw", "attachments");
  ...
  it("normalizes explicit cwd before materializing native subagent attachments", async () => {
    const homeDir = fs.mkdtempSync(
      path.join(os.tmpdir(), `daisyclaw-subagent-home-attachments-${process.pid}-${Date.now()}-`),
    );
    ...
    try {
      await withEnvAsync({ HOME: homeDir }, async () => {
        ...
        expect(result.status).toBe("accepted");
        const attachmentsRoot = path.join(expectedCwd, ".daisyclaw", "attachments");
  ...
  it("removes materialized attachments when lineage patching fails", async () => {
    ...
    const attachmentsRoot = path.join(workspaceDirOverride, ".daisyclaw", "attachments");
