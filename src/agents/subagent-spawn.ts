...
import { finiteSecondsToTimerSafeMilliseconds } from "@daisyclaw/normalization-core/number-coercion";
import {
...
function resolveConfiguredAgentIds(cfg: DaisyClawConfig): string[] {
...
async function prepareSubagentSessionContext(params: {
  ...
  try {
    const forked = (await updateSubagentSessionStore(childTarget.storePath, async (store) => {
      ...
      if (!fork) {
        throw new Error(
          'context="fork" requested but DaisyClaw could not fork the requester transcript.',
        );
      }
      pruneLegacyStoreKeys({
        store,
    ...
    if (params.contextMode === "fork") {
      if (!parentEntry || !forked) {
        ...
        return {
          status: "error",
          error: 'context="fork" requested but DaisyClaw could not prepare forked context.',
        };
      }
      return {
        status: "ok",
