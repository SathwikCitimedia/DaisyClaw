...
describe("buildAgentSystemPrompt", () => {
  it("resolves helper session keys to scoped prompt surfaces", () => {
    ...
    expect(resolveAgentPromptSurfaceForSessionKey("agent:main")).toBe("daisyclaw_main");
    expect(resolveAgentPromptSurfaceForSessionKey(undefined)).toBe("daisyclaw_main");
  });
  ...
  it("includes an DaisyClaw control section", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/daisyclaw",
    });

    expect(prompt).toContain("## DaisyClaw Control");
    expect(prompt).toContain("prefer `gateway` tool");
    expect(prompt).toContain("CLI lifecycle only on explicit user request");
    expect(prompt).toContain("daisyclaw gateway status|restart|start|stop");
  ...
  it("includes embed guidance only for webchat sessions", () => {
    ...
    expect(prompt).toContain(
      '[embed url="/__daisyclaw__/canvas/documents/cv_123/index.html" title="Status" height="320" /]',
    );
    expect(prompt).toContain(
  ...
  it("keeps the DaisyClaw empty-tool fallback on the main prompt surface", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/daisyclaw",
      toolNames: [],
    });

    expect(prompt).toContain("DaisyClaw lists the standard tools above");
  ...
  it("omits ACP harness guidance when ACP is disabled", () => {
    ...
    expect(prompt).toContain("- sessions_spawn: Spawn an isolated sub-agent session");
    expect(prompt).toContain("- agents_list: List DaisyClaw agent ids allowed for sessions_spawn");
  });
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("Docs: /tmp/daisyclaw/docs");
    expect(prompt).toContain("DaisyClaw behavior/config/architecture: read local docs first.");
  });
  ...
  it("includes docs guidance when docsPath is provided", () => {
    ...
    expect(prompt).toContain("Docs: /tmp/daisyclaw/docs");
    expect(prompt).toContain("Source: /tmp/daisyclaw");
    expect(prompt).toContain("DaisyClaw behavior/config/architecture: read local docs first.");
  ...
  it("falls back to public docs and GitHub source guidance when local docs are unavailable", () => {
    ...
    expect(prompt).toContain("Docs: https://docs.daisyclaw.ai");
    expect(prompt).toContain("Source: https://github.com/daisyclaw/daisyclaw");
  ...
  it("includes bootstrap instructions in system prompt when bootstrap is pending", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/daisyclaw",
      bootstrapMode: "full",
      contextFiles: [{ path: "/tmp/daisyclaw/BOOTSTRAP.md", content: "Ask who I am." }],
    });
    ...
    expect(prompt).toContain("## /tmp/daisyclaw/BOOTSTRAP.md");
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", () => {
    ...
    expect(prompt).toContain("## DaisyClaw Self-Update");
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/daisyclaw. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
...
describe("buildAgentBootstrapSystemPromptSections", () => {
  it("can render bootstrap guidance without duplicating Project Context", () => {
    ...
    expect(sections).not.toContain("## /tmp/daisyclaw/BOOTSTRAP.md");
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Do not use `exec` (`daisyclaw ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for DaisyClaw subagents");
