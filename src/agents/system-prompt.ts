/**
 * DaisyClaw system prompt renderer.
 *
...
import {
  buildDaisyClawToolFallbackText,
  shouldRenderDaisyClawToolWorkflowHints,
...
function buildWebchatCanvasSection(params: {
  ...
  return [
    ...
    '- You may also use an explicit hosted URL: `[embed url="/__daisyclaw__/canvas/documents/cv_123/index.html" title="Status" height="320" /]`.',
    '- Never use local filesystem paths or `file://...` URLs in `[embed ...]`. Hosted embeds must point at `/__daisyclaw__/canvas/...` URLs or use `ref="..."`.',
    ...
    "- Quote all attribute values. Prefer `ref` for hosted documents unless you already have the full `/__daisyclaw__/canvas/documents/<id>/index.html` URL.",
    "",
  ];
}
...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; DaisyClaw handles all routing internally.",
      ...
      ? [
          ...
          messageToolOnly
            ...
            : suppressSilentTokenGuidance
              ? "- Do not use `message(action=send)` to deliver the current source-channel reply; reply normally so DaisyClaw can route it once."
...
function buildDocsSection(params: {
  ...
  const lines = [
    "## Documentation",
    docsPath ? `Docs: ${docsPath}` : "Docs: https://docs.daisyclaw.ai",
    docsPath ? "Mirror: https://docs.daisyclaw.ai" : undefined,
    sourcePath ? `Source: ${sourcePath}` : "Source: https://github.com/daisyclaw/daisyclaw",
    docsPath
      ? "DaisyClaw behavior/config/architecture: read local docs first."
      : "DaisyClaw behavior/config/architecture: read docs mirror first.",
    ...
    "Diagnosing issues: run `daisyclaw status` when possible; ask user only if blocked.",
    "",
  ];
...
export function buildAgentSystemPrompt(params: {
  ...
  acpEnabled?: boolean;
  /** Prompt surface controls runtime-specific fallback fragments. Defaults to DaisyClaw main. */
  ...
  const promptSurface = params.promptSurface ?? "daisyclaw_main";
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running DaisyClaw process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List DaisyClaw agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List DaisyClaw agent ids allowed for sessions_spawn",
  ...
  const renderDaisyClawToolWorkflowHints = shouldRenderDaisyClawToolWorkflowHints({
  ...
  const workspaceOnlyGuidance =
    params.fsWorkspaceOnly === true
      ? "tools.fs.workspaceOnly is enabled: scratch/temp/meta files that file tools must later read/write/edit must stay inside the workspace, preferably as relative paths under `.daisyclaw/tmp/`. Do not write files to `/tmp/...` with exec if a later read/write/edit/apply_patch tool needs them; use `.daisyclaw/tmp/...` instead."
      : "";
  const safetySection = [
    "## Safety",
  ...
  if (promptMode === "none") {
    return ["You are a personal assistant running inside DaisyClaw.", modelIdentityLine]
      .filter(Boolean)
      .join("\n");
  }
  ...
  const stablePrefixCacheKey = hashStablePromptInput({
    ...
    promptSurface,
    toolLines,
    renderDaisyClawToolWorkflowHints,
    hasGateway,
    readToolName,
    execToolName,
    processToolName,
  ...
  const stablePrefix = cacheStablePromptPrefix(stablePrefixCacheKey, () => {
    const lines = [
      "You are a personal assistant running inside DaisyClaw.",
      "",
      "## Tooling",
      ...
      "CLI lifecycle only on explicit user request: `daisyclaw gateway status|restart|start|stop`.",
      "`restart`, not stop+start.",
      "",
      ...skillsSection,
      ...skillWorkshopSection,
      ...memorySection,
      hasGateway && !isMinimal ? "## DaisyClaw Self-Update" : "",
      hasGateway && !isMinimal
        ? [
            ...
            "After restart, DaisyClaw pings the last active session automatically.",
          ].join("\n")
        : "",
      hasGateway && !isMinimal ? "" : "",
      "",
      ...
      "These user-editable files are loaded by DaisyClaw and included below in Project Context.",
      "",
