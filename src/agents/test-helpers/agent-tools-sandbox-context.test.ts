...
describe("createAgentToolsSandboxContext", () => {
  it("provides stable defaults for agent-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("daisyclaw-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("daisyclaw-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("daisyclaw-sbx-");
