...
type CoreToolDefinition = {
  ...
  includeInDaisyClawGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const daisyclawTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInDaisyClawGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:daisyclaw": daisyclawTools,
