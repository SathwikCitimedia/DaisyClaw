...
describe("tool display details", () => {
  it("summarizes tool-search code targets from described tool ids", () => {
    expect(
      resolveToolSearchCodeDisplayTarget({
        code: "const tool = await daisyclaw.tools.describe('daisyclaw:core:exec'); return await daisyclaw.tools.call(tool.id, { command: 'echo hi' });",
      }),
    ).toEqual({
      toolName: "daisyclaw:core:exec",
  ...
  it("normalizes direct tool-search catalog ids to native display names and args", () => {
    expect(
      resolveToolSearchCodeDisplayTarget({
        code: 'return await daisyclaw.tools.call("daisyclaw:core:exec", { command: "echo hi" });',
      }),
    ).toEqual({
      toolName: "daisyclaw:core:exec",
      displayToolName: "exec",
  ...
  it("preserves JS numeric literals in tool-search call args", () => {
    expect(
      resolveToolSearchCodeDisplayTarget({
        code: 'return await daisyclaw.tools.call("web_search", { query: "DaisyClaw", count: 1e3, limit: +3, threshold: .5 });',
      })?.displayArgs,
    ).toEqual({
      query: "DaisyClaw",
      count: 1000,
  ...
  it("formats web_search query with quotes", () => {
    const detail = formatToolDetail(
      resolveToolDisplay({
        name: "web_search",
        args: { query: "DaisyClaw docs", count: 3 },
      }),
    );

    expect(detail).toBe('for "DaisyClaw docs" (top 3)');
  });
  ...
  it("formats Parallel's native objective + search_queries shape", () => {
    expect(
      ...
      'for "Find the DaisyClaw repository on GitHub", "daisyclaw github", "daisyclaw repository" (top 5)',
    );
  });
  ...
  it("summarizes exec commands with context", () => {
    const detail = formatToolDetail(
      resolveToolDisplay({
        ...
        args: {
          command:
            "set -euo pipefail\ngit -C /Users/adityasingh/.daisyclaw/workspace status --short | head -n 3",
          workdir: "/Users/adityasingh/.daisyclaw/workspace",
        },
      }),
    );
  ...
  it("uses compact workspace markers for common workspace paths", () => {
    expect(
      formatToolDetail(
        resolveToolDisplay({
          name: "bash",
          args: { command: "git fetch", workdir: "/Users/peter/mantis-workspace/daisyclaw" },
          detailMode: "explain",
        }),
      ),
    ...
    expect(
      formatToolDetail(
        resolveToolDisplay({
          name: "bash",
          args: { command: "git status", workdir: "/Users/peter/Projects/daisyclaw" },
          detailMode: "explain",
        }),
      ),
      ...
      formatToolDetail(
        resolveToolDisplay({
          ...
          args: {
            ...
            workdir: "/root/.daisyclaw/sandboxes/agent-clawsweeper-sandbox-discor-766423d0",
          },
          detailMode: "explain",
        }),
      ),
  ...
  it("omits bash and exec names from compact tool summaries", () => {
    ...
    expect(
      formatToolSummary(
        resolveToolDisplay({
          name: "web_search",
          args: { query: "DaisyClaw docs" },
        }),
      ),
    ).toBe('🔎 Web Search: for "DaisyClaw docs"');
  });
