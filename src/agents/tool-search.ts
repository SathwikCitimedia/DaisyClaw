 ...
 *
 * Presents large DaisyClaw/MCP/client tool inventories through search, describe, call, and optional code-mode tools.
 */
...
type CatalogSource = "daisyclaw" | "mcp" | "client";
...
function buildModelScriptSource(code) {
  return "(async (daisyclaw, console) => {\n" + code + "\n})(daisyclaw, console)";
}
...
async function runModelCode(code, timeoutMs) {
  ...
  Object.defineProperties(sandbox, {
    ...
    daisyclaw: { value: controller.daisyclaw, enumerable: true },
  });
...
const SESSION_CATALOGS_KEY = Symbol.for("daisyclaw.toolSearch.sessionCatalogs");
...
function readToolSearchConfig(config?: DaisyClawConfig): Record<string, unknown> {
...
export function resolveToolSearchConfig(config?: DaisyClawConfig): ToolSearchConfig {
...
function classifyTool(tool: CatalogTool): {
  ...
  if (pluginId) {
    return { source: "daisyclaw", sourceName: pluginId };
  }
  return { source: "daisyclaw", sourceName: "core" };
}
  ...
  return [
    {
      ...
      description:
        "Run JavaScript in an isolated Node subprocess with daisyclaw.tools.search, daisyclaw.tools.describe, and daisyclaw.tools.call for large tool catalogs.",
      parameters: Type.Object({
        code: Type.String({
          description:
            "JavaScript body for an async function. Use return to return the final value. The daisyclaw.tools bridge is available.",
        }),
      }),
      execute: async (
        toolCallId: string,
    ...
    },
    {
      name: TOOL_CALL_RAW_TOOL_NAME,
      label: "Tool Call",
      description: "Call a selected Tool Search catalog entry through DaisyClaw.",
      parameters: Type.Object({
        id: Type.String({ description: "Tool search result id or tool name." }),
        args: Type.Optional(
