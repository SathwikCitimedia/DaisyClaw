...
function policyDeniesTool(policy: { deny?: string[] } | undefined, toolName: string): boolean {
  return (
    ...
    listIncludesTool(policy?.deny, "group:daisyclaw")
  );
}

function hasExplicitBrowserIntent(cfg: DaisyClawConfig): boolean {
...
export function resolveEffectiveToolInventory(
  ...
  const effectiveTools = createDaisyClawCodingTools({
    agentId,
