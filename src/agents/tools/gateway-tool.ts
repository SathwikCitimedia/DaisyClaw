...
import { isRecord as isPlainObject } from "@daisyclaw/normalization-core/record-coerce";
import {
...
function assertGatewayConfigMutationAllowed(params: {
  ...
  // Uses the same flag enumeration as `daisyclaw security audit`.
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as DaisyClawConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as DaisyClawConfig);
