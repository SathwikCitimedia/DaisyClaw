...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    daisyclaw: process.env.DAISYCLAW_GATEWAY_TOKEN,
    gatewayUrl: process.env.DAISYCLAW_GATEWAY_URL,
  };
  ...
  afterAll(() => {
    if (envSnapshot.daisyclaw === undefined) {
      delete process.env.DAISYCLAW_GATEWAY_TOKEN;
    } else {
      process.env.DAISYCLAW_GATEWAY_TOKEN = envSnapshot.daisyclaw;
    }
    if (envSnapshot.gatewayUrl === undefined) {
      delete process.env.DAISYCLAW_GATEWAY_URL;
    } else {
      process.env.DAISYCLAW_GATEWAY_URL = envSnapshot.gatewayUrl;
    }
  });
  ...
  it("uses DAISYCLAW_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    ...
    process.env.DAISYCLAW_GATEWAY_TOKEN = "local-env-token";
    mocks.configState.value = {
      gateway: {
  ...
  it("does not send the local approval runtime token to env-selected gateways", async () => {
    process.env.DAISYCLAW_GATEWAY_URL = "wss://gateway.example";
    mocks.callGateway.mockResolvedValueOnce({ decision: "allow-once" });
  ...
  it("does not send the local approval runtime token to loopback env-selected gateways", async () => {
    process.env.DAISYCLAW_GATEWAY_URL = "ws://127.0.0.1:18789";
    mocks.callGateway.mockResolvedValueOnce({ decision: "allow-once" });
  ...
  it("does not send the local approval runtime token to loopback env-selected gateway paths", async () => {
    process.env.DAISYCLAW_GATEWAY_URL = "ws://127.0.0.1:18789/ws";
    mocks.callGateway.mockResolvedValueOnce({ decision: "allow-once" });
