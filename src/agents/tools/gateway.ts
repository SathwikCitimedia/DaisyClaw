...
function resolveLocalGatewayUrlKeys(cfg: DaisyClawConfig): Set<string> {
...
function resolveConfiguredRemoteGatewayKey(cfg: DaisyClawConfig): string | undefined {
...
export function resolveGatewayOptions(opts?: GatewayCallOptions) {
  ...
  const timeoutMs =
    typeof opts?.timeoutMs === "number" && Number.isFinite(opts.timeoutMs)
      ...
      : 30_000;
  const envGatewayUrl = trimToUndefined(process.env.DAISYCLAW_GATEWAY_URL);
  const target =
  ...
  try {
    ...
    throw new Error(
      [
        ...
        "Fix the DaisyClaw state directory permissions or use the local approval-runtime gateway.",
      ].join(" "),
      { cause: error },
    );
  }
