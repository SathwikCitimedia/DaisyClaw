...
async function createStoreConfig(): Promise<{ config: DaisyClawConfig; template: string }> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-goal-tools-"));
  const template = path.join(dir, "{agentId}", "sessions.json");
  return {
    config: { session: { store: template } } as DaisyClawConfig,
    template,
  };
}
