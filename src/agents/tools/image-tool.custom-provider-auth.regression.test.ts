...
import { resolveImageToolFactoryAvailable } from "../daisyclaw-tools.media-factory-plan.js";
...
function createUserReportedConfig(params?: { includeApiKey?: boolean }): DaisyClawConfig {
...
async function withEmptyAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-image-auth-regression-"));
  try {
