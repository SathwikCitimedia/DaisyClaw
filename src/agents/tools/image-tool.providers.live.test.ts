...
const LIVE_IMAGE_TOOL_ENABLED = isLiveTestEnabled(["DAISYCLAW_LIVE_IMAGE_TOOL_TEST"]);
const LIVE_OPENAI_MODEL =
  process.env.DAISYCLAW_LIVE_IMAGE_TOOL_OPENAI_MODEL?.trim() ||
  process.env.DAISYCLAW_LIVE_IMAGE_TOOL_MODEL?.trim() ||
  "gpt-4.1-mini";
const LIVE_ANTHROPIC_MODEL =
  process.env.DAISYCLAW_LIVE_IMAGE_TOOL_ANTHROPIC_MODEL?.trim() || "claude-sonnet-4-6";
...
function createLiveConfig(testCase: LiveProviderCase): DaisyClawConfig {
  return {
    agents: {
      defaults: {
...
async function withLiveWorkspace<T>(
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-image-tool-live-"));
  try {
