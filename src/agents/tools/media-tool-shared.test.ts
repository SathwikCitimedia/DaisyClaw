...
describe("resolveMediaToolLocalRoots", () => {
  it("does not widen default local roots from media sources", () => {
    const stateDir = path.join("/tmp", "daisyclaw-media-tool-roots-state");
    const picturesDir =
    ...
    const roots = withEnv({ DAISYCLAW_STATE_DIR: stateDir }, () =>
  ...
  it("keeps channel inbound attachment roots separate from local roots", () => {
    ...
    const accountRoot = path.join("/tmp", "daisyclaw-imessage-work");
    const sharedRoot = path.join("/tmp", "daisyclaw-imessage-shared");
    const cfg = {
      channels: {
        imessage: {
