...
function hasExplicitMusicGenerationModelConfig(cfg?: DaisyClawConfig): boolean {
...
export function createMusicGenerateTool(options?: {
  ...
  const cfg: DaisyClawConfig = options?.config ?? getRuntimeConfig();
  if (
    !hasGenerationToolAvailability({
      cfg,
