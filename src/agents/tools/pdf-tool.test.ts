...
function withPdfModel(primary: string): DaisyClawConfig {
  return {
    agents: { defaults: { pdfModel: { primary } } },
  } as DaisyClawConfig;
}

function withDefaultModel(primary: string): DaisyClawConfig {
  return {
...
async function stubPdfToolInfra(
  ...
  vi.spyOn(modelsConfig, "ensureDaisyClawModelsJson").mockResolvedValue({
    agentDir,
    wrote: false,
  });
...
async function withManagedInboundPdf(
  ...
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-pdf-managed-inbound-"));
...
describe("createPdfTool", () => {
  ...
  it("respects fsPolicy.workspaceOnly for non-sandbox pdf paths", async () => {
    await withTempPdfAgentDir(async (agentDir) => {
      const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-pdf-ws-"));
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-pdf-out-"));
      try {
        const cfg = withPdfModel(ANTHROPIC_PDF_MODEL);
        const tool = requirePdfTool(
  ...
  it("uses native PDF path without eager extraction", async () => {
    ...
    await withTempPdfAgentDir(async (agentDir) => {
      ...
      const ensureModelsJsonMock = vi.mocked(modelsConfig.ensureDaisyClawModelsJson);
