...
describe("sessions_history redaction", () => {
  beforeAll(async () => {
    previousConfigPath = process.env.DAISYCLAW_CONFIG_PATH;
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-sessions-history-redact-"));
  ...
  afterAll(() => {
    if (previousConfigPath === undefined) {
      ...
      process.env.DAISYCLAW_CONFIG_PATH = previousConfigPath;
    }
    if (tempDir) {
