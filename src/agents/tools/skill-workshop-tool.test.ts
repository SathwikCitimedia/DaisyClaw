...
describe("skill_workshop tool", () => {
  it("is exposed in the DaisyClaw tool set", async () => {
    const workspaceDir = await tempDirs.make("daisyclaw-skill-workshop-tool-");
  ...
  it("is not exposed from sandboxed DaisyClaw tool sets", async () => {
    const workspaceDir = await tempDirs.make("daisyclaw-skill-workshop-tool-");
    const tools = createDaisyClawTools({
      workspaceDir,
      config: {},
  ...
  it("scopes proposal discovery to the tool workspace", async () => {
    const firstWorkspaceDir = await tempDirs.make("daisyclaw-skill-workshop-tool-first-");
    const secondWorkspaceDir = await tempDirs.make("daisyclaw-skill-workshop-tool-second-");
