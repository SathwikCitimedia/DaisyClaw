...
let testState: DaisyClawTestState | undefined;

beforeEach(async () => {
  testState = await createDaisyClawTestState({
    layout: "state-only",
    prefix: "daisyclaw-workspace-state-",
  });
});
...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses DAISYCLAW_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      DAISYCLAW_HOME: "/srv/daisyclaw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/daisyclaw-home"), ".daisyclaw", "workspace"));
  });

  it("prefers DAISYCLAW_WORKSPACE_DIR for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      DAISYCLAW_WORKSPACE_DIR: "/srv/daisyclaw-workspace",
      DAISYCLAW_HOME: "/srv/daisyclaw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.resolve("/srv/daisyclaw-workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".daisyclaw", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("daisyclaw-workspace-");
  ...
  it("refuses to re-seed a recently attested workspace after only generated git metadata survives", async () => {
    ...
    await fs.mkdir(path.join(tempDir, ".daisyclaw"), { recursive: true });
  ...
  it("does not overwrite a sibling file that is not an DaisyClaw attestation marker", async () => {
    const tempDir = await makeTempWorkspace("daisyclaw-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-workspace-hardlink-"));
    try {
