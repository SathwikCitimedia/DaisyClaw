...
describe("initSessionState BodyStripped", () => {
  it("prefers BodyForAgent over Body for group chats", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-sender-meta-"));
  ...
  it("prefers BodyForAgent over Body for direct chats", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-sender-meta-direct-"));
    const storePath = path.join(root, "sessions.json");
    const cfg = { session: { store: storePath } } as DaisyClawConfig;
...
describe("mention helpers", () => {
  it("builds regexes and skips invalid or unsafe patterns", () => {
    const regexes = buildMentionRegexes({
      messages: {
        groupChat: { mentionPatterns: ["\\bdaisyclaw\\b", "(invalid", "(a+)+$"] },
      },
    });
    expect(regexes).toHaveLength(1);
    expect(regexes[0]?.test("daisyclaw")).toBe(true);
  });

  it("normalizes zero-width characters", () => {
    expect(normalizeMentionText("open\u200bclaw")).toBe("daisyclaw");
  });

  it("matches patterns case-insensitively", () => {
    const regexes = buildMentionRegexes({
      messages: { groupChat: { mentionPatterns: ["\\bdaisyclaw\\b"] } },
    });
    expect(matchesMentionPatterns("DAISYCLAW: hi", regexes)).toBe(true);
  });
  ...
  it("scopes configured mention patterns by provider conversation policy", () => {
    ...
    expect(matchesMentionPatterns("daisyclaw: hi", allowed)).toBe(true);
    expect(matchesMentionPatterns("daisyclaw: hi", denied)).toBe(false);
  });
  ...
  it("preserves mention patterns for callers without scoped policy facts", () => {
    ...
    expect(matchesMentionPatterns("daisyclaw", regexes)).toBe(true);
  });
  ...
  it("strips safe mention patterns and ignores unsafe ones", () => {
    const stripped = stripMentions("daisyclaw " + "a".repeat(28) + "!", {} as MsgContext, {
      messages: {
        groupChat: { mentionPatterns: ["\\bdaisyclaw\\b", "(a+)+$"] },
      },
    });
