...
function createRemoteStageParams(home: string): {
  ...
  return {
    ...
    workspaceDir: join(home, "daisyclaw"),
    sessionKey,
    remoteCacheDir: join(home, ".daisyclaw", "media", "remote-cache", slugifySessionKey(sessionKey)),
  };
}
...
describe("stageSandboxMedia scp remote paths", () => {
  ...
  it("rejects remote attachment filenames with shell metacharacters before spawning scp", async () => {
    await withSandboxMediaTempHome("daisyclaw-triggers-", async (home) => {
