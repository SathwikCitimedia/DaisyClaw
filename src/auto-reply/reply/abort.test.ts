...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop daisyclaw!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@daisyclaw_bot", { botUsername: "daisyclaw_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@daisyclaw_bot", { botUsername: "daisyclaw_bot" })).toBe(true);
