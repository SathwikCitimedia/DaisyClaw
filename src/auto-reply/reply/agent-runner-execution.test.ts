...
describe("runAgentTurnWithFallback", () => {
  ...
  it("surfaces gateway reauth guidance for known OAuth refresh failures", async () => {
    ...
    if (result.kind === "final") {
      expect(result.payload.text).toBe(
        "⚠️ Model login expired on the gateway for openai. Re-auth with `daisyclaw models auth login --provider openai`, then try again.",
      );
    }
  });
  ...
  it("surfaces typed missing API-key auth guidance without parsing the message", async () => {
    ...
    if (result.kind === "final") {
      expect(result.payload.text).toBe(
        '⚠️ Missing API key for provider "openai". Run `daisyclaw doctor --fix` to repair stale OpenAI model/session routes, restart the gateway if doctor asks, then try again. If doctor has nothing to repair or the error persists, re-auth with `daisyclaw models auth login --provider openai` or run `daisyclaw configure`.',
      );
    }
  });
  ...
  it("formats auth-profile failover copy from typed FailoverError metadata", async () => {
    ...
    if (result.kind === "final") {
      ...
      expect(result.payload.text).toContain("daisyclaw configure");
  ...
  it("falls back to a generic reauth command when the provider in the OAuth error is unsafe", async () => {
    ...
    if (result.kind === "final") {
      expect(result.payload.text).toBe(
        "⚠️ Model login expired on the gateway. Re-auth with `daisyclaw models auth login`, then try again.",
      );
    }
  });
  ...
  it("keeps raw generic errors on internal control surfaces", async () => {
    ...
    if (result.kind === "final") {
      ...
      expect(result.payload.text).toContain("Logs: daisyclaw logs --follow");
    }
  });
  ...
  it("drops authProfileId when fallback switches providers", async () => {
    ...
    followupRun.run.authProfileId = "anthropic:daisyclaw";
  ...
  it("keeps same-provider auth profile when fallback only changes model", async () => {
    ...
    const entry = {
      sessionId: "session",
      updatedAt: 1,
      authProfileOverride: "anthropic:daisyclaw",
    ...
    const { updated } = applyFallbackCandidateSelectionToEntry({
      ...
      run: {
        provider: "anthropic",
        model: "claude-opus",
        authProfileId: "anthropic:daisyclaw",
        authProfileIdSource: "user",
      } as FollowupRun["run"],
      provider: "anthropic",
      model: "claude-sonnet",
