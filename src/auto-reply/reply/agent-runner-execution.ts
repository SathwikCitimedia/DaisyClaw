...
// See: https://github.com/daisyclaw/daisyclaw/issues/58348
...
function buildCodexAppServerFailureText(message: string): string | null {
  ...
  if (CODEX_APP_SERVER_CLIENT_CLOSED_BEFORE_REPLY_RE.test(normalizedMessage)) {
    return "⚠️ Codex app-server connection closed before this turn finished. DaisyClaw retried once when the stdio turn was still replay-safe; please try again if this keeps happening.";
  }
  if (CODEX_APP_SERVER_TURN_COMPLETION_IDLE_TIMEOUT_RE.test(normalizedMessage)) {
    return "⚠️ Codex app-server stopped before confirming turn completion. DaisyClaw did not replay the turn automatically because it may still be active; try again, or use /new if the session stays stuck.";
  }
  return null;
}
...
function buildMissingApiKeyFailureText(input: { message: string; error?: unknown }): string | null {
  ...
  if (provider === "openai") {
    return '⚠️ Missing API key for provider "openai". Run `daisyclaw doctor --fix` to repair stale OpenAI model/session routes, restart the gateway if doctor asks, then try again. If doctor has nothing to repair or the error persists, re-auth with `daisyclaw models auth login --provider openai` or run `daisyclaw configure`.';
  }
        ...
        runWithModelFallback<EmbeddedAgentRunResult>({
          ...
          run: async (provider, model, runOptions) => {
            ...
            const embeddedRunHarnessOverride =
              ...
              (agentHarnessPolicy.runtime === "daisyclaw" && embeddedRunProvider !== provider
                ? "daisyclaw"
                : undefined);
            return (async () => {
                  ...
                  runEmbeddedAgent({
                    ...
                    onToolResult: onToolResult
                      ? (() => {
                          ...
                          // See: https://github.com/daisyclaw/daisyclaw/issues/11044
