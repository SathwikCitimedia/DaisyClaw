...
describe("runReplyAgent media path normalization", () => {
  ...
  it("reuses the provided media context inside runAgentTurnWithFallback", async () => {
    // Regression test for daisyclaw/daisyclaw#68056.
  ...
  it("passes current inbound media paths as native DaisyClaw images", async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), "daisyclaw-native-agent-media-"));
  ...
  it("does not pass recent history images as unlabeled native DaisyClaw images", async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), "daisyclaw-native-agent-history-"));
    cleanupPaths.push(tmpDir);
  ...
  it("falls back to prompt refs instead of forwarding partial current media", async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), "daisyclaw-native-agent-partial-"));
    cleanupPaths.push(tmpDir);
