...
function createMinimalRun(params?: {
  ...
  const followupRun = {
    ...
    run: {
      ...
      skipProviderRuntimeHints: process.env.DAISYCLAW_TEST_FAST === "1",
      ...params?.runOverrides,
    },
...
describe("runReplyAgent pending final delivery capture", () => {
  async function createSessionStoreFile(entry: SessionEntry) {
    const dir = await mkdtemp(join(tmpdir(), "daisyclaw-agent-runner-pending-"));
...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  it("does not persist heartbeat ack text as pending final delivery", async () => {
    const dir = await mkdtemp(join(tmpdir(), "daisyclaw-heartbeat-pending-"));
    const storePath = join(dir, "sessions.json");
  ...
  it("does not persist active fallback state for internal subagent announce fallback", async () => {
    ...
    const sessionStore = { main: sessionEntry };
    const storeRoot = await mkdtemp(join(tmpdir(), "daisyclaw-internal-fallback-"));
  ...
  it("does not persist fallback state for an equivalent CLI runtime alias", async () => {
    ...
    const sessionStore = { main: sessionEntry };
    const dir = await mkdtemp(join(tmpdir(), "daisyclaw-agent-runner-cli-alias-"));
    const storePath = join(dir, "sessions.json");
