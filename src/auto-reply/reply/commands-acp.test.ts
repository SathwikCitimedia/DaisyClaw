...
vi.mock("../../agents/acp-spawn.js", () => ({
  resolveAcpSpawnRuntimePolicyError: (params: { cfg?: DaisyClawConfig }) =>
...
function createDiscordParams(commandBody: string, cfg: DaisyClawConfig = baseCfg) {
...
function createThreadParams(commandBody: string, cfg: DaisyClawConfig = baseCfg) {
...
async function runDiscordAcpCommand(commandBody: string, cfg: DaisyClawConfig = baseCfg) {
  return handleAcpCommand(createDiscordParams(commandBody, cfg), true);
}

async function runThreadAcpCommand(commandBody: string, cfg: DaisyClawConfig = baseCfg) {
  return handleAcpCommand(createThreadParams(commandBody, cfg), true);
}

async function runTelegramAcpCommand(commandBody: string, cfg: DaisyClawConfig = baseCfg) {
...
async function runTelegramDmAcpCommand(commandBody: string, cfg: DaisyClawConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runSlackDmAcpCommand(commandBody: string, cfg: DaisyClawConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
function createMatrixThreadParams(commandBody: string, cfg: DaisyClawConfig = baseCfg) {
...
async function runMatrixAcpCommand(commandBody: string, cfg: DaisyClawConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runMatrixThreadAcpCommand(commandBody: string, cfg: DaisyClawConfig = baseCfg) {
  return handleAcpCommand(createMatrixThreadParams(commandBody, cfg), true);
}

async function runFeishuDmAcpCommand(commandBody: string, cfg: DaisyClawConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runLineDmAcpCommand(commandBody: string, cfg: DaisyClawConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runIMessageDmAcpCommand(commandBody: string, cfg: DaisyClawConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
describe("/acp command", () => {
  ...
  it("inherits the target agent workspace when /acp spawn omits --cwd", async () => {
    ...
    const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-codex-"));
    try {
      const cfg = {
        ...baseCfg,
        agents: {
  ...
  it("sanitizes leaked task and runtime details in ACP status output", async () => {
    ...
    expect(result?.reply?.text).not.toContain("DaisyClaw runtime context (internal):");
