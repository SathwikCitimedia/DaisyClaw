...
function withAcpConfig(acp: DaisyClawConfig["acp"]): DaisyClawConfig {
  return { acp } as DaisyClawConfig;
}
...
describe("ACP install hints", () => {
  it("prefers explicit runtime install command", () => {
    const cfg = withAcpConfig({
      runtime: { installCommand: "pnpm daisyclaw plugins install acpx" },
    });
    expect(resolveAcpInstallCommandHint(cfg)).toBe("pnpm daisyclaw plugins install acpx");
  });
  ...
  it("uses local acpx extension path when present", () => {
    ...
    expect(hint).toBe(`daisyclaw plugins install ${path.join(repoRoot, "extensions", "acpx")}`);
  });
  ...
  it("falls back to scoped install hint for acpx when local extension is absent", () => {
    ...
    const cfg = withAcpConfig({ backend: "acpx" });
    expect(resolveAcpInstallCommandHint(cfg)).toBe("daisyclaw plugins install acpx");
  });
