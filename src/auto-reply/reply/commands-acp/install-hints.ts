...
export function resolveAcpInstallCommandHint(cfg: DaisyClawConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(workspaceLocalPath)) {
      return `daisyclaw plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^daisyclaw plugins install /u, "");
    ...
    return "daisyclaw plugins install acpx";
  }
