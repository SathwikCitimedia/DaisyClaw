...
async function withTempConfigPath<T>(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-allowlist-config-"));
  const configPath = path.join(dir, "daisyclaw.json");
  const previous = process.env.DAISYCLAW_CONFIG_PATH;
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.DAISYCLAW_CONFIG_PATH = previous;
    }
        ...
        run: async () => {
          await withTempConfigPath(
            ...
            async (configPath) => {
              ...
              const written = await readJsonFile<DaisyClawConfig>(configPath);
      ...
      createTestRegistry([
        {
          ...
          plugin: {
            ...
            config: {
              ...
              defaultAccountId: (cfg: DaisyClawConfig) =>
                (cfg.channels?.telegram as TelegramTestSectionConfig | undefined)?.defaultAccount ??
                DEFAULT_ACCOUNT_ID,
            },
          },
