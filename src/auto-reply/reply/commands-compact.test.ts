...
function requireResolveSessionAgentIdCall(index = 0) {
  ...
  )?.[0] as { sessionKey?: string; config?: DaisyClawConfig } | undefined;
  if (!call) {
...
function requireResolveAgentDirCall(index = 0) {
  const call = resolveAgentDirMock.mock.calls[index] as [DaisyClawConfig, string] | undefined;
  if (!call) {
...
describe("handleCompactCommand", () => {
  beforeEach(() => {
    ...
    resolveAgentDirMock.mockImplementation(
      (_cfg: unknown, agentId: string) => `/tmp/workspace/.daisyclaw/agents/${agentId}/agent`,
    );
  ...
  it("routes manual compaction with explicit trigger and context metadata", async () => {
    ...
    const result = await handleCompactCommand(
      {
        ...buildCompactParams("/compact", {
          commands: { text: true },
          channels: { whatsapp: { allowFrom: ["*"] } },
          session: { store: "/tmp/daisyclaw-session-store.json" },
        } as DaisyClawConfig),
        ctx: {
          Provider: "whatsapp",
          Surface: "whatsapp",
        ...
        agentDir: "/tmp/daisyclaw-agent-compact",
        sessionEntry: {
          sessionId: "session-1",
          updatedAt: Date.now(),
          groupId: "group-1",
    ...
    expect(call.agentDir).toBe("/tmp/daisyclaw-agent-compact");
