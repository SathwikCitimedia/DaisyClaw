...
import { buildCurrentDaisyClawCliCommand } from "./commands-daisyclaw-cli.js";
import {
...
const DIAGNOSTICS_DOCS_URL = "https://docs.daisyclaw.ai/gateway/diagnostics";
const GATEWAY_DIAGNOSTICS_EXPORT_JSON_LABEL = "daisyclaw gateway diagnostics export --json";
...
function buildGatewayDiagnosticsExportJsonCommand(): string {
  return buildCurrentDaisyClawCliCommand(["gateway", "diagnostics", "export", "--json"]);
}
...
function isCodexDiagnosticsUnavailableText(text: string | undefined): boolean {
  return (
    text?.startsWith("No Codex thread is attached to this DaisyClaw session yet.") === true ||
    text?.startsWith(
      "Cannot send Codex diagnostics because this command did not include an DaisyClaw session file.",
    ) === true
  );
}
