...
describe("buildExportSessionReply", () => {
  ...
  it("injects scripts and session data through the real export template", async () => {
    ...
    expect(html).not.toContain("data-daisyclaw-export-placeholder");
  ...
  it("suffixes colliding default export filenames instead of overwriting", async () => {
    ...
    const expectedBase = path.join(
      "/tmp/workspace",
      "daisyclaw-session-session--2026-05-05T10-11-12.html",
    );
    const expectedSuffix = path.join(
      "/tmp/workspace",
      "daisyclaw-session-session--2026-05-05T10-11-12-2.html",
    );
    ...
    expect(reply.text).toContain("📄 File: daisyclaw-session-session--2026-05-05T10-11-12-2.html");
  });
  ...
  it("preserves replacement text with dollar sequences", async () => {
    hoisted.exportHtmlTemplateContents.set(
      "template.html",
      [
        '<style data-daisyclaw-export-placeholder="CSS"></style>',
        '<script id="session-data" type="application/json" data-daisyclaw-export-placeholder="SESSION_DATA"></script>',
        '<script data-daisyclaw-export-placeholder="MARKED_JS"></script>',
        '<script data-daisyclaw-export-placeholder="HIGHLIGHT_JS"></script>',
        '<script data-daisyclaw-export-placeholder="JS"></script>',
      ].join(""),
    );
