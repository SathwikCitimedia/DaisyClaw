...
vi.mock("../../config/config.js", () => ({
  ...
  transformConfigFileWithRetry: async (params: {
    afterWrite?: unknown;
    transform: (
      currentConfig: DaisyClawConfig,
      context: { snapshot: ConfigSnapshotMock; previousHash: string | null; attempt: number },
    ) =>
      | Promise<{ nextConfig: DaisyClawConfig; result?: unknown }>
      | {
    ...
    return {
      path: snapshot.path ?? "/tmp/daisyclaw.json",
      previousHash,
...
function buildParams(commandBody: string, cfg: DaisyClawConfig): HandleCommandsParams {
  const ctx = {
    Body: commandBody,
...
describe("command gating", () => {
  ...
  it("blocks disabled bash", async () => {
    const result = await handleBashChatCommand({
      ...
      cfg: { commands: { bash: false } } as DaisyClawConfig,
      sessionKey: "agent:main:main",
      isGroup: false,
      elevated: { enabled: true, allowed: true, failures: [] },
    });
  ...
  it("blocks bash when elevated access is unavailable", async () => {
    const result = await handleBashChatCommand({
      ...
      cfg: { commands: { bash: true } } as DaisyClawConfig,
      sessionKey: "agent:main:main",
      isGroup: false,
      elevated: {
        enabled: true,
  ...
  it("redacts secret-shaped fields from full /config show replies", async () => {
    readConfigFileSnapshotMock.mockResolvedValueOnce({
      ...
      parsed: {
        gateway: {
          auth: {
            mode: "token",
            token: "DAISYCLAW_CONFIG_SHOW_CANARY_TOKEN_65623",
            password: "DAISYCLAW_CONFIG_SHOW_CANARY_PASSWORD_65623",
          },
          bind: "127.0.0.1",
          port: 3210,
        },
        models: {
          providers: {
            openai: {
              apiKey: "DAISYCLAW_CONFIG_SHOW_CANARY_API_KEY_65623",
        ...
        browser: {
          cdpUrl:
            "wss://chrome.example.test/devtools?token=DAISYCLAW_CONFIG_SHOW_CANARY_CDP_TOKEN_65623&apiKey=DAISYCLAW_CONFIG_SHOW_CANARY_CDP_API_KEY_65623",
          profiles: {
            local: {
            ...
            remote: {
              cdpUrl:
                "wss://chrome.remote.example.test/devtools?apiKey=DAISYCLAW_CONFIG_SHOW_CANARY_CDP_PROFILE_API_KEY_65623",
            },
          },
        },
        talk: {
    ...
    expect(output).not.toContain("DAISYCLAW_CONFIG_SHOW_CANARY_TOKEN_65623");
    expect(output).not.toContain("DAISYCLAW_CONFIG_SHOW_CANARY_PASSWORD_65623");
    expect(output).not.toContain("DAISYCLAW_CONFIG_SHOW_CANARY_API_KEY_65623");
    expect(output).not.toContain("DAISYCLAW_CONFIG_SHOW_CANARY_CDP_TOKEN_65623");
    expect(output).not.toContain("DAISYCLAW_CONFIG_SHOW_CANARY_CDP_API_KEY_65623");
    expect(output).not.toContain("DAISYCLAW_CONFIG_SHOW_CANARY_CDP_PROFILE_API_KEY_65623");
  ...
  it("redacts secret-shaped values from /config set acknowledgements", async () => {
    ...
    const params = buildParams(
      '/config set gateway.auth.token="DAISYCLAW_CONFIG_SET_CANARY_TOKEN_65623"',
      {
        commands: { config: true, text: true },
        channels: { whatsapp: { allowFrom: ["*"] } },
      } as DaisyClawConfig,
    ...
    expect(output).toContain(REDACTED_SENTINEL);
    expect(output).not.toContain("DAISYCLAW_CONFIG_SET_CANARY_TOKEN_65623");
  });
  ...
  it("ignores inherited command flags", () => {
    ...
    const cfg = { commands: inheritedCommands as never } as DaisyClawConfig;
  ...
  it("enforces gateway client permissions for /config commands", async () => {
    const baseCfg = { commands: { config: true, text: true } } as DaisyClawConfig;
