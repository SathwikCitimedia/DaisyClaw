...
const mocks = vi.hoisted(() => ({
  ...
  formatDoctorNonInteractiveHint: vi.fn(
    () =>
      "Recommended follow-up: run daisyclaw doctor --non-interactive in a terminal or approvals-capable DaisyClaw surface.",
  ),
  ...
  triggerDaisyClawRestart: vi.fn(() => ({ ok: true, method: "launchctl" })),
}));
...
vi.mock("../../infra/restart.js", () => ({
  ...
  triggerDaisyClawRestart: mocks.triggerDaisyClawRestart,
}));
...
describe("handleRestartCommand", () => {
  beforeEach(() => {
    ...
    mocks.triggerDaisyClawRestart.mockReset();
    mocks.triggerDaisyClawRestart.mockReturnValue({ ok: true, method: "launchctl" });
  });
  ...
  it("writes a routed restart sentinel before restarting from chat", async () => {
    ...
    expect(mocks.triggerDaisyClawRestart).toHaveBeenCalledTimes(1);
  });
  ...
  it("prepares the routed sentinel only when SIGUSR1 restart emits", async () => {
    ...
    try {
      ...
      expect(mocks.triggerDaisyClawRestart).not.toHaveBeenCalled();
  ...
  it("removes the success sentinel when fallback restart fails", async () => {
    mocks.triggerDaisyClawRestart.mockReturnValueOnce({
      ok: false,
      method: "launchctl",
    });
