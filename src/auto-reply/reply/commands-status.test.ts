...
vi.mock("../../agents/harness/builtin-daisyclaw.js", () => ({
  createDaisyClawAgentHarness: () => ({
    id: "daisyclaw",
    label: "DaisyClaw Default",
...
function saveStatusTestAuthProfiles(params: {
  ...
  const agentDir = path.join(params.dir, ".daisyclaw", "agents", "main", "agent");
...
describe("buildStatusReply subagent summary", () => {
  ...
  it("does not leak internal runtime context through the task status line", async () => {
    ...
    expect(reply?.text).not.toContain("DaisyClaw runtime context (internal):");
  ...
  it("shows the effective non-DaisyClaw embedded harness in /status", async () => {
  ...
  it("uses Codex OAuth auth labels for openai models running on the Codex harness", async () => {
    ...
    await withTempHome(
      async (dir) => {
        const agentDir = path.join(dir, ".daisyclaw", "agents", "main", "agent");
        fs.mkdirSync(agentDir, { recursive: true });
        saveAuthProfileStore(
          {
            version: 1,
  ...
  it("uses Codex OAuth auth labels for explicit OpenAI DaisyClaw auth order", async () => {
    await withTempHome(
      async (dir) => {
        const agentDir = path.join(dir, ".daisyclaw", "agents", "main", "agent");
        fs.mkdirSync(agentDir, { recursive: true });
  ...
  it("uses workspace-scoped auth evidence in /status auth labels", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-status-auth-label-"));
    const workspaceDir = path.join(tempRoot, "workspace");
    const pluginDir = path.join(workspaceDir, ".daisyclaw", "extensions", "workspace-auth-label");
  ...
  it("keeps /status on a session-pinned DaisyClaw harness after config changes", async () => {
    registerStatusCodexHarness();
