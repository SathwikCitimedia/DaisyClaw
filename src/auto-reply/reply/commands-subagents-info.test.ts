...
const TEST_SESSION_STORE_PATH = path.join(
  os.tmpdir(),
  `daisyclaw-commands-subagents-info-${process.pid}.json`,
);

function buildCommandTestConfig(): DaisyClawConfig {
  return {
    ...baseCommandTestConfig,
    session: {
...
function buildInfoContext(params: { cfg: DaisyClawConfig; runs: object[]; restTokens: string[] }) {
  return {
    params: {
      cfg: params.cfg,
...
describe("subagents info", () => {
  ...
  it("sanitizes leaked task details in /subagents info", () => {
    ...
    expect(text).not.toContain("DaisyClaw runtime context (internal):");
