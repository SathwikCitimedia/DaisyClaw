...
import { createDaisyClawCodingTools } from "../../agents/agent-tools.js";
...
const { createDaisyClawCodingToolsMock } = vi.hoisted(() => ({
  createDaisyClawCodingToolsMock: vi.fn(() => []),
}));
...
vi.mock("../../agents/agent-tools.js", () => ({
  createDaisyClawCodingTools: createDaisyClawCodingToolsMock,
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    createDaisyClawCodingToolsMock.mockClear();
  ...
  it("opts command tool builds into gateway subagent binding", async () => {
    ...
    const toolParams = requireFirstArg(
      vi.mocked(createDaisyClawCodingTools),
  ...
  it("uses materialized sandbox skill paths for sandbox command prompts", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-command-sandbox-skills-"));
    try {
      ...
      vi.mocked(resolveReusableWorkspaceSkillSnapshot).mockReturnValue({
        snapshot: {
          prompt:
            "<available_skills>~/.npm-global/lib/node_modules/daisyclaw/skills/gog/SKILL.md</available_skills>",
          skills: [],
          resolvedSkills: [],
        },
        shouldRefresh: false,
      ...
      expect(result.skillsPrompt).toContain(
        "/workspace/.daisyclaw/sandbox-skills/skills/gog/SKILL.md",
      );
  ...
  it("uses config-backed prompt settings for the target agent", async () => {
    ...
    createDaisyClawCodingToolsMock.mockReturnValue([{ name: "sessions_spawn" }] as never);
    const params = makeParams();
    params.cfg = {
      agents: {
        defaults: {
