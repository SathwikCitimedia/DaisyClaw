...
import { mimeTypeFromFilePath } from "@daisyclaw/media-core/mime";
...
export async function resolveCurrentTurnImages(params: {
  ...
  try {
    ...
    if (images.length < undescribedImageAttachments.length) {
      logVerbose(
        `agent-runner: native DaisyClaw media resolution produced ${images.length}/${undescribedImageAttachments.length} current image attachment(s); falling back to prompt image refs`,
      );
      return { images: params.images, imageOrder: params.imageOrder };
    }
