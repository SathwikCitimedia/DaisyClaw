...
import type { ModelDefinitionConfig, DaisyClawConfig } from "../../config/config.js";
...
function baseConfig(): DaisyClawConfig {
  return {
    commands: { text: true },
    agents: { defaults: {} },
...
function resolveModelSelectionForCommand(params: {
  ...
  return resolveModelSelectionFromDirective({
    ...
    cfg: { commands: { text: true } } as unknown as DaisyClawConfig,
    agentDir: TEST_AGENT_DIR,
...
describe("/model chat UX", () => {
  ...
  it("passes workspace scope through the /model list browser alias", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-model-list-auth-label-"));
    const workspaceDir = path.join(tempRoot, "workspace");
    const pluginDir = path.join(workspaceDir, ".daisyclaw", "extensions", "workspace-model-list");
  ...
  it("does not borrow Codex auth when OpenAI model policy pins DaisyClaw runtime", async () => {
    setAuthProfiles({
      "openai:patrick@example.test": {
        type: "oauth",
        provider: "openai",
  ...
  it("uses workspace-scoped auth evidence in /model status labels", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-model-status-auth-label-"));
    const workspaceDir = path.join(tempRoot, "workspace");
    const pluginDir = path.join(workspaceDir, ".daisyclaw", "extensions", "workspace-model-auth");
    const bundledDir = path.join(tempRoot, "bundled");
    const stateDir = path.join(tempRoot, "state");
    const credentialPath = path.join(tempRoot, "credentials.json");
    fs.mkdirSync(pluginDir, { recursive: true });
  ...
  it("auto-applies closest match for typos", () => {
    ...
    const cfg = { commands: { text: true } } as unknown as DaisyClawConfig;
  ...
  it("includes additive allowlist repair when a runtime switch targets a blocked model", () => {
    ...
    expect(resolved.errorText).toContain(
      `daisyclaw config set agents.defaults.models '{"openai/gpt-5.5":{}}' --strict-json --merge`,
    );
    expect(resolved.errorText).toContain("Then retry: /model openai/gpt-5.5 --runtime codex");
    expect(resolved.errorText).toContain("daisyclaw plugins enable codex");
  });
  ...
  it("ignores runtime overrides that do not belong to the selected provider", async () => {
    ...
    const { sessionEntry } = await persistModelDirectiveForTest({
      ...
      allowedModelKeys: ["openai/gpt-4o"],
      sessionEntry: createSessionEntry({ agentRuntimeOverride: "daisyclaw" }),
      provider: "openai",
      model: "gpt-4o",
      initialModelLabel: "openai/gpt-4o",
    });
