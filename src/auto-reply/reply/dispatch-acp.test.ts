...
const policyMocks = vi.hoisted(() => ({
  resolveAcpDispatchPolicyError: vi.fn<(cfg: DaisyClawConfig) => AcpRuntimeError | null>(() => null),
  resolveAcpAgentPolicyError: vi.fn<(cfg: DaisyClawConfig, agent: string) => AcpRuntimeError | null>(
    () => null,
  ),
}));
...
const sessionMetaMocks = vi.hoisted(() => ({
  readAcpSessionEntry: vi.fn<
    (params: { sessionKey: string; cfg?: DaisyClawConfig }) => AcpSessionStoreEntry | null
  >(() => null),
}));
...
vi.mock("../../acp/policy.js", () => ({
  resolveAcpDispatchPolicyError: (cfg: DaisyClawConfig) =>
    policyMocks.resolveAcpDispatchPolicyError(cfg),
  resolveAcpAgentPolicyError: (cfg: DaisyClawConfig, agent: string) =>
...
function createAcpConfigWithVisibleToolTags(): DaisyClawConfig {
...
describe("tryDispatchAcpReply", () => {
  ...
  it("preserves chat.send inline image attachments over recent history images", async () => {
    ...
    const historyPath = "/tmp/daisyclaw-history-inline.png";
    ...
    sessionMetaMocks.readAcpSessionEntry.mockImplementation(
      (params: { sessionKey: string; cfg?: DaisyClawConfig }) =>
        params.sessionKey === canonicalSessionKey
          ? {
              cfg: params.cfg ?? createAcpTestConfig(),
              storePath: "/tmp/daisyclaw-session-store.json",
              sessionKey: canonicalSessionKey,
  ...
  it("marks accumulated ACP block TTS finals as trusted local media for WebChat", async () => {
    ...
    ttsMocks.resolveTtsConfig.mockReturnValue({ mode: "final" });
    queueTtsReplies({
      mediaUrl: "/tmp/daisyclaw-media/acp-tts.ogg",
      audioAsVoice: true,
    } as MockTtsReply);
    ...
    expect(finalPayload.mediaUrl).toBe("/tmp/daisyclaw-media/acp-tts.ogg");
