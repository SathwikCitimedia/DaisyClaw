...
const acpMocks = vi.hoisted(() => ({
  ...
  readAcpSessionEntry: vi.fn<(params: { sessionKey: string; cfg?: DaisyClawConfig }) => unknown>(
    () => null,
  ),
  readAcpSessionMeta: vi.fn<(params: { sessionKey: string; cfg?: DaisyClawConfig }) => unknown>(
    () => null,
  ),
...
const ttsMocks = vi.hoisted(() => {
  ...
  return {
    ...
    resolveTtsConfig: vi.fn((_cfg: DaisyClawConfig) => ({ mode: "final" })),
  };
});
...
vi.mock("../../tts/tts.js", () => ({
  ...
  resolveTtsConfig: (cfg: DaisyClawConfig) => ttsMocks.resolveTtsConfig(cfg),
}));
...
vi.mock("./dispatch-acp-session.runtime.js", () => ({
  readAcpSessionEntry: (params: { sessionKey: string; cfg?: DaisyClawConfig }) =>
    acpMocks.readAcpSessionEntry(params),
}));
vi.mock("../../tts/tts-config.js", () => ({
  normalizeTtsAutoMode: (value: unknown) => ttsMocks.normalizeTtsAutoMode(value),
  resolveConfiguredTtsMode: (cfg: DaisyClawConfig) => ttsMocks.resolveTtsConfig(cfg).mode,
  shouldCleanTtsDirectiveText: () => true,
  shouldAttemptTtsPayload: () => true,
}));

const noAbortResult = { handled: false, aborted: false } as const;
const emptyConfig = {} as DaisyClawConfig;
...
} as const satisfies DaisyClawConfig;
...
function createMockAcpSessionManager() {
  return {
    resolveSession: (params: { cfg: DaisyClawConfig; sessionKey: string }) => {
...
describe("dispatchReplyFromConfig", () => {
  ...
  it("skips a Telegram topic heartbeat turn while a reply operation is active", async () => {
    ...
    const result = await dispatchReplyFromConfig({
      ctx: buildTestCtx({
        ...
        TransportThreadId: 3731,
        To: "telegram:-1003774691294:topic:3731",
        BodyForAgent: "[DaisyClaw heartbeat poll]",
      }),
      cfg: automaticGroupReplyConfig,
      dispatcher,
  ...
  it("does not pre-latch terminal tool-error suppression when diagnostics are disabled", async () => {
    ...
    await dispatchReplyFromConfig({
      ctx,
      cfg: { diagnostics: { enabled: false } } as DaisyClawConfig,
      dispatcher,
      replyResolver,
    });
  ...
  it("retargets reply_dispatch to a bound generic ACP session before model fallback", async () => {
    ...
    acpMocks.readAcpSessionEntry.mockImplementation(
      (params: { sessionKey: string; cfg?: DaisyClawConfig }) =>
  ...
  it("normalizes accumulated block TTS-only media before final delivery", async () => {
    ...
    replyMediaPathMocks.createReplyMediaPathNormalizer.mockReturnValue(
      async (payload: ReplyPayload) => ({
        ...payload,
        mediaUrl: "/tmp/daisyclaw-media/normalized-tts.ogg",
        mediaUrls: ["/tmp/daisyclaw-media/normalized-tts.ogg"],
      }),
    );
    const dispatcher = createDispatcher();
    const ctx = buildTestCtx({
    ...
    const finalPayload = firstFinalReplyPayload(dispatcher);
    expect(finalPayload?.mediaUrl).toBe("/tmp/daisyclaw-media/normalized-tts.ogg");
    expect(finalPayload?.mediaUrls).toStrictEqual(["/tmp/daisyclaw-media/normalized-tts.ogg"]);
  ...
  it("emits diagnostics when enabled", async () => {
    setNoAbort();
    const cfg = { diagnostics: { enabled: true } } as DaisyClawConfig;
    const dispatcher = createDispatcher();
    const ctx = buildTestCtx({
      Provider: "slack",
      Surface: "slack",
  ...
  it("routes plugin-owned bindings to the owning plugin before generic inbound claim broadcast", async () => {
    ...
    hookMocks.registry.plugins = [{ id: "daisyclaw-codex-app-server", status: "loaded" }];
    hookMocks.runner.runInboundClaimForPluginOutcome.mockResolvedValue({
      status: "handled",
      result: { handled: true },
    });
    sessionBindingMocks.resolveByConversation.mockReturnValue({
      ...
      metadata: {
        ...
        pluginRoot: "/Users/huntharo/github/daisyclaw-app-server",
        data: {
          kind: "codex-app-server-session",
          version: 1,
          sessionFile: "/tmp/session.jsonl",
          workspaceDir: "/workspace/daisyclaw",
        },
      },
    } satisfies SessionBindingRecord);
    const cfg = emptyConfig;
    ...
    const inboundClaimCall = hookMocks.runner.runInboundClaimForPluginOutcome.mock
      ...
      | undefined;
    expect(inboundClaimCall?.[0]).toBe("daisyclaw-codex-app-server");
        ...
        ref.conversationId === "1510164477642014740"
          ? ({
              ...
              metadata: {
                pluginBindingOwner: "plugin",
                pluginId: "daisyclaw-codex-app-server",
                pluginRoot: "/plugins/daisyclaw-codex-app-server",
              },
            } satisfies SessionBindingRecord)
          : null,
    );
  ...
  it("does not run plugin-owned binding delivery when the caller already aborted", async () => {
    ...
    sessionBindingMocks.resolveByConversation.mockReturnValue({
      ...
      metadata: {
        pluginBindingOwner: "plugin",
        pluginId: "daisyclaw-codex-app-server",
        pluginRoot: "/workspace/daisyclaw-app-server",
        data: {
          kind: "codex-app-server-session",
          version: 1,
          sessionFile: "/tmp/session.jsonl",
  ...
  it("falls back to DaisyClaw once per startup when a bound plugin is missing", async () => {
    setNoAbort();
    hookMocks.runner.hasHooks.mockImplementation(
      ((hookName?: string) =>
        hookName === "inbound_claim" || hookName === "message_received") as () => boolean,
    ...
    const replyResolver = vi.fn(async () => ({ text: "daisyclaw fallback" }) satisfies ReplyPayload);
  ...
  it("falls back to DaisyClaw when the bound plugin is loaded but has no inbound_claim handler", async () => {
    setNoAbort();
    hookMocks.runner.hasHooks.mockImplementation(
      ((hookName?: string) =>
        hookName === "inbound_claim" || hookName === "message_received") as () => boolean,
  ...
  it("releases inbound dedupe when dispatch fails before completion", async () => {
    ...
    const replyResolver = vi
      .fn<
        (_ctx: MsgContext, _opts?: GetReplyOptions, _cfg?: DaisyClawConfig) => Promise<ReplyPayload>
      >()
  ...
  it("passes the loaded config plus configOverride patch to replyResolver when provided", async () => {
    ...
    let receivedCfg: DaisyClawConfig | undefined;
    const replyResolver = async (
      _ctx: MsgContext,
      _opts?: GetReplyOptions,
      cfgArg?: DaisyClawConfig,
    ) => {
      receivedCfg = cfgArg;
      return { text: "hi" } satisfies ReplyPayload;
    };
  ...
  it("passes the already loaded config to replyResolver when configOverride is not provided", async () => {
    setNoAbort();
    const cfg = { agents: { defaults: { userTimezone: "UTC" } } } as DaisyClawConfig;
    const dispatcher = createDispatcher();
...
describe("sendPolicy deny — suppress delivery, not processing (#53328)", () => {
  ...
  it("passes suppressUserDelivery to tail reply_dispatch when sendPolicy is deny", async () => {
    ...
    await dispatchReplyFromConfig({
      ctx,
      cfg: { diagnostics: { enabled: true } } as DaisyClawConfig,
      dispatcher,
  ...
  it("suppresses tool result delivery when sendPolicy is deny", async () => {
    ...
    const replyResolver = vi.fn(
      async (_ctx: MsgContext, opts?: GetReplyOptions, _cfg?: DaisyClawConfig) => {
  ...
  it("keeps unmentioned plugin-bound fallback from ordinary group agent dispatch", async () => {
    ...
    expect(claimCall[0]).toBe("daisyclaw-codex-app-server");
  ...
  it("lets authorized control commands without CommandSource escape plugin-bound fallback", async () => {
    ...
    const cfg = { messages: { visibleReplies: "message_tool" } } as DaisyClawConfig;
    const dispatcher = createDispatcher();
    ...
    const ctx = buildTestCtx({
      ...
      ChatType: "group",
      GroupSubject: "Dev",
      Body: "/reset@daisyclaw",
      RawBody: "/reset@daisyclaw",
      CommandBody: "/reset@daisyclaw",
      BotUsername: "daisyclaw",
  ...
  it("preserves configured message-tool delivery for internal WebChat direct replies", async () => {
    ...
    const result = await dispatchReplyFromConfig({
      ...
      cfg: { messages: { visibleReplies: "message_tool" } } as DaisyClawConfig,
      dispatcher,
      replyResolver,
    });
  ...
  it("lets config restore automatic Codex direct source delivery", async () => {
    ...
    const result = await dispatchReplyFromConfig({
      ...
      cfg: { messages: { visibleReplies: "automatic" } } as DaisyClawConfig,
      dispatcher,
      replyResolver,
    });
  ...
  it("falls back when a channel precomputed message-tool-only delivery but the message tool is unavailable", async () => {
    ...
    const result = await dispatchReplyFromConfig({
      ...
      cfg: { tools: { allow: ["read"] } } as DaisyClawConfig,
      dispatcher,
      replyResolver,
      replyOptions: {
        sourceReplyDeliveryMode: "message_tool_only",
