...
const COMPLETE_REPLY_CONFIG_SYMBOL = Symbol.for("daisyclaw.reply.complete-config");
const FULL_REPLY_RUNTIME_SYMBOL = Symbol.for("daisyclaw.reply.full-runtime");

type ReplyConfigWithMarker = DaisyClawConfig & {
...
function isSlowReplyTestAllowed(env: NodeJS.ProcessEnv = process.env): boolean {
  return (
    env.DAISYCLAW_ALLOW_SLOW_REPLY_TESTS === "1" || env.DAISYCLAW_STRICT_FAST_REPLY_CONFIG === "0"
  );
}
...
export function markCompleteReplyConfig<T extends DaisyClawConfig>(
  config: T,
...
export function withFastReplyConfig<T extends DaisyClawConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "fast" });
}

export function withFullRuntimeReplyConfig<T extends DaisyClawConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "full" });
}

function isCompleteReplyConfig(config: unknown): config is DaisyClawConfig {
  return Boolean(
    config &&
...
export function resolveGetReplyConfig(params: {
  getRuntimeConfig: () => DaisyClawConfig;
  ...
  if (params.isFastTestEnv && !isCompleteReplyConfig(configOverride) && !isSlowReplyTestAllowed()) {
    throw new Error(
      "Fast reply tests must pass with withFastReplyConfig()/markCompleteReplyConfig(); set DAISYCLAW_ALLOW_SLOW_REPLY_TESTS=1 to opt out.",
    );
  }
  ...
  return applyMergePatch(params.getRuntimeConfig(), configOverride) as DaisyClawConfig;
}
