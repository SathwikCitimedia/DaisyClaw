...
vi.mock("../../agents/daisyclaw-tools.runtime.js", () => ({
  createDaisyClawTools: (...args: unknown[]) => createDaisyClawToolsMock(...args),
}));
...
function mockToolDispatchedSkillCommand() {
  ...
  createDaisyClawToolsMock.mockReturnValue([
    {
      name: "send_status",
      execute: toolExecute,
    },
...
describe("handleInlineActions", () => {
  beforeEach(() => {
    ...
    createDaisyClawToolsMock.mockReset();
  ...
  it("passes requesterAgentIdOverride into inline tool runtimes", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    expect(
      mockObjectArg(createDaisyClawToolsMock, "createDaisyClawTools").requesterAgentIdOverride,
    ).toBe("named-worker");
  ...
  it("passes sender identity into inline tool runtimes", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    const toolsArgs = mockObjectArg(createDaisyClawToolsMock, "createDaisyClawTools");
  ...
  it("applies subagent policy to ACP envelope inline dispatch sessions", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-inline-acp-policy-"));
    try {
  ...
  it("passes sandboxed runtime state into inline tool construction", async () => {
    ...
    expect(createDaisyClawToolsMock).toHaveBeenCalledWith(
