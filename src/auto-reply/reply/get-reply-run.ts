...
import { asDateTimestampMs } from "@daisyclaw/normalization-core/number-coercion";
...
type AgentDefaults = NonNullable<DaisyClawConfig["agents"]>["defaults"];
...
type RunPreparedReplyParams = {
  ...
  sessionCfg: DaisyClawConfig["session"];
...
export async function runPreparedReply(
  ...
  const useFastReplyRuntime = shouldUseReplyFastTestRuntime({
    cfg,
    isFastTestEnv: process.env.DAISYCLAW_TEST_FAST === "1",
  });
  ...
  const userTurnInput =
    userTurnTranscriptText !== undefined || userTurnMediaForPersistence.length > 0
      ? {
          ...
          // replayed as history. See: https://github.com/daisyclaw/daisyclaw/issues/3658
