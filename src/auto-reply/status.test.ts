...
describe("buildStatusMessage", () => {
  it("summarizes agent readiness and context usage", () => {
    ...
    expect(normalized).toContain("DaisyClaw");
    ...
    expect(normalized).toContain("Runtime: DaisyClaw Default");
  ...
  it("shows the default DaisyClaw harness as the model runtime", () => {
    const text = buildStatusMessage({
      agent: {
        model: "openai/gpt-5.4",
      },
      sessionEntry: {
        sessionId: "daisyclaw-harness",
        updatedAt: 0,
        fastMode: true,
      },
      sessionKey: "agent:main:main",
    ...
    expect(normalized).toContain("Fast");
    expect(normalized).toContain("Runtime: DaisyClaw Default");
    expect(normalized).not.toContain("· daisyclaw");
  });
