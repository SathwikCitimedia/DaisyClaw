 ...
 *
 * Stores, claims, completes, and tombstones inbound channel events in DaisyClaw state.
 */
...
import type {
  ...
  DB as DaisyClawStateKyselyDatabase,
} from "../../state/daisyclaw-state-db.generated.js";
import {
...
type ChannelIngressDatabase = Pick<DaisyClawStateKyselyDatabase, "channel_ingress_events">;
...
function openStateDatabase(stateDir?: string) {
  return openDaisyClawStateDatabase({
    env: stateDir ? { ...process.env, DAISYCLAW_STATE_DIR: stateDir } : process.env,
  });
}
...
/** Creates a durable channel/account-scoped ingress queue backed by the DaisyClaw state database. */
