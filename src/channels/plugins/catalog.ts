...
import type { DaisyClawPackageManifest } from "../../plugins/manifest.js";
...
const ENV_CATALOG_PATHS = ["DAISYCLAW_PLUGIN_CATALOG_PATHS", "DAISYCLAW_MPM_CATALOG_PATHS"];
...
function resolveOfficialCatalogPaths(options: CatalogOptions): string[] {
  ...
  const packageRoots = uniqueStrings(
    [
      resolveDaisyClawPackageRootSync({ cwd: process.cwd() }),
      resolveDaisyClawPackageRootSync({ moduleUrl: import.meta.url }),
...
function toChannelMeta(params: {
  channel: NonNullable<DaisyClawPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
