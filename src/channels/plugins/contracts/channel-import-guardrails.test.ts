...
const SAME_CHANNEL_SDK_GUARDS: GuardedSource[] = [
  {
    ...
    forbiddenPatterns: [/["']daisyclaw\/plugin-sdk\/discord["']/, /plugin-sdk-internal\/discord/],
  },
  {
    path: bundledPluginFile("slack", "src/shared.ts"),
    forbiddenPatterns: [/["']daisyclaw\/plugin-sdk\/slack["']/, /plugin-sdk-internal\/slack/],
  },
  {
    path: bundledPluginFile("telegram", "src/shared.ts"),
    forbiddenPatterns: [/["']daisyclaw\/plugin-sdk\/telegram["']/, /plugin-sdk-internal\/telegram/],
  },
  {
    path: bundledPluginFile("telegram", "src/account-inspect.ts"),
    forbiddenPatterns: [/["']daisyclaw\/plugin-sdk\/account-resolution["']/],
  },
  {
  ...
  },
  {
    ...
    forbiddenPatterns: [/["']daisyclaw\/plugin-sdk\/imessage["']/, /plugin-sdk-internal\/imessage/],
  },
  {
    path: bundledPluginFile("whatsapp", "src/shared.ts"),
    forbiddenPatterns: [/["']daisyclaw\/plugin-sdk\/whatsapp["']/, /plugin-sdk-internal\/whatsapp/],
  },
  {
    path: bundledPluginFile("signal", "src/shared.ts"),
    forbiddenPatterns: [/["']daisyclaw\/plugin-sdk\/signal["']/, /plugin-sdk-internal\/signal/],
  },
  {
...
const CHANNEL_CONFIG_SCHEMA_GUARDS: GuardedSource[] = [
  {
    ...
    forbiddenPatterns: [/["']daisyclaw\/plugin-sdk\/core["']/],
  },
];
...
function expectNoCrossPluginSdkFacadeImports(file: string, imports: string[]): void {
  ...
  for (const specifier of imports) {
    if (!specifier.startsWith("daisyclaw/plugin-sdk/")) {
      continue;
    }
    const targetSubpath = specifier.slice("daisyclaw/plugin-sdk/".length);
  ...
  it("keeps internalized extension helper surfaces behind local api barrels", () => {
    for (const extensionId of LOCAL_EXTENSION_API_BARREL_GUARDS) {
      for (const file of collectExtensionFiles(extensionId)) {
        ...
        ).not.toMatch(new RegExp(`["']daisyclaw/plugin-sdk/${extensionId}(?:["'/])`, "u"));
      }
    }
  });
});
