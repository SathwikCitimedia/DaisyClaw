...
describe("message action capability checks", () => {
  ...
  it("aggregates capabilities across plugins", () => {
    ...
    expect(listChannelMessageCapabilities({} as DaisyClawConfig).toSorted()).toEqual([
      "delivery-pin",
      "presentation",
    ]);
    expect(channelSupportsMessageCapability({} as DaisyClawConfig, "presentation")).toBe(true);
    expect(channelSupportsMessageCapability({} as DaisyClawConfig, "delivery-pin")).toBe(true);
  });
  ...
  it("checks per-channel capabilities", () => {
    ...
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as DaisyClawConfig, channel: "demo-buttons" },
        "presentation",
      ),
    ).toBe(true);
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as DaisyClawConfig, channel: "demo-cards" },
        "presentation",
      ),
    ).toBe(false);
    expect(
    ...
    expect(
      channelSupportsMessageCapabilityForChannel({ cfg: {} as DaisyClawConfig }, "delivery-pin"),
    ).toBe(false);
  });
  ...
  it("uses unified message tool discovery for actions, capabilities, and schema", () => {
    ...
    expect(listChannelMessageActions({} as DaisyClawConfig)).toEqual(["send", "broadcast", "react"]);
    expect(listChannelMessageCapabilities({} as DaisyClawConfig)).toEqual(["presentation"]);
    expect(
      resolveChannelMessageToolSchemaProperties({
        cfg: {} as DaisyClawConfig,
  ...
  it("skips crashing action/capability discovery paths and logs once", () => {
    ...
    expect(listChannelMessageActions({} as DaisyClawConfig)).toEqual(["send", "broadcast"]);
    expect(listChannelMessageCapabilities({} as DaisyClawConfig)).toStrictEqual([]);
