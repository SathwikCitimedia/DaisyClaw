...
function listBuiltPluginLoaderModuleCandidateUrls(importerUrl: string): URL[] {
  ...
  // ../../plugins candidates would escape the installed daisyclaw package there.
...
function getChannelConfigRecord(cfg: DaisyClawConfig, channelId: string): Record<string, unknown> {
...
function listManifestChannelAccountIds(cfg: DaisyClawConfig, channelId: string): string[] {
...
function rebindChannelPluginConfig(
  ...
  const rebind = (cfg: DaisyClawConfig) =>
...
export function resolveReadOnlyChannelPluginsForConfig(
  ...
  if (externalPluginIds.length > 0) {
    ...
    if (missingConfiguredChannelIds.length > 0 && options.includeSetupFallbackPlugins === true) {
      ...
      const registry = loadPluginLoaderModule().loadDaisyClawPlugins({
        config: cfg,
        activationSourceConfig: options.activationSourceConfig ?? cfg,
        env,
        workspaceDir,
