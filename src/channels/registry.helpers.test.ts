...
describe("channel registry helpers", () => {
  ...
  it("formats Telegram selection lines without a docs prefix and with website extras", () => {
    ...
    expect(line).toContain("https://daisyclaw.ai");
  });

  it("prefers the pinned channel registry when resolving registered plugin channels", () => {
    const startupRegistry = createRegistryWithRegisteredChannel("daisyclaw-weixin", ["weixin"]);
    ...
    expect(listRegisteredChannelPluginIds()).toEqual(["daisyclaw-weixin"]);
    expect(normalizeAnyChannelId("weixin")).toBe("daisyclaw-weixin");
    expect(getRegisteredChannelPluginMeta("DAISYCLAW-WEIXIN")?.aliases).toEqual(["weixin"]);
  });
