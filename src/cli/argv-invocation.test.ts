...
describe("argv-invocation", () => {
  it("resolves root help and empty command path", () => {
    expect(resolveCliArgvInvocation(["node", "daisyclaw", "--help"])).toEqual({
  ...
  it("resolves command path and primary with root options", () => {
    expect(
      resolveCliArgvInvocation(["node", "daisyclaw", "--profile", "work", "gateway", "status"]),
    ).toEqual({
      argv: ["node", "daisyclaw", "--profile", "work", "gateway", "status"],
