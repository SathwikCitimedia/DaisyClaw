...
export function formatCliBannerLine(version: string, options: BannerOptions = {}): string {
  ...
  const title = decorativePrefix("🌼", "DaisyClaw", emojiOptions);
...
/** Build the plain (uncolored) banner art lines, including the centered title. */
export function formatCliBannerArtLines(options: BannerOptions = {}): string[] {
  const width = visibleWidth(DAISYCLAW_ASCII_BODY[0] ?? "");
  const emojiOptions = resolveEmojiOptions(options);
  const title = supportsDecorativeEmoji(emojiOptions) ? "🌼 DAISYCLAW 🌼" : "DAISYCLAW";
  return [...DAISYCLAW_ASCII_BODY, centerText(title, width), " "];
}

/** Format the large decorative DaisyClaw banner art. */
export function formatCliBannerArt(options: BannerOptions = {}): string {
  ...
  const colored = lines.map((line) => {
    if (line.includes("DAISYCLAW")) {
      if (!icon) {
        return theme.info(centerText("DAISYCLAW", visibleWidth(line)));
      }
      return (
        theme.muted("              ") +
        theme.accent(icon) +
        theme.info(" DAISYCLAW ") +
        theme.accent(icon)
      );
    }
