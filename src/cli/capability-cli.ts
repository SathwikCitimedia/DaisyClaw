...
import { detectMime, extensionForMime, normalizeMimeType } from "@daisyclaw/media-core/mime";
import {
...
const LOCAL_MODEL_RUN_SYSTEM_PROMPT = "You are a personal assistant running inside DaisyClaw.";
...
function getAuthProfileIdsForProvider(cfg: DaisyClawConfig, providerId: string): string[] {
...
function pinRuntimeConfigSnapshot(config: DaisyClawConfig): void {
...
export function registerCapabilityCli(program: Command) {
  ...
  const capability = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/infer", "docs.daisyclaw.ai/cli/infer")}\n`,
    );
