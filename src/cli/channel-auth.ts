...
import { getRuntimeConfig, readConfigFileSnapshot, type DaisyClawConfig } from "../config/config.js";
...
function isConfiguredAuthPlugin(plugin: ChannelPlugin, cfg: DaisyClawConfig): boolean {
  const key = plugin.id;
...
function resolveConfiguredAuthChannelInput(cfg: DaisyClawConfig, mode: ChannelAuthMode): string {
  ...
  if (configured.length === 0) {
    throw new Error(
      `No configured channel supports ${mode}. Run ${formatCliCommand("daisyclaw channels status")} to inspect channels or ${formatCliCommand("daisyclaw channels add --channel <channel>")} to add one.`,
    );
  }
...
async function resolveChannelPluginForMode(
  ...
  if (!channelId) {
    throw new Error(
      `Unsupported channel "${channelInput}". Run ${formatCliCommand("daisyclaw channels list")} to see available channels.`,
    );
  }
  ...
  if (!plugin || !supportsChannelAuthMode(plugin, mode)) {
    throw new Error(
      formatUnsupportedChannelActionMessage({
        channel: channelId,
        action: mode,
        inspectCommand: "daisyclaw channels status --channel " + channelId,
      }),
    );
  }
  return {
  ...
  if (!login) {
    throw new Error(
      formatUnsupportedChannelActionMessage({
        ...
        action: "login",
        inspectCommand: "daisyclaw channels status --channel " + channelInput,
      }),
    );
  }
