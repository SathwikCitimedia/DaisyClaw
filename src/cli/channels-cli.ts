...
export async function registerChannelsCli(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["daisyclaw channels list", "List configured channels."],
          ["daisyclaw channels list --all", "Show configured, bundled, and installable channels."],
          ["daisyclaw channels add", "Open guided channel setup."],
          ["daisyclaw channels status --probe", "Run channel status checks and probes."],
          [
            "daisyclaw channels add --channel telegram --token <token>",
            "Add or update a channel account non-interactively.",
          ],
          ["daisyclaw channels login --channel whatsapp", "Link a WhatsApp Web account."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink(
          "/cli/channels",
          "docs.daisyclaw.ai/cli/channels",
        )}\n`,
    );
  ...
  const addCommand = channels
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["daisyclaw channels add", "Open guided setup for available chat channels."],
          [
            "daisyclaw channels add --channel telegram --token <token>",
            "Add or update Telegram non-interactively.",
          ],
          ["daisyclaw channels list --all", "Find channel ids before using --channel."],
        ])}\n`,
    )
    .option("--channel <name>", `Channel (${channelNames})`)
