...
const DEFAULT_CLI_NAME = "daisyclaw";

const KNOWN_CLI_NAMES = new Set([DEFAULT_CLI_NAME]);
const CLI_PREFIX_RE = /^(?:((?:pnpm|npm|bunx|npx)\s+))?(daisyclaw)\b/;

/** Resolve the displayed CLI binary name from argv, falling back to `daisyclaw`. */
...
/** Replace a leading `daisyclaw` command prefix with the active CLI name. */
