...
describe("shouldSkipRespawnForArgv", () => {
  it.each([
    ...
    { argv: ["node", "daisyclaw", "terminal"] },
    { argv: ["node", "daisyclaw", "chat"] },
    { argv: ["node", "daisyclaw", "gateway"] },
    { argv: ["node", "daisyclaw", "gateway", "--port", "14720", "--bind", "loopback"] },
    { argv: ["node", "daisyclaw", "gateway", "run", "--port=14720", "--bind", "loopback"] },
    {
      argv: ["node", "daisyclaw", "--profile", "server", "gateway", "run", "--allow-unconfigured"],
    },
  ...
  it.each([
    ...
    { argv: ["node", "daisyclaw", "gateway", "status"] },
    { argv: ["node", "daisyclaw", "gateway", "call", "health"] },
...
describe("shouldSkipStartupEnvironmentRespawnForArgv", () => {
  it.each([
    { argv: ["node", "daisyclaw", "--help"] },
    { argv: ["node", "daisyclaw", "gateway"] },
    { argv: ["node", "daisyclaw", "gateway", "run", "--port=14720"] },
...
describe("dns cli", () => {
  it("prints setup info (no apply)", async () => {
    ...
    try {
      ...
      await program.parseAsync(["dns", "setup", "--domain", "daisyclaw.internal"], { from: "user" });
      const output = log.mock.calls.map((call) => call.join(" ")).join("\\n");
      expect(output).toContain("DNS setup");
      expect(output).toContain("daisyclaw.internal");
    } finally {
      log.mockRestore();
    }
  });
