...
describe("completion-cli", () => {
  it("generates zsh functions for nested subcommands", () => {
    ...
    expect(script).toContain("_daisyclaw_gateway()");
    expect(script).toContain("(status) _daisyclaw_gateway_status ;;");
    expect(script).toContain("(restart) _daisyclaw_gateway_restart ;;");
  ...
  it("defers zsh registration until compinit is available", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-zsh-completion-"));
    try {
      const scriptPath = path.join(tempDir, "daisyclaw.zsh");
      ...
      const result = spawnSync(
        ...
        [
          ...
          `
            ...
            [[ -z "\${_comps[daisyclaw]-}" ]] || exit 10
            [[ "\${precmd_functions[(r)_daisyclaw_register_completion]}" = "_daisyclaw_register_completion" ]] || exit 11
            autoload -Uz compinit
            compinit -C
            _daisyclaw_register_completion
            [[ -z "\${precmd_functions[(r)_daisyclaw_register_completion]}" ]] || exit 12
            [[ "\${_comps[daisyclaw]-}" = "_daisyclaw_root_completion" ]]
          `,
        ],
        {
          encoding: "utf8",
  ...
  it("generates PowerShell command paths without the executable prefix", () => {
    ...
    expect(script).not.toContain("if ($commandPath -eq 'daisyclaw gateway') {");
  ...
  it("generates valid PowerShell root arrays when commands or options are empty", () => {
    const commandsOnly = new Command().name("daisyclaw");
    commandsOnly.command("status");
    const optionsOnly = new Command().name("daisyclaw").option("--json", "JSON output");
    const empty = new Command().name("daisyclaw");
  ...
  it("generates fish completions for root and nested command contexts", () => {
    ...
    expect(script).toContain(
      'complete -c daisyclaw -n "__fish_use_subcommand" -a "gateway" -d \'Gateway commands\'',
    );
    expect(script).toContain(
      'complete -c daisyclaw -n "__daisyclaw_command_path_matches gateway -- -t --token" -a "status" -d \'Show gateway status\'',
    );
    expect(script).toContain(
      "complete -c daisyclaw -n \"__daisyclaw_command_path_matches gateway -- -t --token\" -l force -d 'Force the action'",
    );
    expect(script).toContain(
      "complete -c daisyclaw -n \"__daisyclaw_command_path_matches gateway status -- -t --token\" -l json -d 'JSON output'",
    );
    expect(script).toContain("__daisyclaw_command_path_matches gateway -- -t --token");
  ...
  it("scopes fish value-taking option skips to the active command path", () => {
    ...
    expect(script).toContain("__daisyclaw_command_path_matches agent -- --verbose");
    expect(script).toContain("__daisyclaw_command_path_matches sessions cleanup --");
    expect(script).not.toContain("__daisyclaw_command_path_matches sessions cleanup -- --verbose");
    expect(script).toContain(
      "complete -c daisyclaw -n \"__daisyclaw_command_path_matches sessions cleanup --\" -l dry-run -d 'Preview cleanup'",
    );
  });
