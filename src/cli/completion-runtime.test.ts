...
describe("completion-runtime", () => {
  ...
  it("formats PowerShell source and reload commands with single-quoted paths", () => {
    expect(
      formatCompletionSourceLine("powershell", "daisyclaw", "C:\\Users\\Ada\\open'claw.ps1"),
  ...
  it("installs PowerShell completion into the concrete profile path", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-completion-home-"));
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-completion-state-"));

    process.env.HOME = homeDir;
    process.env.DAISYCLAW_STATE_DIR = stateDir;

    try {
      const cachePath = resolveCompletionCachePath("powershell", "daisyclaw");
      await fs.mkdir(path.dirname(cachePath), { recursive: true });
      await fs.writeFile(cachePath, "# powershell completion\n", "utf-8");

      await installCompletion("powershell", true, "daisyclaw");

      const profilePath = resolveCompletionProfilePath("powershell");
      const profile = await fs.readFile(profilePath, "utf-8");
      expect(profile).toBe(`# DaisyClaw Completion\n. '${cachePath}'\n`);
    } finally {
  ...
  it("rejects install when the completion cache is missing", async () => {
    ...
    try {
      await expect(installCompletion("zsh", true, "daisyclaw")).rejects.toThrow(
