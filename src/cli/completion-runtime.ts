...
export const COMPLETION_SKIP_PLUGIN_COMMANDS_ENV = "DAISYCLAW_COMPLETION_SKIP_PLUGIN_COMMANDS";
...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# DaisyClaw Completion";
}
...
function updateCompletionProfile(
  ...
  const block = `# DaisyClaw Completion\n${sourceLine}`;
  const next = trimmed ? `${trimmed}\n\n${block}\n` : `${block}\n`;
  return { next, changed: next !== content, hadExisting };
}

/** Resolves the shell startup profile path that should contain the DaisyClaw completion block. */
...
/** Returns whether a shell profile already contains an DaisyClaw completion block or source line. */
 ...
 * Returns true if profile has `source <(daisyclaw completion ...)` instead of cached file.
 */
...
export async function installCompletion(shell: string, yes: boolean, binName = "daisyclaw") {
