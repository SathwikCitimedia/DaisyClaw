...
import type { ConfigFileSnapshot, DaisyClawConfig } from "../config/types.js";
...
/**
 * Test for issue #6070:
 * `daisyclaw config set/unset` must update snapshot.resolved (user config after $include/${ENV},
...
function buildSnapshot(params: {
  resolved: DaisyClawConfig;
  config: DaisyClawConfig;
}): ConfigFileSnapshot {
  return {
    path: "/tmp/daisyclaw.json",
    exists: true,
...
function setSnapshot(resolved: DaisyClawConfig, config: DaisyClawConfig) {
...
function withRuntimeDefaults(resolved: DaisyClawConfig): DaisyClawConfig {
  return {
    ...resolved,
    agents: {
      ...resolved.agents,
...
function createPluginManifestRecord(
  ...
  return {
    ...
    cliBackends: [],
    hooks: [],
    manifestPath: `/tmp/${overrides.id}/daisyclaw.plugin.json`,
    origin: "bundled",
    providers: [],
...
function makeInvalidSnapshot(params: {
  ...
  return {
    path: params.path ?? "/tmp/custom-daisyclaw.json",
    exists: true,
    raw: "{}",
    parsed: {},
    sourceConfig: {},
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      const resolved: DaisyClawConfig = {
        agents: {
      ...
      const runtimeMerged: DaisyClawConfig = {
    ...
    it("rejects plugin install record config updates", async () => {
      await expect(
        runConfigCommand([
          "config",
          "set",
          'plugins.installs["daisyclaw-web-search"].spec',
          '"@ollama/daisyclaw-web-search@0.2.2"',
          "--strict-json",
          "--dry-run",
        ]),
      ...
      expectErrorIncludes("daisyclaw plugins install <spec>");
      expectErrorIncludes("daisyclaw plugins update <plugin-id>");
    });
  ...
  describe("config get", () => {
    it("redacts sensitive values", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("__DAISYCLAW_REDACTED__");
    });
  ...
  describe("config validate", () => {
    ...
    it("returns machine-readable JSON with --json for invalid config", async () => {
      ...
      expect(payload.path).toBe("/tmp/custom-daisyclaw.json");
  ...
  describe("config set parsing flags", () => {
    it("falls back to raw string when parsing fails and strict mode is off", async () => {
      const resolved: DaisyClawConfig = { gateway: { port: 18789 } };
      setSnapshot(resolved, resolved);
  ...
  describe("config set builders and dry-run", () => {
    ...
    it("dry-runs config patch channel fields against plugin-owned schemas", async () => {
      ...
      setSnapshot(resolved, resolved);
      const pathname = writeTempJson5File("daisyclaw-config-plugin-channel-schema", {
        channels: {
          feishu: {
            appId: "app-id",
            appSecret: "secret",
    ...
    it("rejects --allow-exec without --dry-run", async () => {
      const nonexistentBatchPath = path.join(
        os.tmpdir(),
        `daisyclaw-config-batch-nonexistent-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
      await expect(
    ...
    it("supports batch-file mode", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `daisyclaw-config-batch-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
    ...
    it("batch-file nested leaf updates preserve agents defaults and list siblings", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `daisyclaw-config-memory-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
      fs.writeFileSync(
        pathname,
        JSON.stringify([
    ...
    it("rejects malformed batch-file payloads", async () => {
      const pathname = path.join(
        os.tmpdir(),
        `daisyclaw-config-batch-invalid-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
      fs.writeFileSync(pathname, '{"path":"gateway.auth.mode","value":"token"}', "utf8");
      try {
    ...
    it("patches config from one object in one write", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `daisyclaw-config-patch-${Date.now()}-${Math.random().toString(16).slice(2)}.json5`,
      );
      fs.writeFileSync(
        pathname,
        JSON.stringify({
    ...
    it("preserves empty object values in config patch", async () => {
      ...
      const pathname = writeTempJson5File("daisyclaw-config-patch-empty-object", {
        agents: {
          defaults: {
            models: {
              "openai/gpt-5.5": {},
    ...
    it("treats empty object config patches as recursive merges", async () => {
      ...
      const pathname = writeTempJson5File("daisyclaw-config-patch-empty-merge", {
        channels: {
          slack: {},
        },
      });
    ...
    it("keeps numeric config patch object keys as object keys", async () => {
      ...
      const pathname = writeTempJson5File("daisyclaw-config-patch-numeric-object-key", {
        channels: {
          discord: {
            guilds: {
              "123456789012345678": {
    ...
    it("dry-runs config patch and resolves changed SecretRefs", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `daisyclaw-config-patch-dry-${Date.now()}-${Math.random().toString(16).slice(2)}.json5`,
      );
      fs.writeFileSync(
        pathname,
        JSON.stringify({
    ...
    it("dry-runs pluginIntegration provider patches against manifest integration metadata", async () => {
      ...
      const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-config-plugin-provider-"));
      try {
          ...
          createPluginMetadataSnapshot({
            ...
            plugins: [
              createPluginManifestRecord({
                ...
                manifestPath: path.join(rootDir, "daisyclaw.plugin.json"),
        ...
        setSnapshot(resolved, resolved);
        const validPatch = writeTempJson5File("daisyclaw-config-plugin-provider-valid", {
          secrets: {
            providers: {
              team: {
                source: "exec",
        ...
        setSnapshot(resolved, resolved);
        const invalidPatch = writeTempJson5File("daisyclaw-config-plugin-provider-invalid", {
          secrets: {
            providers: {
              team: {
                source: "exec",
    ...
    it("does not revalidate untouched pluginIntegration providers when disabling plugins", async () => {
      ...
      const patch = writeTempJson5File("daisyclaw-config-plugin-disable", {
        plugins: {
          entries: {
    ...
    it("validates pluginIntegration providers referenced by newly assigned SecretRefs", async () => {
      ...
      const patch = writeTempJson5File("daisyclaw-config-plugin-provider-ref", {
        gateway: {
          auth: {
    ...
    it("schema-validates SecretRef-only config patch operations", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `daisyclaw-config-patch-ref-schema-${Date.now()}-${Math.random()
          .toString(16)
          .slice(2)}.json5`,
      );
      fs.writeFileSync(
    ...
    it("dry-runs nested SecretRefs inside config patch replacements", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `daisyclaw-config-patch-nested-ref-${Date.now()}-${Math.random()
          .toString(16)
          .slice(2)}.json5`,
      );
      fs.writeFileSync(
    ...
    it("supports replace-path and null deletes in config patch", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `daisyclaw-config-patch-replace-${Date.now()}-${Math.random().toString(16).slice(2)}.json5`,
      );
      fs.writeFileSync(
        pathname,
        JSON.stringify({
    ...
    it("rejects unused config patch replace paths", async () => {
      const pathname = path.join(
        os.tmpdir(),
        `daisyclaw-config-patch-unused-replace-${Date.now()}-${Math.random()
          .toString(16)
          .slice(2)}.json5`,
      );
      fs.writeFileSync(
  ...
  describe("path hardening", () => {
    ...
    it("rejects impractical array indexes for config set", async () => {
      const resolved = { agents: { list: [] } } as unknown as DaisyClawConfig;
      setSnapshot(resolved, resolved);
    ...
    it("rejects signed array indexes for config set", async () => {
      const resolved = { agents: { list: [{ id: "main" }] } } as unknown as DaisyClawConfig;
      setSnapshot(resolved, resolved);
  ...
  describe("config unset - issue #6070", () => {
    ...
    it("dry-runs an unset without writing the config file", async () => {
      ...
      expect(mockWriteConfigFile).not.toHaveBeenCalled();
      expectLogIncludes("Dry run successful: 1 update(s) validated against /tmp/daisyclaw.json.");
  ...
  describe("config file", () => {
    it("prints the active config file path", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("/tmp/daisyclaw.json");
      expect(mockWriteConfigFile).not.toHaveBeenCalled();
    });
    ...
    it("handles config file path with home directory", async () => {
      ...
      snapshot.path = "/home/user/.daisyclaw/daisyclaw.json";
      mockReadConfigFileSnapshot.mockResolvedValueOnce(snapshot);

      await runConfigCommand(["config", "file"]);

      expect(mockLog).toHaveBeenCalledWith("/home/user/.daisyclaw/daisyclaw.json");
    });
  });
});
