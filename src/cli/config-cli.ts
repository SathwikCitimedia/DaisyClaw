...
import { isRecord as isPlainRecord } from "@daisyclaw/normalization-core/record-coerce";
...
function normalizeModelProviderRefsForConfigMutation(
  providers: NonNullable<DaisyClawConfig["models"]>["providers"] | undefined,
): unknown {
...
function normalizeConfigMutationModelRefs(cfg: DaisyClawConfig): DaisyClawConfig {
...
const CONFIG_SET_EXAMPLE_PROVIDER = formatCliCommand(
  "daisyclaw config set secrets.providers.vault --provider-source file --provider-path /etc/daisyclaw/secrets.json --provider-mode json",
);
...
const CONFIG_PATCH_EXAMPLE_STDIN = formatCliCommand("daisyclaw config patch --stdin");
...
function formatDoctorHint(message: string): string {
  return `Run \`${formatCliCommand("daisyclaw doctor --fix")}\` ${message}`;
}
...
async function loadValidConfig(runtime: RuntimeEnv = defaultRuntime) {
  ...
  runtime.error(`DaisyClaw config is invalid: ${shortenHomePath(snapshot.path)}`);
...
function formatPluginInstallConfigSetError(): string {
  return [
    ...
    `  ${formatCliCommand("daisyclaw plugins install <spec>")}`,
    `  ${formatCliCommand("daisyclaw plugins update <plugin-id>")}`,
    `  ${formatCliCommand("daisyclaw plugins uninstall <plugin-id>")}`,
  ].join("\n");
}
...
function formatAutoManagedMetaError(paths: readonly PathSegment[][]): string {
  ...
  return [
    `${subject} is auto-managed by DaisyClaw and cannot be edited; the value would be overwritten on the next config write.`,
    "",
    "These fields are stamped on every config write to record the DaisyClaw version and timestamp that produced the file.",
  ].join("\n");
}
...
function collectDryRunSchemaErrors(params: { config: DaisyClawConfig }): ConfigSetDryRunError[] {
...
async function runConfigOperations(params: {
  ...
  const nextConfig = normalizeConfigMutationModelRefs(next as DaisyClawConfig);
...
export async function runConfigGet(opts: { path: string; json?: boolean; runtime?: RuntimeEnv }) {
  ...
  try {
    ...
    if (!res.found) {
      runtime.error(
        danger(
          `Config path not found: ${opts.path}. Run ${formatCliCommand("daisyclaw config validate")} to inspect config shape.`,
        ),
      );
      runtime.exit(1);
      return;
...
export async function runConfigUnset(opts: {
  ...
  try {
    ...
    if (!unsetResult.removed) {
      ...
      runtime.error(
        danger(
          `Config path not found: ${opts.path}. Nothing was changed. Run ${formatCliCommand("daisyclaw config get <path>")} first if you are unsure of the path.`,
        ),
      );
      runtime.exit(1);
      return;
...
export async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  const runtime = opts.runtime ?? defaultRuntime;
  let outputPath = CONFIG_PATH ?? "daisyclaw.json";
  ...
  try {
    ...
    if (!snapshot.exists) {
      if (opts.json) {
        ...
        runtime.error(
          `Create one with ${formatCliCommand("daisyclaw onboard")} or run ${formatCliCommand("daisyclaw doctor --fix")}.`,
        );
      }
      runtime.exit(1);
      return;
    ...
    if (!snapshot.valid) {
      ...
      if (opts.json) {
        ...
        runtime.error(danger(`DaisyClaw config is invalid: ${shortPath}`));
        ...
        runtime.error(`Inspect with ${formatCliCommand("daisyclaw config validate")}.`);
      }
      runtime.exit(1);
      return;
    }
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.daisyclaw.ai/cli/config")}\n`,
    )
    .option(
      "--section <section>",
  ...
  setCommandJsonMode(cmd.command("set"), "parse-only")
    ...
    .option(
      "--dry-run",
      "Validate changes without writing daisyclaw.json (checks run in builder/json/batch modes; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    ...
    .option(
      "--dry-run",
      "Validate changes without writing daisyclaw.json (checks schema and SecretRef resolvability; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    .command("schema")
    .description("Print the JSON schema for daisyclaw.json")
    .action(async () => {
