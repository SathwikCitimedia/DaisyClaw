...
const buildGatewayInstallPlan = vi.fn(
  async (params: {
    ...
    environment: {
      DAISYCLAW_GATEWAY_PORT: String(params.port),
      ...(params.wrapperPath ? { DAISYCLAW_WRAPPER: params.wrapperPath } : {}),
      ...(params.token ? { DAISYCLAW_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-daemon-cli-"));
    ...
    process.env.DAISYCLAW_STATE_DIR = tmpDir;
    process.env.DAISYCLAW_CONFIG_PATH = path.join(tmpDir, "daisyclaw.json");
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        ...
        DAISYCLAW_STATE_DIR: "/tmp/daisyclaw-daemon-state",
        DAISYCLAW_CONFIG_PATH: "/tmp/daisyclaw-daemon-state/daisyclaw.json",
        DAISYCLAW_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.daisyclaw.gateway.plist",
    });
  ...
  it("passes the existing service environment into the install plan on forced reinstall", async () => {
    ...
    expect((installPlanParams.env as NodeJS.ProcessEnv).DAISYCLAW_WRAPPER).toBe(
      "/usr/local/bin/daisyclaw-doppler",
    );
  });
