...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("daisyclaw-daemon-install-int-");
    configPath = path.join(tempHome, "daisyclaw.json");
    process.env.HOME = tempHome;
    process.env.DAISYCLAW_STATE_DIR = tempHome;
  ...
  it("refuses service install when config was written by a newer DaisyClaw", async () => {
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.DAISYCLAW_GATEWAY_TOKEN).toBeUndefined();
  });
});
