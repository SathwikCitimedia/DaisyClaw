...
const createInstallPlanFixture = vi.hoisted(() => {
  return async (params?: {
    ...
    if (params?.wrapperPath || params?.env?.DAISYCLAW_WRAPPER) {
      environment.DAISYCLAW_WRAPPER = params.wrapperPath ?? params.env?.DAISYCLAW_WRAPPER;
    }
    return {
...
vi.mock("../../config/io.js", () => ({
  ...
  readConfigFileSnapshotForWrite: vi.fn(async () => ({
    ...
    writeOptions: { expectedConfigPath: "/tmp/daisyclaw.json" },
  })),
}));
...
vi.mock("../../daemon/program-args.js", () => ({
  ...
  resolveDaisyClawWrapperPath: async (value: string | undefined) => value?.trim() || undefined,
}));
...
function mockResolvedGatewayTokenSecretRef() {
  resolveSecretInputRefMock.mockReturnValue({
    ref: { source: "env", provider: "default", id: "DAISYCLAW_GATEWAY_TOKEN" },
  });
  resolveSecretRefValuesMock.mockResolvedValue(
    new Map([["env:default:DAISYCLAW_GATEWAY_TOKEN", "resolved-from-secretref"]]),
  );
}
...
describe("runDaemonInstall", () => {
  ...
  it("does not treat env-template gateway.auth.token as plaintext during install", async () => {
    loadConfigMock.mockReturnValue({
      gateway: { auth: { mode: "token", token: "${DAISYCLAW_GATEWAY_TOKEN}" } },
    });
    mockResolvedGatewayTokenSecretRef();
  ...
  it("reinstalls when the loaded service still embeds DAISYCLAW_GATEWAY_TOKEN", async () => {
    service.isLoaded.mockResolvedValue(true);
    service.readCommand.mockResolvedValue({
      programArguments: ["daisyclaw", "gateway", "run"],
      environment: {
        DAISYCLAW_GATEWAY_TOKEN: "stale-service-token",
      },
    } as never);
    ...
    expect(installDaemonServiceAndEmitMock).toHaveBeenCalledTimes(1);
    expect(actionState.warnings).toContain(
      "Gateway service DAISYCLAW_GATEWAY_TOKEN differs from the current install plan; refreshing the install.",
    );
  });
  ...
  it("returns already-installed when the embedded gateway token matches the install plan", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["daisyclaw", "gateway", "run"],
      environment: {
        DAISYCLAW_GATEWAY_TOKEN: "durable-token",
      },
    } as never);
    buildGatewayInstallPlanMock.mockResolvedValueOnce({
      programArguments: ["daisyclaw", "gateway", "run"],
  ...
  it("preserves wrapper env from an installed but unloaded service during forced reinstall", async () => {
    service.isLoaded.mockResolvedValue(false);
    service.readCommand.mockResolvedValue({
      programArguments: ["/usr/local/bin/daisyclaw-doppler", "gateway", "run"],
      environment: {
    ...
    expectFields(installPlanArg, { wrapperPath: "/usr/local/bin/daisyclaw-doppler" });
  ...
  it("reinstalls when wrapper command matches but wrapper env is missing", async () => {
    ...
    await runDaemonInstall({
      json: true,
      wrapper: "/usr/local/bin/daisyclaw-doppler",
    });

    expect(installDaemonServiceAndEmitMock).toHaveBeenCalledTimes(1);
    expect(actionState.warnings).toContain(
      "Gateway service DAISYCLAW_WRAPPER differs from the current wrapper install plan; refreshing the install.",
    );
  });
  ...
  it("does not reinstall when DAISYCLAW_GATEWAY_TOKEN comes from an env file", async () => {
    service.isLoaded.mockResolvedValue(true);
    service.readCommand.mockResolvedValue({
      programArguments: ["daisyclaw", "gateway", "run"],
      environment: {
  ...
  it("does not reuse stale service control env during forced reinstall", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["daisyclaw", "gateway", "run"],
      environment: {
        DAISYCLAW_STATE_DIR: "/tmp/daisyclaw-doctor-manual",
        DAISYCLAW_CONFIG_PATH: "/tmp/daisyclaw-doctor-manual/daisyclaw.json",
        DAISYCLAW_GATEWAY_TOKEN: "stale-service-token",
    ...
    try {
      ...
      expect(env.DAISYCLAW_STATE_DIR).toBeUndefined();
      expect(env.DAISYCLAW_CONFIG_PATH).toBeUndefined();
