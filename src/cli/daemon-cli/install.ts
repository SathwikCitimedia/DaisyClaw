...
import { DAISYCLAW_WRAPPER_ENV_KEY, resolveDaisyClawWrapperPath } from "../../daemon/program-args.js";
...
export function mergeInstallInvocationEnv(params: {
  ...
  for (const [rawKey, rawValue] of Object.entries(params.existingServiceEnv)) {
    ...
    if (upper === DAISYCLAW_WRAPPER_ENV_KEY) {
      const value = rawValue.trim();
      if (value) {
        preservedServiceEnv[DAISYCLAW_WRAPPER_ENV_KEY] = value;
      }
      continue;
    }
    if (
...
export async function runDaemonInstall(opts: DaemonInstallOptions) {
  ...
  if (opts.wrapper !== undefined) {
    try {
      wrapperPath = await resolveDaisyClawWrapperPath(opts.wrapper);
      if (!wrapperPath) {
  ...
  if (!wrapperPath) {
    try {
      wrapperPath = await resolveDaisyClawWrapperPath(installEnv[DAISYCLAW_WRAPPER_ENV_KEY]);
    } catch (err) {
      fail(`Invalid ${DAISYCLAW_WRAPPER_ENV_KEY}: ${String(err)}`);
      return;
    }
  }
  if (loaded) {
    if (!opts.force) {
      ...
      if (autoRefreshMessage) {
        ...
        if (!json) {
          ...
          defaultRuntime.log(
            `Reinstall with: ${formatCliCommand("daisyclaw gateway install --force")}`,
          );
        }
        return;
      }
  ...
  try {
    ...
    if (currentEmbeddedToken) {
      ...
      const plannedEmbeddedToken = normalizeOptionalString(
        plannedInstall.environment.DAISYCLAW_GATEWAY_TOKEN,
      );
      if (currentEmbeddedToken !== plannedEmbeddedToken) {
        return "Gateway service DAISYCLAW_GATEWAY_TOKEN differs from the current install plan; refreshing the install.";
      }
    }
    const wrapperRequested = Boolean(
      params.wrapperPath || normalizeOptionalString(params.installEnv[DAISYCLAW_WRAPPER_ENV_KEY]),
    );
    if (wrapperRequested) {
      const plannedInstall = await buildGatewayInstallPlan({
        env: params.installEnv,
      ...
      const plannedWrapperPath = normalizeOptionalString(
        plannedInstall.environment[DAISYCLAW_WRAPPER_ENV_KEY],
      );
      const currentWrapperPath = normalizeOptionalString(
        currentCommand.environment?.[DAISYCLAW_WRAPPER_ENV_KEY],
      );
      if (plannedWrapperPath !== currentWrapperPath) {
        return `Gateway service ${DAISYCLAW_WRAPPER_ENV_KEY} differs from the current wrapper install plan; refreshing the install.`;
      }
    }
