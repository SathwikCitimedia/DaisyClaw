...
describe("runDaemonRestart health checks", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["DAISYCLAW_CONTAINER_HINT", "DAISYCLAW_PROFILE"]);
    delete process.env.DAISYCLAW_CONTAINER_HINT;
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["daisyclaw", "gateway", "--port", "18789"],
      environment: {},
    });
  ...
  it("repairs stale loaded service definitions from gateway start", async () => {
    ...
    runServiceStart.mockImplementation(
      async (params: {
        ...
        await params.repairLoadedService?.({
          json: true,
          stdout: process.stdout,
          state: { command: { environment: { DAISYCLAW_SERVICE_VERSION: "2026.4.24" } } },
    ...
    expect(repairParams.state?.command?.environment).toEqual({
      DAISYCLAW_SERVICE_VERSION: "2026.4.24",
    });
  ...
  it("does not fall back to unmanaged restart when launchd repair reports headless GUI bootstrap failure", async () => {
    ...
    recoverInstalledLaunchAgent.mockRejectedValue(
      new Error("LaunchAgent daisyclaw gateway restart requires a logged-in macOS GUI session"),
    );
    findVerifiedGatewayListenerPidsOnPortSync.mockReturnValue([4200]);
  ...
  it("delegates system-scope restart to systemctl without unmanaged signaling when root (daisyclaw#87577)", async () => {
  ...
  it("surfaces systemd sudo guidance and never signals when restarting a system-scope unit as non-root (daisyclaw#87577)", async () => {
    vi.spyOn(process, "platform", "get").mockReturnValue("linux");
    findInstalledSystemdGatewayScope.mockResolvedValue({
      scope: "system",
      unitName: "daisyclaw.service",
    ...
    restartSystemdService.mockRejectedValue(
      new Error(
        "daisyclaw.service is a system-scope unit (/etc/systemd/system/daisyclaw.service); run `sudo systemctl restart daisyclaw.service` to restart it",
      ),
    );
    findVerifiedGatewayListenerPidsOnPortSync.mockReturnValue([4200]);
    mockUnmanagedRestart();
    ...
    await expect(runDaemonRestart({ json: true })).rejects.toThrow(
      /sudo systemctl restart daisyclaw\.service/,
    );
  ...
  it("delegates system-scope stop to systemctl without unmanaged signaling when root (daisyclaw#87577)", async () => {
    vi.spyOn(process, "platform", "get").mockReturnValue("linux");
    findInstalledSystemdGatewayScope.mockResolvedValue({
      scope: "system",
      unitName: "daisyclaw-gateway.service",
  ...
  it("surfaces systemd sudo guidance and never signals when stopping a system-scope unit as non-root (daisyclaw#87577)", async () => {
    vi.spyOn(process, "platform", "get").mockReturnValue("linux");
    findInstalledSystemdGatewayScope.mockResolvedValue({
      scope: "system",
      unitName: "daisyclaw-gateway.service",
    ...
    stopSystemdService.mockRejectedValue(
      new Error(
        "daisyclaw-gateway.service is a system-scope unit (/etc/systemd/system/daisyclaw-gateway.service); run `sudo systemctl stop daisyclaw-gateway.service` to stop it",
      ),
    );
    findVerifiedGatewayListenerPidsOnPortSync.mockReturnValue([4200]);
    runServiceStop.mockImplementation(async (params: { onNotLoaded?: () => Promise<unknown> }) => {
