...
async function confirmGatewayReachable(params: {
  ...
  const token = normalizeOptionalString(params.auth?.token ?? process.env.DAISYCLAW_GATEWAY_TOKEN);
  const password = normalizeOptionalString(
    params.auth?.password ?? process.env.DAISYCLAW_GATEWAY_PASSWORD,
  );
