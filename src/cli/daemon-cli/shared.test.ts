...
describe("renderGatewayServiceStartHints", () => {
  ...
  it("resolves daemon container context from either env key", () => {
    expect(
      resolveDaemonContainerContext({
        DAISYCLAW_CONTAINER: "daisyclaw-demo-container",
      } as NodeJS.ProcessEnv),
    ).toBe("daisyclaw-demo-container");
    expect(
      resolveDaemonContainerContext({
        DAISYCLAW_CONTAINER_HINT: "daisyclaw-demo-container",
      } as NodeJS.ProcessEnv),
    ).toBe("daisyclaw-demo-container");
  });

  it("prepends a single container restart hint when DAISYCLAW_CONTAINER is set", () => {
    expect(
      renderGatewayServiceStartHints({
        DAISYCLAW_CONTAINER: "daisyclaw-demo-container",
      } as NodeJS.ProcessEnv),
  ...
  it("prepends a single container restart hint when DAISYCLAW_CONTAINER_HINT is set", () => {
    expect(
      renderGatewayServiceStartHints({
        DAISYCLAW_CONTAINER_HINT: "daisyclaw-demo-container",
      } as NodeJS.ProcessEnv),
...
describe("filterContainerGenericHints", () => {
  it("drops the generic container foreground hint when DAISYCLAW_CONTAINER is set", () => {
    expect(
      filterContainerGenericHints(
        [
          "systemd user services are unavailable; install/enable systemd or run the gateway under your supervisor.",
          "If you're in a container, run the gateway in the foreground instead of `daisyclaw gateway`.",
        ],
        { DAISYCLAW_CONTAINER: "daisyclaw-demo-container" } as NodeJS.ProcessEnv,
      ),
    ).toStrictEqual([]);
  });

  it("drops the generic container foreground hint when DAISYCLAW_CONTAINER_HINT is set", () => {
    expect(
      filterContainerGenericHints(
        [
          "systemd user services are unavailable; install/enable systemd or run the gateway under your supervisor.",
        ...
        { DAISYCLAW_CONTAINER_HINT: "daisyclaw-demo-container" } as NodeJS.ProcessEnv,
      ),
    ).toStrictEqual([]);
  });
});
