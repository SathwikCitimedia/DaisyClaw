...
export async function repairLoadedGatewayServiceForStart(params: {
  ...
  const wrapperPath = await resolveDaisyClawWrapperPath(installEnv[DAISYCLAW_WRAPPER_ENV_KEY]);
  ...
  return {
    result: "started",
    message:
      "Gateway service definition repaired and started. Reopen the Control UI with `daisyclaw dashboard` or copy a fresh auth URL with `daisyclaw dashboard --no-open`.",
