...
import type { StaleDaisyClawUpdateLaunchdJob } from "../../daemon/launchd.js";
...
const findStaleDaisyClawUpdateLaunchdJobs = vi.fn<
  (env?: NodeJS.ProcessEnv) => Promise<StaleDaisyClawUpdateLaunchdJob[]>
>(async () => []);
...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    DAISYCLAW_STATE_DIR: "/tmp/daisyclaw-daemon",
    DAISYCLAW_CONFIG_PATH: "/tmp/daisyclaw-daemon/daisyclaw.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.DAISYCLAW_STATE_DIR ?? "/tmp/daisyclaw-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.DAISYCLAW_CONFIG_PATH ?? `${stateDir}/daisyclaw.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({
    ...
    const isDaemon = configPath.includes("/daisyclaw-daemon/");
...
vi.mock("../../daemon/launchd.js", () => ({
  findStaleDaisyClawUpdateLaunchdJobs: (env?: NodeJS.ProcessEnv) =>
    findStaleDaisyClawUpdateLaunchdJobs(env),
}));
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.DAISYCLAW_STATE_DIR = "/tmp/daisyclaw-cli";
    process.env.DAISYCLAW_CONFIG_PATH = "/tmp/daisyclaw-cli/daisyclaw.json";
    delete process.env.DAISYCLAW_GATEWAY_TOKEN;
    delete process.env.DAISYCLAW_GATEWAY_PASSWORD;
    ...
    findStaleDaisyClawUpdateLaunchdJobs.mockReset();
    findStaleDaisyClawUpdateLaunchdJobs.mockResolvedValue([]);
  ...
  it("forwards requireRpc and configPath to the daemon probe", async () => {
    ...
    expect(probeInput.configPath).toBe("/tmp/daisyclaw-daemon/daisyclaw.json");
  });
  ...
  it("reuses the shared CLI config snapshot when the daemon uses the same config path", async () => {
    ...
    expect(readConfigFileSnapshotCalls).toHaveBeenCalledWith("/tmp/daisyclaw-cli/daisyclaw.json");
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.DAISYCLAW_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.DAISYCLAW_GATEWAY_PORT ?? "missing-port",
    }));
    ...
    expect(
      serviceReadRuntime.mock.calls.some(([env]) => env?.DAISYCLAW_GATEWAY_PORT === "19001"),
    ).toBe(true);
  ...
  it("surfaces recent service restart handoffs only during deep status", async () => {
    ...
    expect(handoffInput.DAISYCLAW_STATE_DIR).toBe("/tmp/daisyclaw-daemon");
    expect(handoffInput.DAISYCLAW_CONFIG_PATH).toBe("/tmp/daisyclaw-daemon/daisyclaw.json");
    ...
    async () => {
      serviceReadCommand.mockResolvedValueOnce({
        ...
        environment: {
          DAISYCLAW_STATE_DIR: "/tmp/daisyclaw-daemon",
          DAISYCLAW_CONFIG_PATH: "/tmp/daisyclaw-daemon/daisyclaw.json",
          DAISYCLAW_LAUNCHD_LABEL: "ai.daisyclaw.manual-update.gateway",
        },
      });
      findStaleDaisyClawUpdateLaunchdJobs.mockResolvedValueOnce([
        {
      ...
      const staleScanEnv = findStaleDaisyClawUpdateLaunchdJobs.mock.calls[0]?.[0];
      expect(staleScanEnv?.DAISYCLAW_STATE_DIR).toBe("/tmp/daisyclaw-daemon");
      expect(staleScanEnv?.DAISYCLAW_CONFIG_PATH).toBe("/tmp/daisyclaw-daemon/daisyclaw.json");
      expect(staleScanEnv?.DAISYCLAW_LAUNCHD_LABEL).toBe("ai.daisyclaw.manual-update.gateway");
  ...
  it("does not read restart handoffs during normal status", async () => {
    ...
    expect(findStaleDaisyClawUpdateLaunchdJobs).not.toHaveBeenCalled();
      ...
      connections: [
        {
          ...
          ppid: 1,
          command: "node",
          commandLine: "node /tmp/newer-daisyclaw/dist/index.js logs --follow",
  ...
  it("uses the fast config path for plain same-file status reads", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-status-config-"));
    const configPath = path.join(tmp, "daisyclaw.json");
    await fs.writeFile(
      configPath,
      JSON.stringify({
        gateway: {
    ...
    process.env.DAISYCLAW_STATE_DIR = tmp;
    process.env.DAISYCLAW_CONFIG_PATH = configPath;
    serviceReadCommand.mockResolvedValueOnce({
      programArguments: ["/bin/node", "cli", "gateway", "--port", "19001"],
      environment: {
        DAISYCLAW_STATE_DIR: tmp,
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.DAISYCLAW_GATEWAY_TOKEN = "env-token";
    process.env.DAISYCLAW_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection when probe fails", async () => {
    ...
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "daisyclaw-gateway" }],
        hints: [],
      },
      healthy: false,
  ...
  it("compares plugin drift against the running gateway version from the probe, not the CLI VERSION", async () => {
    ...
    loadInstalledPluginIndexInstallRecords.mockResolvedValueOnce({
      whatsapp: {
        source: "npm",
        resolvedName: "@daisyclaw/whatsapp",
  ...
  it("reads install records from the merged daemon service environment, not the CLI process env", async () => {
    ...
    // The mock daemon service command sets DAISYCLAW_STATE_DIR=/tmp/daisyclaw-daemon,
    // distinct from the CLI process DAISYCLAW_STATE_DIR=/tmp/daisyclaw-cli. Drift
