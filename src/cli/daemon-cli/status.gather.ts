...
type DaemonConfigContext = {
  ...
  cliCfg: DaisyClawConfig;
  daemonCfg: DaisyClawConfig;
...
function resolveSnapshotRuntimeConfig(snapshot: ConfigFileSnapshot | null): DaisyClawConfig | null {
...
function coerceStatusConfig(value: unknown): DaisyClawConfig {
...
export type DaemonStatus = {
  ...
  service: {
    ...
    staleUpdateLaunchdJobs?: StaleDaisyClawUpdateLaunchdJob[];
  };
  config?: {
    cli: ConfigSummary;
   ...
   * can happen after `npm install -g daisyclaw@<v>` updates the gateway binary
   * without a corresponding `daisyclaw plugins update`.
   */
  ...
  const staleUpdateLaunchdJobs =
    opts.deep && process.platform === "darwin"
      ? await loadLaunchdModule()
          .then(({ findStaleDaisyClawUpdateLaunchdJobs }) =>
            findStaleDaisyClawUpdateLaunchdJobs(serviceEnv),
          )
          .catch(() => [])
      : [];
