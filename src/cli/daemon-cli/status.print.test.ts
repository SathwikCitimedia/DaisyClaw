...
vi.mock("../../daemon/restart-logs.js", () => ({
  ...
  resolveGatewaySupervisorLogPaths: () => ({
    logDir: "/Users/test/Library/Logs/daisyclaw",
    stdoutPath: "/Users/test/Library/Logs/daisyclaw/gateway.log",
    stderrPath: "/Users/test/Library/Logs/daisyclaw/gateway.err.log",
  }),
...
describe("printDaemonStatus", () => {
  ...
  it("prints stale gateway pid guidance when runtime does not own the listener", () => {
    ...
    expectMockLineContains(runtime.error, formatCliCommand("daisyclaw gateway restart"));
  });
  ...
  it("prints established gateway client guidance gathered by deep status", () => {
    ...
    expectMockLineContains(runtime.log, "newer-daisyclaw");
  ...
  it("prints stale updater launchd job guidance", () => {
    ...
    expectMockLineContains(runtime.error, "Stale DaisyClaw updater launchd job(s) detected.");
    expectMockLineContains(runtime.error, "ai.daisyclaw.update.2026.5.12");
    expectMockLineContains(runtime.error, "ai.daisyclaw.manual-update.1717168800");
  ...
  it("prints macOS launchd stdout and suppressed stderr when gateway is not listening", () => {
    ...
    expectMockLineContains(runtime.error, "/Users/test/Library/Logs/daisyclaw/gateway.log");
  ...
  it("prints CLI and gateway versions with readable guidance when they differ", () => {
    printDaemonStatus(
      {
        cli: {
          version: "2026.4.23",
          entrypoint: "/usr/local/bin/daisyclaw",
        },
        service: {
          label: "LaunchAgent",
          loaded: true,
    ...
    expectMockLineContains(runtime.log, "CLI version: 2026.4.23 (/usr/local/bin/daisyclaw)");
    expectMockLineContains(runtime.log, "Gateway version: 2026.5.6");
    expectMockLineContains(runtime.error, "this DaisyClaw command is version 2026.4.23");
    expectMockLineContains(
      runtime.error,
      "if this mismatch is unexpected, update PATH so `daisyclaw` points to the version you want",
    );
  });
    ...
    printDaemonStatus(
      {
        ...
        config: {
          cli: {
            path: "/tmp/daisyclaw-cli/daisyclaw.json",
            exists: true,
            valid: true,
          },
          daemon: {
            path: "/tmp/daisyclaw-daemon/daisyclaw.json",
            exists: true,
            valid: true,
    ...
    printDaemonStatus(
      {
        ...
        extraServices: [{ label: "ai.daisyclaw.gateway.rescue", scope: "user", detail: "loaded" }],
      },
      { json: false },
    );

    expectMockLineContains(runtime.log, "Other gateway-like services detected");
    expectMockLineContains(runtime.log, "ai.daisyclaw.gateway.rescue");
  ...
  it("prints a terse plugin drift warning outside deep mode", () => {
    ...
    expectMockLineContains(runtime.log, "daisyclaw gateway status --deep");
  ...
  it("prints detailed plugin drift entries in deep mode", () => {
    ...
    expectMockLineContains(runtime.log, "daisyclaw plugins update <plugin-id>");
    expectMockLineContains(runtime.log, "daisyclaw gateway restart");
  });
