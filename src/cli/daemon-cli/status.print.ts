...
export function printDaemonStatus(status: DaemonStatus, opts: { json: boolean; deep?: boolean }) {
  ...
  if (service.configAudit?.issues.length) {
    ...
    defaultRuntime.error(
      warnText(
        `Recommendation: run "${formatCliCommand("daisyclaw doctor")}" (or "${formatCliCommand("daisyclaw doctor --repair")}").`,
      ),
    );
  }
  ...
  if (status.config) {
    ...
    if (status.config.mismatch) {
      ...
      defaultRuntime.error(
        errorText(
          `Fix: rerun \`${formatCliCommand("daisyclaw gateway install --force")}\` from the same --profile / DAISYCLAW_STATE_DIR you expect.`,
        ),
      );
    }
    spacer();
  ...
  if (gatewayVersion) {
    ...
    if (status.cli?.version && status.cli.version !== gatewayVersion) {
      defaultRuntime.error(
        warnText(
          `Warning: this DaisyClaw command is version ${status.cli.version}, but the running Gateway is version ${gatewayVersion}.`,
        ),
      );
      defaultRuntime.error(
        warnText(
          "Check `daisyclaw --version`, `which daisyclaw`, and `daisyclaw gateway status --deep`; if this mismatch is unexpected, update PATH so `daisyclaw` points to the version you want, or reinstall the Gateway service from that same DaisyClaw install.",
        ),
      );
    }
    spacer();
  ...
  if (
    ...
    defaultRuntime.error(
      errorText(
        `Fix: run ${formatCliCommand("daisyclaw gateway restart")} and re-check with ${formatCliCommand("daisyclaw gateway status --deep")}.`,
      ),
    );
    spacer();
  }
  ...
  if (status.connections?.established.length) {
    ...
    defaultRuntime.log(
      warnText(
        "If logs show protocol mismatch after rollback, stop stale DaisyClaw client processes listed here and re-run gateway status.",
      ),
    );
    spacer();
  }
  ...
  if (service.runtime?.cachedLabel) {
    ...
    const labelValue = resolveGatewayLaunchAgentLabel(env.DAISYCLAW_PROFILE);
    defaultRuntime.error(
      errorText(
    ...
    defaultRuntime.error(
      errorText(`Then reinstall: ${formatCliCommand("daisyclaw gateway install")}`),
    );
    spacer();
  }

  if (service.staleUpdateLaunchdJobs?.length) {
    defaultRuntime.error(errorText("Stale DaisyClaw updater launchd job(s) detected."));
    ...
    defaultRuntime.error(
      errorText(
        `Fix after confirming no update is running: launchctl remove <label>, then run ${formatCliCommand("daisyclaw gateway restart")}.`,
      ),
    );
    spacer();
  }
  ...
  if (
    ...
    if (process.platform === "linux") {
      const unit = resolveGatewaySystemdServiceName(serviceEnv.DAISYCLAW_PROFILE);
      defaultRuntime.error(
  ...
  if (drift && drift.drifts.length > 0) {
    ...
    if (opts.deep) {
      ...
      defaultRuntime.log(
        `${label("Fix:")} ${formatCliCommand("daisyclaw plugins update <plugin-id>")} for each drifted plugin, then ${formatCliCommand("daisyclaw gateway restart")}.`,
      );
    } else {
      defaultRuntime.log(
        infoText(
          `Run ${formatCliCommand("daisyclaw gateway status --deep")} for affected plugin ids and fix commands.`,
        ),
      );
    }
    spacer();
  ...
  defaultRuntime.log(`${label("Troubles:")} run ${formatCliCommand("daisyclaw status")}`);
  defaultRuntime.log(`${label("Troubleshooting:")} https://docs.daisyclaw.ai/troubleshooting`);
}
