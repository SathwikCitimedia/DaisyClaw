...
const mocks = vi.hoisted(() => {
  ...
  let configState: DaisyClawConfig = {
    tools: {
      exec: {
        host: "auto",
        security: "allowlist",
  ...
  return {
    ...
    setConfig: (next: DaisyClawConfig) => {
      configState = next;
    },
    ...
    replaceConfigFile: vi.fn(
      async ({ nextConfig }: { nextConfig: DaisyClawConfig; baseHash?: string }) => {
    ...
    readConfigFileSnapshot: vi.fn<
      () => Promise<{ path: string; hash: string; config: DaisyClawConfig }>
    >(async () => ({
      path: "/tmp/daisyclaw.json",
      hash: "config-hash-1",
      config: configState,
...
describe("exec-policy CLI", () => {
  ...
  beforeEach(() => {
    ...
    mocks.mutateConfigFile.mockImplementation(
      async ({ mutate }: { mutate: (draft: DaisyClawConfig) => void }) => {
  ...
  it("sanitizes terminal control content before rendering the text table", async () => {
    ...
    expect(output).toContain("/tmp/daisyclaw.json");
    ...
    expect(output).not.toContain("/tmp/daisyclaw.json\nforged");
