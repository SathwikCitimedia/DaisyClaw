...
>(() => ({ mode: "disabled", detail: "DAISYCLAW_NO_RESPAWN" }));
...
describe("runGatewayLoop", () => {
    ...
    await withIsolatedSignals(async ({ captureSignal }) => {
      ...
      expect(gatewayLog.warn).toHaveBeenNthCalledWith(
        ...
        "An unauthorized SIGUSR1 restart signal was received and ignored. " +
          "If a pending gateway restart needs to be applied, run `daisyclaw gateway restart` " +
  ...
  it("releases the lock before exiting on spawned restart", async () => {
    vi.clearAllMocks();
    peekGatewaySigusr1RestartReason.mockReturnValue(undefined);
    const originalTraceEnv = process.env.DAISYCLAW_GATEWAY_RESTART_TRACE;
    process.env.DAISYCLAW_GATEWAY_RESTART_TRACE = "1";
    ...
    try {
      await withIsolatedSignals(async ({ captureSignal }) => {
        ...
        expect(respawnOpts?.env?.DAISYCLAW_GATEWAY_RESTART_TRACE_STARTED_AT_MS).toMatch(/^\d/u);
        expect(respawnOpts?.env?.DAISYCLAW_GATEWAY_RESTART_TRACE_LAST_AT_MS).toMatch(/^\d/u);
        expect(writeGatewayRestartHandoffSync).not.toHaveBeenCalled();
      });
    } finally {
      if (originalTraceEnv === undefined) {
        delete process.env.DAISYCLAW_GATEWAY_RESTART_TRACE;
      } else {
        process.env.DAISYCLAW_GATEWAY_RESTART_TRACE = originalTraceEnv;
      }
    }
  });
  ...
  it("waits briefly before exiting on launchd supervised restart", async () => {
    ...
    try {
      ...
      vi.useRealTimers();
      delete process.env.DAISYCLAW_LAUNCHD_LABEL;
      if (originalPlatformDescriptor) {
