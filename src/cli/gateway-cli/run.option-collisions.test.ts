...
const writeDiagnosticStabilityBundleForFailureSync = vi.fn((_reason: string, _error: unknown) => ({
  ...
  message: "wrote stability bundle: /tmp/daisyclaw-stability.json",
  path: "/tmp/daisyclaw-stability.json",
}));
const controlUiState = vi.hoisted(() => ({
  root: "/tmp/daisyclaw-control-ui" as string | null,
}));
...
const withoutGatewayAuthEnv = {
  ...
  DAISYCLAW_GATEWAY_PASSWORD: undefined,
};

const { runtimeErrors, defaultRuntime, resetRuntimeCapture } = createCliRuntimeCapture();

vi.mock("../../config/config.js", () => ({
  getConfigPath: () => "/tmp/daisyclaw-test-missing-config.json",
...
vi.mock("../../config/paths.js", () => ({
  CONFIG_PATH: "/tmp/daisyclaw-test-missing-config.json",
...
describe("gateway run option collisions", () => {
  ...
  beforeEach(() => {
    ...
    controlUiState.root = "/tmp/daisyclaw-control-ui";
  ...
  it("blocks service-mode startup from an older binary with newer config", async () => {
    ...
    const previousMarker = process.env.DAISYCLAW_SERVICE_MARKER;
    process.env.DAISYCLAW_SERVICE_MARKER = "gateway";
    try {
      ...
      if (previousMarker === undefined) {
        delete process.env.DAISYCLAW_SERVICE_MARKER;
      } else {
        process.env.DAISYCLAW_SERVICE_MARKER = previousMarker;
      }
    }
  ...
  it.each([
    ...
    delete process.env.DAISYCLAW_CLI_BACKEND_LOG_OUTPUT;
    ...
    expect(process.env.DAISYCLAW_CLI_BACKEND_LOG_OUTPUT).toBe("1");
  });
  ...
  it("blocks startup when the observed snapshot loses gateway.mode", async () => {
    ...
    expect(runtimeErrors).toContain(
      "Gateway start blocked: existing config is missing gateway.mode. Treat this as suspicious or clobbered config. Re-run `daisyclaw onboard --mode local` or `daisyclaw setup`, set gateway.mode=local manually, or pass --allow-unconfigured.",
    );
    expect(runtimeErrors).toContain(
  ...
  it("blocks invalid startup config without automatic recovery", async () => {
    ...
    configState.snapshot = {
      exists: true,
      valid: false,
      path: "/tmp/daisyclaw-test-missing-config.json",
      config: {},
      parsed: null,
