...
function createGatewayCliStartupTrace() {
  const enabled = isTruthyEnvValue(process.env.DAISYCLAW_GATEWAY_STARTUP_TRACE);
...
function warnInlinePasswordFlag() {
  defaultRuntime.error(
    "Warning: --password can be exposed via process listings. Prefer --password-file or DAISYCLAW_GATEWAY_PASSWORD.",
  );
}
...
async function maybeLogPendingControlUiBuild(cfg: DaisyClawConfig): Promise<void> {
...
function getGatewayStartGuardErrors(params: {
  ...
  if (!params.configExists) {
    return [
      `Missing config. Run \`${formatCliCommand("daisyclaw setup")}\` or set gateway.mode=local (or pass --allow-unconfigured).`,
    ];
  }
  if (params.mode === undefined) {
    return [
      [
        ...
        `Re-run \`${formatCliCommand("daisyclaw onboard --mode local")}\` or \`${formatCliCommand("daisyclaw setup")}\`, set gateway.mode=local manually, or pass --allow-unconfigured.`,
      ].join(" "),
...
export async function runGatewayCommand(opts: GatewayRunOpts) {
  ...
  if (process.env.DAISYCLAW_SERVICE_MARKER?.trim()) {
    process.env[GATEWAY_SERVICE_RUNTIME_PID_ENV] = String(process.pid);
  }
  const isDevProfile = normalizeOptionalLowercaseString(process.env.DAISYCLAW_PROFILE) === "dev";
  ...
  if (opts.cliBackendLogs || opts.claudeCliLogs) {
    ...
    process.env.DAISYCLAW_CLI_BACKEND_LOG_OUTPUT = "1";
  }
  ...
  if (opts.rawStream) {
    process.env.DAISYCLAW_RAW_STREAM = "1";
  }
  const rawStreamPath = toOptionString(opts.rawStreamPath);
  if (rawStreamPath) {
    process.env.DAISYCLAW_RAW_STREAM_PATH = rawStreamPath;
  }
  ...
  if (futureStartupBlock && process.env.DAISYCLAW_SERVICE_MARKER?.trim()) {
  ...
  if (opts.force) {
    try {
      ...
      defaultRuntime.error(
        `Could not free port ${port}: ${formatErrorMessage(err)}. Run ${formatCliCommand("daisyclaw gateway status --deep")} to inspect the listener.`,
      );
      defaultRuntime.exit(1);
      return;
    }
  ...
  if (resolvedAuthMode === "password" && !passwordConfigured) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.password (or DAISYCLAW_GATEWAY_PASSWORD), or pass --password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
      ...
      [
        ...
        ...(isContainerEnvironment()
          ? [
              ...
              "Set DAISYCLAW_GATEWAY_TOKEN or DAISYCLAW_GATEWAY_PASSWORD, or pass --token/--password to start with auth.",
            ]
          : [
              "Set gateway.auth.token/password (or DAISYCLAW_GATEWAY_TOKEN/DAISYCLAW_GATEWAY_PASSWORD) or pass --token/--password.",
            ]),
        ...authHints,
      ]
        .filter(Boolean)
  ...
  const deferStartupSidecars =
    isTruthyEnvValue(process.env.DAISYCLAW_SKIP_CHANNELS) ||
    isTruthyEnvValue(process.env.DAISYCLAW_SKIP_PROVIDERS);
  ...
  try {
    ...
    if (isGatewayLockError(err)) {
      ...
      defaultRuntime.error(
        `Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("daisyclaw gateway stop")}`,
      );
      try {
    ...
    defaultRuntime.error(
      `Gateway failed to start: ${formatErrorMessage(err)}. Run ${formatCliCommand("daisyclaw gateway status --deep")} for diagnostics.`,
    );
    defaultRuntime.exit(1);
  }
}
