    ...
    it("uses the active systemd journal for implicit local follow failures", async () => {
      ...
      expect(execFileUtf8Tail).toHaveBeenCalledWith(
        ...
        expect.arrayContaining([
          "--user",
          "--boot",
          "--user-unit=daisyclaw-gateway.service",
          "_PID=2557",
          "--output=cat",
          "--show-cursor",
        ]),
      ...
      expect(stdoutWrites.join("")).toContain(
        "Log source: journalctl --user --boot --user-unit=daisyclaw-gateway.service _PID=2557",
      );
      expect(stdoutWrites.join("")).toContain("Service PID: 2557");
      expect(stdoutWrites.join("")).toContain("Service Unit: daisyclaw-gateway.service");
