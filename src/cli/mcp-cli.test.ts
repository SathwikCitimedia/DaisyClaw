...
const mocks = vi.hoisted(() => {
  ...
  return {
    runtime,
    serveDaisyClawChannelMcp: vi.fn(),
...
const serveDaisyClawChannelMcp = mocks.serveDaisyClawChannelMcp;
...
vi.mock("../mcp/channel-server.js", () => ({
  serveDaisyClawChannelMcp: mocks.serveDaisyClawChannelMcp,
}));
...
async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("mcp cli", () => {
  ...
  it("sets and shows a configured MCP server", async () => {
    await withTempHome("daisyclaw-cli-mcp-home-", async (home) => {
  ...
  it("adds a configured MCP server from flags without replacing operator knobs", async () => {
    await withTempHome("daisyclaw-cli-mcp-home-", async () => {
      const workspaceDir = await createWorkspace();
      vi.spyOn(process, "cwd").mockReturnValue(workspaceDir);
  ...
  it("fails named probes for disabled MCP servers", async () => {
    await withTempHome("daisyclaw-cli-mcp-home-", async (home) => {
      ...
      expect(lastErrorLine()).toBe(
        `MCP server "docs" is disabled in ${configPath}. Run daisyclaw mcp configure docs --enable before probing it.`,
      );
    });
  });
  ...
  it("fails when removing an unknown MCP server", async () => {
    await withTempHome("daisyclaw-cli-mcp-home-", async (home) => {
      ...
      expect(lastErrorLine()).toBe(
        `No MCP server named "missing" in ${configPath}. Run daisyclaw mcp list to see configured servers.`,
      );
    });
  });
  ...
  it("starts the channel bridge with parsed serve options", async () => {
    await withTempHome("daisyclaw-cli-mcp-home-", async () => {
      ...
      expect(serveDaisyClawChannelMcp).toHaveBeenCalledWith({
