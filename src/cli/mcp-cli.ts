...
import { serveDaisyClawChannelMcp } from "../mcp/channel-server.js";
      ...
      if (server.auth === "oauth") {
        ...
        if (!authStatus.hasTokens) {
          issues.push(
            issue(
              "warning",
              `OAuth credentials are not authorized; run ${formatCliCommand(`daisyclaw mcp login ${name}`)}`,
            ),
          );
        }
...
async function probeMcpServerIssue(params: {
  ...
  const runtime = createSessionMcpRuntime({
    sessionId: "daisyclaw-cli-mcp-doctor",
...
async function probeMcpServersOrFail(params: {
  ...
  const runtime = createSessionMcpRuntime({
    sessionId: "daisyclaw-cli-mcp-probe",
    workspaceDir: process.cwd(),
...
export function registerMcpCli(program: Command) {
  const mcp = program.command("mcp").description("Manage DaisyClaw MCP config and channel bridge");

  mcp
    .command("serve")
    .description("Expose DaisyClaw channels over MCP stdio")
    ...
    .action(async (opts) => {
      try {
        ...
        await serveDaisyClawChannelMcp({
        ...
        defaultRuntime.error(
          `MCP server failed to start: ${formatErrorMessage(err)}. Run ${formatCliCommand("daisyclaw mcp list")} to inspect configured servers.`,
        );
        defaultRuntime.exit(1);
      }
    });
  ...
  mcp
    ...
    .action(async (opts: { json?: boolean }) => {
      ...
      if (names.length === 0) {
        defaultRuntime.log(
          `No MCP servers configured in ${loaded.path}. Add one with ${formatCliCommand('daisyclaw mcp set <name> \'{"command":"uvx","args":["context7-mcp"]}\'')}.`,
        );
        return;
      }
  ...
  mcp
    ...
    .action(async (name: string | undefined, opts: { json?: boolean }) => {
      ...
      if (name && !value) {
        fail(
          `No MCP server named "${name}" in ${loaded.path}. Run ${formatCliCommand("daisyclaw mcp list")} to see configured servers.`,
        );
      }
      if (opts.json) {
        printJson(value ?? {});
  ...
  mcp
    ...
    .action(async (name: string | undefined, opts: { json?: boolean }) => {
      ...
      if (name && loaded.mcpServers[name]?.enabled === false) {
        fail(
          `MCP server "${name}" is disabled in ${loaded.path}. Run ${formatCliCommand(`daisyclaw mcp configure ${name} --enable`)} before probing it.`,
        );
      }
      const runtime = createSessionMcpRuntime({
        sessionId: "daisyclaw-cli-mcp-probe",
  ...
  mcp
    ...
    .action(async (name: string | undefined, opts: { probe?: boolean; json?: boolean }) => {
      ...
      if (servers.length === 0) {
        defaultRuntime.log(
          `No MCP servers configured in ${loaded.path}. Add one with ${formatCliCommand("daisyclaw mcp add <name> --command <command>")}.`,
        );
        return;
      }
  ...
  mcp
    ...
    .action(
      async (
        ...
        if (server.auth === "oauth") {
          defaultRuntime.log(
            `Run ${formatCliCommand(`daisyclaw mcp login ${name}`)} to authorize this MCP server.`,
          );
        }
      },
    );
  ...
  mcp
    ...
    .action(async (name: string, opts: { include?: string; exclude?: string; clear?: boolean }) => {
      ...
      if (!result.updated) {
        fail(
          `No MCP server named "${name}" in ${result.path}. Run ${formatCliCommand("daisyclaw mcp list")} to see configured servers.`,
        );
      }
    ...
    .action(async (name: string, opts: { code?: string }) => {
      ...
      const result = await runMcpOAuthLogin({
        ...
        onAuthorizationUrl: (url) => {
          ...
          defaultRuntime.log(
            `After approval, run ${formatCliCommand(`daisyclaw mcp login ${name} --code <code>`)}.`,
          );
        },
      });
