    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "daisyclaw node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["daisyclaw node status", "Check node host service status."],
          ["daisyclaw node install", "Install the node host service."],
          ["daisyclaw node start", "Start the installed node host service."],
          ["daisyclaw node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.daisyclaw.ai/cli/node")}\n`,
    );
