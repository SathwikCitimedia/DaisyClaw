    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["daisyclaw nodes status", "List known nodes with live status."],
          ["daisyclaw nodes pairing pending", "Show pending node pairing requests."],
          ["daisyclaw nodes remove --node <id|name|ip>", "Remove a stale paired node entry."],
          [
            'daisyclaw nodes invoke --node <id> --command system.which --params \'{"name":"uname"}\'',
            "Invoke a node command directly.",
          ],
          ["daisyclaw nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.daisyclaw.ai/cli/nodes")}\n`,
    );
