...
const CLI_STATE_ROOT = "/tmp/daisyclaw";
...
describe("npm-resolution helpers", () => {
  it("keeps the requested selector when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@daisyclaw/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@daisyclaw/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@daisyclaw/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@daisyclaw/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @daisyclaw/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@daisyclaw/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@daisyclaw/plugin-alpha@latest",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@daisyclaw/plugin-alpha@1.2.3",
      installPath: ALPHA_INSTALL_PATH,
      version: "1.2.3",
      resolvedName: "@daisyclaw/plugin-alpha",
      resolvedVersion: "1.2.3",
    ...
    expect(logs).toEqual(["Pinned npm install record to @daisyclaw/plugin-alpha@1.2.3."]);
  ...
  it("keeps install record selector for CLI unless --pin is requested", () => {
    ...
    expect(record.spec).toBe("@daisyclaw/plugin-alpha");
    expect(logs).toEqual([]);
  });
});
