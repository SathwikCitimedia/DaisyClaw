...
function parseChannel(raw: unknown, channels: PairingChannel[]): PairingChannel {
  ...
  if (!value) {
    throw new Error(
      `Missing channel. Use ${formatCliCommand("daisyclaw pairing list --channel <channel>")}.`,
    );
  }
...
export function registerPairingCli(program: Command) {
  ...
  const pairing = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/pairing", "docs.daisyclaw.ai/cli/pairing")}\n`,
    );
  ...
  pairing
    ...
    .action(async (codeOrChannel, code, opts) => {
      ...
      if (!channelRaw || !resolvedCode) {
        throw new Error(
          `Usage: ${formatCliCommand("daisyclaw pairing approve <channel> <code>")} (or: ${formatCliCommand("daisyclaw pairing approve --channel <channel> <code>")})`,
        );
      }
      if (opts.channel && code != null) {
        throw new Error(
          `Too many arguments. Use: ${formatCliCommand("daisyclaw pairing approve --channel <channel> <code>")}`,
        );
      }
      const channel = parseChannel(channelRaw, channels);
      const accountId = normalizeStringifiedOptionalString(opts.account) ?? "";
      ...
      if (!approved) {
        throw new Error(
          `No pending pairing request found for code "${String(resolvedCode)}". Run ${formatCliCommand(`daisyclaw pairing list --channel ${channel}`)} to list pending requests.`,
        );
      }
