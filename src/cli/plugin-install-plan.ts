...
export function resolveBundledInstallPlanBeforeNpm(params: {
  ...
  return {
    bundledSource,
    warning: `Using bundled plugin "${bundledSource.pluginId}" from ${shortenHomePath(bundledSource.localPath)} for npm install spec "${rawSpec}" because this plugin ships with the current DaisyClaw build. To force an external npm override, use npm:${rawSpec}.`,
  };
}
