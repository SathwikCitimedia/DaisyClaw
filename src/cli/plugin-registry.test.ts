...
function expectLoadDaisyClawPluginsCall(
  callIndex: number,
  expected: {
    config?: unknown;
  ...
  const params = mocks.loadDaisyClawPlugins.mock.calls[callIndex]?.[0] as
    | {
        config?: unknown;
        activationSourceConfig?: unknown;
        autoEnabledReasons?: unknown;
...
const mocks = vi.hoisted(() => ({
  loadDaisyClawPlugins: vi.fn<typeof import("../plugins/loader.js").loadDaisyClawPlugins>(),
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  it("uses the resolved runtime load context for configured channel scope", () => {
    ...
    expectLoadDaisyClawPluginsCall(0, {
      config: autoEnabledConfig,
  ...
  it("reloads when escalating from configured-channels to channels", () => {
    ...
    expectLoadDaisyClawPluginsCall(1, {
