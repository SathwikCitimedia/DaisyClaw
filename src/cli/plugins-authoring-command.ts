...
export function buildToolPluginPackageManifest(params: {
  ...
  const daisyclaw =
    params.packageManifest.daisyclaw &&
    typeof params.packageManifest.daisyclaw === "object" &&
    !Array.isArray(params.packageManifest.daisyclaw)
      ? { ...(params.packageManifest.daisyclaw as JsonObject) }
      : {};
  const existingExtensions = Array.isArray(daisyclaw.extensions)
    ? daisyclaw.extensions.filter((entry): entry is string => typeof entry === "string")
    : [];
...
export function validateToolPluginProject(params: {
  ...
  if (JSON.stringify(params.manifest) !== JSON.stringify(expectedManifest)) {
    errors.push("daisyclaw.plugin.json generated metadata is stale. Run daisyclaw plugins build.");
  }
  if (params.manifest.id !== params.metadata.id) {
    errors.push(
      `daisyclaw.plugin.json id (${String(params.manifest.id)}) must match entry id (${params.metadata.id})`,
    );
  }
  if (!params.manifest.configSchema || typeof params.manifest.configSchema !== "object") {
    errors.push("daisyclaw.plugin.json must include object configSchema");
  }
  ...
  if (missing.length > 0) {
    errors.push(`daisyclaw.plugin.json contracts.tools is missing: ${missing.join(", ")}`);
  }
  if (extra.length > 0) {
    errors.push(
      `daisyclaw.plugin.json contracts.tools has no matching defineToolPlugin tool: ${extra.join(
        ", ",
      )}`,
    );
  }
  ...
  if (extensionResolution.status !== "ok") {
    errors.push(
      extensionResolution.status === "missing" || extensionResolution.status === "empty"
        ? "package.json must include daisyclaw.extensions"
        : extensionResolution.error,
    );
  } else if (!extensionResolution.entries.includes(params.entry)) {
    errors.push(`package.json daisyclaw.extensions must include ${params.entry}`);
  }
  return errors;
}
...
export async function runPluginsBuildCommand(opts: PluginsBuildOptions): Promise<void> {
  ...
  if (opts.check) {
    ...
    if (
      ...
      defaultRuntime.error("Generated plugin metadata is out of date. Run daisyclaw plugins build.");
...
export async function runPluginsInitCommand(id: string, opts: PluginsInitOptions): Promise<void> {
  ...
  const packageManifest = {
    name: `daisyclaw-plugin-${id}`,
    version: "0.1.0",
    type: "module",
    private: true,
    scripts: {
      build: "tsc -p tsconfig.json",
      "plugin:build": "npm run build && daisyclaw plugins build --entry ./dist/index.js",
      "plugin:validate": "npm run build && daisyclaw plugins validate --entry ./dist/index.js",
      test: "vitest run",
    },
  ...
  const descriptionLiteral = jsStringLiteral(`Add ${name} tools to DaisyClaw.`);
...
import { getToolPluginMetadata } from "daisyclaw/plugin-sdk/tool-plugin";
...
`;
  ...
  writeJsonFile(path.join(rootDir, PLUGIN_MANIFEST_FILENAME), {
    id,
    name,
    description: `Add ${name} tools to DaisyClaw.`,
