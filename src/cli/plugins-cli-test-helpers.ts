...
export const loadConfig: Mock<LoadConfigFn> = vi.fn<LoadConfigFn>(() => ({}) as DaisyClawConfig);
...
export const replaceConfigFile: AsyncUnknownMock = vi.fn(
  async (params: { nextConfig: DaisyClawConfig }) => await writeConfigFile(params.nextConfig),
) as AsyncUnknownMock;
const resolveStateDir: Mock<() => string> = vi.fn(() => "/tmp/daisyclaw-state");
...
vi.mock("../config/config.js", () => ({
  ...
  writeConfigFile: ((config: DaisyClawConfig) =>
    invokeMock<
      [DaisyClawConfig],
...
export function resetPluginsCliTestState() {
  ...
  replaceConfigFile.mockImplementation(
    (async (params: { nextConfig: DaisyClawConfig }) =>
      await writeConfigFile(params.nextConfig)) as (...args: unknown[]) => Promise<unknown>,
  );
  resolveStateDir.mockReturnValue("/tmp/daisyclaw-state");
  resolveMarketplaceInstallShortcut.mockResolvedValue(null);
  installPluginFromMarketplace.mockResolvedValue({
    ok: false,
    error: "marketplace install failed",
  });
  enablePluginInConfig.mockImplementation(((cfg: DaisyClawConfig, pluginId: string) => ({
    config: cfg,
    enabled: true,
    pluginId,
  })) as (...args: unknown[]) => unknown);
  recordPluginInstall.mockImplementation(
    ((cfg: DaisyClawConfig) => cfg) as (...args: unknown[]) => unknown,
  );
  ...
  applyExclusiveSlotSelection.mockImplementation((({ config }: { config: DaisyClawConfig }) => ({
    config,
    warnings: [],
  })) as (...args: unknown[]) => unknown);
