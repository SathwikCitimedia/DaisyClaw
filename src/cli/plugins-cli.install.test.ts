...
import { installedPluginRoot } from "daisyclaw/plugin-sdk/test-fixtures";
...
const CLI_STATE_ROOT = "/tmp/daisyclaw-state";
const ORIGINAL_DAISYCLAW_STATE_DIR = process.env.DAISYCLAW_STATE_DIR;
const ORIGINAL_DAISYCLAW_NIX_MODE = process.env.DAISYCLAW_NIX_MODE;
const PROFILE_STATE_ROOT = "/tmp/daisyclaw-ledger-profile";
...
function useProfileExtensionsDir(): string {
  process.env.DAISYCLAW_STATE_DIR = PROFILE_STATE_ROOT;
  return path.resolve(PROFILE_STATE_ROOT, "extensions");
}

function createEnabledPluginConfig(pluginId: string): DaisyClawConfig {
  return {
    plugins: {
      entries: {
        [pluginId]: {
...
function createEmptyPluginConfig(): DaisyClawConfig {
  return {
    plugins: {
      entries: {},
    },
...
function createNpmPackPluginInstallResult(
  ...
  return {
    ...
    manifestName: `@daisyclaw/${pluginId}`,
    npmTarballName: `daisyclaw-${pluginId}-1.2.3.tgz`,
    npmResolution: {
      name: `@daisyclaw/${pluginId}`,
      version: "1.2.3",
      resolvedSpec: `@daisyclaw/${pluginId}@1.2.3`,
...
function createPathHookPackInstalledConfig(tmpRoot: string): DaisyClawConfig {
  return {
    hooks: {
      internal: {
        installs: {
...
function createNpmHookPackInstalledConfig(): DaisyClawConfig {
  return {
    hooks: {
      internal: {
        installs: {
...
function primeHookPackNpmFallback() {
  ...
  installPluginFromNpmSpec.mockResolvedValue({
    ok: false,
    error: "package.json missing daisyclaw.plugin.json",
  });
...
function primeBlockedNpmPluginInstall(params: {
  ...
  loadConfig.mockReturnValue({} as DaisyClawConfig);
...
function replaceConfigCall(callIndex = 0): { baseHash?: string; nextConfig?: DaisyClawConfig } {
...
describe("plugins cli install", () => {
  ...
  afterEach(() => {
    if (ORIGINAL_DAISYCLAW_STATE_DIR === undefined) {
      delete process.env.DAISYCLAW_STATE_DIR;
    } else {
      process.env.DAISYCLAW_STATE_DIR = ORIGINAL_DAISYCLAW_STATE_DIR;
    }
    if (ORIGINAL_DAISYCLAW_NIX_MODE === undefined) {
      delete process.env.DAISYCLAW_NIX_MODE;
    } else {
      process.env.DAISYCLAW_NIX_MODE = ORIGINAL_DAISYCLAW_NIX_MODE;
    }
  });
  ...
  it("fails closed for unrelated invalid config before installer side effects", async () => {
    ...
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/daisyclaw-config.json5",
      exists: true,
    ...
    expect(runtimeErrors.at(-1)).toContain(
      "Config invalid; run `daisyclaw doctor --fix` before installing plugins.",
    );
    expect(installPluginFromMarketplace).not.toHaveBeenCalled();
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(writeConfigFile).not.toHaveBeenCalled();
  ...
  it("resolves exact official external plugin ids through their npm package", async () => {
    ...
    expect(npmInstallCall().spec).toBe("@daisyclaw/brave-plugin");
    ...
    expect(record.spec).toBe("@daisyclaw/brave-plugin");
  ...
  it("passes third-party external catalog integrity with catalog install trust", async () => {
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("wecom-daisyclaw-plugin");
    loadConfig.mockReturnValue(cfg);
    findBundledPluginSourceMock.mockReturnValue(undefined);
    installPluginFromNpmSpec.mockResolvedValue(
      createNpmPluginInstallResult("wecom-daisyclaw-plugin"),
    );
    enablePluginInConfig.mockReturnValue({ config: enabledCfg });
    applyExclusiveSlotSelection.mockReturnValue({
      config: enabledCfg,
    ...
    expect(npmInstallCall().spec).toBe("@wecom/wecom-daisyclaw-plugin@2026.5.7");
    expect(npmInstallCall().expectedPluginId).toBe("wecom-daisyclaw-plugin");
  ...
  it("passes third-party external catalog integrity to hook-pack fallback", async () => {
    ...
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
      error: "package.json missing daisyclaw.extensions",
      code: "missing_daisyclaw_extensions",
    });
    installHooksFromNpmSpec.mockResolvedValue({
      ok: false,
      error:
        "aborted: npm package integrity drift detected for @wecom/wecom-daisyclaw-plugin@2026.5.7",
    });

    await expect(runPluginsCommand(["plugins", "install", "wecom"])).rejects.toThrow("__exit__:1");

    expect(npmInstallCall().trustedSourceLinkedOfficialInstall).toBe(true);
    expect(hookNpmInstallCall().spec).toBe("@wecom/wecom-daisyclaw-plugin@2026.5.7");
  ...
  it("installs npm-pack archives through npm install semantics", async () => {
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("demo");
    const archivePath = "/tmp/daisyclaw-demo-1.2.3.tgz";
    ...
    const record = persistedInstallRecord("demo");
    expect(record.source).toBe("npm");
    expect(record.spec).toBe("@daisyclaw/demo@1.2.3");
    ...
    expect(record.npmTarballName).toBe("daisyclaw-demo-1.2.3.tgz");
    expect(writeConfigFile).toHaveBeenCalledWith(enabledCfg);
  });
  ...
  it("marks explicit official npm package installs as trusted", async () => {
    ...
    await runPluginsCommand(["plugins", "install", "npm:@daisyclaw/discord"]);
  ...
  it("marks scoped official npm package installs as trusted", async () => {
    ...
    await runPluginsCommand(["plugins", "install", "@daisyclaw/discord"]);
  ...
  it("uses bundled DaisyClaw package specs instead of pinning stale managed npm overrides", async () => {
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("discord");
    ...
    findBundledPluginSourceMock.mockImplementation((params: unknown) => {
      ...
      return lookup.kind === "npmSpec" && lookup.value === "@daisyclaw/discord"
        ? {
            pluginId: "discord",
    ...
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(findBundledPluginSourceMock).toHaveBeenCalledWith({
      lookup: { kind: "npmSpec", value: "@daisyclaw/discord@2026.5.20" },
    });
    expect(findBundledPluginSourceMock).toHaveBeenCalledWith({
      lookup: { kind: "npmSpec", value: "@daisyclaw/discord" },
    });
    const record = persistedInstallRecord("discord");
    expect(record.source).toBe("path");
    expect(record.spec).toBe("@daisyclaw/discord@2026.5.20");
    expect(record.sourcePath).toBe(bundledPath);
    expect(record.installPath).toBe(bundledPath);
    expect(runtimeLogsContain("ships with the current DaisyClaw build")).toBe(true);
    expect(runtimeLogsContain("npm:@daisyclaw/discord@2026.5.20")).toBe(true);
  });
  ...
  it("marks catalog npm package installs with alternate selectors as trusted", async () => {
    ...
    await runPluginsCommand(["plugins", "install", "@wecom/wecom-daisyclaw-plugin@latest"]);
    ...
    expect(npmInstallCall().spec).toBe("@wecom/wecom-daisyclaw-plugin@latest");
    expect(npmInstallCall().expectedPluginId).toBe("wecom-daisyclaw-plugin");
    expect(npmInstallCall().trustedSourceLinkedOfficialInstall).toBe(true);
    expect(npmInstallCall().expectedIntegrity).toBeUndefined();
    expect(installPluginFromClawHub).not.toHaveBeenCalled();
  ...
  it("reports npm install failures without trying ClawHub when npm: prefix is used", async () => {
    ...
    installHooksFromNpmSpec.mockResolvedValue({
      ok: false,
      error: "package.json missing daisyclaw.hooks",
    });
  ...
  it("adds a Git PATH hint when npm plugin dependency install cannot spawn git", async () => {
    ...
    await expect(
      runPluginsCommand(["plugins", "install", "npm:@daisyclaw/whatsapp"]),
    ).rejects.toThrow("__exit__:1");
  ...
  it("does not resolve npm: prefixed bundled plugin ids through bundled installs", async () => {
    loadConfig.mockReturnValue({ plugins: { load: { paths: [] } } } as DaisyClawConfig);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
  ...
  it("rejects --pin for git installs and points at git refs", async () => {
    ...
    expect(runtimeErrors.at(-1)).toContain("daisyclaw plugins install git:<repo>@<ref>");
  });
  ...
  it("passes dangerous force unsafe install to linked path probe installs", async () => {
    ...
    const enabledCfg = createEnabledPluginConfig("demo");
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-plugin-link-"));
  ...
  it("passes dangerous force unsafe install to linked hook-pack probe fallback", async () => {
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-hook-link-"));
  ...
  it("does not fall back to hook pack for linked path when a no-flag security scan blocks", async () => {
    const localPluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-link-plugin-"));
  ...
  it("passes dangerous force unsafe install to local hook-pack fallback installs", async () => {
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-hook-install-"));
    primeHookPackPathFallback({
      tmpRoot,
  ...
  it("passes the active profile extensions dir to local path installs", async () => {
    const extensionsDir = useProfileExtensionsDir();
    const localPluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-local-plugin-"));
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("demo");
  ...
  it("suggests update or --force when npm plugin install target already exists", async () => {
    ...
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
      error:
        "plugin already exists: /home/daisyclaw/.daisyclaw/extensions/lossless-claw (delete it first)",
    });
    installHooksFromNpmSpec.mockResolvedValue({
      ok: false,
      error: "package.json missing daisyclaw.hooks",
    ...
    expect(runtimeErrors.at(-1)).toContain(
      "Use `daisyclaw plugins update <id-or-npm-spec>` to upgrade the tracked plugin, or rerun install with `--force` to replace it.",
    );
    expect(runtimeErrors.at(-1)).not.toContain("Also not a valid hook pack");
  });
  ...
  it("still falls back to local hook pack when dangerous force unsafe install is set for non-security errors", async () => {
    const localHookDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-local-hook-pack-"));
    const cfg = {} as DaisyClawConfig;
    const installedCfg = {
      hooks: {
        internal: {
  ...
  it("does not fall back to npm when explicit ClawHub rejects a real package", async () => {
    ...
    installPluginFromClawHub.mockResolvedValue({
      ok: false,
      error: 'Use "daisyclaw skills install demo" instead.',
      code: "skill_package",
    });
    ...
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(runtimeErrors.at(-1)).toContain('Use "daisyclaw skills install demo" instead.');
  });
