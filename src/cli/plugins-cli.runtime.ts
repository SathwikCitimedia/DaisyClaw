// Runtime implementations for `daisyclaw plugins` subcommands. Heavy plugin modules stay
...
function formatBlockedRuntimePluginGuidance(params: {
  ...
  const alternative =
    pluginId === "acpx"
      ...
      : 'change the runtime policy to "daisyclaw"';
...
function collectConfiguredRuntimePluginWarnings(params: {
  ...
  return collectConfiguredRuntimePluginIds(params.cfg, {
    ...
    return [
      `- Configured runtime "${runtimeId}" requires the ${candidate.label} plugin, but no enabled "${runtimeId}" plugin was found. Run "daisyclaw doctor --fix" to install ${installSpec}, or install it manually with "daisyclaw plugins install ${installSpec}".`,
    ];
  });
}
...
export async function runPluginsEnableCommand(idInput: string): Promise<void> {
  ...
  const cfg = (snapshot.sourceConfig ?? snapshot.config) as DaisyClawConfig;
  ...
  let next: DaisyClawConfig = enableResult.config;
...
export async function runPluginsRegistryCommand(opts: PluginRegistryOptions): Promise<void> {
  ...
  if (inspection.refreshReasons.length > 0) {
    ...
    lines.push(`${theme.muted("Repair:")} ${theme.command("daisyclaw plugins registry --refresh")}`);
  }
...
export async function runPluginsDoctorCommand(): Promise<void> {
  ...
  if (shadowed.length > 0) {
    ...
    for (const diag of shadowed) {
      ...
      lines.push("  repair:");
      lines.push("    daisyclaw plugins inspect " + (diag.pluginId ?? "<plugin-id>"));
      lines.push("    edit or remove the config-selected plugin source");
      lines.push("    daisyclaw plugins registry --refresh");
      lines.push("    daisyclaw gateway restart --force");
    }
  }
  ...
  const docs = formatDocsLink("/plugin", "docs.daisyclaw.ai/plugin");
  lines.push("");
