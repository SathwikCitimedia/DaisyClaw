...
describe("persistPluginInstall", () => {
  ...
  it("adds installed plugins to restrictive allowlists before enabling", async () => {
    ...
    enablePluginInConfig.mockImplementation((...args: unknown[]) => {
      const [cfg, pluginId] = args as [DaisyClawConfig, string];
  ...
  it("removes a replaced managed install directory before refreshing the registry", async () => {
    ...
    setInstalledPluginIndexInstallRecords({
      codex: {
        source: "clawhub",
        spec: "clawhub:@daisyclaw/codex",
        installPath: "/tmp/daisyclaw/extensions/codex",
      },
    });
    planPluginUninstall.mockReturnValueOnce({
      ...
      directoryRemoval: {
        target: "/tmp/daisyclaw/extensions/codex",
      },
    });
    ...
    await persistPluginInstall({
      ...
      install: {
        ...
        installPath: "/tmp/daisyclaw/npm/node_modules/@daisyclaw/codex",
      },
    });
    ...
    expect(runtimeLogs.join("\n")).toContain(
      "Removed previous plugin install directory: /tmp/daisyclaw/extensions/codex",
    );
  });
  ...
  it("warns when an installed npm plugin remains shadowed by a config-selected source", async () => {
    ...
    const next = await persistPluginInstall({
      ...
      install: {
        ...
        installPath: "/tmp/daisyclaw/npm/node_modules/@daisyclaw/discord/index.ts",
      },
    });
    ...
    expect(runtimeLogs.join("\n")).toContain(
      "active config source: /tmp/daisyclaw-upstream/extensions/discord/index.ts",
    );
    expect(runtimeLogs.join("\n")).toContain(
      "installed npm source: /tmp/daisyclaw/npm/node_modules/@daisyclaw/discord/index.ts",
    );
    expect(runtimeLogs.join("\n")).toContain("daisyclaw plugins doctor");
  });
  ...
  it("does not warn when the config-selected source is inside the npm install path", async () => {
      ...
      plugins: [
        {
          id: "discord",
          origin: "config",
          source: "/tmp/daisyclaw/npm/node_modules/@daisyclaw/discord/dist/index.js",
          status: "loaded",
        },
      ],
      diagnostics: [],
    ...
    await persistPluginInstall({
      ...
      install: {
        source: "npm",
        spec: "@daisyclaw/discord",
        installPath: "/tmp/daisyclaw/npm/node_modules/@daisyclaw/discord",
      },
    });
