// `daisyclaw plugins list`: builds registry reports and defers terminal-only formatting modules.
...
export async function runPluginsListCommand(
  ...
  if (list.length === 0) {
    runtime.log(
      theme.muted(
        `No plugins found. Run ${formatCliCommand("daisyclaw plugins install <plugin>")} to add one, or ${formatCliCommand("daisyclaw plugins list --json")} to inspect raw discovery state.`,
      ),
    );
    return;
  }
  ...
  if (!opts.verbose) {
    ...
    const rows = list.map((plugin) => {
      ...
      return {
        ...
        Format: plugin.format ?? "daisyclaw",
        Status:
          plugin.status === "error"
            ? theme.error("error")
            : plugin.enabled
