...
/** Options accepted by `daisyclaw plugins search`. */
...
function formatPackageSearchLine(entry: ClawHubPackageSearchResult): string {
  ...
  return `${pkg.name}  ${theme.muted(flags.join(" | "))}${summary}\n  ${theme.muted(`Install: daisyclaw plugins install clawhub:${pkg.name}`)}`;
}
...
export async function runPluginsSearchCommand(
  ...
  if (!query) {
    runtime.error("Usage: daisyclaw plugins search <query>");
