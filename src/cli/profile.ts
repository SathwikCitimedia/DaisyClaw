...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.daisyclaw${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.DAISYCLAW_PROFILE = profile;

  const existingStateDir = normalizeOptionalString(env.DAISYCLAW_STATE_DIR);
  const stateDir = existingStateDir || resolveProfileStateDir(profile, env, homedir);
  if (!existingStateDir) {
    env.DAISYCLAW_STATE_DIR = stateDir;
  }

  if (!normalizeOptionalString(env.DAISYCLAW_CONFIG_PATH)) {
    env.DAISYCLAW_CONFIG_PATH = path.join(stateDir, "daisyclaw.json");
  }

  if (profile === "dev" && !env.DAISYCLAW_GATEWAY_PORT?.trim()) {
    env.DAISYCLAW_GATEWAY_PORT = "19001";
  }
}
