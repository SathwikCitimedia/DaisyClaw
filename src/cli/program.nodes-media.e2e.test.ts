...
describe("cli program (nodes media)", () => {
  ...
  it("runs nodes camera snap and prints two MEDIA paths", async () => {
    ...
    expect(mediaPaths[0]).toContain("daisyclaw-camera-snap-");
    expect(mediaPaths[1]).toContain("daisyclaw-camera-snap-");
  ...
  it("runs nodes camera clip and prints one MEDIA path", async () => {
    ...
    await expectLoggedSingleMediaFile({
      expectedPathPattern: /daisyclaw-camera-clip-front-.*\.mp4$/,
    });
  });
    ...
    it.each([
      {
        ...
        expectedPathPattern: /daisyclaw-camera-snap-front-.*\.jpg$/,
      },
      {
