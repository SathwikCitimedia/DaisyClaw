...
describe("reparseProgramFromActionArgs", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    buildParseArgvMock.mockReturnValue(["node", "daisyclaw", "status"]);
  ...
  it("uses action command name + args as fallback argv", async () => {
    ...
    const actionCommand = {
      name: () => "status",
      parent: {
        rawArgs: ["node", "daisyclaw", "status", "--json"],
      },
    ...
    expect(parseAsync).toHaveBeenCalledWith(["node", "daisyclaw", "status"]);
  });
