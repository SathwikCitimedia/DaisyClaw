...
describe("ensureConfigReady", () => {
  ...
  const originalDaisyClawStateDir = process.env.DAISYCLAW_STATE_DIR;
  ...
  function useTempDaisyClawHome(): string {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-config-guard-"));
    tempRoots.push(root);
    process.env.DAISYCLAW_HOME = root;
    delete process.env.DAISYCLAW_STATE_DIR;
    return root;
  }

  function writeLegacyTaskSidecarMarker(root: string): void {
    const markerPath = path.join(root, ".daisyclaw", "tasks", "runs.sqlite");
    fs.mkdirSync(path.dirname(markerPath), { recursive: true });
    fs.writeFileSync(markerPath, "");
  }

  function writeStateMarker(root: string, relativePath: string): void {
    const markerPath = path.join(root, ".daisyclaw", relativePath);
    fs.mkdirSync(path.dirname(markerPath), { recursive: true });
    fs.writeFileSync(markerPath, "{}");
  }
  ...
  afterEach(() => {
    ...
    if (originalDaisyClawStateDir === undefined) {
      delete process.env.DAISYCLAW_STATE_DIR;
    } else {
      process.env.DAISYCLAW_STATE_DIR = originalDaisyClawStateDir;
    }
    for (const root of tempRoots.splice(0)) {
      fs.rmSync(root, { recursive: true, force: true });
    }
  ...
  it("runs doctor flow when lightweight startup detection finds legacy state", async () => {
    const root = useTempDaisyClawHome();
  ...
  it("runs doctor flow for legacy sessions without task sidecars", async () => {
    const root = useTempDaisyClawHome();
    fs.mkdirSync(path.join(root, ".daisyclaw", "sessions"), { recursive: true });
  ...
  it("uses shared tilde expansion for DAISYCLAW_HOME in the startup detector", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-config-guard-home-"));
    tempRoots.push(root);
    process.env.HOME = root;
    process.env.DAISYCLAW_HOME = "~/svc";
    delete process.env.DAISYCLAW_STATE_DIR;
  ...
  it("exits for invalid config on non-allowlisted commands", async () => {
    ...
    expect(plainErrorCalls(runtime)).toEqual([
      "DaisyClaw config is invalid",
      "File: /tmp/daisyclaw.json",
      "Problem:",
      "  - channels.quietchat: invalid",
      "",
      `Fix: ${formatCliCommand("daisyclaw doctor --fix")}`,
      `Inspect: ${formatCliCommand("daisyclaw config validate")}`,
  ...
  it("replaces doctor fix advice for plugin packaging-only invalid config", async () => {
    ...
    expect(calls).not.toContain(`Fix: ${formatCliCommand("daisyclaw doctor --fix")}`);
    expect(runtime.exit).toHaveBeenCalledWith(1);
  });
  ...
  it("runs doctor migration flow only once per module instance", async () => {
    writeLegacyTaskSidecarMarker(useTempDaisyClawHome());
