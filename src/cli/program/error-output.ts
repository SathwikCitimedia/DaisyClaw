...
function resolveHelpCommand(argv: string[] | undefined, options?: { root?: boolean }): string {
  if (options?.root || !argv) {
    return formatCliCommand("daisyclaw --help");
  }
  const commandPath = getCommandPathWithRootOptions(argv, 2);
  if (commandPath.length === 0) {
    return formatCliCommand("daisyclaw --help");
  }
  return formatCliCommand(`daisyclaw ${commandPath.join(" ")} --help`);
}
...
function formatDocsHint(): string {
  return `${theme.muted("Docs:")} ${formatDocsLink("/cli", "docs.daisyclaw.ai/cli")}`;
}

/** Convert Commander parse errors into DaisyClaw-specific help and docs guidance. */
export function formatCliParseErrorOutput(
  ...
  if (unknownCommand) {
    ...
    return lines(
      theme.error(`DaisyClaw does not know the command ${quote(command)}.`),
      formatHelpHint(options.argv, { root: true }),
      `${theme.muted("Plugin command?")} ${theme.command(formatCliCommand("daisyclaw plugins list"))}`,
      formatDocsHint(),
    );
  }
  ...
  if (unknownOption) {
    ...
    return lines(
      theme.error(`DaisyClaw does not recognize option ${quote(option)}.`),
  ...
  return lines(
    theme.error(`DaisyClaw could not parse this command: ${message}`),
    formatHelpHint(options.argv),
  );
}
