...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["daisyclaw status", "Show channel health + session summary."],
          ["daisyclaw status --all", "Full diagnosis (read-only)."],
          ["daisyclaw status --json", "Machine-readable output."],
          ["daisyclaw status --usage", "Show model provider usage/quota snapshots."],
          [
            "daisyclaw status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["daisyclaw status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.daisyclaw.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.daisyclaw.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
  ...
  const sessionsCmd = addSessionsListOptions(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["daisyclaw sessions", "List all sessions."],
          ["daisyclaw sessions --agent work", "List sessions for one agent."],
          ["daisyclaw sessions --all-agents", "Aggregate sessions across agents."],
          ["daisyclaw sessions --active 120", "Only last 2 hours."],
          ["daisyclaw sessions --limit 25", "Show the newest 25 sessions."],
          ["daisyclaw sessions --json", "Machine-readable output."],
          ["daisyclaw sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.daisyclaw.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["daisyclaw sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "daisyclaw sessions cleanup --dry-run --fix-missing",
          ...
          ["daisyclaw sessions cleanup --enforce", "Apply maintenance now."],
          ["daisyclaw sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["daisyclaw sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "daisyclaw sessions cleanup --enforce --store ./tmp/sessions.json",
  ...
  sessionsCmd
    ...
    .option("--output <path>", "Output directory name inside .daisyclaw/trajectory-exports")
  ...
  const commitmentsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["daisyclaw commitments", "List pending inferred follow-ups."],
          ["daisyclaw commitments --all", "List all inferred follow-ups."],
          ["daisyclaw commitments --agent work", "List one agent's inferred follow-ups."],
          ["daisyclaw commitments dismiss cm_abc123", "Dismiss a follow-up."],
        ])}`,
    )
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
