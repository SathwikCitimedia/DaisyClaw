...
describe("registerSubCliCommands", () => {
  ...
  const originalDisableLazySubcommands = process.env.DAISYCLAW_DISABLE_LAZY_SUBCOMMANDS;
  const originalEnablePrivateQaCli = process.env.DAISYCLAW_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    if (originalDisableLazySubcommands === undefined) {
      delete process.env.DAISYCLAW_DISABLE_LAZY_SUBCOMMANDS;
    } else {
      process.env.DAISYCLAW_DISABLE_LAZY_SUBCOMMANDS = originalDisableLazySubcommands;
    }
    process.env.DAISYCLAW_ENABLE_PRIVATE_QA_CLI = "1";
  ...
  afterEach(() => {
    ...
    if (originalEnablePrivateQaCli === undefined) {
      delete process.env.DAISYCLAW_ENABLE_PRIVATE_QA_CLI;
    } else {
      process.env.DAISYCLAW_ENABLE_PRIVATE_QA_CLI = originalEnablePrivateQaCli;
    }
  });

  it("registers the primary placeholder plus completion and dispatches", async () => {
    const program = createRegisteredProgram(["node", "daisyclaw", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "daisyclaw"]);
  ...
  it("re-parses argv for lazy subcommands", async () => {
    const program = createRegisteredProgram(["node", "daisyclaw", "nodes", "list"], "daisyclaw");
  ...
  it("registers the infer placeholder and dispatches through the capability registrar", async () => {
    const program = createRegisteredProgram(["node", "daisyclaw", "infer"], "daisyclaw");
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    const program = createRegisteredProgram(["node", "daisyclaw", "acp", "--help"], "daisyclaw");
  ...
  it("registers only the gateway run surface for gateway startup", async () => {
    const argv = ["node", "daisyclaw", "gateway", "--force"];
    process.argv = argv;
    const program = new Command().name("daisyclaw");
  ...
  it("keeps the full gateway CLI for non-run gateway subcommands", async () => {
    const argv = ["node", "daisyclaw", "gateway", "call", "health"];
    process.argv = argv;
    const program = new Command().name("daisyclaw");
  ...
  it("passes completion context to channel registration", async () => {
    const argv = ["node", "daisyclaw", "completion", "--write-state"];
    const program = new Command().name("daisyclaw");
  ...
  it.each([
    ...
    process.argv = ["node", "daisyclaw", ...args];
    const program = new Command().name("daisyclaw");
  ...
  it("does not preload plugin CLI registrations for bare plugin parent help", async () => {
    process.argv = ["node", "daisyclaw", "plugins"];
    const program = new Command().name("daisyclaw");
