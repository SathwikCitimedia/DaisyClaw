...
function readDevicePairPublicUrlFromConfig(cfg: DaisyClawConfig): string | undefined {
...
function shouldResolveLocalGatewayPasswordSecret(
  ...
  if (trimToUndefined(env.DAISYCLAW_GATEWAY_PASSWORD)) {
    return false;
  }
  ...
  const envToken = trimToUndefined(env.DAISYCLAW_GATEWAY_TOKEN);
...
async function resolveLocalGatewayPasswordSecretIfNeeded(cfg: DaisyClawConfig): Promise<void> {
...
export function registerQrCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/qr", "docs.daisyclaw.ai/cli/qr")}\n`,
    )
    .option(
      "--remote",
    ...
    .action(async (opts: QrCliOptions) => {
      try {
        ...
        const lines: string[] = [
          ...
          "Scan this with the DaisyClaw mobile app (Onboarding -> Scan QR).",
          "",
        ];
        ...
        lines.push(
          ...
          `  ${theme.command("daisyclaw devices list")}`,
          `  ${theme.command("daisyclaw devices approve <requestId>")}`,
        );
