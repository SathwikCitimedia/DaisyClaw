...
export function hasPluginHelpAffectingConfig(config: DaisyClawConfig | null | undefined): boolean {
...
export function hasPluginHelpAffectingEnv(env: NodeJS.ProcessEnv): boolean {
  return Boolean(
    env.DAISYCLAW_BUNDLED_PLUGINS_DIR?.trim() || env.DAISYCLAW_DISABLE_BUNDLED_PLUGINS?.trim(),
  );
}
