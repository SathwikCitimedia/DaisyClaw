...
describe("tryRouteCli", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    originalDisableRouteFirst = process.env.DAISYCLAW_DISABLE_ROUTE_FIRST;
    originalHideBanner = process.env.DAISYCLAW_HIDE_BANNER;
    delete process.env.DAISYCLAW_DISABLE_ROUTE_FIRST;
  ...
  afterEach(() => {
    ...
    if (originalDisableRouteFirst === undefined) {
      ...
      process.env.DAISYCLAW_DISABLE_ROUTE_FIRST = originalDisableRouteFirst;
    }
  ...
  it("keeps config guard for routed status --json commands", async () => {
    await expect(tryRouteCli(["node", "daisyclaw", "status", "--json"])).resolves.toBe(true);
  ...
  it("keeps config guard for the parent tasks JSON list alias", async () => {
    await expect(tryRouteCli(["node", "daisyclaw", "tasks", "--json"])).resolves.toBe(true);
  ...
  it("does not pass suppressDoctorStdout for routed non-json commands", async () => {
    await expect(tryRouteCli(["node", "daisyclaw", "status"])).resolves.toBe(true);
  ...
  it("keeps logs routed to stderr for routed --json commands", async () => {
    ...
    await tryRouteCli(["node", "daisyclaw", "agents", "--json"]);
  ...
  it("routes command-run logs to stderr for config-guard-skipping --json routes", async () => {
    ...
    await expect(tryRouteCli(["node", "daisyclaw", "models", "status", "--json"])).resolves.toBe(
      true,
    );
  ...
  it("does not route logs to stderr during plugin loading without --json", async () => {
    ...
    await tryRouteCli(["node", "daisyclaw", "agents"]);
  ...
  it("routes status when root options precede the command", async () => {
    await expect(tryRouteCli(["node", "daisyclaw", "--log-level", "debug", "status"])).resolves.toBe(
      true,
    );

    expect(findRoutedCommandMock).toHaveBeenCalledWith(
      ["status"],
      ["node", "daisyclaw", "--log-level", "debug", "status"],
    );
    expect(ensureConfigReadyMock).toHaveBeenCalledTimes(1);
    const configReadyCall = firstConfigReadyCall();
    expect(typeof configReadyCall?.runtime).toBe("object");
  ...
  it("respects DAISYCLAW_HIDE_BANNER for routed commands", async () => {
  ...
  it("falls back before bootstrap when the route cannot parse the argv", async () => {
    ...
    await expect(tryRouteCli(["node", "daisyclaw", "tasks", "list"])).resolves.toBe(false);
