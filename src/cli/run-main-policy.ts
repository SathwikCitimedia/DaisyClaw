  ...
  if (parentPluginId) {
    if (allow.length > 0 && !allow.includes(parentPluginId)) {
      if (parentPluginId === normalizedPluginId) {
        return (
          `The \`daisyclaw ${normalizedPluginId}\` command is unavailable because ` +
    ...
    if (
      ...
      return (
        `The \`daisyclaw ${normalizedPluginId}\` command is provided by the ` +
        `"${parentPluginId}" plugin, but that bundled plugin is disabled by default. Run ` +
        `\`daisyclaw plugins enable ${parentPluginId}\` to enable that CLI surface.`
      );
    }
    if (commandAlias.kind === "runtime-slash") {
      const cliHint = commandAlias.cliCommand
        ? `Use \`daisyclaw ${commandAlias.cliCommand}\` for related CLI operations, or `
        : "Use ";
      return (
    ...
    if (ownerEnabled) {
      ...
      if (toolOwner.availability === "manifest-only") {
        return (
          ...
          "Run `daisyclaw --help` to see available CLI subcommands."
        );
      }
      return (
