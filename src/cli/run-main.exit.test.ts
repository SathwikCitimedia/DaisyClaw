...
vi.mock("../infra/path-env.js", () => ({
  ensureDaisyClawCliOnPath: ensurePathMock,
}));
...
describe("runCli exit behavior", () => {
  beforeEach(() => {
    ...
    delete process.env.DAISYCLAW_DISABLE_CLI_STARTUP_HELP_FAST_PATH;
    delete process.env.DAISYCLAW_HIDE_BANNER;
  ...
  it("does not force process.exit after successful routed command", async () => {
    ...
    await runCli(["node", "daisyclaw", "status"]);

    expect(maybeRunCliInContainerMock).toHaveBeenCalledWith(["node", "daisyclaw", "status"]);
    expect(tryRouteCliMock).toHaveBeenCalledWith(["node", "daisyclaw", "status"]);
  ...
  it("disposes registered harnesses after full CLI command completion", async () => {
    ...
    await runCli(["node", "daisyclaw", "agent", "--local"]);

    expect(parseAsync).toHaveBeenCalledWith(["node", "daisyclaw", "agent", "--local"]);
  ...
  it("shows the standard spinner while loading the full CLI", async () => {
    ...
    await runCli(["node", "daisyclaw", "config"]);

    expect(createCliProgressMock).toHaveBeenCalledWith({
      label: "Loading DaisyClaw CLI…",
      indeterminate: true,
      delayMs: 0,
    });
  ...
  it("suppresses startup progress for json output commands before full CLI parsing", async () => {
    ...
    await runCli(["node", "daisyclaw", "sessions", "--json", "--limit", "all"]);
  ...
  it("pauses non-tty stdin after full CLI command completion", async () => {
    ...
    try {
      await runCli(["node", "daisyclaw", "channels"]);

      expect(parseAsync).toHaveBeenCalledWith(["node", "daisyclaw", "channels"]);
  ...
  it("emits the startup banner before gateway foreground fast-path startup", async () => {
    await runCli(["node", "daisyclaw", "gateway", "--force"]);

    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(emitCliBannerMock).toHaveBeenCalledWith("9.9.9-test", {
      argv: ["node", "daisyclaw", "gateway", "--force"],
    });
  ...
  it("honors banner suppression on the gateway foreground fast path", async () => {
    process.env.DAISYCLAW_HIDE_BANNER = "1";

    await runCli(["node", "daisyclaw", "gateway"]);

    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(emitCliBannerMock).not.toHaveBeenCalled();
    expect(commanderParseAsyncMock).toHaveBeenCalledWith(["node", "daisyclaw", "gateway"]);
  });
  ...
  it("renders browser help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "daisyclaw", "browser", "--help"]);
  ...
  it("renders secrets help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "daisyclaw", "secrets", "--help"]);
  ...
  it("renders nodes help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "daisyclaw", "nodes", "--help"]);
  ...
  it("defers nodes help startup metadata when plugin config can change command metadata", async () => {
    const argv = ["node", "daisyclaw", "nodes", "--help"];
    const parseAsync = vi.fn().mockResolvedValueOnce(undefined);
    const program = {
  ...
  it("renders selected subcommand help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "daisyclaw", "doctor", "--help"]);
  ...
  it("keeps root help on the precomputed path without proxy bootstrap", async () => {
    ...
    await runCli(["node", "daisyclaw", "--help"]);
  ...
  it("renders setup/onboard/configure help without building the full program", async () => {
    await runCli(["node", "daisyclaw", "setup", "--help"]);
  ...
  it("renders root help without building the full program", async () => {
    ...
    expect(maybeRunCliInContainerMock).toHaveBeenCalledWith(["node", "daisyclaw", "--help"]);
    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(loadRootHelpRenderOptionsForConfigSensitivePluginsMock).toHaveBeenCalledTimes(1);
    expect(outputPrecomputedRootHelpTextMock).toHaveBeenCalledTimes(1);
  ...
  it.each([
    ["gateway runtime", ["node", "daisyclaw", "gateway", "run"]],
    ["bare gateway runtime", ["node", "daisyclaw", "gateway"]],
    ["node runtime", ["node", "daisyclaw", "node", "run"]],
    ["local agent runtime", ["node", "daisyclaw", "agent", "--local"]],
    ["provider inference", ["node", "daisyclaw", "infer", "web", "fetch", "https://example.com"]],
    ["model command", ["node", "daisyclaw", "models", "auth", "login", "openai"]],
    ["plugin command", ["node", "daisyclaw", "plugins", "marketplace", "list"]],
    ["skill command", ["node", "daisyclaw", "skills", "search", "browser"]],
    ["update command", ["node", "daisyclaw", "update", "check"]],
    ["channel probe", ["node", "daisyclaw", "channels", "status", "--probe"]],
    ["channel capabilities probe", ["node", "daisyclaw", "channels", "capabilities"]],
    ["directory plugin command", ["node", "daisyclaw", "directory", "peers", "list"]],
    ["message plugin command", ["node", "daisyclaw", "message", "send", "--to", "demo"]],
    ["metadata-owned plugin command", ["node", "daisyclaw", "googlemeet", "login"]],
  ])("starts managed proxy routing for %s", (_name, argv) => {
    expect(shouldStartProxyForCli(argv)).toBe(true);
  });

  it.each([
    ["root help", ["node", "daisyclaw", "--help"]],
    ["root version", ["node", "daisyclaw", "--version"]],
    ["gateway help", ["node", "daisyclaw", "gateway", "--help"]],
    ["gateway run help", ["node", "daisyclaw", "gateway", "run", "--help"]],
    ["status", ["node", "daisyclaw", "status"]],
    ["health", ["node", "daisyclaw", "health"]],
    ["gateway status", ["node", "daisyclaw", "gateway", "status"]],
    ["gateway health", ["node", "daisyclaw", "gateway", "health"]],
    ["remote agent control-plane", ["node", "daisyclaw", "agent", "run"]],
    ["chat control-plane", ["node", "daisyclaw", "chat"]],
    ["terminal control-plane", ["node", "daisyclaw", "terminal"]],
    ["config", ["node", "daisyclaw", "config", "get", "proxy.enabled"]],
    ["channels parent help", ["node", "daisyclaw", "channels"]],
    ["completion", ["node", "daisyclaw", "completion", "zsh"]],
    ["debug proxy cli", ["node", "daisyclaw", "proxy", "start"]],
    ["agents list", ["node", "daisyclaw", "agents", "list"]],
    ["models list", ["node", "daisyclaw", "models", "list"]],
    ["models status without live probe", ["node", "daisyclaw", "models", "status"]],
    ["skills check", ["node", "daisyclaw", "skills", "check"]],
    ["skills info", ["node", "daisyclaw", "skills", "info", "weather"]],
    ["skills list", ["node", "daisyclaw", "skills", "list"]],
    ["tasks list", ["node", "daisyclaw", "tasks", "list"]],
    ["legacy singular tool namespace", ["node", "daisyclaw", "tool", "image_generate"]],
    ["gateway tools namespace typo", ["node", "daisyclaw", "tools", "effective"]],
    ["migrate", ["node", "daisyclaw", "migrate"]],
  ...
  it("starts the managed proxy for network-capable commands by default", async () => {
    ...
    await runCli(["node", "daisyclaw", "plugins", "marketplace", "list"]);
  ...
  it("starts the managed proxy for metadata-owned plugin commands by default", async () => {
    ...
    await runCli(["node", "daisyclaw", "googlemeet", "login"]);

    expect(startProxyMock).toHaveBeenCalledWith(undefined);
  });

  it("rejects unowned command roots before proxy and plugin runtime registration", async () => {
    await expect(runCli(["node", "daisyclaw", "foo"])).rejects.toThrow(
  ...
  it("rejects unowned command roots even when --help is appended (regression for #81077)", async () => {
    await expect(runCli(["node", "daisyclaw", "foo", "--help"])).rejects.toThrow(
      'No built-in command or plugin CLI metadata owns "foo"',
    );
  ...
  it("rejects unowned command roots even when --version is appended", async () => {
    await expect(runCli(["node", "daisyclaw", "foo", "--version"])).rejects.toThrow(
      'No built-in command or plugin CLI metadata owns "foo"',
    );
  ...
  it("does not suggest plugins.allow for unknown command roots before proxy startup", async () => {
    ...
    let error: unknown;
    try {
      await runCli(["node", "daisyclaw", "totally-unknown"]);
    } catch (caught) {
      error = caught;
    }
  ...
  it("preserves plugins.allow diagnostics for roots owned only by CLI metadata", async () => {
    ...
    await expect(runCli(["node", "daisyclaw", "qa"])).rejects.toThrow(
  ...
  it("reports plugin tool command mistakes before proxy startup", async () => {
    ...
    await expect(runCli(["node", "daisyclaw", "lcm_recent"])).rejects.toThrow(
  ...
  it("does not install the env proxy dispatcher for bypassed skills inspection commands", async () => {
    ...
    await runCli(["node", "daisyclaw", "skills", "check"]);
  ...
  it.each([
    ["auth", ["node", "daisyclaw", "auth", "--help"]],
    ["tool", ["node", "daisyclaw", "tool", "image_generate"]],
    ["tools", ["node", "daisyclaw", "tools", "effective"]],
  ...
  it("routes lazy plugin registration logs to stderr only during --json registration", async () => {
    ...
    await runCli(["node", "daisyclaw", "memory", "search", "query", "--json"]);
  ...
  it("does not route lazy plugin registration logs for pass-through --json after terminator", async () => {
    ...
    await runCli(["node", "daisyclaw", "memory", "--", "--json"]);
  ...
  it("fails protected commands when managed proxy activation fails", async () => {
    ...
    await expect(runCli(["node", "daisyclaw", "gateway", "run"])).rejects.toThrow(
  ...
  it("stops the managed proxy after normal gateway runtime completion", async () => {
    ...
    await runCli(["node", "daisyclaw", "gateway", "run"]);
  ...
  it("stops the managed proxy and exits after SIGINT", async () => {
    ...
    try {
      const runPromise = runCli(["node", "daisyclaw", "plugins", "marketplace", "list"]);
      await vi.waitFor(() => {
        expect(
  ...
  it("starts onboarding for bare root invocations when config only has metadata", async () => {
    readConfigFileSnapshotMock.mockResolvedValueOnce({
      ...
      valid: true,
      sourceConfig: {
        $schema: "https://daisyclaw.ai/config.json",
  ...
  it("points noninteractive fresh bare root invocations to onboarding automation", async () => {
    ...
    try {
      ...
      expect(errorSpy).toHaveBeenCalledWith(
        "Onboarding needs an interactive TTY. Use `daisyclaw onboard --non-interactive --accept-risk ...` for automation.",
      );
  ...
  it("bootstraps env proxy before modern onboard Crestodian startup", async () => {
    ...
    await runCli(["node", "daisyclaw", "onboard", "--modern", "--json"]);
  ...
  it("does not fail the command when memory cleanup is unavailable", async () => {
    ...
    await expect(runCli(["node", "daisyclaw", "status"])).resolves.toBeUndefined();
  ...
  it("returns after a handled container-target invocation", async () => {
    ...
    await runCli(["node", "daisyclaw", "--container", "demo", "status"]);
  ...
  it("swallows Commander parse exits after recording the exit code", async () => {
    ...
    expect(registerSubCliByNameMock.mock.calls).toEqual([
      [program, "status", ["node", "daisyclaw", "status"]],
    ]);
    expect(process.exitCode).toBe(1);
    process.exitCode = exitCode;
  });
  ...
  it("loads the real primary command before rendering command help", async () => {
    ...
    expect(registerCoreCliByNameMock.mock.calls).toEqual([
      [program, ctx, "doctor", ["node", "daisyclaw", "doctor", "--help"]],
    ]);
    expect(registerSubCliByNameMock.mock.calls).toEqual([
      [program, "doctor", ["node", "daisyclaw", "doctor", "--help"]],
    ]);
  });
  ...
  it("restores terminal state before uncaught CLI exits", async () => {
    ...
    try {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        "[daisyclaw] DaisyClaw hit an unexpected runtime error.",
      );
      expect(consoleErrorSpy).toHaveBeenCalledWith("[daisyclaw] Reason: boom");
  ...
  it("does not exit for transient uncaught CLI exceptions", async () => {
    ...
    try {
      ...
      expect(consoleWarnSpy.mock.calls).toEqual([
        ["[daisyclaw] Non-fatal uncaught exception (continuing):", hostUnreachable.stack],
      ]);
