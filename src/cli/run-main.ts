...
import type { ConfigFileSnapshot, DaisyClawConfig } from "../config/types.daisyclaw.js";
import { isTruthyEnvValue, normalizeEnv } from "../infra/env.js";
import { isMainModule } from "../infra/is-main.js";
import type { ProxyHandle } from "../infra/net/proxy/proxy-lifecycle.js";
import { ensureDaisyClawCliOnPath } from "../infra/path-env.js";
...
function createGatewayCliMainStartupTrace(argv: string[]) {
  ...
  const enabled =
    isTruthyEnvValue(process.env.DAISYCLAW_GATEWAY_STARTUP_TRACE) &&
...
function shouldBootstrapCliProxyBeforeFastPath(env: NodeJS.ProcessEnv = process.env): boolean {
  if (
    isTruthyEnvValue(env.DAISYCLAW_DEBUG_PROXY_ENABLED) ||
    isTruthyEnvValue(env.DAISYCLAW_DEBUG_PROXY_REQUIRE)
  ) {
    return true;
  }
...
function createAllowlistAgnosticCliLookupConfig(config: DaisyClawConfig): DaisyClawConfig {
...
async function resolveUnownedCliPrimaryMessage(params: {
  ...
  return (
    ...
    `Unknown command: daisyclaw ${params.primary}. No built-in command or plugin CLI metadata owns "${params.primary}".`
  );
}
...
export async function runCli(argv: string[] = process.argv) {
  ...
  const containerTargetName =
    parsedContainer.container ?? normalizeOptionalString(process.env.DAISYCLAW_CONTAINER) ?? null;
  ...
  let bestEffortConfigPromise: Promise<DaisyClawConfig> | null = null;
  const readBestEffortCliConfig = async (): Promise<DaisyClawConfig> => {
  ...
  try {
    ...
    // `daisyclaw <typo> --help` surfaces the same Unknown command error as
    // `daisyclaw <typo>` instead of silently showing generic top-level help.
    ...
    if (shouldRunBareRootCrestodian) {
      ...
      if (!process.stdin.isTTY || !process.stdout.isTTY) {
        console.error(
          'Crestodian needs an interactive TTY. Use `daisyclaw crestodian --message "status"` for one command.',
        );
        process.exitCode = 1;
        return;
      }
    ...
    try {
      ...
      process.on("uncaughtException", (error) => {
        ...
        if (isBenignUncaughtExceptionError(error)) {
          console.warn(
            "[daisyclaw] Non-fatal uncaught exception (continuing):",
            formatUncaughtError(error),
          );
          return;
        }
        for (const line of formatCliFailureLines({
          title: "DaisyClaw hit an unexpected runtime error.",
          error,
          argv: normalizedArgv,
        })) {
          console.error(line);
        ...
        for (const message of runFatalErrorHooks({ reason: "uncaught_exception", error })) {
          console.error("[daisyclaw]", message);
        }
