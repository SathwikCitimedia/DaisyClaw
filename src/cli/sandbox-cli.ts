...
const SANDBOX_EXAMPLES = {
  main: [
    ["daisyclaw sandbox list", "List all sandbox containers."],
    ["daisyclaw sandbox list --browser", "List only browser containers."],
    ["daisyclaw sandbox recreate --all", "Recreate all containers."],
    ["daisyclaw sandbox recreate --session main", "Recreate a specific session."],
    ["daisyclaw sandbox recreate --agent mybot", "Recreate agent containers."],
    ["daisyclaw sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["daisyclaw sandbox list", "List all sandbox containers."],
    ["daisyclaw sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["daisyclaw sandbox recreate --all", "Recreate all containers."],
    ["daisyclaw sandbox recreate --session main", "Recreate a specific session."],
    ["daisyclaw sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["daisyclaw sandbox recreate --browser --all", "Recreate only browser containers."],
    ["daisyclaw sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["daisyclaw sandbox explain", "Show effective sandbox config."],
    ["daisyclaw sandbox explain --session agent:main:main", "Explain a specific session."],
    ["daisyclaw sandbox explain --agent work", "Explain an agent sandbox."],
    ["daisyclaw sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.daisyclaw.ai/cli/sandbox")}\n`,
    )
    .action(() => {
