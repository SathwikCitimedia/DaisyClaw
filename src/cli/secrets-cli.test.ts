...
function createConfigureInteractiveResult(options?: {
  ...
  return {
    ...
    preflight: {
      ...
      changedFiles: options?.changed ? ["/tmp/daisyclaw.json"] : [],
      checks: {
        resolvability: true,
...
async function withPlanFile(run: (planPath: string) => Promise<void>) {
  const planPath = path.join(
    os.tmpdir(),
    `daisyclaw-secrets-cli-test-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
  );
...
describe("secrets CLI", () => {
  ...
  it("explains Gateway reload failures without duplicate doctor noise", async () => {
    callGatewayFromCli.mockRejectedValue(
      new Error(
        "gateway closed (1006 abnormal closure). Gateway target: ws://127.0.0.1:18789 Source: local loopback Config: /tmp/daisyclaw.json Bind: loopback Possible causes: - Gateway not yet ready. Run `daisyclaw doctor` for diagnostics.",
      ),
    );
    ...
    expect(runtimeErrors.at(-1)).toContain("daisyclaw gateway status --deep");
