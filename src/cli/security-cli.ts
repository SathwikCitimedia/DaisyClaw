...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["daisyclaw security audit", "Run a local security audit."],
          [
            "daisyclaw security audit --deep",
            "Include best-effort live Gateway probes and plugin-owned security audit collectors.",
          ],
          ["daisyclaw security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "daisyclaw security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          [
            "daisyclaw security audit --auth password --password <password>",
            "Audit a runtime-only password-mode Gateway secret.",
          ],
          ["daisyclaw security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["daisyclaw security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.daisyclaw.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("DaisyClaw security audit"));
      ...
      lines.push(muted(`Run deeper: ${formatCliCommand("daisyclaw security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("daisyclaw security audit --fix")}`));
        if (!fixResult) {
