...
describe("daisyclaw skills install ClawHub GitHub-backed E2E", () => {
  it("installs from the install resolver and reports install telemetry", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-clawhub-cli-e2e-"));
    try {
      const result = await spawnDaisyClaw(["skills", "install", "aiq-deploy", "--global"], {
        cwd: process.cwd(),
        env: {
          ...process.env,
          DAISYCLAW_STATE_DIR: stateDir,
          DAISYCLAW_CONFIG_PATH: path.join(stateDir, "daisyclaw.json"),
          DAISYCLAW_CLAWHUB_URL: registry,
          DAISYCLAW_CLAWHUB_TOKEN: "test-token",
          DAISYCLAW_CLAWHUB_GITHUB_CODELOAD_BASE_URL: registry,
