// Formatting layer for `daisyclaw skills` commands; keeps discovery data separate from terminal UI.
...
function appendClawHubHint(output: string, json?: boolean): string {
  ...
  return `${output}\n\nTip: use \`daisyclaw skills search\`, \`daisyclaw skills install\`, and \`daisyclaw skills update\` for ClawHub-backed skills.`;
}
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("daisyclaw skills list")}\` to see all skills.`
      : "No skills found.";
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendClawHubHint(
      `Skill "${safeRequestedName}" not found. Run \`${formatCliCommand("daisyclaw skills list")}\` to see available skills.`,
      opts.json,
    );
  }
  ...
  if (skill.primaryEnv && skill.missing.env.includes(skill.primaryEnv)) {
    ...
    lines.push(
      `  Save via CLI: ${formatCliCommand(`daisyclaw config set skills.entries.${safeSkillKey}.apiKey YOUR_KEY`)}`,
    );
    lines.push(
      `  Stored in: ${theme.muted("$DAISYCLAW_CONFIG_PATH")} ${theme.muted("(default: ~/.daisyclaw/daisyclaw.json)")}`,
    );
  }
