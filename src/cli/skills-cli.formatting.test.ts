...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-bundled-skills-test-"));
    process.env.DAISYCLAW_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
