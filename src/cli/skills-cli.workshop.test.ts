...
vi.mock("../terminal/links.js", () => ({
  formatDocsLink: () => "docs.daisyclaw.ai/cli/skills",
}));
...
describe("skills workshop cli", () => {
  ...
  beforeEach(async () => {
    testState = await createDaisyClawTestState({
      layout: "state-only",
      prefix: "daisyclaw-skills-cli-workshop-state-",
    });
    mocks.workspaceDir = await tempDirs.make("daisyclaw-skills-cli-workshop-");
  ...
  it("scopes list and inspect to the selected workspace", async () => {
    ...
    mocks.workspaceDir = await tempDirs.make("daisyclaw-skills-cli-workshop-second-");
    await runCommand(["skills", "workshop", "list"]);
