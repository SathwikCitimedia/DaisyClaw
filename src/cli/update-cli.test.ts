...
import type { DaisyClawConfig, ConfigFileSnapshot } from "../config/types.daisyclaw.js";
...
const loadInstalledPluginIndexInstallRecords = vi.fn(
  async (params: { config?: DaisyClawConfig } = {}) => params.config?.plugins?.installs ?? {},
);
...
const launchdUpdateCleanupMocks = vi.hoisted(() => ({
  disableCurrentDaisyClawUpdateLaunchdJob: vi.fn(async () => false),
}));
...
vi.mock("../infra/daisyclaw-root.js", () => ({
  resolveDaisyClawPackageRoot: vi.fn(),
  resolveDaisyClawPackageRootSync: vi.fn(() => process.cwd()),
}));

vi.mock("../config/config.js", () => ({
  assertConfigWriteAllowedInCurrentMode: () => {
    if (process.env.DAISYCLAW_NIX_MODE === "1") {
      throw new Error(
        [
          "Config is managed by Nix (`DAISYCLAW_NIX_MODE=1`), so DaisyClaw treats daisyclaw.json as immutable.",
          "Do not run setup, onboarding, daisyclaw update, plugin install/update/uninstall/enable, doctor repair/token-generation, or config set against this file.",
          "Agent-first Nix setup: https://github.com/daisyclaw/nix-daisyclaw#quick-start",
          "DaisyClaw Nix overview: https://docs.daisyclaw.ai/install/nix",
        ].join("\n"),
      );
    }
  },
...
vi.mock("../utils.js", async (importOriginal) => {
  ...
  return {
    ...
    resolveConfigDir: () => "/tmp/daisyclaw-config",
  };
});
...
vi.mock("../daemon/launchd.js", async (importOriginal) => ({
  ...
  disableCurrentDaisyClawUpdateLaunchdJob:
    launchdUpdateCleanupMocks.disableCurrentDaisyClawUpdateLaunchdJob,
}));
...
const { resolveDaisyClawPackageRoot } = await import("../infra/daisyclaw-root.js");
const {
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/daisyclaw-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as DaisyClawConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/daisyclaw-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveDaisyClawPackageRoot).mockResolvedValue(root);
  ...
  const stripDaisyClawPackageAlias = (spec: string) => {
    const trimmed = spec.trim();
    return trimmed.toLowerCase().startsWith("daisyclaw@")
      ? trimmed.slice("daisyclaw@".length)
      : trimmed;
  };

  const isNpmGitPackageSpec = (spec: string) => {
    const target = stripDaisyClawPackageAlias(spec);
  ...
  const syncPluginCall = (index = 0) => {
    const calls = syncPluginsForUpdateChannel.mock.calls as unknown as Array<
      [{ channel?: string; config?: DaisyClawConfig }]
    >;
  ...
  const npmPluginUpdateCall = (index = 0) => {
    const calls = updateNpmInstalledPlugins.mock.calls as unknown as Array<
      [{ config?: DaisyClawConfig; timeoutMs?: number }]
    >;
    return calls[index]?.[0];
  };
  ...
  const setupConfigMutationWithRetryMock = () => {
    vi.mocked(mutateConfigFileWithRetry).mockImplementation(async (params) => {
      ...
      const nextConfig = structuredClone(snapshot.sourceConfig) as DaisyClawConfig;
  ...
  const expectPackageInstallSpec = (spec: string) => {
    ...
    if (isNpmGitPackageSpec(spec)) {
      ...
      installSpec = path.join(packDir, "daisyclaw-9999.0.0.tgz");
    } else {
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("daisyclaw-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("daisyclaw-updated-root");
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveDaisyClawPackageRoot).mockResolvedValue(process.cwd());
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        if (destination) {
          await fs.writeFile(path.join(destination, "daisyclaw-9999.0.0.tgz"), "packed\n", "utf8");
        }
      }
      return {
        stdout: "",
    ...
    readPackageName.mockResolvedValue("daisyclaw");
    ...
    serviceReadCommand.mockImplementation(async () =>
      (await serviceLoaded()) ? { programArguments: ["daisyclaw", "gateway", "run"] } : null,
    );
    ...
    prepareRestartScript.mockResolvedValue("/tmp/daisyclaw-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "daisyclaw-gateway" }],
      hints: [],
    });
    ...
    checkShellCompletionStatus.mockResolvedValue({
      ...
      cacheExists: false,
      cachePath: "/tmp/daisyclaw-completion.zsh",
    ...
    launchdUpdateCleanupMocks.disableCurrentDaisyClawUpdateLaunchdJob.mockReset();
    launchdUpdateCleanupMocks.disableCurrentDaisyClawUpdateLaunchdJob.mockResolvedValue(false);
  ...
  it("bounds completion cache refresh during update follow-up", async () => {
    const root = createCaseDir("daisyclaw-completion-timeout");
    ...
    expect(call?.[1]).toEqual([path.join(root, "daisyclaw.mjs"), "completion", "--write-state"]);
    expect(call?.[2]?.env?.DAISYCLAW_COMPLETION_SKIP_PLUGIN_COMMANDS).toBe("1");
  ...
  it("disarms legacy launchd updater jobs before refusing mutating updates in Nix mode", async () => {
    await withEnvAsync({ DAISYCLAW_NIX_MODE: "1" }, async () => {
      await expect(updateCommand({ yes: true })).rejects.toThrow("DAISYCLAW_NIX_MODE=1");
    });

    expect(launchdUpdateCleanupMocks.disableCurrentDaisyClawUpdateLaunchdJob).toHaveBeenCalledOnce();
    expect(runGatewayUpdate).not.toHaveBeenCalled();
  ...
  it("logs friendly hint with manual refresh command when completion cache write times out", async () => {
    const root = createCaseDir("daisyclaw-completion-timeout-msg");
    pathExists.mockResolvedValue(true);
    ...
    expect(logOutput).toContain("daisyclaw completion --write-state");
  ...
  it("keeps update completion refresh best-effort when profile install fails", async () => {
    ...
    expect(installCompletion).toHaveBeenCalledWith("zsh", true, "daisyclaw");
    const logOutput = vi
      .mocked(runtimeCapture.log)
      .mock.calls.map((call) => String(call[0]))
      .join("\n");
  ...
  it("respawns into the updated package root before running post-update tasks", async () => {
    ...
    expect(call?.[2]?.env?.DAISYCLAW_UPDATE_IN_PROGRESS).toBe("1");
    expect(call?.[2]?.env?.DAISYCLAW_UPDATE_POST_CORE).toBe("1");
    expect(call?.[2]?.env?.DAISYCLAW_UPDATE_POST_CORE_CHANNEL).toBe("dev");
    expect(call?.[2]?.env?.DAISYCLAW_COMPATIBILITY_HOST_VERSION).toBe("1.0.0");
  ...
  it("finishes package updates when the post-core process writes a result but keeps handles open", async () => {
    ...
    spawn.mockImplementationOnce((_command: unknown, _argv: unknown, options: unknown) => {
      const resultPath = (options as { env?: NodeJS.ProcessEnv }).env
        ?.DAISYCLAW_UPDATE_POST_CORE_RESULT_PATH;
      if (!resultPath) {
  ...
  it("does not restart a stopped managed gateway after post-core plugin errors", async () => {
    const root = createCaseDir("daisyclaw-update");
            ...
            warnings: [
              {
                ...
                guidance: ["Run daisyclaw update repair to retry post-update plugin repair."],
              },
            ],
            sync: {
              changed: false,
  ...
  it("does not carry gateway service markers into the post-core update process", async () => {
    ...
    await withEnvAsync(
      {
        DAISYCLAW_SERVICE_MARKER: "daisyclaw",
        DAISYCLAW_SERVICE_KIND: "gateway",
    ...
    expect(spawnEnv?.DAISYCLAW_SERVICE_MARKER).toBeUndefined();
    expect(spawnEnv?.DAISYCLAW_SERVICE_KIND).toBeUndefined();
  ...
  it("passes pre-update plugin install records into the post-core update process", async () => {
    ...
    const pluginInstallRecords = {
      demo: {
        source: "npm",
        spec: "@daisyclaw/demo@1.0.0",
        installPath: "/tmp/daisyclaw-demo-plugin",
      },
    } as const;
    ...
    spawn.mockImplementationOnce((_node, _argv, options) => {
      ...
      const recordsPath = env?.DAISYCLAW_UPDATE_POST_CORE_INSTALL_RECORDS_PATH;
      const sourceConfigPath = env?.DAISYCLAW_UPDATE_POST_CORE_SOURCE_CONFIG_PATH;
      if (!recordsPath) {
  ...
  it("clears stale npm resolution metadata before post-core downgrade resume", async () => {
    ...
    const pluginInstallRecords = {
      msteams: {
        source: "npm",
        spec: "@daisyclaw/msteams",
        installPath: "/tmp/daisyclaw-msteams-plugin",
        version: "1.0.0",
        resolvedName: "@daisyclaw/msteams",
        resolvedVersion: "1.0.0",
        resolvedSpec: "@daisyclaw/msteams@1.0.0",
  ...
  it("respawns into the updated git root before requested channel persistence", async () => {
    ...
    expect(call?.[2]?.env?.DAISYCLAW_UPDATE_POST_CORE).toBe("1");
    expect(call?.[2]?.env?.DAISYCLAW_UPDATE_POST_CORE_CHANNEL).toBe("dev");
    expect(call?.[2]?.env?.DAISYCLAW_UPDATE_POST_CORE_REQUESTED_CHANNEL).toBe("dev");
    expect(replaceConfigFile).not.toHaveBeenCalled();
    expect(syncPluginsForUpdateChannel).not.toHaveBeenCalled();
    expect(updateNpmInstalledPlugins).not.toHaveBeenCalled();
  });
  ...
  it("keeps downgrade post-update work in the current process", async () => {
    const downgradedRoot = createCaseDir("daisyclaw-downgraded-root");
    setupUpdatedRootRefresh({
  ...
  it("pins the compatibility host version to the downgraded target during current-process post-core plugin convergence (#87914)", async () => {
    const downgradedRoot = createCaseDir("daisyclaw-downgraded-compat-root");
    setupUpdatedRootRefresh({
      gatewayUpdateImpl: async () =>
        makeOkUpdateResult({
          mode: "npm",
    ...
    delete process.env.DAISYCLAW_COMPATIBILITY_HOST_VERSION;
    let hostVersionDuringPluginUpdate: string | undefined = "unset";
    updateNpmInstalledPlugins.mockImplementation(async () => {
      hostVersionDuringPluginUpdate = process.env.DAISYCLAW_COMPATIBILITY_HOST_VERSION;
    ...
    try {
      ...
      expect(process.env.DAISYCLAW_COMPATIBILITY_HOST_VERSION).toBeUndefined();
    } finally {
      delete process.env.DAISYCLAW_COMPATIBILITY_HOST_VERSION;
    }
  });
    ...
    await withEnvAsync(
      {
        ...
        DAISYCLAW_UPDATE_POST_CORE_CHANNEL: "stable",
      },
      async () => {
  ...
  it("post-core resume children exit after writing a plugin update result", async () => {
    const resultDir = createCaseDir("daisyclaw-post-core-result");
    ...
    await withEnvAsync(
      {
        DAISYCLAW_UPDATE_POST_CORE: "1",
        DAISYCLAW_UPDATE_POST_CORE_CHANNEL: "stable",
        DAISYCLAW_UPDATE_POST_CORE_RESULT_PATH: resultPath,
      },
      async () => {
        await updateCommand({ restart: false });
      },
  ...
  it("post-core resume mode uses the parent install records snapshot for missing payload warnings", async () => {
    const resultDir = createCaseDir("daisyclaw-post-core-records");
    ...
    await withEnvAsync(
      {
        DAISYCLAW_UPDATE_POST_CORE: "1",
        DAISYCLAW_UPDATE_POST_CORE_CHANNEL: "stable",
        DAISYCLAW_UPDATE_POST_CORE_INSTALL_RECORDS_PATH: recordsPath,
      },
      async () => {
  ...
  it("post-core resume mode prefers post-doctor disk install records over the stale parent snapshot", async () => {
    const resultDir = createCaseDir("daisyclaw-post-core-disk-records");
    const recordsPath = path.join(resultDir, "plugin-install-records.json");
    await fs.mkdir(resultDir, { recursive: true });
    await fs.writeFile(
      recordsPath,
    ...
    const postDoctorRecords = {
      codex: {
        source: "npm",
        spec: "@daisyclaw/codex@2026.5.17",
  ...
  it("post-core resume mode persists the requested update channel with the updated process", async () => {
    vi.mocked(readConfigFileSnapshot).mockResolvedValue({
      ...
      resolved: { update: { channel: "stable" } } as DaisyClawConfig,
      sourceConfig: { update: { channel: "stable" } } as DaisyClawConfig,
      runtimeConfig: { update: { channel: "stable" } } as DaisyClawConfig,
      config: { update: { channel: "stable" } } as DaisyClawConfig,
      hash: "stable-hash",
    });
    ...
    await withEnvAsync(
      {
        DAISYCLAW_UPDATE_POST_CORE: "1",
        DAISYCLAW_UPDATE_POST_CORE_CHANNEL: "dev",
        DAISYCLAW_UPDATE_POST_CORE_REQUESTED_CHANNEL: "dev",
      },
      async () => {
        await updateCommand({ restart: false });
      },
  ...
  it("uses a fail-closed integrity policy for post-core plugin updates", async () => {
    ...
    await expect(
      onIntegrityDrift({
        pluginId: "demo",
        spec: "@daisyclaw/demo@1.0.0",
        resolvedSpec: "@daisyclaw/demo@1.0.0",
  ...
  it("keeps json update output successful when post-core plugin updates warn", async () => {
          ...
          outcomes: [
            {
              ...
              message:
                proceed === false
                  ? "Failed to update demo: aborted: npm package integrity drift detected for @daisyclaw/demo@1.0.0"
    ...
    expect(pluginWarning(jsonOutput)?.guidance).toEqual([
      "Run daisyclaw update repair to retry post-update plugin repair.",
      "Run daisyclaw plugins inspect demo --runtime --json for details.",
    ]);
  ...
  it("detects missing plugin payloads from persisted records before npm updates", async () => {
    const installPath = createCaseDir("daisyclaw-missing-plugin-payload");
  ...
  it("prints non-fatal plugin warnings in human update output", async () => {
    ...
    expect(logs).toContain("Run daisyclaw update repair to retry post-update plugin repair.");
    expect(logs).toContain("Run daisyclaw plugins inspect demo --runtime --json for details.");
  });
      ...
      outcomes: [
        {
          ...
          status: "skipped",
          message:
            'Disabled "demo" after plugin update failure; DaisyClaw will continue without it. Failed to update demo: registry timeout',
        },
      ],
    });
    vi.mocked(defaultRuntime.writeJson).mockClear();
  ...
  it("preserves fresh-process plugin warning details in parent json output", async () => {
    ...
    spawn.mockImplementationOnce((_node, _argv, options) => {
      ...
      queueMicrotask(() => {
        void (async () => {
          const resultPath = env?.DAISYCLAW_UPDATE_POST_CORE_RESULT_PATH;
          if (resultPath) {
            await fs.writeFile(
              resultPath,
              JSON.stringify({
                ...
                warnings: [
                  {
                    ...
                    message:
                      'Plugin "demo" could not be processed after the core update: Failed to update demo: registry timeout Run daisyclaw update repair to retry post-update plugin repair. Run daisyclaw plugins inspect demo --runtime --json for details.',
                    guidance: [
                      "Run daisyclaw update repair to retry post-update plugin repair.",
                      "Run daisyclaw plugins inspect demo --runtime --json for details.",
                    ],
  ...
  it.each([
    {
      ...
      assert: () => {
        ...
        expect(logs.join("\n")).toContain("DaisyClaw update status");
      },
    },
    {
      name: "json output",
  ...
  it("parses update status --json as the subcommand option", async () => {
    ...
    await program.parseAsync(["node", "daisyclaw", "update", "status", "--json"]);
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as DaisyClawConfig,
        });
      },
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expectPackageInstallSpec("daisyclaw@latest");
      }
  ...
  it("allows package updates from inherited gateway service env when the managed gateway is not running", async () => {
    mockPackageInstallStatus(createCaseDir("daisyclaw-update"));
    ...
    expect(defaultRuntime.error).not.toHaveBeenCalledWith(
      [
        ...
        "That path replaces the active DaisyClaw dist tree while the live gateway may still lazy-load old chunks.",
        "Run `daisyclaw update` from a shell outside the gateway service, or stop the gateway service first and then update.",
      ].join("\n"),
    );
    expectPackageInstallSpec("daisyclaw@latest");
  });
  ...
  it("refuses package updates from inherited gateway service env when --no-restart leaves the gateway running", async () => {
    ...
    serviceReadCommand.mockResolvedValue({
      programArguments: ["daisyclaw", "gateway", "run"],
      environment: {
        DAISYCLAW_SERVICE_MARKER: "daisyclaw",
        DAISYCLAW_SERVICE_KIND: "gateway",
      },
  ...
  it("refuses package updates from inside the active gateway process tree", async () => {
    ...
    expect(errors.join("\n")).toContain(
      "daisyclaw update detected it is running inside the gateway process tree.",
    );
  ...
  it("blocks package updates when the target requires a newer Node runtime", async () => {
    ...
    expect(errors.join("\n")).toContain("Node ");
    expect(errors.join("\n")).toContain(
      "Bare `npm i -g daisyclaw` can silently install an older compatible release.",
    );
  });
  ...
  it.each([
    ...
    },
    {
      ...
      expectedSpec: "github:daisyclaw/daisyclaw#main",
    },
    {
      name: "explicit git package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("daisyclaw-update"));
        await updateCommand({ yes: true, tag: "github:daisyclaw/daisyclaw#main" });
      },
      expectedSpec: "github:daisyclaw/daisyclaw#main",
    },
    {
      ...
      run: async () => {
        mockPackageInstallStatus(createCaseDir("daisyclaw-update"));
        await updateCommand({ yes: true, tag: "DaisyClaw@github:daisyclaw/daisyclaw#main" });
      },
      expectedSpec: "DaisyClaw@github:daisyclaw/daisyclaw#main",
    },
    {
      name: "full git URL package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("daisyclaw-update"));
        await updateCommand({ yes: true, tag: "https://github.com/daisyclaw/daisyclaw.git#main" });
      },
      expectedSpec: "https://github.com/daisyclaw/daisyclaw.git#main",
    },
    {
      name: "hosted GitHub URL package spec without git suffix",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("daisyclaw-update"));
        await updateCommand({ yes: true, tag: "https://github.com/daisyclaw/daisyclaw#main" });
      },
      expectedSpec: "https://github.com/daisyclaw/daisyclaw#main",
    },
    {
      ...
      run: async () => {
        ...
        await updateCommand({
          yes: true,
          tag: "daisyclaw@https://github.com/daisyclaw/daisyclaw#main",
        });
      },
      expectedSpec: "https://github.com/daisyclaw/daisyclaw#main",
    },
    {
      ...
      run: async () => {
        mockPackageInstallStatus(createCaseDir("daisyclaw-update"));
        await updateCommand({ yes: true, tag: "daisyclaw/daisyclaw#main" });
      },
      expectedSpec: "daisyclaw/daisyclaw#main",
    },
    {
      name: "SCP-style SSH package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("daisyclaw-update"));
        await updateCommand({ yes: true, tag: "git@github.com:daisyclaw/daisyclaw.git#main" });
      },
      expectedSpec: "git@github.com:daisyclaw/daisyclaw.git#main",
    },
    {
      name: "DAISYCLAW_UPDATE_PACKAGE_SPEC override",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("daisyclaw-update"));
        await withEnvAsync(
          { DAISYCLAW_UPDATE_PACKAGE_SPEC: "http://10.211.55.2:8138/daisyclaw-next.tgz" },
          async () => {
            await updateCommand({ yes: true, tag: "latest" });
          },
        );
      },
      expectedSpec: "http://10.211.55.2:8138/daisyclaw-next.tgz",
    },
  ] as const)(
  ...
  it("fails package updates when the installed correction version does not match the requested target", async () => {
    ...
    const pkgRoot = path.join(nodeModules, "daisyclaw");
    mockPackageInstallStatus(tempDir);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "daisyclaw", version: "2026.3.23" }),
      "utf-8",
    );
  ...
  it("stops package post-update work when staged npm install verification fails", async () => {
    const tempDir = await createTrackedTempDir("daisyclaw-update-staged-fail-");
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "daisyclaw", version: "2026.4.20" }),
      "utf-8",
    );
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      ...
      if (
        ...
        const stageRoot = path.join(stagePrefix, "lib", "node_modules", "daisyclaw");
        await fs.mkdir(path.join(stageRoot, "dist"), { recursive: true });
        await fs.writeFile(
          path.join(stageRoot, "package.json"),
          JSON.stringify({ name: "daisyclaw", version: "2026.4.25" }),
          "utf-8",
        );
  ...
  it("marks package post-update doctor as update-in-progress", async () => {
    const tempDir = await createTrackedTempDir("daisyclaw-update-package-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "daisyclaw");
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "daisyclaw", version: "2026.4.21" }),
      "utf-8",
    );
    ...
    expect(
      (doctorCall?.[1].env as NodeJS.ProcessEnv | undefined)?.DAISYCLAW_UPDATE_IN_PROGRESS,
    ).toBe("1");
  });

  it("runs package post-update doctor from the verified package root after a staged swap", async () => {
    const tempDir = await createTrackedTempDir("daisyclaw-update-staged-doctor-");
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      ...
      if (argv[0] === "npm" && argv[1] === "i" && argv.includes("--prefix")) {
        ...
        await fs.writeFile(
          ...
          JSON.stringify({ name: "daisyclaw", version: "2026.5.14" }),
          "utf-8",
        );
    ...
    expect(
      (doctorCall?.[1].env as NodeJS.ProcessEnv | undefined)?.DAISYCLAW_COMPATIBILITY_HOST_VERSION,
    ).toBe("2026.5.14");
    expect(defaultRuntime.exit).not.toHaveBeenCalledWith(1);
  });

  it("stops a running managed gateway before package replacement", async () => {
    const tempDir = await createTrackedTempDir("daisyclaw-update-stop-service-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "daisyclaw");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    const serviceStopCall = serviceStop.mock.calls[0]?.[0] as
      ...
      | undefined;
    expect(serviceStopCall?.env?.DAISYCLAW_SERVICE_MARKER).toBe("daisyclaw");
    expect(serviceStopCall?.env?.DAISYCLAW_SERVICE_KIND).toBe("gateway");
  ...
  it("stops a running managed git gateway when wrapper commands hide the service root", async () => {
    const wrapperPath = path.join(
      createCaseDir("daisyclaw-update-wrapper-service"),
      "gateway-wrapper",
    );
    serviceReadCommand.mockResolvedValue({
  ...
  it("stops a managed gateway rooted at the git checkout when switching package installs to dev", async () => {
    const packageRoot = createCaseDir("daisyclaw-update-package-root");
    const gitRoot = await createTrackedTempDir("daisyclaw-update-git-service-root-");
    ...
    await withEnvAsync({ DAISYCLAW_GIT_DIR: gitRoot }, async () => {
  ...
  it("stops a managed gateway rooted at the package install when switching package installs to dev", async () => {
    const packageRoot = await createTrackedTempDir("daisyclaw-update-package-service-root-");
  ...
  it("does not stop or restart a managed gateway owned by another git checkout", async () => {
    const otherRoot = await createTrackedTempDir("daisyclaw-update-other-service-root-");
  ...
  it("keeps managed service stop output off stdout during json package updates", async () => {
    const tempDir = await createTrackedTempDir("daisyclaw-update-json-stop-service-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "daisyclaw");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    ...
    serviceStop.mockImplementationOnce(async (params: { stdout?: NodeJS.WritableStream }) => {
      params.stdout?.write("Stopped systemd service: daisyclaw-gateway.service\n");
    });
    pathExists.mockImplementation(async (candidate: string) => {
      try {
        await fs.access(candidate);
  ...
  it("disarms legacy launchd updater jobs before stopping the gateway", async () => {
    const tempDir = await createTrackedTempDir("daisyclaw-update-launchd-loop-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "daisyclaw");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    launchdUpdateCleanupMocks.disableCurrentDaisyClawUpdateLaunchdJob.mockResolvedValue(true);
    pathExists.mockImplementation(async (candidate: string) => {
      try {
        await fs.access(candidate);
        return true;
    ...
    const cleanupOrder =
      launchdUpdateCleanupMocks.disableCurrentDaisyClawUpdateLaunchdJob.mock.invocationCallOrder[0];
  ...
  it("refreshes package installs even when the current version already matches the target", async () => {
    const tempDir = await createTrackedTempDir("daisyclaw-update-current-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "daisyclaw");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "daisyclaw", version: "2026.4.23" }),
      "utf-8",
    );
    await fs.writeFile(entryPath, "export {};\n", "utf-8");
    for (const relativePath of TEST_BUNDLED_RUNTIME_SIDECAR_PATHS) {
    ...
    expect(postCoreSpawn?.[2].env?.DAISYCLAW_UPDATE_POST_CORE).toBe("1");
    expect(postCoreSpawn?.[2].env?.DAISYCLAW_UPDATE_POST_CORE_CHANNEL).toBe("stable");
    expect(updateNpmInstalledPlugins).not.toHaveBeenCalled();
    expect(
      vi
        .mocked(defaultRuntime.log)
  ...
  it("retries package updates without optional deps when npm global update fails", async () => {
    const tempDir = await createTrackedTempDir("daisyclaw-update-optional-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "daisyclaw");
    mockPackageInstallStatus(pkgRoot);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "daisyclaw", version: "1.0.0" }),
      "utf-8",
    );
  ...
  it("uses the owning npm binary for package updates when PATH npm points elsewhere", async () => {
    ...
    const pkgRoot = path.join(brewRoot, "daisyclaw");
    ...
    const installCall = vi
      ...
      .mock.calls.find(
        ([argv]) =>
          ...
          argv[1] === "i" &&
          argv[2] === "-g" &&
          argv.includes("daisyclaw@latest"),
      );
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("daisyclaw-update");
    const localAppData = createCaseDir("daisyclaw-localappdata");
  ...
  it("warns when a package update targets a managed service root outside the shell root", async () => {
    const shellRoot = createCaseDir("daisyclaw-shell-root");
    const serviceRoot = await createTrackedTempDir("daisyclaw-service-root-");
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "daisyclaw", version: "2026.5.18" }),
      "utf-8",
    );
    ...
    expect(logs).toContain(
      `Shell DaisyClaw root differs from the managed gateway service root: ${shellRoot}`,
    );
    expect(logs).toContain("make sure `daisyclaw` on PATH resolves to the managed service root");
  ...
  it("runs managed service package follow-up commands with the service Node", async () => {
    const shellRoot = createCaseDir("daisyclaw-shell-root");
    const servicePrefix = await createTrackedTempDir("daisyclaw-service-prefix-");
    const nodeModules = path.join(servicePrefix, "lib", "node_modules");
    const serviceRoot = path.join(nodeModules, "daisyclaw");
  ...
  it("uses the managed service Node when package roots match but node binaries differ", async () => {
    const root = createCaseDir("daisyclaw-same-root");
  ...
  it("uses the managed service Node for follow-up commands when roots match but nodes differ", async () => {
    const servicePrefix = await createTrackedTempDir("daisyclaw-service-prefix-");
    const nodeModules = path.join(servicePrefix, "lib", "node_modules");
    const root = path.join(nodeModules, "daisyclaw");
    const serviceNode = path.join(servicePrefix, "bin", "node");
    const serviceNpm = path.join(servicePrefix, "bin", "npm");
    const entrypoint = path.join(root, "dist", "index.js");
    await fs.mkdir(path.dirname(entrypoint), { recursive: true });
  ...
  it("pins package install to the service root when nodes differ and no owning npm exists at the prefix", async () => {
    const servicePrefix = await createTrackedTempDir("daisyclaw-no-npm-prefix-");
    const nodeModules = path.join(servicePrefix, "lib", "node_modules");
    const root = path.join(nodeModules, "daisyclaw");
    const serviceNode = path.join(servicePrefix, "bin", "node");
    const entrypoint = path.join(root, "dist", "index.js");
  ...
  it("does not repair legacy config during a dry run", async () => {
    ...
    expect(replaceConfigFile).not.toHaveBeenCalled();
    expect(runCommandWithTimeout).not.toHaveBeenCalled();
    expect(launchdUpdateCleanupMocks.disableCurrentDaisyClawUpdateLaunchdJob).not.toHaveBeenCalled();
    expect(defaultRuntime.exit).toHaveBeenCalledWith(1);
  });
  ...
  it("refreshes post-doctor config before post-update plugin sync", async () => {
    const tempDir = createCaseDir("daisyclaw-update");
    mockPackageInstallStatus(tempDir);
    const preUpdateConfig = { update: { channel: "stable" } } as DaisyClawConfig;
    ...
    syncPluginsForUpdateChannel.mockImplementation(async ({ config }) => ({
      ...
      config: {
        ...
        plugins: {
          ...config.plugins,
          load: { paths: ["/tmp/daisyclaw-updated-plugin"] },
        },
      },
      summary: {
        switchedToBundled: [],
    ...
    const syncConfig = syncPluginCall()?.config as
      | (DaisyClawConfig & { meta?: { lastTouchedVersion?: string } })
      | undefined;
    const lastWrite = lastReplaceConfigCall() as
      | {
          baseHash?: string;
          nextConfig?: DaisyClawConfig & { meta?: { lastTouchedVersion?: string } };
        }
      | undefined;
  ...
  it("restores pre-update channels when post-core resume sees post-doctor config without them", async () => {
    const tempDir = createCaseDir("daisyclaw-update");
    const configPath = path.join(tempDir, "daisyclaw.json");
    const preUpdateConfig = {
      update: { channel: "stable" },
      channels: {
        whatsapp: {
    ...
    await withEnvAsync(
      {
        DAISYCLAW_UPDATE_POST_CORE: "1",
        DAISYCLAW_UPDATE_POST_CORE_CHANNEL: "stable",
        DAISYCLAW_UPDATE_POST_CORE_STARTED_AT_MS: String(updateStartedAtMs),
      },
      async () => {
        await updateCommand({ yes: true, restart: false });
      },
  ...
  it("persists authored channel values when post-core restore input is resolved", async () => {
    ...
    await withEnvAsync(
      {
        DAISYCLAW_UPDATE_POST_CORE: "1",
        DAISYCLAW_UPDATE_POST_CORE_CHANNEL: "stable",
        DAISYCLAW_UPDATE_POST_CORE_SOURCE_CONFIG_PATH: sourceConfigPath,
      },
      async () => {
        await updateCommand({ yes: true, restart: false });
      },
    ...
    const syncConfig = syncPluginCall()?.config as
      | (DaisyClawConfig & { channels?: { whatsapp?: { token?: string } } })
      | undefined;
    const lastWrite = lastReplaceConfigCall() as
      | {
          nextConfig?: DaisyClawConfig & {
  ...
  it("persists channel and runs post-update work after switching from package to git", async () => {
    const tempDir = createCaseDir("daisyclaw-update");
    const gitRoot = path.join(tempDir, "..", "daisyclaw");
    ...
    const syncCall = syncPluginCall() as
      | { channel?: string; config?: DaisyClawConfig; workspaceDir?: string }
      | undefined;
  ...
  it("explains why git updates cannot run with edited files", async () => {
    ...
    expect(logs.join("\n")).toContain(
      "Commit, stash, or discard the local changes, then rerun `daisyclaw update`.",
    );
    expect(serviceStop).not.toHaveBeenCalled();
    expect(defaultRuntime.exit).toHaveBeenCalledWith(0);
  });
  ...
  it("tries the updated install restart when package service refresh fails", async () => {
    const updatedRoot = createCaseDir("daisyclaw-updated-root");
  ...
  it("accepts same-version refresh failure recovery when the managed service restarts", async () => {
    ...
    await fs.writeFile(
      updatedPackageJson,
      JSON.stringify({ name: "daisyclaw", version: "2026.4.24" }),
      "utf8",
    );
    setupUpdatedRootRefresh({
      entrypoints: [updatedEntrypoint],
  ...
  it("rejects same-version refresh failure recovery from a stale service definition", async () => {
    const oldRoot = createCaseDir("daisyclaw-old-root");
    const updatedRoot = createCaseDir("daisyclaw-updated-root");
  ...
  it("writes the control-plane update sentinel after managed package restart health passes", async () => {
    const stateDir = await createTrackedTempDir("daisyclaw-update-sentinel-state-");
    const metaDir = await createTrackedTempDir("daisyclaw-update-sentinel-meta-");
  ...
  it.each([
    ...
    },
    {
        ...
        await withEnvAsync(
          {
            ...
            DAISYCLAW_CONFIG_PATH: "./config/daisyclaw.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedEnv: () => ({
        DAISYCLAW_STATE_DIR: path.resolve("./state"),
        DAISYCLAW_CONFIG_PATH: path.resolve("./config/daisyclaw.json"),
      }),
      assertExtra: () => {
        expect(runDaemonInstall).not.toHaveBeenCalled();
      },
    },
    {
      ...
      expectedEnv: (context?: { originalCwd: string }) => ({
        DAISYCLAW_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
      }),
      assertExtra: () => {
        expect(runDaemonInstall).not.toHaveBeenCalled();
      },
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ DAISYCLAW_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.DAISYCLAW_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("marks the whole update command as update-in-progress", async () => {
    await withEnvAsync({ DAISYCLAW_UPDATE_IN_PROGRESS: undefined }, async () => {
      ...
      vi.mocked(runGatewayUpdate).mockImplementationOnce(async () => {
        observedUpdateEnv = process.env.DAISYCLAW_UPDATE_IN_PROGRESS;
  ...
  it("updateFinalizeCommand runs doctor and plugin convergence with full update env", async () => {
    await withEnvAsync(
      {
        DAISYCLAW_UPDATE_IN_PROGRESS: undefined,
        DAISYCLAW_UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR: undefined,
        DAISYCLAW_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE: undefined,
      },
      async () => {
        ...
        expect(doctorEnv?.DAISYCLAW_UPDATE_IN_PROGRESS).toBe("1");
        expect(doctorEnv?.DAISYCLAW_UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR).toBe("1");
        expect(doctorEnv?.DAISYCLAW_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE).toBe("1");
        expect(process.env.DAISYCLAW_UPDATE_IN_PROGRESS).toBeUndefined();
        expect(process.env.DAISYCLAW_UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR).toBeUndefined();
        expect(process.env.DAISYCLAW_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE).toBeUndefined();
  ...
  it("updateFinalizeCommand repairs doctor by default and refreshes plugin state after doctor", async () => {
    ...
    syncPluginsForUpdateChannel.mockImplementationOnce(
      async (params: { config?: DaisyClawConfig }) => ({
        changed: true,
  ...
  it("updateFinalizeCommand reapplies requested channel against post-doctor config", async () => {
    const preDoctorConfig = { update: { channel: "stable" } } as DaisyClawConfig;
    const postDoctorConfig = { update: { channel: "beta" } } as DaisyClawConfig;
    const preDoctorSnapshot: ConfigFileSnapshot = {
      ...baseSnapshot,
      sourceConfig: preDoctorConfig,
      resolved: preDoctorConfig,
  ...
  it.each([
    ...
    },
    {
      ...
      requireTty: false,
      expectedError:
        "Update wizard requires a TTY. Use `daisyclaw update --channel <stable|beta|dev>` instead.",
    },
  ] as const)(
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("daisyclaw-update-wizard");
    await withEnvAsync({ DAISYCLAW_GIT_DIR: tempDir }, async () => {
      setTty(true);
  ...
  it("uses ~/daisyclaw as the default dev checkout directory", async () => {
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/tmp/oc-home");
    try {
      await withEnvAsync(
        {
          HOME: undefined,
          DAISYCLAW_GIT_DIR: undefined,
          DAISYCLAW_HOME: undefined,
          USERPROFILE: undefined,
        },
        async () => {
          expect(resolveGitInstallDir()).toBe(path.posix.join("/tmp/oc-home", "daisyclaw"));
        },
      );
    } finally {
  ...
  it("uses DAISYCLAW_HOME for the default dev checkout directory", async () => {
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/tmp/oc-home");
    try {
      await withEnvAsync(
        { DAISYCLAW_GIT_DIR: undefined, DAISYCLAW_HOME: "/srv/daisyclaw-home" },
        async () => {
          expect(resolveGitInstallDir()).toBe(path.posix.join("/srv/daisyclaw-home", "daisyclaw"));
        },
      );
    } finally {
      homedirSpy.mockRestore();
  ...
  it("creates the parent directory before cloning the default dev checkout", async () => {
    const root = await createTrackedTempDir("daisyclaw-update-home-");
    const home = path.join(root, "custom-daisyclaw-home");
    const checkoutDir = path.join(home, "daisyclaw");

    await withEnvAsync({ DAISYCLAW_GIT_DIR: undefined, DAISYCLAW_HOME: home }, async () => {
    ...
    expect(cloneCall?.[0]).toEqual([
      "git",
      "clone",
      "https://github.com/daisyclaw/daisyclaw.git",
      checkoutDir,
    ]);
  });
});
