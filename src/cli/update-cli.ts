// Commander wiring for `daisyclaw update`, its status/finalize subcommands, and help text.
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["daisyclaw update repair", "Rerun post-update doctor and plugin convergence."],
          ["daisyclaw update repair --channel beta", "Repair against the beta update channel."],
          ["daisyclaw update repair --json", "JSON output for automation."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          ...
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.daisyclaw.ai/cli/update")}`,
    )
...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update DaisyClaw and inspect update channel status")
    .option("--json", "Output result as JSON", false)
    ...
    .addHelpText("after", () => {
      const examples = [
        ["daisyclaw update", "Update a source checkout (git)"],
        ["daisyclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["daisyclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["daisyclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["daisyclaw update --tag main", "One-off package update from GitHub main"],
        ["daisyclaw update --dry-run", "Preview actions without changing anything"],
        ["daisyclaw update --no-restart", "Update without restarting the service"],
        ["daisyclaw update --json", "Output result as JSON"],
        ["daisyclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["daisyclaw update repair", "Repair stranded post-update plugin state"],
        ["daisyclaw update wizard", "Interactive update wizard"],
        ["daisyclaw --update", "Shorthand for daisyclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run daisyclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.daisyclaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["daisyclaw update status", "Show channel + version status."],
          ["daisyclaw update status --json", "JSON output."],
          ["daisyclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
