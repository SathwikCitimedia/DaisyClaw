...
import { auditDaisyClawPeerDependencyLink } from "../../plugins/plugin-peer-link.js";
...
export async function runPluginPayloadSmokeCheck(params: {
  ...
  for (const [pluginId, record] of Object.entries(params.records).toSorted(([a], [b]) =>
    ...
    if (manifestDeclaresDaisyClawPeer(manifest)) {
      const peerIssue = await auditDaisyClawPeerDependencyLink({
      ...
      if (peerIssue) {
        failures.push({
          ...
          detail: `Plugin declares peerDependency "daisyclaw" but peer link audit failed: ${peerIssue.reason}.`,
        });
      }
    }
      ...
      failures.push({
        ...
        detail: `Plugin extension entry validation failed: ${
          extensionResolution.status === "invalid"
            ...
            : "package.json daisyclaw.extensions is empty"
        }`,
      });
      continue;
...
function manifestDeclaresDaisyClawPeer(manifest: PackageManifest): boolean {
  const peerDependencies = (manifest as { peerDependencies?: unknown }).peerDependencies;
  return (
    typeof peerDependencies === "object" &&
    peerDependencies !== null &&
    !Array.isArray(peerDependencies) &&
    typeof (peerDependencies as Record<string, unknown>).daisyclaw === "string"
  );
}
