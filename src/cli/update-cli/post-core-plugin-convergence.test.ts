...
const mocks = vi.hoisted(() => ({
  ...
  relinkDaisyClawPeerDependenciesInManagedNpmRoot: vi.fn(),
...
vi.mock("../../plugins/plugin-peer-link.js", () => ({
  relinkDaisyClawPeerDependenciesInManagedNpmRoot:
    mocks.relinkDaisyClawPeerDependenciesInManagedNpmRoot,
}));
...
describe("runPostCorePluginConvergence", () => {
  ...
  beforeEach(() => {
    ...
    mocks.relinkDaisyClawPeerDependenciesInManagedNpmRoot.mockResolvedValue({
      checked: 0,
      attempted: 0,
      repaired: 0,
      skipped: 0,
  ...
  function makeTempDir(): string {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-post-core-convergence-"));
    tempDirs.push(dir);
    return dir;
  }
  ...
  it("calls repair with DAISYCLAW_UPDATE_POST_CORE_CONVERGENCE=1 set", async () => {
    const cfg = { plugins: { entries: {} } } as unknown as DaisyClawConfig;
    await runPostCorePluginConvergence({
      cfg,
      env: { DAISYCLAW_UPDATE_IN_PROGRESS: "1" },
    });
    ...
    expect(mocks.repairMissingConfiguredPluginInstalls).toHaveBeenCalledWith({
      ...
      env: {
        ...
        DAISYCLAW_COMPATIBILITY_HOST_VERSION: VERSION,
  ...
  it("uses the candidate runtime version over a stale inherited host version", async () => {
    ...
    await runPostCorePluginConvergence({
      cfg,
      env: { DAISYCLAW_COMPATIBILITY_HOST_VERSION: "2026.5.12" },
    });
    expect(mocks.repairMissingConfiguredPluginInstalls).toHaveBeenCalledWith({
      cfg,
      env: {
  ...
  it("returns the post-repair install records so callers can re-seed pluginConfig", async () => {
    ...
    const result = await runPostCorePluginConvergence({
      cfg: { plugins: { entries: { discord: { enabled: true } } } } as unknown as DaisyClawConfig,
      env: {},
    });
  ...
  it("repairs managed npm daisyclaw peer links in every managed npm project before payload smoke checks", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValue({
      changes: [],
      warnings: [],
    ...
    mocks.listManagedPluginNpmRoots.mockResolvedValue([
      "/tmp/daisyclaw-state/npm",
      "/tmp/daisyclaw-state/npm/projects/codex",
    ]);
    mocks.relinkDaisyClawPeerDependenciesInManagedNpmRoot
    ...
    const result = await runPostCorePluginConvergence({
      cfg: { plugins: { entries: { codex: { enabled: true } } } } as unknown as DaisyClawConfig,
      env: { DAISYCLAW_STATE_DIR: "/tmp/daisyclaw-state" },
    });

    expect(mocks.relinkDaisyClawPeerDependenciesInManagedNpmRoot).toHaveBeenNthCalledWith(1, {
      npmRoot: "/tmp/daisyclaw-state/npm",
      logger: {},
    });
    expect(mocks.relinkDaisyClawPeerDependenciesInManagedNpmRoot).toHaveBeenNthCalledWith(2, {
      npmRoot: "/tmp/daisyclaw-state/npm/projects/codex",
      logger: {},
    });
    expect(result.changes).toEqual([
      "Repaired DaisyClaw host peer link(s) for 1 managed npm plugin package(s).",
    ]);
    expect(
      mocks.relinkDaisyClawPeerDependenciesInManagedNpmRoot.mock.invocationCallOrder[0],
  ...
  it("keeps repair warnings nonblocking with actionable guidance", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValue({
      changes: [],
      warnings: [
        'Failed to install missing configured plugin "discord" from @daisyclaw/discord: ENETUNREACH.',
      ],
      records: {},
    });
    const result = await runPostCorePluginConvergence({
    ...
    expect(result.warnings).toStrictEqual([
      {
        ...
        message:
          'Failed to install missing configured plugin "discord" from @daisyclaw/discord: ENETUNREACH.',
        guidance: ["Run `daisyclaw update repair` to retry plugin repair."],
      },
    ]);
  });
  ...
  it("keeps failed configured-plugin repair fetches nonblocking", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValue({
      changes: [],
      warnings: [
        'Failed to install missing configured plugin "matrix" from clawhub:@daisyclaw/matrix@beta: ClawHub ClawPack download for @daisyclaw/matrix@2026.6.1-beta.1 body stalled after 30000ms.',
      ],
    ...
    expect(result.warnings).toStrictEqual([
      {
        ...
        guidance: [
          "Run `daisyclaw update repair` to retry plugin repair.",
          "Run `daisyclaw plugins inspect brave --runtime --json` for details.",
        ],
      },
    ]);
  });
      ...
      warnings: [
        {
          ...
          guidance: ["Run `daisyclaw update repair`."],
        },
        {
