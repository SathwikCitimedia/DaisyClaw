...
import { relinkDaisyClawPeerDependenciesInManagedNpmRoot } from "../../plugins/plugin-peer-link.js";
...
const REPAIR_GUIDANCE = "Run `daisyclaw update repair` to retry plugin repair.";
const inspectGuidance = (pluginId: string) =>
  `Run \`daisyclaw plugins inspect ${pluginId} --runtime --json\` for details.`;

async function repairManagedNpmDaisyClawPeerLinks(params: {
  ...
  try {
    ...
    const message = `Failed to repair managed npm DaisyClaw host peer links: ${err instanceof Error ? err.message : String(err)}`;
    return {
      changes: [],
      warnings: [
        {
...
export async function runPostCorePluginConvergence(params: {
  ...
  const peerLinkRepair = await repairManagedNpmDaisyClawPeerLinks({ env });
