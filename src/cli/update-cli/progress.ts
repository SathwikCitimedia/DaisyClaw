...
const STEP_LABELS: Record<string, string> = {
  ...
  "daisyclaw doctor entry": "Checking doctor entrypoint",
  "daisyclaw doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (isGlobalPackageInstallStep && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g daisyclaw@latest");
    hints.push(
      "System install outline: daisyclaw gateway stop -> sudo <system-npm> i -g daisyclaw@latest -> daisyclaw gateway install --force -> daisyclaw gateway restart.",
    );
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g daisyclaw@latest --omit=optional");
  }
