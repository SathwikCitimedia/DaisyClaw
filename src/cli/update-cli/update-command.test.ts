...
describe("resolveGatewayInstallEntrypointCandidates", () => {
  it("prefers index.js before legacy entry.js", () => {
    expect(resolveGatewayInstallEntrypointCandidates("/tmp/daisyclaw-root")).toEqual([
      path.join("/tmp/daisyclaw-root", "dist", "index.js"),
      path.join("/tmp/daisyclaw-root", "dist", "index.mjs"),
      path.join("/tmp/daisyclaw-root", "dist", "entry.js"),
      path.join("/tmp/daisyclaw-root", "dist", "entry.mjs"),
    ]);
  });
});
  ...
  it("uses the managed service port ahead of the caller environment", () => {
    expect(
      resolveUpdatedGatewayRestartPort({
        ...
        processEnv: { DAISYCLAW_GATEWAY_PORT: "19001" },
        serviceEnv: { DAISYCLAW_GATEWAY_PORT: "19002" },
      }),
    ).toBe(19002);
  });
...
describe("resolvePostUpdateServiceStateReadEnv", () => {
  it("keeps package restart preparation anchored to the pre-update service env", () => {
    const processEnv = {
      DAISYCLAW_STATE_DIR: "/source/state",
      DAISYCLAW_CONFIG_PATH: "/source/daisyclaw.json",
    } as NodeJS.ProcessEnv;
    const prePackageServiceEnv = {
      DAISYCLAW_STATE_DIR: "/managed/state",
      DAISYCLAW_CONFIG_PATH: "/managed/daisyclaw.json",
    } as NodeJS.ProcessEnv;
  ...
  it("keeps git updates tied to the caller environment", () => {
    const processEnv = { DAISYCLAW_STATE_DIR: "/source/state" } as NodeJS.ProcessEnv;
    const prePackageServiceEnv = { DAISYCLAW_STATE_DIR: "/managed/state" } as NodeJS.ProcessEnv;
  ...
  it("uses the managed service environment for git updates stopped by this updater", () => {
    const processEnv = { DAISYCLAW_STATE_DIR: "/source/state" } as NodeJS.ProcessEnv;
    const preManagedServiceEnv = { DAISYCLAW_STATE_DIR: "/managed/state" } as NodeJS.ProcessEnv;
...
describe("resolvePostInstallDoctorEnv", () => {
  it("uses the managed service profile paths for post-install doctor", () => {
    const env = resolvePostInstallDoctorEnv({
      invocationCwd: "/srv/daisyclaw",
      baseEnv: {
        PATH: "/bin",
        DAISYCLAW_STATE_DIR: "/wrong/state",
        DAISYCLAW_CONFIG_PATH: "/wrong/daisyclaw.json",
        DAISYCLAW_PROFILE: "wrong",
      },
      serviceEnv: {
        DAISYCLAW_STATE_DIR: "daemon-state",
        DAISYCLAW_CONFIG_PATH: "daemon-state/daisyclaw.json",
    ...
    expect(env.DAISYCLAW_STATE_DIR).toBe(path.join("/srv/daisyclaw", "daemon-state"));
    expect(env.DAISYCLAW_CONFIG_PATH).toBe(
      path.join("/srv/daisyclaw", "daemon-state", "daisyclaw.json"),
    );
    expect(env.DAISYCLAW_PROFILE).toBe("work");
  });
  ...
  it("keeps the caller env when no managed service env is available", () => {
    const env = resolvePostInstallDoctorEnv({
      baseEnv: {
        PATH: "/bin",
        DAISYCLAW_STATE_DIR: "/caller/state",
        DAISYCLAW_PROFILE: "caller",
      },
    });

    expect(env.PATH).toBe("/bin");
    expect(env.NODE_DISABLE_COMPILE_CACHE).toBe("1");
    expect(env.DAISYCLAW_STATE_DIR).toBe("/caller/state");
    expect(env.DAISYCLAW_PROFILE).toBe("caller");
  });
});

describe("collectMissingPluginInstallPayloads", () => {
  it("reports tracked npm install records whose package payload is absent", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-update-plugin-payload-"));
    const presentDir = path.join(tmpDir, "state", "npm", "node_modules", "@daisyclaw", "present");
    const missingDir = path.join(tmpDir, "state", "npm", "node_modules", "@daisyclaw", "missing");
    ...
    try {
      ...
      await fs.writeFile(path.join(presentDir, "package.json"), '{"name":"@daisyclaw/present"}\n');
      ...
      await expect(
        collectMissingPluginInstallPayloads({
          ...
          records: {
            present: {
              source: "npm",
              spec: "@daisyclaw/present@beta",
              installPath: presentDir,
            },
            missing: {
              source: "npm",
              spec: "@daisyclaw/missing@beta",
              installPath: missingDir,
            },
            "no-package-json": {
              source: "npm",
              spec: "@daisyclaw/no-package-json@beta",
              installPath: noPackageJsonDir,
            },
            "missing-install-path": {
              source: "npm",
              spec: "@daisyclaw/missing-install-path@beta",
            },
            local: {
              source: "path",
  ...
  it("keeps disabled trusted official npm records eligible for payload repair when requested", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-update-plugin-payload-"));
    const missingDir = path.join(tmpDir, "state", "npm", "node_modules", "@daisyclaw", "codex");
    try {
      await expect(
        collectMissingPluginInstallPayloads({
          env: { HOME: tmpDir } as NodeJS.ProcessEnv,
...
describe("formatPostUpdateGatewayRecoveryInstructions", () => {
  ...
  it("uses systemd wording on Linux instead of macOS LaunchAgent instructions", () => {
    ...
    expect(line).toContain("daisyclaw gateway restart");
    expect(line).toContain("daisyclaw gateway install --force");
    expect(line).toContain("daisyclaw gateway status --deep");
...
describe("recoverInstalledLaunchAgentAfterUpdate", () => {
  it("re-bootstraps an installed-but-not-loaded macOS LaunchAgent after update", async () => {
    ...
    const serviceEnv = { DAISYCLAW_PROFILE: "stomme" } as NodeJS.ProcessEnv;
    const recoveredEnv = { ...serviceEnv, DAISYCLAW_PORT: "18790" } as NodeJS.ProcessEnv;
  ...
  it("does not recover a loaded LaunchAgent", async () => {
    const readState = vi.fn(async () => ({
      ...
      loaded: true,
      running: true,
      env: { DAISYCLAW_PROFILE: "stomme" } as NodeJS.ProcessEnv,
      command: null,
  ...
  it("does not report recovered update health until the gateway passes the post-recovery wait", async () => {
    ...
    await expect(
      recoverLaunchAgentAndRecheckGatewayHealth({
        ...
        env: { DAISYCLAW_PROFILE: "stomme", DAISYCLAW_PORT: "18790" },
...
describe("resolvePostCoreUpdateChildStdio", () => {
  it('returns "pipe" on Windows so the child never inherits the parent console handles', () => {
    ...
    // causing the terminal to hang after `daisyclaw update` completes (#78445).
