...
import { isRecord } from "@daisyclaw/normalization-core/record-coerce";
...
import { disableCurrentDaisyClawUpdateLaunchdJob } from "../../daemon/launchd.js";
...
const POST_CORE_UPDATE_ENV = "DAISYCLAW_UPDATE_POST_CORE";
const POST_CORE_UPDATE_CHANNEL_ENV = "DAISYCLAW_UPDATE_POST_CORE_CHANNEL";
const POST_CORE_UPDATE_REQUESTED_CHANNEL_ENV = "DAISYCLAW_UPDATE_POST_CORE_REQUESTED_CHANNEL";
const POST_CORE_UPDATE_RESULT_PATH_ENV = "DAISYCLAW_UPDATE_POST_CORE_RESULT_PATH";
const POST_CORE_UPDATE_INSTALL_RECORDS_PATH_ENV = "DAISYCLAW_UPDATE_POST_CORE_INSTALL_RECORDS_PATH";
const POST_CORE_UPDATE_SOURCE_CONFIG_PATH_ENV = "DAISYCLAW_UPDATE_POST_CORE_SOURCE_CONFIG_PATH";
const POST_CORE_UPDATE_STARTED_AT_ENV = "DAISYCLAW_UPDATE_POST_CORE_STARTED_AT_MS";
...
type PreUpdateConfigRestoreInput = {
  ...
  authoredConfig: DaisyClawConfig;
};
...
function restoreDroppedPreUpdateChannels(
  ...
  const authoredChannels = resolveRestoredAuthoredChannels({
    ...
    currentAuthoredChannels: isRecord(snapshot.parsed)
      ? (snapshot.parsed as DaisyClawConfig).channels
...
function formatPostUpdatePluginInspectGuidance(pluginId: string): string {
  return `Run daisyclaw plugins inspect ${pluginId} --runtime --json for details.`;
}
...
export function buildInvalidConfigPostCoreUpdateResult(): {
  ...
  const guidance = [
    "Run `daisyclaw doctor` to inspect the config validation errors.",
    "Once the config parses, rerun `daisyclaw update repair`.",
  ];
  const message =
...
function formatPostUpdateGatewayRecoveryLine(platform: NodeJS.Platform): string {
  const restartCommand = replaceCliName(formatCliCommand("daisyclaw gateway restart"), CLI_NAME);
  const installCommand = replaceCliName(
    formatCliCommand("daisyclaw gateway install --force"),
    CLI_NAME,
  );
...
export function formatPostUpdateGatewayRecoveryInstructions(
  ...
  if (isPackageManagerUpdateMode(result.mode) && beforeVersion) {
    lines.push(
      `Rollback: reinstall DaisyClaw ${beforeVersion} with the same package manager, then rerun \`${replaceCliName(formatCliCommand("daisyclaw gateway install --force"), CLI_NAME)}\`.`,
    );
  }
  return lines;
}
...
function formatGatewayAncestryBlockMessage(pid: number): string {
  return `daisyclaw update detected it is running inside the gateway process tree.
Gateway PID ${pid} is an ancestor of this process, so this updater cannot safely stop or restart the gateway that owns it.
Run \`${replaceCliName(formatCliCommand("daisyclaw update"), CLI_NAME)}\` from a shell outside the gateway service, or stop the gateway service first and then update.`;
}
...
async function maybeStopManagedServiceBeforeMutableUpdate(params: {
  ...
  if (
    ...
    if (!params.jsonMode) {
      defaultRuntime.log(
        theme.muted(
          "Managed gateway service points at a different DaisyClaw root; leaving it running during this git update.",
        ),
      );
    }
    return {
...
function isRunningInsideGatewayService(
  ...
  if (env.DAISYCLAW_SERVICE_MARKER?.trim() !== GATEWAY_SERVICE_MARKER) {
    return false;
  }
  const serviceKind = env.DAISYCLAW_SERVICE_KIND?.trim();
...
async function resolvePackageRuntimePreflightError(params: {
  ...
  return [
    `${runtimeLabel} is too old for daisyclaw@${targetLabel}.`,
    `The requested package requires ${status.nodeEngine}.`,
    runtime.nodeRunner
      ? "Upgrade the Node runtime that owns the managed Gateway service, then rerun `daisyclaw update`."
      : "Upgrade Node to 22.19+ or Node 24, then rerun `daisyclaw update`.",
    "Bare `npm i -g daisyclaw` can silently install an older compatible release.",
    "After upgrading Node, use `npm i -g daisyclaw@latest`.",
  ].join("\n");
}
...
function stripGatewayServiceMarkerEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  delete resolvedEnv.DAISYCLAW_SERVICE_MARKER;
  delete resolvedEnv.DAISYCLAW_SERVICE_KIND;
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("daisyclaw completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
        ...
        return await runUpdateStep({
          ...
          env: {
            ...
            ...(candidateHostVersion === null
              ? {}
              : { DAISYCLAW_COMPATIBILITY_HOST_VERSION: candidateHostVersion }),
          },
          timeoutMs: params.timeoutMs,
          progress: params.progress,
        });
...
export async function updatePluginsAfterCoreUpdate(params: {
  ...
  if (pluginsChanged) {
    ...
    if (params.restoredAuthoredChannels !== undefined) {
      nextConfig = {
        ...nextConfig,
        channels: structuredClone(params.restoredAuthoredChannels) as DaisyClawConfig["channels"],
      };
    }
...
async function maybeRestartService(params: {
  ...
  const verifyRestartedGateway = async (
    ...
    const diagnosticLines = [
      ...
      `Run \`${replaceCliName(formatCliCommand("daisyclaw gateway status --deep"), CLI_NAME)}\` for details.`,
  ...
  if (params.shouldRestart) {
    ...
    try {
      ...
      if (!params.opts.json && restarted) {
        ...
        defaultRuntime.log("");
        await createUpdateConfigSnapshot();
        process.env.DAISYCLAW_UPDATE_IN_PROGRESS = "1";
        ...
        try {
          ...
          delete process.env.DAISYCLAW_UPDATE_IN_PROGRESS;
      ...
      if (!params.opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("daisyclaw gateway restart"), CLI_NAME)}`,
          ),
        );
      }
      if (
  ...
  if (!params.opts.json) {
    ...
    if (params.result.mode === "npm" || params.result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("daisyclaw doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("daisyclaw gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("daisyclaw gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
function withUpdateFinalizationEnv<T>(run: () => Promise<T>): Promise<T> {
  const previousUpdateInProgress = process.env.DAISYCLAW_UPDATE_IN_PROGRESS;
  ...
  return run().finally(() => {
    if (previousUpdateInProgress === undefined) {
      ...
      process.env.DAISYCLAW_UPDATE_IN_PROGRESS = previousUpdateInProgress;
    }
  ...
  const preFinalizeConfig = configSnapshot.valid
    ? {
        ...
        authoredConfig: isRecord(configSnapshot.parsed)
          ? (configSnapshot.parsed as DaisyClawConfig)
...
function normalizePreUpdateConfigRestoreInput(
  ...
  if (isRecord(sourceConfig) && isRecord(authoredConfig)) {
    return {
      sourceConfig: sourceConfig as DaisyClawConfig,
      authoredConfig: authoredConfig as DaisyClawConfig,
    };
  }
  const authored = parsed as DaisyClawConfig;
  return {
...
function resolvePreUpdateSourceConfigFromAuthored(
  ...
  try {
    ...
    return isRecord(resolved) ? (resolved as DaisyClawConfig) : authoredConfig;
  } catch {
...
async function continuePostCoreUpdateInFreshProcess(params: {
  ...
  const resultDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-update-post-core-"));
    ...
    const child = spawn(params.nodeRunner ?? resolveNodeRunner(), argv, {
      ...
      env: {
        ...
        ...(postCoreHostVersion === null
          ? {}
          : { DAISYCLAW_COMPATIBILITY_HOST_VERSION: postCoreHostVersion }),
...
async function updateCommandInternal(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (opts.dryRun !== true) {
    await disableCurrentDaisyClawUpdateLaunchdJob().catch(() => undefined);
    assertConfigWriteAllowedInCurrentMode();
  }
  ...
  if (postCoreUpdateResume) {
    ...
    process.env.DAISYCLAW_COMPATIBILITY_HOST_VERSION = (await readPackageVersion(root)) ?? VERSION;
  ...
  const devTargetRef =
    channel === "dev" ? process.env.DAISYCLAW_UPDATE_DEV_TARGET_REF?.trim() || undefined : undefined;
  ...
  if (updateInstallKind === "package") {
    ...
    if (managedServiceRootRedirect) {
      ...
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.warn(
            `Shell DaisyClaw root differs from the managed gateway service root: ${managedServiceRootRedirect.previousRoot}`,
          ),
        );
        defaultRuntime.log(
          theme.muted(
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating DaisyClaw..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "dirty") {
      ...
      defaultRuntime.log(
        theme.muted("Commit, stash, or discard the local changes, then rerun `daisyclaw update`."),
      );
    }
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this DaisyClaw install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("daisyclaw doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("daisyclaw gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g daisyclaw@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g daisyclaw@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (!pluginsUpdatedInFreshProcess) {
    ...
    const previousCompatibilityHostVersion = process.env.DAISYCLAW_COMPATIBILITY_HOST_VERSION;
    if (compatibilityDowngradeTarget) {
      process.env.DAISYCLAW_COMPATIBILITY_HOST_VERSION = compatibilityDowngradeTarget;
    }
    try {
      ...
      if (compatibilityDowngradeTarget) {
        if (previousCompatibilityHostVersion === undefined) {
          ...
          process.env.DAISYCLAW_COMPATIBILITY_HOST_VERSION = previousCompatibilityHostVersion;
        }
      }
    }
  }
