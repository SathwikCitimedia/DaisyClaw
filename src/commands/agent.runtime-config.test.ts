...
type ConfigSnapshotForWrite = {
  snapshot: { valid: boolean; resolved: DaisyClawConfig };
...
const loadConfigMock = vi.hoisted(() => vi.fn<() => DaisyClawConfig>());
...
const setRuntimeConfigSnapshotMock = vi.hoisted(() =>
  vi.fn<(cfg: DaisyClawConfig, sourceConfig: DaisyClawConfig) => void>(),
);
...
const resolveCommandConfigWithSecretsMock = vi.hoisted(() =>
  vi.fn<
    (params: ResolveCommandConfigParams) => Promise<{
      resolvedConfig: DaisyClawConfig;
      effectiveConfig: DaisyClawConfig;
      diagnostics: never[];
    }>
  >(),
);
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "daisyclaw-agent-" });
}
...
function mockConfig(home: string, storePath: string): DaisyClawConfig {
  const cfg = {
    agents: {
      defaults: {
