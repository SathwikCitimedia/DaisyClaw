...
vi.mock("../config/sessions/transcript-resolve.runtime.js", () => {
  ...
  const resolveSessionFile = (sessionId: string, agentId: string, sessionsDir?: string): string =>
    joinPath(sessionsDir ?? ".daisyclaw", "agents", agentId, "sessions", `${sessionId}.jsonl`);
...
function mockConfig(
  home: string,
  storePath: string,
  agentOverrides?: Partial<NonNullable<NonNullable<DaisyClawConfig["agents"]>["defaults"]>>,
  telegramOverrides?: Partial<NonNullable<NonNullable<DaisyClawConfig["channels"]>["telegram"]>>,
  agentsList?: NonNullable<NonNullable<DaisyClawConfig["agents"]>["list"]>,
) {
  const cfg = {
    agents: {
      defaults: {
...
beforeEach(() => {
  ...
  configIoMocks.readConfigFileSnapshotForWrite.mockResolvedValue({
    snapshot: { valid: false, resolved: {} as DaisyClawConfig },
    writeOptions: {},
  });
});
...
describe("agentCommand", () => {
  it("enables the Codex runtime plugin and provider owner for one-shot OpenAI model overrides", async () => {
    await withTempHome(async (home) => {
      ...
      for (const [registryLoad] of pluginRegistryMocks.ensurePluginRegistryLoaded.mock.calls) {
        ...
        expect(registryLoad?.workspaceDir).toBe(path.join(home, "daisyclaw"));
  ...
  it("does not enable Codex for one-shot OpenAI overrides when the provider forces DaisyClaw", async () => {
    await withTempHome(async (home) => {
      const storePath = path.join(home, "sessions.json");
  ...
  it("uses no-tools plain prompt mode for one-shot model runs", async () => {
    await withTempHome(async (home) => {
      ...
      await agentCommand(
        {
          message: "Reply with exactly DAISYCLAW-MODEL-OK",
          agentId: "main",
          model: "openrouter/auto",
          modelRun: true,
          promptMode: "none",
  ...
  it("bypasses ACP sessions for one-shot model runs", async () => {
    await withTempHome(async (home) => {
      ...
      expect(callArgs?.provider).toBe("openrouter");
      expect(callArgs?.model).toBe("openrouter/auto");
      expect(callArgs?.prompt).toBe("Reply with exactly DAISYCLAW-MODEL-OK");
      expect(callArgs?.modelRun).toBe(true);
      expect(callArgs?.promptMode).toBe("none");
      expect(callArgs?.disableTools).toBe(true);
    });
