...
describe("agents add command", () => {
  ...
  async function withAgentsAddStateRoot(
    ...
    try {
      await withEnvAsync({ DAISYCLAW_STATE_DIR: root }, async () => await run(root));
    } finally {
  ...
  it("requires --workspace when flags are present", async () => {
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      `Non-interactive agent creation requires --workspace. Re-run ${formatCliCommand("daisyclaw agents add <id> --workspace <path>")} or omit flags to use the wizard.`,
    );
  ...
  it("skips catalog validation when checking the interactive wizard model config", async () => {
    ...
    wizardMocks.createClackPrompter.mockReturnValue({
      intro: vi.fn(),
      text: vi.fn().mockResolvedValueOnce("Jon").mockResolvedValueOnce("/tmp/daisyclaw-jon"),
  ...
  it("copies only portable auth profiles when seeding a new agent store", async () => {
    await withAgentsAddStateRoot("daisyclaw-agents-add-auth-copy-", async (root) => {
  ...
  it("copies portable Codex OAuth profiles inline", async () => {
    await withAgentsAddStateRoot("daisyclaw-agents-add-oauth-copy-", async (root) => {
      const sourceAgentDir = path.join(root, "main", "agent");
      const destAgentDir = path.join(root, "work", "agent");
  ...
  it("skips unresolved OAuth profiles when seeding a new agent store", async () => {
    await withAgentsAddStateRoot("daisyclaw-agents-add-oauth-ref-skip-", async (root) => {
      const sourceAgentDir = path.join(root, "main", "agent");
      const destAgentDir = path.join(root, "work", "agent");
      const profileId = "openai:oauth";
      const ref = {
        source: "daisyclaw-credentials" as const,
