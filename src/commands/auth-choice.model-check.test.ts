...
describe("warnIfModelConfigLooksOff", () => {
  ...
  it("skips catalog validation when requested while keeping auth checks", async () => {
    ...
    expect(note).toHaveBeenCalledWith(
      'No auth configured for provider "openai". The agent may fail until credentials are added. Run `daisyclaw models auth login --provider openai`, `daisyclaw configure`, or set an API key env var.',
      "Model check",
    );
  });
