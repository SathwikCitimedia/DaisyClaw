...
vi.mock("../agents/agent-scope.js", () => ({
  ...
  resolveAgentDir: (configForTest: unknown, agentId: string) =>
    `${process.env.DAISYCLAW_STATE_DIR ?? "/tmp/daisyclaw-state"}/agents/${agentId}/agent`,
  resolveAgentWorkspaceDir: (configForTest: unknown, agentId: string) =>
    `/tmp/daisyclaw-workspaces/${agentId}`,
...
vi.mock("../agents/workspace.js", () => ({
  resolveDefaultAgentWorkspaceDir: () => "/tmp/daisyclaw-workspace",
}));
...
function resolveTestAuthStoreKey(agentDir?: string): string {
  return agentDir?.trim() || process.env.DAISYCLAW_AGENT_DIR || "__main__";
}
...
async function createApiKeyProvider(params: {
  ...
  noteMessage?: string;
  noteTitle?: string;
  applyConfig?: Partial<DaisyClawConfig>;
    ...
    auth: [
      {
        ...
        run: async (ctx) => {
          ...
          return {
            ...
            ...(params.applyConfig ? { configPatch: params.applyConfig as DaisyClawConfig } : {}),
  ...
  const createZaiMethod = (choiceId: "zai-api-key" | "zai-coding-global"): ProviderAuthMethod => ({
    ...
    run: async (ctx) => {
      ...
      return {
        ...
        configPatch: providerConfigPatch("zai", { baseUrl }) as DaisyClawConfig,
...
describe("applyAuthChoice", () => {
  ...
  async function setupTempState() {
    ...
    process.env.DAISYCLAW_AGENT_DIR = agentDir;
  }
  ...
  async function readAuthProfiles() {
    return readTestAuthProfileStore(resolveAgentDir({} as DaisyClawConfig, "main"));
  }
  ...
  beforeAll(async () => {
    authTestRoot = (await setupAuthTestEnv("daisyclaw-auth-")).stateDir;
  ...
  it("fails fast when a removed provider auth choice is passed to the interactive flow", async () => {
    ...
    try {
      await expect(
        ...
        'Auth choice "openai-chatgpt-import" is no longer supported. Use "openai" instead, or run daisyclaw onboard to choose interactively.',
      );
    } finally {
      spy.mockRestore();
    }
  ...
  it("escapes removed provider auth choice guidance for terminal output", async () => {
    ...
    try {
      await expect(
        ...
        'Auth choice "legacy\\u001b[31mchoice" is no longer supported. Use "modern\\nchoice" instead, or run daisyclaw onboard to choose interactively.',
      );
    } finally {
      spy.mockRestore();
    }
