...
const mocks = vi.hoisted(() => ({
  ...
  readConfigFileSnapshot: vi.fn(async () => ({ path: "/tmp/daisyclaw.json" })),
  ...
  resolveMissingOfficialExternalChannelPluginRepairHint: ({ channelId }: { channelId: string }) =>
    mocks.missingOfficialExternalChannels.has(channelId)
      ? {
          ...
          channelId,
          label: "Feishu",
          installSpec: "@daisyclaw/feishu",
          installCommand: "daisyclaw plugins install @daisyclaw/feishu",
          doctorFixCommand: "daisyclaw doctor --fix",
          repairHint:
            "Install the official external plugin with: daisyclaw plugins install @daisyclaw/feishu, or run: daisyclaw doctor --fix.",
        }
      : null,
}));
...
describe("channelsStatusCommand SecretRef fallback flow", () => {
  ...
  it("shows missing official external plugin repair hints in config-only output", async () => {
    ...
    expect(joined).toContain(
      "Feishu: Install the official external plugin with: daisyclaw plugins install @daisyclaw/feishu, or run: daisyclaw doctor --fix.",
    );
  });
    ...
    mocks.callGateway.mockRejectedValue(
      new Error(
        [
          ...
          "Source: env DAISYCLAW_GATEWAY_URL",
        ].join("\n"),
      ),
    );
    mocks.requireValidConfigSnapshot.mockResolvedValue({ secretResolved: false, channels: {} });
