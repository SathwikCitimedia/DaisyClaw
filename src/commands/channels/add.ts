// Implements guided and non-interactive `daisyclaw channels add` account setup.
...
async function resolveCatalogChannelEntry(raw: string, cfg: DaisyClawConfig | null) {
...
async function channelsAddCommandImpl(
  ...
  const cfg = (configSnapshot.sourceConfig ?? configSnapshot.config) as DaisyClawConfig;
  ...
  if (!channel) {
    const hint = catalogEntry
      ? `Plugin ${catalogEntry.meta.label} could not be loaded after install. Run daisyclaw doctor --fix, then retry daisyclaw channels add.`
  ...
  if (!plugin?.setup?.applyAccountConfig) {
    runtime.error(
      ...
      })} Run ${formatCliCommand("daisyclaw channels add")} with no flags for guided setup.`,
    );
    runtime.exit(1);
    return;
  }
