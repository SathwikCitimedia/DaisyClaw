...
describe("buildCleanupPlan", () => {
  test("resolves inside-state flags and workspace dirs", () => {
    ...
    const defaultWorkspace = path.join(tmpRoot, "daisyclaw-workspace-default");
    const opsWorkspace = path.join(tmpRoot, "daisyclaw-workspace-ops");
    const cfg = {
      agents: {
    ...
    const plan = buildCleanupPlan({
      cfg: cfg as unknown as DaisyClawConfig,
      stateDir: path.join(tmpRoot, "daisyclaw-state"),
      configPath: path.join(tmpRoot, "daisyclaw-state", "daisyclaw.json"),
      oauthDir: path.join(tmpRoot, "daisyclaw-oauth"),
    });
  ...
  test("includes implicit per-agent workspaces under the state dir", () => {
    const tmpRoot = path.join(path.parse(process.cwd()).root, "tmp", "daisyclaw-cleanup-plan");
    ...
    return withEnvAsync(
      {
        ...
        DAISYCLAW_WORKSPACE_DIR: undefined,
      },
      async () => {
        const plan = buildCleanupPlan({
          cfg: cfg as unknown as DaisyClawConfig,
          stateDir,
          configPath: path.join(stateDir, "daisyclaw.json"),
...
describe("applyAgentDefaultPrimaryModel", () => {
  it("does not mutate when already set", () => {
    const cfg = { agents: { defaults: { model: { primary: "a/b" } } } } as DaisyClawConfig;
  ...
  it("normalizes legacy models", () => {
    const cfg = { agents: { defaults: { model: { primary: "legacy" } } } } as DaisyClawConfig;
  ...
  it("normalizes retired Google Gemini primary models before writing config", () => {
    const cfg = { agents: { defaults: {} } } as DaisyClawConfig;
    const result = applyAgentDefaultPrimaryModel({
      cfg,
...
describe("cleanup path removals", () => {
  ...
  it("removes state and only linked paths outside state", async () => {
    ...
    const tmpRoot = path.join(path.parse(process.cwd()).root, "tmp", "daisyclaw-cleanup");
    await removeStateAndLinkedPaths(
      {
        stateDir: path.join(tmpRoot, "state"),
        configPath: path.join(tmpRoot, "state", "daisyclaw.json"),
    ...
    expect(runtime.log.mock.calls.map(([line]) => line.replaceAll("\\", "/"))).toEqual([
      "[dry-run] remove /tmp/daisyclaw-cleanup/state",
      "[dry-run] remove /tmp/daisyclaw-cleanup/oauth",
    ]);
  });

  it("preserves nested workspace paths during state-only removal", async () => {
    const runtime = createRuntimeMock();
    const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-cleanup-"));
    const stateDir = path.join(tmpRoot, ".daisyclaw");
  ...
  it("removes every workspace directory", async () => {
    const runtime = createRuntimeMock();
    const workspaces = ["/tmp/daisyclaw-workspace-1", "/tmp/daisyclaw-workspace-2"];

    await removeWorkspaceDirs(workspaces, runtime, { dryRun: true });

    const logs = runtime.log.mock.calls.map(([line]) => line);
    expect(logs).toEqual([
      "[dry-run] remove /tmp/daisyclaw-workspace-1",
      "[dry-run] remove /tmp/daisyclaw-workspace-2",
    ]);
  });

  it("removes owned legacy workspace attestations", async () => {
    const runtime = createRuntimeMock();
    const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-cleanup-attest-"));
    ...
    try {
      ...
      await fs.writeFile(
        ...
        `daisyclaw-workspace-attestation:v1\n${new Date().toISOString()}\n`,
      );
  ...
  it("refuses to remove a directory containing the current working directory", async () => {
    const runtime = createRuntimeMock();
    const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-cleanup-cwd-"));
