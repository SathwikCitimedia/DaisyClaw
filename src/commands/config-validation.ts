...
export async function requireValidConfigFileSnapshot(
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    runtime.error(`DaisyClaw config is invalid: ${snapshot.path}\n${issues}`);
    runtime.error(
    ...
    runtime.error(`Inspect: ${formatCliCommand("daisyclaw config validate")}`);
    runtime.exit(1);
    return null;
  }
...
/** Read and return a valid DaisyClaw config, or null after reporting validation errors. */
