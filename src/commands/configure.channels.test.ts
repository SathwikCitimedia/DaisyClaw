...
function expectUnknownChannelRemovalPrompt(unsafeChannel: string, label: string) {
  ...
  expect(confirmArg().message).toBe(
    `Delete ${label} configuration from ~/.daisyclaw/daisyclaw.json?`,
  );
...
describe("removeChannelConfigWizard", () => {
  ...
  it("lists configured channels from daisyclaw.json even when no plugins are loaded", async () => {
  ...
  it("deletes the selected channel block from daisyclaw.json", async () => {
    ...
    expect(confirmArg().message).toBe(
      "Delete Telegram configuration from ~/.daisyclaw/daisyclaw.json?",
    );
  ...
  it("deletes a real channel block named done", async () => {
    ...
    expect(confirmArg().message).toBe("Delete done configuration from ~/.daisyclaw/daisyclaw.json?");
  ...
  it("sanitizes known channel labels before rendering prompts", async () => {
    ...
    expect(confirmArg().message).toBe(
      "Delete Telegram\\nBot configuration from ~/.daisyclaw/daisyclaw.json?",
    );
    expect(note).toHaveBeenCalledWith(
