...
/** Prompt for configured channel sections to remove from daisyclaw.json. */
export async function removeChannelConfigWizard(
  ...
  while (true) {
    ...
    if (configured.length === 0) {
      note(
        [
          "No channel config found in daisyclaw.json.",
          `Tip: \`${formatCliCommand("daisyclaw channels status")}\` shows what is configured and enabled.`,
        ].join("\n"),
        "Remove channel",
      );
      return next;
    ...
    if (Object.keys(nextChannels).length) {
      next.channels = nextChannels as DaisyClawConfig["channels"];
    } else {
