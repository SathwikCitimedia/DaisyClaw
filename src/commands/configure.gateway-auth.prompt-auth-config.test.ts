...
const mocks = vi.hoisted(() => ({
  ...
  applyPrimaryModel: vi.fn((cfg: DaisyClawConfig, model: string) => ({
    ...cfg,
    agents: {
      ...cfg.agents,
      defaults: {
  ...
  applyModelAllowlist: vi.fn(
    (cfg: DaisyClawConfig, models: string[], opts: { scopeKeys?: string[] } = {}) => {
  ...
  applyModelFallbacksFromSelection: vi.fn(
    (cfg: DaisyClawConfig, selection: string[], opts: { scopeKeys?: string[] } = {}) => {
      const defaults = cfg.agents?.defaults;
