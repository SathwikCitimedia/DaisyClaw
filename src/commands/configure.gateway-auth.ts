...
import type { DaisyClawConfig, GatewayAuthConfig } from "../config/config.js";
...
function hasConfiguredProviderModels(cfg: DaisyClawConfig, provider: string | undefined): boolean {
  if (!provider) {
    return false;
  }
...
function hasStaticManifestCatalogRows(cfg: DaisyClawConfig, provider: string | undefined): boolean {
  if (!provider) {
    return false;
  }
  return (
...
function listConfiguredModelProviders(cfg: DaisyClawConfig): string[] {
  return Object.entries(cfg.models?.providers ?? {})
    .filter(([, provider]) => (provider.models?.length ?? 0) > 0)
    .map(([provider]) => provider);
}

function resolveSingleConfiguredProvider(cfg: DaisyClawConfig): string | undefined {
...
function resolveConfiguredProviderFromAuthChange(params: {
  before: DaisyClawConfig;
  after: DaisyClawConfig;
...
export function buildGatewayAuthConfig(params: {
  ...
  if (params.mode === "trusted-proxy") {
    if (!params.trustedProxy) {
      throw new Error(
        `trustedProxy config is required when mode is trusted-proxy. Run ${formatCliCommand("daisyclaw configure --section gateway")} to configure Gateway auth interactively.`,
      );
    }
