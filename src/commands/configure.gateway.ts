...
import { validateIPv4AddressInput } from "@daisyclaw/net-policy/ipv4";
import {
...
export async function promptGatewayConfig(
  ...
  if (authMode === "token") {
    ...
    if (tokenInputMode === "ref") {
      ...
      note(`Validated ${envVarName}. DaisyClaw will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: DaisyClaw trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.daisyclaw.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
