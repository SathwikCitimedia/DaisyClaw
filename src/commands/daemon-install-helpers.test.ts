...
const mocks = vi.hoisted(() => ({
  ...
  resolveDaisyClawWrapperPath: vi.fn(),
...
vi.mock("../daemon/program-args.js", () => ({
  DAISYCLAW_WRAPPER_ENV_KEY: "DAISYCLAW_WRAPPER",
  resolveGatewayProgramArguments: mocks.resolveGatewayProgramArguments,
  resolveDaisyClawWrapperPath: mocks.resolveDaisyClawWrapperPath,
}));
...
function mockNodeGatewayPlanFixture(
  ...
  const {
    ...
    supported = true,
    warning,
    serviceEnvironment = { DAISYCLAW_PORT: "3000" },
  } = params;
  const workingDirectory = Object.hasOwn(params, "workingDirectory")
    ? params.workingDirectory
    : "/Users/me";
  mocks.resolvePreferredNodePath.mockResolvedValue("/opt/node");
  mocks.resolveDaisyClawWrapperPath.mockImplementation(async (value: string | undefined) =>
...
describe("buildGatewayInstallPlan", () => {
  // Prevent tests from reading the developer's real ~/.daisyclaw/.env when
  ...
  it("uses provided nodePath and returns plan", async () => {
    ...
    expect(plan.environment).toEqual({ DAISYCLAW_PORT: "3000" });
  ...
  it("adds the active daisyclaw command bin directory to the managed service PATH", async () => {
    mockNodeGatewayPlanFixture();
    const originalArgv = process.argv;
    const daisyclawBinPath = path.join(isolatedHome, ".npm-global", "bin", "daisyclaw");
    process.argv = ["node", daisyclawBinPath, "gateway", "install"];
    ...
    ).toStrictEqual(["/opt/homebrew/opt/node/bin", path.dirname(daisyclawBinPath)]);
  });
  ...
  it("uses the state dir as the default macOS launchd working directory", async () => {
    ...
    expect(plan.workingDirectory).toBe(path.join(isolatedHome, ".daisyclaw"));
    expect(mocks.buildServiceEnvironment).toHaveBeenCalledOnce();
  ...
  it("passes DAISYCLAW_WRAPPER through program args and managed service env", async () => {
    const wrapperPath = path.resolve("/usr/local/bin/daisyclaw-doppler");
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        DAISYCLAW_PORT: "3000",
        DAISYCLAW_WRAPPER: wrapperPath,
      },
    });
    ...
    expect(mocks.buildServiceEnvironment).toHaveBeenCalledOnce();
    expect(
      firstMockArg(mocks.buildServiceEnvironment, "buildServiceEnvironment").env?.DAISYCLAW_WRAPPER,
    ).toBe(wrapperPath);
    expect(plan.environment.DAISYCLAW_WRAPPER).toBe(wrapperPath);
  });

  it("clears a Windows wrapper env that points at the generated gateway.cmd script", async () => {
    const selfWrapperPath = path.join(isolatedHome, ".daisyclaw", "gateway.cmd");
    const warn = vi.fn();
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        DAISYCLAW_PORT: "3000",
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        DAISYCLAW_WRAPPER: selfWrapperPath,
      }),
      port: 3000,
      runtime: "node",
      platform: "win32",
    ...
    expect(plan.environment.DAISYCLAW_WRAPPER).toBeUndefined();
    expect(warn).toHaveBeenCalledWith(
      expect.stringContaining(
        "Ignoring DAISYCLAW_WRAPPER because it points to the Windows task script",
      ),
    );
  });
  ...
  it("tracks safe config env keys without embedding literal values", async () => {
    ...
    expect(plan.environment.DAISYCLAW_PORT).toBe("3000");
    expect(plan.environment.DAISYCLAW_SERVICE_MANAGED_ENV_KEYS).toBe(
  ...
  it("includes env SecretRef values from config into the service environment", async () => {
    ...
    expect(plan.environment.DAISYCLAW_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
  ...
  it("allows safe inherited passEnv names while blocking dangerous exec SecretRef env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        BASH_ENV: "/tmp/daisyclaw-test-bashenv",
        XDG_CONFIG_HOME: "/tmp/daisyclaw-test-xdg-home",
  ...
  it("does not embed gateway auth SecretRef values into the service environment", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        DAISYCLAW_GATEWAY_TOKEN: "gateway-test-token",
      }),
      port: 3000,
      runtime: "node",
      config: {
        gateway: {
          auth: {
            token: { source: "env", provider: "default", id: "DAISYCLAW_GATEWAY_TOKEN" },
          },
        },
      },
    });

    expect(plan.environment.DAISYCLAW_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environment.DAISYCLAW_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
  ...
  it("does not inline config env SecretRef values already backed by state-dir dotenv", async () => {
    await writeStateDirDotEnv("DISCORD_BOT_TOKEN=discord-dotenv-token\n", {
      stateDir: path.join(isolatedHome, ".daisyclaw"),
    });
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        DAISYCLAW_PORT: "3000",
    ...
    expect(plan.environment.DAISYCLAW_SERVICE_MANAGED_ENV_KEYS).toBe("DISCORD_BOT_TOKEN");
  });
...
describe("buildGatewayInstallPlan — dotenv merge", () => {
  ...
  it("tracks .env vars with config while preserving service precedence", async () => {
    await writeStateDirDotEnv(
      ...
      {
        stateDir: path.join(tmpDir, ".daisyclaw"),
      },
    );
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
  ...
  it("retains .env values for macOS LaunchAgent env SecretRefs", async () => {
    ...
    expect(plan.environment.DAISYCLAW_SERVICE_MANAGED_ENV_KEYS).toBe("MINIMAX_API_KEY");
  });
  ...
  it("works when .env file does not exist", async () => {
    mockNodeGatewayPlanFixture({ serviceEnvironment: { DAISYCLAW_PORT: "3000" } });
  ...
  it("preserves safe custom vars from an existing service env and merges PATH", async () => {
    ...
    expect(plan.environment.DAISYCLAW_SERVICE_MARKER).toBeUndefined();
  });
  ...
  it("drops keys that were previously tracked as managed service env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        BLOGWATCHER_HOME: "/Users/test/.blogwatcher",
        GOPATH: "/Users/test/.local/gopath",
        DAISYCLAW_SERVICE_MANAGED_ENV_KEYS: "GOBIN,GOPATH",
      },
    });
  ...
  it("drops legacy inline env values when the key is now managed by .env", async () => {
    ...
    expect(plan.environment.DAISYCLAW_SERVICE_MANAGED_ENV_KEYS).toBe("TAVILY_API_KEY");
  ...
  it("keeps source metadata for EnvironmentFile-backed preserved vars", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        CUSTOM_TOOL_HOME: "/Users/test/.custom-tool",
        DAISYCLAW_GATEWAY_TOKEN: "old-token",
      },
    ...
    expect(plan.environment.DAISYCLAW_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environmentValueSources?.DAISYCLAW_GATEWAY_TOKEN).toBeUndefined();
  });
  ...
  it("does not embed auth-profile env refs when the key is already durable", async () => {
    ...
    expect(plan.environment.DAISYCLAW_SERVICE_MANAGED_ENV_KEYS).toBe("OPENAI_API_KEY");
  });
});
...
describe("gatewayInstallErrorHint", () => {
  it("returns platform-specific hints", () => {
    ...
    expect(gatewayInstallErrorHint("linux")).toMatch(
      /(?:daisyclaw|daisyclaw)( --profile isolated)? gateway install/,
    );
  });
});
...
describe("collectPreservedExistingServiceEnvVars — operator opt-in allowlist", () => {
  ...
  it("continues to drop stale DAISYCLAW_ALLOW_ROOT", () => {
    const result = collectPreservedExistingServiceEnvVars(
      { DAISYCLAW_ALLOW_ROOT: "1" },
      managedKeys,
    );
    expect(result.DAISYCLAW_ALLOW_ROOT).toBeUndefined();
  });

  it("preserves DAISYCLAW_CLI_CONTAINER_BYPASS and DAISYCLAW_CONTAINER_HINT", () => {
    const result = collectPreservedExistingServiceEnvVars(
      {
        DAISYCLAW_CLI_CONTAINER_BYPASS: "1",
        DAISYCLAW_CONTAINER_HINT: "ci",
      },
      managedKeys,
    );
    expect(result.DAISYCLAW_CLI_CONTAINER_BYPASS).toBe("1");
    expect(result.DAISYCLAW_CONTAINER_HINT).toBe("ci");
  });

  it("still drops arbitrary DAISYCLAW_FOO", () => {
    const result = collectPreservedExistingServiceEnvVars({ DAISYCLAW_FOO: "bar" }, managedKeys);
    expect(result.DAISYCLAW_FOO).toBeUndefined();
  });

  it("preserves container opt-ins while dropping unrelated DAISYCLAW_* keys", () => {
    const result = collectPreservedExistingServiceEnvVars(
      {
        DAISYCLAW_CLI_CONTAINER_BYPASS: "1",
        DAISYCLAW_CONTAINER_HINT: "ci",
    ...
    expect(result.DAISYCLAW_CLI_CONTAINER_BYPASS).toBe("1");
    expect(result.DAISYCLAW_CONTAINER_HINT).toBe("ci");
    expect(result.DAISYCLAW_BAZ).toBeUndefined();
  });
});
