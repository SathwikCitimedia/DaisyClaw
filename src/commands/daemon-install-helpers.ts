...
import {
  DAISYCLAW_WRAPPER_ENV_KEY,
  resolveGatewayProgramArguments,
  resolveDaisyClawWrapperPath,
...
// they share the DAISYCLAW_ prefix that is otherwise stripped from preserved
...
const PRESERVED_DAISYCLAW_OPERATOR_OPT_IN_ENV_KEYS = new Set([
  "DAISYCLAW_CLI_CONTAINER_BYPASS",
  "DAISYCLAW_CONTAINER_HINT",
]);
...
export function collectPreservedExistingServiceEnvVars(
  ...
  for (const [rawKey, rawValue] of Object.entries(existingEnvironment)) {
    ...
    if (
      ...
      upper === "PATH" ||
      upper === "TMPDIR" ||
      (upper.startsWith("DAISYCLAW_") && !PRESERVED_DAISYCLAW_OPERATOR_OPT_IN_ENV_KEYS.has(upper))
    ) {
      continue;
    }
...
export async function buildGatewayInstallPlan(params: {
  ...
  const wrapperInput = params.wrapperPath ?? params.env[DAISYCLAW_WRAPPER_ENV_KEY];
  ...
  if (wrapperPointsAtWindowsTaskScript) {
    params.warn?.(
      `Ignoring ${DAISYCLAW_WRAPPER_ENV_KEY} because it points to the Windows task script; using the DaisyClaw gateway entrypoint directly to avoid a recursive gateway.cmd wrapper.`,
    );
  }
  const wrapperPath = wrapperPointsAtWindowsTaskScript
    ? undefined
    : await resolveDaisyClawWrapperPath(wrapperInput);
  const serviceInputEnv: Record<string, string | undefined> = wrapperPath
    ? { ...params.env, [DAISYCLAW_WRAPPER_ENV_KEY]: wrapperPath }
    : wrapperPointsAtWindowsTaskScript
      ? omitEnvKey(params.env, DAISYCLAW_WRAPPER_ENV_KEY)
      : params.env;
  ...
  const serviceEnvironment = buildServiceEnvironment({
    ...
    launchdLabel:
      platform === "darwin"
        ? resolveGatewayLaunchAgentLabel(serviceInputEnv.DAISYCLAW_PROFILE)
        : undefined,
    platform,
...
export function gatewayInstallErrorHint(platform = process.platform): string {
  return platform === "win32"
    ...
    : `Tip: rerun \`${formatCliCommand("daisyclaw gateway install")}\` after fixing the error.`;
}
