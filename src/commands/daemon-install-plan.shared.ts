...
function isDaisyClawCommandBasename(basename: string, platform: NodeJS.Platform): boolean {
  if (basename === "daisyclaw") {
    return true;
  }
  if (platform === "win32") {
    return (
      basename === "daisyclaw.cmd" || basename === "daisyclaw.ps1" || basename === "daisyclaw.exe"
    );
  }
  return false;
}
...
/** Resolve the DaisyClaw CLI binary directory from argv/PATH for daemon PATH. */
export function resolveDaemonDaisyClawBinDir(
  params: {
    argv?: string[];
    env?: Record<string, string | undefined>;
  ...
  if (
    ...
    isDaisyClawCommandBasename(path.basename(argv1), platform)
  ) {
  ...
  for (const rawSegment of (env.PATH ?? "").split(path.delimiter)) {
    ...
    const candidate = path.join(segment, platform === "win32" ? "daisyclaw.cmd" : "daisyclaw");
...
/** Merge Node and DaisyClaw binary directories for the daemon service PATH. */
export function resolveDaemonServicePathDirs(params: {
  ...
  for (const dir of resolveDaemonDaisyClawBinDir(params) ?? []) {
    addUniquePathDir(dirs, dir);
  }
  return dirs.length > 0 ? dirs : undefined;
}
