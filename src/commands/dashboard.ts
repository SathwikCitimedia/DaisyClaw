// Implements `daisyclaw dashboard` URL resolution, readiness check, clipboard, and browser launch.
...
export async function dashboardCommand(
  ...
  if (resolvedToken.unresolvedRefReason) {
    ...
    runtime.log(
      "Set DAISYCLAW_GATEWAY_TOKEN in this shell or resolve your secret provider, then rerun `daisyclaw dashboard`.",
    );
  }
  ...
  if (opened) {
    runtime.log("Opened in your browser. Keep that tab to control DaisyClaw.");
  ...
  if (fallbackToManualAuth) {
    runtime.log(
      "Token auto-auth not delivered. Append your gateway token (from DAISYCLAW_GATEWAY_TOKEN or gateway.auth.token) as a URL fragment with key `token` to authenticate.",
    );
  }
}
