...
const states: DaisyClawTestState[] = [];
...
async function makeTestState(seed = "legacy-oauth-seed"): Promise<DaisyClawTestState> {
  const state = await createDaisyClawTestState({
    layout: "state-only",
    prefix: "daisyclaw-doctor-oauth-sidecar-",
    env: {
      DAISYCLAW_AGENT_DIR: undefined,
      DAISYCLAW_AUTH_PROFILE_SECRET_KEY: seed,
    },
  });
  states.push(state);
  return state;
...
function encryptLegacySidecarMaterial(params: {
  ref: { source: "daisyclaw-credentials"; provider: "openai-codex"; id: string };
  profileId: string;
  provider: string;
  seed: string;
...
describe("maybeRepairLegacyOAuthSidecarProfiles", () => {
  ...
  it("scans DAISYCLAW_AGENT_DIR before treating sidecars as unreferenced", async () => {
    const state = await makeTestState();
    const previousAgentDir = process.env.DAISYCLAW_AGENT_DIR;
    ...
    try {
      ...
      if (previousAgentDir === undefined) {
        ...
        process.env.DAISYCLAW_AGENT_DIR = previousAgentDir;
      }
    }
  });
