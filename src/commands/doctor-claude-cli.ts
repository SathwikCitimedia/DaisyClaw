...
function usesClaudeCliModelSelection(cfg: DaisyClawConfig): boolean {
...
function resolveClaudeCliCommand(cfg: DaisyClawConfig): string {
...
function formatWorkspaceHealthLine(
  ...
  if (health === "missing") {
    return `- ${label}: ${display} (missing; DaisyClaw will create it on first run).`;
  }
...
function resolveClaudeCliAgentIds(cfg: DaisyClawConfig): string[] {
...
export function noteClaudeCliHealth(
  ...
  if (!storedProfile) {
    lines.push(`- DaisyClaw auth profile: missing (${CLAUDE_CLI_PROFILE_ID}) in ${authStorePath}.`);
    fixHints.push(
    ...
    lines.push(
      `- DaisyClaw auth profile: ${CLAUDE_CLI_PROFILE_ID} is wired to provider "${storedProfile.provider}" instead of "${CLAUDE_CLI_PROVIDER}".`,
    );
    fixHints.push(
    ...
    lines.push(
      `- DaisyClaw auth profile: ${CLAUDE_CLI_PROFILE_ID} (provider ${CLAUDE_CLI_PROVIDER}).`,
    );
  }
