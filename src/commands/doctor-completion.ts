...
export type ShellCompletionStatus = {
  ...
  cacheExists: boolean;
  cachePath: string;
  /** True if profile uses slow dynamic pattern like `source <(daisyclaw completion ...)` */
    ...
    return [
      {
        ...
        path: pathLocal,
        fixHint: "Run `daisyclaw doctor --fix` to upgrade to cached completion.",
      },
    ];
  }
    ...
    return [
      {
        ...
        path: pathLocal,
        fixHint: `Run \`daisyclaw completion --write-state\` or \`daisyclaw doctor --fix\` to regenerate ${status.cachePath}.`,
      },
    ];
  }
  return [];
...
export async function ensureCompletionCacheExists(binName = "daisyclaw"): Promise<boolean> {
  const shell = resolveShellFromEnv() as CompletionShell;
  const cacheExists = await completionCacheExists(shell, binName);
