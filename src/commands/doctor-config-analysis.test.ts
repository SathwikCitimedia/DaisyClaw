...
import { DaisyClawSchema } from "../config/zod-schema.js";
import {
...
describe("doctor config analysis helpers", () => {
  ...
  it("preserves user-authored model and agent metadata during unknown-key cleanup", () => {
    ...
    expect(DaisyClawSchema.safeParse({ defaultModel: "minimax/MiniMax-M2.7" }).success).toBe(false);
  ...
  describe("stripUnknownConfigKeys during update", () => {
    const originalEnv = process.env.DAISYCLAW_UPDATE_IN_PROGRESS;
    ...
    afterEach(() => {
      if (originalEnv !== undefined) {
        process.env.DAISYCLAW_UPDATE_IN_PROGRESS = originalEnv;
      } else {
        delete process.env.DAISYCLAW_UPDATE_IN_PROGRESS;
      }
    });

    it("returns input unchanged when DAISYCLAW_UPDATE_IN_PROGRESS=1", () => {
    ...
    it("returns input unchanged when DAISYCLAW_UPDATE_IN_PROGRESS=true", () => {
      process.env.DAISYCLAW_UPDATE_IN_PROGRESS = "true";
      const input = { hooks: {}, unexpected: true } as never;
      const result = stripUnknownConfigKeys(input);
      expect(result.config).toBe(input);
      expect(result.removed).toEqual([]);
...
describe("collectImplicitFallbackClobberWarnings", () => {
  function buildConfig(overrides: { defaults?: unknown; list?: unknown[] }): DaisyClawConfig {
    return {
      agents: {
