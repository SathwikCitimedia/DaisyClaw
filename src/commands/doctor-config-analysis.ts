...
import { resolvePrimaryStringValue } from "@daisyclaw/normalization-core/string-coerce";
...
function isUpdateInProgress(): boolean {
  const value = process.env.DAISYCLAW_UPDATE_IN_PROGRESS;
 ...
 */
export function stripUnknownConfigKeys(config: DaisyClawConfig): {
  ...
  const parsed = DaisyClawSchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }
...
export function noteOpencodeProviderOverrides(cfg: DaisyClawConfig): void {
...
export function collectImplicitFallbackClobberWarnings(cfg: DaisyClawConfig): string[] {
...
export function noteImplicitFallbackClobberWarnings(cfg: DaisyClawConfig): void {
