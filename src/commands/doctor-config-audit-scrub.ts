...
export async function maybeScrubConfigAuditLog(params: {
  ...
  try {
    if (params.shouldRepair) {
      ...
      if (result.aborted) {
        note(
          "Config audit scrub was aborted because new entries were appended to config-audit.jsonl during the rewrite. No records were modified. Stop the gateway (or wait until it is idle) and rerun `daisyclaw doctor --fix`.",
          NOTE_TITLE,
        );
        return;
      }
    ...
    if (preview.rewritten > 0) {
      const fixCommand = params.doctorFixCommand ?? "daisyclaw doctor --fix";
      note(
