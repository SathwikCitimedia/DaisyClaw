...
function collectUnsupportedInternalHookEntryWarnings(cfg: DaisyClawConfig): string[] {
...
function collectConfiguredChannelIds(cfg: DaisyClawConfig): string[] {
  const channels =
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: DaisyClawConfig = baseCfg;
  let candidate = structuredClone(baseCfg);
  let pendingChanges = false;
  let fixHints: string[] = [];
  const doctorFixCommand = formatCliCommand("daisyclaw doctor --fix");
  ...
  if (hasLegacyInternalHookHandlers(snapshot.parsed)) {
    note(
      [
        ...
        "- daisyclaw doctor --fix does not rewrite this shape automatically.",
      ].join("\n"),
      "Legacy config keys detected",
    );
  }
