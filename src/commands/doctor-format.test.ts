...
describe("buildGatewayRuntimeHints", () => {
  ...
  it("surfaces suspicious systemd cgroup hygiene with inspection commands", () => {
    expect(
      ...
      "Run: systemctl --user show daisyclaw-gateway.service -p KillMode -p TasksCurrent -p MemoryCurrent -p MainPID",
      "Run: systemd-cgls --user-unit daisyclaw-gateway.service",
      "After reviewing service settings, run: daisyclaw gateway restart",
    ]);
  });
