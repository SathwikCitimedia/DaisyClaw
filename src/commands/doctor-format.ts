...
export function buildGatewayRuntimeHints(
  ...
  if (runtime.cachedLabel && platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(env.DAISYCLAW_PROFILE);
    hints.push(
      `LaunchAgent label cached but plist missing. Clear with: launchctl bootout gui/$UID/${label}`,
    );
    hints.push(`Then reinstall: ${formatCliCommand("daisyclaw gateway install", env)}`);
  }
  if (runtime.missingUnit) {
    hints.push(`Service not installed. Run: ${formatCliCommand("daisyclaw gateway install", env)}`);
    if (fileLog) {
      hints.push(`File logs: ${fileLog}`);
    }
    return hints;
  ...
  if (runtime.missingGuiSession && platform === "darwin") {
    ...
    hints.push(
      `Sign in to the macOS desktop as this user, then run: ${formatCliCommand("daisyclaw gateway restart", env)}`,
    );
    hints.push(
  ...
  if (runtime.missingSupervision && platform === "darwin") {
    hints.push(
      `LaunchAgent installed but not loaded. Run: ${formatCliCommand("daisyclaw gateway restart", env)}`,
    );
    if (fileLog) {
      hints.push(`File logs: ${fileLog}`);
    }
  ...
  if (runtime.status === "stopped") {
    ...
    hints.push(
      ...buildPlatformRuntimeLogHints({
        platform,
        env,
        systemdServiceName: resolveGatewaySystemdServiceName(env.DAISYCLAW_PROFILE),
        windowsTaskName: resolveGatewayWindowsTaskName(env.DAISYCLAW_PROFILE),
      }),
    );
  }
  if (platform === "linux" && isSystemdCgroupHygieneRisk(runtime.systemd)) {
    const unit =
      runtime.systemd?.unit ?? `${resolveGatewaySystemdServiceName(env.DAISYCLAW_PROFILE)}.service`;
    ...
    if (summary) {
      hints.push(
        ...
        `After reviewing service settings, run: ${formatCliCommand("daisyclaw gateway restart", env)}`,
      );
    }
  }
  return hints;
