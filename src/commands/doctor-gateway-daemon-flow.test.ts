...
vi.mock("../daemon/constants.js", () => ({
  resolveGatewayLaunchAgentLabel: vi.fn(() => "ai.daisyclaw.gateway"),
  resolveNodeLaunchAgentLabel: vi.fn(() => "ai.daisyclaw.node"),
}));
...
describe("maybeRepairGatewayDaemon", () => {
  ...
  it("reports recent restart handoffs during deep doctor", async () => {
    ...
    service.readCommand.mockResolvedValueOnce({
      ...
      environment: {
        DAISYCLAW_STATE_DIR: "/tmp/daisyclaw-service",
        DAISYCLAW_CONFIG_PATH: "/tmp/daisyclaw-service/daisyclaw.json",
      },
    });
    ...
    const [handoffEnv] = readGatewayRestartHandoffSync.mock.calls[0] as unknown as [
      { DAISYCLAW_STATE_DIR?: string; DAISYCLAW_CONFIG_PATH?: string },
    ];
    expect(handoffEnv?.DAISYCLAW_STATE_DIR).toBe("/tmp/daisyclaw-service");
    expect(handoffEnv?.DAISYCLAW_CONFIG_PATH).toBe("/tmp/daisyclaw-service/daisyclaw.json");
    expect(note).toHaveBeenCalledWith(
      ...
      connections: [
        {
          pid: 4242,
          command: "node",
          commandLine: "/tmp/newer-daisyclaw/bin/daisyclaw logs --follow",
  ...
  it("suppresses busy-port note for expected Gateway listeners", async () => {
    setPlatform("linux");
    const listeners = [{ pid: 5001, commandLine: "daisyclaw-gateway", address: "0.0.0.0:18789" }];
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners,
  ...
  it("keeps busy-port note for unexpected Gateway listeners", async () => {
    ...
    inspectPortUsage.mockResolvedValue({
      ...
      status: "busy",
      listeners: [
        { pid: 5001, commandLine: "daisyclaw-gateway", address: "0.0.0.0:18789" },
        { pid: 5002, commandLine: "daisyclaw-gateway", address: "127.0.0.1:18789" },
      ],
  ...
  it("skips gateway install during non-interactive doctor repairs", async () => {
    ...
    expect(service.restart).not.toHaveBeenCalled();
    expect(note).toHaveBeenCalledWith(
      `Run ${formatCliCommand("daisyclaw gateway install")} when you want to install the gateway service.`,
      "Gateway",
    );
  });
  ...
  it("skips gateway service install when a system DaisyClaw gateway service exists", async () => {
    setPlatform("linux");
    service.isLoaded.mockResolvedValue(false);
    findSystemGatewayServices.mockResolvedValue([
      {
        platform: "linux",
        label: "daisyclaw-gateway.service",
        detail: "unit: /etc/systemd/system/daisyclaw-gateway.service",
        scope: "system",
        marker: "daisyclaw",
        legacy: false,
      },
    ...
    expect(note).toHaveBeenCalledWith(
      [
        "System-level DaisyClaw gateway service detected while the user gateway service is not installed.",
        "- daisyclaw-gateway.service (unit: /etc/systemd/system/daisyclaw-gateway.service)",
        "DaisyClaw will not install a second user-level gateway service automatically.",
        "Run `daisyclaw gateway status --deep` or `daisyclaw doctor --deep` to inspect duplicate services.",
