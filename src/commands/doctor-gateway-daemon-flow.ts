...
function renderEstablishedGatewayConnections(connections: PortConnection[]): string {
  return [
    ...
    "If logs show protocol mismatch after rollback, stop stale DaisyClaw client processes listed here and rerun doctor.",
  ].join("\n");
}
...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.DAISYCLAW_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("daisyclaw gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
