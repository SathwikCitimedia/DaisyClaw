...
function noteCliGatewayVersionSkew(status: StatusSummary | undefined): void {
  ...
  note(
    [
      `This command is DaisyClaw ${VERSION}; the running Gateway is DaisyClaw ${gatewayVersion}.`,
      "Check `daisyclaw --version`, `which daisyclaw`, and `daisyclaw gateway status --deep`.",
      "If this mismatch is unexpected, update PATH so `daisyclaw` points to the version you want, or reinstall the Gateway service from that same DaisyClaw install.",
    ].join("\n"),
    "DaisyClaw version mismatch",
  );
}
