...
import { replaceConfigFile, type DaisyClawConfig } from "../config/config.js";
...
import { DAISYCLAW_WRAPPER_ENV_KEY } from "../daemon/program-args.js";
...
function buildGatewayServiceRepairEnv(
  ...
  const wrapperPath = command?.environment?.[DAISYCLAW_WRAPPER_ENV_KEY]?.trim();
  if (!wrapperPath || Object.hasOwn(process.env, DAISYCLAW_WRAPPER_ENV_KEY)) {
    return process.env;
  }
  return {
    ...process.env,
    [DAISYCLAW_WRAPPER_ENV_KEY]: wrapperPath,
  };
}
...
function resolveGatewayServiceWrapperPath(
  ...
  return normalizeOptionalString(command?.environment?.[DAISYCLAW_WRAPPER_ENV_KEY]) ?? null;
}
...
function resolveSystemdUnitNameFromServicePath(sourcePath: string | undefined): string {
  ...
  return base.endsWith(".service") ? base : "daisyclaw-gateway.service";
}
...
export async function maybeRepairGatewayServiceConfig(
  ...
  if (serviceWrapperPath) {
    note(`Gateway service invokes ${DAISYCLAW_WRAPPER_ENV_KEY}: ${serviceWrapperPath}`, "Gateway");
  }
  ...
  const sourceCheckoutWarning = serviceLayout?.entrypointSourceCheckout
    ? [
        ...
        "Run `daisyclaw doctor --fix` from the intended package install, or reinstall the gateway service with `daisyclaw gateway install --force`.",
      ].join("\n")
    : null;
  ...
  if (tokenRefConfigured && serviceToken) {
    audit.issues.push({
      ...
      message:
        "Gateway service DAISYCLAW_GATEWAY_TOKEN should be unset when gateway.auth.token is SecretRef-managed",
  ...
  if (serviceRewriteBlocked) {
    note(
      "Gateway service is running; leaving supervisor metadata unchanged. Stop the service first or use `daisyclaw gateway install --force` when you want to replace the active launcher.",
      "Gateway service config",
    );
    return;
  }
  ...
  if (
    ...
    note(
      "Update-mode doctor detected gateway service drift but left the live systemd unit unchanged. Review the service file and run `daisyclaw gateway install --force` when you want DaisyClaw to replace operator-owned systemd directives.",
      "Gateway service config",
    );
    return;
  }
  ...
  if (!repair) {
    if (!emittedSourceCheckoutWarning) {
      note(
        "Run `daisyclaw gateway install --force` when you want to replace the gateway service definition.",
        "Gateway service config",
      );
    }
    return;
...
export async function maybeScanExtraGatewayServices(
  ...
  if (legacyServices.length > 0) {
    ...
    if (shouldRemove) {
      ...
      if (removed.length > 0) {
        runtime.log("Legacy gateway services removed. Installing DaisyClaw gateway next.");
      }
    }
  }
