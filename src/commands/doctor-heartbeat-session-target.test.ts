...
describe("describeHeartbeatSessionTargetIssues", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-heartbeat-doctor-"));
  });
  ...
  function cfgWithSession(session: string, target: string | null = "slack"): DaisyClawConfig {
  ...
  function writeStore(cfg: DaisyClawConfig, entries: Record<string, unknown>) {
