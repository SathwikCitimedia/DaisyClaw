...
async function makeTempRoot(): Promise<string> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-heartbeat-template-"));
  tempDirs.push(root);
  return root;
}
...
\`HEARTBEAT.md\` lives in the agent workspace. Keep the file empty, or with only Markdown comments and headings, when you want DaisyClaw to skip heartbeat model calls.
