...
export async function collectInstallPolicyHealthLines(
  ...
  if (!options.deep) {
    lines.push(
      `- Static checks passed. Run ${formatCliCommand("daisyclaw doctor --deep")} to execute a synthetic policy probe.`,
    );
    return lines;
  }

  const probeDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-install-policy-probe-"));
  try {
