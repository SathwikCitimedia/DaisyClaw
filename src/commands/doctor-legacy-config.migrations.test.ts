...
vi.mock("../plugins/setup-registry.js", () => ({
  ...
  runPluginSetupConfigMigrations: ({ config }: { config: DaisyClawConfig }) => ({
    config,
    changes: [],
  }),
}));
...
vi.mock("./doctor/shared/channel-legacy-config-migrate.js", () => ({
  applyChannelDoctorCompatibilityMigrations: (cfg: DaisyClawConfig) => ({
    next: cfg,
    changes: [],
  }),
}));
...
vi.mock("../secrets/target-registry.js", () => {
  ...
  return {
    discoverConfigSecretTargets: (cfg: DaisyClawConfig) => {
      const targets: Array<{
        entry: typeof entry;
        path: string;
        pathSegments: string[];
...
describe("normalizeCompatibilityConfigValues", () => {
  ...
  beforeAll(() => {
    previousOauthDir = process.env.DAISYCLAW_OAUTH_DIR;
    tempOauthDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-oauth-"));
    process.env.DAISYCLAW_OAUTH_DIR = tempOauthDir;
  });
  ...
  afterAll(() => {
    if (previousOauthDir === undefined) {
      ...
      process.env.DAISYCLAW_OAUTH_DIR = previousOauthDir;
    }
    fs.rmSync(tempOauthDir, { recursive: true, force: true });
  });
        ...
        list: [
          {
            ...
            models: {
              "anthropic/claude-opus-4-7": { agentRuntime: { id: "daisyclaw" } },
            },
          },
        ],
      },
  ...
  it("does not report talk provider normalization for realtime voice aliases", () => {
    ...
    const res = normalizeCompatibilityConfigValues(input as DaisyClawConfig);
