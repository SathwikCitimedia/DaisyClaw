...
describe("noteMemorySearchHealth", () => {
  ...
  beforeEach(() => {
    ...
    resolveActiveMemoryBackendConfig.mockImplementation(
      ({ cfg: cfgLocal }: { cfg: DaisyClawConfig }) =>
  ...
  it("warns when local provider is set but readiness was not confirmed", async () => {
    ...
    expect(message).toContain("daisyclaw plugins install @daisyclaw/llama-cpp-provider");
  });
  ...
  it("does not warn when an enabled alternate memory plugin owns the memory slot", async () => {
    ...
    const cfgWithLancedb = {
      plugins: {
        ...
        entries: { "memory-lancedb": { enabled: true, config: { dbPath: ".daisyclaw/memory" } } },
      },
  ...
  it("does not warn when QMD backend is active", async () => {
    const qmdCfg = { memory: { backend: "qmd", qmd: { command: "qmd" } } } as DaisyClawConfig;
    resolveMemorySearchConfig.mockReturnValue({
      provider: "auto",
      local: {},
      remote: {},
  ...
  it("does not warn when key-optional provider (lmstudio) probe was skipped (skipped: true)", async () => {
    // When `daisyclaw doctor` runs without --deep, the probe is skipped and returns
    ...
    // Regression test for: https://github.com/daisyclaw/daisyclaw/issues/74608
    resolveMemorySearchConfig.mockReturnValue({
      provider: "lmstudio",
      local: {},
      remote: {},
  ...
  it("warns when openai-compatible is missing its required baseUrl even if probe was skipped", async () => {
    ...
    expect(message).toContain("daisyclaw config set");
    expect(resolveApiKeyForProvider).not.toHaveBeenCalled();
  });
  ...
  it("warns for key-optional provider (lmstudio) when gateway probe timed out", async () => {
    ...
    // Regression guard: https://github.com/daisyclaw/daisyclaw/issues/74608
    resolveMemorySearchConfig.mockReturnValue({
      provider: "lmstudio",
      local: {},
      remote: {},
  ...
  it("uses model configure hint when gateway probe is unavailable and API key is missing", async () => {
    ...
    expect(message).toContain("daisyclaw configure --section model");
    expect(message).not.toContain("daisyclaw auth add --provider");
  });
...
describe("memory recall doctor integration", () => {
  ...
  it("runs dreaming artifact repair during doctor --fix", async () => {
    ...
    repairDreamingArtifacts.mockResolvedValueOnce({
      changed: true,
      archiveDir: "/tmp/agent-default/workspace/.daisyclaw-repair/dreaming/2026-04-11T21-35-00-000Z",
      archivedDreamsDiary: false,
...
describe("detectLegacyWorkspaceDirs", () => {
  it("returns active workspace and no legacy dirs", () => {
    const workspaceDir = "/home/user/daisyclaw";
