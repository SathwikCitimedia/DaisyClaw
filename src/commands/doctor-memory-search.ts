...
import {
  ...
  normalizeProviderId,
} from "@daisyclaw/model-catalog-core/provider-id";
...
function isOpenAICompatibleMemoryProvider(providerId: string, cfg: DaisyClawConfig): boolean {
...
function isKeyOptionalMemoryProvider(providerId: string, cfg: DaisyClawConfig): boolean {
  return (
    providerId === "local" ||
    providerId === "ollama" ||
    providerId === "lmstudio" ||
...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("daisyclaw doctor --fix")} or ${formatCliCommand("daisyclaw memory status --fix")}`
    : `Verify: ${formatCliCommand("daisyclaw memory status --deep")}`;
  return [
...
export async function noteMemoryRecallHealth(cfg: DaisyClawConfig): Promise<void> {
  try {
...
function hasActiveAlternateMemoryPluginSlot(cfg: DaisyClawConfig): boolean {
 ...
 * Runs as part of `daisyclaw doctor` — config-only checks where possible;
...
export async function noteMemorySearchHealth(
      ...
      note(
        [
          ...
          workspaceProbeFailed
            ...
            : `- Set an explicit binary path: ${formatCliCommand("daisyclaw config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("daisyclaw config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("daisyclaw memory status --deep")}`,
        ]
          .filter(Boolean)
  ...
  if (provider === "local") {
    ...
    note(
      [
        ...
        "",
        "Fix (pick one):",
        `- Install the llama.cpp provider plugin: ${formatCliCommand("daisyclaw plugins install @daisyclaw/llama-cpp-provider")}`,
        `- Set a local GGUF model path in config`,
        suggestedRemoteProvider
          ? `- Switch to a remote provider: ${formatCliCommand(`daisyclaw config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
  ...
  if (
    ...
    note(
      [
        ...
        "",
        "Fix:",
        `- ${formatCliCommand("daisyclaw config set agents.defaults.memorySearch.remote.baseUrl http://127.0.0.1:1234/v1")}`,
        "",
        `Verify: ${formatCliCommand("daisyclaw memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  if (isOpenAICompatibleMemoryProvider(provider, cfg) && !normalizeOptionalString(resolved.model)) {
    note(
      [
        ...
        "",
        "Fix:",
        `- ${formatCliCommand("daisyclaw config set agents.defaults.memorySearch.model text-embedding-bge-m3")}`,
        "",
        `Verify: ${formatCliCommand("daisyclaw memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  if (isKeyOptionalMemoryProvider(provider, cfg)) {
    ...
    // not warn. A skipped probe means the user ran `daisyclaw doctor` without
  ...
  note(
    [
      ...
      `- Configure credentials: ${formatCliCommand("daisyclaw configure --section model")}`,
      `- To disable: ${formatCliCommand("daisyclaw config set agents.defaults.memorySearch.enabled false")}`,
      "",
      `Verify: ${formatCliCommand("daisyclaw memory status --deep")}`,
    ].join("\n"),
    "Memory search",
