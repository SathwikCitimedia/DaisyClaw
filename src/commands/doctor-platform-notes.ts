...
import { findStaleDaisyClawUpdateLaunchdJobs } from "../daemon/launchd.js";
...
export function collectMacLaunchAgentOverrideWarning(deps?: {
  ...
  const markerCandidates = [path.join(home, ".daisyclaw", "disable-launchagent")];
...
/** Returns a warning for stale DaisyClaw updater launchd jobs left after interrupted updates. */
export async function collectMacStaleDaisyClawUpdateLaunchdJobsWarning(deps?: {
  platform?: NodeJS.Platform;
  findJobs?: typeof findStaleDaisyClawUpdateLaunchdJobs;
  ...
  const jobs = await (deps?.findJobs ?? findStaleDaisyClawUpdateLaunchdJobs)(scanEnv).catch(
    () => [],
  );
  if (jobs.length === 0) {
    return null;
  ...
  return [
    "- Stale DaisyClaw updater launchd job(s) detected.",
    ...jobs.map((job) => {
      const exitStatus =
    ...
    `  ${formatCliCommand("daisyclaw gateway restart")}`,
  ].join("\n");
}

/** Emits stale updater launchd job notes using the gateway service environment when available. */
export async function noteMacStaleDaisyClawUpdateLaunchdJobs(deps?: {
  platform?: NodeJS.Platform;
  findJobs?: typeof findStaleDaisyClawUpdateLaunchdJobs;
  env?: NodeJS.ProcessEnv;
...
function hasConfigGatewayCreds(cfg: DaisyClawConfig): boolean {
...
export async function collectMacLaunchctlGatewayEnvOverrideWarning(
  ...
  const tokenEntries = [
    ["DAISYCLAW_GATEWAY_TOKEN", await getenv("DAISYCLAW_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["DAISYCLAW_GATEWAY_PASSWORD", await getenv("DAISYCLAW_GATEWAY_PASSWORD")],
  ] as const;
  ...
  return [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "DAISYCLAW_GATEWAY_PASSWORD"}\` is set; it can make local clients use a different password than gateway.auth.password.`
      : undefined,
...
export async function collectMacGatewayPlatformWarnings(
  ...
  const serviceEnv =
    platform === "darwin" ? await resolveGatewayServiceEnvForPlatformNotes(deps) : deps?.env;
  const staleUpdateWarning = await collectMacStaleDaisyClawUpdateLaunchdJobsWarning({
    env: serviceEnv,
    findJobs: deps?.findJobs,
    platform,
  });
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = normalizeOptionalString(env.DAISYCLAW_NO_RESPAWN) ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- DAISYCLAW_NO_RESPAWN is not set to 1; set it when you want routine gateway restarts to stay in-process instead of handing off to a managed supervisor.",
    );
  }
