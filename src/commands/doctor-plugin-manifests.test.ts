...
function makeTrustedBundledPluginsDir() {
  ...
  const dir = fs.mkdtempSync(path.join(fixturesRoot, "daisyclaw-doctor-plugin-manifests-"));
  tempDirs.push(dir);
  return dir;
}

function configWithPluginLoadPath(pluginRoot: string): DaisyClawConfig {
  return {
    plugins: {
      load: {
        paths: [pluginRoot],
...
describe("doctor plugin manifest legacy contract repair", () => {
  ...
  it("collects legacy top-level capability keys for migration", () => {
    ...
    const manifestPath = path.join(root, "daisyclaw.plugin.json");
  ...
  it("rewrites legacy top-level capability keys into contracts", async () => {
    ...
    const next = JSON.parse(fs.readFileSync(path.join(root, "daisyclaw.plugin.json"), "utf-8")) as {
