...
function readPluginManifestId(packageDir: string): string | undefined {
  const manifest = readJsonObject(path.join(packageDir, "daisyclaw.plugin.json"));
...
function listStaleManagedNpmBundledPlugins(
  ...
  for (const npmRoot of listManagedPluginNpmRoots(params)) {
    ...
    for (const packageName of Object.keys(dependencies).toSorted((left, right) =>
      ...
      if (!packageName.startsWith("@daisyclaw/")) {
        continue;
      }
  ...
  if (!params.prompter.shouldRepair) {
    note(
      [
        ...
        `Repair with ${formatCliCommand("daisyclaw doctor --fix")} to remove stale managed npm packages and rebuild the plugin registry.`,
      ].join("\n"),
      "Plugin registry",
    );
    return false;
  ...
  if (!params.prompter.shouldRepair) {
    note(
      [
        ...
        `Repair with ${formatCliCommand("daisyclaw doctor --fix")} to remove stale local install records and rebuild the plugin registry.`,
      ].join("\n"),
      "Plugin registry",
    );
    return [];
...
/** Relinks managed npm plugin packages to the current DaisyClaw host packages. */
export async function maybeRepairManagedNpmDaisyClawPeerLinks(
  params: PluginRegistryDoctorRepairParams,
): Promise<boolean> {
  const npmRoots = listManagedPluginNpmRoots(params);
  if (!params.prompter.shouldRepair) {
    const audits = await Promise.all(
      npmRoots.map((npmRoot) => auditDaisyClawPeerDependenciesInManagedNpmRoot({ npmRoot })),
    );
    const issues = audits.flatMap((audit) => audit.issues);
    if (issues.length > 0) {
      note(
        [
          "Managed npm DaisyClaw host peer links need repair:",
          ...issues.map((issue) => `- ${issue.packageName}: ${issue.reason}`),
          `Repair with ${formatCliCommand("daisyclaw doctor --fix")} to relink managed npm plugin packages.`,
        ].join("\n"),
        "Plugin registry",
      );
    }
  ...
  if (repaired > 0) {
    note(
      `Repaired DaisyClaw host peer link(s) for ${repaired} managed npm plugin package(s).`,
      "Plugin registry",
    );
  }
  ...
  if (warnings.length > 0) {
    note(
      ["Could not repair all managed npm DaisyClaw host peer links:", ...warnings].join("\n"),
      "Plugin registry",
    );
  }
...
export async function maybeRepairPluginRegistryState(
  ...
  const repairedManagedNpmDaisyClawPeerLinks = await maybeRepairManagedNpmDaisyClawPeerLinks(params);
    ...
    if (preflight.action === "migrate") {
      note(
        [
          ...
          `Repair with ${formatCliCommand("daisyclaw doctor --fix")} to rebuild ${shortenHomePath(preflight.filePath)} from enabled plugins.`,
        ].join("\n"),
        "Plugin registry",
      );
    }
  ...
  if (
    ...
    repairedManagedNpmDaisyClawPeerLinks
  ) {
