...
function codexBwrapNeedsNetworkNamespaceProbe(cfg: DaisyClawConfig): boolean {
  const network = cfg.agents?.defaults?.sandbox?.docker?.network?.trim().toLowerCase();
  return network === undefined || network === "" || network === "none";
}

async function probeCodexBwrapNamespaces(cfg: DaisyClawConfig): Promise<CodexBwrapNamespaceProbe> {
...
async function noteCodexBwrapNamespaceWarning(cfg: DaisyClawConfig): Promise<void> {
  ...
  const lines = [
    ...
    "",
    "Fix the host namespace policy for the DaisyClaw service user, then restart the gateway.",
    "Prefer an AppArmor profile that grants the required namespaces to the DaisyClaw service process.",
...
function resolveSandboxDockerImage(cfg: DaisyClawConfig): string {
  const image = cfg.agents?.defaults?.sandbox?.docker?.image?.trim();
  return image ? image : DEFAULT_SANDBOX_IMAGE;
}

function resolveSandboxBackend(cfg: DaisyClawConfig): string {
  const backend = cfg.agents?.defaults?.sandbox?.backend?.trim();
  return backend || "docker";
}

function resolveSandboxBrowserImage(cfg: DaisyClawConfig): string {
  const image = cfg.agents?.defaults?.sandbox?.browser?.image?.trim();
  return image ? image : DEFAULT_SANDBOX_BROWSER_IMAGE;
}

function updateSandboxDockerImage(cfg: DaisyClawConfig, image: string): DaisyClawConfig {
  return {
    ...cfg,
    agents: {
      ...cfg.agents,
...
function updateSandboxBrowserImage(cfg: DaisyClawConfig, image: string): DaisyClawConfig {
  return {
    ...cfg,
    agents: {
      ...cfg.agents,
...
export async function maybeRepairSandboxImages(
  ...
  if (!dockerAvailable) {
    const lines = [
      ...
      "- Disable sandbox mode: daisyclaw config set agents.defaults.sandbox.mode off",
    ];
    note(lines.join("\n"), "Sandbox");
    return cfg;
  }
  ...
  if (!prompter.shouldRepair) {
    note(
      [
        ...
        `Run ${formatCliCommand("daisyclaw doctor --fix")} to migrate them to SQLite.`,
      ].join("\n"),
      "Sandbox",
    );
    return;
...
export function noteSandboxScopeWarnings(cfg: DaisyClawConfig) {
