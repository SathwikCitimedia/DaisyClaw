...
describe("maybeRepairSandboxImages", () => {
  ...
  function createSandboxConfig(mode: "off" | "all" | "non-main"): DaisyClawConfig {
    return {
      agents: {
        defaults: {
          sandbox: {
  ...
  function createSandboxConfigWithDockerNetwork(network: string): DaisyClawConfig {
    return {
      agents: {
        defaults: {
          sandbox: {
...
describe("maybeRepairSandboxRegistryFiles", () => {
  ...
  it("warns about legacy registry files without migrating outside doctor --fix", async () => {
    inspectLegacySandboxRegistryFiles.mockResolvedValue([
      {
        kind: "containers",
        registryPath: "/tmp/daisyclaw/sandbox/containers.json",
        shardedDir: "/tmp/daisyclaw/sandbox/containers",
        source: "monolithic",
        exists: true,
        valid: true,
        entries: 2,
    ...
    expect(note).toHaveBeenCalledWith(
      [
        ...
        "- containers monolithic: /tmp/daisyclaw/sandbox/containers.json (2 entries)",
        "Run daisyclaw doctor --fix to migrate them to SQLite.",
      ].join("\n"),
      "Sandbox",
    );
  });
