...
describe("noteSecurityWarnings gateway exposure", () => {
  ...
  beforeEach(() => {
    ...
    pluginRegistry.list = [];
    prevToken = process.env.DAISYCLAW_GATEWAY_TOKEN;
    prevPassword = process.env.DAISYCLAW_GATEWAY_PASSWORD;
    prevHome = process.env.HOME;
    prevServiceKind = process.env.DAISYCLAW_SERVICE_KIND;
    delete process.env.DAISYCLAW_GATEWAY_TOKEN;
    delete process.env.DAISYCLAW_GATEWAY_PASSWORD;
    delete process.env.DAISYCLAW_SERVICE_KIND;
  });
  ...
  afterEach(() => {
    if (prevToken === undefined) {
      ...
      process.env.DAISYCLAW_GATEWAY_TOKEN = prevToken;
    }
    if (prevPassword === undefined) {
      delete process.env.DAISYCLAW_GATEWAY_PASSWORD;
    } else {
      process.env.DAISYCLAW_GATEWAY_PASSWORD = prevPassword;
    }
    ...
    if (prevServiceKind === undefined) {
      ...
      process.env.DAISYCLAW_SERVICE_KIND = prevServiceKind;
    }
  });
  ...
  async function withExecApprovalsFile(
    ...
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-doctor-security-"));
    process.env.HOME = home;
    await fs.mkdir(path.join(home, ".daisyclaw"), { recursive: true });
    await fs.writeFile(
      path.join(home, ".daisyclaw", "exec-approvals.json"),
  ...
  it("warns when exposed without auth", async () => {
    const cfg = { gateway: { bind: "lan" } } as DaisyClawConfig;
  ...
  it("uses env token to avoid critical warning", async () => {
    process.env.DAISYCLAW_GATEWAY_TOKEN = "token-123";
    const cfg = { gateway: { bind: "lan" } } as DaisyClawConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("warns when DAISYCLAW_GATEWAY_TOKEN env conflicts with gateway.auth.token config (#74271)", async () => {
    process.env.DAISYCLAW_GATEWAY_TOKEN = "env-token-123";
    const cfg = {
      gateway: {
        auth: {
    ...
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).toContain("DAISYCLAW_GATEWAY_TOKEN conflicts with gateway.auth.token");
    expect(message).toContain("Direct local Gateway clients commonly prefer the env token");
    expect(message).toContain("~/.daisyclaw/.env");
  });

  it("does not warn when only env token is set without config token", async () => {
    process.env.DAISYCLAW_GATEWAY_TOKEN = "env-token-only";
    const cfg = { gateway: { bind: "lan" } } as DaisyClawConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).not.toContain("DAISYCLAW_GATEWAY_TOKEN overrides");
  });
  ...
  it("does not warn inside the managed gateway service credential context", async () => {
    ...
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).not.toContain("DAISYCLAW_GATEWAY_TOKEN conflicts");
  });

  it("does not warn when config token uses DAISYCLAW_GATEWAY_TOKEN SecretRef", async () => {
    process.env.DAISYCLAW_GATEWAY_TOKEN = "env-token-123";
    const cfg = {
  ...
  it("skips warning for loopback bind", async () => {
    const cfg = { gateway: { bind: "loopback" } } as DaisyClawConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("treats unset bind as loopback for host-side doctor checks", async () => {
    const cfg = { gateway: {} } as DaisyClawConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).toContain("No channel security warnings detected");
    expect(message).not.toContain("Gateway bound");
  ...
  it("shows explicit dmScope config command for multi-user DMs", async () => {
    ...
    const cfg = { session: { dmScope: "main" } } as DaisyClawConfig;
    await noteSecurityWarnings(cfg);
  ...
  it("clarifies approvals.exec forwarding-only behavior", async () => {
    ...
    expect(message).toContain("daisyclaw approvals get --gateway");
  });
  ...
  it("warns when model provider API keys are stored as plaintext in config", async () => {
    ...
    expect(message).toContain("daisyclaw secrets audit --check");
  });
  ...
  it("warns when normalized tools.exec mode is broader than host exec defaults", async () => {
    ...
    expect(message).toContain('defaults.security="allowlist"');
    expect(message).not.toContain("DaisyClaw default");
  });
  ...
  it("degrades safely when channel account resolution fails in read-only security checks", async () => {
    ...
    await noteSecurityWarnings({} as DaisyClawConfig);
    ...
    expect(message).toContain("Run: daisyclaw security audit --deep");
  });
