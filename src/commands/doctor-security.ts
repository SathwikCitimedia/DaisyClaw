...
import type { DaisyClawConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: DaisyClawConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: DaisyClawConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "DaisyClaw default (full)";
  const defaultRequestedAskSource = "DaisyClaw default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("daisyclaw approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: DaisyClawConfig): string[] {
  void cfg;
  return [];
}

function collectExecFilesystemPolicyWarnings(cfg: DaisyClawConfig): string[] {
...
function collectPlaintextConfigSecretWarnings(cfg: DaisyClawConfig): string[] {
  ...
  return [
    "- WARNING: daisyclaw.json contains plaintext secret-bearing config fields.",
    `  Paths: ${pathLine}`,
    "  Agents or workspace tools that can read config files may see these API keys/tokens.",
    `  Migrate them to SecretRefs with ${formatCliCommand("daisyclaw secrets configure")} or ${formatCliCommand("daisyclaw secrets apply")}, then verify with ${formatCliCommand("daisyclaw secrets audit --check")}.`,
  ];
}
...
export async function collectSecurityWarnings(
  ...
  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      ...
      `  Check local policy with: ${formatCliCommand("daisyclaw approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.daisyclaw.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("daisyclaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("daisyclaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("daisyclaw doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("daisyclaw config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('daisyclaw config set session.dmScope "per-channel-peer"') +
...
export async function noteSecurityWarnings(cfg: DaisyClawConfig) {
  const warnings = await collectSecurityWarnings(cfg);
  const auditHint = `- Run: ${formatCliCommand("daisyclaw security audit --deep")}`;
