...
export function readEmbeddedGatewayTokenForTest(
  ...
  return isEnvironmentFileOnlySource(command?.environmentValueSources?.DAISYCLAW_GATEWAY_TOKEN)
    ? undefined
    : normalizeOptionalString(command?.environment?.DAISYCLAW_GATEWAY_TOKEN);
}
