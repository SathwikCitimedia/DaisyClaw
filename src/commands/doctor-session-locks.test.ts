...
describe("noteSessionLockHealth", () => {
  let state: DaisyClawTestState;

  beforeEach(async () => {
    note.mockClear();
    state = await createDaisyClawTestState({
      layout: "state-only",
      prefix: "daisyclaw-doctor-locks-",
    });
  });
  ...
  it("uses configured stale threshold without removing live DaisyClaw lock files", async () => {
    const sessionsDir = state.sessionsDir();
    await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it("removes fresh live locks when the owner is not an DaisyClaw process", async () => {
    const sessionsDir = state.sessionsDir();
    await fs.mkdir(sessionsDir, { recursive: true });
    ...
    expect(note).toHaveBeenCalledTimes(1);
    const [message] = firstNoteCall();
    expect(message).toContain("stale=yes (non-daisyclaw-owner)");
    expect(message).toContain("[removed]");
    expect(message).toContain("Removed 1 stale session lock file");
