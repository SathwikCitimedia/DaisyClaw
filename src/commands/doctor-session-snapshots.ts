...
function isTempBackedDaisyClawRoot(segments: readonly string[]): boolean {
  const lower = segments.map((segment) => segment.toLowerCase());
  const daisyclawIndex = lower.lastIndexOf("daisyclaw");
  if (daisyclawIndex < 1) {
    return false;
  }
  return lower[daisyclawIndex - 1] === "tmp" || lower[daisyclawIndex - 1] === "temp";
}
...
function isBundledRuntimeSkillsPath(cachedPath: string, skillRootIndex: number): boolean {
  ...
  return (
    lower.some(
      (segment) =>
        segment === "dist-runtime" || segment === "node_modules" || segment.startsWith("daisyclaw@"),
    ) || isTempBackedDaisyClawRoot(beforeSkillRoot)
  );
}
