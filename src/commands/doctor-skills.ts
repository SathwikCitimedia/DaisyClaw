...
export function formatUnavailableSkillDoctorLines(skills: SkillStatusEntry[]): string[] {
  ...
  lines.push(`Disable unused skills: ${formatCliCommand("daisyclaw doctor --fix")}`);
  lines.push(
    `Inspect details: ${formatCliCommand("daisyclaw skills check --agent <id>")} or ${formatCliCommand("daisyclaw skills info <name> --agent <id>")}`,
  );
  return lines;
}
