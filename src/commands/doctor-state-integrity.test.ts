...
type EnvSnapshot = {
  HOME?: string;
  DAISYCLAW_HOME?: string;
  DAISYCLAW_STATE_DIR?: string;
  DAISYCLAW_OAUTH_DIR?: string;
  DAISYCLAW_AGENT_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    DAISYCLAW_HOME: process.env.DAISYCLAW_HOME,
    DAISYCLAW_STATE_DIR: process.env.DAISYCLAW_STATE_DIR,
    DAISYCLAW_OAUTH_DIR: process.env.DAISYCLAW_OAUTH_DIR,
    DAISYCLAW_AGENT_DIR: process.env.DAISYCLAW_AGENT_DIR,
  };
}
...
function setupSessionState(cfg: DaisyClawConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
function createAgentDir(agentId: string, includeNestedAgentDir = true) {
  ...
  if (!stateDir) {
    throw new Error("DAISYCLAW_STATE_DIR is not set");
  }
...
async function runStateIntegrity(cfg: DaisyClawConfig) {
...
async function runStateIntegrityText(cfg: DaisyClawConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.DAISYCLAW_HOME = tempHome;
    ...
    fs.mkdirSync(process.env.DAISYCLAW_STATE_DIR, { recursive: true, mode: 0o700 });
    noteMock.mockClear();
  });
  ...
  it("prompts for oauth dir when DAISYCLAW_OAUTH_DIR is explicitly configured", async () => {
    process.env.DAISYCLAW_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: DaisyClawConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(hasRepairPromptMessage(confirmRuntimeRepair, "Create OAuth dir at")).toBe(true);
  ...
  it("does not warn when DAISYCLAW_AGENT_DIR points at the live compatibility agent dir", async () => {
    ...
    process.env.DAISYCLAW_AGENT_DIR = legacyAgentDir;
  ...
  it("warns about tombstoned subagent restart recovery sessions", async () => {
    ...
    expect(text).toContain("daisyclaw tasks maintenance --apply");
  ...
  it("does not warn when a case-mismatched dir resolves to the configured agent path", async () => {
    ...
    const resolvedResearchAgentDir = realpathNative(
      path.join(process.env.DAISYCLAW_STATE_DIR ?? "", "agents", "Research", "agent"),
    );
    const realpathSpy = vi
      .spyOn(fs.realpathSync, "native")
      .mockImplementation((target, options) => {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      try {
        ...
        process.env.DAISYCLAW_HOME = symlinkHome;
        process.env.DAISYCLAW_STATE_DIR = path.join(symlinkHome, ".daisyclaw");
  ...
  it("prints daisyclaw-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: DaisyClawConfig = {};
    writeSessionStore(cfg, {
      "agent:main:main": {
    ...
    expect(text).toMatch(/daisyclaw sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/daisyclaw sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /daisyclaw sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
  ...
  it("does not let synthetic heartbeat metadata override mixed transcript history", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-heartbeat-main-mixed-"));
    try {
  ...
  it("does not let heartbeat-looking routing metadata skip mixed transcript checks", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-heartbeat-main-route-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
      fs.writeFileSync(
        transcriptPath,
  ...
  it("does not classify transcripts with real user activity after 400 heartbeat messages", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-heartbeat-main-cap-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
  ...
  it("keeps the heartbeat main-session helper conservative", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-heartbeat-main-helper-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
      fs.writeFileSync(
        transcriptPath,
  ...
  it("moves store entries and clears matching TUI pointers without touching others", () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-tui-pointer-clear-"));
    try {
