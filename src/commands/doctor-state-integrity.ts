...
import { asNullableObjectRecord } from "@daisyclaw/normalization-core/record-coerce";
...
import { resolveDaisyClawAgentDir } from "../plugin-sdk/agent-dir-compat.js";
...
function listOrphanAgentDirs(cfg: DaisyClawConfig, stateDir: string): OrphanAgentDir[] {
  ...
  const liveCompatibilityAgentDir = resolveDaisyClawAgentDir();
  try {
...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidates = [".daisyclaw"].map((dir) => path.resolve(root, entry.name, dir));
...
export function formatLinuxSdBackedStateDirWarning(
  ...
  return [
    ...
    "- For better startup and state durability, prefer SSD/NVMe (or USB SSD on Raspberry Pi) for DAISYCLAW_STATE_DIR.",
  ].join("\n");
}
...
export function detectMacCloudSyncedStateDir(
  ...
  // DAISYCLAW_HOME can relocate app data defaults, but iCloud/CloudStorage remain under the OS home.
...
function shouldRequireOAuthDir(cfg: DaisyClawConfig, env: NodeJS.ProcessEnv): boolean {
  if (env.DAISYCLAW_OAUTH_DIR?.trim()) {
    return true;
  }
...
function shouldSuppressOrphanTranscriptWarning(cfg: DaisyClawConfig, agentId: string): boolean {
...
export async function noteStateIntegrity(
  ...
  const defaultStateDir = path.join(homedir(), ".daisyclaw");
  ...
  if (cloudSyncedStateDir) {
    warnings.push(
      [
        ...
        "- Prefer a local non-synced state dir (for example: ~/.daisyclaw).",
        `  Set locally: DAISYCLAW_STATE_DIR=~/.daisyclaw ${formatCliCommand("daisyclaw doctor")}`,
      ].join("\n"),
    );
  }
    ...
    if (missing.length > 0) {
      warnings.push(
        [
          ...
          `  Verify sessions in store: ${formatCliCommand(`daisyclaw sessions --store "${absoluteStorePath}"`)}`,
          `  Preview cleanup impact: ${formatCliCommand(`daisyclaw sessions cleanup --store "${absoluteStorePath}" --dry-run`)}`,
          `  Prune missing entries: ${formatCliCommand(`daisyclaw sessions cleanup --store "${absoluteStorePath}" --enforce --fix-missing`)}`,
        ].join("\n"),
      );
    }
    ...
    if (wedgedSubagentSessions.length > 0) {
      ...
      warnings.push(
        [
          ...
          "  DaisyClaw will not auto-resume these child sessions on restart; reconcile their task records instead.",
          `  Examples: ${wedgedSubagentSessions
            .slice(0, 3)
            .map(([key]) => key)
            .join(", ")}`,
          `  Fix: ${formatCliCommand("daisyclaw tasks maintenance --apply")}`,
        ].join("\n"),
      );
...
export function collectWorkspaceBackupTip(workspaceDir: string): string | null {
  ...
  return [
    ...
    "- Keep ~/.daisyclaw out of git; it contains credentials and session history.",
