...
import { closeDaisyClawStateDatabaseForTest } from "../state/daisyclaw-state-db.js";
...
vi.mock("../channels/plugins/bundled.js", async () => {
  ...
  function resolveTelegramAccountId(cfg: DaisyClawConfig): string {
  ...
  function detectTelegramAllowFromMigration(params: {
    ...
    const root = params.env.DAISYCLAW_STATE_DIR;
    if (!root) {
      return [];
    }
  ...
  return {
    ...
    listBundledChannelLegacyStateMigrationDetectors: vi.fn(() => [
      ...
      ({ cfg, env }: { cfg: DaisyClawConfig; env: NodeJS.ProcessEnv }) =>
...
async function makeTempRoot() {
  const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "daisyclaw-doctor-"));
  tempRoots.push(root);
  return root;
}
...
async function runTelegramAllowFromMigration(params: { root: string; cfg: DaisyClawConfig }) {
  const oauthDir = ensureCredentialsDir(params.root);
  writeLegacyTelegramAllowFromStore(oauthDir);
  const detected = await detectLegacyStateMigrations({
    cfg: params.cfg,
    env: { DAISYCLAW_STATE_DIR: params.root } as NodeJS.ProcessEnv,
  });
...
async function runLegacyStateMigrationsForRoot(root: string) {
  const detected = await detectLegacyStateMigrations({
    cfg: {},
    env: { DAISYCLAW_STATE_DIR: root } as NodeJS.ProcessEnv,
  });
...
function getStateDirMigrationPaths(root: string) {
  return {
    targetDir: path.join(root, ".daisyclaw"),
...
describe("doctor legacy state migrations", () => {
      ...
      expect(() =>
        db.exec(`
          INSERT INTO agent_databases (
            ...
            'worker-1',
            '/relocated/worker-1/daisyclaw-agent.sqlite',
            1,
            20,
            30
          )
  ...
  it("migrates legacy ACP metadata from sessions.json into shared SQLite", async () => {
    ...
    const sqlite = requireNodeSqlite();
    const db = new sqlite.DatabaseSync(path.join(root, "state", "daisyclaw.sqlite"));
    try {
      const row = db
        .prepare(
  ...
  it("auto-migrates the plugin-state sidecar when custom agent dirs skip session migration", async () => {
    ...
    const result = await autoMigrateLegacyState({
      cfg: {},
      env: {
        DAISYCLAW_STATE_DIR: root,
        DAISYCLAW_AGENT_DIR: path.join(root, "custom-agent"),
  ...
  it("migrates default exec approvals into an explicit state dir", async () => {
    ...
    const sourcePath = path.join(root, ".daisyclaw", "exec-approvals.json");
    const legacySocketPath = path.join(root, ".daisyclaw", "exec-approvals.sock");
    ...
    const detected = await detectLegacyStateMigrations({
      cfg: {},
      env: { DAISYCLAW_STATE_DIR: stateDir } as NodeJS.ProcessEnv,
      homedir: () => root,
    });
  ...
  it("keeps task sidecars when shared state already has conflicting task rows", async () => {
    ...
    await withStateDir(root, async () => {
      const sqlite = requireNodeSqlite();
      const sharedPath = path.join(root, "state", "daisyclaw.sqlite");
  ...
  it("honors session.mainKey when seeding the direct-chat bucket", async () => {
    const root = await makeTempRoot();
    const cfg: DaisyClawConfig = { session: { mainKey: "work" } };
    writeLegacySessionsFixture({
      root,
      sessions: {
        "+1555": { sessionId: "a", updatedAt: 10 },
