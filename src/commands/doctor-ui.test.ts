...
function issue(overrides: Partial<UiProtocolFreshnessIssue> = {}): UiProtocolFreshnessIssue {
  return {
    kind: "missing-assets",
    root: "/repo/daisyclaw",
    uiIndexPath: "/repo/daisyclaw/dist/control-ui/index.html",
    canBuild: true,
    ...overrides,
  } as UiProtocolFreshnessIssue;
}

async function createDaisyClawRoot(): Promise<string> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-doctor-ui-"));
  tempRoots.push(root);
  await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "daisyclaw" }));
...
describe("UI protocol freshness health mapping", () => {
  ...
  it("maps missing UI assets to a structured finding and dry-run effect", () => {
    ...
    expect(uiProtocolFreshnessIssueToHealthFinding(current)).toEqual(
      expect.objectContaining({
        ...
        severity: "warning",
        path: "/repo/daisyclaw/dist/control-ui/index.html",
        fixHint: expect.stringContaining("daisyclaw doctor --fix"),
      }),
    );
  ...
  it("maps stale UI assets to rebuild effects without file diffs", () => {
    ...
    expect(finding.fixHint).toContain("daisyclaw doctor --fix --force");
    expect(uiProtocolFreshnessIssueToRepairEffects(current)).toEqual([
      {
        kind: "process",
  ...
  it("does not report stale assets when git finds no schema changes", async () => {
    const root = await createDaisyClawRoot();
