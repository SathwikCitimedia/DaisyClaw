...
vi.mock("node:child_process", async () => {
  const { mockNodeChildProcessSpawnSync } = await import("daisyclaw/plugin-sdk/test-node-mocks");
...
describe("doctor WhatsApp responsiveness", () => {
  ...
  it("lists only verified local TUI processes", () => {
    spawnSyncMock.mockReturnValue({
      ...
      stdout: [
        " 101 daisyclaw-tui",
        " 102 /usr/bin/node /usr/lib/node_modules/daisyclaw/dist/index.js gateway --port 18789",
        " 103 daisyclaw channels",
        " 104 daisyclaw tui --local",
        " 105 /usr/bin/daisyclaw chat",
        " 106 helper --note 'daisyclaw tui'",
        " 107 daisyclaw-helper daisyclaw terminal",
        " 108 daisyclaw --flag tui",
      ].join("\n"),
    });

    expect(listLocalTuiProcesses()).toEqual([
      { pid: 101, command: "daisyclaw-tui" },
      { pid: 104, command: "daisyclaw tui --local" },
      { pid: 105, command: "/usr/bin/daisyclaw chat" },
    ]);
  });
  ...
  it("terminates stale local TUI processes with a kill fallback", async () => {
    ...
    await expect(
      terminateLocalTuiProcesses({
        processes: [{ pid: 101, command: "daisyclaw-tui" }],
        controller,
        graceMs: 0,
      }),
  ...
  it("warns and repairs local TUI pressure when WhatsApp is enabled and the gateway is degraded", async () => {
    ...
    const cfg = { channels: { whatsapp: { enabled: true } } } as DaisyClawConfig;
    ...
    await noteWhatsappResponsivenessHealth({
      ...
      shouldRepair: true,
      listLocalTuiProcesses: () => [{ pid: 101, command: "daisyclaw-tui" }],
