...
function isLocalTuiCommand(command: string): boolean {
  ...
  if (executable === "daisyclaw-tui") {
    return true;
  }
  return executable === "daisyclaw" && LOCAL_TUI_SUBCOMMANDS.has(argv[1] ?? "");
}
...
/** Lists local DaisyClaw TUI processes that can contend with gateway responsiveness. */
...
function hasWhatsappEnabled(cfg: DaisyClawConfig): boolean {
...
export async function noteWhatsappResponsivenessHealth(params: {
  ...
  if (gatewayDegraded && tuiProcesses.length > 0) {
    ...
    if (params.shouldRepair) {
      ...
      warnings.push(
        `Fix: close those TUI sessions, or run ${formatCliCommand("daisyclaw doctor --fix")}.`,
      );
    }
  }
