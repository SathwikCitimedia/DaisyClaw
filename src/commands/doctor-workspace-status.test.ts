...
async function runNoteWorkspaceStatusForTest(
  ...
  const cfg: DaisyClawConfig = opts?.cfg ?? {};
...
describe("noteWorkspaceStatus", () => {
  ...
  it("surfaces active official managed plugin version drift", async () => {
    ...
    try {
      ...
      expect(body).toContain("1 active official plugin not on DaisyClaw 2026.6.1");
      expect(body).toContain("codex: 2026.5.30-beta.1 (npm) -> expected 2026.6.1");
      expect(body).toContain("daisyclaw plugins update codex");
      expect(body).toContain("daisyclaw gateway restart");
    } finally {
      noteSpy.mockRestore();
    }
  });
  ...
  it("adds TaskFlow recovery hints for broken blocked flows", async () => {
    ...
    try {
      ...
      expect(body).toContain("daisyclaw tasks flow show <flow-id>");
    } finally {
      noteSpy.mockRestore();
    }
  });
