...
function noteFlowRecoveryHints() {
  ...
  note(
    [
      ...
      `Inspect: ${formatCliCommand("daisyclaw tasks flow show <flow-id>")}`,
      `Cancel: ${formatCliCommand("daisyclaw tasks flow cancel <flow-id>")}`,
    ]
...
function notePluginVersionDrift(drift: PluginVersionDriftReport | undefined) {
  ...
  const lines = [
    ...
    } not on DaisyClaw ${drift.gatewayVersion}`,
    ...
    singleDrift
      ? `Fix: ${formatCliCommand(
          `daisyclaw plugins update ${singleDrift.pluginId}`,
        )} && ${formatCliCommand("daisyclaw gateway restart")}.`
      : `Fix: ${formatCliCommand(
          "daisyclaw plugins update <plugin-id>",
        )} for each drifted plugin, then ${formatCliCommand("daisyclaw gateway restart")}.`,
  ];
  note(lines.join("\n"), "Plugin version drift");
}

/** Emits workspace, skills, plugin, and TaskFlow recovery status notes for doctor. */
export function noteWorkspaceStatus(cfg: DaisyClawConfig, options: NoteWorkspaceStatusOptions = {}) {
