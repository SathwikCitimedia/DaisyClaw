...
export const resolveDaisyClawPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadDaisyClawPlugins: () => createEmptyPluginRegistry(),
...
vi.mock("../config/config.js", async () => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/daisyclaw.json",
    createConfigIO,
...
vi.mock("daisyclaw/plugin-sdk/provider-auth", () => ({
  isNonSecretApiKeyMarker: () => false,
}));

vi.mock("daisyclaw/plugin-sdk/provider-model-shared", () => ({
  DEFAULT_CONTEXT_TOKENS: 32768,
  normalizeProviderId: (value: string) => normalizeLowercaseStringOrEmpty(value),
}));

vi.mock("daisyclaw/plugin-sdk/provider-stream-shared", () => ({
...
vi.mock("../infra/daisyclaw-root.js", () => ({
  ...
  resolveDaisyClawPackageRootSync: vi.fn(() => "/tmp/daisyclaw"),
}));
...
beforeEach(() => {
  ...
  resolveDaisyClawPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.DAISYCLAW_STATE_DIR;
  originalUpdateInProgress = process.env.DAISYCLAW_UPDATE_IN_PROGRESS;
  process.env.DAISYCLAW_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-doctor-state-"));
