...
describe("doctor command", () => {
  ...
  it("migrates anthropic oauth config profile id when only email profile exists", async () => {
    ...
    const previousConfigWriteSupport =
      process.env.DAISYCLAW_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE;
    process.env.DAISYCLAW_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE = "1";
    try {
      await doctorCommand(createDoctorRuntime(), { yes: true });
    } finally {
      if (previousConfigWriteSupport === undefined) {
        delete process.env.DAISYCLAW_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE;
      } else {
        process.env.DAISYCLAW_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE =
