...
async function makeTempStorePath() {
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-doctor-cron-"));
...
function createCronConfig(storePath: string): DaisyClawConfig {
  return {
    cron: {
      store: storePath,
...
function insertEarlySQLiteCronRow(
  ...
  runDaisyClawStateWriteTransaction(({ db }) => {
    db.prepare(
...
describe("maybeRepairLegacyCronStore", () => {
  ...
  it("rewrites stale managed dreaming jobs to the isolated agentTurn shape", async () => {
    ...
    expect(payload.message).toBe("__daisyclaw_memory_core_short_term_promotion_dream__");
    ...
    const warning = await collectLegacyWhatsAppCrontabHealthWarning({
      ...
      readCrontab: async () => ({
        stdout: [
          ...
          "*/5 * * * * ~/.daisyclaw/bin/ensure-whatsapp.sh >> ~/.daisyclaw/logs/whatsapp-health.log 2>&1",
          "0 9 * * * /usr/bin/true",
          "",
        ].join("\n"),
      }),
  ...
  it("ignores malformed crontab output instead of crashing", async () => {
    ...
    await expect(
      noteLegacyWhatsAppCrontabHealthCheck({
        platform: "linux",
        readCrontab: async () => ({
          stdout: { lines: ["*/5 * * * * ~/.daisyclaw/bin/ensure-whatsapp.sh"] },
        }),
      }),
    ).resolves.toBeUndefined();
