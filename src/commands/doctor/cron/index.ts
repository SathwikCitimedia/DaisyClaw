...
export async function maybeRepairLegacyCronStore(params: {
  ...
  try {
    ...
    note(
      [
        ...
        `- ${reason}`,
        `Fix the file's permissions or contents and re-run ${formatCliCommand("daisyclaw doctor")}; later health checks will continue.`,
      ].join("\n"),
      "Cron",
    );
    return;
  ...
  if (rawJobs.length === 0) {
    ...
    note(
      [
        ...
        ...previewLines,
        `Repair with ${formatCliCommand("daisyclaw doctor --fix")} to finish the migration.`,
      ].join("\n"),
      "Cron",
    );
  ...
  note(
    [
      ...
      ...previewLines,
      `Repair with ${formatCliCommand("daisyclaw doctor --fix")} to normalize the store before the next scheduler run.`,
    ].join("\n"),
    "Cron",
  );
