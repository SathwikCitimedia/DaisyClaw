...
describe("normalizeStoredCronJobs", () => {
  ...
  it("converts legacy agent command prompts into command cron payloads", () => {
    const command =
      "cd /home/daisyclaw/.razor/quant && ./scripts/system/run_position_control.sh --write-card --silent-token NO_REPLY";
    const { job, result } = normalizeOneJob(
      makeLegacyJob({
        ...
        payload: {
          ...
          message: [
            ...
            "Command to run:",
            `- command: ${command}`,
            "- workdir: /home/daisyclaw/.razor/quant",
            "- background: false",
            "- timeout: 840",
            "",
    ...
    expect(payload).toEqual({
      kind: "command",
      argv: ["sh", "-lc", command],
      cwd: "/home/daisyclaw/.razor/quant",
      timeoutSeconds: 900,
    });
  });
      ...
      makeLegacyJob({
        ...
        payload: {
          ...
          message: [
            "Command to run:",
            `- command: ${command}`,
            "- workdir: /home/daisyclaw/.razor/clawd",
          ].join("\n"),
