...
export function noteCronModelOverrides(params: {
  ...
  lines.push(
    `Review with ${formatCliCommand("daisyclaw cron list")} and ${formatCliCommand("daisyclaw cron show <job-id>")}; remove \`payload.model\` from jobs that should inherit the default.`,
  );
...
export async function collectLegacyWhatsAppCrontabHealthWarning(
  ...
  return [
    ...
    "`~/.daisyclaw/bin/ensure-whatsapp.sh` is not maintained by current DaisyClaw and can misreport `Gateway inactive` from cron when the systemd user bus environment is missing.",
    `Remove the stale crontab entry with ${formatCliCommand("crontab -e")}; use ${formatCliCommand("daisyclaw channels status --probe")}, ${formatCliCommand("daisyclaw doctor")}, and ${formatCliCommand("daisyclaw gateway status")} for current health checks.`,
