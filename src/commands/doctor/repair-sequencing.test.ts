...
const mocks = vi.hoisted(() => ({
  ...
  maybeRepairManagedNpmDaisyClawPeerLinks: vi.fn(),
...
vi.mock("../doctor-plugin-registry.js", () => ({
  maybeRepairManagedNpmDaisyClawPeerLinks: mocks.maybeRepairManagedNpmDaisyClawPeerLinks,
...
vi.mock("./shared/channel-doctor.js", () => ({
  collectChannelDoctorRepairMutations: ({ cfg }: { cfg: DaisyClawConfig }) => {
...
vi.mock("./shared/empty-allowlist-scan.js", () => ({
  scanEmptyAllowlistPolicyWarnings: (cfg: DaisyClawConfig) =>
    cfg.channels?.signal
...
vi.mock("./shared/allowlist-policy-repair.js", () => ({
  maybeRepairAllowlistPolicyAllowFrom: async (cfg: DaisyClawConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
vi.mock("./shared/bundled-plugin-load-paths.js", () => ({
  maybeRepairBundledPluginLoadPaths: (cfg: DaisyClawConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
vi.mock("./shared/invalid-plugin-config.js", () => ({
  maybeRepairInvalidPluginConfig: (cfg: DaisyClawConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
vi.mock("./shared/legacy-tools-by-sender.js", () => ({
  maybeRepairLegacyToolsBySenderKeys: (cfg: DaisyClawConfig) => {
...
vi.mock("./shared/exec-safe-bins.js", () => ({
  maybeRepairExecSafeBinProfiles: (cfg: DaisyClawConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
describe("doctor repair sequencing", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.applyPluginAutoEnable.mockImplementation((params: { config: DaisyClawConfig }) => ({
    ...
    mocks.maybeRepairGroupAllowFromFallback.mockImplementation((cfg: DaisyClawConfig) => ({
      config: cfg,
      changes: [],
    }));
    mocks.maybeRepairManagedNpmDaisyClawPeerLinks.mockResolvedValue(false);
    ...
    mocks.maybeRepairOpenAICodexAuthConfig.mockImplementation((cfg: DaisyClawConfig) => ({
      changes: [],
      config: cfg,
      warnings: [],
    }));
    ...
    mocks.maybeRepairOpenPolicyAllowFrom.mockImplementation((cfg: DaisyClawConfig) => ({
      config: cfg,
      changes: [],
    }));
    ...
    mocks.maybeRepairStalePluginConfig.mockImplementation((cfg: DaisyClawConfig) => ({
      config: cfg,
      changes: [],
    }));
  });
  ...
  it("applies ordered repairs and sanitizes empty-allowlist warnings", async () => {
    const result = await runDoctorRepairSequence({
      ...
      doctorFixCommand: "daisyclaw doctor --fix",
    });
  ...
  it("repairs managed npm plugin drift before missing plugin install repair", async () => {
    ...
    mocks.maybeRepairManagedNpmDaisyClawPeerLinks.mockImplementation(async () => {
      events.push("daisyclaw-peer-links");
      return true;
    });
    ...
    expect(events).toEqual(["bundled-shadow-cleanup", "daisyclaw-peer-links", "missing-installs"]);
    ...
    expect(mocks.maybeRepairManagedNpmDaisyClawPeerLinks).toHaveBeenCalledOnce();
    const peerLinkCall = mocks.maybeRepairManagedNpmDaisyClawPeerLinks.mock.calls[0]?.[0];
  ...
  it("auto-enables newly installed configured plugins after doctor repair", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: ['Installed missing configured plugin "brave" from @daisyclaw/brave-plugin.'],
      warnings: [],
    });
    mocks.applyPluginAutoEnable.mockImplementationOnce((params: { config: DaisyClawConfig }) => ({
      config: {
        ...params.config,
        plugins: {
    ...
    expect(result.changeNotes).toStrictEqual([
      'Installed missing configured plugin "brave" from @daisyclaw/brave-plugin.',
  ...
  it("moves legacy Codex routes to canonical OpenAI before missing plugin install repair", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockImplementationOnce(
      async (params: { cfg: DaisyClawConfig }) => {
  ...
  it("runs group allowFrom fallback migration after open-policy allowFrom repair", async () => {
    const events: string[] = [];
    mocks.maybeRepairOpenPolicyAllowFrom.mockImplementationOnce((cfg: DaisyClawConfig) => {
    ...
    mocks.maybeRepairGroupAllowFromFallback.mockImplementationOnce((cfg: DaisyClawConfig) => {
  ...
  it("does not remove deferred configured plugins during the package update doctor pass", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: [
        'Skipped package-manager repair for configured plugin "brave" during package update; rerun "daisyclaw doctor --fix" after the update completes.',
      ],
      warnings: [],
    });
    const result = await runDoctorRepairSequence({
  ...
  it("preserves configured plugins when their install repair fails", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: [],
      warnings: [
        'Failed to install missing configured plugin "brave" from @daisyclaw/brave-plugin: package install failed',
      ],
  ...
  it("preserves configured channels when their install repair fails", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: [],
      warnings: [
        'Failed to install missing configured channel plugin "whatsapp" from @daisyclaw/whatsapp: package install failed',
      ],
