...
import { bundledDistPluginRootAt, bundledPluginRootAt } from "daisyclaw/plugin-sdk/test-fixtures";
...
describe("bundled plugin load path repair", () => {
  beforeEach(() => {
    const packageRoot = "/app/node_modules/daisyclaw";
  ...
  it("detects legacy bundled plugin paths that still point at source extensions", () => {
    const packageRoot = path.resolve("app-node-modules", "daisyclaw");
  ...
  it("removes stale bundled paths from old versioned DaisyClaw package roots", () => {
    const currentPackageRoot = path.resolve("node_modules", "daisyclaw");
    const stalePackageRoot = path.resolve(
      "pnpm-global",
      ".pnpm",
      "daisyclaw@2026.3.28_@napi-rs+canvas@0.1.97",
      "node_modules",
      "daisyclaw",
    );
  ...
  it("removes stale legacy bundled paths from old versioned DaisyClaw package roots", () => {
    const currentPackageRoot = path.resolve("node_modules", "daisyclaw");
    const stalePackageRoot = path.resolve(
      "pnpm-global",
      ".pnpm",
  ...
  it("formats a doctor hint for legacy bundled plugin paths", () => {
    ...
    expect(warnings).toEqual([
      `- plugins.load.paths: bundled plugin path "${legacyPath}" still aliases feishu; DaisyClaw loads the packaged bundled plugin from "${bundledPath}".`,
      '- Run "daisyclaw doctor --fix" to remove these redundant bundled plugin paths.',
    ]);
  });

  it("ignores bundled plugins that already resolve to source extensions", () => {
    const sourcePath = path.resolve("repo", "daisyclaw", "extensions", "feishu");
    vi.spyOn(bundledSources, "resolveBundledPluginSources").mockReturnValue(
