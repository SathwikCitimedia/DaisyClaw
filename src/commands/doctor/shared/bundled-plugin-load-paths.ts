...
function resolveBundledWorkspaceDir(cfg: DaisyClawConfig): string | undefined {
  return resolveAgentWorkspaceDir(cfg, resolveDefaultAgentId(cfg)) ?? undefined;
}

function isDaisyClawNodeModulesPackageRoot(packageRoot: string): boolean {
  ...
  return packageDir === "daisyclaw" && parentDir === "node_modules";
}

/** Find configured plugin load paths that alias bundled plugins already shipped by DaisyClaw. */
  ...
  for (const rawPath of rawPaths) {
    ...
    if (!match) {
      ...
      const oldPackageMatch =
        // Only rewrite paths rooted in the installed DaisyClaw package; user plugin paths stay intact.
        oldPackageRoot && oldBundledLeaf && isDaisyClawNodeModulesPackageRoot(oldPackageRoot)
...
export function collectBundledPluginLoadPathWarnings(params: {
  ...
  const lines = params.hits.map(
    (hit) =>
      `- ${hit.pathLabel}: bundled plugin path "${hit.fromPath}" still aliases ${hit.pluginId}; DaisyClaw loads the packaged bundled plugin from "${hit.toPath}".`,
  );
