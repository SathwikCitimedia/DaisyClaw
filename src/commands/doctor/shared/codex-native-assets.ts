...
function isCodexRuntimeConfigured(cfg: DaisyClawConfig, _env: NodeJS.ProcessEnv): boolean {
  return collectConfiguredAgentHarnessRuntimes(cfg).includes("codex");
}

function isCodexPluginConfigured(cfg: DaisyClawConfig): boolean {
...
function shouldScanCodexNativeAssets(cfg: DaisyClawConfig, env: NodeJS.ProcessEnv): boolean {
...
export async function collectCodexNativeAssetInfoNotes(params: {
  ...
  return [
    [
      "- Personal Codex CLI assets were found, but native Codex-mode DaisyClaw agents use isolated per-agent Codex homes.",
      ...
      "- If the Codex plugin is not installed, run `daisyclaw plugins install npm:@daisyclaw/codex` first. Then run `daisyclaw migrate plan codex` to inventory them. Applying that migration copies skills into the current DaisyClaw agent workspace; Codex plugins, hooks, and config stay manual-review only.",
    ].join("\n"),
  ];
}
