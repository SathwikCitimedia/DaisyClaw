...
describe("collectCodexRouteWarnings", () => {
  ...
  it("ignores DAISYCLAW_AGENT_RUNTIME when reporting legacy model refs", () => {
    const warnings = collectCodexRouteWarnings({
      cfg: {
        agents: {
          defaults: {
  ...
  it("warns when Codex app-server command includes inline arguments", () => {
    ...
    expect(warnings).toStrictEqual([
      [
        ...
        '- plugins.entries.codex.config.appServer.command: "node C:\\Users\\me\\.daisyclaw\\npm\\node_modules\\@openai\\codex\\bin\\codex.js" starts with "node" and embeds "C:\\Users\\me\\.daisyclaw\\npm\\node_modules\\@openai\\codex\\bin\\codex.js". The command field must be only the executable path.',
  ...
  it("warns when Codex runtime has DaisyClaw compaction summarizer overrides", () => {
    const warnings = collectCodexRouteWarnings({
      cfg: {
        agents: {
          defaults: {
          ...
          list: [
            {
              ...
              model: "anthropic/claude-sonnet-4-6",
              models: {
                "anthropic/claude-sonnet-4-6": { agentRuntime: { id: "daisyclaw" } },
              },
            },
          ],
        },
  ...
  it("keeps the Codex plugin disabled when OpenAI routes explicitly use the DaisyClaw runtime", () => {
    const result = maybeRepairCodexRoutes({
      cfg: {
        plugins: {
          entries: {
  ...
  it("keeps the Codex plugin disabled when an auth-profiled OpenAI route explicitly uses the DaisyClaw runtime", () => {
    const result = maybeRepairCodexRoutes({
      cfg: {
        plugins: {
          entries: {
  ...
  it("keeps repaired OpenAI refs on Codex runtime even when the OpenAI provider is otherwise DaisyClaw/API-key routed", () => {
    const result = maybeRepairCodexRoutes({
      cfg: {
        models: {
          providers: {
  ...
  it("preserves provider-level legacy runtime pins while repairing default legacy refs", () => {
    ...
    expect(result.changes.join("\n")).toContain(
      'Set agents.defaults.models.openai/gpt-5.4.agentRuntime.id to "daisyclaw"',
    );
    expect(
      resolveAgentHarnessPolicy({
        provider: "openai",
  ...
  it("preserves legacy provider catalog runtime pins while repairing listed-agent legacy refs", () => {
    ...
    expect(result.changes.join("\n")).toContain(
      'Set agents.list.worker.models.openai/gpt-5.4.agentRuntime.id to "daisyclaw"',
    );
    expect(
      resolveAgentHarnessPolicy({
        provider: "openai",
  ...
  it("preserves explicit DaisyClaw runtime pins while repairing legacy session routes", () => {
    const store: Record<string, SessionEntry> = {
      main: {
        sessionId: "s1",
        updatedAt: 1,
  ...
  it("clears stale Codex overrides while preserving explicit DaisyClaw session pins", () => {
    const store: Record<string, SessionEntry> = {
      main: {
        sessionId: "s1",
        updatedAt: 1,
  ...
  it("preserves canonical OpenAI sessions that are explicitly pinned to DaisyClaw", () => {
    const store: Record<string, SessionEntry> = {
      main: {
        sessionId: "s1",
        updatedAt: 1,
        ...
        agentRuntimeOverride: "daisyclaw",
        authProfileOverride: "openai:work",
      },
    };
    ...
    expect(result).toEqual({ changed: false, sessionKeys: [] });
    expect(store.main.updatedAt).toBe(1);
    expect(store.main.agentHarnessId).toBe("daisyclaw");
    expect(store.main.agentRuntimeOverride).toBe("daisyclaw");
  ...
  it("preserves an explicit non-default agentRuntime pin on the legacy model entry during migration (#84038)", () => {
    ...
    expect(migrated?.agentRuntime).toEqual({ id: "daisyclaw" });
    expect(result.cfg.agents?.defaults?.models?.["openai-codex/gpt-5.4"]).toBeUndefined();
  });
