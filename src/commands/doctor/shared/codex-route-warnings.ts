...
import { AGENT_MODEL_CONFIG_KEYS } from "@daisyclaw/model-catalog-core/configured-model-refs";
import { normalizeProviderId } from "@daisyclaw/model-catalog-core/provider-id";
import { asOptionalRecord as asMutableRecord } from "@daisyclaw/normalization-core/record-coerce";
import { normalizeOptionalLowercaseString as normalizeString } from "@daisyclaw/normalization-core/string-coerce";
...
function resolveImplicitDefaultAgentModelRef(cfg: DaisyClawConfig): string {
...
function collectConfigModelRefs(cfg: DaisyClawConfig): CodexRouteHit[] {
...
function codexPluginAllowlistIsRestrictive(cfg: DaisyClawConfig): boolean {
  const allow = cfg.plugins?.allow;
  return Array.isArray(allow) && allow.length > 0 && !pluginIdListIncludes(allow, "codex");
}

function isCodexPluginUnavailableByConfig(cfg: DaisyClawConfig): boolean {
...
function codexPluginIsBlockedOutsideEntry(cfg: DaisyClawConfig): boolean {
...
function collectDisabledCodexPluginRouteHits(cfg: DaisyClawConfig): DisabledCodexPluginRouteHit[] {
...
}): { cfg: DaisyClawConfig; changes: string[] } {
...
function clearConfigLegacyAgentRuntimePolicies(cfg: DaisyClawConfig): string[] {
  const changes: string[] = [];
...
function configRepairWouldClearLegacyRuntimePins(params: { cfg: DaisyClawConfig }): boolean {
...
function rewriteConfigModelRefs(params: { cfg: DaisyClawConfig }): ConfigRouteRepairResult {
...
function formatUnsupportedCompactionWarning(params: {
  ...
  return [
    "- Codex runtime uses native server-side compaction and ignores DaisyClaw compaction summarizer overrides.",
    ...params.hits.map(
...
function formatLegacyLosslessCompactionWarning(params: {
  ...
  return [
    ...
    ...configLines,
    params.canAutoFix
      ? "- Run `daisyclaw doctor --fix`: it migrates legacy Lossless compaction config to the Lossless context-engine slot."
...
function formatDisabledCodexPluginWarning(params: {
  ...
  const fixHint = params.blockedOutsideEntry
    ? "- Enable plugin loading and remove `codex` from plugins.deny, or set the affected OpenAI models to an DaisyClaw runtime policy."
    : "- Run `daisyclaw doctor --fix`: it enables plugins.entries.codex, or set the affected OpenAI models to an DaisyClaw runtime policy.";
  return [
...
function collectCodexAppServerCommandWarnings(cfg: DaisyClawConfig): string[] {
  const plugins = asMutableRecord(cfg.plugins);
  const entries = asMutableRecord(plugins?.entries);
  ...
  if (hits.length > 0) {
    warnings.push(
      [
        ...
        "- Run `daisyclaw doctor --fix`: it rewrites configured model refs and stale sessions to `openai/*`, moves Codex intent to provider/model runtime policy, and clears old whole-agent runtime pins.",
      ].join("\n"),
    );
  }
  ...
  if (fixableHits.length > 0) {
    warnings.push(
      formatUnsupportedCompactionWarning({
        hits: fixableHits,
        fixHint:
          "- Run `daisyclaw doctor --fix`: it removes unsupported Codex compaction overrides.",
      }),
    );
  }
  return warnings;
...
}): { cfg: DaisyClawConfig; warnings: string[]; changes: string[] } {
  const hits = collectConfigModelRefs(params.cfg);
  const disabledCodexPluginHits = collectDisabledCodexPluginRouteHits(params.cfg);
  const ignoreLegacyAgentRuntimePins = configRepairWouldClearLegacyRuntimePins(params);
  const unsupportedCompactionOverrides = collectUnsupportedCodexCompactionOverrides({
...
function clearStaleSessionRuntimePins(entry: SessionEntry): boolean {
  ...
  let changed = false;
  if (entry.agentHarnessId !== undefined && harnessRuntime !== "daisyclaw") {
    delete entry.agentHarnessId;
    changed = true;
  }
  if (entry.agentRuntimeOverride !== undefined && overrideRuntime !== "daisyclaw") {
        ...
        stale.length > 0
          ? [
              [
                ...
                "- Run `daisyclaw doctor --fix` to rewrite stale session model/provider pins across all agent session stores.",
              ].join("\n"),
            ]
          : [],
      changes: [],
