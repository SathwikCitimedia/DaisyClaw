...
function runtimeHostCandidate(params: {
  ...
  if (runtimeId === "daisyclaw" || runtimeId === "auto") {
    return { runtimeId, host: DAISYCLAW_EMBEDDED_CONTEXT_ENGINE_HOST, paths: params.paths };
  }
  if (runtimeId === "codex") {
...
function selectedContextEngineSlotId(cfg: DaisyClawConfig): string {
...
export async function maybeRepairContextEngineHostCompatibility(params: {
  ...
  doctorFixCommand: string;
  env?: NodeJS.ProcessEnv;
}): Promise<{ config: DaisyClawConfig; changes: string[]; warnings?: string[] }> {
  const resolved = await resolveSelectedContextEngineInfo(params);
  if (!resolved.info) {
