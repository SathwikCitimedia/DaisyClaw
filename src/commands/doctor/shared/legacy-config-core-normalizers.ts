...
export function normalizeLegacyCommandsConfig(
  ...
  return {
    ...cfg,
    commands: commands as DaisyClawConfig["commands"],
  };
}
...
export function normalizeLegacyBrowserConfig(
  ...
  return {
    ...cfg,
    browser: browser as DaisyClawConfig["browser"],
  };
}
...
export function seedMissingDefaultAccountsFromSingleAccountBase(
  ...
  return {
    ...cfg,
    channels: nextChannels as DaisyClawConfig["channels"],
  };
}

type ModelProviderEntry = Partial<
  NonNullable<NonNullable<DaisyClawConfig["models"]>["providers"]>[string]
>;
type ModelsConfigPatch = Partial<NonNullable<DaisyClawConfig["models"]>>;
...
function normalizeLegacyCodexCliProviderRuntimePins(
  cfg: DaisyClawConfig,
  changes: string[],
): { config: DaisyClawConfig; changed: boolean } {
  ...
  return changed
    ? {
        config: {
          ...
          models: {
            ...rawModels,
            providers: nextProviders as NonNullable<DaisyClawConfig["models"]>["providers"],
          },
        },
        changed: true,
      }
...
export function normalizeLegacyRuntimeModelRefs(
  ...
  const nextCfg = changed
    ? {
        ...cfgWithProviders,
        agents: nextAgents as DaisyClawConfig["agents"],
      }
    : cfgWithProviders;
  return nextCfg;
}
...
export function normalizeLegacyNanoBananaSkill(
  ...
  if (!hasGoogleApiKey && legacyApiKey) {
    ...
    rawModels.providers = rawProviders as NonNullable<DaisyClawConfig["models"]>["providers"];
    next = {
      ...next,
      models: rawModels as DaisyClawConfig["models"],
    };
    changes.push(
...
export function normalizeLegacyMediaProviderOptions(
  ...
  return {
    ...
    tools: {
      ...cfg.tools,
      media: nextMedia as NonNullable<DaisyClawConfig["tools"]>["media"],
    },
  };
}
...
export function normalizeLegacyOllamaNativeNumCtxParams(
  ...
  type ProviderConfigMap = NonNullable<NonNullable<DaisyClawConfig["models"]>["providers"]>;
  for (const [providerId, rawProvider] of Object.entries(rawProviders)) {
    if (!isRecord(rawProvider)) {
      continue;
    }
