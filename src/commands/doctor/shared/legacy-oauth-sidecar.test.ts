...
async function writeLegacySidecarThatNeedsKeychain(): Promise<{
  ...
  const state = await createDaisyClawTestState({
    layout: "state-only",
    prefix: "daisyclaw-legacy-oauth-keychain-warn-",
    env: {
      DAISYCLAW_AGENT_DIR: undefined,
      DAISYCLAW_AUTH_PROFILE_SECRET_KEY: undefined,
    },
  });
  states.push(state);
...
describe("loadLegacyOAuthSidecarMaterial keychain-only headless warning", () => {
  ...
  function envWithoutVitestSignals(state: DaisyClawTestState): NodeJS.ProcessEnv {
  ...
  it("emits a single doctor-pointer warning when only Keychain can decrypt and prompts are disabled", async () => {
    ...
    expect(String(firstMessage)).toContain("daisyclaw doctor --fix");
