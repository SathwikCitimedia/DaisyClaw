...
function buildLegacyRealtimeTalkCompat(
  talk: Record<string, unknown>,
  normalizedTalk: NonNullable<DaisyClawConfig["talk"]>,
): Record<string, unknown> | undefined {
  ...
  return normalizeTalkSection({ realtime: compat } as DaisyClawConfig["talk"])?.realtime;
}
...
export function normalizeLegacyTalkConfig(cfg: DaisyClawConfig, changes: string[]): DaisyClawConfig {
  const rawTalk = cfg.talk;
  if (!isRecord(rawTalk)) {
    return cfg;
  }

  const normalizedTalk = normalizeTalkSection(rawTalk as DaisyClawConfig["talk"]) ?? {};
