...
const DAISYCLAW_BETA_COMPANION_VERSION_RE = /^(\d{4}\.[1-9]\d?\.[1-9]\d?)-beta\.[1-9]\d*$/;
const DAISYCLAW_STABLE_OR_BETA_COMPANION_VERSION_RE =
...
function addConfiguredAgentRuntimePluginIds(ids: Set<string>, cfg: DaisyClawConfig): void {
...
function collectConfiguredPluginIds(cfg: DaisyClawConfig): Set<string> {
...
function collectBlockedPluginIds(cfg: DaisyClawConfig): Set<string> {
  const ids = new Set<string>();
...
function collectConfiguredChannelIds(cfg: DaisyClawConfig, env?: NodeJS.ProcessEnv): Set<string> {
  const ids = new Set<string>();
...
function installedRuntimePackageVersionIsStale(params: {
  ...
  const comparison = compareDaisyClawReleaseVersions(params.installedVersion, params.currentVersion);
...
function betaCompanionMatchesCurrentStableVersion(params: {
  ...
  const installedBase = DAISYCLAW_BETA_COMPANION_VERSION_RE.exec(params.installedVersion)?.[1];
  const currentBase = DAISYCLAW_STABLE_OR_BETA_COMPANION_VERSION_RE.exec(params.currentVersion)?.[1];
...
/** Repair missing installs inferred from the current DaisyClaw config. */
...
async function repairMissingPluginInstalls(params: {
  ...
  if (shouldDeferConfiguredPluginInstallRepair(env)) {
    ...
    for (const pluginId of updateDeferredPluginIds) {
      ...
      changes.push(
        `Skipped package-manager repair for configured plugin "${pluginId}" during package update; rerun "daisyclaw doctor --fix" after the update completes.`,
      );
    }
  }
