...
function isRuntimeDependencyMarkerName(name: string): boolean {
  return (
    name === ".daisyclaw-runtime-deps.json" ||
    name === ".daisyclaw-runtime-deps-stamp.json" ||
    name.startsWith(".daisyclaw-runtime-deps-")
  );
}

function isInstallStageDebrisName(name: string): boolean {
  return /^\.daisyclaw-install-stage(?:-.+)?$/u.test(name);
}

function isLegacyDependencyDebrisName(name: string): boolean {
  return (
    isRuntimeDependencyMarkerName(name) ||
    name === ".daisyclaw-pnpm-store" ||
    name === ".daisyclaw-install-backups" ||
...
function collectExplicitStageTargets(env: NodeJS.ProcessEnv): CleanupTarget[] {
  return splitPathList(env.DAISYCLAW_PLUGIN_STAGE_DIR).map((entry) => ({
    kind: "explicit-stage",
...
async function hasDaisyClawRenameResidue(root: string): Promise<boolean> {
  const nodeModulesRoot = path.join(root, "node_modules");
  if (await isFile(path.join(nodeModulesRoot, ".daisyclaw-rename-tmp"))) {
    return true;
  }
  ...
  for (const entry of entries) {
    ...
    if (await isFile(path.join(entryPath, ".daisyclaw-rename-tmp"))) {
      return true;
    }
    ...
    for (const scopedEntry of scopedEntries) {
      ...
      if (await isFile(path.join(entryPath, scopedEntry.name, ".daisyclaw-rename-tmp"))) {
        return true;
      }
    }
  }
...
async function hasExplicitStageDebrisProof(root: string): Promise<boolean> {
  ...
  return await hasDaisyClawRenameResidue(root);
}
...
function filterLegacyStaleRootCandidates(
  ...
  for (const target of targets) {
    ...
    if (!cleanupRootPaths.some((rootPath) => isPathInsideRoot(targetPath, rootPath))) {
      warnings.push(
        `Skipped legacy plugin dependency state ${targetPath}: outside DaisyClaw cleanup roots`,
      );
      continue;
    }
    safeTargets.push({ ...target, path: targetPath });
...
async function resolveSafeRemovalTarget(
  ...
  if (!cleanupRoots.some((root) => isPathInsideRoot(realPath, root.realPath))) {
    return {
      warning: `Skipped legacy plugin dependency state ${targetPath}: resolved outside DaisyClaw cleanup roots`,
    };
  }
  return { target: targetPath };
}
...
/** Remove legacy plugin dependency state under trusted DaisyClaw cleanup roots. */
