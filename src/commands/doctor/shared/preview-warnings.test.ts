...
vi.mock("./stale-plugin-config.js", () => ({
  scanStalePluginConfig: (cfg: {
    ...
    if (cfg.channels?.["daisyclaw-weixin"]) {
      hits.push({ id: "daisyclaw-weixin", surface: "channel" });
    }
    return hits.filter(
...
describe("doctor preview warnings", () => {
  ...
  it("routes personal Codex asset notices to info instead of warnings", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-preview-codex-assets-"));
    tempRoots.add(root);
  ...
  it("includes stale plugin config warnings", async () => {
    ...
    expect(warning).toContain('Run "daisyclaw doctor --fix"');
  ...
  it("includes stale channel config warnings without plugin config", async () => {
    ...
    expectSingleWarningContaining(warnings, "channels.daisyclaw-weixin: dangling channel config");
  });
  ...
  it("includes stale OAuth profile shadow warnings", async () => {
    staleOAuthShadowState.warnings = [
      '- ~/.daisyclaw/agents/telegram/agent/auth-profiles.json has stale OAuth auth profile openai-codex:default. Run "daisyclaw doctor --fix".',
    ];
  ...
  it("warns but skips auto-removal when plugin discovery has errors", async () => {
    ...
    expect(warning).toContain('rerun "daisyclaw doctor --fix"');
  });
