...
import { isRecord as hasRecord } from "@daisyclaw/normalization-core/record-coerce";
...
function listAgentRecords(cfg: DaisyClawConfig): Record<string, unknown>[] {
  return Array.isArray(cfg.agents?.list) ? cfg.agents.list.filter(hasRecord) : [];
}

function hasChannels(cfg: DaisyClawConfig): boolean {
  return hasRecord(cfg.channels);
}

function hasPlugins(cfg: DaisyClawConfig): boolean {
  return hasRecord(cfg.plugins);
}

function hasPluginLoadPaths(cfg: DaisyClawConfig): boolean {
...
function hasSubagentAllowlistConfig(cfg: DaisyClawConfig): boolean {
...
function hasExplicitChannelPluginBlockerConfig(cfg: DaisyClawConfig): boolean {
...
function hasConfiguredSafeBins(cfg: DaisyClawConfig): boolean {
...
function sourceReplyRuntimeMayAllowMessageTool(cfg: DaisyClawConfig): boolean {
...
function resolveGroupVisibleReplyProvenance(cfg: DaisyClawConfig): {
...
export function collectVisibleReplyToolPolicyWarnings(cfg: DaisyClawConfig): string[] {
  const groupPolicy = resolveGroupVisibleReplyProvenance(cfg);
  ...
  if (groupPolicy.value === "message_tool") {
    ...
    warnings.push(
      `- ${groupPolicy.path} is set to "message_tool", but the message tool is unavailable for ${formatTargets(
        targets,
      )}; DaisyClaw falls back to automatic visible replies, so normal replies may post to the source chat. Enable the message tool or set ${groupPolicy.path} to "automatic".`,
    );
  }
  ...
  if (globalVisibleReplies === "message_tool" && groupPolicy.path !== "messages.visibleReplies") {
    ...
    warnings.push(
      `- messages.visibleReplies is set to "message_tool", but the message tool is unavailable for ${formatTargets(
        targets,
      )}; DaisyClaw falls back to automatic direct-chat replies, so normal replies may post to the source chat. Enable the message tool or set messages.visibleReplies to "automatic".`,
    );
  }
  return warnings;
}
...
export function collectChannelBoundMessageToolPolicyWarnings(cfg: DaisyClawConfig): string[] {
...
export function collectProfileConfiguredToolSectionWarnings(cfg: DaisyClawConfig): string[] {
  const warnings: string[] = [];
