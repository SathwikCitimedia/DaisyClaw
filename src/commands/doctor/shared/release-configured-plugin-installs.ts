...
import { normalizeNullableString as normalizeId } from "@daisyclaw/normalization-core/string-coerce";
...
import { compareDaisyClawVersions } from "../../../config/version.js";
...
function isPluginsGloballyDisabled(cfg: DaisyClawConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function isDenied(cfg: DaisyClawConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function collectBlockedPluginIds(cfg: DaisyClawConfig): string[] {
...
function isPluginEntryDisabled(cfg: DaisyClawConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function isChannelDisabled(cfg: DaisyClawConfig, channelId: string): boolean {
  const channels = asObjectRecord(cfg.channels);
  const entry = asObjectRecord(channels?.[channelId]);
  return entry?.enabled === false;
}

function isDisabled(cfg: DaisyClawConfig, pluginId: string): boolean {
...
function collectMaterialPluginEntryIds(cfg: DaisyClawConfig): string[] {
  const entries = asObjectRecord(cfg.plugins?.entries);
  if (!entries) {
    return [];
  }
...
function collectSlotPluginIds(cfg: DaisyClawConfig): string[] {
...
function collectConfiguredChannelIds(cfg: DaisyClawConfig, env: NodeJS.ProcessEnv): string[] {
  const ids = new Set<string>();
  const channels = asObjectRecord(cfg.channels);
  if (channels) {
...
function collectConfiguredProviderIds(cfg: DaisyClawConfig): Set<string> {
  const ids = new Set<string>();
...
function collectProviderPluginIds(cfg: DaisyClawConfig, env: NodeJS.ProcessEnv): string[] {
...
function collectWebSearchPluginIds(cfg: DaisyClawConfig): string[] {
...
function collectAcpRuntimePluginIds(cfg: DaisyClawConfig): string[] {
...
function collectAllowOnlyOfficialPluginIds(cfg: DaisyClawConfig): string[] {
...
function addEligiblePluginId(cfg: DaisyClawConfig, pluginIds: Set<string>, pluginId: string): void {
...
export function shouldRunConfiguredPluginInstallReleaseStep(params: {
  ...
  const currentComparedToRelease = compareDaisyClawVersions(
  ...
  const touchedComparedToRelease = compareDaisyClawVersions(params.touchedVersion, releaseVersion);
