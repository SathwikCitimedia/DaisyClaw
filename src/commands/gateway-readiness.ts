...
function printGatewayNotReadyHints(runtime: RuntimeEnv, reason: string): void {
  runtime.log(reason);
  runtime.log("Run `daisyclaw gateway status --deep` for details.");
  runtime.log("Run `daisyclaw gateway start` to start a managed gateway.");
  runtime.log("Run `daisyclaw gateway run` for a foreground gateway.");
}
...
export async function ensureGatewayReadyForOperation(
  ...
  const prompt = shouldInstall
    ? `Gateway is not installed. Install and start it now so DaisyClaw can ${options.operation}?`
    : `Gateway is not running. Start it now so DaisyClaw can ${options.operation}?`;
