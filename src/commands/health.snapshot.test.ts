  ...
  return {
    ...
    status: {
      ...
      probeAccount: async () => ({
        ok: false,
        error:
          "imsg cannot access /Users/alice/Library/Messages/chat.db. Grant Full Disk Access to the Gateway/launcher process and restart Gateway. privateApi=/tmp/daisyclaw/private.sock",
        privateApi: {
...
describe("getHealthSnapshot", () => {
  ...
  it("probes telegram getMe + webhook info when configured", async () => {
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-health-"));
