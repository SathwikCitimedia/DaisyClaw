...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.DAISYCLAW_DEBUG_HEALTH)) {
...
const resolveHeartbeatSummary = (cfg: DaisyClawConfig, agentId: string) =>
  resolveHeartbeatSummaryForAgent(cfg, agentId);

const resolveAgentOrder = (cfg: DaisyClawConfig) => {
...
/** Runs the `daisyclaw health` command against the gateway and renders JSON or text. */
export async function healthCommand(
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.DAISYCLAW_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
...
async function readBestEffortHealthConfig(): Promise<DaisyClawConfig> {
  const { readBestEffortConfig } = await loadConfigRuntime();
  return await readBestEffortConfig();
}

async function readRuntimeHealthConfig(): Promise<DaisyClawConfig> {
