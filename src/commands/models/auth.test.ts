...
import { MAX_DATE_TIMESTAMP_MS } from "@daisyclaw/normalization-core/number-coercion";
...
vi.mock("../../plugins/provider-auth-choice-helpers.js", async (importOriginal) => {
  ...
  return {
    ...
    applyProviderAuthConfigPatch: vi.fn(
      (cfg: DaisyClawConfig, patch: unknown, options?: { replaceDefaultModels?: boolean }) => {
    ...
    applyDefaultModel: vi.fn((cfg: DaisyClawConfig, model: string) => ({
      ...cfg,
      agents: {
        ...cfg.agents,
        defaults: {
...
describe("modelsAuthLoginCommand", () => {
  ...
  let currentConfig: DaisyClawConfig;
  let lastUpdatedConfig: DaisyClawConfig | null;
  ...
  beforeEach(() => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/daisyclaw/agents/main");
    mocks.resolveAgentWorkspaceDir.mockReturnValue("/tmp/daisyclaw/workspace");
    mocks.resolveDefaultAgentWorkspaceDir.mockReturnValue("/tmp/daisyclaw/workspace");
    ...
    mocks.updateConfig.mockImplementation(
      async (mutator: (cfg: DaisyClawConfig) => DaisyClawConfig) => {
  ...
  function useCoderAgentConfig() {
    currentConfig = {
      agents: {
        list: [{ id: "main" }, { id: "coder", workspace: "/tmp/daisyclaw/workspaces/coder" }],
      },
    };
    const originalConfig = currentConfig;
    mocks.resolveAgentDir.mockImplementation((_cfg: DaisyClawConfig, agentId: string) =>
      agentId === "coder" ? "/tmp/daisyclaw/agents/coder" : "/tmp/daisyclaw/agents/main",
    );
    mocks.resolveAgentWorkspaceDir.mockImplementation((_cfg: DaisyClawConfig, agentId: string) =>
      agentId === "coder" ? "/tmp/daisyclaw/workspaces/coder" : "/tmp/daisyclaw/workspace",
    );
  ...
  it("runs plugin-owned openai login", async () => {
    ...
    expect(mocks.loadAuthProfileStoreForRuntime).toHaveBeenCalledWith("/tmp/daisyclaw/agents/main", {
      externalCli: {
        mode: "scoped",
    ...
    expect(mocks.clearAuthProfileCooldown).toHaveBeenCalledWith({
      store: fakeStore,
      profileId: "openai:user@example.com",
      agentDir: "/tmp/daisyclaw/agents/main",
    });
    ...
    expect(upsertCall.agentDir).toBe("/tmp/daisyclaw/agents/main");
  ...
  it("defaults OpenAI login to ChatGPT OAuth when API key is also available", async () => {
    ...
    expect(mocks.resolvePluginSetupProvider).toHaveBeenCalledWith({
      ...
      workspaceDir: "/tmp/daisyclaw/workspace",
    });
  ...
  it("uses the requested agent store for provider auth login", async () => {
    ...
    expect(authRunCall.agentDir).toBe("/tmp/daisyclaw/agents/coder");
    expect(authRunCall.workspaceDir).toBe("/tmp/daisyclaw/workspaces/coder");
    expect(
      (readMockCallArg(mocks.upsertAuthProfileWithLock) as UpsertAuthProfileCall).agentDir,
    ).toBe("/tmp/daisyclaw/agents/coder");
  });
  ...
  it("loads the owning plugin for an explicit provider even in a clean config", async () => {
    ...
    expect(providerResolutionCall.workspaceDir).toBe("/tmp/daisyclaw/workspace");
  ...
  it("runs the requested anthropic cli auth method with the full login context", async () => {
    ...
    const runClaudeCliMigration = vi.fn().mockImplementation(async (ctx) => {
      ...
      expect(ctx.agentDir).toBe("/tmp/daisyclaw/agents/main");
      expect(ctx.workspaceDir).toBe("/tmp/daisyclaw/workspace");
  ...
  it("reports loaded plugin providers when requested provider is unavailable", async () => {
    ...
    await expect(modelsAuthLoginCommand({ provider: "anthropic" }, runtime)).rejects.toThrow(
      'Unknown provider "anthropic". Loaded providers: openai. Verify plugins via `daisyclaw plugins list --json`.',
    );
  });
  ...
  it("writes pasted Anthropic setup-tokens and logs the preference note", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Anthropic setup-token auth is supported in DaisyClaw.",
    );
    expect(runtime.log).toHaveBeenCalledWith(
      "DaisyClaw prefers Claude CLI reuse when it is available on the host.",
    );
    expect(runtime.log).toHaveBeenCalledWith(
      "Anthropic staff told us this DaisyClaw path is allowed again.",
    );
  });
  ...
  it("writes pasted tokens to the requested agent store", async () => {
    ...
    expect(mocks.upsertAuthProfileWithLock).toHaveBeenCalledWith({
      ...
      agentDir: "/tmp/daisyclaw/agents/coder",
    });
  });
  ...
  it("rejects OpenAI API keys pasted as OpenAI Codex token material", async () => {
    ...
    expect(validateMessages).toEqual([
      "That looks like an OpenAI API key. Use daisyclaw models auth paste-api-key --provider openai for API-key auth.",
    ]);
    expect(mocks.upsertAuthProfileWithLock).not.toHaveBeenCalled();
    expect(mocks.updateConfig).not.toHaveBeenCalled();
  });
  ...
  it("rejects token material pasted into the OpenAI Codex API-key command", async () => {
    ...
    expect(validateMessages).toEqual([
      "That looks like token or OAuth material, not an OpenAI API key. Use daisyclaw models auth paste-token --provider openai for token auth material.",
    ]);
    expect(mocks.upsertAuthProfileWithLock).not.toHaveBeenCalled();
    expect(mocks.updateConfig).not.toHaveBeenCalled();
  });
  ...
  it("rejects an unknown agent before prompting for pasted tokens", async () => {
    ...
    await expect(
      ...
      'Unknown agent id "missing". Use "daisyclaw agents list" to see configured agents.',
    );
  ...
  it("uses the requested agent store for setup-token provider auth", async () => {
    ...
    expect(tokenAuthCall.agentDir).toBe("/tmp/daisyclaw/agents/coder");
    expect(tokenAuthCall.workspaceDir).toBe("/tmp/daisyclaw/workspaces/coder");
    expect(
      (readMockCallArg(mocks.upsertAuthProfileWithLock) as UpsertAuthProfileCall).agentDir,
    ).toBe("/tmp/daisyclaw/agents/coder");
  });
