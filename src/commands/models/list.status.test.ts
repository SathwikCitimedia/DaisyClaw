...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/daisyclaw-agent"),
    resolveAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/daisyclaw-agent/workspace"),
    ...
    resolveAuthStorePathForDisplay: vi.fn(
      (agentDir?: string) => `${agentDir ?? "/tmp/daisyclaw-agent"}/auth-profiles.json`,
    ),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/daisyclaw-dev/daisyclaw.json",
    }),
...
vi.mock("../../agents/workspace.js", () => ({
  resolveDefaultAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/daisyclaw-agent/workspace"),
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/daisyclaw-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.configPath).toBe("/tmp/daisyclaw-dev/daisyclaw.json");
    expect(payload.auth.storePath).toBe("/tmp/daisyclaw-agent/auth-profiles.json");
  ...
  it("honors DAISYCLAW_AGENT_DIR when no --agent override is provided", async () => {
    const localRuntime = createRuntime();
    mocks.resolveAgentDir.mockClear();
    await withEnvAsync({ DAISYCLAW_AGENT_DIR: "/tmp/daisyclaw-isolated-agent" }, async () => {
      await modelsStatusCommand({ json: true }, localRuntime as never);
    });

    expect(mocks.resolveAgentDir).not.toHaveBeenCalled();
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/daisyclaw-isolated-agent");
    const payload = parseFirstJsonLog(localRuntime);
    expect(payload.agentDir).toBe("/tmp/daisyclaw-isolated-agent");
    expect(payload.auth.storePath).toBe("/tmp/daisyclaw-isolated-agent/auth-profiles.json");
  });

  it("honors deprecated PI_CODING_AGENT_DIR when DAISYCLAW_AGENT_DIR is unset", async () => {
    const localRuntime = createRuntime();
    mocks.resolveAgentDir.mockClear();
    await withEnvAsync(
      {
        DAISYCLAW_AGENT_DIR: undefined,
    ...
    expect(mocks.resolveAgentDir).not.toHaveBeenCalled();
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/daisyclaw-legacy-agent");
    const payload = parseFirstJsonLog(localRuntime);
    expect(payload.agentDir).toBe("/tmp/daisyclaw-legacy-agent");
  });
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/daisyclaw-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/daisyclaw-agent-custom");
        ...
        expect(openAiCodex?.effective).toEqual({
          kind: "profiles",
          detail: "/tmp/daisyclaw-agent-custom/auth-profiles.json",
        });
      },
    );
  });
      ...
      expect(payload.auth.runtimeAuthRoutes).toEqual([
        {
          ...
          effective: {
            kind: "profiles",
            detail: "/tmp/daisyclaw-agent/auth-profiles.json",
          },
        },
      ]);
      expect(localRuntime.exit).not.toHaveBeenCalledWith(1);
  ...
  it("includes auth-evidence-only providers in the auth overview", async () => {
    ...
    mocks.resolveEnvApiKey.mockImplementation(
      (provider: string, _env?: NodeJS.ProcessEnv, options?: { workspaceDir?: string }) =>
        provider === "workspace-cloud" && options?.workspaceDir === "/tmp/daisyclaw-agent/workspace"
          ? {
