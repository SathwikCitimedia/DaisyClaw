...
export async function loadValidConfigOrThrow(): Promise<DaisyClawConfig> {
...
export async function updateConfig(
  mutator: (cfg: DaisyClawConfig, context: UpdateConfigContext) => DaisyClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: DaisyClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: DaisyClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("daisyclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
