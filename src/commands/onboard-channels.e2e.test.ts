...
function createTelegramCfg(botToken: string, enabled?: boolean): DaisyClawConfig {
  return {
    channels: {
      telegram: {
        botToken,
...
function createMSTeamsCatalogEntry(): ChannelPluginCatalogEntry {
  return {
    id: "external-chat",
    pluginId: "@daisyclaw/external-chat-plugin",
    meta: {
      id: "external-chat",
      label: "External Chat",
    ...
    install: {
      npmSpec: "@daisyclaw/external-chat",
    },
  };
}
        ...
        plugin: {
          ...
          setupWizard: {
            ...
            status: {
              ...
              resolveConfigured: ({ cfg }: { cfg: DaisyClawConfig }) =>
            ...
            credentials: [
              {
                ...
                inspect: ({ cfg }: { cfg: DaisyClawConfig }) => ({
        ...
        plugin: {
          ...
          setupWizard: {
            ...
            status: {
              ...
              resolveSelectionHint: async ({ cfg }: { cfg: DaisyClawConfig }) =>
            ...
            textInputs: [
              {
                ...
                required: true,
                applySet: ({ cfg, value }: { cfg: DaisyClawConfig; value: string }) =>
                  ({
                    ...cfg,
                    channels: {
                      ...cfg.channels,
...
function patchTelegramAdapter(overrides: ChannelSetupWizardAdapterPatch) {
  ...
  const patch = {
    ...
    getStatus:
      overrides.getStatus ??
      vi.fn(async ({ cfg }: { cfg: DaisyClawConfig }) => ({
        channel: "telegram",
...
async function runQuickstartTelegramSetupWithInteractive(params: {
  ...
  try {
    const cfg = await runSetupChannels({} as DaisyClawConfig, prompter, {
      quickstartDefaults: true,
      onSelection: selection,
      onAccountId,
    });
...
vi.mock("../commands/channel-setup/plugin-install.js", async () => {
  ...
  return {
    ...
    ensureChannelSetupPluginInstalled: vi.fn(async ({ cfg }: { cfg: DaisyClawConfig }) => ({
      cfg,
      installed: true,
    })),
...
describe("setupChannels", () => {
  ...
  it("shows explicit dmScope config command in channel primer", async () => {
    ...
    await runSetupChannels({} as DaisyClawConfig, prompter);
    ...
    await runSetupChannels(
      {
        ...
        plugins: {
          entries: {
            "@daisyclaw/external-chat-plugin": { enabled: true },
          },
        },
      } as DaisyClawConfig,
      prompter,
        ...
        if (channel === "external-chat") {
          registry.channels.push({
            ...
            plugin: {
              ...
              config: {
                listAccountIds: (cfg: DaisyClawConfig) =>
                  Object.keys(
                    (
                      cfg.channels?.["external-chat"] as
                        | { accounts?: Record<string, unknown> }
                ...
                resolveAccount: (cfg: DaisyClawConfig, accountId: string) =>
                  (
                    cfg.channels?.["external-chat"] as
                      | {
                          accounts?: Record<string, Record<string, unknown>>;
  ...
  it("applies configureInteractive result cfg/account updates", async () => {
    const configureInteractive = vi.fn(async ({ cfg }: { cfg: DaisyClawConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
  ...
  it("uses configureWhenConfigured when channel is already configured", async () => {
    const configureWhenConfigured = vi.fn(async ({ cfg }: { cfg: DaisyClawConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
