...
describe("handleReset", () => {
  it("uses active profile paths for destructive reset targets", async () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-reset-profile-"));
    const profileStateDir = path.join(homeDir, ".daisyclaw-work");
    const defaultStateDir = path.join(homeDir, ".daisyclaw");
    const profileConfigPath = path.join(profileStateDir, "daisyclaw.json");
    ...
    vi.stubEnv("DAISYCLAW_HOME", homeDir);
    vi.stubEnv("DAISYCLAW_PROFILE", "work");
    vi.stubEnv("DAISYCLAW_STATE_DIR", profileStateDir);
    vi.stubEnv("DAISYCLAW_CONFIG_PATH", profileConfigPath);
...
describe("moveToTrash", () => {
  it("uses fs-safe trash instead of resolving a PATH trash command", async () => {
    const testRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-trash-helper-"));
  ...
  it("allows fs-safe trash to move a symlink whose target resolves outside the parent", async () => {
    const testRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-trash-symlink-"));
    const targetPath = path.join(testRoot, "target-link");
    const outsideTarget = path.join(os.tmpdir(), "daisyclaw-trash-symlink-target");
  ...
  it("canonicalizes a symlinked parent before calling fs-safe trash", async () => {
    const testRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-trash-parent-link-"));
    const lexicalParent = path.join(testRoot, "state-link");
    const realParent = path.join(testRoot, "state-real");
    const targetPath = path.join(lexicalParent, "daisyclaw.json");
    const sourcePath = path.join(realParent, "daisyclaw.json");
