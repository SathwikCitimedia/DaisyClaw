...
import { formatCliBannerArtLines } from "../cli/banner.js";
...
export function summarizeExistingConfig(config: DaisyClawConfig): string {
  const rows: string[] = [];
...
function summarizeGatewayConfig(config: DaisyClawConfig): string | null {
...
export function printWizardHeader(runtime: RuntimeEnv) {
  // Reuse the single banner art source (plain/uncolored) so the size stays in sync with the CLI banner.
  runtime.log(formatCliBannerArtLines().join("\n"));
}
...
export function formatControlUiSshHint(params: {
  ...
  return [
    ...
    "Docs:",
    "https://docs.daisyclaw.ai/gateway/remote",
    "https://docs.daisyclaw.ai/web/control-ui",
  ]
    .filter(Boolean)
    .join("\n");
}
