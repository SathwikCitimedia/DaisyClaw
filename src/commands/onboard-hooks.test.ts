...
describe("onboard-hooks", () => {
  ...
  const createMockHook = (
    ...
    ...params,
    source: "daisyclaw-bundled" as const,
    pluginId: undefined,
    homepage: undefined,
    always: false,
  ...
  const createMockHookReport = (eligible = true): HookStatusReport => ({
    ...
    managedHooksDir: "/mock/.daisyclaw/hooks",
    hooks: [
      createMockHook(
        {
          name: "session-memory",
  ...
  describe("setupInternalHooks", () => {
    ...
    it("localizes built-in hook prompts when DAISYCLAW_LOCALE is set", async () => {
    ...
    it("should show informative notes to user", async () => {
      vi.stubEnv("DAISYCLAW_CONTAINER_HINT", "");
      vi.stubEnv("DAISYCLAW_PROFILE", "");
      const { prompter } = await runSetupInternalHooks({
        selected: ["session-memory"],
      });
      ...
      expect(noteCalls).toEqual([
        [
          [
            ...
            "",
            "Learn more: https://docs.daisyclaw.ai/automation/hooks",
          ].join("\n"),
          "Hooks",
        ],
        [
