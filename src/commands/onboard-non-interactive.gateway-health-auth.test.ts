...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-gateway-health-auth-"));
  try {
...
describe("resolveGatewayHealthProbeToken", () => {
  ...
  it("resolves file SecretRefs for the local onboarding health probe without persisting plaintext", async () => {
    await withTempDir(async (dir) => {
      ...
      process.env.DAISYCLAW_GATEWAY_TOKEN = "stale-env-token";
  ...
  it("does not fall back to stale DAISYCLAW_GATEWAY_TOKEN when a SecretRef is unresolved", async () => {
    await withTempDir(async (dir) => {
      process.env.DAISYCLAW_GATEWAY_TOKEN = "stale-env-token";
  ...
  it("resolves password auth for the local onboarding health probe", async () => {
    process.env.DAISYCLAW_GATEWAY_TOKEN = "stale-env-token";
    process.env.DAISYCLAW_GATEWAY_PASSWORD = "resolved-password"; // pragma: allowlist secret
