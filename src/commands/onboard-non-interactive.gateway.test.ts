...
const testConfigStore = new Map<string, DaisyClawConfig>();
...
function resolveTestConfigPath() {
  const override = process.env.DAISYCLAW_CONFIG_PATH?.trim();
  if (override) {
    return override;
  }
  const stateDir = process.env.DAISYCLAW_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("DAISYCLAW_STATE_DIR must be set before config IO in this test");
  }
  return path.join(stateDir, "daisyclaw.json");
}
...
function readTestConfig<T = DaisyClawConfig>(): T {
...
vi.mock("../config/config.js", () => ({
  ...
  resolveGatewayPort: (cfg: DaisyClawConfig) => cfg.gateway?.port ?? 18789,
}));
...
vi.mock("./onboard-helpers.js", () => {
  ...
  return {
    DEFAULT_WORKSPACE: "/tmp/daisyclaw-workspace",
    ...
    runNonInteractiveSetup(
      {
        nonInteractive: true,
        mode: "local",
        workspace: path.join(stateDir, "daisyclaw"),
        authChoice: "skip",
        skipSkills: true,
        skipHealth: false,
        installDaemon: true,
...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("daisyclaw-onboard-");
  ...
  it("preserves existing agents.list and bindings on onboard rerun (daisyclaw#84692)", async () => {
    await withStateDir("state-preserve-agents-", async (stateDir) => {
      const workspace = path.join(stateDir, "daisyclaw");
      const seededAgents = [
  ...
  it("preserves existing agents.list and bindings on remote onboard rerun (daisyclaw#84692)", async () => {
  ...
  it("emits structured JSON diagnostics when daemon health fails", async () => {
    await withStateDir("state-local-daemon-health-json-fail-", async (stateDir) => {
      ...
      expect(parsed.hints).toContain("Run `daisyclaw gateway status --deep` for more detail.");
  ...
  it("classifies daemon health ECONNREFUSED failures with a recovery command", async () => {
    await withStateDir("state-local-daemon-health-refused-", async (stateDir) => {
      ...
      expect(parsed.hints).toContain("Fix: run `daisyclaw gateway restart`.");
    });
  }, 60_000);
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    await withStateDir("state-lan-", async (stateDir) => {
      process.env.DAISYCLAW_STATE_DIR = stateDir;
      process.env.DAISYCLAW_CONFIG_PATH = path.join(stateDir, "daisyclaw.json");
