...
export async function runNonInteractiveRemoteSetup(params: {
  ...
  if (!remoteUrl) {
    ...
    runtime.error(
      `Missing --remote-url for remote mode. Example: ${formatCliCommand("daisyclaw onboard --non-interactive --mode remote --remote-url ws://127.0.0.1:3000")}.`,
    );
    runtime.exit(1);
    return;
  }

  let nextConfig: DaisyClawConfig = {
    ...baseConfig,
    gateway: {
      ...baseConfig.gateway,
      mode: "remote",
