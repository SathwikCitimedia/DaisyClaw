...
function getWebSearchConfig(config: DaisyClawConfig | undefined, pluginId: string) {
  return (config as WebSearchConfigRecord | undefined)?.plugins?.entries?.[pluginId]?.config
    ?.webSearch;
}

function ensureWebSearchConfig(config: DaisyClawConfig, pluginId: string) {
...
function createSearchProviderEntry(id: string): PluginWebSearchProviderEntry {
  ...
  const entry: PluginWebSearchProviderEntry = {
    ...
    applySelectionConfig: (config) => {
      const next: DaisyClawConfig = { ...config, plugins: { ...config.plugins } };
      const entries = { ...next.plugins?.entries } as NonNullable<
        NonNullable<DaisyClawConfig["plugins"]>["entries"]
      >;
...
function createPerplexityConfig(apiKey: string, enabled?: boolean): DaisyClawConfig {
  return {
    tools: {
      web: {
        search: {
...
function pluginWebSearchApiKey(config: DaisyClawConfig, pluginId: string): unknown {
  const entry = (
...
function createDisabledFirecrawlConfig(apiKey?: string): DaisyClawConfig {
  return {
    tools: {
      web: {
        search: {
...
function readFirecrawlPluginApiKey(config: DaisyClawConfig): string | undefined {
...
async function runQuickstartPerplexitySetup(
  apiKey: string,
  enabled?: boolean,
): Promise<{ result: DaisyClawConfig; prompter: WizardPrompter }> {
  const cfg = createPerplexityConfig(apiKey, enabled);
