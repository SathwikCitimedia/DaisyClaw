...
export async function setupSkills(
  ...
  let next: DaisyClawConfig = cfg;
  if (installable.length === 0 && missing.length === 0) {
    await prompter.note(
      [
        "No missing skill dependencies to install.",
        `To inspect available skills, run: ${formatCliCommand("daisyclaw skills list --verbose")}`,
        `To check skill status, run: ${formatCliCommand("daisyclaw skills check")}`,
      ].join("\n"),
  ...
  if (installable.length > 0) {
    ...
    for (const name of selected) {
      ...
      // visible through `daisyclaw skills list --verbose`.
      ...
      runtime.log(
        `Tip: run \`${formatCliCommand("daisyclaw doctor")}\` to review skills + requirements.`,
      );
