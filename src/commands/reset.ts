...
/** CLI options accepted by `daisyclaw reset`. */
...
async function stopGatewayIfRunning(runtime: RuntimeEnv) {
  if (isNixMode) {
    // Nix mode owns service lifecycle outside DaisyClaw-managed launchd/systemd
...
export async function resetCommand(runtime: RuntimeEnv, opts: ResetOptions) {
  ...
  if (scope === "config+creds+sessions") {
    ...
    runtime.log(`Next: ${formatCliCommand("daisyclaw onboard --install-daemon")}`);
    return;
  }
