 ...
 *
 *   - RED: ACP-keyed session resolves to `id: "daisyclaw"`, `source: "implicit"`.
...
/**
 * Build a minimal `DaisyClawConfig` that mirrors the deployed scenario:
 ...
 *
 * Result: the old metadata resolver fell through to the implicit "daisyclaw"
 * branch — which is the bug under test.
 */
function buildConfigWithoutAgentRuntimePolicy(): DaisyClawConfig {
  return {
    agents: {
      list: [
        {
...
describe("sessions --json agentRuntime classifier (catalog #18)", () => {
  it("RED→GREEN: ACP session key is no longer misclassified (overlay applies)", () => {
    ...
    // resolved metadata said id="daisyclaw", source="implicit".
    ...
    expect(
      agentRuntime.id,
      `ACP session ${ACP_SESSION_KEY} should no longer be misclassified as "auto" or "daisyclaw". ` +
  ...
  it("FIX-SHAPE expectation: ACP session should resolve to 'acpx'", () => {
    ...
    // from "daisyclaw" and reflects the actual runtime driving the session.
