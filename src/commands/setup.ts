...
async function writeDefaultConfigFile(config: DaisyClawConfig): Promise<void> {
...
async function readConfigFileRaw(configPath: string): Promise<{
  exists: boolean;
  parsed: DaisyClawConfig;
}> {
  try {
    const raw = await fs.readFile(configPath, "utf-8");
    const parsed = safeParseWithSchema(JsonRecordSchema, JSON5.parse(raw));
    return { exists: true, parsed: (parsed ?? {}) as DaisyClawConfig };
  } catch {
...
export async function setupCommand(
  ...
  runtime.log(`Next guided path: ${formatCliCommand("daisyclaw onboard")}.`);
  runtime.log(
    `Next targeted changes: ${formatCliCommand("daisyclaw configure")} for models, channels, Gateway, plugins, skills, and health checks.`,
  );
  runtime.log(`Add a chat channel later: ${formatCliCommand("daisyclaw channels add")}.`);
}
