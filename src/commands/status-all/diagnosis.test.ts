...
describe("status-all diagnosis port checks", () => {
  ...
  it("labels DaisyClaw Tailscale exposure separately from daemon state", async () => {
  ...
  it("treats same-process dual-stack loopback listeners as healthy", async () => {
    const params = createBaseParams([
      { pid: 5001, commandLine: "daisyclaw-gateway", address: "127.0.0.1:18789" },
      { pid: 5001, commandLine: "daisyclaw-gateway", address: "[::1]:18789" },
    ]);
  ...
  it("treats a single wildcard Gateway listener as healthy", async () => {
    ...
    const output = params.lines.join("\n");
    expect(output).toContain("✓ Port 18789");
    expect(output).toContain("Detected DaisyClaw Gateway listener on the configured port.");
    expect(output).not.toContain("Port 18789 is already in use.");
  });
  ...
  it("keeps warning for multi-process listener conflicts", async () => {
    const params = createBaseParams([
      { pid: 5001, commandLine: "daisyclaw-gateway", address: "127.0.0.1:18789" },
      { pid: 5002, commandLine: "daisyclaw-gateway", address: "[::1]:18789" },
    ]);
    ...
    const output = params.lines.join("\n");
    expect(output).toContain("! Port 18789");
    expect(output).toContain("2 DaisyClaw gateway processes appear to be listening on port 18789");
  ...
  it("adds direct update restart guidance for failed update sentinels", async () => {
    ...
    expect(output).toContain(
      "Update restart: failed · managed-service-handoff-failed · run daisyclaw gateway status --deep",
    );
    expect(output).toContain("Update restart failed; run daisyclaw gateway status --deep.");
    expect(output).toContain(
      "If the service is down, run daisyclaw gateway restart or daisyclaw gateway install --force.",
    );
  });
  ...
  it("adds direct update restart guidance for pending update sentinels", async () => {
    ...
    const output = params.lines.join("\n");
    expect(output).toContain(
      "Update restart: restart pending health verification · run daisyclaw gateway status --deep",
    );
    expect(output).toContain(
      "Update restart is still pending; run daisyclaw update status --json for handoff state.",
    );
  });
  ...
  it("does not read or display stale stderr tails on Darwin", async () => {
    ...
    try {
      ...
      restartLogMocks.resolveGatewayRestartLogPath.mockReturnValue(
        "/tmp/daisyclaw/logs/gateway-restart.log",
      );
      ...
      expect(gatewayMocks.readFileTailLines).not.toHaveBeenCalledWith(
        "/Users/test/Library/Logs/daisyclaw/gateway.err.log",
        40,
      );
      expect(output).toContain("# stdout: /Users/test/Library/Logs/daisyclaw/gateway.log");
