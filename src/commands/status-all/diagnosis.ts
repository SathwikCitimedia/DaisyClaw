// Appends the read-only diagnosis section for `daisyclaw status --all`.
...
export async function appendStatusAllDiagnosis(params: {
  ...
  if (params.portUsage) {
    ...
    if (!portOk) {
      ...
      if (gatewayPidCount > 1) {
        lines.push(
          `  ${muted(`${gatewayPidCount} DaisyClaw gateway processes appear to be listening on port ${params.port}; stop stale gateway processes before trusting channel health.`)}`,
        );
      }
      ...
      lines.push(`  ${muted("Detected DaisyClaw Gateway listener on the configured port.")}`);
    }
  }
  ...
  lines.push("Troubleshooting: https://docs.daisyclaw.ai/troubleshooting");
  lines.push("");
}
