// Builds overview table rows for `daisyclaw status` and `daisyclaw status --all`.
...
/** Builds the default `daisyclaw status` overview rows from scan, health, memory, and session inputs. */
  ...
  return buildStatusOverviewRowsFromSurface({
    ...
    middleRows: [
      ...(params.updateRestartValue
        ? [{ Item: "Update restart", Value: params.updateRestartValue }]
        : []),
      { Item: "Security", Value: `Run: ${formatCliCommand("daisyclaw security audit --deep")}` },
    ],
