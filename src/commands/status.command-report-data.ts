// Builds the data model for the standard `daisyclaw status` text report.
...
export async function buildStatusCommandReportData(
  ...
  const securityAuditLines = params.securityAudit
    ...
    : [
        params.theme.muted(
          `Skipped in fast status. Full report: ${params.formatCliCommand("daisyclaw security audit")}`,
        ),
        params.theme.muted(`Deep probe: ${params.formatCliCommand("daisyclaw status --deep")}`),
      ];
  ...
  return {
    ...
    taskMaintenanceHint: `Task maintenance: ${params.formatCliCommand("daisyclaw tasks maintenance --apply")}`,
