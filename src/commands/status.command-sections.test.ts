...
describe("status.command-sections", () => {
  it("formats security audit lines with finding caps and follow-up commands", () => {
    ...
    expect(lines.at(-2)).toBe("muted(Full report: cmd:daisyclaw security audit)");
    expect(lines.at(-1)).toBe("muted(Deep probe: cmd:daisyclaw security audit --deep)");
  });
  ...
  it("builds footer lines from update and reachability state", () => {
    expect(
      ...
      "warn(upgrade ready)",
      "Next steps:",
      "  Need to share?      cmd:daisyclaw status --all",
      "  Need to debug live? cmd:daisyclaw logs --follow",
      "  Fix reachability first: cmd:daisyclaw gateway probe",
    ]);
  });
  ...
  it("builds plugin compatibility lines and pairing recovery guidance", () => {
    ...
    expect(
      ...
      "muted(Recovery: cmd:daisyclaw devices approve req-123)",
      "muted(Fallback: cmd:daisyclaw devices approve --latest)",
      "muted(Inspect: cmd:daisyclaw devices list)",
    ]);
  });
