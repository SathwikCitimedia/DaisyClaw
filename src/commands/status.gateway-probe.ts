...
export async function resolveGatewayProbeAuthResolution(cfg: DaisyClawConfig): Promise<{
  auth: {
    token?: string;
    password?: string;
  };
...
export async function resolveGatewayProbeAuth(cfg: DaisyClawConfig): Promise<{
  token?: string;
  password?: string;
}> {
