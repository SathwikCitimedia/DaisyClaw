...
describe("status.scan.config-shared", () => {
  beforeEach(() => {
    ...
    mocks.resolveConfigPath.mockReturnValue(
      `/tmp/daisyclaw-status-scan-config-shared-missing-${process.pid}.json`,
    );
  });
  ...
  it("adds a status diagnostic for gateway token source conflicts", async () => {
    ...
    const result = await loadStatusScanCommandConfig({
      ...
      readBestEffortConfig,
      resolveConfig,
      env: { VITEST: "true", DAISYCLAW_GATEWAY_TOKEN: "env-token" },
      allowMissingConfigFastPath: true,
    });

    expect(result.secretDiagnostics).toEqual([
      "DAISYCLAW_GATEWAY_TOKEN conflicts with gateway.auth.token: Remove DAISYCLAW_GATEWAY_TOKEN from the shell, ~/.daisyclaw/.env, or launchctl env if gateway.auth.token is intended, or point gateway.auth.token at ${DAISYCLAW_GATEWAY_TOKEN} if the env var should be canonical.",
    ]);
  });
  ...
  it("does not add a token conflict diagnostic inside the managed gateway service context", async () => {
    ...
    const result = await loadStatusScanCommandConfig({
      ...
      env: {
        VITEST: "true",
        DAISYCLAW_GATEWAY_TOKEN: "env-token",
  ...
  it("does not add a status diagnostic when config uses DAISYCLAW_GATEWAY_TOKEN", async () => {
    const sourceConfig = {
