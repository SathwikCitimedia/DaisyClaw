...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/daisyclaw-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = DaisyClawConfig>(
  overrides: T = {} as T,
): DaisyClawConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): DaisyClawConfig {
