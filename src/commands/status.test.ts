...
beforeAll(() => {
  envSnapshot = captureEnv(["DAISYCLAW_PROFILE"]);
  process.env.DAISYCLAW_PROFILE = "isolated";
});
...
async function createStatusServiceSummary(
  ...
  return {
    ...
    loaded,
    managedByDaisyClaw: Boolean(command),
  ...
  return {
    ...
    update: {
      ...
      deps: {
        manager: "pnpm",
        status: "ok",
        lockfilePath: "/tmp/daisyclaw/pnpm-lock.yaml",
        markerPath: "/tmp/daisyclaw/node_modules/.modules.yaml",
      },
    ...
    gatewayProbeAuth: process.env.DAISYCLAW_GATEWAY_TOKEN
...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.daisyclaw.gateway.plist",
    }),
  }),
  resolveNodeService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.daisyclaw.node.plist",
    }),
  }),
}));
...
vi.mock("../gateway/call.js", () => ({
  ...
  resolveGatewayCredentialsWithSecretInputs: vi.fn(
    async (params: {
      ...
      const envToken = process.env.DAISYCLAW_GATEWAY_TOKEN?.trim();
...
vi.mock("../infra/daisyclaw-root.js", () => ({
  resolveDaisyClawPackageRoot: vi.fn().mockResolvedValue("/tmp/daisyclaw"),
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines with verbose cache details", async () => {
    ...
    expectLogsMatch(logs, /daisyclaw (?:--profile isolated )?status --all/);
  ...
  it("shows node-only gateway info when no local gateway service is installed", async () => {
    ...
    expect(joined).toContain("daisyclaw --profile isolated node status");
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("DAISYCLAW_GATEWAY_TOKEN", "abcd1234", async () => {
