...
import type { DaisyClawTestState } from "../test-utils/daisyclaw-test-state.js";
import {
  tasksAuditCommand,
  tasksCancelCommand,
...
async function withTaskCommandStateDir(
  run: (state: DaisyClawTestState) => Promise<void>,
): Promise<void> {
  await withDaisyClawTestState(
    { layout: "state-only", prefix: "daisyclaw-tasks-command-" },
    async (state) => {
