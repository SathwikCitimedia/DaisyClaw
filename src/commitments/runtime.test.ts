...
function requireFirstEmbeddedAgentRequest(): {
  ...
  if (!call) {
    throw new Error("expected embedded DaisyClaw agent extraction request");
  }
  const [request] = call;
...
describe("commitment extraction runtime", () => {
  ...
  async function createConfig(): Promise<DaisyClawConfig> {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-commitment-runtime-"));
    tmpDirs.push(tmpDir);
    stateDirEnvSnapshot ??= captureEnv(["DAISYCLAW_STATE_DIR"]);
    setTestEnvValue("DAISYCLAW_STATE_DIR", tmpDir);
    return {
      commitments: {
        enabled: true,
      },
