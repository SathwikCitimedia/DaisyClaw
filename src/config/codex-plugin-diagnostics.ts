...
function isOpenAiCodexDefaultRuntimeSelection(params: {
  ...
  // Custom OpenAI-compatible base URLs stay on the DaisyClaw runtime path.
...
function codexPluginEntryEnabled(cfg: DaisyClawConfig): boolean | undefined {
...
function openAiProviderRuntimePolicy(cfg: DaisyClawConfig): AgentRuntimePolicyConfig | undefined {
...
function listConfiguredAgentIds(cfg: DaisyClawConfig): Array<string | undefined> {
...
function openAiHasCodexDefaultRuntimePolicy(cfg: DaisyClawConfig): boolean {
  for (const [providerId, providerConfig] of Object.entries(cfg.models?.providers ?? {})) {
    if (normalizeProviderId(providerId) !== OPENAI_PROVIDER_ID) {
      continue;
    }
    ...
    // A model-scoped explicit "auto"/"default" overrides provider-wide PI/DaisyClaw
...
function openAiDefaultRouteKeepsCodexUnavailable(cfg: DaisyClawConfig): boolean {
  const policy = openAiDefaultRouteRuntimePolicy(cfg);
  if (!policy?.id?.trim()) {
    // With no explicit runtime policy, the OpenAI route only needs Codex on the
    // official OpenAI endpoint. OpenAI-compatible proxies stay on DaisyClaw.
    return !openAIProviderUsesCodexRuntimeByDefault({
      provider: OPENAI_PROVIDER_ID,
      config: cfg,
    });
 ...
 */
export function configExplicitlyKeepsCodexUnavailableForOpenAi(cfg: DaisyClawConfig): boolean {
 ...
 * all default routes would otherwise stay on the DaisyClaw runtime.
 */
export function shouldSuppressMissingCodexPluginDiagnostics(cfg: DaisyClawConfig): boolean {
