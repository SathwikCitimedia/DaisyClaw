...
function collectConfigEnvVarsByTarget(cfg?: DaisyClawConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: DaisyClawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** Collects config env vars safe to persist into managed service environments. */
export function collectConfigServiceEnvVars(cfg?: DaisyClawConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.DAISYCLAW_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
