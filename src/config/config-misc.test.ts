...
import { buildWebSearchProviderConfig, withTempHome, writeDaisyClawConfig } from "./test-helpers.js";
...
describe("boolean config validation", () => {
  it.each(nonBooleanConfigCases)("rejects non-boolean values for $name", ({ config }) => {
    const result = DaisyClawSchema.safeParse(config);
...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    ...
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://daisyclaw.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = DaisyClawSchema.safeParse({ $schema: 123 });
    expect(result.success).toBe(false);
  });
  ...
  it("preserves $schema through validateConfigObject round-trip", () => {
    ...
    if (res.ok) {
      expect(res.config.$schema).toBe("https://daisyclaw.ai/config.json");
    }
  });
});
...
describe("config identity/materialization regressions", () => {
  it("keeps explicit responsePrefix and group mention patterns", () => {
    ...
    if (res.ok) {
      ...
      expect(res.config.agents?.list?.[0]?.groupChat?.mentionPatterns).toEqual(["@daisyclaw"]);
    }
  });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = DaisyClawSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("rejects top-level memorySearch without read-time auto-migration", async () => {
    await withTempHome(async (home) => {
      await writeDaisyClawConfig(home, {
        memorySearch: {
          provider: "local",
          fallback: "none",
          query: { maxResults: 7 },
  ...
  it("rejects resolved-only gateway.bind aliases as invalid schema values, not legacy", async () => {
    await withTempHome(async (home) => {
      await writeDaisyClawConfig(home, {
        gateway: { bind: "${DAISYCLAW_BIND}" },
      });

      const prev = process.env.DAISYCLAW_BIND;
      process.env.DAISYCLAW_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.DAISYCLAW_BIND;
        } else {
          process.env.DAISYCLAW_BIND = prev;
        }
      }
    });
  });
