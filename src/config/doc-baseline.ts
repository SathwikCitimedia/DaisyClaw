...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.DAISYCLAW_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveDaisyClawPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    DAISYCLAW_STATE_DIR: path.join(os.tmpdir(), "daisyclaw-config-doc-baseline-state"),
