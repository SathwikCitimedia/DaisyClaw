...
function createRenameAuditRecord(home: string) {
  return finalizeConfigWriteAuditRecord({
    base: createAuditRecordBase(path.join(home, ".daisyclaw", "daisyclaw.json")),
    result: "rename",
    nextMetadata: {
      dev: "12",
      ino: "13",
...
function readAuditLog(home: string): unknown[] {
  const auditPath = path.join(home, ".daisyclaw", "logs", "config-audit.jsonl");
  return fs
...
describe("config io audit helpers", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "daisyclaw-config-audit-" });
  ...
  it('ignores literal "undefined" home env values when choosing the audit log path', async () => {
    ...
    expect(auditPath).toBe(path.join(home, ".daisyclaw", "logs", "config-audit.jsonl"));
  ...
  it("formats overwrite warnings with hash transition and backup path", () => {
    expect(
      formatConfigOverwriteLogMessage({
        configPath: "/tmp/daisyclaw.json",
        previousHash: "prev-hash",
        nextHash: "next-hash",
        changedPathCount: 3,
      }),
    ).toBe(
      "Config overwrite: /tmp/daisyclaw.json (sha256 prev-hash -> next-hash, backup=/tmp/daisyclaw.json.bak, changedPaths=3)",
    );
  });
  ...
  it("captures watch markers and next stat metadata for successful writes", () => {
    const base = createConfigWriteAuditRecordBase({
      configPath: "/tmp/daisyclaw.json",
      env: {
        DAISYCLAW_WATCH_MODE: "1",
        DAISYCLAW_WATCH_SESSION: "watch-session-1",
        DAISYCLAW_WATCH_COMMAND: "gateway --force",
      } as NodeJS.ProcessEnv,
      existsBefore: true,
      previousHash: "prev-hash",
      nextHash: "next-hash",
  ...
  it("drops next-file metadata and preserves error details for failed writes", () => {
    const base = createAuditRecordBase("/tmp/daisyclaw.json");
  ...
  it("redacts structured audit records before persistence", async () => {
    ...
    const record = finalizeConfigWriteAuditRecord({
      base: {
        ...createAuditRecordBase(path.join(home, ".daisyclaw", "daisyclaw.json")),
        suspicious: [
    ...
    const raw = fs.readFileSync(
      path.join(home, ".daisyclaw", "logs", "config-audit.jsonl"),
      "utf-8",
    );
  ...
  it("redacts the value half of `--flag=value` for secret flags", () => {
    const argv = ["daisyclaw", "--token=ghp_realgithubtoken1234567890ABCD", "--port=8080"];
    expect(redactConfigAuditArgv(argv)).toEqual(["daisyclaw", "--token=***", "--port=8080"]);
  });
  ...
  it("redacts standalone token shapes via the shared logging redaction patterns", () => {
    ...
    expect(result[1]).toBe("daisyclaw");
  ...
  it("leaves non-secret arguments untouched", () => {
    const argv = ["node", "daisyclaw", "gateway", "--port", "8080", "--bind", "lan"];
  ...
  it("masks the next arg after a secret flag even when it looks like another option", () => {
    const argv = ["daisyclaw", "--token", "--port", "8080"];
    expect(redactConfigAuditArgv(argv)).toEqual(["daisyclaw", "--token", "***", "8080"]);
  });

  it("redacts dash-leading secret values after bare secret flags", () => {
    const argv = ["daisyclaw", "--password", "-secret-value"];
    expect(redactConfigAuditArgv(argv)).toEqual(["daisyclaw", "--password", "***"]);
  });

  it("does not mask when a secret flag is the final arg with no value", () => {
    const argv = ["daisyclaw", "--token"];
    expect(redactConfigAuditArgv(argv)).toEqual(["daisyclaw", "--token"]);
  });
  ...
  it("redacts processInfo.argv when explicitly supplied to createConfigWriteAuditRecordBase", () => {
    const base = createConfigWriteAuditRecordBase({
      ...
      processInfo: {
        ...
        ppid: 1,
        cwd: "/work",
        argv: ["node", "daisyclaw", "--token", "leaked-but-not-anymore-12345"],
        execArgv: [],
      },
    });
    expect(base.argv).toEqual(["node", "daisyclaw", "--token", "***"]);
  });
  ...
  it("rewrites historical config-audit entries through redactConfigAuditArgv and preserves 0600 mode", async () => {
    ...
    const unredactedRecord = {
      ...
      source: "config-io",
      event: "config.write",
      configPath: path.join(home, ".daisyclaw", "daisyclaw.json"),
      pid: 1590563,
      ppid: 1590548,
      cwd: home,
      argv: [
        "/usr/bin/node",
        "/usr/local/bin/daisyclaw.mjs",
        "config",
        "set",
    ...
    const alreadyRedactedRecord = {
      ...
      ppid: 1,
      cwd: home,
      argv: ["/usr/bin/node", "/usr/local/bin/daisyclaw.mjs", "config", "set", "ui.theme", "dark"],
      execArgv: ["--disable-warning=ExperimentalWarning"],
      suspicious: [],
      result: "rename",
    };
  ...
  it("preserves malformed lines verbatim and counts them as skipped", async () => {
    ...
    const validUnredacted = {
      ts: "2026-05-02T00:03:48.471Z",
      argv: ["node", "daisyclaw.mjs", "config", "set", "x", "xoxb-bad-token-1234567890abcdef"],
    };
  ...
  it("aborts without overwriting when the audit log is appended to after temp write", async () => {
    ...
    const appended = {
      ...
      argv: ["node", "daisyclaw.mjs", "config", "set", "theme", "dark"],
      execArgv: [],
    };
    const original = `${JSON.stringify(unredacted)}\n`;
