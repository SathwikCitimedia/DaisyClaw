...
import { withTempHome, writeDaisyClawConfig } from "./test-helpers.js";
...
describe("readBestEffortConfig", () => {
  it("can read snapshots without updating config observation state", async () => {
    await withTempHome(async (home) => {
      ...
      const healthPath = `${home}/.daisyclaw/logs/config-health.json`;
  ...
  it("does not restore suspicious direct edits from .bak during ordinary reads", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeDaisyClawConfig(home, {
      ...
      const entries = await fs.readdir(`${home}/.daisyclaw`);
      expect(entries.some((entry) => entry.startsWith("daisyclaw.json.clobbered."))).toBe(false);
    });
  });
