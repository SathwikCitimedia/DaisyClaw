...
describe("config io paths", () => {
  ...
  it("uses ~/.daisyclaw/daisyclaw.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".daisyclaw", 19001);
      const io = createIoForHome(home);
      expect(io.configPath).toBe(configPath);
    });
  });

  it("defaults to ~/.daisyclaw/daisyclaw.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".daisyclaw", "daisyclaw.json"));
    });
  });

  it("uses DAISYCLAW_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { DAISYCLAW_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".daisyclaw", "daisyclaw.json"));
    });
  });

  it("honors explicit DAISYCLAW_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".daisyclaw", 20002, "custom.json");
      const io = createIoForHome(home, { DAISYCLAW_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("explains what to check when config was written by a newer DaisyClaw", async () => {
    await withTempHome(async (home) => {
      const configPath = path.join(home, ".daisyclaw", "daisyclaw.json");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        [
          `Your DaisyClaw config was written by version 9999.1.1, but this command is running ${VERSION}.`,
  ...
  it("does not warn about newer config during internal update handoff reads", async () => {
    await withTempHome(async (home) => {
      ...
      const io = createConfigIO({
        configPath,
        env: { HOME: home, DAISYCLAW_UPDATE_POST_CORE: "1" } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger,
      });
      io.loadConfig();
