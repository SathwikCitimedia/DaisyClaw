...
describe("config observe recovery", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "daisyclaw-config-observe-recovery-"));
  });
  ...
  function makeDeps(
    ...
    return {
      ...
      configPath,
      auditPath: path.join(home, ".daisyclaw", "logs", "config-audit.jsonl"),
      warn,
    };
  }
  ...
  it("loadConfig clears env vars from the discarded clobbered config before rereading backup", async () => {
    await withSuiteHome(async (home) => {
      ...
      await writeConfigRaw(configPath, {
        meta: { lastTouchedVersion: "2026.5.28" },
        env: { vars: { DAISYCLAW_CLOBBER_ONLY: "bad" } },
      });

      const config = io.loadConfig();

      expect(config.gateway?.mode).toBe("local");
      expect(env.DAISYCLAW_CLOBBER_ONLY).toBeUndefined();
    });
  });
  ...
  it("validates backup candidates without leaking their env into live state", async () => {
    await withSuiteHome(async (home) => {
      ...
      await seedConfigBackup(configPath, {
        gateway: { mode: "local" },
        env: { vars: { DAISYCLAW_BACKUP_ONLY: "stale" } },
        agents: { defaults: { model: 123 } },
      });
      await writeConfigRaw(configPath, {
        meta: { lastTouchedVersion: "2026.5.28" },
      ...
      expect(env.DAISYCLAW_BACKUP_ONLY).toBeUndefined();
    });
  });
