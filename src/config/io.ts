// Loads, validates, migrates, snapshots, and writes DaisyClaw config files.
import crypto from "node:crypto";
import fs from "node:fs";
import os from "node:os";
import path from "node:path";
import { collectManifestModelIdNormalizationPolicies } from "@daisyclaw/model-catalog-core/provider-model-id-normalization";
...
import type { DaisyClawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
export type ConfigWriteResult = { persistedHash: string; persistedConfig: DaisyClawConfig };
...
export type ConfigWriteOptions = {
   ...
   */
  explicitSetValueSource?: DaisyClawConfig;
  /**
   ...
   */
  preCommitRuntimePreflight?: (sourceConfig: DaisyClawConfig) => Promise<unknown>;
};
...
function coerceConfig(value: unknown): DaisyClawConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as DaisyClawConfig;
}
...
function stampConfigVersion(cfg: DaisyClawConfig, version?: string): DaisyClawConfig {
  return stampConfigWriteMetadata(cfg, new Date().toISOString(), version);
}

function warnIfConfigFromFuture(cfg: DaisyClawConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    ...
    logger.warn(
      [
        `Your DaisyClaw config was written by version ${touched}, but this command is running ${VERSION}.`,
        "Check: `daisyclaw --version`, `which daisyclaw`, and `daisyclaw gateway status --deep`.",
        "If unexpected, update PATH so `daisyclaw` points to the version you want, or reinstall the Gateway service from that same DaisyClaw install.",
      ].join("\n"),
    );
  }
}
...
function shouldSuppressFutureVersionWarningForEnv(env: NodeJS.ProcessEnv): boolean {
  return (
    isTruthyEnvValue(env.DAISYCLAW_UPDATE_IN_PROGRESS) ||
    isTruthyEnvValue(env.DAISYCLAW_UPDATE_POST_CORE)
  );
}
...
export function parseConfigJson5(
  ...
  try {
    ...
    // on the JSON files DaisyClaw writes itself.
  }
  try {
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as DaisyClawConfig, env);
  }
...
export function createConfigIO(
  ...
  function finalizeLoadedRuntimeConfig(cfg: DaisyClawConfig): DaisyClawConfig {
  ...
  function resolveRuntimePreflightSourceConfig(candidate: DaisyClawConfig): DaisyClawConfig {
  ...
  function ensureShippedPluginInstallConfigRecordsMigratedForWrite(
    ...
    try {
      ...
      throw new Error(
        `Config write blocked: shipped plugins.installs records in ${configPath} could not be migrated into the plugin index. Fix state directory permissions or run daisyclaw plugins registry --refresh, then retry. ${formatErrorMessage(
          err,
        )}`,
        { cause: err },
      );
  ...
  function validateSuspiciousRecoveryBackup(parsed: unknown): boolean {
    try {
      ...
      const loadValidationPluginMetadataSnapshot = (config: DaisyClawConfig) => {
  ...
  function loadConfigLocal(options: { skipSuspiciousRecovery?: boolean } = {}): DaisyClawConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(
        validationConfigRaw as DaisyClawConfig,
        {
          env: deps.env,
          homedir: deps.homedir,
        },
  ...
  async function readConfigFileSnapshotInternal(
    ...
    let fallbackSourceConfig: DaisyClawConfig = {};
  ...
  async function readBestEffortConfigLocal(): Promise<DaisyClawConfig> {
    const result = await readConfigFileSnapshotInternal();
  ...
  async function readSourceConfigBestEffortLocal(): Promise<DaisyClawConfig> {
    maybeLoadDotEnvForConfig(deps.env);
    const exists = deps.fs.existsSync(configPath);
    if (!exists) {
      return {};
  ...
  async function writeConfigFileLocal(
    ...
    persistCandidate = applyUnsetPathsForWrite(persistCandidate as DaisyClawConfig, unsetPaths);
    ...
    let cfgToWrite = persistCandidate as DaisyClawConfig;
    try {
      if (deps.fs.existsSync(configPath)) {
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as DaisyClawConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.DAISYCLAW_TEST_CONFIG_OVERWRITE_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
      if (!isVerbose() && deps.env.DAISYCLAW_CONFIG_OVERWRITE_LOG !== "1" && !shouldLogInVitest) {
        return;
      }
      deps.logger.warn(
        formatConfigOverwriteLogMessage({
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.DAISYCLAW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
      const shouldLogBenignMissingMeta =
        isVerbose() || deps.env.DAISYCLAW_CONFIG_WRITE_ANOMALY_LOG === "1" || shouldLogInVitest;
    ...
    const preCommitRuntimePreflight =
      ...
      (async (sourceConfig: DaisyClawConfig) => {
...
// module scope. `DAISYCLAW_CONFIG_PATH` (and friends) are expected to work even
...
function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: DaisyClawConfig;
  candidate: DaisyClawConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: DaisyClawConfig): DaisyClawConfig {
...
export function loadConfig(options?: {
  ...
  // reparsing daisyclaw.json on hot code paths.
...
export async function readBestEffortConfig(options?: {
  skipPluginValidation?: boolean;
}): Promise<DaisyClawConfig> {
  return await createConfigIO(
    options?.skipPluginValidation ? { pluginValidation: "skip" } : {},
  ).readBestEffortConfig();
}

export async function readSourceConfigBestEffort(): Promise<DaisyClawConfig> {
...
export async function writeConfigFile(
  ...
  let canonicalSourceConfig: DaisyClawConfig = nextCfg;
