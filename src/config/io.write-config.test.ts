...
import type { ConfigFileSnapshot, DaisyClawConfig } from "./types.daisyclaw.js";
...
describe("config io write", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "daisyclaw-config-io-" });
  const silentLogger = {
    warn: () => {},
    error: () => {},
  };
  ...
  const createFastConfigIO = (home: string) =>
    createConfigIO({
      env: { DAISYCLAW_TEST_FAST: "1" } as NodeJS.ProcessEnv,
      homedir: () => home,
      logger: silentLogger,
    });
  ...
  it("logs health-state write failures through public config reads", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".daisyclaw", "daisyclaw.json");
      const healthPath = path.join(home, ".daisyclaw", "logs", "config-health.json");
  ...
  it("loads shipped plugin install config records without mutating config or plugin index", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".daisyclaw", "daisyclaw.json");
      const pluginDir = path.join(home, ".daisyclaw", "plugins", "demo");
      const manifestPath = path.join(pluginDir, "daisyclaw.plugin.json");
      ...
      try {
        ...
        await expect(
          readPersistedInstalledPluginIndex({
            stateDir: path.join(home, ".daisyclaw"),
          }),
        ).resolves.toBeNull();
        await expect(fs.readFile(configPath, "utf-8")).resolves.toBe(initialRaw);
      } finally {
  ...
  it("migrates shipped plugin install config records during explicit writes even when the manifest is missing", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".daisyclaw", "daisyclaw.json");
      const pluginDir = path.join(home, ".daisyclaw", "plugins", "missing");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        configPath,
        `${JSON.stringify(
  ...
  it("keeps shipped plugin install config records when index migration fails", async () => {
    ...
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".daisyclaw", "daisyclaw.json");
      const unwritableStatePath = path.join(home, ".daisyclaw");
  ...
  it("keeps writes inside an DAISYCLAW_STATE_DIR override even when the real home config exists", async () => {
    await withSuiteHome(async (home) => {
      const liveConfigPath = path.join(home, ".daisyclaw", "daisyclaw.json");
      ...
      expect(io.configPath).toBe(path.join(overrideDir, "daisyclaw.json"));
      ...
      const overridePersisted = JSON.parse(
        await fs.readFile(path.join(overrideDir, "daisyclaw.json"), "utf-8"),
      ) as {
  ...
  it("prints missing-meta write anomalies when anomaly logging is requested", async () => {
    await withSuiteHome(async (home) => {
      ...
      const io = createConfigIO({
        env: {
          DAISYCLAW_CONFIG_WRITE_ANOMALY_LOG: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: {
          warn,
  ...
  it("suppresses overwrite audit output when skipOutputLogs is set", async () => {
    await withSuiteHome(async (home) => {
      ...
      const io = createConfigIO({
        env: {
          VITEST: "true",
          DAISYCLAW_TEST_CONFIG_OVERWRITE_LOG: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: {
          warn,
  ...
  it("preserves root $schema during partial writes", async () => {
    await withSuiteHome(async (home) => {
      ...
      expect(persisted.$schema).toBe("https://daisyclaw.ai/config.json");
  ...
  it("rejects destructive internal writes before replacing the config", async () => {
    await withSuiteHome(async (home) => {
      ...
      const original = {
        ...
        agents: { list: [{ id: "main", default: true, workspace: "/tmp/daisyclaw-main" }] },
  ...
  it("keeps authored agent provider params during narrowed internal agent writes", async () => {
    await withSuiteHome(async (home) => {
      ...
      const persisted = JSON.parse(await fs.readFile(configPath, "utf-8")) as DaisyClawConfig;
  ...
  it("rejects root-include partial writes instead of flattening the root config", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".daisyclaw", "daisyclaw.json");
      const includePath = path.join(home, ".daisyclaw", "extra.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify({ $schema: "https://daisyclaw.ai/config-from-include.json" }, null, 2)}\n`,
        "utf-8",
      );
      await fs.writeFile(
        configPath,
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/daisyclaw-test-required-plugin",
          source: "/tmp/daisyclaw-test-required-plugin/index.ts",
          manifestPath: "/tmp/daisyclaw-test-required-plugin/daisyclaw.plugin.json",
          configSchema: {
            type: "object",
            properties: {
              token: { type: "string" },
  ...
  it("writes runtime-derived edits back to source SecretRef markers", async () => {
    await withSuiteHome(async (home) => {
      ...
      await withEnvAsync({ DAISYCLAW_CONFIG_PATH: configPath }, async () => {
        setRuntimeConfigSnapshot(
          {
            gateway: { mode: "local" },
            models: {
        ...
        `${JSON.stringify(
          {
            gateway: {
              mode: "local",
              auth: { mode: "token", token: "${DAISYCLAW_GATEWAY_TOKEN}" },
            },
        ...
        await withEnvAsync(
          {
            ...
            DAISYCLAW_GATEWAY_TOKEN: "gateway-token-runtime",
          },
          async () => {
            setRuntimeConfigSnapshot(
              {
            ...
            expect(persisted.gateway?.auth?.token).toBe("${DAISYCLAW_GATEWAY_TOKEN}");
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/daisyclaw-test-demo",
          source: "/tmp/daisyclaw-test-demo/index.ts",
          manifestPath: "/tmp/daisyclaw-test-demo/daisyclaw.plugin.json",
          configSchema: {
            type: "object",
            properties: {
      ...
      try {
        await withEnvAsync({ DAISYCLAW_CONFIG_PATH: configPath }, async () => {
          ...
          const postWriteSnapshot = await createConfigIO({
            env: { DAISYCLAW_CONFIG_PATH: configPath, VITEST: "true" } as NodeJS.ProcessEnv,
            homedir: () => home,
            logger: silentLogger,
  ...
  it("rolls back the root config when post-write runtime refresh fails", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".daisyclaw", "daisyclaw.json");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies DaisyClawConfig;
  ...
  it("blocks runtime preflight failures before committing root writes", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".daisyclaw", "daisyclaw.json");
      const initialRaw = `${JSON.stringify({ gateway: { mode: "local" } }, null, 2)}\n`;
      let observedSource: DaisyClawConfig | undefined;
  ...
  it("restores config env vars when post-write runtime refresh rollback succeeds", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".daisyclaw", "daisyclaw.json");
      const envKey = "DAISYCLAW_TEST_RUNTIME_ROLLBACK_ENV";
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies DaisyClawConfig;
      const initialRaw = `${JSON.stringify(initialConfig, null, 2)}\n`;
    ...
    await withSuiteHome(async (home) => {
        ...
        plugins: [
          {
            ...
            skills: [],
            hooks: [],
            rootDir: "/tmp/daisyclaw-test-strict-plugin",
            source: "/tmp/daisyclaw-test-strict-plugin/index.ts",
            manifestPath: "/tmp/daisyclaw-test-strict-plugin/daisyclaw.plugin.json",
            configSchema: {
              type: "object",
      ...
      try {
        ...
        await withEnvAsync({ DAISYCLAW_CONFIG_PATH: configPath }, async () => {
          ...
          await expect(
            writeConfigFile({ agents: { list: "not-array" } } as unknown as DaisyClawConfig, {
  ...
  it("preserves authored tilde paths when runtime-shaped writes hand back absolute paths", async () => {
    await withSuiteHome(async (home) => {
      ...
      await fs.writeFile(
        ...
        `${JSON.stringify(
          {
            logging: { file: "~/daisyclaw-upgrade-survivor/gateway.jsonl" },
          },
          null,
          2,
        )}\n`,
      ...
      await io.writeConfigFile(
        {
          logging: {
            file: path.join(home, "daisyclaw-upgrade-survivor", "gateway.jsonl"),
            level: "debug",
          },
        },
      ...
      const persisted = JSON.parse(await fs.readFile(configPath, "utf-8")) as DaisyClawConfig;
      expect(persisted.logging?.file).toBe("~/daisyclaw-upgrade-survivor/gateway.jsonl");
