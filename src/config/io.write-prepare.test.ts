...
describe("config io write prepare", () => {
      ...
      resolvePersistCandidateForWrite({
        ...
        sourceConfig: {
          plugins: {
            entries: {},
            installs: {
              "daisyclaw-web-search": {
                source: "npm",
                spec: "@ollama/daisyclaw-web-search",
                installPath: "/tmp/daisyclaw-web-search",
                resolvedName: "@ollama/daisyclaw-web-search",
        ...
        nextConfig: {
          plugins: {
            ...
            installs: {
              "daisyclaw-web-search": {
                source: "npm",
                spec: "@ollama/daisyclaw-web-search@0.2.2",
                installPath: "/tmp/daisyclaw-web-search",
                resolvedName: "@ollama/daisyclaw-web-search",
                resolvedVersion: "0.2.2",
              },
  ...
  it('formats actionable guidance for dmPolicy="open" without wildcard allowFrom', () => {
    ...
    expect(message).toContain("daisyclaw config set channels.telegram.allowFrom '[\"*\"]'");
    expect(message).toContain('daisyclaw config set channels.telegram.dmPolicy "pairing"');
  });
  ...
  it("keeps the read-time env snapshot when writing the same config path", () => {
    ...
    expect(
      resolveWriteEnvSnapshotForPath({
        actualConfigPath: "/tmp/daisyclaw.json",
        expectedConfigPath: "/tmp/daisyclaw.json",
  ...
  it("keeps runtime-only channel defaults out of the persisted candidate", () => {
    ...
    const nextConfig: DaisyClawConfig = structuredClone(runtimeConfig);
    nextConfig.gateway = {
      ...nextConfig.gateway,
      auth: { mode: "token" },
    };
  ...
  it("rejects writes that would flatten a root include", () => {
    const sourceConfig = {
      $schema: "https://daisyclaw.ai/config-from-include.json",
      gateway: { mode: "local" },
    };
