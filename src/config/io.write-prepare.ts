...
import { normalizeConfiguredProviderCatalogModelId } from "@daisyclaw/model-catalog-core/provider-model-id-normalization";
...
export function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  daisyclaw config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  daisyclaw config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
export function unsetPathForWrite(
  root: DaisyClawConfig,
  pathSegments: string[],
): { changed: boolean; next: DaisyClawConfig } {
