...
describe("config logging", () => {
  it("formats the live config path when no explicit path is provided", () => {
    expect(formatConfigPath()).toBe("/tmp/daisyclaw-dev/daisyclaw.json");
  });

  it("logs the live config path when no explicit path is provided", () => {
    const runtime = { log: vi.fn() };
    logConfigUpdated(runtime as never);
    expect(runtime.log).toHaveBeenCalledWith("Updated config: /tmp/daisyclaw-dev/daisyclaw.json");
  });

  it("formats backup as an indented detail when present", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-config-log-"));
