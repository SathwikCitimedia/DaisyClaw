...
type DaisyClawMcpHttpTransport = "sse" | "streamable-http";

const CLI_MCP_TYPE_TO_DAISYCLAW_TRANSPORT: Record<string, DaisyClawMcpHttpTransport | "stdio"> = {
...
/** Maps CLI-native MCP type aliases to DaisyClaw HTTP transport names. */
export function resolveDaisyClawMcpTransportAlias(
  value: unknown,
): DaisyClawMcpHttpTransport | undefined {
  const mapped = CLI_MCP_TYPE_TO_DAISYCLAW_TRANSPORT[normalizeMcpString(value)];
  return mapped === "sse" || mapped === "streamable-http" ? mapped : undefined;
}

/** Checks whether a raw MCP `type` value is a legacy CLI alias DaisyClaw can rewrite. */
export function isKnownCliMcpTypeAlias(value: unknown): boolean {
  return Object.hasOwn(CLI_MCP_TYPE_TO_DAISYCLAW_TRANSPORT, normalizeMcpString(value));
}
...
export function canonicalizeConfiguredMcpServer(
  ...
  const next = { ...server };
  const transportAlias = resolveDaisyClawMcpTransportAlias(next.type);
  // `transport` is DaisyClaw's canonical field; legacy `type` only fills a gap.
