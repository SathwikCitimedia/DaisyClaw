...
const mockReadSourceConfigSnapshot = vi.hoisted(() => async () => {
  ...
  const configPath = pathValue.join(process.env.DAISYCLAW_STATE_DIR ?? "", "daisyclaw.json");
  try {
...
const mockReplaceConfigFile = vi.hoisted(() => async ({ nextConfig }: { nextConfig: unknown }) => {
  ...
  const configPath = pathLocal.join(process.env.DAISYCLAW_STATE_DIR ?? "", "daisyclaw.json");
