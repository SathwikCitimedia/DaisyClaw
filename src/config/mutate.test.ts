...
import type { ConfigFileSnapshot, DaisyClawConfig } from "./types.js";

type MockValidationIssue = { path: string; message: string };
type MockValidationResult =
  | { ok: true; config: DaisyClawConfig; warnings: MockValidationIssue[] }
...
const validationMocks = vi.hoisted(() => ({
  validateConfigObjectWithPlugins: vi.fn(
    (config: DaisyClawConfig): MockValidationResult => ({
      ok: true,
      config,
      warnings: [],
    }),
...
function createSnapshot(params: {
  ...
  return {
    path: params.path ?? "/tmp/daisyclaw.json",
    exists: true,
    raw: "{}",
...
describe("config mutate helpers", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "daisyclaw-config-mutate-" });
  const originalNixMode = process.env.DAISYCLAW_NIX_MODE;
  ...
  afterAll(async () => {
    if (originalNixMode === undefined) {
      ...
      process.env.DAISYCLAW_NIX_MODE = originalNixMode;
    }
  ...
  it("refuses mutate writes in Nix mode before touching disk", async () => {
    ...
    ).rejects.toThrow("DaisyClaw Nix overview: https://docs.daisyclaw.ai/install/nix");
  ...
  it("writes through a single-file top-level plugins include", async () => {
    ...
    const pluginsPath = path.join(home, ".daisyclaw", "config", "plugins.json5");
  ...
  it("keeps single-file top-level plugins include writes when plugin validation is skipped", async () => {
    ...
    const nextConfig: DaisyClawConfig = {
      plugins: {
        entries: {
          "strict-plugin": { enabled: true },
        },
  ...
  it("rolls back single-file top-level include writes when runtime refresh fails", async () => {
    ...
    const envKey = "DAISYCLAW_TEST_INCLUDE_ROLLBACK_ENV";
    await fs.mkdir(path.dirname(pluginsPath), { recursive: true });
    await fs.writeFile(
      configPath,
      `${JSON.stringify({ plugins: { $include: "./config/plugins.json5" } }, null, 2)}\n`,
  ...
  it("preflights injected root writers before persisting", async () => {
    ...
    const initialConfig = { gateway: { mode: "local" } } satisfies DaisyClawConfig;
    ...
    const injectedWrite = vi.fn(async (config: DaisyClawConfig, options?: ConfigWriteOptions) => {
