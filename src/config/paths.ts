...
import type { DaisyClawConfig } from "./types.js";

/**
 * Nix mode detection: When DAISYCLAW_NIX_MODE=1, the gateway is running under Nix.
 * In this mode:
...
export function resolveIsNixMode(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.DAISYCLAW_NIX_MODE === "1";
}
...
const NEW_STATE_DIRNAME = ".daisyclaw";
const CONFIG_FILENAME = "daisyclaw.json";
...
/** Build a homedir thunk that respects DAISYCLAW_HOME for the given env. */
 ...
 * Can be overridden via DAISYCLAW_STATE_DIR.
 * Default: ~/.daisyclaw
 */
export function resolveStateDir(
  ...
  const override = env.DAISYCLAW_STATE_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, effectiveHomedir);
  }
  const newDir = newStateDir(effectiveHomedir);
  if (env.DAISYCLAW_TEST_FAST === "1") {
    return newDir;
  }
...
export function normalizeStateDirEnv(env: NodeJS.ProcessEnv = process.env): void {
  ...
  const daisyclawOverride = env.DAISYCLAW_STATE_DIR?.trim();
  if (daisyclawOverride) {
    env.DAISYCLAW_STATE_DIR = resolveUserPath(daisyclawOverride, env, effectiveHomedir);
  }
}
 ...
 * outside the config directory. Set via `DAISYCLAW_INCLUDE_ROOTS` as a
 ...
 * directory containing `daisyclaw.json`.
 */
export function resolveIncludeRoots(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = envHomedir(env),
): string[] {
  const raw = env.DAISYCLAW_INCLUDE_ROOTS?.trim();
  if (!raw) {
    return [];
  }
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
 ...
 * Can be overridden via DAISYCLAW_CONFIG_PATH.
 * Default: ~/.daisyclaw/daisyclaw.json (or $DAISYCLAW_STATE_DIR/daisyclaw.json)
 */
export function resolveCanonicalConfigPath(
  env: NodeJS.ProcessEnv = process.env,
  stateDir: string = resolveStateDir(env, envHomedir(env)),
): string {
  const override = env.DAISYCLAW_CONFIG_PATH?.trim();
  if (override) {
...
export function resolveConfigPath(
  ...
  const stateOverride = env.DAISYCLAW_STATE_DIR?.trim();
  const candidates = [
...
export function resolveDefaultConfigCandidates(
  ...
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
  const explicit = env.DAISYCLAW_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveUserPath(explicit, env, effectiveHomedir)];
  }

  const candidates: string[] = [];
  const daisyclawStateDir = env.DAISYCLAW_STATE_DIR?.trim();
  if (daisyclawStateDir) {
    const resolved = resolveUserPath(daisyclawStateDir, env, effectiveHomedir);
 ...
 * Default: os.tmpdir()/daisyclaw-<uid> (uid suffix when available).
 */
export function resolveGatewayLockDir(tmpdir: () => string = os.tmpdir): string {
  const base = tmpdir();
  const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
  const suffix = uid != null ? `daisyclaw-${uid}` : "daisyclaw";
 ...
 *
 * Precedence:
 * - `DAISYCLAW_OAUTH_DIR` (explicit override)
...
export function resolveOAuthDir(
  ...
  const override = env.DAISYCLAW_OAUTH_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, envHomedir(env));
  }
...
export function resolveGatewayPort(
  ...
  const envRaw = env.DAISYCLAW_GATEWAY_PORT?.trim();
