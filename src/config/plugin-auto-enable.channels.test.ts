...
describe("applyPluginAutoEnable channels", () => {
        ...
        entries: [
          {
            ...
            daisyclaw: {
              ...
              install: {
                npmSpec: "@daisyclaw/env-secondary",
              },
            },
          },
        ],
  ...
  describe("third-party channel plugins (pluginId ≠ channelId)", () => {
    ...
    it("prefers an external plugin that declares preferOver for a bundled channel", () => {
      ...
      expect(result.config.plugins?.entries?.["daisyclaw-modern-chat"]?.enabled).toBe(true);
    ...
    it("falls back to the bundled channel when the preferred external plugin is disabled", () => {
      const result = applyPluginAutoEnable({
        config: {
          channels: { "legacy-bundled-chat": { token: "legacy" } },
          plugins: { entries: { "daisyclaw-modern-chat": { enabled: false } } },
        },
        env: makeIsolatedEnv(),
        manifestRegistry: makeRegistry([
          {
      ...
      expect(result.config.plugins?.entries?.["daisyclaw-modern-chat"]?.enabled).toBe(false);
    ...
    it("does not auto-disable a lower-priority channel plugin that was explicitly selected", () => {
      ...
      expect(result.config.plugins?.entries?.["daisyclaw-qqbot"]?.enabled).toBe(true);
  ...
  describe("preferOver channel prioritization", () => {
    it("uses the plugin manifest id for built-in channel claims", () => {
      ...
      expect(result.config.plugins?.entries?.["wecom-daisyclaw-plugin"]?.enabled).toBe(true);
      expect(result.config.plugins?.entries?.wecom).toBeUndefined();
      expect(result.config.plugins?.allow).toEqual(["existing-plugin", "wecom-daisyclaw-plugin"]);
