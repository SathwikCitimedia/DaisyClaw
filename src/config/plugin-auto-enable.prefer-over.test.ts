...
vi.mock("../plugins/bundled-dir.js", async (importOriginal) => {
  ...
  return {
    ...
    resolveBundledPluginsDir: (env: NodeJS.ProcessEnv = process.env) =>
      env.DAISYCLAW_BUNDLED_PLUGINS_DIR,
  };
});
...
function makeTempDir(): string {
  ...
  const dir = fs.mkdtempSync(path.join(trustedRoot, ".daisyclaw-plugin-prefer-over-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("plugin auto-enable preferOver", () => {
  it("tolerates bundled channel id metadata drift during auto-enable", async () => {
    ...
    vi.stubEnv("DAISYCLAW_BUNDLED_PLUGINS_DIR", rootDir);
    ...
    vi.stubEnv("DAISYCLAW_BUNDLED_PLUGINS_DIR", path.join(rootDir, "missing"));
    await setBundledPluginsDirFixture(undefined);
    ...
    const result = materializePluginAutoEnableCandidates({
      ...
      env: {
        ...
        DAISYCLAW_BUNDLED_PLUGINS_DIR: path.join(rootDir, "missing"),
      },
