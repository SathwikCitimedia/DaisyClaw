...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.daisyclaw) || !isRecord(entry.daisyclaw.channel)) {
      continue;
    }
    const channel = entry.daisyclaw.channel;
...
export function shouldSkipPreferredPluginAutoEnable(params: {
  ...
  env: NodeJS.ProcessEnv;
  registry: PluginManifestRegistry;
  isPluginDenied: (config: DaisyClawConfig, pluginId: string) => boolean;
  isPluginExplicitlyDisabled: (config: DaisyClawConfig, pluginId: string) => boolean;
