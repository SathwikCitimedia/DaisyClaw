...
import { normalizeOptionalLowercaseString } from "@daisyclaw/normalization-core/string-coerce";
...
function canReuseUnscopedCurrentPluginMetadataSnapshot(config: DaisyClawConfig): boolean {
...
function isProviderConfigured(cfg: DaisyClawConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: DaisyClawConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: DaisyClawConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: DaisyClawConfig, plugin: PluginManifestRecord): boolean {
...
function hasConfiguredWebSearchPluginEntry(cfg: DaisyClawConfig): boolean {
...
function hasConfiguredWebSearchProviderSelection(cfg: DaisyClawConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: DaisyClawConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasConfiguredPluginConfigEntry(cfg: DaisyClawConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: DaisyClawConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: DaisyClawConfig): string[] {
  const entries = cfg.plugins?.entries;
...
function hasOwnPluginEntry(cfg: DaisyClawConfig, pluginId: string): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.hasOwn(entries, pluginId);
}

function isPluginEntryExplicitlyDisabled(cfg: DaisyClawConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function hasNonDisabledPluginEntry(cfg: DaisyClawConfig, pluginId: string): boolean {
...
function hasBrowserSetupAutoEnableRelevantConfig(cfg: DaisyClawConfig): boolean {
...
function hasAcpxSetupAutoEnableRelevantConfig(cfg: DaisyClawConfig): boolean {
...
function hasXaiSetupAutoEnableRelevantConfig(cfg: DaisyClawConfig): boolean {
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: DaisyClawConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: DaisyClawConfig): boolean {
  return (
...
function hasPluginEntries(cfg: DaisyClawConfig): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithMaterialEntries(cfg: DaisyClawConfig): boolean {
  if (
...
function hasConfiguredProviderModelOrHarness(cfg: DaisyClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function arePluginsGloballyDisabled(cfg: DaisyClawConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function configMayNeedPluginManifestRegistry(cfg: DaisyClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function isPluginExplicitlyDisabled(cfg: DaisyClawConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: DaisyClawConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isPluginExplicitlySelected(cfg: DaisyClawConfig, pluginId: string): boolean {
...
function disableImplicitPreferredOverPlugin(params: {
  config: DaisyClawConfig;
  originalConfig: DaisyClawConfig;
  pluginId: string;
...
function isBuiltInChannelAlreadyEnabled(cfg: DaisyClawConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
