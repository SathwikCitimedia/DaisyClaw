...
import { isSensitiveUrlConfigPath } from "@daisyclaw/net-policy/redact-sensitive-url";
...
describe("mapSensitivePaths", () => {
  ...
  it("main schema yields correct hints (samples)", () => {
    const schema = DaisyClawSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "DaisyClawConfig";
    const hints = mapSensitivePaths(DaisyClawSchema, "", {});
...
describe("collectMatchingSchemaPaths", () => {
  it("finds base-config URL fields that may embed secrets", () => {
    const paths = collectMatchingSchemaPaths(DaisyClawSchema, "", isSensitiveUrlConfigPath);
