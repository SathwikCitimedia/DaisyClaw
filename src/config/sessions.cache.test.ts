...
describe("Session Store Cache", () => {
  ...
  afterEach(() => {
    clearSessionStoreCacheForTest();
    delete process.env.DAISYCLAW_SESSION_CACHE_TTL_MS;
    delete process.env.DAISYCLAW_SESSION_SERIALIZED_CACHE_MAX_BYTES;
  });

  it("bounds the serialized session store cache by total bytes", () => {
    process.env.DAISYCLAW_SESSION_SERIALIZED_CACHE_MAX_BYTES = "64";
    clearSessionStoreCacheForTest();
  ...
  it("should respect DAISYCLAW_SESSION_CACHE_TTL_MS=0 to disable cache", async () => {
