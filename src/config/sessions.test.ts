...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ DAISYCLAW_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-daisyclaw",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-daisyclaw#general");
  });
  ...
  it("derives session transcripts dir from DAISYCLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { DAISYCLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when DAISYCLAW_STATE_DIR differs from stored paths", () => {
