...
import type { DaisyClawConfig } from "../config.js";
...
describe("session path safety", () => {
  ...
  it("resolves transcript path inside an explicit sessions dir", () => {
    const sessionsDir = "/tmp/daisyclaw/agents/main/sessions";
  ...
  it("falls back to derived path when sessionFile is outside known agent sessions dirs", () => {
    ...
    const resolved = resolveSessionFilePath(
      "sess-1",
      { sessionFile: "/tmp/daisyclaw/agents/work/not-sessions/abc-123.jsonl" },
      { sessionsDir },
    );
  ...
  it("accepts symlink-alias session paths that resolve under the sessions dir", () => {
    ...
    withTempDirSync({ prefix: "daisyclaw-symlink-session-" }, (tmpDir) => {
  ...
  it("falls back when sessionFile is a symlink that escapes sessions dir", () => {
    ...
    withTempDirSync({ prefix: "daisyclaw-symlink-escape-" }, (tmpDir) => {
...
describe("session lifecycle timestamps", () => {
  it("falls back to the JSONL session header for legacy session start time", async () => {
    const dir = await fsPromises.mkdtemp("/tmp/daisyclaw-lifecycle-test-");
    try {
...
describe("session store writer queue", () => {
  const writerFixtureRootTracker = createSuiteTempRootTracker({ prefix: "daisyclaw-writer-test-" });
  ...
  it("clones session store cache hits from cached serialized JSON", () => {
    ...
    const storePath = "/tmp/daisyclaw-serialized-cache-test.json";
    const store = {
      [key]: {
  ...
  it("rewrites generated sessionFile paths when session id changes", () => {
    ...
    const merged = mergeSessionEntry(
      {
        ...
        updatedAt: 100,
        sessionFile: `/tmp/daisyclaw/sessions/${previousSessionId}.jsonl`,
      },
      {
    ...
    expect(merged.sessionFile).toBe(`/tmp/daisyclaw/sessions/${nextSessionId}.jsonl`);
  });

  it("rewrites stale generated sessionFile patches during session rollover", () => {
    const previousSessionId = "11111111-1111-4111-8111-111111111111";
    const nextSessionId = "22222222-2222-4222-8222-222222222222";
    const previousSessionFile = `/tmp/daisyclaw/sessions/${previousSessionId}-topic-456.jsonl`;
    const merged = mergeSessionEntry(
      {
        sessionId: previousSessionId,
        updatedAt: 100,
    ...
    expect(merged.sessionFile).toBe(`/tmp/daisyclaw/sessions/${nextSessionId}-topic-456.jsonl`);
  });
  ...
  it("preserves custom sessionFile paths when session id changes", () => {
    const merged = mergeSessionEntry(
      {
        ...
        updatedAt: 100,
        sessionFile: "/tmp/daisyclaw/sessions/custom-transcript.jsonl",
      },
      {
    ...
    expect(merged.sessionFile).toBe("/tmp/daisyclaw/sessions/custom-transcript.jsonl");
  });
