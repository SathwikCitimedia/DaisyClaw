...
// Keep integration tests deterministic: never read a real daisyclaw.json.
...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "daisyclaw-pruning-integ-" });
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.DAISYCLAW_SESSION_CACHE_TTL_MS;
    process.env.DAISYCLAW_SESSION_CACHE_TTL_MS = "0";
    clearSessionStoreCacheForTest();
  });
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.DAISYCLAW_SESSION_CACHE_TTL_MS;
    } else {
      process.env.DAISYCLAW_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
  ...
  it("removes trajectory sidecars for stale sessions pruned on write", async () => {
    ...
    await fs.writeFile(staleTranscript, '{"type":"session"}\n', "utf-8");
    await fs.writeFile(freshTranscript, '{"type":"session"}\n', "utf-8");
    await fs.writeFile(staleRuntime, '{"traceSchema":"daisyclaw-trajectory"}\n', "utf-8");
    await fs.writeFile(freshRuntime, '{"traceSchema":"daisyclaw-trajectory"}\n', "utf-8");
    await fs.writeFile(
      stalePointer,
      JSON.stringify({
        traceSchema: "daisyclaw-trajectory-pointer",
        schemaVersion: 1,
        sessionId: staleSessionId,
