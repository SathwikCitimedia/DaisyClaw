...
function createCustomRootCfg(customRoot: string, defaultAgentId = "ops"): DaisyClawConfig {
  return {
    session: {
...
const discoveryResolvers = [
  {
    label: "async",
    resolve: async (cfg: DaisyClawConfig, env: NodeJS.ProcessEnv) =>
...
describe("resolveAllAgentSessionStoreTargets", () => {
  ...
  it("respects the caller env when resolving configured and discovered store roots", async () => {
    await withTempHome(async (home) => {
      ...
      const env = {
        ...process.env,
        DAISYCLAW_STATE_DIR: envStateDir,
      };
      const cfg: DaisyClawConfig = {};
