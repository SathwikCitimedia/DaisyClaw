...
export function listConfiguredSessionStoreAgentIds(cfg: DaisyClawConfig): string[] {
...
export function resolveSessionStoreTargets(
  ...
  if (hasAgent) {
    ...
    if (!knownAgents.includes(requested)) {
      throw new Error(
        `Unknown agent id "${opts.agent}". Use "daisyclaw agents list" to see configured agents.`,
      );
    }
    return [
      {
