...
describe("appendAssistantMessageToSessionTranscript", () => {
  ...
  it("emits transcript update events for delivery mirrors", async () => {
    ...
    expect(message?.provider).toBe("daisyclaw");
  ...
  it("skips transcript-only DaisyClaw assistant entries when reading latest assistant text", async () => {
    writeTranscriptStore();
  ...
  it("does not report transcript-only DaisyClaw assistant entries as latest assistant text", async () => {
    writeTranscriptStore();
  ...
  it("keeps transcript-only DaisyClaw assistant entries available to the tail reader", async () => {
    writeTranscriptStore();
  ...
  it("scans past trailing non-assistant entries (e.g. daisyclaw.cache-ttl) to find the latest assistant text", async () => {
    // Regression for daisyclaw/daisyclaw#83427: the cache-ttl custom entry was
  ...
  it("does not reuse an older matching assistant message across turns", async () => {
    ...
    if (olderResult.ok && latestResult.ok && mirrorResult.ok) {
      ...
      expect(messageLine.message.provider).toBe("daisyclaw");
