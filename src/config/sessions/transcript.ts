...
function parseAssistantTranscriptText(
  line: string,
  options?: { excludeTranscriptOnlyDaisyClawAssistant?: boolean },
  ...
  if (
    options?.excludeTranscriptOnlyDaisyClawAssistant &&
    isTranscriptOnlyDaisyClawAssistantMessage(message)
  ) {
    return undefined;
  }
...
function isTranscriptOnlyDaisyClawAssistantMessage(message: {
  provider?: unknown;
  model?: unknown;
}): boolean {
  return isTranscriptOnlyDaisyClawAssistantModel(message.provider, message.model);
}
  ...
  for await (const line of streamSessionTranscriptLinesReverse(sessionFile)) {
    try {
      const assistantText = parseAssistantTranscriptText(line, {
        excludeTranscriptOnlyDaisyClawAssistant: true,
      });
      if (assistantText) {
...
export async function readTailAssistantTextFromSessionTranscript(
  ...
  for await (const line of streamSessionTranscriptLinesReverse(sessionFile)) {
    try {
      ...
      // Skip non-message entries (e.g. `daisyclaw.cache-ttl` custom events) so
...
function isRedundantDeliveryMirror(message: SessionTranscriptAssistantMessage): boolean {
  return message.provider === "daisyclaw" && message.model === "delivery-mirror";
}
