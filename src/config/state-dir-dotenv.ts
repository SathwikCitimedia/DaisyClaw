...
export function isUnresolvedShellReference(value: string): boolean {
  ...
  //   $VAR_NAME          (simple reference, DaisyClaw env-var style)
...
type ParsedStateDirDotEnv = {
   ...
   * reference ($VAR, ${VAR}, or $(cmd)). These are still DaisyClaw-managed keys:
 ...
 * literal references for keys DaisyClaw previously managed.
 */
...
/**
 * Read and parse `~/.daisyclaw/.env` (or `$DAISYCLAW_STATE_DIR/.env`), returning
