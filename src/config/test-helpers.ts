...
import { withTempHome as withTempHomeBase } from "daisyclaw/plugin-sdk/test-env";
import { resetPluginLoaderTestStateForTest } from "../plugins/loader.test-fixtures.js";
import { clearPluginSetupRegistryCache } from "../plugins/setup-registry.js";
import { resetConfigRuntimeState, type DaisyClawConfig } from "./config.js";
  ...
  try {
    return await withTempHomeBase(fn, {
      ...
      env: {
        ...
        DAISYCLAW_MPM_CATALOG_PATHS: undefined,
        DAISYCLAW_LOAD_SHELL_ENV: undefined,
        DAISYCLAW_DEFER_SHELL_ENV_FALLBACK: undefined,
        DAISYCLAW_SHELL_ENV_TIMEOUT_MS: undefined,
...
export async function writeDaisyClawConfig(home: string, config: unknown): Promise<string> {
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.DAISYCLAW_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected DAISYCLAW_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeDaisyClawConfig(home, config);
