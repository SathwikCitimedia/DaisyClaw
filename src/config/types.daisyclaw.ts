// Defines the top-level DaisyClaw configuration type.
...
/** Top-level DaisyClaw config as read from user/project config files. */
export type DaisyClawConfig = {
  /** JSON schema URL used by editors and generated config files. */
  $schema?: string;
  meta?: {
    /** Last DaisyClaw version that wrote this config. */
  ...
  wizard?: {
    ...
    lastRunAt?: string;
    /** DaisyClaw version used by the last completed wizard run. */
  ...
  ui?: {
    /** Accent color for DaisyClaw UI chrome (hex). */
    seamColor?: string;
    assistant?: {
...
export type DaisyClawConfigInput = Omit<DaisyClawConfig, "models"> & {
  models?: ModelsConfigInput;
};

declare const daisyClawConfigStateBrand: unique symbol;

type BrandedConfigState<TState extends string> = DaisyClawConfig & {
  readonly [daisyClawConfigStateBrand]?: TState;
};
