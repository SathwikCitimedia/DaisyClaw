// Validates normalized DaisyClaw config and reports user-facing errors.
import path from "node:path";
import { collectConfiguredModelRefs } from "@daisyclaw/model-catalog-core/configured-model-refs";
import { isCanonicalDottedDecimalIPv4, isLoopbackIpAddress } from "@daisyclaw/net-policy/ip";
...
import type { DaisyClawConfig, ConfigValidationIssue } from "./types.js";
...
function formatRemovedPluginConfigWarning(pluginId: string): string {
  if (pluginId === "skill-workshop") {
    return "plugin removed: skill-workshop (stale plugin config ignored; Skill Workshop is built into DaisyClaw skills now. Use skills.workshop settings and daisyclaw skills workshop commands, then remove this plugins config entry)";
  }
...
function materializeBundledModelProviderOverlays(config: DaisyClawConfig): DaisyClawConfig {
...
const SECRETREF_POLICY_DOC_URL = "https://docs.daisyclaw.ai/reference/secretref-credential-surface";
...
function formatMissingOfficialExternalPluginWarning(
  ...
  if (pluginId === "memory-lancedb" && opts?.selectedMissingMemorySlot) {
    return `plugin not installed: ${pluginId} — gateway will run without persistent memory until installed; install the official external plugin with: daisyclaw plugins install ${installSpec}`;
  }
  return `plugin not installed: ${pluginId} — install the official external plugin with: daisyclaw plugins install ${installSpec}`;
}
...
function collectRawBundledChannelConfigIssues(config: DaisyClawConfig): ConfigValidationIssue[] {
...
function validateIdentityAvatar(config: DaisyClawConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: DaisyClawConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleAuth(config: DaisyClawConfig): ConfigValidationIssue[] {
  const tailscaleMode = config.gateway?.tailscale?.mode ?? "off";
...
export function validateConfigObjectRaw(
...
): { ok: true; config: DaisyClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = DaisyClawSchema.safeParse(normalizedRaw);
  ...
  const validatedConfig = materializeBundledModelProviderOverlays(validated.data as DaisyClawConfig);
  const channelIssues =
...
function validateConfigObjectWithPluginsBase(
  ...
  let compatConfig: DaisyClawConfig | null | undefined;
  ...
  const ensureCompatConfig = (): DaisyClawConfig => {
  ...
  const validateWebSearchProvider = () => {
    ...
    if (installCatalogEntry) {
      const issue = {
        path: pathEntry,
        message: `web_search provider is not available: ${trimmed} (install or enable plugin "${installCatalogEntry.pluginId}", then run daisyclaw doctor --fix)`,
      ...
      if (hasPluginEvidenceForWebSearchProvider(trimmed, installCatalogEntry.pluginId)) {
        warnings.push({
          ...issue,
          message: `web_search provider is not available: ${trimmed} (configured plugin "${installCatalogEntry.pluginId}" is unavailable; Gateway will ignore this optional provider until the plugin is installed/enabled or daisyclaw doctor --fix repairs the config)`,
        });
        return;
      }
      issues.push(issue);
    ...
    if (hasStalePluginEvidenceForUnknownWebSearchProvider(trimmed)) {
      warnings.push({
        ...issue,
        message: `${issue.message} (stale web search plugin config ignored; run daisyclaw doctor --fix to remove stale config, or install the plugin)`,
      });
      return;
    }
    issues.push(issue);
    ...
    for (const key of Object.keys(config.channels)) {
      ...
      if (!allowedChannels.has(trimmed)) {
        ...
        if (hasStalePluginEvidenceForUnknownChannel(trimmed)) {
          warnings.push({
            ...issue,
            message: `${issue.message} (stale channel plugin config ignored; run daisyclaw doctor --fix to remove stale config, or install the plugin)`,
          });
        } else {
          issues.push(issue);
        }
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native DaisyClaw config schema.
