...
/** Parses stable, prerelease, and legacy dot-beta DaisyClaw versions. */
export function parseDaisyClawVersion(raw: string | null | undefined): DaisyClawVersion | null {
  if (!raw) {
    return null;
  }
...
export function normalizeDaisyClawVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseDaisyClawVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameDaisyClawStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseDaisyClawVersion(a);
  const parsedB = parseDaisyClawVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareDaisyClawVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseDaisyClawVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseDaisyClawVersion(current);
  const parsedTouched = parseDaisyClawVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameDaisyClawStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareDaisyClawVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: DaisyClawVersion): number {
