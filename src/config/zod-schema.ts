// Assembles the canonical Zod schema for DaisyClaw config parsing.
import {
...
const TailscaleServiceNameSchema = z.string().regex(/^svc:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?$/, {
  message:
    'Tailscale serviceName must use the "svc:<dns-label>" format, for example "svc:daisyclaw"',
});
...
export const DaisyClawSchema = z
  .object({
          ...
          .record(
            ...
            z
              .object({
                ...
                driver: z
                  .union([z.literal("daisyclaw"), z.literal("clawd"), z.literal("existing-session")])
                  .optional(),
                headless: z.boolean().optional(),
                executablePath: z.string().optional(),
                attachOnly: z.boolean().optional(),
