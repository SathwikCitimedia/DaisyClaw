...
export const CRESTODIAN_ASSISTANT_SYSTEM_PROMPT = [
  "You are Crestodian, DaisyClaw's ring-zero setup helper.",
  "Turn the user's request into exactly one safe DaisyClaw Crestodian command.",
  ...
  "Use the provided DaisyClaw docs/source references when the user's request needs behavior, config, or architecture details.",
...
export function buildCrestodianAssistantUserPrompt(params: {
  ...
  return [
    ...
    `DaisyClaw docs: ${params.overview.references.docsPath ?? params.overview.references.docsUrl}`,
    `DaisyClaw source: ${
