...
vi.mock("./overview.js", () => ({
  ...
  loadCrestodianOverview: vi.fn(async () => ({
    ...
    config: { path: "/tmp/daisyclaw.json", exists: true, valid: true, issues: [], hash: null },
    tools: {
    ...
    references: {
      docsUrl: "https://docs.daisyclaw.ai",
      sourceUrl: "https://github.com/daisyclaw/daisyclaw",
    },
  })),
}));
...
describe("parseCrestodianOperation", () => {
  ...
  it("parses plugin management operations", () => {
    ...
    expect(parseCrestodianOperation("install npm plugin @daisyclaw/demo")).toEqual({
      kind: "plugin-install",
      spec: "npm:@daisyclaw/demo",
    });
    expect(parseCrestodianOperation("plugin install clawhub:daisyclaw-demo")).toEqual({
      kind: "plugin-install",
      spec: "clawhub:daisyclaw-demo",
    });
    expect(parseCrestodianOperation("plugin uninstall daisyclaw-demo")).toEqual({
      kind: "plugin-uninstall",
      pluginId: "daisyclaw-demo",
    });
  });
  ...
  it("validates missing config without exiting the process", async () => {
    mockConfig.missing("/tmp/daisyclaw.json");
    const { runtime, lines } = createCrestodianTestRuntime();
  ...
  it("installs plugins only after approval and audits the write", async () => {
    ...
    const plan = await executeCrestodianOperation(
      { kind: "plugin-install", spec: "clawhub:daisyclaw-demo" },
      runtime,
      { deps: { runPluginInstall } },
    );
    expectRecordFields(plan as unknown as Record<string, unknown>, {
      applied: false,
      message: "Plan: install plugin clawhub:daisyclaw-demo. Say yes to apply.",
    });
    ...
    expect(installCall[0]).toBe("clawhub:daisyclaw-demo");
    ...
    expectAuditRecord(
      audit,
      {
        ...
        summary: "Installed plugin clawhub:daisyclaw-demo",
      },
      { rescue: true, spec: "clawhub:daisyclaw-demo" },
    );
  });
  ...
  it("uninstalls plugins only after approval and audits the write", async () => {
    ...
    const plan = await executeCrestodianOperation(
      { kind: "plugin-uninstall", pluginId: "daisyclaw-demo" },
      runtime,
      { deps: { runPluginUninstall } },
    );
    expectRecordFields(plan as unknown as Record<string, unknown>, {
      applied: false,
      message: "Plan: uninstall plugin daisyclaw-demo. Say yes to apply.",
    });
    ...
    expect(uninstallCall[0]).toBe("daisyclaw-demo");
    ...
    expectAuditRecord(
      audit,
      {
        ...
        summary: "Uninstalled plugin daisyclaw-demo",
      },
      { rescue: true, pluginId: "daisyclaw-demo" },
    );
  });
