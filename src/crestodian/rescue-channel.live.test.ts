...
const runLive =
  truthy(process.env.DAISYCLAW_LIVE_TEST) &&
  truthy(process.env.DAISYCLAW_LIVE_CRESTODIAN_RESCUE_CHANNEL);
const describeLive = runLive ? describe : describe.skip;

function commandContext(channel = process.env.DAISYCLAW_LIVE_CRESTODIAN_CHANNEL ?? "whatsapp") {
  return {
    surface: channel,
    channel,
    channelId: channel,
...
describeLive("Crestodian live rescue channel smoke", () => {
  ...
  it("handles /crestodian status and a persistent approval roundtrip", async () => {
    ...
    const config = JSON.parse(await fs.readFile(configPath, "utf8")) as DaisyClawConfig;
