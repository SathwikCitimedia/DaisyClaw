...
async function withRescueStateDir(
  ...
  await withEnvAsync({ DAISYCLAW_STATE_DIR: stateDir }, async () => await run(stateDir));
}
...
describe("Crestodian rescue message", () => {
  ...
  it("refuses TUI handoff from remote rescue", async () => {
    const cfg: DaisyClawConfig = { crestodian: { rescue: { enabled: true } } };
    const deps = {
      runTui: vi.fn(async () => {
  ...
  it("refuses plugin install from remote rescue", async () => {
    ...
    await expect(
      runRescue("/crestodian plugin install clawhub:daisyclaw-demo", cfg, commandContext(), deps),
