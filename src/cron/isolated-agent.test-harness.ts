...
/** Runs a test callback with an isolated DaisyClaw home for cron tests. */
export async function withTempCronHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "daisyclaw-cron-" });
}
...
export async function writeSessionStoreEntries(
  ...
  const dir = path.join(home, ".daisyclaw", "sessions");
...
export function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<DaisyClawConfig> = {},
): DaisyClawConfig {
  const base: DaisyClawConfig = {
    agents: {
      defaults: {
