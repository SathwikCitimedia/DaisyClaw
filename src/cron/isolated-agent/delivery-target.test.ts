      ...
      },
      {
        ...
        plugin: {
          ...
          config: {
            listAccountIds: () => [],
            resolveAccount: () => ({}),
            resolveAllowFrom: ({ cfg }: { cfg: DaisyClawConfig }) =>
...
function makeCfg(overrides?: Partial<DaisyClawConfig>): DaisyClawConfig {
  return {
    bindings: [],
    channels: {},
    ...overrides,
...
function makeForumBoundCfg(accountId = "account-b"): DaisyClawConfig {
  return makeCfg({
    bindings: [
      {
        agentId: AGENT_ID,
...
async function resolveLastTarget(cfg: DaisyClawConfig) {
