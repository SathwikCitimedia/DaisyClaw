...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-cron-store-"));
});
...
describe("resolveCronStorePath", () => {
  ...
  it("uses DAISYCLAW_HOME for tilde expansion", () => {
    vi.stubEnv("DAISYCLAW_HOME", "/srv/daisyclaw-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/daisyclaw-home", "cron", "jobs.json"));
  });
});
