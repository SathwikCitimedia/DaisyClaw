/** Inspects installed platform services for extra DaisyClaw or legacy gateway jobs. */
...
const EXTRA_MARKERS = ["daisyclaw", "clawdbot"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
  ...
  const markerKeys = ["daisyclaw_service_marker"];
  const kindKeys = ["daisyclaw_service_kind"];
...
function isDaisyClawGatewayLaunchdService(label: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (detectLaunchdGatewayExecutionMarker(contents) !== "daisyclaw") {
    return false;
  }
  return label.startsWith("ai.daisyclaw.");
}

function isDaisyClawGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("daisyclaw-gateway")) {
    return false;
  }
  return normalizeLowercaseStringOrEmpty(contents).includes("gateway");
}

function isDaisyClawGatewayTaskName(name: string): boolean {
  const normalized = normalizeLowercaseStringOrEmpty(name);
  if (!normalized) {
    return false;
  }
  const defaultName = normalizeLowercaseStringOrEmpty(resolveGatewayWindowsTaskName());
  return normalized === defaultName || normalized.startsWith("daisyclaw gateway");
}
...
async function scanLaunchdDir(params: {
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    const marker =
      hasGatewayServiceMarker(contents) || executionMarker === "daisyclaw"
        ? "daisyclaw"
    ...
    if (marker === "daisyclaw" && isDaisyClawGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "daisyclaw" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  dir: string;
  scope: "user" | "system";
  includeManagedDaisyClaw?: boolean;
}): Promise<ExtraGatewayService[]> {
  const results: ExtraGatewayService[] = [];
  const candidates = await collectServiceFiles({
    dir: params.dir,
    extension: ".service",
    isIgnoredName: params.includeManagedDaisyClaw ? () => false : isIgnoredSystemdName,
  });
  ...
  for (const { entry, name, fullPath, contents } of candidates) {
    ...
    if (
      !params.includeManagedDaisyClaw &&
      marker === "daisyclaw" &&
      isDaisyClawGatewaySystemdService(name, contents)
    ) {
      continue;
    }
    results.push({
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isDaisyClawGatewayTaskName(name)) {
        continue;
      }
