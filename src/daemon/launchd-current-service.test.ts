...
describe("isCurrentProcessLaunchdServiceLabel", () => {
  it("matches launchd-provided service labels", () => {
    expect(
      isCurrentProcessLaunchdServiceLabel("ai.daisyclaw.gateway", {
        LAUNCH_JOB_LABEL: "ai.daisyclaw.gateway",
      }),
    ).toBe(true);
  });

  it("falls back to DaisyClaw service markers when XPC_SERVICE_NAME is inherited", () => {
    expect(
      isCurrentProcessLaunchdServiceLabel("ai.daisyclaw.gateway", {
        XPC_SERVICE_NAME: "0",
