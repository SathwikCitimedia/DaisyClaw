...
export function isCurrentProcessLaunchdServiceLabel(
  ...
  const configuredLabel = normalizeOptionalString(env.DAISYCLAW_LAUNCHD_LABEL);
  if (!configuredLabel || configuredLabel !== label) {
    return false;
  }
  if (
    normalizeOptionalString(env.DAISYCLAW_SERVICE_MARKER) === "daisyclaw" &&
    Boolean(normalizeOptionalString(env.DAISYCLAW_SERVICE_KIND))
  ) {
