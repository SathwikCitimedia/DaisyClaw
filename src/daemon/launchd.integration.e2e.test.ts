...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `daisyclaw-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      DAISYCLAW_LAUNCHD_LABEL: `ai.daisyclaw.launchd-int-${testId}`,
      DAISYCLAW_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
