...
import {
  ...
  disableDaisyClawUpdateLaunchdJob,
  findStaleDaisyClawUpdateLaunchdJobs,
  isLaunchAgentListed,
  isDaisyClawUpdateLaunchdLabel,
  parseLaunchctlPrint,
  parseLaunchctlListDaisyClawUpdateJobs,
  readLaunchAgentProgramArguments,
  readLaunchAgentRuntime,
  repairLaunchAgentBootstrap,
  removeDaisyClawUpdateLaunchdJob,
  restartLaunchAgent,
...
function expectLaunchctlEnableBootstrapOrder(env: Record<string, string | undefined>) {
  ...
  const label = "ai.daisyclaw.gateway";
...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 ai.daisyclaw.gateway\n";
    const listed = await isLaunchAgentListed({
      env: { HOME: "/Users/test", DAISYCLAW_PROFILE: "default" },
    });
  ...
  it("parses stale DaisyClaw updater jobs from launchctl list", () => {
    const jobs = parseLaunchctlListDaisyClawUpdateJobs(
      [
        "123 0 ai.daisyclaw.gateway",
        "- 127 ai.daisyclaw.update.2026.5.12",
        "- 0 ai.daisyclaw.manual-update.1717168800",
        "8142 0 ai.daisyclaw.update.2026.5.13-beta.1",
        "- 0 ai.daisyclaw.manual-updater.1717168800",
        "- 0 com.example.other",
      ].join("\n"),
    );
    ...
    expect(jobs).toEqual([
      {
        label: "ai.daisyclaw.manual-update.1717168800",
        lastExitStatus: 0,
      },
      {
        label: "ai.daisyclaw.update.2026.5.12",
        lastExitStatus: 127,
      },
      {
        label: "ai.daisyclaw.update.2026.5.13-beta.1",
        pid: 8142,
        lastExitStatus: 0,
      },
    ]);
  ...
  it.runIf(process.platform === "darwin")(
    "finds stale DaisyClaw updater jobs via launchctl list",
    async () => {
      state.listOutput = "- 127 ai.daisyclaw.update.2026.5.12\n";

      const jobs = await findStaleDaisyClawUpdateLaunchdJobs();
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      state.listOutput = [
        "- 0 ai.daisyclaw.manual-update.1717168800",
        "812 0 ai.daisyclaw.manual-update.profile",
        "913 0 ai.daisyclaw.manual-update.custom-label",
      ].join("\n");

      const jobs = await findStaleDaisyClawUpdateLaunchdJobs({
        DAISYCLAW_PROFILE: "manual-update.profile",
        DAISYCLAW_LAUNCHD_LABEL: "ai.daisyclaw.manual-update.custom-label",
        DAISYCLAW_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
        DAISYCLAW_SERVICE_KIND: GATEWAY_SERVICE_KIND,
  ...
  it("recognizes only DaisyClaw updater launchd labels", () => {
    expect(isDaisyClawUpdateLaunchdLabel("ai.daisyclaw.update.2026.5.12")).toBe(true);
    expect(isDaisyClawUpdateLaunchdLabel("ai.daisyclaw.manual-update.1717168800")).toBe(true);
    expect(isDaisyClawUpdateLaunchdLabel("ai.daisyclaw.gateway")).toBe(false);
    expect(isDaisyClawUpdateLaunchdLabel("ai.daisyclaw.manual-update.gateway")).toBe(false);
    expect(isDaisyClawUpdateLaunchdLabel("ai.daisyclaw.manual-update.profile")).toBe(false);
    expect(isDaisyClawUpdateLaunchdLabel("ai.daisyclaw.manual-updater.1717168800")).toBe(false);
    expect(isDaisyClawUpdateLaunchdLabel("com.example.update")).toBe(false);
  });

  it.runIf(process.platform === "darwin")("removes legacy updater launchd jobs", async () => {
    await expect(removeDaisyClawUpdateLaunchdJob(" ai.daisyclaw.update.2026.5.12 ")).resolves.toBe(
      true,
    );

    expect(state.launchctlCalls).toContainEqual(["remove", "ai.daisyclaw.update.2026.5.12"]);
  });
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentDaisyClawUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.daisyclaw.update.2026.5.12",
        }),
      ).resolves.toBe(true);
      ...
      expect(state.launchctlCalls).toContainEqual([
        "disable",
        `${domain}/ai.daisyclaw.update.2026.5.12`,
      ]);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentDaisyClawUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.daisyclaw.manual-update.1717168800",
        }),
      ).resolves.toBe(true);
      ...
      expect(state.launchctlCalls).toContainEqual([
        "disable",
        `${domain}/ai.daisyclaw.manual-update.1717168800`,
      ]);
      expect(launchctlCommandNames()).not.toContain("remove");
    },
  );
  ...
  it.runIf(process.platform === "darwin")(
    "disables the current legacy updater launchd job from DaisyClaw label env",
    async () => {
      await expect(
        disableCurrentDaisyClawUpdateLaunchdJob({
          DAISYCLAW_LAUNCHD_LABEL: "ai.daisyclaw.update.2026.5.12",
        }),
      ).resolves.toBe(true);
  ...
  it.runIf(process.platform === "darwin")(
    "does not let non-update launchd markers mask the DaisyClaw update label",
    async () => {
      await expect(
        disableCurrentDaisyClawUpdateLaunchdJob({
          XPC_SERVICE_NAME: "0",
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentDaisyClawUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.daisyclaw.gateway",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentDaisyClawUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.daisyclaw.update.2026.5.12",
          DAISYCLAW_PROFILE: "update.2026.5.12",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentDaisyClawUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.daisyclaw.manual-update.1717168800",
          DAISYCLAW_PROFILE: "manual-update.1717168800",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentDaisyClawUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.daisyclaw.manual-update.gateway",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")("disables explicit legacy updater jobs", async () => {
    await expect(disableDaisyClawUpdateLaunchdJob("ai.daisyclaw.update.2026.5.12")).resolves.toBe(
      true,
    );
  ...
  it.runIf(process.platform === "darwin")("disables explicit manual updater jobs", async () => {
    await expect(
      disableDaisyClawUpdateLaunchdJob("ai.daisyclaw.manual-update.1717168800"),
    ).resolves.toBe(true);
...
describe("launchd install", () => {
  ...
  it("writes LaunchAgent environment to an owner-only env file when provided", async () => {
    const env = createDefaultLaunchdEnv();
    const tmpDir = "/Users/test/.daisyclaw/tmp";
    ...
    const plistPath = resolveLaunchAgentPlistPath(env);
    const envFilePath = "/Users/test/.daisyclaw/service-env/ai.daisyclaw.gateway.env";
    const wrapperPath = "/Users/test/.daisyclaw/service-env/ai.daisyclaw.gateway-env-wrapper.sh";
    ...
    expect(state.dirModes.get("/Users/test/.daisyclaw/service-env")).toBe(0o700);
  ...
  it("repairs a mangled label-derived service-env wrapper path on restart", async () => {
    ...
    const serviceEnv = {
      ...callerEnv,
      DAISYCLAW_STATE_DIR: "/Users/test/service-env/custom-state",
    };
    await installLaunchAgent({
      env: serviceEnv,
      stdout: new PassThrough(),
      ...
      environment: {
        ...
        DAISYCLAW_STATE_DIR: serviceEnv.DAISYCLAW_STATE_DIR,
      },
    });

    const plistPath = resolveLaunchAgentPlistPath(callerEnv);
    const envFilePath = "/Users/test/service-env/custom-state/service-env/ai.daisyclaw.gateway.env";
    const wrapperPath =
      "/Users/test/service-env/custom-state/service-env/ai.daisyclaw.gateway-env-wrapper.sh";
    const callerEnvFilePath = "/Users/test/.daisyclaw/service-env/ai.daisyclaw.gateway.env";
    const callerWrapperPath =
      "/Users/test/.daisyclaw/service-env/ai.daisyclaw.gateway-env-wrapper.sh";
    const mangledEnvFilePath =
      "/Users/test/service-env/custom-state/service-env/[ai.daisyclaw.gateway.env](http:/ai.daisyclaw.gateway.env)";
    const mangledWrapperPath =
      "/Users/test/service-env/custom-state/service-env/[ai.daisyclaw.gateway-env-wrapper.sh](http:/ai.daisyclaw.gateway-env-wrapper.sh)";
    state.files.set(
      plistPath,
    ...
    expect(command?.programArguments).toEqual(defaultProgramArguments);
    expect(command?.environment?.DAISYCLAW_GATEWAY_PORT).toBe("18789");
    expect(command?.environment?.DAISYCLAW_STATE_DIR).toBe(serviceEnv.DAISYCLAW_STATE_DIR);
    expect(command?.environmentValueSources?.DAISYCLAW_GATEWAY_PORT).toBe("file");
    ...
    expect(rewrittenEnv).toContain("export DAISYCLAW_GATEWAY_PORT='18789'");
    expect(rewrittenEnv).toContain(
      "export DAISYCLAW_STATE_DIR='/Users/test/service-env/custom-state'",
    );
  });
  ...
  it("writes KeepAlive=true policy with shutdown and throttle limits", async () => {
    ...
    expect(plist).toContain("<string>/Users/test/Library/Logs/daisyclaw/gateway.log</string>");
  ...
  it("rewrites the plist before bootstrap during restart fallback", async () => {
    ...
    state.files.set(
      ...
      [
        ...
        "  <dict>",
        "    <key>Label</key>",
        "    <string>ai.daisyclaw.gateway</string>",
  ...
  it("stops LaunchAgent via bootout by default, preserving KeepAlive for future crashes", async () => {
    ...
    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const serviceId = `${domain}/ai.daisyclaw.gateway`;
    ...
    await withProcessEnv(
      ...
      async () => {
        await expect(stopLaunchAgent({ env, stdout: new PassThrough() })).rejects.toThrow(
          "Refusing to stop LaunchAgent ai.daisyclaw.gateway from inside the same launchd service",
        );
      },
    );
    ...
    await withProcessEnv(
      {
        ...
        DAISYCLAW_SERVICE_MARKER: "daisyclaw",
        DAISYCLAW_SERVICE_KIND: "gateway",
        DAISYCLAW_LAUNCHD_LABEL: "ai.daisyclaw.gateway",
      },
      async () => {
        await expect(stopLaunchAgent({ env, stdout: new PassThrough() })).rejects.toThrow(
          "Refusing to stop LaunchAgent ai.daisyclaw.gateway from inside the same launchd service",
  ...
  it("allows external LaunchAgent label overrides to stop the selected target", async () => {
    const env = {
      ...
      DAISYCLAW_LAUNCHD_LABEL: "com.example.daisyclaw.gateway",
    };
    const stdout = new PassThrough();
    let output = "";
    stdout.on("data", (chunk: Buffer) => {
    ...
    await withProcessEnv(
      {
        ...
        DAISYCLAW_LAUNCHD_LABEL: undefined,
        DAISYCLAW_SERVICE_MARKER: undefined,
        DAISYCLAW_SERVICE_KIND: undefined,
      },
      async () => {
        await stopLaunchAgent({ env, stdout });
      },
    ...
    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const serviceId = `${domain}/com.example.daisyclaw.gateway`;
    expect(state.launchctlCalls).toEqual([["bootout", serviceId]]);
    expect(output).toContain("Stopped LaunchAgent");
  });
  ...
  it("resolves the stop postcondition port from the stored LaunchAgent environment", async () => {
    ...
    await installLaunchAgent({
      ...
      stdout: new PassThrough(),
      programArguments: defaultProgramArguments,
      environment: { DAISYCLAW_GATEWAY_PORT: "19006" },
    });
    state.launchctlCalls.length = 0;
  ...
  it("reloads launchd after rewriting an existing plist", async () => {
    ...
    state.files.set(
      ...
      [
        ...
        "    <string>/Users/test/.daisyclaw-default/logs/gateway.log</string>",
        "  </dict>",
        "</plist>",
      ].join("\n"),
    );
  ...
  it("uses the stored LaunchAgent environment port for restart stale cleanup", async () => {
    ...
    await installLaunchAgent({
      ...
      stdout: new PassThrough(),
      programArguments: defaultProgramArguments,
      environment: { DAISYCLAW_GATEWAY_PORT: "19007" },
    });
    state.launchctlCalls.length = 0;
  ...
  it("ignores invalid stored LaunchAgent environment ports for stale cleanup", async () => {
    ...
    await installLaunchAgent({
      ...
      stdout: new PassThrough(),
      programArguments: defaultProgramArguments,
      environment: { DAISYCLAW_GATEWAY_PORT: "65536" },
    });
    state.launchctlCalls.length = 0;
  ...
  it("hands restart off to a detached helper when invoked from the current LaunchAgent", async () => {
    ...
    const result = await withProcessEnv({ LAUNCH_JOB_LABEL: "ai.daisyclaw.gateway" }, async () =>
      restartLaunchAgent({
        env,
        stdout: new PassThrough(),
      }),
  ...
  it("hands plist reload off when current LaunchAgent needs rewritten paths", async () => {
    ...
    expect(state.files.get(plistPath)).toContain("/Users/test/Library/Logs/daisyclaw/gateway.log");
    expect(state.launchctlCalls).toStrictEqual([]);
  });
  ...
  it("surfaces detached handoff failures", async () => {
    ...
    await expect(
      withProcessEnv({ LAUNCH_JOB_LABEL: "ai.daisyclaw.gateway" }, async () =>
        restartLaunchAgent({
          env,
          stdout: new PassThrough(),
        }),
  ...
  it("shows actionable guidance when launchctl gui domain does not support bootstrap", async () => {
    ...
    expect(message).toContain("https://docs.daisyclaw.ai/gateway");
  });
...
describe("resolveLaunchAgentPlistPath", () => {
  it.each([
    {
      name: "uses default label when DAISYCLAW_PROFILE is unset",
      env: { HOME: "/Users/test" },
      expected: "/Users/test/Library/LaunchAgents/ai.daisyclaw.gateway.plist",
    },
    {
      name: "uses profile-specific label when DAISYCLAW_PROFILE is set to a custom value",
      env: { HOME: "/Users/test", DAISYCLAW_PROFILE: "jbphoenix" },
      expected: "/Users/test/Library/LaunchAgents/ai.daisyclaw.jbphoenix.plist",
    },
    {
      name: "prefers DAISYCLAW_LAUNCHD_LABEL over DAISYCLAW_PROFILE",
      env: {
        HOME: "/Users/test",
        DAISYCLAW_PROFILE: "jbphoenix",
        DAISYCLAW_LAUNCHD_LABEL: "com.custom.label",
      },
      expected: "/Users/test/Library/LaunchAgents/com.custom.label.plist",
    },
    {
      name: "trims whitespace from DAISYCLAW_LAUNCHD_LABEL",
      env: {
        HOME: "/Users/test",
        DAISYCLAW_LAUNCHD_LABEL: "  com.custom.label  ",
      },
    ...
    {
      name: "ignores empty DAISYCLAW_LAUNCHD_LABEL and falls back to profile",
      env: {
        HOME: "/Users/test",
        DAISYCLAW_PROFILE: "myprofile",
        DAISYCLAW_LAUNCHD_LABEL: "   ",
      },
      expected: "/Users/test/Library/LaunchAgents/ai.daisyclaw.myprofile.plist",
    },
  ])("$name", ({ env, expected }) => {
  ...
  it("rejects invalid launchd labels that contain path separators", () => {
    expect(() =>
      resolveLaunchAgentPlistPath({
        HOME: "/Users/test",
        DAISYCLAW_LAUNCHD_LABEL: "../evil/label",
      }),
