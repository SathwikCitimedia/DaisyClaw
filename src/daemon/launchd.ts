...
import { normalizeLowercaseStringOrEmpty } from "@daisyclaw/normalization-core/string-coerce";
...
const DAISYCLAW_UPDATE_LAUNCHD_LABEL_PREFIX = "ai.daisyclaw.update.";
const DAISYCLAW_MANUAL_UPDATE_LAUNCHD_LABEL_PATTERN = /^ai\.daisyclaw\.manual-update\.\d+$/;

export type StaleDaisyClawUpdateLaunchdJob = {
  label: string;
  pid?: number;
  lastExitStatus?: number;
};

function normalizeDaisyClawUpdateLaunchdLabel(label: unknown): string | null {
  if (typeof label !== "string") {
    return null;
  }
  const trimmed = label.trim();
  if (trimmed.startsWith(DAISYCLAW_UPDATE_LAUNCHD_LABEL_PREFIX)) {
    return trimmed;
  }
  // Manual update jobs include a timestamp-like suffix and should be cleaned up
  // without matching arbitrary ai.daisyclaw labels.
  return DAISYCLAW_MANUAL_UPDATE_LAUNCHD_LABEL_PATTERN.test(trimmed) ? trimmed : null;
}

function isCurrentGatewayLaunchdLabel(label: string, env: NodeJS.ProcessEnv): boolean {
  const gatewayProfileLabel = resolveGatewayLaunchAgentLabel(env.DAISYCLAW_PROFILE);
  if (label === gatewayProfileLabel) {
    return true;
  }
  if (
    env.DAISYCLAW_SERVICE_MARKER?.trim() !== GATEWAY_SERVICE_MARKER ||
    env.DAISYCLAW_SERVICE_KIND?.trim() !== GATEWAY_SERVICE_KIND
  ) {
    return false;
  }
  const configuredLabel = env.DAISYCLAW_LAUNCHD_LABEL?.trim();
  return Boolean(configuredLabel && label === configuredLabel);
}

export function isDaisyClawUpdateLaunchdLabel(label: unknown): label is string {
  return normalizeDaisyClawUpdateLaunchdLabel(label) !== null;
}

function resolveCurrentDaisyClawUpdateLaunchdJobLabel(
  ...
  for (const label of [
    ...
    env.DAISYCLAW_LAUNCHD_LABEL,
  ]) {
    const normalized = normalizeDaisyClawUpdateLaunchdLabel(label);
    if (normalized) {
...
function resolveLaunchAgentLabel(args?: { env?: Record<string, string | undefined> }): string {
  const envLabel = args?.env?.DAISYCLAW_LAUNCHD_LABEL?.trim();
  if (envLabel) {
    return assertValidLaunchAgentLabel(envLabel);
  }
  return assertValidLaunchAgentLabel(resolveGatewayLaunchAgentLabel(args?.env?.DAISYCLAW_PROFILE));
}
...
function buildLaunchAgentEnvironmentFile(entries: Array<[string, string]>): string {
  return [
    "# Generated by DaisyClaw. Do not edit while the gateway service is installed.",
...
export function parseLaunchctlListDaisyClawUpdateJobs(
  output: string,
): StaleDaisyClawUpdateLaunchdJob[] {
  const jobs: StaleDaisyClawUpdateLaunchdJob[] = [];
  for (const rawLine of output.split(/\r?\n/)) {
    ...
    const label = normalizeDaisyClawUpdateLaunchdLabel(labelParts.join(" "));
    if (!label) {
      continue;
    }
...
export async function findStaleDaisyClawUpdateLaunchdJobs(
  env: NodeJS.ProcessEnv = process.env,
): Promise<StaleDaisyClawUpdateLaunchdJob[]> {
  ...
  return parseLaunchctlListDaisyClawUpdateJobs(result.stdout).filter(
    (job) => !isCurrentGatewayLaunchdLabel(job.label, env),
  );
}

export async function removeDaisyClawUpdateLaunchdJob(label: string): Promise<boolean> {
  const normalizedLabel = normalizeDaisyClawUpdateLaunchdLabel(label);
...
export async function disableDaisyClawUpdateLaunchdJob(label: string): Promise<boolean> {
  const normalizedLabel = normalizeDaisyClawUpdateLaunchdLabel(label);
  if (process.platform !== "darwin" || !normalizedLabel) {
    return false;
  }
...
export async function disableCurrentDaisyClawUpdateLaunchdJob(
  env: NodeJS.ProcessEnv = process.env,
): Promise<boolean> {
  const label = resolveCurrentDaisyClawUpdateLaunchdJobLabel(env);
  if (!label) {
    return false;
  }
  return await disableDaisyClawUpdateLaunchdJob(label);
}
...
async function resolveLaunchAgentGatewayPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  const fromServiceEnv = parseTcpPort(command?.environment?.DAISYCLAW_GATEWAY_PORT ?? "");
  if (fromServiceEnv !== null) {
    return fromServiceEnv;
  }
  return parseTcpPort(env.DAISYCLAW_GATEWAY_PORT ?? "");
}
...
export function formatLaunchAgentGuiSessionError(params: {
  ...
  return [
    ...
    "Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://docs.daisyclaw.ai/gateway",
  ].join("\n");
}
...
export async function stopLaunchAgent({
  ...
  if (!persistDisable) {
    ...
    // `daisyclaw gateway start` re-enables cleanly without a manual `launchctl enable`.
...
async function writeLaunchAgentPlist({
  ...
  const domain = resolveGuiDomain();
  const label = resolveLaunchAgentLabel({ env });
  for (const legacyLabel of resolveLegacyGatewayLaunchAgentLabels(env.DAISYCLAW_PROFILE)) {
...
async function activateLaunchAgent(params: { env: GatewayServiceEnv; plistPath: string }) {
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    actionHint: "daisyclaw gateway install --force",
  });
}
...
async function ensureLaunchAgentLoadedAfterFailure(params: {
  ...
  try {
    await bootstrapLaunchAgentOrThrow({
      ...
      plistPath: params.plistPath,
      actionHint: "daisyclaw gateway start",
    });
  } catch {
...
export async function restartLaunchAgent({
  ...
  // `daisyclaw gateway restart` is an explicit operator request to bring the
  ...
  if (plistReloadNeeded) {
    ...
    await bootstrapLaunchAgentOrThrow({
      ...
      serviceTarget,
      plistPath,
      actionHint: "daisyclaw gateway restart",
    });
