...
export const DAISYCLAW_WRAPPER_ENV_KEY = "DAISYCLAW_WRAPPER";
...
async function resolveCliEntrypointPathForService(): Promise<string> {
  ...
  if (looksLikeDist) {
    ...
    // since symlinks like node_modules/daisyclaw -> .pnpm/daisyclaw@X.Y.Z/...
...
function appendNodeModulesBinCandidates(
  ...
  // daisyclaw from node_modules/.bin points at the package root sibling.
...
export async function resolveDaisyClawWrapperPath(
  ...
  try {
    ...
    throw new Error(
      `${DAISYCLAW_WRAPPER_ENV_KEY} must point to an executable file: ${resolved}${detail}`,
      { cause: error },
    );
  }
  return resolved;
...
async function resolveCliProgramArguments(params: {
  ...
  const wrapperPath = await resolveDaisyClawWrapperPath(params.wrapperPath);
  if (wrapperPath) {
