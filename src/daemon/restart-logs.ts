...
function resolveGatewayLogPrefix(env: GatewayServiceEnv): string {
  return env.DAISYCLAW_LOG_PREFIX?.trim() || "gateway";
}

function resolveMacLaunchAgentLogPrefix(env: GatewayServiceEnv): string {
  return (
    env.DAISYCLAW_LOG_PREFIX?.trim() || `gateway${resolveGatewayProfileSuffix(env.DAISYCLAW_PROFILE)}`
  );
}
...
export function resolveMacLaunchAgentLogPaths(env: GatewayServiceEnv): GatewayLogPaths {
  ...
  const logDir = path.posix.join(home, "Library", "Logs", "daisyclaw");
...
export function resolveGatewaySupervisorLogPaths(
  ...
  // DaisyClaw state dir so generated service users can create the directory.
...
export function renderCmdRestartLogSetup(env: GatewayServiceEnv): {
  ...
  return {
    ...
    lines: [
      ...
      `>> ${quotedLogPath} 2>&1 echo [%DATE% %TIME%] daisyclaw restart log initialized`,
    ],
  };
}
