...
function resolveStartupEntryPath(env: Record<string, string>, extension = "cmd") {
  const taskName = env.DAISYCLAW_WINDOWS_TASK_NAME ?? "DaisyClaw Gateway";
  return path.join(
    env.APPDATA,
    "Microsoft",
    "Windows",
...
async function writeNodeScript(env: Record<string, string>, port = "18789") {
  ...
  await fs.writeFile(
    ...
    [
      "@echo off",
      `set "DAISYCLAW_SERVICE_KIND=node"`,
      `set "DAISYCLAW_GATEWAY_PORT=${port}"`,
      `"C:\\bin\\daisyclaw.cmd" node run --host 127.0.0.1 --port ${port}`,
      "",
    ].join("\r\n"),
    "utf8",
  );
...
function makeNodeServiceEnv(env: Record<string, string>): Record<string, string> {
  return {
    ...
    DAISYCLAW_WINDOWS_TASK_NAME: "DaisyClaw Node",
  };
}
    ...
    if (command === "powershell" && Array.isArray(args) && args.includes(NODE_PROCESS_QUERY)) {
      return makeSpawnSyncResult({
        stdout: JSON.stringify([
          {
            ProcessId: processId,
            CommandLine: "C:\\bin\\daisyclaw.cmd node run --host 127.0.0.1 --port 18789",
          },
        ]),
      });
    }
...
function expectStartupFallbackSpawn() {
  ...
  expect((options.env as Record<string, string> | undefined)?.DAISYCLAW_GATEWAY_PORT).toBe("18789");
...
function installNodeScheduledTask(env: Record<string, string>, stdout = new PassThrough()) {
  return installScheduledTask({
    ...
    stdout,
    programArguments: ["node", "daisyclaw", "node", "run", "--host", "127.0.0.1", "--port", "18789"],
    environment: {
      DAISYCLAW_SERVICE_KIND: "node",
...
describe("Windows startup fallback", () => {
  it("falls back to a Startup-folder launcher when schtasks create is denied", async () => {
    await withWindowsEnv("daisyclaw-win-startup-", async ({ env }) => {
      addStartupFallbackMissingResponses([
        ...
        if (
          ...
          return {
            ...
            stdout: JSON.stringify([
              {
                ProcessId: 5151,
                CommandLine: "node daisyclaw node run --host 127.0.0.1 --port 18789",
              },
            ]),
            stderr: "",
            status: 0,
  ...
  it("does not report a node task as running from a gateway listener", async () => {
    await withWindowsEnv("daisyclaw-win-startup-", async ({ env }) => {
      env.DAISYCLAW_SERVICE_KIND = "node";
      env.DAISYCLAW_WINDOWS_TASK_NAME = "DaisyClaw Node";
      findVerifiedGatewayListenerPidsOnPortSync.mockReturnValue([4242]);
      schtasksResponses.push(
        { code: 0, stdout: "", stderr: "" },
        { code: 0, stdout: notYetRunTaskQueryOutput(), stderr: "" },
        ...
        if (
          ...
          return {
            ...
            stdout: JSON.stringify([
              {
                ProcessId: 4242,
                CommandLine: "C:\\manual\\daisyclaw.cmd node run --host 127.0.0.1 --port 18789",
              },
              {
                ProcessId: 5151,
                CommandLine: "C:\\bin\\daisyclaw.cmd node run --host 127.0.0.1 --port 18789",
  ...
  it("kills the Startup fallback runtime even when the CLI env omits the gateway port", async () => {
    await withWindowsEnv("daisyclaw-win-startup-", async ({ env }) => {
      ...
      delete envWithoutPort.DAISYCLAW_GATEWAY_PORT;
