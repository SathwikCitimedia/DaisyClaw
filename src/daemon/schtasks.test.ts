...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when DAISYCLAW_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".daisyclaw", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when DAISYCLAW_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", DAISYCLAW_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".daisyclaw-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers DAISYCLAW_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        DAISYCLAW_PROFILE: "rescue",
        DAISYCLAW_STATE_DIR: "C:\\State\\daisyclaw",
      },
      expected: path.join("C:\\State\\daisyclaw", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", DAISYCLAW_PROFILE: "default" },
      expected: path.join("/home/test", ".daisyclaw", "gateway.cmd"),
    },
    {
      name: "uses a custom task script file name inside the state directory",
      env: {
        USERPROFILE: "C:\\Users\\test",
        DAISYCLAW_TASK_SCRIPT_NAME: "gateway-node.cmd",
      },
      expected: path.join("C:\\Users\\test", ".daisyclaw", "gateway-node.cmd"),
    },
  ])("$name", ({ env, expected }) => {
  ...
  it.each([
    ...
    expect(() =>
      resolveTaskScriptPath({
        USERPROFILE: "C:\\Users\\test",
        DAISYCLAW_TASK_SCRIPT_NAME: scriptName,
      }),
    ).toThrow("DAISYCLAW_TASK_SCRIPT_NAME must be a file name only");
  });
});
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem DaisyClaw Gateway",
          "cd /d C:\\Projects\\daisyclaw",
          "set NODE_ENV=production",
          "set DAISYCLAW_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\daisyclaw",
          environment: {
            NODE_ENV: "production",
            DAISYCLAW_PORT: "18789",
          },
          environmentValueSources: {
            NODE_ENV: "inline",
            DAISYCLAW_PORT: "inline",
          },
          sourcePath: resolveTaskScriptPath(env),
        });
      },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\daisyclaw\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\daisyclaw\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\DaisyClaw Share\\node.exe" "\\\\fileserver\\DaisyClaw Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\DaisyClaw Share\\node.exe",
            "\\\\fileserver\\DaisyClaw Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from DAISYCLAW_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ DAISYCLAW_STATE_DIR: path.join(tmpDir, "custom-state") }),
