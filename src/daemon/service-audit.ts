...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds DAISYCLAW_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `daisyclaw gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service DAISYCLAW_GATEWAY_TOKEN does not match gateway.auth.token in daisyclaw.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (isEnvironmentFileOnlySource(command.environmentValueSources?.DAISYCLAW_GATEWAY_TOKEN)) {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.DAISYCLAW_GATEWAY_TOKEN);
}
...
function auditGatewayServiceVersion(command: GatewayServiceCommand, issues: ServiceConfigIssue[]) {
  const serviceVersion = command?.environment?.DAISYCLAW_SERVICE_VERSION?.trim();
  ...
  issues.push({
    ...
    message: `Gateway service was installed by DaisyClaw ${serviceVersion}; current CLI is ${VERSION}.`,
