...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.DAISYCLAW_GATEWAY_PORT).toBe("18789");
    expect(env.DAISYCLAW_GATEWAY_TOKEN).toBeUndefined();
    expect(env.DAISYCLAW_SERVICE_MARKER).toBe("daisyclaw");
    expect(env.DAISYCLAW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.DAISYCLAW_SERVICE_VERSION).toBe("string");
    expect(env.DAISYCLAW_SYSTEMD_UNIT).toBe("daisyclaw-gateway.service");
    expect(env.DAISYCLAW_WINDOWS_TASK_NAME).toBe("DaisyClaw Gateway");
    if (process.platform === "darwin") {
      expect(env.DAISYCLAW_LAUNCHD_LABEL).toBe("ai.daisyclaw.gateway");
    }
  });

  it("sets the DaisyClaw-owned launchd marker for macOS gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/Users/user" },
      port: 18789,
      platform: "darwin",
  ...
  it("passes through DAISYCLAW_WRAPPER for gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        DAISYCLAW_WRAPPER: " /usr/local/bin/daisyclaw-doppler ",
      },
      port: 18789,
    });

    expect(env.DAISYCLAW_WRAPPER).toBe("/usr/local/bin/daisyclaw-doppler");
  });
  ...
  it("uses a durable state temp directory for macOS LaunchAgents", () => {
    ...
    expect(env.TMPDIR).toBe(path.join("/Users/user", ".daisyclaw", "tmp"));
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", DAISYCLAW_PROFILE: "work" },
      port: 18789,
    });
    expect(env.DAISYCLAW_SYSTEMD_UNIT).toBe("daisyclaw-gateway-work.service");
    expect(env.DAISYCLAW_WINDOWS_TASK_NAME).toBe("DaisyClaw Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.DAISYCLAW_LAUNCHD_LABEL).toBe("ai.daisyclaw.work");
    }
  });
  ...
  it("preserves explicit systemd unit overrides", () => {
    const env = buildServiceEnvironment({
      env: {
        ...
        DAISYCLAW_SYSTEMD_UNIT: "daisyclaw-gateway-maintenance",
      },
      port: 18789,
      platform: "linux",
    });

    expect(env.DAISYCLAW_SYSTEMD_UNIT).toBe("daisyclaw-gateway-maintenance.service");
  });
  ...
  it("preserves explicit systemd unit overrides with service suffix", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        DAISYCLAW_SYSTEMD_UNIT: "daisyclaw-gateway-maintenance.service",
      },
      port: 18789,
      platform: "linux",
    });
  ...
  it("sets a profile-specific launchd marker for macOS gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/Users/user", DAISYCLAW_PROFILE: "work" },
      port: 18789,
      platform: "darwin",
    });
  ...
  it("forwards proxy URL env fallback for installed gateway services", () => {
    ...
    expect(env.DAISYCLAW_PROXY_URL).toBe("http://127.0.0.1:3128");
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("sets the DaisyClaw-owned launchd marker for macOS node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/Users/user" },
      platform: "darwin",
    });

    expect(env.DAISYCLAW_LAUNCHD_LABEL).toBe("ai.daisyclaw.node");
  });

  it("passes through DAISYCLAW_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", DAISYCLAW_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.DAISYCLAW_GATEWAY_TOKEN).toBe("node-token");
  });

  it("passes through DAISYCLAW_ALLOW_INSECURE_PRIVATE_WS for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", DAISYCLAW_ALLOW_INSECURE_PRIVATE_WS: " 1 " },
    });
    expect(env.DAISYCLAW_ALLOW_INSECURE_PRIVATE_WS).toBe("1");
  });

  it("omits DAISYCLAW_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
  ...
  it("marks Windows node tasks for hidden launcher startup", () => {
    ...
    expect(env.DAISYCLAW_WINDOWS_TASK_NAME).toBe("DaisyClaw Node");
    expect(env.DAISYCLAW_WINDOWS_TASK_HIDDEN_LAUNCHER).toBe("1");
    expect(env.DAISYCLAW_TASK_SCRIPT_NAME).toBe("node.cmd");
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".daisyclaw"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", DAISYCLAW_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".daisyclaw-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", DAISYCLAW_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".daisyclaw"));
  });

  it("uses DAISYCLAW_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", DAISYCLAW_STATE_DIR: "/var/lib/daisyclaw" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/daisyclaw"));
  });

  it("expands ~ in DAISYCLAW_STATE_DIR", () => {
    const env = { HOME: "/Users/test", DAISYCLAW_STATE_DIR: "~/daisyclaw-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/daisyclaw-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { DAISYCLAW_STATE_DIR: "C:\\State\\daisyclaw" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\daisyclaw");
  });
});
