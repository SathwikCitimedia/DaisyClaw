...
function readServiceProxyEnvironment(
  ...
  // Service env intentionally preserves only the canonical DaisyClaw proxy knob;
  // generic shell proxy vars are audited but not frozen into services.
  const proxyUrl = normalizeOptionalString(env.DAISYCLAW_PROXY_URL);
  return proxyUrl ? { DAISYCLAW_PROXY_URL: proxyUrl } : {};
}
...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.DAISYCLAW_PROFILE;
  const wrapperPath = normalizeOptionalString(env.DAISYCLAW_WRAPPER);
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = resolveGatewaySystemdUnitEnv(env);
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    DAISYCLAW_PROFILE: profile,
    DAISYCLAW_WRAPPER: wrapperPath,
    DAISYCLAW_GATEWAY_PORT: String(port),
    DAISYCLAW_LAUNCHD_LABEL: resolvedLaunchdLabel,
    DAISYCLAW_SYSTEMD_UNIT: systemdUnit,
    DAISYCLAW_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    DAISYCLAW_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    DAISYCLAW_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    DAISYCLAW_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = normalizeOptionalString(env.DAISYCLAW_GATEWAY_TOKEN);
  const allowInsecurePrivateWs = normalizeOptionalString(env.DAISYCLAW_ALLOW_INSECURE_PRIVATE_WS);
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    DAISYCLAW_GATEWAY_TOKEN: gatewayToken,
    DAISYCLAW_ALLOW_INSECURE_PRIVATE_WS: allowInsecurePrivateWs,
    DAISYCLAW_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    DAISYCLAW_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    DAISYCLAW_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    DAISYCLAW_WINDOWS_TASK_HIDDEN_LAUNCHER: "1",
    DAISYCLAW_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    DAISYCLAW_LOG_PREFIX: "node",
    DAISYCLAW_SERVICE_MARKER: NODE_SERVICE_MARKER,
    DAISYCLAW_SERVICE_KIND: NODE_SERVICE_KIND,
    DAISYCLAW_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    DAISYCLAW_STATE_DIR: sharedEnv.stateDir,
    DAISYCLAW_CONFIG_PATH: sharedEnv.configPath,
    ...sharedEnv.proxyEnv,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.DAISYCLAW_STATE_DIR;
