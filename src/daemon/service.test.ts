...
describe("resolveGatewayService", () => {
  ...
  it("guards mutating service adapters when config was written by a newer DaisyClaw", async () => {
    const tempHome = await makeTempWorkspace("daisyclaw-service-future-config-");
    const stateDir = path.join(tempHome, ".daisyclaw");
    const configPath = path.join(stateDir, "daisyclaw.json");
    const envSnapshot = captureEnv(["HOME", "DAISYCLAW_STATE_DIR", "DAISYCLAW_CONFIG_PATH"]);
    try {
...
describe("readGatewayServiceState", () => {
  it("tracks installed, loaded, and running separately", async () => {
    ...
    const state = await readGatewayServiceState(service, {
      env: { DAISYCLAW_GATEWAY_PORT: "1" },
    });

    expect(state.installed).toBe(true);
    expect(state.loaded).toBe(true);
    expect(state.running).toBe(true);
    expect(state.env.DAISYCLAW_GATEWAY_PORT).toBe("18789");
  });
  ...
  it("keeps the caller-selected service identity when merging persisted env", async () => {
    ...
    const service = createService({
      ...
      readCommand: vi.fn(async () => ({
        ...
        environment: {
          ...
          DAISYCLAW_SYSTEMD_UNIT: "daisyclaw-gateway.service",
        },
      })),
      readRuntime,
    });
    ...
    const state = await readGatewayServiceState(service, {
      env: { DAISYCLAW_SYSTEMD_UNIT: "daisyclaw-gateway-maintenance.service" },
    });

    expect(state.env.DAISYCLAW_SYSTEMD_UNIT).toBe("daisyclaw-gateway-maintenance.service");
...
describe("startGatewayService", () => {
  ...
  it("requests repair before start when the loaded service version is stale", async () => {
    const service = createService({
      readCommand: vi.fn(async () => ({
        programArguments: ["daisyclaw", "gateway", "run"],
        environment: { DAISYCLAW_SERVICE_VERSION: "2026.4.24" },
      })),
      isLoaded: vi.fn(async () => true),
    ...
    if (result.outcome === "repair-required") {
      expect(formatGatewayServiceStartRepairIssues(result.issues)).toContain(
        "service was installed by DaisyClaw 2026.4.24",
      );
    }
    expect(service.restart).not.toHaveBeenCalled();
  });
  ...
  it("requests repair before start when the loaded service points at temporary install paths", async () => {
    const service = createService({
      readCommand: vi.fn(async () => ({
        programArguments: [
          "/private/tmp/daisyclaw-ai-install-cli-pr118/tools/node/bin/node",
          "/tmp/daisyclaw-ai-install-cli-pr118/lib/node_modules/daisyclaw/dist/index.js",
          "gateway",
        ],
        environment: {},
      })),
