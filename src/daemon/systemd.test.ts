...
const TEST_MANAGED_HOME = "/tmp/daisyclaw-test-home";
const GATEWAY_SERVICE = "daisyclaw-gateway.service";
const NODE_SERVICE = "daisyclaw-node.service";
...
async function expectExecStartWithoutEnvironment(envFileLine: string) {
  mockReadGatewayServiceFile(["[Service]", "ExecStart=/usr/bin/daisyclaw gateway run", envFileLine]);

  const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
  expect(command?.programArguments).toEqual(["/usr/bin/daisyclaw", "gateway", "run"]);
...
describe("systemd availability", () => {
  ...
  it("does not let stale SUDO_USER override a sudo-u target user scope", async () => {
    ...
    await expect(
      isSystemdUserServiceAvailable({ USER: "daisyclaw", SUDO_USER: "admin" }),
    ).resolves.toBe(true);
    expect(execFileMock).toHaveBeenCalledTimes(1);
  });
});
...
describe("isSystemdServiceEnabled", () => {
  ...
  it("returns false without calling systemctl when the managed unit file is missing", async () => {
    ...
    const result = await isSystemdServiceEnabled({ env: { HOME: "/tmp/daisyclaw-test-home" } });
  ...
  it("throws when systemctl is-enabled fails for non-state errors", async () => {
    ...
    execFileMock
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "is-enabled", "daisyclaw-gateway.service"]);
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        ...
        expect(args.slice(2)).toEqual(["--user", "is-enabled", "daisyclaw-gateway.service"]);
        const err = new Error("permission denied") as Error & { code?: number };
        err.code = 1;
        cb(err, "", "permission denied");
      });
    await expect(
      isSystemdServiceEnabled({ env: { HOME: "/tmp/daisyclaw-test-home" } }),
  ...
  it("returns false when systemctl is-enabled exits with code 4 (not-found)", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, _args, _opts, cb) => {
      ...
      const err = new Error(
        "Command failed: systemctl --user is-enabled daisyclaw-gateway.service",
      ) as Error & { code?: number };
      err.code = 4;
      cb(err, "not-found\n", "");
    });
...
describe("system-scope gateway unit detection (daisyclaw#87577)", () => {
  beforeEach(() => {
    vi.restoreAllMocks();
    execFileMock.mockReset();
  });
  ...
  it("findInstalledSystemdGatewayScope prefers user scope when both exist", async () => {
    mockUnitFileLayout({
      user: true,
      system: "/etc/systemd/system/daisyclaw-gateway.service",
    });
    ...
    expect(result?.unitPath).toContain("/.config/systemd/user/daisyclaw-gateway.service");
  });

  it("findInstalledSystemdGatewayScope detects system-scope unit in /etc/systemd/system", async () => {
    mockUnitFileLayout({ system: "/etc/systemd/system/daisyclaw-gateway.service" });
    const result = await findInstalledSystemdGatewayScope({ HOME: TEST_MANAGED_HOME });
    expect(result).toEqual({
      scope: "system",
      unitName: GATEWAY_SERVICE,
      unitPath: "/etc/systemd/system/daisyclaw-gateway.service",
    });
  });

  it("findInstalledSystemdGatewayScope falls back to /usr/lib/systemd/system", async () => {
    mockUnitFileLayout({ system: "/usr/lib/systemd/system/daisyclaw-gateway.service" });
    const result = await findInstalledSystemdGatewayScope({ HOME: TEST_MANAGED_HOME });
    expect(result?.scope).toBe("system");
    expect(result?.unitPath).toBe("/usr/lib/systemd/system/daisyclaw-gateway.service");
  });
  ...
  it("findInstalledSystemdGatewayScope falls back to marker-owned system unit with custom name", async () => {
    ...
    findSystemGatewayServicesMock.mockResolvedValueOnce([
      {
        platform: "linux",
        label: "daisyclaw.service",
        detail: "unit: /etc/systemd/system/daisyclaw.service",
        scope: "system",
        marker: "daisyclaw",
      },
    ]);
    ...
    expect(result).toEqual({
      scope: "system",
      unitName: "daisyclaw.service",
      unitPath: "/etc/systemd/system/daisyclaw.service",
    });
  });
  ...
  it("isSystemdServiceEnabled queries the marker-owned custom system unit name", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, args, _opts, cb) => {
      expect(args).toEqual(["is-enabled", "daisyclaw.service"]);
      cb(null, "enabled\n", "");
    });
  ...
  it("restartSystemdService surfaces sudo guidance using the marker-owned custom unit name", async () => {
    ...
    await expect(
      ...
      /daisyclaw\.service is a system-scope unit \(\/etc\/systemd\/system\/daisyclaw\.service\); run `sudo systemctl restart daisyclaw\.service`/,
    );
    expect(execFileMock).not.toHaveBeenCalled();
  ...
  it("readSystemdServiceRuntime queries the system manager for system-scope units", async () => {
    ...
    expect(runtime.systemd?.unit).toBe("daisyclaw-gateway.service");
  });
  ...
  it("restartSystemdService refuses to use the user manager when the unit is system-scope and the caller is not root", async () => {
    ...
    await expect(
      ...
      /system-scope unit .* run `sudo systemctl restart daisyclaw-gateway\.service`/,
    );
    expect(execFileMock).not.toHaveBeenCalled();
    expect(write).not.toHaveBeenCalled();
  });
  ...
  it("stopSystemdService surfaces sudo guidance for system-scope units without root", async () => {
    ...
    await expect(stopSystemdService({ stdout, env: { HOME: TEST_MANAGED_HOME } })).rejects.toThrow(
      /sudo systemctl stop daisyclaw-gateway\.service/,
    );
    expect(execFileMock).not.toHaveBeenCalled();
  });
});
...
describe("isNonFatalSystemdInstallProbeError", () => {
  it("matches wrapper-only WSL install probe failures", () => {
    expect(
      isNonFatalSystemdInstallProbeError(
        new Error("Command failed: systemctl --user is-enabled daisyclaw-gateway.service"),
      ),
    ).toBe(true);
  });
...
describe("readSystemdServiceRuntime", () => {
  it("surfaces systemd cgroup metrics and KillMode", async () => {
    ...
    expect(runtime).toEqual({
      ...
      systemd: {
        unit: "daisyclaw-gateway.service",
        killMode: "process",
        tasksCurrent: 807,
...
describe("resolveSystemdUserUnitPath", () => {
  it.each([
    {
      name: "uses default service name when DAISYCLAW_PROFILE is unset",
      env: { HOME: "/home/test" },
      expected: "/home/test/.config/systemd/user/daisyclaw-gateway.service",
    },
    {
      name: "uses profile-specific service name when DAISYCLAW_PROFILE is set to a custom value",
      env: { HOME: "/home/test", DAISYCLAW_PROFILE: "jbphoenix" },
      expected: "/home/test/.config/systemd/user/daisyclaw-gateway-jbphoenix.service",
    },
    {
      name: "prefers DAISYCLAW_SYSTEMD_UNIT over DAISYCLAW_PROFILE",
      env: {
        HOME: "/home/test",
        DAISYCLAW_PROFILE: "jbphoenix",
        DAISYCLAW_SYSTEMD_UNIT: "custom-unit",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "handles DAISYCLAW_SYSTEMD_UNIT with .service suffix",
      env: {
        HOME: "/home/test",
        DAISYCLAW_SYSTEMD_UNIT: "custom-unit.service",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "trims whitespace from DAISYCLAW_SYSTEMD_UNIT",
      env: {
        HOME: "/home/test",
        DAISYCLAW_SYSTEMD_UNIT: "  custom-unit  ",
      },
...
describe("splitArgsPreservingQuotes", () => {
  it("splits on whitespace outside quotes", () => {
    expect(splitArgsPreservingQuotes('/usr/bin/daisyclaw gateway start --name "My Bot"')).toEqual([
      "/usr/bin/daisyclaw",
      "gateway",
      "start",
      "--name",
  ...
  it("supports systemd-style backslash escaping", () => {
    expect(
      splitArgsPreservingQuotes('daisyclaw --name "My \\"Bot\\"" --foo bar', {
        escapeMode: "backslash",
      }),
    ).toEqual(["daisyclaw", "--name", 'My "Bot"', "--foo", "bar"]);
  });

  it("supports schtasks-style escaped quotes while preserving other backslashes", () => {
    expect(
      splitArgsPreservingQuotes('daisyclaw --path "C:\\\\Program Files\\\\DaisyClaw"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["daisyclaw", "--path", "C:\\\\Program Files\\\\DaisyClaw"]);

    expect(
      splitArgsPreservingQuotes('daisyclaw --label "My \\"Quoted\\" Name"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["daisyclaw", "--label", 'My "Quoted" Name']);
  });
});
...
describe("parseSystemdEnvAssignments", () => {
  it("parses single-quoted whole assignments", () => {
    expect(
      parseSystemdEnvAssignments("'DAISYCLAW_GATEWAY_TOKEN=single quoted token' FOO=bar"),
    ).toEqual([
      { key: "DAISYCLAW_GATEWAY_TOKEN", value: "single quoted token" },
      { key: "FOO", value: "bar" },
    ]);
  });

  it("keeps apostrophes inside unquoted assignment values literal", () => {
    expect(parseSystemdEnvAssignments("FOO=can't DAISYCLAW_GATEWAY_TOKEN=token")).toEqual([
      { key: "FOO", value: "can't" },
      { key: "DAISYCLAW_GATEWAY_TOKEN", value: "token" },
    ]);
  });
});
...
describe("parseSystemdExecStart", () => {
  it("preserves quoted arguments", () => {
    const execStart = '/usr/bin/daisyclaw gateway start --name "My Bot"';
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads DAISYCLAW_GATEWAY_TOKEN from EnvironmentFile", async () => {
    const readFileSpy = mockReadGatewayServiceFile(
      ["[Service]", "ExecStart=/usr/bin/daisyclaw gateway run", "EnvironmentFile=%h/.daisyclaw/.env"],
      { [`${TEST_SERVICE_HOME}/.daisyclaw/.env`]: "DAISYCLAW_GATEWAY_TOKEN=env-file-token\n" },
    );

    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.DAISYCLAW_GATEWAY_TOKEN).toBe("env-file-token");
  ...
  it("lets EnvironmentFile override inline Environment values", async () => {
    mockReadGatewayServiceFile(
      [
        "[Service]",
        "ExecStart=/usr/bin/daisyclaw gateway run",
        "EnvironmentFile=%h/.daisyclaw/.env",
        'Environment="DAISYCLAW_GATEWAY_TOKEN=inline-token"',
      ],
      { [`${TEST_SERVICE_HOME}/.daisyclaw/.env`]: "DAISYCLAW_GATEWAY_TOKEN=env-file-token\n" },
    );
    ...
    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.DAISYCLAW_GATEWAY_TOKEN).toBe("env-file-token");
    expect(command?.environmentValueSources?.DAISYCLAW_GATEWAY_TOKEN).toBe("inline-and-file");
  });

  it("ignores missing optional EnvironmentFile entries", async () => {
    await expectExecStartWithoutEnvironment("EnvironmentFile=-%h/.daisyclaw/missing.env");
  });
  ...
  it("supports multiple EnvironmentFile entries and quoted paths", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      const pathValue = pathLikeToString(pathname);
      if (pathValue.endsWith("/daisyclaw-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/daisyclaw gateway run",
          'EnvironmentFile=%h/.daisyclaw/first.env "%h/.daisyclaw/second env.env"',
        ].join("\n");
      }
      if (pathValue === "/home/test/.daisyclaw/first.env") {
        return "DAISYCLAW_GATEWAY_TOKEN=first-token\n"; // pragma: allowlist secret
      }
      if (pathValue === "/home/test/.daisyclaw/second env.env") {
        return 'DAISYCLAW_GATEWAY_PASSWORD="second password"\n'; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    expect(command?.environment).toEqual({
      DAISYCLAW_GATEWAY_TOKEN: "first-token",
      DAISYCLAW_GATEWAY_PASSWORD: "second password", // pragma: allowlist secret
    });
  });
  ...
  it("resolves relative EnvironmentFile paths from the unit directory", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/.config/systemd/user/gateway.env")) {
        return [
          "DAISYCLAW_GATEWAY_TOKEN=relative-token", // pragma: allowlist secret
          "DAISYCLAW_GATEWAY_PASSWORD=relative-password", // pragma: allowlist secret
        ].join("\n");
      }
      if (pathValue.endsWith("/.config/systemd/user/override.env")) {
        return "DAISYCLAW_GATEWAY_TOKEN=override-token\n"; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      DAISYCLAW_GATEWAY_TOKEN: "override-token",
      DAISYCLAW_GATEWAY_PASSWORD: "relative-password", // pragma: allowlist secret
    });
  });
  ...
  it("parses EnvironmentFile content with comments and quoted values", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/daisyclaw-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/daisyclaw gateway run",
          "EnvironmentFile=%h/.daisyclaw/gateway.env",
        ].join("\n");
      }
      if (pathValue === "/home/test/.daisyclaw/gateway.env") {
        return [
          "# comment",
          "; another comment",
          'DAISYCLAW_GATEWAY_TOKEN="quoted token"', // pragma: allowlist secret
          "DAISYCLAW_GATEWAY_PASSWORD=quoted-password", // pragma: allowlist secret
        ].join("\n");
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      DAISYCLAW_GATEWAY_TOKEN: "quoted token",
      DAISYCLAW_GATEWAY_PASSWORD: "quoted-password", // pragma: allowlist secret
    });
    expect(command?.environmentValueSources).toEqual({
      DAISYCLAW_GATEWAY_TOKEN: "file",
      DAISYCLAW_GATEWAY_PASSWORD: "file", // pragma: allowlist secret
    });
  });
});
...
describe("stageSystemdService", () => {
  async function withStageFixture(
    ...
    const tempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-systemd-stage-"));
    const home = path.join(tempHomeRoot, "home");
    const stateDir = path.join(home, ".daisyclaw");
    const env = {
      HOME: home,
      DAISYCLAW_STATE_DIR: stateDir,
      DAISYCLAW_SYSTEMD_UNIT: "daisyclaw-gateway-stage-test",
    };
  ...
  it("writes dotenv-backed values to a separate env file and keeps inline env minimal", async () => {
    await withStageFixture(async ({ env, stateDir, unitPath, envFilePath }) => {
      await fs.writeFile(
        ...
        ["DAISYCLAW_GATEWAY_TOKEN=dotenv-token", "LLM_API_KEY=dotenv-key"].join("\n"),
        "utf8",
      );
      ...
      await stageSystemdService({
        ...
        programArguments: ["/usr/bin/daisyclaw", "gateway", "run"],
        workingDirectory: "/tmp",
        environment: {
          DAISYCLAW_GATEWAY_TOKEN: "dotenv-token",
      ...
      expect(unit).toContain("Environment=DAISYCLAW_GATEWAY_PORT=18789");
      expect(unit).not.toContain("Environment=DAISYCLAW_GATEWAY_TOKEN=dotenv-token");
      expect(unit).not.toContain("Environment=LLM_API_KEY=dotenv-key");
      expect(envFile).toBe("DAISYCLAW_GATEWAY_TOKEN=dotenv-token\nLLM_API_KEY=dotenv-key\n");
  ...
  it("writes node file-backed managed values to the node env file instead of the unit", async () => {
    await withStageFixture(async ({ env, stateDir, unitPath, envFilePath, nodeEnvFilePath }) => {
      ...
      await stageSystemdService({
        env,
        stdout: { write: vi.fn() } as unknown as NodeJS.WritableStream,
        programArguments: ["/usr/bin/daisyclaw", "node", "run"],
        workingDirectory: "/tmp",
        environment: {
          DAISYCLAW_GATEWAY_TOKEN: "file-backed-token",
          DAISYCLAW_GATEWAY_PORT: "18789",
          DAISYCLAW_SERVICE_KIND: "node",
        },
      ...
      expect(unit).toContain("Environment=DAISYCLAW_GATEWAY_PORT=18789");
      expect(unit).not.toContain("Environment=DAISYCLAW_GATEWAY_TOKEN=file-backed-token");
      expect(envFile).toBe("DAISYCLAW_GATEWAY_TOKEN=file-backed-token\n");
      expect(envFileStat.mode & 0o777).toBe(0o600);
  ...
  it("migrates operator entries from the legacy gateway env file when writing node env files", async () => {
    await withStageFixture(async ({ env, unitPath, envFilePath, nodeEnvFilePath }) => {
      const legacyGatewayEnvFile =
        ["DAISYCLAW_GATEWAY_TOKEN=legacy-node-token", "OPENROUTER_API_KEY=operator-key"].join("\n") +
        "\n";
      ...
      await stageSystemdService({
        ...
        workingDirectory: "/tmp",
        environment: {
          DAISYCLAW_GATEWAY_TOKEN: "fresh-file-token",
          DAISYCLAW_GATEWAY_PORT: "18789",
          DAISYCLAW_SERVICE_KIND: "node",
        },
        environmentValueSources: {
      ...
      expect(unit).toContain(`EnvironmentFile=-${nodeEnvFilePath}`);
      expect(unit).not.toContain("DAISYCLAW_GATEWAY_TOKEN=fresh-file-token");
  ...
  it("clears stale node file-backed managed keys without touching the gateway env file", async () => {
    await withStageFixture(async ({ env, unitPath, envFilePath, nodeEnvFilePath }) => {
      await fs.writeFile(envFilePath, "DAISYCLAW_GATEWAY_TOKEN=stale-token\n", {
        encoding: "utf8",
        mode: 0o600,
      });
      await fs.writeFile(nodeEnvFilePath, "DAISYCLAW_GATEWAY_TOKEN=stale-node-token\n", {
        encoding: "utf8",
        mode: 0o600,
      });
      ...
      await expect(fs.readFile(envFilePath, "utf8")).resolves.toBe(
        "DAISYCLAW_GATEWAY_TOKEN=stale-token\n",
      );
    });
  });
  ...
  it("sanitizes file-backed managed values out of the backup unit on re-stage", async () => {
    await withStageFixture(async ({ env, unitPath }) => {
      ...
      await fs.writeFile(
        ...
        [
          "[Service]",
          "ExecStart=/usr/bin/daisyclaw node run",
          "Environment=FOO=bar DAISYCLAW_GATEWAY_TOKEN=inline-token BAZ=qux",
          "Environment=DAISYCLAW_GATEWAY_TOKEN=token-only-line",
          "Environment='DAISYCLAW_GATEWAY_TOKEN=single-quoted-token' FROM_SINGLE=kept",
          "Environment=DAISYCLAW_GATEWAY_PORT=18789",
        ].join("\n"),
      ...
      await stageSystemdService({
        ...
        workingDirectory: "/tmp",
        environment: {
          DAISYCLAW_GATEWAY_TOKEN: "fresh-token",
          DAISYCLAW_GATEWAY_PORT: "18789",
          DAISYCLAW_SERVICE_KIND: "node",
        },
        environmentValueSources: {
      ...
      expect(unit).not.toContain("Environment=DAISYCLAW_GATEWAY_TOKEN=fresh-token");
      expect(backupUnit).not.toContain("Environment=DAISYCLAW_GATEWAY_TOKEN=inline-token");
      expect(backupUnit).not.toContain("Environment=DAISYCLAW_GATEWAY_TOKEN=token-only-line");
      ...
      expect(backupUnit).toContain("Environment=DAISYCLAW_GATEWAY_PORT=18789");
  ...
  it("keeps inline overrides out of the generated env file", async () => {
    await withStageFixture(async ({ env, stateDir, unitPath, envFilePath }) => {
      await fs.writeFile(
        path.join(stateDir, ".env"),
        ["DAISYCLAW_GATEWAY_TOKEN=stale-token", "LLM_API_KEY=dotenv-key"].join("\n"),
        "utf8",
      );
      ...
      expect(unit).toContain(`EnvironmentFile=-${envFilePath}`);
      expect(unit).toContain("Environment=DAISYCLAW_GATEWAY_TOKEN=fresh-token");
  ...
  it("clears stale inline-managed keys from env file on re-stage (#76860)", async () => {
    await withStageFixture(async ({ env, stateDir, unitPath, envFilePath }) => {
      // Existing env file carries a stale DAISYCLAW_GATEWAY_TOKEN that the
      // operator previously wrote there but staging now supplies inline.
      await fs.writeFile(
        envFilePath,
        ["DAISYCLAW_GATEWAY_TOKEN=stale-gateway-token", "OPENROUTER_API_KEY=or-operator-key"].join(
          "\n",
        ) + "\n",
        { encoding: "utf8", mode: 0o600 },
      );
      ...
      await stageSystemdService({
        ...
        programArguments: ["/usr/bin/daisyclaw", "gateway", "run"],
        workingDirectory: "/tmp",
        // Staging manages DAISYCLAW_GATEWAY_TOKEN inline; DAISYCLAW_SERVICE_MANAGED_ENV_KEYS
        // marks it as an DaisyClaw-managed key so the stale env-file copy is cleared.
        environment: {
          DAISYCLAW_GATEWAY_TOKEN: "fresh-gateway-token",
          LLM_API_KEY: "dotenv-key",
          OPENROUTER_API_KEY: "or-operator-key",
          DAISYCLAW_SERVICE_MANAGED_ENV_KEYS: "DAISYCLAW_GATEWAY_TOKEN",
        },
        environmentValueSources: {
          DAISYCLAW_GATEWAY_TOKEN: "inline-and-file",
          LLM_API_KEY: "inline",
          OPENROUTER_API_KEY: "file",
          DAISYCLAW_SERVICE_MANAGED_ENV_KEYS: "inline",
        },
      });
      ...
      expect(envFile).not.toContain("DAISYCLAW_GATEWAY_TOKEN");
      ...
      expect(unit).toContain("Environment=DAISYCLAW_GATEWAY_TOKEN=fresh-gateway-token");
...
describe("systemd service install and uninstall", () => {
  async function withNodeSystemdFixture(
    ...
    const tempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-node-systemd-"));
    const home = path.join(tempHomeRoot, "home");
    const stateDir = path.join(home, ".daisyclaw");
    const env = {
      HOME: home,
      DAISYCLAW_STATE_DIR: stateDir,
      DAISYCLAW_SYSTEMD_UNIT: "daisyclaw-node",
      DAISYCLAW_SERVICE_KIND: "node",
    };
    const unitPath = resolveSystemdUserUnitPath(env);
    const nodeEnvFilePath = path.join(stateDir, "node.systemd.env");
  ...
  it("activates the DAISYCLAW_SYSTEMD_UNIT override during install", async () => {
    await withNodeSystemdFixture(async ({ env, unitPath }) => {
      ...
      const unit = await fs.readFile(unitPath, "utf8");
      expect(unitPath).toMatch(/daisyclaw-node\.service$/);
      expect(unit).toContain("daisyclaw node run");
      ...
      execFileMock
        ...
        .mockImplementationOnce((_cmd, args, _opts, cb) => {
          ...
          cb(
            ...
            "",
            "Unit file daisyclaw-node.service does not exist.",
          );
        })
        .mockImplementationOnce((_cmd, args, _opts, cb) => {
          assertUserSystemctlArgs(args, "daemon-reload");
  ...
  it("uses the sudo-u target user for install activation machine-scope retry", async () => {
    await withNodeSystemdFixture(async ({ env }) => {
      mockEffectiveUid(1000);
      const installEnv = { ...env, USER: "daisyclaw", SUDO_USER: "admin" };
      execFileMock
        .mockImplementationOnce((_cmd, args, _opts, cb) => {
          assertUserSystemctlArgs(args, "status");
          cb(null, "", "");
        ...
        .mockImplementationOnce((_cmd, args, _opts, cb) => {
          assertMachineUserSystemctlArgs(args, "daisyclaw", "enable", NODE_SERVICE);
          cb(null, "", "");
        })
        .mockImplementationOnce((_cmd, args, _opts, cb) => {
          assertUserSystemctlArgs(args, "restart", NODE_SERVICE);
  ...
  it("disables the DAISYCLAW_SYSTEMD_UNIT override during uninstall", async () => {
    await withNodeSystemdFixture(async ({ env, unitPath, nodeEnvFilePath }) => {
      await fs.mkdir(path.dirname(unitPath), { recursive: true });
      await fs.writeFile(unitPath, "[Unit]\nDescription=DaisyClaw Node\n", "utf8");
      await fs.writeFile(
        nodeEnvFilePath,
        "DAISYCLAW_GATEWAY_TOKEN=stale-node-token\nOPENROUTER_API_KEY=operator-key\n",
        { encoding: "utf8", mode: 0o600 },
      );
  ...
  it("preserves node env file values when unit removal fails during uninstall", async () => {
    await withNodeSystemdFixture(async ({ env, unitPath, nodeEnvFilePath }) => {
      ...
      await expect(fs.readFile(unitPath, "utf8")).resolves.toContain("DaisyClaw Node");
      await expect(fs.readFile(nodeEnvFilePath, "utf8")).resolves.toBe(
        "DAISYCLAW_GATEWAY_TOKEN=stale-node-token\nOPENROUTER_API_KEY=operator-key\n",
      );
      expect(execFileMock).toHaveBeenCalledTimes(2);
...
describe("systemd service control", () => {
  ...
  it("restarts a profile-specific user unit", async () => {
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        assertUserSystemctlArgs(args, "restart", "daisyclaw-gateway-work.service");
        cb(null, "", "");
      });
    await assertRestartSuccess({ DAISYCLAW_PROFILE: "work" });
  });
