...
function resolveSystemdServiceName(env: GatewayServiceEnv): string {
  const override = env.DAISYCLAW_SYSTEMD_UNIT?.trim();
  if (override) {
    return override.endsWith(".service") ? override.slice(0, -".service".length) : override;
  }
  return resolveGatewaySystemdServiceName(env.DAISYCLAW_PROFILE);
}
...
function sanitizeSystemdUnitBackupContent(params: {
  ...
  // Backups should not retain file-managed secrets that DaisyClaw moved into the
...
function resolveSystemdEnvironmentFilePath(params: {
  ...
  const serviceKind = params.environment?.DAISYCLAW_SERVICE_KIND?.trim();
  const filename =
...
function resolveLegacyNodeSystemdEnvironmentFilePath(params: {
  ...
  if (params.environment?.DAISYCLAW_SERVICE_KIND?.trim() !== "node") {
    return null;
  }
...
function isNodeSystemdEnvironment(env: GatewayServiceEnv): boolean {
  return env.DAISYCLAW_SERVICE_KIND?.trim() === "node";
}
...
async function writeSystemdGatewayEnvironmentFile(params: {
  ...
  /** DaisyClaw-managed keys that must not be preserved from an old env file; stale file values
  ...
  fileManagedKeys?: ReadonlySet<string>;
  /** State-dir .env keys DaisyClaw previously managed but is now skipping (unresolved shell
  ...
  // DaisyClaw-managed keys (identified by inlineManagedKeys) are excluded: a stale
...
async function removeNodeSystemdManagedEnvironmentKeys(env: GatewayServiceEnv): Promise<void> {
  ...
  const managedKeys = new Set([normalizeSystemdEnvironmentKey("DAISYCLAW_GATEWAY_TOKEN")]);
