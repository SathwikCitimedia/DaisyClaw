...
describe("docker build cache layout", () => {
  ...
  it("uses pnpm cache mounts in Dockerfiles that install repo dependencies", async () => {
    for (const path of [
      ...
      expect(
        ...
        /--mount=type=cache,id=daisyclaw-pnpm-store,target=\/(?:root|home\/appuser)\/\.local\/share\/pnpm\/store,sharing=locked/,
      );
    }
  });
  ...
  it("keeps the shared e2e image on the packaged tarball install path", async () => {
    ...
    expect(dockerfile).toMatch(
      /^COPY --from=daisyclaw_package --chown=appuser:appuser daisyclaw-current\.tgz \/tmp\/daisyclaw-current\.tgz$/m,
    );
    expect(dockerfile).toContain(
      "npm install -g --prefix /tmp/daisyclaw-prefix /tmp/daisyclaw-current.tgz --no-fund --no-audit",
    );
    expect(dockerfile).not.toContain(
      "cp -a /tmp/daisyclaw-prefix/lib/node_modules/. /app/node_modules/",
    );
    expect(dockerfile).toContain("cp -a /tmp/daisyclaw-prefix/lib/node_modules/daisyclaw/. /app/");
    expect(dockerfile).toContain("rm -rf /app/node_modules/daisyclaw");
    expect(dockerfile).toContain("ln -sf /app /app/node_modules/daisyclaw");
  });
