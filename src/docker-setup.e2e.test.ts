...
const sandboxRootTracker = createSuiteTempRootTracker({ prefix: "daisyclaw-docker-setup-" });

const prestartContainerEnvFlags = [
  "-e HOME=/home/node",
  "-e DAISYCLAW_HOME=/home/node",
  "-e DAISYCLAW_STATE_DIR=/home/node/.daisyclaw",
  "-e DAISYCLAW_CONFIG_PATH=/home/node/.daisyclaw/daisyclaw.json",
  "-e DAISYCLAW_CONFIG_DIR=/home/node/.daisyclaw",
  "-e DAISYCLAW_WORKSPACE_DIR=/home/node/.daisyclaw/workspace",
].join(" ");
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    DAISYCLAW_GATEWAY_TOKEN: "test-token",
    DAISYCLAW_CONFIG_DIR: join(sandbox.rootDir, "config"),
    DAISYCLAW_WORKSPACE_DIR: join(sandbox.rootDir, "daisyclaw"),
    DAISYCLAW_AUTH_PROFILE_SECRET_DIR: join(sandbox.rootDir, "auth-profile-secrets"),
  };
...
function isGatewayStartLine(line: string) {
  return line.includes("compose") && line.includes(" up -d") && line.includes("daisyclaw-gateway");
}
...
async function runDockerSetupWithUnsetGatewayToken(
  ...
  const result = runDockerSetup(sandbox, {
    DAISYCLAW_GATEWAY_TOKEN: undefined,
    DAISYCLAW_CONFIG_DIR: configDir,
    DAISYCLAW_WORKSPACE_DIR: workspaceDir,
  });
...
describe("scripts/docker/setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and Docker build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      DAISYCLAW_DOCKER_APT_PACKAGES: "curl wget",
      DAISYCLAW_EXTRA_MOUNTS: undefined,
      DAISYCLAW_HOME_VOLUME: "daisyclaw-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("DAISYCLAW_IMAGE_APT_PACKAGES=curl wget");
    expect(envFile).toContain("DAISYCLAW_EXTRA_MOUNTS=");
    expect(envFile).toContain("DAISYCLAW_HOME_VOLUME=daisyclaw-home"); // pragma: allowlist secret
    expect(envFile).toContain("DAISYCLAW_DISABLE_BONJOUR=");
    expect(envFile).toContain(
      `DAISYCLAW_AUTH_PROFILE_SECRET_DIR=${join(activeSandbox.rootDir, "auth-profile-secrets")}`,
    );
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("daisyclaw-home:/home/node");
    expect(extraCompose).toContain(
      `${join(activeSandbox.rootDir, "auth-profile-secrets")}:/home/node/.config/daisyclaw`,
    );
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("daisyclaw-home:");
    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg DAISYCLAW_IMAGE_APT_PACKAGES=curl wget");
    expect(log).toContain(
      `run --rm --no-deps ${prestartContainerEnvFlags} --entrypoint node daisyclaw-gateway dist/index.js onboard --mode local --no-install-daemon --gateway-auth token --gateway-token-ref-env DAISYCLAW_GATEWAY_TOKEN --skip-ui --suppress-gateway-token-output`,
    );
    ...
    expect(log).toContain(
      `run --rm --no-deps ${prestartContainerEnvFlags} --entrypoint node daisyclaw-gateway dist/index.js config set --batch-json [{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:18789","http://127.0.0.1:18789"]}]`,
    );
    expect(log).not.toContain("run --rm daisyclaw-cli onboard --mode local --no-install-daemon");
  });
  ...
  it("allows ordinary spaces in host persistence paths and quotes generated mounts", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      DAISYCLAW_CONFIG_DIR: configDir,
      DAISYCLAW_WORKSPACE_DIR: workspaceDir,
      DAISYCLAW_AUTH_PROFILE_SECRET_DIR: authProfileSecretDir,
      DAISYCLAW_HOME_VOLUME: homeVolumeDir,
      DAISYCLAW_EXTRA_MOUNTS: `${extraMountSource}:/mnt/extra data:ro`,
    });

    expect(result.status).toBe(0);
    expect(result.stderr).not.toContain("cannot contain whitespace");
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain(`DAISYCLAW_CONFIG_DIR=${configDir}`);
    expect(envFile).toContain(`DAISYCLAW_WORKSPACE_DIR=${workspaceDir}`);
    expect(envFile).toContain(`DAISYCLAW_AUTH_PROFILE_SECRET_DIR=${authProfileSecretDir}`);
    ...
    expect(extraCompose).toContain(`"${configDir}:/home/node/.daisyclaw"`);
    expect(extraCompose).toContain(`"${workspaceDir}:/home/node/.daisyclaw/workspace"`);
    expect(extraCompose).toContain(`"${authProfileSecretDir}:/home/node/.config/daisyclaw"`);
  ...
  it("persists explicit Docker Bonjour opt-in overrides", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      DAISYCLAW_DISABLE_BONJOUR: "0",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("DAISYCLAW_DISABLE_BONJOUR=0");
  });

  it("normalizes legacy DAISYCLAW_DOCKER_APT_PACKAGES into DAISYCLAW_IMAGE_APT_PACKAGES", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);
    ...
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("DAISYCLAW_IMAGE_APT_PACKAGES=curl wget");
    expect(envFile).not.toContain("DAISYCLAW_DOCKER_APT_PACKAGES");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg DAISYCLAW_IMAGE_APT_PACKAGES=curl wget");
    expect(log).not.toContain("--build-arg DAISYCLAW_DOCKER_APT_PACKAGES");
  });

  it("prefers DAISYCLAW_IMAGE_APT_PACKAGES over legacy DAISYCLAW_DOCKER_APT_PACKAGES", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);

    const result = runDockerSetup(activeSandbox, {
      DAISYCLAW_IMAGE_APT_PACKAGES: "curl wget httpie",
      DAISYCLAW_DOCKER_APT_PACKAGES: "curl wget",
    });
    expect(result.status).toBe(0);

    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("DAISYCLAW_IMAGE_APT_PACKAGES=curl wget httpie");
    expect(envFile).not.toContain("DAISYCLAW_DOCKER_APT_PACKAGES");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg DAISYCLAW_IMAGE_APT_PACKAGES=curl wget httpie");
    expect(log).not.toMatch(/--build-arg DAISYCLAW_IMAGE_APT_PACKAGES=curl wget(?! httpie)/);
  });

  it("explicitly empty DAISYCLAW_IMAGE_APT_PACKAGES suppresses legacy fallback", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);

    const result = runDockerSetup(activeSandbox, {
      DAISYCLAW_IMAGE_APT_PACKAGES: "",
      DAISYCLAW_DOCKER_APT_PACKAGES: "curl wget",
    });
    expect(result.status).toBe(0);

    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("DAISYCLAW_IMAGE_APT_PACKAGES=");
    expect(envFile).not.toContain("curl wget");

    const log = await readDockerLog(activeSandbox);
    expect(log).not.toContain("--build-arg DAISYCLAW_IMAGE_APT_PACKAGES=curl wget");
  });

  it("avoids shared-network daisyclaw-cli before the gateway is started", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    const prestartCliRunLines = collectMatchingLines(prestartLines, (line) =>
      /\bcompose\b.*\brun\b.*\bdaisyclaw-cli\b/.test(line),
    );
  ...
  it("pins setup-time CLI state paths inside the container", async () => {
    ...
    await resetDockerLog(activeSandbox);
    const result = runDockerSetup(activeSandbox, {
      DAISYCLAW_HOME: "/mnt/c/Users/Trevor",
      DAISYCLAW_STATE_DIR: "/mnt/c/Users/Trevor/.daisyclaw",
      DAISYCLAW_CONFIG_PATH: "/mnt/c/Users/Trevor/.daisyclaw/daisyclaw.json",
      DAISYCLAW_SKIP_ONBOARDING: "1",
    });
    expect(result.status).toBe(0);
  ...
  it("writes DAISYCLAW_TZ into .env when given a real IANA timezone", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      DAISYCLAW_TZ: "Asia/Shanghai",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("DAISYCLAW_TZ=Asia/Shanghai");
  });
  ...
  it("precreates agent data dirs to avoid EACCES in container", async () => {
    ...
    expect(log).toContain("run --rm --no-deps --user root --entrypoint sh daisyclaw-gateway -c");
  ...
  it("precreates auth profile secret key dir outside the mounted state dir", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      DAISYCLAW_CONFIG_DIR: configDir,
      DAISYCLAW_WORKSPACE_DIR: workspaceDir,
      DAISYCLAW_AUTH_PROFILE_SECRET_DIR: secretDir,
    });
    ...
    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("find /home/node/.config/daisyclaw -xdev");
  });

  it("reuses existing config token when DAISYCLAW_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("DAISYCLAW_GATEWAY_TOKEN=config-token-123"); // pragma: allowlist secret
  });

  it("reuses existing .env token when DAISYCLAW_GATEWAY_TOKEN and config token are unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await writeFile(
      join(activeSandbox.rootDir, ".env"),
      "DAISYCLAW_GATEWAY_TOKEN=dotenv-token-123\nDAISYCLAW_GATEWAY_PORT=18789\n", // pragma: allowlist secret
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
      "dotenv-token-reuse",
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("DAISYCLAW_GATEWAY_TOKEN=dotenv-token-123"); // pragma: allowlist secret
  ...
  it("reuses the last non-empty .env token and strips CRLF without truncating '='", async () => {
    ...
    await writeFile(
      join(activeSandbox.rootDir, ".env"),
      [
        "DAISYCLAW_GATEWAY_TOKEN=",
        "DAISYCLAW_GATEWAY_TOKEN=first-token",
        "DAISYCLAW_GATEWAY_TOKEN=last=token=value\r", // pragma: allowlist secret
      ].join("\n"),
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("DAISYCLAW_GATEWAY_TOKEN=last=token=value"); // pragma: allowlist secret
    expect(envFile).not.toContain("DAISYCLAW_GATEWAY_TOKEN=first-token");
    expect(envFile).not.toContain("\r");
  });

  it("treats DAISYCLAW_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("DAISYCLAW_SANDBOX=");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg DAISYCLAW_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg DAISYCLAW_INSTALL_DOCKER_CLI=1");
  ...
  it("resets stale sandbox mode and overlay when sandbox is not active", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      DAISYCLAW_SANDBOX: "1",
      DOCKER_STUB_FAIL_MATCH: "--entrypoint docker daisyclaw-gateway --version",
    });
  ...
  it("skips sandbox gateway restart when sandbox config writes fail", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        DAISYCLAW_SANDBOX: "1",
        DAISYCLAW_DOCKER_SOCKET: socketPath,
      ...
      expect(log).toContain(
        "run --rm --no-deps daisyclaw-cli config set agents.defaults.sandbox.mode non-main",
      );
      expect(log).toContain("config set agents.defaults.sandbox.mode off");
      const forceRecreateLine = log
        .split("\n")
        .find((line) => line.includes("up -d --force-recreate daisyclaw-gateway"));
      expect(forceRecreateLine).toBe(
        `compose compose -f ${join(activeSandbox.rootDir, "docker-compose.yml")} up -d --force-recreate daisyclaw-gateway`,
      );
  ...
  it("rejects injected multiline DAISYCLAW_EXTRA_MOUNTS values", () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      DAISYCLAW_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("DAISYCLAW_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid DAISYCLAW_EXTRA_MOUNTS mount format", () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      DAISYCLAW_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid DAISYCLAW_HOME_VOLUME names", () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      DAISYCLAW_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("DAISYCLAW_HOME_VOLUME must match");
  });

  it("rejects DAISYCLAW_TZ values that are not present in zoneinfo", () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      DAISYCLAW_TZ: "Nope/Bad",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("DAISYCLAW_TZ must match a timezone in /usr/share/zoneinfo");
  });

  it("skips onboarding when DAISYCLAW_SKIP_ONBOARDING is set", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);
    ...
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("DAISYCLAW_SKIP_ONBOARDING=1");
  });

  it("treats DAISYCLAW_SKIP_ONBOARDING=0 as disabled and runs onboarding", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);

    const result = runDockerSetup(activeSandbox, {
      DAISYCLAW_SKIP_ONBOARDING: "0",
    });

    expect(result.status).toBe(0);
    const log = await readDockerLog(activeSandbox);
    expect(log).toContain(
      "onboard --mode local --no-install-daemon --gateway-auth token --gateway-token-ref-env DAISYCLAW_GATEWAY_TOKEN --skip-ui --suppress-gateway-token-output",
    );
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toMatch(/DAISYCLAW_SKIP_ONBOARDING=\n/);
  });
  ...
  it("keeps docker-compose gateway Bonjour advertising in auto mode by default", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(
      compose.match(/DAISYCLAW_DISABLE_BONJOUR: \$\{DAISYCLAW_DISABLE_BONJOUR:-\}/g),
    ).toHaveLength(1);
  });

  it("keeps docker-compose CLI network namespace settings in sync", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose).toContain('network_mode: "service:daisyclaw-gateway"');
    expect(compose).toContain("depends_on:\n      - daisyclaw-gateway");
  });

  it("keeps docker-compose gateway token env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/DAISYCLAW_GATEWAY_TOKEN: \$\{DAISYCLAW_GATEWAY_TOKEN:-\}/g)).toHaveLength(
      2,
    );
  });
  ...
  it("keeps docker-compose auth profile secret key source durable outside state", async () => {
    ...
    expect(
      compose.split(
        '"${DAISYCLAW_AUTH_PROFILE_SECRET_DIR:-${HOME:-/tmp}/.daisyclaw-auth-profile-secrets}:/home/node/.config/daisyclaw"',
      ),
    ).toHaveLength(3);
  });
  ...
  it("keeps docker-compose timezone env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/TZ: \$\{DAISYCLAW_TZ:-UTC\}/g)).toHaveLength(2);
  });
  ...
  it("pins container-side state, workspace, and config dirs on both services so host .env paths cannot leak (#77436)", async () => {
    ...
    expect(compose.match(/DAISYCLAW_HOME: \/home\/node$/gm)).toHaveLength(2);
    expect(compose.match(/DAISYCLAW_STATE_DIR: \/home\/node\/\.daisyclaw$/gm)).toHaveLength(2);
    expect(
      compose.match(/DAISYCLAW_CONFIG_PATH: \/home\/node\/\.daisyclaw\/daisyclaw\.json$/gm),
    ).toHaveLength(2);
    expect(compose.match(/DAISYCLAW_CONFIG_DIR: \/home\/node\/\.daisyclaw$/gm)).toHaveLength(2);
    expect(
      compose.match(/DAISYCLAW_WORKSPACE_DIR: \/home\/node\/\.daisyclaw\/workspace$/gm),
    ).toHaveLength(2);
  });

  it("Dockerfile ARG DAISYCLAW_IMAGE_APT_PACKAGES must not have a default value", async () => {
    // If the ARG has a default (e.g. ARG DAISYCLAW_IMAGE_APT_PACKAGES=""), Docker treats it as
    // "set" even when no --build-arg is passed. That breaks the RUN fallback expression
    // ${DAISYCLAW_IMAGE_APT_PACKAGES-$DAISYCLAW_DOCKER_APT_PACKAGES} because the variable is
    // never truly unset, so legacy-only callers using --build-arg DAISYCLAW_DOCKER_APT_PACKAGES
    ...
    const argLine = dockerfile
      .split("\n")
      .find((line) => line.startsWith("ARG DAISYCLAW_IMAGE_APT_PACKAGES"));
    expect(argLine).toBeDefined();
    // Must be bare `ARG DAISYCLAW_IMAGE_APT_PACKAGES` with no default assignment
    expect(argLine).toBe("ARG DAISYCLAW_IMAGE_APT_PACKAGES");
  });
});
