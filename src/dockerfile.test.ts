...
import { BUNDLED_PLUGIN_ROOT_DIR } from "daisyclaw/plugin-sdk/test-fixtures";
...
describe("Dockerfile", () => {
  ...
  it("uses full bookworm for build stages and slim bookworm for runtime", async () => {
    ...
    expect(dockerfile).toContain("FROM ${DAISYCLAW_NODE_BOOKWORM_IMAGE} AS workspace-deps");
    expect(dockerfile).toContain("FROM ${DAISYCLAW_NODE_BOOKWORM_IMAGE} AS build");
    expect(dockerfile).toContain("FROM ${DAISYCLAW_NODE_BOOKWORM_SLIM_IMAGE} AS base-runtime");
    ...
    expect(dockerfile).not.toContain("DAISYCLAW_VARIANT");
  });
  ...
  it("installs optional browser dependencies after pnpm install", async () => {
    ...
    const browserArgIndex = dockerfile.indexOf("ARG DAISYCLAW_INSTALL_BROWSER");
  ...
  it("prunes runtime dependencies and omitted plugin packages after the build stage", async () => {
    ...
    expect(dockerfile).toContain("ARG DAISYCLAW_EXTENSIONS");
    expect(dockerfile).toContain("ARG DAISYCLAW_BUNDLED_PLUGIN_DIR");
    expect(dockerfile).toContain(
  ...
  it("keeps the Codex plugin in official Docker release images", async () => {
    ...
    const releaseKeepList = "DAISYCLAW_EXTENSIONS=diagnostics-otel,codex";

    expect(workflow.match(new RegExp(releaseKeepList, "g"))).toHaveLength(4);
    expect(workflow).not.toContain("DAISYCLAW_EXTENSIONS=diagnostics-otel\n");
  });
  ...
  it("publishes official Docker browser images with baked Chromium", async () => {
    ...
    expect(workflow).toContain("DAISYCLAW_INSTALL_BROWSER=1");
    ...
    expect(workflow).toContain("grep -q '^ARG DAISYCLAW_INSTALL_BROWSER' Dockerfile");
  ...
  it("smokes runtime workspace templates before Docker release manifests publish", async () => {
    ...
    expect(workflow).toContain('test -f "${temp_root}/home/.daisyclaw/workspace/HEARTBEAT.md"');
  });
  ...
  it("keeps only the runtime-assets prune proof in full release validation", async () => {
    ...
    expect(workflow).not.toContain('test -f "${temp_root}/home/.daisyclaw/workspace/HEARTBEAT.md"');
  ...
  it("does not override bundled plugin discovery in runtime images", async () => {
    const dockerfile = collapseDockerContinuations(await readFile(dockerfilePath, "utf8"));
    expect(dockerfile).toContain(`ARG DAISYCLAW_BUNDLED_PLUGIN_DIR=${BUNDLED_PLUGIN_ROOT_DIR}`);
    expect(dockerfile).not.toMatch(/^\s*ENV\b[^\n]*\bDAISYCLAW_BUNDLED_PLUGINS_DIR\b/m);
  });
  ...
  it("Docker GPG fingerprint awk uses correct quoting for DAISYCLAW_SANDBOX=1 build", async () => {
    const dockerfile = await readFile(dockerfilePath, "utf8");
  ...
  it("pre-creates named-volume mount points before switching to the node user", async () => {
    ...
    // before the leaf .config/daisyclaw dir (issue #85968).
    ...
    expect(dockerfile).not.toContain("mkdir -p /home/node/.daisyclaw");
    expect(dockerfile).toContain("/home/node/.daisyclaw/workspace");
    expect(dockerfile).toContain("/home/node/.config/daisyclaw");
    expect(dockerfile).toContain(
