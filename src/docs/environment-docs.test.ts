...
describe("environment docs", () => {
  it("documents the trusted sources for provider credentials", async () => {
    ...
    expect(markdown).toContain(
      "DaisyClaw ignores provider credential environment variables from workspace `.env` files",
    );
    expect(markdown).toContain("~/.daisyclaw/.env");
    expect(markdown).toContain("$DAISYCLAW_STATE_DIR/.env");
    expect(markdown).toContain("The config `env` block");
    expect(markdown).toContain("DAISYCLAW_LOAD_SHELL_ENV=1");
