...
import {
  ...
  DAISYCLAW_NODE_EXTRA_CA_CERTS_READY,
  DAISYCLAW_NODE_OPTIONS_READY,
...
describe("buildCliRespawnPlan", () => {
  ...
  it("adds NODE_EXTRA_CA_CERTS and warning suppression in one respawn", () => {
    ...
    expect(respawnPlan.env[DAISYCLAW_NODE_EXTRA_CA_CERTS_READY]).toBe("1");
    expect(respawnPlan.env[DAISYCLAW_NODE_OPTIONS_READY]).toBe("1");
  });
  ...
  it.each(["tui", "terminal", "chat"] as const)(
    ...
    (command) => {
      ...
      const respawnPlan = expectCliRespawnPlan(plan);
      expect(respawnPlan.argv).toEqual(["daisyclaw", command]);
      expect(respawnPlan.env.NODE_EXTRA_CA_CERTS).toBe("/etc/ssl/certs/ca-certificates.crt");
      expect(respawnPlan.env[DAISYCLAW_NODE_EXTRA_CA_CERTS_READY]).toBe("1");
      expect(respawnPlan.env[DAISYCLAW_NODE_OPTIONS_READY]).toBeUndefined();
    },
  );
  ...
  it("does not respawn interactive commands for warning suppression only", () => {
    expect(
      buildCliRespawnPlan({
        argv: ["node", "daisyclaw", "tui"],
        env: { [DAISYCLAW_NODE_EXTRA_CA_CERTS_READY]: "1" },
        execArgv: [],
  ...
  it("returns null when both respawn guards are already satisfied", () => {
    expect(
      buildCliRespawnPlan({
        argv: ["node", "daisyclaw", "status"],
        env: {
          [DAISYCLAW_NODE_EXTRA_CA_CERTS_READY]: "1",
          [DAISYCLAW_NODE_OPTIONS_READY]: "1",
        },
  ...
  it("adds a larger V8 stack size on Windows", () => {
    ...
    expect(respawnPlan.env[DAISYCLAW_NODE_EXTRA_CA_CERTS_READY]).toBeUndefined();
    expect(respawnPlan.env[DAISYCLAW_NODE_OPTIONS_READY]).toBeUndefined();
  });
  ...
  it("respawns Volta shims through node so the shim is not called directly", () => {
    const plan = buildCliRespawnPlan({
      argv: ["/home/alice/.volta/bin/volta-shim", "/usr/local/bin/daisyclaw", "status"],
    ...
    runCliRespawnPlan(
      {
        ...
        argv: ["/repo/daisyclaw/dist/entry.js", "status"],
        env: { DAISYCLAW_NODE_OPTIONS_READY: "1" },
      },
      {
  ...
  it("force-kills a signaled respawn child that does not exit", () => {
    ...
    try {
      runCliRespawnPlan(
        {
          command: "/usr/bin/node",
          argv: ["/repo/daisyclaw/dist/entry.js", "tui"],
          env: {},
        },
        {
          spawn: spawn as unknown as typeof import("node:child_process").spawn,
