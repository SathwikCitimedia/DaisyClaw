...
export const DAISYCLAW_NODE_OPTIONS_READY = "DAISYCLAW_NODE_OPTIONS_READY";
export const DAISYCLAW_NODE_EXTRA_CA_CERTS_READY = "DAISYCLAW_NODE_EXTRA_CA_CERTS_READY";
...
export function buildCliRespawnPlan(
  ...
  if (
    ...
    isTruthyEnvValue(env.DAISYCLAW_NO_RESPAWN)
  ) {
    return null;
  }
  ...
  if (
    ...
    !isTruthyEnvValue(env[DAISYCLAW_NODE_EXTRA_CA_CERTS_READY]) &&
    !env.NODE_EXTRA_CA_CERTS
  ) {
    childEnv.NODE_EXTRA_CA_CERTS = autoNodeExtraCaCerts;
    childEnv[DAISYCLAW_NODE_EXTRA_CA_CERTS_READY] = "1";
    needsRespawn = true;
  }

  if (
    !shouldSkipRespawnForArgv(argv) &&
    !isTruthyEnvValue(env[DAISYCLAW_NODE_OPTIONS_READY]) &&
    !hasExperimentalWarningSuppressed({ env, execArgv })
  ) {
    childEnv[DAISYCLAW_NODE_OPTIONS_READY] = "1";
...
export function runCliRespawnPlan(
  ...
  return runRespawnChildWithSignalBridge({
    ...
    onError: (error) => {
      runtime.writeError(
        "[daisyclaw] Failed to respawn CLI:",
