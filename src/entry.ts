#!/usr/bin/env node
// Boots the DaisyClaw CLI entry point under Node.
// CLI process entrypoint for DaisyClaw command execution.
...
import {
  enableDaisyClawCompileCache,
  resolveEntryInstallRoot,
  respawnWithoutDaisyClawCompileCacheIfNeeded,
} from "./entry.compile-cache.js";
import { buildCliRespawnPlan, runCliRespawnPlan } from "./entry.respawn.js";
import { tryHandleRootVersionFastPath } from "./entry.version-fast-path.js";
import { isTruthyEnvValue, normalizeEnv } from "./infra/env.js";
import { isMainModule } from "./infra/is-main.js";
import { ensureDaisyClawExecMarkerOnProcess } from "./infra/daisyclaw-exec-env.js";
import { installProcessWarningFilter } from "./infra/warning-filter.js";

const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "daisyclaw.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "daisyclaw.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  const waitingForCompileCacheRespawn = respawnWithoutDaisyClawCompileCacheIfNeeded({
    currentFile: entryFile,
    installRoot,
  });
  if (!waitingForCompileCacheRespawn) {
    process.title = "daisyclaw";
    ...
    if (shouldForceReadOnlyAuthStore(process.argv)) {
      process.env.DAISYCLAW_AUTH_STORE_READONLY = "1";
    }
    ...
    if (!ensureCliRespawnReady()) {
      ...
      if (!parsedContainer.ok) {
        console.error(`[daisyclaw] ${parsedContainer.error}`);
        process.exit(2);
      }
      ...
      if (!parsed.ok) {
        ...
        console.error(`[daisyclaw] ${parsed.error}`);
        process.exit(2);
      }

      const containerTargetName = resolveCliContainerTarget(process.argv);
      if (containerTargetName && parsed.profile) {
        console.error("[daisyclaw] --container cannot be combined with --profile/--dev");
        process.exit(2);
      }
...
export async function tryHandleRootHelpFastPath(
  ...
  const handleError =
    ...
    ((error: unknown) => {
      console.error(
        "[daisyclaw] Failed to display help:",
...
export async function tryHandlePrecomputedCommandHelpFastPath(
  ...
  if (env.DAISYCLAW_DISABLE_CLI_STARTUP_HELP_FAST_PATH === "1") {
    return false;
  }
