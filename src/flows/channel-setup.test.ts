...
const resolveAgentWorkspaceDir = vi.hoisted(() =>
  vi.fn((_cfg?: unknown, _agentId?: unknown) => "/tmp/daisyclaw-workspace"),
);
...
describe("setupChannels workspace shadow exclusion", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    resolveAgentWorkspaceDir.mockReturnValue("/tmp/daisyclaw-workspace");
  ...
  it("preloads configured external plugins from the trusted catalog boundary", async () => {
    ...
    expect(trustedInput.workspaceDir).toBe("/tmp/daisyclaw-workspace");
    ...
    expect(registryInput.workspaceDir).toBe("/tmp/daisyclaw-workspace");
  });
  ...
  it("allowlists ClickClack when it is explicitly selected for setup", async () => {
    const setupWizard = {
      ...
      configure: vi.fn(async ({ cfg }: { cfg: DaisyClawConfig }) => ({
        cfg: {
          ...cfg,
          channels: {
            ...cfg.channels,
  ...
  it("loads the selected bundled catalog plugin without writing explicit plugin enablement", async () => {
    ...
    expect(firstRegistryInput.workspaceDir).toBe("/tmp/daisyclaw-workspace");
    ...
    expect(secondRegistryInput.workspaceDir).toBe("/tmp/daisyclaw-workspace");
  ...
  it(
    ...
    async () => {
      ...
      expect(catalogLookupCall[1]?.workspaceDir).toBe("/tmp/daisyclaw-workspace");
      expect(ensureChannelSetupPluginInstalled).toHaveBeenCalledTimes(1);
      expectExternalCatalogInstallCall();
      expect(note).not.toHaveBeenCalledWith("external-chat plugin not available.", "Channel setup");
      expect(configure).toHaveBeenCalledTimes(1);
