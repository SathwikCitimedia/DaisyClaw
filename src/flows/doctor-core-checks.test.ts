...
function createSkill(overrides: Partial<SkillStatusEntry> = {}): SkillStatusEntry {
  return {
    ...
    source: "workspace",
    bundled: false,
    filePath: "/tmp/daisyclaw-test-workspace/skills/missing-tool/SKILL.md",
    baseDir: "/tmp/daisyclaw-test-workspace/skills/missing-tool",
    ...
    requirements: {
      bins: ["daisyclaw-test-missing-skill-bin"],
      anyBins: [],
      env: [],
      config: [],
      os: [],
...
describe("registerCoreHealthChecks", () => {
  ...
  it("converts unavailable skills into repair-capable health findings", async () => {
    ...
    const cfg: DaisyClawConfig = {
      agents: {
        defaults: {
          workspace: "/tmp/daisyclaw-test-workspace",
          skills: ["missing-tool"],
        },
      },
    };
    ...
    const findings = await check.detect({
      ...
      runtime,
      cfg,
      cwd: "/tmp/daisyclaw-test-workspace",
    });
  ...
  it("skips gateway auth warning when SecretRef-managed token resolves in lint checks", async () => {
    ...
    await withEnvAsync({ DAISYCLAW_TEST_GATEWAY_TOKEN: "resolved-test-token" }, async () => {
      const findings = await check?.detect({
        mode: "lint",
          ...
          gateway: {
            ...
            auth: {
              ...
              token: {
                source: "env",
                provider: "default",
                id: "DAISYCLAW_TEST_GATEWAY_TOKEN",
              },
            },
          },
          secrets: {
  ...
  it("reports unresolved SecretRefs even when DAISYCLAW_GATEWAY_TOKEN is set", async () => {
    const check = CORE_HEALTH_CHECKS.find((entry) => entry.id === "core/doctor/gateway-auth");
    await withEnvAsync(
      {
        DAISYCLAW_GATEWAY_TOKEN: "fallback-token",
        DAISYCLAW_MISSING_GATEWAY_REF_TOKEN: undefined,
      },
      async () => {
        const findings = await check?.detect({
          mode: "lint",
  ...
  it("does not execute or warn for valid exec SecretRefs during default gateway auth lint checks", async () => {
    tmp = await fs.mkdtemp(join(tmpdir(), "daisyclaw-health-exec-ref-"));
  ...
  it("reports exec SecretRef failures when gateway auth lint explicitly allows exec checks", async () => {
    ...
    const findings = await withEnvAsync({ DAISYCLAW_GATEWAY_TOKEN: "fallback-token" }, async () => {
    ...
    expect(findings).toContainEqual(
      expect.objectContaining({
        ...
        message: expect.stringContaining("Gateway token SecretRef could not be resolved:"),
        fixHint:
          "Run `daisyclaw doctor --allow-exec` to verify exec SecretRefs during doctor, or `daisyclaw secrets audit --allow-exec` to audit all exec SecretRefs.",
      }),
    );
  });
