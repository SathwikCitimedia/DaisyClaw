...
import type { ConfigValidationIssue, DaisyClawConfig } from "../config/types.daisyclaw.js";
...
export type CoreHealthCheckDeps = {
  readonly detectUnavailableSkills: (cfg: DaisyClawConfig) => Promise<readonly SkillStatusEntry[]>;
  readonly collectSecurityWarnings: (cfg: DaisyClawConfig) => Promise<readonly string[]>;
...
async function collectSecurityWarningsWithRuntime(cfg: DaisyClawConfig): Promise<readonly string[]> {
...
const gatewayConfigCheck: HealthCheck = {
  ...
  kind: "core",
  description: "daisyclaw.jsonc gateway block is set and unambiguous.",
  source: "doctor",
  async detect(ctx) {
    ...
    if (!ctx.cfg.gateway?.mode) {
      findings.push({
        ...
        path: "gateway.mode",
        fixHint:
          "Run `daisyclaw configure` and set Gateway mode (local/remote), or `daisyclaw config set gateway.mode local`.",
      });
    }
    if (ctx.cfg.gateway?.mode !== "remote" && hasAmbiguousGatewayAuthModeConfig(ctx.cfg)) {
      findings.push({
        ...
        fixHint:
          "Set an explicit mode: `daisyclaw config set gateway.auth.mode token` or `... password`.",
      });
    }
    return findings;
  },
    ...
    return [
      {
        ...
        fixHint:
          "Set commands.ownerAllowFrom to your channel user id, e.g. `daisyclaw config set commands.ownerAllowFrom '[\"telegram:123456789\"]'`.",
      },
    ];
  },
};

function resolveDoctorMode(cfg: DaisyClawConfig): "local" | "remote" {
...
export function buildGatewayTokenSecretRefFixHint(ref: SecretRef): string {
  if (ref.source === "exec") {
    return "Run `daisyclaw doctor --allow-exec` to verify exec SecretRefs during doctor, or `daisyclaw secrets audit --allow-exec` to audit all exec SecretRefs.";
  }
    ...
    return [
      {
        ...
        path: "gateway.auth",
        fixHint: "Run `daisyclaw doctor --fix --generate-gateway-token` to generate a token.",
      },
    ];
  },
};
  ...
  async detect(ctx) {
    ...
    return detected.preview.map(
      (line): HealthFinding => ({
        ...
        fixHint: "Run `daisyclaw doctor --fix` to migrate legacy state.",
      }),
    );
  },
};
      ...
      (issue): HealthFinding => ({
        ...
        fixHint: issue.blockedOutsideEntry
          ? [
              ...
              "or set the affected OpenAI models to an DaisyClaw runtime policy.",
            ].join(" ")
          : [
              "Run `daisyclaw doctor --fix`: it enables plugins.entries.codex,",
              "or set the affected OpenAI models to an DaisyClaw runtime policy.",
            ].join(" "),
      }),
    );
    ...
    return [
      {
        ...
        path: workspaceDir,
        fixHint:
          "Inspect the legacy directories and migrate or remove them; see `daisyclaw doctor` for the detailed migration prompt.",
      },
    ];
  },
};
...
function unavailableSkillToFinding(skill: SkillStatusEntry): HealthFinding {
  return {
    ...
    fixHint:
      "Install/configure the missing requirement, or run `daisyclaw doctor --fix` to disable unused unavailable skills.",
  };
}
...
function browserResidueFinding(residue: LegacyClawdBrowserProfileResidue): HealthFinding {
  return {
    ...
    fixHint:
      "Run `daisyclaw doctor --fix` to archive the stale clawd profile safely instead of deleting it in place.",
  };
}
...
const browserClawdProfileResidueCheck: HealthCheck = {
  ...
  kind: "core",
  description:
    "Legacy clawd managed browser profile residue has been archived after the DaisyClaw rename.",
  source: "doctor",
  async detect(ctx, scope) {
...
const finalConfigValidationCheck: HealthCheck = {
  ...
  kind: "core",
  description: "Active daisyclaw.jsonc parses and conforms to the config schema.",
  source: "doctor",
  async detect() {
