...
type DoctorHealthFlowContext = {
  ...
  cfg: DaisyClawConfig;
  cfgForPersistence: DaisyClawConfig;
  sourceConfigValid: boolean;
  configPath: string;
...
function isUpdateDoctorRun(env: NodeJS.ProcessEnv | Record<string, string | undefined>): boolean {
  const value = env.DAISYCLAW_UPDATE_IN_PROGRESS;
  return value === "1" || value === "true";
}

function resolveDoctorMode(cfg: DaisyClawConfig): DoctorFlowMode {
...
export function shouldSkipLegacyUpdateDoctorConfigWrite(params: {
  ...
  if (!isTruthyEnvValue(params.env.DAISYCLAW_UPDATE_IN_PROGRESS)) {
    return false;
  }
...
async function runGatewayConfigHealth(ctx: DoctorHealthFlowContext): Promise<void> {
  ...
  if (!ctx.cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("daisyclaw configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("daisyclaw config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(ctx.configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("daisyclaw setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveDoctorMode(ctx.cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(ctx.cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("daisyclaw config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("daisyclaw config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
...
async function runGatewayHealthChecks(ctx: DoctorHealthFlowContext): Promise<void> {
  ...
  if ((await hasActiveGatewayExecCredential(ctx)) && ctx.options.allowExec !== true) {
    note(
      "Gateway health probes skipped because gateway credentials use an exec SecretRef. Run `daisyclaw doctor --allow-exec` to verify Gateway health with exec SecretRefs.",
      "Gateway",
    );
...
async function runWriteConfigHealth(ctx: DoctorHealthFlowContext): Promise<void> {
  ...
  if (!ctx.prompter.shouldRepair) {
    ctx.runtime.log(`Run "${formatCliCommand("daisyclaw doctor --fix")}" to apply changes.`);
  }
}
