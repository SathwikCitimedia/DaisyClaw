        ...
        plugins: {
          ...
          installs: {
            ...
            [entry.pluginId]: {
              ...
              installPath: `/tmp/daisyclaw-plugins/${entry.pluginId}`,
            },
          },
        },
      },
...
describe("runSearchSetupFlow", () => {
  ...
  it("localizes setup copy for web search provider selection", async () => {
    ...
    await withEnvAsync({ DAISYCLAW_LOCALE: "zh-CN" }, async () => {
    ...
    expect(select).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        options: expect.arrayContaining([
          expect.objectContaining({
            label: "暂时跳过",
            hint: "稍后可用 daisyclaw configure --section web 配置",
          }),
        ]),
      }),
    );
  ...
  it("installs an external catalog search provider before enabling it", async () => {
    ...
    expect(installRequest.entry?.install?.npmSpec).toBe("@daisyclaw/brave-plugin");
    ...
    expect(next.plugins?.installs?.brave?.spec).toBe("@daisyclaw/brave-plugin");
  });
