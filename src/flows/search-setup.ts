...
export type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<DaisyClawConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<DaisyClawConfig["tools"]>["web"]>["search"]>;
...
const WEB_SEARCH_DOCS_URL = "https://docs.daisyclaw.ai/tools/web";
...
function rawKeyValue(config: DaisyClawConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: DaisyClawConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: DaisyClawConfig, provider: SearchProvider): SecretRef {
  const entry =
...
export function applySearchKey(
  ...
  const nextBase: DaisyClawConfig = {
    ...config,
    tools: {
      ...config.tools,
...
function preserveDisabledState(original: DaisyClawConfig, result: DaisyClawConfig): DaisyClawConfig {
  if (original.tools?.web?.search?.enabled !== false) {
    return result;
  }

  const next: DaisyClawConfig = {
    ...result,
    tools: {
      ...result.tools,
  ...
  return {
    ...next,
    plugins: nextPlugins as DaisyClawConfig["plugins"],
  };
}
...
export async function runSearchSetupFlow(
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "DaisyClaw will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.daisyclaw.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — DaisyClaw will store a reference instead of the API key.",
