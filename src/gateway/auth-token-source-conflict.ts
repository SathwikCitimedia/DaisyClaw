...
const GATEWAY_ENV_TOKEN = "DAISYCLAW_GATEWAY_TOKEN";
const GATEWAY_SERVICE_KIND = "gateway";

// Doctor/startup warning shape for shells where DAISYCLAW_GATEWAY_TOKEN would
...
export function resolveGatewayAuthTokenSourceConflict(params: {
  ...
  const envToken = normalizeOptionalString(params.env.DAISYCLAW_GATEWAY_TOKEN);
  if (!envToken) {
    return null;
  }

  if (params.env.DAISYCLAW_SERVICE_KIND?.trim() === GATEWAY_SERVICE_KIND) {
  ...
  const remediation =
    `Remove ${GATEWAY_ENV_TOKEN} from the shell, ~/.daisyclaw/.env, or launchctl env if gateway.auth.token is intended, ` +
