...
function resetGatewayCallMocks() {
  ...
  const loadConfigForTests = getRuntimeConfig as unknown as () => DaisyClawConfig;
...
describe("callGateway url resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot.restore();
    delete process.env.DAISYCLAW_ALLOW_INSECURE_PRIVATE_WS;
  ...
  it("allows Tailscale-authenticated backend calls without client-side credentials", async () => {
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "remote",
        remote: { url: "wss://daisyclaw.example.test" },
    ...
    expect(lastClientOptions?.url).toBe("wss://daisyclaw.example.test");
    expect(lastClientOptions?.token).toBeUndefined();
  ...
  it("uses DAISYCLAW_GATEWAY_URL env override in remote mode when remote URL is missing", async () => {
    getRuntimeConfig.mockReturnValue({
      gateway: { mode: "remote", bind: "loopback", remote: {} },
    });
    resolveGatewayPort.mockReturnValue(18789);
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
    process.env.DAISYCLAW_GATEWAY_URL = "wss://gateway-in-container.internal:9443/ws";
...
describe("buildGatewayConnectionDetails", () => {
  ...
  it("uses env DAISYCLAW_GATEWAY_URL when set", () => {
    getRuntimeConfig.mockReturnValue({ gateway: { mode: "local", bind: "loopback" } });
    resolveGatewayPort.mockReturnValue(18800);
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
    const prevUrl = process.env.DAISYCLAW_GATEWAY_URL;
    try {
      process.env.DAISYCLAW_GATEWAY_URL = "wss://browser-gateway.local:9443/ws";

      const details = buildGatewayConnectionDetails();

      expect(details.url).toBe("wss://browser-gateway.local:9443/ws");
      expect(details.urlSource).toBe("env DAISYCLAW_GATEWAY_URL");
      expect(details.bindDetail).toBeUndefined();
    } finally {
      if (prevUrl === undefined) {
        delete process.env.DAISYCLAW_GATEWAY_URL;
      } else {
        process.env.DAISYCLAW_GATEWAY_URL = prevUrl;
      }
    }
  });

  it("falls back to the default config loader when test deps drift", () => {
    const tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-gateway-call-"));
    process.env.DAISYCLAW_STATE_DIR = tempStateDir;
    process.env.DAISYCLAW_CONFIG_PATH = path.join(tempStateDir, "missing-config.json");
    try {
      getRuntimeConfig.mockReturnValue({ gateway: { mode: "local", bind: "loopback" } });
      resolveGatewayPort.mockReturnValue(18800);
      testing.setDepsForTests({
  ...
  it("throws for insecure ws:// remote URLs (CWE-319)", () => {
    ...
    expect((thrown as Error).message).toContain("daisyclaw doctor --fix");
  });
  ...
  it("allows ws:// hostname remote URLs when DAISYCLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.DAISYCLAW_ALLOW_INSECURE_PRIVATE_WS = "1";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "remote",
        bind: "loopback",
        remote: { url: "ws://daisyclaw-gateway.ai:18789" },
      },
    });
    resolveGatewayPort.mockReturnValue(18789);
    ...
    expect(details.url).toBe("ws://daisyclaw-gateway.ai:18789");
    expect(details.urlSource).toBe("config gateway.remote.url");
  });
...
describe("callGateway error details", () => {
  ...
  it("keeps the default wrapper timeout aligned with env handshake timeout", async () => {
    const envSnapshot = captureEnv(["DAISYCLAW_HANDSHAKE_TIMEOUT_MS"]);
    try {
      process.env.DAISYCLAW_HANDSHAKE_TIMEOUT_MS = "30000";
      startMode = "silent";
      setLocalLoopbackGatewayConfig();
  ...
  it("runs the signal abort hook on the active gateway connection before teardown", async () => {
    ...
    testing.setDepsForTests({
      ...
      getRuntimeConfig: getRuntimeConfig as unknown as () => DaisyClawConfig,
...
describe("callGateway url override auth requirements", () => {
  ...
  it("throws when url override is set without explicit credentials", async () => {
    process.env.DAISYCLAW_GATEWAY_TOKEN = "env-token";
    process.env.DAISYCLAW_GATEWAY_PASSWORD = "env-password";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "local",
  ...
  it("throws when env URL override is set without env credentials", async () => {
    process.env.DAISYCLAW_GATEWAY_URL = "wss://override.example/ws";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "local",
        auth: { token: "local-token", password: "local-password" },
...
describe("callGateway password resolution", () => {
  ...
  const explicitAuthCases = [
    {
      ...
      envKey: "DAISYCLAW_GATEWAY_PASSWORD",
      envValue: "from-env",
    ...
    },
    {
      ...
      envKey: "DAISYCLAW_GATEWAY_TOKEN",
      envValue: "env-token",
  ...
  it.each([
    ...
    if (envPassword !== undefined) {
      process.env.DAISYCLAW_GATEWAY_PASSWORD = envPassword;
    }
  ...
  it("does not resolve local password ref when env password takes precedence", async () => {
    process.env.DAISYCLAW_GATEWAY_PASSWORD = "from-env";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "local",
        bind: "loopback",
