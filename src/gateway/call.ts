...
import { isLoopbackIpAddress } from "@daisyclaw/net-policy/ip";
import { redactSensitiveUrlLikeString } from "@daisyclaw/net-policy/redact-sensitive-url";
...
type GatewayRuntimeConfigLoader = () => DaisyClawConfig | Promise<DaisyClawConfig>;
const defaultGetRuntimeConfig = async (): Promise<DaisyClawConfig> =>
...
async function loadGatewayConfig(): Promise<DaisyClawConfig> {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: DaisyClawConfig, env?: NodeJS.ProcessEnv): number {
...
function resolveGatewayCallAuth(config: DaisyClawConfig) {
...
function resolveGatewayCallTimeout(
  ...
  const hasEnvHandshakeTimeout =
    Boolean(process.env.DAISYCLAW_HANDSHAKE_TIMEOUT_MS) ||
    Boolean(process.env.VITEST && process.env.DAISYCLAW_TEST_HANDSHAKE_TIMEOUT_MS);
...
async function resolveGatewayCallContext(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.DAISYCLAW_GATEWAY_URL);
  ...
  const config =
    opts.config ?? (canSkipConfigLoad ? ({} as DaisyClawConfig) : await loadGatewayConfig());
...
function formatGatewayCloseError(
  ...
  if (code === 1006) {
    message +=
      ...
      "\nRun `daisyclaw doctor` for diagnostics.";
  }
  return message;
}
