...
import { asFiniteNumber } from "@daisyclaw/normalization-core/number-coercion";
import { asOptionalRecord as readRecord } from "@daisyclaw/normalization-core/record-coerce";
import { normalizeOptionalString } from "@daisyclaw/normalization-core/string-coerce";
import { DAISYCLAW_RUNTIME_CONTEXT_CUSTOM_TYPE } from "../agents/internal-runtime-context.js";
...
import { isDaisyClawDeliveryMirrorAssistantMessage } from "../shared/transcript-only-daisyclaw-assistant.js";
...
function sanitizeChatHistoryContentBlock(
  ...
  if ("daisyclawReasoningReplay" in entry) {
    delete entry.daisyclawReasoningReplay;
    changed = true;
  }
...
function buildMessageToolVisibleReplyMirror(
  ...
  const transcriptMeta = readRecord((pending.completionAnchor ?? pending.anchor)["__daisyclaw"]);
  if (transcriptMeta) {
    mirror["__daisyclaw"] = { ...transcriptMeta };
  }
  return mirror;
}
...
function readMessageToolDeliveryMirrorText(message: Record<string, unknown>): string | undefined {
  ...
  if (!isDaisyClawDeliveryMirrorAssistantMessage(message)) {
    return undefined;
  }
...
function readTtsSupplementMarker(
  ...
  const marker = message.daisyclawTtsSupplement;
...
function readChatHistoryRecordTimestampMs(message: unknown): number | undefined {
  const meta = readRecord(readRecord(message)?.["__daisyclaw"]);
...
function isDisplayHiddenProjectedMessage(message: Record<string, unknown>): boolean {
  ...
  return message.role === "custom" && message.customType === DAISYCLAW_RUNTIME_CONTEXT_CUSTOM_TYPE;
}
...
function daisyclawAssistantModel(message: Record<string, unknown>): string | undefined {
...
function isDuplicateAcpGatewayInjectedMessage(
  ...
  if (
    daisyclawAssistantModel(previousVisible) !== "acp-runtime" ||
    daisyclawAssistantModel(current) !== "gateway-injected"
  ) {
    return false;
  }
