...
// Converts Claude project JSONL into DaisyClaw transcript-compatible messages.
...
import { attachDaisyClawTranscriptMeta } from "./session-utils.fs.js";
...
function normalizeClaudeCliContent(
  ...
  for (const item of content) {
    ...
    if (type === "tool_use") {
      // Claude stores tool calls as `tool_use` with `input`; DaisyClaw history
...
function parseClaudeCliHistoryEntry(
  ...
  if (type === "user") {
    return attachDaisyClawTranscriptMeta(
      {
        role: "user",
        content,
