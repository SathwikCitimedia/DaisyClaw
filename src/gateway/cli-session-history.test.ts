...
function expectCliSessionMarker(message: unknown, sessionId: string): void {
  expectFields(readRecord(message)["__daisyclaw"], { cliSessionId: sessionId });
}
...
async function withClaudeProjectsDir<T>(
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-claude-history-"));
...
describe("cli session history", () => {
  ...
  it("reads claude-cli session messages from the Claude projects store", async () => {
    await withClaudeProjectsDir(async ({ homeDir, sessionId, filePath }) => {
      ...
      expectFields(messages[0]?.["__daisyclaw"], {
      ...
      expectFields(messages[1]?.["__daisyclaw"], {
        importedFrom: "claude-cli",
  ...
  it("deduplicates imported messages against similar local transcript entries", () => {
    ...
    expectFields(readRecord(merged[2])["__daisyclaw"], {
      importedFrom: "claude-cli",
      externalId: "user-2",
    });
  });
  ...
  it("augments anthropic-routed chat history when a Claude CLI binding has local messages", async () => {
    await withClaudeProjectsDir(async ({ homeDir, sessionId }) => {
      ...
      const importedUser = messages.find((message) => {
        ...
        return (
          record.role === "user" &&
          (record["__daisyclaw"] as { cliSessionId?: unknown } | undefined)?.cliSessionId ===
            sessionId
        );
      });
      if (!importedUser) {
...
describe("readClaudeCliFallbackSeed", () => {
  ...
  beforeEach(async () => {
    tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-fallback-seed-"));
