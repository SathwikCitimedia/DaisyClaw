...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("daisyclaw doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  beforeEach(async () => {
    ...
    delete process.env.DAISYCLAW_PROXY_ACTIVE;
    delete process.env.DAISYCLAW_PROXY_LOOPBACK_MODE;
  ...
  it("allows ws:// hostnames with DAISYCLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.DAISYCLAW_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://daisyclaw-gateway.ai:18789",
      onConnectError,
    });
...
describe("GatewayClient close handling", () => {
  ...
  it("clears stale token on device token mismatch close", () => {
    const onClose = vi.fn();
    const env = { DAISYCLAW_HOME: "/tmp/custom-daisyclaw-home" };
...
describe("GatewayClient connect auth payload", () => {
  ...
  it("loads stored device auth from the provided env", () => {
    ...
    const env = {
      ...process.env,
      DAISYCLAW_STATE_DIR: "/tmp/daisyclaw-client-service-state",
