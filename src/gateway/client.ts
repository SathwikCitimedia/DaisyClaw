// DaisyClaw Gateway client facade.
// Wraps the shared gateway-client package with DaisyClaw host dependencies.
import {
...
function createDaisyClawGatewayClientHostDeps(
  overrides?: GatewayClientHostDeps,
): GatewayClientHostDeps {
  return {
    // This wrapper is the only place the package reaches into DaisyClaw runtime
...
export class GatewayClient {
  ...
  constructor(opts: GatewayClientOptions) {
    ...
    // DaisyClaw device identity, token storage, proxy routing, and logging.
    this.#client = new BaseGatewayClient({
      ...opts,
      clientVersion: opts.clientVersion ?? VERSION,
      hostDeps: createDaisyClawGatewayClientHostDeps(opts.hostDeps),
    });
  }
