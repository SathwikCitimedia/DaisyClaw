...
function createReloaderHarness(
  ...
  options: {
    initialCompareConfig?: DaisyClawConfig;
  ...
  const onHotReload = vi.fn(async (_plan: GatewayReloadPlan, _nextConfig: DaisyClawConfig) => {});
  const onRestart = vi.fn((_plan: GatewayReloadPlan, _nextConfig: DaisyClawConfig) => {});
...
function getOnlyRestartCall(harness: ReloaderHarness): [GatewayReloadPlan, DaisyClawConfig] {
...
function getOnlyHotReloadCall(harness: ReloaderHarness): [GatewayReloadPlan, DaisyClawConfig] {
...
describe("startGatewayConfigReloader", () => {
  ...
  it("skips plugin-local invalid reloads without degraded mode", async () => {
    const activeConfig: DaisyClawConfig = {
      gateway: { reload: { debounceMs: 0 } },
    ...
    const promoteSnapshot = vi.fn(async (_snapshot: ConfigFileSnapshot, _reason: string) => true);
    const previousConfig: DaisyClawConfig = {
      ...activeConfig,
      plugins: {
        entries: {
          "lossless-claw": {
  ...
  it("does not suppress functional install changes that collide with timestamp paths", async () => {
    ...
    const nextSourceConfig: DaisyClawConfig = {
      gateway: { reload: { debounceMs: 0 } },
      plugins: {
        installs: {
          lossless: {
  ...
  it("queues restart when an external plugin source write only changes the managed index", async () => {
    ...
    const readPluginInstallRecords = vi.fn().mockResolvedValueOnce({
      "lossless-claw": {
        source: "npm",
        spec: "@martian-engineering/lossless-claw",
        installPath: "/tmp/daisyclaw/plugins/lossless-claw",
        installedAt: "2026-04-22T00:00:00.000Z",
      },
  ...
  it("keeps external plugin policy-only writes on the hot reload path", async () => {
    ...
    const installRecords = {
      telegram: {
        source: "npm",
        spec: "@daisyclaw/telegram",
        installPath: "/tmp/daisyclaw/plugins/telegram",
      },
    } satisfies Record<string, PluginInstallRecord>;
    const readSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>().mockResolvedValueOnce(
      makeSnapshot({
