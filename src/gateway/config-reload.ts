...
function asPluginInstallConfig(records: PluginInstallRecords): DaisyClawConfig {
  return {
    plugins: {
      installs: records,
    },
...
export function startGatewayConfigReloader(opts: {
  ...
  onHotReload: (plan: GatewayReloadPlan, nextConfig: DaisyClawConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: DaisyClawConfig) => void | Promise<void>;
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: DaisyClawConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (
    ...
    nextCompareConfig: DaisyClawConfig,
    afterWrite?: ConfigWriteNotification["afterWrite"],
  ) => {
