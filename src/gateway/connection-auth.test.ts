...
type ConnectionAuthCase = {
  name: string;
  cfgLocal: DaisyClawConfig;
  env: NodeJS.ProcessEnv;
  options?: Partial<Omit<GatewayConnectionAuthOptions, "config" | "env">>;
  expected: ResolvedAuth;
};

function cfg(input: Partial<DaisyClawConfig>): DaisyClawConfig {
  return input as DaisyClawConfig;
}
...
const DEFAULT_ENV = {
  ...
  DAISYCLAW_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when DAISYCLAW env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when DAISYCLAW env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when DAISYCLAW env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
