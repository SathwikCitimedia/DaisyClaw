...
import { resolvePreferredDaisyClawTmpDir } from "../infra/tmp-daisyclaw-dir.js";
...
describe("handleControlUiHttpRequest", () => {
  async function withControlUiRoot<T>(params: {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-ui-"));
    try {
  ...
  async function runTrustedProxyAssistantMediaRequest(params: {
    ...
    return await runAssistantMediaRequest({
      url: `/__daisyclaw__/assistant-media?${params.meta ? "meta=1&" : ""}source=${encodeURIComponent(params.filePath)}`,
      method: "GET",
  ...
  async function withAllowedAssistantMediaRoot<T>(params: {
    ...
    const tmpRoot = await fs.mkdtemp(path.join(resolvePreferredDaisyClawTmpDir(), params.prefix));
    try {
  ...
  async function withBasePathRootFixture<T>(params: {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-ui-root-"));
    try {
  ...
  async function withPairedOperatorDeviceToken<T>(params: {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-ui-device-token-"));
    try {
      return await withEnvAsync({ DAISYCLAW_HOME: tempHome }, async () => {
    ...
    await withAllowedAssistantMediaRoot({
      ...
      fn: async (tmpRoot) => {
        ...
        const { res, handled } = await runAssistantMediaRequest({
          url: `/__daisyclaw__/assistant-media?source=${encodeURIComponent(filePath)}&token=test-token`,
          method: "GET",
  ...
  it("serves assistant media from canonical inbound media refs", async () => {
    ...
    try {
      const { res, handled } = await runAssistantMediaRequest({
        url: `/__daisyclaw__/assistant-media?source=${encodeURIComponent(`media://inbound/${id}`)}&token=test-token`,
        method: "GET",
        auth: { mode: "token", token: "test-token", allowTailscale: false },
      });
      expect(handled).toBe(true);
  ...
  it("reports assistant media metadata for canonical inbound media refs", async () => {
    ...
    try {
      const { res, handled, end } = await runAssistantMediaRequest({
        url: `/__daisyclaw__/assistant-media?meta=1&source=${encodeURIComponent(`media://inbound/${id}`)}&token=test-token`,
        method: "GET",
        auth: { mode: "token", token: "test-token", allowTailscale: false },
      });
      expect(handled).toBe(true);
  ...
  it("rejects assistant local media outside allowed preview roots", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-ui-media-blocked-"));
    try {
  ...
  it("reports assistant local media availability metadata", async () => {
    await withAllowedAssistantMediaRoot({
      ...
      fn: async (tmpRoot) => {
        ...
        await fs.writeFile(filePath, Buffer.from("not-a-real-png"));
        const { res, handled, end } = await runAssistantMediaRequest({
          url: `/__daisyclaw__/assistant-media?meta=1&source=${encodeURIComponent(filePath)}&token=test-token`,
          method: "GET",
          auth: { mode: "token", token: "test-token", allowTailscale: false },
        });
        expect(handled).toBe(true);
    ...
    await withAllowedAssistantMediaRoot({
      ...
      fn: async (tmpRoot) => {
        ...
        const meta = await runAssistantMediaRequest({
          url: `/__daisyclaw__/assistant-media?meta=1&source=${encodeURIComponent(filePath)}`,
          method: "GET",
          auth: { mode: "token", token: "test-token", allowTailscale: false },
          headers: {
        ...
        const media = await runAssistantMediaRequest({
          url: `/__daisyclaw__/assistant-media?source=${encodeURIComponent(filePath)}&mediaTicket=${encodeURIComponent(payload.mediaTicket ?? "")}`,
          method: "GET",
          auth: { mode: "token", token: "test-token", allowTailscale: false },
        });
    ...
    await withAllowedAssistantMediaRoot({
      ...
      fn: async (tmpRoot) => {
        ...
        const refresh = await runAssistantMediaRequest({
          url: `/__daisyclaw__/assistant-media?meta=1&source=${encodeURIComponent(filePath)}&mediaTicket=${encodeURIComponent(payload.mediaTicket ?? "")}`,
          method: "GET",
          auth: { mode: "token", token: "test-token", allowTailscale: false },
        });
  ...
  it("rejects assistant local media with an invalid scoped media ticket", async () => {
    await withAllowedAssistantMediaRoot({
      ...
      fn: async (tmpRoot) => {
        ...
        await fs.writeFile(filePath, Buffer.from("not-a-real-png"));
        const { res, handled, end } = await runAssistantMediaRequest({
          url: `/__daisyclaw__/assistant-media?source=${encodeURIComponent(filePath)}&mediaTicket=v1.invalid.invalid`,
          method: "GET",
          auth: { mode: "token", token: "test-token", allowTailscale: false },
        });
        expect(handled).toBe(true);
  ...
  it("reports assistant local media availability failures with a reason", async () => {
    const { res, handled, end } = await runAssistantMediaRequest({
      url: `/__daisyclaw__/assistant-media?meta=1&source=${encodeURIComponent("/Users/test/Documents/private.pdf")}&token=test-token`,
      method: "GET",
      auth: { mode: "token", token: "test-token", allowTailscale: false },
    });
    expect(handled).toBe(true);
  ...
  it("rejects assistant local media without a valid auth token when auth is enabled", async () => {
    await withAllowedAssistantMediaRoot({
      ...
      fn: async (tmpRoot) => {
        ...
        await fs.writeFile(filePath, Buffer.from("not-a-real-png"));
        const { res, handled, end } = await runAssistantMediaRequest({
          url: `/__daisyclaw__/assistant-media?source=${encodeURIComponent(filePath)}`,
          method: "GET",
          auth: { mode: "token", token: "test-token", allowTailscale: false },
        });
        expect(handled).toBe(true);
      ...
      fn: async (operatorToken) => {
        await withAllowedAssistantMediaRoot({
          ...
          fn: async (tmpRoot) => {
            ...
            await fs.writeFile(filePath, Buffer.from("not-a-real-png"));
            const { res, handled } = await runAssistantMediaRequest({
              url: `/__daisyclaw__/assistant-media?source=${encodeURIComponent(filePath)}&token=${encodeURIComponent(operatorToken)}`,
              method: "GET",
              auth: { mode: "token", token: "shared-token", allowTailscale: false },
            });
            expect(handled).toBe(true);
    ...
    await withAllowedAssistantMediaRoot({
      ...
      fn: async (tmpRoot) => {
        ...
        const { res, handled, end } = await runTrustedProxyAssistantMediaRequest({
          filePath,
          headers: {
            "x-daisyclaw-scopes": "operator.approvals",
          },
        });
  ...
  it("serves bootstrap config JSON under basePath", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const { res, end } = makeMockHttpResponse();
        const handled = await handleControlUiHttpRequest(
          { url: `/daisyclaw${CONTROL_UI_BOOTSTRAP_CONFIG_PATH}`, method: "GET" } as IncomingMessage,
          res,
          {
            basePath: "/daisyclaw",
            root: { kind: "resolved", path: tmp },
        ...
        expect(handled).toBe(true);
        const parsed = parseBootstrapPayload(end);
        expect(parsed.basePath).toBe("/daisyclaw");
        expect(parsed.assistantName).toBe("Ops");
        expect(parsed.assistantAvatar).toBe("/daisyclaw/avatar/main");
        expect(parsed.assistantAgentId).toBe("main");
        expect(Array.isArray(parsed.localMediaPreviewRoots)).toBe(true);
      },
    });
  ...
  it("serves bootstrap config under the configured /__daisyclaw__ basePath (#66946)", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const { res, end } = makeMockHttpResponse();
        const handled = await handleControlUiHttpRequest(
          {
            url: "/__daisyclaw__/control-ui-config.json",
            method: "GET",
          } as IncomingMessage,
          res,
          {
            basePath: "/__daisyclaw__",
            root: { kind: "resolved", path: tmp },
            config: {
              agents: { defaults: { workspace: tmp } },
              ui: { assistant: { name: "Ops", avatar: "ops.png" } },
        ...
        const parsed = parseBootstrapPayload(end);
        expect(parsed.basePath).toBe("/__daisyclaw__");
        expect(parsed.assistantAgentId).toBe("main");
      },
    });
  });
  ...
  // so the SPA is served at the default `/__daisyclaw__/` namespace. The browser opens
  // the default entry, `inferBasePathFromPathname("/__daisyclaw__/")` yields `/__daisyclaw__`,
  // and the loader fetches `/__daisyclaw__/control-ui-config.json`. Before this fix the
  ...
  it("serves bootstrap config at the default /__daisyclaw__ entry with no configured basePath (#66946)", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const { res, end } = makeMockHttpResponse();
        const handled = await handleControlUiHttpRequest(
  ...
  // single-underscore `/__daisyclaw/control-ui-config.json` endpoint under an empty
  ...
  // vacuous.
  it("still serves bootstrap config at the legacy /__daisyclaw/control-ui-config.json with no configured basePath (#66946)", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const { res, end } = makeMockHttpResponse();
        const handled = await handleControlUiHttpRequest(
          {
            url: "/__daisyclaw/control-ui-config.json",
            method: "GET",
          } as IncomingMessage,
          res,
          {
  ...
  // `gateway.controlUi.basePath` is set (e.g. `/daisyclaw`), current main and
  // v2026.6.1 serve the bootstrap config at `${basePath}/__daisyclaw/control-ui-config.json`
  ...
  it("still serves bootstrap config at the legacy ${basePath}/__daisyclaw/control-ui-config.json under a configured basePath (#66946)", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const { res, end } = makeMockHttpResponse();
        const handled = await handleControlUiHttpRequest(
          {
            url: "/daisyclaw/__daisyclaw/control-ui-config.json",
            method: "GET",
          } as IncomingMessage,
          res,
          {
  ...
  it("does not serve bootstrap config from the doubled /__daisyclaw__/__daisyclaw path (#66946)", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const { res, end, handled } = await runControlUiRequest({
          url: "/__daisyclaw__/__daisyclaw/control-ui-config.json",
          method: "GET",
          rootPath: tmp,
        });
        expectNotFoundResponse({ handled, res, end });
  ...
  it("serves local avatar bytes through hardened avatar handler", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-avatar-http-"));
    try {
  ...
  it("rejects avatar symlink paths from resolver", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-avatar-http-link-"));
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-avatar-http-outside-"));
    try {
  ...
  it("serves local avatar bytes when auth is enabled and the token is valid", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-avatar-auth-"));
    try {
      const avatarPath = path.join(tmp, "main.png");
      await fs.writeFile(avatarPath, "avatar-bytes\n");
  ...
  it("serves local avatar bytes when paired device-token auth is valid", async () => {
    await withPairedOperatorDeviceToken({
      fn: async (operatorToken) => {
        const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-avatar-device-token-"));
        try {
          const avatarPath = path.join(tmp, "main.png");
          await fs.writeFile(avatarPath, "avatar-bytes\n");
  ...
  it("rejects symlinked assets that resolve outside control-ui root", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        ...
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-ui-outside-"));
        try {
  ...
  it("rejects symlinked SPA fallback index.html outside control-ui root", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-ui-index-outside-"));
        try {
  ...
  it("rejects hardlinked index.html for non-package control-ui roots", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-ui-index-hardlink-"));
        try {
          const outsideIndex = path.join(outsideDir, "index.html");
    ...
    await withControlUiRoot({
      fn: async (tmp) => {
        ...
        for (const [url, expectedType] of [
          ["/__daisyclaw__/favicon.svg", "image/svg+xml"],
          ["/__daisyclaw__/manifest.webmanifest", "application/manifest+json; charset=utf-8"],
          ["/__daisyclaw__/apple-touch-icon.png", "image/png"],
          ["/__daisyclaw__/sw.js", "application/javascript; charset=utf-8"],
        ] as const) {
          const { res, end, handled } = await runControlUiRequest({
            url,
            method: "GET",
    ...
    await withControlUiRoot({
      fn: async (tmp) => {
        ...
        const handled = await handleControlUiHttpRequest(
          ...
          res,
          { basePath: "/daisyclaw", root: { kind: "resolved", path: tmp } },
        );
  ...
  it("falls through POST requests under configured basePath (plugin webhook passthrough)", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        for (const route of ["/daisyclaw", "/daisyclaw/", "/daisyclaw/some-page"]) {
          const { handled, end } = await runControlUiRequest({
            url: route,
            method: "POST",
            rootPath: tmp,
