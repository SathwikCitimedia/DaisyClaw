...
import { detectMime } from "@daisyclaw/media-core/mime";
import {
  asDateTimestampMs,
  resolveTimestampMsToIsoString,
} from "@daisyclaw/normalization-core/number-coercion";
...
const ROOT_PREFIX = "/";
const CONTROL_UI_ASSISTANT_MEDIA_PREFIX = "/__daisyclaw__/assistant-media";
...
const CONTROL_UI_NAMESPACE_PREFIX = "/__daisyclaw__/";
...
// `/__daisyclaw__/`, so a browser that opens the default entry infers
// `/__daisyclaw__` as its base path (see `inferBasePathFromPathname`) and fetches
// `/__daisyclaw__/control-ui-config.json`. Accept that namespaced alias so the
...
// Single-underscore `/__daisyclaw` prefix used by the pre-base-path-relative
...
// `/__daisyclaw/control-ui-config.json`, so current main and the v2026.6.1
// release serve and document that exact path under an empty base path.
const LEGACY_CONTROL_UI_NAMESPACE_PREFIX = "/__daisyclaw";

// The old documented no-base-path bootstrap endpoint
// (`/__daisyclaw/control-ui-config.json`, single underscore). It is derived from
// the legacy `/__daisyclaw` namespace joined with the canonical config constant
 ...
 * single-underscore suffix `${basePath}/__daisyclaw/control-ui-config.json` that
 ...
 * the default-namespace alias `/__daisyclaw__/control-ui-config.json`, which is
 * what the default `/__daisyclaw__/` entry requests after inferring its base path
...
function matchesControlUiBootstrapConfigPath(pathname: string, basePath: string): boolean {
  ...
  // `/__daisyclaw` namespace, so joining it with the base path yields
  // `${basePath}/__daisyclaw/control-ui-config.json` (or the bare legacy path
  ...
  // The default `/__daisyclaw__/` namespace alias only applies when no base path
