...
type GatewaySecretDefaults = NonNullable<DaisyClawConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${DAISYCLAW_GATEWAY_TOKEN}` from being
...
export function hasGatewayTokenEnvCandidate(env: NodeJS.ProcessEnv = process.env): boolean {
  return Boolean(trimToUndefined(env.DAISYCLAW_GATEWAY_TOKEN));
}
...
export function hasGatewayPasswordEnvCandidate(env: NodeJS.ProcessEnv = process.env): boolean {
  return Boolean(trimToUndefined(env.DAISYCLAW_GATEWAY_PASSWORD));
}
...
export function createGatewayCredentialPlan(params: {
  ...
  const envPassword = trimToUndefined(env.DAISYCLAW_GATEWAY_PASSWORD);
