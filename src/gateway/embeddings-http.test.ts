...
const WRITE_SCOPE_HEADER = { "x-daisyclaw-scopes": "operator.write" };
...
describe("OpenAI-compatible embeddings HTTP API (e2e)", () => {
  it("embeds string and array inputs", async () => {
    ...
    const qualified = await postEmbeddings(
      ...
      { "x-daisyclaw-model": "openai/text-embedding-3-small" },
    );
    expect(qualified.status).toBe(200);
    const qualifiedJson = (await qualified.json()) as { model?: string };
    expect(qualifiedJson.model).toBe("daisyclaw/default");
  ...
  it("rejects disallowed x-daisyclaw-model provider overrides", async () => {
    const res = await postEmbeddings(
      {
        model: "daisyclaw/default",
        input: "hello",
      },
      { "x-daisyclaw-model": "ollama/nomic-embed-text" },
    );
    await expectInvalidEmbeddingRequest(
      res,
