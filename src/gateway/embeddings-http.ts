...
// Bridges /v1/embeddings requests to configured DaisyClaw memory providers.
...
// OpenAI-compatible `/v1/embeddings` bridge. It maps DaisyClaw agent/model
...
export async function handleOpenAiEmbeddingsHttpRequest(
  ...
  if (requestModel !== DAISYCLAW_MODEL_ID && !resolveAgentIdFromModel(requestModel, cfg)) {
    sendJson(res, 400, {
      error: {
        message: "Invalid `model`. Use `daisyclaw` or `daisyclaw/<agentId>`.",
  ...
  const overrideModel =
    normalizeOptionalString(getHeader(req, "x-daisyclaw-model")) ||
