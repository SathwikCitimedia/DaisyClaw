...
// Emits a one-shot notice for ignored pre-DaisyClaw environment prefixes.
...
export function warnLegacyDaisyClawEnvVars(env: NodeJS.ProcessEnv = process.env): void {
  ...
  process.emitWarning(
    [
      `Legacy ${detectedPrefixes} environment variables were detected (${legacyVarCount} total), but DaisyClaw only reads DAISYCLAW_* names now.`,
      "Rename them by replacing the legacy prefix with DAISYCLAW_; the old names are ignored.",
    ].join("\n"),
    { code: "DAISYCLAW_LEGACY_ENV_VARS", type: "DeprecationWarning" },
  );
  warned = true;
}

/** Resets the one-shot legacy env warning latch for tests. */
export function resetLegacyDaisyClawEnvWarningForTest(): void {
  warned = false;
}
