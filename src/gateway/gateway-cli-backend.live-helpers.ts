...
export const CLI_BACKEND_LIVE_PROVIDER_SKIP_ENV = "DAISYCLAW_LIVE_CLI_BACKEND_ALLOW_PROVIDER_SKIP";
export const CLI_BACKEND_LIVE_ADVISORY_ENV = "DAISYCLAW_LIVE_CLI_BACKEND_ADVISORY";
...
export function resolveCliBackendLiveModelSelection(params: {
  ...
  if (!parsed) {
    throw new Error(
      `DAISYCLAW_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${params.rawModel}`,
    );
  }

  if (parsed.provider === "codex-cli") {
    throw new Error(
      "DAISYCLAW_LIVE_CLI_BACKEND_MODEL=codex-cli/... is no longer supported. Use a supported CLI backend such as claude-cli or google-gemini-cli.",
    );
  }
...
export function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("DAISYCLAW_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}

export function shouldRunCliImageProbe(providerId: string): boolean {
  const raw = process.env.DAISYCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE?.trim();
  if (raw) {
...
export function shouldRunCliMcpProbe(providerId: string): boolean {
  const raw = process.env.DAISYCLAW_LIVE_CLI_BACKEND_MCP_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function resolveCliBackendLiveArgs(params: {
  ...
  const args =
    parseJsonStringArray(
      "DAISYCLAW_LIVE_CLI_BACKEND_ARGS",
      process.env.DAISYCLAW_LIVE_CLI_BACKEND_ARGS,
    ) ?? params.defaultArgs;
  if (!args || args.length === 0) {
    throw new Error(
      `DAISYCLAW_LIVE_CLI_BACKEND_ARGS is required for provider "${params.providerId}".`,
    );
  }
  const resumeArgs =
    parseJsonStringArray(
      "DAISYCLAW_LIVE_CLI_BACKEND_RESUME_ARGS",
      process.env.DAISYCLAW_LIVE_CLI_BACKEND_RESUME_ARGS,
...
export function shouldRunCliModelSwitchProbe(providerId: string, modelRef: string): boolean {
  const raw = process.env.DAISYCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function snapshotCliBackendLiveEnv(): CliBackendLiveEnvSnapshot {
  return {
    ...
    skipProviders: process.env.DAISYCLAW_SKIP_PROVIDERS,
    ...
    skipBrowserControl: process.env.DAISYCLAW_SKIP_BROWSER_CONTROL_SERVER,
    bundledPluginsDir: process.env.DAISYCLAW_BUNDLED_PLUGINS_DIR,
    minimalGateway: process.env.DAISYCLAW_TEST_MINIMAL_GATEWAY,
...
export function applyCliBackendLiveEnv(preservedEnv: ReadonlySet<string>): void {
  ...
  process.env.DAISYCLAW_SKIP_PROVIDERS = "1";
  process.env.DAISYCLAW_SKIP_GMAIL_WATCHER = "1";
  process.env.DAISYCLAW_SKIP_CRON = "1";
  process.env.DAISYCLAW_SKIP_CANVAS_HOST = "1";
  process.env.DAISYCLAW_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.DAISYCLAW_TEST_MINIMAL_GATEWAY = "1";
...
export function restoreCliBackendLiveEnv(snapshot: CliBackendLiveEnvSnapshot): void {
  restoreEnvVar("DAISYCLAW_CONFIG_PATH", snapshot.configPath);
  restoreEnvVar("DAISYCLAW_STATE_DIR", snapshot.stateDir);
  restoreEnvVar("DAISYCLAW_GATEWAY_TOKEN", snapshot.token);
  restoreEnvVar("DAISYCLAW_SKIP_CHANNELS", snapshot.skipChannels);
  restoreEnvVar("DAISYCLAW_SKIP_PROVIDERS", snapshot.skipProviders);
  restoreEnvVar("DAISYCLAW_SKIP_GMAIL_WATCHER", snapshot.skipGmail);
  restoreEnvVar("DAISYCLAW_SKIP_CRON", snapshot.skipCron);
  restoreEnvVar("DAISYCLAW_SKIP_CANVAS_HOST", snapshot.skipCanvas);
  restoreEnvVar("DAISYCLAW_SKIP_BROWSER_CONTROL_SERVER", snapshot.skipBrowserControl);
  restoreEnvVar("DAISYCLAW_BUNDLED_PLUGINS_DIR", snapshot.bundledPluginsDir);
  restoreEnvVar("DAISYCLAW_TEST_MINIMAL_GATEWAY", snapshot.minimalGateway);
