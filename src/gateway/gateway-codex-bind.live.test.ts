...
const CODEX_BIND_LIVE = isTruthyEnvValue(process.env.DAISYCLAW_LIVE_CODEX_BIND);
const describeLive = LIVE && CODEX_BIND_LIVE ? describe : describe.skip;
const CODEX_BIND_TIMEOUT_MS = resolveLiveTimeoutMs(
  process.env.DAISYCLAW_LIVE_CODEX_BIND_TIMEOUT_MS,
  900_000,
);
const CODEX_BIND_REQUEST_TIMEOUT_MS = resolveLiveTimeoutMs(
  process.env.DAISYCLAW_LIVE_CODEX_BIND_REQUEST_TIMEOUT_MS,
  300_000,
);
...
async function writePluginBindingApproval(params: {
  ...
  const daisyclawDir = path.join(params.homeDir, ".daisyclaw");
  await fs.mkdir(daisyclawDir, { recursive: true });
  await fs.writeFile(
    path.join(daisyclawDir, "plugin-binding-approvals.json"),
    `${JSON.stringify(
      {
        version: 1,
        approvals: [
...
function resolveCodexBindModelProvider(): string | undefined {
  const configured = process.env.DAISYCLAW_LIVE_CODEX_BIND_PROVIDER?.trim();
  if (configured) {
    return configured;
  }
  return process.env.DAISYCLAW_LIVE_CODEX_HARNESS_AUTH === "api-key" ? "openai" : undefined;
}
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        gatewayToken: process.env.DAISYCLAW_GATEWAY_TOKEN,
      ...
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-live-codex-bind-"));
      ...
      const bindModel =
        process.env.DAISYCLAW_LIVE_CODEX_BIND_MODEL?.trim() || DEFAULT_CODEX_BIND_MODEL;
      ...
      try {
        ...
        try {
          ...
          restoreEnvVar("DAISYCLAW_CONFIG_PATH", previous.configPath);
          restoreEnvVar("DAISYCLAW_GATEWAY_TOKEN", previous.gatewayToken);
          restoreEnvVar("HOME", previous.home);
          restoreEnvVar("DAISYCLAW_SKIP_CANVAS_HOST", previous.skipCanvas);
          restoreEnvVar("DAISYCLAW_SKIP_CHANNELS", previous.skipChannels);
          restoreEnvVar("DAISYCLAW_SKIP_CRON", previous.skipCron);
          restoreEnvVar("DAISYCLAW_SKIP_GMAIL_WATCHER", previous.skipGmail);
          restoreEnvVar("DAISYCLAW_STATE_DIR", previous.stateDir);
        }
      }
    },
