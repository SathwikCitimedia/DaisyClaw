...
export const EXPECTED_CODEX_MODELS_COMMAND_TEXT = [
  ...
  "Current DaisyClaw session status reports the active model as:",
] as const;
...
export function isExpectedCodexStatusCommandText(text: string): boolean {
  ...
  const mentionsDaisyClawStatus =
    normalized.includes("daisyclaw is running on") ||
    /daisyclaw\s+\S+\s+is running on/u.test(normalized) ||
    normalized.includes("daisyclaw status:") ||
  ...
  return (
    ...
    isReadyStatus ||
    isOnlineIdleStatus ||
    (mentionsDaisyClawStatus && mentionsHarnessSession && mentionsModel)
  );
}
...
export function isExpectedCodexModelsCommandText(text: string): boolean {
  ...
  const mentionsSessionModel =
    ...
    normalized.includes("current session model from daisyclaw status") ||
  ...
  const mentionsConfigSummary =
    ...
    normalized.includes("live daisyclaw config shows") ||
