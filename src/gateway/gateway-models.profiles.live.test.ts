...
import { ensureDaisyClawModelsJson } from "../agents/models-config.js";
import { STREAM_ERROR_FALLBACK_TEXT } from "../agents/stream-message-shared.js";
import { clearRuntimeConfigSnapshot, getRuntimeConfig } from "../config/io.js";
import type { ModelsConfig, ModelProviderConfig, DaisyClawConfig } from "../config/types.js";
...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.DAISYCLAW_LIVE_GATEWAY_ZAI_FALLBACK);
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_CREDENTIAL_PRECEDENCE = REQUIRE_PROFILE_KEYS ? "profile-first" : "env-first";
const PROVIDERS = parseFilter(process.env.DAISYCLAW_LIVE_GATEWAY_PROVIDERS);
const GATEWAY_LIVE_SMOKE = isTruthyEnvValue(process.env.DAISYCLAW_LIVE_GATEWAY_SMOKE);
const THINKING_LEVEL = resolveGatewayLiveThinkingLevel({
  raw: process.env.DAISYCLAW_LIVE_GATEWAY_THINKING,
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.DAISYCLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
...
const GATEWAY_LIVE_HEARTBEAT_MS = Math.max(
  1_000,
  toInt(process.env.DAISYCLAW_LIVE_GATEWAY_HEARTBEAT_MS, 30_000),
);
...
const QUIET_LIVE_LOGS = process.env.DAISYCLAW_LIVE_TEST_QUIET !== "0";

const describeLive = isLiveTestEnabled(["DAISYCLAW_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveSetupTimeoutMs(
  raw = process.env.DAISYCLAW_LIVE_GATEWAY_SETUP_TIMEOUT_MS,
): number {
  return Math.max(1_000, toInt(raw, 180_000));
}

function resolveGatewayLiveMaxModels(): number {
  const gatewayRaw = process.env.DAISYCLAW_LIVE_GATEWAY_MAX_MODELS?.trim();
  if (gatewayRaw) {
    return Math.max(0, toInt(gatewayRaw, 0));
  }
  const rawModels = process.env.DAISYCLAW_LIVE_GATEWAY_MODELS?.trim();
  ...
  return resolveHighSignalLiveModelLimit({
    rawMaxModels: process.env.DAISYCLAW_LIVE_MAX_MODELS,
    useExplicitModels,
...
function resolveGatewayLiveModelTimeoutMs(
  gatewayModelTimeoutRaw = process.env.DAISYCLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS,
  liveModelTimeoutRaw = process.env.DAISYCLAW_LIVE_MODEL_TIMEOUT_MS,
...
function enterProductionEnvForLiveRun() {
  const previous = {
    ...
    testFast: process.env.DAISYCLAW_TEST_FAST,
  };
...
function restoreProductionEnvForLiveRun(previous: {
  ...
  if (previous.testFast === undefined) {
    ...
    process.env.DAISYCLAW_TEST_FAST = previous.testFast;
  }
...
describe("resolveGatewayLiveMaxModels", () => {
  const originalGatewayModels = process.env.DAISYCLAW_LIVE_GATEWAY_MODELS;
  const originalGatewayMax = process.env.DAISYCLAW_LIVE_GATEWAY_MAX_MODELS;
  const originalSharedMax = process.env.DAISYCLAW_LIVE_MAX_MODELS;
  ...
  afterEach(() => {
    restoreEnvValue("DAISYCLAW_LIVE_GATEWAY_MODELS", originalGatewayModels);
    restoreEnvValue("DAISYCLAW_LIVE_GATEWAY_MAX_MODELS", originalGatewayMax);
    restoreEnvValue("DAISYCLAW_LIVE_MAX_MODELS", originalSharedMax);
  });

  it("defaults modern gateway sweeps to the curated high-signal cap", () => {
    delete process.env.DAISYCLAW_LIVE_GATEWAY_MODELS;
    delete process.env.DAISYCLAW_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.DAISYCLAW_LIVE_MAX_MODELS;
  ...
  it("defaults small gateway sweeps to the curated small-model cap", () => {
    process.env.DAISYCLAW_LIVE_GATEWAY_MODELS = "small";
    delete process.env.DAISYCLAW_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.DAISYCLAW_LIVE_MAX_MODELS;
  ...
  it("keeps explicit gateway model lists uncapped unless a cap is provided", () => {
    process.env.DAISYCLAW_LIVE_GATEWAY_MODELS = "openai/gpt-5.5,anthropic/claude-opus-4-6";
    delete process.env.DAISYCLAW_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.DAISYCLAW_LIVE_MAX_MODELS;

    expect(resolveGatewayLiveMaxModels()).toBe(0);

    process.env.DAISYCLAW_LIVE_GATEWAY_MAX_MODELS = "2";
...
describe("buildLiveGatewayConfig", () => {
  it("pins selected live gateway models to the DaisyClaw runtime", () => {
...
describe("enterProductionEnvForLiveRun", () => {
  it("clears Vitest fast-reply flags while preserving caller state", () => {
    ...
    try {
      ...
      restoreOptionalEnv("DAISYCLAW_TEST_FAST", previous.testFast);
    }
  });
});
      ...
      defaults: {
        ...
        models: Object.fromEntries(
          params.candidates.map((m) => [
            `${m.provider}/${m.id}`,
            { agentRuntime: { id: "daisyclaw" as const } },
          ]),
        ),
      },
    },
...
async function sanitizeAuthConfig(params: {
  cfg: DaisyClawConfig;
  agentDir: string;
}): Promise<DaisyClawConfig["auth"] | undefined> {
  ...
  let profiles: NonNullable<DaisyClawConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const runtimeEnv = enterProductionEnvForLiveRun();
  const previous = {
    configPath: process.env.DAISYCLAW_CONFIG_PATH,
    token: process.env.DAISYCLAW_GATEWAY_TOKEN,
    skipChannels: process.env.DAISYCLAW_SKIP_CHANNELS,
    skipGmail: process.env.DAISYCLAW_SKIP_GMAIL_WATCHER,
    skipCron: process.env.DAISYCLAW_SKIP_CRON,
    skipCanvas: process.env.DAISYCLAW_SKIP_CANVAS_HOST,
    disableBonjour: process.env.DAISYCLAW_DISABLE_BONJOUR,
    logLevel: process.env.DAISYCLAW_LOG_LEVEL,
    agentDir: process.env.DAISYCLAW_AGENT_DIR,
    stateDir: process.env.DAISYCLAW_STATE_DIR,
  };

  process.env.DAISYCLAW_SKIP_CHANNELS = "1";
  process.env.DAISYCLAW_SKIP_GMAIL_WATCHER = "1";
  process.env.DAISYCLAW_SKIP_CRON = "1";
  process.env.DAISYCLAW_SKIP_CANVAS_HOST = "1";
  if (QUIET_LIVE_LOGS) {
    process.env.DAISYCLAW_DISABLE_BONJOUR = "1";
    process.env.DAISYCLAW_LOG_LEVEL = "silent";
  }

  const token = `test-${randomUUID()}`;
  process.env.DAISYCLAW_GATEWAY_TOKEN = token;
  const agentId = "dev";
  ...
  const tempStateDir: string | undefined = await fs.mkdtemp(
    path.join(os.tmpdir(), "daisyclaw-live-state-"),
  );
  process.env.DAISYCLAW_STATE_DIR = tempStateDir;
  ...
  process.env.DAISYCLAW_AGENT_DIR = tempAgentDir;

  const workspaceDir = resolveAgentWorkspaceDir(params.cfg, agentId);
  await fs.mkdir(workspaceDir, { recursive: true });
  await fs.mkdir(path.join(workspaceDir, ".daisyclaw"), { recursive: true });
  await fs.writeFile(
    path.join(workspaceDir, ".daisyclaw", "workspace-state.json"),
    `${JSON.stringify(
      {
        version: 1,
  ...
  const toolProbePath = path.join(workspaceDir, `.daisyclaw-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveDefaultAgentDir(params.cfg);
  const sanitizedCfg: DaisyClawConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-live-"));
  const tempConfigPath = path.join(tempDir, "daisyclaw.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.DAISYCLAW_CONFIG_PATH = tempConfigPath;
  ...
  try {
              ...
              for (
                ...
                try {
                  toolText = await requestGatewayAgentText({
                    ...
                    modelKey,
                    message: strictReply
                      ? "DaisyClaw live tool probe (local, safe): " +
    ...
    restoreOptionalEnv("DAISYCLAW_CONFIG_PATH", previous.configPath);
    restoreOptionalEnv("DAISYCLAW_GATEWAY_TOKEN", previous.token);
    restoreOptionalEnv("DAISYCLAW_SKIP_CHANNELS", previous.skipChannels);
    restoreOptionalEnv("DAISYCLAW_SKIP_GMAIL_WATCHER", previous.skipGmail);
    restoreOptionalEnv("DAISYCLAW_SKIP_CRON", previous.skipCron);
    restoreOptionalEnv("DAISYCLAW_SKIP_CANVAS_HOST", previous.skipCanvas);
    restoreOptionalEnv("DAISYCLAW_DISABLE_BONJOUR", previous.disableBonjour);
    restoreOptionalEnv("DAISYCLAW_LOG_LEVEL", previous.logLevel);
    restoreOptionalEnv("DAISYCLAW_AGENT_DIR", previous.agentDir);
    restoreOptionalEnv("DAISYCLAW_STATE_DIR", previous.stateDir);
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
    ...
    async () =>
      await withSuppressedGatewayLiveWarnings(async () => {
        ...
        const modelsJsonResult = await withGatewayLiveSetupTimeout(
          ensureDaisyClawModelsJson(cfg, undefined, {
            workspaceDir,
        ...
        if (selectedCandidates.length < candidates.length) {
          logProgress(
            `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via DAISYCLAW_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
          );
        }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    try {
      ...
      await ensureDaisyClawModelsJson(cfg);
      ...
      const nonceA = randomUUID();
      const nonceB = randomUUID();
      toolProbePath = path.join(workspaceDir, `.daisyclaw-live-zai-fallback.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
