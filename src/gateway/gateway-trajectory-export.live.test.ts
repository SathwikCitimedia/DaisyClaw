...
const CODEX_HARNESS_LIVE = process.env.DAISYCLAW_LIVE_CODEX_HARNESS === "1";
const CODEX_HARNESS_DEBUG = process.env.DAISYCLAW_LIVE_CODEX_HARNESS_DEBUG === "1";
...
function snapshotEnv(): LiveEnvSnapshot {
  return snapshotLiveEnv(["DAISYCLAW_TRAJECTORY", "DAISYCLAW_TRAJECTORY_DIR"]);
}
...
describeLive("gateway live trajectory export", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempDir = await fs.mkdtemp(path.join(process.cwd(), ".tmp-daisyclaw-trajectory-live-"));
      cleanup.push(async () => {
      ...
      process.env.DAISYCLAW_TRAJECTORY = "1";
      process.env.DAISYCLAW_TRAJECTORY_DIR = trajectoryDir;
      ...
      const bundleDir = path.join(workspaceDir, ".daisyclaw", "trajectory-exports", "bundle");
