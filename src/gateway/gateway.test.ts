...
async function createEmptyBundledPluginsDir(tempHome: string): Promise<string> {
  const bundledPluginsDir = path.join(tempHome, "daisyclaw-test-empty-bundled-plugins");
  await fs.mkdir(bundledPluginsDir, { recursive: true });
  return bundledPluginsDir;
}

async function createGatewayConfigPath(tempHome: string): Promise<string> {
  const configPath = path.join(tempHome, ".daisyclaw", "daisyclaw.json");
...
async function writeWorkspacePlugin(params: {
  ...
  const pluginDir = path.join(params.workspaceDir, ".daisyclaw", "extensions", params.id);
...
async function setupGatewayTempHome(params: { prefix: string; minimalGateway?: boolean }) {
  const envSnapshot = captureEnv([
    ...
    ...(params.minimalGateway ? (["DAISYCLAW_TEST_MINIMAL_GATEWAY"] as const) : []),
  ]);
  ...
  const workspaceDir = path.join(tempHome, "daisyclaw");
  await fs.mkdir(workspaceDir, { recursive: true });
  process.env.DAISYCLAW_BUNDLED_PLUGINS_DIR = await createEmptyBundledPluginsDir(tempHome);
...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      const { envSnapshot, tempHome, workspaceDir } = await setupGatewayTempHome({
        prefix: "daisyclaw-gw-http-tools-home-",
      });
...
`.trimStart(),
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-minimal-gateway-home-"));
      const configPath = await createGatewayConfigPath(tempHome);
      const bundledPluginsDir = path.join(tempHome, "daisyclaw-test-no-bundled-extensions");
      process.env.HOME = tempHome;
      process.env.DAISYCLAW_STATE_DIR = path.join(tempHome, ".daisyclaw");
      process.env.DAISYCLAW_CONFIG_PATH = configPath;
      process.env.DAISYCLAW_SKIP_CHANNELS = "1";
