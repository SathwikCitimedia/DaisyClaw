...
describe("gateway handshake timeouts", () => {
  ...
  test("prefers DAISYCLAW_HANDSHAKE_TIMEOUT_MS and falls back on the test-only env", () => {
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        DAISYCLAW_HANDSHAKE_TIMEOUT_MS: "75",
        DAISYCLAW_TEST_HANDSHAKE_TIMEOUT_MS: "20",
      }),
    ).toBe(75);
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        DAISYCLAW_HANDSHAKE_TIMEOUT_MS: "",
        DAISYCLAW_TEST_HANDSHAKE_TIMEOUT_MS: "20",
        VITEST: "1",
      }),
    ).toBe(20);
  ...
  test("resolves preauth handshake timeout with env over config over default", () => {
    expect(
      resolvePreauthHandshakeTimeoutMs({
        env: { DAISYCLAW_HANDSHAKE_TIMEOUT_MS: "75000" },
    ...
    expect(
      resolvePreauthHandshakeTimeoutMs({
        env: { DAISYCLAW_HANDSHAKE_TIMEOUT_MS: "garbage" },
        configuredTimeoutMs: 30_000,
      }),
    ).toBe(30_000);
  ...
  test("caps preauth handshake timeout env and config values to the safe timer range", () => {
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        DAISYCLAW_HANDSHAKE_TIMEOUT_MS: "3000000000",
      }),
  ...
  test("resolves preauth handshake timeout from the test-only env before config", () => {
    expect(
      resolvePreauthHandshakeTimeoutMs({
        env: { VITEST: "1", DAISYCLAW_TEST_HANDSHAKE_TIMEOUT_MS: "50" },
        configuredTimeoutMs: 30_000,
      }),
    ).toBe(50);
  });
  ...
  test("ignores invalid handshake timeout overrides and falls back safely", () => {
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        DAISYCLAW_HANDSHAKE_TIMEOUT_MS: "abc",
      }),
    ).toBe(DEFAULT_PREAUTH_HANDSHAKE_TIMEOUT_MS);
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        DAISYCLAW_HANDSHAKE_TIMEOUT_MS: "-1",
      }),
    ).toBe(DEFAULT_PREAUTH_HANDSHAKE_TIMEOUT_MS);
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        DAISYCLAW_HANDSHAKE_TIMEOUT_MS: "0",
      }),
    ).toBe(DEFAULT_PREAUTH_HANDSHAKE_TIMEOUT_MS);
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
    ...
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        DAISYCLAW_HANDSHAKE_TIMEOUT_MS: "1e3",
      }),
    ).toBe(DEFAULT_PREAUTH_HANDSHAKE_TIMEOUT_MS);
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        DAISYCLAW_HANDSHAKE_TIMEOUT_MS: "0x10",
      }),
    ).toBe(DEFAULT_PREAUTH_HANDSHAKE_TIMEOUT_MS);
  });

  test("getConnectChallengeTimeoutMsFromEnv reads DAISYCLAW_CONNECT_CHALLENGE_TIMEOUT_MS", () => {
    expect(getConnectChallengeTimeoutMsFromEnv({})).toBeUndefined();
    expect(
      getConnectChallengeTimeoutMsFromEnv({ DAISYCLAW_CONNECT_CHALLENGE_TIMEOUT_MS: "15000" }),
    ).toBe(15_000);
    expect(
      getConnectChallengeTimeoutMsFromEnv({ DAISYCLAW_CONNECT_CHALLENGE_TIMEOUT_MS: " 015000 " }),
    ).toBe(15_000);
    expect(
      getConnectChallengeTimeoutMsFromEnv({ DAISYCLAW_CONNECT_CHALLENGE_TIMEOUT_MS: "garbage" }),
    ).toBeUndefined();
    expect(
      getConnectChallengeTimeoutMsFromEnv({ DAISYCLAW_CONNECT_CHALLENGE_TIMEOUT_MS: "1e3" }),
    ).toBeUndefined();
    expect(
      getConnectChallengeTimeoutMsFromEnv({ DAISYCLAW_CONNECT_CHALLENGE_TIMEOUT_MS: "0x10" }),
    ).toBeUndefined();
  });
  ...
  test("caps connect challenge timeout env and explicit values to the safe timer range", () => {
    expect(
      getConnectChallengeTimeoutMsFromEnv({
        DAISYCLAW_CONNECT_CHALLENGE_TIMEOUT_MS: "3000000000",
      }),
    ).toBe(MAX_SAFE_TIMEOUT_DELAY_MS);
    expect(
    ...
    expect(
      resolveConnectChallengeTimeoutMs(undefined, {
        env: { DAISYCLAW_CONNECT_CHALLENGE_TIMEOUT_MS: "3000000000" },
      }),
    ).toBe(MAX_SAFE_TIMEOUT_DELAY_MS);
  });

  test("resolveConnectChallengeTimeoutMs falls back to env override", () => {
    const original = process.env.DAISYCLAW_CONNECT_CHALLENGE_TIMEOUT_MS;
    const originalHandshake = process.env.DAISYCLAW_HANDSHAKE_TIMEOUT_MS;
    try {
      process.env.DAISYCLAW_CONNECT_CHALLENGE_TIMEOUT_MS = "5000";
      ...
      process.env.DAISYCLAW_CONNECT_CHALLENGE_TIMEOUT_MS = "";
      process.env.DAISYCLAW_HANDSHAKE_TIMEOUT_MS = "30000";
      expect(resolveConnectChallengeTimeoutMs()).toBe(30_000);
    } finally {
      if (original === undefined) {
        delete process.env.DAISYCLAW_CONNECT_CHALLENGE_TIMEOUT_MS;
      } else {
        process.env.DAISYCLAW_CONNECT_CHALLENGE_TIMEOUT_MS = original;
      }
      if (originalHandshake === undefined) {
        delete process.env.DAISYCLAW_HANDSHAKE_TIMEOUT_MS;
      } else {
        process.env.DAISYCLAW_HANDSHAKE_TIMEOUT_MS = originalHandshake;
      }
    }
  });
