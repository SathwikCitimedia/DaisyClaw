...
export function resolveHooksConfig(cfg: DaisyClawConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: DaisyClawConfig, defaultAgentId: string): Set<string> {
...
/** Extract the hook bearer token from Authorization or x-daisyclaw-token headers. */
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken = normalizeOptionalString(req.headers["x-daisyclaw-token"]) ?? "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-daisyclaw-idempotency-key"]) ||
