...
export const DAISYCLAW_MODEL_ID = "daisyclaw";
/** Default OpenAI-compatible model alias that targets the default DaisyClaw agent. */
export const DAISYCLAW_DEFAULT_MODEL_ID = "daisyclaw/default";

function resolveAgentIdFromHeader(req: IncomingMessage): string | undefined {
  const raw =
    normalizeOptionalString(getHeader(req, "x-daisyclaw-agent-id")) ||
    normalizeOptionalString(getHeader(req, "x-daisyclaw-agent")) ||
    "";
  if (!raw) {
    return undefined;
  }
...
export function resolveAgentIdFromModel(
  ...
  if (lowered === DAISYCLAW_MODEL_ID || lowered === DAISYCLAW_DEFAULT_MODEL_ID) {
    return resolveDefaultAgentId(cfg);
  }

  const m =
    raw.match(/^daisyclaw[:/](?<agentId>[a-z0-9][a-z0-9_-]{0,63})$/i) ??
...
/** Validates and resolves the `x-daisyclaw-model` override for OpenAI-compatible requests. */
export async function resolveOpenAiCompatModelOverride(params: {
  ...
  if (requestModel && !resolveAgentIdFromModel(requestModel)) {
    return {
      errorMessage: "Invalid `model`. Use `daisyclaw` or `daisyclaw/<agentId>`.",
    };
  }

  const raw = getHeader(params.req, "x-daisyclaw-model")?.trim();
  if (!raw) {
    return {};
  }
  ...
  if (!parsed) {
    return { errorMessage: "Invalid `x-daisyclaw-model`." };
  }
...
function resolveSessionKey(params: {
  ...
  const explicit = getHeader(params.req, "x-daisyclaw-session-key")?.trim();
  if (explicit) {
    return explicit;
  }
...
export function resolveGatewayRequestContext(params: {
  ...
  const messageChannel = params.useMessageChannelHeader
    ? (normalizeMessageChannel(getHeader(params.req, "x-daisyclaw-message-channel")) ??
