...
const DAISYCLAW_CLI_GATEWAY_TIMEOUT_MS = 30_000;
const DAISYCLAW_CLI_CHILD_TIMEOUT_MS = DAISYCLAW_CLI_GATEWAY_TIMEOUT_MS + 45_000;
...
export function buildLiveCronProbeMessage(params: {
  ...
  if (params.attempt === 0) {
    return (
      "Use the DaisyClaw MCP tool `daisyclaw-tools/cron` (server `daisyclaw-tools`, tool `cron`). " +
      "If the harness shows Claude-style MCP names, use `mcp__daisyclaw-tools__cron` or `mcp__daisyclaw_tools__cron`. " +
      ...
      "Do the actual tool call; I will verify externally with the DaisyClaw cron CLI. " +
      `After the cron job is created, reply exactly: ${params.exactReply}`
    );
  }
  if (claudeLike) {
    return (
      "Retry the DaisyClaw MCP tool `daisyclaw-tools/cron` now. " +
      "If the harness shows Claude-style MCP names, use `mcp__daisyclaw-tools__cron` or `mcp__daisyclaw_tools__cron`. " +
      ...
      "I will verify externally with the DaisyClaw cron CLI."
    );
  }
  return (
    "Your previous DaisyClaw cron MCP tool call was cancelled before the job was created. " +
    "Retry the DaisyClaw MCP tool `daisyclaw-tools/cron` now. " +
    "If the harness shows Claude-style MCP names, use `mcp__daisyclaw-tools__cron` or `mcp__daisyclaw_tools__cron`. " +
    `Use these exact JSON arguments: ${params.argsJson}. ` +
    "Preserve job.sessionTarget and job.sessionKey exactly as provided. " +
...
export async function runDaisyClawCliJson<T>(args: string[], env: NodeJS.ProcessEnv): Promise<T> {
  const childEnv = { ...env };
  ...
  const cliArgs = args.includes("--timeout")
    ? args
    : [...args, "--timeout", String(DAISYCLAW_CLI_GATEWAY_TIMEOUT_MS)];
  const { stdout, stderr } = await execFileAsync(process.execPath, ["daisyclaw.mjs", ...cliArgs], {
    cwd: process.cwd(),
    env: childEnv,
    timeout: DAISYCLAW_CLI_CHILD_TIMEOUT_MS,
    maxBuffer: 1024 * 1024,
  });
  ...
  if (!trimmed) {
    throw new Error(
      [
        `daisyclaw ${args.join(" ")} produced no JSON stdout`,
  ...
  try {
    ...
    throw new Error(
      [
        `daisyclaw ${args.join(" ")} returned invalid JSON`,
        `stdout: ${trimmed}`,
        stderr.trim() ? `stderr: ${stderr.trim()}` : undefined,
...
export async function assertCronJobVisibleViaCli(params: {
  ...
  const cronList = await runDaisyClawCliJson<CronListCliResult>(
    [
      "cron",
      "list",
      "--all",
