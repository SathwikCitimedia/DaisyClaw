...
function shouldLogMcpLoopbackHttp(): boolean {
  return (
    isTruthyEnvValue(process.env.DAISYCLAW_CLI_BACKEND_LOG_OUTPUT) ||
    isTruthyEnvValue(process.env.DAISYCLAW_LIVE_CLI_BACKEND_DEBUG)
  );
}
...
function resolveScopedSessionKey(cfg: DaisyClawConfig, rawSessionKey: string | undefined): string {
...
export function resolveMcpHttpBodyTimeoutMs(): number {
  return readPositiveIntEnv("DAISYCLAW_MCP_LOOPBACK_BODY_TIMEOUT_MS", DEFAULT_MCP_BODY_TIMEOUT_MS);
}
...
export function resolveMcpRequestContext(
  ...
  return {
    ...
    messageProvider:
      normalizeMessageChannel(getHeader(req, "x-daisyclaw-message-channel")) ?? undefined,
    currentChannelId: normalizeOptionalString(getHeader(req, "x-daisyclaw-current-channel-id")),
    currentThreadTs: normalizeOptionalString(getHeader(req, "x-daisyclaw-current-thread-ts")),
    currentMessageId: normalizeOptionalString(getHeader(req, "x-daisyclaw-current-message-id")),
    currentInboundAudio: normalizeMcpCurrentInboundAudio(
      getHeader(req, "x-daisyclaw-current-inbound-audio"),
    ),
    accountId: normalizeOptionalString(getHeader(req, "x-daisyclaw-account-id")),
    inboundEventKind: normalizeMcpInboundEventKind(getHeader(req, "x-daisyclaw-inbound-event-kind")),
    sourceReplyDeliveryMode: normalizeMcpSourceReplyDeliveryMode(
      getHeader(req, "x-daisyclaw-source-reply-delivery-mode"),
    ),
