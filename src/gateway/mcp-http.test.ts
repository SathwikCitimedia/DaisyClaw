...
describe("mcp loopback server", () => {
  it("passes session, account, message channel, and inbound event headers into shared tool resolution", async () => {
    ...
    const response = await sendRaw({
      ...
      headers: jsonHeaders({
        ...
        "x-daisyclaw-account-id": "work",
        "x-daisyclaw-message-channel": "telegram",
        "x-daisyclaw-current-channel-id": "telegram:chat123",
        "x-daisyclaw-current-thread-ts": "42",
        "x-daisyclaw-current-message-id": "reply-message-1",
        "x-daisyclaw-current-inbound-audio": "true",
        "x-daisyclaw-inbound-event-kind": "room_event",
        "x-daisyclaw-source-reply-delivery-mode": "message_tool_only",
      }),
      body: mcpToolsListBody(),
    });
    ...
    const sendToolsList = async (
      ...
      await sendLoopbackToolsList({
        ...
        headers: {
          "x-session-key": "agent:main:telegram:group:chat123",
          "x-daisyclaw-message-channel": "telegram",
          "x-daisyclaw-inbound-event-kind": inboundEventKind,
          ...(sourceReplyDeliveryMode
            ? { "x-daisyclaw-source-reply-delivery-mode": sourceReplyDeliveryMode }
            : {}),
          ...(currentInboundAudio ? { "x-daisyclaw-current-inbound-audio": "true" } : {}),
        },
      });
    ...
    const sendToolsList = async (token?: string) =>
      await sendLoopbackToolsList({
        ...
        headers: {
          ...
          "x-daisyclaw-message-channel": "matrix",
        },
      });
  ...
  it("ignores spoofed owner headers on loopback requests", async () => {
    ...
    const response = await sendLoopbackToolsList({
      ...
      headers: {
        "x-session-key": "agent:main:matrix:dm:test",
        "x-daisyclaw-message-channel": "matrix",
        "x-daisyclaw-sender-is-owner": "true",
      },
    });
  ...
  it("times out stalled request bodies and closes uploads after flushing 408", async () => {
    const previousTimeout = process.env.DAISYCLAW_MCP_LOOPBACK_BODY_TIMEOUT_MS;
    process.env.DAISYCLAW_MCP_LOOPBACK_BODY_TIMEOUT_MS = "20";
    try {
      server = await startMcpLoopbackServer(0);
      const runtime = getActiveMcpLoopbackRuntime();
      if (!runtime) {
      ...
      if (previousTimeout === undefined) {
        delete process.env.DAISYCLAW_MCP_LOOPBACK_BODY_TIMEOUT_MS;
      } else {
        process.env.DAISYCLAW_MCP_LOOPBACK_BODY_TIMEOUT_MS = previousTimeout;
      }
    }
  });
...
describe("createMcpLoopbackServerConfig", () => {
  it("builds a server entry with env-driven headers", () => {
    ...
    expect(config.mcpServers?.daisyclaw?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(config.mcpServers?.daisyclaw?.headers?.Authorization).toBe(
      "Bearer ${DAISYCLAW_MCP_TOKEN}",
    );
    expect(config.mcpServers?.daisyclaw?.headers?.["x-daisyclaw-message-channel"]).toBe(
      "${DAISYCLAW_MCP_MESSAGE_CHANNEL}",
    );
    expect(config.mcpServers?.daisyclaw?.headers?.["x-daisyclaw-current-channel-id"]).toBe(
      "${DAISYCLAW_MCP_CURRENT_CHANNEL_ID}",
    );
    expect(config.mcpServers?.daisyclaw?.headers?.["x-daisyclaw-current-thread-ts"]).toBe(
      "${DAISYCLAW_MCP_CURRENT_THREAD_TS}",
    );
    expect(config.mcpServers?.daisyclaw?.headers?.["x-daisyclaw-current-message-id"]).toBe(
      "${DAISYCLAW_MCP_CURRENT_MESSAGE_ID}",
    );
    expect(config.mcpServers?.daisyclaw?.headers?.["x-daisyclaw-current-inbound-audio"]).toBe(
      "${DAISYCLAW_MCP_CURRENT_INBOUND_AUDIO}",
    );
    expect(config.mcpServers?.daisyclaw?.headers?.["x-daisyclaw-source-reply-delivery-mode"]).toBe(
      "${DAISYCLAW_MCP_SOURCE_REPLY_DELIVERY_MODE}",
    );
    expect(config.mcpServers?.daisyclaw?.headers).not.toHaveProperty("x-daisyclaw-sender-is-owner");
  });
