...
const pluginManifestRegistryMocks = vi.hoisted(() => ({
  ...
  listDaisyClawPluginManifestMetadata: vi.fn(),
}));
...
vi.mock("../plugins/manifest-metadata-scan.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    listDaisyClawPluginManifestMetadata: (
      params?: Parameters<typeof actual.listDaisyClawPluginManifestMetadata>[0],
    ) => {
      pluginManifestRegistryMocks.listDaisyClawPluginManifestMetadata(params);
      return actual.listDaisyClawPluginManifestMetadata(params);
    },
  };
});
...
describe("model-pricing-cache", () => {
  beforeEach(() => {
    ...
    pluginManifestRegistryMocks.listDaisyClawPluginManifestMetadata.mockClear();
  ...
  it("collects manifest-owned web search plugin model refs without a hardcoded plugin list", () => {
    ...
    } as DaisyClawConfig).map((ref) => modelKey(ref.provider, ref.model));
  ...
  it("does not load plugin manifests for pricing when plugins are globally disabled", async () => {
    ...
    expect(pluginManifestRegistryMocks.listDaisyClawPluginManifestMetadata).not.toHaveBeenCalled();
...
function createManifestRecord(overrides: Partial<PluginManifestRecord>): PluginManifestRecord {
  return {
    ...
    manifestPath: "/tmp/plugin/daisyclaw.plugin.json",
    ...overrides,
  };
}
