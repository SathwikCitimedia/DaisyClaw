...
describe("OpenAI-compatible models HTTP API (e2e)", () => {
  it("serves /v1/models when compatibility endpoints are enabled", async () => {
    ...
    expect(json.data?.map((entry) => entry.id)).toContain("daisyclaw");
    expect(json.data?.map((entry) => entry.id)).toContain("daisyclaw/default");
    expect(
      json.data?.every((entry) => typeof entry.id === "string" && entry.id?.startsWith("daisyclaw")),
    ).toBe(true);
  });
  ...
  it("rejects operator scopes that lack read access", async () => {
    const res = await getModels("/v1/models", { "x-daisyclaw-scopes": "operator.approvals" });
    await expectMissingReadScope(res);
  });

  it("rejects requests with no declared operator scopes", async () => {
    const res = await getModels("/v1/models", { "x-daisyclaw-scopes": "" });
    await expectMissingReadScope(res);
  });
