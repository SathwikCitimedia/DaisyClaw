// OpenAI-compatible `/v1/models` HTTP route backed by configured DaisyClaw agents.
...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([DAISYCLAW_MODEL_ID, DAISYCLAW_DEFAULT_MODEL_ID]);
  ids.add(`daisyclaw/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`daisyclaw/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== DAISYCLAW_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
