...
import type { DaisyClawPluginNodeInvokePolicyContext } from "../plugins/types.js";
...
function createApprovalRequestPolicy(params?: {
  ...
  return createDemoPolicy(async (ctx: DaisyClawPluginNodeInvokePolicyContext) => {
...
describe("applyPluginNodeInvokePolicy", () => {
  ...
  it("uses a matching plugin policy when one is registered", async () => {
    setDangerousDemoCommandRegistry([
      createDemoPolicy((ctx: DaisyClawPluginNodeInvokePolicyContext) => ctx.invokeNode()),
    ]);
    const { context, invoke } = createContext();
