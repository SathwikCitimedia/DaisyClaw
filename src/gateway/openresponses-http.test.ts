...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("DAISYCLAW_CONFIG_PATH is required for gateway config tests");
  }
...
describe("OpenResponses HTTP API (e2e)", () => {
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "daisyclaw", input: "hi" }),
      });
      ...
      expect(invalidModelJson.error?.message).toBe(
        "Invalid `model`. Use `daisyclaw` or `daisyclaw/<agentId>`.",
      );
      ...
      const resHeader = await postResponses(
        port,
        { model: "daisyclaw", input: "hi" },
        { "x-daisyclaw-agent-id": "beta" },
      );
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "daisyclaw/beta", input: "hi" });
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resDefaultAlias = await postResponses(port, { model: "daisyclaw/default", input: "hi" });
      ...
      const resChannelHeader = await postResponses(
        port,
        { model: "daisyclaw", input: "hi" },
        { "x-daisyclaw-message-channel": "custom-client-channel" },
      );
      ...
      expect(invalidOverrideJson.error?.message).toBe("Invalid `x-daisyclaw-model`.");
      expect(agentCommand).toHaveBeenCalledTimes(0);
  ...
  it("accepts write-scoped and admin-scoped HTTP callers", async () => {
    ...
    const streamingResponse = await postResponses(
      port,
      { stream: true, model: "daisyclaw", input: "hi" },
      { "x-daisyclaw-scopes": "operator.admin, operator.write" },
    );
