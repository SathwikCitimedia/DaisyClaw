...
function useTempHome(): string {
  const home = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-approval-runtime-"));
  tempHomes.push(home);
...
function execApprovalsPath(home: string): string {
  return path.join(home, ".daisyclaw", "exec-approvals.json");
}

function writeExecApprovalsToken(home: string, token: string): void {
  fs.mkdirSync(path.join(home, ".daisyclaw"), { recursive: true });
  fs.writeFileSync(
...
afterEach(() => {
  ...
  if (originalEnv.DAISYCLAW_HOME === undefined) {
    delete process.env.DAISYCLAW_HOME;
  } else {
    process.env.DAISYCLAW_HOME = originalEnv.DAISYCLAW_HOME;
  }
