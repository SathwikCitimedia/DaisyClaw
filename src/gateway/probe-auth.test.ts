...
function configWithDefaultEnvProvider(gateway: NonNullable<DaisyClawConfig["gateway"]>) {
  return {
    gateway,
    secrets: {
      providers: {
...
function resolveSafeProbeAuth(cfg: DaisyClawConfig, mode: "local" | "remote" = "local") {
...
function expectUnresolvedProbeTokenWarning(cfg: DaisyClawConfig) {
  ...
  it("resolves env SecretRef token via async secret-inputs path", async () => {
    const result = await resolveGatewayProbeAuthSafeWithSecretInputs({
      cfg: configWithDefaultEnvProvider({
        auth: tokenAuthConfig("DAISYCLAW_GATEWAY_TOKEN"),
      }),
      mode: "local",
      env: {
        DAISYCLAW_GATEWAY_TOKEN: "test-token-from-env",
