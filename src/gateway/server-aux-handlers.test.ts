...
function createSnapshot(config: DaisyClawConfig): PreparedSecretsRuntimeSnapshot {
  return {
...
function activateSnapshot(config: DaisyClawConfig) {
  activateSecretsRuntimeSnapshot(createSnapshot(config));
}

function mockResolvedSecrets(config: DaisyClawConfig) {
...
// set DAISYCLAW_SKIP_CHANNELS / DAISYCLAW_SKIP_PROVIDERS at module load. When a
