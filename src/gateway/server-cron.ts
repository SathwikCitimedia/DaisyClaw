...
export function buildGatewayCronService(params: {
  ...
  const cronEnabled = process.env.DAISYCLAW_SKIP_CRON !== "1" && params.cfg.cron?.enabled !== false;

  const findAgentEntry = (cfg: DaisyClawConfig, agentId: string) =>
  ...
  const hasConfiguredAgent = (cfg: DaisyClawConfig, agentId: string) =>
    Boolean(findAgentEntry(cfg, agentId));

  const mergeRuntimeAgentConfig = (runtimeConfig: DaisyClawConfig, requestedAgentId: string) => {
