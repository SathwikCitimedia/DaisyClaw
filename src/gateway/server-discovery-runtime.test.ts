...
const mocks = vi.hoisted(() => ({
  ...
  resolveWideAreaDiscoveryDomain: vi.fn<ResolveWideAreaDiscoveryDomain>(() => "daisyclaw.internal."),
  writeWideAreaGatewayZone: vi.fn<WriteWideAreaGatewayZone>(async () => ({
    changed: true,
    zonePath: "/tmp/daisyclaw.internal.db",
  })),
  formatBonjourInstanceName: vi.fn((name: string) => `${name} (DaisyClaw)`),
  resolveBonjourCliPath: vi.fn(() => "/usr/local/bin/daisyclaw"),
...
async function expectSshPortOmitted(rawPort: string) {
  ...
  process.env.DAISYCLAW_SSH_PORT = rawPort;
...
function startStuckDiscovery(timeoutMs: string) {
  vi.useFakeTimers();
  useDevelopmentDiscoveryEnv();
  process.env.DAISYCLAW_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS = timeoutMs;
...
describe("startGatewayDiscovery", () => {
  ...
  it("starts registered local discovery services with gateway advertisement context", async () => {
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.DAISYCLAW_SSH_PORT = "2222";
    ...
    expect(bonjour.service.advertise).toHaveBeenCalledWith({
      ...
      cliPath: "/usr/local/bin/daisyclaw",
      minimal: false,
    });
  ...
  it("skips local discovery services for truthy DAISYCLAW_DISABLE_BONJOUR values", async () => {
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.DAISYCLAW_DISABLE_BONJOUR = "yes";
  ...
  it("keeps wide-area DNS-SD publishing active when local discovery is off", async () => {
    ...
    const result = await startGatewayDiscovery({
      ...
      wideAreaDiscoveryDomain: "daisyclaw.internal.",
      tailscaleMode: "serve",
      mdnsMode: "off",
      gatewayDiscoveryServices: [service],
      logDiscovery: logs,
    ...
    expect(zoneParams.domain).toBe("daisyclaw.internal.");
    expect(zoneParams.gatewayPort).toBe(18789);
    expect(zoneParams.gatewayDirectReachable).toBe(true);
    expect(zoneParams.displayName).toBe("Lab Mac (DaisyClaw)");
    ...
    expect(logs.info.mock.calls).toEqual([
      ["wide-area DNS-SD updated (daisyclaw.internal. → /tmp/daisyclaw.internal.db)"],
    ]);
    expect(result.bonjourStop).toBeNull();
  });
