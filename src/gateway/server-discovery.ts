...
/** Formats the Bonjour instance name while preserving user-provided DaisyClaw names. */
export function formatBonjourInstanceName(displayName: string) {
  const trimmed = displayName.trim();
  if (!trimmed) {
    return "DaisyClaw";
  }
  if (/daisyclaw/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (DaisyClaw)`;
}
...
export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  ...
  const envPath = env.DAISYCLAW_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "daisyclaw");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.DAISYCLAW_TAILNET_DNS?.trim();
