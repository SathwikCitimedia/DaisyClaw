...
const ORIGINAL_STATE_DIR = process.env.DAISYCLAW_STATE_DIR;
...
describe("gateway agent handler", () => {
  afterEach(() => {
    if (ORIGINAL_STATE_DIR === undefined) {
      ...
      process.env.DAISYCLAW_STATE_DIR = ORIGINAL_STATE_DIR;
    }
    resetDetachedTaskLifecycleRuntimeForTests();
    resetTaskRegistryForTests();
  ...
  it("drops a stale transcript path when a stale session rotates ids", async () => {
    ...
    const staleEntry = {
      ...
      sessionFile: "/tmp/daisyclaw/agents/main/sessions/old-session-id.jsonl",
      updatedAt: 0,
      sessionStartedAt: 0,
    };
  ...
  it("rotates a failed session instead of resuming when its transcript is missing", async () => {
    ...
    const missingTranscriptEntry = {
      ...
      sessionFile: "/tmp/daisyclaw/missing/failed-missing-session-id.jsonl",
      status: "failed",
      updatedAt: now,
  ...
  it.each([
    ...
    async (scenario) => {
      ...
      await withTempDir(
        { prefix: "daisyclaw-gateway-terminal-main-newer-transcript-" },
        async (root) => {
  ...
  it("reuses terminal main sessions when the fresh store row has the transcript marker", async () => {
    ...
    await withTempDir({ prefix: "daisyclaw-gateway-terminal-main-fresh-marker-" }, async (root) => {
      const sessionsDir = `${root}/sessions`;
      await fs.mkdir(sessionsDir, { recursive: true });
      const sessionFile = "terminal-main-session.jsonl";
      const transcriptPath = `${sessionsDir}/${sessionFile}`;
  ...
  it("honors explicit gateway session-id resumes for terminal main rows", async () => {
    ...
    await withTempDir(
      { prefix: "daisyclaw-gateway-terminal-main-explicit-resume-" },
      async (root) => {
        const sessionsDir = `${root}/sessions`;
        await fs.mkdir(sessionsDir, { recursive: true });
        const sessionFile = "terminal-main-session.jsonl";
  ...
  it.each(["heartbeat", "cron"] as const)(
    ...
    async (runKind) => {
      ...
      await withTempDir(
        { prefix: `daisyclaw-gateway-terminal-main-${runKind}-reuse-` },
        async (root) => {
          const sessionsDir = `${root}/sessions`;
          await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it("keeps a failed session reusable when its default transcript exists", async () => {
    ...
    await withTempDir({ prefix: "daisyclaw-gateway-failed-default-session-file-" }, async (root) => {
      const sessionsDir = `${root}/sessions`;
      await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it("keeps a failed session reusable when its relative transcript resolves and exists", async () => {
    ...
    await withTempDir({ prefix: "daisyclaw-gateway-failed-session-file-" }, async (root) => {
      const sessionsDir = `${root}/sessions`;
      await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it("does not create task rows for inter-session completion wakes", async () => {
    ...
    await invokeAgent(
      {
        message: [
          "[Mon 2026-04-06 02:42 GMT+1] <<<BEGIN_DAISYCLAW_INTERNAL_CONTEXT>>>",
          "DaisyClaw runtime context (internal):",
  ...
  it("terminalizes successful async gateway agent runs in the shared task registry", async () => {
    await withTempDir({ prefix: "daisyclaw-gateway-agent-task-" }, async (root) => {
      process.env.DAISYCLAW_STATE_DIR = root;
      resetTaskRegistryForTests();
      primeMainAgentRun();
  ...
  it("tracks plugin SDK subagent agent runs through the subagent registry only", async () => {
    await withTempDir({ prefix: "daisyclaw-gateway-plugin-subagent-task-" }, async (root) => {
      process.env.DAISYCLAW_STATE_DIR = root;
      resetTaskRegistryForTests();
      resetSubagentRegistryForTests({ persist: false });
  ...
  it("keeps plugin SDK subagent runs best-effort when registry persistence fails", async () => {
    await withTempDir(
      { prefix: "daisyclaw-gateway-plugin-subagent-registry-fail-" },
      async (root) => {
        process.env.DAISYCLAW_STATE_DIR = root;
        resetTaskRegistryForTests();
        resetSubagentRegistryForTests({ persist: false });
  ...
  it("terminalizes failed async gateway agent runs in the shared task registry", async () => {
    await withTempDir({ prefix: "daisyclaw-gateway-agent-task-error-" }, async (root) => {
      process.env.DAISYCLAW_STATE_DIR = root;
      resetTaskRegistryForTests();
      primeMainAgentRun();
  ...
  it("preserves aborted async gateway agent runs as timed out", async () => {
    await withTempDir({ prefix: "daisyclaw-gateway-agent-task-aborted-" }, async (root) => {
      process.env.DAISYCLAW_STATE_DIR = root;
      resetTaskRegistryForTests();
      primeMainAgentRun();
  ...
  it("classifies aborted async gateway agent rejections as timed out", async () => {
    await withTempDir({ prefix: "daisyclaw-gateway-agent-task-abort-error-" }, async (root) => {
      process.env.DAISYCLAW_STATE_DIR = root;
      resetTaskRegistryForTests();
      primeMainAgentRun();
  ...
  it("classifies timeout async gateway agent rejections as timed out", async () => {
    await withTempDir({ prefix: "daisyclaw-gateway-agent-task-timeout-error-" }, async (root) => {
      process.env.DAISYCLAW_STATE_DIR = root;
      resetTaskRegistryForTests();
      primeMainAgentRun();
  ...
  it("classifies wrapped rejections after gateway timeout as timed out", async () => {
    await withTempDir(
      { prefix: "daisyclaw-gateway-agent-task-wrapped-timeout-error-" },
      async (root) => {
        process.env.DAISYCLAW_STATE_DIR = root;
        resetTaskRegistryForTests();
        primeMainAgentRun();
  ...
  it("does not hide provider timeout async gateway agent rejections", async () => {
    await withTempDir({ prefix: "daisyclaw-gateway-agent-task-provider-timeout-" }, async (root) => {
      process.env.DAISYCLAW_STATE_DIR = root;
      resetTaskRegistryForTests();
      primeMainAgentRun();
  ...
  it("does not overwrite operator-cancelled async gateway agent tasks after late completion", async () => {
    await withTempDir({ prefix: "daisyclaw-gateway-agent-task-cancelled-" }, async (root) => {
      process.env.DAISYCLAW_STATE_DIR = root;
      resetTaskRegistryForTests();
      primeMainAgentRun();
  ...
  it("dispatches async gateway agent task creation through the detached task runtime seam", async () => {
    await withTempDir({ prefix: "daisyclaw-gateway-agent-seam-" }, async (root) => {
      process.env.DAISYCLAW_STATE_DIR = root;
      resetTaskRegistryForTests();
      primeMainAgentRun();
  ...
  it("logs a swallowed finalize error without blocking the background run", async () => {
    await withTempDir({ prefix: "daisyclaw-gateway-agent-finalize-throw-" }, async (root) => {
      process.env.DAISYCLAW_STATE_DIR = root;
      resetTaskRegistryForTests();
      primeMainAgentRun();
