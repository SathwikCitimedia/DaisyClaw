...
import { isAudioFileName } from "@daisyclaw/media-core/mime";
...
function resolveWebchatPromptCacheKey(params: {
  ...
  const digest = createHash("sha256")
    ...
    .digest("hex")
    .slice(0, 32);
  return `daisyclaw-webchat-${digest}`;
}
...
export function buildOversizedHistoryPlaceholder(message?: unknown): Record<string, unknown> {
  ...
  const rawMetadata =
    message && typeof message === "object"
      ? (message as Record<string, unknown>)["__daisyclaw"]
      : undefined;
  const metadata =
...
async function findSourceReplyTranscriptMirrorByIdempotencyKey(
  ...
  if (found?.message.provider !== "daisyclaw" || found.message.model !== "delivery-mirror") {
    return null;
  }
  return found;
}
...
function sendGlobalAwareNodeChatPayload(params: {
  ...
  const deliveryKeys = resolveGlobalAwareNodeChatDeliveryKeys({
    cfg: params.context.getRuntimeConfig?.() ?? ({} as DaisyClawConfig),
    sessionKey: params.sessionKey,
    agentId: params.agentId,
  });
...
function readChatHistoryMessageId(message: unknown): string | undefined {
  const metadata = asOptionalRecord(asOptionalRecord(message)?.["__daisyclaw"]);
...
export const chatHandlers: GatewayRequestHandlers = {
  ...
  "chat.send": async ({ params, respond, context, client }) => {
      ...
      try {
        await measureDiagnosticsTimelineSpan(
          ...
          async () => {
            ...
            // attachments even when the parent DaisyClaw session model is text-only.
    ...
    try {
      ...
      // See: https://github.com/daisyclaw/daisyclaw/issues/3658
      ...
      if (moodgraphClient.isConfigured()) {
        // Option B: moodgraph does NOT replace the agent. The full DaisyClaw agent
        // (all tools/capabilities) still produces the reply below. We only feed the
        // turn to moodgraph in the background so its stateful emotional engine keeps
        // tracking this session (queryable via /v1/emotions and /v1/coherence).
        // Fire-and-forget: never block, delay, or fail the user's reply on moodgraph.
        // (The warm tone itself comes from the agent persona in SOUL.md.)
        ...
        void (async () => {
          ...
          await moodgraphClient.observe({
            session: {
              provider: resolvedSessionModel.provider,
              model: resolvedSessionModel.model,
              apiKey: providerAuth.apiKey ?? "",
              baseUrl: cfg.models?.providers?.[resolvedSessionModel.provider]?.baseUrl ?? "",
            },
            sessionId: moodSessionId,
            message: parsedMessage,
            systemPrompt:
              process.env["MOODGRAPH_SYSTEM_PROMPT"] ??
              "You are a warm, emotionally attuned companion.",
          });
        })().catch((moodErr) => {
          context.logGateway.warn(`moodgraph observe failed: ${formatForLog(moodErr)}`);
        });
      }
        ...
        .then(async () => {
          ...
          await measureDiagnosticsTimelineSpan(
            ...
            async () => {
              ...
              // through DaisyClaw runtime's SessionManager; this dispatcher only owns live delivery payloads.
                  ...
                  if (
                    ...
                    if (appended.ok) {
                      ...
                      message = {
                        ...
                        timestamp: nowValue,
                        ...(ttsSupplementMarker
                          ? { daisyclawTtsSupplement: ttsSupplementMarker }
                          : {}),
