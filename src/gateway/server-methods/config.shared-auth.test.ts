...
const persistedConfigResultMock = vi.fn((config: DaisyClawConfig) => config);
...
vi.mock("../../config/config.js", async () => {
  ...
  return {
    ...actual,
    createConfigIO: () => ({ configPath: "/tmp/daisyclaw.json" }),
    writeConfigFile: writeConfigFileMock,
    replaceConfigFile: async (params: { nextConfig: DaisyClawConfig; writeOptions?: unknown }) => {
...
function tokenAuthConfig(token: string): DaisyClawConfig {
  return {
    gateway: {
      auth: {
        mode: "token",
...
function hotReloadConfig(): DaisyClawConfig {
  return {
    gateway: {
      reload: {
        mode: "hot",
...
function mockPreviousConfig(config: DaisyClawConfig): void {
...
beforeEach(() => {
  validateConfigObjectWithPluginsMock.mockImplementation((config: DaisyClawConfig) => ({
    ok: true,
    config,
  }));
  prepareSecretsRuntimeSnapshotMock.mockImplementation(
    async ({ config }: { config: DaisyClawConfig }) => ({
      config,
    }),
  );
  restartSentinelMocks.writeRestartSentinel.mockClear();
  persistedConfigResultMock.mockImplementation((config: DaisyClawConfig) => config);
});

describe("config shared auth disconnects", () => {
  it("returns the persisted config from config.set write results", async () => {
    const prevConfig: DaisyClawConfig = {
      gateway: {
        port: 19000,
      },
    };
    const submittedConfig: DaisyClawConfig = {
      gateway: {
        port: 19001,
      },
    };
    const persistedConfig: DaisyClawConfig = {
      gateway: {
        port: 19001,
      },
      meta: {
