...
describe("resolveConfigOpenCommand", () => {
  it("uses open on macOS", () => {
    expect(resolveConfigOpenCommand("/tmp/daisyclaw.json", "darwin")).toEqual({
      command: "open",
      args: ["/tmp/daisyclaw.json"],
    });
  });

  it("uses xdg-open on Linux", () => {
    expect(resolveConfigOpenCommand("/tmp/daisyclaw.json", "linux")).toEqual({
      command: "xdg-open",
      args: ["/tmp/daisyclaw.json"],
    });
  });
...
describe("config.openFile", () => {
  ...
  it("opens the configured file without shell interpolation", async () => {
    process.env.DAISYCLAW_CONFIG_PATH = "/tmp/config $(touch pwned).json";
    execFileMock.mockImplementation((...args: unknown[]) => {
  ...
  it("returns a detailed error and logs details when the opener fails", async () => {
    process.env.DAISYCLAW_CONFIG_PATH = "/tmp/config.json";
