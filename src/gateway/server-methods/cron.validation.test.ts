...
const getRuntimeConfig = vi.hoisted(() =>
  vi.fn<() => DaisyClawConfig>(() => ({}) as DaisyClawConfig),
);
...
function setRuntimeConfig(config: DaisyClawConfig): void {
  getRuntimeConfig.mockReturnValue(config);
}

function pluginEntries(...ids: string[]): DaisyClawConfig["plugins"] {
  return {
    entries: Object.fromEntries(ids.map((id) => [id, { enabled: true }])),
  };
}

function telegramConfig(): DaisyClawConfig {
  return {
    channels: {
      telegram: {
...
function telegramSlackConfig(params: { includeMainSession?: boolean } = {}): DaisyClawConfig {
  return {
...
function msteamsConfig(): DaisyClawConfig {
  return {
    channels: {
      msteams: {
...
function slackSynologyConfig(): DaisyClawConfig {
  return {
    channels: {
      slack: {
        botToken: "xoxb-slack-token",
...
function slackConfig(params: { includeMainSession?: boolean } = {}): DaisyClawConfig {
  return {
    ...(params.includeMainSession ? { session: { mainKey: "main" } } : {}),
    channels: {
      slack: {
...
describe("cron method validation", () => {
  beforeEach(() => {
    getRuntimeConfig.mockReset().mockReturnValue({} as DaisyClawConfig);
