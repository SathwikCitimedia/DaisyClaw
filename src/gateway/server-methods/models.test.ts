...
function requestModelsList(params: {
  ...
  const request = modelsHandlers["models.list"]({
    ...
    context: {
      getRuntimeConfig: () => params.runtimeConfig ?? ({} as DaisyClawConfig),
...
describe("models.list", () => {
  ...
  it("marks legacy OpenAI Codex aliases available through ChatGPT OAuth", async () => {
    await withDaisyClawTestState(
      {
        layout: "state-only",
        prefix: "daisyclaw-models-list-codex-alias-",
        agentEnv: "main",
      },
      async (state) => {
  ...
  it("does not mark catalog rows available from expired OAuth profiles", async () => {
    await withDaisyClawTestState(
      {
        layout: "state-only",
        prefix: "daisyclaw-models-list-expired-profile-",
        agentEnv: "main",
      },
      async (state) => {
        await state.writeAuthProfiles({
  ...
  it("marks env SecretRef-backed auth profiles available", async () => {
    await withDaisyClawTestState(
      {
        layout: "state-only",
        prefix: "daisyclaw-models-list-env-profile-",
        agentEnv: "main",
        env: {
  ...
  it("keeps non-env SecretRef-backed auth profile availability unknown", async () => {
    await withDaisyClawTestState(
      {
        layout: "state-only",
        prefix: "daisyclaw-models-list-file-profile-",
        agentEnv: "main",
      },
      async (state) => {
        await state.writeAuthProfiles({
