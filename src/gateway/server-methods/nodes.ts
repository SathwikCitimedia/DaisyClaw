...
import { normalizeUniqueTrimmedStringList } from "@daisyclaw/normalization-core/string-normalization";
import {
  type ConnectParams,
  ErrorCodes,
  errorShape,
...
export async function maybeWakeNodeWithApns(
  nodeId: string,
  opts?: { force?: boolean; wakeReason?: string; cfg?: DaisyClawConfig },
  ...
  try {
    ...
    if (registration.transport === "relay") {
      ...
      result = await sendApnsAlert({
        registration,
        nodeId,
        title: "DaisyClaw needs a quick reopen",
        body: "Tap to reopen DaisyClaw and restore the node connection.",
        relayConfig: relay.relayConfig,
      });
    } else {
      const auth = await resolveDirectNodePushConfig();
