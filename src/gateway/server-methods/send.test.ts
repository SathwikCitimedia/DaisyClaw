...
const TEST_AGENT_WORKSPACE = "/tmp/daisyclaw-test-workspace";
...
describe("gateway send mirroring", () => {
  ...
  it("maps gateway asVoice sends onto outbound audioAsVoice payloads", async () => {
    ...
    const { respond } = await runSend({
      to: "channel:C1",
      message: "voice note",
      mediaUrl: "file:///tmp/daisyclaw-voice.ogg",
      asVoice: true,
      channel: "slack",
    ...
    expect(deliveryCall()?.payloads?.[0]?.mediaUrl).toBe("file:///tmp/daisyclaw-voice.ogg");
