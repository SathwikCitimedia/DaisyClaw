  ...
  return {
    ...
    movePathWithCopyFallback: async (
      ...
      if (
        ...
        options.from.includes(".daisyclaw-install-stage-") &&
...
let tempDirs: string[] = [];
let testStates: DaisyClawTestState[] = [];
...
async function makeHarness(): Promise<{
  ...
  const testState = await createDaisyClawTestState({
    layout: "state-only",
    prefix: "daisyclaw-skill-upload-handler-",
  });
