...
vi.mock("../../skills/security/clawhub-verdicts.js", () => ({
  ...
  fetchDaisyClawSkillSecurityVerdicts: vi.fn(),
}));
...
describe("skills proposal gateway handlers", () => {
  beforeEach(async () => {
    testState = await createDaisyClawTestState({
      layout: "state-only",
      prefix: "daisyclaw-skills-proposals-gateway-state-",
    });
    ...
    mocks.workspaceDir = await tempDirs.make("daisyclaw-skills-proposals-gateway-");
  ...
  it("scopes list and inspect to the resolved agent workspace", async () => {
    ...
    const secondWorkspaceDir = await tempDirs.make("daisyclaw-skills-proposals-gateway-second-");
