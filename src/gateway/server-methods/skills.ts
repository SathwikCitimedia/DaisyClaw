...
import {
  ...
  fetchDaisyClawSkillSecurityVerdicts,
...
export const skillsHandlers: GatewayRequestHandlers = {
  ...
  "skills.securityVerdicts": async ({ params, respond, context }) => {
    ...
    try {
      ...
      if (targets.length === 0) {
        respond(true, { schema: "daisyclaw.skills.security-verdicts.v1", items: [] }, undefined);
        return;
      }
      const items = await fetchDaisyClawSkillSecurityVerdicts(targets);
      respond(true, { schema: "daisyclaw.skills.security-verdicts.v1", items }, undefined);
    } catch (err) {
