...
const startManagedServiceUpdateHandoffMock = vi.fn(async () => ({
  ...
  pid: 12345,
  command: "daisyclaw update --yes --timeout 1800",
  logPath: "/tmp/daisyclaw-update-run-handoff/handoff.log",
}));
...
vi.mock("../../infra/daisyclaw-root.js", async () => {
  const actual = await vi.importActual<typeof import("../../infra/daisyclaw-root.js")>(
    "../../infra/daisyclaw-root.js",
  );
  return {
    ...actual,
    resolveDaisyClawPackageRoot: async () => "/tmp/daisyclaw",
  };
});
...
vi.mock("../../infra/update-managed-service-handoff.js", () => ({
  ...
  formatManagedServiceUpdateCommand: (params?: {
    ...
    params?.timeoutMs
      ? `daisyclaw update --yes --timeout ${Math.ceil(params.timeoutMs / 1000)}`
      : "daisyclaw update --yes",
  buildManagedServiceHandoffUnavailableMessage: (command: string) =>
    [
      "DaisyClaw updates cannot safely run inside the live gateway process without a managed-service handoff.",
...
function mockGlobalInstallSurface() {
  resolveUpdateInstallSurfaceMock.mockResolvedValueOnce({
    kind: "global",
    mode: "npm",
    root: "/tmp/daisyclaw-global",
    packageRoot: "/tmp/daisyclaw-global",
  });
}
...
describe("update.run restart scheduling", () => {
  ...
  it("hands managed package updates to the CLI path instead of running them in-process", async () => {
    ...
    const payload = await withProcessEnv({ DAISYCLAW_LAUNCHD_LABEL: "ai.daisyclaw.gateway" }, () =>
  ...
  it("keeps a startup grace before restarting after systemd handoff spawn", async () => {
    ...
    await withProcessEnv({ DAISYCLAW_SYSTEMD_UNIT: "daisyclaw-gateway.service" }, () =>
  ...
  it("starts managed package handoff when the gateway cwd is unavailable", async () => {
    ...
    try {
      await withProcessEnv({ DAISYCLAW_LAUNCHD_LABEL: "ai.daisyclaw.gateway" }, () =>
        invokeUpdateRun({}),
      );
    } finally {
      cwdSpy.mockRestore();
  ...
  it("hands supervised git/dev updates to the CLI path instead of rebuilding live dist in-process", async () => {
    detectRespawnSupervisorMock.mockReturnValueOnce("launchd");
    mockGitInstallSurface("/tmp/daisyclaw-git");
    const payload = await withProcessEnv({ DAISYCLAW_LAUNCHD_LABEL: "ai.daisyclaw.gateway" }, () =>
      captureUpdateRunPayload(),
    );
  ...
  it("returns a safe command when package updates cannot be handed off", async () => {
    ...
    expect(payload?.handoff).toEqual({
      ...
      message:
        "DaisyClaw updates cannot safely run inside the live gateway process without a managed-service handoff.\n" +
        "Run `daisyclaw update --yes --timeout 1800` from a shell outside the gateway service, or restart/update from the host UI.",
    });
  });
