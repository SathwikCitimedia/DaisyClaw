...
const loadDaisyClawPlugins = vi.hoisted(() => vi.fn());
...
function getLastPluginLoadOptions(): Record<string, unknown> {
  return requireRecord(
    getLastMockFirstArg(loadDaisyClawPlugins, "plugin load"),
    "plugin load options",
  );
}
...
function getLastPluginLoadLogger(): {
  ...
  const call = getLastMockFirstArg(loadDaisyClawPlugins, "plugin load") as
    | {
        logger?: {
          info: (message: string) => void;
          warn: (message: string) => void;
...
async function createSubagentRuntime(
  ...
  loadDaisyClawPlugins.mockReturnValue(createRegistry([]));
...
beforeEach(() => {
  ...
  loadDaisyClawPlugins.mockReset();
...
describe("loadGatewayPlugins", () => {
  test("logs plugin errors with details", () => {
    ...
    loadDaisyClawPlugins.mockReturnValue(createRegistry(diagnostics));
  ...
  test("pins the initial startup channel registry against later active-registry churn", () => {
    ...
    loadDaisyClawPlugins.mockReturnValue(startupRegistry);
  ...
  test("treats an empty startup scope as no plugin load instead of an unscoped load", () => {
    ...
    expect(loadDaisyClawPlugins).not.toHaveBeenCalled();
  ...
  test("includes docs guidance when a plugin fallback override is not trusted", async () => {
    ...
    await expect(
      ...
      'plugin "voice-call" is not trusted for fallback provider/model override requests. See https://docs.daisyclaw.ai/plugins/sdk-runtime#api-runtime-subagent and search for: plugins.entries.<id>.subagent.allowModelOverride',
    );
  });
  ...
  test("runs registry hook before priming configured bindings", () => {
    ...
    loadDaisyClawPlugins.mockReturnValue(pluginRegistry);
