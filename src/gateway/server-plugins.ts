...
import { clearActivatedPluginRuntimeState, loadDaisyClawPlugins } from "../plugins/loader.js";
...
const FALLBACK_GATEWAY_CONTEXT_STATE_KEY: unique symbol = Symbol.for(
  "daisyclaw.fallbackGatewayContextState",
);
...
const PLUGIN_SUBAGENT_POLICY_STATE_KEY: unique symbol = Symbol.for(
  "daisyclaw.pluginSubagentOverridePolicyState",
);
...
export function setPluginSubagentOverridePolicies(cfg: DaisyClawConfig): void {
  ...
  if (!policy?.allowModelOverride) {
    return {
      ...
      reason:
        ...
        "See https://docs.daisyclaw.ai/plugins/sdk-runtime#api-runtime-subagent and search for: " +
...
export function loadGatewayPlugins(params: {
  ...
  const pluginRegistry = loadDaisyClawPlugins({
    config: resolvedConfig,
