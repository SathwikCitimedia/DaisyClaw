...
type GatewayReloadHandlerParams = {
  ...
  createHealthMonitor: (config: DaisyClawConfig) => ChannelHealthMonitor | null;
...
> & {
  ...
  resolveSharedGatewaySessionGenerationForConfig: (config: DaisyClawConfig) => string | undefined;
...
export function createGatewayReloadHandlers(params: GatewayReloadHandlerParams) {
  ...
  const markActiveMainSessionsForRestart = async (nextConfig: DaisyClawConfig, reason: string) => {
  ...
  const applyHotReload = async (plan: GatewayReloadPlan, nextConfig: DaisyClawConfig) => {
            ...
            await startGmailWatcherWithLogs({
              ...
              onSkipped: () =>
                params.logHooks.info(
                  "skipping gmail watcher restart (DAISYCLAW_SKIP_GMAIL_WATCHER=1)",
                ),
            });
          }
        }
    ...
    if (channelsToRestart.size > 0) {
      if (shouldSkipChannelRestart()) {
        params.logChannels.info(
          "skipping channel reload (DAISYCLAW_SKIP_CHANNELS=1 or DAISYCLAW_SKIP_PROVIDERS=1)",
        );
      } else {
        if (!plan.reloadPlugins) {
          await waitForActiveWorkBeforeChannelReload(channelsToRestart, nextConfig);
  ...
  const requestGatewayRestart = (plan: GatewayReloadPlan, nextConfig: DaisyClawConfig): boolean => {
    setGatewaySigusr1RestartPolicy({ allowExternal: isRestartEnabled(nextConfig) });
