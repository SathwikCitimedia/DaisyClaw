...
const hoisted = vi.hoisted(() => ({
  loadConfigMock: vi.fn<() => DaisyClawConfig>(),
...
function mockCombinedSessionStore(cfg: DaisyClawConfig, store: Record<string, unknown>) {
...
describe("resolveSessionKeyForRun", () => {
  ...
  it("does not overwrite active run context when a scoped lookup finds another agent store entry", () => {
    ...
    hoisted.loadCombinedSessionStoreForGatewayMock.mockImplementation(
      (_cfg: DaisyClawConfig, opts?: { agentId?: string }) => ({
        storePath: "(multiple)",
        store:
          opts?.agentId === "main"
            ? {
