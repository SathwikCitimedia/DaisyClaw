...
import type { ConfigFileSnapshot, ModelDefinitionConfig, DaisyClawConfig } from "../config/types.js";
import type { PluginMetadataSnapshot } from "../plugins/plugin-metadata-snapshot.js";
import { buildTestConfigSnapshot } from "./test-helpers.config-snapshots.js";

const applyPluginAutoEnable = vi.hoisted(() =>
  vi.fn((params: { config: DaisyClawConfig }) => ({
...
vi.mock("../config/runtime-overrides.js", () => ({
  applyConfigOverrides: vi.fn((config: DaisyClawConfig) => config),
}));
...
vi.mock("../config/plugin-auto-enable.js", () => ({
  applyPluginAutoEnable: (params: { config: DaisyClawConfig }) => applyPluginAutoEnable(params),
}));
...
const configPath = "/tmp/daisyclaw-startup-recovery.json";
...
function buildRuntimeSnapshot(
  ...
  runtimeConfig: DaisyClawConfig = sourceConfig,
): ConfigFileSnapshot {
  return {
...
function expectPluginAutoEnableFor(config: DaisyClawConfig) {
...
function mockRuntimeAutoEnable(config: DaisyClawConfig) {
...
function buildInvalidConfigSnapshot(params: {
  ...
  return buildTestConfigSnapshot({
    ...
    valid: false,
    config: params.config ?? (params.rawConfig as DaisyClawConfig),
...
describe("gateway startup config validation", () => {
  ...
  it("does not suggest doctor repair for plugin packaging compiled-output failures", async () => {
    ...
    const invalidSnapshot = buildInvalidConfigSnapshot({
      rawConfig,
      config: rawConfig as DaisyClawConfig,
      issues: [
        {
    ...
    await start.catch((error: unknown) => {
      expect(String(error)).not.toContain("daisyclaw doctor --fix");
    });
  });
  ...
  it("keeps doctor repair guidance for mixed plugin packaging and core invalidity", async () => {
    ...
    const invalidSnapshot = buildInvalidConfigSnapshot({
      rawConfig,
      config: rawConfig as unknown as DaisyClawConfig,
      issues: [
        {
          path: "plugins.slots.memory",
          message: "plugin not found: source-only-pack",
    ...
    await expectStartupRejects('Run "daisyclaw doctor --fix" to repair, then retry.');
  });
      ...
      issues: [
        {
          ...
          message:
            "plugin feishu: plugin requires DaisyClaw >=2026.4.23, but this host is 2026.4.22; skipping load",
        },
      ],
    });
    vi.mocked(configIo.readConfigFileSnapshot).mockResolvedValueOnce(invalidSnapshot);
