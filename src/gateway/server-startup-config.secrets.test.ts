...
type GatewayStartupStateEmitterMock = ReturnType<
  typeof vi.fn<(code: string, message: string, cfg: DaisyClawConfig) => void>
>;

const RESOLVED_GATEWAY_TOKEN = "resolved-gateway-token";

function gatewayTokenConfig(config: DaisyClawConfig): DaisyClawConfig {
  return {
    ...config,
    gateway: {
      ...config.gateway,
...
function asConfig(value: unknown): DaisyClawConfig {
  return value as DaisyClawConfig;
}

function buildSnapshot(config: DaisyClawConfig): ConfigFileSnapshot {
  const raw = `${JSON.stringify(config, null, 2)}\n`;
  return buildTestConfigSnapshot({
    path: "/tmp/daisyclaw-startup-secrets-test.json",
    exists: true,
    raw,
    parsed: config,
    valid: true,
...
function preparedSnapshot(config: DaisyClawConfig): PreparedSecretsRuntimeSnapshot {
  return {
    sourceConfig: config,
    config,
    authStores: [],
...
function runtimeSecretsActivatorOptionsForTest() {
  return {
    ...
    emitStateEvent: vi.fn<(code: string, message: string, cfg: DaisyClawConfig) => void>(),
  };
}
...
function installDiagnosticsTimelineEnv() {
  const root = mkdtempSync(path.join(tmpdir(), "daisyclaw-startup-secrets-timeline-"));
  const timelinePath = path.join(root, "timeline.jsonl");
  const previousDiagnostics = process.env.DAISYCLAW_DIAGNOSTICS;
  const previousTimelinePath = process.env.DAISYCLAW_DIAGNOSTICS_TIMELINE_PATH;
  process.env.DAISYCLAW_DIAGNOSTICS = "timeline";
  process.env.DAISYCLAW_DIAGNOSTICS_TIMELINE_PATH = timelinePath;

  return {
    timelinePath,
    cleanup: () => {
      if (previousDiagnostics === undefined) {
        delete process.env.DAISYCLAW_DIAGNOSTICS;
      } else {
        process.env.DAISYCLAW_DIAGNOSTICS = previousDiagnostics;
      }
      if (previousTimelinePath === undefined) {
        delete process.env.DAISYCLAW_DIAGNOSTICS_TIMELINE_PATH;
      } else {
        process.env.DAISYCLAW_DIAGNOSTICS_TIMELINE_PATH = previousTimelinePath;
      }
...
async function activateImportedStartupConfig(config: DaisyClawConfig) {
...
describe("gateway startup config secret preflight", () => {
  const previousSkipChannels = process.env.DAISYCLAW_SKIP_CHANNELS;
  const previousSkipProviders = process.env.DAISYCLAW_SKIP_PROVIDERS;

  afterEach(() => {
    if (previousSkipChannels === undefined) {
      delete process.env.DAISYCLAW_SKIP_CHANNELS;
    } else {
      process.env.DAISYCLAW_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.DAISYCLAW_SKIP_PROVIDERS;
    } else {
      process.env.DAISYCLAW_SKIP_PROVIDERS = previousSkipProviders;
    }
  });
  ...
  it("activates no-SecretRef startup config without importing the full secrets runtime", async () => {
    vi.resetModules();
    const agentDir = mkdtempSync(path.join(tmpdir(), "daisyclaw-startup-fast-path-"));
    const runtimeImport = vi.fn();
    const prepareRuntimeSecretsSnapshot = vi.fn(async ({ config }) => preparedSnapshot(config));
    const activateRuntimeSecretsSnapshot = vi.fn();
  ...
  it("keeps the full secrets runtime path when startup config has a SecretRef", async () => {
    const harness = createGatewayStartupSecretsRuntimeHarness("daisyclaw-startup-secret-ref-");
  ...
  it("keeps the full secrets runtime path when auth profile files are present", async () => {
    const harness = createGatewayStartupSecretsRuntimeHarness("daisyclaw-startup-auth-store-");
    writeFileSync(
