...
export async function prepareGatewayStartupConfig(params: {
  ...
  const canReusePreflightPreparedSnapshot = (config: DaisyClawConfig): boolean =>
    Boolean(
      preflightPrepared &&
  ...
  const activateStartupSecrets = async (config: DaisyClawConfig) => {
...
function hasActiveGatewayAuthSecretRef(config: DaisyClawConfig): boolean {
...
function pruneSkippedStartupSecretSurfaces(config: DaisyClawConfig): DaisyClawConfig {
  const skipChannels =
...
function assertRuntimeGatewayAuthNotKnownWeak(config: DaisyClawConfig): void {
