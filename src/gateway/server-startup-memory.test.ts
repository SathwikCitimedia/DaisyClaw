...
async function startMemoryBackendForTest(cfg: DaisyClawConfig) {
  const log = createGatewayLogMock();
  await startGatewayMemoryBackend({ cfg, log });
  return log;
}

async function startQmdBackendWithManager(cfg: DaisyClawConfig) {
...
function expectQmdManagerRequests(cfg: DaisyClawConfig, agentIds: string[]) {
