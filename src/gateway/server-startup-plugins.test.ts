...
const resolveDaisyClawPackageRootSync = vi.hoisted(() => vi.fn((_params: unknown) => "/package"));
...
vi.mock("../infra/daisyclaw-root.js", () => ({
  resolveDaisyClawPackageRootSync: (params: unknown) => resolveDaisyClawPackageRootSync(params),
}));
...
function slackConfig(): DaisyClawConfig {
  return {
    channels: {
...
describe("prepareGatewayPluginBootstrap startup plugins", () => {
  beforeEach(() => {
    ...
    resolveDaisyClawPackageRootSync.mockClear().mockReturnValue("/package");
...
describe("warnUnregisteredConfiguredMemoryEmbeddingProviders", () => {
  ...
  function customOllamaConfig(source: "provider" | "fallback" = "provider"): DaisyClawConfig {
    const memorySearch =
      source === "provider"
