...
const hoisted = vi.hoisted(() => {
  ...
  const ensureDaisyClawModelsJson = vi.fn(async () => {});
...
vi.mock("../config/paths.js", async () => {
  ...
  return {
    ...actual,
    STATE_DIR: "/tmp/daisyclaw-state",
    resolveConfigPath: vi.fn(() => "/tmp/daisyclaw-state/daisyclaw.json"),
    resolveGatewayPort: vi.fn(() => 18789),
    resolveStateDir: vi.fn(() => "/tmp/daisyclaw-state"),
  };
});
...
vi.mock("../agents/models-config.js", () => ({
  ensureDaisyClawModelsJson: hoisted.ensureDaisyClawModelsJson,
}));
...
describe("startGatewayPostAttachRuntime", () => {
  beforeEach(() => {
    vi.stubEnv("DAISYCLAW_SKIP_CHANNELS", "0");
    vi.stubEnv("DAISYCLAW_SKIP_PROVIDERS", "0");
    ...
    hoisted.ensureDaisyClawModelsJson.mockReset();
    hoisted.ensureDaisyClawModelsJson.mockResolvedValue(undefined);
  ...
  it("skips heavy restart sentinel refresh when no sentinel file exists", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-no-sentinel-"));
  ...
  it("refreshes the restart sentinel when the sentinel file exists", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-sentinel-"));
  ...
  it("expands tilde-based restart sentinel state paths", async () => {
    const osHome = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-home-"));
    try {
      const daisyclawHome = path.join(osHome, "daisyclaw-home");
      const stateDirFromHome = path.join(daisyclawHome, ".daisyclaw");
      ...
      expect(
        await testing.hasRestartSentinelFileFast({
          HOME: osHome,
          DAISYCLAW_HOME: "~/daisyclaw-home",
        } as NodeJS.ProcessEnv),
      ).toBe(true);

      const backslashStateDir = path.resolve(`${osHome}\\daisyclaw-state`);
      ...
      expect(
        await testing.hasRestartSentinelFileFast({
          HOME: osHome,
          DAISYCLAW_STATE_DIR: "~\\daisyclaw-state",
        } as NodeJS.ProcessEnv),
      ).toBe(true);
    } finally {
  ...
  it("avoids sync filesystem probes while checking restart sentinel presence", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-async-sentinel-"));
    try {
      fs.writeFileSync(path.join(stateDir, "restart-sentinel.json"), "{}\n");
  ...
  it("defers qmd memory backend startup refresh until the idle delay elapses", async () => {
    ...
    try {
      await startGatewaySidecars({
        ...
        defaultWorkspaceDir: "/tmp/daisyclaw-workspace",
        deps: {} as never,
  ...
  it("cleans startup session locks with bounded concurrency", async () => {
    ...
    let maxActive = 0;
    const cleanedLock = {
      lockPath: "/tmp/daisyclaw-state/agents/main/sessions/a.jsonl.lock",
      pid: null,
      pidAlive: false,
      createdAt: null,
      ageMs: null,
  ...
  it("starts channels when channel startup is enabled", async () => {
    await withEnvAsync(
      {
        DAISYCLAW_SKIP_CHANNELS: undefined,
        DAISYCLAW_SKIP_PROVIDERS: undefined,
      },
      async () => {
  ...
  it("starts and reports plugin services after channel startup completes", async () => {
    await withEnvAsync(
      { DAISYCLAW_SKIP_CHANNELS: undefined, DAISYCLAW_SKIP_PROVIDERS: undefined },
      async () => {
  ...
  it("emits a startup trace span when channel startup is skipped", async () => {
    ...
    await withEnvAsync(
      { DAISYCLAW_SKIP_CHANNELS: "1", DAISYCLAW_SKIP_PROVIDERS: undefined },
      async () => {
        await startGatewaySidecars({
          cfg: { hooks: { internal: { enabled: false } } } as never,
          pluginRegistry: createPostAttachParams().pluginRegistry,
    ...
    expect(logChannels.info).toHaveBeenCalledWith(
      "skipping channel start (DAISYCLAW_SKIP_CHANNELS=1 or DAISYCLAW_SKIP_PROVIDERS=1)",
    );
  });
