...
import type { loadDaisyClawPlugins } from "../plugins/loader.js";
...
const SKIP_STARTUP_MODEL_PREWARM_ENV = "DAISYCLAW_SKIP_STARTUP_MODEL_PREWARM";
...
function resolveGatewayMemoryStartupPolicy(cfg: DaisyClawConfig): GatewayMemoryStartupPolicy {
...
async function resolveRestartSentinelPathFast(
  ...
  const resolveHome = () => {
    const explicitHome = normalizePathEnv(env.DAISYCLAW_HOME);
    if (explicitHome) {
  ...
  const override = normalizePathEnv(env.DAISYCLAW_STATE_DIR);
  if (override) {
    return path.join(resolveUserPath(override), RESTART_SENTINEL_FILENAME);
  }
  const home = resolveHome();
  const newStateDir = path.join(home, ".daisyclaw");
  if (env.DAISYCLAW_TEST_FAST === "1" || (await pathExists(newStateDir))) {
...
function hasGatewayStartHooks(pluginRegistry: ReturnType<typeof loadDaisyClawPlugins>): boolean {
...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  const { normalizeProviderId } = await import("@daisyclaw/model-catalog-core/provider-id");
  if (
  ...
  const { ensureDaisyClawModelsJson } = await import("../agents/models-config.js");
  const agentDir = resolveDefaultAgentDir(params.cfg);
  const workspaceDir =
    params.workspaceDir ?? resolveAgentWorkspaceDir(params.cfg, resolveDefaultAgentId(params.cfg));
  try {
    await ensureDaisyClawModelsJson(params.cfg, agentDir, {
      workspaceDir,
...
export async function startGatewaySidecars(params: {
  cfg: DaisyClawConfig;
  pluginRegistry: ReturnType<typeof loadDaisyClawPlugins>;
      ...
      await measureStartup(params.startupTrace, "sidecars.acp.identity-reconcile", async () => {
        const [{ getAcpSessionManager }, { ACP_SESSION_IDENTITY_RENDERER_VERSION }] =
          await Promise.all([
            ...
            import("@daisyclaw/acp-core/runtime/session-identifiers"),
          ]);
...
export async function startGatewayPostAttachRuntime(
  params: {
    ...
    gatewayPluginConfigAtStart: DaisyClawConfig;
    pluginRegistry: ReturnType<typeof loadDaisyClawPlugins>;
    defaultWorkspaceDir: string;
    deps: CliDeps;
    startChannels: () => Promise<void>;
