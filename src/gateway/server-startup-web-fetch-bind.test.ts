...
async function writeConfig(config: DaisyClawConfig): Promise<void> {
...
describe("gateway startup web fetch config", () => {
  it("binds HTTP with credential-free tools.web.fetch config without fetch provider discovery", async () => {
    const previousMinimal = process.env.DAISYCLAW_TEST_MINIMAL_GATEWAY;
    process.env.DAISYCLAW_TEST_MINIMAL_GATEWAY = "0";
    ...
    try {
      ...
      if (previousMinimal === undefined) {
        ...
        process.env.DAISYCLAW_TEST_MINIMAL_GATEWAY = previousMinimal;
      }
    }
  });
});
