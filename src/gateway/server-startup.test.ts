...
const ensureDaisyClawModelsJsonMock = vi.fn<
  (
    config: unknown,
    agentDir: unknown,
    options?: unknown,
...
vi.mock("../agents/models-config.js", () => ({
  ensureDaisyClawModelsJson: (config: unknown, agentDir: unknown, options?: unknown) =>
    ensureDaisyClawModelsJsonMock(config, agentDir, options),
}));
...
function expectModelsJsonPrewarmCall(cfg: DaisyClawConfig) {
  expect(ensureDaisyClawModelsJsonMock).toHaveBeenCalledTimes(1);
  const [calledConfig, agentDir, options] = ensureDaisyClawModelsJsonMock.mock.calls.at(0) ?? [];
...
describe("gateway startup primary model warmup", () => {
  ...
  beforeEach(() => {
    ensureDaisyClawModelsJsonMock.mockClear();
  ...
  it("honors the startup model prewarm skip env", () => {
    ...
    expect(
      shouldSkipStartupModelPrewarm({
        DAISYCLAW_SKIP_STARTUP_MODEL_PREWARM: "1",
      }),
    ).toBe(true);
    expect(
      shouldSkipStartupModelPrewarm({
        DAISYCLAW_SKIP_STARTUP_MODEL_PREWARM: "true",
      }),
    ).toBe(true);
  });
  ...
  it("warns when scoped models.json preparation fails", async () => {
    ensureDaisyClawModelsJsonMock.mockRejectedValueOnce(new Error("models write failed"));
    const warn = vi.fn();
