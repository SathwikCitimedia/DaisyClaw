...
describe("startGatewayTailscaleExposure preserveFunnel", () => {
  ...
  it("passes serviceName through to Tailscale Serve setup and cleanup", async () => {
    ...
    expect(mocks.enableTailscaleServe).toHaveBeenCalledWith(18789, undefined, "svc:daisyclaw");
    expect(logTailscale.info).toHaveBeenCalledWith(
      "serve enabled for svc:daisyclaw: https://daisyclaw.tailnet.ts.net/ (WS via wss://daisyclaw.tailnet.ts.net)",
    );

    await cleanup?.();

    expect(mocks.disableTailscaleServe).toHaveBeenCalledWith(undefined, "svc:daisyclaw");
  });
