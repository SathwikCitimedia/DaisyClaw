...
function resolveGatewayTokenOrEnv(): string {
  ...
  if (typeof token !== "string") {
    throw new Error("expected gateway token in test state or DAISYCLAW_GATEWAY_TOKEN");
  }
  return token;
}
...
async function startRateLimitedTokenServerWithPairedDeviceToken() {
  ...
  const deviceIdentityPath = nextAuthIdentityPath("daisyclaw-auth-rate-limit");
  try {
...
async function ensurePairedDeviceTokenForCurrentIdentity(ws: WebSocket): Promise<{
  ...
  const deviceIdentityPath = nextAuthIdentityPath("daisyclaw-auth-device");
