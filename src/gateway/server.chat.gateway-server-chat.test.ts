...
describe("gateway server chat", () => {
  ...
  const withMainSessionStore = async <T>(run: (dir: string) => Promise<T>): Promise<T> => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-gw-"));
    try {
  ...
  test("sessions.send accepts dashboard messages for existing sessions", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-sessions-send-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("sessions.send creates a configured agent main session before sending", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-sessions-send-agent-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
  ...
  test("sessions.steer accepts dashboard follow-up messages for existing sessions", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-sessions-steer-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("sessions.abort stops active dashboard runs", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-sessions-abort-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("sessions.abort resolves active runs by runId without a caller session key", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-sessions-abort-runid-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("handles chat send and history flows", async () => {
    ...
    try {
      ...
      const sendPolicyDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-gw-"));
      ...
      const agentBlockedDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-gw-"));
      ...
      const historyDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-gw-"));
  ...
  test("chat.history mirrors current-session message tool sends before NO_REPLY", async () => {
    ...
    expect(
      historyMessages.some((message) => {
        ...
        const entry = message as { role?: unknown; daisyclawMessageToolMirror?: unknown };
        return entry.role === "assistant" && Boolean(entry.daisyclawMessageToolMirror);
      }),
    ).toBe(true);
  });
    ...
    expect(
      historyMessages.some(
        (message) =>
          ...
          Boolean((message as { daisyclawMessageToolMirror?: unknown }).daisyclawMessageToolMirror),
      ),
    ).toBe(true);
    expect(historyMessages).not.toContainEqual(
      expect.objectContaining({ provider: "daisyclaw", model: "delivery-mirror" }),
    );
  });
  ...
  test("chat.history persists assistant image data URLs as managed image blocks", async () => {
    await withMainSessionStore(async (dir) => {
      ...
      process.env.DAISYCLAW_STATE_DIR = dir;
      const pngB64 =
