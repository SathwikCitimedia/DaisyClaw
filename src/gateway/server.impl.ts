...
function createGatewayStartupTrace() {
  const logEnabled = isTruthyEnvValue(process.env.DAISYCLAW_GATEWAY_STARTUP_TRACE);
  let timelineConfig: DaisyClawConfig | undefined;
  ...
  const eventLoopTimelineEnabled = () =>
    ...
    isTruthyEnvValue(process.env.DAISYCLAW_DIAGNOSTICS_EVENT_LOOP);
  ...
  return {
    setConfig(config: DaisyClawConfig) {
...
function formatRuntimeGatewayAuthTokenWarning(): string {
  ...
  if (!isNixMode) {
    return `${base} Persist one with \`daisyclaw config set gateway.auth.mode token\` and \`daisyclaw config set gateway.auth.token <token>\`.`;
  }
  return [
    base,
    "In Nix mode, set gateway.auth.token in your Nix-managed DaisyClaw config and rebuild.",
    "For the first-party Nix flow, see https://github.com/daisyclaw/nix-daisyclaw#quick-start and https://docs.daisyclaw.ai/install/nix.",
  ].join(" ");
}
...
export type GatewayServerOptions = {
   ...
   * reparsing daisyclaw.json during server startup.
   */
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    isVitestRuntimeEnv() && process.env.DAISYCLAW_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.DAISYCLAW_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "DAISYCLAW_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "DAISYCLAW_RAW_STREAM_PATH",
  ...
  let cfgAtStart: DaisyClawConfig;
  ...
  const resolveSharedGatewaySessionGenerationForConfig = (config: DaisyClawConfig) =>
    ...
    const gatewayRequestContext = await startupTrace.measure(
      ...
      async () => {
        ...
        return createGatewayRequestContext({
          ...
          clients,
          enforceSharedGatewayAuthGenerationForConfigWrite: (nextConfig: DaisyClawConfig) => {
            enforceSharedGatewaySessionGenerationForConfigWrite({
