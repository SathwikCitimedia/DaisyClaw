...
const originalTrace = process.env.DAISYCLAW_GATEWAY_STARTUP_TRACE;

describe("gateway server boundary", () => {
  beforeEach(() => {
    vi.resetModules();
    process.env.DAISYCLAW_GATEWAY_STARTUP_TRACE = "1";
  });
  ...
  afterEach(() => {
    ...
    if (originalTrace === undefined) {
      delete process.env.DAISYCLAW_GATEWAY_STARTUP_TRACE;
    } else {
      process.env.DAISYCLAW_GATEWAY_STARTUP_TRACE = originalTrace;
    }
  });
