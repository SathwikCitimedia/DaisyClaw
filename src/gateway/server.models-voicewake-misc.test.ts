...
describe("gateway server models + voicewake", () => {
  ...
  const withTempHome = async <T>(fn: (homeDir: string) => Promise<T>): Promise<T> => {
    const tempHome = await createTempHomeEnv("daisyclaw-home-");
    try {
  ...
  test(
    ...
    async () => {
      await withTempHome(async (homeDir) => {
        ...
        expect(initial.payload?.triggers).toEqual(["daisyclaw", "claude", "computer"]);
        ...
        const onDisk = JSON.parse(
          await fs.readFile(path.join(homeDir, ".daisyclaw", "settings", "voicewake.json"), "utf8"),
  ...
  test("voicewake.routing.get/set persists and broadcasts", { timeout: 60_000 }, async () => {
    await withTempHome(async (homeDir) => {
      ...
      const onDisk = JSON.parse(
        await fs.readFile(
          path.join(homeDir, ".daisyclaw", "settings", "voicewake-routing.json"),
          "utf8",
        ),
      ) as { routes?: unknown };
