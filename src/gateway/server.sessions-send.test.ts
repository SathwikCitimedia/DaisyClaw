...
const { createDaisyClawTools } = await import("../agents/daisyclaw-tools.js");
...
type SessionSendTool = ReturnType<typeof createDaisyClawTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createDaisyClawTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["DAISYCLAW_GATEWAY_PORT", "DAISYCLAW_GATEWAY_TOKEN"]);
  ...
  process.env.DAISYCLAW_GATEWAY_PORT = String(gatewayPort);
  process.env.DAISYCLAW_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send gateway loopback", () => {
  ...
  it(
    ...
    async () => {
      const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-sessions-send-route-"));
