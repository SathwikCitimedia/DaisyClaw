...
test("sessions.compact without maxLines runs embedded manual compaction for checkpoint-capable flows", async () => {
  ...
  expect(compactionCall.workspaceDir).toBe(path.join(os.tmpdir(), "daisyclaw-gateway-test"));
  ...
  expect(callConfig.agents?.defaults?.workspace).toBe(
    path.join(os.tmpdir(), "daisyclaw-gateway-test"),
  );
...
test("sessions.patch preserves nested model ids under provider overrides", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-gw-sessions-nested-"));
  ...
  await withEnvAsync({ DAISYCLAW_CONFIG_PATH: undefined }, async () => {
