...
/**
 * Lazily resolve the local node host's nodeId from ~/.daisyclaw/node.json.
 * Process-stable: only changes on `daisyclaw node install`, which requires restart.
 */
...
function resolveTrustedProxyControlUiScopes(params: {
  ...
  const rawHeader = firstHeaderValue(params.upgradeReq.headers["x-daisyclaw-scopes"]);
  ...
  const handleMessage = async (data: RawData) => {
    ...
    try {
      ...
      if (!client) {
        ...
        // matching instanceId against ~/.daisyclaw/node.json nodeId). SSH-tunneled remote
