...
import { CURRENT_SESSION_VERSION, SessionManager } from "daisyclaw/plugin-sdk/agent-sessions";
import type { AssistantMessage } from "daisyclaw/plugin-sdk/llm";
...
function checkpointConfig(storePath: string): DaisyClawConfig {
  return {
...
describe("session-compaction-checkpoints", () => {
  test("async capture stores pre-compaction identity without copying the transcript", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-checkpoint-async-"));
    tempDirs.push(dir);
  ...
  test("async capture derives session metadata without synchronous SessionManager.open", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-checkpoint-async-metadata-"));
    tempDirs.push(dir);
  ...
  test("async capture scans bounded metadata without copying oversized transcripts", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-checkpoint-async-oversized-"));
    tempDirs.push(dir);
  ...
  test("async fork creates a checkpoint branch transcript without SessionManager sync reads", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-checkpoint-fork-"));
    tempDirs.push(dir);
  ...
  test("async fork truncates mutable checkpoint sources at the stored leaf", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-checkpoint-fork-leaf-"));
    tempDirs.push(dir);
  ...
  test("async fork migrates legacy checkpoint snapshots before writing a current header", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-checkpoint-legacy-fork-"));
    tempDirs.push(dir);
  ...
  test("async fork skips JSON-valid garbage transcript entries", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-checkpoint-garbage-fork-"));
    tempDirs.push(dir);
  ...
  test("persist trims retained checkpoint snapshots by total byte budget", async () => {
    const { dir, storePath, sessionId, sessionKey, now } = await makeTempSessionStore(
      "daisyclaw-checkpoint-byte-trim-",
    );
