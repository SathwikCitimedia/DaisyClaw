...
function expectMessageFields(
  message: unknown,
  fields: { role?: string; content?: unknown; daisyclaw?: Record<string, unknown> },
) {
  ...
  if (fields.daisyclaw) {
    const metadata = requireRecord(record["__daisyclaw"], "message metadata");
    for (const [key, value] of Object.entries(fields.daisyclaw)) {
...
describe("readFirstUserMessageFromTranscript", () => {
  ...
  registerTempSessionStore("daisyclaw-session-fs-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
...
describe("readSessionMessages", () => {
  ...
  test("includes synthetic compaction markers for compaction entries", () => {
    ...
    const marker = out[1] as {
      ...
      __daisyclaw?: { kind?: string; id?: string };
      timestamp?: number;
    };
    expect(marker.role).toBe("system");
    expect(marker.content?.[0]?.text).toBe("Compaction");
    expect(marker["__daisyclaw"]?.kind).toBe("compaction");
    expect(marker["__daisyclaw"]?.id).toBe("comp-1");
  ...
  test("reads recent messages from the transcript tail without loading the whole file", () => {
    ...
    expectMessageFields(out[0], { role: "user", content: "recent", daisyclaw: { seq: 3 } });
    expectMessageFields(out[1], { role: "assistant", content: "latest", daisyclaw: { seq: 4 } });
  });
  ...
  test("preserves real sequence metadata for bounded recent-message reads", () => {
    ...
    expectMessageFields(result.messages[0], { content: "recent", daisyclaw: { seq: 3 } });
    expectMessageFields(result.messages[1], { content: "latest", daisyclaw: { seq: 4 } });
  });
  ...
  test("forwards the outer JSONL record timestamp to __daisyclaw.recordTimestampMs (#85648)", async () => {
    ...
    expectMessageFields(result[0], {
      content: "old turn",
      daisyclaw: { recordTimestampMs: Date.parse(t1) },
    });
    expectMessageFields(result[1], {
      content: "fresh turn",
      daisyclaw: { recordTimestampMs: Date.parse(t2) },
    });
  });
  ...
  test("reads active tree branch asynchronously without SessionManager.open", async () => {
    ...
    try {
      ...
      expectMessageFields(messages[2], { daisyclaw: { id: "user-2", seq: 3 } });
      expect(sessionManagerOpenSpy).not.toHaveBeenCalled();
      expect(readFileSpy).not.toHaveBeenCalled();
    } finally {
      sessionManagerOpenSpy.mockRestore();
  ...
  test("keeps async active branch rows when imported parent links are incomplete", async () => {
    ...
    expectMessageFields(messages[0], { daisyclaw: { id: "orphan-tail", seq: 1 } });
  });
  ...
  test("keeps legacy async parents when tree transcripts reference pre-v3 rows", async () => {
    ...
    expectMessageFields(messages[0], { daisyclaw: { id: "legacy-user", seq: 1 } });
    expectMessageFields(messages[1], { daisyclaw: { id: "tree-assistant", seq: 2 } });
  });
  ...
  test("reads only the active branch when transcript rewrites abandon older entries", () => {
    ...
    try {
      ...
      expect(out).toHaveLength(2);
      expect(out).toHaveLength(2);
      expectMessageFields(out[0], { role: "user", content: "clean prompt", daisyclaw: { seq: 1 } });
  ...
  test.each([
    ...
    ({ sessionId, sessionFileParts, wrongStorePathParts, message }) => {
      ...
      expect((out[0] as { __daisyclaw?: { seq?: number } })["__daisyclaw"]?.seq).toBe(1);
    },
  );
  ...
  test("keeps compaction markers when reading only the active SessionManager branch", () => {
    ...
    expect(
      out.map((message) => ({
        role: (message as { role?: string }).role,
        content: (message as { content?: unknown }).content,
        kind: (message as { __daisyclaw?: { kind?: string } })["__daisyclaw"]?.kind,
      })),
    ).toEqual([
...
describe("readSessionPreviewItemsFromTranscript", () => {
  ...
  registerTempSessionStore("daisyclaw-session-preview-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });
...
describe("readLatestSessionUsageFromTranscript", () => {
  ...
  registerTempSessionStore("daisyclaw-session-usage-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });
...
describe("resolveSessionTranscriptCandidates", () => {
  ...
  test("fallback candidate uses DAISYCLAW_HOME instead of os.homedir()", () => {
    vi.stubEnv("DAISYCLAW_HOME", "/srv/daisyclaw-home");
    vi.stubEnv("HOME", "/home/other");

    const candidates = resolveSessionTranscriptCandidates("sess-1", undefined);
    const fallback = candidates[candidates.length - 1];
    expect(fallback).toBe(
      path.join(path.resolve("/srv/daisyclaw-home"), ".daisyclaw", "sessions", "sess-1.jsonl"),
    );
  });
});
...
describe("resolveSessionTranscriptCandidates safety", () => {
  test.each([
    {
      storePath: "/tmp/daisyclaw/agents/main/sessions/sessions.json",
      sessionFile: "/tmp/daisyclaw/agents/ops/sessions/sess-safe.jsonl",
    },
    {
  ...
  test("drops unsafe session IDs instead of producing traversal paths", () => {
    const candidates = resolveSessionTranscriptCandidates(
      "../etc/passwd",
      "/tmp/daisyclaw/agents/main/sessions/sessions.json",
    );
  ...
  test("drops unsafe sessionFile candidates and keeps safe fallbacks", () => {
    const storePath = "/tmp/daisyclaw/agents/main/sessions/sessions.json";
    const candidates = resolveSessionTranscriptCandidates(
      "sess-safe",
      storePath,
      "../../etc/passwd",
  ...
  test("prefers the current sessionId transcript before a stale sessionFile candidate", () => {
    ...
    const candidates = resolveSessionTranscriptCandidates(
      ...
      storePath,
      "/tmp/daisyclaw/agents/main/sessions/22222222-2222-4222-8222-222222222222.jsonl",
    );

    expect(candidates[0]).toBe(
      path.resolve("/tmp/daisyclaw/agents/main/sessions/11111111-1111-4111-8111-111111111111.jsonl"),
    );
    expect(candidates).toContain(
      path.resolve("/tmp/daisyclaw/agents/main/sessions/22222222-2222-4222-8222-222222222222.jsonl"),
    );
  });

  test("keeps explicit custom sessionFile ahead of synthesized fallback", () => {
    const storePath = "/tmp/daisyclaw/agents/main/sessions/sessions.json";
    const sessionFile = "/tmp/daisyclaw/agents/main/sessions/custom-transcript.jsonl";
    const candidates = resolveSessionTranscriptCandidates(
      "11111111-1111-4111-8111-111111111111",
      storePath,
      sessionFile,
  ...
  test("keeps custom topic-like transcript paths ahead of synthesized fallback", () => {
    const storePath = "/tmp/daisyclaw/agents/main/sessions/sessions.json";
    const sessionFile = "/tmp/daisyclaw/agents/main/sessions/custom-topic-notes.jsonl";
    const candidates = resolveSessionTranscriptCandidates(
      "11111111-1111-4111-8111-111111111111",
      storePath,
      sessionFile,
  ...
  test("keeps forked transcript paths ahead of synthesized fallback", () => {
    ...
    const sessionFile =
      "/tmp/daisyclaw/agents/main/sessions/2026-03-23T16-30-00-000Z_11111111-1111-4111-8111-111111111111.jsonl";
  ...
  test("keeps timestamped custom transcript paths ahead of synthesized fallback", () => {
    const storePath = "/tmp/daisyclaw/agents/main/sessions/sessions.json";
    const sessionId = "11111111-1111-4111-8111-111111111111";
    const sessionFile = "/tmp/daisyclaw/agents/main/sessions/2026-03-23T16-30-00-000Z_notes.jsonl";
    const candidates = resolveSessionTranscriptCandidates(sessionId, storePath, sessionFile);
  ...
  test("still treats generated topic transcripts from another session as stale", () => {
    ...
    const staleSessionFile =
      "/tmp/daisyclaw/agents/main/sessions/22222222-2222-4222-8222-222222222222-topic-thread.jsonl";
...
describe("archiveSessionTranscripts", () => {
  ...
  registerTempSessionStore("daisyclaw-archive-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });

  beforeAll(() => {
    vi.stubEnv("DAISYCLAW_HOME", tmpDir);
  });
...
describe("oversized transcript line guards", () => {
  ...
  registerTempSessionStore("daisyclaw-session-fs-oversized-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });
  ...
  test("oversized line metadata extraction preserves id and parentId", async () => {
    ...
    // to the __daisyclaw metadata. Both must be correct for the record to
    ...
    // id is preserved in __daisyclaw transcript metadata
    const meta = (oversized as Record<string, Record<string, unknown>>)["__daisyclaw"];
