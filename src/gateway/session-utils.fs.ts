...
/** Attach DaisyClaw metadata to a transcript message without dropping existing metadata. */
export function attachDaisyClawTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record["__daisyclaw"] &&
    typeof record["__daisyclaw"] === "object" &&
    !Array.isArray(record["__daisyclaw"])
      ? (record["__daisyclaw"] as Record<string, unknown>)
      : {};
  return {
    ...record,
    __daisyclaw: {
...
function buildOversizedTranscriptRecord(line: string): TailTranscriptRecord {
  ...
  const record: Record<string, unknown> = {
    ...
    message: {
      ...
      __daisyclaw: { truncated: true, reason: "oversized" },
    },
  };
  return {
    ...(id ? { id } : {}),
...
export function readRecentSessionMessagesWithStats(
  ...
  const messagesWithSeq = messages.map((message, index) =>
    attachDaisyClawTranscriptMeta(message, { seq: firstSeq + index }),
  );
    ...
    const recordTimestampMs =
      typeof entry.timestamp === "string"
        ...
        : typeof entry.timestamp === "number"
          ? entry.timestamp
          : Number.NaN;
    return attachDaisyClawTranscriptMeta(entry.message, {
...
function extractTranscriptTokenEstimateFromLine(line: string): {
  ...
  try {
    ...
    const isDeliveryMirror =
      role === "assistant" && modelProvider === "daisyclaw" && model === "delivery-mirror";
    if (isDeliveryMirror) {
      return null;
    }
    ...
    const model =
      typeof message.model === "string"
        ...
        : typeof parsed.model === "string"
          ? parsed.model.trim()
          : undefined;
    const isDeliveryMirror = modelProvider === "daisyclaw" && model === "delivery-mirror";
