...
const READ_SCOPE_HEADER = { "x-daisyclaw-scopes": "operator.read" };
...
async function createSessionStoreFile(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-session-history-"));
...
type SessionHistoryMessage = {
  ...
  __daisyclaw?: { id?: string; seq?: number };
};
...
function expectDaisyClawMetadata(
...
async function expectMessageEventMatch(
  ...
  if (params.id !== undefined) {
    expectDaisyClawMetadata(
      (event.data as { message?: { __daisyclaw?: { id?: string; seq?: number } } }).message?.[
        "__daisyclaw"
      ],
      {
        id: params.id,
...
describe("session history HTTP endpoints", () => {
  test("returns session history over direct REST", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expectDaisyClawMetadata(body.messages?.[0]?.["__daisyclaw"], {
        seq: 1,
      });
    });
  });
  ...
  test("supports cursor pagination over direct REST while preserving the messages field", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expect(firstBody.messages?.map((message) => message["__daisyclaw"]?.seq)).toEqual([2, 3]);
      ...
      expect(secondBody.messages?.map((message) => message["__daisyclaw"]?.seq)).toEqual([1]);
  ...
  test("streams bounded history windows over SSE", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expectDaisyClawMetadata(nextData.messages?.[0]?.["__daisyclaw"], {
        id: thirdMessageId,
        seq: 3,
      });
  ...
  test("seeds bounded SSE windows from visible history when transcript refreshes are silent", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      const refreshData = refreshEvent.data as {
        messages?: Array<{ content?: Array<{ text?: string }>; __daisyclaw?: { seq?: number } }>;
      };
      expect(refreshData.messages?.[0]?.content?.[0]?.text).toBe("second message");
      expect(refreshData.messages?.[0]?.["__daisyclaw"]?.seq).toBe(2);
