...
const mocks = vi.hoisted(() => ({
  replaceConfigFile: vi.fn(async (_params: { nextConfig: DaisyClawConfig }) => {}),
}));
...
type GatewayAuthConfig = NonNullable<NonNullable<DaisyClawConfig["gateway"]>["auth"]>;
...
function gatewayAuthConfig(auth: GatewayAuthConfig): DaisyClawConfig {
  return {
    gateway: { auth },
  };
}

function gatewayAuthConfigWithDefaultEnvProvider(auth: GatewayAuthConfig): DaisyClawConfig {
  return {
...
describe("mergeGatewayTailscaleConfig", () => {
  ...
  it("preserves explicit serviceName overrides", () => {
    expect(
      mergeGatewayTailscaleConfig(
        { mode: "serve", serviceName: "svc:old-daisyclaw", resetOnExit: false },
        { serviceName: "svc:daisyclaw" },
      ),
    ).toEqual({ mode: "serve", serviceName: "svc:daisyclaw", resetOnExit: false });
  });
});
...
describe("ensureGatewayStartupAuth", () => {
  ...
  async function expectEphemeralGeneratedTokenWhenOverridden(cfg: DaisyClawConfig) {
  ...
  async function expectNoTokenGeneration(cfg: DaisyClawConfig, mode: string) {
    const result = await runStartupAuth({
      cfg,
      persist: true,
    });
  ...
  function createMissingGatewayTokenSecretRefConfig(): DaisyClawConfig {
  ...
  it("resolves env-template gateway.auth.token before env-token short-circuiting", async () => {
    await expectResolvedToken({
      cfg: gatewayAuthConfig({ mode: "token", token: "${DAISYCLAW_GATEWAY_TOKEN}" }),
      env: {
        DAISYCLAW_GATEWAY_TOKEN: "resolved-token",
      } as NodeJS.ProcessEnv,
      expectedToken: "resolved-token",
      expectedConfiguredToken: "${DAISYCLAW_GATEWAY_TOKEN}",
    });
  });

  it("uses DAISYCLAW_GATEWAY_TOKEN without resolving configured token SecretRef", async () => {
    await expectResolvedToken({
      cfg: createMissingGatewayTokenSecretRefConfig(),
      env: {
        DAISYCLAW_GATEWAY_TOKEN: "token-from-env",
      } as NodeJS.ProcessEnv,
  ...
  it("uses DAISYCLAW_GATEWAY_PASSWORD without resolving configured password SecretRef", async () => {
    const result = await runStartupAuth({
      cfg: gatewayAuthConfigWithDefaultEnvProvider({
        mode: "password",
      ...
      env: {
        DAISYCLAW_GATEWAY_PASSWORD: "password-from-env", // pragma: allowlist secret
      } as NodeJS.ProcessEnv,
      persist: true,
    });
  ...
  it("keeps startup non-breaking when hooks token reuses gateway token resolved from env", async () => {
    ...
    const result = await runStartupAuth({
      ...
      env: {
        DAISYCLAW_GATEWAY_TOKEN: "shared-gateway-token-1234567890",
      } as NodeJS.ProcessEnv,
      warn,
    });
    ...
    expect(warn).toHaveBeenCalledWith(expect.stringContaining("daisyclaw security audit"));
  });
