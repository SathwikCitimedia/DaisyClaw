...
describe("talk realtime gateway relay", () => {
  ...
  it("bridges browser audio, transcripts, and tool results through a backend provider", async () => {
    ...
    expect(bridge.submitToolResult).toHaveBeenNthCalledWith(
      ...
      {
        status: "working",
        tool: "daisyclaw_agent_consult",
        message:
          "Tell the person briefly that you are checking, then wait for the final DaisyClaw result before answering with the actual result.",
      },
      { willContinue: true },
    );
    expect(bridge.submitToolResult).toHaveBeenNthCalledWith(
    ...
    expectRecordFields(toolResultPayloads[0]?.talkEvent, {
      type: "tool.progress",
      callId: "call-1",
      payload: { name: "daisyclaw_agent_consult", status: "working" },
    });
  ...
  it("forces an agent consult when configured and realtime transcript finalizes without a provider tool call", async () => {
    ...
    expect(bridge.sendUserMessage).toHaveBeenLastCalledWith(
      "Briefly tell the person that you are checking with DaisyClaw. Do not answer the request yet. Wait for the DaisyClaw result before giving the actual answer.",
    );
    ...
    expect(bridge.submitToolResult).toHaveBeenLastCalledWith(
      ...
      {
        status: "already_delivered",
        message: "DaisyClaw already delivered this consult result internally. Do not repeat it.",
      },
      { suppressResponse: true },
    );
    expect(bridge.sendUserMessage).toHaveBeenLastCalledWith(
      [
        "DaisyClaw finished checking. Speak this result naturally and concisely.",
