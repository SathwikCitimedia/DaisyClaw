...
function buildForcedConsultCheckingPrompt(): string {
  return [
    "Briefly tell the person that you are checking with DaisyClaw.",
    "Do not answer the request yet. Wait for the DaisyClaw result before giving the actual answer.",
  ].join(" ");
}
  ...
  session.forcedConsults.schedule(handle, FORCED_CONSULT_FALLBACK_DELAY_MS, () => {
    ...
    broadcastToOwner(session.context, session.connId, {
      ...
      args: {
        ...
        context:
          "The realtime provider produced a final user transcript without invoking daisyclaw_agent_consult, so DaisyClaw is forcing the consult for realtime Talk.",
