...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "DAISYCLAW_SKIP_BROWSER_CONTROL_SERVER",
  "DAISYCLAW_SKIP_GMAIL_WATCHER",
  "DAISYCLAW_SKIP_CANVAS_HOST",
  "DAISYCLAW_BUNDLED_PLUGINS_DIR",
  "DAISYCLAW_DISABLE_BUNDLED_PLUGINS",
  "DAISYCLAW_SKIP_CHANNELS",
  "DAISYCLAW_SKIP_PROVIDERS",
  "DAISYCLAW_SKIP_CRON",
  "DAISYCLAW_TEST_MINIMAL_GATEWAY",
] as const;
...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.DAISYCLAW_CONFIG_PATH) {
    configPaths.add(process.env.DAISYCLAW_CONFIG_PATH);
  }
  if (process.env.DAISYCLAW_STATE_DIR) {
    configPaths.add(path.join(process.env.DAISYCLAW_STATE_DIR, "daisyclaw.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.DAISYCLAW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "daisyclaw-test-no-bundled-extensions")
    : "daisyclaw-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".daisyclaw-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".daisyclaw-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".daisyclaw-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>daisyclaw-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__daisyclawTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__daisyclawTestConnectChallengeTracked";
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["DAISYCLAW_GATEWAY_TOKEN"]);
  const prev = process.env.DAISYCLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.DAISYCLAW_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.DAISYCLAW_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
