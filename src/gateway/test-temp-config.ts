...
function withStableOwnerDisplaySecretForTest(cfg: unknown): unknown {
  ...
  return {
    ...
    commands: {
      ...commands,
      ownerDisplaySecret: "daisyclaw-test-owner-display-secret",
    },
  };
}

/** Writes a temp DaisyClaw config, installs it as runtime state, then restores globals. */
export async function withTempConfig(params: {
  cfg: unknown;
  run: () => Promise<void>;
  prefix?: string;
}): Promise<void> {
  const prevConfigPath = process.env.DAISYCLAW_CONFIG_PATH;

  const testConfig = withStableOwnerDisplaySecretForTest(params.cfg) as DaisyClawConfig;
  const dir = await mkdtemp(path.join(os.tmpdir(), params.prefix ?? "daisyclaw-test-config-"));
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.DAISYCLAW_CONFIG_PATH = prevConfigPath;
    }
    clearConfigCache();
    resetConfigRuntimeState();
    clearSecretsRuntimeSnapshot();
