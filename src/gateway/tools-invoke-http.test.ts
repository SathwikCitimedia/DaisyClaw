...
let lastCreateDaisyClawToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/daisyclaw-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateDaisyClawToolsContext?.agentTo,
          agentThreadId: lastCreateDaisyClawToolsContext?.agentThreadId,
        },
        inheritedToolDenylist: lastCreateDaisyClawToolsContext?.inheritedToolDenylist,
      }),
    },
    {
      name: "sessions_send",
  ...
  return {
    createDaisyClawTools: (ctx: Record<string, unknown>) => {
      lastCreateDaisyClawToolsContext = ctx;
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateDaisyClawToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-daisyclaw-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-daisyclaw-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateDaisyClawToolsContext?.allowMediaInvokeCommands).toBe(true);
    expect(lastCreateDaisyClawToolsContext?.disablePluginTools).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateDaisyClawToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("keeps plugin tools enabled for non-core tool invokes", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateDaisyClawToolsContext?.disablePluginTools).toBe(false);
  });
  ...
  it("allows the requested plugin tool through Gateway profile filtering", async () => {
    ...
    expect(lastCreateDaisyClawToolsContext?.pluginToolAllowlist).toContain("plugin_doctor");
  });
  ...
  it("uses tools.alsoAllow for optional plugin discovery without loading every plugin tool", async () => {
    ...
    expect(body.result?.permissionFlow).toBe(true);
    expect(lastCreateDaisyClawToolsContext?.pluginToolAllowlist).toContain("plugin_doctor");
    expect(lastCreateDaisyClawToolsContext?.pluginToolAllowlist).not.toContain("*");
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-daisyclaw-message-to": "channel:24514",
        "x-daisyclaw-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
  ...
  it("keeps shared-secret bearer auth as owner for explicitly allowed owner-only tools", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        authorization: "Bearer secret",
        "x-daisyclaw-scopes": "operator.write",
      },
      tool: "nodes",
      sessionKey: "main",
    });
    ...
    expect(lastCreateDaisyClawToolsContext?.senderIsOwner).toBe(true);
  });
  ...
  it("derives sender owner identity from HTTP auth instead of caller headers", async () => {
    ...
    expect(lastCreateDaisyClawToolsContext?.senderIsOwner).toBe(false);
