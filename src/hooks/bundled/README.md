...
This directory contains hooks that ship with DaisyClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.daisyclaw/workspace`)
...
homepage: https://docs.daisyclaw.ai/automation/hooks#my-hook
metadata:
...
- **Managed hooks**: `~/.daisyclaw/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.daisyclaw/daisyclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'daisyclaw.*gateway' && pnpm daisyclaw gateway`
3. Enable the hook: `daisyclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.daisyclaw.ai/automation/hooks
