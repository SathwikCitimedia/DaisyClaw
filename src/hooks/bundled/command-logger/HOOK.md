...
homepage: https://docs.daisyclaw.ai/automation/hooks#command-logger
metadata:
  {
    "daisyclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.daisyclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.daisyclaw/logs/commands.log ~/.daisyclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/daisyclaw`:
   ```
   /home/username/.daisyclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
