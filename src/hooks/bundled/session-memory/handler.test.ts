...
// Avoid calling the embedded DaisyClaw agent (global command lane); keep this unit test deterministic.
...
beforeAll(async () => {
  ...
  suiteWorkspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-session-memory-"));
});
...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => DaisyClawConfig;
  action?: "new" | "reset";
