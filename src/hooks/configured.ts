...
export function hasConfiguredInternalHooks(config: DaisyClawConfig): boolean {
...
export function resolveConfiguredInternalHookNames(config: DaisyClawConfig): Set<string> | null {
  const internal = config.hooks?.internal;
  if (!internal || internal.enabled === false) {
    return new Set();
  }
