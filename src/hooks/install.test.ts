...
const fixtureRoot = path.join(process.cwd(), ".tmp", `daisyclaw-hook-install-${randomUUID()}`);
...
const zipHooksBuffer = await createZipHookPackBuffer({
  packageName: "@daisyclaw/zip-hooks",
  hookName: "zip-hook",
...
const npmPackHooksBuffer = await createTarGzHookPackBuffer({
  packageName: "@daisyclaw/test-hooks",
  hookName: "one-hook",
...
function writeHookPackManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@daisyclaw/test-hooks",
      version: "0.0.1",
      daisyclaw: { hooks: params.hooks },
...
function writeHookPackFiles(params: {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      'metadata: {"daisyclaw":{"events":["command:new"]}}',
      "---",
      "",
      `# ${params.heading}`,
    ].join("\n"),
...
async function createZipHookPackBuffer(params: {
  ...
  const packageJson = JSON.stringify({
    ...
    version: "0.0.1",
    daisyclaw: { hooks: [`./hooks/${params.hookName}`] },
  });
    ...
    const cases = [
      {
        hooks: ["../outside"],
        setupLink: false,
        expected: "daisyclaw.hooks entry escapes package directory",
      },
      {
        hooks: ["./linked"],
        setupLink: true,
        expected: "daisyclaw.hooks entry resolves outside package directory",
      },
    ] as const;
...
describe("installHooksFromNpmSpec", () => {
  it("does not expose dangerous force unsafe install through npm-spec archive params", async () => {
    ...
    try {
      const result = await installHooksFromNpmSpec({
        spec: "@daisyclaw/test-hooks@0.0.1",
      });
  ...
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(result.hookPackId).toBe("test-hooks");
    expect(result.npmResolution?.resolvedSpec).toBe("@daisyclaw/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@daisyclaw/test-hooks@0.0.1",
    });
  ...
  it("rejects invalid npm spec shapes", async () => {
    ...
    const run = runCommandWithTimeoutMock;
    mockNpmPackMetadataResult(run, {
      id: "@daisyclaw/test-hooks@0.0.2-beta.1",
      name: "@daisyclaw/test-hooks",
    ...
    const result = await installHooksFromNpmSpec({
      spec: "@daisyclaw/test-hooks",
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("prerelease version 0.0.2-beta.1");
      expect(result.error).toContain('"@daisyclaw/test-hooks@beta"');
    }
  });
});
