...
vi.mock("../agents/agent-scope.js", () => ({
  ...
  resolveAgentWorkspaceDir: vi.fn(() => "/tmp/daisyclaw-agent"),
  resolveAgentDir: vi.fn(() => "/tmp/daisyclaw-agent/.daisyclaw-agent"),
  resolveAgentEffectiveModelPrimary: vi.fn((cfg: DaisyClawConfig) => {
...
function requireFirstRunOptions(): Record<string, unknown> {
  ...
  if (!call) {
    throw new Error("expected embedded DaisyClaw agent run");
  }
  const [options] = call;
  if (!options || typeof options !== "object") {
    throw new Error("expected embedded DaisyClaw agent run options");
  }
