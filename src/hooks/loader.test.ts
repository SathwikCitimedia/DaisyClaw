...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["DAISYCLAW_BUNDLED_HOOKS_DIR"]);
  ...
  describe("loadInternalHooks", () => {
    it("detects configured internal hook surfaces", () => {
      expect(hasConfiguredInternalHooks({} satisfies DaisyClawConfig)).toBe(false);
      expect(
    ...
    const expectNoCommandHookRegistration = async (cfg: DaisyClawConfig) => {
    ...
    it("skips hook discovery until internal hooks are configured", async () => {
      for (const cfg of [
        {} satisfies DaisyClawConfig,
        { hooks: {} } satisfies DaisyClawConfig,
        { hooks: { internal: {} } } satisfies DaisyClawConfig,
      ]) {
        const count = await loadInternalHooks(cfg, tmpDir);
        expect(count).toBe(0);
      }
