...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "daisyclaw-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.DAISYCLAW_BUNDLED_HOOKS_DIR;
    process.env.DAISYCLAW_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(() => {
    ...
    if (previousBundledHooksDir === undefined) {
      delete process.env.DAISYCLAW_BUNDLED_HOOKS_DIR;
    } else {
      process.env.DAISYCLAW_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    const bundleRoot = path.join(workspaceDir, ".daisyclaw", "extensions", "sample-bundle");
  ...
  function createConfig(enabled: boolean): DaisyClawConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entry.hook.source).toBe("daisyclaw-plugin");
  ...
  it("does not treat Claude hooks.json bundles as DaisyClaw hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".daisyclaw", "extensions", "claude-bundle");
