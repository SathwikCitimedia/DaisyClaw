...
import type { DaisyClawConfig, HookConfig } from "../config/config.js";
...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  "daisyclaw-bundled": {
    ...
    canOverride: ["daisyclaw-bundled"],
    canBeOverriddenBy: ["daisyclaw-managed", "daisyclaw-plugin"],
  },
  "daisyclaw-plugin": {
    precedence: 20,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["daisyclaw-bundled", "daisyclaw-plugin"],
    canBeOverriddenBy: ["daisyclaw-managed"],
  },
  "daisyclaw-managed": {
    precedence: 30,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["daisyclaw-bundled", "daisyclaw-managed", "daisyclaw-plugin"],
    canBeOverriddenBy: ["daisyclaw-managed"],
  },
  "daisyclaw-workspace": {
    precedence: 40,
    ...
    canOverride: ["daisyclaw-workspace"],
    canBeOverriddenBy: ["daisyclaw-workspace"],
  },
};
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "daisyclaw-plugin") {
    return { enabled: true };
  }
