...
import { closeDaisyClawAgentDatabasesForTest } from "../state/daisyclaw-agent-db.js";
import {
  closeDaisyClawStateDatabase,
  openDaisyClawStateDatabase,
} from "../state/daisyclaw-state-db.js";
import { withDaisyClawTestState } from "../test-utils/daisyclaw-test-state.js";
import {
...
function makeResult(overrides: Partial<BackupCreateResult> = {}): BackupCreateResult {
  return {
    ...
    archiveRoot: "daisyclaw-backup-2026-01-01",
    archivePath: "/tmp/daisyclaw-backup.tar.gz",
    dryRun: false,
...
describe("formatBackupCreateSummary", () => {
  const backupArchiveLine = "Backup archive: /tmp/daisyclaw-backup.tar.gz";
  ...
  it.each([
    {
        ...
        skipped: [
          {
            ...
            displayPath: "~/Projects/daisyclaw",
            reason: "covered",
            coveredBy: "~/.daisyclaw",
          },
        ],
      ...
      expected: [
        ...
        "- state: ~/.daisyclaw",
        "Skipped 1 path:",
        "- workspace: ~/Projects/daisyclaw (covered by ~/.daisyclaw)",
        "Created /tmp/daisyclaw-backup.tar.gz",
        ...
        assets: [
          {
            ...
            displayPath: "~/.daisyclaw/config.json",
          },
          {
            kind: "credentials",
            sourcePath: "/oauth",
            archivePath: "archive/oauth",
            displayPath: "~/.daisyclaw/oauth",
          },
        ],
      }),
      expected: [
        backupArchiveLine,
        "Included 2 paths:",
        "- config: ~/.daisyclaw/config.json",
        "- credentials: ~/.daisyclaw/oauth",
  ...
  it("surfaces the volatile skip count in the summary", () => {
    expect(
      ...
      "Backup archive: /tmp/daisyclaw-backup.tar.gz",
      "Included 1 path:",
      "- state: ~/.daisyclaw",
      "Created /tmp/daisyclaw-backup.tar.gz",
...
describe("buildExtensionsNodeModulesFilter", () => {
  it("excludes dependency trees only under state extensions", () => {
    ...
    expect(filter("/state/extensions/demo/daisyclaw.plugin.json")).toBe(true);
  ...
  it("normalizes Windows path separators", () => {
    const filter = buildExtensionsNodeModulesFilter("C:\\Users\\me\\.daisyclaw\\");

    expect(filter(String.raw`C:\Users\me\.daisyclaw\extensions\demo\index.js`)).toBe(true);
    expect(
      filter(String.raw`C:\Users\me\.daisyclaw\extensions\demo\node_modules\dep\index.js`),
    ).toBe(false);
  });
});
...
describe("createBackupArchive", () => {
  it("falls back when injected nowMs is outside Date range", async () => {
    await withDaisyClawTestState(
      {
        layout: "state-only",
        prefix: "daisyclaw-backup-invalid-now-",
        scenario: "minimal",
      },
      async (state) => {
        ...
        try {
          ...
          expect(path.basename(result.archivePath)).toContain("daisyclaw-backup.tar.gz");
  ...
  it("falls back to epoch when injected nowMs and Date.now are outside Date range", async () => {
    await withDaisyClawTestState(
      {
        layout: "state-only",
        prefix: "daisyclaw-backup-invalid-fallback-now-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
  ...
  it("skips current live volatile state files while preserving workspace locks", async () => {
    await withDaisyClawTestState(
      {
        layout: "split",
        prefix: "daisyclaw-backup-volatile-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
  ...
  it("scrubs transient SQLite delivery queue rows from archive snapshots", async () => {
    await withDaisyClawTestState(
      {
        layout: "state-only",
        prefix: "daisyclaw-backup-sqlite-queue-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
        ...
        const { db } = openDaisyClawStateDatabase({ env: state.env });
        db.prepare(
          `
        ...
        try {
          ...
          const archivedDbEntry = entries.find((entry) =>
            entry.endsWith("/state/state/daisyclaw.sqlite"),
          );
          expect(archivedDbEntry).toBeDefined();
          expect(entries.some((entry) => entry.endsWith("/state/state/daisyclaw.sqlite-wal"))).toBe(
            false,
          );
  ...
  it("snapshots per-agent SQLite auth stores into the archive", async () => {
    await withDaisyClawTestState(
      {
        layout: "state-only",
        prefix: "daisyclaw-backup-agent-sqlite-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
        ...
        const entries = await listArchiveEntries(result.archivePath);
        const archivedDbEntry = entries.find((entry) =>
          entry.endsWith("/state/agents/main/agent/daisyclaw-agent.sqlite"),
        );
        expect(archivedDbEntry).toBeDefined();
        expect(
          entries.some((entry) =>
            entry.endsWith("/state/agents/main/agent/daisyclaw-agent.sqlite-wal"),
          ),
        ).toBe(false);
  ...
  it("omits installed plugin node_modules from the real archive while keeping plugin files", async () => {
    await withDaisyClawTestState(
      {
        layout: "state-only",
        prefix: "daisyclaw-backup-plugin-deps-",
        scenario: "minimal",
      },
      async (state) => {
        ...
        await fs.writeFile(
          path.join(stateDir, "extensions", "demo", "daisyclaw.plugin.json"),
          '{"id":"demo"}\n',
          "utf8",
        );
        await fs.writeFile(
        ...
        expect(entrySuffixes).toContain("/state/extensions/demo/daisyclaw.plugin.json");
  ...
  it("dereferences hardlinks instead of emitting restore-hostile Link entries", async () => {
    await withDaisyClawTestState(
      {
        layout: "state-only",
        prefix: "daisyclaw-backup-hardlink-",
        scenario: "minimal",
      },
      async (state) => {
        const stateDir = state.stateDir;
        const outputDir = state.path("backups");
        const sourcePath = path.join(stateDir, "workspace-adx", "daisyclaw-src", "node_modules");
  ...
  it("does not duplicate the root manifest when the system tempdir lives inside the state dir", async () => {
    await withDaisyClawTestState(
      {
        layout: "state-only",
        prefix: "daisyclaw-backup-tmp-overlap-",
        scenario: "minimal",
      },
      async (state) => {
        const stateDir = state.stateDir;
  ...
  it("does not duplicate the root manifest when the system tempdir is the state dir itself", async () => {
    await withDaisyClawTestState(
      {
        layout: "state-only",
        prefix: "daisyclaw-backup-tmp-equals-state-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
