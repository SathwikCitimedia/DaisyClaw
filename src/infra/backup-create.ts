...
import { resolveDateTimestampMs } from "@daisyclaw/normalization-core/number-coercion";
import {
...
async function createSanitizedStateSqliteBackupAsset(params: {
  ...
  const archiveSourcePath = resolveDaisyClawStateSqlitePath({
    ...process.env,
  ...
  const sourcePath = path.join(params.tempDir, "daisyclaw-state-backup.sqlite");
  try {
...
async function listAgentSqlitePaths(stateDir: string): Promise<string[]> {
  ...
  async function visit(dir: string): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      } else if (entry.isFile() && entry.name === "daisyclaw-agent.sqlite") {
...
async function createAgentSqliteBackupAssets(params: {
  ...
  for (const archiveSourcePath of sqlitePaths) {
    ...
    const sourcePath = path.join(
      params.tempDir,
      `daisyclaw-agent-backup-${snapshots.length}.sqlite`,
    );
    try {
      source.exec("PRAGMA busy_timeout = 30000;");
      source.prepare("VACUUM INTO ?").run(sourcePath);
...
export async function createBackupArchive(
  ...
  if (plan.included.length === 0) {
    throw new Error(
      onlyConfig
        ? "No DaisyClaw config file was found to back up."
        : "No local DaisyClaw state was found to back up.",
    );
  }
  ...
  const tempDir = await fs.mkdtemp(path.join(tempRoot, "daisyclaw-backup-"));
