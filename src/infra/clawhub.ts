...
import { readResponseWithLimit } from "@daisyclaw/media-core/read-response-with-limit";
import {
...
// Keep aligned with @daisyclaw/plugin-package-contract ExternalPluginCompatibility.
...
export type ClawHubPackageReadiness = {
  ready?: boolean | null;
  readyForDaisyClaw?: boolean | null;
...
function normalizeBaseUrl(baseUrl?: string): string {
  const envValue =
    normalizeOptionalString(process.env.DAISYCLAW_CLAWHUB_URL) ||
...
function normalizeGitHubCodeloadBaseUrl(): string {
  const value =
    normalizeOptionalString(process.env.DAISYCLAW_CLAWHUB_GITHUB_CODELOAD_BASE_URL) ||
...
function resolveClawHubConfigPaths(): string[] {
  const explicit =
    normalizeOptionalString(process.env.DAISYCLAW_CLAWHUB_CONFIG_PATH) ||
...
export async function resolveClawHubAuthToken(): Promise<string | undefined> {
  const envToken =
    normalizeOptionalString(process.env.DAISYCLAW_CLAWHUB_TOKEN) ||
...
function normalizePluginApiVersionForComparator(version: string, target: string): string {
  const normalizedCorrection = normalizeDaisyClawNumericCorrectionForPluginApi(version);
  if (normalizedCorrection) {
    return normalizedCorrection;
  }
  return shouldPreservePluginApiPrereleaseFloor(target)
    ? version
    : normalizeDaisyClawReleaseSuffixForPluginApi(version);
}
...
const DAISYCLAW_RELEASE_SUFFIX_PATTERN =
  /^[vV]?(\d{4}\.[1-9]\d?\.[1-9]\d*)(?:-\d+|-(?:alpha|beta|rc)\.\d+)$/i;
const DAISYCLAW_NUMERIC_CORRECTION_PATTERN = /^[vV]?(\d{4}\.[1-9]\d?\.[1-9]\d*)-\d+$/;

function normalizeDaisyClawNumericCorrectionForPluginApi(
  pluginApiVersion: string,
): string | undefined {
  return DAISYCLAW_NUMERIC_CORRECTION_PATTERN.exec(pluginApiVersion.trim())?.[1];
}

function normalizeDaisyClawReleaseSuffixForPluginApi(pluginApiVersion: string): string {
  const match = DAISYCLAW_RELEASE_SUFFIX_PATTERN.exec(pluginApiVersion.trim());
...
export async function downloadClawHubPackageArchive(params: {
  ...
  if (params.artifact === "clawpack") {
    ...
    const target = await createTempDownloadTarget({
      prefix: "daisyclaw-clawhub-clawpack",
  ...
  const sha256Hex = formatSha256Hex(bytes);
  const target = await createTempDownloadTarget({
    prefix: "daisyclaw-clawhub-package",
...
export async function downloadClawHubSkillArchive(params: {
  ...
  const sha256Hex = formatSha256Hex(bytes);
  const target = await createTempDownloadTarget({
    prefix: "daisyclaw-clawhub-skill",
...
export async function downloadClawHubGitHubSkillArchive(params: {
  ...
  const sha256Hex = formatSha256Hex(bytes);
  const target = await createTempDownloadTarget({
    prefix: "daisyclaw-clawhub-github-skill",
