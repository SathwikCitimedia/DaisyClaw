...
describe("command-analysis risks", () => {
  ...
  it("builds executable payload candidates through carriers and shell wrappers", () => {
    ...
    expect(buildCommandPayloadCandidates(["sudo", "DAISYCLAW_ENV=1", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    expect(buildCommandPayloadCandidates(["exec", "-a", "daisyclaw", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    ).toContain("daisyclaw channels login --channel whatsapp");
  });
