...
describe("command explainer tree-sitter runtime", () => {
  ...
  it("reports missing parser packages and wasm files with explainer context", () => {
    expect(() =>
      resolvePackageFileForCommandExplanation(
        "definitely-missing-daisyclaw-parser-package",
        "parser.wasm",
      ),
    ).toThrow("Unable to resolve definitely-missing-daisyclaw-parser-package");

    expect(() =>
      resolvePackageFileForCommandExplanation("web-tree-sitter", "missing-daisyclaw-parser.wasm"),
    ).toThrow("Unable to locate missing-daisyclaw-parser.wasm in web-tree-sitter");
  });
  ...
  it("detects eval, source, aliases, and carrier shell wrappers", async () => {
    const evalCommand = await explainShellCommand('eval "$DAISYCLAW_CMD"');
