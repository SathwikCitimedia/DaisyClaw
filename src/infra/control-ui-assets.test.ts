...
vi.mock("./daisyclaw-root.js", () => ({
  resolveDaisyClawPackageRoot: vi.fn(async () => null),
  resolveDaisyClawPackageRootSync: vi.fn(() => null),
}));
...
let daisyclawRoot: typeof import("./daisyclaw-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    daisyclawRoot = await import("./daisyclaw-root.js");
  });
  ...
  it("resolves dist control-ui index path for symlinked argv1 via realpath", async () => {
    const pkgRoot = abs("fixtures/bun-global/daisyclaw");
    const wrapperArgv1 = abs("fixtures/bin/daisyclaw");
  ...
  it("uses resolveDaisyClawPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/daisyclaw");
    (
      daisyclawRoot.resolveDaisyClawPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/daisyclaw"))).resolves.toBe(
      path.join(pkgRoot, "dist", "control-ui", "index.html"),
    );
  });
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "daisyclaw" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "daisyclaw.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-daisyclaw");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", () => {
    const pkgRoot = abs("fixtures/daisyclaw-bundle");
    (
      daisyclawRoot.resolveDaisyClawPackageRootSync as unknown as ReturnType<typeof vi.fn>
  ...
  it("prefers packaged app Control UI assets in Contents/Resources", () => {
    const execPath = abs("fixtures/DaisyClaw.app/Contents/MacOS/DaisyClaw");
    const bundledUiDir = abs("fixtures/DaisyClaw.app/Contents/Resources/control-ui");
  ...
  it("detects package-proven control-ui roots", () => {
    const pkgRoot = abs("fixtures/daisyclaw-package-root");
    const uiDir = path.join(pkgRoot, "dist", "control-ui");
    setDir(uiDir);
    setFile(path.join(uiDir, "index.html"), "<html></html>\n");
    (
