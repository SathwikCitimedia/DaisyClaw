// Covers DaisyClaw package root resolution.
...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__daisyclaw_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "daisyclaw-root");
...
function setPackageRoot(root: string, name = "daisyclaw") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./daisyclaw-root.js").resolveDaisyClawPackageRootSync,
  asyncResolver: typeof import("./daisyclaw-root.js").resolveDaisyClawPackageRoot,
  opts: Parameters<typeof import("./daisyclaw-root.js").resolveDaisyClawPackageRootSync>[0],
...
vi.mock("./daisyclaw-root.fs.runtime.js", () => ({
  daisyClawRootFsSync: mockFsModule(),
  daisyClawRootFs: mockFsPromisesModule(),
}));

describe("resolveDaisyClawPackageRoot", () => {
  let resolveDaisyClawPackageRoot: typeof import("./daisyclaw-root.js").resolveDaisyClawPackageRoot;
  let resolveDaisyClawPackageRootSync: typeof import("./daisyclaw-root.js").resolveDaisyClawPackageRootSync;
  let clearDaisyClawPackageRootCaches: typeof import("./daisyclaw-root.js").testing.clearDaisyClawPackageRootCaches;

  beforeAll(async () => {
    ({
      resolveDaisyClawPackageRoot,
      resolveDaisyClawPackageRootSync,
      testing: { clearDaisyClawPackageRootCaches },
    } = await import("./daisyclaw-root.js"));
  });

  beforeEach(() => {
    clearDaisyClawPackageRootCaches();
    state.entries.clear();
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "daisyclaw");
        const pkgRoot = path.join(project, "node_modules", "daisyclaw");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "daisyclaw");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "daisyclaw.mjs")));
    ...
    {
      name: "falls through from a non-daisyclaw moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-daisyclaw");
    ...
    {
      name: "returns null for non-daisyclaw package roots",
      setup: () => {
        const pkgRoot = fx("not-daisyclaw");
        setPackageRoot(pkgRoot, "not-daisyclaw");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "daisyclaw.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "daisyclaw");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
