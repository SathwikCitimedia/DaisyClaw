// Resolves the DaisyClaw package root from runtime and package metadata.
import path from "node:path";
import { fileURLToPath } from "node:url";
import { daisyClawRootFs, daisyClawRootFsSync } from "./daisyclaw-root.fs.runtime.js";

const CORE_PACKAGE_NAMES = new Set(["daisyclaw"]);
...
async function readPackageName(dir: string): Promise<string | null> {
  ...
  try {
    const name = parsePackageName(await daisyClawRootFs.readFile(packageJsonPath, "utf-8"));
...
function readPackageNameSync(dir: string): string | null {
  ...
  try {
    const name = parsePackageName(daisyClawRootFsSync.readFileSync(packageJsonPath, "utf-8"));
    packageNameCache.set(packageJsonPath, name);
    return name;
  } catch {
    packageNameCache.set(packageJsonPath, null);
...
function candidateDirsFromArgv1(argv1: string): string[] {
  ...
  try {
    const resolved = daisyClawRootFsSync.realpathSync(normalized);
...
export async function resolveDaisyClawPackageRoot(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): Promise<string | null> {
...
export function resolveDaisyClawPackageRootSync(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): string | null {
...
export const testing = {
  clearDaisyClawPackageRootCaches(): void {
