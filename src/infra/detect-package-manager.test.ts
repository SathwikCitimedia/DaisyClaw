...
async function withPackageManagerRoot<T>(
  ...
  return await withTempDir({ prefix: "daisyclaw-detect-pm-" }, async (root) => {
...
async function writePublishedDaisyClawRoot(root: string): Promise<void> {
...
describe("detectPackageManager", () => {
  ...
  it("keeps pnpm-owned direct package roots that ship npm-shrinkwrap", async () => {
    await withTempDir({ prefix: "daisyclaw-detect-pm-pnpm-direct-" }, async (base) => {
      const nodeModulesRoot = path.join(base, "pnpm-global", "node_modules");
      const packageRoot = path.join(nodeModulesRoot, "daisyclaw");
      await writePublishedDaisyClawRoot(packageRoot);
  ...
  it("keeps pnpm-owned virtual-store package roots that ship npm-shrinkwrap", async () => {
    await withTempDir({ prefix: "daisyclaw-detect-pm-pnpm-virtual-" }, async (base) => {
  ...
  it("keeps bun-owned global package roots that ship npm-shrinkwrap", async () => {
    await withTempDir({ prefix: "daisyclaw-detect-pm-bun-" }, async (base) => {
      const bunInstall = path.join(base, "bun-home");
      await withEnvAsync({ BUN_INSTALL: bunInstall }, async () => {
        const packageRoot = path.join(bunInstall, "install", "global", "node_modules", "daisyclaw");
        await writePublishedDaisyClawRoot(packageRoot);
