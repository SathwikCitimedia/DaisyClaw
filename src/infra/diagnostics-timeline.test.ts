...
async function createTimelineEnv() {
  const dir = await mkdtemp(join(tmpdir(), "daisyclaw-diagnostics-timeline-"));
  tempDirs.push(dir);
  return {
    env: {
      DAISYCLAW_DIAGNOSTICS: "timeline",
      DAISYCLAW_DIAGNOSTICS_RUN_ID: "run-1",
      DAISYCLAW_DIAGNOSTICS_ENV: "env-1",
      DAISYCLAW_DIAGNOSTICS_TIMELINE_PATH: join(dir, "nested", "timeline.jsonl"),
...
describe("diagnostics timeline", () => {
  it("detects when timeline output is enabled", async () => {
    ...
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, DAISYCLAW_DIAGNOSTICS: "1" } })).toBe(true);
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, DAISYCLAW_DIAGNOSTICS: "yes" } })).toBe(
      true,
    );
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, DAISYCLAW_DIAGNOSTICS: "on" } })).toBe(
      true,
    );
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, DAISYCLAW_DIAGNOSTICS: "all" } })).toBe(
      true,
    );
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, DAISYCLAW_DIAGNOSTICS: "*" } })).toBe(true);
    expect(
      isDiagnosticsTimelineEnabled({
        env: { ...env, DAISYCLAW_DIAGNOSTICS: "diagnostics.timeline" },
      }),
    ).toBe(true);
    expect(
      isDiagnosticsTimelineEnabled({ env: { ...env, DAISYCLAW_DIAGNOSTICS: "telegram.http" } }),
    ).toBe(false);
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, DAISYCLAW_DIAGNOSTICS: "0" } })).toBe(
      false,
    );
    expect(
      isDiagnosticsTimelineEnabled({
        env: { ...env, DAISYCLAW_DIAGNOSTICS_TIMELINE_PATH: "" },
      }),
    ).toBe(false);
  });
  ...
  it("honors config diagnostics flags after config is available", async () => {
    ...
    delete envWithoutFlag.DAISYCLAW_DIAGNOSTICS;
    const configWithTimeline = { diagnostics: { flags: ["timeline"] } } as DaisyClawConfig;
    const configWithWildcard = { diagnostics: { flags: ["*"] } } as DaisyClawConfig;
    const configWithoutTimeline = { diagnostics: { flags: ["telegram.http"] } } as DaisyClawConfig;
  ...
  it("lets false-like env diagnostics disable config-enabled timeline output", async () => {
    ...
    expect(
      isDiagnosticsTimelineEnabled({
        ...
        env: { ...env, DAISYCLAW_DIAGNOSTICS: "0" },
      }),
    ).toBe(false);
  });
  ...
  it("writes JSONL diagnostic events with the stable envelope", async () => {
    ...
    expect(event?.schemaVersion).toBe("daisyclaw.diagnostics.v1");
  ...
  it("records span start and end events around successful work", async () => {
    ...
    delete configOnlyEnv.DAISYCLAW_DIAGNOSTICS;

    await expect(
      measureDiagnosticsTimelineSpan("runtimeDeps.stage", () => "ok", {
        phase: "startup",
        attributes: { pluginCount: 3 },
        config: { diagnostics: { flags: ["timeline"] } } as DaisyClawConfig,
        env: configOnlyEnv,
      }),
    ).resolves.toBe("ok");
