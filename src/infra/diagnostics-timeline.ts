...
const DAISYCLAW_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION = "daisyclaw.diagnostics.v1";
...
export function isDiagnosticsTimelineEnabled(options: DiagnosticsTimelineOptions = {}): boolean {
  ...
  return (
    (isDiagnosticFlagEnabled("timeline", config, env) ||
      ...
      isTruthyEnvValue(env.DAISYCLAW_DIAGNOSTICS)) &&
    typeof env.DAISYCLAW_DIAGNOSTICS_TIMELINE_PATH === "string" &&
    env.DAISYCLAW_DIAGNOSTICS_TIMELINE_PATH.trim().length > 0
  );
}
...
function serializeTimelineEvent(event: DiagnosticsTimelineEvent, env: NodeJS.ProcessEnv): string {
  const normalized = {
    schemaVersion: DAISYCLAW_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION,
    type: event.type,
    timestamp: event.timestamp ?? new Date().toISOString(),
    name: event.name,
    ...(env.DAISYCLAW_DIAGNOSTICS_RUN_ID ? { runId: env.DAISYCLAW_DIAGNOSTICS_RUN_ID } : {}),
    ...(env.DAISYCLAW_DIAGNOSTICS_ENV ? { envName: env.DAISYCLAW_DIAGNOSTICS_ENV } : {}),
    pid: process.pid,
...
export function emitDiagnosticsTimelineEvent(
  ...
  const path = env.DAISYCLAW_DIAGNOSTICS_TIMELINE_PATH?.trim();
  if (!path) {
    return;
  }
