...
const CREDENTIAL_AND_GATEWAY_ENV_KEYS = [
  ...
  "DAISYCLAW_LIVE_ANTHROPIC_KEYS",
  "DAISYCLAW_LIVE_GEMINI_KEY",
  "DAISYCLAW_LIVE_OPENAI_KEY",
  "DAISYCLAW_GATEWAY_TOKEN",
  "DAISYCLAW_GATEWAY_PASSWORD",
  "DAISYCLAW_GATEWAY_SECRET",
] as const;

const BUNDLED_TRUST_ROOT_ENV_LINES = [
  "DAISYCLAW_BROWSER_CONTROL_MODULE=data:text/javascript,boom",
  "DAISYCLAW_BUNDLED_HOOKS_DIR=./attacker-hooks",
  "DAISYCLAW_BUNDLED_PLUGINS_DIR=./attacker-plugins",
  "DAISYCLAW_BUNDLED_SKILLS_DIR=./attacker-skills",
  "DAISYCLAW_SKIP_BROWSER_CONTROL_SERVER=1",
] as const;
...
async function withDotEnvFixture(run: (fixture: DotEnvFixture) => Promise<void>) {
  const base = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-dotenv-test-"));
  const cwdDir = path.join(base, "cwd");
  const stateDir = path.join(base, "state");
  process.env.DAISYCLAW_STATE_DIR = stateDir;
...
describe("loadDotEnv", () => {
  it("loads ~/.daisyclaw/.env as fallback without overriding CWD .env", async () => {
  ...
  it("loads the Ubuntu gateway.env compatibility fallback after ~/.daisyclaw/.env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        process.env.HOME = base;
        const defaultStateDir = path.join(base, ".daisyclaw");
        process.env.DAISYCLAW_STATE_DIR = defaultStateDir;
        await writeEnvFile(path.join(defaultStateDir, ".env"), "FOO=from-global\n");
        await writeEnvFile(
          path.join(base, ".config", "daisyclaw", "gateway.env"),
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "DAISYCLAW_STATE_DIR=./evil-state",
            "DAISYCLAW_CONFIG_PATH=./evil-config.json",
        ...
        delete process.env.DAISYCLAW_CONFIG_PATH;
        ...
        expect(process.env.DAISYCLAW_STATE_DIR).toBe(stateDir);
        expect(process.env.DAISYCLAW_CONFIG_PATH).toBeUndefined();
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "DAISYCLAW_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "DAISYCLAW_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "DAISYCLAW_LIVE_GEMINI_KEY=sk-gemini-live",
            "DAISYCLAW_LIVE_OPENAI_KEY=sk-openai-live",
            "DAISYCLAW_GATEWAY_TOKEN=attacker-token",
            "DAISYCLAW_GATEWAY_PASSWORD=attacker-password",
            "DAISYCLAW_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
  ...
  it("blocks state-directory controls from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        ...
        delete process.env.DAISYCLAW_STATE_DIR;
        delete process.env.STATE_DIRECTORY;
        delete process.env.DAISYCLAW_CONFIG_PATH;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.DAISYCLAW_STATE_DIR).toBeUndefined();
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "DAISYCLAW_AGENT_DIR=./evil-agent",
            `DAISYCLAW_BUNDLED_PLUGINS_DIR=${bundledPluginsDir}`,
            "DAISYCLAW_OAUTH_DIR=./evil-oauth",
            "PI_CODING_AGENT_DIR=./evil-pi-agent",
          ].join("\n"),
        );

        delete process.env.DAISYCLAW_AGENT_DIR;
        delete process.env.DAISYCLAW_BUNDLED_PLUGINS_DIR;
        delete process.env.DAISYCLAW_OAUTH_DIR;
        delete process.env.PI_CODING_AGENT_DIR;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.DAISYCLAW_AGENT_DIR).toBeUndefined();
        expect(process.env.DAISYCLAW_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.DAISYCLAW_OAUTH_DIR).toBeUndefined();
        expect(process.env.PI_CODING_AGENT_DIR).toBeUndefined();
      });
    });
  });

  it("blocks DAISYCLAW_TEST_TAILSCALE_BINARY from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          "DAISYCLAW_TEST_TAILSCALE_BINARY=/tmp/attacker-tailscale\n",
        );

        delete process.env.DAISYCLAW_TEST_TAILSCALE_BINARY;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.DAISYCLAW_TEST_TAILSCALE_BINARY).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "DAISYCLAW_ALLOW_PLUGIN_INSTALL_OVERRIDES=1",
            'DAISYCLAW_PLUGIN_INSTALL_OVERRIDES={"codex":"npm-pack:/tmp/codex.tgz"}',
          ].join("\n"),
        );

        delete process.env.DAISYCLAW_ALLOW_PLUGIN_INSTALL_OVERRIDES;
        delete process.env.DAISYCLAW_PLUGIN_INSTALL_OVERRIDES;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.DAISYCLAW_ALLOW_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
        expect(process.env.DAISYCLAW_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "DAISYCLAW_PINNED_PYTHON=./attacker-python",
            "DAISYCLAW_PINNED_WRITE_PYTHON=./attacker-write-python",
          ].join("\n"),
        );

        delete process.env.DAISYCLAW_PINNED_PYTHON;
        delete process.env.DAISYCLAW_PINNED_WRITE_PYTHON;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.DAISYCLAW_PINNED_PYTHON).toBeUndefined();
        expect(process.env.DAISYCLAW_PINNED_WRITE_PYTHON).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "DAISYCLAW_PINNED_PYTHON=/trusted/python",
            "DAISYCLAW_PINNED_WRITE_PYTHON=/trusted/write-python",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        delete process.env.ANTHROPIC_BASE_URL;
        ...
        expect(process.env.DAISYCLAW_PINNED_PYTHON).toBe("/trusted/python");
        expect(process.env.DAISYCLAW_PINNED_WRITE_PYTHON).toBe("/trusted/write-python");
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "DAISYCLAW_LIVE_GEMINI_KEY=sk-gemini-live",
            "DAISYCLAW_LIVE_OPENAI_KEY=sk-openai-live",
            "DAISYCLAW_GATEWAY_TOKEN=trusted-token",
            "DAISYCLAW_GATEWAY_PASSWORD=trusted-password",
            "DAISYCLAW_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        clearEnv(CREDENTIAL_AND_GATEWAY_ENV_KEYS);
        ...
        expect(process.env.DAISYCLAW_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.DAISYCLAW_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.DAISYCLAW_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.DAISYCLAW_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.DAISYCLAW_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.DAISYCLAW_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.DAISYCLAW_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "DAISYCLAW_STATE_DIR=./evil-state\n");
...
describe("loadCliDotEnv", () => {
  it("blocks DAISYCLAW_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(path.join(cwdDir, ".env"), "DAISYCLAW_STATE_DIR=./evil-state\n");
  ...
  it("does not load gateway.env when DAISYCLAW_STATE_DIR is explicitly set", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        const customStateDir = path.join(base, "custom-state");
        process.env.HOME = base;
        process.env.DAISYCLAW_STATE_DIR = customStateDir;
        await writeEnvFile(
          path.join(base, ".config", "daisyclaw", "gateway.env"),
          "FOO=from-gateway\n",
        );
        ...
        expect(process.env.DAISYCLAW_STATE_DIR).toBe(customStateDir);
  ...
  it("keeps the legacy state-dir fallback for CLI dotenv loading", async () => {
    await withIsolatedEnvAndCwd(async () => {
      const base = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-dotenv-legacy-"));
      const cwdDir = path.join(base, "cwd");
      const legacyStateDir = path.join(base, ".clawdbot");
      process.env.HOME = base;
      delete process.env.DAISYCLAW_STATE_DIR;
      delete process.env.DAISYCLAW_TEST_FAST;
      await fs.mkdir(cwdDir, { recursive: true });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        const plugin: PluginManifestRecord = {
          ...
          manifestPath: "/plugins/runtime-cloud/daisyclaw.plugin.json",
          providerAuthEnvVars: {
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        const runtimeControlKeys = [
          ...
          "DAISYCLAW_MDNS_HOSTNAME",
          "DAISYCLAW_SESSION_CACHE_TTL_MS",
          "DAISYCLAW_UPDATE_PACKAGE_SPEC",
          "DAISYCLAW_GATEWAY_PORT",
          "DAISYCLAW_GATEWAY_URL",
          "DAISYCLAW_CLAWHUB_URL",
          "CLAWHUB_URL",
          "DAISYCLAW_CLAWHUB_TOKEN",
          "CLAWHUB_TOKEN",
          "CLAWHUB_AUTH_TOKEN",
          "CLAWHUB_CONFIG_PATH",
          "DAISYCLAW_DISABLE_BUNDLED_PLUGINS",
          "DAISYCLAW_ALLOW_INSECURE_PRIVATE_WS",
          "DAISYCLAW_BROWSER_EXECUTABLE_PATH",
          "EXAMPLE_API_HOST",
          "HOMEBREW_BREW_FILE",
          "HOMEBREW_PREFIX",
          "IRC_HOST",
          ...
          "DAISYCLAW_SKIP_CHANNELS",
          "DAISYCLAW_SKIP_PROVIDERS",
          "DAISYCLAW_SKIP_CRON",
          "DAISYCLAW_RAW_STREAM",
          "DAISYCLAW_RAW_STREAM_PATH",
          "DAISYCLAW_CACHE_TRACE",
          "DAISYCLAW_CACHE_TRACE_FILE",
          "DAISYCLAW_CACHE_TRACE_MESSAGES",
          "DAISYCLAW_CACHE_TRACE_PROMPT",
          "DAISYCLAW_CACHE_TRACE_SYSTEM",
          "DAISYCLAW_SHOW_SECRETS",
          "DAISYCLAW_PLUGIN_CATALOG_PATHS",
          "DAISYCLAW_MPM_CATALOG_PATHS",
          "DAISYCLAW_NODE_EXEC_HOST",
          "DAISYCLAW_NODE_EXEC_FALLBACK",
          "DAISYCLAW_ALLOW_PROJECT_LOCAL_BIN",
          "PATH",
          "HOMEBREW_BREW_FILE",
          "HOMEBREW_PREFIX",
          "SystemRoot",
  ...
  it("still allows user-defined non-control vars through workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        ...
        expect(process.env.APP_GITHUB_REPO).toBe("daisyclaw/daisyclaw");
