...
describe("exec approvals wildcard agent", () => {
  it("merges wildcard allowlist entries with agent entries", () => {
    ...
    const prevDaisyClawHome = process.env.DAISYCLAW_HOME;

    try {
      process.env.DAISYCLAW_HOME = dir;
      const approvalsPath = path.join(dir, ".daisyclaw", "exec-approvals.json");
      ...
      if (prevDaisyClawHome === undefined) {
        delete process.env.DAISYCLAW_HOME;
      } else {
        process.env.DAISYCLAW_HOME = prevDaisyClawHome;
      }
    }
  });
});
