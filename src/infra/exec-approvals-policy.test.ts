...
function expectMalformedAgentAskUsesDefaults(agentAsk: unknown): void {
  ...
  expectFields(summary.ask, {
    requested: "off",
    host: "always",
    hostSource: "~/.daisyclaw/exec-approvals.json defaults.ask",
    effective: "always",
...
describe("exec approvals policy helpers", () => {
  ...
  it("explains stricter host security and ask precedence", () => {
    ...
    expectFields(summary.security, {
      ...
      host: "allowlist",
      effective: "allowlist",
      hostSource: "~/.daisyclaw/exec-approvals.json defaults.security",
    ...
    expect(summary.askFallback).toEqual({
      effective: "deny",
      source: "~/.daisyclaw/exec-approvals.json defaults.askFallback",
    });
  });
  ...
  it("uses DAISYCLAW_STATE_DIR when reporting default host sources", () => {
    const originalDaisyClawStateDir = process.env.DAISYCLAW_STATE_DIR;
    const stateDir = path.join(process.cwd(), ".tmp-daisyclaw-state");
  ...
  it("attributes host policy to wildcard agent entries before defaults", () => {
    ...
    expectFields(summary.security, {
      host: "allowlist",
      hostSource: "~/.daisyclaw/exec-approvals.json agents.*.security",
    });
    expectFields(summary.ask, {
      host: "always",
      hostSource: "~/.daisyclaw/exec-approvals.json agents.*.ask",
    });
    expect(summary.askFallback).toEqual({
      effective: "deny",
      source: "~/.daisyclaw/exec-approvals.json agents.*.askFallback",
    });
  });
  ...
  it("reports askFallback from the DaisyClaw default when approvals omit it", () => {
    const summary = resolveExecPolicyScopeSummary({
      approvals: {
        version: 1,
        agents: {},
  ...
  it("avoids a duplicate default-agent scope when main only appears in approvals", () => {
    ...
    expectFields(snapshots[0]?.security, {
      host: "allowlist",
      hostSource: "~/.daisyclaw/exec-approvals.json agents.main.security",
    });
    expectFields(snapshots[0]?.ask, {
      host: "always",
      hostSource: "~/.daisyclaw/exec-approvals.json agents.main.ask",
    });
  });
