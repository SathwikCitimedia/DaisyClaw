...
function createHomeDir(): string {
  ...
  tempDirs.push(dir);
  process.env.DAISYCLAW_HOME = dir;
  delete process.env.DAISYCLAW_STATE_DIR;
  return dir;
}

function approvalsFilePath(homeDir: string): string {
  return path.join(homeDir, ".daisyclaw", "exec-approvals.json");
}
...
describe("exec approvals store helpers", () => {
  it("expands home-prefixed default file and socket paths", () => {
    ...
    expect(path.normalize(resolveExecApprovalsPath())).toBe(
      path.normalize(path.join(dir, ".daisyclaw", "exec-approvals.json")),
    );
    expect(path.normalize(resolveExecApprovalsSocketPath())).toBe(
      path.normalize(path.join(dir, ".daisyclaw", "exec-approvals.sock")),
    );
    expect(resolveExecApprovalsDisplayPath()).toBe("~/.daisyclaw/exec-approvals.json");
  });

  it("uses DAISYCLAW_STATE_DIR for default file and socket paths", () => {
    const dir = createHomeDir();
    ...
    expect(resolveExecApprovalsTranscriptPath()).toBe("$DAISYCLAW_STATE_DIR/exec-approvals.json");
  ...
  it("fails closed without writing target approvals before state migration runs", () => {
    ...
    fs.writeFileSync(
      ...
      `${JSON.stringify({
        version: 1,
        socket: {
          path: path.join(dir, ".daisyclaw", "exec-approvals.sock"),
          token: "legacy-token",
        },
        defaults: {
          security: "deny",
  ...
  it("accepts a symlinked DAISYCLAW_HOME as the trusted approvals root", () => {
    ...
    process.env.DAISYCLAW_HOME = linkedHome;

    saveExecApprovals({ version: 1, defaults: { security: "full" }, agents: {} });

    expect(
      fs.readFileSync(path.join(realHome, ".daisyclaw", "exec-approvals.json"), "utf8"),
  ...
  it("refuses to traverse symlinked approvals components below a symlinked home", () => {
    ...
    fs.symlinkSync(realHome, linkedHome, "dir");
    fs.symlinkSync(linkedStateTarget, path.join(realHome, ".daisyclaw"), "dir");
    process.env.DAISYCLAW_HOME = linkedHome;
