...
const DEFAULT_EXEC_APPROVALS_STATE_DIR = "~/.daisyclaw";
...
export function resolveExecApprovalsTranscriptPath(): string {
  return process.env.DAISYCLAW_STATE_DIR?.trim()
    ? `$DAISYCLAW_STATE_DIR/${EXEC_APPROVALS_FILE}`
...
function hasUnmigratedLegacyExecApprovals(filePath: string): boolean {
  if (!process.env.DAISYCLAW_STATE_DIR?.trim()) {
    return false;
  }
...
function isReadOnlySecurityAuditSuppressionInspection(argv: string[]): boolean {
  ...
  let offset = command === "pnpm" && argv[1] === "daisyclaw" ? 1 : 0;
  if (normalizeCommandName(argv[offset]) !== "daisyclaw") {
    return false;
  }
  offset += 1;
