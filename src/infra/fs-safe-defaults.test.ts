// Covers DaisyClaw's default fs-safe Python helper configuration.
...
describe("fs-safe defaults", () => {
  afterEach(() => {
    ...
    delete process.env.DAISYCLAW_FS_SAFE_PYTHON_MODE;
  });

  it("disables the Python helper by default in DaisyClaw", async () => {
  ...
  it("honors the DaisyClaw-specific env mode override", async () => {
    process.env.DAISYCLAW_FS_SAFE_PYTHON_MODE = "auto";
