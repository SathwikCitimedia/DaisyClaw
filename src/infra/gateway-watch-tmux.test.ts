...
describe("gateway-watch tmux wrapper", () => {
  it("derives stable session names from profile and port", () => {
    expect(resolveGatewayWatchTmuxSessionName({ args: ["gateway", "--force"], env: {} })).toBe(
      "daisyclaw-gateway-watch-main",
    );
    expect(
      resolveGatewayWatchTmuxSessionName({
        args: ["gateway", "--force", "--port", "19001"],
        env: { DAISYCLAW_PROFILE: "Dev Profile" },
      }),
    ).toBe("daisyclaw-gateway-watch-dev-profile-19001");
    expect(
      resolveGatewayWatchTmuxSessionName({
        args: ["--dev", "gateway", "--port=18789"],
        env: {},
      }),
    ).toBe("daisyclaw-gateway-watch-dev");
  });
  ...
  it("builds a login-shell command that runs the raw watcher in the repo", () => {
    const command = buildGatewayWatchTmuxCommand({
      ...
      cwd: "/repo with spaces/daisyclaw",
      env: {
        DAISYCLAW_GATEWAY_PORT: "19001",
        DAISYCLAW_GATEWAY_RESTART_TRACE: "1",
        DAISYCLAW_GATEWAY_STARTUP_TRACE: "1",
        DAISYCLAW_PROFILE: "Dev Profile",
        DAISYCLAW_TRACE_SYNC_IO: "0",
        SHELL: "/bin/zsh",
      },
      nodePath: "/opt/node",
      sessionName: "daisyclaw-gateway-watch-main",
    });

    expect(command).toContain("exec '/bin/zsh' -lc");
    expect(command).toContain("/repo with spaces/daisyclaw");
    expect(command).toContain("'DAISYCLAW_GATEWAY_WATCH_TMUX_CHILD=1'");
    expect(command).toContain("'DAISYCLAW_GATEWAY_WATCH_SESSION=daisyclaw-gateway-watch-main'");
    expect(command).toContain("'\\''-u'\\'' '\\''NO_COLOR'\\''");
    expect(command).toContain("'FORCE_COLOR=1'");
    expect(command).toContain("'DAISYCLAW_GATEWAY_PORT=19001'");
    expect(command).toContain("'DAISYCLAW_GATEWAY_RESTART_TRACE=1'");
    expect(command).toContain("'DAISYCLAW_GATEWAY_STARTUP_TRACE=1'");
    expect(command).toContain("'DAISYCLAW_PROFILE=Dev Profile'");
    expect(command).toContain("'DAISYCLAW_TRACE_SYNC_IO=0'");
  ...
  it("consumes benchmark flags and passes the CPU profile dir to the watched child", () => {
    ...
    expect(command).toContain("'DAISYCLAW_RUN_NODE_CPU_PROF_DIR=.artifacts/gateway-watch-profiles'");
    expect(command).toContain("'DAISYCLAW_RUN_NODE_CPU_PROF_MAX_FILES=40'");
    expect(command).toContain("'DAISYCLAW_TRACE_SYNC_IO=0'");
  ...
  it("preserves an explicit benchmark CPU profile retention cap", () => {
    ...
    const code = runGatewayWatchTmuxMain({
      args: ["gateway", "--force", "--benchmark"],
      cwd: "/repo",
      env: { DAISYCLAW_RUN_NODE_CPU_PROF_MAX_FILES: "8", SHELL: "/bin/zsh" },
      nodePath: "/node",
      spawnSync,
      stderr: stderr.stream,
      stdout: stdout.stream,
    ...
    expect(code).toBe(0);
    const command = spawnShellCommand(spawnSync, 1);
    expect(command).toContain("'DAISYCLAW_RUN_NODE_CPU_PROF_MAX_FILES=8'");
  });
  ...
  it("preserves explicit sync I/O tracing in benchmark mode", () => {
    ...
    const code = runGatewayWatchTmuxMain({
      args: ["gateway", "--force", "--benchmark"],
      cwd: "/repo",
      env: { DAISYCLAW_TRACE_SYNC_IO: "1", SHELL: "/bin/zsh" },
      nodePath: "/node",
      spawnSync,
      stderr: stderr.stream,
      stdout: stdout.stream,
    ...
    expect(code).toBe(0);
    const command = spawnShellCommand(spawnSync, 1);
    expect(command).toContain("'DAISYCLAW_TRACE_SYNC_IO=1'");
    expect(command).toContain(
      "'DAISYCLAW_RUN_NODE_OUTPUT_LOG=.artifacts/gateway-watch-profiles/gateway-watch-output.log'",
    );
    expect(command).toContain("'DAISYCLAW_RUN_NODE_FILTER_SYNC_IO_STDERR=1'");
    expect(stderr.chunks.join("")).toContain(
  ...
  it("creates a detached tmux session when none exists", () => {
    ...
    expect(code).toBe(0);
    expect(
      expectSpawn(spawnSync, 0, "tmux", ["has-session", "-t", "daisyclaw-gateway-watch-main"])
        .encoding,
    ).toBe("utf8");
    ...
    expect(
      expectSpawn(spawnSync, 3, "tmux", [
        ...
        "-t",
        "daisyclaw-gateway-watch-main",
        "DAISYCLAW_GATEWAY_WATCH_CWD",
        "/repo",
      ]).encoding,
    ).toBe("utf8");
    expect(stderr.chunks.join("")).toContain(
      "gateway:watch started in tmux session daisyclaw-gateway-watch-main",
    );
    expect(stdout.chunks.join("")).toContain("tmux attach -t daisyclaw-gateway-watch-main");
    expect(stdout.chunks.join("")).toContain(
      "tmux show-options -v -t daisyclaw-gateway-watch-main @daisyclaw.gateway_watch.cwd",
    );
  });
  ...
  it("auto-attaches in an interactive terminal after creating a session", () => {
    ...
    expect(code).toBe(0);
    expect(
      expectSpawn(spawnSync, 4, "tmux", ["attach-session", "-t", "daisyclaw-gateway-watch-main"])
        .stdio,
    ).toBe("inherit");
  ...
  it("switches tmux clients instead of nesting attach when already inside tmux", () => {
    ...
    expect(code).toBe(0);
    expect(
      expectSpawn(spawnSync, 4, "tmux", ["switch-client", "-t", "daisyclaw-gateway-watch-main"])
        .stdio,
    ).toBe("inherit");
  });
  ...
  it("respawns the existing tmux pane on repeated runs", () => {
    ...
    const code = runGatewayWatchTmuxMain({
      args: ["gateway", "--force", "--port=19001"],
      cwd: "/repo",
      env: { DAISYCLAW_PROFILE: "dev", SHELL: "/bin/zsh" },
      nodePath: "/node",
      spawnSync,
      stderr: stderr.stream,
      stdout: stdout.stream,
    ...
    expect(respawnArgs.slice(0, 6)).toEqual([
      ...
      "-k",
      "-t",
      "daisyclaw-gateway-watch-dev-19001",
      "-c",
      "/repo",
    ]);
    ...
    expect(stderr.chunks.join("")).toContain(
      "gateway:watch restarted in tmux session daisyclaw-gateway-watch-dev-19001",
    );
  });
  ...
  it("recreates a stale session when its active pane target is missing", () => {
    ...
    expect(
      expectSpawn(spawnSync, 2, "tmux", ["kill-session", "-t", "daisyclaw-gateway-watch-main"])
        .encoding,
    ).toBe("utf8");
  ...
  it("runs the raw foreground watcher when tmux mode is disabled", () => {
    ...
    const code = runGatewayWatchTmuxMain({
      args: ["gateway", "--force"],
      cwd: "/repo",
      env: { DAISYCLAW_GATEWAY_WATCH_TMUX: "0" },
      nodePath: "/node",
      spawnSync,
    });
