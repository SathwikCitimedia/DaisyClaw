...
import { timestampMsToIsoString } from "@daisyclaw/normalization-core/number-coercion";
import {
...
function resolveHeartbeatTimeoutOverrideSeconds(cfg: DaisyClawConfig, heartbeat?: HeartbeatConfig) {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: DaisyClawConfig) => void;
};
...
function hasExplicitHeartbeatAgents(cfg: DaisyClawConfig) {
...
function resolveHeartbeatAgents(cfg: DaisyClawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
function resolveHeartbeatPromptRaw(cfg: DaisyClawConfig, heartbeat?: HeartbeatConfig) {
  return heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt;
}

export function resolveHeartbeatPrompt(cfg: DaisyClawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(resolveHeartbeatPromptRaw(cfg, heartbeat));
}

function resolveHeartbeatResponseToolPrompt(cfg: DaisyClawConfig, heartbeat?: HeartbeatConfig) {
...
function resolveHeartbeatAckMaxChars(cfg: DaisyClawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
function isHeartbeatTypingEnabled(params: { cfg: DaisyClawConfig; hasChatDelivery: boolean }) {
...
function resolveHeartbeatTypingIntervalSeconds(cfg: DaisyClawConfig) {
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: DaisyClawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
