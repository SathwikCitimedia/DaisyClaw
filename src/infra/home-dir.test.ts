// Tests DaisyClaw home directory resolution.
...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers DAISYCLAW_HOME over HOME and USERPROFILE",
      env: {
  ...
  it("expands DAISYCLAW_HOME when set to ~", () => {
    const env = {
      DAISYCLAW_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for DAISYCLAW_HOME",
      env: { DAISYCLAW_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when DAISYCLAW_HOME is tilde-only and no fallback home exists",
      env: { DAISYCLAW_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores DAISYCLAW_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          DAISYCLAW_HOME: "/srv/daisyclaw-home",
  ...
  it.each([
    {
      ...
      expected: `${path.resolve("/srv/daisyclaw-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/daisyclaw-home " },
      expected: "/srv/daisyclaw-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/daisyclaw-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of DAISYCLAW_HOME", () => {
    expect(
