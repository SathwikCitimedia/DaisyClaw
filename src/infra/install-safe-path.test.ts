...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@daisyclaw/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@daisyclaw/matrix", id: "matrix", expected: true },
    { packageName: "@daisyclaw/matrix", id: "@daisyclaw/matrix", expected: true },
    { packageName: "@daisyclaw/matrix", id: "quietchat", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@daisyclaw/matrix", id: " ", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      ...
      ok: true,
      path: path.join("/tmp/plugins", "@daisyclaw__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTempDir({ prefix: "daisyclaw-install-safe-" }, async (baseDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "daisyclaw-install-safe-" }, async (baseDir) => {
        await withTempDir({ prefix: "daisyclaw-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "daisyclaw-install-safe-" }, async (parentDir) => {
