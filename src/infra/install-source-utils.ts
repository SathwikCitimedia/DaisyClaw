...
function normalizeNpmViewMetadata(value: unknown): NpmSpecResolution | null {
  ...
  return {
    ...
    ...(isRecord(rec.daisyclaw) ? { packageDaisyClaw: rec.daisyclaw } : {}),
  };
}
...
> {
  ...
  if (res.code !== 0) {
    ...
    if (/E404|is not in this registry/i.test(raw)) {
      return {
        ok: false,
        error: `Package not found on npm: ${params.spec}. See https://docs.daisyclaw.ai/tools/plugin for installable plugins.`,
      };
    }
