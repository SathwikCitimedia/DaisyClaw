...
const DAISYCLAW_DEBUG_PROXY_ENABLED = "DAISYCLAW_DEBUG_PROXY_ENABLED";
...
function isManagedProxyActive(): boolean {
  return process.env["DAISYCLAW_PROXY_ACTIVE"] === "1";
}
...
async function captureGuardedFetchExchange(params: {
  ...
  if (params.capture === false || !isTruthyEnvValue(process.env[DAISYCLAW_DEBUG_PROXY_ENABLED])) {
    return;
  }
