// Bridges DaisyClaw-managed proxy TLS trust into Undici EnvHttpProxyAgent and
// explicit ProxyAgent options without changing unrelated operator proxies.
import { isRecord as isProxyTlsRecord } from "@daisyclaw/normalization-core/record-coerce";
...
function resolveManagedProxyUrl(env: ManagedProxyTlsEnv = process.env): string | undefined {
  ...
  if (env["DAISYCLAW_PROXY_ACTIVE"] !== "1") {
    return undefined;
  }
...
/** Resolves managed proxy TLS trust only when the target proxy is DaisyClaw's active proxy. */
export function resolveActiveManagedProxyTlsOptions(
  ...
  const proxyCaFile = resolveManagedProxyCaFileForUrl({
    ...
    caFileOverride: env["DAISYCLAW_PROXY_CA_FILE"],
  });
  try {
