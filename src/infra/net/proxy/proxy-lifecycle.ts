...
function captureProxyEnv(): ProxyEnvSnapshot {
  return {
    ...
    NO_PROXY: process.env["NO_PROXY"],
    DAISYCLAW_PROXY_ACTIVE: process.env["DAISYCLAW_PROXY_ACTIVE"],
    DAISYCLAW_PROXY_LOOPBACK_MODE: process.env["DAISYCLAW_PROXY_LOOPBACK_MODE"],
    DAISYCLAW_PROXY_CA_FILE: process.env["DAISYCLAW_PROXY_CA_FILE"],
  };
}
...
function applyProxyEnv(
  ...
  process.env["DAISYCLAW_PROXY_LOOPBACK_MODE"] = loopbackMode;
  if (proxyCaFile) {
    process.env["DAISYCLAW_PROXY_CA_FILE"] = proxyCaFile;
  } else {
    delete process.env["DAISYCLAW_PROXY_CA_FILE"];
  }
...
function resolveProxyUrl(config: ProxyConfig | undefined): string {
  const candidate = config?.proxyUrl?.trim() || process.env["DAISYCLAW_PROXY_URL"]?.trim();
  if (!candidate) {
    throw new Error(
      "proxy: enabled but no HTTP proxy URL is configured; set proxy.proxyUrl " +
        "or DAISYCLAW_PROXY_URL to an http:// or https:// forward proxy.",
    );
  }
...
export function ensureInheritedManagedProxyRoutingActive(): void {
  if (process.env["DAISYCLAW_PROXY_ACTIVE"] !== "1") {
    return;
  }
  ...
  const proxyCaFile = resolveManagedProxyCaFileForUrl({
    proxyUrl,
    caFileOverride: process.env["DAISYCLAW_PROXY_CA_FILE"],
  });
 ...
 * loopback CDP probe to a Chromium instance DaisyClaw spawned itself.
 *
