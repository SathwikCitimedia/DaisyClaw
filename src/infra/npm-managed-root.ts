...
import { normalizeOptionalString as readOptionalString } from "@daisyclaw/normalization-core/string-coerce";
...
import { resolveDaisyClawPackageRootSync } from "./daisyclaw-root.js";
...
type ManagedNpmRootDaisyClawMetadata = {
...
type ManagedNpmRootDaisyClawHostState = "none" | "managed-active-host" | "linked-active-host";
...
function buildManagedDaisyClawMetadata(params: {
  current: unknown;
  managedOverrideKeys: string[];
  managedPeerDependencyKeys?: string[];
}): ManagedNpmRootDaisyClawMetadata | undefined {
  const metadata: ManagedNpmRootDaisyClawMetadata = isRecord(params.current)
    ? { ...params.current }
    : {};
...
/** Read host DaisyClaw pnpm overrides for reuse inside a managed npm root. */
export async function readDaisyClawManagedNpmRootOverrides(params?: {
  argv1?: string;
  cwd?: string;
  moduleUrl?: string;
  ...
  const packageRoot =
    params?.packageRoot ??
    resolveDaisyClawPackageRootSync({
...
export async function upsertManagedNpmRootDependency(params: {
  ...
  for (const key of readManagedOverrideKeys(manifest.daisyclaw)) {
    delete overrides[key];
  }
  Object.assign(overrides, managedOverrides);
  const daisyclawMetadata = buildManagedDaisyClawMetadata({
    current: manifest.daisyclaw,
    managedOverrideKeys,
  });
  ...
  if (daisyclawMetadata) {
    next.daisyclaw = daisyclawMetadata;
  } else {
    delete next.daisyclaw;
  }
...
function scrubHostPeerFromLockPackage(value: unknown): boolean {
  ...
  let changed = false;
  if (isRecord(value.peerDependencies) && "daisyclaw" in value.peerDependencies) {
    const peerDependencies = { ...value.peerDependencies };
    delete peerDependencies.daisyclaw;
  ...
  if (isRecord(value.peerDependenciesMeta) && "daisyclaw" in value.peerDependenciesMeta) {
    const peerDependenciesMeta = { ...value.peerDependenciesMeta };
    delete peerDependenciesMeta.daisyclaw;
...
function isHostPeerResolutionFailure(
  ...
  return /(^|[^@\w.-])daisyclaw(?=$|[@\s:,"'])/i.test(output);
}
...
async function collectNpmResolvedManagedNpmRootPeerDependencyPins(params: {
  ...
  const dependencies = readDependencyRecord(manifest.dependencies);
  const previousManagedPeerDependencies = readManagedPeerDependencyKeys(manifest.daisyclaw);
  ...
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-managed-peer-plan-"));
  try {
    delete dependencies.daisyclaw;
    await writeJson(
    ...
    await copyPathIfExists(
      path.join(params.npmRoot, "_daisyclaw-pack-archives"),
      path.join(tempRoot, "_daisyclaw-pack-archives"),
    );
...
export async function readManagedNpmRootPeerDependencySnapshot(params: {
  ...
  const manifest = await readManagedNpmRootManifest(path.join(params.npmRoot, "package.json"));
  const dependencies = readDependencyRecord(manifest.dependencies);
  const managedPeerDependencies = readManagedPeerDependencyKeys(manifest.daisyclaw).toSorted();
...
export async function restoreManagedNpmRootPeerDependencySnapshot(params: {
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  for (const packageName of readManagedPeerDependencyKeys(manifest.daisyclaw)) {
    delete dependencies[packageName];
  }
  Object.assign(dependencies, params.snapshot.dependencies);
  const managedOverrideKeys = readManagedOverrideKeys(manifest.daisyclaw).toSorted();
  const daisyclawMetadata = buildManagedDaisyClawMetadata({
    current: manifest.daisyclaw,
    managedOverrideKeys,
...
/** Remove stale managed-root daisyclaw peer installs while preserving active host links. */
export async function repairManagedNpmRootDaisyClawPeer(params: {
  npmRoot: string;
  packageRoot?: string | null;
  timeoutMs?: number;
  ...
  const activeHostState = await readManagedNpmRootDaisyClawHostState({
    npmRoot: params.npmRoot,
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  const hasManifestDependency = "daisyclaw" in dependencies;
  const hasLockDependency = await managedNpmRootLockfileHasDaisyClawPeer(params.npmRoot);
  const hasPackageDir = await pathExists(path.join(params.npmRoot, "node_modules", "daisyclaw"));
  ...
  if (preserveActiveHostLink) {
    await scrubManagedNpmRootDaisyClawPeer({
      npmRoot: params.npmRoot,
  ...
  try {
    ...
    if (result.code !== 0) {
      params.logger?.warn?.(
        `npm ${hasManifestDependency ? "uninstall daisyclaw" : "prune"} failed while repairing managed npm root; falling back to direct cleanup: ${result.stderr.trim() || result.stdout.trim()}`,
      );
    }
  } catch (error) {
    params.logger?.warn?.(
      `npm ${hasManifestDependency ? "uninstall daisyclaw" : "prune"} failed while repairing managed npm root; falling back to direct cleanup: ${String(error)}`,
    );
  }

  await scrubManagedNpmRootDaisyClawPeer({ npmRoot: params.npmRoot });
  return true;
}

async function readManagedNpmRootDaisyClawHostState(params: {
  npmRoot: string;
  packageRoot?: string | null;
}): Promise<ManagedNpmRootDaisyClawHostState> {
  const packageRoot =
  ...
  const managedDaisyClawPackageDir = path.join(params.npmRoot, "node_modules", "daisyclaw");
  const [hostPackageRoot, managedPackageRoot, managedPackageStat] = await Promise.all([
    realpathIfExists(packageRoot),
    realpathIfExists(managedDaisyClawPackageDir),
    lstatIfExists(managedDaisyClawPackageDir),
  ]);
...
async function managedNpmRootLockfileHasDaisyClawPeer(npmRoot: string): Promise<boolean> {
  ...
  try {
    ...
    if (isRecord(parsed.packages)) {
      ...
      if (
        ...
        "daisyclaw" in rootPackage.dependencies
      ) {
        return true;
      }
      if ("node_modules/daisyclaw" in parsed.packages) {
        return true;
      }
    }
    return isRecord(parsed.dependencies) && "daisyclaw" in parsed.dependencies;
  } catch (err) {
    if ((err as NodeJS.ErrnoException).code === "ENOENT") {
      return false;
    }
...
async function scrubManagedNpmRootDaisyClawPeer(params: {
  npmRoot: string;
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  if ("daisyclaw" in dependencies) {
    const { daisyclaw: _removed, ...nextDependencies } = dependencies;
    await fs.writeFile(
      manifestPath,
  ...
  try {
    ...
    if (isRecord(parsed.packages)) {
      ...
      if (isRecord(rootPackage) && isRecord(rootPackage.dependencies)) {
        ...
        if ("daisyclaw" in dependenciesValue) {
          delete dependenciesValue.daisyclaw;
          parsed.packages[""] = { ...rootPackage, dependencies: dependenciesValue };
          lockChanged = true;
        }
      }
      if ("node_modules/daisyclaw" in parsed.packages) {
        delete parsed.packages["node_modules/daisyclaw"];
        lockChanged = true;
      }
    }
    if (isRecord(parsed.dependencies) && "daisyclaw" in parsed.dependencies) {
      const dependenciesLocal = { ...parsed.dependencies };
      delete dependenciesLocal.daisyclaw;
  ...
  const daisyclawPackageDir = path.join(params.npmRoot, "node_modules", "daisyclaw");
  if (!params.preservePackageDir && (await pathExists(daisyclawPackageDir))) {
    await fs.rm(daisyclawPackageDir, { recursive: true, force: true });
  }
  const binDir = path.join(params.npmRoot, "node_modules", ".bin");
  await Promise.all(
    ["daisyclaw", "daisyclaw.cmd", "daisyclaw.ps1"].map((binName) =>
