  ...
  resolveMissingOfficialExternalChannelPluginRepairHint: ({ channelId }: { channelId: string }) =>
    mocks.missingOfficialExternalChannels.has(channelId)
      ? {
          ...
          installSpec: `@daisyclaw/${channelId}`,
          installCommand: `daisyclaw plugins install @daisyclaw/${channelId}`,
          doctorFixCommand: "daisyclaw doctor --fix",
          repairHint: `Install the official external plugin with: daisyclaw plugins install @daisyclaw/${channelId}, or run: daisyclaw doctor --fix.`,
        }
      : null,
}));
...
describe("resolveMessageChannelSelection", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      expectedMessage:
        "Channel is unavailable: feishu. Install the official external plugin with: daisyclaw plugins install @daisyclaw/feishu, or run: daisyclaw doctor --fix.",
    },
    {
      params: { cfg: {} as never },
      expectedMessage:
        "Channel is required (no configured channels detected). Run daisyclaw channels add to configure one",
    },
    {
      setup: () => {
        mocks.resolveOutboundChannelPlugin.mockReturnValue(undefined);
      ...
      expectedMessage:
        "Channel is required (no available channels detected). Configured official external channel WhatsApp is missing its plugin. Install the official external plugin with: daisyclaw plugins install @daisyclaw/whatsapp, or run: daisyclaw doctor --fix.",
    },
    {
      setup: () => {
        mocks.listChannelPlugins.mockReturnValue([
