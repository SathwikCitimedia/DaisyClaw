...
function resolveAvailableKnownChannel(params: {
  ...
  // been eagerly loaded (e.g. `daisyclaw agent --local`). Already-loaded and
...
export function isConfiguredChannel(cfg: DaisyClawConfig, channelId: string): boolean {
...
function formatMissingOfficialExternalChannelsMessage(
  ...
  return `Configured official external channels ${labels} are missing their plugins. Run: daisyclaw doctor --fix, or install individually: ${installCommands}.`;
}

function formatNoConfiguredChannelsMessage(): string {
  return [
    "Channel is required (no configured channels detected).",
    "Run daisyclaw channels add to configure one, or pass --channel <channel> after enabling a channel.",
    "Use daisyclaw channels list --all to see available channel ids.",
  ].join(" ");
}
...
async function isPluginConfigured(plugin: ChannelPlugin, cfg: DaisyClawConfig): Promise<boolean> {
