...
export function installDeliveryQueueTmpDirHooks(): { readonly tmpDir: () => string } {
  ...
  beforeAll(() => {
    fixtureRoot = fs.mkdtempSync(path.join(resolvePreferredDaisyClawTmpDir(), "daisyclaw-dq-suite-"));
  });
...
export function readQueuedEntry(tmpDir: string, id: string): Record<string, unknown> {
  const { db } = openDaisyClawStateDatabase({ env: { ...process.env, DAISYCLAW_STATE_DIR: tmpDir } });
  const row = db
    .prepare(
