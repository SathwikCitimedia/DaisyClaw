...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: DaisyClawConfig | null = null;
   ...
   */
  get(key: string, cfg: DaisyClawConfig): T | undefined {
   ...
   */
  set(key: string, value: T, cfg: DaisyClawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
   ...
   */
  clear(cfg?: DaisyClawConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: DaisyClawConfig): void {
