...
async function withTempDaisyClawStateDir<T>(test: (stateDir: string) => Promise<T>): Promise<T> {
...
describe("runMessageAction media behavior", () => {
  ...
  it("materializes buffer-only send attachments into outbound media paths", async () => {
    ...
    await withTempDaisyClawStateDir(async () => {
  ...
  it("does not stage buffer-only send attachments before target validation passes", async () => {
    ...
    await withTempDaisyClawStateDir(async (stateDir) => {
      await expect(
        runMessageAction({
          cfg: workspaceConfig,
          action: "send",
  ...
  describe("reply hydration", () => {
    ...
    // params as send. Before daisyclaw#79864 the runner only hydrated
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred DaisyClaw tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideDaisyClaw = path.join(os.tmpdir(), "outside-daisyclaw", "test-media.png");
        await expect(
          runMessageAction({
            cfg: workspaceConfig,
            action: "send",
