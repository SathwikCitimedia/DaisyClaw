...
function createNpmTarget(globalRoot: string): ResolvedGlobalInstallTarget {
  return {
    ...
    command: "npm",
    globalRoot,
    packageRoot: path.join(globalRoot, "daisyclaw"),
  };
}
...
describe("runGlobalPackageUpdateSteps", () => {
  it("installs npm updates into a clean staged prefix before swapping the global package", async () => {
    await withTempDir({ prefix: "daisyclaw-package-update-staged-" }, async (base) => {
      ...
      const runStep = vi.fn(
        async ({ name, argv, cwd, timeoutMs }): Promise<PackageUpdateStepResult> => {
          ...
          await fs.symlink(
            "../lib/node_modules/daisyclaw/dist/index.js",
            path.join(stagePrefix, "bin", "daisyclaw"),
          );
          return {
            name,
            command: argv.join(" "),
      ...
      await expect(fs.readlink(path.join(prefix, "bin", "daisyclaw"))).resolves.toBe(
        "../lib/node_modules/daisyclaw/dist/index.js",
      );
    });
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "daisyclaw-package-update-hardlinks-" }, async (base) => {
        const prefix = path.join(base, "prefix");
        const globalRoot = path.join(prefix, "lib", "node_modules");
        const packageRoot = path.join(globalRoot, "daisyclaw");
        await writePackageRoot(packageRoot, "1.0.0");
        ...
        const result = await runGlobalPackageUpdateSteps({
          ...
          runStep: async ({ name, argv, cwd }): Promise<PackageUpdateStepResult> => {
            ...
            const stagedPackageRoot = path.join(stagePrefix, "lib", "node_modules", "daisyclaw");
  ...
  it("swaps staged npm updates into an explicitly selected direct node_modules root", async () => {
    await withTempDir({ prefix: "daisyclaw-package-update-direct-root-" }, async (base) => {
      const managedRoot = path.join(base, ".daisyclaw", "npm", "node_modules");
      const packageRoot = path.join(managedRoot, "daisyclaw");
      await writePackageRoot(packageRoot, "1.0.0");
      ...
      const runStep = vi.fn(async ({ name, argv, cwd }): Promise<PackageUpdateStepResult> => {
        ...
        await writePackageRoot(path.join(stagePrefix, "lib", "node_modules", "daisyclaw"), "2.0.0");
        await fs.mkdir(path.join(stagePrefix, "bin"), { recursive: true });
        await fs.symlink(
          "../lib/node_modules/daisyclaw/dist/index.js",
          path.join(stagePrefix, "bin", "daisyclaw"),
      ...
      await expectPathMissing(path.join(managedRoot, ".bin", "daisyclaw"));
    });
  });

  it("accepts v-prefixed exact npm specs when verifying staged installs", async () => {
    await withTempDir({ prefix: "daisyclaw-package-update-v-prefix-" }, async (base) => {
      const prefix = path.join(base, "prefix");
      const globalRoot = path.join(prefix, "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "daisyclaw");
      await writePackageRoot(packageRoot, "1.0.0");
      ...
      const runStep = vi.fn(async ({ name, argv, cwd }): Promise<PackageUpdateStepResult> => {
        ...
        expect(argv).toContain("daisyclaw@v2.0.0");
        const prefixIndex = argv.indexOf("--prefix");
        const stagePrefix = argv[prefixIndex + 1];
        if (!stagePrefix) {
          throw new Error("missing staged prefix");
  ...
  it("packs npm GitHub specs before installing into the staged prefix", async () => {
    await withTempDir({ prefix: "daisyclaw-package-update-npm-pack-" }, async (base) => {
      const prefix = path.join(base, "prefix");
      const globalRoot = path.join(prefix, "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "daisyclaw");
      const sourceSpec = "DaisyClaw@github:daisyclaw/daisyclaw#release/2026.5.12";
      await writePackageRoot(packageRoot, "1.0.0");
      ...
      const runStep = vi.fn(async ({ name, argv, cwd }): Promise<PackageUpdateStepResult> => {
        ...
        expect(argv).toEqual([
          ...
          "--prefix",
          stagePrefix,
          path.join(packDir, "daisyclaw-2.0.0.tgz"),
          "--no-fund",
          "--no-audit",
          "--loglevel=error",
          "--min-release-age=0",
  ...
  it.each([
    {
      name: "full git url",
      sourceSpec: "https://github.com/daisyclaw/daisyclaw.git#main",
    },
    {
      name: "hosted GitHub URL without git suffix",
      sourceSpec: "https://github.com/daisyclaw/daisyclaw#main",
    },
    {
      name: "aliased hosted GitHub URL without git suffix",
      sourceSpec: "daisyclaw@https://github.com/daisyclaw/daisyclaw#main",
    },
    {
      name: "GitHub shorthand",
      sourceSpec: "daisyclaw/daisyclaw#main",
    },
    {
      name: "SCP-style SSH",
      sourceSpec: "git@github.com:daisyclaw/daisyclaw.git#main",
    },
  ] as const)(
    "packs additional npm git source spec forms before install: $name",
    async ({ sourceSpec }) => {
      await withTempDir({ prefix: "daisyclaw-package-update-npm-pack-variant-" }, async (base) => {
        ...
        const runStep = vi.fn(async ({ name, argv, cwd }): Promise<PackageUpdateStepResult> => {
          if (name === "global update pack") {
            ...
            tarball = path.join(destination, "daisyclaw-2.0.0.tgz");
  ...
  it("swaps staged npm package roots through the copy fallback when rename crosses devices", async () => {
    await withTempDir({ prefix: "daisyclaw-package-update-exdev-" }, async (base) => {
      const prefix = path.join(base, "prefix");
      const globalRoot = path.join(prefix, "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "daisyclaw");
      ...
      const renameSpy = vi
        ...
        .mockImplementation(async (...args: Parameters<typeof fs.rename>) => {
          ...
          if (
            exdevMoves === 0 &&
            fromPath.includes(`${path.sep}.daisyclaw-update-stage-`) &&
            path.basename(fromPath) === "daisyclaw" &&
      ...
      try {
        const result = await runGlobalPackageUpdateSteps({
          ...
          runStep: async ({ name, argv, cwd }) => {
            ...
            await writePackageRoot(path.join(stageLayout.globalRoot, "daisyclaw"), "2.0.0");
            return {
              name,
              command: argv.join(" "),
              cwd: cwd ?? process.cwd(),
  ...
  it("stages pnpm-detected updates through npm when the global root has npm prefix layout", async () => {
    await withTempDir({ prefix: "daisyclaw-package-update-pnpm-staged-" }, async (base) => {
      const prefix = path.join(base, "prefix");
      const globalRoot = path.join(prefix, "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "daisyclaw");
      ...
      const runStep = vi.fn(async ({ name, argv, cwd }): Promise<PackageUpdateStepResult> => {
        ...
        expect(argv).toContain("daisyclaw@2.0.0");
  ...
  it("keeps Windows pnpm global roots on the pnpm update path", async () => {
    ...
    try {
      await withTempDir({ prefix: "daisyclaw-package-update-win32-pnpm-" }, async (base) => {
        ...
        const runStep = vi.fn(async ({ name, argv, cwd }): Promise<PackageUpdateStepResult> => {
          ...
          expect(argv).toEqual(["pnpm", "add", "-g", "--global-dir", globalDir, "daisyclaw@2.0.0"]);
  ...
  it("keeps a successful staged swap when old package cleanup hits a transient Windows native module error", async () => {
    await withTempDir({ prefix: "daisyclaw-package-update-staged-cleanup-" }, async (base) => {
      const prefix = path.join(base, "prefix");
      const globalRoot = path.join(prefix, "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "daisyclaw");
      await writePackageRoot(packageRoot, "1.0.0");
      ...
      const rmSpy = vi.spyOn(fs, "rm").mockImplementation(async (target, options) => {
        ...
        if (
          targetPath.includes(`${path.sep}.daisyclaw-`) &&
          !targetPath.includes(".daisyclaw-update-stage-") &&
          !targetPath.includes(".daisyclaw-shim-backup-")
        ) {
  ...
  it("does not run post-verify work when staged npm verification fails", async () => {
    await withTempDir({ prefix: "daisyclaw-package-update-verify-" }, async (base) => {
      const prefix = path.join(base, "prefix");
      const globalRoot = path.join(prefix, "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "daisyclaw");
      await writePackageRoot(packageRoot, "1.0.0");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "daisyclaw-package-update-shim-rollback-" }, async (base) => {
        const prefix = path.join(base, "prefix");
        const globalRoot = path.join(prefix, "lib", "node_modules");
        const packageRoot = path.join(globalRoot, "daisyclaw");
        const targetShim = path.join(prefix, "bin", "daisyclaw");
        await writePackageRoot(packageRoot, "1.0.0");
        ...
        try {
          result = await runGlobalPackageUpdateSteps({
            ...
            runStep: async ({ name, argv, cwd }) => {
              ...
              const stagedShim = path.join(stagePrefix, "bin", "daisyclaw");
  ...
  it("cleans the staged npm prefix when the install command throws", async () => {
    await withTempDir({ prefix: "daisyclaw-package-update-cleanup-" }, async (base) => {
      const prefix = path.join(base, "prefix");
      const globalRoot = path.join(prefix, "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "daisyclaw");
      await writePackageRoot(packageRoot, "1.0.0");
