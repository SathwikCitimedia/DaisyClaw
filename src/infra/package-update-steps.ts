...
async function createStagedNpmInstall(
  ...
  const prefix = await fs.mkdtemp(path.join(targetLayout.globalRoot, ".daisyclaw-update-stage-"));
...
async function prepareNpmGitSourceInstallSpec(params: {
  ...
  const packDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-update-pack-"));
...
async function replaceNpmBinShims(params: {
  ...
  const names = new Set([params.packageName, "daisyclaw"]);
  ...
  const backup: NpmBinShimBackup = {
    backupDir: await fs.mkdtemp(
      path.join(params.targetLayout.globalRoot, ".daisyclaw-shim-backup-"),
    ),
...
async function swapStagedNpmInstall(params: {
  ...
  const backupRoot = path.join(targetLayout.globalRoot, `.daisyclaw-${process.pid}-${Date.now()}`);
