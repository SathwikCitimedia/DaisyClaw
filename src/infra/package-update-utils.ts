...
/** Return true when an installed package needs an daisyclaw peer link repair. */
export function installedPackageNeedsDaisyClawPeerLinkRepair(dir: string): boolean {
  ...
  try {
    fsSync.statSync(path.join(dir, "node_modules", "daisyclaw"));
    return false;
  } catch (error) {
