...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/daisyclaw-state", "devices")).toEqual({
      dir: path.join("/tmp/daisyclaw-state", "devices"),
      pendingPath: path.join("/tmp/daisyclaw-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/daisyclaw-state", "devices", "paired.json"),
    });
  });
