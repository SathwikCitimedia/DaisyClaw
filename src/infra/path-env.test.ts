// Covers DaisyClaw CLI PATH construction.
import path from "node:path";
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import { ensureDaisyClawCliOnPath } from "./path-env.js";
...
describe("ensureDaisyClawCliOnPath", () => {
  const envKeys = [
    "PATH",
    "DAISYCLAW_PATH_BOOTSTRAPPED",
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/daisyclaw-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "daisyclaw");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function bootstrapPath(params: {
    ...
    ensureDaisyClawCliOnPath(params);
    return (process.env.PATH ?? "").split(path.delimiter);
  }

  function resetBootstrapEnv(pathValue = "/usr/bin") {
    process.env.PATH = pathValue;
    delete process.env.DAISYCLAW_PATH_BOOTSTRAPPED;
    delete process.env.DAISYCLAW_ALLOW_PROJECT_LOCAL_BIN;
  ...
  it("prepends the bundled app bin dir when a sibling daisyclaw exists", () => {
  ...
  it("keeps the current runtime directory ahead of system PATH hardening", () => {
    const tmp = abs("/tmp/daisyclaw-path/case-runtime-dir");
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "daisyclaw");
      ...
      if (envValue === undefined) {
        ...
        process.env.DAISYCLAW_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
  ...
  it.each([
    ...
    },
    {
      ...
      setup: () => {
        const tmp = abs("/tmp/daisyclaw-path/case-linuxbrew");
