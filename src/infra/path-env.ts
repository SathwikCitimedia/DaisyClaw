// Builds PATH values for DaisyClaw child processes.
...
function candidateBinDirs(
  ...
  // subprocesses keep using the same Node/Bun the current DaisyClaw process is on.
  try {
  ...
  // Bundled macOS app: `daisyclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "daisyclaw");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.DAISYCLAW_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "daisyclaw"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `daisyclaw`
...
/**
 * Best-effort PATH bootstrap so skills that require the `daisyclaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureDaisyClawCliOnPath(opts: EnsureDaisyClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.DAISYCLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
  // Mark before filesystem probing so repeated calls from nested bootstraps do
  // not keep reshuffling PATH.
  process.env.DAISYCLAW_PATH_BOOTSTRAPPED = "1";
