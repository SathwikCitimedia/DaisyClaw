...
const MATRIX_REPO_INSTALL_COMMAND = `daisyclaw plugins install ${repoInstallSpec("matrix")}`;
...
describe("plugin install path warnings", () => {
  ...
  it("detects stale custom plugin install paths", async () => {
    const issue = await detectPluginInstallPathIssue({
      ...
      install: {
        source: "path",
        sourcePath: "/tmp/daisyclaw-matrix-missing",
        installPath: "/tmp/daisyclaw-matrix-missing",
      },
    });
    ...
    expect(issue).toEqual({
      kind: "missing-path",
      pluginId: "matrix",
      path: "/tmp/daisyclaw-matrix-missing",
    });
    expect(
      formatPluginInstallPathIssue({
        issue: issue!,
      ...
      "Matrix is installed from a custom path that no longer exists: /tmp/daisyclaw-matrix-missing",
      'Reinstall with "daisyclaw plugins install @daisyclaw/matrix".',
  ...
  it("uses the second candidate path when the first one is stale", async () => {
    const { issue, pluginPath } = await detectMatrixCustomPathIssue("/tmp/daisyclaw-matrix-missing");
    expect(issue).toEqual({
      kind: "custom-path",
      pluginId: "matrix",
      path: pluginPath,
  ...
  it("applies custom command formatting in warning messages", () => {
    expect(
      ...
      'Reinstall with "<daisyclaw plugins install @daisyclaw/matrix>" when you want to return to the standard Matrix plugin.',
