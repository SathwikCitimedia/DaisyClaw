...
/** Classifies a listener as DaisyClaw Gateway, SSH tunnel, or unknown process. */
...
export function buildPortHints(listeners: PortListener[], port: number): string[] {
  ...
  if (kinds.has("gateway") && !expectedGatewayListeners) {
    hints.push(
      `Gateway already running locally. Stop it (${formatCliCommand("daisyclaw gateway stop")}) or use a different port.`,
    );
  }
  if (kinds.has("ssh")) {
    hints.push(
