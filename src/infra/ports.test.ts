...
describe("ports helpers", () => {
  ...
  it("prints an DaisyClaw-specific hint when port details look like another DaisyClaw instance", async () => {
    const runtime = {
      error: vi.fn(),
      log: vi.fn(),
      exit: vi.fn() as unknown as (code: number) => never,
    ...
    await handlePortError(
      new PortInUseError(18789, "node dist/index.js daisyclaw gateway"),
      18789,
      "gateway start",
      runtime,
    ).catch(() => {});
    ...
    const messages = runtime.error.mock.calls.map((call) => stripAnsi(String(call[0] ?? "")));
    expect(messages.join("\n")).toContain("another DaisyClaw instance is already running");
  });
});
...
describeUnix("inspectPortUsage", () => {
  ...
  it("falls back to ss when lsof is unavailable", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "ps") {
        if (argv.includes("command=")) {
          return {
            stdout: "node /tmp/daisyclaw/dist/index.js gateway --port 18789\n",
            stderr: "",
            code: 0,
          };
        }
    ...
    try {
      ...
      expect(result.listeners[0]?.commandLine).toContain("daisyclaw");
      ...
      if (command === "ps") {
        ...
        if (argv.includes("command=")) {
          return {
            stdout:
              pid === "111"
                ? "node /tmp/newer-daisyclaw/dist/index.js logs --follow\n"
                : pid === "222"
                  ? "node /tmp/older-daisyclaw/dist/index.js gateway run\n"
  ...
  it("deduplicates repeated lsof listener records for one process address", async () => {
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "ps") {
        if (argv.includes("command=")) {
          return {
            stdout: "node /tmp/daisyclaw/dist/index.js gateway run\n",
            stderr: "",
            code: 0,
          };
        }
  ...
  it("reports multiple lsof socket records for one process", async () => {
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "ps") {
        if (argv.includes("command=")) {
          return {
            stdout: "node /tmp/newer-daisyclaw/dist/index.js logs --follow\n",
            stderr: "",
            code: 0,
          };
        }
...
describe("inspectPortUsage on Windows", () => {
  it("reports established gateway client connections from netstat", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "powershell") {
        return {
          stdout:
            '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\me\\AppData\\Roaming\\npm\\node_modules\\daisyclaw\\dist\\index.js logs --follow\r\n',
          stderr: "",
          code: 0,
        };
      }
    ...
    expect(result.connections[0]?.commandLine).toContain("daisyclaw");
  });

  it("uses PowerShell process command lines to classify DaisyClaw listeners", async () => {
    setPlatform("win32");
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      const [command] = argv;
      if (command === "netstat") {
      ...
      if (command === "powershell") {
        return {
          stdout:
            '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\me\\AppData\\Roaming\\npm\\node_modules\\daisyclaw\\dist\\index.js gateway run\r\n',
          stderr: "",
          code: 0,
        };
      }
  ...
  it("falls back to wmic when PowerShell cannot read the command line", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "wmic") {
        return {
          stdout: "CommandLine=node.exe C:\\daisyclaw\\dist\\index.js gateway run\r\n",
          stderr: "",
          code: 0,
        };
      }
