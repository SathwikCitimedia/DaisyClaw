...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "daisyclaw-provider-auth-suite-" });
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".daisyclaw");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".daisyclaw", "agents", "main", "agent");
  }
