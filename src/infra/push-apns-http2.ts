  ...
  try {
    return await new Promise<ProbeApnsHttp2ReachabilityViaProxyResult>((resolve, reject) => {
      ...
      const request = session.request({
        ...
        authorization: "bearer intentionally.invalid.daisyclaw.proxy.validation",
        "apns-topic": "ai.daisyclaw.ios",
      ...
      request.end(JSON.stringify({ aps: { alert: "DaisyClaw APNs proxy validation" } }));
    });
  } finally {
