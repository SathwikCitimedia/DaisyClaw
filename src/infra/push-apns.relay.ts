...
import { resolveTimerTimeoutMs } from "@daisyclaw/normalization-core/number-coercion";
import {
...
export const DEFAULT_APNS_RELAY_BASE_URL = "https://ios-push-relay.daisyclaw.ai";
const DEFAULT_APNS_RELAY_TIMEOUT_MS = 10_000;
const GATEWAY_DEVICE_ID_HEADER = "x-daisyclaw-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-daisyclaw-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-daisyclaw-gateway-signed-at-ms";
...
export function normalizeApnsRelayBaseUrl(
  ...
  try {
    ...
    if (parsed.protocol === "http:" && !readAllowHttp(env.DAISYCLAW_APNS_RELAY_ALLOW_HTTP)) {
      throw new Error(
        "http relay URLs require DAISYCLAW_APNS_RELAY_ALLOW_HTTP=true (development only)",
      );
    }
...
export function resolveApnsRelayConfigFromEnv(
  ...
  const envBaseUrl = normalizeNonEmptyString(env.DAISYCLAW_APNS_RELAY_BASE_URL);
  ...
  if (!baseUrl) {
    return {
      ok: false,
      error:
        "APNs relay config missing: set gateway.push.apns.relay.baseUrl or DAISYCLAW_APNS_RELAY_BASE_URL for relay registrations without the hosted relay origin",
    };
  }
  ...
  return {
    ...
    value: {
      ...
      timeoutMs: normalizeTimeoutMs(
        env.DAISYCLAW_APNS_RELAY_TIMEOUT_MS ?? configuredRelay?.timeoutMs,
      ),
    },
  };
}
