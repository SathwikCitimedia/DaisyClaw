...
function createRelayApnsSendFixture(params: {
  ...
  return {
    ...
    relayConfig: {
      baseUrl: "https://relay.daisyclaw.test",
      timeoutMs: 2_500,
    },
    gatewayIdentity: {
...
describe("push APNs send semantics", () => {
  ...
  it("sends alert pushes with alert headers and payload", async () => {
    ...
    const daisyclawPayload = requireRecord(payload.daisyclaw, "daisyclaw payload");
    expectRecordFields(daisyclawPayload, {
      kind: "push.test",
      nodeId: "ios-node-alert",
    });
    expect(typeof daisyclawPayload.ts).toBe("number");
  ...
  it("routes direct APNs HTTP/2 requests through the active managed proxy", async () => {
    ...
    try {
      ...
      expect(request?.headers["apns-topic"]).toBe("ai.daisyclaw.ios");
  ...
  it("sends exec approval alert pushes with generic modal-only metadata", async () => {
    ...
    expect(payload.aps).toEqual({
      alert: {
        ...
        body: "Open DaisyClaw to review this request.",
      },
      sound: "default",
      category: "daisyclaw.exec-approval",
      "content-available": 1,
    });
    const daisyclawPayload = requireRecord(payload.daisyclaw, "daisyclaw payload");
    expectRecordFields(daisyclawPayload, {
    ...
    expect(typeof daisyclawPayload.ts).toBe("number");
    expectNoProperties(daisyclawPayload, [
      "host",
      "nodeId",
      "agentId",
      "commandText",
  ...
  it("defaults background wake reason when not provided", async () => {
    ...
    expectRecordFields(requireRecord(payload.daisyclaw, "daisyclaw payload"), {
      kind: "node.wake",
      reason: "node.invoke",
      nodeId: "ios-node-wake-default-reason",
    });
