...
const EXEC_APPROVAL_GENERIC_ALERT_BODY = "Open DaisyClaw to review this request.";
const EXEC_APPROVAL_NOTIFICATION_CATEGORY = "daisyclaw.exec-approval";
...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.DAISYCLAW_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.DAISYCLAW_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set DAISYCLAW_APNS_TEAM_ID and DAISYCLAW_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.DAISYCLAW_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.DAISYCLAW_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.DAISYCLAW_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set DAISYCLAW_APNS_PRIVATE_KEY_P8 or DAISYCLAW_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading DAISYCLAW_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
