...
describe("resolveVapidKeys", () => {
  it("generates and persists VAPID keys on first call", async () => {
    ...
    expect(keys.subject).toBe("https://daisyclaw.ai");
    const persistedKeys = JSON.parse(
      await fs.readFile(path.join(tmpDir, "push", "vapid-keys.json"), "utf8"),
    ) as { subject?: string };
    expect(persistedKeys.subject).toBe("https://daisyclaw.ai");
  ...
  it("prefers env vars over persisted keys", async () => {
    ...
    // Set env overrides.
    process.env.DAISYCLAW_VAPID_PUBLIC_KEY = "env-public";
    process.env.DAISYCLAW_VAPID_PRIVATE_KEY = "env-private";
    process.env.DAISYCLAW_VAPID_SUBJECT = "mailto:env@test.com";
    try {
      const keys = await resolveVapidKeys(tmpDir);
      ...
      delete process.env.DAISYCLAW_VAPID_PUBLIC_KEY;
      delete process.env.DAISYCLAW_VAPID_PRIVATE_KEY;
      delete process.env.DAISYCLAW_VAPID_SUBJECT;
    }
  });
});
