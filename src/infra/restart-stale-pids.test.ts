...
function createDaisyClawBusyResult(pid: number, overrides: Partial<MockLsofResult> = {}) {
  return createLsofResult({
    stdout: lsofOutput([{ pid, cmd: "daisyclaw-gateway" }]),
    ...overrides,
  });
}
...
function installInitialBusyPoll(
  ...
  mockSpawnSync.mockImplementation((command: unknown) => {
    ...
    call += 1;
    if (call === 1) {
      return createDaisyClawBusyResult(stalePid);
    }
...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses daisyclaw-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "daisyclaw-gateway" },
          { pid: process.pid, cmd: "daisyclaw-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("verifies argv when lsof reports the node process name instead of daisyclaw", () => {
      ...
      mockSpawnSync.mockImplementation((command: unknown) => {
        if (command === "ps") {
          return {
            error: null,
            status: 0,
            stdout: "node /opt/daisyclaw/dist/entry.js gateway\n",
            stderr: "",
          };
        }
        return {
    ...
    it("excludes ancestor pids so a sidecar cannot kill its parent gateway — regression for #68451", () => {
      // Regression: daisyclaw-weixin sidecar (child of the gateway) invoked
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: parentGatewayPid, cmd: "daisyclaw-gateway" },
          { pid: unrelatedStalePid, cmd: "daisyclaw-gateway" },
        ]),
        stderr: "",
      });
    ...
    it.skipIf(process.platform !== "linux")(
      ...
      () => {
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: directParentPid, cmd: "daisyclaw-gateway" },
            { pid: grandparentPid, cmd: "daisyclaw-gateway" },
            { pid: benignStalePid, cmd: "daisyclaw-gateway" },
          ]),
          stderr: "",
        });
    ...
    it.skipIf(process.platform !== "linux")(
      ...
      () => {
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: pluginHostPid, cmd: "daisyclaw-gateway" },
            { pid: gatewayGrandparentPid, cmd: "daisyclaw-gateway" },
          ]),
          stderr: "",
        });
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          return {
            ...
            status: 0,
            stdout: lsofOutput([
              { pid: toolHostPid, cmd: "daisyclaw-gateway" },
              { pid: gatewayGrandparentPid, cmd: "daisyclaw-gateway" },
              { pid: benignStalePid, cmd: "daisyclaw-gateway" },
            ]),
            stderr: "",
    ...
    it("excludes pids whose command does not include 'daisyclaw'", () => {
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          return {
            error: null,
            status: 0,
            stdout: lsofOutput([{ pid: process.pid + 3152, cmd: "daisyclaw-gateway" }]),
            stderr: "",
          };
        });
    ...
    it("returns verified gateway pids from Windows helpers on win32", () => {
      ...
      try {
        ...
        mockReadWindowsProcessArgs.mockReturnValue(["daisyclaw", "gateway"]);
        expect(findGatewayPidsOnPortSync(18789)).toEqual([stalePid]);
        expect(mockReadWindowsListeningPids).toHaveBeenCalledWith(18789, undefined);
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple daisyclaw pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "daisyclaw-gateway" },
          { pid: pid2, cmd: "daisyclaw-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18789);
    ...
    it("returns [] when status 0 but only non-daisyclaw pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks daisyclaw processes — non-daisyclaw listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
    ...
    it("lsof status 1 with non-empty daisyclaw stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty daisyclaw stdout must not be treated as port-free.
      const stalePid = process.pid + 601;
      const getCallCount = installInitialBusyPoll(stalePid, (call) => {
        if (call === 2) {
          // status 1 + daisyclaw pid in stdout — container-restricted lsof reports partial results
          return createDaisyClawBusyResult(stalePid, {
            status: 1,
    ...
    it("pollPortOnce outer catch returns { free: null, permanent: false } when resolveLsofCommandSync throws", () => {
      ...
      mockSpawnSync.mockImplementationOnce(() => {
        ...
        return {
          error: null,
          status: 0,
          stdout: lsofOutput([{ pid: stalePid, cmd: "daisyclaw-gateway" }]),
          stderr: "",
        };
      });
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'daisyclaw'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("daisyclaw")
      // inside the mid-loop flush: a non-daisyclaw cmd between two entries must not
      // be pushed, but the following daisyclaw entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-daisyclaw entry first, then daisyclaw entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — ternary false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid daisyclaw entry must still be found.
      const stdout = `p0\ncdaisyclaw-gateway\np${stalePid}\ncdaisyclaw-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(18789);
      expect(result).toContain(stalePid);
  ...
  // -------------------------------------------------------------------------
  // pollPortOnce branch — status 1 + non-empty stdout with zero daisyclaw pids
  // -------------------------------------------------------------------------
  describe("pollPortOnce — status 1 + non-empty non-daisyclaw stdout (line 145)", () => {
    it("treats status 1 + non-daisyclaw stdout as port-free (not an daisyclaw process)", () => {
      // status 1 + non-empty stdout where no daisyclaw pids are present:
      // the port may be held by an unrelated process. From our perspective
      // (we only kill daisyclaw pids) it is effectively free.
      const stalePid = process.pid + 800;
      const getCallCount = installInitialBusyPoll(stalePid, () => {
        // status 1 + non-daisyclaw output — should be treated as free:true for our purposes
        return createLsofResult({
          status: 1,
      ...
      vi.spyOn(process, "kill").mockReturnValue(true);
      // No daisyclaw pids in status-1 output means the port is free for this cleanup.
      expect(cleanStaleGatewayProcessesSync()).toContain(stalePid);
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          if (command === "lsof") {
            ...
            return createLsofResult({
              stdout: lsofOutput([{ pid: gatewayParentPid, cmd: "daisyclaw-gateway" }]),
            });
          }
          return createLsofResult();
        });
