 ...
 * returning control to the caller (typically `triggerDaisyClawRestart` →
...
function parsePidsFromLsofOutput(stdout: string, spawnTimeoutMs: number): number[] {
  ...
  for (const entry of parseLsofEntries(stdout)) {
    ...
    if (entry.cmd && normalizeLowercaseStringOrEmpty(entry.cmd).includes("daisyclaw")) {
      pids.push(entry.pid);
      continue;
    }
...
/**
 * Windows: find listening PIDs on the port, then verify each is an daisyclaw
 ...
 * Returns only PIDs that belong to daisyclaw gateway processes (not the current process).
 */
export function findGatewayPidsOnPortSync(
  ...
  if (process.platform === "win32") {
    ...
    // command-line verification to find only daisyclaw gateway processes.
