...
export function emitGatewayRestart(
  ...
  try {
    if (process.listenerCount("SIGUSR1") > 0) {
      ...
      // triggerDaisyClawRestart() uses schtasks to restart the gateway.
...
export function triggerDaisyClawRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.DAISYCLAW_SYSTEMD_UNIT,
      process.env.DAISYCLAW_PROFILE,
    );
  ...
  const label =
    process.env.DAISYCLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.DAISYCLAW_PROFILE);
