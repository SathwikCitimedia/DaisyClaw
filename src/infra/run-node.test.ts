...
const EXTENSION_MANIFEST = bundledPluginFile("demo", "daisyclaw.plugin.json");
...
const DIST_RUNTIME_EXTENSION_MANIFEST = "dist-runtime/extensions/demo/daisyclaw.plugin.json";
...
const DIST_DAISYCLAW_ALIAS_PACKAGE = "dist/extensions/node_modules/daisyclaw/package.json";
const DIST_DAISYCLAW_ALIAS_PLUGIN_SDK_INDEX =
  "dist/extensions/node_modules/daisyclaw/plugin-sdk/index.js";
const DIST_DAISYCLAW_ALIAS_PLUGIN_SDK_STRING_COERCE =
  "dist/extensions/node_modules/daisyclaw/plugin-sdk/string-coerce-runtime.js";
...
const DIST_EXTENSION_MANIFEST = bundledDistPluginFile("demo", "daisyclaw.plugin.json");
...
const BASE_PROJECT_FILES = {
  [ROOT_TSCONFIG]: "{}\n",
  [ROOT_PACKAGE]: '{"name":"daisyclaw-test"}\n',
...
async function writeRuntimePostBuildScaffold(tmp: string): Promise<void> {
  ...
  await writeProjectFiles(tmp, {
    ...
    [DIST_DAISYCLAW_ALIAS_PACKAGE]:
      '{"name":"daisyclaw","type":"module","exports":{"./plugin-sdk":"./plugin-sdk/index.js"}}\n',
  });
...
function statusCommandSpawn() {
  return [process.execPath, "daisyclaw.mjs", "status"];
}

function gatewayCallStatusCommandSpawn() {
  return [process.execPath, "daisyclaw.mjs", "gateway", "call", "status", "--json"];
}
...
describe("run-node script", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "daisyclaw-run-node-" }, async (tmp) => {
        ...
        expect(nodeCalls).toEqual([
          ...
          [process.execPath, "daisyclaw.mjs", "--version"],
        ]);
      });
    },
  );
  ...
  it("skips DTS generation only for launcher-triggered local builds", async () => {
    await withTempDir({ prefix: "daisyclaw-run-node-" }, async (tmp) => {
      ...
      expect(spawnCalls[2]?.args).toEqual(["daisyclaw.mjs", "status"]);
      expect(spawnCalls[0]?.env.DAISYCLAW_RUN_NODE_SKIP_DTS_BUILD).toBeUndefined();
      expect(spawnCalls[1]?.env.DAISYCLAW_RUN_NODE_SKIP_DTS_BUILD).toBe("1");
      expect(spawnCalls[2]?.env.DAISYCLAW_RUN_NODE_SKIP_DTS_BUILD).toBeUndefined();
    });
  });
    ...
    await withTempDir({ prefix: "daisyclaw-run-node-" }, async (tmp) => {
      ...
      const spawn = (_cmd: string, args: string[], options?: unknown) => {
        ...
        return createPipedExitedProcess({
          stdout: args[0] === "daisyclaw.mjs" ? "child stdout\n" : "",
          stderr: args[0] === "daisyclaw.mjs" ? "child stderr\n" : "",
        });
      };
      const mutedStream = {
        write: () => true,
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...process.env,
          DAISYCLAW_FORCE_BUILD: "1",
          DAISYCLAW_RUNNER_LOG: "1",
          DAISYCLAW_RUN_NODE_OUTPUT_LOG: outputPath,
        },
        spawn,
        stderr: mutedStream,
        stdout: mutedStream,
      ...
      await expect(fs.readFile(outputPath, "utf-8")).resolves.toContain("[daisyclaw]");
      expect(spawnCalls.at(-1)?.args).toEqual(["daisyclaw.mjs", "status"]);
      expect(spawnCalls.at(-1)?.env.DAISYCLAW_RUN_NODE_OUTPUT_LOG).toBe(outputPath);
    ...
    await withTempDir({ prefix: "daisyclaw-run-node-" }, async (tmp) => {
      ...
      const spawn = (_cmd: string, args: string[]) =>
        createPipedExitedProcess({
          stderr: args[0] === "daisyclaw.mjs" ? childStderr : "",
        });
      const stderrChunks: string[] = [];
      const stderr = {
        write: (chunk: string | Buffer) => {
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...process.env,
          DAISYCLAW_RUNNER_LOG: "0",
          DAISYCLAW_RUN_NODE_FILTER_SYNC_IO_STDERR: "1",
          DAISYCLAW_RUN_NODE_OUTPUT_LOG: outputPath,
        },
        spawn,
        stderr,
  ...
  it("adds Node CPU profiling flags to the launched DaisyClaw child when requested", async () => {
    await withTempDir({ prefix: "daisyclaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...process.env,
          DAISYCLAW_RUNNER_LOG: "0",
          DAISYCLAW_RUN_NODE_CPU_PROF_DIR: ".artifacts/profiles",
        },
        spawn,
        spawnSync,
        runRuntimePostBuild: skipRuntimePostBuild,
      ...
      expect(childArgs[2]).toMatch(
        /^--cpu-prof-name=daisyclaw-status-4242-\d{4}-\d{2}-\d{2}T.*\.cpuprofile$/,
      );
      expect(childArgs.slice(3)).toEqual(["daisyclaw.mjs", "status"]);
      expect(spawnCalls.at(-1)?.env.DAISYCLAW_RUN_NODE_CPU_PROF_DIR).toBe(profileDir);
  ...
  it("rotates old Node CPU profiles when a retention cap is set", async () => {
    await withTempDir({ prefix: "daisyclaw-run-node-" }, async (tmp) => {
      ...
      const oldProfiles = [
        "daisyclaw-status-oldest.cpuprofile",
        "daisyclaw-status-middle.cpuprofile",
        "daisyclaw-status-newest.cpuprofile",
      ];
      ...
      fsSync.writeFileSync(path.join(profileDir, "daisyclaw-models-old.cpuprofile"), "{}");
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...
          DAISYCLAW_RUNNER_LOG: "0",
          DAISYCLAW_RUN_NODE_CPU_PROF_DIR: ".artifacts/profiles",
          DAISYCLAW_RUN_NODE_CPU_PROF_MAX_FILES: "2",
        },
        spawn,
        spawnSync,
        runRuntimePostBuild: skipRuntimePostBuild,
      ...
      expect(fsSync.existsSync(path.join(profileDir, "daisyclaw-models-old.cpuprofile"))).toBe(true);
    });
  });

  it("adds Node sync I/O tracing flag to the launched DaisyClaw child when requested", async () => {
    await withTempDir({ prefix: "daisyclaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
      ...
      expect(exitCode).toBe(0);
      expect(spawnCalls.at(-1)).toEqual(["--trace-sync-io", "daisyclaw.mjs", "gateway", "--force"]);
    });
  });
  ...
  it("does not mutate Matrix QA args when no generic output log is requested", async () => {
    await withTempDir({ prefix: "daisyclaw-run-node-" }, async (tmp) => {
      ...
      expect(exitCode).toBe(0);
      const childArgs = spawnCalls.at(-1)?.args ?? [];
      expect(childArgs).toEqual(["daisyclaw.mjs", "qa", "matrix"]);
      expect(spawnCalls.at(-1)?.env.DAISYCLAW_RUN_NODE_OUTPUT_LOG).toBeUndefined();
    });
  });
  ...
  it("passes the synthesized private QA env into runtime postbuild staging", async () => {
    await withTempDir({ prefix: "daisyclaw-run-node-" }, async (tmp) => {
      ...
      expect(postBuildParams?.env?.DAISYCLAW_BUILD_PRIVATE_QA).toBe("1");
      expect(postBuildParams?.env?.DAISYCLAW_ENABLE_PRIVATE_QA_CLI).toBe("1");
      expect(postBuildParams?.env?.DAISYCLAW_DISABLE_BUNDLED_PLUGINS).toBe("0");
    });
  });
  ...
  it("preserves an explicit bundled plugin disable flag for QA runs", async () => {
    await withTempDir({ prefix: "daisyclaw-run-node-" }, async (tmp) => {
      ...
      const exitCode = await runQaCommand({
        ...
        spawnSync,
        runRuntimePostBuild,
        env: { DAISYCLAW_DISABLE_BUNDLED_PLUGINS: "1" },
      });
      ...
      const postBuildParams = firstMockCall(runRuntimePostBuild)?.[0] as
        | { cwd?: string; env?: Record<string, string | undefined> }
        | undefined;
      expect(postBuildParams?.env?.DAISYCLAW_DISABLE_BUNDLED_PLUGINS).toBe("1");
    });
  });
    ...
    await withTempDir({ prefix: "daisyclaw-run-node-" }, async (tmp) => {
      ...
      const requirement = resolveBuildRequirement(
        createBuildRequirementDeps(tmp, {
          env: { DAISYCLAW_BUILD_PRIVATE_QA: "1" },
          gitHead: "abc123\n",
          gitStatus: "",
        }),
      );
  ...
  it("reruns runtime postbuild in watch mode when required outputs are missing with no runtime stamp", async () => {
    await withTempDir({ prefix: "daisyclaw-run-node-" }, async (tmp) => {
      ...
      await fs.rm(resolvePath(tmp, DIST_DAISYCLAW_ALIAS_PACKAGE));
  ...
  it("reruns runtime postbuild for dirty extension package metadata in watch mode", async () => {
    await withTempDir({ prefix: "daisyclaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
          [EXTENSION_PACKAGE]: '{"daisyclaw":{"extensions":["./index.ts"]}}\n',
    ...
    await withTempDir({ prefix: "daisyclaw-run-node-" }, async (tmp) => {
      ...
      const runs = Promise.all([
        runStatusCommand({
          ...
          spawnSync,
          env: {
            DAISYCLAW_RUN_NODE_BUILD_LOCK_POLL_MS: "1",
          },
          runRuntimePostBuild,
        }),
        runStatusCommand({
  ...
  it("forwards wrapper SIGTERM to the active daisyclaw child and returns 143", async () => {
    await withTempDir({ prefix: "daisyclaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
      ...
      expect(spawnCall?.[1]).toEqual(["daisyclaw.mjs", "status"]);
    ...
    await withTempDir({ prefix: "daisyclaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [EXTENSION_INDEX]: "export default {};\n",
          [EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [EXTENSION_PACKAGE]: '{"name":"demo","daisyclaw":{"extensions":["./index.ts"]}}\n',
          [ROOT_TSDOWN]: "export default {};\n",
          [DIST_EXTENSION_INDEX]: "export default {};\n",
          [DIST_EXTENSION_PACKAGE]: '{"name":"demo","daisyclaw":{"extensions":["./stale.js"]}}\n',
        },
    ...
    await withTempDir({ prefix: "daisyclaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [EXTENSION_PACKAGE]: '{"daisyclaw":{"extensions":["./src/index.ts","./src/extra.ts"]}}\n',
    ...
    await withTempDir({ prefix: "daisyclaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [EXTENSION_SRC]: "export default {};\n",
          [EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [EXTENSION_PACKAGE]: '{"daisyclaw":{"extensions":["./src/index.ts"]}}\n',
        },
        buildPaths: [
          ROOT_SRC,
          EXTENSION_SRC,
    ...
    await withTempDir({ prefix: "daisyclaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [DIST_EXTENSION_PACKAGE]: '{"daisyclaw":{"extensions":["./src/index.js"]}}\n',
          [DIST_EXTENSION_RUNTIME_SRC]: "export default {};\n",
          [DIST_RUNTIME_EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [DIST_RUNTIME_EXTENSION_PACKAGE]: '{"daisyclaw":{"extensions":["./src/index.js"]}}\n',
          [RUNTIME_POSTBUILD_STAMP]: '{"head":"abc123"}\n',
        },
        buildPaths: [
          ROOT_SRC,
    ...
    await withTempDir({ prefix: "daisyclaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [DIST_EXTENSION_INDEX]: "export default {};\n",
          [DIST_EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [DIST_EXTENSION_PACKAGE]: '{"daisyclaw":{"extensions":["./index.js"]}}\n',
          [DIST_RUNTIME_EXTENSION_INDEX]: "export default {};\n",
          [DIST_RUNTIME_EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [DIST_RUNTIME_EXTENSION_PACKAGE]: '{"daisyclaw":{"extensions":["./index.js"]}}\n',
          [RUNTIME_POSTBUILD_STAMP]: '{"head":"abc123"}\n',
        },
        buildPaths: [
          ROOT_SRC,
  ...
  it("does not require DaisyClaw SDK alias outputs when dist extensions are absent", async () => {
    await withTempDir({ prefix: "daisyclaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
  ...
  it("reports missing DaisyClaw SDK alias outputs when runtime stamps match HEAD", async () => {
    await withTempDir({ prefix: "daisyclaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
          ...
          [DIST_DAISYCLAW_ALIAS_PACKAGE]:
            '{"name":"daisyclaw","type":"module","exports":{"./plugin-sdk":"./plugin-sdk/index.js"}}\n',
          [DIST_DAISYCLAW_ALIAS_PLUGIN_SDK_INDEX]:
      ...
      await fs.rm(resolvePath(tmp, DIST_DAISYCLAW_ALIAS_PLUGIN_SDK_INDEX));
  ...
  it("does not require private DaisyClaw SDK dist files that package exports omit", async () => {
    await withTempDir({ prefix: "daisyclaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
          ...
          [DIST_DAISYCLAW_ALIAS_PACKAGE]:
            '{"name":"daisyclaw","type":"module","exports":{"./plugin-sdk":"./plugin-sdk/index.js","./plugin-sdk/string-coerce-runtime":"./plugin-sdk/string-coerce-runtime.js"}}\n',
          [DIST_DAISYCLAW_ALIAS_PLUGIN_SDK_INDEX]:
            "export * from '../../../../plugin-sdk/index.js';\n",
          [DIST_DAISYCLAW_ALIAS_PLUGIN_SDK_STRING_COERCE]:
    ...
    await withTempDir({ prefix: "daisyclaw-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
          [DIFFS_PACKAGE]:
            '{"daisyclaw":{"build":{"staticAssets":[{"source":"./assets/viewer-runtime.js","output":"assets/viewer-runtime.js"}]}}}\n',
  ...
  it("does not require static asset outputs when runtime static assets are disabled", async () => {
    await withTempDir({ prefix: "daisyclaw-run-node-" }, async (tmp) => {
      ...
      const requirement = resolveRuntimePostBuildRequirement(
        createBuildRequirementDeps(tmp, {
          env: { DAISYCLAW_RUNTIME_POSTBUILD_STATIC_ASSETS: "0" },
          gitHead: "abc123\n",
          gitStatus: "",
        }),
      );
