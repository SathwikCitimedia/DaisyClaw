// Validates the current runtime against DaisyClaw's Node engine floor.
...
/** Returns whether a detected runtime meets DaisyClaw's minimum runtime contract. */
...
/** Checks a Node version label against DaisyClaw's current minimum Node version. */
...
export function assertSupportedRuntime(
  ...
  runtime.error(
    [
      "daisyclaw requires Node >=22.19.0.",
      `Detected: ${runtimeLabel} (exec: ${execLabel}).`,
      `PATH searched: ${details.pathEnv}`,
      "Install Node: https://nodejs.org/en/download",
      "Upgrade Node and re-run daisyclaw.",
    ].join("\n"),
  );
  runtime.exit(1);
}
