...
async function withStateFixture(
  ...
  await withTempDir({ prefix: "orphan-keys-test-" }, async (tmpDir) => {
    const stateDir = path.join(tmpDir, ".daisyclaw");
...
function sharedMainOpsConfig(sharedStorePath: string): DaisyClawConfig {
  return {
...
async function migrateFixtureState(stateDir: string, cfg: DaisyClawConfig = OPS_WORK_CONFIG) {
