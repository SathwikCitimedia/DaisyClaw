...
vi.mock("../channels/plugins/bundled.js", () => {
  ...
  function resolveChatAppAccountId(cfg: DaisyClawConfig): string {
  ...
  return {
    ...
    listBundledChannelLegacyStateMigrationDetectors: vi.fn(() => [
      ...
      ({ cfg, env }: { cfg: DaisyClawConfig; env: NodeJS.ProcessEnv }) => {
        const root = env.DAISYCLAW_STATE_DIR;
        if (!root) {
          return [];
        }
...
const createTempDir = () => tempDirs.make("daisyclaw-state-migrations-test-");
...
describe("state migrations", () => {
  ...
  it("migrates legacy delivery queue files into shared SQLite state", async () => {
    ...
    const { db } = openDaisyClawStateDatabase({ env });
    const rows = db
      .prepare(
