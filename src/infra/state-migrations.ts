// Applies persisted state migrations across DaisyClaw config files.
...
import type { DB as DaisyClawStateKyselyDatabase } from "../state/daisyclaw-state-db.generated.js";
import {
  detectDaisyClawStateDatabaseSchemaMigrations,
  repairDaisyClawStateDatabaseSchema,
...
type LegacyPluginStateImportDatabase = Pick<DaisyClawStateKyselyDatabase, "plugin_state_entries">;
...
function resolveDefaultExecApprovalsStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env, homedir), ".daisyclaw");
}
...
function detectLegacyExecApprovalsMigration(params: {
  ...
  return {
    ...
    targetPath,
    hasLegacy:
      Boolean(params.env.DAISYCLAW_STATE_DIR?.trim()) &&
...
async function migrateLegacyTaskRunsSidecar(params: {
  ...
  try {
    ...
    runDaisyClawStateWriteTransaction(
      ...
      { env: { ...process.env, DAISYCLAW_STATE_DIR: params.stateDir } },
    );
    if (importedTasks > 0) {
      changes.push(
...
async function withPluginStateImportEnv<T>(
  ...
  const previous = process.env.DAISYCLAW_STATE_DIR;
  process.env.DAISYCLAW_STATE_DIR = plan.stateDir;
  try {
    return await run();
  } finally {
    if (previous === undefined) {
      ...
      process.env.DAISYCLAW_STATE_DIR = previous;
    }
  }
}
...
export async function autoMigrateLegacyStateDir(params: {
  ...
  const warnings: string[] = [];
  const changes: string[] = [];
  const hasCustomStateDir = Boolean(env.DAISYCLAW_STATE_DIR?.trim());
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        warnings.push(
          `Rollback failed; set DAISYCLAW_STATE_DIR=${targetDir} to avoid split state: ${String(rollbackErr)}`,
        );
...
export async function detectLegacyStateMigrations(params: {
  ...
  const stateSchemaMigrations = detectDaisyClawStateDatabaseSchemaMigrations({
    env: { ...env, DAISYCLAW_STATE_DIR: stateDir },
  });
...
async function migrateLegacySessions(
  ...
  if (!targetReadable) {
    if (options.recoverCorruptTargetStore) {
      ...
      warnings.push(
        `Target sessions store unreadable; left untouched to avoid overwriting at ${detected.sessions.targetStorePath}. Run daisyclaw doctor --fix to archive it and retry the legacy merge.`,
      );
    }
  }
...
function migrateLegacyStateSchema(detected: LegacyStateDetection): {
  ...
  return repairDaisyClawStateDatabaseSchema({
    env: { ...process.env, DAISYCLAW_STATE_DIR: detected.stateDir },
  });
}
...
export async function autoMigrateLegacyState(params: {
  ...
  const stateSchema = repairDaisyClawStateDatabaseSchema({
    env: { ...env, DAISYCLAW_STATE_DIR: stateDir },
  });
  ...
  const hasCustomAgentDir = env.DAISYCLAW_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim();
