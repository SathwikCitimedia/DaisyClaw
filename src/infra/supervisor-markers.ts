...
const SUPERVISOR_HINTS = {
  launchd: ["DAISYCLAW_LAUNCHD_LABEL"],
  systemd: ["DAISYCLAW_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["DAISYCLAW_WINDOWS_TASK_NAME"],
} as const;
...
export const SUPERVISOR_HINT_ENV_VARS = [
  ...
  "DAISYCLAW_SERVICE_MARKER",
  "DAISYCLAW_SERVICE_KIND",
] as const;
...
function isCurrentGatewayLaunchdJob(env: NodeJS.ProcessEnv): boolean {
  const expectedLabel = resolveGatewayLaunchAgentLabel(env.DAISYCLAW_PROFILE);
  if (
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.DAISYCLAW_SERVICE_MARKER?.trim();
