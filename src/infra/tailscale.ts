...
import { asNullableObjectRecord as readRecord } from "@daisyclaw/normalization-core/record-coerce";
import {
...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.DAISYCLAW_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for DaisyClaw. You can keep running the web gateway without it: \`${formatCliCommand("daisyclaw gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
