...
describe("tmp-daisyclaw-dir browser-safe import", () => {
  it("loads when a browser fs shim omits constants", async () => {
    const resultKey = `__daisyclawTmpDirBrowserImport_${crypto.randomUUID().replaceAll("-", "_")}`;
    ...
    const bundled = await build({
      ...
      stdin: {
        contents: `
          import { POSIX_DAISYCLAW_TMP_DIR, resolvePreferredDaisyClawTmpDir } from "./src/infra/tmp-daisyclaw-dir.ts";
          globalThis.${resultKey} = {
            posixTmpDir: POSIX_DAISYCLAW_TMP_DIR,
            resolverType: typeof resolvePreferredDaisyClawTmpDir,
          };
        `,
        loader: "ts",
        resolveDir: process.cwd(),
        sourcefile: "tmp-daisyclaw-dir-browser-entry.ts",
      },
      write: false,
    });
