// Covers preferred DaisyClaw temp directory resolution.
import { constants as fsConstants } from "node:fs";
import path from "node:path";
import { describe, expect, it, vi } from "vitest";
import { POSIX_DAISYCLAW_TMP_DIR, resolvePreferredDaisyClawTmpDir } from "./tmp-daisyclaw-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredDaisyClawTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `daisyclaw-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredDaisyClawTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_DAISYCLAW_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredDaisyClawTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredDaisyClawTmpDir", () => {
  it("prefers /tmp/daisyclaw when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_DAISYCLAW_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/daisyclaw when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_DAISYCLAW_TMP_DIR, {
      recursive: true,
      mode: 0o700,
    });
    expect(tmpdir).not.toHaveBeenCalled();
  ...
  it.each([
    {
      name: "falls back to os.tmpdir()/daisyclaw when /tmp/daisyclaw is not a directory",
      lstatSync: vi.fn(() => makeDirStat({ isDirectory: false, mode: 0o100644 })),
    },
    {
      name: "falls back to os.tmpdir()/daisyclaw when /tmp is not writable",
      lstatSync: vi.fn(() => {
        throw nodeErrorWithCode("ENOENT");
      }),
    ...
    {
      name: "falls back when /tmp/daisyclaw exists but is not writable",
    ...
    {
      name: "falls back when /tmp/daisyclaw is a symlink",
      lstatSync: symlinkTmpDirLstat(),
    },
    {
      name: "falls back when /tmp/daisyclaw is not owned by the current user",
      lstatSync: vi.fn(() => makeDirStat({ uid: 0 })),
    },
    {
      name: "falls back when /tmp/daisyclaw is group/other writable",
  ...
  it("repairs existing /tmp/daisyclaw permissions when they are too broad", () => {
    let preferredMode = 0o40777;
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === POSIX_DAISYCLAW_TMP_DIR && mode === 0o700) {
    ...
    expect(resolved).toBe(POSIX_DAISYCLAW_TMP_DIR);
    expect(chmodSync).toHaveBeenCalledWith(POSIX_DAISYCLAW_TMP_DIR, 0o700);
    expect(warn).toHaveBeenCalledWith(
      `[daisyclaw] tightened permissions on temp dir: ${POSIX_DAISYCLAW_TMP_DIR}`,
    );
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("repairs /tmp/daisyclaw after create when the initial mode stays too broad", () => {
    ...
    const chmodSync = vi.fn((target: string, mode: number) => {
      chmodCalls += 1;
      if (target === POSIX_DAISYCLAW_TMP_DIR && mode === 0o700 && chmodCalls > 1) {
        preferredMode = 0o40700;
      }
    });
    const warn = vi.fn();
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback DaisyClaw temp dir/);
  });
  ...
  it("uses an unscoped fallback suffix when process uid is unavailable", () => {
    ...
    const fallbackPath = path.join(tmpdirPath, "daisyclaw");
  ...
  it("repairs existing fallback directory when permissions are too broad", () => {
    ...
    expect(chmodSync).toHaveBeenCalledWith(fallbackPath, 0o700);
    expect(warn).toHaveBeenCalledWith(
      `[daisyclaw] tightened permissions on temp dir: ${fallbackPath}`,
    );
  });

  it("uses /tmp/daisyclaw when another process tightened permissions before repair", () => {
  ...
  it("uses /tmp/daisyclaw when chmod loses a concurrent repair race", () => {
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === POSIX_DAISYCLAW_TMP_DIR && mode === 0o700) {
  ...
  it("throws when the fallback directory cannot be created", () => {
    expect(() =>
      resolvePreferredDaisyClawTmpDir({
        accessSync: readOnlyTmpAccessSync(),
        lstatSync: vi.fn((target: string) => {
          if (target === POSIX_DAISYCLAW_TMP_DIR || target === fallbackTmp()) {
            throw nodeErrorWithCode("ENOENT");
          }
          return secureDirStat();
        }),
    ...
    ).toThrow(/Unable to create fallback DaisyClaw temp dir/);
  });
  ...
  it("skips the POSIX preferred path on Windows even when /tmp is accessible (#60713)", () => {
    ...
    // create it), the previous code path returned `/tmp/daisyclaw` and routed
    // log files / TTS temp files there instead of `%TEMP%\daisyclaw`. The
    // platform: "win32" branch must skip the POSIX path entirely.
    const winFallback = path.win32.join("C:\\Users\\u\\AppData\\Local\\Temp", "daisyclaw-501");
    const accessSync = vi.fn();
    const lstatSync = vi.fn((target: string) => {
      if (target === POSIX_DAISYCLAW_TMP_DIR || target === winFallback) {
        return secureDirStat();
      }
      throw nodeErrorWithCode("ENOENT");
    });
    ...
    const result = resolvePreferredDaisyClawTmpDir({
      platform: "win32",
      accessSync,
      lstatSync,
      mkdirSync,
    ...
    expect(result).not.toBe(POSIX_DAISYCLAW_TMP_DIR);
    expect(tmpdir).toHaveBeenCalled();
  });
  ...
  it("still uses the POSIX preferred path on non-Windows platforms when available", () => {
    ...
    expect(result).toBe(POSIX_DAISYCLAW_TMP_DIR);
  });
});
