// Creates temporary DaisyClaw directories for runtime scratch work.
import fs from "node:fs";
import { tmpdir as getOsTmpDir } from "node:os";
import path from "node:path";

/** Preferred shared DaisyClaw temp root on POSIX systems when ownership and permissions are safe. */
export const POSIX_DAISYCLAW_TMP_DIR = "/tmp/daisyclaw";
...
export type ResolvePreferredDaisyClawTmpDirOptions = {
...
/** Resolves a safe DaisyClaw temp root, falling back to user-scoped os.tmpdir paths when needed. */
export function resolvePreferredDaisyClawTmpDir(
  options: ResolvePreferredDaisyClawTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const suffix = uid === undefined ? "daisyclaw" : `daisyclaw-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[daisyclaw] tightened permissions on temp dir: ${candidatePath}`);
      return resolveDirState(candidatePath) === "available";
    } catch {
      return false;
    }
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback DaisyClaw temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback DaisyClaw temp dir: ${fallbackPath}`);
    }
  ...
  const preferredDir = POSIX_DAISYCLAW_TMP_DIR;
