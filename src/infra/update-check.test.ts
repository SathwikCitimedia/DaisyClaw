...
describe("compareSemverStrings", () => {
  ...
  it("treats DaisyClaw stable correction releases as newer than their base release", () => {
...
describe("checkDepsStatus", () => {
  it("reports unknown, missing, stale, and ok states from lockfile markers", async () => {
    await withTempDir({ prefix: "daisyclaw-update-check-" }, async (base) => {
  ...
  it("uses npm-shrinkwrap as the npm dependency lock marker when present", async () => {
    await withTempDir({ prefix: "daisyclaw-update-check-shrinkwrap-" }, async (root) => {
...
describe("checkUpdateStatus", () => {
  ...
  it("detects package installs for non-git roots", async () => {
    await withTempDir({ prefix: "daisyclaw-update-check-" }, async (root) => {
      await fs.writeFile(
  ...
  it("detects npm package installs that ship pnpm package metadata with shrinkwrap", async () => {
    await withTempDir({ prefix: "daisyclaw-update-check-npm-shrinkwrap-" }, async (root) => {
      await fs.writeFile(
        path.join(root, "package.json"),
        JSON.stringify({ name: "daisyclaw", packageManager: "pnpm@11.2.2" }),
        "utf8",
      );
  ...
  it("treats symlinked git installs as git roots", async () => {
    await withTempDir({ prefix: "daisyclaw-update-check-git-" }, async (base) => {
      const repoRoot = path.join(base, "repo");
      const linkedRoot = path.join(base, "linked-daisyclaw");
      await fs.mkdir(repoRoot, { recursive: true });
      await fs.writeFile(
        path.join(repoRoot, "package.json"),
        JSON.stringify({ name: "daisyclaw", packageManager: "pnpm@10.0.0" }),
        "utf8",
      );
