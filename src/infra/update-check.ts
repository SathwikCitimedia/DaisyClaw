// Computes git, dependency, and registry update status for DaisyClaw installs.
import fs from "node:fs/promises";
import path from "node:path";
import { runCommandWithTimeout } from "../process/exec.js";
import { fetchWithTimeout } from "../utils/fetch-timeout.js";
import { detectPackageManager as detectPackageManagerImpl } from "./detect-package-manager.js";
import { compareDaisyClawReleaseVersions } from "./npm-registry-spec.js";
...
export async function fetchNpmPackageTargetStatus(params: {
  ...
  try {
    const res = await fetchWithTimeout(
      `https://registry.npmjs.org/daisyclaw/${encodeURIComponent(target)}`,
      {},
...
export function compareSemverStrings(a: string | null, b: string | null): number | null {
  if (a && b) {
    const daisyClawReleaseCmp = compareDaisyClawReleaseVersions(a, b);
    if (daisyClawReleaseCmp != null) {
      return daisyClawReleaseCmp;
    }
  }
