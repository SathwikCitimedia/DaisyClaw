...
import { bundledDistPluginFile } from "daisyclaw/plugin-sdk/test-fixtures";
...
const execFileSyncMock = vi.hoisted(() => vi.fn(() => "/tmp/daisyclaw-test-global-npmrc\n"));
...
async function writeGlobalPackageJson(packageRoot: string, version = "1.0.0") {
  await fs.writeFile(
    ...
    JSON.stringify({ name: "daisyclaw", version }),
    "utf-8",
  );
}
...
describe("update global helpers", () => {
  ...
  it("prefers explicit package spec overrides", () => {
    envSnapshot = captureEnv(["DAISYCLAW_UPDATE_PACKAGE_SPEC"]);
    process.env.DAISYCLAW_UPDATE_PACKAGE_SPEC = "file:/tmp/daisyclaw.tgz";

    expect(resolveGlobalInstallSpec({ packageName: "daisyclaw", tag: "latest" })).toBe(
      "file:/tmp/daisyclaw.tgz",
    );
    expect(
      resolveGlobalInstallSpec({
        packageName: "daisyclaw",
        tag: "beta",
        env: { DAISYCLAW_UPDATE_PACKAGE_SPEC: "daisyclaw@next" },
      }),
    ).toBe("daisyclaw@next");
  });
  ...
  it("resolves global roots and package roots from runner output", async () => {
    ...
    await expect(resolveGlobalPackageRoot("npm", runCommand, 1000)).resolves.toBe(
      path.join("/tmp/npm-root", "daisyclaw"),
    );
  });

  it("maps main and explicit install specs for global installs", () => {
    expect(resolveGlobalInstallSpec({ packageName: "daisyclaw", tag: "main" })).toBe(
      DAISYCLAW_MAIN_PACKAGE_SPEC,
    );
    expect(
      resolveGlobalInstallSpec({
        packageName: "daisyclaw",
        tag: "github:daisyclaw/daisyclaw#feature/my-branch",
      }),
    ).toBe("github:daisyclaw/daisyclaw#feature/my-branch");
    expect(
      resolveGlobalInstallSpec({
        packageName: "daisyclaw",
        tag: "https://example.com/daisyclaw-main.tgz",
      }),
    ).toBe("https://example.com/daisyclaw-main.tgz");
  });
  ...
  it("resolves portable Git paths from process-local app data only", async () => {
    await withMockedWindowsPlatform(async () => {
      await withTempDir({ prefix: "daisyclaw-update-portable-git-" }, async (base) => {
  ...
  it("classifies main and raw install specs separately from registry selectors", () => {
    ...
    expect(isExplicitPackageInstallSpec("github:daisyclaw/daisyclaw#main")).toBe(true);
    expect(isExplicitPackageInstallSpec("https://example.com/daisyclaw-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("file:/tmp/daisyclaw-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("/tmp/daisyclaw-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("daisyclaw-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("beta")).toBe(false);

    expect(canResolveRegistryVersionForPackageTarget("latest")).toBe(true);
    expect(canResolveRegistryVersionForPackageTarget("2026.3.22")).toBe(true);
    expect(canResolveRegistryVersionForPackageTarget("main")).toBe(false);
    expect(canResolveRegistryVersionForPackageTarget("github:daisyclaw/daisyclaw#main")).toBe(false);
    expect(canResolveRegistryVersionForPackageTarget("/tmp/daisyclaw-main.tgz")).toBe(false);
  });

  it("detects install managers from resolved roots and on-disk presence", async () => {
    await withTempDir({ prefix: "daisyclaw-update-global-" }, async (base) => {
      const npmRoot = path.join(base, "npm-root");
      const pnpmRoot = path.join(base, "pnpm-root");
      const bunRoot = path.join(base, ".bun", "install", "global", "node_modules");
      const pkgRoot = path.join(pnpmRoot, "daisyclaw");
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.mkdir(path.join(npmRoot, "daisyclaw"), { recursive: true });
      await fs.mkdir(path.join(bunRoot, "daisyclaw"), { recursive: true });
      ...
      await fs.rm(path.join(npmRoot, "daisyclaw"), { recursive: true, force: true });
      await fs.rm(path.join(pnpmRoot, "daisyclaw"), { recursive: true, force: true });
  ...
  it("prefers the owning npm prefix when PATH npm points at a different global root", async () => {
    await withMockedPlatform("darwin", async () => {
      await withTempDir({ prefix: "daisyclaw-update-npm-prefix-" }, async (base) => {
        ...
        expect(globalInstallArgs("npm", "daisyclaw@latest", pkgRoot)).toEqual([
          brewNpm,
          "i",
          "-g",
          "daisyclaw@latest",
        ...
        expect(globalInstallFallbackArgs("npm", "daisyclaw@latest", pkgRoot)).toEqual([
          brewNpm,
          "i",
          "-g",
          "daisyclaw@latest",
  ...
  it("does not infer npm ownership from path shape alone when the owning npm binary is absent", async () => {
    await withTempDir({ prefix: "daisyclaw-update-npm-missing-bin-" }, async (base) => {
  ...
  it("honors an explicitly selected direct npm node_modules package root", async () => {
    await withTempDir({ prefix: "daisyclaw-update-managed-service-root-" }, async (base) => {
      const managedNpmRoot = path.join(base, ".daisyclaw", "npm", "node_modules");
      const pkgRoot = path.join(managedNpmRoot, "daisyclaw");
      ...
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.mkdir(path.join(otherPnpmRoot, "daisyclaw"), { recursive: true });
  ...
  it("preserves bun ownership for direct node_modules package roots", async () => {
    await withTempDir({ prefix: "daisyclaw-update-managed-bun-root-" }, async (base) => {
      envSnapshot = captureEnv(["BUN_INSTALL"]);
      process.env.BUN_INSTALL = path.join(base, ".bun");
      const bunRoot = path.join(process.env.BUN_INSTALL, "install", "global", "node_modules");
      const pkgRoot = path.join(bunRoot, "daisyclaw");
      const pathNpmRoot = path.join(base, "shell", "lib", "node_modules");
      await fs.mkdir(pkgRoot, { recursive: true });
  ...
  it("prefers npm.cmd for win32-style global npm roots", async () => {
    await withMockedWindowsPlatform(async () => {
      await withTempDir({ prefix: "daisyclaw-update-win32-npm-prefix-" }, async (base) => {
        const npmPrefix = path.join(base, "Roaming", "npm");
        const npmRoot = path.join(npmPrefix, "node_modules");
        const pkgRoot = path.join(npmRoot, "daisyclaw");
  ...
  it("detects custom pnpm global layouts from the running package root", async () => {
    await withTempDir({ prefix: "daisyclaw-update-pnpm-custom-root-" }, async (base) => {
      const customGlobalDir = path.join(base, "custom-pnpm");
      const customGlobalRoot = path.join(customGlobalDir, "5", "node_modules");
      const pkgRoot = path.join(customGlobalRoot, "daisyclaw");
  ...
  it("detects custom pnpm global layouts from virtual-store package roots", async () => {
    await withTempDir({ prefix: "daisyclaw-update-pnpm-virtual-root-" }, async (base) => {
      const customGlobalDir = path.join(base, "custom-pnpm");
      const customGlobalRoot = path.join(customGlobalDir, "5", "node_modules");
      const pkgRoot = path.join(
        ...
        "5",
        ".pnpm",
        "daisyclaw@file+..+pack+daisyclaw-2026.5.6.tgz",
        "node_modules",
        "daisyclaw",
      );
      const defaultPnpmRoot = path.join(base, "default-pnpm", "5", "node_modules");
      ...
      await expect(
        ...
        command: "pnpm",
        globalRoot: customGlobalRoot,
        packageRoot: path.join(customGlobalRoot, "daisyclaw"),
      });
    });
  });

  it("does not infer pnpm ownership without pnpm node_modules metadata", async () => {
    await withTempDir({ prefix: "daisyclaw-update-pnpm-shape-only-" }, async (base) => {
      const customGlobalDir = path.join(base, "custom-pnpm");
      const customGlobalRoot = path.join(customGlobalDir, "5", "node_modules");
      const pkgRoot = path.join(customGlobalRoot, "daisyclaw");
      const defaultPnpmRoot = path.join(base, "default-pnpm", "5", "node_modules");
      ...
      await expect(
        ...
        packageRoot: path.join(defaultPnpmRoot, "daisyclaw"),
      });
    });
  });
  ...
  it("builds install argv and npm fallback argv", () => {
    ...
    expect(globalInstallArgs("npm", "daisyclaw@latest")).toEqual([
      "npm",
      "i",
      "-g",
      "daisyclaw@latest",
    ...
    expect(globalInstallArgs("pnpm", "daisyclaw@latest")).toEqual([
      "pnpm",
      "add",
      "-g",
      "daisyclaw@latest",
    ]);
    expect(globalInstallArgs("pnpm", "github:daisyclaw/daisyclaw#release/2026.5.12")).toEqual([
      "pnpm",
      "add",
      "-g",
      "--allow-build=daisyclaw",
      "github:daisyclaw/daisyclaw#release/2026.5.12",
    ]);
    expect(
      globalInstallArgs("pnpm", "daisyclaw@git+https://github.com/daisyclaw/daisyclaw.git"),
    ).toEqual([
      "pnpm",
      "add",
      "-g",
      "--allow-build=daisyclaw",
      "daisyclaw@git+https://github.com/daisyclaw/daisyclaw.git",
    ]);
    expect(globalInstallArgs("bun", "daisyclaw@latest")).toEqual([
      "bun",
      "add",
      "-g",
      "daisyclaw@latest",
    ...
    expect(globalInstallFallbackArgs("npm", "daisyclaw@latest")).toEqual([
      "npm",
      "i",
      "-g",
      "daisyclaw@latest",
    ...
    expect(globalInstallFallbackArgs("pnpm", "daisyclaw@latest")).toBeNull();
    expect(
      globalInstallArgs({ manager: "pnpm", command: "/opt/homebrew/bin/pnpm" }, "daisyclaw@latest"),
    ).toEqual(["/opt/homebrew/bin/pnpm", "add", "-g", "daisyclaw@latest"]);
    expect(globalInstallArgs("pnpm", "daisyclaw@latest", null, "/opt/pnpm-global")).toEqual([
      "pnpm",
      "add",
      "-g",
      "--global-dir",
  ...
  it("builds npm staged install argv with an explicit prefix", () => {
    expect(globalInstallArgs("npm", "daisyclaw@latest", null, "/tmp/stage")).toEqual([
      "npm",
      "i",
      "-g",
      "--prefix",
    ...
    expect(globalInstallFallbackArgs("npm", "daisyclaw@latest", null, "/tmp/stage")).toEqual([
      "npm",
      "i",
      "-g",
      "--prefix",
  ...
  it("resolves npm prefix layouts for normal global roots", () => {
    expect(resolveNpmGlobalPrefixLayoutFromGlobalRoot("/opt/daisyclaw/lib/node_modules")).toEqual({
      prefix: "/opt/daisyclaw",
      globalRoot: "/opt/daisyclaw/lib/node_modules",
      binDir: "/opt/daisyclaw/bin",
    });
  ...
  it("cleans only renamed package directories", async () => {
    await withTempDir({ prefix: "daisyclaw-update-cleanup-" }, async (root) => {
      await fs.mkdir(path.join(root, ".daisyclaw-123"), { recursive: true });
      await fs.mkdir(path.join(root, ".daisyclaw-456"), { recursive: true });
      await fs.writeFile(path.join(root, ".daisyclaw-file"), "nope", "utf8");
      await fs.mkdir(path.join(root, "daisyclaw"), { recursive: true });
      ...
      await expect(
        ...
        removed: [".daisyclaw-123", ".daisyclaw-456"],
      });
      const packageDirStat = await fs.stat(path.join(root, "daisyclaw"));
      const markerFileStat = await fs.stat(path.join(root, ".daisyclaw-file"));
  ...
  it("checks installed dist against the packaged inventory", async () => {
    await withTempDir({ prefix: "daisyclaw-update-global-pkg-" }, async (packageRoot) => {
  ...
  it("reports bundled plugin install stages during installed dist verification", async () => {
    await withTempDir({ prefix: "daisyclaw-update-global-plugin-stage-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
      await fs.mkdir(path.join(packageRoot, "dist", "extensions", "brave"), { recursive: true });
      await writePackageDistInventory(packageRoot);

      for (const stageDir of [".daisyclaw-install-stage", ".daisyclaw-install-stage-retry"]) {
      ...
      await expect(collectInstalledGlobalPackageErrors({ packageRoot })).resolves.toEqual([
        "unexpected packaged dist file dist/extensions/brave/.daisyclaw-install-stage-retry/node_modules/typebox/build/compile/code.mjs",
        "unexpected packaged dist file dist/extensions/brave/.daisyclaw-install-stage/node_modules/typebox/build/compile/code.mjs",
      ]);
    });
  });

  it("flags global package roots that resolve into source checkouts", async () => {
    await withTempDir({ prefix: "daisyclaw-update-global-source-checkout-" }, async (base) => {
      const checkoutRoot = path.join(base, "checkout");
      const globalRoot = path.join(base, "prefix", "lib", "node_modules");
      const packageRoot = path.join(globalRoot, "daisyclaw");
  ...
  it("does not require private QA sidecars when the inventory is missing", async () => {
    await withTempDir({ prefix: "daisyclaw-update-global-legacy-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
  ...
  it("fails closed on newer installs when the inventory is missing", async () => {
    await withTempDir(
      { prefix: "daisyclaw-update-global-missing-inventory-new-" },
      async (packageRoot) => {
  ...
  it("rejects invalid inventory files during global verify", async () => {
    await withTempDir(
      { prefix: "daisyclaw-update-global-invalid-inventory-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
  ...
  it("verifies legacy sidecars for installed bundled plugins without inventory", async () => {
    await withTempDir({ prefix: "daisyclaw-update-global-legacy-plugin-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
      await writeBundledPluginPackageJson(packageRoot, "telegram", "@daisyclaw/telegram");
  ...
  it("still enforces critical sidecars when the inventory omits them", async () => {
    await withTempDir(
      { prefix: "daisyclaw-update-global-critical-sidecars-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
        await writeBundledPluginPackageJson(packageRoot, "telegram", "@daisyclaw/telegram");
        await writePackageDistInventory(packageRoot);
  ...
  it("ignores stale metadata for non-packaged private QA plugins during inventory verify", async () => {
    await withTempDir(
      { prefix: "daisyclaw-update-global-stale-private-qa-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
        await writeBundledPluginPackageJson(packageRoot, "qa-lab", "@daisyclaw/qa-lab");
        await writePackageDistInventory(packageRoot);
