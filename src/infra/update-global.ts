...
/** Supported package managers for DaisyClaw global install and update flows. */
...
const PRIMARY_PACKAGE_NAME = "daisyclaw";
const ALL_PACKAGE_NAMES = [PRIMARY_PACKAGE_NAME] as const;
const GLOBAL_RENAME_PREFIX = ".";
/** npm-compatible spec used when the user asks to install the moving main branch. */
export const DAISYCLAW_MAIN_PACKAGE_SPEC = "github:daisyclaw/daisyclaw#main";
const COREPACK_ENABLE_DOWNLOAD_PROMPT_DEFAULT = "0";
const NPM_GLOBAL_INSTALL_QUIET_FLAGS = ["--no-fund", "--no-audit", "--loglevel=error"] as const;
const PNPM_DAISYCLAW_BUILD_ALLOWLIST_FLAG = `--allow-build=${PRIMARY_PACKAGE_NAME}`;
...
function isPnpmDaisyClawSourceInstallSpec(spec: string): boolean {
...
/**
 * Extracts a pinned installed version from package specs like `daisyclaw@1.2.3`.
...
/**
 * Verifies that a global package root looks like a packaged DaisyClaw install
  ...
  await Promise.all(
    BUNDLED_RUNTIME_SIDECAR_PATHS.map(async (relativePath) => {
      ...
      if (
        ...
        (await pathExists(path.join(packageRoot, pluginRoot, "daisyclaw.plugin.json")))
      ) {
...
async function resolvePortableGitPathPrepend(): Promise<string[]> {
  ...
  const portableGitRoot = path.join(localAppData, "DaisyClaw", "deps", "portable-git");
  const candidates = [
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.DAISYCLAW_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.DAISYCLAW_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
  const target = normalizePackageTarget(params.tag);
  if (isMainPackageTarget(target)) {
    return DAISYCLAW_MAIN_PACKAGE_SPEC;
  }
 ...
 * Used for staged installs where DaisyClaw creates the prefix itself.
 */
...
/** Resolves the DaisyClaw package root under a package manager's global root. */
...
/**
 * Detects an installed global DaisyClaw package by probing package-manager roots
...
/**
 * Builds the primary package-manager argv for a global DaisyClaw install.
...
export function globalInstallArgs(
  ...
  if (resolved.manager === "pnpm") {
    return [
      ...
      ...(isPnpmDaisyClawSourceInstallSpec(spec) ? [PNPM_DAISYCLAW_BUILD_ALLOWLIST_FLAG] : []),
      spec,
    ];
  }
  if (resolved.manager === "bun") {
