...
async function runHelperWithExistingSentinel(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-handoff-helper-test-"));
  tempDirs.add(tmpDir);
  ...
  await startManagedServiceUpdateHandoff({
    ...
    argv1: "/opt/daisyclaw/daisyclaw.mjs",
...
describe("managed service update handoff", () => {
  it("strips process supervisor hints while preserving service identity for the CLI handoff", async () => {
    ...
    const serviceIdentityEnv = {
      DAISYCLAW_LAUNCHD_LABEL: "com.example.daisyclaw.test",
      DAISYCLAW_SYSTEMD_UNIT: "daisyclaw-test.service",
      DAISYCLAW_WINDOWS_TASK_NAME: "DaisyClaw Test Gateway",
    ...
    expect(result.command).toBe("daisyclaw update --yes --timeout 1800");
    ...
    expect(options.env.DAISYCLAW_UPDATE_RUN_HANDOFF).toBe("1");
  ...
  it("launches systemd handoffs through a transient user scope", async () => {
    const { startManagedServiceUpdateHandoff } =
      await import("./update-managed-service-handoff.js");
    const binDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-systemd-run-bin-"));
    tempDirs.add(binDir);
    ...
    expect(args.slice(0, 4)).toEqual([
      ...
      "--scope",
      "--collect",
      "--unit=daisyclaw-update-handoff-123.scope",
    ]);
    ...
    expect(options.detached).toBe(true);
    expect(options.env.DAISYCLAW_SYSTEMD_UNIT).toBe("daisyclaw-gateway.service");
  ...
  it("sweeps stale handoff temp directories while keeping fresh handoff logs", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-handoff-cleanup-test-"));
    tempDirs.add(tmpDir);
    ...
    const unrelatedDir = path.join(tmpDir, "daisyclaw-other-temp");
