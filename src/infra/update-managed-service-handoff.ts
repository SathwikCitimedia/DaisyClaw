...
const SERVICE_IDENTITY_ENV_VARS = new Set<string>([
  ...
  "DAISYCLAW_SYSTEMD_UNIT",
  "DAISYCLAW_WINDOWS_TASK_NAME",
] as const);
...
export function formatManagedServiceUpdateCommand(params?: {
  ...
  const args = ["daisyclaw", "update", "--yes"];
  if (params?.channel) {
...
function buildSystemdHandoffUnitName(handoffId: string | undefined): string {
  const suffix =
    ...
    "handoff";
  return `daisyclaw-update-${suffix}.scope`;
}
...
async function resolveHandoffSpawn(params: {
  ...
  if (!systemdRunPath) {
    throw new Error(
      "systemd-run is required to start the managed update handoff outside daisyclaw-gateway.service",
    );
  }
...
export async function startManagedServiceUpdateHandoff(params: {
  ...
  const env = {
    ...
    DAISYCLAW_UPDATE_RUN_HANDOFF: "1",
  };
