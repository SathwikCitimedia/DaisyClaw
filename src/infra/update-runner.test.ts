...
const fixtureRootTracker = createSuiteTempRootTracker({ prefix: "daisyclaw-update-" });
...
describe("runGatewayUpdate", () => {
  const preflightPrefixPattern = /(?:daisyclaw-update-preflight-|ocu-pf-)/;
  ...
  beforeEach(async () => {
    ...
    await fs.writeFile(path.join(tempDir, "daisyclaw.mjs"), "export {};\n", "utf-8");
  });
  ...
  async function createStableTagRunner(params: {
    ...
    const doctorKey = `${doctorNodePath} ${path.join(tempDir, "daisyclaw.mjs")} doctor --non-interactive --fix`;
  ...
  async function setupGitCheckout(options?: { packageManager?: string }) {
    ...
    const pkg: Record<string, string> = { name: "daisyclaw", version: "1.0.0" };
  ...
  async function writePreflightPackageManagerFixture(root: string, packageManager = "pnpm@8.0.0") {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "daisyclaw", version: "1.0.0", packageManager }),
      "utf-8",
    );
  }
  ...
  function createGlobalNpmUpdateRunner(params: {
    ...
    const baseInstallKey = npmGlobalInstallCommand("daisyclaw@latest");
    const omitOptionalInstallKey = npmGlobalInstallCommand("daisyclaw@latest", ["--omit=optional"]);
  ...
  it("does not fetch tags for dev updates", async () => {
    ...
    const doctorNodePath = await resolveStableNodePath(process.execPath);
    const doctorCommand = `${doctorNodePath} ${path.join(tempDir, "daisyclaw.mjs")} doctor --non-interactive --fix`;
  ...
  it("uses pnpm highest resolution mode for update installs", async () => {
    ...
    const { runCommand } = createGitInstallRunner({
      ...
      doctorCommand: `${doctorNodePath} ${path.join(tempDir, "daisyclaw.mjs")} doctor --non-interactive --fix`,
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    const { runner, calls } = createRunner({
      ...
      "pnpm build": { stdout: "" },
      "pnpm ui:build": { stdout: "" },
      [`${doctorNodePath} ${path.join(tempDir, "daisyclaw.mjs")} doctor --non-interactive --fix`]: {
        stdout: "",
      },
    });
  ...
  it("bootstraps pnpm via npm when pnpm and corepack are unavailable", async () => {
    ...
    const { calls, runCommand } = createGitInstallRunner({
      ...
      buildCommand: "pnpm build",
      uiBuildCommand: "pnpm ui:build",
      doctorCommand: `${process.execPath} ${path.join(tempDir, "daisyclaw.mjs")} doctor --non-interactive`,
      onCommand: (key, options) => {
        if (key === "pnpm --version") {
          const envPath = options?.env?.PATH ?? options?.env?.Path ?? "";
          if (envPath.includes("daisyclaw-update-pnpm-")) {
  ...
  it("uses npm-bootstrapped pnpm for dev preflight when pnpm and corepack are missing", async () => {
    ...
    expect(pnpmEnvPaths.filter((envPath) => envPath.includes("daisyclaw-update-pnpm-"))).not.toEqual(
      [],
    );
  });
  ...
  it("runs dev preflight lint in constrained mode when explicitly enabled", async () => {
    ...
    const result = await withEnvAsync({ DAISYCLAW_UPDATE_PREFLIGHT_LINT: "1" }, async () =>
    ...
    expect(lintEnv[0]?.DAISYCLAW_LOCAL_CHECK).toBe("1");
    expect(lintEnv[0]?.DAISYCLAW_LOCAL_CHECK_MODE).toBe("throttled");
    expect(lintEnv[0]?.DAISYCLAW_OXLINT_SHARDS_SERIAL).toBe("1");
  });
  ...
  it("retries windows pnpm git installs with --ignore-scripts for dev updates", async () => {
    ...
    await withMockedWindowsPlatform(async () => {
      const runCommand = async (
        ...
        if (key === "pnpm install") {
          if (options?.cwd && /(?:daisyclaw-update-preflight-|ocu-pf-)/.test(options.cwd)) {
  ...
  it("falls back to the cloned cwd when git root probing misses a fresh checkout", async () => {
    ...
    const doctorNodePath = await resolveStableNodePath(process.execPath);
    const doctorCommand = `${doctorNodePath} ${path.join(gitRoot, "daisyclaw.mjs")} doctor --non-interactive --fix`;
  ...
  it("skips update when no git root", async () => {
    await fs.writeFile(
      ...
      JSON.stringify({ name: "daisyclaw", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
  ...
  async function runNpmGlobalUpdateCase(params: {
    ...
    const { calls, runCommand } = createGlobalInstallHarness({
      ...
      onInstall: async () => {
        await fs.writeFile(
          path.join(pkgRoot, "package.json"),
          JSON.stringify({ name: "daisyclaw", version: "2.0.0" }),
          "utf-8",
        );
      },
    });
  ...
  const createGlobalInstallHarness = (params: {
    ...
    const runCommand = async (argv: string[], options?: { env?: NodeJS.ProcessEnv }) => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        await fs.writeFile(path.join(destination, "daisyclaw-2.0.0.tgz"), "packed\n", "utf-8");
        return {
          stdout: JSON.stringify([{ filename: "daisyclaw-2.0.0.tgz" }]),
          stderr: "",
          code: 0,
        };
      }
        ...
        if (installCommandMatches(params.installCommand, normalizedInstallCommand)) {
          const packageRoot =
            process.platform === "win32"
              ? path.join(installPrefix, "node_modules", "daisyclaw")
              : path.join(installPrefix, "lib", "node_modules", "daisyclaw");
  ...
  it.each([
    {
      ...
      expectedInstallCommand: npmGlobalInstallCommand("daisyclaw@latest"),
    },
    {
      title: "uses update channel for global npm installs when tag is omitted",
      expectedInstallCommand: npmGlobalInstallCommand("daisyclaw@beta"),
  ...
  it("updates global npm installs from the GitHub main package spec", async () => {
    const sourceSpec = "github:daisyclaw/daisyclaw#main";
    const { calls, result } = await runNpmGlobalUpdateCase({
      expectedInstallCommand: (argv) =>
        argv[0] === "npm" &&
        argv[1] === "i" &&
        argv[2] === "-g" &&
        path.basename(argv[3] ?? "") === "daisyclaw-2.0.0.tgz" &&
    ...
    const installCall = calls.find((call) => call.includes("daisyclaw-2.0.0.tgz"));
  ...
  it("runs doctor after global npm updates before reporting success", async () => {
    ...
    const { calls, runCommand } = createGlobalInstallHarness({
      pkgRoot,
      npmRootOutput: nodeModules,
      installCommand: npmGlobalInstallCommand("daisyclaw@latest"),
      onInstall: async () => {
    ...
    expect(result.steps.map((step) => step.name)).toContain("daisyclaw doctor");
    expect(doctorEnv?.DAISYCLAW_UPDATE_IN_PROGRESS).toBe("1");
    expect(doctorEnv?.DAISYCLAW_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE).toBe("1");
    expect(doctorEnv?.DAISYCLAW_COMPATIBILITY_HOST_VERSION).toBe("2.0.0");
  });
  ...
  it("fails global npm updates when post-update doctor fails", async () => {
    ...
    expect(lastStep?.name).toBe("daisyclaw doctor");
  ...
  it("falls back to global npm update when git is missing from PATH", async () => {
    ...
    expect(result.status).toBe("ok");
    expect(result.mode).toBe("npm");
    expect(calls).toContain(npmGlobalInstallCommand("daisyclaw@latest"));
  });

  it("cleans stale npm rename dirs before global update", async () => {
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "daisyclaw");
    const staleDir = path.join(nodeModules, ".daisyclaw-stale");
  ...
  it("fails global npm update when the installed version misses the requested correction", async () => {
    const { calls, result } = await runNpmGlobalUpdateCase({
      expectedInstallCommand: npmGlobalInstallCommand("daisyclaw@2026.3.23-2"),
      tag: "2026.3.23-2",
    });
    ...
    expect(calls).toContain(npmGlobalInstallCommand("daisyclaw@2026.3.23-2"));
  });

  it("fails global npm update when bundled runtime sidecars are missing after install", async () => {
    const { nodeModules, pkgRoot } = await createGlobalPackageFixture(tempDir);
    const expectedInstallCommand = npmGlobalInstallCommand("daisyclaw@latest");
    ...
    const { runCommand } = createGlobalInstallHarness({
      ...
      onInstall: async (options) => {
        ...
        if (options?.installPrefix) {
          ...
          await fs.writeFile(path.join(binDir, "daisyclaw"), "#!/bin/sh\n", "utf-8");
        }
      },
    });
  ...
  it("uses DAISYCLAW_UPDATE_PACKAGE_SPEC for global package updates", async () => {
    const { nodeModules, pkgRoot } = await createGlobalPackageFixture(tempDir);
    const expectedInstallCommand = npmGlobalInstallCommand(
      "http://10.211.55.2:8138/daisyclaw-next.tgz",
    );
    const { calls, runCommand } = createGlobalInstallHarness({
      pkgRoot,
      npmRootOutput: nodeModules,
  ...
  it("updates global bun installs when detected", async () => {
    ...
    await withEnvAsync({ BUN_INSTALL: bunInstall }, async () => {
      ...
      const { calls, runCommand } = createGlobalInstallHarness({
        pkgRoot,
        installCommand: "bun add -g daisyclaw@latest",
        onInstall: async () => {
          await writeGlobalPackageVersion(pkgRoot);
        },
      });
      ...
      expect(result.before?.version).toBe("1.0.0");
      expect(result.after?.version).toBe("2.0.0");
      expect(calls).toContain("bun add -g daisyclaw@latest");
    });
  });

  it("rejects git roots that are not a daisyclaw checkout", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    ...
    expect(result.status).toBe("error");
    expect(result.reason).toBe("not-daisyclaw-root");
    expect(calls.filter((call) => call.includes("status --porcelain"))).toEqual([]);
  });

  it("fails with a clear reason when daisyclaw.mjs is missing", async () => {
    await setupGitCheckout({ packageManager: "pnpm@8.0.0" });
    await fs.rm(path.join(tempDir, "daisyclaw.mjs"), { force: true });
    ...
    expect(result.steps.some((step) => step.name === "daisyclaw doctor entry")).toBe(true);
