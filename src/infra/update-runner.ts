// Runs DaisyClaw package update checks, package steps, and restart handoff.
...
const DEFAULT_PACKAGE_NAME = "daisyclaw";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR_ENV =
  "DAISYCLAW_UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR";
const UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE_ENV =
  "DAISYCLAW_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE";
const PREFLIGHT_TEMP_PREFIX =
  process.platform === "win32" ? "ocu-pf-" : "daisyclaw-update-preflight-";
...
const DEV_PREFLIGHT_LINT_ENV: NodeJS.ProcessEnv = {
  ...
  DAISYCLAW_OXLINT_SHARDS_SERIAL: "1",
};
const DEV_PREFLIGHT_LINT_OPT_IN_ENV = "DAISYCLAW_UPDATE_PREFLIGHT_LINT";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["daisyclaw", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && (await pathsReferToSameLocation(gitRoot, pkgRoot))) {
    ...
    try {
      ...
      const doctorEntry = path.join(gitRoot, "daisyclaw.mjs");
      ...
      const doctorStep = await runStep(
        step("daisyclaw doctor", doctorArgv, gitRoot, {
