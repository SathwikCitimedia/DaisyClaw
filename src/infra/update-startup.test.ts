...
const {
  ...
  startManagedServiceUpdateHandoffMock: vi.fn(async () => ({
    ...
    pid: 12345,
    command: "daisyclaw update --yes --channel beta --timeout 2700",
    logPath: "/tmp/daisyclaw-handoff.log",
  })),
}));

vi.mock("./daisyclaw-root.js", async () => {
  const actual = await vi.importActual<typeof import("./daisyclaw-root.js")>("./daisyclaw-root.js");
  return {
    ...actual,
...
describe("update-startup", () => {
  let tempDir: string;
  let testState: DaisyClawTestState;
  ...
  beforeEach(async () => {
    ...
    testState = await createDaisyClawTestState({
      layout: "state-only",
      prefix: "daisyclaw-update-check-suite-",
      env: {
        DAISYCLAW_NO_AUTO_UPDATE: undefined,
        DAISYCLAW_SERVICE_KIND: undefined,
        DAISYCLAW_SERVICE_MARKER: undefined,
        DAISYCLAW_GATEWAY_SERVICE_PID: undefined,
        DAISYCLAW_LAUNCHD_LABEL: undefined,
        DAISYCLAW_SYSTEMD_UNIT: undefined,
        DAISYCLAW_WINDOWS_TASK_NAME: undefined,
    ...
    if (!loaded) {
      ({ resolveDaisyClawPackageRoot } = await import("./daisyclaw-root.js"));
    ...
    vi.mocked(resolveDaisyClawPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveDaisyClawPackageRoot).mockResolvedValue("/opt/daisyclaw");
  ...
  it.each([
    ...
    expect(log.info).toHaveBeenCalledWith(
      `update available (latest): v2.0.0 (current v1.0.0). Run: ${formatCliCommand("daisyclaw update")}`,
    );
  ...
  it("honors DAISYCLAW_NO_AUTO_UPDATE for configured auto-updates", async () => {
    mockPackageUpdateStatus("beta", "2.0.0-beta.1");
    process.env.DAISYCLAW_NO_AUTO_UPDATE = "1";
    const log = { info: vi.fn() };
    const runAutoUpdate = createAutoUpdateSuccessMock();
    ...
    const disabledLogCall = log.info.mock.calls.find(
      ([message]) => message === "auto-update disabled by DAISYCLAW_NO_AUTO_UPDATE",
    );
    expect(disabledLogCall).toEqual([
      "auto-update disabled by DAISYCLAW_NO_AUTO_UPDATE",
      {
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/daisyclaw/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
    ...
    expect(options.env).toEqual({ DAISYCLAW_AUTO_UPDATE: "1" });
  });
