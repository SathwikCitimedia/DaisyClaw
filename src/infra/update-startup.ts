...
import { resolveDaisyClawPackageRoot } from "./daisyclaw-root.js";
...
function resolveAutoUpdatePolicy(cfg: DaisyClawConfig): AutoUpdatePolicy {
...
function resolveCheckIntervalMs(cfg: DaisyClawConfig): number {
...
export async function runGatewayUpdateCheck(params: {
  ...
  const autoDisabledByEnv = isTruthyEnvValue(process.env.DAISYCLAW_NO_AUTO_UPDATE);
  ...
  const root = await resolveDaisyClawPackageRoot({
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("daisyclaw update")}`,
      );
    ...
    if (auto.enabled && autoDisabledByEnv) {
      params.log.info("auto-update disabled by DAISYCLAW_NO_AUTO_UPDATE", {
