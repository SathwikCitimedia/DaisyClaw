...
const baseZoneOpts: WideAreaGatewayZoneOpts = {
  domain: "daisyclaw.internal.",
  gatewayPort: 18789,
  displayName: "Mac Studio (DaisyClaw)",
...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "daisyclaw.internal", expected: "daisyclaw.internal." },
    { value: "daisyclaw.internal.", expected: "daisyclaw.internal." },
    { value: "  daisyclaw.internal  ", expected: "daisyclaw.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { DAISYCLAW_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { DAISYCLAW_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
    ...
    },
    {
      ...
      params: {
        env: { DAISYCLAW_WIDE_AREA_DOMAIN: "foo/bar" } as NodeJS.ProcessEnv,
      },
      expected: null,
    },
  ...
  it("builds valid zone paths under the DNS config directory", () => {
    ...
    const zonePath = getWideAreaZonePath("daisyclaw.internal.");

    expect(zonePath).toBe(path.join(dnsDir, "daisyclaw.internal.db"));
    expect(path.relative(dnsDir, zonePath)).toBe("daisyclaw.internal.db");
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    ...
    expectZoneRecords(txt, [
      `$ORIGIN daisyclaw.internal.`,
      `studio-london IN A 100.123.224.76`,
      `studio-london IN AAAA fd7a:115c:a1e0::8801:e04c`,
      `_daisyclaw-gw._tcp IN PTR studio-london._daisyclaw-gw._tcp`,
      `studio-london._daisyclaw-gw._tcp IN SRV 0 0 18789 studio-london`,
      `displayName=Mac Studio (DaisyClaw)`,
      `gatewayPort=18789`,
      `sshPort=22`,
      `cliPath=/opt/homebrew/bin/daisyclaw`,
  ...
  it.each([
    ...
    },
    {
      ...
      records: [
        `$ORIGIN daisyclaw.internal.`,
        `studio-london IN A 100.123.224.76`,
        `studio-london._daisyclaw-gw._tcp IN TXT`,
        `displayName=Mac Studio (DaisyClaw)`,
        `gatewayTls=1`,
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("daisyclaw.internal."),
      changed: false,
    });
    expect(writeSpy).not.toHaveBeenCalled();
  });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("daisyclaw.internal."),
      expectedZoneText,
      "utf-8",
    );
  });
