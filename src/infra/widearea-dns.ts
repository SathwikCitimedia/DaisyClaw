...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.DAISYCLAW_WIDE_AREA_DOMAIN ?? null;
  try {
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*daisyclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "daisyclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "daisyclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "daisyclaw-gw");
  ...
  records.push(`_daisyclaw-gw._tcp IN PTR ${instanceLabel}._daisyclaw-gw._tcp`);
  records.push(`${instanceLabel}._daisyclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._daisyclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; daisyclaw-content-hash: ${contentHash}\n${contentBody}`;
}
