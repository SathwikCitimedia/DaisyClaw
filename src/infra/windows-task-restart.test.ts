...
const resolvePreferredDaisyClawTmpDirMock = vi.hoisted(() => vi.fn(() => os.tmpdir()));
const resolveTaskScriptPathMock = vi.hoisted(() =>
  vi.fn((env: Record<string, string | undefined>) => {
    const home = env.USERPROFILE || env.HOME || os.homedir();
    return path.join(home, ".daisyclaw", "gateway.cmd");
  }),
);
...
vi.mock("./tmp-daisyclaw-dir.js", () => ({
  resolvePreferredDaisyClawTmpDir: () => resolvePreferredDaisyClawTmpDirMock(),
}));
...
describe("relaunchGatewayScheduledTask", () => {
  ...
  beforeEach(() => {
    spawnMock.mockReset();
    resolvePreferredDaisyClawTmpDirMock.mockReset();
    resolvePreferredDaisyClawTmpDirMock.mockReturnValue(os.tmpdir());
  ...
  it("writes a detached schtasks relaunch helper", () => {
    ...
    const result = relaunchGatewayScheduledTask({ DAISYCLAW_PROFILE: "work" });

    expect(result.ok).toBe(true);
    expect(result.method).toBe("schtasks");
    expect(result.tried).toContain('schtasks /Run /TN "DaisyClaw Gateway (work)"');
    ...
    expect(script).toContain(
      'daisyclaw restart attempt source=windows-task-handoff target="DaisyClaw Gateway (work)"',
    );
    expect(script).toContain(
      `powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command "(Get-ScheduledTask -TaskName 'DaisyClaw Gateway (work)' -ErrorAction SilentlyContinue).State" 2>nul | findstr /I /C:"Running" >nul 2>&1`,
    );
    expect(script).toContain('schtasks /Run /TN "DaisyClaw Gateway (work)" >>');
    expect(script.indexOf("powershell.exe -NoProfile")).toBeLessThan(
      script.indexOf('schtasks /Run /TN "DaisyClaw Gateway (work)"'),
    );
    expect(script).toContain('del "%~f0" >nul 2>&1');
  });

  it("prefers DAISYCLAW_WINDOWS_TASK_NAME overrides", () => {
    spawnMock.mockImplementation((_file: string, args: string[]) => {
      createdScriptPaths.add(decodeCmdPathArg(args[3]));
      return { unref: vi.fn() };
    });
    ...
    const scriptPath = [...createdScriptPaths][0];
    const script = fs.readFileSync(scriptPath, "utf8");
    expect(script).toContain('schtasks /Run /TN "DaisyClaw Gateway (custom)" >>');
  });
  ...
  it("escapes custom task names in the PowerShell running-task probe", () => {
    ...
    relaunchGatewayScheduledTask({
      DAISYCLAW_WINDOWS_TASK_NAME: "DaisyClaw Gateway (Bob's work)",
    });

    const scriptPath = [...createdScriptPaths][0];
    const script = fs.readFileSync(scriptPath, "utf8");
    expect(script).toContain(
      "-Command \"(Get-ScheduledTask -TaskName 'DaisyClaw Gateway (Bob''s work)' -ErrorAction SilentlyContinue).State\"",
    );
  });
  ...
  it("quotes the cmd /c script path when temp paths contain metacharacters", () => {
    const unref = vi.fn();
    const metacharTmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw&(restart)-"));
    createdTmpDirs.add(metacharTmpDir);
    resolvePreferredDaisyClawTmpDirMock.mockReturnValue(metacharTmpDir);
    spawnMock.mockReturnValue({ unref });

    relaunchGatewayScheduledTask({ DAISYCLAW_PROFILE: "work" });
  ...
  it("includes startup fallback", () => {
    const taskScriptDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-state-"));
