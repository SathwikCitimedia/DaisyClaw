...
import { createAssistantMessageEventStream } from "daisyclaw/plugin-sdk/llm";
...
describe("createCodexNativeWebSearchWrapper", () => {
  ...
  it("does not enable code-mode transport enforcement when config is on but controls are inactive", () => {
    ...
    expect(observedOptions[0]?.daisyclawCodeModeToolSurface).toBeUndefined();
  ...
  it("enforces the code-mode transport surface when the run enables it at agent scope", () => {
    ...
    expect(observedOptions[0]?.daisyclawCodeModeToolSurface).toBe(true);
    expect(payloads[0]?.tools).toEqual([
      { type: "function", name: "exec" },
      { type: "function", name: "wait" },
    ]);
...
describe("createOpenAIAttributionHeadersWrapper", () => {
  it("routes native Codex traffic through the DaisyClaw transport so attribution survives DaisyClaw defaults", () => {
    let codexCalls = 0;
    ...
    expect(capturedHeaders?.originator).toBe("daisyclaw");
    expect(capturedHeaders?.["User-Agent"]).toMatch(/^daisyclaw\//);
  });
  ...
  it("keeps existing wrapped Codex streams so runtime OAuth injection is preserved", () => {
    ...
    expect(capturedOptions?.headers?.originator).toBe("daisyclaw");
    expect(capturedOptions?.headers?.["User-Agent"]).toMatch(/^daisyclaw\//);
  });
});
