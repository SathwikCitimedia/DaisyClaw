...
type DaisyClawSimpleStreamOptions = SimpleStreamOptions & {
...
function isCodeModeEnabled(config?: DaisyClawConfig): boolean {
...
export function createCodexNativeWebSearchWrapper(
  ...
  return (model, context, options) => {
    if (
      ...
      const originalOnPayload = options?.onPayload;
      const codeModeOptions: DaisyClawSimpleStreamOptions = {
        ...options,
