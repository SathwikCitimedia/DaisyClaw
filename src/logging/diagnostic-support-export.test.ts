...
describe("diagnostic support export", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-support-export-"));
  ...
  it("writes a shareable zip without raw chats, webhook bodies, or secrets", async () => {
    ...
    const logTail: LogTailPayload = {
      file: path.join(tempDir, "logs", "daisyclaw.log"),
      cursor: 200,
      size: 200,
      truncated: false,
      reset: false,
    ...
    const result = await writeDiagnosticSupportExport({
      ...
      readStatusSnapshot: async () => ({
        service: {
          loaded: true,
          command: {
            programArguments: ["daisyclaw", "gateway", "run", "--token", fakeToken],
            environment: {
              HOME: tempDir,
              DAISYCLAW_GATEWAY_TOKEN: fakeToken,
            },
          },
        },
        gateway: {
    ...
    expect(Object.keys(entries).toSorted()).toEqual([
      ...
      "logs/daisyclaw-sanitized.jsonl",
      "manifest.json",
    ...
    expect(combined).toContain("$DAISYCLAW_STATE_DIR");
    ...
    const sanitizedLogs = entries["logs/daisyclaw-sanitized.jsonl"];
    ...
    expect(status.data?.service?.command?.environment?.DAISYCLAW_GATEWAY_TOKEN).toBe("<redacted>");
  ...
  it("redacts Windows USERPROFILE paths when HOME is unset", () => {
    ...
    const stateDir = `${userProfile}\\AppData\\Roaming\\daisyclaw`;
    const redaction = {
      env: {
    ...
    expect(redactSupportString(`${stateDir}\\logs\\gateway.log`, redaction)).toBe(
      "$DAISYCLAW_STATE_DIR\\logs\\gateway.log",
    );
    expect(
    ...
    const status = sanitizeSupportSnapshotValue(
      {
        service: {
          command: {
            programArguments: [
              "node",
              `${userProfile}\\daisyclaw\\dist\\index.js`,
              "--config",
              `${stateDir}\\daisyclaw.json`,
            ],
            sourcePath: "c:\\users\\support-user\\AppData\\Local\\daisyclaw\\gateway-service.json",
          },
        },
      },
      redaction,
    ...
    expect(serialized).toContain("~\\\\daisyclaw\\\\dist\\\\index.js");
    expect(serialized).toContain("$DAISYCLAW_STATE_DIR\\\\daisyclaw.json");
    expect(serialized).toContain("~\\\\AppData\\\\Local\\\\daisyclaw\\\\gateway-service.json");
  });
  ...
  it("keeps writing when log tail collection fails", async () => {
    ...
    await writeDiagnosticSupportExport({
      ...
      readLogTail: async () => {
        throw new Error(`log tail failed at ${tempDir}/daisyclaw.log with token ${fakeToken}`);
      },
    });

    const entries = await readZipTextEntries(outputPath);
    expect(Object.keys(entries).toSorted()).toContain("logs/daisyclaw-sanitized.jsonl");
