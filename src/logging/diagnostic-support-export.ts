...
const SUPPORT_EXPORT_PREFIX = "daisyclaw-diagnostics-";
...
function resolveBonjourEnvOverride(
  ...
  const raw = env.DAISYCLAW_DISABLE_BONJOUR?.trim().toLowerCase();
  if (!raw) {
    return "unset";
  }
  switch (raw) {
...
function readStabilityBundle(
  ...
  if (target === false) {
    return { status: "missing", dir: "$DAISYCLAW_STATE_DIR/logs/stability" };
  }
...
function renderSummary(params: {
  ...
  return [
    ...
    "- `logs/daisyclaw-sanitized.jsonl`: sanitized log summaries and metadata",
