...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-log-tail-redaction-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("readConfiguredLogTail redaction", () => {
  it("redacts raw auth headers before returning log lines", async () => {
    ...
    const logFile = path.join(dir, "daisyclaw.log");
    const configFile = path.join(dir, "daisyclaw.json");
    ...
    expect(text).not.toContain(daisyClawToken);
