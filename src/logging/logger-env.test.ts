...
const logPathTracker = createSuiteLogPathTracker("daisyclaw-test-env-log-level-");

describe("DAISYCLAW_LOG_LEVEL", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["DAISYCLAW_LOG_LEVEL"]);
  ...
  it("applies a valid env override to both file and console levels", () => {
    ...
    process.env.DAISYCLAW_LOG_LEVEL = "debug";
  ...
  it("warns once and ignores invalid env values", () => {
    ...
    process.env.DAISYCLAW_LOG_LEVEL = "nope";
    ...
    const warnings = stderrSpy.mock.calls
      ...
      .filter((line) => line.includes("DAISYCLAW_LOG_LEVEL"));
    expect(warnings).toHaveLength(1);
    expect(warnings[0]).toContain('Ignoring invalid DAISYCLAW_LOG_LEVEL="nope"');
  });
});
