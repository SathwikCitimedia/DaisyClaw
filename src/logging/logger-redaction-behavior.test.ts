...
const logPathTracker = createSuiteLogPathTracker("daisyclaw-log-redaction-");
...
describe("file log redaction", () => {
  ...
  it("uses logging.file from the active config path", () => {
    ...
    withEnv({ DAISYCLAW_CONFIG_PATH: configPath, DAISYCLAW_TEST_FILE_LOG: "1" }, () => {
  ...
  it("expands leading tilde in logging.file", () => {
    ...
    withEnv({ HOME: home }, () => {
      expect(loggerTest.resolveActiveLogFile("~/custom-daisyclaw.log")).toBe(
        path.join(home, "custom-daisyclaw.log"),
      );
    });
  });
