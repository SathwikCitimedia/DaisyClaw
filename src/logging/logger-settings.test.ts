...
beforeEach(() => {
  envSnapshot = captureEnv(["DAISYCLAW_TEST_FILE_LOG", "DAISYCLAW_LOG_LEVEL"]);
  delete process.env.DAISYCLAW_TEST_FILE_LOG;
  delete process.env.DAISYCLAW_LOG_LEVEL;
  logging.resetLogger();
...
describe("getResolvedLoggerSettings", () => {
  ...
  it("reads logging config when test file logging is explicitly enabled", () => {
    ...
    logging.setLoggerConfigLoaderForTests(() => ({
      level: "debug",
      file: "/tmp/daisyclaw-configured.log",
      maxFileBytes: 2048,
    }));
    ...
    expect(settings.file).toBe("/tmp/daisyclaw-configured.log");
  ...
  it("uses defaults when no logging config is available", () => {
    ...
    expect(path.basename(settings.file)).toMatch(/^daisyclaw-vitest-\d+-\d{4}-\d{2}-\d{2}\.log$/);
    expect(settings.file).not.toBe(
      `/tmp/daisyclaw/daisyclaw-${new Date().toISOString().slice(0, 10)}.log`,
    );
  });
});
