...
function writeConfig(source: string): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-redact-config-"));
  tempDirs.push(dir);
...
describe("redactSensitiveText", () => {
  ...
  it("masks URL userinfo and database connection-string passwords", () => {
    const input = [
      ...
      "postgres://dbuser:database-password-1234567890@db.example.test/daisyclaw",
      "postgres://secret:secret@db.example.test/daisyclaw",
    ...
    expect(output).toContain("postgres://dbuser:databa…7890@db.example.test/daisyclaw");
    expect(output).toContain("postgres://secret:***@db.example.test/daisyclaw");
  ...
  it("does not redact ordinary identifiers containing short token-prefix substrings", () => {
    const input =
      "npm_telegram_package_spec ask_daisyclaw_query_patterns team_management risk_assessment glpat-docs dapi-example sbp_short nfp_site CCIPAT_docs ATATT-example";
    const output = redactSensitiveText(input, { mode: "tools" });
    expect(output).toBe(input);
  });
  ...
  it("masks connection-string passwords through the default options path", () => {
    expect(
      redactSensitiveText("postgres://dbuser:opaquepw12345@db.example.test/daisyclaw", {
        mode: "tools",
      }),
    ).toBe("postgres://dbuser:***@db.example.test/daisyclaw");
  });
