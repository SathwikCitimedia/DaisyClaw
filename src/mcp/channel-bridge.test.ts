...
function makeBridge(verbose = false): BridgeInternals {
  return new DaisyClawChannelBridge({} as never, {
    claudeChannelMode: "off",
    verbose,
  }) as unknown as BridgeInternals;
}

describe("DaisyClawChannelBridge — pendingClaudePermissions / pendingApprovals memory bounds", () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(0);
  });
  ...
  test("a failed notification still emits exactly one diagnostic record with verbose off", async () => {
    ...
    try {
      ...
      expect(writes[0]).toBe("daisyclaw mcp: notification channel/event failed\n");
