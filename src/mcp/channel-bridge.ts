...
/**
 * Runtime bridge between MCP tools and the DaisyClaw Gateway channel APIs.
 *
...
export class DaisyClawChannelBridge {
  ...
  constructor(
    private readonly cfg: DaisyClawConfig,
  ...
  async start(): Promise<void> {
    ...
    this.gateway = new GatewayClientCtor({
      ...
      clientDisplayName: "DaisyClaw MCP",
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`daisyclaw mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
  ...
  private async sendNotification(notification: ServerNotification): Promise<void> {
    ...
    try {
      ...
      process.stderr.write(`daisyclaw mcp: notification ${notification.method} failed\n`);
      if (this.verbose) {
        process.stderr.write(
          `daisyclaw mcp: notification ${notification.method} error: ${String(error)}\n`,
        );
      }
    }
  }
