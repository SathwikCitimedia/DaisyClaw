...
vi.mock("./channel-bridge.js", () => ({
  DaisyClawChannelBridge: class MockDaisyClawChannelBridge {
...
describe("serveDaisyClawChannelMcp shutdown", () => {
  ...
  it("does not leak unhandled rejections when shutdown close fails", async () => {
    ...
    const { serveDaisyClawChannelMcp } = await import("./channel-server.js");

    const servePromise = serveDaisyClawChannelMcp({ verbose: false });
