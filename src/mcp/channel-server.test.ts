...
import { createDaisyClawChannelMcpServer, DaisyClawChannelBridge } from "./channel-server.js";
...
async function connectMcpWithoutGateway(params?: { claudeChannelMode?: "auto" | "on" | "off" }) {
  const serverHarness = await createDaisyClawChannelMcpServer({
...
describe("daisyclaw channel mcp server", () => {
  ...
  describe("gateway-backed flows", () => {
    describe("gateway integration", () => {
      ...
      test("lists conversations and reads messages", async () => {
        ...
        const bridge = new DaisyClawChannelBridge({} as never, {
        ...
        expect((messages[1]?.["__daisyclaw"] as { id?: string } | undefined)?.id).toBe(
          "msg-attachment",
        );
        expect(
