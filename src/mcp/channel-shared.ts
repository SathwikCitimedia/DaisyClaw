...
/** Resolve the visible message id, including DaisyClaw metadata attached to raw entries. */
export function resolveMessageId(entry: Record<string, unknown>): string | undefined {
  return (
    toText(entry.id) ??
    (entry["__daisyclaw"] && typeof entry["__daisyclaw"] === "object"
      ? toText((entry["__daisyclaw"] as { id?: unknown }).id)
      : undefined)
  );
}
