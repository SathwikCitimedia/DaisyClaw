/**
 * Standalone MCP server that exposes DaisyClaw plugin-registered tools
...
function resolvePluginToolPolicy(config: DaisyClawConfig): {
...
function resolveTools(config: DaisyClawConfig): AnyAgentTool[] {
...
export function createPluginToolsMcpServer(
  ...
  return createToolsMcpServer({ name: "daisyclaw-plugin-tools", tools });
}
