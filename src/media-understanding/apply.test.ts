...
const TEMP_MEDIA_PREFIX = "daisyclaw-media-";
...
function createGroqAudioConfig(): DaisyClawConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): DaisyClawConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): DaisyClawConfig {
  return {
  ...
  return await withEnvAsync(
    {
      ...
      DAISYCLAW_ANTIGRAVITY_CLI: undefined,
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: DaisyClawConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredDaisyClawTmpDir();
  ...
  it("uses Antigravity CLI as the last auto image fallback", async () => {
    ...
    const cfg: DaisyClawConfig = { tools: { media: { image: {} } } };
    mockedResolveApiKey.mockResolvedValue({
      source: "none",
      mode: "api-key",
    });
    ...
    expect(probeOptions).toEqual({
      timeoutMs: 3000,
      cwd: expect.stringContaining("daisyclaw-antigravity-probe-"),
    });
