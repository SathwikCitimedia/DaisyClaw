...
const hoisted = vi.hoisted(() => ({
  completeMock: vi.fn(),
  ensureDaisyClawModelsJsonMock: vi.fn(async () => {}),
...
vi.mock("../agents/models-config.js", async () => ({
  ...
  ensureDaisyClawModelsJson: ensureDaisyClawModelsJsonMock,
}));
...
describe("describeImageWithModel", () => {
  ...
  it("resolves configured image models when discovery has not registered the provider", async () => {
    ...
    expect(resolveRequest.agentDir).toBe("/tmp/daisyclaw-agent");
    expect(
      requireRecord(
        requireRecord(
