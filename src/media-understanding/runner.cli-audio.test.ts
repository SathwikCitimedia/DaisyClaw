...
describe("media-understanding CLI audio entry", () => {
  ...
  it("applies per-request prompt and language overrides to CLI transcription templating", async () => {
    ...
    await withAudioFixture("daisyclaw-cli-audio", async ({ ctx, cache }) => {
  ...
  it("treats sherpa structured JSON with empty text as empty output", async () => {
    ...
    await withAudioFixture("daisyclaw-cli-audio-empty-sherpa", async ({ ctx, cache }) => {
      const result = await runCliEntry({
        ...
        cfg: { tools: { media: { audio: {} } } } as DaisyClawConfig,
        ctx,
        attachmentIndex: 0,
        cache,
        config: {} as never,
  ...
  it("extracts sherpa text from the final structured output line", async () => {
    ...
    await withAudioFixture("daisyclaw-cli-audio-sherpa-json", async ({ ctx, cache }) => {
      const result = await runCliEntry({
        capability: "audio",
        entry: {
          type: "cli",
