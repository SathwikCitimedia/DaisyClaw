...
import { mergeInboundPathRoots } from "@daisyclaw/media-core/inbound-path-policy";
import { findNormalizedProviderValue } from "@daisyclaw/model-catalog-core/provider-id";
import {
...
async function probeAntigravityCliCandidate(command: string): Promise<string | null> {
  ...
  const probeDir = await fs.mkdtemp(
    path.join(resolvePreferredDaisyClawTmpDir(), "daisyclaw-antigravity-probe-"),
  );
  try {
...
async function resolveAntigravityCliBinary(): Promise<string | null> {
  ...
  const resolved = (async () => {
    const configured = process.env.DAISYCLAW_ANTIGRAVITY_CLI?.trim();
