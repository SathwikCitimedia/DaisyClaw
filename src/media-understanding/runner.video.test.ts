...
describe("runCapability video provider wiring", () => {
  it("merges video baseUrl and headers with entry precedence", async () => {
    ...
    await withTempDir({ prefix: "daisyclaw-video-auth-" }, async (isolatedAgentDir) => {
      await withVideoFixture("daisyclaw-video-merge", async ({ ctx, media, cache }) => {
        const cfg = {
          models: {
            providers: {
              moonshot: {
  ...
  it("auto-selects moonshot for video when google is unavailable", async () => {
    await withTempDir({ prefix: "daisyclaw-video-agent-" }, async (isolatedAgentDir) => {
      await withEnvAsync(
        {
        ...
        async () => {
          await withVideoFixture("daisyclaw-video-auto-moonshot", async ({ ctx, media, cache }) => {
            const cfg = {
              models: {
                providers: {
                  moonshot: {
  ...
  it("does not use provider api config as video auth modelApi", async () => {
    ...
    await withTempDir({ prefix: "daisyclaw-video-provider-api-" }, async (isolatedAgentDir) => {
      await withVideoFixture("daisyclaw-video-provider-api", async ({ ctx, media, cache }) => {
