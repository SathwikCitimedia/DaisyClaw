...
import { canonicalizeBase64, estimateBase64DecodedBytes } from "@daisyclaw/media-core/base64";
import { parseMediaContentLength } from "@daisyclaw/media-core/content-length";
...
export async function fetchWithGuard(params: {
  ...
  const { response, release } = await fetchWithSsrFGuard({
    ...
    init: { headers: { "User-Agent": "DaisyClaw-Gateway/1.0" } },
  });
