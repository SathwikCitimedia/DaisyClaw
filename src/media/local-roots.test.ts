...
describe("local media roots", () => {
  function withStateDir<T>(stateDir: string, run: () => T): T {
    return withEnv({ DAISYCLAW_STATE_DIR: stateDir }, run);
  }
  ...
  it.each([
    {
      ...
      stateDir: path.join("/tmp", "daisyclaw-media-roots-state"),
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "daisyclaw-agent-media-roots-state"),
  ...
  it.each([
    {
      ...
      stateDir: path.join("/tmp", "daisyclaw-flexible-media-roots-state"),
      cfg: {},
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "daisyclaw-messaging-media-roots-state"),
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "daisyclaw-messaging-fs-media-roots-state"),
      cfg: {
        tools: {
          profile: "messaging",
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "daisyclaw-messaging-read-media-roots-state"),
      cfg: {
        tools: {
          profile: "messaging",
  ...
  it("keeps the config-dir media cache root when state and config paths differ", () => {
    const stateDir = path.join("/tmp", "daisyclaw-legacy-state");
    const configDir = path.join("/tmp", "daisyclaw-current-config");
