...
describe("renderQrPngBase64", () => {
  const tmpRoot = path.join(os.tmpdir(), "daisyclaw-qr-image-tests");
  ...
  it("delegates PNG rendering to qrcode", async () => {
    await expect(renderQrPngBase64("daisyclaw", { scale: 8, marginModules: 2 })).resolves.toBe(
      MOCK_PNG_BASE64,
    );
    expect(toDataURL).toHaveBeenCalledWith("daisyclaw", {
      margin: 2,
      scale: 8,
      type: "image/png",
    });
  ...
  it("uses the default PNG rendering options", async () => {
    await renderQrPngBase64("daisyclaw");
    expect(toDataURL).toHaveBeenCalledWith("daisyclaw", {
      margin: 4,
      scale: 6,
      type: "image/png",
  ...
  it("floors finite PNG rendering options before delegating", async () => {
    await renderQrPngBase64("daisyclaw", { scale: 8.9, marginModules: 2.9 });
    expect(toDataURL).toHaveBeenCalledWith("daisyclaw", {
      margin: 2,
      scale: 8,
      type: "image/png",
  ...
  it.each([
    ...
    await expect(renderQrPngBase64("daisyclaw", { scale, marginModules })).rejects.toThrow(message);
    expect(toDataURL).not.toHaveBeenCalled();
  });

  it("rejects non-PNG qrcode data URLs", async () => {
    toDataURL.mockResolvedValue("data:image/svg+xml;base64,PHN2Zz4=");
    await expect(renderQrPngBase64("daisyclaw")).rejects.toThrow(
  ...
  it("formats QR PNG data URLs", async () => {
    ...
    await expect(renderQrPngDataUrl("daisyclaw")).resolves.toBe(
  ...
  it("writes QR PNGs to a scoped temp file", async () => {
    ...
    const result = await writeQrPngTempFile("daisyclaw", {
      tmpRoot,
      dirPrefix: "pair-",
