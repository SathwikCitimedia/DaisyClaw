...
describe("renderQrTerminal", () => {
  ...
  it("delegates terminal rendering to qrcode", async () => {
    await expect(renderQrTerminal("daisyclaw")).resolves.toBe("ASCII-QR");
    expect(toString).toHaveBeenCalledWith("daisyclaw", {
      small: false,
      type: "terminal",
    });
  });
  ...
  it("renders compact QR output without qrcode terminal small mode", async () => {
    const rendered = await renderQrTerminal("daisyclaw", { small: true });
    expect(rendered).toContain("▄");
    expect(create).toHaveBeenCalledWith("daisyclaw");
