...
describe("media store", () => {
  ...
  beforeAll(async () => {
    tempHome = await createTempHomeEnv("daisyclaw-test-home-");
    home = tempHome.home;
  ...
  it.each([
    {
      ...
      run: async () => {
        await withTempStore(async (storeLocal17, homeLocal4) => {
          ...
          expect(path.normalize(dir)).toContain(`${path.sep}.daisyclaw${path.sep}media`);
    ...
    },
    {
      ...
      run: async () => {
        await withTempStore(async (storeLocal9, homeInner) => {
          ...
          const targetDir = path.join(homeInner, ".daisyclaw", "media", "oversized-stream");
  ...
  it("prefers header mime extension when sniffed mime lacks mapping", async () => {
    await withTempStore(async (_store, homeLocal) => {
      vi.doMock("@daisyclaw/media-core/mime", async () => {
        const actual = await vi.importActual<typeof import("@daisyclaw/media-core/mime")>(
          "@daisyclaw/media-core/mime",
        );
        return {
          ...actual,
      ...
      try {
        ...
        vi.doUnmock("@daisyclaw/media-core/mime");
      }
    });
  });
