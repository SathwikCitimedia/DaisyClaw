...
const CANVAS_HOST_PATH = "/__daisyclaw__/canvas";
...
beforeAll(async () => {
  ...
  fixtureRoot = await fs.mkdtemp(path.join(resolvePreferredDaisyClawTmpDir(), "web-media-core-"));
...
describe("loadWebMedia", () => {
  ...
  it("resolves home-relative local media paths through allowed local roots", async () => {
    await withEnvAsync({ DAISYCLAW_HOME: fixtureRoot }, async () => {
  ...
  it("allows trusted generated host-read HTML reports under DaisyClaw temp root", async () => {
  ...
  it("rejects host-read HTML files outside the trusted DaisyClaw temp root", async () => {
  ...
  it("rejects trusted host-read HTML symlinks that resolve outside DaisyClaw temp root", async () => {
    const outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), "web-media-host-html-"));
  ...
  it("rejects trusted host-read HTML hardlinks to files outside DaisyClaw temp root", async () => {
    const outsideRoot = await fs.mkdtemp(
      path.join(path.dirname(resolvePreferredDaisyClawTmpDir()), "web-media-host-html-"),
    );
    const outsideHtml = path.join(outsideRoot, "report.html");
