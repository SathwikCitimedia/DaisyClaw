...
import { maxBytesForKind, type MediaKind } from "@daisyclaw/media-core/constants";
import { basenameFromAnyPath, extnameFromAnyPath } from "@daisyclaw/media-core/file-name";
import {
  detectMime,
  extensionForMime,
  getFileExtension,
...
async function isTrustedGeneratedHostReadHtmlPath(filePath: string | undefined): Promise<boolean> {
  ...
  const [resolvedFilePath, resolvedTmpRoot] = await Promise.all([
    ...
    realpath(resolvePreferredDaisyClawTmpDir()).catch(() => undefined),
  ]);
  return Boolean(
