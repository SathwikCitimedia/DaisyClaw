// Generates setup codes used to pair external channels with DaisyClaw.
...
function resolvePairingSetupAuthLabel(
  ...
  const envToken = normalizeOptionalString(env.DAISYCLAW_GATEWAY_TOKEN);
  const envPassword = normalizeOptionalString(env.DAISYCLAW_GATEWAY_PASSWORD);
  const token =
...
export async function resolvePairingSetupFromConfig(
  ...
  const cfgForAuth = await materializeGatewayAuthSecretRefs({
    ...
    hasTokenCandidate: Boolean(normalizeOptionalString(env.DAISYCLAW_GATEWAY_TOKEN)),
    hasPasswordCandidate: Boolean(normalizeOptionalString(env.DAISYCLAW_GATEWAY_PASSWORD)),
  });
