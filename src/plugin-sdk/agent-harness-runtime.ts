...
export { VERSION as DAISYCLAW_VERSION } from "../version.js";
...
export { resolveDaisyClawAgentDir } from "./agent-dir-compat.js";
export {
  resolveAgentDir,
...
export async function detectAndLoadAgentHarnessPromptImages(params: {
  ...
  config?: import("../config/types.daisyclaw.js").DaisyClawConfig;
  ...
  const [{ resolveImageSanitizationLimits }, { detectAndLoadPromptImages }, { MAX_IMAGE_BYTES }] =
    await Promise.all([
      ...
      import("@daisyclaw/media-core/constants"),
    ]);
...
/**
 * Derive the same compact user-facing tool detail that embedded DaisyClaw uses for progress logs.
 */
 ...
 * preserve the same DaisyClaw-owned fallback signals as the built-in DaisyClaw path
