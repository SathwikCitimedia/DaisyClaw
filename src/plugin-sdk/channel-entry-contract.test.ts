...
import type { DaisyClawPluginApi, PluginRegistrationMode } from "../plugins/types.js";
...
const pluginModuleLoaderJitiFactoryOverrideKey = Symbol.for(
  "daisyclaw.pluginModuleLoaderJitiFactoryOverride",
);
...
function createApi(registrationMode: PluginRegistrationMode): DaisyClawPluginApi {
  return {
    registrationMode,
    runtime: { registrationMode } as unknown as PluginRuntime,
    registerChannel: vi.fn(),
    registerTool: vi.fn(),
  } as unknown as DaisyClawPluginApi;
}
...
function createBundledChannelEntry(params: {
  importerPath: string;
  pluginId: string;
  registerCliMetadata?: (api: DaisyClawPluginApi) => void;
  registerFull?: (api: DaisyClawPluginApi) => void;
}) {
...
describe("defineBundledChannelEntry", () => {
  it("runs tool registrations without channel sidecar hydration during tool discovery", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-bundled-entry-tools-"));
    ...
    const registerCliMetadata = vi.fn<(api: DaisyClawPluginApi) => void>();
    const registerFull = vi.fn<(api: DaisyClawPluginApi) => void>((api) => {
      api.registerTool(
        {
          name: "channel_tool",
          label: "Channel Tool",
  ...
  it("loads runtime sidecars during discovery registration", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-bundled-entry-runtime-"));
    tempDirs.push(tempRoot);
    const runtimeMarker = path.join(tempRoot, "runtime-loaded");
    ...
    const registerCliMetadata = vi.fn<(api: DaisyClawPluginApi) => void>();
    const registerFull = vi.fn<(api: DaisyClawPluginApi) => void>();
    const entry = createBundledChannelEntry({
      importerPath,
      pluginId,
      registerCliMetadata,
...
describe("defineBundledChannelSetupEntry", () => {
  it("exposes setup-runtime registrations without loading the full channel entry", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-bundled-setup-entry-"));
    tempDirs.push(tempRoot);
    const runtimeMarker = path.join(tempRoot, "runtime-loaded");
    const setupRuntimeRegister = vi.fn<(api: DaisyClawPluginApi) => void>();
...
async function expectBuiltArtifactNodeRequireFastPath(
  ...
  vi.stubEnv("DAISYCLAW_PLUGIN_LOAD_PROFILE", "1");
  ...
  try {
    ...
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-channel-entry-contract-"));
    tempDirs.push(tempRoot);
...
function runCompiledEsmSidecarFastPathProbe(): SpawnSyncReturns<string> {
  ...
  fs.writeFileSync(path.join(tempRoot, "daisyclaw.mjs"), "#!/usr/bin/env node\n", "utf8");
  ...
  return spawnSync(process.execPath, ["--import", "tsx", probePath], {
    cwd: process.cwd(),
    encoding: "utf8",
    env: { ...process.env, DAISYCLAW_PLUGIN_LOAD_PROFILE: "1" },
  });
}
...
describe("loadBundledEntryExportSync", () => {
  ...
  it("normalizes Windows absolute sidecar paths before module loads them", async () => {
    ...
    vi.doMock("../infra/boundary-file-read.js", () => ({
      openRootFileSync: () => ({
        ok: true,
        path: "C:\\Users\\alice\\daisyclaw\\dist\\extensions\\feishu\\helper.ts",
      ...
      try {
        ...
        expect(
          channelEntryContract.loadBundledEntryExportSync<number>(
            "file:///C:/Users/alice/daisyclaw/dist/extensions/feishu/index.js",
            {
              specifier: "./helper.ts",
              exportName: "load",
            },
        ...
        expect(jitiLoad).toHaveBeenCalledWith(
          "file:///C:/Users/alice/daisyclaw/dist/extensions/feishu/helper.ts",
        );
      } finally {
  ...
  it("can disable source-tree fallback for dist bundled entry checks", () => {
    ...
    fs.writeFileSync(path.join(tempRoot, "package.json"), '{"name":"daisyclaw"}\n', "utf8");
    const pluginRoot = path.join(tempRoot, "dist", "extensions", "telegram");
    ...
    vi.stubEnv("DAISYCLAW_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK", "1");
