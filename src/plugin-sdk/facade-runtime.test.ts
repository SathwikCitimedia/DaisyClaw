...
const originalBundledPluginsDir = process.env.DAISYCLAW_BUNDLED_PLUGINS_DIR;
...
function useBundledPluginDirOverrideForTest(dir: string): void {
  process.env.DAISYCLAW_BUNDLED_PLUGINS_DIR = dir;
...
afterEach(() => {
  ...
  if (originalBundledPluginsDir === undefined) {
    ...
    process.env.DAISYCLAW_BUNDLED_PLUGINS_DIR = originalBundledPluginsDir;
  }
...
describe("plugin-sdk facade runtime", () => {
  it("honors trusted bundled plugin dir overrides", () => {
    const overrideA = createBundledPluginDir("daisyclaw-facade-runtime-a-", "override-a");
    const overrideB = createBundledPluginDir("daisyclaw-facade-runtime-b-", "override-b");
  ...
  it("falls back to package source surfaces when an override dir is partial", () => {
    const overrideDir = createTrustedBundledFixtureRoot("daisyclaw-facade-runtime-empty-");
  ...
  it("returns the same object identity on repeated calls (sentinel consistency)", () => {
    const dir = createBundledPluginDir("daisyclaw-facade-identity-", "identity-check");
  ...
  it("breaks circular facade re-entry during module evaluation", () => {
    const dir = createBundledPluginDir("daisyclaw-facade-circular-", "circular-ok");
    const location = {
      modulePath: path.join(dir, "demo", "api.js"),
      boundaryRoot: dir,
    };
  ...
  it("back-fills the sentinel before post-load facade tracking re-enters", () => {
    const dir = createBundledPluginDir("daisyclaw-facade-post-load-", "post-load-ok");
    const location = {
      modulePath: path.join(dir, "demo", "api.js"),
      boundaryRoot: dir,
    };
  ...
  it("clears the cache on load failure so retries re-execute", () => {
    const dir = createThrowingPluginDir("daisyclaw-facade-throw-");
    useBundledPluginDirOverrideForTest(dir);
  ...
  it("allows runtime-api facade loads when the bundled plugin is explicitly enabled", () => {
    const dir = createTempDirSync("daisyclaw-facade-runtime-enabled-");
  ...
  it("resolves a globally-installed plugin whose rootDir basename matches the dirName", () => {
    const lineDir = createTempDirSync("daisyclaw-facade-global-line-");
    ...
    fs.writeFileSync(
      path.join(lineDir, "daisyclaw.plugin.json"),
      JSON.stringify({
        id: "line",
        channels: ["line"],
  ...
  it("resolves a globally-installed plugin public surface from package dist", () => {
    const lineDir = createTempDirSync("daisyclaw-facade-global-line-dist-");
  ...
  it("resolves a globally-installed plugin with an encoded scoped rootDir basename", () => {
    const encodedDir = createTempDirSync("daisyclaw-facade-encoded-line-");
    ...
    fs.writeFileSync(
      path.join(encodedDir, "daisyclaw.plugin.json"),
      JSON.stringify({
        id: "line",
        channels: ["line"],
        configSchema: { type: "object", additionalProperties: false, properties: {} },
  ...
  it("prefers the source runtime snapshot for facade activation checks", () => {
    const dir = createTempDirSync("daisyclaw-facade-source-snapshot-");
    ...
    fs.writeFileSync(
      path.join(dir, "demo", "daisyclaw.plugin.json"),
      JSON.stringify({
        id: "demo",
      }),
      "utf8",
  ...
  it("validates current snapshot against facade boundary config and ignores on mismatch", () => {
    const dir = createTempDirSync("daisyclaw-facade-snapshot-validate-");
    fs.mkdirSync(path.join(dir, "demo"), { recursive: true });
    fs.writeFileSync(
      path.join(dir, "demo", "runtime-api.js"),
    ...
    // Do NOT write daisyclaw.plugin.json on disk to force fallback to registry scan
    useBundledPluginDirOverrideForTest(dir);
      ...
      plugins: [
        {
          ...
          manifestPath: path.join(dir, "demo", "daisyclaw.plugin.json"),
          channels: ["demo"],
          providers: [],
          cliBackends: [],
          skills: [],
