...
const DAISYCLAW_SOURCE_EXTENSIONS_ROOT = path.resolve(DAISYCLAW_PACKAGE_ROOT, "extensions");
...
function resolveFacadeModuleLocationUncached(params: {
  ...
  if (!areBundledPluginsDisabled(env)) {
    ...
    const bundledLocation = resolveBundledFacadeModuleLocation({
      ...
      packageRoot: DAISYCLAW_PACKAGE_ROOT,
      bundledPluginsDir,
    });
    if (bundledLocation) {
...
function buildFacadeActivationCheckParams(
  ...
  return {
    ...params,
    location,
    sourceExtensionsRoot: DAISYCLAW_SOURCE_EXTENSIONS_ROOT,
    resolutionKey: createFacadeResolutionKey(params),
  };
}
