...
const ORIGINAL_ENV = {
  ...
  DAISYCLAW_CONFIG_PATH: process.env.DAISYCLAW_CONFIG_PATH,
  DAISYCLAW_STATE_DIR: process.env.DAISYCLAW_STATE_DIR,
  DAISYCLAW_TEST_FAST: process.env.DAISYCLAW_TEST_FAST,
} as const;
...
describe("plugin-sdk qa-runner-runtime linked plugin smoke", () => {
  ...
  it("loads an activated qa runner from a linked plugin path without a bundled install fallback", async () => {
    const stateDir = makeTempDir("daisyclaw-qa-runner-state-");
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@daisyclaw/qa-linked",
        type: "module",
        daisyclaw: {
          extensions: ["./index.js"],
          install: {
            npmSpec: "@daisyclaw/qa-linked",
          },
        },
      }),
      "utf8",
