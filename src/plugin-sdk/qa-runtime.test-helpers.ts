...
export async function expectPrivateQaLabRuntimeSurfaceLoad(params: {
  ...
  importRuntime: () => Promise<QaRuntimeModule>;
  loadBundledPluginPublicSurfaceModuleSync: SurfaceLoaderMock;
  resolveDaisyClawPackageRootSync: SurfaceLoaderMock;
}) {
  const sourceRoot = makePrivateQaSourceRoot(params.tempDirs, "daisyclaw-qa-runtime-root-");
  params.resolveDaisyClawPackageRootSync.mockReturnValue(sourceRoot);
  ...
  expect(params.loadBundledPluginPublicSurfaceModuleSync).toHaveBeenCalledWith({
    ...
    env: expect.objectContaining({
      ...
      DAISYCLAW_BUNDLED_PLUGINS_DIR: path.join(sourceRoot, "extensions"),
    }),
  });
}
