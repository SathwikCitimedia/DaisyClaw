...
const pluginSdkPackageNames = ["daisyclaw/plugin-sdk", "@daisyclaw/plugin-sdk"];
...
const DIAGNOSTIC_EVENTS_STATE_KEY = Symbol.for("daisyclaw.diagnosticEvents.state.v1");
...
const shouldPreferSourceGraph =
  ...
  (process.env.NODE_ENV !== "production" ||
    ...
    process.env.DAISYCLAW_PLUGIN_SDK_SOURCE_IN_TESTS === "1");
...
function listPluginSdkExportedSubpaths() {
  ...
  const cacheKey = `${packageRoot}::privateQa=${process.env.DAISYCLAW_ENABLE_PRIVATE_QA_CLI === "1" ? "1" : "0"}`;
...
function listPrivateLocalOnlyPluginSdkSubpaths() {
  if (process.env.DAISYCLAW_ENABLE_PRIVATE_QA_CLI !== "1") {
    return [];
  }
  try {
...
function buildPluginSdkAliasMap(useDist) {
  ...
  // Agent-core intentionally imports @daisyclaw/llm-core by package name so built
...
function getModuleLoader(tryNative) {
  ...
  const moduleLoader = createJiti(__filename, {
    ...
    // so local plugins do not create a second transpiled DaisyClaw core graph.
    tryNative,
