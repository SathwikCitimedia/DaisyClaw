// Temp home test helpers create isolated DaisyClaw home directories for plugin tests.
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked DAISYCLAW_HOME.
  delete process.env.DAISYCLAW_HOME;
  process.env.DAISYCLAW_STATE_DIR = path.join(base, ".daisyclaw");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "daisyclaw-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".daisyclaw", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
