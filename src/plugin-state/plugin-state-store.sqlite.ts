// Plugin state SQLite helpers persist plugin state in the DaisyClaw state database.
...
import { resolveDaisyClawStateSqlitePath } from "../state/daisyclaw-state-db.paths.js";
import {
...
type PluginStateEntriesTable = DaisyClawStateKyselyDatabase["plugin_state_entries"];
type PluginStateStoreDatabase = Pick<DaisyClawStateKyselyDatabase, "plugin_state_entries">;
...
function wrapPluginStateError(
  ...
  message: string,
  pathname = resolveDaisyClawStateSqlitePath(process.env),
): PluginStateStoreError {
...
function parseStoredJson(raw: string, operation: PluginStateStoreOperation): unknown {
  try {
    ...
    throw createPluginStateError({
      ...
      path: resolveDaisyClawStateSqlitePath(process.env),
      cause: error,
    });
  }
}
...
function openPluginStateDatabase(
  ...
  const pathname = resolveDaisyClawStateSqlitePath(env);
...
function envOptions(env?: NodeJS.ProcessEnv): DaisyClawStateDatabaseOptions {
  return env ? { env } : {};
}
...
function runWriteTransaction<T>(
  ...
  return runDaisyClawStateWriteTransaction(() => {
...
export function probePluginStateStore(): PluginStateStoreProbeResult {
  const databasePath = resolveDaisyClawStateSqlitePath(process.env);
  const steps: PluginStateStoreProbeStep[] = [];
  const wasOpen = cachedDatabase !== null;
  const stateWasOpen = isDaisyClawStateDatabaseOpen();
  ...
  try {
    ...
    openDaisyClawStateDatabase().walMaintenance.checkpoint();
    pushOk("checkpoint");
  } catch (error) {
