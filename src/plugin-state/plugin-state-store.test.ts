...
beforeAll(async () => {
  testState = await createDaisyClawTestState({ label: "plugin-state-store" });
  rmSync(path.dirname(resolveDaisyClawStateSqlitePath()), { recursive: true, force: true });
});
...
describe("plugin state keyed store", () => {
      ...
      async (stateA) => {
        await withDaisyClawTestState(
          ...
          async (stateB) => {
            ...
            expect(resolveDaisyClawStateSqlitePath(stateA.env)).not.toBe(
              resolveDaisyClawStateSqlitePath(stateB.env),
            );
          },
        );
      },
  ...
  it("reopens after the shared state DB cache closes its handle", async () => {
    await withPluginStateTestState(async () => {
      ...
      const secondary = await createDaisyClawTestState({
        label: "plugin-state-cache-secondary",
        applyEnv: false,
      });
      try {
        openDaisyClawStateDatabase({ env: secondary.env });
        testState?.applyEnv();
        await expect(store.lookup("k")).resolves.toEqual({ ok: true });
      } finally {
  ...
  it.runIf(process.platform !== "win32")("hardens DB directory and file permissions", async () => {
    await withPluginStateTestState(async () => {
      ...
      const databasePath = resolveDaisyClawStateSqlitePath();
