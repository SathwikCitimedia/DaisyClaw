...
function resolveBundlePluginRoot(homeDir: string, pluginId: string) {
  return path.join(homeDir, ".daisyclaw", "extensions", pluginId);
}
...
describe("loadEnabledClaudeBundleCommands", () => {
  it("loads enabled Claude bundle markdown commands and skips disabled-model-invocation entries", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "DAISYCLAW_HOME", "DAISYCLAW_STATE_DIR"]);
    try {
      const homeDir = await createTempDir("daisyclaw-bundle-commands-home-");
      const workspaceDir = await createTempDir("daisyclaw-bundle-commands-workspace-");
