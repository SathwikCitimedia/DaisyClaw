  ...
  return listExtensionPackagePaths(extensionsDir)
    .filter((packagePath) => {
      const packageJson = JSON.parse(fs.readFileSync(packagePath, "utf-8")) as {
        daisyclaw?: DaisyClawPackageManifest;
      };
      return normalizeBundledPluginStringList(packageJson.daisyclaw?.extensions).length > 0;
    })
    .map(
      (packagePath) =>
        JSON.parse(
          fs.readFileSync(path.join(path.dirname(packagePath), "daisyclaw.plugin.json"), "utf-8"),
        ) as PluginManifest,
    )
