...
import { discoverDaisyClawPlugins, type PluginDiscoveryResult } from "./discovery.js";
...
import type { DaisyClawPluginDefinition, DaisyClawPluginModule } from "./types.js";
...
export function buildVitestCapabilityShimAliasMap(): Record<string, string> {
  return Object.fromEntries(
    CAPABILITY_VITEST_SHIM_ALIASES.flatMap(({ subpath, target }) => {
      ...
      return [
        [`daisyclaw/plugin-sdk/${subpath}`, targetPath],
        [`@daisyclaw/plugin-sdk/${subpath}`, targetPath],
      ];
    }),
  );
...
function applyVitestCapabilityAliasOverrides(params: {
  ...
  const {
    "daisyclaw/plugin-sdk": _ignoredLegacyRootAlias,
    "@daisyclaw/plugin-sdk": _ignoredScopedRootAlias,
    ...scopedAliasMap
  } = params.aliasMap;
  return {
    ...scopedAliasMap,
...
function resolvePluginModuleExport(moduleExport: unknown): {
  ...
  if (resolved && typeof resolved === "object") {
    const definition = resolved as DaisyClawPluginDefinition;
    return {
      definition,
...
export function loadBundledCapabilityRuntimeRegistry(params: {
  ...
  const discovery = params.discovery ?? discoverDaisyClawPlugins({ env });
  ...
  for (const candidate of discovery.candidates) {
    ...
    try {
      mod = getModuleLoader(safeSource)(safeSource) as DaisyClawPluginModule;
    } catch (error) {
