...
const DISABLED_BUNDLED_PLUGINS_DIR = path.join(os.tmpdir(), "daisyclaw-empty-bundled-plugins");
const TEST_TRUST_BUNDLED_PLUGINS_DIR_ENV = "DAISYCLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR";
...
export function areBundledPluginsDisabled(env: NodeJS.ProcessEnv = process.env): boolean {
  const raw = normalizeOptionalLowercaseString(env.DAISYCLAW_DISABLE_BUNDLED_PLUGINS);
  ...
  try {
    return fs.readdirSync(pluginsDir, { withFileTypes: true }).some((entry) => {
      ...
      return (
        ...
        fs.existsSync(path.join(pluginDir, "daisyclaw.plugin.json"))
      );
    });
  } catch {
    return false;
...
function resolvePackageRootsForBundledPlugins(): string[] {
  const argvRoot = resolveDaisyClawPackageRootSync({ argv1: process.argv[1] });
  const moduleRoot = resolveDaisyClawPackageRootSync({ moduleUrl: import.meta.url });
...
function resolveTrustedExistingOverride(resolvedOverride: string): string | null {
  ...
  const modulePackageRoot = resolveDaisyClawPackageRootSync({ moduleUrl: import.meta.url });
...
function createBundledPluginsDirCacheKey(env: NodeJS.ProcessEnv): string {
  return JSON.stringify({
    disabled: env.DAISYCLAW_DISABLE_BUNDLED_PLUGINS ?? "",
    override: env.DAISYCLAW_BUNDLED_PLUGINS_DIR ?? "",
    ...
    daisyClawHome: env.DAISYCLAW_HOME ?? "",
    home: env.HOME ?? "",
...
function resolveBundledPluginsDirUncached(env: NodeJS.ProcessEnv): string | undefined {
  ...
  const override = env.DAISYCLAW_BUNDLED_PLUGINS_DIR?.trim();
