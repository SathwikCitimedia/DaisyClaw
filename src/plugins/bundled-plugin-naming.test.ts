...
function listExternalBundledPluginDirs(): string[] | null {
  ...
  for (const file of files) {
    const match = /^extensions\/([^/]+)\/(daisyclaw\.plugin\.json|package\.json)$/u.exec(file);
    if (!match) {
      continue;
    }
  ...
  return [...metadataByDir.entries()]
    .filter(
      ([, metadataFiles]) =>
        metadataFiles.has("package.json") && metadataFiles.has("daisyclaw.plugin.json"),
    )
    .map(([dirName]) => dirName)
    .toSorted();
}
...
function listGitPluginMetadataFiles(): string[] | null {
  return listGitTrackedFiles({
    pathspecs: ["extensions/*/package.json", "extensions/*/daisyclaw.plugin.json"],
  });
}
...
function readBundledPluginRecords(): BundledPluginRecord[] {
  return listBundledPluginDirs().flatMap((dirName) => {
    ...
    const manifestPath = path.join(rootDir, "daisyclaw.plugin.json");
    if (!fs.existsSync(packagePath) || !fs.existsSync(manifestPath)) {
      return [];
    }

    const manifest = readJsonFile(manifestPath) as PluginManifestShape;
    const pkg = readJsonFile(packagePath) as DaisyClawPackageShape;
    ...
    return [
      {
        ...
        packageName,
        manifestId,
        installNpmSpec: normalizeText(pkg.daisyclaw?.install?.npmSpec),
        channelId: normalizeText(pkg.daisyclaw?.channel?.id),
      },
    ];
  });
}
...
function resolveAllowedPackageNamesForId(pluginId: string): string[] {
  return ALLOWED_PACKAGE_SUFFIXES.map((suffix) => `@daisyclaw/${pluginId}${suffix}`);
}
  ...
  it.each([
    {
      ...
      message: `Bundled extension package names must stay anchored to the manifest id via @daisyclaw/<id> or an approved suffix (${ALLOWED_PACKAGE_SUFFIXES.join(", ")}). Update the plugin naming docs and this invariant before adding a new naming form.`,
    ...
    },
    {
      ...
      message:
        "Bundled extension directory names should match daisyclaw.plugin.json:id. If a legacy exception is unavoidable, add it to DIR_ID_EXCEPTIONS with a comment.",
      collectMismatches: (records: BundledPluginRecord[]) =>
        records
          .filter(
    ...
    {
      name: "keeps bundled daisyclaw.install.npmSpec aligned with the package name",
      message:
        "Bundled daisyclaw.install.npmSpec values must match the package name so install/update paths stay deterministic.",
      collectMismatches: (records: BundledPluginRecord[]) =>
        records
          .filter(
    ...
    },
    {
      ...
      message:
        "Non-packaged bundled plugins are source-only/private and must not advertise daisyclaw.install.npmSpec.",
      collectMismatches: (records: BundledPluginRecord[]) =>
        records
          .filter(
    ...
    },
    {
      ...
      message:
        "Bundled daisyclaw.channel.id values must match daisyclaw.plugin.json:id for the owning plugin.",
      collectMismatches: (records: BundledPluginRecord[]) =>
        records
          .filter(
