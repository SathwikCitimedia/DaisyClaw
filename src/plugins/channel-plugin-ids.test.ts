...
function withManifestLoadPaths<T extends { id: string }>(
  ...
  return {
    ...
    manifestPath: `/tmp/plugins/${plugin.id}/daisyclaw.plugin.json`,
    skills: [],
    hooks: [],
    ...plugin,
  };
...
describe("resolveGatewayStartupPluginIds", () => {
  beforeEach(() => {
    listPotentialConfiguredChannelIds.mockReset().mockImplementation((config: DaisyClawConfig) => {
    ...
    listPotentialConfiguredChannelPresenceSignals
      .mockReset()
      .mockImplementation((config: DaisyClawConfig) => {
        return listPotentialConfiguredChannelIds(config).map((channelId: string) => ({
          channelId,
          source: "config",
        }));
      });
    hasPotentialConfiguredChannels.mockReset().mockImplementation((config: DaisyClawConfig) => {
      if (Object.hasOwn(config, "channels")) {
  ...
  it("does not treat persisted auth alone as gateway startup intent", () => {
    ...
    expectStartupPluginIdsCase({
      ...
      env: createPluginPlanningTestEnv({
        DAISYCLAW_STATE_DIR: "/tmp/daisyclaw-with-persisted-demo-channel",
      }),
      expected: ["browser", "memory-core"],
    });
  });
  ...
  it("does not include Codex when an OpenAI model is manually pinned to DaisyClaw", () => {
    expectStartupPluginIdsCase({
      config: {
        agents: {
          defaults: {
  ...
  it("ignores env runtime overrides during startup planning", () => {
    expectStartupPluginIdsCase({
      ...
      env: { DAISYCLAW_AGENT_RUNTIME: "codex" },
      expected: ["demo-channel", "browser", "memory-core"],
    });
  });
