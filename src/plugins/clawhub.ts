...
function isTrustedSourceLinkedOfficialPackage(pkg: NonNullable<ClawHubPackageDetail["package"]>) {
  ...
  return (
    ...
    (sourceRepo === "daisyclaw/daisyclaw" ||
      sourceRepo === "github.com/daisyclaw/daisyclaw" ||
      sourceRepo === "https://github.com/daisyclaw/daisyclaw")
  );
}
...
function validateClawHubPluginPackage(params: {
  ...
  if (pkg.family === "skill") {
    return buildClawHubInstallFailure(
      `"${pkg.name}" is a skill. Use "daisyclaw skills install ${pkg.name}" instead.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires plugin API ${compatibility.pluginApiRange}, but this DaisyClaw runtime exposes ${runtimeVersion}.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires DaisyClaw >=${compatibility.minGatewayVersion}, but this host is ${runtimeVersion}.`,
...
> {
  ...
  try {
    ...
    const clawhubAuthority = isDefaultClawHubBaseUrl(params.baseUrl) ? "daisyclaw" : "third-party";
    params.logger?.info?.(
