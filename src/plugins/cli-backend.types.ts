...
export type PluginTextTransforms = {
  ...
  /** Rewrites applied to inbound assistant text before DaisyClaw consumes it. */
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether DaisyClaw should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume DaisyClaw's bundle
   ...
   *
   * Use this when the backend should consume a canonical DaisyClaw auth profile
   ...
   * `profile-only` treats the selected DaisyClaw auth profile as the sole auth
   ...
   * as static config, such as mapping DaisyClaw thinking levels to a backend's
   * native effort flag.
   */
  resolveExecutionArgs?: CliBackendResolveExecutionArgs;
  /**
   * Whether this CLI backend can expose native tools outside DaisyClaw's tool
