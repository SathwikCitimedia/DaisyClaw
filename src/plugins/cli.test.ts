...
const mocks = vi.hoisted(() => ({
  ...
  loadDaisyClawPluginCliRegistry: vi.fn(),
  loadDaisyClawPlugins: vi.fn(),
...
vi.mock("./loader.js", () => ({
  loadDaisyClawPluginCliRegistry: (...args: unknown[]) =>
    mocks.loadDaisyClawPluginCliRegistry(...args),
  loadDaisyClawPlugins: (...args: unknown[]) => mocks.loadDaisyClawPlugins(...args),
}));
...
function expectAutoEnabledCliLoad(params: {
  rawConfig: DaisyClawConfig;
  autoEnabledConfig: DaisyClawConfig;
  ...
  const loadOptions = getMockCallObject(mocks.loadDaisyClawPlugins);
...
describe("registerPluginCliCommands", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadDaisyClawPluginCliRegistry.mockReset();
    mocks.loadDaisyClawPluginCliRegistry.mockResolvedValue(createCliRegistry());
    mocks.loadDaisyClawPlugins.mockReset();
    mocks.loadDaisyClawPlugins.mockReturnValue({
      ...createCliRegistry(),
      diagnostics: [],
    });
    ...
    mocks.loadConfig.mockReturnValue({} as DaisyClawConfig);
  ...
  it("skips plugin CLI registrars when commands already exist", async () => {
    ...
    await registerPluginCliCommands(program, {} as DaisyClawConfig);
  ...
  it("forwards an explicit env to plugin loading", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as DaisyClawConfig, env);
  ...
  it("injects gateway-backed node runtime into plugin CLI commands", async () => {
    await registerPluginCliCommands(createProgram(), {} as DaisyClawConfig);

    const loadOptions = getMockCallObject(mocks.loadDaisyClawPlugins) as {
  ...
  it("reuses loaded plugin CLI entries on repeat calls for the same program", async () => {
    ...
    expect(mocks.loadDaisyClawPlugins).toHaveBeenCalledTimes(1);
  });

  it("reloads plugin CLI entries when the requested primary command changes", async () => {
    const program = createProgram();

    await registerPluginCliCommands(program, {} as DaisyClawConfig, undefined, undefined, {
      primary: "memory",
    });
    await registerPluginCliCommands(program, {} as DaisyClawConfig);

    expect(mocks.loadDaisyClawPlugins).toHaveBeenCalledTimes(2);
  });

  it("reloads plugin CLI entries when config or environment identity changes", async () => {
    const program = createProgram();
    const configA = {} as DaisyClawConfig;
    const configB = { plugins: {} } as DaisyClawConfig;
    const envA = { DAISYCLAW_HOME: "/tmp/a" } as NodeJS.ProcessEnv;
    const envB = { DAISYCLAW_HOME: "/tmp/b" } as NodeJS.ProcessEnv;
    ...
    expect(mocks.loadDaisyClawPlugins).toHaveBeenCalledTimes(3);
  });
  ...
  it("loads root-help descriptors through the dedicated non-activating CLI collector", async () => {
    ...
    mocks.loadDaisyClawPluginCliRegistry.mockResolvedValue({
      cliRegistrars: [
        {
          pluginId: "matrix",
          register: vi.fn(),
    ...
    const registryOptions = getMockCallObject(mocks.loadDaisyClawPluginCliRegistry);
  ...
  it("keeps root-help descriptor load failures quiet", async () => {
    ...
    mocks.loadDaisyClawPluginCliRegistry.mockImplementationOnce((options: { logger?: unknown }) => {
    ...
    await expect(
      getPluginCliCommandDescriptors({ plugins: { entries: { stale: {} } } } as DaisyClawConfig),
    ).resolves.toEqual([]);
  ...
  it("keeps runtime CLI command registration on the full plugin loader for legacy channel plugins", async () => {
    ...
    expect(mocks.loadDaisyClawPluginCliRegistry).not.toHaveBeenCalled();
  });
  ...
  it("falls back to eager registration when descriptors do not cover every command root", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as DaisyClawConfig, undefined, undefined, {
      mode: "lazy",
    });
  ...
  it("scopes full CLI loading through CLI metadata when manifest planning finds no plugin match", async () => {
    ...
    expect(mocks.loadDaisyClawPluginCliRegistry).toHaveBeenCalled();
    const loadOptions = getMockCallObject(mocks.loadDaisyClawPlugins);
    expect(loadOptions.onlyPluginIds).toEqual(["memory-core"]);
  });
  ...
  it("scopes nested CLI loading through CLI metadata parent paths", async () => {
    ...
    mocks.loadDaisyClawPluginCliRegistry.mockResolvedValue(nestedRegistry);
    mocks.loadDaisyClawPlugins.mockReturnValue(nestedRegistry);
    const program = createProgram("nodes");
    program.exitOverride();
  ...
  it("skips full plugin runtime loading when no metadata owns the requested primary", async () => {
    ...
    expect(mocks.loadDaisyClawPluginCliRegistry).toHaveBeenCalled();
    expect(mocks.loadDaisyClawPlugins).not.toHaveBeenCalled();
  ...
  it("loads validated plugin CLI config when the snapshot is valid", async () => {
    const loadedConfig = { plugins: { enabled: true } } as DaisyClawConfig;
    mocks.readConfigFileSnapshot.mockResolvedValueOnce({
      valid: true,
      config: loadedConfig,
    });
