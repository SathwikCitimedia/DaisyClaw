...
describe("registerPluginCommand", () => {
  it.each([
    ...
    },
    {
      ...
      expected: {
        ok: false,
        error:
          "Agent prompt guidance 1 surface 1 must be one of: daisyclaw_main, pi_main, codex_app_server, cli_backend, acp_backend, subagent",
      },
    },
    {
  ...
  it("normalizes and filters structured agent prompt guidance by surface", () => {
    ...
    expect(listRegisteredPluginAgentPromptGuidance({ surface: "daisyclaw_main" })).toEqual([
      "Use /demo_cmd everywhere.",
      "Use /demo_cmd for main agent routing.",
    ]);
  ...
  it("binds plugin-owned command sessions to the host-resolved agent", async () => {
    ...
    await executePluginCommand({
      ...
      isAuthorizedSender: true,
      agentId: "codex",
      sessionKey: "plugin-binding:daisyclaw-codex-app-server:dm",
