...
function makeRepoFixture(): string {
  const repoRoot = mkdtempSync(join(tmpdir(), "daisyclaw-config-boundary-"));
...
describe("config boundary guard", () => {
  ...
  it("flags broad config-runtime barrel imports in production code", () => {
    ...
    writeFixture(
      ...
      "extensions/telegram/src/index.ts",
      [
        'import type { DaisyClawConfig } from "daisyclaw/plugin-sdk/config-runtime";',
        'import { requireRuntimeConfig } from "daisyclaw/plugin-sdk/config-runtime";',
        'type Loader = typeof import("daisyclaw/plugin-sdk/config-runtime").getRuntimeConfig;',
        "export type Config = DaisyClawConfig;",
        "export const load: Loader = requireRuntimeConfig;",
      ].join("\n"),
    );

    expect(collectDeprecatedInternalConfigApiViolations({ repoRoot })).toEqual([
      "extensions/telegram/src/index.ts:1 use narrow plugin-sdk config subpaths instead of daisyclaw/plugin-sdk/config-runtime",
      "extensions/telegram/src/index.ts:2 use narrow plugin-sdk config subpaths instead of daisyclaw/plugin-sdk/config-runtime",
      "extensions/telegram/src/index.ts:3 use narrow plugin-sdk config subpaths instead of daisyclaw/plugin-sdk/config-runtime",
    ]);
  });
  ...
  it("flags broad config-runtime test mocks outside compat guard fixtures", () => {
    ...
    writeFixture(
      ...
      'vi.mock("daisyclaw/plugin-sdk/config-runtime", () => ({}));',
    );

    expect(collectDeprecatedInternalConfigApiViolations({ repoRoot })).toEqual([
      "extensions/telegram/src/index.test.ts:1 use narrow plugin-sdk config subpaths instead of daisyclaw/plugin-sdk/config-runtime",
    ]);
  });
  ...
  it("allows narrow config SDK subpaths in production code", () => {
    ...
    writeFixture(
      ...
      [
        ...
        'import { requireRuntimeConfig } from "daisyclaw/plugin-sdk/plugin-config-runtime";',
        'type Loader = typeof import("daisyclaw/plugin-sdk/runtime-config-snapshot").getRuntimeConfig;',
        'export const load = (cfg: DaisyClawConfig) => requireRuntimeConfig(cfg, "telegram");',
      ].join("\n"),
    );
