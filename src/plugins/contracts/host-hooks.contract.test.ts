...
import {
  ...
  registerTestPlugin,
} from "daisyclaw/plugin-sdk/plugin-test-contracts";
...
async function withHostHookState(
  ...
  const stateDir = await fs.mkdtemp(path.join(resolvePreferredDaisyClawTmpDir(), prefix));
...
describe("host-hook fixture plugin contract", () => {
  ...
  it("allows the official npm Codex plugin to keep /codex command ownership", () => {
    const { config, registry } = createPluginRegistryFixture();
    const codexRoot = path.join("/tmp", ".daisyclaw", "npm", "node_modules", "@daisyclaw", "codex");
    registerTestPlugin({
      registry,
      config,
      record: createPluginRecord({
  ...
  it("allows the official ClawHub Codex plugin to keep /codex command ownership", () => {
    const { config, registry } = createPluginRegistryFixture();
    const codexRoot = path.join("/tmp", ".daisyclaw", "extensions", "codex");
    registerTestPlugin({
      registry,
      config,
      record: createPluginRecord({
  ...
  it("requires explicit unset to remove plugin session extension state", async () => {
    ...
    await withHostHookState("daisyclaw-host-hooks-patch-", async ({ storePath, tempConfig }) => {
  ...
  it("reports duplicate next-turn injections as not newly enqueued", async () => {
    await withHostHookState("daisyclaw-host-hooks-injection-", async ({ storePath, tempConfig }) => {
      await updateSessionStore(storePath, (store) => {
        store["agent:main:main"] = {
          sessionId: "session-1",
          updatedAt: Date.now(),
  ...
  it("preserves global enqueue order when draining live next-turn injections", async () => {
    ...
    setActivePluginRegistry(registry);
    await withHostHookState("daisyclaw-host-hooks-order-", async ({ storePath, tempConfig }) => {
      await updateSessionStore(storePath, (store) => {
        store["agent:main:main"] = {
          sessionId: "session-1",
          updatedAt: Date.now(),
  ...
  it("cleans plugin-owned session state and lifecycle resources on reset/disable", async () => {
    ...
    await withHostHookState("daisyclaw-host-hooks-state-", async ({ tempConfig }) => {
  ...
  it("removes persistent plugin-owned session state and pending injections during cleanup", async () => {
    ...
    await withHostHookState("daisyclaw-host-hooks-store-", async ({ storePath, tempConfig }) => {
      await updateSessionStore(storePath, (store) => {
        store["agent:main:main"] = {
          sessionId: "session-1",
          updatedAt: Date.now(),
  ...
  it("does not clear unrelated run context during session-scoped cleanup", async () => {
    ...
    await withHostHookState("daisyclaw-host-hooks-run-context-", async ({ tempConfig }) => {
      await runPluginHostCleanup({
        cfg: tempConfig,
        registry,
        reason: "reset",
  ...
  it("preserves durable plugin session state during plugin restart cleanup", async () => {
    ...
    await withHostHookState(
      "daisyclaw-host-hooks-restart-state-",
      async ({ storePath, tempConfig }) => {
        await updateSessionStore(storePath, (store) => {
          store["agent:main:main"] = {
            sessionId: "session-1",
  ...
  it("cleans pending injections for plugins that registered no host-hook callbacks", async () => {
    ...
    await withHostHookState(
      "daisyclaw-host-hooks-injection-only-",
      async ({ storePath, tempConfig }) => {
        await updateSessionStore(storePath, (store) => {
          store["agent:main:main"] = {
            sessionId: "session-1",
