...
const PLUGIN_SDK_SUBPATH_PATTERN = /daisyclaw\/plugin-sdk\/([a-z0-9][a-z0-9-]*)\b/g;
...
const DEPRECATED_EXTENSION_SDK_SPECIFIERS = new Set([
  ...
  "daisyclaw/plugin-sdk/channel-config-schema-legacy",
  "daisyclaw/plugin-sdk/compat",
  "daisyclaw/plugin-sdk/testing",
  "daisyclaw/plugin-sdk/test-utils",
]);
...
const LEGACY_MEMORY_EMBEDDING_PROVIDER_MANIFEST_FILES = new Set([
  "extensions/amazon-bedrock/daisyclaw.plugin.json",
  "extensions/deepinfra/daisyclaw.plugin.json",
  "extensions/github-copilot/daisyclaw.plugin.json",
  "extensions/google/daisyclaw.plugin.json",
  "extensions/lmstudio/daisyclaw.plugin.json",
  "extensions/memory-core/daisyclaw.plugin.json",
  "extensions/mistral/daisyclaw.plugin.json",
  "extensions/ollama/daisyclaw.plugin.json",
  "extensions/openai/daisyclaw.plugin.json",
  "extensions/voyage/daisyclaw.plugin.json",
]);
...
function collectDeprecatedExtensionSdkImports(): Array<{ file: string; specifier: string }> {
  const leaks: Array<{ file: string; specifier: string }> = [];
  const importPatterns = [
    /\b(?:import|export)\b[\s\S]*?\bfrom\s*["'](daisyclaw\/plugin-sdk(?:\/[a-z0-9][a-z0-9-]*)?)["']/g,
    /\bimport\s*\(\s*["'](daisyclaw\/plugin-sdk(?:\/[a-z0-9][a-z0-9-]*)?)["']\s*\)/g,
    /\bvi\.(?:mock|doMock)\s*\(\s*["'](daisyclaw\/plugin-sdk(?:\/[a-z0-9][a-z0-9-]*)?)["']/g,
  ];
  for (const file of collectExtensionFiles(resolve(REPO_ROOT, "extensions"))) {
    const repoRelativePath = toRepoRelativePath(file);
    const source = fs.readFileSync(file, "utf8");
...
function collectNewDeprecatedMemoryEmbeddingProviderManifestFiles(): string[] {
  ...
  const manifestFiles =
    listGitTrackedFiles({
      repoRoot: REPO_ROOT,
      pathspecs: "extensions/**/daisyclaw.plugin.json",
    }) ?? [];
...
function collectDeprecatedTestBarrelImports(): string[] {
  const leaks: Array<{ file: string; specifier: string }> = [];
  const importPatterns = [
    /\b(?:import|export)\b[\s\S]*?\bfrom\s*["'](daisyclaw\/plugin-sdk\/(?:testing|test-utils))["']/g,
    /\bimport\s*\(\s*["'](daisyclaw\/plugin-sdk\/(?:testing|test-utils))["']\s*\)/g,
    /\bvi\.(?:mock|doMock)\s*\(\s*["'](daisyclaw\/plugin-sdk\/(?:testing|test-utils))["']/g,
  ];
...
function collectCrossOwnerReservedSdkImports(): Array<{
  ...
  for (const file of collectExtensionFiles(resolve(REPO_ROOT, "extensions"))) {
    ...
    for (const match of source.matchAll(importPattern)) {
      ...
      leaks.push({
        file: repoRelativePath,
        specifier: `daisyclaw/plugin-sdk/${subpath}`,
        owner,
      });
    }
  }
    ...
    for (const importPattern of importPatterns) {
      for (const match of source.matchAll(importPattern)) {
        ...
        if (subpath && subpaths.has(subpath)) {
          imports.add(`${repoRelativePath}: daisyclaw/plugin-sdk/${subpath}`);
        }
      }
    }
  }
...
describe("plugin-sdk package contract guardrails", () => {
  ...
  it("keeps curated public plugin-sdk references on exported built subpaths", () => {
    ...
    for (const reference of collectPluginSdkSubpathReferences()) {
      ...
      failures.push(
        `${reference.file} references daisyclaw/plugin-sdk/${reference.subpath}, but ${reference.subpath} is missing from ${missingFrom.join(" and ")}`,
      );
    }
  ...
  it("keeps Matrix dependencies local to the Matrix plugin", () => {
    ...
    expect(rootRuntimeDeps.has("@daisyclaw/plugin-package-contract")).toBe(false);
  });
