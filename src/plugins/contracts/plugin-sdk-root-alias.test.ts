...
const diagnosticEventsStateKey = Symbol.for("daisyclaw.diagnosticEvents.state.v1");
...
function loadRootAliasWithStubs(options?: {
  ...
  const localRequire = ((id: string) => {
    ...
    if (id === "node:os") {
      return {
        tmpdir: () =>
          context.process.env.TMPDIR ?? options?.defaultTmpDir ?? "/tmp/daisyclaw-root-alias-test",
      };
    }
    if (id === "jiti") {
      return {
...
function ensureDiagnosticEventsStateFixture(
  ...
  const state = vm.runInNewContext(
    `({
      marker: Symbol.for("daisyclaw.diagnosticEvents.state.v1"),
      enabled: true,
      seq: 0,
      listeners: new Set(),
      dispatchDepth: 0,
...
describe("plugin-sdk root alias", () => {
  ...
  it("loads legacy root exports on demand and preserves reflection", () => {
    ...
    expect(lazyModule.createJitiOptions.at(-1)?.fsCache).toBe(
      path.join("/tmp/daisyclaw-root-alias-test", "jiti", "daisyclaw", "3.4.5", "12345-678"),
    );
  ...
  it("preserves jiti's tmpdir guard when root-alias TMPDIR resolves to cwd", () => {
    const lazyModule = loadRootAliasWithStubs({
      cwd: "/tmp/daisyclaw-root-alias-cwd",
      defaultTmpDir: "/tmp/daisyclaw-root-alias-fallback",
      env: { TMPDIR: "/tmp/daisyclaw-root-alias-cwd" },
      packageVersion: "3.4.5",
    });

    expect("slowHelper" in lazyModule.moduleExports).toBe(true);
    expect(lazyModule.createJitiOptions.at(-1)?.fsCache).toBe(
      path.join("/tmp/daisyclaw-root-alias-fallback", "jiti", "daisyclaw", "3.4.5", "12345-678"),
    );
  });
  ...
  it("builds scoped and unscoped plugin-sdk aliases for jiti loads", () => {
    ...
    expect(aliasMap["daisyclaw/plugin-sdk"]).toBe(rootAliasPath);
    expect(aliasMap["@daisyclaw/plugin-sdk"]).toBe(rootAliasPath);
    expect(aliasMap["daisyclaw/plugin-sdk/group-access"]).toContain(
  ...
  it("keeps agent-core package imports on the source graph when llm-core dist is missing", () => {
    ...
    expect((lazyModule.moduleExports.slowHelper as () => string)()).toBe("loaded");
    const aliasMap = (lazyModule.createJitiOptions.at(-1)?.alias ?? {}) as Record<string, string>;
    expect(aliasMap["@daisyclaw/llm-core"]).toBe(sourceLlmCorePath);
  });
  ...
  it("keeps bootstrap plugin-sdk aliases deterministic and ignores unsafe subpaths", () => {
    ...
    expect(aliasKeys).toEqual([
      "daisyclaw/plugin-sdk/alpha",
      "@daisyclaw/plugin-sdk/alpha",
      "daisyclaw/plugin-sdk/group-access",
      "@daisyclaw/plugin-sdk/group-access",
      "daisyclaw/plugin-sdk/zeta",
      "@daisyclaw/plugin-sdk/zeta",
      "@daisyclaw/llm-core",
      "@daisyclaw/llm-core/diagnostics",
      "@daisyclaw/llm-core/event-stream",
      "@daisyclaw/llm-core/types",
      "@daisyclaw/llm-core/validation",
      "daisyclaw/plugin-sdk",
      "@daisyclaw/plugin-sdk",
    ]);
  });
  ...
  it("ignores unsafe private local-only plugin-sdk subpaths in the CJS root alias", () => {
    ...
    const lazyModule = loadRootAliasWithStubs({
      env: { DAISYCLAW_ENABLE_PRIVATE_QA_CLI: "1" },
    ...
    expect((lazyModule.moduleExports.slowHelper as () => string)()).toBe("loaded");
    const aliasMap = (lazyModule.createJitiOptions.at(-1)?.alias ?? {}) as Record<string, string>;
    expect(aliasMap["daisyclaw/plugin-sdk/qa-lab"]).toBe(qaLabPath);
    expect(aliasMap["@daisyclaw/plugin-sdk/qa-lab"]).toBe(qaLabPath);
    expect(aliasMap).not.toHaveProperty("daisyclaw/plugin-sdk/../escape");
    expect(aliasMap).not.toHaveProperty("daisyclaw/plugin-sdk/nested/path");
    expect(aliasMap).not.toHaveProperty("daisyclaw/plugin-sdk/ssrf-runtime-internal");
    expect(aliasMap).not.toHaveProperty("@daisyclaw/plugin-sdk/ssrf-runtime-internal");
  });
  ...
  it("keeps non-QA private local-only plugin-sdk subpaths out of the CJS root alias", () => {
    ...
    expect((lazyModule.moduleExports.slowHelper as () => string)()).toBe("loaded");
    const aliasMap = (lazyModule.createJitiOptions.at(-1)?.alias ?? {}) as Record<string, string>;
    expect(aliasMap).not.toHaveProperty("daisyclaw/plugin-sdk/codex-mcp-projection");
    expect(aliasMap).not.toHaveProperty("@daisyclaw/plugin-sdk/codex-mcp-projection");
    expect(aliasMap).not.toHaveProperty("daisyclaw/plugin-sdk/qa-runtime");
  });
  ...
  it("builds source plugin-sdk subpath aliases through the wider source extension family", () => {
    ...
    expect((lazyModule.moduleExports.slowHelper as () => string)()).toBe("loaded");
    const aliasMap = (lazyModule.createJitiOptions.at(-1)?.alias ?? {}) as Record<string, string>;
    expect(aliasMap["daisyclaw/plugin-sdk/channel-runtime"]).toBe(
  ...
  it("bridges diagnostic listeners through shared process state when the lazy module is isolated", () => {
    ...
    const state = lazyModule.globalContext[Symbol.for("daisyclaw.diagnosticEvents.state.v1")] as {
      listeners: Set<(event: { type: string }, metadata: { trusted: boolean }) => void>;
    };
