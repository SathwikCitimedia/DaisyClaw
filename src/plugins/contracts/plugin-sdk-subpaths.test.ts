...
import * as commandAuthSdk from "daisyclaw/plugin-sdk/command-auth";
import type {
  ChannelMessageActionContext as CoreChannelMessageActionContext,
  DaisyClawPluginApi as CoreDaisyClawPluginApi,
  PluginRuntime as CorePluginRuntime,
} from "daisyclaw/plugin-sdk/core";
import * as providerEntrySdk from "daisyclaw/plugin-sdk/provider-entry";
import * as zalouserSdk from "daisyclaw/plugin-sdk/zalouser";
...
import type {
  ...
  DaisyClawPluginApi as SharedDaisyClawPluginApi,
...
import type { DaisyClawPluginApi } from "../types.js";
...
const BROWSER_HELPER_EXPORT_PARITY_CONTRACTS: readonly BrowserHelperExportParityContract[] = [
  ...
  },
  {
    ...
    expectedExports: [
      ...
      "DEFAULT_DAISYCLAW_BROWSER_ENABLED",
...
describe("plugin-sdk subpath exports", () => {
  ...
  beforeAll(() => {
    deprecatedChannelRuntimeMatches = findRepoFilesContaining({
      ...
      pattern:
        /(?:from\s+|import\s+(?:type\s+)?|import\s*\(\s*)["']daisyclaw\/plugin-sdk\/channel-runtime(?=["'])/u,
      exclude: [
  ...
  it("keeps shared plugin-sdk types aligned", () => {
    ...
    expectTypeOf<CoreDaisyClawPluginApi>().toMatchTypeOf<DaisyClawPluginApi>();
    expectTypeOf<CorePluginRuntime>().toMatchTypeOf<PluginRuntime>();
    expectTypeOf<CoreChannelMessageActionContext>().toMatchTypeOf<ChannelMessageActionContext>();
    expectTypeOf<CoreDaisyClawPluginApi>().toMatchTypeOf<SharedDaisyClawPluginApi>();
    expectTypeOf<CorePluginRuntime>().toMatchTypeOf<SharedPluginRuntime>();
    expectTypeOf<CoreChannelMessageActionContext>().toMatchTypeOf<SharedChannelMessageActionContext>();
  });

  it("keeps runtime entry subpaths importable", async () => {
    const coreSdk = await importResolvedPluginSdkSubpath("daisyclaw/plugin-sdk/core");
    const channelActionsSdk = await importResolvedPluginSdkSubpath(
      "daisyclaw/plugin-sdk/channel-actions",
    );
    const globalSingletonSdk = await importResolvedPluginSdkSubpath(
      "daisyclaw/plugin-sdk/global-singleton",
    );
    const pluginEntrySdk = await importResolvedPluginSdkSubpath("daisyclaw/plugin-sdk/plugin-entry");
    const channelLifecycleSdk = await importResolvedPluginSdkSubpath(
      "daisyclaw/plugin-sdk/channel-lifecycle",
    );
    const channelPairingSdk = await importResolvedPluginSdkSubpath(
      "daisyclaw/plugin-sdk/channel-pairing",
    );
    const channelReplyPipelineSdk = await importResolvedPluginSdkSubpath(
      "daisyclaw/plugin-sdk/channel-reply-pipeline",
    );
    const representativeModules = [];
    for (const id of representativeRuntimeSmokeSubpaths) {
      representativeModules.push(await importResolvedPluginSdkSubpath(`daisyclaw/plugin-sdk/${id}`));
    }
  ...
  it("keeps repeated silent-token semantics visible through the reply-chunking subpath", async () => {
    const replyChunkingSdk = await importResolvedPluginSdkSubpath(
      "daisyclaw/plugin-sdk/reply-chunking",
    );
