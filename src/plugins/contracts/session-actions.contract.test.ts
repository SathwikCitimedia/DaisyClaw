...
describe("plugin session actions", () => {
  ...
  it("emits plugin-attributed agent events through the plugin API", () => {
    ...
    const { config, registry } = createPluginRegistryFixture();
    let bundledApi: DaisyClawPluginApi | undefined;
    let workspaceApi: DaisyClawPluginApi | undefined;
    registerTestPlugin({
      registry,
      config,
      record: createPluginRecord({
  ...
  it("blocks agent events from stale and non-activating plugin API closures", () => {
    ...
    try {
      ...
      let neverActiveApi: DaisyClawPluginApi | undefined;
      registerTestPlugin({
      ...
      let inactiveApi: DaisyClawPluginApi | undefined;
      registerTestPlugin({
