...
import { FILE_TYPE_SNIFF_MAX_BYTES } from "@daisyclaw/media-core/mime";
import {
...
async function withSessionStore(
  ...
  const stateDir = await fs.mkdtemp(
    path.join(resolvePreferredDaisyClawTmpDir(), "daisyclaw-session-attachments-"),
  );
...
describe("plugin session attachments", () => {
  afterEach(() => {
    ...
    delete (globalThis as { proofAttachmentApi?: DaisyClawPluginApi }).proofAttachmentApi;
  ...
  it("wires sendSessionAttachment through the plugin API with stale-registry protection", async () => {
    await withSessionStore(async ({ storePath, filePath }) => {
      ...
      let capturedApi: DaisyClawPluginApi | undefined;
      registerTestPlugin({
        registry,
        config,
