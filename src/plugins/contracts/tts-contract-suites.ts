...
import type { ResolvedTtsConfig, SpeechProviderPlugin } from "daisyclaw/plugin-sdk/speech-core";
import { withEnv, withEnvAsync } from "daisyclaw/plugin-sdk/test-env";
...
type TtsRuntimeModule = typeof import("daisyclaw/plugin-sdk/tts-runtime");
type TtsCoreModule = typeof import("daisyclaw/plugin-sdk/speech-core");
...
const speechCoreRuntimeApiModuleId = resolveWorkspacePackagePublicModuleUrl({
  packageName: "@daisyclaw/speech-core",
...
let completeSimple: typeof import("daisyclaw/plugin-sdk/llm").completeSimple;
...
vi.mock("daisyclaw/plugin-sdk/llm", () => {
...
function asLegacyTtsConfig(value: unknown): DaisyClawConfig {
  return value as DaisyClawConfig;
}

function asLegacyDaisyClawConfig(value: Record<string, unknown>): DaisyClawConfig {
  return value as unknown as DaisyClawConfig;
}
...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): DaisyClawConfig {
...
async function loadTtsCore(): Promise<TtsCoreModule> {
  ttsCorePromise ??= import("daisyclaw/plugin-sdk/speech-core");
...
function createResolvedSummarizationConfig(cfg: DaisyClawConfig): ResolvedTtsConfig {
  const rawConfig =
...
async function setupSummarizationMocks() {
  ...
  ({ completeSimple } = await import("daisyclaw/plugin-sdk/llm"));
    ...
    describe("getTtsProvider", () => {
      ...
      it("passes cfg into auto-selection so model-provider Google keys can configure TTS", () => {
        withIsolatedSpeechProviderEnv(
          ...
          () => {
            const cfg = asLegacyDaisyClawConfig({
              agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
              models: {
                providers: {
                  google: {
...
export function describeTtsAutoApplyContract() {
  describe("tts auto-apply contract", () => {
    ...
    const baseCfg: DaisyClawConfig = asLegacyDaisyClawConfig({
      agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
      messages: {
        tts: {
          auto: "inbound",
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.DAISYCLAW_TTS_PREFS;
      process.env.DAISYCLAW_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.DAISYCLAW_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: DaisyClawConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
