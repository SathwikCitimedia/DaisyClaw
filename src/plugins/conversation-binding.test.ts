...
const tempRoot = makeTrackedTempDir("daisyclaw-plugin-binding", tempDirs);
...
vi.mock("../infra/home-dir.js", async () => {
  ...
  return {
    ...
    expandHomePrefix: (value: string) => {
      if (value === "~/.daisyclaw/plugin-binding-approvals.json") {
...
describe("plugin conversation binding approvals", () => {
  ...
  it("persists plugin-owned binding data on approved plugin bindings", async () => {
    const data = {
      ...
      version: 1,
      sessionFile: "/tmp/daisyclaw/session.jsonl",
  ...
  it.each([
    ...
    },
    {
      ...
      existingRecord: {
        ...
        targetSessionKey: "daisyclaw-app-server:thread:019ce411-6322-7db2-a821-1a61c530e7d9",
        targetKind: "session" as const,
        conversation: {
          channel: "telegram",
          accountId: "default",
