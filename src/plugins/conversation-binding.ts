...
const APPROVALS_PATH = "~/.daisyclaw/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "daisyclaw-app-server:thread:",
  "daisyclaw-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("daisyclaw.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("daisyclaw.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to DaisyClaw instead. If this started after an update, run "daisyclaw doctor --fix"; otherwise reinstall or enable the plugin.${buildDetachHintSuffix(binding.detachHint)}`;
}
