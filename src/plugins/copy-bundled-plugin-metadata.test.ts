...
const excludeOptionalEnv = { DAISYCLAW_INCLUDE_OPTIONAL_BUNDLED: "0" } as const;
...
function createPlugin(
  ...
  params: {
    ...
    packageDaisyClaw?: Record<string, unknown>;
  },
) {
  const pluginDir = path.join(repoRoot, "extensions", params.id);
  fs.mkdirSync(pluginDir, { recursive: true });
  writeJson(path.join(pluginDir, "daisyclaw.plugin.json"), {
    id: params.id,
  ...
  writeJson(path.join(pluginDir, "package.json"), {
    ...
    ...(params.packageDaisyClaw ? { daisyclaw: params.packageDaisyClaw } : {}),
  });
  return pluginDir;
}
...
function readBundledManifest(repoRoot: string, pluginId: string): Record<string, unknown> {
  return JSON.parse(
    fs.readFileSync(
      path.join(repoRoot, "dist", "extensions", pluginId, "daisyclaw.plugin.json"),
      "utf8",
    ),
...
function readBundledPackageJson(repoRoot: string, pluginId: string) {
  ...
  ) as { daisyclaw?: { extensions?: string[] } };
}
...
function createTlonSkillPlugin(repoRoot: string, skillPath = "node_modules/@tloncorp/tlon-skill") {
  return createPlugin(repoRoot, {
    id: "tlon",
    packageName: "@daisyclaw/tlon",
    manifest: { skills: [skillPath] },
    packageDaisyClaw: { extensions: ["./index.ts"] },
  });
}
...
describe("copyBundledPluginMetadata", () => {
  it("copies plugin manifests, package metadata, and local skill directories", () => {
    const repoRoot = makeRepoRoot("daisyclaw-bundled-plugin-meta-");
    const pluginDir = createPlugin(repoRoot, {
      id: "acpx",
      packageName: "@daisyclaw/acpx",
    ...
    expect(
      fs.existsSync(path.join(repoRoot, "dist", "extensions", "acpx", "daisyclaw.plugin.json")),
    ).toBe(true);
    expect(
      fs.readFileSync(
    ...
    expect(packageJson.daisyclaw?.extensions).toEqual(["./index.js"]);
  });

  it("copies generated bundled channel config schemas into dist manifests", () => {
    const repoRoot = makeRepoRoot("daisyclaw-bundled-channel-config-meta-");
    createPlugin(repoRoot, {
      id: "telegram",
      packageName: "@daisyclaw/telegram",
      manifest: {
        channels: ["telegram"],
        channelConfigs: {
          telegram: {
  ...
  it("relocates node_modules-backed skill paths into bundled-skills and rewrites the manifest", () => {
    const repoRoot = makeRepoRoot("daisyclaw-bundled-plugin-node-modules-");
  ...
  it("falls back to repo-root hoisted node_modules skill paths", () => {
    const repoRoot = makeRepoRoot("daisyclaw-bundled-plugin-hoisted-skill-");
    const pluginDir = createTlonSkillPlugin(repoRoot);
  ...
  it("omits missing declared skill paths and removes stale generated outputs", () => {
    const repoRoot = makeRepoRoot("daisyclaw-bundled-plugin-missing-skill-");
  ...
  it("retries transient skill copy races from concurrent runtime postbuilds", () => {
    const repoRoot = makeRepoRoot("daisyclaw-bundled-plugin-retry-");
    const pluginDir = createPlugin(repoRoot, {
      id: "diffs",
  ...
  it("removes generated outputs for plugins no longer present in source", () => {
    const repoRoot = makeRepoRoot("daisyclaw-bundled-plugin-removed-");
    const staleBundledSkillDir = path.join(
      repoRoot,
      "dist",
      "extensions",
    ...
    writeJson(path.join(repoRoot, "dist", "extensions", "removed-plugin", "daisyclaw.plugin.json"), {
      id: "removed-plugin",
      configSchema: { type: "object" },
      skills: ["./bundled-skills/@scope/skill"],
    });
    writeJson(path.join(repoRoot, "dist", "extensions", "removed-plugin", "package.json"), {
      name: "@daisyclaw/removed-plugin",
    });
  ...
  it("removes stale dist outputs when a source extension directory no longer has a manifest", () => {
    const repoRoot = makeRepoRoot("daisyclaw-bundled-plugin-manifestless-source-");
    ...
    writeJson(path.join(staleDistDir, "daisyclaw.plugin.json"), {
      id: "google-gemini-cli-auth",
      configSchema: { type: "object" },
    });
    writeJson(path.join(staleDistDir, "package.json"), {
      name: "@daisyclaw/google-gemini-cli-auth",
    });
  ...
  it("removes non-packaged private QA plugin metadata unless private QA build is enabled", () => {
    const repoRoot = makeRepoRoot("daisyclaw-private-qa-metadata-");
    createPlugin(repoRoot, {
      id: "qa-lab",
      packageName: "@daisyclaw/qa-lab",
      packageDaisyClaw: { extensions: ["./index.ts"] },
    });
    ...
    copyBundledPluginMetadataWithEnv({
      repoRoot,
      env: { DAISYCLAW_BUILD_PRIVATE_QA: "1" } as NodeJS.ProcessEnv,
    });

    expect(fs.existsSync(path.join(staleDistDir, "daisyclaw.plugin.json"))).toBe(true);
  ...
  it.each([
    {
      ...
      pluginId: "acpx",
      packageName: "@daisyclaw/acpx-plugin",
      packageDaisyClaw: { extensions: ["./index.ts"] },
    ...
    },
    {
      ...
      packageDaisyClaw: {
        extensions: ["./index.ts"],
        install: { npmSpec: "@daisyclaw/whatsapp" },
      },
      env: {},
      expectedExists: false,
    },
  ] as const)("$name", ({ pluginId, packageName, packageDaisyClaw, env, expectedExists }) => {
    const repoRoot = makeRepoRoot(`daisyclaw-bundled-plugin-${pluginId}-`);
    createPlugin(repoRoot, {
      id: pluginId,
      packageName,
      packageDaisyClaw,
  ...
  it("removes build-excluded bundled plugin metadata", () => {
    const repoRoot = makeRepoRoot("daisyclaw-bundled-plugin-excluded-meta-");
    createPlugin(repoRoot, {
      id: "qqbot",
      packageName: "@daisyclaw/qqbot",
      packageDaisyClaw: {
        extensions: ["./index.ts"],
  ...
  it("preserves manifest-less runtime support package outputs and copies package metadata", () => {
    const repoRoot = makeRepoRoot("daisyclaw-bundled-runtime-support-");
    const pluginDir = path.join(repoRoot, "extensions", "image-generation-core");
    fs.mkdirSync(pluginDir, { recursive: true });
    writeJson(path.join(pluginDir, "package.json"), {
      name: "@daisyclaw/image-generation-core",
      version: "0.0.1",
      private: true,
      type: "module",
    });
    ...
    expect(
      fs.existsSync(
        path.join(repoRoot, "dist", "extensions", "image-generation-core", "daisyclaw.plugin.json"),
      ),
    ).toBe(false);
    expect(
      JSON.parse(
