...
/** Env var that points bundled-plugin lookup at an DaisyClaw source checkout. */
export const DAISYCLAW_DEV_SOURCE_ROOT_ENV = "DAISYCLAW_DEV_SOURCE_ROOT";
...
/** Resolves and validates the configured DaisyClaw development source root. */
export function resolveDaisyClawDevSourceRoot(env: NodeJS.ProcessEnv = process.env): string | null {
  const rawRoot = env[DAISYCLAW_DEV_SOURCE_ROOT_ENV]?.trim();
  if (!rawRoot) {
    return null;
  }
  ...
  if (readPackageName(path.join(realRoot, "package.json")) !== "daisyclaw") {
    return null;
  }
...
export function isBundledPluginInsideDevSourceRoot(params: {
  ...
  const devSourceRoot = resolveDaisyClawDevSourceRoot(params.env);
  if (!devSourceRoot) {
    return false;
  }
