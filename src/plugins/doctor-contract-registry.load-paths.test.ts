...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(
    path.join(fs.realpathSync(os.tmpdir()), "daisyclaw-doctor-contract-load-paths-"),
  );
  tempDirs.push(dir);
  return dir;
}
...
function createDoctorPluginConfig(pluginRoot: string, pluginId: string): DaisyClawConfig {
  return {
    plugins: {
...
function readPluginLlmPolicy(config: DaisyClawConfig, pluginId: string): Record<string, unknown> {
