...
function isPackageRootDaisyClawPeerSymlink(segments: string[]): boolean {
  return (
    (segments.length === 2 && segments[0] === "node_modules" && segments[1] === "daisyclaw") ||
    (segments.length === 3 &&
      segments[0] === "node_modules" &&
      segments[1] === ".bin" &&
      segments[2] === "daisyclaw")
  );
}
...
function isManagedNpmRootPackagePeerSymlink(segments: string[]): boolean {
  ...
  return isPackageRootDaisyClawPeerSymlink(segments.slice(packageEndIndex));
}

function isTrustedDaisyClawPeerSymlink(params: {
  allowManagedNpmRootPackagePeerSymlinks?: boolean;
  relativePath: string;
}): boolean {
  const segments = params.relativePath.split(/[\\/]+/);
  return (
    isPackageRootDaisyClawPeerSymlink(segments) ||
    (params.allowManagedNpmRootPackagePeerSymlinks === true &&
      isManagedNpmRootPackagePeerSymlink(segments))
  );
}

async function resolveTrustedHostDaisyClawRootRealPath(): Promise<string | null> {
...
function isTrustedHostDaisyClawPath(params: {
  resolvedTargetPath: string;
  trustedHostDaisyClawRootRealPath: string | null;
}): boolean {
  return (
    params.trustedHostDaisyClawRootRealPath !== null &&
    isPathInside(params.trustedHostDaisyClawRootRealPath, params.resolvedTargetPath)
  );
}
...
> {
  ...
  if (!isPathInside(params.rootRealPath, resolvedTargetPath)) {
    if (
      ...
      isTrustedHostDaisyClawPath({
        resolvedTargetPath,
        trustedHostDaisyClawRootRealPath: params.trustedHostDaisyClawRootRealPath,
      })
    ) {
      return {};
    }
...
function collectManifestRuntimeDependencyNames(manifest: PackageManifest): string[] {
  ...
  for (const dependencyName of Object.keys(manifest.peerDependencies ?? {})) {
    if (dependencyName !== "daisyclaw" && isInstallScannableDependencyName(dependencyName)) {
      dependencyNames.add(dependencyName);
    }
  }
...
async function collectPackageManifestPaths(params: {
  ...
  const trustedHostDaisyClawRootRealPath = await resolveTrustedHostDaisyClawRootRealPath();
      ...
      if (entry.isSymbolicLink()) {
        ...
        if (pathContainsNodeModulesSegment(relativeNextPath)) {
          const symlinkTargetInspection = await inspectNodeModulesSymlinkTarget({
            ...
            trustedHostDaisyClawRootRealPath,
          });
...
function resolvePolicySource(params: {
  ...
  if (params.requestKind === "skill-install") {
    switch (params.origin?.type) {
      case "clawhub":
        return { kind: "clawhub", authority: "daisyclaw", mutable: false, network: true };
      case "git":
        return {
          kind: "git",
      ...
      case "daisyclaw-bundled":
        return { kind: "bundled", authority: "daisyclaw", mutable: false, network: false };
      case "daisyclaw-managed":
      case "daisyclaw-extra":
        return { kind: "managed", authority: "daisyclaw", mutable: false, network: false };
      default:
