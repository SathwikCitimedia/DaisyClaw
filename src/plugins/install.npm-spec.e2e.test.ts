...
async function makeTempDir(label: string): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), `daisyclaw-${label}-`));
  tempDirs.push(dir);
  return dir;
}

function configWithInstalledPackageTreeBlockPolicy(): DaisyClawConfig {
  return {
    security: {
      installPolicy: {
        enabled: true,
...
async function packPlugin(params: {
  ...
  await fs.writeFile(
    path.join(packageDir, "daisyclaw.plugin.json"),
    `${JSON.stringify(
      {
        id: params.pluginId,
            ...
            versions: Object.fromEntries(
              [...pkg.versionsByVersion.entries()].map(([version, entry]) => [
                version,
                {
                  name: pkg.packageName,
                  version,
                  ...(entry.daisyclaw ? { daisyclaw: entry.daisyclaw } : {}),
...
describe("installPluginFromNpmSpec e2e", () => {
  it("installs the newest compatible stable package when npm latest requires a newer plugin API", async () => {
    ...
    const versions = [
      ...
      await packPlugin({
        ...
        version: "2026.5.27",
        rootDir,
        daisyclaw: incompatibleDaisyClaw,
      }),
    ];
    const registry = await startStaticRegistry([{ packageName, latest: "2026.5.27", versions }]);
    process.env.NPM_CONFIG_REGISTRY = registry;
    process.env.npm_config_registry = registry;
    const previousHostVersion = process.env.DAISYCLAW_COMPATIBILITY_HOST_VERSION;
    process.env.DAISYCLAW_COMPATIBILITY_HOST_VERSION = "2026.5.10-beta.1";
    ...
    try {
      ...
      if (previousHostVersion === undefined) {
        ...
        process.env.DAISYCLAW_COMPATIBILITY_HOST_VERSION = previousHostVersion;
      }
    }
  });

  it("scrubs root daisyclaw materialized by required npm peers", async () => {
    ...
    const daisyClawVersions = [
      await packPlugin({
        packageName: "daisyclaw",
        pluginId: "registry-daisyclaw-copy",
        version: "2026.0.0",
        rootDir,
      }),
    ];
    const registry = await startStaticRegistry([
      ...
      { packageName: "daisyclaw", latest: "2026.0.0", versions: daisyClawVersions },
    ]);
    process.env.NPM_CONFIG_REGISTRY = registry;
    process.env.npm_config_registry = registry;
    ...
    const rawDaisyClawLockEntry = rawLock.packages?.["node_modules/daisyclaw"] as
      | { peer?: unknown; version?: unknown }
      | undefined;
    expect(rawDaisyClawLockEntry?.peer).toBe(true);
    expect(rawDaisyClawLockEntry?.version).toBe("2026.0.0");
    ...
    expect(lock.packages?.["node_modules/daisyclaw"]).toBeUndefined();
    await expect(
      fs.lstat(path.join(projectRoot, "node_modules", "daisyclaw")),
    ).rejects.toHaveProperty("code", "ENOENT");
    await expect(
      fs
        .lstat(path.join(result.targetDir, "node_modules", "daisyclaw"))
  ...
  it("plans peers from installed optional dependencies", async () => {
    ...
    const rootManifest = JSON.parse(
      ...
      dependencies?: Record<string, string>;
      daisyclaw?: { managedPeerDependencies?: string[] };
    };
    expect(rootManifest.dependencies?.[runtimePeer]).toBe("1.0.0");
    expect(rootManifest.daisyclaw?.managedPeerDependencies ?? []).toContain(runtimePeer);
  });
  ...
  it("leaves legacy flat-root peer dependencies alone during isolated later installs", async () => {
    ...
    expect(rootManifest.daisyclaw?.managedPeerDependencies ?? []).not.toContain(runtimePeer);
  });
  ...
  it("falls back to the legacy npm peer mode inside the plugin project when npm cannot plan third-party peers", async () => {
    ...
    expect(rootManifest.daisyclaw?.managedPeerDependencies ?? []).not.toContain(missingPeer);
    await expect(
      fs.lstat(path.join(projectRoot, "node_modules", blockedPlugin, "package.json")),
    ).resolves.toBeTruthy();
  });
  ...
  it("does not take ownership of an existing root dependency observed as a peer", async () => {
    ...
    expect(rootManifest.dependencies?.[blockedPlugin]).toBeUndefined();
    expect(rootManifest.dependencies?.[runtimePeer]).toBeUndefined();
    expect(rootManifest.daisyclaw?.managedPeerDependencies ?? []).not.toContain(
  ...
  it("scrubs host peers inside each isolated npm project", async () => {
    ...
    const registry = await startStaticRegistry([
      {
        ...
        versions: [
          await packPlugin({
            packageName: codexName,
            peerDependencies: { daisyclaw: ">=2026.5.5-beta.2" },
            peerDependenciesMeta: { daisyclaw: { optional: true } },
            pluginId: codexName,
            version: "1.0.0",
            rootDir,
          }),
      ...
      },
      {
        ...
        versions: [
          await packPlugin({
            packageName: opikName,
            peerDependencies: { daisyclaw: ">=2026.3.2" },
            peerDependenciesMeta: {},
            pluginId: opikName,
            version: "1.0.0",
            rootDir,
    ...
    await expect(
      fs
        .lstat(path.join(first.targetDir, "node_modules", "daisyclaw"))
        .then((stat) => stat.isSymbolicLink()),
    ).resolves.toBe(true);
    await expect(
      fs
        .lstat(path.join(second.targetDir, "node_modules", "daisyclaw"))
        .then((stat) => stat.isSymbolicLink()),
    ).resolves.toBe(true);
  });

  it("keeps an earlier isolated daisyclaw peer link after later plugin installs", async () => {
    ...
    const peerLink = path.join(first.targetDir, "node_modules", "daisyclaw");
    ...
    expect(manifest.dependencies?.daisyclaw).toBeUndefined();
    const lock = JSON.parse(
