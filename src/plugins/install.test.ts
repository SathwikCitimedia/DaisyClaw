...
import { resolveDaisyClawPackageRootSync } from "../infra/daisyclaw-root.js";
...
vi.mock("../infra/daisyclaw-root.js", () => ({
  resolveDaisyClawPackageRootSync: vi.fn(),
}));
...
const suiteTempRootTracker = createSuiteTempRootTracker("daisyclaw-plugin-install");
...
const DYNAMIC_ARCHIVE_TEMPLATE_PRESETS = [
  {
    ...
    packageJson: {
      name: "@evil/..",
      version: "0.0.1",
      daisyclaw: { extensions: ["./dist/index.js"] },
...
process.stdin.on("end", () => {
  fs.appendFileSync(process.env.DAISYCLAW_POLICY_LOG, input + "\\n");
...
function configWithInstallPolicy(scriptPath: string, logPath: string): DaisyClawConfig {
  return {
    security: {
      installPolicy: {
        enabled: true,
        exec: {
          ...
          args: [scriptPath],
          env: { DAISYCLAW_POLICY_LOG: logPath },
...
function setPluginMinHostVersion(pluginDir: string, minHostVersion: string) {
  const packageJsonPath = path.join(pluginDir, "package.json");
  const manifest = JSON.parse(fs.readFileSync(packageJsonPath, "utf-8")) as {
    daisyclaw?: { install?: Record<string, unknown> };
  };
  manifest.daisyclaw = {
    ...manifest.daisyclaw,
    install: {
      ...manifest.daisyclaw?.install,
      minHostVersion,
    },
  };
  fs.writeFileSync(packageJsonPath, JSON.stringify(manifest), "utf-8");
...
function setPluginPackageCompatibility(pluginDir: string, pluginApiRange: unknown) {
  const packageJsonPath = path.join(pluginDir, "package.json");
  const manifest = JSON.parse(fs.readFileSync(packageJsonPath, "utf-8")) as {
    daisyclaw?: { compat?: Record<string, unknown> };
  };
  manifest.daisyclaw = {
    ...manifest.daisyclaw,
    compat: {
      ...manifest.daisyclaw?.compat,
...
async function ensureDynamicArchiveTemplate(params: {
  ...
  if (params.writePluginManifest !== false) {
    ...
    fs.writeFileSync(
      path.join(pkgDir, "daisyclaw.plugin.json"),
      JSON.stringify({
...
beforeAll(async () => {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@daisyclaw/cognee-daisyclaw",
      version: "0.0.1",
      daisyclaw: { extensions: ["./dist/index.js"] },
    }),
    "utf-8",
  ...
  fs.writeFileSync(
    path.join(manifestInstallTemplateDir, "daisyclaw.plugin.json"),
    JSON.stringify({
...
describe("installPluginFromArchive", () => {
  it("installs package archive runtime dependencies", async () => {
    ...
    expect(commandOptions.cwd).toContain(".daisyclaw-install-stage-");
  });
  ...
  it("installs scoped archives, rejects duplicate installs, and allows updates", async () => {
    ...
    expectSuccessfulArchiveInstall({ result: first, stateDir, pluginId: "@daisyclaw/voice-call" });
  ...
  it("rejects native plugin zip archives without daisyclaw.plugin.json", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package missing valid daisyclaw.plugin.json");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_PLUGIN_MANIFEST);
    }
    expect(fs.existsSync(resolvePluginInstallDir("@daisyclaw/zipper", extensionsDir))).toBe(false);
  });
  ...
  it("rejects packages without daisyclaw.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@daisyclaw/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
    ...
    expect(result.error).toContain("daisyclaw.extensions");
    expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_DAISYCLAW_EXTENSIONS);
  });

  it("rejects legacy plugin package shape when daisyclaw.extensions is missing", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@daisyclaw/legacy-entry-fallback",
        version: "0.0.1",
      }),
      "utf-8",
    );
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package.json missing daisyclaw.extensions");
      expect(result.error).toContain("update the plugin package");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_DAISYCLAW_EXTENSIONS);
      return;
    }
    expect.unreachable("expected install to fail without daisyclaw.extensions");
  });

  it("rejects package installs when daisyclaw.extensions entries escape the package", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.mkdirSync(path.join(pluginDir, "dist"), { recursive: true });
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.INVALID_DAISYCLAW_EXTENSIONS);
  ...
  it("allows missing TypeScript source entries when an inferred built runtime entry exists", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        daisyclaw: { extensions: ["./src/index.ts"] },
      }),
    );
    fs.writeFileSync(path.join(pluginDir, "dist", "index.js"), "export {};\n");
  ...
  it("rejects package installs when daisyclaw.extensions contains a blank entry", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.mkdirSync(path.join(pluginDir, "dist"), { recursive: true });
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
    ...
    if (!result.ok) {
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.INVALID_DAISYCLAW_EXTENSIONS);
      expect(result.error).toContain("daisyclaw.extensions[1]");
  ...
  it("rejects package installs when runtimeExtensions contains a blank entry", async () => {
    ...
    if (!result.ok) {
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.INVALID_DAISYCLAW_EXTENSIONS);
      expect(result.error).toContain("daisyclaw.runtimeExtensions[0]");
      expect(result.error).toContain("non-empty string");
    }
  });
  ...
  it("rejects package installs when an extension entry is a symlink escape", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        daisyclaw: { extensions: ["./linked/escape.js"] },
      }),
    );
  ...
  it("rejects package installs when an extension entry is a hardlinked alias", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        daisyclaw: { extensions: ["./escape.js"] },
      }),
    );
  ...
  it("allows declared package entrypoints with dangerous code under test-looking paths", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        daisyclaw: { extensions: ["tests/runtime.test.js"] },
      }),
    );
    fs.mkdirSync(path.join(pluginDir, "tests"), { recursive: true });
    fs.writeFileSync(
  ...
  it("does not flag the real qa-matrix plugin as dangerous install code", async () => {
    ...
    vi.mocked(resolveDaisyClawPackageRootSync).mockReturnValue(process.cwd());
    ...
    const scanResult = await installSecurityScan.scanPackageInstallSource({
      ...
      pluginId: "qa-matrix",
      packageName: "@daisyclaw/qa-matrix",
  ...
  it("allows extension entry files in hidden directories without built-in scanner warnings", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        daisyclaw: { extensions: [".hidden/index.js"] },
      }),
    );
    fs.writeFileSync(
...
describe("installPluginFromDir", () => {
  ...
  it("preserves local package manifests without dependency surgery", async () => {
    ...
    expect(manifest.devDependencies?.daisyclaw).toBe("workspace:*");
  ...
  it.each([
    {
      ...
      expectedMessageIncludes: ["requires DaisyClaw >=2026.3.22, but this host is 2026.3.21"],
    },
    {
      name: "rejects plugins with invalid minHostVersion metadata",
      minHostVersion: "2026.3.22",
      expectedCode: PLUGIN_INSTALL_ERROR_CODE.INVALID_MIN_HOST_VERSION,
      expectedMessageIncludes: ["invalid package.json daisyclaw.install.minHostVersion"],
    },
    {
  ...
  it("rejects plugins whose package plugin API metadata is malformed", async () => {
    ...
    expectFailedInstallResult({
      ...
      messageIncludes: ["daisyclaw.compat.pluginApi", "must be a string"],
    });
    expect(vi.mocked(runCommandWithTimeout)).not.toHaveBeenCalled();
  });
  ...
  it("checks package plugin API before current-host extension shape validation", async () => {
    ...
    if (!result.ok) {
      expect(result.error).not.toContain("daisyclaw.extensions");
    }
    expect(vi.mocked(runCommandWithTimeout)).not.toHaveBeenCalled();
  });
  ...
  it("rejects bundle package installs whose package plugin API range is newer than the current host", async () => {
    ...
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@daisyclaw/future-bundle",
        version: "2026.5.27",
        daisyclaw: { compat: { pluginApi: ">=2026.5.27" } },
      }),
      "utf-8",
    );
  ...
  it("allows plugins when a beta host is on the package plugin API floor", async () => {
    ...
    expect(result.pluginId).toBe("@daisyclaw/test-plugin");
  });

  it("uses daisyclaw.plugin.json id as install key when it differs from package name", async () => {
    ...
    expect(
      infoMessages.some((msg) =>
        msg.includes(
          'Plugin manifest id "memory-cognee" differs from npm package name "@daisyclaw/cognee-daisyclaw"',
        ),
      ),
    ).toBe(true);
  });
  ...
  it.each([
    ...
    },
    {
      ...
      expectedPluginId: "@daisyclaw/test-plugin",
      install: (pluginDir: string, extensionsDir: string) =>
        installPluginFromDir({
          dirPath: pluginDir,
          extensionsDir,
...
describe("linkDaisyClawPeerDependencies (via installPluginFromDir)", () => {
  const resolveRootMock = vi.mocked(resolveDaisyClawPackageRootSync);
  ...
  it("creates a node_modules/daisyclaw symlink when peerDependencies declares daisyclaw", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    const fakeHostRoot = suiteTempRootTracker.makeTempDir();
    const run = vi.mocked(runCommandWithTimeout);
    resolveRootMock.mockReturnValue(fakeHostRoot);

    writePluginWithPeerDeps(pluginDir, { daisyclaw: "*" });
    ...
    const symlinkPath = path.join(result.targetDir, "node_modules", "daisyclaw");
  ...
  it("keeps the daisyclaw peer symlink when a local plugin already has dependencies", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    const fakeHostRoot = suiteTempRootTracker.makeTempDir();
    resolveRootMock.mockReturnValue(fakeHostRoot);

    writePluginWithPeerDeps(pluginDir, { daisyclaw: "*" }, { "is-number": "7.0.0" });
  ...
  it("replaces a copied local daisyclaw package with the host peer symlink", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    const fakeHostRoot = suiteTempRootTracker.makeTempDir();
    resolveRootMock.mockReturnValue(fakeHostRoot);

    writePluginWithPeerDeps(pluginDir, { daisyclaw: "*" });
    fs.mkdirSync(path.join(pluginDir, "node_modules", "daisyclaw"), { recursive: true });
    fs.writeFileSync(
      path.join(pluginDir, "node_modules", "daisyclaw", "package.json"),
      JSON.stringify({ name: "daisyclaw", version: "2026.5.31" }),
      "utf-8",
    );
  ...
  it("does not create a symlink when peerDependencies is empty", async () => {
    ...
    const symlinkPath = path.join(nodeModulesDir, "daisyclaw");
  ...
  it("is idempotent - re-installing replaces an existing symlink without error", async () => {
    ...
    const symlinkPath = path.join(second.targetDir, "node_modules", "daisyclaw");
    expect(fs.lstatSync(symlinkPath).isSymbolicLink()).toBe(true);
  });

  it("rejects when resolveDaisyClawPackageRootSync returns null", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    ...
    expectWarningIncludes(warnings, "Could not locate daisyclaw package root");
  });
});
