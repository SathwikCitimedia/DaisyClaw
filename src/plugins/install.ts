...
import { normalizeOptionalString } from "@daisyclaw/normalization-core/string-coerce";
...
import {
  ...
  readDaisyClawManagedNpmRootOverrides,
  repairManagedNpmRootDaisyClawPeer,
...
import { installedPackageNeedsDaisyClawPeerLinkRepair } from "../infra/package-update-utils.js";
import {
...
function formatUnresolvedDaisyClawPeerLinkError(packageName: string): string {
  return `Installed plugin ${packageName} declares daisyclaw as a peer dependency, but DaisyClaw could not create a plugin-local node_modules/daisyclaw link. Run from a packaged DaisyClaw install or reinstall DaisyClaw, then retry.`;
}
...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing daisyclaw.extensions; update the plugin package to include daisyclaw.extensions (for example ["./dist/index.js"]). See https://docs.daisyclaw.ai/help/troubleshooting#plugin-install-fails-with-missing-daisyclaw-extensions';
const PLUGIN_ARCHIVE_ROOT_MARKERS = [
  "package.json",
  "daisyclaw.plugin.json",
  ".codex-plugin/plugin.json",
  ".claude-plugin/plugin.json",
  ".cursor-plugin/plugin.json",
];
const MANAGED_NPM_PACK_ARCHIVE_DIR = "_daisyclaw-pack-archives";
const MANAGED_NPM_PROJECT_QUARANTINE_DIR = "_daisyclaw-quarantined-npm-projects";
...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_DAISYCLAW_EXTENSIONS: "missing_daisyclaw_extensions",
  MISSING_PLUGIN_MANIFEST: "missing_plugin_manifest",
  EMPTY_DAISYCLAW_EXTENSIONS: "empty_daisyclaw_extensions",
  INVALID_DAISYCLAW_EXTENSIONS: "invalid_daisyclaw_extensions",
...
function validateDaisyClawPackageCompatibility(params: {
  pluginId: string;
  currentHostVersion: string;
  packageMetadata?: DaisyClawPackageManifest;
  ...
  if (!pluginApiRangeCheck.ok) {
    return {
      ok: false,
      error: `invalid package.json daisyclaw.compat.pluginApi: ${pluginApiRangeCheck.error}`,
  ...
  if (pluginApiRange && !satisfiesPluginApiRange(params.currentHostVersion, pluginApiRange)) {
    return {
      ok: false,
      error: `plugin "${params.pluginId}" requires plugin API ${pluginApiRange}, but this DaisyClaw runtime exposes ${params.currentHostVersion}. Upgrade DaisyClaw or install a compatible plugin version and retry.`,
...
function validateDaisyClawPackageInstallCompatibility(params: {
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json daisyclaw.install.minHostVersion: ${minHostVersionCheck.error}`,
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${params.pluginId}" requires DaisyClaw >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set DAISYCLAW_VERSION and retry.`,
        code: PLUGIN_INSTALL_ERROR_CODE.UNKNOWN_HOST_VERSION,
      };
    }
    return {
      ok: false,
      error: `plugin "${params.pluginId}" requires DaisyClaw >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}. Upgrade DaisyClaw and retry.`,
      code: PLUGIN_INSTALL_ERROR_CODE.INCOMPATIBLE_HOST_VERSION,
    };
  }

  return validateDaisyClawPackageCompatibility({
...
function ensureDaisyClawExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_DAISYCLAW_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json daisyclaw.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_DAISYCLAW_EXTENSIONS,
    };
  }
  if (resolved.status === "invalid") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.INVALID_DAISYCLAW_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
function compareNpmSemver(a: string, b: string): number {
  const releaseCmp = compareDaisyClawReleaseVersions(a, b);
  if (releaseCmp !== null) {
    return releaseCmp;
  }
...
async function resolveTrustedOfficialPrereleaseResolution(params: {
  ...
  if (!params.spec.name.startsWith("@daisyclaw/")) {
    return null;
  }
  ...
  if (!stableVersion) {
    ...
    if (prereleaseVersion && semverVersions.every(isPrereleaseSemverVersion)) {
      if (prereleaseVersion !== params.resolvedPrereleaseVersion) {
        ...
        params.logger.warn?.(
          `Resolved ${params.spec.raw} to prerelease version ${params.resolvedPrereleaseVersion}; using newest prerelease ${prereleaseSpec} because this trusted official DaisyClaw package has no stable npm versions yet.`,
        );
        return { kind: "prerelease-only", resolution: metadataResult.metadata };
      }
      params.logger.warn?.(
        `Resolved ${params.spec.raw} to prerelease version ${params.resolvedPrereleaseVersion}; allowing it because this trusted official DaisyClaw package has no stable npm versions yet.`,
      );
  ...
  params.logger.warn?.(
    `Resolved ${params.spec.raw} to prerelease version ${params.resolvedPrereleaseVersion}; falling back to stable ${stableSpec} for this trusted official DaisyClaw install.`,
  );
...
function validateNpmResolutionCompatibility(params: {
  ...
  return validateDaisyClawPackageInstallCompatibility({
    runtime: params.runtime,
    pluginId: params.expectedPluginId ?? params.resolution.name ?? params.parsedSpec.name,
    packageMetadata: params.resolution.packageDaisyClaw as DaisyClawPackageManifest | undefined,
  });
}
...
async function resolveLatestCompatibleNpmResolution(params: {
  ...
  for (const version of candidates) {
    ...
    if (!compatibilityError) {
      params.logger.warn?.(
        `Resolved ${params.parsedSpec.raw} to ${params.currentResolution.resolvedSpec ?? currentVersion}, but that version is incompatible with this DaisyClaw runtime; using newest compatible ${metadataResult.metadata.resolvedSpec ?? spec}.`,
      );
...
async function rollbackManagedNpmPluginInstall(params: {
  ...
  if (params.snapshot) {
    try {
      ...
      await relinkDaisyClawPeerDependenciesInManagedNpmRoot({
        npmRoot: params.npmRoot,
  ...
  if (params.packageName !== "daisyclaw") {
    try {
      await repairManagedNpmRootDaisyClawPeer({
        npmRoot: params.npmRoot,
        timeoutMs: params.timeoutMs,
        logger: params.logger,
      });
      ...
      params.logger.warn?.(
        `Failed to repair managed npm daisyclaw peer after rollback: ${String(error)}`,
      );
    }
  }
  try {
...
async function createManagedNpmPluginInstallRollbackSnapshot(params: {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-npm-plugin-rollback-"));
...
async function listManagedNpmRootPackageNames(npmRoot: string): Promise<Set<string>> {
  ...
  for (const entry of entries.toSorted((left, right) => left.name.localeCompare(right.name))) {
    if (entry.name === ".bin" || entry.name === "daisyclaw") {
      continue;
    }
...
async function installPluginFromManagedNpmRoot(
  ...
  const runManagedNpmInstall = async (
    ...
    if (params.packageName !== "daisyclaw") {
      const repairedDaisyClawPeer = await repairManagedNpmRootDaisyClawPeer({
        npmRoot,
        timeoutMs,
        logger,
      });
      if (repairedDaisyClawPeer) {
        logger.info?.(`Repaired stale daisyclaw peer dependency in ${npmRoot}`);
      }
    }
    let preInstallRootPackageNames = await listManagedNpmRootPackageNames(npmRoot);
    const managedOverrides = await readDaisyClawManagedNpmRootOverrides();
    ...
    if (install.code !== 0 && isManagedNpmProjectCorruptionInstallFailure(install)) {
      ...
      try {
        ...
        return await rollbackFailedManagedNpmInstall({
          ok: false,
          error: `npm install failed with a managed npm project corruption signature, but DaisyClaw could not quarantine ${npmRoot} for rebuild: ${String(error)}. Original npm error: ${originalError}`,
        });
      }
      logger.warn?.(
    ...
    if (params.packageName !== "daisyclaw") {
      ...
      if (repairedDaisyClawPeer) {
        logger.info?.(`Repaired stale daisyclaw peer dependency in ${npmRoot} after npm install`);
      }
    }
    try {
      await relinkDaisyClawPeerDependenciesInManagedNpmRoot({
      ...
      return await rollbackFailedManagedNpmInstall({
        ok: false,
        error: `Failed to repair daisyclaw peer links after npm install: ${String(error)}`,
      });
    }
    if (installedPackageNeedsDaisyClawPeerLinkRepair(installRoot)) {
      return await rollbackFailedManagedNpmInstall({
        ok: false,
        error: formatUnresolvedDaisyClawPeerLinkError(params.packageName),
      });
    }
...
> {
  ...
  try {
    ...
    backupTempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-npm-pack-archive-"));
...
async function runInstallSourceScan(params: {
  ...
  try {
    ...
    return {
      ok: false,
      error: `${params.subject} installation blocked: code safety scan failed (${String(err)}). Run "daisyclaw security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
}
...
async function installBundleFromSourceDir(
  ...
  const packageMetadata = packageManifestResult.manifest
    ...
    : undefined;
  const compatibilityError = validateDaisyClawPackageInstallCompatibility({
    runtime,
    pluginId,
    packageMetadata,
  });
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureDaisyClawExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
> {
  ...
  if (!ocManifestResult.ok && params.requirePluginManifest) {
    return {
      ok: false,
      error: `package missing valid daisyclaw.plugin.json: ${ocManifestResult.error}`,
  ...
  const extensionsResult = ensureDaisyClawExtensions({
    manifest,
  });
...
async function scanAndLinkInstalledPackage(params: {
  ...
  const peerLinkRepair = await linkDaisyClawPeerDependencies({
  ...
  if (peerLinkRepair.skipped > 0) {
    return {
      ok: false,
      error: formatUnresolvedDaisyClawPeerLinkError(params.pluginId),
    };
  }
  return null;
}
...
export async function installPluginFromArchive(
  ...
  return await runtime.withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "daisyclaw-plugin-",
    timeoutMs,
    logger,
...
export async function installPluginFromNpmSpec(
  ...
  const policyTempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-npm-policy-"));
  try {
...
export async function installPluginFromPath(
  ...
  return {
    ...
    error:
      "Plain file plugin installs are not supported. Install a plugin directory or archive that contains daisyclaw.plugin.json, or list standalone plugin files in plugins.load.paths.",
  };
}
