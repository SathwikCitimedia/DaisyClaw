...
import type { DaisyClawStateDatabaseOptions } from "../state/daisyclaw-state-db.js";
...
function hasPackagePluginMetadata(manifest: Record<string, unknown>): boolean {
  const daisyclaw = manifest.daisyclaw;
  if (!isRecord(daisyclaw)) {
    return false;
  }
  const extensions = daisyclaw.extensions;
...
function readManifestPluginId(packageDir: string): string | undefined {
  const manifest = readJsonObjectFileSync(path.join(packageDir, "daisyclaw.plugin.json"));
  ...
  if (options.stateDir) {
    return {
      env: {
        ...
        DAISYCLAW_STATE_DIR: options.stateDir,
      },
    };
  }
...
function readPersistedInstalledPluginIndexForRecords(
  ...
  try {
    const database = openDaisyClawStateDatabase(resolveStateDatabaseOptions(options));
