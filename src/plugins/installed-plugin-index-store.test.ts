...
function makeTempDir() {
  return makeTrackedTempDir("daisyclaw-installed-plugin-index-store", tempDirs);
}
...
function createIndex(overrides: Partial<InstalledPluginIndex> = {}): InstalledPluginIndex {
  return {
    ...
    plugins: [
      {
        pluginId: "demo",
        manifestPath: "/plugins/demo/daisyclaw.plugin.json",
  ...
  it("preserves startup config paths across persisted index roundtrips", async () => {
    ...
    const index = createIndex({
      plugins: [
        {
          pluginId: "browser",
          manifestPath: "/plugins/browser/daisyclaw.plugin.json",
      ...
      plugins: [
        {
          ...
          manifestPath: "/plugins/provider-owner/daisyclaw.plugin.json",
