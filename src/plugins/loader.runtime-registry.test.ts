...
function makeDaisyClawDevSourceRoot(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-loader-dev-source-"));
...
describe("resolveRuntimePluginRegistry", () => {
  ...
  it("keeps the full workspace registry warm when scoped cron registries churn", () => {
    ...
    try {
      ...
      const fullRegistry = loadDaisyClawPlugins(loadOptions);

      loadDaisyClawPlugins({ ...loadOptions, onlyPluginIds: ["alpha"] });
      loadDaisyClawPlugins({ ...loadOptions, onlyPluginIds: ["bravo"] });
...
describe("loadDaisyClawPlugins active runtime clearing", () => {
  it("clears plugin-owned global providers before activating a new registry", () => {
    ...
    loadDaisyClawPlugins({ onlyPluginIds: [] });
...
describe("clearPluginRegistryLoadCache", () => {
  ...
  it("invalidates full-workspace load snapshots", () => {
    ...
    const registry = loadDaisyClawPlugins(loadOptions);

    clearPluginRegistryLoadCache();

    expect(loadDaisyClawPlugins(loadOptions)).not.toBe(registry);
  });
});
