...
import { clearPluginLoaderCache, loadDaisyClawPlugins } from "./loader.js";
import { resetPluginRuntimeStateForTest } from "./runtime.js";

export type TempPlugin = { dir: string; file: string; id: string };
export type PluginLoadConfig = NonNullable<Parameters<typeof loadDaisyClawPlugins>[0]>["config"];
export type PluginRegistry = ReturnType<typeof loadDaisyClawPlugins>;
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "daisyclaw-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.DAISYCLAW_BUNDLED_PLUGINS_DIR;
const prevDisableBundledPlugins = process.env.DAISYCLAW_DISABLE_BUNDLED_PLUGINS;
...
export function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".daisyclaw", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ DAISYCLAW_STATE_DIR: stateDir, ...params.env }, () =>
...
export function resetPluginLoaderTestStateForTest() {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.DAISYCLAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
  if (prevDisableBundledPlugins === undefined) {
    delete process.env.DAISYCLAW_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.DAISYCLAW_DISABLE_BUNDLED_PLUGINS = prevDisableBundledPlugins;
  }
}
