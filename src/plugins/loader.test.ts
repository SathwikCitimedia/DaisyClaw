...
import { ensureDaisyClawPluginSdkAlias } from "./plugin-sdk-dist-alias.js";
...
function updatePluginManifest(plugin: Pick<TempPlugin, "dir">, patch: Record<string, unknown>) {
  const manifestPath = path.join(plugin.dir, "daisyclaw.plugin.json");
...
function writeBundledPlugin(params: {
  ...
  delete process.env.DAISYCLAW_DISABLE_BUNDLED_PLUGINS;
  process.env.DAISYCLAW_BUNDLED_PLUGINS_DIR = bundledDir;
  return { bundledDir, plugin };
}

function makeDaisyClawDevSourceRoot() {
  const root = makeTempDir();
  fs.writeFileSync(path.join(root, "package.json"), JSON.stringify({ name: "daisyclaw" }), "utf-8");
...
function writeWorkspacePlugin(params: {
  ...
  const workspacePluginDir = path.join(workspaceDir, ".daisyclaw", "extensions", params.id);
...
function withStateDir<T>(run: (stateDir: string) => T) {
  ...
  return withEnv({ DAISYCLAW_STATE_DIR: stateDir }, () => run(stateDir));
}
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.DAISYCLAW_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadDaisyClawPlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          daisyclaw: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
...
function setupBundledTelegramPlugin() {
  ...
  process.env.DAISYCLAW_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadDaisyClawPlugins>) {
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadDaisyClawPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
function loadRegistryFromAllowedPlugins(
  plugins: TempPlugin[],
  options?: Omit<Parameters<typeof loadDaisyClawPlugins>[0], "cache" | "config">,
) {
  return loadDaisyClawPlugins({
    cache: false,
    ...options,
...
function resolveLoadedPluginSource(
  registry: ReturnType<typeof loadDaisyClawPlugins>,
  pluginId: string,
) {
...
function expectCachePartitionByPluginSource(params: {
  pluginId: string;
  loadFirst: () => ReturnType<typeof loadDaisyClawPlugins>;
  loadSecond: () => ReturnType<typeof loadDaisyClawPlugins>;
...
function expectCacheMissThenHit(params: {
  loadFirst: () => ReturnType<typeof loadDaisyClawPlugins>;
  loadVariant: () => ReturnType<typeof loadDaisyClawPlugins>;
}) {
  const first = params.loadFirst();
...
function createEnvResolvedPluginFixture(pluginId: string) {
  useNoBundledPlugins();
  const daisyclawHome = makeTempDir();
  const ignoredHome = makeTempDir();
  const stateDir = makeTempDir();
  const pluginDir = path.join(daisyclawHome, "plugins", pluginId);
  mkdirSafe(pluginDir);
  const plugin = writePlugin({
    id: pluginId,
    dir: pluginDir,
  ...
  const env = {
    ...process.env,
    DAISYCLAW_HOME: daisyclawHome,
    HOME: ignoredHome,
    DAISYCLAW_STATE_DIR: stateDir,
    DAISYCLAW_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
  };
  return { plugin, env };
}
...
function expectEscapingEntryRejected(params: {
  ...
  const registry = loadDaisyClawPlugins({
    cache: false,
    config: {
      plugins: {
...
describe("loadDaisyClawPlugins", () => {
  ...
  it("can load scoped plugins from a supplied manifest registry without rereading manifests", () => {
    ...
    fs.rmSync(path.join(plugin.dir, "daisyclaw.plugin.json"));
  ...
  it("loads installed plugin packages discovered from persisted install records", () => {
    ...
    const registry = withEnv({ DAISYCLAW_STATE_DIR: stateDir }, () =>
      loadDaisyClawPlugins({
        cache: false,
        config: {
          plugins: {
  ...
  it("refreshes bundled plugin-sdk aliases without deleting the shared alias directory", () => {
    ...
    const aliasDir = path.join(distRoot, "extensions", "node_modules", "daisyclaw", "plugin-sdk");
    ...
    ensureDaisyClawPluginSdkAlias(distRoot);
  ...
  it("keeps private local-only plugin-sdk artifacts out of package dist aliases", () => {
    ...
    const pluginSdkDir = path.join(distRoot, "plugin-sdk");
    const aliasRoot = path.join(distRoot, "extensions", "node_modules", "daisyclaw");
  ...
  it("keeps private local-only plugin-sdk artifacts out of legacy package dist aliases", () => {
    ...
    fs.writeFileSync(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "daisyclaw", version: "2026.4.22", type: "module" }, null, 2),
      "utf8",
    );
    fs.writeFileSync(path.join(pluginSdkDir, "index.js"), "export const root = true;\n", "utf8");
    fs.writeFileSync(
  ...
  it("loads bundled plugins with plugin-sdk imports from a package dist root", () => {
    ...
    fs.writeFileSync(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "daisyclaw", version: "2026.4.22", type: "module" }),
      "utf-8",
    );
    fs.writeFileSync(
    ...
    ensureDaisyClawPluginSdkAlias(path.join(packageRoot, "dist"));
    fs.writeFileSync(
    ...
    fs.writeFileSync(
      path.join(pluginRoot, "daisyclaw.plugin.json"),
      JSON.stringify(
        {
          id: "discord",
  ...
  it.each([
    {
      ...
      assert: (registry: ReturnType<typeof loadDaisyClawPlugins>) => {
...
module.exports = { id: "manifest-surfaces-plugin", register() { throw new Error("manifest-only snapshot should not register"); } };`,
        ...
        fs.writeFileSync(
          path.join(plugin.dir, "daisyclaw.plugin.json"),
          JSON.stringify(
            {
              id: "manifest-surfaces-plugin",
              configSchema: EMPTY_PLUGIN_SCHEMA,
  ...
  register() {},
        ...
        fs.writeFileSync(
          path.join(memoryPlugin.dir, "daisyclaw.plugin.json"),
          JSON.stringify(
            {
              id: "memory-demo",
              kind: "memory",
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const importMarker = "__daisyclaw_loader_import_throw_marker";
...
module.exports = { id: "throws-after-import", register() {} };`,
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const marker = "__daisyclaw_loader_reentry_error";
        const reenterFnMarker = "__daisyclaw_loader_reentry_fn";
        Reflect.deleteProperty(globalThis, marker);
        Reflect.set(
          globalThis,
          reenterFnMarker,
          (options: Parameters<typeof loadDaisyClawPlugins>[0]) => loadDaisyClawPlugins(options),
        );
        const pluginDir = makeTempDir();
        ...
        } satisfies Parameters<typeof loadDaisyClawPlugins>[0];
        writePlugin({
  ...
  register() {
    try {
      ...
      throw error;
        ...
        const registry = loadDaisyClawPlugins(nestedOptions);
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const marker = "__daisyclaw_runtime_registry_reentry_marker";
        const resolverMarker = "__daisyclaw_runtime_registry_reentry_fn";
        Reflect.deleteProperty(globalThis, marker);
        Reflect.set(
          globalThis,
          resolverMarker,
  ...
  register() {
    ...
    },
    {
      ...
      run: () => {
        ...
        const full = loadDaisyClawPlugins(options);
        const scoped = loadDaisyClawPlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
        const scopedAgain = loadDaisyClawPlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
  ...
  it("skips discovery and manifest registry loading entirely when onlyPluginIds is an explicit empty array", async () => {
    ...
    const discoverySpy = vi.spyOn(discovery, "discoverDaisyClawPlugins");
  ...
  it("only publishes plugin commands to the global registry during activating loads", () => {
    ...
    const active = loadDaisyClawPlugins({
      cache: false,
      workspaceDir: plugin.dir,
      config: {
        plugins: {
  ...
  it("replaces prior plugin hook registrations on activating reloads", async () => {
    ...
    loadDaisyClawPlugins(loadOptions);
    loadDaisyClawPlugins(loadOptions);
  ...
  it("can scope bundled provider loads without hanging", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@daisyclaw/scoped-provider",
        daisyclaw: { extensions: ["./index.cjs"] },
      }),
      "utf-8",
    );
    const plugin = writePlugin({
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@daisyclaw/unscoped-provider",
        daisyclaw: { extensions: ["./index.cjs"] },
      }),
      "utf-8",
    );
  ...
  it("restores cached memory capability public artifacts on cache hits", async () => {
    ...
    const first = loadDaisyClawPlugins(options);
    ...
    const second = loadDaisyClawPlugins(options);
  ...
  it("uses discovery registration mode for non-activating loads", () => {
    useNoBundledPlugins();
    const marker = "__daisyclawDiscoveryModeTest";
    const plugin = writePlugin({
    ...
    const snapshot = loadDaisyClawPlugins({
      activate: false,
      cache: false,
      workspaceDir: plugin.dir,
      config,
  ...
  it("caches non-activating snapshots without restoring global side effects", () => {
    useNoBundledPlugins();
    clearPluginCommands();
    const marker = "__daisyclawSnapshotCacheRegisterCount";
    const plugin = writePlugin({
      id: "snapshot-cache",
  ...
  it("does not re-register non-bundled plugins after gateway-bindable boot loads", () => {
    useNoBundledPlugins();
    const marker = "__daisyclawGatewayBootRegisterCount";
    const plugin = writePlugin({
  ...
  it("reuses a gateway-bindable cache entry for later default-mode loads", () => {
    useNoBundledPlugins();
    const marker = "__daisyclawGatewayBindableCacheRegisterCount";
    const plugin = writePlugin({
    ...
    const gatewayBindable = loadDaisyClawPlugins({
      ...options,
      runtimeOptions: {
        allowGatewaySubagentBinding: true,
      },
    });
    const defaultMode = loadDaisyClawPlugins(options);
  ...
  it("preserves the gateway-bindable hook runner across later default-mode activating loads", () => {
    ...
    const gatewayRegistry = loadDaisyClawPlugins({
    ...
    const defaultRegistry = loadDaisyClawPlugins({
        ...
        return {
          ...
          loadFirst: () =>
            loadDaisyClawPlugins({
              ...
              env: {
                ...process.env,
                DAISYCLAW_BUNDLED_PLUGINS_DIR: bundledA,
              },
            }),
          loadSecond: () =>
            loadDaisyClawPlugins({
              ...
              env: {
                ...process.env,
                DAISYCLAW_BUNDLED_PLUGINS_DIR: bundledB,
              },
            }),
        };
      },
          ...
          loadFirst: () =>
            loadDaisyClawPlugins({
              ...
              env: {
                ...
                DAISYCLAW_STATE_DIR: stateDir,
                DAISYCLAW_BUNDLED_PLUGINS_DIR: bundledDir,
              },
            }),
          loadSecond: () =>
            loadDaisyClawPlugins({
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const ignoredHome = makeTempDir();
        const stateDir = makeTempDir();
        const pluginDir = path.join(daisyclawHome, "plugins", "tracked-install-cache");
        mkdirSafe(pluginDir);
        const plugin = writePlugin({
    ...
    },
    {
      ...
      setup: () => {
        ...
        return {
          loadFirst: () => loadDaisyClawPlugins(options),
          loadVariant: () =>
            loadDaisyClawPlugins({
              ...options,
  ...
  it("normalizes bundled plugin env overrides against the provided env", () => {
    ...
    const registry = loadDaisyClawPlugins({
      env: {
        ...
        HOME: homeDir,
        DAISYCLAW_HOME: undefined,
        DAISYCLAW_BUNDLED_PLUGINS_DIR: override,
      },
      config: {
        plugins: {
          allow: ["tilde-bundled"],
  ...
  it("prefers DAISYCLAW_HOME over HOME for env-expanded load paths", () => {
    const ignoredHome = makeTempDir();
    const daisyclawHome = makeTempDir();
    const stateDir = makeTempDir();
    const bundledDir = makeTempDir();
    const plugin = writePlugin({
      id: "daisyclaw-home-demo",
      dir: path.join(daisyclawHome, "plugins", "daisyclaw-home-demo"),
      filename: "index.cjs",
      body: `module.exports = { id: "daisyclaw-home-demo", register() {} };`,
    });
    ...
    const registry = loadDaisyClawPlugins({
      ...
      config: {
        plugins: {
          allow: ["daisyclaw-home-demo"],
          entries: {
            "daisyclaw-home-demo": { enabled: true },
          },
          load: {
            paths: ["~/plugins/daisyclaw-home-demo"],
          },
        },
      },
    });
    ...
    expect(
      fs.realpathSync(
        registry.plugins.find((entry) => entry.id === "daisyclaw-home-demo")?.source ?? "",
      ),
    ).toBe(fs.realpathSync(plugin.file));
  });
  ...
  it("can include plugin export shape when register is missing", () => {
    ...
    const registry = withEnv({ DAISYCLAW_PLUGIN_LOAD_DEBUG: "1" }, () =>
      loadRegistryFromSinglePlugin({
        plugin,
        pluginConfig: {
  ...
  api.registerHook("gateway:startup", () => {}, { name: "shared-hook" });
        ...
        selectCount: (registry: ReturnType<typeof loadDaisyClawPlugins>) =>
  ...
  api.registerGatewayDiscoveryService({ id: "shared-discovery", advertise() {} });
        ...
        assertPrimaryOwner: (registry: ReturnType<typeof loadDaisyClawPlugins>) => {
          expect(
  ...
  it("loads bundled channel entries through nested default export wrappers", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@daisyclaw/nested-default-channel",
          daisyclaw: {
            extensions: ["./index.cjs"],
          },
        },
  ...
  it("does not treat manifest channel ids as scoped plugin id matches", () => {
    ...
    fs.writeFileSync(
      path.join(unrelated.dir, "daisyclaw.plugin.json"),
      JSON.stringify(
        {
          id: "unrelated-plugin",
          configSchema: EMPTY_PLUGIN_SCHEMA,
...
module.exports = {
  ...
  register(api) {
    ...
    const broadSetupRegistry = loadDaisyClawPlugins({
      cache: false,
      config,
    ...
    const scopedSetupRegistry = loadDaisyClawPlugins({
      cache: false,
      config,
      includeSetupOnlyChannelPlugins: true,
...
module.exports = {
  ...
  register(api) {
    ...
    fs.writeFileSync(
      path.join(workspacePluginDir, "daisyclaw.plugin.json"),
      JSON.stringify(
        {
          id: "workspace-shadow",
          configSchema: EMPTY_PLUGIN_SCHEMA,
...
module.exports = {
  ...
  register(api) {
    api.registerChannel({
      ...
      fs.writeFileSync(
        path.join(globalDir, "daisyclaw.plugin.json"),
        JSON.stringify(
          {
            id: "untrusted-global-channel",
            configSchema: EMPTY_PLUGIN_SCHEMA,
      ...
      fs.writeFileSync(
        ...
        JSON.stringify(
          {
            name: "@daisyclaw/untrusted-global-channel",
            version: "0.0.0-test",
            main: "./index.cjs",
            daisyclaw: {
              extensions: ["./index.cjs"],
  ...
  register(api) {
    api.registerChannel({
      ...
      fs.writeFileSync(
        ...
        JSON.stringify(
          {
            name: "@daisyclaw/trusted-global-channel",
            version: "0.0.0-test",
            main: "./index.cjs",
            daisyclaw: {
              extensions: ["./index.cjs"],
...
module.exports = {
  ...
  it.each([
    {
      ...
      fixture: {
        ...
        packageName: "@daisyclaw/setup-entry-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@daisyclaw/setup-only-bundled-contract-test",
        fullBlurb: "full entry should not run in setup-only mode",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@daisyclaw/setup-runtime-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@daisyclaw/setup-runtime-bundled-contract-test",
        fullBlurb: "full entry should not run while unconfigured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@daisyclaw/setup-runtime-bundled-contract-secrets-test",
        fullBlurb: "full entry should not run while unconfigured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@daisyclaw/setup-runtime-bundled-contract-runtime-test",
        fullBlurb: "full entry should not run while unconfigured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@daisyclaw/setup-runtime-bundled-route-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@daisyclaw/setup-runtime-bundled-runtime-merge-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@daisyclaw/setup-runtime-external-deferred-test",
        fullBlurb: "full entry should defer while configured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@daisyclaw/setup-runtime-not-preferred-test",
  ...
  it("applies the bundled runtime setter before loading the merged setup-runtime plugin", () => {
    ...
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@daisyclaw/setup-runtime-order-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledFullEntryContract: true,
  ...
  it("records setup runtime setter failures without aborting the full load pass", () => {
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@daisyclaw/setup-runtime-error-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledSetupEntryContract: true,
  ...
  it("rolls back setup-runtime registrations when setup side effects fail", () => {
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@daisyclaw/setup-runtime-route-error-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime route",
      configured: true,
      startupDeferConfiguredChannelFullLoadUntilAfterListen: true,
  ...
  it("closes setup-runtime registration APIs after synchronous registration", async () => {
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@daisyclaw/setup-runtime-late-route-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime route",
      configured: true,
      startupDeferConfiguredChannelFullLoadUntilAfterListen: true,
  ...
  it("rejects mismatched bundled runtime entry ids before applying setup-runtime setters", () => {
    ...
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@daisyclaw/setup-runtime-mismatch-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledFullEntryContract: true,
  ...
  it("rejects mismatched bundled setup export ids before loading setup-runtime entry code", () => {
    ...
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@daisyclaw/setup-export-mismatch-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledFullEntryContract: true,
  ...
  it("isolates loadSetupPlugin errors as per-plugin diagnostics instead of crashing registry load", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@daisyclaw/setup-entry-throws-test",
          daisyclaw: {
            extensions: ["./index.cjs"],
            setupEntry: "./setup-entry.cjs",
          },
  ...
  it("keeps healthy sibling channel plugins loadable when a setup entry throws", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@daisyclaw/setup-entry-throws-sibling-test",
          daisyclaw: {
            extensions: ["./index.cjs"],
            setupEntry: "./setup-entry.cjs",
          },
    ...
    fs.writeFileSync(
      path.join(brokenDir, "daisyclaw.plugin.json"),
      JSON.stringify(
        {
  ...
  it("records a diagnostic when registerChannel throws in the setup-entry path", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@daisyclaw/register-channel-throws-test",
          daisyclaw: {
            extensions: ["./index.cjs"],
            setupEntry: "./setup-entry.cjs",
          },
...
module.exports = {
  ...
  it("prefers built bundled plugin artifacts over source TS when requested", () => {
    ...
    fs.writeFileSync(
      path.join(sourceDir, "daisyclaw.plugin.json"),
      JSON.stringify(
        {
    ...
    const registry = withEnv(
      {
        DAISYCLAW_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
        DAISYCLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR: "1",
        DAISYCLAW_DISABLE_BUNDLED_PLUGINS: undefined,
      },
      () =>
        loadDaisyClawPlugins({
          cache: false,
    ...
    const scenarios = [
      {
        ...
        loadRegistry: () => {
          ...
          return withEnv(
            {
              DAISYCLAW_DISABLE_BUNDLED_PLUGINS: "1",
              DAISYCLAW_BUNDLED_PLUGINS_DIR: undefined,
            },
            () =>
              loadDaisyClawPlugins({
                cache: false,
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryADir, "daisyclaw.plugin.json"),
            JSON.stringify(
              {
                id: "memory-a",
                kind: "memory",
          ...
          fs.writeFileSync(
            path.join(memoryBDir, "daisyclaw.plugin.json"),
            JSON.stringify(
              {
                id: "memory-b",
                kind: "memory",
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryCoreDir, "daisyclaw.plugin.json"),
            JSON.stringify(
          ...
          fs.writeFileSync(
            path.join(memoryLanceDir, "daisyclaw.plugin.json"),
            JSON.stringify(
  ...
  it("resolves duplicate plugin ids by source precedence", () => {
    const scenarios = [
      ...
      },
      {
        ...
        bundledFilename: "index.cjs",
        loadRegistry: () => {
          const devSourceRoot = makeDaisyClawDevSourceRoot();
          ...
          process.env.DAISYCLAW_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
          return withEnv({ DAISYCLAW_DEV_SOURCE_ROOT: devSourceRoot }, () =>
            withStateDir((stateDir) => {
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          return withStateDir((stateDir) => {
            const globalDir = path.join(stateDir, "node_modules", "@daisyclaw", "memory-lancedb");
            mkdirSafe(globalDir);
              ...
              config: {
                plugins: {
                  ...
                  installs: {
                    "memory-lancedb": {
                      source: "npm",
                      spec: "@daisyclaw/memory-lancedb",
                      resolvedName: "@daisyclaw/memory-lancedb",
  ...
  it("evaluates load-path provenance warnings", () => {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          const stateDir = makeTempDir();
          return withEnv({ DAISYCLAW_STATE_DIR: stateDir }, () => {
            const globalDir = path.join(stateDir, "extensions", "rogue");
            mkdirSafe(globalDir);
            writePlugin({
              id: "rogue",
  ...
  it("uses the source runtime snapshot allowlist for plugin trust checks", () => {
    useNoBundledPlugins();
    const stateDir = makeTempDir();
    withEnv({ DAISYCLAW_STATE_DIR: stateDir }, () => {
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("daisyclaw/plugin-sdk").emptyPluginConfigSchema)(),
        register() {},
      };`,
    });

    const registry = withEnv({ DAISYCLAW_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins" }, () =>
      loadDaisyClawPlugins({
        cache: false,
        workspaceDir: plugin.dir,
        config: {
  ...
  it("supports legacy plugins subscribing to diagnostic events from the root sdk", async () => {
    useNoBundledPlugins();
    const seenKey = "__daisyclawLegacyRootDiagnosticSeen";
  ...
  configSchema: (require("daisyclaw/plugin-sdk").emptyPluginConfigSchema)(),
  register() {
    const { onDiagnosticEvent } = require("daisyclaw/plugin-sdk");
