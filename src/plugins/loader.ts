...
import { installDaisyClawPluginSdkNativeResolver } from "./plugin-sdk-native-resolver.js";
...
function createGuardedPluginRegistrationApi(api: DaisyClawPluginApi): {
...
function runPluginRegisterSync(
  ...
  api: Parameters<NonNullable<DaisyClawPluginDefinition["register"]>>[0],
): void {
...
function resolvePluginLoadCacheContext(options: PluginLoadOptions = {}) {
  ...
  const devSourceRoot = resolveDaisyClawDevSourceRoot(env);
...
export function resolveRuntimePluginRegistry(
  ...
  // plugin registration is still in flight. Let direct loadDaisyClawPlugins(...)
  // callers surface the hard error instead.
  if (isPluginRegistryLoadInFlight(options)) {
    return undefined;
  }
  return loadDaisyClawPlugins(options);
}
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: DaisyClawPluginDefinition;
  register?: DaisyClawPluginDefinition["register"];
} {
  ...
  for (let index = 0; index < candidates.length && index < 12; index += 1) {
    ...
    if (typeof resolved === "function") {
      return {
        register: resolved as DaisyClawPluginDefinition["register"],
      };
    }
    if (resolved && typeof resolved === "object") {
      const def = resolved as DaisyClawPluginDefinition;
...
export function loadDaisyClawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  try {
    ...
    for (const candidate of orderedCandidates) {
      ...
      if (record.format === "bundle") {
        ...
        for (const capability of unsupportedCapabilities) {
          registry.diagnostics.push({
            ...
            pluginId: record.id,
            source: record.source,
            message: `bundle capability detected but not wired into DaisyClaw yet: ${capability}`,
          });
        }
        if (
          enableState.enabled &&
      ...
      let mod: DaisyClawPluginModule | null = null;
      let moduleLoadMs = 0;
      ...
      try {
        ...
        mod = withProfile(
          ...
          registrationMode,
          () => loadPluginModule(safeSource) as DaisyClawPluginModule,
        );
      } catch (err) {
        recordPluginError({
          logger,
      ...
      if (registrationPlan.loadSetupEntry && manifestRecord.setupSource) {
        ...
        if (setupRegistration.plugin) {
          ...
          if (
            ...
            let runtimeMod: DaisyClawPluginModule | null = null;
            try {
              runtimeMod = withProfile(
                { pluginId: record.id, source: safeRuntimeSource },
                "load-setup-runtime-entry",
                () => loadPluginModule(safeRuntimeSource) as DaisyClawPluginModule,
              );
            } catch (err) {
              recordPluginError({
                logger,
      ...
      if (failedPlugins.length > 0) {
        logger.warn(
          `[plugins] ${failedPlugins.length} plugin(s) failed to initialize (${formatPluginFailureSummary(
            failedPlugins,
          )}). Run 'daisyclaw plugins inspect <id> --runtime --json' for runtime diagnostics, 'daisyclaw plugins list' for registry state, and restart the Gateway after plugin code or load-path changes.`,
        );
      }
    }
...
export async function loadDaisyClawPluginCliRegistry(
  options: PluginLoadOptions = {},
  ...
  for (const candidate of orderedCandidates) {
    ...
    let mod: DaisyClawPluginModule | null;
    try {
      mod = withProfile(
        { pluginId: record.id, source: safeSource },
        "cli-metadata",
