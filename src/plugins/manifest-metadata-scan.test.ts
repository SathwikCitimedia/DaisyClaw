...
function createTempRoot(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-manifest-metadata-"));
  tempRoots.push(root);
  return root;
}
...
describe("listDaisyClawPluginManifestMetadata", () => {
  afterEach(() => {
  ...
  it("prefers the active bundled manifest over stale persisted bundled installs", () => {
    ...
    writeJson(path.join(bundledRoot, "openai", "daisyclaw.plugin.json"), {
      id: "openai",
      providerEndpoints: [{ endpointClass: "openai-public", hosts: ["api.openai.com"] }],
    });
    writeJson(path.join(staleBundledRoot, "openai", "daisyclaw.plugin.json"), {
      id: "openai",
      providers: ["openai"],
    });
    writePersistedInstalledPluginIndexSync(
      {
        ...
        plugins: [
          {
            pluginId: "openai",
            manifestPath: path.join(staleBundledRoot, "openai", "daisyclaw.plugin.json"),
    ...
    const records = listDaisyClawPluginManifestMetadata({
      DAISYCLAW_HOME: home,
