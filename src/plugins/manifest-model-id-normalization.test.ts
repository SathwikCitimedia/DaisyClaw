...
import { listDaisyClawPluginManifestMetadata } from "./manifest-metadata-scan.js";
...
const ORIGINAL_ENV = {
  ...
  DAISYCLAW_DISABLE_BUNDLED_PLUGINS: process.env.DAISYCLAW_DISABLE_BUNDLED_PLUGINS,
  DAISYCLAW_BUNDLED_PLUGINS_DIR: process.env.DAISYCLAW_BUNDLED_PLUGINS_DIR,
} as const;

const tempDirs: string[] = [];

function restoreDaisyClawStateDirEnv(): void {
  const value = ORIGINAL_ENV.DAISYCLAW_STATE_DIR;
  if (value === undefined) {
    delete process.env.DAISYCLAW_STATE_DIR;
  } else {
    process.env.DAISYCLAW_STATE_DIR = value;
  }
}

function restoreDaisyClawHomeEnv(): void {
  const value = ORIGINAL_ENV.DAISYCLAW_HOME;
  if (value === undefined) {
    delete process.env.DAISYCLAW_HOME;
  } else {
    process.env.DAISYCLAW_HOME = value;
  }
}

function restoreDaisyClawDisableBundledPluginsEnv(): void {
  const value = ORIGINAL_ENV.DAISYCLAW_DISABLE_BUNDLED_PLUGINS;
  if (value === undefined) {
    delete process.env.DAISYCLAW_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.DAISYCLAW_DISABLE_BUNDLED_PLUGINS = value;
  }
}

function restoreDaisyClawBundledPluginsDirEnv(): void {
  const value = ORIGINAL_ENV.DAISYCLAW_BUNDLED_PLUGINS_DIR;
  if (value === undefined) {
    delete process.env.DAISYCLAW_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.DAISYCLAW_BUNDLED_PLUGINS_DIR = value;
  }
}

function restoreEnv(): void {
  restoreDaisyClawStateDirEnv();
  restoreDaisyClawHomeEnv();
  restoreDaisyClawDisableBundledPluginsEnv();
  restoreDaisyClawBundledPluginsDirEnv();
}

function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-model-id-normalization-"));
  tempDirs.push(dir);
  return dir;
}
  ...
  writePersistedInstalledPluginIndexSync(
    {
      ...
      plugins: [
        {
          pluginId: "normalizer",
          manifestPath: path.join(params.pluginDir, "daisyclaw.plugin.json"),
...
function createCurrentSnapshot(params: {
  ...
  const index: InstalledPluginIndex = {
    ...
    plugins: [
      {
        pluginId: "normalizer",
        manifestPath: `/tmp/normalizer-${params.manifestHash}/daisyclaw.plugin.json`,
...
describe("manifest model id normalization", () => {
  ...
  it("reuses current metadata when callers omit config", () => {
    const config: DaisyClawConfig = { plugins: { allow: ["normalizer"] } };
    setCurrentPluginMetadataSnapshot(
      createCurrentSnapshot({
        manifestHash: "alpha",
        prefix: "alpha",
  ...
  it("keeps process metadata stable across manifest edits and reflects state-dir changes", () => {
    ...
    process.env.DAISYCLAW_STATE_DIR = stateDirA;
    process.env.DAISYCLAW_HOME = undefined;
    process.env.DAISYCLAW_DISABLE_BUNDLED_PLUGINS = "1";
    process.env.DAISYCLAW_BUNDLED_PLUGINS_DIR = undefined;
    ...
    process.env.DAISYCLAW_STATE_DIR = stateDirB;
    clearPluginMetadataLifecycleCaches();
  ...
  it("reuses manifest metadata while file fingerprints are unchanged", () => {
    ...
    expect(listDaisyClawPluginManifestMetadata(process.env)).toHaveLength(1);
    expect(listDaisyClawPluginManifestMetadata(process.env)).toHaveLength(1);
