...
import type { DaisyClawPackageManifest } from "./manifest.js";
...
function makeTempDir() {
  return makeTrackedTempDir("daisyclaw-manifest-registry", tempDirs);
}
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "daisyclaw.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function createPluginCandidate(params: {
  ...
  format?: "daisyclaw" | "bundle";
  bundleFormat?: "codex" | "claude" | "cursor";
  packageName?: string;
  packageVersion?: string;
  packageManifest?: DaisyClawPackageManifest;
  packageDir?: string;
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "daisyclaw.plugin.json");
  const linkedManifest = path.join(rootDir, "daisyclaw.plugin.json");
  ...
  return loadPluginManifestRegistry({
    ...
    candidates: [
      createPluginCandidate({
        ...
        packageManifest: {
          install: {
            npmSpec: "@daisyclaw/synology-chat",
...
describe("loadPluginManifestRegistry", () => {
  beforeAll(() => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@daisyclaw/cached-manifest",
  ...
  it("prefers dev source bundled plugins over installed globals with the same id", () => {
    ...
    const registry = loadPluginManifestRegistry({
      env: hermeticEnv({ DAISYCLAW_DEV_SOURCE_ROOT: devSourceRoot }),
      installRecords: {
        codex: {
          source: "npm",
          installPath: globalDir,
    ...
    const registry = loadPluginManifestRegistry({
      installRecords: {
        "diagnostics-prometheus": {
          source: "npm",
          installPath: dir,
          resolvedName: "@daisyclaw/diagnostics-prometheus",
      ...
      candidates: [
        createPluginCandidate({
          ...
          rootDir: dir,
          packageName: "@daisyclaw/diagnostics-prometheus",
          origin: "global",
        }),
      ],
    });
  ...
  it("reports non-bundled providerAuthEnvVars as deprecated compat metadata", () => {
    ...
    expectDiagnosticFields(registry, {
      ...
      source: path.join(dir, "daisyclaw.plugin.json"),
  ...
  it("hydrates supplemental official external catalog contracts for lagging npm manifests", () => {
    ...
    const registry = loadRegistry([
      createPluginCandidate({
        idHint: "wecom-daisyclaw-plugin",
        rootDir: dir,
        origin: "global",
        packageName: "@wecom/wecom-daisyclaw-plugin",
      }),
    ]);
  ...
  it("ignores provider catalog entries outside the plugin root", () => {
    ...
    expectDiagnosticFields(registry, {
      ...
      source: path.join(pluginDir, "daisyclaw.plugin.json"),
  ...
  it("preserves host-trusted plugin contracts from catalog overlays", () => {
    const contracts = manifestRegistryTesting.mergeManifestContracts(
      {
        agentToolResultMiddleware: ["daisyclaw"],
      },
      {
  ...
  it("hydrates bundled channel config metadata onto manifest records", () => {
    ...
    const registry = loadRegistry([
      createPluginCandidate({
        ...
        rootDir: dir,
        origin: "bundled",
        bundledManifestPath: path.join(dir, "daisyclaw.plugin.json"),
        bundledManifest: {
          id: "telegram",
          configSchema: { type: "object" },
          channels: ["telegram"],
  ...
  it.each([
    {
      ...
      env: { DAISYCLAW_VERSION: "2026.3.21" } as NodeJS.ProcessEnv,
      expectedMessage: "plugin requires DaisyClaw >=2026.3.22, but this host is 2026.3.21",
      expectWarn: true,
    },
    {
      name: "skips plugins whose beta minHostVersion is newer than the current host",
      minHostVersion: ">=2026.5.1-beta.1",
      env: { DAISYCLAW_VERSION: "2026.4.30" } as NodeJS.ProcessEnv,
      expectedMessage: "plugin requires DaisyClaw >=2026.5.1-beta.1, but this host is 2026.4.30",
      expectWarn: true,
    },
    {
      name: "rejects invalid minHostVersion metadata",
      minHostVersion: "2026.3.22",
      expectedMessage: "plugin manifest invalid | daisyclaw.install.minHostVersion must use",
      expectWarn: false,
    },
    {
      name: "warns distinctly when host version cannot be determined",
      minHostVersion: ">=2026.3.22",
      env: { DAISYCLAW_VERSION: "unknown" } as NodeJS.ProcessEnv,
  ...
  it("accepts legacy bare minHostVersion metadata for recorded installed globals", () => {
    ...
    expectNoRegistryDiagnosticContains(registry, "daisyclaw.install.minHostVersion must use");
  });
  ...
  it("does not runtime-gate bundled source plugins by install minHostVersion", () => {
    ...
    expectNoRegistryDiagnosticContains(registry, "requires DaisyClaw");
  });
  ...
  it("skips installed plugins whose package plugin API range is newer than the current host", () => {
    ...
    const registry = loadRegistryForPluginApiCase({
      rootDir: dir,
      pluginApi: ">=2026.5.27",
      env: { DAISYCLAW_VERSION: "2026.5.10-beta.1" } as NodeJS.ProcessEnv,
    });
  ...
  it("skips installed plugins whose package plugin API metadata is malformed", () => {
    ...
    const registry = loadRegistryForPluginApiCase({
      rootDir: dir,
      pluginApi: 20260527,
      env: { DAISYCLAW_VERSION: "2026.5.27" } as NodeJS.ProcessEnv,
    });

    expect(registry.plugins).toStrictEqual([]);
    expectRegistryDiagnosticContains(
      registry,
      "plugin manifest invalid | package.json daisyclaw.compat.pluginApi must be a string",
    );
  ...
  it("loads installed plugins when a beta host is on the package plugin API floor", () => {
    ...
    const registry = loadRegistryForPluginApiCase({
      rootDir: dir,
      pluginApi: ">=2026.5.27",
      env: { DAISYCLAW_VERSION: "2026.5.27-beta.1" } as NodeJS.ProcessEnv,
    });
  ...
  it("suppresses duplicate warning when global candidates come from the same package artifact", () => {
    ...
    const manifest = { id: "opik-daisyclaw", configSchema: { type: "object" } };
    writeManifest(firstDir, manifest);
    writeManifest(secondDir, manifest);

    const candidates: PluginCandidate[] = [
      createPluginCandidate({
        idHint: "opik-daisyclaw",
        rootDir: firstDir,
        origin: "global",
  ...
  it("still rejects config manifest hardlinks outside the Nix store in Nix mode", () => {
    ...
    const registry = loadPluginManifestRegistry({
      env: hermeticEnv({ DAISYCLAW_NIX_MODE: "1" }),
      candidates: [
        createPluginCandidate({
  ...
  it("resolves load-path manifests from the current env home", () => {
    ...
    const first = loadPluginManifestRegistry({
      ...
      env: hermeticEnv({
        ...
        DAISYCLAW_STATE_DIR: path.join(homeA, ".state"),
      }),
    });
    const second = loadPluginManifestRegistry({
      ...
      env: hermeticEnv({
        HOME: homeB,
        DAISYCLAW_HOME: undefined,
        DAISYCLAW_STATE_DIR: path.join(homeB, ".state"),
      }),
    });
  ...
  it("resolves manifests against the current host version", () => {
    ...
    const olderHost = loadPluginManifestRegistry({
      candidates,
      env: hermeticEnv({
        DAISYCLAW_VERSION: "2026.3.21",
      }),
    });
    const newerHost = loadPluginManifestRegistry({
      candidates,
      env: hermeticEnv({
        DAISYCLAW_VERSION: "2026.3.22",
      }),
    });
