/** Loads and normalizes DaisyClaw plugin manifests, including contracts and config schemas. */
import fs from "node:fs";
import path from "node:path";
import { normalizeModelCatalog } from "@daisyclaw/model-catalog-core/model-catalog-normalize";
import { normalizeModelCatalogProviderId } from "@daisyclaw/model-catalog-core/model-catalog-refs";
import type {
  ModelCatalog,
  ModelCatalogAlias,
  ModelCatalogCost,
...
export const PLUGIN_MANIFEST_FILENAME = "daisyclaw.plugin.json";
...
export type PluginManifestQaRunner = {
  /** Subcommand mounted beneath `daisyclaw qa`, for example `matrix`. */
  commandName: string;
...
// package.json "daisyclaw" metadata (used for setup/catalog)
...
export type DaisyClawPackageStartup = {
  /**
...
export type DaisyClawPackageSetupFeatures = {
  configPromotion?: boolean;
  legacyStateMigrations?: boolean;
  legacySessionSurfaces?: boolean;
};

export type DaisyClawPackageCompat = {
  pluginApi?: string;
};

export type DaisyClawPackageManifest = {
  extensions?: string[];
  runtimeExtensions?: string[];
  setupEntry?: string;
  runtimeSetupEntry?: string;
  setupFeatures?: DaisyClawPackageSetupFeatures;
  plugin?: {
    id?: string;
    label?: string;
  };
  channel?: PluginPackageChannel;
  compat?: DaisyClawPackageCompat;
  install?: PluginPackageInstall;
  startup?: DaisyClawPackageStartup;
};
...
} & Partial<Record<ManifestKey, DaisyClawPackageManifest>>;

export function getPackageManifestMetadata(
  manifest: PackageManifest | undefined,
): DaisyClawPackageManifest | undefined {
  if (!manifest) {
    return undefined;
  }
...
export function resolvePackageExtensionEntries(
  ...
  const rawDaisyClaw = manifest?.[MANIFEST_KEY] as unknown;
  if (rawDaisyClaw === undefined || rawDaisyClaw === null) {
    return { status: "missing", entries: [] };
  }
  if (!isRecord(rawDaisyClaw)) {
    return {
      status: "invalid",
      entries: [],
      error: "package.json daisyclaw must be an object",
    };
  }
  const raw = rawDaisyClaw.extensions;
  ...
  if (!Array.isArray(raw)) {
    return {
      status: "invalid",
      entries: [],
      error: "package.json daisyclaw.extensions must be an array",
    };
  }
  ...
  for (const [index, entry] of raw.entries()) {
    ...
    if (!normalized) {
      return {
        status: "invalid",
        entries: [],
        error: `package.json daisyclaw.extensions[${index}] must be a non-empty string`,
      };
    }
