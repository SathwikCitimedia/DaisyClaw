...
async function listMarketplaceDownloadTempDirs(): Promise<string[]> {
  ...
  return entries
    .filter(
      (entry) => entry.isDirectory() && entry.name.startsWith("daisyclaw-marketplace-download-"),
    )
...
describe("marketplace plugins", () => {
  ...
  it("lists plugins from a local marketplace root", async () => {
    await withTempDir("daisyclaw-marketplace-test-", async (rootDir) => {
  ...
  it("resolves Claude-style plugin@marketplace shortcuts from known_marketplaces.json", async () => {
    await withTempDir("daisyclaw-marketplace-test-", async (homeDir) => {
      const daisyClawHome = path.join(homeDir, "daisyclaw-home");
      await fs.mkdir(path.join(homeDir, ".claude", "plugins"), { recursive: true });
      await fs.mkdir(daisyClawHome, { recursive: true });
      await fs.writeFile(
      ...
      const shortcut = await withEnvAsync(
        { HOME: homeDir, DAISYCLAW_HOME: daisyClawHome },
