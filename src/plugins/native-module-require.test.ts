...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-native-require-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("tryNativeRequireJavaScriptModule", () => {
  ...
  it("declines missing dependency errors when source-transform fallback is available", () => {
    const dir = makeTempDir();
    const modulePath = path.join(dir, "plugin.cjs");
    fs.writeFileSync(modulePath, 'require("daisyclaw/plugin-sdk");\n', "utf8");
  ...
  beforeAll(() => {
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "  allowWindows: true,",
        `  aliasMap: { "daisyclaw/plugin-sdk/channel-outbound": ${JSON.stringify(sdkPath)} },`,
        "});",
        "if (!result.ok) {",
