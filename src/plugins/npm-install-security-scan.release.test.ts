...
const REQUIRED_REVIEWED_PUBLISHABLE_CRITICAL_FINDINGS = new Set([
  "@daisyclaw/acpx:dangerous-exec:src/codex-auth-bridge.ts",
  "@daisyclaw/acpx:dangerous-exec:src/runtime-internals/mcp-proxy.mjs",
  "@daisyclaw/codex:dangerous-exec:src/app-server/sandbox-exec-server/http.ts",
  "@daisyclaw/codex:dangerous-exec:src/app-server/sandbox-exec-server/processes.ts",
  "@daisyclaw/codex:dangerous-exec:src/app-server/transport-stdio.ts",
  "@daisyclaw/codex:dangerous-exec:src/node-cli-sessions.ts",
  "@daisyclaw/discord:dangerous-exec:src/voice/audio.ts",
  "@daisyclaw/google-meet:dangerous-exec:src/node-host.ts",
  "@daisyclaw/google-meet:dangerous-exec:src/realtime.ts",
  "@daisyclaw/matrix:dangerous-exec:src/matrix/deps.ts",
  "@daisyclaw/voice-call:dangerous-exec:src/tunnel.ts",
  "@daisyclaw/voice-call:dangerous-exec:src/webhook/tailscale.ts",
]);

const OPTIONAL_REVIEWED_PUBLISHABLE_DIST_CRITICAL_FINDINGS = new Set([
  "@daisyclaw/acpx:dangerous-exec:dist/mcp-proxy.mjs",
  "@daisyclaw/acpx:dangerous-exec:dist/service-<hash>.js",
  "@daisyclaw/codex:dangerous-exec:dist/client-<hash>.js",
  "@daisyclaw/google-meet:dangerous-exec:dist/index.js",
  "@daisyclaw/slack:dynamic-code-execution:dist/outbound-payload.test-harness-<hash>.js",
  "@daisyclaw/voice-call:dangerous-exec:dist/runtime-entry-<hash>.js",
]);
...
function stageScannerRelevantPackedFiles(
  ...
  const stageDir = mkdtempSync(join(tmpdir(), "daisyclaw-plugin-npm-scan-"));
...
function collectPublishablePluginPackages(): PublishablePluginPackage[] {
  return listPublishablePluginPackageDirs()
    .flatMap((packageDir) => {
      ...
      let packageJson: {
        name?: unknown;
        daisyclaw?: { release?: { publishToNpm?: unknown } };
      };
      try {
      ...
      if (packageJson.daisyclaw?.release?.publishToNpm !== true) {
        return [];
      }
