...
/** Result of reading package.json daisyclaw.compat.pluginApi metadata. */
...
export function resolvePackagePluginApiRange(
  ...
  if (!isRecord(compat)) {
    return { ok: false, error: "package.json daisyclaw.compat must be an object" };
  }
  ...
  if (typeof pluginApi !== "string") {
    return { ok: false, error: "package.json daisyclaw.compat.pluginApi must be a string" };
  }
  const range = pluginApi.trim();
  if (!range) {
    return { ok: false, error: "package.json daisyclaw.compat.pluginApi must not be empty" };
  }
