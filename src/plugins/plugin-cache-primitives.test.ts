...
describe("createConfigScopedPromiseLoader", () => {
  ...
  it("caches loads by config object", async () => {
    const firstConfig = { plugins: { load: { disabled: true } } } as DaisyClawConfig;
    const secondConfig = { plugins: { load: { disabled: false } } } as DaisyClawConfig;
    const load = vi.fn(async (config?: DaisyClawConfig) =>
  ...
  it("clears default and config-scoped entries", async () => {
    ...
    const loader = createConfigScopedPromiseLoader(
      async (owner?: DaisyClawConfig) => `${owner ? "config" : "default"}-${++calls}`,
    );
