...
export type ConfigScopedRuntimeCache<T> = WeakMap<DaisyClawConfig, Map<string, T>>;
...
export type ConfigScopedPromiseLoader<T> = {
  load(config?: DaisyClawConfig): Promise<T>;
  clear(): void;
};
...
export function createConfigScopedPromiseLoader<T>(
  load: (config?: DaisyClawConfig) => T | Promise<T>,
): ConfigScopedPromiseLoader<T> {
  let defaultPromise: Promise<T> | undefined;
  let promisesByConfig = new WeakMap<DaisyClawConfig, Promise<T>>();

  const createPromise = (config?: DaisyClawConfig): Promise<T> => {
  ...
  return {
    async load(config?: DaisyClawConfig): Promise<T> {
      if (!config) {
    ...
    clear(): void {
      defaultPromise = undefined;
      promisesByConfig = new WeakMap<DaisyClawConfig, Promise<T>>();
    },
  };
}
