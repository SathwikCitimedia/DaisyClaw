...
describe("plugin control-plane context", () => {
  it("resolves env-sensitive discovery roots and load paths before fingerprinting", () => {
    ...
    const envA = { HOME: "/home/a", DAISYCLAW_HOME: "/daisyclaw/a" } as NodeJS.ProcessEnv;
    const envB = { HOME: "/home/b", DAISYCLAW_HOME: "/daisyclaw/b" } as NodeJS.ProcessEnv;

    const contextA = resolvePluginDiscoveryContext({ config, env: envA });
    const contextB = resolvePluginDiscoveryContext({ config, env: envB });

    expect(contextA.loadPaths).toEqual(["/daisyclaw/a/plugins", "/opt/shared"]);
    expect(contextB.loadPaths).toEqual(["/daisyclaw/b/plugins", "/opt/shared"]);
  ...
  it("includes policy, inventory, and activation in one control-plane fingerprint", () => {
    ...
    const base = resolvePluginControlPlaneFingerprint({
      config,
      env: { HOME: "/home/a", DAISYCLAW_HOME: "/daisyclaw/a" } as NodeJS.ProcessEnv,
  ...
  it("keeps the canonical context inspectable for cache diagnostics", () => {
    ...
    expect(context).toStrictEqual({
      discovery: {
        ...
        roots: {
          stock: path.join(os.tmpdir(), "daisyclaw-empty-bundled-plugins"),
          global: "/daisyclaw/a/.daisyclaw/extensions",
          workspace: undefined,
        },
      },
      inventoryFingerprint: "inventory",
