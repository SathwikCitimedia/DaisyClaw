...
const fixtureRoot = makeTrackedTempDir("daisyclaw-plugin-graceful", fixtureTempDirs);
let tempDirIndex = 0;
const { loadDaisyClawPlugins, clearPluginLoaderCache } = await import("./loader.js");
...
function readPluginId(pluginPath: string): string {
  const manifestPath = path.join(path.dirname(pluginPath), "daisyclaw.plugin.json");
...
describe("graceful plugin initialization failure", () => {
  ...
  it("logs a startup summary grouped by failure phase", async () => {
    ...
    expect(summary).toContain("daisyclaw plugins inspect <id> --runtime --json");
    expect(summary).toContain("daisyclaw plugins list");
  });
});
