...
vi.mock("../channels/config-presence.js", () => ({
  ...
  listExplicitlyDisabledChannelIdsForConfig: (config: DaisyClawConfig) =>
...
function createManifestRecord(
  ...
  return {
    ...
    manifestPath: `/plugins/${plugin.id}/daisyclaw.plugin.json`,
    ...plugin,
  };
}
...
const indexDiagnostic = {
  level: "warn",
  source: "/plugins/demo/daisyclaw.plugin.json",
...
describe("loadPluginLookUpTable", () => {
  beforeEach(() => {
    ...
    listPotentialConfiguredChannelIds
      .mockReset()
      .mockImplementation((config: DaisyClawConfig) => Object.keys(config.channels ?? {}));
  ...
  it("does not reuse lookup tables when channel env presence changes startup scope", async () => {
    listPotentialConfiguredChannelIds.mockImplementation(
      (_config: DaisyClawConfig, env: NodeJS.ProcessEnv) => (env.TELEGRAM_TOKEN ? ["telegram"] : []),
    );
    const plugins = [
      createManifestRecord({
        id: "telegram",
  ...
  it("rebuilds when a provided metadata snapshot has stale plugin paths", async () => {
    ...
    const requestedPlugins = [
      createManifestRecord({
        ...
        manifestPath: "/plugins-moved/telegram/daisyclaw.plugin.json",
      }),
    ];
    const config = {
      channels: {
