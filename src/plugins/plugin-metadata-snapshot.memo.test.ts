...
import { runDaisyClawStateWriteTransaction } from "../state/daisyclaw-state-db.js";
import {
...
function tempStateDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-metadata-memo-"));
  tempDirs.push(dir);
  return dir;
}
...
function writePersistedIndex(params: {
  ...
  const manifestPath = params.manifestPath ?? path.join(pluginDir, "daisyclaw.plugin.json");
...
function writeRecoverableNpmPlugin(params: {
  ...
  writeJson(path.join(packageDir, "daisyclaw.plugin.json"), { id: params.pluginId });
}
...
function makeIndex(
  ...
  const manifestPath = options.manifestPath ?? path.join(rootDir, "daisyclaw.plugin.json");
  return {
    version: 1,
    hostContractVersion: "test",
    compatRegistryVersion: "test",
...
function makeManifestRegistry(pluginId = "demo"): PluginManifestRegistry {
  const plugin: PluginManifestRecord = {
    ...
    manifestPath: `/plugins/${pluginId}/daisyclaw.plugin.json`,
    origin: "global",
  };
...
describe("loadPluginMetadataSnapshot process memo", () => {
  ...
  it("reuses the expanded freshness fingerprint on hot cache hits", () => {
    const stateDir = tempStateDir();
    const manifestPath = path.join(stateDir, "extensions", "demo", "daisyclaw.plugin.json");
  ...
  it.each([
    ["manifest", "daisyclaw.plugin.json", "manifestPath"],
  ...
  it("does not fingerprint persisted plugin paths outside the plugin root", () => {
    const stateDir = tempStateDir();
    const outsideManifestPath = path.join(stateDir, "outside", "daisyclaw.plugin.json");
