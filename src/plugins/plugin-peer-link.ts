...
export type DaisyClawPeerLinkAuditIssue = {
  packageName: string;
  packageDir: string;
  reason: string;
};
...
type AuditManagedNpmRootResult = {
  checked: number;
  broken: number;
  issues: DaisyClawPeerLinkAuditIssue[];
};

type DaisyClawPeerLinkResult = "linked" | "skipped" | "unchanged";
...
async function auditDaisyClawPeerDependency(params: {
  hostRoot: string;
  packageDir: string;
  npmRoot?: string;
  packageName?: string;
}): Promise<DaisyClawPeerLinkAuditIssue | null> {
  const packageName =
    params.packageName ??
    (params.npmRoot
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === "ENOENT") {
      return {
        packageName,
        packageDir: params.packageDir,
        reason: `missing ${path.join(nodeModulesDir, "daisyclaw")}`,
      };
    }
    throw error;
  }

  const linkPath = path.join(nodeModulesDir, "daisyclaw");
...
export async function auditDaisyClawPeerDependencyLink(params: {
  packageDir: string;
  packageName?: string;
}): Promise<DaisyClawPeerLinkAuditIssue | null> {
  ...
  if (!hostRoot) {
    return {
      packageName,
      packageDir: params.packageDir,
      reason: "could not locate daisyclaw package root",
    };
  }
  return await auditDaisyClawPeerDependency({
    hostRoot,
    packageDir: params.packageDir,
    packageName,
  });
...
async function ensureRealNodeModulesDir(params: {
  ...
  try {
    ...
    if (!existing.isDirectory() || existing.isSymbolicLink()) {
      params.logger.warn?.(
        `Skipping daisyclaw peerDependency link because ${nodeModulesDir} is not a real directory.`,
      );
      return null;
    }
...
async function linkDaisyClawPeerDependency(params: {
  hostRoot: string;
  installedDir: string;
  peerName: string;
  logger: PluginPeerLinkLogger;
}): Promise<DaisyClawPeerLinkResult> {
  ...
  try {
    ...
    if (existing) {
      if (!existing.isSymbolicLink()) {
        if (params.peerName === "daisyclaw" && existing.isDirectory()) {
          const existingPackageName = await readPackageName(linkPath);
          if (existingPackageName === "daisyclaw") {
        ...
        params.logger.warn?.(
          `Skipping daisyclaw peerDependency link because ${linkPath} already exists and is not a symlink.`,
        );
        return "skipped";
      }
...
/**
 * Symlink the host daisyclaw package for plugins that declare it as a peer.
 ...
 */
export async function linkDaisyClawPeerDependencies(params: {
  installedDir: string;
  peerDependencies: Record<string, string>;
  logger: PluginPeerLinkLogger;
}): Promise<{ repaired: number; skipped: number }> {
  const peers = Object.keys(params.peerDependencies).filter((name) => name === "daisyclaw");
  if (peers.length === 0) {
  ...
  if (!hostRoot) {
    params.logger.warn?.(
      "Could not locate daisyclaw package root to symlink peerDependencies; plugin may fail to resolve daisyclaw at runtime.",
    );
  ...
  for (const peerName of peers) {
    const result = await linkDaisyClawPeerDependency({
      hostRoot,
      installedDir: params.installedDir,
      peerName,
      logger: params.logger,
...
export async function relinkDaisyClawPeerDependenciesInManagedNpmRoot(params: {
  npmRoot: string;
  logger: PluginPeerLinkLogger;
...
export async function auditDaisyClawPeerDependenciesInManagedNpmRoot(params: {
  npmRoot: string;
  ...
  let checked = 0;
  const issues: DaisyClawPeerLinkAuditIssue[] = [];
  for (const packageDir of await listManagedNpmRootPackageDirs(params.npmRoot)) {
    const peerDependencies = await readPackagePeerDependencies(packageDir);
    if (!Object.hasOwn(peerDependencies, "daisyclaw")) {
      continue;
    ...
    checked += 1;
    const issue = await auditDaisyClawPeerDependency({
      hostRoot,
      npmRoot: params.npmRoot,
      packageDir,
    });
