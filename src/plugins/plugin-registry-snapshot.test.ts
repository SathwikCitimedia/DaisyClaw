...
function makeTempDir() {
  return makeTrackedTempDir("daisyclaw-plugin-registry-snapshot", tempDirs);
}

function createHermeticEnv(rootDir: string): NodeJS.ProcessEnv {
  return {
    DAISYCLAW_BUNDLED_PLUGINS_DIR: path.join(rootDir, "bundled"),
    DAISYCLAW_STATE_DIR: path.join(rootDir, "state"),
...
describe("loadPluginRegistrySnapshotWithMetadata", () => {
  ...
  it("recovers managed npm plugins missing from a stale persisted registry", () => {
    ...
    expect(result.snapshot.installRecords.whatsapp).toEqual({
      source: "npm",
      spec: "@daisyclaw/whatsapp@2026.5.2",
      installPath: whatsappDir,
      version: "2026.5.2",
      resolvedName: "@daisyclaw/whatsapp",
      resolvedVersion: "2026.5.2",
      resolvedSpec: "@daisyclaw/whatsapp@2026.5.2",
    });
  ...
  it("keeps persisted manifestless Claude bundles on the fast path", () => {
    ...
    const stateDir = path.join(tempRoot, "state");
    const env = { ...createHermeticEnv(tempRoot), DAISYCLAW_DISABLE_BUNDLED_PLUGINS: "1" };
    const config = {
      plugins: {
        load: { paths: [rootDir] },
      },
  ...
  it("refreshes a memoized derived snapshot when workspace plugins are installed", () => {
    ...
    writePackagePlugin(path.join(workspaceDir, ".daisyclaw", "extensions", "demo"));
  ...
  it("keeps persisted package plugins when file hashes match", () => {
    ...
    expect(record.manifestFile.size).toBe(
      fs.statSync(path.join(rootDir, "daisyclaw.plugin.json")).size,
    );
