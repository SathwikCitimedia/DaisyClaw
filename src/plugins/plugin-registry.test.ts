...
function makeTempDir() {
  return makeTrackedTempDir("daisyclaw-plugin-registry", tempDirs);
}

function hermeticEnv(overrides: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
  return {
    DAISYCLAW_BUNDLED_PLUGINS_DIR: undefined,
    DAISYCLAW_VERSION: "2026.4.25",
    VITEST: "true",
    ...overrides,
  };
}
...
function createCandidate(rootDir: string, pluginId = "demo"): PluginCandidate {
  ...
  fs.writeFileSync(
    path.join(rootDir, "daisyclaw.plugin.json"),
    JSON.stringify({
      id: pluginId,
      name: pluginId,
...
function createIndex(
  ...
  return {
    ...
    plugins: [
      {
        pluginId,
        manifestPath: path.join(pluginRoot, "daisyclaw.plugin.json"),
...
describe("plugin registry facade", () => {
  ...
  it("keeps absolute and home plugin API paths user-resolved", () => {
    ...
    expect(resolvePluginPath("~/daisyclaw/plugin.txt", pluginRoot)).toBe(
      resolvePluginPath("~/daisyclaw/plugin.txt", undefined),
    );
  });
  ...
  it("resolves contribution owners from a plugin lookup table without rereading manifests", () => {
    ...
    fs.unlinkSync(path.join(rootDir, "daisyclaw.plugin.json"));
  ...
  it("reads the persisted registry before deriving from discovered candidates", async () => {
    ...
    fs.writeFileSync(
      path.join(persistedRootDir, "daisyclaw.plugin.json"),
        ...
        plugins: [
          {
            ...
            manifestPath: path.join(persistedRootDir, "daisyclaw.plugin.json"),
            manifestHash: hashFile(path.join(persistedRootDir, "daisyclaw.plugin.json")),
  ...
  it("falls back to the derived registry when persisted bundled roots point at another checkout", async () => {
    ...
    await writePersistedInstalledPluginIndex(
      createIndex("persisted", {
        plugins: [
          {
            ...createIndex("persisted").plugins[0],
            manifestPath: path.join(staleBundledRootDir, "daisyclaw.plugin.json"),
    ...
    const result = loadPluginRegistrySnapshotWithMetadata({
      stateDir,
      candidates: [candidate],
      env: hermeticEnv({ DAISYCLAW_BUNDLED_PLUGINS_DIR: rootDir }),
    });
  ...
  it("reuses config-scoped derived registries within the process", () => {
    ...
    createCandidate(rootDir);
    const env = hermeticEnv({ DAISYCLAW_BUNDLED_PLUGINS_DIR: bundledRoot });
    ...
    const manifestReadsAfterFirst = readFileSyncSpy.mock.calls.filter((call) =>
      String(call[0]).endsWith("daisyclaw.plugin.json"),
    ).length;
  ...
  it("does not reuse the process registry memo after profile extensions change", () => {
    ...
    const env = hermeticEnv({
      DAISYCLAW_CONFIG_PATH: path.join(configDir, "daisyclaw.json"),
  ...
  it("keys the process registry memo by resolved host contract version", () => {
    ...
    const first = loadPluginRegistrySnapshotWithMetadata({
      stateDir,
      config,
      env: hermeticEnv({ DAISYCLAW_BUNDLED_PLUGINS_DIR: bundledRoot }),
    });
    const second = loadPluginRegistrySnapshotWithMetadata({
      stateDir,
      config,
