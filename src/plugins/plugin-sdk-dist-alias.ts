...
function readPublicPluginSdkDistFileNames(distRoot: string): Set<string> | undefined {
  const packageRoot = path.dirname(path.resolve(distRoot));
  const packageJson = tryReadJsonSync<DaisyClawPackageJson>(path.join(packageRoot, "package.json"));
...
export function ensureDaisyClawPluginSdkAlias(distRoot: string): void {
  const pluginSdkDir = path.join(distRoot, "plugin-sdk");
  if (!fs.existsSync(pluginSdkDir)) {
    return;
  }
