...
import {
  installDaisyClawPluginSdkNativeResolver,
  resetDaisyClawPluginSdkNativeResolverForTest,
...
function writeFakeDaisyClawPackage(root: string): { distRoot: string; loaderModulePath: string } {
  ...
  fs.writeFileSync(path.join(root, "daisyclaw.mjs"), "#!/usr/bin/env node\n", "utf8");
...
describe("installDaisyClawPluginSdkNativeResolver", () => {
  it("resolves installed plugin SDK imports to the dev source root", () => {
    const stableRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-sdk-native-stable-"));
    const devRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-sdk-native-dev-source-"));
    const { loaderModulePath } = writeFakeDaisyClawPackage(stableRoot);
    writeFakeDaisyClawPackage(devRoot);
    ...
    const previousDevSourceRoot = process.env.DAISYCLAW_DEV_SOURCE_ROOT;
    process.env.DAISYCLAW_DEV_SOURCE_ROOT = devRoot;

    try {
      const installedAliases = installDaisyClawPluginSdkNativeResolver({
        modulePath: loaderModulePath,
        pluginModulePath: externalPluginEntry,
      });

      expect(installedAliases).toContain("daisyclaw/plugin-sdk/agent-runtime");
      const requireFromPlugin = createRequire(externalPluginEntry);
      expect(fs.realpathSync(requireFromPlugin.resolve("daisyclaw/plugin-sdk/agent-runtime"))).toBe(
        fs.realpathSync(path.join(devRoot, "dist", "plugin-sdk", "agent-runtime.js")),
      );
    } finally {
      if (previousDevSourceRoot === undefined) {
        delete process.env.DAISYCLAW_DEV_SOURCE_ROOT;
      } else {
        process.env.DAISYCLAW_DEV_SOURCE_ROOT = previousDevSourceRoot;
      }
    }
  });
  ...
  it("removes stale native SDK aliases when a later dev root omits a subpath", () => {
    ...
    expect(fs.realpathSync(requireFromPlugin.resolve("daisyclaw/plugin-sdk/stable-extra"))).toBe(
    ...
    expect(() => requireFromPlugin.resolve("daisyclaw/plugin-sdk/stable-extra")).toThrow();
  });

  it("keeps native aliases on JS dist artifacts when source files exist", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-sdk-native-source-resolver-"));
    const { loaderModulePath } = writeFakeDaisyClawPackage(root);
    ...
    expect(installedAliases).toContain("daisyclaw/plugin-sdk/channel-outbound");
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(fs.realpathSync(requireFromPlugin.resolve("daisyclaw/plugin-sdk/channel-outbound"))).toBe(
      fs.realpathSync(path.join(root, "dist", "plugin-sdk", "channel-outbound.js")),
    );
  });

  it("lets built external plugins resolve DaisyClaw SDK subpaths with createRequire", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-sdk-native-resolver-"));
    const { distRoot, loaderModulePath } = writeFakeDaisyClawPackage(root);
    const externalPluginEntry = writeExternalPluginEntry(path.join(root, "external-plugin"));
    ...
    try {
      ...
      const requireFromPlugin = createRequire(externalPluginEntry);
      expect(
        fs.realpathSync(requireFromPlugin.resolve("daisyclaw/plugin-sdk/channel-outbound")),
      ).toBe(fs.realpathSync(path.join(root, "dist", "plugin-sdk", "channel-outbound.js")));
      const sdk = requireFromPlugin("daisyclaw/plugin-sdk/channel-outbound") as {
        defineChannelMessageAdapter?: () => string;
      };

      expect(sdk.defineChannelMessageAdapter?.()).toBe("adapter");
      expect(() => requireFromPlugin.resolve("daisyclaw/not-plugin-sdk/channel-message")).toThrow();
    } finally {
      if (process.platform !== "win32") {
  ...
  beforeAll(() => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-sdk-native-esm-resolver-"));
    ...
    fs.writeFileSync(
      ...
      [
        ...
        'import path from "node:path";',
        'import { pathToFileURL } from "node:url";',
        `import { installDaisyClawPluginSdkNativeResolver, resetDaisyClawPluginSdkNativeResolverForTest } from ${JSON.stringify(resolverModuleUrl)};`,
        ...
        "fs.writeFileSync(",
        "  entryPath,",
        '  "import { defineChannelMessageAdapter } from \\"daisyclaw/plugin-sdk/channel-outbound\\"; export const eager = defineChannelMessageAdapter(); export const loadLazy = () => import(\\"./lazy.js\\");\\n",',
        '  "utf8",',
        ");",
        "fs.writeFileSync(",
        "  lazyPath,",
        '  "import { defineChannelMessageAdapter } from \\"daisyclaw/plugin-sdk/channel-outbound\\"; export const lazy = defineChannelMessageAdapter();\\n",',
        '  "utf8",',
        ");",
        "installDaisyClawPluginSdkNativeResolver({",
        "  modulePath: loaderModulePath,",
  ...
  it("does not resolve SDK aliases for parents outside registered plugin roots", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-sdk-native-guard-"));
    const { loaderModulePath } = writeFakeDaisyClawPackage(root);
    const externalPluginEntry = writeExternalPluginEntry(path.join(root, "external-plugin"));
    const unrelatedRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-sdk-native-outside-"));
    ...
    expect(requireFromPlugin.resolve("daisyclaw/plugin-sdk/channel-outbound")).toBeTruthy();
    expect(() => requireFromOutside.resolve("daisyclaw/plugin-sdk/channel-outbound")).toThrow();
  });

  it("resolves internal core packages only for DaisyClaw-owned source parents", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-sdk-native-core-internal-"));
    const { loaderModulePath } = writeFakeDaisyClawPackage(root);
    ...
    expect(installedAliases).toContain("@daisyclaw/normalization-core/string-coerce");
    expect(installedAliases).toContain("@daisyclaw/media-core/mime");
    expect(installedAliases).toContain("@daisyclaw/acp-core/runtime/types");
    expect(installedAliases).toContain("@daisyclaw/llm-core");
    const requireFromCoreSource = createRequire(coreSourceParent);
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@daisyclaw/normalization-core/string-coerce")),
    ).toBe(fs.realpathSync(normalizationSource));
    expect(fs.realpathSync(requireFromCoreSource.resolve("@daisyclaw/media-core/mime"))).toBe(
      fs.realpathSync(mediaCoreSource),
    );
    expect(fs.realpathSync(requireFromCoreSource.resolve("@daisyclaw/acp-core/runtime/types"))).toBe(
      fs.realpathSync(acpCoreSource),
    );
    expect(fs.realpathSync(requireFromCoreSource.resolve("@daisyclaw/llm-core"))).toBe(
      fs.realpathSync(llmCoreSource),
    );
    expect(() => requireFromPlugin.resolve("@daisyclaw/normalization-core/string-coerce")).toThrow();
    expect(() => requireFromPlugin.resolve("@daisyclaw/media-core/mime")).toThrow();
    expect(() => requireFromPlugin.resolve("@daisyclaw/acp-core/runtime/types")).toThrow();
    expect(() => requireFromPlugin.resolve("@daisyclaw/llm-core")).toThrow();
  });

  it("does not register source-only SDK subpaths for native resolution", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-sdk-native-source-only-"));
    const { loaderModulePath } = writeFakeDaisyClawPackage(root);
    ...
    expect(installedAliases).toContain("daisyclaw/plugin-sdk/channel-outbound");
    expect(installedAliases).not.toContain("daisyclaw/plugin-sdk/source-only");
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(() => requireFromPlugin.resolve("daisyclaw/plugin-sdk/source-only")).toThrow();
  });

  it("scopes private SSRF SDK aliases to bundled local IPC native parents", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-sdk-native-ssrf-"));
    const { loaderModulePath } = writeFakeDaisyClawPackage(root);
    ...
    expect(installedAliases).toContain("daisyclaw/plugin-sdk/ssrf-runtime-internal");
    const requireFromOllama = createRequire(ollamaEntry);
    expect(
      fs.realpathSync(requireFromOllama.resolve("daisyclaw/plugin-sdk/ssrf-runtime-internal")),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromRuntimeOllama = createRequire(runtimeOllamaEntry);
    expect(
      fs.realpathSync(
        requireFromRuntimeOllama.resolve("daisyclaw/plugin-sdk/ssrf-runtime-internal"),
      ),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromBrowser = createRequire(browserEntry);
    expect(
      fs.realpathSync(requireFromBrowser.resolve("daisyclaw/plugin-sdk/ssrf-runtime-internal")),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromRuntimeBrowser = createRequire(runtimeBrowserEntry);
    expect(
      fs.realpathSync(
        requireFromRuntimeBrowser.resolve("daisyclaw/plugin-sdk/ssrf-runtime-internal"),
      ),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromOther = createRequire(otherEntry);
    expect(() => requireFromOther.resolve("daisyclaw/plugin-sdk/ssrf-runtime-internal")).toThrow();
  });
});
