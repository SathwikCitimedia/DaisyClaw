/** Installs native Node resolution aliases so plugins can import the DaisyClaw SDK in dev and tests. */
...
export type InstallDaisyClawPluginSdkNativeResolverOptions = {
  modulePath?: string;
...
const PLUGIN_SDK_PACKAGE_PREFIXES = ["daisyclaw/plugin-sdk", "@daisyclaw/plugin-sdk"] as const;
const INTERNAL_CORE_PACKAGE_ALIASES = [
  ...
  {
    packageName: "@daisyclaw/llm-core",
    packageDir: "llm-core",
    subpaths: [
      ["", "index.ts"],
...
function resolveLoaderModulePath(options: InstallDaisyClawPluginSdkNativeResolverOptions): string {
  ...
  for (let i = 0; i < 12; i += 1) {
    ...
    if (fs.existsSync(packageJsonPath)) {
      try {
        ...
        if (
          packageJson.name === "daisyclaw" ||
          (typeof packageJson.bin === "object" &&
            packageJson.bin !== null &&
            typeof (packageJson.bin as { daisyclaw?: unknown }).daisyclaw === "string")
        ) {
          return cursor;
        }
      } catch {
...
function resolveAllowedParentRoots(
  options: InstallDaisyClawPluginSdkNativeResolverOptions,
): string[] {
...
export function installDaisyClawPluginSdkNativeResolver(
  options: InstallDaisyClawPluginSdkNativeResolverOptions = {},
): string[] {
...
export function installDaisyClawInternalCorePackageNativeResolver(
  options: Pick<InstallDaisyClawPluginSdkNativeResolverOptions, "moduleUrl"> = {},
): string[] {
  for (const alias of listInternalCorePackageNativeAliases(options)) {
    registerNativeAlias(alias);
  }
...
export function resetDaisyClawPluginSdkNativeResolverForTest(): void {
