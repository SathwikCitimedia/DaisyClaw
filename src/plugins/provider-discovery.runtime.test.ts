...
function createManifestPlugin(id: string): PluginManifestRecord {
  return {
    ...
    rootDir: `/tmp/${id}`,
    source: "bundled",
    manifestPath: `/tmp/${id}/daisyclaw.plugin.json`,
...
describe("resolvePluginDiscoveryProvidersRuntime", () => {
  ...
  it("clears bundled dist discovery chunks from the dist root", () => {
    ...
    mocks.loadPluginMetadataSnapshot.mockReturnValue({
      ...
      manifestRegistry: {
        plugins: [
          {
            ...createManifestPlugin("deepseek"),
            rootDir: "/tmp/daisyclaw/dist/extensions/deepseek",
            manifestPath: "/tmp/daisyclaw/dist/extensions/deepseek/daisyclaw.plugin.json",
            providerDiscoverySource: "/tmp/daisyclaw/dist/extensions/deepseek/provider-discovery.js",
          },
        ],
        diagnostics: [],
      },
    ...
    expect(mocks.clearNativeRequireJavaScriptModuleCache).toHaveBeenCalledWith(
      "/tmp/daisyclaw/dist/extensions/deepseek/provider-discovery.js",
      { dependencyRoot: "/tmp/daisyclaw/dist" },
    );
  });
