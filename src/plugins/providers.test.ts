...
import { sortUniqueStrings } from "@daisyclaw/normalization-core/string-normalization";
...
type LoadDaisyClawPlugins = typeof import("./loader.js").loadDaisyClawPlugins;
...
const loadDaisyClawPluginsMock = vi.fn<LoadDaisyClawPlugins>();
...
function createManifestProviderPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/tmp/${params.id}/daisyclaw.plugin.json`,
  };
}
...
function expectLastSetupRegistryCall(params: {
  ...
  const call = getLastMockCallArg(loadDaisyClawPluginsMock, "DaisyClaw plugin setup loader");
...
function getLastSetupLoadedPluginConfig() {
  const call = expectRecordFields(
    getLastMockCallArg(loadDaisyClawPluginsMock, "DaisyClaw plugin setup loader"),
    {},
  );
...
function createAutoEnabledProviderConfig() {
  const rawConfig: DaisyClawConfig = {
    plugins: {},
  };
  const autoEnabledConfig: DaisyClawConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    ...
    vi.doMock("./loader.js", () => ({
      loadDaisyClawPlugins: (...args: Parameters<LoadDaisyClawPlugins>) =>
        loadDaisyClawPluginsMock(...args),
