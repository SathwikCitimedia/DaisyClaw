...
} from "@daisyclaw/normalization-core/string-coerce";
import { uniqueValues } from "@daisyclaw/normalization-core/string-normalization";
import {
  normalizeStringEntries,
  normalizeUniqueStringEntries,
} from "@daisyclaw/normalization-core/string-normalization";
...
import type {
  ...
  DaisyClawPluginApi,
  DaisyClawPluginChannelRegistration,
  DaisyClawPluginCliCommandDescriptor,
  DaisyClawPluginCliRegistrar,
  DaisyClawPluginCommandDefinition,
  PluginConversationBindingResolvedEvent,
  DaisyClawPluginGatewayRuntimeScopeSurface,
  DaisyClawGatewayDiscoveryService,
  DaisyClawPluginHostedMediaResolver,
  DaisyClawPluginHttpRouteParams,
  DaisyClawPluginHookOptions,
  DaisyClawPluginNodeHostCommand,
  DaisyClawPluginNodeInvokePolicy,
  DaisyClawPluginReloadRegistration,
  DaisyClawPluginSecurityAuditCollector,
...
function isOfficialCodexPluginRecord(
  ...
  if (record.packageName === "@daisyclaw/codex") {
    return true;
  }
  const sourcePath = path
    ...
    .split(path.sep)
    .join("/");
  return sourcePath.includes("/node_modules/@daisyclaw/codex");
}
...
const ACTIVE_PLUGIN_HOOK_REGISTRATIONS_KEY = Symbol.for("daisyclaw.activePluginHookRegistrations");
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerCodexAppServerExtensionFactory = (
    record: PluginRecord,
    factory: Parameters<DaisyClawPluginApi["registerCodexAppServerExtensionFactory"]>[0],
  ) => {
  ...
  const registerAgentToolResultMiddleware = (
    record: PluginRecord,
    handler: Parameters<DaisyClawPluginApi["registerAgentToolResultMiddleware"]>[0],
    options: Parameters<DaisyClawPluginApi["registerAgentToolResultMiddleware"]>[1],
  ) => {
  ...
  const registerTool = (
    ...
    const factory: DaisyClawPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: DaisyClawPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: DaisyClawPluginHookOptions | undefined,
    config: DaisyClawPluginApi["config"],
  ...
  const registerHttpRoute = (record: PluginRecord, params: DaisyClawPluginHttpRouteParams) => {
  ...
  const registerHostedMediaResolver = (
    record: PluginRecord,
    resolver: DaisyClawPluginHostedMediaResolver,
  ) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: DaisyClawPluginChannelRegistration | ChannelPlugin,
    ...
    const normalized =
      typeof (registration as DaisyClawPluginChannelRegistration).plugin === "object"
        ? (registration as DaisyClawPluginChannelRegistration)
  ...
  const registerCli = (
    ...
    const descriptors = (opts?.descriptors ?? [])
      ...
      .filter(
        (descriptor): descriptor is DaisyClawPluginCliCommandDescriptor => descriptor !== null,
      );
    const commands = [
      ...(opts?.commands ?? []),
  ...
  const registerReload = (record: PluginRecord, registration: DaisyClawPluginReloadRegistration) => {
    const normalized: DaisyClawPluginReloadRegistration = {
  ...
  const registerNodeHostCommand = (
    record: PluginRecord,
    nodeCommand: DaisyClawPluginNodeHostCommand,
  ) => {
  ...
  const registerNodeInvokePolicy = (
    record: PluginRecord,
    policy: DaisyClawPluginNodeInvokePolicy,
  ...
  const registerSecurityAuditCollector = (
    record: PluginRecord,
    collector: DaisyClawPluginSecurityAuditCollector,
  ) => {
  ...
  const registerService = (record: PluginRecord, service: DaisyClawPluginService) => {
  ...
  const registerGatewayDiscoveryService = (
    record: PluginRecord,
    service: DaisyClawGatewayDiscoveryService,
  ) => {
    const id = service.id.trim();
    if (!id) {
      return;
  ...
  const registerCommand = (record: PluginRecord, command: DaisyClawPluginCommandDefinition) => {
      ...
      handlers: {
        ...(registrationCapabilities.capabilityHandlers
          ? {
              ...
              registerCompactionProvider: (
                provider: Parameters<DaisyClawPluginApi["registerCompactionProvider"]>[0],
              ) => {
                const id = normalizeOptionalString(
                  (
                    provider as Partial<
                      Parameters<DaisyClawPluginApi["registerCompactionProvider"]>[0]
                    > | null
                  )?.id,
                );
                if (!id) {
              ...
              enqueueNextTurnInjection: (injection) => {
                ...
                return enqueuePluginNextTurnInjection({
                  cfg: registryParams.runtime.config.current() as DaisyClawConfig,
                  pluginId: record.id,
                  pluginName: record.name,
                  injection,
                });
              ...
              sendSessionAttachment: async (attachment) => {
                ...
                try {
                  ...
                  const runtimeConfig =
                    (registryParams.runtime.config?.current?.() as DaisyClawConfig | undefined) ??
                    params.config;
