...
vi.mock("../loader.js", () => ({
  loadDaisyClawPlugins: (...args: unknown[]) => loadDaisyClawPluginsMock(...args),
}));
...
function getOnlyLoadDaisyClawPluginsOptions(): PluginLoadOptions {
  expect(loadDaisyClawPluginsMock).toHaveBeenCalledTimes(1);
  const options = loadDaisyClawPluginsMock.mock.calls[0]?.[0];
  if (!options || typeof options !== "object") {
    throw new Error("expected loadDaisyClawPlugins to receive plugin load options");
  }
...
describe("loadPluginMetadataRegistrySnapshot", () => {
  ...
  beforeEach(() => {
    ...
    loadDaisyClawPluginsMock.mockReturnValue({ plugins: [], diagnostics: [] });
  });
  ...
  it("defaults to a non-activating validate snapshot", () => {
    loadPluginMetadataRegistrySnapshot({
      ...
      env: { HOME: "/tmp/daisyclaw-home" } as NodeJS.ProcessEnv,
      workspaceDir: "/workspace",
      onlyPluginIds: ["demo"],
    });

    const loadOptions = getOnlyLoadDaisyClawPluginsOptions();
    expect(loadOptions).toMatchObject({
      config: { plugins: {} },
      activationSourceConfig: { plugins: { allow: ["demo"] } },
      autoEnabledReasons: {},
      workspaceDir: "/workspace",
      env: { HOME: "/tmp/daisyclaw-home" },
  ...
  it("forwards an explicit logger through metadata snapshots", () => {
    ...
    expect(getOnlyLoadDaisyClawPluginsOptions()).toMatchObject({
      config: { plugins: {} },
      activationSourceConfig: { plugins: {} },
      autoEnabledReasons: {},
      workspaceDir: "/workspace",
  ...
  it("honors explicit load options when reusing a resolved runtime context", () => {
    ...
    expect(getOnlyLoadDaisyClawPluginsOptions()).toEqual({
      config: { plugins: { allow: ["compat-provider"] } },
      activationSourceConfig: { plugins: { allow: ["raw-plugin"] } },
      autoEnabledReasons: {},
      workspaceDir: "/compat-workspace",
