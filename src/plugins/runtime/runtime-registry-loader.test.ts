...
const mocks = vi.hoisted(() => ({
  loadDaisyClawPlugins: vi.fn<typeof import("../loader.js").loadDaisyClawPlugins>(),
...
function loadOptions(index = 0) {
  return requireRecord(mocks.loadDaisyClawPlugins.mock.calls[index]?.[0], `load options ${index}`);
}
...
vi.mock("../loader.js", () => ({
  loadDaisyClawPlugins: (...args: Parameters<typeof mocks.loadDaisyClawPlugins>) =>
    mocks.loadDaisyClawPlugins(...args),
  resolveCompatibleRuntimePluginRegistry: (
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadDaisyClawPlugins.mockReturnValue(createEmptyPluginRegistry());
    mocks.resolveRuntimePluginRegistry.mockImplementation(
      (...args: Parameters<typeof mocks.loadDaisyClawPlugins>) => mocks.loadDaisyClawPlugins(...args),
    );
  ...
  it("uses the shared runtime load context for configured-channel loads", () => {
    ...
    const env = { HOME: "/tmp/daisyclaw-home" } as NodeJS.ProcessEnv;
  ...
  it("does not reuse non-empty all-scope registries without loader compatibility", () => {
    ...
    mocks.loadDaisyClawPlugins.mockClear();
