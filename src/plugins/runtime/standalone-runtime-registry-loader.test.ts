...
vi.mock("../loader.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    loadDaisyClawPlugins: (...args: Parameters<typeof loaderMocks.loadDaisyClawPlugins>) =>
      loaderMocks.loadDaisyClawPlugins(...args),
  };
});
...
beforeEach(() => {
  loaderMocks.loadDaisyClawPlugins.mockReset();
});
...
describe("ensureStandaloneRuntimePluginRegistryLoaded", () => {
  it("reuses a compatible gateway startup registry for gateway-bindable dispatch load options", () => {
    ...
    expect(loaderMocks.loadDaisyClawPlugins).not.toHaveBeenCalled();
  });
  ...
  it("loads a fresh registry when dispatch config is not startup-compatible", () => {
    ...
    loaderMocks.loadDaisyClawPlugins.mockReturnValue(loadedRegistry);
    ...
    expect(loaderMocks.loadDaisyClawPlugins).toHaveBeenCalledOnce();
  });
});
