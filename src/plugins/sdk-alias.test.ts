...
const fixtureRoot = makeTrackedTempDir("daisyclaw-sdk-alias-root", fixtureTempDirs);
let tempDirIndex = 0;
...
function createTrustedDaisyClawPackageFixture(version: string) {
  const root = makeTempDir();
  fs.writeFileSync(path.join(root, "daisyclaw.mjs"), "export {};\n", "utf-8");
  fs.writeFileSync(
...
function createExtensionApiAliasFixture(params?: {
  ...
  fs.writeFileSync(
    path.join(root, "package.json"),
    JSON.stringify({ name: "daisyclaw", type: "module" }, null, 2),
    "utf-8",
  );
  fs.writeFileSync(path.join(root, "daisyclaw.mjs"), "export {};\n", "utf-8");
  fs.writeFileSync(srcFile, params?.srcBody ?? "export {};\n", "utf-8");
...
function createBundledPluginPackagePublicSurfaceAliasFixture() {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: "@daisyclaw/slack", type: "module" }, null, 2),
    "utf-8",
  );
...
function createUserInstalledPluginSdkAliasFixture() {
  ...
  const externalPluginRoot = path.join(makeTempDir(), ".daisyclaw", "extensions", "demo");
...
function expectPluginSdkAliasTargets(
  ...
  expect(fs.realpathSync(aliases["daisyclaw/plugin-sdk"] ?? "")).toBe(
  ...
  if (params.channelRuntimePath) {
    expect(fs.realpathSync(aliases["daisyclaw/plugin-sdk/channel-runtime"] ?? "")).toBe(
  ...
  if (params.pluginEntryPath) {
    expect(fs.realpathSync(aliases["daisyclaw/plugin-sdk/plugin-entry"] ?? "")).toBe(
...
function expectCwdFallbackPluginSdkAliasResolution(params: {
  ...
  const resolved = withCwd(params.fixture.root, () =>
    resolvePluginSdkAlias({
      ...
      distFile: "channel-runtime.js",
      modulePath: "/tmp/tsx-cache/daisyclaw-loader.js",
...
describe("plugin sdk alias helpers", () => {
  it.each([
    ...
    },
    {
      ...
      modulePath: () => "/tmp/tsx-cache/daisyclaw-loader.js",
      argv1: (root: string) => path.join(root, "daisyclaw.mjs"),
      srcFile: "index.ts",
      distFile: "index.js",
      env: { NODE_ENV: undefined },
      expected: "src" as const,
  ...
  it("resolves extension-api aliases from an explicit dev source root", () => {
    ...
    expect(fs.realpathSync(aliases["daisyclaw/extension-api"] ?? "")).toBe(
  ...
  it("adds private qa plugin-sdk subpaths for trusted local checkouts when enabled", () => {
    ...
    const subpaths = withEnv({ DAISYCLAW_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: path.join(fixture.root, "src", "plugins", "loader.ts"),
      }),
    );
  ...
  it("adds non-QA private Codex helper subpaths only for trusted Codex plugins", () => {
    ...
    const { packageRoot: installedCodexRoot, pluginEntry: installedCodexEntry } =
      writeInstalledPluginEntry({
        installRoot: path.join(makeTempDir(), ".daisyclaw", "npm"),
        packageName: "@daisyclaw/codex",
      });
    const { packageRoot: installedOtherRoot, pluginEntry: installedOtherEntry } =
      writeInstalledPluginEntry({
        installRoot: path.join(makeTempDir(), ".daisyclaw", "npm"),
        packageName: "@daisyclaw/demo",
      });
    const shadowCodexRoot = path.join(makeTempDir(), ".daisyclaw", "extensions", "codex-shadow");
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: "@daisyclaw/codex", type: "module" }, null, 2),
      "utf-8",
    );
    fs.writeFileSync(shadowCodexEntry, 'export const plugin = "shadow";\n', "utf-8");

    const codexSubpaths = withEnv({ DAISYCLAW_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: sourceCodexEntry,
      }),
    );
    const otherSubpaths = withEnv({ DAISYCLAW_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: sourceOtherEntry,
      }),
    );
    const installedCodexSubpaths = withCwd(installedCodexRoot, () =>
      withEnv({ DAISYCLAW_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
        listPluginSdkExportedSubpaths({
          modulePath: installedCodexEntry,
          argv1: path.join(fixture.root, "daisyclaw.mjs"),
        }),
      ),
    );
  ...
  it("does not reuse a non-private cached subpath list after private qa gets enabled", () => {
    ...
    const privateSubpaths = withEnv({ DAISYCLAW_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: path.join(fixture.root, "src", "plugins", "loader.ts"),
      }),
    );
  ...
  it.each([
    {
      name: "does not derive plugin-sdk subpaths from cwd fallback when package root is not an DaisyClaw root",
      fixture: () =>
        createPluginSdkAliasFixture({
  ...
  it("adds private qa plugin-sdk aliases for source plugins when enabled", () => {
    ...
    const aliases = withEnv({ DAISYCLAW_ENABLE_PRIVATE_QA_CLI: "1", NODE_ENV: undefined }, () =>
      buildPluginLoaderAliasMap(sourcePluginEntry),
    );
    ...
    expect(fs.realpathSync(aliases["daisyclaw/plugin-sdk/qa-runtime"] ?? "")).toBe(
      fs.realpathSync(sourceQaRuntimePath),
    );
    expect(fs.realpathSync(aliases["daisyclaw/plugin-sdk/qa-channel"] ?? "")).toBe(
      fs.realpathSync(sourceQaChannelPath),
    );
    expect(fs.realpathSync(aliases["daisyclaw/plugin-sdk/qa-channel-protocol"] ?? "")).toBe(
      fs.realpathSync(sourceQaChannelProtocolPath),
    );
    expect(fs.realpathSync(aliases["daisyclaw/plugin-sdk/qa-lab"] ?? "")).toBe(
  ...
  it("aliases non-QA private plugin-sdk subpaths for trusted Codex runtime loading", () => {
    ...
    const aliases = withEnv(
      { DAISYCLAW_ENABLE_PRIVATE_QA_CLI: undefined, NODE_ENV: undefined },
      () => buildPluginLoaderAliasMap(sourcePluginEntry),
    );
    ...
    const devRootAliases = withEnv(
      ...
      () =>
        buildPluginLoaderAliasMap(
          distCodexEntry,
          path.join(fixture.root, "daisyclaw.mjs"),
          undefined,
          "dist",
          devFixture.root,
        ),
    ...
    const installedAliases = withCwd(installedCodexRoot, () =>
      withEnv({ DAISYCLAW_ENABLE_PRIVATE_QA_CLI: undefined, NODE_ENV: undefined }, () =>
        buildPluginLoaderAliasMap(
          installedCodexEntry,
          path.join(fixture.root, "daisyclaw.mjs"),
          undefined,
    ...
    expect(fs.realpathSync(aliases["daisyclaw/plugin-sdk/codex-mcp-projection"] ?? "")).toBe(
      fs.realpathSync(sourceCodexMcpProjectionPath),
    );
    expect(fs.realpathSync(aliases["daisyclaw/plugin-sdk/codex-native-task-runtime"] ?? "")).toBe(
      fs.realpathSync(sourceCodexNativeTaskRuntimePath),
    );
    expect(
      fs.realpathSync(installedAliases["daisyclaw/plugin-sdk/codex-mcp-projection"] ?? ""),
    ).toBe(fs.realpathSync(distCodexMcpProjectionPath));
    expect(
      fs.realpathSync(installedAliases["daisyclaw/plugin-sdk/codex-native-task-runtime"] ?? ""),
    ).toBe(fs.realpathSync(distCodexNativeTaskRuntimePath));
    expect(fs.realpathSync(devRootAliases["daisyclaw/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(devRootAlias),
    );
    expect(fs.realpathSync(devRootAliases["daisyclaw/plugin-sdk/codex-mcp-projection"] ?? "")).toBe(
      fs.realpathSync(devCodexMcpProjectionPath),
    );
    expect(
      fs.realpathSync(devRootAliases["daisyclaw/plugin-sdk/codex-native-task-runtime"] ?? ""),
    ).toBe(fs.realpathSync(devCodexNativeTaskRuntimePath));
    expect(aliases["daisyclaw/plugin-sdk/qa-runtime"]).toBeUndefined();
    expect(otherAliases["daisyclaw/plugin-sdk/codex-mcp-projection"]).toBeUndefined();
    expect(otherAliases["daisyclaw/plugin-sdk/codex-native-task-runtime"]).toBeUndefined();
    expect(installedOtherAliases["daisyclaw/plugin-sdk/codex-mcp-projection"]).toBeUndefined();
    expect(installedOtherAliases["daisyclaw/plugin-sdk/codex-native-task-runtime"]).toBeUndefined();
    expect(shadowCodexAliases["daisyclaw/plugin-sdk/codex-mcp-projection"]).toBeUndefined();
    expect(shadowCodexAliases["daisyclaw/plugin-sdk/codex-native-task-runtime"]).toBeUndefined();
  });
  ...
  it("aliases the SSRF internal helper only for bundled local IPC owner plugins", async () => {
    ...
    const entryBody = [
      'import { ssrfInternal } from "daisyclaw/plugin-sdk/ssrf-runtime-internal";',
    ...
    const { packageRoot: installedOllamaRoot, pluginEntry: installedOllamaEntry } =
      writeInstalledPluginEntry({
        installRoot: path.join(makeTempDir(), ".daisyclaw", "npm"),
        packageName: "@daisyclaw/ollama",
      });

    const sourceSubpaths = withEnv({ DAISYCLAW_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: sourceOllamaEntry,
      }),
    );
    const sourceBrowserSubpaths = withEnv({ DAISYCLAW_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: sourceBrowserEntry,
      }),
    );
    const privateQaOtherSubpaths = withEnv({ DAISYCLAW_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      listPluginSdkExportedSubpaths({
    ...
    const privateQaOtherAliases = withEnv(
      { DAISYCLAW_ENABLE_PRIVATE_QA_CLI: "1", NODE_ENV: undefined },
      () => buildPluginLoaderAliasMap(sourceOtherPluginEntry),
    );
    ...
    expect(fs.realpathSync(sourceAliases["daisyclaw/plugin-sdk/ssrf-runtime-internal"] ?? "")).toBe(
      fs.realpathSync(sourceSsrFInternalPath),
    );
    expect(
      fs.realpathSync(sourceBrowserAliases["daisyclaw/plugin-sdk/ssrf-runtime-internal"] ?? ""),
    ).toBe(fs.realpathSync(sourceSsrFInternalPath));
    expect(fs.realpathSync(distAliases["daisyclaw/plugin-sdk/ssrf-runtime-internal"] ?? "")).toBe(
      fs.realpathSync(distSsrFInternalPath),
    );
    expect(
      fs.realpathSync(distBrowserAliases["daisyclaw/plugin-sdk/ssrf-runtime-internal"] ?? ""),
    ).toBe(fs.realpathSync(distSsrFInternalPath));
    expect(
      fs.realpathSync(distRuntimeAliases["daisyclaw/plugin-sdk/ssrf-runtime-internal"] ?? ""),
    ).toBe(fs.realpathSync(distSsrFInternalPath));
    expect(
      fs.realpathSync(distRuntimeBrowserAliases["daisyclaw/plugin-sdk/ssrf-runtime-internal"] ?? ""),
    ).toBe(fs.realpathSync(distSsrFInternalPath));
    expect(otherAliases["daisyclaw/plugin-sdk/ssrf-runtime-internal"]).toBeUndefined();
    expect(privateQaOtherAliases["daisyclaw/plugin-sdk/ssrf-runtime-internal"]).toBeUndefined();
    expect(installedAliases["daisyclaw/plugin-sdk/ssrf-runtime-internal"]).toBeUndefined();
  ...
  it("aliases workspace packages to source when dist artifacts are missing", () => {
    ...
    expect(fs.realpathSync(aliases["@daisyclaw/gateway-client"] ?? "")).toBe(
      fs.realpathSync(gatewayClient.srcFile),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/gateway-client/timeouts"] ?? "")).toBe(
      fs.realpathSync(gatewayClientTimeouts.srcFile),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/gateway-protocol"] ?? "")).toBe(
      fs.realpathSync(gatewayProtocol.srcFile),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/gateway-protocol/schema"] ?? "")).toBe(
      fs.realpathSync(gatewayProtocolSchema.srcFile),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/markdown-core"] ?? "")).toBe(
      fs.realpathSync(markdownCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/markdown-core/tables"] ?? "")).toBe(
      fs.realpathSync(markdownCoreTables.srcFile),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/media-generation-core"] ?? "")).toBe(
      fs.realpathSync(mediaGenerationCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/media-generation-core/model-ref"] ?? "")).toBe(
      fs.realpathSync(mediaGenerationModelRef.srcFile),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/media-core"] ?? "")).toBe(
      fs.realpathSync(mediaCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/media-core/mime"] ?? "")).toBe(
      fs.realpathSync(mediaCoreMime.srcFile),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/acp-core"] ?? "")).toBe(
      fs.realpathSync(acpCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/acp-core/runtime/types"] ?? "")).toBe(
      fs.realpathSync(acpCoreRuntimeTypes.srcFile),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/normalization-core"] ?? "")).toBe(
      fs.realpathSync(normalizationCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/normalization-core/string-coerce"] ?? "")).toBe(
      fs.realpathSync(normalizationStringCoerce.srcFile),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/terminal-core"] ?? "")).toBe(
      fs.realpathSync(terminalCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/terminal-core/theme"] ?? "")).toBe(
      fs.realpathSync(terminalCoreTheme.srcFile),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/net-policy"] ?? "")).toBe(
      fs.realpathSync(netPolicy.srcFile),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/net-policy/ip"] ?? "")).toBe(
      fs.realpathSync(netPolicyIp.srcFile),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/model-catalog-core/provider-id"] ?? "")).toBe(
  ...
  it("aliases workspace package subpaths to dist when available", () => {
    ...
    expect(fs.realpathSync(aliases["@daisyclaw/gateway-client/readiness"] ?? "")).toBe(
      fs.realpathSync(gatewayClient.distFile),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/gateway-protocol/connect-error-details"] ?? "")).toBe(
      fs.realpathSync(gatewayProtocol.distFile),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/markdown-core/render"] ?? "")).toBe(
      fs.realpathSync(markdownCore.distFile),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/media-generation-core/catalog"] ?? "")).toBe(
      fs.realpathSync(mediaGenerationCore.distFile),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/media-core/read-response-with-limit"] ?? "")).toBe(
      fs.realpathSync(mediaCoreRootDistFile),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/acp-core/normalize-text"] ?? "")).toBe(
      fs.realpathSync(acpCoreRootDistFile),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/normalization-core/record-coerce"] ?? "")).toBe(
      fs.realpathSync(normalizationCoreRootDistFile),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/terminal-core/links"] ?? "")).toBe(
      fs.realpathSync(terminalCoreRootDistFile),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/net-policy/redact-sensitive-url"] ?? "")).toBe(
      fs.realpathSync(netPolicy.distFile),
    );
    expect(
      fs.realpathSync(aliases["@daisyclaw/model-catalog-core/provider-model-id-normalize"] ?? ""),
  ...
  it("derives acp-core aliases from packaged root dist when package metadata is absent", () => {
    ...
    const cwdWithoutDaisyClawPackage = makeTempDir();

    const aliases = withCwd(cwdWithoutDaisyClawPackage, () =>
      withEnv({ NODE_ENV: undefined }, () =>
        buildPluginLoaderAliasMap(sourcePluginEntry, undefined, undefined, "dist"),
      ),
    );

    expect(fs.realpathSync(aliases["@daisyclaw/acp-core/runtime/errors"] ?? "")).toBe(
  ...
  it("aliases bundled plugin package public surfaces for source plugin transforms", () => {
    ...
    expect(fs.realpathSync(aliases["@daisyclaw/slack/api.js"] ?? "")).toBe(
      fs.realpathSync(sourceApiPath),
    );
    expect(fs.realpathSync(aliases["@daisyclaw/slack/runtime-api.js"] ?? "")).toBe(
      fs.realpathSync(sourceRuntimeApiPath),
    );
    expect(aliases["@daisyclaw/slack/test-api.js"]).toBeUndefined();
    expect(aliases["@daisyclaw/slack/internal.js"]).toBeUndefined();
  });
  ...
  it("aliases bundled plugin package test surfaces only in private QA mode", () => {
    ...
    expect(fs.realpathSync(aliases["@daisyclaw/slack/test-api.js"] ?? "")).toBe(
  ...
  it("falls back to source plugin-sdk subpath aliases when dist chunks are stale", () => {
    ...
    expect(fs.realpathSync(distAliases["daisyclaw/plugin-sdk/provider-entry"] ?? "")).toBe(
  ...
  it("resolves plugin-sdk aliases for user-installed plugins via the running daisyclaw argv hint", () => {
    const {
      externalPluginEntry,
      externalPluginRoot,
      fixture,
    ...
    const aliases = withCwd(externalPluginRoot, () =>
      withEnv({ NODE_ENV: undefined }, () =>
        buildPluginLoaderAliasMap(externalPluginEntry, path.join(fixture.root, "daisyclaw.mjs")),
      ),
    );
  ...
  it("resolves plugin-sdk aliases for user-installed plugins via moduleUrl hint", () => {
    ...
    // This covers installations where argv1 does not resolve to the daisyclaw root
    // (e.g. single-binary distributions or custom process launchers).
    // Use daisyclaw.mjs which is created by createPluginSdkAliasFixture (bin+marker mode).
    // Use fixture.root as cwd so process.cwd() fallback also resolves to fixture, not the
    // real daisyclaw repo root in the test runner environment.
    const loaderModuleUrl = pathToFileURL(path.join(fixture.root, "daisyclaw.mjs")).href;
    ...
    // runner binary, inside the daisyclaw repo), which resolves before moduleUrl is checked.
    // An empty string is falsy so resolveTrustedDaisyClawRootFromArgvHint returns null,
  ...
  it.each([
    {
      name: "does not resolve plugin-sdk alias files from cwd fallback when package root is not an DaisyClaw root",
      fixture: () =>
        createPluginSdkAliasFixture({
          srcFile: "channel-runtime.ts",
          distFile: "channel-runtime.js",
  ...
  it("scopes plugin loader module config by plugin-sdk resolution", () => {
    ...
    expect(fs.realpathSync(auto.aliasMap["daisyclaw/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(sourceRootAlias),
    );
    expect(fs.realpathSync(dist.aliasMap["daisyclaw/plugin-sdk"] ?? "")).toBe(
  ...
  it("loads source runtime shims through the non-native module loading boundary", async () => {
    ...
    fs.writeFileSync(
      ...
      `import { resolveOutboundSendDep } from "@daisyclaw/plugin-sdk/channel-outbound";
...
`,
    ...
    const withAlias = createJiti(sourceLoaderBaseUrl, {
      ...buildPluginLoaderJitiOptions({
        "daisyclaw/plugin-sdk/channel-outbound": copiedChannelRuntimeShim,
        "@daisyclaw/plugin-sdk/channel-outbound": copiedChannelRuntimeShim,
      }),
      tryNative: false,
    });
  ...
  it("falls back to ancestor runtime candidates when package-root markers are unavailable", () => {
    const root = makeTempDir();
    const distFile = path.join(root, "dist", "plugins", "runtime", "index.js");
    const loaderCachePath = path.join(root, ".cache", "tsx", "daisyclaw-loader.js");
    mkdirSafeDir(path.dirname(distFile));
    ...
    expect(
      resolvePluginRuntimeModulePath({
        ...
        argv1: path.join(root, "bin", "daisyclaw"),
        pluginSdkResolution: "dist",
      }),
    ).toBe(distFile);
  });
  ...
  it("uses the default startup argv hint for runtime fallback when argv1 is omitted", () => {
    ...
    const loaderCachePath = path.join(loaderCacheRoot, "tsx", "daisyclaw-loader.js");
    ...
    process.argv[1] = path.join(root, "bin", "daisyclaw");
    try {
      expect(
        resolvePluginRuntimeModulePath({
          modulePath: loaderCachePath,
  ...
  it("resolves runtime fallback through symlinked startup argv", () => {
    const root = makeTempDir();
    const distFile = path.join(root, "dist", "plugins", "runtime", "index.js");
    const binFile = path.join(root, "bin", "daisyclaw");
    const shimRoot = makeTempDir();
    const shimFile = path.join(shimRoot, "bin", "daisyclaw");
    const loaderCachePath = path.join(makeTempDir(), "tsx", "daisyclaw-loader.js");
    mkdirSafeDir(path.dirname(distFile));
  ...
  it("resolves runtime fallback through npm .bin startup argv", () => {
    const root = makeTempDir();
    const packageRoot = path.join(root, "node_modules", "daisyclaw");
    ...
    const binFile = path.join(root, "node_modules", ".bin", "daisyclaw");
    const loaderCachePath = path.join(makeTempDir(), "tsx", "daisyclaw-loader.js");
    mkdirSafeDir(path.dirname(distFile));
...
describe("buildPluginLoaderAliasMap memoization", () => {
  ...
  it("returns different references for different modulePath inputs", () => {
    ...
    expect(aliasA["daisyclaw/plugin-sdk"]).not.toBe(aliasB["daisyclaw/plugin-sdk"]);
  });
  ...
  it("returns different references when an explicit dev source root differs", () => {
    ...
    expect(fs.realpathSync(stableAliases["daisyclaw/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(stableRootAlias),
    );
    expect(fs.realpathSync(devAliases["daisyclaw/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(devRootAlias),
    );
  });
  ...
  it("does not reuse a public alias map after private qa aliases are enabled", () => {
    ...
    const publicAliases = withEnv({ DAISYCLAW_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      buildPluginLoaderAliasMap(entry),
    );
    const privateAliases = withEnv({ DAISYCLAW_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      buildPluginLoaderAliasMap(entry),
    );

    expect(publicAliases).not.toBe(privateAliases);
    expect(publicAliases["daisyclaw/plugin-sdk/qa-runtime"]).toBeUndefined();
    expect(fs.realpathSync(privateAliases["daisyclaw/plugin-sdk/qa-runtime"] ?? "")).toBe(
      fs.realpathSync(sourceQaRuntimePath),
    );
  });
  ...
  it("does not reuse a development alias map in production mode", () => {
    ...
    expect(fs.realpathSync(developmentAliases["daisyclaw/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(sourceRootAlias),
    );
    expect(fs.realpathSync(productionAliases["daisyclaw/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(distRootAlias),
    );
  });
...
describe("buildPluginLoaderJitiOptions", () => {
  it("scopes jiti fs cache by DaisyClaw package version and install metadata", () => {
    const root = createTrustedDaisyClawPackageFixture("1.2.3-beta.4");
    ...
    expect(fsCache).toContain(path.join(tmpDir, "jiti", "daisyclaw", "1.2.3-beta.4") + path.sep);
  ...
  it("preserves jiti's tmpdir guard when TMPDIR resolves to cwd", () => {
    ...
    expect(guardedFsCache).toContain(path.join("jiti", "daisyclaw", "1.2.3-beta.4") + path.sep);
    expect(guardedFsCache.startsWith(path.join(root, "jiti") + path.sep)).toBe(false);
    expect(respectedFsCache).toContain(
      path.join(root, "jiti", "daisyclaw", "1.2.3-beta.4") + path.sep,
    );
  });

  it("adds the versioned fs cache directory to plugin loader jiti options", () => {
    const root = createTrustedDaisyClawPackageFixture("2.0.0");
    const tmpDir = path.join(root, "tmp");

    const options = withEnv({ TMPDIR: tmpDir }, () =>
      buildPluginLoaderJitiOptions(
        { "daisyclaw/plugin-sdk": path.join(root, "dist", "plugin-sdk", "root-alias.cjs") },
        { modulePath: path.join(root, "dist", "plugins", "loader.js") },
      ),
    );

    expect(options.fsCache).toContain(path.join(tmpDir, "jiti", "daisyclaw", "2.0.0"));
  });
  ...
  it("pre-normalizes and marks alias maps for source transforms", () => {
    ...
    const aliasMap = {
      "daisyclaw/plugin-sdk/core": "/repo/src/plugin-sdk/core.ts",
      "daisyclaw/plugin-sdk": "/repo/src/plugin-sdk/root-alias.cjs",
      "@daisyclaw/plugin-sdk": "/repo/src/plugin-sdk/root-alias.cjs",
    };
