...
import { resolveDaisyClawDevSourceRoot } from "./dev-source-root.js";
...
function hasTrustedDaisyClawRootIndicator(params: {
  packageRoot: string;
  ...
  const hasDaisyClawBin =
    (typeof params.packageJson.bin === "string" &&
      normalizeLowercaseStringOrEmpty(params.packageJson.bin).includes("daisyclaw")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.daisyclaw === "string");
  const hasDaisyClawEntrypoint = fs.existsSync(path.join(params.packageRoot, "daisyclaw.mjs"));
  return hasCliEntryExport || hasDaisyClawBin || hasDaisyClawEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedDaisyClawRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedDaisyClawRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  const packageRoot = resolveDaisyClawPackageRootSync({
    cwd: params.cwd,
    argv1: params.argv1,
  });
  if (!packageRoot) {
  ...
  return hasTrustedDaisyClawRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveDevSourceRootParam(params: { devSourceRoot?: string | null }): string | null {
  return params.devSourceRoot !== undefined
    ...
    : resolveDaisyClawDevSourceRoot(process.env);
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveDaisyClawPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedDaisyClawRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
...
const PLUGIN_SDK_PACKAGE_NAMES = ["daisyclaw/plugin-sdk", "@daisyclaw/plugin-sdk"] as const;
...
const PRIVATE_PLUGIN_SDK_SUBPATH_OWNERS: readonly PrivatePluginSdkSubpathOwner[] = [
  {
    ...
    officialInstalledPackageName: "@daisyclaw/codex",
...
const WORKSPACE_PACKAGE_ALIAS_ENTRIES: WorkspacePackageAliasEntry[] = [
  {
    packageName: "@daisyclaw/gateway-client",
  ...
  {
    packageName: "@daisyclaw/gateway-protocol",
  ...
  {
    packageName: "@daisyclaw/markdown-core",
  ...
  {
    packageName: "@daisyclaw/media-generation-core",
  ...
  {
    packageName: "@daisyclaw/media-core",
  ...
  {
    packageName: "@daisyclaw/normalization-core",
  ...
  {
    packageName: "@daisyclaw/terminal-core",
  ...
  {
    packageName: "@daisyclaw/net-policy",
  ...
  {
    packageName: "@daisyclaw/model-catalog-core",
...
export function listWorkspacePackageExportAliasEntries(params: {
  ...
  const fallbackPackageRoot = resolveDaisyClawPackageRootSync({ cwd: process.cwd() });
  const packageJson =
...
function readBundledPluginPackageName(packageJsonPath: string): string | null {
  ...
  return name.startsWith("@daisyclaw/") ? name : null;
}
...
function shouldIncludePrivateLocalOnlyPluginSdkSubpaths() {
  return process.env.DAISYCLAW_ENABLE_PRIVATE_QA_CLI === "1";
}
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (Object.hasOwn(aliasMap, `daisyclaw/plugin-sdk/${subpath}`)) {
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(
  ...
  const result: Record<string, string> = {
    ...(extensionApiAlias
      ? { "daisyclaw/extension-api": normalizeJitiAliasTargetPath(extensionApiAlias) }
      : {}),
...
export function isBundledPluginExtensionPath(params: {
  modulePath: string;
  daisyClawPackageRoot: string;
  bundledPluginsDir?: string;
}): boolean {
  const normalizedModulePath = path.resolve(params.modulePath);
  const roots = [
    params.bundledPluginsDir ? path.resolve(params.bundledPluginsDir) : null,
    path.join(params.daisyClawPackageRoot, "extensions"),
    path.join(params.daisyClawPackageRoot, "dist", "extensions"),
    path.join(params.daisyClawPackageRoot, "dist-runtime", "extensions"),
