...
function collectConfiguredPluginEntryIds(config: DaisyClawConfig): string[] {
...
function resolveRegister(mod: DaisyClawPluginModule): {
...
function resolveSetupRegistration(record: PluginManifestRecord): {
  ...
  let mod: DaisyClawPluginModule;
  try {
    mod = getModuleLoader(setupSource)(setupSource) as DaisyClawPluginModule;
  } catch {
    return null;
  }

  const resolved = resolveRegister((mod as { default?: DaisyClawPluginModule }).default ?? mod);
...
function pushDescriptorRuntimeDisabledDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message:
      "setup.requiresRuntime is false, so DaisyClaw ignored the plugin setup runtime entry. Remove setup-api/daisyclaw.setupEntry or set requiresRuntime true if setup lookup still needs plugin code.",
  });
}
