...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "daisyclaw", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".daisyclaw", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".daisyclaw", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("ignores untrusted explicit env override for the stock source root", () => {
    const homeDir = path.resolve(path.sep, "tmp", "daisyclaw-home");
    const rawEnv = {
    ...
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".daisyclaw", "extensions"),
      },
    });
  });
});
