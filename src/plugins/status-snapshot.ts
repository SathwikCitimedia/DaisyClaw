...
/** Control-plane plugin status shape used by `daisyclaw plugins status` style surfaces. */
...
function isPluginLifecycleTraceEnabled(): boolean {
  const raw = process.env.DAISYCLAW_PLUGIN_LIFECYCLE_TRACE?.trim().toLowerCase();
...
function buildPluginRecordFromInstalledIndex(
  ...
  const format = plugin.format ?? manifest?.format ?? "daisyclaw";
