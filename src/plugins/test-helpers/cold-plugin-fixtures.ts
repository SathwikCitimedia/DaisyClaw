...
export function createColdPluginFixture(options: ColdPluginFixtureOptions): ColdPluginFixture {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        name: options.packageName ?? "@example/daisyclaw-cold-control-plane",
  ...
  fs.writeFileSync(
    path.join(options.rootDir, "daisyclaw.plugin.json"),
    JSON.stringify(
      {
        id: pluginId,
...
export function createColdPluginConfig(pluginDir: string, pluginId: string): DaisyClawConfig {
  return {
    plugins: {
...
export function createColdPluginHermeticEnv(
  ...
  return {
    ...process.env,
    DAISYCLAW_HOME: path.join(homeDir, "home"),
    DAISYCLAW_BUNDLED_PLUGINS_DIR: options.bundledPluginsDir,
