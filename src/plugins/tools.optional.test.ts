...
const loadDaisyClawPluginsMock = vi.fn();
const resolveRuntimePluginRegistryMock = vi.fn();
const applyPluginAutoEnableMock = vi.fn();

vi.mock("./loader.js", () => ({
  loadDaisyClawPlugins: (params: unknown) => loadDaisyClawPluginsMock(params),
...
function setRegistry(entries: MockRegistryToolEntry[]) {
  ...
  loadDaisyClawPluginsMock.mockReturnValue(registry);
...
function expectLoaderCall(overrides: Record<string, unknown>) {
  void overrides;
  expect(loadDaisyClawPluginsMock).not.toHaveBeenCalled();
}
...
function expectLoaderSelectedOnlyPluginIds(expectedPluginIds: readonly string[]) {
  const selectedPluginIds = loadDaisyClawPluginsMock.mock.calls.map(
...
describe("resolvePluginTools optional tools", () => {
  ...
  it("does not load plugin-owned tools whose manifest metadata has no available signal", () => {
    ...
    loadDaisyClawPluginsMock.mockImplementation((params) =>
  ...
  it("loads manifest-gated tools when a named account supplies required config", () => {
    ...
    loadDaisyClawPluginsMock.mockReturnValue(
      createToolRegistry([
        {
          pluginId: "feishu",
          optional: false,
  ...
  it("does not reuse a partial active registry for wildcard-selected plugin tools", () => {
    ...
    loadDaisyClawPluginsMock.mockReturnValue(fullRegistry);
    ...
    const loaderParams = mockCallParams(loadDaisyClawPluginsMock) as {
      activate?: unknown;
      cache?: unknown;
  ...
  it("uses the fresh cold-loaded registry for diagnostics when partial active registries remain incomplete", () => {
    ...
    loadDaisyClawPluginsMock.mockReturnValue(freshRegistry);
  ...
  it("does not reuse a pinned gateway registry for manifest-unavailable tools", () => {
    ...
    loadDaisyClawPluginsMock.mockReturnValue({ tools: [], diagnostics: [] });
    ...
    const config = {
      ...
      secrets: {
        providers: {
          vault: {
            source: "file",
            path: "/tmp/daisyclaw-secrets.json",
            mode: "json",
          },
        },
      },
  ...
  it("does not cold-load unrelated manifest-optional plugins when alsoAllow opts into one optional tool", () => {
    ...
    loadDaisyClawPluginsMock.mockReturnValue(createToolRegistry([explicitOptionalEntry]));
    installToolManifestSnapshots({
      config,
      plugins: [
        {
  ...
  it.each([
    {
      ...
      params: {
        env: { DAISYCLAW_HOME: "/srv/daisyclaw-home" } as NodeJS.ProcessEnv,
        toolAllowlist: ["optional_tool"],
      },
      expectedLoaderCall: {
        env: { DAISYCLAW_HOME: "/srv/daisyclaw-home" },
      },
    },
    {
  ...
  it("loads a standalone registry when cached runtime registries lack matching tool entries", () => {
    ...
    resolveRuntimePluginRegistryMock.mockReturnValue(undefined);
    loadDaisyClawPluginsMock.mockReturnValue(loadedRegistry);
  ...
  it("adds enabled non-startup tool plugins to the active tool runtime scope", () => {
    ...
    setActivePluginRegistry(activeRegistry as never, "gateway-startup", "gateway-bindable", "/tmp");
    resolveRuntimePluginRegistryMock.mockReturnValue(activeRegistry);
    loadDaisyClawPluginsMock.mockReturnValue(createToolRegistry([]));
