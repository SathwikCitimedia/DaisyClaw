...
import type {
  DaisyClawPluginHookOptions,
  DaisyClawPluginToolFactory,
  DaisyClawPluginToolOptions,
} from "./tool-types.js";
...
export type {
  DaisyClawPluginActiveModelContext,
  DaisyClawPluginHookOptions,
 ...
 */
export type DaisyClawPluginConfigSchema = {
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<DaisyClawConfig>;
  defaultModel?: string;
  notes?: string[];
  /**
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). DaisyClaw uses this to expose
 ...
 *
 * Runs after DaisyClaw resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common DaisyClaw auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and DaisyClaw wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before DaisyClaw builds its generic
 ...
 * `buildReplayPolicy`, `normalizeToolSchemas`, and `wrapStreamFn`. DaisyClaw no
...
/**
 * Provider-owned stream wrapper hook after DaisyClaw applies its generic
 ...
 *
 * Return `true` or `false` to override DaisyClaw's built-in provider cache TTL
 ...
 *
 * Runs only after DaisyClaw exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that DaisyClaw should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after DaisyClaw loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, DaisyClaw falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * DaisyClaw calls this only from async model resolution paths. After it
   ...
   *
   * Runs after DaisyClaw performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before DaisyClaw registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic DaisyClaw wrappers.
   *
   ...
   *
   * Called after DaisyClaw resolves the raw configured credential but before the
   ...
   * hint than DaisyClaw's generic auth-store guidance.
   */
   ...
   * DaisyClaw's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when DaisyClaw should surface a provider-specific hint for
   ...
   * merging. DaisyClaw deduplicates by `provider/id`, so plugins only need to
   ...
   * or model exposes a custom set of thinking levels. DaisyClaw stores the
   ...
   * without replacing the full DaisyClaw-owned system prompt.
   */
   ...
   *
   * Runs after DaisyClaw's built-in overlay is resolved and before the
   ...
   *
   * Return undefined/null to keep DaisyClaw's default fallback policy.
   */
   ...
   * rewrites after DaisyClaw has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before DaisyClaw handles
  ...
  ) => DaisyClawConfig | null | undefined;
  /**
   ...
   *
   * DaisyClaw uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `daisyclaw doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `daisyclaw doctor` should offer.
   *
   ...
   *
   * DaisyClaw calls this before falling back to the shared `shared model runtime` OAuth
   ...
   * path. Return nothing to keep DaisyClaw's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * DaisyClaw falls back to that stored profile.
   */
...
export type DaisyClawPluginGatewayMethod = {
  method: string;
...
export type PluginCommandDiagnosticsSession = {
  ...
  sessionKey?: string;
  /** Ephemeral DaisyClaw session id when available. */
  sessionId?: string;
  /** Transcript file for this DaisyClaw session when available. */
  sessionFile?: string;
...
export type PluginCommandContext = {
  ...
  sessionId?: string;
  /** Transcript file for the active DaisyClaw session when available. */
  sessionFile?: string;
  /** Raw command arguments after the command name */
  args?: string;
  /** The full normalized command body */
  commandBody: string;
  /** Current DaisyClaw configuration */
  config: DaisyClawConfig;
...
export type DaisyClawPluginCommandDefinition = {
...
export type DaisyClawPluginHttpRouteAuth = "gateway" | "plugin";
export type DaisyClawPluginHttpRouteMatch = "exact" | "prefix";
export type DaisyClawPluginGatewayRuntimeScopeSurface = "write-default" | "trusted-operator";

export type DaisyClawPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type DaisyClawPluginHttpRouteUpgradeHandler = (
  req: IncomingMessage,
  socket: Duplex,
  head: Buffer,
) => Promise<boolean | void> | boolean | void;

export type DaisyClawPluginHttpRouteParams = {
  path: string;
  handler: DaisyClawPluginHttpRouteHandler;
  handleUpgrade?: DaisyClawPluginHttpRouteUpgradeHandler;
  auth: DaisyClawPluginHttpRouteAuth;
  match?: DaisyClawPluginHttpRouteMatch;
  gatewayRuntimeScopeSurface?: DaisyClawPluginGatewayRuntimeScopeSurface;
  nodeCapability?: {
    surface: string;
    ttlMs?: number;
  };
...
export type DaisyClawPluginHostedMediaResolver = (
  mediaUrl: string,
) => string | null | undefined | Promise<string | null | undefined>;

export type DaisyClawPluginCliContext = {
  /**
   * Command object where this plugin should register its commands.
   *
   * For root CLI registrations this is the root `daisyclaw` program. For nested
...
export type DaisyClawPluginCliRegistrar = (ctx: DaisyClawPluginCliContext) => void | Promise<void>;

/**
 * Top-level CLI metadata for plugin-owned commands.
 *
 * Descriptors are the parse-time contract for lazy plugin CLI registration.
 * If you want DaisyClaw to keep a plugin command lazy-loaded while still
 ...
 */
export type DaisyClawPluginCliCommandDescriptor = {
  name: string;
  description: string;
  hasSubcommands: boolean;
};

export type DaisyClawPluginNodeCliFeatureOptions = {
  /** Explicit node feature command names owned under `daisyclaw nodes`. */
  commands?: string[];
  /**
   * Parse-time command descriptors for lazy node feature CLI registration.
   *
   * Descriptors are registered under `daisyclaw nodes`, so a descriptor named
   * `"camera"` exposes `daisyclaw nodes camera`.
   */
  descriptors?: DaisyClawPluginCliCommandDescriptor[];
};

export type DaisyClawPluginReloadRegistration = {
  restartPrefixes?: string[];
  hotPrefixes?: string[];
  noopPrefixes?: string[];
};

export type DaisyClawPluginNodeHostCommand = {
  command: string;
  cap?: string;
  dangerous?: boolean;
...
export type DaisyClawPluginNodeInvokeTransportResult =
  | {
      ok: true;
      payload?: unknown;
...
export type DaisyClawPluginNodeInvokeApprovalDecision = "allow-once" | "allow-always" | "deny";

export type DaisyClawPluginNodeInvokePolicyApprovalRuntime = {
  request: (input: {
    title: string;
    description: string;
    ...
    id?: string;
    decision?: DaisyClawPluginNodeInvokeApprovalDecision | null;
  }>;
};

export type DaisyClawPluginNodeInvokePolicyContext = {
  nodeId: string;
  command: string;
  params: unknown;
  timeoutMs?: number;
  ...
  approvals?: DaisyClawPluginNodeInvokePolicyApprovalRuntime;
  invokeNode: (input?: {
    params?: unknown;
    timeoutMs?: number;
    idempotencyKey?: string;
  }) => Promise<DaisyClawPluginNodeInvokeTransportResult>;
};

export type DaisyClawPluginNodeInvokePolicyResult =
  | {
      ok: true;
      payload?: unknown;
      payloadJSON?: string | null;
...
export type DaisyClawPluginNodeInvokePolicy = {
  commands: string[];
  /**
  ...
  handle: (
    ctx: DaisyClawPluginNodeInvokePolicyContext,
  ) => Promise<DaisyClawPluginNodeInvokePolicyResult> | DaisyClawPluginNodeInvokePolicyResult;
};

export type DaisyClawPluginSecurityAuditContext = {
  config: DaisyClawConfig;
  sourceConfig: DaisyClawConfig;
  env: NodeJS.ProcessEnv;
  stateDir: string;
  configPath: string;
};

export type DaisyClawPluginSecurityAuditCollector = (
  ctx: DaisyClawPluginSecurityAuditContext,
) => SecurityAuditFinding[] | Promise<SecurityAuditFinding[]>;

export type DaisyClawGatewayDiscoveryAdvertiseContext = {
...
export type DaisyClawGatewayDiscoveryService = {
  id: string;
  advertise: (
    ctx: DaisyClawGatewayDiscoveryAdvertiseContext,
  ) => void | Promise<void | { stop?: () => void | Promise<void> }>;
};

/** Context passed to long-lived plugin services. */
export type DaisyClawPluginServiceContext = {
  config: DaisyClawConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type DaisyClawPluginService = {
  id: string;
  start: (ctx: DaisyClawPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: DaisyClawPluginServiceContext) => void | Promise<void>;
};

export type DaisyClawPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type DaisyClawPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  /**
   * @deprecated Declare exclusive plugin kind in `daisyclaw.plugin.json` via
  ...
  configSchema?: DaisyClawPluginConfigSchema;
  reload?: DaisyClawPluginReloadRegistration;
  nodeHostCommands?: DaisyClawPluginNodeHostCommand[];
  securityAuditCollectors?: DaisyClawPluginSecurityAuditCollector[];
  register?: (api: DaisyClawPluginApi) => void;
  activate?: (api: DaisyClawPluginApi) => void;
};

export type DaisyClawPluginModule = DaisyClawPluginDefinition | ((api: DaisyClawPluginApi) => void);
...
export type PluginConfigMigration = (config: DaisyClawConfig) =>
  | {
      config: DaisyClawConfig;
      changes: string[];
    }
...
/** Migration source implemented by a plugin and orchestrated by `daisyclaw migrate`. */
...
export type DaisyClawPluginSessionStateApi = {
...
export type DaisyClawPluginSessionWorkflowApi = {
...
export type DaisyClawPluginSessionControlsApi = {
...
export type DaisyClawPluginSessionApi = {
  state: DaisyClawPluginSessionStateApi;
  workflow: DaisyClawPluginSessionWorkflowApi;
  controls: DaisyClawPluginSessionControlsApi;
};

export type DaisyClawPluginAgentEventsApi = {
...
export type DaisyClawPluginAgentApi = {
  events: DaisyClawPluginAgentEventsApi;
};

export type DaisyClawPluginRunContextApi = {
...
export type DaisyClawPluginLifecycleApi = {
...
export type DaisyClawPluginApi = {
  id: string;
  name: string;
  version?: string;
  description?: string;
   ...
   */
  session: DaisyClawPluginSessionApi;
  /** Grouped facade for agent-event workflow seams. */
  agent: DaisyClawPluginAgentApi;
  /** Grouped facade for run-scoped plugin scratch state. */
  runContext: DaisyClawPluginRunContextApi;
  /** Grouped facade for plugin-owned lifecycle cleanup hooks. */
  lifecycle: DaisyClawPluginLifecycleApi;
  registerTool: (
    tool: AnyAgentTool | DaisyClawPluginToolFactory,
    opts?: DaisyClawPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: DaisyClawPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: DaisyClawPluginHttpRouteParams) => void;
  /** Register a plugin-owned resolver for browser-style hosted media URLs. */
  registerHostedMediaResolver: (resolver: DaisyClawPluginHostedMediaResolver) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: DaisyClawPluginChannelRegistration | ChannelPlugin) => void;
  /**
  ...
  registerCli: (
    registrar: DaisyClawPluginCliRegistrar,
    opts?: {
       ...
       *
       * When descriptors cover every command exposed at `parentPath`, DaisyClaw
  ...
  /**
   * Register a plugin-owned node feature command group under `daisyclaw nodes`.
   *
  ...
  registerNodeCliFeature: (
    registrar: DaisyClawPluginCliRegistrar,
    opts?: DaisyClawPluginNodeCliFeatureOptions,
  ) => void;
  registerReload: (registration: DaisyClawPluginReloadRegistration) => void;
  registerNodeHostCommand: (command: DaisyClawPluginNodeHostCommand) => void;
  registerNodeInvokePolicy: (policy: DaisyClawPluginNodeInvokePolicy) => void;
  registerSecurityAuditCollector: (collector: DaisyClawPluginSecurityAuditCollector) => void;
  registerService: (service: DaisyClawPluginService) => void;
  /** Register a local gateway discovery advertiser such as mDNS/Bonjour. */
  registerGatewayDiscoveryService: (service: DaisyClawGatewayDiscoveryService) => void;
  ...
  /** Register an importer for `daisyclaw migrate` (migration capability). */
   ...
   */
  registerCommand: (command: DaisyClawPluginCommandDefinition) => void;
