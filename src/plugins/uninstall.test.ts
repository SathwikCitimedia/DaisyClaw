...
type PluginConfig = NonNullable<DaisyClawConfig["plugins"]>;
...
function createPluginConfig(params: {
  ...
  loadPaths?: string[];
  channels?: DaisyClawConfig["channels"];
}): DaisyClawConfig {
...
function createSinglePluginWithEmptySlotsConfig(): DaisyClawConfig {
...
function createSingleNpmInstallConfig(installPath: string): DaisyClawConfig {
  return createPluginConfig({
    entries: createSinglePluginEntries(),
    installs: {
...
describe("removePluginFromConfig", () => {
  ...
  it("removes absolute load path for a workspace-relative install source path", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(tempRoot, "daisyclaw-uninstall-portable-source-"));
    try {
    ...
    },
    {
      ...
      config: createPluginConfig({
        ...
        channels: {
          ...
          timbot: { sdkAppId: "123" },
        } as unknown as DaisyClawConfig["channels"],
      }),
      pluginId: "timbot",
      expectedChannels: {
        defaults: { groupPolicy: "opt-in" },
...
describe("uninstallPlugin", () => {
      ...
      config: createPluginConfig({
        installs: {
          "missing-linked-plugin": createPathInstallRecord(
            "/missing/daisyclaw/plugin",
            "/missing/daisyclaw/plugin",
          ),
        },
        loadPaths: ["/missing/daisyclaw/plugin", "/keep/this/plugin"],
      }),
      expectedActions: {
        entry: false,
        install: true,
  ...
  it("uninstalls npm-managed packages through npm before deleting the package directory", async () => {
    ...
    const pluginDir = path.join(npmRoot, "node_modules", "@daisyclaw", "kitchen-sink");
    ...
    const plan = planPluginUninstall({
      config: createPluginConfig({
        entries: createSinglePluginEntries("daisyclaw-kitchen-sink-fixture"),
        installs: {
          "daisyclaw-kitchen-sink-fixture": {
            source: "npm",
            spec: "@daisyclaw/kitchen-sink@1.0.0",
            installPath: pluginDir,
          },
        },
      }),
    ...
    expect(plan.directoryRemoval).toEqual({
      ...
      cleanup: {
        kind: "npm",
        npmRoot,
        packageName: "@daisyclaw/kitchen-sink",
      },
    });
    ...
    expect(applied).toEqual({ directoryRemoved: true, warnings: [] });
    expectNpmUninstallCommand({ packageName: "@daisyclaw/kitchen-sink", npmRoot });
    await expectPathAccessState(pluginDir, "missing");
  });
  ...
  it("repairs remaining npm plugin daisyclaw peer links after npm uninstall prunes them", async () => {
    const stateDir = path.join(tempDir, "state");
    const npmRoot = path.join(stateDir, "npm");
    ...
    const peerLink = path.join(peerPluginDir, "node_modules", "daisyclaw");
    ...
    await expectPathAccessState(path.join(npmRoot, "node_modules", "daisyclaw"), "missing");
  ...
  it("prunes managed peer dependencies after their owning npm plugin is uninstalled", async () => {
    ...
    expect(rootManifest.daisyclaw?.managedPeerDependencies ?? []).not.toContain("runtime-peer");
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("returns managed install path for copied path installs", () => {
    const extensionsDir = path.join(os.tmpdir(), "daisyclaw-uninstall-safe");
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    ...
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        ...
        installPath: "/tmp/not-daisyclaw-plugin-install/my-plugin",
      },
      extensionsDir,
    });
  ...
  it("uses configured installPath when npm installed it under the managed npm root", () => {
    const stateDir = path.join(os.tmpdir(), "daisyclaw-uninstall-safe");
    const extensionsDir = path.join(stateDir, "extensions");
    const installPath = path.join(stateDir, "npm", "node_modules", "@daisyclaw", "kitchen-sink");
  ...
  it("uses configured installPath when npm installed it under a managed npm project", () => {
    ...
    const installPath = path.join(npmRoot, "node_modules", "@daisyclaw", "kitchen-sink");
    ...
    expect(
      resolveUninstallDirectoryTarget({
        ...
        installRecord: {
          source: "npm",
          spec: "@daisyclaw/kitchen-sink@latest",
          installPath: path.join(os.tmpdir(), "npm", "node_modules", "@daisyclaw", "kitchen-sink"),
        },
        extensionsDir,
      }),
    ).toBe(resolvePluginInstallDir("daisyclaw-kitchen-sink-fixture", extensionsDir));
  });

  it("uses configured installPath when it is under the recorded managed extensions root", () => {
    const currentExtensionsDir = path.join(os.tmpdir(), "daisyclaw-uninstall-current", "extensions");
    const recordedExtensionsDir = path.join(
      os.tmpdir(),
      "daisyclaw-uninstall-recorded",
      "extensions",
    );
    const installPath = resolvePluginInstallDir("my-plugin", recordedExtensionsDir);
