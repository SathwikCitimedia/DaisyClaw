...
import {
  ...
  installedPackageNeedsDaisyClawPeerLinkRepair,
...
import { linkDaisyClawPeerDependencies } from "./plugin-peer-link.js";
...
function compareNpmSemverForUpdate(left: string, right: string): number {
  const releaseCmp = compareDaisyClawReleaseVersions(left, right);
  if (releaseCmp !== null) {
    return releaseCmp;
  }
...
> {
  ...
  if (
    !parsedSpec ||
    !parsedSpec.name.startsWith("@daisyclaw/") ||
...
function isNpmMetadataCompatibleWithCurrentHost(metadata: NpmSpecResolution): boolean {
  ...
  const installMetadata = metadata.packageDaisyClaw?.install;
  ...
  const pluginApiRangeCheck = resolvePackagePluginApiRange(metadata.packageDaisyClaw);
...
function isBundledVersionNewer(bundledVersion: string, installedVersion: string): boolean {
  const releaseCmp = compareDaisyClawReleaseVersions(bundledVersion, installedVersion);
  if (releaseCmp !== null) {
    return releaseCmp > 0;
  }
...
function shouldFallbackClawHubBridgeToNpm(params: {
  ...
  if (!isDaisyClawOrgNpmSpec(params.npmSpec)) {
    return false;
  }
  return (
...
function migratePluginConfigId(cfg: DaisyClawConfig, fromId: string, toId: string): DaisyClawConfig {
  if (fromId === toId) {
    return cfg;
  }
...
function withoutPluginInstallRecord(cfg: DaisyClawConfig, pluginId: string): DaisyClawConfig {
  const installs = cfg.plugins?.installs;
...
function resetDisabledPluginSlots(
  slots: NonNullable<DaisyClawConfig["plugins"]>["slots"] | undefined,
  pluginId: string,
): NonNullable<DaisyClawConfig["plugins"]>["slots"] | undefined {
  if (!slots) {
    return slots;
  }
  let next = slots;
...
function disablePluginConfigEntry(config: DaisyClawConfig, pluginId: string): DaisyClawConfig {
...
async function repairDaisyClawPeerLinksForNpmInstalls(params: {
  config: DaisyClawConfig;
  logger: PluginUpdateLogger;
}): Promise<boolean> {
  let repaired = false;
  for (const [pluginId, record] of Object.entries(params.config.plugins?.installs ?? {})) {
    ...
    try {
      ...
      params.logger.warn?.(
        `Could not repair daisyclaw peer link for "${pluginId}" due to invalid install path: ${String(err)}`,
      );
      continue;
    }

    if (!installedPackageNeedsDaisyClawPeerLinkRepair(installPath)) {
      continue;
    }

    const peerDependencies = readInstalledPackagePeerDependencies(installPath);
    if (!Object.hasOwn(peerDependencies, "daisyclaw")) {
      continue;
    }
    ...
    try {
      ...
      if (peerLinkRepair.skipped > 0) {
        params.logger.warn?.(
          `Could not repair daisyclaw peer link for "${pluginId}" at ${installPath}: ${warnings.join("; ") || "peer link repair was skipped"}`,
        );
        continue;
      }
      repaired = !installedPackageNeedsDaisyClawPeerLinkRepair(installPath) || repaired;
    } catch (err) {
      params.logger.warn?.(
        `Could not repair daisyclaw peer link for "${pluginId}" at ${installPath}: ${String(err)}`,
      );
    }
  }
  return repaired;
...
export async function updateNpmInstalledPlugins(params: {
  ...
  const recordFailure = (
    ...
    if (params.disableOnFailure && !params.dryRun) {
      const disabledMessage =
        `Disabled "${pluginId}" after plugin update failure; DaisyClaw will continue without it. ` +
        message;
    ...
    if (
      ...
      if (metadataResult.ok) {
        ...
        if (
          ...
          !installedPackageNeedsDaisyClawPeerLinkRepair(installPath) &&
  ...
  if (ranNpmInstaller) {
    changed =
      (await repairDaisyClawPeerLinksForNpmInstalls({
        config: next,
        logger,
      })) || changed;
  }
