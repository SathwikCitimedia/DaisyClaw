...
vi.mock("./loader.js", () => ({
  ...
  loadDaisyClawPlugins: mocks.loadDaisyClawPlugins,
...
describe("web-provider-runtime-shared", () => {
  ...
  it("caches setup web provider plugin loads by default", () => {
    ...
    const mapRegistryProviders = vi.fn(() => ["provider"]);
    mocks.loadDaisyClawPlugins.mockReturnValue(loadedRegistry as never);
    ...
    expect(providers).toEqual(["provider"]);
    expect(mockArg(mocks.loadDaisyClawPlugins).cache).toBe(true);
    expect(mockArg(mocks.loadDaisyClawPlugins).onlyPluginIds).toEqual(["brave"]);
  });
  ...
  it("falls back to a scoped provider load when the active runtime registry has no web providers", () => {
    ...
    mocks.loadDaisyClawPlugins.mockReturnValue(fallbackRegistry as never);
  ...
  it("keeps explicit setup web provider cache opt-outs", () => {
    ...
    expect(mockArg(mocks.loadDaisyClawPlugins).cache).toBe(false);
    expect(mockArg(mocks.loadDaisyClawPlugins).onlyPluginIds).toEqual(["brave"]);
  });
});
