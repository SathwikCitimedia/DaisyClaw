...
import { normalizeOptionalString as normalizeString } from "@daisyclaw/normalization-core/string-coerce";
...
function getConfigPath(config: DaisyClawConfig | undefined, path: string): unknown {
...
function setConfigPath(target: DaisyClawConfig, path: string, value: unknown): void {
...
function buildProviderEntry(params: {
  ...
  return {
    ...
    getConfiguredCredentialValue: (config?: DaisyClawConfig) =>
      getConfigPath(config, credentialPath),
    setConfiguredCredentialValue: (configTarget: DaisyClawConfig, value: unknown) => {
      setConfigPath(configTarget, credentialPath, value);
    },
    applySelectionConfig: (config: DaisyClawConfig) =>
