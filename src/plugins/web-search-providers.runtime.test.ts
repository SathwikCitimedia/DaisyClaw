...
let loadDaisyClawPluginsMock: ReturnType<typeof vi.fn>;
...
function createWebSearchEnv(overrides?: Partial<NodeJS.ProcessEnv>) {
  return {
    DAISYCLAW_HOME: "/tmp/daisyclaw-home",
    ...overrides,
    ...
    plugins: [
      {
        ...
        manifestPath: "/tmp/brave/daisyclaw.plugin.json",
        channels: [],
        providers: [],
        cliBackends: [],
        syntheticAuthRefs: [],
      ...
      },
      {
        ...
        manifestPath: "/tmp/noise/daisyclaw.plugin.json",
        channels: [],
        providers: [],
        cliBackends: [],
        syntheticAuthRefs: [],
...
function expectLoaderCallCount(count: number) {
  expect(loadDaisyClawPluginsMock).toHaveBeenCalledTimes(count);
}
...
function expectScopedWebSearchCandidates(pluginIds: readonly string[]) {
  ...
  expect(
    requireLastCallFirstArg(loadDaisyClawPluginsMock, "loadDaisyClawPlugins").onlyPluginIds,
  ).toEqual([...pluginIds]);
}
...
function expectAutoEnabledWebSearchLoad(params: {
  ...
  const loaderParams = requireLastCallFirstArg(loadDaisyClawPluginsMock, "loadDaisyClawPlugins");
...
describe("resolvePluginWebSearchProviders", () => {
  ...
  beforeEach(() => {
    ...
    loadDaisyClawPluginsMock = vi
      .spyOn(loaderModule, "loadDaisyClawPlugins")
  ...
  it("resolves current config contents when config changes in place", () => {
    ...
    const env = createWebSearchEnv({ DAISYCLAW_HOME: "/tmp/daisyclaw-home-a" });
  ...
  it("resolves current env contents when env changes in place", () => {
    ...
    expectSnapshotLoaderCalls({
      ...
      env,
      mutate: () => {
        env.DAISYCLAW_HOME = "/tmp/daisyclaw-home-b";
      },
      expectedLoaderCalls: 2,
    });
  });
  ...
  it("does not reuse snapshot provider loads across host Vitest env changes", () => {
    ...
    expect(loadDaisyClawPluginsMock).toHaveBeenCalledTimes(2);
  });
