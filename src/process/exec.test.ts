...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        DAISYCLAW_BASE_ENV: "base",
        DAISYCLAW_TO_REMOVE: undefined,
      },
      env: {
        DAISYCLAW_TEST_ENV: "ok",
      },
    });

    expect(resolved.DAISYCLAW_BASE_ENV).toBe("base");
    expect(resolved.DAISYCLAW_TEST_ENV).toBe("ok");
    expect(resolved.DAISYCLAW_TO_REMOVE).toBeUndefined();
    expect(resolved.DAISYCLAW_CLI).toBe(DAISYCLAW_CLI_ENV_VALUE);
  });
