...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[daisyclaw proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[daisyclaw proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`daisyclaw proxy coverage\` for details.`,
  );
}
