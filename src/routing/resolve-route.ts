...
function listAgents(cfg: DaisyClawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: DaisyClawConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<DaisyClawConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: DaisyClawConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: DaisyClawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: DaisyClawConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<DaisyClawConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  DaisyClawConfig,
  {
    bindingsRef: DaisyClawConfig["bindings"];
    agentsRef: DaisyClawConfig["agents"];
    sessionRef: DaisyClawConfig["session"];
...
function resolveRouteCacheForConfig(cfg: DaisyClawConfig): Map<string, ResolvedAgentRoute> {
