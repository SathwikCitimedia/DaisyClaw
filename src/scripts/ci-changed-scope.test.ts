...
describe("detectChangedScope", () => {
  ...
  it("keeps node lane off for native-only changes", () => {
    ...
    expect(
      detectChangedScope(["apps/macos-mlx-tts/Sources/DaisyClawMLXTTSHelper/main.swift"]),
    ).toEqual({
      runNode: false,
      runMacos: true,
      runAndroid: false,
    ...
    expect(detectChangedScope(["apps/shared/DaisyClawKit/Sources/Foo.swift"])).toEqual({
      runNode: false,
      runMacos: true,
      runAndroid: true,
      runWindows: false,
  ...
  it("does not force macOS for generated protocol model-only changes", () => {
    expect(
      detectChangedScope(["apps/shared/DaisyClawKit/Sources/DaisyClawProtocol/GatewayModels.swift"]),
    ).toEqual({
      runNode: false,
      runMacos: false,
      runAndroid: false,
  ...
  it("treats base and head as literal git args", () => {
    const markerPath = path.join(
      os.tmpdir(),
      `daisyclaw-ci-changed-scope-${Date.now()}-${Math.random().toString(16).slice(2)}.tmp`,
    );
  ...
  it("uses the merge commit first parent instead of a stale PR payload base", () => {
    const { repoDir, staleBase } = createSyntheticMergeRepo("daisyclaw-ci-scope-merge-");
  ...
  it("keeps direct CLI preflight empty diffs as no-op scope", () => {
    const repoDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-ci-scope-empty-"));
    tempDirs.push(repoDir);
