...
describe("syncPluginVersions", () => {
  ...
  it("preserves workspace daisyclaw devDependencies and plugin host floors", () => {
    const rootDir = makeTempDir(tempDirs, "daisyclaw-sync-plugin-versions-");
    ...
    expect(summary.updated).toContain("@daisyclaw/imessage");
    expect(updatedPackage.version).toBe("2026.4.1");
    expect(updatedPackage.devDependencies?.daisyclaw).toBe("workspace:*");
    expect(updatedPackage.peerDependencies?.daisyclaw).toBe(">=2026.4.1");
    expect(updatedPackage.daisyclaw?.install?.minHostVersion).toBe(">=2026.3.30");
    expect(updatedPackage.daisyclaw?.compat?.pluginApi).toBe(">=2026.4.1");
    expect(updatedPackage.daisyclaw?.build?.daisyclawVersion).toBe("2026.4.1");
  });

  it("reports pending version sync without writing in check mode", () => {
    const rootDir = makeTempDir(tempDirs, "daisyclaw-sync-plugin-versions-check-");
    ...
    expect(summary.updated).toEqual(["@daisyclaw/discord"]);
    expect(unchangedPackage.version).toBe("2026.4.1");
    expect(unchangedPackage.peerDependencies?.daisyclaw).toBe(">=2026.4.1");
    expect(unchangedPackage.daisyclaw?.compat?.pluginApi).toBe(">=2026.4.1");
  });

  it("uses the base release version for beta changelog entries", () => {
    const rootDir = makeTempDir(tempDirs, "daisyclaw-sync-plugin-versions-beta-changelog-");
    ...
    expect(summary.changelogged).toEqual(["@daisyclaw/matrix"]);
