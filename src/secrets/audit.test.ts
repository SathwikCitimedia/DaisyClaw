...
async function writeJsonFile(filePath: string, value: unknown): Promise<void> {
  if (path.basename(filePath) === "daisyclaw-agent.sqlite") {
...
async function createAuditFixture(): Promise<AuditFixture> {
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-secrets-audit-"));
  const stateDir = path.join(rootDir, ".daisyclaw");
...
describe("secrets audit", () => {
  ...
  it("scans active agent-dir override models.json even when outside state dir", async () => {
    ...
    const report = await runSecretsAudit({
      env: {
        ...fixture.env,
        DAISYCLAW_AGENT_DIR: externalAgentDir,
      },
    });
    expect(
      hasFinding(
  ...
  it("does not flag non-sensitive routing headers in daisyclaw config", async () => {
    await writeJsonFile(fixture.configPath, {
      models: {
        providers: {
          openai: {
  ...
  it("keeps request headers in daisyclaw config covered by plaintext audit", async () => {
    await writeJsonFile(fixture.configPath, {
      models: {
        providers: {
          openai: {
