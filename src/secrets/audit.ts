...
import { resolveStateDir, type DaisyClawConfig } from "../config/config.js";
...
/** Stable finding codes emitted by `daisyclaw secrets audit`. */
...
export async function runSecretsAudit(
  ...
  const config = snapshot.valid ? snapshot.config : ({} as DaisyClawConfig);
  let resolution = {
    refsChecked: 0,
    skippedExecRefs: 0,
