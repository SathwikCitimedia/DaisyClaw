...
function writeExternalChannelPlugin(params: { pluginId: string; channelId: string }) {
  const rootDir = makeTrackedTempDir("daisyclaw-channel-secret-contract", tempDirs);
  fs.writeFileSync(
...
describe("external channel secret contract api", () => {
  ...
  it("loads dist/ secret-contract-api sidecars for compiled npm-published external channel plugins", () => {
    const rootDir = makeTrackedTempDir("daisyclaw-channel-secret-contract-dist", tempDirs);
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const rootDir = makeTrackedTempDir("daisyclaw-channel-secret-contract-hardlink", tempDirs);
      const outsideDir = makeTrackedTempDir(
        "daisyclaw-channel-secret-contract-hardlink-outside",
        tempDirs,
      );
      ...
      const env = { DAISYCLAW_NIX_MODE: "1" };
      loadPluginMetadataSnapshotMock.mockReturnValue({
        plugins: [record],
      });
