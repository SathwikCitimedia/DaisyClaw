...
function resolvePluginContractApiPath(rootDir: string): string | null {
  ...
  // (per package.json `daisyclaw.runtimeExtensions`), while flat-layout plugins keep
  // them at <rootDir>/. Search both, preferring dist/ when running from built daisyclaw
...
function loadExternalChannelSecretContractFromRecord(
  ...
  try {
    ...
    if (process.env.DAISYCLAW_DEBUG_CHANNEL_CONTRACT_API === "1") {
