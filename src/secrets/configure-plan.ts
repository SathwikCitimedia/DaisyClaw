/** Builds the interactive `daisyclaw secrets configure` target list and apply plan. */
...
/** Credential target shown by `daisyclaw secrets configure` before a SecretRef is selected. */
...
function getSecretProviders(config: DaisyClawConfig): Record<string, SecretProviderConfig> {
...
/** Builds configure candidates for the current DaisyClaw config only. */
export function buildConfigureCandidates(config: DaisyClawConfig): ConfigureCandidate[] {
...
function configureCandidateSortKey(candidate: ConfigureCandidate): string {
  ...
  return `daisyclaw:${candidate.path}`;
}
...
/** Builds configure candidates for DaisyClaw config plus an optional auth-profile scope. */
export function buildConfigureCandidatesForScope(params: {
  config: DaisyClawConfig;
  authoredDaisyClawConfig?: DaisyClawConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredDaisyClawConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const daisyclawCandidates = discoverConfigSecretTargets(params.config)
      ...
      return Object.assign(
        {
          ...
          label: entry.path,
          configFile: `daisyclaw.json` as const,
  ...
  return [...daisyclawCandidates, ...authCandidates].toSorted((a, b) =>
...
export function buildSecretsConfigurePlan(params: {
  ...
  return {
    ...
    generatedBy: "daisyclaw secrets configure",
