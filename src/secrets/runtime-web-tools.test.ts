...
function setConfiguredFetchProviderKey(configTarget: DaisyClawConfig, value: unknown): void {
  const plugins = ensureRecord(configTarget as Record<string, unknown>, "plugins");
  const entries = ensureRecord(plugins, "entries");
...
async function runRuntimeWebTools(params: { config: DaisyClawConfig; env?: NodeJS.ProcessEnv }) {
...
function readProviderKey(config: DaisyClawConfig, provider: ProviderUnderTest): unknown {
...
describe("runtime web tools resolution", () => {
  ...
  it("uses runtime web search discovery when the managed plugin index install records is populated", async () => {
    loadInstalledPluginIndexInstallRecordsSyncMock.mockReturnValue({
      "external-search": {
        source: "npm",
        spec: "@daisyclaw/external-search",
      },
    });
  ...
  describe("when brave is installed as an external plugin and explicitly configured", () => {
    ...
    beforeEach(() => {
      loadInstalledPluginIndexInstallRecordsSyncMock.mockReturnValue({
        brave: { source: "npm", spec: "@daisyclaw/brave-search" },
      });
