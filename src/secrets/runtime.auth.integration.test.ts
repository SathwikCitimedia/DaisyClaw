...
describe("secrets runtime snapshot auth integration", () => {
  ...
  it("recomputes config-derived agent dirs when refreshing active secrets runtime snapshots", async () => {
    await withTempHome("daisyclaw-secrets-runtime-agent-dirs-", async (home) => {
      const mainAgentDir = path.join(home, ".daisyclaw", "agents", "main", "agent");
      const opsAgentDir = path.join(home, ".daisyclaw", "agents", "ops", "agent");
