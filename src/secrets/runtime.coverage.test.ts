...
function createCoverageWebSearchProvider(params: {
  ...
  const readConfiguredCredential = (config?: DaisyClawConfig): unknown =>
...
type SecretRegistryEntry = {
  id: string;
  configFile: "daisyclaw.json" | "auth-profiles.json";
  pathPattern: string;
...
const DEBUG_COVERAGE_BATCHES = process.env.DAISYCLAW_DEBUG_RUNTIME_COVERAGE === "1";
...
const PLUGIN_OWNED_DAISYCLAW_COVERAGE_EXCLUSIONS = new Set([
...
const previousTrustBundledPluginsDir = process.env.DAISYCLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR;

process.env.DAISYCLAW_BUNDLED_PLUGINS_DIR ??= "extensions";
process.env.DAISYCLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR ??= "1";
...
afterAll(() => {
  ...
  if (previousTrustBundledPluginsDir === undefined) {
    ...
    process.env.DAISYCLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR = previousTrustBundledPluginsDir;
  }
});
...
function collectDaisyClawCoverageEntries(options: {
  includePluginEntries: boolean;
}): SecretRegistryEntry[] {
  return COVERAGE_REGISTRY_ENTRIES.filter(
    (entry) =>
      entry.configFile === "daisyclaw.json" &&
      entry.id.startsWith("plugins.entries.") === options.includePluginEntries &&
      !PLUGIN_OWNED_DAISYCLAW_COVERAGE_EXCLUSIONS.has(entry.id),
  );
}

function applyConfigForDaisyClawTarget(
...
async function expectDaisyClawCoverageBatchResolved(
  label: string,
  batch: readonly SecretRegistryEntry[],
): Promise<void> {
  logCoverageBatch(label, batch);
  const config = {} as DaisyClawConfig;
  const env: Record<string, string> = {};
  for (const [index, entry] of batch.entries()) {
    const envId = `DAISYCLAW_SECRET_TARGET_${entry.id}`;
    ...
    applyConfigForDaisyClawTarget(config, entry, envId, wildcardToken);
  }
...
const DAISYCLAW_CORE_COVERAGE_BATCHES = buildCoverageBatches(
  collectDaisyClawCoverageEntries({ includePluginEntries: false }),
);
const DAISYCLAW_PLUGIN_COVERAGE_BATCHES = buildCoverageBatches(
  collectDaisyClawCoverageEntries({ includePluginEntries: true }),
);
...
describe("secrets runtime target coverage", () => {
  beforeAll(async () => {
    ...
    const googleChatBatch = DAISYCLAW_CORE_COVERAGE_BATCHES.find((batch) =>
      batch.some((entry) => entry.id === "channels.googlechat.serviceAccount"),
    );
    if (googleChatBatch) {
      await expectDaisyClawCoverageBatchResolved("daisyclaw.json core", googleChatBatch);
    }
  });

  describe("daisyclaw.json core and channel registry targets", () => {
    test.each(DAISYCLAW_CORE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectDaisyClawCoverageBatchResolved("daisyclaw.json core", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });

  describe("daisyclaw.json plugin registry targets", () => {
    test.each(DAISYCLAW_PLUGIN_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectDaisyClawCoverageBatchResolved("daisyclaw.json plugins", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });
  ...
  describe("auth-profiles registry targets", () => {
    test.each(AUTH_PROFILE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      ...
      async ({ batch }) => {
        ...
        for (const [index, entry] of batch.entries()) {
          const envId = `DAISYCLAW_AUTH_SECRET_TARGET_${entry.id}`;
        ...
        const snapshot = await prepareAuthCoverageSnapshot({
          ...
          env,
          agentDirs: ["/tmp/daisyclaw-agent-main"],
