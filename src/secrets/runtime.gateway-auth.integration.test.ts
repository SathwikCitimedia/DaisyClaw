...
describe("secrets runtime snapshot gateway-auth integration", () => {
  ...
  it(
    ...
    async () => {
      await withTempHome("daisyclaw-secrets-runtime-gateway-auth-reload-lkg-", async (home) => {
        ...
        await fs.writeFile(
          path.join(home, ".daisyclaw", "daisyclaw.json"),
          `${JSON.stringify(
            {
              gateway: {
                auth: {
        ...
        const persistedConfig = JSON.parse(
          await fs.readFile(path.join(home, ".daisyclaw", "daisyclaw.json"), "utf8"),
        ) as DaisyClawConfig;
