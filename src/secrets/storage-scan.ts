...
import { isRecord as isJsonObject } from "@daisyclaw/normalization-core/record-coerce";
...
export function listAuthProfileStorePaths(config: DaisyClawConfig, stateDir: string): string[] {
...
export function listAuthProfileStoreAgentDirs(config: DaisyClawConfig, stateDir: string): string[] {
...
function resolveActiveAgentDir(stateDir: string, env: NodeJS.ProcessEnv = process.env): string {
  const override = env.DAISYCLAW_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim();
  if (override) {
