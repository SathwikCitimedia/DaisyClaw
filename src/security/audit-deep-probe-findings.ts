...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("daisyclaw status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("daisyclaw security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set DAISYCLAW_GATEWAY_TOKEN/DAISYCLAW_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("daisyclaw security audit --deep")}".`,
    });
  }
  return findings;
}
