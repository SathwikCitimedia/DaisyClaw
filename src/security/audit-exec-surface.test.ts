...
describe("security audit exec surface findings", () => {
  // Redirect the DaisyClaw home (DAISYCLAW_HOME wins over HOME/USERPROFILE in
  // `resolveRawHomeDir`) to a per-test tempdir so `saveExecApprovals` never
  // touches the real `~/.daisyclaw/exec-approvals.json` on the host running
  // the suite.
  ...
  it("warns when YOLO exec overrides restrictive Claude permission mode", () => {
    ...
    expect(finding?.detail).toContain("DaisyClaw exec is YOLO");
  });
  ...
  it("does not warn for restrictive Claude permission mode when DaisyClaw exec is restrictive", () => {
    const findings = collectExecRuntimeFindings({
