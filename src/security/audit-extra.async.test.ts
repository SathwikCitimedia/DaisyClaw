...
`,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-security-audit-async-"));
  ...
  it("flags plugin extension entry path traversal in deep audit", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "escape-plugin",
        daisyclaw: { extensions: ["../outside.js"] },
      }),
    );
  ...
  it("ignores install backup and debris dirs when scanning installed plugin roots", async () => {
    ...
    try {
      ...
      expect(findings.map((f) => f.title).join("\n")).not.toContain(".daisyclaw-install-backups");
    } finally {
      scanSpy.mockRestore();
    }
  });
  ...
  it("audits canonical auth profile SQLite store permissions", async () => {
    ...
    const databasePath = path.join(agentDir, "daisyclaw-agent.sqlite");
    ...
    expect(readableAuthTargets).toEqual(
      expect.arrayContaining([
        expect.stringContaining("daisyclaw-agent.sqlite"),
        expect.stringContaining("daisyclaw-agent.sqlite-wal"),
        expect.stringContaining("daisyclaw-agent.sqlite-shm"),
      ]),
    );
  });
});
