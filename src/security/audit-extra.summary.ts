...
function summarizeGroupPolicy(cfg: DaisyClawConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function hasWebSearchKey(cfg: DaisyClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: DaisyClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: DaisyClawConfig): boolean {
...
function isBrowserEnabled(cfg: DaisyClawConfig): boolean {
  return cfg.browser?.enabled !== false;
}

/** Produce a concise inventory of major security-relevant surfaces. */
export function collectAttackSurfaceSummaryFindings(cfg: DaisyClawConfig): SecurityAuditFinding[] {
