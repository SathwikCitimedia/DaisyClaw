...
import { normalizeUniqueStringEntries } from "@daisyclaw/normalization-core/string-normalization";
...
function isGatewayRemotelyExposed(cfg: DaisyClawConfig): boolean {
...
function formatHooksTokenReuseRemediation(reuse: GatewayAuthSharedSecretReuse): string {
  ...
  return `Run ${formatCliCommand("daisyclaw doctor --fix")} to rotate a persisted hooks.token, then update external hook senders to use the new hook token.`;
}
...
function listKnownNodeCommands(cfg: DaisyClawConfig): Set<string> {
  const baseCfg: DaisyClawConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function listGroupPolicyOpen(cfg: DaisyClawConfig): string[] {
  const out: string[] = [];
...
function listPotentialMultiUserSignals(cfg: DaisyClawConfig): string[] {
  const out = new Set<string>();
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return [];
...
function collectRiskyToolExposureContexts(cfg: DaisyClawConfig): {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep DAISYCLAW_STATE_DIR on a local-only volume and re-run "${formatCliCommand("daisyclaw security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: DaisyClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer DAISYCLAW_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-daisyclaw-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: DaisyClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: DaisyClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
...
export function collectNodeDenyCommandPatternFindings(cfg: DaisyClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: DaisyClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: DaisyClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: DaisyClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectLikelyMultiUserSetupFindings(cfg: DaisyClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  findings.push({
    ...
    detail:
      ...
      `\n${impactLine}\n${riskyContextsDetail}\n` +
      "DaisyClaw's default security model is personal-assistant (one trusted operator boundary), not hostile multi-tenant isolation on one shared gateway.",
    remediation:
