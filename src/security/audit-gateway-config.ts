...
type CollectDangerousConfigFlags = (cfg: DaisyClawConfig) => string[];
...
export function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.DAISYCLAW_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.DAISYCLAW_GATEWAY_PASSWORD);
