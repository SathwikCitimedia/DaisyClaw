...
describe("security audit gateway config findings", () => {
  it("evaluates gateway auth presence and rate-limit guardrails", async () => {
    await Promise.all([
      ...
      (async () => {
        ...
        const resolvedConfig: DaisyClawConfig = {
          gateway: {
            bind: "lan",
            auth: {},
          },
  ...
  it("warns when DAISYCLAW_GATEWAY_TOKEN shadows a different configured token source", () => {
    const cfg: DaisyClawConfig = {
  ...
  it("does not warn when gateway.auth.token resolves from DAISYCLAW_GATEWAY_TOKEN", () => {
    const cfg: DaisyClawConfig = {
