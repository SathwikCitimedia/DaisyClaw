...
describe("security audit install metadata findings", () => {
  ...
  const runInstallMetadataAudit = async (cfg: DaisyClawConfig, stateDir: string) => {
  ...
  const writePluginIndexInstallRecords = async (
    ...
    const index: InstalledPluginIndex = {
      ...
      plugins: Object.keys(records).map((pluginId) => ({
        pluginId,
        manifestPath: path.join(stateDir, "extensions", pluginId, "daisyclaw.plugin.json"),
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-security-install-"));
  });
              ...
              hooks: {
                internal: {
                  installs: {
                    "test-hooks": {
                      source: "npm",
                      spec: "@daisyclaw/test-hooks@1.2.3",
        ...
        run: async () => {
          ...
          await writePluginIndexInstallRecords(stateDir, {
            "voice-call": {
              source: "npm",
              spec: "@daisyclaw/voice-call",
              resolvedSpec: "@daisyclaw/voice-call@1.2.3",
              integrity: "sha512-plugin",
            },
          });
          return runInstallMetadataAudit(
                ...
                internal: {
                  installs: {
                    "test-hooks": {
                      source: "npm",
                      spec: "@daisyclaw/test-hooks",
                      resolvedSpec: "@daisyclaw/test-hooks@1.2.3",
                      integrity: "sha512-hook",
                    },
                  },
                },
...
describe("security audit extension tool reachability findings", () => {
  ...
  const runSharedExtensionsAudit = async (config: DaisyClawConfig) => {
  ...
  beforeAll(async () => {
    ...
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-security-extensions-"));
    isolatedHome = path.join(fixtureRoot, "home");
    const isolatedEnv = createPathResolutionEnv(isolatedHome, { DAISYCLAW_HOME: isolatedHome });
