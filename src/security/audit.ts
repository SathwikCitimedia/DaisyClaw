...
import { asNullableRecord } from "@daisyclaw/normalization-core/record-coerce";
import {
  normalizeOptionalLowercaseString,
  normalizeOptionalString,
} from "@daisyclaw/normalization-core/string-coerce";
import { normalizeStringEntries } from "@daisyclaw/normalization-core/string-normalization";
import { resolveAgentWorkspaceDir, resolveDefaultAgentId } from "../agents/agent-scope.js";
import { resolveExecDefaults } from "../agents/exec-defaults.js";
import { resolveSandboxConfigForAgent } from "../agents/sandbox/config.js";
import type { ChannelPlugin } from "../channels/plugins/types.plugin.js";
import type { ConfigFileSnapshot, DaisyClawConfig } from "../config/config.js";
import { resolveConfigPath, resolveStateDir } from "../config/paths.js";
import type { CliBackendConfig } from "../config/types.agent-defaults.js";
import type { GatewayAuthConfig } from "../config/types.gateway.js";
import type { SecurityAuditSuppression } from "../config/types.daisyclaw.js";
import {
...
async function materializeAuditGatewayAuthRefs(params: {
  ...
  const materializeParams = {
    ...
    hasTokenCandidate: Boolean(normalizeOptionalString(params.env.DAISYCLAW_GATEWAY_TOKEN)),
    hasPasswordCandidate: Boolean(normalizeOptionalString(params.env.DAISYCLAW_GATEWAY_PASSWORD)),
  };
...
function shouldMaterializeHooksGatewayAuthRefs(cfg: DaisyClawConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your DaisyClaw state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your DaisyClaw state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
export function collectLoggingFindings(cfg: DaisyClawConfig): SecurityAuditFinding[] {
...
export function collectElevatedFindings(cfg: DaisyClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectYoloExecScopeIds(cfg: DaisyClawConfig, approvals: ExecApprovalsFile): string[] {
...
export function collectExecRuntimeFindings(cfg: DaisyClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  if (claudePermissionModeHits.length > 0 && yoloExecScopeIds.length > 0) {
    findings.push({
      ...
      detail: `claude-cli sets ${claudePermissionModeHits.map((hit) => `${hit.argSet}=${hit.mode}`).join(", ")}, but DaisyClaw exec is YOLO for: ${yoloExecScopeIds.join(", ")}. Managed Claude live sessions use --permission-mode bypassPermissions.`,
      remediation:
        "Restrict DaisyClaw tools.exec.security/tools.exec.ask, or remove the Claude --permission-mode override.",
    });
  }
...
function collectOpenExecSurfacePaths(cfg: DaisyClawConfig): string[] {
