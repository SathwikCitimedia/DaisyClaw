...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-install-policy-"));
  tempDirs.push(dir);
  return dir;
}
...
function configWithPolicy(scriptPath: string, env: Record<string, string>): DaisyClawConfig {
  return {
    security: {
      installPolicy: {
        enabled: true,
...
describe("runInstallPolicy", () => {
  ...
  it("executes policy for skills when targets are omitted", async () => {
    ...
    expect(captured.daisyclawVersion).toEqual(expect.any(String));
  ...
  it("prefixes operator blocks", async () => {
    ...
    expect(warnings.join("\n")).toContain("source=clawhub/daisyclaw");
