...
import type { DaisyClawConfig, SecurityConfig } from "../config/types.daisyclaw.js";
...
export type InstallPolicySource = {
  kind:
    ...
    | "upload"
    | "workspace";
  authority: "daisyclaw" | "official" | "third-party" | "unknown" | "user";
  mutable: boolean;
  network: boolean;
};
...
export async function runInstallPolicy(params: {
  ...
  if (!config) {
    try {
      ...
      return failClosed(`could not load DaisyClaw config (${formatErrorMessage(err)})`);
    }
  }
  ...
  const input = JSON.stringify({
    protocolVersion: 1,
    daisyclawVersion: resolveRuntimeServiceVersion(params.env ?? process.env),
    ...params.request,
  });
