...
describe("shared/assistant-identity-values", () => {
  ...
  it("trims values and preserves strings within the limit", () => {
    expect(coerceIdentityValue("  DaisyClaw  ", 20)).toBe("DaisyClaw");
    expect(coerceIdentityValue("  DaisyClaw  ", 8)).toBe("DaisyClaw");
  });

  it("truncates overlong trimmed values at the exact limit", () => {
    expect(coerceIdentityValue("  DaisyClaw Assistant  ", 8)).toBe("DaisyClaw");
  });

  it("returns an empty string when truncating to a zero-length limit", () => {
    expect(coerceIdentityValue("  DaisyClaw  ", 0)).toBe("");
    expect(coerceIdentityValue("  DaisyClaw  ", -1)).toBe("OpenCla");
  });
});
