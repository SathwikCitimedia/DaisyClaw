...
import { normalizeCsvOrLooseStringList } from "@daisyclaw/normalization-core/string-normalization";
...
/** Parses the JSON5 DaisyClaw manifest block embedded inside a string frontmatter field. */
export function resolveDaisyClawManifestBlock(params: {
  frontmatter: Record<string, unknown>;
  key?: string;
...
export type DaisyClawManifestRequires = {
...
/** Extracts normalized runtime requirement lists from an DaisyClaw manifest block. */
export function resolveDaisyClawManifestRequires(
  metadataObj: Record<string, unknown>,
): DaisyClawManifestRequires | undefined {
  const requiresRaw =
...
export function resolveDaisyClawManifestInstall<T>(
  metadataObj: Record<string, unknown>,
...
/** Extracts normalized OS allowlist entries from an DaisyClaw manifest block. */
export function resolveDaisyClawManifestOs(metadataObj: Record<string, unknown>): string[] {
  return normalizeStringList(metadataObj.os);
}

export type ParsedDaisyClawManifestInstallBase = {
...
export function parseDaisyClawManifestInstallBase(
  input: unknown,
  allowedKinds: readonly string[],
): ParsedDaisyClawManifestInstallBase | undefined {
  ...
  const spec: ParsedDaisyClawManifestInstallBase = {
    raw,
    kind,
  };
...
export function applyDaisyClawManifestInstallCommonFields<
  T extends { id?: string; label?: string; bins?: string[] },
>(spec: T, parsed: Pick<ParsedDaisyClawManifestInstallBase, "id" | "label" | "bins">): T {
  if (parsed.id) {
    spec.id = parsed.id;
  }
  if (parsed.label) {
