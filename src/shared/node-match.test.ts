...
describe("shared/node-match", () => {
  ...
  it("matches candidates by node id, remote ip, normalized name, and long prefix", () => {
    ...
    expect(resolveNodeMatches(nodes, "daisyclaw")).toStrictEqual([]);
  ...
  it("prefers a unique current DaisyClaw client over a legacy clawdbot client", () => {
    expect(
      resolveNodeIdFromCandidates(
        [
          {
  ...
  it("prints client ids in ambiguous-node errors when available", () => {
    expect(() =>
      ...
      /ambiguous node: Peter's Mac Studio.*node=other-mac.*client=daisyclaw-macos.*node=third-mac.*client=daisyclaw-macos/,
    );
  });
