...
export type NodeMatchCandidate = {
  ...
  connected?: boolean;
  /** Client id used to prefer current DaisyClaw nodes over legacy migration ties. */
  clientId?: string;
};
...
function isCurrentDaisyClawClient(clientId: string | undefined): boolean {
  const normalized = normalizeOptionalLowercaseString(clientId) ?? "";
  return normalized.startsWith("daisyclaw-");
}
...
function pickPreferredLegacyMigrationMatch(
  ...
  const current = matches.filter((match) => isCurrentDaisyClawClient(match.clientId));
  if (current.length !== 1) {
    return undefined;
  }
  ...
  // During Clawdbot -> DaisyClaw migration, a unique current client should win only
...
function scoreNodeCandidate(node: NodeMatchCandidate, matchScore: number): number {
  ...
  if (isCurrentDaisyClawClient(node.clientId)) {
    score += 10;
