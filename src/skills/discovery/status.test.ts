...
describe("buildWorkspaceSkillStatus", () => {
  it("surfaces valid ClawHub linkage and local Skill Card metadata", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-skill-status-"));
    try {
  ...
  it("classifies a mixed broken skill pack without flattening visibility reasons", () => {
    const missingBin = "daisyclaw-test-definitely-missing-skill-bin";
    const report = buildWorkspaceSkillStatus("/tmp/ws", {
      agentId: "specialist",
      config: {
        agents: {
      ...
      entries: [
        ...
        createEntry("needs-bin", {
          metadata: {
            ...
            install: [
              {
                kind: "node",
                package: "@daisyclaw/missing-skill-bin",
                bins: [missingBin],
              },
            ],
          },
        ...
        createEntry("needs-env", {
          metadata: {
            primaryEnv: "DAISYCLAW_TEST_MISSING_SKILL_KEY",
            requires: { env: ["DAISYCLAW_TEST_MISSING_SKILL_KEY"] },
          },
        }),
        ...
        createEntry("bundled-blocked", { source: "daisyclaw-bundled" }),
      ],
    });
    ...
    expect(needsBin.install).toStrictEqual([
      {
        kind: "node",
        id: "node-0",
        label: "Install @daisyclaw/missing-skill-bin (pnpm)",
        bins: [missingBin],
      },
    ]);
    const needsEnv = requireSkillStatus(byName, "needs-env");
    expect(needsEnv.eligible).toBe(false);
    expect(needsEnv.primaryEnv).toBe("DAISYCLAW_TEST_MISSING_SKILL_KEY");
    expect(needsEnv.missing).toStrictEqual({
      anyBins: [],
      bins: [],
      config: [],
      env: ["DAISYCLAW_TEST_MISSING_SKILL_KEY"],
      os: [],
    });
