...
import { resolveDaisyClawMetadata, resolveSkillInvocationPolicy } from "../loading/frontmatter.js";
...
metadata: {"daisyclaw":{"install":[{"id":"deps","kind":"node","package":"example-package"}]}}
---
...
function loadTestWorkspaceSkillEntries(workspaceDir: string): SkillEntry[] {
  ...
  return skills.map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveDaisyClawMetadata(frontmatter),
      invocation,
      exposure: {
...
const workspaceSuite = createFixtureSuite("daisyclaw-skills-install-");
...
async function withWorkspaceCase(
  ...
  const envSnapshot = captureEnv(["DAISYCLAW_STATE_DIR"]);
  try {
...
describe("installSkill install policy hooks", () => {
  beforeEach(() => {
    ...
    skillsInstallTesting.setDepsForTest({
      ...
      resolveNodeInstallStateDir: () => {
        ...
        if (!stateDir) {
          throw new Error("DAISYCLAW_STATE_DIR missing in skills install test");
        }
        return stateDir;
      },
    });
  ...
  it("runs npm node installs with an DaisyClaw-managed user prefix", async () => {
  ...
  it("keeps the default npm prefix out of env-overridden state paths", () => {
    const envSnapshot = captureEnv(["DAISYCLAW_STATE_DIR", "DAISYCLAW_CONFIG_PATH"]);
    try {
      process.env.DAISYCLAW_STATE_DIR = "/tmp/untrusted-state";
      process.env.DAISYCLAW_CONFIG_PATH = "/tmp/untrusted-config/daisyclaw.json";
  ...
  it("surfaces plugin hook findings from before_install", async () => {
    ...
    await withWorkspaceCase(async ({ workspaceDir }) => {
      ...
      expect(payload?.origin).toBe("daisyclaw-workspace");
