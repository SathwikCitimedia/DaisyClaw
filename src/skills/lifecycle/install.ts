...
function resolveDefaultNodeInstallStateDir({
  ...
  if (platform !== "win32" && getuid?.() === 0) {
    return path.join(path.parse(cwd).root, "var", "lib", "daisyclaw");
  }
  return path.join(homedir(), ".daisyclaw");
}
...
export async function installSkill(params: SkillInstallRequest): Promise<SkillInstallResult> {
  ...
  const scanResult = await evaluateSkillInstallPolicy({
    ...
    source:
      skillSource === "daisyclaw-bundled"
        ? { kind: "bundled", authority: "daisyclaw", mutable: false, network: false }
        : skillSource === "daisyclaw-managed" || skillSource === "daisyclaw-extra"
          ? { kind: "managed", authority: "daisyclaw", mutable: false, network: false }
  ...
  const trustedInstallSources = new Set(["daisyclaw-bundled", "daisyclaw-managed", "daisyclaw-extra"]);
