...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-skill-upload-store-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("skill upload store", () => {
  ...
  beforeAll(async () => {
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-skill-upload-store-"));
    try {
