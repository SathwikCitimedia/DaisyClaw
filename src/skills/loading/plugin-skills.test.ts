    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "daisyclaw.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "daisyclaw.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "daisyclaw.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("daisyclaw-");
  const acpxRoot = await tempDirs.make("daisyclaw-acpx-plugin-");
  const helperRoot = await tempDirs.make("daisyclaw-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("daisyclaw-");
  const pluginRoot = await tempDirs.make("daisyclaw-plugin-");
  const outsideDir = await tempDirs.make("daisyclaw-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("daisyclaw-");
    const pluginRoot = await tempDirs.make("daisyclaw-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("daisyclaw-");
    const pluginRoot = await tempDirs.make("daisyclaw-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
