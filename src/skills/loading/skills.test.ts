...
const fixtureSuite = createFixtureSuite("daisyclaw-skills-suite-");
let tempHome: TempHomeEnv | null = null;
let skillsHomeEnv: SkillsHomeEnvSnapshot | null = null;
const pluginEnvSnapshot = captureEnv(["DAISYCLAW_DISABLE_BUNDLED_PLUGINS"]);
...
function rawSkillApiKeyRefConfig(skillName: string): DaisyClawConfig {
  return {
    skills: {
      entries: {
        [skillName]: {
...
function resolvedSkillApiKeyConfig(skillName: string, apiKey: string): DaisyClawConfig {
  return {
    skills: {
      entries: {
        [skillName]: {
...
beforeAll(async () => {
  ...
  tempHome = await createTempHomeEnv("daisyclaw-skills-home-");
  skillsHomeEnv = setMockSkillsHomeEnv(tempHome.home);
  await fs.mkdir(path.join(tempHome.home, ".daisyclaw", "agents", "main", "sessions"), {
    recursive: true,
  });
});
...
describe("buildWorkspaceSkillCommandSpecs", () => {
  ...
  it("includes enabled Claude bundle markdown commands as native DaisyClaw slash commands", async () => {
    const workspaceDir = await makeWorkspace();
    const config = {
      plugins: {
        entries: {
    ...
    const pluginRoot = path.join(workspaceDir, ".daisyclaw", "extensions", "compound-bundle");
