...
vi.mock("../../plugins/manifest-registry.js", async () => {
  ...
  return {
    loadPluginManifestRegistry: (params: { workspaceDir?: string }) => {
      const extensionsRoot = pathLocal.join(params.workspaceDir ?? "", ".daisyclaw", "extensions");
      const plugins = [];
      for (const id of ["open-prose", "browser"]) {
        const rootDir = pathLocal.join(extensionsRoot, id);
        const manifestPath = pathLocal.join(rootDir, "daisyclaw.plugin.json");
...
function createWorkspacePluginRegistry(workspaceDir: string): PluginManifestRegistry {
  const extensionsRoot = path.join(workspaceDir, ".daisyclaw", "extensions");
...
function setWorkspacePluginMetadataSnapshot(workspaceDir: string, config?: DaisyClawConfig): void {
...
beforeAll(async () => {
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-skills-workspace-"));
...
async function setupWorkspaceWithProsePlugin() {
  ...
  const pluginRoot = path.join(workspaceDir, ".daisyclaw", "extensions", "open-prose");
...
describe("loadWorkspaceSkillEntries", () => {
  ...
  it("loads the browser plugin automation skill when the bundled plugin is enabled", async () => {
    const workspaceDir = await createTempWorkspaceDir();
    const managedDir = path.join(workspaceDir, ".managed");
    const pluginRoot = path.join(workspaceDir, ".daisyclaw", "extensions", "browser");
  ...
  it("keeps remote-eligible skills when agent filtering is active", async () => {
    ...
    await writeSkill({
      ...
      metadata: '{"daisyclaw":{"requires":{"anyBins":["missingbin","sandboxbin"]}}}',
    });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      expect(warningLine).toContain("source=daisyclaw-workspace");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      try {
        ...
        const warningLine = firstWarningLine(warn);
        expect(warningLine).toContain("Skipping escaped skill path outside its configured root:");
        expect(warningLine).toContain("source=daisyclaw-managed");
        expect(warningLine).toContain("reason=symlink-escape");
      } finally {
        await fs.unlink(symlinkPath).catch(() => undefined);
      }
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const warningLine = firstWarningLine(warn);
      expect(warningLine).toContain("Skipping escaped skill path outside its configured root:");
      expect(warningLine).toContain("source=daisyclaw-bundled");
