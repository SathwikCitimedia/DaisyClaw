...
import { resolveDaisyClawMetadata, resolveSkillInvocationPolicy } from "./frontmatter.js";
...
const SKILL_SOURCE_ORIGIN_RELATIVE_PATH = path.join(".daisyclaw", "source-origin.json");
...
function resolveSkillsLimits(config?: DaisyClawConfig, agentId?: string): ResolvedSkillsLimits {
...
export function resolveSkillRootScanLimit(config?: DaisyClawConfig): number {
...
function buildEscapedSkillPathReason(params: { source: string; candidatePath: string }): {
  ...
  if (params.source === "daisyclaw-bundled" && candidateIsSymlink) {
    return {
  ...
  if (params.source === "daisyclaw-bundled") {
    return {
...
function resolveSkillEntryMetadata(params: {
  frontmatter: ParsedSkillFrontmatter;
  skillDir: string;
}): DaisyClawSkillMetadata | undefined {
  const metadata = resolveDaisyClawMetadata(params.frontmatter);
...
function loadGeneratedPluginSkillRecords(params: {
  ...
  for (const name of childDirs) {
    ...
    // Plugin skills live as symlinks under ~/.daisyclaw/plugin-skills/, so
...
function buildSkillsLimitNote(params: {
  ...
  if (params.truncated) {
    return `⚠️ Skills truncated: included ${params.included} of ${params.total}${params.compact ? " (compact format, descriptions omitted)" : ""}. Run \`daisyclaw skills check\` to audit.`;
  }
  if (params.compact) {
    return `⚠️ Skills catalog using compact format (descriptions omitted). Run \`daisyclaw skills check\` to audit.`;
  }
  return "";
}
