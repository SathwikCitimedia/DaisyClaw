...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("daisyclaw-bundled-");
  const workspaceDir = await createTempDir("daisyclaw-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: DaisyClawConfig) {
