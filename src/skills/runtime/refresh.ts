...
function resolveWatchTargets(workspaceDir: string, config?: DaisyClawConfig): WatchTarget[] {
  const baseRoots: Array<{ path: string; source: string }> = [];
  if (workspaceDir.trim()) {
    baseRoots.push({ path: path.join(workspaceDir, "skills"), source: "daisyclaw-workspace" });
    baseRoots.push({
      path: path.join(workspaceDir, ".agents", "skills"),
      source: "agents-skills-project",
    });
  }
  baseRoots.push({ path: path.join(CONFIG_DIR, "skills"), source: "daisyclaw-managed" });
  baseRoots.push({
...
function isTrustedSymlinkSkillTarget(
  ...
  if (source === "daisyclaw-managed" || source === "agents-skills-personal") {
    return true;
  }
  return (
...
function resolveWatchDebounceMs(config?: DaisyClawConfig): number {
...
export function ensureSkillsWatcher(params: { workspaceDir: string; config?: DaisyClawConfig }) {
