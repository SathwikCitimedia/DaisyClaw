...
function createRemoteSkillWorkspace(bin: string): { cfg: DaisyClawConfig; workspaceDir: string } {
  const workspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-remote-skills-"));
  ...
  fs.writeFileSync(
    ...
    [
      ...
      `metadata: { "daisyclaw": { "os": ["darwin"], "requires": { "bins": ["${bin}"] } } }`,
      "---",
      "# Remote Skill",
      "",
    ].join("\n"),
