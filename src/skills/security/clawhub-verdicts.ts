...
export type DaisyClawSkillSecurityVerdictItem = Omit<
...
export function projectClawHubVerdictItem(
  ...
  registry: string,
): DaisyClawSkillSecurityVerdictItem {
  const projected: DaisyClawSkillSecurityVerdictItem = {
    registry,
    ok: item.ok,
  ...
  if (item.error) {
    const error: DaisyClawSkillSecurityVerdictItem["error"] = {};
...
export async function fetchDaisyClawSkillSecurityVerdicts(
  targets: Array<{ registry: string; slug: string; version: string }>,
): Promise<DaisyClawSkillSecurityVerdictItem[]> {
  ...
  const items: DaisyClawSkillSecurityVerdictItem[] = [];
