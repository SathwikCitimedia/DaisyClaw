...
beforeEach(async () => {
  testState = await createDaisyClawTestState({
    layout: "state-only",
    prefix: "daisyclaw-skill-workshop-state-",
  });
...
async function makeWorkspace(): Promise<string> {
  return await tempDirs.make("daisyclaw-skill-workshop-");
}
...
describe("skill workshop proposals", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = await makeWorkspace();
      const targetSkillsDir = await tempDirs.make("daisyclaw-skill-workshop-target-skills-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = await makeWorkspace();
      const targetSkillsDir = await tempDirs.make("daisyclaw-skill-workshop-readonly-skills-");
      await fs.symlink(targetSkillsDir, path.join(workspaceDir, "skills"), "dir");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = await makeWorkspace();
      const targetSkillsDir = await tempDirs.make("daisyclaw-skill-workshop-support-trusted-");
      const untrustedSkillsDir = await tempDirs.make("daisyclaw-skill-workshop-support-untrusted-");
      await fs.symlink(targetSkillsDir, path.join(workspaceDir, "skills"), "dir");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = await makeWorkspace();
      const targetSkillsDir = await tempDirs.make("daisyclaw-skill-workshop-untrusted-skills-");
      await fs.symlink(targetSkillsDir, path.join(workspaceDir, "skills"), "dir");
      const proposal = await proposeCreateSkill({
        workspaceDir,
