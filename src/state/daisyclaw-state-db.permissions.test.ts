...
// daisyclaw-state-db.ts hardens permissions via the named import `chmodSync`
...
vi.mock("node:fs", async (importOriginal) => {
  ...
  const chmodSync: typeof actual.chmodSync = ((target: unknown, mode: unknown) => {
    ...
    const isProbe = String(target).includes(".daisyclaw-chmod-probe-");
...
} = await import("./daisyclaw-state-db.js");
...
describe("state database permission hardening without chmod support", () => {
  ...
  it("opens the state database when chmodSync throws ENOTSUP", () => {
    stateDir = fs.mkdtempSync(join(tmpdir(), "daisyclaw-state-chmod-"));
    chmodFailHook.error = enotsupError();

    const database = openDaisyClawStateDatabase({ env: { DAISYCLAW_STATE_DIR: stateDir } });
  ...
  it("rethrows EPERM when existing permissions are too broad", () => {
    ...
    expect(() => openDaisyClawStateDatabase({ env: { DAISYCLAW_STATE_DIR: stateDir } })).toThrow(
      /EPERM/,
    );
  });
  ...
  it("opens when EPERM leaves existing permissions restrictive", () => {
    stateDir = fs.mkdtempSync(join(tmpdir(), "daisyclaw-state-chmod-"));
    openDaisyClawStateDatabase({ env: { DAISYCLAW_STATE_DIR: stateDir } });
    closeDaisyClawStateDatabaseForTest();
    chmodFailHook.error = chmodError("EPERM");
  ...
  it("repairs the schema when chmodSync throws ENOTSUP", () => {
    ...
    expect(() =>
      repairDaisyClawStateDatabaseSchema({ env: { DAISYCLAW_STATE_DIR: stateDir } }),
    ).not.toThrow();
  });
  ...
  it("commits write transactions when chmodSync throws ENOTSUP", () => {
    ...
    const result = runDaisyClawStateWriteTransaction((database) => {
      expect(database.db.isOpen).toBe(true);
      return "committed";
    }, options);
