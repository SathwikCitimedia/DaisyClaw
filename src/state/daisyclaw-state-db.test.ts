// DaisyClaw state database tests cover state DB migrations and persistence.
...
import { resolveDaisyClawStateSqlitePath } from "./daisyclaw-state-db.paths.js";
import {
  collectSqliteSchemaShape,
  createSqliteSchemaShapeFromSql,
} from "./sqlite-schema-shape.test-support.js";

type StateDbTestDatabase = Pick<DaisyClawStateKyselyDatabase, "diagnostic_events" | "schema_meta">;

function createTempStateDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-state-db-"));
}
...
describe("daisyclaw state database", () => {
  it("resolves under the shared state database directory", () => {
    const stateDir = createTempStateDir();

    expect(resolveDaisyClawStateSqlitePath({ DAISYCLAW_STATE_DIR: stateDir })).toBe(
      path.join(stateDir, "state", "daisyclaw.sqlite"),
    );
  });
  ...
  it("keeps test default state under a worker-sharded temp directory", () => {
    expect(
      ...
      path.join(os.tmpdir(), "daisyclaw-test-state", `${process.pid}-7`, "state", "daisyclaw.sqlite"),
    );
  });

  it("creates the shared state schema from the committed SQL shape", () => {
    const stateDir = createTempStateDir();
    const database = openDaisyClawStateDatabase({
      env: { DAISYCLAW_STATE_DIR: stateDir },
    });

    expect(collectSqliteSchemaShape(database.db)).toEqual(
      createSqliteSchemaShapeFromSql(new URL("./daisyclaw-state-schema.sql", import.meta.url)),
    );
    expect(database.path).toBe(path.join(stateDir, "state", "daisyclaw.sqlite"));
  });
  ...
  it("does not chmod shared parent directories for explicit database paths", () => {
    const databasePath = path.join(
      os.tmpdir(),
      `daisyclaw-explicit-state-${process.pid}-${Date.now()}.sqlite`,
    );

    expect(() => openDaisyClawStateDatabase({ path: databasePath })).not.toThrow();
  ...
  it("keeps cached handles open when another state path is opened", () => {
    ...
    const first = openDaisyClawStateDatabase({ path: firstPath });
    const second = openDaisyClawStateDatabase({ path: secondPath });

    expect(first.db.isOpen).toBe(true);
    expect(second.db.isOpen).toBe(true);
    expect(openDaisyClawStateDatabase({ path: firstPath })).toBe(first);
    expect(readSqliteNumberPragma(first.db, "user_version")).toBe(1);
  });

  it("uses savepoints for nested write transaction rollback", () => {
    const stateDir = createTempStateDir();
    const options = { env: { DAISYCLAW_STATE_DIR: stateDir } };
    ...
    runDaisyClawStateWriteTransaction((database) => {
      ...
      expect(() =>
        runDaisyClawStateWriteTransaction((inner) => {
  ...
  it("rejects Promise-returning write transactions", () => {
    ...
    expect(() =>
      runDaisyClawStateWriteTransaction(async () => {
        return "not sync";
      }, options),
