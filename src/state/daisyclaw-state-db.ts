// DaisyClaw state database manages shared persisted state and migrations.
...
import {
  resolveDaisyClawStateSqliteDir,
  resolveDaisyClawStateSqlitePath,
} from "./daisyclaw-state-db.paths.js";
import { DAISYCLAW_STATE_SCHEMA_SQL } from "./daisyclaw-state-schema.generated.js";

/**
 * Shared DaisyClaw SQLite state database lifecycle and metadata writers.
 *
 ...
 */
const DAISYCLAW_STATE_SCHEMA_VERSION = 1;
/** Shared timeout used by state and agent SQLite handles before surfacing busy errors. */
export const DAISYCLAW_SQLITE_BUSY_TIMEOUT_MS = 30_000;
const DAISYCLAW_STATE_DIR_MODE = 0o700;
const DAISYCLAW_STATE_FILE_MODE = 0o600;
const DAISYCLAW_STATE_SIDECAR_SUFFIXES = ["", "-shm", "-wal"] as const;

/** Open shared SQLite database handle plus WAL maintenance lifecycle. */
export type DaisyClawStateDatabase = {
  db: DatabaseSync;
  path: string;
...
export type DaisyClawStateDatabaseOptions = {
  env?: NodeJS.ProcessEnv;
  path?: string;
};

/** Status stored for a state migration run. */
export type DaisyClawMigrationRunStatus = "completed" | "warning" | "failed";
/** Status stored for a state backup run. */
export type DaisyClawBackupRunStatus = "completed" | "failed";

export type DaisyClawStateDatabaseSchemaMigration = {
  kind: "agent-databases-composite-primary-key";
  path: string;
};

/** Input for recording one state migration run summary. */
export type RecordDaisyClawStateMigrationRunOptions = DaisyClawStateDatabaseOptions & {
  id?: string;
  startedAt: number;
  finishedAt?: number;
  status: DaisyClawMigrationRunStatus;
  report: Record<string, unknown>;
};

/** Input for recording one migrated source file/table pair. */
export type RecordDaisyClawStateMigrationSourceOptions = DaisyClawStateDatabaseOptions & {
  runId: string;
...
export type RecordDaisyClawStateBackupRunOptions = DaisyClawStateDatabaseOptions & {
  id?: string;
  createdAt: number;
  archivePath: string;
  status: DaisyClawBackupRunStatus;
  manifest: Record<string, unknown>;
};

const cachedDatabases = new Map<string, DaisyClawStateDatabase>();

type DaisyClawStateMetadataDatabase = Pick<
  DaisyClawStateKyselyDatabase,
...
function assertSupportedSchemaVersion(db: DatabaseSync, pathname: string): void {
  ...
  if (userVersion > DAISYCLAW_STATE_SCHEMA_VERSION) {
    throw new Error(
      `DaisyClaw state database ${pathname} uses newer schema version ${userVersion}; this DaisyClaw build supports ${DAISYCLAW_STATE_SCHEMA_VERSION}.`,
    );
  }
}
...
function filesystemRejectsChmod(target: string): boolean {
  ...
  try {
    ...
    probePath = path.join(probeDir, `.daisyclaw-chmod-probe-${randomUUID()}`);
    writeFileSync(probePath, "", { flag: "wx", mode: DAISYCLAW_STATE_FILE_MODE });
  } catch {
    return false;
  }
  try {
    chmodSync(probePath, DAISYCLAW_STATE_FILE_MODE);
    return false;
  } catch (err) {
...
function ensureDaisyClawStatePermissions(pathname: string, env: NodeJS.ProcessEnv): void {
  const dir = path.dirname(pathname);
  const defaultDir = resolveDaisyClawStateSqliteDir(env);
  const isDefaultStateDatabase =
    path.resolve(pathname) === path.resolve(resolveDaisyClawStateSqlitePath(env));
  if (isDefaultStateDatabase && dir !== defaultDir) {
    throw new Error(`DaisyClaw state database path resolved outside its state dir: ${pathname}`);
  }
  const dirExisted = existsSync(dir);
  mkdirSync(dir, { recursive: true, mode: DAISYCLAW_STATE_DIR_MODE });
  ...
  if (isDefaultStateDatabase || !dirExisted) {
    bestEffortChmodSync(dir, DAISYCLAW_STATE_DIR_MODE);
  }
  for (const suffix of DAISYCLAW_STATE_SIDECAR_SUFFIXES) {
    const candidate = `${pathname}${suffix}`;
    if (existsSync(candidate)) {
      bestEffortChmodSync(candidate, DAISYCLAW_STATE_FILE_MODE);
    }
  }
}
...
function assertCanonicalStateSchemaShape(db: DatabaseSync, pathname: string): void {
  ...
  throw new Error(
    `DaisyClaw state database ${pathname} has a legacy agent database registry schema; run daisyclaw doctor --fix to migrate it.`,
  );
}

export function detectDaisyClawStateDatabaseSchemaMigrations(
  options: DaisyClawStateDatabaseOptions = {},
): DaisyClawStateDatabaseSchemaMigration[] {
...
export function repairDaisyClawStateDatabaseSchema(options: DaisyClawStateDatabaseOptions = {}): {
  changes: string[];
  warnings: string[];
} {
  ...
  ensureDaisyClawStatePermissions(pathname, env);
  const sqlite = requireNodeSqlite();
...
function ensureSchema(db: DatabaseSync, pathname: string): void {
  ...
  db.exec(DAISYCLAW_STATE_SCHEMA_SQL);
  ensureAdditiveStateColumns(db);
  db.exec(`PRAGMA user_version = ${DAISYCLAW_STATE_SCHEMA_VERSION};`);
  const now = Date.now();
  const kysely = getNodeSqliteKysely<DaisyClawStateMetadataDatabase>(db);
  executeSqliteQuerySync(
    db,
    kysely
      ...
      .values({
        meta_key: "primary",
        role: "global",
        schema_version: DAISYCLAW_STATE_SCHEMA_VERSION,
        agent_id: null,
        app_version: null,
        created_at: now,
        updated_at: now,
...
function resolveDatabasePath(options: DaisyClawStateDatabaseOptions = {}): string {
  return options.path ?? resolveDaisyClawStateSqlitePath(options.env ?? process.env);
}

/** Open or return a cached shared state database after schema and migration checks. */
export function openDaisyClawStateDatabase(
  options: DaisyClawStateDatabaseOptions = {},
): DaisyClawStateDatabase {
  const env = options.env ?? process.env;
  const pathname = resolveDatabasePath(options);
...
export function runDaisyClawStateWriteTransaction<T>(
  operation: (database: DaisyClawStateDatabase) => T,
  options: DaisyClawStateDatabaseOptions = {},
): T {
  const database = openDaisyClawStateDatabase(options);
  const result = runSqliteImmediateTransactionSync(database.db, () => operation(database));
  try {
    ensureDaisyClawStatePermissions(database.path, options.env ?? process.env);
  } catch {
...
export function recordDaisyClawStateMigrationRun(
  options: RecordDaisyClawStateMigrationRunOptions,
): string {
  const id = options.id ?? randomUUID();
  runDaisyClawStateWriteTransaction((database) => {
    const db = getNodeSqliteKysely<DaisyClawStateMetadataDatabase>(database.db);
    executeSqliteQuerySync(
      database.db,
...
export function recordDaisyClawStateMigrationSource(
  options: RecordDaisyClawStateMigrationSourceOptions,
): void {
  runDaisyClawStateWriteTransaction((database) => {
    const db = getNodeSqliteKysely<DaisyClawStateMetadataDatabase>(database.db);
    executeSqliteQuerySync(
...
export function recordDaisyClawStateBackupRun(options: RecordDaisyClawStateBackupRunOptions): string {
  const id = options.id ?? randomUUID();
  runDaisyClawStateWriteTransaction((database) => {
    const db = getNodeSqliteKysely<DaisyClawStateMetadataDatabase>(database.db);
    executeSqliteQuerySync(
...
export function closeDaisyClawStateDatabase(): void {
...
export function isDaisyClawStateDatabaseOpen(): boolean {
  return Array.from(cachedDatabases.values()).some((database) => database.db.isOpen);
}

/** Test alias for closing shared state handles from teardown code. */
export const closeDaisyClawStateDatabaseForTest = closeDaisyClawStateDatabase;
