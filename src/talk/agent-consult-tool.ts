/**
 * Realtime voice tool definition and helpers for delegating work to DaisyClaw.
 *
...
export const REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME = "daisyclaw_agent_consult";
...
export const REALTIME_VOICE_AGENT_CONSULT_TOOL: RealtimeVoiceTool = {
  ...
  description:
    "Delegate the caller's request to the configured DaisyClaw agent for normal tool-backed work, actions, context, memory, or reasoning before speaking.",
  parameters: {
    type: "object",
    properties: {
      question: {
...
export function buildRealtimeVoiceAgentConsultWorkingResponse(
  ...
  return {
    ...
    message: `Tell the ${audienceLabel} briefly that you are checking, then wait for the final DaisyClaw result before answering with the actual result.`,
  };
}
...
/** Resolve the DaisyClaw tool allowlist paired with the consult exposure policy. */
...
export function buildRealtimeVoiceAgentConsultPolicyInstructions(config: {
  ...
  if (config.consultPolicy === "always") {
    return [
      "Consult behavior:",
      "- Call daisyclaw_agent_consult before every substantive answer.",
  ...
  return [
    ...
    "- Call daisyclaw_agent_consult before answering requests that need facts, memory, current information, tools, workspace state, or the user's DaisyClaw-specific context.",
...
/** Build the delegated DaisyClaw agent prompt for a live voice consult. */
export function buildRealtimeVoiceAgentConsultPrompt(params: {
  ...
  return [
    ...
    "Act as the configured DaisyClaw agent on behalf of this user. Use available tools when the request asks you to do work.",
