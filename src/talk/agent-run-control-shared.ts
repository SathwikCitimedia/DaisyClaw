/**
 * Shared realtime voice controls for active DaisyClaw agent runs.
 *
...
export const REALTIME_VOICE_AGENT_CONTROL_TOOL_NAME = "daisyclaw_agent_control";
...
export function buildRealtimeVoiceAgentControlSpeechMessage(text: string): string {
  return [
    "Internal DaisyClaw voice control result.",
    "Do not call daisyclaw_agent_consult or any other tool for this message.",
    "Speak this exact DaisyClaw status to the voice call, without adding, removing, or rephrasing words.",
...
export function formatRealtimeVoiceAgentQueueRejection(
  ...
  if (reason === "compacting") {
    return "DaisyClaw is compacting the active run and cannot accept voice steering yet.";
  }
  if (reason === "not_streaming") {
    return "DaisyClaw has an active run, but it is not currently accepting steering.";
  }
  return mode === "followup"
    ? "DaisyClaw could not queue that follow-up."
    : "DaisyClaw could not steer the active run.";
}
...
export function formatRealtimeVoiceAgentStatus(params: {
  ...
  if (!params.active) {
    ...
    return turnEnded
      ? "DaisyClaw finished the last voice request."
  ...
  if (toolEvent) {
    ...
    if (toolEvent.type === "tool.call") {
      return name ? `DaisyClaw is starting ${name}.` : "DaisyClaw is starting a tool.";
    }
    if (toolEvent.type === "tool.result") {
      return name
        ? `DaisyClaw finished ${name} and is continuing.`
        : "DaisyClaw finished a tool and is continuing.";
    }
    if (toolEvent.type === "tool.progress") {
      return name
        ? `DaisyClaw is working in ${name}${phase ? ` (${phase})` : ""}.`
        : "DaisyClaw is still working.";
    }
  }

  if (params.activity?.activeToolName) {
    return `DaisyClaw is running ${params.activity.activeToolName}.`;
  }
  if (params.activity?.activeWorkKind === "model_call") {
    return "DaisyClaw is waiting on the model.";
  }
  if (params.activity?.activeWorkKind === "embedded_run" || params.activity?.hasActiveEmbeddedRun) {
    return "DaisyClaw is working on the current voice request.";
  }
