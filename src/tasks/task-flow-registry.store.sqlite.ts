// Persists managed task-flow records through the DaisyClaw SQLite state database.
...
type FlowRunsTable = DaisyClawStateKyselyDatabase["flow_runs"];
type FlowRegistryStoreDatabase = Pick<DaisyClawStateKyselyDatabase, "flow_runs">;
...
// SQLite-backed task-flow store mirrors the in-process registry into daisyclaw-state.db.
...
function pruneFlowsNotInSnapshot(params: { db: DatabaseSync; ids: readonly string[] }) {
  const tempTableName = "daisyclaw_live_flow_ids";
