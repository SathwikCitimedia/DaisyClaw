...
type TaskFlowRegistryTestDatabase = Pick<DaisyClawStateKyselyDatabase, "flow_runs">;
...
async function withFlowRegistryTempDir<T>(run: (root: string) => Promise<T>): Promise<T> {
  return await withDaisyClawTestState(
    {
      layout: "state-only",
      prefix: "daisyclaw-task-flow-store-",
    },
    async (state) => {
      const root = state.stateDir;
      resetTaskFlowRegistryForTests();
      try {
        return await withEnvAsync({ DAISYCLAW_STATE_DIR: root }, async () => await run(root));
      } finally {
        resetTaskFlowRegistryForTests();
      }
    },
