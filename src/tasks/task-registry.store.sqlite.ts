// Persists task registry records and events through the DaisyClaw SQLite state database.
...
type TaskRunsTable = DaisyClawStateKyselyDatabase["task_runs"];
type TaskDeliveryStateTable = DaisyClawStateKyselyDatabase["task_delivery_state"];
...
// SQLite-backed task store mirrors task records and delivery state into daisyclaw-state.db.
...
export function saveTaskRegistryStateToSqlite(snapshot: TaskRegistryStoreSnapshot) {
  withWriteTransaction(({ db }) => {
    ...
    pruneRowsNotInSnapshot({
      ...
      tableName: "task_runs",
      columnName: "task_id",
      tempTableName: "daisyclaw_live_task_run_ids",
      ids: taskIds,
    });
    ...
    if (deliveryTaskIds.length === 0) {
      ...
      pruneRowsNotInSnapshot({
        ...
        columnName: "task_id",
        tempTableName: "daisyclaw_live_task_delivery_ids",
        ids: deliveryTaskIds,
      });
    }
