...
function configureTaskRegistryMaintenanceRuntimeForTest(params: {
  ...
  setTaskRegistryMaintenanceRuntimeForTests({
    ...
    resolveCronJobsStorePath: () => "/tmp/daisyclaw-test-cron/jobs.json",
...
function createAcpSessionStoreEntry(params: {
  ...
  return {
    cfg: {} as never,
    storePath: "/tmp/daisyclaw-test-sessions.json",
...
async function withTaskRegistryTempDir<T>(
  ...
  return await withTempDir({ prefix: "daisyclaw-task-registry-" }, async (root) => {
    return await withEnvAsync({ DAISYCLAW_STATE_DIR: root }, async () => {
...
describe("task-registry", () => {
  ...
  it("cancels childless codex-native tasks without routing through DaisyClaw subagent sessions", async () => {
    await withTaskRegistryTempDir(async () => {
      resetTaskRegistryForTests();
      const task = createTaskRecord({
        runtime: "subagent",
