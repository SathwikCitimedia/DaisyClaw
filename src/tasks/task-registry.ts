...
const TASK_REGISTRY_DELIVERY_RUNTIME_OVERRIDE_KEY = Symbol.for(
  "daisyclaw.taskRegistry.deliveryRuntimeOverride",
);
const TASK_REGISTRY_CONTROL_RUNTIME_OVERRIDE_KEY = Symbol.for(
  "daisyclaw.taskRegistry.controlRuntimeOverride",
);
  ...
  try {
    if (task.runtime !== "cli") {
      ...
      if (task.runtime === "cron") {
        ...
        // not have DaisyClaw child sessions to terminate. Cancellation clears
