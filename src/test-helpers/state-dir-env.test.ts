...
function snapshotCurrentStateDirVars(): EnvSnapshot {
  return {
    daisyclaw: process.env.DAISYCLAW_STATE_DIR,
  };
}

function expectStateDirVars(snapshot: EnvSnapshot) {
  expect(process.env.DAISYCLAW_STATE_DIR).toBe(snapshot.daisyclaw);
}
...
describe("state-dir-env helpers", () => {
  it("set/snapshot/restore round-trips DAISYCLAW_STATE_DIR", () => {
    const prev = snapshotCurrentStateDirVars();
    const snapshot = snapshotStateDirEnv();

    setStateDirEnv("/tmp/daisyclaw-state-dir-test");
    expect(process.env.DAISYCLAW_STATE_DIR).toBe("/tmp/daisyclaw-state-dir-test");
  ...
  it("withStateDirEnv sets env for callback and cleans up temp root", async () => {
    ...
    await withStateDirEnv("daisyclaw-state-dir-env-", async ({ tempRoot, stateDir }) => {
  ...
  it("withStateDirEnv restores env and cleans temp root when callback throws", async () => {
    ...
    let capturedStateDir = "";
    await expect(
      withStateDirEnv("daisyclaw-state-dir-env-", async ({ tempRoot, stateDir }) => {
        capturedTempRoot = tempRoot;
        capturedStateDir = stateDir;
