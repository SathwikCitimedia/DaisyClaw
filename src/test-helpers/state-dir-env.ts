...
// DAISYCLAW_STATE_DIR test helpers isolate stateful tests and restore the caller
// environment even when session cleanup fails.
export function snapshotStateDirEnv() {
  return captureEnv(["DAISYCLAW_STATE_DIR"]);
}
...
export function setStateDirEnv(stateDir: string): void {
  setTestEnvValue("DAISYCLAW_STATE_DIR", stateDir);
}
