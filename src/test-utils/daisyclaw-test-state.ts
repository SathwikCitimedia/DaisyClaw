// Creates isolated DaisyClaw state directories for integration-style tests.
...
type DaisyClawTestStateLayout = "home" | "state-only" | "split";

type DaisyClawTestStateScenario =
  | "empty"
  | "minimal"
  | "update-stable"
  | "upgrade-survivor"
...
export type DaisyClawTestStateOptions = {
  prefix?: string;
  label?: string;
  layout?: DaisyClawTestStateLayout;
  scenario?: DaisyClawTestStateScenario;
  agentEnv?: "clear" | "main";
  applyEnv?: boolean;
...
export type DaisyClawTestState = {
  root: string;
  home: string;
  stateDir: string;
  configPath: string;
...
const DEFAULT_PREFIX = "daisyclaw-test-state-";
const ENV_KEYS = [
  "HOME",
  "USERPROFILE",
  "HOMEDRIVE",
  ...
  "DAISYCLAW_AGENT_DIR",
  "DAISYCLAW_SERVICE_REPAIR_POLICY",
] as const;
...
function resolveLayout(
  ...
  if (layout === "split") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(root, "config", "daisyclaw.json"),
...
function scenarioConfig(options: DaisyClawTestStateOptions): Record<string, unknown> | undefined {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.gateway?.token ?? "daisyclaw-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function scenarioEnv(options: DaisyClawTestStateOptions): Record<string, string | undefined> {
...
export async function createDaisyClawTestState(
  options: DaisyClawTestStateOptions = {},
): Promise<DaisyClawTestState> {
  ...
  const state: DaisyClawTestState = {
    root,
    ...paths,
    env,
    envVars,
    ...
    applyEnv: () => {
      ...
      for (const [key, value] of Object.entries(envVars)) {
        // Test fixtures apply a fixed DaisyClaw env set, not plugin-provided host env.
        if (value === undefined) {
...
export async function withDaisyClawTestState<T>(
  options: DaisyClawTestStateOptions,
  fn: (state: DaisyClawTestState) => Promise<T>,
): Promise<T> {
  const state = await createDaisyClawTestState(options);
  try {
