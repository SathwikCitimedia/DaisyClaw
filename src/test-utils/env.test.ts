...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "DAISYCLAW_ENV_TEST_A";
    const keyB = "DAISYCLAW_ENV_TEST_B";
  ...
  it("captureFullEnv restores added keys and baseline values", () => {
    const key = "DAISYCLAW_ENV_TEST_ADDED";
  ...
  it("withEnv applies values only inside callback", () => {
    const key = "DAISYCLAW_ENV_TEST_SYNC";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "DAISYCLAW_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "DAISYCLAW_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync restores values when callback throws", async () => {
    const key = "DAISYCLAW_ENV_TEST_ASYNC";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "DAISYCLAW_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "DAISYCLAW_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    process.env[key] = "outer";
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    const homeDir = path.join(path.sep, "tmp", "daisyclaw-home");
    const resolvedHomeDir = path.resolve(homeDir);
    const previousDaisyClawHome = process.env.DAISYCLAW_HOME;
    const previousStateDir = process.env.DAISYCLAW_STATE_DIR;
    const previousBundledDir = process.env.DAISYCLAW_BUNDLED_PLUGINS_DIR;
    process.env.DAISYCLAW_HOME = "/srv/daisyclaw-home";
    process.env.DAISYCLAW_STATE_DIR = "/srv/daisyclaw-state";
    process.env.DAISYCLAW_BUNDLED_PLUGINS_DIR = "/srv/daisyclaw-bundled";
    ...
    try {
      ...
      expect(env.DAISYCLAW_HOME).toBeUndefined();
      expect(env.DAISYCLAW_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.DAISYCLAW_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("DAISYCLAW_HOME", previousDaisyClawHome);
      restoreEnvKey("DAISYCLAW_STATE_DIR", previousStateDir);
      restoreEnvKey("DAISYCLAW_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
    ...
    try {
      const seen = withPathResolutionEnv(
        ...
        (env) => ({
          ...
          processDaisyClawHome: process.env.DAISYCLAW_HOME,
          processBundledDir: process.env.DAISYCLAW_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.DAISYCLAW_BUNDLED_PLUGINS_DIR,
        }),
      );

      expect(seen).toEqual({
        processHome: resolvedHomeDir,
        processDaisyClawHome: undefined,
        processBundledDir: "~/bundled",
        envBundledDir: "~/bundled",
      });
      expect(process.env.DAISYCLAW_HOME).toBe("/srv/daisyclaw-home");
    } finally {
      restoreEnvKey("DAISYCLAW_HOME", previousDaisyClawHome);
    }
  });
