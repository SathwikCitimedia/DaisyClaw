...
describe("trajectory metadata", () => {
  it("redacts harness argv and local paths with the support redaction rules", () => {
    ...
    process.argv = [
      "node",
      "/Users/tester/project/daisyclaw.js",
      "--api-key",
      "super-secret",
      "--config=/Users/tester/.daisyclaw/daisyclaw.json",
    ];
    try {
      const metadata = buildTrajectoryRunMetadata({
        env: {
          HOME: "/Users/tester",
          DAISYCLAW_STATE_DIR: "/Users/tester/.daisyclaw",
        },
      ...
      expect(harness.invocation).toEqual([
        "node",
        "~/project/daisyclaw.js",
        "--api-key",
        "<redacted>",
        "--config=$DAISYCLAW_STATE_DIR/daisyclaw.json",
      ]);
      expect(harness.entrypoint).toBe("~/project/daisyclaw.js");
  ...
  it("redactPathForSupport returns empty string for null/undefined input", () => {
    const ctx: SupportRedactionContext = { env: {}, stateDir: "/tmp/.daisyclaw" };
