...
async function replaceTrajectoryWindow(params: {
  ...
  await writeSiblingTempFile({
    ...
    chmodDir: false,
    mode: 0o600,
    tempPrefix: ".daisyclaw-trajectory-",
...
export function createTrajectoryRuntimeRecorder(
  ...
  // override so operators can still disable recording with DAISYCLAW_TRAJECTORY=0.
  const enabled = parseBooleanValue(env.DAISYCLAW_TRAJECTORY) ?? true;
  if (!enabled) {
    return null;
  }
