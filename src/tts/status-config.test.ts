...
beforeAll(() => {
  fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-tts-status-"));
});
...
describe("resolveStatusTtsSnapshot", () => {
  it("uses prefs overrides without loading speech providers", async () => {
    await withStatusTempHome(async (home) => {
      const prefsPath = path.join(home, ".daisyclaw", "settings", "tts.json");
  ...
  it("derives the default prefs path from DAISYCLAW_CONFIG_PATH when set", async () => {
    await withStatusTempHome(async (home) => {
      const stateDir = path.join(home, ".daisyclaw-dev");
