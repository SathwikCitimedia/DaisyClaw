...
async function withModeExecProviderFixture(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), `daisyclaw-tui-mode-${label}-`));
...
describe("resolveGatewayConnection", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv([
      ...
      "DAISYCLAW_TUI_SETUP_AUTH_SOURCE",
    ]);
    ...
    resolveStateDir.mockImplementation(
      (env: NodeJS.ProcessEnv) => env.DAISYCLAW_STATE_DIR ?? "/tmp/daisyclaw",
    );
    resolveConfigPath.mockImplementation(
      (env: NodeJS.ProcessEnv, stateDir: string) =>
        env.DAISYCLAW_CONFIG_PATH ?? `${stateDir}/daisyclaw.json`,
    );
    ...
    delete process.env.DAISYCLAW_TUI_SETUP_AUTH_SOURCE;
  });
  ...
  it("falls back to DAISYCLAW_GATEWAY_TOKEN when config token is missing", async () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });
  ...
  it("keeps normal TUI local password mode env precedence by default", async () => {
    ...
    await withEnvAsync({ DAISYCLAW_GATEWAY_PASSWORD: "env-password" }, async () => {
      const result = await resolveGatewayConnection({});
  ...
  it("uses configured local password for setup-launched TUI despite stale gateway password env", async () => {
    ...
    await withEnvAsync(
      {
        DAISYCLAW_GATEWAY_PASSWORD: "stale-env-password", // pragma: allowlist secret
        DAISYCLAW_TUI_SETUP_AUTH_SOURCE: "config",
      },
      async () => {
        const result = await resolveGatewayConnection({});
        expect(result.password).toBe("config-password");
  ...
  it("still resolves env SecretRefs for setup-launched TUI config auth", async () => {
    ...
    await withEnvAsync(
      {
        DAISYCLAW_GATEWAY_PASSWORD: "resolved-ref-password", // pragma: allowlist secret
        DAISYCLAW_TUI_SETUP_AUTH_SOURCE: "config",
      },
      async () => {
        const result = await resolveGatewayConnection({});
  ...
  it("prefers DAISYCLAW_GATEWAY_PASSWORD over remote password fallback", async () => {
    loadConfig.mockReturnValue({
      gateway: {
        mode: "remote",
    ...
    const gatewayPasswordEnv = "DAISYCLAW_GATEWAY_PASSWORD"; // pragma: allowlist secret
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-tui-file-secret-"));
