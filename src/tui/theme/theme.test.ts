...
describe("light background detection", () => {
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      DAISYCLAW_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when DAISYCLAW_THEME=light", async () => {
    const mod = await importThemeWithEnv({ DAISYCLAW_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when DAISYCLAW_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ DAISYCLAW_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats DAISYCLAW_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ DAISYCLAW_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("DAISYCLAW_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      DAISYCLAW_THEME: "dark",
      COLORFGBG: "0;15",
    });
    expect(mod.lightMode).toBe(false);
  });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ DAISYCLAW_THEME: "light" });
    const darkMod = await importThemeWithEnv({ DAISYCLAW_THEME: "dark" });
