...
describe("launchTuiCli", () => {
  beforeEach(() => {
    ...
    process.argv[1] = "/repo/daisyclaw.mjs";
  ...
  it("passes local mode through to the relaunched TUI", async () => {
    ...
    const options = expectSpawned(["/repo/daisyclaw.mjs", "tui", "--local"]);
    expect(options.stdio).toBe("inherit");
  });
  ...
  it("pins the child gateway URL and config auth source through env without adding url argv", async () => {
    ...
    const options = expectSpawned(["/repo/daisyclaw.mjs", "tui"]);
    expect(options.env?.DAISYCLAW_GATEWAY_URL).toBe("ws://127.0.0.1:18789");
    expect(options.env?.DAISYCLAW_TUI_SETUP_AUTH_SOURCE).toBe("config");
  });
});
