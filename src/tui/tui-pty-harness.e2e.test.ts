...
async function writeTuiPtyFixtureScript(dir: string) {
  ...
  await writeFile(
    ...
    `
      ...
      const actionLogPath = process.env.DAISYCLAW_TUI_PTY_LOG_PATH;
      const gatewayStatus = process.env.DAISYCLAW_TUI_PTY_GATEWAY_STATUS ?? "fixture gateway ok";
      ...
      let fastMode = process.env.DAISYCLAW_TUI_PTY_FAST_MODE === "true";
      ...
      async function main() {
        await runTui({
          ...
          deliver: false,
          historyLimit: 5,
          title: "daisyclaw tui pty fixture",
        });
      }
...
async function startTuiFixture(opts: { env?: NodeJS.ProcessEnv } = {}) {
  const tempDir = await mkdtemp(path.join(tmpdir(), "daisyclaw-tui-pty-"));
  ...
  const run = startPty(process.execPath, ["--import", "tsx", scriptPath], {
    ...
    env: {
      ...
      DAISYCLAW_TUI_PTY_LOG_PATH: logPath,
      NO_COLOR: undefined,
      ...opts.env,
    },
