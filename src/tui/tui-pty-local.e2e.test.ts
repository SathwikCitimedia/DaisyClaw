  ...
  return {
    ...
    agents: {
      defaults: {
        ...
        models: {
          "tui-pty-mock/gpt-5.5": { agentRuntime: { id: "daisyclaw" } },
        },
        skills: [],
        skipBootstrap: true,
      },
...
async function startLocalModeTui() {
  ...
  const tempDir = await mkdtemp(path.join(tmpdir(), "daisyclaw-tui-pty-local-"));
  ...
  const script = [
    ...
    `program.parseAsync([process.execPath, "daisyclaw", "tui", "--local"], { from: "node" }).catch((error) => {`,
    `  console.error(error);`,
    `  process.exit(1);`,
    `});`,
  ].join("\n");
  ...
  const run = startPty(process.execPath, ["--import", "tsx", "--eval", script], {
    ...
    env: {
      ...
      DAISYCLAW_CODEX_DISCOVERY_LIVE: "0",
      NO_COLOR: undefined,
    },
