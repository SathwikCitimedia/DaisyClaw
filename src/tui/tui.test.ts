...
describe("resolveTuiFooterHostLabel", () => {
  ...
  it("renders only remote hosts when explicitly enabled", () => {
    const config = { tui: { footer: { showRemoteHost: true } } } satisfies DaisyClawConfig;
...
describe("resolveTuiShutdownHardExitMs", () => {
  ...
  it("adds local run shutdown grace before forcing embedded shutdown", () => {
    withEnv({ DAISYCLAW_TUI_LOCAL_RUN_SHUTDOWN_GRACE_MS: "3456" }, () => {
      expect(resolveTuiShutdownHardExitMs({ localMode: true })).toBe(5456);
    });
  });

  it("ignores partial local run shutdown grace values", () => {
    withEnv({ DAISYCLAW_TUI_LOCAL_RUN_SHUTDOWN_GRACE_MS: "3456abc" }, () => {
...
describe("resolveInitialTuiAgentId", () => {
  const cfg: DaisyClawConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/daisyclaw" },
        { id: "ops", workspace: "/tmp/daisyclaw/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/daisyclaw/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run daisyclaw devices list");
    expect(state.pairingHint).toContain("daisyclaw devices list");
  });
...
describe("TUI shutdown safety", () => {
  ...
  it("schedules a process-exit guard after standalone TUI return", () => {
    ...
    callback?.();
    expect(writeStderr).toHaveBeenCalledWith("daisyclaw tui forcing process exit after return\n");
...
describe("resolveLocalAuthCliInvocation", () => {
  it("uses the source runner when dist is unavailable", () => {
    expect(
      resolveLocalAuthCliInvocation({
        ...
        wrapperPath: "/repo/daisyclaw.mjs",
  ...
  it("uses the packaged wrapper when dist is available", () => {
    expect(
      ...
      command: "/usr/bin/node",
      args: ["/repo/daisyclaw.mjs", "models", "auth", "login"],
    });
  });
});
