...
import { getRuntimeConfig, type DaisyClawConfig } from "../config/config.js";
...
const DAISYCLAW_CLI_WRAPPER_PATH = fileURLToPath(new URL("../../daisyclaw.mjs", import.meta.url));
const DAISYCLAW_RUN_NODE_SCRIPT_PATH = fileURLToPath(
  new URL("../../scripts/run-node.mjs", import.meta.url),
);
const DAISYCLAW_DIST_ENTRY_JS_PATH = fileURLToPath(new URL("../../dist/entry.js", import.meta.url));
const DAISYCLAW_DIST_ENTRY_MJS_PATH = fileURLToPath(
...
export function resolveLocalAuthCliInvocation(params?: {
  ...
  const hasDistEntry =
    params?.hasDistEntry ??
    (existsSync(DAISYCLAW_DIST_ENTRY_JS_PATH) || existsSync(DAISYCLAW_DIST_ENTRY_MJS_PATH));
  const hasRunNodeScript = params?.hasRunNodeScript ?? existsSync(DAISYCLAW_RUN_NODE_SCRIPT_PATH);
  const command = params?.execPath ?? process.execPath;
  const wrapperPath = params?.wrapperPath ?? DAISYCLAW_CLI_WRAPPER_PATH;
  const runNodePath = params?.runNodePath ?? DAISYCLAW_RUN_NODE_SCRIPT_PATH;
...
export function resolveGatewayDisconnectState(reason?: string): {
  ...
  if (/pairing required/i.test(reasonLabel)) {
    return {
      ...
      activityStatus: "pairing required: run daisyclaw devices list",
      pairingHint:
        "Pairing required. Run `daisyclaw devices list`, approve your request ID, then reconnect.",
    };
  }
  return {
    connectionStatus: `gateway disconnected: ${reasonLabel}`,
...
export function scheduleProcessExitAfterTuiReturn(
  ...
  const timer = setTimeoutFn(() => {
    try {
      writeStderr("daisyclaw tui forcing process exit after return\n");
    } catch {
...
function resolveEmptySessionInfoDefaults(config: DaisyClawConfig): SessionInfo {
  return {
...
export async function runTui(opts: RunTuiOptions): Promise<TuiResult> {
  ...
  const previousConsoleSubsystemFilter = isLocalMode
    ...
    : null;
  if (isLocalMode) {
    setConsoleSubsystemFilter(["__daisyclaw_tui_quiet__"]);
  }
  ...
  const updateHeader = () => {
    ...
    const title = opts.title ?? "daisyclaw tui";
    header.setText(
      theme.header(
  ...
  const forceExit = () => {
    try {
      process.stderr.write("daisyclaw tui forcing exit\n");
    } catch {
    ...
    void Promise.resolve()
      ...
      .catch((err: unknown) => {
        if (!isTuiTerminalLossError(err)) {
          try {
            process.stderr.write(`daisyclaw tui shutdown failed: ${String(err)}\n`);
          } catch {
