...
/** Resolves `~` and DaisyClaw home-relative paths with injectable env/home sources. */
...
/** Resolves the DaisyClaw config directory from state/config env overrides or home. */
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".daisyclaw");
  try {
...
/** Resolves the effective DaisyClaw home directory, if one can be determined. */
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.DAISYCLAW_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$DAISYCLAW_HOME" };
  }
  return { home, prefix: "~" };
}

/** Replaces the leading home directory in a path with `~` or `$DAISYCLAW_HOME`. */
...
// Configuration root; can be overridden via DAISYCLAW_STATE_DIR.
