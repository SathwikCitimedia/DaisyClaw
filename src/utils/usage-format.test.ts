...
describe("usage-format", () => {
  ...
  beforeEach(async () => {
    envSnapshot = captureEnv(["DAISYCLAW_AGENT_DIR", "DAISYCLAW_STATE_DIR"]);
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-usage-format-"));
  ...
  it("prefers models.json pricing over daisyclaw config and cached pricing", async () => {
    const config = {
      models: {
        providers: {
          "demo-preferred": {
  ...
  it("falls back to daisyclaw config pricing when models.json is absent", () => {
    const config = {
      models: {
        providers: {
