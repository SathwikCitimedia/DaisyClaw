...
const versionFixtureRoot = createSuiteTempRootTracker({ prefix: "daisyclaw-version-" });
...
describe("version resolution", () => {
  it("keeps bundled version injection as a direct define identifier", async () => {
    ...
    expect(source).toContain("typeof __DAISYCLAW_VERSION__");
    expect(source).toContain("? __DAISYCLAW_VERSION__");
  });

  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "daisyclaw", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "daisyclaw", version: "2.3.4" });
  ...
  it("ignores non-daisyclaw package and blank build-info versions", async () => {
    await withVersionFixtureDir(async (root) => {
  ...
  it("prefers DAISYCLAW_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        DAISYCLAW_VERSION: "9.9.9",
        DAISYCLAW_SERVICE_VERSION: "2.2.2",
  ...
  it("prefers runtime VERSION over stale DAISYCLAW_VERSION for compatibility checks", () => {
    const previous = process.env.DAISYCLAW_VERSION;
    const previousService = process.env.DAISYCLAW_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      process.env.DAISYCLAW_VERSION = "2026.3.25";
      process.env.DAISYCLAW_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      restoreEnvValue("DAISYCLAW_VERSION", previous);
      restoreEnvValue("DAISYCLAW_SERVICE_VERSION", previousService);
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        DAISYCLAW_VERSION: "2026.3.99",
        DAISYCLAW_SERVICE_VERSION: "2026.3.98",
  ...
  it("prefers explicit compatibility host overrides over runtime and stale env versions", () => {
    expect(
      resolveCompatibilityHostVersion({
        DAISYCLAW_COMPATIBILITY_HOST_VERSION: "2026.4.8",
        DAISYCLAW_VERSION: "2026.3.99",
        DAISYCLAW_SERVICE_VERSION: "2026.3.98",
        npm_package_version: "2026.3.97",
      }),
  ...
  it("prefers runtime VERSION over service/package markers and ignores unusable env values", () => {
    expect(
      resolveRuntimeServiceVersion({
        DAISYCLAW_VERSION: "   ",
        DAISYCLAW_SERVICE_VERSION: "  2.0.0  ",
    ...
    expect(
      resolveRuntimeServiceVersion({
        ...
        DAISYCLAW_SERVICE_VERSION: "null",
        npm_package_version: "1.0.0-package",
      }),
    ).toBe(VERSION);
  });
