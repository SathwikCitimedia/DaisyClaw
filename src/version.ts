...
declare const __DAISYCLAW_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "daisyclaw";
...
function readInjectedVersion(): string | undefined {
  return typeof __DAISYCLAW_VERSION__ === "string" ? __DAISYCLAW_VERSION__ : undefined;
}
...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["DAISYCLAW_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["DAISYCLAW_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["DAISYCLAW_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.DAISYCLAW_COMPATIBILITY_HOST_VERSION);
...
// Single source of truth for the current DaisyClaw version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: readInjectedVersion(),
  bundledVersion: process.env.DAISYCLAW_BUNDLED_VERSION,
});
