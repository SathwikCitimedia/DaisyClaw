...
let listedConfigs: Array<DaisyClawConfig | undefined> = [];
...
describe("video-generation runtime", () => {
    ...
    providers = [
      {
        ...
        async generateVideo(req) {
          seenSupportedDurationHint = (req as Record<symbol, readonly number[] | undefined>)[
            Symbol.for("daisyclaw.videoGeneration.supportedDurations")
          ];
          seenRequest = {
  ...
  it("lists runtime video-generation providers through the provider registry", () => {
    ...
    expect(
      listRuntimeVideoGenerationProviders({ config: {} as DaisyClawConfig }, runtimeDeps),
    ).toEqual(registryProviders);
    expect(listedConfigs).toEqual([{} as DaisyClawConfig]);
  });
  ...
  it("builds a generic config hint without hardcoded provider ids", async () => {
    ...
    await expect(
      runGenerateVideo({ cfg: {} as DaisyClawConfig, prompt: "animate a cat" }),
    ).rejects.toThrow(
