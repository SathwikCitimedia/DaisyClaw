...
export async function finalizeSetupWizard(
  ...
  if (!opts.skipHealth) {
    ...
    if (gatewayProbe.ok) {
      try {
        const healthConfig: DaisyClawConfig =
      ...
      await prompter.note(
        [
          ...
          t("wizard.finalize.startGatewayNow", {
            command: formatCliCommand("daisyclaw gateway run"),
          }),
          t("wizard.finalize.rerunInstallDaemon", {
            command: formatCliCommand("daisyclaw onboard --install-daemon"),
          }),
          t("wizard.finalize.skipHealthNextTime", {
            command: formatCliCommand("daisyclaw onboard --skip-health"),
          }),
        ].join("\n"),
        "Gateway",
      );
    ...
    if (gatewayProbe.ok) {
      const tokenNotes = [
        ...
        t("wizard.finalize.gatewayTokenView", {
          command: formatCliCommand("daisyclaw config get gateway.auth.token"),
        }),
        t("wizard.finalize.gatewayTokenGenerate", {
          command: formatCliCommand("daisyclaw doctor --generate-gateway-token"),
        }),
        suppressGatewayTokenOutput ? undefined : t("wizard.finalize.dashboardTokenMemory"),
        t("wizard.finalize.dashboardOpenAnytime", {
          command: formatCliCommand("daisyclaw dashboard --no-open"),
        }),
    ...
    if (!entry) {
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("daisyclaw configure --section web")}`,
          "",
      ...
      await prompter.note(
        [
          ...
          t("wizard.finalize.webSearchGetKey", {
            url: entry?.signupUrl ?? "https://docs.daisyclaw.ai/tools/web",
          }),
          t("wizard.finalize.webDocs"),
        ].join("\n"),
        t("wizard.finalize.webSearchTitle"),
      ...
      await prompter.note(
        [
          ...
          t("wizard.finalize.webSearchReenable", {
            command: formatCliCommand("daisyclaw configure --section web"),
          }),
          "",
          t("wizard.finalize.webDocs"),
        ].join("\n"),
