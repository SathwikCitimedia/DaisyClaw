...
import type { GatewayBindMode, GatewayTailscaleMode, DaisyClawConfig } from "../config/config.js";
    ...
    if (tokenMode === "ref") {
      if (flow === "quickstart" && quickstartTokenRef) {
        ...
        const resolved = await promptSecretRefForSetup({
          ...
          config: nextConfig,
          prompter,
          preferredEnvVar: "DAISYCLAW_GATEWAY_TOKEN",
          copy: {
            sourceMessage: t("wizard.gateway.authTokenStoredMessage"),
            envVarPlaceholder: "DAISYCLAW_GATEWAY_TOKEN",
          },
        });
      ...
      gatewayToken =
        (quickstartTokenString ?? normalizeGatewayTokenInput(process.env.DAISYCLAW_GATEWAY_TOKEN)) ||
        randomToken();
    ...
    if (!password) {
      ...
      if (selectedMode === "ref") {
        const resolved = await promptSecretRefForSetup({
          ...
          config: nextConfig,
          prompter,
          preferredEnvVar: "DAISYCLAW_GATEWAY_PASSWORD",
          copy: {
            sourceMessage: t("wizard.gateway.authPasswordStoredMessage"),
            envVarPlaceholder: "DAISYCLAW_GATEWAY_PASSWORD",
          },
        });
