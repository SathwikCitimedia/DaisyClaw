...
function hasMeaningfulConfig(config: DaisyClawConfig): boolean {
...
function assertFreshSetupMigrationTarget(freshness: {
  ...
  if (freshness.fresh || process.env.DAISYCLAW_MIGRATION_EXISTING_IMPORT === "1") {
    return;
  }
  throw new Error(
    [
      "Migration import during onboarding requires a fresh DaisyClaw setup.",
...
export async function runSetupMigrationImport(params: {
  ...
  prompter: WizardPrompter;
  runtime: RuntimeEnv;
  commitConfigFile: (config: DaisyClawConfig) => Promise<DaisyClawConfig>;
}): Promise<void> {
  const [
  ...
  // can reference a concrete DaisyClaw config target.
