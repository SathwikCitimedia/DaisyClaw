...
function makeConfig(): DaisyClawConfig {
  return {
    secrets: {
      providers: {
        default: { source: "env" },
...
describe("resolveSetupSecretInputString", () => {
  it("resolves env-template SecretInput strings", async () => {
    const resolved = await resolveSetupSecretInputString({
      config: makeConfig(),
      value: "${DAISYCLAW_GATEWAY_PASSWORD}",
      path: "gateway.auth.password",
      env: {
        DAISYCLAW_GATEWAY_PASSWORD: "gateway-secret", // pragma: allowlist secret
      },
    });
  ...
  it("throws with path context when env-template SecretRef cannot resolve", async () => {
    await expect(
      ...
      'gateway.auth.password: failed to resolve SecretRef "env:default:DAISYCLAW_GATEWAY_PASSWORD"',
    );
  });
});
