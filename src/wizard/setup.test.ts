...
import type { ProviderPlugin } from "daisyclaw/plugin-sdk/provider-model-shared";
...
const resolveGatewayPort = vi.hoisted(() =>
  vi.fn((_cfg?: unknown, env?: NodeJS.ProcessEnv) => {
    const raw = env?.DAISYCLAW_GATEWAY_PORT ?? process.env.DAISYCLAW_GATEWAY_PORT;
...
const readConfigFileSnapshot = vi.hoisted(() =>
  vi.fn(async () => ({
    path: "/tmp/.daisyclaw/daisyclaw.json",
    exists: false,
...
describe("runSetupWizard", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-onboard-suite-"));
  });
  ...
  it("allows size-drop writes for pending plugin install record migration", async () => {
    ...
    readConfigFileSnapshot.mockResolvedValueOnce({
      ...
      config: {
        plugins: {
          installs: {
            demo: { source: "npm", spec: "@daisyclaw/demo-plugin" },
          },
        },
      },
      issues: [],
  ...
  it("resolves gateway.auth.password SecretRef for local setup probe", async () => {
    const previous = process.env.DAISYCLAW_GATEWAY_PASSWORD;
    process.env.DAISYCLAW_GATEWAY_PASSWORD = "gateway-ref-password"; // pragma: allowlist secret
    ...
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.DAISYCLAW_GATEWAY_PASSWORD = previous;
      }
    }
  ...
  it("shows the resolved gateway port in quickstart for fresh envs", async () => {
    const previousPort = process.env.DAISYCLAW_GATEWAY_PORT;
    process.env.DAISYCLAW_GATEWAY_PORT = "18791";
    const note: WizardPrompter["note"] = vi.fn(async () => {});
    const prompter = buildWizardPrompter({ note });
    const runtime = createRuntime();
    ...
    try {
      ...
      if (previousPort === undefined) {
        ...
        process.env.DAISYCLAW_GATEWAY_PORT = previousPort;
      }
    }
