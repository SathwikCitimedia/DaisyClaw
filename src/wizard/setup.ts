// Setup wizard orchestrates onboarding prompts and generated DaisyClaw config.
...
async function writeWizardConfigFile(
  configInput: DaisyClawConfig,
  opts: {
    allowConfigSizeDrop?: boolean;
    migrationBaseConfig?: DaisyClawConfig;
...
export async function runSetupWizard(
  ...
  let baseConfig: DaisyClawConfig = snapshot.valid
    ? snapshot.exists
  ...
  let pendingPluginInstallMigrationBaseConfig: DaisyClawConfig | undefined = baseConfig;
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("daisyclaw doctor")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }
    ...
    await prompter.note(
      [
        ...
        ...(compatibilityNotices.length > 4
          ...
          : []),
        "",
        `Review: ${formatCliCommand("daisyclaw doctor")}`,
        `Inspect: ${formatCliCommand("daisyclaw plugins inspect --all")}`,
      ].join("\n"),
  ...
  const quickstartHint = t("wizard.setup.flowQuickstartHint", {
    command: formatCliCommand("daisyclaw configure"),
  });
  ...
  if (
    ...
    runtime.error(
      "Invalid --flow. Use quickstart, manual, advanced, or import. Example: daisyclaw onboard --flow quickstart",
    );
    runtime.exit(1);
    return;
  }
  ...
  let localGatewayToken = process.env.DAISYCLAW_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword = process.env.DAISYCLAW_GATEWAY_PASSWORD;
  try {
  ...
  let nextConfig: DaisyClawConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
  if (opts.skipBootstrap) {
    nextConfig = applySkipBootstrapConfig(nextConfig);
  }
