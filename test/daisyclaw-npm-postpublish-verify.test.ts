// DaisyClaw npm postpublish tests validate postpublish verification behavior.
...
} from "../scripts/daisyclaw-npm-postpublish-verify.ts";
...
describe("buildPublishedInstallScenarios", () => {
  it("uses a single fresh scenario for plain stable releases", () => {
    expect(buildPublishedInstallScenarios("2026.3.23")).toEqual([
      {
        name: "fresh-exact",
        installSpecs: ["daisyclaw@2026.3.23"],
  ...
  it("adds a stable-to-correction upgrade scenario for correction releases", () => {
    expect(buildPublishedInstallScenarios("2026.3.23-2")).toEqual([
      {
        name: "fresh-exact",
        installSpecs: ["daisyclaw@2026.3.23-2"],
        expectedVersion: "2026.3.23-2",
      },
      {
        name: "upgrade-from-base-stable",
        installSpecs: ["daisyclaw@2026.3.23", "daisyclaw@2026.3.23-2"],
        expectedVersion: "2026.3.23-2",
      },
    ]);
  });
...
describe("buildPublishedInstallCommandArgs", () => {
  it("runs lifecycle scripts for published install verification", () => {
    const args = buildPublishedInstallCommandArgs("/tmp/daisyclaw-prefix", "daisyclaw@2026.4.10");

    expect(args).toEqual([
      "install",
      "-g",
      "--prefix",
      "/tmp/daisyclaw-prefix",
      "daisyclaw@2026.4.10",
      "--no-fund",
      "--no-audit",
    ]);
...
describe("collectInstalledPackageErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "daisyclaw-postpublish-package-"));
  }
  ...
  it("flags version mismatches", () => {
    const errors = collectInstalledPackageErrors({
      ...
      packageRoot: "/tmp/empty-daisyclaw",
    });
...
describe("collectInstalledContextEngineRuntimeErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "daisyclaw-postpublish-context-engine-"));
  }
...
describe("collectInstalledPluginSdkZodArtifactErrors", () => {
  function withInstalledPackageRoot(run: (packageRoot: string) => void): void {
    const packageRoot = mkdtempSync(join(tmpdir(), "daisyclaw-postpublish-zod-sdk-"));
    try {
      run(packageRoot);
    } finally {
      rmSync(packageRoot, { recursive: true, force: true });
...
describe("normalizeInstalledBinaryVersion", () => {
  it("accepts decorated CLI version output", () => {
    expect(normalizeInstalledBinaryVersion("DaisyClaw 2026.4.8 (9ece252)")).toBe("2026.4.8");
    expect(normalizeInstalledBinaryVersion("DaisyClaw 2026.4.8-beta.1 (9ece252)")).toBe(
      "2026.4.8-beta.1",
    );
    expect(normalizeInstalledBinaryVersion("DaisyClaw 2026.4.8-alpha.1 (9ece252)")).toBe(
      "2026.4.8-alpha.1",
    );
  });
});
...
describe("resolveInstalledBinaryPath", () => {
  it("uses the Unix global bin path on non-Windows platforms", () => {
    expect(resolveInstalledBinaryPath("/tmp/daisyclaw-prefix", "darwin")).toBe(
      "/tmp/daisyclaw-prefix/bin/daisyclaw",
    );
  });

  it("uses the Windows npm shim path on win32", () => {
    expect(resolveInstalledBinaryPath("C:/daisyclaw-prefix", "win32")).toBe(
      "C:\\daisyclaw-prefix\\daisyclaw.cmd",
    );
  });
});
...
describe("resolveInstalledBinaryCommandInvocation", () => {
  it("runs the Unix installed binary directly", () => {
    expect(
      resolveInstalledBinaryCommandInvocation("/tmp/daisyclaw-prefix", ["--version"], {
        platform: "linux",
      }),
    ).toEqual({
      command: "/tmp/daisyclaw-prefix/bin/daisyclaw",
      args: ["--version"],
    });
  });
  ...
  it("wraps the Windows installed npm shim without Node shell argv", () => {
    expect(
      ...
      args: [
        ...
        "/s",
        "/c",
        '""C:\\daisyclaw prefix\\daisyclaw.cmd" agent --message "hello world""',
      ],
...
describe("collectInstalledRootDependencyManifestErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "daisyclaw-postpublish-root-deps-"));
  }
