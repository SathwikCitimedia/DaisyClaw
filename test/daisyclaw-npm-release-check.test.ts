// DaisyClaw npm release check tests validate package release checks.
...
} from "../scripts/daisyclaw-npm-release-check.ts";
import {
...
describe("shouldSkipPackedTarballValidation", () => {
  ...
  it("accepts truthy values for metadata-only validation", () => {
    expect(
      shouldSkipPackedTarballValidation({
        DAISYCLAW_NPM_RELEASE_SKIP_PACK_CHECK: "1",
      }),
    ).toBe(true);
  });
  ...
  it("treats false-like values as disabled", () => {
    expect(
      shouldSkipPackedTarballValidation({
        DAISYCLAW_NPM_RELEASE_SKIP_PACK_CHECK: "false",
      }),
    ).toBe(false);
  });
});
...
describe("resolveNpmCommandInvocation", () => {
  it("uses npm_execpath when it points to npm", () => {
    expect(
      resolveNpmCommandInvocation({
        ...
        npmArgs: ["view", "daisyclaw", "version"],
        nodeExecPath: "/usr/local/bin/node",
        platform: "linux",
      }),
    ).toEqual({
      command: "/usr/local/bin/node",
      args: ["/usr/local/lib/node_modules/npm/bin/npm-cli.js", "view", "daisyclaw", "version"],
    });
  });
  ...
  it("wraps the Windows npm command when npm_execpath is missing", () => {
    expect(
      resolveNpmCommandInvocation({
        ...
        npmArgs: ["view", "daisyclaw@beta", "version"],
        npmExecPath: "",
        platform: "win32",
      }),
    ).toEqual({
      command: "C:\\Windows\\System32\\cmd.exe",
      args: ["/d", "/s", "/c", "npm.cmd view daisyclaw@beta version"],
  ...
  it("wraps Windows npm_execpath command shims", () => {
    expect(
      resolveNpmCommandInvocation({
        comSpec: "C:\\Windows\\System32\\cmd.exe",
        npmArgs: ["install", "-g", "C:\\tmp\\daisyclaw package.tgz"],
      ...
      args: [
        ...
        "/s",
        "/c",
        '""C:\\Program Files\\nodejs\\npm.cmd" install -g "C:\\tmp\\daisyclaw package.tgz""',
      ],
      windowsVerbatimArguments: true,
    });
  });
  ...
  if (process.platform === "win32") {
    it("executes fallback npm.cmd through cmd.exe on Windows", () => {
      const dir = mkdtempSync(join(tmpdir(), "daisyclaw-fake-npm-cmd-"));
      try {
        ...
        writeFileSync(
          ...
          [
            ...
            "fs.writeFileSync(process.env.DAISYCLAW_FAKE_NPM_OUT, JSON.stringify(process.argv.slice(2)));",
          ].join("\n"),
        );
        writeFileSync(
          join(dir, "npm.cmd"),
        ...
        execFileSync(invocation.command, invocation.args, {
          ...
          env: {
            ...process.env,
            DAISYCLAW_FAKE_NPM_OUT: outputPath,
...
describe("resolveNpmReleaseCheckCommandTimeoutMs", () => {
  it("parses only positive integer environment timeouts", () => {
    ...
    expect(
      resolveNpmReleaseCheckCommandTimeoutMs({ DAISYCLAW_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS: "" }),
    ).toBe(10 * 60 * 1000);
    expect(
      resolveNpmReleaseCheckCommandTimeoutMs({
        DAISYCLAW_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS: "1234",
      }),
    ).toBe(1234);
    ...
    for (const raw of ["nope", "10m", "1e3", "0", "-1", "9007199254740992"]) {
      expect(() =>
        resolveNpmReleaseCheckCommandTimeoutMs({
          DAISYCLAW_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS: raw,
        }),
      ).toThrow(`invalid DAISYCLAW_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS: ${raw}`);
    }
  });
});
...
describe("parseNpmPackJsonOutput", () => {
  it("parses a plain npm pack JSON array", () => {
    expect(parseNpmPackJsonOutput('[{"filename":"daisyclaw.tgz","files":[]}]')).toEqual([
      { filename: "daisyclaw.tgz", files: [] },
    ]);
  });
  ...
  it("parses the trailing JSON payload after npm lifecycle logs", () => {
    const stdout = [
      ...
      "",
      "> daisyclaw@2026.3.23 prepack",
      "> pnpm build && pnpm ui:build",
      "",
      "[copy-hook-metadata] Copied 4 hook metadata files.",
      '[{"filename":"daisyclaw.tgz","files":[{"path":"dist/control-ui/index.html"}]}]',
    ].join("\n");

    expect(parseNpmPackJsonOutput(stdout)).toEqual([
      {
        filename: "daisyclaw.tgz",
  ...
  it("returns null when no JSON payload is present", () => {
    expect(parseNpmPackJsonOutput("> daisyclaw@2026.3.23 prepack")).toBeNull();
  });
});
...
describe("collectForbiddenPackedPathErrors", () => {
  ...
  it("rejects root dist chunks that still reference the private qa lab", () => {
    const rootDir = mkdtempSync(join(tmpdir(), "daisyclaw-pack-private-qa-"));
  ...
  it("rejects private QA paths in the generated dist inventory", () => {
    const rootDir = mkdtempSync(join(tmpdir(), "daisyclaw-pack-inventory-"));
  ...
  it("validates the expected npm package metadata", () => {
    expect(
      collectReleasePackageMetadataErrors({
        ...
        license: "MIT",
        repository: { url: "git+https://github.com/daisyclaw/daisyclaw.git" },
