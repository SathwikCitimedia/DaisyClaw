// DaisyClaw prepack tests validate package prepack output.
import { describe, expect, it } from "vitest";
import {
  collectPreparedPrepackErrors,
  resolvePrepackCommandTimeoutMs,
  runPrepackCommand,
} from "../scripts/daisyclaw-prepack.ts";
...
describe("resolvePrepackCommandTimeoutMs", () => {
  it("parses only positive integer environment timeouts", () => {
    ...
    expect(resolvePrepackCommandTimeoutMs({ DAISYCLAW_PREPACK_COMMAND_TIMEOUT_MS: "" })).toBe(
      30 * 60 * 1000,
    );
    expect(resolvePrepackCommandTimeoutMs({ DAISYCLAW_PREPACK_COMMAND_TIMEOUT_MS: "1234" })).toBe(
      1234,
    );

    for (const raw of ["nope", "10m", "1e3", "0", "-1", "9007199254740992"]) {
      expect(() =>
        resolvePrepackCommandTimeoutMs({ DAISYCLAW_PREPACK_COMMAND_TIMEOUT_MS: raw }),
      ).toThrow(`invalid DAISYCLAW_PREPACK_COMMAND_TIMEOUT_MS: ${raw}`);
    }
  });
});
