// DaisyClaw test instance tests cover spawned test instance lifecycle.
import fs from "node:fs/promises";
import path from "node:path";
import { describe, expect, it } from "vitest";
import { createDaisyClawTestInstance, testing } from "./daisyclaw-test-instance.js";
...
describe("daisyclaw test instance", () => {
  ...
  it("creates isolated config and spawn env without mutating process env", async () => {
    ...
    const inst = await createDaisyClawTestInstance({
      name: "instance-unit",
    ...
    try {
      ...
      expect(inst.stateDir).toBe(path.join(inst.homeDir, ".daisyclaw"));
      expect(inst.configPath).toBe(path.join(inst.stateDir, "daisyclaw.json"));
      expect(inst.env.HOME).toBe(inst.homeDir);
      expect(inst.env.DAISYCLAW_STATE_DIR).toBe(inst.stateDir);
      expect(inst.env.DAISYCLAW_CONFIG_PATH).toBe(inst.configPath);
      expect(inst.env.DAISYCLAW_SKIP_CRON).toBe("0");
