...
import { createDaisyClawTestInstance, type DaisyClawTestInstance } from "./daisyclaw-test-instance.js";
...
export type GatewayInstance = DaisyClawTestInstance;
...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  const inst = await createDaisyClawTestInstance({ name });
  try {
