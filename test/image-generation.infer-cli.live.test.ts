...
const LIVE =
  isLiveTestEnabled() &&
  isTruthyEnvValue(process.env.DAISYCLAW_LIVE_INFER_CLI_TEST) &&
...
describeLive("image generation infer CLI live", () => {
  it("generates an image through daisyclaw infer", () => {
    const outputBase = path.join(os.tmpdir(), `daisyclaw-infer-image-${process.pid}.png`);
