...
const SHARED_TEST_SETUP = Symbol.for("daisyclaw.sharedTestSetup");
const EMBEDDED_RUN_STATE = Symbol.for("daisyclaw.embeddedRunState");
const REPLY_RUN_REGISTRY = Symbol.for("daisyclaw.replyRunRegistry");
...
function getSharedTestHome(): string | undefined {
  ...
  return globalState[SHARED_TEST_SETUP]?.tempHome ?? process.env.DAISYCLAW_TEST_HOME;
}
...
function restoreSharedTestHomeAfterEnvUnstub(testHomeRaw: string | undefined): void {
  ...
  process.env.DAISYCLAW_TEST_HOME = testHome;
...
function resetDaisyClawGlobalRunState(): void {
...
export default class DaisyClawNonIsolatedRunner extends TestRunner {
  override onCollectStart(file: { filepath: string }) {
    super.onCollectStart(file);
    restoreRealTimers();
    restoreNativeTimerGlobals();
    restoreSharedTestHomeAfterEnvUnstub(getSharedTestHome());
    const orderLogPath = process.env.DAISYCLAW_VITEST_FILE_ORDER_LOG?.trim();
    if (orderLogPath) {
