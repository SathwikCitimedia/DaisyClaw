...
describe("package scripts", () => {
  ...
  it("enables live cache validation in the package script", () => {
    expect(readPackageJson().scripts["test:live:cache"]).toBe(
      "node scripts/run-with-env.mjs DAISYCLAW_LIVE_TEST=1 DAISYCLAW_LIVE_CACHE_TEST=1 -- node --import tsx scripts/check-live-cache.ts",
    );
  });

  it("uses the shipped package launcher for npm start", () => {
    expect(readPackageJson().scripts.start).toBe("node daisyclaw.mjs");
  });
