...
describe("collectClawHubPublishablePluginPackages", () => {
  ...
  it("validates only selected package names when filters are provided", () => {
    ...
    ).toEqual(["@daisyclaw/demo-plugin"]);
  });
});

describe("DaisyClaw dual-published plugin metadata", () => {
  const dualPublishedPlugins = [
    {
      extensionId: "diagnostics-otel",
      packageName: "@daisyclaw/diagnostics-otel",
    },
    {
  ...
  it("keeps diagnostics plugins selectable through both ClawHub and npm release paths", () => {
    ...
    for (const plugin of dualPublishedPlugins) {
      ...
      expect(packageJson.daisyclaw?.install).toEqual({
        clawhubSpec: `clawhub:${plugin.packageName}`,
        defaultChoice: "npm",
        minHostVersion: ">=2026.4.25",
        npmSpec: plugin.packageName,
      });
      expect(packageJson.daisyclaw?.release).toEqual({
        publishToClawHub: true,
        publishToNpm: true,
      });
    }
...
describe("collectClawHubVersionGateErrors", () => {
  it("requires a version bump when a publishable plugin changes", () => {
    ...
    expect(errors).toEqual([
      "@daisyclaw/demo-plugin@2026.4.1: changed publishable plugin still has the same version in package.json.",
    ]);
  });
...
describe("collectPluginClawHubReleasePlan", () => {
  it("skips versions that already exist on ClawHub", async () => {
    ...
    const plan = await collectPluginClawHubReleasePlan({
      rootDir: repoDir,
      selection: ["@daisyclaw/demo-plugin"],
    ...
    expect(plan.skippedPublished[0]).toEqual({
      ...
      artifactName: "clawhub-package-daisyclaw-demo-plugin-2026.4.1",
      channel: "stable",
      extensionId: "demo-plugin",
  ...
  it("plans selected packages without validating unrelated publishable packages", async () => {
    ...
    expect(plan.candidates.map((plugin) => plugin.packageName)).toEqual(["@daisyclaw/demo-plugin"]);
    expect(plan.candidates.map((plugin) => plugin.artifactName)).toEqual([
      "clawhub-package-daisyclaw-demo-plugin-2026.4.1",
    ]);
  });
});
...
if [[ "\${1:-}" == "package" && "\${2:-}" == "pack" ]]; then
  ...
  pack_path="$pack_destination/daisyclaw-demo-plugin-2026.4.1.tgz"
  printf 'fake tgz\\n' > "$pack_path"
  printf '{"path":"%s","name":"@daisyclaw/demo-plugin","version":"2026.4.1"}\\n' "$pack_path"
fi
exit 0
`,
    );
    ...
    const output = execFileSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          DAISYCLAW_PLUGIN_NPM_RUNTIME_BUILD: "0",
...
`,
    ...
    const output = execFileSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          DAISYCLAW_CLAWHUB_PACK_OUTPUT_DIR: outputDir,
          DAISYCLAW_PLUGIN_NPM_RUNTIME_BUILD: "0",
          PATH: `${binDir}${delimiter}${process.env.PATH ?? ""}`,
        },
      },
    ...
    expect(existsSync(join(outputDir, "daisyclaw-demo-plugin-2026.4.1.tgz"))).toBe(true);
    const invocations = readFileSync(markerPath, "utf8");
...
function createTempPluginRepo(
  ...
  const repoDir = makeTempRepoRoot(tempDirs, "daisyclaw-clawhub-release-");
  const extensionId = options.extensionId ?? "demo-plugin";
  const extensionIds = [extensionId, ...(options.extraExtensionIds ?? [])];

  writeFileSync(
    join(repoDir, "package.json"),
    JSON.stringify({ name: "daisyclaw-test-root" }, null, 2),
  );
  ...
  for (const currentExtensionId of extensionIds) {
    ...
    writeFileSync(
      ...
      JSON.stringify(
        {
          name: `@daisyclaw/${currentExtensionId}`,
          version: "2026.4.1",
          repository: {
            type: "git",
            url: DAISYCLAW_PLUGIN_NPM_REPOSITORY_URL,
          ...
          daisyclaw: {
            ...
            install: {
              npmSpec: `@daisyclaw/${currentExtensionId}`,
            },
            release: {
