...
import { bundledPluginFile, bundledPluginRoot } from "daisyclaw/plugin-sdk/test-fixtures";
...
import {
  ...
  DAISYCLAW_PLUGIN_NPM_REPOSITORY_URL,
...
describe("parsePluginReleaseSelection", () => {
  ...
  it("dedupes and sorts comma or whitespace separated package names", () => {
    expect(
      parsePluginReleaseSelection(" @daisyclaw/zalo, @daisyclaw/feishu  @daisyclaw/zalo "),
    ).toEqual(["@daisyclaw/feishu", "@daisyclaw/zalo"]);
  });
});
...
describe("collectPublishablePluginPackageErrors", () => {
    ...
    expect(
      collectPublishablePluginPackageErrors({
        ...
        packageJson: {
          ...
          repository: {
            type: "git",
            url: DAISYCLAW_PLUGIN_NPM_REPOSITORY_URL,
          },
          daisyclaw: {
            extensions: ["./index.ts"],
            ...externalPluginContract("2026.3.15"),
            install: {
              npmSpec: "@daisyclaw/zalo",
            },
            release: {
              publishToNpm: true,
            },
  ...
  it("flags invalid publishable plugin metadata", () => {
    expect(
      ...
      'package name must start with "@daisyclaw/"; found "broken".',
      "package.json private must not be true.",
      `package.json repository.url must be "${DAISYCLAW_PLUGIN_NPM_REPOSITORY_URL}" so npm provenance can validate GitHub trusted publishing; found "<missing>".`,
      'package.json version must match YYYY.M.PATCH, YYYY.M.PATCH-N, YYYY.M.PATCH-alpha.N, or YYYY.M.PATCH-beta.N; found "latest".',
      "daisyclaw.extensions must contain only non-empty strings.",
      "daisyclaw.install.npmSpec must be a non-empty string for publishable plugins.",
    ]);
  });
  ...
  it("requires npm install metadata for publishable plugins", () => {
    ...
    ).toEqual(["daisyclaw.install.npmSpec must be a non-empty string for publishable plugins."]);
  });
  ...
  it("requires the external plugin package compatibility contract for npm publish", () => {
    expect(
      ...
      "daisyclaw.compat.pluginApi is required for external code plugin packages.",
      "daisyclaw.build.daisyclawVersion is required for external code plugin packages.",
    ]);
  });
});
...
describe("collectPluginReleaseVersionFloorErrors", () => {
  it("blocks selected plugin stable and beta releases below the June 2026 floor", () => {
    expect(
      ...
      '@daisyclaw/demo@2026.6.4-beta.1: June 2026 stable and beta release trains must use patch 5 or higher because 2026.6.5-beta.1 is already published; found "2026.6.4-beta.1".',
    ]);
  });
...
describe("collectPublishablePluginPackages", () => {
  ...
  it("collects publishable npm plugins from extension package manifests", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "daisyclaw-plugin-npm-release-");
    ...
    expect(collectPublishablePluginPackages(repoDir)).toEqual([
      {
        ...
        packageName: "@daisyclaw/demo-plugin",
        version: "2026.4.10",
        channel: "stable",
        publishTag: "latest",
        installNpmSpec: "@daisyclaw/demo-plugin",
      },
    ]);
  });
  ...
  it("does not validate unselected publishable plugin manifests", () => {
    ...
    writeJsonFile(join(repoDir, "extensions", "private-plugin", "package.json"), {
      ...
      daisyclaw: {
        ...
        ...externalPluginContract("2026.4.10-beta.1"),
        install: {
          npmSpec: "@daisyclaw/private-plugin",
        },
        release: {
          publishToNpm: true,
        },
    ...
    expect(
      collectPublishablePluginPackages(repoDir, {
        packageNames: ["@daisyclaw/demo-plugin"],
      }),
    ).toEqual([
      {
        extensionId: "demo-plugin",
...
describe("resolveSelectedPublishablePluginPackages", () => {
  ...
  it("throws when the selection contains an unknown package name", () => {
    expect(() =>
      resolveSelectedPublishablePluginPackages({
        plugins: publishablePlugins,
        selection: ["@daisyclaw/missing"],
      }),
    ).toThrowError("Unknown or non-publishable plugin package selection: @daisyclaw/missing.");
  });
});
