...
import { bundledDistPluginFile, bundledPluginFile } from "daisyclaw/plugin-sdk/test-fixtures";
...
import { collectInstalledRootDependencyManifestErrors } from "../scripts/daisyclaw-npm-postpublish-verify.ts";
import {
...
describe("packed CLI smoke", () => {
  ...
  it("builds a packed CLI smoke env with packaged-install guardrails", () => {
    expect(
      createPackedCliSmokeEnv(
        {
          ...
          DAISYCLAW_CONFIG_PATH: "/tmp/leaky-config.json",
        },
        { HOME: "/tmp/smoke-home", DAISYCLAW_STATE_DIR: "/tmp/smoke-state" },
      ),
    ).toEqual({
      PATH:
      ...
      TMPDIR: "/tmp/original-tmp",
      SystemRoot: "C:\\Windows",
      DAISYCLAW_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK: "1",
      DAISYCLAW_NO_ONBOARD: "1",
      DAISYCLAW_SERVICE_REPAIR_POLICY: "external",
      DAISYCLAW_SUPPRESS_NOTES: "1",
      DAISYCLAW_STATE_DIR: "/tmp/smoke-state",
    });
  });
  ...
  it("skips plugin command discovery during packed completion cache smoke", () => {
    expect(
      createPackedCompletionSmokeEnv(
        {
          PATH: "/usr/bin",
          DAISYCLAW_COMPLETION_SKIP_PLUGIN_COMMANDS: "0",
        },
        {
          HOME: "/tmp/smoke-home",
          DAISYCLAW_STATE_DIR: "/tmp/smoke-state",
...
describe("runReleaseCheckCommand", () => {
  ...
  it("rejects malformed command limit environment values", () => {
    withProcessEnv({ DAISYCLAW_RELEASE_CHECK_COMMAND_TIMEOUT_MS: "1e3" }, () => {
      expect(() =>
        runReleaseCheckCommand(
          { command: process.execPath, args: ["--eval", "process.stdout.write('ok')"] },
          { stdio: ["ignore", "pipe", "pipe"] },
        ),
      ).toThrow("invalid DAISYCLAW_RELEASE_CHECK_COMMAND_TIMEOUT_MS: 1e3");
    });

    withProcessEnv({ DAISYCLAW_RELEASE_CHECK_COMMAND_MAX_BUFFER_BYTES: "16mb" }, () => {
      expect(() =>
        runReleaseCheckCommand(
          { command: process.execPath, args: ["--eval", "process.stdout.write('ok')"] },
          { stdio: ["ignore", "pipe", "pipe"] },
        ),
      ).toThrow("invalid DAISYCLAW_RELEASE_CHECK_COMMAND_MAX_BUFFER_BYTES: 16mb");
    });
  });
});
...
describe("resolveReleaseNpmCommand", () => {
  ...
  it("rejects bare npm fallback on Windows release checks", () => {
    ...
    ).toThrow("DaisyClaw refuses to shell out to bare npm on Windows");
  });
});
...
describe("workspace bootstrap smoke", () => {
  it("runs with a sterile env instead of maintainer provider credentials", () => {
    expect(
      ...
      DAISYCLAW_HOME: "/tmp/bootstrap-home",
      TMPDIR: "/tmp/original-tmp",
      DAISYCLAW_NO_ONBOARD: "1",
      DAISYCLAW_SUPPRESS_NOTES: "1",
...
describe("collectBundledExtensionManifestErrors", () => {
  it("flags invalid bundled extension install metadata", () => {
    expect(
      ...
      "bundled extension 'broken' manifest invalid | daisyclaw.install.npmSpec must be a non-empty string",
    ]);
  });
      ...
      collectBundledExtensionManifestErrors([
        {
          ...
          packageJson: {
            daisyclaw: {
              install: { npmSpec: "@daisyclaw/broken", minHostVersion: "2026.3.14" },
            },
          },
        },
      ]),
    ).toEqual([
      "bundled extension 'broken' manifest invalid | daisyclaw.install.minHostVersion must use a semver floor in the form \">=x.y.z[-prerelease][+build]\"",
    ]);
  });
  ...
  it("flags non-object install metadata instead of throwing", () => {
    ...
    ).toEqual(["bundled extension 'broken' manifest invalid | daisyclaw.install must be an object"]);
  });
});
...
describe("bundled plugin package dependency checks", () => {
  ...
  it("does not require root deps for root chunks sourced from the owning installed plugin", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "daisyclaw-root-owned-installed-"));

    try {
      mkdirSync(join(tempRoot, "dist", "extensions", "memory-lancedb"), { recursive: true });
      writeFileSync(
        join(tempRoot, "package.json"),
        `{"name":"daisyclaw","dependencies":{}}\n`,
        "utf8",
      );
      writeFileSync(
        join(tempRoot, "dist", "extensions", "memory-lancedb", "package.json"),
        `{"name":"@daisyclaw/memory-lancedb","dependencies":{"root-owned-test-dep":"^1.0.0"}}\n`,
        "utf8",
      );
      writeFileSync(
  ...
  it("still requires root deps for root-owned installed chunks", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "daisyclaw-root-owned-installed-missing-"));
...
describe.skipIf(process.platform === "win32")("collectSkillShellScriptExecutableErrors", () => {
  it("flags non-executable shell scripts under skills/*/scripts", () => {
    const root = mkdtempSync(join(tmpdir(), "daisyclaw-release-check-"));
...
describe("collectForbiddenPackPaths", () => {
  ...
  it("blocks legacy runtime dependency stamps from npm pack output", () => {
    expect(
      collectForbiddenPackPaths([
        "dist/index.js",
        "dist/extensions/browser/.DaisyClaw-Install-Stage/package.json",
        "dist/extensions/codex/.daisyclaw-runtime-deps-backup-node_modules-old/zod/index.js",
        "dist/extensions/discord/.daisyclaw-runtime-deps-stamp.json",
      ]),
    ).toEqual([
      "dist/extensions/browser/.DaisyClaw-Install-Stage/package.json",
      "dist/extensions/codex/.daisyclaw-runtime-deps-backup-node_modules-old/zod/index.js",
  ...
  it("blocks root dist chunks that still reference private qa lab sources", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "daisyclaw-release-private-qa-"));
  ...
  it("blocks private QA paths in the generated dist inventory", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "daisyclaw-release-inventory-"));
  ...
  it("blocks root plugin SDK declarations that still reference private test helpers", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "daisyclaw-release-private-sdk-"));
...
describe("collectMissingPackPaths", () => {
  it("requires the shipped channel catalog, control ui, and optional bundled metadata", () => {
    ...
    for (const path of [
      ...
      bundledDistPluginFile("telegram", "daisyclaw.plugin.json"),
  ...
  it("runs postpublish package integrity checks against the packed install before publish", () => {
    ...
    try {
      const packageRoot = join(root, "daisyclaw");
      const distDir = join(packageRoot, "dist");
      mkdirSync(distDir, { recursive: true });
      writeFileSync(
        join(packageRoot, "package.json"),
        `${JSON.stringify({ name: "daisyclaw", version: "2026.5.14-beta.3", dependencies: {} })}\n`,
      );
      ...
      expect(
        collectPackedInstalledPackageVerificationErrors({
          ...
          installedBinaryVersion: "daisyclaw 2026.5.14-beta.3",
          packageRoot,
        }),
      ).toEqual([
...
describe("createPackedPluginSdkTypescriptSmokeProject", () => {
  it("writes a consumer project that imports representative public SDK subpaths", () => {
    ...
    try {
      ...
      expect(packageJson.dependencies?.daisyclaw).toBe(`file:${packageRoot}`);
      expect(tsconfig.compilerOptions?.skipLibCheck).toBe(true);
      expect(source).toBe(fixtureSource);
      expect(source).toContain('"daisyclaw/plugin-sdk"');
      expect(source).toContain('"daisyclaw/plugin-sdk/provider-entry"');
      expect(source).toContain('"daisyclaw/plugin-sdk/channel-entry-contract"');
      expect(source).toContain('"daisyclaw/plugin-sdk/config-contracts"');
      expect(source).toContain('"daisyclaw/plugin-sdk/runtime-env"');
      expect(source).toContain("type PublicPluginSdkModules = [");
      expect(source).not.toContain("TelegramAccountConfig");
      expect(source).not.toContain("daisyclaw/plugin-sdk/channel-contract-testing");
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
...
describe("collectPackUnpackedSizeErrors", () => {
  it("accepts pack results within the unpacked size budget", () => {
    expect(
      collectPackUnpackedSizeErrors([makePackResult("daisyclaw-2026.3.14.tgz", 120_354_302)]),
    ).toStrictEqual([]);
  });

  it("flags oversized pack results that risk low-memory startup failures", () => {
    expect(
      collectPackUnpackedSizeErrors([makePackResult("daisyclaw-2026.3.12.tgz", 224_002_564)]),
    ).toEqual([
      "daisyclaw-2026.3.12.tgz unpackedSize 224002564 bytes (213.6 MiB) exceeds budget 211812352 bytes (202.0 MiB). Investigate duplicate channel shims, copied extension trees, or other accidental pack bloat before release.",
    ]);
  });
  ...
  it("fails closed when npm pack output omits unpackedSize for every result", () => {
    expect(
      collectPackUnpackedSizeErrors([
        { filename: "daisyclaw-2026.3.14.tgz" },
        { filename: "daisyclaw-extra.tgz", unpackedSize: Number.NaN },
      ]),
    ).toEqual([
