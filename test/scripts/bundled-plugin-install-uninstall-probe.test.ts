...
function makePackageRoot(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-bundled-probe-"));
  tempDirs.push(root);
...
function writePluginManifest(root: string, pluginRoot: string, manifest: Record<string, unknown>) {
  ...
  fs.writeFileSync(
    path.join(dir, "daisyclaw.plugin.json"),
...
function runProbe(root: string, env: Record<string, string | undefined> = {}) {
  ...
  childEnv.DAISYCLAW_ENTRY = path.join(root, "dist", "index.js");
...
function runRuntimeSmoke(root: string, args: string[]) {
  return spawnSync(process.execPath, [runtimeSmokePath, ...args], {
    ...
    env: {
      ...process.env,
      DAISYCLAW_ENTRY: path.join(root, "dist", "index.js"),
    },
  });
}
...
describe("bundled plugin install/uninstall probe", () => {
  ...
  it("bounds bundled plugin package lifecycle commands", () => {
    ...
    expect(sweep).toContain("DAISYCLAW_BUNDLED_PLUGIN_SWEEP_COMMAND_TIMEOUT:-300s");
    expect(sweep.match(/daisyclaw_e2e_maybe_timeout/g)).toHaveLength(1);
  ...
  it("caps noisy runtime gateway logs", async () => {
    const runtimeSmoke = await importRuntimeSmokeWithEnv({
      DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_GATEWAY_LOG_BYTES: "64",
    });
  ...
  it("rejects loose runtime output limit env values instead of parsing prefixes", async () => {
    await expect(
      importRuntimeSmokeWithEnv({
        DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_OUTPUT_CHARS: "5chars",
      }),
    ).rejects.toThrow("invalid DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_OUTPUT_CHARS: 5chars");
  });
  ...
  it("rejects loose runtime log scan byte env values instead of parsing prefixes", async () => {
    await expect(
      importRuntimeSmokeWithEnv({
        DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_LOG_SCAN_BYTES: "64bytes",
      }),
    ).rejects.toThrow("invalid DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_LOG_SCAN_BYTES: 64bytes");
  });
  ...
  it.runIf(process.platform !== "win32")("stops runtime gateway process groups", async () => {
    const runtimeSmoke = await importRuntimeSmokeWithEnv({
      DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_TEARDOWN_GRACE_MS: "50",
      DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_TEARDOWN_KILL_GRACE_MS: "1000",
    });
    const root = makePackageRoot();
  ...
  it("cleans per-call RPC state directories", async () => {
    ...
    fs.writeFileSync(
      ...
      [
        "import fs from 'node:fs';",
        "fs.writeFileSync(process.env.DAISYCLAW_TEST_RPC_STATE_PATH, process.env.DAISYCLAW_STATE_DIR);",
    ...
    await expect(
      runtimeSmoke.rpcCall(
        ...
        {
          entrypoint,
          env: { DAISYCLAW_TEST_RPC_STATE_PATH: statePath },
          port: 19001,
        },
      ),
    ...
    expect(path.basename(rpcStateDir)).toMatch(/^daisyclaw-plugin-runtime-rpc-/u);
  ...
  it("allows degraded runtime readiness only for expected channel failures", async () => {
    const runtimeSmoke = await importRuntimeSmokeWithEnv({
      DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_HTTP_MS: "100",
      DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_RPC_READY_MS: "50",
    });
    const server = createHttpServer((_request, response) => {
  ...
  it("keeps stalled runtime readiness probes inside the ready deadline", async () => {
    const runtimeSmoke = await importRuntimeSmokeWithEnv({
      DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_HTTP_MS: "1000",
      DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_READY_MS: "50",
    });
    const sockets = new Set<Socket>();
    const server = createNetServer((socket) => {
      sockets.add(socket);
  ...
  it("creates runtime smoke state with DAISYCLAW_HOME at the test home", async () => {
    const runtimeSmoke = await import(pathToFileURL(runtimeSmokePath).href);
  ...
  it("selects packaged installable bundled sources instead of raw dist extension dirs", () => {
    ...
    fs.writeFileSync(
      path.join(root, "dist", "extensions", "qa-channel", "daisyclaw.plugin.json"),
      '{"id":"qa-channel"}\n',
      "utf8",
    );
    ...
    const result = runProbe(root, {
      DAISYCLAW_BUNDLED_PLUGIN_SWEEP_IDS: undefined,
    });
  ...
  it("does not select source-only bundled plugins for package-backed sweeps", () => {
    ...
    const result = runProbe(root, {
      DAISYCLAW_BUNDLED_PLUGIN_SWEEP_IDS: "qa-channel",
    });

    expect(result.status).toBe(1);
    expect(result.stderr).toContain(
      "DAISYCLAW_BUNDLED_PLUGIN_SWEEP_IDS entry is not an installable bundled plugin in this package: qa-channel",
    );
  ...
  it("rejects loose packaged plugin list limit env values", () => {
    ...
    const timeout = runProbe(root, {
      DAISYCLAW_BUNDLED_PLUGIN_LIST_TIMEOUT_MS: "100ms",
    });
    expect(timeout.status).toBe(1);
    expect(timeout.stderr).toContain("invalid DAISYCLAW_BUNDLED_PLUGIN_LIST_TIMEOUT_MS: 100ms");

    const maxBuffer = runProbe(root, {
      DAISYCLAW_BUNDLED_PLUGIN_LIST_MAX_BUFFER_BYTES: "64bytes",
    });
    expect(maxBuffer.status).toBe(1);
    expect(maxBuffer.stderr).toContain(
      "invalid DAISYCLAW_BUNDLED_PLUGIN_LIST_MAX_BUFFER_BYTES: 64bytes",
    );
  });
  ...
  it("rejects loose bundled plugin sweep shard env values", () => {
    ...
    const total = runProbe(root, {
      DAISYCLAW_BUNDLED_PLUGIN_SWEEP_TOTAL: "2shards",
    });
    expect(total.status).toBe(1);
    expect(total.stderr).toContain("invalid DAISYCLAW_BUNDLED_PLUGIN_SWEEP_TOTAL: 2shards");

    const index = runProbe(root, {
      DAISYCLAW_BUNDLED_PLUGIN_SWEEP_INDEX: "0of2",
    });
    expect(index.status).toBe(1);
    expect(index.stderr).toContain("invalid DAISYCLAW_BUNDLED_PLUGIN_SWEEP_INDEX: 0of2");
  });
  ...
  it("bounds plugin list selection when the CLI hangs", () => {
    ...
    const startedAt = Date.now();
    const result = runProbe(root, {
      DAISYCLAW_BUNDLED_PLUGIN_LIST_TIMEOUT_MS: "100",
    });
  ...
  it("loads runtime smoke manifests from the selected packaged root", () => {
    ...
    fs.writeFileSync(
      path.join(root, "dist-runtime", "extensions", "runtime-only", "daisyclaw.plugin.json"),
      '{"id":"runtime-only"}\n',
      "utf8",
    );
  ...
  it("accepts native Windows bundled source paths when asserting install state", () => {
    ...
    fs.writeFileSync(
      path.join(stateDir, "daisyclaw.json"),
