...
describe("scripts/changed-lanes", () => {
  ...
  it("prints changed check help without running the changed gate", () => {
    const result = spawnSync(process.execPath, ["scripts/check-changed.mjs", "--help"], {
      cwd: repoRoot,
      encoding: "utf8",
      env: { ...createNestedGitEnv(), DAISYCLAW_TESTBOX: "1" },
    });
  ...
  it("includes untracked worktree files in the default local diff", () => {
    const dir = makeTempRepoRoot(tempDirs, "daisyclaw-changed-lanes-");
    git(dir, ["init", "-q", "--initial-branch=main"]);
  ...
  it("falls back to a two-dot diff when a delegated checkout has no merge base", () => {
    const dir = makeTempRepoRoot(tempDirs, "daisyclaw-changed-lanes-no-merge-base-");
    git(dir, ["init", "-q", "--initial-branch=main"]);
    writeFileSync(path.join(dir, "README.md"), "initial\n", "utf8");
    git(dir, ["add", "README.md"]);
    git(dir, [
  ...
  it("prefers raw sync worktree paths over an implausibly broad no-merge-base diff", () => {
    const dir = makeTempRepoRoot(tempDirs, "daisyclaw-changed-lanes-raw-sync-");
    git(dir, ["init", "-q", "--initial-branch=main"]);
    ...
    const previousRawSync = process.env.DAISYCLAW_CHANGED_LANES_RAW_SYNC;
    delete process.env.DAISYCLAW_CHANGED_LANES_RAW_SYNC;
    try {
      ...
      process.env.DAISYCLAW_CHANGED_LANES_RAW_SYNC = "1";
      expect(listChangedPathsFromGit({ base: "origin/main", cwd: dir })).toEqual([
        "src/feature.ts",
      ]);
    } finally {
      if (previousRawSync === undefined) {
        ...
        process.env.DAISYCLAW_CHANGED_LANES_RAW_SYNC = previousRawSync;
      }
    }
  });

  it("uses the merge commit first parent instead of a stale PR payload base", () => {
    const { dir, staleBase } = createSyntheticMergeRepo("daisyclaw-changed-lanes-merge-");
  ...
  it("ignores local Crabbox metadata in the default local diff", () => {
    const dir = makeTempRepoRoot(tempDirs, "daisyclaw-changed-lanes-crabbox-");
    git(dir, ["init", "-q", "--initial-branch=main"]);
  ...
  it("includes deleted worktree files in the default local diff", () => {
    const dir = makeTempRepoRoot(tempDirs, "daisyclaw-changed-lanes-deleted-");
    git(dir, ["init", "-q", "--initial-branch=main"]);
  ...
  it("includes deleted staged files in the staged diff", () => {
    const dir = makeTempRepoRoot(tempDirs, "daisyclaw-changed-lanes-staged-deleted-");
    git(dir, ["init", "-q", "--initial-branch=main"]);
    mkdirSync(path.join(dir, "src", "shared"), { recursive: true });
    writeFileSync(
      path.join(dir, "src", "shared", "obsolete.ts"),
  ...
  it("routes core production changes to core prod and core test lanes", () => {
    ...
    expect(plan.commands.find((command) => command.args[0] === "tsgo:core")?.env).toEqual({
      PATH: "/usr/bin",
      DAISYCLAW_OXLINT_SKIP_LOCK: "1",
      DAISYCLAW_TEST_HEAVY_CHECK_LOCK_HELD: "1",
      DAISYCLAW_TSGO_HEAVY_CHECK_LOCK_HELD: "1",
      DAISYCLAW_TSGO_SPARSE_SKIP: "1",
    });
  ...
  it("reenables local-check policy for changed typecheck commands", () => {
    ...
    const plan = createChangedCheckPlan(result, {
      env: { DAISYCLAW_LOCAL_CHECK: "0", PATH: "/usr/bin" },
    });

    expect(plan.commands.find((command) => command.args[0] === "tsgo:core")?.env).toEqual({
      DAISYCLAW_LOCAL_CHECK: "1",
      DAISYCLAW_OXLINT_SKIP_LOCK: "1",
      DAISYCLAW_TEST_HEAVY_CHECK_LOCK_HELD: "1",
      DAISYCLAW_TSGO_HEAVY_CHECK_LOCK_HELD: "1",
      DAISYCLAW_TSGO_SPARSE_SKIP: "1",
  ...
  it("cleans CI Corepack pnpm shim temp dirs", () => {
    ...
    expect(path.basename(shimDir)).toMatch(/^daisyclaw-corepack-pnpm-/u);
  ...
  it("delegates staged changed gates as explicit remote paths", () => {
    const dir = makeTempRepoRoot(tempDirs, "daisyclaw-check-changed-staged-delegate-");
    git(dir, ["init", "-q", "--initial-branch=main"]);
    writeFileSync(path.join(dir, "README.md"), "initial\n", "utf8");
    git(dir, ["add", "README.md"]);
    git(dir, [
  ...
  it("delegates empty staged changed gates without rediscovering unstaged paths", () => {
    const dir = makeTempRepoRoot(tempDirs, "daisyclaw-check-changed-empty-staged-delegate-");
    git(dir, ["init", "-q", "--initial-branch=main"]);
    writeFileSync(path.join(dir, "README.md"), "initial\n", "utf8");
    git(dir, ["add", "README.md"]);
    git(dir, [
  ...
  it("does not delegate dry-run, CI, or remote-child changed gates", () => {
    ...
    expect(
      shouldDelegateChangedCheckToCrabbox([], { DAISYCLAW_CHECK_CHANGED_REMOTE_CHILD: "1" }),
    ).toBe(false);
  });
  ...
  it("routes live Docker ACP tooling changes through a focused gate", () => {
    ...
    expect(schedulerDryRun?.env?.DAISYCLAW_DOCKER_ALL_DRY_RUN).toBe("1");
    expect(schedulerDryRun?.env?.DAISYCLAW_DOCKER_ALL_LIVE_MODE).toBe("only");
  });
    ...
    const after = `${JSON.stringify(
      {
        ...
        scripts: {
          ...
          "test:docker:live-acp-bind:droid":
            "DAISYCLAW_LIVE_ACP_BIND_AGENT=droid bash scripts/test-live-acp-bind-docker.sh",
        },
        dependencies: {
          leftpad: "1.0.0",
        },
  ...
  it("classifies live Docker package script changes from the git diff", () => {
    const dir = makeTempRepoRoot(tempDirs, "daisyclaw-live-docker-package-");
    git(dir, ["init", "-q", "--initial-branch=main"]);
    writeFileSync(
  ...
  it("classifies normal package script changes from the git diff", () => {
    const dir = makeTempRepoRoot(tempDirs, "daisyclaw-package-scripts-");
    git(dir, ["init", "-q", "--initial-branch=main"]);
    writeFileSync(
      path.join(dir, "package.json"),
      `${JSON.stringify(
  ...
  it("keeps release metadata commits off the full changed gate", () => {
    const result = detectChangedLanes([
      ...
      "apps/macos/Sources/DaisyClaw/Resources/Info.plist",
  ...
  it("guards release metadata package changes to the top-level version field", () => {
    const dir = makeTempRepoRoot(tempDirs, "daisyclaw-release-metadata-");
    git(dir, ["init", "-q", "--initial-branch=main"]);
    writeFileSync(
      path.join(dir, "package.json"),
  ...
  it("keeps app lint explicit when non-macOS hosts lack SwiftLint", () => {
    const result = detectChangedLanes([
      "apps/shared/DaisyClawKit/Sources/DaisyClawProtocol/GatewayModels.swift",
    ]);
    const plan = createChangedCheckPlan(result, {
      env: { PATH: "/usr/bin" },
      platform: "linux",
