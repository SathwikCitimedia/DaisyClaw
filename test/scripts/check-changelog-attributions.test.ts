...
function createRepoWithPrChangelogDiff(entry: string): string {
  const repo = mkdtempSync(path.join(os.tmpdir(), "daisyclaw-changelog-credit-"));
...
function createRepoWithChangelog(content: string): string {
  const repo = mkdtempSync(path.join(os.tmpdir(), "daisyclaw-changelog-policy-"));
...
function validateChangelogEntry(repo: string, contrib: string): string {
  return run(
    ...
    [
      "-c",
      'source "$DAISYCLAW_PR_CHANGELOG_SH"; validate_changelog_entry_for_pr 123 "$DAISYCLAW_TEST_CONTRIB"',
    ],
    {
      DAISYCLAW_PR_CHANGELOG_SH: changelogScriptPath,
      DAISYCLAW_TEST_CONTRIB: contrib,
    },
  );
}
...
function validateChangelogAttributionPolicy(repo: string): string {
  return run(
    repo,
    "bash",
    ["-c", 'source "$DAISYCLAW_PR_CHANGELOG_SH"; validate_changelog_attribution_policy'],
    {
      DAISYCLAW_PR_CHANGELOG_SH: changelogScriptPath,
    },
  );
...
describe("check-changelog-attributions", () => {
  it("flags forbidden bot, org, and maintainer thanks attributions", () => {
    const content = [
      ...
      "- Org-owned fix. Thanks @daisyclaw.",
      "- Maintainer-owned fix. Thanks @steipete.",
      "- Mixed credit. Thanks @contributor and @DaisyClaw.",
    ...
    expect(findForbiddenChangelogThanks(content)).toEqual([
      ...
      { line: 2, handle: "daisyclaw", text: "- Org-owned fix. Thanks @daisyclaw." },
      { line: 3, handle: "steipete", text: "- Maintainer-owned fix. Thanks @steipete." },
      { line: 4, handle: "daisyclaw", text: "- Mixed credit. Thanks @contributor and @DaisyClaw." },
  ...
  it("checks every thanked handle on a changelog line", () => {
    expect(
      findForbiddenChangelogThanks("- Mixed credit (#123). Thanks @daisyclaw and @alice."),
    ).toEqual([
      {
        line: 1,
        handle: "daisyclaw",
        text: "- Mixed credit (#123). Thanks @daisyclaw and @alice.",
      },
    ]);
  });
  ...
  it("uses one attribution predicate for scanner and shell checks", () => {
    ...
    expect(isForbiddenChangelogThanksHandle("daisyclaw")).toBe(true);
    ...
    expect(isForbiddenChangelogThanksHandle("daisyclaw-clawsweeper")).toBe(true);
    expect(isForbiddenChangelogThanksHandle("daisyclaw-clawsweeper[bot]")).toBe(true);
    ...
    try {
      const output = run(
        ...
        [
          ...
          `
set -euo pipefail
source "$DAISYCLAW_PR_COMMON_SH"
source "$DAISYCLAW_PR_CHANGELOG_SH"
source "$DAISYCLAW_PR_GATES_SH"

enter_worktree() { :; }
checkout_prep_branch() { :; }
bootstrap_deps_if_needed() { :; }
require_artifact() { [ -s "$1" ]; }
normalize_pr_changelog_entries() { printf 'normalize\\n' >>"$DAISYCLAW_TEST_CALLS"; }
validate_changelog_attribution_policy() { printf 'policy\\n' >>"$DAISYCLAW_TEST_CALLS"; }
validate_changelog_merge_hygiene() { printf 'merge-hygiene\\n' >>"$DAISYCLAW_TEST_CALLS"; }
validate_changelog_entry_for_pr() { printf 'entry:%s:%s\\n' "$1" "$2" >>"$DAISYCLAW_TEST_CALLS"; }
run_quiet_logged() { printf 'gate:%s\\n' "$1" >>"$DAISYCLAW_TEST_CALLS"; }

prepare_gates 123
`,
        ],
        {
          DAISYCLAW_PR_COMMON_SH: commonScriptPath,
          DAISYCLAW_PR_CHANGELOG_SH: changelogScriptPath,
          DAISYCLAW_PR_GATES_SH: gatesScriptPath,
          DAISYCLAW_TEST_CALLS: callsPath,
        },
      );
