...
function makeTempRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), "daisyclaw-startup-memory-test-"));
  tempRoots.push(root);
  return root;
}
...
describe("check-cli-startup-memory", () => {
  ...
  it("keeps invalid startup memory env values from bypassing budgets", () => {
    expect(() =>
      testing.readPositiveNumberEnv("DAISYCLAW_STARTUP_MEMORY_HELP_MB", 100, {
        DAISYCLAW_STARTUP_MEMORY_HELP_MB: "abc",
      }),
    ).toThrow("DAISYCLAW_STARTUP_MEMORY_HELP_MB must be a positive number");
    expect(() =>
      testing.readPositiveNumberEnv("DAISYCLAW_STARTUP_MEMORY_HELP_MB", 100, {
        DAISYCLAW_STARTUP_MEMORY_HELP_MB: "1e3",
      }),
    ).toThrow("DAISYCLAW_STARTUP_MEMORY_HELP_MB must be a positive number");
    expect(() =>
      testing.readPositiveNumberEnv("DAISYCLAW_STARTUP_MEMORY_HELP_MB", 100, {
        DAISYCLAW_STARTUP_MEMORY_HELP_MB: "0x10",
      }),
    ).toThrow("DAISYCLAW_STARTUP_MEMORY_HELP_MB must be a positive number");
    expect(() =>
      testing.readPositiveNumberEnv("DAISYCLAW_STARTUP_MEMORY_HELP_MB", 100, {
        DAISYCLAW_STARTUP_MEMORY_HELP_MB: "0",
      }),
    ).toThrow("DAISYCLAW_STARTUP_MEMORY_HELP_MB must be a positive number");
    expect(
      testing.readPositiveNumberEnv("DAISYCLAW_STARTUP_MEMORY_HELP_MB", 100, {
        DAISYCLAW_STARTUP_MEMORY_HELP_MB: "125.5",
      }),
    ).toBe(125.5);
  });
  ...
  it("keeps invalid startup memory timeout env values from parsing loosely", () => {
    expect(() =>
      testing.readPositiveIntEnv("DAISYCLAW_STARTUP_MEMORY_TIMEOUT_MS", 60_000, {
        DAISYCLAW_STARTUP_MEMORY_TIMEOUT_MS: "1e3",
      }),
    ).toThrow("DAISYCLAW_STARTUP_MEMORY_TIMEOUT_MS must be a positive number");
    expect(() =>
      testing.readPositiveIntEnv("DAISYCLAW_STARTUP_MEMORY_TIMEOUT_MS", 60_000, {
        DAISYCLAW_STARTUP_MEMORY_TIMEOUT_MS: "1000.5",
      }),
    ).toThrow("DAISYCLAW_STARTUP_MEMORY_TIMEOUT_MS must be a positive integer");
    expect(
      testing.readPositiveIntEnv("DAISYCLAW_STARTUP_MEMORY_TIMEOUT_MS", 60_000, {
        DAISYCLAW_STARTUP_MEMORY_TIMEOUT_MS: "1000",
      }),
    ).toBe(1000);
  });
      ...
      testing.runStartupMemoryCheck(
        ...
        ],
        {
          ...
          spawnSync: () => ({
            signal: null,
            status: 0,
            stderr: "__DAISYCLAW_MAX_RSS_KB__=0\n",
            stdout: "",
          }),
        },
      ),
