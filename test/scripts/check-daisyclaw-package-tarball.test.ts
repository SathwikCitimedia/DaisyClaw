// Check Daisyclaw Package Tarball tests cover check daisyclaw package tarball script behavior.
...
const CHECK_SCRIPT = "scripts/check-daisyclaw-package-tarball.mjs";
...
function withTarball(
  ...
  const root = mkdtempSync(join(tmpdir(), "daisyclaw-package-tarball-test-"));
  try {
    const packageRoot = join(root, "package");
    mkdirSync(join(packageRoot, "dist"), { recursive: true });
    writeFileSync(join(packageRoot, "package.json"), JSON.stringify({ name: "daisyclaw", version }));
    ...
    const tarFiles =
      options.includeControlUi === false
        ? files
        : {
            "dist/control-ui/index.html": "<!doctype html><daisyclaw-app></daisyclaw-app>",
    ...
    const tarball = join(root, "daisyclaw.tgz");
...
describe("check-daisyclaw-package-tarball", () => {
  it.runIf(process.platform !== "win32")(
    "removes the extract dir when tar extraction fails",
    () => {
      const root = mkdtempSync(join(tmpdir(), "daisyclaw-package-tarball-extract-fail-"));
      try {
        ...
        writeFileSync(
          ...
          [
            ...
            "fs.writeFileSync(process.env.DAISYCLAW_TEST_EXTRACT_DIR_FILE, outputDir);",
        ...
        const result = spawnSync("node", [CHECK_SCRIPT, tarball], {
          ...
          env: {
            ...process.env,
            DAISYCLAW_TEST_EXTRACT_DIR_FILE: extractDirFile,
  ...
  it("allows legacy private QA inventory entries omitted from shipped tarballs through 2026.4.25", () => {
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.stdout).toContain("DaisyClaw package tarball integrity passed.");
      },
      "2026.4.25-beta.10",
    );
  });
