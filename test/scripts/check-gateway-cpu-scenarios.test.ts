...
describe("gateway CPU scenario guard", () => {
  ...
  it("prebuilds private QA dist before running QA scenarios when it is missing", async () => {
    ...
    const result = await testing.runGatewayCpuScenarios(options, {
      ...
      spawnSync: (_command: string, args: string[], opts?: { env?: Record<string, string> }) => {
        ...
        if (args.includes("daisyclaw") && args.includes("qa")) {
    ...
    expect(calls[0]?.env).toMatchObject({
      ...
      DAISYCLAW_CONFIG_PATH: path.join(outputDir, "qa-state-root", "state", "daisyclaw.json"),
      DAISYCLAW_ENABLE_PRIVATE_QA_CLI: "1",
      DAISYCLAW_HOME: path.join(outputDir, "qa-state-root", "home"),
      DAISYCLAW_RUN_NODE_SKIP_DTS_BUILD: "1",
      DAISYCLAW_STATE_DIR: path.join(outputDir, "qa-state-root", "state"),
      DAISYCLAW_TEST_DISABLE_UPDATE_CHECK: "1",
      USERPROFILE: path.join(outputDir, "qa-state-root", "home"),
    });
    expect(calls[0]?.env?.DAISYCLAW_BUNDLED_PLUGIN_BUILD_IDS).toBeUndefined();
  });
  ...
  it("does not prebuild private QA dist when the required entries already exist", async () => {
    ...
    const result = await testing.runGatewayCpuScenarios(options, {
      ...
      env: {
        HOME: "/real/user/home",
        DAISYCLAW_CONFIG_PATH: "/real/user/.daisyclaw/daisyclaw.json",
        DAISYCLAW_HOME: "/real/user/home",
        DAISYCLAW_STATE_DIR: "/real/user/.daisyclaw",
      },
      silent: true,
      spawnSync: (_command: string, args: string[], opts?: { env?: Record<string, string> }) => {
        calls.push({ args, env: opts?.env });
