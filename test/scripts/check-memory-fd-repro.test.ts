...
describe("check-memory-fd-repro", () => {
  ...
  it("rejects loose numeric environment limits before generating files", () => {
    expect(
      withEnv(
        {
          DAISYCLAW_MEMORY_FD_REPRO_FILES: "17",
          DAISYCLAW_MEMORY_FD_REPRO_MAX_WORKSPACE_REG_FDS: "0",
          DAISYCLAW_MEMORY_FD_REPRO_SAMPLE_DELAY_MS: "0",
        },
        () => parseArgs([]),
      ),
    ).toMatchObject({
    ...
    expect(() =>
      withEnv({ DAISYCLAW_MEMORY_FD_REPRO_FILES: "17files" }, () => parseArgs([])),
    ).toThrow("DAISYCLAW_MEMORY_FD_REPRO_FILES must be a non-negative integer");
    expect(() =>
      withEnv({ DAISYCLAW_MEMORY_FD_REPRO_TIMEOUT_MS: "1e3" }, () => parseArgs([])),
    ).toThrow("DAISYCLAW_MEMORY_FD_REPRO_TIMEOUT_MS must be a non-negative integer");
  });
  ...
  it("lets explicit CLI numeric flags override malformed inherited env defaults", () => {
    expect(
      withEnv(
        {
          DAISYCLAW_MEMORY_FD_REPRO_FILES: "17files",
          DAISYCLAW_MEMORY_FD_REPRO_MAX_WORKSPACE_REG_FDS: "4fds",
          DAISYCLAW_MEMORY_FD_REPRO_TIMEOUT_MS: "1e3",
          DAISYCLAW_MEMORY_FD_REPRO_SAMPLE_DELAY_MS: "soon",
          DAISYCLAW_MEMORY_FD_REPRO_SETTLE_DELAY_MS: "later",
        },
        () =>
          parseArgs([
            "--files",
  ...
  it("stops parsing options after the argument terminator", () => {
    ...
    expect(
      withEnv({ DAISYCLAW_MEMORY_FD_REPRO_FILES: "17" }, () => parseArgs(["--", "--unknown"])),
    ).toMatchObject({
      fileCount: 17,
    });
  });
  ...
  it("writes an offline FTS-only memory search config for repro indexing", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-memory-fd-config-"));
    try {
      ...
      expect(memorySearch).toMatchObject({
        ...
        model: "",
        store: {
          path: path.join(homeDir, ".daisyclaw", "memory", "main.sqlite"),
