  ...
  it("flags wildcard re-exports from plugin-sdk subpaths", () => {
    expect(
      findPluginSdkWildcardReexports(
        [
          'export * from "daisyclaw/plugin-sdk/foo";',
          'export type * from "daisyclaw/plugin-sdk/bar";',
          'export { named } from "daisyclaw/plugin-sdk/foo";',
        ].join("\n"),
      ),
    ).toEqual([
      { line: 1, text: 'export * from "daisyclaw/plugin-sdk/foo";' },
      { line: 2, text: 'export type * from "daisyclaw/plugin-sdk/bar";' },
    ]);
  });
  ...
  it("allows explicit SDK exports and local wildcard barrels", () => {
    expect(
      findPluginSdkWildcardReexports(
        [
          'export { named } from "daisyclaw/plugin-sdk/foo";',
          'export type { Named } from "daisyclaw/plugin-sdk/foo";',
