...
describe("ci workflow guards", () => {
  ...
  it("fails and retries quiet Node test shard stalls quickly", () => {
    ...
    expect(runStep.env.DAISYCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe("300000");
    expect(runStep.env.DAISYCLAW_VITEST_NO_OUTPUT_RETRY).toBe("1");
    expect(runStep.env.DAISYCLAW_TEST_PROJECTS_PARALLEL).toBe("2");
  });
  ...
  it("keeps push docs validation ClawHub-backed", () => {
    ...
    expect(workflow).toContain("repository: daisyclaw/clawhub");
    expect(workflow).toContain("path: clawhub-source");
    expect(workflow).toContain(
      "DAISYCLAW_DOCS_SYNC_CLAWHUB_REPO: ${{ github.workspace }}/clawhub-source",
    );
  });
