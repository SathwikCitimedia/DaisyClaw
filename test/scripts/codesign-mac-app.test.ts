...
function entitlementTemps(dir: string): string[] {
  return readdirSync(dir).filter((name) => name.startsWith("daisyclaw-entitlements"));
}
...
describe("codesign-mac-app temp file hygiene", () => {
  ...
  it("does not allocate entitlement temp files for help output", () => {
    const tempRoot = makeTempDir("daisyclaw-codesign-help-");
  ...
  it("does not allocate entitlement temp files before app validation", () => {
    const tempRoot = makeTempDir("daisyclaw-codesign-missing-");
  ...
  it("cleans entitlement temp files when signing fails", () => {
    const tempRoot = makeTempDir("daisyclaw-codesign-fail-");
  ...
  it("passes generated app entitlements to signing commands and cleans them", () => {
    const tempRoot = makeTempDir("daisyclaw-codesign-success-");
    const app = path.join(tempRoot, "Fake.app");
    ...
    writeFileSync(path.join(app, "Contents", "MacOS", "DaisyClaw"), "#!/bin/sh\n");
    ...
    expect(signLines[0]).toContain(`${path.join(app, "Contents", "MacOS", "DaisyClaw")}\t`);
    ...
    for (const line of signLines) {
      ...
      expect(entitlementPath).toContain("daisyclaw-entitlements");
