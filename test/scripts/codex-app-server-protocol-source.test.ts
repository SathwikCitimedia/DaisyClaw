...
const originalDaisyClawCodexRepo = process.env.DAISYCLAW_CODEX_REPO;

afterEach(() => {
  if (originalDaisyClawCodexRepo === undefined) {
    delete process.env.DAISYCLAW_CODEX_REPO;
  } else {
    process.env.DAISYCLAW_CODEX_REPO = originalDaisyClawCodexRepo;
  }
});
...
describe("codex app-server protocol source resolver", () => {
  ...
  it("allows an explicit local disk headroom override", () => {
    expect(resolveCodexProtocolMinFreeBytes({ DAISYCLAW_CODEX_PROTOCOL_MIN_FREE_BYTES: "0" })).toBe(
      0,
    );
    expect(() =>
      validateCodexProtocolGenerationHeadroom({
  ...
  it("rejects malformed local disk headroom overrides", () => {
    expect(() =>
      resolveCodexProtocolMinFreeBytes({ DAISYCLAW_CODEX_PROTOCOL_MIN_FREE_BYTES: "nope" }),
  ...
  it("uses DAISYCLAW_CODEX_REPO when provided", async () => {
    const root = createTempDir("daisyclaw-protocol-source-root-");
    const codexRepo = createTempDir("daisyclaw-protocol-source-codex-");
    createProtocolSchema(codexRepo);
    process.env.DAISYCLAW_CODEX_REPO = codexRepo;
  ...
  it("finds the primary checkout sibling from a git worktree", async () => {
    const parentDir = createTempDir("daisyclaw-protocol-source-parent-");
    const primaryDaisyClaw = path.join(parentDir, "daisyclaw");
    const codexRepo = path.join(parentDir, "codex");
    const worktreeRoot = createTempDir("daisyclaw-protocol-source-worktree-");
    fs.mkdirSync(path.join(primaryDaisyClaw, ".git", "worktrees", "codex-harness"), {
      recursive: true,
    });
    fs.mkdirSync(worktreeRoot, { recursive: true });
    fs.writeFileSync(
      path.join(worktreeRoot, ".git"),
      `gitdir: ${path.join(primaryDaisyClaw, ".git", "worktrees", "codex-harness")}\n`,
    );
    createProtocolSchema(codexRepo);
    delete process.env.DAISYCLAW_CODEX_REPO;
