...
function makeTempRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), "daisyclaw-codex-media-path-"));
  tempRoots.push(root);
  return root;
}
...
describe("codex media path limits", () => {
  it("rejects loose numeric env values instead of parsing prefixes", () => {
    expect(() =>
      readPositiveIntEnv("DAISYCLAW_CODEX_MEDIA_PATH_TIMEOUT_SECONDS", 180, {
        DAISYCLAW_CODEX_MEDIA_PATH_TIMEOUT_SECONDS: "1e3",
      }),
    ).toThrow("invalid DAISYCLAW_CODEX_MEDIA_PATH_TIMEOUT_SECONDS: 1e3");
    expect(() =>
      readPositiveIntEnv("DAISYCLAW_CODEX_MEDIA_PATH_LOG_TAIL_MAX_BYTES", 2 * 1024 * 1024, {
        DAISYCLAW_CODEX_MEDIA_PATH_LOG_TAIL_MAX_BYTES: "64bytes",
      }),
    ).toThrow("invalid DAISYCLAW_CODEX_MEDIA_PATH_LOG_TAIL_MAX_BYTES: 64bytes");
  });

  it("writes strict positive timeout and port values into generated config", () => {
    const root = makeTempRoot();
    const result = runWriteConfig(root, {
      DAISYCLAW_CODEX_MEDIA_PATH_TIMEOUT_SECONDS: "240",
      PORT: "19002",
    });
  ...
  it("rejects loose write-config timeout env values", () => {
    ...
    expect(result.stderr).toContain("invalid DAISYCLAW_CODEX_MEDIA_PATH_TIMEOUT_SECONDS: 1e3");
  });
});
...
describe("codex media path fake app-server", () => {
  it("returns a structured error when request logging fails", async () => {
    ...
    const child: ChildProcessWithoutNullStreams = spawn(process.execPath, [fakeAppServerPath], {
      env: {
        ...process.env,
        DAISYCLAW_CODEX_MEDIA_PATH_APP_SERVER_LOG: requestLogDirectory,
      },
