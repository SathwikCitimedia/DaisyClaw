...
function makeFakeCrabbox(helpText: string): string {
  ...
  const binDir = mkdtempSync(path.join(tmpdir(), "daisyclaw-fake-crabbox-"));
...
function writeFakeCrabbox(binDir: string, helpText: string): string {
  ...
  if (process.platform !== "win32") {
    const script = [
      "#!/bin/sh",
      'if [ "$1" = "--version" ]; then',
      '  printf "%s\\n" "${DAISYCLAW_FAKE_CRABBOX_VERSION:-crabbox 0.22.1}"',
      "  exit 0",
      "fi",
      ...
      '  for arg in "$@"; do',
      '    if [ "$arg" = "--json" ]; then',
      '      status="${DAISYCLAW_FAKE_CRABBOX_CONFIG_STATUS:-0}"',
      '      if [ "$status" != "0" ]; then',
      '        printf "%s\\n" "config unavailable" >&2',
      '        exit "$status"',
      "      fi",
      '      if [ -n "${DAISYCLAW_FAKE_CRABBOX_CONFIG_JSON+x}" ]; then',
      '        printf "%s" "$DAISYCLAW_FAKE_CRABBOX_CONFIG_JSON"',
      "      else",
      ...
      "fi",
      'if [ "$1" = "whoami" ]; then',
      '  status="${DAISYCLAW_FAKE_CRABBOX_WHOAMI_STATUS:-0}"',
      '  if [ "$status" != "0" ]; then',
      ...
      '  previous_arg="$arg"',
      "done",
      'if [ "${DAISYCLAW_FAKE_CRABBOX_DELETE_CWD_AND_EXIT:-}" = "1" ]; then',
      '  deleted_cwd="$PWD"',
      "  cd / || exit 1",
      '  rm -rf "$deleted_cwd"',
      "  exit 0",
      "fi",
      'if [ "${DAISYCLAW_FAKE_CRABBOX_DELETE_CWD_ONCE:-}" = "1" ]; then',
      '  deleted_cwd="$PWD"',
      "  cd / || exit 1",
      '  rm -rf "$deleted_cwd"',
      "  deadline=100",
      ...
      "fi",
      'printf "%s\\0" "__DAISYCLAW_FAKE_CRABBOX_V1__"',
      'printf "%s\\0" "$PWD"',
      'printf "%s\\0" "$#"',
      'for arg in "$@"; do',
      '  printf "%s\\0" "$arg"',
  ...
  const helperScript = [
    ...
    "  const status = Number.parseInt(process.env.DAISYCLAW_FAKE_CRABBOX_CONFIG_STATUS || '0', 10);",
    "  if (status !== 0) {",
    "    process.stderr.write('config unavailable\\n');",
    "    process.exit(status);",
    "  }",
    '  process.stdout.write(process.env.DAISYCLAW_FAKE_CRABBOX_CONFIG_JSON || \'{"coordinator":"configured-broker","brokerAuth":"configured"}\');',
    "  process.exit(0);",
    "}",
    'if (args[0] === "whoami") {',
    "  const status = Number.parseInt(process.env.DAISYCLAW_FAKE_CRABBOX_WHOAMI_STATUS || '0', 10);",
    "  if (status !== 0) {",
  ...
  const script = [
    ...
    "const args = process.argv.slice(2);",
    'if (args[0] === "--version") {',
    "  console.log(process.env.DAISYCLAW_FAKE_CRABBOX_VERSION || 'crabbox 0.22.1');",
    "  process.exit(0);",
    "}",
...
function makeSlowVersionCrabbox(helpText: string): string {
  const binDir = mkdtempSync(path.join(tmpdir(), "daisyclaw-slow-crabbox-"));
  tempDirs.push(binDir);
  const crabboxPath = path.join(binDir, "crabbox");
...
function makeFakeGit(
  ...
  const binDir = mkdtempSync(path.join(tmpdir(), "daisyclaw-fake-git-"));
  tempDirs.push(binDir);
  ...
  const script = [
    ...
    "const responses = new Map(Object.entries(JSON.parse(process.env.DAISYCLAW_FAKE_GIT_RESPONSES || '{}')));",
    "const args = process.argv.slice(2);",
  ...
  return spawnSync(process.execPath, ["scripts/crabbox-wrapper.mjs", ...args], {
    ...
    env: {
      ...
      PATH: [...(options.extraPathEntries ?? []), binDir, gitBinDir, process.env.PATH ?? ""]
        ...
        .join(path.delimiter),
      CRABBOX_PROVIDER: "",
      DAISYCLAW_CRABBOX_ALLOW_DIRECT_AWS: "",
      DAISYCLAW_CRABBOX_SYNC_MIN_FREE_BYTES: "0",
      DAISYCLAW_CRABBOX_WRAPPER_IGNORE_REPO_BINARY: "1",
      ...(options.configJson
        ? { DAISYCLAW_FAKE_CRABBOX_CONFIG_JSON: JSON.stringify(options.configJson) }
        : {}),
      ...(options.configStatus
        ? { DAISYCLAW_FAKE_CRABBOX_CONFIG_STATUS: String(options.configStatus) }
        : {}),
      ...options.env,
      DAISYCLAW_FAKE_GIT_RESPONSES: JSON.stringify(gitResponses),
    },
    timeout: 10_000,
  });
}
...
function parseFakeCrabboxOutput(result: ReturnType<typeof runWrapper>): {
  ...
  const marker = "__DAISYCLAW_FAKE_CRABBOX_V1__\0";
...
const remoteChangedGateEnvPrefix =
  "DAISYCLAW_CHECK_CHANGED_REMOTE_CHILD=1 DAISYCLAW_CHANGED_LANES_RAW_SYNC=1 CI=1";
...
describe.concurrent("scripts/crabbox-wrapper", () => {
  ...
  it("requires a current Crabbox binary for Blacksmith Testbox runs", () => {
    const result = runWrapper(
      ...
      { env: { DAISYCLAW_FAKE_CRABBOX_VERSION: "crabbox 0.21.9" } },
    );
  ...
  it("rejects prerelease Crabbox builds at the Blacksmith minimum boundary", () => {
    const result = runWrapper(
      "provider: hetzner, aws, local-container, blacksmith-testbox, or cloudflare\n",
      ["run", "--provider", "blacksmith-testbox", "--", "echo ok"],
      { env: { DAISYCLAW_FAKE_CRABBOX_VERSION: "crabbox 0.22.0-rc.1" } },
    );
  ...
  it("accepts post-release Crabbox describe builds at the Blacksmith minimum boundary", () => {
    const result = runWrapper(
      "provider: hetzner, aws, local-container, blacksmith-testbox, or cloudflare\n",
      ["run", "--provider", "blacksmith-testbox", "--", "echo ok"],
      { env: { DAISYCLAW_FAKE_CRABBOX_VERSION: "crabbox 0.22.0-3-gabc1234" } },
    );
  ...
  it("only forces the short local-container Docker work root on Linux", () => {
    ...
    const expectedMessage =
      "[crabbox] provider=docker using short host-visible work root for DaisyClaw Docker tests";
    ...
    const result = runWrapper(
      ...
      ["run", "--target", "windows", "--", "echo ok"],
      {
        env: {
          ...
          DAISYCLAW_CRABBOX_ALLOW_DIRECT_AWS: "1",
        },
      },
    );
  ...
  it("fails closed for AWS proof when broker auth is missing", () => {
    ...
    expect(result.stderr).toContain(
      "crabbox login --url https://crabbox.daisyclaw.ai --provider aws",
    );
  });
  ...
  it("fails closed for AWS proof when broker auth is stale", () => {
    const result = runWrapper(
      ...
      ["run", "--provider", "aws", "--", "echo ok"],
      {
        configJson: { coordinator: "https://crabbox.daisyclaw.ai", brokerAuth: "configured" },
        env: { DAISYCLAW_FAKE_CRABBOX_WHOAMI_STATUS: "1" },
      },
    );
  ...
  it("allows explicit direct AWS debugging without broker auth", () => {
    const result = runWrapper(
      ...
      {
        configJson: { coordinator: "", brokerAuth: "missing" },
        env: { DAISYCLAW_CRABBOX_ALLOW_DIRECT_AWS: "1" },
      },
    );
  ...
  it("bootstraps only Node for raw AWS macOS node commands", () => {
    ...
    expect(remoteCommand).toContain("daisyclaw_crabbox_bootstrap_macos_js");
  ...
  it("normalizes inherited Linux UTF-8 locale names for raw AWS macOS bootstrap", () => {
    ...
    const remoteCommand = normalizeShellLineEndings(output.args.at(-1) ?? "");
    expect(result.status).toBe(0);
    expect(remoteCommand).toContain('macos_locale="${DAISYCLAW_CRABBOX_MACOS_LOCALE:-en_US.UTF-8}"');
  ...
  it("bootstraps Corepack for raw AWS macOS pnpm commands", () => {
    ...
    expect(remoteCommand).toContain('ready_marker="$node_dir/.daisyclaw-crabbox-node-ready"');
    expect(remoteCommand).toContain(
  ...
  it("bootstraps Corepack for raw AWS macOS env option pnpm commands", () => {
    ...
    expect(result.status).toBe(0);
    expect(remoteCommand).toContain("daisyclaw_crabbox_bootstrap_macos_js");
    expect(remoteCommand).toContain("daisyclaw_crabbox_env");
    expect(remoteCommand).not.toContain("export -f env daisyclaw_crabbox_env");
    expect(remoteCommand).not.toContain('env() { daisyclaw_crabbox_env "$@"; };');
    expect(remoteCommand).toContain("PATH=$PATH:${1#PATH=}");
    expect(remoteCommand).toContain("pnpm --version >&2");
    expectGroupedShellCommand(
      remoteCommand,
      "daisyclaw_crabbox_env -i PATH=/usr/bin:/bin pnpm --version",
    );
  });
  ...
  it("bootstraps Corepack for raw AWS macOS env options before ignore-environment", () => {
    ...
    expectGroupedShellCommand(
      remoteCommand,
      "daisyclaw_crabbox_env -u FOO -i PATH=/usr/bin:/bin pnpm --version",
    );
  });
  ...
  it("does not bootstrap absolute env ignore-environment commands it cannot preserve", () => {
    ...
    const remoteCommand = normalizeShellLineEndings(output.args.at(-1) ?? "");
    expect(result.status).toBe(0);
    expect(remoteCommand).not.toContain("daisyclaw_crabbox_bootstrap_macos_js");
  ...
  it("bootstraps Corepack for AWS macOS node changed-gate commands", () => {
    ...
    expectGroupedShellCommand(
      remoteCommand,
      `daisyclaw_crabbox_env ${remoteChangedGateEnvPrefix} node scripts/check-changed.mjs`,
    );
  });
  ...
  it("bootstraps AWS macOS script-stdin runs before the uploaded script body", () => {
    ...
    expect(output.scriptContent).toContain("daisyclaw_crabbox_bootstrap_macos_js");
    expect(output.scriptContent).toContain('if [ ! -d "$TMPDIR" ]; then mkdir -p "$TMPDIR"');
    expect(output.scriptContent).toContain("daisyclaw_crabbox_bootstrap_macos_js || exit $?");
  ...
  it("preserves AWS macOS script-stdin shebang payloads behind the bootstrap wrapper", () => {
    ...
    expect(output.scriptContent).toContain("cat >\"$tmp_script\" <<'DAISYCLAW_CRABBOX_SCRIPT_0'");
    expect(output.scriptContent).toContain(`\n${script}\nDAISYCLAW_CRABBOX_SCRIPT_0\n`);
  ...
  it("restores hydrated node_modules before AWS native Windows shell commands", () => {
    ...
    expect(result.status).toBe(0);
    expect(output.args).toContain("--shell");
    expect(remoteCommand).toContain("$daisyclawModulesDir = $env:PNPM_CONFIG_MODULES_DIR");
    expect(remoteCommand).toContain('mklink /J "$daisyclawSelfModules" "$daisyclawModulesDir"');
    expect(remoteCommand).toContain('mklink /J "$daisyclawWorkspaceModules" "$daisyclawModulesDir"');
  ...
  it("restores hydrated node_modules before AWS native Windows direct commands", () => {
    ...
    expect(output.args).toContain("--shell");
    expect(remoteCommand).toContain("$daisyclawModulesDir = $env:PNPM_CONFIG_MODULES_DIR");
    expect(remoteCommand).toContain("pnpm --filter '@daisyclaw/discord' test");
  });
  ...
  itWithPosixLinkedWorktreeFixture(
    ...
    () => {
      const fakeWorkspaceParent = mkdtempSync(path.join(tmpdir(), "daisyclaw-linked-worktree-"));
      tempDirs.push(fakeWorkspaceParent);
      const gitCommonDir = path.join(fakeWorkspaceParent, "daisyclaw", ".git");
  ...
  if (process.platform !== "win32") {
    it("keeps POSIX PATH lookup semantics for non-executable entries", () => {
      const staleBinDir = mkdtempSync(path.join(tmpdir(), "daisyclaw-stale-crabbox-"));
  ...
  it("uses a temporary full checkout for clean sparse Blacksmith syncs", () => {
    ...
    expect(parseFakeCrabboxOutput(result).cwd).toContain("daisyclaw-crabbox-sync-");
  });
  ...
  it("bootstraps Git metadata for sparse changed gates on remote raw syncs", () => {
    ...
    expect(remoteCommand).toMatch(
      /&& env DAISYCLAW_CHECK_CHANGED_REMOTE_CHILD=1 DAISYCLAW_CHANGED_LANES_RAW_SYNC=1 CI=1 corepack pnpm check:changed$/u,
    );
  });
  ...
  it("bootstraps Git metadata for non-sparse changed gates on remote raw syncs", () => {
    ...
    expect(output.cwd).toContain("daisyclaw-crabbox-sync-");
    expect(output.args).toContain("--shell");
    expect(remoteCommand).toContain("git init -q");
    expect(remoteCommand).toContain(
      "git fetch -q --depth=1 origin abc123:refs/remotes/origin/main",
  ...
  it("bootstraps Git metadata for env-prefixed sparse changed gates", () => {
    const result = runWrapper(
      ...
      [
        ...
        "--",
        "env",
        "DAISYCLAW_CHECK_CHANGED_REMOTE_CHILD=1",
        "DAISYCLAW_CHANGED_LANES_RAW_SYNC=1",
        "CI=1",
        "corepack",
        "pnpm",
        "check:changed",
  ...
  it("preserves macOS JS bootstrapping for sparse changed gates on remote raw syncs", () => {
    ...
    expectGroupedShellCommand(
      remoteCommand,
      `daisyclaw_crabbox_env ${remoteChangedGateEnvPrefix} pnpm check:changed`,
    );
  });
  ...
  it("preserves sparse changed-gate Git bootstrap for bash -lc shell commands", () => {
    ...
    expect(remoteCommand).toContain(
      `&& export DAISYCLAW_CHECK_CHANGED_REMOTE_CHILD=1 DAISYCLAW_CHANGED_LANES_RAW_SYNC=1; ${shellScript}`,
    );
  });
  ...
  it("preserves sparse changed-gate Git bootstrap for direct timeout-wrapped node commands", () => {
    ...
    expect(remoteCommand).toContain("git init -q");
    expect(remoteCommand).toMatch(
      /&& env DAISYCLAW_CHECK_CHANGED_REMOTE_CHILD=1 DAISYCLAW_CHANGED_LANES_RAW_SYNC=1 CI=1 timeout 1200s node scripts\/check-changed\.mjs --base origin\/main --head HEAD$/u,
    );
  });
  ...
  it("preserves sparse changed-gate Git bootstrap for direct timeout-wrapped shell commands", () => {
    ...
    expect(remoteCommand).toContain("git init -q");
    expect(remoteCommand).toMatch(
      /&& env DAISYCLAW_CHECK_CHANGED_REMOTE_CHILD=1 DAISYCLAW_CHANGED_LANES_RAW_SYNC=1 CI=1 timeout 1200s bash -lc 'pnpm check:changed'$/u,
    );
  });
  ...
  it("preserves existing shell changed-gate commands after remote Git bootstrap", () => {
    ...
    expect(remoteCommand).toMatch(
      /&& export DAISYCLAW_CHECK_CHANGED_REMOTE_CHILD=1 DAISYCLAW_CHANGED_LANES_RAW_SYNC=1; env CI=1 pnpm check:changed$/u,
    );
  });
  ...
  it("creates sparse-sync temporary full checkouts under the durable cache root", () => {
    ...
    try {
      const result = runWrapper(
        ...
        ["run", "--provider", "aws", "--", "echo ok"],
        {
          env: { DAISYCLAW_CRABBOX_SYNC_TMPDIR: syncRoot },
          gitResponses: {
            [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
            [GIT_STATUS_PORCELAIN_KEY]: { stdout: "" },
          },
      ...
      const output = parseFakeCrabboxOutput(result);
      expect(result.status).toBe(0);
      expect(output.cwd).toContain(`${syncRoot}${path.sep}daisyclaw-crabbox-sync-`);
  ...
  it("fails sparse-sync full checkout early when the sync root is too low on disk", () => {
    ...
    try {
      const result = runWrapper(
        ...
        {
          env: {
            DAISYCLAW_CRABBOX_SYNC_MIN_FREE_BYTES: "999999999999999",
            DAISYCLAW_CRABBOX_SYNC_TMPDIR: syncRoot,
          },
          gitResponses: {
            [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
            [GIT_STATUS_PORCELAIN_KEY]: { stdout: "" },
      ...
      expect(result.stderr).toContain("DAISYCLAW_CRABBOX_SYNC_TMPDIR");
      expect(result.stderr).toContain("DAISYCLAW_CRABBOX_SYNC_MIN_FREE_BYTES");
      expect(readdirSync(syncRoot)).toEqual([]);
    } finally {
      rmSync(syncRoot, { recursive: true, force: true });
    }
  ...
  it("rejects malformed sparse-sync minimum free byte limits", () => {
    ...
    try {
      const result = runWrapper(
        ...
        {
          env: {
            DAISYCLAW_CRABBOX_SYNC_MIN_FREE_BYTES: "1024mb",
            DAISYCLAW_CRABBOX_SYNC_TMPDIR: syncRoot,
          },
          gitResponses: {
            [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
      ...
      expect(result.status).toBe(1);
      expect(result.stderr).toContain(
        'DAISYCLAW_CRABBOX_SYNC_MIN_FREE_BYTES must be a non-negative integer byte count, got "1024mb"',
      );
      expect(readdirSync(syncRoot)).toEqual([]);
    } finally {
      rmSync(syncRoot, { recursive: true, force: true });
  ...
  it("rejects unsafe sparse-sync minimum free byte limits", () => {
    ...
    try {
      const result = runWrapper(
        ...
        {
          env: {
            DAISYCLAW_CRABBOX_SYNC_MIN_FREE_BYTES: String(Number.MAX_SAFE_INTEGER + 1),
            DAISYCLAW_CRABBOX_SYNC_TMPDIR: syncRoot,
          },
          gitResponses: {
            [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
      ...
      expect(result.status).toBe(1);
      expect(result.stderr).toContain(
        "DAISYCLAW_CRABBOX_SYNC_MIN_FREE_BYTES must be a safe non-negative integer byte count",
      );
      expect(readdirSync(syncRoot)).toEqual([]);
    } finally {
      rmSync(syncRoot, { recursive: true, force: true });
  ...
  it("rejects malformed sparse-sync keepalive intervals", () => {
    ...
    try {
      const result = runWrapper(
        ...
        {
          env: {
            DAISYCLAW_CRABBOX_SYNC_KEEPALIVE_MS: "10ms",
            DAISYCLAW_CRABBOX_SYNC_TMPDIR: syncRoot,
          },
          gitResponses: {
            [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
      ...
      expect(result.status).toBe(1);
      expect(result.stderr).toContain(
        'DAISYCLAW_CRABBOX_SYNC_KEEPALIVE_MS must be a non-negative integer millisecond interval, got "10ms"',
      );
      expect(readdirSync(syncRoot)).toEqual([]);
    } finally {
      rmSync(syncRoot, { recursive: true, force: true });
  ...
  (process.platform === "win32" ? it.skip : it)(
    ...
    () => {
      const result = runWrapper(
        ...
        {
          env: {
            DAISYCLAW_CRABBOX_SYNC_KEEPALIVE_MS: "10",
            DAISYCLAW_FAKE_CRABBOX_DELETE_CWD_ONCE: "1",
          },
          gitResponses: {
            [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
            [GIT_STATUS_PORCELAIN_KEY]: { stdout: "" },
  ...
  (process.platform === "win32" ? it.skip : it)(
    ...
    () => {
      const result = runWrapper(
        ...
        {
          env: {
            DAISYCLAW_CRABBOX_SYNC_KEEPALIVE_MS: "60000",
            DAISYCLAW_FAKE_CRABBOX_DELETE_CWD_AND_EXIT: "1",
          },
          gitResponses: {
            [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
            [GIT_STATUS_PORCELAIN_KEY]: { stdout: "" },
