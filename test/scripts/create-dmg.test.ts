...
function makeApp(plistEntries: string[]): string {
  const dir = mkdtempSync(path.join(tmpdir(), "daisyclaw-create-dmg-"));
  tempDirs.push(dir);
  const app = path.join(dir, "DaisyClaw.app");
...
describe("create-dmg plist validation", () => {
  ...
  it("keeps temporary DMG artifacts scoped to one run", () => {
    ...
    expect(script).toContain('DMG_TEMP="$(mktemp -d "${TMPDIR:-/tmp}/daisyclaw-dmg.XXXXXX")"');
    ...
    expect(script).not.toContain("/tmp/daisyclaw-dmg-limits.txt");
  });

  it.runIf(process.platform === "darwin")(
    "fails before hdiutil when required plist keys are missing",
    () => {
      const app = makeApp(["<key>CFBundleName</key>", "<string>DaisyClaw</string>"]);
