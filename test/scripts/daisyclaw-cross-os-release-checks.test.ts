// Daisyclaw Cross Os Release Checks tests cover daisyclaw cross os release checks script behavior.
...
} from "../../scripts/daisyclaw-cross-os-release-checks.ts";
...
describe("scripts/daisyclaw-cross-os-release-checks", () => {
  ...
  it("requires dashboard root markers and same-origin asset URLs", () => {
    const html = [
      "<title>DaisyClaw Control</title>",
      "<daisyclaw-app></daisyclaw-app>",
  ...
  it("keeps gateway status RPC probing when help probing is unavailable", () => {
    ...
    expect(buildGatewayStatusArgsFromHelpText("Usage: daisyclaw gateway status")).toEqual([
      "gateway",
      "status",
    ]);
    expect(
  ...
  it("gives the Windows packaged updater wrapper enough headroom for DaisyClaw timeout output", () => {
  ...
  it("rejects malformed cross-OS positive integer environment values", () => {
    expect(parsePositiveIntegerEnv("DAISYCLAW_CROSS_OS_COMMAND_HEARTBEAT_SECONDS", 60, {})).toBe(60);
    expect(
      parsePositiveIntegerEnv("DAISYCLAW_CROSS_OS_COMMAND_HEARTBEAT_SECONDS", 60, {
        DAISYCLAW_CROSS_OS_COMMAND_HEARTBEAT_SECONDS: "25",
      }),
    ).toBe(25);
    ...
    for (const raw of ["1e3", "25ms", "1.5", "0", "-1", String(Number.MAX_SAFE_INTEGER + 1)]) {
      expect(() =>
        parsePositiveIntegerEnv("DAISYCLAW_CROSS_OS_COMMAND_HEARTBEAT_SECONDS", 60, {
          DAISYCLAW_CROSS_OS_COMMAND_HEARTBEAT_SECONDS: raw,
        }),
      ).toThrow("DAISYCLAW_CROSS_OS_COMMAND_HEARTBEAT_SECONDS must be a positive integer");
    }
  });

  it("records packaged-fresh phase timings for release-check summaries", () => {
    const source = readFileSync("scripts/daisyclaw-cross-os-release-checks.ts", "utf8");
  ...
  it("accepts OK agent output from the captured log when stdout is empty", () => {
    const dir = mkdtempSync(join(tmpdir(), "daisyclaw-cross-os-agent-output-"));
    try {
  ...
  it("ignores stale OK markers outside the recent agent log tail", () => {
    const dir = mkdtempSync(join(tmpdir(), "daisyclaw-cross-os-agent-output-tail-"));
    try {
      const logPath = join(dir, "agent.log");
      writeFileSync(
        logPath,
  ...
  it("requires explicit opt-in before cross-OS agent turns become optional", () => {
    ...
    expect(resolveCrossOsAgentTurnOptional({ DAISYCLAW_CROSS_OS_AGENT_TURN_OPTIONAL: "1" })).toBe(
      true,
    );
    expect(
      resolveCrossOsAgentTurnOptional({ DAISYCLAW_CROSS_OS_AGENT_TURN_OPTIONAL: "false" }),
    ).toBe(false);
  });

  it("detects embedded fallback agent turns as non-gateway proof", () => {
    const dir = mkdtempSync(join(tmpdir(), "daisyclaw-cross-os-agent-fallback-"));
    const logPath = join(dir, "agent.log");
    expect(
      agentTurnUsedEmbeddedFallback({
  ...
  it("skips optional live agent turns only for model availability failures", () => {
    const dir = mkdtempSync(join(tmpdir(), "daisyclaw-cross-os-agent-skip-"));
    try {
      const logPath = join(dir, "agent.log");
      writeFileSync(
        logPath,
  ...
  it("does not classify stale timeout logs as current optional agent-turn failures", () => {
    const dir = mkdtempSync(join(tmpdir(), "daisyclaw-cross-os-agent-skip-tail-"));
    try {
      const logPath = join(dir, "agent.log");
      writeFileSync(
        logPath,
  ...
  it("only skips opted-in cross-OS live agent turns after retry exhaustion", () => {
    const dir = mkdtempSync(join(tmpdir(), "daisyclaw-cross-os-agent-skip-retry-"));
    try {
      const logPath = join(dir, "agent.log");
  ...
  it("allows cross-OS provider smoke models to use faster CI overrides", () => {
    expect(
      resolveProviderConfig("openai", {
        DAISYCLAW_CROSS_OS_OPENAI_MODEL: "openai/gpt-5.4-mini",
      })?.model,
    ).toBe("openai/gpt-5.4-mini");
    expect(
      resolveProviderConfig("openai", {
        DAISYCLAW_CROSS_OS_MODEL: "openai/gpt-5.4-nano",
      })?.model,
  ...
  it("keeps release cross-OS OpenAI smoke on GPT-5.5", () => {
    ...
    const releaseChecks = readFileSync(".github/workflows/daisyclaw-release-checks.yml", "utf8");

    expect(workflow).toContain(
      "DAISYCLAW_CROSS_OS_OPENAI_MODEL: ${{ inputs.openai_model || vars.DAISYCLAW_CROSS_OS_OPENAI_MODEL || 'openai/gpt-5.5' }}",
    );
  ...
  it("can stage packaged-upgrade baselines without npm lifecycle scripts", () => {
    expect(buildNpmGlobalInstallArgs("daisyclaw@2026.5.2", { ignoreScripts: true })).toEqual([
      "install",
      "-g",
      "daisyclaw@2026.5.2",
      "--omit=dev",
  ...
  it("keeps packaged-upgrade release updates out of service restart flow", () => {
    const args = buildPackagedUpgradeUpdateArgs("http://127.0.0.1:49152/daisyclaw-current.tgz");
    expect(args.slice(0, 6)).toEqual([
      "update",
      "--tag",
      "http://127.0.0.1:49152/daisyclaw-current.tgz",
      "--yes",
      "--json",
      "--no-restart",
    ]);
  ...
  it("keeps cross-OS live smoke agent turns on GPT-5-safe timeouts and minimal context", () => {
    ...
    expect(source).toContain('agentRuntime: { id: "daisyclaw" }');
  ...
  it("prefers the freshly installed Windows CLI under npm's prefix before PATH lookup", () => {
    ...
    expect(script).toContain("(Join-Path $npmPrefix 'daisyclaw.cmd')");
    expect(script).toContain("$cmd = Get-Command daisyclaw -ErrorAction Stop");
  });
  ...
  it("prefers workflow-injected runner override env names over legacy ones", () => {
    expect(
      readRunnerOverrideEnv({
        ...
        DAISYCLAW_RELEASE_CHECKS_UBUNTU_RUNNER: "legacy-linux",
        DAISYCLAW_RELEASE_CHECKS_WINDOWS_RUNNER: "legacy-windows",
        DAISYCLAW_RELEASE_CHECKS_MACOS_RUNNER: "legacy-macos",
      }),
    ).toEqual({
  ...
  it("serves installer scripts as UTF-8 text and package payloads as binary", () => {
    ...
    expect(resolveStaticFileContentType("daisyclaw-2026.4.14.tgz")).toBe("application/octet-stream");
  });

  it("streams release artifacts from the static file server", async () => {
    const dir = mkdtempSync(join(tmpdir(), "daisyclaw-cross-os-static-server-"));
    const filePath = join(dir, "daisyclaw-2026.4.14.tgz");
  ...
  it("closes static release artifact sockets left by aborted clients", async () => {
    const dir = mkdtempSync(join(tmpdir(), "daisyclaw-cross-os-static-server-close-"));
    const filePath = join(dir, "daisyclaw-2026.4.14.tgz");
    const logPath = join(dir, "server.log");
    let server: Awaited<ReturnType<typeof startStaticFileServer>> | undefined;
  ...
  it("flushes static release artifact logs before close resolves", async () => {
    const dir = mkdtempSync(join(tmpdir(), "daisyclaw-cross-os-static-server-log-flush-"));
    const filePath = join(dir, "daisyclaw-2026.4.14.tgz");
    const logPath = join(dir, "server.log");
    let server: Awaited<ReturnType<typeof startStaticFileServer>> | undefined;
  ...
  it("uses the published installer URLs for native installer lanes", () => {
    expect(resolvePublishedInstallerUrl("darwin")).toBe("https://daisyclaw.ai/install.sh");
    expect(resolvePublishedInstallerUrl("linux")).toBe("https://daisyclaw.ai/install.sh");
    expect(resolvePublishedInstallerUrl("win32")).toBe("https://daisyclaw.ai/install.ps1");
  });
  ...
  it("runs the installed browser override import smoke only on native Windows", () => {
    ...
    expect(script).toContain('from "daisyclaw/plugin-sdk/plugin-runtime"');
    expect(script).toContain('overrideEnvVar: "DAISYCLAW_BROWSER_CONTROL_MODULE"');
    ...
    const installedScript = buildInstalledBrowserOverrideImportProbeScript(
      "file:///C:/Users/runner/AppData/Roaming/npm/node_modules/daisyclaw/dist/plugin-sdk/plugin-runtime.js",
    );
    expect(installedScript).toContain(
      'from "file:///C:/Users/runner/AppData/Roaming/npm/node_modules/daisyclaw/dist/plugin-sdk/plugin-runtime.js"',
    );
    expect(readFileSync("scripts/daisyclaw-cross-os-release-checks.ts", "utf8")).toContain(
      "DAISYCLAW_BROWSER_CONTROL_MODULE: pathToFileURL(overridePath).href",
    );
  });
  ...
  it("wraps Windows cmd shims without Node shell argv", () => {
    expect(
      ...
      args: [
        ...
        "/s",
        "/c",
        String.raw`""C:\Program Files\nodejs\npm.cmd" view daisyclaw@latest version"`,
      ],
      shell: false,
  ...
  it("wraps installed Windows CLI cmd fallbacks without Node shell argv", () => {
    expect(
      resolveInstalledCliInvocation(
        win32.join(String.raw`C:\DaisyClaw Prefix`, "daisyclaw.cmd"),
      ...
      args: [
        ...
        "/s",
        "/c",
        String.raw`""C:\DaisyClaw Prefix\daisyclaw.cmd" gateway run --port 1234"`,
      ],
      shell: false,
      windowsVerbatimArguments: true,
    });
  ...
  it("runs resolved command invocations and writes command logs", async () => {
    const dir = mkdtempSync(join(tmpdir(), "daisyclaw-cross-os-run-command-"));
    try {
  ...
  it("bounds retained command output while preserving full command logs", async () => {
    const dir = mkdtempSync(join(tmpdir(), "daisyclaw-cross-os-run-command-output-"));
    try {
      const logPath = join(dir, "command.log");
  ...
  it("flushes command logs before resolving", async () => {
    const dir = mkdtempSync(join(tmpdir(), "daisyclaw-cross-os-run-command-flush-"));
    try {
  ...
  it("kills timed-out command process groups", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "daisyclaw-cross-os-run-command-timeout-"));
    ...
    try {
      ...
      const parentScript = [
        ...
        "fs.writeFileSync(process.env.DAISYCLAW_TEST_CHILD_PID, String(child.pid));",
        "setInterval(() => {}, 1000);",
      ].join("");

      const command = runCommand(process.execPath, ["-e", parentScript], {
        cwd: dir,
        env: { ...process.env, DAISYCLAW_TEST_CHILD_PID: childPidPath },
        logPath,
        timeoutMs: 500,
      });
  ...
  it("forwards external termination to command process groups", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "daisyclaw-cross-os-run-command-signal-"));
    const childPidPath = join(dir, "child.pid");
    const scriptUrl = pathToFileURL(
      resolvePath("scripts/daisyclaw-cross-os-release-checks.ts"),
    ).href;
    ...
    try {
      ...
      const runner = spawn(
        ...
        ["--import", "tsx", "--input-type=module", "-e", runnerScript],
        {
          ...
          env: {
            ...process.env,
            DAISYCLAW_CROSS_OS_PROCESS_TREE_KILL_AFTER_MS: "25",
            DAISYCLAW_TEST_CHILD_PID: childPidPath,
          },
  ...
  it("derives the installed prefix from resolved CLI paths", () => {
    ...
    expect(
      resolveInstalledPrefixDirFromCliPath("/Users/runner/.npm-global/bin/daisyclaw", "darwin"),
    ).toBe("/Users/runner/.npm-global");
  });

  it("resolves Linux npm package roots when the CLI is a user-local shim", () => {
    const homeDir = mkdtempSync(join(tmpdir(), "daisyclaw-cross-os-linux-home-"));
    try {
      const packageRoot = join(homeDir, ".npm-global", "lib", "node_modules", "daisyclaw");
      const distDir = join(packageRoot, "dist");
      const cliDir = join(homeDir, ".local", "bin");
      mkdirSync(distDir, { recursive: true });
      mkdirSync(cliDir, { recursive: true });
      writeFileSync(join(packageRoot, "package.json"), JSON.stringify({ name: "daisyclaw" }));
      writeFileSync(join(distDir, "entry.js"), "#!/usr/bin/env node\n");

      expect(
        resolveInstalledPackageRootFromCliPath(join(cliDir, "daisyclaw"), "linux", {
          HOME: homeDir,
        }),
      ).toBe(packageRoot);

      rmSync(join(cliDir, "daisyclaw"), { force: true });
      symlinkSync(join(distDir, "entry.js"), join(cliDir, "daisyclaw"));
  ...
  it("drops the bundled plugin postinstall disable flag for real updater calls", () => {
    expect(
      buildRealUpdateEnv({
        FOO: "bar",
        NODE_COMPILE_CACHE: "/tmp/stale-daisyclaw-cache",
        DAISYCLAW_DISABLE_BUNDLED_PLUGIN_POSTINSTALL: "1",
      }),
    ).toEqual({
      FOO: "bar",
      DAISYCLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS: "1",
      ...
      verifyPackagedUpgradeUpdateResult(
        {
          ...
          stderr:
            "[daisyclaw] Failed to start CLI: Error [ERR_MODULE_NOT_FOUND]: Cannot find module '/tmp/prefix/lib/node_modules/daisyclaw/dist/memory-state-old.js'",
        },
  ...
  it("recognizes the shipped Windows updater packaged-upgrade timeout", () => {
    const error = new Error(
      "Command timed out: C:\\hostedtoolcache\\windows\\node\\24.15.0\\x64\\node.exe C:\\Users\\RUNNER~1\\AppData\\Local\\Temp\\daisyclaw-upgrade-q9DsA7\\prefix\\node_modules\\daisyclaw\\daisyclaw.mjs update --tag http://127.0.0.1:49951/daisyclaw-2026.5.4-beta.1.tgz --yes --json --no-restart --timeout 1500",
    );
    ...
    expect(
      isRecoverableWindowsPackagedUpgradeTimeoutError(
        new Error(
          "Command timed out: C:\\prefix\\node_modules\\daisyclaw\\daisyclaw.mjs update --tag http://127.0.0.1:49951/daisyclaw-current.tgz --yes --json --timeout 1500",
        ),
        "win32",
      ),
    ).toBe(true);
    ...
    expect(
      isRecoverableWindowsPackagedUpgradeTimeoutError(
        new Error("Command timed out: node daisyclaw.mjs update --tag daisyclaw@beta"),
        "win32",
      ),
    ).toBe(false);
  });
      ...
      isRecoverableWindowsPackagedUpgradeSwapCleanupFailure(
        {
          exitCode: 1,
          stdout:
            "EPERM: operation not permitted, unlink '/tmp/prefix/node_modules/.daisyclaw-1-2/native.node'",
          stderr: "",
        },
        "linux",
      ),
  ...
  it("only treats pinned baseline specs as exact installer version assertions", () => {
    ...
    expect(resolveExplicitBaselineVersion("daisyclaw@latest")).toBe("");
    expect(resolveExplicitBaselineVersion("daisyclaw@2026.4.10")).toBe("2026.4.10");
  ...
  it("reads an installed baseline version without requiring build metadata", () => {
    const prefixDir = mkdtempSync(join(tmpdir(), "daisyclaw-cross-os-installed-version-"));
    try {
      const packageRoot =
        process.platform === "win32"
          ? join(prefixDir, "node_modules", "daisyclaw")
          : join(prefixDir, "lib", "node_modules", "daisyclaw");
  ...
  it("treats missing package scripts as optional in older refs", () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "daisyclaw-cross-os-scripts-"));
    try {
      writeFileSync(
        join(packageRoot, "package.json"),
        JSON.stringify({
  ...
  it("rejects legacy plugin dependency staging debris before candidate inventory generation", async () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "daisyclaw-cross-os-stage-debris-"));
    try {
      mkdirSync(
        join(packageRoot, "dist", "Extensions", "demo", ".DaisyClaw-Install-Stage", "node_modules"),
        { recursive: true },
      );
      writeFileSync(
        join(packageRoot, "dist", "Extensions", "demo", ".DaisyClaw-Install-Stage", "package.json"),
        "{}\n",
        "utf8",
      );
  ...
  it("omits local build metadata from candidate package inventories", async () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "daisyclaw-cross-os-local-stamps-"));
    try {
      mkdirSync(join(packageRoot, "dist"), { recursive: true });
      writeFileSync(
        join(packageRoot, "package.json"),
        JSON.stringify({ name: "daisyclaw-fixture", version: "0.0.0", files: ["dist/"] }),
        "utf8",
      );
