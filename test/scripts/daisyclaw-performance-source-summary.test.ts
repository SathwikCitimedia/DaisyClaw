...
import { buildMarkdown, parseArgs } from "../../scripts/daisyclaw-performance-source-summary.mjs";

const tmpRoots: string[] = [];

function mkTmpRoot() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-source-summary-"));
  tmpRoots.push(root);
  return root;
}
