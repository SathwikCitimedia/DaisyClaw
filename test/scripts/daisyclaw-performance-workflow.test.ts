// Daisyclaw Performance Workflow tests cover daisyclaw performance workflow script behavior.
import { readFileSync } from "node:fs";
import { describe, expect, it } from "vitest";
import { parse } from "yaml";

const WORKFLOW = ".github/workflows/daisyclaw-performance.yml";
...
describe("DaisyClaw performance workflow", () => {
  it("uses the clawgrit reports token for every report repo push path", () => {
    ...
    expect(prepare.run).toContain(
      'remote add origin "https://x-access-token:${CLAWGRIT_REPORTS_TOKEN}@github.com/daisyclaw/clawgrit-reports.git"',
    );
    expect(publish.run).toContain(
      'remote set-url origin "https://x-access-token:${CLAWGRIT_REPORTS_TOKEN}@github.com/daisyclaw/clawgrit-reports.git"',
    );
