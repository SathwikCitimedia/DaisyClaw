...
describe("dependency guard workflow", () => {
  ...
  it("keeps contents write scoped to the conditional autoscrub job", () => {
    ...
    expect(detectSteps[1].env?.DAISYCLAW_DEPENDENCY_GUARD_MODE).toBe("detect");
    ...
    expect(autoscrubSteps[3].env?.DAISYCLAW_DEPENDENCY_GUARD_AUTOSCRUB_TOKEN).toBe(
      "${{ steps.app-token.outputs.token || steps.app-token-fallback.outputs.token }}",
    );
    expect(autoscrubSteps[3].env?.DAISYCLAW_DEPENDENCY_GUARD_MODE).toBe("autoscrub");
    expect(finalSteps[1].env?.DAISYCLAW_DEPENDENCY_GUARD_MODE).toBe("enforce");
  });
  ...
  it("uses a dedicated checked-in script and bounded sticky comments", () => {
    ...
    expect(runStep.env?.DAISYCLAW_SECURITY_TEAM_SLUG).toBe("daisyclaw-secops");
    expect(runStep.env?.DAISYCLAW_SECURITY_APPROVERS).toBe("vincentkoc,steipete,joshavant");
  ...
  it("blocks package lockfile and manifest graph changes unless secops approves the current head sha", () => {
    ...
    expect(script).toContain("daisyclaw-secops");
  ...
  it("requires secops review for future workflow or guard changes", () => {
    ...
    expect(codeowners).toContain(
      "/.github/workflows/dependency-guard.yml @daisyclaw/daisyclaw-secops",
    );
    expect(codeowners).toContain(
      "/test/scripts/dependency-guard-workflow.test.ts @daisyclaw/daisyclaw-secops",
    );
    expect(codeowners).toContain("/scripts/github/dependency-guard.mjs @daisyclaw/daisyclaw-secops");
    expect(codeowners).toContain("/package-lock.json @daisyclaw/daisyclaw-secops");
    expect(codeowners).toContain("/npm-shrinkwrap.json @daisyclaw/daisyclaw-secops");
    expect(codeowners).toContain("/extensions/*/package-lock.json @daisyclaw/daisyclaw-secops");
    expect(codeowners).toContain("/extensions/*/npm-shrinkwrap.json @daisyclaw/daisyclaw-secops");
  });
});
