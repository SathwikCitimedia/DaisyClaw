...
describe("scripts/test-docker-all scheduler", () => {
  ...
  it("prints CLI help without a stack trace", () => {
    ...
    expect(result.stdout).toContain("DAISYCLAW_DOCKER_ALL_* env vars");
  });
  ...
  it("rejects loose numeric runner env vars without a stack trace", () => {
    const result = spawnSync(process.execPath, ["scripts/test-docker-all.mjs", "--plan-json"], {
      ...
      env: {
        ...process.env,
        DAISYCLAW_DOCKER_ALL_PARALLELISM: "1e3",
      },
    });

    expect(result.status).toBe(1);
    expect(result.stdout).toBe("");
    expect(result.stderr).toContain("DAISYCLAW_DOCKER_ALL_PARALLELISM must be a positive integer");
    expect(result.stderr).not.toContain("at ");
  });

  it("rejects loose numeric resource limit env vars before scheduling lanes", () => {
    const logDir = mkdtempSync(`${tmpdir()}/daisyclaw-docker-all-`);
    try {
      const result = spawnSync(process.execPath, ["scripts/test-docker-all.mjs"], {
        ...
        env: {
          ...
          DAISYCLAW_DOCKER_ALL_DOCKER_LIMIT: "1e3",
          DAISYCLAW_DOCKER_ALL_DRY_RUN: "1",
          DAISYCLAW_DOCKER_ALL_LOG_DIR: logDir,
          DAISYCLAW_DOCKER_ALL_PREFLIGHT: "0",
          DAISYCLAW_DOCKER_ALL_TIMINGS: "0",
        },
      });

      expect(result.status).toBe(1);
      expect(result.stderr).toContain(
        "DAISYCLAW_DOCKER_ALL_DOCKER_LIMIT must be a positive integer",
      );
      expect(result.stderr).not.toContain("at ");
    } finally {
  ...
  it("rejects release-path configs that schedule zero Docker lanes", () => {
    ...
    try {
      const result = spawnSync(process.execPath, ["scripts/test-docker-all.mjs"], {
        ...
        env: {
          ...process.env,
          DAISYCLAW_DOCKER_ALL_CHUNK: "openwebui",
          DAISYCLAW_DOCKER_ALL_DRY_RUN: "1",
          DAISYCLAW_DOCKER_ALL_INCLUDE_OPENWEBUI: "0",
          DAISYCLAW_DOCKER_ALL_LOG_DIR: logDir,
          DAISYCLAW_DOCKER_ALL_PREFLIGHT: "0",
  ...
  posixIt("writes Docker run artifacts when cleanup smoke fails", () => {
    const root = mkdtempSync(`${tmpdir()}/daisyclaw-docker-all-cleanup-`);
    const logDir = path.join(root, "logs");
    const packageTgz = path.join(root, "daisyclaw-current.tgz");
...
`,
    ...
    try {
      const result = spawnSync(process.execPath, ["scripts/test-docker-all.mjs"], {
        ...
        env: {
          ...process.env,
          DAISYCLAW_CURRENT_PACKAGE_TGZ: packageTgz,
          DAISYCLAW_DOCKER_ALL_BUILD: "0",
          DAISYCLAW_DOCKER_ALL_LIVE_MODE: "skip",
          DAISYCLAW_DOCKER_ALL_LOG_DIR: logDir,
          DAISYCLAW_DOCKER_ALL_PARALLELISM: "16",
          DAISYCLAW_DOCKER_ALL_PREFLIGHT: "0",
          DAISYCLAW_DOCKER_ALL_START_STAGGER_MS: "0",
          DAISYCLAW_DOCKER_ALL_STATUS_INTERVAL_MS: "0",
          DAISYCLAW_DOCKER_ALL_TAIL_PARALLELISM: "16",
          DAISYCLAW_DOCKER_ALL_TIMINGS: "0",
  ...
  it("cleans stale stopped containers from all named Docker E2E lanes", () => {
    expect(
      dockerPreflightContainerNames(`
daisyclaw-gateway-e2e-123 Exited (1) 2 minutes ago
daisyclaw-config-reload-e2e-234 Created
daisyclaw-plugin-binding-command-escape-e2e-345 Dead
daisyclaw-kitchen-sink-rpc-e2e-456 Exited (137) 10 seconds ago
daisyclaw-openwebui-gateway-567 Exited (1) 3 minutes ago
daisyclaw-openwebui-678 Created
daisyclaw-not-an-e2e-container Exited (1) 2 minutes ago
postgres Created
`),
    ).toEqual([
      "daisyclaw-gateway-e2e-123",
      "daisyclaw-config-reload-e2e-234",
      "daisyclaw-plugin-binding-command-escape-e2e-345",
      "daisyclaw-kitchen-sink-rpc-e2e-456",
      "daisyclaw-openwebui-gateway-567",
      "daisyclaw-openwebui-678",
    ]);
  });
  ...
  it("reads bounded lane log tails instead of full noisy logs", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "daisyclaw-docker-all-log-tail-"));
    try {
  ...
  posixIt("kills timed-out shell command groups when the leader exits first", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "daisyclaw-docker-all-timeout-"));
