...
describe("docker build helper", () => {
  it("forces BuildKit for centralized Docker builds", () => {
    ...
    expect(helper).toContain("DAISYCLAW_DOCKER_BUILD_RETRIES");
    expect(helper).toContain("DAISYCLAW_DOCKER_BUILD_TIMEOUT");
    expect(helper).toContain('docker_build_run_logged "$label" "$timeout_value" "$log_file"');
    expect(helper).toContain("DAISYCLAW_DOCKER_BUILD_REQUIRE_TIMEOUT");
  ...
  it("treats Docker registry auth 5xx failures as transient build failures", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-build-transient-"));
...
`;
  ...
  it("routes standalone Docker smoke runs through the timeout-aware helper", () => {
    ...
    expect(cleanupSmoke).toContain(
      'DOCKER_COMMAND_TIMEOUT="${DOCKER_COMMAND_TIMEOUT:-${DAISYCLAW_CLEANUP_SMOKE_DOCKER_TIMEOUT:-600s}}"',
    );
    expect(cleanupSmoke).toContain(
  ...
  it("bounds cleanup-smoke failure log output", () => {
    ...
    expect(cleanupRun).toContain("DAISYCLAW_CLEANUP_SMOKE_LOG_PRINT_BYTES");
    expect(cleanupRun.match(/print_log_tail \/tmp\/daisyclaw-cleanup-/g)).toHaveLength(3);
    expect(cleanupRun).not.toContain("cat /tmp/daisyclaw-cleanup-");
  });
  ...
  it("prints in-container Docker client logs through bounded helpers", () => {
    for (const scriptPath of [CODEX_MEDIA_PATH_SCENARIO_PATH, OPENAI_CHAT_TOOLS_SCENARIO_PATH]) {
      ...
      expect(script, scriptPath).toContain("source scripts/lib/daisyclaw-e2e-instance.sh");
      expect(script, scriptPath).toContain('daisyclaw_e2e_print_log "$CLIENT_LOG"');
      expect(script, scriptPath).not.toContain('cat "$CLIENT_LOG"');
    }
  });
  ...
  it("runs cleanup smoke on the native ARM platform instead of pulling an amd64 tag", () => {
    ...
    expect(
      runCleanupDefaultPlatform({ DAISYCLAW_CLEANUP_SMOKE_PLATFORM: "linux/s390x" }, "x86_64"),
    ).toBe("linux/s390x");
  });
  ...
  it("lets Testbox fall back to building when a reused Docker image is missing", () => {
    ...
    expect(helper).toContain("DAISYCLAW_DOCKER_BUILD_ON_MISSING");
    expect(helper).toContain("DAISYCLAW_TESTBOX");
    ...
    expect(liveBuild).toContain(
      'DOCKER_COMMAND_TIMEOUT="${DOCKER_COMMAND_TIMEOUT:-${DAISYCLAW_LIVE_DOCKER_PULL_TIMEOUT:-600s}}"',
    );
    expect(liveBuild).toContain(
      'LIVE_IMAGE_PULL_ATTEMPTS="${DAISYCLAW_LIVE_DOCKER_PULL_ATTEMPTS:-3}"',
    );
    ...
    expect(liveCliBackend).not.toContain(
      'echo "==> Reuse live-test image: $LIVE_IMAGE_NAME (DAISYCLAW_SKIP_DOCKER_BUILD=1)"',
    );
  });

  it("wraps centralized Docker builds with the timeout helper", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-build-timeout-"));
...
export DAISYCLAW_DOCKER_BUILD_TIMEOUT=17s
...
`;
  ...
  it("prints heartbeat progress for long successful centralized Docker builds", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-build-heartbeat-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin:$PATH"
export DAISYCLAW_DOCKER_BUILD_HEARTBEAT_SECONDS=1
...
`;
  ...
  it("stops the tracked build command without retrying when interrupted", async () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-build-signal-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin:$PATH"
export DAISYCLAW_DOCKER_BUILD_RETRIES=3
source "$ROOT_DIR/scripts/lib/docker-build.sh"
docker_build_run e2e-build -t demo-image .
`,
      );
  ...
  it("does not delay fast successful centralized Docker builds until the next heartbeat", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-build-fast-heartbeat-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin:$PATH"
export DAISYCLAW_DOCKER_BUILD_HEARTBEAT_SECONDS=30
...
ROOT_DIR=${shellQuote(rootDir)}
export ROOT_DIR
export DAISYCLAW_DOCKER_BUILD_HEARTBEAT_SECONDS=08
...
`;
  ...
  it("fails centralized Docker builds fast when timeout is unavailable", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-build-timeout-required-"));
...
export DAISYCLAW_DOCKER_BUILD_TIMEOUT=19s
...
`;
  ...
  it("keeps setup-style Docker builds compatible when timeout is unavailable", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-build-timeout-optional-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin"
export DAISYCLAW_DOCKER_BUILD_TIMEOUT=23s
...
`;
  ...
  it("keeps reused Docker image probes behind the timeout-aware helper", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-image-reuse-timeout-"));
...
export DAISYCLAW_SKIP_DOCKER_BUILD=1
...
grep -q '^image inspect daisyclaw-reuse-image$' "$TMPDIR/docker-seen"
grep -q '^pull daisyclaw-reuse-image$' "$TMPDIR/docker-seen"
`;
  ...
  it("fails Docker commands fast when timeout is unavailable", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-timeout-required-"));
...
`;
  ...
  it("uses a Node watchdog for Docker commands when timeout is unavailable", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-node-timeout-"));
...
export PATH="$TMPDIR/bin"
export DOCKER_COMMAND_TIMEOUT=7s
unset DAISYCLAW_DOCKER_E2E_DISABLE_RESOURCE_LIMITS
unset DAISYCLAW_DOCKER_E2E_MEMORY DAISYCLAW_DOCKER_E2E_CPUS DAISYCLAW_DOCKER_E2E_PIDS_LIMIT
...
`;
  ...
  it("adds default Docker run resource limits without overriding explicit limits", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-resource-limits-"));
...
unset DAISYCLAW_DOCKER_E2E_DISABLE_RESOURCE_LIMITS
unset DAISYCLAW_DOCKER_E2E_MEMORY DAISYCLAW_DOCKER_E2E_CPUS DAISYCLAW_DOCKER_E2E_PIDS_LIMIT
export DAISYCLAW_DOCKER_E2E_AVAILABLE_CPUS=32
...
DAISYCLAW_DOCKER_E2E_MEMORY=12g DAISYCLAW_DOCKER_E2E_CPUS=4 DAISYCLAW_DOCKER_E2E_PIDS_LIMIT=512 docker_e2e_docker_cmd run demo
DAISYCLAW_DOCKER_E2E_AVAILABLE_CPUS=8 DAISYCLAW_DOCKER_E2E_MEMORY=12g DAISYCLAW_DOCKER_E2E_CPUS=16 DAISYCLAW_DOCKER_E2E_PIDS_LIMIT=512 docker_e2e_docker_cmd run demo
docker_e2e_docker_cmd run --memory 2g --cpus 3 --pids-limit 99 demo
DAISYCLAW_DOCKER_E2E_DISABLE_RESOURCE_LIMITS=1 docker_e2e_docker_cmd run demo
...
`;
  ...
  for (const [shellSignal, expectedStatus] of [
    ...
    it(`escalates Docker watchdog children that ignore parent SIG${shellSignal}`, () => {
      const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-node-signal-"));
...
export DAISYCLAW_DOCKER_TIMEOUT_KILL_GRACE_MS=100
...
`;
  ...
  it("uses plain timeout when kill-after is unsupported", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-plain-timeout-"));
...
`;
  ...
  it("uses gtimeout when timeout is unavailable", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-gtimeout-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin"
export DAISYCLAW_DOCKER_E2E_RUN_TIMEOUT=13s
export DAISYCLAW_DOCKER_E2E_AVAILABLE_CPUS=8
unset DAISYCLAW_DOCKER_E2E_DISABLE_RESOURCE_LIMITS
unset DAISYCLAW_DOCKER_E2E_MEMORY DAISYCLAW_DOCKER_E2E_CPUS DAISYCLAW_DOCKER_E2E_PIDS_LIMIT
...
`;
  ...
  it("keeps package-backed Docker runs bounded without the shared timeout helper", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-package-timeout-required-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin"
export DAISYCLAW_DOCKER_E2E_RUN_TIMEOUT=11s
...
`;
  ...
  it("uses gtimeout for package-backed Docker runs without the shared timeout helper", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-package-gtimeout-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin"
export DAISYCLAW_DOCKER_E2E_RUN_TIMEOUT=15s
export DAISYCLAW_DOCKER_E2E_AVAILABLE_CPUS=8
unset DAISYCLAW_DOCKER_E2E_DISABLE_RESOURCE_LIMITS
unset DAISYCLAW_DOCKER_E2E_MEMORY DAISYCLAW_DOCKER_E2E_CPUS DAISYCLAW_DOCKER_E2E_PIDS_LIMIT
...
`;
  ...
  it("removes functional Docker build package inputs after the build", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-build-cleanup-"));
...
node() {
  local script="$1"
  shift
  if [[ "$script" != "$ROOT_DIR/scripts/package-daisyclaw-for-docker.mjs" ]]; then
    command node "$script" "$@"
    return
  fi
...
docker_build_run() {
  ...
  for arg in "$@"; do
    case "$arg" in
      daisyclaw_package=*)
        build_context="\${arg#daisyclaw_package=}"
        ;;
    esac
  done

  test -n "$build_context"
  test -f "$build_context/daisyclaw-current.tgz"
  printf "%s\\n" "$build_context" >"$TMPDIR/build-context-seen"
}

docker_e2e_build_or_reuse \\
  daisyclaw-test-image \\
  cleanup-proof \\
  "$ROOT_DIR/scripts/e2e/Dockerfile" \\
  "$ROOT_DIR" \\
  functional
...
leftovers="$(find "$TMPDIR" -maxdepth 1 \\( \\
  -name 'daisyclaw-docker-e2e-pack.*' \\
  -o -name 'daisyclaw-docker-e2e-package-context.*' \\
\\) -print)"
if [[ -n "$leftovers" ]]; then
...
`;
  ...
  it("keeps caller-provided functional Docker build packages", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-build-external-package-"));
...
printf fixture >"$external_dir/daisyclaw-current.tgz"
DAISYCLAW_CURRENT_PACKAGE_TGZ="$external_dir/daisyclaw-current.tgz"
export DAISYCLAW_CURRENT_PACKAGE_TGZ
...
test -f "$TMPDIR/build-context-seen"
test -f "$DAISYCLAW_CURRENT_PACKAGE_TGZ"
leftovers="$(find "$TMPDIR" -maxdepth 1 -name 'daisyclaw-docker-e2e-package-context.*' -print)"
if [[ -n "$leftovers" ]]; then
...
`;
  ...
  it("cleans generated package mounts after harness Docker runs", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-package-mount-cleanup-"));
...
test ! -e "$pack_dir"
test -z "$(find "$TMPDIR" -maxdepth 1 -name 'daisyclaw-docker-e2e-container.*' -print)"

external_dir="$TMPDIR/external-package"
mkdir -p "$external_dir"
printf fixture >"$external_dir/daisyclaw-current.tgz"
docker_e2e_package_mount_args "$external_dir/daisyclaw-current.tgz"
...
test -f "$external_dir/daisyclaw-current.tgz"
`;
  ...
  it("propagates shared E2E command timeouts into package-backed containers", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-package-timeout-env-"));
...
package="$TMPDIR/daisyclaw-current.tgz"
printf fixture >"$package"
export DAISYCLAW_E2E_NPM_INSTALL_TIMEOUT=42s
export DAISYCLAW_E2E_COMMAND_TIMEOUT=23s
docker_e2e_package_mount_args "$package"
printf "%s\\n" "\${DOCKER_E2E_PACKAGE_ARGS[@]}" >"$TMPDIR/package-args"

grep -qx -- "-e" "$TMPDIR/package-args"
grep -qx -- "DAISYCLAW_CURRENT_PACKAGE_TGZ=/tmp/daisyclaw-current.tgz" "$TMPDIR/package-args"
grep -qx -- "DAISYCLAW_E2E_NPM_INSTALL_TIMEOUT=42s" "$TMPDIR/package-args"
grep -qx -- "DAISYCLAW_E2E_COMMAND_TIMEOUT=23s" "$TMPDIR/package-args"
`;
  ...
  it("passes plugin lifecycle sampler timeout overrides into Docker", () => {
    ...
    expect(runner).toContain('if [ -n "${DAISYCLAW_PLUGIN_LIFECYCLE_PHASE_TIMEOUT_MS:-}" ]; then');
    expect(runner).toContain(
      'DOCKER_ENV_ARGS+=(-e "DAISYCLAW_PLUGIN_LIFECYCLE_PHASE_TIMEOUT_MS=$DAISYCLAW_PLUGIN_LIFECYCLE_PHASE_TIMEOUT_MS")',
    );
    expect(runner).toContain(
      'if [ -n "${DAISYCLAW_PLUGIN_LIFECYCLE_TIMEOUT_KILL_GRACE_MS:-}" ]; then',
    );
    expect(runner).toContain(
      'DOCKER_ENV_ARGS+=(-e "DAISYCLAW_PLUGIN_LIFECYCLE_TIMEOUT_KILL_GRACE_MS=$DAISYCLAW_PLUGIN_LIFECYCLE_TIMEOUT_KILL_GRACE_MS")',
    );
    expect(runner).toContain('if [ -n "${DAISYCLAW_PLUGIN_LIFECYCLE_MAX_RSS_KB:-}" ]; then');
    expect(runner).toContain(
      'DOCKER_ENV_ARGS+=(-e "DAISYCLAW_PLUGIN_LIFECYCLE_MAX_RSS_KB=$DAISYCLAW_PLUGIN_LIFECYCLE_MAX_RSS_KB")',
    );
    expect(runner).toContain('if [ -n "${DAISYCLAW_PLUGIN_LIFECYCLE_MAX_WALL_MS:-}" ]; then');
    expect(runner).toContain(
      'DOCKER_ENV_ARGS+=(-e "DAISYCLAW_PLUGIN_LIFECYCLE_MAX_WALL_MS=$DAISYCLAW_PLUGIN_LIFECYCLE_MAX_WALL_MS")',
    );
    expect(runner).toContain('if [ -n "${DAISYCLAW_PLUGIN_LIFECYCLE_MAX_CPU_CORE_RATIO:-}" ]; then');
    expect(runner).toContain(
      'DOCKER_ENV_ARGS+=(-e "DAISYCLAW_PLUGIN_LIFECYCLE_MAX_CPU_CORE_RATIO=$DAISYCLAW_PLUGIN_LIFECYCLE_MAX_CPU_CORE_RATIO")',
    );
  ...
  it("cleans plugin lifecycle matrix temp roots on exit", () => {
    ...
    expect(sweep).toContain("daisyclaw_plugins_cleanup_fixture_servers");
    expect(sweep).toContain(
      'resource_dir="$(mktemp -d "/tmp/daisyclaw-plugin-lifecycle-matrix.XXXXXX")"',
    );
    ...
    expect(sweep).not.toContain('resource_dir="/tmp/daisyclaw-plugin-lifecycle-matrix"');
  ...
  it("wraps direct Docker E2E npm installs with the shared timeout helper", () => {
    ...
    expect(multiNode).toContain(
      'daisyclaw_e2e_install_package "$ARTIFACTS/install-a.log" "DaisyClaw package under node-A prefix" "$NPM_PREFIX_A"',
    );
    expect(updateChannel).toContain(
      'daisyclaw_e2e_maybe_timeout "${DAISYCLAW_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install --omit=optional --no-fund --no-audit',
    );
    expect(updateChannel).toContain(
      'daisyclaw_e2e_maybe_timeout "${DAISYCLAW_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix /tmp/npm-prefix --omit=optional "$pkg_tgz_path"',
    );
    expect(updateChannel).toContain("daisyclaw_e2e_print_log /tmp/daisyclaw-git-install.log");
    expect(updateChannel).toContain('daisyclaw_e2e_print_log "$package_install_log"');
    expect(updateChannel).not.toContain("cat /tmp/daisyclaw-git-install.log");
    expect(updateChannel).not.toContain('cat "$package_install_log"');
    expect(doctorSwitch).toContain(
      'daisyclaw_e2e_maybe_timeout "${DAISYCLAW_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install --omit=optional --no-fund --no-audit',
    );
    expect(doctorSwitch).toContain(
      'daisyclaw_e2e_maybe_timeout "${DAISYCLAW_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix /tmp/npm-prefix --omit=optional "$package_tgz"',
    );
    for (const script of [releaseUpgrade, upgradeSurvivor, pluginCorrupt]) {
      expect(script).toContain(
        'daisyclaw_e2e_maybe_timeout "${DAISYCLAW_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g',
      );
    }
  });
  ...
  it("keeps upgrade survivor mutable state off the host-mounted artifact tree", () => {
    ...
    for (const script of [runner, publishedRunner]) {
      expect(script).toContain("daisyclaw-upgrade-survivor-runtime");
      expect(script).toContain("DAISYCLAW_UPGRADE_SURVIVOR_TMPDIR");
      expect(script).toContain("DAISYCLAW_UPGRADE_SURVIVOR_TEST_STATE_TMPDIR");
      expect(script).toContain(
        'export npm_config_cache="${DAISYCLAW_UPGRADE_SURVIVOR_NPM_CACHE:-$DAISYCLAW_UPGRADE_SURVIVOR_RUNTIME_ROOT/npm-cache}"',
      );
      ...
      expect(script).not.toContain('export TMPDIR="$DAISYCLAW_UPGRADE_SURVIVOR_ARTIFACT_ROOT/tmp"');
      expect(script).not.toContain('export npm_config_cache="$ARTIFACT_ROOT/npm-cache"');
      expect(script).not.toContain(
        'export npm_config_cache="$DAISYCLAW_UPGRADE_SURVIVOR_ARTIFACT_ROOT/npm-cache"',
      );
    }
  });

  it("wraps package-backed scenario DaisyClaw CLI calls with the shared timeout helper", () => {
    const paths = [
    ...
    for (const path of paths) {
      ...
      expect(script, path).toContain("daisyclaw_e2e_enable_daisyclaw_cli_timeout");
    }
    expect(readFileSync(RELEASE_UPGRADE_USER_JOURNEY_SCENARIO_PATH, "utf8")).toContain(
      'daisyclaw_e2e_run_command node "$baseline_entry" onboard',
    );
  });
  ...
  it("keeps append-only mock E2E state under per-run scratch roots", () => {
    const scripts = [
      {
        ...
        scratch:
          'scenario_tmp="$(mktemp -d "${TMPDIR:-/tmp}/daisyclaw-release-typed-onboarding.XXXXXX")"',
        ...
        removed: [
          "/tmp/daisyclaw-release-typed-onboarding-openai.jsonl",
          "/tmp/daisyclaw-release-typed-onboarding-install.log",
          "/tmp/daisyclaw-release-typed-onboarding.log",
          "/tmp/daisyclaw-release-typed-onboarding-openai.log",
          "/tmp/daisyclaw-release-typed-onboarding-agent.log",
          'mktemp -d "/tmp/daisyclaw-release-typed-onboarding.XXXXXX"',
        ],
      },
      {
        path: RELEASE_USER_JOURNEY_SCENARIO_PATH,
        scratch:
          'scenario_tmp="$(mktemp -d "${TMPDIR:-/tmp}/daisyclaw-release-user-journey.XXXXXX")"',
        logDir: 'LOG_DIR="$scenario_tmp/logs"',
        requestLog: 'MOCK_REQUEST_LOG="$scenario_tmp/openai-requests.jsonl"',
        ...
        removed: [
          "/tmp/daisyclaw-release-user-journey-openai.jsonl",
          "/tmp/daisyclaw-release-user-journey-clickclack.json",
          "/tmp/daisyclaw-release-user-journey-install.log",
          "/tmp/daisyclaw-release-user-journey-onboard.log",
          "/tmp/daisyclaw-release-user-journey-agent.log",
          "/tmp/daisyclaw-release-user-journey-plugin-a-install-path.txt",
          "/tmp/daisyclaw-release-user-journey-plugin-a-source-path.txt",
          'mktemp -d "/tmp/daisyclaw-release-journey-plugin-a.XXXXXX"',
          'mktemp -d "/tmp/daisyclaw-release-journey-plugin-b.XXXXXX"',
        ],
      },
      {
        path: RELEASE_UPGRADE_USER_JOURNEY_SCENARIO_PATH,
        scratch:
          'scenario_tmp="$(mktemp -d "${TMPDIR:-/tmp}/daisyclaw-release-upgrade-user-journey.XXXXXX")"',
        logDir: 'LOG_DIR="$scenario_tmp/logs"',
        requestLog: 'MOCK_REQUEST_LOG="$scenario_tmp/openai-requests.jsonl"',
        extraState: 'CLICKCLACK_STATE="$scenario_tmp/clickclack.json"',
        expectedPaths: [
        ...
        removed: [
          "/tmp/daisyclaw-release-upgrade-user-journey-openai.jsonl",
          "/tmp/daisyclaw-release-upgrade-user-journey-clickclack.json",
          "/tmp/daisyclaw-release-upgrade-baseline-install.log",
          "/tmp/daisyclaw-release-upgrade-candidate-install.log",
          "/tmp/daisyclaw-release-upgrade-onboard.log",
          "/tmp/daisyclaw-release-upgrade-agent.log",
          'mktemp -d "/tmp/daisyclaw-release-upgrade-plugin.XXXXXX"',
        ],
      },
      {
        path: NPM_ONBOARD_CHANNEL_AGENT_DOCKER_E2E_PATH,
        scratch:
          'scenario_tmp="$(mktemp -d "${TMPDIR:-/tmp}/daisyclaw-npm-onboard-channel-agent.XXXXXX")"',
        requestLog: 'MOCK_REQUEST_LOG="$scenario_tmp/mock-openai-requests.jsonl"',
        removed: ["/tmp/daisyclaw-mock-openai-requests.jsonl"],
      },
    ];
    ...
    for (const {
      ...
      expect(script, path).not.toMatch(/\/tmp\/daisyclaw-release-[\w-]+\.(?:log|json|err|txt)/u);
    }
  });
  ...
  it("keeps multi-node update Docker artifacts isolated by default", () => {
    ...
    expect(multiNode).toContain(
      'RUN_ID="${DAISYCLAW_MULTI_NODE_RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)-$$}"',
    );
    expect(multiNode).toContain(
      'ARTIFACT_DIR="${DAISYCLAW_MULTI_NODE_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/multi-node-update/$RUN_ID}"',
    );
    expect(multiNode).toContain('-v "$ARTIFACT_DIR:/tmp/artifacts"');
    expect(multiNode).not.toContain(
      'ARTIFACT_DIR="${DAISYCLAW_MULTI_NODE_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/multi-node-update}"',
    );
  });

  it("bounds upgrade survivor foreground DaisyClaw CLI calls", () => {
    const runner = readFileSync(UPGRADE_SURVIVOR_DOCKER_E2E_PATH, "utf8");
    const publishedRunner = readFileSync(UPGRADE_SURVIVOR_RUN_SCRIPT, "utf8");
    const updateRestartAuth = readFileSync(UPGRADE_SURVIVOR_UPDATE_RESTART_AUTH_PATH, "utf8");

    expect(runner).toContain(
      'COMMAND_TIMEOUT="${DAISYCLAW_UPGRADE_SURVIVOR_COMMAND_TIMEOUT:-900s}"',
    );
    expect(runner).toContain('-e DAISYCLAW_UPGRADE_SURVIVOR_COMMAND_TIMEOUT="$COMMAND_TIMEOUT"');
    expect(runner).toContain(
      'command_timeout="${DAISYCLAW_UPGRADE_SURVIVOR_COMMAND_TIMEOUT:-900s}"',
    );
    expect(runner).toContain(
      'daisyclaw_e2e_maybe_timeout "$command_timeout" env -u DAISYCLAW_GATEWAY_TOKEN',
    );
    expect(runner).toContain(
      'daisyclaw_e2e_maybe_timeout "$command_timeout" daisyclaw doctor --fix --non-interactive',
    );
    expect(runner).toContain(
      'daisyclaw_e2e_maybe_timeout "$command_timeout" daisyclaw config validate',
    );
    expect(runner).toContain(
      'daisyclaw_e2e_maybe_timeout "$command_timeout" daisyclaw gateway status',
    );
    expect(runner).toContain(
      'daisyclaw gateway --port "$PORT" --bind loopback --allow-unconfigured',
    );
    ...
    expect(publishedRunner).toContain(
      'daisyclaw_e2e_maybe_timeout "$COMMAND_TIMEOUT" daisyclaw --version',
    );
    expect(publishedRunner).toContain(
      'daisyclaw_e2e_maybe_timeout "$COMMAND_TIMEOUT" daisyclaw config validate >"$BASELINE_CONFIG_VALIDATE_LOG"',
    );
    expect(publishedRunner).toContain(
      'daisyclaw_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${update_env[@]}" daisyclaw',
    );
    expect(publishedRunner).toContain(
      'daisyclaw_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${root_cli_env[@]}" daisyclaw',
    );
    expect(publishedRunner).toContain(
      'daisyclaw_e2e_maybe_timeout "$COMMAND_TIMEOUT" daisyclaw doctor --fix --non-interactive',
    );
    ...
    expect(publishedRunner).toContain('daisyclaw gateway --port "$port" --bind loopback');
    expect(publishedRunner).toContain("start_gateway legacy-ready-log-ok");
    expect(publishedRunner).toContain(
      'daisyclaw_e2e_wait_gateway_ready "$gateway_pid" "$GATEWAY_LOG" 360 "$port" "${1:-strict}"',
    );
    ...
    expect(updateRestartAuth).toContain('daisyclaw gateway --port "$port" --bind loopback');
    expect(updateRestartAuth).toContain(
      'daisyclaw_e2e_wait_gateway_ready "$gateway_pid" "$log_file" 360 "$port"',
    );
  });
  ...
  it("bounds upgrade survivor failure log diagnostics", () => {
    ...
    expect(runner).toContain("daisyclaw_e2e_print_log /tmp/daisyclaw-upgrade-survivor-update.err");
    expect(runner).toContain("daisyclaw_e2e_print_log /tmp/daisyclaw-upgrade-survivor-update.json");
    expect(runner).toContain("daisyclaw_e2e_print_log /tmp/daisyclaw-upgrade-survivor-doctor.log");
    expect(runner).toContain("daisyclaw_e2e_print_log /tmp/daisyclaw-upgrade-survivor-status.err");
    expect(runner).toContain("daisyclaw_e2e_print_log /tmp/daisyclaw-upgrade-survivor-status.json");
    expect(runner).toContain('daisyclaw_e2e_print_log "$GATEWAY_LOG"');
    expect(runner).toContain('daisyclaw_e2e_print_log "$SYSTEMCTL_SHIM_DAEMON_LOG"');
    expect(runner).toContain('daisyclaw_e2e_print_log "$log_file"');
    expect(runner).not.toContain("cat /tmp/daisyclaw-upgrade-survivor-update.err");
    expect(runner).not.toContain("cat /tmp/daisyclaw-upgrade-survivor-update.json");
    expect(runner).not.toContain("cat /tmp/daisyclaw-upgrade-survivor-doctor.log");
    expect(runner).not.toContain("cat /tmp/daisyclaw-upgrade-survivor-status.err");
    expect(runner).not.toContain("cat /tmp/daisyclaw-upgrade-survivor-status.json");
    ...
    expect(publishedRunner).toContain('daisyclaw_e2e_print_log "$BASELINE_INSTALL_LOG"');
    expect(publishedRunner).toContain('daisyclaw_e2e_print_log "$BASELINE_CONFIG_VALIDATE_LOG"');
    expect(publishedRunner).toContain('daisyclaw_e2e_print_log "$BASELINE_SERVICE_INSTALL_ERR"');
    expect(publishedRunner).toContain('daisyclaw_e2e_print_log "$BASELINE_SERVICE_INSTALL_JSON"');
    expect(publishedRunner).toContain('daisyclaw_e2e_print_log "$UPDATE_ERR"');
    expect(publishedRunner).toContain('daisyclaw_e2e_print_log "$UPDATE_JSON"');
    expect(publishedRunner).toContain('daisyclaw_e2e_print_log "$DOCTOR_LOG"');
    expect(publishedRunner).toContain('daisyclaw_e2e_print_log "$GATEWAY_LOG"');
    expect(publishedRunner).toContain('daisyclaw_e2e_print_log "$STATUS_ERR"');
    expect(publishedRunner).toContain('daisyclaw_e2e_print_log "$STATUS_JSON"');
    expect(publishedRunner).toContain('daisyclaw_e2e_print_log "$log_file"');
  ...
  it("keeps the harness run wrapper available with pre-sourced Docker command helpers", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-package-helper-guard-"));
...
`;
  ...
  it("forwards harness stdin to backgrounded Docker runs", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-harness-stdin-"));
...
`;
  ...
  it("preserves caller-owned file descriptors around harness runs", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-harness-fd-"));
    try {
      const rootDir = process.cwd();
...
`;
  ...
  it("bounds Codex npm plugin live assertion file and transcript reads", () => {
    ...
    expect(assertions).toContain("DAISYCLAW_CODEX_NPM_PLUGIN_ASSERT_MAX_TEXT_FILE_BYTES");
    expect(assertions).toContain("DAISYCLAW_CODEX_NPM_PLUGIN_ASSERT_MAX_ERROR_TAIL_BYTES");
    expect(assertions).toContain("DAISYCLAW_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_FILES");
    expect(assertions).toContain("DAISYCLAW_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_WALK_ENTRIES");
    expect(assertions).toContain("DAISYCLAW_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_SCAN_BYTES");
    ...
    expect(runner).toContain("docker_e2e_print_log /tmp/daisyclaw-codex-plugin-pack.log");
    expect(runner).not.toContain("cat /tmp/daisyclaw-codex-plugin-pack.log");
    expect(runner).toContain("tail -n 120 /tmp/daisyclaw-codex-agent-after-uninstall.err");
    expect(runner).not.toContain("cat /tmp/daisyclaw-codex-agent-after-uninstall.err");
  });
  ...
  it("threads the live plugin tool output cap into the Docker harness", () => {
    ...
    expect(runner).toContain(
      'AGENT_OUTPUT_MAX_BYTES="${DAISYCLAW_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_MAX_BYTES:-1048576}"',
    );
    expect(runner).toContain(
      '-e "DAISYCLAW_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_MAX_BYTES=$AGENT_OUTPUT_MAX_BYTES"',
    );
    expect(runner).toContain("DAISYCLAW_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_DUMP_BYTES");
    expect(runner).toContain('tail -c "$agent_output_dump_bytes" /tmp/daisyclaw-agent.json');
    const dumpLogsStart = runner.indexOf("daisyclaw_e2e_dump_logs \\");
    const dumpLogsEnd = runner.indexOf("\n}", dumpLogsStart);
    expect(runner.slice(dumpLogsStart, dumpLogsEnd)).not.toContain("/tmp/daisyclaw-agent.json");
  });
  ...
  it("prints plugins Docker E2E logs on successful runs", () => {
    ...
    expect(helper).toContain("DAISYCLAW_DOCKER_E2E_LOG_HEARTBEAT_SECONDS");
    expect(readFileSync("scripts/lib/docker-e2e-logs.sh", "utf8")).toContain(
      "DAISYCLAW_DOCKER_E2E_LOG_PRINT_BYTES",
    );
  ...
  it("bounds printed Docker E2E logs to the configured tail", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-e2e-log-print-tail-"));
...
TMPDIR=${shellQuote(workDir)}
export ROOT_DIR TMPDIR
export DAISYCLAW_DOCKER_E2E_LOG_PRINT_BYTES=64
...
`;
  ...
  it("prints heartbeat progress for long successful Docker E2E log captures", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-e2e-log-heartbeat-"));
...
`;
  ...
  it("cleans the heartbeat command when the wrapper is terminated", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-e2e-log-term-cleanup-"));
...
`;
  ...
  it("cleans harness containers when heartbeat-wrapped Docker runs are terminated", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-e2e-harness-term-cleanup-"));
...
`;
  ...
  it("does not delay fast successful Docker E2E log captures until the next heartbeat", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-e2e-log-fast-heartbeat-"));
...
`;
  ...
  it("normalizes zero-padded Docker E2E log heartbeat intervals", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-e2e-log-zero-heartbeat-"));
...
`;
  ...
  it("normalizes zero-padded Docker E2E stats heartbeat intervals", () => {
    const workDir = mkdtempSync(join(tmpdir(), "daisyclaw-docker-e2e-stats-zero-heartbeat-"));
...
`;
  ...
  it("keeps onboarding Docker E2E resource-guarded", () => {
    ...
    expect(runner).toContain("DAISYCLAW_ONBOARD_MAX_MEMORY_MIB");
    expect(runner).toContain("DAISYCLAW_ONBOARD_MAX_CPU_PERCENT");
    expect(runner).toContain(
      'COMMAND_TIMEOUT="${DAISYCLAW_ONBOARD_COMMAND_TIMEOUT:-${DAISYCLAW_E2E_COMMAND_TIMEOUT:-300s}}"',
    );
    expect(runner).toContain('-e "DAISYCLAW_E2E_COMMAND_TIMEOUT=$COMMAND_TIMEOUT"');
  ...
  it("cleans resource-sampled Docker E2E temp logs on every exit path", () => {
    for (const { path, label } of [
      ...
      expect(runner, path).toContain('DOCKER_RUN_TIMEOUT="${DAISYCLAW_');
  ...
  it("keeps Open WebUI Docker E2E resource-guarded", () => {
    ...
    expect(runner).toContain("DAISYCLAW_OPENWEBUI_MAX_MEMORY_MIB");
    expect(runner).toContain("DAISYCLAW_OPENWEBUI_MAX_CPU_PERCENT");
  ...
  it("forwards every kitchen-sink RPC runtime env knob into Docker", () => {
    ...
    const consumed = new Set(
      [...walk.matchAll(/\b(?:env|process\.env)\.(DAISYCLAW_KITCHEN_SINK_[A-Z0-9_]+)/gu)].map(
        (match) => match[1],
      ),
    );
    const forwarded = new Set(
      [...runner.matchAll(/\b(DAISYCLAW_KITCHEN_SINK_[A-Z0-9_]+)\b/gu)].map((match) => match[1]),
    );
  ...
  it("bounds kitchen-sink plugin CLI commands inside the Docker sweep", () => {
    ...
    expect(runner).toContain(
      'KITCHEN_SINK_CLI_TIMEOUT="${DAISYCLAW_KITCHEN_SINK_PLUGIN_CLI_TIMEOUT:-${KITCHEN_SINK_CLI_TIMEOUT:-180s}}"',
    );
    ...
    expect(sweep).toContain("run_kitchen_sink_daisyclaw_logged()");
    expect(sweep).toContain("run_kitchen_sink_daisyclaw_capture()");
    expect(sweep).toContain(
      'daisyclaw_e2e_maybe_timeout "$KITCHEN_SINK_CLI_TIMEOUT" node "$DAISYCLAW_ENTRY" "$@" >"$log_file" 2>&1',
    );
    expect(sweep).toContain('local log_file="${KITCHEN_SINK_TMP_DIR}/${safe_label}.log"');
    for (const line of sweep.split("\n")) {
      if (!line.includes('node "$DAISYCLAW_ENTRY" plugins')) {
        continue;
      }

      expect(line).toContain("daisyclaw_e2e_maybe_timeout");
    }
  });
  ...
  it("requires TCP readiness for the gateway network runner", () => {
    ...
    expect(runner).toContain("daisyclaw_e2e_probe_tcp 127.0.0.1 $PORT");
    expect(runner).not.toMatch(/daisyclaw_e2e_probe_tcp[^\n]*\|\|[^\n]*gateway-net-e2e\.log/u);
  });
  ...
  it("preserves pnpm lookup paths for scheduled Docker child lanes", () => {
    ...
    expect(scheduler).toContain("DAISYCLAW_DOCKER_ALL_PNPM_COMMAND");
  });
  ...
  it("runs release installer E2E against the npm beta tag", () => {
    ...
    expect(scenarios).toContain(
      '"DAISYCLAW_INSTALL_TAG=beta DAISYCLAW_E2E_MODELS=openai DAISYCLAW_INSTALL_E2E_IMAGE=daisyclaw-install-e2e-openai:local DAISYCLAW_INSTALL_E2E_AGENT_TOOL_SMOKE=0 DAISYCLAW_INSTALL_E2E_OPENAI_MODEL=openai/gpt-5.4-mini DAISYCLAW_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS=120 DAISYCLAW_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS=120 pnpm test:install:e2e"',
    );
    expect(scenarios).toContain(
      '"DAISYCLAW_INSTALL_TAG=beta DAISYCLAW_E2E_MODELS=anthropic DAISYCLAW_INSTALL_E2E_IMAGE=daisyclaw-install-e2e-anthropic:local pnpm test:install:e2e"',
    );
    expect(scenarios).toContain(
      '"DAISYCLAW_OPENWEBUI_MODEL=openai/gpt-5.4-mini DAISYCLAW_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS=300 DAISYCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:openwebui"',
    );
    expect(scenarios).not.toContain("OPENWEBUI_SMOKE_MODE=models");
    expect(openWebUiRunner).toContain(
      'SMOKE_MODE="${OPENWEBUI_SMOKE_MODE:-${DAISYCLAW_OPENWEBUI_SMOKE_MODE:-chat}}"',
    );
  ...
  it("times and parallelizes release installer E2E agent turns after gateway startup", () => {
    ...
    expect(runner).toContain(
      'AGENT_TURNS_PARALLEL="${DAISYCLAW_INSTALL_E2E_AGENT_TURNS_PARALLEL:-1}"',
    );
    ...
    expect(wrapper).toContain("DAISYCLAW_INSTALL_E2E_AGENT_TURNS_PARALLEL");
    expect(wrapper).toContain("DAISYCLAW_INSTALL_E2E_AGENT_TOOL_SMOKE");
    expect(wrapper).toContain("DAISYCLAW_INSTALL_E2E_OPENAI_MODEL");
    expect(wrapper).toContain("DAISYCLAW_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS");
    expect(wrapper).toContain("DAISYCLAW_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS:-300");
    expect(wrapper).toContain("DAISYCLAW_INSTALL_E2E_PROFILE_FILE");
    expect(wrapper).toContain("DAISYCLAW_PROFILE_FILE");
    expect(wrapper).toContain("DAISYCLAW_TESTBOX_PROFILE_FILE");
    ...
    expect(runner).toContain("DAISYCLAW_INSTALL_E2E_OPENAI_MODEL");
    expect(runner).toContain("DAISYCLAW_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS");
    expect(runner).toContain(
      'AGENT_TURN_TIMEOUT_SECONDS="${DAISYCLAW_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS:-300}"',
    );
  });
  ...
  it("keeps package acceptance plugin coverage offline-capable", () => {
    ...
    expect(scenarios).toContain("DAISYCLAW_PLUGINS_E2E_CLAWHUB=0");
  });
  ...
  it("fails the multi-node update probe on update or restart regressions", () => {
    ...
    expect(runner).toContain("DAISYCLAW_NO_RESPAWN=1");
  ...
  it("caps package acceptance legacy compatibility at 2026.4.25", () => {
    ...
    expect(scripts.join("\n")).toContain("DAISYCLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT");
    expect(scripts.join("\n")).toContain(
  ...
  it("routes doctor install switch commands through the E2E timeout helper", () => {
    ...
    expect(scenario).toContain(
      'command_timeout="${DAISYCLAW_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-900s}"',
    );
    expect(scenario).toContain(
      'daisyclaw_e2e_maybe_timeout "$command_timeout" bash -c "$install_cmd"',
    );
    expect(scenario).toContain(
      'daisyclaw_e2e_maybe_timeout "$command_timeout" bash -c "$doctor_cmd"',
    );
    expect(scenario).toContain(
      'daisyclaw_e2e_maybe_timeout "$command_timeout" "$npm_bin" gateway install --wrapper "$wrapper" --force',
    );
    expect(scenario).toContain(
      'daisyclaw_e2e_maybe_timeout "$command_timeout" node "$git_cli" doctor --repair --force --yes',
    );
  ...
  it("bounds doctor install switch command log diagnostics", () => {
    ...
    expect(scenario).toContain('daisyclaw_e2e_print_log "$npm_log"');
    expect(scenario).toContain('daisyclaw_e2e_print_log "$install_log"');
    expect(scenario).toContain('daisyclaw_e2e_print_log "$doctor_log"');
    expect(scenario).toContain('daisyclaw_e2e_print_log "$reinstall_log"');
    expect(scenario).toContain('daisyclaw_e2e_print_log "$env_repair_log"');
    expect(scenario).toContain('daisyclaw_e2e_print_log "$clear_log"');
  ...
  it("prepares pnpm workspace package fixtures without package dependencies", () => {
    const root = mkdtempSync(join(tmpdir(), "daisyclaw-update-channel-fixture-"));
    try {
      mkdirSync(join(root, "patches"));
      writeFileSync(
        join(root, "package.json"),
        `${JSON.stringify({ name: "daisyclaw", version: "2026.5.6", scripts: {} }, null, 2)}\n`,
        "utf8",
      );
      writeFileSync(
  ...
  it("keeps bundled plugin install/uninstall sweep chunkable", () => {
    ...
    expect(runner).toContain("DAISYCLAW_BUNDLED_PLUGIN_SWEEP_TOTAL");
    expect(runner).toContain("DAISYCLAW_BUNDLED_PLUGIN_SWEEP_INDEX");
    expect(runner).toContain("DAISYCLAW_BUNDLED_PLUGIN_SWEEP_COMMAND_TIMEOUT");
    expect(runner).toContain("DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_READY_MS");
    expect(runner).toContain("DAISYCLAW_PLUGIN_LIFECYCLE_TRACE");
    ...
    expect(probe).toContain('"daisyclaw.plugin.json"');
    expect(runtimeSmoke).toContain(
      'readPositiveIntEnv("DAISYCLAW_BUNDLED_PLUGIN_RUNTIME_READY_MS", 900000)',
    );
    expect(sweep).toContain("read -r plugin_id plugin_dir requires_config");
    expect(sweep).toContain('node "$DAISYCLAW_ENTRY" plugins install "$plugin_id"');
    expect(sweep).toContain('node "$DAISYCLAW_ENTRY" plugins uninstall "$plugin_id" --force');
  ...
  it("passes installer tag env to bash, not curl", () => {
    ...
    expect(runner).toContain('curl -fsSL "$INSTALL_URL" | DAISYCLAW_BETA=1 bash');
    expect(runner).toContain('curl -fsSL "$INSTALL_URL" | DAISYCLAW_VERSION="$INSTALL_TAG" bash');
    expect(runner).not.toContain('DAISYCLAW_BETA=1 curl -fsSL "$INSTALL_URL" | bash');
    expect(runner).not.toContain(
      'DAISYCLAW_VERSION="$INSTALL_TAG" curl -fsSL "$INSTALL_URL" | bash',
    );
  });
  ...
  it("bounds installer E2E session transcript tool scans", () => {
    ...
    expect(helper).toContain("DAISYCLAW_INSTALL_E2E_SESSION_SCAN_BYTES");
    expect(helper).toContain("DAISYCLAW_INSTALL_E2E_SESSION_LINE_BYTES");
    expect(helper).toContain("DAISYCLAW_INSTALL_E2E_SESSION_SCAN_DEPTH");
    expect(helper).toContain("DAISYCLAW_INSTALL_E2E_SESSION_SCAN_NODES");
  ...
  it("cleans OpenAI web search smoke processes through the E2E helpers", () => {
    ...
    expect(scenario).toContain('daisyclaw_e2e_terminate_gateways "${gateway_pid:-}"');
    expect(scenario).toContain('daisyclaw_e2e_stop_process "${mock_pid:-}"');
    expect(scenario).toContain(
      'gateway_pid="$(daisyclaw_e2e_start_gateway "$entry" "$PORT" "$GATEWAY_LOG")"',
    );
    expect(scenario).toContain('daisyclaw_e2e_wait_mock_openai "$MOCK_PORT"');
    expect(scenario).toContain(
  ...
  it("keeps OpenAI web search smoke logs isolated per run", () => {
    ...
    expect(scenario).toContain('daisyclaw_e2e_print_log "$file"');
    expect(scenario).toContain('rm -rf "$scenario_tmp"');
    expect(scenario).not.toContain("sed -n '1,260p'");
    expect(scenario).not.toContain("/tmp/daisyclaw-openai-web-search-minimal-requests.jsonl");
    expect(scenario).not.toContain("/tmp/daisyclaw-openai-web-search-minimal-client-success.log");
    expect(scenario).not.toContain("/tmp/daisyclaw-openai-web-search-minimal-client-reject.log");
  });
  ...
  it("keeps ClawHub plugin Docker smoke hermetic by default", () => {
    ...
    expect(runner).toContain("DAISYCLAW_PLUGINS_E2E_LIVE_CLAWHUB");
    expect(sweep).toContain("scripts/e2e/lib/plugins/clawhub.sh");
    expect(clawhub).toContain("start_clawhub_fixture_server()");
    expect(clawhub).toContain('DAISYCLAW_CLAWHUB_URL="http://127.0.0.1:');
    expect(clawhub).toContain("DAISYCLAW_PLUGINS_E2E_LIVE_CLAWHUB");
    expect(clawhub).toContain("DAISYCLAW_PLUGINS_E2E_LIVE_NPM_REGISTRY");
    expect(clawhub).toContain("live ClawHub can rate-limit CI");
    expect(clawhub).toContain('[[ -n "${DAISYCLAW_CLAWHUB_URL:-}" || -n "${CLAWHUB_URL:-}" ]]');
    expect(clawhub).toContain("Ignoring ambient ClawHub URL for fixture-mode plugin E2E");
    expect(clawhub).toContain("unset DAISYCLAW_CLAWHUB_URL CLAWHUB_URL");
  });
  ...
  it("keeps the plugin binding command escape Docker smoke focused", () => {
    ...
    expect(runner).toContain(
      'DOCKER_RUN_TIMEOUT="${DAISYCLAW_PLUGIN_BINDING_COMMAND_ESCAPE_DOCKER_RUN_TIMEOUT:-900s}"',
    );
    expect(runner).toContain(
    ...
    expect(dockerfile).toContain("DAISYCLAW_DISABLE_BUNDLED_PLUGIN_POSTINSTALL=1");
    expect(dockerfile).toContain(
      "pnpm install --frozen-lockfile --ignore-scripts --filter daisyclaw",
    );
  });
  ...
  it("covers plugin CLI sources in the Docker plugin sweep", () => {
    ...
    expect(sweep).toContain('DAISYCLAW_PLUGINS_CLI_TIMEOUT="${DAISYCLAW_PLUGINS_CLI_TIMEOUT:-180s}"');
    expect(sweep).toContain(
      'run_logged "$label" daisyclaw_e2e_maybe_timeout "$DAISYCLAW_PLUGINS_CLI_TIMEOUT" node "$DAISYCLAW_ENTRY" "$@"',
    );
    expect(sweep).toContain("run_plugins_daisyclaw_capture()");
    expect(sweep).toContain(
      'daisyclaw_e2e_maybe_timeout "$DAISYCLAW_PLUGINS_CLI_TIMEOUT" node "$DAISYCLAW_ENTRY" "$@" >"$output_file"',
    );
    expect(sweep).not.toContain('run_logged install-npm node "$DAISYCLAW_ENTRY"');
    for (const [path, script] of [
      ...
      const unboundedPluginCliLines = script
        .split("\n")
        .filter((line) => line.includes('node "$DAISYCLAW_ENTRY" plugins'))
        .filter((line) => !line.includes("daisyclaw_e2e_maybe_timeout"));
    ...
    expect(sweep).toContain('plugins install "npm:@daisyclaw/demo-plugin-npm@0.0.1"');
    ...
    expect(clawhub).toContain("run_plugins_daisyclaw_logged install-clawhub");
    expect(clawhub).toContain('daisyclaw_e2e_maybe_timeout "$DAISYCLAW_PLUGINS_CLI_TIMEOUT"');
    expect(clawhub).toContain("clawhub:@daisyclaw/kitchen-sink");
