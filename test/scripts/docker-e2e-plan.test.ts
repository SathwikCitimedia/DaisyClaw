...
function publishedUpgradeSurvivorLane(
  ...
  return {
    command: `DAISYCLAW_UPGRADE_SURVIVOR_ARTIFACT_DIR="$PWD/.artifacts/upgrade-survivor/${name}" DAISYCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC='${baselineSpec}' ${
      scenario ? `DAISYCLAW_UPGRADE_SURVIVOR_SCENARIO='${scenario}' ` : ""
...
function updateMigrationLane(name: string, baselineSpec: string): ReturnType<typeof summarizeLane> {
  return {
    command: `DAISYCLAW_UPGRADE_SURVIVOR_ARTIFACT_DIR="$PWD/.artifacts/upgrade-survivor/${name}" DAISYCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC='${baselineSpec}' DAISYCLAW_UPGRADE_SURVIVOR_SCENARIO='plugin-deps-cleanup' DAISYCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-migration`,
    imageKind: "bare",
    live: false,
    name,
    resources: ["docker", "npm"],
...
function bundledPluginSweepLane(index: number): ReturnType<typeof summarizeLane> {
  return {
    command: `DAISYCLAW_BUNDLED_PLUGIN_SWEEP_TOTAL=24 DAISYCLAW_BUNDLED_PLUGIN_SWEEP_INDEX=${index} DAISYCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:bundled-plugin-install-uninstall`,
...
describe("scripts/lib/docker-e2e-plan", () => {
  ...
  it("splits release-path package and plugin chunks across shorter CI jobs", () => {
    ...
    expect(packageUpdateCore.lanes.map(summarizeLane)).toEqual([
      {
        command: "DAISYCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:npm-onboard-channel-agent",
        imageKind: "bare",
        live: false,
      ...
      {
        command: "DAISYCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:doctor-switch",
        imageKind: "bare",
        live: false,
        name: "doctor-switch",
        resources: ["docker", "npm"],
      ...
      {
        command: "DAISYCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-channel-switch",
        imageKind: "bare",
        live: false,
      ...
      {
        command: "DAISYCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:skill-install",
        imageKind: "bare",
        live: false,
        name: "skill-install",
        resources: ["docker", "npm"],
      ...
      {
        command: "DAISYCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:upgrade-survivor",
        imageKind: "bare",
        live: false,
      ...
      {
        command: "DAISYCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:published-upgrade-survivor",
        imageKind: "bare",
        live: false,
      ...
      {
        command: "DAISYCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:root-managed-vps-upgrade",
        imageKind: "bare",
        live: false,
      ...
      {
        command: "DAISYCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-restart-auth",
        imageKind: "bare",
        live: false,
    ...
    expect(pluginsRuntimeServices.lanes.map(summarizeLane)).toEqual([
      {
        command: "DAISYCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:cron-mcp-cleanup",
        imageKind: "functional",
        live: false,
      ...
      {
        command: "DAISYCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:kitchen-sink-rpc",
        imageKind: "functional",
        live: false,
      ...
      {
        command: "DAISYCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:openai-web-search-minimal",
        imageKind: "functional",
        live: false,
  ...
  it("expands the published upgrade survivor lane across deduped baselines", () => {
    const plan = planFor({
      ...
      upgradeSurvivorBaselines:
        "daisyclaw@2026.4.29 2026.4.23 daisyclaw@2026.4.23 daisyclaw@2026.3.13-1",
    });

    expect(plan.lanes.map(summarizeLane)).toEqual([
      publishedUpgradeSurvivorLane("published-upgrade-survivor-2026.4.29", "daisyclaw@2026.4.29"),
      publishedUpgradeSurvivorLane("published-upgrade-survivor-2026.4.23", "daisyclaw@2026.4.23"),
      publishedUpgradeSurvivorLane(
        "published-upgrade-survivor-2026.3.13-1",
        "daisyclaw@2026.3.13-1",
      ),
    ]);
  });
  ...
  it("expands update migration across baselines and cleanup scenarios", () => {
    ...
    expect(plan.lanes.map(summarizeLane)).toEqual([
      updateMigrationLane("update-migration-2026.4.29-plugin-deps-cleanup", "daisyclaw@2026.4.29"),
      updateMigrationLane("update-migration-2026.4.23-plugin-deps-cleanup", "daisyclaw@2026.4.23"),
    ]);
  });
  ...
  it("plans Codex harness Docker-all lanes for API-key Testbox auth", () => {
    for (const name of ["live-codex-harness", "live-codex-bind"]) {
      ...
      expect(lane.command, name).toContain("DAISYCLAW_LIVE_CODEX_HARNESS_AUTH=api-key");
  ...
  it("plans the Codex npm plugin live lane as package-backed OpenAI proof", () => {
    ...
    expect(plan.lanes.map(summarizeLane)).toEqual([
      {
        command:
          'DAISYCLAW_SKIP_DOCKER_BUILD=1 bash -c \'harness="${DAISYCLAW_DOCKER_E2E_TRUSTED_HARNESS_DIR:-.}"; DAISYCLAW_LIVE_DOCKER_REPO_ROOT="${DAISYCLAW_DOCKER_E2E_REPO_ROOT:-$PWD}" bash "$harness/scripts/e2e/codex-npm-plugin-live-docker.sh"\'',
        imageKind: "bare",
        live: true,
  ...
  it("plans the Codex on-demand onboarding lane as package-backed npm proof", () => {
    ...
    const lane = requireFirstLane(plan);
    expect(lane.command).toBe("DAISYCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:codex-on-demand");
  ...
  it("plans the Droid ACP bind live lane as Factory-auth proof", () => {
    ...
    const lane = requireFirstLane(plan);
    expect(lane.command).toBe(
      'DAISYCLAW_LIVE_ACP_BIND_AGENT=droid DAISYCLAW_LIVE_ACP_BIND_REQUIRE_TRANSCRIPT=1 DAISYCLAW_SKIP_DOCKER_BUILD=1 bash -c \'harness="${DAISYCLAW_DOCKER_E2E_TRUSTED_HARNESS_DIR:-.}"; DAISYCLAW_LIVE_DOCKER_REPO_ROOT="${DAISYCLAW_DOCKER_E2E_REPO_ROOT:-$PWD}" bash "$harness/scripts/test-live-acp-bind-docker.sh"\'',
    );
    expect(lane.imageKind).toBeUndefined();
    expect(lane.live).toBe(true);
  ...
  it("rejects unknown selected lanes with the available lane names", () => {
    expect(() => planFor({ selectedLaneNames: ["missing-lane"] })).toThrow(
      /DAISYCLAW_DOCKER_ALL_LANES unknown lane\(s\): missing-lane/u,
    );
  });
});
