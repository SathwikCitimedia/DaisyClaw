...
describe("e2e shell tempfile hygiene", () => {
  ...
  it("preserves wizard exit status when reporting failures", async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), "daisyclaw-onboard-status-test-"));
...
export DAISYCLAW_ONBOARD_SCENARIO_SOURCE_ONLY=1
export DAISYCLAW_ONBOARD_E2E_TMPDIR=${JSON.stringify(tempRoot)}
DAISYCLAW_ENTRY=node
daisyclaw_test_state_create() { :; }
...
`,
  ...
  it("checks local onboarding logs for systemd noise", async () => {
    ...
    expect(contents).toContain('DAISYCLAW_E2E_LOG_DIR="$ONBOARD_TMP_DIR/logs"');
    ...
    expect(contents).toContain('validate_local_basic_log "$DAISYCLAW_E2E_LAST_LOG_PATH"');
    expect(contents).not.toContain(
      "validate_local_basic_log /tmp/daisyclaw-onboard-local-basic.log",
    );
    expect(contents).toContain(
  ...
  it("probes onboarding gateway readiness through TCP", async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), "daisyclaw-onboard-gateway-log-"));
...
daisyclaw_e2e_probe_tcp() { return 0; }
sleep 30 &
GATEWAY_PID="$!"
...
`,
  ...
  it("rejects onboarding gateway readiness when the TCP probe fails", async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), "daisyclaw-onboard-gateway-tcp-"));
...
export DAISYCLAW_ONBOARD_SCENARIO_SOURCE_ONLY=1
export DAISYCLAW_ONBOARD_E2E_TMPDIR=${JSON.stringify(tempRoot)}
export DAISYCLAW_ONBOARD_GATEWAY_WAIT_ATTEMPTS=2
export DAISYCLAW_ONBOARD_GATEWAY_WAIT_INTERVAL_S=0.1
DAISYCLAW_ENTRY=node
source scripts/e2e/lib/onboard/scenario.sh

daisyclaw_e2e_probe_tcp() { return 1; }
sleep 30 &
GATEWAY_PID="$!"
printf 'listening on ws://127.0.0.1:18789\\n' >"$GATEWAY_LOG_PATH"
...
`,
  ...
  it("removes fallback ClawHub skill install HOME on failure", async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), "daisyclaw-clawhub-home-test-"));
...
`,
    ...
    try {
      ...
      expect(
        scratchEntries.filter((entry) => entry.startsWith("daisyclaw-skill-install-home.")),
      ).toEqual([]);
    } finally {
      await rm(tempRoot, { force: true, recursive: true });
    }
