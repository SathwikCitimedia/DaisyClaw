...
describe("generate-dependency-release-evidence", () => {
  ...
  it("collects report counts and renders human summaries", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "daisyclaw-release-dependency-evidence-test-"));
    try {
      ...
      const stepSummary = renderDependencyEvidenceStepSummary({
        evidenceArtifactName: "daisyclaw-release-dependency-evidence-v2026.5.13",
        baseRef: "v2026.5.1",
        counts,
      });
      expect(stepSummary).toContain(
        "- Evidence artifact: `daisyclaw-release-dependency-evidence-v2026.5.13`",
      );
