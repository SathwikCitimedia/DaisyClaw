...
describe("gh-read helpers", () => {
  ...
  it("finds repo from gh args", () => {
    expect(parseRepoArg(["pr", "view", "42", "-R", "daisyclaw/daisyclaw"])).toBe("daisyclaw/daisyclaw");
    expect(parseRepoArg(["run", "list", "--repo=daisyclaw/docs"])).toBe("daisyclaw/docs");
    expect(parseRepoArg(["pr", "view", "42"])).toBeNull();
  });

  it("normalizes repo strings from common git formats", () => {
    expect(normalizeRepo("daisyclaw/daisyclaw")).toBe("daisyclaw/daisyclaw");
    expect(normalizeRepo("github.com/daisyclaw/daisyclaw")).toBe("daisyclaw/daisyclaw");
    expect(normalizeRepo("https://github.com/daisyclaw/daisyclaw.git")).toBe("daisyclaw/daisyclaw");
    expect(normalizeRepo("git@github.com:daisyclaw/daisyclaw.git")).toBe("daisyclaw/daisyclaw");
  ...
  it("rejects invalid GitHub API timeout values", () => {
    ...
    expect(() => resolveGitHubFetchTimeoutMs("1s")).toThrow(
      /DAISYCLAW_GH_READ_FETCH_TIMEOUT_MS must be an integer/u,
    );
  });
});
