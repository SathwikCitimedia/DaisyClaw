...
const helperPath = path.join(
  repoRoot,
  ".agents/skills/daisyclaw-pr-maintainer/scripts/github-activity.sh",
);
...
describe("daisyclaw-pr-maintainer github activity helper", () => {
  it("counts PRs and issues from one paginated issues response", () => {
    ...
    expect(result.stdout).toContain("daisyclaw/daisyclaw last 1mo: 2 PRs, 1 issues, 2 commits");
    expect(log.match(/repos\/daisyclaw\/daisyclaw\/issues/g)).toHaveLength(1);
    expect(log.match(/repos\/daisyclaw\/daisyclaw\/commits/g)).toHaveLength(1);
