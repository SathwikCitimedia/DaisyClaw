...
describe("install.ps1 failure handling", () => {
  ...
  it("keeps failure termination in the top-level completion handler", () => {
    ...
    expect(completeInstallBody).toMatch(/\bthrow "DaisyClaw installation failed with exit code/);
  ...
  it("runs npm install through the resolved command with quiet CI defaults", () => {
    const npmInstallBody = extractFunctionBody(source, "Install-DaisyClaw");
  ...
  it("runs Windows command shims from a Windows-local cwd", () => {
    ...
    const daisyClawPathBody = extractFunctionBody(source, "Ensure-DaisyClawOnPath");
    ...
    expect(daisyClawPathBody).toContain('Invoke-NpmCommand -Arguments @("config", "get", "prefix")');
    ...
    expect(mainBody).toContain('Invoke-NpmCommand -Arguments @("uninstall", "-g", "daisyclaw")');
  ...
  it("rejects DaisyClaw GitHub source targets for npm installs", () => {
    const npmInstallBody = extractFunctionBody(source, "Install-DaisyClaw");
    const sourceTargetBody = extractFunctionBody(source, "Test-DaisyClawSourcePackageInstallSpec");
    expect(sourceTargetBody).toContain('$normalizedTag -eq "main"');
    expect(sourceTargetBody).toContain("^github:daisyclaw/daisyclaw");
    expect(npmInstallBody).toContain("Test-DaisyClawSourcePackageInstallSpec -RequestedTag $Tag");
    expect(npmInstallBody).toContain("npm installs do not support DaisyClaw GitHub source targets");
  ...
  it("preserves caller-relative local tarball install specs before safe-cwd npm calls", () => {
    const resolveSpecBody = extractFunctionBody(source, "Resolve-NpmDaisyClawInstallSpec");
  ...
  it("falls back to a user-local portable Node.js bootstrap when package managers are absent", () => {
    ...
    const depsRootBody = extractFunctionBody(source, "Get-DaisyClawDepsRoot");
    ...
    expect(depsRootBody).toContain("DaisyClaw\\deps");
  ...
  it("persists user-local portable Git for future git-backed updates", () => {
    ...
    expect(portableGitRootBody).toContain("Get-DaisyClawDepsRoot");
  ...
  it("activates the repo-pinned pnpm version for git installs", () => {
    ...
    const ensurePnpmBody = extractFunctionBody(source, "Ensure-Pnpm");
    const gitInstallBody = extractFunctionBody(source, "Install-DaisyClawFromGit");
    ...
    expect(mainBody).toContain("$gitInstallResults = @(Install-DaisyClawFromGit");
    expect(mainBody).toContain("Test-BooleanSuccessResult -Results $gitInstallResults");
    expect(mainBody).toContain("$npmInstallResults = @(Install-DaisyClaw)");
    ...
    expect(gitInstallBody).toContain('Write-Host "[!] DaisyClaw build did not produce $entryPath"');
  ...
  it("launches interactive onboarding outside Main's captured output", () => {
    const interactiveCommandBody = extractFunctionBody(source, "Invoke-InteractiveDaisyClawCommand");
    const mainBody = extractFunctionBody(source, "Main");
    ...
    expect(mainBody).toContain("Invoke-InteractiveDaisyClawCommand onboard");
  });

  runIfPowerShell("fails install when interactive onboarding exits non-zero", () => {
    const tempDir = harness.createTempDir("daisyclaw-install-ps1-");
    ...
    writeFileSync(
      ...
      [
        ...
        "function Check-ExistingDaisyClaw { return $false }",
        "function Get-NpmCommandPath { return 'npm.cmd' }",
        "function Install-DaisyClaw { return $true }",
        "function Ensure-DaisyClawOnPath { return $true }",
        "function Add-ToUserPath { param([string]$Path) }",
        "function Get-DaisyClawCommandPath { return 'cmd.exe' }",
    ...
    expect(`${result.stdout}\n${result.stderr}`).toContain(
      "daisyclaw onboard failed with exit code 17",
    );
  });
  ...
  runIfPowerShell("throws without killing the caller when run as a scriptblock", () => {
    ...
    expect(result.stdout).toContain("caught=DaisyClaw installation failed with exit code 1.");
  ...
  runIfPowerShell("treats noisy Git install false as failure", () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "  return $false",
        "}",
        "function Ensure-DaisyClawOnPath { throw 'should not continue after failed git install' }",
        "$InstallMethod = 'git'",
        "$GitDir = 'C:\\\\daisyclaw-test'",
        "$NoOnboard = $true",
        "$result = Main",
  ...
  runIfPowerShell("keeps npm chatter out of Main's success return value", () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "function Install-DaisyClaw { Write-Output 'npm stdout'; return $true }",
        "function Ensure-DaisyClawOnPath { return $true }",
        "function Refresh-GatewayServiceIfLoaded { }",
        "function Invoke-DaisyClawCommand { return 'DaisyClaw test-version' }",
        "$NoOnboard = $true",
        "$result = Main",
