...
function runInstallShell(script: string, env: NodeJS.ProcessEnv = {}) {
  const home = mkdtempSync(join(tmpdir(), "daisyclaw-install-home-"));
  try {
...
describe("install.sh", () => {
  ...
  it("runs installer snippets without inherited shell startup files", () => {
    const tmp = mkdtempSync(join(tmpdir(), "daisyclaw-install-shell-env-"));
    const bashEnvPath = join(tmp, "bash_env");
    writeFileSync(bashEnvPath, "export DAISYCLAW_BASH_ENV_LEAKED=1\n");

    try {
      const result = runInstallShell('printf "leaked=%s\\n" "${DAISYCLAW_BASH_ENV_LEAKED:-0}"', {
  ...
  it("installs Git with apk on Alpine", () => {
    const tmp = mkdtempSync(join(tmpdir(), "daisyclaw-install-git-apk-"));
  ...
  it("does not select apk Git on non-Alpine hosts", () => {
    const tmp = mkdtempSync(join(tmpdir(), "daisyclaw-install-git-native-"));
    const bin = join(tmp, "bin");
    const apkLog = join(tmp, "apk-args.txt");
    mkdirSync(bin, { recursive: true });
    const fakeApk = join(bin, "apk");
  ...
  it("does not emit --before when raw user npmrc config contains min-release-age", () => {
    const tmp = mkdtempSync(join(tmpdir(), "daisyclaw-install-npmrc-"));
    const bin = join(tmp, "bin");
    ...
    try {
      const result = runInstallShell(
        [
          ...
          `run_npm_global_install daisyclaw@latest ${JSON.stringify(join(tmp, "install.log"))}`,
  ...
  it("does not emit --before when default global npmrc config contains min-release-age", () => {
    const tmp = mkdtempSync(join(tmpdir(), "daisyclaw-install-global-npmrc-"));
    const bin = join(tmp, "bin");
    const home = join(tmp, "home");
  ...
  it("does not emit --before when builtin npmrc config contains min-release-age", () => {
    const tmp = mkdtempSync(join(tmpdir(), "daisyclaw-install-builtin-npmrc-"));
    const bin = join(tmp, "bin");
    const home = join(tmp, "home");
  ...
  it("uses DAISYCLAW_HOME for git and onboarding defaults", () => {
    const tmp = mkdtempSync(join(tmpdir(), "daisyclaw-install-home-"));
    const osHome = join(tmp, "os-home");
    const daisyclawHome = join(tmp, "daisyclaw-home");
    mkdirSync(osHome, { recursive: true });
    mkdirSync(daisyclawHome, { recursive: true });
    ...
    expect(output).toContain(`git=${join(daisyclawHome, "daisyclaw")}`);
    expect(output).toContain(`workspace=${join(daisyclawHome, ".daisyclaw", "workspace")}`);
    expect(output).toContain(
      `workspaceProfile=${join(daisyclawHome, ".daisyclaw", "workspace-work")}`,
    );
  ...
  it("skips bootstrap onboarding when legacy HOME config exists with DAISYCLAW_HOME", () => {
    const tmp = mkdtempSync(join(tmpdir(), "daisyclaw-install-legacy-config-"));
    const osHome = join(tmp, "os-home");
    const daisyclawHome = join(tmp, "daisyclaw-home");
    const legacyConfigDir = join(osHome, ".daisyclaw");
    const bootstrapDir = join(daisyclawHome, ".daisyclaw", "workspace");
    mkdirSync(legacyConfigDir, { recursive: true });
    mkdirSync(bootstrapDir, { recursive: true });
    writeFileSync(join(legacyConfigDir, "daisyclaw.json"), "{}\n");
  ...
  it("rejects DaisyClaw GitHub source targets for npm installs", () => {
    const result = runInstallShell(`
      set -euo pipefail
      source "${SCRIPT_PATH}"
      set +e
      DAISYCLAW_VERSION=main
      USE_BETA=0
      install_daisyclaw
      status=$?
      printf 'status=%s\\n' "$status"
    ...
    expect(result.stdout).toContain("npm installs do not support DaisyClaw GitHub source targets");
  ...
  it("does not emit before args when npmrc min-release-age computes a before cutoff", () => {
    const tmp = mkdtempSync(join(tmpdir(), "daisyclaw-install-npm-freshness-"));
    const bin = join(tmp, "bin");
    const home = join(tmp, "home");
  ...
  it("ignores project npmrc when choosing global install freshness args", () => {
    const tmp = mkdtempSync(join(tmpdir(), "daisyclaw-install-global-freshness-"));
    const bin = join(tmp, "bin");
    const home = join(tmp, "home");
  ...
  it("bounds installer npm prefix probes during finalization helpers", () => {
    const result = runInstallShell(
      [
        ...
        '  if [[ "$1" == "config" && "$2" == "get" && "$3" == "prefix" ]]; then printf "/tmp/daisyclaw-npm\\n"; return 0; fi',
        "  return 1",
        "}",
        "npm_global_bin_dir",
      ].join("\n"),
      { DAISYCLAW_INSTALL_PROBE_TIMEOUT_SECONDS: "0.1" },
    );

    expect(result.status).toBe(0);
    expect(result.stdout.trim()).toBe("/tmp/daisyclaw-npm/bin");
  ...
  it("bounds daemon status probes during finalization helpers", () => {
    const tmp = mkdtempSync(join(tmpdir(), "daisyclaw-install-probe-"));
    const claw = join(tmp, "daisyclaw");
    writeFileSync(
      claw,
      [
        "#!/usr/bin/env bash",
    ...
    try {
      ...
      expect(result.stderr).toContain(
        "timed out during installer finalization probe: daisyclaw daemon status --json",
      );
    } finally {
      rmSync(tmp, { force: true, recursive: true });
    }
  ...
  it("loads nvm before checking Node.js so stale system Node does not win", () => {
    ...
    const tmp = mkdtempSync(join(tmpdir(), "daisyclaw-install-nvm-"));
    const home = join(tmp, "home");
  ...
  it("promotes a supported Linux Node binary over stale PATH entries", () => {
    const tmp = mkdtempSync(join(tmpdir(), "daisyclaw-install-node-promote-"));
  ...
  it("uses the package engine floor when accepting existing Node runtimes", () => {
    ...
    const tmp = mkdtempSync(join(tmpdir(), "daisyclaw-install-node-floor-"));
    const bin = join(tmp, "bin");
    mkdirSync(bin, { recursive: true });
  ...
  it("persists a supported Linux Node path before noninteractive shell guards", () => {
    const tmp = mkdtempSync(join(tmpdir(), "daisyclaw-install-linux-node-path-"));
    const home = join(tmp, "home");
  ...
  it("warns before redirecting an unwritable npm prefix", () => {
    const tmp = mkdtempSync(join(tmpdir(), "daisyclaw-install-npm-prefix-"));
    const home = join(tmp, "home");
    ...
    expect(result?.stdout).toContain("npm i -g daisyclaw@latest");
  ...
  it("persists npm prefix PATH before noninteractive shell guards", () => {
    const tmp = mkdtempSync(join(tmpdir(), "daisyclaw-install-npm-prefix-shell-"));
    const home = join(tmp, "home");
    mkdirSync(home, { recursive: true });
    writeFileSync(
      join(home, ".bashrc"),
  ...
  it("uses a quoted absolute daisyclaw path in follow-up commands when npm bin is not on the original PATH", () => {
    const tmp = mkdtempSync(join(tmpdir(), "daisyclaw-install-command-"));
    const npmBin = join(tmp, "npm bin");
    const visibleBin = join(tmp, "visible-bin");
    mkdirSync(npmBin, { recursive: true });
    mkdirSync(visibleBin, { recursive: true });
    const daisyclawBin = join(npmBin, "daisyclaw");
    writeFileSync(daisyclawBin, "#!/bin/sh\nexit 0\n");
    chmodSync(daisyclawBin, 0o755);
    ...
    try {
      result = runInstallShell(`
        ...
        printf 'missing=%s\\n' "$(daisyclaw_command_for_user "${daisyclawBin}")"
    ...
    expect(result?.status).toBe(0);
    expect(result?.stdout).toContain(`missing=${daisyclawBin.replace(/ /g, "\\ ")}`);
    expect(result?.stdout).toContain("present=daisyclaw");
  });
  ...
  it("resolves requested git install versions to checkout refs", () => {
    const result = runInstallShell(`
      ...
      source "${SCRIPT_PATH}"
      npm() {
        if [[ "$1" == "view" && "$2" == "daisyclaw" && "$3" == "dist-tags.beta" ]]; then
          printf '2026.5.12-beta.3\\n'
          return 0
        fi
        return 1
      }
      DAISYCLAW_VERSION=v2026.5.12-beta.3
      printf 'tag=%s\\n' "$(resolve_git_daisyclaw_ref)"
      DAISYCLAW_VERSION=2026.5.12-beta.3
      printf 'semver=%s\\n' "$(resolve_git_daisyclaw_ref)"
      DAISYCLAW_VERSION=beta
      printf 'beta=%s\\n' "$(resolve_git_daisyclaw_ref)"
      DAISYCLAW_VERSION=main
      printf 'main=%s\\n' "$(resolve_git_daisyclaw_ref)"
    `);
...
describe("install.sh macOS Homebrew Node behavior", () => {
  ...
  it("reruns spinner-wrapped commands when gum reports ioctl failure", () => {
    const dir = mkdtempSync(join(tmpdir(), "daisyclaw-install-sh-gum-"));
    try {
...
describe("install.sh duplicate DaisyClaw install detection", () => {
  it("warns with concrete package paths and versions for duplicate npm roots", () => {
    const result = runInstallShell(`
      ...
      mkdir -p "$root/brew/daisyclaw" "$root/fnm/daisyclaw"
      printf '{"version":"2026.3.7"}\\n' > "$root/brew/daisyclaw/package.json"
      printf '{"version":"2026.3.1"}\\n' > "$root/fnm/daisyclaw/package.json"
      collect_daisyclaw_npm_root_candidates() { printf '%s\\n' "$root/brew" "$root/fnm"; }
      DAISYCLAW_BIN="$root/fnm/.bin/daisyclaw"
      ui_warn() { echo "WARN: $*"; }
      warn_duplicate_daisyclaw_global_installs
    `);

    expect(result.status).toBe(0);
    expect(result.stdout).toContain("Multiple DaisyClaw global installs detected");
    expect(result.stdout).toContain("2026.3.7");
    expect(result.stdout).toContain("2026.3.1");
    expect(result.stdout).toContain("/brew/daisyclaw");
    expect(result.stdout).toContain("/fnm/daisyclaw");
    expect(result.stdout).toContain("Active daisyclaw:");
    expect(result.stdout).toContain("npm uninstall -g daisyclaw");
  });

  it("stays quiet when only one DaisyClaw npm root exists", () => {
    const result = runInstallShell(`
      set -euo pipefail
      source "${SCRIPT_PATH}"
      root="$(mktemp -d)"
      trap 'rm -rf "$root"' EXIT
      mkdir -p "$root/only/daisyclaw"
      printf '{"version":"2026.3.7"}\\n' > "$root/only/daisyclaw/package.json"
      collect_daisyclaw_npm_root_candidates() { printf '%s\\n' "$root/only"; }
      ui_warn() { echo "WARN: $*"; }
      warn_duplicate_daisyclaw_global_installs
    `);
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).not.toContain("Multiple DaisyClaw global installs detected");
  });
});
