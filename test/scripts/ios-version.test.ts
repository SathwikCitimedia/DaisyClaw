...
describe("resolveIosVersion", () => {
  it("parses pinned release versions and derives Apple marketing fields", () => {
    const rootDir = writeIosFixture({
      version: "2026.4.6",
      changelog: "# DaisyClaw iOS Changelog\n\n## 2026.4.6\n\nStable notes.\n",
    });
...
describe("renderIosVersionXcconfig", () => {
  it("renders checked-in defaults from the pinned iOS version", () => {
    ...
    expect(renderIosVersionXcconfig(version)).toContain("DAISYCLAW_IOS_VERSION = 2026.4.8");
    expect(renderIosVersionXcconfig(version)).toContain("DAISYCLAW_MARKETING_VERSION = 2026.4.8");
    expect(renderIosVersionXcconfig(version)).toContain("DAISYCLAW_BUILD_VERSION = 1");
  });
});
