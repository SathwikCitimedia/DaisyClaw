...
function runAssertInstalled({
  ...
  const pluginId = "daisyclaw-kitchen-sink-fixture";
  const home = mkdtempSync(path.join(tmpdir(), "daisyclaw-kitchen-sink-home-"));
  const installPath = mkdtempSync(path.join(tmpdir(), "daisyclaw-kitchen-sink-install-"));
  ...
  const installsPath = path.join(home, ".daisyclaw", "plugins", "installs.json");
  ...
  try {
    ...
    writeJson(installsPath, {
      installRecords: {
        [pluginId]: {
          installPath,
          resolvedSpec: "@daisyclaw/kitchen-sink@latest",
          resolvedVersion: "1.0.0",
          source: "npm",
          spec: "@daisyclaw/kitchen-sink@latest",
        },
      },
    });
    ...
    return spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "assert-installed"], {
      ...
      env: {
        ...
        ...env,
        HOME: home,
        DAISYCLAW_STATE_DIR: path.join(home, ".daisyclaw"),
        KITCHEN_SINK_ID: pluginId,
        KITCHEN_SINK_LABEL: label,
        KITCHEN_SINK_SOURCE: "npm",
        KITCHEN_SINK_SPEC: "npm:@daisyclaw/kitchen-sink@latest",
  ...
  try {
    ...
    writeJson(installsPath, {
      installRecords: {
        [pluginId]: {
          ...
          clawhubPackage: "@daisyclaw/kitchen-sink",
          integrity: "sha256-test",
          installPath,
          resolvedSpec: "clawhub:@daisyclaw/kitchen-sink@latest",
          resolvedVersion: "1.0.0",
          resolvedAt: 1,
          source: "clawhub",
          spec: "clawhub:@daisyclaw/kitchen-sink@latest",
          version: "1.0.0",
        },
      },
    });
    ...
    return spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "assert-installed"], {
      ...
      env: {
        ...
        KITCHEN_SINK_SPEC: "clawhub:@daisyclaw/kitchen-sink@latest",
...
describe("kitchen-sink plugin assertions", () => {
  it("bounds expected-failure output before matching failure diagnostics", () => {
    const scratchRoot = mkdtempSync(path.join(tmpdir(), "daisyclaw-kitchen-sink-failure-cap-"));
      ...
      const result = spawnSync(
        ...
        [ASSERTIONS_SCRIPT, "expect-failure", outputPath],
        {
          ...
          env: {
            ...
            KITCHEN_SINK_SOURCE: "npm",
            KITCHEN_SINK_SPEC: "npm:@daisyclaw/kitchen-sink@0.0.0",
          },
        },
      );
  ...
  it("fails kitchen-sink inspect-all diagnostics for the installed plugin", () => {
    const inspectPayload = fullSurfaceInspectPayload("daisyclaw-kitchen-sink-fixture");
    const result = runAssertInstalled({
      allInspectPayload: [
        {
          ...inspectPayload,
  ...
  it("accepts ClawHub kitchen-sink fixtures with a context engine", () => {
    const result = runAssertClawhubInstalled({
      contextEngineIds: ["daisyclaw-kitchen-sink-fixture"],
    });
  ...
  it("scans only the configured kitchen-sink scratch root", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "daisyclaw-kitchen-sink-scan-"));
  ...
  it("bounds irrelevant DaisyClaw home traversal during log scans", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "daisyclaw-kitchen-sink-scan-"));
    const home = path.join(parent, "home");
    const scratchRoot = path.join(parent, "scratch");
    try {
      mkdirSync(path.join(home, ".daisyclaw"), { recursive: true });
      mkdirSync(scratchRoot, { recursive: true });
      writeFileSync(path.join(scratchRoot, "scenario.log"), "0 errors\n");
      for (let index = 0; index < 20; index += 1) {
        const dir = path.join(home, ".daisyclaw", `cache-${index}`);
  ...
  it("allocates an isolated scratch root by default", () => {
    ...
    expect(sweep).toContain('mktemp -d "/tmp/daisyclaw-kitchen-sink.XXXXXX"');
    ...
    expect(sweep).not.toContain('mktemp -d "/tmp/daisyclaw-kitchen-sink-clawhub.XXXXXX"');
  });

  it("cleans the default kitchen-sink scratch root", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "daisyclaw-kitchen-sink-cleanup-"));
...
`,
      ...
      expect(scratchRoot).toContain("/tmp/daisyclaw-kitchen-sink.");
  ...
  it("preserves successful kitchen-sink CLI command logs for the final scan", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "daisyclaw-kitchen-sink-log-"));
    const scratchRoot = path.join(parent, "scratch");
...
source scripts/e2e/lib/kitchen-sink-plugin/sweep.sh
run_kitchen_sink_daisyclaw_logged "install/log" plugins install demo
...
`,
  ...
  it("bounds printed kitchen-sink CLI command logs without truncating saved logs", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "daisyclaw-kitchen-sink-log-print-"));
    const scratchRoot = path.join(parent, "scratch");
    const entry = path.join(parent, "entry.mjs");
    try {
      mkdirSync(scratchRoot, { recursive: true });
...
source scripts/e2e/lib/kitchen-sink-plugin/sweep.sh
run_kitchen_sink_daisyclaw_logged "install/noisy" plugins install demo
...
`,
  ...
  it("includes expected-failure transcripts in the final kitchen-sink log scan", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "daisyclaw-kitchen-sink-failure-log-"));
    const home = path.join(parent, "home");
    const scratchRoot = path.join(parent, "scratch");
    try {
      mkdirSync(home, { recursive: true });
...
export KITCHEN_SINK_SPEC=npm:@daisyclaw/kitchen-sink@0.0.0
source scripts/e2e/lib/kitchen-sink-plugin/sweep.sh
run_expect_failure "install/failure" bash -c 'printf "%s\\n" "npm ERR! No matching version @daisyclaw/kitchen-sink@0.0.0"; exit 1'
...
`,
  ...
  it("cleans a ClawHub fixture server that times out before readiness", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "daisyclaw-kitchen-sink-clawhub-"));
...
export KITCHEN_SINK_SWEEP_SOURCE_ONLY=1
export KITCHEN_SINK_TMP_DIR="$SCRATCH_ROOT"
export DAISYCLAW_CLAWHUB_FIXTURE_WAIT_ATTEMPTS=1
source scripts/e2e/lib/kitchen-sink-plugin/sweep.sh
set +e
...
`,
  ...
  it("bounds ClawHub fixture server logs on startup timeout", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "daisyclaw-kitchen-sink-clawhub-log-"));
    const fakeBin = path.join(parent, "bin");
    const scratchRoot = path.join(parent, "scratch");
    const fixtureDir = path.join(scratchRoot, "clawhub-fixture");
    const nodeShim = path.join(fakeBin, "node");
...
export KITCHEN_SINK_SWEEP_SOURCE_ONLY=1
export KITCHEN_SINK_TMP_DIR="$SCRATCH_ROOT"
export DAISYCLAW_CLAWHUB_FIXTURE_WAIT_ATTEMPTS=5
export DAISYCLAW_DOCKER_E2E_LOG_PRINT_BYTES=64
source scripts/e2e/lib/kitchen-sink-plugin/sweep.sh
set +e
start_kitchen_sink_clawhub_fixture_server "$FIXTURE_DIR"
