...
describe("limit-edge-case live proof", () => {
  it("cleans the generated session-log temp root", async () => {
    const tempRoot = mkdtempSync(path.join(tmpdir(), "daisyclaw-limit-proof-test-"));
    try {
      let proofRoot = "";
      ...
      expect(readdirSync(tempRoot).filter((entry) => entry.startsWith("daisyclaw-proof-"))).toEqual(
        [],
      );
    } finally {
