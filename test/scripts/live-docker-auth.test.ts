...
function runDockerRunArgs(pathPrefix: string) {
  const script = [
    ...
    "unset DAISYCLAW_LIVE_DOCKER_DISABLE_RESOURCE_LIMITS DAISYCLAW_DOCKER_E2E_DISABLE_RESOURCE_LIMITS",
    "unset DAISYCLAW_LIVE_DOCKER_MEMORY DAISYCLAW_DOCKER_E2E_MEMORY",
    "unset DAISYCLAW_LIVE_DOCKER_CPUS DAISYCLAW_DOCKER_E2E_CPUS",
    "unset DAISYCLAW_LIVE_DOCKER_PIDS_LIMIT DAISYCLAW_DOCKER_E2E_PIDS_LIMIT",
    "ARGS=()",
    "daisyclaw_live_init_docker_run_args ARGS 42s || exit $?",
    "printf '%s\\n' \"${ARGS[@]}\"",
  ].join("\n");
...
describe("scripts/lib/live-docker-auth.sh", () => {
  it("adds a kill-after grace period when timeout supports it", () => {
    const binDir = makeTempBin("daisyclaw-live-docker-auth-gnu-");
    writeExecutable(
  ...
  it("caps default CPU limits to the runner capacity", () => {
    const binDir = makeTempBin("daisyclaw-live-docker-auth-cpus-");
    writeExecutable(
      path.join(binDir, "timeout"),
      [
        "#!/bin/sh",
    ...
    const result = spawnSync(
      ...
      [
        ...
        [
          "source scripts/lib/live-docker-auth.sh",
          "ARGS=()",
          "DAISYCLAW_LIVE_DOCKER_AVAILABLE_CPUS=8 daisyclaw_live_init_docker_run_args ARGS 42s",
          "printf '%s\\n' \"${ARGS[@]}\"",
        ].join("\n"),
      ],
      {
  ...
  it("falls back to plain timeout when kill-after is unavailable", () => {
    const binDir = makeTempBin("daisyclaw-live-docker-auth-plain-");
    writeExecutable(
      path.join(binDir, "timeout"),
  ...
  it("uses gtimeout when timeout is unavailable", () => {
    const binDir = makeTempBin("daisyclaw-live-docker-auth-gtimeout-");
    writeExecutable(
  ...
  it("allows live Docker resource limits to be disabled", () => {
    const binDir = makeTempBin("daisyclaw-live-docker-auth-no-limits-");
    writeExecutable(
      path.join(binDir, "timeout"),
      [
        "#!/bin/sh",
    ...
    const result = spawnSync(
      ...
      [
        ...
        [
          "source scripts/lib/live-docker-auth.sh",
          "ARGS=()",
          "DAISYCLAW_LIVE_DOCKER_DISABLE_RESOURCE_LIMITS=1 daisyclaw_live_init_docker_run_args ARGS 42s",
          "printf '%s\\n' \"${ARGS[@]}\"",
        ].join("\n"),
      ],
      {
  ...
  it("fails fast when no timeout wrapper is available", () => {
    const binDir = makeTempBin("daisyclaw-live-docker-auth-no-timeout-");
