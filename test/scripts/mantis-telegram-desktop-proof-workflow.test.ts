...
describe("Mantis Telegram Desktop proof workflow", () => {
  ...
  it("releases Telegram Desktop proof leases left by interrupted agents", () => {
    ...
    expect(codexStep.env?.DAISYCLAW_QA_CREDENTIAL_OWNER_ID).toContain(
      "mantis-telegram-desktop-${{ github.run_id }}-${{ github.run_attempt }}",
    );
    expect(workflowStep("Prepare Codex user").run).toContain("DAISYCLAW_QA_CREDENTIAL_OWNER_ID");
    ...
    expect(cleanupStep.env?.DAISYCLAW_QA_CONVEX_SECRET_CI).toContain(
      "secrets.DAISYCLAW_QA_CONVEX_SECRET_CI",
    );
    expect(cleanupStep.env?.DAISYCLAW_QA_CONVEX_SITE_URL).toContain(
      "secrets.DAISYCLAW_QA_CONVEX_SITE_URL",
    );
  ...
  it("uses the DaisyClaw Mantis mention as the comment trigger", () => {
    const workflow = readFileSync(WORKFLOW, "utf8");
    const liveWorkflow = readFileSync(LIVE_WORKFLOW, "utf8");
    expect(workflow).toContain("@daisyclaw-mantis");
    expect(workflow).toContain("/daisyclaw-mantis");
  ...
  it("keeps Telegram Desktop proof credentials out of the generic qa-lab API", () => {
    ...
    expect(readFileSync(TELEGRAM_PROOF_SKILL, "utf8")).toContain(
      "DAISYCLAW_TELEGRAM_USER_PROOF_CMD",
    );
  ...
  it("installs local proof tools before the Codex agent runs", () => {
    ...
    expect(install.run).toContain("/usr/local/bin/daisyclaw-telegram-user-crabbox-proof");
    ...
    expect(agent.env?.DAISYCLAW_TELEGRAM_USER_DRIVER_SCRIPT).toBe(
      "${{ github.workspace }}/scripts/e2e/telegram-user-driver.py",
    );
    expect(agent.env?.DAISYCLAW_TELEGRAM_USER_PROOF_CMD).toBe(
      "/usr/local/bin/daisyclaw-telegram-user-crabbox-proof",
    );
    expect(agent.env?.DAISYCLAW_TELEGRAM_USER_CRABBOX_BIN).toBe("/usr/local/bin/crabbox");
    expect(agent.env?.CRABBOX_COORDINATOR).toContain(
      "secrets.CRABBOX_COORDINATOR || secrets.DAISYCLAW_QA_MANTIS_CRABBOX_COORDINATOR",
    );
    expect(agent.env?.CRABBOX_COORDINATOR_TOKEN).toContain(
      "secrets.CRABBOX_COORDINATOR_TOKEN || secrets.DAISYCLAW_QA_MANTIS_CRABBOX_COORDINATOR_TOKEN",
    );

    const prepare = workflowStep("Prepare Codex user");
    expect(prepare.run).toContain(
      "DAISYCLAW_TELEGRAM_USER_CRABBOX_BIN DAISYCLAW_TELEGRAM_USER_CRABBOX_PROVIDER DAISYCLAW_TELEGRAM_USER_DRIVER_SCRIPT DAISYCLAW_TELEGRAM_USER_PROOF_CMD",
    );
    expect(prepare.run).toContain("MANTIS_CANDIDATE_TRUST");

    const prompt = readFileSync(PROMPT, "utf8");
    expect(prompt).toContain("$DAISYCLAW_TELEGRAM_USER_PROOF_CMD");
