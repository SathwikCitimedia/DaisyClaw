...
function writeConfig(home: string, channels: Record<string, unknown>): void {
  ...
  fs.writeFileSync(path.join(configDir, "daisyclaw.json"), JSON.stringify({ channels }));
}
...
function writeAuthProfileStoreSqlite(agentDir: string, store: unknown): void {
  ...
  const db = new DatabaseSync(path.join(agentDir, "daisyclaw-agent.sqlite"));
  try {
    db.exec(`
...
function runStatusAssert(channel: string, channelsStatus: unknown, statusText: string) {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-status-assertions-"));
  try {
...
describe("npm onboard channel agent assertions", () => {
  it("validates OpenAI env refs from the SQLite auth profile store", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-onboard-assertions-"));
    const agentDir = path.join(tempDir, ".daisyclaw", "agents", "main", "agent");
      ...
      writeAuthProfileStoreSqlite(agentDir, {
        ...
        profiles: {
          "openai:api-key": {
            type: "api_key",
            provider: "openai",
            key: "sk-daisyclaw-npm-onboard-e2e",
          },
        },
      });
  ...
  it("validates channel tokens in their canonical config fields", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-channel-assertions-"));
    try {
      writeConfig(tempDir, {
