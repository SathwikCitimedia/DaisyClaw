...
function mkTempRoot() {
  const root = mkdtempSync(path.join(tmpdir(), "daisyclaw-npm-telegram-live-"));
  tempRoots.push(root);
  return root;
}
...
describe("package Telegram live Docker E2E", () => {
  it("supports npm-specific Convex credential aliases", () => {
    ...
    expect(script).toContain("DAISYCLAW_NPM_TELEGRAM_CREDENTIAL_SOURCE");
    expect(script).toContain("DAISYCLAW_NPM_TELEGRAM_CREDENTIAL_ROLE");
    expect(script).toContain('docker_env+=(-e DAISYCLAW_QA_CREDENTIAL_SOURCE="$credential_source")');
    expect(script).toContain('docker_env+=(-e DAISYCLAW_QA_CREDENTIAL_ROLE="$credential_role")');
  });
  ...
  it("defaults CI runs to Convex when broker credentials are present", () => {
    ...
    expect(script).toContain("DAISYCLAW_QA_CONVEX_SECRET_CI");
    expect(script).toContain("DAISYCLAW_QA_CONVEX_SECRET_MAINTAINER");
  ...
  it("bounds installed-package hot path DaisyClaw commands", () => {
    const script = readFileSync(DOCKER_SCRIPT_PATH, "utf8");
    ...
    expect(runtimeRun).toContain("source scripts/lib/daisyclaw-e2e-instance.sh");
    expect(runtimeRun).toContain("daisyclaw_e2e_run_command daisyclaw --version");
    expect(runtimeRun).toContain("daisyclaw_e2e_run_command daisyclaw onboard");
    ...
    expect(runtimeRun).toContain("daisyclaw_e2e_run_command daisyclaw channels add");
    expect(runtimeRun).toContain("daisyclaw_e2e_run_command daisyclaw doctor --fix");
    expect(runtimeRun).toContain("daisyclaw_e2e_run_command daisyclaw doctor --non-interactive");
    expect(runtimeRun).toContain('daisyclaw_e2e_print_log "$file"');
    expect(runtimeRun).not.toContain("sed -n '1,220p'");
    expect(runtimeRun).not.toMatch(/^\s*daisyclaw (onboard|channels add|doctor )/mu);
  });

  it("can install a resolved package tarball instead of a registry spec", () => {
    const script = readFileSync(DOCKER_SCRIPT_PATH, "utf8");

    expect(script).toContain("DAISYCLAW_NPM_TELEGRAM_PACKAGE_TGZ");
    expect(script).toContain("DAISYCLAW_CURRENT_PACKAGE_TGZ");
    expect(script).toContain(
      'package_mount_args=(-v "$resolved_package_tgz:$package_install_source:ro")',
    );
    expect(script).toContain('validate_daisyclaw_package_spec "$PACKAGE_SPEC"');
    expect(script.indexOf('if [ -n "$resolved_package_tgz" ]; then')).toBeLessThan(
      script.indexOf('validate_daisyclaw_package_spec "$PACKAGE_SPEC"'),
    );
  });
  ...
  it("keeps live Docker artifacts isolated by default", () => {
    ...
    expect(script).not.toContain(
      'OUTPUT_DIR="${DAISYCLAW_NPM_TELEGRAM_OUTPUT_DIR:-.artifacts/qa-e2e/npm-telegram-live}"',
    );
  });
  ...
  it("keeps private QA harness imports local while using the installed package dist", () => {
    ...
    expect(script).toContain('ln -sfnT "$daisyclaw_package_dir/dist" /app/dist');
    expect(script).toContain('cp "$daisyclaw_package_dir/package.json" /app/package.json');
    expect(script).toContain('-v "$ROOT_DIR/extensions/qa-lab:/app/extensions/qa-lab:ro"');
    expect(script).not.toContain('ln -sfnT /app/extensions "$daisyclaw_package_dir/extensions"');
    expect(script).toContain("node scripts/e2e/lib/npm-telegram-live/prepare-package.mjs");
    expect(script).toContain("/app/node_modules/daisyclaw/package.json");
    ...
    expect(gatewayRpcClient).toContain('from "daisyclaw/plugin-sdk/gateway-runtime"');
    expect(qaRuntimeApi).toContain('from "daisyclaw/plugin-sdk/gateway-runtime"');
  });
  ...
  it("lets npm-specific credential aliases override shared QA env", () => {
    expect(
      testing.resolveCredentialSource({
        DAISYCLAW_NPM_TELEGRAM_CREDENTIAL_SOURCE: "convex",
        DAISYCLAW_QA_CREDENTIAL_SOURCE: "env",
      }),
    ).toBe("convex");
    expect(
      testing.resolveCredentialRole({
        DAISYCLAW_NPM_TELEGRAM_CREDENTIAL_ROLE: "ci",
  ...
  it("gates package Telegram status on the summary artifact", async () => {
    ...
    await expect(
      testing.shouldFailPackageTelegramRun(
        { summaryPath },
        { DAISYCLAW_NPM_TELEGRAM_ALLOW_FAILURES: "" },
      ),
    ).resolves.toBe(true);
  });
  ...
  it("does not read package Telegram summaries when failures are allowed", async () => {
    await expect(
      testing.shouldFailPackageTelegramRun(
        ...
        { DAISYCLAW_NPM_TELEGRAM_ALLOW_FAILURES: "1" },
      ),
    ).resolves.toBe(false);
  });
});
