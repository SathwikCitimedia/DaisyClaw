...
function runDriver(env: Record<string, string>) {
  return spawnSync(process.execPath, [DRIVER_SCRIPT], {
    ...
    env: {
      ...process.env,
      DAISYCLAW_QA_TELEGRAM_API_BASE_URL: "http://127.0.0.1:9",
      DAISYCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN: "driver-token",
      DAISYCLAW_QA_TELEGRAM_GROUP_ID: "-100123",
      DAISYCLAW_QA_TELEGRAM_SUT_BOT_TOKEN: "sut-token",
      ...env,
    },
  });
}
    ...
    void (async () => {
      ...
      if (method === "sendMessage") {
        ...
        if (token === "driver-token" && text.startsWith("/status@")) {
          ...
          const marker = text.match(/Reply with exactly ([^.]+)\./u)?.[1] ?? "DAISYCLAW_E2E_OK_1";
...
async function runStalledTelegramBodyCase(): Promise<DriverCaseResult> {
  const root = mkdtempSync(path.join(tmpdir(), "daisyclaw-telegram-rtt-driver-"));
  ...
  try {
    ...
    const result = spawnSync(process.execPath, [DRIVER_SCRIPT], {
      ...
      env: {
        ...process.env,
        DAISYCLAW_NPM_TELEGRAM_BOT_API_TIMEOUT_MS: "100",
        DAISYCLAW_NPM_TELEGRAM_OUTPUT_DIR: outputDir,
        DAISYCLAW_NPM_TELEGRAM_WARM_SAMPLES: "1",
        DAISYCLAW_QA_TELEGRAM_API_BASE_URL: `http://127.0.0.1:${port}`,
        DAISYCLAW_QA_TELEGRAM_CANARY_TIMEOUT_MS: "1000",
        DAISYCLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN: "driver-token",
        DAISYCLAW_QA_TELEGRAM_GROUP_ID: "-100123",
        DAISYCLAW_QA_TELEGRAM_SCENARIO_TIMEOUT_MS: "1000",
        DAISYCLAW_QA_TELEGRAM_SUT_BOT_TOKEN: "sut-token",
      },
      killSignal: "SIGKILL",
      timeout: 2500,
...
async function runOversizedTelegramBodyCase(): Promise<DriverCaseResult> {
  ...
  try {
    ...
    const result = spawnSync(process.execPath, [DRIVER_SCRIPT], {
      ...
      env: {
        ...process.env,
        DAISYCLAW_NPM_TELEGRAM_BOT_API_BODY_MAX_BYTES: "16",
        DAISYCLAW_NPM_TELEGRAM_BOT_API_TIMEOUT_MS: "1000",
        DAISYCLAW_NPM_TELEGRAM_OUTPUT_DIR: outputDir,
        DAISYCLAW_NPM_TELEGRAM_WARM_SAMPLES: "1",
        DAISYCLAW_QA_TELEGRAM_API_BASE_URL: `http://127.0.0.1:${port}`,
        DAISYCLAW_QA_TELEGRAM_CANARY_TIMEOUT_MS: "1000",
...
describe("npm Telegram RTT driver", () => {
  ...
  it("rejects loose numeric env values instead of parsing prefixes", () => {
    for (const [name, value] of [
      ["DAISYCLAW_QA_TELEGRAM_SCENARIO_TIMEOUT_MS", "180000ms"],
      ["DAISYCLAW_QA_TELEGRAM_CANARY_TIMEOUT_MS", "1e3"],
      ["DAISYCLAW_NPM_TELEGRAM_WARM_SAMPLES", "20samples"],
      ["DAISYCLAW_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS", "30000ms"],
      ["DAISYCLAW_NPM_TELEGRAM_BOT_API_TIMEOUT_MS", "100ms"],
      ["DAISYCLAW_NPM_TELEGRAM_BOT_API_BODY_MAX_BYTES", "1mb"],
      ["DAISYCLAW_NPM_TELEGRAM_MAX_FAILURES", "2failures"],
    ]) {
  ...
  it("rejects zero where positive numeric env values are required", () => {
    const result = runDriver({ DAISYCLAW_NPM_TELEGRAM_WARM_SAMPLES: "0" });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("invalid DAISYCLAW_NPM_TELEGRAM_WARM_SAMPLES: 0");
  });

  it("rejects empty scenario selections before live Telegram calls", () => {
    const result = runDriver({ DAISYCLAW_NPM_TELEGRAM_SCENARIOS: "," });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain(
      "DAISYCLAW_NPM_TELEGRAM_SCENARIOS must include at least one RTT scenario",
    );
  });

  it("rejects unknown scenario selections before live Telegram calls", () => {
    const result = runDriver({ DAISYCLAW_NPM_TELEGRAM_SCENARIOS: "does-not-exist" });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("unknown DAISYCLAW_NPM_TELEGRAM_SCENARIOS: does-not-exist");
  });
  ...
  it("rejects unrelated SUT messages during the Telegram canary", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "daisyclaw-telegram-rtt-causal-"));
    ...
    try {
      ...
      const result = await runDriverAsync({
        DAISYCLAW_NPM_TELEGRAM_OUTPUT_DIR: outputDir,
        DAISYCLAW_NPM_TELEGRAM_WARM_SAMPLES: "1",
        DAISYCLAW_QA_TELEGRAM_API_BASE_URL: server.baseUrl,
        DAISYCLAW_QA_TELEGRAM_CANARY_TIMEOUT_MS: "250",
        DAISYCLAW_QA_TELEGRAM_SCENARIO_TIMEOUT_MS: "250",
      });
