...
function makeTempRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), "daisyclaw-npm-verify-exec-"));
  tempDirs.push(root);
  return root;
}
...
describe("npm verifier command execution", () => {
  ...
  it("rejects malformed command limit environment values", () => {
    ...
    withProcessEnv({ DAISYCLAW_NPM_VERIFY_COMMAND_TIMEOUT_MS: "5m" }, () => {
      expect(() =>
        runNpmVerifyCommand(
          { command: process.execPath, args: ["-e", "process.stdout.write('ok')"] },
          root,
        ),
      ).toThrow("invalid DAISYCLAW_NPM_VERIFY_COMMAND_TIMEOUT_MS: 5m");
    });

    withProcessEnv({ DAISYCLAW_NPM_VERIFY_COMMAND_MAX_BUFFER_BYTES: "16mb" }, () => {
      expect(() =>
        runNpmVerifyCommand(
          { command: process.execPath, args: ["-e", "process.stdout.write('ok')"] },
          root,
        ),
      ).toThrow("invalid DAISYCLAW_NPM_VERIFY_COMMAND_MAX_BUFFER_BYTES: 16mb");
    });
  });
});
