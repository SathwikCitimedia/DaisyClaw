...
const LIVE_E2E_WORKFLOW = ".github/workflows/daisyclaw-live-and-e2e-checks-reusable.yml";
...
const RELEASE_CHECKS_WORKFLOW = ".github/workflows/daisyclaw-release-checks.yml";
const RELEASE_PUBLISH_WORKFLOW = ".github/workflows/daisyclaw-release-publish.yml";
...
const SCHEDULED_LIVE_CHECKS_WORKFLOW = ".github/workflows/daisyclaw-scheduled-live-checks.yml";
...
describe("package acceptance workflow", () => {
  ...
  it("keeps Crabbox hydration compatible with local Actions replay", () => {
    ...
    expect(hydrateWindowsPnpm.run).toContain('"daisyclaw",');
    expect(hydrateWindowsPnpm.run).toContain(
    ...
    expect(workflowText).toContain("DAISYCLAW_CRABBOX_HYDRATE_DOWNLOAD_TIMEOUT_SECONDS:-300");
    expect(workflowText).toContain("DAISYCLAW_CRABBOX_HYDRATE_DOWNLOAD_RETRIES:-3");
  ...
  it("resolves candidate package sources before reusing Docker E2E lanes", () => {
    ...
    expect(workflow).toContain("scripts/resolve-daisyclaw-package-candidate.mjs");
    ...
    expect(workflow).toContain(
      "uses: ./.github/workflows/daisyclaw-live-and-e2e-checks-reusable.yml",
    );
    expect(workflow).toContain(
    ...
    expect(workflow).toContain(
      "node scripts/check-daisyclaw-package-tarball.mjs .artifacts/docker-e2e-package/daisyclaw-current.tgz",
    );
  ...
  it("offers bounded product profiles and can run Telegram against the resolved artifact", () => {
    ...
    expect(workflow).toContain(
      "package_label: daisyclaw@${{ needs.resolve_package.outputs.package_version }}",
    );
  ...
  it("requires pinned full release child workflows to run at the resolved target SHA", () => {
    ...
    expect(releaseChecksWorkflow).toContain(
      'codex_plugin_spec="npm:@daisyclaw/codex@${BASH_REMATCH[1]}"',
    );
    expect(releaseChecksWorkflow).toContain(
    ...
    expect(releaseChecksWorkflow).toContain(
      "package_spec: ${{ needs.resolve_target.outputs.package_acceptance_package_spec || needs.resolve_target.outputs.release_package_spec || 'daisyclaw@beta' }}",
    );
  });
...
describe("package artifact reuse", () => {
  it("lets reusable Docker E2E consume an already resolved package artifact", () => {
    ...
    expect(workflow).toContain("DAISYCLAW_DOCKER_E2E_SELECTED_SHA:");
    expect(workflow).toContain(
      "DAISYCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC: ${{ inputs.published_upgrade_survivor_baseline }}",
    );
    expect(workflow).toContain(
      "DAISYCLAW_UPGRADE_SURVIVOR_BASELINE_SPECS: ${{ matrix.group.published_upgrade_survivor_baselines || inputs.published_upgrade_survivor_baselines }}",
    );
    expect(workflow).toContain(
      "DAISYCLAW_UPGRADE_SURVIVOR_SCENARIOS: ${{ inputs.published_upgrade_survivor_scenarios }}",
    );
    expect(workflow).toContain("Download current-run DaisyClaw Docker E2E package");
    expect(workflow).toContain("Download previous-run DaisyClaw Docker E2E package");
    ...
    expect(workflow).toContain("DAISYCLAW_DOCKER_E2E_REPO_ROOT:");
    ...
    expect(workflowStep(prepareDockerImage, "Plan Docker E2E images").env).toEqual({
      ...
      DAISYCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC: "${{ inputs.published_upgrade_survivor_baseline }}",
      DAISYCLAW_UPGRADE_SURVIVOR_BASELINE_SPECS:
    ...
    expect(workflow).toContain(
      "DAISYCLAW_UPGRADE_SURVIVOR_BASELINE_SPECS: ${{ inputs.published_upgrade_survivor_baselines }}",
    );
    ...
    expect(scheduler).toContain(
      "published_upgrade_survivor_baseline=${shellQuote(process.env.DAISYCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC)}",
    );
    expect(scheduler).toContain(
      "published_upgrade_survivor_baselines=${shellQuote(process.env.DAISYCLAW_UPGRADE_SURVIVOR_BASELINE_SPECS)}",
    );
    expect(scheduler).toContain(
      '["DAISYCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC", baseEnv.DAISYCLAW_UPGRADE_SURVIVOR_BASELINE_SPEC]',
    );
    expect(scheduler).toContain('["DAISYCLAW_UPGRADE_SURVIVOR_BASELINE_SPECS",');
    expect(scheduler).toContain('["DAISYCLAW_UPGRADE_SURVIVOR_SCENARIOS",');
    expect(packageJson).toContain("DAISYCLAW_UPGRADE_SURVIVOR_PUBLISHED_BASELINE=1");
    expect(packageJson).toContain("test:docker:update-restart-auth");
    expect(packageJson).toContain("DAISYCLAW_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE=auto-auth");
    expect(publishedUpgradeSurvivor).toContain("validate_baseline_package_spec");
    expect(publishedUpgradeSurvivor).toContain("DAISYCLAW_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE");
    ...
    expect(publishedUpgradeSurvivor).toContain(
      "env -u DAISYCLAW_GATEWAY_TOKEN -u DAISYCLAW_GATEWAY_PASSWORD daisyclaw",
    );
    expect(publishedUpgradeSurvivor).toContain("phase prepare-update-restart-probe");
    expect(publishedUpgradeSurvivor).toContain("daisyclaw@(alpha|beta|latest|");
    ...
    expect(publishedUpgradeSurvivor).toContain('"id": "opik-daisyclaw"');
  ...
  it("bounds shared Docker image pulls so package acceptance cannot stall forever", () => {
    ...
    expect(pullHelper).toContain("DAISYCLAW_DOCKER_PULL_ATTEMPTS");
    expect(pullHelper).toContain("DAISYCLAW_DOCKER_PULL_TIMEOUT_SECONDS");
    expect(pullHelper).toContain('timeout_seconds="${DAISYCLAW_DOCKER_PULL_TIMEOUT_SECONDS:-180}"');
    expect(pullHelper).toContain(
      'retry_delay_seconds="${DAISYCLAW_DOCKER_PULL_RETRY_DELAY_SECONDS:-5}"',
    );
    expect(pullHelper).toContain(
    ...
    expect(dockerE2ePlanAction).not.toContain('docker pull "${DAISYCLAW_DOCKER_E2E_');
  });
  ...
  it("shards broad native live tests instead of one serial live-all job", () => {
    ...
    expect(workflow).toContain("DAISYCLAW_LIVE_COMMAND: ${{ matrix.command }}");
    ...
    expect(workflow).toContain("DAISYCLAW_LIVE_GATEWAY_PROVIDERS=deepseek,fireworks");
    expect(workflow).toContain("DAISYCLAW_LIVE_GATEWAY_PROVIDERS=opencode-go,openrouter");
    expect(workflow).toContain("DAISYCLAW_LIVE_GATEWAY_PROVIDERS=xai,zai");
    expect(workflow).toContain("inputs.live_suite_filter == 'live-gateway-advisory-docker'");
    expect(workflow).toContain("DAISYCLAW_LIVE_CLI_BACKEND_MODEL=claude-cli/claude-sonnet-4-6");
    expect(workflow).toContain("DAISYCLAW_LIVE_CLI_BACKEND_AUTH=api-key");
    expect(workflow).not.toContain("DAISYCLAW_LIVE_CLI_BACKEND_USE_CI_SAFE_CODEX_CONFIG=1");
    expect(workflow).not.toContain('service_tier=\\"fast\\"');
    expect(workflow).not.toContain("DAISYCLAW_LIVE_CLI_BACKEND_ARGS=");
    expect(workflow).not.toContain("DAISYCLAW_LIVE_CLI_BACKEND_RESUME_ARGS=");
    expect(workflow).not.toContain(
      'DAISYCLAW_LIVE_CLI_BACKEND_ARGS=["exec","--json","--color","never","--sandbox","danger-full-access","--skip-git-repo-check"]',
    );
    ...
    expect(workflow).toContain(
      "command: DAISYCLAW_LIVE_CODEX_HARNESS=1 DAISYCLAW_LIVE_CODEX_HARNESS_AUTH=api-key node .release-harness/scripts/test-live-shard.mjs native-live-src-gateway-core",
    );
    expect(workflow).toContain(
      "command: DAISYCLAW_LIVE_CODEX_HARNESS=1 DAISYCLAW_LIVE_CODEX_HARNESS_AUTH=api-key node .release-harness/scripts/test-live-shard.mjs native-live-src-gateway-backends",
    );
    expect(workflow).toContain("suite_id: native-live-src-infra");
    expect(workflow).toContain(
      "command: DAISYCLAW_LIVE_APNS_REACHABILITY=1 node .release-harness/scripts/test-live-shard.mjs native-live-src-infra",
    );
    ...
    expect(workflow).toContain("DAISYCLAW_LIVE_GATEWAY_MODELS=anthropic/claude-opus-4-8");
    ...
    expect(workflow).toContain(
      "command: DAISYCLAW_LIVE_GATEWAY_THINKING=off DAISYCLAW_LIVE_GATEWAY_PROVIDERS=openai DAISYCLAW_LIVE_GATEWAY_MODELS=openai/gpt-5.5 DAISYCLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS=180000 DAISYCLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS=600000",
    );
    expect(workflow).toContain(
      "DAISYCLAW_LIVE_GATEWAY_MODELS=google/gemini-3.1-pro-preview node .release-harness/scripts/test-live-shard.mjs native-live-src-gateway-profiles",
    );
    expect(workflow).toContain(
      "DAISYCLAW_LIVE_GATEWAY_MODELS=minimax/MiniMax-M3,minimax-portal/MiniMax-M3 DAISYCLAW_LIVE_GATEWAY_MAX_MODELS=2",
    );
    expect(workflow).toMatch(
    ...
    expect(workflow).not.toContain(
      "DAISYCLAW_LIVE_GATEWAY_PROVIDERS=deepseek,opencode-go,openrouter,xai,zai",
    );
    expect(workflow).toContain("suite_id: live-gateway-anthropic-docker");
    expect(workflow).toContain("DAISYCLAW_LIVE_GATEWAY_MAX_MODELS=2");
    expect(workflow).toContain(
      "DAISYCLAW_LIVE_GATEWAY_THINKING=off DAISYCLAW_LIVE_GATEWAY_PROVIDERS=openai DAISYCLAW_LIVE_GATEWAY_MODELS=openai/gpt-5.5 DAISYCLAW_LIVE_GATEWAY_MAX_MODELS=1 DAISYCLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS=90000 DAISYCLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS=600000",
    );
    expect(workflow).toContain(
      "DAISYCLAW_LIVE_GATEWAY_MODELS=anthropic/claude-sonnet-4-6,anthropic/claude-haiku-4-5 DAISYCLAW_LIVE_GATEWAY_MAX_MODELS=2",
    );
    expect(workflow).toContain("DAISYCLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS=600000");
    ...
    expect(workflow).toContain("DAISYCLAW_LIVE_SUITE_ADVISORY: ${{ matrix.advisory }}");
    expect(workflow).toContain("Advisory live suite failed with exit code");
    expect(workflow).toMatch(
      /validate_live_media_provider_suites:[\s\S]*?DAISYCLAW_LIVE_SUITE_ADVISORY: \$\{\{ matrix\.advisory \}\}[\s\S]*?shell: bash[\s\S]*?Advisory live suite failed with exit code/u,
    );
    expect(workflow).toMatch(
      /suite_id: live-gateway-advisory-docker-deepseek-fireworks[\s\S]*?advisory: true/u,
    );
    expect(workflow).toMatch(
      /validate_live_media_provider_suites:[\s\S]*?DAISYCLAW_LIVE_SUITE_ADVISORY: \$\{\{ matrix\.advisory \}\}/u,
    );
    expect(workflow).toMatch(
    ...
    expect(workflow).toContain("image: ghcr.io/daisyclaw/daisyclaw-live-media-runner:ubuntu-24.04");
    ...
    expect(workflow).toContain("DAISYCLAW_LIVE_VIDEO_GENERATION_PROVIDERS=google,minimax");
    expect(workflow).toContain("DAISYCLAW_LIVE_VIDEO_GENERATION_PROVIDERS=openai,openrouter,xai");
    ...
    expect(retryHelper).toContain("DAISYCLAW_LIVE_COMMAND_ATTEMPTS:-2");
    ...
    expect(retryHelper).toContain("DAISYCLAW_LIVE_COMMAND_RATE_LIMIT_RETRY_DELAY_SECONDS:-60");
  ...
  it("runs Docker live harnesses from trusted helper scripts", () => {
    ...
    expect(workflow).toContain(
      'run: DAISYCLAW_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 35m bash .release-harness/scripts/test-live-models-docker.sh',
    );
    expect(workflow).toContain(
      "command: DAISYCLAW_LIVE_GATEWAY_THINKING=off DAISYCLAW_LIVE_GATEWAY_PROVIDERS=openai DAISYCLAW_LIVE_GATEWAY_MODELS=openai/gpt-5.5 DAISYCLAW_LIVE_GATEWAY_MAX_MODELS=1",
    );
    expect(workflow).toContain(
      'command: DAISYCLAW_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 45m bash .release-harness/scripts/test-live-cli-backend-docker.sh',
    );
    expect(workflow).toContain(
      'command: DAISYCLAW_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 45m bash .release-harness/scripts/test-live-acp-bind-docker.sh',
    );
    expect(workflow).toContain(
      'command: DAISYCLAW_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 35m bash .release-harness/scripts/test-live-codex-harness-docker.sh',
    );
    expect(workflow).toContain(
      'command: DAISYCLAW_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 20m bash .release-harness/scripts/test-live-subagent-announce-docker.sh',
    );
    ...
    expect(scenarios).toContain("process.env.DAISYCLAW_DOCKER_E2E_REPO_ROOT");
    expect(scenarios).toContain(
      'harness="\\${DAISYCLAW_DOCKER_E2E_TRUSTED_HARNESS_DIR:-${LIVE_DOCKER_DEFAULT_HARNESS_DIR}}"',
    );
    ...
    expect(liveDockerAuth).toContain("daisyclaw_live_init_docker_run_args()");
    expect(liveDockerAuth).toContain("daisyclaw_live_stage_profile_into_home()");
    expect(liveDockerAuth).toContain("daisyclaw_live_chown_bind_dirs_for_container_user()");
    expect(liveDockerAuth).toContain("daisyclaw_live_uses_managed_bind_dirs()");
    expect(liveDockerAuth).toContain('daisyclaw_live_truthy "${DAISYCLAW_TESTBOX:-}"');
    expect(liveDockerAuth).toContain('[[ -n "${DAISYCLAW_DOCKER_CACHE_HOME_DIR:-}" ]]');
    expect(liveDockerAuth).toContain(
      'timeout_value="${2:-${DAISYCLAW_LIVE_DOCKER_RUN_TIMEOUT:-2700s}}"',
    );
    ...
    for (const script of [harness, ...sharedLiveScripts]) {
      ...
      expect(script).toContain("daisyclaw_live_init_docker_run_args DOCKER_RUN_ARGS");
      expect(script).toContain("daisyclaw_live_prepare_bind_dir_for_container_user");
    ...
    for (const script of sharedLiveScripts) {
      expect(script).toContain("daisyclaw_live_uses_managed_bind_dirs");
    ...
    for (const script of [
      ...
      expect(script).not.toContain('timeout --kill-after=30s "${DAISYCLAW_LIVE_');
    }
    expect(readFileSync("scripts/test-live-models-docker.sh", "utf8")).toContain(
      "DAISYCLAW_LIVE_MODELS_DOCKER_RUN_TIMEOUT:-2100s",
    );
    expect(readFileSync("scripts/test-live-gateway-models-docker.sh", "utf8")).toContain(
      "DAISYCLAW_LIVE_GATEWAY_DOCKER_RUN_TIMEOUT:-2100s",
    );
    expect(readFileSync("scripts/test-live-cli-backend-docker.sh", "utf8")).toContain(
      "DAISYCLAW_LIVE_CLI_BACKEND_DOCKER_RUN_TIMEOUT:-2700s",
    );
    expect(readFileSync("scripts/test-live-cli-backend-docker.sh", "utf8")).toContain(
      'timeout_value="${DAISYCLAW_LIVE_CLI_BACKEND_SETUP_TIMEOUT_SECONDS:-180}s"',
    );
    expect(readFileSync("scripts/test-live-cli-backend-docker.sh", "utf8")).toContain(
    ...
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
      "DAISYCLAW_LIVE_ACP_BIND_DOCKER_RUN_TIMEOUT:-2700s",
    );
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
      "DAISYCLAW_LIVE_ACP_BIND_SETUP_TIMEOUT_SECONDS:-180",
    );
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
      'timeout_value="${DAISYCLAW_LIVE_ACP_BIND_SETUP_TIMEOUT_SECONDS:-180}s"',
    );
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
    ...
    expect(readFileSync("scripts/test-live-codex-harness-docker.sh", "utf8")).toContain(
      "DAISYCLAW_LIVE_CODEX_HARNESS_DOCKER_RUN_TIMEOUT:-2100s",
    );
    expect(readFileSync("scripts/test-live-codex-harness-docker.sh", "utf8")).toContain(
      "DAISYCLAW_LIVE_CODEX_HARNESS_SETUP_TIMEOUT_SECONDS:-180",
    );
    expect(readFileSync("scripts/test-live-codex-harness-docker.sh", "utf8")).toContain(
      'timeout_value="${DAISYCLAW_LIVE_CODEX_HARNESS_SETUP_TIMEOUT_SECONDS:-180}s"',
    );
    expect(readFileSync("scripts/test-live-codex-harness-docker.sh", "utf8")).toContain(
    ...
    expect(readFileSync("scripts/test-live-subagent-announce-docker.sh", "utf8")).toContain(
      "DAISYCLAW_LIVE_SUBAGENT_DOCKER_RUN_TIMEOUT:-1200s",
    );
    expect(build).toContain('ROOT_DIR="${DAISYCLAW_LIVE_DOCKER_REPO_ROOT:-$SCRIPT_ROOT_DIR}"');
    ...
    expect(build).toContain('LIVE_IMAGE_PULL_ATTEMPTS="${DAISYCLAW_LIVE_DOCKER_PULL_ATTEMPTS:-3}"');
    ...
    expect(stage).toContain(
      'local scripts_dir="${DAISYCLAW_LIVE_DOCKER_SCRIPTS_DIR:-/src/scripts}"',
    );
  ...
  it("fails Droid ACP Docker live proof when Factory auth is missing", () => {
    ...
    expect(script).toContain("daisyclaw_live_acp_bind_load_factory_api_key_from_profile");
    expect(script).not.toContain('source "$PROFILE_FILE"');
    expect(script.indexOf("daisyclaw_live_acp_bind_load_factory_api_key_from_profile")).toBeLessThan(
  ...
  it("plumbs live credentials through planned Docker E2E live lanes", () => {
    ...
    expectTextToIncludeAll(dockerPlanAction, [
      ...
      "require_any Codex DAISYCLAW_CODEX_AUTH_JSON",
      'if [[ "$credentials" == *",anthropic,"* ]]; then',
      "require_any Anthropic ANTHROPIC_API_TOKEN ANTHROPIC_API_KEY DAISYCLAW_CLAUDE_CREDENTIALS_JSON DAISYCLAW_CLAUDE_JSON",
      'if [[ "$credentials" == *",factory,"* ]]; then',
      "require_any Factory FACTORY_API_KEY",
      'if [[ "$credentials" == *",gemini,"* ]]; then',
      "require_any Gemini GEMINI_API_KEY GOOGLE_API_KEY DAISYCLAW_GEMINI_SETTINGS_JSON",
  ...
  it("fails Testbox changed-check delegation when the remote command fails", () => {
    ...
    expect(workflow).toContain('PNPM_CONFIG_STORE_DIR: "/tmp/daisyclaw-pnpm-store"');
  ...
  it("allows the Telegram lane to run from reusable package acceptance artifacts", () => {
    ...
    expect(workflow).toContain("DAISYCLAW_NPM_TELEGRAM_PACKAGE_TGZ");
  ...
  it("includes package acceptance in release checks", () => {
    ...
    expect(workflow).toContain(
      "DAISYCLAW_QA_CONVEX_SITE_URL: ${{ secrets.DAISYCLAW_QA_CONVEX_SITE_URL }}",
    );
    expect(workflow).toContain(
      "DAISYCLAW_QA_CONVEX_SECRET_CI: ${{ secrets.DAISYCLAW_QA_CONVEX_SECRET_CI }}",
    );
    ...
    expect(workflow).toContain("DAISYCLAW_RELEASE_QA_*_LIVE_CI_ENABLED");
    expect(workflow).not.toContain(
  ...
  it("detects Matrix fail-fast support for older release refs", () => {
    ...
    expect(releaseWorkflow).toContain(
      'pnpm daisyclaw qa matrix --help 2>/dev/null | grep -F -q -- "--fail-fast"',
    );
    expect(releaseWorkflow).toContain("matrix_args+=(--fail-fast)");
    expect(releaseWorkflow).toContain(
      'pnpm daisyclaw qa matrix --output-dir "${attempt_output_dir}" "${matrix_args[@]}"',
    );
    expect(releaseWorkflow).toContain(
  ...
  it("runs live transport lanes nightly while release checks stay gated", () => {
    ...
    for (const channel of ["DISCORD", "WHATSAPP", "SLACK"]) {
      ...
      expect(releaseWorkflow).toContain(
        `RELEASE_QA_${channel}_LIVE_CI_ENABLED: \${{ vars.DAISYCLAW_RELEASE_QA_${channel}_LIVE_CI_ENABLED || 'false' }}`,
      );
      ...
      expect(releaseWorkflow).not.toContain(
        `vars.DAISYCLAW_RELEASE_QA_${channel}_LIVE_CI_ENABLED == 'true'`,
      );
      expect(qaWorkflow).not.toContain(`DAISYCLAW_QA_${channel}_LIVE_CI_ENABLED`);
    }
  });
  ...
  it("runs full release children from the trusted workflow ref", () => {
    ...
    expectTextToIncludeAll(
      ...
      [
        "scripts/resolve-daisyclaw-package-candidate.mjs",
        "--source ref",
    ...
    expectTextToIncludeAll(dispatchStep.run, [
      ...
      'args=(-f package_spec="${PACKAGE_SPEC:-daisyclaw@beta}"',
  ...
  it("lets npm Telegram consume current-run or release-run package artifacts", () => {
    ...
    expectTextToIncludeAll(validateStep.run, [
      ...
      "package_spec must be daisyclaw@alpha",
    ]);
    expectTextToIncludeAll(runStep.run, [
      'export DAISYCLAW_NPM_TELEGRAM_PACKAGE_TGZ="${package_tgzs[0]}"',
    ]);
  });
  ...
  it("lets CI Telegram consumers wait on Convex leases instead of GitHub concurrency", () => {
    ...
    for (const [workflowPath, jobName, stepName] of telegramJobs) {
      ...
      expect(step.env?.DAISYCLAW_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS).toBe("1800000");
    }
  });
  ...
  it("keeps release QA and repo E2E lanes off scarce 32-core runners", () => {
    ...
    expectTextToIncludeAll(liveE2eWorkflow, [
      "DAISYCLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS=180000",
      "DAISYCLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS=600000",
    ]);
  });
  ...
  it("keeps release publish creation compatible with gh api and prerelease notes", () => {
    ...
    const npmWorkflow = readFileSync(".github/workflows/daisyclaw-npm-release.yml", "utf8");
    const fullReleaseWorkflow = readFileSync(FULL_RELEASE_VALIDATION_WORKFLOW, "utf8");

    expect(workflow).toContain("timeout-minutes: 60");
    expect(workflow).toContain("environment: npm-release");
    expect(workflow).toContain("Download DaisyClaw npm preflight manifest");
    expect(workflow).toContain("Validate DaisyClaw npm preflight manifest");
    ...
    expect(workflow).toContain(
      "publish_daisyclaw_npm=true requires plugin_publish_scope=all-publishable",
    );
    ...
    expect(fullReleaseWorkflow).not.toContain("node /app/daisyclaw.mjs agent");
    expect(fullReleaseWorkflow).toContain('DAISYCLAW_EXTENSIONS="diagnostics-otel,codex"');
  ...
  it("keeps beta release verification and ClawHub publish repair hooks wired", () => {
    ...
    const daisyclawNpmWorkflow = readFileSync(".github/workflows/daisyclaw-npm-release.yml", "utf8");
    ...
    expect(clawHubWorkflow).toContain(
      "uses: daisyclaw/clawhub/.github/workflows/package-publish.yml@c9bb13023598dcc547fdf4a93b9d42512b8c8854",
    );
    ...
    expect(releaseWorkflow).toContain("DaisyClaw npm run ID");
    ...
    expect(releaseWorkflow).toContain("daisyclaw-release-postpublish-evidence");
    ...
    expect(daisyclawNpmWorkflow).toContain("Validate release publish approval run");
    ...
    expect(daisyclawNpmWorkflow).toContain("Direct DaisyClaw npm publish");
    ...
    expect(daisyclawNpmWorkflow).toContain('GITHUB_ACTOR}" != "github-actions[bot]"');
    ...
    expect(daisyclawNpmWorkflow).toContain("Direct DaisyClaw npm recovery");
    ...
    expect(daisyclawNpmWorkflow).toContain("validate-release-publish-approval.mjs");
    ...
    expect(daisyclawNpmWorkflow).toContain("environment: npm-release");
  ...
  it("keeps release workflow setup and timeout budgets bounded", () => {
    ...
    const crossOs = readWorkflow(".github/workflows/daisyclaw-cross-os-release-checks-reusable.yml");
    const liveE2e = readWorkflow(LIVE_E2E_WORKFLOW);
    const releaseWorkflowPaths = [
      FULL_RELEASE_VALIDATION_WORKFLOW,
      RELEASE_CHECKS_WORKFLOW,
      ".github/workflows/daisyclaw-cross-os-release-checks-reusable.yml",
      LIVE_E2E_WORKFLOW,
      NPM_TELEGRAM_WORKFLOW,
      ".github/workflows/daisyclaw-release-publish.yml",
      ".github/workflows/daisyclaw-npm-release.yml",
  ...
  it("kills timed TUI PTY workflow runs after the grace period", () => {
    ...
    expect(job.env?.DAISYCLAW_TUI_PTY_INCLUDE_LOCAL).toBe("1");
