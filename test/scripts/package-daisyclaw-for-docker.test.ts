// Package Daisyclaw For Docker tests cover package daisyclaw for docker script behavior.
...
import {
  ...
  packDaisyClawPackageForDocker,
  parseArgs,
  runCommandForTest,
} from "../../scripts/package-daisyclaw-for-docker.mjs";
...
describe("package-daisyclaw-for-docker", () => {
  ...
  it("uses build-all as the single bounded package artifact build step", async () => {
    ...
    const previousTimeout = process.env.DAISYCLAW_DOCKER_PACKAGE_BUILD_TIMEOUT_MS;
    process.env.DAISYCLAW_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = "1234";
    ...
    try {
      await buildPackageArtifacts("/repo", {
        runImpl: async (
          ...
          calls.push({
            ...
            args,
            cwd,
            noPnpm: options.env?.DAISYCLAW_BUILD_ALL_NO_PNPM,
            skipDts: options.env?.DAISYCLAW_RUN_NODE_SKIP_DTS_BUILD,
      ...
      if (previousTimeout === undefined) {
        delete process.env.DAISYCLAW_DOCKER_PACKAGE_BUILD_TIMEOUT_MS;
      } else {
        process.env.DAISYCLAW_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = previousTimeout;
      }
    }
  ...
  it("trims and restores the changelog around ignore-scripts package artifacts", async () => {
    const calls: string[] = [];
    const tarball = await packDaisyClawPackageForDocker("/repo", "/out", {
      ...
      runCaptureImpl: async (
        ...
        return "daisyclaw-2026.5.28.tgz\n";
      },
    });

    expect(tarball).toBe(path.join("/out", "daisyclaw-2026.5.28.tgz"));
    expect(calls).toEqual([
      "prepare:/repo",
  ...
  it("restores the changelog when ignore-scripts packaging fails", async () => {
    ...
    await expect(
      packDaisyClawPackageForDocker("/repo", "/out", {
        prepareChangelog: async (cwd: string) => {
          calls.push(`prepare:${cwd}`);
        },
        restoreChangelog: async (cwd: string) => {
  ...
  it("kills timed-out child process groups", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-package-timeout-"));
  ...
  it("keeps fallback SIGKILL armed for descendants after the direct child exits", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-package-descendant-"));
    const childPidPath = path.join(tempDir, "child.pid");
    let childPid;
    try {
      const childScript = ["process.on('SIGTERM', () => {});", "setInterval(() => {}, 1000);"].join(
  ...
  it("forwards external termination to active child process groups", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-package-signal-"));
    const childPidPath = path.join(tempDir, "child.pid");
    const scriptUrl = pathToFileURL(path.resolve("scripts/package-daisyclaw-for-docker.mjs")).href;
    let childPid = 0;
    let runnerPid;
    try {
