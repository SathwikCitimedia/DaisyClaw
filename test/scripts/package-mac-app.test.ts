...
function makePlist(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "daisyclaw-plistbuddy-"));
  tempDirs.push(dir);
...
function runStopPackagedAppHarness(killZeroStatus: 0 | 1) {
  const root = mkdtempSync(path.join(tmpdir(), "daisyclaw-package-stop-root-"));
  const toolsDir = mkdtempSync(path.join(tmpdir(), "daisyclaw-package-stop-tools-"));
  tempDirs.push(root, toolsDir);

  const appRoot = path.join(root, "dist", "DaisyClaw.app");
  const appBinary = path.join(appRoot, "Contents", "MacOS", "DaisyClaw");
...
function runSwiftCompatibilityHarness(buildConfig: "debug" | "release") {
  const root = mkdtempSync(path.join(tmpdir(), "daisyclaw-package-swift-root-"));
  const toolsDir = mkdtempSync(path.join(tmpdir(), "daisyclaw-package-swift-tools-"));
  const developerDir = path.join(root, "Xcode.app", "Contents", "Developer");
  const appRoot = path.join(root, "DaisyClaw.app");
...
describe("package-mac-app plist stamping", () => {
  ...
  it("falls back to corepack pnpm when the pnpm shim is absent", () => {
    ...
    const tempRoot = mkdtempSync(path.join(tmpdir(), "daisyclaw-package-pnpm-root-"));
    const toolsDir = mkdtempSync(path.join(tmpdir(), "daisyclaw-package-pnpm-tools-"));
    ...
    const result = runHelper(`
      ...
      DAISYCLAW_TEST_LOG=${JSON.stringify(logPath)}
      export DAISYCLAW_TEST_LOG
      PATH=${JSON.stringify(`${toolsDir}:/usr/bin:/bin`)}
      ${helperBlock}
  ...
  it("runs Sparkle build metadata derivation from the repository root", () => {
    ...
    const tempRoot = mkdtempSync(path.join(tmpdir(), "daisyclaw-package-sparkle-root-"));
    const toolsDir = mkdtempSync(path.join(tmpdir(), "daisyclaw-package-sparkle-tools-"));
    tempDirs.push(tempRoot, toolsDir);
    ...
    const result = runHelper(`
      set -euo pipefail
      ROOT_DIR=${JSON.stringify(tempRoot)}
      DAISYCLAW_ROOT=${JSON.stringify(tempRoot)}
      PATH=${JSON.stringify(`${toolsDir}:/usr/bin:/bin`)}
      export DAISYCLAW_ROOT PATH
      cd /tmp
      ${helperBlock}
  ...
  it("does not kill unrelated DaisyClaw processes during packaging", () => {
    const script = readFileSync(scriptPath, "utf8");
    ...
    expect(script).not.toContain("killall -q DaisyClaw");
    expect(stopBlock).toContain('local app_binary="$APP_ROOT/Contents/MacOS/DaisyClaw"');
  ...
  it("fails when the packaged app survives forced shutdown", () => {
    ...
    expect(result.status).toBe(1);
    expect(result.stderr).toContain("ERROR: Packaged DaisyClaw bundle did not exit: 123");
  });
  ...
  it("fails closed when required Swift resources are missing", () => {
    const script = readFileSync(scriptPath, "utf8");
    const daisyClawKitBlock = script.slice(
      script.indexOf(
        'DAISYCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/DaisyClawKit_DaisyClawKit.bundle"',
      ),
      script.indexOf('echo "📦 Copying Textual resources"'),
    );

    expect(daisyClawKitBlock).toContain("ERROR: DaisyClawKit resource bundle not found");
    expect(daisyClawKitBlock).toContain("exit 1");
    expect(daisyClawKitBlock).not.toContain("WARN:");
    expect(daisyClawKitBlock).not.toContain("continuing");
  });
  ...
  it.runIf(process.platform === "darwin")(
    ...
    () => {
      ...
      const result = runHelper(`
        ...
        plist_set_string_required ${JSON.stringify(plist)} CFBundleIdentifier 'ai.daisyclaw.test'
      ...
      expect(result.status).toBe(1);
      expect(result.stdout).toContain("ai.daisyclaw.test");
