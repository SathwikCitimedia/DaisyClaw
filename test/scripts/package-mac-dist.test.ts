...
function makePlist(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "daisyclaw-dist-plist-"));
  tempDirs.push(dir);
...
describe("package-mac-dist plist validation", () => {
  it("fails closed for required Info.plist reads", () => {
    ...
    const readBlock = script.slice(
      ...
      script.indexOf('ZIP="$ROOT_DIR/dist/DaisyClaw-$VERSION.zip"'),
    );
  ...
  it("keeps dependency bootstrap output out of captured Sparkle build values", () => {
    ...
    const dir = mkdtempSync(path.join(tmpdir(), "daisyclaw-dist-sparkle-"));
    tempDirs.push(dir);
    ...
    const result = runHelper(`
      ...
      DAISYCLAW_ROOT=${JSON.stringify(process.cwd())}
      DAISYCLAW_MARKER=${JSON.stringify(marker)}
      PATH=${JSON.stringify(tools)}:/usr/bin:/bin
      export DAISYCLAW_MARKER DAISYCLAW_ROOT PATH
      ${helpers}
