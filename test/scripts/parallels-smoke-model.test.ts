...
import {
  extractLastDaisyClawVersionFromLog,
...
describe("Parallels smoke model selection", () => {
  ...
  it("extracts the last DaisyClaw version from a bounded log tail", async () => {
    const tempDir = mkdtempSync(join(tmpdir(), "daisyclaw-parallels-log-tail-"));
    const logPath = join(tempDir, "phase.log");
    try {
      writeFileSync(logPath, ["DaisyClaw 0.0.1", "x".repeat(4096), "DaisyClaw 2026.6.7"].join("\n"));

      await expect(extractLastDaisyClawVersionFromLog(logPath, undefined, 128)).resolves.toBe(
        "2026.6.7",
      );
    } finally {
  ...
  beforeAll(() => {
    ...
    invalidModelTimeoutResult = spawnNodeEvalSync(
      `process.env.DAISYCLAW_PARALLELS_MACOS_MODEL_TIMEOUT_S = "1800s"; const { resolveParallelsModelTimeoutSeconds } = await import("./${TS_PATHS.common}"); resolveParallelsModelTimeoutSeconds("macos");`,
      { env: process.env, imports: ["tsx"] },
    );
    ...
    invalidLinuxAgentTimeoutResult = spawnNodeEvalSync(
      `process.env.DAISYCLAW_PARALLELS_LINUX_AGENT_TIMEOUT_S = "1e3"; process.argv = ["node", "${TS_PATHS.linux}"]; await import("./${TS_PATHS.linux}");`,
      { env: process.env, imports: ["tsx"] },
    );
    invalidWindowsAgentTimeoutResult = spawnNodeEvalSync(
      `process.env.DAISYCLAW_PARALLELS_WINDOWS_AGENT_TIMEOUT_S = "2700s"; process.argv = ["node", "${TS_PATHS.windows}"]; await import("./${TS_PATHS.windows}");`,
      { env: process.env, imports: ["tsx"] },
    );
    invalidWindowsUpdateTimeoutResult = spawnNodeEvalSync(
      `process.env.DAISYCLAW_PARALLELS_WINDOWS_UPDATE_TIMEOUT_S = "12.5"; process.argv = ["node", "${TS_PATHS.windows}"]; await import("./${TS_PATHS.windows}");`,
      { env: process.env, imports: ["tsx"] },
    );
  });
  ...
  it("accepts leading package-manager separators and still honors later terminators", () => {
    ...
    expect(parseNpmUpdateSmokeArgs(["--", "--package-spec", "daisyclaw@2026.5.1"]).packageSpec).toBe(
      "daisyclaw@2026.5.1",
    );
    expect(
  ...
  it("keeps provider auth and model defaults in the shared TypeScript helper", () => {
    ...
    expect(providerAuth).toContain("DAISYCLAW_PARALLELS_OPENAI_MODEL");
    expect(providerAuth).toContain("DAISYCLAW_PARALLELS_WINDOWS_OPENAI_MODEL");
  ...
  it("keeps snapshot, host, package, and quote helpers shared", () => {
    ...
    expect(packageArtifact).toContain("export async function packDaisyClaw");
  ...
  it("uses a temporary npmrc file and cleans it after resolving the latest package version", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "daisyclaw-parallels-version-"));
    let userConfigPath = "";
    try {
      const version = resolveLatestVersion("", {
        createTempDir: (prefix) => {
          expect(prefix).toBe(join(tmpdir(), "daisyclaw-npm-"));
          return mkdtempSync(join(tempRoot, "npm-"));
        },
        runCommand: (command, args, options) => {
          userConfigPath = args.at(-1) ?? "";
          expect(command).toBe("npm");
          expect(args).toEqual(["view", "daisyclaw", "version", "--userconfig", userConfigPath]);
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = mkdtempSync(join(tmpdir(), "daisyclaw-parallels-host-server-"));
...
`,
  ...
  it("quotes shell args and resolves fuzzy snapshot hints through the shared TypeScript helper", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "daisyclaw-parallels-helper-"));
    writeFakePrlctl(
      tempDir,
      `#!/usr/bin/env bash
set -euo pipefail
...
`,
  ...
  it("resolves a latest snapshot hint to the matching version before older LATEST labels", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "daisyclaw-parallels-snapshot-latest-"));
    writeFakePrlctl(
      tempDir,
      `#!/usr/bin/env bash
set -euo pipefail
...
`,
  ...
  it("rejects skip-restore for combined Parallels smoke lanes", () => {
    ...
    expect(invalidSkipBothResult.stderr).toContain(
      "DAISYCLAW_PARALLELS_SKIP_SNAPSHOT_RESTORE=1 requires --mode fresh or --mode upgrade",
    );
    expect(() =>
  ...
  it("uses one Ubuntu VM fallback resolver for Linux lanes", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "daisyclaw-parallels-vm-helper-"));
    writeFakePrlctl(
      tempDir,
      `#!/usr/bin/env bash
set -euo pipefail
...
`,
  ...
  it("keeps Linux bad-plugin diagnostics gated for historical update baselines", () => {
    ...
    expect(script).toContain("parseDaisyClawPackageVersion");
      ...
      withEnv(
        {
          ...
          DAISYCLAW_PARALLELS_WINDOWS_OPENAI_MODEL: "openai/custom-windows",
        },
        () => resolveWindowsProviderAuth({ provider: "openai" }),
      ),
    ).toEqual({
  ...
  it("streams full phase logs to disk while bounding the failure tail", async () => {
    const runDir = mkdtempSync(join(tmpdir(), "daisyclaw-parallels-phase-"));
  ...
  it("preseeds dev update channel before stable-to-dev update lanes", () => {
    ...
    expect(macos).toContain("DAISYCLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1");
    expect(windows).toContain("DAISYCLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS");
  });
  ...
  it("passes aggregate model overrides into each OS fresh lane", () => {
    ...
    expect(script).toContain("DAISYCLAW_PARALLELS_LINUX_DISABLE_BONJOUR");
  });
  ...
  it("keeps the Windows update config scrub compatible with PowerShell 5.1", () => {
    ...
    expect(script).toContain("function Get-DaisyClawJsonProperty");
    expect(script).toContain("function Remove-DaisyClawJsonProperty");
    expect(script).toContain("Remove-DaisyClawJsonProperty $entries $pluginId");
  });
  ...
  it("keeps aggregate update guest scripts isolated from the npm-update orchestrator", () => {
    ...
    expect(updateScripts).toContain("Invoke-DaisyClaw update");
  ...
  it("resolves macOS smoke commands from the guest PATH", () => {
    ...
    expect(macos).toContain('const guestDaisyClaw = "daisyclaw"');
    expect(macos).toContain('const guestNode = "node"');
    expect(macos).toContain('const guestNpm = "npm"');
    expect(macos).toContain("$(npm root -g)/daisyclaw/daisyclaw.mjs");
    expect(macos).toContain("guestDaisyClawEntryExec");
    expect(macos).not.toContain('const guestDaisyClaw = "/opt/homebrew/bin/daisyclaw"');
    ...
    expect(macos).not.toContain("/opt/homebrew/lib/node_modules/daisyclaw/daisyclaw.mjs");
  });

  it("keeps Windows gateway reachability on a real deadline with start recovery", () => {
    const script = readFileSync(TS_PATHS.windows, "utf8");

    expect(script).toContain("DAISYCLAW_PARALLELS_WINDOWS_GATEWAY_RECOVERY_AFTER_S");
  ...
  it("runs Windows ref onboarding through a detached done-file runner", () => {
    ...
    expect(transports).toContain("__DAISYCLAW_BACKGROUND_DONE__");
    expect(transports).toContain("__DAISYCLAW_BACKGROUND_EXIT__");
    expect(transports).toContain("__DAISYCLAW_LOG_OFFSET__");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = mkdtempSync(join(tmpdir(), "daisyclaw-parallels-host-command-"));
...
`,
  ...
  it.runIf(process.platform !== "win32")("preserves timed host command spawn errors", () => {
    expect(() =>
      run("daisyclaw-definitely-missing-host-command", [], {
        check: false,
        quiet: true,
        timeoutMs: 50,
      }),
  ...
  it("rejects streaming host commands when log writes fail", async () => {
    const tempDir = mkdtempSync(join(tmpdir(), "daisyclaw-parallels-host-command-log-"));
    try {
      await expect(
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const result = run(
        process.execPath,
        ["-e", "process.stderr.write('__DAISYCLAW_HOST_COMMAND_SPAWN_ERROR__{}\\n')"],
        {
          check: false,
          quiet: true,
          timeoutMs: 500,
  ...
  it("routes Windows host pnpm and npm shims through safe runners", () => {
    ...
    expect(
      resolveHostCommandInvocation("npm", ["view", "daisyclaw", "version"], {
        env: { ComSpec: comSpec },
        execPath,
      ...
      args: ["/d", "/s", "/c", `${npmCmdPath} view daisyclaw version`],
      command: comSpec,
      shell: false,
      windowsVerbatimArguments: true,
    });
  ...
  it("runs the Windows agent turn through the detached done-file runner", () => {
    ...
    expect(script).toContain('guestPowerShellBackground(\n      "agent-turn"');
    expect(script).toContain("DAISYCLAW_PARALLELS_WINDOWS_AGENT_TIMEOUT_S");
    expect(script).toContain(
      'readPositiveIntEnv(\n    "DAISYCLAW_PARALLELS_WINDOWS_AGENT_TIMEOUT_S"',
    );
  ...
  it("gives GPT-5.5 enough Parallels model time on slower desktop guests", () => {
    ...
    expect(readFileSync(TS_PATHS.macos, "utf8")).toContain(
      'this.agentTimeoutSeconds = readPositiveIntEnv("DAISYCLAW_PARALLELS_MACOS_AGENT_TIMEOUT_S", 2700)',
    );
  ...
  it("rejects loose Parallels numeric limits before starting smoke lanes", () => {
    expect(
      withEnv({ DAISYCLAW_PARALLELS_MODEL_TIMEOUT_S: "1200" }, () =>
        resolveParallelsModelTimeoutSeconds("linux"),
      ),
    ).toBe(1200);
    expect(
      withEnv({ DAISYCLAW_PARALLELS_NUMERIC_TEST: " 42 " }, () =>
        readPositiveIntEnv("DAISYCLAW_PARALLELS_NUMERIC_TEST", 7),
      ),
    ).toBe(42);

    expect(invalidModelTimeoutResult.status).toBe(1);
    expect(invalidModelTimeoutResult.stderr).toContain(
      "invalid DAISYCLAW_PARALLELS_MACOS_MODEL_TIMEOUT_S: 1800s",
    );
    ...
    expect(invalidLinuxAgentTimeoutResult.stderr).toContain(
      "invalid DAISYCLAW_PARALLELS_LINUX_AGENT_TIMEOUT_S: 1e3",
    );

    expect(invalidWindowsAgentTimeoutResult.status).toBe(1);
    expect(invalidWindowsAgentTimeoutResult.stderr).toContain(
      "invalid DAISYCLAW_PARALLELS_WINDOWS_AGENT_TIMEOUT_S: 2700s",
    );

    expect(invalidWindowsUpdateTimeoutResult.status).toBe(1);
    expect(invalidWindowsUpdateTimeoutResult.stderr).toContain(
      "invalid DAISYCLAW_PARALLELS_WINDOWS_UPDATE_TIMEOUT_S: 12.5",
    );

    expect(readFileSync(TS_PATHS.macos, "utf8")).toContain(
      'this.updateDevTimeoutSeconds = readPositiveIntEnv(\n      "DAISYCLAW_PARALLELS_MACOS_UPDATE_DEV_TIMEOUT_S"',
    );
    expect(readFileSync(TS_PATHS.linux, "utf8")).toContain(
      'readPositiveIntEnv(\n    "DAISYCLAW_PARALLELS_LINUX_AGENT_TIMEOUT_S"',
    );
    expect(readFileSync(TS_PATHS.windows, "utf8")).toContain(
      'readPositiveIntEnv(\n    "DAISYCLAW_PARALLELS_WINDOWS_UPDATE_TIMEOUT_S"',
    );
    expect(readFileSync(TS_PATHS.packageArtifact, "utf8")).toContain(
      'readPositiveIntEnv("DAISYCLAW_PARALLELS_PACKAGE_LOCK_TIMEOUT_MS", 30 * 60_000)',
    );
    expect(readFileSync(TS_PATHS.npmUpdate, "utf8")).toContain(
      'readPositiveIntEnv("DAISYCLAW_PARALLELS_NPM_UPDATE_TIMEOUT_S", 1200)',
    );
  });
  ...
  it("keeps Windows update-only env flags scoped before verification", () => {
    ...
    expect(windows).toContain(
      "Invoke-WithScopedEnv @{ DAISYCLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS",
    );
    expect(windows).toContain("$script:DaisyClawUpdateExit = $LASTEXITCODE");
    expect(windows).not.toContain("$env:DAISYCLAW_DISABLE_BUNDLED_PLUGINS = '1'");
  });
  ...
  it("resolves Windows DaisyClaw commands without assuming the npm shim path", () => {
    const powershell = readFileSync(TS_PATHS.powershell, "utf8");
    const windows = readFileSync(TS_PATHS.windows, "utf8");

    expect(powershell).toContain("windowsDaisyClawResolver");
    ...
    expect(powershell).toContain("DAISYCLAW_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED");
    expect(powershell).toContain("Programs\\nodejs");
    expect(powershell).toContain('selectedModelEntry.agentRuntime = { id: "daisyclaw" }');
    ...
    expect(powershell).toContain("Resolve-DaisyClawCommand");
    expect(powershell).toContain("npm\\node_modules\\daisyclaw\\daisyclaw.mjs");
    ...
    expect(windows).toContain("windowsDaisyClawResolver");
    expect(windows).toContain("Invoke-DaisyClaw gateway");
    expect(windows).not.toContain("Join-Path $env:APPDATA 'npm\\\\daisyclaw.cmd'");
  });
});
