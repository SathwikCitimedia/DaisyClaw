...
describe("plugin gateway gauntlet helpers", () => {
  ...
  it("discovers bundled plugin manifests into lifecycle matrix rows", async () => {
    ...
    expect(matrix[0]).toEqual({
      ...
      id: "alpha",
      manifestPath: path.join("extensions", "alpha", "daisyclaw.plugin.json"),
      name: "alpha",
  ...
  it("keeps manifest ids separate from bounded build entry ids", async () => {
    await writeManifest("kimi-coding", "daisyclaw.plugin.json", JSON.stringify({ id: "kimi" }));
    ...
    expect(buildGauntletPrebuildEnv({}, { buildIds: [matrix[0].buildId] })).toEqual({
      DAISYCLAW_BUNDLED_PLUGIN_BUILD_IDS: "kimi-coding",
      PNPM_CONFIG_VERIFY_DEPS_BEFORE_RUN: "false",
    });
  });

  it("skips source-only plugin dirs that are excluded from the built runtime", async () => {
    await writeManifest("qa-lab", "daisyclaw.plugin.json", JSON.stringify({ id: "qa-lab" }));
    await writeManifest("qqbot", "daisyclaw.plugin.json", JSON.stringify({ id: "qqbot" }));
    await writeManifest("telegram", "daisyclaw.plugin.json", JSON.stringify({ id: "telegram" }));
  ...
  it("prebuilds private QA dist when QA chunks are enabled", () => {
    expect(buildGauntletPrebuildEnv({ EXISTING: "1" }, { includePrivateQa: true })).toEqual({
      ...
      DAISYCLAW_BUNDLED_PLUGIN_BUILD_IDS: "qa-channel,qa-lab,qa-matrix",
  ...
  it("marks gauntlet prebuilds as runtime-only when requested", () => {
    expect(
      ...
      EXISTING: "1",
      DAISYCLAW_BUNDLED_PLUGIN_BUILD_IDS: "acpx",
      DAISYCLAW_RUN_NODE_SKIP_DTS_BUILD: "1",
      PNPM_CONFIG_VERIFY_DEPS_BEFORE_RUN: "false",
    });
  });
  ...
  it("prebuilds only selected plugin dist entries for bounded gauntlet runs", () => {
    expect(
      ...
      EXISTING: "1",
      DAISYCLAW_BUILD_PRIVATE_QA: "1",
      DAISYCLAW_BUNDLED_PLUGIN_BUILD_IDS: "acpx,active-memory,qa-channel,qa-lab,qa-matrix",
      DAISYCLAW_ENABLE_PRIVATE_QA_CLI: "1",
      PNPM_CONFIG_VERIFY_DEPS_BEFORE_RUN: "false",
    });
  });
...
`,
  ...
  it("parses observation failure mode from CLI and env", () => {
    ...
    vi.stubEnv("DAISYCLAW_PLUGIN_GATEWAY_GAUNTLET_FAIL_ON_OBSERVATION", "1");
  ...
  it("probes plugin-owned slash help while the plugin is installed", async () => {
    ...
    await fs.writeFile(
      ...
      [
        ...
        "const stateDir = process.env.DAISYCLAW_STATE_DIR ?? process.cwd();",
        ...
        '    console.log("Usage: daisyclaw workboard");',
        "    process.exit(0);",
        "  }",
  ...
  it("carries bounded build ids into QA run-node chunks", async () => {
    ...
    await writeManifest("alpha", "daisyclaw.plugin.json", JSON.stringify({ id: "alpha" }));
    ...
    await fs.writeFile(
      ...
      [
        ...
        'fs.writeFileSync(path.join(outputDir, "env.txt"), process.env.DAISYCLAW_BUNDLED_PLUGIN_BUILD_IDS ?? "", "utf8");',
