...
function makeTempDir(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "daisyclaw-plugin-lifecycle-probe-"));
  tempDirs.push(dir);
  return dir;
}
...
function runProbe(args: string[], home = makeTempDir()) {
  return spawnSync(process.execPath, [probePath, ...args], {
    ...
    env: {
      ...process.env,
      HOME: home,
      DAISYCLAW_CONFIG_PATH: path.join(home, ".daisyclaw", "daisyclaw.json"),
      USERPROFILE: home,
    },
  });
}
...
describe("plugin lifecycle matrix probe", () => {
  ...
  it("rejects unreadable config during uninstall proof", () => {
    const dir = makeTempDir();
    const configPath = path.join(dir, ".daisyclaw", "daisyclaw.json");
