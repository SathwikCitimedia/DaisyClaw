...
describe("scripts/lib/plugin-prerelease-test-plan.mjs", () => {
  ...
  it("uses kitchen-sink npm and ClawHub scenarios as the registry install canary", () => {
    ...
    expect(lane).toEqual({
      command: "DAISYCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:kitchen-sink-plugin",
    ...
    expect(script).toContain("npm:@daisyclaw/kitchen-sink@latest");
    expect(script).toContain("npm-latest-conformance");
    expect(script).toContain("npm-latest-adversarial");
    expect(script).toContain("npm:@daisyclaw/kitchen-sink@beta");
    expect(script).toContain("clawhub:@daisyclaw/kitchen-sink@latest");
    expect(script).toContain("clawhub:@daisyclaw/kitchen-sink@beta");
    expect(script).toContain("DAISYCLAW_KITCHEN_SINK_PLUGIN_MAX_MEMORY_MIB");
    expect(script).toContain(
      "npm-to-clawhub|clawhub:@daisyclaw/kitchen-sink@latest|daisyclaw-kitchen-sink-fixture|clawhub|success|basic||${KITCHEN_SINK_NPM_SPEC}",
    );
    ...
    expect(sweepScript).toContain("DAISYCLAW_KITCHEN_SINK_PERSONALITY");
  ...
  it("keeps kitchen-sink RPC coverage package-backed and resource-guarded", () => {
    ...
    expect(script).toContain("DAISYCLAW_ENTRY=/app/daisyclaw.mjs");
    expect(script).toContain("DAISYCLAW_KITCHEN_SINK_COMMAND_MAX_RSS_MIB");
    expect(script).toContain("docker_e2e_sample_stats_until_exit");
    expect(script).toContain("scripts/e2e/lib/docker-stats/assert-resource-ceiling.mjs");
    ...
    expect(walkScript).toContain("usesBuiltDaisyClawEntry(runner)");
  ...
  it("keeps the generic plugin Docker lane as an external install contract canary", () => {
    ...
    expect(lane).toEqual({
      command: "DAISYCLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugins",
      e2eImageKind: "functional",
      live: false,
      name: "plugins",
    ...
    expect(assertionsScript).toContain("assertClawHubExternalInstallContract");
    expect(assertionsScript).toContain('node_modules", "daisyclaw');
    expect(fixtureServer).toContain('"is-number": "7.0.0"');
    expect(fixtureServer).toContain('daisyclaw: ">=2026.4.11"');
  ...
  it("wires the full plugin prerelease plan into its release workflow", () => {
    ...
    expect(inspectorRun.env).toEqual({
      DAISYCLAW_PLUGIN_INSPECTOR_ROOT: ".artifacts/plugin-inspector",
      DAISYCLAW_PLUGIN_INSPECTOR_VERSION: "0.3.10",
    });
    expect(inspectorRun.run).toContain("extensions/");
    expect(inspectorRun.run).toContain(
      'npm exec --yes "@daisyclaw/plugin-inspector@${DAISYCLAW_PLUGIN_INSPECTOR_VERSION}" -- ci',
    );
  ...
  it("keeps release-check reruns independent while cancelling superseded umbrella runs", () => {
    const releaseChecksWorkflow = parse(
      readFileSync(".github/workflows/daisyclaw-release-checks.yml", "utf8"),
    );
    const fullReleaseWorkflow = readFullReleaseValidationWorkflow();

    expect(releaseChecksWorkflow.concurrency).toEqual({
      group:
        "daisyclaw-release-checks-${{ inputs.expected_sha || inputs.ref }}-${{ inputs.rerun_group }}",
    ...
    expect(dockerPreflightStep?.run).toContain(
      '--build-arg DAISYCLAW_EXTENSIONS="diagnostics-otel,codex"',
    );
    expect(
  ...
  it("keeps runtime tool coverage blocking in release checks", () => {
    const releaseChecksSource = readFileSync(
      ".github/workflows/daisyclaw-release-checks.yml",
      "utf8",
    );
    ...
    expect(runtimeToolCoverage.steps).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          run: expect.stringContaining("pnpm daisyclaw qa coverage"),
        }),
      ]),
    );
    expect(runtimeToolCoverage.steps).toEqual(
  ...
  it("keeps the live-ish availability check redacted", () => {
    ...
    expect(output).toContain("channel-discord: present (DISCORD_TOKEN, DAISYCLAW_DISCORD_TOKEN)");
