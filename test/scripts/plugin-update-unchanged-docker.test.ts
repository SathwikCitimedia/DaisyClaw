...
function runProbe(command: string, payload: unknown): void {
  const root = mkdtempSync(path.join(tmpdir(), "daisyclaw-plugin-update-probe-"));
...
describe("plugin update unchanged Docker E2E", () => {
  it("seeds current plugin install ledger state before checking config stability", () => {
    ...
    expect(probe).toContain("writeJson(process.env.DAISYCLAW_CONFIG_PATH, { plugins: {} });");
    expect(probe).not.toContain(
      "writeJson(process.env.DAISYCLAW_CONFIG_PATH, { plugins: { installs",
    );
  ...
  it("bounds the update command and prints diagnostics on hangs", () => {
    ...
    expect(script).toContain("DAISYCLAW_PLUGIN_UPDATE_TIMEOUT_SECONDS");
    expect(script).toContain(
      'daisyclaw_e2e_maybe_timeout "${plugin_update_timeout_seconds}s" node "$entry" plugins update',
    );
    ...
    expect(script).toContain("daisyclaw_e2e_print_log /tmp/plugin-update-output.log");
    expect(script).toContain("daisyclaw_e2e_print_log /tmp/daisyclaw-e2e-registry.log");
    expect(script).not.toContain("cat /tmp/plugin-update-output.log");
    expect(script).not.toContain("cat /tmp/daisyclaw-e2e-registry.log");
  });
  ...
  it("waits for the local registry process during cleanup", () => {
    ...
    expect(script).toContain('daisyclaw_e2e_stop_process "${registry_pid:-}"');
  ...
  it("bounds corrupt plugin update commands and prints diagnostics on hangs", () => {
    ...
    expect(script).toContain("DAISYCLAW_UPDATE_CORRUPT_PLUGIN_TIMEOUT_SECONDS");
    expect(
      script.match(/daisyclaw_e2e_maybe_timeout "\$\{update_timeout_seconds\}s" \\/gu)?.length,
    ).toBe(2);
    expect(script).toContain("--channel beta");
    expect(script).toContain("DAISYCLAW_UPDATE_POST_CORE=1");
    expect(script).not.toContain(
      'node "$entry" update --channel beta --tag "${DAISYCLAW_CURRENT_PACKAGE_TGZ',
    );
    expect(script).toContain(
      "daisyclaw update failed or timed out after ${update_timeout_seconds}s",
    );
    expect(script).toContain(
      "updated DaisyClaw entry failed or timed out after ${update_timeout_seconds}s",
    );
    expect(script.match(/daisyclaw_e2e_print_log \/tmp\/daisyclaw-update-corrupt-/g)).toHaveLength(8);
    expect(script).not.toContain("cat /tmp/daisyclaw-update-corrupt-");
  });
        ...
        outcomes: [
          {
            ...
            status: "skipped",
            message: `Disabled "${CORRUPT_PLUGIN_ID}" after plugin update failure; DaisyClaw will continue without it. Failed to update ${CORRUPT_PLUGIN_ID}: registry timeout`,
          },
        ],
      },
    };
        ...
        warnings: [
          {
            ...
            message:
              ...
              `Run daisyclaw plugins inspect ${CORRUPT_PLUGIN_ID} --runtime --json for details.`,
          },
        ],
      }),
    ).not.toThrow();
