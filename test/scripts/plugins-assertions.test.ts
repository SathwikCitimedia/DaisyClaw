...
function writeFixtureServerShims(binDir: string, pidPath: string): void {
  ...
  writeFileSync(
    ...
    [
      "#!/bin/bash",
      'printf "%s\\n" "$$" >"$DAISYCLAW_TEST_FIXTURE_SERVER_PID"',
      "trap 'exit 0' TERM",
...
describe("plugins Docker assertions", () => {
  it("rejects loose ClawHub preflight limits instead of parsing prefixes", () => {
    const timeoutResult = spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "clawhub-preflight"], {
      ...
      env: {
        ...process.env,
        CLAWHUB_PLUGIN_SPEC: "clawhub:@daisyclaw/kitchen-sink",
        DAISYCLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: "1e3",
      },
    });
    expect(timeoutResult.status).not.toBe(0);
    expect(timeoutResult.stderr).toContain(
      "invalid DAISYCLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: 1e3",
    );
    ...
    const bodyLimitResult = spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "clawhub-preflight"], {
      ...
      env: {
        ...process.env,
        CLAWHUB_PLUGIN_SPEC: "clawhub:@daisyclaw/kitchen-sink",
        DAISYCLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_BODY_MAX_BYTES: "1000bytes",
      },
    });
    expect(bodyLimitResult.status).not.toBe(0);
    expect(bodyLimitResult.stderr).toContain(
      "invalid DAISYCLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_BODY_MAX_BYTES: 1000bytes",
    );
  });
  ...
  it("keeps sweep artifact paths aligned with the assertion scratch root", () => {
    ...
    for (const scriptPath of scripts) {
      ...
      const scriptWithoutDefaultScratch = script.replace(
        'mktemp -d "/tmp/daisyclaw-plugins.XXXXXX"',
        "",
      );
      expect(script).toContain("DAISYCLAW_PLUGINS_TMP_DIR");
      expect(scriptWithoutDefaultScratch).not.toMatch(
        /\/tmp\/(?:plugins|marketplace|demo-plugin|is-number|daisyclaw-plugin|daisyclaw-clawhub)/,
      );
    }
  });

  it("cleans the default plugin sweep scratch root", () => {
    const root = mkdtempSync(path.join(tmpdir(), "daisyclaw-plugin-sweep-cleanup-"));
    const marker = path.join(root, "scratch-path.txt");
    try {
      const result = runPluginsSweepShell(
        `
set -euo pipefail
export DAISYCLAW_PLUGINS_SWEEP_SOURCE_ONLY=1
source scripts/e2e/lib/plugins/sweep.sh
printf '%s\\n' "$DAISYCLAW_PLUGINS_TMP_DIR" > "$MARKER"
test -d "$DAISYCLAW_PLUGINS_TMP_DIR"
cleanup_daisyclaw_plugins_sweep
test ! -e "$DAISYCLAW_PLUGINS_TMP_DIR"
`,
        { MARKER: marker },
      );
      ...
      expect(scratchRoot).toContain("/tmp/daisyclaw-plugins.");
  ...
  it("preserves caller-provided plugin sweep scratch roots", () => {
    const root = mkdtempSync(path.join(tmpdir(), "daisyclaw-plugin-sweep-caller-"));
...
set -euo pipefail
export DAISYCLAW_PLUGINS_SWEEP_SOURCE_ONLY=1
export DAISYCLAW_PLUGINS_TMP_DIR="$SCRATCH_ROOT"
source scripts/e2e/lib/plugins/sweep.sh
test -d "$DAISYCLAW_PLUGINS_TMP_DIR"
cleanup_daisyclaw_plugins_sweep
test -d "$DAISYCLAW_PLUGINS_TMP_DIR"
`,
  ...
  it("scans plugin assertion logs without echoing whole files on failure", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "daisyclaw-plugin-update-log-"));
    try {
      ...
      const pass = await runAssertionAsync(["plugin-dir-update-skipped"], {
        DAISYCLAW_PLUGINS_TMP_DIR: passRoot,
      });
      ...
      const fail = await runAssertionAsync(["plugin-dir-update-skipped"], {
        DAISYCLAW_PLUGINS_TMP_DIR: failRoot,
      });
      ...
      writeFileSync(
        path.join(invalidRoot, "plugins-invalid-daisyclaw-extensions.log"),
        `daisyclaw.extensions[1]\n${"x".repeat(256 * 1024)}\nmissing validation tail`,
        "utf8",
      );
      writeJson(path.join(invalidRoot, "plugins-invalid-daisyclaw-extensions-list.json"), {
        plugins: [],
      });
      const invalid = await runAssertionAsync(["invalid-daisyclaw-extensions"], {
        HOME: invalidHome,
        DAISYCLAW_PLUGINS_TMP_DIR: invalidRoot,
      });
  ...
  it("cleans npm fixture registry children when readiness times out", () => {
    const root = mkdtempSync(path.join(tmpdir(), "daisyclaw-plugin-npm-fixture-cleanup-"));
    try {
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            DAISYCLAW_TEST_FIXTURE_SERVER_PID: pidPath,
  ...
  it("bounds npm fixture registry logs when readiness fails", () => {
    const root = mkdtempSync(path.join(tmpdir(), "daisyclaw-plugin-npm-fixture-log-"));
    try {
      const binDir = path.join(root, "bin");
      const fixtureDir = path.join(root, "fixture");
      mkdirSync(fixtureDir);
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            DAISYCLAW_DOCKER_E2E_LOG_PRINT_BYTES: "80",
            PATH: `${binDir}${path.delimiter}/usr/bin${path.delimiter}/bin`,
          },
        },
      );
  ...
  it("cleans ClawHub fixture children when readiness times out", () => {
    const root = mkdtempSync(path.join(tmpdir(), "daisyclaw-plugin-clawhub-fixture-cleanup-"));
    try {
      const binDir = path.join(root, "bin");
      const cleanupPath = path.join(root, "caller-cleanup");
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            DAISYCLAW_PLUGINS_E2E_LIVE_CLAWHUB: "0",
            DAISYCLAW_PLUGINS_TMP_DIR: tmpDir,
            DAISYCLAW_TEST_FIXTURE_SERVER_PID: pidPath,
            PATH: `${binDir}${path.delimiter}/usr/bin${path.delimiter}/bin`,
          },
        },
  ...
  it("bounds ClawHub fixture server logs when readiness fails", () => {
    const root = mkdtempSync(path.join(tmpdir(), "daisyclaw-plugin-clawhub-fixture-log-"));
    try {
      const binDir = path.join(root, "bin");
      const tmpDir = path.join(root, "scratch");
      mkdirSync(tmpDir);
  ...
  it("uses the configured scratch root and resolves Windows home-relative install paths", () => {
    const root = mkdtempSync(path.join(tmpdir(), "daisyclaw-plugins-assertions-"));
    ...
    try {
      ...
      writeJson(path.join(home, ".daisyclaw", "plugins", "installs.json"), {
        installRecords: {
          "demo-plugin-tgz": {
            source: "archive",
      ...
      const result = spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "plugin-tgz"], {
        ...
        env: {
          ...
          HOME: home,
          DAISYCLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: "1e3",
          DAISYCLAW_PLUGINS_TMP_DIR: scratchRoot,
        },
      });
  ...
  it("compares local plugin source paths by canonical path", () => {
    ...
    const installPath = path.join(home, ".daisyclaw", "extensions", "demo-plugin-dir");
  ...
  it("still requires archive managed install directories to be removed", () => {
    ...
    const home = path.join(root, "home");
    const scratchRoot = path.join(root, "scratch");
    const installPath = path.join(home, ".daisyclaw", "extensions", "demo-plugin-tgz");
    mkdirSync(installPath, { recursive: true });
  ...
  it("rejects unreadable config during plugin uninstall proof", () => {
    ...
    const home = path.join(root, "home");
    const scratchRoot = path.join(root, "scratch");
    const removedInstallPath = path.join(home, ".daisyclaw", "extensions", "demo-plugin-tgz");
    ...
    try {
      ...
      writeFileSync(path.join(home, ".daisyclaw", "daisyclaw.json"), "{ malformed\n", "utf8");
      ...
      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain("failed to read DaisyClaw config");
    } finally {
      rmSync(root, { force: true, recursive: true });
    }
  });
  ...
  it("times out stalled ClawHub package metadata requests", async () => {
    ...
    try {
      ...
      const result = await runAssertionAsync(["clawhub-preflight"], {
        CLAWHUB_PLUGIN_ID: "daisyclaw-kitchen-sink-fixture",
        CLAWHUB_PLUGIN_SPEC: "clawhub:@daisyclaw/kitchen-sink",
        DAISYCLAW_CLAWHUB_URL: `http://127.0.0.1:${address.port}`,
        DAISYCLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: "25",
      });

      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain(
        "ClawHub package preflight for @daisyclaw/kitchen-sink timed out after 25ms",
      );
    } finally {
      await new Promise<void>((resolve) => {
  ...
  it("times out stalled ClawHub package metadata bodies", async () => {
    ...
    try {
      ...
      const result = await runAssertionAsync(["clawhub-preflight"], {
        ...
        CLAWHUB_PLUGIN_SPEC: "clawhub:@daisyclaw/kitchen-sink",
        DAISYCLAW_CLAWHUB_URL: `http://127.0.0.1:${address.port}`,
        DAISYCLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: "75",
      });

      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain(
        "ClawHub package preflight response for @daisyclaw/kitchen-sink timed out after 75ms",
      );
    } finally {
      await new Promise<void>((resolve) => {
        server.close(() => resolve());
  ...
  it("bounds ClawHub package metadata response bodies", async () => {
    ...
    try {
      ...
      const result = await runAssertionAsync(["clawhub-preflight"], {
        ...
        CLAWHUB_PLUGIN_SPEC: "clawhub:@daisyclaw/kitchen-sink",
        DAISYCLAW_CLAWHUB_URL: `http://127.0.0.1:${address.port}`,
        DAISYCLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_BODY_MAX_BYTES: "16",
        DAISYCLAW_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: "1000",
      });

      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain(
        "ClawHub package preflight response for @daisyclaw/kitchen-sink response body exceeded 16 bytes",
      );
