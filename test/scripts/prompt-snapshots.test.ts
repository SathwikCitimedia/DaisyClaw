...
describe("happy path prompt snapshots", () => {
  ...
  it("deletes stale generated snapshot artifacts", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-prompt-snapshot-stale-"));
    try {
  ...
  it("renders the Codex model-bound prompt layers", async () => {
    ...
    expect(telegram).toContain("DaisyClaw runtime context for this turn:");
  ...
  it("keeps heartbeat guidance in heartbeat collaboration mode only", async () => {
    ...
    const agentSoulHeading = "## DaisyClaw Agent Soul";
    ...
    expect(direct).not.toContain("This is an DaisyClaw heartbeat turn.");
    expect(group).not.toContain("This is an DaisyClaw heartbeat turn.");

    expect(heartbeat).toContain('"collaborationMode": {');
    expect(heartbeat).toContain('"developer_instructions": "This is an DaisyClaw heartbeat turn.');
    expect(heartbeat).toContain(agentSoulHeading);
    const daisyClawRuntimeInstructions = renderedPromptSection(
      heartbeat,
    ...
    expect(daisyClawRuntimeInstructions).not.toContain(heartbeatPhrase);
    ...
    expect(collaborationModeInstructions).toContain(
      "/tmp/daisyclaw-happy-path/workspace/HEARTBEAT.md",
    );
  ...
  it("finds the first available default Codex model catalog source", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-codex-catalog-"));
    try {
  ...
  it("skips Codex model prompt fixture sync when no default catalog exists", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-codex-catalog-missing-"));
    const chunks: string[] = [];
    try {
