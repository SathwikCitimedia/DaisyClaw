...
describe("qa-otel-smoke receiver bounds", () => {
  ...
  beforeAll(() => {
    configuredBodyLimitLoad = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          DAISYCLAW_QA_OTEL_MAX_CAPTURED_BODY_TEXT_BYTES: "1024",
          DAISYCLAW_QA_OTEL_MAX_COMPRESSED_BODY_BYTES: "2048",
          DAISYCLAW_QA_OTEL_MAX_DECODED_BODY_BYTES: "4096",
        },
      },
    );
  });
  ...
  it("times out and kills a wedged QA suite child with a detached gateway", async () => {
    ...
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "daisyclaw-qa-otel-child-"));
    ...
    try {
      ...
      await expect(testing.waitForChild(child, 100, 100)).rejects.toThrow(
        "daisyclaw qa suite timed out after 100ms",
      );
  ...
  it("cleans Docker collector containers and temp config after readiness failures", async () => {
    const tempRoot = mkdtempSync(path.join(os.tmpdir(), "daisyclaw-qa-otel-collector-"));
    ...
    try {
      ...
      expect(stopDockerContainer).toHaveBeenCalledWith(
        "daisyclaw-otel-smoke-00000000-0000-4000-8000-000000000000",
      );
  ...
  it("reports bounded Docker collector output when readiness exits", async () => {
    const tempRoot = mkdtempSync(path.join(os.tmpdir(), "daisyclaw-qa-otel-collector-output-"));
    const collectorDir = path.join(tempRoot, "collector");
    const child = new EventEmitter() as EventEmitter & {
      stderr: EventEmitter;
      stdout: EventEmitter;
