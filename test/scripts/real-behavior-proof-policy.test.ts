...
function proofBody(evidence: string, overrides: Record<string, string> = {}) {
  const fields = {
    ...
    environment: "macOS 15.4, Node 24, local DaisyClaw gateway with a redacted Discord token.",
    steps: "pnpm daisyclaw gateway restart, then pnpm daisyclaw gateway status",
    evidence,
...
describe("real-behavior-proof-policy", () => {
  it.each([
    ...
    "Linked artifact: https://github.com/daisyclaw/daisyclaw/actions/runs/123456789/artifacts/987654321",
    "Redacted runtime log: gateway connected Discord channel and delivered the reply.",
    ["Terminal transcript:", "```text", "$ daisyclaw gateway status", "discord ready", "```"].join(
      "\n",
    ),
  ...
  it("keeps proof fields after Markdown headings copied inside fenced output", () => {
    ...
    expect(evaluation.fields?.evidence).toContain("daisyclaw gateway status");
    expect(evaluation.status).toBe("passed");
  ...
  it("uses the latest real behavior proof section when duplicates exist", () => {
    ...
    expect(laterValid.fields?.evidence).toContain("daisyclaw doctor --non-interactive");
  ...
  it("accepts out-of-scope follow-ups as not-tested proof detail", () => {
    const body = [
      ...
      "  - `lowSupported: true`",
      "  - `fallbackFromLow: low`",
      "  - `local command version: DaisyClaw 2026.5.21`",
      "",
  ...
  it("accepts source PR proof when explicit gaps live in out-of-scope follow-ups", () => {
    const body = [
      ...
      "- Real environment tested: Local macOS source checkout, Node v24.8.0, DaisyClaw 2026.5.21 (c8a35c4), local `daisyclaw` shim pointed at the freshly built checkout. No channel credentials or provider API keys were used.",
      "- Exact steps or command run after this patch:",
      "  1. Built the local checkout with `node scripts/build-all.mjs`.",
      "  2. Updated `/Users/example/.local/bin/daisyclaw` to run this checkout's `daisyclaw.mjs` and verified `/Users/example/.local/bin/daisyclaw --version`.",
      ...
      "  - `lowSupported: true`",
      "  - `fallbackFromLow: low`",
      "  - `local command version: DaisyClaw 2026.5.21 (c8a35c4)`",
      "",
      "## Out-of-scope Follow-ups",
  ...
  it("accepts exact DaisyClaw ClawSweeper bot pass verdict markers when GitHub omits the app source", () => {
    const pullRequest = {
      number: 83581,
      head: {
        sha: "06ee95df6608d29a395c52ba8ab53fdd93a9dc4f",
    ...
    const comments = [
      {
        user: {
          login: "daisyclaw-clawsweeper[bot]",
          type: "Bot",
        },
        body: "<!-- clawsweeper-verdict:pass item=83581 sha=06ee95df6608d29a395c52ba8ab53fdd93a9dc4f confidence=high -->",
      },
...
describe("isMaintainerTeamMember", () => {
  ...
  it("returns true for active members", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "https://api.github.com/orgs/daisyclaw/teams/maintainer/memberships/private-maint",
