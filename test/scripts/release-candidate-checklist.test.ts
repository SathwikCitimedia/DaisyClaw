...
describe("release candidate checklist", () => {
  ...
  it("rejects selected plugin publish scope for release candidates", () => {
    ...
    ).toThrow("release candidates publish DaisyClaw with --plugin-publish-scope all-publishable");
  });

  it("extracts a workflow run id from gh dispatch output", () => {
    expect(
      parseRunIdFromDispatchOutput(
        "https://github.com/daisyclaw/daisyclaw/actions/runs/25922042055\n",
      ),
    ).toBe("25922042055");
  });
  ...
  it("falls back to a single compatible artifact from the same run", () => {
    expect(
      resolveArtifactName(
        [{ name: "daisyclaw-npm-preflight-dba00", expired: false }],
        "daisyclaw-npm-preflight-v2026.5.16-beta.2",
        "daisyclaw-npm-preflight-",
      ),
    ).toBe("daisyclaw-npm-preflight-dba00");
  });
  ...
  it("bounds GitHub API requests with a timeout signal", async () => {
    ...
    await expect(
      githubApi("repos/daisyclaw/daisyclaw/actions/runs", {
        fetchImpl,
        timeoutMs: 1234,
        token: "test-token",
      }),
    ...
    expect(fetchImpl).toHaveBeenCalledWith(
      "https://api.github.com/repos/daisyclaw/daisyclaw/actions/runs",
  ...
  it("includes the GitHub API path when a request times out", async () => {
    ...
    await expect(
      githubApi("repos/daisyclaw/daisyclaw/actions/runs/123/jobs", {
        fetchImpl,
        timeoutMs: 5,
        token: "test-token",
      }),
    ).rejects.toThrow(
      "GitHub API repos/daisyclaw/daisyclaw/actions/runs/123/jobs timed out after 5ms",
    );
  });
});
