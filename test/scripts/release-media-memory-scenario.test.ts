...
describe("release media memory scenario", () => {
  it("fails when packaged plugin listing is broken or omits memory-core", () => {
    ...
    const listIndex = script.indexOf('daisyclaw plugins list --json >"$PLUGINS_JSON"');
  ...
  it("uses a per-run temp root for generated media artifacts", () => {
    ...
    expect(script).toContain('media_root="$(mktemp -d /tmp/daisyclaw-release-media-memory.XXXXXX)"');
    ...
    expect(script).not.toContain("/tmp/daisyclaw-release-media-memory-plugins.json");
    expect(script).not.toContain(
      'mkdir -p "$DAISYCLAW_STATE_DIR/workspace/memory" /tmp/daisyclaw-release-media-memory',
    );
  });
});
