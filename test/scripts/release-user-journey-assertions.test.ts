...
describe("release user journey assertions", () => {
  it("scans large files when checking release user journey output text", () => {
    const root = mkdtempSync(path.join(tmpdir(), "daisyclaw-release-user-assertions-"));
  ...
  it("fails when uninstall leaves the managed plugin directory behind", () => {
    ...
    const installPath = path.join(home, ".daisyclaw", "extensions", pluginId);
    const installPathFile = path.join(root, "install-path.txt");

    try {
      writeJson(path.join(home, ".daisyclaw", "daisyclaw.json"), {
        plugins: {
          entries: {},
          allow: [],
          deny: [],
  ...
  it("passes after uninstall clears config, records, and managed files", () => {
    ...
    try {
      ...
      writeFileSync(
        installPathFile,
        path.join(home, ".daisyclaw", "extensions", "journey-plugin-a"),
        "utf8",
      );
  ...
  it("accepts ready ClickClack fixture state", async () => {
    ...
    try {
      await expect(
        withEnv({ HOME: home, DAISYCLAW_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS: "1000" }, () =>
  ...
  it("bounds stalled ClickClack fixture HTTP probes", async () => {
    ...
    try {
      ...
      await expect(
        withEnv({ HOME: home, DAISYCLAW_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS: "100" }, () =>
          runReleaseUserJourneyAssertion("wait-clickclack-socket", [
            `http://127.0.0.1:${server.port}`,
            "0.2",
          ]),
  ...
  it("rejects loose HTTP timeout env values instead of parsing prefixes", async () => {
    ...
    try {
      await expect(
        withEnv({ HOME: home, DAISYCLAW_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS: "100ms" }, () =>
          runReleaseUserJourneyAssertion("wait-clickclack-socket", [
            `http://127.0.0.1:${server.port}`,
            "0.2",
          ]),
        ...
        'DAISYCLAW_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS must be a positive integer. Got: "100ms"',
      );
    } finally {
      await server.stop();
      rmSync(root, { force: true, recursive: true });
    ...
    try {
      await expect(
        withEnv(
          {
            HOME: home,
            DAISYCLAW_RELEASE_USER_JOURNEY_HTTP_BODY_MAX_BYTES: "16",
            DAISYCLAW_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS: "1000",
          },
          () =>
    ...
    try {
      await expect(
        withEnv(
          {
            HOME: home,
            DAISYCLAW_RELEASE_USER_JOURNEY_HTTP_BODY_MAX_BYTES: "16bytes",
            DAISYCLAW_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS: "1000",
          },
          () =>
            runReleaseUserJourneyAssertion("post-clickclack-inbound", [
        ...
        'DAISYCLAW_RELEASE_USER_JOURNEY_HTTP_BODY_MAX_BYTES must be a positive integer. Got: "16bytes"',
      );
    } finally {
      await server.stop();
      rmSync(root, { force: true, recursive: true });
