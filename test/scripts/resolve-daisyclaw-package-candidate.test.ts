// Resolve Daisyclaw Package Candidate tests cover resolve daisyclaw package candidate script behavior.
...
} from "../../scripts/resolve-daisyclaw-package-candidate.mjs";
...
describe("resolve-daisyclaw-package-candidate", () => {
  it("accepts only DaisyClaw release package specs for npm candidates", () => {
    for (const spec of [
      "daisyclaw@beta",
      "daisyclaw@alpha",
      "daisyclaw@latest",
      "daisyclaw@2026.4.27",
      "daisyclaw@2026.4.27-1",
      "daisyclaw@2026.4.27-beta.2",
      "daisyclaw@2026.4.27-alpha.2",
    ]) {
      expect(validateDaisyClawPackageSpec(spec), spec).toBeUndefined();
    }

    expect(() => validateDaisyClawPackageSpec("@evil/daisyclaw@1.0.0")).toThrow(
      "package_spec must be daisyclaw@alpha",
    );
    expect(() => validateDaisyClawPackageSpec("daisyclaw@canary")).toThrow(
      "package_spec must be daisyclaw@alpha",
    );
    expect(() => validateDaisyClawPackageSpec("daisyclaw@2026.04.27")).toThrow(
      "package_spec must be daisyclaw@alpha",
    );
    expect(() => validateDaisyClawPackageSpec("daisyclaw@npm:other-package")).toThrow(
      "package_spec must be daisyclaw@alpha",
    );
    expect(() => validateDaisyClawPackageSpec("daisyclaw@file:../other-package.tgz")).toThrow(
      "package_spec must be daisyclaw@alpha",
    );
  });
  ...
  it("parses optional empty workflow inputs without rejecting the command line", () => {
    expect(
      ...
      outputName: "daisyclaw-current.tgz",
      packageSha256: "",
      packageRef: "release/2026.4.27",
      packageSpec: "daisyclaw@beta",
      packageUrl: "",
      source: "npm",
      trustedSourceId: "",
  ...
  it("resolves npm package candidates through the Windows npm.cmd toolchain shim", () => {
    ...
    const runner = resolveNpmPackageCandidatePackRunner(
      "daisyclaw@2026.5.26-beta.1",
      "C:\\daisyclaw\\.artifacts\\docker-e2e-package",
      {
    ...
    expect(runner).toEqual({
      ...
      args: [
        ...
        "/s",
        "/c",
        `${npmCmdPath} pack daisyclaw@2026.5.26-beta.1 --ignore-scripts --json --pack-destination C:\\daisyclaw\\.artifacts\\docker-e2e-package`,
      ],
      shell: false,
  ...
  it("kills timed-out package runner process groups", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "daisyclaw-package-runner-timeout-"));
    tempDirs.push(dir);
  ...
  it("forwards external termination to package runner process groups", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "daisyclaw-package-runner-signal-"));
    tempDirs.push(dir);
    const childPidPath = path.join(dir, "child.pid");
    const scriptUrl = pathToFileURL(
      path.resolve("scripts/resolve-daisyclaw-package-candidate.mjs"),
    ).href;
    let childPid: number | undefined;
  ...
  it("fails successful ref candidates when package source worktree cleanup fails", async () => {
    await expect(
      cleanupPackageSourceWorktreeForTest("/tmp/daisyclaw-package-source-stuck", {
        runImpl: async () => {
  ...
  it("preserves original ref candidate failures when worktree cleanup also fails", async () => {
    ...
    expect(warnings).toEqual([
      "warning: failed to remove temporary package source worktree /tmp/daisyclaw-package-source-stuck: worktree remove denied",
    ]);
  });

  it("loads named trusted package URL source policies", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "daisyclaw-trusted-package-source-"));
    tempDirs.push(dir);
      ...
      JSON.stringify({
        ...
        sources: {
          "enterprise-artifactory": {
            ...
            pathPrefixes: ["/artifactory/daisyclaw/"],
            ports: [443, 8443],
  ...
  it("rejects unsafe package_url downloads before fetching private targets", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "daisyclaw-package-download-"));
    tempDirs.push(dir);
    const target = path.join(dir, "daisyclaw.tgz");

    await expect(
      downloadUrl("http://packages.example/daisyclaw.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "93.184.216.34", family: 4 }]),
      }),
    ).rejects.toThrow("package_url must use https");
    await expect(
      downloadUrl("https://user@packages.example/daisyclaw.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "93.184.216.34", family: 4 }]),
      }),
    ).rejects.toThrow("package_url must not include credentials");
    await expect(
      downloadUrl("https://localhost/daisyclaw.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "127.0.0.1", family: 4 }]),
      }),
    ).rejects.toThrow(/private\/internal\/special-use/iu);
    await expect(
      downloadUrl("https://packages.example/daisyclaw.tgz", target, {
        fetchImpl: unexpectedFetch,
  ...
  it("allows private package_url downloads only through an explicit trusted source policy", async () => {
    ...
    await downloadUrl("https://packages.internal:8443/artifactory/daisyclaw/daisyclaw.tgz", target, {
    ...
    expect(requestedUrls).toEqual([
      "https://packages.internal:8443/artifactory/daisyclaw/daisyclaw.tgz",
    ]);
    await expect(readFile(target)).resolves.toEqual(Buffer.from([4, 5, 6]));

    await expect(
      downloadUrl("https://evil.internal:8443/artifactory/daisyclaw/daisyclaw.tgz", target, {
        fetchImpl: unexpectedFetch,
    ...
    await expect(
      downloadUrl("https://packages.internal:8443/other/daisyclaw.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "10.0.0.8", family: 4 }]),
        trustedSource,
      }),
  ...
  it("keeps trusted package_url redirects inside the named source policy", async () => {
    ...
    await expect(
      downloadUrl("https://packages.internal:8443/artifactory/daisyclaw/daisyclaw.tgz", target, {
        fetchImpl: async () =>
          new Response(null, {
            headers: { location: "https://metadata.internal:8443/artifactory/daisyclaw/pwn.tgz" },
            status: 302,
          }),
        lookupHost: lookupAddresses([{ address: "10.0.0.8", family: 4 }]),
        trustedSource,
  ...
  it("validates redirects for package_url downloads", async () => {
    ...
    expect(requestedUrls).toEqual(["https://packages.example/daisyclaw.tgz"]);
  });
  ...
  it("bounds package_url downloads and writes completed files atomically", async () => {
    ...
    await downloadUrl("https://packages.example/daisyclaw.tgz", target, {
      fetchImpl: async () =>
  ...
  it("reads package source metadata from package artifacts", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "daisyclaw-package-candidate-"));
    tempDirs.push(dir);
    await writeFile(
  ...
  it("reads the source SHA from packed npm build metadata", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "daisyclaw-package-build-info-"));
    tempDirs.push(dir);
    const root = path.join(dir, "package");
    await mkdir(path.join(root, "dist"), { recursive: true });
    await writeFile(path.join(root, "package.json"), JSON.stringify({ name: "daisyclaw" }));
    await writeFile(
      path.join(root, "dist", "build-info.json"),
      JSON.stringify({ commit: "66CE632B9B7C5C7FDD3E66C739687D51638AD6E2" }),
    );
    const tarball = path.join(dir, "daisyclaw.tgz");
