...
function runGatewayPortCheck(fakeLsof: string) {
  const root = mkdtempSync(join(tmpdir(), "daisyclaw-restart-mac-test-"));
  tempRoots.push(root);
...
function runCleanupFunction(fakePs: string) {
  ...
  const cleanupFunction = script.slice(
    script.indexOf("kill_all_daisyclaw()"),
  ...
  writeFileSync(
    ...
    [
      ...
      'DEBUG_PROCESS_PATTERN="/worktree/apps/macos/.build/debug/DaisyClaw"',
      'LOCAL_PROCESS_PATTERN="/worktree/apps/macos/.build-local/debug/DaisyClaw"',
      'RELEASE_PROCESS_PATTERN="/worktree/apps/macos/.build/release/DaisyClaw"',
      "kill() {",
      '  printf "%s\\n" "$*" >> "$DAISYCLAW_TEST_KILL_CALLS"',
      "  return 0",
      "}",
      "kill_all_daisyclaw",
    ].join("\n"),
  ...
  const result = spawnSync("bash", [harnessPath], {
    ...
    env: {
      ...process.env,
      DAISYCLAW_TEST_KILL_CALLS: killCallsPath,
      PATH: `${binDir}:${process.env.PATH ?? ""}`,
    },
  });
...
describe("scripts/restart-mac.sh", () => {
  ...
  it("keeps the default restart log scoped to the current worktree lock", () => {
    ...
    expect(script).not.toContain('LOG_PATH="${DAISYCLAW_RESTART_LOG:-/tmp/daisyclaw-restart.log}"');
  });
  ...
  it("prefers the freshly packaged app unless an explicit app bundle is set", () => {
    ...
    expect(script).toContain('fail "DAISYCLAW_APP_BUNDLE does not exist: ${APP_BUNDLE}"');
    expect(chooseBlock).toContain("canonicalize_app_bundle");
    expect(chooseBlock.indexOf("${ROOT_DIR}/dist/DaisyClaw.app")).toBeGreaterThan(-1);
    expect(chooseBlock.indexOf("/Applications/DaisyClaw.app")).toBeGreaterThan(-1);
    expect(chooseBlock.indexOf("${ROOT_DIR}/dist/DaisyClaw.app")).toBeLessThan(
      chooseBlock.indexOf("/Applications/DaisyClaw.app"),
    );
  });

  it("keeps restart cleanup scoped to known DaisyClaw app and build paths", () => {
    const script = readFileSync(restartScriptPath, "utf8");
    ...
    expect(cleanupBlock).toContain('"/Applications/DaisyClaw.app/${APP_EXECUTABLE_RELATIVE_PATH}"');
    ...
    expect(cleanupBlock).not.toContain('pkill -x "DaisyClaw"');
    expect(cleanupBlock).not.toContain("pgrep");
    expect(cleanupBlock).not.toContain('pgrep -x "DaisyClaw"');
  });

  it("stops launchd supervision before killing app processes", () => {
    const script = readFileSync(restartScriptPath, "utf8");
    const stopIndex = script.indexOf("stop_launch_agent\nlog");
    const killIndex = script.indexOf("if ! kill_all_daisyclaw");
  ...
  it("normalizes custom app bundle paths before process matching", () => {
    const root = mkdtempSync(join(tmpdir(), "daisyclaw-restart-mac-test-"));
    tempRoots.push(root);
    const appBundle = join(root, "dist", "DaisyClaw.app");
    mkdirSync(appBundle, { recursive: true });

    const { result } = runCanonicalizeAppBundle(`${appBundle}/../DaisyClaw.app/`);
  ...
  it("fails restart cleanup when scoped processes survive every kill attempt", () => {
    const { killCalls, result } = runCleanupFunction(
      [
        "#!/usr/bin/env bash",
        "printf '%s\\n' '  321 /worktree/dist/DaisyClaw.app/Contents/MacOS/DaisyClaw --attach-only'",
      ].join("\n"),
    );
  ...
  it("passes restart cleanup when the final kill attempt clears the process", () => {
    const { killCalls, result } = runCleanupFunction(
      [
        "#!/usr/bin/env bash",
        'kill_count="$(wc -l < "$DAISYCLAW_TEST_KILL_CALLS" 2>/dev/null || echo 0)"',
  ...
  it("does not kill unrelated DaisyClaw app bundles", () => {
    const { killCalls, result } = runCleanupFunction(
      [
        "#!/usr/bin/env bash",
        "printf '%s\\n' '  654 /tmp/Other/DaisyClaw.app/Contents/MacOS/DaisyClaw'",
      ].join("\n"),
    );
