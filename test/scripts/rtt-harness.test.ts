...
const CHUNKED_PAYLOAD_MARKER = "__daisyclawQaCredentialPayloadChunksV1";
...
function credentialBrokerEnv(port: number) {
  return {
    ...process.env,
    DAISYCLAW_QA_ALLOW_INSECURE_HTTP: "1",
    DAISYCLAW_QA_CONVEX_SECRET_MAINTAINER: "test-secret",
    DAISYCLAW_QA_CONVEX_SITE_URL: `http://127.0.0.1:${port}`,
    DAISYCLAW_QA_CREDENTIAL_HTTP_TIMEOUT_MS: "1000",
    DAISYCLAW_QA_CREDENTIAL_OWNER_ID: "test-owner",
    DAISYCLAW_NPM_TELEGRAM_CREDENTIAL_ROLE: "maintainer",
  };
}

describe("RTT harness", () => {
  it("validates DaisyClaw package specs", () => {
    expect(validateDaisyClawPackageSpec("daisyclaw@main")).toBe("daisyclaw@main");
    expect(validateDaisyClawPackageSpec("daisyclaw@alpha")).toBe("daisyclaw@alpha");
    expect(validateDaisyClawPackageSpec("daisyclaw@beta")).toBe("daisyclaw@beta");
    expect(validateDaisyClawPackageSpec("daisyclaw@latest")).toBe("daisyclaw@latest");
    expect(validateDaisyClawPackageSpec("daisyclaw@2026.4.30")).toBe("daisyclaw@2026.4.30");
    expect(validateDaisyClawPackageSpec("daisyclaw@2026.4.30-beta.2")).toBe(
      "daisyclaw@2026.4.30-beta.2",
    );
    expect(validateDaisyClawPackageSpec("daisyclaw@2026.4.30-alpha.2")).toBe(
      "daisyclaw@2026.4.30-alpha.2",
    );

    expect(() => validateDaisyClawPackageSpec("@daisyclaw/daisyclaw@beta")).toThrow(
      /Package spec must be/,
    );
    expect(() => validateDaisyClawPackageSpec("daisyclaw@next")).toThrow(/Package spec must be/);
  });

  it("builds stable run labels", () => {
    expect(safeRunLabel("daisyclaw@beta")).toBe("daisyclaw_beta");
    expect(
      buildRunId({
    ...
    ).toBe("2026-05-01T030405678Z-daisyclaw_beta-2");
  });

  it("constructs harness env without dropping caller env", () => {
    const env = createHarnessEnv({
      baseEnv: {
        DAISYCLAW_QA_TELEGRAM_GROUP_ID: "-100123",
        DAISYCLAW_NPM_TELEGRAM_FAST: "0",
      },
    ...
    expect(env.DAISYCLAW_QA_TELEGRAM_GROUP_ID).toBe("-100123");
    expect(env.DAISYCLAW_NPM_TELEGRAM_PACKAGE_SPEC).toBe("daisyclaw@beta");
    expect(env.DAISYCLAW_NPM_TELEGRAM_PACKAGE_LABEL).toBe("daisyclaw@beta (2026.4.30-beta.1)");
    expect(env.DAISYCLAW_NPM_TELEGRAM_PROVIDER_MODE).toBe("mock-openai");
    expect(env.DAISYCLAW_NPM_TELEGRAM_SCENARIOS).toBe("telegram-mentioned-message-reply");
    expect(env.DAISYCLAW_NPM_TELEGRAM_OUTPUT_DIR).toBe(".artifacts/rtt/run/raw");
    expect(env.DAISYCLAW_NPM_TELEGRAM_FAST).toBe("0");
    expect(env.DAISYCLAW_NPM_TELEGRAM_WARM_SAMPLES).toBe("20");
    expect(env.DAISYCLAW_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS).toBe("30000");
    expect(env.DAISYCLAW_QA_TELEGRAM_CANARY_TIMEOUT_MS).toBe("180000");
    expect(env.DAISYCLAW_QA_TELEGRAM_SCENARIO_TIMEOUT_MS).toBe("180000");
  });

  it("forwards Convex credential controls without dropping RTT sample controls", () => {
    const env = createHarnessEnv({
      baseEnv: {
        DAISYCLAW_QA_CONVEX_SITE_URL: "https://qa-credentials.example.convex.site",
        DAISYCLAW_QA_CONVEX_SECRET_MAINTAINER: "maintainer-secret",
      },
    ...
    expect(env.DAISYCLAW_NPM_TELEGRAM_CREDENTIAL_SOURCE).toBe("convex");
    expect(env.DAISYCLAW_NPM_TELEGRAM_CREDENTIAL_ROLE).toBe("maintainer");
    expect(env.DAISYCLAW_NPM_TELEGRAM_WARM_SAMPLES).toBe("7");
    expect(env.DAISYCLAW_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS).toBe("45000");
    expect(() =>
  ...
  it("exports the Telegram bot token after Convex credentials are sourced", async () => {
    ...
    const tokenExportIndex = script.indexOf(
      'export TELEGRAM_BOT_TOKEN="${DAISYCLAW_QA_TELEGRAM_SUT_BOT_TOKEN:?missing DAISYCLAW_QA_TELEGRAM_SUT_BOT_TOKEN}"',
    );
    ...
    const bodyLimitForwardIndex = script.indexOf(
      "DAISYCLAW_QA_CREDENTIAL_HTTP_MAX_BODY_BYTES",
      installEnvSnapshotIndex,
    );
    const payloadByteLimitForwardIndex = script.indexOf(
      "DAISYCLAW_QA_CREDENTIAL_PAYLOAD_MAX_BYTES",
      installEnvSnapshotIndex,
    );
    const payloadChunkLimitForwardIndex = script.indexOf(
      "DAISYCLAW_QA_CREDENTIAL_PAYLOAD_MAX_CHUNKS",
      installEnvSnapshotIndex,
    );
    ...
    expect(script).not.toContain(
      "running package install without DAISYCLAW_E2E_NPM_INSTALL_TIMEOUT",
    );
    expect(script).toContain("run_logged docker_e2e_docker_run_cmd run --rm");
    expect(script).not.toContain("run_logged docker run --rm");
    expect(script).toContain("source scripts/lib/daisyclaw-e2e-instance.sh");
    ...
    expect(script).toContain('daisyclaw_e2e_print_log "$mock_log"');
    expect(script).toContain('daisyclaw_e2e_print_log "$file"');
    ...
    expect(script).not.toContain('export TELEGRAM_BOT_TOKEN="$DAISYCLAW_QA_TELEGRAM_SUT_BOT_TOKEN"');
  });
  ...
  it("keeps RTT Docker artifacts isolated by default", async () => {
    ...
    expect(script).toContain(
      'OUTPUT_DIR="${DAISYCLAW_NPM_TELEGRAM_OUTPUT_DIR:-.artifacts/qa-e2e/npm-telegram-rtt/$RUN_ID}"',
    );
    expect(script).toContain('-e DAISYCLAW_NPM_TELEGRAM_OUTPUT_DIR="$OUTPUT_DIR"');
    expect(script).not.toContain(
      'OUTPUT_DIR="${DAISYCLAW_NPM_TELEGRAM_OUTPUT_DIR:-.artifacts/qa-e2e/npm-telegram-rtt}"',
    );
  });
  ...
  it("bounds Convex credential broker response bodies", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-rtt-credentials-"));
      ...
      await execFileAsync(
        ...
        ],
        {
          env: {
            ...
            DAISYCLAW_QA_CREDENTIAL_HTTP_MAX_BODY_BYTES: "16",
          },
          maxBuffer: 128 * 1024,
        },
      );
  ...
  it("does not start another credential acquire after retry delay exhausts the deadline", async () => {
    ...
    const { port } = await listenOnLoopback(server);
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-rtt-credentials-retry-"));
    tempDirs.push(tempDir);
    ...
    try {
      await execFileAsync(
        ...
        ],
        {
          env: {
            ...credentialBrokerEnv(port),
            DAISYCLAW_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS: "75",
            DAISYCLAW_QA_CREDENTIAL_HTTP_TIMEOUT_MS: "250",
          },
          maxBuffer: 128 * 1024,
        },
      );
  ...
  it("caps credential acquire HTTP retries to the remaining acquire deadline", async () => {
    ...
    const { port } = await listenOnLoopback(server);
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-rtt-credentials-cap-"));
    tempDirs.push(tempDir);
    const startedAt = Date.now();
    ...
    try {
      await execFileAsync(
        ...
        ],
        {
          env: {
            ...credentialBrokerEnv(port),
            DAISYCLAW_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS: "100",
            DAISYCLAW_QA_CREDENTIAL_HTTP_TIMEOUT_MS: "900",
          },
          maxBuffer: 128 * 1024,
        },
      );
  ...
  it("generates final-only Telegram RTT delivery config for release packages", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-rtt-config-test-"));
    tempDirs.push(tempDir);
  ...
  it("builds normalized result JSON", async () => {
    ...
    expect(result).toStrictEqual({
      ...
      package: { spec: "daisyclaw@beta", version: "2026.4.30-beta.1" },
      run: {
        durationMs: 12_000,
  ...
  it("appends JSONL rows", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-rtt-test-"));
    tempDirs.push(tempDir);
  ...
  it("parses CLI options", () => {
    ...
    expect(parsed.spec).toBe("daisyclaw@latest");
    expect(parsed.options).toStrictEqual({
      packageTgz: "/tmp/daisyclaw.tgz",
      credentialRole: "ci",
      credentialSource: "convex",
  ...
  it("rejects missing CLI path option values", () => {
    for (const [flag, next] of [
      ...
      expect(() => cliTesting.parseArgs(["daisyclaw@latest", flag, next])).toThrow(
