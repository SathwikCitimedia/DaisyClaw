...
describe("run-opengrep.sh", () => {
  it("validates the rulepack when only OpenGrep rulepack files changed", () => {
    const repo = createTempDir("daisyclaw-run-opengrep-");
    git(repo, "init", "-q");
    ...
    execFileSync("bash", ["scripts/run-opengrep.sh", "--changed"], {
      ...
      env: {
        ...
        DAISYCLAW_OPENGREP_BASE_REF: "HEAD",
      },
      encoding: "utf8",
    });
  ...
  it("scans PR files instead of main-only files when the payload base is stale", () => {
    const repo = createTempDir("daisyclaw-run-opengrep-merge-");
    ...
    execFileSync("bash", ["scripts/run-opengrep.sh", "--changed"], {
      ...
      env: {
        ...process.env,
        PATH: `${binDir}${path.delimiter}${process.env.PATH ?? ""}`,
        DAISYCLAW_OPENGREP_BASE_REF: `${staleBase}...HEAD`,
        DAISYCLAW_OPENGREP_MERGE_HEAD_FIRST_PARENT: "1",
      },
      encoding: "utf8",
    });
