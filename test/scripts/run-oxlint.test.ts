...
describe("run-oxlint", () => {
  ...
  it("does not run package-boundary artifact prep twice in pnpm check", () => {
    ...
    expect(shardedLintRunner).toContain('DAISYCLAW_OXLINT_SKIP_PREPARE: "1"');
  });

  it("holds one parent heavy-check lock for sharded lint runs", () => {
    const shardedLintRunner = readFileSync("scripts/run-oxlint-shards.mjs", "utf8");
    const skipLockIndex = shardedLintRunner.indexOf('env.DAISYCLAW_OXLINT_SKIP_LOCK === "1"');
    const lockIndex = shardedLintRunner.indexOf("acquireLocalHeavyCheckLockSync({");
    const childSkipIndex = shardedLintRunner.indexOf('DAISYCLAW_OXLINT_SKIP_LOCK: "1"');
  ...
  it("keeps a serial oxlint shard path available", () => {
    ...
    expect(shardedLintRunner).toContain("DAISYCLAW_OXLINT_SHARDS_SERIAL");
  ...
  it("serializes broad oxlint shards on constrained CI hosts", () => {
    ...
    expect(
      shouldRunOxlintShardsSerial({
        env: { CI: "true", DAISYCLAW_LOCAL_CHECK_MODE: "throttled" },
        platform: "linux",
        hostResources: constrainedHost,
      }),
    ).toBe(true);
  ...
  it("keeps oxlint shards parallel for roomy CI and explicit full-speed runs", () => {
    ...
    expect(
      shouldRunOxlintShardsSerial({
        env: { DAISYCLAW_LOCAL_CHECK_MODE: "full" },
        platform: "linux",
        hostResources: constrainedHost,
      }),
    ).toBe(false);
  ...
  it("honors explicit oxlint shard serial overrides", () => {
    ...
    expect(
      shouldRunOxlintShardsSerial({
        env: { DAISYCLAW_OXLINT_SHARDS_SERIAL: "1", CI: "true" },
        platform: "linux",
        hostResources: roomyHost,
      }),
    ).toBe(true);
    expect(
      shouldRunOxlintShardsSerial({
        env: { DAISYCLAW_OXLINT_SHARDS_SERIAL: "0" },
        platform: "linux",
        hostResources: roomyHost,
      }),
    ).toBe(false);
    ...
    expect(
      resolveOxlintShardConcurrency({
        env: {
          ...
          DAISYCLAW_LOCAL_CHECK_MODE: "throttled",
        },
        platform: "linux",
        hostResources: roomyHost,
        splitCore: true,
  ...
  it("honors explicit oxlint shard concurrency overrides", () => {
    ...
    expect(
      resolveOxlintShardConcurrency({
        env: { CI: "true", DAISYCLAW_OXLINT_SHARD_CONCURRENCY: "2" },
        platform: "linux",
        hostResources: roomyHost,
        splitCore: true,
      }),
    ...
    expect(() =>
      resolveOxlintShardConcurrency({
        env: { CI: "true", DAISYCLAW_OXLINT_SHARD_CONCURRENCY: "2x" },
        platform: "linux",
        hostResources: roomyHost,
        splitCore: true,
      }),
    ).toThrow("DAISYCLAW_OXLINT_SHARD_CONCURRENCY must be a positive integer; got: 2x");
  });

  it("uses a bounded oxlint shard heartbeat by default", () => {
    expect(resolveShardHeartbeatMs({})).toBe(30_000);
    expect(resolveShardHeartbeatMs({ DAISYCLAW_OXLINT_SHARD_HEARTBEAT_MS: "0" })).toBe(0);
    expect(resolveShardHeartbeatMs({ DAISYCLAW_OXLINT_SHARD_HEARTBEAT_MS: "5000" })).toBe(5000);
    expect(() => resolveShardHeartbeatMs({ DAISYCLAW_OXLINT_SHARD_HEARTBEAT_MS: "5000ms" })).toThrow(
      "DAISYCLAW_OXLINT_SHARD_HEARTBEAT_MS must be a non-negative integer; got: 5000ms",
    );
  });

  it("uses a bounded oxlint shard timeout by default", () => {
    expect(resolveShardTimeoutMs({})).toBe(900_000);
    expect(resolveShardTimeoutMs({ DAISYCLAW_OXLINT_SHARD_TIMEOUT_MS: "0" })).toBe(0);
    expect(resolveShardTimeoutMs({ DAISYCLAW_OXLINT_SHARD_TIMEOUT_MS: "5000" })).toBe(5000);
    expect(() => resolveShardTimeoutMs({ DAISYCLAW_OXLINT_SHARD_TIMEOUT_MS: "1e3" })).toThrow(
      "DAISYCLAW_OXLINT_SHARD_TIMEOUT_MS must be a non-negative integer; got: 1e3",
    );
    expect(resolveShardKillGraceMs({})).toBe(5_000);
    expect(resolveShardKillGraceMs({ DAISYCLAW_OXLINT_SHARD_KILL_GRACE_MS: "0" })).toBe(0);
    expect(() => resolveShardKillGraceMs({ DAISYCLAW_OXLINT_SHARD_KILL_GRACE_MS: "-1" })).toThrow(
      "DAISYCLAW_OXLINT_SHARD_KILL_GRACE_MS must be a non-negative integer; got: -1",
    );
  });

  it("fails a stuck oxlint shard instead of waiting forever", async () => {
    const tempDir = mkdtempSync(join(tmpdir(), "daisyclaw-oxlint-shard-"));
    ...
    try {
      ...
      const status = await runShard({
        env: {
          ...process.env,
          DAISYCLAW_OXLINT_SHARD_HEARTBEAT_MS: "0",
          DAISYCLAW_OXLINT_SHARD_TIMEOUT_MS: "25",
          DAISYCLAW_OXLINT_SHARD_KILL_GRACE_MS: "25",
        },
        extraArgs: [],
        runner,
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const tempDir = mkdtempSync(join(tmpdir(), "daisyclaw-oxlint-signal-"));
      ...
      try {
        ...
        writeFileSync(
          ...
          [
            ...
            "    ...process.env,",
            "    DAISYCLAW_OXLINT_SHARD_HEARTBEAT_MS: '0',",
            "    DAISYCLAW_OXLINT_SHARD_TIMEOUT_MS: '0',",
            "  },",
            "  extraArgs: [],",
      ...
      try {
        ...
        writeFileSync(
          ...
          [
            ...
            "    DAISYCLAW_OXLINT_SHARD_HEARTBEAT_MS: '0',",
            "    DAISYCLAW_OXLINT_SHARD_TIMEOUT_MS: '0',",
            "    DAISYCLAW_OXLINT_SHARD_KILL_GRACE_MS: '250',",
            "  },",
            "  extraArgs: [],",
            "  runner: process.env.RUNNER_FILE,",
            "  shard: { name: 'signal-test', args: [] },",
  ...
  it("chunks extension oxlint shards on Windows", () => {
    const shards = createOxlintShards({
      cwd: "/repo",
      env: {
        DAISYCLAW_OXLINT_WINDOWS_EXTENSION_CHUNK_SIZE: "2",
      },
      platform: "win32",
      readDir: () =>
        [
  ...
  it("rejects invalid Windows oxlint extension chunk size overrides", () => {
    ...
    expect(() =>
      resolveWindowsExtensionChunkSize({ DAISYCLAW_OXLINT_WINDOWS_EXTENSION_CHUNK_SIZE: "0" }),
    ).toThrow("DAISYCLAW_OXLINT_WINDOWS_EXTENSION_CHUNK_SIZE must be a positive integer; got: 0");
    expect(() =>
      resolveWindowsExtensionChunkSize({
        DAISYCLAW_OXLINT_WINDOWS_EXTENSION_CHUNK_SIZE: "8 chunks",
      }),
    ).toThrow(
      "DAISYCLAW_OXLINT_WINDOWS_EXTENSION_CHUNK_SIZE must be a positive integer; got: 8 chunks",
    );
  });
