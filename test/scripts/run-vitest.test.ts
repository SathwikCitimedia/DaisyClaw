...
describe("scripts/run-vitest", () => {
  ...
  it("restores the workspace node_modules link from a hydrated pnpm modules directory", () => {
    ...
    expect(
      resolveVitestCliEntry({
        baseDir: "/repo",
        env: { PNPM_CONFIG_MODULES_DIR: "/runner/daisyclaw-pnpm-node-modules" },
        fsImpl: {
          existsSync: (filePath: string) =>
            filePath.replaceAll("\\", "/") ===
            "/runner/daisyclaw-pnpm-node-modules/vitest/package.json",
    ...
    expect(symlinks).toEqual([
      {
        target: "/runner/daisyclaw-pnpm-node-modules",
        path: "/runner/daisyclaw-pnpm-node-modules/node_modules",
        type: "junction",
      },
      {
        target: "/runner/daisyclaw-pnpm-node-modules",
  ...
  it("self-links hydrated pnpm modules when pnpm lowercases the env key", () => {
    ...
    expect(
      resolveVitestCliEntry({
        baseDir: "/repo",
        env: { npm_config_modules_dir: "/runner/daisyclaw-pnpm-node-modules" },
        fsImpl: {
          existsSync: (filePath: string) =>
            filePath.replaceAll("\\", "/") ===
            "/runner/daisyclaw-pnpm-node-modules/vitest/package.json",
        ...
        platform: "win32",
        requireResolve: () => "/runner/daisyclaw-pnpm-node-modules/vitest/package.json",
      }),
    ).toBe("/repo/node_modules/vitest/vitest.mjs");
    expect(symlinks).toEqual([
      {
  ...
  it("keeps tooling-excluded explicit tests on existing routing", () => {
    const argv = ["run", "test/scripts/daisyclaw-e2e-instance.test.ts"];
    expect(resolveImplicitVitestArgs(argv)).toBe(argv);
  });
  ...
  it("allows opting back into Maglev explicitly", () => {
    expect(
      resolveVitestNodeArgs({
        DAISYCLAW_VITEST_ENABLE_MAGLEV: "1",
        PATH: "/usr/bin",
      }),
    ).toStrictEqual([]);
  });
  ...
  it("parses the optional no-output timeout env", () => {
    ...
    expect(resolveVitestNoOutputTimeoutMs({ DAISYCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "2500" })).toBe(
      2500,
    );
    expect(
      resolveVitestNoOutputTimeoutMs({ DAISYCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "0" }),
    ).toBeNull();
  });
  ...
  it("defaults direct non-watch runs to the stall watchdog", () => {
    expect(resolveRunVitestSpawnEnv({ PATH: "/usr/bin" }, ["run"])).toEqual({
      PATH: "/usr/bin",
      DAISYCLAW_VITEST_NO_OUTPUT_HEARTBEAT_MS: "30000",
      DAISYCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "120000",
    });
    ...
    expect(
      resolveRunVitestSpawnEnv({ DAISYCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "0", PATH: "/usr/bin" }, [
        "run",
      ]),
    ).toEqual({
  ...
  it("uses a longer default stall watchdog for broad e2e and project shard configs", () => {
    ...
    for (const configArg of [
      ...
      expect(resolveRunVitestSpawnEnv({ PATH: "/usr/bin" }, ["run", configArg])).toEqual({
        PATH: "/usr/bin",
        DAISYCLAW_VITEST_NO_OUTPUT_HEARTBEAT_MS: "30000",
        DAISYCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: timeout,
      });
    }
    expect(
      resolveDefaultVitestNoOutputTimeoutMs([
  ...
  it("does not force the stall watchdog into delegated runner environments", () => {
    ...
    expect(
      resolveTestProjectsRunnerEnv({
        DAISYCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "2500",
        PATH: "/usr/bin",
      }),
    ).toEqual({
      DAISYCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "2500",
  ...
  posixIt("cleans delegated test-project children when the wrapper is signaled", async () => {
    ...
    const childPidPath = nodePath.join(
      os.tmpdir(),
      `daisyclaw-run-vitest-delegated-child-${process.pid}-${Date.now()}.pid`,
    );
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "  fs.writeFileSync(process.env.DAISYCLAW_DELEGATED_SIGNAL_CHILD_PID!, String(process.pid));",
        "  await new Promise(() => {});",
        "});",
        "",
      ].join("\n"),
    ...
    const runner = spawn(
      ...
      ["scripts/run-vitest.mjs", fixturePath, "--reporter=verbose"],
      {
        env: {
          ...process.env,
          DAISYCLAW_DELEGATED_SIGNAL_CHILD_PID: childPidPath,
        },
        stdio: "ignore",
      },
    );
  ...
  it("caps native Rust worker pools for serial Vitest runs", () => {
    expect(
      resolveVitestSpawnParams(
        {
          DAISYCLAW_TEST_PROJECTS_SERIAL: "1",
          PATH: "/usr/bin",
        },
        "darwin",
      ).env,
  ...
  it("keeps explicit native Rust worker pool settings", () => {
    expect(
      resolveVitestSpawnParams(
        {
          DAISYCLAW_VITEST_MAX_WORKERS: "2",
          PATH: "/usr/bin",
          RAYON_NUM_THREADS: "8",
  ...
  it("parses the optional watchdog heartbeat interval", () => {
    expect(
      resolveVitestNoOutputHeartbeatMs({ DAISYCLAW_VITEST_NO_OUTPUT_HEARTBEAT_MS: "120000" }),
    ).toBe(120000);
    expect(
      resolveVitestNoOutputHeartbeatMs({ DAISYCLAW_VITEST_NO_OUTPUT_HEARTBEAT_MS: "0" }),
    ).toBeNull();
  });
});
