...
function makeTempDir(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-secret-provider-proof-"));
  tempDirs.push(root);
  return root;
}

function writeStallingDaisyClaw(
  root: string,
  options: {
  ...
  const descendantScript = options.gatewayDescendantMarkerPath
    ...
    : "";
  const scriptPath = path.join(root, "fake-daisyclaw.mjs");
  fs.writeFileSync(
    scriptPath,
    [
      "#!/usr/bin/env node",
      ...
      "  process.exit(0);",
      "}",
      "console.error(`unexpected fake daisyclaw args: ${args.join(' ')}`);",
      "process.exit(2);",
      "",
    ].join("\n"),
    { mode: 0o755 },
...
function writeLeakingStartupDaisyClaw(root: string): string {
  const scriptPath = path.join(root, "fake-leaking-daisyclaw.mjs");
  fs.writeFileSync(
    scriptPath,
    [
      "#!/usr/bin/env node",
...
function writeSignaledStartupDaisyClaw(root: string): string {
  const scriptPath = path.join(root, "fake-signaled-daisyclaw.mjs");
  fs.writeFileSync(
    scriptPath,
    [
      "#!/usr/bin/env node",
...
function writeNoisySecretsConfigureDaisyClaw(root: string): string {
  const scriptPath = path.join(root, "fake-noisy-secrets-configure-daisyclaw.mjs");
  fs.writeFileSync(
    scriptPath,
    [
      "#!/usr/bin/env node",
...
function runProofHarness(
  ...
  return spawnSync(process.execPath, [harnessPath, proofScriptPath, root, mode], {
    ...
    env: {
      ...process.env,
      DAISYCLAW_ENTRY: fakeDaisyClaw,
      DAISYCLAW_SECRET_PROOF_READY_MS: "60",
      DAISYCLAW_SECRET_PROOF_RPC_MS: "1000",
      ...envOverrides,
    },
    timeout: 5_000,
  });
...
describe("secret provider integration proof harness", () => {
  it("runs pnpm-backed DaisyClaw commands through the repo pnpm runner", async () => {
    ...
    const command = await proof.resolveDaisyClawCommand(
      ["gateway", "status"],
      { ...process.env, DAISYCLAW_SECRET_PROOF_SENTINEL: "1" },
      {
        nodeExecPath: "/opt/node/bin/node",
        npmExecPath: fakePnpm,
        runner: { pnpm: true, baseArgs: ["daisyclaw"], label: "pnpm daisyclaw" },
      },
    );

    expect(command.command).toBe("/opt/node/bin/node");
    expect(command.args).toEqual([fakePnpm, "daisyclaw", "gateway", "status"]);
    expect(command.options.env.DAISYCLAW_SECRET_PROOF_SENTINEL).toBe("1");
    expect(command.options.shell).toBe(false);
  });

  it("keeps stalled startup health probes inside the ready deadline", async () => {
    const root = makeTempDir();
    const fakeDaisyClaw = writeStallingDaisyClaw(root);
    const result = runProofHarness(root, fakeDaisyClaw, "start");
  ...
  it("fails fast when startup exits by signal", () => {
    const root = makeTempDir();
    const fakeDaisyClaw = writeSignaledStartupDaisyClaw(root);
    const result = runProofHarness(root, fakeDaisyClaw, "start", {
      DAISYCLAW_SECRET_PROOF_READY_MS: "2000",
    });
  ...
  it("kills a stalled startup gateway before returning a readiness failure", async () => {
    ...
    const result = runProofHarness(root, fakeDaisyClaw, "start", {
      DAISYCLAW_SECRET_PROOF_TEARDOWN_GRACE_MS: "100",
    });
  ...
  it("bounds captured command output", async () => {
    const previousLimit = process.env.DAISYCLAW_SECRET_PROOF_OUTPUT_BYTES;
    process.env.DAISYCLAW_SECRET_PROOF_OUTPUT_BYTES = "1024";
    try {
      ...
      if (previousLimit === undefined) {
        delete process.env.DAISYCLAW_SECRET_PROOF_OUTPUT_BYTES;
      } else {
        process.env.DAISYCLAW_SECRET_PROOF_OUTPUT_BYTES = previousLimit;
      }
    }
  });
  ...
  it("blocks skipped secret proofs unless local rehearsals explicitly allow skips", async () => {
    const previousAllowSkips = process.env.DAISYCLAW_SECRET_PROOF_ALLOW_SKIPS;
    const proof = await import(
    ...
    try {
      delete process.env.DAISYCLAW_SECRET_PROOF_ALLOW_SKIPS;
      expect(proof.collectBlockingProofResults(entries)).toEqual(entries);

      process.env.DAISYCLAW_SECRET_PROOF_ALLOW_SKIPS = "1";
      expect(proof.collectBlockingProofResults(entries)).toEqual([]);
    } finally {
      if (previousAllowSkips === undefined) {
        delete process.env.DAISYCLAW_SECRET_PROOF_ALLOW_SKIPS;
      } else {
        process.env.DAISYCLAW_SECRET_PROOF_ALLOW_SKIPS = previousAllowSkips;
      }
    }
  });
  ...
  it.runIf(process.platform !== "win32")("bounds captured PTY configure output", async () => {
    const root = makeTempDir();
    const fakeDaisyClaw = writeNoisySecretsConfigureDaisyClaw(root);
    const previousLimit = process.env.DAISYCLAW_SECRET_PROOF_OUTPUT_BYTES;
    const previousEntry = process.env.DAISYCLAW_ENTRY;
    process.env.DAISYCLAW_SECRET_PROOF_OUTPUT_BYTES = "128";
    process.env.DAISYCLAW_ENTRY = fakeDaisyClaw;
    try {
      const proof = await import(
      ...
      if (previousEntry === undefined) {
        delete process.env.DAISYCLAW_ENTRY;
      } else {
        process.env.DAISYCLAW_ENTRY = previousEntry;
      }
    }
  });
  ...
  it.each([
    ["DAISYCLAW_SECRET_PROOF_COMMAND_MS", "150ms"],
    ["DAISYCLAW_SECRET_PROOF_READY_MS", "0"],
    ["DAISYCLAW_SECRET_PROOF_OUTPUT_BYTES", "4mb"],
    ["DAISYCLAW_SECRET_PROOF_RESOLVER_STDIN_BYTES", "4mb"],
  ...
  it("bounds generated resolver stdin before reading the secret store", async () => {
    ...
    const previousLimit = process.env.DAISYCLAW_SECRET_PROOF_RESOLVER_STDIN_BYTES;
    process.env.DAISYCLAW_SECRET_PROOF_RESOLVER_STDIN_BYTES = "64";
    ...
    try {
      ...
      if (previousLimit === undefined) {
        delete process.env.DAISYCLAW_SECRET_PROOF_RESOLVER_STDIN_BYTES;
      } else {
        process.env.DAISYCLAW_SECRET_PROOF_RESOLVER_STDIN_BYTES = previousLimit;
      }
    }
  });
  ...
  it("fails when proof temp cleanup cannot remove the root", async () => {
    ...
    try {
      await expect(
        proof.cleanupEnv("/tmp/daisyclaw-secret-provider-proof-stuck", {
          attempts: 3,
          retryDelayMs: 1,
        }),
  ...
  it("detects startup secret leaks after the retained output cap", () => {
    const root = makeTempDir();
    const fakeDaisyClaw = writeLeakingStartupDaisyClaw(root);
    const result = runProofHarness(root, fakeDaisyClaw, "startup-fails", {
      DAISYCLAW_SECRET_PROOF_OUTPUT_BYTES: "128",
    });
  ...
  it("keeps stalled managed status probes inside the ready deadline", async () => {
    const root = makeTempDir();
    const fakeDaisyClaw = writeStallingDaisyClaw(root);
    const result = runProofHarness(root, fakeDaisyClaw, "status");
