...
import {
  COMMAND_TIMEOUT_MS,
  createDaisyClawGatewaySpawnSpec,
  readLogTail,
...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-telegram-proof-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("telegram user Crabbox proof log polling", () => {
  it("starts the local gateway through the repo pnpm runner", () => {
    ...
    const spec = createDaisyClawGatewaySpawnSpec({
      env: { ...process.env, DAISYCLAW_TELEGRAM_PROOF_SENTINEL: "1" },
      gatewayPort: 19042,
    ...
    expect(spec.args).toEqual([fakePnpm, "daisyclaw", "gateway", "--port", "19042"]);
    expect(spec.options.cwd).toBe(root);
    expect(spec.options.env?.DAISYCLAW_TELEGRAM_PROOF_SENTINEL).toBe("1");
  ...
  it("rejects loose numeric log tail limits instead of parsing prefixes", () => {
    expect(() =>
      readTelegramUserProofLogTailBytes({
        DAISYCLAW_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: "1e3",
      }),
    ).toThrow("invalid DAISYCLAW_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: 1e3");
    expect(() =>
      readTelegramUserProofLogTailBytes({
        DAISYCLAW_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: "1000bytes",
      }),
    ).toThrow("invalid DAISYCLAW_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: 1000bytes");
    expect(
      readTelegramUserProofLogTailBytes({
        DAISYCLAW_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: "4096",
      }),
    ).toBe(4096);
  });
