...
function makeEnvMutationFixture(): string {
  const repoRoot = createTempDir("daisyclaw-test-env-mutations-");
  writeRepoFile(
    repoRoot,
    "src/runtime.ts",
    `
process.env.DAISYCLAW_STATE_DIR = "ignored because this is not a test file";
`,
  );
  writeRepoFile(
    repoRoot,
    "src/example.test.ts",
    `
process.env.DAISYCLAW_STATE_DIR = "state";
delete process.env["DAISYCLAW_CONFIG_PATH"];
vi.stubEnv("HOME", "home");
vi.stubEnv("DAISYCLAW_WORKSPACE_DIR", "workspace");
process.env = { ...process.env, DAISYCLAW_HOME: "home", OTHER_KEY: "ignored" };
const dynamicEnvKey = "DAISYCLAW_STATE_DIR";
process.env[dynamicEnvKey] = "dynamic";
delete process.env[dynamicEnvKey];
const fixture = 'process.env.DAISYCLAW_HOME = "not code"';
process.env.NOT_DAISYCLAW = "ignored";
`,
  );
  writeRepoFile(
    repoRoot,
    ...
    `
process.env.DAISYCLAW_CONFIG_PATH = "script-harness";
`,
  );
  writeRepoFile(
    repoRoot,
    ...
    `
process.env.DAISYCLAW_AGENT_DIR = "agent";
`,
  );
  writeRepoFile(
    repoRoot,
    "src/test-utils/daisyclaw-test-state.ts",
    `
...
describe("collectTestEnvMutationReport", () => {
  it("reports active DaisyClaw env mutations while ignoring strings and non-test files", () => {
    ...
    expect(
    ...
    ).toEqual([
      {
        ...
        key: "DAISYCLAW_CONFIG_PATH",
        operation: "assign",
      },
      {
        file: "src/agents/auth-profiles/oauth-test-utils.ts",
        key: "DAISYCLAW_AGENT_DIR",
        operation: "assign",
      },
      { file: "src/example.test.ts", key: "DAISYCLAW_STATE_DIR", operation: "assign" },
      { file: "src/example.test.ts", key: "DAISYCLAW_CONFIG_PATH", operation: "delete" },
      { file: "src/example.test.ts", key: "HOME", operation: "stubEnv" },
      { file: "src/example.test.ts", key: "DAISYCLAW_WORKSPACE_DIR", operation: "stubEnv" },
      { file: "src/example.test.ts", key: "DAISYCLAW_HOME", operation: "replace" },
  ...
  it("renders a non-blocking text report for active findings", () => {
    ...
    expect(rendered).toContain("DaisyClaw test env mutation report");
    ...
    expect(rendered).toContain("L2 DAISYCLAW_STATE_DIR assign process.env");
