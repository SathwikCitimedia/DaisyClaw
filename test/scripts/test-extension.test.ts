...
describe("scripts/test-extension.mjs", () => {
  ...
  it("can fail safe to all extensions when the base revision is unavailable", () => {
    const extensionIds = listChangedExtensionIds({
      base: "refs/heads/daisyclaw-test-missing-base",
  ...
  it("runs extension batch config groups concurrently when requested", async () => {
    ...
    const runPromise = runExtensionBatchPlan(
      ...
      {
        env: { DAISYCLAW_EXTENSION_BATCH_PARALLEL: "2" },
        runGroup,
    ...
    expect(firstRunGroupParams).toEqual({
      ...
      config: "heavy",
      env: {
        DAISYCLAW_EXTENSION_BATCH_PARALLEL: "2",
  ...
  it("keeps extension batch parallelism bounded by group count", () => {
    expect(resolveExtensionBatchParallelism(3, { DAISYCLAW_EXTENSION_BATCH_PARALLEL: "2" })).toBe(2);
    expect(resolveExtensionBatchParallelism(1, { DAISYCLAW_EXTENSION_BATCH_PARALLEL: "4" })).toBe(1);
  ...
  it("rejects malformed extension batch parallelism", () => {
    for (const value of ["nope", "2x", "0"]) {
      expect(() =>
        resolveExtensionBatchParallelism(3, { DAISYCLAW_EXTENSION_BATCH_PARALLEL: value }),
      ).toThrow("DAISYCLAW_EXTENSION_BATCH_PARALLEL must be a positive integer");
    }
  });
  ...
  posixIt("relativizes single-extension Vitest paths from extension cwd", () => {
    const root = mkdtempSync(path.join(tmpdir(), "daisyclaw-test-extension-args-"));
    ...
    try {
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            DAISYCLAW_FAKE_PNPM_ARGS_PATH: argsPath,
  ...
  posixIt(
    ...
    async () => {
      const root = mkdtempSync(path.join(tmpdir(), "daisyclaw-test-extension-signal-"));
      const fakePnpmPath = path.join(root, "pnpm");
      ...
      const runner = spawn(process.execPath, [scriptPath, "firecrawl"], {
        ...
        env: {
          ...process.env,
          DAISYCLAW_FAKE_PNPM_PID_PATH: childPidPath,
          DAISYCLAW_FAKE_PNPM_SIGNALED_PATH: signaledPath,
          npm_execpath: fakePnpmPath,
        },
        stdio: "ignore",
      });
...
function writeFakePnpm(filePath: string): void {
  writeFileSync(
    ...
    [
      ...
      "if (process.env.DAISYCLAW_FAKE_PNPM_ARGS_PATH) {",
      "  fs.writeFileSync(process.env.DAISYCLAW_FAKE_PNPM_ARGS_PATH, JSON.stringify(process.argv.slice(2)));",
      "  process.exit(0);",
      "}",
      "fs.writeFileSync(process.env.DAISYCLAW_FAKE_PNPM_PID_PATH, String(process.pid));",
      'process.on("SIGTERM", () => {',
      '  fs.writeFileSync(process.env.DAISYCLAW_FAKE_PNPM_SIGNALED_PATH, "SIGTERM");',
      "  process.exit(0);",
      "});",
      "setInterval(() => {}, 1000);",
      "",
