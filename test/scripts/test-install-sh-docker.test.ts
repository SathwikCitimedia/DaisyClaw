...
const PODMAN_RUN_PATH = "scripts/run-daisyclaw-podman.sh";
...
const RELEASE_CHECKS_WORKFLOW_PATH = ".github/workflows/daisyclaw-release-checks.yml";
const LIVE_E2E_WORKFLOW_PATH = ".github/workflows/daisyclaw-live-and-e2e-checks-reusable.yml";
...
describe("test-install-sh-docker", () => {
  it("defaults ARM hosts to native arm64 while keeping x64 CI on amd64", () => {
    ...
    expect(
      runDefaultSmokePlatform({ DAISYCLAW_INSTALL_SMOKE_PLATFORM: "linux/s390x" }, "x86_64"),
    ).toBe("linux/s390x");
  });
  ...
  it("supports npm update package specs without a separate expected-version env", () => {
    ...
    expect(script).toContain(
      'UPDATE_EXPECT_VERSION="${DAISYCLAW_INSTALL_SMOKE_UPDATE_EXPECT_VERSION:-}"',
    );
  ...
  it("uses npm latest as the update baseline and resolves it to the concrete packed version", () => {
    ...
    expect(script).toContain(
      'UPDATE_BASELINE_VERSION="${DAISYCLAW_INSTALL_SMOKE_UPDATE_BASELINE:-latest}"',
    );
    ...
    expect(runner).toContain(
      'UPDATE_BASELINE_VERSION="${DAISYCLAW_INSTALL_UPDATE_BASELINE:-latest}"',
    );
    ...
    expect(workflow).toContain(
      "DAISYCLAW_INSTALL_SMOKE_UPDATE_BASELINE: ${{ inputs.update_baseline_version || 'latest' }}",
    );
  });
  ...
  it("can reuse dist from the already-built root Docker smoke image", () => {
    ...
    expect(script).toContain('UPDATE_DIST_IMAGE="${DAISYCLAW_INSTALL_SMOKE_UPDATE_DIST_IMAGE:-}"');
    ...
    expect(script).toContain(
      'DOCKER_COMMAND_TIMEOUT="${DOCKER_COMMAND_TIMEOUT:-${DAISYCLAW_INSTALL_SMOKE_DOCKER_COMMAND_TIMEOUT:-600s}}"',
    );
  ...
  it("bounds installer smoke container runs", () => {
    ...
    expect(script).toContain(
      'INSTALL_SMOKE_DOCKER_RUN_TIMEOUT="${DAISYCLAW_INSTALL_SMOKE_DOCKER_RUN_TIMEOUT:-2700s}"',
    );
  ...
  it("passes the baked browser build arg through Docker setup", () => {
    ...
    expect(script).toContain('export DAISYCLAW_INSTALL_BROWSER="${DAISYCLAW_INSTALL_BROWSER:-}"');
    expect(script).toContain("DAISYCLAW_INSTALL_BROWSER \\");
    expect(script).toContain('--build-arg "DAISYCLAW_INSTALL_BROWSER=${DAISYCLAW_INSTALL_BROWSER}"');
  });

  it("bounds Docker setup image pulls", () => {
    const script = readFileSync(DOCKER_SETUP_PATH, "utf8");

    expect(script).toContain('DOCKER_PULL_TIMEOUT="${DAISYCLAW_DOCKER_SETUP_PULL_TIMEOUT:-600s}"');
  ...
  it("bounds Podman setup image pulls", () => {
    ...
    expect(script).toContain('PODMAN_PULL_TIMEOUT="${DAISYCLAW_PODMAN_SETUP_PULL_TIMEOUT:-600s}"');
    ...
    expect(script).toContain('run_podman_pull "$DAISYCLAW_IMAGE"');
    expect(script).not.toContain('podman pull "$DAISYCLAW_IMAGE"');
  });
  ...
  it("bounds Podman setup image builds", () => {
    ...
    expect(script).toContain('run_podman_build -t "$DAISYCLAW_IMAGE"');
    expect(script).not.toContain('podman build -t "$DAISYCLAW_IMAGE"');
  });

  it("bounds detached Podman launches without timing out onboarding", () => {
    const script = readFileSync(PODMAN_RUN_PATH, "utf8");

    expect(script).toContain('PODMAN_RUN_TIMEOUT="${DAISYCLAW_PODMAN_RUN_TIMEOUT:-600s}"');
    expect(script).toContain("DAISYCLAW_PODMAN_RUN_TIMEOUT|DAISYCLAW_PODMAN_GATEWAY_HOST_PORT");
  ...
  it("passes image-scoped pip packages through Docker and Podman setup", () => {
    ...
    expect(dockerfile).toContain("ARG DAISYCLAW_IMAGE_PIP_PACKAGES");
    expect(dockerfile).toContain(
    ...
    expect(dockerSetup).toContain("DAISYCLAW_IMAGE_PIP_PACKAGES \\");
    expect(dockerSetup).toContain(
      '--build-arg "DAISYCLAW_IMAGE_PIP_PACKAGES=${DAISYCLAW_IMAGE_PIP_PACKAGES}"',
    );
    expect(dockerSetup).not.toContain("DAISYCLAW_DOCKER_PIP_PACKAGES");
    expect(podmanSetup).toContain('DAISYCLAW_IMAGE_PIP_PACKAGES="${DAISYCLAW_IMAGE_PIP_PACKAGES:-}"');
    expect(podmanSetup).toContain(
      'BUILD_ARGS+=(--build-arg "DAISYCLAW_IMAGE_PIP_PACKAGES=${DAISYCLAW_IMAGE_PIP_PACKAGES}")',
    );
    expect(podmanSetup).not.toContain("DAISYCLAW_DOCKER_PIP_PACKAGES");
  });
  ...
  it("allows repository branch history and release tags for secret-backed Docker release checks", () => {
    ...
    expect(workflow).toContain("reachable from an DaisyClaw branch or release tag");
  });
  ...
  it("writes the package dist inventory before packing ignore-scripts tarballs", () => {
    ...
    expect(script).toContain("node scripts/check-daisyclaw-package-tarball.mjs");
  });
  ...
  it("runs candidate tarballs through the installer script instead of direct npm", () => {
    ...
    expect(wrapper).toContain('-v "$ROOT_DIR/scripts/install.sh:/tmp/daisyclaw-install.sh:ro"');
  ...
  it("uses public npm latest as the non-root installer expectation", () => {
    ...
    expect(wrapper).toContain('-e DAISYCLAW_INSTALL_EXPECT_VERSION="$LATEST_VERSION"');
  });
});
...
describe("install-sh smoke runner", () => {
  it("wraps long npm/update operations with heartbeat and install-size audits", () => {
    ...
    expect(script).toContain(
      'HEARTBEAT_INTERVAL="${DAISYCLAW_INSTALL_SMOKE_HEARTBEAT_INTERVAL:-60}"',
    );
    expect(script).toContain(
      'INSTALL_COMMAND_TIMEOUT="${DAISYCLAW_INSTALL_SMOKE_COMMAND_TIMEOUT:-900}"',
    );
    ...
    expect(script).toContain("daisyclaw update --tag");
  ...
  it("covers plain npm global installs and npm-driven updates", () => {
    ...
    expect(script).toContain('SKIP_NPM_GLOBAL="${DAISYCLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL:-0}"');
    expect(script).toContain('NPM_CACHE_DIR="${DAISYCLAW_INSTALL_SMOKE_NPM_CACHE_DIR:-}"');
    ...
    expect(script).toContain("DAISYCLAW_INSTALL_SMOKE_MODE=npm-global");
  ...
  it("forwards smoke-runner control knobs into Docker containers", () => {
    ...
    for (const envName of [
      "DAISYCLAW_INSTALL_ALLOW_LEGACY_UPDATE_WARNING",
      "DAISYCLAW_INSTALL_SELF_UPDATE_WARNING_FIXED_VERSION",
      "DAISYCLAW_INSTALL_SMOKE_COMMAND_TIMEOUT",
      "DAISYCLAW_INSTALL_SMOKE_HEARTBEAT_INTERVAL",
      "DAISYCLAW_INSTALL_SMOKE_PREVIOUS",
      "DAISYCLAW_INSTALL_SMOKE_SKIP_PREVIOUS",
    ]) {
...
describe("bun global install smoke", () => {
  it("packs the current tree and verifies image-provider discovery through Bun", () => {
    ...
    expect(script).toContain("DAISYCLAW_BUN_GLOBAL_SMOKE_DIST_IMAGE");
    expect(script).toContain('source "$ROOT_DIR/scripts/lib/docker-e2e-container.sh"');
    expect(script).toContain(
      'DOCKER_COMMAND_TIMEOUT="${DOCKER_COMMAND_TIMEOUT:-${DAISYCLAW_BUN_GLOBAL_SMOKE_DOCKER_COMMAND_TIMEOUT:-600s}}"',
    );
    expect(script).toContain('container_id="$(docker_e2e_docker_cmd create "$image")"');
    expect(script).toContain(
  ...
  it("gates workflow Bun install smoke to scheduled and release-check runs", () => {
    ...
    expect(workflow).toContain(
      "DAISYCLAW_BUN_GLOBAL_SMOKE_DIST_IMAGE: ${{ needs.root_dockerfile_image.outputs.image_ref }}",
    );
    expect(workflow).toContain(
    ...
    expect(workflow).toContain("DAISYCLAW_CI_WORKFLOW_BUN_GLOBAL_INSTALL_SMOKE");
    ...
    expect(workflow).toContain("DAISYCLAW_INSTALL_URL: file:///tmp/daisyclaw-install.sh");
    expect(workflow).toContain("DAISYCLAW_INSTALL_CLI_URL: file:///tmp/daisyclaw-install-cli.sh");
    expect(workflow).toContain('DAISYCLAW_INSTALL_SMOKE_SKIP_CLI: "0"');
    ...
    expect(workflow).toContain("bash /tmp/install-cli.sh --prefix /tmp/daisyclaw-cli");
    ...
    expect(workflow).toContain('DAISYCLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL: "1"');
  ...
  it("kills Bun global install smoke commands that ignore TERM after timeout", () => {
    const result = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          DAISYCLAW_BUN_GLOBAL_SMOKE_TIMEOUT_KILL_GRACE_MS: "50",
        },
        timeout: 5000,
      },
    );
