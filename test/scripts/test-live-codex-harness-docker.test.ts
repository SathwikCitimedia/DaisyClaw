...
describe("scripts/test-live-codex-harness-docker.sh", () => {
  it("mounts cache and npm tool dirs outside the bind-mounted Docker home", () => {
    ...
    expect(script).toContain('DOCKER_CACHE_CONTAINER_DIR="/tmp/daisyclaw-cache"');
    expect(script).toContain('DOCKER_CLI_TOOLS_CONTAINER_DIR="/tmp/daisyclaw-npm-global"');
    expect(script).toContain("daisyclaw_live_codex_harness_is_ci()");
    expect(script).toContain("daisyclaw_live_is_ci");
    ...
    expect(script).toContain('daisyclaw_live_prepare_bind_dir_for_container_user "$CLI_TOOLS_DIR"');
    expect(script).toContain('daisyclaw_live_prepare_bind_dir_for_container_user "$CACHE_HOME_DIR"');
  ...
  it("fails before Docker build when codex-auth has no host auth file", () => {
    ...
    expect(script).toContain(
      "DAISYCLAW_LIVE_CODEX_HARNESS_AUTH=codex-auth requires ~/.codex/auth.json before building the live Docker image",
    );
    expect(script).toContain(
      "If this is a Testbox/API-key run, set DAISYCLAW_LIVE_CODEX_HARNESS_AUTH=api-key and run through daisyclaw-testbox-env.",
    );
    expect(script.indexOf("requires ~/.codex/auth.json before building")).toBeLessThan(
      script.indexOf('DAISYCLAW_LIVE_DOCKER_REPO_ROOT="$ROOT_DIR"'),
    );
  });
  ...
  it("keeps API-key runs on the ephemeral Docker home", () => {
    ...
    expect(script).toContain("if daisyclaw_live_uses_managed_bind_dirs; then");
    ...
    expect(script.indexOf('PROFILE_STATUS="api-key-env"')).toBeLessThan(
      script.indexOf("daisyclaw_live_append_array DOCKER_RUN_ARGS PROFILE_MOUNT"),
    );
  ...
  it("installs the plugin-pinned Codex CLI package for app-server proof", () => {
    ...
    expect(script).toContain('-e DAISYCLAW_LIVE_CODEX_CLI_PACKAGE_SPEC="$CODEX_CLI_PACKAGE_SPEC"');
    expect(script).toContain(
  ...
  it("fails instead of skipping when Codex auth cannot identify an account", () => {
    ...
    expect(script).toContain(
      "ERROR: Codex auth cannot extract accountId from the available token; refresh DAISYCLAW_CODEX_AUTH_JSON or use DAISYCLAW_LIVE_CODEX_HARNESS_AUTH=api-key.",
    );
