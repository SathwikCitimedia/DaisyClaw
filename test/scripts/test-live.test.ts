...
describe("scripts/test-live", () => {
  ...
  posixIt("signals the live pnpm child when the wrapper is terminated", async () => {
    const root = mkdtempSync(join(tmpdir(), "daisyclaw-test-live-signal-"));
  ...
  it("rejects loose heartbeat intervals instead of parsing prefixes", () => {
    ...
    expect(resolveTestLiveHeartbeatMs({ DAISYCLAW_LIVE_WRAPPER_HEARTBEAT_MS: "2500" })).toBe(2500);
    expect(() => resolveTestLiveHeartbeatMs({ DAISYCLAW_LIVE_WRAPPER_HEARTBEAT_MS: "1e3" })).toThrow(
      "invalid DAISYCLAW_LIVE_WRAPPER_HEARTBEAT_MS: 1e3",
    );
    expect(() =>
      resolveTestLiveHeartbeatMs({ DAISYCLAW_LIVE_WRAPPER_HEARTBEAT_MS: "1000ms" }),
    ).toThrow("invalid DAISYCLAW_LIVE_WRAPPER_HEARTBEAT_MS: 1000ms");
    expect(() => resolveTestLiveHeartbeatMs({ DAISYCLAW_LIVE_WRAPPER_HEARTBEAT_MS: "0" })).toThrow(
      "invalid DAISYCLAW_LIVE_WRAPPER_HEARTBEAT_MS: 0",
    );
  });
