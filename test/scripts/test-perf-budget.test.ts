...
function withReport(payload: unknown, run: (reportPath: string) => void) {
  const reportPath = path.join(os.tmpdir(), `daisyclaw-test-perf-budget-${Date.now()}.json`);
  fs.writeFileSync(
    reportPath,
...
describe("test perf budget script", () => {
  it("requires a budget source unless report-only mode is explicit", () => {
    ...
    expect(
      testing.parseArgs([], {
        DAISYCLAW_TEST_PERF_REPORT_ONLY: "yes",
      }),
    ).toMatchObject({
      baselineWallMs: null,
      maxWallMs: null,
  ...
  it("parses numeric budget env vars strictly before running Vitest", () => {
    expect(
      testing.parseArgs([], {
        DAISYCLAW_TEST_PERF_BASELINE_WALL_MS: "1000",
        DAISYCLAW_TEST_PERF_MAX_REGRESSION_PCT: "12.5",
        DAISYCLAW_TEST_PERF_MAX_WALL_MS: "1500",
      }),
    ).toMatchObject({
      baselineWallMs: 1000,
    ...
    expect(() =>
      testing.parseArgs([], {
        DAISYCLAW_TEST_PERF_MAX_WALL_MS: "1000ms",
      }),
    ).toThrow("DAISYCLAW_TEST_PERF_MAX_WALL_MS must be a non-negative number");
  });
