...
async function loadRawVitestConfig(configPath: string): Promise<VitestConfig> {
  ...
  const previousIncludeFile = process.env.DAISYCLAW_VITEST_INCLUDE_FILE;
  process.argv = [previousArgv[0] ?? "node", previousArgv[1] ?? "vitest"];
  delete process.env.DAISYCLAW_VITEST_INCLUDE_FILE;
  try {
    ...
    if (previousIncludeFile === undefined) {
      ...
      process.env.DAISYCLAW_VITEST_INCLUDE_FILE = previousIncludeFile;
    }
  }
}
...
function withTinyGitRepo(files: Record<string, string>, test: (cwd: string) => void): void {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-test-projects-"));
  try {
...
function commitTinyGitRepo(cwd: string): void {
  const commit = spawnSync("git", ["commit", "-m", "initial"], {
    ...
    env: {
      ...
      GIT_AUTHOR_NAME: "DaisyClaw Test",
      GIT_COMMITTER_EMAIL: "test@example.com",
      GIT_COMMITTER_NAME: "DaisyClaw Test",
    },
    stdio: "ignore",
  });
...
describe("scripts/test-projects changed-target routing", () => {
  ...
  it("keeps the broad changed run available for Vitest wiring edits", () => {
    expect(
      resolveChangedTargetArgs(
        ...
        { env: { DAISYCLAW_TEST_CHANGED_BROAD: "1" } },
      ),
    ).toBeNull();
  });
  ...
  it("keeps release-check workflow edits on release workflow regression tests", () => {
    expect(resolveChangedTestTargetPlan([".github/workflows/daisyclaw-release-checks.yml"])).toEqual(
      {
        mode: "targets",
        targets: ["test/scripts/package-acceptance-workflow.test.ts"],
      },
  ...
  it("keeps package, release, and install tooling edits on owner tests", () => {
    const expectedTargets = new Map([
      ...
      [
        "scripts/package-daisyclaw-for-docker.mjs",
        ["test/scripts/package-daisyclaw-for-docker.test.ts"],
      ],
      ...
      ["scripts/daisyclaw-prepack.ts", ["test/daisyclaw-prepack.test.ts"]],
      ["scripts/daisyclaw-npm-release-check.ts", ["test/daisyclaw-npm-release-check.test.ts"]],
      [
        "scripts/daisyclaw-npm-postpublish-verify.ts",
        ["test/daisyclaw-npm-postpublish-verify.test.ts"],
      ],
      [
  ...
  it("routes the shell helper test to the isolated tooling shard", () => {
    expect(
      buildVitestRunPlans(["--changed", "origin/main"], process.cwd(), () => [
        "test/scripts/daisyclaw-e2e-instance.test.ts",
      ]),
    ).toEqual([
      {
        config: "test/vitest/vitest.tooling-isolated.config.ts",
        forwardedArgs: [],
        includePatterns: ["test/scripts/daisyclaw-e2e-instance.test.ts"],
        watchMode: false,
      },
    ]);
  });
    ...
    expect(buildVitestRunPlans(targets, process.cwd())).toEqual([
      ...
      },
      {
        ...
        includePatterns: [
          ...
          "test/scripts/daisyclaw-test-state.test.ts",
  ...
  it("rejects explicit test-support helper files with no importing tests", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-test-targets-"));
    try {
  ...
  it("explains changed paths that need explicit broad fallback before skipping", () => {
    withTinyGitRepo({ "package.json": '{"scripts":{}}\n' }, (cwd) => {
      ...
      expect(result.stderr).toContain(
        "[test] run `DAISYCLAW_TEST_CHANGED_BROAD=1 pnpm test:changed` for broad coverage.",
      );
    });
  });
  ...
  it("skips app-only changes because app tests are separate from Vitest lanes", () => {
    expect(
      buildVitestRunPlans(["--changed", "origin/main"], process.cwd(), () => [
        "apps/macos/DaisyClaw/AppDelegate.swift",
      ]),
    ).toStrictEqual([]);
  });
...
describe("scripts/test-projects local heavy-check lock", () => {
  const localCheckEnv = () => ({
    ...process.env,
    DAISYCLAW_TEST_HEAVY_CHECK_LOCK_HELD: undefined,
    DAISYCLAW_TEST_PROJECTS_FORCE_LOCK: undefined,
  });
  ...
  it("skips the lock when a parent changed gate already holds it", () => {
    expect(
      shouldAcquireLocalHeavyCheckLock(
        ...
        {
          ...localCheckEnv(),
          DAISYCLAW_TEST_HEAVY_CHECK_LOCK_HELD: "1",
        },
      ),
    ).toBe(false);
  });
  ...
  it("allows forcing the lock back on", () => {
    expect(
      shouldAcquireLocalHeavyCheckLock(
        ...
        {
          ...localCheckEnv(),
          DAISYCLAW_TEST_PROJECTS_FORCE_LOCK: "1",
        },
      ),
    ).toBe(true);
  });
...
describe("scripts/test-projects full-suite sharding", () => {
  ...
  beforeAll(async () => {
    ...
    const previous = process.env.DAISYCLAW_TEST_PROJECTS_LEAF_SHARDS;
    const gatewayServerConfig = "test/vitest/vitest.gateway-server.config.ts";
    process.env.DAISYCLAW_TEST_PROJECTS_LEAF_SHARDS = "1";
    try {
      ...
      if (previous === undefined) {
        delete process.env.DAISYCLAW_TEST_PROJECTS_LEAF_SHARDS;
      } else {
        process.env.DAISYCLAW_TEST_PROJECTS_LEAF_SHARDS = previous;
      }
    }
  });
  ...
  it("keeps explicit parallel overrides ahead of the host-aware profile", () => {
    expect(
      resolveParallelFullSuiteConcurrency(
        61,
        {
          DAISYCLAW_TEST_PROJECTS_PARALLEL: "3",
        },
        {
          cpuCount: 14,
          loadAverage1m: 0,
  ...
  it("rejects malformed parallel full-suite overrides", () => {
    expect(() =>
      resolveParallelFullSuiteConcurrency(
        61,
        {
          DAISYCLAW_TEST_PROJECTS_PARALLEL: "3x",
        },
        {
          cpuCount: 14,
          loadAverage1m: 0,
    ...
    ).toThrow("DAISYCLAW_TEST_PROJECTS_PARALLEL must be a positive integer; got: 3x");

    expect(() =>
      resolveParallelFullSuiteConcurrency(
        61,
        {
          DAISYCLAW_TEST_PROJECTS_PARALLEL: "0",
        },
        {
          cpuCount: 14,
          loadAverage1m: 0,
    ...
    ).toThrow("DAISYCLAW_TEST_PROJECTS_PARALLEL must be a positive integer; got: 0");
  });
  ...
  it("rejects malformed conservative worker budget values", () => {
    expect(() =>
      resolveParallelFullSuiteConcurrency(
        61,
        {
          DAISYCLAW_VITEST_MAX_WORKERS: "1e0",
        },
        {
          cpuCount: 14,
          loadAverage1m: 0,
    ...
    ).toThrow("DAISYCLAW_VITEST_MAX_WORKERS must be a positive integer; got: 1e0");

    expect(() =>
      resolveParallelFullSuiteConcurrency(
        61,
        {
          DAISYCLAW_TEST_WORKERS: "1 worker",
        },
        {
          cpuCount: 14,
          loadAverage1m: 0,
    ...
    ).toThrow("DAISYCLAW_TEST_WORKERS must be a positive integer; got: 1 worker");
  });

  it("keeps serial untargeted runs on aggregate shards", () => {
    const previousParallel = process.env.DAISYCLAW_TEST_PROJECTS_PARALLEL;
    const previousSerial = process.env.DAISYCLAW_TEST_PROJECTS_SERIAL;
    delete process.env.DAISYCLAW_TEST_PROJECTS_LEAF_SHARDS;
    delete process.env.DAISYCLAW_TEST_SKIP_FULL_EXTENSIONS_SHARD;
    delete process.env.DAISYCLAW_TEST_PROJECTS_PARALLEL;
    process.env.DAISYCLAW_TEST_PROJECTS_SERIAL = "1";
    try {
      ...
      if (previousParallel === undefined) {
        ...
        process.env.DAISYCLAW_TEST_PROJECTS_PARALLEL = previousParallel;
      }
      if (previousSerial === undefined) {
        delete process.env.DAISYCLAW_TEST_PROJECTS_SERIAL;
      } else {
        process.env.DAISYCLAW_TEST_PROJECTS_SERIAL = previousSerial;
      }
    }
  });

  it("expands untargeted local runs to leaf project configs by default", () => {
    const previousLeafShards = process.env.DAISYCLAW_TEST_PROJECTS_LEAF_SHARDS;
    const previousParallel = process.env.DAISYCLAW_TEST_PROJECTS_PARALLEL;
    const previousSerial = process.env.DAISYCLAW_TEST_PROJECTS_SERIAL;
    const previousCi = process.env.CI;
    const previousActions = process.env.GITHUB_ACTIONS;
    const previousVitestMaxWorkers = process.env.DAISYCLAW_VITEST_MAX_WORKERS;
    const previousTestWorkers = process.env.DAISYCLAW_TEST_WORKERS;
    delete process.env.DAISYCLAW_TEST_PROJECTS_LEAF_SHARDS;
    delete process.env.DAISYCLAW_TEST_PROJECTS_PARALLEL;
    delete process.env.DAISYCLAW_TEST_PROJECTS_SERIAL;
    delete process.env.CI;
    delete process.env.GITHUB_ACTIONS;
    delete process.env.DAISYCLAW_VITEST_MAX_WORKERS;
    delete process.env.DAISYCLAW_TEST_WORKERS;
    try {
      ...
      if (previousLeafShards === undefined) {
        ...
        process.env.DAISYCLAW_TEST_PROJECTS_LEAF_SHARDS = previousLeafShards;
      }
      if (previousParallel === undefined) {
        delete process.env.DAISYCLAW_TEST_PROJECTS_PARALLEL;
      } else {
      ...
      if (previousVitestMaxWorkers === undefined) {
        ...
        process.env.DAISYCLAW_VITEST_MAX_WORKERS = previousVitestMaxWorkers;
      }
      if (previousTestWorkers === undefined) {
        delete process.env.DAISYCLAW_TEST_WORKERS;
      } else {
        process.env.DAISYCLAW_TEST_WORKERS = previousTestWorkers;
      }
    }
  });

  it("can skip the aggregate extension shard when CI runs dedicated extension shards", () => {
    const previous = process.env.DAISYCLAW_TEST_SKIP_FULL_EXTENSIONS_SHARD;
    const previousParallel = process.env.DAISYCLAW_TEST_PROJECTS_PARALLEL;
    const previousSerial = process.env.DAISYCLAW_TEST_PROJECTS_SERIAL;
    delete process.env.DAISYCLAW_TEST_PROJECTS_PARALLEL;
    process.env.DAISYCLAW_TEST_PROJECTS_SERIAL = "1";
    process.env.DAISYCLAW_TEST_SKIP_FULL_EXTENSIONS_SHARD = "1";
    try {
      const configs = buildFullSuiteVitestRunPlans([], process.cwd()).map((plan) => plan.config);
      ...
      if (previous === undefined) {
        ...
        process.env.DAISYCLAW_TEST_SKIP_FULL_EXTENSIONS_SHARD = previous;
      }
      if (previousParallel === undefined) {
        delete process.env.DAISYCLAW_TEST_PROJECTS_PARALLEL;
      } else {
  ...
  it("skips extension project configs when leaf sharding and the aggregate extension shard is disabled", () => {
    const previousLeafShards = process.env.DAISYCLAW_TEST_PROJECTS_LEAF_SHARDS;
    const previousSkipExtensions = process.env.DAISYCLAW_TEST_SKIP_FULL_EXTENSIONS_SHARD;
    process.env.DAISYCLAW_TEST_PROJECTS_LEAF_SHARDS = "1";
    process.env.DAISYCLAW_TEST_SKIP_FULL_EXTENSIONS_SHARD = "1";
    try {
      const configs = buildFullSuiteVitestRunPlans([], process.cwd()).map((plan) => plan.config);
      ...
      if (previousSkipExtensions === undefined) {
        ...
        process.env.DAISYCLAW_TEST_SKIP_FULL_EXTENSIONS_SHARD = previousSkipExtensions;
      }
    }
  });
  ...
  it("expands full-suite shards before running them in parallel", () => {
    ...
    const previousParallel = process.env.DAISYCLAW_TEST_PROJECTS_PARALLEL;
    delete process.env.DAISYCLAW_TEST_PROJECTS_LEAF_SHARDS;
    process.env.DAISYCLAW_TEST_PROJECTS_PARALLEL = "6";
    try {
      const configs = buildFullSuiteVitestRunPlans([], process.cwd()).map((plan) => plan.config);
  ...
  it("rejects malformed full-suite expansion parallel overrides", () => {
    ...
    const previousParallel = process.env.DAISYCLAW_TEST_PROJECTS_PARALLEL;
    delete process.env.DAISYCLAW_TEST_PROJECTS_LEAF_SHARDS;
    process.env.DAISYCLAW_TEST_PROJECTS_PARALLEL = "6x";
    try {
      expect(() => buildFullSuiteVitestRunPlans([], process.cwd())).toThrow(
        "DAISYCLAW_TEST_PROJECTS_PARALLEL must be a positive integer; got: 6x",
      );
    } finally {
      if (previousLeafShards === undefined) {
        delete process.env.DAISYCLAW_TEST_PROJECTS_LEAF_SHARDS;
...
describe("scripts/test-projects parallel cache paths", () => {
  it("assigns isolated Vitest fs-module cache paths per parallel shard", () => {
    ...
    expect(specs.map((spec) => spec.env)).toEqual([
      {
        DAISYCLAW_VITEST_FS_MODULE_CACHE_PATH: path.join(
          "/repo",
          "node_modules",
  ...
  it("keeps an explicit global cache path", () => {
    const [spec] = applyParallelVitestCachePaths(
      [{ config: "test/vitest/vitest.gateway.config.ts", env: {}, pnpmArgs: [] }],
      { cwd: "/repo", env: { DAISYCLAW_VITEST_FS_MODULE_CACHE_PATH: "/tmp/cache" } },
    );

    expect(spec?.env.DAISYCLAW_VITEST_FS_MODULE_CACHE_PATH).toBeUndefined();
  });
});
...
describe("scripts/test-projects Vitest stall watchdog", () => {
  it("adds default no-output watchdog settings to non-watch specs", () => {
    ...
    expect(spec?.env.DAISYCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe(
      DEFAULT_TEST_PROJECTS_VITEST_NO_OUTPUT_TIMEOUT_MS,
    );
    expect(spec?.env.DAISYCLAW_VITEST_NO_OUTPUT_HEARTBEAT_MS).toBe(
      DEFAULT_TEST_PROJECTS_VITEST_NO_OUTPUT_HEARTBEAT_MS,
    );
  });
  ...
  it("keeps explicit watchdog settings and watch mode untouched", () => {
    const specs = applyDefaultVitestNoOutputTimeout(
      [
        ...
        },
        {
          ...
          env: {
            DAISYCLAW_VITEST_NO_OUTPUT_HEARTBEAT_MS: "25000",
            DAISYCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "0",
            PATH: "/usr/bin",
          },
          includeFilePath: null,
          includePatterns: null,
    ...
    expect(specs[0]?.env.DAISYCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS).toBeUndefined();
    expect(specs[0]?.env.DAISYCLAW_VITEST_NO_OUTPUT_HEARTBEAT_MS).toBeUndefined();
    expect(specs[1]?.env.DAISYCLAW_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe("0");
    expect(specs[1]?.env.DAISYCLAW_VITEST_NO_OUTPUT_HEARTBEAT_MS).toBe("25000");
  });
  ...
  it("allows changed checks to disable automatic silent-run retries", () => {
    ...
    expect(shouldRetryVitestNoOutputTimeout({ DAISYCLAW_VITEST_NO_OUTPUT_RETRY: "1" })).toBe(true);
    expect(shouldRetryVitestNoOutputTimeout({ DAISYCLAW_VITEST_NO_OUTPUT_RETRY: "0" })).toBe(false);
    expect(shouldRetryVitestNoOutputTimeout({ DAISYCLAW_VITEST_NO_OUTPUT_RETRY: "false" })).toBe(
      false,
    );
  });
});
...
describe("scripts/test-projects Vitest cache isolation", () => {
  it("assigns isolated fs-module caches to multi-spec non-watch runs", () => {
    ...
    expect(specs.map((spec) => spec.env.DAISYCLAW_VITEST_FS_MODULE_CACHE_PATH)).toEqual([
      path.join(
        "/repo",
        "node_modules",
        ".experimental-vitest-cache",
