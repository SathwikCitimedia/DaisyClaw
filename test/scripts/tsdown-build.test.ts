...
const NO_MEMORY_LIMIT = {
  ...
  procMeminfoPath: "/daisyclaw-test-missing-proc-meminfo",
};
...
describe("resolveTsdownBuildInvocation", () => {
  ...
  it("routes Windows tsdown builds through the pnpm runner instead of shell=true", () => {
    const rootDir = createTempDir("daisyclaw-pnpm-runner-");
  ...
  it("can run tsdown without invoking pnpm", () => {
    const result = resolveTsdownBuildInvocation({
      platform: "linux",
      nodeExecPath: "/usr/bin/node",
      env: { DAISYCLAW_BUILD_ALL_NO_PNPM: "1" },
      ...NO_MEMORY_LIMIT,
    });
  ...
  it("prunes stale hashed root chunk files but keeps stable aliases and nested assets", async () => {
    const rootDir = createTempDir("daisyclaw-tsdown-build-");
  ...
  it("cleans tsdown output roots before using tsdown --no-clean", async () => {
    const rootDir = createTempDir("daisyclaw-tsdown-clean-");
  ...
  it("removes CLI startup metadata during default tsdown clean", async () => {
    const rootDir = createTempDir("daisyclaw-tsdown-clean-metadata-default-");
  ...
  it("preserves CLI startup metadata across opted-in build-all tsdown clean", async () => {
    const rootDir = createTempDir("daisyclaw-tsdown-clean-metadata-");
    const metadataFile = path.join(rootDir, "dist", "cli-startup-metadata.json");
    ...
    cleanTsdownOutputRoots({
      cwd: rootDir,
      env: { DAISYCLAW_PRESERVE_CLI_STARTUP_METADATA: "1" },
    });
  ...
  it("preserves existing package declarations when tsdown DTS output is skipped", async () => {
    const rootDir = createTempDir("daisyclaw-tsdown-clean-skip-dts-");
    ...
    cleanTsdownOutputRoots({
      cwd: rootDir,
      env: { DAISYCLAW_RUN_NODE_SKIP_DTS_BUILD: "1" },
    });
  ...
  it("prunes untracked generated declaration files that shadow source entries", async () => {
    const rootDir = createTempDir("daisyclaw-tsdown-source-dts-");
...
describe("createTsdownOutputScanner", () => {
  ...
  it("ignores unresolved imports from bundled plugin and dependency paths", () => {
    ...
    scanner.append(
      "[UNRESOLVED_IMPORT] ../../../../tmp/daisyclaw-pnpm-node-modules/baileys/lib/Utils/messages-media.js\n",
    );
...
describe("runTsdownBuildInvocation", () => {
  ...
  it("streams child output while preserving diagnostics for post-run checks", async () => {
    ...
    const result = await runTsdownBuildInvocation(
      ...
      },
      {
        stdout: output.sink,
        stderr: output.sink,
        env: { ...process.env, DAISYCLAW_TSDOWN_HEARTBEAT_MS: "0" },
      },
    );
  ...
  it("rejects malformed DAISYCLAW_TSDOWN_TIMEOUT_MS values", async () => {
    const invocation = {
      command: process.execPath,
    ...
    for (const value of ["1.5", "1e3", "10ms", "0"]) {
      await expect(
        runTsdownBuildInvocation(invocation, {
          env: {
            ...process.env,
            DAISYCLAW_TSDOWN_TIMEOUT_MS: value,
          },
        }),
      ).rejects.toThrow("DAISYCLAW_TSDOWN_TIMEOUT_MS must be");
    }
  });

  it("rejects malformed DAISYCLAW_TSDOWN_HEARTBEAT_MS values", async () => {
    const invocation = {
      command: process.execPath,
      args: ["-e", "process.exit(0)"],
      options: {
    ...
    for (const value of ["1.5", "1e3", "10ms", "-1"]) {
      await expect(
        runTsdownBuildInvocation(invocation, {
          env: {
            ...process.env,
            DAISYCLAW_TSDOWN_HEARTBEAT_MS: value,
          },
        }),
      ).rejects.toThrow("DAISYCLAW_TSDOWN_HEARTBEAT_MS must be");
    }
  });

  it("terminates the child when DAISYCLAW_TSDOWN_TIMEOUT_MS elapses", async () => {
    const output = createWriteSink();
    const result = await runTsdownBuildInvocation(
      {
        command: process.execPath,
      ...
      },
      {
        ...
        env: {
          ...process.env,
          DAISYCLAW_TSDOWN_HEARTBEAT_MS: "0",
          DAISYCLAW_TSDOWN_TIMEOUT_MS: "50",
        },
      },
    );
