...
describe("scripts/ui windows spawn behavior", () => {
  ...
  it("detects direct execution through a junctioned script path", () => {
    const realScriptPath = path.resolve("repo/daisyclaw/scripts/ui.js");
    const junctionScriptPath = path.resolve("linked/daisyclaw/scripts/ui.js");
  ...
  it.runIf(process.platform !== "win32").each(["SIGTERM", "SIGHUP"] as const)(
    ...
    async (signal) => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-ui-wrapper-signals-"));
