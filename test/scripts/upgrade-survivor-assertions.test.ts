...
function writeMigratedSessionState(stateDir: string): void {
  ...
  const db = new DatabaseSync(join(agentDbDir, "daisyclaw-agent.sqlite"));
  try {
    db.exec(`
...
function assertConfiguredPluginState(params: { installPath?: string } = {}): void {
  const root = mkdtempSync(join(tmpdir(), "daisyclaw-upgrade-survivor-"));
  try {
    ...
    writeJson(join(stateDir, "plugins", "installs.json"), {
      installRecords: {
        matrix: {
          source: "clawhub",
          spec: "clawhub:@daisyclaw/matrix",
          installPath: matrixInstallDir,
          clawhubPackage: "@daisyclaw/matrix",
    ...
    execFileSync(process.execPath, [ASSERTIONS_PATH, "assert-state"], {
      env: {
        ...
        DAISYCLAW_TEST_WORKSPACE_DIR: workspace,
        DAISYCLAW_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON: coveragePath,
        DAISYCLAW_UPGRADE_SURVIVOR_SCENARIO: "configured-plugin-installs",
      },
      stdio: "pipe",
    });
  } finally {
...
describe("upgrade survivor assertions", () => {
  ...
  it("rejects ClawHub npm-pack installs outside the managed extensions root", () => {
    const root = mkdtempSync(join(tmpdir(), "daisyclaw-upgrade-survivor-outside-"));
    try {
      expect(() =>
