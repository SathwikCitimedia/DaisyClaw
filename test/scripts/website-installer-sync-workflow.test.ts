...
describe("website installer sync workflow", () => {
  ...
  it("treats all website installer scripts as DaisyClaw-owned inputs", () => {
  ...
  it("verifies installers on Linux Docker plus native macOS and Windows runners", () => {
    ...
    expect(workflow).toContain("bash /tmp/install.sh --version latest && daisyclaw --version");
    expect(workflow).not.toContain("bash /tmp/install.sh --no-prompt --no-onboard");
    expect(workflow).toContain("bash /tmp/install-cli.sh --prefix /tmp/daisyclaw");
    expect(workflow).toContain("macos-installer:");
    expect(workflow).toContain("runs-on: macos-15");
    expect(workflow).toContain("node-version: 24");
    expect(workflow).toContain('DAISYCLAW_NO_ONBOARD: "1"');
    expect(workflow).toContain('DAISYCLAW_NO_PROMPT: "1"');
    expect(workflow).toContain("bash scripts/install.sh --no-onboard --no-prompt --version latest");
    expect(workflow).toContain("daisyclaw --version");
  ...
  it("syncs verified scripts to daisyclaw.ai only after all installer checks pass", () => {
    expect(workflow).toContain("needs: [static, linux-docker, macos-installer, windows-installer]");
    expect(workflow).toContain("repository: daisyclaw/daisyclaw.ai");
    expect(workflow).toContain("DAISYCLAW_GH_TOKEN: ${{ secrets.DAISYCLAW_GH_TOKEN }}");
    expect(workflow).toContain("DAISYCLAW_GH_TOKEN is not configured");
    expect(workflow).toContain("token: ${{ env.DAISYCLAW_GH_TOKEN }}");
    expect(workflow).toContain("cp daisyclaw/scripts/install.sh daisyclaw.ai/public/install.sh");
    expect(workflow).toContain(
      "cp daisyclaw/scripts/install-cli.sh daisyclaw.ai/public/install-cli.sh",
    );
    expect(workflow).toContain("cp daisyclaw/scripts/install.ps1 daisyclaw.ai/public/install.ps1");
    expect(workflow).toContain("rm -f daisyclaw.ai/public/install.cmd");
