...
function writeStartupMetadataSourceSignatureFixture(rootDir: string): void {
  const fixtures = new Map<string, string>([
    ...
    ["src/cli/banner.ts", "export const banner = 'daisyclaw';\n"],
    [
...
describe("write-cli-startup-metadata", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempRoot = createTempDir("daisyclaw-startup-metadata-timeout-");
  ...
  it("writes startup metadata with populated root help text when dist falls back to source rendering", async () => {
    const tempRoot = createTempDir("daisyclaw-startup-metadata-");
    ...
    await writeCliStartupMetadata({
      ...
      renderSourceRootHelpText: () => "Usage: daisyclaw\n",
      renderSourceBrowserHelpText: () => "Usage: daisyclaw browser\n",
      renderSourceSecretsHelpText: () => "Usage: daisyclaw secrets\n",
      renderSourceNodesHelpText: () => "Usage: daisyclaw nodes\n",
      renderSourceSubcommandHelpTextRecord: () => ({
        doctor: "Usage: daisyclaw doctor\n",
        gateway: "Usage: daisyclaw gateway\n",
        models: "Usage: daisyclaw models\n",
        plugins: "Usage: daisyclaw plugins\n",
      }),
    });
    ...
    expect(written.browserHelpText).toContain("daisyclaw browser");
    expect(written.secretsHelpText).toContain("Usage:");
    expect(written.secretsHelpText).toContain("daisyclaw secrets");
    expect(written.nodesHelpText).toContain("Usage:");
    expect(written.nodesHelpText).toContain("daisyclaw nodes");
    expect(written.rootHelpText).toContain("Usage:");
    expect(written.rootHelpText).toContain("daisyclaw");
    expect(written.subcommandHelpText.doctor).toContain("daisyclaw doctor");
    expect(written.subcommandHelpText.gateway).toContain("daisyclaw gateway");
    expect(written.subcommandHelpText.models).toContain("daisyclaw models");
    expect(written.subcommandHelpText.plugins).toContain("daisyclaw plugins");
  });

  it("renders independent startup help snapshots concurrently", async () => {
    const tempRoot = createTempDir("daisyclaw-startup-metadata-concurrency-");
    const distDir = path.join(tempRoot, "dist");
    const extensionsDir = path.join(tempRoot, "extensions");
    const outputPath = path.join(distDir, "cli-startup-metadata.json");
    ...
    const writePromise = writeCliStartupMetadata({
      ...
      renderBundledRootHelpText: async () => "Usage: daisyclaw\n",
      renderSourceBrowserHelpText: renderAfterUnblock("browser", "Usage: daisyclaw browser\n"),
      renderSourceSecretsHelpText: renderAfterUnblock("secrets", "Usage: daisyclaw secrets\n"),
      renderSourceNodesHelpText: renderAfterUnblock("nodes", "Usage: daisyclaw nodes\n"),
  ...
  it("regenerates nodes help when bundled canvas CLI help sources change", async () => {
    const tempRoot = createTempDir("daisyclaw-startup-metadata-signature-");
    const distDir = path.join(tempRoot, "dist");
    const extensionsDir = path.join(tempRoot, "extensions");
    const outputPath = path.join(distDir, "cli-startup-metadata.json");
    ...
    const writeMetadata = async (): Promise<void> => {
      await writeCliStartupMetadata({
        ...
        renderSourceNodesHelpText: () => {
          nodesRenderCount += 1;
          return `Usage: daisyclaw nodes ${nodesRenderCount}\n`;
        },
        renderSourceSubcommandHelpTextRecord: () => ({
          doctor: "Usage: daisyclaw doctor\n",
          gateway: "Usage: daisyclaw gateway\n",
    ...
    expect(written.nodesHelpText).toContain("daisyclaw nodes 3");
  });
});
