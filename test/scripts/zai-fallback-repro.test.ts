...
describe("zai fallback repro command resolution", () => {
  it("wraps Windows pnpm.cmd without Node shell argv", () => {
    expect(
      resolveZaiFallbackPnpmCommand(["daisyclaw", "agent", "--message", "hello world"], {
      ...
      args: [
        ...
        "/s",
        "/c",
        String.raw`""C:\Program Files\nodejs\pnpm.cmd" daisyclaw agent --message "hello world""`,
      ],
  ...
  it("scans session transcripts with a byte cap", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-zai-session-test-"));
    try {
  ...
  it("cleans temporary repro state after fallback proof", async () => {
    ...
    const exitCode = await runZaiFallbackRepro({
      env: {
        ...
        DAISYCLAW_ZAI_FALLBACK_SESSION_ID: "session-test",
      ...
      log: () => {},
      mkdtemp: async () => {
        const root = await fs.mkdtemp(path.join(os.tmpdir(), "daisyclaw-zai-fallback-test-"));
        tempRoots.push(root);
        return root;
      },
      ...
      runCommand: async (label, _args, env) => {
        ...
        if (label === "run1") {
          const sessionFile = path.join(
            String(env.DAISYCLAW_STATE_DIR),
            "agents",
            "main",
            "sessions",
            "session-test.jsonl",
