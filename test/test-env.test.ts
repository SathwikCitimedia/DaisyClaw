...
function createTempHome(): string {
  return makeTempDir(tempDirs, "daisyclaw-test-env-real-home-");
}
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-daisyclaw.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.daisyclaw/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.daisyclaw/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".daisyclaw", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".daisyclaw", "external-plugins", "glueclaw", "daisyclaw.plugin.json"),
      '{"id":"glueclaw"}\n',
    );
    writeFile(
      path.join(realHome, ".daisyclaw", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.DAISYCLAW_LIVE_TEST_QUIET = "1";
    process.env.DAISYCLAW_CONFIG_PATH = "~/custom-daisyclaw.json5";
    process.env.DAISYCLAW_TEST_HOME = priorIsolatedHome;
    process.env.DAISYCLAW_STATE_DIR = path.join(priorIsolatedHome, ".daisyclaw");
    ...
    expect(process.env.DAISYCLAW_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".daisyclaw", "daisyclaw.json");
    ...
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".daisyclaw", "credentials", "token.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(
    ...
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".daisyclaw", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.DAISYCLAW_LIVE_TEST = "1";
    process.env.DAISYCLAW_LIVE_USE_REAL_HOME = "1";
    process.env.DAISYCLAW_LIVE_TEST_QUIET = "1";
  ...
  it("does not load ~/.profile for normal isolated test runs", () => {
    ...
    delete process.env.DAISYCLAW_LIVE_TEST;
    delete process.env.DAISYCLAW_LIVE_GATEWAY;
    delete process.env.DAISYCLAW_LIVE_USE_REAL_HOME;
    delete process.env.DAISYCLAW_LIVE_TEST_QUIET;
