...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.DAISYCLAW_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "DAISYCLAW_TEST_FAST", value: process.env.DAISYCLAW_TEST_FAST },
    {
      key: "DAISYCLAW_STRICT_FAST_REPLY_CONFIG",
      value: process.env.DAISYCLAW_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "DAISYCLAW_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.DAISYCLAW_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "DAISYCLAW_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.DAISYCLAW_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "DAISYCLAW_STATE_DIR", value: process.env.DAISYCLAW_STATE_DIR },
    { key: "DAISYCLAW_CONFIG_PATH", value: process.env.DAISYCLAW_CONFIG_PATH },
    { key: "DAISYCLAW_GATEWAY_PORT", value: process.env.DAISYCLAW_GATEWAY_PORT },
    { key: "DAISYCLAW_BRIDGE_ENABLED", value: process.env.DAISYCLAW_BRIDGE_ENABLED },
    { key: "DAISYCLAW_BRIDGE_HOST", value: process.env.DAISYCLAW_BRIDGE_HOST },
    { key: "DAISYCLAW_BRIDGE_PORT", value: process.env.DAISYCLAW_BRIDGE_PORT },
    { key: "DAISYCLAW_CANVAS_HOST_PORT", value: process.env.DAISYCLAW_CANVAS_HOST_PORT },
    { key: "DAISYCLAW_TEST_HOME", value: process.env.DAISYCLAW_TEST_HOME },
    { key: "DAISYCLAW_AGENT_DIR", value: process.env.DAISYCLAW_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "daisyclaw-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.DAISYCLAW_TEST_HOME = tempHome;
  process.env.DAISYCLAW_TEST_FAST = "1";
  process.env.DAISYCLAW_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.DAISYCLAW_ALLOW_SLOW_REPLY_TESTS;
  ...
  delete process.env.DAISYCLAW_BRIDGE_ENABLED;
  delete process.env.DAISYCLAW_BRIDGE_HOST;
  delete process.env.DAISYCLAW_BRIDGE_PORT;
  delete process.env.DAISYCLAW_CANVAS_HOST_PORT;
  ...
  if (process.platform === "win32") {
    process.env.DAISYCLAW_STATE_DIR = path.join(tempHome, ".daisyclaw");
  }
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.DAISYCLAW_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.DAISYCLAW_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".daisyclaw");
  const priorIsolatedHome = params.env.DAISYCLAW_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".daisyclaw")
  ) {
    realStateDir = path.join(params.realHome, ".daisyclaw");
  }
  const tempStateDir = path.join(params.tempHome, ".daisyclaw");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.DAISYCLAW_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.DAISYCLAW_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "daisyclaw.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "daisyclaw.json"),
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    process.env.LIVE === "1" ||
    process.env.DAISYCLAW_LIVE_TEST === "1" ||
    process.env.DAISYCLAW_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.DAISYCLAW_LIVE_USE_REAL_HOME);
