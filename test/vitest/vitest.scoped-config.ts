...
export function createScopedVitestConfig(
  ...
  options?: {
    ...
    exclude?: string[];
    argv?: string[];
    includeDaisyClawRuntimeSetup?: boolean;
    isolate?: boolean;
    name?: string;
  ...
  const includeFromEnv = loadPatternListFromEnv("DAISYCLAW_VITEST_INCLUDE_FILE", env);
  ...
  const setupFiles = [
    ...new Set([
      ...(baseTest.setupFiles ?? []),
      ...(options?.setupFiles ?? []),
      ...(options?.includeDaisyClawRuntimeSetup === false ? [] : ["test/setup-daisyclaw-runtime.ts"]),
    ]),
