...
const RUN_NODE_SKIP_DTS_BUILD = process.env.DAISYCLAW_RUN_NODE_SKIP_DTS_BUILD === "1";
...
function buildInputOptions(options: InputOptionsArg): InputOptionsReturn {
  if (process.env.DAISYCLAW_BUILD_VERBOSE === "1") {
    return undefined;
  }
...
const shouldBuildPrivateQaEntries = process.env.DAISYCLAW_BUILD_PRIVATE_QA === "1";
...
function shouldAlwaysBundleDependency(id: string): boolean {
  return (
    ...
    id === "@daisyclaw/normalization-core" ||
    id.startsWith("@daisyclaw/normalization-core/") ||
    id === "@daisyclaw/media-core" ||
    id.startsWith("@daisyclaw/media-core/") ||
    id === "@daisyclaw/acp-core" ||
    id.startsWith("@daisyclaw/acp-core/") ||
    id === "zod" ||
    id.startsWith("zod/")
  );
}
...
function shouldExternalizeAgentCoreDependency(id: string): boolean {
  return (
    id === "@daisyclaw/llm-core" ||
    id.startsWith("@daisyclaw/llm-core/") ||
    id === "ignore" ||
    id === "daisyclaw" ||
    id.startsWith("daisyclaw/") ||
    id === "typebox" ||
...
function shouldExternalizeGatewayClientDependency(id: string): boolean {
  return (
    id === "ws" ||
    id.startsWith("ws/") ||
    id === "@daisyclaw/gateway-protocol" ||
    id.startsWith("@daisyclaw/gateway-protocol/")
  );
}
...
function shouldExternalizeSpeechCoreDependency(id: string): boolean {
  return id === "daisyclaw" || id.startsWith("daisyclaw/");
}
...
function shouldExternalizeLlmRuntimeDependency(id: string): boolean {
  return id === "@daisyclaw/llm-core" || id.startsWith("@daisyclaw/llm-core/");
}
