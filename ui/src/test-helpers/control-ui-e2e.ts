...
export async function startControlUiE2eServer(): Promise<ControlUiE2eServer> {
  ...
  const server = await createServer({
    ...
    configFile: false,
    define: {
      DAISYCLAW_CONTROL_UI_BUILD_ID: JSON.stringify("e2e"),
    },
    logLevel: "error",
    optimizeDeps: {
      include: [
...
function normalizeScenario(
  ...
  return {
    ...
    assistantName: scenario.assistantName?.trim() || "DaisyClaw",
    defaultAgentId,
...
function installControlUiMockGateway(input: {
  ...
  type WindowWithGateway = Window & {
    daisyclawControlUiE2eGateway?: ExposedGateway;
  };
  ...
  (window as WindowWithGateway).daisyclawControlUiE2eGateway = exposed;
