...
import type { DaisyClawModalDialog } from "../ui/components/modal-dialog.ts";
...
export async function getRenderedModalDialog(container: HTMLElement) {
  const modal = container.querySelector<DaisyClawModalDialog>("daisyclaw-modal-dialog");
  expect(modal).toBeInstanceOf(HTMLElement);
  if (!modal) {
    throw new Error("Expected daisyclaw-modal-dialog");
  }
