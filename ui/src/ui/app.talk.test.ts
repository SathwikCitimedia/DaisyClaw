...
describe("DaisyClawApp Talk controls", () => {
  beforeEach(() => {
    vi.resetModules();
  ...
  it("retries Talk immediately when the previous session is already in error state", async () => {
    const { DaisyClawApp } = await import("./app.ts");
    const app = Object.create(DaisyClawApp.prototype) as {
      client: unknown;
      connected: boolean;
    ...
    await DaisyClawApp.prototype.toggleRealtimeTalk.call(app as never);
  ...
  it("keeps the Talk options toggle inside the open-panel click guard", async () => {
    await import("./app.ts");
    const app = document.createElement("daisyclaw-app");
