...
declare global {
  interface Window {
    __DAISYCLAW_CONTROL_UI_BASE_PATH__?: string;
  }
}
...
export class DaisyClawApp extends LitElement {
  ...
  @state() clawhubAmbiguousMatches: Array<{
    ownerHandle: string;
    slug: string;
    ref: string;
    url: string;
  ...
  async toggleRealtimeTalk() {
    ...
    const session = new RealtimeTalkSession(
      ...
      this.sessionKey,
      {
        ...
        onTranscript: (entry) => {
          this.realtimeTalkTranscript = `${entry.role === "user" ? "You" : "DaisyClaw"}: ${entry.text}`;
...
if (!customElements.get("daisyclaw-app")) {
  customElements.define("daisyclaw-app", DaisyClawApp);
}
