...
describe("resolveCanvasIframeUrl", () => {
  it("allows same-origin hosted canvas document paths", () => {
    expect(resolveCanvasIframeUrl("/__daisyclaw__/canvas/documents/cv_demo/index.html")).toBe(
      "/__daisyclaw__/canvas/documents/cv_demo/index.html",
    );
  });
  ...
  it("rewrites safe canvas paths through the scoped canvas host", () => {
    expect(
      resolveCanvasIframeUrl(
        "/__daisyclaw__/canvas/documents/cv_demo/index.html",
        "http://127.0.0.1:19003/__daisyclaw__/cap/cap_123",
      ),
    ).toBe(
      "http://127.0.0.1:19003/__daisyclaw__/cap/cap_123/__daisyclaw__/canvas/documents/cv_demo/index.html",
    );
  });
