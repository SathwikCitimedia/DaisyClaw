...
const SKIP_DELETE_CONFIRM_KEY = "daisyclaw:skipDeleteConfirm";
...
function isLocalAssistantAttachmentSource(source: string): boolean {
  ...
  if (/^\/(?:__daisyclaw__|media|api\/chat\/media\/outgoing)\//.test(trimmed)) {
    return false;
  }
  return (
...
function buildAssistantAttachmentUrl(
  ...
  return `${normalizedBasePath}/__daisyclaw__/assistant-media?${params.toString()}`;
}
  ...
  if (!pending) {
    pending = (async () => {
      ...
      if (requesterSessionKey) {
        headers.set("x-daisyclaw-requester-session-key", requesterSessionKey);
      }
...
function renderGroupedMessage(
  ...
  const transcriptMeta =
    m["__daisyclaw"] && typeof m["__daisyclaw"] === "object" && !Array.isArray(m["__daisyclaw"])
      ? (m["__daisyclaw"] as Record<string, unknown>)
      : null;
