...
import type { DaisyClawFilePreviewModal } from "./file-preview-modal.ts";
...
async function renderPreview(query = "") {
  ...
  const modal = container.querySelector<DaisyClawFilePreviewModal>("daisyclaw-file-preview-modal");
...
function shadowText(modal: DaisyClawFilePreviewModal): string {
  return modal.shadowRoot?.textContent ?? "";
}

describe("daisyclaw-file-preview-modal", () => {
  beforeEach(() => {
