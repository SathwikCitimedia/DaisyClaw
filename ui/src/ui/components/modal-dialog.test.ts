...
import type { DaisyClawModalDialog } from "./modal-dialog.ts";
...
function expectShadowElement(modal: DaisyClawModalDialog, id: string): HTMLElement {
...
describe("daisyclaw-modal-dialog", () => {
  beforeEach(() => {
  ...
  it("opens a labelled modal dialog with an optional description", async () => {
    ...
    expect(labelId).toBe("daisyclaw-modal-dialog-label");
    expect(descriptionId).toBe("daisyclaw-modal-dialog-description");
    expect(dialog.getRootNode()).toBe(modal.shadowRoot);
    expect(dialog.ownerDocument.querySelector(`#${labelId}`)).toBeNull();
    expect(expectShadowElement(modal, "daisyclaw-modal-dialog-label").textContent).toBe(
      "Confirm action",
    );
    expect(expectShadowElement(modal, "daisyclaw-modal-dialog-description").textContent).toBe(
