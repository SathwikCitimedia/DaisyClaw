...
describe("Control UI Vite config", () => {
  it("resolves root tsconfig package aliases for source imports", () => {
    expect(findStringAlias("@daisyclaw/net-policy/ip")?.replacement).toBe(
  ...
  it("resolves Control UI dev-server source aliases for internal packages", () => {
    ...
    expect(
      aliases.find((alias) => alias.find === "@daisyclaw/normalization-core/string-coerce"),
    )?.toEqual({
  ...
  it("keeps specific tsconfig aliases ahead of broad package aliases", () => {
    ...
    const netPolicyIpIndex = aliases.findIndex((alias) => alias.find === "@daisyclaw/net-policy/ip");
    const netPolicyPackageIndex = aliases.findIndex(
      (alias) => alias.find === "@daisyclaw/net-policy",
    );
    ...
    const broadDaisyClawWildcardIndex = aliases.findIndex(
    ...
    expect(broadDaisyClawWildcardIndex).toBeGreaterThanOrEqual(0);
    expect(netPolicyIpIndex).toBeLessThan(netPolicyPackageIndex);
    expect(netPolicyWildcardIndex).toBeLessThan(broadDaisyClawWildcardIndex);
  });
