  ...
  if (typeof window === "undefined") {
    vi.stubGlobal(
      ...
      value == null
        ...
        : ({ __DAISYCLAW_CONTROL_UI_BASE_PATH__: value } as Window & typeof globalThis),
    );
    return;
  }
  if (value == null) {
...
describe("loadSettings default gateway URL derivation", () => {
  ...
  it("uses configured base path and normalizes trailing slash", () => {
    ...
    setControlUiBasePath(" /daisyclaw/ ");

    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/daisyclaw"));
  });
  ...
  it("infers base path from nested pathname when configured base path is not set", () => {
    setTestLocation({
      ...
      pathname: "/apps/daisyclaw/chat",
    });

    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/apps/daisyclaw"));
  });
  ...
  it("ignores and scrubs legacy persisted tokens", () => {
    ...
    sessionStorage.setItem("daisyclaw.control.token.v1", "legacy-session-token");
    localStorage.setItem(
      "daisyclaw.control.settings.v1",
      JSON.stringify({
        gatewayUrl: "wss://gateway.example:8443/daisyclaw",
    ...
    const settings = loadSettings();
    expect(settings.gatewayUrl).toBe("wss://gateway.example:8443/daisyclaw");
    expect(settings.token).toBe("");
    expect(settings.sessionKey).toBe("agent");
    const scopedKey = "daisyclaw.control.settings.v1:wss://gateway.example:8443/daisyclaw";
    expect(JSON.parse(localStorage.getItem(scopedKey) ?? "{}")).toEqual({
      ...
      textScale: 100,
      sessionsByGateway: {
        "wss://gateway.example:8443/daisyclaw": {
          sessionKey: "agent",
  ...
  it("does not persist gateway tokens when saving settings", () => {
    ...
    const scopedKey = `daisyclaw.control.settings.v1:${gwUrl}`;
    expect(JSON.parse(localStorage.getItem(scopedKey) ?? "{}")).toEqual({
      gatewayUrl: gwUrl,
      theme: "claw",
      themeMode: "system",
  ...
  it("normalizes persisted text scale to the nearest supported stop", () => {
    ...
    const gwUrl = expectedGatewayUrl("");
    localStorage.setItem(
      `daisyclaw.control.settings.v1:${gwUrl}`,
      JSON.stringify({
        gatewayUrl: gwUrl,
        textScale: 123,
      }),
  ...
  it("caps persisted session scopes to the most recent gateways", () => {
    ...
    const gwUrl = expectedGatewayUrl("");
    const scopedKey = `daisyclaw.control.settings.v1:wss://gateway.example:8443`;
  ...
  it("persists local user identity separately from gateway settings", () => {
    ...
    expect(JSON.parse(localStorage.getItem("daisyclaw.control.user.v1") ?? "{}")).toEqual({
      name: "Buns",
      avatar: "🦞",
    });
  });
  ...
  it("removes the persisted local user identity when cleared", () => {
    ...
    expect(localStorage.getItem("daisyclaw.control.user.v1")).toBeNull();
  });
});
