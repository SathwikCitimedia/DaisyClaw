...
const SETTINGS_KEY_PREFIX = "daisyclaw.control.settings.v1:";
const LEGACY_SETTINGS_KEY = "daisyclaw.control.settings.v1";
const LOCAL_USER_IDENTITY_KEY = "daisyclaw.control.user.v1";
const LOCAL_ASSISTANT_IDENTITY_KEY = "daisyclaw.control.assistant.v1";
const LEGACY_TOKEN_SESSION_KEY = "daisyclaw.control.token.v1";
const TOKEN_SESSION_KEY_PREFIX = "daisyclaw.control.token.v1:";
...
function deriveDefaultGatewayUrl(): { pageUrl: string; effectiveUrl: string } {
  ...
  const configured =
    ...
    normalizeOptionalString(window["__DAISYCLAW_CONTROL_UI_BASE_PATH__"]);
...
function persistSettings(next: UiSettings) {
  ...
  try {
    ...
    const raw =
      storage?.getItem(scopedKey) ??
      storage?.getItem(SETTINGS_KEY_PREFIX + "default") ??
      storage?.getItem("daisyclaw.control.settings.v1");
    if (raw) {
      const parsed = JSON.parse(raw) as PersistedUiSettings;
