...
import "../app.ts";
import type { DaisyClawApp } from "../app.ts";
...
const mountedApps = new Set<DaisyClawApp>();

function collectMountedApps() {
  return new Set<DaisyClawApp>([
    ...mountedApps,
    ...document.querySelectorAll<DaisyClawApp>("daisyclaw-app"),
  ]);
}
...
async function waitForAppUpdates(apps: Iterable<DaisyClawApp>) {
  for (const app of apps) {
    await app.updateComplete;
  }
}

async function drainAppWork(apps: Iterable<DaisyClawApp>) {
  const snapshot = [...apps];
  await nextMicrotask();
...
export function mountApp(pathname: string) {
  ...
  const app = document.createElement("daisyclaw-app") as DaisyClawApp;
  mountedApps.add(app);
...
export function registerAppMountHooks() {
  beforeEach(async () => {
    ...
    window["__DAISYCLAW_CONTROL_UI_BASE_PATH__"] = undefined;
