...
function resolveControlUiBuildId(): string {
  const explicit =
    process.env.DAISYCLAW_CONTROL_UI_BUILD_ID?.trim() || process.env.DAISYCLAW_VERSION?.trim();
  if (explicit) {
...
function controlUiServiceWorkerBuildIdPlugin(buildId: string): Plugin {
  return {
    ...
    closeBundle() {
      ...
      const placeholder = '"__DAISYCLAW_CONTROL_UI_BUILD_ID__"';
...
export default function controlUiViteConfig(): UserConfig {
  const envBase = process.env.DAISYCLAW_CONTROL_UI_BASE_PATH?.trim();
  ...
  return {
    base,
    define: {
      DAISYCLAW_CONTROL_UI_BUILD_ID: JSON.stringify(controlUiBuildId),
    },
